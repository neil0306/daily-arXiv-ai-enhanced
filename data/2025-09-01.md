...
Motivation: Deep learning models often learn spurious correlations from training data, leading to poor generalization and performance degradation on unseen data due to biases like gender and simplicity bias.

Method: CoBA framework operates at semantic triple level: decomposing text into subject-predicate-object triples, selectively modifying these triples to disrupt spurious correlations, then reconstructing text to generate counterbias data.
...
Conclusion: CoBA offers a versatile and robust solution to address challenges posed by spurious correlations through unified counterbias data augmentation.
...
<details>
  <summary>Details</summary>
Motivation: Address the lack of demographic context in existing toxic speech datasets to better understand how different age groups communicate toxic content online.

Method: Collaborated with German public service network funk to collect and annotate 33,048 comments using predefined toxic keywords. Combined human annotation (3,024 comments) with LLM annotation (30,024 comments) to identify toxicity categories including insults, disinformation, and criticism.

Result: Dataset contains 16.7% problematic content. Revealed age-based differences: younger users favor expressive language while older users more often engage in disinformation and devaluation.
...
<details>
  <summary>Details</summary>
Motivation: To address enterprise needs for dense retrieval with improved context length, speed, and accuracy across diverse domains while maintaining enterprise-appropriate data governance.

Method: Developed both bi-encoder and cross-encoder architectures including 22-layer retriever and 12-layer efficient models, plus reranker model, trained exclusively on enterprise-appropriate data with governance oversight.

Result: Achieved state-of-the-art performance across text, code, long-document search, conversational, and tabular data retrieval, with 19-44% speed advantages over leading competitors while maintaining superior accuracy.

Conclusion: Granite R2 models set new standards for open-source embedding models, offering cutting-edge performance, enterprise licensing, and transparent data provenance for mission-critical deployments under Apache 2.0 license.
...
Method: Transformer-based model with scaled positional embeddings and Gaussian memory mask in cross-attention module, plus comprehensive subjective and objective evaluation pipelines.
...
<details>
  <summary>Details</summary>
Motivation: To investigate cognitive biases in LLMs, specifically the anchoring effect in price negotiations, and understand how factors like reasoning and personality influence this bias.

Method: Instructed seller LLM agents to apply anchoring effect in price negotiations and evaluated using both objective and subjective metrics. Examined relationship with reasoning models and personality traits.

Result: LLMs are influenced by anchoring effect like humans. Reasoning models with longer chain of thought show reduced susceptibility to anchoring. No significant correlation found between personality traits and anchoring susceptibility.

Conclusion: Findings enhance understanding of cognitive biases in LLMs and support development of safer, more responsible LLM applications in society.
...
Result: Significant performance drop with increasing problem complexity across all categories. Models show similar accuracy trends, with some cognitive skills proving more difficult than others.

Conclusion: MLLMs struggle with basic visual perception tasks, revealing important limitations despite their strong performance on complex reasoning problems.
...
<details>
  <summary>Details</summary>
Motivation: Sci-LLMs are transforming scientific research but face unique challenges due to the complex nature of scientific data - multimodal, cross-scale, domain-specific, and uncertainty-laden corpora that require specialized approaches different from general NLP.

Method: The survey employs a data-centric synthesis approach, creating unified taxonomies of scientific data and hierarchical knowledge models, systematically reviewing recent Sci-LLMs across disciplines, and analyzing over 270 training datasets and 190 benchmark datasets.

Result: The analysis reveals Sci-LLMs require representations that preserve domain invariance and enable cross-modal reasoning, and identifies a shift from static evaluations toward process- and discovery-oriented assessments with advanced evaluation protocols.

Conclusion: The work outlines a paradigm shift toward closed-loop systems where autonomous Sci-LLM agents actively experiment and contribute to evolving knowledge bases, providing a roadmap for building trustworthy AI systems that accelerate scientific discovery.
...
<details>
  <summary>Details</summary>
Motivation: To examine bias in self- and cross-model evaluations by large language models when influenced by identity labels, as LLMs are increasingly used for evaluation tasks.

Method: Evaluated blog posts from ChatGPT, Gemini, and Claude under four label conditions (no labels, true labels, two false labels) using preference voting and quality ratings for Coherence, Informativeness, and Conciseness, with scores converted to percentages.

Result: Striking asymmetries: Claude labels consistently boosted scores while Gemini labels depressed them. False labels frequently reversed rankings with up to 50 percentage point shifts in preference votes and 12 percentage point shifts in quality ratings.

Conclusion: Perceived model identity heavily distorts LLM judgments, requiring blind or multimodel evaluation protocols to ensure fairness in benchmarking.
...
<details>
  <summary>Details</summary>
Motivation: To enhance LLMs' capability to intelligently and adaptively gather information from users or external sources in multi-turn conversations, moving beyond simple prompting strategies.

Method: Uses sequential Bayesian experimental design framework with probabilistic model derived from LLM's belief distribution, includes EIG estimator, non-contextual conditioning, and targeted query proposal strategy.
...
Result: 350% increase in exact match accuracy (0.0400 to 0.1800) and improved partial match accuracy of 0.8800, outperforming GPT-5-mini and Gemini-2.5-fiash on key metrics.
...
Method: Propose a pixel-based generative language model that renders words as individual images instead of using text-based embeddings, providing robustness to noisy inputs and multilingual compatibility.

Result: Evaluated on multilingual LAMBADA, WMT24 dataset and SST-2 benchmark, demonstrating resilience to orthographic noise and effectiveness in multilingual settings.

Conclusion: Pixel-based representations offer stronger robustness against orthographic attacks and better multilingual text handling compared to traditional text-based embeddings.
...
Result: S3Ms do not exhibit clear evidence of Critical Period effects. Models with delayed L2 exposure onset tend to perform better on L2, and delayed L1 exposure offset leads to L1 forgetting (contrary to human CP effects).

Conclusion: Self-supervised speech models do not replicate human-like Critical Period effects in phonological acquisition, showing different patterns of language learning and retention compared to humans.
...
<details>
  <summary>Details</summary>
Motivation: Existing self-consistency methods for hallucination detection in LLMs suffer from high computational costs due to repeated generation, while current detection methods perform poorly at sentence-level or require domain-specific knowledge.

Method: Proposes Decoding Memory Pipeline (DMP) that identifies shared prefix tokens across generations and uses selective inference with annealed decoding to accelerate multi-response generation by skipping redundant computations.

Result: Extensive experiments show DMP achieves up to 3x speedup in generation efficiency without sacrificing AUROC performance, making it orthogonal to model, dataset, and decoding strategy.

Conclusion: DMP provides an effective solution to reduce computational overhead in self-consistency methods while maintaining detection performance, with potential applications in alignment and reasoning tasks.
...
<details>
  <summary>Details</summary>
Motivation: To create an efficient code embedding model that can retrieve code from natural language queries, perform technical QA, and identify semantically similar code snippets across different programming languages.

Method: Uses an autoregressive backbone pre-trained on both text and code, generating embeddings via last-token pooling technique with a specialized training recipe.

Result: Achieves state-of-the-art performance despite relatively small model size, demonstrating the effectiveness of this approach to code embedding model construction.

Conclusion: The approach validates that using autoregressive backbones with last-token pooling can create highly effective code embedding models that perform well on various code-related tasks across multiple programming languages.
...
Method: Updated BLUEX dataset with 2024-2025 exams and automatically generated image captions using state-of-the-art models, evaluating commercial and open-source LLMs' ability to leverage visual context.

Result: Captioning strategies increased accessibility to text-only models by more than 40%, producing 1,422 usable questions (more than double the original BLUEX).

Conclusion: The enhanced BLUEX dataset provides improved evaluation capabilities for multilingual LLM assessment and data contamination studies through automated image captioning.
...
<details>
  <summary>Details</summary>
Motivation: LLMs are transforming AI but development/deployment remains complex. Need to understand current capabilities, limitations, and best practices for researchers, developers, and decision-makers.

Method: Comparative analysis of 16 LLM challenges and examination of how two state-of-the-art models (OpenAI's GPT-4o and DeepSeek-V3-0324) address these challenges through different approaches.
...
Conclusion: Provides guidance on LLM selection based on specific needs, highlighting that both closed and open source approaches have distinct advantages depending on the application domain and requirements.
...
Method: Conceptual analysis of the Turing test using statistical interpretations of normality, examining both the target intelligence (normal/average human) and the judging mechanism (statistical aggregation of multiple judges).

Result: The paper concludes that: 1) Large language models like ChatGPT are unlikely to pass the true Turing test as they target exceptional rather than normal human intelligence, representing artificial smartness not AI; 2) The fundamental question is whether human cognition can be reduced to average/normal mind.

Conclusion: The Turing test is fundamentally a test of normal intelligence assessed through statistical aggregation of judgments, raising deeper questions about the reducibility of human cognition to average/normal patterns and challenging the normalist paradigm underlying the test.
...
<details>
  <summary>Details</summary>
Motivation: To address reproducibility challenges in automatic text summarization evaluation and investigate the gap between reported metric performances in literature versus experimental results.
...
Result: Found significant performance discrepancies between literature reports and experimental results, revealing a structural trade-off: metrics with highest human judgment alignment are computationally intensive and less stable across runs. LLM-based metrics show randomness, technical dependencies, and limited reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: To understand the capabilities and limitations of Large Language Models as automatic review generators, particularly in detecting faulty research logic which is crucial for scholarly peer review integrity.

Method: Developed a fully automated counterfactual evaluation framework that isolates and tests the ability to detect research logic flaws under controlled conditions, testing various ARG approaches.

Result: Contrary to expectations, flaws in research logic had no significant effect on the output reviews generated by automatic review generators.

Conclusion: Current ARG approaches lack the ability to detect faulty research logic, necessitating three actionable recommendations for future work and public release of the evaluation framework and dataset.
...
Method: Created a multimodal dataset with 1,026 expert-annotated cases through a rigorous three-step annotation process, evaluating 32 state-of-the-art MLLMs across six clinically critical dimensions.

Result: Evaluation revealed substantial challenges in aligning model outputs with expert judgment, while baseline models showed significant performance improvements through fine-tuning.

Conclusion: Med-RewardBench addresses the critical gap in medical reward model evaluation and demonstrates the need for specialized benchmarks to ensure clinically reliable MLLM performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing semantic dimension approaches use broad predefined categories that miss finer conceptual distinctions, limiting understanding of how meaning is organized in language and the brain.

Method: Introduces Disentangled Continuous Semantic Representation Model (DCSRM) to decompose LLM word embeddings into sub-embeddings encoding specific semantics, then uses voxel-wise encoding models to map subdimensions to brain activation.

Result: Identifies interpretable semantic subdimensions with neural correlates, showing semantic dimensions are structured by distinct principles with polarity as key decomposition factor.

Conclusion: Provides fine-grained interpretable semantic subdimensions that are cognitively and neuroscientifically plausible, advancing understanding of conceptual meaning organization.
...
<details>
  <summary>Details</summary>
Motivation: To understand the stability and coherence of ideological positions in LLMs beyond surface-level responses that can be manipulated through prompt engineering, and to investigate whether they reflect robust internal political representations.

Method: Dual approach: 1) Measuring steerability using instruction prompting and activation steering to assess how easily models switch between liberal/conservative viewpoints; 2) Probing internal mechanisms using Sparse Autoencoders (SAEs) to analyze ideological features and targeted ablation of core political features.

Result: Models with lower steerability possess more distinct and abstract ideological features (one model had 7.3x more political features than another similar-sized model). Targeted ablation in ideologically deep models leads to consistent logical shifts in reasoning, while shallow models show increased refusal outputs.
...
<details>
  <summary>Details</summary>
Motivation: Large Language Models have impressive creative capabilities but are computationally expensive. Small Language Models offer an alternative but current methods like SFT struggle with novelty and RLHF is costly. The paper aims to develop more efficient AI-driven approaches to enhance creative writing in SLMs.

Method: Two RLAIF strategies: 1) Reward Model trained on high-quality preference data from multi-agent rejection sampling framework, 2) Principle-guided LLM-as-a-Judge with adversarial training and reflection mechanism to provide direct reward signals. Both applied to 7B-parameter SLM for Chinese greeting generation.

Result: Both approaches significantly enhance creative output over baselines. The principle-guided LLM-as-a-Judge yields superior generation quality, offers better training efficiency, and reduces dependency on human-annotated data. Automated evaluation methods show strong alignment with human judgments.

Conclusion: The principle-guided LLM-as-a-Judge approach provides a more scalable and effective path for creative SLMs, demonstrating advantages in quality, efficiency, and reduced human annotation requirements while maintaining strong alignment with human evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Psychological biases make people vulnerable to fake news on social media. Ensemble methods are effective but depend on classifier diversity, which is challenging to achieve as models often learn redundant patterns.

Method: Proposes HierarchySelect approach that computes pairwise diversity between classifiers, applies hierarchical clustering to group them at different granularity levels, selects diverse pools per level, and incorporates performance metrics to ensure generalization.

Result: Achieved highest accuracy on 2 out of 6 datasets when compared against Elbow heuristic and state-of-the-art baselines using 40 heterogeneous classifiers across diverse application domains.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of high-quality human-annotated sentence similarity datasets and optimized models for Marathi language, which is considered a low-resource language in NLP.

Method: Created a uniformly distributed dataset across 0-5 similarity score buckets, fine-tuned MahaSBERT model on this dataset, and benchmarked against other models like MahaBERT, MuRIL, IndicBERT, and IndicSBERT.

Result: MahaSTS enables effective training for sentence similarity tasks in Marathi, demonstrating the importance of human-curated annotations and structured supervision in low-resource settings.

Conclusion: The combination of human-annotated balanced dataset and targeted fine-tuning produces state-of-the-art results for Marathi sentence similarity, with both dataset and model made publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Protecting sensitive personal information in textual data while maintaining data usability for downstream tasks, driven by privacy regulations and the need for robust anonymization methods.

Method: Survey methodology examining foundational NER approaches, LLM-based anonymization/de-anonymization, domain-specific solutions (healthcare, law, finance, education), formal privacy models, risk-aware frameworks, and evaluation toolkits.

Result: Consolidated current knowledge on text anonymization, identified emerging trends including LLM capabilities impact, quasi-identifier challenges, and evolving privacy-utility trade-offs across various domains.

Conclusion: Provides comprehensive guidance for future research directions in text anonymization, addressing both academic and practical needs while highlighting persistent challenges and the dual role of LLMs in privacy protection and threats.
...
<details>
  <summary>Details</summary>
Motivation: Existing data selection and synthesis approaches are limited by static dataset curation that fails to adapt to evolving model capabilities, creating a need for dynamic data optimization that evolves with the model.

Method: A closed-loop optimization system with: (1) self-referential diagnostic module using tri-axial model signals (loss patterns, embedding cluster dynamics, self-alignment scores), (2) adaptive optimization engine that transforms suboptimal samples while preserving semantic integrity, and (3) dynamic learning principles for continuous evolution.
...
Method: Study with 32 participants across four Keirsey personality types, evaluating interactions with GPT-4 and Claude 3.5 across four collaborative tasks: data analysis, creative writing, information retrieval, and writing assistance.

Result: Significant personality-driven preferences: Rationals strongly preferred GPT-4 (goal-oriented tasks), Idealists favored Claude 3.5 (creative/analytical tasks). Other types showed task-dependent preferences. Sentiment analysis confirmed patterns.
...
<details>
  <summary>Details</summary>
Motivation: To develop a general-purpose contextual text embedding model with superior text representation capabilities by leveraging diverse training data and advanced training methodologies to advance retrieval model performance.

Method: Built on Qwen2.5-7B-Instruct foundation model with unified multi-task framework, specialized data transformation, LLM API-based data synthesis (paraphrasing, augmentation, hard negative generation), and two-stage training (retrieval-focused pretraining followed by full-task fine-tuning).
...
Conclusion: Higher-quality, more diverse data is crucial for retrieval model advancement, and leveraging LLMs' generative capabilities can optimize data quality for embedding model breakthroughs. Model weights are released under Apache 2.0 license with evaluation code provided.
...
<details>
  <summary>Details</summary>
Motivation: Misleading visualizations drive misinformation on social media, but AI model training has been limited by lack of large, diverse datasets for detecting visualization deception.
...
Result: The task of automatically detecting misleading visualizations remains highly challenging despite comprehensive evaluation across multiple AI approaches.
...
<details>
  <summary>Details</summary>
Motivation: Supervised fine-tuning suffers from the 'seesaw phenomenon' where performance improvements on some tasks come at the expense of others due to indiscriminate parameter updates.

Method: Core Parameter Isolation Fine-Tuning: 1) Independently fine-tune on each task to identify core parameter regions, 2) Group tasks with similar core regions, 3) Use parameter fusion - transplant core parameters directly and integrate non-core parameters via Spherical Linear Interpolation (SLERP), 4) Lightweight pipelined SFT training while freezing core regions.

Result: Extensive experiments on multiple public benchmarks show significant alleviation of task interference and forgetting, consistently outperforming vanilla multi-task and multi-stage fine-tuning baselines.

Conclusion: The CPI-FT framework effectively mitigates destructive interference in multi-task fine-tuning and prevents catastrophic forgetting through strategic parameter isolation and fusion techniques.
...
TL;DR: MENTAT combines prompt optimization with neural ensemble learning to significantly improve performance on reasoning-intensive regression tasks where both prompting frozen LLMs and finetuning encoders struggle.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of reasoning-intensive regression (RiR) tasks that require deep text analysis for numerical property deduction, particularly in scenarios with limited training data and computation.

Method: Proposes MENTAT - a method combining batch-reflective prompt optimization with neural ensemble learning to enhance performance on RiR tasks.

Result: MENTAT achieves up to 65% improvement over baseline methods (prompting frozen LLMs and finetuning Transformer encoders) on established RiR benchmark tasks.

Conclusion: The method shows significant improvements but substantial room remains for future advances in reasoning-intensive regression problems.
...
Conclusion: Correct reasoning chains show significantly higher reasoning and answer confidence, validating PiCSAR's effectiveness as a simple, training-free scoring method.
...
<details>
  <summary>Details</summary>
Motivation: Web-scale datasets like Common Crawl used for LLM training raise data quality, safety, and ethical challenges, but prior research has been limited to small samples due to computational constraints.

Method: ElasticSearch-based pipeline for indexing and analyzing LLM training datasets, applied to a 1.5TB multilingual corpus (FineWeb-2) across four languages.
...
Conclusion: Provides practical tools for safer and more accountable AI systems by enabling efficient analysis of large-scale training datasets to address quality and safety concerns.
...
<details>
  <summary>Details</summary>
Motivation: Addressing novel scenarios and out-of-distribution hazards is critical for achieving safe self-driving cars, as current systems struggle with unexpected situations.

Method: The workshop provides a forum for researchers and industry experts to share advancements in novelty handling, including vision-language models, anomaly detection, open-set recognition, and domain adaptation techniques.

Result: Building on the success of the inaugural workshop at WACV 2025, the 2nd edition aims to inspire new algorithms and systems for hazard avoidance in autonomous driving.

Conclusion: 2COOOL workshop serves as a dedicated platform to advance state-of-the-art in handling novel scenarios and improving safety in autonomous driving systems through interdisciplinary collaboration.
...
Method: Used dataset of 1,512 radiographs with 11,137 expert annotations. Evaluated three approaches: custom CNN, hybrid CNN+traditional classifiers, and fine-tuned pre-trained architectures (VGG16, Xception, ResNet50) with 5-fold cross validation.

Result: Hybrid CNN-Random Forest achieved highest performance: 85.4% accuracy. VGG16 best pre-trained at 82.3%. Hybrid models improved discrimination of morphologically similar conditions.
...
<details>
  <summary>Details</summary>
Motivation: Zero-shot appearance transfer with large-scale image generation models faces attention leakage issues where semantic mapping between images is inadequately captured by Query-Key alignment.

Method: Proposes Q-Align with three components: 1) Query-Query alignment for better spatial semantic mapping, 2) Key-Value rearrangement for enhanced feature correspondence, and 3) Attention refinement using rearranged keys/values for semantic consistency.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from substantial computational overhead due to iterative inference, and naive feature caching causes noticeable quality degradation.

Method: Proposes ERTACache framework with offline residual profiling to identify reusable steps, dynamic integration interval adjustment via trajectory-aware correction, and closed-form residual linearization for error approximation.
...
Conclusion: ERTACache effectively maintains baseline fidelity while significantly improving efficiency through principled error correction in diffusion model caching.
...
Method: Proposes Temporal Visual Screening (TVS) that retains focus-critical video segments, synchronously reconstructs queries to direct form while preserving answer consistency, and maintains invariance for any possible answer.

Result: TVS achieves relative gains of 7.33% in training and 34.6% in inference, with ReSimplifyIt baseline outperforming prior approaches by 0.47 F-1 score on video trimming while maintaining competitive query rewriting performance.

Conclusion: TVS effectively improves video-language understanding by optimizing reasoning burden distribution and cognitive load through temporal information screening, serving as a modular front-end adapter for both training and inference pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for surgical tool localization are limited by the availability of diverse annotated data. The authors propose that skeletal pose annotations strike a better balance between semantic information richness and annotation ease, enabling faster growth of annotated datasets.

Method: The authors created ROBUST-MIPS by enriching the existing ROBUST-MIS dataset with tool pose annotations and instance segmentation data. They developed custom annotation software and established a benchmark using popular pose estimation methods to evaluate the effectiveness of pose annotations.

Result: The study demonstrates high-quality results using pose estimation methods for surgical tool localization, showing that pose annotations are adequate for this task. The enriched dataset enables joint study of both annotation styles and head-to-head comparisons.

Conclusion: Skeletal pose annotations represent an efficient alternative to segmentation for surgical tool localization, facilitating accelerated growth of annotated datasets. The released dataset, benchmark models, and annotation software aim to encourage adoption of this annotation approach in computer-assisted interventional technologies.
...
<details>
  <summary>Details</summary>
Motivation: Existing T2I model safety mechanisms are vulnerable to evasion under distribution shifts or require extensive model-specific adjustments, creating safety concerns for potential misuse.

Method: Uses data-driven strategies and safety-aware conditions to inject safety control signals into locked T2I models as a patch-like update, with flexible merging of multiple safety patches.

Result: Reduces unsafe content generation probability to 7% compared to ~20% for baseline methods, maintains benign image quality and text alignment, and works across six diverse T2I models.

Conclusion: Safe-Control provides an effective, adaptable plug-and-play safety solution that significantly outperforms existing safety mechanisms while maintaining compatibility with similar denoising architecture T2I models.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods underperform in handling stuff-type target regions with ambiguous boundaries, and struggle with absent or multiple targets in navigation tasks.
...
Conclusion: GENNAV effectively addresses the challenges of stuff-type region segmentation and multiple/absent target handling, demonstrating practical applicability in real-world navigation scenarios with strong generalization capabilities.
...
Result: Outperforms Qwen2.5-VL-7B in most tasks and achieves comparable performance to larger models like Kimi-VL-A3B-Thinking-2506 (16B) with lower computational cost.

Conclusion: R-4B demonstrates efficient adaptive thinking capabilities, providing optimal performance while reducing unnecessary computational overhead for simple problems.
...
Method: Proposes a Mamba-based fusion mechanism that integrates RGB, thermal, and depth data by identifying modality-specific features and fusing them into a unified representation.
...
Conclusion: Mamba-based fusion architectures significantly advance multimodal object detection, especially under visually degraded or complex conditions, exposing limitations of current unimodal and naive fusion strategies.
...
<details>
  <summary>Details</summary>
Motivation: Traditional CT/X-ray registration methods suffer from spatial information loss and domain gap, while current 3D reconstruction methods struggle with noisy X-rays and require dense views, creating challenges for accurate real-time image-guided navigation.

Method: Joint 3D Radiative Gaussians reconstruction and 3D/3D registration framework with Counterfactual Attention Learning mechanism for vertebral region focus, plus patient-specific pre-training strategy that adapts from simulated to real data while learning vertebral shape priors.

Result: State-of-the-art performance on in-house datasets for both reconstruction and registration tasks, surpassing existing methods in accuracy and handling of noisy X-ray conditions.

Conclusion: RadGS-Reg effectively addresses limitations of traditional methods by combining joint reconstruction and registration with attention mechanisms and adaptive pre-training, demonstrating superior performance for vertebral-level CT/X-ray registration in challenging conditions.
...
Method: Created a synthetic dataset with over 6.2 million residential buildings, each represented through three modalities: semantically enriched 3D wireframe graphs (LoD4), floor plan images, and LiDAR-like roof point clouds with semantic annotations derived from floor plans.

Result: A publicly available large-scale multi-modal dataset that provides semantic annotations including rooms, doors, and windows, enabling development of generative AI algorithms for automated 3D building model creation.

Conclusion: SYNBUILD-3D fills a critical gap in 3D building data availability and enables future research in automated 3D building generation with semantic-geometric consistency constraints.
...
<details>
  <summary>Details</summary>
Motivation: Homography estimation is crucial in computer vision, but real images often have lens distortions that require simultaneous estimation of homography and radial distortion for accurate results. Previous methods addressed different distortion configurations separately.

Method: Proposed a novel unified approach that handles all three radial distortion cases: distortion in only one image, identical distortion in both images, and independent distortion in both images. Developed new minimal solvers that are fast, stable, and accurate.

Result: The proposed solvers are faster than existing state-of-the-art solvers while maintaining similar accuracy. Tested on established benchmarks including fisheye camera images.

Conclusion: The unified approach successfully addresses all three radial distortion configurations with improved computational efficiency, providing practical solvers for real-world computer vision applications involving distorted images.
...
<details>
  <summary>Details</summary>
Motivation: CAVs computed independently at different layers often show inconsistencies, making cross-layer comparisons unreliable for interpreting neural networks.

Method: Leverages contrastive learning to align concept representations across layers and uses attention-based fusion to construct globally integrated CAVs, introducing TGCAV for testing.

Result: Significantly reduces variance in TCAV scores, preserves concept relevance, enhances concept localization, and improves robustness against adversarial perturbations.
...
<details>
  <summary>Details</summary>
Motivation: Current ReID methods are domain-specific and require costly labeled data for new categories. Self-supervised learning struggles with identity-sensitive features needed for ReID.

Method: VICP synergizes LLMs and vision foundation models: LLMs infer semantic identity rules from few-shot examples, which guide VFMs (like DINO) to extract ID-discriminative features via dynamic visual prompts.

Result: VICP outperforms baselines on unseen categories across ShopID10K and diverse ReID benchmarks, demonstrating strong generalization without dataset-specific retraining.

Conclusion: The framework enables generalization to novel object categories using only in-context examples, eliminating the need for costly retraining while maintaining strong performance.
...
<details>
  <summary>Details</summary>
Motivation: Automated pancreatic tumor segmentation is critical for diagnosis and treatment planning but remains challenging due to anatomical variability and limited dataset availability.

Method: Used SegResNet models within Auto3DSeg architecture with 5-fold cross-validation and STAPLE ensembling on region-of-interest focused data. Evaluated on two tasks: 91 T1-weighted arterial contrast-enhanced MRI cases and 50 T2-weighted MR-Linac cases.
...
Conclusion: Results demonstrate challenges in MRI-based pancreatic tumor segmentation with small datasets and highlight performance variability across different MRI sequences. Shows potential for automated delineation but emphasizes need for larger, standardized datasets to improve clinical utility.
...
<details>
  <summary>Details</summary>
Motivation: Pretrained segmentation models struggle to generalize across different cardiac MRI sequences due to significant variations in image contrast caused by different imaging protocols, despite being governed by the same fundamental spin properties.

Method: The method reversely infers underlying spin properties (proton density, T1, T2 values) from observed MRI images by solving ill-posed nonlinear inverse problems regularized by a prior distribution of spin properties learned from a multiparametric mSASHA dataset using a generative diffusion model.

Result: The method enables approximate but meaningful spin-property estimates that serve as interpretable latent variables, allowing highly flexible image synthesis of arbitrary novel sequences and achieving highly accurate segmentation across vastly different image contrasts and protocols.
...
<details>
  <summary>Details</summary>
Motivation: Traditional HMR methods are user-agnostic and optimize for generalization, but they compromise 3D accuracy by not jointly accounting for person-specific body shapes and 3D pose plausibility when refining poses using 2D constraints.

Method: The pipeline decouples the process by first calibrating the user's body shape, then employs a personalized pose fitting process conditioned on that shape using a body shape-conditioned 3D pose prior implemented as a Point Diffusion Transformer with Point Distillation Sampling loss.
...
Conclusion: PHD provides an effective framework for personalized 3D human mesh recovery that addresses limitations of traditional methods by leveraging user-specific shape information and learned 3D pose priors, offering improved accuracy and versatility.
...
<details>
  <summary>Details</summary>
Motivation: OCT provides high-resolution 3D fingerprint data but is expensive and time-consuming to acquire, leading to scarce datasets that hinder deep learning model development.

Method: Three-stage framework: 2D style transfer to mimic OCT scans, 3D structure expansion network for anatomical volume generation, and 3D GAN-based realism refiner for authentic textures and noise.
...
Conclusion: Print2Volume effectively addresses OCT data scarcity by generating high-quality synthetic 3D fingerprints that significantly improve recognition performance when used for model pre-training.
...
<details>
  <summary>Details</summary>
Motivation: Manual analysis of surgical recordings is time-consuming, and computer vision approaches need annotated medical data which is scarce in the field.

Method: Created the first publicly available image dataset with region-based annotations for endometriosis, developed in collaboration with medical experts.

Result: GLENDA dataset enables development of sophisticated computer vision and machine learning approaches for automated surgical video analysis.

Conclusion: This dataset addresses the critical need for annotated medical data and supports research in automated analysis of gynecologic laparoscopy procedures.
...
<details>
  <summary>Details</summary>
Motivation: Surgical videos are valuable information sources but automatic content indexing for searchable archives remains challenging due to the specialized nature of medical video content.
...
Conclusion: While binary segmentation of surgical instruments from background is feasible with good accuracy, identifying specific instrument types remains difficult because of their inherent visual similarity, indicating need for improved recognition methods.
...
TL;DR: SatDINO is a self-supervised learning model for satellite imagery that outperforms MAE-based methods and achieves competitive results across multiple benchmarks with novel enhancements like GSD encoding and adaptive view sampling.
...
Method: Develop SatDINO - a tailored DINO-based model for satellite imagery with novel enhancements including ground sample distance (GSD) encoding and adaptive view sampling techniques.

Result: SatDINO outperforms state-of-the-art masked autoencoder (MAE) methods and achieves competitive results across multiple remote sensing benchmarks and testing setups.

Conclusion: SatDINO demonstrates the effectiveness of contrastive self-supervised learning for remote sensing, with proposed enhancements that can be independently applied to improve satellite imagery representation learning.
...
TL;DR: A framework for generating 2D index maps and profiling mechanisms for Whole Slide Images (WSIs) to enable automated content analysis and AI algorithm development without manual inspection.


<details>
  <summary>Details</summary>
Motivation: Current WSI collections lack standardized metadata for content identification, requiring manual inspection which is impractical for large collections with millions of images, hindering AI algorithm development and cohort assembly.

Method: Proposes a general framework with 2D index maps and domain-specific profiling. Uses three-layer tissue map organization (source, tissue type, pathological alterations) with common syntax and semantics for interoperability between different catalogs.

Result: Enables automated fine-grained content analysis of WSIs, demonstrated in clinical pathology with specific examples showing advantages in WSI catalogs, machine learning, and graph-based WSI representations.

Conclusion: The proposed standard provides an effective solution for automated WSI content profiling, facilitating large-scale analysis and AI algorithm development across various medical domains while ensuring interoperability between different systems.
...
Method: Proposes ICPL method using confidence-based pseudo-labels to replace human annotations. Integrates pseudo-labels into various CIL methods with confidence-based selection. Evaluated on CIFAR100, ImageNet100, and fine-grained datasets.

Result: Achieves competitive results compared to supervised methods. Outperforms state-of-the-art class-iNCD methods by more than 5% in final accuracy. Validated suitability for resource-constrained environments through computational complexity analysis.

Conclusion: ICPL provides an effective unsupervised solution for incremental learning that works with unlabeled datasets, demonstrating practical applicability in real-world scenarios while maintaining competitive performance.
...
<details>
  <summary>Details</summary>
Motivation: Synthetic medical data has domain gaps that limit its generalizability to real-world clinical settings, particularly in X-ray imaging where discrepancies exist in attenuation behavior, noise characteristics, and soft tissue representation.

Method: Proposes MedShift, a class-conditional generative model based on Flow Matching and Schrodinger Bridges that learns a shared domain-agnostic latent space and supports unpaired translation between any training domains without requiring paired data.

Result: Despite smaller model size than diffusion approaches, MedShift shows strong performance, remains flexible at inference (can prioritize perceptual fidelity or structural consistency), and is benchmarked on new X-DigiSkull dataset of aligned synthetic/real skull X-rays.

Conclusion: MedShift provides a scalable and generalizable solution for domain adaptation in medical imaging, offering high-fidelity translation between synthetic and real medical images with tunable output characteristics.
...
Method: Deep learning model using 3-m resolution PlanetScope imagery with simulated tree crowns using Gaussian kernels of scalable size. Training based on billions of points automatically extracted from airborne lidar data.
...
TL;DR: M is a scalable Gaussian Splatting framework that uses hierarchical Gaussian organization and hybrid deformation/spawning for efficient dynamic scene training with reduced computational overhead.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting has limitations for dynamic scenes due to large data volume and long training times per frame, requiring more efficient streaming solutions.

Method: Hierarchical organization of Gaussian spheres by scale, hybrid deformation and spawning strategy for motion modeling, and bidirectional adaptive masking for training efficiency.

Result: Achieves superior visual quality while significantly reducing training time compared to state-of-the-art methods.

Conclusion: M provides an effective framework for scalable and efficient dynamic scene rendering with 3D Gaussian Splatting.
...
TL;DR: A lightweight 125M-parameter specialist model achieves performance comparable to large multimodal generalists for image captioning, but suffers from visual blindness. A novel Sharp-Eyed Refinement framework with DeepLens component improves visual grounding to address this limitation.
...
Method: Implemented a specialist model based on 125M-parameter language model (56x smaller than LLaMA-7B). Developed Sharp-Eyed Refinement framework with DeepLens component that extracts detailed visual representations by focusing on informative regions identified during initial glance.

Result: The lightweight specialist achieves performance comparable to large multimodal generalists. The framework effectively addresses visual blindness limitations and improves caption quality through enhanced visual grounding.

Conclusion: Small specialist models can serve as strong visual specialists for on-device applications. The proposed refinement framework successfully mitigates visual blindness issues and demonstrates advantages over both prior small captioning models and large generalists.
...
TL;DR: Benchmark study evaluates foundation models in federated learning for dementia diagnosis, finding classification head architecture, freezing encoders, and advanced aggregation methods significantly impact performance.


<details>
  <summary>Details</summary>
Motivation: Foundation models show promise for AI-based dementia diagnosis but their integration into federated learning systems remains underexplored, particularly for decentralized clinical settings.
...
Conclusion: Provides practical insights for deploying foundation models in decentralized clinical settings and highlights trade-offs to guide future method development in federated learning for medical applications.
...
TL;DR: Proposes Multi-Method Ensemble (MME) score that combines feature truncation and multiple scoring functions to significantly improve out-of-distribution detection performance across various benchmarks.
...
Method: Theoretically and empirically demonstrates that state-of-the-art feature truncation and scoring functions can be effectively combined. Proposes MME score that unifies multiple OOD detectors into a single scoring function through aggregation.

Result: MME significantly outperforms recent state-of-the-art methods across large-scale and small-scale benchmarks. Achieves average FPR95 of 27.57% on ImageNet-1K using BiT model, improving by 6% over best baseline.

Conclusion: Combining multiple OOD detection techniques through ensemble methods provides superior performance and robustness against various types of OOD samples compared to individual approaches.
...
TL;DR: Proposes localized augmentation method for adversarial patch attacks on ship detection, focusing only on target regions to improve attack success rate and transferability by reducing background interference.
...
Method: Localized augmentation that applies transformations only to target regions, avoiding influence on non-target areas, allowing loss function to focus specifically on adversarial patch impact.
...
Conclusion: The localized augmentation approach reduces background interference and improves attack effectiveness by focusing augmentation only on target regions.
...
TL;DR: ELV-Halluc is the first benchmark for long-video hallucination in Video-MLLMs, focusing on Semantic Aggregation Hallucination (SAH) where models produce incorrect outputs despite correct frame-level semantics.


<details>
  <summary>Details</summary>
Motivation: Existing video hallucination benchmarks focus on short videos and oversimplify hallucination causes. SAH occurs during frame-to-event semantic aggregation and becomes critical in long videos due to increased semantic complexity.

Method: Introduces ELV-Halluc benchmark, analyzes SAH patterns, experiments with positional encoding strategies, and uses DPO (Direct Preference Optimization) with 8K adversarial data pairs to enhance semantic distinction capabilities.

Result: Confirms SAH existence showing it increases with semantic complexity, especially with rapidly changing semantics. Achieves 27.7% reduction in SAH ratio and improvements on both ELV-Halluc and Video-MME benchmarks.

Conclusion: SAH is a distinct hallucination type in long videos that requires specialized attention. Positional encoding and DPO strategies effectively mitigate SAH, providing promising directions for improving Video-MLLM reliability in complex video understanding tasks.
...
<details>
  <summary>Details</summary>
Motivation: Patch-based vision transformers struggle with fine features in micrographs and large image sizes common in materials/biological analysis. Current methods lack computational efficiency for high-resolution segmentation.

Method: Train a convolutional neural network to upsample low-resolution foundation model features by referencing the original input image. Apply this pre-trained upsampler without additional training to various microscopy images.

Result: Successfully segmented plant cells, lithium-ion battery cathodes, and organic crystals. The upsampled features enabled separation of hard-to-segment phases like hairline cracks. Interactive segmentation required far fewer labels and was much faster than traditional CNN approaches.

Conclusion: The proposed upsampler network efficiently enhances foundation model features for microscopy image analysis, achieving high-quality segmentation with minimal labeling effort and computational cost compared to traditional methods.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in drone vision-language understanding caused by wide field of view, complex compositional semantics, and incomplete/ambiguous text descriptions in drone scenarios.

Method: Proposes HCCM framework with two components: (1) Region-Global Image-Text Contrastive Learning (RG-ITC) for hierarchical local-to-global semantics, (2) Region-Global Image-Text Matching (RG-ITM) for local semantic consistency. Also includes Momentum Contrast and Distillation (MCD) mechanism for robustness.
...
Conclusion: HCCM effectively addresses drone vision-language challenges without requiring precise scene partitioning or rigid constraints, showing superior performance and generalization capabilities.
...
TL;DR: A generative method using latent diffusion to reconstruct complete 3D scenes with Gaussian splats from single images, addressing occlusion and unobserved areas through self-supervised learning.


<details>
  <summary>Details</summary>
Motivation: Gaussian splatting fails on occluded/unobserved areas and conventional regression methods produce blurry, implausible results that can't capture multiple plausible solutions for scene completion.

Method: Proposes a generative formulation using a Variational AutoReconstructor to learn latent space from 2D images self-supervised, then trains a diffusion model over this latent space to generate diverse 3D Gaussian splat representations.
...
Conclusion: The method successfully addresses scene completion challenges by combining generative modeling with self-supervised learning, producing complete and plausible 3D reconstructions from minimal input.
...
<details>
  <summary>Details</summary>
Motivation: Pairwise comparisons are more reliable than absolute ratings but require O(n²) annotations, creating a massive annotation burden that needs to be reduced.

Method: Uses CLIP model for zero-shot hierarchical pre-ordering, initializes bucket-aware Elo scores, and runs uncertainty-guided human-in-the-loop MergeSort with automated easy comparisons.

Result: Achieved 90.5% reduction in human annotations compared to exhaustive pairwise comparisons and 19.8% reduction compared to prior work (n=100), while maintaining or improving inter-rater reliability across multiple datasets.

Conclusion: Combining CLIP-based priors with uncertainty-aware sampling provides an efficient and scalable solution for pairwise ranking tasks.
...
TL;DR: ECHO is a unified framework that recovers human pose, object motion, and contact from minimal egocentric observations (head and wrists tracking) using a Diffusion Transformer and three-variate diffusion process.


<details>
  <summary>Details</summary>
Motivation: Modeling human-object interactions from egocentric perspective is important but largely unexplored, especially with the increasing adoption of wearable devices like smart glasses and watches.

Method: Employs Diffusion Transformer architecture with three-variate diffusion process to jointly model human motion, object trajectory, and contact sequence. Operates in head-centric canonical space and uses conveyor-based inference for sequences of any length.

Result: ECHO outperforms existing methods that lack the same flexibility, achieving state-of-the-art performance in egocentric HOI reconstruction.

Conclusion: The proposed framework successfully recovers three modalities from minimal observations, demonstrating the feasibility of comprehensive interaction modeling from head and wrists tracking alone.
...
TL;DR: This paper evaluates how well general vision-language models (VLMs) perform on urban scene understanding tasks, showing that fine-tuning with synthetic urban VQA data significantly improves performance, especially for complex reasoning questions.


<details>
  <summary>Details</summary>
Motivation: To assess how well current general-purpose vision-language models transfer their spatial reasoning abilities to urban scenes, which remains underexplored despite being crucial for urban scene understanding.

Method: Comparative study of three VLMs (BLIP-2, InstructBLIP, LLaVA-1.5) using both zero-shot evaluation and fine-tuning with a synthetic urban VQA dataset constructed from segmentation, depth, and object detection predictions, paired with LLM-generated Chain-of-Thought answers.

Result: VLMs perform reasonably well in zero-shot settings, but fine-tuning with the synthetic CoT-supervised dataset substantially boosts performance, particularly for challenging question types like negation and counterfactuals.
...
TL;DR: TFM is a unified generative method for learning temporal distributions in medical imaging that supports 3D volumes, multiple prior scans, and irregular sampling, outperforming existing spatio-temporal methods.
...
Conclusion: TFM provides a robust baseline and unified framework for temporal medical image analysis, addressing fundamental gaps in existing methods and demonstrating superior performance across multiple datasets.
...
TL;DR: Multimodal deep learning combining CT scans and pathology images improves recurrence prediction in kidney cancer, with pathology being most prognostic and intermediate fusion providing best performance.
...
Method: Modular deep learning framework with pretrained encoders and Cox survival modeling, tested across unimodal, late fusion, and intermediate fusion setups using CT and whole-slide pathology images.

Result: WSI-based models outperformed CT-only models, intermediate fusion further improved performance, best model approached adjusted Leibovich score, and radiology added value primarily through fusion.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning-based CGH methods have three main limitations: (1) end-to-end networks ignore physical relationships, reducing interpretability; (2) CNNs have limited receptive fields for capturing long-range dependencies; (3) ASM-based models are constrained to finite near-fields.
...
Result: Achieves PSNR over 35 dB, outperforming existing methods. Experiments on both simulated and real data demonstrate state-of-the-art performance with improved flexibility and working distance range.

Conclusion: The proposed DUN framework successfully addresses key limitations in CGH by combining physical modeling with deep learning, providing both interpretability and superior reconstruction quality across various working distances.
...
TL;DR: Interactive PET/CT lesion segmentation using promptable nnU-Net with Euclidean Distance Transform encoding of user clicks, achieving improved performance over baseline models.


<details>
  <summary>Details</summary>
Motivation: Address the need for human-in-the-loop refinement in clinical PET/CT segmentation by developing interactive methods that can efficiently incorporate user prompts to improve automated segmentation accuracy.

Method: Extended the winning autoPET III nnU-Net pipeline with promptable capabilities by encoding user-provided foreground/background clicks as additional input channels using Euclidean Distance Transform (EDT) encodings. Implemented online simulation of user interactions and custom point sampling strategy for robustness.

Result: EDT-based ensemble models achieved strongest cross-validation performance, reducing both false positives and false negatives compared to baseline models. EDT encodings consistently outperformed Gaussian kernel representations.

Conclusion: Promptable models with EDT encoding show significant potential for enabling efficient, user-guided segmentation workflows in multi-tracer, multi-center PET/CT imaging, bridging the gap between fully automated methods and clinical practice needs.
...
Method: Proposes a geometric mapping based on camera parameters adapted to the scene, with a probabilistic projection mechanism using confidence scores to refine mapping alignment and filter irrelevant elements. Also improves temporal processing by selectively accumulating reliable information over time.
...
Conclusion: The proposed probabilistic projection with confidence scores effectively addresses generalization issues in HD map construction, providing more accurate and reliable mapping results while reducing hallucinations of non-existent road elements.
...
<details>
  <summary>Details</summary>
Motivation: Word-level OCR has shifted the bottleneck from character segmentation to word segmentation, limiting context for language models and causing segmentation errors.

Method: Proposed line-level OCR that processes entire lines of text instead of individual words, bypassing word detection errors and providing larger sentence context for better language model utilization.
...
Conclusion: Line-level OCR represents a natural progression that improves both accuracy and efficiency, with potential to leverage advances in large language models for document images.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of resource-intensive full fine-tuning of large diffusion models that require enterprise GPUs and thousands of synthetic images for dataset augmentation.
...
Result: Achieves up to 21.3% improvement in mAP@.50:.95 compared to ODGEN baseline, using only 10% of the data and fraction of computational cost.

Conclusion: Quality and efficiency-focused approach with FLORA is more effective than brute-force generation, making synthetic data creation practical and accessible for real-world scenarios.
...
Method: Monitor activation entropy in CNN layers without modifying the model, using parallel entropy monitoring on VGG-16 to detect distribution shifts.
...
Conclusion: CNNs inherently encode distribution shifts in activation patterns, enabling real-time adversarial detection through entropy monitoring without compromising original performance.
...
TL;DR: CAD2DMD-SET is a synthetic data generation tool that creates diverse VQA-labeled datasets for training LVLMs to read digital measurement devices in challenging real-world conditions, achieving 200% performance improvement on InternVL.


<details>
  <summary>Details</summary>
Motivation: LVLMs struggle with reading values from digital measurement devices in real-world conditions involving clutter, occlusions, extreme viewpoints, and motion blur, which are common in head-mounted cameras and AR applications.

Method: Developed CAD2DMD-SET tool using 3D CAD models, advanced rendering, and high-fidelity image composition to generate synthetic DMD datasets. Created DMDBench validation set with 1,000 annotated real-world images. Fine-tuned LVLMs using LoRA with the synthetic dataset.

Result: Substantial improvements in model performance with InternVL showing 200% score increase using Average Normalised Levenshtein Similarity (ANLS) metric, without degrading performance on other tasks.

Conclusion: CAD2DMD-SET significantly improves LVLM robustness and performance for reading digital measurement devices under challenging conditions. The tool will be released as open-source to enable community use and dataset generation.
...
TL;DR: SSL-SaN is a self-supervised model for visual sound source localization that improves robustness against negative audio cases (silence, noise, offscreen sounds) through novel training with negative samples, achieving SOTA performance.
...
Conclusion: The approach addresses key limitations in current visual sound source localization by handling negative audio cases and providing better evaluation metrics and datasets.
...
Method: Systematic data engineering strategies, establishment of interactive environment connecting Mobile and PC devices, supervised finetuning over perception and planning tasks, and curriculum reinforcement learning framework for complex reasoning and exploration.

Result: UItron achieves superior performance in GUI perception, grounding, and planning benchmarks. It shows significant progress in Chinese app scenarios with over one million steps of operation trajectories collected across top 100 popular apps.

Conclusion: UItron propels GUI agents closer to real-world application by addressing key challenges through systemic data engineering and interactive infrastructure, demonstrating particular strength in Chinese mobile app interaction proficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current domain generalization evaluation for CLIP is inadequate because web-scale pretraining data may cover existing benchmarks, making it insufficiently challenging. There's a need to test genuinely unseen data scenarios where CLIP's performance deteriorates significantly.
...
Conclusion: Enhancing domain awareness is a prerequisite for effective domain-invariant classification in foundation models, contrary to standard domain invariance approaches that can harm foundation models by discarding beneficial domain-aware representations.
...
TL;DR: This paper introduces a new video dataset of atypical unusual content and demonstrates that incorporating such data improves performance in open-world learning tasks including OOD detection, novel category discovery, and zero-shot action recognition.


<details>
  <summary>Details</summary>
Motivation: Humans excel at generalizing from uncommon concepts, but most video learning focuses on typical closed-set data. The authors explore how exposing models to atypical unusual videos can benefit open-world learning capabilities.

Method: Collected a new video dataset with various types of unusual atypical data (sci-fi, animation, etc.), fed them into model training for representation learning, and evaluated on three key open-world tasks: OOD detection, novel category discovery, and zero-shot action recognition.

Result: Even straightforward learning approaches with atypical data consistently improved performance across all settings. Categorical diversity of atypical samples boosted OOD detection, smaller but more semantically diverse atypical sets outperformed larger typical datasets in NCD, and semantic diversity helped zero-shot generalization.

Conclusion: Atypical videos provide significant benefits for visual representation learning in open-world scenarios, with the newly proposed dataset encouraging further research in this under-explored direction.
...
TL;DR: Proposes unsupervised video continual learning (uVCL) using non-parametric Kernel Density Estimation of video features with novelty detection for dynamic memory expansion, achieving strong performance without labels or task boundaries.


<details>
  <summary>Details</summary>
Motivation: Addresses the gap in unsupervised video continual learning where prior work focused only on supervised settings with labels and task boundaries, which are costly and impractical for real-world video applications.

Method: Uses Kernel Density Estimation (KDE) of deep embedded video features from unsupervised video transformers, with novelty detection to dynamically expand memory clusters and transfer learning from previous tasks.

Result: Substantially enhances performance when learning multiple tasks successively, validated on UCF101, HMDB51, and Something-to-Something V2 datasets without using labels or class boundaries.

Conclusion: The proposed uVCL framework effectively handles unsupervised video continual learning through non-parametric probabilistic representations and dynamic memory management, demonstrating strong performance on standard video datasets.
...
Method: Built on nnU-Net framework with deep multi-stage cascaded pre-training strategy, starting from general anatomical foundation model and fine-tuning on CT pancreatic lesion datasets and target MRI modalities. Used extensive five-fold cross-validation to evaluate data augmentation schemes and training schedules, then created custom heterogeneous ensembles of specialist models.
...
Conclusion: Presents a robust methodology for developing specialized high-performance models in limited data scenarios with complex medical imaging tasks, demonstrating effectiveness of metric-aware ensembling strategy for PDAC segmentation.
...
TL;DR: GPT-5 demonstrates superior performance over previous models in radiation oncology benchmarks, achieving 92.8% accuracy on multiple-choice exams and generating comprehensive treatment plans with rare hallucinations, though expert oversight remains essential.


<details>
  <summary>Details</summary>
Motivation: To evaluate GPT-5's capabilities in clinical decision support specifically for radiation oncology, assessing its performance against established benchmarks and real-world clinical scenarios.
...
Result: GPT-5 achieved 92.8% accuracy on TXIT exam, outperforming GPT-4 (78.8%) and GPT-3.5 (62.1%). Vignette evaluation showed high correctness (3.24/4) and comprehensiveness (3.59/4) ratings with rare hallucinations. Low inter-rater reliability (kappa 0.083) reflected clinical judgment variability.

Conclusion: GPT-5 shows significant improvement over previous models in radiation oncology but requires expert oversight due to substantive errors in complex scenarios. While hallucinations are infrequent, clinical implementation demands rigorous validation.
...
TL;DR: TMUAD is a unified anomaly detection framework that uses three complementary memory banks (class-level text, object-level image, and patch-level image) to detect both structural and logical anomalies, achieving state-of-the-art performance across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly detection methods struggle with limited normal data and rely on carefully designed image feature extractors. The paper aims to address logical anomaly detection by incorporating text descriptions to capture logical relationships between objects.

Method: Proposes a Three-Memory framework: 1) class-level text memory bank using logic-aware text extractor, 2) object-level image memory bank preserving complete object contours, 3) patch-level memory bank for structural anomalies. These banks retrieve similar normal images and compute fused anomaly scores.
...
Method: Propose VoCap model that processes videos with multimodal prompts; create SAV-Caption dataset by annotating existing segmentation data with pseudo captions using VLMs; train model on mixed image/video datasets.

Result: State-of-the-art results on referring expression video object segmentation, competitive performance on semi-supervised VOS, and establishes video object captioning benchmark.
...
Method: Reformulates verb classification as single positive multi-label learning (SPMLL) and develops Graph Enhanced Verb Multilayer Perceptron (GE-VerbMLP) that combines graph neural networks to capture label correlations and adversarial training to optimize decision boundaries.
...
Conclusion: Verb classification in scene recognition should be treated as a multi-label problem, and the proposed SPMLL framework with GE-VerbMLP effectively addresses the challenges of semantic ambiguity and achieves superior performance.
...
TL;DR: DriveQA is a comprehensive benchmark for testing LLMs and MLLMs on driving knowledge, revealing weaknesses in numerical reasoning, complex scenarios, and environmental variations while showing that fine-tuning improves performance.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving benchmarks lack comprehensive testing of traffic rules, signage, and edge cases that human drivers must understand to pass driving tests.

Method: Created DriveQA benchmark covering traffic regulations and scenarios, tested state-of-the-art LLMs/MLLMs, conducted fine-tuning experiments, and evaluated sensitivity to environmental factors.

Result: LLMs/MLLMs perform well on basic rules but struggle with numerical reasoning, complex right-of-way, sign variations, and spatial layouts. Fine-tuning improves accuracy, particularly in sign recognition and intersection decisions.

Conclusion: DriveQA enhances model performance on real-world driving tasks and demonstrates that models can internalize traffic knowledge to generalize effectively across downstream QA tasks.
...
TL;DR: Proposes hybrid clustering pipeline for transaction counterparty analysis combining string similarity, topic modeling, hierarchical clustering and rules to overcome limitations of NLP models and fuzzy matching in banking contexts.


<details>
  <summary>Details</summary>
Motivation: Natural language models fail for clustering transaction counterparties in banking systems (like SWIFT) due to lack of sentence structure and manual entry variations, creating gaps in fraud detection and payment tracing capabilities.
...
Result: Significantly outperforms baseline rule-based keyword approach on real-life labeled dataset, maintains interpretability, reduces manual review needs, and improves risk control for entity variation detection.

Conclusion: The hybrid approach effectively bridges the gap in transaction counterparty clustering for banking systems, offering improved performance over traditional methods while preserving interpretability and reducing manual effort in investigations.
...
### [95] [Model-Task Alignment Drives Distinct RL Outcomes](https://arxiv.org/abs/2508.21188)
...
<details>
  <summary>Details</summary>
Motivation: Recent RL advances in LLMs show puzzling patterns like single-example learning and negative-only training outperforming sophisticated methods, but it's unclear when these actually work vs fail.
...
Result: Counterintuitive results (single-example learning, inaccurate rewards, negative-only training) only occur when pretrained models already show strong task alignment. These techniques fail in challenging regimes where standard RL remains effective.
...
### [96] [Accept or Deny? Evaluating LLM Fairness and Performance in Loan Approval across Table-to-Text Serialization Approaches](https://arxiv.org/abs/2508.21512)
...
<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly used in high-stakes decision-making like loan approvals, there's a need to assess their performance and fairness when processing tabular data from different geographical regions to ensure reliable financial decisions.

Method: Evaluated LLMs on serialized loan approval datasets from Ghana, Germany, and US using zero-shot and in-context learning (ICL) capabilities. Assessed different serialization formats (GReat, LIFT, etc.) for tabular data conversion.

Result: Serialization format significantly affects both performance and fairness - some formats yield higher F1 scores but exacerbate fairness disparities. ICL improved performance by 4.9-59.6% over zero-shot baselines, but fairness effects varied considerably across datasets.

Conclusion: Effective tabular data representation methods and fairness-aware models are crucial for improving LLM reliability in financial decision-making, as format choices and learning approaches have significant impacts on both performance and fairness outcomes.
...
### [97] [Summarize-Exemplify-Reflect: Data-driven Insight Distillation Empowers LLMs for Few-shot Tabular Classification](https://arxiv.org/abs/2508.21561)
*Yifei Yuan,Jiatong Li,Weijia Zhang,Mohammad Aliannejadi,Evangelos Kanoulas,Renjun Hu*

Main category: cs.LG

TL;DR: InsightTab is a framework that distills tabular data into actionable insights to improve LLM performance on few-shot tabular classification through principles of divide-and-conquer, easy-first, and reflective learning.


<details>
  <summary>Details</summary>
Motivation: Address challenges in few-shot tabular classification with LLMs due to variability in structured data by distilling data into actionable insights inspired by human learning processes.

Method: InsightTab framework integrates rule summarization, strategic exemplification, and insight reflection through collaboration between LLMs and data modeling techniques, guided by divide-and-conquer, easy-first, and reflective learning principles.

Result: Consistent improvement over state-of-the-art methods on nine datasets, with ablation studies validating the principle-guided distillation process and analyses showing effectiveness in leveraging labeled data and managing bias.

Conclusion: InsightTab enables LLMs to better align their general knowledge with specific tabular task requirements through insight distillation, demonstrating robust and effective few-shot classification performance.
...
### [98] [Activation Subspaces for Out-of-Distribution Detection](https://arxiv.org/abs/2508.21695)
...
Method: Utilizes singular value decomposition (SVD) of the weight matrix from the classification head to decompose model activations into two components: decisive (maximally contributing to classifier output) and insignificant (minimally contributing). For Far-OOD, uses insignificant subspace features as they remain unaffected by classification objective. For Near-OOD, uses decisive subspace to avoid interference from insignificant components.

Result: Achieves state-of-the-art results across various standard OOD benchmarks, demonstrating superior performance in both Far-OOD and Near-OOD detection scenarios compared to existing methods.

Conclusion: The proposed ActSub method effectively handles different types of distribution shifts by strategically leveraging different activation subspaces, providing a unified approach that outperforms current state-of-the-art OOD detection techniques.
...
### [99] [Learning Unified Representations from Heterogeneous Data for Robust Heart Rate Modeling](https://arxiv.org/abs/2508.21785)
*Peng Yang,Zhengdong Huang,Zicheng Xie,Wentao Tian,Jingyu Liu,Lunhong Dong*

Main category: cs.LG

TL;DR: Proposed framework handles heart rate prediction data heterogeneity through random feature dropout for source variation and time-aware attention with contrastive learning for user differences, achieving 17-15% performance gains on new benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Address data heterogeneity challenges in real-world heart rate prediction, specifically source heterogeneity from fragmented device markets with varying feature sets and user heterogeneity reflecting distinct physiological patterns across individuals and activities.

Method: Framework with random feature dropout strategy to handle source heterogeneity, time-aware attention module to capture long-term physiological traits, and contrastive learning objective to build discriminative representation space. Created new benchmark dataset ParroTao.

Result: Significantly outperforms existing baselines by 17% on ParroTao dataset and 15% on FitRec dataset. Learned representations demonstrate strong discriminative power and practical value in downstream applications.

Conclusion: The proposed framework effectively addresses both source and user heterogeneity in heart rate prediction, enabling consistent performance under heterogeneous data patterns with significant performance improvements over existing methods.
...
TL;DR: Proposes a drone imagery and MLLM-based framework for assessing rural livability in China, identifying spatial patterns and key influencing factors like government expenditure.


<details>
  <summary>Details</summary>
Motivation: Current rural livability assessment methods are limited - questionnaires don't scale well and urban-oriented visual methods are unsuitable for rural contexts. Need better tools to measure effectiveness of poverty alleviation and rural revitalization strategies.

Method: Used drone imagery from 1,766 villages across China, developed efficient image comparison with binary search interpolation, created chain-of-thought prompting for MLLMs considering living quality and ecological habitability dimensions.

Result: Identified dual-core-periphery spatial pattern radiating from Sichuan and Zhejiang provinces with declining gradients. Government fiscal expenditure was the core determinant (3.9-4.9 unit livability increase per unit expenditure increase).

Conclusion: The framework provides reliable rural livability assessment and valuable insights for rural construction policy-making, demonstrating the effectiveness of drone imagery and MLLMs for large-scale rural environment evaluation.
...
### [101] [Quantum-Enhanced Natural Language Generation: A Multi-Model Framework with Hybrid Quantum-Classical Architectures](https://arxiv.org/abs/2508.21332)
*Chi-Sheng Chen,En-Jui Kuo*

Main category: quant-ph

TL;DR: Quantum text generation models show competitive performance in specific scenarios but traditional Transformers maintain overall superiority in comprehensive evaluation across multiple metrics and datasets.
...
Result: Traditional Transformers maintain overall superiority with lowest average perplexity (1.21) and highest BLEU-1 score (0.2895). Quantum models show competitive performance: QKSAN achieves BLEU-1 of 0.2800 with zero repetition rates, QRWKV demonstrates perfect vocabulary diversity (Distinct-1 = 1.000) in certain tasks.

Conclusion: While quantum-inspired models demonstrate promising capabilities in specific areas like repetition avoidance and vocabulary diversity, traditional Transformer architectures remain superior in overall text generation quality and performance metrics.
...
### [102] [Designing Smarter Conversational Agents for Kids: Lessons from Cognitive Work and Means-Ends Analyses](https://arxiv.org/abs/2508.21209)
*Vanessa Figueiredo*

Main category: cs.HC

TL;DR: Brazilian children use conversational agents for schoolwork, discovery, and entertainment, with structured scaffolds improving interaction quality. Two studies show recipe-based prompting outperforms unstructured approaches.


<details>
  <summary>Details</summary>
Motivation: To understand how Brazilian children interact with conversational agents and develop effective scaffolding methods to enhance their learning and engagement experiences.

Method: Study 1: 7-week online investigation with 23 participants using interviews, observations, and Cognitive Work Analysis. Study 2: GPT-4o-mini testing on 1,200 simulated child-CA exchanges comparing structured-prompting recipes vs unstructured baseline.

Result: Identified three CA functions (School, Discovery, Entertainment) and developed recipe scaffolds. Quantitative evaluation showed gains in readability, question metrics, and coherence with structured approach.

Conclusion: Proposed design recommendations including scaffolded conversation-trees, child-dedicated profiles, and caregiver-curated content. First CWA application with Brazilian children and empirical framework for child-CA information flows.
...
### [103] [Morae: Proactively Pausing UI Agents for User Choices](https://arxiv.org/abs/2508.21456)
*Yi-Hao Peng,Dingzeyu Li,Jeffrey P. Bigham,Amy Pavel*

Main category: cs.HC

TL;DR: Morae is a UI agent that identifies decision points during web tasks and pauses to let blind/low-vision users make choices, improving user agency and preference matching compared to end-to-end automation agents.
...
Method: Morae uses large multimodal models to interpret user queries alongside UI code and screenshots, automatically identifies decision points during task execution, and prompts users for clarification when choices need to be made.

Result: In a study with BLV participants over real-world web tasks, Morae helped users complete more tasks and select options that better matched their preferences compared to baseline agents including OpenAI Operator.

Conclusion: This work demonstrates a mixed-initiative approach where users benefit from UI automation while maintaining the ability to express preferences, addressing the agency problem in current UI agents.
...
### [104] [CrossTL: A Universal Programming Language Translator with Unified Intermediate Representation](https://arxiv.org/abs/2508.21256)
...
Method: Uses a single universal IR (CrossGL) with language-specific lexers/parsers to convert source code to ASTs, bidirectional translation modules implementing ToCrossGLConverter classes for importing code and CodeGen classes for target generation, and comprehensive backend implementations for full translation pipelines.

Result: Successful compilation and execution across all supported backends, demonstrating practical viability through comprehensive evaluation across programming domains including GPU compute, graphics programming, and systems languages.

Conclusion: CrossTL represents a significant step toward language-agnostic programming, enabling write-once, deploy-everywhere development with minimal effort required to add new languages through its modular architecture and unified IR design.
...
### [105] [ScanMove: Motion Prediction and Transfer for Unregistered Body Meshes](https://arxiv.org/abs/2508.21095)
...
Method: Couples a robust motion embedding network with a learned per-vertex feature field to generate spatio-temporal deformation fields that drive mesh deformation.
...
Conclusion: The proposed approach shows effectiveness and versatility in handling challenging unregistered meshes for motion prediction and transfer tasks.
...
### [106] [ARGS: Advanced Regularization on Aligning Gaussians over the Surface](https://arxiv.org/abs/2508.21344)
...
<details>
  <summary>Details</summary>
Motivation: Address limitations in existing 3DGS models like SuGaR by improving both visual fidelity and scene consistency in 3D mesh reconstruction.

Method: Two complementary regularizations: 1) Effective rank regularization to prevent extreme anisotropy and favor disk-like Gaussian shapes, 2) Neural Signed Distance Function with Eikonal loss for continuous global surface prior and better geometric alignment.

Result: More accurate and coherent visuals from 3DGS data with improved fidelity of individual Gaussian primitives and collective surface behavior.

Conclusion: The proposed regularization strategies successfully enhance both the shape quality of individual Gaussians and the overall surface coherence in 3D reconstruction from Gaussian splatting data.
...
TL;DR: Miffie is an automated database normalization framework using large language models with a dual-model self-refinement architecture that generates and verifies normalized schemas iteratively until requirements are met.


<details>
  <summary>Details</summary>
Motivation: Database normalization is crucial for data integrity but is time-consuming and error-prone when done manually by data engineers, creating a need for automated solutions.

Method: Uses a dual-model self-refinement architecture combining generation and verification modules with task-specific zero-shot prompts. The generation module eliminates anomalies based on verification feedback until normalization requirements are satisfied.

Result: Experimental results show Miffie can normalize complex database schemas while maintaining high accuracy without human effort.

Conclusion: Miffie provides an effective automated solution for database normalization that preserves data integrity through LLM-powered generation and verification cycles.
...
### [108] [QuadKAN: KAN-Enhanced Quadruped Motion Control via End-to-End Reinforcement Learning](https://arxiv.org/abs/2508.19153)
...
<details>
  <summary>Details</summary>
Motivation: Vision-guided quadruped motion control requires robust integration of proprioception with vision, as relying solely on vision is insufficient for stable locomotion across diverse terrains.

Method: Proposed QuadKAN framework with spline-encoder for proprioception and spline fusion head for cross-modal inputs, trained end-to-end with PPO and Multi-Modal Delay Randomization (MMDR).
...
Conclusion: Spline-parameterized policies provide a simple, effective, and interpretable alternative for robust vision-guided locomotion, reducing action jitter and energy consumption while improving sample efficiency.
...
### [109] [Mini Autonomous Car Driving based on 3D Convolutional Neural Networks](https://arxiv.org/abs/2508.21271)
*Pablo Moraes,Monica Rodriguez,Kristofer S. Kappel,Hiago Sodre,Santiago Fernandez,Igor Nunes,Bruna Guterres,Ricardo Grando*

Main category: cs.RO

TL;DR: This paper presents a 3D CNN-based approach for autonomous driving using Mini Autonomous Cars in simulated environments, showing promising results compared to RNNs.


<details>
  <summary>Details</summary>
Motivation: To address challenges in autonomous driving development such as high complexity, long training periods, and uncertainty by using cost-effective Mini Autonomous Cars as testbeds for rapid evaluation of machine learning models.

Method: A methodology based on RGB-D information and three-dimensional convolutional neural networks (3D CNNs) for autonomous driving in simulated environments, evaluated against recurrent neural networks (RNNs) on two tracks with distinct environmental features.

Result: The proposed 3D CNN demonstrated promising results compared to RNNs, with performance assessed using task completion success, lap-time metrics, and driving consistency. Architectural modifications and track complexity were found to influence models' generalization capability and vehicle control performance.

Conclusion: 3D CNNs show strong potential for autonomous driving applications in simulated environments, offering a viable alternative to RNN approaches with better performance in the tested scenarios.
...
Method: Collected over two hours of synchronized sensor data including stereo infrared camera, color camera, IMU (accelerometer, gyroscope, magnetometer), GNSS (multiple positioning modes), and wheel odometry from a soybean field platform with 6-DOF ground truth.
...
Conclusion: This publicly released dataset provides a valuable benchmark for developing and evaluating localization, mapping, perception, and navigation algorithms specifically for agricultural robotics applications.
...
### [111] [Fuzzy, Symbolic, and Contextual: Enhancing LLM Instruction via Cognitive Scaffolding](https://arxiv.org/abs/2508.21204)
*Vanessa Figueiredo*

Main category: cs.AI

TL;DR: Study shows architectural scaffolds with symbolic reasoning and memory improve LLM instructional dialogue performance in Socratic tutoring.
...
Method: Introduced symbolic scaffolding with short-term memory schema, conducted controlled ablation across five system variants, evaluated using expert-designed rubrics and LLM-based evaluation framework.
...
Conclusion: Architectural scaffolds can reliably shape emergent instructional strategies in LLMs, supporting a processing-level account of cognitive behavior.
...
### [112] [AHELM: A Holistic Evaluation of Audio-Language Models](https://arxiv.org/abs/2508.21376)
*Tony Lee,Haoqin Tu,Chi Heem Wong,Zijun Wang,Siwei Yang,Yifan Mai,Yuyin Zhou,Cihang Xie,Percy Liang*

Main category: cs.AI

TL;DR: AHELM is a comprehensive benchmark for audio-language models that standardizes evaluation across 10 key aspects including perception, reasoning, fairness, and safety, using both existing and new synthetic datasets to enable equitable model comparisons.


<details>
  <summary>Details</summary>
Motivation: Current ALM evaluations lack standardization, measure limited capabilities, and omit important aspects like fairness and safety, making cross-model comparisons difficult due to varying prompts and inference parameters.

Method: Introduces AHELM benchmark aggregating various datasets including 2 new synthetic ones (PARADE for stereotype avoidance and CoRe-Bench for conversational reasoning), standardizes prompts/inference parameters/metrics, and tests 14 ALMs plus 3 baseline ASR+LM systems.

Result: Gemini 2.5 Pro ranks top in 5/10 aspects but shows group unfairness (p=0.01) on ASR tasks, while baseline systems perform surprisingly well with one ranking 5th overall despite only speech-to-text capabilities.
...
### [113] [Stairway to Fairness: Connecting Group and Individual Fairness](https://arxiv.org/abs/2508.21334)
*Theresia Veronika Rampisela,Maria Maistro,Tuukka Ruotsalo,Falk Scholer,Christina Lioma*

Main category: cs.IR

TL;DR: The paper investigates the relationship between group fairness and individual fairness in recommender systems, finding that recommendations highly fair for groups can be very unfair for individuals.


<details>
  <summary>Details</summary>
Motivation: There is no established scientific understanding of how group fairness and individual fairness relate to each other in recommender systems, and it's unknown how improving one type affects the other.

Method: Comprehensive comparison of evaluation measures that can be used for both fairness types, conducting experiments with 8 runs across 3 datasets.

Result: Recommendations that are highly fair for groups can be very unfair for individuals, showing a tension between these two fairness types.

Conclusion: The findings provide novel insights for RS practitioners aiming to improve fairness, highlighting the need to consider both group and individual fairness simultaneously rather than optimizing for one type alone.
...
### [114] [From Canonical to Complex: Benchmarking LLM Capabilities in Undergraduate Thermodynamics](https://arxiv.org/abs/2508.21452)
*Anna Geißler,Luca-Sophie Bien,Friedrich Schöppler,Tobias Hertel*

Main category: physics.ed-ph

TL;DR: Current large language models (LLMs) fail to meet the 95% competence threshold required for unsupervised thermodynamics tutoring, achieving only 82% accuracy at best, particularly struggling with image reasoning and irreversible scenarios.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether LLMs are ready for unsupervised use as tutoring aids in undergraduate science education, specifically testing their consistent, principle-grounded reasoning capabilities in thermodynamics.

Method: Created UTQA, a 50-item undergraduate thermodynamics question answering benchmark covering ideal-gas processes, reversibility, and diagram interpretation, and tested leading 2025-era LLMs on this benchmark.

Result: No LLM exceeded the 95% competence threshold - best models achieved 82% accuracy. Text-only items performed better than image reasoning tasks (which often fell to chance levels). Performance showed modest correlation with prompt phrasing and little correlation with syntactic complexity.

Conclusion: Current LLMs are not yet suitable for unsupervised tutoring in thermodynamics due to significant gaps in handling finite-rate/irreversible scenarios and binding visual features to thermodynamic meaning.
