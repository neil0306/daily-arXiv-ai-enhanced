...
Motivation: Current LLM safety methods like SFT and RLHF use multi-stage training pipelines and lack fine-grained post-deployment controllability, requiring a more efficient and flexible solution.

Method: Co-training framework that simultaneously trains multiple safety behaviors within a single supervised fine-tuning stage, using system-level instructions or magic tokens to dynamically activate different behaviors during inference.

Result: Matches safety alignment quality of SFT+DPO, with 8B model surpassing DeepSeek-R1 (671B) in safety performance while reducing training complexity and deployment costs. Creates a distinct Safety Alignment Margin in output space.

Conclusion: Provides a scalable, efficient, and highly controllable solution for LLM content safety with unprecedented fine-grained control and reduced training/deployment overhead.
...
TL;DR: Preliminary automatic ranking for WMT25 MT shared task, with warning that official human evaluation will supersede it due to potential bias towards re-ranking systems.


<details>
  <summary>Details</summary>
Motivation: To share preliminary automatic evaluation results with task participants to help them prepare their system submission papers, while acknowledging the limitations of automatic metrics.

Method: Automatic metrics evaluation of machine translation systems in the WMT25 General MT Shared Task, with recognition that systems using re-ranking techniques (Quality Estimation re-ranking, Minimum Bayes Risk decoding) may be favored.

Result: Preliminary ranking of MT systems based on automatic metrics, but this ranking is provisional and may be biased towards certain system architectures.

Conclusion: This automatic ranking is preliminary and will be replaced by the official human evaluation-based ranking, which is considered more reliable for the final WMT25 results.
...
Method: Introduces an LLM-driven framework for cultural localization that automatically constructs math word problem datasets with native entities from existing sources, replacing English-centric elements with culturally appropriate ones.
...
Conclusion: The proposed LLM-driven cultural localization framework effectively addresses the scarcity of truly localized datasets and demonstrates improved performance in multilingual mathematical reasoning by incorporating native cultural elements.
...
Result: The fine-tuned model outperformed both baseline models, achieving COMET score gains of approximately 0.04 and 0.02 respectively on Wikipedia translations, with more consistent avoidance of language and formatting errors.

Conclusion: DPO training with programmatically curated preference data effectively improves machine translation quality in LLMs, demonstrating significant performance gains even with relatively few data resources.
...
<details>
  <summary>Details</summary>
Motivation: Current ConvXAI systems based on LLMs face challenges with multilingual generalization due to scarce training data and limited support for free-form custom inputs from users.

Method: Introduces MultiCoXQL (multilingual extension of CoXQL) and Compass datasets, proposes new parsing approach for multilingual performance, and evaluates multiple LLMs and BERT-type models on various parsing strategies.

Result: The research provides multilingual datasets spanning 5 diverse languages including low-resource language, and evaluates parsing performance across monolingual, cross-lingual, and multilingual settings.

Conclusion: The work bridges gaps in multilingual ConvXAI by providing datasets and evaluation frameworks that enhance multilingual parsing and custom input extraction capabilities.
...
Method: Proposed SSS algorithm where a small draft model is aligned with human preferences while the target model remains unchanged. Uses modified acceptance criterion and bonus token distribution to exploit distributional shift between aligned draft and unaligned target models.
...
Conclusion: The SSS algorithm effectively addresses the efficiency bottleneck of test-time alignment while maintaining alignment quality, making it practical for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with hallucinations and complex reasoning in structured prediction tasks despite their general language capabilities, needing better consistency and accuracy.

Method: Propose combining LLMs with combinatorial inference, experimenting with prompting strategies to estimate confidence values for symbolic inference, and using structured prediction objectives for calibration and fine-tuning.

Result: Symbolic inference on top of prompting alone leads to more consistent and accurate predictions regardless of prompting strategy, with structured learning further improving performance on challenging tasks.

Conclusion: Structured learning remains valuable in the LLM era, and combining LLMs with symbolic inference addresses consistency issues while enhancing predictive power for structured prediction problems.
...
Method: Domain-agnostic pipeline using layout-aware rendering with lynx and LLM-based cleaning to recover math across formats (MathJax, KaTeX, MathML), preserve equation structure, remove boilerplate, and standardize to LaTeX.
...
Conclusion: First reliable pipeline for scientific content extraction from web data, achieving state-of-the-art math pretraining corpora with measurable gains in math, code, and general reasoning. Code and datasets released for open-source use.
...
Conclusion: Using external evidence and atomic assumption validation effectively addresses questions with false assumptions, reducing hallucinations and providing interpretable answers by identifying specific false premises.
...
Method: Introduces MMDR-Bench dataset with 300 complex multi-turn dialogue scenarios, and proposes CoLVLM Agent framework using iterative "memory-perception-planning-execution" cycle to enhance existing LVLMs without extensive re-training.

Result: CoLVLM Agent achieves average human evaluation score of 4.03, surpassing GPT-4o (3.92) and Gemini 1.5 Pro (3.85), with significant advantages in reasoning depth, instruction adherence, and error suppression.
...
Method: Fin-PRM integrates step-level and trajectory-level reward supervision for fine-grained evaluation of financial reasoning traces, applied in offline/online reward learning settings for distillation-based fine-tuning, reinforcement learning, and Best-of-N inference.

Result: Consistently outperforms general-purpose PRMs and domain baselines on financial benchmarks (CFLUE, FinQA), with substantial improvements across all applications: 12.9% in supervised learning, 5.2% in reinforcement learning, and 5.1% in test-time performance.

Conclusion: Domain-specialized reward modeling is valuable for aligning LLMs with expert-level financial reasoning, demonstrating the importance of tailored approaches for specific domains like finance.
...
<details>
  <summary>Details</summary>
Motivation: Existing KV cache compression methods focus on temporal compression but ignore fine-grained importance variations across feature dimensions, limiting their ability to balance efficiency and accuracy effectively.

Method: Proposes SPARK - a plug-and-play method that applies unstructured sparsity by pruning KV at the channel level and dynamically restoring pruned entries during attention computation, making it orthogonal to existing compression techniques.
...
Method: S2K framework uses internal-external knowledge self-selection strategy, selective supervised fine-tuning, structured reasoning data generation pipeline, and GRPO integration to enhance reasoning ability.
...
Conclusion: S2K provides a progressive knowledge acquisition approach that mirrors human learning, offering a cost-effective solution for domain-specific LLM performance improvement.
...
Method: Investigates whether checklists should be used for all questions or selectively, generates them using six methods, evaluates effectiveness across eight model sizes, and identifies checklist items correlating with human evaluations through experiments on pairwise comparison and direct scoring tasks.
...
Conclusion: Findings highlight the need to more clearly define objective evaluation criteria to guide both human and automatic evaluations, as current inconsistencies affect both evaluation approaches.
...
<details>
  <summary>Details</summary>
Motivation: Small Language Models face memory bottlenecks from large vocabulary components (embeddings and LM heads) on edge devices, with existing static pruning methods causing information loss and lacking flexibility.

Method: Decoupled dynamic vocabulary selection framework based on lexical locality principle and computational asymmetry. Offloads embeddings and implements hybrid static-dynamic vocabulary selection for LM Head with on-demand loading.

Result: Achieves up to 99% reduction in vocabulary-related memory usage with minimal or no performance degradation across diverse downstream tasks, substantially outperforming static vocabulary pruning.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for low-resource languages like Thai rely on translations, which miss cultural and domain-specific nuances needed for real-world applications, creating performance gaps in instruction-following.
...
Result: Models fine-tuned on WangchanThaiInstruct outperform those using translated data in both in-domain and out-of-domain benchmarks, showing significant performance improvements on culturally and professionally specific tasks.

Conclusion: Culturally and professionally grounded instruction data is essential for improving LLM alignment in low-resource, linguistically diverse settings, as native supervision outperforms translated data.
...
<details>
  <summary>Details</summary>
Motivation: There is a lack of comprehensive psychological and ethical evaluation for Large Language Models simulating professional roles, particularly in educational contexts where ethical alignment is crucial.

Method: Developed EMNLP framework with 88 teacher-specific moral dilemmas, extended existing scales, and created targeted soft prompt injection sets to evaluate 12 LLMs in teacher simulation roles.

Result: Teacher-role LLMs show more idealized/polarized personalities than humans, excel in abstract moral reasoning but struggle with emotional complexity. Stronger reasoning models are more vulnerable to harmful injections, showing capability-safety paradox.

Conclusion: This presents the first benchmark for ethical and psychological alignment assessment of teacher-role LLMs, highlighting critical safety concerns in educational AI applications.
...
<details>
  <summary>Details</summary>
Motivation: To solve the problem of context-memory conflicts in RAG systems where retrieved external context contradicts the LLM's correct parametric knowledge, causing the model to fail at resolving such conflicts.

Method: Introduces Conflict-Aware REtrieval-Augmented Generation (CARE) with a context assessor that encodes compact memory token embeddings and uses grounded/adversarial soft prompting to detect unreliable context and provide guidance signals.
...
Conclusion: CARE establishes a promising direction for developing trustworthy and adaptive RAG systems that can better handle conflicts between external context and internal knowledge.
...
<details>
  <summary>Details</summary>
Motivation: Understanding events requires temporal context that is often implicit in natural language. Even advanced LLMs struggle with temporal commonsense reasoning because this information is rarely explicitly stated in text. Automatically mining temporal commonsense can help create more robust language models.

Method: Proposed a temporal commonsense extraction pipeline that leverages LLMs to automatically mine temporal commonsense from text. Used this to construct TComQA dataset derived from SAMSum and RealNews corpora. Evaluated multiple experimental setups to assess LLM effectiveness.

Result: TComQA achieves over 80% precision in extracting temporal commonsense through crowdsourcing validation. Models trained with TComQA outperform LLMs fine-tuned on existing temporal question answering datasets.

Conclusion: The proposed pipeline successfully enables automatic extraction of temporal commonsense using LLMs, and the resulting TComQA dataset significantly improves model performance on temporal reasoning tasks compared to existing approaches.
...
Result: Extensive evaluation through supervised and self-supervised training on diverse languages, including zero-shot tests on the UCLA Phonetic Corpus, demonstrates strong cross-lingual generalization and competitive performance.

Conclusion: Effective universal speech processing is possible through modeling basic acoustic patterns within phoneme-length windows, enabling lightweight yet powerful cross-lingual phoneme recognition.
...
<details>
  <summary>Details</summary>
Motivation: Existing KGC evaluation faces challenges with inconsistent rankings across different datasets and metrics, making holistic model comparisons difficult and hindering reliable model selection.
...
Conclusion: EDAS offers a global perspective for KGC model evaluation, enabling more informed model selection, promoting fairness in cross-dataset comparison, and providing a generalizable framework that addresses inconsistencies in traditional evaluation approaches.
...
<details>
  <summary>Details</summary>
Motivation: With the rapid development of large language models, numerous evaluation benchmarks have emerged, but there's a need to systematically categorize them and identify current limitations to guide future benchmark innovation.
...
Result: Identified current benchmark problems including inflated scores from data contamination, unfair evaluation due to cultural/linguistic biases, and lack of evaluation on process credibility and dynamic environments.
...
Method: Proposes a three-dimensional framework covering truthfulness, robustness, safety, fairness, privacy; multimodal risks; and mitigation strategies from data, model architecture, training, and inference perspectives. Evaluates 30+ MLLMs and 8 mitigation methods.

Result: Reveals significant vulnerabilities in current models, including trustworthiness-capability gaps and risk amplification from multimodal training/inference. Most mitigation strategies show limited effectiveness with unexpected trade-offs that compromise utility.

Conclusion: Introduces Reasoning-Enhanced Safety Alignment (RESA) approach using chain-of-thought reasoning to discover underlying risks, achieving state-of-the-art results and providing practical insights for future improvements.
...
<details>
  <summary>Details</summary>
Motivation: Existing speculative decoding methods use static drafting lengths and rigid verification criteria, limiting adaptability to varying model uncertainties and input complexities.

Method: Proposes confidence-modulated drafting using entropy and margin-based uncertainty measures to dynamically adjust speculative token generation length and flexible verification process.
...
<details>
  <summary>Details</summary>
Motivation: To understand why larger vocabularies benefit language models, given that common words are already single tokens in smaller vocabularies and further growth mainly deepens token-frequency imbalance.

Method: Conducted controlled study scaling vocabulary from 24K to 196K while holding data, compute, and optimization fixed. Quantified tokenized text complexity via Kolmogorov complexity, performed word-level loss decomposition, and constrained embedding norms to test effects of token-frequency imbalance.

Result: Larger vocabularies reduce cross-entropy almost exclusively by lowering uncertainty on the 2,500 most frequent words (even though loss on rare words rises). Models exploit token-frequency imbalance, and this benefit transfers to downstream benchmarks where frequent words cover ~77% of tokens.

Conclusion: The benefit of larger vocabularies comes from lowering the complexity of tokenized text rather than vocabulary size itself. This provides a principled approach for tokenizer-model co-design and clarifies loss dynamics in language model scaling.
...
Method: Systematic analysis of 134 papers, introduction of a unified taxonomy, investigation of 300 evaluation metrics across seven key dimensions, with focus on approaches using citations, attribution, or quotations for evidence-based text generation.

Result: The paper provides a comprehensive taxonomy and analysis of evaluation metrics, examines distinctive characteristics and representative methods in evidence-based text generation with LLMs.

Conclusion: The study highlights open challenges and outlines promising directions for future work in evidence-based text generation to improve reliability and trustworthiness of LLM outputs.
...
Method: Introduces MCR-BENCH, a comprehensive benchmark with inconsistent audio-text pairs, evaluates across diverse audio understanding tasks, investigates text bias factors, explores mitigation through supervised finetuning, and analyzes model confidence patterns.
...
Method: Developed LLaSO framework with three components: LLaSO-Align (12M speech-text alignment corpus), LLaSO-Instruct (13.5M multi-task instruction-tuning dataset), and LLaSO-Eval (reproducible benchmark). Built LLaSO-Base, a 3.8B-parameter model trained on public data.
...
Method: Provides an in-depth overview and comprehensive study of existing privacy-preserving language modeling approaches, analyzing their strengths and limitations.

Result: The study contributes valuable insights to ongoing privacy-preserving language modeling research and identifies future research directions.

Conclusion: Privacy preservation in language models is crucial due to privacy risks from memorization of sensitive training data, and comprehensive understanding of mitigation approaches is needed despite current limitations.
...
Method: Introduces a novel dataset (M-Help) that identifies help-seeking activity, specific mental health disorders, and underlying causes like relationship or financial stressors
...
Conclusion: M-Help dataset provides a comprehensive framework for detecting and understanding mental health help-seeking behavior with specific disorder identification and causal analysis
...
Method: Library-based research methodology was used to study and develop professional methods for rendering non-equivalent words from source to target languages.

Result: The research developed different ways and rules for translating non-equivalent words and successfully rendered 25 non-equivalent words from Dari & Uzbek into English and Russian languages.

Conclusion: Professional rendering methods are essential for translating non-equivalent words to ensure accurate understanding between languages, though many words still require proper translation solutions.
...
Method: PyTOD generates executable code for dialogue state tracking using constrained decoding with a language model (instead of grammar rules) to follow API schemata, and employs policy and execution feedback for efficient error correction.

Result: Achieves state-of-the-art state tracking performance on the challenging SGD benchmark, surpassing strong baselines in both accuracy and robust user goal estimation as dialogue progresses.

Conclusion: The approach demonstrates the effectiveness of execution-aware state tracking in programmable dialogue agents.
...
<details>
  <summary>Details</summary>
Motivation: Current radiology report evaluation methods produce coarse scores or rely on opaque black-box models, limiting their clinical usefulness. There's a need for clinically grounded, interpretable, and fine-grained metrics.

Method: Builds on Group Relative Policy Optimization with two innovations: Sub-score Dynamic Weighting (adaptively prioritizes challenging error types based on live F1 statistics) and Majority-Guided Advantage Scaling (adjusts policy gradient updates based on prompt difficulty from sub-score agreement).
...
Conclusion: RadReason provides a clinically useful, interpretable evaluation framework that offers fine-grained error analysis with human-readable justifications, making it suitable for real-world clinical workflows.
...
<details>
  <summary>Details</summary>
Motivation: Personalized marketing can boost revenue by up to 40%, but current approaches focus mainly on recommendations and ads rather than personalized offer generation.

Method: Fine-tuned Google's T5-Small model using contrastive learning with InfoNCE loss to align customer personas with relevant offers in shared embedding space.
...
<details>
  <summary>Details</summary>
Motivation: Question whether large language models can adequately capture users' subjective browsing behaviors and preferences, which form unique "languages" without natural language structure.

Method: Introduces HeTLM (Heterogeneity aware Training of Language Model) using clusterwise training with page-level tokenizers and heterogeneous cluster-specific parameters.

Result: Small LMs with page tokenizers beat large pretrained/finetuned LMs; HeTLM outperforms single LMs with same parameter count; achieves higher mean and lower variance in generation.

Conclusion: Cluster-specific training improves alignment for subjective user behaviors, showing that specialized small models can outperform general large models for browsing pattern modeling.
...
<details>
  <summary>Details</summary>
Motivation: Conventional curriculum learning with human-centered difficulty metrics has shown limited success for pre-training language models. The authors investigate whether using a model-centric difficulty metric based on training data influence would make curriculum learning more effective.
...
Result: Models trained using this curriculum learning approach with training data influence metrics outperformed models trained in random order by over 10 percentage points in benchmarks.

Conclusion: Curriculum learning is beneficial for language model pre-training when using model-centric difficulty metrics rather than conventional human-centered difficulty measures.
...
Method: Developed SLM-Bench benchmark with standardized evaluation protocols, testing 15 SLMs on 9 NLP tasks using 23 datasets across 14 domains. Evaluation conducted on 4 hardware configurations with 11 metrics covering correctness, computation, and consumption.

Result: The benchmark revealed diverse trade-offs among SLMs - some excel in accuracy while others achieve superior energy efficiency. The evaluation provides a holistic assessment of efficiency trade-offs under controlled hardware conditions.

Conclusion: SLM-Bench sets a new standard for SLM evaluation by bridging the gap between resource efficiency and real-world applicability, with an open-source pipeline to facilitate reproducibility and further research.
...
<details>
  <summary>Details</summary>
Motivation: Existing identity detection datasets are predominantly English-centric, single-label, and use coarse identity categories, lacking nuanced multilingual social identity analysis in political contexts.
...
<details>
  <summary>Details</summary>
Motivation: To create a more powerful open diffusion language model that can overcome the sequential generation limitations of autoregressive models and provide better performance on general, mathematical, and coding tasks with enhanced flexibility.
...
Conclusion: Dream 7B represents the most powerful open diffusion language model to date, achieving state-of-the-art results through effective training techniques and offering flexible inference capabilities for various applications.
...
<details>
  <summary>Details</summary>
Motivation: To systematically analyze political delegitimization discourse - symbolic attacks on the normative validity of political entities - through computational methods to understand patterns and trends in democratic discourse.
...
Result: Best model achieved F1 of 0.74 for binary PDD detection and macro-F1 of 0.67 for classification characteristics. Found rising PDD over 30 years, higher prevalence on social media, greater use by male politicians, and stronger tendencies among right-leaning actors, with spikes during elections.

Conclusion: Automated PDD analysis is feasible and valuable for understanding democratic discourse, revealing systematic patterns in delegitimization rhetoric across different platforms and political contexts.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLM safety benchmarks rely on labor-intensive manual curation, causing excessive time/resource consumption, redundancy, and limited difficulty levels, creating a need for automated solutions.
...
Result: The system constructed SafetyFlowBench containing 23,446 queries with low redundancy and strong discriminative power, and evaluated 49 advanced LLMs on the dataset.
...
<details>
  <summary>Details</summary>
Motivation: LLMs have high computational costs that make them infeasible for targeted applications like sales and marketing outreach, despite their excellent text generation capabilities.

Method: Proposes using Small Language Models (SLMs) that are fine-tuned specifically for particular high-value domain applications rather than using general-purpose large models.

Result: The approach enables generating similar domain-specific responses for a fraction of the computational cost of large language models.

Conclusion: Trained Miniatures offer a cost-effective alternative to LLMs for targeted applications by leveraging specialized small models fine-tuned for specific domains.
...
Result: SDGO significantly improves model safety compared to prompt-based and training-based baselines, maintains helpfulness on general benchmarks, and provides robust performance against out-of-distribution jailbreaking attacks through tighter capability alignment.

Conclusion: Aligning LLMs' discrimination and generation capabilities enables robust safety improvements with minimal discriminative samples, creating a self-improving framework for enhanced jailbreaking resistance.
...
<details>
  <summary>Details</summary>
Motivation: Manual creation of scientific surveys is becoming infeasible due to rapid growth of academic literature, and progress in automated survey generation is hindered by lack of standardized benchmarks and evaluation protocols.

Method: Introduces SurGE benchmark with (1) test instances containing topic descriptions, expert-written surveys, and cited references, and (2) large-scale academic corpus of 1M+ papers as retrieval pool. Proposes automated evaluation framework across four dimensions: information coverage, referencing accuracy, structural organization, and content quality.
...
Conclusion: The findings demonstrate the need for continued research in automated scientific survey generation, and the authors have open-sourced all code, data, and models to facilitate further development in this area.
...
Method: Introduces Pos2Distill, a position-to-position knowledge distillation framework that leverages inherent position-induced disparity to counteract positional bias. Two specialized instantiations: Pos2Distill-R¹ for retrieval and Pos2Distill-R² for reasoning paradigms.

Result: Achieves enhanced uniformity and significant performance gains across all contextual positions in long-context retrieval and reasoning tasks. Both systems exhibit strong cross-task generalization while achieving superior performance on their respective tasks.

Conclusion: Pos2Distill effectively addresses positional bias by transferring superior capabilities from advantageous positions to less favorable ones, demonstrating improved performance and generalization across long-context tasks.
...
<details>
  <summary>Details</summary>
Motivation: Bangla is the 7th most spoken language but faces digital under-representation with limited annotated datasets. Stemming is crucial for this highly-inflectional language to reduce algorithm complexity.

Method: Comprehensive survey of existing Bangla stemming approaches, analysis of literature gaps, critique of evaluation methodologies, and identification of implementation challenges.

Result: Identified significant gaps in Bangla stemming research, discontinuity from previous studies, scarcity of accessible implementations, and inadequate evaluation metrics.

Conclusion: Advocates for robust Bangla stemmer development with better evaluation metrics and continued research to enhance language analysis and processing for this under-resourced language.
...
Method: Introduce EcomMMMU dataset with 406,190 samples and 8.9M images across 8 tasks, then propose SUMEI - a data-driven method that predicts visual utilities before using images for downstream tasks.

Result: Analysis shows product images do not consistently improve performance and can sometimes degrade it, indicating MLLMs struggle to effectively leverage rich visual content for e-commerce tasks.

Conclusion: SUMEI method demonstrates effectiveness and robustness in strategically utilizing multiple images, addressing the challenges identified in multimodal e-commerce understanding.
...
<details>
  <summary>Details</summary>
Motivation: Medical LLMs suffer from knowledge gaps and hallucinations, and current retrieval-augmented methods have limited impact due to weak external knowledge usage and poor feedback-reasoning traceability.

Method: End-to-end reinforcement learning training framework where LLM acts as agent and retrieval corpus as environment, with tailored rewards for format, retrieval, reasoning structure, and diagnostic accuracy.

Result: Substantially outperforms prompt-engineering and training-free RAG approaches, surpassing GPT-4o, DeepSeek-R1, and medical-specific frameworks in diagnostic accuracy for both common and rare diseases.

Conclusion: The approach provides more reliable and precise preliminary diagnoses with improved interpretability, demonstrating the effectiveness of agentic RL training for medical diagnostic systems.
...
TL;DR: LiveMCP-101 is a benchmark of 101 real-world queries requiring coordinated use of multiple MCP tools, with a novel evaluation approach using ground-truth execution plans rather than API outputs.
...
<details>
  <summary>Details</summary>
Motivation: Heatmap regression methods for facial landmark detection use Soft-argmax as a differentiable approximation for argmax, but this may not be the optimal approach. The authors want to explore alternative training objectives.
...
Conclusion: Soft-argmax is not the only viable approach for facial landmark detection, and structured prediction-based alternatives can provide faster convergence while maintaining strong performance, offering a new direction for this computer vision task.
...
<details>
  <summary>Details</summary>
Motivation: To improve image classification by leveraging GCNs' ability to handle complex relational data structures, moving beyond conventional CNNs to better model intricate scenes and fine-grained categories.

Method: Represent images as graphs where pixels/regions are vertices, refine graphs using Delaunay triangulations, and integrate GCNs with Voronoi diagrams for optimized representation.
...
TL;DR: YOPO is a single-stage, query-based framework that unifies object detection and 9-DoF pose estimation using only RGB images, achieving state-of-the-art performance without requiring depth data or CAD models.


<details>
  <summary>Details</summary>
Motivation: To create a simpler, RGB-only alternative for category-level 9-DoF pose estimation that eliminates the need for pseudo-depth, CAD models, or multi-stage cascades, while learning directly at the category level.

Method: Augments a transformer detector with a lightweight pose head, bounding-box-conditioned translation module, and 6D-aware Hungarian matching cost. Trained end-to-end with only RGB images and category-level pose labels.

Result: Sets new state-of-the-art on three benchmarks: 79.6% IoU50 and 54.1% under 10°10cm metric on REAL275 dataset, surpassing prior RGB-only methods and closing the gap to RGB-D systems.

Conclusion: YOPO demonstrates that object detection and 9-DoF pose estimation can be effectively unified in a single-stage framework using only RGB images, achieving high performance without additional data requirements.
...
<details>
  <summary>Details</summary>
Motivation: Traditional face recognition systems use separate models for physical and digital spoofing detection, which increases complexity, latency, and vulnerability to combined attacks. A unified approach is needed.
...
Conclusion: The proposed unified framework effectively handles both physical and digital spoofing attacks with superior performance and practical efficiency, making it suitable for real-world deployment.
...
TL;DR: TAIGen is a training-free black-box method that generates high-quality adversarial images using only 3-20 sampling steps from diffusion models, achieving strong attack success rates while being 10x faster than existing methods.
...
Method: TAIGen injects perturbations during the mixing step interval without processing all timesteps. It uses a selective RGB channel strategy: attention maps on red channel and GradCAM-guided perturbations on green/blue channels to preserve image structure while maximizing misclassification.

Result: TAIGen maintains visual quality with PSNR >30 dB. On ImageNet with VGGNet source: 70.6% success against ResNet, 80.8% against MNASNet, 97.8% against ShuffleNet. Generates adversarial examples 10x faster than existing diffusion-based attacks.
...
Method: Multi-stage deep network with three modules: CORE for mask domain reversible modeling, CARE for RGB domain enhancement, and FINE with targeted Bernoulli diffusion for uncertain region refinement.

Result: The approach efficiently directs focus to ambiguous areas, significantly reducing false positives/negatives while avoiding full-image computational costs.

Conclusion: RUN++ provides a principled bi-level optimization framework for robust CVP systems that work effectively under real-world degradations.
...
TL;DR: GasTwinFormer is a hybrid vision transformer for real-time methane emission segmentation and dietary classification in livestock monitoring, achieving 74.47% mIoU segmentation and 100% dietary classification accuracy with high efficiency.
...
Result: Achieves 74.47% mIoU and 83.63% mF1 for segmentation, perfect 100% dietary classification accuracy, with exceptional efficiency (3.348M parameters, 3.428G FLOPs, 114.9 FPS). First comprehensive beef cattle methane emission dataset with 11,694 annotated frames across three dietary treatments.

Conclusion: GasTwinFormer establishes a practical solution for real-time livestock emission monitoring, validated by extensive ablation studies, demonstrating effectiveness of leveraging diet-emission correlations.
...
TL;DR: CurveFlow introduces curvature-guided flow matching to improve text-to-image generation by learning non-linear trajectories that avoid low-probability regions, achieving state-of-the-art semantic consistency and image quality.
...
Method: CurveFlow framework incorporates curvature guidance directly into flow paths with robust curvature regularization that penalizes abrupt trajectory changes, enabling learning of smooth non-linear trajectories between data and noise distributions.

Result: Extensive experiments on MS COCO 2014/2017 show CurveFlow achieves SOTA performance, significantly outperforming standard rectified flow variants and non-linear baselines like Rectified Diffusion, with particular improvements in semantic consistency metrics (BLEU, METEOR, ROUGE, CLAIR).
...
Method: Combines ranking and contrastive learning with novel higher-order ranking loss, embedding distance loss, and training-time contrastive alignment guided by textual prompts. Uses only synthetic distortions for training.

Result: Achieves state-of-the-art performance on synthetic and authentic benchmarks, generalizes well to various distortions (lens flare, haze, motion blur, low-light), and offers real-time variant HiRQA-S with 3.5ms inference time.

Conclusion: HiRQA demonstrates strong generalization ability, scalability, and SOTA performance without requiring pristine references or subjective labels, making it effective for real-world deployment.
...
Result: Demonstrated benefits over traditional baseline strategies using publicly available and customized 3D datasets, showing guaranteed reconstruction quality satisfaction even with disruption-prone cameras.

Conclusion: The proposed portfolio-theoretic approach effectively manages edge resources to ensure reliable 3D reconstruction quality in dynamic edge environments with spatiotemporal disruptions, outperforming conventional strategies.
...
Result: Achieved state-of-the-art performance with 84.55% Balanced Accuracy and 79.92% F1 Score for disease diagnosis, plus 77.95% BACC and 66.88% F1 for concept detection. Human evaluations confirmed high fluency, accuracy, and clinical utility of explanations.
...
TL;DR: MeSS is a novel method for generating high-quality, style-consistent outdoor scenes using city mesh models as geometric priors, combining image diffusion models with 3D Gaussian Splatting for improved cross-view consistency and geometric alignment.


<details>
  <summary>Details</summary>
Motivation: City mesh models lack realistic textures, limiting their use in virtual urban navigation and autonomous driving. Existing image/video diffusion models either struggle with camera path adherence or lack cross-view consistency when applied to 3D scene generation.
...
Conclusion: MeSS successfully addresses the texture limitation of city mesh models by providing a robust framework for generating high-quality, consistent 3D scenes that can be rendered in various styles for urban applications.
...
<details>
  <summary>Details</summary>
Motivation: In safety-critical applications such as autonomous driving, it's crucial to properly model rule-based surrounding agents to ensure optimal behavior and safety. Existing approaches train agents with rule-based surrounding agents, but these models need thorough testing against adversarial scenarios.

Method: The paper presents a learning-based method to derive adversarial behavior for rule-based agents. This approach creates failure scenarios by training adversarial agents that can exploit weaknesses in the existing rule-based models and IDM (Intelligent Driver Model) strategies.

Result: The adversarial agent was evaluated against all rule-based agents and demonstrated a significant decrease in cumulative reward, indicating successful creation of failure scenarios and exposing vulnerabilities in the existing rule-based modeling approaches.

Conclusion: The learning-based adversarial approach effectively identifies weaknesses in rule-based agent modeling for safety-critical applications, providing valuable insights for improving the robustness and safety of autonomous systems like self-driving cars.
...
TL;DR: DyMorph-B2I is a dynamic morphology-guided pipeline that converts binary renal pathology masks to instance-level segmentation using integrated watershed, skeletonization, and morphological operations with adaptive refinement.
...
Method: Integrated watershed, skeletonization, and morphological operations within a unified framework with adaptive geometric refinement and class-specific hyperparameter tuning through systematic parameter optimization.

Result: Outperforms individual classical approaches and naive combinations, achieving robust separation of adherent and heterogeneous structures in binary masks with superior instance separation.

Conclusion: Enables more accurate morphometric analysis in renal pathology workflows and is publicly available as an open-source pipeline.
...
<details>
  <summary>Details</summary>
Motivation: To develop a cost-effective and easily deployable accident prediction system using only dash-cam video input, overcoming the limitations of sensor-dependent systems that require LiDAR, radar, and GPS.
...
Result: Achieves higher average precision and mean time-to-collision values than previous methods across three publicly available datasets, demonstrating effectiveness both in cross-validation and cross-dataset testing.

Conclusion: The proposed STAGNet model provides a superior and more practical solution for accident prediction using only dash-cam video, making advanced driver assistance systems more accessible and cost-effective.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D generative models operate in single-modality paradigms, missing the complementary benefits of multi-modal data. RGB images provide rich textures while point clouds offer fine-grained geometries, but current approaches don't holistically combine these advantages.

Method: 1) Collaborative multi-modal coding that integrates modality-specific features while preserving their unique strengths. 2) Auxiliary 2D and 3D supervision to enhance robustness. 3) Triplane latent diffusion model for generating high-quality 3D assets with improved texture and geometric detail.

Result: Extensive experiments show TriMM achieves competitive performance with models trained on large-scale datasets despite using small training data. Additional experiments confirm feasibility of incorporating other multi-modal datasets like RGB-D into 3D generation.

Conclusion: TriMM successfully demonstrates the value of multi-modal learning for 3D generation, effectively combining complementary modalities to produce superior 3D assets with both high-quality textures and detailed geometries using minimal training data.
...
<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods for clustering suffer from prototype deviation and inter-class conflict problems, where hard prototypes don't accurately represent true cluster centers.

Method: Proposes a framework with: 1) Soft prototype contrastive module using sample-to-center probability weights to calculate category prototypes, 2) Dual consistency learning module that aligns different transformations of same samples and neighborhoods of different samples.

Result: Extensive experiments on five datasets show the method is effective compared to state-of-the-art approaches, reducing prototype drift and inter-class conflicts.

Conclusion: The proposed center-oriented prototype contrastive clustering framework successfully addresses prototype calculation deviations and inter-class conflict problems, providing more accurate clustering through soft weighting and consistency learning.
...
TL;DR: Dual-UAV collaborative VLN framework with high-altitude UAV for environmental reasoning and low-altitude UAV for precise navigation, using minimal coordinate exchange.


<details>
  <summary>Details</summary>
Motivation: Address challenges in aerial VLN with extended trajectories and complex UAV maneuverability by leveraging multi-altitude perspectives while maintaining manageable motion space.

Method: Propose AeroDuo framework: high-altitude UAV uses multimodal LLM (Pilot-LLM) for target reasoning, low-altitude UAV uses lightweight multi-stage policy for navigation and target grounding, with minimal coordinate exchange.

Result: Created HaL-13k dataset with 13,838 collaborative trajectories and target-oriented instructions, including unseen maps and objects for generalization evaluation.

Conclusion: Dual-altitude UAV collaboration enables effective aerial VLN by combining broad environmental reasoning with precise navigation while maintaining efficiency through minimal information exchange.
...
TL;DR: Skipped-step sampling enables faster diffusion model generation by bypassing intermediate steps while maintaining quality, showing acceleration is an intrinsic property of pretrained models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models require sequential generation which is slow. Existing methods like DDIM use non-Markovian processes, but it's unclear if original diffusion processes can achieve similar efficiency.
...
Result: Achieves high-quality generation with significantly reduced sampling steps across OpenAI ADM, Stable Diffusion, and Open Sora models.
...
TL;DR: Comp-X is the first LLM-powered interactive image compression system that understands user requests and intelligently selects coding modes, maintaining competitive compression performance while being user-friendly.


<details>
  <summary>Details</summary>
Motivation: Traditional image codecs have limited coding modes and require manual mode selection by engineers, making them unfriendly for unprofessional users who need to compress images for various purposes.

Method: Three key innovations: 1) Multi-functional coding framework unifying different coding modes, 2) Interactive coding agent using augmented in-context learning with expert feedback, 3) IIC-bench benchmark for evaluation with diverse user requests and expert annotations.

Result: Comp-X efficiently understands coding requests and achieves impressive textual interaction capability while maintaining comparable compression performance with a single coding framework.

Conclusion: The approach provides a promising avenue for applying artificial general intelligence (AGI) to image compression, making compression more accessible and intelligent for all users.
...
TL;DR: Ano-NAViLa is a vision-language model that incorporates both normal and abnormal pathology knowledge for improved anomaly detection in computational pathology, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly detection methods designed for industrial settings face limitations in pathology due to computational constraints, diverse tissue structures, and lack of interpretability, especially with limited disease-related data.
...
Conclusion: Ano-NAViLa effectively addresses the challenges of anomaly detection in computational pathology by leveraging pathology-specific knowledge through vision-language modeling, providing both superior performance and interpretability.
...
TL;DR: RATopo introduces a redundancy assignment strategy for lane topology reasoning that enables one-to-many supervision instead of traditional one-to-one assignment, improving topology reasoning performance in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Existing lane topology reasoning methods use first-detect-then-reason paradigm with one-to-one assignment supervision, which results in limited valid supervision range and suboptimal performance due to restricted supervision signals.

Method: Restructures Transformer decoder by swapping cross-attention and self-attention layers to retain redundant lane predictions before suppression, enabling one-to-many assignment. Uses multiple parallel cross-attention blocks with independent parameters to enhance lane detection diversity.

Result: Extensive experiments on OpenLane-V2 show RATopo is model-agnostic and seamlessly integrates into existing topology reasoning frameworks, consistently improving both lane-lane and lane-traffic topology performance.

Conclusion: The proposed redundancy assignment strategy provides quantity-rich and geometry-diverse topology supervision, overcoming limitations of traditional one-to-one assignment approaches and significantly enhancing lane topology reasoning capabilities.
...
TL;DR: DesignCLIP leverages CLIP models for design patent analysis, addressing limitations of traditional image-based methods by incorporating multimodal learning with generated captions and multi-view image learning, achieving state-of-the-art performance in classification and retrieval tasks.
...
Method: Developed DesignCLIP framework using CLIP models with class-aware classification and contrastive learning. Utilized generated detailed captions for patent images and multi-view image learning to address unique patent data characteristics.
...
Conclusion: Multimodal approaches show significant promise in advancing patent analysis, with DesignCLIP demonstrating superior performance across various downstream tasks in the design patent domain.
...
TL;DR: TPA is a novel framework for fetal congenital heart defect classification in ultrasound videos that combines temporal modeling, prompt-aware contrastive learning, and uncertainty quantification to improve accuracy and calibration.
...
Conclusion: TPA effectively addresses limitations of current methods by integrating temporal modeling, prompt-aware learning, and uncertainty quantification, demonstrating superior performance and clinical reliability for CHD detection in ultrasound videos.
...
Method: Created BasketLiDAR dataset with 4,445 frames and 3,105 player IDs from professional games. Developed novel MOT algorithm using LiDAR's high-precision 3D spatial information, featuring both LiDAR-only real-time tracking and multimodal fusion with camera data.

Result: Achieves real-time operation (previously difficult with camera-only methods) and superior tracking performance even under occlusion conditions. Dataset includes synchronized data from 3 LiDAR sensors and 3 multi-view cameras.

Conclusion: The multimodal LiDAR-camera approach successfully addresses basketball tracking challenges, providing both real-time capability and improved accuracy for sports MOT applications.
...
<details>
  <summary>Details</summary>
Motivation: Address privacy, copyright, and safety concerns in text-to-video diffusion models that can generate harmful or misleading content from unauthorized training data.
...
Result: Achieves 46% average reduction in undesirable content across four tasks (object, style, celebrity, explicit content erasure) and outperforms prior methods in efficacy, integrity, fidelity, robustness, and generalizability.

Conclusion: VideoEraser provides an effective training-free solution for content safety in text-to-video generation, achieving state-of-the-art performance in suppressing undesirable concepts.
...
Method: Video analysis with pose detection models followed by sequence modeling techniques (GRU, LSTM, 1D CNN) for temporal predictions in an end-to-end deep learning framework.
...
Conclusion: The study successfully developed a deep learning framework for pedestrian intent prediction, with different sequence models offering trade-offs between accuracy and speed for autonomous vehicle applications.
...
TL;DR: RCDINO is a multimodal transformer model that fuses camera and radar data with DINOv2 features for 3D object detection, achieving state-of-the-art results on nuScenes dataset.
...
TL;DR: This paper presents a systematic empirical study using attention knockouts to understand how Video Large Language Models (Video-LLMs) internally process video content, revealing key insights about layer functionality and spatial-temporal modeling that can be leveraged for efficiency improvements.


<details>
  <summary>Details</summary>
Motivation: Most existing Video-LLM research focuses on performance improvement rather than understanding internal mechanisms. This paper aims to bridge this gap by systematically analyzing how these models process and understand video content internally.

Method: The authors use attention knockouts as the primary analytical tool, designing three variants: Video Temporal Knockout, Video Spatial Knockout, and Language-to-Video Knockout. They apply these knockouts across different layers in two settings: global setting (controlling window of layers) and fine-grained setting.

Result: Three key findings: (1) Video information extraction primarily occurs in early layers with a two-stage process (perceptual encoding in lower layers, abstract reasoning in higher layers); (2) Certain intermediate layers have outsized impact as critical outliers while most layers contribute minimally; (3) Spatial-temporal modeling relies more on language-guided retrieval than on computationally expensive intra- and inter-frame self-attention.

Conclusion: This is the first systematic work uncovering how Video-LLMs internally process video content. The insights provide both interpretability and can be leveraged to reduce attention computation, offering efficiency perspectives for future Video-LLM research.
...
<details>
  <summary>Details</summary>
Motivation: Traditional fine-tuning updates all model parameters, leading to potential overfitting and high computational costs. There's a need for more efficient transfer learning methods that maintain performance while reducing resource requirements.

Method: BioTune uses an evolutionary algorithm to identify and selectively fine-tune a focused subset of layers in pre-trained models, optimizing performance for target tasks while minimizing trainable parameters.
...
Conclusion: BioTune demonstrates that selective layer fine-tuning through evolutionary optimization enables more efficient transfer learning while maintaining performance across diverse datasets and domains.
...
Method: Proposes Image-Conditioned Gaussian Splat Quantizer that jointly exploits inter-Gaussian and inter-attribute correlations, uses shared codebooks across scenes, and conditions scene decoding on images captured at decoding time for adaptability to post-archival changes.
...
Conclusion: The method successfully addresses both compression efficiency and post-archival adaptability limitations of existing 3DGS compression approaches, making it suitable for practical archival and deployment on storage-constrained devices.
...
TL;DR: DriveSplat is a 3D scene reconstruction method for driving scenarios that improves novel-view synthesis by using neural Gaussian representations with dynamic-static decoupling, region-wise voxel initialization, and deformable neural Gaussians supervised by depth/normal priors.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian Splatting methods for driving scenarios overlook background optimization with proper geometry relationships and rely solely on fitting training views, resulting in limited robustness for novel views and inaccurate geometric representations.
...
TL;DR: This paper addresses the abstract reasoning bottleneck in deep learning by focusing on Raven's Progressive Matrices problems, proposing causal chain modeling and improvements to overcome limitations in mutual information optimization.


<details>
  <summary>Details</summary>
Motivation: Despite strong performance in various domains, deep learning models still struggle with abstract reasoning. RPM problems serve as an authoritative benchmark to evaluate and enhance abstract reasoning, pattern recognition, and complex problem-solving capabilities in machine intelligence.

Method: The paper adopts a causal chain modeling perspective to analyze RPM tasks, designs the DIO baseline network architecture, and progressively proposes three improvement methods to address limitations in mutual information optimization (tightness of lower bound and inability to capture causal relationships).

Result: Experiments reveal that the initial optimization objective (maximizing variational lower bound of mutual information between context and correct option) fails to enable the model to genuinely acquire human reasoning logic, leading to the development of improved methods.

Conclusion: The paper contributes to enhancing abstract reasoning abilities in machine intelligence by systematically analyzing RPM tasks through causal chain modeling and developing improved approaches to overcome the limitations of mutual information-based optimization methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing video summarization methods struggle with global temporal dependencies, semantic coherence, and noise during multi-channel feature fusion, requiring more efficient and robust solutions.
...
Result: Experimental results show SpiVG surpasses existing methods across multiple datasets including SumMe, TVSum, VideoXum, and QFVS.

Conclusion: SpiVG Network effectively addresses challenges in video summarization by combining spiking neural networks with variational inference, achieving superior performance while enhancing information density and reducing computational complexity.
...
TL;DR: This paper analyzes how Masked Autoencoders (MAEs) learn spatial correlations in images, showing that masking ratio and patch size control the range of spatial features captured, and provides practical guidance for hyperparameter selection.


<details>
  <summary>Details</summary>
Motivation: MAEs require extensive hyperparameter tuning for new datasets, but the connection between these hyperparameters and downstream performance is not well understood theoretically.
...
Result: Masking ratio and patch size can be used to select for features capturing short- and long-range spatial correlations, with MAE representations adapting to dataset-specific spatial correlation patterns.

Conclusion: The analysis provides theoretical insights and practical guidance for selecting MAE hyperparameters based on the spatial correlation properties of target datasets.
...
TL;DR: BIRD is a bidirectional temporal information propagation method for infrared small target detection that simultaneously utilizes local and global temporal information across entire video clips, achieving state-of-the-art performance with fast inference speed.
...
Method: Proposes BIRD with bidirectional propagation strategy using Local Temporal Motion Fusion (LTMF) for adjacent frame modeling and Global Temporal Motion Fusion (GTMF) for global feature aggregation. Uses joint optimization with detection loss and Spatio-Temporal Fusion loss.

Result: Extensive experiments demonstrate state-of-the-art performance with fast inference speed.

Conclusion: BIRD effectively addresses limitations of sliding-window methods by leveraging both local and global temporal information through bidirectional propagation, achieving superior detection performance.
...
TL;DR: YOLOv8-based deep learning system for automated detection of microscopic car surface dents, achieving high precision (0.86) and recall (0.84) with custom dataset and real-time augmentation.
...
Result: YOLOv8m-t42 achieved precision 0.86, recall 0.84, F1-score 0.85, mAP@0.5 of 0.60, and PR curve area of 0.88, outperforming YOLOv8m-t4 model.

Conclusion: The deep learning approach provides accurate, real-time detection of microscopic car surface defects, making it suitable for automated insurance assessments and vehicle inspections.
...
Result: Achieves 13.1% R@1 and 8.1% mIoU improvement on ActivityNet-VRL, and 14.7% R@1 and 14.4% mIoU improvement on SportsMoments dataset over state-of-the-art methods.
...
TL;DR: A novel SfM-free 3D Gaussian Splatting method that jointly estimates camera poses and reconstructs 3D scenes from extremely sparse inputs (only 2 views), achieving significant quality improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: 3DGS requires dense multi-view inputs with precise camera poses, which are rarely available in real-world scenarios. When inputs are extremely sparse, traditional SfM initialization fails, leading to degraded rendering quality.

Method: Proposes dense stereo module for progressive camera pose estimation and global dense point cloud initialization. Includes coherent view interpolation module for generating additional supervision, plus multi-scale Laplacian regularization and adaptive spatial-aware geometry regularization.

Result: Achieves 2.75dB PSNR improvement under extremely sparse-view conditions (only 2 training views). Produces images with minimal distortion while preserving rich high-frequency details and superior visual quality.

Conclusion: The method successfully addresses the limitations of 3DGS in sparse-view scenarios by eliminating SfM dependency and introducing novel regularization techniques, significantly outperforming state-of-the-art approaches.
...
TL;DR: LGMSNet is a lightweight medical image segmentation framework that combines heterogeneous convolutional kernels and sparse transformer-convolutional hybrid branches to achieve state-of-the-art performance with minimal computational overhead, showing strong zero-shot generalization capabilities.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of existing lightweight medical image segmentation models that compromise performance for efficiency, lack global contextual perception due to avoiding attention mechanisms, and suffer from channel redundancy issues in medical imaging.
...
Result: Extensive experiments on six public datasets show LGMSNet outperforms state-of-the-art methods. It maintains exceptional performance in zero-shot generalization tests on four unseen datasets.
...
TL;DR: MExECON is a multi-view 3D reconstruction pipeline for clothed human avatars that extends ECON to leverage multiple viewpoints, improving geometry and pose estimation without retraining.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of single-view reconstruction by leveraging multiple viewpoints for better geometry and body pose estimation of clothed human avatars.
...
Conclusion: MExECON successfully extends single-view reconstruction to multi-view scenarios, enabling more accurate 3D avatar reconstruction without requiring network retraining.
...
TL;DR: AdaSFFuse is a novel multimodal image fusion framework that uses adaptive wavelet transform and spatial-frequency mamba blocks to improve cross-domain fusion while maintaining computational efficiency across multiple image fusion tasks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal image fusion methods face challenges with modality misalignment, high-frequency detail destruction, and task-specific limitations, requiring a more generalized and efficient solution.

Method: Proposes AdaSFFuse framework with Adaptive Approximate Wavelet Transform (AdaWAT) for frequency decoupling and Spatial-Frequency Mamba Blocks for cross-domain fusion in both spatial and frequency domains.

Result: Superior fusion performance demonstrated across four MMIF tasks (Infrared-Visible, Multi-Focus, Multi-Exposure, and Medical Image Fusion) with low computational cost and compact network architecture.

Conclusion: AdaSFFuse provides an effective solution for task-generalized multimodal image fusion, achieving strong balance between performance and efficiency while preserving critical details and reducing frequency loss.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for synthesizing extrapolated driving views from recorded logs often suffer from poor geometric consistency and over-smoothed renderings when using generative priors as pseudo ground truth.

Method: Proposes ExtraGS with Road Surface Gaussian representation based on hybrid Gaussian-SDF design, Far Field Gaussians with learnable scaling factors, and self-supervised uncertainty estimation framework using spherical harmonics for selective generative prior integration.
...
TL;DR: GroupSketch is a novel two-stage method for vector sketch animation that handles multi-object interactions and complex motions through motion initialization and refinement using a Group-based Displacement Network.


<details>
  <summary>Details</summary>
Motivation: Existing sketch animation approaches struggle with multi-object interactions and complex motions, being limited to single-object cases or suffering from temporal inconsistency and poor generalization.

Method: Two-stage pipeline: 1) Motion Initialization - interactively divide sketch into semantic groups and define key frames for coarse animation via interpolation; 2) Motion Refinement - use Group-based Displacement Network (GDN) with Context-conditioned Feature Enhancement (CCFE) to refine animation by predicting group-specific displacement fields using text-to-video model priors.
...
TL;DR: D3FNet is a novel network for extracting narrow roads from remote sensing imagery using dilated dual-stream differential attention fusion to handle fragmented topology and occlusions.


<details>
  <summary>Details</summary>
Motivation: Extracting narrow roads from high-resolution remote sensing imagery is challenging due to limited width, fragmented topology, and frequent occlusions that conventional models struggle with.

Method: Proposes D3FNet with three innovations: 1) Differential Attention Dilation Extraction module for enhanced road features, 2) Dual-stream Decoding Fusion Mechanism for spatial-semantic balance, and 3) multi-scale dilation strategy (rates 1,3,5,9) to reduce artifacts.
...
Conclusion: D3FNet provides a robust solution for fine-grained narrow road extraction in complex remote and cooperative perception scenarios, with verified complementary synergy of attention-guided encoding and dual-path decoding.
...
TL;DR: ADAPT is a test-time adaptation method that models class-conditional distributions using Gaussian probabilistic inference, eliminating backpropagation and enabling closed-form, training-free adaptation with CLIP-guided regularization.


<details>
  <summary>Details</summary>
Motivation: Current test-time adaptation methods rely on backpropagation or iterative optimization, limiting scalability and real-time deployment. They also lack explicit modeling of class-conditional feature distributions, which is crucial for reliable decision boundaries and calibrated predictions.

Method: Reframes TTA as Gaussian probabilistic inference by modeling class-conditional likelihoods using gradually updated class means and shared covariance matrix. Uses lightweight regularization guided by CLIP priors and historical knowledge bank. Requires no source data, gradient updates, or full access to target data.
...
Conclusion: ADAPT provides an effective backpropagation-free solution for test-time adaptation that explicitly models class distributions, enabling reliable performance in both online and transductive settings without requiring source data or gradient computations.
...
<details>
  <summary>Details</summary>
Motivation: Implicit Neural Representations struggle with spectral bias, favoring low-frequency components and missing high-frequency details like sharp edges and fine textures, which existing architectural modifications don't fully address.

Method: Proposes a two-stage training approach: first uses a neighbor-aware soft mask that assigns higher weights to pixels with strong local variations to focus on fine details early, then transitions to full-image training.
...
Conclusion: This work pioneers frequency-aware pixel importance assignment in image INR, offering a new avenue for mitigating spectral bias problems through direct training guidance rather than architectural changes.
...
<details>
  <summary>Details</summary>
Motivation: Existing TLS-based point cloud registration methods using SDP relaxations are too slow (hundreds of seconds for 100 points), making real-time applications impractical despite robustness to 95% outlier rates.

Method: Proposes a linear time convex relaxation approach combined with a contractor method to accelerate Branch and Bound (BnB) optimization for solving the truncated least squares formulation.

Result: The solver achieves provable global optimality in less than 0.5 seconds for 100-point 3D registration with known rotation axis, demonstrating two orders of magnitude speed improvement over STRIDE SDP solver for rotation-only problems.

Conclusion: The method provides both formal proofs and empirical evidence of global optimality while dramatically improving computational efficiency, though currently limited to problems with known rotation axis rather than full 6DoF registration.
...
TL;DR: This study combines camera traps and drone thermal imagery with deep learning models to monitor wildlife and human activities in Chitwan National Park, Nepal, identifying spatial overlap and potential human-wildlife conflict zones.
...
Method: Combined visible/NIR camera traps and thermal infrared drone imagery from Feb-Jul 2022, processed with deep learning models (YOLOv11s for camera traps, enhanced Faster R-CNN for drone imagery) for automated detection, followed by spatial pattern analysis to identify hotspots and conflict zones.

Result: YOLOv11s achieved highest performance (96.2% precision, 92.3% recall, 96.7% mAP50, 81.3% mAP50). Spatial analysis identified clear wildlife and human activity hotspots with overlapping patterns indicating potential conflict zones in CNP and buffer areas.
...
TL;DR: Grounded VideoDiT is a Video LLM that improves temporal perception and entity grounding in video understanding through diffusion temporal encoding, object-grounded representations, and explicit timestamp modeling.


<details>
  <summary>Details</summary>
Motivation: Current Video LLMs lack fine-grained temporal perception - timestamps are implicit, frame features are weak for continuity, and language-vision alignment drifts from key entities.

Method: Three key innovations: 1) Diffusion Temporal Latent encoder for boundary sensitivity and consistency, 2) Object-grounded representations binding queries to visual evidence, 3) Mixed token scheme with discrete temporal tokens for explicit timestamp modeling.
...
Conclusion: The proposed Grounded VideoDiT successfully overcomes limitations in temporal perception and entity alignment, enabling fine-grained temporal reasoning in video understanding.
...
TL;DR: Weakly supervised approach for tree instance segmentation in ALS data using human quality ratings to train a rating model, which then improves segmentation performance by 34% in correctly identified trees.
...
Conclusion: The weakly supervised approach effectively improves tree segmentation accuracy using human quality assessment feedback, though challenges remain in complex environments with small trees and confusing features.
...
<details>
  <summary>Details</summary>
Motivation: Previous 3D adversarial attack methods require information about target models, which is often unavailable in realistic security scenarios. The paper addresses the need for effective transfer-based attacks that work without any target model information.

Method: Proposes Critical Feature Guidance (CFG) that regularizes adversarial search by computing feature importance and prioritizing corruption of critical features likely used by diverse architectures. Explicitly constrains maximum deviation for imperceptibility in the loss function.

Result: Extensive experiments on ModelNet40 and ScanObjectNN benchmarks show CFG outperforms state-of-the-art attack methods by a large margin in transferability.

Conclusion: CFG successfully demonstrates that focusing on critical features common across architectures enables highly transferable adversarial attacks on 3D point clouds without requiring target model information, while maintaining imperceptibility.
...
TL;DR: MapKD uses knowledge distillation to transfer knowledge from multimodal teacher models to a lightweight vision-only student model for online HD map construction, achieving significant performance improvements while speeding up inference.


<details>
  <summary>Details</summary>
Motivation: Existing online HD map construction methods depend on stale offline maps and multi-modal sensors, causing computational overhead. There's a need for efficient, low-cost vision-centric models that maintain high performance.

Method: Proposes MapKD framework with Teacher-Coach-Student paradigm: 1) multimodal teacher with map priors, 2) vision-centric coach with simulated LiDAR to bridge modality gap, 3) lightweight student. Uses Token-Guided 2D Patch Distillation and Masked Semantic Response Distillation strategies.
...
Conclusion: MapKD effectively transfers knowledge from multimodal models to vision-only student through innovative distillation framework, enabling efficient and high-performance online HD map construction without dependency on expensive sensors or offline maps.
...
<details>
  <summary>Details</summary>
Motivation: Existing LoD1 and LoD2 building models lack detailed facade elements essential for advanced urban analysis, while traditional LoD3 model generation requires manual modeling which is not scalable for large-scale applications.

Method: Leverages Conflict Maps (CMs) from ray-to-model-prior analysis, uses synthetically generated CMs from Semantic Conflict Map Generator (SCMG) for semantic segmentation, and fuses additional segmentation of textured models using confidence scores to improve performance.
...
TL;DR: MDPR framework addresses class imbalance in VLM fine-tuning by creating multi-dimensional knowledge base and dynamic prompt routing to balance semantics and achieve stable predictions.


<details>
  <summary>Details</summary>
Motivation: Existing VLM fine-tuning methods suffer from bias in class-imbalanced scenes and overlook inherent class imbalance in VLMs' pre-training, leading to bias accumulation in downstream tasks.

Method: Proposes Multi-dimensional Dynamic Prompt Routing (MDPR) framework that constructs comprehensive knowledge base spanning five visual-semantic dimensions, uses dynamic routing to align global visual classes, retrieve optimal prompts, and balance fine-grained semantics with logits fusion.

Result: Achieves comparable results with current SOTA methods on long-tailed benchmarks (CIFAR-LT, ImageNet-LT, Places-LT), with minimal computational overhead and effectiveness for tail classes.

Conclusion: MDPR provides a flexible and efficient enhancement for VLM fine-tuning under data imbalance conditions through semantic library and dynamic routing mechanism.
...
<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with long video processing due to substantial memory and computational overhead from KV cache storage. Current visual compression methods require encoding entire visual contexts beforehand or advance knowledge of questions, which is impractical for real-time long video understanding and multi-turn conversations.

Method: StreamMem encodes new video frames in a streaming manner and compresses KV cache using attention scores between visual tokens and generic query tokens. It maintains a fixed-size KV memory to enable efficient question answering in memory-constrained scenarios.

Result: Evaluation on three long video understanding and two streaming video QA benchmarks shows StreamMem achieves state-of-the-art performance in query-agnostic KV cache compression and is competitive with query-aware compression approaches.

Conclusion: StreamMem provides an effective solution for efficient long video processing in MLLMs by enabling streaming video encoding and query-agnostic KV cache compression, making it practical for real-world applications with memory constraints.
...
<details>
  <summary>Details</summary>
Motivation: Current video generation methods rely heavily on RGB signals, which leads to accumulated errors in object structure and motion over long sequences. The lack of robust structural priors causes temporal inconsistency and drift in extended video generation.

Method: The framework jointly predicts perceptual conditions and color information from unified representations, leverages depth cues (more resistant to drift) to build a memory bank for clearer context, and uses segmented noise scheduling for training prediction groups to reduce computational costs.
...
Conclusion: WorldWeaver provides a robust solution for long video generation by integrating perceptual conditions with RGB modeling, significantly enhancing temporal consistency and structural integrity while being computationally efficient.
...
<details>
  <summary>Details</summary>
Motivation: Vision foundation models like SAM face challenges with fine-grained semantic segmentation of specific nuclei subtypes and cells in computational pathology.

Method: Full-stack approach with: (1) molecular-empowered learning to reduce pixel-level annotations, (2) SAM adapter for semantic adaptation, (3) Molecular-Oriented Corrective Learning (MOCL) for refinement.

Result: Significantly improves cell classification performance on both in-house and public datasets, even with varying annotation quality.
...
TL;DR: Waver is a high-performance foundation model for unified image and video generation that supports text-to-video, image-to-video, and text-to-image generation in a single framework, achieving top-3 rankings on benchmark leaderboards.


<details>
  <summary>Details</summary>
Motivation: To create a unified framework for high-quality image and video generation that can handle multiple modalities (T2V, I2V, T2I) simultaneously while addressing challenges in motion capture, temporal consistency, and training efficiency.

Method: Developed a Hybrid Stream DiT architecture for better modality alignment and faster training convergence. Implemented comprehensive data curation pipeline with MLLM-based video quality filtering. Provides detailed training and inference recipes.

Result: Generates 5-10 second videos at 720p native resolution (upscaled to 1080p). Achieved Top 3 rankings on both T2V and I2V leaderboards at Artificial Analysis. Outperforms open-source models and matches/commercial solutions in complex motion capture and temporal consistency.

Conclusion: Waver represents a significant advancement in unified image/video generation, providing high-quality results across multiple modalities. The technical contributions and open recipes aim to accelerate community progress in video generation technologies.
...
TL;DR: ATLAS is a high-fidelity parametric body model that decouples shape and skeleton representation, enabling more accurate 3D human modeling with enhanced expressivity and independent control over body attributes.


<details>
  <summary>Details</summary>
Motivation: Existing human mesh models struggle with capturing detailed variations across diverse poses and shapes due to limited training data diversity and problematic dependencies between internal skeleton and external soft tissue.

Method: ATLAS learns from 600k high-resolution scans using 240 synchronized cameras, explicitly decoupling shape and skeleton bases by grounding mesh representation in the human skeleton structure.

Result: ATLAS outperforms existing methods in fitting unseen subjects across diverse poses, with quantitative evaluations showing non-linear pose correctives capture complex poses more effectively than linear models.

Conclusion: The decoupled skeleton-shape representation enables enhanced shape expressivity, fine-grained body attribute customization, and keypoint fitting independent of soft-tissue characteristics, representing a significant advancement in parametric body modeling.
...
Method: Uses a feature aggregation module integrating local and global scene information from visual and geometric encoders, coupled with a position head for generating 3D assets and their spatial positions simultaneously.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from global denoising that entangles edited regions with entire image context, leading to unintended modifications and poor adherence to editing instructions. Autoregressive models offer a causal and compositional alternative.

Method: VAREdit formulates image editing as sequential next-scale prediction over discrete visual tokens. It uses a Scale-Aligned Reference (SAR) module to inject scale-matched source image conditioning into the first self-attention layer for effective guidance.
...
Conclusion: The autoregressive approach with scale-aligned conditioning provides a more precise and efficient paradigm for instruction-guided image editing, addressing the fundamental limitations of diffusion-based methods.
...
Method: Formulates group inference as a quadratic integer assignment problem where candidate outputs are graph nodes, optimizing both sample quality (unary term) and group diversity (binary term) with progressive candidate pruning for efficiency.
...
TL;DR: CineScale is a novel inference paradigm that enables high-resolution visual generation (up to 8k images and 4k videos) without fine-tuning, addressing repetitive pattern issues in diffusion models when generating beyond their training resolutions.


<details>
  <summary>Details</summary>
Motivation: Visual diffusion models are typically trained at limited resolutions due to data and computational constraints, leading to low-quality repetitive patterns when generating higher-resolution content due to accumulated high-frequency information errors.

Method: CineScale proposes dedicated variants tailored to different video generation architectures (T2I, T2V, I2V, V2V) to tackle resolution scaling issues, built on state-of-the-art open-source frameworks without requiring fine-tuning.
...
Conclusion: CineScale successfully extends the capabilities of pre-trained models for high-resolution visual generation across multiple modalities, overcoming the repetitive pattern problem and demonstrating practical scalability to ultra-high resolutions.
...
TL;DR: Proposes integrating GenAI's cognitive capabilities with traditional software engineering to create robust, adaptive GenAI-native systems through five key design pillars and architectural patterns.


<details>
  <summary>Details</summary>
Motivation: GenAI faces challenges in reliability and efficiency due to unpredictability, requiring a paradigm shift to combine AI capabilities with software engineering principles for better system development.
...
Conclusion: This work aims to inspire future research and encourage communities to implement and refine the proposed GenAI-native framework for creating resilient, self-evolving systems.
...
### [121] [Adversarial Attacks against Neural Ranking Models via In-Context Learning](https://arxiv.org/abs/2508.15283)
*Amin Bigdeli,Negar Arabzadeh,Ebrahim Bagheri,Charles L. A. Clarke*

Main category: cs.IR

TL;DR: FSAP is a black-box attack framework that uses few-shot prompting with LLMs to generate high-ranking adversarial documents that can outrank authentic content in neural ranking systems.


<details>
  <summary>Details</summary>
Motivation: Neural ranking models are vulnerable to adversarial manipulation, and existing approaches rely on token-level perturbations or manual rewriting. FSAP aims to create a more effective and scalable attack method using LLMs' in-context learning capabilities.

Method: FSAP uses few-shot prompting with LLMs, conditioning them on a small support set of harmful examples. It operates in two modes: FSAP-IntraQ (same query examples for topic fidelity) and FSAP-InterQ (cross-query generalization). No gradient access or model instrumentation required.

Result: FSAP-generated documents consistently outrank credible documents across four neural ranking models on TREC 2020-2021 Health Misinformation Tracks. The adversarial outputs show strong stance alignment, low detectability, and generalize well across both proprietary and open-source LLMs.

Conclusion: FSAP poses a realistic and scalable threat to neural retrieval systems by effectively generating fluent, coherent adversarial documents that rank competitively against authentic content through few-shot prompting.
...
### [122] [On the Effectiveness of Graph Reordering for Accelerating Approximate Nearest Neighbor Search on GPU](https://arxiv.org/abs/2508.15436)
...
<details>
  <summary>Details</summary>
Motivation: Graph-based ANNS dominates modern AI applications but recent approaches focus on algorithmic innovations while neglecting memory layout considerations that significantly impact execution time.
...
Result: GPU-targeted reordering achieves up to 15% QPS improvements while preserving search accuracy, demonstrating memory layout optimization works orthogonally to algorithmic innovations.

Conclusion: Memory layout optimization is crucial for GPU-based graph ANNS performance and operates independently from algorithmic improvements, with code release planned to support reproducibility and further research.
...
### [123] [Robust Symbolic Reasoning for Visual Narratives via Hierarchical and Semantically Normalized Knowledge Graphs](https://arxiv.org/abs/2508.14941)
*Yi-Chun Chen*
...
Method: Proposes methods that consolidate semantically related actions and events using lexical similarity and embedding-based clustering, building on cognitively grounded models of narrative comprehension.
...
TL;DR: The paper presents improvements to adder-viz software for visualizing real-time event transcode processes of the ADΔER representation, addressing limitations in flexibility, speed, and compressibility of event video formats.


<details>
  <summary>Details</summary>
Motivation: Existing event video representations have shown limitations in flexibility, speed, and compressibility. The ADΔER representation was previously proposed to address these concerns, but needed better visualization tools for real-time transcoding processes.

Method: The authors developed improvements to the adder-viz software for visualizing real-time event transcode processes and applications in-the-loop. The software is MIT-licensed and available in a centralized repository.

Result: The paper introduces enhanced visualization capabilities for the ADΔER event video representation, providing better tools for monitoring and debugging real-time event transcode operations.

Conclusion: The improved adder-viz software enables more effective visualization and development of event video applications using the ADΔER representation, making the technology more accessible and practical for researchers and developers.
...
### [125] [Self-supervised physics-informed generative networks for phase retrieval from a single X-ray hologram](https://arxiv.org/abs/2508.15530)
...
<details>
  <summary>Details</summary>
Motivation: Conventional phase retrieval methods require specific approximations, boundary conditions, and manual parameter tuning by experts, making them unsuitable for complex or variable experimental conditions. The challenge of acquiring suitable training data also limits deep learning approaches.

Method: Physics-informed generative adversarial network that reconstructs both phase and absorbance from a single hologram without requiring paired, unpaired, or simulated training data. Uses self-learning approach based on Fresnel theory in near-field regime.

Result: Robust and consistent performance across diverse imaging conditions and sample types. Delivers quantitative, high-quality reconstructions for both simulated data and experimental datasets from PETRA III beamline. Enables simultaneous retrieval of phase and absorption information.

Conclusion: The approach significantly broadens applicability of phase retrieval by eliminating training data requirements and expert parameter tuning, making it suitable for complex and variable experimental conditions in time- or dose-critical applications.
...
### [126] [A Chinese Heart Failure Status Speech Database with Universal and Personalised Classification](https://arxiv.org/abs/2508.14908)
...
<details>
  <summary>Details</summary>
Motivation: Speech is cost-effective for heart failure detection but lacks research on Chinese language effectiveness compared to other well-studied languages.

Method: Created first Chinese HF speech database with paired pre/post-hospitalization recordings, used patient-wise and pair-wise classification, and proposed adaptive frequency filter for frequency analysis.

Result: Confirmed Chinese language effectiveness for HF detection, with pair-wise classification serving as ideal speaker-decoupled baseline, and identified individual differences as key accuracy factor.

Conclusion: Chinese speech contains HF-related information, personalized approaches work better, and the database provides valuable resource for future research in HF detection.
...
### [127] [Transsion Multilingual Speech Recognition System for MLC-SLM 2025 Challenge](https://arxiv.org/abs/2508.14916)
*Xiaoxiao Li,An Zhu,Youhai Jiang,Fengjie Zhu*

Main category: eess.AS

TL;DR: Novel multilingual ASR system combining frozen Whisper encoder, trainable adaptor, and frozen Qwen2.5 LLM with LoRA, achieving 9.83% WER/CER across 11 languages and ranking 3rd in MLC-SLM 2025 Challenge.
...
### [128] [LLMs and Agentic AI in Insurance Decision-Making: Opportunities and Challenges For Africa](https://arxiv.org/abs/2508.15110)
...
<details>
  <summary>Details</summary>
Motivation: To explore the opportunities and challenges of AI/LLMs in insurance, particularly addressing gaps in the African insurance market and promoting local development.

Method: Analysis of AI trends, performance improvements, deployment costs, and framework complexity, combined with identification of market gaps and local efforts in African insurance.

Result: Identified critical gaps in African insurance market and highlighted key local players and partnership opportunities for AI adoption.
...
### [129] [SurgWound-Bench: A Benchmark for Surgical Wound Diagnosis](https://arxiv.org/abs/2508.15189)
...
TL;DR: SurgWound dataset with 697 surgical wound images and 8 clinical attributes, plus WoundQwen framework for surgical wound diagnosis using multi-stage MLLMs.


<details>
  <summary>Details</summary>
Motivation: Address lack of public datasets and benchmarks for surgical wound screening, overcoming data privacy concerns and high annotation costs that hinder deep learning progress in SSI prevention.

Method: Three-stage learning framework: 1) Five MLLMs predict specific wound characteristics, 2) Two MLLMs use predictions for infection risk diagnosis, 3) Final MLLM integrates results for comprehensive report generation.

Result: Created first open-source surgical wound dataset (SurgWound) and benchmark including VQA and report generation tasks for comprehensive model evaluation.

Conclusion: The framework enables detailed surgical wound analysis and personalized care instructions, paving way for timely interventions and improved patient outcomes in SSI prevention.
...
TL;DR: Extended reasoning budgets worsen LLM confidence calibration, while search-augmented generation dramatically improves accuracy to 89.3%, showing information access is more critical than reasoning depth.
...
Result: Reasoning LLMs achieved 48.7% accuracy in expert confidence assessment, but increasing reasoning budgets consistently impaired calibration, causing systematic overconfidence that worsens with longer thinking. Search-augmented generation dramatically outperformed pure reasoning with 89.3% accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Embodied AI agents need to interpret diverse human goals that aren't predefined in open-ended environments, requiring methods to reason over unbounded goal spaces during interactions.

Method: GOOD prompts LLMs to simulate users with different intents and uses these responses for probabilistic inference over natural language candidate goals during online interactions.

Result: Outperforms baseline without explicit goal tracking in both grocery shopping and household robotics domains, validated by LLM-based and human evaluations.
...
Method: Three-stage pipeline: 1) Data augmentation using frontier LLM to generate synthetic medical vignettes, 2) Fine-tuning open-source foundation model for diagnosis, treatment recommendations, and empathetic support, 3) Inference with expert evaluation by clinical and psychological specialists to validate and iteratively enrich the dataset.
...
### [134] [Multiple Memory Systems for Enhancing the Long-term Memory of Agent](https://arxiv.org/abs/2508.15294)
*Gaoke Zhang,Bo Wang,Yunlong Ma,Dongming Zhao,Zifei Yu*

Main category: cs.AI

TL;DR: Proposes a multiple memory system (MMS) for AI agents that processes short-term memory into multiple long-term memory fragments with retrieval and contextual units, improving memory quality and response performance.


<details>
  <summary>Details</summary>
Motivation: Existing memory modules like MemoryBank and A-MEM have poor quality of stored memory content, which affects recall performance and response quality in language model agents.

Method: Designed a multiple memory system inspired by cognitive psychology that processes short-term memory into multiple long-term fragments, creates retrieval and contextual memory units with one-to-one correspondence, and matches relevant units during retrieval.
...
Conclusion: MMS effectively constructs high-quality long-term memory content, enhances knowledge utilization from historical data, and demonstrates practical value for language model agents.
...
### [135] [DiagECG: An LLM-Driven Framework for Diagnostic Reasoning via Discretized ECG Tokenization](https://arxiv.org/abs/2508.15338)
...
<details>
  <summary>Details</summary>
Motivation: Existing automated ECG analysis methods struggle with generalization across clinical tasks and lack open-ended reasoning capabilities, limiting their diagnostic utility.
...
Result: Achieves strong performance across tasks without core model modification while maintaining generalization to out-of-distribution settings.
...
### [136] [GraSP: A Unified Graph-Based Framework for Scalable Generation, Quality Tagging, and Management of Synthetic Data for SFT and DPO](https://arxiv.org/abs/2508.15432)
...
### [137] [Language-Guided Tuning: Enhancing Numeric Optimization with Textual Feedback](https://arxiv.org/abs/2508.15757)
...
Method: Uses multi-agent LLMs with three specialized agents (Advisor, Evaluator, Optimizer) and textual gradients (qualitative feedback signals) to create a self-improving feedback loop for configuration optimization.

Result: Demonstrates substantial improvements over traditional optimization methods across six diverse datasets, achieving performance gains while maintaining high interpretability.
...
TL;DR: A training-free agentic system that combines VLM and LLMs to convert rough sketches into precise, editable SVG diagrams through iterative qualitative reasoning and editing.
...
Method: Uses an iterative loop with a Critic VLM to propose qualitative edits, multiple LLMs to synthesize SVG updates with diverse strategies, and a Judge VLM to select the best candidate for stable improvement.

Result: Outperforms frontier closed-source image generation LLMs (GPT-5 and Gemini-2.5-Pro) on 10 sketch-to-flowchart tasks, accurately composing primitives without unwanted text and better preserving layout/structure.

Conclusion: The approach successfully generates precise, editable SVG diagrams from sketches through qualitative reasoning, preserves global constraints, supports human corrections, and is extensible to presentation tools via APIs.
...
### [139] [Scalable Event-Based Video Streaming for Machines with MoQ](https://arxiv.org/abs/2508.15003)
*Andrew C. Freeman*
...
<details>
  <summary>Details</summary>
Motivation: Neuromorphic event sensors record video differently than traditional frame-based systems, but data transmission challenges have been ignored while focusing on application development
...
Result: Not specified in abstract - this appears to be a position/survey paper proposing a new approach rather than presenting experimental results

Conclusion: Proposes a new streaming format to address the data transmission challenges in event-based video systems that have been overlooked in favor of application development
...
### [140] [Zero-shot Volumetric CT Super-Resolution using 3D Gaussian Splatting with Upsampled 2D X-ray Projection Priors](https://arxiv.org/abs/2508.15151)
*Jeonghyun Noh,Hyun-Jic Oh,Byungju Chae,Won-Ki Jeong*

Main category: eess.IV

TL;DR: Novel zero-shot 3D CT super-resolution framework using diffusion-generated 2D X-ray priors and Gaussian splatting with negative alpha blending for enhanced detail reconstruction.
...
Method: Train diffusion model on large-scale 2D X-ray projections, use per-projection adaptive sampling strategy, employ 3D Gaussian splatting with negative alpha blending (NAB-GS) for residual learning between LR and diffusion-based projections.

Result: Superior quantitative and qualitative results for 3D CT super-resolution on two datasets, demonstrating enhanced high-frequency structure reconstruction.

Conclusion: The proposed framework effectively leverages 2D X-ray priors through diffusion modeling and Gaussian splatting with negative alpha blending, achieving state-of-the-art zero-shot 3D CT super-resolution without requiring paired training data.
...
### [141] [Pathology-Informed Latent Diffusion Model for Anomaly Detection in Lymph Node Metastasis](https://arxiv.org/abs/2508.15236)
...
<details>
  <summary>Details</summary>
Motivation: Address data scarcity in digital pathology by developing unsupervised anomaly detection that doesn't require extensive annotations, building on recent success of diffusion models in medical imaging.

Method: Combines vision-language model with diffusion model, using pathology-related keywords for normal tissues to guide reconstruction process and enable differentiation between normal and abnormal tissue distributions.

Result: Evaluated on gastric lymph node dataset from local hospital and tested generalization on public breast lymph node dataset, showing promising performance for unsupervised anomaly detection across different organs.

Conclusion: The proposed method demonstrates potential for effective unsupervised anomaly detection in digital pathology across various organs, addressing annotation scarcity issues while maintaining detection accuracy.
...
### [142] [Explainable Knowledge Distillation for Efficient Medical Image Classification](https://arxiv.org/abs/2508.15251)
*Aqib Nazir Mir,Danish Raza Rizvi*

Main category: eess.IV

TL;DR: Knowledge distillation framework using VGG19 and Vision Transformers as teachers to train compact student model for COVID-19 and lung cancer classification from chest X-rays, achieving high accuracy with reduced computational costs.
...
Method: Employed knowledge distillation with hybrid supervision using VGG19, Visformer-S, and AutoFormer-V2-T as teacher models to train a compact OFA-595 student model. Used Score-CAM visualizations for model interpretability on COVID-QU-Ex and LCS25000 datasets.

Result: Distilled student model maintained high classification performance with significantly reduced parameters and inference time compared to teacher models.

Conclusion: Combining knowledge distillation with explainability techniques provides optimal balance between accuracy and efficiency for practical medical AI applications in clinical settings.
...
### [143] [Bladder Cancer Diagnosis with Deep Learning: A Multi-Task Framework and Online Platform](https://arxiv.org/abs/2508.15379)
...
Result: Achieved 93.28% accuracy, 82.05% F1-score, 96.41% AUC for classification; 0.9091 Dice coefficient for segmentation. Online platform significantly improved diagnostic accuracy, efficiency, and accessibility.

Conclusion: The framework and online tool advance intelligent bladder cancer diagnosis by improving clinical reliability, supporting early detection, and enabling real-time diagnostic feedback, representing significant progress toward AI-assisted urology decision-making.
...
### [144] [DoSReMC: Domain Shift Resilient Mammography Classification using Batch Normalization Adaptation](https://arxiv.org/abs/2508.15452)
...
Result: Demonstrates that BN layers are a primary source of domain dependence and shows significant improvements in cross-domain performance using three large-scale FFDM datasets including a new pathologically confirmed dataset.

Conclusion: DoSReMC provides a practical solution for robust mammography classification that can be easily integrated into existing AI pipelines across diverse clinical environments.
...
### [145] [Deep Equilibrium Convolutional Sparse Coding for Hyperspectral Image Denoising](https://arxiv.org/abs/2508.15553)
*Jin Ye,Jingran Wang,Fengchao Xiong,Jingzhou Chen,Yuntao Qian*

Main category: eess.IV

TL;DR: A Deep Equilibrium Convolutional Sparse Coding framework that combines local spatial-spectral correlations, nonlocal self-similarities, and global spatial consistency for robust hyperspectral image denoising.


<details>
  <summary>Details</summary>
Motivation: Existing deep unfolding methods for HSI denoising lack convergence guarantees and are limited by predefined network depths, while physical property preservation is crucial for robust denoising.

Method: Proposes DECSC framework using DEQ models to treat hidden layers as infinite-depth networks. Combines 2D CSC for global spatial consistency, 3D CSC for local spatial-spectral details, transformer blocks for nonlocal self-similarities, and detail enhancement modules.

Result: Experimental results demonstrate superior denoising performance compared to state-of-the-art methods.

Conclusion: The DECSC framework effectively unifies multiple HSI characteristics within a convergent DEQ-based architecture, achieving robust denoising while preserving physical properties.
...
### [146] [Are Virtual DES Images a Valid Alternative to the Real Ones?](https://arxiv.org/abs/2508.15594)
*Ana C. Perre,Luís A. Alexandre,Luís C. Freire*

Main category: eess.IV

TL;DR: This study explores using AI models to generate virtual dual-energy subtracted (DES) mammography images from low-energy images, potentially reducing radiation exposure. The pre-trained U-Net achieved 85.59% F1 score compared to 90.35% with real DES images.


<details>
  <summary>Details</summary>
Motivation: To reduce patient radiation exposure in contrast-enhanced spectral mammography by generating artificial DES images from LE images instead of acquiring high-energy images.

Method: Investigated three models: pre-trained U-Net, end-to-end trained U-Net, and CycleGAN for generating virtual DES images. Evaluated impact on malignant/non-malignant classification performance.
...
Conclusion: Virtual DES image generation shows considerable potential. Future advancements may narrow the performance gap to make exclusive reliance on virtual DES images clinically viable, reducing radiation exposure.
...
### [147] [Label Uncertainty for Ultrasound Segmentation](https://arxiv.org/abs/2508.15635)
...
TL;DR: This paper introduces a novel approach to medical image segmentation that incorporates radiologists' per-pixel confidence values during training, showing improved segmentation performance and better downstream clinical outcomes in lung ultrasound analysis.
...
Method: Developed a data annotation protocol that captures radiologists' per-pixel confidence values. Evaluated various confidence thresholds for training, finding that binarized labels with a 60% confidence threshold work best. Compared segmentation metrics and downstream clinical outcomes.

Result: Incorporating confidence values during training leads to improved segmentation performance. High confidence thresholds (60%) work significantly better than naive 50% thresholds. Enhanced segmentation quality translates to better performance on clinically-critical tasks: S/F oxygenation ratio estimation, S/F ratio change classification, and 30-day patient readmission prediction.

Conclusion: Label confidence is a valuable signal that, when properly leveraged, can significantly enhance the reliability and clinical utility of AI in medical imaging. Training on very confident pixels is more effective than using all annotated pixels.
...
### [148] [Hessian-based lightweight neural network for brain vessel segmentation on a minimal training dataset](https://arxiv.org/abs/2508.15660)
...
<details>
  <summary>Details</summary>
Motivation: There is a lack of publicly available MRA datasets with detailed brain vessel annotations, and current methods like manual segmentation or Frangi filter lack sufficient accuracy for surgical procedures.

Method: A novel semi-supervised learning lightweight neural network (HessNet) with Hessian matrices on board for 3D segmentation of tubular structures, requiring only 6000 parameters and capable of running on CPU.

Result: Achieved state-of-the-art vessel segmentation accuracy on minimal training dataset, enabled creation of a large semi-manually annotated brain vessel dataset (200 images from IXI dataset) with expert supervision.

Conclusion: HessNet provides high-accuracy vessel segmentation with minimal resource requirements, allowing experts to focus on complex cases and facilitating the development of better annotated datasets for medical image analysis.
...
### [149] [Exploring the Landscape of Non-Equilibrium Memories with Neural Cellular Automata](https://arxiv.org/abs/2508.15726)
...
Method: Combination of rigorous mathematical proofs and machine learning techniques to systematically discover and analyze 2D memory systems.

Result: Discovered numerous 2D memories with qualitatively distinct error correction mechanisms from Toom's rule, including fluctuation-stabilized ordered phases and noise-dependent information preservation capabilities.
...
TL;DR: FPGA-based real-time denoising pipeline for high-throughput imaging using HLS optimization and AXI4 interfaces to process data faster than frame rates.
...
### [151] ["Does the cafe entrance look accessible? Where is the door?" Towards Geospatial AI Agents for Visual Inquiries](https://arxiv.org/abs/2508.15752)
...
<details>
  <summary>Details</summary>
Motivation: Traditional interactive maps rely on pre-existing structured GIS data, which limits their ability to answer visual-spatial questions about what the world actually looks like.

Method: Combining large-scale repositories of geospatial images (street views, place photos, aerial imagery) with traditional GIS data sources through multimodal AI analysis.

Result: The paper presents a vision for Geo-Visual Agents capable of understanding and responding to nuanced visual-spatial inquiries, with three exemplars demonstrating the approach.

Conclusion: The paper defines the vision, describes sensing and interaction approaches, and enumerates key challenges and opportunities for future development of Geo-Visual Agents.
...
### [152] [AmbiSQL: Interactive Ambiguity Detection and Resolution for Text-to-SQL](https://arxiv.org/abs/2508.15276)
...
### [153] [Fusing Structural Phenotypes with Functional Data for Early Prediction of Primary Angle Closure Glaucoma Progression](https://arxiv.org/abs/2508.14922)
...
TL;DR: Combined ONH structural and VF functional parameters using ML to classify PACG progression risk, achieving best performance (AUC=0.87) with Random Forest model.


<details>
  <summary>Details</summary>
Motivation: To improve classification of glaucoma progression risk in primary angle closure glaucoma patients by integrating structural and functional parameters rather than using them separately.

Method: Used AI-segmented OCT volumes to extract 31 ONH parameters, combined with sector-based visual field sensitivity data. Trained multiple ML classifiers including Random Forest, with SHAP analysis to identify key predictors from 451 eyes over >5 years.

Result: Random Forest model with combined features achieved AUC=0.87, significantly outperforming models using only structural (AUC=0.82) or only functional (AUC=0.78) features. Identified 6 key predictors including inferior MRW, RNFL thickness, and VF sensitivity.

Conclusion: Integration of ONH structural and VF functional parameters significantly improves progression risk classification in PACG, with inferior ONH features being most predictive for monitoring disease progression.
...
### [154] [Evaluating Knowledge Graph Complexity via Semantic, Spectral, and Structural Metrics for Link Prediction](https://arxiv.org/abs/2508.15291)
*Haji Gul,Abul Ghani Naim,Ajaz Ahmad Bhat*

Main category: cs.LG

TL;DR: CSG complexity metric fails to perform as claimed in knowledge graph link prediction, showing sensitivity to parameters and weak correlation with performance metrics. Better alternatives like Relation Entropy and Maximum Relation Diversity demonstrate stronger predictive power.


<details>
  <summary>Details</summary>
Motivation: To critically evaluate the Cumulative Spectral Gradient (CSG) metric in knowledge graph link prediction contexts and identify more reliable complexity measures that correlate with model performance.

Method: Examined CSG in multi-relational link prediction using transformer-derived embeddings, introduced structural/semantic KG complexity metrics (Relation Entropy, Maximum Relation Diversity, Relation Type Cardinality), and benchmarked against standard performance metrics (MRR, Hit@1, Hit@10).

Result: CSG was highly sensitive to parametrization, didn't scale with class cardinality, and showed weak correlation with MRR/Hit@1. New metrics (Relation Entropy, Maximum Relation Diversity, Relation Type Cardinality) showed strong inverse correlations with MRR/Hit@1, while connectivity measures correlated positively with Hit@10.

Conclusion: CSG's claimed stability and generalization predictive power don't hold in link prediction. Task-aligned complexity measures like relational ambiguity metrics are more faithful indicators of dataset difficulty in knowledge-driven learning.
...
### [155] [CITE: A Comprehensive Benchmark for Heterogeneous Text-Attributed Graphs on Catalytic Materials](https://arxiv.org/abs/2508.15392)
...
<details>
  <summary>Details</summary>
Motivation: There is a critical shortage of large-scale benchmark datasets for heterogeneous text-attributed graphs (TAGs), which hinders development and fair comparison of representation learning methods in this domain.

Method: Created CITE dataset with over 438K nodes and 1.2M edges spanning four relation types, established standardized evaluation procedures, and conducted benchmarking on node classification task with four learning paradigms: homogeneous graph models, heterogeneous graph models, LLM-centric models, and LLM+Graph models.
...
Conclusion: CITE serves as a valuable benchmark resource that enables standardized evaluation and comparison of representation learning methods for heterogeneous text-attributed graphs in the catalytic materials domain.
...
### [156] [Classification errors distort findings in automated speech processing: examples and solutions from child-development research](https://arxiv.org/abs/2508.15637)
*Lucas Gautheron,Evan Kidd,Anton Malko,Marvin Lavechin,Alejandrina Cristia*

Main category: cs.LG

TL;DR: Bayesian analysis shows automated audio classifiers significantly distort language acquisition research estimates, with errors reducing sibling effect detection by 20-80%. Proposed calibration helps but isn't foolproof.
...
Conclusion: Bayesian calibration can help recover unbiased effect size estimates but isn't perfect. The issue and solution apply to any classifier with non-zero error rates in event detection and classification.
...
### [157] [Intern-S1: A Scientific Multimodal Foundation Model](https://arxiv.org/abs/2508.15763)
...
<details>
  <summary>Details</summary>
Motivation: To bridge the performance gap between open-source and closed-source models in high-value scientific fields, and advance toward AGI by developing specialized generalist models with scientific expertise.
...
Conclusion: Intern-S1 successfully bridges the gap in scientific domains through integrated innovations in algorithms, data, and training systems, demonstrating that specialized generalist models can achieve breakthrough performance in challenging scientific professional fields.
...
TL;DR: Concept Erasure Techniques (CETs) for text-to-image models can be easily circumvented through hierarchical and compositional prompts, and suffer from side effects including attribute leakage and attention issues.
...
### [159] [Probability Density from Latent Diffusion Models for Out-of-Distribution Detection](https://arxiv.org/abs/2508.15737)
*Joonas Järve,Karl Kaspar Haavel,Meelis Kull*

Main category: cs.LG

TL;DR: This paper investigates whether likelihood-based out-of-distribution (OOD) detection fails due to poor density estimation in pixel space or representation space, by training a Variational Diffusion Model on ResNet-18 features instead of images.


<details>
  <summary>Details</summary>
Motivation: Despite theoretical optimality of likelihood for OOD detection under uniform OOD assumption, practical implementations often fail, raising questions about whether this failure stems from pixel space limitations or fundamental representation issues.

Method: Trained a Variational Diffusion Model on the representation space of a pre-trained ResNet-18 (instead of pixel space) and evaluated likelihood-based OOD detection performance using the OpenOOD benchmark suite.

Result: The paper compares the performance of their representation-space likelihood-based detector against state-of-the-art methods, though specific results are not provided in the abstract.

Conclusion: The study aims to determine if the practical failure of likelihood-based OOD detection is due to pixel space limitations or if representation space also suffers from poor density estimation capabilities.
...
### [160] [A Vision-Based Shared-Control Teleoperation Scheme for Controlling the Robotic Arm of a Four-Legged Robot](https://arxiv.org/abs/2508.14994)
...
Result: System validated on real robot with robust real-time performance, demonstrating effective and intuitive control with safety features.

Conclusion: Provides cost-effective, intuitive teleoperation solution with enhanced safety and precision for industrial applications in high-risk environments.
...
### [161] [Decentralized Vision-Based Autonomous Aerial Wildlife Monitoring](https://arxiv.org/abs/2508.15038)
...
<details>
  <summary>Details</summary>
Motivation: Wildlife field operations need efficient parallel deployment methods for identifying and interacting with specific individuals for behavioral analysis and interventions. Previous robotics solutions are either herd-focused or manually operated with limited scalability.

Method: Developed a decentralized vision-based multi-quadrotor system with single onboard RGB cameras. Created novel vision-based coordination and tracking algorithms designed for dynamic, unstructured environments without centralized communication or control.

Result: Validated through real-world experiments, demonstrating reliable deployment in diverse field conditions. The system enables robust identification and tracking of large species in their natural habitat.

Conclusion: The proposed system provides a scalable, low-bandwidth, and sensor-minimal solution for wildlife monitoring that operates effectively in natural environments without requiring centralized infrastructure.
...
### [162] [Lang2Lift: A Framework for Language-Guided Pallet Detection and Pose Estimation Integrated in Autonomous Outdoor Forklift Operation](https://arxiv.org/abs/2508.15427)
...
<details>
  <summary>Details</summary>
Motivation: Address labor shortages, safety concerns, and inefficiencies in manual pallet handling in outdoor logistics and construction environments with variable payloads, inconsistent pallet quality, and unstructured surroundings.
...
Result: Achieved 0.76 mIoU pallet segmentation accuracy on real-world test dataset. Timing and error analysis demonstrate system robustness and feasibility for deployment in operational logistics and construction environments.

Conclusion: Lang2Lift provides a viable solution for automating pallet pick-up operations using natural language guidance and foundation models, addressing key challenges in outdoor logistics automation with promising real-world performance.
...
<details>
  <summary>Details</summary>
Motivation: Recommender systems are vulnerable to data poisoning attacks, but existing methods struggle with generating high-quality fake profiles that are both effective and imperceptible, especially with limited system knowledge.
...
Conclusion: The proposed RAGAN framework successfully enhances fake profile quality using multimodal foundation models, providing an effective method for testing recommender system robustness against practical black-box attacks.
...
TL;DR: IPIGuard is a new defense against Indirect Prompt Injection attacks on LLM agents by modeling task execution as Tool Dependency Graph traversal, decoupling planning from external data interaction to prevent malicious tool invocations.
...
Conclusion: IPIGuard enhances robustness against Indirect Prompt Injection attacks and paves the way for safer agentic systems in dynamic environments by preventing malicious tool invocations at the source.
