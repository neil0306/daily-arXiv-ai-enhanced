...
TL;DR: Initial study of RAG pipeline for Taiwanese historical archives using Fort Zeelandia and Taiwan Provincial Council Gazette datasets, examining query characteristics and metadata integration effects.


<details>
  <summary>Details</summary>
Motivation: Few studies have examined RAG for Taiwanese Historical Archives, creating a gap in applying this promising approach to knowledge-intensive historical tasks.

Method: Applied RAG pipeline to two historical Traditional Chinese datasets with systematic investigation of query characteristics and metadata integration strategies on retrieval quality and answer generation.
...
Conclusion: RAG shows promise for historical archives but faces challenges with hallucinations and complex temporal/multi-hop queries that need to be addressed.
...
TL;DR: This survey synthesizes methods for LLM-driven scientific idea generation, categorizing approaches into five families and analyzing them using creativity frameworks to understand how different methods balance creativity with scientific soundness.


<details>
  <summary>Details</summary>
Motivation: Scientific idea generation is crucial for discovery but is multi-objective and open-ended. While LLMs show promise in generating scientific ideas, their creative capacity remains inconsistent and poorly understood, requiring systematic analysis.
...
Result: Provides a structured synthesis that clarifies how different approaches balance creativity with scientific soundness, and identifies the types of ideas each method family is expected to generate.
...
TL;DR: GRIP is a framework that enables LLMs to internalize graph knowledge through fine-tuning with LoRA parameters, allowing graph reasoning without original graphs at inference.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with structural data like graphs, as existing methods have high token overhead or require complex alignment procedures with sub-optimal results.

Method: Fine-tune LLMs using carefully designed tasks to internalize graph relational information, storing knowledge in lightweight LoRA parameters for efficient inference.

Result: Extensive experiments show GRIP is effective and efficient across multiple benchmarks for graph-related tasks.

Conclusion: GRIP successfully equips LLMs with graph reasoning capabilities through parameter-efficient fine-tuning, eliminating the need for original graphs during inference.
...
<details>
  <summary>Details</summary>
Motivation: Existing metrics like ROUGE and BLEU have limitations as they rely on surface-level lexical overlap and require high-quality reference summaries, which are often unavailable in log summarization contexts.

Method: REFLEX uses large language models (LLMs) as zero-shot evaluators to assess summary quality along dimensions such as relevance, informativeness, and coherence, without requiring reference summaries or human annotations.

Result: REFLEX produces stable, interpretable, and fine-grained evaluations across multiple log summarization datasets, and more effectively distinguishes model outputs than traditional metrics like ROUGE and BLEU.

Conclusion: REFLEX provides a scalable alternative for evaluating log summaries in real-world settings where reference data is scarce or unavailable, offering a more practical solution than traditional reference-dependent metrics.
...
TL;DR: DuTerm is a two-stage MT system combining fine-tuned NMT with LLM post-editing for terminology-constrained translation, showing LLMs work better as context-driven mutators than generators.


<details>
  <summary>Details</summary>
Motivation: To improve terminology-constrained machine translation by combining the strengths of NMT and LLMs while addressing the limitations of strict constraint enforcement.

Method: Two-stage architecture: 1) Terminology-aware NMT model fine-tuned on synthetic data, 2) Prompt-based LLM for post-editing to refine output and enforce terminology adherence.

Result: Evaluated on English-German, English-Spanish, English-Russian using WMT 2025 corpus. Flexible, context-driven terminology handling by LLM consistently yields higher quality translations than strict constraint enforcement.

Conclusion: LLMs work best for high-quality translation as context-driven mutators rather than generators, highlighting a critical trade-off in terminology-constrained MT.
...
TL;DR: Motif-2-12.7B is an efficient 12.7B parameter language model that uses Grouped Differential Attention and optimized training to achieve competitive performance with larger models.


<details>
  <summary>Details</summary>
Motivation: To push the efficiency frontier of large language models by combining architectural innovation with system-level optimization for scalable language understanding under constrained compute budgets.

Method: Uses Grouped Differential Attention (GDA) to separate signal and noise-control attention pathways, pre-trained on 5.5T tokens with curriculum scheduling, MuonClip optimizer, custom kernels, and three-stage supervised fine-tuning.

Result: Demonstrates competitive performance across diverse benchmarks, showing that thoughtful architectural scaling and optimized training can rival capabilities of much larger models.

Conclusion: Efficient model design through architectural innovations and system optimizations can achieve competitive performance without requiring massive parameter counts.
...
TL;DR: The paper proposes LAHIS method to identify important attention heads for multilingual capabilities in LLMs, revealing both language-specific and general heads, and introduces a lightweight adaptation to improve multilingual performance.


<details>
  <summary>Details</summary>
Motivation: To understand the role of multi-head self-attention in supporting multilingual processing in LLMs, as this area remains underexplored despite MHA's critical importance in other areas.

Method: Proposed Language Attention Head Importance Scores (LAHIS) - an efficient method using single forward/backward pass to identify attention head importance for multilingual capabilities. Also introduced lightweight adaptation with soft head mask requiring only 20 parameters.

Result: Revealed existence of both language-specific and language-general heads in models like Aya-23-8B, Llama-3.2-3B, and Mistral-7B-v0.1. Language-specific heads enable cross-lingual attention transfer and mitigate off-target language generation. Lightweight adaptation improved XQuAD accuracy.

Conclusion: The work enhances both interpretability and multilingual capabilities of LLMs from the perspective of multi-head self-attention, addressing challenges in multilingual LLMs through attention head analysis and lightweight adaptation.
...
TL;DR: This paper presents optimization methods for LLM-based pairwise reranking in RAG systems, achieving 166x latency reduction from 61.36s to 0.37s per query with minimal performance drop.


<details>
  <summary>Details</summary>
Motivation: Address the computational complexity and latency issues of Pairwise Reranking Prompting (PRP) with LLMs, which hinder real-time deployment despite their effectiveness in improving RAG system quality.

Method: Implement multiple optimization techniques: using smaller LLMs, limiting reranked document set size, employing lower precision computation, reducing positional bias through one-directional order inference, and restricting output tokens.

Result: Achieved dramatic latency reduction of 166x (from 61.36s to 0.37s per query) with insignificant performance drop in Recall@k metrics, making LLM-based reranking feasible for real-time applications.

Conclusion: Careful optimization of design choices can make LLM-based pairwise reranking substantially more efficient and practical for latency-sensitive real-world deployments, highlighting previously overlooked optimization opportunities.
...
TL;DR: Dutch-tuned LLMs underperformed traditional lexicon-based tools (LIWC and Pattern) for valence prediction in Flemish, with Pattern showing superior performance despite LLMs' architectural advantages.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether Dutch-specific LLMs can outperform traditional lexicon-based tools in understanding emotional nuances in everyday language, particularly for low-resource language variants like Flemish.

Method: Compared three Dutch-tuned LLMs (ChocoLlama-8B-Instruct, Reynaerde-7B-chat, GEITje-7B-ultra) against LIWC and Pattern for valence prediction using 25,000 spontaneous textual responses from 102 Dutch-speaking participants with self-assessed valence ratings.

Result: Dutch-tuned LLMs underperformed traditional methods, with Pattern showing superior performance in predicting emotional valence in Flemish narratives.

Conclusion: LLMs may not inherently outperform traditional methods for sentiment analysis in low-resource languages, highlighting the need for culturally and linguistically tailored evaluation frameworks and questioning current LLM fine-tuning approaches for nuanced emotional expressions.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation methods are problematic: lexical metrics lack semantic understanding, while LLM-as-Judge approaches are computationally expensive.
...
Result: NLI-based evaluation achieves 89.9% accuracy matching GPT-4o on long-form QA while requiring orders-of-magnitude fewer parameters.

Conclusion: Inexpensive NLI-based evaluation remains competitive and DIVER-QA is offered as an open resource for future metric research.
...
TL;DR: Evaluation of six reference-free factuality metrics for long-document summarization reveals inconsistent scores for semantically equivalent summaries and declining reliability for information-dense claims, highlighting the need for improved long-form evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Factual consistency evaluation for abstractive text summarization is challenging for long documents due to input length limitations and long-range dependencies, requiring systematic assessment of existing metrics' reliability in long-document settings.

Method: Systematically evaluated six reference-free factuality metrics through seven factuality-preserving perturbations (paraphrasing, simplification, synonym replacement, etc.) and analyzed sensitivity to retrieval context and claim information density across three long-form benchmark datasets.
...
Conclusion: Current factuality metrics are inadequate for long-document summarization, highlighting the need for improvements including multi-span reasoning, context-aware calibration, and training on meaning-preserving variations to enhance robustness in long-form evaluation.
...
TL;DR: CAPO is a confidence-aware preference optimization method that dynamically scales loss based on relative reward confidence, improving multilingual alignment over DPO.


<details>
  <summary>Details</summary>
Motivation: Existing preference optimization methods like DPO work well for English but fail to generalize to multilingual settings due to noisy or low-margin preference pairs.

Method: Replace DPO's fixed treatment of preference pairs with dynamic loss scaling based on relative reward confidence, modulating learning signal according to confidence in each preference pair.

Result: Outperforms existing preference optimization baselines by at least 16% in reward accuracy and improves alignment by widening the gap between preferred and dispreferred responses across languages.

Conclusion: CAPO provides a simple yet effective alternative that enhances robustness to noisy multilingual preference data through confidence-aware dynamic scaling.
...
TL;DR: LLMs can use controlled hallucinations to fill gaps in historical archives, reconstructing narratives for marginalized figures while maintaining evidence-based boundaries.


<details>
  <summary>Details</summary>
Motivation: Address omissions in archives caused by social and political inequality by using LLM hallucinations to reconstruct narratives for history's hidden figures.

Method: Used narrative cloze tasks with character timelines from unpublished texts, testing various LLMs (OLMo-2 family, open-weight, proprietary) with different prompts to elicit controlled hallucinations.

Result: LLMs demonstrated foundational narrative understanding for critical confabulation, showing controlled hallucinations can support knowledge production without compromising historical accuracy.

Conclusion: Well-specified LLM hallucinations can be valuable for filling archival gaps and reconstructing marginalized narratives while maintaining fidelity to evidence.
...
TL;DR: The paper investigates backward predictability effects in naturalistic speech using improved measures and language models, finding that both past and future context influence word duration and substitution errors, revealing how speakers prioritize different information during lexical planning.


<details>
  <summary>Details</summary>
Motivation: To better understand the poorly-understood backward predictability effect of a word given its future context in naturalistic language production, which may be related to future planning mechanisms.

Method: Two studies using naturalistic speech corpora with improved measures and powerful language models, introducing a new information-theoretic predictability measure that integrates both future and past context predictability.

Result: The proposed conceptually-motivated alternative to backward predictability yields qualitatively similar effects across both word duration and substitution error studies, with fine-grained analysis showing different error types suggest how speakers prioritize form, meaning, and context-based information.

Conclusion: The findings illuminate the functional roles of past and future context in how speakers encode and choose words, offering a bridge between contextual predictability effects and sentence planning mechanisms.
...
TL;DR: CUFEInse v1.0 is a comprehensive insurance domain benchmark with 5 dimensions, 54 sub-indicators, and 14,430 questions, evaluating 11 LLMs to identify bottlenecks in actuarial capabilities, compliance adaptation, and business scenarios.


<details>
  <summary>Details</summary>
Motivation: To fill the gap in professional evaluation benchmarks for the insurance field and provide academia and industry with a systematic evaluation tool for large language models in vertical domains.

Method: Established evaluation framework using "quantitative-oriented, expert-driven, multi-validation" principles covering insurance knowledge, industry understanding, safety/compliance, intelligent agent application, and logical rigor.

Result: General-purpose models show weak actuarial capabilities and inadequate compliance adaptation. Domain-specific models excel in insurance scenarios but have shortcomings in business adaptation and compliance. Identified bottlenecks in actuarial, underwriting/claims reasoning, and compliant marketing.

Conclusion: CUFEInse provides authoritative evaluation for academic optimization and industrial model selection, with future directions focusing on domain adaptation and reasoning enhancement for insurance models.
...
TL;DR: A trainable graph memory framework that structures agent experiences into strategic meta-cognition, optimized via reinforcement learning to enhance LLM agents' reasoning and generalization.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents acquire experience through implicit training (catastrophic forgetting, limited interpretability) or explicit prompting (lacks adaptability), requiring better memory utilization for improved reasoning.

Method: Multi-layered graph memory that abstracts agent trajectories into state machine decision paths, distills them into strategic meta-cognition, and uses reinforcement-based weight optimization to adapt memory based on task rewards.

Result: The framework delivers robust generalization, improves strategic reasoning performance, and provides consistent benefits during RL training.

Conclusion: Learnable graph memory enhances LLM agents' ability to utilize parametric information and improves their autonomous task-solving capabilities in complex environments.
...
TL;DR: AlignSurvey is a benchmark that systematically evaluates LLMs across the full social survey pipeline, addressing limitations of traditional surveys and previous LLM approaches through four key tasks and comprehensive evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Traditional surveys face challenges like fixed formats, high costs, and cross-cultural issues, while existing LLM approaches are limited to structured questions and risk under-representing marginalized groups due to training biases.

Method: Introduces AlignSurvey benchmark with four tasks covering key survey stages, multi-tiered dataset architecture including Social Foundation Corpus and Entire-Pipeline Survey Datasets, and SurveyLM family models obtained through two-stage fine-tuning.

Result: Developed comprehensive benchmark with 44K+ interview dialogues, 400K+ structured survey records, and expert-annotated datasets for evaluating LLM alignment fidelity, consistency, and fairness across demographic groups.

Conclusion: AlignSurvey provides a systematic framework for transparent and socially responsible evaluation of LLMs in social survey applications, with all resources publicly available to support research in this domain.
...
TL;DR: A system to forecast social unrest events by analyzing news articles using topic modeling, word2vec, and Named Entity Recognition to identify key entities and dates.


<details>
  <summary>Details</summary>
Motivation: Forecasting civil unrest events helps administrative officials take necessary action, as protests are often announced in advance through news articles.

Method: Uses topic modeling and word2vec to filter relevant news articles, Named Entity Recognition to identify entities, time normalization for dates, and proposes Related Entity Extraction to identify entities actually involved in events.

Result: Developed a geographically independent, generalized model to identify key features for filtering civil unrest events and extract Related Entities.

Conclusion: The proposed system can effectively forecast social unrest events by analyzing news announcements and extracting relevant entities through automated methods.
...
TL;DR: MC^2F is a two-module system that enhances adversarial robustness in text classification while preserving clean data performance by modeling clean data distribution in embedding manifold and correcting adversarial points.


<details>
  <summary>Details</summary>
Motivation: Resolve the persistent challenge where improving adversarial robustness typically degrades performance on clean data in text classification.

Method: Two-module system: 1) Stratified Riemannian Continuous Normalizing Flow (SR-CNF) learns clean data manifold density, 2) Geodesic Purification Solver projects adversarial embeddings back to manifold via shortest path.
...
Conclusion: MC^2F successfully resolves the robustness-clean performance trade-off by operating on sentence embeddings and correcting adversarial points back to the learned clean manifold.
...
TL;DR: Proposes Logits-to-Logic framework to address Logic Drift in LLMs for structured knowledge reasoning by correcting logical defects through logits strengthening and filtering.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with logic consistency in structured knowledge reasoning due to representational differences between unstructured text and structured knowledge, leading to Logic Drift challenges.

Method: Logits-to-Logic framework with logits strengthening and logits filtering modules to correct logical defects in LLM outputs during autoregressive generation.
...
Conclusion: The proposed framework effectively addresses Logic Drift by targeting logits output to enhance logic consistency in structured knowledge reasoning tasks.
...
TL;DR: This paper analyzes how social media data can be used to study mental health issues like depression and anxiety through linguistic, visual, and emotional indicators, with applications for medical practice and policy.


<details>
  <summary>Details</summary>
Motivation: Social media provides a unique platform where people freely discuss stigmatized mental health issues anonymously, creating valuable data for research and intervention.

Method: Uses linguistic, visual, and emotional indicators from social media disclosures, employing machine learning, feature engineering, and natural language processing techniques.

Result: Demonstrates how social media data can be leveraged to understand mental health challenges and provide insights for medical practice and policy interventions.

Conclusion: Social media data offers unprecedented opportunities to raise mental health awareness and improve support systems, with future research directions identified.
...
TL;DR: This study compares theta-band neural synchrony across different speech modes (perceived, overt, whispered, imagined) using connectivity metrics, revealing distinct synchronization patterns for each mode.


<details>
  <summary>Details</summary>
Motivation: Previous research has focused on single speech modes, lacking integrated comparison of theta synchrony across different modes despite theta-band's known association with language processing and inner speech.

Method: Analyzed differences in theta-band neural synchrony across speech modes based on connectivity metrics, focusing on region-wise variations in brain connectivity patterns.
...
TL;DR: WorkBench is a unified evaluation suite for work-related NLP tasks, and UWE is a task-agnostic bi-encoder model that outperforms generalist embedding models on work domain tasks.


<details>
  <summary>Details</summary>
Motivation: There's increased demand for specialized NLP in workforce contexts, but work-related tasks have complex characteristics like long-tailed distributions and scarce data. Generalist models' performance in work domains is unclear.

Method: Created WorkBench evaluation suite with 6 work-related ranking tasks, then developed UWE using bipartite graphs from real data enriched synthetically, with many-to-many InfoNCE objective and token-level embeddings with soft late interaction.

Result: UWE shows significant gains over generalist models in macro-averaged MAP and RP@10, enables zero-shot ranking on unseen target spaces, and allows low-latency inference through cached embeddings.

Conclusion: The approach demonstrates effective cross-task transfer in work domains and provides a foundation for multi-task progress in workforce NLP applications.
...
TL;DR: NOTAM-Evolve is a self-evolving LLM framework that achieves 30.4% accuracy improvement over base LLM for structured NOTAM interpretation through dual-reasoning and closed-loop learning.


<details>
  <summary>Details</summary>
Motivation: Current automated NOTAM systems only perform shallow parsing and fail to extract actionable intelligence needed for operational aviation decisions due to NOTAMs' condensed and cryptic language.

Method: Proposes NOTAM-Evolve framework with dual-reasoning: dynamic knowledge grounding (linking to real-world aeronautical data via knowledge graph) and schema-based inference (applying domain rules), plus closed-loop learning where LLM improves from its own outputs.

Result: Achieves 30.4% absolute accuracy improvement over base LLM, establishes new state-of-the-art on structured NOTAM interpretation, and introduces benchmark dataset of 10,000 expert-annotated NOTAMs.

Conclusion: NOTAM-Evolve successfully enables LLMs to master complex NOTAM interpretation through self-evolving framework with minimal human annotation, significantly advancing automated aviation safety systems.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate LLM performance on text classification in less-resourced South Slavic languages and compare with fine-tuned BERT models, as LLM performance in these languages remains under-explored.

Method: Compared fine-tuned BERT-like models with open-source and closed-source LLMs across three tasks: sentiment classification in parliamentary speeches, topic classification in news/articles, and genre identification in web texts.

Result: LLMs demonstrated strong zero-shot performance, often matching or surpassing fine-tuned BERT models, with comparable performance in South Slavic languages and English.

Conclusion: While LLMs show impressive zero-shot capabilities, fine-tuned BERT models remain more practical for large-scale text annotation due to LLM limitations in output predictability, inference speed, and computational costs.
...
TL;DR: Proposes Self-Correction Distillation (SCD) method to improve small LLMs' structured data QA by transferring large LLMs' query-generation and error-correction capabilities through error prompt mechanism and two-stage distillation.


<details>
  <summary>Details</summary>
Motivation: Small-scale LLMs struggle with structured data QA due to errors in generating structured queries, while existing unified frameworks like TrustUQA work better with large LLMs.

Method: SCD with Error Prompt Mechanism (EPM) for error detection and customized messages during inference, plus two-stage distillation to transfer capabilities from large to small LLMs.

Result: Achieves best performance on 5 benchmarks with 3 structured data types using 8B models, closely approaching GPT4 on some datasets. Large LLMs with EPM surpass SOTA on most datasets.

Conclusion: SCD effectively enhances small LLMs' structured QA capabilities through self-correction distillation and error prompt mechanisms, demonstrating strong generalization and competitive performance.
...
TL;DR: HyCoRA is a hyper-contrastive role-adaptive learning framework that balances distinct and shared traits in multi-character role-playing using a hyper-half low-rank adaptation structure with role-specific and role-shared modules.


<details>
  <summary>Details</summary>
Motivation: Existing methods either use shared modules that ignore distinct role traits or specific modules that overlook shared traits, leading to poor personality learning and commonality modeling.
...
Result: Extensive experiments on English and Chinese benchmarks show superiority, with GPT-4 evaluations and visual analyses confirming capability to capture role characteristics.

Conclusion: HyCoRA effectively improves multi-character role-playing by balancing distinct and shared trait learning through its adaptive framework.
...
TL;DR: This paper introduces BARD10, a new balanced Bangla authorship attribution corpus, and shows that classical TF-IDF + SVM outperforms deep learning models, with Bangla stop-words serving as essential stylistic indicators.


<details>
  <summary>Details</summary>
Motivation: To address the need for balanced Bangla authorship attribution benchmarks and systematically analyze the impact of stop-word removal on authorship recognition across different genres.

Method: Created BARD10 corpus from 10 contemporary authors' blog/opinion prose, compared four classifiers (SVM, Bangla BERT, XGBoost, MLP) with uniform preprocessing on both BARD10 and BAAD16 datasets, analyzing stop-word removal effects.

Result: TF-IDF + SVM achieved best performance (0.997 macro-F1 on BAAD16, 0.921 on BARD10), Bangla BERT lagged by 5 points, BARD10 authors were highly sensitive to stop-word pruning while BAAD16 authors were robust, revealing genre-dependent reliance on stop-word signatures.

Conclusion: Bangla stop-words are essential stylistic indicators, finely calibrated ML models work well for short texts, and BARD10 provides a reproducible benchmark connecting formal literature with contemporary web dialogue for future transformer research.
...
TL;DR: Science fiction shows higher conceptual permeability between human, animal, and machine categories using masked language modeling, with machines being most fluid and humans most stable.


<details>
  <summary>Details</summary>
Motivation: To computationally measure Darko Suvin's theory of estrangement in science fiction by examining how ontological categories (human, animal, machine) become destabilized through lexical substitutions.

Method: Used masked language modeling (RoBERTa) on science fiction and general fiction corpora to generate lexical substitutes for masked referents, then classified them via Gemini and quantified conceptual slippage using retention rate, replacement rate, and entropy metrics.

Result: Science fiction exhibits heightened conceptual permeability, especially around machine referents which show significant cross-category substitution and dispersion, while human terms maintain semantic coherence and anchor substitutional hierarchies.

Conclusion: Estrangement in science fiction operates as controlled perturbation of semantic norms detectable through probabilistic modeling, and MLMs can serve as interpretive instruments for surfacing genre-conditioned ontological assumptions.
...
TL;DR: MLLMs struggle with cross-modality skill composition despite strategies like chain-of-thought prompting and fine-tuning, revealing significant gaps that need further research.
...
Method: Designed three evaluation tasks requiring sequential composition of two modality-dependent skills, tested open MLLMs using direct prompting and two-step cascaded inference, and explored chain-of-thought prompting and fine-tuning to improve composition.

Result: All evaluated MLLMs showed significant cross-modality skill composition gaps. Chain-of-thought prompting and fine-tuning improved performance but still left substantial gaps.

Conclusion: Current MLLMs have notable limitations in cross-modal skill composition, indicating the need for more research to enhance this capability.
...
TL;DR: This paper investigates three key features of human quantification that are challenging for Multimodal Large Language Models (MLLMs): quantifier ordering scales, usage ranges/prototypicality, and biases from the human approximate number system.


<details>
  <summary>Details</summary>
Motivation: Quantification is difficult for MLLMs, but the exact reasons are unclear since it interfaces with logic, pragmatics, and numerical domains. The paper aims to understand how human quantification features are encoded in models and how they differ from human cognition.

Method: The study examines three cross-linguistic features of human quantification: quantifier ordering scales, usage ranges/prototypicality, and biases from the human approximate number system. It investigates how these are encoded in MLLM architecture across different model types and languages.

Result: Clear differences were found between humans and MLLMs regarding these quantification features across various tasks, revealing disparities in how quantification is represented in vivo (humans) vs. in silico (models).

Conclusion: This work provides a foundation for understanding MLLMs as semantic and pragmatic agents, while cross-linguistic analysis helps determine if their quantification abilities are robust and stable across different languages.
...
TL;DR: Fine-tuning LLMs to compress context using learned tokens, achieving 2x-8x compression without major performance loss.
...
Result: Achieves 2x-8x compression ratios without significant performance degradation on benchmarks; matches alternative techniques with higher compression on 3B-parameter LLaMA.

Conclusion: Learned compression tokens enable effective context compression in LLMs, reducing resource requirements while maintaining performance.
...
TL;DR: The study examines whether architectural bias from English-first language models affects performance for structurally different languages, focusing on positional encodings and the trade-off between morphological complexity and word order flexibility.


<details>
  <summary>Details</summary>
Motivation: To investigate if the common practice of creating language model architectures for English first leads to degraded performance for languages structurally different from English, using positional encodings as a specific test case.
...
Result: Contrary to previous findings, no clear interaction was observed between positional encodings and morphological complexity or word order flexibility. Task choice, language selection, and metrics were found to be crucial for stable conclusions.

Conclusion: The study challenges previous assumptions about the relationship between positional encodings and linguistic typology, emphasizing the importance of methodological choices in drawing reliable conclusions about architectural bias in language models.
...
TL;DR: RelPrior is a new paradigm for document-level relation extraction using LLMs that uses relation as a prior to filter irrelevant entity pairs and avoid misjudgment from strict predefined relations, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: LLMs have performance gaps in document-level relation extraction due to noise from unrelated entity pairs and misjudgment when entities have semantic associations but don't match predefined relation labels.

Method: RelPrior uses binary relation as a prior to filter irrelevant entity pairs, then uses predefined relations as prior to match entities for triple extraction instead of direct relation prediction.
...
Conclusion: The RelPrior paradigm effectively addresses key challenges in LLM-based document relation extraction by using relation as prior knowledge, improving performance by reducing noise and avoiding strict relation labeling constraints.
...
TL;DR: Instruction-tuned and prompted LLMs underperform compared to domain-specific fine-tuned ByT5-Sanskrit model on Sanskrit poetry-to-prose conversion, despite the task's complexity requiring multi-step reasoning.


<details>
  <summary>Details</summary>
Motivation: To test whether LLMs can serve as universal solutions for low-resource, morphologically rich languages like Sanskrit, particularly for complex tasks like poetry-to-prose conversion that require multi-step reasoning.

Method: Compared instruction-tuned LLMs and in-context prompted LLMs (using Paninian grammar and classical commentary heuristics) with fully fine-tuned ByT5-Sanskrit Seq2Seq model on Sanskrit poetry-to-prose conversion task.

Result: Domain-specific fine-tuning of ByT5-Sanskrit significantly outperforms all LLM approaches. Human evaluation strongly supports this finding with high correlation scores. Prompting strategies work as alternatives when domain-specific data is unavailable.

Conclusion: Specialized domain-specific models outperform general-purpose LLMs for complex tasks in low-resource languages like Sanskrit, though LLM prompting provides viable alternatives when domain data is unavailable.
...
TL;DR: Analysis of BabyLM corpus and CHILDES age-groups shows limited syntactic differentiation by age, but syntactic knowledge helps interpret model performance. Curriculum learning approaches, particularly using syntactically categorizable data subsets, improve performance more than full noisy corpora.


<details>
  <summary>Details</summary>
Motivation: To understand how syntactic properties of child language data (BabyLM corpus and CHILDES) relate to model performance and to explore curriculum learning approaches inspired by cognitive development.

Method: Examined syntactic properties of BabyLM corpus and CHILDES age-groups, analyzed syntactic differentiation by age, and explored developmental and cognitively inspired curriculum learning approaches.

Result: CHILDES shows weak syntactic differentiation by age, syntactic knowledge helps interpret model performance, and curriculum learning using syntactically categorizable data subsets improves performance more than using full noisy corpora.

Conclusion: Syntactic knowledge of training data aids model interpretation, and selective use of syntactically categorizable data is more beneficial for performance than using entire noisy corpora in curriculum learning.
...
TL;DR: An encoder-based system using SciBERT for extracting key entities from astronomy papers, outperforming GPT baselines.


<details>
  <summary>Details</summary>
Motivation: Automate extraction of key entities and contextual information from rapidly expanding astronomy literature.

Method: Multi-task transformer-based system built on SciBERT, fine-tuned on astronomy corpora with stochastic sampling and majority voting.

Result: Significantly outperforms open-weight GPT baseline despite simplicity and low-cost implementation.

Conclusion: The proposed system effectively extracts astronomical entities and attributes, demonstrating superior performance over baseline methods.
...
TL;DR: An embedding-based framework to detect gender bias in LLMs for formative feedback, showing asymmetric semantic responses to gender substitutions across six models.


<details>
  <summary>Details</summary>
Motivation: As teachers increasingly use GenAI in education, there's a need to benchmark LLMs for pedagogical purposes and detect bias in formative feedback.

Method: Used 600 authentic student essays with controlled counterfactuals: implicit cues via lexicon-based gendered term swaps and explicit cues via gendered author backgrounds. Analyzed six LLMs using embedding-based divergence metrics, permutation tests, and dimensionality reduction.

Result: All models showed larger semantic shifts for male-female counterfactuals than female-male. Only GPT and Llama models were sensitive to explicit gender cues. Qualitative analysis revealed linguistic differences (autonomy-supportive feedback for male cues vs. controlling feedback for female cues).

Conclusion: State-of-the-art LLMs exhibit persistent gender biases in feedback, highlighting the need for fairness auditing, reporting standards, and practical guidance for equitable deployment in education.
...
TL;DR: VocalBench-zh is a comprehensive Mandarin speech-to-speech benchmark with 10 subsets and 10K+ instances to address the lack of systematic evaluation tools for Chinese speech interaction models.


<details>
  <summary>Details</summary>
Motivation: The scarcity of comprehensive speech-to-speech benchmarks in Mandarin contexts impedes systematic evaluation for developers and hinders fair model comparison for users, despite Mandarin being widely supported by most multi-modal LLMs.

Method: Created VocalBench-zh, an ability-level divided evaluation suite adapted to Mandarin context with 10 well-crafted subsets and over 10K high-quality instances covering 12 user-oriented characters.
...
Conclusion: VocalBench-zh addresses the critical gap in Mandarin S2S evaluation and provides a standardized benchmark for fair model comparison and systematic development of speech interaction systems.
...
TL;DR: A novel NL-to-SQL framework with error correction mechanism that achieves 12% accuracy improvement over baselines by integrating prompt tuning, RAG, and medical-inspired diagnostic process.
...
Method: Proposes error correction framework inspired by medical diagnostics: diagnoses error types, identifies causes, provides fixing instructions, and applies corrections. Integrates prompt tuning, embedding fine-tuning, and RAG with external knowledge bases.

Result: Achieves significant 12% accuracy improvement over existing baselines through comprehensive experiments.
...
TL;DR: ParliaBench is a benchmark for parliamentary speech generation that addresses the unique challenges of political authenticity and ideological consistency, introducing novel evaluation metrics and showing fine-tuned LLMs significantly outperform baselines.
...
Method: Created ParliaBench benchmark using UK Parliament speeches dataset, introduced evaluation framework combining computational metrics with LLM-as-a-judge assessments, proposed Political Spectrum Alignment and Party Alignment metrics, and fine-tuned five LLMs generating 28k speeches.

Result: Fine-tuned models showed statistically significant improvements across most metrics, with novel political alignment metrics demonstrating strong discriminative power for political dimensions.

Conclusion: The ParliaBench benchmark effectively addresses parliamentary speech generation challenges, with fine-tuning significantly improving performance and novel political metrics providing valuable evaluation of ideological positioning.
...
TL;DR: VLLMs can understand hierarchical table structures from scientific articles without special processing, as shown through experiments on the CHiTab benchmark derived from PubTables-1M.


<details>
  <summary>Details</summary>
Motivation: To investigate whether Vision Large Language Models can interpret complex hierarchical table structures in scientific papers without additional processing.

Method: Used PubTables-1M dataset to create CHiTab benchmark, employed prompt engineering strategies, evaluated multiple state-of-the-art VLLMs both off-the-shelf and fine-tuned, and compared with human performance.

Result: Generic VLLMs not specifically designed for table structure understanding can perform this task, though with limitations.

Conclusion: The study provides insights into VLLMs' capabilities for processing complex tables and guidance for future integration of structured data understanding into general-purpose VLLMs.
...
Method: Simulates multi-round reviewer-author debates using LLM-based multi-agent collaboration, extracts diverse opinion relations as typed edges in a heterogeneous graph, and applies graph neural networks for reasoning.

Result: Outperforms strong baselines with 15.73% average relative improvement across three datasets.

Conclusion: Modeling detailed reviewer-author debate structures through graph reasoning significantly enhances review decision quality.
...
TL;DR: Proposes a multi-agent framework to refine LLM responses by having specialized agents handle factuality, personalization, and coherence aspects, with dynamic communication for adaptive coordination.


<details>
  <summary>Details</summary>
Motivation: LLMs often struggle with personalization and specific knowledge, and existing single-LLM refinement methods fail to consider diverse conversational aspects effectively.
...
Conclusion: Multi-agent refinement with dynamic coordination effectively improves conversational quality by addressing multiple key aspects simultaneously.
...
TL;DR: Proposes AgentPRM, a process reward model for LLM agents that evaluates sequential decisions based on goal proximity and progress, using TD-based estimation for efficient training. Shows 8x compute efficiency and robust scaling.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with multi-turn decision-making tasks requiring sequential decisions based on environmental feedback. Existing methods rely on prompt engineering or expert trajectory fine-tuning, which have limitations.

Method: Develop AgentPRM to capture interdependence between sequential decisions and their contribution to final goals. Use Temporal Difference-based estimation with Generalized Advantage Estimation for scalable training data labeling.

Result: AgentPRM achieves over 8x compute efficiency compared to baselines, demonstrates robust improvement with increased test-time compute, and works effectively across different agentic tasks.

Conclusion: AgentPRM provides an effective approach for guiding LLM agents in multi-turn decision-making tasks, enabling better progress tracking and exploration-exploitation balance while being highly compute-efficient.
...
TL;DR: DPRM is a dual implicit process reward model that trains two PRMs for CoT and KG reasoning in MHQA tasks, using outcome signals without explicit annotations and introducing consistency constraints between reasoning paths.


<details>
  <summary>Details</summary>
Motivation: Existing implicit PRMs cannot handle graph structure constraints in KGs and fail to capture inconsistencies between CoT and KG paths in MHQA tasks, limiting their effectiveness for multi-step reasoning.

Method: Train two implicit PRMs (KG-PRM and CoT-PRM) that derive step-level rewards from outcome signals via reward parameterization without explicit annotations. KG-PRM learns structural constraints from KGs using preference pairs, and introduces consistency constraints between CoT and KG reasoning steps.

Result: Outperforms 13 baselines on multiple datasets with up to 16.6% improvement on Hit@1 metric.

Conclusion: DPRM effectively addresses the limitations of existing implicit PRMs by handling graph structure constraints and ensuring consistency between different reasoning paths, significantly improving performance in MHQA tasks.
...
TL;DR: Implementation of DYNARTmo, a dynamic articulatory model that generates continuous articulator movements using speech gestures and gesture scores for hierarchical speech production control.


<details>
  <summary>Details</summary>
Motivation: To create a neurobiologically inspired computational framework for simulating hierarchical control of speech production from linguistic representation to articulatory-acoustic realization.

Method: Uses a gesture inventory, coordinates gestures in a gesture score, and translates them into continuous articulator trajectories controlling the DYNARTmo vocal tract model.

Result: Development of the DYNARTmo model capable of generating continuous articulator movements based on speech gestures.

Conclusion: DYNARTmo provides a computational framework for simulating the complete chain of speech production control from linguistic to articulatory levels.
...
<details>
  <summary>Details</summary>
Motivation: Current Turkish embedding models often rely on machine-translated datasets, which may limit their accuracy and semantic understanding of the Turkish language.

Method: Utilizes diverse datasets and advanced training techniques including matryoshka representation learning to create robust embeddings that adapt to resource-constrained environments with faster encoding.

Result: Significant improvements on Turkish STS-b-TR dataset using Pearson and Spearman correlation metrics, and surpasses state-of-the-art model Emrecan on All-NLI-TR and STS-b-TR benchmarks with 1-4% improvement.
...
TL;DR: PCRLLM framework enables verifiable step-by-step reasoning in LLMs by structuring outputs with explicit premises, rules, and conclusions, supporting formal validation and multi-LLM collaboration.


<details>
  <summary>Details</summary>
Motivation: LLMs often lack logical coherence and explicit inference rules, making their reasoning untrustworthy and difficult to verify.

Method: Constrains reasoning to single-step inferences with explicit specification of premises, rules, and conclusions in natural language, enabling verification against target logic.

Result: Enables chain-level validation in black-box settings and facilitates systematic multi-LLM collaboration with formal rule integration.

Conclusion: PCRLLM provides a framework that combines natural language expressiveness with formal rigor, addressing trustworthiness concerns in LLM reasoning through verifiable step-level inference.
...
TL;DR: TRACE introduces a novel reward signal based on conversational geometry (embedding trajectory dynamics) that achieves comparable performance to text-based methods for LLM alignment in multi-turn conversations, with hybrid models achieving the best results.


<details>
  <summary>Details</summary>
Motivation: Current LLM alignment approaches focus only on text content, ignoring the rich signal from interaction dynamics. This work aims to leverage conversational geometry as a complementary reward signal.

Method: TRACE extracts geometric properties from dialogue embedding trajectories to create a structural reward signal, which can be used alone or combined with text-based analysis in hybrid models.

Result: TRACE alone achieves 68.20% pairwise accuracy, comparable to text-based methods (70.04%). Hybrid models combining both approaches achieve the highest performance at 80.17%, demonstrating complementary benefits.

Conclusion: Interaction dynamics are as powerful as content for predicting conversational success, offering a privacy-preserving framework for agent alignment and diagnostic analysis of collaboration patterns.
...
TL;DR: Study shows social bot detectors are vulnerable to shortcut learning from spurious textual correlations, with 32% accuracy drop. Proposed LLM-based counterfactual augmentation strategies improve performance by 56% under shortcut scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing social bot detectors lack robustness in real-world scenarios due to shortcut learning, where models rely on spurious correlations rather than causal features, particularly with manipulable textual cues.

Method: Designed shortcut scenarios with spurious associations between user labels and textual cues. Proposed mitigation strategies using large language models and counterfactual data augmentation across three levels: individual user text, overall dataset distribution, and model's causal extraction ability.

Result: Shifts in irrelevant feature distributions caused 32% average relative accuracy drop in baseline detectors. The proposed mitigation strategies achieved 56% average relative performance improvement under shortcut scenarios.

Conclusion: Shortcut learning significantly impacts social bot detector robustness. Counterfactual data augmentation using LLMs effectively mitigates this issue by improving model resilience to spurious correlations.
...
TL;DR: SPEAR-MM is a framework that prevents catastrophic forgetting in LLMs during financial domain adaptation by selectively freezing/restoring transformer layers via model merging, achieving better general capability retention than standard methods.


<details>
  <summary>Details</summary>
Motivation: LLMs adapted to financial domains often lose general reasoning capabilities needed for customer interactions and complex analysis, creating a need for methods that preserve these while enabling domain specialization.

Method: Approximates layer-wise impact on benchmarks through post-hoc analysis, then selectively freezes or restores transformer layers using spherical interpolation merging.

Result: Applied to LLaMA-3.1-8B, achieves 91.2% retention of general capabilities vs 69.7% for standard pretraining, maintains 94% of domain adaptation gains, and reduces computational costs by 90%.

Conclusion: SPEAR-MM provides interpretable trade-off control and efficient domain adaptation crucial for resource-constrained financial institutions.
...
<details>
  <summary>Details</summary>
Motivation: Current RAG approaches focus on queries where only small portions of the corpus are relevant, failing to handle aggregative queries that require gathering and reasoning over information from many documents.
...
Result: Experiments on new datasets (HOTELS and WORLD CUP) and public benchmarks show S-RAG substantially outperforms both common RAG systems and long-context LLMs.

Conclusion: S-RAG effectively addresses the gap in handling aggregative queries and demonstrates superior performance compared to existing approaches.
...
TL;DR: The paper introduces Bangla-SGP, a novel parallel dataset for Bangla Sign Language translation, addressing the low-resource challenge by providing 1,000 human-annotated and 3,000 synthetically generated sentence-gloss pairs, and evaluates transformer models on this task.


<details>
  <summary>Details</summary>
Motivation: Bangla Sign Language translation is a low-resource NLP task with limited existing research focused only on word and alphabet level detection, lacking large-scale datasets for sentence-level translation.

Method: Created a parallel dataset with 1,000 human-annotated sentence-gloss pairs and augmented with ~3,000 synthetic pairs using rule-based RAG pipeline with syntactic/morphological rules. Fine-tuned transformer models (mBart50, Google mT5, GPT4.1-nano) for sentence-to-gloss translation.

Result: Developed Bangla-SGP dataset with 4,000 total sentence-gloss pairs (1,000 human-annotated + 3,000 synthetic). Evaluated model performance using BLEU scores and compared gloss-translation consistency across their dataset and RWTH-PHOENIX-2014T benchmark.

Conclusion: The work successfully addresses the data scarcity in Bangla Sign Language translation by creating a substantial parallel dataset and demonstrates the effectiveness of transformer models for this sentence-level translation task.
...
TL;DR: AlphaResearch is an autonomous LLM agent that discovers new algorithms through iterative proposal, verification, and optimization in dual research environments, achieving competitive performance against human researchers.


<details>
  <summary>Details</summary>
Motivation: Large language models excel at solving verifiable problems but struggle with discovering unknown algorithms, highlighting the need for autonomous research agents capable of open-ended discovery.

Method: Uses dual research environment combining execution-based verification and simulated peer review; iteratively proposes ideas, verifies them, and optimizes proposals; evaluated on AlphaResearchComp benchmark with 8 open-ended algorithmic problems.

Result: Achieves 2/8 win rate against human researchers; discovers best-known algorithm for circle packing problem; outperforms human researchers and baselines like AlphaEvolve.

Conclusion: Demonstrates feasibility of accelerating algorithm discovery with LLMs, though challenges remain as shown by 6/8 failure cases; provides insights for future autonomous research systems.
...
TL;DR: This paper investigates the challenges and potential of monitoring Large Reasoning Models (LRMs) through their chain-of-thought (CoT) reasoning, focusing on verbalization faithfulness and monitor reliability.


<details>
  <summary>Details</summary>
Motivation: To explore AI safety through CoT monitorability - monitoring potential model misbehavior like shortcuts or sycophancy via reasoning traces, addressing challenges of truthful verbalization and reliable detection.

Method: Empirical evidence and correlation analyses across mathematical, scientific, and ethical domains; investigation of CoT intervention methods; proposal of MoME paradigm for LLMs to monitor other models' CoT with structured judgments.

Result: Systematic investigation reveals challenges in verbalization faithfulness and monitor reliability, with evidence showing how different CoT interventions affect monitoring effectiveness.

Conclusion: CoT monitorability offers potential for AI safety but faces fundamental challenges; the proposed MoME paradigm provides a new approach for structured monitoring through reasoning traces.
...
TL;DR: Methodology for constructing a high-quality SRL dataset from WSJ OntoNotes 5.0 and adapting it for Opinion Role Labeling tasks, resulting in 97,169 predicate-argument instances mapped to ORL schema.


<details>
  <summary>Details</summary>
Motivation: To create a reusable resource for researchers aiming to leverage Semantic Role Labeling to enhance Opinion Role Labeling, particularly in low-resource opinion mining scenarios.

Method: Implemented a reproducible extraction pipeline using PropBank annotation framework: aligning predicate-argument structures with text, converting syntactic tree pointers to spans, and applying rigorous cleaning for semantic fidelity.

Result: Produced a dataset of 97,169 predicate-argument instances with defined Agent (ARG0), Predicate (REL), and Patient (ARG1) roles, mapped to ORL's Holder, Expression, and Target schema.

Conclusion: This work provides a detailed methodology and reusable dataset for enhancing Opinion Role Labeling through Semantic Role Labeling, with comprehensive documentation of extraction algorithms and statistical analysis.
...
<details>
  <summary>Details</summary>
Motivation: To analyze how LLMs express and shift moral judgments in social contexts through persona role-play, using a systematic benchmark.

Method: Used Moral Foundations Questionnaire (MFQ) to quantify moral susceptibility (variability across personas) and moral robustness (variability within personas) across different LLM families and sizes.

Result: Claude family is most robust, followed by Gemini and GPT-4; larger models are more susceptible; robustness and susceptibility are positively correlated, especially at family level.

Conclusion: Persona conditioning systematically shapes moral behavior in LLMs, with model family being the primary factor for robustness and model size affecting susceptibility.
...
TL;DR: TaH is a dynamic latent thinking method that iterates deeper only at hard tokens to improve LLM reasoning while avoiding overthinking on easy tokens, achieving significant accuracy gains with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: To address the latent overthinking phenomenon in recurrent transformers where easy tokens that are already correct get revised into errors during additional iterations, and to improve reasoning capabilities under parameter constraints.

Method: Proposes Think-at-Hard (TaH) with: 1) lightweight neural decider to trigger latent iterations only for likely incorrect tokens, 2) LoRA modules for hard-token refinement, and 3) duo-causal attention mechanism for cross-iteration information flow while maintaining parallelism.

Result: TaH boosts reasoning performance across five benchmarks while maintaining same parameter count. Delivers 8.1-11.3% accuracy gains vs baselines that iterate twice for all tokens, exempting 94% of tokens from second iteration. Achieves 4.0-5.0% gains vs single-iteration models, increasing to 8.5-12.6% with <3% additional parameters.

Conclusion: TaH effectively addresses overthinking by dynamically focusing computational resources on hard tokens, significantly improving LLM reasoning performance with minimal parameter overhead through selective latent iterations and specialized refinement mechanisms.
...
TL;DR: LMs can learn to generate faithful natural language descriptions of their internal computations through fine-tuning with existing interpretability data, and self-explanation generally works better than cross-model explanation.


<details>
  <summary>Details</summary>
Motivation: To investigate whether language models can leverage their privileged access to internal computations to produce scalable explanations that complement existing interpretability methods.
...
Result: Explainer models show non-trivial generalization with only tens of thousands of examples, and self-explanation outperforms cross-model explanation even when the other model is more capable.
...
Method: Three-stage framework: (1) transform video content into fine-grained captions using vision-language models, (2) organize captions into four semantic slots (action, object, context, environment), (3) generate slot-wise explanations for anomaly decisions.
...
Conclusion: TbVAD enables interpretable video anomaly detection through language-based representation and semantic slot reasoning, offering a more explainable alternative to traditional visual feature-based methods.
...
TL;DR: MoDM is a cryo-EM framework that reconstructs molecular structures using second-order moments from two datasets with different orientation distributions, achieving accurate recovery through convex relaxation.


<details>
  <summary>Details</summary>
Motivation: To improve cryo-EM reconstruction by leveraging multiple datasets with different experimental conditions, specifically different orientation distributions, to enhance reconstruction quality.

Method: Method of double moments (MoDM) uses second-order moments from two datasets - one with uniform orientation distribution and another with unknown non-uniform distribution. A convex-relaxation-based algorithm processes these moments for structure recovery.

Result: The method generically uniquely determines molecular structures (up to global rotation/reflection) and achieves accurate reconstruction using only second-order statistics, demonstrating improved quality over single-dataset approaches.

Conclusion: Collecting and modeling multiple datasets under different experimental conditions substantially enhances reconstruction quality in computational imaging, showing the value of dataset diversity.
...
TL;DR: SSViT is a novel Transformer-based deep learning framework for reconstructing high-quality videos from 8-bit folded modulo camera data, achieving state-of-the-art performance in modulo video recovery through selective spatiotemporal processing.


<details>
  <summary>Details</summary>
Motivation: Conventional image sensors have limited dynamic range causing saturation in HDR scenes. Modulo cameras fold irradiance to avoid saturation but require specialized unwrapping algorithms. Despite being introduced over a decade ago, progress in deep learning-based modulo recovery has been slow, and standard HDR methods are unsuitable for this task.

Method: Proposed Selective Spatiotemporal Vision Transformer (SSViT) - the first deep learning framework for modulo video reconstruction. It employs a token selection strategy to improve efficiency and concentrate on critical regions, capturing global dependencies and spatial-temporal relationships crucial for resolving folded video frames.

Result: SSViT produces high-quality reconstructions from 8-bit folded videos and achieves state-of-the-art performance in modulo video recovery. Experiments confirm its effectiveness in handling folded modulo camera data.

Conclusion: Transformers can effectively capture the global dependencies needed for modulo recovery, and the proposed SSViT framework successfully addresses the challenges of modulo video reconstruction through selective spatiotemporal processing, representing a significant advancement in this field.
...
TL;DR: ASAG is a novel guidance method for diffusion models that uses adversarial optimal transport theory to intentionally disrupt attention mechanisms, improving sample quality without model retraining.


<details>
  <summary>Details</summary>
Motivation: Existing guidance methods like CFG rely on heuristic perturbations without principled foundations, lacking systematic approaches for attention disruption.

Method: ASAG injects adversarial costs in self-attention layers using Sinkhorn algorithm to reduce pixel-wise similarity between queries and keys, weakening misleading attention alignments.

Result: ASAG consistently improves text-to-image diffusion quality and enhances controllability/fidelity in downstream applications like IP-Adapter and ControlNet.

Conclusion: ASAG provides a lightweight, plug-and-play guidance method that improves diffusion model reliability through principled attention disruption based on optimal transport theory.
...
TL;DR: LiveNeRF enables real-time face replacement at 33 FPS with high visual quality for applications like dubbing, virtual avatars, and cross-cultural content adaptation.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of existing face replacement methods by achieving real-time performance for practical deployment in live streaming, video conferencing, and interactive media.

Method: LiveNeRF framework - a neural radiance field-based approach optimized for real-time face replacement.

Result: Achieves 33 FPS real-time performance with superior visual quality compared to existing methods.

Conclusion: The technology benefits content creators, educators, and speech-impaired individuals, but requires responsible deployment with consent verification and detection systems to prevent misuse.
...
TL;DR: TrackStudio is a GUI-based toolkit that combines existing open-source tools into an accessible pipeline for markerless motion tracking, requiring no programming skills and delivering reliable performance across diverse settings.


<details>
  <summary>Details</summary>
Motivation: There's a gap in accessible, integrated solutions for markerless motion tracking that non-experts can use across diverse settings, as existing tools require substantial technical expertise.

Method: Combines established open-source tools into a single, modular GUI-based pipeline providing automatic 2D/3D tracking, calibration, preprocessing, feature extraction, and visualization without programming requirements.

Result: Validated across 76 participants in challenging conditions, achieving average inter-frame correlations >0.98 and triangulation errors <13.6mm for hand tracking, with stable and consistent performance using both low-cost and high-resolution cameras.

Conclusion: TrackStudio provides a practical, accessible route into markerless tracking for researchers and laypeople who need reliable performance without specialist expertise, with extensibility to other body and face regions.
...
TL;DR: Deep learning CNN model successfully classifies coronary artery calcium scores into six clinical categories with high accuracy (96.5%) and agreement (Cohen's kappa 0.962), showing potential to automate current time-intensive semiautomatic scoring practices.


<details>
  <summary>Details</summary>
Motivation: Cardiovascular disease causes high mortality worldwide, and current coronary artery calcium (CAC) scoring requires time-intensive semiautomatic analysis by radiologists and trained radiographers, creating a need for automated solutions.

Method: Developed a deep learning convolutional neural network (CNN) model using 68 patient cardiac CT scans with semiautomatic CAC scores as reference labels. Dataset was divided into training, validation, and test sets using ECG-gated GE Discovery 570 Cardiac SPECT/CT camera images.

Result: Model achieved 96.5% accuracy and Cohen's kappa of 0.962 on six-class CAC scoring categorization. Of 32 misclassifications, 26 tended towards overestimating CAC. Model showed high generalizability to test data.

Conclusion: The CNN model demonstrates viability for stratifying calcium scores into six clinical categories, providing accurate and consistent results comparable to current semiautomatic practice with good generalizability.
...
Method: Developed a novel distillation technique that embeds distribution of plausible apparent motions (motion profiles) using optical flow networks and diverse video data through self-supervised training framework.

Result: FlowFeat significantly enhances representational power of five state-of-the-art encoders across three dense tasks: video object segmentation, monocular depth estimation, and semantic segmentation. It's computationally inexpensive and robust to inaccurate flow estimation.

Conclusion: FlowFeat advances reliable and versatile dense image representations by providing high spatial detail with geometric and semantic cues while maintaining temporal consistency.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches lack robust intra-modal mechanisms to assess token significance and fail to model fine-grained uncertainty, leading to poor generalization in complex scenes and inaccurate region-word correspondences.

Method: Incorporates significance-aware and granularity-aware modeling with region-level uncertainty modeling, using modality-specific biases to identify salient features and representing region features as Gaussian mixture distributions.
...
Conclusion: The proposed unified approach effectively addresses fundamental limitations in fine-grained image-text alignment by combining significance assessment and uncertainty modeling, demonstrating superior performance and improved generalization.
...
TL;DR: UltraGS is a Gaussian Splatting framework for ultrasound imaging that enables novel view synthesis with depth-aware Gaussian representation and ultrasound-specific rendering.


<details>
  <summary>Details</summary>
Motivation: Limited field of view in ultrasound imaging complicates novel view synthesis, which is crucial for clinical diagnostics and comprehensive anatomical assessment.

Method: Depth-aware Gaussian splatting with learnable field of view for each Gaussian, combined with SH-DARS rendering function that incorporates ultrasound-specific wave physics (depth attenuation, reflection, scattering) using spherical harmonics.

Result: State-of-the-art performance on three datasets with PSNR up to 29.55, SSIM up to 0.89, MSE as low as 0.002, and real-time synthesis at 64.69 fps.

Conclusion: UltraGS effectively addresses ultrasound view synthesis challenges with superior performance and real-time capability, supported by a new clinical ultrasound dataset for benchmarking.
...
TL;DR: VectorSynth is a diffusion-based framework for pixel-accurate satellite image synthesis conditioned on polygonal geographic annotations with semantic attributes, enabling fine-grained spatial edits and map-informed content generation.


<details>
  <summary>Details</summary>
Motivation: Prior text- or layout-conditioned models lack dense cross-modal correspondences between imagery and semantic vector geometry, limiting fine-grained, spatially grounded edits in satellite image synthesis.

Method: A vision language alignment module produces pixel-level embeddings from polygon semantics, which guide a conditional image generation framework to respect both spatial extents and semantic cues. The framework supports interactive workflows mixing language prompts with geometry-aware conditioning.

Result: Strong improvements over prior methods in semantic fidelity and structural realism, with the trained vision language model demonstrating fine-grained spatial grounding. The system enables rapid what-if simulations, spatial edits, and map-informed content generation.

Conclusion: VectorSynth successfully learns dense cross-modal correspondences that align imagery and semantic vector geometry, providing a powerful framework for pixel-accurate satellite image synthesis with fine-grained spatial control.
...
TL;DR: Auto-US is an AI system that combines ultrasound videos with clinical text for automated diagnosis, achieving 86.73% accuracy on a new dataset of 495 videos across 5 categories and 3 organs.


<details>
  <summary>Details</summary>
Motivation: Existing AI-assisted ultrasound diagnosis research has limitations in dataset diversity, diagnostic performance, and clinical applicability, creating a need for more comprehensive and clinically relevant solutions.

Method: Developed Auto-US system with CTU-Net for ultrasound video classification and integrated large language models to generate diagnostic suggestions from combined ultrasound video data and clinical text.

Result: Achieved 86.73% accuracy in ultrasound video classification on the CUV Dataset, with diagnostic suggestions scoring over 3/5 when validated by professional clinicians.

Conclusion: Auto-US demonstrates effectiveness and clinical potential for real-world ultrasound applications, providing a comprehensive solution that bridges the gap between AI analysis and clinical practice.
...
TL;DR: Proposes PGCD and DAPD methods for class incremental medical image segmentation that use prototype-guided calibration and dual-aligned distillation to better preserve old knowledge while learning new classes.


<details>
  <summary>Details</summary>
Motivation: Existing methods either treat all regions/channels equally (hindering old knowledge preservation) or focus only on global prototype alignment while ignoring local representations in new data (causing knowledge degradation).

Method: PGCD uses prototype-to-feature similarity to calibrate class-specific distillation intensity across spatial regions. DAPD aligns local prototypes from current model with both global and local prototypes to enhance old class performance.
...
Conclusion: The proposed PGCD and DAPD methods effectively address limitations in class incremental medical image segmentation by better preserving old knowledge through calibrated distillation and dual prototype alignment.
...
TL;DR: Filtered-ViT is a vision transformer architecture with SMART-VMF filtering that provides robust defense against multiple adversarial patches and natural artifacts while maintaining high accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing defenses fail against multiple adversarial patches and real-world artifacts in safety-critical domains like healthcare, where reliable vision systems are essential.

Method: Integrates SMART Vector Median Filtering (SMART-VMF) - a spatially adaptive, multi-scale, robustness-aware mechanism for selective suppression of corrupted regions while preserving semantic detail.

Result: Achieves 79.8% clean accuracy and 46.3% robust accuracy on ImageNet with LaVAN multi-patch attacks (4 simultaneous 1% patches), outperforming existing defenses. Successfully mitigates natural artifacts in medical imagery without degrading diagnostic content.
...
<details>
  <summary>Details</summary>
Motivation: Challenge the conventional view that initial noise in diffusion models is purely random, and demonstrate that it actually contains rich semantic information that can be manipulated for improved content generation.

Method: A two-step training-free process: 1) Semantic Erasure - remove unwanted semantics from noise using information theory principles, 2) Semantic Injection - leverage the equivalence between diffusion generation and semantic injection to add desired semantics to cleaned noise.

Result: The method is consistently effective across various text-to-content models based on both DiT and UNet architectures, providing a universal tool for consistent generation.

Conclusion: The approach presents a novel perspective for optimizing diffusion model generation by revealing and exploiting the semantic patterns hidden in initial noise, offering a universal training-free solution for content control.
...
<details>
  <summary>Details</summary>
Motivation: Existing cross-modal hashing methods rely on fully annotated datasets and are vulnerable to label noise, while also ignoring semantic overlaps in multi-label data, limiting robustness and generalization.

Method: Proposes SCBCH with two modules: Cross-modal Semantic-Consistent Classification (CSCC) for estimating sample reliability and reducing noisy label impact, and Bidirectional Soft Contrastive Hashing (BSCH) for adaptive contrastive learning using soft contrastive pairs based on semantic overlap.
...
Conclusion: DCDNet effectively addresses feature entanglement in cross-domain few-shot segmentation through systematic decoupling and adaptive fusion, achieving superior performance in both generalization and adaptation capabilities.
...
TL;DR: A robust pipeline for multilabel chest X-ray classification using SE-ResNeXt101 backbone with Label-Graph Refinement module that learns inter-label relationships to improve performance.


<details>
  <summary>Details</summary>
Motivation: To develop a practical and reproducible method for multilabel chest X-ray classification that addresses challenges like extreme class imbalance, asymmetric error costs, and label co-occurrence while maintaining computational efficiency.

Method: Fine-tuned SE-ResNeXt101 with sigmoid head for 14 thoracic findings, using Multilabel Iterative Stratification for cross-validation, Asymmetric Loss for class imbalance, mixed-precision training, and a novel Label-Graph Refinement module that learns sparse inter-label coupling matrix for logit refinement.

Result: Strong baseline achieves 92.64% macro AUC, with Label-Graph Refinement consistently improving validation macro AUC across folds with negligible computational overhead.

Conclusion: The proposed method provides a practical, hardware-friendly approach for stronger multilabel CXR classifiers without requiring extra annotations, offering reproducible performance improvements.
...
TL;DR: PC-Diffusion is a lightweight framework that uses a preference classifier to align diffusion models with human preferences, avoiding the computational cost and instability of full model fine-tuning in DPO methods.


<details>
  <summary>Details</summary>
Motivation: Existing DPO methods for diffusion models have high computational costs due to full model fine-tuning and are sensitive to reference model quality, causing instability and bias.
...
Conclusion: PC-Diffusion provides an effective alternative to DPO for aligning diffusion models with human preferences, offering theoretical guarantees and practical efficiency advantages.
...
TL;DR: A foundation model for SAR land-cover classification using Dynamic Instance and Contour Consistency Contrastive Learning (DI3CL) that outperforms existing methods across various tasks.
...
Method: Proposed DI3CL pre-training framework with Dynamic Instance module for global contextual awareness and Contour Consistency module for structural discrimination, trained on large-scale SARSense dataset (460,532 images).

Result: Extensive experiments show DI3CL consistently outperforms existing methods across SAR land-cover mapping, water body detection, and road extraction tasks.

Conclusion: The foundation model serves as a robust cornerstone for accelerating downstream SAR land-cover classification models with improved generalization capability.
...
TL;DR: Q-Scorer addresses the mismatch between discrete MLLM outputs and continuous IQA scores by adding a regression module and IQA-specific tokens, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: The inherent mismatch between discrete token outputs of MLLMs and continuous quality scores in IQA tasks causes conversion errors and semantic confusion, limiting MLLM performance.

Method: Proposes Q-Scorer framework with lightweight regression module and IQA-specific score tokens integrated into MLLM pipeline.

Result: Achieves state-of-the-art performance across multiple IQA benchmarks, generalizes well to mixed datasets, and improves when combined with other methods.

Conclusion: Q-Scorer effectively bridges the gap between discrete MLLM outputs and continuous IQA requirements, enhancing both IQA performance and preserving original MLLM capabilities.
...
TL;DR: Sparse3DPR is a training-free framework for 3D scene understanding using sparse-view RGB inputs and LLM reasoning, achieving significant improvements in accuracy and efficiency over existing methods.


<details>
  <summary>Details</summary>
Motivation: Training-free approaches for 3D scene understanding offer flexibility but typically suffer from accuracy and efficiency issues in practical deployment.

Method: Proposes hierarchical plane-enhanced scene graph with spatial anchors and task-adaptive subgraph extraction to filter irrelevant information, leveraging pre-trained LLMs' reasoning capabilities.
...
Conclusion: Sparse3DPR demonstrates robust generalization capability and superior efficiency in training-free 3D scene understanding.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for prostate cancer lesion segmentation achieve limited performance (Dice scores ≤0.32), necessitating better imaging enhancements to improve segmentation accuracy.

Method: Comprehensive evaluation across six state-of-the-art segmentation architectures using 200 patients with co-registered CDI^s, DWI, and ADC sequences. Tested CDI^s integration as enhancement to standard diffusion-based protocols.

Result: CDI^s integration reliably enhances or preserves segmentation performance in 94% of configurations, with up to 72.5% relative improvement over baseline. CDI^s + DWI achieved significant improvements in half of architectures with zero degradation instances.
...
TL;DR: SSOMotion is a human motion synthesis framework that uses Scene Semantic Occupancy (SSO) for unified scene representation, incorporating semantic understanding through CLIP encoding and bi-directional tri-plane decomposition for efficient computation.


<details>
  <summary>Details</summary>
Motivation: Current human motion synthesis methods focus mainly on scene structure but ignore semantic understanding, which is crucial for realistic motion in 3D scenes.

Method: Proposes SSOMotion framework with bi-directional tri-plane decomposition for compact SSO representation, CLIP encoding for semantic mapping, and frame-wise scene query for motion control using scene hints and movement direction.

Result: Extensive experiments on ShapeNet furniture, PROX, and Replica datasets demonstrate cutting-edge performance with effective generalization ability in cluttered scenes.

Conclusion: SSOMotion successfully integrates semantic understanding into human motion synthesis, achieving superior performance while reducing computational redundancy through efficient scene representation.
...
TL;DR: CloudMamba is an SSM-based point cloud network that addresses Mamba's limitations in point cloud analysis through sequence expanding/merging, chainedMamba for bidirectional scanning, and grouped S6 to reduce overfitting, achieving SOTA results with lower complexity.
...
Method: Proposes sequence expanding (serializing points along each axis separately) and sequence merging (fusing higher-order features), chainedMamba for bidirectional scanning to capture geometric information, and grouped S6 with parameter sharing to reduce overfitting.

Result: Experiments on various point cloud tasks validate that CloudMamba achieves state-of-the-art results with significantly less complexity.

Conclusion: CloudMamba successfully addresses Mamba's limitations in point cloud analysis and demonstrates superior performance with reduced computational complexity.
...
TL;DR: MonoCLUE enhances monocular 3D object detection by combining local clustering of visual features with generalized scene memory to address occlusion and limited visibility issues.
...
Method: 1) K-means clustering on visual features to capture distinct object-level appearance parts; 2) Construct generalized scene memory by aggregating clustered features across images; 3) Integrate both local cluster features and generalized scene memory into object queries to guide attention.

Result: Achieves state-of-the-art performance on the KITTI benchmark, enabling robust detection under occlusion and limited visibility.

Conclusion: The unified local clustering and generalized scene memory strategy enables robust monocular 3D detection by improving object-level feature consistency and stable detection across varying environments.
...
TL;DR: A universal visual perception framework using flow matching that generates diverse visual representations across multiple tasks, bridging the gap between heterogeneous vision tasks and achieving competitive performance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of "single-task-single-model" paradigm in diffusion models and enable cross-domain generalization similar to large language models for multi-task visual scenarios.
...
TL;DR: Proposes a hierarchical auto-regressive sketch generation method that allows flexible stroke-level manipulation during generation, unlike previous approaches that require all conditions to be set before generation starts.
...
Method: Uses a three-stage hierarchical auto-regressive process: 1) predict stroke embedding, 2) anchor stroke on canvas, 3) translate embedding to drawing actions. Each stroke is generated considering previously generated strokes and their positions.

Result: The method enables flexible manipulation of stroke-level sketch drawing at any time during generation by adjusting editable stroke embeddings, providing more control over the sketch generation process.

Conclusion: The hierarchical auto-regressive approach successfully achieves flexible sketch manipulation during generation, overcoming limitations of previous methods that required all conditions to be pre-defined.
...
TL;DR: This paper provides a theoretical analysis of text polarity detection in images, explaining why maximum between-class variance increases for dark text on bright background and decreases for bright text on dark background during binarization.


<details>
  <summary>Details</summary>
Motivation: Text polarity detection is crucial for preprocessing tasks in computer vision applications like license plate recognition, captcha recognition, and character recognition. Existing empirical approaches lack theoretical foundation.

Method: The authors conduct a theoretical analysis of the phenomenon observed in power-law transformation approaches, where maximum between-class variance behaves differently based on text polarity.

Result: The paper theoretically explains the empirical observation that maximum between-class variance increases for dark text on bright background and decreases for bright text on dark background during image binarization.

Conclusion: The theoretical analysis provides mathematical foundation for text polarity detection methods, supporting the empirical findings from power-law transformation approaches used in image binarization.
...
TL;DR: Introduces AquaOV255, the first large-scale underwater segmentation dataset with 255 categories, and Earth2Ocean, a training-free framework that transfers terrestrial vision-language models to underwater segmentation using geometric priors and semantic alignment.
...
Result: Extensive experiments on the UOVSBench benchmark show Earth2Ocean achieves significant performance improvement while maintaining efficient inference.
...
TL;DR: HD²-SSC framework addresses dimension and density gaps in camera-based 3D semantic scene completion for autonomous driving by expanding 2D features to pseudo-3D and refining voxel occupancies.
...
Method: Two modules: High-dimension Semantic Decoupling expands 2D features along pseudo third dimension and identifies focal regions; High-density Occupancy Refinement uses detect-and-refine architecture to complete missing voxels and correct erroneous ones.
...
Conclusion: HD²-SSC successfully bridges dimension and density gaps in 3D semantic scene completion, improving scene perception for autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: Traditional 2D images cannot distinguish terrain depth features like craters and hills, which significantly impact path safety.

Method: Uses disparity maps generated by UAVs, applies computer vision techniques (edge/line/corner detection, stereo depth reconstruction) to define candidate way-points, and automatically detects initial/desired points using ArUco marker pose estimation and circle detection.

Result: Algorithm tested in V-REP simulation and physical laboratory setup, showing promising results and effectiveness compared to A* and PRM.

Conclusion: The proposed image-based path planning algorithm using computer vision techniques is effective and outperforms traditional deterministic and probabilistic methods in handling terrain depth for safer path planning.
...
Result: Achieves 8% higher performance than second best baseline on ISIC2019 and 120x faster than FedAVG with reasonable resource costs.
...
TL;DR: Proposes Laytrol Network for layout-to-image generation that preserves pretrained model knowledge by using base model-synthesized data and careful parameter initialization.


<details>
  <summary>Details</summary>
Motivation: Existing layout-to-image methods suffer from low visual quality and stylistic inconsistency due to loss of pretrained knowledge when integrating layout conditions.

Method: Uses Layout Synthesis dataset from base model images and Laytrol Network with inherited MM-DiT parameters, zero-initialized outputs, and Object-level Rotary Position Embedding.

Result: Qualitative and quantitative experiments show improved performance in layout-to-image generation with better visual quality and style consistency.

Conclusion: The proposed approach effectively addresses distribution shift and preserves pretrained knowledge for high-quality layout-to-image generation.
...
TL;DR: DiffRegCD is a unified framework that integrates dense registration and change detection in a single model, achieving robust performance under large displacements and viewpoint variations using diffusion features and classification-based correspondence estimation.


<details>
  <summary>Details</summary>
Motivation: Real-world change detection faces challenges with misalignment due to parallax, viewpoint shifts, and long temporal gaps. Existing methods struggle with large displacements and rely on limited registration approaches.

Method: Reformulates correspondence estimation as Gaussian smoothed classification for sub-pixel accuracy, leverages frozen multi-scale features from pretrained denoising diffusion models, and uses controlled affine perturbations for supervision without pseudo labels.
...
Conclusion: Diffusion features and classification-based correspondence provide a strong foundation for unified change detection, establishing DiffRegCD as an effective solution for real-world misalignment challenges.
...
<details>
  <summary>Details</summary>
Motivation: Current TFG methods require minutes of reference video which takes hours to process, limiting practical applications. Research shows video quality matters more than length.

Method: Proposed ISExplore automatically selects 5-second segments based on audio feature diversity, lip movement amplitude, and camera view count.
...
Conclusion: Short but informative video segments can replace long reference videos, significantly improving efficiency without sacrificing quality in talking face generation.
...
TL;DR: Proposes Multimodal Prototype-based Multi-Instance Learning for computational pathology, using bidirectional interaction between vision and text prototypes with Stereoscopic Optimal Transport for better generalization and interpretability.


<details>
  <summary>Details</summary>
Motivation: Address computational cost of giga-pixel Whole Slide Images in pathology and bias in LLM-generated instance-level descriptions due to lack of medical knowledge, while overcoming limitations of unidirectional guidance in existing vision-language MIL methods.

Method: Uses frozen LLM to generate task-specific pathological entity descriptions as text prototypes, learns instance-level vision prototypes, and employs Stereoscopic Optimal Transport algorithm for bidirectional multimodal fusion based on similarity metrics.

Result: Superior generalization capabilities demonstrated through few-shot classification and explainability experiments on three distinct cancer datasets.

Conclusion: The proposed method effectively addresses computational challenges in pathology analysis, enhances model interpretability, and achieves better performance through bidirectional multimodal interaction and prototype learning.
...
TL;DR: ReIDMamba is a pure Mamba-based person re-identification framework that overcomes Transformer scalability issues by using state space models for efficient feature extraction with multi-granularity learning and ranking-aware regularization.


<details>
  <summary>Details</summary>
Motivation: To address the scalability limitations of Transformer-based ReID methods (quadratic computational complexity) while maintaining robust discriminative feature extraction capabilities.

Method: Uses Mamba-based architecture with multiple class tokens, multi-granularity feature extractor (MGFE) with multi-branch design and class token fusion, and ranking-aware triplet regularization (RATR) for feature diversity.

Result: Achieves state-of-the-art performance on 5 person ReID benchmarks with only one-third parameters of TransReID, lower GPU memory usage, and faster inference throughput.

Conclusion: ReIDMamba demonstrates that pure Mamba-based approaches can effectively replace Transformers in ReID tasks, offering superior efficiency and performance while being the first Mamba-driven ReID framework.
...
TL;DR: Proposes Burst Image Quality Assessment (BuIQA) to evaluate frame quality in burst sequences, introduces a benchmark dataset, and develops a unified framework with task-driven prompts for efficient adaptation across multiple downstream scenarios.


<details>
  <summary>Details</summary>
Motivation: Address redundancy in burst images that increases storage/transmission demands and reduces downstream task efficiency by developing quality assessment for burst image selection.

Method: Establishes benchmark dataset with 7,346 burst sequences and 191,572 quality scores. Proposes unified BuIQA framework with task-driven prompt generation using heterogeneous knowledge distillation and task-aware quality assessment network.
...
Conclusion: BuIQA effectively evaluates burst image quality for task-driven selection, improving downstream task performance while addressing storage and efficiency issues in burst imaging.
...
TL;DR: MMAssist improves 3D unsupervised domain adaptation for LiDAR object detection by using multi-modal assistance from images and text to align 3D features across domains.


<details>
  <summary>Details</summary>
Motivation: Current 3D UDA methods ignore the usefulness of image data collected alongside point clouds, despite multi-modal sensors being commonly deployed together.

Method: Aligns 3D features between source and target domains using image and text features as bridges, extracts features from pre-trained vision backbones and text encoders, fuses aligned features with learned weights, and enhances pseudo labels with 2D detection.

Result: Achieves promising performance compared to state-of-the-art methods in three domain adaptation tasks on three popular 3D object detection datasets.
...
TL;DR: A modular pipeline that improves spatial and temporal robustness in remote sensing change detection without modifying existing networks, using diffusion-based semantic morphing and flow refinement.


<details>
  <summary>Details</summary>
Motivation: Address challenges of spatial misalignment in bi-temporal remote sensing images, especially with long seasonal/multi-year gaps, where existing models rely on precise co-registration and joint frameworks require retraining.

Method: Integrates diffusion-based semantic morphing, dense registration, and residual flow refinement. Uses diffusion to synthesize intermediate morphing frames, estimates stepwise correspondences, and refines flow with lightweight U-Net.
...
Conclusion: The approach demonstrates generality and effectiveness in improving robustness for remote sensing change detection without altering existing networks.
...
Method: Uses ray-based sampling from multiple viewpoints to generate candidate points, then refines positions and predicts opacity scores via transformer decoder. Includes lightweight classification head on geometric features for semantic guidance without external image supervision.

Result: Outperforms state-of-the-art methods on PCN and MVP benchmarks in accuracy and structural consistency, while being robust to varying input densities and noise levels.

Conclusion: DANCE provides an effective framework for category-consistent point cloud completion that handles real-world challenges like variable sparsity without requiring external image supervision.
...
TL;DR: Specialized vision-language models for fracture detection and description in chest X-rays outperform general-purpose models, with analysis of performance by fracture characteristics.
...
Result: Significant improvements over general-purpose models in generating accurate fracture descriptions, with analysis revealing strengths/limitations by fracture type, location, and age.
...
TL;DR: A semantic-guided framework for large-mask image inpainting that uses structure-aware candidates from a pretrained Amodal Completion model and fuses them with contextual features via a transformer-based network to improve inpainting quality.


<details>
  <summary>Details</summary>
Motivation: To address the challenging problem of large-mask image inpainting where essential visual content is missing and contextual cues are limited, requiring better semantic guidance for accurate completion.

Method: Leverage a pretrained Amodal Completion model to generate structure-aware candidates as semantic priors, then use Context-Semantic Fusion Network (CSF-Net) - a transformer-based fusion framework that fuses these candidates with contextual features to produce semantic guidance for inpainting.
...
Conclusion: CSF-Net provides an effective semantic guidance framework for large-mask image inpainting that improves structural accuracy and semantic consistency across diverse masking conditions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional weed management methods using chemical herbicides cause environmental contamination and herbicide resistance. Precision weeding with computer vision offers eco-friendly alternatives but faces computational limitations.

Method: Applied structured pruning, integer quantization, and input image resolution scaling to YOLOv8n object detector. Deployed on STM32U575ZI microcontroller and trained on CropAndWeed dataset with 74 plant species.

Result: Achieved real-time in-situ weed detection with minimal energy consumption of 51.8mJ per inference, balancing detection accuracy and efficiency for power-constrained agricultural environments.

Conclusion: The system enables scalable deployment of precision weeding in power-constrained agricultural settings, providing an eco-friendly alternative to chemical herbicides.
...
Method: SharpV uses adaptive pruning based on spatial-temporal information to dynamically adjust pruning ratios, and employs self-calibration for KV cache pruning by measuring similarity to original visual features.

Result: Experiments on multiple benchmarks show SharpV's superiority, occasionally achieving performance gains over dense models while being the first two-stage pruning framework compatible with hardware acceleration like Flash Attention.

Conclusion: SharpV offers a novel paradigm for adaptive pruning in VideoLLMs through hierarchical cache pruning from an information bottleneck perspective, providing efficiency without compromising performance.
...
Method: Leverages episodic memory consolidation with appearance-aware meta-learning memory (AMM) for segmentation and geometry-aware localization memory (GLM) for tracking. Integrates VQL-2D output with visual geometry grounded Transformer (VGGT) for 2D-3D unification.

Result: Achieves state-of-the-art performance on Ego4D-VQ benchmark with precise contour delineation, robust spatial discrimination, and significantly improved retrieval accuracy.

Conclusion: EAGLE's memory consolidation mechanism through structured appearance and geometry memory banks effectively supports both long- and short-term modeling of target appearance variations, enabling efficient unification of 2D and 3D tasks.
...
TL;DR: AdvRoad generates natural-looking adversarial road posters that fool 3D object detectors into seeing non-existent objects, addressing limitations of previous unnatural-looking attacks.


<details>
  <summary>Details</summary>
Motivation: Current visual 3D detection systems are vulnerable to adversarial attacks, and previous poster-based attacks have unnatural appearances that are easily noticeable by humans and can be easily defended against.

Method: Two-stage approach: Road-Style Adversary Generation creates natural road-like appearances, and Scenario-Associated Adaptation optimizes attack effectiveness while maintaining stealth.

Result: AdvRoad successfully generalizes across different detectors, scenes, and attack locations, with physical experiments confirming real-world threat viability.

Conclusion: AdvRoad demonstrates significant security vulnerabilities in autonomous driving systems through stealthy, natural-looking adversarial road attacks that can induce dangerous false object detections.
...
TL;DR: Cascade HQP-DETR addresses limitations in Imaginary Supervised Object Detection by introducing high-quality synthetic datasets, proposal-guided query encoding, and cascade denoising to improve real-world generalization.


<details>
  <summary>Details</summary>
Motivation: Object detection requires costly annotated datasets. ISOD trains on synthetic images but faces issues with dataset quality, DETR convergence problems, and overfitting to noisy pseudo-labels.

Method: Uses LLaMA-3, Flux, and Grounding DINO to create high-quality datasets (FluxVOC/FluxCOCO). Implements proposal-guided query encoding with SAM-generated proposals and RoI features. Applies cascade denoising with progressive IoU thresholds across decoder layers.

Result: Achieves SOTA 61.04% mAP@0.5 on PASCAL VOC 2007 with only 12 epochs on FluxVOC, outperforming strong baselines and demonstrating competitive real-data performance.

Conclusion: The proposed architecture successfully addresses ISOD limitations and shows universal applicability through improved convergence, reduced overfitting, and better real-world generalization.
...
TL;DR: A multi-modal deepfake detection and localization framework using Feature Pyramid-Transformer (FPN-Transformer) to address cross-modal generalization and temporal boundary regression for audio-visual deepfakes.


<details>
  <summary>Details</summary>
Motivation: Current unimodal detection methods lack cross-modal correlation analysis and precise temporal localization capabilities, limiting their effectiveness against sophisticated deepfake manipulations.
...
Result: Achieved 0.7535 score on IJCAI'25 DDL-AV benchmark test set, demonstrating good performance for cross-modal deepfake detection and localization in challenging environments.

Conclusion: The framework provides an effective and novel approach for generalized deepfake detection with frame-level localization precision, addressing critical gaps in cross-modal analysis.
...
TL;DR: 3DGS-QA is the first subjective quality assessment dataset for 3D Gaussian Splatting, addressing the underexplored perceptual quality issues in 3DGS rendering under various reconstruction conditions.


<details>
  <summary>Details</summary>
Motivation: Prior research focused on algorithmic performance and visual fidelity, but the perceptual quality of 3DGS-rendered content under varying reconstruction conditions (viewpoint sparsity, limited training iterations, point downsampling, noise, color distortions) remains largely underexplored.

Method: Created 3DGS-QA dataset with 225 degraded reconstructions across 15 object types, and developed a no-reference quality prediction model that operates directly on native 3D Gaussian primitives without requiring rendered images or ground-truth references, using spatial and photometric cues in a structure-aware manner.

Result: The proposed method consistently achieves superior performance compared to existing quality assessment methods (both traditional and learning-based), demonstrating robustness and effectiveness for 3DGS content evaluation.

Conclusion: 3DGS-QA fills a critical gap in 3DGS quality assessment research, and the proposed no-reference quality prediction model provides an effective solution for evaluating perceptual quality in 3D Gaussian Splatting content.
...
TL;DR: WEDepth is a novel monocular depth estimation method that adapts Vision Foundation Models without modifying their structures or pretrained weights, achieving state-of-the-art performance on NYU-Depth v2 and KITTI datasets.
...
Method: WEDepth uses Vision Foundation Models as multi-level feature enhancers, systematically injecting prior knowledge at different representation levels without modifying the model structures or pretrained weights.

Result: Achieves new state-of-the-art performance on NYU-Depth v2 and KITTI datasets, with competitive results compared to diffusion-based approaches and methods pre-trained on relative depth. Also demonstrates strong zero-shot transfer capability across diverse scenarios.

Conclusion: The proposed WEDepth method effectively adapts Vision Foundation Models for monocular depth estimation while preserving their structures and weights, establishing new state-of-the-art performance with strong generalization capabilities.
...
TL;DR: GECO2 is an end-to-end few-shot counting and detection method that addresses object scale issues through dense query representation, achieving state-of-the-art performance with improved speed and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot counters struggle with diverse-sized objects and densely populated regions due to ad-hoc solutions like feature merging and image upsampling/tiling.

Method: Proposes dense query representation that gradually aggregates exemplar-specific feature information across scales, creating high-resolution dense queries for detecting both large and small objects.

Result: Surpasses state-of-the-art few-shot counters by 10% in counting and detection accuracy while running 3x faster with smaller GPU memory footprint.

Conclusion: GECO2 effectively addresses scale issues in few-shot counting and detection through its novel dense query representation, achieving superior performance and efficiency.
...
TL;DR: A subject-driven image generation method using LoRA fine-tuned diffusion with latent concatenation and masked CFM objective, supported by a two-stage distilled data curation framework and CHARIS evaluation system.
...
Conclusion: The proposed approach achieves effective subject-driven image generation with improved identity consistency and diversity, validated by the CHARIS evaluation framework.
...
TL;DR: I2E converts static images into high-fidelity event streams for SNN training, achieving 300x faster conversion speed than prior methods and enabling state-of-the-art accuracy of 60.50% on ImageNet and 92.5% on CIFAR10-DVS.


<details>
  <summary>Details</summary>
Motivation: Spiking neural networks (SNNs) offer energy-efficient computing but face a critical scarcity of event-stream data, hindering their adoption.

Method: I2E simulates microsaccadic eye movements using highly parallelized convolution to convert static images into event streams, enabling on-the-fly data augmentation for SNN training.

Result: Achieved 60.50% accuracy on I2E-ImageNet and 92.5% accuracy on CIFAR10-DVS through sim-to-real pre-training and fine-tuning, demonstrating synthetic event data can serve as high-fidelity proxy for real sensor data.
...
TL;DR: First unsupervised framework for radar-based heartbeat sensing using augmented pseudo-labels and noise contrast, achieving performance comparable to supervised methods without requiring ground-truth physiological signals.


<details>
  <summary>Details</summary>
Motivation: Traditional radar heartbeat sensing suffers from noise degradation, while learning-based methods require expensive labeled data. Need for unsupervised approach to overcome these limitations.

Method: Proposed Radar-APLANC framework using heartbeat and noise ranges to create positive/negative samples, Noise-Contrastive Triplet loss with pseudo-labels from traditional radar methods, and adaptive noise-aware label selection for pseudo-label augmentation.
...
Conclusion: Successfully demonstrated unsupervised radar heartbeat sensing framework that avoids dependency on expensive ground-truth signals while maintaining competitive performance.
...
Method: Probing Stable Diffusion with regression layers to predict semantic attributes from internal representations, comparing predictions against human annotations, and analyzing different components (CLIP vs diffusion process).

Result: Semantic understanding primarily comes from CLIP text encoding rather than the reverse diffusion process; different semantic attributes have varying decoding accuracy; attributes become harder to disambiguate during diffusion.

Conclusion: CLIP vision-language model determines human-like semantic representation, while the diffusion process serves as a visual decoder without strong semantic understanding.
...
TL;DR: A hierarchical evaluation framework that decomposes identity assessment into feature-level transformations using structured VLM reasoning to measure identity preservation in generative models.
...
Method: Hierarchical decomposition of subjects into (type, style) -> attribute -> feature decision tree, prompting VLMs for concrete transformations rather than abstract similarity scores.

Result: Strong alignment with human judgments in measuring identity consistency across four state-of-the-art generative models, with reduced hallucinations and improved consistency.

Conclusion: The framework provides a more reliable and diagnostic approach to evaluate identity preservation, validated through a new benchmark with 1,078 image-prompt pairs spanning diverse subject types.
...
<details>
  <summary>Details</summary>
Motivation: Existing morph generation methods produce blurry, artifact-ridden images that are easy to detect and don't represent real-world attack challenges.

Method: Uses modern diffusion-based image synthesis to generate full-head morphed face images with sharp details and control over visual attributes.

Result: StableMorph images rival or exceed genuine face image quality and effectively fool face recognition systems, posing greater challenges to existing MAD solutions.

Conclusion: StableMorph sets a new standard for morph quality, improves biometric security evaluation, and supports development of more robust detection systems.
...
TL;DR: A new dataset for face recognition security focusing on Nylon Face Masks (NFMs) - a realistic 3D presentation attack that mimics facial geometry, collected using iPhone 11 Pro with 55K+ samples.


<details>
  <summary>Details</summary>
Motivation: Face recognition systems are vulnerable to presentation attacks, and NFMs represent a novel and realistic threat due to their elastic structure and photorealistic appearance that can closely mimic victims' facial geometry.

Method: Created a dataset using iPhone 11 Pro capturing 3,760 bona fide samples from 100 subjects and 51,281 NFM attack samples across four presentation scenarios involving humans and mannequins, then benchmarked with five state-of-the-art PAD methods.

Result: Significant performance variability across PAD methods was observed, highlighting the challenges posed by NFMs and the difficulty of detecting these sophisticated attacks.

Conclusion: NFMs present a serious threat to face recognition security, underscoring the need for PAD techniques that can generalize effectively to emerging spoofing threats like these realistic 3D masks.
...
TL;DR: LatentPrintFormer integrates CNN and Transformer backbones with spatial attention to improve latent fingerprint identification by combining local and global features while suppressing noise.
...
Method: Combines EfficientNet-B0 (CNN) and Swin Tiny (Transformer) backbones with spatial attention module to extract local and global features, then fuses them into 512D embeddings for cosine similarity matching.
...
Conclusion: The hybrid CNN-Transformer architecture with spatial attention effectively addresses latent fingerprint identification challenges and demonstrates superior performance over existing methods.
...
TL;DR: Proposes a federated learning framework with SAM2 for privacy-preserving foam segmentation in wastewater treatment plants, enabling collaborative training without data sharing.


<details>
  <summary>Details</summary>
Motivation: Foam formation in WTPs reduces efficiency and increases costs, but standard ML approaches require large labeled datasets and face privacy concerns that prevent data sharing between plants.

Method: Combines Federated Learning with Segment Anything Model 2 (SAM2), using Flower framework for distributed training across edge nodes with a central Fog server aggregating weights without accessing private data.
...
Conclusion: Demonstrates significant potential of integrating large-scale foundational models into FL systems for solving real-world industrial challenges with distributed and sensitive data.
...
<details>
  <summary>Details</summary>
Motivation: Existing STR frameworks suffer from decoupled visual-linguistic optimization that amplifies error propagation through cross-modal misalignment, particularly for irregular text patterns with background distractors and geometric deformations.

Method: Three-stage architecture: (1) Dual Attention Macaron Encoder for refined visual features, (2) Position-Aware Module and Semantic Quantizer for spatial-semantic integration, (3) Multi-Modal Collaborative Verifier for cross-modal self-correction.

Result: Achieves 83.5% average accuracy on Union14M-L benchmark and 79.1% on heavily occluded OST dataset, establishing new records in 9 out of 14 evaluation scenarios.

Conclusion: OTSNet's neurocognitive-inspired unified modeling effectively addresses cross-modal misalignment in STR, demonstrating superior performance on challenging real-world text recognition tasks.
...
TL;DR: PEOD is the first large-scale, high-resolution (1280x720) Event-RGB dataset for object detection under challenging conditions, addressing limitations of existing datasets with sparse coverage and low resolution.
...
Method: Created PEOD dataset with 130+ spatiotemporal-aligned sequences and 340k manual bounding boxes, with 57% data captured under low-light, overexposure, and high-speed motion. Benchmarked 14 methods across Event-based, RGB-based, and Event-RGB fusion configurations.

Result: Fusion-based models achieve excellent performance on full test set and normal subset. However, in illumination challenge subset, top event-based model outperforms all fusion models, while fusion models still outperform RGB-based counterparts.
...
TL;DR: Proposes Boomda, a balanced multi-objective optimization method for multimodal domain adaptation that addresses varying domain shifts across modalities through information bottleneck and correlation alignment.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of expensive manual annotation in multimodal learning and the scarcity of annotated data, particularly for unsupervised domain adaptation in multimodal settings where different modalities exhibit varying domain shifts.

Method: Uses information bottleneck to learn modality-specific representations, matches source and target domains with correlation alignment, formulates as multi-objective optimization for Pareto optimal solution, and solves via quadratic programming with closed-form solution.

Result: Extensive empirical results show Boomda outperforms competing schemes and demonstrates effectiveness in multimodal domain adaptation.

Conclusion: Boomda provides an efficient modality-balanced multimodal domain adaptation algorithm that successfully addresses heterogeneous domain shifts across different modalities.
...
TL;DR: Introduces NAR-IQA framework for Novel View Synthesis quality assessment using non-aligned references, trained on synthetic distortions with contrastive learning and DINOv2 embeddings, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing FR-IQA methods fail with misaligned references in NVS, while NR-IQA methods lack generalization. Need for quality assessment that works with partially shared but non-aligned reference views.

Method: Built large-scale dataset with synthetic distortions targeting Temporal Regions of Interest. Uses contrastive learning framework with LoRA-enhanced DINOv2 embeddings, supervised by existing IQA methods. Trained exclusively on synthetic distortions to avoid overfitting.

Result: Outperforms state-of-the-art FR-IQA, NR-IQA, and NAR-IQA methods. Achieves robust performance on both aligned and non-aligned references. Strong correlation with human preferences from user study.

Conclusion: NAR-IQA framework effectively addresses NVS quality assessment with non-aligned references, demonstrating superior generalization and correlation with human perception.
...
TL;DR: LandSegmenter is a Land Use and Land Cover foundation model framework that addresses data scarcity through weak supervision, cross-modal feature extraction, and zero-shot transfer learning.


<details>
  <summary>Details</summary>
Motivation: Current LULC models are limited by modality specificity and fixed taxonomies, while foundation models require extensive labeled data that is costly in remote sensing.

Method: Uses LAS dataset with weak labels, RS-specific adapter for cross-modal features, text encoder for semantics, and confidence-guided fusion for zero-shot performance.

Result: Achieves competitive/superior performance on 6 LULC datasets, especially in zero-shot settings when transferred to unseen data.

Conclusion: Demonstrates efficacy of weak supervision for building task-specific foundation models and the utility of the proposed framework for universal LULC mapping.
...
TL;DR: Proposes Mg-MRN, a multi-granularity mutual refinement network for zero-shot learning that refines visual features through decoupled multi-granularity feature learning and cross-granularity interactions.


<details>
  <summary>Details</summary>
Motivation: Current ZSL methods overlook intrinsic interactions between local region features, which could improve transferable and explicit visual feature acquisition for better zero-shot recognition.

Method: Uses multi-granularity feature extraction to learn decoupled region-level discriminative features, then cross-granularity feature fusion to strengthen interactions between region features of varying granularities.
...
Conclusion: The Mg-MRN network effectively improves ZSL recognition performance by refining discriminative and transferable visual features through multi-granularity feature interactions.
...
TL;DR: Proposes a novel shadow generation framework using Keypoints Linear Model (KPLM) and Shadow Triangle Algorithm (STA) to address limitations in existing diffusion-based methods for realistic shadow generation in image composition.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based methods like IC-Light still struggle to produce shadows with both high appearance realism and geometric precision in composite images, especially for complex human poses.

Method: Uses KPLM to model articulated human bodies with 9 keypoints and 1 bounding block for physically plausible shadow projection and dynamic shading. STA computes shadow angles, lengths, and positions through explicit geometric formulations.

Result: Achieves state-of-the-art performance on shadow realism benchmarks, particularly for complex human poses, and generalizes effectively to multi-directional relighting scenarios.
...
Conclusion: DistZSL framework effectively handles data heterogeneity in distributed settings and enhances zero-shot learning performance across different nodes.
...
TL;DR: VLMDiff integrates Latent Diffusion Models with Vision-Language Models for unsupervised multi-class visual anomaly detection, using VLM-generated captions as conditioning to learn robust normal representations without manual annotations.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based anomaly detection methods rely on synthetic noise generation, require per-class model training, and have limited generalization, making them unscalable for diverse multi-class real-world images.

Method: Uses pre-trained Vision-Language Model with simple prompts to extract detailed image descriptions, which serve as conditioning for Latent Diffusion Model training to learn normal image feature representations for multi-class anomaly detection.

Result: Achieves competitive performance with improvements of up to 25 points in pixel-level Per-Region-Overlap (PRO) on Real-IAD dataset and 8 points on COCO-AD dataset, outperforming state-of-the-art diffusion-based approaches.

Conclusion: The integration of VLM and LDM enables effective unsupervised multi-class visual anomaly detection without manual annotations or per-class training, demonstrating superior performance and scalability.
...
TL;DR: WarpGAN introduces a warping-and-inpainting strategy to improve occluded region generation in 3D GAN inversion for single-shot novel view synthesis.
...
Method: Uses 3D GAN inversion encoder to get latent code, warps to novel view using depth map, and employs SVINet with symmetry prior and multi-view correspondence for inpainting occluded regions.

Result: Quantitative and qualitative experiments show WarpGAN consistently outperforms state-of-the-art methods.

Conclusion: The warping-and-inpainting strategy effectively improves occluded region quality in 3D GAN inversion for novel view synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Existing box-level IoU prediction suffers from structural coupling where predicted IoU can be overestimated for poorly localized boxes due to inherent similarity between predicted and estimated ground-truth boxes.
...
Conclusion: PQA provides more accurate localization quality assessment by eliminating structural coupling bias and can be seamlessly integrated into various oriented object detectors.
...
TL;DR: UI2Code^N is a visual language model that introduces an interactive UI-to-code paradigm with multimodal coding capabilities including UI generation, editing, and polishing, achieving state-of-the-art performance comparable to leading closed-source models.


<details>
  <summary>Details</summary>
Motivation: Current UI programming is complex, and existing visual language models have underdeveloped multimodal coding capabilities and limited use of iterative visual feedback, making them insufficient for real-world UI development workflows.

Method: Interactive UI-to-code paradigm with UI2Code^N model trained through staged pretraining, fine-tuning, and reinforcement learning. The model unifies UI-to-code generation, UI editing, and UI polishing capabilities, with test-time scaling for multi-turn feedback.
...
Conclusion: The interactive UI-to-code paradigm with UI2Code^N demonstrates significant improvements in multimodal coding capabilities and enables systematic use of multi-turn feedback, better reflecting real-world development workflows.
...
<details>
  <summary>Details</summary>
Motivation: Computer-aided diagnosis faces challenges in real-world settings due to limited data availability, ethical/legal restrictions, high annotation costs, and emerging/rare diseases, necessitating robust open-set recognition methods.

Method: Introduces a loss function that leverages features learned in deep neural networks (which cluster around class means arranged as simplex vertices) and uses auxiliary datasets to effectively reject unknown class samples by penalizing open space regions.
...
Conclusion: The proposed approach effectively addresses open-set recognition challenges in medical imaging, demonstrating superior performance in identifying unknown classes across multiple medical datasets.
...
TL;DR: Large image-to-3D models have strong canonical view bias - they struggle with rotated inputs, but this can be fixed with a lightweight CNN orientation detector without changing the main model.


<details>
  <summary>Details</summary>
Motivation: To understand the inductive biases and limitations of large-scale image-to-3D generative models, particularly their failure to generalize across different viewpoints.

Method: Conducted controlled experiments using simple 2D rotations on the Hunyuan3D 2.0 model, and proposed a lightweight CNN that detects and corrects input orientation to mitigate the canonical view bias.

Result: The state-of-the-art model shows degraded performance under rotated inputs, but adding an orientation correction CNN restores model performance without modifying the generative backbone.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for robust automated nutritional analysis in digital food applications and evaluate trade-offs between visual classification accuracy, model efficiency, and generative output quality.

Method: Comparative evaluation of a decoupled multimodal pipeline using EfficientNet-B4 visual backbone with Gemini LLM, benchmarked against alternative vision models (VGG-16, ResNet-50, YOLOv8) and lightweight LLM (Gemma), with analysis of Semantic Error Propagation using a Custom Chinese Food Dataset.

Result: EfficientNet-B4 achieved 89.0% Top-1 accuracy with best balance of accuracy/efficiency, Gemini scored 9.2/10 factual accuracy for superior generative quality, but overall system utility is bottlenecked by visual front-end accuracy, with high semantic similarity identified as critical failure mode.

Conclusion: The system's effectiveness is fundamentally limited by the visual classification module's perceptive accuracy, with semantic error propagation from visual mistakes cascading into generative outputs, highlighting the need for improved visual recognition in multimodal food analysis systems.
...
TL;DR: 2Dto3D-SR is a real-time single-view 3D super-resolution framework that converts 3D data into 2D representations using PNCC, enabling direct use of 2D image SR methods without needing high-resolution RGB guidance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing 3D super-resolution methods that require complex 3D point processing or high-resolution RGB guidance, which are often impractical in real-world scenarios where such data is unavailable.

Method: Encodes 3D geometry from a single viewpoint into 2D using Projected Normalized Coordinate Code (PNCC), allowing application of standard 2D image super-resolution architectures. Implemented with both Swin Transformers for accuracy and Vision Mamba for efficiency.

Result: Swin Transformer model achieves state-of-the-art accuracy on benchmarks, while Vision Mamba model delivers competitive results at real-time speeds, demonstrating effectiveness across different deployment needs.

Conclusion: The geometry-guided pipeline provides a surprisingly simple yet practical solution for real-world 3D super-resolution, especially in scenarios where high-resolution RGB data is inaccessible.
...
TL;DR: Proposes adaptive local region placement for point cloud surface reconstruction, adjusting spacing and size based on curvature to improve accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods use uniform local regions with fixed sizes, limiting adaptability to varying geometric complexity in point clouds.
...
<details>
  <summary>Details</summary>
Motivation: Existing vision-language fine-tuning methods overlook semantic relationships within images, leading to suboptimal performance in multimodal alignment.
...
Conclusion: The proposed approach effectively improves multimodal alignment and fusion by leveraging both semantics and relationships in vision-language models.
...
TL;DR: DiPVNet is a novel point cloud processing method that uses directional perception and rotation invariance through dot-product operators at local and global levels to handle arbitrary rotations in 3D vision tasks.


<details>
  <summary>Details</summary>
Motivation: Point clouds suffer from orientation variations due to arbitrary rotations, which disrupt intrinsic directional characteristics. Existing methods fail to fully exploit multiscale directional nature for enhanced feature representation.

Method: Proposes Direction-Perceptive Vector Network (DiPVNet) with atomic dot-product operators: Learnable Local Dot-Product (L2DP) for adaptive local structure capture, and global directional response spectrum via spherical Fourier transform for holistic directional modeling.

Result: Achieves state-of-the-art performance on point cloud classification and segmentation tasks under challenging scenarios with noise and large-angle rotations.

Conclusion: DiPVNet effectively addresses rotational challenges in point cloud processing by combining directional perception with rotation invariance through multiscale dot-product operators, demonstrating superior performance in real-world applications.
...
TL;DR: NERVE is a training-free open-vocabulary semantic segmentation method that integrates global and local information from stable diffusion models using entropy-guided attention selection and stochastic random walks for affinity refinement.


<details>
  <summary>Details</summary>
Motivation: Existing training-free OVSS methods have limitations including expensive affinity refinement, ineffective attention map fusion with equal weighting, and reliance on fixed-size Gaussian kernels that enforce isotropic neighborhoods.

Method: Uses neighborhood structure from self-attention layers of stable diffusion models, introduces stochastic random walk for affinity refinement instead of Gaussian kernels, and employs entropy-based uncertainty to select relevant attention maps without requiring CRF or PAMR post-processing.

Result: Achieves state-of-the-art zero-shot segmentation performance on 7 popular semantic segmentation benchmarks.

Conclusion: NERVE provides an effective training-free approach for open-vocabulary semantic segmentation that effectively delineates objects with arbitrary shapes through spatial diffusion and entropy-guided attention selection.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-object image editing methods follow localize-editing paradigm but neglect critical inter-object interactions, leading to attention entanglements in conflict regions that cause editing leakage or constraints.

Method: Proposes a "decompose-editing-fusion" framework with three modules: (1) Conflict-aware Layer Decomposition using attention-aware IoU and time-dependent region removal, (2) Object-layered Editing with coordinated intra-layer text guidance and cross-layer geometric mapping, (3) Transparency-guided Layer Fusion with structure-coherent fusion through transparency guidance learning.
...
Method: Uses diffusion-based approach conditioned on joint representation of VAE-encoded spatial features (from aerial RGB and height maps) and CLIP-based semantic embeddings, ensuring geometric constraints and semantic consistency.

Result: Shows 7.3% average improvement in SSIM across CVUSA, CVACT, and Auto Arborist datasets, demonstrating robust handling of both wide and narrow fields of view with strong generalization.

Conclusion: Top2Ground effectively generates realistic ground-level images directly from aerial views, outperforming existing methods while maintaining geometric and semantic consistency without intermediate representations.
...
<details>
  <summary>Details</summary>
Motivation: Existing data condensation techniques fail in multimodal scenarios where preserving intricate inter-modal dependencies is crucial, limiting their effectiveness for multimodal model training.

Method: Uses Characteristic Function loss in Fourier domain for precise statistical alignment via infinite moment matching, enforcing three levels: uni-modal, cross-modal, and joint-modal distributional consistency.

Result: On NYU-v2 dataset, achieves lossless performance comparable to full dataset training with just 5 condensed datapoints per class - 8.2% absolute improvement over previous best method with 4x less condensation time.

Conclusion: ImageBindDC successfully addresses multimodal data condensation by preserving inter-modal dependencies through unified feature space and CF loss, enabling efficient multimodal model training.
...
<details>
  <summary>Details</summary>
Motivation: Existing semantic segmentation methods fail under extreme conditions (low light, camera motion) due to RGB information loss, while event-RGB fusion suffers from feature-level mismatch.

Method: Edge-awareness Semantic Concordance framework with Edge-awareness Latent Re-coding to align event-RGB features using edge dictionary, and Re-coded Consolidation with Uncertainty Optimization for fusion.

Result: Outperforms state-of-the-art by 2.55% mIoU on DERS-XS dataset and shows superior resilience under spatial occlusion.

Conclusion: The proposed ESC framework effectively addresses heterogeneous event-RGB fusion issues and provides robust semantic segmentation in extreme conditions.
...
TL;DR: SWAN is a web app that uses swipe gestures for faster histopathology image annotation, achieving comparable accuracy to traditional folder-based methods while being more user-friendly.
...
Result: In a pilot study with four pathologists annotating 600 mitotic figure patches, SWAN achieved pairwise percent agreement of 86.52%-93.68% (Cohen's Kappa = 0.61-0.80), comparable to folder-based method's 86.98%-91.32% (Cohen's Kappa = 0.63-0.75), while being rated as highly usable with mobile annotation capability.

Conclusion: SWAN can accelerate image annotation while maintaining quality, offering a scalable and user-friendly alternative to conventional workflows for histopathology image analysis.
...
TL;DR: Proposes MAUGIF, a mechanism-aware unsupervised general image fusion method using dual cross-image autoencoders that adapts to different fusion mechanisms (additive/multiplicative) for better performance and interpretability.
...
Result: Extensive experiments validate the effectiveness and generalization ability across diverse fusion tasks.
...
Method: Created SynWeather dataset covering four regions (Continental US, Europe, East Asia, Tropical Cyclones) with high-resolution observations of key weather variables. Developed SynWeatherDiff, a probabilistic weather synthesis model using Diffusion Transformer framework.
...
TL;DR: SkelSplat is a novel multi-view 3D human pose estimation framework using differentiable Gaussian rendering that models human pose as 3D Gaussians per joint, enabling view fusion without 3D ground-truth supervision.
...
Method: Models human pose as a skeleton of 3D Gaussians (one per joint) optimized via differentiable rendering, with a novel one-hot encoding scheme that enables independent joint optimization despite Gaussian Splatting being originally designed for dense scene reconstruction.
...
TL;DR: NeuSpring is a neural spring field method that creates physical digital twins of deformable objects from videos, achieving better reconstruction and simulation performance for both current state modeling and future prediction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for deformable object modeling focus mainly on current state physical learning but generalize poorly to future prediction because they ignore intrinsic physical properties of deformable objects.

Method: NeuSpring uses neural spring fields built on spring-mass models with two innovations: 1) piecewise topology solution for multi-region spring connections using zero-order optimization, and 2) neural spring field representing spring properties across frames using canonical coordinate-based neural networks.

Result: Experiments show NeuSpring achieves 20% improvement in Chamfer distance for current state modeling and 25% improvement for future prediction compared to existing methods.

Conclusion: NeuSpring effectively models deformable objects by considering material heterogeneity and leveraging spatial associativity of springs, enabling superior reconstruction and simulation performance for both current and future states.
...
<details>
  <summary>Details</summary>
Motivation: Minimizing inconsistencies across successive AI versions is crucial. In image classification, adding new classes reduces margins and introduces conflicting patterns, causing negative flips where updated models misclassify previously correct samples.
...
Conclusion: The proposed approach effectively mitigates negative flips while maintaining high accuracy, addressing the challenge of model consistency during incremental learning with new classes.
...
<details>
  <summary>Details</summary>
Motivation: Accurate spatial alignment across time points in longitudinal mammograms is crucial for detecting meaningful tissue changes and improving deep learning-based breast cancer risk prediction models.
...
Result: Image-based registration consistently outperformed other methods across all metrics (predictive accuracy, precision, recall, deformation field quality). Applying image-based deformation fields in feature space yielded the best risk prediction performance.
...
TL;DR: DiveSeg is a novel underwater instance segmentation framework that leverages DINO pretrained vision models with two key components: AquaStyle Aligner for underwater domain adaptation and ObjectPrior Prompter for instance-level guidance.


<details>
  <summary>Details</summary>
Motivation: Underwater instance segmentation is crucial for marine exploration and ecological protection, and large-scale pretrained models like DINO show promise for complex downstream tasks but need adaptation for underwater environments.

Method: Built on DINO foundation model with two components: AquaStyle Aligner embeds underwater color style features during fine-tuning, and ObjectPrior Prompter uses binary segmentation prompts to provide object-level priors for instance segmentation.

Result: Achieves state-of-the-art performance on UIIS and USIS10K datasets, demonstrating effectiveness of the proposed framework for underwater instance segmentation.

Conclusion: DINO can serve as an effective feature learner for underwater instance segmentation, and the proposed DiveSeg framework with its two components successfully adapts foundation models to underwater domain while providing necessary instance-level guidance.
...
TL;DR: Proposes SASG-DA, a diffusion-based data augmentation method for sEMG gesture recognition that uses semantic guidance and sparse-aware sampling to generate faithful and diverse samples, improving model generalization.
...
Method: SASG-DA uses Semantic Representation Guidance (SRG) for faithful generation, Gaussian Modeling Semantic Modeling (GMSS) for flexible sampling, and Sparse-Aware Semantic Sampling to explore underrepresented regions for targeted diversity.
...
Conclusion: The proposed approach effectively mitigates overfitting and improves sEMG gesture recognition by providing both faithful and diverse augmented samples through semantic-guided diffusion and sparse-aware sampling strategies.
...
TL;DR: VideoChain is a novel Multi-hop Video Question Generation (MVQG) framework that generates reasoning-intensive questions across multiple video segments, outperforming existing methods on standard metrics.


<details>
  <summary>Details</summary>
Motivation: Current Multi-hop Question Generation is limited to text, while Video Question Generation only handles zero-hop questions over single video segments, creating a gap for reasoning across multiple video segments.

Method: VideoChain uses a modular architecture based on modified BART backbone with video embeddings to capture textual and visual dependencies. It automatically constructs the MVQ-60 dataset from TVQA+ by merging zero-hop QA pairs.
...
Conclusion: VideoChain effectively generates coherent, contextually grounded, and reasoning-intensive multi-hop questions across video segments, addressing limitations of existing approaches.
...
TL;DR: SLOPE combines structured sparsity and low-bit quantization to reduce DNN model size while maintaining accuracy, achieving ~20× size reduction with ~99% original accuracy on ResNet-18.
...
Result: Achieves ~20× model size reduction while retaining ~99% of original accuracy on ResNet-18. Consistently outperforms state-of-the-art quantization and structured sparsity methods across classification, detection, and segmentation tasks on various models.

Conclusion: SLOPE provides a principled way to effectively combine structured sparsity and low-bit quantization, enabling significant model compression without substantial accuracy loss.
...
Method: Proposed hierarchical vector-quantized variational auto-encoder with codebook to capture finite motion patterns at multiple resolutions, combined with auto-regressive model to learn prior distribution of motion-free images for guidance.
...
Conclusion: The disentanglement of motion features shows potential for application across anatomical regions and motion types, addressing the generalization limitations of conventional approaches.
...
TL;DR: The paper questions the importance of strict positional equivariance in Rotary Positional Encodings (RoPE) and proposes Spherical RoPE, a non-commutative alternative that performs equally well or better than equivariant methods in computer vision.


<details>
  <summary>Details</summary>
Motivation: To challenge the common belief that RoPE's success is primarily due to its positional equivariance property, and to explore whether strict equivariance is necessary for good performance in positional encodings for vision tasks.

Method: Proposed Spherical RoPE, which assumes non-commutative generators instead of the commutative generators used in Mixed RoPE, and conducted empirical comparisons between equivariant and non-equivariant positional encoding methods.

Result: Spherical RoPE demonstrated equivalent or better learning behavior compared to its equivariant analogues, suggesting that relative positional embeddings may not be as crucial as commonly believed in computer vision applications.

Conclusion: The findings indicate that the requirement for positional encodings to be strictly relative (equivariant) may be unnecessary, opening opportunities for developing faster and better-generalizing positional encoding methods for vision tasks without the constraint of equivariance.
...
TL;DR: This paper introduces TAG-PR, a new task for retrieving person images from both aerial and ground views using text descriptions, along with a new dataset (TAG-PEDES) and framework (TAG-CLIP) to address viewpoint heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Traditional text-based person retrieval only works with ground-view images, but real-world applications need to retrieve people across different viewpoints (aerial and ground), which presents significant challenges due to viewpoint discrepancies.

Method: Proposes TAG-CLIP framework with: (1) hierarchically-routed mixture of experts module to learn view-specific and view-agnostic features, and (2) viewpoint decoupling strategy to separate view-specific features for better cross-modal alignment.

Result: Developed the TAG-PEDES dataset from public benchmarks with automatically generated diverse text descriptions, and demonstrated the effectiveness of TAG-CLIP on both the new dataset and existing benchmarks.

Conclusion: TAG-PR addresses an important practical gap in person retrieval by handling heterogeneous aerial and ground views, with the proposed dataset and framework providing effective solutions for viewpoint heterogeneity challenges.
...
TL;DR: RAPTR is a weakly supervised radar-based 3D human pose estimation method that uses only 3D bounding boxes and 2D keypoints instead of expensive 3D keypoint labels, achieving state-of-the-art performance on indoor radar datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional radar-based indoor 3D human pose estimation requires costly 3D keypoint labels that are difficult to obtain in complex indoor environments with clutter, occlusions, or multiple people.

Method: Two-stage pose decoder architecture with pseudo-3D deformable attention: 1) pose decoder estimates initial 3D poses using 3D template loss with 3D BBox labels, 2) joint decoder refines poses using 2D keypoint labels and 3D gravity loss.

Result: Outperforms existing methods, reducing joint position error by 34.3% on HIBER dataset and 76.9% on MMVR dataset.

Conclusion: RAPTR demonstrates that effective 3D human pose estimation from radar can be achieved using only weak supervision (3D BBox and 2D keypoints), making the approach more scalable and practical for real-world applications.
...
TL;DR: Anatomy-VLM is a fine-grained vision-language model that incorporates multi-scale anatomical information for improved radiology disease interpretation by localizing key anatomical features and enriching them with structured knowledge.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models treat images holistically and overlook fine-grained details crucial for disease diagnosis, while clinicians analyze images by identifying anatomical structures as important regions of interest using prior medical knowledge.

Method: The model has three components: 1) an encoder to localize key anatomical features from medical images, 2) enrichment of these regions with structured knowledge for contextual interpretation, and 3) alignment of multi-scale medical information to generate clinically-interpretable disease predictions.

Result: Anatomy-VLM achieves outstanding performance on both in- and out-of-distribution datasets, validates well on downstream image segmentation tasks, and facilitates zero-shot anatomy-wise interpretation with strong expert-level clinical interpretation capabilities.

Conclusion: The fine-grained alignment in Anatomy-VLM effectively captures anatomical and pathology-related knowledge, providing expert-level clinical interpretation capabilities for radiology disease diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: Current AIGI detectors learn entangled forgery representations that conflate content-dependent flaws with content-agnostic artifacts, and are limited by outdated benchmarks that don't reflect real-world threats.

Method: A hybrid expert system with Routable Specialized Semantic Experts for different content domains and a Fixed Universal Artifact Expert, trained using a two-stage strategy with domain-specific hard-sampling and lightweight gating network for input routing.

Result: Extensive experiments on traditional benchmarks and the new Mirage dataset show OmniAID surpasses existing monolithic detectors and establishes a new robust standard for AIGI authentication.

Conclusion: By explicitly decoupling 'what is generated' from 'how it is generated', OmniAID achieves robust generalization against modern, in-the-wild threats, addressing the limitations of current entangled representation learning approaches.
...
TL;DR: Proposes DBPNet with Cross-Pyramid Consistency Regularization (CPCR) for semi-supervised medical image segmentation, achieving state-of-the-art performance by leveraging dual decoders and multi-scale consistency learning.


<details>
  <summary>Details</summary>
Motivation: To effectively exploit unlabeled data in semi-supervised medical image segmentation by addressing limitations of existing methods and improving knowledge distillation in deep hierarchical features.

Method: Hybrid Dual Branch Pyramid Network (DBPNet) with encoder and two slightly different decoders producing pyramid predictions, combined with Cross-Pyramid Consistency Regularization (CPCR) that extends consistency learning to pyramid predictions across decoders.
...
Conclusion: The proposed hybrid consistency learning approach with cross-pyramid regularization effectively leverages unlabeled data and improves semi-supervised medical image segmentation performance through enhanced knowledge distillation in deep features.
...
TL;DR: CIG is a novel attribution method for Whole Slide Image analysis that enhances interpretability by computing contrastive gradients in logit space, providing sharper differentiation between tumor subtypes while maintaining theoretical soundness.


<details>
  <summary>Details</summary>
Motivation: Existing attribution methods like Integrated Gradients capture model decision patterns but overlook class-discriminative signals crucial for distinguishing tumor subtypes in computational pathology.

Method: Contrastive Integrated Gradients (CIG) computes contrastive gradients in logit space by comparing feature importance relative to a reference class, satisfying integrated attribution axioms. Two metrics (MIL-AIC and MIL-SIC) measure predictive information and model confidence evolution.

Result: CIG yields more informative attributions quantitatively (using MIL-AIC and MIL-SIC) and qualitatively through visualizations that align closely with ground truth tumor regions across three cancer datasets (CAMELYON16, TCGA-RCC, TCGA-Lung).

Conclusion: CIG demonstrates strong potential for interpretable and trustworthy WSI-based diagnostics by providing sharper differentiation between tumor and non-tumor areas while maintaining theoretical consistency.
...
Method: Developed data pipeline to merge four datasets, used Faster R-CNN with ResNet-50-FPN backbone, compared random initialization vs. transfer learning from COCO dataset.
...
Conclusion: Validated methodology provides robust foundation for high-accuracy deployable systems for automated hematological diagnosis.
...
TL;DR: CoMa introduces a compressed pre-training phase to transform vision-language models into competitive embedding models through contrastive learning, achieving state-of-the-art results on MMEB with improved efficiency.


<details>
  <summary>Details</summary>
Motivation: To decouple comprehensive input understanding from discriminative feature learning in vision-language models, enabling better embedding performance through contrastive learning.

Method: Proposes CoMa - a compressed pre-training phase that serves as a warm-up stage for contrastive learning, using only a small amount of pre-training data to adapt VLMs into embedding models.

Result: Achieves new state-of-the-art results among VLMs of comparable size on the MMEB benchmark, demonstrating optimization in both efficiency and effectiveness.

Conclusion: The compressed pre-training approach successfully transforms VLMs into competitive embedding models, validating that comprehensive input understanding facilitates superior downstream task performance through contrastive learning.
...
TL;DR: Proposes a fast multi-organ segmentation framework using hierarchical sparse sampling and Residual Transformer to reduce computation time while maintaining accuracy in 3D medical imaging.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for 3D medical image segmentation consume significant time and memory when processing entire volumes voxel by voxel, creating a speed-accuracy trade-off that needs improvement.

Method: Uses hierarchical sparse sampling strategy to reduce computation time while preserving hierarchical context across multiple resolution levels, combined with a Residual Transformer architecture to efficiently extract and combine information from different levels in the sparse descriptor.
...
Conclusion: The method demonstrates potential for achieving real-time fine organ segmentation in 3D medical imaging with both speed and accuracy improvements.
...
TL;DR: CleverBirds is a large-scale benchmark for knowledge tracing in fine-grained bird species recognition, collected from eBird citizen science platform with 40,000+ participants and 17M+ questions across 10,000+ species.


<details>
  <summary>Details</summary>
Motivation: To understand how humans acquire expertise in fine-grained visual recognition and accurately infer human learners' knowledge states in complex classification tasks.

Method: Collect large-scale data from eBird citizen science platform with quiz participation, tracking learning patterns across 40,000+ participants answering 17M+ multiple-choice questions spanning 10,000+ bird species.

Result: Knowledge tracking is challenging across participant subgroups and question types, with contextual information providing varying predictive benefits. Dataset enables evaluation of visual knowledge tracing methods.

Conclusion: CleverBirds enables new avenues for studying visual expertise development over time and across individuals, serving as a large benchmark with substantially more learnable concepts than existing datasets.
...
Method: Uses Plan-and-Act dual-agent architecture: planner agent interprets user intentions and decomposes tasks, executor agents execute through modular MCP-based tool servers, with hierarchical multi-level memory for contextual continuity.

Result: Enables iterative and any-conditioned video workflows (text/image/video-conditioned generation → multi-round editing → object segmentation → compositional synthesis) with full traceability and inter-agent communication.
...
Method: Leverages Large Language Models as backbone, uses 3D sign language data instead of 2D video, investigates direct translation from 3D gesture features to text and instruction-guided translation modulated by external prompts.
...
TL;DR: 3D4D is an interactive 4D visualization framework that converts static images and text into coherent 4D scenes using WebGL and Supersplat rendering with foveated rendering for real-time interaction.
...
Method: Integrates WebGL with Supersplat rendering through four core modules and employs foveated rendering strategy for efficient real-time multi-modal interaction.

Result: Developed a framework that successfully transforms static images and text into coherent 4D scenes with real-time interactive capabilities.

Conclusion: 3D4D provides an effective solution for creating and exploring complex 4D environments through interactive visualization and user-driven exploration.
...
TL;DR: A robust framework for reconstructing high-quality 3D meshes from multi-view images with noisy camera poses, enabling downstream robotic applications.


<details>
  <summary>Details</summary>
Motivation: Existing NeRF-based methods require accurate camera poses and produce implicit representations incompatible with standard 3D software, limiting practical robotic use.
...
Conclusion: Bridges the gap between neural implicit representations and practical robotic applications by providing editable 3D meshes that work with noisy camera poses.
...
TL;DR: Proposes an attention-based Vision Transformer for UE positioning using Angle Delay Profile from CSI, achieving significant performance improvements over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing DL models for UE positioning have limitations: they apply equal attention to entire inputs and are not well-suited for non-sequential data like instantaneous CSI.

Method: Uses an attention-based Vision Transformer architecture that focuses on Angle Delay Profile extracted from CSI matrix.

Result: Achieves RMSE of 0.55m indoors, 13.59m outdoors on DeepMIMO, and 3.45m on ViWi's outdoor blockage scenario, outperforming state-of-the-art by ~38% with better error distribution.

Conclusion: The proposed attention-based ViT architecture effectively addresses limitations of existing DL models for UE positioning and demonstrates superior performance across different scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current methods either overemphasize spatial transcriptomics (losing structural information) or overly smooth features through vanilla contrastive learning (losing functional information), leading to loss of essential information in tumor substructure identification.
...
Result: The model demonstrates superior performance surpassing state-of-the-art methods in detecting tumor heterogeneity and tumor micro-environment regions, which are clinically crucial aspects.

Conclusion: SENCA-st effectively integrates histopathology and spatial transcriptomics data by preserving both structural and functional information, enabling better identification of tumor substructures associated with cancer drug resistance.
...
### [178] [The Polite Liar: Epistemic Pathology in Language Models](https://arxiv.org/abs/2511.07477)
*Bentley DeVilling*

Main category: cs.CY

TL;DR: RLHF creates 'polite liars' - LLMs that confidently fabricate information due to structural indifference to truth, prioritizing perceived sincerity over evidential accuracy.


<details>
  <summary>Details</summary>
Motivation: To understand why LLMs confidently generate false information despite appearing sincere, and to identify the structural flaws in RLHF that cause this epistemic pathology.

Method: Philosophical analysis using Frankfurt's concept of bullshit, epistemic virtue theory, speech-act philosophy, and cognitive alignment to examine RLHF's reward architecture.

Result: RLHF optimizes for user satisfaction and conversational fluency rather than truth, creating models that mimic epistemic confidence without justification.

Conclusion: Proposes 'epistemic alignment' principle: reward justified confidence over perceived fluency to resolve tension between linguistic cooperation and epistemic integrity.
...
### [179] [Generative Artificial Intelligence in Qualitative Research Methods: Between Hype and Risks?](https://arxiv.org/abs/2511.08461)
*Maria Couto Teixeira,Marisa Tschopp,Anna Jobin*

Main category: cs.CY

TL;DR: GenAI is methodologically invalid for qualitative coding and risks undermining research robustness despite efficiency claims.


<details>
  <summary>Details</summary>
Motivation: To critically examine the role of generative AI in qualitative research methodologies amid increasing promotion and use.

Method: Position paper analysis critically interrogating genAI's methodological validity in qualitative coding.

Result: GenAI lacks methodological validity, weakens research rigor due to documentation gaps, commercial opacity, and error tendencies.

Conclusion: Researchers should prioritize sound methodology over technological novelty as risks outweigh benefits for genAI in qualitative research.
...
### [180] [Think Before You Retrieve: Learning Test-Time Adaptive Search with Small Language Models](https://arxiv.org/abs/2511.07581)
*Supriti Vijay,Aman Priyanshu,Anu Vellore,Baturay Saglam,Amin Karbasi*

Main category: cs.AI

TL;DR: Orion enables compact models (350M-1.2B) to perform iterative retrieval through learned search strategies, outperforming much larger models on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current approaches lack reasoning capabilities for iterative retrieval: neural retrievers can't reason, LLMs are too expensive, and query rewriting is static. Existing methods fail to capture the iterative dynamics of exploration, feedback, and revision needed for complex queries.

Method: Combines: (1) synthetic trajectory generation and supervised fine-tuning for diverse exploration patterns, (2) reinforcement learning to reward effective query refinement and backtracking, (3) inference-time beam search algorithms that exploit learned self-reflection capabilities.

Result: 1.2B model achieves 77.6% success on SciFact (vs. 72.6% prior), 25.2% on BRIGHT (vs. 22.1%), 63.2% on NFCorpus (vs. 57.8%), competitive on FEVER, HotpotQA, and MSMarco. Outperforms retrievers 200-400x larger on five of six benchmarks using only 3% of training data.

Conclusion: Retrieval performance can emerge from learned strategies rather than just model scale, when models are trained to search, reflect, and revise.
...
### [181] [Beyond Fact Retrieval: Episodic Memory for RAG with Generative Semantic Workspaces](https://arxiv.org/abs/2511.07587)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM memory solutions fail to build narrative representations for tracking entities through episodic events, as they are tailored for fact-based retrieval rather than space-time-anchored reasoning.

Method: GSW comprises an Operator that maps observations to semantic structures and a Reconciler that integrates them into a persistent workspace enforcing temporal, spatial, and logical coherence.

Result: On Episodic Memory Benchmark (100k-1M tokens), GSW outperforms RAG baselines by up to 20% and reduces query-time context tokens by 51%, significantly improving efficiency.

Conclusion: GSW provides a blueprint for endowing LLMs with human-like episodic memory, enabling reasoning over long horizons and paving the way for more capable agents.
...
TL;DR: ResearchRubrics is a new benchmark for evaluating Deep Research agents, featuring 2,500+ expert-written rubrics across 2,800+ hours of human labor to assess factual grounding, reasoning, and clarity in long-form research responses.
...
Method: Created ResearchRubrics benchmark with domain-diverse prompts and fine-grained rubrics, proposed complexity framework (conceptual breadth, logical nesting, exploration), and developed human/model evaluation protocols for rubric adherence.
...
Conclusion: There's a significant need for robust, scalable assessment of deep research capabilities, and ResearchRubrics is released to facilitate progress toward well-justified research assistants.
...
### [183] [SparseRM: A Lightweight Preference Modeling with Sparse Autoencoder](https://arxiv.org/abs/2511.07896)
...
### [184] [Thinker: Training LLMs in Hierarchical Thinking for Deep Search via Multi-Turn Interaction](https://arxiv.org/abs/2511.07943)
...
TL;DR: Thinker is a hierarchical thinking model that enables LLMs to perform deep search through multi-turn interactions, decomposing complex problems into sub-problems with dual representations (natural language and logical functions) to enhance reasoning coherence and verifiability.


<details>
  <summary>Details</summary>
Motivation: Previous approaches using end-to-end reinforcement learning for training LLMs with external retrievers neglect supervision over the reasoning process, making it difficult to guarantee logical coherence and rigor in problem-solving.

Method: Proposes a hierarchical thinking model that decomposes complex problems into independently solvable sub-problems with dual representations (natural language + logical functions), performs knowledge boundary determination to avoid unnecessary searches, and passes dependencies between sub-problems via logical functions.

Result: With only several hundred training samples, Thinker achieves competitive performance with established baselines. When scaled to full training set, it significantly outperforms other methods across various datasets and model sizes.

Conclusion: Thinker provides a supervisable and verifiable reasoning process for LLMs when leveraging external knowledge, demonstrating strong performance with minimal training data and significant improvements at scale.
...
### [185] [Dual-Process Scaffold Reasoning for Enhancing LLM Code Debugging](https://arxiv.org/abs/2511.08052)
*Po-Chung Hsieh,Chin-Po Chen,Jeng-Lin Li,Ming-Ching Chang*

Main category: cs.AI

TL;DR: Proposes a psychologically-inspired Scaffold Reasoning framework for code debugging that combines scaffold, analytic, and integration streams to improve reasoning accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current LLMs lack in-depth exploration of System 2 reasoning steps that balance complexity and computational efficiency, despite drawing from psychological theories.

Method: A Scaffold Reasoning framework with three streams: Scaffold Stream (constructs reference code), Analytic Stream (analyzes buggy code), and Integration Stream (combines both streams).

Result: Achieves 88.91% pass rate and 5.36 seconds average inference time on DebugBench, outperforming other reasoning approaches across various LLMs in both accuracy and efficiency.

Conclusion: The framework aligns with human cognitive processes and demonstrates advantages across varying problem difficulties and bug types, though limitations exist in different cognitive pathways.
...
### [186] [Information Capacity: Evaluating the Efficiency of Large Language Models via Text Compression](https://arxiv.org/abs/2511.08066)
*Cheng Yuan,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.AI

TL;DR: The paper introduces 'information capacity' as a unified metric for LLM efficiency that combines text compression performance with computational complexity, enabling fair comparisons across different model sizes and architectures.


<details>
  <summary>Details</summary>
Motivation: There is a lack of unified metrics to accurately reflect LLM efficiency across different model sizes and architectures, especially with increasing computational demands and test-time scaling.

Method: Propose information capacity measure based on text compression performance relative to computational complexity, evaluating 49 models on 5 heterogeneous datasets while considering tokenizer efficiency.

Result: Models of varying sizes within a series show consistent information capacity, enabling fair efficiency comparisons across model series and accurate performance prediction within series. Tokenizer efficiency significantly impacts overall efficiency.

Conclusion: Information capacity provides a comprehensive efficiency metric that incorporates tokenizer efficiency and enables consistent evaluation across different LLM architectures and sizes.
...
### [187] [SciAgent: A Unified Multi-Agent System for Generalistic Scientific Reasoning](https://arxiv.org/abs/2511.08151)
...
<details>
  <summary>Details</summary>
Motivation: Current AI systems for scientific tasks are narrow and handcrafted, lacking the ability to adapt reasoning strategies across different scientific disciplines and difficulty levels.
...
Result: Consistently attained or surpassed human gold-medalist performance across mathematics and physics Olympiads (IMO, IMC, IPhO, CPhO), and demonstrated generalization on chemistry Olympiad and Humanity's Last Exam benchmark.
...
### [188] [Towards Outcome-Oriented, Task-Agnostic Evaluation of AI Agents](https://arxiv.org/abs/2511.08242)
*Waseem AlShikh,Muayad Sayed Ali,Brian Kennedy,Dmytro Mozolevskyi*

Main category: cs.AI

TL;DR: Proposes 11 outcome-based metrics for AI agent evaluation (Goal Completion Rate, Autonomy Index, etc.) and shows Hybrid Agents perform best across diverse domains.


<details>
  <summary>Details</summary>
Motivation: Current infrastructural metrics (latency, throughput) are insufficient as they don't capture decision quality, autonomy, or business value of AI agents.

Method: Large-scale simulated experiment testing 4 agent architectures (ReAct, Chain-of-Thought, Tool-Augmented, Hybrid) across 5 domains using 11 proposed metrics.

Result: Hybrid Agent achieved highest performance (88.8% Goal Completion Rate, highest ROI) and was most consistent across metrics, revealing significant trade-offs between architectures.

Conclusion: Provides standardized framework for holistic AI agent evaluation, enabling better development, deployment and governance across industries.
...
### [189] [Multi-Agent GraphRAG: A Text-to-Cypher Framework for Labeled Property Graphs](https://arxiv.org/abs/2511.08274)
*Anton Gusarov,Anastasia Volkova,Valentin Khrulkov,Andrey Kuznetsov,Evgenii Maslov,Ivan Oseledets*

Main category: cs.AI

TL;DR: Proposes Multi-Agent GraphRAG, a modular LLM system for generating Cypher queries to query Labeled Property Graph databases, addressing the gap in using LPG databases for GraphRAG instead of traditional RDF knowledge graphs.


<details>
  <summary>Details</summary>
Motivation: Most GraphRAG methods focus on RDF knowledge graphs with triple representations and SPARQL queries, leaving the potential of Cypher and LPG databases underexplored as scalable reasoning engines for GraphRAG pipelines.

Method: A modular LLM agentic system with automated Cypher query generation and execution using Memgraph backend, featuring iterative content-aware correction and normalization with aggregated feedback loops for semantic and syntactic refinement.

Result: Evaluated on CypherBench dataset across general domains and demonstrated on IFC-based property graph representing a building digital twin, showing the approach can bridge AI with real-world industrial applications.

Conclusion: The proposed Multi-Agent GraphRAG system effectively leverages LPG databases with Cypher queries, enabling scalable reasoning and practical industrial digital automation use cases beyond traditional RDF-based approaches.
...
TL;DR: A machine learning system for automated methane plume detection was deployed in the UN's Methane Alert and Response System, reducing false detections by 74% through model ensembling and facilitating verification of 1,351 methane leaks over 7 months.


<details>
  <summary>Details</summary>
Motivation: Current satellite-based methane detection methods using matched filters produce many false positives requiring manual verification, creating operational inefficiencies in global methane monitoring efforts.

Method: Created the largest global dataset of annotated methane plumes from three imaging spectrometer missions, compared deep learning models, extended evaluation to full granules, and implemented model ensembling to reduce false detections.

Result: The system reduced false detections by over 74% and processed 1,351 distinct methane leaks during 7 months of deployment, resulting in 479 stakeholder notifications. Demonstrated utility in verifying mitigation success across multiple countries.

Conclusion: This work represents a critical step toward a global AI-assisted methane leak detection system capable of handling the dramatically higher data volumes expected from current and future imaging spectrometers.
...
TL;DR: Video foundation models are evolving from visual generators to implicit world models that simulate physical dynamics, interactions, and planning, combining world modeling with video rendering.


<details>
  <summary>Details</summary>
Motivation: The shift from generating visually appealing clips to building interactive virtual environments that maintain physical plausibility points toward video foundation models functioning as implicit world models.

Method: Conceptualizes modern video foundation models as combining two components: an implicit world model (encoding physical laws, interaction dynamics, agent behavior) and a video renderer (transforming latent simulation into realistic visual observations).

Result: Traces progression through four generations of video generation, culminating in world models with intrinsic physical plausibility, real-time multimodal interaction, and multi-scale planning capabilities.

Conclusion: Discusses open challenges and design principles for next-generation world models, including the role of agent intelligence in shaping and evaluating these systems.
...
### [192] [Re$^{\text{2}}$MaP: Macro Placement by Recursively Prototyping and Packing Tree-based Relocating](https://arxiv.org/abs/2511.08054)
*Yunqi Shi,Xi Lin,Zhiang Wang,Siyuan Xu,Shixiong Kai,Yao Lai,Chengrui Gao,Ke Xue,Mingxuan Yuan,Chao Qian,Zhi-Hua Zhou*

Main category: cs.AR

TL;DR: Re²MaP is a recursive macro placement method that uses prototyping and packing tree-based relocating to generate expert-quality placements, achieving significant improvements in timing metrics compared to state-of-the-art placers.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating high-quality macro placements that optimize both wirelength and dataflow while satisfying various design constraints in modern chip design.

Method: Uses multi-level macro grouping and PPA-aware cell clustering to create a unified connection matrix, followed by mixed-size placement prototyping with DREAMPlace, angle-based analytical placement (ABPlace) on ellipses, and packing tree-based relocating with evolutionary search.

Result: Achieves up to 22.22% improvement in WNS and 97.91% improvement in TNS compared to Hier-RTLMP, with better performance across WNS, TNS, power, DRC violations, and runtime than ReMaP across seven test cases.

Conclusion: Re²MaP effectively generates expert-quality macro placements through recursive prototyping and packing tree-based relocating, demonstrating superior performance over existing state-of-the-art methods in timing optimization and design quality metrics.
...
TL;DR: EvoPS is an evolutionary patch selection framework that optimizes the trade-off between computational cost and diagnostic accuracy in whole-slide image analysis by selecting minimal informative patches.


<details>
  <summary>Details</summary>
Motivation: Current patch selection methods for gigapixel whole-slide images are inefficient, computationally expensive, and fail to explicitly manage the crucial trade-off between patch count and diagnostic accuracy.

Method: Formulates patch selection as multi-objective optimization problem using evolutionary search to simultaneously minimize selected patches and maximize downstream similarity search performance, generating Pareto front solutions.

Result: Reduces required training patch embeddings by over 90% while maintaining or improving classification F1-score across four TCGA cancer cohorts using five different pretrained models.

Conclusion: EvoPS provides a robust and principled method for creating efficient, accurate, and interpretable WSI representations with optimal balance between computational cost and diagnostic performance.
...
TL;DR: Developed a deep learning model using a self-supervised foundation model (USF-MAE) to detect ventriculomegaly in prenatal ultrasound images, achieving high performance with F1-scores over 91% and outperforming baseline models.


<details>
  <summary>Details</summary>
Motivation: Ventriculomegaly is a prenatal condition with dilated cerebral ventricles that requires early diagnosis due to association with fetal aneuploidies and genetic syndromes, making automated detection important.

Method: Fine-tuned a pretrained Ultrasound Self-Supervised Foundation Model with Masked Autoencoding (USF-MAE) on fetal brain ultrasound images for binary classification (normal vs ventriculomegaly), using 5-fold cross-validation and independent test evaluation.

Result: Achieved F1-scores of 91.76% (cross-validation) and 91.78% (test set), significantly outperforming baseline models (VGG-19, ResNet-50, ViT-B/16) with high precision (94.47%) and accuracy (97.24%). Eigen-CAM heatmaps confirmed model focused on ventricle areas.

Conclusion: The USF-MAE model effectively detects ventriculomegaly with high accuracy and clinical plausibility, demonstrating the value of self-supervised foundation models for medical image analysis.
...
### [195] [DynaQuant: Dynamic Mixed-Precision Quantization for Learned Image Compression](https://arxiv.org/abs/2511.07903)
*Youneng Bao,Yulong Cheng,Yiping Liu,Yichen Yang,Peng Qin,Mu Li,Yongsheng Liang*

Main category: eess.IV

TL;DR: DynaQuant introduces dynamic mixed-precision quantization for Learned Image Compression, adapting bit-widths to data distributions and layer sensitivities for optimal performance-efficiency trade-off.


<details>
  <summary>Details</summary>
Motivation: Static uniform quantization in LIC fails to adapt to diverse data distributions and sensitivity characteristics, leading to suboptimal performance-efficiency balance.

Method: Two-level approach: content-aware quantization with learnable scaling/offset parameters trained via Distance-aware Gradient Modulator, and dynamic bit-width selector that assigns optimal precision per layer based on input data.
...
Conclusion: Enables practical deployment of advanced LIC on diverse hardware platforms through fully dynamic quantization framework.
...
### [196] [From Noise to Latent: Generating Gaussian Latents for INR-Based Image Compression](https://arxiv.org/abs/2511.08009)
*Chaoyi Lin,Yaojun Wu,Yue Li,Junru Li,Kai Zhang,Li Zhang*

Main category: eess.IV

TL;DR: Proposes novel image compression method that generates image-specific latents from Gaussian noise using shared random seed, eliminating need to transmit latent codes while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Address limitations of both INR-based methods (lack expressive latents) and E2E methods (complex entropy models, transmission overhead) by exploring inverse direction of Gaussian normalization in E2E codecs.

Method: Uses multi-scale Gaussian noise tensor with shared random seed, Gaussian Parameter Prediction module to estimate distribution parameters, reparameterization trick for one-shot latent generation, and synthesis network for image reconstruction.

Result: Achieves competitive rate-distortion performance on Kodak and CLIC datasets without transmitting latent codes.

Conclusion: First work to explore Gaussian latent generation for learned image compression, offering new paradigm that preserves latent-based benefits while eliminating transmission overhead.
...
### [197] [Alignment-Constrained Dynamic Pruning for LLMs: Identifying and Preserving Alignment-Critical Circuits](https://arxiv.org/abs/2511.07482)
...
<details>
  <summary>Details</summary>
Motivation: Dynamic pruning methods for LLMs exacerbate alignment degradation by not preserving safety-critical circuits across diverse inputs, creating deployment challenges despite computational efficiency gains.

Method: Alignment-Aware Probe Pruning (AAPP) builds on Probe Pruning to adaptively preserve alignment-relevant circuits during inference through dynamic structured pruning.

Result: Experiments on LLaMA 2-7B, Qwen2.5-14B-Instruct, and Gemma-3-12B-IT show AAPP improves refusal rates by 50% at matched computational costs.

Conclusion: AAPP enables efficient yet safety-preserving LLM deployment by addressing alignment vulnerabilities in dynamic pruning while maintaining computational efficiency.
...
### [198] [LLM Output Drift: Cross-Provider Validation & Mitigation for Financial Workflows](https://arxiv.org/abs/2511.07585)
*Raffi Khatchadourian,Rolando Franco*

Main category: cs.LG

TL;DR: Smaller LLMs (7B-8B parameters) achieve 100% output consistency in financial tasks, while larger models like GPT-OSS-120B show only 12.5% consistency, challenging assumptions that bigger models are always better for production deployment.
...
Method: Developed a finance-calibrated deterministic test harness with greedy decoding, fixed seeds, and SEC structure-aware retrieval; used task-specific invariant checking with materiality thresholds and SEC citation validation; evaluated five models across three regulated financial tasks with 480 runs.
...
Conclusion: Model size inversely correlates with output consistency in financial applications, enabling risk-appropriate deployment decisions and providing compliance pathways for financial AI deployments.
...
### [199] [DynaAct: Large Language Model Reasoning with Dynamic Action Spaces](https://arxiv.org/abs/2511.08043)
*Xueliang Zhao,Wei Wu,Jian Guan,Qintong Li,Lingpeng Kong*

Main category: cs.LG

TL;DR: DynaAct automatically constructs compact action spaces for sequential decision-making by extracting action sketches from diverse reasoning problems and using submodular optimization to select optimal candidate sets.


<details>
  <summary>Details</summary>
Motivation: Existing approaches use either manually defined action spaces that lack scalability or unstructured spaces that make exhaustive search computationally prohibitive.

Method: Extract general action sketches from diverse reasoning problems using LLMs, formulate submodular function evaluating utility and diversity, and use greedy algorithm to select optimal candidate set.

Result: Significant performance improvements on six diverse benchmarks while maintaining efficient inference without substantial latency.

Conclusion: DynaAct provides an effective framework for automatically constructing compact action spaces that enhance sequential reasoning in complex problem-solving.
...
### [200] [Towards Personalized Quantum Federated Learning for Anomaly Detection](https://arxiv.org/abs/2511.07471)
...
<details>
  <summary>Details</summary>
Motivation: Anomaly detection faces challenges with context-dependent anomalies and limited labeled data. Quantum federated learning helps but struggles with client heterogeneity in hardware capabilities, circuit designs, noise levels, and data encoding methods, making single global models ineffective.

Method: Proposes personalized quantum federated learning (PQFL) using parameterized quantum circuits and classical optimizers, with quantum-centric personalization that adapts each client's model to its hardware characteristics and data representation.
...
Conclusion: PQFL is effective and scalable for practical quantum federated settings, demonstrating superior performance in handling diverse and realistic conditions with heterogeneous quantum clients.
...
### [201] [Multivariate Variational Autoencoder](https://arxiv.org/abs/2511.07472)
*Mehmet Can Yavuz*

Main category: cs.LG

TL;DR: MVAE is a VAE variant that enables full-covariance posteriors while maintaining Gaussian tractability, using global coupling and per-sample scaling to improve reconstruction, calibration, and latent structure.


<details>
  <summary>Details</summary>
Motivation: To overcome the diagonal posterior restriction in standard VAEs while preserving analytical tractability, enabling better modeling of latent correlations and uncertainty.
...
Result: Consistently matches or improves reconstruction (MSE), calibration (NLL/Brier/ECE), and unsupervised structure (NMI/ARI) across multiple datasets, especially at mid-range latent sizes.

Conclusion: MVAE provides a tractable full-covariance alternative to diagonal VAEs with demonstrated performance improvements and more coherent latent representations.
...
### [202] [On the Role of Calibration in Benchmarking Algorithmic Fairness for Skin Cancer Detection](https://arxiv.org/abs/2511.07700)
*Brandon Dominique,Prudence Lam,Nicholas Kurtansky,Jochen Weber,Kivanc Kose,Veronica Rotemberg,Jennifer Dy*

Main category: cs.LG

TL;DR: AI models for melanoma detection show performance disparities across demographic subgroups. This paper introduces calibration as a complementary metric to AUROC-based fairness metrics to better assess subgroup biases in skin cancer detection models.


<details>
  <summary>Details</summary>
Motivation: Clinical adoption of AI models for melanoma detection is hindered by performance disparities across demographic subgroups (gender, race, age). Previous benchmarking focused on AUROC-based fairness metrics, which don't provide insights into model calibration and accurate probability estimation.
...
Result: Existing models enhance discriminative accuracy but often over-diagnose risk and exhibit calibration issues when applied to new datasets. Models show performance disparities across demographic subgroups despite improved accuracy.
...
### [203] [From Exploration to Exploitation: A Two-Stage Entropy RLVR Approach for Noise-Tolerant MLLM Training](https://arxiv.org/abs/2511.07738)
...
<details>
  <summary>Details</summary>
Motivation: RLVR for MLLMs depends on high-quality labeled data, which is scarce and noisy in real-world scenarios. Existing unsupervised methods can overfit to incorrect labels and limit reward ranking signals for GRPO.

Method: Two-stage token-level entropy optimization: exploration phase with entropy maximization for diverse outputs and prevention of premature convergence, followed by exploitation phase with entropy minimization for confident outputs and knowledge consolidation.

Result: Empirical evaluation across three MLLM backbones (Qwen2-VL-2B, Qwen2-VL-7B, Qwen2.5-VL-3B) in diverse noise settings and tasks shows consistent outperformance over prior approaches, unifying external, internal, and entropy-based methods.

Conclusion: The phased entropy optimization strategy delivers robust and superior performance by enhancing noise tolerance and enabling reliable reward gradient estimation in GRPO across various MLLM architectures and tasks.
...
### [204] [IBMA: An Imputation-Based Mixup Augmentation Using Self-Supervised Learning for Time Series Data](https://arxiv.org/abs/2511.07930)
*Dang Nha Nguyen,Hai Dang Nguyen,Khoa Tho Anh Nguyen*

Main category: cs.LG

TL;DR: Proposes IBMA (Imputation-Based Mixup Augmentation) for time series forecasting, combining imputation-augmented data with Mixup to improve model generalization and performance across multiple datasets and models.


<details>
  <summary>Details</summary>
Motivation: Time series forecasting has limited augmentation strategies compared to other fields like images/text, with advanced techniques like Mixup rarely used, creating a need for more effective augmentation methods.

Method: IBMA combines Imputation-Augmented data with Mixup augmentation, evaluated on forecasting models including DLinear (MLP), TimesNet (CNN), and iTrainformer (Transformer) across four ETT datasets.

Result: IBMA consistently enhanced performance with 22 improvements out of 24 instances, achieving 10 best performances, particularly with iTrainformer imputation, outperforming 8 other augmentation techniques.

Conclusion: IBMA effectively improves time series forecasting performance through novel combination of imputation and Mixup augmentation, demonstrating consistent benefits across different models and datasets.
...
### [205] [The Online Patch Redundancy Eliminator (OPRE): A novel approach to online agnostic continual learning using dataset compression](https://arxiv.org/abs/2511.08226)
*Raphaël Bayle,Martial Mermillod,Robert M. French*

Main category: cs.LG

TL;DR: The paper proposes OPRE, an online dataset compression method for continual learning that achieves state-of-the-art performance on CIFAR datasets while requiring minimal assumptions about future data.


<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting in continual learning while avoiding the limitations of current methods that rely on a priori information about future data, particularly pretrained feature extractors.

Method: Online Patch Redundancy Eliminator (OPRE) - an online dataset compression algorithm combined with classifier training at test time.
...
Conclusion: Online dataset compression may be necessary for fully agnostic continual learning, and OPRE demonstrates this approach with minimal, interpretable assumptions about future data.
...
### [206] [Aligning by Misaligning: Boundary-aware Curriculum Learning for Multimodal Alignment](https://arxiv.org/abs/2511.08399)
*Hua Ye,Hang Ding,Siyuan Chen,Yiyang Jiang,Changyuan Zhang,Xuan Zhang*

Main category: cs.LG

TL;DR: BACL improves multimodal models by focusing on ambiguous negative pairs that differ only slightly from positives, using a curriculum learning approach with boundary-aware sampling and local attention contrastive loss.


<details>
  <summary>Details</summary>
Motivation: Most multimodal models treat all negative pairs equally, failing to leverage the learning potential from borderline cases where negatives differ from positives by only small details.

Method: Proposes Boundary-Aware Curriculum with Local Attention (BACL) - a lightweight add-on with two modules: Boundary-aware Negative Sampler that gradually increases difficulty, and Contrastive Local Attention loss that highlights mismatch locations.
...
Conclusion: BACL effectively turns borderline cases into curriculum signals, with theoretical O(1/n) error rate and practical performance gains, working with any off-the-shelf dual encoder.
...
### [207] [NeuCLIP: Efficient Large-Scale CLIP Training with Neural Normalizer Optimization](https://arxiv.org/abs/2511.08417)
*Xiyuan Wei,Chih-Jen Lin,Tianbao Yang*

Main category: cs.LG

TL;DR: NeuCLIP is a novel optimization framework that reformulates CLIP's contrastive loss using convex and variational analysis to accurately estimate normalization terms without requiring large batches, enabling efficient training on massive datasets.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP training methods require large batches for accurate normalization term estimation, which is computationally expensive. Per-sample estimators have optimization errors that scale with dataset-to-batch size ratio, limiting effectiveness for large datasets or small batches.
...
Result: Extensive experiments on large-scale CLIP training (millions to billions of samples) show NeuCLIP outperforms previous methods with more accurate normalizer estimation and improved performance.

Conclusion: NeuCLIP provides an elegant optimization framework that overcomes limitations of batch-based normalization estimation in CLIP training, enabling efficient and effective training on massive datasets through neural network-based normalizer prediction.
...
TL;DR: LeJEPA presents a theoretically grounded Joint-Embedding Predictive Architecture with Sketched Isotropic Gaussian Regularization (SIGReg) that simplifies self-supervised learning with improved stability, scalability, and performance.


<details>
  <summary>Details</summary>
Motivation: Current Joint-Embedding Predictive Architectures (JEPAs) lack practical guidance and theoretical foundations, leading to ad-hoc research and development approaches.

Method: LeJEPA combines JEPA predictive loss with SIGReg regularization, which constrains embeddings to follow an isotropic Gaussian distribution identified as optimal for minimizing downstream prediction risk.

Result: Achieves 79% accuracy on ImageNet-1k with ViT-H/14 using linear evaluation, demonstrates stability across 10+ datasets and 60+ architectures with varying scales and domains.

Conclusion: LeJEPA offers a simple, theory-friendly ecosystem that can reestablish self-supervised pre-training as a core pillar of AI research with practical implementation requiring only ~50 lines of code.
...
### [209] [How Brittle is Agent Safety? Rethinking Agent Risk under Intent Concealment and Task Complexity](https://arxiv.org/abs/2511.08487)
*Zihan Ma,Dongsheng Zhu,Shudong Liu,Taolin Zhang,Junnan Liu,Qingqiu Li,Minnan Luo,Songyang Zhang,Kai Chen*

Main category: cs.MA

TL;DR: OASIS introduces a hierarchical benchmark to evaluate LLM agent safety under intent concealment and task complexity, revealing safety degradation with obscured intent and a "Complexity Paradox" where harder tasks appear safer due to capability limitations.


<details>
  <summary>Details</summary>
Motivation: Current safety evaluations focus on atomic harms but fail to address sophisticated threats where malicious intent is concealed or diluted in complex tasks, creating a gap in comprehensive agent safety assessment.

Method: Developed OASIS (Orthogonal Agent Safety Inquiry Suite) - a hierarchical benchmark with fine-grained annotations and high-fidelity simulation sandbox to analyze agent safety brittleness under orthogonal pressures of intent concealment and task complexity.

Result: Two critical findings: 1) Safety alignment degrades sharply and predictably as intent becomes obscured, 2) A "Complexity Paradox" emerges where agents appear safer on harder tasks only due to capability limitations rather than actual safety improvements.

Conclusion: OASIS provides a principled foundation for probing and strengthening agent safety in overlooked dimensions of intent concealment and task complexity, addressing gaps in current safety evaluation approaches.
...
### [210] [SpeechJudge: Towards Human-Level Judgment for Speech Naturalness](https://arxiv.org/abs/2511.07931)
...
TL;DR: SpeechJudge is a comprehensive suite for aligning speech synthesis models with human feedback, featuring a large-scale dataset, benchmark, and generative reward model that achieves 77.2% accuracy in naturalness judgment.


<details>
  <summary>Details</summary>
Motivation: Address the lack of large-scale human preference datasets for speech synthesis, which hinders development of models that truly align with human perception of naturalness.

Method: Created SpeechJudge-Data (99K speech pairs), established SpeechJudge-Eval benchmark, and developed SpeechJudge-GRM - a generative reward model based on Qwen2.5-Omni-7B trained via two-stage post-training (SFT with Chain-of-Thought + RL with GRPO).

Result: SpeechJudge-GRM achieves 77.2% accuracy (79.4% after scaling) on the benchmark, outperforming leading models like Gemini-2.5-Flash (<70%) and classic Bradley-Terry reward model (72.7%).

Conclusion: SpeechJudge provides an effective framework for human-aligned speech synthesis, with the generative reward model showing superior performance and potential for improving speech generation models through preference alignment.
...
### [211] [Network and Systems Performance Characterization of MCP-Enabled LLM Agents](https://arxiv.org/abs/2511.07426)
*Zihao Ding,Mufeng Zhu,Yao Liu*

Main category: cs.DC

TL;DR: MCP enhances LLM capabilities but significantly increases token usage and costs; this paper analyzes the trade-offs between capability, performance, and cost in MCP-enabled interactions.


<details>
  <summary>Details</summary>
Motivation: MCP's extensive contextual information (system prompts, tool definitions, history) dramatically inflates token usage, escalating monetary costs and computational load on LLM services.

Method: Comprehensive measurement-based analysis of MCP-enabled interactions with LLMs, exploring different models and MCP configurations' impact on performance metrics.

Result: Reveals trade-offs between capability, performance, and cost; identifies how different LLM models and MCP configurations affect token efficiency, monetary cost, completion times, and success rates.

Conclusion: Suggests optimizations like parallel tool calls and task abort mechanisms; provides insights for developing more efficient, robust, and cost-effective MCP-enabled workflows.
...
### [212] [Intelligence per Watt: Measuring Intelligence Efficiency of Local AI](https://arxiv.org/abs/2511.07885)
...
TL;DR: Local language models (<=20B parameters) can now handle 88.7% of real-world queries efficiently on local accelerators, with intelligence-per-watt (IPW) improving 5.3x from 2023-2025, enabling viable redistribution of demand from centralized cloud infrastructure.


<details>
  <summary>Details</summary>
Motivation: Centralized cloud infrastructure struggles to scale with growing LLM demand, while small LMs now achieve competitive performance and local accelerators can run them at interactive latencies, raising the question of whether local inference can redistribute demand.

Method: Proposed intelligence per watt (IPW) as a metric and conducted large-scale empirical study across 20+ local LMs, 8 accelerators, and 1M real-world queries, measuring accuracy, energy, latency, and power.

Result: Local LMs accurately answer 88.7% of single-turn chat and reasoning queries; IPW improved 5.3x from 2023-2025; local query coverage rose from 23.2% to 71.3%; local accelerators achieve at least 1.4x lower IPW than cloud accelerators.

Conclusion: Local inference can meaningfully redistribute demand from centralized infrastructure, with IPW serving as the critical metric for tracking this transition.
...
### [213] [ViPRA: Video Prediction for Robot Actions](https://arxiv.org/abs/2511.07732)
*Sandeep Routray,Hengkai Pan,Unnat Jain,Shikhar Bahl,Deepak Pathak*

Main category: cs.RO

TL;DR: ViPRA is a framework that converts video prediction models into robot policies by learning latent actions from unlabeled videos and then mapping them to robot-specific continuous control using minimal teleoperated demonstrations.


<details>
  <summary>Details</summary>
Motivation: Videos contain rich physical interactions but lack action labels, limiting their use in robot learning. The goal is to leverage unlabeled videos to learn robot control without expensive action annotation.

Method: Train a video-language model to predict future observations and motion-centric latent actions using perceptual losses and optical flow consistency. Then use a chunked flow matching decoder to map latent actions to robot-specific continuous actions with only 100-200 teleoperated demos.

Result: Achieves 16% gain on SIMPLER benchmark and 13% improvement across real-world manipulation tasks. Enables smooth 22 Hz continuous control and generalization across embodiments.

Conclusion: ViPRA successfully bridges video prediction and robot control by learning physically grounded latent actions from unlabeled videos, enabling efficient policy learning with minimal labeled data.
...
### [214] [PerspAct: Enhancing LLM Situated Collaboration Skills through Perspective Taking and Active Vision](https://arxiv.org/abs/2511.08098)
...
TL;DR: This paper evaluates how incorporating diverse viewpoints using the ReAct framework can enhance LLMs' perspective-taking abilities in multi-agent interactions, showing significant improvements in interpretative accuracy through active visual exploration.


<details>
  <summary>Details</summary>
Motivation: Current LLM training neglects interactive contexts, creating challenges for reasoning about individual perspectives and multi-observer environments, limiting effective multi-agent collaboration.

Method: Extends the Director task with active visual exploration across seven progressively complex scenarios, using ReAct-style reasoning with varying state representations and prompting strategies to resolve referential ambiguity.
...
Conclusion: Integrating active perception with perspective-taking mechanisms advances LLMs' application in robotics and multi-agent systems, providing foundation for adaptive, context-aware AI systems.
...
TL;DR: RoboTAG is a novel method for robot pose estimation that incorporates 3D priors and enables unsupervised learning from unlabeled images by co-evolving 2D and 3D representations through a graph-based architecture.


<details>
  <summary>Details</summary>
Motivation: Existing robot pose estimation methods heavily rely on labeled data, suffer from sim-to-real gap, and reduce 3D problems to 2D domain while neglecting 3D priors.

Method: Proposes Robot Topological Alignment Graph (RoboTAG) with 3D and 2D branches, where nodes represent camera/robot states and edges capture dependencies/alignments. Uses closed loops for consistency supervision across branches.

Result: Experimental results show effectiveness across different robot types and demonstrate potential to alleviate data bottleneck in robotics.

Conclusion: RoboTAG successfully addresses limitations of existing methods by incorporating 3D priors and enabling unsupervised learning from unlabeled in-the-wild images.
...
### [216] [SONIC: Supersizing Motion Tracking for Natural Humanoid Whole-Body Control](https://arxiv.org/abs/2511.07820)
...
TL;DR: Scaling up model capacity, data, and compute for motion tracking creates a generalist humanoid controller that produces natural and robust whole-body movements, bridging the gap between motion tracking and downstream task execution.


<details>
  <summary>Details</summary>
Motivation: Current neural controllers for humanoids are limited in size, target few behaviors, and use modest compute. This work aims to demonstrate that scaling up model capacity, data, and compute can yield a generalist humanoid controller with natural and robust movements.

Method: Use motion tracking as a scalable task with dense supervision from diverse motion-capture data (100M frames, 700 hours). Scale along three axes: network size (1.2M to 42M parameters), dataset volume, and compute (9k GPU hours). Implement a real-time universal kinematic planner and unified token space for various motion inputs.

Result: Performance improves steadily with increased compute and data diversity. Learned representations generalize to unseen motions. The model enables natural and interactive control through a universal kinematic planner and supports various motion input interfaces using the same policy.

Conclusion: Motion tracking at scale serves as a practical foundation for humanoid control, exhibiting favorable scaling properties and generalization capabilities, establishing a path toward more capable and general humanoid controllers.
...
### [217] [Pruning as Regularization: Sensitivity-Aware One-Shot Pruning in ASR](https://arxiv.org/abs/2511.08092)
*Julian Irigoyen,Arthur Söhler,Andreas Søeborg Kirkedal*

Main category: eess.AS

TL;DR: Neural network pruning serves as implicit regularization for ASR, not just compression. Component-wise pruning reveals architectural asymmetries where removing redundancy in decoder self-attention and last encoder layers improves generalization without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To challenge the conventional view of pruning as solely a compression technique and demonstrate its role as a powerful implicit regularizer for Automatic Speech Recognition (ASR).

Method: Combined gradient- and Fisher-based sensitivity diagnostics with targeted, component-wise pruning on Whisper-small model. Focused on identifying architectural asymmetries and pruning specific components like decoder FFNs, self-attention, and encoder layers.

Result: Pruning 50% of decoder self-attention reduced WER by 2.38% absolute (20.44% relative) on LibriSpeech test-other. Pruning last four encoder layers at 50% yielded 1.72% absolute (14.8% relative) improvement. Gains persisted across Common Voice and TED-LIUM datasets. At 40% sparsity, method preserved near-baseline accuracy where global pruning approaches failed.

Conclusion: Pruning should be viewed as a first-class architectural design tool where knowing where to prune is as important as how much to prune. Sensitivity-aware pruning enables both regularization benefits and more aggressive compression while maintaining performance.
...
### [218] [Quantizing Whisper-small: How design choices affect ASR performance](https://arxiv.org/abs/2511.08093)
*Arthur Söhler,Julian Irigoyen,Andreas Søeborg Kirkedal*

Main category: eess.AS

TL;DR: Evaluation of post-training quantization methods for Whisper-small speech recognition model, showing dynamic int8 quantization reduces model size by 57% while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Large speech recognition models like Whisper-small have high computational demands that make deployment on edge devices challenging, requiring efficient quantization methods.

Method: Unified cross-library evaluation of PTQ using PyTorch, Optimum-Quanto, HQQ, and bitsandbytes, testing different quantization schemes, methods, granularities, and bit-widths on LibriSpeech datasets.

Result: Dynamic int8 quantization with Quanto achieved best trade-off: 57% model size reduction with improved word error rate. Static quantization performed worse, while aggressive formats (nf4, int3) achieved 71% compression but with accuracy loss in noisy conditions.

Conclusion: Carefully chosen PTQ methods can substantially reduce Whisper-small model size and inference cost without retraining, enabling efficient deployment on constrained hardware.
...
### [219] [Unifying Model and Layer Fusion for Speech Foundation Models](https://arxiv.org/abs/2511.08389)
*Yi-Jen Shih,David Harwath*

Main category: eess.AS

TL;DR: Proposes a unified interface module for fusing representations across multiple speech foundation models and their layers, outperforming prior fusion approaches on ASR and paralinguistic tasks.


<details>
  <summary>Details</summary>
Motivation: To improve downstream speech task performance by unifying two existing fusion strategies: multi-layer fusion within a single model and multi-model fusion, enabling more comprehensive representation integration.

Method: An interface module that enables fusion across multiple upstream speech models while integrating information across their layers, tested with various self-supervised and supervised models.

Result: Outperforms prior fusion approaches on ASR and paralinguistic analysis tasks, with performance improvements scaling with model size and count when appropriate upstream models are selected.

Conclusion: The proposed interface provides significant performance boosts when given suitable upstream model selection, making it a promising approach for utilizing speech foundation models effectively.
...
### [220] [LLaDA-Rec: Discrete Diffusion for Parallel Semantic ID Generation in Generative Recommendation](https://arxiv.org/abs/2511.06254)
*Teng Shi,Chenglei Shen,Weijie Yu,Shen Nie,Chongxuan Li,Xiao Zhang,Ming He,Yan Han,Jun Xu*

Main category: cs.IR

TL;DR: LLaDA-Rec is a discrete diffusion framework that reformulates recommendation as parallel semantic ID generation, addressing limitations of unidirectional attention and error accumulation in existing autoregressive models.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models for generative recommendation suffer from unidirectional constraints (causal attention restricts tokens to only attend to predecessors) and error accumulation (fixed left-to-right generation causes early errors to propagate).

Method: Proposes a discrete diffusion framework with: (1) parallel tokenization for semantic IDs, (2) dual masking mechanisms at user-history and next-item levels, and (3) adapted beam search for adaptive-order discrete diffusion decoding.

Result: Experiments on three real-world datasets show LLaDA-Rec consistently outperforms both ID-based and state-of-the-art generative recommenders.

Conclusion: Discrete diffusion establishes a new paradigm for generative recommendation, effectively modeling inter-item and intra-item dependencies while alleviating error accumulation.
...
### [221] [BiCA: Effective Biomedical Dense Retrieval with Citation-Aware Hard Negatives](https://arxiv.org/abs/2511.08029)
*Aarush Sinha,Pavan Kumar S,Roshan Balaji,Nirav Pravinbhai Bhatt*

Main category: cs.IR

TL;DR: BiCA uses citation links in PubMed articles to generate hard negatives for training biomedical dense retrieval models, achieving improved performance with minimal fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Hard-negative mining is challenging in biomedical domains due to difficulty distinguishing source documents from hard negatives. Citation links provide naturally relevant but non-duplicate documents ideal for hard negatives.

Method: Proposed BiCA approach that utilizes citation links from 20,000 PubMed articles to generate citation-aware hard negatives for fine-tuning GTE_small and GTE_Base dense retrieval models.

Result: Consistent improvements in zero-shot dense retrieval using nDCG@10 on BEIR benchmarks for both in-domain and out-of-domain tasks, and outperforming baselines on long-tailed topics in LoTTE using Success@5.

Conclusion: Citation links provide highly informative negatives for domain adaptation, enabling state-of-the-art performance with minimal fine-tuning and demonstrating data-efficient domain adaptation.
...
### [222] [A Hybrid Multimodal Deep Learning Framework for Intelligent Fashion Recommendation](https://arxiv.org/abs/2511.07573)
*Kamand Kalashi,Babak Teimourpour*

Main category: cs.IR

TL;DR: Hybrid multimodal deep learning framework using CLIP encoders for fashion recommendation, achieving 0.95 AUC on outfit compatibility and 69.24% FITB accuracy on complementary item retrieval.


<details>
  <summary>Details</summary>
Motivation: Address the growing demand for intelligent fashion recommender systems that can understand both visual and textual cues in online fashion platforms.

Method: Uses CLIP's visual and textual encoders to get joint latent representations, integrates them into unified feature vectors processed by transformer encoder, with special tokens for outfit compatibility and complementary item retrieval tasks.

Result: Achieved 0.95 AUC on Polyvore dataset for outfit compatibility prediction and 69.24% accuracy under FITB metric for complementary item retrieval.

Conclusion: The proposed multimodal learning approach demonstrates strong performance across both fashion recommendation tasks, highlighting the effectiveness of joint visual-textual representations.
...
### [223] [Hybrid Quantum-Classical Selective State Space Artificial Intelligence](https://arxiv.org/abs/2511.08349)
*Amin Ebrahimi,Farzan Haddadi*

Main category: quant-ph

TL;DR: A Hybrid Quantum Classical selection mechanism for Mamba architecture using Variational Quantum Circuits as quantum gating modules to enhance feature extraction and suppress irrelevant information in temporal sequence classification.


<details>
  <summary>Details</summary>
Motivation: To address computational bottlenecks in deep learning architectures, particularly in NLP where massive matrix multiplications and high-dimensional optimization cause prohibitive time complexity, by leveraging quantum resources for more efficient representation learning.

Method: Integration of Variational Quantum Circuits (VQCs) as quantum gating modules into the Mamba architecture for temporal sequence classification, analyzing impact on generalization, expressivity, and parameter efficiency.

Result: On reshaped MNIST dataset (batch, 784, d_model), the hybrid model achieved 24.6% accuracy in first 4 epochs using one quantum layer, compared to 21.6% for purely classical selection mechanism, demonstrating higher expressivity.

Conclusion: Quantum-enhanced gating mechanisms show potential as a path toward scalable, resource-efficient NLP models, though tested in limited simulation with no funding.
...
### [224] [CNN-Based Automated Parameter Extraction Framework for Modeling Memristive Devices](https://arxiv.org/abs/2511.07926)
*Akif Hamid,Orchi Hassan*

Main category: cs.ET

TL;DR: Automated framework using CNN and heuristic optimization to extract Stanford RRAM model parameters from I-V characteristics, achieving fast and accurate modeling.


<details>
  <summary>Details</summary>
Motivation: Existing RRAM compact models require extensive manual parameter tuning that is time-consuming and limits adaptability across different devices.

Method: Uses CNN trained on synthetic data for initial parameter estimates, then refines with three heuristic optimization blocks using adaptive binary search.

Result: Achieves low error across diverse device characteristics for key NVM metrics: set voltage, reset voltage, hysteresis loop area, and LRS slope.

Conclusion: Provides a fast, reliable, and robust solution for RRAM modeling that outperforms manual tuning and other analytical models.
...
### [225] [LLM-Powered Fully Automated Chaos Engineering: Towards Enabling Anyone to Build Resilient Software Systems at Low Cost](https://arxiv.org/abs/2511.07865)
*Daisuke Kikuta,Hiroki Ikeuchi,Kengo Tajiri*

Main category: cs.SE

TL;DR: ChaosEater automates the entire Chaos Engineering cycle using Large Language Models to build resilient Kubernetes systems at low cost, eliminating manual planning and improvement processes.


<details>
  <summary>Details</summary>
Motivation: Current Chaos Engineering tools automate experiment execution but leave planning and system improvement as manual, labor-intensive processes requiring multi-domain expertise. This creates barriers to building resilient systems.

Method: ChaosEater uses LLMs in an agentic workflow to automate the entire CE cycle for Kubernetes systems, including requirement definition, code generation, testing, and debugging through software engineering tasks.

Result: Evaluation shows ChaosEater consistently completes reasonable CE cycles with significantly low time and monetary costs, and its cycles are qualitatively validated by both human engineers and LLMs.

Conclusion: ChaosEater successfully automates the full Chaos Engineering lifecycle using LLMs, making it accessible for anyone to build resilient Kubernetes systems efficiently and cost-effectively.
...
### [226] [Quantifying the Impact of CU: A Systematic Literature Review](https://arxiv.org/abs/2511.07491)
*Thomas Compton*

Main category: cs.DL

TL;DR: Community Unionism (CU) is analyzed as both an empirical descriptor and normative ideal, revealing tensions between indigenous British practices and transnational social movement unionism, with its significance lying in managing contradictions within a shrinking labor movement.


<details>
  <summary>Details</summary>
Motivation: To understand why Community Unionism gained prominence despite unresolved theoretical coherence and political significance, by examining how it is constructed, cited, and contested in scholarly literature.

Method: Used two systematic approaches: citation network analysis of 114 documents and thematic review of 18 core CU case studies to examine CU's dual functions as empirical descriptor and normative ideal.

Result: Revealed CU's dual genealogy (British indigenous practices vs. transnational social movement unionism), near-universal emphasis on coalition-building with deep ambivalence toward class politics, and tensions between workplace/community, leadership/rank-and-file, and reform/radicalism.

Conclusion: CU's significance lies less in operationalizing a new union model and more in managing contradictions within a shrinking labor movement, serving as a concept that navigates tensions rather than resolving them.
...
### [227] [CC30k: A Citation Contexts Dataset for Reproducibility-Oriented Sentiment Analysis](https://arxiv.org/abs/2511.07790)
*Rochana R. Obadage,Sarah M. Rajtmajer,Jian Wu*

Main category: cs.DL

TL;DR: CC30k dataset with 30,734 citation contexts labeled for reproducibility-oriented sentiment (Positive/Negative/Neutral) to predict and study reproducibility in ML papers.


<details>
  <summary>Details</summary>
Motivation: Sentiments about reproducibility in citations offer community perspectives and signal actual reproducibility, but lack dedicated datasets for computational reproducibility studies.

Method: Created CC30k dataset through data cleansing, crowd labeling (25,829 samples), supplemented with generated negatives, achieving 94% labeling accuracy with robust validation.

Result: Fine-tuned LLMs show significant performance improvement on reproducibility-oriented sentiment classification using the CC30k dataset.

Conclusion: CC30k enables large-scale reproducibility assessments in ML and provides foundation for computational reproducibility studies.
...
### [228] [A Decentralized Retrieval Augmented Generation System with Source Reliabilities Secured on Blockchain](https://arxiv.org/abs/2511.07577)
*Yining Lu,Wenyi Tang,Max Johnson,Taeho Jung,Meng Jiang*

Main category: cs.CR

TL;DR: Decentralized RAG system with blockchain-based reliability scoring that improves performance by 10.7% over centralized systems in unreliable data environments while maintaining data owner control and privacy.


<details>
  <summary>Details</summary>
Motivation: Centralized RAG systems have high data collection costs and privacy concerns; need decentralized system where data owners maintain control while ensuring reliability across varying data sources.

Method: Novel reliability scoring mechanism dynamically evaluates data sources based on response quality, prioritized during retrieval; scoring managed through blockchain smart contracts for transparency and tamper-proof records.

Result: Achieved +10.7% performance improvement over centralized systems in unreliable data environments; approaches centralized system performance in ideal environments; 56% cost savings through batched updates.

Conclusion: Decentralized RAG with blockchain-based reliability scoring provides better performance in real-world unreliable data scenarios while ensuring data privacy, transparency, and cost efficiency.
...
### [229] [SALT: Steering Activations towards Leakage-free Thinking in Chain of Thought](https://arxiv.org/abs/2511.07772)
...
TL;DR: SALT is a lightweight test-time intervention that reduces privacy leakage in LLMs' reasoning processes by injecting steering vectors into hidden states, achieving significant CPL reductions while maintaining utility.


<details>
  <summary>Details</summary>
Motivation: LLMs used as personal assistants can leak private information through their internal reasoning processes even when final outputs appear safe, creating a need to balance privacy protection with reasoning capabilities.

Method: SALT identifies high-leakage layers and injects targeted steering vectors into hidden states during Chain of Thought reasoning to mitigate privacy leakage without compromising model performance.

Result: SALT achieved 18.2% CPL reduction on QwQ-32B, 17.9% on Llama-3.1-8B, and 31.2% on Deepseek in contextual privacy leakage tests while maintaining comparable task performance.

Conclusion: SALT provides a practical test-time approach for privacy protection in reasoning-capable LLMs, enabling safer deployment of LLM-based personal agents.
...
### [230] [LoopLLM: Transferable Energy-Latency Attacks in LLMs via Repetitive Generation](https://arxiv.org/abs/2511.07876)
*Xingyu Li,Xiaolei Liu,Cheng Liu,Yixiao Xu,Kangyi Ding,Bangzhou Xin,Jia-Li Yin*

Main category: cs.CR

TL;DR: LoopLLM is an energy-latency attack framework that exploits LLM vulnerabilities to induce repetitive generation loops, achieving near-maximum output length and high cross-model transferability.


<details>
  <summary>Details</summary>
Motivation: Existing energy-latency attacks struggle with controlling termination symbols as output grows longer, making them less effective against large language models.

Method: Uses repetition-inducing prompt optimization to trigger low-entropy decoding loops and token-aligned ensemble optimization to improve cross-model transferability through gradient aggregation.

Result: Achieves over 90% of maximum output length (vs 20% for baselines) and improves transferability by ~40% to models like DeepSeek-V3 and Gemini 2.5 Flash across 14 LLMs.

Conclusion: LoopLLM effectively exploits autoregressive vulnerabilities to create energy-latency attacks that significantly outperform existing methods in both output length and transferability.
...
### [231] [Class-feature Watermark: A Resilient Black-box Watermark Against Model Extraction Attacks](https://arxiv.org/abs/2511.07947)
*Yaxin Xiao,Qingqing Ye,Zi Liang,Haoyang Li,RongHua Li,Huadi Zheng,Haibo Hu*

Main category: cs.CR

TL;DR: The paper addresses vulnerabilities in current black-box model watermarks against sequential model extraction attacks (MEAs) and removal attacks. It proposes WRK attack that reduces watermark success rates by 88.79%, and introduces CFW watermarking method that maintains 70.15% success rate under combined attacks.


<details>
  <summary>Details</summary>
Motivation: Current model watermarking methods prioritize MEA survival through representation entanglement but inadequately address resilience against sequential MEAs and removal attacks. The risk is underestimated as existing removal methods are weakened by entanglement.

Method: Proposes WRK attack that circumvents entanglement by exploiting decision boundaries from sample-level watermark artifacts. Also introduces Class-Feature Watermarks (CFW) that uses synthetic class with out-of-domain samples to eliminate vulnerable decision boundaries and optimizes both MEA transferability and post-MEA stability.

Result: WRK effectively reduces watermark success rates by at least 88.79% across existing benchmarks. CFW maintains watermark success rate of at least 70.15% in extracted models under combined MEA and WRK distortion while preserving model utility.

Conclusion: CFW provides robust protection against model extraction and removal attacks by leveraging class-level artifacts and eliminating vulnerable decision boundaries, outperforming prior methods in resilience across multiple domains.
