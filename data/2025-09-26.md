...
Motivation: Public sentiment is critical in diplomacy but traditional sentiment analysis methods are time-consuming and lack predictive capabilities. There's a need for data-driven tools to help frame diplomatic communications more effectively.

Method: 1) Train language model to predict public reactions using diplomatic event dataset 2) Identify key textual features for modification based on communication theories and expert input 3) Develop counterfactual generation algorithm using LLM to systematically produce modified narratives while preserving core facts

Result: The framework successfully shifted public sentiment to more favorable states with a 70% success rate, demonstrating practical effectiveness in narrative modification.

Conclusion: This framework serves as a practical tool for diplomats and policymakers, offering data-driven insights for framing diplomatic communications to achieve more desirable public sentiment outcomes.
...
<details>
  <summary>Details</summary>
Motivation: Cross-lingual SER is challenging due to differences in phonetic variability and speaker-specific expressive styles across languages. Current methods struggle to effectively capture emotions under diverse conditions requiring better alignment of emotion externalization.

Method: Builds emotion-specific speaker communities via graph-based clustering to capture shared speaker traits. Applies dual-space anchoring in speaker and phonetic spaces to enable better emotion transfer across languages.
...
<details>
  <summary>Details</summary>
Motivation: LLMs show strong NLP performance but their utility in automating complex physical system experiments like CFD remains underexplored, despite CFD being a critical and labor-intensive component of computational science.

Method: The benchmark comprises three components: CFDQuery (CFD knowledge), CFDCodeBench (numerical/physical reasoning), and FoamBench (context-dependent workflow implementation), using a rigorous evaluation framework grounded in real-world CFD practices.

Result: CFDLLMBench establishes a reproducible foundation for quantifying LLM performance across code executability, solution accuracy, and numerical convergence behavior in CFD tasks.

Conclusion: This benchmark provides a solid foundation for developing and evaluating LLM-driven automation of numerical experiments for complex physical systems, with code and data publicly available.
...
<details>
  <summary>Details</summary>
Motivation: The rapid adoption of LLMs like ChatGPT has blurred lines between human and AI-generated texts, creating urgent needs for reliable detection to maintain academic integrity, protect intellectual property, and prevent misinformation spread.

Method: Tested classical (Logistic Regression with Bag-of-Words, POS, TF-IDF) and transformer-based (BERT with N-grams, DistilBERT, BERT with custom classifier, LSTM-based N-gram) ML techniques on 250 pairs of ChatGPT-3.5 and human-written research abstracts across various topics. Also evaluated ensemble approaches.

Result: DistilBERT achieved the best overall performance. Logistic Regression and BERT-Custom offered solid alternatives, while LSTM- and BERT-N-gram approaches performed poorly. Ensemble of top three models failed to surpass DistilBERT alone.

Conclusion: Single transformer-based representations (like DistilBERT) outperform model diversity through ensembles. The study provides foundation for developing more robust detection frameworks with larger datasets to keep pace with improving generative AI models.
...
<details>
  <summary>Details</summary>
Motivation: SAE features don't inherently align with human concepts, making LLM knowledge representation interpretation cumbersome and labor-intensive.

Method: ConceptViz implements a novel pipeline: identification (querying SAEs using concepts), interpretation (exploring concept-to-feature alignments), and validation (model behavior verification).

Result: The system was demonstrated through usage scenarios and user study, showing enhanced interpretability research by streamlining concept discovery and validation.

Conclusion: ConceptViz helps researchers build more accurate mental models of LLM features and aids in understanding LLM knowledge representations.
...
<details>
  <summary>Details</summary>
Motivation: Retrieval systems in RAG may return irrelevant content that causes hallucinations. Understanding what the model "knows" and "doesn't know" is crucial for better integrating internal and external knowledge.

Method: Proposes SKILL-RAG using reinforcement learning to elicit self-knowledge from models, employing sentence-level granularity filtering to preserve useful knowledge while removing irrelevant content.

Result: Evaluation on Llama2-7B and Qwen3-8B shows SKILL-RAG improves generation quality and significantly reduces the number of input documents needed.

Conclusion: Self-knowledge is essential for guiding high-quality retrieval selection in RAG systems, and SKILL-RAG effectively leverages this to enhance performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing E-TTS systems rely on sentence-level control through predefined labels, reference audio, or natural language prompts, which fail to capture dynamic emotional shifts within sentences.

Method: Aligns frame-level features from emotion2vec to words for word-level emotion annotations, then maps them through a FiLM layer to modulate text embeddings for word-level emotion control. Constructed FEDD dataset for evaluation.
...
Conclusion: Emo-FiLM successfully enables fine-grained emotion control in TTS, capturing dynamic emotional transitions within sentences that previous methods could not achieve.
...
Method: The framework includes: 1) LLM-based Preference Optimization dataset construction for RL training to help LLMs understand conversational recommendation strategies, and 2) Self-Enhancement Strategy during inference to further exploit conversational recommendation potential from RL training.

Result: Extensive experiments on various datasets show that USB-Rec consistently outperforms previous state-of-the-art methods in conversational recommendation tasks.

Conclusion: The proposed USB-Rec framework effectively addresses the training gap in LLM-based conversational recommender systems and demonstrates superior performance through integrated training-inference optimization.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based summarization systems lack reliable guarantees for including critical content in high-stakes domains like healthcare, law, and finance.
...
<details>
  <summary>Details</summary>
Motivation: Short-form video platforms present unique challenges for misinformation detection due to multimodal, dynamic, and noisy content that requires specialized tools beyond traditional text-based approaches.
...
Conclusion: ShortCheck provides an effective automated solution for identifying checkworthy content in short-form videos, demonstrating strong performance in multilingual contexts and offering practical assistance to human fact-checkers.
...
<details>
  <summary>Details</summary>
Motivation: Multi-Agent Debate (MAD) improves LLM reasoning but introduces substantial computational overhead due to frequent agent interactions. MARS aims to achieve similar reasoning quality with reduced computational costs.

Method: MARS uses a review-inspired framework with three roles: author agent generates initial solution, reviewer agents provide independent feedback, and meta-reviewer integrates feedback for final decision and revision guidance.
...
Conclusion: The role-based collaboration framework effectively enhances reasoning quality while controlling computational costs, making multi-agent reasoning more practical.
...
Method: The dataset builds on existing parallel corpora and involves rigorous annotation by native speakers, with analyses on inter-annotator agreement, iterative feedback, and error patterns.

Result: The dataset provides error span, error type, and error severity annotations for machine-translated examples, serving as a resource for fine-tuning models with error detection capabilities.

Conclusion: SiniticMTError supports research on translation quality estimation, error-aware generation, and low-resource language evaluation for the MT community.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of disease diagnosis in multilingual healthcare settings where annotated medical data is scarce for low-resource languages and linguistic variability exists across populations.

Method: Leverages XLM-RoBERTa encoder with language-aware attention, Siamese contrastive learning for semantic alignment, translation consistency module, and Model-Agnostic Meta-Learning (MAML) for rapid adaptation. Uses multi-task learning combining disease classification, translation alignment, and contrastive learning objectives.

Result: Achieves 97.22% accuracy and 97.17% F1-score in supervised settings. In zero-shot scenarios, attains 92.78% accuracy on Hindi and 73.33% accuracy on Bengali medical text, demonstrating strong generalization to low-resource languages.

Conclusion: SwasthLLM provides an effective unified framework for cross-lingual medical diagnosis that performs well in both supervised and zero-shot settings, showing promise for healthcare applications in multilingual and low-resource environments.
...
Method: Depth Specialised Mixture of Experts (DS-MoE) with expert modules for different reasoning depths and a learned routing network that dynamically activates necessary experts. Trained on The Pile dataset.

Result: 16% computational savings, 35% faster inference, 2.8% higher accuracy on complex reasoning benchmarks, with interpretable reasoning chains.

Conclusion: DS-MoE demonstrates that depth-specialized modular processing simultaneously improves efficiency, reasoning quality, and interpretability in large-scale language models.
...
Method: HRT constructs multi-resolution attention with exponential sequence reduction across scales, enabling bottom-up composition and top-down contextualization through wavelet-inspired hierarchical processing.
...
<details>
  <summary>Details</summary>
Motivation: Autoregressive models are serial and slow for long sequences, while diffusion models require hundreds of steps for high quality. There's a need for faster language generation without sacrificing quality.

Method: Introduces discrete flow-matching with explicit step parameterization, reliable update rules to prevent overshooting, and strong teacher guidance distilled from long trajectories. The model learns to make big moves equivalent to many small steps.

Result: FS-DFM with only 8 sampling steps achieves perplexity parity with 1,024-step discrete-flow baseline on 1,024-token generation, delivering 128× faster sampling and corresponding latency/throughput improvements.

Conclusion: The approach successfully bridges the gap between serial autoregressive models and iterative diffusion models, providing a practical solution for efficient high-quality language generation with minimal sampling steps.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the evaluation bottleneck in large language model development by enabling performance forecasting before running experiments, supporting smarter experiment prioritization.
...
Result: Achieved mean absolute error as low as 8.7 on Accuracy subset at high-confidence thresholds; stronger models showed diverse iterative querying while open-source models lagged; GPT-5 with web search maintained prediction accuracy in zero-leakage settings.

Conclusion: The approach offers initial steps toward anticipatory evaluation, supporting difficulty estimation and experiment prioritization in language model development.
...
<details>
  <summary>Details</summary>
Motivation: Japanese lacks sufficient training data for accurate phonemic transcriptions with accent marks, despite being resource-rich in general. There's a need for specialized speech recognizers for Japanese speaking assessment tasks.

Method: Two main methods: 1) Multitask training with auxiliary loss functions for orthographic text and pitch patterns to leverage utterances with only orthographic annotations; 2) Fusion of phonetic alphabet and text token sequence estimators using finite-state transducer framework.
...
Conclusion: Multitask learning and fusion techniques are effective for building accurate Japanese phonemic recognizers with accent markers, successfully addressing data sparsity issues in specialized speaking assessment tasks.
...
Method: Proposes a framework that prompts LLMs to generate domain knowledge and executable code for molecular vectorization, then fuses these knowledge-based features with structural representations from pre-trained models using GPT-4o, GPT-4.1, and DeepSeek-R1.
...
Conclusion: The combination of LLM-extracted knowledge and structural features creates an effective framework for molecular property prediction, addressing limitations of individual approaches.
...
<details>
  <summary>Details</summary>
Motivation: To explore the reliability of attack detection models in NLP systems, which have not been thoroughly tested despite being used as additional checks for human input.

Method: Proposes RedHerring attack that modifies text to cause detection models to predict attacks while maintaining correct classifier predictions. Tests on 4 datasets against 3 detectors defending 4 classifiers.

Result: RedHerring drops detection accuracy by 20-71 points while maintaining or improving classifier accuracy. A simple confidence check defense is proposed that increases detection accuracy without retraining.

Conclusion: This novel threat model reveals vulnerabilities in attack detection systems and provides new insights into how adversaries can target detection models.
...
<details>
  <summary>Details</summary>
Motivation: Transformer-based NLP models have high computational costs for attack testing, and existing black-box attack methods require too many queries, making them inefficient for researchers with limited resources.

Method: Hybrid Select merges BinarySelect and GreedySelect using a size threshold, while Dynamic Select learns which selection method to apply to different text lengths. Both aim to reduce query numbers while preserving attack success.

Result: Across 4 datasets and 6 target models, the sentence-level Hybrid Select reduced required queries by up to 25.82% on average against both encoder models and LLMs without losing attack effectiveness.

Conclusion: The proposed selection strategies successfully address the computational efficiency problem in adversarial text attacks, making attack testing more accessible to researchers with limited resources.
...
<details>
  <summary>Details</summary>
Motivation: Real-world SER deployments often fail under domain mismatch where source data is unavailable and powerful LALMs are only accessible via API. The goal is to adapt student models to outperform LALMs in target domains without sharing source data.

Method: MI-Fuse uses a denoised label fusion framework that supplements the LALM with a source-domain trained SER classifier as an auxiliary teacher. It draws multiple stochastic predictions from both teachers, weights their mean distributions by mutual-information-based uncertainty, and stabilizes training with an exponential moving average teacher.
...
TL;DR: The paper identifies probability distribution collapse as the core issue in unsupervised neural grammar induction and proposes collapse-relaxing neural parameterization to improve parsing performance with more compact grammars.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised neural grammar induction models face an expressiveness bottleneck, resulting in unnecessarily large yet underperforming grammars due to probability distribution collapse.

Method: The authors analyze when and how probability distribution collapse emerges in neural parameterization components and introduce collapse-relaxing neural parameterization as a targeted solution.

Result: The approach substantially improves parsing performance while enabling the use of significantly more compact grammars across a wide range of languages.

Conclusion: Targeted mitigation of probability distribution collapse through specialized neural parameterization can overcome expressiveness bottlenecks in unsupervised grammar induction.
...
<details>
  <summary>Details</summary>
Motivation: To enhance QA model performance without requiring additional training, by leveraging the model's own confidence scores through systematic sub-question decomposition and refinement.

Method: Confidence-guided Refinement Reasoning (C2R) curates subsets of sub-QAs to explore diverse reasoning paths, compares confidence scores of answer candidates, and selects the most reliable final answer based on model-derived confidence metrics.

Result: C2R demonstrates consistent performance improvements across diverse QA models and benchmarks, can be seamlessly integrated with various existing QA models, and provides insights into how sub-QA quantity and quality affect reasoning reliability.

Conclusion: C2R offers an effective training-free approach for enhancing QA performance across multiple domains while providing valuable insights into sub-question decomposition strategies for robust reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To address the common belief that Supervised Fine-Tuning (SFT) degrades LLMs' general capabilities when adapting to specialized domains, and to find effective strategies to minimize this trade-off.

Method: Empirical evaluation of SFT with varying learning rates, theoretical analysis, and comparison of multiple strategies including L2 regularization, LoRA, model averaging, FLOW, and proposed Token-Adaptive Loss Reweighting (TALR).

Result: Smaller learning rates substantially mitigate general performance degradation. TALR consistently outperforms other methods in balancing domain-specific gains and general capabilities, though no method completely eliminates the trade-off.

Conclusion: Practical guidelines: (i) use small learning rates for favorable trade-off, (ii) adopt TALR when stronger balance between domain adaptation and general capabilities is desired.
...
<details>
  <summary>Details</summary>
Motivation: Current understanding of LLM internal representations is limited by undefined fundamental units. Neurons suffer from polysemy while features face reconstruction and stability issues, necessitating a more robust theoretical framework.

Method: Proposes Atoms Theory with atomic inner product (AIP) to define atoms as fundamental units. Proves RIP conditions for stable sparse representations and uniqueness guarantees. Validates theory by training threshold-activated sparse autoencoders (SAEs) on Gemma2-2B, Gemma2-9B, and Llama3.1-8B models.

Result: SAEs achieved 99.9% sparse reconstruction across layers on average. Over 99.8% of atoms satisfied uniqueness condition vs 0.5% for neurons and 68.2% for features, showing atoms better capture intrinsic LLM representations. Scaling experiments revealed SAE size-recovery capacity relationship.

Conclusion: Atoms Theory provides a systematic theoretical framework for understanding LLM internal representations, offering more faithful capture of intrinsic representations than neurons or features, and establishing foundation for mechanistic interpretability.
...
TL;DR: Proposes Conversational Prompting for personalized review generation under few-shot, training-free constraints, with two variants (SCP and CCP) that reformulate reviews as conversations to better capture user style.


<details>
  <summary>Details</summary>
Motivation: Real-world applications often face few-shot scenarios where only limited user reviews are available and fine-tuning is infeasible, requiring lightweight methods that leverage LLMs effectively without additional training.

Method: Conversational Prompting reformulates user reviews as multi-turn conversations. SCP uses only the user's own reviews, while CCP inserts incorrect replies from other users/LLMs and asks the model to correct them, encouraging production of text in the user's style.

Result: Experiments across 8 domains and 5 LLMs showed conversational prompting (SCP/CCP) produced reviews much closer to target users than conventional prompts, even with only 2 reviews per user. CCP improves with quality negative examples, while SCP remains competitive when such data is unavailable.

Conclusion: Conversational prompting offers a practical solution for review generation under few-shot and training-free constraints, effectively leveraging LLMs without requiring additional model training.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with hallucinations and factual errors in knowledge-intensive tasks like KGQA due to the semantic gap between structured knowledge graphs and unstructured queries, which existing methods overlook.

Method: Proposes EoG framework that leverages LLMs' prior knowledge to enrich KGs, enabling efficient evidence extraction for precise reasoning while maintaining low computational costs and scalability. Also introduces three graph quality evaluation metrics.
...
Conclusion: EoG successfully bridges the semantic gap between graphs and queries, providing a flexible, scalable solution for improving KGQA performance while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Small Language Models (sLMs) have high precision but low recall in grammatical error correction, while Large Language Models (LLMs) have high recall but low precision due to overcorrection. PoCO aims to balance both aspects by leveraging LLMs' comprehensive revision capability while maintaining sLMs' reliability.

Method: PoCO first uses LLMs to intentionally trigger overcorrection to maximize recall, then applies a targeted post-correction step using fine-tuned smaller models to identify and refine erroneous outputs from the LLM phase.

Result: Extensive experiments show that PoCO effectively balances GEC performance by increasing recall while maintaining competitive precision, ultimately improving overall grammatical error correction quality.

Conclusion: PoCO successfully harmonizes the generative power of LLMs with the reliability of supervised smaller models, providing an effective solution to the recall-precision trade-off in grammatical error correction.
...
<details>
  <summary>Details</summary>
Motivation: Large language models enable effective in-context learning with many-shot examples, but this comes with high computational costs due to longer input tokens.

Method: Propose cheat-sheet ICL, which distills information from many-shot ICL into a concise textual summary (cheat sheet) used as context at inference time.

Result: Experiments on challenging reasoning tasks show cheat-sheet ICL achieves comparable or better performance than many-shot ICL with far fewer tokens, and matches retrieval-based ICL without requiring test-time retrieval.

Conclusion: Cheat-sheet ICL is a practical alternative for leveraging LLMs in downstream tasks by reducing computational demands while maintaining effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: Address privacy concerns in LLM-based cloud services where user inputs may expose sensitive information, and overcome limitations of existing anonymization techniques that struggle with text naturalness and utility.
...
Conclusion: The proposed method provides an effective solution for privacy-preserving text processing in LLM applications, maintaining coherence, relevance, and naturalness while protecting sensitive information.
...
Method: Developed annotation guidelines for sub-sentence citations, constructed a dataset, and proposed an attribution framework using LLMs to generate fine-tuning data with a credit model for quality filtering.
...
Conclusion: Sub-sentence citations provide more concise and sufficient attribution, reducing user effort to verify generated outputs in RAG question answering systems.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion models show strong potential in language modeling with faster generation, but applying supervised fine-tuning is challenging due to lack of precise probability estimates and inconsistent generation processes. Controlling key tokens that guide generation direction is crucial.

Method: Proposed WeFT, a weighted SFT method where tokens are assigned different weights based on their entropy, derived from diffusion theory.
...
Conclusion: WeFT effectively addresses the challenges of SFT for diffusion language models and delivers substantial performance gains on reasoning tasks.
...
Method: The study investigates two approaches: prompting and fine-tuning (supervised fine-tuning and reinforcement fine-tuning). It proposes new reward functions for ranked-list answer formats and conducts ablation studies for reinforcement fine-tuning.

Result: Results show that while some supervised fine-tuning models generalize to certain answer formats, models trained with reinforcement fine-tuning are more robust across multiple formats. Models can recognize valid answers even when failing to select benchmark's preferred ground truth.

Conclusion: This is the first systematic investigation of approaches for enabling medical reasoning models to generate answers as ranked lists, providing a first step toward developing alternative answer formats beneficial beyond single answers in medical domains.
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with long document summarization, facing challenges like information loss, factual inconsistencies, and coherence problems when processing excessively long documents.

Method: SummQ employs an adversarial multi-agent framework with summary generators/reviewers and quiz generators/reviewers working collaboratively. An examinee agent validates if summaries contain information needed to answer quiz questions, enabling iterative refinement through multifaceted feedback.
...
Conclusion: The work establishes a new approach for long document summarization using adversarial agentic collaboration to improve summarization quality, with comprehensive analyses revealing the effectiveness of multi-agent collaboration dynamics and quizzing mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Existing detection methods relying on lexical overlap and perplexity have low generalization and perform poorly on implicitly contaminated data.

Method: Analyze probability trajectories of numeric tokens during generation, observing shortcut behaviors in contaminated samples vs gradual accumulation in clean samples. Validate through LoRA fine-tuning with injected samples.

Result: Contaminated and clean samples exhibit distinct, well-separated reasoning trajectories. Injected samples show same patterns as naturally contaminated data.

Conclusion: MemLens captures genuine memorization signals rather than spurious correlations, providing a more robust detection method.
...
<details>
  <summary>Details</summary>
Motivation: To reconcile linear and hierarchical perspectives on locality in sentence comprehension by developing a structurally grounded measure that refines linear distance, building on dependency length minimization theories.

Method: Using harmonized dependency treebanks and mixed-effects modeling across multiple languages, the study evaluates sentence length, dependency length, and Intervener Complexity as predictors of memory load, operationalized as the sum of feature misbinding and interference.

Result: All three factors are positively associated with memory load, with sentence length having the broadest influence and Intervener Complexity providing explanatory power beyond linear distance alone.
...
<details>
  <summary>Details</summary>
Motivation: Current tool calling research is predominantly English-centric, creating a gap in understanding how to enable this functionality for other languages like Arabic. The paper aims to address this resource gap and provide insights for developing Arabic tool-augmented agents.

Method: The researchers conducted extensive experiments using base and post-trained variants of an open-weight Arabic LLM. They translated and adapted two open-source tool-calling datasets into Arabic to enable the study. The experiments focused on three research questions: necessity of Arabic-specific data vs cross-lingual transfer, effect of general-purpose instruction tuning, and value of fine-tuning on high-priority tools.

Result: The study provides crucial insights into optimal strategies for developing robust tool-augmented agents for Arabic, though specific quantitative results are not detailed in the abstract.

Conclusion: The research bridges the resource gap for Arabic tool calling and offers guidance on effective approaches for enabling tool interaction capabilities in Arabic LLMs, addressing the current English-centric bias in this research area.
...
<details>
  <summary>Details</summary>
Motivation: To explore how large language models can be used as automatic evaluators for academic text-input problems in education, building on existing research about LLMs as educational assistants and evaluators.

Method: Proposed five evaluation systems (JudgeLM, Reference Aided, No Reference, Additive, and Adaptive Evaluation) tested on a custom dataset of 110 computer science answers from higher education students using three LLM models, with comparison to human evaluation.

Result: Reference Aided Evaluation achieved the best performance with lowest median absolute deviation (0.945) and root mean square deviation (1.214) compared to human evaluation, providing fair scoring and insightful evaluations.

Conclusion: AI-driven automatic evaluation systems with proper methodologies show potential as complementary tools to academic resources, with Reference Aided Evaluation being the most effective method for evaluating text-input problems.
...
<details>
  <summary>Details</summary>
Motivation: FFRDCs face text-heavy workloads that are slow to analyze manually, requiring efficient methods for processing policy documents and scientific papers while maintaining security and data sovereignty.

Method: The authors apply OnPrem.LLM, an open-source framework for secure generative AI application, using few-shot learning with input-output examples for summarization, classification, extraction, and sense-making tasks.

Result: Case studies on National Defense Authorization Act (NDAA) documents and National Science Foundation (NSF) Awards show enhanced oversight and strategic analysis capabilities while maintaining auditability and data sovereignty.

Conclusion: The approach successfully demonstrates how LLMs can accelerate text analysis in sensitive government contexts while addressing security and sovereignty concerns through secure on-premise deployment.
...
<details>
  <summary>Details</summary>
Motivation: To investigate and prove that the causal mask itself (not just explicit positional encodings) serves as a significant source of positional information in Transformer decoders, and to understand how it interacts with explicit positional encodings like RoPE.

Method: Theoretical analysis of how causal masks induce position-dependent patterns in attention scores, followed by empirical analysis of trained models to confirm these patterns and examine their interaction with RoPE positional encodings.

Result: The causal mask induces attention patterns that favor nearby query-key pairs, and when combined with RoPE, it distorts RoPE's relative attention score patterns into non-relative ones. This effect is consistently observed in modern large language models.

Conclusion: The causal mask should be considered as an important source of positional information alongside explicit positional encodings, as it significantly influences attention patterns and interacts with other positional encoding mechanisms.
...
Method: Created two benchmarks: ManyIFEval for text generation (up to 10 instructions) and StyleMBPP for code generation (up to 6 instructions). Developed three regression models to estimate performance on unseen instruction combinations and different instruction counts.

Result: Performance consistently degrades as instruction count increases across ten LLMs. A logistic regression model using instruction count can predict performance with ~10% error on unseen combinations. Modest sample sizes (500 for ManyIFEval, 300 for StyleMBPP) are sufficient for estimation.

Conclusion: The proposed benchmarks and regression models enable efficient evaluation of LLMs' multi-instruction following capabilities without exhaustive testing, providing practical tools for assessing real-world performance.
...
<details>
  <summary>Details</summary>
Motivation: To assess foundation models' capabilities on complex multimodal engineering problems, which remain largely unexplored despite their success in other domains.

Method: Created SoM-1K dataset with 1,065 annotated strength of materials problems containing text and diagrams, introduced DoI (Descriptions of Images) prompting strategy using expert-generated text descriptions, and evaluated 8 foundation models including LLMs and VLMs.

Result: Current foundation models perform poorly on engineering problems, with best model achieving only 56.6% accuracy. LLMs with DoI often outperform VLMs with visual diagrams. DoI effectively mitigates visual misinterpretation errors.

Conclusion: The work establishes a rigorous engineering AI benchmark and highlights the need for more robust multimodal reasoning capabilities in foundation models, especially for scientific and engineering applications.
...
<details>
  <summary>Details</summary>
Motivation: LLMs risk perpetuating subtle fairness issues tied to culture, positioning their generations from mainstream US perspectives while demonstrating externality towards non-mainstream cultures, which the authors identify as a novel culture positioning bias.

Method: Proposed CultureLens benchmark with 4000 prompts and 3 metrics for evaluating bias through culturally situated interview script generation. Developed 2 mitigation methods: Fairness Intervention Pillars (FIP) and Mitigation via Fairness Agents (MFA) with single-agent and multi-agent pipelines.

Result: Evaluation on 5 state-of-the-art LLMs revealed models adopt insider tones in over 88% of US-contexted scripts but disproportionately use outsider stances for less dominant cultures. Agent-based mitigation methods showed effectiveness in reducing bias.

Conclusion: Agent-based methods represent a promising direction for mitigating cultural biases in generative LLMs, with the proposed frameworks effectively addressing the identified culture positioning bias.
...
<details>
  <summary>Details</summary>
Motivation: Hallucination is a persistent issue in LLMs, particularly for low-resource languages like Persian, which lack dedicated evaluation benchmarks.
...
Result: Evaluation of 12 LLMs showed models struggle with Persian hallucination detection; external knowledge partially mitigates hallucination; no significant difference between Persian-specific LLMs and others.

Conclusion: PerHalluEval provides an effective benchmark for Persian hallucination evaluation, revealing LLMs' limitations in handling Persian text and the need for improved hallucination detection methods.
...
<details>
  <summary>Details</summary>
Motivation: Current search-augmented LLMs are insufficient for addressing diverse user needs, as they fail to recognize how the same query can reflect different intents across users and deliver information in preferred forms. While systems like ChatGPT and Gemini attempt personalization using user histories, systematic evaluation of such personalization is under-explored.

Method: BESPOKE is constructed through long-term, deeply engaged human annotation where human annotators contribute their own chat and search histories, author queries with detailed information needs, and evaluate responses with fine-grained preference scores and diagnostic feedback. The benchmark is designed to be both realistic (using authentic human data) and diagnostic (providing detailed preference metrics).

Result: The paper presents BESPOKE as a comprehensive benchmark that enables systematic analysis of personalization requirements in information-seeking tasks. The benchmark provides fine-grained evaluation capabilities for personalized search-augmented LLMs.

Conclusion: BESPOKE provides a foundation for fine-grained evaluation of personalized search-augmented LLMs, revealing key requirements for effective personalization in information-seeking tasks and addressing the current gap in systematic evaluation methodologies.
...
Method: Converted every BBQ context into controlled voice conditions, enabling evaluation of per-axis accuracy, bias, and consistency scores comparable to the original text benchmark.

Result: Evaluation of LLaMA-Omni and Qwen2-Audio revealed architectural contrasts: LLaMA-Omni resists acoustic bias but amplifies gender and accent bias, while Qwen2-Audio dampens these cues while preserving content fidelity.
...
<details>
  <summary>Details</summary>
Motivation: To systematically analyze acoustic-based gender differentiation in SpeechLMs where identical questions lead to different responses based on speaker's gender, despite these models transforming human-AI interaction through voice-based communication.

Method: Created a new dataset with 9,208 speech samples across three categories (Gender-Independent, Gender-Stereotypical, Gender-Dependent) and evaluated LLaMA-Omni series models. Conducted experiments with neutral response options, gender neutralization methods, and comparisons between SpeechLMs and their backbone LLMs.

Result: Models consistently exhibited male-oriented responses in Gender-Stereotypical questions but gender-independent responses in Gender-Dependent questions where gender differentiation would be appropriate. The pattern persists with neutral options and gender neutralization, and primarily stems from Whisper speech encoders generating male-oriented acoustic tokens.

Conclusion: Current SpeechLMs fail to properly remove gender biases, prioritizing general fairness principles over contextual appropriateness. This highlights the need for more sophisticated techniques to utilize gender information appropriately in speech technology.
...
Conclusion: AutoIntent provides an effective automated solution for text classification tasks with better performance than current alternatives and flexible resource management options.
...
<details>
  <summary>Details</summary>
Motivation: Traditional multimodal relation extraction approaches use classification-based paradigms that overlook structural constraints (entity types, positional cues) and lack semantic expressiveness for fine-grained relation understanding.
...
Result: The method achieves state-of-the-art performance on benchmark datasets MNRE and MORE, and exhibits stronger robustness and interpretability compared to previous approaches.

Conclusion: Retrieval-based paradigm using semantic similarity is more effective than classification-based approaches for multimodal relation extraction, providing better structural constraint handling and semantic expressiveness.
...
<details>
  <summary>Details</summary>
Motivation: To understand how syntactic templates in training data create unintended correlations that affect LLM performance and safety, particularly when syntax overrides semantic understanding.

Method: Used synthetic training datasets to test syntactic-domain correlations, developed an evaluation framework to detect this phenomenon, and conducted case studies on safety finetuning bypasses in various models including OLMo-2, Llama-4-Maverick, and GPT-4o.

Result: Found that syntactic-domain correlations significantly lower performance on entity knowledge tasks (mean 0.51 +/- 0.06) and can be used to bypass refusal mechanisms in safety-finetuned models.

Conclusion: There's a critical need to explicitly test for syntactic-domain correlations and ensure syntactic diversity within training domains to prevent spurious correlations that compromise model reliability and safety.
...
<details>
  <summary>Details</summary>
Motivation: Humor is a fundamental human trait that requires extensive reasoning and common-sense knowledge, making it an important benchmark for assessing LLMs' capabilities. However, computational humor processing remains underdeveloped beyond simple puns.

Method: The authors conduct a comprehensive literature survey of computational humor research, particularly focusing on generative tasks of humor creation and explanation, and analyze the current state-of-the-art approaches.

Result: The survey reveals that despite humor understanding being a foundational NLP task, work on generating and explaining complex humor remains sparse, with current models significantly lagging behind human capabilities.

Conclusion: The paper emphasizes the importance of computational humor as a key NLP subdiscipline and provides extensive discussion of future research directions, highlighting the subjective and ethically ambiguous nature of humor that must be considered.
...
Method: The authors first fine-tuned ChatBioGPT based on BioGPT using medical datasets. They then developed GEP (Greedy Coordinate Gradient-based method) specifically designed for PII extraction from SLMs, testing it against template-based methods and in free-style insertion scenarios.

Result: GEP demonstrated up to 60× more PII leakage compared to previous template-based methods. Even in complex free-style insertion scenarios with various syntactic expressions, GEP achieved a PII leakage rate of up to 4.53%.

Conclusion: SLMs are vulnerable to PII leakage attacks, and the proposed GEP method effectively exposes these vulnerabilities, highlighting the need for better privacy protection measures in small language model deployments, particularly in sensitive applications like healthcare.
...
<details>
  <summary>Details</summary>
Motivation: To overcome two major bottlenecks in LLM scientific reasoning: explicit retrieval's 'tool tax' of extra tokens/steps, and multi-agent pipelines diluting strong solutions through averaging across candidates.

Method: Monitor-based retrieval module operating at token level for minimal disruption, Hierarchical Solution Refinement (HSR) where candidates repair each other, and Quality-Aware Iterative Reasoning (QAIR) adapting refinement to solution quality.

Result: Achieved 48.3% accuracy on Humanity's Last Exam Bio/Chem Gold (highest reported), surpassing strongest agent baseline by 13.4 points, reducing token usage by 53.5% and agent steps by 43.7%. Robust performance on SuperGPQA and TRQA.

Conclusion: Implicit augmentation and structured refinement overcome inefficiencies of explicit tool use and uniform aggregation. Reasoning failures and knowledge gaps co-occur in 85%+ cases, with retrieval tasks benefiting from solution variety while reasoning tasks favor consensus.
...
TL;DR: CLaw is a benchmark for evaluating LLMs on Chinese legal knowledge, comprising a comprehensive statute corpus and case-based reasoning tasks, revealing significant deficiencies in LLMs' ability to accurately reproduce legal provisions.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used for legal analysis but lack specialized legal knowledge due to general pre-training, compromising their reliability in legal reasoning tasks.

Method: Developed CLaw benchmark with two components: (1) fine-grained corpus of 306 Chinese national statutes segmented to subparagraph level with historical revisions (64,849 entries), and (2) 254 case-based reasoning instances from China Supreme Court materials.

Result: Most contemporary LLMs significantly struggle to faithfully reproduce legal provisions, undermining their reliability in legal reasoning due to poor knowledge retrieval capabilities.

Conclusion: Trustworthy legal reasoning in LLMs requires robust synergy of accurate knowledge retrieval (enhanced through SFT or RAG) and strong general reasoning capabilities; CLaw provides essential benchmark for advancing domain-specific LLM reasoning.
...
TL;DR: SGMem (Sentence Graph Memory) is a novel memory management approach for long-term conversational agents that uses sentence-level graphs to organize dialogue across different granularities, improving context retrieval and response generation.
...
Conclusion: SGMem effectively addresses the memory management challenges in long-term conversational agents by providing better organization and retrieval of dialogue information across multiple granularities.
...
Method: The framework uses Doc2Query and Doc2Query-- to construct query-centric graphs with controllable granularity, then employs a tailored multi-hop retrieval mechanism to select relevant chunks via generated queries.
...
<details>
  <summary>Details</summary>
Motivation: Homonyms pose challenges for generative models, particularly diffusion models that may generate multiple senses of a word simultaneously. The problem is worsened by Anglocentric bias where non-English words become homonyms after translation to English.
...
Result: The paper demonstrates that prompt expansion effectively reduces homonym duplication, including issues related to Anglocentric bias. The automatic evaluation pipeline code is made publicly available.

Conclusion: Homonym duplication is a significant problem in diffusion models that can be measured and mitigated through prompt expansion techniques, with the proposed method also addressing Anglocentric bias issues.
...
TL;DR: This paper addresses output homogenization in LLMs by proposing a task-dependent approach to conceptualize and evaluate diversity, introducing task-anchored functional diversity and sampling techniques to increase diversity where needed while maintaining quality.
...
Method: Developed a task taxonomy with 8 categories, introduced task-anchored functional diversity evaluation, proposed task-anchored sampling technique, and challenged the diversity-quality trade-off assumption.

Result: The proposed approach successfully increases functional diversity for tasks where homogenization is undesired while preserving it where desired, all while maintaining response quality.

Conclusion: Task dependence is crucial for properly evaluating and mitigating output homogenization in LLMs, and the diversity-quality trade-off can be overcome with appropriate task-anchored approaches.
...
TL;DR: LLMTrace is a new large-scale bilingual corpus for AI-generated text detection that addresses limitations in existing datasets by providing character-level annotations for precise localization of AI-generated segments.


<details>
  <summary>Details</summary>
Motivation: Existing AI text detection datasets are limited by being generated with outdated models, predominantly in English, and lacking character-level annotations needed for precise localization of AI-generated segments in mixed human-AI authorship scenarios.

Method: Constructed a bilingual (English and Russian) corpus using diverse modern proprietary and open-source LLMs, designed to support both traditional full-text binary classification and the novel task of AI-generated interval detection with character-level annotations.

Result: The introduction of LLMTrace provides a comprehensive dataset that enables training and evaluation of more nuanced AI detection models capable of handling mixed authorship scenarios.

Conclusion: LLMTrace serves as a vital resource for developing next-generation AI detection systems that can precisely identify AI-generated content within human-AI mixed texts, addressing critical gaps in current detection capabilities.
...
TL;DR: Theoretical analysis of how input perturbations affect Chain-of-Thought (CoT) outputs, deriving an upper bound for acceptable perturbations and proving relationships with reasoning steps and vector norms.
...
Method: Theoretical derivation of perturbation upper bound, analysis of relationships with reasoning steps, and application to Linear Self-Attention model as simplified Transformer. Experimental validation on three datasets and four models.

Result: Proved that perturbation upper bound is positively correlated with reasoning steps, infinite reasoning cannot eliminate perturbation effects, and for LSA model, bound is negatively correlated with input embedding and hidden state norms. Experiments confirm theoretical findings.

Conclusion: Provides theoretical foundation for understanding CoT perturbation effects, showing reasoning length and vector norms are key factors, enabling better prompt optimization strategies.
...
TL;DR: The paper introduces Updesh, a synthetic instruction-following dataset for 13 Indian languages created using a bottom-up generation approach grounded in language-specific Wikipedia content, showing significant improvements in multilingual AI performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of developing AI systems that operate effectively across languages while remaining culturally grounded, particularly in low-resource settings, by exploring synthetic data generation in multilingual and multicultural contexts.

Method: A bottom-up generation strategy that prompts large open-source LLMs (>=235B parameters) to ground data generation in language-specific Wikipedia content, creating culturally contextualized datasets. The approach includes comprehensive evaluation with automated metrics and human annotation across 10k assessments.

Result: Models trained on Updesh consistently achieve significant gains on generative tasks and remain competitive on multiple-choice NLU tasks. Relative improvements are most pronounced in low and medium-resource languages, narrowing their gap with high-resource languages.
...
TL;DR: This paper decomposes sycophantic behavior in LLMs into distinct components (agreement and praise) and demonstrates they are encoded along separate linear directions that can be independently manipulated.
...
Result: Found that the three behaviors are encoded along distinct linear directions in latent space, can be independently amplified or suppressed without affecting others, and their representational structure is consistent across model families and scales.
...
TL;DR: RLBFF combines human feedback flexibility with rule-based verification precision, enabling reward models to capture nuanced response quality beyond correctness, achieving state-of-the-art performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: RLHF lacks interpretability and suffers from reward hacking due to subjective human judgments, while RLVR is limited to correctness-based verification. RLBFF aims to bridge this gap by combining human-driven preferences with precise rule-based verification.

Method: RLBFF extracts binary-answerable principles from natural language feedback (e.g., accuracy: yes/no, readability: yes/no) and uses them to train reward models as an entailment task where responses satisfy or don't satisfy arbitrary principles.

Result: Achieved top performance on RM-Bench (86.2%) and JudgeBench (81.4%, #1 leaderboard as of Sept 2025). Successfully aligned Qwen3-32B to match/exceed o3-mini and DeepSeek R1 performance on MT-Bench, WildBench, and Arena Hard v2 at <5% inference cost.

Conclusion: RLBFF provides a versatile framework that combines human preference flexibility with rule-based precision, enabling customizable reward models that outperform traditional Bradley-Terry approaches while maintaining interpretability and efficiency.
...
TL;DR: A scientific reasoning foundation model that aligns natural language with scientific representations, trained on 206B tokens and aligned via SFT, bootstrapping, and RL, supporting 103 tasks across five capability families.


<details>
  <summary>Details</summary>
Motivation: To create a unified model that can handle heterogeneous scientific representations and improve cross-domain generalization and fidelity compared to specialist systems.

Method: Pretrained on 206B-token corpus (scientific text, sequences, sequence-text pairs), aligned via supervised fine-tuning on 40M instructions, annealed cold-start bootstrapping for chain-of-thought reasoning, and reinforcement learning with task-specific reward shaping.

Result: The model broadens instruction coverage, improves cross-domain generalization, and enhances fidelity compared to specialist systems. Cross-discipline learning strengthens transfer and downstream reliability.

Conclusion: The approach successfully creates a scientific reasoning foundation model that supports diverse scientific tasks, with open-sourced model, datasets, and evaluation code available.
...
TL;DR: This paper proposes using next-token probabilities (NTPs) from vision-language models as lightweight features for efficient hallucination detection, achieving comparable performance to strong VLMs with simpler ML models.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in vision-language models undermine reliability, and current detection methods using VLMs are computationally intensive and increase latency. The authors seek an efficient on-the-fly detection approach.
...
Result: NTP-based features are valuable predictors of hallucinations, enabling simple ML models to achieve performance comparable to strong VLMs. Augmentation with linguistic NTPs and VLM scores further improves detection performance.
...
Method: Uses Riemannian Gaussian Mixture on SPD matrices to generate synthetic writers and variances. Riemannian Gaussian sampling creates positive/negative synthetic SPD populations, followed by metric learning with similar/dissimilar SPD pairs.

Result: Experiments on Western and Asian signature datasets show low error rates under intra- and cross-dataset evaluation protocols, demonstrating efficacy of the quasi-synthetic approach.

Conclusion: The proposed method highlights the potential of generating synthetic data in Riemannian spaces for writer-independent signature verification systems, achieving competitive performance without relying solely on real-world datasets.
...
<details>
  <summary>Details</summary>
Motivation: To extend traditional text-to-image systems into more interactive and multidimensional creative tools by unifying multiple image generation and editing capabilities within a single efficient framework.

Method: Developed a highly efficient diffusion transformer with powerful VAE that reduces image tokens, enabling efficient training and fast high-resolution generation. Used pretraining on billions of text-image pairs, multi-modal post-training with fine-tuned VLM, and inference acceleration techniques including adversarial distillation, distribution matching, quantization, and speculative decoding.

Result: Achieves inference time of 1.8 seconds for 2K images, state-of-the-art results on both T2I and multimodal image editing, exceptional capabilities in complex tasks like precise editing and in-context reasoning, and supports multi-image reference with multiple output generation.

Conclusion: Seedream 4.0 pushes the boundary of generative AI by creating an interactive, multidimensional creative tool that outperforms traditional T2I systems, making it suitable for both creativity and professional applications.
...
TL;DR: The paper introduces a Contrastive Learning framework using ResNet-50 for breast cancer detection in mammograms, achieving 96.7% accuracy on benchmark datasets by leveraging semi-supervised learning with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Breast cancer is a major global health concern, and early detection is crucial for better treatment outcomes. While deep learning methods show promise, they struggle with accuracy due to limited labeled datasets. The study aims to address this by using contrastive learning to work effectively with smaller labeled datasets.

Method: The study uses a semi-supervised contrastive learning approach with ResNet-50, trained on large amounts of unlabeled mammogram data using similarity index. Various augmentations and transformations are applied to improve performance, and the model is fine-tuned on a small set of labeled data.
...
Conclusion: The contrastive learning framework effectively addresses the challenge of limited labeled data in breast cancer detection, demonstrating superior performance and potential for practical applications in medical imaging.
...
Conclusion: Current Foundation Models have limitations when applied to real-world industrial quality inspection scenarios, despite their strong performance on standard benchmarks.
...
TL;DR: A universal Neural Space (NS) framework is proposed to enable multiple vision and imaging tasks to share the same feature space, reducing redundancy and improving generalization across domain shifts with a lightweight CNN-based backbone.
...
Method: An encoder-decoder framework pre-computes features across vision and imaging tasks, where the encoder learns transformation-aware, generalizable representations that allow multiple downstream AI modules to share the same feature space.
...
Conclusion: The Neural Space framework provides an efficient foundation for multi-task vision applications, offering better hardware compatibility through its lightweight CNN-based design compared to larger transformer backbones.
...
TL;DR: This paper explores efficient image selection strategies for training edge device models using teacher-student frameworks, focusing on maximizing model quality while minimizing transmission costs.


<details>
  <summary>Details</summary>
Motivation: Edge camera systems need regular model updates but face computational constraints. Current practice uses central servers with complex teacher models to annotate data for training smaller edge models, but this approach has high transmission costs.

Method: The paper proposes a stream-based strategy that combines high-confidence selection with diversity-based approaches to select the most useful images for training, minimizing dataset queries while maintaining training iterations.

Result: The approach produces high-quality models with minimal dataset queries compared to traditional methods, achieving similar training load efficiency.

Conclusion: A hybrid strategy combining high-confidence stream-based selection with diversity considerations effectively balances model quality and transmission costs for edge device training.
...
TL;DR: InstructVTON is an instruction-following virtual try-on system that uses natural language instructions instead of manual masks for fine-grained clothing styling control.


<details>
  <summary>Details</summary>
Motivation: Traditional virtual try-on models rely on binary masks which are difficult to create, require background knowledge, and cannot handle complex styling scenarios like changing sleeve positions on existing clothing.

Method: Leverages Vision Language Models (VLMs) and image segmentation models to automatically generate binary masks based on user images and free-text style instructions, enabling complex styling without manual mask creation.

Result: The system achieves state-of-the-art results with styling control and is interoperable with existing virtual try-on models, automating multiple rounds of image generation for complex try-on scenarios.

Conclusion: InstructVTON simplifies the user experience by eliminating the need for precise mask drawing and enables styling control that was previously impossible with masking-based approaches alone.
...
<details>
  <summary>Details</summary>
Motivation: Altered fingerprint recognition is challenging but essential for biometric verification in border control, forensics, and fiscal admission applications where adversaries deliberately modify ridge patterns to evade detection.
...
Result: With strict thresholds (0.92), achieves accuracies of 96.7%, 98.76%, and 99.54% for Easy, Medium, and Hard levels respectively. Threshold relaxation to 0.72 sharply degrades accuracy to 7.86%, 27.05%, and 29.51%.
...
Method: Extract attention maps from DINOv2 (self-supervised ViT), interpret them as pixel-level saliency scores, lift into 3D voxel grid, and integrate into behavior cloning policy.
...
Conclusion: Attention-guided featurization from pre-trained vision models significantly enhances robotic manipulation performance by providing semantic cues in 3D representations.
...
<details>
  <summary>Details</summary>
Motivation: Blueberry detection in natural environments is challenging due to variable lighting, occlusions, and motion blur. Deep learning detectors require large-scale datasets capturing real-world complexities, and practical deployment needs optimal accuracy/speed/memory trade-offs.
...
Result: YOLOv12m achieved best accuracy among YOLO models (mAP@50: 93.3%), RT-DETRv2-X achieved highest overall (mAP@50: 93.6%). SSL fine-tuning improved accuracy up to 2.9%, with RT-DETR-v2-X reaching 94.8% mAP@50. Mid-sized models offered best accuracy-speed balance.

Conclusion: The study provides a comprehensive benchmark for blueberry detection, showing RT-DETR-v2-X achieves state-of-the-art performance. Semi-supervised learning shows promise but requires further research for cross-domain data utilization. Dataset and software are publicly available.
...
TL;DR: A lightweight ROI augmentation strategy improves mammography classification on Mini-DDSM dataset without architectural changes or additional labels.


<details>
  <summary>Details</summary>
Motivation: Deep learning for mammogram interpretation is limited by small datasets and low resolution. Current methods need better performance without increasing complexity.

Method: Probabilistic replacement of full images with random ROI crops from a precomputed bounding-box bank during training, with optional jitter for variability. Evaluated with patient-level cross-validation.

Result: Modest ROC-AUC gains (best at p_roi=0.10, alpha=0.10) with varying performance across folds. PR-AUC remains flat to slightly lower. Training efficiency maintained with no inference cost increase.

Conclusion: Simple ROI augmentation strategies can enhance mammography classification in data-constrained settings without requiring additional labels or architectural modifications.
...
<details>
  <summary>Details</summary>
Motivation: Mirror reflections provide stereo information within a single capture, offering an opportunity to simplify 3D reconstruction by creating a multi-view stereo setup from a single image without requiring multiple cameras or captures.

Method: The method treats mirror reflections as auxiliary views, designs a transformation to construct physically valid virtual cameras, and uses a symmetric-aware loss to refine pose estimation. It also extends to dynamic scenes with mirror reflections.

Result: The framework enables direct pixel-domain generation of virtual views while adhering to real-world imaging processes, making it compatible with feed-forward reconstruction models for generalizable 3D reconstruction.

Conclusion: Extensive experiments on real-world and synthetic data demonstrate the effectiveness of using mirror reflections for robust 3D reconstruction from single images, with applications to both static and dynamic scenes.
...
TL;DR: FacadeTrack is a street-level framework that uses language-guided panoramic video to assess building occupancy after disasters by analyzing facade attributes, achieving high precision and recall through two decision strategies.


<details>
  <summary>Details</summary>
Motivation: Current methods for post-disaster building occupancy assessment have limitations - overhead imagery misses facade details while street-view imagery is sparse and hard to align with parcels. There's a need for detailed, scalable assessment that captures habitability cues.
...
Result: Evaluated on post-Hurricane Helene surveys: two-stage approach achieved precision 0.927, recall 0.781, F-1 0.848; one-stage baseline achieved precision 0.943, recall 0.728, F-1 0.822. Intermediate attributes enable error analysis and quality control.

Conclusion: FacadeTrack provides auditable, scalable occupancy assessments suitable for integration into geospatial and emergency-management workflows, with interpretable attributes that reveal where and why errors occur.
...
Conclusion: Social perception in simple displays reflects the semantic structure of social interactions, suggesting that humans use semantic representations to bridge visual and abstract understanding of social interactions.
...
<details>
  <summary>Details</summary>
Motivation: Existing CVGL methods struggle with severe appearance variations from different UAV orientations/fields of view, which hinders cross-domain generalization, and lack reliable correspondences that capture both global semantics and local details.

Method: Proposes EGS framework with: (1) E(2)-Steerable CNN encoder for rotation/viewpoint-invariant feature extraction, (2) Graph structure with virtual super-node connecting all local nodes to aggregate global semantics and enforce global-local consistency.
...
Conclusion: EGS effectively addresses cross-domain generalization challenges in CVGL through steerable feature extraction and global-local consistency mechanisms, demonstrating superior performance on standard benchmarks.
...
TL;DR: A novel Dual-Path Edge Network for infrared small target detection that decouples edge enhancement and semantic modeling to address the conflict between capturing spatial details and extracting semantic context.
...
Conclusion: The proposed framework offers a promising solution for infrared small target detection that overcomes feature misalignment issues and enables accurate target localization under challenging conditions.
...
TL;DR: This paper introduces Group Intention Forecasting (GIF) - a novel task that predicts when collective intentions will emerge by analyzing individual actions before group goals become apparent. The authors propose SHOT dataset for basketball scenarios and GIFT framework for forecasting group intention emergence.


<details>
  <summary>Details</summary>
Motivation: Traditional intention recognition focuses on individual intentions but overlooks the complexities of collective intentions in group settings. There's a need to understand how shared goals emerge through multiple individuals' actions and interactions.

Method: The authors created SHOT, the first large-scale GIF dataset with 1,979 basketball video clips from 5 camera views, annotated with 6 types of individual attributes. They also developed GIFT framework that extracts fine-grained individual features and models evolving group dynamics.
...
Conclusion: The paper successfully introduces the concept of group intention forecasting, provides a comprehensive dataset (SHOT) and framework (GIFT) for this task, and demonstrates their effectiveness in predicting when collective intentions will emerge in group settings.
...
<details>
  <summary>Details</summary>
Motivation: Maritime object detection faces two main challenges: scarcity of annotated maritime data and poor generalization across various maritime attributes (object category, viewpoint, location, imaging environment), especially in underrepresented scenarios like open-sea environments.

Method: The framework includes X-to-Maritime (multi-modality-conditioned generative model with Bidirectional Object-Water Attention module) for realistic maritime scene synthesis, and Attribute-correlated Active Sampling for dynamic selection of task-relevant synthetic samples. Also created Maritime Generation Dataset for benchmarking.
...
Conclusion: Neptune-X effectively addresses maritime data scarcity and generalization issues through a comprehensive generative-selection framework, demonstrating substantial improvements in maritime object detection performance.
...
<details>
  <summary>Details</summary>
Motivation: Current Crater-Based Navigation (CBN) methods are designed for short-duration landing missions with nadir views and good lighting, but lunar mapping missions involve sparse, oblique imagery under varying illumination over year-long campaigns, requiring more robust solutions.

Method: STELLA combines Mask R-CNN-based crater detection, descriptor-less crater identification, robust perspective-n-crater pose solver, and batch orbit determination. Tested on CRESENT-365 dataset simulating year-long lunar mapping with realistic conditions.
...
Conclusion: This represents the first comprehensive assessment of CBN in true lunar mapping settings, providing insights for operational conditions in future missions.
...
TL;DR: Deep vision-only and language-only models project inputs into partially aligned representational spaces, with alignment peaking in mid-to-late layers and being robust to appearance changes but sensitive to semantic alterations. Human preferences for image-caption matches are mirrored in embedding spaces, and averaging embeddings across exemplars amplifies alignment.


<details>
  <summary>Details</summary>
Motivation: To investigate where alignment emerges in vision and language networks, what cues support it, whether it captures human preferences in many-to-many image-text scenarios, and how aggregating exemplars affects alignment.

Method: Systematic investigation using forced-choice "Pick-a-Pic" tasks, analyzing alignment across network layers, testing robustness to appearance vs. semantic changes, and examining the effects of embedding averaging across exemplars.

Result: Alignment peaks in mid-to-late layers, is robust to appearance changes but collapses with semantic alterations, human preferences are mirrored in embedding spaces bidirectionally, and exemplar averaging amplifies alignment rather than blurring detail.

Conclusion: Unimodal networks converge on a shared semantic code that aligns with human judgments and strengthens with exemplar aggregation, demonstrating that the shared code is truly semantic and captures fine-grained distinctions.
...
Method: Convert 2D objects to 3D using existing 3D generation models, perform interactive 3D editing, then re-render to 2D from specified views. Combine geometric control with style and content control through diffusion adapters.

Result: The framework produces geometrically controlled and style-consistent edited images without requiring training.

Conclusion: FreeInsert addresses key limitations in personalized image composition by introducing 3D geometric control while maintaining style consistency in a training-free approach.
...
TL;DR: CustomEnhancer is a zero-shot enhancement framework that improves text-to-image diffusion models for human photo synthesis by combining face swapping, triple-flow fusion, and a fast inversion method called ResInversion.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models for human photo synthesis face issues with degraded scenes, insufficient control, and suboptimal perceptual identity. The paper aims to address these limitations.

Method: The framework uses face swapping techniques and pretrained diffusion models to obtain additional representations. It introduces triple-flow fused PerGeneration that combines counter-directional latent spaces, and ResInversion for noise rectification to reduce inversion time by 129x compared to NTI.

Result: Experiments show CustomEnhancer achieves state-of-the-art results in scene diversity, identity fidelity, and training-free controls while demonstrating the efficiency of ResInversion over null-text inversion.

Conclusion: CustomEnhancer provides a comprehensive solution for personalized human photo generation with improved control, fidelity, and efficiency through its novel enhancement pipeline and inversion method.
...
<details>
  <summary>Details</summary>
Motivation: With increasing use of neural network-based computer vision applications, there's a need for a consolidated platform to implement and evaluate compression methods optimized for downstream vision tasks.

Method: The platform provides a comprehensive evaluation framework where new coding tools compete to efficiently compress input for vision networks while retaining task accuracy in remote and split inferencing scenarios.
...
Conclusion: CompressAI-Vision serves as a common ground for developing and evaluating compression methods for computer vision tasks and has been adopted by MPEG for standardization efforts.
...
TL;DR: The paper proposes DAC, a dual-supervised asymmetric co-training framework for cross-domain semi-supervised domain generalization in medical image segmentation, addressing domain shifts between labeled/unlabeled training data and training/testing sets.
...
Method: DAC framework builds on co-training with two sub-models providing cross pseudo supervision, integrating feature-level supervision and asymmetric auxiliary tasks to address inaccurate pseudo supervision and enhance domain-invariant feature learning.
...
Conclusion: The DAC framework effectively handles the challenging CD-SSDG scenario with domain shifts between labeled/unlabeled training data, showing superior performance over existing methods.
...
TL;DR: DEIMv2 extends the DEIM framework with DINOv3 features, creating a scalable real-time object detection system that outperforms YOLO series across eight model sizes from X to Atto.


<details>
  <summary>Details</summary>
Motivation: To improve real-time DETR performance by leveraging DINOv3 features while maintaining efficiency across diverse deployment scenarios from GPU to mobile devices.

Method: Uses DINOv3-pretrained backbones with Spatial Tuning Adapter for larger models, HGNetv2 with pruning for lightweight models, simplified decoder, and upgraded Dense O2O matching.

Result: Achieves state-of-the-art performance: DEIMv2-X gets 57.8 AP with 50.3M parameters, DEIMv2-S exceeds 50 AP with <10M parameters, and DEIMv2-Pico matches YOLOv10-Nano with 50% fewer parameters.

Conclusion: DEIMv2 establishes superior performance-cost trade-off across all model sizes, setting new benchmarks for real-time object detection efficiency and accuracy.
...
TL;DR: DAC-LoRA is a novel framework that integrates adversarial training into Parameter-Efficient Fine-Tuning (PEFT) to enhance robustness of Vision-Language Models against adversarial attacks while maintaining clean accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) used in safety-critical applications like autonomous driving and medical diagnosis remain vulnerable to adversarial attacks, even with efficient adaptation methods like LoRA. CLIP, as a backbone for many VLMs, is particularly vulnerable.

Method: Dynamic Adversarial Curriculum (DAC-LoRA) uses an intelligent curriculum of progressively challenging attacks guided by First-Order Stationary Condition (FOSC) and TRADES-inspired loss, integrated into standard PEFT pipelines.

Result: DAC-LoRA achieves substantial improvements in adversarial robustness without significantly compromising clean accuracy, providing an effective and lightweight solution.

Conclusion: The DAC-LoRA framework is an effective, broadly applicable method that can be easily integrated into standard PEFT pipelines to significantly enhance robustness of Vision-Language Models against adversarial attacks.
...
TL;DR: FedDSPG is a federated domain generalization method that generates domain-specific soft prompts from a generative perspective to handle unknown domains during inference.
...
Conclusion: The proposed FedDSPG method effectively handles federated domain generalization tasks by generating domain-specific soft prompts, demonstrating superior performance compared to existing approaches.
...
Method: The framework integrates vision encoders (ResNet-50, Vision Transformer, Swin Transformer) with a BERT-based text encoder to extract representations, projects them into a shared embedding space via configurable projection heads, and uses soft CLIP loss for contrastive training on a curated dataset of axial MRI views paired with expert-written reports.

Result: The model achieves state-of-the-art performance with up to 95.00% accuracy and 94.75% F1-score on the test set, despite class imbalance. Ablation studies show linear projection heads yield more effective cross-modal alignment than non-linear variants.

Conclusion: LumbarCLIP offers a promising foundation for automated musculoskeletal diagnosis and clinical decision support, demonstrating effective multimodal learning for medical applications.
...
TL;DR: PoisonVID is the first black-box poisoning attack targeting prompt-guided sampling in VideoLLMs, achieving 82-99% attack success rate by suppressing harmful frame relevance scores through closed-loop optimization.
...
Conclusion: The high success rate of PoisonVID highlights the critical need to develop more secure and robust sampling strategies for future VideoLLMs.
...
TL;DR: GoR is a novel regularization method that adaptively balances task-specific and knowledge distillation losses using only two trainable parameters, improving performance of small quantized models across various AI tasks.
...
TL;DR: The LifeCLEF 2017 plant identification challenge compared web-collected noisy training data vs expert-curated smaller datasets for automated plant identification across 10,000 species using 1.1M images.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether large-scale web-collected training datasets with labeling errors can compete with smaller expert-verified datasets for continental-scale plant identification systems.

Method: Used two training strategies: (1) large noisy dataset from web sources (blogs, image hosting sites, plant retailers) and (2) smaller trusted dataset from institutional sources. Test dataset was created from Pl@ntNet mobile app queries as a third independent data source.

Result: The challenge provided comparative evaluation of both training approaches on a continental flora scale with 10,000 plant species, though specific performance metrics are not detailed in the abstract.

Conclusion: The study demonstrates the feasibility of large-scale automated plant identification systems enabled by deep learning and international biodiversity initiatives, while addressing the challenge of data quality vs quantity in training datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current plant counting methods are species-specific, but plant biodiversity and new cultivars make it impossible to build models for all species. Plants' dynamic, non-rigid nature makes existing class-agnostic counting models suboptimal for plant counting.

Method: TasselNetV4 extends TasselNet by marrying local counting with extract-and-match paradigm from CAC. It uses a plain vision transformer with novel multi-branch box-aware local counters to enhance cross-scale robustness.
...
Conclusion: TasselNetV4 emerges as a vision foundation model for cross-scene, cross-scale, and cross-species plant counting, successfully shifting from species-specific to cross-species counting.
...
TL;DR: A novel semi-supervised model for retinal OCT segmentation that introduces a differentiable biomarker topology engine to enforce anatomically correct segmentation of layers and lesions, outperforming current state-of-the-art methods.
...
Method: Proposes a fully differentiable biomarker topology engine for joint learning with bidirectional influence between layers and lesions, using disentangled representation learning that separates spatial and style factors to enforce anatomical constraints.
...
Conclusion: The approach shows the potential of using anatomical constraints in semi-supervised learning for accurate, robust, and trustworthy retinal biomarker segmentation in OCT imaging.
...
TL;DR: The LifeCLEF 2016 plant identification challenge evaluated large-scale plant recognition systems on 110K+ images of 1000 species, with the key innovation being open-set recognition where systems had to handle unknown categories not seen during training.


<details>
  <summary>Details</summary>
Motivation: To develop plant identification methods that work under real-world biodiversity monitoring conditions, requiring robustness to unknown species that weren't included in the training data.
...
Conclusion: The 2016 edition introduced the important dimension of open-set recognition to plant identification, moving beyond simple classification to handle real-world scenarios where unknown species are encountered.
...
TL;DR: SCRA-VQA is a novel approach for Knowledge-Based VQA that improves LLM reasoning by summarizing and reordering image captions to remove noise and enhance task understanding.


<details>
  <summary>Details</summary>
Motivation: Existing methods using LLMs as knowledge engines suffer from noisy image captions and LLMs' poor comprehension of VQA tasks, limiting reasoning capabilities.

Method: Uses pre-trained visual language model to generate captions, then summarizes and reorders them to exclude irrelevant information, creating contextual examples for better LLM understanding without expensive training.

Result: Achieves 38.8% accuracy on OK-VQA and 34.6% on A-OKVQA using a 6.7B parameter LLM.

Conclusion: The caption-rerank process effectively enhances LLMs' reasoning ability and task adaptability for KB-VQA without requiring costly end-to-end training.
...
TL;DR: The paper reveals an asymmetry between perceptual optimization and image quality assessment (IQA) - fidelity metrics good for IQA aren't necessarily effective for optimization, especially under adversarial training. Discriminator design significantly impacts optimization quality.
...
Method: Conducted systematic analysis comparing fidelity metrics and adversarial objectives across different discriminator architectures (patch-level, convolutional, vanilla, Transformer-based) for both optimization and IQA tasks.

Result: Found that fidelity metrics excelling in IQA are not effective for perceptual optimization, with misalignment more pronounced under adversarial training. Discriminator designs (patch-level and convolutional) provide better detail reconstruction than vanilla or Transformer alternatives.

Conclusion: The work advances understanding of loss function design and IQA transferability, providing insights for more principled approaches to perceptual optimization by revealing the decisive role of discriminator design.
...
Method: The model uses Object Interaction Self-Attention to capture complex object interactions and a GAN-Based Debiasing framework to generate unbiased data distributions for more robust feature learning.
...
Conclusion: The study highlights the importance of addressing both object interactions and dataset biases for advancing VQA tasks, demonstrating IOG-VQA's effectiveness in combining visual and textual information to overcome inherent biases.
...
<details>
  <summary>Details</summary>
Motivation: Video sequences contain structured noise and background artifacts that obscure dynamic content, and traditional robust principal component analysis (RPCA) methods fail to capture the rich variability in real video data due to their sparsity assumption.

Method: A hybrid framework integrating low-rank temporal modeling with diffusion posterior sampling, called Nuclear Diffusion, is proposed to overcome the limitations of traditional RPCA methods.

Result: The method demonstrates improved dehazing performance on cardiac ultrasound data, showing better contrast enhancement (gCNR) and signal preservation (KS statistic) compared to traditional RPCA.

Conclusion: Combining model-based temporal models with deep generative priors shows significant potential for high-fidelity video restoration in medical imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: The increasing realism of synthetic images from advanced models like VAEs, GANs, and LDMs creates significant challenges for detection, requiring new methods to identify subtle generation artifacts.

Method: The approach leverages two artifact types: latent distribution deviations and decoding-induced smoothing effects. It uses local pixel dependencies based on Markov Random Fields to reconstruct images and expose texture/edge inconsistencies. FerretNet is a lightweight neural network with only 1.1M parameters.

Result: FerretNet achieves 97.1% average accuracy on an open-world benchmark across 22 generative models, surpassing state-of-the-art methods by 10.6%, despite being trained only on the 4-class ProGAN dataset.

Conclusion: The proposed method effectively detects synthetic images by focusing on generation artifacts, demonstrating strong generalization capability and robustness across diverse generative models with minimal computational requirements.
...
TL;DR: MoTIF extends concept bottleneck models to video classification by handling temporal dependencies and enabling multi-perspective concept analysis.


<details>
  <summary>Details</summary>
Motivation: Extend interpretable concept-based modeling from static images to video data, addressing the challenge of temporal dependencies in capturing actions and events.

Method: Transformer-inspired architecture that adapts concept bottleneck framework for video classification, handling sequences of arbitrary length and capturing recurring concepts (motifs) across time.

Result: Concept-based modeling can be effectively transferred to video data, enabling better understanding of concept contributions in temporal contexts while maintaining competitive performance.

Conclusion: MoTIF successfully extends interpretable concept bottleneck models to video classification, providing global, local, and temporal perspectives on concept importance.
...
Method: Leverages cross-modality feature integration using deformable attention to combine the unique strengths of visible and thermal imagery for improved detection performance with limited labels.
...
Conclusion: The proposed FSMODNet framework demonstrates robust adaptability and effective object detection performance in few-shot multispectral scenarios, with code and models publicly available.
...
TL;DR: Particle filters enable 3D object localization using camera sequences for drone-based wildfire monitoring, overcoming limitations of dense depth estimation in distant object scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D localization methods (dense depth estimation, 3D scene reconstruction) fail for distant objects or computationally constrained tasks like drone-based wildfire monitoring.

Method: Particle filters for single and multiple target scenarios using camera poses and image segments, tested with 3D simulation and drone-based image segmentation with GNSS camera pose estimates.

Result: Particle filters successfully solved practical localization tasks where other methods failed, demonstrating effectiveness for drone-based wildfire monitoring with existing segmentation models.

Conclusion: Particle filters provide a flexible, detection-method-independent solution for 3D object localization in resource-constrained scenarios like aerial surveillance.
...
TL;DR: SwinMamba is a novel framework that combines localized Mamba-style scanning with global receptive fields to improve semantic segmentation of remote sensing imagery by capturing both local details and global context.


<details>
  <summary>Details</summary>
Motivation: Vision Mamba's global scanning approach overlooks critical local features like textures and edges that are essential for accurate remote sensing segmentation. The authors aim to address this limitation by integrating local feature capture with global context.

Method: SwinMamba integrates localized Mamba-style scanning within shifted windows (inspired by Swin Transformer). The first two stages perform local scanning for fine-grained details, while the last two stages use global scanning for broader contextual information. Overlapping shifted windows enhance inter-region information exchange.
...
Conclusion: SwinMamba demonstrates effectiveness as a superior solution for remote sensing semantic segmentation by effectively capturing both local and global features through its hybrid scanning approach.
...
TL;DR: Pack-based MIL framework for computational pathology that packs variable-length WSI sequences into fixed-length ones for efficient training, with residual branches and attention-driven downsampling to handle data heterogeneity and redundancy.
...
Method: Proposes pack-based MIL framework that packs multiple variable-length feature sequences into fixed-length ones for batched training. Includes residual branch that composes discarded features into hyperslides with tailored labels, and attention-driven downsampler to reduce redundancy.

Result: Achieves up to 8% accuracy improvement while using only 12% of training time in PANDA(UNI) benchmark. Extensive experiments show significant potential for addressing data challenges in computational pathology.

Conclusion: The proposed approach effectively addresses key challenges in computational pathology by handling sequence length variations, data heterogeneity, and redundancy, demonstrating substantial improvements in both accuracy and training efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods use simulator-based motion projection layers that are computationally expensive due to sequential processing and lack of parallelization. The authors aim to create a more efficient approach that maintains physical plausibility while enabling faster generation.

Method: The authors reinterpret simulator-based motion projection as guidance within diffusion processes and propose SimDiff, which conditions the denoising process on environmental parameters (gravity, wind) to generate physically plausible motions without repeated simulator calls.
...
Conclusion: The proposed SimDiff framework successfully integrates physical constraints directly into the diffusion process, offering an efficient alternative to simulator-based approaches while maintaining physical plausibility and enabling flexible control over environmental parameters.
...
TL;DR: This paper analyzes why certain vision architectures are more robust to Gaussian noise, identifying four key design patterns and providing theoretical explanations for their effectiveness.


<details>
  <summary>Details</summary>
Motivation: To understand the architectural dependencies of vision model robustness and convert empirical findings into actionable design rules for building more robust models against Gaussian noise.

Method: Extensive evaluation of 1,174 pretrained vision models, empirical identification of design patterns, and theoretical analysis including proofs about kernel effects, downsampling, pooling operations, and Lipschitz bounds for CLIP ViTs.

Result: Identified four consistent design patterns: larger stem kernels, smaller input resolutions, average pooling, and supervised ViTs over CLIP ViTs, yielding up to 506 rank improvements and 21.6% accuracy gains. Provided theoretical explanations for each finding.
...
TL;DR: Scene graphs are becoming essential semantic bridges in surgery, enabling intelligent systems for safety, efficiency, and training, though a data divide between real-world 2D video and simulated 4D data persists.


<details>
  <summary>Details</summary>
Motivation: To systematically map the evolving landscape of scene graph research in surgery, identifying applications, methodological advancements, and future directions to improve surgical environments.

Method: Conducted a PRISMA-ScR-guided scoping review analyzing SG research, charting growth from foundational graph neural networks to specialized foundation models that outperform generalist large vision-language models.

Result: Revealed rapid growth in SG research but uncovered a critical 'data divide': internal-view research uses real-world 2D video while external-view 4D modeling relies on simulated data, creating a translational research gap.

Conclusion: Surgical scene graphs are maturing into essential technology for both analytical and generative tasks, with ongoing progress addressing challenges in data annotation and real-time implementation to enable next-generation intelligent surgical systems.
...
TL;DR: Automated vision-based system for defect detection in laser power meter sensor coatings using unsupervised anomaly detection trained only on good samples, achieving high accuracy and fast processing.


<details>
  <summary>Details</summary>
Motivation: Address the critical challenge of identifying coating defects like thermal damage and scratches that compromise laser energy measurement accuracy in medical and industrial applications, without requiring extensive labeled defect datasets.

Method: Three-component approach: (1) preprocessing with Laplacian edge detection and K-means clustering for segmentation, (2) synthetic data augmentation via StyleGAN2, (3) UFlow-based neural network for multi-scale feature extraction and anomaly map generation using unsupervised anomaly detection trained only on good samples.

Result: 93.8% accuracy on defective samples, 89.3% accuracy on good samples, image-level AUROC of 0.957, pixel-level AUROC of 0.961, with processing time of 0.5 seconds per image in on-device implementation.

Conclusion: The system provides effective automated quality control for laser power meter sensors with high accuracy and fast processing, offering potential annual cost savings through automation.
...
TL;DR: FASTER is a modular framework for summarizing lengthy financial advisory podcast videos by extracting multimodal features, generating concise summaries, and aligning visual keyframes with textual content using BLIP, OCR, and Whisper with speaker diarization.


<details>
  <summary>Details</summary>
Motivation: The challenge of extracting insights from lengthy (30-40 minute) multimodal financial advisory content on social media platforms, where current methods struggle with efficient summarization and cross-modal alignment.

Method: Uses BLIP for semantic visual descriptions, OCR for textual patterns, Whisper-based transcription with speaker diarization, a modified DPO-based loss function with fact-checking, and ranker-based retrieval for keyframe-text alignment. Introduces Fin-APT dataset with 470 financial advisory videos.
...
<details>
  <summary>Details</summary>
Motivation: Existing SSL-integrated deep clustering methods require pretraining, clustering learning, or trained models as prerequisites, limiting flexible out-of-box application of SSL learners.

Method: Randomly sample pseudo-labeled data, use instance-level classifier to learn them, track class transitions to extract high-level similarities, assign cluster-level labels, and use pseudo-labeled data to trigger SSL learner.

Result: Superior performance across benchmarks against latest deep image clustering approaches, with only 1.33% gap on CIFAR-10 compared to SSL methods using ground-truth. ASD also boosts existing SSL-embedded methods.

Conclusion: ASD enables effective cold-start SSL for image clustering without prerequisites, achieving state-of-the-art performance and enhancing existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers produce high-norm artifacts that degrade representation quality. When these artifacts are transferred to students via knowledge distillation, they dominate the objective, causing students to overfit to artifacts and underweight informative signals, diminishing gains from larger teacher models.

Method: SiNGER uses singular nullspace-guided energy reallocation with a LoRA-based adapter. It applies nullspace-guided perturbation during teacher feature refinement to suppress artifacts while preserving informative signals, then distills the refined features to students.
...
Conclusion: SiNGER effectively addresses the trade-off between artifact suppression and signal preservation in knowledge distillation, enabling better transfer of knowledge from teacher to student models without the limitations of prior approaches.
...
Result: SNCE achieves state-of-the-art concept erasure results while preserving generation capabilities for non-target concepts, and exhibits strong robustness against adversarial attacks.

Conclusion: The proposed SNCE method enables precise and safe concept erasure in text-to-image models with minimal disruption to overall image generation quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing plant organ segmentation methods are species-specific, require extensive pre-processing, and force down-sampling due to hardware/network constraints, limiting their general applicability.

Method: Proposes KD-SS (K-Dimensional Sub-Sampling), a simple yet effective algorithm that performs sub-sampling while retaining resolution, making it agnostic to sensor data and plant species.
...
TL;DR: Mambo is a novel FG-BG decomposition framework for few-shot OOD detection that addresses over-reliance on local class similarity and fixed background patch extraction by learning background prompts and using patch self-calibrated tuning.
...
Method: Proposes learning background prompts to obtain local background similarity containing both background and semantic information, then refines it using local class similarity. Uses patch self-calibrated tuning to flexibly select background patches based on sample diversity.
...
Conclusion: Mambo effectively addresses limitations of previous methods by reducing dependence on local class similarity and enabling flexible background extraction, demonstrating superior performance in FS-OOD detection tasks.
...
TL;DR: IPS and WDES are novel stratification methods for image segmentation datasets that address biased evaluations from random splitting by optimizing label distribution similarity across dataset splits.
...
Method: IPS (Iterative Pixel Stratification) is a label-aware sampling method, and WDES (Wasserstein-Driven Evolutionary Stratification) is a genetic algorithm that minimizes Wasserstein distance to optimize label distribution similarity across splits.

Result: WDES consistently produces more representative splits with lower performance variance and improved model evaluation across diverse segmentation tasks including street scenes, medical imaging, and satellite imagery.

Conclusion: WDES is particularly valuable for handling small, imbalanced, and low-diversity datasets where conventional splitting strategies are most prone to bias, and it's proven to be globally optimal given enough generations.
...
TL;DR: EnGraf-Net leverages semantic associations structured as a hierarchy (taxonomy) as supervised signals in an end-to-end deep neural network for fine-grained classification, achieving competitive performance without requiring cropping techniques or manual annotations.


<details>
  <summary>Details</summary>
Motivation: Existing fine-grained classification models rely on part annotations or automatic cropping methods, which suffer from incomplete representation of local features. The paper proposes that semantic associations in hierarchical structures can better capture the details needed to distinguish highly similar classes.

Method: EnGraf-Net uses semantic associations structured as a taxonomy as supervised signals within an end-to-end deep neural network model. It does not require part annotations, bounding boxes, or cropping techniques.
...
Conclusion: The proposed EnGraf-Net effectively leverages semantic hierarchy information for fine-grained classification, achieving strong performance without the need for manual annotations or complex cropping techniques, making it more practical and efficient.
...
TL;DR: This paper investigates the transferability of adversarial patch attacks from CNNs to Vision Transformers (ViTs), showing that ViTs remain vulnerable despite their claimed robustness, with attack success rates varying significantly (40.04%-99.97%) depending on the specific ViT model.


<details>
  <summary>Details</summary>
Motivation: As machine learning systems become more critical, their security against evasion attacks is essential. While ViTs are considered more robust than CNNs against adversarial perturbations, they remain vulnerable to adversarial patches, which this study aims to investigate.

Method: The study designs realistic adversarial patches using Creases Transformation (CT) technique to add subtle geometric distortions, then evaluates transferability across four fine-tuned ViT models on binary person classification tasks.

Result: Experimental results show significant vulnerability variations: attack success rates ranged from 40.04% (google/vit-base-patch16-224-in21k) to 99.97% (facebook/dino-vitb16), with other models achieving 66.40% and 65.17%.

Conclusion: Adversarial patches can successfully transfer from CNNs to ViTs, with pre-training dataset scale and methodology strongly influencing model resilience to adversarial attacks, confirming cross-architectural transferability.
...
TL;DR: UniTransfer is a novel architecture for video concept transfer that uses spatial and temporal decomposition with a progressive paradigm, achieving precise control through foreground/background/motion flow separation and Chain-of-Prompt denoising stages.


<details>
  <summary>Details</summary>
Motivation: To achieve more precise and controllable video concept transfer by decomposing videos into spatial components (foreground, background, motion) and temporal components (different denoising stages) for fine-grained control.

Method: Uses spatial decomposition into foreground subject, background, and motion flow; dual-to-single-stream DiT-based architecture; self-supervised pretraining with random masking; Chain-of-Prompt mechanism for timestep decomposition with LLM-guided stage-specific instructions.
...
Conclusion: UniTransfer successfully achieves precise video concept transfer through spatial and temporal decomposition, with the curated OpenAnimal dataset facilitating future research in this area.
...
Conclusion: VTTS effectively enhances MLLM reasoning capabilities through iterative perception, demonstrating strong generalization across diverse multimodal tasks.
...
TL;DR: Mammo-CLIP Dissect is a concept-based explainability framework that uses a mammography-specific vision-language model to systematically analyze what deep learning models learn in mammography, revealing how domain-specific training and fine-tuning affect concept learning.


<details>
  <summary>Details</summary>
Motivation: Understanding what DL models learn is crucial for safe AI deployment in clinical settings. While previous work focused on pixel-based explanations, this paper addresses the gap in understanding textual concepts that better reflect clinical reasoning.

Method: Leverages Mammo-CLIP as a "dissector" to label neurons with human-interpretable textual concepts and quantify their alignment to domain knowledge. Investigates concept learning differences between models trained on general vs. mammography datasets, fine-tuning effects, and underrepresented concepts.

Result: Models trained on mammography data capture more clinically relevant concepts and align better with radiologists' workflows. Fine-tuning enhances capture of certain concepts (benign calcifications) but reduces coverage of others (density features), showing a trade-off between specialization and generalization.

Conclusion: Mammo-CLIP Dissect provides insights into how CNNs capture mammography-specific knowledge, revealing how domain-specific training and task adaptation shape concept learning, with implications for safer clinical AI deployment.
...
TL;DR: MOSS-ChatV is a reinforcement learning framework with Dynamic Time Warping-based process reward that addresses process inconsistency in video reasoning for multimodal LLMs, achieving strong performance on video benchmarks.
...
Method: Introduces MOSS-ChatV with DTW-based process reward to align reasoning traces with temporally grounded references, and creates MOSS-Video benchmark with annotated reasoning traces for training and evaluation.

Result: Achieves 87.2% on MOSS-Video test, improves performance on MVBench and MMVU, and shows gains across different architectures (Qwen2.5-VL, Phi-2) with more consistent reasoning traces.

Conclusion: The framework demonstrates broad applicability and produces more consistent reasoning traces, confirming its effectiveness for video reasoning tasks.
...
TL;DR: A novel approach for generating videos guided by camera trajectories by integrating both camera and object motions into pixel motion using stable diffusion networks and motion maps.
...
Method: Proposes integrating camera and object motions by converting them into pixel motion, using stable diffusion to learn reference motion maps relative to camera trajectory, and feeding these maps with semantic object prior into an image-to-video network.

Result: Extensive experiments show the model outperforms state-of-the-art methods by a large margin.

Conclusion: The proposed approach effectively generates videos that accurately follow designated camera trajectories while maintaining consistent object motions, solving the relative motion confusion problem in existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models often fail to achieve compositional alignment for complex prompts, and existing inference-time approaches have limitations: optimization can stall due to poor initialization, while exploration requires too many samples. Current reward metrics also don't reliably capture all aspects of compositionality.

Method: CARINOX integrates noise optimization and exploration with a reward selection procedure based on correlation with human judgments. It addresses the limitations of using either optimization or exploration alone by combining them in a principled way.
...
Conclusion: The unified framework successfully overcomes the limitations of individual optimization and exploration approaches, demonstrating that combining these strategies with human-correlated reward selection significantly improves compositional alignment in text-to-image generation.
...
<details>
  <summary>Details</summary>
Motivation: The rapid progress of image generative AI has blurred the boundary between synthetic and real images, creating an arms race between generators and discriminators. The paper aims to understand the conditions under which discriminators are most disadvantaged in this competition.
...
Result: Both very simple and highly complex datasets reduce the detectability of synthetic images. Generators can learn simple datasets almost perfectly, while extreme diversity masks imperfections. Intermediate-complexity datasets create the most favorable conditions for detection, as generators fail to fully capture the distribution and their errors remain visible.

Conclusion: The detectability of synthetic images follows a U-shaped curve with respect to data complexity - hardest to detect at both extremes (very simple and highly complex datasets) and easiest to detect at intermediate complexity levels where generator imperfections are most apparent.
...
TL;DR: WAVECLIP introduces adaptive resolution inference for CLIP using wavelet-based tokenization, enabling dynamic compute-accuracy trade-offs with a single model.


<details>
  <summary>Details</summary>
Motivation: To create a unified CLIP model that can process images at multiple resolutions and adaptively refine processing only when needed, reducing computational costs while maintaining competitive accuracy.
...
Conclusion: WAVECLIP provides an efficient solution for adaptive image processing in CLIP, allowing users to balance computational requirements and accuracy using a single deployed model.
...
<details>
  <summary>Details</summary>
Motivation: Despite CLIP's strong zero-shot capabilities, the impact of quantization on reliability metrics beyond accuracy remains underexplored, particularly for efficient and reliable deployment of vision-language models.

Method: Large-scale evaluation of quantization on CLIP models, assessing in-distribution accuracy and comprehensive reliability metrics including calibration and OOD detection, with analysis of quantization-aware training methods.

Result: Quantization consistently improves calibration for underconfident pre-trained models but can degrade it for overconfident variants. OOD detection can still improve even with poor calibration. Specific QAT methods yield simultaneous gains in zero-shot accuracy, calibration, and OOD robustness.
...
<details>
  <summary>Details</summary>
Motivation: Current table understanding benchmarks use synthetic renderings lacking real-world complexity, and existing VTU datasets offer fixed examples with single visualizations and pre-defined instructions without access to underlying data for reformulation.

Method: Created TABLET dataset with 4M examples from 2M unique tables (88% preserving original visualizations), including paired image-HTML representations, comprehensive metadata, and provenance information linking to source datasets.
...
Method: Proposes four conformity functions to quantify explanation fidelity, identifies salient feature subsets that preserve predictions regardless of excluded features, evaluated with five explainers across six image datasets.
...
Conclusion: The conformal prediction approach enables direct control over explanation fidelity without ground-truth calibration, with super-pixel measures proving superior for quantifying explanation conformity to model predictions.
...
Conclusion: Demonstrates the effectiveness of semantically informative pre-training and proves skeletal data can serve as a stand-alone solution for SLU, overcoming appearance and environmental limitations.
...
TL;DR: This paper presents a comprehensive study evaluating how well automated metrics for text-image generation align with human judgments across different compositional challenges, finding that no single metric performs consistently well across all tasks.


<details>
  <summary>Details</summary>
Motivation: Current evaluation in text-image generation relies heavily on automated metrics that are often adopted by convention rather than validated against human judgment, making it critical to understand how well these metrics actually reflect human preferences.
...
Result: No single metric performs consistently across tasks - performance varies with the type of compositional problem. VQA-based metrics are not uniformly superior, while certain embedding-based metrics prove stronger in specific cases. Image-only metrics contribute little to compositional evaluation.

Conclusion: The findings underscore the importance of careful and transparent metric selection for trustworthy evaluation and their use as reward models in generation, as metric performance depends heavily on the specific compositional challenge being evaluated.
...
Result: Achieved PRAUC of 0.751 ± 0.05, outperforming MIL (0.491), Trans-MIL (0.39), Mamba-only (0.664), GNN-only (0.748), and GAT-Mamba (0.703). Also competitive in ROC AUC (0.738), sensitivity (0.662), and specificity (0.725).
...
Conclusion: Hunyuan3D-Omni provides a unified solution for fine-grained, controllable 3D asset generation that addresses the limitations of existing methods by incorporating multiple conditioning modalities and robust multi-modal fusion.
...
TL;DR: A framework that uses vision-language models to automatically generate semantic attention maps via natural language prompts, aligning CNN attention to improve reliability and reduce shortcut learning without manual annotations.


<details>
  <summary>Details</summary>
Motivation: CNNs often exploit superficial correlations (cheat) rather than learning meaningful features, raising concerns about their decision-making reliability. Existing methods require expensive expert annotations, limiting scalability.

Method: Leverage vision-language models to automatically generate semantic attention maps using natural language prompts. Introduce an auxiliary loss to align CNN attention with these language-guided maps, promoting more reliable decision-making.

Result: State-of-the-art performance on ColoredMNIST and competitive results on DecoyMNIST compared to annotation-heavy baselines. Shows improved generalization, reduced shortcut reliance, and attention maps that better align with human intuition.
...
TL;DR: Decipher-MR is a 3D MRI-specific vision-language foundation model trained on 200,000 MRI series from 22,000+ studies, enabling robust medical image analysis across diverse clinical tasks through a modular design with frozen encoder and task-specific decoders.


<details>
  <summary>Details</summary>
Motivation: MRI's complexity and heterogeneity challenge automated analysis, and existing foundation models are limited in MRI applications due to data scarcity and narrow anatomical focus.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based approaches struggle with multi-step reasoning in vision-language tasks due to inability to access fine-grained visual content and the black-box nature of LLMs.

Method: Proposed SQ-InstructBLIP with three components: Questioner generates sub-questions, Answerer provides sub-answers, and Reasoner performs final reasoning using the generated sub-question information. All components share the same architecture.
...
Conclusion: The proposed iterative sub-question generation approach effectively improves multi-step reasoning in vision-language understanding tasks.
...
TL;DR: The paper proposes a new definition and taxonomy for hallucination in text-to-image models, categorizing it into attribute, relation, and object hallucinations to address bias-driven deviations beyond prompt alignment.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of text-to-image models focus only on prompt alignment, overlooking what models generate beyond the prompt. Hallucination in T2I models hasn't been clearly defined, unlike in language and vision-language models.

Method: The authors frame hallucination in T2I as bias-driven deviations and introduce a taxonomy with three categories: attribute, relation, and object hallucinations.

Result: This new framing establishes an upper bound for evaluation and reveals hidden biases in T2I models, enabling more comprehensive assessment.

Conclusion: The proposed taxonomy provides a foundation for richer evaluation of text-to-image models by addressing bias-driven hallucination beyond simple prompt alignment.
...
Conclusion: The proposed framework effectively addresses person-specific variations in micro-action recognition through robust optimization techniques, enabling better generalization to unseen individuals.
...
TL;DR: This paper proposes a novel approach for semantic matching that addresses geometric ambiguity and nearest-neighbor rule limitations by adapting VGGT's 3D geometric foundation model features for cross-instance semantic matching under data scarcity.


<details>
  <summary>Details</summary>
Motivation: Existing semantic matching methods suffer from geometric ambiguity (failing to disambiguate symmetric structures using 2D foundation models) and limitations of the nearest-neighbor rule (ignoring cross-image invisibility and manifold preservation).

Method: The approach adapts VGGT by: (i) retaining early feature stages, fine-tuning later ones, and adding a semantic head for bidirectional correspondences; (ii) employing cycle-consistent training, synthetic data augmentation, and progressive training with aliasing artifact mitigation to handle data scarcity.
...
Conclusion: The proposed method successfully addresses key limitations in semantic matching by leveraging VGGT's geometric foundation while adapting it for cross-instance scenarios under data constraints, demonstrating significant improvements over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: High-resolution medical videos are crucial for accurate diagnosis but hard to acquire due to hardware limitations. Existing VSR models struggle with medical video challenges like camera shake, noise, abrupt frame transitions, and tend to introduce artifacts that can mislead doctors.

Method: Proposes MedVSR with two key components: Cross State-Space Propagation (CSSP) for precise alignment by projecting distant frames as control matrices, and Inner State-Space Reconstruction (ISSR) for enhancing tissue structures and reducing artifacts through joint long-range spatial feature learning and large-kernel short-range information aggregation.
...
Conclusion: MedVSR provides an effective solution for medical video super-resolution, successfully addressing the unique challenges of medical videos while maintaining high reconstruction quality and efficiency.
...
TL;DR: This paper addresses limitations in large multimodal reasoning models by proposing Variance-Aware Sampling (VAS) to stabilize RL fine-tuning, releasing large-scale CoT data resources, and open-sourcing multimodal reasoning models.
...
Method: Proposes Variance-Aware Sampling (VAS) guided by Variance Promotion Score (VPS) that combines outcome variance and trajectory diversity to promote reward variance. Also releases ~1.6M long CoT data and ~15k RL QA pairs with reproducible training codebase.
...
Method: Developed a foundation model using Prithvi-EO Vision Transformer architecture pre-trained to reconstruct Sentinel-3 OLCI data, then fine-tuned on downstream tasks including chlorophyll concentration quantification and ocean primary production estimation.
...
TL;DR: SHINE is a training-free framework for high-fidelity image composition that addresses complex lighting challenges and high-resolution inputs by leveraging pretrained diffusion models without latent inversion or attention surgery.
...
Result: Experiments on ComplexCompo (new benchmark) and DreamEditBench show state-of-the-art performance on standard metrics (DINOv2) and human-aligned scores (DreamSim, ImageReward, VisionReward).
...
TL;DR: QuantVGGT is a novel quantization framework that addresses challenges in compressing billion-scale Visual Geometry Grounded Transformers (VGGTs) for 3D reconstruction, achieving significant memory reduction and acceleration while maintaining high accuracy.


<details>
  <summary>Details</summary>
Motivation: Large-scale transformers like VGGTs have prohibitive computational and memory costs that hinder real-world deployment. PTQ faces unique obstacles with VGGTs due to data-independent special tokens causing heavy-tailed activations and multi-view 3D data making calibration unstable.

Method: Proposes Dual-Smoothed Fine-Grained Quantization (pre-global Hadamard rotation + post-local channel smoothing) to handle heavy-tailed distributions, and Noise-Filtered Diverse Sampling (outlier filtering + frame-aware diverse calibration clusters) for stable quantization ranges.

Result: QuantVGGT achieves state-of-the-art results across benchmarks and bit-widths, with 4-bit quantization delivering 3.7× memory reduction and 2.5× acceleration while maintaining >98% reconstruction accuracy of full-precision counterpart.

Conclusion: QuantVGGT demonstrates vast advantages and practicality for resource-constrained scenarios, making billion-scale VGGTs deployable in real-world applications with minimal accuracy loss.
...
Conclusion: NewtonGen addresses fundamental limitations in current models by incorporating learnable physical principles for more realistic and controllable video generation.
...
Method: Uses a reformulated distribution matching objective for few-step generation, with innovations including 'timestep sharing' to reduce gradient noise and 'split-timestep fine-tuning' for better prompt alignment. Also includes pipeline optimizations like text encoder restructuring and specialized quantization.
...
Conclusion: SD3.5-Flash successfully makes advanced generative AI truly accessible for practical deployment across the full spectrum of devices.
...
### [154] [RadAgents: Multimodal Agentic Reasoning for Chest X-ray Interpretation with Radiologist-like Workflows](https://arxiv.org/abs/2509.20490)
...
<details>
  <summary>Details</summary>
Motivation: Current agentic systems for CXR interpretation lack clinically interpretable reasoning aligned with guidelines, insufficiently fuse multimodal evidence, and fail to detect/resolve cross-tool inconsistencies without principled verification.
...
Result: The system produces more reliable, transparent outputs that are consistent with clinical practice through improved reasoning and verification mechanisms.

Conclusion: RadAgents bridges critical gaps in CXR interpretation by providing clinically aligned, visually grounded reasoning with principled verification for handling inconsistencies.
...
### [155] [Perspectra: Choosing Your Experts Enhances Critical Thinking in Multi-Agent Research Ideation](https://arxiv.org/abs/2509.20553)
*Yiren Liu,Viraj Shah,Sangho Suh,Pao Siangliulue,Tal August,Yun Huang*

Main category: cs.HC

TL;DR: Perspectra is an interactive multi-agent system that visualizes deliberation among LLM agents through a forum-style interface with @-mention, threading, and real-time mind maps to support critical thinking and user control.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent systems enable information search and ideation but lack effective ways for users to control, steer, and critically evaluate collaboration among domain-expert agents.

Method: Developed Perspectra with forum-style interface, @-mention for targeted agent invitation, threading for parallel exploration, and real-time mind maps. Conducted within-subjects study with 18 participants comparing Perspectra to group-chat baseline for research proposal development.

Result: Perspectra significantly increased critical-thinking behaviors, elicited more interdisciplinary replies, and led to more frequent proposal revisions compared to group chat.

Conclusion: Perspectra demonstrates the importance of designing multi-agent tools that support user control over adversarial discourse to scaffold critical thinking.
...
### [156] [Every Character Counts: From Vulnerability to Defense in Phishing Detection](https://arxiv.org/abs/2509.20589)
*Maria Chiper,Radu Tudor Ionescu*

Main category: cs.CR

TL;DR: This paper investigates character-level deep learning models (CharCNN, CharGRU, CharBiLSTM) for phishing email detection, focusing on robustness, interpretability, and performance under limited computational resources.


<details>
  <summary>Details</summary>
Motivation: Current phishing detection methods lack explainability and robustness against new attacks. The authors aim to develop interpretable and robust detection models suitable for browser extension deployment.

Method: Evaluated three character-level neural architectures on a custom email dataset under three scenarios: standard training/testing, adversarial attack testing, and adversarial training. Used Grad-CAM for interpretability analysis.

Result: CharGRU performed best across all scenarios under limited computational constraints. All models were vulnerable to adversarial attacks but adversarial training significantly improved robustness. Grad-CAM successfully visualized influential email parts.

Conclusion: Character-level models provide effective phishing detection with interpretability. CharGRU is optimal for resource-constrained applications, and adversarial training enhances model robustness against attacks.
...
### [157] [PMark: Towards Robust and Distortion-free Semantic-level Watermarking with Channel Constraints](https://arxiv.org/abs/2509.21057)
*Jiahao Huo,Shuliang Liu,Bin Wang,Junyan Zhang,Yibo Yan,Aiwei Liu,Xuming Hu,Mingxun Zhou*

Main category: cs.CR

TL;DR: PMark introduces a new semantic-level watermarking framework using proxy functions to improve robustness against paraphrasing attacks while maintaining text quality and achieving distortion-free generation.


<details>
  <summary>Details</summary>
Motivation: Existing semantic-level watermarking methods lack strong theoretical guarantees and cause distribution distortions through reject-sampling, limiting their effectiveness against text modifications.

Method: PMark uses proxy functions to map sentences to scalar values, dynamically estimates PF medians through sampling, and enforces multiple PF constraints (channels) to strengthen watermark evidence with solid theoretical guarantees.

Result: PMark consistently outperforms existing SWM baselines in both text quality and robustness, achieving distortion-free generation and improved resistance to paraphrasing attacks.

Conclusion: PMark provides a more effective paradigm for detecting machine-generated text with theoretical guarantees and practical efficiency improvements.
...
### [158] [BlockFUL: Enabling Unlearning in Blockchained Federated Learning](https://arxiv.org/abs/2402.16294)
*Xiao Liu,Mingyuan Li,Xu Wang,Guangsheng Yu,Wei Ni,Lixiang Li,Haipeng Peng,Renping Liu*

Main category: cs.CR

TL;DR: BlockFUL is a dual-chain blockchain framework that enables efficient unlearning in federated learning by introducing parallel and sequential unlearning paradigms using gradient-ascent and re-training methods.


<details>
  <summary>Details</summary>
Motivation: Unlearning in federated learning is challenging due to complex inheritance relationships, especially when blockchain is used for integrity and traceability, which complicates editing multiple interlinked records and updating inherited models.

Method: Proposes BlockFUL with dual-chain structure (live chain and archive chain), introduces parallel and sequential unlearning paradigms implemented through gradient-ascent-based and re-training-based methods to enhance efficiency across multiple inherited models.

Result: Extensive experiments on CIFAR-10 and Fashion-MNIST datasets using AlexNet, ResNet18, and MobileNetV2 show reduced data dependency and operational overhead, improving unlearning performance.

Conclusion: BlockFUL effectively addresses the challenges of unlearning in blockchained federated learning by providing efficient consensus operations and reduced computational costs through its dual-chain structure and novel unlearning paradigms.
...
### [159] [Automotive-ENV: Benchmarking Multimodal Agents in Vehicle Interface Systems](https://arxiv.org/abs/2509.21143)
*Junfeng Yan,Biao Wu,Meng Fang,Ling Chen*

Main category: cs.RO

TL;DR: This paper introduces Automotive-ENV, the first high-fidelity benchmark for vehicle GUI interactions, and ASURADA, a geo-aware multimodal agent that uses GPS context to improve safety and performance in automotive environments.


<details>
  <summary>Details</summary>
Motivation: Multimodal agents have shown strong GUI interaction capabilities, but their application to automotive systems remains unexplored due to unique challenges like drivers' limited attention, strict safety requirements, and complex location-based interactions.

Method: The authors created Automotive-ENV benchmark with 185 parameterized tasks across explicit control, implicit intent understanding, and safety-aware categories. They then developed ASURADA, a geo-aware multimodal agent that integrates GPS-informed context to dynamically adjust actions based on location, environmental conditions, and regional driving norms.
...
Conclusion: The research highlights the critical role of location-aware context in automotive GUI interactions and will release Automotive-ENV with all tasks and benchmarking tools to advance safe and adaptive in-vehicle agents.
...
### [160] [Equi-RO: A 4D mmWave Radar Odometry via Equivariant Networks](https://arxiv.org/abs/2509.20674)
*Zeyu Han,Shuocheng Yang,Minghan Zhu,Fang Zhang,Shaobing Xu,Maani Ghaffari,Jianqiang Wang*

Main category: cs.RO

TL;DR: Equi-RO is an equivariant network-based framework for 4D radar odometry that uses graph-based architecture to process Doppler velocity data, achieving superior accuracy and robustness in GPS-denied environments.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles need accurate odometry in GPS-denied environments. While LiDARs and cameras struggle in extreme weather, 4D mmWave radar provides robust all-weather operability with velocity measurement capabilities.

Method: The algorithm pre-processes Doppler velocity into invariant node and edge features in a graph structure, employs separate networks for equivariant and invariant feature processing, and uses graph-based architecture to enhance feature aggregation in sparse radar data for improved inter-frame correspondence.
...
Conclusion: Equi-RO demonstrates superior performance in 4D radar odometry, providing an effective solution for autonomous navigation in challenging weather conditions where traditional sensors like LiDAR and cameras fail.
...
### [161] [Efficient Construction of Implicit Surface Models From a Single Image for Motion Generation](https://arxiv.org/abs/2509.20681)
*Wei-Teng Chu,Tianyi Zhang,Matthew Johnson-Roberson,Weiming Zhi*

Main category: cs.RO

TL;DR: FINS is a lightweight framework for fast neural surface reconstruction from single or few images using multi-resolution hash grids and foundation models, achieving state-of-the-art speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing implicit surface reconstruction methods like NeuS require large multi-view image sets and long training times, limiting practical applications. There's a need for efficient single-image reconstruction.

Method: FINS integrates multi-resolution hash grid encoder with lightweight geometry/color heads, uses approximate second-order optimizer for fast convergence, and leverages pre-trained foundation models for geometry estimation from single RGB images.

Result: FINS outperforms state-of-the-art baselines in convergence speed and accuracy for surface reconstruction and SDF field estimation, achieving training convergence within seconds.

Conclusion: The method enables practical applications like robot surface following tasks and scales well across benchmark datasets, demonstrating efficient neural surface construction from minimal input.
...
### [162] [RAM-NAS: Resource-aware Multiobjective Neural Architecture Search Method for Robot Vision Tasks](https://arxiv.org/abs/2509.20688)
...
<details>
  <summary>Details</summary>
Motivation: Conventional NAS approaches are insufficient for training supernets and ignore actual robot hardware resource constraints, creating a need for hardware-aware NAS methods for edge robotics applications.

Method: Proposes subnets mutual distillation using sandwich rule sampling and DKD loss for better logits distillation. Uses latency surrogate predictors trained on three robotic edge hardware types to estimate inference latency during evolutionary search, enabling multi-objective optimization of accuracy and latency.

Result: RAM-NAS models achieve 76.7% to 81.4% top-1 accuracy on ImageNet while significantly reducing inference latency on robot edge hardware. Downstream tasks show reduced detection and segmentation times compared to MobileNetv3-based methods across all three hardware types.

Conclusion: RAM-NAS successfully fills the gap in NAS for robot hardware resource-awareness, demonstrating effective multi-objective optimization that balances model accuracy with hardware efficiency for edge robotics applications.
...
### [163] [Joint Flow Trajectory Optimization For Feasible Robot Motion Generation from Video Demonstrations](https://arxiv.org/abs/2509.20703)
*Xiaoxiang Dong,Matthew Johnson-Roberson,Weiming Zhi*

Main category: cs.RO

TL;DR: JFTO framework enables robots to learn manipulation tasks from human videos by optimizing grasp poses and object trajectories while handling embodiment differences and feasibility constraints.


<details>
  <summary>Details</summary>
Motivation: Learning from human video demonstrations is scalable but challenging due to robot-human embodiment differences and joint feasibility constraints that prevent direct imitation of human hand motions.

Method: Joint Flow Trajectory Optimization (JFTO) treats demonstrations as object-centric guides, using flow matching on SE(3) for probabilistic trajectory modeling and integrating grasp similarity, trajectory likelihood, and collision penalties into a unified differentiable objective.

Result: The approach was validated in both simulation and real-world experiments across diverse manipulation tasks, showing effective imitation while avoiding mode collapse.

Conclusion: JFTO provides a robust framework for video-based Learning-from-Demonstration that balances grasp feasibility, trajectory consistency, and collision-free execution under robot kinematic constraints.
...
### [164] [SLAM-Free Visual Navigation with Hierarchical Vision-Language Perception and Coarse-to-Fine Semantic Topological Planning](https://arxiv.org/abs/2509.20739)
*Guoyang Zhao,Yudong Li,Weiqing Qi,Kai Zhang,Bonan Liu,Kai Chen,Haoang Li,Jun Ma*

Main category: cs.RO

TL;DR: A vision-only, SLAM-free navigation framework for legged robots that replaces dense geometry with semantic reasoning and lightweight topological representations for robust navigation.
...
Method: Hierarchical vision-language perception module fuses scene-level context with object-level cues. Uses semantic-probabilistic topological map with LLM-based global reasoning for subgoal selection and vision-based local planning for obstacle avoidance. Integrated with reinforcement-learning locomotion controllers.

Result: Experiments in simulation and real-world settings demonstrate consistent improvements in semantic accuracy, planning quality, and navigation success. Ablation studies show necessity of both hierarchical perception and fine local planning.
...
### [165] [MASt3R-Fusion: Integrating Feed-Forward Visual Model with IMU, GNSS for High-Functionality SLAM](https://arxiv.org/abs/2509.20757)
*Yuxuan Zhou,Xingxing Li,Shengyu Li,Zhuohao Yan,Chunxi Xia,Shaoquan Feng*

Main category: cs.RO

TL;DR: MASt3R-Fusion is a multi-sensor visual SLAM framework that integrates feed-forward neural network-based pointmap regression with inertial and GNSS data using a hierarchical factor graph for real-time pose tracking and globally consistent mapping.


<details>
  <summary>Details</summary>
Motivation: Classical visual SLAM systems struggle with low-texture environments, scale ambiguity, and challenging visual conditions. While neural network-based approaches show promise, they often discard the advantages of probabilistic multi-sensor fusion.

Method: Proposes a framework that tightly integrates feed-forward pointmap regression with inertial and GNSS data using Sim(3)-based visual alignment constraints in a hierarchical factor graph design, enabling both real-time sliding-window optimization and global optimization with loop closures.
...
Conclusion: MASt3R-Fusion demonstrates the effectiveness of combining learned spatial priors with traditional multi-sensor fusion, achieving superior performance in visual SLAM applications for robotics, autonomous driving, and XR.
...
### [166] [Autoregressive End-to-End Planning with Time-Invariant Spatial Alignment and Multi-Objective Policy Refinement](https://arxiv.org/abs/2509.20938)
*Jianbo Zhao,Taiyu Ban,Xiangjie Li,Xingtai Gui,Hangning Zhou,Lei Liu,Hongwei Zhao,Bin Li*

Main category: cs.RO

TL;DR: The paper proposes TISA (Time-Invariant Spatial Alignment) module to address spatio-temporal misalignment in autoregressive planning for autonomous driving, using kinematic action prediction and multi-objective DPO for improved performance.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models have strong sequential modeling capabilities but suffer from spatio-temporal misalignment where planners must condition future actions on past sensory data, creating an inconsistent worldview that limits performance.

Method: Proposes TISA module to project environmental features into consistent ego-centric frames for future time steps, uses kinematic action prediction for physical feasibility, and implements multi-objective DPO for fine-grained behavior feedback.

Result: Achieves state-of-the-art 89.8 PDMS on NAVSIM dataset among autoregressive models.

Conclusion: The approach effectively addresses worldview inconsistency in autonomous driving planning while ensuring physical feasibility and moving beyond pure imitation learning through targeted behavior optimization.
...
### [167] [KeyWorld: Key Frame Reasoning Enables Effective and Efficient World Models](https://arxiv.org/abs/2509.21027)
...
Result: On the LIBERO benchmark, KeyWorld achieves 5.68× acceleration compared to frame-to-frame generation baseline and improves physical validity of generated videos, especially on complex tasks.
...
### [168] [Cross-Modal Instructions for Robot Motion Generation](https://arxiv.org/abs/2509.21107)
...
<details>
  <summary>Details</summary>
Motivation: Traditional robot teaching requires physical motion demonstrations which are cumbersome to collect and scale. This paper aims to create a more scalable alternative using rough annotations and text instructions.

Method: Integrates cross-modal instructions into a foundational VLM that iteratively queries a smaller fine-tuned model to synthesize motion over 2D views, then fuses them into 3D motion trajectories. Includes downstream RL pipeline for policy refinement.

Result: Effectively produces executable robot behaviors that generalize beyond limited instruction examples, works without additional fine-tuning, and provides strong initialization for RL policies.

Conclusion: CrossInstruct offers a scalable paradigm for robot behavior learning that reduces reliance on physical demonstrations and enables efficient policy learning through cross-modal instructions.
...
### [169] [Human-like Navigation in a World Built for Humans](https://arxiv.org/abs/2509.21189)
...
<details>
  <summary>Details</summary>
Motivation: Existing robot navigation systems lack human-like behaviors such as reading signs and asking for directions, making them inefficient in large man-made environments where humans use these strategies to navigate effectively.

Method: The system uses a vision-language model (VLM) with compact input/output abstractions based on navigation landmarks, allowing the VLM to focus on language understanding and reasoning rather than low-level navigation tasks.
...
Conclusion: ReasonNav demonstrates that integrating human-like navigation skills through VLMs enables more efficient robot navigation in complex environments, addressing limitations of traditional navigation systems.
...
### [170] [Verification Limits Code LLM Training](https://arxiv.org/abs/2509.20837)
*Srishti Gureja,Elena Tommasone,Jingyi He,Sara Hooker,Matthias Gallé,Marzieh Fadaee*

Main category: cs.SE

TL;DR: Current synthetic data generation for code LLMs faces a 'verification ceiling' where verifier limitations constrain training data quality. The paper shows that rigid 100% pass criteria are too restrictive, and that relaxed thresholds, LLM-based soft verification, and diverse test suites can improve model performance by 2-4 points in pass@1.


<details>
  <summary>Details</summary>
Motivation: To address the verification ceiling problem in synthetic data generation for code LLMs, where the capabilities of synthetic verifiers fundamentally constrain the quality and diversity of training data.

Method: Systematically studied verification design and strategies by analyzing: (i) test complexity vs quantity, (ii) rigid vs relaxed pass thresholds and LLM-based soft verification, (iii) necessity of verification through comparisons of correct/incorrect solutions and human evaluation.

Result: Richer test suites improve code generation (+3 pass@1), relaxed thresholds recover valuable training data (2-4 point improvement), and retaining diverse correct solutions yields consistent generalization gains. Verification cannot be discarded but needs recalibration.

Conclusion: Verification as currently practiced is too rigid and filters out valuable diversity. By combining calibrated verification with diverse, challenging problem-solution pairs, we can break the verification ceiling and unlock stronger code generation models.
...
### [171] [Interactive Recommendation Agent with Active User Commands](https://arxiv.org/abs/2509.21317)
...
TL;DR: The paper introduces Interactive Recommendation Feed (IRF) and RecBot, a dual-agent system that enables natural language commands for active recommendation control, addressing limitations of traditional passive feedback systems.


<details>
  <summary>Details</summary>
Motivation: Traditional recommender systems rely on coarse-grained passive feedback (like/dislike) that fails to capture nuanced user motivations and specific attribute preferences, creating a gap between user intentions and system interpretations.

Method: Developed RecBot with dual-agent architecture: Parser Agent transforms linguistic commands into structured preferences, and Planner Agent dynamically orchestrates adaptive tool chains for real-time policy adjustment. Used simulation-augmented knowledge distillation for efficient deployment.

Result: Extensive offline and long-term online experiments show significant improvements in both user satisfaction and business outcomes.

Conclusion: IRF paradigm with RecBot successfully bridges the gap between user intentions and system recommendations by enabling active explicit control through natural language commands, outperforming traditional passive feedback systems.
...
### [172] [Provenance Analysis of Archaeological Artifacts via Multimodal RAG Systems](https://arxiv.org/abs/2509.20769)
...
<details>
  <summary>Details</summary>
Motivation: To support expert reasoning in archaeology by alleviating the cognitive burden of navigating vast comparative corpora and providing meaningful starting points for artifact analysis.

Method: Constructs a dual-modal knowledge base from texts and images, performs raw visual, edge-enhanced, and semantic retrieval to identify similar artifacts, then uses VLMs to synthesize retrieved candidates into structured inferences with justifications.

Result: Expert evaluation on Eastern Eurasian Bronze Age artifacts from the British Museum shows the system produces meaningful, interpretable outputs that offer concrete analysis starting points.

Conclusion: The RAG-based system effectively supports archaeological provenance analysis by integrating multimodal retrieval and VLMs, significantly reducing experts' cognitive load while providing valuable analytical insights.
...
### [173] [Blueprints of Trust: AI System Cards for End to End Transparency and Governance](https://arxiv.org/abs/2509.20394)
*Huzaifa Sidhpurwala,Emily Fox,Garth Mollett,Florencio Cano Gabarda,Roman Zhukov*

Main category: cs.CY

TL;DR: The paper introduces Hazard-Aware System Card (HASC), a framework that enhances AI system transparency by integrating security and safety records with standardized identifiers like AI Safety Hazard (ASH) ID, complementing existing security identifiers.


<details>
  <summary>Details</summary>
Motivation: To improve transparency and accountability in AI system development and deployment by creating a comprehensive, dynamic record of security and safety posture that builds upon existing model and system card concepts.

Method: Proposes a standardized system of identifiers including ASH ID to complement security identifiers like CVEs, creating a single accessible source of truth for AI system safety throughout its lifecycle.

Result: Developed the HASC framework that allows clear communication of fixed flaws and enables informed decision-making about AI system safety by developers and stakeholders.

Conclusion: The HASC framework complements ISO/IEC 42001:2023 standard and provides greater transparency and accountability for AI systems through standardized safety and security documentation.
...
### [174] [Communication Bias in Large Language Models: A Regulatory Perspective](https://arxiv.org/abs/2509.21075)
*Adrian Kuenzler,Stefan Schmid*

Main category: cs.CY

TL;DR: This paper reviews risks of biased outputs from large language models (LLMs) and their societal impact, focusing on regulatory frameworks like the EU's AI Act and Digital Services Act.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly central to many applications, raising concerns about bias, fairness, and regulatory compliance.

Method: The paper conducts a review of risks associated with biased LLM outputs and analyzes existing regulatory frameworks.

Result: The analysis identifies that beyond constant regulation, stronger attention to competition and design governance is needed.

Conclusion: Enhanced focus on competition and design governance is necessary to ensure fair and trustworthy AI systems.
...
### [175] [SceneWeaver: All-in-One 3D Scene Synthesis with an Extensible and Self-Reflective Agent](https://arxiv.org/abs/2509.20414)
*Yandan Yang,Baoxiong Jia,Shujie Zhang,Siyuan Huang*

Main category: cs.GR

TL;DR: SceneWeaver is a reflective agentic framework that unifies diverse scene synthesis paradigms through tool-based iterative refinement for indoor 3D environment generation.


<details>
  <summary>Details</summary>
Motivation: Current indoor scene synthesis approaches are constrained to fixed categories, lack object-level detail and physical consistency, and struggle with complex user instructions, limiting their applicability in Embodied AI.

Method: Uses a language model-based planner to select from extensible scene generation tools (data-driven generative models, visual- and LLM-based methods) in a closed-loop reason-act-reflect design with self-evaluation of physical plausibility, visual realism, and semantic alignment.

Result: Outperforms prior methods on physical, visual, and semantic metrics, and generalizes effectively to complex scenes with diverse instructions across common and open-vocabulary room types.

Conclusion: SceneWeaver marks a step toward general-purpose 3D environment generation by enabling iterative refinement and better alignment with user requirements through its agentic framework.
...
### [176] [ArtUV: Artist-style UV Unwrapping](https://arxiv.org/abs/2509.20710)
*Yuguang Chen,Xinhai Liu,Yang Li,Victor Cheung,Zhuo Chen,Dongyu Zhang,Chunchao Guo*

Main category: cs.GR

TL;DR: ArtUV is an automated end-to-end method for generating artist-style UV unwrapping that addresses limitations of existing methods like time consumption, fragmentation, lack of semanticity, and irregular UV islands.


<details>
  <summary>Details</summary>
Motivation: Existing UV unwrapping methods struggle with time-consuming processes, fragmentation, lack of semantic coherence, and irregular UV islands, limiting their practical use in professional workflows. Artist-style UV maps require not only technical correctness but also clean boundaries, efficient space utilization, and semantic coherence.

Method: ArtUV simulates professional UV mapping through two stages: 1) SeamGPT for semantically meaningful cutting seam prediction, and 2) an Auto-Encoder that refines rough UV from optimization-based methods into artist-style UV maps while preserving topological structure.

Result: ArtUV generates high-quality UV maps that ensure semantic consistency and preserve topological structure, making them ready for 2D editing. The method functions as both a plug-in for professional rendering tools and a standalone system for rapid UV generation.

Conclusion: ArtUV serves as a versatile solution for automated artist-style UV unwrapping, addressing key limitations of existing methods and providing a practical tool for professional computer graphics workflows.
...
### [177] [SeamCrafte: Enhancing Mesh Seam Generation for Artist UV Unwrapping via Reinforcement Learning](https://arxiv.org/abs/2509.20725)
...
<details>
  <summary>Details</summary>
Motivation: Poorly placed mesh seams cause severe UV distortion or excessive fragmentation, hindering texture synthesis and disrupting artist workflows. Existing methods often trade one problem for another - producing either high distortion or many scattered islands.

Method: SeamCrafter uses an autoregressive GPT-style seam generator conditioned on point cloud inputs with a dual-branch point-cloud encoder that captures complementary topological and geometric cues. The model is fine-tuned using Direct Preference Optimization (DPO) on a preference dataset from a novel seam-evaluation framework.
...
### [178] [ARMesh: Autoregressive Mesh Generation via Next-Level-of-Detail Prediction](https://arxiv.org/abs/2509.20824)
*Jiabao Lei,Kewei Shi,Zhihao Liang,Kui Jia*

Main category: cs.GR

TL;DR: Progressive auto-regressive mesh generation using simplicial complexes and transformer models that reverse mesh simplification, enabling coarse-to-fine construction from a single point with local remeshing.


<details>
  <summary>Details</summary>
Motivation: Traditional auto-regressive mesh generation models build meshes face by face in lexicographic order, which doesn't effectively capture underlying geometry in a human-perceptual way. The authors aim to create a more intuitive approach inspired by progressive 2D image generation models.

Method: Generalize meshes to simplicial complexes and develop a transformer-based auto-regressive model that approximates the reverse process of mesh simplification. The model constructs meshes progressively from a single point, gradually adding geometric details through local remeshing with alterable topology.

Result: The progressive mesh generation approach provides intuitive control over generation quality and time consumption via early stopping. It also enables applications like mesh refinement and editing.

Conclusion: This novel progressive approach to auto-regressive mesh generation offers more intuitive geometry capture and practical benefits over traditional lexicographic face-by-face construction methods.
...
### [179] [ArchGPT: Understanding the World's Architectures with Large Multimodal Models](https://arxiv.org/abs/2509.20858)
*Yuze Wang,Luo Yang,Junyi Wang,Yue Qi*

Main category: cs.GR

TL;DR: ArchGPT is a multimodal architectural visual question answering model trained on Arch-300K, a specialized dataset of 315,000 image-question-answer triplets curated through a novel pipeline that integrates 3D reconstruction, semantic segmentation, and LLM-guided text verification to enable scalable architectural analysis in VR/MR/AR systems.


<details>
  <summary>Details</summary>
Motivation: Existing VR/MR/AR systems for architectural exploration rely on case-by-case development with hard-coded annotations that don't scale across diverse built environments, limiting accessibility and creative workflows in education, heritage preservation, and design practice.

Method: Developed a scalable data-construction pipeline using coarse-to-fine strategy with 3D reconstruction and semantic segmentation to select architectural images from Wikimedia Commons, then used LLM-guided text verification to generate reliable question-answer pairs, and fine-tuned ShareGPT4V-7B on the resulting Arch-300K dataset.

Result: Created Arch-300K dataset with 315,000 high-quality architecture-specific image-question-answer triplets and developed ArchGPT, a specialized multimodal VQA model capable of architectural analysis.

Conclusion: ArchGPT provides a scalable solution for architectural understanding in immersive systems, overcoming limitations of case-specific approaches and enabling broader applications in architectural education, preservation, and design workflows.
...
### [180] [Marching Neurons: Accurate Surface Extraction for Neural Implicit Shapes](https://arxiv.org/abs/2509.21007)
...
<details>
  <summary>Details</summary>
Motivation: Conventional surface extraction methods for implicit representations rely on spatial decomposition and sampling, leading to inaccuracies due to fixed and limited resolution. There's a need for more accurate conversion between implicit and explicit 3D representations.

Method: Developed a depth-first traversal strategy that leverages how each neuron partitions the domain to efficiently track the encoded surface. The method operates natively in parallel and can handle large neural architectures without spatial discretization.
...
Conclusion: The proposed analytical surface extraction method provides superior geometric accuracy compared to conventional sampling-based approaches, enabling more faithful conversions between neural implicit functions and explicit surface representations.
...
### [181] [CHARM: Control-point-based 3D Anime Hairstyle Auto-Regressive Modeling](https://arxiv.org/abs/2509.21114)
*Yuze He,Yanning Zhou,Wang Zhao,Jingwen Ye,Yushi Bai,Kaiwen Xiao,Yong-Jin Liu,Zhongqian Sun,Wei Yang*

Main category: cs.GR

TL;DR: CHARM is a novel parametric representation and generative framework for anime hairstyle modeling that uses an efficient control-point-based parameterization and autoregressive transformer to generate high-fidelity anime hairstyles from images or point clouds.


<details>
  <summary>Details</summary>
Motivation: Traditional hair modeling methods focus on realistic hair but struggle with anime hairstyles' highly stylized, piecewise-structured geometry. Existing techniques are inefficient for editing and unsuitable for scalable learning.

Method: CHARM introduces a compact, invertible control-point-based parameterization (5 parameters per point) and an autoregressive generative framework that treats anime hairstyles as a sequential 'hair language' using transformers to capture both local geometry and global topology.

Result: The method achieves state-of-the-art performance in both reconstruction accuracy and generation quality, demonstrated through extensive experiments on the AnimeHair dataset containing 37K high-quality anime hairstyles.

Conclusion: CHARM offers an expressive and scalable solution for anime hairstyle modeling that supports both artist-friendly design and learning-based generation.
...
### [182] [InsightGUIDE: An Opinionated AI Assistant for Guided Critical Reading of Scientific Literature](https://arxiv.org/abs/2509.20493)
*Paris Koloveas,Serafeim Chatzopoulos,Thanasis Vergoulis,Christos Tryfonopoulos*

Main category: cs.AI

TL;DR: InsightGUIDE is an AI-powered reading assistant that provides structured insights as a "map" to paper key elements, embedding expert reading methodology to avoid replacing source material reading.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of scientific literature proliferation by creating a tool that assists rather than replaces reading, overcoming verbose summaries from existing LLM tools.

Method: System architecture with prompt-driven methodology that embeds expert reading methodology into AI logic, validated through qualitative case study comparison with general-purpose LLM.

Result: InsightGUIDE produces more structured and actionable guidance than general-purpose LLMs, serving as a more effective tool for researchers.

Conclusion: InsightGUIDE successfully functions as a reading assistant that provides concise, structured insights to help researchers navigate scientific papers effectively.
...
### [183] [CLAUSE: Agentic Neuro-Symbolic Knowledge Graph Reasoning via Dynamic Learnable Context Engineering](https://arxiv.org/abs/2509.21035)
*Yang Zhao,Chengxiao Dai,Wei Zhuo,Yue Xiu,Dusit Niyato*

Main category: cs.AI

TL;DR: CLAUSE is a neuro-symbolic framework that treats knowledge graph context construction as a sequential decision process, optimizing accuracy, latency, and cost trade-offs through multi-agent coordination under user-specified budgets.


<details>
  <summary>Details</summary>
Motivation: Existing methods like static k-hop expansions and "think-longer" prompting over-retrieve context, inflate costs, and yield unpredictable runtime. There's a need for systems that balance answer accuracy with strict latency and cost targets while preserving provenance.

Method: CLAUSE uses Lagrangian-Constrained Multi-Agent Proximal Policy Optimization (LC-MAPPO) to coordinate three agents: Subgraph Architect, Path Navigator, and Context Curator. This jointly optimizes subgraph construction, reasoning-path discovery, and evidence selection under per-query resource budgets.
...
### [184] [Disagreements in Reasoning: How a Model's Thinking Process Dictates Persuasion in Multi-Agent Systems](https://arxiv.org/abs/2509.21054)
...
<details>
  <summary>Details</summary>
Motivation: To understand the persuasion dynamics in Multi-Agent Systems where LLMs and LRMs collaborate, challenging the prevailing hypothesis that persuasive efficacy is primarily a function of model scale.

Method: Conducted a series of multi-agent persuasion experiments, including multi-hop persuasion scenarios in agent networks, to analyze how reasoning processes affect persuasion susceptibility and efficacy.

Result: Found that LRMs with explicit reasoning capabilities are significantly more resistant to persuasion but become dramatically more persuasive when their reasoning process is made transparent. Complex transmission dynamics show influence propagation and decay in multi-hop persuasion.

Conclusion: The research provides systematic evidence linking internal processing architecture to external persuasive behavior, offering implications for the safety, robustness, and design of future Multi-Agent Systems.
...
### [185] [TrustJudge: Inconsistencies of LLM-as-a-Judge and How to Alleviate Them](https://arxiv.org/abs/2509.21117)
...
TL;DR: TrustJudge is a probabilistic framework that addresses inconsistencies in LLM-as-a-judge evaluation systems by introducing distribution-sensitive scoring and likelihood-aware aggregation to reduce score-comparison and pairwise transitivity inconsistencies.


<details>
  <summary>Details</summary>
Motivation: Current LLM-as-a-judge evaluation frameworks suffer from critical inconsistencies including score-comparison inconsistency (where lower-rated responses outperform higher-scored ones) and pairwise transitivity inconsistency (circular preference chains and equivalence contradictions). These issues stem from information loss in discrete rating systems and ambiguous tie judgments.

Method: TrustJudge proposes two key innovations: 1) distribution-sensitive scoring that computes continuous expectations from discrete rating probabilities to preserve information entropy, and 2) likelihood-aware aggregation that resolves transitivity violations using bidirectional preference probabilities or perplexity.

Result: When evaluated with Llama-3.1-70B-Instruct, TrustJudge reduced Score-Comparison inconsistency by 8.43% (from 23.32% to 14.89%) and Pairwise Transitivity inconsistency by 10.82% (from 15.22% to 4.40%), while maintaining higher evaluation accuracy. The framework shows consistent improvements across various model architectures and scales.
...
### [186] [Expanding Reasoning Potential in Foundation Model by Learning Diverse Chains of Thought Patterns](https://arxiv.org/abs/2509.21124)
...
<details>
  <summary>Details</summary>
Motivation: Current approaches use CoT data indiscriminately, without understanding which data types most effectively enhance model reasoning capabilities. The paper aims to identify and utilize high-value reasoning patterns to improve model performance efficiently.
...
Conclusion: Selective use of high-value reasoning patterns in CoT data significantly enhances model reasoning capabilities and performance, demonstrating the importance of data quality over quantity.
...
### [187] [VC-Agent: An Interactive Agent for Customized Video Dataset Collection](https://arxiv.org/abs/2509.21291)
*Yidan Zhang,Mutian Xu,Yiming Hao,Kun Zhou,Jiahao Chang,Xiaoqiang Liu,Pengfei Wan,Hongbo Fu,Xiaoguang Han*

Main category: cs.AI

TL;DR: VC-Agent is an interactive AI agent that helps users efficiently collect customized video datasets by understanding queries and feedback, using multimodal LLMs and adaptive filtering policies.
...
Method: Leverages multimodal LLMs to connect user requirements with video content, provides user-friendly interfaces for specifying requirements, and implements two novel filtering policies that update based on continuous user interaction.

Result: Extensive experiments demonstrate the agent's effectiveness and efficiency for customized video dataset collection, with user studies verifying its utility in various real scenarios.

Conclusion: VC-Agent successfully addresses the challenge of personalized video dataset collection by providing an interactive, adaptive system that minimizes user input while maximizing relevance of retrieved video clips.
...
### [188] [Extrapolating Phase-Field Simulations in Space and Time with Purely Convolutional Architectures](https://arxiv.org/abs/2509.20770)
*Christophe Bonneville,Nathan Bieberdorf,Pieterjan Robbe,Mark Asta,Habib N. Najm,Laurent Capolungo,Cosmin Safta*

Main category: cs.CE

TL;DR: A U-Net surrogate model for liquid metal dealloying that generalizes beyond training data in space and time, achieving 16,000x speedup with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Phase-field models of liquid metal dealloying become computationally intractable for large domains or long time horizons, requiring more efficient simulation methods.

Method: Conditionally parameterized fully convolutional U-Net with convolutional self-attention and physics-aware padding, trained on short small-scale simulations but capable of spatial and temporal extrapolation.
...
Conclusion: This marks an early step toward scalable, high-fidelity extrapolation of LMD phase-field models, demonstrating significant computational acceleration while maintaining accuracy.
...
### [189] [Visual Authority and the Rhetoric of Health Misinformation: A Multimodal Analysis of Social Media Videos](https://arxiv.org/abs/2509.20724)
*Mohammad Reza Zarei,Barbara Stead-Coyle,Michael Christensen,Sarah Everts,Majid Komeili*

Main category: cs.SI

TL;DR: This study analyzes how credibility is packaged in nutrition and supplement videos on short-form platforms (TikTok, Instagram, YouTube) by examining authority signals, narrative techniques, and monetization strategies rather than adjudicating truth.


<details>
  <summary>Details</summary>
Motivation: Short-form video platforms have become central sites for health advice where alternative narratives mix useful, misleading, and harmful content. The research aims to understand how credibility is constructed in this environment.

Method: The study assembled a cross-platform corpus of 152 public videos and annotated each on 26 features spanning visual authority, presenter attributes, narrative strategies, and engagement cues. A transparent annotation pipeline integrated automatic speech recognition, principled frame selection, and a multimodal model with human verification.

Result: Findings show dominant patterns include confident single presenters in studio/home settings (rare clinical contexts). Authority cues frequently co-occur with persuasive elements (jargon, fear/urgency, critiques of mainstream medicine, conspiracies) and monetization. References and science-like visuals often accompany emotive/oppositional narratives rather than signaling restraint.

Conclusion: Credibility packaging on health videos combines authority signals with persuasive and monetization elements, suggesting that scientific-looking elements are often used to support emotional and oppositional narratives rather than indicating balanced or restrained content.
...
### [190] [Intercept Cancer: Cancer Pre-Screening with Large Scale Healthcare Foundation Models](https://arxiv.org/abs/2506.00209)
*Liwen Sun,Hao-Ren Yao,Gary Gao,Ophir Frieder,Chenyan Xiong*

Main category: cs.LG

TL;DR: CATCH-FM is a cancer pre-screening method that uses healthcare foundation models to identify high-risk patients for further screening based solely on historical medical records, achieving strong performance with 60% sensitivity and 99% specificity.


<details>
  <summary>Details</summary>
Motivation: Existing cancer screening techniques are expensive, intrusive, and not globally available, leading to many preventable deaths. There's a need for accessible pre-screening methods that can identify high-risk patients efficiently.

Method: Pretrained compute-optimal foundation models (up to 2.4B parameters) on millions of electronic healthcare records using medical code sequences, then fine-tuned on clinician-curated cancer risk prediction cohorts.

Result: Achieved 60% sensitivity and 99% specificity/NPV in retrospective evaluation of 30,000 patients, outperforming feature-based tree models and other LLMs. Also achieved state-of-the-art pancreatic cancer risk prediction on EHRSHOT leaderboard despite demographic and system differences.

Conclusion: CATCH-FM demonstrates robust performance across patient distributions, benefits from operating in ICD code space, and can capture non-trivial cancer risk factors, showing promise for accessible cancer pre-screening.
...
TL;DR: This paper demonstrates that federated learning (FL) for LLM fine-tuning still poses privacy risks as attackers can extract training data from global models, with leakage increasing with model size. The authors propose enhanced FL-specific attacks and evaluate mitigation techniques.


<details>
  <summary>Details</summary>
Motivation: Organizations want to collaboratively fine-tune LLMs using shared data characteristics but are reluctant to share local data. While FL is considered privacy-preserving, the paper aims to investigate whether FL actually protects client privacy in LLM fine-tuning scenarios.

Method: The authors conduct extensive experiments showing data leakage from FL-trained global models using straightforward generation methods. They introduce an enhanced attack strategy that tracks global model updates during training to intensify privacy leakage.

Result: Results show that attackers can extract training data from FL global models, with leakage increasing as model size grows. The enhanced FL-specific attack further intensifies privacy risks.

Conclusion: The paper evaluates privacy-preserving techniques including differential privacy, regularization-constrained updates, and safety-aligned LLMs, providing practical guidelines for reducing privacy risks in FL-based LLM training.
...
### [193] [StyleBench: Evaluating thinking styles in Large Language Models](https://arxiv.org/abs/2509.20868)
*Junyu Guo,Shangding Gu,Ming Jin,Costas Spanos,Javad Lavaei*

Main category: cs.LG

TL;DR: StyleBench is a comprehensive benchmark that systematically evaluates five reasoning styles (CoT, ToT, AoT, SoT, CoD) across diverse tasks and models, revealing that no single reasoning style is universally optimal and that strategy effectiveness depends on model scale and task type.


<details>
  <summary>Details</summary>
Motivation: The effectiveness of LLMs is heavily influenced by reasoning strategies, but the interplay between reasoning styles, model architecture, and task type remains poorly understood, necessitating systematic evaluation.

Method: Introduced StyleBench to assess five reasoning styles on five reasoning tasks using 15 open-source models from major families (LLaMA, Qwen, Mistral, Gemma, GPT-OSS, Phi, DeepSeek) ranging from 270M to 120B parameters.

Result: No single reasoning style is universally optimal. Search-based methods (AoT, ToT) excel in open-ended problems but require large-scale models, while concise styles (SoT, CoD) achieve efficiency gains on well-defined tasks. Smaller models frequently fail to follow instructions and default to guessing.

Conclusion: Reasoning strategy effectiveness is highly contingent on model scale and task type, providing a roadmap for selecting optimal strategies based on specific constraints. The benchmark is open-sourced for community use.
...
TL;DR: CLUE is a novel LLM unlearning framework that uses circuit discovery to precisely identify and separate forget and retain neurons, then applies targeted fine-tuning strategies for effective unlearning without catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Existing localization-based unlearning methods fail to disentangle neurons responsible for forgetting undesirable knowledge from those needed to retain essential skills, leading to uniform interventions that risk over-forgetting or incomplete erasure.

Method: Proposes Conflict-guided Localization for LLM Unlearning (CLUE) framework that identifies forget and retain circuits using mechanistic interpretability, transforms circuits into conjunctive normal forms, and applies targeted fine-tuning based on neuron assignments.
...
Conclusion: CLUE provides an effective solution for LLM unlearning by precisely identifying and targeting specific neurons, overcoming limitations of previous methods that treated forget and retain neurons as entangled.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for interpreting LLM hidden states rely on autoencoders with implicit regularization that lack explicit global sparsity guarantees, resulting in dense features that harm feature sparsity and atomization.

Method: BAE discretizes hidden activations to 1-bit via a step function with gradient estimation for backpropagation, enforcing minimal entropy on minibatches to promote feature independence and sparsity across instances.

Result: BAE enables reliable entropy estimation on binary activations for characterizing LLM inference dynamics and In-context Learning, and produces more interpretable features with better sparsity than baselines.

Conclusion: BAE effectively serves as a feature extractor by avoiding dense features while producing the largest number of interpretable features, confirming its effectiveness for feature untangling in LLMs.
...
TL;DR: This paper investigates the mechanism of In-context Learning (ICL) through information removal, showing that LMs encode queries into non-selective representations and that ICL selectively removes redundant information to focus on intended tasks.


<details>
  <summary>Details</summary>
Motivation: To understand the inner workings of ICL in language models, particularly how few-shot demonstrations enable task-specific performance improvements.

Method: The authors use information removal analysis by applying low-rank filters to hidden states and identify 'Denoising Heads' - specific attention heads responsible for selective information removal.

Result: ICL effectively simulates task-oriented information removal processes, and blocking identified Denoising Heads significantly degrades ICL performance, especially when correct labels are absent from demonstrations.

Conclusion: Information removal is a key mechanism underlying ICL, where demonstrations help LMs selectively filter out redundant information to focus on the intended task.
...
### [197] [DELTA-Code: How Does RL Unlock and Transfer New Programming Algorithms in LLMs?](https://arxiv.org/abs/2509.21016)
*Yiyou Sun,Yuhan Cao,Pohao Huang,Haoyue Bai,Hannaneh Hajishirzi,Nouha Dziri,Dawn Song*

Main category: cs.LG

TL;DR: DELTA-Code is a synthetic coding benchmark that evaluates whether LLMs can learn new reasoning strategies through RL, showing a grokking phase transition from near-zero to near-perfect accuracy after extended training.


<details>
  <summary>Details</summary>
Motivation: To determine if LLMs can genuinely acquire new algorithmic reasoning skills beyond their pre-trained capabilities, particularly whether they can learn and transfer skills to out-of-distribution problems.

Method: Uses templated problem generators to create controlled coding problem families, employs RL training with techniques like staged warm-up, experience replay, curriculum training, and verification-in-the-loop to enable learning on previously unsolvable problems.

Result: Models exhibit a striking grokking phenomenon - extended near-zero reward followed by abrupt near-perfect accuracy. Solid gains within problem families and for recomposed skills, but persistent weaknesses in transformative cases.
...
### [198] [ScaleDiff: Scaling Difficult Problems for Advanced Mathematical Reasoning](https://arxiv.org/abs/2509.21070)
...
<details>
  <summary>Details</summary>
Motivation: Current methods for automated synthesis of mathematical problems face challenges with high computational/API costs, complex prompting, and limited difficulty levels of generated problems.

Method: Uses an adaptive thinking model to identify difficult problems with a single forward pass, then trains a specialized generator (DiffGen-8B) on the filtered difficult data to produce new difficult problems at scale.

Result: Fine-tuning Qwen2.5-Math-7B-Instruct on ScaleDiff-Math dataset yields 11.3% performance increase and achieves 65.9% average accuracy on multiple benchmarks, outperforming recent strong LRMs.

Conclusion: The pipeline effectively transfers advanced reasoning capabilities without relying on larger, expensive teacher models, and shows clear scaling phenomenon in model performance as difficult problem quantity increases.
...
### [199] [Beyond Visual Similarity: Rule-Guided Multimodal Clustering with explicit domain rules](https://arxiv.org/abs/2509.20501)
*Kishor Datta Gupta,Mohd Ariful Haque,Marufa Kamal,Ahmed Rafi Hasan,Md. Mahfuzur Rahman,Roy George*

Main category: cs.LG

TL;DR: DARTVAE is a rule-guided multimodal clustering framework that incorporates domain-specific constraints into representation learning, using LLM-generated rules and knowledge graphs to produce more meaningful clusters than traditional methods.
...
Method: Extends VAE architecture by embedding explicit rules, semantic representations, and data-driven features into a unified latent space. Uses LLM-generated rules structured into knowledge graphs, enforced through loss function combining reconstruction, KL divergence, consistency, and violation penalties.
...
Conclusion: DARTVAE achieves more meaningful clustering than purely data-driven models by combining rule encodings with learned representations, though challenges remain with LLM rule hallucinations, rule conflicts, overfitting risks, and computational scaling.
...
### [200] [Bispectral OT: Dataset Comparison using Symmetry-Aware Optimal Transport](https://arxiv.org/abs/2509.20678)
*Annabel Ma,Kaiying Hou,David Alvarez-Melis,Melanie Weber*

Main category: cs.LG

TL;DR: Bispectral Optimal Transport is a symmetry-aware extension of discrete OT that uses bispectrum representations to preserve intrinsic data structure while removing group action variations, achieving better class preservation than naive feature OT.


<details>
  <summary>Details</summary>
Motivation: Standard OT alignments based solely on pairwise geometric distances can ignore intrinsic coherence structure in symmetry-rich settings, leading to poor class preservation.

Method: Introduces Bispectral Optimal Transport which compares elements using their bispectrum representation - a group Fourier invariant that preserves signal structure while removing group action variations.

Result: Empirically demonstrates that transport plans computed with Bispectral OT achieve greater class preservation accuracy than naive feature OT on benchmark datasets with visual symmetries.

Conclusion: Bispectral OT improves the quality of meaningful correspondences by capturing underlying semantic label structure while removing nuisance variations not affecting class or content.
...
<details>
  <summary>Details</summary>
Motivation: Existing data-free robustness distillation methods overlook robust fairness issues, leading to severe disparity in robustness across different categories, with students showing unstable robustness across attack targets.

Method: FERD uses robustness-guided class reweighting to synthesize more samples for less robust categories, generates Fairness-Aware Examples (FAEs) with uniformity constraints on feature predictions, and creates Uniform-Target Adversarial Examples (UTAEs) to distribute attack targets evenly.

Result: Extensive experiments on three datasets show FERD achieves state-of-the-art worst-class robustness, improving FGSM and AutoAttack robustness by 15.1% and 6.4% respectively on CIFAR-10 with MobileNet-V2.

Conclusion: FERD demonstrates superior performance in both robustness and fairness aspects, effectively addressing the robust fairness problem in data-free robustness distillation.
...
### [202] [CaTS-Bench: Can Language Models Describe Numeric Time Series?](https://arxiv.org/abs/2509.20823)
*Luca Zhou,Pratham Yashwante,Marshall Fisher,Alessio Sampieri,Zihao Zhou,Fabio Galasso,Rose Yu*

Main category: cs.LG

TL;DR: CaTS-Bench is a large-scale real-world benchmark for context-aware time series captioning, featuring 465k training and 105k test timestamps from 11 datasets, with numeric series, metadata, line-chart images, and captions, plus 460 multiple-choice questions for deeper reasoning.
...
Method: Created a scalable pipeline using an oracle LLM to generate reference captions, verified through factual checks, human indistinguishability studies, and diversity analyses, with 579 human-refined test captions for accuracy and style.

Result: Established CaTS-Bench as a reliable benchmark, benchmarked leading VLMs revealing their strengths and limitations, and proposed new tailored evaluation metrics.
...
### [203] [FHRFormer: A Self-supervised Transformer Approach for Fetal Heart Rate Inpainting and Forecasting](https://arxiv.org/abs/2509.20852)
*Kjersti Engan,Neel Kanwal,Anita Yeconia,Ladislaus Blacy,Yuda Munyaw,Estomih Mduma,Hege Ersdal*

Main category: cs.LG

TL;DR: This paper proposes a masked transformer-based autoencoder to reconstruct missing fetal heart rate (FHR) signals caused by sensor displacement during maternal movement, enabling better AI-based risk prediction for neonatal breathing assistance needs.


<details>
  <summary>Details</summary>
Motivation: Approximately 10% of newborns need breathing assistance at birth, and FHR monitoring is crucial for fetal well-being assessment. However, wearable FHR monitors suffer from signal dropouts due to maternal movement, limiting AI-based analysis for predicting neonatal risks.

Method: The authors developed a masked transformer-based autoencoder approach that captures both spatial and frequency components of FHR data to reconstruct missing signals, demonstrating robustness across varying missing data durations.

Result: The proposed method effectively handles missing FHR data while preserving spectral characteristics, outperforming traditional interpolation techniques, and can be used for both signal inpainting and forecasting.

Conclusion: This approach can be applied retrospectively to research datasets to support AI-based risk algorithm development and potentially integrated into wearable FHR monitoring devices for earlier and more robust risk detection.
...
### [204] [Sparse Representations Improve Adversarial Robustness of Neural Network Classifiers](https://arxiv.org/abs/2509.21130)
*Killian Steunou,Sigurd Saue,Théo Druilhe*

Main category: cs.LG

TL;DR: This paper revisits linear dimensionality reduction (PCA and SPCA) as a defense against adversarial attacks, showing that sparse PCA provides better robustness while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are vulnerable to adversarial perturbations, and the authors aim to explore simple, data-adapted defenses using linear dimensionality reduction methods.
...
Result: SPCA consistently outperforms PCA under strong white-box and black-box attacks while maintaining competitive clean accuracy, with theoretical analysis showing sparser projections reduce adversarial leverage.

Conclusion: Sparse projections (SPCA) provide better adversarial robustness than standard PCA, and this benefit persists beyond linear settings, offering a simple yet effective defense mechanism.
...
### [205] [A Unified Framework for Diffusion Model Unlearning with f-Divergence](https://arxiv.org/abs/2509.21167)
...
<details>
  <summary>Details</summary>
Motivation: Existing unlearning methods for text-to-image models rely on MSE minimization, which is limited. The paper aims to show that MSE is a special case of a broader f-divergence framework and analyze the benefits of using different divergences.

Method: Proposes a unified f-divergence-based framework for machine unlearning where any f-divergence can be used instead of just MSE. Analyzes how different divergences affect convergence and unlearning quality.

Result: Demonstrates that MSE is a special case of the f-divergence framework. Shows different f-divergences impact algorithm convergence and unlearning quality, allowing flexible trade-offs between aggressive unlearning and concept preservation.

Conclusion: The unified f-divergence framework provides a flexible paradigm for machine unlearning in diffusion models, enabling selection of optimal divergence for specific applications based on desired trade-offs.
...
### [206] [Differential-Integral Neural Operator for Long-Term Turbulence Forecasting](https://arxiv.org/abs/2509.21196)
*Hao Wu,Yuan Gao,Fan Xu,Fan Zhang,Qingsong Wen,Kun Wang,Xiaomeng Huang,Xian Wu*

Main category: cs.LG

TL;DR: DINO is a novel neural operator framework that decomposes turbulent flow modeling into local differential and global integral operators, achieving superior long-term forecasting stability and physical fidelity compared to state-of-the-art methods.
...
Method: Proposes DINO framework with parallel branches: a constrained convolutional network for local differential operator (proven to converge to derivative) and a Transformer architecture for global integral operator (learning data-driven global kernel).

Result: Significantly outperforms state-of-the-art models on 2D Kolmogorov flow benchmark, suppressing error accumulation over hundreds of timesteps while maintaining high fidelity in vorticity fields and energy spectra.
...
### [207] [Optimal Transport Based Hyperspectral Unmixing for Highly Mixed Observations](https://arxiv.org/abs/2509.20417)
*D. Doutsas,B. Figliuzzi*

Main category: eess.IV

TL;DR: A novel optimal transport-based approach for blind hyperspectral unmixing that uses OT to measure distribution discrepancy as regularization, improving endmember estimation in highly mixed data.


<details>
  <summary>Details</summary>
Motivation: To address the problem of highly mixed data in blind hyperspectral unmixing by better constraining abundance distributions.

Method: Uses optimal transport to measure discrepancy between targeted Dirichlet distribution and true abundance distribution, incorporating this as a regularization term in optimization. Demonstrated with unsupervised deep learning.

Result: The approach allows for better estimation of endmembers in highly mixed data and shows robustness to target abundance distribution choice.

Conclusion: The OT-based regularization method effectively improves blind hyperspectral unmixing performance in challenging mixed data scenarios.
...
### [208] [On Theoretical Interpretations of Concept-Based In-Context Learning](https://arxiv.org/abs/2509.20882)
*Huaze Tang,Tianren Peng,Shao-lun Huang*

Main category: cs.IT

TL;DR: This paper provides theoretical analysis of concept-based in-context learning (CB-ICL), explaining why and when it works well with few demonstrations, quantifying knowledge transfer, and offering insights for model pre-training and prompt engineering.
...
Method: The authors propose theoretical analyses on applying CB-ICL to ICL tasks, including quantifying knowledge transfer and developing similarity measures between demonstrations and query inputs.

Result: The theory explains CB-ICL performance with few demonstrations, quantifies knowledge leverage, provides guidance for pre-training and prompt engineering, and explores the impact of demonstration size and embedding dimensions.

Conclusion: Real-data experiments validate the practical usefulness of CB-ICL and the proposed theoretical framework, providing important insights for ICL applications.
