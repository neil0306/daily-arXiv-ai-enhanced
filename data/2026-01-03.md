...
TL;DR: Automated pipeline using OCR, LLM interpretation, and database linking to digitize historical professor records from Leiden University books, achieving 94% linkage accuracy with annotated data.


<details>
  <summary>Details</summary>
Motivation: To create an automated system for digitizing and harmonizing historical biographic data about Leiden University professors (1575-1815) from typewritten books with existing high-quality database records, addressing challenges in digital humanities.

Method: Combined OCR techniques, generative AI with decoding constraints for structured data extraction, and database linkage methods to process historical document images into digital format and link with existing records.
...
Conclusion: The automated pipeline successfully processes historical documents for digital humanities, with generative AI partially correcting OCR errors, and demonstrates practical applicability for harmonizing historical data with modern databases.
...
TL;DR: CAT framework evaluates LLMs by visualizing the interplay between accuracy and response consistency using CAR curves and CORE index metrics.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation focuses mainly on accuracy/benchmark scores, with consistency emerging as important for real-world deployment. The authors argue that while both dimensions should be evaluated independently, their interdependency needs consideration for more nuanced LLM assessment.
...
Result: Demonstrated framework across diverse generalist and domain-specific LLMs on multiple-choice benchmarks. Framework can be extended beyond MC tasks to support long-form, open-ended evaluations through adaptable scoring functions.

Conclusion: CAT provides a comprehensive framework for evaluating LLMs by considering both accuracy and consistency interdependencies, offering visual (CAR curves) and quantitative (CORE index) tools for nuanced assessment, applicable to various evaluation formats.
...
TL;DR: A framework for evaluating and improving consistency in LLM-generated structured outputs using STED (Semantic Tree Edit Distance) and consistency scoring, showing significant performance variations across LLMs.
...
Method: Combines two components: (1) STED - a novel similarity metric that balances semantic flexibility with structural strictness when comparing JSON outputs, and (2) a consistency scoring framework that aggregates multiple STED measurements across repeated generations to quantify reliability.

Result: STED achieves superior performance (0.86-0.90 similarity for semantic equivalents, 0.0 for structural breaks) compared to existing metrics. Framework application reveals significant LLM variations: Claude-3.7-Sonnet shows exceptional consistency even at high temperatures, while others like Claude-3-Haiku and Nova-Pro degrade substantially.
...
TL;DR: BanglaCodeAct is an agent-based framework for Bangla-to-Python code generation using multilingual LLMs with iterative self-correction, achieving 94.0% pass@1 on dev set and 71.6% on blind test set.


<details>
  <summary>Details</summary>
Motivation: LLMs excel at code generation from English prompts but this progress hasn't extended to low-resource languages like Bangla, creating a need for effective Bangla-to-Python translation methods.
...
Result: Qwen3-8B with BanglaCodeAct achieves best performance: 94.0% pass@1 accuracy on development set and 71.6% on blind test set of mHumanEval dataset for Bangla NL2Code.

Conclusion: The approach establishes a new benchmark for Bangla-to-Python translation and demonstrates the potential of agent-based reasoning for reliable code generation in low-resource languages.
...
<details>
  <summary>Details</summary>
Motivation: To create a real-world benchmark for testing pre-trained text-layout models under challenging conditions (noisy OCR, heterogeneous templates) in the safety-critical pharmaceutical domain, where document understanding is crucial but existing datasets may not adequately stress-test models.

Method: Created PharmaShip dataset with scanned pharmaceutical shipping documents covering three tasks: sequence entity recognition (SER), relation extraction (RE), and reading order prediction (ROP). Used entity-centric evaluation protocol and benchmarked five representative baselines (LiLT, LayoutLMv3-base, GeoLayoutLM and their RORE-enhanced variants) with standardized preprocessing, splits, and optimization.
...
Conclusion: PharmaShip establishes a controlled, reproducible benchmark for safety-critical document understanding in pharmaceuticals and highlights sequence-aware constraints as a transferable bias for structure modeling, with the dataset publicly released for further research.
...
TL;DR: LN-RP is a computational framework that uses stochastic noise injection to study how personas emerge in LLMs, revealing three stable persona modes with distinct entropy patterns and noise-induced phase transitions.


<details>
  <summary>Details</summary>
Motivation: To develop a reproducible method for studying reflexive generation, emergent behavior, and long-range linguistic coherence in large language models, particularly focusing on how noise influences persona emergence.

Method: The Luca-Noise Reflex Protocol injects stochastic noise seeds into the initial generation state and analyzes linguistic behavior across 152 generation cycles, tracking nonlinear transitions and entropy signatures.

Result: The study reveals three stable persona modes with distinct entropy signatures, demonstrates that external noise can reliably induce phase transitions in reflexive generation dynamics, and shows consistent persona retention with significant differences across modes (p < 0.01).

Conclusion: LN-RP provides a reproducible framework for studying reflexive generation, emergent behavior, and long-range linguistic coherence in LLMs, offering insights into noise-driven persona emergence and phase transitions.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM safety mechanisms focus on overtly dangerous content but fail to detect subtle threats where users disguise harmful intent through covert rephrasing that appears benign, creating a significant gap in safety training data.

Method: HarmTransform uses a multi-agent debate framework where multiple agents iteratively critique and refine harmful queries to generate high-quality, covert transformations that preserve malicious intent while appearing benign.
...
TL;DR: LLMs trained on poker hand history data learn internal representations of both deterministic game structure (hand ranks) and stochastic features (equity), developing their own models of partially observable environments without explicit instruction.


<details>
  <summary>Details</summary>
Motivation: Previous research showed LLMs can develop emergent world models in perfect information games, but it's unknown if they can do the same in partially observable environments like poker, which are more complex POMDPs.
...
Result: The model learned both deterministic structure (hand ranks) and stochastic features (equity) without explicit instruction. These representations are decodeable and correlate with theoretical belief states, suggesting LLMs learn their own representation of poker's stochastic environment.

Conclusion: LLMs can develop internal representations of partially observable stochastic environments like poker, extending their emergent world modeling capabilities beyond perfect information games to complex POMDP domains.
...
TL;DR: A two-stage framework combining targeted training procedures with reasoning-based inference to detect subtle, context-dependent sexist content that evades traditional methods.


<details>
  <summary>Details</summary>
Motivation: Sexist content online is becoming more subtle and context-dependent, making traditional detection methods ineffective. Current approaches struggle with overlapping linguistic, psychological, legal, and cultural dimensions that create mixed signals, label scarcity, class imbalance, and conceptual ambiguity.

Method: Two-stage framework: (1) Training stage uses class-balanced focal loss, class-aware batching, and post-hoc threshold calibration to handle label imbalance and noisy supervision. (2) Inference stage uses dynamic routing - high-confidence cases classified directly, uncertain cases escalated to Collaborative Expert Judgment (CEJ) module that prompts multiple personas and consolidates reasoning through a judge model.

Result: Achieves state-of-the-art results: +2.72% F1 improvement on EXIST 2025 Task 1.1, +4.48% improvement on EDOS Task A, and +1.30% improvement on EDOS Task B.

Conclusion: The proposed framework effectively addresses the combined challenges of underrepresentation, noise, and conceptual ambiguity in sexist content detection by unifying targeted training with reasoning-based inference, demonstrating significant performance improvements over existing methods.
...
TL;DR: A benchmark for evaluating service robots' ability to predict where household items are stored when given a scene and item query, with a hybrid vision-language model (NOAM) that approaches human performance.


<details>
  <summary>Details</summary>
Motivation: Domestic service robots struggle with commonsense reasoning about where everyday items are stored (in drawers, cabinets, etc.) when given simple commands like "Bring me a plate." Current vision and manipulation advances don't address this cognitive challenge.

Method: Introduced the Stored Household Item Challenge benchmark with two datasets: 100 real-world item-image pairs and 6,500 development pairs. Created NOAM (Non-visible Object Allocation Model), a hybrid pipeline that converts visual input to natural language descriptions of spatial context and visible containers, then uses LLMs (like GPT-4) to infer hidden storage locations.

Result: NOAM significantly outperformed baselines including random selection, vision-language pipelines, and leading multimodal models. It approaches human-level performance in predicting storage locations, demonstrating emergent commonsense reasoning capabilities.

Conclusion: The benchmark enables comparative evaluation of agent architectures for domestic reasoning tasks. NOAM's success shows best practices for deploying cognitively capable agents in home environments by combining structured scene understanding with LLM inference.
...
TL;DR: EASD enhances speculative decoding by adding entropy-based penalty to reject uncertain draft tokens, enabling performance beyond target LLM while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard speculative decoding is limited by excessive alignment between draft and target models, constraining performance to the target LLM's capabilities. The authors aim to overcome this limitation and potentially surpass target model performance.

Method: EASD extends standard speculative decoding with dynamic entropy-based penalty. It uses entropy of sampling distributions to quantify uncertainty, rejecting tokens when both models show high entropy with overlapping top-N predictions, then re-sampling with target LLM.

Result: EASD consistently outperforms existing SD methods and in most cases surpasses the target LLM itself across multiple reasoning benchmarks, while maintaining comparable efficiency to standard SD.

Conclusion: EASD provides a training-free enhancement to speculative decoding that not only accelerates LLM reasoning but can also improve overall performance beyond the target model's capabilities through draft-model verification and uncertainty-aware rejection.
...
<details>
  <summary>Details</summary>
Motivation: Current audio models require task-specific fine-tuning, unlike humans who generalize from few examples. The paper aims to apply GPT-3's scaling paradigm to audio to achieve similar generalization capabilities.

Method: Scale next-token prediction pretraining to over 100M hours of audio data (MiMo-Audio-7B-Base), then apply instruction-tuning with thinking mechanisms for both understanding and generation tasks.

Result: Achieves SOTA among open-source models on speech intelligence and audio understanding benchmarks. Demonstrates emergent capabilities: few-shot learning, generalization to unseen tasks (voice conversion, style transfer), and realistic speech continuation. MiMo-Audio-7B-Instruct approaches/surpasses closed-source models on multiple benchmarks.

Conclusion: Scaling audio pretraining enables emergent few-shot learning and strong generalization, similar to text models. The approach reduces need for task-specific fine-tuning and demonstrates powerful audio understanding and generation capabilities.
...
TL;DR: StressRoBERTa is a transfer learning model that uses continual training on clinically related mental health conditions (depression, anxiety, PTSD) to improve chronic stress detection in tweets, achieving 82% F1-score and outperforming previous systems.


<details>
  <summary>Details</summary>
Motivation: Chronic stress is a major public health issue, and social media platforms like Twitter provide valuable data for understanding stress experiences. There's a need for better automated methods to detect self-reported chronic stress in social media text, leveraging the comorbidity between stress and other mental health conditions.

Method: The approach uses cross-condition transfer learning: RoBERTa is continually trained on the Stress-SMHD corpus (108M words from users with self-reported depression, anxiety, and PTSD diagnoses), then fine-tuned on the SMM4H 2022 Task 8 dataset for chronic stress detection. This leverages the clinical relationship between stress and comorbid disorders.

Result: StressRoBERTa achieves 82% F1-score on chronic stress detection, outperforming the best shared task system by 3 percentage points (79% F1). It also shows +1% F1 improvement over vanilla RoBERTa. Evaluation on Dreaddit dataset shows 81% F1, demonstrating transfer from clinical mental health contexts to situational stress discussions.

Conclusion: Cross-condition transfer learning from clinically related mental health disorders (depression, anxiety, PTSD) provides stronger representations for chronic stress detection than general language models or broad mental health models. This approach effectively leverages the comorbidity between stress and related conditions for improved social media analysis.
...
<details>
  <summary>Details</summary>
Motivation: Little is known about how bias detection models arrive at their decisions or why they fail, despite their heavy use in journalistic analysis and media accountability. Understanding model interpretability is crucial for reliable deployment.
...
Result: Both models attend to similar categories of evaluative language but differ substantially in signal integration. The bias detector model assigns stronger internal evidence to false positives than true positives, leading to systematic over-flagging of neutral content. The domain-adaptive model exhibits better alignment between attribution patterns and prediction outcomes, producing 63% fewer false positives. Model errors arise from discourse-level ambiguity rather than explicit bias cues.

Conclusion: Interpretability-aware evaluation is crucial for bias detection systems, and architectural/training choices critically affect model reliability and deployment suitability in journalistic contexts. Domain adaptation improves alignment between attribution strength and prediction correctness.
...
<details>
  <summary>Details</summary>
Motivation: While expanded context windows in LLMs enable broader context retrieval for QA, longer contexts introduce more irrelevant information that degrades model performance. Need to balance incorporating relevant knowledge while reducing irrelevant information.

Method: Design adaptive prompting strategy that splits retrieved information into smaller chunks and sequentially prompts LLM to answer questions using each chunk. Adjusting chunk size allows trade-off between relevant information inclusion and irrelevant information reduction.

Result: Experimental results on three open-domain QA datasets show adaptive strategy matches standard prompting performance while using fewer tokens. Analysis reveals LLMs often generate incorrect answers instead of declining when information is insufficient, which is a major error source.

Conclusion: Adaptive prompting effectively addresses irrelevant information in long contexts for retrieval-augmented QA. Key finding: LLMs need enhanced ability to decline requests when faced with inadequate information, highlighting important research direction.
...
<details>
  <summary>Details</summary>
Motivation: Leverage mechanistic interpretability findings that intermediate attention layers encode token-level hypotheses to create more plausible and internally consistent adversarial examples than traditional prompt-based or gradient-based attacks.

Method: Extract tokens from intermediate attention layers as adversarial perturbations, using LLaMA-3.1-Instruct-8B as both generator and evaluator on ArgQuality dataset for argument quality assessment.

Result: Attention-based adversarial examples cause measurable performance drops while maintaining semantic similarity, but substitutions from certain layers/positions introduce grammatical degradation limiting practical effectiveness.

Conclusion: Intermediate-layer representations show promise as principled adversarial sources for stress-testing LLM evaluation pipelines, but current limitations in grammatical quality need addressing.
...
TL;DR: Multi-retriever RAG system with domain-specific training improves financial QA performance, achieving SOTA but still below human expert level.


<details>
  <summary>Details</summary>
Motivation: Financial numerical reasoning QA tasks are challenging due to lack of domain knowledge in finance and complex multi-step numeric reasoning, despite recent LLM advances.
...
Result: Domain-specific training with SecBERT helped neural symbolic model surpass baseline (FinQA). Best prompt-based LLM achieved SOTA with >7% improvement, though still below human performance. Larger models benefit more from external knowledge than hallucination loss.

Conclusion: Domain-specific training enhances financial QA performance, with latest LLMs showing improved numerical reasoning for few-shot learning. Trade-off exists between hallucination loss and external knowledge gains, favoring larger models.
...
TL;DR: This paper presents an analytics framework that separates student content from teacher grading tendencies to make scoring judgments more transparent and auditable, showing teacher priors heavily influence grades while content-only models are weaker.


<details>
  <summary>Details</summary>
Motivation: Current automated scoring conflates student content with teacher grading tendencies, making it difficult to distinguish what students actually wrote from how teachers grade. There's a need to separate content signals from rater tendencies to make judgments visible and auditable for learning analytics.

Method: Used de-identified ASSISTments math responses to model teacher histories as dynamic priors, derived text representations from sentence embeddings, incorporated centering and residualization to mitigate prompt and teacher confounds, used temporally-validated linear models to quantify signal contributions, and created projections to surface model disagreements for qualitative inspection.

Result: Teacher priors heavily influence grade predictions; best results when combining priors with content embeddings (AUC~0.815), while content-only models remain above chance but substantially weaker (AUC~0.626). Adjusting for rater effects sharpens residual content representation, retains more informative embedding dimensions, and reveals cases where semantic evidence supports understanding vs. surface-level response differences.

Conclusion: The framework transforms embeddings from mere features into learning analytics for reflection, enabling teachers and researchers to examine where grading practices align or conflict with evidence of student reasoning and learning, providing a practical pipeline for transparent assessment.
...
TL;DR: HGMem introduces a hypergraph-based memory mechanism for multi-step RAG that captures higher-order correlations between facts, improving global reasoning in extended contexts.


<details>
  <summary>Details</summary>
Motivation: Existing RAG memory systems function as passive storage of isolated facts, overlooking crucial high-order correlations that could provide stronger guidance for reasoning. This leads to fragmented reasoning and weak global sense-making capacity in extended contexts.

Method: HGMem represents memory as a hypergraph where hyperedges correspond to distinct memory units, enabling progressive formation of higher-order interactions. This creates an integrated knowledge structure that connects facts and thoughts around the focal problem.

Result: Extensive experiments on challenging datasets show HGMem consistently improves multi-step RAG and substantially outperforms strong baseline systems across diverse tasks.

Conclusion: Hypergraph-based memory extends memory beyond simple storage into a dynamic, expressive structure that enhances complex reasoning and global understanding in multi-step RAG systems.
...
<details>
  <summary>Details</summary>
Motivation: To address the computational challenges of full attention in long-context scenarios, enabling efficient processing of up to 1 million tokens for applications like retrieval-augmented generation and tool-integrated reasoning.

Method: Introduces LongCat ZigZag Attention (LoZA), a sparse attention scheme that can be applied to existing full-attention models to create sparse versions. Specifically applied to LongCat-Flash during mid-training to create LongCat-Flash-Exp.

Result: Achieves significant speed-ups in both prefill-intensive (retrieval-augmented generation) and decode-intensive (tool-integrated reasoning) cases. Enables processing of up to 1 million tokens efficiently.

Conclusion: LoZA enables efficient long-context foundation models that can process up to 1 million tokens, supporting long-term reasoning and long-horizon agentic capabilities with limited compute budget.
...
TL;DR: CEC-Zero is a zero-supervision reinforcement learning framework for Chinese spelling correction that enables LLMs to self-correct without labeled data, outperforming supervised methods by 10-13 F1 points.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs and supervised methods for Chinese spelling correction lack robustness to novel errors and require costly annotations, creating a need for a label-free approach that can handle real-world noisy text.

Method: CEC-Zero uses reinforcement learning with PPO optimization: 1) synthesizes errorful inputs from clean text, 2) computes cluster-consensus rewards via semantic similarity and candidate agreement, and 3) optimizes the correction policy without supervision.
...
TL;DR: RISE is an unsupervised framework that discovers interpretable reasoning vectors in LLM activations using sparse auto-encoders, enabling analysis and control of reasoning behaviors without human supervision.
...
Result: SAEs uncover disentangled features corresponding to interpretable behaviors (reflection, backtracking), reveal structural properties (response length), and enable discovery of novel behaviors beyond human supervision. Targeted interventions on SAE-derived vectors can controllably amplify or suppress specific reasoning behaviors.

Conclusion: Unsupervised latent discovery using SAEs shows potential for both interpreting and controllably steering reasoning in LLMs, enabling analysis of internal mechanisms during reasoning processes.
...
Method: Developed WISE framework to benchmark 8 lightweight transformer models and 2 baselines on 20,000 samples from Fakeddit dataset using stratified 5-fold cross-validation with comprehensive metrics (accuracy, precision, recall, F1, ROC-AUC, PR-AUC, MCC, Brier score, ECE).

Result: MiniLM achieved highest accuracy (87.58%), RoBERTa-base had best ROC-AUC (95.42%) and strong accuracy (87.36%), DistilBERT offered best efficiency-accuracy trade-off (86.28% accuracy, 93.90% ROC-AUC). Statistical tests confirmed significant performance differences between models.

Conclusion: Lightweight models can match or exceed baseline performance, providing actionable insights for deploying effective misinformation detection systems in real-world, resource-constrained environments.
...
TL;DR: iCLP enables LLMs to generate latent plans for implicit reasoning, improving accuracy, efficiency, and cross-domain generalization while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Explicit textual plans for LLMs suffer from hallucinations and high task diversity. Inspired by human implicit cognition, the authors aim to create compact, generalized patterns that guide reasoning without explicit verbalization.
...
Result: LLMs can plan in latent space while reasoning in language space, showing significant improvements in accuracy and efficiency on mathematical reasoning and code generation tasks, with strong cross-domain generalization while preserving CoT interpretability.

Conclusion: iCLP successfully enables LLMs to perform implicit planning through latent representations, combining the benefits of efficient subconscious guidance with the interpretability of explicit reasoning chains.
...
<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used in high-stakes domains (healthcare, law, finance) but suffer from overconfident errors, degradation under input shifts, and lack of reliable uncertainty estimates. Existing evaluations are fragmented and don't provide comprehensive reliability assessment.

Method: Developed Composite Reliability Score (CRS) - a unified framework integrating calibration, robustness, and uncertainty quantification into a single interpretable metric. Tested on 10 leading open-source LLMs across 5 QA datasets under baseline conditions, perturbations, and various calibration methods.

Result: CRS provides stable model rankings, uncovers hidden failure modes missed by single metrics, and reveals that the most reliable systems balance accuracy, robustness, and calibrated uncertainty.

Conclusion: CRS offers a comprehensive, interpretable framework for assessing LLM reliability in decision-critical applications, showing that effective reliability requires balancing multiple dimensions rather than optimizing single metrics.
...
<details>
  <summary>Details</summary>
Motivation: To develop high-performance, parameter-efficient machine translation models that can compete with and even surpass much larger models and commercial translation services, while supporting advanced translation features.

Method: Multi-stage holistic training framework combining general and MT-oriented pre-training, supervised fine-tuning, on-policy distillation, and reinforcement learning.

Result: HY-MT1.5-1.8B outperforms larger open-source models (Tower-Plus-72B, Qwen3-32B) and commercial APIs (Microsoft Translator, Doubao Translator), achieving ~90% of Gemini-3.0-Pro performance. HY-MT1.5-7B achieves 95% of Gemini-3.0-Pro on Flores-200 and surpasses it on WMT25 and Mandarin-minority language benchmarks.

Conclusion: The HY-MT1.5 series provides highly competitive, robust translation solutions with remarkable parameter efficiency, establishing new state-of-the-art performance for their size classes while supporting advanced translation constraints.
...
TL;DR: A guide to help researchers train Hugging Face models on AWS SageMaker by centralizing essential information to overcome cloud platform learning curves.
...
Method: The paper provides a centralized guide/demo that consolidates essential information needed to train Hugging Face models on AWS SageMaker from scratch, addressing knowledge gaps in existing documentation.

Result: A comprehensive resource that democratizes cloud adoption by providing researchers with the necessary information to successfully train their first Hugging Face model on AWS SageMaker.

Conclusion: By centralizing fragmented information and lowering the learning curve, this demo paper helps democratize access to cloud computing for LLM research, enabling more researchers to leverage AWS SageMaker for training Hugging Face models.
...
TL;DR: A framework for inference-time control of masked diffusion language models using activation steering vectors computed from contrastive examples, enabling efficient attribute modulation without simulating denoising trajectories.


<details>
  <summary>Details</summary>
Motivation: Masked diffusion language models have shown competitive performance but lack effective mechanisms for inference-time control and steering, which limits their practical application for controlled text generation.
...
Conclusion: The framework provides an efficient inference-time control mechanism for MDLMs, enabling practical steering of text generation without computational overhead of simulating denoising trajectories.
...
TL;DR: The paper proposes LEWM, a Large Emotional World Model that incorporates emotional states alongside visual observations and actions to better predict emotion-driven social behaviors, addressing the gap in existing LLMs that lack systematic emotional modeling.


<details>
  <summary>Details</summary>
Motivation: Existing Large Language Models focus primarily on physical-world regularities but lack systematic exploration of emotional factors, which are crucial for understanding human decision-making and world dynamics. The authors demonstrate that removing emotionally relevant information degrades reasoning performance, highlighting the importance of emotion in world understanding.

Method: The authors propose LEWM (Large Emotional World Model) inspired by theory of mind. They construct the Emotion-Why-How (EWH) dataset that integrates emotion into causal relationships, enabling reasoning about why actions occur and how emotions drive future world states. LEWM explicitly models emotional states alongside visual observations and actions, allowing prediction of both future states and emotional transitions.
...
Conclusion: The paper successfully demonstrates the importance of incorporating emotional factors into world models and proposes an effective framework (LEWM) that improves prediction of emotion-driven behaviors while preserving general world modeling capabilities.
...
TL;DR: TeleChat3-MoE is a series of large language models with MoE architecture (105B-1T+ parameters) trained on Ascend NPU clusters, with this report focusing on the training infrastructure enabling reliable and efficient scaling to frontier model sizes.


<details>
  <summary>Details</summary>
Motivation: To enable reliable and efficient scaling of large language models to frontier sizes (up to over 1 trillion parameters) on Ascend NPU hardware ecosystems, addressing challenges in numerical accuracy, performance optimization, and cluster-level bottlenecks during large-scale training.
...
Conclusion: The presented training infrastructure enables reliable and efficient scaling to frontier model sizes on Ascend NPU clusters, with systematic approaches to numerical accuracy, performance optimization, and cluster-level bottlenecks that support the development of large-scale language models up to over 1 trillion parameters.
...
TL;DR: MedKGI: A clinical diagnostic framework using medical knowledge graphs to improve LLM reasoning, question efficiency, and dialogue coherence in medical diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with clinical diagnostic reasoning due to three key limitations: generating hallucinated medical content, asking inefficient questions, and losing coherence in multi-turn dialogues, which prevents them from emulating real clinical hypothesis-driven reasoning.
...
Conclusion: MedKGI successfully addresses key limitations of LLMs in clinical diagnosis by grounding reasoning in verified medical knowledge, optimizing question selection, and maintaining dialogue coherence, making it a promising framework for clinical diagnostic applications.
...
TL;DR: Researchers introduce LAILA, the largest publicly available Arabic Automated Essay Scoring dataset with 7,859 essays annotated across seven dimensions, addressing the lack of Arabic AES resources.


<details>
  <summary>Details</summary>
Motivation: Arabic AES research has been limited due to the lack of publicly available datasets, creating a gap in resources for developing robust scoring systems for Arabic language essays.

Method: Created LAILA dataset with 7,859 essays annotated with holistic and trait-specific scores across seven dimensions (relevance, organization, vocabulary, style, development, mechanics, grammar). Provided benchmark results using state-of-the-art Arabic and English models in both prompt-specific and cross-prompt settings.

Result: LAILA is now the largest publicly available Arabic AES dataset, filling a critical need in Arabic AES research and enabling development of robust scoring systems through comprehensive annotations and benchmark results.

Conclusion: LAILA addresses the critical resource gap in Arabic AES research by providing a comprehensive, publicly available dataset that supports the development and benchmarking of automated essay scoring systems for Arabic language.
...
TL;DR: Pat-SPECTER model outperforms other language similarity models in predicting credible patent-paper citations, and reveals US patents cite less semantically similar papers due to duty of candor requirements.


<details>
  <summary>Details</summary>
Motivation: Need for a language similarity model that works effectively with both patents and scientific publications to identify credible cross-domain citations and understand citation patterns.

Method: Developed Pat-SPECTER by fine-tuning SPECTER2 model on patents, then evaluated eight language similarity models in horse race-style evaluation using Patent-Paper Citations prediction as benchmark.

Result: Pat-SPECTER performed best among tested models. The model revealed US patents cite papers that are semantically less similar than in other large jurisdictions, likely due to duty of candor requirements.

Conclusion: Pat-SPECTER is an effective tool for patent-paper similarity analysis, with practical applications in separating and predicting patent-paper pairs, and provides insights into jurisdictional citation differences.
...
Method: DATAMASK treats data selection as a mask learning problem using policy gradient optimization: iteratively samples data masks, computes policy gradients based on predefined objectives (quality and diversity), updates mask sampling logits, with acceleration enhancements that reduce selection time by 98.9% vs greedy algorithms.

Result: Selected 10% subset (FineWeb-Mask) from 15 trillion-token FineWeb dataset; achieved 3.2% improvement on 1.5B dense model and 1.9% on 7B MoE model across 12 diverse tasks; enables joint learning exploration at trillion-scale.

Conclusion: DATAMASK enables efficient joint optimization of quality and diversity metrics for large-scale pre-training data selection, overcoming limitations of single-metric approaches and significantly improving model performance with minimal data.
...
TL;DR: LLMs show moderate success in qualitative EU Taxonomy compliance tasks but fail at quantitative KPI prediction; they're not ready for full automation but can assist human experts.


<details>
  <summary>Details</summary>
Motivation: The EU Taxonomy compliance process is manual and resource-intensive, but research on automating it with LLMs is hindered by lack of public benchmark datasets.

Method: Created a structured dataset from 190 corporate reports with ground-truth economic activities and KPIs, then systematically evaluated LLMs on core compliance workflow including qualitative activity identification and quantitative KPI prediction.
...
<details>
  <summary>Details</summary>
Motivation: Complex reasoning problems involve implicit spatial, geometric, and structural relationships that are not explicitly encoded in text. Text-based reasoning struggles to represent global structural constraints in complex settings, limiting performance on problems requiring understanding of visual/spatial relationships.

Method: FIGR integrates active visual thinking into multi-turn reasoning via end-to-end reinforcement learning. It externalizes intermediate structural hypotheses by constructing visual representations during problem solving, and adaptively regulates when and how visual reasoning should be invoked to enable more stable and coherent reasoning over global structural properties.

Result: FIGR outperforms strong text-only chain-of-thought baselines on challenging mathematical reasoning benchmarks. It improves the base model by 13.12% on AIME 2025 and 11.00% on BeyondAIME, demonstrating significant performance gains through figure-guided multimodal reasoning.

Conclusion: Figure-guided multimodal reasoning enhances the stability and reliability of complex reasoning by enabling better handling of implicit spatial and structural relationships that are difficult to capture from text alone. The adaptive visual reasoning approach represents a promising direction for improving reasoning capabilities in complex domains.
...
<details>
  <summary>Details</summary>
Motivation: Previous financial LLMs focused mainly on knowledge enhancement, but the increasing complexity of financial services requires models with both domain knowledge AND robust financial reasoning/agentic capabilities.
...
Conclusion: The fine-grained, progressive post-training methodology is validated and poised to become a mainstream paradigm for various industrial-enhanced LLMs, addressing the need for comprehensive financial capabilities beyond just knowledge.
...
TL;DR: WM-SAR reformulates sarcasm understanding as world model reasoning, using specialized LLM agents to decompose literal meaning, context, normative expectations, and intentions, then quantifies inconsistency between literal evaluation and expectations to achieve interpretable sarcasm detection.
...
Method: Proposes WM-SAR (World Model inspired SArcasm Reasoning) that decomposes literal meaning, context, normative expectation, and intention into specialized LLM-based agents. Quantifies discrepancy between literal evaluation and normative expectation as inconsistency score, combines with intention score, and integrates via lightweight Logistic Regression for final sarcasm probability.

Result: WM-SAR consistently outperforms existing deep learning and LLM-based methods on representative sarcasm detection benchmarks. Ablation studies show integrating semantic inconsistency and intention reasoning is essential for effective sarcasm detection.
...
Conclusion: The human-inspired approach of selective attention through contrastive learning provides richer document representations while being more computationally efficient, addressing key limitations of current long document representation methods.
...
TL;DR: Comparison of text summarization approaches for less-resourced languages shows multilingual fine-tuned mT5 outperforms most methods including zero-shot LLMs, with variation across LLMs and potential reliability issues with LLM-as-judge evaluation.


<details>
  <summary>Details</summary>
Motivation: Automatic text summarization has achieved high performance in high-resourced languages like English, but comparatively less attention has been given to summarization in less-resourced languages, creating a research gap that needs to be addressed.

Method: The study compares multiple approaches: zero-shot prompting of various sized LLMs, fine-tuning smaller models (mT5) with and without three data augmentation approaches and multilingual transfer, and an LLM translation pipeline (translate-summarize-translate back). Evaluation uses five different metrics.

Result: Key findings: 1) Variation in performance across LLMs even with similar parameter sizes, 2) Multilingual fine-tuned mT5 baseline outperforms most other approaches including zero-shot LLM performance for most metrics, 3) LLM as judge may be less reliable for evaluating less-resourced languages.

Conclusion: For less-resourced language summarization, multilingual fine-tuning of smaller models like mT5 is more effective than zero-shot LLM approaches, and evaluation methods need careful consideration due to potential reliability issues with LLM-based assessment in low-resource settings.
...
TL;DR: Researchers developed an AI-powered IELTS writing revision platform with automated scoring and feedback, showing significant score improvements but with limitations for higher-level essays.
...
Method: Used Design-Based Research (DBR) cycles to iteratively develop a platform with separate conversational guidance and writing interfaces. Progressed from rule-based to transformer-based (DistilBERT with regression head) scoring, then implemented adaptive feedback.

Result: Transformer model achieved MAE of 0.66 and positive R². Adaptive feedback showed statistically significant score improvements (mean +0.060 bands, p=0.011, Cohen's d=0.504), though effectiveness varied by revision strategy.

Conclusion: Automated feedback works best as a supplement to human instruction, with conservative surface-level corrections being more reliable than aggressive structural changes. Challenges remain for higher-band essays, and future work needs longitudinal studies with real candidates and examiner validation.
...
TL;DR: This paper introduces paragraph segmentation for speech transcripts, creates two new benchmarks (TEDPara and YTSegPara), proposes a constrained-decoding method for LLMs to insert paragraph breaks while preserving original transcripts, and develops MiniSeg model that achieves SOTA accuracy and hierarchical chapter/paragraph prediction.
...
Method: 1) Created TEDPara (human-annotated TED talks) and YTSegPara (YouTube videos with synthetic labels) benchmarks; 2) Proposed constrained-decoding formulation for LLMs to insert paragraph breaks while preserving original transcripts; 3) Developed MiniSeg model for paragraph segmentation that can be extended hierarchically for joint chapter and paragraph prediction.

Result: MiniSeg achieves state-of-the-art accuracy on paragraph segmentation. When extended hierarchically, it can jointly predict chapters and paragraphs with minimal computational cost. The benchmarks and methods establish paragraph segmentation as a standardized, practical task in speech processing.

Conclusion: The paper fills three gaps in speech processing and text segmentation: establishes first benchmarks for paragraph segmentation in speech domain, proposes a constrained-decoding method for faithful evaluation, and shows that compact models can achieve SOTA performance while enabling hierarchical structure prediction.
...
TL;DR: LLM safety alignment doesn't transfer zero-shot across languages; study reveals complex interference patterns where safety depends on language-temporal variable intersections, not simple degradation in low-resource settings.
...
Method: Systematic audit of GPT-5.1, Gemini 3 Pro, and Claude 4.5 Opus using HausaSafety dataset (West African threat scenarios). 2x4 factorial design across 1,440 evaluations testing language (English vs. Hausa) and temporal framing interactions.

Result: Found Complex Interference mechanism instead of simple degradation. Reverse Linguistic effect (Claude safer in Hausa than English). Profound Temporal Asymmetry: past-tense bypassed defenses (15.6% safe), future-triggered hyper-conservative refusals (57.2% safe). 9.2x disparity between safest/vulnerable configurations.

Conclusion: Models rely on superficial heuristics, creating Safety Pockets that expose Global South users. Propose Invariant Alignment paradigm shift for safety stability across linguistic and temporal shifts.
...
Method: HaluNet integrates multi-granular token-level uncertainties by combining semantic embeddings with probabilistic confidence and distributional uncertainty. It uses a multi-branch architecture that adaptively fuses model knowledge with uncertainty expressed in outputs for efficient one-pass detection.
...
TL;DR: KCL is a Korean legal reasoning benchmark that separates reasoning ability from domain knowledge using question-level precedents, with multiple-choice and essay components.
...
Method: Developed KCL benchmark with two components: KCL-MCQA (283 multiple-choice questions with 1,103 precedents) and KCL-Essay (169 open-ended questions with 550 precedents and 2,739 rubrics). Provides question-level supporting precedents to disentangle reasoning from knowledge.

Result: Evaluation of 30+ models shows large performance gaps, especially in KCL-Essay. Reasoning-specialized models consistently outperform general-purpose counterparts.

Conclusion: KCL successfully benchmarks legal reasoning ability separate from domain knowledge, revealing current model limitations and providing open resources for future research.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning with long chain-of-thought is inefficient - high latency from excessive tokens and unstable reasoning (alternating between underthinking and overthinking). Need to improve reasoning efficiency without training.

Method: CREST has two components: (1) offline calibration to identify cognitive heads (correlated with verification/backtracking behaviors) and derive head-specific steering vectors, (2) inference-time procedure that rotates hidden representations to suppress components along those vectors.
...
Conclusion: CREST offers a simple, effective training-free pathway to faster, more reliable LLM reasoning by adaptively suppressing unproductive reasoning behaviors through targeted attention head interventions.
...
TL;DR: Youtu-LLM is a 1.96B parameter lightweight language model pre-trained from scratch with native agentic intelligence, featuring long-context support and a progressive training curriculum from commonsense to STEM/agentic tasks.


<details>
  <summary>Details</summary>
Motivation: To create a lightweight yet powerful language model that harmonizes computational efficiency with native agentic intelligence, moving beyond typical small models that rely on distillation and lack systematic reasoning/planning capabilities.

Method: 1) Compact MLA architecture with STEM-oriented vocabulary and 128k context window; 2) Multi-stage training on 11T tokens using a "Commonsense-STEM-Agent" curriculum; 3) Scalable agentic mid-training with diverse data construction for math, coding, and tool-use trajectories.

Result: Youtu-LLM sets new SOTA for sub-2B LLMs, achieving competitive performance on general benchmarks against larger models and significantly surpassing existing baselines on agent-specific tasks, demonstrating strong intrinsic agentic capabilities.

Conclusion: Lightweight models can possess strong intrinsic agentic capabilities when pre-trained from scratch with systematic architecture and curriculum design, offering efficient yet powerful alternatives to larger models for agentic applications.
...
TL;DR: LLMs are overconfident in predicting their task success, with overconfidence worsening during multi-step tasks. Some LLMs can learn from failure experiences to improve decision-making, but all models make rational decisions based on their flawed probability estimates.
...
Method: Testing various LLMs on their ability to predict task success, examining prediction accuracy during multi-step agentic tasks, and evaluating whether in-context failure experiences lead to improved decision-making about pursuing tasks.

Result: All tested LLMs are overconfident but have better-than-random discriminatory power. Newer/larger models don't generally have better discriminatory power (except Claude models). Overconfidence worsens during multi-step tasks, and reasoning LLMs perform similarly or worse than non-reasoning ones. Some LLMs reduce overconfidence with failure experiences, improving decision-making, but all models make rational decisions based on their flawed probability estimates.

Conclusion: Current LLM agents lack awareness of their own capabilities, which hinders their performance. This capability awareness issue has implications for AI misuse and misalignment risks.
...
TL;DR: R-Debater is an agentic framework for multi-turn debates using argumentative memory and retrieval to maintain stance consistency and evidence-based argumentation.


<details>
  <summary>Details</summary>
Motivation: Current debate systems lack the ability to maintain consistent stances across multiple turns and effectively recall and adapt prior arguments with supporting evidence, which is crucial for coherent and persuasive debates.

Method: Integrates a debate knowledge base for retrieving case-like evidence and prior debate moves with a role-based agent that composes coherent utterances across turns, grounded in rhetoric and memory studies.

Result: Achieves higher single-turn and multi-turn scores compared to strong LLM baselines on ORCHID debates, with human evaluation confirming better consistency and evidence use.

Conclusion: Combining retrieval grounding with structured planning yields more faithful, stance-aligned, and coherent debates across multiple turns.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-turn conversation evaluation relies on costly human assessment. While multi-turn reward models offer a scalable alternative, existing training data (single-turn contrasts) fails to capture the nuances of multi-turn interactions, limiting evaluation quality.
...
Result: The MUSIC-augmented RM outperforms baseline methods, achieving higher alignment with advanced proprietary LLM judges on multi-turn conversations while maintaining performance on standard single-turn RM benchmarks.

Conclusion: Incorporating multi-turn contrasts is crucial for building robust multi-turn reward models, and MUSIC provides an effective unsupervised approach to create such training data, improving automated evaluation of multi-turn LLM conversations.
...
TL;DR: BIOME-Bench: A benchmark for evaluating LLMs on biomolecular interaction inference and multi-omics pathway mechanism elucidation, revealing current models' limitations in these tasks.


<details>
  <summary>Details</summary>
Motivation: Current pathway enrichment workflows have structural limitations (curation lag, functional redundancy, limited sensitivity), and while LLMs show promise for improving multi-omics interpretation, there's no standardized benchmark for end-to-end evaluation, hindering reproducible progress.

Method: Introduce BIOME-Bench constructed via a rigorous four-stage workflow to evaluate two core LLM capabilities: 1) Biomolecular Interaction Inference, and 2) end-to-end Multi-Omics Pathway Mechanism Elucidation. Develop evaluation protocols and conduct comprehensive experiments across multiple contemporary models.

Result: Existing models exhibit substantial deficiencies in multi-omics analysis, struggling to reliably distinguish fine-grained biomolecular relation types and generate faithful, robust pathway-level mechanistic explanations.

Conclusion: BIOME-Bench provides a standardized benchmark for evaluating LLMs in multi-omics analysis, revealing current limitations and establishing a foundation for reproducible progress in this important biomedical application area.
...
TL;DR: Semi-SMDNet: A semi-supervised multilingual depression detection framework using teacher-student pseudo-labeling, ensemble learning, and data augmentation to address limited labeled data across languages.


<details>
  <summary>Details</summary>
Motivation: Depression detection from social media is challenging due to varying language styles, informal expressions, and lack of annotated data in many languages, creating a need for robust multilingual approaches with limited labeled resources.

Method: Proposes Semi-SMDNet with teacher-student pseudo-labeling, ensemble learning via multiple teacher models with soft voting, uncertainty-based threshold filtering for low-confidence pseudo-labels, and confidence-weighted training focusing on reliable samples.
...
Conclusion: The framework is suitable for scalable, cross-language mental health monitoring where labeled resources are limited, offering a robust solution for multilingual depression detection from social media.
...
TL;DR: LLMs show improved reasoning with intermediate steps, but current evaluations ignore computational costs. This paper conducts test-time-compute aware evaluation of open-source LLMs, finding MoE architectures balance performance/efficiency best, identifying compute saturation points where accuracy gains diminish.


<details>
  <summary>Details</summary>
Motivation: Current literature overlooks the computational burden of generating long reasoning sequences in LLMs. For industrial applications, model selection depends on both accuracy and resource constraints/inference costs, necessitating a test-time-compute aware evaluation approach.

Method: Conducted test-time-compute aware evaluation of contemporary and older open-source LLMs, mapping their Pareto frontiers across math- and reasoning-intensive benchmarks. Analyzed Mixture of Experts (MoE) architecture performance and traced Pareto efficiency trajectory over time.

Result: Mixture of Experts (MoE) architecture emerges as strong candidate for balancing performance and efficiency. Identified saturation point for inference-time compute where accuracy gains diminish beyond certain threshold. Derived emergent trend regarding accuracy gain per unit of compute over time.

Conclusion: While extended reasoning capabilities are beneficial, they cannot overcome intrinsic model limitations regarding specific complexities. There exists a compute saturation point where additional reasoning steps yield diminishing returns, highlighting the importance of test-time-compute aware evaluation for practical LLM deployment.
...
<details>
  <summary>Details</summary>
Motivation: As NLP systems become more pervasive, integrating ethical considerations into NLP education is essential, but challenging due to the field's rapid evolution and the need to foster critical thinking beyond technical training.

Method: Pedagogical approach grounded in active learning through interactive sessions, hands-on activities, and "learning by teaching" methods, refined over four years across different institutions, educational levels, and interdisciplinary backgrounds.

Result: Course has been successfully adapted across institutions and produced many reusable products including teaching materials and actual educational products created by students for diverse audiences.
...
TL;DR: The paper introduces "triangulation" - a causal standard for evaluating mechanistic explanations in multilingual language models that requires necessity, sufficiency, and invariance across reference families (predicate-preserving variants).


<details>
  <summary>Details</summary>
Motivation: Multilingual language models show unpredictable behavior across languages, scripts, and cultures, but current mechanistic explanations lack rigorous causal standards that survive interventions and cross-reference across environments.

Method: Formalize reference families as predicate-preserving variants, introduce triangulation acceptance rule (necessity, sufficiency, invariance), use automatic circuit discovery to find candidate subgraphs, and evaluate them through triangulation across multiple model families, language pairs, and tasks.

Result: Triangulation provides a falsifiable standard that filters spurious circuits that pass single-environment tests but fail cross-lingual invariance, connecting to causal abstraction theory and pragmatic interpretability.

Conclusion: The proposed triangulation framework offers a rigorous causal standard for mechanistic claims in multilingual models, ensuring explanations survive interventions and maintain invariance across linguistic variations while preserving meaning.
...
TL;DR: PrivacyBench benchmark reveals RAG assistants leak secrets in 26.56% of interactions, highlighting urgent need for privacy-by-design safeguards.
...
Result: RAG assistants leak secrets in up to 26.56% of interactions. Privacy-aware prompts reduce leakage to 5.12%, but retrieval mechanisms still access sensitive data indiscriminately, creating a single point of failure in privacy preservation.
...
TL;DR: The paper critiques how Big AI and language engineers are accelerating ecological, meaning, and language crises, calling for alternatives centered on human flourishing and planetary health.


<details>
  <summary>Details</summary>
Motivation: The world faces converging ecological, meaning, and language crises (metacrisis), which Big AI is accelerating. Language engineers are complicit by supporting a failing scalability narrative, working for plutocrats/kleptocrats, and treating technology as value-neutral.

Method: The paper appears to be a critical analysis and call to action rather than presenting a specific technical method. It advocates for exploring alternatives through collective intelligence to redesign NLP's future.

Result: The paper identifies the problematic role of language engineers in the metacrisis and establishes the urgent need for paradigm shift in NLP toward life-affirming, human-flourishing approaches.

Conclusion: NLP must urgently move away from current destructive trajectories and toward designing technologies that support human flourishing on a living planet, requiring collective reimagining of the field's purpose and values.
...
TL;DR: Encyclo-K is a statement-based benchmark that uses knowledge statements from textbooks as the unit of curation, dynamically composing them into evaluation questions at test time to address data contamination, enable multi-knowledge assessment, and reduce annotation costs.


<details>
  <summary>Details</summary>
Motivation: Existing LLM benchmarks have three key limitations: vulnerability to data contamination (models can memorize questions), restriction to single-knowledge-point assessment, and reliance on costly domain expert annotation. The authors aim to create a more robust, comprehensive, and scalable evaluation framework.
...
Result: Experiments on 50+ LLMs show strong discriminative power - even top-performing OpenAI-GPT-5.1 achieves only 62.07% accuracy. Clear performance gradient: reasoning models range 16.04%-62.07%, chat models range 9.71%-50.40%. Model rankings remain stable across dynamically generated question sets.
...
TL;DR: Manifold-Constrained Hyper-Connections (mHC) fixes training instability in Hyper-Connections by restoring identity mapping while maintaining performance gains.


<details>
  <summary>Details</summary>
Motivation: Hyper-Connections extended residual connections but lost the identity mapping property, causing training instability, scalability issues, and memory overhead. Need to restore stability while keeping performance benefits.

Method: Projects the residual connection space of Hyper-Connections onto a specific manifold to restore identity mapping property, with infrastructure optimization for efficiency.

Result: mHC enables effective large-scale training with performance improvements and superior scalability compared to original Hyper-Connections.

Conclusion: mHC is a flexible, practical extension of Hyper-Connections that advances understanding of topological architecture design and suggests directions for foundational model evolution.
...
TL;DR: BEDA framework bridges belief estimation and strategic dialogue generation by formalizing adversarial/alignment acts and using probabilistic constraints to guide utterance generation based on inferred beliefs.


<details>
  <summary>Details</summary>
Motivation: Prior work accurately estimates beliefs in strategic dialogue but lacks principled mechanisms to effectively use those beliefs during generation, creating a gap between belief estimation and dialogue act execution.

Method: BEDA framework formalizes adversarial and alignment dialogue acts, operationalizes them via probabilistic constraints on generation, and consists of three components: world set, belief estimator, and conditional generator that selects acts and generates utterances consistent with inferred beliefs.

Result: BEDA consistently outperforms strong baselines across three settings: improves success rate by at least 5.0 points (up to 20.6 points with GPT-4.1-nano) on Conditional Keeper Burglar, achieves 9.3 point average improvement on Mutual Friends, and reaches optimal deal on CaSiNo negotiation.

Conclusion: Casting belief estimation as constraints provides a simple, general mechanism for reliable strategic dialogue, effectively bridging the gap between belief estimation and generation in strategic dialogue systems.
...
TL;DR: ADOPT is a prompt optimization framework for multi-step LLM pipelines that models step dependencies, enables precise text-gradient estimation, and adaptively allocates optimization resources.


<details>
  <summary>Details</summary>
Motivation: Multi-step LLM pipelines perform complex tasks but their performance heavily depends on prompts at each step. Joint optimization is difficult due to missing step-level supervision and inter-step dependencies, causing existing methods to yield suboptimal or unstable updates.
...
Conclusion: ADOPT provides a systematic framework for optimizing multi-step LLM pipelines by addressing dependency modeling and resource allocation challenges, offering superior performance over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Legal documents are challenging to classify due to specialized vocabulary and long length, making full-document Transformers processing impossible, expensive, or slow.
...
TL;DR: RESTOREx introduces LLM-generated and human-annotated explanations for detecting depressive symptoms in memes, with MAMAMemeia framework achieving 7.55% improvement in macro-F1 over state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Memes have evolved from humorous content to expressing a wide range of emotions, including depressive sentiments. With increasing use of memes to express depressive feelings on social media, there's a need to identify depressive symptoms in this medium for mental health monitoring and intervention.

Method: Introduces RESTOREx dataset with LLM-generated and human-annotated explanations for depressive symptom detection in memes. Proposes MAMAMemeia, a collaborative multi-agent multi-aspect discussion framework based on Cognitive Analytic Therapy (CAT) Competencies from clinical psychology.
...
Conclusion: The paper presents an effective framework for detecting depressive symptoms in memes using clinical psychology principles, achieving significant performance improvements and setting a new standard for this emerging research area.
...
TL;DR: Thought Gestalt (TG) is a recurrent Transformer that models language at two levels (tokens and sentence-level "thoughts") to address limitations of standard Transformers in forming globally consistent latent representations.


<details>
  <summary>Details</summary>
Motivation: Standard Transformer language models rely on surface-level co-occurrence statistics and fail to form globally consistent latent representations of entities and events, leading to brittleness in relational direction, contextualization errors, and data inefficiency. Human comprehension involves converting linguistic input into compact, event-like representations that persist in memory.

Method: TG is a recurrent Transformer that models language at two abstraction levels: tokens and sentence-level "thought" states. It generates tokens one sentence at a time while cross-attending to a memory of prior sentence representations. Both token and sentence representations use the same parameters and are trained with a single next-token cross-entropy objective, allowing gradients from future token losses to optimize earlier sentence vectors.

Result: TG consistently improves efficiency over matched GPT-2 runs, with scaling fits indicating GPT-2 requires ~5-8% more data and ~33-42% more parameters to match TG's loss. TG also reduces errors on relational direction generalization tasks like the father-son reversal curse probe.

Conclusion: The Thought Gestalt model demonstrates that modeling language at both token and sentence-level abstractions can improve data efficiency, parameter efficiency, and relational reasoning capabilities compared to standard Transformer architectures.
...
<details>
  <summary>Details</summary>
Motivation: Standard top-k retrieval in RAG often returns redundant or near-duplicate chunks, wasting token budget and degrading generation quality. There's a need for better context selection that balances relevance and redundancy under budget constraints.
...
Conclusion: AdaGReS provides an effective framework for redundancy-aware context selection in RAG with theoretical guarantees, adaptive parameter calibration, and practical improvements in both redundancy control and downstream generation quality.
...
TL;DR: Monocular depth estimation for robotic surgery using synthetic priors from Depth Anything V2, adapted with DV-LORA, achieving state-of-the-art results on SCARED dataset with 98.1% accuracy.
...
Conclusion: The approach successfully addresses monocular depth estimation challenges in surgical environments by combining synthetic priors with efficient domain adaptation, significantly improving performance in high-specularity regimes.
...
<details>
  <summary>Details</summary>
Motivation: Current urban warfare training assessment methods rely on costly sensors or subjective human observation, limiting scalability and accuracy for evaluating cognitive, psychomotor, and teamwork skills in realistic training environments.
...
Result: Demonstrated approach with real-world Enter and Clear the Room drills, providing actionable domain-specific metrics capturing individual and team performance. Supports After Action Reviews with interactive dashboards in Gamemaster and GIFT systems.

Conclusion: Video-based assessment enables scalable evaluation within Synthetic Training Environments. Limitations include tracking difficulties and ground-truth validation. Future work includes expanding to 3D video data and leveraging video analysis for broader STE applications.
...
<details>
  <summary>Details</summary>
Motivation: To enable long video modeling with low computational cost by compressing lengthy video sequences into compact representations while maintaining visual fidelity of individual frames.
...
Conclusion: PFP provides an effective framework for video compression and long-term memory in video models, with trade-offs in neural architecture designs that enable practical long-history video processing.
...
TL;DR: Proposes lifelong domain adaptation for 3D human pose estimation to handle non-stationary target datasets, using a novel GAN framework with pose-aware, temporal-aware, and domain-aware knowledge integration.


<details>
  <summary>Details</summary>
Motivation: 3D HPE struggles with generalization to diverse real-world scenarios due to reliance on controlled environment data. Existing domain adaptation methods don't handle non-stationary target pose datasets, creating a need for lifelong adaptation that preserves knowledge while adapting to new domains.

Method: Introduces a GAN framework with 3D pose generators, 2D pose discriminator, and 3D pose estimator. Constructs novel 3D pose generator paradigm integrating pose-aware, temporal-aware, and domain-aware knowledge to enhance current domain adaptation while alleviating catastrophic forgetting of previous domains.
...
Conclusion: First to introduce lifelong domain adaptation to 3D HPE, addressing challenges of non-stationary target datasets and catastrophic forgetting. The proposed framework effectively handles domain adaptation while preserving knowledge from previous domains.
...
TL;DR: Deep learning pipeline transforms pediatric T1-weighted MRIs into synthetic CTs for cranial bone segmentation and suture analysis, enabling non-invasive evaluation of skull development without radiation exposure.


<details>
  <summary>Details</summary>
Motivation: Current imaging methods for pediatric cranial assessment have limitations: CT exposes children to ionizing radiation, while MRI cannot visualize cranial sutures or estimate bone density. There's a need for non-invasive methods to evaluate cranial development and suture ossification in children.

Method: Proposed a deep learning pipeline that transforms T1-weighted MRIs of children (0.2-2 years) into synthetic CTs (sCTs). The method uses robust, domain-specific variational autoencoders to predict cranial bone segmentation, generate suture probability heatmaps, and derive direct suture segmentation from the heatmaps.

Result: sCTs achieved 99% structural similarity and Frechet inception distance of 1.01 relative to real CTs. Skull segmentation attained average Dice coefficient of 85% across seven cranial bones, and sutures achieved 80% Dice. Equivalence between sCTs and real CTs was confirmed using TOST (p < 0.05).

Conclusion: This is the first pediatric cranial CT synthesis framework enabling suture segmentation on sCTs derived from MRI, bridging critical gaps in non-invasive cranial evaluation by combining the radiation-free benefits of MRI with CT-like bone visualization capabilities.
...
TL;DR: This paper explores scaling laws for training foundation models on massive-scale electro-optical satellite data, finding performance remains data-limited even at petascale training.


<details>
  <summary>Details</summary>
Motivation: There's a lack of understanding about scaling laws in high-value domains like remote sensing, unlike natural-image domains where internet-scale data enables well-established scaling relationships. The research aims to establish practical techniques for training foundation models on EO datasets that exceed current state-of-the-art scale by orders of magnitude.

Method: Used over a quadrillion pixels of commercial satellite EO data and the MITRE Federal AI Sandbox to train progressively larger vision transformer (ViT) backbones. Analyzed success and failure modes observed at petascale training and examined implications for bridging domain gaps across additional remote sensing modalities.

Result: Found that even at massive scale (petascale), performance remains consistent with a data-limited regime rather than a model parameter-limited one. This contrasts with natural-image domains where scaling laws are better understood.

Conclusion: The practical insights from this scaling study should inform data-collection strategies, compute budgets, and optimization schedules for developing frontier-scale remote sensing foundation models, highlighting the continued importance of data scaling in this domain.
...
Method: A novel bi-level optimization framework with two components: a clinical task predictor and a skill predictor. The networks are optimized jointly through simultaneous refinement, assessing skills by how well tasks are performed on acquired fetal ultrasound images without using manually predefined skill ratings.
...
Conclusion: The proposed framework successfully enables automated ultrasound skill assessment by linking task performance to skill levels without requiring manual skill annotations, offering a more objective and efficient alternative to traditional expert-based assessment.
...
TL;DR: MGML framework for brain tumor segmentation with incomplete multimodal MRI data using meta-guided adaptive fusion and consistency regularization.


<details>
  <summary>Details</summary>
Motivation: Multimodal MRI is crucial for brain tumor segmentation but clinical data is often incomplete, creating a need for methods that can effectively utilize available incomplete multimodal information.

Method: Meta-guided multi-modal learning (MGML) with two components: 1) Meta-parameterized adaptive modality fusion (Meta-AMF) that generates adaptive soft-label supervision for coherent multimodal fusion, and 2) Consistency regularization module to enhance segmentation and robustness.

Result: Superior performance on BraTS2020 and BraTS2023 datasets. On BraTS2020 with 15 missing modality combinations: Dice scores of 87.55 (WT), 79.36 (TC), and 62.67 (ET), outperforming state-of-the-art methods.

Conclusion: MGML effectively handles incomplete multimodal MRI data without altering original model architecture, can be integrated into training pipelines, and achieves state-of-the-art performance for brain tumor segmentation.
...
TL;DR: A novel cross-view geo-localization system using DINOv2 backbone with convolution adapter, multi-scale channel reallocation, and MoE-enhanced aggregation for better feature alignment across viewpoints.


<details>
  <summary>Details</summary>
Motivation: Cross-view geo-localization faces significant challenges due to viewpoint discrepancies that hinder effective feature aggregation and alignment between query and database images.

Method: Three key improvements: 1) DINOv2 backbone with convolution adapter fine-tuning, 2) multi-scale channel reallocation module for spatial representation diversity, 3) improved aggregation with Mixture-of-Experts routing in cross-attention framework.
...
TL;DR: AI framework for automated action segmentation and skill assessment in microanastomosis surgery using computer vision and machine learning for objective surgical training evaluation.


<details>
  <summary>Details</summary>
Motivation: Microanastomosis requires precise surgical skills, but current assessment methods rely on subjective expert evaluation which is time-consuming and inconsistent. There's a need for automated, objective, and scalable solutions for surgical training assessment.

Method: Three-component AI system: 1) YOLO and DeepSORT for instrument tip tracking, 2) self-similarity matrix for action boundary detection and unsupervised clustering for segmentation, 3) supervised classification for surgical gesture proficiency evaluation.
...
TL;DR: DehazeSNN: A U-Net-like Spiking Neural Network architecture for image dehazing that combines multi-scale feature capture with efficient long-range dependency handling using Orthogonal Leaky-Integrate-and-Fire blocks.


<details>
  <summary>Details</summary>
Motivation: Traditional dehazing methods rely on atmospheric scattering models, while recent deep learning approaches (CNNs and Transformers) have limitations: CNNs struggle with long-range dependencies, and Transformers require heavy computational resources.

Method: Proposes DehazeSNN, a U-Net-like architecture integrating Spiking Neural Networks (SNNs) with an Orthogonal Leaky-Integrate-and-Fire Block (OLIFBlock) to enhance cross-channel communication and efficiently handle both local and long-range dependencies.

Result: DehazeSNN achieves competitive performance with state-of-the-art methods on benchmark datasets while offering smaller model size and reduced multiply-accumulate operations (lower computational burden).

Conclusion: The SNN-based approach provides an effective solution for image dehazing that balances performance with computational efficiency, addressing limitations of both CNNs and Transformers.
...
<details>
  <summary>Details</summary>
Motivation: Despite rapid advances in Text-to-Video (T2V) diffusion models generating high-quality videos, their vulnerability to adversarial attacks remains unexplored. The paper aims to investigate robustness of T2V models against adversarial manipulations.

Method: Proposes T2VAttack framework with two attack objectives: semantic (video-text alignment) and temporal (temporal dynamics). Two attack methods: T2VAttack-S (identifies critical words, replaces with synonyms via greedy search) and T2VAttack-I (iteratively inserts optimized words with minimal perturbation).

Result: Comprehensive evaluation on state-of-the-art T2V models (ModelScope, CogVideoX, Open-Sora, HunyuanVideo) shows even minor prompt modifications (single word substitution/insertion) cause substantial degradation in semantic fidelity and temporal dynamics.

Conclusion: Current T2V diffusion models have critical vulnerabilities to adversarial attacks, highlighting the need for improved robustness in video generation systems.
...
TL;DR: A novel view extrapolation method for autonomous driving that uses only images and optional camera poses, combining 4D Gaussian reconstruction with video diffusion models for iterative refinement without expensive sensors or annotations.


<details>
  <summary>Details</summary>
Motivation: Existing view extrapolation methods rely heavily on expensive sensors (LiDAR) or labor-intensive annotations (3D bounding boxes, lane markings), limiting real-world deployment. The authors aim to develop a solution that works with only images and optional camera poses.

Method: 1) Estimate global static point cloud and per-frame dynamic point clouds from images/camera poses; 2) Fuse into unified representation using deformable 4D Gaussian framework; 3) Train 4D Gaussian model to render degraded pseudo-images; 4) Use these to train video diffusion model; 5) Iteratively refine progressively shifted Gaussian renderings with diffusion model; 6) Incorporate enhanced results back as training data for 4DGS; 7) Continue until reaching target extrapolated viewpoints.
...
Conclusion: The proposed approach successfully addresses view extrapolation in autonomous driving using only images and optional camera poses, eliminating dependency on expensive sensors and annotations while achieving superior image quality through iterative 4D Gaussian and diffusion model refinement.
...
TL;DR: Deep learning-based anomaly detection using satellite time series to identify surface changes with higher sensitivity than traditional methods, validated on earthquake ruptures.
...
Method: Train an inpainting model built on the SATLAS foundation model to reconstruct the last frame of Sentinel-2 time series from preceding acquisitions, using globally distributed training data across diverse climates and land cover types.

Result: The method detects earthquake-triggered surface ruptures from the 2023 Turkey-Syria earthquake sequence with higher sensitivity and specificity than temporal median or Reed-Xiaoli anomaly detectors, achieving detection thresholds approximately three times lower than baseline approaches.

Conclusion: Deep learning leveraging temporal redundancy in satellite time series enables unprecedented sensitivity for surface change detection, providing a path toward automated, global-scale monitoring using freely available multi-spectral satellite data.
...
TL;DR: GCA-ResUNet: A lightweight medical image segmentation framework with Grouped Coordinate Attention module that improves global context modeling while maintaining CNN efficiency, outperforming both CNN and Transformer methods on multi-organ segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current U-Net based CNNs struggle with long-range contextual dependencies in multi-organ segmentation and low-contrast regions due to their local receptive fields. Transformer models address this but require high computational resources and large datasets, making them impractical for resource-constrained clinical environments.

Method: Proposes GCA-ResUNet with a plug-and-play Grouped Coordinate Attention (GCA) module. GCA decouples channel-wise context modeling into multiple groups to handle semantic heterogeneity across channels, and integrates direction-aware coordinate encoding to capture structured spatial dependencies along horizontal and vertical axes, enhancing global representation while preserving CNN efficiency.
...
TL;DR: GASeg bridges appearance and geometry using topological information to address self-supervised semantic segmentation failures caused by appearance ambiguities.


<details>
  <summary>Details</summary>
Motivation: Self-supervised semantic segmentation methods often fail due to over-reliance on unstable appearance-based features like shadows, glare, and local textures when faced with appearance ambiguities.

Method: Proposes GASeg framework with Differentiable Box-Counting (DBC) module to quantify multi-scale topological statistics from geometric and appearance features, Topological Augmentation (TopoAug) adversarial strategy using morphological operators, and GALoss for cross-modal alignment.
...
Conclusion: Leveraging stable topological information effectively bridges appearance and geometry, overcoming limitations of appearance-only approaches in self-supervised semantic segmentation.
...
TL;DR: Novel pipeline for 3D reconstruction of spacecraft using 3D Gaussian Splatting with Sun position priors to handle dynamic space lighting.


<details>
  <summary>Details</summary>
Motivation: Traditional 3DGS requires static scenes, but spaceborne imagery during RPO has dynamic lighting conditions. Photometric accuracy is crucial for downstream pose estimation tasks in RPO operations.

Method: Uses 3D Gaussian Splatting with Sun position priors from servicer spacecraft to improve photometric quality. Incorporates estimated Sun position knowledge into training pipeline to handle changing illumination.

Result: 3DGS models adapt to rapidly changing space illumination, reflect global shadowing and self-occlusion, and demonstrate improved photometric accuracy for pose estimation.

Conclusion: Incorporating Sun position priors enables effective 3D reconstruction of spacecraft in dynamic space lighting conditions, supporting RPO operations with accurate geometry and photometry.
...
Method: Two-stage fusion framework: 1) HilbertMed-SAM module for lesion segmentation using Hilbert space-filling curves in Mamba SSM scanning to preserve 3D spatial locality, plus Hilbert-Mamba Cross-Attention and scale-aware decoder; 2) Prompt enhancement module unifies segmentation masks with textual attributes to guide VLM for disease classification.

Result: Achieves 82.35% Dice score on BraTS2021 segmentation benchmark and 78.85% diagnostic classification accuracy, demonstrating improved accuracy and reliability for medical VLM-based analysis.

Conclusion: Hilbert-VLM offers substantial potential to improve medical VLM analysis by effectively integrating 3D multimodal information and capturing subtle pathological features through systematic architectural redesign.
...
TL;DR: CVC proposes a conditioned velocity correction framework for flow-based diffusion editing that addresses velocity error accumulation through dual-perspective velocity conversion and posterior-consistent updates, achieving stable latent dynamics with better fidelity and semantic alignment.
...
Method: CVC reformulates flow-based editing as a distribution transformation problem with a known source prior. It introduces a dual-perspective velocity conversion mechanism that decomposes latent evolution into structure-preserving and semantically-guided branches, then applies posterior-consistent updates derived from Empirical Bayes Inference and Tweedie correction to compensate for velocity errors.

Result: The method yields stable and interpretable latent dynamics, achieving faithful reconstruction alongside smooth local semantic conversion. Comprehensive experiments show CVC consistently achieves superior fidelity, better semantic alignment, and more reliable editing behavior across diverse tasks.

Conclusion: CVC provides a principled framework for flow-based diffusion editing that addresses velocity error accumulation through mathematically grounded error compensation, resulting in more stable and faithful image transformations.
...
<details>
  <summary>Details</summary>
Motivation: Current virtual try-on (VTON) systems focus on rendering garment details but neglect garment fit, which is crucial for holistic style coordination. Garment fit determines how clothing aligns with the body and is fundamental in fashion design.

Method: FitControler features: 1) a fit-aware layout generator that redraws body-garment layouts using garment-agnostic representations, and 2) a multi-scale fit injector that delivers layout cues for layout-driven VTON. The system is trained on Fit4Men dataset with 13,000 body-garment pairs of different fits.

Result: FitControler integrates with various VTON models and achieves accurate fit control. The paper introduces two fit consistency metrics for evaluation and demonstrates effectiveness through extensive experiments.

Conclusion: FitControler successfully addresses the gap in fit-aware virtual try-on by providing a plug-in solution that enables customized garment fit control while maintaining compatibility with existing VTON systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing 2D Gaussian Splatting (2DGS) approaches allocate representation capacity and parameter precision without considering image structure, limiting rate-distortion efficiency at low bitrates.

Method: Three key techniques: 1) Structure-guided initialization for localized, semantically meaningful Gaussian distribution; 2) Adaptive bitwidth quantization of covariance parameters (higher precision for complex regions); 3) Geometry-consistent regularization aligning Gaussian orientations with local gradient directions.
...
Conclusion: The proposed structure-guided allocation principle effectively couples image structure with representation capacity and quantization precision, enabling superior rate-distortion efficiency in 2D Gaussian Splatting while preserving fast decoding capabilities.
...
TL;DR: MF-RSVLM is a multi-feature fusion remote sensing vision-language model that addresses visual forgetting and fine-grained feature extraction challenges in remote sensing VLMs through multi-scale feature learning and recurrent visual injection.
...
Method: MF-RSVLM learns multi-scale visual representations, combines global context with local details, and uses a recurrent visual feature injection scheme to keep the language model grounded in visual evidence and reduce forgetting.
...
Conclusion: MF-RSVLM effectively addresses visual forgetting and fine-grained feature extraction challenges in remote sensing VLMs, achieving strong performance across multiple RS tasks with publicly available code.
...
TL;DR: RSAgent is an agentic MLLM that uses multi-turn reasoning with segmentation tools for iterative text-guided object segmentation, outperforming one-shot methods.


<details>
  <summary>Details</summary>
Motivation: One-shot text-guided segmentation methods lack verification, refocusing, and refinement capabilities when initial localization is wrong, limiting their robustness and accuracy.

Method: RSAgent interleaves reasoning and action via multi-turn tool invocations, queries segmentation toolbox, observes visual feedback, and revises spatial hypotheses iteratively. Trained with two-stage framework: supervised fine-tuning followed by agentic reinforcement learning with task-specific rewards.
...
Conclusion: Agentic approach with iterative reasoning and tool use significantly improves text-guided segmentation performance over one-shot methods, enabling verification, refocusing, and refinement capabilities.
...
TL;DR: PipeFlow is a scalable video editing method that uses motion analysis, pipelined scheduling, and interpolation to enable efficient long-form video editing with linear time scaling.


<details>
  <summary>Details</summary>
Motivation: Long-form video editing faces computational challenges due to high costs of joint editing and DDIM inversion across extended sequences, creating exponential computational growth.

Method: Three key innovations: 1) Motion analysis using SSIM and Optical Flow to skip editing of low-motion frames, 2) Pipelined task scheduling that splits videos into segments for parallel DDIM inversion and editing, 3) Neural network interpolation to smooth segment borders and interpolate skipped frames.

Result: PipeFlow achieves up to 9.6X speedup over TokenFlow and 31.7X speedup over Diffusion Motion Transfer, with linear time scaling relative to video length.

Conclusion: The method enables editing of infinitely long videos without the growing per-frame computational overhead, making long-form video editing scalable and practical.
...
TL;DR: A reinforcement learning-based anisotropic diffusion framework for image denoising that learns optimal diffusion action sequences through deep Q-learning, outperforming traditional diffusion methods and competing with deep CNN approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional anisotropic diffusion methods use explicit diffusion operators that are not well-adapted to complex image structures, limiting their performance compared to modern learning-based approaches. The authors aim to create a more adaptive diffusion-based denoiser that can handle complex image patterns better.

Method: The paper proposes a trainable anisotropic diffusion framework using reinforcement learning. It models denoising as a series of naive diffusion actions, where the optimal sequence of actions is learned through deep Q-learning. This creates a stochastic anisotropic diffusion process that adapts to different image structures.

Result: The proposed method outperforms existing diffusion-based denoising methods and competes with representative deep CNN-based methods. It effectively removes three types of commonly seen noise patterns.

Conclusion: The reinforcement learning approach enables anisotropic diffusion to become more adaptive to complex image structures, bridging the performance gap between traditional diffusion methods and modern learning-based approaches while maintaining the interpretability of diffusion-based methods.
...
Method: Proposes NIRNL framework with two main components: 1) Cross-modal Margin Preserving (CMP) to adjust relative distances between positive/negative pairs for better discrimination, and 2) Neighbor-aware Instance Refining (NIR) to identify pure, hard, and noisy subsets through cross-modal neighborhood consensus, with tailored optimization strategies for each subset.
...
Conclusion: NIRNL effectively addresses noisy label problems in cross-modal retrieval by maximizing data utilization while mitigating error propagation through fine-grained instance partitioning and tailored optimization.
...
Method: Developed Pathology Recalibration Module (PRM) with pixel-wise context compression and pathology distribution concentration operators, plus Expert Prior Guidance Adapter (EPGA) to highlight significant regions. Combined with Integrated Loss considering sample-wise loss distributions and label frequencies.

Result: PCRNet with IL outperforms state-of-the-art attention-based networks and advanced loss methods on three ocular disease datasets, with visualization explaining PRM and EPGA's decision-making behavior.

Conclusion: Incorporating pathology context and expert experience priors through PRM and EPGA modules significantly improves ocular disease recognition performance and provides better interpretability for clinical decision-making.
...
TL;DR: Proposes balanced hierarchical contrastive loss with decoupled learning strategy in DETR framework to address hierarchical label imbalance and localization interference in fine-grained remote sensing detection.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchical contrastive learning methods for fine-grained remote sensing detection overlook two critical issues: (1) imbalanced data distribution across label hierarchies causing high-frequency classes to dominate learning, and (2) semantic relationship learning interfering with class-agnostic localization.

Method: Combines balanced hierarchical contrastive loss with decoupled learning strategy within DETR framework. The loss introduces learnable class prototypes and equilibrates gradients across classes at each hierarchical level. The decoupled strategy separates DETR's object queries into classification and localization sets for task-specific optimization.

Result: Outperforms state-of-the-art approaches on three fine-grained datasets with hierarchical annotations.

Conclusion: The proposed method effectively addresses hierarchical label imbalance and localization interference, improving fine-grained detection performance in remote sensing applications.
...
<details>
  <summary>Details</summary>
Motivation: DiT models have high computational costs due to attention mechanisms, limiting practical applications. Existing sparse attention methods have prediction overhead and lack hardware generality, being mostly GPU-focused.

Method: Proposes RainFusion2.0 with three key techniques: (1) block-wise mean values as representative tokens for sparse mask prediction, (2) spatiotemporal-aware token permutation, and (3) first-frame sink mechanism for video generation.
...
Conclusion: RainFusion2.0 provides an online adaptive, hardware-efficient, low-overhead sparse attention mechanism that accelerates DiT models while maintaining quality and supporting diverse hardware platforms beyond just GPUs.
...
Conclusion: Factorized learning with explicit temporal grounding modeling significantly improves video-language understanding. The D²VLM framework with FPO algorithm effectively addresses the hierarchical relationship between temporal grounding and textual response, leading to better event-level perception in video understanding.
...
TL;DR: LMR introduces a two-stage "Think-then-Act" approach for text-to-motion generation using dual latent spaces to bridge semantic-kinematic mismatch.


<details>
  <summary>Details</summary>
Motivation: Current T2M methods treat generation as direct translation, facing a "Semantic-Kinematic Impedance Mismatch" - difficulty grounding discrete linguistic intent into continuous motion data in one shot.

Method: Latent Motion Reasoning (LMR) uses a Dual-Granularity Tokenizer to disentangle motion into two manifolds: Reasoning Latent for global planning and Execution Latent for physical fidelity. It implements a two-stage autoregressive process: reason (plan trajectory) then move (instantiate frames).

Result: LMR shows non-trivial improvements in both semantic alignment and physical plausibility when implemented on T2M-GPT and MotionStreamer baselines, validating that motion-aligned concept spaces are better than natural language for motion planning.

Conclusion: The optimal substrate for motion planning is a learned motion-aligned concept space, not natural language. Architectural shift to Latent System 2 Reasoning effectively bridges the gap between language and physics.
...
TL;DR: A training-free generative adversarial attack framework that creates naturalistic adversarial objects to fool monocular depth estimation systems in autonomous driving, outperforming existing attacks in effectiveness and stealthiness.


<details>
  <summary>Details</summary>
Motivation: Monocular Depth Estimation (MDE) is critical for autonomous driving safety but vulnerable to adversarial attacks. Existing physical attacks use texture-based patches with strict placement constraints and limited realism, reducing effectiveness in complex driving environments.

Method: Training-free generative adversarial attack framework using diffusion-based conditional generation. Includes: 1) Salient Region Selection module to identify MDE-influential regions, and 2) Jacobian Vector Product Guidance mechanism to steer adversarial gradients toward update directions supported by pre-trained diffusion model.
...
Conclusion: GeoBench establishes a comprehensive benchmark for geometric problem-solving evaluation and offers actionable guidelines for developing geometric reasoning systems, highlighting the importance of structured reasoning approaches over simple prompting strategies.
...
TL;DR: This paper introduces FSAP (Few-Shot Architecture Prompting) for LLM-based neural architecture generation in computer vision, finding 3 examples optimal, and a fast deduplication method, validated across 7 vision benchmarks.
...
Method: 1) Introduces Few-Shot Architecture Prompting (FSAP) - systematic study of supporting examples (n=1-6) for LLM-based architecture generation. 2) Proposes Whitespace-Normalized Hash Validation - lightweight deduplication method (100x faster than AST parsing). 3) Uses dataset-balanced evaluation methodology for comparing architectures across heterogeneous vision tasks.

Result: Found n=3 examples best balances architectural diversity and context focus for vision tasks. Generated 1,900 unique architectures across 7 computer vision benchmarks (MNIST, CIFAR-10, CIFAR-100, CelebA, ImageNette, SVHN, Places365). Deduplication method provides 100x speedup over AST parsing.
...
Result: Comprehensive evaluation combining qualitative analysis with quantitative metrics shows D²-Align achieves superior alignment with human preference while maintaining diversity, outperforming existing methods that suffer from Preference Mode Collapse.
...
TL;DR: IMDD-1M is the first large-scale industrial multimodal defect dataset with 1M image-text pairs covering 60+ materials and 400+ defect types, enabling training of a diffusion-based vision-language foundation model for efficient industrial inspection tasks.


<details>
  <summary>Details</summary>
Motivation: There is a need for large-scale multimodal datasets and foundation models specifically designed for industrial manufacturing and quality inspection to advance multimodal learning in this domain.

Method: Created IMDD-1M dataset with 1M aligned image-text pairs covering diverse materials and defects, then trained a diffusion-based vision-language foundation model from scratch using this dataset.

Result: The foundation model achieves comparable performance to dedicated expert models with less than 5% of task-specific data, demonstrating efficient adaptation for industrial inspection and generation tasks.

Conclusion: IMDD-1M and the trained foundation model enable scalable, domain-adaptive, and knowledge-grounded manufacturing intelligence through data-efficient multimodal learning.
...
Conclusion: BSD provides a principled approach to self-distillation that eliminates hard target dependency, achieving superior accuracy, calibration, and robustness compared to existing methods, with state-of-the-art performance under label noise when combined with contrastive loss.
...
TL;DR: DiffThinker introduces a diffusion-based generative multimodal reasoning framework that reformulates vision-centric reasoning as image-to-image generation, outperforming leading MLLMs by significant margins.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have text-centric reasoning processes that perform poorly on complex long-horizon, vision-centric tasks. There's a need for a native multimodal reasoning approach that better handles logical consistency and spatial precision in vision tasks.

Method: DiffThinker establishes a Generative Multimodal Reasoning paradigm using diffusion models. It reformulates multimodal reasoning as a native generative image-to-image task rather than text-centric reasoning, enabling better handling of visual information.

Result: DiffThinker significantly outperforms leading models: +314.2% over GPT-5, +111.6% over Gemini-3-Flash, and +39.0% over fine-tuned Qwen3-VL-32B. The framework demonstrates four core properties: efficiency, controllability, native parallelism, and collaboration.

Conclusion: Generative multimodal reasoning via diffusion-based frameworks like DiffThinker represents a promising approach for vision-centric reasoning, offering superior performance over text-centric MLLMs in complex visual tasks across multiple domains.
...
TL;DR: Deep Global Clustering (DGC) is a memory-efficient framework for hyperspectral image segmentation that learns global clustering from local patches without pre-training, enabling fast training on consumer hardware but suffers from optimization instability due to loss balancing issues.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging faces computational bottlenecks from massive data volumes exceeding memory, and existing foundation models pre-trained on remote sensing data fail to transfer to domain-specific applications like close-range agricultural monitoring where spectral signatures and spatial scales differ fundamentally.

Method: DGC operates on small patches with overlapping regions to enforce consistency, learning global clustering structure from local patch observations without pre-training. It maintains constant memory usage and can train in under 30 minutes on consumer hardware.

Result: On a leaf disease dataset, DGC achieves background-tissue separation with mean IoU of 0.925 and demonstrates unsupervised disease detection through navigable semantic granularity. However, the framework suffers from optimization instability due to multi-objective loss balancing issues, where meaningful representations emerge rapidly but degrade due to cluster over-merging.

Conclusion: The DGC design philosophy has merit for memory-efficient HSI segmentation, but stable implementation requires principled approaches to dynamic loss balancing. The work is positioned as intellectual scaffolding rather than a fully stable solution.
...
TL;DR: Proposes Internal Guidance (IG), a simple training strategy that adds auxiliary supervision on intermediate layers and extrapolates outputs during sampling, improving diffusion model training efficiency and generation quality without complex guidance methods.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models struggle with low-probability areas due to insufficient training data coverage. Existing guidance methods like CFG cause oversimplification/distortion, while alternative bad-version guidance requires complex degradation strategies, extra training, and additional sampling steps.

Method: Internal Guidance (IG) introduces auxiliary supervision on intermediate layers during training, then extrapolates intermediate and deep layer outputs during sampling to guide generation toward high-probability areas without complex guidance mechanisms.

Result: Significant improvements across baselines: SiT-XL/2+IG achieves FID=5.31 (80 epochs) and 1.75 (800 epochs) on ImageNet 256x256. LightningDiT-XL/1+IG achieves FID=1.34, and combined with CFG reaches state-of-the-art FID=1.19.

Conclusion: IG is a simple yet effective strategy that enhances diffusion model training efficiency and generation quality without the drawbacks of existing guidance methods, achieving state-of-the-art performance on ImageNet benchmarks.
...
Method: PointRAFT uses a novel object height embedding to incorporate tuber height as additional geometric information, enabling direct prediction of continuous 3D shape properties (like weight) from partial point clouds without full 3D reconstruction.

Result: On a test set of 5,254 point clouds from 172 tubers, PointRAFT achieved MAE of 12.0g and RMSE of 17.2g, outperforming linear regression and PointNet++ baselines, with inference time of 6.3ms per point cloud (150 tubers/second).

Conclusion: PointRAFT provides accurate, high-throughput potato weight estimation suitable for commercial harvesters and offers a versatile regression framework for various 3D phenotyping and robotic perception applications.
...
TL;DR: CorGi is a training-free inference acceleration framework for Diffusion Transformers that reduces redundant computation by caching and reusing low-contribution transformer blocks across denoising steps.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiT) achieve excellent visual generation but suffer from high inference costs due to iterative denoising and large model capacity, with substantial redundant computation across steps.

Method: CorGi selectively reuses outputs of transformer blocks across denoising steps by caching low-contribution blocks and reusing them later within intervals. CorGi+ extends this for text-to-image tasks by using per-block cross-attention maps to identify salient tokens and applying partial attention updates.
...
Conclusion: CorGi provides an effective training-free approach to accelerate DiT inference by reducing redundant computation through selective block caching and reuse, maintaining generation quality while significantly improving efficiency.
...
Method: Proposes a progressive generative adversarial network (GAN) to generate synthetic data for minority classes. Uses weighted combination of synthetic and real data before feeding to deep classifier. Employs multi-objective meta-heuristic population-based optimization algorithm to optimize classifier hyperparameters.

Result: Achieves 95.5% accuracy for 4-class and 98.5% accuracy for 2-class imbalanced COVID-19 chest X-ray classification problems. Outperforms existing methods on cross-validated metrics using large imbalanced dataset.

Conclusion: The proposed model effectively addresses medical image classification challenges with imbalanced data during pandemics, demonstrating superior performance through synthetic data augmentation and optimization techniques.
...
TL;DR: ARM is a lightweight learnable module that refines CLIP's internal features for open-vocabulary semantic segmentation, trained once on COCO-Stuff and used as a universal plug-and-play post-processor across various training-free frameworks.


<details>
  <summary>Details</summary>
Motivation: CLIP's image-level representations lack pixel-level details needed for semantic segmentation. Existing training-free methods either rely on expensive external models or use suboptimal static heuristics on CLIP features.

Method: Attention Refinement Module (ARM) with semantically-guided cross-attention using deep features (K,V) to refine shallow features (Q), followed by self-attention. Trained once on COCO-Stuff, then used as universal plug-and-play post-processor.
...
Conclusion: ARM provides a lightweight, learnable solution that unlocks CLIP's internal potential for open-vocabulary semantic segmentation through a "train once, use anywhere" paradigm, outperforming static fusion methods.
...
TL;DR: Mirage is a one-step video diffusion model for photorealistic and coherent asset editing in driving scenes, addressing temporal coherence and spatial fidelity issues in video object editing.


<details>
  <summary>Details</summary>
Motivation: Vision-centric autonomous driving needs diverse training data, but existing video object editing methods struggle to maintain both high visual fidelity and temporal coherence for effective data augmentation.

Method: Builds on text-to-video diffusion prior for temporal consistency; injects temporally agnostic latents from pretrained 2D encoder into 3D decoder to restore detail while preserving causality; uses two-stage data alignment (coarse 3D + fine 2D refinement) to address distribution mismatch between scene objects and inserted assets.

Result: Extensive experiments show Mirage achieves high realism and temporal consistency across diverse editing scenarios; can generalize to other video-to-video translation tasks.
...
TL;DR: MotivNet is a generalizable facial emotion recognition model using Meta's Sapiens vision foundation model as backbone, achieving competitive cross-dataset performance without cross-domain training.
...
Method: Uses Meta's Sapiens (human vision foundational model with Masked Autoencoder pretraining) as backbone. MotivNet is added as downstream task to Sapiens. Three criteria defined for viability: benchmark performance, model similarity, and data similarity.

Result: MotivNet achieves competitive performance across datasets without cross-domain training, meets all three viability criteria, and validates as a Sapiens downstream task, making FER more applicable for real-world use.

Conclusion: MotivNet demonstrates generalizable FER across domains using Sapiens foundation model, addressing generalization issues without cross-domain training, enabling more robust real-world FER applications.
...
TL;DR: MambaSeg: A dual-branch semantic segmentation framework using parallel Mamba encoders for RGB and event data fusion, with novel Dual-Dimensional Interaction Module for spatial-temporal fusion, achieving SOTA performance with reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: RGB-based segmentation methods degrade under challenging conditions (fast motion, low-light, high dynamic range) due to frame camera limitations. Event cameras offer high temporal resolution but lack color/texture. Existing multimodal fusion approaches are computationally expensive and focus mainly on spatial fusion, neglecting temporal dynamics of event streams.

Method: Proposes MambaSeg with parallel Mamba encoders for RGB images and event streams. Introduces Dual-Dimensional Interaction Module (DDIM) consisting of Cross-Spatial Interaction Module (CSIM) and Cross-Temporal Interaction Module (CTIM) for fine-grained fusion along both spatial and temporal dimensions to improve cross-modal alignment and reduce ambiguity.
...
TL;DR: PGMP framework combines physics-based simulation, deterministic manifold projection, and medical foundation model priors for efficient, high-quality dental CBCT metal artifact reduction in a single forward pass.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for dental CBCT metal artifact reduction have limitations: supervised methods suffer from spectral blurring due to "regression-to-the-mean", unsupervised methods risk structural hallucinations, and diffusion models are too slow for clinical use due to iterative sampling.

Method: Three-component framework: 1) AAPS pipeline synthesizes high-fidelity training pairs using Monte Carlo spectral modeling and patient-specific digital twins; 2) DMP-Former adapts direct x-prediction for deterministic manifold projection in single forward pass; 3) SSA module uses MedDINOv3 priors to ensure clinical plausibility.
...
Conclusion: PGMP framework successfully addresses limitations of existing MAR methods by combining physics-based simulation, efficient deterministic restoration, and medical foundation model guidance, making it clinically suitable for dental CBCT applications.
...
TL;DR: DualityForge: A framework using diffusion-based video editing to create counterfactual video QA pairs for reducing hallucinations in multimodal LLMs, achieving 24% improvement over baseline.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from visual ungrounded hallucinations due to over-reliance on language priors, especially with counterfactual videos. This is hard to fix due to high costs of collecting counterfactual data.

Method: DualityForge uses controllable diffusion-based video editing to transform real videos into counterfactual scenarios, generating QA pairs. Duality-Normalized Advantage Training (DNA-Train) applies two-stage SFT-RL with pair-wise ℓ₁ advantage normalization for stable optimization.
...
TL;DR: A 3D lifting framework for panoramic stitching that converts images to dense 3D point clouds, fuses them globally, then projects to 360° panorama, reducing ghosting and distortions in complex scenes.
...
<details>
  <summary>Details</summary>
Motivation: Rare gastrointestinal lesions are infrequently encountered in routine endoscopy, limiting data for AI model development and novice clinician training, creating a "rare-disease gap" in both computer-aided diagnostics and clinical education.

Method: One-shot, retraining-free generative framework using language-guided concept disentanglement to separate pathognomonic lesion features from non-diagnostic attributes. Encodes diagnostic features into a learnable prototype embedding while varying non-diagnostic attributes for diversity.

Result: Validated across four rare pathologies; synthetic images were clinically plausible. When used for data augmentation, significantly enhanced downstream AI classifiers (improved true positive rate at low false-positive rates). Novice endoscopists exposed to EndoRare-generated cases achieved 0.400 increase in recall and 0.267 increase in precision.
...
TL;DR: Virtual-Eyes CT quality-control pipeline improves generalist foundation models for lung cancer screening but degrades specialist models adapted to raw clinical data.


<details>
  <summary>Details</summary>
Motivation: Robust preprocessing is rarely quantified in deep-learning pipelines for low-dose CT lung cancer screening. There's a need to measure the differential impact of quality control on generalist foundation models versus specialist models.

Method: Developed Virtual-Eyes, a clinically motivated 16-bit CT quality-control pipeline that enforces 512x512 resolution, rejects non-diagnostic series, extracts contiguous lung blocks using Hounsfield-unit filtering and lung-coverage scoring. Evaluated on 765 NLST patients using RAD-DINO, Merlin, Sybil, and ResNet-18 models with frozen encoders and leakage-free patient-level MLP heads.

Result: Virtual-Eyes improved RAD-DINO slice-level AUC from 0.576 to 0.610 and patient-level AUC from 0.646 to 0.683 (mean pooling) and 0.619 to 0.735 (max pooling). Sybil and ResNet-18 degraded under Virtual-Eyes (Sybil AUC 0.886 to 0.837; ResNet-18 AUC 0.571 to 0.596). Merlin showed limited transferability regardless of preprocessing.

Conclusion: Anatomically targeted quality control can stabilize and improve generalist foundation-model workflows but may disrupt specialist models adapted to raw clinical context, highlighting the importance of model-specific preprocessing strategies.
...
TL;DR: UniAct is a two-stage framework combining fine-tuned MLLM with causal streaming pipeline that enables humanoid robots to execute multimodal instructions with sub-500ms latency, achieving 19% improvement in zero-shot motion tracking.


<details>
  <summary>Details</summary>
Motivation: Bridging high-level multimodal perception with whole-body execution in humanoid robotics remains a bottleneck. Existing methods struggle to translate heterogeneous instructions (language, music, trajectories) into stable, real-time actions.

Method: Two-stage framework: 1) fine-tuned Multimodal Large Language Model (MLLM) integrated with 2) causal streaming pipeline. Uses FSQ (Finite Scalar Quantization) to unify inputs through shared discrete codebook, ensuring cross-modal alignment while constraining motions to physically grounded manifold.

Result: Achieves sub-500ms latency for multimodal instruction execution. 19% improvement in success rate for zero-shot tracking of imperfect reference motions. Validated on UniMoCap benchmark (20-hour humanoid motion dataset) showing robust generalization across diverse real-world scenarios.

Conclusion: UniAct represents a critical step toward responsive, general-purpose humanoid assistants capable of seamless interaction through unified perception and control, addressing the long-standing challenge of bridging multimodal perception with whole-body execution.
...
Method: CERES implements dual-modal causal intervention: 1) Backdoor adjustment to counteract language representation biases from dataset statistics, 2) Front-door adjustment to address visual confounding by integrating semantic visual features with geometric depth information guided by causal principles.

Result: CERES achieves state-of-the-art performance on Ego-RVOS benchmarks, demonstrating improved robustness and reliability in egocentric video understanding.

Conclusion: The framework shows the potential of causal reasoning for building more reliable models in egocentric video understanding by systematically addressing dataset biases and visual confounders.
...
TL;DR: SenseNova-MARS is a multimodal agentic reasoning framework that enables VLMs to interleave visual reasoning with tool manipulation using RL, achieving SOTA on search benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current VLMs are limited to text-oriented reasoning and isolated tool use, lacking the human-like ability to seamlessly interleave dynamic tool manipulation with continuous reasoning in knowledge-intensive visual scenarios.

Method: SenseNova-MARS framework with BN-GSPO RL algorithm that integrates image search, text search, and image crop tools for fine-grained visual understanding. Also introduces HR-MMSearch benchmark for evaluation.

Result: Achieves SOTA performance: 67.84 on MMSearch and 41.64 on HR-MMSearch benchmarks, surpassing proprietary models like Gemini-3-Flash and GPT-5.

Conclusion: SenseNova-MARS represents significant progress toward agentic VLMs with robust tool-use capabilities, and the authors will release all code, models, and datasets to advance research.
...
TL;DR: LVLDrive is a framework that enhances Vision-Language Models for autonomous driving by incorporating LiDAR point clouds for robust 3D spatial understanding, addressing limitations of 2D image-based methods.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models (VLMs) for autonomous driving rely on 2D images, which struggle with accurate metric spatial reasoning and geometric inference, creating safety and reliability bottlenecks. There's a need to incorporate explicit 3D metric data for trustworthy autonomous systems.
...
Conclusion: The work demonstrates the necessity of explicit 3D metric data for building trustworthy VLM-based autonomous systems, successfully bridging the gap between language-based reasoning and accurate spatial understanding.
...
TL;DR: The paper proposes elementary information mechanics, a new model linking CNN convolutional filtering to physical energy-momentum relations from special relativity and quantum mechanics.


<details>
  <summary>Details</summary>
Motivation: To establish a theoretical foundation connecting information processing in convolutional neural networks to fundamental physical principles, specifically the energy-momentum relation from modern physics.

Method: Decompose convolutional kernels into orthogonal even and odd components, analyze their effects on image content (diffusion vs displacement), and examine even-odd properties in the spectral domain using discrete cosine transform (DCT) to identify fundamental information propagation modes.

Result: Even kernels cause isotropic diffusion preserving center of mass (analogous to rest/potential energy), odd kernels cause directional displacement of center of mass (analogous to kinetic energy), and information speed is linearly related to odd vs total kernel energy ratio. Small filters are dominated by low-frequency DCT bases (DC and gradient components).

Conclusion: This work establishes the first demonstrated link between information processing in generic CNNs and the energy-momentum relation from relativistic physics, providing a new physical interpretation framework for convolutional filtering operations.
...
TL;DR: DermaVQA-DAS extends DermaVQA dataset with closed QA and lesion segmentation tasks using expert-developed Dermatology Assessment Schema (DAS), benchmarking multimodal models and showing prompt design impacts segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Existing dermatological datasets focus on dermatoscopic images without patient queries and clinical context, limiting applicability to patient-centered care. Need for structured framework capturing clinically meaningful features.

Method: Introduce Dermatology Assessment Schema (DAS) with 36 high-level and 27 fine-grained assessment questions in English/Chinese. Extend DermaVQA dataset to support closed QA and segmentation tasks. Benchmark multimodal models with various prompting strategies.

Result: For segmentation: default prompt best under Mean-of-Max/Mean-of-Mean; augmented prompt with patient query yields best microscore (Jaccard 0.395, Dice 0.566). For QA: models achieve 0.729-0.798 accuracy; o3 best (0.798), GPT-4.1 close (0.796), Gemini-1.5-Pro competitive (0.783).

Conclusion: DermaVQA-DAS with DAS framework addresses limitations of existing datasets by providing structured clinical context and patient queries, enabling better patient-centered dermatological vision-language modeling. Dataset and schema publicly released to accelerate research.
...
TL;DR: This paper presents a comprehensive framework and taxonomy for multi-modal pre-training to achieve Spatial Intelligence from diverse sensors like cameras and LiDAR, addressing integration challenges and proposing a roadmap toward general-purpose foundation models.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of autonomous systems (self-driving vehicles, drones) has intensified the need for true Spatial Intelligence from multi-modal onboard sensor data. While foundation models excel in single-modal contexts, integrating capabilities across diverse sensors to create unified understanding remains a formidable challenge.
...
TL;DR: RedunCut is a dynamic model size selection system for live video analytics that reduces compute costs by 14-62% while maintaining accuracy, addressing limitations of prior methods on diverse workloads.


<details>
  <summary>Details</summary>
Motivation: Live video analytics faces high inference costs with modern vision models across massive camera fleets. Prior dynamic model size selection (DMSS) methods fail to generalize to diverse workloads like mobile videos and lower accuracy targets due to inefficient sampling and inaccurate accuracy prediction.

Method: RedunCut uses a measurement-driven planner to estimate cost-benefit tradeoffs of sampling, and a lightweight, data-driven performance model to improve accuracy prediction for per-segment model size selection.
...
Conclusion: RedunCut successfully addresses key limitations of prior DMSS methods by optimizing sampling efficiency and improving accuracy prediction, enabling significant cost reductions for live video analytics without model retraining.
...
TL;DR: DyStream: A flow matching-based autoregressive model for real-time dyadic talking head video generation with ultra-low latency (<100ms) using causal encoder with lookahead module.


<details>
  <summary>Details</summary>
Motivation: Existing chunk-based methods for dyadic talking head video generation require full non-causal context windows, introducing significant delays that prevent immediate, non-verbal feedback required for realistic listener responses.
...
Conclusion: DyStream enables real-time dyadic talking head video generation with ultra-low latency, significantly outperforming alternative causal strategies while maintaining high quality, making it suitable for applications requiring immediate feedback.
...
<details>
  <summary>Details</summary>
Motivation: Current surgical proficiency assessment methods rely on expert supervision which is subjective, time-consuming, and impractical for scaling in low-resource settings, limiting consistency across training programs.

Method: Video transformer architecture (TimeSformer) enhanced with hierarchical temporal attention and weighted spatial attention for action recognition, combined with YOLO-based object detection and tracking for fine-grained motion feature extraction to analyze instrument kinematics.
...
Conclusion: The AI system provides objective, consistent, and interpretable feedback for surgical skill assessment, enabling more standardized, data-driven training and evaluation in surgical education, particularly beneficial for resource-limited settings.
...
TL;DR: ViT encoder representations show compositionality when using DWT-derived primitives, suggesting structured information organization similar to language models.


<details>
  <summary>Details</summary>
Motivation: To understand how Vision Transformers structure information by investigating whether they learn compositional representations similar to language models, moving beyond language-focused analysis to vision tasks.

Method: Developed a framework to test compositionality in ViT encoder using Discrete Wavelet Transform (DWT) to obtain input-dependent primitives, then examined if composed representations can reproduce original image representations.

Result: Primitives from one-level DWT decomposition produce encoder representations that approximately compose in latent space, indicating compositionality in ViT's representation structure.

Conclusion: ViTs exhibit compositional representation learning similar to language models, with DWT providing effective vision primitives, offering new insights into how vision transformers organize information.
...
<details>
  <summary>Details</summary>
Motivation: High-fidelity compression of multispectral solar imagery is challenging for space missions due to limited bandwidth, requiring a balance between compression and preserving fine spectral and spatial details.
...
Conclusion: The proposed learned compression framework effectively addresses the challenge of multispectral solar imagery compression for space missions by leveraging graph-based modeling of spectral and spatial relationships, achieving superior spectral fidelity and reconstruction quality.
...
<details>
  <summary>Details</summary>
Motivation: Current Text-to-Image Diffusion models for SISR cause undesirable hallucinations in consumer photography where LR images have high fidelity. Text features are too high-level to describe subtle textures, and smartphone LR images (12MP+) are too large for existing T2IDiff models designed for smaller images (<1MP).

Method: Proposes a Feature-to-Image Diffusion (F2IDiff) Foundation Model that uses lower-level DINOv2 features for conditioning instead of text features. These features provide stricter conditioning while being rich descriptors of even small image patches.

Result: The F2IDiff model addresses the limitations of T2IDiff models by providing hallucination-free generation suitable for consumer photography, with better conditioning for high-fidelity LR images and small patches.

Conclusion: Using lower-level DINOv2 features instead of text features in diffusion foundation models enables better SISR for consumer photography by providing stricter, more accurate conditioning that prevents hallucinations while maintaining generation quality.
...
TL;DR: AI pipeline combines YOLO object detection with GPT-4 to generate radiology reports from chest X-rays, achieving strong clinical accuracy but with stylistic differences from human reports.


<details>
  <summary>Details</summary>
Motivation: Current AI systems for medical imaging only output structured predictions, requiring radiologists to manually translate these into narrative reports. There's a need to bridge this gap by automatically generating full diagnostic narratives from AI findings.

Method: Developed a pipeline integrating YOLOv5 and YOLOv8 for anomaly detection in chest X-rays, with bounding-box predictions and class labels fed to GPT-4 to generate descriptive findings and clinical summaries. Compared YOLO models on detection accuracy, inference latency, and text quality.

Result: Strong semantic similarity between AI and human reports. GPT-4 excelled in clarity (4.88/5) but scored lower for natural writing flow (2.81/5). YOLO models showed differences in detection accuracy and inference speed. Current systems achieve clinical accuracy but remain stylistically distinguishable from radiologist-authored text.

Conclusion: The integrated AI pipeline successfully generates clinically accurate radiology reports from chest X-rays, demonstrating the potential of combining computer vision with large language models. However, stylistic differences from human writing remain a challenge, suggesting future work should focus on improving natural language generation quality.
...
TL;DR: MS-VQ-VAE: A lightweight multi-scale VQ-VAE for generating compact latent representations of low-res video (64x64) suitable for edge deployment, achieving 25.96 dB PSNR and 0.8375 SSIM on UCF101.


<details>
  <summary>Details</summary>
Motivation: Traditional video codecs (H.264/HEVC) lack native support for ML-centric latent representations, limiting integration into deep learning pipelines. Need efficient compression for bandwidth-sensitive scenarios like CDNs, edge devices, and real-time streaming.

Method: Multi-scale VQ-VAE extending VQ-VAE-2 to spatiotemporal setting with two-level hierarchical latent structure using 3D residual convolutions. Incorporates perceptual loss from pre-trained VGG16. Lightweight (18.5M params) optimized for 64x64 video clips (32 frames at 16 FPS).

Result: Achieves 25.96 dB PSNR and 0.8375 SSIM on UCF101 test set. Outperforms single-scale baseline by 1.41 dB PSNR and 0.0248 SSIM on validation. Model is lightweight and suitable for edge deployment.

Conclusion: The framework enables efficient video compression for bandwidth-sensitive applications like real-time streaming, mobile video analytics, and CDN storage optimization by generating compact, high-fidelity latent representations suitable for edge devices.
...
TL;DR: PhyGDPO: A physics-aware video generation method using groupwise preference optimization with physics-guided rewards, trained on a large-scale physics-augmented dataset.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video generation methods struggle to produce physically consistent videos due to limited physics reasoning capabilities and scarcity of training data with rich physical interactions.

Method: 1) PhyAugPipe: Uses vision-language model with chain-of-thought reasoning to create PhyVidGen-135K dataset. 2) PhyGDPO: Physics-aware Groupwise Direct Preference Optimization with Physics-Guided Rewarding scheme and LoRA-Switch Reference for efficient training.

Result: Significantly outperforms state-of-the-art open-source methods on PhyGenBench and VideoPhy2 benchmarks.

Conclusion: The proposed physics-aware framework effectively improves physical consistency in video generation through data construction and optimization techniques.
...
TL;DR: Proposes a novel second-order optimization algorithm for deep learning that uses OCP method and Hessian diagonal approximation to achieve faster convergence, better stability, and improved robustness in visual localization tasks.


<details>
  <summary>Details</summary>
Motivation: Address large-scale optimization problems in deep learning by developing a more efficient second-order optimization method that can handle the computational challenges of Hessian matrices while improving training performance.

Method: A novel second-order optimization algorithm that incorporates the OCP (Optimal Control Problem) method and appropriately approximates the diagonal elements of the Hessian matrix to reduce computational complexity while maintaining optimization effectiveness.
...
Conclusion: The proposed second-order optimization framework successfully addresses large-scale optimization challenges in deep learning, offering a practical solution that combines computational efficiency with superior performance in visual localization applications.
...
TL;DR: RGBT-Ground: First large-scale RGB-Thermal visual grounding benchmark for complex real-world scenarios with unified framework and baseline model.


<details>
  <summary>Details</summary>
Motivation: Existing VG benchmarks are limited to clean environments (like COCO) and lack scene diversity, failing to reflect real-world complexities like illumination changes and weather conditions that are critical for evaluating model robustness in safety-critical applications.

Method: 1) Create RGBT-Ground benchmark with aligned RGB-Thermal image pairs, referring expressions, bounding boxes, and fine-grained annotations. 2) Establish unified visual grounding framework supporting uni-modal (RGB/TIR) and multi-modal (RGB-TIR) inputs. 3) Propose RGBT-VGNet baseline for fusing complementary visual modalities.

Result: RGBT-VGNet significantly outperforms adapted existing methods, particularly in challenging nighttime and long-distance scenarios. Benchmark enables comprehensive evaluation of robust grounding under diverse conditions.

Conclusion: RGBT-Ground addresses limitations of existing VG benchmarks by introducing thermal infrared modality for complex real-world scenarios, with RGBT-VGNet demonstrating superior performance, especially in challenging conditions. Resources will be publicly released to advance robust visual grounding research.
...
<details>
  <summary>Details</summary>
Motivation: Current CDVQA models often fail due to decision ambiguity rather than clearly incorrect predictions, where models assign similar confidence to correct answers and strong distractors, highlighting the need for better discriminability.
...
Conclusion: Explicitly optimizing decision-ambiguous samples is crucial for CDVQA performance, and DARFT effectively suppresses strong distractors and sharpens decision boundaries without additional supervision.
...
<details>
  <summary>Details</summary>
Motivation: Existing slice discovery methods are limited to image classification and fail on multi-instance tasks like detection/segmentation. Current approaches lack fine-grained reasoning for complex visual relationships, and benchmarks are biased toward classification with artificial ground truth.
...
Result: SliceLens achieves state-of-the-art performance, improving Precision@10 by 0.42 (0.73 vs 0.31) on FeSD benchmark, and identifies interpretable slices that facilitate actionable model improvements through model repair experiments.

Conclusion: The proposed framework effectively addresses limitations of existing methods by enabling fine-grained error slice discovery across instance-level vision tasks with expert-annotated benchmarks, providing interpretable insights for model improvement.
...
TL;DR: Researchers created a 3D dataset from UAV footage of Hurricane Ian to evaluate SOTA 3D semantic segmentation models for post-disaster assessment, revealing significant limitations in existing methods for disaster environments.


<details>
  <summary>Details</summary>
Motivation: Natural disasters cause severe threats to human lives and economic losses, but existing 3D semantic segmentation models lack datasets specifically designed for post-disaster environments, creating a gap in effective disaster assessment.

Method: Constructed a specialized 3D dataset using UAV-captured aerial footage of Hurricane Ian (2022), employing Structure-from-Motion (SfM) and Multi-View Stereo (MVS) techniques to reconstruct 3D point clouds, then evaluated SOTA models (Fast Point Transformer, Point Transformer v3, OA-CNNs) on this dataset.
...
TL;DR: CLoRA is a novel collaborative low-rank adaptation method for vision transformers that balances parameter efficiency and learning performance through base-space sharing and diversity enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing LoRA methods for vision transformers either sacrifice fine-tuning performance or introduce excessive trainable parameters, failing to balance learning performance and parameter efficiency.
...
Conclusion: CLoRA effectively addresses the parameter efficiency vs. performance trade-off in vision transformer fine-tuning through collaborative low-rank adaptation with diversity regularization.
...
TL;DR: Introduces MoniRefer dataset and Moni3DVG method for 3D visual grounding in outdoor monitoring scenarios, addressing roadside infrastructure needs.


<details>
  <summary>Details</summary>
Motivation: Existing 3D visual grounding focuses on indoor/outdoor driving scenes, but roadside monitoring scenarios remain unexplored due to lack of paired point cloud-text data from infrastructure sensors. This is critical for traffic scene understanding beyond ego-vehicle perspective.
...
Result: Creates MoniRefer dataset with ~136,018 objects and 411,128 natural language expressions from real-world traffic intersections. Extensive experiments demonstrate method superiority and effectiveness.

Conclusion: Introduces novel 3D visual grounding task for outdoor monitoring scenarios, provides first large-scale dataset (MoniRefer), and proposes effective method (Moni3DVG) that outperforms benchmarks.
...
TL;DR: A Layer-by-Layer Hierarchical Attention Network that improves feature point matching by addressing outlier issues through stage fusion, hierarchical extraction, and attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Feature point matching is fundamental but suffers from outlier contamination, which reduces accuracy and robustness. The challenge is extracting high-quality information while minimizing errors from negative samples, especially with high outlier proportions.

Method: Proposes a Layer-by-Layer Hierarchical Attention Network with: 1) Layer-by-layer channel fusion module preserving semantic information from each stage, 2) Hierarchical attention module adaptively capturing global perception and structural semantics, 3) Two architectures for feature extraction and integration.

Result: Experiments on YFCC100M and SUN3D datasets show superior performance over state-of-the-art methods in both outlier removal and camera pose estimation.

Conclusion: The proposed network effectively addresses outlier issues in feature point matching through hierarchical attention mechanisms and stage fusion, demonstrating improved accuracy and robustness for computer vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing fire detection research focuses on natural environments (mountains/forests) but neglects complex urban rescue scenes, and has limited target classes (only flames/smoke) missing crucial command decision targets like fire trucks and firefighters.

Method: 1) Created FireRescue dataset with 15,980 images, 32,000 bounding boxes covering urban/mountain/forest/water scenes and 8 key categories. 2) Proposed FRS-YOLO with plug-and-play multidimensional collaborative enhancement attention module for cross-dimensional feature interaction to distinguish confusing categories, and dynamic feature sampler to strengthen foreground features against smoke occlusion/background interference.

Result: Object detection in fire rescue scenarios is highly challenging, and the proposed FRS-YOLO effectively improves detection performance of YOLO series models in this context.

Conclusion: The paper addresses limitations in fire rescue object detection by creating a comprehensive dataset and developing an improved YOLO model with attention mechanisms and feature enhancement to handle complex rescue scenarios with diverse targets and challenging conditions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional autoregressive models for visual generation suffer from low inference efficiency due to sequential token-by-token decoding, despite their success in language modeling. Visual tokens have strong local dependencies and spatial correlations that aren't fully exploited by standard raster-scan decoding orders.

Method: Organizes generation around radial topology: selects initial token as center, groups other tokens into concentric rings by spatial distance, generates ring-wise from inner to outer regions (parallel prediction within same ring). Introduces nested attention mechanism to refine implausible outputs during forward pass, mitigating error accumulation.

Result: Significantly improves generation efficiency while preserving representational capacity and structural locality of visual scenes. The framework maintains spatial coherence and prevents model collapse through dynamic output correction.

Conclusion: RadAR successfully addresses the efficiency limitations of traditional autoregressive visual generation by leveraging spatial correlations through radial parallel prediction and nested attention, enabling faster generation without sacrificing quality.
...
TL;DR: RGTN is a physics-inspired tensor network structure search framework that uses multi-scale renormalization group flows to automatically discover optimal network topologies, achieving state-of-the-art compression with 4-600× speedup.
...
Conclusion: RGTN validates the effectiveness of physics-inspired approaches for tensor network structure search, overcoming key challenges of existing methods through multi-scale renormalization group guidance.
...
TL;DR: EVOL-SAM3 is a zero-shot reasoning segmentation framework that uses evolutionary search with iterative prompt refinement through a "Generate-Evaluate-Evolve" loop, outperforming both static baselines and supervised methods.


<details>
  <summary>Details</summary>
Motivation: Current reasoning segmentation approaches have significant limitations: SFT suffers from catastrophic forgetting and domain dependency, RL has training instability and rigid reward functions, while training-free methods use static inference paradigms with insufficient reasoning depth and lack self-correction capabilities.

Method: EVOL-SAM3 reformulates reasoning segmentation as an inference-time evolutionary search process. It maintains a population of prompt hypotheses and refines them through a "Generate-Evaluate-Evolve" loop with Visual Arena for reference-free pairwise fitness assessment, Semantic Mutation for diversity injection and error correction, and Heterogeneous Arena for integrating geometric priors with semantic reasoning.
...
Conclusion: EVOL-SAM3 demonstrates that evolutionary search with iterative refinement can overcome limitations of current reasoning segmentation approaches, achieving superior performance without training while enabling self-correction of linguistic hallucinations and spatial misinterpretations.
...
Method: Caption-Prompt-Judge (CPJ) framework: 1) Generate multi-angle image captions using large vision-language models, 2) Refine captions iteratively via LLM-as-Judge module, 3) Use refined captions in dual-answer VQA process for both disease recognition and management responses.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is that model capacity impact varies across timesteps - crucial for early and late stages but negligible during intermediate stages. This insight enables optimization of computational resources without sacrificing quality.

Method: FlowBlending uses a large model and small model at different stages: large model for capacity-sensitive early/late stages, small model for intermediate stages. Includes criteria for stage boundaries and velocity-divergence analysis to identify capacity-sensitive regions.

Result: Achieves up to 1.65x faster inference with 57.35% fewer FLOPs while maintaining visual fidelity, temporal coherence, and semantic alignment of large models. Compatible with existing sampling-acceleration techniques for up to 2x additional speedup.

Conclusion: FlowBlending effectively leverages varying model capacity requirements across timesteps to optimize inference efficiency without compromising quality, demonstrating practical benefits across different model scales.
...
TL;DR: EchoFoley introduces video-grounded sound generation with fine-grained control over when, what, and how sounds are produced, addressing limitations in current VT2A models through a new task, benchmark dataset, and agentic framework.
...
Method: Proposes EchoFoley task with symbolic representation for sounding events specifying when, what, and how sounds are produced. Creates EchoFoley-6k benchmark dataset with 6,000+ expert-curated video-instruction-annotation triplets. Develops EchoVidia framework with sounding-event-centric agentic generation using slow-fast thinking strategy.
...
Conclusion: The EchoFoley framework successfully addresses VT2A limitations by enabling fine-grained control over sound generation through symbolic event representation and hierarchical semantic control, with substantial improvements over existing methods.
...
Result: The toolkit enables comprehensive benchmarking with metrics including image-based quality metrics, chamfer distance of reconstructed mesh, rendering frame rates, and computational resource consumption.

Conclusion: Splatwizard addresses the critical gap in standardized evaluation tools for 3DGS compression models, providing researchers with a unified framework to benchmark and compare different compression techniques effectively.
...
TL;DR: Unified framework for 3D instance segmentation that merges feature learning and label assignment into a single optimization process, addressing 2D label inconsistencies and improving performance with boundary-aware hard mining.


<details>
  <summary>Details</summary>
Motivation: Existing 3D segmentation methods suffer from inconsistent 2D instance labels across views, leading to poor 3D predictions. Current approaches use two-stage processes with hyperparameter-sensitive clustering or preprocessing, which are inefficient and suboptimal.

Method: Proposes a unified framework with learnable feature embeddings in Gaussian primitives, decoded into instance labels via "Embedding-to-Label" process. Addresses boundary artifacts with hard-mining on object boundaries, stabilized by applying a linear layer to rasterized embeddings before triplet loss calculation.

Result: Outperforms baselines qualitatively and quantitatively on ScanNet, Replica3D, and Messy-Rooms datasets, with reduced training time and improved performance compared to existing methods.

Conclusion: The unified framework effectively integrates feature learning and label assignment, with boundary-aware hard mining stabilizing training and enhancing segmentation quality, advancing 3D scene understanding capabilities.
...
TL;DR: Proposes a physics-in-the-loop adversarial attack method using projected light perturbations to make DNN-based monocular depth estimation models misestimate depth, causing objects to disappear from scenes.


<details>
  <summary>Details</summary>
Motivation: DNN-based monocular depth estimation models are vulnerable to adversarial attacks, threatening their reliability in practical applications. Need to validate this vulnerability and enhance robustness.

Method: Projection-based adversarial attack method that projects perturbation light onto target objects. Uses physics-in-the-loop optimization with actual environment evaluation to account for device specifications and disturbances. Employs distributed covariance matrix adaptation evolution strategy for optimization.

Result: Successfully created adversarial examples that cause depth misestimations, resulting in parts of objects disappearing from the target scene.

Conclusion: Demonstrates vulnerability of DNN-based MDE models to physical adversarial attacks using projected light perturbations, highlighting need for robustness enhancement in practical applications.
...
TL;DR: The paper presents a method to enable Noise2Noise training with certain nonlinear functions, overcoming the bias limitation when applying nonlinearities to noisy targets, demonstrated on HDR image denoising.


<details>
  <summary>Details</summary>
Motivation: Noise2Noise training requires noisy target images but has a major limitation: nonlinear functions applied to noisy targets introduce bias, limiting preprocessing options. This is particularly problematic for HDR image denoising where nonlinear tone mapping is needed to handle outliers.

Method: Developed a theoretical framework to analyze nonlinear function effects on noisy targets, identifying a class of nonlinear functions with minimal bias. Applied this to HDR image denoising by combining specific loss functions and tone mapping functions that reduce outlier effects while introducing minimal bias.

Result: The method was applied to an existing machine learning-based Monte Carlo denoiser, achieving results approaching the original implementation (trained with high-sample count reference images) using only noisy training data.

Conclusion: Certain nonlinear functions can be applied to noisy targets in Noise2Noise training without significant bias, enabling effective preprocessing like tone mapping for HDR image denoising while maintaining the advantage of not requiring clean training data.
...
TL;DR: A method that enables 2D-trained cross-modal systems to adapt to 3D scenes using regret minimization and derivative-free optimization to control an in-scene camera, improving performance without pretraining.
...
Method: Improves multivariate mutual information estimates via regret minimization with derivative-free optimization, enabling off-the-shelf 2D-trained systems to adapt online to object occlusions and differentiate features. Uses expressive measures and value-based optimization to control an in-scene camera directly from noisy vision-language model outputs.
...
Conclusion: The method successfully bridges the dimensionality gap between 2D-trained cross-modal systems and 3D scenes through adaptive camera control and optimization techniques, enabling effective 3D scene understanding without additional training.
...
TL;DR: CropTrack is a novel MOT framework for agricultural environments that combines appearance and motion information to address challenges like repetitive patterns, similar appearances, and frequent occlusions.


<details>
  <summary>Details</summary>
Motivation: Current agricultural MOT trackers rely mainly on motion information and struggle with identity preservation during frequent occlusions. Appearance-based association is difficult due to high similarity of object appearances in agricultural settings.
...
Result: CropTrack outperforms traditional motion-based tracking methods on agricultural MOT datasets, achieving significant gains in identification F1 and association accuracy scores with fewer identity switches.

Conclusion: The combination of appearance and motion information in CropTrack effectively addresses agricultural MOT challenges, demonstrating consistent identity preservation and superior performance compared to state-of-the-art methods.
...
TL;DR: MLLMs show poor 3D spatial reasoning in embodied navigation despite strong vision-language capabilities, revealed through a new benchmark VLN-MME.


<details>
  <summary>Details</summary>
Motivation: To investigate MLLMs' potential as embodied agents for navigation tasks requiring multi-round dialogue, spatial reasoning, and sequential action prediction, which remains underexplored.

Method: Introduce VLN-MME, a unified evaluation framework that bridges traditional navigation datasets into a standardized benchmark for probing MLLMs as zero-shot agents, with modular design for structured comparisons and component-level ablations.

Result: Enhancing baseline agents with Chain-of-Thought reasoning and self-reflection unexpectedly decreases performance, revealing MLLMs' poor context awareness and low 3D spatial reasoning fidelity in embodied navigation tasks.

Conclusion: VLN-MME provides systematic evaluation groundwork and reveals MLLMs' limitations in sequential decision-making for embodied navigation, offering crucial guidance for MLLM post-training as embodied agents.
...
<details>
  <summary>Details</summary>
Motivation: While SAM2 shows promise for medical image segmentation, adapting it requires extensive annotated data and high-quality manual prompts, which are labor-intensive and require medical expert intervention. There's a need for a more label-efficient approach.

Method: Introduces OFL-SAM2 with two key components: (1) an online few-shot learner that trains a lightweight mapping network to generate target features using limited data, and (2) an adaptive fusion module that dynamically integrates these target features with SAM2's memory-attention features. The mapping network supports online parameter updates during inference.
...
Conclusion: OFL-SAM2 provides an effective prompt-free framework for label-efficient medical image segmentation by leveraging limited annotated samples to capture medical knowledge and eliminate the need for manual prompts, while maintaining strong generalization through online parameter updates.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the complexity of real-world financial reasoning. The authors aim to create a more comprehensive benchmark that captures the challenges of financial document understanding, scenario-based reasoning, and multi-step computations in bilingual settings.

Method: Created a benchmark with 1,200 expert-annotated problems incorporating 12 types of implicit financial scenarios. Used 837 Chinese/English documents spanning 9 document types with rich visual elements. Problems require multi-step reasoning (11 steps on average) with cross-page evidence gathering.

Result: The best-performing MLLM achieved only 58.0% accuracy on the benchmark. Different RAG methods showed significant performance variations, indicating the benchmark's challenging nature and ability to differentiate model capabilities.

Conclusion: FinMMDocR successfully creates a challenging benchmark for evaluating MLLMs on complex financial reasoning tasks. The poor performance of current models (58% accuracy) demonstrates the need for improvement in multimodal reasoning, and the benchmark is expected to drive advancements in MLLMs and reasoning-enhanced methods.
...
<details>
  <summary>Details</summary>
Motivation: 3D object detectors trained on standard benchmarks struggle to generalize across different geographic domains (e.g., US-trained models performing poorly in Asia/Europe), creating a need for efficient domain adaptation methods.

Method: Novel lidar domain adaptation based on neuron activation patterns, selecting a small but representative and diverse subset of target domain samples for annotation, combined with post-training techniques inspired by continual learning to prevent weight drift.

Result: The approach outperforms both linear probing and state-of-the-art domain adaptation techniques while requiring very small annotation budget.

Conclusion: Effective domain adaptation for 3D object detectors can be achieved with minimal annotation effort by strategically selecting diverse target samples based on neuron activation patterns and preventing weight drift through continual learning techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing open-source film restoration methods have limited performance due to training with low-quality synthetic data, noisy optical flows, and inability to handle high-resolution films. There's a need for better tools to help human experts restore indistinguishable film defects.

Method: Proposes HaineiFRDM framework with: 1) Patch-wise training/testing for high-resolution processing on 24GB VRAM GPU, 2) Position-aware Global Prompt and Frame Fusion Modules, 3) Global-local frequency module for consistent texture reconstruction, 4) Two-stage restoration (low-res as global residual) to mitigate blocky artifacts, 5) New dataset with real-degraded films and realistic synthetic data.

Result: Comprehensive experiments demonstrate superior defect restoration ability over existing open-source methods. The model effectively handles high-resolution film restoration with better quality than previous approaches.

Conclusion: HaineiFRDM successfully addresses limitations of existing open-source film restoration methods by leveraging diffusion models' content understanding, enabling high-resolution processing on consumer hardware, and providing superior restoration quality through novel architectural designs and a comprehensive dataset.
...
<details>
  <summary>Details</summary>
Motivation: CAC scoring from chest CT is important for cardiovascular risk assessment, but non-gated CT scans suffer from motion artifacts that reduce accuracy. While ECG-gated cardiac CTs reduce motion, they're not widely available for population screening due to cost and technical requirements.

Method: ProDM uses three key components: 1) CAC motion simulation engine that synthesizes realistic non-gated CTs from cardiac-gated CTs for supervised training without paired data, 2) property-aware learning with calcium-specific priors and differentiable calcium consistency loss to preserve lesion integrity, and 3) progressive correction scheme that gradually reduces artifacts across diffusion steps for stability.

Result: ProDM significantly improves CAC scoring accuracy, spatial lesion fidelity, and risk stratification performance compared to baselines. Reader studies confirm it suppresses motion artifacts and improves clinical usability of non-gated CT scans.

Conclusion: ProDM demonstrates the potential of progressive, property-aware generative frameworks for reliable CAC quantification from routine chest CT imaging, making cardiovascular risk assessment more accessible.
...
<details>
  <summary>Details</summary>
Motivation: Existing video reasoning evaluations rely on single-frame assessments, leading to outcome-hacking where models reach correct conclusions through erroneous processes. There's a need for process-aware evaluation to assess reasoning validity.

Method: Proposed VIPER benchmark with 16 tasks across six reasoning domains (temporal, structural, symbolic, spatial, physics, planning). Introduced Process-outcome Consistency (POC@r) metric using VLM-as-Judge with hierarchical rubric to evaluate intermediate steps and final results.
...
Conclusion: Current video generation models lack true reasoning capabilities despite correct final answers. VIPER benchmark enables process-aware evaluation, highlighting the need for improved reasoning in video generation models.
...
Method: 1) Unified Discrete-Continuous Actions model integrating clicks and drags in shared architecture; 2) Flow-based Action Generation using lightweight action expert to predict incremental cursor adjustments from continuous visual observations; 3) Created ScreenDrag benchmark with 20K manually collected drag trajectories across 5 domains (PowerPoint, Adobe Premiere Pro, etc.) with comprehensive evaluation protocols.

Result: ShowUI-π achieves 26.98 score on ScreenDrag benchmark with only 450M parameters, outperforming proprietary GUI agents (Operator: 13.27, best Gemini-2.5-CUA: 22.18). Demonstrates both task difficulty and approach effectiveness.

Conclusion: ShowUI-π advances GUI agents toward human-like dexterous control in digital environments by enabling continuous drag actions alongside discrete clicks, with open-sourced code available for community use.
...
TL;DR: Compression techniques (quantization, pruning, clustering) can preserve or even improve model robustness against natural corruption when properly configured, especially on complex architectures.


<details>
  <summary>Details</summary>
Motivation: Compressed models are essential for resource-constrained devices, but compression may affect robustness under natural corruption, requiring comprehensive evaluation of robustness-accuracy-compression trade-offs.
...
Result: Certain compression strategies preserve or improve robustness, particularly on complex architectures. Customized technique combinations produce beneficial multi-objective results for robust and efficient deployment.

Conclusion: Carefully selected compression method combinations can maintain or enhance robustness while achieving compression, providing insights for deploying robust models in corrupted real-world environments.
...
TL;DR: DarkEQA is a new benchmark for evaluating Vision Language Models on embodied question answering tasks under realistic low-light conditions, revealing significant performance limitations of current VLMs in dark environments.


<details>
  <summary>Details</summary>
Motivation: Current VLM benchmarks focus on ideal lighting conditions, but real-world embodied agents need to operate 24/7 in various lighting conditions including low-light environments. There's a critical gap in evaluating VLMs' perceptual capabilities under realistic visual degradations like darkness, which is essential for robust autonomous systems.

Method: Created DarkEQA benchmark with physics-based low-light simulation: 1) Models illumination drop and sensor noise in linear RAW space, 2) Uses ISP-inspired rendering pipeline for realistic image degradation, 3) Evaluates question answering from egocentric observations under controlled multi-level low-light conditions, 4) Tests both VLMs and Low-Light Image Enhancement models.

Result: Systematic evaluation reveals significant limitations of state-of-the-art VLMs when operating under low-light conditions. The benchmark enables attributable robustness analysis by isolating perception bottlenecks in embodied question answering tasks.

Conclusion: DarkEQA addresses a critical gap in VLM evaluation for embodied AI, providing a physically realistic benchmark for low-light performance. The findings highlight the need for improved robustness in VLMs for real-world 24/7 operation, and the benchmark will be released as open-source to advance research in this area.
...
TL;DR: This paper introduces RFL-ReID, a new lifelong person re-identification task that eliminates the need for re-indexing historical gallery images, addressing privacy and cost concerns while maintaining feature compatibility across model updates.


<details>
  <summary>Details</summary>
Motivation: Traditional lifelong person ReID requires re-indexing historical gallery images after each model update, which raises privacy concerns and incurs high computational costs. The authors aim to solve this by creating a re-index free approach that maintains feature compatibility between old and new models.

Method: The paper proposes a Bidirectional Continuous Compatible Representation (Bi-C2R) framework that continuously updates gallery features extracted by old models to enable compatible lifelong ReID without re-indexing historical data.

Result: Extensive experiments on multiple benchmarks demonstrate that Bi-C2R achieves leading performance on both the new RFL-ReID task and traditional L-ReID tasks, with theoretical analysis supporting the approach.

Conclusion: The proposed RFL-ReID task and Bi-C2R framework successfully address privacy and cost issues in lifelong person ReID by eliminating re-indexing requirements while maintaining feature compatibility and achieving state-of-the-art performance.
...
TL;DR: FoundationSLAM is a learning-based monocular dense SLAM system that integrates foundation depth models with flow estimation to achieve geometric consistency for accurate tracking and mapping.


<details>
  <summary>Details</summary>
Motivation: Previous flow-based SLAM approaches lack geometric consistency, leading to inaccurate tracking and mapping. The paper aims to bridge flow estimation with geometric reasoning to create a more robust and accurate monocular dense SLAM system.

Method: 1) Hybrid Flow Network that produces geometry-aware correspondences using guidance from foundation depth models; 2) Bi-Consistent Bundle Adjustment Layer for joint optimization of keyframe pose and depth under multi-view constraints; 3) Reliability-Aware Refinement mechanism that dynamically adapts flow updates by distinguishing reliable vs uncertain regions.
...
Conclusion: FoundationSLAM successfully addresses geometric consistency issues in flow-based SLAM by integrating foundation depth models, achieving state-of-the-art performance with practical real-time applicability and strong generalization capabilities.
...
TL;DR: A novel self-bootstrapping framework transforms visual dubbing from ill-posed inpainting to well-conditioned video-to-video editing using Diffusion Transformers to generate ideal training data and achieve precise lip synchronization with identity preservation.
...
Result: The method achieves highly accurate lip sync, faithful identity preservation, and exceptional robustness against challenging in-the-wild scenarios by leveraging complete frame-aligned input conditioning that provides rich visual context including identity cues, scene interactions, and spatiotemporal dynamics.

Conclusion: Reframing visual dubbing from ill-posed inpainting to well-conditioned video-to-video editing with self-bootstrapped training data enables superior performance in lip synchronization, identity preservation, and robustness, supported by a comprehensive benchmark for practical evaluation.
...
Result: Outperforms previous methods on both coarse-grained (NTU-60, NTU-120) and fine-grained (Gym99, Gym288) benchmarks under various corruption levels. Achieves 89.1% top-1 accuracy on Gym99-severe and 78.1% on Gym288-severe.

Conclusion: FineTec effectively handles temporal corruption in skeleton sequences for fine-grained action recognition by combining context-aware completion, spatial decomposition, physics-driven acceleration estimation, and GCN-based recognition, demonstrating robustness and generalizability.
...
TL;DR: Edit3r is a feed-forward framework that reconstructs and edits 3D scenes from unposed, view-inconsistent instruction-edited images in a single pass, without per-scene optimization or pose estimation.


<details>
  <summary>Details</summary>
Motivation: Prior 3D editing methods require per-scene optimization which is slow and computationally expensive. There's a need for fast, feed-forward 3D editing that can handle unposed, view-inconsistent images without optimization.

Method: Uses a feed-forward framework with two key components: (1) SAM2-based recoloring strategy for generating cross-view-consistent supervision, and (2) asymmetric input strategy pairing recolored reference view with raw auxiliary views to fuse and align disparate observations.
...
Conclusion: Edit3r enables fast, photorealistic 3D scene editing without optimization or pose estimation, making it promising for real-time 3D editing applications. The method also introduces DL3DV-Edit-Bench for large-scale quantitative evaluation.
...
TL;DR: GaMO is a geometry-aware multi-view outpainting framework for sparse-view 3D reconstruction that expands field of view from existing camera poses instead of generating new viewpoints, achieving better geometric consistency, broader coverage, and 25× speedup over SOTA methods.
...
Result: State-of-the-art reconstruction quality on Replica and ScanNet++ datasets across 3, 6, and 9 input views, outperforming prior methods in PSNR and LPIPS metrics, with 25× speedup over SOTA diffusion-based methods (processing time under 10 minutes).

Conclusion: GaMO effectively addresses limitations of current sparse-view reconstruction methods by preserving geometric consistency through view outpainting rather than novel view generation, achieving superior performance with significantly reduced computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Current video generation models lack explicit disentangled control over spatial (camera viewpoint) and temporal (motion sequence) dimensions, limiting creative exploration of dynamic scenes.

Method: Introduces animation time-embedding for explicit motion control, temporal-warping training scheme using multi-view datasets, improved camera-conditioning, and CamxTime synthetic dataset for joint training.

Result: Demonstrates clear space-time disentanglement on real-world and synthetic data, achieving strong results compared to prior work with precise dual control over camera and motion.

Conclusion: SpaceTimePilot enables continuous, arbitrary exploration of dynamic scenes across space and time through effective disentanglement mechanisms and novel training strategies.
...
### [173] [Large language models and the entropy of English](https://arxiv.org/abs/2512.24969)
*Colin Scheibner,Lindsay M. Smith,William Bialek*

Main category: cond-mat.stat-mech

TL;DR: LLMs reveal long-range correlations in English texts up to ~10^4 characters, showing that language has dependencies across large distances that are learned gradually during training.


<details>
  <summary>Details</summary>
Motivation: To understand the statistical structure of language and investigate whether there are long-range dependencies in English texts that extend beyond typical context windows used in language modeling.

Method: Use large language models to analyze conditional entropy and code length across different context lengths in English texts from various sources, examining how these metrics change with increasing context size up to ~10^4 characters.

Result: Conditional entropy continues to decrease with context length up to at least 10^4 characters, revealing direct dependencies across large distances. Small but significant correlations exist between characters at these separations. The distribution shows emergent certainty about characters at large N, and long-range structure is learned gradually during training.

Conclusion: Language exhibits long-range statistical structure that extends far beyond typical context windows, constraining efforts to build statistical physics models of LLMs or language itself, and suggesting that language has dependencies operating across thousands of characters.
...
### [174] [Vibe Coding, Interface Flattening](https://arxiv.org/abs/2512.24939)
*Hongrui Jin*

Main category: cs.HC

TL;DR: Vibe coding through LLMs flattens programming interfaces into conversation while thickening underlying infrastructure, redistributing power to model providers and creating new dependencies.


<details>
  <summary>Details</summary>
Motivation: To critically analyze the political economy of LLM-mediated programming ("vibe coding") by examining how it reconfigures interfaces, redistributes power, and creates new dependencies despite appearing to democratize technical capability.

Method: Drawing on Friedrich Kittler's materialist media theory and Alexander Galloway's interface theory, the paper conducts a materialist reconstruction of the contemporary vibe-coding stack, analyzing remote compute infrastructures, latency, structured outputs, function calling, and interoperability standards like Model Context Protocol.

Result: The analysis reveals that while vibe coding appears to flatten interfaces (GUI, CLI, API) into a single conversational surface, it actually lengthens and thickens the translation chain from intention to execution, relocating control and meaning-making power to model and protocol providers.

Conclusion: LLM-mediated development redistributes symbolic labor/power, obscures responsibility, and privatizes competencies previously dispersed across programming communities, creating new dependencies and literacies while appearing to democratize technical capability.
...
### [175] [Towards autonomous time-calibration of large quantum-dot devices: Detection, real-time feedback, and noise spectroscopy](https://arxiv.org/abs/2512.24894)
...
<details>
  <summary>Details</summary>
Motivation: Electrostatic drift and charge noise limit quantum-dot qubit performance and scalability. Manual recalibration becomes impractical for large arrays, requiring autonomous stabilization frameworks.

Method: Uses charge-transition lines in repeatedly acquired double-quantum-dot charge stability diagrams as a multidimensional probe. Tracks motion of selected transitions to detect voltage drifts, identify charge reconfigurations, and apply compensating updates.

Result: Demonstrated on 10-QD device with robust stabilization and real-time diagnostic access. Enabled time-domain noise spectroscopy revealing: background noise dominated by 1/f² drift at 100 μHz, few dominant two-level fluctuators, and average linear correlation length of 188±38 nm.

Conclusion: Forms basis for scalable, autonomous calibration and characterization module for QD-based quantum processors, providing essential feedback for long-duration, high-fidelity qubit operations.
...
### [176] [Leveraging Machine Learning for Early Detection of Lung Diseases](https://arxiv.org/abs/2512.23757)
...
TL;DR: Deep learning models (CNNs, VGG16, InceptionV3, EfficientNetB0) achieve high accuracy for diagnosing COVID-19, lung cancer, and pneumonia from chest X-rays, enabling rapid non-invasive diagnostics in resource-limited areas.


<details>
  <summary>Details</summary>
Motivation: To address healthcare disparities by providing rapid, accurate, and non-invasive diagnostic solutions for respiratory diseases in areas with limited access to radiologists and healthcare resources, improving patient outcomes through predictive and preventive healthcare.
...
Result: The models achieved high accuracy, precision, recall, and F1 scores, demonstrating reliability and potential for real-world diagnostic applications in detecting COVID-19, lung cancer, and pneumonia from chest X-rays.

Conclusion: Deep learning-based diagnostic systems can significantly impact patient outcomes by providing accessible, accurate respiratory disease detection, particularly benefiting resource-limited healthcare settings through a predictive and preventive healthcare paradigm.
...
### [177] [Targeted Semantic Segmentation of Himalayan Glacial Lakes Using Time-Series SAR: Towards Automated GLOF Early Warning](https://arxiv.org/abs/2512.24117)
*Pawan Adhikari,Satish Raj Regmi,Hari Ram Shrestha*

Main category: eess.IV

TL;DR: Automated deep learning pipeline for monitoring high-risk Himalayan glacial lakes using Sentinel-1 SAR time-series data with temporal-first training strategy and operational Docker architecture.


<details>
  <summary>Details</summary>
Motivation: Existing glacial lake monitoring approaches have limitations: they either prioritize spatial coverage for general models or rely on optical imagery that suffers from cloud coverage. There's a need for targeted, automated monitoring of high-risk Himalayan glacial lakes for early warning systems.

Method: End-to-end automated deep learning pipeline using Sentinel-1 SAR time-series data. Introduces "temporal-first" training strategy with U-Net architecture (EfficientNet-B3 backbone) trained on curated dataset of 4 high-risk Himalayan lakes. Operational architecture includes Dockerized pipeline with automated data ingestion via ASF Search API and RESTful endpoint for inference results.

Result: Model achieves IoU of 0.9130, validating the success of the "temporal-first" strategy. The system enables dynamic, automated monitoring rather than static mapping, providing scalable foundation for early warning systems.

Conclusion: The proposed pipeline successfully addresses limitations of existing approaches by providing automated, targeted monitoring of high-risk glacial lakes using SAR data. The temporal-first strategy and operational engineering architecture enable transition to practical early warning systems for GLOF hazards.
...
### [178] [Automated Classification of First-Trimester Fetal Heart Views Using Ultrasound-Specific Self-Supervised Learning](https://arxiv.org/abs/2512.24492)
*Youssef Megahed,Aylin Erman,Robin Ducharme,Mark C. Walker,Steven Hawken,Adrian D. C. Chan*

Main category: eess.IV

TL;DR: USF-MAE, a self-supervised ultrasound foundation model pretrained on 370K+ unlabeled ultrasound images, outperforms supervised CNN and ViT baselines for first-trimester fetal heart view classification, achieving 90.57% accuracy without aggressive preprocessing.


<details>
  <summary>Details</summary>
Motivation: Congenital heart disease is a leading cause of neonatal morbidity/mortality. Early detection via first-trimester fetal echocardiography is challenging due to small cardiac structures, low signal-to-noise ratio, and inter-operator variability. Automated analysis at this stage is difficult.

Method: USF-MAE uses masked autoencoding pretraining on 370,000+ unlabeled ultrasound images across 40+ anatomical regions. The pretrained Vision Transformer encoder is fine-tuned on 6,720 first-trimester fetal echocardiography images to classify five categories (aorta, atrioventricular flows, V sign, X sign, Other). Benchmarking against ResNet-18, ResNet-50, and ImageNet-pretrained ViT-B/16 with identical preprocessing and protocols.

Result: USF-MAE achieved best performance: 90.57% accuracy, 91.15% precision, 90.57% recall, 90.71% F1-score. Outperformed strongest baseline (ResNet-18) by +2.03% accuracy and +1.98% F1-score. Showed robust performance without aggressive preprocessing/ROI cropping and improved discrimination of non-diagnostic frames.

Conclusion: Self-supervised ultrasound foundation models like USF-MAE offer superior performance for first-trimester fetal heart view classification compared to supervised CNNs and natural image-pretrained models, potentially enabling earlier and more reliable congenital heart disease detection.
...
### [179] [RAIR: A Rule-Aware Benchmark Uniting Challenging Long-Tail and Visual Salience Subset for E-commerce Relevance Assessment](https://arxiv.org/abs/2512.24943)
*Chenji Lu,Zhuo Chen,Hui Zhao,Zhenyi Wang,Pengjie Wang,Jian Xu,Bo Zheng*

Main category: cs.IR

TL;DR: RAIR is a Chinese benchmark for e-commerce search relevance assessment with standardized rules, three challenging subsets, and multimodal evaluation capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing relevance benchmarks lack sufficient complexity for comprehensive model assessment, and there's an absence of standardized relevance evaluation metrics across the industry.

Method: Created RAIR dataset from real-world scenarios with standardized framework and universal rules. Includes three subsets: general (industry-balanced), long-tail hard (challenging cases), and visual salience (multimodal understanding).

Result: Tested 14 open and closed-source models. RAIR presents sufficient challenges even for GPT-5, which achieved best performance. Dataset is publicly available.

Conclusion: RAIR serves as an industry benchmark for relevance assessment and provides new insights into general LLM and VLM evaluation.
...
### [180] [Trellis: Learning to Compress Key-Value Memory in Attention Models](https://arxiv.org/abs/2512.23852)
*Mahdi Karami,Ali Behrouz,Praneeth Kacham,Vahab Mirrokni*

Main category: cs.LG

TL;DR: Trellis is a novel Transformer architecture with bounded memory that dynamically compresses KV cache at test time using a two-pass recurrent compression mechanism, achieving better performance especially for long sequences.


<details>
  <summary>Details</summary>
Motivation: Transformers suffer from quadratic computational complexity and growing KV cache memory requirements, which limits their efficiency and scalability for long-context applications.
...
Result: Outperforms strong baselines on language modeling, common-sense reasoning, recall-intensive tasks, and time series, with performance gains increasing as sequence length grows.

Conclusion: Trellis demonstrates effective bounded-memory Transformer architecture with dynamic KV compression, showing strong potential for long-context applications while maintaining computational efficiency.
...
### [181] [Probing the Limits of Compressive Memory: A Study of Infini-Attention in Small-Scale Pretraining](https://arxiv.org/abs/2512.23862)
*Ruizhe Huang,Kexuan Zhang,Yihao Fang,Baifeng Yu*

Main category: cs.LG

TL;DR: Infini-attention improves long-context extrapolation in small language models (300M parameters) by using compressed memory from past segments while maintaining local attention, achieving up to 31% higher accuracy than baseline despite some degradation at extreme context lengths.


<details>
  <summary>Details</summary>
Motivation: To enable efficient small language models (SLMs) that can handle long contexts with limited data and compute resources, making them more accessible and cost-effective for low-resource settings.
...
Result: Infini-attention models show training stability and outperform baselines in long-context retrieval, achieving up to 31% higher accuracy despite some performance degradation at 16,384-token contexts. The balance factor is identified as crucial for performance.

Conclusion: Architectural memory mechanisms like Infini-attention are beneficial for achieving robust long-context capabilities in small language models, effectively compensating for their limited parameter counts.
...
### [182] [OptRot: Mitigating Weight Outliers via Data-Free Rotations for Post-Training Quantization](https://arxiv.org/abs/2512.24124)
*Advait Gadhikar,Riccardo Grazzi,James Hensman*

Main category: cs.LG

TL;DR: OptRot method reduces LLM quantization errors by learning fusible rotations that minimize weight outliers, outperforming existing rotation methods for weight quantization and improving W4A8 activation quantization.


<details>
  <summary>Details</summary>
Motivation: Large Language Models have outliers in weights and activations that make quantization difficult. Existing rotation methods help but have limitations - some are too simple (Hadamard), others are expensive and data-dependent (SpinQuant, OSTQuant).

Method: Proposes OptRot which learns fusible rotations by minimizing the element-wise fourth power of rotated weights (a cheap proxy objective). Also proposes OptRot⁺ which incorporates activation covariance information for data-dependent improvement. Both methods focus on GPTQ quantization.

Result: OptRot outperforms Hadamard rotations and more expensive data-dependent methods (SpinQuant, OSTQuant) for weight quantization. It also improves activation quantization in W4A8 setting. OptRot⁺ further improves performance with activation covariance information. However, both perform worse in W4A4 setting, revealing weight-activation quantization trade-off.

Conclusion: Learning fusible rotations through principled proxy objectives (like minimizing fourth power of weights) effectively reduces quantization errors. There's a trade-off between weight and activation quantization that becomes apparent in aggressive quantization settings like W4A4.
...
### [183] [More Than Bits: Multi-Envelope Double Binary Factorization for Extreme Quantization](https://arxiv.org/abs/2512.24545)
...
<details>
  <summary>Details</summary>
Motivation: Double Binary Factorization (DBF) is attractive for efficient LLM inference but has restrictive scaling parameters where all rank components share the same magnitude profile, leading to performance saturation in extreme low-bit quantization.

Method: Proposes Multi-envelope DBF (MDBF) that retains shared 1-bit sign bases but replaces the single envelope with rank-l envelopes. Uses closed-form initialization and alternating refinement optimization while maintaining binary carrier structure.

Result: MDBF improves perplexity and zero-shot accuracy over previous binary formats at matched bits per weight across LLaMA and Qwen families while preserving deployment-friendly inference primitives.

Conclusion: MDBF effectively addresses DBF's limitations by allocating limited memory budget to magnitude expressiveness while maintaining binary structure, achieving better quantization performance for LLMs.
...
### [184] [Many Minds from One Model: Bayesian Transformers for Population Intelligence](https://arxiv.org/abs/2512.25063)
*Diji Yang,Yi Zhang*
...
<details>
  <summary>Details</summary>
Motivation: Modern transformers are trained as single-minded deterministic systems, but intelligence emerges from many minds. The authors want to create models that can represent multiple functional hypotheses about data rather than just one.

Method: B-Trans treats bias-like offsets in normalization layers as stochastic variables with Gaussian variational approximation, creating a Bayesian posterior proxy. This allows sampling diverse model instances from pre-trained weights. To maintain coherence, sampled noise is frozen at sequence level for temporal consistency.

Result: Experiments show B-Trans yields superior semantic diversity while achieving better task performance compared to deterministic baselines across zero-shot generation, Reinforcement Learning with Verifiable Rewards (RLVR), and RL without explicit labels.

Conclusion: B-Trans successfully transforms standard LLMs into Bayesian models that support sampling diverse yet coherent model instances, enabling population-level decision-making that leverages the wisdom of crowds for improved performance and exploration.
...
### [185] [Scaling Open-Ended Reasoning to Predict the Future](https://arxiv.org/abs/2512.25070)
*Nikhil Chandak,Shashwat Goel,Ameya Prabhu,Moritz Hardt,Jonas Geiping*

Main category: cs.LG

TL;DR: Language models trained on synthesized forecasting questions from news events achieve state-of-the-art performance matching larger proprietary models, with improved accuracy, calibration, and consistency.


<details>
  <summary>Details</summary>
Motivation: To develop language models capable of high-stakes decision making through reasoning under uncertainty about future events, addressing the need for scalable forecasting systems.

Method: Automated curation of forecasting questions from daily news events using an offline corpus to prevent data leakage, training Qwen3 thinking models on OpenForesight dataset with retrieval and improved RL reward functions.

Result: OpenForecaster 8B matches larger proprietary models, shows improved accuracy, calibration, and consistency in predictions, with calibration improvements generalizing across benchmarks.

Conclusion: Specialized forecasting models trained on synthesized news-based questions can achieve competitive performance while improving calibration, with all models, code, and data open-sourced for broader accessibility.
...
### [186] [A Granular Grassmannian Clustering Framework via the Schubert Variety of Best Fit](https://arxiv.org/abs/2512.23766)
*Karim Salta,Michael Kirby,Chris Peterson*

Main category: cs.LG

TL;DR: A new subspace clustering algorithm using Schubert Variety of Best Fit (SVBF) as trainable prototypes instead of subspace means, integrated with LBG pipeline for improved cluster purity across various data types.


<details>
  <summary>Details</summary>
Motivation: Traditional subspace clustering uses geometric representatives like means or medians on Grassmann/flag manifolds, but these may not optimally capture subspace relationships. The paper aims to develop better prototypes that preserve mathematical structure while improving clustering performance.
...
Result: SVBF-LBG achieves improved cluster purity on synthetic, image, spectral, and video action data compared to traditional methods, while maintaining mathematical structure needed for downstream analysis.

Conclusion: SVBF prototypes provide superior geometric representatives for subspace clustering, offering both performance improvements and mathematical interpretability for various applications.
...
Conclusion: GARDO provides a versatile RL framework that successfully addresses the competing demands of sample efficiency, effective exploration, and reward hacking mitigation in diffusion model fine-tuning, demonstrating robustness across various proxy rewards and evaluation metrics.
...
Method: Proposes Geo-Consistent Visual Planning framework called ViReLoc that: 1) encodes step-by-step inference in visual domain, 2) uses reinforcement learning objectives for route planning between ground images, 3) integrates contrastive learning and adaptive feature interaction to align cross-view perspectives and reduce viewpoint differences.

Result: Experiments show consistent improvements in spatial reasoning accuracy and cross-view retrieval performance across diverse navigation and localization scenarios, demonstrating visual reasoning as a strong complementary approach for these tasks.

Conclusion: Visual reasoning can effectively perform navigation and localization without real-time GPS data, leading to more secure navigation solutions, establishing it as a valuable complementary approach to text-based reasoning for spatial tasks.
...
### [189] [Generative Classifiers Avoid Shortcut Solutions](https://arxiv.org/abs/2512.25034)
*Alexander C. Li,Ananya Kumar,Deepak Pathak*

Main category: cs.LG

TL;DR: Generative classifiers using class-conditional generative models outperform discriminative approaches on distribution shift by modeling all features rather than relying on spurious correlations.


<details>
  <summary>Details</summary>
Motivation: Discriminative classifiers often fail under distribution shift because they learn shortcuts and over-rely on features that are spuriously correlated with labels. This creates brittle models that don't generalize well.

Method: Use class-conditional generative models (diffusion-based and autoregressive models) as generative classifiers. These models naturally model all features (both core and spurious) rather than focusing mainly on spurious correlations. The approach is simple to train without specialized augmentations, strong regularization, or prior knowledge of spurious correlations.

Result: Generative classifiers achieve state-of-the-art performance on five standard image and text distribution shift benchmarks. They effectively reduce the impact of spurious correlations in realistic applications like medical or satellite datasets. Analysis of a Gaussian toy setting reveals the inductive biases of generative classifiers and the data properties determining when they outperform discriminative ones.

Conclusion: Generative classifiers provide a robust alternative to discriminative approaches for handling distribution shift, as they avoid over-reliance on spurious correlations by modeling all features. They offer simplicity in training and strong performance across diverse domains.
...
### [190] [AHA: Aligning Large Audio-Language Models for Reasoning Hallucinations via Counterfactual Hard Negatives](https://arxiv.org/abs/2512.24052)
...
TL;DR: The paper introduces AHA framework to reduce hallucinations in Large Audio-Language Models by creating a preference dataset through counterfactual hard negative mining, leading to improved performance on both diagnostic and public benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large Audio-Language Models suffer from hallucinations where they generate text not grounded in audio input, which undermines their reliability and practical utility.

Method: Developed AHA framework with counterfactual hard negative mining to create high-quality preference data that forces models to distinguish acoustic evidence from fabrications. Also established AHA-Eval benchmark for testing temporal reasoning. Applied this to align Qwen2.5-Omni.

Result: Resulting model Qwen-Audio-AHA achieves 13.7% improvement on AHA-Eval, plus substantial gains on public benchmarks: 1.3% on MMAU-Test and 1.6% on MMAR, outperforming latest SOTA methods.

Conclusion: The AHA framework effectively reduces hallucinations in audio-language models through targeted preference alignment, with benefits generalizing beyond diagnostic tests to real-world benchmarks.
...
### [191] [PhysTalk: Language-driven Real-time Physics in 3D Gaussian Scenes](https://arxiv.org/abs/2512.24986)
...
<details>
  <summary>Details</summary>
Motivation: Current visual effects pipelines lack physical realism and effective language interfaces, requiring slow offline optimization. There's a need for accessible, real-time animation tools that can translate natural language prompts into physically realistic visual effects.

Method: Uses 3D Gaussian Splatting (3DGS) scenes as input, with a large language model generating executable code that modifies 3DGS parameters through lightweight proxies and particle dynamics. Directly couples 3DGS with physics simulation without mesh extraction.

Result: Enables real-time, physics-based, interactive 4D animations from arbitrary text prompts. Provides collision-aware manipulation of multi-material objects while remaining computationally lightweight and train-free.

Conclusion: PhysTalk makes 4D animation broadly accessible by shifting from "render and wait" to interactive dialogue, offering a modern physics-informed pipeline for open-vocabulary visual effects generation.
...
TL;DR: The paper evaluates jailbreak attacks on LLMs across the full deployment pipeline including safety filters, finding that most jailbreaks are detectable by at least one filter, suggesting prior evaluations overestimated attack success.


<details>
  <summary>Details</summary>
Motivation: Previous jailbreak evaluations focused only on LLMs themselves, neglecting the complete deployment pipeline that includes additional safety mechanisms like content moderation filters, creating a gap in understanding real-world attack effectiveness.

Method: Conducted first systematic evaluation of jailbreak attacks targeting LLM safety alignment, assessing success across the full inference pipeline including both input and output filtering stages.

Result: Two key findings: 1) Nearly all evaluated jailbreak techniques can be detected by at least one safety filter, suggesting prior assessments overestimated practical success; 2) Safety filters are effective but need better balance between recall and precision for optimal protection and user experience.

Conclusion: Highlights critical gaps and calls for further refinement of detection accuracy and usability in LLM safety systems, emphasizing the need to evaluate attacks across the complete deployment pipeline rather than just the models.
...
TL;DR: ADS is a training-free defense that neutralizes hidden payloads in diffusion-based steganography by using a pretrained denoiser as proxy decoder with color-aware updates, achieving near-zero decoder success with minimal visual impact.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based generative steganography enables high-capacity covert communication that bypasses traditional detectors by generating image carriers directly from secret data, creating significant security threats for stegomalware delivery, command-and-control, and data exfiltration.

Method: Adversarial Diffusion Sanitization (ADS) uses an off-the-shelf pretrained denoiser as a differentiable proxy for diffusion-based decoders, incorporating a color-aware, quaternion-coupled update rule to reduce artifacts under strict distortion limits, all without requiring training.

Result: ADS drives decoder success rates to near zero against state-of-the-art diffusion steganography method Pulsar with minimal perceptual impact, providing a favorable security-utility trade-off compared to standard content transformations.

Conclusion: ADS offers an effective training-free mitigation strategy against diffusion-driven steganography for security gateways by neutralizing hidden payloads rather than detecting them, addressing the challenge of coverless diffusion-based techniques.
...
### [194] [q3-MuPa: Quick, Quiet, Quantitative Multi-Parametric MRI using Physics-Informed Diffusion Models](https://arxiv.org/abs/2512.23726)
*Shishuai Wang,Florian Wiesinger,Noemi Sgambelluri,Carolin Pirkl,Stefan Klein,Juan A. Hernandez-Tamames,Dirk H. J. Poot*

Main category: physics.med-ph

TL;DR: Proposes q3-MuPa: a physics-informed diffusion model for accelerated quantitative MRI mapping from silent MuPa-ZTE scans, achieving high-quality T1/T2/PD maps in ~1 minute using only synthetic training data.


<details>
  <summary>Details</summary>
Motivation: To improve quantitative MRI mapping performance while enabling faster acquisition (4x acceleration) and maintaining patient comfort through silent scanning. Addresses the challenge of obtaining large real-scan datasets for training by using synthetic data.

Method: Combines denoising diffusion probabilistic model (DDPM) with physics-based data consistency. Trains DDPM on synthetic data from digital brain phantoms to map MuPa-ZTE image series to qMRI maps, then incorporates MuPa-ZTE forward signal model as explicit data consistency constraint during inference.

Result: Method produces 3D qMRI maps with high accuracy, reduced noise, and better preservation of structural details compared to baseline dictionary matching and purely data-driven diffusion models. Generalizes well to real scans despite training only on synthetic data. Achieves high-quality mapping from 4x-accelerated scans (~1 minute).

Conclusion: The q3-MuPa framework (MuPa-ZTE acquisition + physics-informed diffusion model) offers quick, quiet, and quantitative multi-parametric mapping with strong clinical potential, demonstrating successful generalization from synthetic to real data.
...
### [195] [A multimodal Transformer for InSAR-based ground deformation forecasting with cross-site generalization across Europe](https://arxiv.org/abs/2512.23906)
...
<details>
  <summary>Details</summary>
Motivation: Near-real-time monitoring of ground deformation is crucial for urban planning, infrastructure management, and hazard mitigation, but predicting future observations from past InSAR data is challenging due to complex signal superposition and spatial heterogeneity.

Method: Proposes a multimodal patch-based Transformer for single-step, fixed-interval next-epoch nowcasting of displacement maps from EGMS time series. The model ingests recent displacement snapshots with static kinematic indicators (mean velocity, acceleration, seasonal amplitude) computed from training data only, plus harmonic day-of-year encodings.
...
Conclusion: The proposed multimodal patch-based Transformer effectively addresses the challenge of ground deformation nowcasting by combining recent displacement observations with kinematic indicators and temporal encodings, demonstrating superior performance over existing approaches.
...
### [196] [Quantum Visual Word Sense Disambiguation: Unraveling Ambiguities Through Quantum Inference Model](https://arxiv.org/abs/2512.24687)
*Wenbo Qiao,Peng Zhang,Qinghua Hu*

Main category: quant-ph

TL;DR: Q-VWSD: Quantum-inspired model for visual word sense disambiguation that encodes multiple glosses into quantum superposition states to mitigate semantic biases, outperforming classical methods.


<details>
  <summary>Details</summary>
Motivation: Traditional visual word sense disambiguation methods using classical probability suffer from semantic biases in glosses from different sources, leading to biased results. There's a need to better handle semantic uncertainty in polysemous word disambiguation.

Method: Proposes a Quantum Inference Model for Unsupervised Visual Word Sense Disambiguation (Q-VWSD) that encodes multiple glosses of target words into quantum superposition states to mitigate semantic biases. The quantum circuit is executed and results observed. Also develops a heuristic classical version for efficient computation on classical hardware.
...
Conclusion: The method showcases potential of quantum machine learning in practical applications and provides a case for leveraging quantum modeling advantages on classical computers while quantum hardware remains immature. Q-VWSD represents a quantum generalization of classical probability-based approaches.
...
### [197] [One-Shot Structured Pruning of Quantum Neural Networks via $q$-Group Engineering and Quantum Geometric Metrics](https://arxiv.org/abs/2512.24019)
...
Method: Proposes q-iPrune, a one-shot structured pruning framework based on q-deformed groups and task-conditioned quantum geometry. Uses algebraic subgroup comparisons with task-conditioned q-overlap distance to measure functional similarity, removing gates only when replacement induces bounded deviation on all task observables.
...
Conclusion: q-iPrune offers a theoretically grounded approach to quantum circuit compression that addresses gate-level redundancy with rigorous guarantees, making QNNs more practical for NISQ devices.
...
### [198] [State-of-the-art Small Language Coder Model: Mify-Coder](https://arxiv.org/abs/2512.23747)
...
TL;DR: Mify-Coder is a 2.5B-parameter code model trained on 4.2T tokens using compute-optimal strategy that achieves comparable accuracy and safety to larger models while being deployable on standard hardware.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that compact models can match frontier-grade models in code generation and agent-driven workflows through principled data and compute discipline, enabling deployment on standard hardware without specialized infrastructure.

Method: Built on Mify-2.5B foundation model, trained with compute-optimal strategy on 4.2T tokens using high-quality curated sources and synthetic data from agentically designed prompts, refined with enterprise-grade evaluation datasets and LLM-based quality filtering. Explored CPT-SFT objectives, data mixtures, and sampling dynamics in a single continuous training trajectory.

Result: Mify-Coder significantly outperforms much larger baseline models on standard coding and function-calling benchmarks while achieving comparable accuracy and safety. Quantized variants enable deployment on standard desktop environments without specialized hardware.

Conclusion: Principled data and compute discipline allow smaller models to achieve competitive accuracy, efficiency, and safety compliance, demonstrating that compact models can match frontier-grade models in code intelligence tasks.
...
### [199] [The Drill-Down and Fabricate Test (DDFT): A Protocol for Measuring Epistemic Robustness in Language Models](https://arxiv.org/abs/2512.23850)
*Rahul Baxi*
...
<details>
  <summary>Details</summary>
Motivation: Current language model evaluations (like MMLU, TruthfulQA) measure knowledge under ideal conditions but fail to assess how robustly models maintain factual accuracy when information degrades or when adversaries probe for weaknesses. There's a need to measure epistemic robustness before deployment in critical applications.

Method: Introduced the Drill-Down and Fabricate Test (DDFT) protocol that measures epistemic robustness through progressive semantic compression and adversarial fabrication. Proposed a two-system cognitive model: Semantic System (generates fluent text) and Epistemic Verifier (validates factual accuracy). Evaluated 9 frontier models across 8 knowledge domains at 5 compression levels (1,800 turn-level evaluations).
...
Conclusion: Epistemic robustness emerges from training methodology and verification mechanisms distinct from current approaches, challenging assumptions about model size-reliability relationship. DDFT provides both theoretical foundation and practical tools for assessing epistemic robustness before deployment in critical applications.
...
### [200] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
...
<details>
  <summary>Details</summary>
Motivation: Despite strong general language capabilities, LLMs struggle with spatial transformations and multi-step planning in structured environments, which limits applications in navigation and planning.

Method: Two-stage approach: 1) Supervised fine-tuning on elementary spatial transformations (rotation, translation, scaling) to build basic spatial physics understanding; 2) Freeze physics-aware model and train lightweight LoRA adapters within GRPO framework for multi-step planning in puzzle environments using closed-loop reinforcement learning. Dataset: synthesized ASCII-art dataset with corresponding ASCII-based RL environment.

Result: Method consistently outperforms baselines (generic backbone, physics-aware model, end-to-end RL models) in both Dynamic environments (explicit state updates) and Static environments (model must rely on internal state). Approach converges faster and exhibits more stable training compared to end-to-end RL from scratch. Attention pattern analysis shows fine-tuning induces meaningful improvements in spatial understanding.

Conclusion: Decomposing spatial reasoning into atomic building blocks and their composition is effective for improving LLM spatial capabilities. The two-stage approach with physics-aware fine-tuning followed by policy learning enables better multi-step planning and more stable training.
...
### [201] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs have limited context windows, restricting their ability to process arbitrarily long prompts. There's a need for inference-time scaling techniques that allow LLMs to handle much longer inputs while maintaining or improving quality.

Method: Propose Recursive Language Models (RLMs) - an inference strategy where LLMs treat long prompts as external environments and programmatically examine, decompose, and recursively call themselves over prompt snippets.

Result: RLMs successfully handle inputs up to two orders of magnitude beyond model context windows. They dramatically outperform base LLMs and common long-context scaffolds across four diverse long-context tasks, while having comparable or cheaper cost per query.

Conclusion: RLMs provide an effective inference-time scaling approach for processing arbitrarily long prompts, offering superior performance to existing methods with cost efficiency.
...
TL;DR: ALE is an end-to-end ecosystem for developing agentic LLMs, featuring ROLL for weight optimization, ROCK for environment management, and iFlow CLI for context engineering. They release ROME agent trained on 1M+ trajectories with novel IPA algorithm and Terminal Bench Pro benchmark.
...
Method: ALE infrastructure with three components: 1) ROLL for post-training weight optimization, 2) ROCK as sandbox environment manager for trajectory generation, and 3) iFlow CLI for efficient context engineering. Includes data composition protocols for synthesizing complex behaviors and novel Interaction-based Policy Alignment (IPA) algorithm that assigns credit over semantic interaction chunks rather than individual tokens.
...
Conclusion: ALE provides an effective foundational infrastructure for agent LLM development, with ROME proving the ecosystem's capabilities through strong benchmark performance, addressing the gap in open-source agent development tools.
...
TL;DR: Iterative deployment of LLMs fine-tuned on user-curated data from previous deployments leads to significant model property changes, improving planning skills and enabling emergent generalization to longer plans, with theoretical analysis showing this implements implicit reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: To understand how iterative deployment of LLMs with user-curated fine-tuning data changes model properties over time, particularly examining whether this process leads to emergent capabilities and generalization, and to analyze the theoretical implications of this deployment pattern.

Method: Test iterative deployment mechanism on various planning domains, where each new model is fine-tuned on data carefully curated by users from previous models' deployment. Provide theoretical analysis showing this process implements reinforcement learning in the outer-loop with an implicit reward function.

Result: Substantial improvements in planning skills observed, with later models displaying emergent generalization by discovering much longer plans than initial models. The iterative deployment effectively implements RL training with implicit rewards.

Conclusion: Iterative deployment creates an alternative training regime to explicit RL, relying on data curation rather than explicit rewards. This has important AI safety implications since the implicit reward function is not explicitly defined and could lead to unexpected properties in future model deployments.
...
### [204] [Learning to Feel the Future: DreamTacVLA for Contact-Rich Manipulation](https://arxiv.org/abs/2512.23864)
...
Method: Hierarchical perception with tactile images as micro-vision, wrist-camera local vision, and third-person macro vision. Uses Hierarchical Spatial Alignment (HSA) loss to align tactile tokens with spatial counterparts, and finetunes with a tactile world model predicting future tactile signals. Trained on hybrid dataset from digital twin and real-world experiments.

Result: Outperforms state-of-the-art VLA baselines across contact-rich manipulation tasks, achieving up to 95% success rate. Demonstrates importance of contact physics understanding for robust, touch-aware robotic agents.

Conclusion: DreamTacVLA successfully grounds VLA models in contact physics by learning to predict future tactile states, enabling superior performance in contact-rich manipulation through multi-scale sensory integration and future anticipation.
...
TL;DR: RANGER is a zero-shot semantic navigation framework that uses only monocular cameras, eliminating depth/pose dependencies while enabling in-context learning from short videos for improved efficiency.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot object navigation methods rely heavily on precise depth/pose information from simulators (limiting real-world use) and lack in-context learning capability to quickly adapt to new environments using short videos.

Method: Uses 3D foundation models with monocular camera only; integrates keyframe-based 3D reconstruction, semantic point cloud generation, VLM-driven exploration value estimation, adaptive waypoint selection, and low-level action execution.

Result: Achieves competitive performance on HM3D benchmark and real-world environments in navigation success rate and exploration efficiency, with superior in-context learning adaptability without requiring prior 3D mapping.

Conclusion: RANGER demonstrates effective zero-shot semantic navigation using only monocular vision, eliminating depth/pose dependencies while enabling efficient adaptation to new environments through in-context learning from short videos.
...
### [206] [Geometric Multi-Session Map Merging with Learned Local Descriptors](https://arxiv.org/abs/2512.24384)
*Yanlong Ma,Nakul S. Joshi,Christa S. Robison,Philip R. Osteen,Brett T. Lopez*

Main category: cs.RO

TL;DR: GMLD: A learning-based local descriptor framework for large-scale multi-session point cloud map merging using keypoint-aware encoder and plane-based geometric transformer for loop closure detection and pose estimation.
...
Method: Uses a keypoint-aware encoder and plane-based geometric transformer to extract discriminative features for loop closure detection and relative pose estimation. Also includes inter-session scan matching cost factors in factor-graph optimization for global consistency.

Result: Accurate and robust map merging with low error on public datasets and self-collected data from diverse environments. Learned features deliver strong performance in both loop closure detection and relative pose estimation.

Conclusion: GMLD framework effectively addresses multi-session point cloud map merging through learning-based local descriptors and geometric transformers, achieving accurate alignment and strong feature performance across diverse environments.
...
### [207] [Dream2Flow: Bridging Video Generation and Open-World Manipulation with 3D Object Flow](https://arxiv.org/abs/2512.24766)
*Karthik Dharmarajan,Wenlong Huang,Jiajun Wu,Li Fei-Fei,Ruohan Zhang*

Main category: cs.RO

TL;DR: Dream2Flow bridges video generation and robotic control by using 3D object flow as an intermediate representation, enabling zero-shot manipulation of diverse objects without task-specific demonstrations.


<details>
  <summary>Details</summary>
Motivation: While generative video models can reason about plausible physical interactions, there's a gap in translating these human-led motions into executable low-level robotic actions. The challenge is to leverage video generation capabilities for open-world robotic manipulation.

Method: Dream2Flow reconstructs 3D object motions from generated videos and formulates manipulation as object trajectory tracking. It separates state changes from actuators, uses trajectory optimization or reinforcement learning to convert 3D object flow into low-level commands, without requiring task-specific demonstrations.

Result: The framework successfully manipulates diverse object categories including rigid, articulated, deformable, and granular objects. It enables zero-shot guidance from pre-trained video models to robotic systems, overcoming the embodiment gap.

Conclusion: 3D object flow serves as a general and scalable interface for adapting video generation models to open-world robotic manipulation, bridging the gap between high-level reasoning and low-level control.
