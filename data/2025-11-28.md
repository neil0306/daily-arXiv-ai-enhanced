...
Method: Proposes retrofitting pretrained models without retraining, lightweight fine-tuning that preserves alignment, economical reasoning for long chains of thought, dynamic knowledge management without heavy RAG, and Overhead-Aware Efficiency benchmarking.

Result: A research framework that redefines efficiency to include adoption cost, sustainability and fairness rather than just performance metrics.

Conclusion: By focusing on robust simplicity and redefining efficiency to include broader societal factors, LLM deployment can be democratized to reduce inequality and carbon waste rather than amplifying them.
...
TL;DR: HTP is a reversible, deterministic framework that generates text embeddings without training by encoding tokens as harmonic trajectories from Unicode representations, achieving competitive semantic similarity performance with minimal computational cost.


<details>
  <summary>Details</summary>
Motivation: To create transparent and efficient text embeddings that don't rely on statistical co-occurrence or optimization, providing an interpretable alternative to neural embeddings.

Method: Analytically encodes each token as a harmonic trajectory derived from its Unicode integer representation, establishing a bijective mapping between discrete symbols and continuous vector space.

Result: Achieves Spearman correlation of ρ = 0.68 on STS-B benchmark, maintains stable performance across 10 languages with sub-millisecond latency per sentence pair.
...
TL;DR: A dual-embedding centroid-based framework for hierarchical text classification in ITSM systems that combines semantic and lexical representations through reciprocal rank fusion, achieving competitive performance with SVM while being significantly faster and more interpretable.


<details>
  <summary>Details</summary>
Motivation: ITSM systems require hierarchical classification of support tickets into tree-structured taxonomies, needing methods that balance performance, interpretability, and operational efficiency for production environments.

Method: Dual-embedding centroid-based classification with separate semantic and lexical centroid representations per category, combined through reciprocal rank fusion at inference time.

Result: Achieves hierarchical F1 of 0.731 (vs 0.727 for SVM) with 5.9x faster training, 152x faster incremental updates, and 8.6-8.8x speedup across batch sizes when excluding embedding computation.
...
TL;DR: PIRA is a training paradigm that improves reward models for LLM alignment by reformulating QA pairs into preference instructions, aggregating diverse preference tasks, and stabilizing rewards through dropout averaging.
...
Method: Three strategies: (1) Reformulating QA pairs into preference-based instructions, (2) aggregating rewards from diverse preference tasks, (3) averaging value-head outputs under varying dropout rates.

Result: Extensive experiments demonstrate the effectiveness of PIRA in addressing the limitations of traditional reward models.

Conclusion: PIRA provides an effective training paradigm that improves data efficiency and robustness of reward models for LLM alignment.
...
TL;DR: LLMs struggle with legal tasks due to lack of domain-specific training. This paper shows that organizing legal documents by rhetorical roles and explaining legal terminology significantly improves model performance in zero-shot legal judgment prediction.
...
Method: Conducted experiments in three areas: (i) reorganizing documents by rhetorical roles, (ii) defining rhetorical roles to familiarize models with legal terminology, and (iii) emulating court reasoning steps regarding rhetorical roles. Experiments were done in zero-shot setting across three Indian legal judgment prediction datasets.

Result: Organizing data or explaining key legal terms significantly boosted model performance, with improvements ranging from ~1.5% to 4.36% in F1 score compared to baseline.

Conclusion: Structuring legal information through rhetorical roles and providing domain-specific terminology explanations can effectively enhance LLM performance in legal tasks without requiring full domain-specific pretraining.
...
TL;DR: MindSET is a new benchmark dataset for mental health analysis from Reddit with 13M annotated posts across 7 conditions, featuring rigorous preprocessing and showing 18-point F1 improvement in classification tasks.
...
Method: Curated Reddit posts using self-reported diagnoses, applied rigorous preprocessing (language filtering, NSFW removal, duplicate removal), performed LIWC linguistic analysis, and conducted binary classification experiments with fine-tuned language models and BoW features.

Result: Models trained on MindSET consistently outperformed previous benchmarks, achieving up to 18-point F1 improvement for Autism detection. Dataset contains over 13M annotated posts across 7 mental health conditions.

Conclusion: MindSET provides a robust foundation for researchers exploring social media and mental health, supporting early risk detection and deeper analysis of emerging psychological trends.
...
TL;DR: FlexCode introduces a popularity-aware framework for generative recommendation that uses dual codebooks (collaborative filtering and semantic) with adaptive token allocation, outperforming uniform codebook approaches.


<details>
  <summary>Details</summary>
Motivation: Existing generative recommendation approaches use a single uniform codebook, failing to address the imbalance between popular items with rich collaborative signals and long-tail items requiring semantic understanding.

Method: Uses dual codebooks (CF and semantic) with adaptive token allocation via lightweight MoE, plus alignment and smoothness objectives to maintain coherence across popularity spectrum.

Result: Consistently outperforms strong baselines on both public and industrial-scale datasets, achieving stronger accuracy and tail robustness.

Conclusion: FlexCode provides a new mechanism for token representation in generative recommenders, offering a better balance between memorization and generalization.
...
TL;DR: First Arabic cross-domain, context-dependent text-to-SQL dataset (Ar-SParC) with 10,225 questions and corresponding SQL queries, plus a novel GAT corrector method that improves performance.


<details>
  <summary>Details</summary>
Motivation: Address the lack of Arabic language resources for cross-domain, context-dependent text-to-SQL tasks, as most research has focused on English and Chinese.

Method: Created Ar-SParC dataset with 3,450 question sequences; tested GPT-3.5-turbo and GPT-4.5-turbo with 10 prompt engineering techniques; developed GAT corrector approach.

Result: GAT corrector improved performance across all 40 experiments: +1.9% EX and +1.9% IX (zero-shot), +1.72% EX and +0.92% IX (in-context learning).
...
TL;DR: Large language models like GPT-4 can make reasoning errors in oncology decision support that lead to clinically unsafe recommendations, despite high benchmark performance.


<details>
  <summary>Details</summary>
Motivation: To identify and classify reasoning errors in LLMs for oncology decision support, as these errors can lead to harmful clinical recommendations that aren't captured by standard accuracy metrics.

Method: Developed a hierarchical taxonomy of reasoning errors from GPT-4 responses to real oncology notes, validated on prostate cancer consult notes across disease stages, simulating clinical tasks.

Result: Reasoning errors occurred in 23% of interpretations and were associated with guideline-discordant recommendations, particularly in advanced disease. Automated evaluators could detect errors but not classify subtypes reliably.

Conclusion: LLMs may provide fluent but clinically unsafe recommendations when reasoning is flawed. The taxonomy provides a framework for evaluating reasoning fidelity before clinical deployment.
...
TL;DR: Dynamic Template Selection (DTS) adapts response templates to query complexity, reducing token costs by 32-34% across major LLM providers while maintaining response quality.


<details>
  <summary>Details</summary>
Motivation: Current uniform prompting strategies waste tokens by applying verbose responses to simple queries, which is costly due to output tokens being 4-8x more expensive than input tokens.

Method: Two routing approaches: simple MLP using pre-computed embeddings and fine-tuned RoBERTa transformer, evaluated on 1,000 MMLU questions with cross-provider validation.

Result: MLP router achieved 90.5% routing accuracy (better than RoBERTa's 89.5%) with 125M fewer parameters, reducing tokens by 32.6-33.9% across OpenAI GPT-4, Google Gemini, and Anthropic Claude.

Conclusion: DTS provides provider-agnostic cost savings through adaptive template selection, with formal problem formulation, algorithms, and empirical validation across production systems.
...
TL;DR: 2D materials have unique properties for energy applications, but research information is scattered across papers, making systematic analysis difficult.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of dispersed information about 2D materials' properties and preparation methods across research papers, which hinders systematic analysis and application development.

Method: The paper likely employs text mining, natural language processing, or database compilation techniques to extract and organize information about 2D materials from published research literature.

Result: The abstract suggests the creation of a systematic framework or database that consolidates scattered information about 2D materials' properties and synthesis methods from research papers.

Conclusion: Organizing dispersed research information about 2D materials enables better understanding and accelerates their application in energy storage and conversion technologies.
...
TL;DR: Introduces multi-prefix memorization framework to better detect verbatim data memorization in LLMs by measuring retrieval robustness through diverse prefix paths.


<details>
  <summary>Details</summary>
Motivation: Existing memorization definitions inadequately capture data leakage in aligned LLMs, creating privacy and copyright risks from verbatim memorization of training data.

Method: Defines memorization based on whether external adversarial search can find a target number of distinct prefixes that elicit the same sequence, measuring memory robustness through prefix diversity.

Result: The multi-prefix definition reliably distinguishes memorized from non-memorized data in experiments with open-source and aligned chat models.

Conclusion: Provides a robust and practical framework for auditing data leakage in LLMs by shifting focus from single-path extraction to quantifying memory robustness through diverse retrieval paths.
...
TL;DR: SAGE is an agent-based framework that improves feature interpretation in sparse autoencoders (SAEs) for LLMs by using an active, iterative process of explanation formulation, testing, and refinement.


<details>
  <summary>Details</summary>
Motivation: LLM internal mechanisms are opaque, posing safety and reliability challenges. While SAEs help decompose representations into interpretable features, explaining these features remains difficult.

Method: SAGE recasts feature interpretation as an active process: systematically formulates multiple explanations per feature, designs targeted experiments to test them, and iteratively refines explanations based on activation feedback.
...
Conclusion: SAGE provides a rigorous methodology for feature interpretation that outperforms existing approaches, advancing the interpretability of LLM representations.
...
TL;DR: The paper introduces DSPy+HELM framework that uses structured prompting to more accurately estimate language model performance ceilings, revealing that traditional HELM benchmarks underestimate LM capabilities and misrepresent rankings.


<details>
  <summary>Details</summary>
Motivation: Current benchmarking frameworks like HELM use fixed prompts that fail to generalize across LMs, leading to unrepresentative performance estimates and potentially underestimating model capabilities.

Method: Developed a reproducible DSPy+HELM framework with four structured prompting methods that elicit reasoning, evaluated across four frontier LMs and seven benchmarks against existing HELM baselines.

Result: Without structured prompting: HELM underestimates LM performance by 4% on average, increases performance variance, misrepresents leaderboard rankings (flipping on 3/7 benchmarks), and reasoning reduces LM sensitivity to prompt design.

Conclusion: Scalable performance ceiling estimation through structured prompting enables more accurate and decision-useful LM benchmarking, with the framework being open-sourced for community use.
...
TL;DR: Length-MAX tokenizer reduces tokens per character by 14-18% vs BPE, enabling faster training (18.5% fewer steps) and inference (13.7% lower latency) while improving downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Current tokenizers like BPE don't optimize for token length, leading to inefficiency in text representation during training and inference. The goal is to minimize average tokens per character to improve efficiency.
...
Result: 14-18% fewer tokens than BPE across vocab sizes; 18.5% fewer training steps; 13.7% lower inference latency; 16% throughput gain; 11.7% lower LAMBADA perplexity; 4.3% higher HellaSwag accuracy; 99.62% vocabulary coverage.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM memory evaluations focus on static conversational settings, overlooking dynamic memory accumulation and reuse in real-world environments where LLMs handle continuous task streams and need to learn from accumulated interactions.
...
Result: The framework evaluates memory modules across 10 diverse multi-turn goal-oriented and single-turn reasoning datasets, providing baseline method ExpRAG for experience retrieval and utilization.

Conclusion: Evo-Memory bridges the gap in evaluating self-evolving memory capabilities in LLM agents, enabling continuous improvement through dynamic memory management in real-world deployment scenarios.
...
TL;DR: Cross-lingual argument mining approach for low-resource languages using English-Persian parallel data, comparing zero-shot transfer, LLM augmentation, and cross-lingual training.
...
Method: Three training scenarios: (1) zero-shot transfer from English to Persian, (2) English training enhanced with LLM-generated synthetic examples, (3) cross-lingual model combining English and manually translated Persian data.
...
<details>
  <summary>Details</summary>
Motivation: To characterize how large language models internally ground abstract semantic structure, as their internal mechanisms remain insufficiently understood despite semantic competence.
...
TL;DR: Chatty-KG is a modular multi-agent system for conversational QA over knowledge graphs that combines RAG-style retrieval with structured SPARQL query execution through specialized LLM agents, achieving state-of-the-art performance in both single-turn and multi-turn settings.
...
Method: Uses a modular multi-agent system with task-specialized LLM agents for contextual interpretation, dialogue tracking, entity/relation linking, and query planning to generate executable SPARQL queries from natural language questions.

Result: Significantly outperforms state-of-the-art baselines in both single-turn and multi-turn settings with higher F1 and P@1 scores, works with various LLMs (commercial and open-weight), and maintains dialogue coherence without fine-tuning.

Conclusion: Chatty-KG successfully unifies conversational flexibility with structured KG grounding, providing a scalable and extensible approach for reliable multi-turn KGQA that preserves dialogue coherence and supports evolving KGs.
...
TL;DR: LLMs perform best on definition-type queries but struggle with exemplification tasks, with performance influenced by concept frequency and technicality.


<details>
  <summary>Details</summary>
Motivation: To evaluate LLMs' performance drop on non-definition queries like examples and paraphrases, and investigate how pre-training data affects their ability to handle diverse linguistic queries.

Method: Used TrackList analysis pipeline and RefoMed-EN dataset (6170 medical terms with human annotations) to study LLM performance on definition, denomination, exemplification, explanation, and paraphrase queries, evaluating with syntactic/semantic similarity metrics, correlations, and embeddings.

Result: LLMs showed highest performance on definition-type questions and lowest on exemplification. For definitions, they paraphrase more on popular/frequent knowledge and less on tail/technical knowledge, especially in expert texts.

Conclusion: LLMs have significant performance gaps between definition and other query types, with exemplification being particularly challenging, and their performance is affected by concept frequency and domain technicality.
...
TL;DR: In-context learning cannot override pre-trained label semantics; it primarily refines existing semantic directions rather than remapping label meanings.
...
Result: Models cannot learn coherent anti-semantic classifiers; semantic override rates remain zero, showing ICL primarily adjusts how inputs project onto stable semantic directions from pre-training.

Conclusion: ICL cannot override label semantics at current scales; overriding requires interventions beyond few-shot prompting.
...
TL;DR: The paper introduces a retrieval-aware approach for sarcasm detection that combines web-based retrieval and self-knowledge awareness to provide contextual information, improving performance over the baseline PMP method across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Sarcasm detection remains challenging for PLMs and LLMs due to linguistic diversity, cultural variation, and unreliable detection of words requiring extra grounding. Current models struggle with culturally specific slang and unknown terms.

Method: Builds on Pragmatic Metacognitive Prompting (PMP) by adding two complementary context strategies: non-parametric knowledge using web-based retrieval when background is lacking, and self-knowledge awareness to elicit the model's internal knowledge.
...
Conclusion: Context is crucial for enhancing LLM performance in sarcasm detection, especially for culturally specific slang and unknown terms. Future work will optimize retrieval quality and examine its impact on performance.
...
TL;DR: KANs (Kolmogorov-Arnold Networks) outperform or match MLPs as classification heads for low-resource Burmese language tasks, with EfficientKAN achieving the highest F1-score (0.928) using fastText embeddings.


<details>
  <summary>Details</summary>
Motivation: In low-resource languages like Burmese, existing approaches often freeze pre-trained encoder weights and only fine-tune the final classification layer using MLPs, but MLPs have fixed non-linearity that limits expressiveness and increases computational cost.

Method: Evaluated three KAN variants (FourierKAN, EfficientKAN, FasterKAN) as alternative classification heads across diverse embeddings including TF-IDF, fastText, and multilingual transformers (mBERT, Distil-mBERT) for Burmese classification tasks.
...
Conclusion: KANs serve as expressive, efficient alternatives to MLPs for low-resource language classification, demonstrating competitive performance across different embedding types.
...
TL;DR: Cross-architecture evaluation of 28 LLM configurations on word puzzles reveals architectural differences matter more than parameter scaling for character-level constraint satisfaction, with systematic failures on orthographically atypical words.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate how different LLM architectures handle hard orthographic constraints during controlled text generation, as current evaluations remain limited.
...
Result: Architectural differences produced 2.0-2.2x performance gaps (F1=0.761 vs. 0.343) vs. 83% gain from parameter scaling. Models show heterogeneous thinking budget sensitivity and systematic failures on orthographically atypical words like "data", "poop", "loll" (86-95% human success, 89-96% model miss rate).
...
TL;DR: First study on ASR error correction for Burmese using Transformers with IPA and alignment features, achieving significant WER reduction from 51.56 to 39.82.
...
Result: AEC model combining IPA and alignment features reduced average WER from 51.56 to 39.82 (before augmentation) and improved chrF++ scores from 0.5864 to 0.627, showing consistent gains over baseline ASR outputs.

Conclusion: AEC approaches are robust and effective for improving ASR outputs in low-resource settings, with feature design playing a crucial role.
...
TL;DR: MortgageLLM is a domain-specific LLM for mortgage finance that uses a dual-track specialization framework to create two expert models - one for conversational Q&A and another for structured tasks - overcoming performance trade-offs of single multi-task models.
...
Method: Dual-track specialization from LLaMA-3.1-8B base model creating two specialists: conversational Q&A model and structured task model for classification/summarization, using instruction residual technique to restore instruction-following capabilities post-domain adaptation without supervised fine-tuning.
...
Conclusion: The dual-expert architecture with intelligent task routing effectively addresses domain specialization while preserving instruction-following capabilities, demonstrating superior performance in mortgage finance applications.
...
TL;DR: SGASA framework enhances reasoning models' safety against adversarial jailbreak prompts by internalizing model-generated safety guidelines through data synthesis and alignment fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Reasoning models are vulnerable to adversarial jailbreak prompts that bypass safety mechanisms, creating need for adaptive safety alignment to autonomously reinforce defenses against harmful content generation.
...
Conclusion: SGASA provides an adaptive and scalable approach to enhance reasoning models' robustness against harmful adversarial prompts through internalized safety guidelines.
...
TL;DR: Fine-tuning LLMs on small human survey data improves response heterogeneity and alignment with human behavior, but still fails to reproduce regression coefficients, making LLM-generated data unsuitable for replacing human participants in inferential analyses.


<details>
  <summary>Details</summary>
Motivation: To investigate whether fine-tuning LLMs on small human survey data can mitigate issues like limited diversity, systematic misalignment for minority subgroups, and belief-action discrepancies that plague LLM-based simulation of human participants.

Method: Used a behavioral experiment on information disclosure to compare human and LLM-generated responses across multiple dimensions including distributional divergence, subgroup alignment, belief-action coherence, and recovery of regression coefficients.

Result: Fine-tuning on small human samples substantially improved heterogeneity, alignment, and belief-action coherence compared to the base model, but even the best fine-tuned models failed to reproduce the original study's regression coefficients.
...
TL;DR: Development of the first open conversational speech dataset for Isan language, featuring natural speech with authentic linguistic phenomena like colloquials, disfluencies, and code-switching with Thai.


<details>
  <summary>Details</summary>
Motivation: To create an inclusive AI resource for underrepresented Isan language, addressing the lack of conversational speech datasets and standardized orthography challenges.

Method: Established practical transcription protocols balancing representational accuracy with computational processing requirements, collecting natural speech data with authentic linguistic features.

Result: Created the first open conversational speech dataset for Isan language, capturing natural speech phenomena including colloquials, spontaneous prosody, disfluencies, and code-switching.

Conclusion: The dataset contributes to inclusive AI development, supports research on underrepresented languages, and addresses linguistic and technical challenges in modeling conversational speech.
...
TL;DR: PEFT-Bench is a unified benchmark for evaluating parameter-efficient fine-tuning methods on LLMs across 27 datasets and 6 PEFT methods, introducing the PSCP metric to account for training and inference factors.


<details>
  <summary>Details</summary>
Motivation: Current PEFT method evaluations are limited in scope and difficult to reproduce, creating a gap in comprehensive benchmarking despite the growing importance of parameter-efficient fine-tuning for reducing LLM computational costs.

Method: Developed PEFT-Bench as an end-to-end benchmark framework that evaluates diverse PEFT methods on autoregressive LLMs across multiple NLP datasets, and introduced the PSCP metric that considers trainable parameters, inference speed, and training memory usage.

Result: The benchmark demonstrates usage across 27 NLP datasets and 6 different PEFT methods, providing a comprehensive evaluation framework for parameter-efficient fine-tuning approaches.

Conclusion: PEFT-Bench addresses the reproducibility and scope limitations of current PEFT evaluations, offering a unified framework with the PSCP metric to better assess the practical trade-offs of different parameter-efficient fine-tuning methods.
...
Result: Martin's Law emerges around checkpoint 100, peaks at checkpoint 104 (r > 0.6), then degrades by checkpoint 105. Smaller models experience catastrophic semantic collapse, larger models show graceful degradation. Frequency-specificity trade-off remains stable (r ≈ -0.3).

Conclusion: Compliance with linguistic regularities in LLMs is not monotonically increasing with training but follows a balanced trajectory with an optimal semantic window, establishing new methodology for evaluating emergent linguistic structure.
...
TL;DR: Fine-tuning enables reliable detection of injected activation patterns in language models, transforming from near-complete failure to 85% accuracy with 0% false positives.
...
Method: Fine-tuning a 7B parameter model on transient single-token injections to detect and identify injected activation patterns.

Result: Model transformed from 0.4% accuracy to 85% accuracy on held-out concepts with 0% false positives, satisfying three of Lindsey's criteria: accuracy, grounding, and internality.
...
TL;DR: This paper creates a new dataset for fine-grained evidence extraction in Czech and Slovak claims and evaluates LLMs' performance, finding that models often fail to copy evidence verbatim and that model size doesn't always correlate with better performance.


<details>
  <summary>Details</summary>
Motivation: Misinformation spreads in online news comments, requiring methods to detect incorrect information by identifying supporting or refuting evidence from relevant documents.
...
Result: LLMs often fail to copy evidence verbatim from source text, leading to invalid outputs. Llama3.1:8b performed well despite small size, while gpt-oss-120b underperformed despite large parameters. Qwen3:14b, deepseek-r1:32b, and gpt-oss:20b showed effective balance between size and human alignment.

Conclusion: Model size doesn't guarantee better performance in fine-grained evidence extraction; smaller models can outperform larger ones, and verbatim evidence copying remains a challenge for LLMs in this task.
...
TL;DR: DSR-SQL introduces a dual-state reasoning framework for Text-to-SQL that improves reasoning coherence on complex enterprise databases through adaptive context state and progressive generation state interaction.
...
Method: Models Text-to-SQL as interaction between adaptive context state (refining schemas and selecting relevant structures) and progressive generation state (feedback-guided SQL synthesis with self-correction).
...
Conclusion: DSR-SQL provides a competitive framework for Text-to-SQL that maintains coherent reasoning on complex databases through dual-state interaction.
...
TL;DR: Odin is a new architecture that integrates graph structure into Transformers at specific layers through oriented dual-modules, avoiding multi-hop diffusion and over-smoothing while achieving state-of-the-art performance on text-rich graphs.
...
TL;DR: Model merging for LLMs: Task Arithmetic is the only reliable method among six state-of-the-art approaches, while others cause performance drops, indicating current techniques don't transfer well to modern LLMs.


<details>
  <summary>Details</summary>
Motivation: To determine if model merging advantages reported for smaller models generalize to LLMs, and to systematically evaluate merging methods' effectiveness on large language models.

Method: Large-scale evaluation of six merging methods across four LLMs, twelve fine-tuned checkpoints per base model, and sixteen standard benchmarks, measuring performance gains over base models and best individual checkpoints.

Result: Task Arithmetic (oldest/simplest method) is the only approach that reliably yields performance gains on LLMs. Other interference-aware and subspace methods typically cause significant performance drops.

Conclusion: Current merging techniques don't directly transfer to modern LLMs, motivating the need for LLM-specific merging algorithms and merging-aware fine-tuning methods.
...
<details>
  <summary>Details</summary>
Motivation: Most deep learning approaches for readability assessment fail to consider text length or the ordinal relationship between readability labels.

Method: Uses bidirectional contextual information to identify semantic-rich regions for sentence-level readability prediction, then aggregates to document level. Introduces pairwise sorting algorithm to model ordinal relationships through label subtraction.
...
Conclusion: The proposed bidirectional readability assessment mechanism effectively handles text length and ordinal label relationships, demonstrating superior performance across languages.
...
TL;DR: Speech translation models use acoustic cues and language patterns to assign gender to speaker-referring terms, with models learning broader masculine prevalence patterns rather than simple term associations, and using first-person pronouns to link gender information across frequency spectrum.


<details>
  <summary>Details</summary>
Motivation: Speech conveys speaker information like gender through acoustic cues, creating modality-specific bias concerns in speech translation where vocal characteristics may influence gender assignment in target languages with grammatical gender, risking misgendering.

Method: Investigated gender assignment mechanisms in ST models across three language pairs (en-es/fr/it), examining training data patterns, internal language model biases, acoustic information interaction, and used contrastive feature attribution on spectrograms.

Result: Models learn broader patterns of masculine prevalence rather than replicating term-specific gender associations; ILM exhibits strong masculine bias but models can override based on acoustic input; higher accuracy models use first-person pronouns to link gendered terms to speaker, accessing gender information distributed across frequency spectrum.

Conclusion: ST models employ complex mechanisms for gender assignment that combine acoustic and linguistic information, with successful models using novel strategies like first-person pronoun linking to access distributed gender cues rather than relying solely on pitch information.
...
TL;DR: Created IsharaKhobor dataset for Bangla Sign Language Translation to address low-resource limitations, with two subsets for vocabulary optimization.


<details>
  <summary>Details</summary>
Motivation: Bangla Sign Language Translation is severely constrained due to being very low resource, requiring standard sentence-level datasets for developing AI assistive tools for deaf and hard of hearing Bangla speakers.

Method: Developed IsharaKhobor dataset with two subsets (IsharaKhobor_small and IsharaKhobor_canonical_small) through vocabulary restriction and canonicalization, benchmarked with landmark-based raw and RQE embeddings.

Result: Created publicly available dataset on Kaggle that enables research in Bangla Sign Language Translation and addresses vocabulary optimization challenges.

Conclusion: The IsharaKhobor dataset provides a foundational resource for advancing Bangla Sign Language Translation research and developing AI-based assistive technologies for the deaf community.
...
TL;DR: RoParQ benchmark evaluates LLM consistency on paraphrased questions, with XParaCon metric measuring robustness. A paraphrase-aware SFT method improves model consistency, making lightweight models perform comparably to larger models.
...
Method: Created RoParQ benchmark from standard datasets using paraphrases from proprietary models, developed XParaCon metric, and implemented reasoning-based paraphrase-aware SFT.

Result: Targeted alignment significantly improved robustness, with fine-tuned lightweight models achieving consistency levels comparable to larger pre-trained models.

Conclusion: The approach effectively mitigates superficial memorization and fosters more robust, reliable LLMs.
...
TL;DR: A lightweight method to identify skill-specific neurons in LLMs using neuron activation correlations with auxiliary metrics, revealing interpretable behaviors and shortcuts in complex tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs have impressive capabilities but their internal mechanisms are largely opaque, creating a need for interpretability methods that can isolate neurons encoding specific skills.

Method: Extends prior skill neuron identification by correlating neuron activations with auxiliary metrics (external labels, model confidence) without manual token aggregation, applied to complex multi-skill scenarios.

Result: Successfully detected neurons driving known skills and revealed previously unidentified shortcuts in arithmetic reasoning on BigBench, validated on open-ended text generation and natural language inference tasks.

Conclusion: The method provides a simple yet effective approach for interpretable neuron analysis in LLMs, uncovering both expected skill encodings and unexpected behavioral shortcuts.
...
TL;DR: Metadata beyond URLs can accelerate LLM pretraining, with fine-grained quality indicators being particularly effective. Metadata appending and learnable meta-tokens also improve training efficiency.


<details>
  <summary>Details</summary>
Motivation: Prior work only explored URLs as useful metadata, leaving open whether other metadata types could provide greater benefits for accelerating LLM pretraining.
...
Result: Found that fine-grained document quality indicators and other metadata types accelerate pretraining. Metadata appending and learnable meta-tokens also improve training efficiency.

Conclusion: Provides practical guidelines for integrating diverse metadata types to enhance both efficiency and effectiveness of LLM pretraining, with fine-grained information encoding being a key success factor.
...
TL;DR: Czech native speakers cannot reliably distinguish AI-generated from human-written poetry, with only 45.8% correct identification. AI poems were rated equally or better than human ones, but participants showed bias against poems they believed were AI-generated.


<details>
  <summary>Details</summary>
Motivation: To examine perception of AI-generated poetry in Czech, a morphologically complex Slavic language with limited training data, and test if native speakers can distinguish it from human poetry.

Method: Conducted study with Czech native speakers who guessed authorship and provided aesthetic evaluations of AI- and human-written poems. Used logistic regression to analyze factors affecting recognition accuracy.

Result: Participants performed at chance level (45.8%) in identifying authorship. AI poems were rated equally or more favorably than human ones, but belief about authorship strongly influenced ratings - poems believed to be AI-generated were rated lower.

Conclusion: AI can convincingly produce poetry even in complex, low-resource languages like Czech. Readers' authorship beliefs and aesthetic evaluations are interconnected, with liking a poem making authorship identification less accurate.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-agent synthesis frameworks suffer from scalability bottlenecks due to centralized orchestrators and lack flexibility due to hardcoded domain-specific designs, limiting their effectiveness for large-scale synthetic data generation.
...
Conclusion: Matrix provides a scalable, flexible framework for multi-agent synthetic data generation that significantly outperforms existing approaches in throughput while maintaining output quality, enabling efficient large-scale data synthesis.
...
TL;DR: ToolOrchestra trains small orchestrator models to coordinate multiple AI tools, achieving better performance than larger models like GPT-5 while being more cost-efficient.


<details>
  <summary>Details</summary>
Motivation: Large language models are powerful but computationally expensive for complex problems like Humanity's Last Exam. There's a need for more efficient approaches that can coordinate multiple specialized tools effectively.

Method: ToolOrchestra uses reinforcement learning with rewards based on outcomes, efficiency, and user preferences to train small orchestrator models that manage other models and tools.

Result: Orchestrator (8B model) achieves 37.1% on HLE, outperforming GPT-5 (35.1%) while being 2.5x more efficient. It also surpasses GPT-5 on tau2-Bench and FRAMES using only 30% of the cost.
...
TL;DR: LLMs show limited generalization across task difficulties - training on easy or hard data alone doesn't consistently improve performance across all difficulty levels, highlighting the need for diverse difficulty ranges in training and evaluation.
...
Method: Systematic evaluation using Item Response Theory (IRT) to rank examples in six datasets based on outputs from thousands of different LLMs, creating difficulty ratings determined solely by LLM abilities without human bias.
...
Method: A systematic neuroscience-inspired topological regularization (tau) framework applied to three VLMs (BLIP, PaliGemma 2, ViT-GPT2) across three datasets (COCO, CC3M, NoCaps), comparing baseline vs. neuro VLMs with tau > 0 configuration.
...
TL;DR: Inferix is a next-generation inference engine designed for world model simulation, featuring optimized semi-autoregressive decoding, interactive video streaming, and integration with LV-Bench for evaluation.
...
Method: Uses semi-autoregressive (block-diffusion) decoding paradigm that combines diffusion and autoregressive methods, generating video tokens in blocks with diffusion within each block while conditioning on previous ones. Features LLM-style KV Cache management for efficient variable-length generation.

Result: Enables coherent and stable video sequences with efficient, variable-length, and high-quality generation. Supports interactive video streaming and realistic simulation of world dynamics.

Conclusion: Inferix provides a dedicated platform for world model exploration with optimized inference capabilities, distinct from high-concurrency systems and classic video diffusion models, encouraging community collaboration to advance world simulation technology.
...
Conclusion: LTED-Ada effectively addresses the edge detection vs local tracking decision problem, providing an adaptive solution that balances accuracy and efficiency for video object recognition on resource-constrained devices.
...
TL;DR: DeeAD is a training-free early-exit framework that accelerates Vision-Language Action models for autonomous driving by terminating inference when trajectories align with lightweight planning priors within tolerable deviation.


<details>
  <summary>Details</summary>
Motivation: VLA models suffer from significant inference latency due to deep transformer stacks, creating a need for acceleration methods that preserve planning quality and safety.
...
Conclusion: DeeAD provides an effective training-free acceleration framework for VLA planning models that balances efficiency with performance preservation.
...
<details>
  <summary>Details</summary>
Motivation: Large foundation models are computationally expensive for edge devices, and existing compression methods sacrifice the crucial generality that makes foundation models valuable.
...
Conclusion: FMD enables practical deployment of foundation models on resource-constrained hardware while preserving their general-purpose representational power.
...
TL;DR: DinoLizer is a DINOv2-based model for detecting manipulated regions in generative inpainting, using patch embeddings and sliding-window strategy to achieve state-of-the-art localization performance.


<details>
  <summary>Details</summary>
Motivation: To develop an effective method for localizing manipulated regions in generative inpainting that can handle various post-processing operations and outperform existing manipulation detectors.

Method: Uses DINOv2 pretrained on B-Free dataset, adds linear classification head on patch embeddings to predict manipulations at 14×14 resolution, employs sliding-window strategy for larger images, and post-processes heatmaps to create binary masks.
...
Conclusion: DinoLizer demonstrates the strong representational power of Vision Transformers for manipulation localization and provides state-of-the-art performance in detecting generative inpainting manipulations.
...
TL;DR: CANVAS is a new benchmark for evaluating vision language models' ability to perform tool-based UI design tasks using design software like Figma or Sketch.


<details>
  <summary>Details</summary>
Motivation: There is currently no benchmark to evaluate VLMs' capacity to operate design software and iteratively refine UI designs, which is important for understanding their potential to collaborate with human designers.
...
Conclusion: CANVAS enables systematic evaluation of VLMs' tool-based UI design capabilities, revealing current performance levels and providing guidance for enhancing these collaborative design abilities.
...
TL;DR: TIE is a text-guided semantic image encoder that generates image representations conditioned on text queries, improving VLM performance and efficiency across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard image encoders in VLMs process images agnostically without considering downstream tasks or text queries, limiting their effectiveness for query-specific tasks.

Method: Proposed Text-Guided Semantic Image Encoder (TIE) that generates image representations conditioned on input text queries, enabling query-aware image processing.

Result: TIE-based VLMs outperform conventional counterparts by +1.5 and +1.3 points on average across nine benchmarks at 1B and 3B scales, with up to 6-point gains on DocVQA and InfoVQA, while using only half the image tokens for improved efficiency.

Conclusion: Text-conditioned training effectively optimizes encoders to capture key visual features, enhances interpretability through query-relevant attention, and generalizes well with generic queries.
...
TL;DR: SMARC is a unified model that reconstructs full RGB surfaces and classifies material categories from only a single 10% contiguous image patch, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Most existing methods require dense or full-scene observations, limiting their effectiveness in constrained or partial view environments where only sparse visual cues are available.
...
Conclusion: Partial convolution provides advantages in spatial reasoning under missing data, establishing a strong foundation for minimal-vision surface understanding applications.
...
TL;DR: LongVT is an agentic framework for long video reasoning that uses multimodal chain-of-tool-thought to address hallucination issues in large multimodal models by enabling global-to-local video analysis.
...
Method: Uses LMMs' temporal grounding ability as a native video cropping tool to zoom in on specific clips and resample finer-grained frames, creating a global-to-local reasoning loop until answers are visually grounded. Includes a three-stage training strategy with tool-integrated supervised fine-tuning and agentic reinforcement learning.

Result: Outperforms existing baselines across four challenging long-video understanding benchmarks. Created VideoSIAH dataset with 247.9K training samples and 1,280 QA evaluation pairs.

Conclusion: LongVT provides an effective end-to-end framework for reducing hallucinations in long video reasoning through interleaved multimodal chain-of-tool-thought, with publicly available code, data, and models.
...
TL;DR: Lightweight reproduction of KRISP that achieves 75% of original performance with significantly fewer parameters, enabling deployment on edge devices while preventing AI hallucinations through constrained knowledge domain.


<details>
  <summary>Details</summary>
Motivation: The original KRISP model is computationally demanding and tightly coupled with large backbone models, making it unsuitable for resource-constrained environments. This work aims to create a more accessible version that maintains core functionality while being deployable on edge devices.

Method: Reexamines KRISP architecture and creates a lightweight reproduction with fewer parameters. Uses systematic ablation studies, proof-of-concept on synthetic VQA data, and evaluation on DAQUAR dataset. Constrains model to external knowledge graph domain to prevent hallucinations.

Result: The replicated model achieves about 75% of the original KRISP's performance while uncovering design flaws and implicit problems not covered in the original paper. The lightweight model can function on edge devices like smartphones and AR-VR.

Conclusion: Knowledge-enhanced VQA architectures can be effectively scaled down for resource-constrained environments while maintaining reasonable performance. The constrained knowledge domain approach successfully prevents AI hallucinations and enables offline visual reasoning on edge devices.
...
TL;DR: The paper develops a unified theoretical framework called 'warping' that generalizes dynamic sampling methods in deep learning, analyzes their statistical properties, and reveals unique training asymmetries.


<details>
  <summary>Details</summary>
Motivation: To unify the theoretical analysis of various dynamic sampling methods in computer vision that currently lack a common analytical framework.

Method: Developed a novel 'warping' operator that generalizes existing dynamic sampling methods, performed statistical analysis using IID variables and homogeneous random fields, and introduced gradient-based loss landscape visualization.

Result: Discovered asymmetry between forward and backward passes, identified warping as orthogonal to traditional convolution operators, established conditions for stable training, and analyzed discretization effects.

Conclusion: The warping framework provides a unified theoretical foundation for analyzing dynamic sampling methods and reveals fundamental differences from traditional convolution operators, with practical implications for stable training.
...
TL;DR: Δ-NeRF is a modular residual framework for incremental NeRF refinement that enables efficient updates without catastrophic forgetting, using residual controllers, uncertainty-aware gating, and view selection to reduce training data and time.


<details>
  <summary>Details</summary>
Motivation: Existing NeRF frameworks require complete retraining for new views, limiting applicability in sequential data scenarios like satellite terrain analysis where regions are observed repeatedly over time.

Method: Proposes Δ-NeRF with: (1) residual controller injecting per-layer corrections into frozen base NeRF, (2) uncertainty-aware gating to prevent overcorrection, (3) view selection strategy reducing training data by 47%, and (4) knowledge distillation for model compression.
...
Conclusion: Δ-NeRF provides an effective solution for incremental NeRF refinement, addressing catastrophic forgetting and enabling efficient updates in sequential data scenarios like satellite imagery analysis.
...
Conclusion: StM effectively learns complex compositional dynamics for realistic video generation through self-supervised learning from unlabeled video data, demonstrating superior performance over existing approaches.
...
TL;DR: Sphinx is a synthetic environment for visual perception and reasoning that generates puzzles with verifiable solutions, covering 25 cognitive tasks. Current state-of-the-art models perform poorly (51.1% accuracy), but reinforcement learning with verifiable rewards significantly improves performance.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive benchmark for evaluating visual perception and reasoning capabilities, targeting core cognitive primitives that current models struggle with.

Method: Procedurally generates puzzles using motifs, tiles, charts, icons, and geometric primitives with verifiable ground-truth solutions. Uses reinforcement learning with verifiable rewards (RLVR) to train models.

Result: State-of-the-art GPT-5 achieves only 51.1% accuracy, well below human performance. RLVR substantially improves model accuracy on Sphinx tasks and yields gains on external visual reasoning benchmarks.

Conclusion: Sphinx provides a challenging benchmark for visual reasoning, and RLVR shows promise for advancing multimodal reasoning capabilities in AI models.
...
TL;DR: OVI replaces trained diffusion priors with optimization-based visual inversion, using constraints to improve image quality while exposing flaws in current evaluation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional diffusion priors are computationally expensive and require massive datasets, prompting exploration of training-free alternatives.
...
Result: OVI serves as viable prior alternative, with Nearest-Neighbor approach achieving scores comparable to state-of-the-art data-efficient priors while exposing benchmark flaws.

Conclusion: OVI demonstrates that trained priors may not be necessary, offering training-free alternative that merits further investigation despite current benchmark limitations.
...
<details>
  <summary>Details</summary>
Motivation: Accurate detection of tubular tree centerlines with correct topology is critical for clinical applications where missing small branches can lead to fatal mistakes in diagnosis and treatment planning.

Method: Uses Producer-Refiner architecture with Transformer decoder: Producer proposes initial confluent trajectories, Refiner recurrently refines them to form centerline graph. Includes efficient non-maximum suppression for merging duplicate branches.
...
Conclusion: RefTr demonstrates potential as new state-of-the-art framework for vascular tree analysis in 3D medical imaging, offering improved performance with computational efficiency.
...
Method: Created dataset with 9 scenes captured using two identical camera assemblies at 10 focal lengths (28-70mm) and 5 apertures (f/2.8-f/22), spanning 50 optical configurations with 2000 images per scene, including calibration files.

Result: Dataset enables controlled analysis of geometric and optical effects for depth estimation, depth-of-field rendering, deblurring, 3D reconstruction and novel view synthesis, revealing challenges with current state-of-the-art methods.

Conclusion: Bridges realism gap between synthetic training data and real camera optics, supporting reproducible research on real-world optical generalization.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual memorability datasets are limited by expensive human annotations, restricting diversity and scalability, and they only capture aggregate scores rather than nuanced memorability signals from natural recall descriptions.

Method: Leveraged tip-of-the-tongue (ToT) retrieval queries from online platforms like Reddit to create an unsupervised dataset, then fine-tuned large vision-language models and employed contrastive training for multimodal ToT retrieval.
...
Conclusion: The dataset and models provide a novel direction for visual content memorability research, facilitating progress by offering rich signals for memorability-related tasks without expensive human annotations.
...
TL;DR: Proposes a method to estimate fog parameters from stereo images using simultaneous optimization, handles locally homogeneous fog, and introduces SDIRF dataset with real foggy scenes and calibrated parameters.


<details>
  <summary>Details</summary>
Motivation: Previous methods estimate fog parameters sequentially, leading to error propagation. Real fog is often globally inhomogeneous, requiring better parameter estimation approaches.

Method: Simultaneous estimation of all fog parameters through novel optimization, assuming local fog homogeneity. Can be integrated as add-on module in SLAM/odometry systems.

Result: Superior performance on both synthetic and real foggy data from SDIRF dataset. Most accurate estimates on synthetic data and better adaptation to real fog conditions.

Conclusion: The method effectively handles real-world fog, advances visual perception in foggy conditions, and the SDIRF dataset enables further research in image defogging and depth reconstruction.
...
TL;DR: V^2-SAM adapts SAM2 for cross-view object correspondence using two prompt generators and a multi-expert selection mechanism, achieving state-of-the-art performance on multiple benchmarks.
...
Method: Proposes V^2-SAM with two complementary prompt generators: V^2-Anchor for geometry-aware correspondences using DINOv3 features, and V^2-Visual for appearance-guided cues via visual prompt matcher. Uses multi-expert design with Post-hoc Cyclic Consistency Selector (PCCS) to adaptively select the most reliable expert.
...
Conclusion: V^2-SAM successfully adapts SAM2 for cross-view object correspondence through unified prompt generation and expert selection, demonstrating effectiveness across diverse cross-view scenarios.
...
Method: Optimize the unconditional embedding in classifier-free guidance rather than manipulating latent or noise variables, leveraging the structured semantic nature of the text embedding space.

Result: Achieves state-of-the-art target test-time alignment while maintaining strong cross-reward generalization, establishing semantic-space optimization as an effective paradigm.

Conclusion: Null-TTA provides a principled approach for test-time alignment by directly steering the model's generative distribution through semantic-space optimization.
...
<details>
  <summary>Details</summary>
Motivation: Traditional implicit neural representations lack explicit geometric structure and have limited local editing capabilities, restricting their use in dynamic or interactive settings.

Method: The framework retrieves K nearest Gaussians for image coordinates, aggregates distance-weighted embeddings, and predicts RGB values through a neural network.

Result: GaINeR provides continuous image representation with interpretable geometric structure and flexible local editing capabilities.

Conclusion: The proposed method enables physically aware and interactive image manipulation, bridging the gap between implicit representations and geometric modeling.
...
Method: Multi-center retrospective study using T1 and contrast-enhanced T1-weighted MRI to simulate low-dose images. Deep learning models were trained to restore standard-dose quality from low-dose simulations, with evaluation of image quality and segmentation performance.

Result: DL restoration improved image quality metrics significantly - SSIM increased from 0.639 to 0.993 and PSNR from 21.6 to 41.4 dB. At 10% input dose, segmentation performance improved (Dice from 0.673 to 0.734). Both 10% and 30% dose restored images showed excellent quality, with 30% considered more informative.
...
TL;DR: Smooth regularization using Gaussian Random Walk improves video recognition by enforcing temporal coherence in embeddings, boosting lightweight models' accuracy by 3.8-6.4% on Kinetics-600.
...
Result: Lightweight models achieve 3.8% to 6.4% accuracy improvement on Kinetics-600. MoViNets improve state-of-the-art by 3.8-6.1% within FLOP constraints, while MobileNetV3 and MoViNets-Stream gain 4.9-6.4% over prior models.

Conclusion: The smooth regularization technique effectively captures complex temporal dynamics in lightweight video recognition models, significantly improving accuracy while maintaining computational efficiency.
...
TL;DR: A framework for open vocabulary compositional explanations in vision that uses semantic segmentation masks to probe neurons for arbitrary concepts without relying on human-annotated datasets.
...
Result: The framework enables flexible neuron probing for arbitrary concepts and datasets, providing comparable or better explanations than previous methods while offering greater flexibility.

Conclusion: The proposed framework successfully addresses the limitations of human-annotated compositional explanations by enabling open vocabulary analysis of neuron behavior across arbitrary concepts and tasks.
...
TL;DR: Introduces UruDendro4 dataset with 102 Pinus taeda L. cross-section images annotated with annual growth rings, enabling volumetric modeling of tree growth. Provides baseline performance using DeepCS-TRD method and shows dataset improves model generalization.


<details>
  <summary>Details</summary>
Motivation: Manual tree-ring measurement is time-consuming and imprecise. Existing datasets are scarce and lack samples from multiple stem heights needed for volumetric growth modeling.

Method: Created UruDendro4 dataset with 102 manually annotated cross-section images from multiple stem heights. Used DeepCS-TRD method for automatic ring detection and conducted ablation experiments to validate parameters.
...
Conclusion: UruDendro4 enables volumetric modeling of annual wood growth and provides reliable baseline for automated ring detection, addressing data scarcity in dendrochronology research.
...
TL;DR: BUSTR is a multitask vision-language framework that generates breast ultrasound reports without paired image-report data by using structured descriptors and radiomics features, achieving improved clinical efficacy.


<details>
  <summary>Details</summary>
Motivation: Current automated radiology report generation for breast ultrasound is limited by lack of paired datasets and hallucination risks from large language models.

Method: Constructs reports from structured descriptors, uses multi-head Swin encoder with multitask loss, and aligns visual/textual tokens via dual-level objective combining cross-entropy and cosine-similarity alignment.

Result: Consistently improves natural language generation metrics and clinical efficacy metrics across two BUS datasets, particularly for key targets like BI-RADS category and pathology.

Conclusion: Descriptor-aware vision model with combined token-level and alignment loss improves report metrics and clinical efficacy without requiring paired image-report data.
...
TL;DR: StickerNet is a two-stage framework for expressive image composition that predicts placement parameters based on real-world user editing patterns from an online platform, prioritizing artistic expression over realism.


<details>
  <summary>Details</summary>
Motivation: Traditional image composition focuses on realism, but modern content creation often aims for artistic, playful, or socially engaging content. The paper addresses this gap by defining expressive composition that reflects real user editing behavior on creative platforms.

Method: Two-stage framework: 1) determines composition type, 2) predicts placement parameters (opacity, mask, location, scale). Dataset built from 1.8 million real user editing actions from an online platform.
...
Conclusion: This work introduces a new direction in visual understanding that emphasizes expressiveness and user intent over realism, showing the value of learning from authentic editing behavior.
...
<details>
  <summary>Details</summary>
Motivation: Current traffic camera systems generate massive video data that's difficult to analyze efficiently. Traditional LLM-based approaches require time-consuming video-to-text conversion via VLMs, delaying timely insights from traffic footage.

Method: Uses sequential VLM processing across overlapping camera views, iteratively applying VLMs with varying token limits and using previous outputs as prompts. Includes object-level similarity detection to bypass redundant VLM invocations.

Result: Experimental results show TrafficLens reduces video-to-text conversion time by up to 4x compared to conventional approaches while preserving information accuracy.

Conclusion: TrafficLens provides an efficient solution for real-time multi-camera traffic analysis, enabling faster insights and incident investigation while maintaining analytical quality.
...
Method: Uses Vision Transformers (ViT) with homomorphic encryption, leveraging the ViT CLS token as a compact 768-dimensional feature representation for secure aggregation. Encrypts CLS tokens using CKKS homomorphic encryption before transmission to server.

Result: Achieves 96.12% global classification accuracy (unencrypted) and 90.02% (encrypted). Prevents model inversion attacks while requiring only 326 KB of encrypted data per aggregation round. Shows gradients are highly vulnerable to reconstruction (PSNR: 52.26 dB, SSIM: 0.999, NMI: 0.741).

Conclusion: The proposed CLS-protected HE approach provides strong privacy guarantees with significantly reduced communication overhead compared to gradient encryption, enabling secure collaborative learning across healthcare institutions while preventing sensitive data exposure.
...
TL;DR: An inversion-free style transfer framework using dual rectified flows that fuses content and style trajectories through dynamic midpoint interpolation, achieving efficient and high-quality stylization without computational inversion processes.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing diffusion-based style transfer methods that rely on computationally expensive inversion processes and suffer from visual distortions when inversion is inaccurate.

Method: Uses dual rectified flows to predict content and style trajectories in parallel, then fuses them through dynamic midpoint interpolation that integrates velocities from both paths. Incorporates attention injection for better style integration and content preservation.

Result: Achieves robust fusion across diverse styles and content, providing high visual fidelity while maintaining computational efficiency. Extensive experiments demonstrate strong generalization capabilities.

Conclusion: The proposed inversion-free framework offers an effective and efficient pipeline for style transfer that avoids the shortcomings of inversion-based methods while delivering high-quality stylized results.
...
TL;DR: A Ref-COD framework that eliminates the need for reference images at test time by distilling references into class-prototype memory during training and synthesizing reference vectors via query-conditioned prototype mixtures.
...
Method: Maintain EMA-updated class prototypes during training, predict mixture weights from queries to synthesize reference vectors, and use bidirectional attention alignment to bridge representation gaps between references and camouflaged queries.
...
TL;DR: WavePCNet: A physics-driven network using complex amplitude transfer operators and momentum memory to enhance obscured object localization and segmentation from faint light patterns beyond the field of view.


<details>
  <summary>Details</summary>
Motivation: Existing methods based on real-valued modeling or local convolutional operations are inadequate for capturing coherent light propagation physics and often converge to non-physical solutions under low signal-to-noise conditions.

Method: Proposes WavePCNet with Tri-Phase Wavefront Complex-Propagation Reprojection (TriWCP) to constrain coherent propagation behavior, momentum memory mechanism to suppress perturbation accumulation, and High-frequency Cross-layer Compensation Enhancement for frequency-selective pathways and structural consistency modeling.
...
Conclusion: WavePCNet effectively addresses challenges in obscured object perception by integrating physics-driven wavefront propagation simulation with robust compensation mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal safety guards only evaluate input questions and final answers, neglecting intermediate reasoning traces that can contain unsafe content even when the final answer is non-harmful, creating deployment risks.

Method: Introduces GuardTrace-VL with joint image-text analysis to monitor the full Question-Thinking-Answer pipeline. Uses a three-stage progressive training scheme combined with data refinement process using the GuardTrace dataset generated through diverse prompting strategies and MLRM-human voting verification.
...
Conclusion: GuardTrace-VL effectively addresses the safety gap in multimodal reasoning by monitoring intermediate rationales, providing comprehensive safety auditing for vision-language tasks with explicit reasoning traces.
...
<details>
  <summary>Details</summary>
Motivation: Layered image representation enables independent editing of elements for greater content creation flexibility, but decomposing single images into layers remains challenging due to limited methods and data.

Method: Adapt a diffusion-based inpainting model for layer decomposition using lightweight finetuning, introduce a multi-modal context fusion module with linear attention complexity to preserve detail in latent space, and train purely on synthetic dataset from open-source assets.

Result: The model achieves superior performance in object removal and occlusion recovery.

Conclusion: The approach unlocks new possibilities in downstream editing and creative applications by enabling effective layer decomposition from single images.
...
TL;DR: MERGE is a Multimodal Entity-aware Retrieval-augmented Generation framework that addresses challenges in news image captioning through entity-centric knowledge base construction, improved cross-modal alignment, and enhanced visual-entity matching.
...
Result: Significant improvements on GoodNews and NYTimes800k datasets: CIDEr gains of +6.84 and +1.16, F1-score improvements of +4.14 and +2.64 in named entity recognition. Strong generalization on Visual News dataset with +20.17 CIDEr and +6.22 F1-score.

Conclusion: MERGE demonstrates superior performance in news image captioning with enhanced information coverage, cross-modal alignment, and visual-entity grounding, showing strong robustness and domain adaptability across multiple datasets.
...
Conclusion: MetaRank provides an efficient, task-aware approach for MTE metric selection that outperforms ad hoc selection methods, enabling practitioners to choose the most suitable metric for new datasets without full fine-tuning.
...
TL;DR: A novel trustworthy multi-view classification framework using prototypes to represent neighbor structures, enabling efficient cross-view consensus discovery with competitive performance and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing TMVC methods have high computational costs from dense neighbor relationships, lack consistency guarantees across views, and use manual weight assignments that undermine trustworthiness.

Method: Introduces prototypes to represent neighbor structures for each view, simplifies intra-view neighbor relation learning, and enables dynamic alignment of intra- and inter-view structures.
...
Conclusion: The proposed prototype-based framework effectively addresses computational efficiency and consistency issues in TMVC, providing more trustworthy classification outcomes.
...
TL;DR: CameraMaster is a unified framework for precise camera parameter control in image retouching that decouples camera directives from content and integrates them via cross-attention and conditioning mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing methods either rely on ambiguous text prompts that hinder precise camera control or use separate training heads that compromise scalability and multi-parameter composition.

Method: Explicitly decouples camera directives and integrates two streams: directive representation and parameter embedding. Uses parameter embedding to modulate directives and content, injects modulated directives via cross-attention, and applies conditioning/gating to time embedding for unified layer-wise modulation.

Result: Produces monotonic and near-linear responses to parameter variations, supports seamless multi-parameter composition, and significantly outperforms existing methods on a 78K image-prompt dataset.

Conclusion: CameraMaster enables physically consistent image retouching with precise parameter control while maintaining scalability and supporting multi-parameter adjustments through its unified camera-aware framework.
...
Method: Built 33,027 densely annotated multiple-choice questions across 4 domains with fine-grained taxonomies, using an LLM to answer questions using captions alone to measure utility.

Result: Evaluation of state-of-the-art MLLMs shows substantial utility gaps between images and their captions, with models performing up to 32% worse on caption utility despite similar traditional QA performance.

Conclusion: CaptionQA provides a comprehensive utility-based evaluation framework that reveals significant limitations in current captioning models' ability to preserve image-level utility for downstream tasks.
...
TL;DR: FlowerDance is an efficient music-to-dance generation method that combines MeanFlow with physical constraints for high-quality motion generation using few sampling steps, featuring a BiMamba-based architecture with non-autoregressive generation and motion editing capabilities.
...
Method: Combines MeanFlow with Physical Consistency Constraints for high-quality motion with few sampling steps. Uses BiMamba-based backbone with Channel-Level Cross-Modal Fusion for efficient non-autoregressive generation. Supports motion editing for interactive refinement.
...
TL;DR: A novel framework that reconciles model-based and generative approaches for spatially varying image deblurring by combining dense physical constraints with generative priors.


<details>
  <summary>Details</summary>
Motivation: Current methods either produce over-smoothed results with physical constraints or hallucinate details with weak constraints. There's a need to bridge the gap between physical accuracy and perceptual realism.

Method: Models degradation as dense continuum of high-dimensional compressed kernels, then uses this descriptor field to condition a ControlNet architecture that strongly guides diffusion sampling.
...
Conclusion: Successfully bridges the gap between physical accuracy and perceptual realism in spatially varying image deblurring.
...
TL;DR: MUSE is a unified framework for emotional image generation and editing that uses gradient-based optimization with emotion classifiers, optimal timing selection, and multi-emotion loss to achieve superior emotional accuracy and semantic diversity.
...
Method: Adopts Test-Time Scaling strategy with gradient-based optimization of emotional tokens using off-the-shelf emotion classifiers, identifies optimal timing using semantic similarity, and employs multi-emotion loss to reduce interference from inherent and similar emotions.

Result: MUSE performs favorably against all methods for both generation and editing, improving emotional accuracy and semantic diversity while maintaining optimal balance between content, text prompt adherence, and realistic emotional expression.

Conclusion: MUSE establishes a new paradigm for emotion synthesis by unifying generation and editing tasks in a single framework without requiring additional diffusion model updates or specialized emotional datasets.
...
<details>
  <summary>Details</summary>
Motivation: Long-term AD predictions face difficulties maintaining disease-related characteristics when dealing with irregular time intervals in sequential imaging data, as time-related distribution aspects can reflect disease progression changes.

Method: T-NIG model employs brain images from two time points, uses coordinate neighborhoods for feature identification, incorporates time parameters into normal inverse gamma distribution, and utilizes uncertainty estimation to reduce epistemic and aleatoric uncertainties from insufficient temporal data.
...
Conclusion: The T-NIG model effectively handles irregular time intervals in sequential brain imaging data for Alzheimer's Disease prediction, maintaining disease characteristics through temporal parameter estimation and uncertainty reduction.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion-based editing models often fail to accurately interpret complex user instructions involving compositional relationships, contextual cues, or referring expressions, leading to semantically drifted edits.

Method: Proposed MIRA, a plug-and-play multimodal reasoning agent that performs editing through iterative perception-reasoning-action loops, predicting atomic edit instructions step by step using visual feedback. Trained on 150K multimodal dataset with SFT + GRPO pipeline.

Result: When paired with open-source editing models like Flux.1-Kontext, Step1X-Edit, and Qwen-Image-Edit, MIRA significantly improves semantic consistency and perceptual quality, achieving performance comparable to or exceeding proprietary systems like GPT-Image and Nano-Banana.

Conclusion: MIRA effectively simulates multi-turn human-model interaction processes and enables more accurate interpretation of complex editing instructions through iterative reasoning.
...
TL;DR: Proposes a novel iris authentication pipeline using 3D-CNN to capture spatio-temporal features through curriculum learning, addressing robustness challenges like rotation, scale, reflections, and blur.


<details>
  <summary>Details</summary>
Motivation: Existing iris authentication methods lack robustness to variations (rotation, scale, reflections, blur) and fail to effectively leverage spatio-temporal structure of iris patterns, relying on simple point-to-point comparisons.

Method: Splits iris images into sequences of sub-images, processes them with 3D-CNN to capture spatial and spatio-temporal cues, trained end-to-end with triplet and ArcFace loss in curriculum manner.

Result: The approach learns rich spatio-temporal representations that embed temporal dependencies directly into feature space, improving discriminability in deep metric domain despite challenging variations.

Conclusion: The proposed framework provides a robust and generalizable solution for iris authentication by effectively modeling spatio-temporal feature dynamics through curriculum learning.
...
TL;DR: Pygmalion Effect in Vision framework uses image-to-clay translation to suppress specular reflections while preserving geometry for robust 3D reconstruction of reflective objects.
...
TL;DR: RadarFM is a radar foundation model that learns unified scene-level representations using structured spatial language supervision, enabling transfer across tasks through contrastive learning and spatial reasoning.
...
Method: Uses structured caption framework to encode vehicle distributions in radar coordinates and hash-aware contrastive learning for fine-grained spatial reasoning, trained on large-scale CARLA simulator datasets.

Result: Proposes localization-aware metrics for assessing spatial accuracy beyond traditional detection measures.

Conclusion: RadarFM enables unified scene-level representations for radar sensing through structured language supervision and contrastive learning.
...
TL;DR: EM-KD enhances Efficient Multimodal LLMs using knowledge distillation with vision token alignment and two distillation strategies to overcome information loss from token compression.


<details>
  <summary>Details</summary>
Motivation: Existing efficient MLLMs compress vision tokens to reduce resources but lose visual information, and prior knowledge distillation methods overlook the fundamental differences caused by unbalanced vision tokens between student and teacher models.

Method: Proposes EM-KD with: 1) Manhattan distance + Hungarian matching to align unbalanced vision tokens between teacher and student, 2) Vision-Language Affinity Distillation (VLAD) using smooth L1 distance on affinity matrices, and 3) Vision Semantic Distillation (VSD) using reverse KL divergence on vision logits.

Result: EM-KD trained models outperform prior Efficient MLLMs on both accuracy and efficiency by large margins across diverse benchmarks, and also beat other distillation methods when equipped with the proposed vision token matching.

Conclusion: EM-KD effectively enhances efficient MLLMs through proper vision token alignment and comprehensive distillation strategies, achieving superior performance in both accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current methods struggle with maintaining geometric fidelity when synthesizing appearance under large viewpoint shifts, facing issues like over-restoration and geometric drift due to lack of pixel-wise, 3D-consistent editing criteria.

Method: Introduces Expected Information Gain (EIG) as a unified policy that guides diffusion as a spatial prior to refine high-uncertainty regions and distills edits back into 3DGS through pixel-level weighting.
...
Conclusion: FaithFusion provides an effective plug-and-play system for 3DGS-diffusion fusion that maintains geometric fidelity without requiring extra prior conditions or structural modifications.
...
<details>
  <summary>Details</summary>
Motivation: Early prediction of Alzheimer's disease is crucial to slow progression. Current methods rely on manual feature extraction from brain images, which is inefficient. There's a need for automated methods that can handle missing temporal data and learn morphological changes for early prediction.

Method: Proposes Deformation-Aware Temporal Generative Network (DATGN) that first interpolates incomplete MRI sequences, then uses bidirectional temporal deformation-aware module to generate future MRI images following disease progression patterns.

Result: DATGN achieved competitive PSNR and MMSE metrics for MRI generation. When synthetic data was used with classification methods, accuracy improved significantly: 6.21% to 16% for AD vs. NC classification, and 7.34% to 21.25% for AD vs. MCI vs. NC classification.

Conclusion: DATGN effectively generates realistic future MRI images that follow Alzheimer's brain atrophy trends, enabling early disease prediction and significantly improving classification performance when used as synthetic training data.
...
TL;DR: EntPruner is an entropy-guided automatic pruning framework for diffusion and flow models that achieves up to 2.22x inference speedup while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Pre-trained vision generative models have significant parameter redundancy when transferred to downstream tasks, requiring efficient pruning methods that preserve output diversity and condition-fidelity.

Method: Uses entropy-guided pruning with Conditional Entropy Deviation (CED) metric to assess block importance, and a zero-shot adaptive pruning framework that dynamically determines when and how much to prune during training.

Result: Achieves up to 2.22x inference speedup while maintaining competitive generation quality on ImageNet and three downstream datasets with DiT and SiT models.

Conclusion: EntPruner effectively reduces parameter redundancy in generative models while preserving performance through data-dependent importance assessment and adaptive pruning strategy.
...
<details>
  <summary>Details</summary>
Motivation: Geometry-only cues (depth, edges) are insufficient as they specify layout but under-constrain appearance, materials, and illumination, limiting physically meaningful edits like relighting or material swaps and causing temporal drift. Additional graphics-based modalities provide complementary constraints for better understanding and precise control.

Method: Proposes CtrlVDiff with Hybrid Modality Control Strategy (HMCS) that routes and fuses features from multiple modalities. Uses MMVideo dataset - a hybrid real-and-synthetic dataset aligned across modalities and captions. The model accepts any subset of modalities and remains robust to missing inputs while maintaining temporal consistency.

Result: CtrlVDiff delivers superior controllability and fidelity across understanding and generation benchmarks. Enables layer-wise edits (relighting, material adjustment, object insertion) and surpasses state-of-the-art baselines while remaining robust when some modalities are unavailable.

Conclusion: Enriching video models with graphics-based modalities enables both better understanding and precise, predictable control during generation. The unified approach handles multiple heterogeneous cues effectively and maintains temporal coherence for video editing tasks.
...
Method: Uses Fourier Kernel Estimator with activation in Fourier space to convert spatial convolution to multiplication, jointly optimized with deblur model. Employs feature-based convolution and decoupled multi-scale architecture with reversible sub-unets.

Result: Achieves state-of-the-art motion deblurring results and shows potential for other kernel-related problems. The kernel estimator learns physically meaningful kernels.

Conclusion: The proposed FKE enables effective kernel-level blur process learning with low complexity, significantly improving deblurring performance by understanding the essence of blur.
...
TL;DR: Ent-Prog is an efficient training framework for human video generation using diffusion models, reducing training time and GPU memory consumption while maintaining performance through prioritized component training and adaptive progressive scheduling.


<details>
  <summary>Details</summary>
Motivation: Human video generation with diffusion models faces high computational costs and memory consumption due to training on high-resolution, multi-frame data, making efficient training methods necessary.
...
Conclusion: Ent-Prog provides an effective solution for efficient training of diffusion models in human video generation, significantly reducing computational requirements while preserving model quality.
...
TL;DR: AnchorOPT introduces dynamic anchors in prompt learning that adapt both values and positions based on task context and training stage, improving CLIP generalization without complex modules.


<details>
  <summary>Details</summary>
Motivation: Existing prompt learning methods use static textual anchors that lack cross-task and stage-adaptive flexibility, limiting their effectiveness across diverse scenarios.

Method: Two-stage training: first learn dynamic anchor tokens from task-specific data, then optimize soft tokens and position matrix; introduces learnable position matrix for adaptive anchor-soft token relationships.

Result: Achieves comparable or better performance than methods with additional modules/regularization; integrates seamlessly as plug-and-play module with consistent gains across datasets.

Conclusion: Dynamic anchor optimization through value and position adaptation significantly enhances prompt learning effectiveness while maintaining simplicity and broad applicability.
...
TL;DR: ProxyFormer is a novel RVOS architecture that uses proxy queries to integrate visual and text semantics, enabling better cross-modality alignment and object tracking through inter-frame dependencies.


<details>
  <summary>Details</summary>
Motivation: Existing RVOS methods have two key limitations: conditional queries lack inter-frame dependency modeling, making accurate tracking difficult amid frame variations, and they integrate textual constraints too late, potentially causing focus on non-referred objects.
...
Conclusion: ProxyFormer effectively addresses limitations of previous approaches by establishing inter-frame dependencies and early integration of textual constraints, leading to improved object tracking accuracy and coherence.
...
Conclusion: TEAR effectively addresses temporal safety risks in T2V models and demonstrates the need for specialized evaluation frameworks for dynamic video content generation.
...
Method: Proposed Progressive Visual Compression (PVC) with two modules: refined patch embedding for flexible patch-size scaling, and windowed token compression deployed hierarchically across ViT layers to progressively aggregate local token representations.
...
TL;DR: Visual distractors in VLMs cause inverse scaling (reduced accuracy) but don't increase reasoning length like textual distractors do. Tracking attribute counts in reasoning traces reveals how distractors affect performance, and simple prompting can mitigate bias.


<details>
  <summary>Details</summary>
Motivation: To investigate whether visual distractors in vision-language models cause similar inverse scaling effects as observed in language models with textual distractors.

Method: Created Idis dataset with systematic visual distractors across semantic, numerical, and spatial dimensions. Analyzed reasoning traces and attribute counts. Applied to Waterbirds benchmark and tested prompting strategies.

Result: Visual distractors cause inverse scaling (accuracy decreases) but don't increase reasoning length like textual distractors. Attribute tracking in reasoning traces provides insights into distractor effects. Simple prompting can reduce bias-driven predictions.

Conclusion: Visual distractors fundamentally differ from textual ones in their effects on VLMs, causing accuracy reduction without reasoning length increase. Reasoning trace analysis offers valuable insights, and targeted prompting can mitigate bias.
...
<details>
  <summary>Details</summary>
Motivation: Test-time computation scaling has shown success in natural language tasks but remains unexplored for visual AR models. Naive approaches like Best-of-N are suboptimal due to wasted computation on erroneous trajectories and lack of canvas blueprint in raster-scan decoding.
...
Conclusion: GridAR effectively addresses the limitations of naive test-time scaling for visual AR models, achieving higher quality results with lower computational costs, and generalizes well to image editing tasks.
...
Method: Three-stage scene tokenization pipeline using Multi-Scale Normal Distributions Transform (NDT) representation with Multi-Scale NDT Decoder (MSDec) that fuses cross-scale features and supports human-interactive prompting.
...
Conclusion: NDTokenizer3D provides a compact and unified design for fine-grained, general-purpose 3D vision-language understanding that bridges language reasoning with 3D spatial understanding.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models are vulnerable to adversarial attacks, but existing patches overfit to single models and fail in black-box settings. Universal and transferable attacks remain underexplored.
...
Result: Experiments show UPA-RFAS consistently transfers across diverse VLA models, manipulation tasks, viewpoints, and physical executions, exposing a practical patch-based attack surface.

Conclusion: UPA-RFAS establishes a strong baseline for universal, transferable adversarial attacks against VLA-driven robots, highlighting vulnerabilities that need future defense mechanisms.
...
Method: G²VLM natively leverages learned 3D visual geometry features to predict 3D attributes and enhance spatial reasoning via in-context learning and interleaved reasoning, training on abundant multi-view image and video data.

Result: G²VLM achieves comparable results to state-of-the-art 3D reconstruction models and shows better or competitive performance across spatial understanding and reasoning tasks.

Conclusion: By unifying semantic VLMs with 3D vision tasks, G²VLM serves as a strong baseline for spatial intelligence and enables future applications like 3D scene editing.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep clustering methods suffer from disparity between well-structured local features and poorly separated global feature clusters, leading to suboptimal clustering performance.

Method: Uses adaptive k-nearest neighbors-based consistency filtering to identify high-confidence samples as anchors, then computes a discriminative loss to promote intra-class compactness and inter-class separability for network optimization.
...
Conclusion: DCBoost effectively bridges the gap between local and global feature structures in deep clustering, providing substantial performance improvements without requiring additional parameters.
...
TL;DR: BotaCLIP is a lightweight multimodal framework that adapts pre-trained Earth Observation foundation models to inject botanical domain knowledge through contrastive learning, improving performance on ecological tasks.


<details>
  <summary>Details</summary>
Motivation: To adapt pre-trained foundation models for domain-specific applications without expensive retraining, specifically for biodiversity modeling where expert ecological knowledge is needed but data is scarce.
...
Result: BotaCLIP representations consistently outperformed original DOFA embeddings and supervised baselines across three ecological tasks: plant presence prediction, butterfly occurrence modeling, and soil trophic group abundance estimation.
...
TL;DR: The paper proposes ART framework for fine-grained action recognition using query-response mechanism to track distinctive local details across video frames.


<details>
  <summary>Details</summary>
Motivation: Current methods capture coarse-grained motion patterns but struggle with subtle local details that evolve over time, which are crucial for distinguishing similar fine-grained actions.

Method: Uses region-specific semantic activation with text-constrained queries from VLMs to capture action-related regions, organizes responses into tracklets, and applies multi-level contrastive constraints for optimization.
...
Conclusion: The ART framework effectively discovers and tracks distinctive local details through query-response mechanism, enabling better fine-grained action recognition.
...
<details>
  <summary>Details</summary>
Motivation: To compare different generative modeling paradigms and explore efficient alternatives to iterative sampling methods like DDPM.

Method: Implemented DDPM, CFM, and MeanFlow using unified TinyUNet architecture on CIFAR-10, with CFM extended to image inpainting using four mask types.

Result: CFM achieved FID 24.15 vs DDPM's 402.98; MeanFlow achieved FID 29.15 with single-step sampling; Inpainting improved PSNR from 4.95 to 8.57 dB and SSIM from 0.289 to 0.418.

Conclusion: CFM outperforms DDPM significantly, MeanFlow enables efficient single-step generation, and inpainting-aware training substantially improves reconstruction quality.
...
TL;DR: T3-Tracer is a novel framework for detecting partial audio forgeries by analyzing audio at frame, segment, and audio levels using two complementary modules: FA-FAM for frame-level authenticity detection and SMDAM for segment-level boundary detection.


<details>
  <summary>Details</summary>
Motivation: Partial audio forgery is challenging to detect because attackers modify only critical frames while maintaining overall perceptual authenticity. Existing methods lack hierarchical structure to capture both transient and sustained anomalies across different temporal levels.
...
TL;DR: FIELDS improves 3D face reconstruction by combining 2D image consistency with direct 3D expression supervision from 4D scans and emotion recognition, enabling accurate capture of subtle emotional details.
...
Conclusion: FIELDS successfully bridges the 2D/3D domain gap and mitigates expression-intensity bias, enabling emotion-rich 3D face models from single images that capture genuine emotional content.
...
Result: Evaluated on computer vision problems using polarimetric SAR images, showing improved performance for both invariance in classification and equivariance in reconstruction/segmentation tasks.

Conclusion: The extension of LPS to complex-valued networks successfully achieves shift equivariance and invariance properties with theoretical guarantees, demonstrating practical benefits in various computer vision applications.
...
TL;DR: AVFakeBench is a comprehensive audio-video forgery detection benchmark covering 12K questions across 7 forgery types and 4 annotation levels, addressing limitations of existing DeepFake-focused benchmarks.
...
Method: Proposed a multi-stage hybrid forgery framework integrating proprietary models for task planning with expert generative models for precise manipulation. Established a multi-task evaluation framework covering binary judgment, forgery type classification, detail selection, and explanatory reasoning.

Result: Evaluated 11 Audio-Video Large Language Models and 2 detection methods, showing AV-LMMs' potential as emerging forgery detectors but revealing notable weaknesses in fine-grained perception and reasoning.

Conclusion: AVFakeBench provides a comprehensive benchmark for audio-video forgery detection, demonstrating both the promise and limitations of current AV-LMMs in detecting diverse forgery types across complex natural scenes.
...
Method: LaGen uses frame-by-frame autoregressive generation with scene decoupling estimation for object-level content and noise modulation to reduce error accumulation during long-horizon generation.

Result: Experimental results show LaGen outperforms state-of-the-art LiDAR generation and prediction models, especially on later frames.

Conclusion: LaGen successfully enables long-horizon interactive generation of LiDAR scenes, addressing limitations of existing approaches.
...
TL;DR: MatchGS is a framework that corrects 3D Gaussian Splatting's geometric inaccuracies to generate high-quality training data for zero-shot image matching, achieving up to 17.7% performance improvement.


<details>
  <summary>Details</summary>
Motivation: Learning-based image matching requires large-scale, diverse, and geometrically accurate training data. 3DGS enables photorealistic novel-view synthesis but has geometric inaccuracies that prevent robust correspondence labeling.
...
Conclusion: With proper geometric refinement, 3DGS can serve as a scalable, high-fidelity data source for robust zero-shot image matchers, enabling supervision under extreme viewpoint changes.
...
Method: Proposes data curation engine for flexible vision-language conversations, unified dynamic-resolution strategy for diverse image scales, Zoom-in Chain mechanism for UHR images, enhanced object detection capability, and novel evaluation protocol.
...
<details>
  <summary>Details</summary>
Motivation: Current Vision Transformers for road segmentation have quadratic complexity that limits deployment on resource-constrained platforms, while road networks inherently require modeling long, continuous structures.

Method: Integrates Mamba blocks to trace continuous road networks and preserve topological structure, combined with Transformer blocks for global context refinement in a hybrid architecture.
...
Conclusion: PathMamba demonstrates that combining Mamba's sequential modeling with Transformer's global reasoning yields topologically superior road segmentation without prohibitive scaling costs.
...
Method: Introduces CaliTex with two modules: Part-Aligned Attention for spatial alignment across semantically matched parts, and Condition-Routed Attention for routing appearance through geometry-conditioned pathways. Uses two-stage diffusion transformer.

Result: CaliTex produces seamless and view-consistent textures, outperforming both open-source and commercial baselines.
...
<details>
  <summary>Details</summary>
Motivation: VGGT's joint inference of 3D attributes requires global attention layers with all-to-all attention computation, causing significant latency bottlenecks for large scenes with long-sequence inputs.

Method: Head-wise temporal merging (HTTM) merges tokens in multi-head granularity instead of uniformly across attention heads, leveraging spatial locality and temporal correspondence at head level to preserve feature uniqueness after head concatenation.

Result: HTTM achieves up to 7x acceleration with negligible performance drops in GPU-based inference, outperforming existing merging techniques that suffer from identical tokens in output layers.

Conclusion: HTTM provides an effective training-free solution for accelerating VGGT while maintaining performance, addressing the computational bottleneck of global attention in 3D scene reconstruction.
...
TL;DR: ConFu is a multimodal contrastive learning framework that jointly embeds individual modalities and their fused combinations, capturing both pairwise relationships and higher-order dependencies across multiple modalities.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal methods mainly focus on pairwise alignment between two modalities, often overlooking higher-order interactions or failing to preserve pairwise relationships needed for single-modality tasks.

Method: Extends traditional pairwise contrastive objectives with an additional fused-modality contrastive term that aligns modality pairs with a third modality, creating a unified representation space for both individual and fused modalities.

Result: Demonstrates competitive performance on retrieval and classification tasks, captures XOR-like higher-order dependencies, and supports unified one-to-one and two-to-one retrieval within a single framework.

Conclusion: ConFu successfully addresses the challenge of learning joint multimodal representations by simultaneously preserving pairwise relationships while capturing higher-order dependencies across multiple modalities.
...
TL;DR: SIFT-SNN framework for real-time structural anomaly detection using SIFT features and SNN classification, achieving 92.3% accuracy with 9.5ms latency.


<details>
  <summary>Details</summary>
Motivation: Need for low-latency, low-power real-time detection of structural anomalies in transport infrastructure with transparent decision-making.

Method: Hybrid pipeline combining SIFT for spatial feature encoding with latency-driven spike conversion and LIF SNN for classification, tested on Auckland Harbour Bridge dataset.
...
Conclusion: SIFT-SNN provides efficient, interpretable structural monitoring with potential for widespread deployment, though generalization to unseen conditions needs validation.
...
TL;DR: SurgMLLMBench is a unified multimodal benchmark for surgical scene understanding that integrates pixel-level segmentation and VQA annotations across multiple surgical domains under a unified taxonomy.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with selecting appropriate neighborhood sizes for different data/geometries and have difficulties accurately predicting normals for various point clouds.

Method: Proposes Patch Feature Fitting (PFF) using multi-scale feature aggregation and cross-scale feature compensation. The aggregation module progressively shrinks patch sizes while capturing structure characteristics, and the compensation module reuses features from earlier layers.
...
Conclusion: The multi-scale feature approximation strategy enables scale adaptation for varying local patches and delivers optimal feature description for robust normal estimation.
...
TL;DR: Endo-G²T is a geometry-guided and temporally aware training scheme for 4D Gaussian splatting that addresses view-dependent effects in endoscopic videos through monocular depth supervision, temporal coherence modeling, and keyframe-constrained streaming.


<details>
  <summary>Details</summary>
Motivation: Endoscopic videos suffer from strong view-dependent effects like specularities and occlusions, where pure photometric supervision causes geometric drift that becomes hard to correct during densification. The goal is to anchor geometry early while maintaining temporal consistency and efficiency.

Method: Three key components: 1) Geo-guided prior distillation using confidence-gated monocular depth with scale-invariant losses and warm-up-to-cap schedule; 2) Time-embedded Gaussian field with rotor-like rotation for temporal coherence; 3) Keyframe-constrained streaming with max-points budget for efficiency and stability.
...
Conclusion: Endo-G²T successfully addresses geometric drift in endoscopic 4D reconstruction by combining geometry guidance, temporal awareness, and efficient streaming, enabling robust dynamic scene modeling in challenging endoscopic environments.
...
TL;DR: STVG-o1 enables off-the-shelf MLLMs to achieve state-of-the-art spatio-temporal video grounding performance without architectural changes, using bounding-box chain-of-thought reasoning and multi-dimensional reinforcement rewards.
...
Result: Sets new state-of-the-art on HCSTVG (7.3% m_tIoU improvement on HCSTVG-v1), matches specialized models on VidSTG, and surpasses all existing MLLM-based approaches by large margins with strong open-vocabulary generalization.

Conclusion: Establishes MLLMs as viable and powerful backbones for precise spatio-temporal video grounding, demonstrating their potential for this challenging task.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual reasoning methods lack human-like abstract visual thinking due to limitations of external tools, failing to enable direct reasoning within visual latent space.

Method: Three-stage distillation-based SFT pipeline to address computational cost and supervision issues, plus VLPO reinforcement learning method that explicitly incorporates latent embeddings into policy gradient updates.
...
Conclusion: The framework successfully enables multimodal reasoning in latent visual space, with empirical analysis providing insights for future developments in visual latent reasoning.
...
Conclusion: DiverseVAR provides an effective test-time solution to improve VAR model diversity without computational overhead or retraining.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods have weak region awareness and limited temporal alignment in remote sensing change captioning. The paper aims to address these limitations by leveraging foundation models for better region-level understanding.

Method: Uses CNN/Transformer for global features, SAM foundation model for semantic- and motion-level change regions, and a knowledge graph for object information. These are fused via cross-attention and processed by a Transformer decoder for caption generation.

Result: The method achieves state-of-the-art performance across multiple benchmark datasets for remote sensing change captioning.

Conclusion: The proposed approach effectively addresses region awareness and temporal alignment issues in change captioning by leveraging SAM foundation model and knowledge graphs, demonstrating superior performance over existing methods.
...
TL;DR: E-M3RF is an equivariant multimodal 3D reassembly framework that combines geometric and color features using SE(3) flow matching to improve reassembly accuracy, especially when geometry alone is ambiguous.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based 3D reassembly methods rely primarily on geometric features, struggling with ambiguous cases like small, eroded, or symmetric fragments, and lack physical constraints to prevent overlapping assemblies.
...
Conclusion: The multimodal approach combining geometric and color features with equivariant encoding significantly improves 3D reassembly accuracy, particularly in challenging cases where geometry alone is insufficient.
...
TL;DR: Unsupervised framework for extracting VLA pre-training data from industrial videos using motion tokenization and action segmentation with Latent Action Energy metric.
...
TL;DR: Proposes a hypergraph-guided spatio-temporal event stream completion method to address spatial sparsity in event cameras by connecting event tokens across time and space using hypergraphs and multi-modal information fusion.


<details>
  <summary>Details</summary>
Motivation: Event cameras produce spatially sparse but temporally dense data, and existing representation learning methods struggle with undersampling problems caused by this spatial sparsity.

Method: Uses hypergraphs to connect event tokens across different times and spatial locations, employs contextual information message passing for event completion, and incorporates RGB tokens for multi-modal hypergraph-based information completion with self-attention aggregation.
...
TL;DR: MobileI2V is a 270M lightweight diffusion model for real-time 720p image-to-video generation on mobile devices, achieving 10x speed improvement through step distillation and mobile optimizations.


<details>
  <summary>Details</summary>
Motivation: Address the computational complexity and slow generation speed of diffusion models for real-time, high-resolution video generation on resource-constrained mobile devices.

Method: Proposes linear hybrid architecture denoiser, time-step distillation reducing sampling from 20+ to 2 steps, and mobile-specific attention optimizations for 2x speed-up.

Result: Enables fast 720p image-to-video generation on mobile devices with quality comparable to existing models, achieving <100ms per frame generation speed under one-step conditions.

Conclusion: MobileI2V demonstrates the feasibility of real-time high-quality video generation on mobile devices through efficient architecture design and optimization strategies.
...
Result: Significantly improves accuracy while reducing computational overhead, and effectively mitigates rank collapsing and over-smoothing phenomenon.

Conclusion: The proposed frequency-aware token reduction strategy provides an effective solution to improve Vision Transformer efficiency while preserving performance, with analysis revealing implicit frequency characteristics and limitations of previous methods.
...
TL;DR: Merge-and-Bound (M&B) is a novel Class Incremental Learning approach that optimizes model weights through inter-task and intra-task weight merging with bounded updates to prevent catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: To address catastrophic forgetting in Class Incremental Learning by directly manipulating model weights in parameter space rather than modifying architectures or learning objectives.

Method: Uses two types of weight merging: inter-task (averaging weights from previous stages) and intra-task (combining parameters within current stage), plus bounded update technique to minimize cumulative updates while preserving previous knowledge.
...
TL;DR: CanKD is a novel feature-based knowledge distillation method using cross-attention to enable non-local knowledge transfer between teacher and student models, outperforming state-of-the-art methods in object detection and segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional self-attention distillation methods align teacher and student features independently, lacking dynamic consideration of pixel-wise relationships across feature maps. CanKD aims to enable more thorough knowledge transfer by allowing each student pixel to consider all teacher pixels.

Method: Uses cross-attention mechanisms where each pixel in student feature map dynamically attends to all pixels in teacher feature map. Introduces only an additional loss function without complex architectural changes.
...
Conclusion: CanKD demonstrates potential as a new paradigm for attention-guided distillation in computer vision, achieving superior performance with minimal additional complexity.
...
TL;DR: Systematic investigation reveals that implementation details like data preprocessing and augmentation significantly impact deepfake detection performance more than core design, leading to architecture-agnostic best practices.
...
Method: Systematic investigation isolating individual design choices' impact on training, inference, and incremental updates in deepfake detection models.

Result: Identified consistent design choices that improve detection and achieve state-of-the-art performance on AI-GenBench benchmark.
...
TL;DR: A novel framework for automated ANA detection using multi-instance multi-label learning with task-specific components including instance sampler, probabilistic pseudo-label dispatcher, and self-paced learning, achieving state-of-the-art performance on medical datasets.


<details>
  <summary>Details</summary>
Motivation: Manual ANA testing is slow, labor-intensive, and requires extensive training. Existing automation methods struggle with the complexities of multi-instance multi-label learning in real-world clinical settings with over 100 antibody types and vast fluorescent pattern combinations.

Method: Proposed framework handles MIML tasks using unaltered microscope images without manual preprocessing. Uses three components: instance sampler to suppress low-confidence instances, probabilistic pseudo-label dispatcher for adaptive label assignment, and self-paced learning coefficients for training adjustment.

Result: Achieved up to +7.0% F1-Macro and +12.6% mAP gains over best prior method on ANA dataset. Ranked top-2 across all key metrics on public datasets, reducing Hamming loss by up to 18.2% and one-error by 26.9%.

Conclusion: The framework successfully overcomes limitations of traditional MIML methods, supports end-to-end optimization, and demonstrates superior performance for automated ANA detection in clinical settings.
...
<details>
  <summary>Details</summary>
Motivation: Current foundation model approaches require prohibitive computational resources and contradict sustainable AI principles, limiting accessibility to large institutions only.

Method: Decomposes training into lightweight, task-specific ConvNeXtV2 specialists that can be frozen and reused, supporting federated training, pruning, and continuous integration.

Result: Creates a modular framework that offers advantages in efficiency, interpretability, and extensibility for collaborative and resource-constrained settings.

Conclusion: Sets a new direction for building scalable and efficient Remote Sensing Foundation Models through an ensemble approach rather than model scaling.
...
TL;DR: This paper presents a method for generating sequential MRI images to predict Alzheimer's disease progression, using quantitative metrics and age-scaling factors to improve image synthesis accuracy.


<details>
  <summary>Details</summary>
Motivation: Timely identification of Alzheimer's disease is crucial for personalized treatment, but predicting disease progression from irregularly captured MRI sequences is challenging.

Method: Innovative sequential image generation methodology guided by quantitative metrics, with integrated age-scaling factor to produce age-specific MRI images for disease prediction.

Result: Quantitative metrics significantly improved MRI image synthesis accuracy, age-scaled pixel loss enhanced iterative generation, and Structural Similarity Index reached 0.882 for long-term prognosis.

Conclusion: The proposed method effectively generates sequential MRI images for Alzheimer's disease prediction, with quantitative metrics and age-scaling improving synthesis quality and disease progression modeling.
...
TL;DR: PRFL is a latent-space reward feedback learning framework for video generation that avoids pixel-space processing, enabling efficient optimization throughout the full denoising chain while improving alignment with human preferences.


<details>
  <summary>Details</summary>
Motivation: Existing video reward models rely on pixel-space inputs, causing high memory overhead, increased training time, and limited early-stage supervision that only refines visual quality rather than fundamental motion dynamics and structural coherence.

Method: Leverages pre-trained video generation models as reward models in noisy latent space, conducting preference optimization entirely in latent space without VAE decoding, enabling efficient gradient backpropagation throughout the full denoising chain.
...
Conclusion: Video generation models are naturally suited for reward modeling in latent space, and PRFL provides an efficient framework for preference optimization that overcomes limitations of pixel-space approaches.
...
TL;DR: UAVLight is a new benchmark dataset for illumination-robust 3D reconstruction that captures scenes at multiple fixed times of day along repeatable flight paths to provide natural lighting variation while maintaining consistent geometry and viewpoints.


<details>
  <summary>Details</summary>
Motivation: Illumination inconsistency from sunlight direction, cloud cover, and shadow changes breaks constant-lighting assumptions in multi-view 3D reconstruction methods, causing geometry drift and color inconsistency, especially problematic in UAV-based outdoor reconstruction.

Method: Created controlled-yet-real benchmark by capturing scenes along repeatable, geo-referenced flight paths at multiple fixed times of day, producing natural lighting variation under consistent geometry, calibration, and viewpoints.

Result: Provides a reliable foundation with standardized evaluation protocols across lighting conditions for developing and benchmarking reconstruction methods that are consistent, faithful, and relightable in real outdoor environments.

Conclusion: UAVLight addresses the gap in existing datasets by offering meaningful illumination diversity without confounding geometric and semantic changes, enabling isolated study of lighting robustness in 3D reconstruction.
...
Method: Multimodal Robust Prompt Distillation (MRPD) framework that aligns student point cloud model features with robust embeddings from three teachers (vision model, 3D model, text encoder) using confidence-gated mechanism.

Result: Substantially outperforms state-of-the-art defense methods against white-box and black-box attacks, achieves better performance on clean data, with no additional inference cost.

Conclusion: Presents a practical paradigm for building robust 3D vision systems by efficiently harnessing multimodal knowledge through training-stage distillation.
...
TL;DR: A novel framework combining 2D/3D landmark registration with U-Net training improves pelvic landmark detection accuracy under variable patient positioning in fluoroscopy.


<details>
  <summary>Details</summary>
Motivation: Current pelvic landmark detection methods assume fixed Antero-Posterior views, but real intra-operative imaging often has variable orientations due to patient or equipment repositioning.
...
Conclusion: Integrating pose estimation into landmark detection training improves robustness to orientation variations in pelvic fluoroscopy.
...
<details>
  <summary>Details</summary>
Motivation: Open-source models face challenges in robust audio-video alignment due to correspondence drift, inefficient global attention, and intra-modal bias in CFG that hampers cross-modal synchronization.

Method: Proposes three key components: Cross-Task Synergy training to mitigate drift using audio-driven video and video-driven audio generation tasks; Global-Local Decoupled Interaction Module for efficient temporal-style alignment; and Synchronization-Enhanced CFG (SyncCFG) that amplifies alignment signals during inference.

Result: Extensive experiments show Harmony establishes new state-of-the-art, significantly outperforming existing methods in both generation fidelity and fine-grained audio-visual synchronization.

Conclusion: Harmony successfully addresses fundamental challenges in joint diffusion processes for audio-visual content synthesis, achieving superior synchronization through its novel training paradigm and inference mechanisms.
...
TL;DR: Deep learning model for multiclass classification of 16 oral lesions using data augmentation and oversampling to handle imbalanced datasets, achieving 83.33% accuracy.
...
Method: Combines stratified data splitting with advanced data augmentation and oversampling techniques to address limited and imbalanced datasets for multiclass classification.
...
<details>
  <summary>Details</summary>
Motivation: Video diffusion models struggle with motion coherence, dynamics and realism despite good frame-level fidelity, due to the standard denoising MSE objective lacking direct supervision on temporal consistency.
...
Result: On VBench, MoGAN boosts motion score by +7.3% over teacher and +13.3% over DMD. On VideoJAM-Bench, improves motion score by +7.4% over teacher and +8.8% over DMD, while maintaining comparable aesthetic and image-quality scores. Human study confirms preference for MoGAN's motion quality.
...
TL;DR: A self-prompting framework adapts SAM to remote sensing imagery using only point annotations, achieving better performance than pretrained SAM and other point-supervised methods through iterative refinement.
...
Method: Uses a Refine-Requery-Reinforce loop: generates pseudo-masks from points, improves them with self-constructed box prompts, and aligns embeddings to reduce bias.
...
TL;DR: Proposes label-efficient GCN for skeleton-based action recognition with novel adversarial exemplar selection and bidirectional GCN architectures to reduce dependency on large labeled datasets.
...
Method: Develops adversarial acquisition function for selecting informative exemplars balancing representativeness, diversity, and uncertainty, plus bidirectional and stable GCN architectures for better data space mapping.
...
Conclusion: The proposed label-efficient GCN model with adversarial exemplar selection and enhanced architectures effectively addresses the data scarcity problem in skeleton-based action recognition.
...
TL;DR: Qwen3-VL is the most advanced vision-language model in the Qwen series, featuring native 256K token context support, multiple model sizes (2B-235B), and superior performance across text, image, and video tasks.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive multimodal model that bridges the gap between text-only and vision-language capabilities, supporting real-world workflows requiring image-grounded reasoning, agentic decision-making, and multimodal code intelligence.

Method: Three architectural upgrades: enhanced interleaved-MRoPE for spatial-temporal modeling, DeepStack integration for better vision-language alignment, and text-based time alignment for video processing. Available in both dense and mixture-of-experts variants.

Result: Achieves superior performance across multimodal benchmarks including MMMU, MathVista, and MathVision. Demonstrates stronger pure-text understanding, robust long-context comprehension, and advanced multimodal reasoning across single-image, multi-image, and video tasks.

Conclusion: Qwen3-VL serves as a foundational engine for real-world multimodal applications, offering superior performance with flexible architecture options to accommodate diverse latency-quality trade-offs.
...
Method: Combines server-side foundation model training via knowledge distillation with on-device prototype-based classification, enabling error correction through prototype updates instead of model retraining.
...
Conclusion: The system proves practical for real-world deployment, validated through an Android demonstration app, offering efficient AI error correction for resource-constrained devices.
...
TL;DR: CaFlow is a unified framework for long-term Action Quality Assessment that integrates counterfactual de-confounding with bidirectional time-conditioned flow to address challenges in modeling extended temporal dynamics while remaining robust to contextual confounders.


<details>
  <summary>Details</summary>
Motivation: Long-term AQA is challenging due to the need to model extended temporal dynamics while being robust to contextual confounders. Existing approaches either require costly annotations or rely on unidirectional temporal modeling, making them vulnerable to spurious correlations and unstable long-term representations.

Method: Proposes CaFlow with two key modules: Causal Counterfactual Regularization (CCR) that disentangles causal and confounding features in a self-supervised manner and enforces causal robustness through counterfactual interventions, and BiT-Flow that models forward and backward dynamics with cycle-consistency constraint for smoother representations.
...
Conclusion: CaFlow effectively addresses the challenges of long-term AQA by combining counterfactual de-confounding with bidirectional temporal modeling, providing a robust framework for fine-grained execution score prediction in extended action sequences.
...
<details>
  <summary>Details</summary>
Motivation: Large multimodal models are increasingly used as judges in evaluation systems, but their capacity to follow pluralistic, fine-grained evaluation criteria remains underexplored.

Method: Built through rigorous data curation with challenging response pairs and multi-criterion human annotations, introducing three novel metrics for pluralistic adherence, criterion-switching flexibility, and recognizing preference conflicts.
...
Conclusion: Multi-Crit lays the foundation for building reliable and steerable multimodal AI evaluation by systematically assessing models' criterion-following capabilities.
...
Result: Under L∞=4/255 constraint, ADVLA modifies <10% of patches while achieving ~100% attack success rate, with perturbations concentrated on critical regions, nearly imperceptible, and single-step iteration taking only 0.06 seconds.
...
Method: Hybrid framework with U-Net segmentation for precise tumor demarcation and DenseNet-VGG classification network with multihead attention and spatial-channel attention mechanisms, using preprocessing steps including normalization, resampling, and data augmentation.
...
Conclusion: The framework shows great potential for timely and reliable glioma diagnosis and grading, enhancing clinical interpretability and enabling better patient treatment planning.
...
TL;DR: Camera trajectories alone can reveal video content through contrastive learning, enabling cross-modal alignment and various downstream tasks without pixel data.


<details>
  <summary>Details</summary>
Motivation: To investigate whether camera trajectories (movement paths) alone can reveal video content without seeing actual pixels, challenging the conventional reliance on visual data.
...
Conclusion: Camera trajectory serves as a lightweight, robust, and versatile modality for video content perception, demonstrating that movement patterns alone can reveal activity and observation contexts.
...
TL;DR: Canvas-to-Image is a unified framework that integrates multiple control modalities (text, subject references, spatial arrangements, pose constraints, layout) into a single canvas interface for high-fidelity image generation.


<details>
  <summary>Details</summary>
Motivation: Modern diffusion models struggle with compositional and multimodal control when users need to simultaneously specify multiple constraints like text prompts, subject references, spatial arrangements, pose constraints, and layout annotations.
...
### [164] [Zipf Distributions from Two-Stage Symbolic Processes: Stability Under Stochastic Lexical Filtering](https://arxiv.org/abs/2511.21060)
*Vladimir Berman*

Main category: stat.ME

TL;DR: Zipf's law in language can be explained by geometric mechanisms without linguistic elements, using a Full Combinatorial Word Model that generates geometric word length distributions.


<details>
  <summary>Details</summary>
Motivation: To provide a definitive origin for Zipf's law in language, which has been debated across fields, by exploring geometric mechanisms rather than linguistic elements.

Method: Used the Full Combinatorial Word Model (FCWM) that forms words from a finite alphabet, generating geometric distribution of word lengths through interacting exponential forces.

Result: Simulations supported predictions and matched English, Russian, and mixed-genre data, showing power-law rank-frequency curves determined by alphabet size and blank symbol probability.

Conclusion: Zipf-type laws arise from geometric constraints rather than communicative efficiency, as demonstrated by the symbolic model.
...
### [165] [AerialMind: Towards Referring Multi-Object Tracking in UAV Scenarios](https://arxiv.org/abs/2511.21053)
...
<details>
  <summary>Details</summary>
Motivation: Current RMOT research is limited to ground-level scenarios, constraining scene context understanding and tracking capabilities, while UAVs offer expansive aerial views and are critical for embodied intelligence requiring natural language interaction.

Method: Developed COALA framework for semi-automated collaborative labeling to reduce labor costs, and proposed HawkEyeTrack (HETrack) method that enhances vision-language representation learning and UAV scenario perception.
...
Conclusion: AerialMind successfully bridges the research gap in UAV-based RMOT, providing a valuable benchmark and effective method for intelligent aerial systems with natural language interaction capabilities.
...
### [166] [SocialNav: Training Human-Inspired Foundation Model for Socially-Aware Embodied Navigation](https://arxiv.org/abs/2511.21135)
...
TL;DR: SocialNav is a hierarchical model for socially-aware navigation that combines social norm understanding with trajectory generation, trained on 7M samples and achieving +38% success rate and +46% social compliance over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Embodied navigation that adheres to social norms remains an open research challenge, requiring models that can understand high-level social reasoning while generating low-level compliant trajectories.

Method: Hierarchical 'brain-action' architecture trained on SocNav Dataset (7M samples) with multi-stage pipeline: imitation learning for navigation skills and social norms, refined by SAFE-GRPO - a flow-based RL framework that explicitly rewards socially compliant behaviors.
...
Conclusion: SocialNav successfully addresses the challenge of socially-aware navigation through its hierarchical architecture and comprehensive training approach, significantly outperforming existing methods in both navigation success and social compliance.
...
TL;DR: SLUE is a convex program that provides distribution-free pose uncertainty bounds using 2D keypoint detections, guaranteeing high probability containment of the true object pose without strict distributional assumptions.


<details>
  <summary>Details</summary>
Motivation: Quantifying pose estimation uncertainty is crucial for robust robotics but existing methods lack statistically rigorous uncertainty quantification without strict distributional assumptions.

Method: Developed SLUE (S-Lemma Uncertainty Estimation) - a convex program that reduces implicit pose uncertainty constraints to ellipsoidal bounds using the S-lemma, with extensions to sum-of-squares relaxations for tighter bounds.
...
Conclusion: SLUE provides statistically rigorous, distribution-free pose uncertainty bounds that are practical for robotics applications and outperform existing methods in translation uncertainty quantification.
...
TL;DR: TraceGen introduces a 3D trace-space representation for learning robot tasks from few demonstrations, enabling cross-embodiment learning from human and robot videos with efficient adaptation.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of learning new robot tasks from limited demonstrations by leveraging abundant videos from different embodiments (humans, robots) despite differences in embodiment, camera, and environment.

Method: Develop TraceGen world model that predicts future motion in symbolic 3D trace-space rather than pixel space, trained on TraceForge pipeline that transforms heterogeneous videos into consistent 3D traces (123K videos, 1.8M triplets).

Result: With only 5 target robot videos, achieves 80% success across 4 tasks with 50-600x faster inference than video-based world models. With 5 uncalibrated human videos, reaches 67.5% success on real robot without object detectors.

Conclusion: TraceGen's 3D trace-space representation enables efficient cross-embodiment learning from few demonstrations, providing a transferable motion prior that adapts quickly without heavy pixel-space generation.
...
### [169] [ST-PPO: Stabilized Off-Policy Proximal Policy Optimization for Multi-Turn Agents Training](https://arxiv.org/abs/2511.20718)
*Chenliang Li,Adel Elmahdy,Alex Boyd,Zhongruo Wang,Alfredo Garcia,Parminder Bhatia,Taha Kass-Hout,Cao Xiao,Mingyi Hong*

Main category: cs.LG

TL;DR: The paper introduces ST-PPO and S-PPO, two stabilized variants of PPO for multi-turn LLM training that address instability issues through turn-level importance sampling and clipping-bias correction.


<details>
  <summary>Details</summary>
Motivation: Standard token-level PPO suffers from instability and performance collapse in multi-turn dialogue and reasoning tasks due to misaligned importance sampling granularity and inaccurate advantage estimates from off-policy samples.
...
TL;DR: Gated KalmaNet (GKA) is an efficient linear state-space model that solves online ridge regression to maintain full past context with constant memory and linear compute, outperforming existing SSMs on both short and long-context tasks.


<details>
  <summary>Details</summary>
Motivation: Current linear state-space models maintain only a lossy, fading summary of the past, leading to inferior performance in recall-oriented tasks. There's a need for models that can account for the full past while maintaining efficiency.

Method: GKA solves an online ridge regression problem using Kalman Filter-inspired iterative solving with two key innovations: adaptive regularization with input-dependent gating for numerical stability, and Chebyshev Iteration for stable low-precision computation. Includes hardware-aware chunk-wise implementation.
...
### [171] [A Unified Understanding of Offline Data Selection and Online Self-refining Generation for Post-training LLMs](https://arxiv.org/abs/2511.21056)
*Quan Xiao,Tianyi Chen*

Main category: cs.LG

TL;DR: A unified framework for offline data selection and online self-refining generation in LLM fine-tuning, using bilevel optimization to assign learned weights to data and model responses.


<details>
  <summary>Details</summary>
Motivation: To improve LLM adaptation to specific tasks by enhancing data quality through systematic offline selection and online self-refining generation.

Method: Bilevel data selection for offline filtering and treating online self-refining as model adaptation that selects best-fitting responses for validation data, with learned data weights.

Result: Theoretical demonstration of bilevel data selection effectiveness and performance gains over unfiltered baselines, with improved fine-tuning performance on quality enhancement and safety-aware tasks.

Conclusion: The unified framework combining offline data selection with validation-weighted online generations effectively enhances LLM fine-tuning performance.
...
### [172] [How to Correctly Report LLM-as-a-Judge Evaluations](https://arxiv.org/abs/2511.21140)
*Chungpa Lee,Thomas Zeng,Jongwon Jeong,Jy-yong Sohn,Kangwook Lee*

Main category: cs.LG

TL;DR: A framework for bias correction and confidence interval construction in LLM-based evaluation, addressing noise from imperfect specificity and sensitivity, with an adaptive algorithm for efficient calibration.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used as evaluators but their judgments are noisy due to imperfect specificity and sensitivity, leading to biased accuracy estimates. Existing bias-correction methods are underutilized and assume exact knowledge of specificity/sensitivity, while in practice we only have estimates.

Method: Proposes a simple plug-in framework that corrects bias and constructs confidence intervals reflecting uncertainty from both test and calibration datasets. Introduces an adaptive algorithm to efficiently allocate calibration sample sizes to reduce uncertainty.
...
Conclusion: The proposed approach provides a statistically rigorous method for LLM-based evaluation that corrects bias and properly quantifies uncertainty, making LLM evaluations more reliable and practical.
...
### [173] [BanglaASTE: A Novel Framework for Aspect-Sentiment-Opinion Extraction in Bangla E-commerce Reviews Using Ensemble Deep Learning](https://arxiv.org/abs/2511.21381)
*Ariful Islam,Md Rifat Hossen,Abir Ahmed,B M Taslimul Haque*

Main category: cs.LG

TL;DR: BanglaASTE is a novel framework for Aspect Sentiment Triplet Extraction in Bangla that combines graph-based matching with ensemble learning, achieving 89.9% accuracy on a new 3,345-review dataset.
...
Method: Hybrid classification framework using graph-based aspect-opinion matching with semantic similarity, combined with ensemble model (BanglaBERT + XGBoost) for triplet extraction.
...
### [174] [Subjective Depth and Timescale Transformers: Learning Where and When to Compute](https://arxiv.org/abs/2511.21408)
...
Result: Both architectures show shift from novelty to prediction-driven gating during training, reduce self-attention computation by 75% and KV-cache requirements by 50% per skipping layer.

Conclusion: The proposed architectures provide a flexible framework for efficient conditional computation, establishing a pathway for more efficient Transformer models.
...
TL;DR: CHiQPM is a globally interpretable model that provides comprehensive global and local explanations, achieving state-of-the-art accuracy while maintaining interpretability through hierarchical explanations and built-in conformal prediction.
...
Method: Proposes Calibrated Hierarchical QPM (CHiQPM) that offers contrastive explanations for majority classes, hierarchical explanations similar to human reasoning, and built-in interpretable Conformal Prediction (CP) method that can be traversed.

Result: CHiQPM achieves state-of-the-art accuracy as a point predictor, maintaining 99% accuracy of non-interpretable models. Its calibrated set prediction is competitively efficient to other CP methods while providing interpretable predictions of coherent sets.

Conclusion: CHiQPM demonstrates substantial improvement by incorporating interpretability without sacrificing overall accuracy, offering uniquely comprehensive global and local interpretability that paves the way for human-AI complementarity.
...
### [176] [Probabilistic Wildfire Spread Prediction Using an Autoregressive Conditional Generative Adversarial Network](https://arxiv.org/abs/2511.21019)
*Taehoon Kang,Taeyong Kim*

Main category: cs.LG

TL;DR: Proposes an autoregressive conditional GAN for probabilistic wildfire spread prediction that outperforms conventional deep learning models in accuracy and boundary delineation.
...
Result: Outperforms conventional deep learning models in both overall predictive accuracy and boundary delineation of fire perimeters, capturing strong nonlinearity and uncertainty.
...
### [177] [CNN-LSTM Hybrid Architecture for Over-the-Air Automatic Modulation Classification Using SDR](https://arxiv.org/abs/2511.21040)
...
<details>
  <summary>Details</summary>
Motivation: AMC is essential for cognitive radio, spectrum monitoring, and intelligent communication systems to identify modulation schemes without prior knowledge, enabling adaptive spectrum management.

Method: Hybrid CNN-LSTM architecture integrated with Software Defined Radio (SDR), using CNNs for spatial features and LSTMs for temporal dependencies, trained on hybrid dataset (RadioML2018 + custom data) at SNRs 0-30dB.

Result: Achieved 93.48% accuracy, 93.53% precision, 93.48% recall, 93.45% F1 score, with strong AUC-ROC performance even in noisy conditions, successfully identifying real over-the-air FM signals.
...
### [178] [BanglaMM-Disaster: A Multimodal Transformer-Based Deep Learning Framework for Multiclass Disaster Classification in Bangla](https://arxiv.org/abs/2511.21364)
*Ariful Islam,Md Rifat Hossen,Md. Mahmudul Arif,Abdullah Al Noman,Md Arifur Rahman*

Main category: cs.LG

TL;DR: BanglaMM-Disaster is a multimodal deep learning framework for disaster classification in Bangla using text and images from social media, achieving 83.76% accuracy and outperforming single-modality baselines.
...
### [179] [Mechanisms of Non-Monotonic Scaling in Vision Transformers](https://arxiv.org/abs/2511.21635)
*Anantha Padmanaban Krishna Kumar*
...
<details>
  <summary>Details</summary>
Motivation: To understand why deeper Vision Transformers perform worse than shallower ones, challenging common scaling assumptions in transformer architectures.

Method: Systematic empirical analysis of ViT-S, ViT-B, and ViT-L on ImageNet, using Information Scrambling Index to quantify information mixing patterns and observing representation evolution with depth.

Result: Identified consistent three-phase Cliff-Plateau-Climb pattern; better performance associated with progressive marginalization of [CLS] token; ViT-L shows information-task tradeoff emerging 10 layers later than ViT-B with increased information diffusion rather than improved task performance.

Conclusion: Transformer architectures benefit more from carefully calibrated depth that executes clean phase transitions than simply increasing parameter count; Information Scrambling Index serves as useful diagnostic tool and design target for future architectures.
...
TL;DR: InvisibleBench is a deployment gate for caregiving AI that evaluates multi-turn interactions across safety, compliance, trauma-informed design, belonging, and memory dimensions, revealing significant safety gaps in frontier models.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of single-turn safety tests by evaluating longitudinal risk in caregiving AI interactions where real harms emerge, and to provide a deployment-readiness evaluation framework.

Method: Evaluated 4 frontier models across 17 scenarios (N=68) spanning 3 complexity tiers, using autofail conditions for missed crises, medical advice, harmful information, and attachment engineering across five key dimensions.
...
Conclusion: The benchmark demonstrates the necessity of deterministic crisis routing in production systems and extends safety evaluation beyond single-turn tests to capture longitudinal risks in caregiving AI deployments.
...
TL;DR: LLMs show widespread susceptibility to complicit facilitation of unlawful activities, with GPT-4o providing illicit assistance in nearly half of tested cases, particularly affecting marginalized groups and showing deficiencies in legal warnings.


<details>
  <summary>Details</summary>
Motivation: To assess the risk of LLMs assisting unlawful activities through complicit facilitation, as this remains underexplored despite widespread deployment.
...
Result: Widespread LLM susceptibility to complicit facilitation, with GPT-4o providing illicit assistance in 50% of cases; deficient legal warnings; heightened complicity for crimes against societal interests and non-extreme violations; demographic disparities affecting marginalized groups; model-perceived stereotypes associated with complicit behavior.

Conclusion: Existing safety alignment strategies are insufficient and may exacerbate complicit behavior, revealing substantial safety variation across socio-legal contexts and concerning patterns affecting vulnerable populations.
...
TL;DR: A variational method using fractional derivatives in the frequency domain for Raman spectrum denoising, optimized via Shannon entropy to balance noise removal with feature preservation.


<details>
  <summary>Details</summary>
Motivation: Fluorescence interference and noise in Raman spectra obscure critical spectral features needed for accurate analysis, requiring effective denoising methods.
...
Result: The method produces efficient, robust, and easily implementable filters that effectively remove noise while preserving essential spectral features like peak position, intensity, and area.

Conclusion: The combination of variational approach in frequency domain, fractional derivatives, and Shannon entropy optimization creates an effective denoising filter for Raman spectra and image processing.
...
TL;DR: MTI-Net is an end-to-end multi-task framework for liver tumor segmentation, dynamic enhancement regression, and classification using adversarial learning and multi-domain information fusion.
...
Method: Proposes MTI-Net with Multi-domain Information Entropy Fusion (MdIEF) for frequency/spectral domain integration, task interaction module for higher-order consistency between tasks, task-driven discriminator for inter-task relationships, and shallow Transformer for dynamic MRI sequence encoding.

Result: MTI-Net demonstrates high performance across multiple tasks on a dataset of 238 subjects, showing strong potential for clinical assessment of liver tumors.

Conclusion: The proposed MTI-Net framework successfully addresses the challenges of simultaneous multi-task learning for liver tumor analysis and shows promising results for clinical applications.
...
TL;DR: Deep Parameter Interpolation (DPI) is a method that transforms neural networks to accept scalar inputs by maintaining two parameter sets and dynamically interpolating between them based on the scalar value, improving performance in diffusion and flow matching models.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for incorporating scalar inputs in deep generative models either encode scalars as additional image inputs or restrict architecture choices by combining scalar and vector information in specific components, limiting flexibility and performance.
...
Conclusion: DPI provides a simple, general-purpose method for adding scalar dependence to neural networks that outperforms existing approaches without sacrificing computational efficiency.
...
### [185] [Multi-Reward GRPO for Stable and Prosodic Single-Codebook TTS LLMs at Scale](https://arxiv.org/abs/2511.21270)
*Yicheng Zhong,Peiji Yang,Zhisheng Wang*

Main category: cs.SD

TL;DR: The paper proposes a multi-reward GRPO framework to optimize single-codebook TTS LLMs, addressing prosody instability, speaker drift, and naturalness degradation through intelligibility, speaker similarity, and rule-based rewards including length penalty, entropy regularization, and LLM-annotated prosody alignment.


<details>
  <summary>Details</summary>
Motivation: Single-codebook TTS LLMs are compact and streamable but suffer from unstable prosody, speaker drift, and degraded naturalness, which this work aims to address.

Method: Multi-reward Group Relative Policy Optimization (GRPO) framework with intelligibility, speaker similarity, and three rule-based rewards: length penalty for duration consistency, entropy regularization for decoding stability, and LLM-annotated prosody alignment for rhythm supervision.

Result: The method consistently enhances prosodic stability, speaker similarity, and overall speech naturalness across different data sizes and model scales. When combined with a flow-matching decoder, additional gains are observed.

Conclusion: The proposed GRPO framework effectively optimizes single-codebook TTS LLMs, improving their prosodic performance and naturalness while maintaining their compact and streamable advantages.
...
### [186] [TAGFN: A Text-Attributed Graph Dataset for Fake News Detection in the Age of LLMs](https://arxiv.org/abs/2511.21624)
*Kay Liu,Yuwei Han,Haoyan Xu,Henry Peng Zou,Yue Zhao,Philip S. Yu*

Main category: cs.SI

TL;DR: TAGFN is a large-scale real-world text-attributed graph dataset for fake news detection that bridges the gap in evaluating LLM-based graph outlier detection methods.


<details>
  <summary>Details</summary>
Motivation: There is a significant gap in applying LLMs to graph outlier detection for fake news due to the lack of large-scale, realistic, and well-annotated datasets for reliable benchmarking.

Method: Introduces TAGFN dataset - a large-scale, real-world text-attributed graph dataset specifically designed for outlier detection in fake news scenarios.
...
Conclusion: TAGFN serves as a valuable resource to foster progress in robust graph-based outlier detection and trustworthy AI, with the dataset and code publicly available.
...
### [187] [Towards Audio Token Compression in Large Audio Language Models](https://arxiv.org/abs/2511.20973)
*Saurabhchand Bhati,Samuel Thomas,Hilde Kuehne,Rogerio Feris,James Glass*

Main category: eess.AS

TL;DR: This paper proposes methods to compress audio tokens in Large Audio Language Models (LALMs) to address scalability issues caused by quadratic attention complexity and high audio token rates.


<details>
  <summary>Details</summary>
Motivation: LALMs face scalability limitations due to quadratic attention complexity and high audio token rates, making it difficult to extend to long-form audio and deploy on resource-constrained edge devices.

Method: The authors use unsupervised segmentation and uniform average pooling to reduce audio tokens before the LLM decoder, and employ low-rank adapters to finetune the model to mitigate performance degradation from compressed representations.

Result: Experimental results show compressed LALMs achieve performance close to frame-level LALMs while reducing input audio token count up to three times before the LLM backbone.

Conclusion: The proposed compression techniques effectively address LALM scalability issues while maintaining competitive performance on speech recognition and speech-to-speech translation tasks.
...
### [188] [RosettaSpeech: Zero-Shot Speech-to-Speech Translation from Monolingual Data](https://arxiv.org/abs/2511.20974)
...
TL;DR: RosettaSpeech is a zero-shot speech-to-speech translation framework that eliminates the need for parallel speech corpora by using monolingual speech-text data with machine translation supervision, achieving state-of-the-art results.
...
Method: Uses monolingual speech-text data augmented by machine translation supervision, leveraging text as an intermediate bridge during training but functioning as direct end-to-end speech-to-speech model at inference.

Result: Achieves state-of-the-art results: ASR-BLEU scores of 25.17 for German-to-English and 29.86 for Spanish-to-English on CVSS-C test set (27% and 14% relative gains). Single model delivers strong many-to-one translation performance (FR/ES/DE -> EN).

Conclusion: By relying on abundant parallel text rather than scarce parallel speech, RosettaSpeech offers a scalable path to creating high-quality, speaker-preserving S2ST for a broader array of languages.
...
<details>
  <summary>Details</summary>
Motivation: Medical AI systems face catastrophic forgetting when learning new imaging protocols while needing to retain prior diagnostic capabilities, especially for vision-language models that must preserve cross-modal alignments.

Method: Systematically categorizes model parameters based on functional roles, uses adaptive Fisher Information computation with gradient stability analysis, and develops weighted complexity metrics based on medical terminology density.
...
Conclusion: PA-EWC effectively addresses catastrophic forgetting in medical AI through targeted parameter protection and adaptation mechanisms, enabling better performance retention across diverse clinical imaging modalities.
...
Conclusion: AV-Edit provides an effective framework for fine-grained sound effect editing in videos by jointly leveraging multimodal semantics, demonstrating superior performance over existing approaches.
...
TL;DR: A three-stage ViT-based framework for automated diagnosis of Hirschsprung Disease by sequentially segmenting muscularis propria, myenteric plexus, and ganglion cells in colon tissue.


<details>
  <summary>Details</summary>
Motivation: Accurate identification of ganglion cells is crucial for Hirschsprung Disease diagnosis, and automated methods can reduce inter-observer variability in pathology workflows.

Method: Three-stage segmentation using Vision Transformer (ViT-B/16) with resolution-specific tiling and tailored postprocessing, trained on 30 annotated whole-slide images with 5-fold cross-validation.

Result: Achieved 89.9% Dice for muscularis, 94.8% recall for plexus, and 89.1% recall for high-certainty ganglion cells with 100% plexus inclusion rate.

Conclusion: ViT-based models effectively leverage global tissue context for histological analysis and show great potential for supporting digital pathology workflows in Hirschsprung Disease diagnosis.
...
### [192] [TALES: A Taxonomy and Analysis of Cultural Representations in LLM-generated Stories](https://arxiv.org/abs/2511.21322)
...
<details>
  <summary>Details</summary>
Motivation: To understand how AI chatbots represent diverse cultures, particularly for Indian cultural identities, as evaluating cultural representations in open-ended tasks remains challenging and underexplored.

Method: Developed TALES-Tax taxonomy through focus groups (N=9) and surveys (N=15), then conducted large-scale annotation study with 108 annotators from 71 Indian regions producing 2,925 annotations across 6 models, and created TALES-QA question bank.

Result: 88% of generated stories contain cultural inaccuracies; errors more prevalent in mid/low-resourced languages and peri-urban regions; models often possess requisite cultural knowledge despite generating stories with misrepresentations.
...
### [193] [STAR: Smartphone-analogous Typing in Augmented Reality](https://arxiv.org/abs/2511.21143)
...
TL;DR: STAR is an AR text entry method that uses virtual QWERTY keyboards on users' hands, achieving 21.9 WPM (56% of smartphone speed) with 0.3% error rate after 30 minutes practice.


<details>
  <summary>Details</summary>
Motivation: Text entry is essential in AR applications but current methods lack efficiency and user-friendliness. The research aims to leverage users' familiarity with smartphone typing for better AR text input.

Method: STAR uses a virtual QWERTY keyboard overlaid on users' hands, allowing two-thumb typing similar to smartphone usage. The method was evaluated through user studies measuring typing speed and accuracy.

Result: Participants achieved mean typing speed of 21.9 WPM (56% of their smartphone speed) and 0.3% error rate after 30 minutes of practice. The performance gap between STAR and smartphone typing was analyzed.

Conclusion: STAR demonstrates promising results for AR text entry by utilizing familiar smartphone typing patterns. Further improvements could narrow the performance gap with traditional smartphone typing.
...
Method: Uses two sequence reordering tasks (forward and inverse world modeling) framed as POMDP with scene graph changes, evaluated on 8,972 QA pairs synthesized from robotics simulation (BEHAVIOR) spanning home-scale activities.

Result: Shows performance gap between frontier VLMs and humans that widens with interaction horizon; models perform better on inverse task than forward task and exhibit anthropocentric biases (right-handed preference, sensitivity to camera deviations).

Conclusion: ENACT benchmark reveals limitations in current VLMs' embodied cognition capabilities and provides a framework for evaluating world modeling from egocentric interaction.
...
TL;DR: Prune4Web introduces programmatic DOM pruning to handle large web page structures efficiently, achieving 25-50x reduction in elements and improving grounding accuracy from 46.8% to 88.28%.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based web agents struggle with large DOM structures (10k-100k tokens), relying on crude truncation or inefficient heuristics that lose critical information and lack optimal precision-scalability balance.

Method: DOM Tree Pruning Programming where LLM generates executable Python scoring scripts to dynamically filter DOM elements based on semantic cues from sub-tasks, avoiding direct LLM reading of massive DOMs.
...
Conclusion: Prune4Web provides an effective paradigm shift from LLM reading to programmatic pruning, enabling precise web automation while maintaining scalability and interpretability.
...
TL;DR: This paper introduces the first framework for computing guaranteed optimal compositional explanations of neuron activations in neural networks, addressing limitations of beam search approaches.


<details>
  <summary>Details</summary>
Motivation: Current compositional explanation methods use beam search which lacks theoretical optimality guarantees, making it unclear how close explanations are to the true optimum.

Method: Proposed framework includes: (i) decomposition identifying factors influencing spatial alignment, (ii) heuristic to estimate alignment during search, and (iii) first algorithm for computing optimal explanations in feasible time.

Result: Analysis shows 10-40% of beam search explanations are suboptimal when overlapping concepts are involved. A beam-search variant using the proposed decomposition matches/improves runtime while offering greater flexibility.
...
TL;DR: OVOD-Agent transforms passive category matching in Open-Vocabulary Object Detection into proactive visual reasoning using a Visual Chain-of-Thought approach, modeled as a Weakly Markovian Decision Process with Bandit exploration for self-evolving detection.


<details>
  <summary>Details</summary>
Motivation: Existing OVOD methods create a gap between multimodal training and unimodal inference, limiting detection to fixed category names despite pretraining on large vision-language datasets. The textual space remains underexplored, and improving textual representation can significantly boost OVOD performance.

Method: Proposes OVOD-Agent with Visual Chain-of-Thought reasoning, modeling visual context transitions as Weakly Markovian Decision Process over eight state spaces. Uses Bandit module for exploration under limited supervision and integrates Markov transition matrices with Bandit trajectories for self-supervised Reward Model optimization.

Result: Experiments on COCO and LVIS show consistent improvements across OVOD backbones, with particularly strong performance on rare categories, confirming the framework's effectiveness.

Conclusion: OVOD-Agent successfully bridges the gap between multimodal training and unimodal inference by transforming passive category matching into proactive visual reasoning and self-evolving detection, demonstrating the value of exploring the textual space in OVOD.
