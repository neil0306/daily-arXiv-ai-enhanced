...
TL;DR: Automatic methods for attribution in summarization, using hybrid summarization (paraphrased extractive summaries) and custom topology to analyze attribution errors.


<details>
  <summary>Details</summary>
Motivation: Analysts need attribution to report information sources, requiring automatic methods to link summary sentences to source text portions.

Method: Hybrid summarization combining extractive and paraphrasing approaches, with custom topology for error analysis.
...
TL;DR: GMTRouter is a personalized LLM routing system that uses heterogeneous graph learning to capture user preferences from few-shot data, achieving better performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LLM routing approaches lack full personalization and fail to capture complex user-LLM interactions. User preference data is often scarce, noisy, and inconsistent, limiting effectiveness of user-specific methods.

Method: Represent multi-turn user-LLM interactions as a heterogeneous graph with four node types (user, LLM, query, response) and use tailored message-passing mechanism within a lightweight inductive graph learning framework.

Result: GMTRouter outperforms strong baselines with 0.9-21.6% higher accuracy and 0.006-0.309 higher AUC across multiple datasets. It adapts to new users and evolving preferences using few-shot data without extensive fine-tuning.

Conclusion: GMTRouter effectively addresses personalization challenges in LLM routing through graph-based learning, demonstrating strong performance and adaptability with minimal data requirements.
...
TL;DR: LLMs can generate social media conversations that are mistaken for human content 39% of the time, demonstrating both potential for social simulation and risks of misuse.


<details>
  <summary>Details</summary>
Motivation: To test whether LLMs can convincingly mimic human group conversations on social media, as their validity for simulating online communities remains largely untested.

Method: Collected real human conversations from Reddit and generated artificial conversations on same topics using Llama 3 70B and GPT-4o, then presented them side-by-side to study participants for identification.

Result: LLM-generated conversations were mistaken for human content 39% of the time; Llama 3 conversations were correctly identified as AI-generated only 56% of the time (barely better than random chance).
...
TL;DR: LLMs show promise for legal analysis but have dangerous vulnerabilities in generating unsafe bioweapon instructions despite safeguards. The paper proposes a knowledge graph + RAG methodology to evaluate LLM understanding of bioweapons law, legal intent assessment, and safety risks.


<details>
  <summary>Details</summary>
Motivation: To address the contradiction where LLMs can analyze complex legal frameworks like bioweapons laws but also demonstrate alarming vulnerabilities in generating unsafe outputs, creating a need for systematic evaluation of their legal reasoning and safety mechanisms.

Method: Proposes integrating knowledge graph construction with Retrieval-Augmented Generation (RAG) to systematically evaluate LLMs' understanding of bioweapons law, capacity to assess legal intent (mens rea), and potential for unsafe applications through structured experiments.

Result: Findings reveal significant limitations in LLMs' reasoning and safety mechanisms, showing they struggle with accurate legal violation identification, prohibited instruction generation, and unlawful intent detection in bioweapons scenarios.

Conclusion: Enhanced safety protocols combined with robust legal reasoning frameworks can develop LLMs that ethically assist in sensitive legal domains, ensuring they protect rather than inadvertently violate the law.
...
TL;DR: Soft Preference Learning addresses LLM diversity loss from alignment algorithms by decoupling KL divergence terms, improving both capabilities and viewpoint representation.


<details>
  <summary>Details</summary>
Motivation: Alignment algorithms like RLHF and DPO reduce LLM output diversity, making models generate repetitive text and reflect narrower societal perspectives due to KL divergence regularizer overweighting majority opinions.

Method: Proposes Soft Preference Learning that decouples entropy and cross-entropy terms in KL penalty, allowing fine-grained control over generation diversity while resembling temperature scaling.

Result: Models trained with Soft Preference Learning achieve higher accuracy on difficult tasks, produce more semantically and lexically diverse outputs, represent wider societal viewpoints, and show improved logit calibration.

Conclusion: Soft Preference Learning is a Pareto improvement over standard temperature scaling that successfully addresses diversity loss in aligned LLMs while maintaining or improving performance.
...
<details>
  <summary>Details</summary>
Motivation: Tree-of-Thought reasoning enhances LLM problem-solving but suffers from high computational costs due to semantic redundancy where multiple branches explore equivalent reasoning paths.
...
Conclusion: SSDP provides a practical approach for efficient and scalable LLM reasoning by significantly reducing computational overhead while preserving reasoning quality.
...
TL;DR: A framework for stress testing LLM factual fidelity against adversarial nudges, revealing varying resilience levels across different models.
...
Method: Three-step framework: 1) Generate truths and lies within closed domains, 2) Verify assertions as truths/lies, 3) Test LLM robustness against self-generated lies.

Result: Evaluation across 5 LLMs in movie/novel domains showed: Claude (strong resilience), GPT/Grok (moderate), Gemini/DeepSeek (weak resilience).

Conclusion: Findings raise alarm about LLM reliability for information seeking, highlighting significant variations in adversarial nudge susceptibility across models.
...
TL;DR: Self-HarmLLM: Using a model's own mitigated harmful queries as new attack vectors to bypass guardrails, achieving up to 65% transformation and 41% jailbreak success rates.


<details>
  <summary>Details</summary>
Motivation: Existing defenses assume external attackers, but a model's own output could become an attack vector. Need to explore this overlooked vulnerability in LLM guardrails.

Method: Proposed Self-HarmLLM scenario using Mitigated Harmful Queries (MHQs) - ambiguous queries preserving original harmful intent but not directly exposed. Tested on GPT-3.5, LLaMA3, DeepSeek under Base, Zero-shot, and Few-shot conditions with automated and human evaluation.

Result: Up to 52% transformation and 33% jailbreak success in Zero-shot; up to 65% transformation and 41% jailbreak in Few-shot. Automated evaluation overestimated jailbreak success by average 52% compared to human evaluation.

Conclusion: Self-HarmLLM proves valid as attack scenario, revealing fundamental guardrail design flaws. Need robust evaluation methodology beyond automated metrics alone.
...
<details>
  <summary>Details</summary>
Motivation: Static knowledge benchmarks fail to capture evolving world knowledge and struggle to keep pace with rapid LLM advancements, creating a need for dynamic evaluation methods.

Method: An agentic framework that automates sourcing, creation, validation, and distribution of knowledge benchmarks using news domain data, with focus on reducing pretraining data overlap.

Result: Evaluation on various LLMs revealed distinct behaviors when handling new information and showed that retrieval-augmented methods narrow performance gaps between small and large models.

Conclusion: Dynamic knowledge benchmarks are essential for proper LLM evaluation, and retrieval augmentation helps models better handle evolving information.
...
Method: Multi-model RAG-based system integrating curated domain knowledge with engineered prompt templates, tested with five commercial and open-source LLMs across seven diverse pediatric disorder scenarios.

Result: Technical feasibility demonstrated for RAG-augmented generation of pediatric SLP vignettes. Commercial models showed marginal quality advantages, but open-source alternatives achieved acceptable performance suitable for privacy-preserving institutional deployment.

Conclusion: The system successfully generated clinically appropriate content aligned with professional guidelines. Future validation through expert review and testing needed before educational implementation, with potential extensions to clinical decision support and automated IEP goal generation.
...
Conclusion: The framework has constraints in translation despite prior claims, highlighting need for future improvements to handle language structural variations.
...
TL;DR: Mina is a multilingual LLM-based legal assistant for Bangladesh that uses RAG and chain-of-tools to provide legal advice, scoring 75-80% on bar exams and matching human performance.


<details>
  <summary>Details</summary>
Motivation: Address barriers to affordable legal advice in Bangladesh due to complex language, procedural opacity, and high costs, with existing AI lacking Bengali support and jurisdiction-specific adaptation.

Method: Uses multilingual embeddings and RAG-based chain-of-tools framework for retrieval, reasoning, translation, and document generation via interactive chat interface.

Result: Scored 75-80% in Bangladesh Bar Council Exams across MCQs, Written, and Viva Voce, matching or surpassing average human performance with clear contextual understanding and legal reasoning.

Conclusion: Demonstrates potential as low-cost, multilingual AI assistant that automates legal tasks and scales access to justice, offering case study for domain-specific, low-resource systems and multilingual adaptation challenges.
...
TL;DR: A super-learner system called MEDAS integrates five major LLMs (Gemini, Llama, Grok, GPT, Claude) to improve diagnostic accuracy in emergency medicine, achieving 70% accuracy compared to individual LLMs (58-65%) and exceeding human doctor performance.


<details>
  <summary>Details</summary>
Motivation: To enhance medical decision-support systems in emergency medicine by leveraging the collective capabilities of multiple LLMs to improve diagnostic accuracy beyond what individual models or human doctors can achieve.

Method: Built a super-learner system (MEDAS) that integrates five major LLMs using a meta-learner approach to learn and leverage the different diagnostic capabilities of each LLM in the cluster.

Result: Individual LLMs achieved 58-65% diagnostic accuracy, while the super-learner achieved 70% accuracy. At least one integrated LLM achieved 85% correct diagnoses, showing the potential of collective capabilities.

Conclusion: Meta-learning approaches that aggregate multiple LLMs can significantly exceed the diagnostic accuracy of individual models by leveraging combined knowledge from different medical datasets used to train the LLM cluster.
...
TL;DR: GrADS is a self-adaptive gradient-aware data selection method that uses gradient analysis to select optimal subsets for supervised fine-tuning, achieving better performance with only 5-50% of data while mitigating catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Supervised fine-tuning for domain specialization is resource-intensive and causes catastrophic forgetting of general capabilities in LLMs, creating a need for more efficient and effective fine-tuning approaches.

Method: Analyzes gradients from preliminary training to identify effective data subsets using self-guided criteria based on gradient magnitude and statistical distribution, prioritizing examples that contribute most to learning.

Result: With only 5% of selected GrADS data, LLMs surpass performance of full dataset fine-tuning; 50% data yields significant improvements across medicine, law, and finance domains while substantially mitigating catastrophic forgetting.

Conclusion: GrADS provides an efficient, cost-effective approach for domain specialization of LLMs that improves performance with minimal data while preventing catastrophic forgetting through intelligent data selection.
...
TL;DR: Proposes a hybrid CNN-BiGRU model with SHAP explainability for detecting suicidal ideation in social media, achieving 93.97% accuracy and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Suicide is the second leading cause of death for adolescents, and social media posts often contain signals of suicidal intentions that can be detected to enable early intervention.

Method: Combines CNN for local feature extraction with BiGRU for bidirectional sequence modeling, enhanced by attention mechanisms and SHAP for explainable AI interpretations.
...
Conclusion: The hybrid CNN-BiGRU framework with explainable AI provides an effective and reliable solution for suicide detection from social media data, demonstrating superior performance over existing approaches.
...
TL;DR: SAGE-Agent uses structured uncertainty modeling to efficiently clarify ambiguous tool-call parameters, achieving higher task coverage with fewer questions through POMDP-based question selection and aspect-based cost modeling.


<details>
  <summary>Details</summary>
Motivation: LLM agents often fail tasks due to incorrect tool invocations from ambiguous user instructions, requiring a principled approach to parameter clarification.

Method: Formulate structured uncertainty over tool-call parameters as a POMDP with EVPI objective for optimal question selection, using aspect-based cost modeling to prevent redundant clarifications.

Result: Increases coverage on ambiguous tasks by 7-39% while reducing clarification questions by 1.5-2.7× compared to baselines. Also boosts When2Call accuracy from 36.5% to 65.2% (3B model) through uncertainty-weighted GRPO training.
...
TL;DR: This study developed NLP models to extract cognitive process categories from Parkinson's disease patients' narratives, finding that fine-tuned Meta-Llama-3-8B-Instruct performed best overall, while Bio_ClinicalBERT showed high precision but low recall.


<details>
  <summary>Details</summary>
Motivation: Understanding how Parkinson's disease patients describe cognitive experiences in daily life can provide insights into disease-related cognitive and emotional changes, but extracting this information from unstructured narratives is challenging due to subtle, overlapping cognitive constructs.
...
Result: Model performance varied substantially across categories and model families. Fine-tuned Meta-Llama-3-8B-Instruct achieved highest overall F1-scores (0.74 micro-average, 0.59 macro-average), excelling in context-dependent categories like thought and social interaction. Bio_ClinicalBERT showed high precision but low recall, performing comparably to Llama for some categories but failing on others.

Conclusion: The task presents greater challenges than conventional information extraction due to abstract and overlapping narrative accounts of cognitive processes, but with continued refinement, these NLP systems hold promise for low-burden longitudinal monitoring of cognitive function and complementing formal neuropsychological assessments in Parkinson's disease.
...
TL;DR: BNLI is a refined Bengali NLI dataset addressing inconsistencies in existing resources, featuring rigorous annotation and balanced semantic classes to support robust language understanding.


<details>
  <summary>Details</summary>
Motivation: Existing Bengali NLI datasets have annotation errors, ambiguous sentence pairs, and inadequate linguistic diversity, hindering effective model training and evaluation in this low-resource language.

Method: Constructed through a rigorous annotation pipeline emphasizing semantic clarity and balance across entailment, contradiction, and neutrality classes, then benchmarked with state-of-the-art transformer models.
...
Conclusion: BNLI addresses critical limitations in Bengali NLI resources and provides a reliable benchmark for advancing research in Bengali and other low-resource language inference tasks.
...
TL;DR: TACT dataset enables unified modeling of task-oriented and chitchat dialogues with fluid transitions, achieving 75.74% joint accuracy and 70.1% win rate against GPT-4o through DPO optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional conversational agents are developed separately for task-oriented dialogue or chitchat, but real-world conversations naturally involve fluid transitions between these modes, creating a gap in unified modeling.

Method: Introduced TACT dataset with structurally diverse mode flows supporting both user- and agent-driven transitions, and applied Direct Preference Optimization (DPO) to enhance transition handling.

Result: Models trained on TACT outperform baselines in intent detection and mode transitions, achieving 75.74% joint mode-intent accuracy and 70.1% win rate against GPT-4o in human evaluation.
...
TL;DR: BioVerge introduces a benchmark and LLM agent framework for biomedical hypothesis generation, addressing limitations of traditional methods by leveraging structured/textual data and self-evaluation to improve novelty and relevance.


<details>
  <summary>Details</summary>
Motivation: Current biomedical hypothesis generation methods rely on single data types or predefined patterns, limiting discovery of novel connections. LLM agents show potential but lack standardized datasets and environments.

Method: BioVerge Agent uses ReAct-based approach with Generation and Evaluation modules that iteratively produce and self-assess hypotheses. Combines structured/textual data from historical biomedical hypotheses and PubMed literature.

Result: Experiments show: 1) different agent architectures affect exploration diversity and reasoning strategies; 2) structured and textual data provide unique critical contexts; 3) self-evaluation significantly improves hypothesis novelty and relevance.

Conclusion: BioVerge provides a standardized environment for biomedical hypothesis generation, demonstrating that LLM agents with self-evaluation and diverse data sources can effectively generate novel and relevant scientific hypotheses.
...
TL;DR: LLMs' citation hallucination depends on citation frequency - highly cited papers (1000+ citations) are memorized verbatim with minimal hallucination, while less cited papers show higher hallucination rates due to generation rather than memorization.


<details>
  <summary>Details</summary>
Motivation: To understand how citation frequency affects LLMs' tendency to hallucinate non-existent papers in bibliographic outputs, and investigate the relationship between training data redundancy and factual accuracy.

Method: Used GPT-4.1 to generate 100 citations across 20 computer-science domains, manually verified them, and measured factual consistency using cosine similarity between generated and authentic metadata.

Result: Citation count strongly correlates with factual accuracy; bibliographic information becomes almost verbatim memorized beyond ~1000 citations; memory interference occurs when multiple highly cited papers share similar content.

Conclusion: There's a threshold where generalization shifts into memorization in LLMs, with highly cited papers being nearly verbatim retained in the model, explaining citation hallucination patterns.
...
TL;DR: HalluClean is a lightweight, task-agnostic framework that detects and corrects hallucinations in LLM-generated text using a reasoning-enhanced paradigm with planning, execution, and revision stages, achieving improved factual consistency across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs often produce hallucinated content that undermines factual reliability, creating a need for methods to detect and correct these hallucinations to improve trustworthiness in real-world applications.

Method: Uses a reasoning-enhanced paradigm with planning, execution, and revision stages; employs minimal task-routing prompts for zero-shot generalization across domains; does not rely on external knowledge sources or supervised detectors.

Result: Significantly improves factual consistency and outperforms competitive baselines across five representative tasks: question answering, dialogue, summarization, math word problems, and contradiction detection.
...
TL;DR: TiDAR is a hybrid architecture that combines diffusion models for parallel token drafting and autoregressive models for final sampling in a single forward pass, achieving AR-level quality with 4.71x-5.91x higher throughput.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between diffusion models' parallel generation capability and autoregressive models' superior quality, creating a solution that offers both high throughput and AR-level performance.

Method: TiDAR uses structured attention masks to enable parallel token drafting with diffusion models and autoregressive sampling in a single forward pass, exploiting GPU compute density while maintaining serving efficiency.

Result: TiDAR outperforms speculative decoding in throughput and surpasses diffusion models in efficiency and quality, achieving 4.71x-5.91x more tokens per second while matching AR model quality.

Conclusion: TiDAR successfully closes the quality gap with AR models while significantly improving generation throughput, representing the first architecture to achieve this balance.
...
TL;DR: EVADE framework uses LLMs to generate and validate explanations for error detection in NLI tasks, outperforming human annotation in fine-tuning performance while reducing costs.


<details>
  <summary>Details</summary>
Motivation: Human label variation in NLI makes it hard to distinguish annotation errors from valid label differences, and existing two-round human annotation frameworks are costly and limited in coverage.

Method: Proposed EVADE framework uses LLMs to generate explanations and validate errors, comparing human- and LLM-detected errors across distribution analysis, validation overlap, and fine-tuning impact.
...
Conclusion: LLMs can scale error detection effectively, reducing human effort while improving dataset quality under label variation scenarios.
...
TL;DR: The paper introduces a trajectory-based evaluation framework for assessing LLMs' emotional support capabilities, moving beyond static dialogues to track user emotional dynamics over time using psychologically-grounded strategies and novel metrics.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of LLMs for emotional support rely on short, static dialogues and fail to capture the dynamic, long-term nature of emotional support interactions, which is crucial for applications like psychological counseling and companionship.

Method: Developed a large-scale benchmark with 328 emotional contexts and 1,152 disturbance events, constrained model outputs using validated emotion regulation strategies, modeled user emotional trajectories as a first-order Markov process, and applied causally-adjusted emotion estimation for unbiased tracking.

Result: Extensive evaluations revealed significant disparities in emotional support capabilities across different LLMs, demonstrating the framework's effectiveness in capturing long-term emotional support performance.

Conclusion: The proposed trajectory-based assessment framework provides comprehensive evaluation of LLMs' emotional support capabilities and offers actionable insights for model development in this critical domain of human-AI interaction.
...
TL;DR: A neurosymbolic framework that combines LLMs with formal logic to validate natural language statements against policies with high accuracy and auditability.


<details>
  <summary>Details</summary>
Motivation: LLMs' stochastic nature limits their use in regulated industries requiring strict policy adherence; this framework addresses the need for reliable logical validation.

Method: Two-stage approach: (1) formalize natural language policies using LLMs with human guidance, (2) use inference-time autoformalization with redundant formalization steps and cross-checking for semantic equivalence.

Result: Achieves over 99% soundness with near-zero false positive rate in identifying logical validity.

Conclusion: The approach provides auditable logical artifacts for verification outcomes and can improve original text, making LLMs more suitable for regulated domains.
...
TL;DR: MM-CRITIC is a comprehensive benchmark for evaluating multimodal critique abilities of Large Multimodal Models across basic, correction, and comparison dimensions, covering 8 task types and over 500 tasks with 4471 samples.


<details>
  <summary>Details</summary>
Motivation: Multimodal critique ability is vital for model self-improvement and reliability, but remains underexplored compared to language-only settings, despite growing LMM capabilities in tasks like captioning and visual reasoning.

Method: Created MM-CRITIC benchmark with expert-informed ground answers and scoring rubrics to guide GPT-4o in annotating responses and generating reference critiques for reliable evaluation across multiple dimensions.

Result: Extensive experiments validated MM-CRITIC's effectiveness and provided comprehensive assessment of leading LMMs' critique capabilities, revealing correlations between response quality and critique, and varying critique difficulty across dimensions.

Conclusion: MM-CRITIC offers a reliable benchmark for evaluating multimodal critique abilities, enabling better understanding and improvement of LMMs' critical evaluation capabilities in multimodal settings.
...
TL;DR: Proposes a streaming speech recognition framework for Amdo Tibetan using hybrid CTC/Attention with dynamic chunking, achieving 6.23% WER with 48.15% improvement over baseline.


<details>
  <summary>Details</summary>
Motivation: To address context truncation problems in fixed-chunk methods and adapt to Amdo Tibetan's linguistic characteristics and varying speaking rates.

Method: Hybrid CTC/Attention architecture with context-aware dynamic chunking mechanism, Tibetan lexicon construction, and external language model integration during decoding.

Result: Achieved 6.23% word error rate on test set, 48.15% relative improvement over fixed-chunk baseline, reduced latency while maintaining near-global decoding performance.

Conclusion: The proposed framework effectively handles Amdo Tibetan speech recognition with adaptive chunking, linguistic modeling, and language model integration for improved accuracy and efficiency.
...
TL;DR: Bi-RAR is a novel retrieval-augmented reasoning framework that evaluates intermediate reasoning steps bidirectionally using information distance metrics to improve multi-step reasoning in complex QA tasks.


<details>
  <summary>Details</summary>
Motivation: Current RAG methods struggle with complex multi-step reasoning and often suffer from reward hacking due to outcome-based supervision that provides no explicit guidance for intermediate steps.

Method: Proposes bidirectional information distance based on Kolmogorov complexity to assess step completeness, and uses multi-objective reinforcement learning with cascading rewards to optimize reasoning trajectories.
...
Conclusion: Bi-RAR effectively addresses limitations of existing RAG approaches by providing bidirectional evaluation of reasoning steps, leading to improved performance in complex reasoning scenarios.
...
TL;DR: This paper evaluates LLMs' humor capabilities using Japanese Oogiri comedy games, finding they perform at low-mid human level but lack empathy, which explains their inability to replicate human humor assessment.


<details>
  <summary>Details</summary>
Motivation: Previous humor evaluations of LLMs have been single-dimensional (just 'funny' or not), but a multifaceted understanding is needed for sophisticated dialogue systems.
...
Result: LLMs generate responses at low-mid human level but show notable lack of Empathy. Correlation analysis reveals LLMs prioritize Novelty while humans prioritize Empathy in humor evaluation.

Conclusion: The empathy deficit in LLMs explains their failure to replicate human humor assessment, and the annotated corpus is released to develop more emotionally intelligent conversational agents.
...
TL;DR: This paper presents a personalized English reading comprehension test generation system that creates interest-aligned passages and questions using AI, showing improved student engagement and comprehension in EFL learners.
...
Method: Developed a structured content transcreation pipeline using GPT-4o, starting with RACE-C dataset, integrating topic extraction, Bloom's taxonomy question classification, linguistic feature analysis, and content transcreation to generate personalized passages and questions.

Result: EFL learners using personalized reading passages showed improved comprehension and motivation retention compared to those using non-personalized materials in a controlled experiment with South Korean students.

Conclusion: Personalized, interest-aligned reading materials effectively enhance both comprehension and motivation in EFL education, demonstrating the value of AI-driven content adaptation for personalized learning.
...
TL;DR: DoPE is a training-free method that improves length extrapolation in Transformers by detecting outlier frequency bands in positional embeddings using truncated matrix entropy and reparameterizing with Gaussian distribution.


<details>
  <summary>Details</summary>
Motivation: Rotary Position Embedding (RoPE) has inherent limits that weaken length extrapolation, and attention sink phenomenon affects model performance on long sequences.

Method: Reinterpret attention map as noisy feature map, use truncated matrix entropy to detect outlier frequency bands, and reparameterize with parameter-free Gaussian distribution for robust extrapolation.
...
Conclusion: DoPE effectively mitigates attention sinks and restores balanced attention patterns, providing a simple yet powerful solution for improving length generalization without requiring training.
...
TL;DR: LoopTool is an automated framework that integrates data synthesis and model training in a closed-loop system to improve LLM tool learning by adaptively focusing on model weaknesses and correcting noisy labels.


<details>
  <summary>Details</summary>
Motivation: Current tool learning approaches use static data pipelines where data generation and model training are separate processes, failing to address specific model weaknesses and allowing noisy labels to persist, which degrades training efficiency.

Method: LoopTool uses three modules: Greedy Capability Probing to diagnose model capabilities, Judgement-Guided Label Verification to correct annotation errors, and Error-Driven Data Expansion to generate challenging samples based on identified failures.
...
Conclusion: Closed-loop, self-refining data pipelines can dramatically enhance LLM tool-use capabilities, demonstrating the effectiveness of adaptive data evolution frameworks.
...
TL;DR: Proposes a cell extraction method for Table Question Answering using hybrid retrieval (language model + TF-IDF) to identify relevant table headers and cells without manual intervention, achieving 74.6% accuracy.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with complex table structures in TQA tasks, often producing incorrect answers when tables are input as long text, necessitating a specialized approach for table structure understanding.

Method: Uses hybrid retrieval combining language model and TF-IDF to compute question-cell similarities, identifies relevant headers, selects cells at row-column intersections, and employs contrastive learning on question-header pairs.

Result: Achieved 74.6% accuracy on TQA dataset from NTCIR-18 U4 shared task, outperforming GPT-4o mini (63.9%) and other existing LLMs.

Conclusion: The proposed pipeline effectively handles complex table structures in TQA without manual header identification, with plans to incorporate more efficient text-search models to further improve performance and close the gap with human evaluation.
...
TL;DR: Context-based sequentiality (using only contextual terms) outperforms topic-based and original sequentiality measures in predicting human-assessed essay flow traits, and combines effectively with linguistic features to beat zero-shot LLM predictions.


<details>
  <summary>Details</summary>
Motivation: To empirically validate the proposal that using only contextual terms in sequentiality measurement is more conceptually valid and interpretable than the original topic-context combination, addressing concerns about topic selection confounding and lack of ground-truth validation.

Method: Used two essay datasets (ASAP++ and ELLIPSE) with human-annotated trait scores, comparing contextual sequentiality against topic-only and original sequentiality formulations, and testing combinations with standard linguistic features and zero-shot LLM predictions.

Result: Contextual sequentiality aligns more closely with human assessments of discourse-level traits like Organization and Cohesion. When combined with linguistic features, it outperforms both topic-only/original sequentiality and zero-shot LLM predictions.

Conclusion: Context-based sequentiality is a validated, interpretable, and complementary feature for automated essay scoring and NLP tasks, supporting explicit modeling of sentence-to-sentence flow.
...
TL;DR: Study of subword segmentation learning dynamics during pretraining and finetuning, showing how subwords evolve across languages with different morphological complexity.


<details>
  <summary>Details</summary>
Motivation: To understand how language models dynamically optimize tokenization during training and how subwords evolve across different morphological typologies.

Method: Extended subword segmental language model (SSLM) framework to support pretraining and finetuning, trained on three typologically diverse languages: Isi-Xhosa (conjunctive), Setswana (disjunctive), and English (middle ground). Analyzed subword dynamics through linguistic metrics.

Result: Identified four stages of subword learning, with morphologically complex Isi-Xhosa showing greater instability. During finetuning, subword boundaries shift to finer granularity. Learnable subwords improve text generation and cross-lingual transfer for low-resource languages.

Conclusion: Learnable subword segmentation offers promising benefits for morphologically complex low-resource languages, with dynamic optimization leading to improved performance in text generation and cross-lingual transfer.
...
TL;DR: Pretrained ModernBERT encoder models in 6 sizes (51M-475M params) with limited multilingual focus on Finnish languages, achieving competitive performance and outperforming monolingual models on long-context tasks.


<details>
  <summary>Details</summary>
Motivation: To develop multilingual BERT models specifically focused on languages relevant to Finland, addressing the need for efficient models that perform well on long-context tasks.

Method: Pretrained ModernBERT encoder models in six different parameter sizes, with emphasis on limited multilingualism targeting Finnish languages, and experimented with different data in final training stage.

Result: Models are competitive with or superior to existing multilingual models, outperform monolingual models on tasks requiring context longer than 512 tokens, with code and models publicly released.

Conclusion: The ModernBERT models successfully demonstrate competitive multilingual performance with Finnish language focus and superior long-context capabilities compared to monolingual alternatives.
...
TL;DR: RLVR framework for aligning language models with reasoning objectives faces collapse from negative rewards. Proposed Anchor method with ground truth trajectories stabilizes training and improves deductive reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Address vulnerability of language models to collapse when negative rewards dominate early training in honesty alignment tasks, particularly for deductive reasoning where models must identify unanswerable queries.

Method: Curated two multi-step deductive reasoning datasets from graph structures (linear algebra and logical inference) with unanswerable cases. Proposed Anchor method that injects ground truth trajectories into rollouts to prevent early training collapse.

Result: GRPO struggles with these tasks. Curriculum learning provides some benefit but requires carefully designed datasets. Anchor method stabilizes learning and significantly improves overall reasoning performance across three models.

Conclusion: Training dynamics are crucial for reliable deductive reasoning in aligned language models, and the proposed Anchor method effectively addresses early training collapse through ground truth trajectory injection.
...
TL;DR: POTSA is a new framework using cross-lingual parallel speech pairs and Optimal Transport to improve multilingual speech-to-text translation by addressing semantic commonalities across languages and reducing performance gaps between high- and low-resource languages.


<details>
  <summary>Details</summary>
Motivation: Existing SpeechLLMs overlook semantic commonalities across source languages, leading to biased translation performance and gaps between high- and low-resource language translation.

Method: Proposes POTSA framework with: 1) Bias Compensation module for coarse speech representation alignment, 2) Token-level Optimal Transport constraints on Q-Former for fine-grained consistency, 3) Layer scheduling strategy to focus OT on most semantically beneficial layers.

Result: Achieves SOTA performance on FLEURS dataset with +0.93 BLEU average improvement on five common languages and +5.05 BLEU on zero-shot languages, using only 10 hours of parallel speech per source language.

Conclusion: POTSA effectively bridges translation gaps between high- and low-resource languages by leveraging cross-lingual semantic commonalities through Optimal Transport, achieving significant improvements with minimal parallel data.
...
TL;DR: C³TG is a two-phase framework for fine-grained multi-dimensional text attribute control that handles attribute conflicts through selective classifier pairing, weighted KL-divergence, and iterative optimization.


<details>
  <summary>Details</summary>
Motivation: Current LLM text generation methods struggle with precise multi-attribute control, lack coordination for conflicting attributes, and don't incorporate iterative optimization in controlled generation pipelines.
...
Result: Significantly outperforms baselines across attribute accuracy, linguistic fluency, output diversity metrics while reducing toxicity, with no model modifications required.

Conclusion: C³TG establishes an effective and flexible solution for multi-dimensional text attribute control that handles attribute conflicts while preserving natural text flow.
...
TL;DR: The paper introduces LiteraryTaste, a dataset for developing personalized creative writing LLMs by capturing individual reading preferences through both self-reported habits and annotated text pair preferences.


<details>
  <summary>Details</summary>
Motivation: Current LLMs for creative writing treat all users' preferences as monolithic, failing to adapt to individual tastes. There's a need for personalized models that can understand and adapt to each user's unique creative writing preferences.

Method: Created LiteraryTaste dataset with 60 participants who provided both stated preferences (self-reported reading habits) and revealed preferences (annotations on 100 pairs of short creative writing texts). Used transformer encoder finetuning and LLM-driven interpretability pipeline to analyze preferences.

Result: Found significant divergence in creative writing preferences among individuals. Transformer encoder achieved 75.8% accuracy for personal preferences and 67.7% for collective preferences. Stated preferences had limited utility in modeling revealed preferences.

Conclusion: The work establishes a foundation for personalizing creative writing technologies by demonstrating the importance of modeling individual preferences and providing a dataset to support future research in this area.
...
TL;DR: This paper presents an explainable Khmer polarity classifier that provides self-explanations for its predictions by fine-tuning an instruction-based Qwen-3 model, along with a new Khmer polarity dataset.


<details>
  <summary>Details</summary>
Motivation: Existing Khmer models predict polarity labels without explaining the rationale behind predictions, creating a need for explainable classification.

Method: Fine-tuned an instruction-based reasoning Qwen-3 model to create an explainable classifier that identifies polarity-related keywords or phrases to support predictions.

Result: The fine-tuned model accurately predicts labels and provides reasoning by identifying polarity-related keywords or phrases. A new Khmer polarity dataset was created and made publicly available.

Conclusion: The proposed explainable Khmer polarity classifier successfully provides both accurate predictions and self-explanations, with the model and dataset made publicly available for further research.
...
TL;DR: This paper introduces AdaptDel methods with adaptable deletion rates for certified robustness against edit distance perturbations in sequence classification, achieving significant improvements over fixed-rate deletion approaches.
...
Method: Developed AdaptDel methods with adaptable deletion rates that dynamically adjust based on input properties, and extended randomized smoothing theoretical framework to variable-rate deletion.

Result: Achieved up to 30 orders of magnitude improvement to median cardinality of the certified region over state-of-the-art certifications in natural language tasks.

Conclusion: The proposed adaptable deletion rate approach provides superior certified robustness for sequence classification against edit distance perturbations compared to fixed-rate methods.
...
TL;DR: mmJEE-Eval is a multimodal bilingual benchmark using JEE Advanced exam questions that reveals significant gaps between frontier VLMs (77-84% accuracy) and open-source models (37-45%) on complex scientific reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal reasoning benchmarks fail to distinguish true scientific reasoning from pattern-matching, requiring a more challenging evaluation that tests genuine problem-solving capabilities.
...
Result: Frontier VLMs (GPT-5, Gemini 2.5) achieve 77-84% accuracy on 2025 questions, while open-source models plateau at 37-45% despite scaling to 400B parameters. Closed models collapse on meta-cognitive reasoning tasks (GPT-5 fixes only 5.2% errors).

Conclusion: mmJEE-Eval effectively segregates superior training and reasoning methodologies, revealing that benchmark difficulty stems from complexity and reasoning depth rather than memorization.
...
TL;DR: SeerSC is a dynamic self-consistency framework that improves token efficiency and reduces latency in LLM inference by integrating System 1 (fast) and System 2 (slow) reasoning.


<details>
  <summary>Details</summary>
Motivation: Test-time scaling improves LLM performance but incurs high computational costs and latency, with existing dynamic self-consistency methods constrained by sequential request latency.

Method: Uses System 1 reasoning to compute answer entropy for queries, then applies this score to evaluate sample scaling potential for dynamic self-consistency under System 2, enabling parallel generation.
...
Conclusion: The integration of System 1 and System 2 reasoning enables efficient dynamic self-consistency that simultaneously improves token efficiency and reduces latency in LLM inference.
...
TL;DR: S2CLite is a rule-based parser that converts SPARQL to Cypher queries without requiring RDF graphs or external tools, achieving 77.8% parsing accuracy and enabling creation of the Spider4SSC multi-query-language dataset.
...
Conclusion: S2CLite provides an effective, standalone solution for SPARQL to Cypher translation with superior accuracy, enabling creation of unified multi-query-language datasets and advancing cross-database query interoperability.
...
TL;DR: MTQ-Eval is a multilingual text quality evaluation framework that trains LLMs using automatically generated quality preference data, showing improved performance across 115 languages and downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To determine if LLMs can extend beyond task-specific evaluations to general text quality assessment in multilingual contexts, addressing uncertainty about their broader evaluation capabilities.

Method: Automatically generate text quality preference data and use it to train open-source base LLMs to align with ratings of high- and low-quality text, adjusting internal representations.
...
Conclusion: The proposed MTQ-Eval framework effectively enables multilingual text quality evaluation and shows that this capability transfers to improved performance in downstream applications.
...
<details>
  <summary>Details</summary>
Motivation: To understand how self-correction mechanisms differ between open-ended text generation and multiple-choice selection in LLMs, as these paradigms have different dynamics that affect emerging agentic applications.
...
Result: Open-ended generation benefits from flexibility of re-interpretation and compositional refinement, while multiple-choice selection leverages clearer solution boundaries but may be limited by provided options. Revealed distinct patterns of improvement and failure modes.
...
TL;DR: AMaPO introduces adaptive margins to solve the Overfitting-Underfitting Dilemma in offline preference optimization, improving ranking accuracy and alignment performance.


<details>
  <summary>Details</summary>
Motivation: Current offline preference optimization methods suffer from the Overfitting-Underfitting Dilemma, where models waste gradients on correctly ranked samples while providing insufficient correction for misranked ones.

Method: AMaPO uses instance-wise adaptive margins with Z-normalization and exponential scaling to dynamically reallocate learning effort, amplifying gradients for misranked samples and suppressing them for correct ones.
...
Conclusion: AMaPO provides a principled solution to the fundamental Overfitting-Underfitting Dilemma in offline preference optimization, offering improved effectiveness through adaptive margin design.
...
TL;DR: Developing a strong Multimodal Large Language Model for Basque, a low-resource language, using custom training data and showing that only 20% Basque multimodal data is sufficient, and that Basque-specific LLM backbones are not necessary.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs perform well in low-resource languages commercially, but open science community lacks comparable results. The paper aims to bridge this gap specifically for Basque language.

Method: Created custom training and evaluation datasets, used Llama-3.1-Instruct and Basque-adapted Latxa LLMs as backbones, experimented with different data mixtures for training.

Result: Achieved solid Basque benchmark performance with only 20% Basque multimodal data, and found that Basque-instructed backbone LLM is not required for strong Basque MLLM performance.

Conclusion: The approach enables development of MLLMs for other low-resource languages, with resources being openly released to support this goal.
...
TL;DR: CARE-Bench is a dynamic, interactive benchmark for evaluating LLMs' counseling competence using diverse client profiles from real cases and multidimensional psychological metrics.


<details>
  <summary>Details</summary>
Motivation: Address the gap between growing demand for psychological counseling and limited services by creating a robust benchmark to assess LLMs' counseling abilities, overcoming limitations of existing unprofessional simulations and unidimensional metrics.

Method: Built CARE-Bench using diverse client profiles from real-world counseling cases, simulated according to expert guidelines, with dynamic interactive evaluation and multidimensional performance assessment based on established psychological scales.

Result: Evaluation of general-purpose LLMs and specialized counseling models revealed current limitations, with detailed failure analysis for different client types conducted in collaboration with psychologists.

Conclusion: The benchmark provides directions for developing more comprehensive, universal, and effective counseling models by identifying specific failure patterns and improvement areas for LLMs in psychological counseling contexts.
...
TL;DR: GSAP-ERE is a manually curated fine-grained dataset for information extraction from ML publications, containing 63K entities and 35K relations from 100 papers, enabling supervised models to significantly outperform LLM prompting methods.
...
Method: Created GSAP-ERE dataset with 10 entity types and 18 relation types from 100 ML publication full texts, then used it to fine-tune models and test LLM prompting strategies for information extraction.

Result: Fine-tuned models achieved NER: 80.6% and RE: 54.0%, significantly outperforming state-of-the-art LLM prompting methods (NER: 44.4%, RE: 10.1%). The dataset enables effective KG construction and reproducibility monitoring.

Conclusion: Supervised models trained on specialized datasets like GSAP-ERE substantially outperform LLM prompting for scholarly information extraction, highlighting the need for domain-specific datasets to advance research in this area.
...
TL;DR: Introduces TPoT (targeted preselection of texts) strategy to filter semantically relevant texts for Big Five personality prediction, improving accuracy and addressing input limits in large language models.


<details>
  <summary>Details</summary>
Motivation: Predicting personalities from large text volumes is challenging, especially with input text limits in large language models.

Method: Semantically filter texts using TPoT strategy as input to BIG5-TPoT deep learning model for Big Five personality trait/facet/item prediction.

Result: Improves Mean Absolute Error and accuracy metrics in predictions for Stream of Consciousness Essays dataset.

Conclusion: TPoT is an effective strategy for personality prediction that addresses text volume challenges and enhances model performance.
...
TL;DR: Readability measures show low correlation with both automatic evaluation metrics and human judgment in automatic text simplification, highlighting the need for clearer definition of simplification constructs.


<details>
  <summary>Details</summary>
Motivation: To investigate the relationship between readability measures and automatic text simplification evaluation, as previous research focused mainly on correlations between automatic metrics and human judgment but not readability measures.
...
Result: Readability measures generally do not correlate well with automatic metrics and human judgment in text simplification.

Conclusion: Since the three assessment angles (readability measures, automatic metrics, and human judgment) show low correlations, there's a need for clearer definition of the construct in automatic text simplification.
...
TL;DR: SynClaimEval framework evaluates synthetic data utility for long-context claim verification, examining input characteristics, synthesis logic, and explanation quality across benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the high cost of constructing annotated resources for training and evaluating LLMs on long documents, and to explore synthetic data as a scalable alternative for claim verification tasks.

Method: Introduced SynClaimEval framework that evaluates synthetic data across three dimensions: input characteristics (context length, out-of-domain generalization), synthesis logic (claim complexity, error type variation), and explanation quality (evidence consistency).
...
Conclusion: Synthetic data has potential to strengthen both performance and explainability in long-context claim verification tasks, offering a scalable alternative to costly annotated resources.
...
TL;DR: AI-based system for automating digitization of gas plant infrastructure using OCR, Vision LLM, Object Detection, and a novel Transformer architecture for relational reasoning, achieving high accuracy in information extraction.


<details>
  <summary>Details</summary>
Motivation: To streamline the digitization process of SNAM's gas infrastructure by automating information extraction from plant documentation, reducing manual work and overcoming data standardization challenges.

Method: Combines OCR, Vision LLM, Object Detection, Relational Reasoning and optimization algorithms with a new Transformer architecture for Scene Graph Generation to analyze complex component relationships in plant P&ID diagrams.

Result: Achieved 91% accuracy in textual design data extraction, 93% component identification accuracy, and approximately 80% accuracy in hierarchical structure extraction.

Conclusion: The synergistic AI approach successfully automates plant digitization, overcoming data variety challenges and demonstrating practical effectiveness for energy infrastructure management.
...
TL;DR: Proposes systematic evaluation methodology to detect and quantify lip leakage in inpainting-based talking face generation, where reference images influence generated lip motion instead of just audio.


<details>
  <summary>Details</summary>
Motivation: Standard metrics and test setups fail to detect lip leaking - where generated lips are influenced by identity reference images rather than solely by driving audio, compromising audio-visual synchronization.
...
Result: Provides framework to analyze how different identity reference selections affect lip leakage, offering insights into reference design for better audio-driven lip generation.

Conclusion: The proposed model-agnostic methodology establishes a more reliable benchmark for evaluating talking face generation systems by systematically quantifying lip leakage issues.
...
Method: Proposes a novel deep learning framework with real-time camera calibration, feature-based image registration, and multi-view feature fusion in bird's-eye-view representation.
...
TL;DR: A topology-driven multi-subspace fusion framework on Grassmannian manifolds that enables adaptive subspace collaboration through dynamic selection and weighting of task-relevant subspaces, outperforming static single-subspace approaches.
...
Result: Achieves state-of-the-art performance on 3D action recognition (HDM05, FPHA), EEG classification (MAMEM-SSVEPII), and graph tasks, demonstrating superior discriminability and robustness.

Conclusion: Successfully adapts multi-channel interaction philosophy from Euclidean networks to non-Euclidean domains, advancing geometric deep learning with improved interpretability and performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video generation lacks precise motion control, and prior methods require expensive model-specific fine-tuning, which is restrictive and computationally demanding.

Method: Uses crude reference animations from user-friendly manipulations as coarse motion cues, adapts SDEdit mechanism to video domain, preserves appearance with image conditioning, and implements dual-clock denoising for region-dependent motion control.

Result: TTM matches or exceeds training-based baselines in realism and motion control on object and camera motion benchmarks, while introducing precise appearance control through pixel-level conditioning.

Conclusion: TTM provides a lightweight, plug-and-play solution for motion-controlled video generation without additional training costs, offering superior motion control and appearance precision compared to text-only approaches.
...
Method: Uses a unified memory bank shared by all tasks, incrementally updates embeddings within a fixed-size coreset during training, and computes anomaly scores via nearest-neighbor matching during inference.

Result: Achieves average image-level AUROC scores of 0.972 on MVTec AD and 0.891 on Visa datasets, with 100% accuracy on a real-world electronic paper dataset.

Conclusion: The proposed framework effectively addresses memory fragmentation issues in continual anomaly detection while maintaining high detection accuracy and practical robustness.
...
TL;DR: A unified framework combining diffusion-based denoising with time-aware actor-critic model for robust accident anticipation in autonomous driving, addressing sensor noise and timing reliability challenges.


<details>
  <summary>Details</summary>
Motivation: To enable proactive and safe autonomous driving by addressing two key real-world challenges: noisy/degraded sensory inputs from weather, motion blur, or hardware limitations, and the need for timely yet reliable predictions that balance early alerts with false-alarm suppression.

Method: Integrates diffusion-based denoising module that reconstructs noise-resilient image and object features through iterative refinement, with a time-aware actor-critic architecture that leverages long-horizon temporal reasoning and time-weighted rewards to determine optimal alert timing.

Result: State-of-the-art accuracy on three benchmark datasets (DAD, CCD, A3D) with significant gains in mean time-to-accident, while maintaining robust performance under Gaussian and impulse noise. Produces earlier, more stable, and human-aligned predictions in both routine and complex traffic scenarios.

Conclusion: The proposed framework demonstrates strong potential for real-world, safety-critical deployment by effectively addressing sensor degradation and prediction timing challenges, enabling earlier and more reliable accident anticipation in autonomous driving systems.
...
Conclusion: RS-Net effectively enhances relation prediction in dynamic scene graph generation by scoring contextual importance of object pairs and can be easily integrated into existing models without architectural changes.
...
<details>
  <summary>Details</summary>
Motivation: Current privacy methods require pixel-level anonymization and retraining, making them unsuitable for video foundation models. Extracted visual features inadvertently reveal sensitive personal information like skin color, gender, and clothing.

Method: Plug-and-play AAM module with three training objectives: clip-level self-supervised privacy objective, co-training objective for utility retention, and latent consistency loss for generalization to unseen tasks.
...
Conclusion: The proposed method provides effective privacy preservation for video foundation models without retraining, addressing both privacy leakage and bias mitigation while maintaining utility across diverse downstream tasks.
...
TL;DR: Autoregressive next-pixel prediction shows task-dependent scaling: classification and generation require different data scaling strategies, with generation needing 3-5x faster data growth. Higher resolutions require faster model size growth than data growth, with compute being the main bottleneck.


<details>
  <summary>Details</summary>
Motivation: To investigate the scaling properties of autoregressive next-pixel prediction as a unified framework for vision models and understand optimal scaling strategies across different tasks and resolutions.

Method: Train Transformers using IsoFlops profiles across compute budgets up to 7e19 FLOPs at 32x32 resolution, evaluating next-pixel prediction, ImageNet classification, and generation quality via Fr'echet Distance.
...
Conclusion: Pixel-by-pixel image modeling is feasible within 5 years given current compute growth rates, as compute rather than data is the limiting factor.
...
TL;DR: The paper explores diffusion model finetuning techniques (LoRA and DreamBooth) to generate balanced training data for debiasing image classification systems, addressing issues with vanilla Stable Diffusion's distribution preservation.


<details>
  <summary>Details</summary>
Motivation: To address biases in image classification systems caused by uneven group representation in training data, and overcome limitations of existing approaches that struggle to preserve original data distributions.

Method: Uses LoRA and DreamBooth finetuning on Stable Diffusion, with clustering within groups to handle intra-group variations (training one DreamBooth model per cluster), followed by group-balanced pretraining and real data fine-tuning.
...
Conclusion: Diffusion model finetuning techniques provide an effective approach for generating balanced training data that can mitigate biases in image classification systems, especially in severely biased datasets.
...
TL;DR: The Women in Computer Vision Workshop (WiCV@CVPR 2025) was the 16th edition focusing on increasing visibility and inclusion of women and underrepresented minorities in computer vision, featuring paper presentations, mentoring programs, and networking opportunities.


<details>
  <summary>Details</summary>
Motivation: To advance diversity, equity, and inclusion within AI and computer vision communities by documenting the impact and evolution of WiCV as a reference for future initiatives.

Method: Organized workshop with paper submissions (32 full, 62 short), accepted 14 papers for proceedings (5 oral presentations), mentoring program matching 80 mentees with 37 mentors, and networking events with over 100 onsite participants.

Result: Successful workshop with 14 accepted papers, 36 extended abstract posters, 80 mentees matched with mentors, over 100 participants, $44,000 in travel grants and diversity awards from 10 sponsors.

Conclusion: WiCV@CVPR 2025 continued its mission to empower emerging researchers and amplify diverse voices in computer vision, serving as a valuable reference for future diversity initiatives in the field.
...
TL;DR: PoseKAN introduces adaptive graph KANs for 3D pose estimation, overcoming GCN limitations by using learnable activation functions and multi-hop feature aggregation for better long-range dependency modeling.


<details>
  <summary>Details</summary>
Motivation: GCN-based methods have limited receptive fields and spectral bias, struggling with long-range dependencies and high-frequency details needed for handling occlusions and depth ambiguities in 3D pose estimation.

Method: Extends KANs to graph learning with learnable functions on edges, multi-hop feature aggregation, residual PoseKAN blocks, and global response normalization for adaptive feature transformations.

Result: Extensive experiments show competitive performance against state-of-the-art methods on benchmark datasets.

Conclusion: PoseKAN's adaptive graph KAN framework provides enhanced expressiveness and adaptability for 3D human pose estimation, effectively addressing GCN limitations.
...
TL;DR: SIFT-Graph is a multimodal defense framework that enhances vision model robustness by integrating SIFT keypoints with Graph Attention Networks to capture perturbation-resistant structural features, then fusing them with traditional vision models.
...
Conclusion: The proposed SIFT-Graph framework successfully enhances vision model robustness by incorporating structurally meaningful features that are resilient to perturbations.
...
TL;DR: DT-NVS is a 3D diffusion model for generalized novel view synthesis from single images, trained on real-world videos with image-only losses and transformer architecture.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on small camera movements or unnatural object-centric scenes, limiting real-world applications. This work aims to handle everyday scenes with large viewpoint changes.

Method: Uses transformer-based 3D diffusion model with novel camera conditioning and training paradigm that swaps reference frame roles between conditioning image and noisy input.

Result: Shows improvements over state-of-the-art 3D diffusion models and deterministic approaches, generating diverse outputs for generalized novel view synthesis.

Conclusion: Proposed approach successfully handles real-world scenes with large viewpoint changes, advancing single-image novel view synthesis capabilities.
...
TL;DR: The paper proposes Shadow-informed Pose Feature (SiPF) and Rotation-invariant Attention Convolution (RIAttnConv) to address the loss of global pose information in rotation-invariant 3D point cloud learning, enabling better differentiation of geometrically similar but spatially distinct structures.
...
Method: Introduces SiPF that augments local RI descriptors with globally consistent reference points (shadows) from learned shared rotations, and RIAttnConv that integrates SiPFs into feature aggregation. Also includes a task-adaptive shadow locating module using Bingham distribution over unit quaternions.

Result: Extensive experiments on 3D classification and part segmentation benchmarks show substantial performance improvements over existing RI methods, especially for fine-grained spatial discrimination under arbitrary rotations.

Conclusion: The proposed approach successfully preserves global pose awareness while maintaining rotation invariance, overcoming limitations of existing RI methods in distinguishing symmetric components.
...
TL;DR: SAS-SSM introduces a structure-aware spatiotemporal convolution and stride-based scan strategy to maintain human skeleton spatial structure while capturing local and global pose dependencies with linear complexity.


<details>
  <summary>Details</summary>
Motivation: Existing SSM-based methods flatten 2D pose sequences into purely temporal sequences, disrupting spatial structure and entangling spatial-temporal features, making it difficult to capture complex pose dependencies.

Method: Proposes Skeleton Structure-Aware Stride SSM (SAS-SSM) with structure-aware spatiotemporal convolution for local joint interactions and stride-based scan for multi-scale global structural representations.
...
Conclusion: SAS-SSM effectively maintains spatial structure while capturing both local and global pose dependencies with linear computational complexity, offering parameter-efficient 3D human pose estimation.
...
TL;DR: The paper proposes MFAVBs, a novel Vision Transformer-based architecture for contrastive image clustering that explicitly fuses features from positive pairs through multiple fusion-augmentation blocks, outperforming state-of-the-art methods.
...
Method: Designs multiple fusing-augmenting ViT blocks (MFAVBs) that fuse output features from two shared-weight ViTs into a larger ViT, then split features into new augmented pairs for multiple fusion operations, with CLIP-based preprocessing.

Result: Experiments on seven public datasets demonstrate that MFAVBs outperform state-of-the-art techniques in clustering performance.

Conclusion: The proposed MFAVBs architecture effectively enhances contrastive clustering by explicitly fusing features from positive pairs through multiple fusion and augmentation operations.
...
TL;DR: A deep learning approach using EfficientNet architectures was developed to classify six histopathological regions in glioblastoma tissue sections, achieving high training performance but showing generalization challenges on validation and test data.


<details>
  <summary>Details</summary>
Motivation: To improve understanding of glioblastoma by enabling automated, robust identification of distinct histological sub-regions, which could contribute to morphological analysis at scale.

Method: Four-step deep learning approach using EfficientNet architectures (B0-B4 variants) on H&E stained GBM tissue sections from BraTS-Path 2024 dataset, evaluated with 5-fold cross-validation.

Result: EfficientNet-B1 and B4 achieved F1 score of 0.98 on training data, but performance dropped to 0.546 on validation and 0.517 on test data, highlighting generalization challenges.

Conclusion: While the approach shows promise for automated GBM histopathological analysis, the performance gap between training and test data emphasizes the need for models that better generalize to new data for clinical applications.
...
TL;DR: Enhanced VisNet variants with RBF neurons, Mahalanobis distance learning, and retinal preprocessing improve object recognition and symmetry classification accuracy across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between efficient biological visual systems and artificial object recognition by developing biologically inspired neural networks that capture robust, transformation-invariant features.

Method: Developed VisNet variants incorporating radial basis function neurons, Mahalanobis distance-based learning, and retinal-like preprocessing, using Hebbian learning and temporal continuity to build invariant representations.

Result: Enhanced VisNet variants substantially improved recognition accuracy compared to baseline across MNIST, CIFAR10, and custom symmetric object datasets.

Conclusion: VisNet-inspired architectures offer adaptable, biologically relevant, and interpretable frameworks for visual recognition in both neuroscience and AI applications.
...
TL;DR: Proposes Asymmetric Cross-modal Knowledge Distillation (ACKD) and SemBridge framework to enable knowledge transfer between modalities with weak semantic consistency, addressing limitations of traditional symmetric approaches that require strongly paired data.


<details>
  <summary>Details</summary>
Motivation: Traditional symmetric cross-modal knowledge distillation requires strongly paired modalities with high semantic overlap, which is often unavailable in real-world scenarios. The paper aims to enable knowledge transfer between modalities with limited semantic consistency.

Method: Proposes SemBridge framework with two modules: Student-Friendly Matching (uses self-supervised learning for semantic-based knowledge and dynamic teacher sample selection) and Semantic-aware Knowledge Alignment (employs Lagrangian optimization for optimal transport paths).

Result: Achieves state-of-the-art performance compared to 7 existing approaches on 6 different model architectures across various datasets, including a curated benchmark with Multi-Spectral and asymmetric RGB images for remote sensing classification.

Conclusion: The proposed ACKD framework effectively addresses the challenges of knowledge transfer between weakly aligned modalities, demonstrating superior performance and flexibility compared to traditional symmetric approaches.
...
<details>
  <summary>Details</summary>
Motivation: Document layout understanding remains data-intensive despite advances in semi-supervised learning, requiring methods that can leverage structural knowledge more efficiently.
...
Conclusion: LLM structural priors are complementary to both lightweight and pretrained architectures, enabling privacy-preserving deployment and targeted semantic disambiguation with minimal system cost.
...
TL;DR: Proposes CCDF framework with Cycle Consistency and Semantic Consistency modules to improve unsupervised change detection by reducing generator overfitting and enabling detail reconstruction.


<details>
  <summary>Details</summary>
Motivation: Previous unsupervised methods using generator networks for style transfer often suffer from generator overfitting, leading to poor change detection performance.
...
TL;DR: HitoMi-Cam is a lightweight, shape-agnostic person detection method using spectral reflectance properties of clothing, achieving real-time performance on edge devices and outperforming CNNs in unpredictable shape scenarios like disaster rescue.


<details>
  <summary>Details</summary>
Motivation: CNN-based object detection suffers from shape dependency that degrades performance for postures not in training data, limiting effectiveness in real-world scenarios with unpredictable shapes like disaster rescue operations.

Method: Spectral-based approach using reflectance properties of clothing, implemented on resource-constrained edge devices without GPU as a lightweight, shape-agnostic detection system.

Result: Achieved 23.2 fps processing speed (253x190 pixels) for real-time operation and 93.5% AP in search/rescue scenarios, significantly outperforming CNN models (best AP 53.8%) with minimal false positives.

Conclusion: HitoMi-Cam serves as a complementary tool to CNN-based detectors, proving spectral-based person detection is viable for real-time edge device operation in environments with unpredictable shapes like disaster rescue.
...
<details>
  <summary>Details</summary>
Motivation: Text-only training for zero-shot image captioning suffers from poor cross-domain generalization and hallucination when encountering novel visual environments, while retrieval-based methods can exacerbate hallucination with irrelevant entities.

Method: NES integrates three stages: (1) uses synthetic images for consistent image-to-text retrieval across training and inference, (2) filters negative entities from retrieved content, and (3) applies attention-level suppression using identified negative entities.
...
TL;DR: SPEED-Q is a novel framework for low-bit weight-only quantization of Vision-Language Models (VLMs) ranging from 1B to 2B parameters, specifically designed for edge device deployment. It addresses quantization sensitivity discrepancies between vision and language components and training instability through staged processing and enhanced distillation.


<details>
  <summary>Details</summary>
Motivation: Deploying VLMs on resource-constrained edge devices requires efficient quantization methods, but existing research rarely explores aggressive quantization for small-scale VLMs (1B-2B parameters) suitable for edge deployment.

Method: SPEED-Q introduces: 1) A staged sensitivity adaptive mechanism to harmonize performance across different modalities, and 2) A distillation-enhanced quantization strategy to stabilize training and reduce data dependence.

Result: Extensive experiments show SPEED-Q achieves up to 6x higher accuracy than existing quantization methods under 2-bit settings and consistently outperforms prior on-device VLMs under both 2-bit and 4-bit settings.

Conclusion: SPEED-Q enables accurate, stable, and data-efficient quantization of complex VLMs, making it the first framework specifically tailored for quantizing entire small-scale billion-parameter VLMs to low bits for edge device deployment.
...
TL;DR: Diff-FCHM is a novel human-machine collaborative compression method that uses machine-vision-oriented compression as the basis, progressively aggregating semantics from machine-vision compression and using diffusion priors to restore high-fidelity details for human vision.
...
Method: Proposes a collaborative compression method based on machine-vision-oriented compression instead of human-vision pipeline. Uses a plug-and-play variable bit-rate strategy for machine vision tasks, progressively aggregates semantics from machine-vision compression, and employs diffusion priors to restore high-fidelity details for human vision.

Result: Experimental results show consistently superior performances on both machine-vision and human-vision compression with remarkable margins compared to existing methods.

Conclusion: Diff-FCHM successfully demonstrates that machine vision can serve as the basis for human vision in collaborative compression, achieving better efficiency and performance than traditional human-vision-based approaches.
...
TL;DR: The paper proposes Hierarchical Distillation (HD) framework that synergistically combines trajectory-based and distribution-based distillation methods to achieve high-fidelity single-step diffusion models, addressing the latency vs quality trade-off.


<details>
  <summary>Details</summary>
Motivation: To overcome the inference latency barrier of diffusion models while maintaining high fidelity, by resolving the fundamental trade-off between trajectory-based methods (preserving global structure but losing details) and distribution-based methods (higher fidelity but unstable training).

Method: Hierarchical Distillation framework that uses trajectory distillation as structural "sketch" initialization, followed by distribution-based refinement with Adaptive Weighted Discriminator (AWD) that dynamically allocates token weights to focus on local imperfections.

Result: State-of-the-art performance with single-step model achieving FID of 2.26 on ImageNet 256×256 (rivaling 250-step teacher) and promising results on high-resolution text-to-image MJHQ benchmark, demonstrating strong generalizability.

Conclusion: The HD framework establishes a robust new paradigm for high-fidelity single-step diffusion models by synergistically combining trajectory and distribution distillation approaches with specialized adversarial training.
...
Method: NAMEA decouples gradients from attention and non-attention areas of ensemble models, then merges them using meta-learning. It specifically leverages non-attention areas since attention areas vary sharply between heterogeneous models.

Result: Empirical evaluations on ImageNet show NAMEA outperforms state-of-the-art ensemble attacks AdaEA and SMER by an average of 15.0% and 9.6% respectively.
...
TL;DR: Proposes BRHVC, a neural B-frame video compression method that harmonizes bi-directional references using Bi-directional Motion Converge and Bi-directional Contextual Fusion to improve compression performance.
...
Method: Uses Bi-directional Motion Converge (BMC) to converge multiple optical flows for more accurate motion compensation, and Bi-directional Contextual Fusion (BCF) to model reference context weights guided by motion compensation accuracy.

Result: Outperforms previous state-of-the-art neural video compression methods and surpasses traditional VTM-RA coding on HEVC datasets.

Conclusion: BRHVC effectively harmonizes bi-directional references through more efficient motion and context modeling, achieving superior compression performance.
...
<details>
  <summary>Details</summary>
Motivation: Fractal Generative Models generate high-quality images but suffer from limited diversity due to inherent self-similarity, creating a need for methods that enhance diversity without sacrificing visual quality.

Method: Introduces learnable HD estimation from image embeddings, HD-based loss with momentum-driven scheduling during training, and HD-guided rejection sampling during inference to select geometrically richer outputs.
...
Conclusion: Successfully enhances FGM diversity through principled HD integration, offering both practical improvements and theoretical contributions to fractal generative modeling.
...
TL;DR: AuthSig is a static electronic signature framework that embeds authentication watermarks using generative models to prevent signature copying and reuse, achieving over 98% extraction accuracy across various distortions.


<details>
  <summary>Details</summary>
Motivation: Static scanned signatures are widely used but lack authentication attributes and are vulnerable to malicious copying and reuse, creating security risks in paperless workflows.

Method: Uses generative models to embed watermark bits by subtly modulating style embeddings in signature images, combined with a keypoint-driven data augmentation strategy to enhance style diversity for robust watermark embedding.

Result: Achieves over 98% watermark extraction accuracy under digital-domain distortions, signature-specific degradations, and even in print-scan scenarios.

Conclusion: AuthSig provides an effective solution for securing static electronic signatures by embedding imperceptible authentication watermarks, enabling reliable verification and preventing unauthorized reuse.
...
TL;DR: Proposes CoDR framework for efficient and effective demonstration selection in in-context learning for Large Visual Language Models using coreset-based dual retrieval.


<details>
  <summary>Details</summary>
Motivation: Traditional demonstration selection methods (random, similarity-based, infoscore-based) are inefficient or suboptimal, struggling to balance efficiency and effectiveness in NP-hard selection process.

Method: Uses cluster-pruning to construct diverse coreset with high mutual information, combined with dual retrieval mechanism for global selection while maintaining efficiency.

Result: Experimental results show significant improvement in ICL performance compared to existing strategies.

Conclusion: CoDR provides robust solution for effective and efficient demonstration selection in in-context learning.
...
TL;DR: WDT-MD is a novel Wavelet Diffusion Transformer framework that addresses key limitations in microaneurysm detection for diabetic retinopathy screening, achieving state-of-the-art performance through noise-encoded conditioning, pseudo-normal pattern synthesis, and wavelet-enhanced reconstruction.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based anomaly detection methods for microaneurysm screening suffer from identity mapping, poor discrimination between MAs and other anomalies, and suboptimal normal feature reconstruction, limiting their clinical application.
...
Conclusion: The proposed framework significantly advances early diabetic retinopathy screening by overcoming fundamental limitations of existing diffusion-based approaches.
...
TL;DR: A variational segmentation model combining denoising with adaptive TV regularization for images with Gamma/Poisson noise and intensity inhomogeneity, using ICTM and RMSAV optimization.
...
Method: Integrates denoising terms (I-divergence + adaptive TV regularizer) with bias field estimation, using ICTM framework coupled with RMSAV scheme for optimization.
...
Conclusion: The proposed model effectively handles challenging segmentation scenarios with noise and intensity variations through integrated denoising and adaptive regularization.
...
TL;DR: T-Rex-Omni introduces negative visual prompts to improve open-set object detection by addressing vulnerability to visually similar distractors through a unified encoder, training-free NNC module, and NNH loss.
...
Method: Proposes a unified visual prompt encoder for positive/negative prompts, training-free Negating Negative Computing (NNC) module, and Negating Negative Hinge (NNH) loss for discriminative margins.
...
Conclusion: Negative prompts are established as a crucial new dimension for advancing open-set visual recognition systems.
...
TL;DR: Owl is a framework that mitigates object hallucination in LVLMs by modeling the causal relationship between visual and textual attention, using VTACR to quantify modality imbalance and implementing attention intervention with contrastive decoding.


<details>
  <summary>Details</summary>
Motivation: Existing approaches address visual or textual attention independently, failing to capture their interaction as key causal factors in object hallucination.

Method: Proposes VTACR metric to measure modality imbalance, implements fine-grained attention intervention, and uses dual-path contrastive decoding to emphasize visual grounding while collapsing hallucinations.
...
Conclusion: Owl effectively addresses object hallucination by causally modeling attention interactions and dynamically balancing modality contributions during decoding.
...
Method: Uses dense cross-scale alignment considering feature correlations, includes a fully spatial correlation module, and incorporates just noticeable difference to focus on distortion-sensitive regions.
...
Conclusion: The proposed model effectively addresses accuracy and efficiency challenges in image alignment through cross-scale feature correlation and distortion-aware optimization.
...
<details>
  <summary>Details</summary>
Motivation: Existing cloud image extrapolation methods have limitations: lack adaptive feature extraction mechanisms, insufficient temporal guidance for long-range dependencies, and high computational costs from attention mechanisms that limit practical deployment.

Method: Proposes USF-Net with encoder-decoder framework. Encoder uses three basic layers with USTM module containing SiB (with SSM for multi-scale context) and TiB (with TAM for temporal dependencies). Includes DSM with TGM for unified spatiotemporal modeling, and DUM in decoder to prevent ghosting effects using temporal state as attention operator.
...
Conclusion: USF-Net effectively addresses key challenges in cloud image sequence extrapolation through adaptive feature extraction and efficient temporal modeling, establishing a practical solution for photovoltaic power systems with released dataset and code.
...
TL;DR: 4KDehazeFlow is a novel Ultra-High-Definition image dehazing method using Flow Matching and Haze-Aware vector field, achieving superior performance with 2dB PSNR improvement over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Address limitations of prior-based methods (limited scene adaptability) and deep learning approaches (high computational complexity with color distortion) in UHD image dehazing.

Method: Models dehazing as progressive optimization of continuous vector field flow using Flow Matching and Haze-Aware vector field. Uses learnable 3D lookup table (LUT) for efficient inference and fourth-order Runge-Kutta ODE solver for stable artifact suppression.

Result: Exceeds seven state-of-the-art methods with 2dB PSNR increase, better performance in dense haze scenarios, and improved color fidelity.

Conclusion: 4KDehazeFlow provides an efficient, general-purpose dehazing solution that is compatible with various deep learning networks and achieves state-of-the-art performance in UHD image dehazing.
...
TL;DR: PAN is a general world model that combines LLM-based latent dynamics with video diffusion to enable interactive, long-horizon world simulation conditioned on natural language actions.


<details>
  <summary>Details</summary>
Motivation: Existing video generation models lack causal control and long-horizon consistency, while current world models are limited to restricted domains with poor generalization. There's a need for general world models that can simulate diverse environments with interactivity.
...
Result: PAN achieves strong performance in action-conditioned world simulation, long-horizon forecasting, and simulative reasoning compared to other models, supporting open-domain simulation with coherent dynamics.
...
Method: Integrates cultural object detection with structured program generation, using a curated knowledge base of Vietnamese cultural entities and a dual-modality explanation module combining visual attention with textual rationales.

Result: Developed a Vietnamese Cultural VQA dataset and demonstrated practicality of programming-based methodologies for cultural AI, providing transparent explanations.

Conclusion: The system supports education and cultural preservation with emphasis on interpretability and cultural sensitivity through transparent explanations of computational rationale and cultural context.
...
TL;DR: DOC enhances adversarial robustness in vision-language models by generating diverse counterattacks using orthogonal gradient directions and momentum-based updates, improving defense against various attacks while maintaining clean accuracy.


<details>
  <summary>Details</summary>
Motivation: Current counterattack methods like TTC generate limited diversity in perturbations due to narrow search spaces, leading to overfitting on specific adversarial patterns and insufficient coverage to neutralize broad range of attacks.

Method: Proposes Directional Orthogonal Counterattack (DOC) that incorporates orthogonal gradient directions and momentum-based updates to expand counterattack space exploration, along with a directional sensitivity score for adaptive counterattack strength modulation.

Result: Extensive experiments on 16 datasets show DOC improves adversarial robustness under various attacks while maintaining competitive clean accuracy compared to existing methods.

Conclusion: Enhancing counterattack diversity and coverage through orthogonal directions and momentum updates is crucial for improving adversarial robustness in test-time defense of vision-language models.
...
<details>
  <summary>Details</summary>
Motivation: Real-world data continually emerges with nearly infinite, long-tailed compositions that are not entirely visible, requiring models to gradually improve compositional generalization capabilities incrementally.

Method: Proposes a pseudo-replay framework with a visual synthesizer to synthesize representations of learned compositions and linguistic primitive distillation to maintain aligned primitive representations across learning.

Result: Developed benchmark datasets (MIT-States-CompIL and C-GQA-CompIL) and extensive experiments demonstrate the effectiveness of the proposed framework.

Conclusion: The CompIL framework successfully enables models to progressively improve compositional generalization capability through incremental learning of new compositions.
...
TL;DR: UL-TTA is a training-free test-time adaptation method that adapts only logit-level parameters (class prototypes, priors, temperature) for Vision-Language Models under domain shift, achieving improved accuracy and calibration without updating backbone parameters.
...
Method: Online EM-style procedure with selective sample filtering, closed-form Bayesian updates for prototypes and priors anchored by text and Dirichlet priors, decoupled temperatures for prediction vs. calibration, and lightweight guards to prevent drift.

Result: Consistently improves top-1 accuracy (+4.7 points over zero-shot CLIP) while reducing ECE by 20-30%, with <8% latency overhead across large-scale cross-domain benchmarks (~726K test samples).

Conclusion: Logit-level Bayesian adaptation is sufficient for state-of-the-art accuracy-calibration trade-offs for VLMs under domain shift without updating backbone parameters.
...
<details>
  <summary>Details</summary>
Motivation: Existing OCR methods for Kuzushiji perform well on clean documents but fail on degraded ones with noise like document degradation and seals, with no existing dataset addressing these specific challenges.
...
Result: Provided baseline results for both tracks - YOLO models for detection and various binarization methods, making dataset and implementation code publicly available.

Conclusion: DKDS serves as a new benchmark to address OCR challenges in degraded Kuzushiji documents with seals, enabling better recognition accuracy in noisy historical documents.
...
TL;DR: PIFF is a physics-informed flow-based generative neural network that provides near real-time flood depth estimation by mapping Digital Elevation Models to flood predictions, integrating hydrodynamic priors and rainfall data.
...
Method: Built on an image-to-image generative framework, PIFF uses a simplified inundation model (SPM) to embed hydrodynamic priors and a transformer-based rainfall encoder to capture temporal precipitation dependencies, combining physics-informed constraints with data-driven learning.

Result: The model was tested on a 26 km study area in Tainan, Taiwan with 182 rainfall scenarios (24-720 mm over 24 hours), demonstrating effective flood prediction capabilities as a data-driven alternative to traditional simulations.

Conclusion: PIFF successfully replaces costly simulations with accurate, real-time flood maps by capturing causal relationships between rainfall, topography, SPM, and flooding, offering an efficient solution for flood prediction and response.
...
TL;DR: MACEval is a multi-agent continual evaluation network that provides dynamic, automated evaluation of large language models using role assignment, in-process data generation, and evaluation routing to address issues with traditional benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarks for large models are closed-ended, prone to overfitting due to data contamination, difficult to maintain, and heavily dependent on human curation, undermining evaluation credibility.

Method: Uses a multi-agent network with role assignment, in-process data generation, and evaluation routing through cascaded agents to create an interactive, autonomous evaluation system.

Result: Experiments on 9 open-ended tasks with 23 large models show MACEval is human-free, efficient (reduces data and overhead), flexible (migrates existing benchmarks), and scalable via customized evaluation topologies.

Conclusion: MACEval provides a sustainable, longitudinal evaluation framework that can broaden future directions for large model assessment and mitigate current benchmark limitations.
...
TL;DR: PressTrack-HMR is a top-down pipeline that recovers multi-person global human meshes from pressure signals using tracking-by-detection to separate individual pressure data from intermingled signals.


<details>
  <summary>Details</summary>
Motivation: Traditional vision-based human mesh recovery faces limitations due to occlusions, lighting issues, and privacy concerns. Pressure-based methods offer occlusion-free and privacy-friendly alternatives, but existing approaches struggle with separating intermingled pressure signals from multiple individuals walking simultaneously.

Method: A top-down pipeline using tracking-by-detection strategy to first identify and segment each individual's pressure signal from raw pressure data, then performs human mesh recovery for each extracted individual signal. Also built the MIP multi-person interaction pressure dataset.
...
TL;DR: HOTFLoc++ is an end-to-end framework for LiDAR place recognition and 6-DoF localization in forests using octree-based transformers and multi-scale geometric verification, achieving significant improvements in recall and runtime over baselines.
...
Method: Uses octree-based transformer to extract hierarchical local descriptors at multiple granularities, learnable multi-scale geometric verification module for re-ranking, and coarse-to-fine registration approach.

Result: Achieves average Recall@1 of 90.7% on CS-Wild-Places (29.6% improvement over baselines), 91.7% on Wild-Places, and 96.0% on MulRan. Runtime improvements of two orders of magnitude over RANSAC, with under 2m and 5 degrees error for 97.2% of registration attempts.

Conclusion: The proposed framework demonstrates superior performance in challenging forest environments, with significant improvements in both accuracy and computational efficiency compared to state-of-the-art methods.
...
Result: DBINDS achieves strong cross-generator performance on GenVidBench, demonstrating good generalization and robustness in limited-data settings when trained on a single generator.

Conclusion: Analyzing latent-space dynamics through diffusion inversion provides a more effective approach for detecting AI-generated videos with better generalization capabilities compared to pixel-level methods.
...
TL;DR: A novel evaluation framework combining DermBench (curated benchmark) and DermEval (automatic evaluator) for reliable assessment of multimodal LLMs in dermatology diagnosis.


<details>
  <summary>Details</summary>
Motivation: Reliable evaluation is the primary bottleneck for responsible clinical deployment of multimodal LLMs in dermatology diagnosis.

Method: Built DermBench with 4,000 real-world dermatology images paired with expert-certified narratives, and trained DermEval as a reference-free multimodal evaluator that produces structured critiques with scores.
...
Conclusion: The framework enables clinically meaningful, reproducible, and scalable assessment of multimodal LLMs for dermatology diagnosis, addressing the critical need for reliable evaluation in clinical deployment.
...
TL;DR: TACO is a framework that reduces hallucinations in MLLMs through self-verification and confidence calibration, decomposing responses into atomic queries and using self-consistency or self-confidence aggregation.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models often suffer from hallucinations (errors in object existence, attributes, or relations) which undermine their reliability.
...
Method: Proposes Spatial Information Bottleneck (S-IB) that maximizes mutual information between foreground VJP and inputs while minimizing mutual information in background regions, encouraging networks to encode information only in class-relevant spatial regions.
...
Conclusion: Directly optimizing VJP's spatial structure during training improves visualization quality across diverse explanation paradigms, providing a principled approach to spatial disentanglement in neural networks.
...
TL;DR: Proposes FPGA-based deployment for Animation-based Generative Codec (AGC) on edge devices, achieving 24.9× and 4.1× higher energy efficiency vs CPU/GPU.
...
Method: Network compression via static quantization and layer fusion, co-processor paradigm with hardware engines (convolution, grid sampling, upsample), parallelization optimizations like double-buffered pipelines and loop unrolling.
...
Conclusion: FPGA-based deployment scheme enables efficient AGC implementation on resource-constrained edge devices with significant energy savings.
...
TL;DR: Deep learning models outperform classical methods for energy expenditure estimation, with transformers achieving best results using minute ventilation signals, though performance varies by activity intensity and shows strong inter-individual variability.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for energy expenditure estimation rarely disentangle the role of neural architecture from signal choice, and there's a need to systematically evaluate both aspects across different physical activities.

Method: Systematic comparison of classical baselines with newer neural architectures (including transformers, CNN, ResNet with attention) across single signals, signal pairs, and grouped sensor inputs from Hexoskin smart shirt for diverse physical activities.

Result: Minute ventilation is the most predictive individual signal, with transformer model achieving lowest RMSE of 0.87 W/kg across all activities. Better results in low-intensity activities (RMSE down to 0.29 W/kg; NRMSE = 0.04), while higher-intensity tasks showed larger RMSE but comparable normalized errors. Strong inter-individual variability observed.

Conclusion: Transformers with minute ventilation signals provide best overall performance, but subject-level analysis highlights need for adaptive modeling strategies due to strong inter-individual variability.
...
TL;DR: RichKD enhances multi-teacher knowledge distillation by fusing conventional teacher knowledge with CLIP's vision-language representations, improving accuracy, confidence, and robustness across benchmarks.
...
Method: Proposes RichKD framework that fuses logits and features from conventional teachers with CLIP's multi-prompt textual guidance, capturing both dataset-specific and semantically enriched visual cues.

Result: Consistently outperforms most existing baselines across multiple benchmarks, yields more confident predictions (increases confident-correct cases, reduces confidently wrong ones), and shows stronger robustness under distribution shifts and input corruptions.

Conclusion: Incorporating CLIP's cross-modal knowledge as complementary supervision significantly improves knowledge distillation quality, producing semantically meaningful probabilities and better inter-class consistency despite method simplicity.
...
TL;DR: DensiCrafter generates lightweight, self-supporting 3D hollow structures by optimizing density fields from pretrained models, achieving up to 43% mass reduction while maintaining geometric fidelity.
...
Method: Optimizes continuous density fields from coarse voxel grids using differentiable, physically constrained loss terms and mass regularization, while preserving outer surfaces and integrating with pretrained Trellis-based models.

Result: Achieves up to 43% material mass reduction on text-to-3D tasks, improves stability compared to baselines, and real-world 3D-printing confirms reliable fabrication of self-supporting hollow designs.

Conclusion: DensiCrafter successfully bridges the gap between 3D generative models and practical manufacturability by producing lightweight, self-supporting structures that maintain geometric quality and can be reliably 3D printed.
...
TL;DR: Introduces a feedback mechanism in teacher-student semi-supervised learning to address error propagation in medical image segmentation, featuring a dual-teacher model with feedback attributor and receiver components.


<details>
  <summary>Details</summary>
Motivation: Teacher-student paradigm in medical image segmentation is vulnerable to erroneous supervision due to image ambiguities, leading to self-reinforcing bias through error reconfirmation. Existing methods overlook intrinsic error correction potential.

Method: Adds feedback mechanism where student provides feedback on teacher's pseudo-label changes. Uses feedback attributor to identify problematic pseudo-labels and feedback receiver to apply corrections. Proposes dual-teacher model for dynamic feedback loops and cross-teacher supervision to resolve disagreements.
...
Conclusion: The feedback mechanism successfully counteracts error reconfirmations in teacher-student framework, with dual-teacher model providing additional gains through dynamic feedback and cross-teacher supervision.
...
<details>
  <summary>Details</summary>
Motivation: Camera-based multi-view 3D detection models like PETR face deployment challenges due to high computational costs and memory footprint. Direct quantization causes severe accuracy degradation due to multi-modal feature disparity and inefficient non-linear operator quantization.
...
Conclusion: The proposed FQ-PETR framework successfully enables efficient deployment of PETR-based 3D detection models through effective quantization techniques that address key challenges in multi-modal feature alignment and non-linear operator quantization.
...
TL;DR: TDCNet is a novel moving infrared small target detection network that combines temporal difference and 3D convolution through re-parameterization to enhance spatio-temporal feature extraction for accurate target detection in complex backgrounds.


<details>
  <summary>Details</summary>
Motivation: Moving infrared small target detection faces challenges due to weak target features and complex background interference. Existing methods using temporal differences or 3D convolutions have limitations - temporal differences lack spatial feature extraction capability while 3D convolutions lack explicit motion awareness.
...
Result: Extensive experiments on IRSTD-UAV and public infrared datasets demonstrate state-of-the-art detection performance in moving target detection, effectively suppressing pseudo-motion clutter in complex backgrounds.

Conclusion: The proposed TDCNet effectively combines the advantages of temporal difference and 3D convolution, achieving superior performance in moving infrared small target detection through enhanced spatio-temporal feature modeling.
...
TL;DR: Flora is a zero-shot skeleton action recognition method that addresses fragile point-to-point alignment and rigid classifiers through flexible neighbor-aware semantic attunement and distribution-aware flow classification.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot skeleton action recognition methods face two key issues: (1) fragile point-to-point alignment due to imperfect semantics, and (2) rigid classifiers with static decision boundaries and coarse-grained anchors.

Method: Flora uses flexible neighbor-aware semantic attunement with cross-modal geometric consistency for robust point-to-region alignment, and employs noise-free flow matching with condition-free contrastive regularization to create distribution-aware classifiers with fine-grained decision boundaries.

Result: Extensive experiments on three benchmark datasets show the method's effectiveness, achieving impressive performance even when trained with only 10% of seen data.

Conclusion: Flora successfully addresses the limitations of existing zero-shot skeleton action recognition approaches through its novel semantic attunement and flow classification framework, demonstrating strong performance across multiple datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing active reconstruction methods for 3DGS use scene-level uncertainty metrics that are biased by background clutter, leading to inefficient view selection for object-centric tasks.

Method: Derives uncertainty directly from 3D Gaussian primitive parameters (position, scale, rotation) by propagating covariance through the rendering Jacobian, then integrates semantic segmentation masks to create object-aware uncertainty scores for targeted view selection.

Result: Experimental evaluations show significant improvements in reconstruction efficiency and higher quality for targeted objects compared to state-of-the-art methods, while also providing robust uncertainty estimation for the global scene.

Conclusion: OUGS provides a principled, physically-grounded uncertainty formulation that enables more effective object-centric 3D reconstruction in complex scenes.
...
<details>
  <summary>Details</summary>
Motivation: Current bronchoscopy navigation faces challenges from respiratory motion, anatomical variability, and CT-to-body divergence, leading to poor generalization and alignment errors across domains and patients.

Method: Vision-based pose optimization framework with modality- and domain-invariant encoder for similarity computation between RGB frames and CT depth maps, plus differentiable rendering for iterative pose refinement through depth consistency.

Result: Achieved average translational error of 2.65 mm and rotational error of 0.19 rad using only synthetic training data, with strong cross-domain generalization on real patient data without domain adaptation.

Conclusion: The framework provides robust, domain-invariant localization through iterative vision-based optimization, while the benchmark enables standardized progress in bronchoscopy navigation.
...
<details>
  <summary>Details</summary>
Motivation: No standardized dataset exists for American football player tracking despite challenges like frequent occlusion and physical contact, making fair method comparisons difficult.

Method: Constructed dedicated detection and tracking dataset for American football players, then conducted comparative evaluation of various detection and tracking methods with fine-tuning.
...
TL;DR: Dual-Teacher Knowledge Distillation framework that uses both ViT and CNN teachers to guide lightweight CNN students, addressing architectural mismatch in cross-architecture distillation.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers are accurate but computationally expensive, while lightweight CNNs are efficient but less accurate. Existing cross-architecture knowledge distillation methods struggle with architectural mismatches and overlook the value of strong CNN teachers.

Method: Proposes Dual-Teacher Knowledge Distillation with two components: 1) Discrepancy-Aware Teacher Weighting that dynamically fuses ViT and CNN teacher predictions based on confidence and student discrepancy, and 2) Structure Discrepancy-Aware Distillation where students learn residual features between ViT and CNN teachers via lightweight auxiliary branch.

Result: Extensive experiments on HMDB51, EPIC-KITCHENS-100, and Kinetics-400 show consistent outperformance over state-of-the-art distillation methods, with maximum accuracy gain of 5.95% on HMDB51.

Conclusion: The proposed dual-teacher framework effectively addresses architectural mismatch in cross-architecture knowledge distillation and achieves superior performance by leveraging both heterogeneous ViT and homogeneous CNN teachers collaboratively.
...
TL;DR: DreamPose3D is a diffusion-based 3D human pose estimation framework that combines action-aware reasoning with temporal imagination, achieving state-of-the-art performance by using action prompts, kinematic joint affinity modeling, and hallucinative pose decoding.


<details>
  <summary>Details</summary>
Motivation: Existing 3D pose estimation methods rely solely on geometric cues and predict poses independently, limiting their ability to resolve ambiguous motions and generalize to real-world scenarios. Humans understand motion through action intent and temporal reasoning, which current methods lack.

Method: DreamPose3D uses a diffusion-based framework that dynamically conditions denoising with action prompts extracted from 2D pose sequences. It incorporates kinematic joint affinity into attention mechanisms and employs a hallucinative pose decoder to predict temporally coherent 3D pose sequences.

Result: Extensive experiments on Human3.6M and MPI-3DHP datasets show state-of-the-art performance across all metrics. The method also demonstrated strong robustness on a broadcast baseball dataset, handling ambiguous and noisy 2D inputs effectively while maintaining temporal consistency.

Conclusion: DreamPose3D successfully addresses limitations of existing methods by integrating action-aware reasoning and temporal imagination, providing a more human-like approach to 3D pose estimation that handles ambiguity and improves generalization to real-world scenarios.
...
TL;DR: vMFCoOp is a framework that uses von Mises-Fisher distributions on a hyperspherical manifold to align semantic biases between LLMs and CLIP models for robust biomedical prompting and superior few-shot classification.


<details>
  <summary>Details</summary>
Motivation: To address semantic misalignment between LLMs and CLIP variants in biomedical applications, overcome scalability limitations across evolving foundation models, and improve multimodal alignment beyond conventional Euclidean-space optimization.

Method: Inversely estimates von Mises-Fisher distributions on a shared Hyperspherical Manifold, aligning semantic biases via Unified Semantic Anchors with three complementary constraints.
...
Conclusion: vMFCoOp provides a robust framework for biomedical prompting that achieves superior few-shot classification performance and will be expanded to more downstream applications.
...
TL;DR: RF-DETR introduces a lightweight specialist detection transformer that uses neural architecture search to find optimal accuracy-latency tradeoffs for any target dataset, achieving state-of-the-art performance on COCO and Roboflow100-VL.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary detectors perform well on COCO but struggle with real-world datasets containing out-of-distribution classes not seen during pre-training. Instead of fine-tuning heavy VLMs, the goal is to create efficient specialist detectors.

Method: Fine-tunes a pre-trained base network on target datasets and evaluates thousands of network configurations with different accuracy-latency tradeoffs using weight-sharing neural architecture search without retraining. Revisits tunable knobs for NAS to improve DETR transferability.

Result: RF-DETR significantly improves prior state-of-the-art: nano version achieves 48.0 AP on COCO (5.3 AP better than D-FINE at similar latency), 2x-large version outperforms GroundingDINO by 1.2 AP on Roboflow100-VL while running 20x faster, and is the first real-time detector to surpass 60 AP on COCO.

Conclusion: RF-DETR provides an effective approach for creating efficient specialist detectors with optimal accuracy-latency tradeoffs for diverse target domains, demonstrating superior performance over existing methods while maintaining real-time capabilities.
...
### [126] [Stabilizing Direct Training of Spiking Neural Networks: Membrane Potential Initialization and Threshold-robust Surrogate Gradient](https://arxiv.org/abs/2511.08708)
*Hyunho Kook,Byeongho Yu,Jeong Min Oh,Eunhyeok Park*

Main category: cs.NE

TL;DR: The paper introduces MP-Init and TrSG to address temporal covariate shift and unstable gradient flow in Spiking Neural Networks, achieving state-of-the-art accuracy on image datasets.


<details>
  <summary>Details</summary>
Motivation: Recent SNN advancements show high-quality outputs at early timesteps, but face challenges with temporal covariate shift and unstable gradient flow due to non-linearity and temporal dependencies.

Method: Two innovations: MP-Init aligns initial membrane potential with stationary distribution to address TCS, and TrSG stabilizes gradient flow with respect to threshold voltage during training.

Result: Extensive experiments validate the approach, achieving state-of-the-art accuracy on both static and dynamic image datasets.

Conclusion: The proposed MP-Init and TrSG methods effectively address key challenges in SNN training, enabling improved performance and energy-efficient AI paradigms.
...
### [127] [A Finite Difference Approximation of Second Order Regularization of Neural-SDFs](https://arxiv.org/abs/2511.08980)
*Haotian Yin,Aleksander Plocharski,Michal Jan Wlodarczyk,Przemyslaw Musialski*

Main category: cs.GR

TL;DR: A finite-difference framework for curvature regularization in neural SDF learning that replaces expensive second-order automatic differentiation with lightweight finite-difference stencils, reducing GPU memory usage and training time by up to 50% while maintaining comparable reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Existing curvature regularization methods for neural SDFs require computationally expensive second-order automatic differentiation or higher-order differentiation, which limits scalability and efficiency in training.

Method: Proposes finite-difference stencils based on Taylor expansion with O(h²) truncation error to approximate second derivatives, serving as drop-in replacements for Gaussian curvature and rank-deficiency losses without requiring explicit Hessian assembly.
...
Conclusion: The finite-difference formulation provides an efficient and scalable alternative for curvature-aware SDF learning, offering significant computational benefits without sacrificing reconstruction quality.
...
### [128] [End-to-end Contrastive Language-Speech Pretraining Model For Long-form Spoken Question Answering](https://arxiv.org/abs/2511.09282)
*Jiliang Hu,Zuchao Li,Baoyuan Qi,Liu Guoming,Ping Wang*

Main category: cs.SD

TL;DR: CLSR is an end-to-end contrastive language-speech retriever that efficiently extracts question-relevant segments from long audio for spoken question answering, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing speech-related retrievers struggle with processing long audio and have poor performance, limiting their effectiveness in long-form spoken question answering applications.

Method: CLSR incorporates an intermediate step that converts acoustic features into text-like representations before alignment, bridging the modality gap between speech and text more effectively than conventional contrastive models.
...
Conclusion: CLSR provides a robust foundation for advancing practical long-form spoken question answering applications by effectively extracting relevant segments from long audio recordings.
...
### [129] [SAMora: Enhancing SAM through Hierarchical Self-Supervised Pre-Training for Medical Images](https://arxiv.org/abs/2511.08626)
*Shuhang Chen,Hangjie Yuan,Pengwei Liu,Hanxue Gu,Tao Feng,Dong Ni*

Main category: eess.IV

TL;DR: SAMora is a framework that enhances medical image segmentation by applying self-supervised learning at image, patch, and pixel levels, achieving state-of-the-art performance with 90% fewer fine-tuning epochs.


<details>
  <summary>Details</summary>
Motivation: SAM's performance is limited with small labeled datasets, while hierarchical information in medical data is often overlooked.

Method: Proposes SAMora with complementary self-supervised learning objectives at image, patch, and pixel levels, and HL-Attn for hierarchical fusion of multi-scale features.

Result: Outperforms existing SAM variants on Synapse, LA, and PROMISE12 datasets in both few-shot and fully supervised settings.

Conclusion: SAMora effectively captures hierarchical medical knowledge and achieves superior performance with significantly reduced training time.
...
### [130] [Fluence Map Prediction with Deep Learning: A Transformer-based Approach](https://arxiv.org/abs/2511.08645)
*Ujunwa Mgboh,Rafi Sultan,Dongxiao Zhu,Joshua Kim*

Main category: eess.IV

TL;DR: Deep learning framework using 3D Swin-UNETR to predict fluence maps directly from CT images and contours for prostate IMRT, achieving high accuracy and clinical quality comparable to conventional planning.


<details>
  <summary>Details</summary>
Motivation: To accelerate fluence map generation in IMRT while maintaining clinical quality, addressing the time-consuming nature and planner dependency of conventional optimization methods.

Method: End-to-end 3D Swin-UNETR network trained on 99 prostate IMRT cases (79 training, 20 testing) to predict nine-beam fluence maps directly from volumetric CT images and anatomical contours using hierarchical self-attention.

Result: Achieved average R^2 of 0.95 ± 0.02, MAE of 0.035 ± 0.008, gamma passing rate of 85 ± 10% (3%/3mm), with no significant differences in DVH parameters between predicted and clinical plans.

Conclusion: The Swin-UNETR framework enables fully automated, inverse-free fluence map prediction with enhanced spatial coherence, accuracy, and efficiency, offering a scalable solution for automated IMRT plan generation.
...
### [131] [3D-TDA -- Topological feature extraction from 3D images for Alzheimer's disease classification](https://arxiv.org/abs/2511.08663)
*Faisal Ahmed,Taymaz Akan,Fatih Gelir,Owen T. Carmichael,Elizabeth A. Disbrow,Steven A. Conrad,Mohammad A. N. Bhuiyan*

Main category: eess.IV

TL;DR: A novel topological feature extraction method using persistent homology on brain MRI achieves high accuracy in Alzheimer's disease classification, outperforming deep learning models while being computationally efficient.
...
Method: Proposes persistent homology to extract topological features from structural MRI, converting them into feature vectors via Betti functions, then using XGBoost for classification.

Result: Achieved 97.43% accuracy and 99.09% sensitivity for binary classification, and 95.47% accuracy and 94.98% sensitivity for three-class classification using 10-fold cross-validation.

Conclusion: The topological approach outperforms deep learning models without requiring data augmentation or extensive preprocessing, and provides complementary information that could be combined with other models.
...
TL;DR: Proposes an efficient ROI-based deep image compression model with implicit bit allocation using a novel Mask-Guided Feature Enhancement module, achieving superior rate-distortion performance compared to explicit bit allocation methods.


<details>
  <summary>Details</summary>
Motivation: Existing ROI-based compression methods use explicit bit allocation with hard gating, which significantly impacts the statistical distribution of entropy models and limits coding performance.

Method: Uses a Mask-Guided Feature Enhancement module with Region-Adaptive Attention and Frequency-Spatial Collaborative Attention blocks for implicit bit allocation, plus dual decoders to separately reconstruct foreground and background images.

Result: Experiments on COCO2017 show the method significantly outperforms explicit bit allocation approaches in rate-distortion performance while maintaining satisfactory visual quality in background regions.

Conclusion: This is the first work to utilize implicit bit allocation for high-quality region-adaptive coding, demonstrating superior performance over traditional explicit approaches.
...
### [133] [Augment to Augment: Diverse Augmentations Enable Competitive Ultra-Low-Field MRI Enhancement](https://arxiv.org/abs/2511.09366)
*Felix F Zimmermann*

Main category: eess.IV

TL;DR: This paper studies how task-adapted data augmentations improve deep learning models for enhancing ultra-low-field (ULF) MRI images to match high-field appearance, achieving competitive results in the ULF-EnC challenge despite limited paired training data.


<details>
  <summary>Details</summary>
Motivation: ULF MRI offers broader accessibility but suffers from low SNR, poor resolution, and non-standard contrasts. Image-to-image translation can help but is limited by scarce paired training data between ULF and high-field MRI.

Method: The researchers used a standard deep learning model for ULF image enhancement and studied the impact of task-adapted data augmentations, including strong and diverse augmentations with auxiliary tasks on high-field data.

Result: The approach ranked third by brain-masked SSIM on the public validation leaderboard and fourth by the official score on the final test leaderboard of the ULF-EnC challenge.

Conclusion: Strong, diverse data augmentations, including auxiliary tasks on high-field data, substantially improve the fidelity of ULF MRI image enhancement when working with limited paired training data.
...
### [134] [Conversational Agents for Building Energy Efficiency -- Advising Housing Cooperatives in Stockholm on Reducing Energy Consumption](https://arxiv.org/abs/2511.08587)
*Shadaab Ghani,Anne Håkansson,Oleksii Pasichnyi,Hossein Shahrokni*

Main category: cs.CY

TL;DR: SPARA is a conversational agent system using Retrieval-Augmented Generation to provide energy efficiency advice to Swedish housing cooperatives, achieving 80% precision comparable to human experts.


<details>
  <summary>Details</summary>
Motivation: Swedish housing cooperative board members lack expertise to manage energy consumption, while EU directives prohibit energy-inefficient buildings by 2033. Conversational agents can bridge this knowledge gap.

Method: Uses Retrieval-Augmented Generation framework with a Language Model, trained on email communications between professional energy advisors and cooperatives' representatives in Stockholm.

Result: Preliminary results show 80% precision in energy efficiency advice, comparable to municipal energy efficiency experts. Currently undergoing pilot implementation with expert evaluation.

Conclusion: Language Models can significantly improve outreach by supporting stakeholders in energy transition, though more research is needed on stability and trustworthiness of advice.
...
### [135] [AI-generated podcasts: Synthetic Intimacy and Cultural Translation in NotebookLM's Audio Overviews](https://arxiv.org/abs/2511.08654)
*Jill Walker Rettberg*

Main category: cs.CY

TL;DR: Analysis of AI-generated podcasts by Google's NotebookLM reveals they use fixed templates and translate cultural contexts to a standardized white, educated, middle-class American default, marking a departure from diverse human podcasting practices.


<details>
  <summary>Details</summary>
Motivation: To analyze AI-generated podcasts as media rather than just tools, examining how they shape content and cultural representation compared to human-produced podcasts.

Method: Uploaded different types of text to NotebookLM and analyzed the generated podcast outputs, focusing on structure, cultural translation, and standardization patterns.

Result: Found that NotebookLM podcasts use fixed templates and translate all content into a standardized Mid-Western American accent and cultural context, erasing original cultural specificity and creating homogenized outputs.

Conclusion: AI-generated podcasts represent a distinct development in media shaping publics, moving away from multiple public spheres in human podcasting toward an abstracted, standardized genre that eliminates community-specific communication.
...
### [136] [OG-PCL: Efficient Sparse Point Cloud Processing for Human Activity Recognition](https://arxiv.org/abs/2511.08910)
*Jiuqi Yan,Chendong Xu,Dongyu Liu*

Main category: eess.SP

TL;DR: Proposed OG-PCL network for radar-based human activity recognition achieves 91.75% accuracy with only 0.83M parameters, outperforming existing methods on sparse 3D point cloud data.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave radar offers privacy-preserving and robust alternative to camera- and wearable-based human activity recognition, but requires efficient processing of sparse 3D point clouds.

Method: Occupancy-Gated Parallel-CNN Bi-LSTM (OG-PCL) network with tri-view parallel structure and Occupancy-Gated Convolution (OGConv) blocks for handling sparse radar point clouds.
...
Conclusion: OG-PCL provides a compact and accurate framework suitable for real-time radar-based human activity recognition on lightweight platforms.
...
### [137] [RadHARSimulator V2: Video to Doppler Generator](https://arxiv.org/abs/2511.09022)
*Weicheng Gao*

Main category: eess.SP

TL;DR: RadHARSimulator V2 generates Doppler spectra from video footage for radar-based human activity recognition, combining computer vision for pose estimation with radar simulation, and proposes a hybrid neural network for HAR.


<details>
  <summary>Details</summary>
Motivation: Existing radar-based HAR simulation methods are limited by model-based or motion-captured approaches, lacking flexibility for comprehensive simulation.

Method: The simulator uses computer vision for 2D/3D pose estimation from video, then radar simulation with pose interpolation, echo modeling, range-time/Doppler-time map generation, and feature extraction. A hybrid parallel-serial neural network is proposed for HAR.

Result: Numerical experiments demonstrate the effectiveness of both the simulator and the proposed network model for radar-based human activity recognition.

Conclusion: RadHARSimulator V2 provides a flexible and comprehensive simulation method for radar-based HAR by converting video footage to Doppler spectra, with promising experimental results.
...
### [138] ["It's trained by non-disabled people": Evaluating How Image Quality Affects Product Captioning with VLMs](https://arxiv.org/abs/2511.08917)
*Kapil Garg,Xinru Tang,Jimin Heo,Dwayne R. Morgan,Darren Gergle,Erik B. Sudderth,Anne Marie Piper*

Main category: cs.HC

TL;DR: Vision-Language Models (VLMs) perform well on perfect images (98% accuracy) but drop significantly to 75% accuracy when image quality issues like blur and misframing are present, with performance worsening as issues compound.


<details>
  <summary>Details</summary>
Motivation: VLMs are widely used by blind and low-vision (BLV) people for product identification, but there's limited understanding of how common image quality issues affect caption accuracy and whether they meet BLV users' information needs.

Method: Conducted a survey with 86 BLV people and systematically evaluated how image quality issues (blur, misframing) affect VLM-generated captions across different models.

Result: Best model achieved 98% accuracy on perfect images but dropped to 75% overall accuracy when quality issues were present, with performance deteriorating further as multiple issues compounded.

Conclusion: Model evaluations should center on disabled people's experiences, and researchers need to make VLMs more reliable for BLV users by addressing image quality sensitivity.
...
### [139] [Plug-and-Play Clarifier: A Zero-Shot Multimodal Framework for Egocentric Intent Disambiguation](https://arxiv.org/abs/2511.08971)
*Sicheng Yang,Yukai Huang,Weitong Cai,Shitong Sun,You He,Jiankang Deng,Hang Zhang,Jifei Song,Zhensong Zhang*

Main category: cs.HC

TL;DR: The paper introduces a modular Plug-and-Play Clarifier framework that resolves multimodal intent ambiguity in egocentric AI agents through three specialized modules for text, vision, and cross-modal clarification, achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing Vision-Language Models struggle with multimodal intent ambiguity caused by underspecified language, imperfect visual data, and deictic gestures, leading to task failures and hallucinated responses.

Method: A zero-shot modular framework with three modules: text clarifier for dialogue-driven linguistic disambiguation, vision clarifier for real-time positioning guidance, and cross-modal clarifier with grounding mechanism for 3D pointing gesture interpretation.

Result: Improves intent clarification performance of small language models (4-8B) by ~30%, making them competitive with larger models. Vision clarifier increases corrective guidance accuracy by over 20%, and cross-modal clarifier improves semantic answer accuracy for referential grounding by 5%.

Conclusion: The Plug-and-Play Clarifier effectively resolves multimodal ambiguity and significantly enhances user experience in egocentric interaction through its modular, zero-shot framework.
...
### [140] [Expand Your SCOPE: Semantic Cognition over Potential-Based Exploration for Embodied Visual Navigation](https://arxiv.org/abs/2511.08935)
*Ningnan Wang,Weihuang Chen,Liming Chen,Haoxuan Ji,Zhongyu Guo,Xuchong Zhang,Hongbin Sun*

Main category: cs.RO

TL;DR: SCOPE is a zero-shot embodied navigation framework that uses frontier-based potential exploration and self-reconsideration to improve long-horizon planning, achieving 4.6% higher accuracy than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot navigation methods overlook visual frontier boundaries and fail to infer relationships between partial observations and goals, limiting their planning performance in unknown environments.

Method: SCOPE leverages frontier information for potential-based exploration using Vision-Language Models, organizes exploration potential into spatio-temporal graphs, and incorporates self-reconsideration to refine prior decisions.

Result: SCOPE outperforms state-of-the-art baselines by 4.6% in accuracy on embodied navigation tasks, with improved calibration, generalization, and decision quality.

Conclusion: Explicitly leveraging frontier boundaries and incorporating self-reconsideration mechanisms significantly enhances zero-shot embodied navigation performance and reliability.
...
TL;DR: UniMM-V2X is an end-to-end multi-agent framework for autonomous driving that enables hierarchical cooperation across perception, prediction, and planning through multi-level fusion and Mixture-of-Experts architecture.


<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems have limited perception and isolated decision-making. While multi-agent approaches exist, they often focus only on perception-level tasks without aligning with downstream planning and control, or fail to leverage full end-to-end capabilities.

Method: Uses a multi-level fusion strategy unifying perception and prediction cooperation, allowing agents to share queries and reason cooperatively. Incorporates Mixture-of-Experts (MoE) architecture to dynamically enhance BEV representations and extend MoE into the decoder to capture diverse motion patterns.

Result: Achieves state-of-the-art performance on DAIR-V2X dataset: 39.7% improvement in perception accuracy, 7.2% reduction in prediction error, and 33.2% improvement in planning performance compared to UniV2X.

Conclusion: The MoE-enhanced multi-level cooperative paradigm demonstrates significant improvements across all autonomous driving tasks, showcasing the strength of hierarchical cooperation in end-to-end multi-agent systems.
...
### [142] [SMF-VO: Direct Ego-Motion Estimation via Sparse Motion Fields](https://arxiv.org/abs/2511.09072)
*Sangheon Yang,Yeongin Yoon,Hong Mo Jung,Jongwoo Lim*

Main category: cs.RO

TL;DR: SMF-VO is a lightweight visual odometry method that directly estimates camera velocity from sparse optical flow, eliminating the need for explicit pose estimation and landmark tracking, achieving over 100 FPS on Raspberry Pi 5.


<details>
  <summary>Details</summary>
Motivation: Traditional VO/VIO methods are computationally expensive due to pose-centric paradigm requiring large-scale landmark maintenance and map optimization, limiting real-time performance on resource-constrained devices.

Method: Proposes a motion-centric framework that directly estimates instantaneous linear and angular velocity from sparse optical flow using a generalized 3D ray-based motion field formulation compatible with various camera models.

Result: Demonstrates superior efficiency and competitive accuracy on benchmark datasets, achieving over 100 FPS on Raspberry Pi 5 using only CPU.

Conclusion: Establishes a scalable and efficient alternative to conventional methods, making it highly suitable for mobile robotics and wearable devices.
...
### [143] [SPIDER: Scalable Physics-Informed Dexterous Retargeting](https://arxiv.org/abs/2511.09484)
...
TL;DR: SPIDER is a physics-based retargeting framework that transforms human motion data into dynamically feasible robot trajectories, addressing the data scarcity problem in dexterous robot control.


<details>
  <summary>Details</summary>
Motivation: Collecting robot-specific data for dexterous and humanoid control is expensive, while abundant human motion data exists but cannot be directly used due to embodiment gaps and missing dynamic information.

Method: Physics-based retargeting that uses human demonstrations for task structure and objectives, combined with large-scale sampling and curriculum-style virtual contact guidance to ensure dynamic feasibility.

Result: SPIDER scales across 9 humanoid/dexterous hand embodiments and 6 datasets, improving success rates by 18% over standard sampling, being 10X faster than RL baselines, and generating a 2.4M frames dataset.

Conclusion: SPIDER serves as a universal physics-based retargeting method that works with diverse quality data and generates high-quality robot data for efficient policy learning.
...
### [144] [MAP-VLA: Memory-Augmented Prompting for Vision-Language-Action Model in Robotic Manipulation](https://arxiv.org/abs/2511.09516)
*Runhao Li,Wenkai Guo,Zhenyu Wu,Changyuan Wang,Haoyuan Deng,Zhenyu Weng,Yap-Peng Tan,Ziwei Wang*

Main category: cs.RO

TL;DR: MAP-VLA enhances pre-trained Vision-Language-Action models with memory prompts from demonstrations to improve performance on long-horizon robotic manipulation tasks, achieving up to 25% gains on real robots.


<details>
  <summary>Details</summary>
Motivation: Pre-trained VLA models struggle with long-horizon tasks due to lack of memory and reliance on immediate sensory inputs only.

Method: Constructs memory library from historical demonstrations as learnable soft prompts, retrieves relevant memory via trajectory similarity matching, and integrates it into frozen VLA model for augmented action generation.
...
Conclusion: MAP-VLA provides a lightweight plug-and-play solution that significantly improves VLA model performance on complex long-horizon robotic manipulation tasks through memory augmentation.
...
### [145] [SpatialActor: Exploring Disentangled Spatial Representations for Robust Robotic Manipulation](https://arxiv.org/abs/2511.09555)
*Hao Shi,Bin Xie,Yingfei Liu,Yang Yue,Tiancai Wang,Haoqiang Fan,Xiangyu Zhang,Gao Huang*

Main category: cs.RO

TL;DR: SpatialActor is a disentangled framework for robotic manipulation that separates semantics and geometry, using semantic-guided geometry fusion and spatial transformers to handle depth noise and improve precision.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from sparse sampling in point-based approaches and sensitivity to depth noise in image-based methods, while overlooking low-level spatial cues needed for precise robotic interaction.

Method: Proposes a disentangled framework with Semantic-guided Geometric Module that fuses noisy depth with semantic priors, and a Spatial Transformer that leverages low-level spatial cues for accurate 2D-3D mapping.

Result: Achieves 87.4% on RLBench with 13.9-19.4% improvement under noisy conditions, shows strong robustness and enhanced few-shot generalization to new tasks.

Conclusion: SpatialActor provides a robust solution for robotic manipulation by explicitly decoupling semantics and geometry, demonstrating superior performance and generalization across various conditions.
...
### [146] [IFG: Internet-Scale Guidance for Functional Grasping Generation](https://arxiv.org/abs/2511.09558)
*Ray Muxin Liu,Mingxuan Li,Kenneth Shaw,Deepak Pathak*

Main category: cs.RO

TL;DR: Combines internet-scale vision models' semantic understanding with simulation-based geometric precision for real-time 3D grasping using diffusion models on camera point clouds.
...
Method: Uses simulation with force-closure grasping generation pipeline to understand local hand-object geometries, then distills this data into a diffusion model for real-time operation on camera point clouds.
...
Conclusion: Successfully bridges the gap between semantic understanding and geometric precision for robotic grasping through simulation-based training and diffusion model distillation.
...
### [147] [MicroEvoEval: A Systematic Evaluation Framework for Image-Based Microstructure Evolution Prediction](https://arxiv.org/abs/2511.08955)
*Qinyi Zhang,Duanyu Feng,Ronghui Han,Yangshuai Wang,Hao Wang*

Main category: cond-mat.mtrl-sci

TL;DR: MicroEvoEval is the first comprehensive benchmark for microstructure evolution prediction, evaluating 14 models across 4 tasks with multi-faceted metrics including physical fidelity and long-term stability.


<details>
  <summary>Details</summary>
Motivation: Address gaps in microstructure evolution simulation: lack of standardized benchmarks, insufficient comparison between specialized and general models, overemphasis on numerical accuracy over physical fidelity, and failure to analyze error propagation over time.

Method: Developed MicroEvoEval benchmark with 4 representative tasks and datasets for short- and long-term assessment. Evaluated 14 models including domain-specific and general-purpose architectures using multi-faceted metrics (numerical accuracy, computational cost, structure-preserving metrics for physical fidelity).

Result: Modern architectures (e.g., VMamba) achieve superior long-term stability and physical fidelity with order-of-magnitude greater computational efficiency compared to other models.

Conclusion: Holistic evaluation is necessary, and modern architectures represent a promising direction for developing efficient and reliable surrogate models in data-driven materials science.
...
### [148] [Moving pattern-based modeling using a new type of interval ARX model](https://arxiv.org/abs/2307.04402)
*Changping Sun*

Main category: stat.ME

TL;DR: A new interval ARX (IARX) model is proposed to handle interval data, overcoming limitations of traditional ARX models, and applied to moving pattern-based modeling with superior performance demonstrated in sintering process simulations.


<details>
  <summary>Details</summary>
Motivation: To overcome the shortcomings of traditional ARX models in handling interval data and improve modeling robustness against parameter variations.

Method: Defined a new operator between interval numbers and real matrices, applied it to traditional ARX model to create IARX model, then implemented moving pattern-based modeling approach.

Result: Simulation results show the IARX-based moving pattern modeling is robust to parameter variations and outperforms previous modeling methods.

Conclusion: The proposed IARX model effectively handles interval data and provides superior modeling performance compared to existing approaches, particularly in industrial processes like sintering.
...
### [149] [Spatio-Temporal Data Enhanced Vision-Language Model for Traffic Scene Understanding](https://arxiv.org/abs/2511.08978)
*Jingtian Ma,Jingyuan Wang,Wayne Xin Zhao,Guoping Liu,Xiang Wen*

Main category: cs.MM

TL;DR: Proposes ST-CLIP, a spatio-temporal enhanced model based on CLIP for Traffic Scene Understanding, incorporating spatio-temporal context through prompt learning to improve scene analysis.


<details>
  <summary>Details</summary>
Motivation: Current Traffic Scene Understanding methods treat it as common image understanding, ignoring spatio-temporal information and interrelations between different aspects of traffic scenes.

Method: Uses CLIP as backbone with SCAMP learning method: dynamic spatio-temporal context representation module and bi-level ST-aware multi-aspect prompt learning module that integrates ST-context into word embeddings.
...
Conclusion: First attempt to integrate spatio-temporal information into vision-language models for Traffic Scene Understanding, showing effectiveness in leveraging both visual-textual and spatio-temporal data.
...
### [150] [Routesplain: Towards Faithful and Intervenable Routing for Software-related Tasks](https://arxiv.org/abs/2511.09373)
*Adam Štorek,Vikas Upadhyay,Marianne Menglin Liu,Daniel W. Peterson,Anshul Mittal,Sujeeth Bharadwaj,Fahad Shah,Dan Roth*

Main category: cs.SE

TL;DR: Routesplain is an interpretable LLM router for software tasks that extracts human-readable concepts from queries to intelligently route to appropriate LLMs, outperforming individual models and matching black-box baselines while reducing costs.


<details>
  <summary>Details</summary>
Motivation: LLM performance varies significantly across software tasks, and existing routing approaches use black-box models without interpretability. There's a need for an intelligible router specifically designed for software-related tasks.

Method: Routesplain extracts human-interpretable concepts (task, domain, reasoning complexity) from queries and routes based on these concepts rather than using black-box models, providing faithful rationales for routing decisions.

Result: Routesplain outperforms individual LLMs in accuracy and cost across 16 state-of-the-art models and 8 software tasks, and equals or surpasses all black-box baseline routers while offering interpretability.

Conclusion: Concept-based routing provides an effective, interpretable approach for LLM selection in software tasks, with concept-level interventions showing potential for further router improvements.
...
### [151] [The Double Contingency Problem: AI Recursion and the Limits of Interspecies Understanding](https://arxiv.org/abs/2511.08927)
*Graham L. Bishop*
...
<details>
  <summary>Details</summary>
Motivation: Current bioacoustic AI systems achieve cross-species performance but fail to address what happens when AI's recursive cognition encounters animals' recursive communicative processes, potentially obscuring or distorting other species' communication structures.

Method: Drawing on philosopher Yuk Hui's work on recursivity and contingency, the paper analyzes how AI systems as recursive cognitive agents may systematically affect animal communication processing, creating a double contingency problem between species' evolutionary conditions and AI's architectural conditions.

Result: The analysis reveals that AI systems are not neutral pattern detectors but recursive cognitive agents whose information processing may distort other species' communicative structures through the double contingency of ecological/evolutionary conditions and AI architectural/training conditions.
...
### [152] [AI Founding Fathers: A Case Study of GIS Search in Multi-Agent Pipelines](https://arxiv.org/abs/2511.09005)
*Alvin Chauhan*
...
<details>
  <summary>Details</summary>
Motivation: To extract stronger reasoning capabilities from LLMs by structuring multi-agent pipelines that ensure controlled, incremental search through the problem space.

Method: Used recursive refinement (self-criticism, adversarial stress-testing, critical feedback integration) in multi-agent pipelines with historical personas (Hamilton, Jefferson, Madison) to address political issues, comparing simple vs. complex pipeline architectures.

Result: Complex model with recursive refinement consistently outperformed simple model across all test cases (average score 88.3 vs 71.7), producing arguments with superior analytical depth, structural nuance, and strategic framing.
...
### [153] [Solving a Million-Step LLM Task with Zero Errors](https://arxiv.org/abs/2511.09030)
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with extended processes due to persistent error rates that prevent scaling beyond a few hundred steps, despite their strong reasoning and tool-use capabilities.
...
Conclusion: Massively decomposed agentic processes (MDAPs) offer a viable path to solve organizational and societal-level problems efficiently, rather than relying solely on continual LLM improvement.
...
### [154] [History-Aware Reasoning for GUI Agents](https://arxiv.org/abs/2511.09127)
*Ziwei Wang,Leyang Yang,Xiaoxuan Tang,Sheng Zhou,Dajun Chen,Wei Jiang,Yong Li*

Main category: cs.AI

TL;DR: Proposes History-Aware Reasoning (HAR) framework to enhance GUI agents' episodic reasoning by addressing their weak short-term memory and history-agnostic reasoning in long-horizon GUI tasks.


<details>
  <summary>Details</summary>
Motivation: Current GUI agents have weak short-term memory and interpret interactions as discrete screen understanding, unaware of historical interactions within episodes, which challenges their performance in GUI automation.
...
Result: Comprehensive evaluations across GUI benchmarks demonstrate effectiveness and generalization, equipping GUI agents with stable short-term memory and reliable screen detail perception.

Conclusion: HAR framework successfully addresses the history-agnostic reasoning limitation in GUI agents, enhancing their episodic reasoning capabilities for improved GUI automation performance.
...
### [155] [Not Everything That Counts Can Be Counted: A Case for Safe Qualitative AI](https://arxiv.org/abs/2511.09325)
...
<details>
  <summary>Details</summary>
Motivation: Qualitative research has been left behind in AI adoption, forcing researchers to use general-purpose tools like ChatGPT despite their limitations (biased, opaque, irreproducible, privacy-compromising), creating a critical gap in scientific discovery.

Method: Argue for developing dedicated qualitative AI systems built from the ground up for interpretive research, and review recent literature to show how automated discovery pipelines could be enhanced by robust qualitative capabilities.
...
Conclusion: There is a critical need to develop specialized qualitative AI systems that address the unique requirements of interpretive research while maintaining transparency, reproducibility, and privacy protection.
...
### [156] [Accelerating Training Speed of Tiny Recursive Models via Curriculum Guided Adaptive Recursion](https://arxiv.org/abs/2511.08653)
*Kaleem Ullah Qasim,Jiashu Zhang*

Main category: cs.LG

TL;DR: CGAR introduces curriculum learning for architectural depth in recursive reasoning models, achieving 1.71x training speedup with minimal accuracy loss through Progressive Depth Curriculum and Hierarchical Supervision Weighting.


<details>
  <summary>Details</summary>
Motivation: Training recursive reasoning models is computationally expensive (36 GPU-hours per dataset), limiting adoption. CGAR aims to reduce training costs while maintaining performance.

Method: Two components: Progressive Depth Curriculum (dynamic recursion depth adjustment) and Hierarchical Supervision Weighting (exponentially decaying supervision importance).

Result: 42% training cost reduction (10.93 to 6.38 hours) with only 0.63% accuracy drop on Sudoku-Extreme. Models show 100% halting accuracy and 11% fewer reasoning steps.

Conclusion: Principled curriculum on architectural depth enables efficient training of recursive reasoning models on modest hardware while maintaining performance.
...
TL;DR: Proposes LLM-ABS framework to study LLM agent behavior in dictator games, addressing prompt sensitivity and providing robust analysis of fairness preferences and linguistic patterns.


<details>
  <summary>Details</summary>
Motivation: Questioning robustness of previous LLM behavior studies that overlook system prompt influence and prompt sensitivity, highlighting need for reliable baseline when studying complex behavioral aspects.

Method: LLM-ABS framework with three components: (i) exploring system prompt influence, (ii) using neutral prompt variations for reliable insights, (iii) analyzing linguistic features in responses to understand reasoning.

Result: Agents show strong preference for fairness, system prompts significantly impact behavior, and models express responses differently linguistically. Prompt sensitivity remains a challenge.

Conclusion: The proposed framework provides a robust foundation for LLM agent behavior studies despite persistent prompt sensitivity issues.
...
### [158] [BayesQ: Uncertainty-Guided Bayesian Quantization](https://arxiv.org/abs/2511.08821)
*Ismail Lamaakal,Chaymae Yahyati,Yassine Maleh,Khalid El Makkaoui,Ibrahim Ouahbi*

Main category: cs.LG

TL;DR: BayesQ is an uncertainty-guided post-training quantization framework that optimizes quantization under posterior expected loss, improving performance over strong PTQ baselines at low bit rates.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of low-bit quantization by reframing it as uncertainty-aware risk minimization, leveraging Bayesian principles to guide the quantization process for better performance.
...
Result: At 3.0/3.5/4.0 bits/weight, BayesQ outperforms GPTQ by +1.5/+0.7/+0.3 top-1 percentage points on ResNet-50 and +1.1/+0.4/+0.2 GLUE points on BERT-base.

Conclusion: BayesQ provides a practical, uncertainty-aware approach to post-training quantization that significantly improves performance over existing methods while requiring comparable preprocessing effort.
...
### [159] [TransactionGPT](https://arxiv.org/abs/2511.08939)
...
TL;DR: TransactionGPT (TGPT) is a foundation model for consumer transaction data that uses a novel 3D-Transformer architecture to understand and generate transaction trajectories while supporting various downstream tasks, achieving superior performance over production models and fine-tuned LLMs.


<details>
  <summary>Details</summary>
Motivation: To develop a specialized foundation model for consumer transaction data that can understand complex payment dynamics and support multiple downstream tasks more effectively than existing approaches.

Method: Proposed a novel 3D-Transformer architecture with innovations for modality fusion and computational efficiency, trained on billion-scale real-world transactions and optimized jointly with downstream objectives.

Result: TGPT significantly improves downstream classification performance over production models, shows advantages in generating future transactions, achieves superior predictive accuracy compared to fine-tuned LLMs, and demonstrates faster training and inference.

Conclusion: The architectural innovations and practical guidelines from TGPT advance foundation models for transaction-like data and are expected to catalyze future research in this emerging field.
...
### [160] [Bayesian Mixture of Experts For Large Language Models](https://arxiv.org/abs/2511.08968)
*Maryam Dialameh,Hossein Rajabzadeh,Weiwei Zhang,Walid Ahmed,Hyock Ju Kwon*

Main category: cs.LG

TL;DR: Bayesian Mixture of Experts (Bayesian-MoE) is a post-hoc uncertainty estimation framework for fine-tuned LLMs that applies structured Laplace approximation to MoE expert pathways for calibrated uncertainty without modifying training.


<details>
  <summary>Details</summary>
Motivation: Existing approaches apply Bayesian inference to added adapter modules, but Bayesian-MoE directly targets the expert pathways already present in MoE models to leverage their modular design for tractable uncertainty estimation.

Method: Applies structured Laplace approximation to the second linear layer of each expert in MoE models, using Kronecker-factored low-rank approximations to model curvature and derive scalable estimates of predictive uncertainty and marginal likelihood.

Result: Experiments on common-sense reasoning benchmarks with Qwen1.5-MoE and DeepSeek-MoE show improved expected calibration error (ECE) and negative log-likelihood (NLL) over baselines.

Conclusion: Bayesian-MoE effectively enables reliable downstream decision-making through calibrated uncertainty estimation in MoE models without requiring modifications to the original training procedure.
...
### [161] [AdaCuRL: Adaptive Curriculum Reinforcement Learning with Invalid Sample Mitigation and Historical Revisiting](https://arxiv.org/abs/2511.09478)
...
Conclusion: AdaCuRL effectively addresses key challenges in RL for LLM reasoning through adaptive curriculum learning and specialized strategies, leading to consistent performance improvements across various models and benchmarks.
...
### [162] [Fast $k$-means clustering in Riemannian manifolds via Fréchet maps: Applications to large-dimensional SPD matrices](https://arxiv.org/abs/2511.08993)
*Ji Shi,Nicolas Charon,Andreas Mang,Demetrio Labate,Robert Azencott*

Main category: cs.LG

TL;DR: Efficient clustering framework for high-dimensional manifolds using p-Fréchet map to embed data into Euclidean space, enabling fast k-means clustering with 100x speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Standard intrinsic clustering methods on high-dimensional non-Euclidean manifolds face computational challenges and inefficiency, particularly for complex manifolds like SPD(n) matrices.

Method: Use p-Fréchet map F^p to embed manifold data into lower-dimensional Euclidean space using reference points, then apply standard Euclidean clustering techniques like k-means.

Result: Achieves up to two orders of magnitude runtime reduction compared to intrinsic manifold approaches while maintaining high clustering accuracy, even in challenging scenarios where other methods fail.

Conclusion: The p-Fréchet map provides an efficient and accurate framework for manifold clustering by transforming the problem into Euclidean space, offering significant computational advantages without sacrificing performance.
...
### [163] [FSampler: Training Free Acceleration of Diffusion Sampling via Epsilon Extrapolation](https://arxiv.org/abs/2511.09180)
*Michael A. Vladimir*
...
<details>
  <summary>Details</summary>
Motivation: To accelerate diffusion model sampling by reducing the number of expensive function evaluations (model calls) while maintaining high fidelity, without requiring retraining or modifying sampler formulas.

Method: Maintains a history of denoising signals (epsilon) and uses finite difference predictors (2nd-4th order) to extrapolate next epsilon values. Substitutes predicted epsilons on selected steps with validation, learning stabilizers, and protective mechanisms like windows and periodic anchors.

Result: Reduces time by 8-22% and model calls by 15-25% at high fidelity (SSIM 0.95-0.99). With aggressive settings, achieves 45-50% fewer model calls at lower fidelity (SSIM 0.73-0.74).

Conclusion: FSampler effectively accelerates diffusion sampling across various samplers without altering their core formulas, providing significant computational savings while maintaining output quality.
