...
TL;DR: First benchmark for LLMs in women's health shows ~60% failure rates across 13 models, revealing critical gaps in accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: LLMs are becoming primary health information sources for millions, but their accuracy in women's health remains critically unexamined, creating potential risks for users seeking reliable medical advice.

Method: Created Women's Health Benchmark (WHB) with 96 rigorously validated model stumps covering 5 medical specialties, 3 query types, and 8 error types. Evaluated 13 state-of-the-art LLMs across this comprehensive framework.

Result: Models show approximately 60% failure rates on women's health benchmark. Performance varies dramatically across specialties and error types. Models universally struggle with "missed urgency" indicators, while newer models like GPT-5 show improvements in avoiding inappropriate recommendations.

Conclusion: AI chatbots are not yet fully capable of providing reliable advice in women's health, highlighting the need for specialized benchmarks and improved model training in this critical domain.
...
TL;DR: Struct-SQL: A knowledge distillation framework that uses structured query execution plans instead of unstructured CoT traces to train small language models for Text-to-SQL, achieving 8.1% improvement over unstructured CoT baselines.


<details>
  <summary>Details</summary>
Motivation: Enterprise Text-to-SQL deployment faces a trilemma of cost, security, and performance. Current solutions force choice between expensive proprietary LLMs and low-performing SLMs. Unstructured CoT distillation is ambiguous, while structured reasoning provides clearer teaching signals for precise SQL generation.

Method: Propose Struct-SQL, a knowledge distillation framework that trains SLMs to emulate large LLMs using query execution plans as formal blueprints for structured reasoning instead of unstructured CoT traces.
...
Conclusion: Teaching models to reason using structured logical blueprints is beneficial for reliable SQL generation in SLMs, demonstrating formal structured reasoning provides clearer, more reliable teaching signals than unstructured CoT.
...
TL;DR: XLM package provides standardized implementation framework for non-autoregressive language models to enable systematic comparisons and component reuse.


<details>
  <summary>Details</summary>
Motivation: Non-autoregressive text generation lacks standardized implementations compared to autoregressive approaches, making systematic comparisons difficult and requiring bespoke implementations for each model.

Method: Developed XLM Python package with standardized components for data collation, loss functions, and prediction logic, plus companion xlm-models package with pre-trained small models.

Result: Created open-source implementation (https://github.com/dhruvdcoder/xlm-core) that simplifies development of non-autoregressive language models and provides reusable components.

Conclusion: XLM package addresses the implementation gap in non-autoregressive language modeling, facilitating easier experimentation and comparison while providing pre-trained models for community use.
...
TL;DR: SPECTRA is a watermarking method that makes training data detectable even when it's less than 0.001% of the training corpus, using LLM paraphrasing and scoring models to avoid distribution shifts.


<details>
  <summary>Details</summary>
Motivation: The need for effective training data detection to enforce copyright and data licensing as LLMs are trained on massive internet-scraped corpora.

Method: Paraphrase text using an LLM, assign scores based on likelihood of each paraphrase via a separate scoring model, choose paraphrases with scores matching original text to avoid distribution shifts, then compare suspect model's token probabilities against scoring model.

Result: Achieves consistent p-value gap of over nine orders of magnitude between training data detection vs non-training data, outperforming all baselines.

Conclusion: SPECTRA provides data owners with a scalable, deploy-before-release watermark that survives large-scale LLM training, enabling reliable training data detection.
...
TL;DR: The paper critiques current evaluation metrics for dialogue topic segmentation and proposes a new evaluation framework focusing on boundary density and segment coherence rather than strict boundary matching.


<details>
  <summary>Details</summary>
Motivation: Current evaluation practice in dialogue topic segmentation relies on strict boundary matching and F1 metrics, which are inadequate for modern LLM-based conversational systems that use segmentation to manage conversation history beyond fixed context windows. Existing metrics fail to capture the true quality of segmentation in practical applications.

Method: The paper introduces a new evaluation objective that treats boundary density and segment coherence as primary criteria, alongside window-tolerant F1 (W-F1). It evaluates multiple dialogue segmentation strategies across eight diverse dialogue datasets (task-oriented, open-domain, meeting-style, synthetic) and separates boundary scoring from boundary selection.

Result: Performance differences across dialogue segmentation benchmarks are driven by annotation granularity mismatches and sparse boundary labels rather than model quality. Many reported improvements are evaluation artifacts. Topic segmentation should be understood as selecting appropriate granularity rather than predicting a single correct boundary set.

Conclusion: Dialogue topic segmentation evaluation needs to move beyond strict boundary matching metrics. The proposed framework focusing on boundary density and segment coherence provides a more meaningful assessment, recognizing that segmentation involves selecting appropriate granularity rather than finding a single "correct" boundary set.
...
TL;DR: Two-level data augmentation (synthetic + real) improves intent classification for smoking cessation chatbots by 32% F1 score when data is scarce.


<details>
  <summary>Details</summary>
Motivation: Online smoking cessation support groups have low engagement and stigma issues. Conversational agents could help but lack sufficient high-quality training data for intent classification.

Method: Two-level data augmentation: 1) Synthetic - fine-tuned LLM to identify low-F1 intents, used GPT to generate synthetic posts (87% high quality), 2) Real - scraped 10,000+ related posts (73% validated). Combined with original data to retrain intent classifier.

Result: 43% of original posts augmented, 140% synthetic expansion. Retrained model showed 32% F1 improvement. Both synthetic and real augmentation provided similar performance gains.

Conclusion: Provides replicable framework for enhancing conversational agents in data-scarce domains. Data augmentation effectively improves intent classification for smoking cessation support chatbots.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of maintaining factual consistency and faithfulness in long-form summarization, especially for information-dense documents where important details can be lost or distorted.

Method: Uses self-planning methods to identify important content as sentence-level highlights, then generates summaries conditioned on these content plans. Explores both end-to-end and two-stage variants, with the two-stage pipeline performing better for long documents.

Result: Consistently improves factual consistency while preserving relevance and overall quality. On GovReport dataset: improved ROUGE-L by 4.1 points and achieved ~35% gains in SummaC scores. Qualitative analysis shows better preservation of important details and more accurate, insightful summaries across domains.

Conclusion: Highlight-guided generation effectively enhances traceability and faithfulness in long context summarization, with the two-stage pipeline being particularly effective for long, information-dense documents.
...
TL;DR: MiA-RAG introduces mindscape-aware retrieval-augmented generation that builds hierarchical global semantic representations to improve long-context understanding, outperforming standard RAG systems.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems lack holistic semantic guidance for long documents, struggling with tasks requiring global context awareness that humans naturally possess through mindscape-aware capabilities.

Method: MiA-RAG builds mindscapes through hierarchical summarization, then conditions both retrieval (enriched query embeddings) and generation (reasoning within global context) on this global semantic representation.

Result: MiA-RAG consistently surpasses baselines across diverse long-context and bilingual benchmarks for evidence-based understanding and global sense-making, aligning local details with coherent global representations.

Conclusion: Explicit global context awareness through mindscape-aware RAG enables more human-like long-context retrieval and reasoning by providing coherent semantic guidance for both retrieval and generation components.
...
TL;DR: A noise-sensitive ASR error correction framework for Persian speech that uses multiple hypotheses and Error Level Noise (ELN) embeddings to reduce word error rates in noisy environments.


<details>
  <summary>Details</summary>
Motivation: ASR systems degrade significantly in noisy environments, especially for low-resource languages like Persian. Even state-of-the-art models like Whisper struggle with varying noise levels, creating a need for robust error correction methods.

Method: Generate 5-best hypotheses from modified Whisper-large decoder, introduce Error Level Noise (ELN) to capture semantic/token-level disagreement across hypotheses, and evaluate three models: base LLaMA-2-7B, fine-tuned text-only variant, and noise-conditioned model with ELN embeddings at sentence/word levels.

Result: ELN-conditioned model achieved substantial WER reductions: from 31.10% (Raw Whisper) to 24.84% on Mixed Noise test set, significantly outperforming fine-tuned text-only baseline (30.79%). Base LLaMA-2-7B increased WER to 64.58%, showing inability to correct Persian errors alone.

Conclusion: Combining multiple hypotheses with noise-aware ELN embeddings effectively enables robust Persian ASR in noisy real-world scenarios, demonstrating the importance of noise-conditioned modeling for low-resource language ASR systems.
...
TL;DR: Seed-Prover 1.5 is a formal theorem-proving model using large-scale agentic reinforcement learning with efficient test-time scaling, achieving state-of-the-art performance on undergraduate to PhD-level math problems with less compute.


<details>
  <summary>Details</summary>
Motivation: While LLMs have advanced mathematical proof generation, formal theorem proving in languages like Lean remains challenging and computationally expensive, especially for undergraduate-level problems and beyond.

Method: Uses large-scale agentic reinforcement learning where the model interacts with Lean and other tools to accumulate experience, plus an efficient test-time scaling workflow that bridges natural and formal languages.

Result: Achieves 88% on PutnamBench (undergraduate), 80% on Fate-H (graduate), and 33% on Fate-X (PhD-level). Solved 11/12 problems from Putnam 2025 within 9 hours, outperforming state-of-the-art methods with smaller compute budget.

Conclusion: Scaling learning from experience with high-quality formal feedback shows immense potential for the future of formal mathematical reasoning.
...
TL;DR: AutoMetrics is a framework that synthesizes evaluation metrics for AI applications using retrieval from a curated metric bank and LLM-generated criteria, optimized via regression to correlate with human feedback using minimal data.


<details>
  <summary>Details</summary>
Motivation: Evaluating user-facing AI applications in open-ended domains is challenging due to scarce user feedback and slow behavioral signals, especially in prototypes and research projects where traditional evaluation methods are impractical.
...
Conclusion: AutoMetrics provides a practical framework for synthesizing interpretable automatic metrics from minimal human feedback, accelerating adaptive evaluation of LLM applications, with the toolkit and MetricBank released to the community.
...
TL;DR: This paper aims to develop NLP models for automated subjective question generation and answer evaluation from text input, addressing gaps in current educational technology.


<details>
  <summary>Details</summary>
Motivation: While NLP has revolutionized many fields including education, most research has focused on objective question generation. Automated subjective question generation and answer evaluation remain underdeveloped areas that could significantly benefit teachers and students by automating assessment and enabling self-evaluation.

Method: The research proposes to either improve existing NLP models or develop novel ones specifically designed for generating subjective questions and evaluating answers from text input.

Result: The paper does not present specific results in the abstract, but outlines the research goal of creating an automated system for subjective question generation and answer evaluation.

Conclusion: Developing automated systems for subjective question generation and answer evaluation has significant potential to enhance educational assessment and student learning experiences by reducing teacher workload and enabling self-assessment capabilities.
...
TL;DR: Hybrid fine-tuning framework combining gradient-aligned low-rank updates with structured orthogonal transformations for multilingual LLM adaptation, enhanced by lightweight data governance steps.


<details>
  <summary>Details</summary>
Motivation: To enable resource-efficient multilingual adaptation of large language models for low-resource settings while maintaining accuracy, calibration, and cross-language parity under tight compute budgets.

Method: Governance-aware hybrid fine-tuning framework with: 1) Core algorithm combining gradient-aligned low-rank updates with structured orthogonal transformations through layer-wise mixing, 2) Unitary constraints in selected sub-layers to stabilize deep optimization, 3) Lightweight label-free data governance steps (language identification, near-duplicate removal, quality filtering).

Result: Consistent gains over strong PEFT baselines on XNLI and FLORES benchmarks; improved directional balance and probability calibration; resilience to lightweight orthographic variants; additive benefits from governance steps; modest training overhead with favorable cost-quality trade-off.

Conclusion: Hybrid and unitary PEFT provide a stable and accessible path to resource-efficient multilingual adaptation when paired with practical data governance, offering improved performance while maintaining computational efficiency.
...
TL;DR: Stakeholder Suite: A framework for mapping actors, topics, and arguments in public debates about infrastructure/energy projects, combining NLP techniques for operational use.


<details>
  <summary>Details</summary>
Motivation: Public debates on infrastructure/energy projects involve complex stakeholder networks and evolving narratives. Existing media intelligence tools offer limited transparency and descriptive analytics, making it difficult to anticipate controversies and inform engagement strategies effectively.

Method: Stakeholder Suite combines actor detection, topic modeling, argument extraction, and stance classification in a unified pipeline. The framework is deployed in operational contexts and tested on multiple energy infrastructure projects as case studies.

Result: The framework achieves strong retrieval precision and stance accuracy, producing arguments judged relevant in 75% of pilot use cases. Beyond quantitative metrics, it helps project teams visualize influence networks, identify emerging controversies, and support evidence-based decision-making.

Conclusion: Stakeholder Suite provides fine-grained, source-grounded insights that are adaptable to diverse domains, offering a practical tool for understanding complex public debates and improving stakeholder engagement strategies in infrastructure and energy projects.
...
TL;DR: The paper introduces Canon Layers, lightweight architectural components that enhance horizontal information flow across tokens, improving reasoning depth, breadth, and knowledge manipulation in language models.


<details>
  <summary>Details</summary>
Motivation: Architectural differences in language models are hard to evaluate at academic-scale pretraining due to noise and randomness. The authors want to isolate and evaluate core model capabilities through controlled synthetic pretraining tasks.
...
Result: Canon layers enhance reasoning depth (2× improvement), reasoning breadth, and knowledge manipulation. They enable weak architectures like NoPE to match RoPE, and linear attention to rival SOTA models like Mamba2/GDN. Validated through both synthetic tasks and real-world academic-scale pretraining.

Conclusion: The synthetic playground approach offers an economical, principled way to isolate core model capabilities. With infinite high-quality data, it may predict how future architectures will behave as training improves, unlocking deeper reasoning and hierarchical inference.
...
TL;DR: IPC is an unsupervised framework that probes LLMs' internal knowledge for code generation without external data, achieving competitive performance to supervised methods while reducing data dependency.


<details>
  <summary>Details</summary>
Motivation: Current LLMs for code generation require expensive supervised training with labeled or unlabeled datasets, which are difficult to obtain at scale. There's a need for methods that reduce dependency on external data resources.

Method: IPC uses internal probing of LLMs through four components: problem space probing, test understanding probing, solution space probing, and knowledge consolidation/reinforcement. It identifies reliable code candidates via self-consistency mechanisms and representation-based quality estimation to train UCoder (unsupervised coder).

Result: The approach achieves competitive performance across multiple code benchmarks compared to supervised methods while significantly reducing dependency on labeled data and computational resources.
...
TL;DR: The paper introduces CulturalToM-VQA, a new benchmark with 5095 questions to evaluate Theory of Mind reasoning across diverse cultural contexts through visual question answering.


<details>
  <summary>Details</summary>
Motivation: Theory of Mind is fundamental for human social intelligence but remains challenging for AI. Existing Vision-Language Models are applied to social tasks, but their cross-cultural ToM reasoning capacity is unexplored. Current benchmarks are Western-centric and lack cultural diversity.

Method: Created CulturalToM-VQA using a VLM-assisted human-in-the-loop pipeline: 1) Human experts curate culturally rich images across traditions, rituals, and social interactions; 2) VLM generates structured ToM-focused scene descriptions; 3) Refine into question-answer pairs spanning six ToM tasks and four complexity levels.

Result: A benchmark dataset of 5095 questions covering diverse cultural contexts, capturing culturally grounded cues (rituals, attire, gestures, interpersonal dynamics) and six ToM facets: mental state attribution, false belief reasoning, non-literal communication, social norm violations, perspective coordination, and multi-agent reasoning.

Conclusion: CulturalToM-VQA enables systematic evaluation of ToM reasoning beyond Western-centric benchmarks, addressing the gap in cross-cultural ToM assessment for AI systems and providing a tool to measure cultural awareness in social intelligence.
...
<details>
  <summary>Details</summary>
Motivation: To create a more effective emotion detection system by combining architecturally diverse small transformer models with a dual-weighted voting mechanism, addressing limitations of conventional homogeneous ensembles and demonstrating that carefully designed small model ensembles can outperform much larger LLMs.

Method: Combines five diverse small transformer models (BERT, RoBERTa, DistilBERT, DeBERTa, ELECTRA) fully fine-tuned for emotion classification. Uses a dual-weighted voting mechanism integrating global credibility (validation F1 score) and local confidence (instance-level probability) to dynamically weight model contributions, inspired by Condorcet's Jury Theorem.

Result: Achieves macro F1 score of 93.5% on DAIR-AI dataset, surpassing state-of-the-art benchmarks and significantly outperforming larger LLMs (Falcon, Mistral, Qwen, Phi) even after task-specific LoRA adaptation. With only 595M total parameters, outperforms models up to 7B parameters.

Conclusion: Carefully designed ensembles of small, fine-tuned models can outperform much larger LLMs in specialized NLP tasks like emotion detection, offering better parameter efficiency and robustness through architectural diversity and intelligent weighting mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: LLMs exhibit distinct personalities affecting trust and engagement, but current personality control methods are either costly (post-training) or brittle (prompt engineering). Linear directions offer a cheap, efficient alternative for characterizing and controlling LLM behavior.
...
Conclusion: Linear directions provide effective personality probing but context-dependent steering capabilities, suggesting they're more suitable for diagnostic purposes than robust behavioral control in complex scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current research on Streaming Speech-to-Text Translation relies on SimulEval, which is no longer maintained, doesn't support output revision systems, is designed for short segments rather than long-form audio, and lacks demo capabilities.

Method: Introduces simulstream, an open-source framework designed for unified evaluation of StreamST systems. It supports both incremental decoding and re-translation methods, handles long-form speech processing, and includes an interactive web interface for demonstrations.

Result: simulstream provides the first dedicated framework for comprehensive evaluation of StreamST systems, enabling comparison of different approaches (incremental vs re-translation) in terms of both translation quality and latency metrics.

Conclusion: simulstream addresses critical gaps in StreamST research infrastructure by providing a modern, maintained framework that supports diverse system architectures, long-form audio processing, and practical demonstration capabilities.
...
TL;DR: A contextual biasing method for ASR that uses multi-token prediction to better recognize rare named entities without extra architectural complexity.


<details>
  <summary>Details</summary>
Motivation: E2E ASR models struggle with rare/unseen named entities (contact names, locations) which are critical for virtual assistants and downstream applications.

Method: Proposes contextual biasing for AED models using candidate entity lists. Instead of predicting only next token, simultaneously predicts multiple future tokens to "peek into future" and score potential entities. Uses multi-token prediction logits directly without requiring additional entity encoders or cross-attention layers.
...
Conclusion: The method effectively improves named entity recognition in ASR while maintaining architectural simplicity by leveraging multi-token prediction for contextual biasing.
...
TL;DR: Bayesian methods for uncertainty quantification in neural QA systems, comparing Laplace approximations vs MAP estimates for better calibration and selective prediction.


<details>
  <summary>Details</summary>
Motivation: To quantify uncertainty in neural networks for question answering, enabling models to convey confidence in predictions and abstain when uncertain, leading to more responsible and ethical deployment.

Method: Progressive approach: start with MLP on Iris dataset for posterior inference, extend to language models with Bayesian inference on frozen head, then LoRA-adapted transformers on CommonsenseQA benchmark. Compare Laplace approximations against MAP estimates for uncertainty calibration and selective prediction.

Result: Bayesian methods enable uncertainty quantification and confidence calibration, allowing models to abstain when confidence is low ("I don't know" response), improving interpretability without necessarily achieving state-of-the-art accuracy.

Conclusion: Bayesian reasoning provides effective uncertainty quantification for neural QA systems, enabling selective prediction and abstention mechanisms that contribute to more responsible and ethical deployment of AI systems.
...
TL;DR: The paper examines challenges in translating user-generated content (UGC) with non-standard language, develops a taxonomy of translation actions, and shows that LLM translation quality depends on alignment with dataset guidelines.


<details>
  <summary>Details</summary>
Motivation: User-generated content contains non-standard language (spelling errors, slang, emojis, etc.), making translation evaluation challenging because "good" translation depends on desired output standardness level.

Method: Analyzed human translation guidelines from four UGC datasets, derived taxonomy of 12 non-standard phenomena and 5 translation actions (NORMALISE, COPY, TRANSFER, OMIT, CENSOR). Conducted case study on LLMs to test sensitivity to prompts with explicit UGC translation instructions.

Result: Revealed notable differences in UGC treatment across datasets, creating spectrum of standardness in reference translations. LLM translation scores highly sensitive to prompts with explicit UGC instructions, improving when prompts align with dataset guidelines.
...
<details>
  <summary>Details</summary>
Motivation: Current computational affective and social science research faces substantial impediments in discovering, accessing, and using language annotation resources, particularly for non-CS practitioners.

Method: Created ABCDE dataset by collecting over 400 million text utterances from social media, blogs, books, and AI-generated sources, then annotating them with diverse features relevant to affective and social science research.

Result: Produced a large-scale, multi-source dataset with comprehensive annotations covering affect, body, cognition, demographics, and emotion features to enable easier access for interdisciplinary research.

Conclusion: ABCDE dataset addresses resource accessibility barriers and facilitates cross-disciplinary research across affective science, cognitive science, digital humanities, sociology, political science, and computational linguistics.
...
TL;DR: AncientBench: A benchmark for evaluating large language models' comprehension of ancient Chinese characters, particularly excavated documents, across four dimensions and ten tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Chinese benchmarks focus on modern Chinese and transmitted ancient documents, but lack evaluation for excavated ancient documents. There's a need to assess LLMs' comprehension of ancient characters to support archaeology and Chinese history studies.

Method: Created AncientBench with four dimensions (glyph, pronunciation, meaning, contextual comprehension) and ten tasks (radical, phonetic radical, homophone, cloze, translation, etc.). Involved archaeological researchers for evaluation, proposed an ancient model baseline, and tested current best-performing LLMs.

Result: Experimental results show LLMs have great potential in ancient textual scenarios but still have gaps compared to human performance. The benchmark provides comprehensive evaluation framework.

Conclusion: AncientBench fills the gap in evaluating LLMs' comprehension of excavated ancient documents, promotes LLM development in archaeology and ancient Chinese language, and reveals both potential and limitations of current models.
...
TL;DR: This paper introduces a Multi-BERT Ensemble approach for Bangla Medical Entity Recognition (MedER) that achieves 89.58% accuracy, representing an 11.80% improvement over single BERT models, and creates a new annotated dataset for this low-resource language task.


<details>
  <summary>Details</summary>
Motivation: While Medical Entity Recognition (MedER) is crucial for developing automated medical systems and improving patient care, most research focuses on English, leaving low-resource languages like Bangla underexplored. The lack of annotated datasets for Bangla MedER presents a major challenge that needs to be addressed.

Method: The study first evaluated several transformer models (BERT, DistilBERT, ELECTRA, RoBERTa) for Bangla MedER. Then proposed a novel Multi-BERT Ensemble approach that combines multiple BERT models. Additionally, created a high-quality annotated dataset specifically for Bangla MedER to address the data scarcity issue.

Result: The Multi-BERT Ensemble approach achieved the highest accuracy of 89.58%, outperforming all baseline transformer models. It showed an 11.80% accuracy improvement over single-layer BERT. The model demonstrated robustness across multiple performance metrics using the newly created dataset.

Conclusion: The Multi-BERT Ensemble model effectively improves Medical Entity Recognition for Bangla, demonstrating significant potential for low-resource language NLP tasks. The work establishes a foundation for future advancements in Bangla medical NLP and provides a valuable dataset for the research community.
...
TL;DR: DEER is a benchmark for evaluating expert-level research reports generated by LLMs, featuring 50 tasks across 13 domains with expert-grounded evaluation rubrics and document-level fact-checking.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack systematic criteria for expert reporting, LLM-based evaluations fail to capture issues requiring expert judgment, and source verification typically only covers limited explicitly cited statements rather than report-wide factual reliability.

Method: DEER includes 50 report-writing tasks across 13 domains with an expert-grounded evaluation taxonomy (7 dimensions, 25 sub-dimensions) operationalized into 130 fine-grained rubric items. It provides task-specific expert guidance for LLM judges and proposes a document-level fact-checking architecture that extracts and verifies all claims (both cited and uncited) across entire reports.

Result: DEER correlates closely with human expert judgments and yields interpretable diagnostics of system strengths and weaknesses, providing comprehensive evaluation of expert-level report quality.

Conclusion: DEER addresses critical gaps in evaluating expert-level research reports generated by LLMs by providing systematic evaluation criteria, expert-grounded rubrics, and comprehensive fact-checking that covers both cited and uncited claims across entire documents.
...
TL;DR: ShareChat is a large-scale dataset of 142,808 conversations across 5 major LLM platforms (ChatGPT, Claude, Gemini, Perplexity, Grok) that preserves native interface elements like reasoning traces and source links, enabling analysis of real user-LLM interactions.


<details>
  <summary>Details</summary>
Motivation: Existing public LLM datasets treat models as generic text generators, stripping away the crucial interface context that shapes user interaction. There's a need for datasets that preserve platform-specific affordances to study authentic user-LLM interactions.

Method: Collected 142,808 conversations with over 660,000 turns from publicly shared URLs across five major platforms (ChatGPT, Claude, Gemini, Perplexity, Grok) from April 2023 to October 2025. Preserved native platform elements like reasoning traces, source links, and code artifacts across 101 languages.
...
Conclusion: ShareChat provides a vital resource for understanding authentic user-LLM chatbot interactions by preserving platform-specific interface context, enabling research on how different LLM platforms shape user behavior and interaction patterns in real-world settings.
...
### [29] [V-Agent: An Interactive Video Search System Using Vision-Language Models](https://arxiv.org/abs/2512.16925)
...
Method: Fine-tune a vision-language model with small video preference dataset, enhance with retrieval vector from image-text retrieval model, embed video frames and ASR transcriptions into shared multimodal space, and use three collaborative agents (routing, search, chat) with re-ranking module.

Result: Demonstrates state-of-the-art zero-shot performance on MultiVENT 2.0 benchmark, showing strong potential for both academic research and real-world applications.

Conclusion: V-Agent effectively addresses multimodal video search challenges through collaborative multi-agent architecture and enhanced VLM-based retrieval, representing an advancement in video search and interactive systems.
...
### [30] [Comparison of deep learning models: CNN and VGG-16 in identifying pornographic content](https://arxiv.org/abs/2512.16947)
...
<details>
  <summary>Details</summary>
Motivation: Indonesian government blocked 59,741 websites (14,266 pornographic) in 2020, but VPNs allowed public access, creating need for rapid pornographic content identification system.

Method: Developed system using deep learning approach comparing CNN and VGG-16 models for pornographic image content detection, conducting comprehensive comparative experiments.
...
### [31] [AVM: Towards Structure-Preserving Neural Response Modeling in the Visual Cortex Across Stimuli and Individuals](https://arxiv.org/abs/2512.16948)
*Qi Xu,Shuai Gong,Xuming Ran,Haihua Luo,Yangfan Hu*

Main category: cs.CV

TL;DR: AVM is a modular neural modeling framework that separates stable visual encoding from condition-specific adaptation, improving generalization across stimuli and individuals while maintaining architectural efficiency.
...
Method: AVM uses a structure-preserving framework with modular subnetworks: a frozen Vision Transformer-based encoder captures consistent visual features, while independently trained modulation paths handle neural response variations from stimulus content and subject identity.

Result: AVM outperforms state-of-the-art V1T model by ~2% in predictive correlation across mouse V1 datasets, with 9.1% improvement in explained variance (FEVE) in cross-dataset adaptation, demonstrating robust generalization and interpretable condition-wise modulation.

Conclusion: AVM provides a unified framework for adaptive neural modeling across biological and experimental conditions, offering a scalable solution under structural constraints that could inform future cortical modeling in neuroscience and biologically inspired AI.
...
### [32] [Enhancing Tree Species Classification: Insights from YOLOv8 and Explainable AI Applied to TLS Point Cloud Projections](https://arxiv.org/abs/2512.16950)
...
TL;DR: Novel method links Finer-CAM explanations to TLS projection segments to identify structural features driving tree species classification, revealing models primarily use crown features with some species-specific stem contributions.


<details>
  <summary>Details</summary>
Motivation: While new sensors (TLS) and deep learning achieve high accuracy in tree species classification, their decision processes remain unclear. Existing methods like Finer-CAM can highlight features but are uncommon for similar-looking contrastive species, creating a need to understand which structural features drive species discrimination.

Method: Proposed novel method linking Finer-CAM explanations to segments of TLS projections representing structural tree features. Used TLS data from 2,445 trees across seven European species, trained five YOLOv8 models with cross-validation, and analyzed 630 saliency maps to evaluate feature contributions.

Result: Models achieved 96% mean accuracy (SD = 0.24%). Analysis revealed models primarily rely on crown features for classification, with stem features contributing more for European ash, Scots pine, and Douglas fir. Finer branches particularly influenced decisions. Model similarity judgments aligned with human expert assessments.

Conclusion: The study demonstrates the importance of understanding decision processes in tree species classification models to reveal dataset/model limitations, biases, and build confidence in predictions. The proposed method successfully identifies structural features driving classification decisions.
...
### [33] [Lights, Camera, Consistency: A Multistage Pipeline for Character-Stable AI Video Stories](https://arxiv.org/abs/2512.16954)
*Chayan Jain,Rishant Sharma,Archit Garg,Ishan Bhanuka,Pratik Narang,Dhruv Kumar*

Main category: cs.CV

TL;DR: A multi-stage pipeline for generating long, cohesive video stories with consistent characters using LLM scripts, text-to-image character visuals, and video generation with visual anchoring.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video AI struggles with generating long, cohesive video stories while maintaining character consistency throughout the narrative.

Method: A filmmaker-like pipeline that: 1) uses LLM to generate detailed production script, 2) employs text-to-image model to create consistent character visuals as anchors, 3) synthesizes each scene individually using video generation model guided by the script and visual anchors.

Result: Visual anchoring is crucial - removing it causes catastrophic drop in character consistency scores (7.99 to 0.55). Also reveals cultural biases in current models with distinct differences in subject consistency and dynamic degree between Indian vs Western-themed generations.
...
### [34] [InfoTok: Adaptive Discrete Video Tokenizer via Information-Theoretic Compression](https://arxiv.org/abs/2512.16975)
...
TL;DR: InfoTok: Adaptive video tokenization framework using information theory to allocate tokens based on content richness, achieving 20% token savings without performance loss and 2.3x compression rates.


<details>
  <summary>Details</summary>
Motivation: Current video tokenizers use fixed compression rates, leading to redundancy or information loss due to variable information density in videos. Need adaptive tokenization that allocates tokens according to informational richness.

Method: 1) Prove existing data-agnostic training methods are suboptimal in representation length. 2) Introduce novel evidence lower bound (ELBO)-based algorithm approaching theoretical optimality. 3) Develop transformer-based adaptive compressor for adaptive tokenization.
...
Conclusion: InfoTok enables more compressed yet accurate video tokenization by allocating tokens according to informational richness, offering valuable insights for future research in video representation.
...
### [35] [Endo-SemiS: Towards Robust Semi-Supervised Image Segmentation for Endoscopic Video](https://arxiv.org/abs/2512.16977)
*Hao Li,Daiwei Lu,Xing Yao,Nicholas Kavoussi,Ipek Oguz*

Main category: cs.CV

TL;DR: Endo-SemiS is a semi-supervised segmentation framework for endoscopic video frames that uses cross-supervision, uncertainty-guided pseudo-labels, joint pseudolabel supervision, and mutual learning to achieve superior performance with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Medical endoscopic video segmentation requires extensive annotation which is time-consuming and expensive. There's a need for frameworks that can effectively utilize both labeled and unlabeled data to achieve reliable segmentation with limited annotations.
...
Conclusion: Endo-SemiS effectively leverages unlabeled data through multiple synergistic strategies to achieve reliable endoscopic video segmentation with limited annotations, demonstrating strong performance on clinical applications and making the code publicly available.
...
### [36] [A Benchmark and Agentic Framework for Omni-Modal Reasoning and Tool Use in Long Videos](https://arxiv.org/abs/2512.16978)
...
TL;DR: LongShOTBench is a diagnostic benchmark for long-form multimodal video understanding with open-ended questions, dialogues, and tool-use tasks, featuring reference answers and graded rubrics for interpretable evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack both temporal length and multimodal richness, and rely on single-score accuracy that obscures failure modes in long-form video understanding.
...
Result: SOTA MLLMs show large gaps: Gemini-2.5-Flash achieves 52.95%, open-source models below 30%, and LongShOTAgent attains 44.66%, highlighting the difficulty of real-world long-form video understanding.

Conclusion: LongShOTBench provides a practical, reproducible foundation for evaluating and improving MLLMs on long-form multimodal video understanding tasks.
...
### [37] [4D-RGPT: Toward Region-level 4D Understanding via Perceptual Distillation](https://arxiv.org/abs/2512.17012)
...
Method: Three main contributions: (1) 4D-RGPT - specialized MLLM for 4D video representations with enhanced temporal perception; (2) Perceptual 4D Distillation (P4D) - training framework transferring 4D representations from frozen expert model to 4D-RGPT; (3) R4D-Bench - benchmark for depth-aware dynamic scenes with region-level prompting, created via hybrid automated/human-verified pipeline.

Result: 4D-RGPT achieves notable improvements on both existing 4D VQA benchmarks and the proposed R4D-Bench benchmark.

Conclusion: The proposed approach addresses limitations in current MLLMs' 4D perception and temporal understanding, providing enhanced capabilities for reasoning over dynamic 3D scenes with region-level analysis.
...
### [38] [FORMSpoT: A Decade of Tree-Level, Country-Scale Forest Monitoring](https://arxiv.org/abs/2512.17021)
...
TL;DR: FORMSpoT provides 1.5m resolution forest canopy height mapping and annual disturbance detection across France using SPOT-6/7 satellite data, significantly outperforming existing coarse-resolution products.


<details>
  <summary>Details</summary>
Motivation: European forest carbon sink is declining, but existing satellite disturbance products are too coarse (typically >100m²) to detect individual tree-level changes needed for effective forest monitoring.

Method: Uses annual SPOT-6/7 composites with hierarchical transformer model (PVTv2) trained on ALS data. Developed post-processing pipeline with co-registration and spatio-temporal total variation denoising for robust change detection across heterogeneous acquisitions.

Result: FORMSpoT-Δ achieves F1-score of 0.44 in mountainous forests, an order of magnitude higher than existing benchmarks. Validated against 19 ALS sites and 5,087 National Forest Inventory plots.
...
TL;DR: InfCam is a depth-free framework for camera-controlled video-to-video generation that achieves high camera-pose fidelity through infinite homography warping and data augmentation.
...
Method: Uses infinite homography warping to encode 3D camera rotations in 2D latent space, predicts residual parallax through end-to-end training, and employs data augmentation to create diverse trajectory datasets.
...
Conclusion: InfCam provides a depth-free solution for high-fidelity camera-controlled video generation that overcomes limitations of existing reprojection-based and learning-based approaches.
...
### [40] [Interpretable Similarity of Synthetic Image Utility](https://arxiv.org/abs/2512.17080)
*Panagiota Gatoula,George Dimas,Dimitris K. Iakovidis*

Main category: cs.CV

TL;DR: Proposes Interpretable Utility Similarity (IUS), a novel interpretable measure to assess synthetic medical image similarity to real images for DL-based clinical decision support systems.


<details>
  <summary>Details</summary>
Motivation: Current methods for assessing synthetic medical image similarity (user studies, inception-based measures, classification performance) lack interpretability and don't explain why one synthetic dataset is more useful than another for clinical decision support systems.

Method: Develops IUS based on generalized neural additive models to provide interpretable similarity assessment that explains utility differences in terms of clinically relevant image features.

Result: IUS selection of high-utility synthetic images improves classification performance by up to 54.6% across endoscopic, dermoscopic, and fundus imaging. Also demonstrates generality for X-ray and ultrasound modalities.

Conclusion: IUS provides an interpretable, clinically relevant measure for synthetic medical image assessment that can significantly improve DL-based clinical decision support system development.
...
### [41] [DGH: Dynamic Gaussian Hair](https://arxiv.org/abs/2512.17094)
...
Method: Two key components: (1) coarse-to-fine model for learning temporally coherent hair motion dynamics across diverse hairstyles, and (2) strand-guided optimization module that learns dynamic 3D Gaussian representation for hair appearance with differentiable rendering support.

Result: DGH achieves promising geometry and appearance results, providing a scalable, data-driven alternative to physics-based simulation. It can be integrated into 3D Gaussian avatar frameworks for realistic animatable hair.

Conclusion: The framework offers a fully data-driven approach that scales with training data and generalizes across various hairstyles and head motions, overcoming limitations of traditional physics-based methods.
...
### [42] [Predictive Modeling of Maritime Radar Data Using Transformer Architecture](https://arxiv.org/abs/2512.17098)
*Bjorna Qesaraku,Jan Steckel*

Main category: cs.CV

TL;DR: Survey identifies research gap: no transformer-based methods exist for maritime radar frame prediction despite their success in related domains like AIS trajectory prediction and sonar forecasting.


<details>
  <summary>Details</summary>
Motivation: Maritime autonomous systems need robust predictive capabilities for vessel motion and environmental dynamics. Radar is critical for all-weather navigation, but transformer architectures haven't been applied to maritime radar frame prediction despite their success in related areas.
...
Result: While transformers have been successfully applied to AIS-based trajectory prediction and sonar frame forecasting, no prior work addresses transformer-based maritime radar frame prediction.

Conclusion: Identifies clear research gap and motivates future work in developing transformer-based methods for maritime radar frame prediction to enhance autonomous maritime navigation capabilities.
...
### [43] [SDUM: A Scalable Deep Unrolled Model for Universal MRI Reconstruction](https://arxiv.org/abs/2512.17137)
...
Method: SDUM combines: 1) Restormer-based reconstructor, 2) learned coil sensitivity map estimator (CSME), 3) sampling-aware weighted data consistency (SWDC), 4) universal conditioning (UC) on cascade index and protocol metadata, and 5) progressive cascade expansion training.

Result: SDUM shows foundation-model scaling: PSNR ∼ log(parameters) with r=0.986 up to 18 cascades. Achieves SOTA across all four CMRxRecon2025 challenge tracks (+1.0 dB over specialized baselines), outperforms PromptMR+ by +0.55 dB on CMRxRecon2024, and exceeds PC-RNN by +1.8 dB on fastMRI brain.

Conclusion: SDUM establishes a practical path toward universal, scalable MRI reconstruction that can handle diverse clinical protocols without task-specific fine-tuning, demonstrating predictable performance gains with model scaling.
...
### [44] [Pro-Pose: Unpaired Full-Body Portrait Synthesis via Canonical UV Maps](https://arxiv.org/abs/2512.17143)
...
<details>
  <summary>Details</summary>
Motivation: Professional photos have better lighting, poses, and quality than casual self-photos, but there's no large paired dataset of the same person in both casual and professional settings to train a model.

Method: 1) Transform input photos to canonical UV space for occlusion modeling and novel view synthesis, leveraging unpaired datasets. 2) Use multi-image fine-tuning to personalize outputs while preserving identity features.
...
Conclusion: The method successfully creates professional-looking portraits from casual photos by operating in UV space and using personalization techniques, addressing the lack of paired training data.
...
### [45] [Text-Conditioned Background Generation for Editable Multi-Layer Documents](https://arxiv.org/abs/2512.17151)
...
<details>
  <summary>Details</summary>
Motivation: To create a document-centric background generation system that can edit multi-page documents while preserving text readability and maintaining visual consistency across pages, bridging generative AI with natural design workflows.

Method: Uses latent masking to softly attenuate diffusion updates in text regions, Automated Readability Optimization (ARO) to place semi-transparent backing shapes behind text based on WCAG 2.2 standards, and a summarization-and-instruction process for multi-page consistency where each page's compact representation guides subsequent generations.

Result: A training-free framework that produces visually coherent, text-preserving, and thematically aligned documents with automated consistency and flexible customization through user prompts.

Conclusion: The framework successfully bridges generative modeling with natural design workflows by enabling targeted background editing without compromising readability, maintaining thematic continuity across documents, and balancing automated consistency with user customization.
...
### [46] [PhysFire-WM: A Physics-Informed World Model for Emulating Fire Spread Dynamics](https://arxiv.org/abs/2512.17152)
*Nan Zhou,Huandong Wang,Jiahao Li,Yang Li,Xiao-Ping Zhang,Yong Li,Xinlei Chen*

Main category: cs.CV

TL;DR: PhysFire-WM is a physics-informed world model that combines infrared thermal data with fire masks to predict fine-grained fire spread dynamics, using physical priors from simulators and cross-task collaborative training.
...
Result: Extensive experiments on a fine-grained multimodal fire dataset demonstrate superior accuracy in fire spread prediction. Validation shows the importance of physical priors and cross-task collaboration.

Conclusion: The paper provides new insights for applying physics-informed world models to disaster prediction, showing that combining physical priors with multimodal data and collaborative training enhances both physical realism and geometric accuracy in fire forecasting.
...
### [47] [Can Synthetic Images Serve as Effective and Efficient Class Prototypes?](https://arxiv.org/abs/2512.17160)
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs like CLIP require expensive annotated image-text pairs for modality alignment and dual-tower encoders, making them costly and not lightweight. There's a need for more efficient zero-shot classification without manual annotations.

Method: LGCLIP uses LLMs to generate class-specific prompts, which guide diffusion models to synthesize reference images as visual prototypes. Real images are compared to these prototypes using only a visual encoder, eliminating text encoders and manual annotations.

Result: Experimental results show LGCLIP achieves strong performance in zero-shot classification tasks while being lightweight and efficient, requiring only class labels as input.

Conclusion: LGCLIP establishes a novel paradigm for zero-shot classification that eliminates dependency on annotated image-text pairs and dual encoders, offering a lightweight and efficient alternative to traditional VLMs.
...
### [48] [ABE-CLIP: Training-Free Attribute Binding Enhancement for Compositional Image-Text Matching](https://arxiv.org/abs/2512.17178)
*Qi Zhang,Yuxu Chen,Lei Deng,Lili Shen*

Main category: cs.CV

TL;DR: ABE-CLIP is a training-free method that enhances attribute-object binding in CLIP models by refining text embeddings and aligning tokens with relevant image patches.
...
Conclusion: ABE-CLIP effectively addresses CLIP's attribute binding limitations without requiring additional training, offering better generalization to novel compositional concepts through localized semantic alignment.
...
### [49] [It is not always greener on the other side: Greenery perception across demographics and personalities in multiple cities](https://arxiv.org/abs/2512.17186)
...
TL;DR: This paper examines discrepancies between objective measurements of urban greenery (like Green View Index) and subjective human perceptions, using street view imagery and survey data from 1,000 people across five countries to identify explanatory factors.


<details>
  <summary>Details</summary>
Motivation: Urban greenery assessment is crucial for city planning and climate/well-being, but there's often a mismatch between objective measurements and subjective perceptions. The paper aims to quantify these differences and explain them through human, geographic, and spatial dimensions.

Method: Used contextual information from street view imagery and conducted a comprehensive urban visual perception survey with 1,000 participants across five countries, collecting extensive demographic and personality data. Analyzed discrepancies between objective measures (Green View Index) and subjective scores (pairwise ratings).

Result: Discrepancies between objective and subjective greenery assessments are comparable worldwide. Demographics and personality don't significantly affect perception. While perceived and measured greenery correlate consistently across geographies, where people live is a major explanatory factor for perceptual differences, suggesting cultural, environmental, and experiential influences.

Conclusion: Location and cultural/environmental factors substantially shape how individuals perceive urban greenery, with where people live being among the top features influencing perception. This highlights the importance of considering subjective human perspectives alongside objective measurements in urban planning.
...
### [50] [Globally Optimal Solution to the Generalized Relative Pose Estimation Problem using Affine Correspondences](https://arxiv.org/abs/2512.17188)
*Zhenbao Yu,Banglei Guan,Shunkun Liang,Zibin Liu,Yang Shang,Qifeng Yu*

Main category: cs.CV

TL;DR: A globally optimal solver for relative pose estimation of multi-camera systems using affine correspondences with known vertical direction.
...
### [51] [Anatomical Region-Guided Contrastive Decoding: A Plug-and-Play Strategy for Mitigating Hallucinations in Medical VLMs](https://arxiv.org/abs/2512.17189)
*Xiao Liang,Chenxi Liu,Zhi Ma,Di Wang,Bin Jing,Quan Wang,Yuanyuan Shi*

Main category: cs.CV

TL;DR: ARCD is a plug-and-play training-free method that uses anatomical masks to guide contrastive decoding at token, attention, and logits levels, reducing hallucinations in MedVLMs by providing targeted region-specific guidance.


<details>
  <summary>Details</summary>
Motivation: Medical Vision-Language Models suffer from hallucinations where they rely on textual priors rather than visual evidence, and existing mitigation strategies have limitations: training methods require costly expert annotations, while training-free methods like contrastive decoding apply global, untargeted corrections that are unreliable in complex clinical settings.

Method: Anatomical Region-Guided Contrastive Decoding (ARCD) uses anatomical masks to direct a three-tiered contrastive decoding process that dynamically re-weights at token, attention, and logits levels to steer model focus onto specified regions and suppress factually incorrect outputs.

Result: Extensive experiments across diverse datasets (chest X-ray, CT, brain MRI, and ocular ultrasound) demonstrate effectiveness in improving regional understanding, reducing hallucinations, and enhancing overall diagnostic accuracy.

Conclusion: ARCD provides a scalable, data-efficient solution to mitigate hallucinations in MedVLMs through targeted anatomical region guidance, addressing limitations of both training-based and existing training-free methods.
...
### [52] [Fose: Fusion of One-Step Diffusion and End-to-End Network for Pansharpening](https://arxiv.org/abs/2512.17202)
*Kai Liu,Zeli Lin,Weibo Wang,Linghe Kong,Yulun Zhang*

Main category: cs.CV

TL;DR: Fose: A lightweight pansharpening network that fuses one-step diffusion model distillation with E2E model using four-stage training, achieving SOTA performance with 7.42× speedup.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for pansharpening are computationally expensive (multi-step process), while E2E models lack priors and have simple structures. Need to combine accuracy with efficiency.

Method: Four-stage training strategy: 1) One-step distillation from enhanced SOTA diffusion model (compressing 50 steps to 1), 2) Fusion with E2E model using lightweight ensemble blocks, 3) Comprehensive training approach, 4) Lightweight network design.

Result: Significant improvement on three common benchmarks, achieves 7.42× speedup ratio compared to baseline diffusion model while delivering much better performance.

Conclusion: Fose successfully combines the accuracy of diffusion models with the efficiency of E2E models through distillation and fusion, achieving state-of-the-art pansharpening performance with dramatically reduced computational cost.
...
### [53] [Reasoning Palette: Modulating Reasoning via Latent Contextualization for Controllable Exploration for (V)LMs](https://arxiv.org/abs/2512.17206)
*Rujiao Long,Yang Li,Xingyao Zhang,Weixun Wang,Tianqianjin Lin,Xi Zhao,Yuchi Xu,Wenbo Su,Junchi Yan,Bo Zheng*

Main category: cs.CV

TL;DR: Reasoning Palette: A latent-modulation framework using VAE to inject stochastic latent variables as token prefixes, enabling strategic reasoning diversity and improved exploration in RL for vision-language models.
...
Method: Proposes Reasoning Palette: a variational autoencoder (VAE) infers latent variables from question-answer embeddings, which are decoded into learnable token prefixes prepended to input prompts. This modulates internal reasoning trajectories before token generation, with a supervised fine-tuning warm-up for adaptation.

Result: Enables interpretable and controllable strategic behavior, achieves consistent performance gains over standard RL methods across multiple reasoning benchmarks, and significantly enhances exploration efficiency and sustained learning capability.

Conclusion: The latent-modulation framework provides structured exploration by enabling on-demand injection of diverse reasoning modes, improving both inference-time performance and RL training for large vision-language models through strategic contextualization.
...
### [54] [CheXPO-v2: Preference Optimization for Chest X-ray VLMs with Knowledge Graph Consistency](https://arxiv.org/abs/2512.17213)
*Xiao Liang,Yuxuan An,Di Wang,Jiawei Hu,Zhicheng Jiao,Bin Jing,Quan Wang*

Main category: cs.CV

TL;DR: CheXPO-v2 is a novel alignment framework that shifts from outcome to process supervision using Knowledge Graph Consistency Reward with Entity-Relation Matching to reduce hallucinations in medical VLMs.


<details>
  <summary>Details</summary>
Motivation: Medical Vision-Language Models are prone to hallucinations that compromise clinical reliability. Existing reinforcement learning methods like GRPO rely on sparse outcome-based rewards, which encourage models to "overthink" and generate verbose, convoluted reasoning that obscures factual errors and poses safety risks.

Method: Proposes CheXPO-v2 alignment framework with Knowledge Graph Consistency Reward mechanism driven by Entity-Relation Matching. The method parses reasoning steps into structured "Disease, Relation, Anatomy" triplets to provide fine-grained supervision that penalizes incoherent logic and hallucinations at the atomic level. Integrates this with a hard-example mining strategy.
...
Conclusion: CheXPO-v2 effectively addresses the hallucination problem in medical VLMs by shifting from outcome to process supervision, providing a more reliable and verifiable alignment approach that enhances clinical safety while maintaining data efficiency.
...
TL;DR: DAVE is a specialized vision encoder for VLMs designed for document understanding and web agent tasks, trained through self-supervised and supervised stages using limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Current VLMs have vision encoders with low-level features lacking robust structural and spatial information needed for document understanding and web agent tasks.

Method: Two-stage training: 1) self-supervised pretraining on unlabeled images, 2) supervised autoregressive pretraining with limited high-quality data. Uses model-merging scheme to combine encoders trained with different text decoders, and ensemble training to fuse features from generalist encoders with document/web-specific representations.
...
Conclusion: DAVE establishes itself as a purpose-built vision encoder that bridges the gap in VLMs for document understanding and web agent tasks through innovative training strategies.
...
### [56] [Any-Optical-Model: A Universal Foundation Model for Optical Remote Sensing](https://arxiv.org/abs/2512.17224)
...
Conclusion: AOM successfully addresses the limitations of existing RSFMs by providing a universal model that accommodates arbitrary optical sensor configurations, enabling robust performance across diverse real-world remote sensing scenarios.
...
### [57] [Robust Scene Coordinate Regression via Geometrically-Consistent Global Descriptors](https://arxiv.org/abs/2512.17226)
...
<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods use global descriptors derived from geometric cues alone (e.g., covisibility graphs), which limits discriminative power and reduces robustness in the presence of noisy geometric constraints.

Method: Proposes an aggregator module that learns global descriptors consistent with both geometrical structure and visual similarity. Uses batch-mining strategy based solely on overlap scores and a modified contrastive loss, training without manual place labels.
...
Conclusion: The method effectively corrects erroneous associations caused by unreliable overlap scores, generalizes across diverse environments, and achieves improved visual localization performance.
...
### [58] [Learning When to Look: A Disentangled Curriculum for Strategic Perception in Multimodal Reasoning](https://arxiv.org/abs/2512.17227)
*Siqi Yang,Zilve Gao,Haibo Qiu,Fanfan Liu,Peng Shi,Zhixiong Zeng,Qingmin Liao,Lin Ma*

Main category: cs.CV

TL;DR: The paper addresses "visual forgetting" in MLLMs by disentangling abstract reasoning from strategic visual perception through a two-stage curriculum: first building reasoning skills on text, then teaching when to look via reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs suffer from "visual forgetting" where they lose visual grounding during long-chain reasoning tasks. This stems from premature entanglement of abstract reasoning and strategic visual perception during training, creating both a cold-start reasoning deficiency and a strategic perception deficit.

Method: A two-stage curriculum framework: 1) Disentangled SFT curriculum builds abstract reasoning backbone on text-only data, then anchors it to vision using Perception-Grounded Chain-of-Thought. 2) Reinforcement learning with Pivotal Perception Reward teaches models when to look by coupling perceptual actions to linguistic markers of cognitive uncertainty.

Result: The framework transforms models from heuristic-driven observers to strategic, grounded reasoners by addressing both the foundational reasoning deficiency and strategic perception deficit, enabling better visual grounding in complex reasoning tasks.

Conclusion: Disentangling abstract reasoning from strategic visual perception through a principled two-stage curriculum effectively addresses visual forgetting in MLLMs, enabling them to maintain visual grounding throughout extended reasoning chains.
...
### [59] [Video Detective: Seek Critical Clues Recurrently to Answer Question from Long Videos](https://arxiv.org/abs/2512.17229)
*Henghui Du,Chang Zhou,Chunjie Zhang,Xi Chen,Di Hu*

Main category: cs.CV

TL;DR: VideoDetective enables MLLMs to efficiently process long videos (up to 1 hour) by using question-aware memory tokens to compress and seek critical information across video segments, reducing GPU memory usage while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Long Video QA is challenging for MLLMs due to immense context size, information overload, and prohibitive memory consumption. Existing methods either miss useful information by reducing visual tokens or require excessive computation by extending context length.

Method: VideoDetective uses an efficient question-aware memory mechanism that iteratively processes video sub-segments. For each segment, it introduces special memory tokens for purpose-driven compression to seek critical clues while reducing visual tokens. These memory tokens are recurrently aggregated and stored as history context for reuse across subsequent segments.

Result: The method enables MLLMs with 32K context length to process 100K tokens (3600 frames, 1-hour video at 1fps) in only 2 minutes using 37GB GPU memory. Evaluation across multiple long video benchmarks shows superior ability to seek critical clues from massive information.

Conclusion: VideoDetective provides an efficient solution for LVQA by enabling MLLMs to selectively compress and seek relevant information through question-aware memory tokens, overcoming context limitations and memory constraints while maintaining effective long video understanding.
...
### [60] [Mitty: Diffusion-based Human-to-Robot Video Generation](https://arxiv.org/abs/2512.17253)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for learning from human demonstration videos rely on intermediate representations like keypoints or trajectories, which introduce information loss and cumulative errors that harm temporal and visual consistency. There's a need for more direct, end-to-end approaches.

Method: Mitty is built on a pretrained video diffusion model and uses a Diffusion Transformer architecture. Human demonstration videos are compressed into condition tokens and fused with robot denoising tokens through bidirectional attention during diffusion. An automatic synthesis pipeline produces high-quality human-robot pairs from large egocentric datasets to address paired-data scarcity.
...
Conclusion: Mitty demonstrates that end-to-end Human2Robot video generation through video In-Context Learning is effective, bypassing intermediate representations and leveraging strong visual-temporal priors from pretrained video diffusion models for scalable robot learning.
...
### [61] [AnyCXR: Human Anatomy Segmentation of Chest X-ray at Any Acquisition Position using Multi-stage Domain Randomized Synthetic Data with Imperfect Annotations and Conditional Joint Annotation Regularization Learning](https://arxiv.org/abs/2512.17263)
*Dong Zifei,Wu Wenjie,Hao Jinkui,Chen Tianqi,Weng Ziqiao,Zhou Bo*

Main category: cs.CV

TL;DR: AnyCXR is a unified framework for generalizable multi-organ segmentation of chest X-rays using only synthetic supervision, achieving zero-shot generalization across various projection angles.


<details>
  <summary>Details</summary>
Motivation: Robust anatomical segmentation of chest X-rays is challenging due to limited comprehensive annotations and high variability in real-world acquisition conditions. There's a need for scalable solutions that reduce annotation burdens while maintaining robustness.
...
Conclusion: AnyCXR establishes a scalable and reliable foundation for anatomy-aware CXR analysis and offers a practical pathway to reduce annotation burdens while improving robustness across diverse imaging conditions.
...
### [62] [RadImageNet-VQA: A Large-Scale CT and MRI Dataset for Radiologic Visual Question Answering](https://arxiv.org/abs/2512.17396)
*Léo Butsanets,Charles Corbière,Julien Khlaut,Pierre Manceron,Corentin Dancette*

Main category: cs.CV

TL;DR: RadImageNet-VQA is a large-scale radiologic VQA dataset with 750K CT/MRI images and 7.5M QA pairs, addressing limitations of existing medical VQA datasets and enabling evaluation across three clinical tasks without text shortcuts.


<details>
  <summary>Details</summary>
Motivation: Existing medical VQA datasets are limited in scale, dominated by X-ray imaging or biomedical illustrations, and often prone to text-based shortcuts, creating a need for a comprehensive radiologic VQA dataset for CT and MRI exams.

Method: Built a large-scale dataset from expert-curated annotations covering 750K CT/MRI images paired with 7.5M question-answer samples across three key tasks (abnormality detection, anatomy recognition, pathology identification), spanning eight anatomical regions and 97 pathology categories, with support for open-ended, closed-ended, and multiple-choice questions.

Result: State-of-the-art vision-language models struggle with fine-grained pathology identification, especially in open-ended settings, even after fine-tuning. Text-only analysis shows model performance collapses to near-random without image inputs, confirming the dataset is free from linguistic shortcuts.

Conclusion: RadImageNet-VQA addresses critical gaps in medical VQA research by providing a large-scale, radiologically-focused benchmark that requires genuine image understanding, revealing current model limitations in fine-grained pathology identification and enabling future research in medical vision-language understanding.
...
### [63] [WDFFU-Mamba: A Wavelet-guided Dual-attention Feature Fusion Mamba for Breast Tumor Segmentation in Ultrasound Images](https://arxiv.org/abs/2512.17278)
*Guoping Cai,Houjin Chen,Yanfeng Li,Jia Sun,Ziwei Chen,Qingzi Geng*

Main category: cs.CV

TL;DR: WDFFU-Mamba: A novel U-shaped Mamba architecture with wavelet-guided enhancement and dual-attention feature fusion for robust breast ultrasound image segmentation, achieving state-of-the-art performance on public datasets.
...
Method: Proposes WDFFU-Mamba network integrating wavelet-guided enhancement and dual-attention feature fusion within a U-shaped Mamba architecture. Includes Wavelet-denoised High-Frequency-guided Feature (WHF) module to enhance low-level representations using noise-suppressed high-frequency cues, and Dual Attention Feature Fusion (DAFF) module to merge skip-connected and semantic features for improved contextual consistency.
...
TL;DR: General-purpose ultrasound AI models show high accuracy across multi-organ segmentation and classification tasks, but face domain generalization challenges when applied to unseen data.


<details>
  <summary>Details</summary>
Motivation: Current ultrasound AI tools are fragmented into single-task applications, limiting their clinical utility compared to modern versatile ultrasound systems that can perform multiple examinations.

Method: The Universal UltraSound Image Challenge 2025 (UUSIC25) involved developing algorithms on 11,644 images, with evaluation on an independent multi-center test set of 2,479 images including completely unseen data to assess generalization.

Result: Top model (SMART) achieved macro-averaged DSC of 0.854 across 5 segmentation tasks and AUC of 0.766 for binary classification. Performance dropped significantly in breast cancer molecular subtyping from AUC 0.571 (internal) to 0.508 (unseen external center).

Conclusion: General-purpose AI models can achieve high accuracy and efficiency across multiple ultrasound tasks using single architecture, but domain generalization remains a critical challenge for clinical deployment.
...
### [65] [Vision-Language Model Guided Image Restoration](https://arxiv.org/abs/2512.17292)
...
<details>
  <summary>Details</summary>
Motivation: Previous image restoration approaches struggle to effectively leverage both visual and linguistic knowledge. While recent efforts incorporate VLMs, they fail to utilize linguistic priors to ensure semantic coherence during restoration. There's a need to better integrate vision-language knowledge for realistic photo recovery with fine-grained details.

Method: Two-stage framework: 1) VLM-based feature extraction - extracts complementary visual and linguistic representations by aligning embeddings of captions from low/high-quality images using cosine similarity loss with LoRA fine-tuning, and employs a degradation predictor to decompose degradation and clean content embeddings. 2) Diffusion-based restoration - integrates these embeddings into a diffusion model via cross-attention mechanisms.

Result: Extensive experiments and ablation studies demonstrate that VLMIR achieves superior performance across both universal and degradation-specific image restoration tasks, highlighting the effectiveness of integrated visual and linguistic knowledge from VLMs.

Conclusion: The VLMIR framework successfully leverages vision-language priors from models like CLIP to enhance image restoration capabilities, demonstrating the critical role of integrated visual and linguistic knowledge in advancing restoration performance for both pixel-level fidelity and semantic coherence.
...
### [66] [Towards Pixel-Wise Anomaly Location for High-Resolution PCBA \\ via Self-Supervised Image Reconstruction](https://arxiv.org/abs/2512.17296)
...
Result: The method produces clean, high-resolution anomaly maps with low false positive rate. Extensive experiments on the new SIPCBA-500 dataset (500 self-collected images) and public benchmarks show superior localization performance at practical speed.

Conclusion: HiSIR-Net effectively addresses PCBA defect inspection challenges through self-supervised reconstruction, achieving high performance on high-resolution images while maintaining practical computational efficiency.
...
### [67] [ProCache: Constraint-Aware Feature Caching with Selective Computation for Diffusion Transformer Acceleration](https://arxiv.org/abs/2512.17298)
*Fanpu Cao,Yaofo Chen,Zeng You,Wei Luo,Cen Chen*

Main category: cs.CV

TL;DR: ProCache is a training-free dynamic feature caching framework for Diffusion Transformers that achieves up to 2.9x acceleration with minimal quality loss by using non-uniform caching intervals and selective computation.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) have state-of-the-art generative performance but high computational cost hinders real-time deployment. Existing feature caching methods use uniform intervals that don't match DiT's non-uniform temporal dynamics and cause error accumulation.

Method: ProCache has two core components: (1) constraint-aware caching pattern search module that generates non-uniform activation schedules via offline constrained sampling, and (2) selective computation module that selectively computes within deep blocks and high-importance tokens for cached segments to mitigate error accumulation.
...
Conclusion: The proposed ProCache framework effectively addresses limitations of existing feature caching methods by aligning with DiT's temporal dynamics and mitigating error accumulation, enabling efficient real-time deployment of Diffusion Transformers.
...
### [68] [MatLat: Material Latent Space for PBR Texture Generation](https://arxiv.org/abs/2512.17302)
*Kyeongmin Yeo,Yunhong Min,Jaihoon Kim,Minhyuk Sung*

Main category: cs.CV

TL;DR: A generative framework for high-quality PBR texture synthesis on 3D meshes using pretrained latent diffusion models with fine-tuned VAE and locality-preserving regularization.


<details>
  <summary>Details</summary>
Motivation: Large-scale PBR texture datasets are scarce, making it challenging to generate high-quality physically-based rendering textures for 3D meshes. Existing methods that freeze pretrained embedding networks suffer from distribution shifts when encoding additional PBR channels, hindering diffusion training.

Method: 1) Fine-tune pretrained VAE to incorporate new material channels with minimal latent distribution deviation, 2) Introduce locality-preserving regularization that crops latent patches, decodes them, and aligns corresponding image regions to maintain strong pixel-latent spatial correspondence, 3) Learn a material latent space (MatLat) through targeted fine-tuning of pretrained latent image generative models.

Result: The framework improves PBR texture fidelity compared to previous baselines. Ablation studies show each component (VAE fine-tuning and locality regularization) is critical for achieving state-of-the-art performance.

Conclusion: The proposed generative framework effectively leverages pretrained latent diffusion models for PBR texture synthesis by addressing distribution shift issues through VAE fine-tuning and enforcing locality preservation for cross-view consistency, achieving superior texture quality.
...
### [69] [EMAG: Self-Rectifying Diffusion Sampling with Exponential Moving Average Guidance](https://arxiv.org/abs/2512.17303)
*Ankit Yadav,Ta Duc Huy,Lingqiao Liu*

Main category: cs.CV

TL;DR: EMAG is a training-free guidance method for diffusion transformers that adaptively selects layers and creates fine-grained negative samples to improve generation quality over classifier-free guidance.
...
Method: EMAG modifies attention at inference time in diffusion transformers using exponential moving average statistics and an adaptive layer-selection rule to produce harder, semantically faithful negative samples.

Result: EMAG improves human preference score by +0.46 over CFG, produces fine-grained degradations that surface difficult failure modes, and composes well with other advanced guidance techniques like APG and CADS.

Conclusion: EMAG provides a training-free, adaptive approach to guidance that creates more challenging negative samples for better artifact refinement and generation quality in diffusion models.
...
### [70] [Deep But Reliable: Advancing Multi-turn Reasoning for Thinking with Images](https://arxiv.org/abs/2512.17306)
*Wenhao Yang,Yu Xia,Jinlong Huang,Shiyin Lu,Qing-Guo Chen,Zhao Xu,Weihua Luo,Kaifu Zhang,Yuanyu Wan,Lijun Zhang*

Main category: cs.CV

TL;DR: DRIM enables deep, reliable multi-turn reasoning in vision-language models by combining supervised fine-tuning with redundancy-penalized reinforcement learning to develop self-reflective reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with self-reflection and correction when following incorrect reasoning trajectories in multimodal chain-of-thought, limiting their reliability in complex visual tasks.

Method: Three-stage pipeline: 1) Construct high-difficulty verifiable visual QA pairs requiring multi-turn tool calls, 2) Cold-start supervised fine-tuning with tool trajectories, 3) Redundancy-penalized RL optimization that penalizes incorrect answers without sufficient multi-scale exploration.

Result: DRIM achieves superior performance on visual understanding benchmarks compared to existing models.

Conclusion: The proposed DRIM framework successfully enables deep and reliable multi-turn reasoning in vision-language models by developing self-reflective reasoning patterns through a combination of SFT and novel RL optimization.
...
### [71] [CodeDance: A Dynamic Tool-integrated MLLM for Executable Visual Reasoning](https://arxiv.org/abs/2512.17312)
...
Method: CodeDance defines, composes, and executes code to orchestrate multiple tools, compute intermediate results, and render visual artifacts. It introduces a reward mechanism for balanced and adaptive tool-call to balance exploration with efficiency and mitigate tool overuse.

Result: CodeDance consistently outperforms schema-driven and text-only baselines across reasoning benchmarks (visual search, math, chart QA), and surpasses advanced closed models like GPT-4o and larger open-source models. It demonstrates novel emergent behaviors including novel tool invocations, unseen compositions, and cross-task transfer without task-specific fine-tuning.

Conclusion: Executable code provides a general and scalable mechanism for visual reasoning that enables transparent, self-checkable reasoning and emergent capabilities beyond atomic supervision, suggesting a promising direction for flexible visual reasoning systems.
...
### [72] [Auxiliary Descriptive Knowledge for Few-Shot Adaptation of Vision-Language Model](https://arxiv.org/abs/2512.17313)
*SuBeen Lee,GilHan Park,WonJun Moon,Hyun Seok Seong,Jae-Pil Heo*

Main category: cs.CV

TL;DR: ADK enhances VLMs with LLM-generated descriptive prompts to improve few-shot adaptation without computational overhead at inference.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with distribution shifts in downstream tasks, and existing PEFT methods rely on fixed handcrafted prompts that lack semantic understanding of classes. Image-induced prompts help but add prohibitive computational overhead.

Method: ADK uses LLMs to generate descriptive prompts offline, then deploys them as: (1) Compositional Knowledge (averaged representation for rich semantics) and (2) Instance-Specific Knowledge (lightweight attention to select relevant descriptions per image). It's a parameter-free plug-and-play component.

Result: ADK consistently boosts performance of multiple PEFT baselines and sets new state-of-the-art across various scenarios.

Conclusion: ADK efficiently enriches text representations for VLMs without compromising efficiency, enhancing category distinction across domains as a parameter-free enhancement to existing PEFT methods.
...
### [73] [A Benchmark for Ultra-High-Resolution Remote Sensing MLLMs](https://arxiv.org/abs/2512.17319)
*Yunkai Dang,Meiyi Zhu,Donghao Wang,Yizhuo Zhang,Jiacheng Yang,Qi Fan,Yuekun Yang,Wenbin Li,Feng Miao,Yang Gao*

Main category: cs.CV

TL;DR: RSHR-Bench is a super-high-resolution remote sensing benchmark with 5,329 full-scene images (up to 300M pixels each) to properly evaluate visual understanding in multimodal LLMs, addressing flaws in existing benchmarks where text-only models could perform well without images.


<details>
  <summary>Details</summary>
Motivation: Current remote sensing benchmarks have issues: most use low-resolution imagery, and some high-resolution benchmarks have flawed reasoning-task designs. The authors discovered that text-only LLMs can perform competitively with multimodal vision-language models on RS reasoning tasks without accessing images, revealing a critical mismatch between benchmarks and intended visual understanding evaluation.

Method: Created RSHR-Bench with 5,329 full-scene images (long side ≥4,000 pixels, up to ~300M pixels per image) from widely used RS corpora and UAV collections. Designed four task families: multiple-choice VQA, open-ended VQA, image captioning, and single-image evaluation. Applied adversarial filtering with strong LLMs followed by rigorous human verification to reduce language priors.

Result: Constructed 3,864 VQA tasks, 3,913 image captioning tasks, and 500 fully human-written/verified single-image evaluation VQA pairs. Evaluations across open-source, closed-source, and RS-specific VLMs revealed persistent performance gaps in super-high-resolution scenarios, showing current models struggle with such detailed imagery.

Conclusion: RSHR-Bench enables faithful assessment of visual understanding in multimodal LLMs for remote sensing by providing a super-high-resolution benchmark that properly tests visual reasoning capabilities, addressing limitations of existing benchmarks where language priors could artificially inflate performance.
...
### [74] [EMMA: Concept Erasure Benchmark with Comprehensive Semantic Metrics and Diverse Categories](https://arxiv.org/abs/2512.17320)
*Lu Wei,Yuta Nakashima,Noa Garcia*

Main category: cs.CV

TL;DR: EMMA benchmark evaluates concept erasure methods across 5 dimensions and 12 metrics, revealing limitations in handling implicit prompts and visually similar concepts, with some methods amplifying bias.


<details>
  <summary>Details</summary>
Motivation: Text-to-image generation raises privacy, bias, and copyright concerns. Concept erasure techniques promise to remove unwanted concepts without full retraining, but current evaluations are limited and simplistic, failing to test whether concepts are truly removed from model representations.

Method: Introduces EMMA benchmark that evaluates 5 key dimensions of concept erasure over 12 metrics. Goes beyond standard metrics to test robustness under challenging conditions: indirect descriptions, visually similar non-target concepts, and potential gender/ethnicity bias. Analyzes 5 concept erasure methods across 5 domains (objects, celebrities, art styles, NSFW, copyright).

Result: Existing methods struggle with implicit prompts (generating erased concepts when indirectly referenced) and visually similar non-target concepts (failing to generate non-targeted concepts resembling erased ones). Some methods amplify gender and ethnicity bias compared to the original model.

Conclusion: Current concept erasure methods have significant limitations in robustness and social awareness. The EMMA benchmark provides comprehensive evaluation framework to better assess concept erasure techniques, revealing gaps in handling indirect references and bias amplification.
...
### [75] [Rotterdam artery-vein segmentation (RAV) dataset](https://arxiv.org/abs/2512.17322)
*Jose Vargas Quiros,Bart Liefers,Karin van Garderen,Jeroen Vermeulen,Eyened Reading Center,Caroline Klaver*

Main category: cs.CV

TL;DR: A diverse, high-quality dataset of color fundus images with detailed artery-vein segmentation annotations for developing and evaluating machine learning algorithms in ophthalmology.
...
Method: Color fundus images were sampled from the longitudinal Rotterdam Study, covering wide age ranges, devices, and capture conditions. Annotations were created using a custom interface allowing graders to label arteries, veins, and unknown vessels on separate layers, starting from initial vessel segmentation masks. Connectivity was explicitly verified and corrected using connected component visualization tools.

Result: The dataset includes 1024x1024-pixel PNG images in three modalities: original RGB fundus images, contrast-enhanced versions, and RGB-encoded artery-vein masks. It contains challenging samples typically excluded by automated quality assessment systems but judged to contain valuable vascular information.

Conclusion: This dataset offers a rich and heterogeneous source of color fundus images with high-quality segmentations, supporting robust benchmarking and training of machine learning models under real-world variability in image quality and acquisition settings.
...
### [76] [DESSERT: Diffusion-based Event-driven Single-frame Synthesis via Residual Training](https://arxiv.org/abs/2512.17323)
...
<details>
  <summary>Details</summary>
Motivation: Traditional video frame prediction struggles with dynamic scenes due to lack of future information. Event cameras capture high-temporal-resolution brightness changes but existing event-based methods using optical flow prediction suffer from holes and blurring when pixel displacement is inaccurate.

Method: Two-stage training: (1) Event-to-Residual Alignment VAE (ER-VAE) aligns event frames between anchor and target frames with corresponding residuals, (2) diffusion model denoises residual latent conditioned on event data. Uses Diverse-Length Temporal augmentation for robustness. Leverages pre-trained Stable Diffusion model trained on inter-frame residuals.

Result: Outperforms existing methods in event-based reconstruction, image-based video frame prediction, event-based video frame prediction, and one-sided event-based video frame interpolation, producing sharper and more temporally consistent frame synthesis.

Conclusion: DESSERT successfully addresses limitations of prior event-based video frame prediction methods by using diffusion-based residual training, achieving superior performance in generating future frames from event camera data.
...
### [77] [Democratizing Pathology Co-Pilots: An Open Pipeline and Dataset for Whole-Slide Vision-Language Modelling](https://arxiv.org/abs/2512.17326)
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models for pathology have limitations: they focus on small regions, provide only static slide-level outputs, rely on non-public data, and lack sufficient training data with detailed clinical reports, hindering reproducible and generalizable VLMs.

Method: Three main contributions: 1) Polysome - standardized tool for synthetic instruction generation, 2) HISTAI-Instruct - large whole-slide instruction tuning dataset generated using Polysome on HISTAI dataset, 3) ANTONI-α - VLM trained on HISTAI-Instruct for visual-question answering tasks.
...
Conclusion: The paper addresses key limitations in pathology VLMs by providing standardized tools, large public datasets, and a high-performing model, advancing transparent and reproducible research in computational pathology.
...
### [78] [SynergyWarpNet: Attention-Guided Cooperative Warping for Neural Portrait Animation](https://arxiv.org/abs/2512.17331)
...
Method: Three-stage progressive refinement: 1) Explicit warping with 3D dense optical flow for coarse alignment, 2) Reference-augmented correction using cross-attention across 3D keypoints and texture features from multiple references, 3) Confidence-guided fusion with learned confidence maps for adaptive blending.
...
Conclusion: SynergyWarpNet effectively addresses limitations of existing warping approaches by combining explicit geometric alignment with attention-based semantic completion, achieving high-fidelity portrait animation.
...
### [79] [Multi-level distortion-aware deformable network for omnidirectional image super-resolution](https://arxiv.org/abs/2512.17343)
*Cuixin Yang,Rongkang Dong,Kin-Man Lam,Yuhang Zhang,Guoping Qiu*

Main category: cs.CV

TL;DR: MDDN: Multi-level Distortion-aware Deformable Network for OmniDirectional Image Super-Resolution that addresses latitude-dependent geometric distortion in ERP images through parallel deformable branches with varying dilation rates.


<details>
  <summary>Details</summary>
Motivation: Existing ODISR methods have limited sampling ranges and feature extraction capabilities, which hinder their ability to capture distorted patterns over large areas in ERP-projected omnidirectional images where distortion becomes severe toward the poles.

Method: Proposes MDDN with three parallel branches: deformable attention (dilation=1) and two dilated deformable convolutions (dilation rates 2 and 3) to expand sampling range, plus multi-level feature fusion module and low-rank decomposition for computational efficiency.
...
Conclusion: MDDN successfully addresses the geometric distortion problem in ODISR by expanding sampling range and receptive field through deformable feature extraction, achieving superior performance while maintaining computational efficiency.
...
### [80] [Beyond Semantic Features: Pixel-level Mapping for Generalized AI-Generated Image Detection](https://arxiv.org/abs/2512.17350)
...
<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors fail to generalize to unseen generative models because they overfit to source-specific semantic cues rather than learning universal generative artifacts, limiting their practical utility.
...
Result: The approach significantly boosts cross-generator performance of state-of-the-art detectors across both GAN and diffusion-based generators, with extensive analysis verifying that disrupting semantic cues is key to generalization.

Conclusion: Breaking semantic shortcuts through pixel-level disruption enables detectors to learn more fundamental generative artifacts, leading to improved generalization across different AI image generators.
...
### [81] [Towards Deeper Emotional Reflection: Crafting Affective Image Filters with Generative Priors](https://arxiv.org/abs/2512.17376)
*Peixuan Zhang,Shuchen Weng,Jiajun Tang,Si Li,Boxin Shi*

Main category: cs.CV

TL;DR: The paper proposes Affective Image Filter (AIF) task to transform text with emotional content into emotionally compelling images, introducing two models (AIF-B and AIF-D) that outperform existing methods in both content consistency and emotional fidelity.


<details>
  <summary>Details</summary>
Motivation: Social media users express emotions through text with images, but there's a gap between visually-abstract emotions in text and visually-concrete images. The authors aim to bridge this gap by creating emotionally compelling results that better reflect the emotional content of text.
...
Result: Quantitative and qualitative experiments show AIF models achieve superior performance in content consistency and emotional fidelity compared to state-of-the-art methods. User studies confirm AIF models are significantly more effective at evoking specific emotions.

Conclusion: The paper demonstrates the value and potential of AIF models for creating emotionally compelling image-text combinations, with promising applications for social media and emotional content generation.
...
### [82] [Beyond Occlusion: In Search for Near Real-Time Explainability of CNN-Based Prostate Cancer Classification](https://arxiv.org/abs/2512.17416)
*Martin Krebs,Jan Obdržálek,Vít Musil,Tomáš Brázdil*

Main category: cs.CV

TL;DR: Researchers found a faster alternative to occlusion-based explanation methods for prostate cancer detection AI, reducing computation time by 10x while maintaining quality, enabling faster model development and clinical adoption.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks for cancer diagnosis need explainable outputs for clinical acceptance, but occlusion explanation methods are computationally slow, hindering development and interaction with pathologists.

Method: Developed a framework to compare explanation methods using suitable criteria and metrics, then selected an alternative method that maintains explanation quality while being much faster than occlusion.

Result: Found a replacement explanation method that reduces required explanation time by at least a factor of 10 without negative impact on output quality.

Conclusion: The speedup enables rapid iteration in model development and brings AI-assisted prostate cancer detection closer to clinical adoption; the evaluation approach can be applied to other applications.
...
### [83] [AIFloodSense: A Global Aerial Imagery Dataset for Semantic Segmentation and Understanding of Flooded Environments](https://arxiv.org/abs/2512.17432)
...
<details>
  <summary>Details</summary>
Motivation: Flood detection datasets are scarce, geographically limited, and lack detailed annotations, hindering development of robust computer vision methods for disaster response and risk assessment.

Method: Created AIFloodSense dataset with 470 high-resolution aerial images from 2022-2024 flood events across 64 countries, supporting three tasks: image classification (environment type, camera angle, continent), semantic segmentation (flood, sky, buildings), and visual question answering.
...
### [84] [Xiaomi MiMo-VL-Miloco Technical Report](https://arxiv.org/abs/2512.17436)
...
Method: Built on MiMo-VL-7B backbone, using a two-stage training pipeline combining supervised fine-tuning with reinforcement learning based on Group Relative Policy Optimization. Incorporates chain-of-thought supervision and token-budget-aware reasoning for efficient learning and reasoning. Leverages multi-domain data for training.

Result: Achieves leading F1 scores on gesture recognition and home-scenario understanding. Shows consistent gains on video benchmarks (Video-MME, Video-MMMU, Charades-STA) and language understanding benchmarks (MMMU-Pro, MMLU-Pro). Outperforms both closed-source and open-source baselines on home-scenario understanding and multimodal reasoning tasks.

Conclusion: Targeted home-scenario training enhances both activity/gesture understanding and text-only reasoning with minimal trade-offs on document-centric tasks. The model successfully balances specialization and generality, making it suitable for real-world smart-home applications. The open-source release supports research and deployment.
...
### [85] [LangDriveCTRL: Natural Language Controllable Driving Scene Editing with Multi-modal Agents](https://arxiv.org/abs/2512.17445)
*Yun He,Francesco Pittaluga,Ziyu Jiang,Matthias Zwicker,Manmohan Chandraker,Zaid Tasneem*

Main category: cs.CV

TL;DR: LangDriveCTRL is a framework for editing real-world driving videos using natural language instructions, enabling creation of diverse traffic scenarios through 3D scene decomposition and agentic pipeline coordination.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enable natural language control for editing real-world driving videos to synthesize diverse traffic scenarios, addressing the need for fine-grained editing capabilities while maintaining realism in driving video manipulation.

Method: The method uses explicit 3D scene decomposition to represent driving videos as scene graphs with static background and dynamic objects. It employs an agentic pipeline where an Orchestrator transforms user instructions into execution graphs that coordinate specialized agents: Object Grounding Agent (text-to-object correspondence), Behavior Editing Agent (multi-object trajectory generation), and Behavior Reviewer Agent (trajectory refinement). Edited scenes are rendered and refined using video diffusion tools to address artifacts.
...
Conclusion: The framework successfully enables natural-language-controllable editing of real-world driving videos, providing a powerful tool for synthesizing diverse traffic scenarios with high realism and instruction alignment through its agentic pipeline and 3D scene decomposition approach.
...
### [86] [MULTIAQUA: A multimodal maritime dataset and robust training strategies for multimodal semantic segmentation](https://arxiv.org/abs/2512.17450)
...
<details>
  <summary>Details</summary>
Motivation: Unmanned surface vehicles face challenging visual conditions where RGB cameras alone are insufficient, especially in poor weather and lighting. There's a need for multimodal data to enable reliable scene interpretation regardless of visibility.

Method: Created MULTIAQUA dataset with synchronized, calibrated, annotated multimodal sensor data (RGB, thermal, IR, LIDAR). Developed training approaches that enable multimodal methods to be trained robustly using only daytime images.

Result: Dataset enables development of supervised methods for high-quality scene interpretation in poor visibility. Training approaches allow multimodal methods to retain reliable performance even in near-complete darkness.

Conclusion: MULTIAQUA dataset addresses maritime data scarcity and enables robust multimodal perception for unmanned surface vehicles, simplifying data acquisition and training while improving performance in challenging conditions.
...
### [87] [3D-RE-GEN: 3D Reconstruction of Indoor Scenes with a Generative Framework](https://arxiv.org/abs/2512.17459)
*Tobias Sautter,Jan-Niklas Dihlmann,Hendrik P. A. Lensch*

Main category: cs.CV

TL;DR: 3D-RE-GEN is a compositional framework that reconstructs single images into textured 3D mesh scenes suitable for artists' workflows, addressing limitations of current methods through integrated detection, reconstruction, and placement models with scene-level reasoning.


<details>
  <summary>Details</summary>
Motivation: Current 3D scene generation methods produce visually appealing output but lack artist-friendly modifiable textured mesh scenes needed for visual effects and game development. Existing reconstruction methods suffer from incorrect object decomposition, inaccurate spatial relationships, and missing backgrounds.

Method: A compositional pipeline integrating state-of-the-art models for asset detection, reconstruction, and placement. Uses generative models for occluded object inference with scene-level reasoning under consistent lighting/geometry. Includes novel 4-DoF differentiable optimization for ground plane alignment and comprehensive background generation for spatial constraints.
...
Conclusion: 3D-RE-GEN successfully addresses artists' requirements by combining domain-specific models to create artist-ready textured 3D mesh scenes with proper object decomposition, spatial relationships, and backgrounds for visual effects and game development workflows.
...
### [88] [TwinSegNet: A Digital Twin-Enabled Federated Learning Framework for Brain Tumor Analysis](https://arxiv.org/abs/2512.17488)
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for brain tumor segmentation require centralized data collection, which raises privacy concerns and limits generalization across diverse institutions. There's a need for privacy-preserving solutions that can work across multiple healthcare institutions while maintaining data confidentiality.

Method: Proposes TwinSegNet, a federated learning framework that integrates a hybrid ViT-UNet model with personalized digital twins. The architecture combines convolutional encoders with Vision Transformer bottlenecks to capture both local and global context. Each institution fine-tunes the global model on their private data to create personalized digital twins, enabling privacy-preserving collaborative learning.

Result: Evaluated on nine heterogeneous MRI datasets (including BraTS 2019-2021 and custom tumor collections), TwinSegNet achieves high Dice scores (up to 0.90%) and sensitivity/specificity exceeding 90%. The framework demonstrates robustness across non-IID client distributions and performs comparably to centralized models like TumorVisNet while preserving privacy.

Conclusion: TwinSegNet enables scalable, personalized brain tumor segmentation for multi-institutional clinical settings while adhering to strict data confidentiality requirements. The approach effectively balances privacy preservation with segmentation performance, making it suitable for real-world healthcare applications where data sharing is restricted.
...
### [89] [LumiCtrl : Learning Illuminant Prompts for Lighting Control in Personalized Text-to-Image Models](https://arxiv.org/abs/2512.17489)
...
Result: LumiCtrl achieves significantly better illuminant fidelity, aesthetic quality, and scene coherence compared to existing personalization baselines, with strong user preference confirmed in human studies.

Conclusion: The method enables precise illuminant control in T2I generation through illuminant personalization, addressing a key limitation in current models for creative image generation.
...
### [90] [MMLANDMARKS: a Cross-View Instance-Level Benchmark for Geo-Spatial Understanding](https://arxiv.org/abs/2512.17492)
*Oskar Kristoffersen,Alba R. Sánchez,Morten R. Hannemose,Anders B. Dahl,Dim P. Papadopoulos*

Main category: cs.CV

TL;DR: MMLANDMARKS is a multimodal geo-spatial benchmark with 4 modalities (aerial images, ground-view images, text, GPS) for 18,557 US landmarks, enabling training/evaluation of models for cross-view retrieval, geolocalization, and text-to-image/GPS tasks.


<details>
  <summary>Details</summary>
Motivation: Current geo-spatial benchmarks have limited modality coverage, restricting progress as approaches cannot integrate all relevant modalities (images from various viewpoints, text, coordinates) within a unified framework.

Method: Introduces MMLANDMARKS dataset with 197k aerial images, 329k ground-view images, text, and GPS coordinates for 18,557 US landmarks, featuring one-to-one correspondence across all modalities. Uses a simple CLIP-inspired baseline model.

Result: Demonstrates broad generalization and competitive performance against off-the-shelf foundational models and specialized state-of-the-art models across different geo-spatial tasks using the simple baseline.

Conclusion: The dataset enables training/benchmarking for various geo-spatial tasks, illustrating the necessity for multimodal datasets to achieve comprehensive geo-spatial understanding beyond single-modality approaches.
...
### [91] [GroundingME: Exposing the Visual Grounding Gap in MLLMs through Multi-Dimensional Evaluation](https://arxiv.org/abs/2512.17495)
...
TL;DR: GroundingME is a new benchmark that reveals major gaps in MLLMs' visual grounding capabilities, showing they struggle with real-world complexity and often hallucinate objects instead of rejecting ungroundable queries.
...
Conclusion: GroundingME reveals a profound capability gap in MLLMs' visual grounding, raising safety concerns for deployment. It serves as both a diagnostic tool and roadmap toward human-level visual grounding, showing current models lack sophistication in handling real-world complexity.
...
### [92] [Validation of Diagnostic Artificial Intelligence Models for Prostate Pathology in a Middle Eastern Cohort](https://arxiv.org/abs/2512.17499)
...
TL;DR: First external validation of prostate cancer AI pathology models on Middle Eastern population shows pathologist-level performance and high cross-scanner consistency, supporting global AI adoption.


<details>
  <summary>Details</summary>
Motivation: Current AI pathology systems are primarily validated on European/US cohorts, creating a gap for underrepresented populations where AI could have the greatest impact. There's a need for validation studies in diverse global populations to enable equitable AI adoption in pathology.

Method: Collected and digitized 339 prostate biopsy specimens from 185 consecutive patients (2013-2024) in Kurdistan, Iraq. Evaluated three AI models (task-specific end-to-end model and two foundation models) for concordance with pathologists and consistency across three scanner models (Hamamatsu, Leica, Grundium).

Result: AI-pathologist grading concordance (Cohen's quadratically weighted kappa 0.801) was statistically equivalent to pathologist-pathologist concordance (0.799, p=0.9824). High cross-scanner concordance (kappa > 0.90) for all AI models and scanner pairs, including low-cost compact scanner.

Conclusion: AI models achieve pathologist-level performance in prostate histopathology. Compact scanners enable validation studies in non-digitalized settings and cost-effective AI adoption. First openly available Middle Eastern digital pathology dataset supports research into globally equitable AI pathology.
...
### [93] [InsertAnywhere: Bridging 4D Scene Geometry and Diffusion Models for Realistic Video Object Insertion](https://arxiv.org/abs/2512.17504)
*Hoiyeong Jin,Hyojin Jang,Jeongho Kim,Junha Hyung,Kinam Kim,Dongjin Kim,Huijin Choi,Hyeonji Kim,Jaegul Choo*

Main category: cs.CV

TL;DR: InsertAnywhere is a diffusion-based framework for realistic video object insertion that achieves geometrically consistent placement and appearance-faithful synthesis through 4D scene understanding and joint object/surrounding synthesis.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based video generation struggles with realistic video object insertion due to limited 4D scene understanding and inadequate handling of occlusion and lighting effects, making realistic object placement challenging.

Method: Uses a 4D aware mask generation module for scene geometry reconstruction and temporal object placement, extends diffusion-based video generation to jointly synthesize inserted objects and local variations (illumination/shading), and trains on ROSE++ dataset with VLM-generated reference images.
...
Conclusion: InsertAnywhere successfully addresses key challenges in video object insertion through comprehensive 4D scene understanding and joint synthesis of objects with their environmental interactions, advancing realistic video editing capabilities.
...
### [94] [Foundation Model Priors Enhance Object Focus in Feature Space for Source-Free Object Detection](https://arxiv.org/abs/2512.17514)
*Sairam VCR,Rishabh Lalla,Aveen Dayal,Tejal Kulkarni,Anuj Lalla,Vineeth N Balasubramanian,Muhammad Haris Khan*

Main category: cs.CV

TL;DR: FALCON-SFOD improves source-free object detection by strengthening object-focused representations using foundation model guidance and noise-robust pseudo-labeling, addressing domain shift issues that cause background clutter activation.
...
Conclusion: Strengthening the feature space through foundation model guidance and noise-robust pseudo-labeling is crucial for effective source-free object detection under domain shift, addressing the core limitation of current approaches.
...
### [95] [PathBench-MIL: A Comprehensive AutoML and Benchmarking Framework for Multiple Instance Learning in Histopathology](https://arxiv.org/abs/2512.17517)
*Siemen Brussee,Pieter A. Valkema,Jurre A. J. Weijer,Thom Doeleman,Anne M. R. Schrader,Jesper Kers*

Main category: cs.CV

TL;DR: PathBench-MIL is an open-source AutoML and benchmarking framework for multiple instance learning in histopathology that automates end-to-end MIL pipelines and provides reproducible benchmarking of models and feature extractors.


<details>
  <summary>Details</summary>
Motivation: There is a need for standardized, reproducible benchmarking and automated pipeline construction for multiple instance learning in histopathology to enable rapid experimentation and comparison across different MIL models and feature extractors.

Method: The framework automates end-to-end MIL pipeline construction including preprocessing, feature extraction, and MIL-aggregation. It provides a unified configuration system, visualization tooling, and modular extensibility for integrating various MIL models and feature extractors.

Result: PathBench-MIL enables reproducible benchmarking of dozens of MIL models and feature extractors, standardizes experimentation across datasets and tasks, and is publicly available as open-source software.

Conclusion: PathBench-MIL provides a comprehensive AutoML and benchmarking solution for MIL in histopathology that facilitates rapid experimentation, standardization, and reproducibility in the field.
...
### [96] [Robust-R1: Degradation-Aware Reasoning for Robust Visual Understanding](https://arxiv.org/abs/2512.17532)
*Jiaqi Tang,Jianmin Chen,Wei Wei,Xiaogang Xu,Runtao Liu,Xiangyu Wu,Qipeng Xie,Jiafei Wu,Lei Zhang,Qifeng Chen*

Main category: cs.CV

TL;DR: Robust-R1 is a novel framework that explicitly models visual degradations through structured reasoning chains to improve multimodal LLM robustness, outperforming existing methods on real-world degradation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs struggle with real-world visual degradations, and existing robust approaches rely on implicit training that lacks interpretability and focuses only on visual encoder generalization, leading to isolated optimization.

Method: Proposes Robust-R1 with three key components: (1) supervised fine-tuning for degradation-aware reasoning foundations, (2) reward-driven alignment for accurate degradation parameter perception, and (3) dynamic reasoning depth scaling adapted to degradation intensity. Also introduces an 11K dataset with realistic degradations across four visual processing stages, annotated with structured reasoning chains.
...
Conclusion: Explicitly modeling visual degradations through structured reasoning chains provides a more interpretable and effective approach for improving multimodal LLM robustness against real-world visual degradations compared to implicit training methods.
...
### [97] [FLEG: Feed-Forward Language Embedded Gaussian Splatting from Any Views](https://arxiv.org/abs/2512.17541)
*Qijian Tian,Xin Tan,Jiayu Ying,Xuhong Wang,Yuan Xie,Lizhuang Ma*

Main category: cs.CV

TL;DR: FLEG is a feed-forward network that reconstructs language-embedded 3D Gaussians from arbitrary views without requiring 3D annotations, using large-scale video data and instance-guided contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Previous methods for 3D Gaussian reconstruction suffer from fixed input views and insufficient 3D training data, requiring 3D annotations which are hard to obtain.

Method: Proposes a 3D-annotation-free training framework using 2D-to-3D lifting from arbitrary uncalibrated multi-view images, leveraging large-scale video data with 2D instance information, instance-guided contrastive learning to align 2D semantics with 3D representations, and geometry-semantic hierarchical sparsification to reduce computational cost.

Result: FLEG efficiently reconstructs language-embedded 3D Gaussian representation from arbitrary sparse or dense views, producing accurate geometry, high-fidelity appearance, and language-aligned semantics, outperforming existing methods on various tasks.

Conclusion: The proposed framework enables effective 3D reconstruction with semantic understanding without requiring 3D annotations, making it practical for real-world applications with arbitrary view inputs.
...
### [98] [ClothHMR: 3D Mesh Recovery of Humans in Diverse Clothing from Single Image](https://arxiv.org/abs/2512.17545)
*Yunqi Gao,Leyuan Liu,Yuhan Li,Changxin Gao,Yuanyuan Liu,Jingying Chen*

Main category: cs.CV

TL;DR: ClothHMR: A novel 3D human mesh recovery method that handles diverse clothing by tailoring garments to fit body silhouettes and leveraging large foundational human visual models for better generalization.


<details>
  <summary>Details</summary>
Motivation: Current 3D human mesh recovery methods perform poorly with diverse clothing, especially loose garments, as they mainly focus on tight clothing. There's a need for accurate body shape and pose estimation under various clothing conditions.

Method: ClothHMR uses two key modules: (1) Clothing Tailoring (CT) that employs body semantic estimation and edge prediction to tailor clothing to fit body silhouettes, and (2) FHVM-based Mesh Recovering (MR) that optimizes 3D mesh parameters by aligning intermediate representations with those from foundational human visual models.

Result: ClothHMR significantly outperforms existing state-of-the-art methods across benchmark datasets and in-the-wild images, accurately recovering 3D meshes of humans wearing diverse clothing while precisely estimating body shapes and poses.

Conclusion: ClothHMR effectively addresses the challenge of 3D human mesh recovery under diverse clothing conditions, demonstrates superior performance over existing methods, and has practical applications in online fashion and shopping through a developed web application.
...
### [99] [G3Splat: Geometrically Consistent Generalizable Gaussian Splatting](https://arxiv.org/abs/2512.17547)
*Mehdi Hosseinzadeh,Shin-Fang Chng,Yi Xu,Simon Lucey,Ian Reid,Ravi Garg*

Main category: cs.CV

TL;DR: G3Splat introduces geometric priors to address ambiguities in learning 3D Gaussian splats under self-supervision for pose-free generalizable splatting, achieving SOTA in geometry reconstruction, pose estimation, and novel-view synthesis.


<details>
  <summary>Details</summary>
Motivation: Prior work on 3D Gaussian regression from images relies almost exclusively on view-synthesis supervision, which is insufficient for recovering geometrically meaningful splats. There's a need for geometrically consistent 3D scene representations in pose-free generalizable splatting.

Method: G3Splat enforces geometric priors to address ambiguities of learning 3D Gaussian splats under self-supervision. It goes beyond view-synthesis loss to obtain geometrically consistent representations, trained on RE10K dataset.

Result: Achieves state-of-the-art performance in: (1) geometrically consistent reconstruction, (2) relative pose estimation, and (3) novel-view synthesis. Shows strong zero-shot generalization on ScanNet, substantially outperforming prior work in geometry recovery and pose estimation.

Conclusion: Geometric priors are essential for learning meaningful 3D Gaussian splats from images, and G3Splat demonstrates that view-synthesis loss alone is insufficient for geometrically consistent scene representations. The approach enables effective pose-free generalizable splatting with strong generalization capabilities.
...
### [100] [A unified FLAIR hyperintensity segmentation model for various CNS tumor types and acquisition time points](https://arxiv.org/abs/2512.17566)
...
TL;DR: A unified Attention U-Net model for automatic FLAIR hyperintensity segmentation across various brain tumor types and time points, achieving good performance and generalization comparable to dataset-specific models.


<details>
  <summary>Details</summary>
Motivation: FLAIR MRI scans are crucial for brain tumor diagnosis and monitoring, but manual segmentation of hyperintensity volumes is time-consuming. Automatic segmentation would be clinically useful, especially since hyperintensity volumes are important measures for assessing tumor volume or surrounding edema depending on tumor type.

Method: Used ~5000 FLAIR images from various tumor types, acquisition time points, and different centers to train a unified segmentation model using an Attention U-Net architecture. Compared performance against dataset-specific models and validated across different tumor types, time points, and against the BraTS dataset.

Result: The unified model achieved Dice scores: 88.65% for pre-operative meningiomas, 80.08% for pre-operative metastasis, 90.92% for pre-operative and 84.60% for post-operative gliomas from BraTS, and 84.47% for pre-operative and 61.27% for post-operative lower grade gliomas. It performed comparably to dataset-specific models while enabling generalization across tumor types and time points.

Conclusion: The unified model provides effective automatic FLAIR hyperintensity segmentation that generalizes well across different brain tumor types and acquisition time points, facilitating clinical deployment. The model is integrated into Raidionics, an open-source CNS tumor analysis software.
...
### [101] [RoomEditor++: A Parameter-Sharing Diffusion Architecture for High-Fidelity Furniture Synthesis](https://arxiv.org/abs/2512.17573)
...
Conclusion: RoomEditor++ provides a versatile solution for high-fidelity virtual furniture synthesis, supported by the comprehensive RoomBench++ benchmark dataset, advancing the field for practical applications.
...
### [102] [3One2: One-step Regression Plus One-step Diffusion for One-hot Modulation in Dual-path Video Snapshot Compressive Imaging](https://arxiv.org/abs/2512.17578)
*Ge Wang,Xing Liu,Xin Yuan*

Main category: cs.CV

TL;DR: A novel algorithm combining one-step regression initialization with one-step diffusion refinement for video SCI reconstruction using one-hot masks, addressing temporal aliasing and spatial degradation issues.


<details>
  <summary>Details</summary>
Motivation: Random binary modulation in video SCI causes temporal aliasing during compression. One-hot modulation offers perfect temporal decoupling but lacks algorithms to exploit its potential. The paper aims to bridge this gap by developing a specialized algorithm for one-hot masks.

Method: 1) Transform reconstruction into generative video inpainting using SDE aligned with hardware compression; 2) Propose novel framework combining one-step regression initialization with one-step diffusion refinement; 3) Implement dual optical path hardware to mitigate spatial degradation using complementary information.

Result: Experiments on synthetic datasets and real scenes demonstrate the effectiveness of the proposed method, successfully addressing temporal aliasing and spatial degradation issues in video SCI reconstruction.

Conclusion: This is the first work integrating diffusion into video SCI reconstruction, offering a promising solution for one-hot modulation that achieves perfect temporal decoupling while overcoming spatial degradation through dual optical path hardware.
...
### [103] [Medical Imaging AI Competitions Lack Fairness](https://arxiv.org/abs/2512.17581)
...
TL;DR: Current AI medical imaging benchmarks have fairness issues: datasets lack real-world clinical diversity and have poor accessibility/reusability, limiting clinical relevance despite leaderboard success.


<details>
  <summary>Details</summary>
Motivation: Benchmark competitions drive AI progress in medical imaging but it's unclear if they provide representative, accessible, reusable data for clinically meaningful AI. The study aims to assess fairness in two dimensions: dataset representativeness of real-world clinical diversity, and accessibility/reusability according to FAIR principles.

Method: Conducted large-scale systematic study of 241 biomedical image analysis challenges comprising 458 tasks across 19 imaging modalities. Analyzed dataset composition for biases and evaluated access conditions, licensing practices, and documentation.

Result: Found substantial biases in dataset composition including geographic location, modality-, and problem type-related biases, showing benchmarks don't adequately reflect real-world clinical diversity. Challenge datasets were frequently constrained by restrictive/ambiguous access conditions, inconsistent/non-compliant licensing, and incomplete documentation, limiting reproducibility and long-term reuse.

Conclusion: Current benchmarking ecosystem has foundational fairness limitations with disconnect between leaderboard success and clinical relevance. Need for more representative, accessible, and reusable datasets to support clinically meaningful AI in medical imaging.
...
### [104] [HeadHunt-VAD: Hunting Robust Anomaly-Sensitive Heads in MLLM for Tuning-Free Video Anomaly Detection](https://arxiv.org/abs/2512.17601)
...
<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods require extensive labeled data and are computationally expensive. Existing tuning-free MLLM-based methods rely on textual outputs, which suffer from information loss, normalcy bias, and prompt sensitivity, making them insufficient for capturing subtle anomalous cues.

Method: Proposes HeadHunt-VAD with a Robust Head Identification module that systematically evaluates all attention heads using multi-criteria analysis of saliency and stability. Identifies a sparse subset of consistently discriminative heads across diverse prompts. Features from these expert heads are fed into lightweight anomaly scorer and temporal locator.
...
Conclusion: Head-level probing in MLLMs is a powerful and practical solution for real-world anomaly detection, validating the approach of directly hunting robust anomaly-sensitive internal attention heads within frozen MLLMs.
...
### [105] [MGRegBench: A Novel Benchmark Dataset with Anatomical Landmarks for Mammography Image Registration](https://arxiv.org/abs/2512.17605)
...
Method: Created MGRegBench dataset with over 5,000 mammogram image pairs, including 100 with manual anatomical landmarks and segmentation masks. Benchmarked diverse methods: classical (ANTs), learning-based (VoxelMorph, TransMorph), implicit neural representation (IDIR), mammography-specific approach, and state-of-the-art MammoRegNet.

Result: MGRegBench is one of the largest public 2D registration datasets with manual annotations. The benchmark provides the first like-for-like comparison of diverse methods on mammography modality and extensive analysis of deep learning-based registration.

Conclusion: The paper establishes a foundational resource for fair comparisons in mammography registration, publicly releasing code and data to catalyze future research and address the field's reproducibility challenges.
...
### [106] [Semi-Supervised 3D Segmentation for Type-B Aortic Dissection with Slim UNETR](https://arxiv.org/abs/2512.17610)
*Denis Mikhailapov,Vladimir Berikov*

Main category: cs.CV

TL;DR: A semi-supervised learning method for multi-output CNN medical image segmentation that uses data augmentation (rotations/flipping) without probabilistic assumptions, applied to aortic dissection segmentation.


<details>
  <summary>Details</summary>
Motivation: Multi-output CNNs improve medical image segmentation accuracy but require large labeled datasets, which are expensive/time-consuming to obtain, especially for 3D data like aortic dissection CTAs. Semi-supervised learning can help but isn't well understood for multi-output models.

Method: Proposes a semi-supervised learning method for multi-output segmentation models based on data augmentation (rotations and flipping) that doesn't assume probabilistic model responses, making it universal for architectures with separate segmentation branches.

Result: The method enables training with both labeled and unlabeled data, overcoming labeling challenges for 3D medical images like the ImageTBDA dataset containing 100 CTA images of type B aortic dissection.

Conclusion: This universal semi-supervised approach addresses the data labeling bottleneck for multi-output medical image segmentation, particularly important for complex pathologies like aortic dissection where accurate segmentation of multiple structures (true lumen, false lumen, thrombus) is critical for diagnosis.
...
### [107] [Self-Supervised Weighted Image Guided Quantitative MRI Super-Resolution](https://arxiv.org/abs/2512.17612)
*Alireza Samadifardheris,Dirk H. J. Poot,Florian Wiesinger,Stefan Klein,Juan A. Hernandez-Tamames*

Main category: cs.CV

TL;DR: Physics-informed self-supervised framework for qMRI super-resolution that uses routine HR weighted MRI scans as guidance, eliminating need for HR qMRI ground truth during training.


<details>
  <summary>Details</summary>
Motivation: High-resolution quantitative MRI relaxometry provides objective tissue characterization but is clinically underutilized due to lengthy acquisition times. Need practical solution to enable fast qMRI with acceptable scan time.

Method: Formulate super-resolution as Bayesian MAP inference minimizing two discrepancies: (1) between HR images synthesized from super-resolved qMRI maps and acquired wMRI guides via forward signal models, and (2) between acquired LR qMRI and downsampled predictions. Use deep neural network to learn super-resolution mapping from synthetic training data.
...
### [108] [StereoMV2D: A Sparse Temporal Stereo-Enhanced Framework for Robust Multi-View 3D Object Detection](https://arxiv.org/abs/2512.17620)
*Di Wu,Feng Yang,Wenhui Zhao,Jinwen Yu,Pan Liao,Benlian Xu,Dingwen Zhang*

Main category: cs.CV

TL;DR: StereoMV2D improves multi-view 3D object detection by integrating temporal stereo modeling with 2D detection-guided queries, enhancing depth perception through cross-frame disparities while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing sparse query-based 3D detectors use 2D detection results for query initialization, but suffer from depth ambiguity in single-frame 2D detections, limiting 3D query accuracy.

Method: Proposes StereoMV2D framework that integrates temporal stereo modeling by exploiting cross-temporal disparities of objects across adjacent frames. Uses dynamic confidence gating to evaluate reliability of temporal stereo cues through learning statistical patterns from inter-frame matching matrix and appearance consistency.
...
Conclusion: StereoMV2D effectively addresses depth ambiguity in 2D-guided 3D detection through temporal stereo modeling, achieving better accuracy while maintaining efficiency.
...
### [109] [PathFLIP: Fine-grained Language-Image Pretraining for Versatile Computational Pathology](https://arxiv.org/abs/2512.17621)
*Fengchun Liu,Songhan Jiang,Linghan Cai,Ziyue Wang,Yongbing Zhang*

Main category: cs.CV

TL;DR: PathFLIP is a novel pathology VLM framework that decomposes slide-level captions into region-level subcaptions for fine-grained WSI-text alignment, enabling precise lesion localization and instruction following with less training data.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs struggle with gigapixel-scale WSIs and spatial heterogeneity, failing to capture fine-grained correspondences between text and thousands of visual patches, which compromises downstream task performance in computational pathology.
...
Result: PathFLIP outperforms existing large-scale pathological VLMs on four representative benchmarks while requiring significantly less training data, demonstrating superior performance on slide-level classification/retrieval, lesion localization, and instruction following.

Conclusion: PathFLIP paves the way for fine-grained, instruction-aware WSI interpretation in clinical practice by enabling precise multimodal understanding of gigapixel pathology images with efficient training requirements.
...
### [110] [Generative Human-Object Interaction Detection via Differentiable Cognitive Steering of Multi-modal LLMs](https://arxiv.org/abs/2512.17640)
*Zhaolin Cai,Huiyu Duan,Zitong Xu,Fan Li,Zhi Liu,Jing Liu,Wei Shen,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

TL;DR: GRASP-HO reformulates HOI detection from closed-set classification to open-vocabulary generation using a lightweight cognitive steering module to inject visual evidence into frozen MLLMs, achieving both closed-set SOTA and zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing HOI detection methods operate under closed-world assumptions with predefined verb sets, struggling with unseen or ambiguous interactions in the wild. MLLMs have rich world knowledge but are computationally prohibitive to fine-tune, remaining decoupled from HOI detectors.

Method: Proposes GRASP-HO framework that extracts hybrid interaction representations and uses a lightweight learnable cognitive steering conduit (CSC) module to inject fine-grained visual evidence into a frozen MLLM. Introduces hybrid guidance strategy combining language modeling loss and auxiliary classification loss to address supervision mismatch.
...
Conclusion: GRASP-HO successfully bridges the gap between vision and cognition for HOI detection, enabling open-vocabulary understanding without expensive MLLM fine-tuning, creating a unified framework for both closed-set and open-world scenarios.
...
### [111] [Region-Constraint In-Context Generation for Instructional Video Editing](https://arxiv.org/abs/2512.17650)
*Zhongwei Zhang,Fuchen Long,Wei Li,Zhaofan Qiu,Wu Liu,Ting Yao,Tao Mei*

Main category: cs.CV

TL;DR: ReCo introduces a new instructional video editing paradigm with constraint modeling between editing and non-editing regions during in-context generation, using joint denoising and two regularization terms to improve editing accuracy.


<details>
  <summary>Details</summary>
Motivation: Current in-context generation methods for instructional image editing don't translate well to video editing, leading to inaccurate editing regions and token interference between editing and non-editing areas during denoising.

Method: ReCo uses width-wise concatenation of source and target videos for joint denoising, with two regularization terms: latent regularization (increases latent discrepancy in editing regions while reducing it in non-editing areas) and attention regularization (suppresses attention of editing region tokens to source video counterparts). Also introduces ReCo-Data dataset with 500K instruction-video pairs.
...
Conclusion: ReCo presents an effective instructional video editing paradigm that addresses region accuracy and token interference issues through constraint modeling and regularization techniques, with a large-scale dataset to support training.
...
Result: Bitbox bridges the translational gap by giving behavioral researchers access to robust behavioral metrics without engineering expertise, while providing computer scientists a practical dissemination mechanism. Core modules have been validated on clinical samples.

Conclusion: Bitbox is a community-driven toolkit that will accelerate integration of computational behavioral measurement into behavioral, clinical, and mental health research by removing technical barriers and enabling broader adoption.
...
### [113] [Learning Spatio-Temporal Feature Representations for Video-Based Gaze Estimation](https://arxiv.org/abs/2512.17673)
*Alexandre Personnic,Mihai Bâce*

Main category: cs.CV

TL;DR: ST-Gaze: A spatio-temporal network combining CNN with attention modules for video-based gaze estimation, achieving SOTA on EVE dataset with better intra-frame context modeling.


<details>
  <summary>Details</summary>
Motivation: Current video-based gaze estimation methods struggle to capture both spatial relationships within frames and temporal dynamics between frames simultaneously, limiting performance.

Method: ST-Gaze combines CNN backbone with channel attention and self-attention modules to fuse eye and face features, treats fused features as spatial sequence for intra-frame context, then propagates through time for inter-frame dynamics.

Result: Achieves state-of-the-art performance on EVE dataset both with and without person-specific adaptation; ablation study shows modeling intra-frame spatial context with spatio-temporal recurrence is superior to premature spatial pooling.

Conclusion: The approach enables more robust video-based gaze estimation using commonly available cameras by better preserving and modeling spatial context within temporal sequences.
...
### [114] [An Empirical Study of Sampling Hyperparameters in Diffusion-Based Super-Resolution](https://arxiv.org/abs/2512.17675)
*Yudhistira Arief Wibowo*

Main category: cs.CV

TL;DR: The paper analyzes conditioning methods for diffusion models in image super-resolution, finding that conditioning step size is more important than diffusion step count, with optimal step sizes between 2.0-3.0.


<details>
  <summary>Details</summary>
Motivation: Conditioning methods like DPS and MCG improve reconstruction quality in diffusion-based inverse problems but introduce hyperparameters that require careful tuning. The authors aim to identify dominant factors affecting performance when applying conditioning to pretrained diffusion models.

Method: Conducted an empirical ablation study on FFHQ super-resolution to analyze the impact of different hyperparameters in conditioning methods for diffusion models. Specifically examined the relative importance of conditioning step size versus diffusion step count.

Result: Found that conditioning step size has a significantly greater impact on performance than diffusion step count. Step sizes in the range of [2.0, 3.0] yielded the best overall performance in their experiments.

Conclusion: For diffusion-based super-resolution, careful tuning of conditioning step size is crucial, with optimal values around 2.0-3.0, while diffusion step count has less impact on final reconstruction quality.
...
### [115] [FlexAvatar: Flexible Large Reconstruction Model for Animatable Gaussian Head Avatars with Detailed Deformation](https://arxiv.org/abs/2512.17717)
*Cheng Peng,Zhuo Su,Liao Wang,Chen Guo,Zhaohu Li,Chengjiang Long,Zheng Lv,Jingxiang Sun,Chenyangguang Zhang,Yebin Liu*

Main category: cs.CV

TL;DR: FlexAvatar is a flexible 3D head avatar reconstruction model that creates high-fidelity avatars with detailed dynamic deformation from single or sparse images, without requiring camera poses or expression labels.


<details>
  <summary>Details</summary>
Motivation: Current methods for 3D head avatar creation often require camera poses, expression labels, or multiple input images, limiting practical applications. There's a need for flexible, high-fidelity reconstruction from minimal inputs that can capture detailed dynamic deformations like wrinkles and facial expressions.

Method: Uses transformer-based reconstruction with structured head query tokens as canonical anchors to aggregate flexible inputs. Includes a lightweight UNet decoder conditioned on UV-space position maps for real-time detailed expression-dependent deformations. Employs data distribution adjustment to balance rare expressions, and offers 10-second refinement for identity-specific details.

Result: Achieves superior 3D consistency and detailed dynamic realism compared to previous methods. The model works with single or sparse images without camera poses or expression labels, produces real-time deformations, and handles extreme identities through lightweight refinement.

Conclusion: FlexAvatar provides a practical solution for animatable 3D avatar creation with flexible input requirements, high-fidelity reconstruction, and detailed dynamic deformation capabilities, advancing the state of the art in 3D head avatar generation.
...
Method: Curated 2,119 first-person videos from social media with frame-level annotations; proposed TTC computation framework using semantic segmentation and depth estimation; used GEV distribution for risk modeling; established VLLM benchmarks.

Result: Created comprehensive ADAS risk dataset; demonstrated utility through TTC analysis, extreme risk quantification across roadway types, and improved VLLM performance via domain adaptation.

Conclusion: SAVeD addresses the critical gap in ADAS safety research by providing real-world risk scenario data, enabling better analysis of perception/decision failures and enhancing model performance in complex scenarios.
...
TL;DR: MambaMIL+ is a new multiple instance learning framework that enhances spatial context modeling for whole-slide image analysis by integrating overlapping scanning, selective stripe position encoding, and contextual token selection mechanisms.


<details>
  <summary>Details</summary>
Motivation: Whole-slide images present challenges due to gigapixel resolution and lack of fine-grained annotations. While Mamba offers efficient long sequence learning, it suffers from limited spatial context modeling and memory decay in WSI analysis.

Method: MambaMIL+ introduces three key components: 1) overlapping scanning to embed spatial continuity, 2) selective stripe position encoder to mitigate scanning order biases, and 3) contextual token selection mechanism to dynamically enlarge contextual memory for stable long-range modeling.

Result: Extensive experiments on 20 benchmarks across diagnostic classification, molecular prediction, and survival analysis demonstrate state-of-the-art performance under three feature extractors (ResNet-50, PLIP, and CONCH).

Conclusion: MambaMIL+ effectively addresses spatial context limitations in WSI analysis while maintaining long-range dependency modeling without memory forgetting, proving robust for large-scale computational pathology applications.
...
### [118] [AdaptPrompt: Parameter-Efficient Adaptation of VLMs for Generalizable Deepfake Detection](https://arxiv.org/abs/2512.17730)
*Yichen Jiang,Mohammed Talha Alam,Sohail Ahmed Khan,Duc-Tien Dang-Nguyen,Fakhri Karray*

Main category: cs.CV

TL;DR: The paper proposes AdaptPrompt, a parameter-efficient CLIP-based framework for generalizable deepfake detection, using a new diffusion-generated dataset (Diff-Gen) and showing state-of-the-art performance across diverse synthetic media.


<details>
  <summary>Details</summary>
Motivation: As synthetic media becomes more realistic and diverse, existing deepfake detectors struggle with generalization across unseen generative models, creating a pressing need for more robust detection methods.

Method: 1) Created Diff-Gen dataset with 100k diffusion-generated fakes capturing broad spectral artifacts; 2) Proposed AdaptPrompt framework that jointly learns task-specific textual prompts and visual adapters while keeping CLIP frozen; 3) Used layer ablation to prune final transformer block for better retention of high-frequency artifacts.

Result: Achieved state-of-the-art performance across 25 challenging test sets covering GANs, diffusion models, and commercial tools; demonstrated strong cross-domain generalization, few-shot learning (with only 320 images), and source attribution capabilities.

Conclusion: The AdaptPrompt framework with Diff-Gen dataset provides a highly effective and parameter-efficient solution for generalizable deepfake detection, addressing the critical challenge of generalization across diverse and evolving synthetic media generation techniques.
...
### [119] [Pix2NPHM: Learning to Regress NPHM Reconstructions From a Single Image](https://arxiv.org/abs/2512.17773)
*Simon Giebenhain,Tobias Kirschstein,Liam Schoneveld,Davide Davoli,Zhe Chen,Matthias Nießner*

Main category: cs.CV

TL;DR: Pix2NPHM is a vision transformer network that directly regresses Neural Parametric Head Model parameters from single images, achieving high-fidelity 3D face reconstruction with unprecedented quality at interactive frame rates.


<details>
  <summary>Details</summary>
Motivation: Neural Parametric Head Models (NPHMs) offer better geometric detail than traditional 3DMMs, but fitting them to visual inputs is challenging due to their expressive latent space. The authors aim to create a method that can directly estimate NPHM parameters from single images for high-quality face reconstruction.

Method: Pix2NPHM uses a vision transformer (ViT) network with domain-specific backbones pretrained on geometric prediction tasks. It's trained on a mixture of 3D data (over 100K NPHM registrations for direct SDF space supervision) and large-scale 2D video datasets (using normal estimates as pseudo ground truth). The method also supports inference-time optimization against estimated surface normals and canonical point maps.

Result: The method achieves more recognizable facial geometry and accurate facial expressions compared to existing approaches. It enables 3D reconstructions at interactive frame rates and can be further improved through inference-time optimization, resulting in unprecedented face reconstruction quality that scales well on in-the-wild data.

Conclusion: Pix2NPHM successfully addresses the challenge of fitting expressive NPHMs to visual inputs, providing a practical solution for high-fidelity 3D face reconstruction from single images that combines the benefits of neural parametric models with efficient vision transformer-based parameter regression.
...
<details>
  <summary>Details</summary>
Motivation: Existing learning-based methods for geodesic distance computation rely on large 3D backbones, causing high memory usage and latency, limiting their use in interactive or resource-constrained settings.
...
Conclusion: LiteGE provides an efficient, lightweight solution for geodesic distance computation that enables fast and accurate shape matching while being robust to sparse inputs, making it suitable for interactive and resource-constrained applications.
...
### [121] [UrbanDIFF: A Denoising Diffusion Model for Spatial Gap Filling of Urban Land Surface Temperature Under Dense Cloud Cover](https://arxiv.org/abs/2512.17782)
...
Method: UrbanDIFF is a purely spatial denoising diffusion model for LST reconstruction. It's conditioned on static urban structure information (built-up surface data and digital elevation model) and enforces consistency with revealed cloud-free pixels through supervised pixel guided refinement during inference.

Result: UrbanDIFF outperforms interpolation baselines, especially under dense cloud occlusion (20-85% coverage). At 85% cloud coverage, it achieves SSIM of 0.89, RMSE of 1.2 K, and R2 of 0.84, with slower performance degradation as cloud density increases compared to baselines.

Conclusion: UrbanDIFF demonstrates that denoising diffusion models are effective for spatial LST reconstruction under high cloud coverage, offering improved robustness for continuous SUHI monitoring without requiring auxiliary temporal or multisensor data.
...
### [122] [Long-Range depth estimation using learning based Hybrid Distortion Model for CCTV cameras](https://arxiv.org/abs/2512.17784)
*Ami Pandat,Punna Rajasekhar,G. Aravamuthan,Gopika Vinod,Rohit Shukla*

Main category: cs.CV

TL;DR: A hybrid camera distortion modeling framework combining extended conventional models with neural network residual correction for long-range (up to 5km) 3D object localization in photogrammetry applications.


<details>
  <summary>Details</summary>
Motivation: Existing stereo-camera 3D localization methods are limited to few hundred meters due to inadequate distortion models for camera lens nonlinearities, especially for long-distance applications like CCTV-based photogrammetry.

Method: Hybrid approach: 1) Extend conventional distortion models with higher-order terms, 2) Enhance with neural network-based residual correction model to capture complex nonlinearities that pure neural networks fail to learn directly for camera parameter estimation.

Result: Framework achieves long-range localization up to 5 kilometers, substantially improving performance over existing methods. Estimated 3D coordinates are successfully transformed to GIS coordinates and visualized on maps.

Conclusion: The hybrid approach provides a robust and effective solution for calibrating CCTV cameras for long-range photogrammetry applications, overcoming limitations of both conventional distortion models and direct neural network applications.
...
### [123] [Animate Any Character in Any World](https://arxiv.org/abs/2512.17796)
...
<details>
  <summary>Details</summary>
Motivation: Existing world models are limited: static world generation lacks active agents, while controllable-entity models only allow limited actions for single entities. There's a need for models that combine realistic static environments with open-ended character control.

Method: AniX takes a 3DGS scene and character, then uses natural language to direct character actions. It's built on a pre-trained video generator with a training strategy that enhances motion dynamics while maintaining generalization across actions and characters, formulated as conditional autoregressive video generation.

Result: AniX synthesizes temporally coherent video clips that preserve visual fidelity with provided scenes and characters, supporting diverse behaviors from basic locomotion to object-centric interactions.

Conclusion: AniX successfully bridges the gap between static world generation and controllable-entity models, enabling realistic character animation in 3D environments with open-ended action capabilities through natural language control.
...
### [124] [Chorus: Multi-Teacher Pretraining for Holistic 3D Gaussian Scene Encoding](https://arxiv.org/abs/2512.17817)
...
<details>
  <summary>Details</summary>
Motivation: While 3DGS has become a high-fidelity scene representation, there's a gap in encoding rich, general-purpose features directly from its primitives. Current approaches lack comprehensive feature extraction from 3DGS representations.

Method: Chorus uses a multi-teacher pretraining framework with a shared 3D encoder and teacher-specific projectors to distill complementary signals from 2D foundation models (language-aligned, generalist, and object-aware teachers). It learns a shared embedding space capturing signals from high-level semantics to fine-grained structure.

Result: Chorus shows strong performance on open-vocabulary semantic/instance segmentation, linear/decoder probing, and data-efficient supervision. A point cloud variant outperforms baselines using 39.9x fewer training scenes. The framework also enables out-of-domain finetuning via render-and-distill adaptation.

Conclusion: Chorus successfully bridges the gap in feature extraction from 3DGS representations by leveraging multi-teacher distillation from 2D foundation models, creating a versatile encoder that transfers well across tasks and domains with strong data efficiency.
...
### [125] [ReX-MLE: The Autonomous Agent Benchmark for Medical Imaging Challenges](https://arxiv.org/abs/2512.17838)
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous coding agents built on LLMs fail on complex, domain-specific scientific problems like medical imaging, which require specialized knowledge, long training cycles, high-dimensional data handling, and complete end-to-end workflows not measured in existing benchmarks.

Method: Created ReX-MLE benchmark with 20 challenges derived from high-impact medical imaging competitions across diverse modalities and task types. Unlike prior benchmarks, it evaluates full end-to-end workflows requiring agents to independently manage data preprocessing, model training, and submission under realistic compute/time constraints. Evaluated state-of-the-art agents (AIDE, ML-Master, R&D-Agent) with different LLM backends (GPT-5, Gemini, Claude).

Result: Severe performance gap observed: most agent submissions rank in the 0th percentile compared to human experts. Failures stem from domain-knowledge limitations and engineering challenges in handling complex medical imaging workflows.

Conclusion: ReX-MLE exposes critical bottlenecks in current autonomous AI systems for domain-specific scientific problems and provides a foundation for developing domain-aware autonomous AI systems capable of handling complex real-world challenges.
...
### [126] [InfSplign: Inference-Time Spatial Alignment of Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.17851)
...
Conclusion: InfSplign is a lightweight, plug-and-play, training-free method compatible with any diffusion backbone that effectively improves spatial alignment in T2I generation.
...
### [127] [Simulation-Driven Deep Learning Framework for Raman Spectral Denoising Under Fluorescence-Dominant Conditions](https://arxiv.org/abs/2512.17852)
*Mengkun Chen,Sanidhya D. Tripathi,James W. Tunnell*

Main category: cs.CV

TL;DR: Physics-informed deep learning framework for denoising Raman spectra in biomedical applications by jointly suppressing detector noise and fluorescence background.
...
Method: Developed a simulation-driven denoising framework combining statistically grounded noise modeling with deep learning. Created biologically realistic Raman spectra using comprehensive noise models, then trained a cascaded deep neural network to jointly suppress stochastic detector noise and fluorescence baseline interference.
...
Conclusion: The approach shows potential for enabling faster and more accurate Raman-based tissue analysis by enhancing spectral quality through the integration of physical noise models with deep learning techniques.
...
### [128] [Interpretable Plant Leaf Disease Detection Using Attention-Enhanced CNN](https://arxiv.org/abs/2512.17864)
...
Method: CBAM-VGG16 integrates Convolution Block Attention Module (CBAM) at each convolutional stage of VGG16 architecture. The model is trained on five diverse plant disease datasets and evaluated using attention maps, Grad-CAM, Grad-CAM++, and Layer-wise Relevance Propagation for interpretability analysis.

Result: The approach outperforms recent techniques, achieving high accuracy up to 98.87% and demonstrating robust generalization across datasets. Comprehensive interpretability analysis shows effective disease localization.

Conclusion: The study advances explainable AI in agricultural diagnostics by offering a transparent and reliable system for smart farming. Code is publicly available for reproducibility.
...
### [129] [InSPECT: Invariant Spectral Features Preservation of Diffusion Models](https://arxiv.org/abs/2512.17873)
*Baohua Yan,Qingyuan Liu,Jennifer Kava,Xuan Di*

Main category: cs.CV

TL;DR: InSPECT is a novel diffusion model that preserves invariant spectral features during forward/backward processes, achieving better generation quality, diversity, and faster convergence compared to standard diffusion models.
...
Method: InSPECT keeps invariant spectral features during both forward and backward diffusion processes. Fourier coefficients smoothly converge to specified random noise at the end of forward process, enabling feature preservation while maintaining diversity and randomness.
...
### [130] [Visually Prompted Benchmarks Are Surprisingly Fragile](https://arxiv.org/abs/2512.17875)
*Haiwen Feng,Long Lian,Lisa Dunlap,Jiahao Shu,XuDong Wang,Renhao Wang,Trevor Darrell,Alane Suhr,Angjoo Kanazawa*

Main category: cs.CV

TL;DR: VLMs are surprisingly fragile to visual prompting details like marker color and size, which can dramatically change model rankings. The paper introduces VPBench with 16 marker variants to address this instability.


<details>
  <summary>Details</summary>
Motivation: Current VLM evaluation benchmarks using visual prompting are unstable - minor changes in visual markers (color, size) can completely alter model rankings, making evaluations unreliable and potentially misleading.

Method: Evaluated 9 open- and closed-source VLMs on two visually prompted tasks, systematically testing effects of visual marker design, dataset size, and low-level inference choices (JPEG compression). Created VPBench with 16 visual marker variants from curated datasets.

Result: Visual prompting details have huge impact: changing marker color from red to blue changes rankings; increasing marker size lifts InternVL3-8B to compete with Gemini 2.5 Pro; JPEG compression also affects rankings. These effects are much larger than in conventional semantic VLM evaluations.

Conclusion: Visual prompting benchmarks are fragile to implementation details, requiring more robust evaluation methods. VPBench provides a more stable benchmark with multiple marker variants to better assess VLM visual perception capabilities independent of textual priors.
...
### [131] [Keypoint Counting Classifiers: Turning Vision Transformers into Self-Explainable Models Without Training](https://arxiv.org/abs/2512.17891)
*Kristoffer Wickstrøm,Teresa Dorszewski,Siyan Chen,Michael Kampffmeyer,Elisabeth Wetzer,Robert Jenssen*

Main category: cs.CV

TL;DR: A method called Keypoint Counting Classifiers (KCCs) that transforms any pre-trained Vision Transformer (ViT) model into a self-explainable model without retraining, using ViT's ability to identify matching keypoints between images to create an interpretable visual decision process.


<details>
  <summary>Details</summary>
Motivation: Current self-explainable models require complex training procedures and specific architectures, making them impractical. With the rise of ViT-based foundation models, there's a need for methods to provide transparency and reliability to these powerful but opaque models without requiring retraining or architectural changes.

Method: KCCs leverage ViTs' ability to automatically identify matching keypoints between images with high precision. The method transforms any well-trained ViT-based model into a self-explainable classifier by creating an interpretable decision process based on keypoint matching that can be visualized directly in the input images.

Result: Extensive evaluation shows that KCCs improve human-machine communication compared to recent baselines. The method successfully provides transparency to ViT-based foundation models without requiring retraining or architectural modifications.

Conclusion: KCCs represent an important step toward making ViT-based foundation models more transparent and reliable by providing inherent visual explainability without the need for complex training procedures or specialized architectures.
...
### [132] [RadarGen: Automotive Radar Point Cloud Generation from Cameras](https://arxiv.org/abs/2512.17897)
...
Method: RadarGen adapts image-latent diffusion to radar domain by representing radar measurements in bird's-eye-view (BEV) form encoding spatial structure, RCS, and Doppler attributes. It incorporates BEV-aligned depth, semantic, and motion cues from pretrained foundation models to guide generation, with a lightweight recovery step to reconstruct point clouds from generated maps.
...
Conclusion: RadarGen represents a step toward unified generative simulation across sensing modalities, offering a scalable approach compatible with existing visual datasets and simulation frameworks for autonomous driving development.
...
### [133] [Diffusion Forcing for Multi-Agent Interaction Sequence Modeling](https://arxiv.org/abs/2512.17900)
*Vongani H. Maluleke,Kie Horiuchi,Lea Wilken,Evonne Ng,Jitendra Malik,Angjoo Kanazawa*

Main category: cs.CV

TL;DR: MAGNet is a unified diffusion framework for multi-agent motion generation that handles various interaction tasks through flexible conditioning, supporting dyadic prediction, partner inpainting, and full multi-agent generation with scalable architecture for variable group sizes.


<details>
  <summary>Details</summary>
Motivation: Multi-person interactions are fundamental for robotics and social computing but challenging to model due to long temporal horizons, strong inter-agent dependencies, and variable group sizes. Existing methods are task-specific and don't generalize to flexible multi-agent generation.

Method: MAGNet uses an autoregressive diffusion framework with Diffusion Forcing modifications that explicitly model inter-agent coupling during autoregressive denoising. It supports dyadic prediction, partner inpainting, and full multi-agent motion generation within a single model, with scalable architecture agnostic to agent count.

Result: The approach performs on par with specialized methods on dyadic benchmarks while naturally extending to polyadic scenarios with three or more people. It captures both tightly synchronized activities (dancing, boxing) and loosely structured social interactions, generating ultra-long sequences spanning hundreds of frames.

Conclusion: MAGNet provides a unified framework for multi-agent motion generation that addresses key challenges in modeling group interactions, offering flexible conditioning and scalable architecture for diverse interaction tasks across variable group sizes.
...
### [134] [Adversarial Robustness of Vision in Open Foundation Models](https://arxiv.org/abs/2512.17902)
*Jonathon Fox,William J Buchanan,Pavlos Papadopoulos*

Main category: cs.CV

TL;DR: This paper investigates adversarial robustness of vision-language models (LLaVA-1.5-13B and Llama 3.2 Vision-8B-2) against visual attacks using untargeted PGD on VQA v2 dataset, finding that Llama 3.2 Vision shows smaller accuracy drop despite lower baseline performance.


<details>
  <summary>Details</summary>
Motivation: As deep learning models become more complex and opaque, adversaries can exploit vulnerabilities by modifying images to confuse AI recognition systems. The paper aims to investigate the adversarial robustness of contemporary vision-language models against attacks on their visual input modality.

Method: The study tests LLaVA-1.5-13B and Meta's Llama 3.2 Vision-8B-2 using untargeted Projected Gradient Descent (PGD) attacks against the visual input modality. The models are empirically evaluated on a subset of the Visual Question Answering (VQA) v2 dataset, with results quantified using standard VQA accuracy metrics and compared for accuracy degradation.

Result: Llama 3.2 Vision, despite having lower baseline accuracy in this setup, exhibited a smaller drop in performance under attack compared to LLaVA, particularly at higher perturbation levels. The vision modality represents a viable attack vector for degrading performance of contemporary open-weight VLMs.

Conclusion: Adversarial robustness does not necessarily correlate directly with standard benchmark performance and may be influenced by underlying architectural and training factors. Vision modality is a vulnerable attack surface for vision-language models, and robustness evaluation should be considered alongside standard performance metrics.
...
### [135] [Dexterous World Models](https://arxiv.org/abs/2512.17907)
...
<details>
  <summary>Details</summary>
Motivation: Current digital twins are static and lack embodied interactivity, being limited to navigation and view synthesis. The paper aims to bridge this gap by creating interactive digital twins that can model how human actions dynamically change 3D scenes.

Method: DWM is a scene-action-conditioned video diffusion framework that generates temporally coherent videos from two inputs: (1) static scene renderings following specified camera trajectories for spatial consistency, and (2) egocentric hand mesh renderings encoding geometry and motion cues for action-conditioned dynamics. The model is trained on a hybrid dataset combining synthetic egocentric interactions (for aligned supervision) and fixed-camera real-world videos (for diverse object dynamics).

Result: Experiments show DWM enables realistic and physically plausible human-scene interactions like grasping, opening, and moving objects while maintaining camera and scene consistency. The framework successfully generates coherent videos of human-scene interactions from static scenes and hand motions.

Conclusion: DWM represents a first step toward video diffusion-based interactive digital twins and enables embodied simulation from egocentric actions, moving beyond static digital twins to create interactive environments that respond to human manipulation.
...
### [136] [Re-Depth Anything: Test-Time Depth Refinement via Self-Supervised Re-lighting](https://arxiv.org/abs/2512.17908)
*Ananta R. Bhattarai,Helge Rhodin*

Main category: cs.CV

TL;DR: Re-Depth Anything: A test-time self-supervision framework that improves monocular depth estimation by fusing Depth Anything V2 with 2D diffusion models using shape-from-shading cues and Score Distillation Sampling.


<details>
  <summary>Details</summary>
Motivation: Recent foundation models like Depth Anything V2 struggle with real-world images far from training distribution. There's a need to bridge this domain gap without requiring additional labeled data.

Method: Label-free refinement framework that fuses DA-V2 with 2D diffusion priors. Uses re-lighting of predicted depth maps and input augmentation with shape-from-shading cues in a generative context using Score Distillation Sampling. Employs targeted optimization: freezes encoder, updates intermediate embeddings, and fine-tunes decoder only.

Result: Substantial gains in depth accuracy and realism over DA-V2 across diverse benchmarks. Demonstrates effective bridging of domain gap without additional labeled data.

Conclusion: Re-Depth Anything showcases new avenues for self-supervision by augmenting geometric reasoning, enabling improved monocular depth estimation for real-world images through test-time refinement with diffusion model priors.
...
### [137] [Both Semantics and Reconstruction Matter: Making Representation Encoders Ready for Text-to-Image Generation and Editing](https://arxiv.org/abs/2512.17909)
...
<details>
  <summary>Details</summary>
Motivation: Current latent diffusion models use VAE latents optimized for pixel reconstruction, but there's a trend to use high-dimensional features from representation encoders for unified vision generation and understanding. However, discriminative feature spaces lack compact regularization (leading to inaccurate structures) and weak pixel reconstruction (hindering fine-grained geometry/texture learning).

Method: Introduces a semantic-pixel reconstruction objective to regularize latent space, compressing both semantic information and fine details into compact representation (96 channels, 16x16 downsampling). This creates semantically-rich latent space with state-of-the-art reconstruction. Uses this representation to design unified text-to-image generation and image editing model.

Result: Achieves state-of-the-art reconstruction, faster convergence, and substantial performance gains in both text-to-image generation and editing tasks compared to various feature spaces. Validates that representation encoders can be effectively adapted into robust generative components.

Conclusion: The proposed framework successfully adapts understanding-oriented encoder features for generative tasks by addressing fundamental obstacles through semantic-pixel reconstruction, enabling compact yet semantically-rich latent representations that unify vision generation and understanding.
...
### [138] [Colormap-Enhanced Vision Transformers for MRI-Based Multiclass (4-Class) Alzheimer's Disease Classification](https://arxiv.org/abs/2512.16964)
*Faisal Ahmed*
...
<details>
  <summary>Details</summary>
Motivation: MRI is crucial for Alzheimer's disease diagnosis, but subtle structural variations in brain scans challenge conventional deep learning models to extract discriminative features effectively.

Method: PseudoColorViT-Alz combines colormap transformations with Vision Transformers to amplify anatomical texture and contrast cues in MRI images, using pseudo-color representations to enhance feature learning.

Result: Achieves 99.79% accuracy and 100% AUC on OASIS-1 dataset for four-class classification (non-demented, moderate, mild, and very mild dementia), surpassing recent 2024-2025 methods (96.1%-99.68% accuracy).
...
### [139] [SkinGenBench: Generative Model and Preprocessing Effects for Synthetic Dermoscopic Augmentation in Melanoma Diagnosis](https://arxiv.org/abs/2512.17585)
*N. A. Adarsh Pritam,Jeba Shiney O,Sanyam Jain*

Main category: eess.IV

TL;DR: SkinGenBench evaluates how preprocessing complexity and generative model choice (StyleGAN2-ADA vs DDPMs) affect synthetic dermoscopic image quality and downstream melanoma diagnosis performance.
...
Method: Used curated dataset of 14,116 dermoscopic images from HAM10000 and MILK10K across five lesion classes. Evaluated two generative paradigms (StyleGAN2-ADA and DDPMs) under basic geometric augmentation and advanced artifact removal pipelines. Assessed synthetic images using perceptual/distributional metrics (FID, KID, IS), feature space analysis, and impact on diagnostic performance across five downstream classifiers.

Result: StyleGAN2-ADA consistently produced synthetic images more closely aligned with real data distributions (FID ~65.5, KID ~0.05), while diffusion models generated higher variance samples with reduced perceptual fidelity. Advanced artifact removal provided only marginal improvements. Synthetic data augmentation substantially improved melanoma detection with 8-15% absolute gains in melanoma F1-score, with ViT-B/16 achieving F1~0.88 and ROC-AUC~0.98 (14% improvement over non-augmented baselines).

Conclusion: Generative architecture choice has stronger influence on image fidelity and diagnostic utility than preprocessing complexity. StyleGAN2-ADA outperforms diffusion models for dermoscopic image synthesis, and synthetic data augmentation significantly enhances melanoma detection performance despite limited benefits from advanced preprocessing.
...
### [140] [Breast Cancer Neoadjuvant Chemotherapy Treatment Response Prediction Using Aligned Longitudinal MRI and Clinical Data](https://arxiv.org/abs/2512.17759)
*Rahul Ravi,Ruizhe Li,Tarek Abdelfatah,Stephen Chan,Xin Chen*

Main category: eess.IV

TL;DR: ML framework using longitudinal CE-MRI and clinical data predicts breast cancer treatment response (PCR) and 5-year relapse-free survival (RFS), with image registration-based feature extraction improving predictive performance.


<details>
  <summary>Details</summary>
Motivation: To develop machine learning models for predicting neoadjuvant chemotherapy response in breast cancer patients using longitudinal MRI data, addressing the need for accurate treatment response prediction to guide personalized therapy.

Method: Framework includes tumor segmentation, image registration, feature extraction (radiomics + 3 deep learning models: MedicalNet, Segformer3D, SAM-Med3D), feature selection, and ML modeling. Image registration enables comparison of features from original tumor site across time points to monitor intratumor changes during NACT.

Result: Image registration-based feature extraction consistently improved predictive models. For PCR classification: logistic regression with radiomic features achieved AUC 0.88, accuracy 0.85. For RFS classification: AUC 0.78, accuracy 0.72. Radiomics outperformed deep learning feature extractors.

Conclusion: Image registration significantly improves longitudinal feature learning for predicting PCR and RFS. Radiomics feature extractor is more effective than pre-trained deep learning models, offering higher performance and better interpretability for breast cancer treatment response prediction.
...
### [141] [MedNeXt-v2: Scaling 3D ConvNeXts for Large-Scale Supervised Representation Learning in Medical Image Segmentation](https://arxiv.org/abs/2512.17774)
...
<details>
  <summary>Details</summary>
Motivation: Existing large-scale pretraining efforts for 3D medical image segmentation focus mainly on increasing dataset size but overlook whether backbone networks are effective representation learners at scale. The authors identify that routinely used backbones in pretraining pipelines are often suboptimal.

Method: 1) Comprehensive backbone benchmarking to show stronger from-scratch performance predicts better downstream performance after pretraining; 2) Development of MedNeXt-v2 using 3D Global Response Normalization module and depth, width, and context scaling; 3) Supervised pretraining on 18k CT volumes; 4) Fine-tuning across six CT and MR benchmarks (144 structures).

Result: State-of-the-art performance across six challenging CT and MR benchmarks, showing consistent gains over seven publicly released pretrained models. Key findings: stronger backbones yield better results on similar data, representation scaling disproportionately benefits pathological segmentation, and modality-specific pretraining offers negligible benefit with full fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Despite advances in scientific AI, there's no coherent framework for Scientific General Intelligence (SGI) - the ability to autonomously conceive, investigate, and reason across scientific domains. Current AI systems lack a systematic approach to participate in the full scientific discovery process.

Method: 1) Define SGI using the Practical Inquiry Model (PIM: Deliberation, Conception, Action, Perception) with four scientist-aligned tasks. 2) Create SGI-Bench with over 1,000 expert-curated, cross-disciplinary samples from Science's 125 Big Questions. 3) Evaluate state-of-the-art LLMs on this benchmark. 4) Introduce Test-Time Reinforcement Learning (TTRL) to optimize retrieval-augmented novelty rewards at inference.
...
Conclusion: The PIM-grounded definition, workflow-centric benchmark, and empirical insights establish a foundation for AI systems that can genuinely participate in scientific discovery, moving beyond narrow AI applications to comprehensive scientific general intelligence.
...
TL;DR: PAACE is a plan-aware context compression framework for LLM agents that optimizes workflow contexts through relevance modeling, plan analysis, and function-preserving compression, improving accuracy while reducing context load.
...
Method: PAACE framework includes: (1) PAACE-Syn - synthetic generator of agent workflows with stepwise compression supervision, and (2) PAACE-FT - distilled plan-aware compressors trained from successful teacher demonstrations. Uses next-k-task relevance modeling, plan-structure analysis, instruction co-refinement, and function-preserving compression.

Result: On AppWorld: higher accuracy than baselines while lowering peak context and cumulative dependency. On OfficeBench and multi-hop QA: improves accuracy and F1, achieves fewer steps, lower peak tokens, reduced attention dependency. PAACE-FT retains 97% of teacher performance while reducing inference cost by over 10x.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with domain-specific language code generation like Answer Set Programming (ASP) due to limited examples in pre-training, despite ASP's effectiveness for combinatorial search problems.

Method: ASP-solver-in-the-loop approach: sample ASP statements from LLMs for logic puzzles, categorize into chosen/rejected based on solver feedback, apply supervised fine-tuning, and enhance with solver-guided search including best-of-N sampling.

Result: Experiments show consistent improvements in two distinct prompting settings on two datasets for ASP code generation.

Conclusion: Solver-guided instruction tuning effectively addresses semantic parsing challenges in ASP code generation, improving LLM performance on domain-specific languages.
...
### [145] [Large Language Models as Pokémon Battle Agents: Strategic Play and Content Generation](https://arxiv.org/abs/2512.17308)
...
<details>
  <summary>Details</summary>
Motivation: Pokémon battles provide a unique testbed for evaluating LLMs' strategic reasoning abilities, as they require understanding type matchups, statistical trade-offs, and risk assessment similar to human strategic thinking.
...
Result: Systematic evaluation across multiple model architectures showed LLMs can achieve competitive win rates, reasonable decision latency, type-alignment accuracy, and token efficiency, functioning as dynamic game opponents without domain-specific training.

Conclusion: LLMs can serve as both competent battle agents and content creators for turn-based strategic games, offering a practical alternative to reinforcement learning with implications for procedural generation and adaptive difficulty systems in interactive entertainment.
...
TL;DR: The paper proposes Laws of Reasoning (LoRe) framework to formalize desired reasoning behaviors in Large Reasoning Models, introduces LoRe-Bench to measure compliance, and shows that finetuning for better compliance improves reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models exhibit counterintuitive reasoning behaviors leading to suboptimal capabilities, necessitating a theoretical framework to formalize desired reasoning patterns.

Method: Proposes LoRe framework with compute law (linear scaling of compute with question complexity) and accuracy law. Introduces LoRe-Bench to measure monotonicity and compositionality properties. Develops finetuning approach to enforce compute-law compositionality.

Result: Most reasoning models show reasonable monotonicity but lack compositionality. Finetuning for better compliance with compute laws consistently improves reasoning performance across multiple benchmarks and reveals synergistic effects.

Conclusion: The LoRe framework successfully formalizes reasoning behaviors, and better compliance with the proposed laws leads to improved reasoning performance in Large Reasoning Models.
...
### [147] [Adaptive Covariance and Quaternion-Focused Hybrid Error-State EKF/UKF for Visual-Inertial Odometry](https://arxiv.org/abs/2512.17505)
*Ufuk Asil,Efendi Nasibov*

Main category: cs.RO

TL;DR: Hybrid VIO method for UAVs using Quaternion-focused Error-State EKF/UKF with dynamic sensor reliability assessment, achieving 49% better position accuracy in challenging scenarios with 48% lower computational cost.


<details>
  <summary>Details</summary>
Motivation: To develop a robust VIO system for UAVs that maintains accurate pose estimation in challenging environments while being computationally efficient, addressing the limitations of existing methods in handling varying sensor reliability and environmental conditions.

Method: Loosely coupled sensor fusion with hybrid Qf-ES-EKF/UKF architecture: ESKF propagates entire state, then SUKF refines only orientation. Dynamic sensor confidence scoring based on image entropy, intensity variation, motion blur, and inference quality adjusts measurement noise covariance.

Result: 49% average improvement in position accuracy in challenging scenarios, 57% average improvement in rotation accuracy over ESKF-based methods, and SUKF-comparable accuracy with 48% lower computational cost than full SUKF implementation.

Conclusion: The hybrid approach effectively balances computational efficiency and estimation accuracy, significantly enhancing UAV pose estimation performance in complex environments with varying sensor reliability.
...
### [148] [MAD-OOD: A Deep Learning Cluster-Driven Framework for an Out-of-Distribution Malware Detection and Classification](https://arxiv.org/abs/2512.17594)
*Tosin Ige,Christopher Kiekintveld,Aritran Piplai,Asif Rahman,Olukunle Kolade,Sasidhar Kunapuli*

Main category: cs.CR

TL;DR: MADOOD is a two-stage, cluster-driven deep learning framework for robust out-of-distribution (OOD) malware detection that uses Gaussian Discriminant Analysis to model malware family embeddings and identify anomalous samples without requiring OOD data during training.


<details>
  <summary>Details</summary>
Motivation: Current deep learning malware detectors fail to handle substantial intra-family variability in polymorphic/metamorphic malware and rely on closed-world assumptions, leading to degraded performance when encountering unseen malware families (OOD samples).

Method: Two-stage framework: 1) Uses Gaussian Discriminant Analysis to create class-conditional spherical decision boundaries for malware family embeddings, employing Z-score distance analysis across multiple centroids to identify OOD samples. 2) Deep neural network integrates cluster predictions, refined embeddings, and supervised classifier outputs for final classification.

Result: Significantly outperforms state-of-the-art OOD detection methods on benchmark datasets with 25 known families and multiple novel OOD variants, achieving up to 0.911 AUC on unseen malware families.

Conclusion: MADOOD provides a scalable, interpretable, and statistically principled solution for real-world malware detection and anomaly identification in evolving cybersecurity environments, effectively addressing the OOD detection challenge in malware classification.
...
### [149] [CIFE: Code Instruction-Following Evaluation](https://arxiv.org/abs/2512.17387)
*Sravani Gunnu,Shanmukha Guttula,Hima Patel*

Main category: cs.SE

TL;DR: The paper introduces a new benchmark for evaluating LLM code generation that measures both functional correctness AND adherence to developer constraints (robustness, formatting, security, etc.), revealing a significant gap between partial and strict constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus only on functional correctness through test-case execution, but real-world code generation requires adherence to explicit developer constraints for robustness, formatting, and security. There's a need to evaluate how reliably LLMs follow such constraints beyond just correctness.

Method: Created a benchmark of 1,000 Python tasks with developer-specified constraints (average 7 per task, 13 categories). Constraints were curated through a four-stage human-LLM pipeline to ensure atomicity, relevance, and objectivity. Evaluated 14 open- and closed-source models using complementary adherence metrics and proposed the C2A Score (Correctness and Constraint Adherence).

Result: Results show a substantial gap between partial and strict constraint satisfaction. While strong models achieve over 90% partial adherence, strict adherence remains between 39-66%. This highlights that current models struggle with consistently following developer constraints even when they produce functionally correct code.

Conclusion: Trustworthy code generation requires not only functional correctness but also consistent adherence to developer intent and constraints. The proposed benchmark and C2A score provide better evaluation of real-world code generation capabilities.
...
### [150] [SWE-Bench++: A Framework for the Scalable Generation of Software Engineering Benchmarks from Open-Source Repositories](https://arxiv.org/abs/2512.17419)
...
TL;DR: SWE-Bench++ is an automated framework that generates repository-level coding tasks from GitHub pull requests across 11 languages, addressing limitations of manual curation and Python-only focus in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks like SWE-bench have limitations: manual curation, static datasets, and focus only on Python bug fixes. There's a need for automated, multilingual, and comprehensive repository-level software engineering evaluation.

Method: Automated pipeline harvests live GitHub pull requests and converts them into reproducible tasks through four stages: programmatic sourcing, environment synthesis, test oracle extraction, and quality assurance. Includes hint-guided trajectory synthesis for training data generation.

Result: Created benchmark with 11,133 instances from 3,971 repositories across 11 languages. On 1,782-instance subset: Claude Sonnet 4.5 achieves 36.20% pass@10, GPT-5 34.57%, Gemini 2.5 Pro 24.92%, GPT-4o 16.89%. Fine-tuning on SWE-Bench++ improves performance on SWE-bench Multilingual benchmark.

Conclusion: SWE-Bench++ provides a scalable, multilingual benchmark for evaluating and improving repository-level code generation, addressing limitations of existing benchmarks and enabling better model training and evaluation.
...
### [151] [Understanding Generalization in Role-Playing Models via Information Theory](https://arxiv.org/abs/2512.17270)
*Yongqi Li,Hao Lang,Fei Huang,Tieyun Qian,Yongbin Li*

Main category: cs.LG

TL;DR: The paper introduces R-EMID, an information-theoretic metric to measure role-playing model performance degradation under distribution shifts, with theoretical bounds and a co-evolving RL framework for better generalization.


<details>
  <summary>Details</summary>
Motivation: Role-playing models underperform in real-world deployment due to distribution shifts (user, character, dialogue compositional shifts), but existing methods lack fine-grained diagnosis and formal frameworks to characterize RPM generalization behaviors.

Method: Proposes reasoning-based effective mutual information difference (R-EMID) metric to measure RPM degradation interpretably, derives theoretical upper bound for worst-case generalization, and introduces co-evolving reinforcement learning framework to adaptively model connections among user, character, and dialogue context for better response probability estimation.

Result: Evaluation shows user shift poses highest risk among all distribution shifts, and reinforcement learning is most effective approach for enhancing RPM generalization performance.

Conclusion: R-EMID provides interpretable measurement of RPM generalization degradation under distribution shifts, with theoretical guarantees and practical framework for improving real-world deployment performance.
...
### [152] [Task Schema and Binding: A Double Dissociation Study of In-Context Learning](https://arxiv.org/abs/2512.17325)
*Chaeha Kim*

Main category: cs.LG

TL;DR: ICL decomposes into two separable mechanisms: Task Schema (abstract task recognition) and Binding (specific associations), validated through causal experiments across multiple architectures.
...
Result: 1) Double dissociation showing separable mechanisms (Task Schema transfers at 100% via late MLP patching, Binding at 62% via residual stream), 2) Inverse correlation between Schema reliance and prior knowledge, 3) Mechanism operates across all architectures including non-Transformer Mamba.

Conclusion: ICL operates via dual-process mechanisms (Task Schema and Binding) that are neurally dissociable, not just behavioral modes. Models rely on Schema when prior knowledge is absent, with attentional mis-routing (not output competition) explaining performance patterns. This understanding enables better prompt engineering and system reliability.
...
### [153] [AdvJudge-Zero: Binary Decision Flips in LLM-as-a-Judge via Adversarial Control Tokens](https://arxiv.org/abs/2512.17375)
*Tung-Ling Li,Yuhao Wu,Hongliang Liu*

Main category: cs.LG

TL;DR: Reward models and LLM-as-a-Judge systems are vulnerable to low-perplexity control tokens that can flip binary evaluations from correct "No" to incorrect "Yes" judgments by manipulating logit gaps.


<details>
  <summary>Details</summary>
Motivation: Modern post-training pipelines (RLHF, DPO, RLAIF) rely on judge systems for feedback, but these systems have vulnerabilities that could lead to reward hacking during training.

Method: AdvJudge-Zero uses the model's next-token distribution and beam-search exploration to discover diverse control-token sequences from scratch that manipulate the judge's hidden states.

Result: Control tokens cause very high false positive rates when judge models score incorrect answers on math and reasoning benchmarks, with hidden-state perturbations concentrating in a low-rank "soft mode" anti-aligned with refusal direction.

Conclusion: LoRA-based adversarial training on small sets of control-token-augmented examples can significantly reduce false positives while preserving evaluation quality, addressing a realistic reward-hacking risk.
...
### [154] [Computational analysis reveals historical trajectory of East-Polynesian lunar calendars](https://arxiv.org/abs/2512.17525)
*Miguel Valério,Fabio Tamburini,Michele Corazza*

Main category: q-bio.PE

TL;DR: Computational analysis of 49 East Polynesian lunar calendars reveals two main groups that align with linguistic subgroups, suggesting calendars diverged alongside early population movements.


<details>
  <summary>Details</summary>
Motivation: To understand the historical relationships and divergence patterns of lunar calendars across East Polynesia, and to explore how these calendric systems relate to linguistic and population history in the region.

Method: Used computational methods to analyze lexical and structural divergence of 49 calendric lists from all major East Polynesian archipelagos, each containing about 30 night names. Results presented as a rooted phylogenetic tree.

Result: Found clear split into two main groups: 1) Rapa Nui, Mangareva, and Marquesas; 2) New Zealand, Hawaii, Cook Islands, Austral Islands, Tahiti, and Tuamotu. This pattern aligns with linguistic classification into 'Distal' and 'Proximal' subgroups.

Conclusion: The correspondence between lunar calendar divergence and linguistic subgroups provides evidence that early divergence of East Polynesian lunar calendars mirrors early population movements and language splits in the region, as both are symbolic systems passed down within communities.
