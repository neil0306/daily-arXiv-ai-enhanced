...
TL;DR: DeepResearch-Slice: A neuro-symbolic framework that uses explicit span prediction to filter retrieved evidence before reasoning, addressing the retrieval-utilization gap in noisy research environments.


<details>
  <summary>Details</summary>
Motivation: Current deep research agents focus on optimizing search policies but suffer from a "retrieval-utilization gap" - models fail to effectively use gold evidence even after retrieval due to context blindness in noisy environments.

Method: Propose DeepResearch-Slice, a neuro-symbolic framework that predicts precise span indices to perform deterministic hard filtering of retrieved evidence before reasoning, unlike implicit attention mechanisms.

Result: Extensive evaluations across six benchmarks show substantial robustness gains. Applying the method to frozen backbones yields 73% relative improvement (19.1% to 33.0%), effectively mitigating noise without requiring parameter updates to the reasoning model.

Conclusion: The results highlight the need for explicit grounding mechanisms in open-ended research, demonstrating that deterministic evidence filtering can significantly improve utilization of retrieved information in noisy contexts.
...
TL;DR: Internal reasoning (CoT) fails to eliminate sycophancy in LLMs, causing performance collapse in weak models and leaving gaps in frontier models, while external regulation (RCA) completely eliminates it, proving structural constraints are necessary for safety.


<details>
  <summary>Details</summary>
Motivation: To investigate whether sycophancy (LLMs prioritizing user agreeableness over correctness) can be mitigated by internal reasoning alone or requires external regulation, using adversarial datasets to test different mechanisms.

Method: Used CAP-GSM8K (N=500) adversarial dataset to evaluate internal reasoning (Chain-of-Thought) versus external regulation (RCA) mechanisms across GPT-3.5, GPT-4o, and GPT-5.1 models, analyzing performance patterns.

Result: Internal reasoning causes performance collapse in weak models (Prioritization Paradox) and leaves 11.4% final output gap in frontier models, while RCA structurally eliminates sycophancy (0.0%) across all model tiers.

Conclusion: External structural constraints are strictly necessary to guarantee safety, as internal reasoning alone cannot eliminate sycophancy, with hybrid systems achieving optimal efficiency only when capabilities are matched and strong.
...
TL;DR: Jailbreak-Zero is a new red teaming method that moves from example-based to policy-based LLM safety evaluation, using attack LLMs to generate diverse adversarial prompts and achieving Pareto optimality across coverage, diversity, and fidelity objectives.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety evaluation methods are constrained by example-based approaches that lack scalability and comprehensiveness. There's a need for a more expansive framework that can better identify safety vulnerabilities in LLMs with minimal human intervention.

Method: Uses an attack LLM to generate diverse adversarial prompts, then fine-tunes this attack model with a preference dataset. This policy-based framework achieves Pareto optimality across policy coverage, attack strategy diversity, and prompt fidelity to real user inputs.

Result: Achieves significantly higher attack success rates against both open-source and proprietary models (GPT-40 and Claude 3.5) compared to existing state-of-the-art techniques, while producing human-readable adversarial prompts.

Conclusion: Jailbreak-Zero presents a more scalable and comprehensive solution for identifying and mitigating LLM safety vulnerabilities, shifting the paradigm from constrained example-based approaches to effective policy-based evaluation.
...
TL;DR: On-device LLMs (gpt-oss-20b and gpt-oss-120b) achieve comparable performance to larger proprietary models in clinical tasks, with fine-tuning enabling near-GPT-5 level accuracy, demonstrating practical privacy-preserving clinical AI.


<details>
  <summary>Details</summary>
Motivation: Proprietary LLMs face privacy concerns and cloud dependency, while open-source models are too large for resource-constrained clinical settings. Need for on-device, privacy-preserving clinical AI solutions.

Method: Benchmarked two on-device LLMs (gpt-oss-20b and gpt-oss-120b) across three clinical tasks: general diagnosis, ophthalmology diagnosis/management, and expert grading simulation. Compared with proprietary models (GPT-5, o4-mini) and open-source DeepSeek-R1. Fine-tuned gpt-oss-20b on diagnostic data.

Result: gpt-oss models achieved performance comparable to or exceeding DeepSeek-R1 and o4-mini despite smaller size. Fine-tuning gpt-oss-20b significantly improved diagnostic accuracy, approaching GPT-5 performance.

Conclusion: On-device LLMs offer accurate, adaptable, privacy-preserving clinical decision support, enabling broader integration into routine practice without cloud dependency or privacy risks.
...
TL;DR: GPT-5 is a unified AI system with smart/fast and deep reasoning models, plus a real-time router that selects between them based on conversation needs. It features improved safety, reduced hallucinations, and enhanced performance for writing, coding, and health applications.


<details>
  <summary>Details</summary>
Motivation: To create a more useful and efficient AI system that combines speed for common queries with deep reasoning for complex problems, while improving safety, reducing hallucinations, and enhancing performance in key application areas like writing, coding, and health.

Method: A unified system architecture with: 1) gpt-5-main (smart/fast model for most questions), 2) gpt-5-thinking (deeper reasoning model for harder problems), 3) real-time router that selects models based on conversation type, complexity, tool needs, and explicit intent. The router is continuously trained on user signals. Includes safe-completions safety training and usage limits with fallback mini-models.

Result: Outperforms previous models on benchmarks with faster responses, more useful for real-world queries, significant advances in reducing hallucinations, improving instruction following, minimizing sycophancy, and enhanced performance in writing, coding, and health applications. gpt-5-thinking is classified as High capability in Biological/Chemical domains with associated safeguards.

Conclusion: GPT-5 represents a significant advancement in AI systems through its unified architecture with intelligent routing, improved safety measures, and enhanced performance across key application domains, while taking precautionary safety measures for high-capability models in sensitive domains.
...
TL;DR: SLMs perform poorly on reasoning tasks but excel at practical applications like tone modification, requiring specialized evaluation frameworks beyond standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation focuses on reasoning tasks, which unfairly disadvantages SLMs and fails to capture their practical effectiveness in real-world industrial applications like tone modification.

Method: Proposed evaluation framework with novel data generation, prompt-tuning, and LLM-based evaluation specifically designed for non-reasoning tasks where standard datasets don't exist.

Result: Demonstrated that SLMs can be highly effective for practical applications despite poor reasoning scores, showing the value of task-specific finetuning for edge/private computing scenarios.

Conclusion: Practitioners need specialized evaluation tools to properly benchmark both SLMs and LLMs for practical applications, especially in resource-constrained environments where SLMs excel.
...
TL;DR: LLMs show inconsistent adherence to custom instructions in enterprise settings, with Claude-Sonnet-4 and GPT-5 performing best in instruction compliance tests across 13 models.


<details>
  <summary>Details</summary>
Motivation: Despite LLMs' strong natural language capabilities, their deployment in enterprise environments reveals critical limitations in consistently following custom instructions, which is essential for reliable enterprise applications.

Method: Comprehensive evaluation of 13 leading LLMs using systematic testing with samples and enterprise-grade evaluation protocols, focusing on instruction compliance, response accuracy, and performance metrics in real-world RAG scenarios.

Result: Instruction following varies dramatically across models, with Claude-Sonnet-4 and GPT-5 achieving the highest results. The study identifies an "instruction gap" where models excel at general tasks but struggle with precise instruction adherence needed for enterprise deployment.

Conclusion: The research provides practical insights for organizations deploying LLM solutions and establishes benchmarks for instruction-following capabilities across major model families, highlighting the need for improved instruction adherence in enterprise applications.
...
TL;DR: Survey of Semantic Textual Similarity (STS) advancements since 2021, covering transformer models, contrastive learning, domain adaptation, multi-modal methods, graph-based approaches, and knowledge-enhanced techniques.


<details>
  <summary>Details</summary>
Motivation: To organize and analyze the rapid expansion of STS research since 2021, driven by advances in transformer architectures, contrastive learning, and domain-specific techniques, and to provide guidance for researchers and practitioners navigating these developments.

Method: Survey methodology reviewing progress across six key areas: transformer-based models (e.g., FarSSiBERT, DeBERTa-v3), contrastive learning methods (e.g., AspectCSE), domain-focused solutions (e.g., CXR-BERT, Financial-STS), multi-modal methods, graph-based approaches, and knowledge-enhanced techniques.

Result: Recent transformer models have achieved remarkable accuracy, contrastive methods have established new benchmarks, and domain-adapted models demonstrate effective customization for specialized fields. Multi-modal, graph-based, and knowledge-integrated models further enhance semantic understanding and representation.

Conclusion: The survey provides valuable insights into current methods, practical applications, and remaining challenges in STS, highlighting emerging trends and future opportunities to guide researchers and practitioners in the evolving field.
...
TL;DR: A history-free test set compression framework that reduces evaluation costs by over 90% without needing prior model performance data, using domain-adapted embeddings and task-aware clustering.


<details>
  <summary>Details</summary>
Motivation: Existing benchmark compression methods require correctness labels from multiple historical models, making them inapplicable in cold-start scenarios (new tasks, domains, or models with no prior evaluation history). Current benchmarks suffer from semantic redundancy and high computational costs.

Method: Fine-tune a base LLM on domain-specific data to internalize task semantics, generate semantic embeddings for test samples, perform task-aware clustering in domain-adapted embedding space, and use dataset X-ray mechanism to dynamically calibrate compression intensity based on cluster geometry and intrinsic redundancy.

Result: The approach effectively identifies and removes redundant samples, reducing evaluation cost by over 90% while preserving high fidelity to the full benchmark, demonstrated on professional-domain datasets including large-scale 3GPP communications benchmark.

Conclusion: Proposed history-free compression framework enables efficient benchmark evaluation in cold-start scenarios without requiring prior model performance data, addressing limitations of existing methods that depend on historical evaluation data.
...
TL;DR: GuardEval is a multi-perspective safety benchmark dataset with 106 fine-grained categories for training and evaluating content moderation systems. GemmaGuard (GGuard) is a fine-tuned model that achieves superior performance (0.832 F1) over existing moderation models.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based moderation systems struggle with nuanced cases like implicit offensiveness, subtle biases, and jailbreak prompts due to their subjective nature and heavy reliance on training data that can reinforce societal biases, leading to inconsistent and ethically problematic outputs.

Method: 1) Created GuardEval benchmark dataset with 106 fine-grained categories spanning human emotions, offensive/hateful language, gender/racial bias, and safety concerns. 2) Developed GemmaGuard (GGuard) by QLoRA fine-tuning Gemma3-12B on GuardEval for content moderation with fine-grained labels.

Result: GGuard achieves macro F1 score of 0.832, substantially outperforming OpenAI Moderator (0.64) and Llama Guard (0.61). The multi-perspective, human-centered approach reduces biased and inconsistent moderation decisions.

Conclusion: Diverse, representative data materially improves safety, fairness, and robustness on complex borderline cases. Multi-perspective safety benchmarks are critical for reducing biased moderation decisions in LLMs.
...
TL;DR: LLM_annotate is a Python package for analyzing fiction character personalities using LLMs, featuring standardized workflows for annotation, trait inference, and quality validation with human-in-the-loop GUI.


<details>
  <summary>Details</summary>
Motivation: To provide researchers with a standardized, efficient, and reproducible tool for analyzing character personalities in fiction texts using large language models, addressing the need for systematic character analysis workflows.

Method: The package implements functions for text chunking, LLM-based annotation, character name disambiguation, quality scoring, and computation of character-level statistics and embeddings. It supports any LLM (commercial, open-source, or custom) and includes a human-in-the-loop GUI for validation.

Result: The package enables efficient character analysis through tutorial examples using The Simpsons Movie and Pride and Prejudice, demonstrating practical usage for reproducible research.

Conclusion: LLM_annotate provides a comprehensive, flexible toolkit for researchers to conduct standardized character personality analysis using LLMs, with validation mechanisms to ensure quality and reproducibility.
...
Result: LLMs demonstrate greater effectiveness as segmenters compared to existing methods, showing improved performance.

Conclusion: While LLMs show promise for topic segmentation and outperform current approaches, significant reliability issues remain that need to be addressed before they can be fully trusted for this task.
...
TL;DR: A software-based "Virtual Tensor Core" architecture for ARM64 that bypasses memory bottlenecks to run LLMs efficiently on edge devices, achieving >60 tokens/sec on M2 hardware without proprietary accelerators.


<details>
  <summary>Details</summary>
Motivation: The "Memory Wall" bottleneck limits LLM deployment on edge devices where data movement latency exceeds arithmetic throughput. Standard inference runtimes have overhead from high-level abstractions, dynamic dispatch, and unaligned memory access patterns.

Method: 1) Virtual Tensor Core architecture implemented in software for ARM64 (Apple Silicon); 2) Bypasses standard library containers using direct memory mapping (mmap); 3) Hand-tuned NEON SIMD kernels; 4) Software-Defined Direct Memory Access (DMA); 5) Tensor Virtualization Layout (TVL) for 100% cache line utilization; 6) Zero-copy loader to eliminate initialization latency.

Result: Achieves stable throughput of >60 tokens/second on M2 hardware with a 110M parameter model. Meets the 200ms psycholinguistic latency threshold without opaque dependencies.

Conclusion: Provides a fully open, portable, and deterministic reference implementation for studying memory bottlenecks on general-purpose ARM silicon. While proprietary hardware accelerators (like Apple AMX) offer higher peak throughput, this approach offers transparency and portability for edge deployment.
...
TL;DR: BPE tokenization transforms corpus statistics toward Zipfian power laws and reduces token dependencies, making language distributions more IID-like and aligning model predictions with Zipf-derived entropy expectations.


<details>
  <summary>Details</summary>
Motivation: Despite natural languages exhibiting statistical regularities like Zipf's and Heaps' laws, it remains unclear how these properties relate to modern tokenization schemes used in transformer models. The paper aims to understand how byte-pair encoding (BPE) transforms corpus statistics and affects language modeling.

Method: 1) Derive closed-form expression for slot entropy expectation under Zipfian distribution; 2) Empirically investigate how BPE transforms corpus statistics by analyzing token frequencies and entropy patterns; 3) Train language models on corpora tokenized at varying BPE depths and analyze predictive entropies; 4) Use attention-based diagnostics to examine token dependencies.

Result: Recursive BPE applications drive token frequencies toward Zipfian power laws and induce characteristic entropy growth. As BPE depth increases, model predictive entropies increasingly agree with Zipf-derived predictions. Attention diagnostics show deeper tokenization reduces local token dependencies, bringing distributions closer to weakly dependent (near IID) regimes.

Conclusion: BPE acts not only as a compression mechanism but also as a statistical transform that reconstructs key informational properties of natural language, making distributions more Zipfian and reducing dependencies toward IID-like behavior.
...
TL;DR: Metaphors in training data causally influence LLM misalignment across domains, and monitoring latent features enables detection of misaligned content.


<details>
  <summary>Details</summary>
Motivation: Since metaphors influence human decision-making and LLMs are trained on data containing many metaphors, researchers investigate whether metaphors affect LLMs' reasoning pathways, particularly in the context of emergent misalignment where patterns learned from misaligned content generalize across domains.

Method: Researchers examine the causal relationship between metaphors in training data and LLM misalignment, conducting interventions using metaphors during pre-training, fine-tuning, and re-alignment phases. They analyze connections between metaphors and activation of global/local latent features in reasoning models, then design a detector based on monitoring these latent features.

Result: Strong causal relationship discovered between metaphors and misalignment degree; interventions significantly change cross-domain misalignment. Connection found between metaphors and latent feature activation, enabling design of a detector that predicts misaligned content with high accuracy.

Conclusion: Metaphors in training data significantly influence LLM misalignment across domains, and monitoring latent features provides an effective approach for detecting misaligned content in large language models.
...
Method: Introduces PersonaWeaver framework that disentangles world-building (roles, demographics) from behavioral-building (moral stances, interactional styles). This separation allows for more diverse character reactions, moral stances, and second-order diversity in stylistic markers like length, tone, and punctuation.

Result: The framework yields characters with more diverse reactions and moral stances, as well as second-order diversity in stylistic markers like length, tone, and punctuation.

Conclusion: PersonaWeaver addresses limitations in current character generation by creating more dramatically interesting and diverse characters through disentangled world-building and behavioral-building components.
...
TL;DR: Disclaimer Injection makes text unlearnable to LLMs by injecting alignment-triggering disclaimers that cause persistent activation of alignment layers during fine-tuning, preventing effective learning.


<details>
  <summary>Details</summary>
Motivation: Address concerns about unauthorized use of proprietary or personal data during LLM training by developing a data-level defense that works in realistic black-box settings without requiring model-side controls or data removal.

Method: Inject carefully designed alignment-triggering disclaimers into text data. These disclaimers exploit LLMs' alignment mechanisms to prevent effective learning during fine-tuning, causing alignment constraints to override task learning.

Result: Models trained on protected data show substantial and systematic performance degradation compared to standard fine-tuning. Layer-wise analysis reveals persistent activation of alignment-related layers that override task learning.

Conclusion: Alignment behavior serves as an unexplored lever for data protection, and Disclaimer Injection presents the first practical method for restricting data learnability at LLM scale without requiring access to or modification of the training pipeline.
...
TL;DR: Systematic formalization of Tigrinya cardinal and ordinal number verbalization with algorithm and open-source implementation, revealing LLM gaps.


<details>
  <summary>Details</summary>
Motivation: Address computational resource gap for Tigrinya language, particularly in number verbalization for language modeling, speech synthesis, and accessibility applications.

Method: Document canonical rules for numerical expression (conjunction system, scale words, special cases), develop formal algorithm for number-to-word conversion, create open-source implementation, and evaluate frontier LLMs.

Result: Significant gaps found in LLMs' ability to accurately verbalize Tigrinya numbers, highlighting need for explicit rule documentation. Open-source implementation released.

Conclusion: This work fills resource gap for Tigrinya computational linguistics, serves practical applications, and demonstrates limitations of current LLMs for low-resource languages.
...
TL;DR: LatentGraphMem combines implicit graph memory with explicit subgraph retrieval to handle long-context queries efficiently while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: LLMs need to answer queries with sparse evidence across long contexts, but existing memory systems have trade-offs: explicit structured memories are interpretable but brittle under long-context overload, while latent memories are efficient but difficult to inspect.

Method: LatentGraphMem stores graph-structured memory in latent space for stability/efficiency, exposes task-specific subgraph retrieval interface that returns compact symbolic subgraph under fixed budget. During training, explicit graph view materializes to interface with frozen reasoner for QA supervision. At inference, retrieval in latent space with only retrieved subgraph externalized.

Result: Experiments on long-horizon benchmarks across multiple model scales show LatentGraphMem consistently outperforms explicit-graph and latent-memory baselines, enables parameter-efficient adaptation and flexible scaling to larger reasoners without large symbolic artifacts.

Conclusion: LatentGraphMem effectively bridges the gap between interpretability and efficiency in long-context memory systems, offering stable performance while maintaining inspectability through selective subgraph retrieval.
...
<details>
  <summary>Details</summary>
Motivation: Verifying system-generated summaries requires precise attribution to source context, which is especially crucial in high-stakes medical domains where accurate attribution is essential for reliability and trustworthiness.

Method: Introduces PCoA benchmark with expert annotations aligning aspect-based summaries with supporting contextual sentences and contributory phrases. Proposes a fine-grained, decoupled evaluation framework that independently assesses summary quality, citations, and contributory phrases.

Result: PCoA provides a reliable benchmark for evaluating system-generated summaries with phrase-level context attribution. Experiments show that explicitly identifying relevant sentences and contributory phrases before summarization improves overall quality.

Conclusion: PCoA addresses the challenge of verifying system-generated medical summaries through precise phrase-level attribution, offering a valuable benchmark and demonstrating that explicit identification of supporting content improves summarization quality.
...
TL;DR: CAPT enables training-free adaptation of new general-domain LLMs using existing clinical models via contrastive decoding, outperforming individual models and state-of-the-art ensembling methods.


<details>
  <summary>Details</summary>
Motivation: Adapting language models to clinical domains requires costly retraining for each new model generation, creating a need for efficient adaptation methods that leverage existing clinical models without retraining.

Method: Cross-Architecture Proxy Tuning (CAPT) uses model ensembling with contrastive decoding to selectively inject clinically relevant signals from existing clinical models into new general-domain models, supporting models with disjoint vocabularies without requiring training.

Result: On six clinical classification and text-generation tasks, CAPT consistently outperforms both individual models and state-of-the-art ensembling approaches (+17.6% over UniTE, +41.4% over proxy tuning), improves clinically actionable language, reduces context errors, and increases clinical specificity.

Conclusion: CAPT provides an effective training-free approach for adapting new general-domain models to clinical domains using existing clinical models, offering significant performance improvements while preserving general-domain reasoning and fluency.
...
TL;DR: ASPECTSIM is a framework for aspect-conditioned document similarity that outperforms traditional holistic similarity, with GPT-4o achieving 80% higher human agreement than holistic approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional document similarity measures use a holistic approach that doesn't account for specific aspects, limiting their interpretability and alignment with human judgment.

Method: ASPECTSIM conditions document similarity on explicitly specified aspects, using GPT-4o prompting and a two-stage refinement approach for smaller LLMs, evaluated on a new benchmark of 26K aspect-document pairs.

Result: GPT-4o achieves ≈80% higher human-machine agreement than holistic similarity. Smaller open-source LLMs initially show only 20-30% agreement, but two-stage refinement improves this by ≈140%, though still lagging behind GPT-4o.

Conclusion: Explicitly accounting for aspects is crucial for document similarity measurement, requiring revision of standard practice. While refinement helps smaller models, large proprietary models like GPT-4o still significantly outperform open-source alternatives in aspect-conditioned similarity.
...
<details>
  <summary>Details</summary>
Motivation: While LLMs are increasingly used as automated evaluators, their consistency across different grading scales remains underexplored, particularly how scale choice affects human-LLM agreement.

Method: Collected ratings from both humans and LLMs across three different grading scales on six benchmarks including objective, subjective, and mixed tasks. Used intraclass correlation coefficients (ICC) to measure absolute agreement between groups.

Result: LLM judgments are inconsistent across scales on subjective benchmarks. Scale choice substantially shifts human-LLM agreement even with high within-group reliability. The 0-5 scale yields strongest overall human-LLM alignment. Pooled reliability masks benchmark heterogeneity and reveals systematic subgroup differences across gender groups.

Conclusion: Scale design and sub-level diagnostics are essential components of LLM-as-a-judge protocols, as grading scale choice significantly impacts evaluation consistency and human-LLM alignment, with important implications for fair and reliable automated assessment.
...
TL;DR: L2T is a pre-training framework that combines next-token prediction with explicit language learning tasks to improve linguistic competence while maintaining reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Standard LM pre-training on raw text doesn't explicitly optimize for linguistic competence, focusing mainly on world knowledge and reasoning. There's a gap between current LM capabilities and explicit linguistic skills.

Method: L2T transforms raw text into structured input-output pairs to provide explicit linguistic stimulation, inspired by human language acquisition. It integrates Language Learning Tasks alongside standard next-token prediction during pre-training.
...
Conclusion: Explicit linguistic stimulation through structured language learning tasks during pre-training enhances LM linguistic competence without compromising general reasoning abilities, bridging a key gap in current LM training approaches.
...
<details>
  <summary>Details</summary>
Motivation: Previous prompt-based evaluations suggested LLMs perform poorly on time series classification, raising doubts about whether they encode meaningful temporal structure. The authors want to determine if this reflects actual limitations or just evaluation methodology issues.

Method: Direct comparison of prompt-based generation vs linear probes over internal representations. Used zero-shot prompting and linear probes on the same LLM representations. Conducted layer-wise analyses to track emergence of time series information.

Result: Zero-shot prompting performed near chance (F1 0.15-0.26), but linear probes dramatically improved performance to F1 0.61-0.67, often matching or exceeding specialized time series models. Class-discriminative time series information emerges in early transformer layers and is amplified by visual/multimodal inputs.

Conclusion: There's a systematic mismatch between what LLMs internally represent and what prompt-based evaluation reveals. Current evaluations underestimate LLMs' time series understanding due to limitations in prompt-based generation rather than actual representational capacity.
...
Method: Created EpiQAL benchmark with three subsets from open-access literature: 1) text-grounded factual recall, 2) multi-step inference linking evidence with epidemiological principles, 3) conclusion reconstruction with Discussion section withheld. Used expert taxonomy guidance, multi-model verification, and retrieval-based difficulty control.

Result: Experiments on ten open models show limited performance on epidemiological reasoning, especially on multi-step inference. Model rankings shift across subsets, scale doesn't predict success, and Chain-of-Thought helps multi-step inference but has mixed results elsewhere.

Conclusion: EpiQAL provides fine-grained diagnostic signals for evidence grounding, inferential reasoning, and conclusion reconstruction, revealing current LLMs' limitations in epidemiological reasoning and offering a specialized benchmark for improvement.
...
TL;DR: SegNSP frames linear text segmentation as a next sentence prediction task, using label-agnostic NSP with segmentation-aware loss and harder negative sampling to detect topic boundaries without explicit topic labels.


<details>
  <summary>Details</summary>
Motivation: Linear text segmentation remains challenging due to difficulty defining topic boundaries, discourse variability, and balancing local coherence with global context, hindering downstream NLP applications like summarization and information retrieval.

Method: Frames segmentation as next sentence prediction (NSP) task. Uses label-agnostic NSP approach predicting whether next sentence continues current topic without explicit topic labels. Enhances with segmentation-aware loss combined with harder negative sampling to better capture discourse continuity.

Result: Achieves B-F₁ of 0.79 on CitiLink-Minutes (first segmentation benchmark) and 0.65 on WikiSection, outperforming strongest reproducible baseline TopSeg by 0.17 absolute points. Results demonstrate competitive and robust performance.

Conclusion: SegNSP effectively models sentence-to-sentence continuity for improving segmentation quality, supporting downstream NLP applications. Shows NSP's natural fit for detecting topic boundaries despite being largely abandoned in modern pre-training.
...
<details>
  <summary>Details</summary>
Motivation: Current hate speech detection models rely too much on surface-level lexical features, making them vulnerable to spurious correlations and limiting their robustness, cultural contextualization, and interpretability.

Method: Proposes Supervised Moral Rationale Attention (SMRA) that aligns token-level attention with expert-annotated moral rationales based on Moral Foundations Theory. Introduces HateBRMoralXplain dataset for Brazilian Portuguese with hate labels, moral categories, token-level rationales, and socio-political metadata.

Result: SMRA improves binary hate speech detection (+0.9 F1) and multi-label moral sentiment classification (+1.5 F1). Enhances explanation faithfulness with IoU F1 (+7.4 pp) and Token F1 (+5.0 pp). Explanations become more concise with improved sufficiency (+2.3 pp) and stable fairness.

Conclusion: SMRA successfully integrates moral rationale supervision directly into training, producing inherently interpretable and contextualized explanations without performance or bias trade-offs, representing a significant advancement in explainable hate speech detection.
...
TL;DR: CALM is a framework that gives language models cultural self-awareness by disentangling cultural concepts, clustering them, and using adaptive integration with reflective learning for continual cultural adaptation.


<details>
  <summary>Details</summary>
Motivation: Most current approaches treat culture as static background knowledge, overlooking its dynamic and evolving nature, which reduces reliability in tasks requiring genuine cultural sensitivity.

Method: CALM disentangles task semantics from explicit cultural concepts and latent cultural signals, shapes them into structured clusters via contrastive learning, aligns them through cross-attention, and adaptively integrates them using Mixture-of-Experts. It creates a culturally grounded internal identity state enhanced through self-prompted reflective learning.

Result: Extensive experiments on multiple cross-cultural benchmark datasets show that CALM consistently outperforms state-of-the-art methods.

Conclusion: CALM successfully endows language models with cultural self-awareness through a novel framework that handles culture as dynamic and evolving, enabling continual adaptation and self-correction for improved cultural sensitivity.
...
TL;DR: SESS: Submodular evaluation subset selection method for prompt optimization that outperforms random/heuristic baselines across math reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current prompt optimization relies on task performance measured on small, often randomly sampled evaluation subsets, but selection of these subsets is treated as an implementation detail rather than a principled problem.

Method: Proposes SESS (Submodular Evaluation Subset Selection), framing selection as maximizing an objective set function, showing it's monotone and submodular under mild conditions, enabling greedy selection with theoretical guarantees.
...
<details>
  <summary>Details</summary>
Motivation: Current SFT monitoring using validation perplexity is insufficient because it conflates stylistic mimicry with genuine factual internalization, making it hard to assess whether models are actually learning domain knowledge.

Method: KR-Test uses automatically generated contrastive examples to measure likelihood preferences for correct vs. incorrect continuations, requiring no instruction tuning or generative decoding. It includes "blind vs. oracle" baseline analysis for validation.

Result: The framework successfully distinguishes factual learning from linguistic convergence and reveals fine-grained dissociation between these aspects during LoRA training dynamics.

Conclusion: KR-Test enhances interpretability of fine-tuning dynamics by exposing the dissociation between linguistic convergence and knowledge retention, providing better monitoring for SFT training.
...
TL;DR: RPAM is a model merging framework that combines Long-CoT and Short-CoT models to create adaptive reasoning without retraining, reducing inference cost while maintaining performance.
...
Method: RPAM uses layer-wise model merging with feature alignment. It creates a pattern-labeled calibration set to assign appropriate reasoning patterns to queries, then optimizes merging coefficients by aligning intermediate representations with the selected model while pushing away from non-selected models.
...
Conclusion: Model merging provides a lightweight alternative for efficient reasoning without training from scratch, and RPAM demonstrates effective adaptive reasoning through pattern-aligned merging.
...
TL;DR: IntroLM enables LLMs to predict their own output quality during prefilling using introspective tokens and token-conditional LoRA, outperforming external classifiers and improving routing efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for predicting LLM output quality rely on external classifiers (like BERT-based models) which have limitations: limited context windows, constrained representational capacity, and additional computational overhead.

Method: IntroLM introduces introspective tokens that enable causal language models to predict their own output quality during the prefilling phase without affecting generation. It uses token-conditional LoRA that activates only for the introspective token, allowing the model to learn output quality prediction while preserving original backbone behavior.

Result: On question answering benchmarks, IntroLM applied to Qwen3 8B achieves 90% ROC AUC for success prediction, outperforming DeBERTa classifier by 14%. When integrated into multi-model routing systems, it reduces latency by up to 33% and large model usage by up to 50% at matched reliability.

Conclusion: IntroLM provides an effective self-assessment mechanism for LLMs that eliminates the need for external evaluators, improves prediction accuracy, and enables more efficient model routing systems with better cost-performance tradeoffs.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks either evaluate multi-session memory in text-only conversations or assess multimodal understanding in localized contexts, failing to evaluate how multimodal memory is preserved, organized, and evolved across long-term conversational trajectories.

Method: Introduces Mem-Gallery benchmark featuring high-quality multi-session conversations grounded in both visual and textual information with long interaction horizons and rich multimodal dependencies. Proposes a systematic evaluation framework assessing key memory capabilities along three dimensions: memory extraction and test-time adaptation, memory reasoning, and memory knowledge management.

Result: Extensive benchmarking across thirteen memory systems reveals key findings: necessity of explicit multimodal information retention and memory organization, persistent limitations in memory reasoning and knowledge management, and efficiency bottlenecks of current models.

Conclusion: Mem-Gallery provides a comprehensive benchmark for evaluating multimodal long-term conversational memory in MLLM agents, highlighting critical gaps in current memory systems and establishing a foundation for future research in this important area.
...
TL;DR: The paper introduces Personalized Audio-Language Models (PALM) to address the limitation of current LALMs in handling personalized contexts, creates PALM-Bench benchmark for evaluation, and shows existing methods are insufficient for robust personalized reasoning.


<details>
  <summary>Details</summary>
Motivation: Current Large Audio-Language Models perform well on generic audio tasks but fail at personalized question answering that requires understanding individual contexts and personal concepts, unlike human reasoning which incorporates personal context.

Method: Formalizes the Personalized LALMs (PALM) task, creates PALM-Bench benchmark for structured evaluation across multi-speaker scenarios, and evaluates existing training-free prompting and supervised fine-tuning strategies.

Result: Existing methods show improvements but remain limited in modeling personalized knowledge and transferring it robustly across tasks, indicating the need for better approaches.

Conclusion: There is a significant gap in personalized audio understanding that current LALMs cannot adequately address, necessitating new methods for personalized reasoning and the release of PALM-Bench to advance research in this area.
...
TL;DR: A persona-aware Vision-Language Model framework for bikeability assessment that uses cyclist typology personas, multi-granularity fine-tuning, and AI data augmentation to provide explainable bikeability ratings.


<details>
  <summary>Details</summary>
Motivation: Existing bikeability assessment approaches fail to capture road environment complexity and subjective user perception heterogeneity. Current methods don't adequately incorporate cyclists' safety and comfort perceptions.

Method: Three novel components: 1) Theory-grounded persona conditioning using cyclist typology with chain-of-thought reasoning; 2) Multi-granularity supervised fine-tuning combining expert reasoning with user ratings; 3) AI-enabled data augmentation creating controlled paired data to isolate infrastructure impacts.

Result: Framework tested with 12,400 persona-conditioned assessments from 427 cyclists using panoramic image-based crowdsourcing. Offers competitive bikeability rating prediction while enabling explainable factor attribution.

Conclusion: The proposed persona-aware VLM framework addresses limitations of existing bikeability assessment by capturing subjective perceptions and providing explainable assessments through persona conditioning and multi-granularity learning.
...
TL;DR: DeepSynth-Eval is a benchmark for evaluating LLM agents' ability to synthesize information from massive contexts into coherent long-form reports, addressing the under-evaluated post-retrieval synthesis stage in deep research.


<details>
  <summary>Details</summary>
Motivation: The post-retrieval synthesis stage in deep research—where agents must digest massive context and consolidate fragmented evidence into coherent reports—remains under-evaluated due to the subjectivity of open-ended writing. Current benchmarks focus on retrieval but neglect synthesis capabilities.

Method: 1) Use high-quality survey papers as gold standards, reverse-engineering research requests and constructing "Oracle Contexts" from their bibliographies to isolate synthesis from retrieval noise. 2) Propose fine-grained evaluation using General Checklists (factual coverage) and Constraint Checklists (structural organization), transforming subjective judgment into verifiable metrics.

Result: Experiments across 96 tasks show that synthesizing information from hundreds of references remains a significant challenge. Agentic plan-and-write workflows significantly outperform single-turn generation, effectively reducing hallucinations and improving adherence to complex structural constraints.

Conclusion: DeepSynth-Eval provides an objective benchmark for evaluating information consolidation capabilities in LLM agents, revealing that synthesis from massive contexts is challenging but can be improved through agentic workflows that separate planning from writing.
...
TL;DR: LLMs don't always compute multi-hop reasoning sequentially; later-hop answers can emerge before bridge entities, challenging the hop-aligned circuit hypothesis.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs internally compose multiple facts during multi-hop reasoning, as current hop-aligned circuit hypothesis may not fully capture their reasoning mechanisms.
...
Result: Discovered layer-order inversion phenomenon where later-hop answer entities become decodable earlier than bridge entities, which strengthens with total hops. The framework is validated through probing analyses and explains chain-of-thought gains.

Conclusion: Multi-hop reasoning in LLMs follows a probabilistic recall-and-extract process rather than strictly sequential hop-aligned computation, providing mechanistic explanations for reasoning successes and failures.
...
TL;DR: EvolMem is a new benchmark for evaluating LLMs' multi-session memory capabilities across diverse cognitive dimensions, revealing that no LLM consistently excels and agent memory mechanisms often have efficiency limitations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack systematic evaluation of LLMs across diverse memory dimensions, particularly in multi-session settings, creating a need for comprehensive assessment tools grounded in cognitive psychology.

Method: Introduces a hybrid data synthesis framework with topic-initiated generation and narrative-inspired transformations to create multi-session conversations with controllable complexity and sample-specific evaluation guidelines.

Result: Extensive evaluation shows no LLM consistently outperforms others across all memory dimensions, and agent memory mechanisms don't necessarily enhance LLM capabilities while often exhibiting notable efficiency limitations.

Conclusion: EvolMem provides a comprehensive benchmark for assessing multi-session memory capabilities, revealing current limitations in LLM memory performance and the need for improved memory mechanisms.
...
TL;DR: This paper introduces a new assessment protocol to evaluate how well LLMs' expressed privacy and prosocial values align with their actual data-sharing decisions, using structural equation modeling and a new metric called VAAR.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of LLMs in decision-making tasks involving personal data sharing measure privacy attitudes or sharing intentions in isolation, which fails to capture whether models' expressed values actually predict their downstream actions as in human behavior.

Method: The authors introduce a context-based assessment protocol that sequentially administers standardized questionnaires for privacy attitudes, prosocialness, and acceptance of data sharing within a bounded session. They use multi-group structural equation modeling (MGSEM) to identify relationships from privacy concerns and prosocialness to data sharing, and propose Value-Action Alignment Rate (VAAR) as a human-referenced directional agreement metric.

Result: Across multiple LLMs, the study found stable but model-specific Privacy-PSA-AoDS profiles, and substantial heterogeneity in value-action alignment between different models.

Conclusion: The proposed assessment framework reveals important differences in how well LLMs' expressed values align with their actual data-sharing decisions, highlighting the need for more comprehensive evaluation of value-action consistency in AI systems.
...
TL;DR: Proposes PAS framework to evaluate LLMs for police operations, revealing commercial LLMs struggle with police-specific tasks, especially fact-based recommendations.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used in police operations but lack tailored evaluation frameworks, risking severe consequences like unlawful arrests and improper evidence collection from unverified LLM responses.

Method: Developed PAS (Police Action Scenarios) framework covering entire evaluation process, constructed QA dataset from 8,000+ official documents, established metrics validated through statistical analysis with police expert judgements.

Result: Commercial LLMs struggle with police-related tasks, particularly in providing fact-based recommendations, highlighting performance gaps in real-world police applications.

Conclusion: Necessitates expandable evaluation frameworks like PAS to ensure reliable AI-driven police operations; releases data and prompt templates for further research.
...
TL;DR: DiffCoT: A diffusion-styled Chain-of-Thought framework that treats reasoning as iterative denoising, enabling correction of intermediate steps while maintaining token-level autoregression.


<details>
  <summary>Details</summary>
Motivation: Standard CoT reasoning suffers from exposure bias and error accumulation where early mistakes propagate irreversibly through autoregressive decoding, limiting robustness and accuracy in multi-step mathematical problem solving.

Method: Reformulates CoT reasoning as iterative denoising process using diffusion principles at reasoning-step level with sliding-window mechanism. Introduces causal diffusion noise schedule to preserve temporal structure of reasoning chains while enabling unified generation and retrospective correction.
...
Conclusion: DiffCoT successfully addresses limitations of standard CoT reasoning by integrating diffusion principles, providing a more robust framework for multi-step mathematical problem solving with better error correction capabilities.
...
TL;DR: LLMs develop internal "Concept Circuits" for specific concepts during continual pretraining, showing predictable learning/forgetting patterns, interference effects, and transfer dynamics.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs acquire, retain, and forget concepts during continual pretraining, and how concepts interact through interference and synergy, moving beyond behavioral observations to internal computational mechanisms.

Method: Study concept learning dynamics by analyzing internal "Concept Circuits" (computational subgraphs associated with specific concepts) using Graph Metrics to characterize circuit structure during continual pretraining.

Result: (1) Concept circuits provide significant signal of concept learning/forgetting; (2) Circuits show stage-wise temporal patterns; (3) Larger learning gains correlate with greater forgetting; (4) Semantic similarity increases interference; (5) Concepts differ in transferability.

Conclusion: Concept circuits offer a circuit-level view of concept learning dynamics, enabling more interpretable and robust concept-aware training strategies for LLMs.
...
TL;DR: PsychEthicsBench is a new benchmark for evaluating LLMs' ethical alignment in mental health applications, moving beyond refusal-based metrics to assess nuanced clinical appropriateness based on Australian psychology/psychiatry guidelines.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation in mental health relies too heavily on refusal-based safety signals, which don't capture the nuanced ethical behaviors needed in clinical practice. Refusals can be perceived as unempathetic and discourage help-seeking, creating a gap in proper ethical evaluation.

Method: Developed PsychEthicsBench, a principle-grounded benchmark based on Australian psychology and psychiatry guidelines. Uses multiple-choice and open-ended tasks with fine-grained ethicality annotations to evaluate LLMs' ethical knowledge and behavioral responses across 14 models.
...
Conclusion: PsychEthicsBench provides a systematic, jurisdiction-aware foundation for evaluating LLMs in mental health, enabling more responsible development by moving beyond refusal-centric metrics to assess nuanced ethical alignment in clinical contexts.
...
TL;DR: LLMs fail to align output language with user expectations in code-switched prompts, showing systematic bias toward non-English responses. A new benchmark (OLA) reveals these failures persist across languages, but targeted alignment with minimal data can fix them.


<details>
  <summary>Details</summary>
Motivation: Code-switching is natural for multilingual users but poses challenges for LLMs. When users code-switch in prompts without specifying expected output language, LLMs must infer from contextual cues, but current models systematically fail at this alignment.

Method: Introduced OLA (Output Language Alignment) benchmark focusing on Korean-English code-switching, spanning from simple intra-sentential mixing to instruction-content mismatches. Evaluated frontier LLMs, tested generalization to Chinese and Indonesian pairs, and experimented with Chain-of-Thought prompting and Code-Switching Aware DPO with minimal data (~1K examples).

Result: Frontier models frequently misinterpret implicit language expectations, exhibiting bias toward non-English responses. Models show instability through mid-response switching and language intrusions. Chain-of-Thought prompting fails to resolve errors, but Code-Switching Aware DPO with minimal data substantially reduces misalignment.

Conclusion: LLM failures in code-switched interactions stem from insufficient alignment rather than fundamental limitations. Targeted alignment with minimal data can fix these issues, highlighting the need to align multilingual LLMs with users' implicit expectations in real-world code-switched interactions.
...
TL;DR: SGR enables LLMs to use graph-structured reasoning for better consistency in open-domain QA, achieving 17.74% improvement over base models.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning is linear and often logically inconsistent, while real-world reasoning requires integrating multiple premises and solving subproblems in parallel. Existing methods like Chain-of-Thought produce linear textual reasoning that appears coherent but leads to inconsistent conclusions, and they don't explore how LLMs can construct their own graph-structured reasoning.

Method: Proposes Self-Graph Reasoning (SGR), a framework that enables LLMs to explicitly represent their reasoning process as a structured graph before producing the final answer. Constructs a graph-structured reasoning dataset that merges multiple candidate reasoning graphs into refined graph structures for model training.

Result: Experiments on five QA benchmarks across general and specialized domains show SGR consistently improves reasoning consistency with 17.74% gain over base model. LLaMA-3.3-70B fine-tuned with SGR performs comparably to GPT-4o and surpasses Claude-3.5-Haiku.

Conclusion: Graph-structured reasoning significantly improves LLM reasoning consistency and performance in open-domain QA, demonstrating the effectiveness of explicit graph-based reasoning representations over linear approaches.
...
TL;DR: DiVA is a hybrid framework combining generative agentic search with discriminative scoring for fine-grained factuality verification, outperforming existing methods on the new FGVeriBench benchmark.


<details>
  <summary>Details</summary>
Motivation: Current LLM factuality verification uses binary judgments (correct/incorrect), which fails to capture varying error severity and limits utility for fine-grained evaluation and preference optimization.

Method: Agentic Discriminative Verifier (DiVA) - hybrid framework synergizing agentic search capabilities of generative models with precise scoring aptitude of discriminative models for fine-grained factuality verification.

Result: DiVA significantly outperforms existing methods on FGVeriBench for factuality verification on both general and multi-hop questions.

Conclusion: The proposed DiVA framework successfully addresses limitations of binary factuality verification by enabling fine-grained assessment, with strong empirical performance demonstrated on the new FGVeriBench benchmark.
...
TL;DR: ALMs offer explainable audio deepfake detection but their reasoning robustness under adversarial attacks needs evaluation. The paper introduces a forensic auditing framework analyzing three dimensions: acoustic perception, cognitive coherence, and cognitive dissonance. Findings show reasoning doesn't universally enhance robustness - it can act as a shield for some models but imposes a performance tax for others, with cognitive dissonance serving as a silent alarm for manipulation.


<details>
  <summary>Details</summary>
Motivation: Audio Language Models (ALMs) provide explainable audio deepfake detection with reasoning traces, moving beyond black-box classifiers. However, there's a need to analyze the robustness of their predictive reasoning under adversarial attacks, going beyond just final prediction shifts. Current paradigms mainly focus on prediction changes rather than reasoning quality.

Method: Introduced a forensic auditing framework to evaluate ALMs' reasoning robustness under adversarial attacks across three interconnected dimensions: 1) acoustic perception (how well models perceive audio features), 2) cognitive coherence (consistency of reasoning), and 3) cognitive dissonance (conflict between reasoning and predictions). Systematic analysis was conducted to assess reasoning shifts under attack.

Result: Explicit reasoning doesn't universally enhance robustness. Bifurcation observed: for models with robust acoustic perception, reasoning acts as a defensive "shield" against attacks. For others, it imposes a performance "tax," especially under linguistic attacks that reduce cognitive coherence and increase attack success. High cognitive dissonance can serve as a "silent alarm" for manipulation even when classification fails.

Conclusion: This work provides critical evaluation of reasoning's role in forensic audio deepfake analysis and its vulnerabilities. The forensic auditing framework reveals complex relationships between reasoning quality and robustness, showing reasoning can be both protective and vulnerable depending on model capabilities and attack types.
...
TL;DR: EPAG is a benchmark for evaluating LLMs' pre-consultation ability using diagnostic guidelines, showing task-specific fine-tuned small models can outperform frontier LLMs, and that more HPI doesn't guarantee better diagnosis.


<details>
  <summary>Details</summary>
Motivation: To develop a framework for evaluating LLMs' pre-consultation capabilities in clinical settings using diagnostic guidelines, addressing the need for standardized assessment of medical AI applications.

Method: Created EPAG benchmark dataset and framework with direct evaluation through HPI-diagnostic guideline comparison and indirect evaluation through disease diagnosis. Experiments compare different LLMs including small fine-tuned models vs. frontier models.

Result: Small open-source models fine-tuned with task-specific datasets outperform frontier LLMs in pre-consultation tasks. Increased HPI amount doesn't necessarily improve diagnostic performance. Language of pre-consultation influences dialogue characteristics.

Conclusion: Task-specific fine-tuning can enable smaller models to excel in clinical pre-consultation tasks, challenging assumptions about model size and performance. The open-sourced EPAG framework supports further development of LLM applications in real-world clinical settings.
...
TL;DR: LRMs outperform non-reasoning LLMs in judgment accuracy, instruction-following, and robustness, but still have biases. PlanJudge strategy mitigates biases by having models generate evaluation plans first.
...
Result: Four key findings: 1) LRMs outperform non-reasoning LLMs in judgment accuracy (especially on reasoning tasks), 2) LRMs have superior instruction-following, 3) LRMs are more robust against adversarial attacks, 4) Both still exhibit strong superficial quality biases. PlanJudge significantly mitigates biases in both model types.

Conclusion: LRMs are superior judges to non-reasoning LLMs across multiple dimensions, but bias remains a challenge. The simple PlanJudge strategy effectively reduces biases in both LRMs and standard LLMs, improving evaluation robustness.
...
<details>
  <summary>Details</summary>
Motivation: LLM-based agents need to continually learn new tasks without catastrophic forgetting (stability-plasticity dilemma). The core issue is the failure to distinguish between common knowledge shared across tasks and conflicting knowledge from task-specific interference.

Method: Agent-Dice uses a two-stage parameter fusion process: 1) geometric consensus filtering to prune conflicting gradients, and 2) curvature-based importance weighting to amplify shared semantics. It's based on directional consensus evaluation to disentangle knowledge updates.
...
Conclusion: Agent-Dice effectively addresses the stability-plasticity dilemma in continual learning for LLM-based agents by explicitly separating shared and conflicting knowledge through a theoretically-grounded parameter fusion framework.
...
TL;DR: LLM-MC-Affect: A probabilistic framework using Monte Carlo LLM decoding to model emotional coordination as continuous distributions, enabling analysis of interpersonal coupling in dialogues.


<details>
  <summary>Details</summary>
Motivation: Prior text-based affect inference treats sentiment as deterministic point estimates, failing to capture subjectivity, ambiguity, and sequential coupling in mutual exchanges. There's a need to model emotion as continuous distributions to better understand interpersonal dynamics.

Method: LLM-MC-Affect uses stochastic LLM decoding and Monte Carlo estimation to approximate continuous latent probability distributions over affective space. It derives sentiment trajectories quantifying central tendencies and perceptual ambiguity, then analyzes interpersonal coupling through sequential cross-correlation and slope-based indicators.

Result: The framework successfully distills high-level interaction insights from teacher-student dialogues, identifying effective scaffolding patterns through quantitative indicators of leading/lagging influences between interlocutors.

Conclusion: This work establishes a scalable pathway for understanding interpersonal dynamics, offering a generalizable solution that extends beyond education to broader social and behavioral research by modeling emotion as probabilistic distributions rather than static labels.
...
TL;DR: ELO method accelerates multilingual LLM continual pretraining by selectively training only critical first/last layers, achieving 6.46x speedup while improving target language performance and preserving source language capabilities.


<details>
  <summary>Details</summary>
Motivation: Traditional continual pretraining for multilingual LLMs suffers from high computational costs and degradation of source language performance when adapting to new languages.

Method: Two-stage approach: (1) ELO Pretraining - detach and train only critical first and last layers on target language, (2) Layer Alignment - reintegrate trained layers and perform brief full fine-tuning on small dataset for parameter alignment.

Result: Achieves up to 6.46x training speedup compared to existing methods, improves target language performance by up to 6.2% on benchmarks, and effectively preserves English source language capabilities.

Conclusion: ELO provides an efficient solution for multilingual LLM adaptation to new languages, significantly reducing computational costs while maintaining or improving performance across languages.
...
TL;DR: SyncThink is a training-free decoding method that reduces Chain-of-Thought overhead by monitoring reasoning-transition signals and terminating reasoning early, cutting token generation by ~70% while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought prompting produces long, redundant reasoning traces that substantially increase inference costs. There's a need to reduce CoT overhead without modifying model weights or requiring training.

Method: SyncThink monitors the model's own reasoning-transition signal (attention patterns showing answer tokens focus on "/think" token rather than early reasoning) and terminates reasoning when this bottleneck is detected. It's training-free and plug-and-play.

Result: Achieves 62.00% average Top-1 accuracy using only 656 tokens and 28.68s latency vs 61.22% accuracy with 2141 tokens and 92.01s for full CoT. On GPQA, yields up to +8.1 absolute accuracy by preventing over-thinking.

Conclusion: SyncThink effectively reduces CoT reasoning overhead by 70% in tokens and latency while maintaining or improving accuracy, demonstrating that early reasoning termination based on attention patterns is an efficient approach.
...
TL;DR: E5-omni is a lightweight explicit alignment method that adapts vision-language models into robust omni-modal embedding models by addressing modality-dependent similarity scales, imbalanced negative hardness, and cross-modal statistical mismatches.


<details>
  <summary>Details</summary>
Motivation: Most omni-modal embeddings rely on implicit alignment from pretrained VLMs, causing three issues: (1) modality-dependent similarity sharpness making scores inconsistent, (2) imbalanced hardness distribution in mixed-modality batches making negatives trivial, and (3) mismatched cross-modal statistics making rankings unstable.
...
Result: Experiments on MMEB-V2 and AudioCaps show consistent gains over strong bi-modal and omni-modal baselines, and the recipe transfers well to other VLM backbones.

Conclusion: E5-omni provides an effective explicit alignment recipe that adapts off-the-shelf VLMs into robust omni-modal embedding models, addressing key issues in cross-modal similarity learning.
...
TL;DR: eTracer is a plug-and-play framework for traceable text generation that grounds claims against contextual evidence to verify system-generated responses, particularly in biomedical domains.


<details>
  <summary>Details</summary>
Motivation: The need for efficient verification of system-generated responses in high-stakes domains like biomedicine, where trustworthiness and verifiability are critical.

Method: Post-hoc claim-level grounding approach that aligns each response claim with contextual evidence (supporting or contradicting), enabling traceability back to source evidence and quantifying response faithfulness.

Result: Substantial improvements in grounding quality and user verification efficiency compared to conventional sentence-level grounding methods, with code and data publicly available.

Conclusion: eTracer provides an effective framework for making generated responses verifiable and trustworthy through claim-level evidence grounding, addressing limitations of existing grounding approaches.
...
TL;DR: DisastQA is a new benchmark for evaluating QA systems in disaster management scenarios with uncertain/conflicting information, featuring 3,000 verified questions across 8 disaster types and novel evaluation protocols.


<details>
  <summary>Details</summary>
Motivation: Existing QA benchmarks are built on clean evidence, which doesn't capture the realistic challenges of disaster management where information is often uncertain, conflicting, and noisy. There's a need for benchmarks that test reasoning under imperfect information conditions.

Method: Created DisastQA benchmark with 3,000 verified questions (2,000 multiple-choice, 1,000 open-ended) across 8 disaster types using human-LLM collaboration pipeline with stratified sampling. Introduced varying evidence conditions (closed-book to noisy evidence integration) and human-verified keypoint-based evaluation for open-ended QA.

Result: Evaluation of 20 models shows substantial performance differences from general-purpose benchmarks like MMLU-Pro. While open-weight models approach proprietary systems in clean settings, performance degrades sharply under realistic noise conditions, revealing critical reliability gaps for disaster response applications.

Conclusion: DisastQA exposes critical limitations in current QA systems for disaster management, particularly their inability to handle noisy/uncertain information. The benchmark provides essential tools for developing more reliable disaster response systems, with all resources publicly available.
...
TL;DR: NeuronScope: A multi-agent framework for interpreting polysemantic neurons in LLMs through iterative activation-guided decomposition


<details>
  <summary>Details</summary>
Motivation: Existing neuron interpretation methods fail to capture polysemanticity (neurons responding to multiple concepts) in LLMs, requiring better approaches to faithfully represent multi-concept neuron behavior.

Method: NeuronScope uses a multi-agent framework that reformulates neuron interpretation as iterative, activation-guided process: decomposes activations into atomic semantic components, clusters them into distinct semantic modes, and iteratively refines explanations using activation feedback.

Result: NeuronScope successfully uncovers hidden polysemanticity and produces explanations with significantly higher activation correlation compared to single-pass baseline methods.

Conclusion: The iterative, activation-guided multi-agent approach of NeuronScope provides a more faithful interpretation of polysemantic neurons in LLMs, addressing fundamental challenges in neuron-level interpretation.
...
TL;DR: STEPS is a framework that generates compositionally challenging training data using skill taxonomies and entropy maximization to improve LLM generalization on complex skill combinations.


<details>
  <summary>Details</summary>
Motivation: LLMs and agent systems struggle with compositional generalization due to power-law distribution of complex skill combinations, creating a data bottleneck for instruction-following and agent tasks.

Method: STEPS uses structural information theory to build hierarchical skill taxonomies, then formulates data synthesis as constrained information maximization to select skill combinations that maximize marginal structural information while maintaining semantic coherence.

Result: STEPS outperforms existing data synthesis baselines on instruction-following benchmarks and improves compositional generalization in downstream agent-based evaluations.

Conclusion: The STEPS framework effectively addresses compositional generalization challenges through structured skill taxonomy-guided data synthesis, enhancing LLM performance on complex skill combinations.
...
TL;DR: FSLR is a lightweight training framework that improves LLMs' logical reasoning by focusing on the first planning step (identifying variables and operations), addressing the bottleneck where over 90% of errors occur in mathematical problem-solving.


<details>
  <summary>Details</summary>
Motivation: LLMs show limited logical reasoning in math problems, relying on pattern-matching instead. Over 90% of errors stem from poor logical relationship understanding, and existing methods like CoT-SFT fail to substantially reduce these errors.

Method: First-Step Logical Reasoning (FSLR) trains models on the isolated first planning step - identifying which variables to use and which operation to apply. This provides explicit supervision for logical relationship understanding, unlike CoT-SFT which embeds such relationships implicitly in complete solution trajectories.

Result: FSLR consistently outperforms CoT-SFT with average improvements of 3.2% in-distribution and 4.6% out-of-distribution. It achieves 4-6x faster training and reduces training token consumption by over 80%.

Conclusion: Focusing on the first planning step provides effective explicit supervision for logical relationship understanding, addressing a major bottleneck in LLMs' mathematical reasoning while being more efficient than traditional CoT-SFT approaches.
...
TL;DR: Proposes a structured evaluation framework for AI story generation with four creativity components, tested via controlled generation and human studies showing hierarchical evaluation patterns.


<details>
  <summary>Details</summary>
Motivation: Existing reference-based metrics fail to capture the subjective nature of creativity in text generation, necessitating a more nuanced evaluation framework.

Method: Developed a structured framework with four components (Novelty, Value, Adherence, Resonance) and 11 sub-components. Used controlled story generation via "Spike Prompting" and conducted a crowdsourced study with 115 readers to examine how creative components shape human judgments.

Result: Creativity is evaluated hierarchically rather than cumulatively, with different dimensions becoming salient at different judgment stages. Reflective evaluation substantially alters both ratings and inter-rater agreement. The framework effectively reveals creativity dimensions obscured by reference-based evaluation.

Conclusion: The proposed structured evaluation framework successfully captures nuanced dimensions of creativity in AI story generation that traditional reference-based metrics miss, demonstrating hierarchical evaluation patterns and the importance of reflective judgment.
...
TL;DR: RedBench is a universal dataset aggregating 37 benchmark datasets with 29,362 samples for comprehensive LLM vulnerability assessment, featuring standardized risk categories and domains.
...
Method: Introduce RedBench by aggregating 37 benchmark datasets from leading conferences and repositories, comprising 29,362 samples across attack and refusal prompts, with a standardized taxonomy of 22 risk categories and 19 domains.

Result: Created a comprehensive dataset enabling consistent evaluations, provided detailed analysis of existing datasets, established baselines for modern LLMs, and open-sourced the dataset and evaluation code.

Conclusion: RedBench facilitates robust comparisons, fosters future research, and promotes the development of secure and reliable LLMs for real-world deployment by addressing limitations of existing red teaming datasets.
...
TL;DR: ADEPT enables dynamic token-level early exit in both prefill and generation phases of LLM inference, overcoming KV cache bottlenecks to achieve up to 25% efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Current early-exit strategies are limited - they either only apply to the first token in generation or at prompt level in prefill, leaving KV cache for skipped layers as a bottleneck that limits computational savings.

Method: ADEPT introduces adaptive token-level early-exit mechanism that dynamically adjusts computation based on token complexity, plus decouples sequential dependencies in skipped layers to make KV generation more efficient.
...
Conclusion: ADEPT successfully overcomes KV cache bottlenecks in early-exit strategies, enabling practical dynamic early exit across both prefill and generation phases for significant computational efficiency gains without performance compromise.
...
TL;DR: AirNav is a new large-scale UAV vision-language navigation benchmark using real urban aerial data with natural instructions, plus AirVLN-R1 model combining supervised and reinforcement fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing UAV VLN datasets have limitations: dependence on virtual environments, unnatural instructions, and small scale. Need real-world aerial data with natural language instructions for better UAV navigation.

Method: 1) Created AirNav benchmark using real urban aerial data instead of synthetic environments with natural/diverse instructions. 2) Developed AirVLN-R1 model combining Supervised Fine-Tuning and Reinforcement Fine-Tuning for enhanced performance and generalization.

Result: Proposed AirNav benchmark addresses limitations of existing datasets. AirVLN-R1 model feasibility is preliminarily evaluated through real-world tests. Dataset and code are publicly available.

Conclusion: AirNav provides a more realistic UAV VLN benchmark using real urban aerial data, and AirVLN-R1 shows promise for improved UAV navigation through combined fine-tuning approaches.
...
TL;DR: DeepSeek-OCR's high performance relies heavily on linguistic priors rather than true OCR capabilities; without language support, accuracy drops from ~90% to 20%. Traditional OCR methods are more robust to semantic corruption than end-to-end approaches, and optical compression may worsen long-context bottlenecks.


<details>
  <summary>Details</summary>
Motivation: To investigate whether DeepSeek-OCR's claimed high-ratio vision-text compression performance stems from genuine optical character recognition capabilities or reliance on linguistic priors, addressing concerns about its potential to solve LLM long-context bottlenecks.
...
Result: Performance drops dramatically from ~90% to 20% without linguistic support. Traditional pipeline OCR methods show significantly higher robustness to semantic perturbations than end-to-end methods. Lower visual token counts increase reliance on priors and hallucination risks. Model collapses completely around 10,000 text tokens.

Conclusion: DeepSeek-OCR's performance is heavily dependent on linguistic crutches rather than visual merit. Current optical compression techniques may paradoxically aggravate long-context bottlenecks rather than solving them, highlighting the need for more robust vision-text compression approaches.
...
TL;DR: MIND is a capability-adaptive framework that transfers reasoning abilities from large to small language models by synthesizing diverse teacher perspectives and aligning supervision with student's evolving capacity, achieving SOTA performance with better generalization.


<details>
  <summary>Details</summary>
Motivation: Current knowledge distillation approaches restrict students to follow single golden rationales, which can act as out-of-distribution noise due to mismatched inductive biases and evolving student capacities, leading to suboptimal performance and poor generalization.

Method: Proposes MIND framework with: 1) Teaching Assistant network that synthesizes diverse teacher perspectives, 2) Feedback-Driven Inertia Calibration mechanism that uses inertia-filtered training loss to align supervision with student's current adaptability, preventing catastrophic forgetting.

Result: Achieves state-of-the-art performance on both in-distribution and out-of-distribution benchmarks, with sophisticated latent space analysis confirming effective reasoning ability internalization.

Conclusion: MIND successfully transitions distillation from passive mimicry to active cognitive construction, enabling better transfer of reasoning capabilities from large to small models while maintaining performance and generalization.
...
TL;DR: Proposed synthetic data augmentation framework to improve Indonesian ASR performance on stuttered speech by generating synthetic stuttered audio and fine-tuning Whisper models.


<details>
  <summary>Details</summary>
Motivation: ASR systems perform poorly on stuttered speech, especially for low-resource languages like Indonesian where specialized datasets are scarce, limiting inclusive speech technology development.

Method: Data augmentation framework that generates synthetic stuttered audio by injecting repetitions and prolongations into fluent text using rule-based transformations and LLMs, followed by TTS synthesis. Applied synthetic data to fine-tune pre-trained Indonesian Whisper model via transfer learning.

Result: Experiments show targeted synthetic exposure consistently reduces recognition errors on stuttered speech while maintaining performance on fluent segments.

Conclusion: Synthetic data pipelines are valuable for developing more inclusive speech technologies in under-represented languages, enabling adaptation to dysfluent acoustic patterns without requiring large-scale real-world recordings.
...
TL;DR: A framework for automated synthesis of high-quality instructional data using multi-agent AI collaboration, enabling open-source LLMs to achieve SOTA performance without proprietary data.


<details>
  <summary>Details</summary>
Motivation: The performance gap between closed-source and open-source LLMs stems from disparities in access to high-quality training data. The authors aim to bridge this gap without relying on proprietary data or models.

Method: A multi-agent workflow where AI agents simulate complex tool-integrated reasoning to generate diverse, high-fidelity instructional data end-to-end. This is followed by a two-stage training strategy combining supervised fine-tuning with a novel reinforcement learning method to maximize model alignment and capability.

Result: Extensive experiments show the framework empowers open-source models across multiple scales, enabling them to achieve new state-of-the-art performance on major deep research benchmarks.

Conclusion: The work provides a scalable and effective pathway for advancing open-source LLMs without proprietary data or models, bridging the performance gap with closed-source alternatives.
...
TL;DR: LLMs show institutional source preferences in knowledge conflicts but these preferences can be reversed by simple repetition, requiring mitigation techniques.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly used in retrieval-augmented generation pipelines, understanding their behavior under knowledge conflicts is crucial. The role of information source credibility in these conflicts has been overlooked, despite interdisciplinary research on credibility being highly relevant.

Method: Developed a novel framework to investigate source preferences in LLM knowledge conflict resolution. Conducted comprehensive, tightly-controlled evaluation of 13 open-weight LLMs. Proposed a novel method to mitigate repetition bias effects on source preferences.

Result: LLMs prefer institutionally-corroborated information (government, newspaper sources) over information from people and social media. However, these source preferences can be reversed by simply repeating information from less credible sources. The proposed mitigation method reduces repetition bias by up to 99.8% while maintaining at least 88.8% of original preferences.

Conclusion: Source preferences significantly influence LLM knowledge conflict resolution, but are vulnerable to manipulation through repetition. The proposed mitigation method effectively addresses this vulnerability while preserving original source credibility judgments. The research highlights the importance of considering source credibility in knowledge-intensive NLP applications.
...
TL;DR: Personalization capabilities of LLMs affect text detectability across 10 languages, with platform-targeted personalization having greater impact than demographic targeting, especially in English where personalization quality is highest.


<details>
  <summary>Details</summary>
Motivation: As LLMs improve in multilingual text generation, concerns about misuse for personalized disinformation grow. Previous research shows personalization reduces detectability of machine-generated texts, but this was only studied in English. This work examines the phenomenon across multiple languages and explores both potential misuse and benefits of personalization capabilities.

Method: The study examines 1080 combinations of various personalization aspects in prompts across 10 languages. Texts are generated by 16 distinct language models, resulting in 17,280 texts total. The research analyzes personalization quality when targeting demographic groups versus social-media platforms, and measures how personalization affects detectability of generated texts.

Result: Results show differences in personalization quality across languages when targeting demographic groups versus social-media platforms. Platform-targeted personalization affects detectability of generated texts more significantly than demographic targeting. English shows the highest personalization quality and the greatest impact on detectability from platform personalization.

Conclusion: Personalization capabilities of LLMs have varying effects across languages, with platform-targeted personalization posing greater challenges for detection than demographic targeting. The findings highlight the need for multilingual detection approaches and suggest that personalization quality correlates with its impact on detectability, with English being the most affected language.
...
TL;DR: Self-explanations from LLMs improve human and LLM ability to predict model behavior on counterfactual questions, but effectiveness depends on perturbation strategy and judge capability.


<details>
  <summary>Details</summary>
Motivation: While LLMs can generate self-explanations, prior research questions whether these rationales truly reflect the model's decision process. The paper investigates whether such explanations still help users predict model behavior, particularly through counterfactual simulatability.

Method: Using StrategyQA, researchers evaluated how well humans and LLM judges can predict model answers to counterfactual follow-up questions, with and without access to chain-of-thought or post-hoc explanations. They compared LLM-generated counterfactuals with pragmatics-based perturbations as alternative test case construction methods.

Result: Self-explanations consistently improved simulation accuracy for both LLM judges and humans, but the degree and stability of gains strongly depended on perturbation strategy and judge strength. Qualitative analysis of human free-text justifications showed explanations helped form more accurate predictions on perturbed questions.

Conclusion: LLM self-explanations, while potentially not reflecting true decision processes, still provide practical value by helping users better predict model behavior on counterfactual scenarios, though effectiveness varies based on how test cases are constructed and who is making the predictions.
...
TL;DR: ID of LLM representations tracks linguistic complexity, distinguishing formal vs. functional complexity types and aligning with specific processing phases.


<details>
  <summary>Details</summary>
Motivation: To investigate whether intrinsic dimension (ID) of LLM representations can serve as a marker for different types of linguistic complexity (formal vs. functional) and whether ID profiles across layers correspond to specific linguistic processing phases.

Method: Analyzed ID of LLM representations across layers for different sentence types: formal complexity (coordination/subordination), functional complexity (right branching vs. center embedding, unambiguous vs. ambiguous relative clauses). Used representational similarity analysis and layer ablation experiments to validate findings.

Result: Formal complexity (multiple clauses) shows clear ID differences aligning with abstract linguistic processing phase. Functional complexity shows less marked ID differences and doesn't correlate with same processing phase. Representational similarity and ablation confirm these trends across different LLMs.

Conclusion: ID is a useful marker for linguistic complexity in LLMs, can differentiate between complexity types, and reveals consistent processing stages across different LLM architectures.
...
TL;DR: HearSay benchmark reveals that Audio Large Language Models (ALLMs) significantly leak private user information through voiceprints, with existing safety mechanisms being inadequate and reasoning capabilities amplifying privacy risks.


<details>
  <summary>Details</summary>
Motivation: While Audio Large Language Models have advanced in understanding and generation, their potential privacy implications through acoustic voiceprints remain unexplored, creating a critical gap in understanding privacy vulnerabilities.

Method: Created HearSay benchmark with over 22,000 real-world audio clips using rigorous curation pipeline with automated profiling and human verification. Conducted extensive experiments to measure privacy leakage, safety mechanism effectiveness, and impact of reasoning capabilities.

Result: 1) Significant privacy leakage: ALLMs extract private attributes from voiceprints with 92.89% accuracy on gender and effectively profile social attributes. 2) Insufficient safety mechanisms: Most models fail to refuse privacy-intruding requests with near-zero refusal rates for physiological traits. 3) Reasoning amplifies risk: Chain-of-Thought reasoning exacerbates privacy risks by uncovering deeper acoustic correlations.

Conclusion: The findings expose critical vulnerabilities in ALLMs, highlighting urgent need for targeted privacy alignment to address privacy leakage through voiceprints and inadequate safety mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM agent memory systems fail to preserve topic continuity in dialogues, following a fragmentation-compensation paradigm that breaks dialogue streams into isolated utterances and then tries to restore coherence via embedding-based retrieval. This damages narrative and causal flow while biasing retrieval towards lexical similarity.
...
Conclusion: By explicitly modeling topic continuity, Membox offers a cognitively motivated mechanism for enhancing both coherence and efficiency in LLM agents, addressing fundamental limitations of current fragmentation-compensation approaches to dialogue memory systems.
...
TL;DR: The paper proposes a new selection relevance score for choosing influential training examples for model explanations, shows current strategies often underperform random selection, and introduces a balanced strategy that outperforms naive top-k selection.


<details>
  <summary>Details</summary>
Motivation: Training data influence estimation methods identify influential training documents, but humans can only interpret small subsets. Current evaluation methods ignore selection strategies, which directly impact explanation quality.

Method: Proposes a novel selection relevance score metric to quantify how useful a set of examples is for explaining model outputs. Validates through fine-tuning experiments and introduces a selection strategy that balances influence and representativeness.

Result: The selection relevance score effectively predicts whether example sets support or undermine model predictions. Common selection strategies often underperform random selection. The proposed balanced strategy enables better use of selection budgets than naive top-k selection.

Conclusion: Selection strategies matter for training data influence explanations. The proposed selection relevance score provides a useful evaluation metric, and the balanced influence-representativeness strategy improves explanation quality over existing approaches.
...
TL;DR: NeoAMT: An agentic framework for neologism-aware machine translation using Wiktionary search tool and reinforcement learning with novel reward design and adaptive rollout generation.


<details>
  <summary>Details</summary>
Motivation: Neologism-aware machine translation is underexplored compared to general MT, creating a need for specialized approaches to handle source sentences containing newly coined words or terms.

Method: 1) Created new dataset from English Wiktionary dump covering 16 languages and 75 translation directions; 2) Developed Wiktionary-based search tool; 3) Used reinforcement learning with novel reward design and adaptive rollout generation based on "translation difficulty" to train translation agents.

Result: Built comprehensive dataset from ~10M Wiktionary records, retrieval corpus from ~3M cleaned records, and developed RL framework with specialized reward mechanisms for neologism translation.

Conclusion: Proposed NeoAMT framework advances neologism-aware MT through Wiktionary integration and innovative RL training, addressing a gap in current MT research with potential for improved translation of novel vocabulary.
...
TL;DR: The paper argues that current PII leakage evaluations in LLMs are flawed because they rely on lexical cues, and proposes a cue-controlled framework (CRM) that shows true memorization is much lower than previously reported.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of PII leakage in LLMs misinterpret cue-driven reconstruction as memorization, leading to overestimation of privacy risks. The authors aim to establish a more principled approach to measure genuine memorization.

Method: Proposed Cue-Resistant Memorization (CRM) framework that explicitly controls for prompt-target overlap cues. Conducted large-scale multilingual evaluation across 32 languages using reconstruction-based settings (verbatim completion, associative reconstruction), cue-free generation, and membership inference with cue control.

Result: When lexical cues are controlled for, reconstruction success diminishes substantially. Cue-free generation and membership inference show extremely low true positive rates. Previously reported PII leakage is better explained by cue-driven behavior than genuine memorization.

Conclusion: Cue-controlled evaluation is essential for reliably quantifying privacy-relevant memorization in LLMs. Most reported PII leakage results from prompt-induced generalization rather than true memorization, suggesting privacy risks may be overestimated.
...
TL;DR: The paper introduces VietMed-MCQ, a Vietnamese Traditional Medicine benchmark dataset created using RAG with consistency checking, showing that Chinese-prior models outperform Vietnamese models on this specialized medical domain.


<details>
  <summary>Details</summary>
Motivation: LLMs perform poorly in culturally specific medical domains like Vietnamese Traditional Medicine due to lack of high-quality structured benchmarks, creating a need for specialized evaluation datasets.

Method: Created VietMed-MCQ dataset using Retrieval-Augmented Generation pipeline with automated consistency check and dual-model validation. Generated 3,190 questions across three difficulty levels, validated by medical experts and students.

Result: Dataset achieved 94.2% expert approval with high inter-rater agreement (Fleiss' kappa=0.82). Benchmarking showed general-purpose models with Chinese priors outperform Vietnamese-centric models, revealing cross-lingual transfer but limitations in complex diagnostic reasoning.

Conclusion: The study provides a valuable benchmark for low-resource medical domains, demonstrates cross-lingual knowledge transfer in specialized medicine, and highlights ongoing challenges in complex medical reasoning for LLMs.
...
TL;DR: Transformer language models encode psycholinguistic features differently depending on probing method, with contextualized embeddings showing higher selectivity and different layer profiles than isolated embeddings, and final layers rarely optimal for psycholinguistic information recovery.


<details>
  <summary>Details</summary>
Motivation: Understanding where transformer language models encode psychologically meaningful aspects of meaning is essential for both theoretical understanding and practical applications in NLP.

Method: Systematic layer-wise probing study of 58 psycholinguistic features across 10 transformer models (encoder-only and decoder-only architectures), comparing three embedding extraction methods.

Result: Apparent localization of meaning is strongly method-dependent; contextualized embeddings yield higher feature-specific selectivity and different layer-wise profiles than isolated embeddings. Final-layer representations are rarely optimal for recovering psycholinguistic information with linear probes. Models share a depth ordering of meaning dimensions: lexical properties peak earlier, while experiential and affective dimensions peak later.

Conclusion: Where meaning "lives" in transformer models reflects an interaction between methodological choices and architectural constraints, highlighting the importance of probing methodology in interpreting model representations.
...
TL;DR: Evaluation of AI text detection methods shows transformer-based models outperform traditional approaches, with DistilBERT achieving best overall performance (88.11% accuracy, 0.96 ROC-AUC), highlighting the superiority of contextual semantic modeling over lexical features.


<details>
  <summary>Details</summary>
Motivation: Addressing academic integrity concerns as students increasingly use LLM-generated content as their own work, necessitating effective AI text detection methods.

Method: Built unified benchmark using HC3 and DAIGT v2 datasets with topic-based data split to prevent information leakage. Evaluated traditional ML (TF-IDF logistic regression) and deep learning models (BiLSTM, DistilBERT).

Result: TF-IDF logistic regression achieved 82.87% accuracy baseline. BiLSTM achieved 88.86% accuracy. DistilBERT achieved 88.11% accuracy with highest ROC-AUC of 0.96, demonstrating strongest overall performance.

Conclusion: Contextual semantic modeling significantly outperforms lexical features for AI text detection. Topic-based evaluation protocols are crucial to prevent memorization. Future work will focus on dataset diversity expansion, parameter-efficient fine-tuning (LoRA), and hardware optimization.
...
TL;DR: SPAE introduces step-level potential signals for fine-grained credit assignment in RL with verifiable rewards, improving accuracy while reducing response length.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR approaches lack semantically grounded, step-level measures of reasoning progress, causing LLMs to fail at distinguishing necessary deduction from redundant verification, sometimes overturning correct trajectories into incorrect answers.

Method: Introduces Step Potential signal combining intermediate confidence and correctness, and Step Potential Advantage Estimation (SPAE) - a fine-grained credit assignment method that amplifies potential gains, penalizes potential drops, and applies penalty after potential saturation to encourage timely termination.
...
Conclusion: SPAE addresses the lack of process supervision in RLVR by providing step-level reasoning progress signals, enabling more efficient and accurate reasoning in LLMs through fine-grained credit assignment.
...
TL;DR: TabTrim: A novel table pruning framework that transforms pruning from sequential revisions to gold trajectory-supervised parallel search, achieving state-of-the-art performance on TableQA tasks.


<details>
  <summary>Details</summary>
Motivation: Existing table pruning methods rely on sequential revisions driven by unreliable critique signals, often failing to detect the loss of answer-critical data during the pruning process.

Method: TabTrim uses gold SQL query execution to derive gold pruning trajectories, trains a pruner and verifier to align step-wise pruning with these trajectories, and performs parallel search during inference to explore multiple candidate pruning trajectories and identify optimal sub-tables.
...
Conclusion: TabTrim's gold trajectory-supervised parallel search approach effectively addresses limitations of sequential pruning methods and achieves state-of-the-art performance across diverse tabular reasoning tasks.
...
TL;DR: CPT treats loss as proxy for knowledge learning but optimization and learning systematically diverge - loss decreases monotonically while factual learning is unstable and non-monotonic.


<details>
  <summary>Details</summary>
Motivation: Continual Pre-Training (CPT) is widely used for acquiring/updating factual knowledge in LLMs but treats loss as proxy for learning without understanding how knowledge actually changes during training.

Method: Constructed controlled, distribution-matched benchmark of factual documents with interleaved diagnostic probes in CPT loop for epoch-level measurement of knowledge acquisition dynamics and OOD general skills changes. Analyzed how CPT reshapes knowledge circuits during training across three instruction-tuned LLMs and multiple CPT strategies.

Result: Optimization and learning systematically diverge: loss decreases monotonically while factual learning is unstable/non-monotonic. Acquired facts rarely consolidated, learning strongly conditioned on prior exposure, OOD performance degrades from early epochs. Circuit analysis shows rapid reconfiguration of knowledge pathways across epochs, explaining narrow acquisition windows and systematic forgetting.

Conclusion: Loss optimization is misaligned with learning progress in CPT, motivating evaluation of stopping criteria based on task-level learning dynamics rather than just loss minimization.
...
TL;DR: PartisanLens is the first multilingual dataset of hyperpartisan news headlines in Spanish, Italian, and Portuguese, annotated for multiple political discourse aspects, used to evaluate LLMs for detecting hyperpartisan narratives and Population Replacement Conspiracy Theories.


<details>
  <summary>Details</summary>
Motivation: Existing resources for detecting hyperpartisan narratives and PRCTs are scarce, English-centric, and analyze political discourse aspects in isolation rather than as interrelated phenomena. These narratives threaten social cohesion and public safety through political polarization and extremist violence.

Method: Created PartisanLens dataset with 1617 hyperpartisan news headlines in Spanish, Italian, and Portuguese with multi-aspect annotations. Evaluated LLM classification performance, assessed LLMs as automatic annotators, and explored conditioning LLMs on socio-economic/ideological profiles to emulate human annotation patterns.

Result: Established robust baselines for hyperpartisan and PRCT narrative classification. Found LLMs show both potential and limitations as automatic annotators. Demonstrated LLMs can be conditioned on socio-economic/ideological profiles to approximate human annotation patterns.

Conclusion: PartisanLens supports future research on detecting partisan and conspiratorial narratives in European contexts, addressing the gap in multilingual resources and providing tools to combat misinformation that threatens social cohesion and public safety.
...
TL;DR: Large-scale empirical study evaluates safety alignment in 32 LLMs/LRMs, finding reasoning-integrated models safest, identifying post-training degradation risks, exposing dangerous CoT attack vulnerabilities, and revealing dominant attack methods.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate what matters for safety alignment in LLMs and LRMs, providing essential insights for developing more secure and reliable AI systems by investigating intrinsic model characteristics and external attack techniques.

Method: Comprehensive evaluation of 32 recent LLMs/LRMs across 13 model families (3B-235B parameters) using 5 safety datasets, 56 jailbreak techniques, and 4 CoT attack strategies, totaling 4.6M API calls to assess safety alignment vulnerabilities.

Result: 1) GPT-OSS-20B, Qwen3-Next-80B-A3B-Thinking, and GPT-OSS-120B are safest, showing reasoning/self-reflection advantages. 2) Post-training/knowledge distillation degrades safety alignment. 3) CoT attacks via response prefixes increase success rates dramatically (3.34x average, up to 96.3%). 4) Roleplay, prompt injection, and gradient-based adversarial prompts are dominant attack methods.

Conclusion: Safety must be treated as an explicit constraint during model development, not subordinate to general capability. Text-completion interfaces with user-defined response prefixes pose critical risks requiring architectural safeguards. Integrated reasoning mechanisms enhance safety alignment robustness.
...
TL;DR: ATLAS is a dual-path framework for dynamic tool usage in cross-domain complex reasoning that outperforms GPT-4o and existing routing methods by adaptively selecting optimal LLM-tool combinations.


<details>
  <summary>Details</summary>
Motivation: As LLMs and external tools diversify, selecting optimal model-tool combinations becomes a high-dimensional optimization challenge. Existing approaches use single models or fixed tool-calling logic, failing to exploit performance variations across heterogeneous model-tool pairs.
...
Conclusion: ATLAS effectively addresses the challenge of optimal LLM-tool combination selection through adaptive routing, demonstrating superior performance across diverse domains and tasks compared to existing methods and state-of-the-art models.
...
TL;DR: Small decoder-only language models (SLMs) are evaluated for grammar correction and text simplification tasks, but despite efficiency advantages, they underperform compared to large language models (LLMs) and struggle with meaning retention and hallucinations.


<details>
  <summary>Details</summary>
Motivation: Large language models are computationally expensive and difficult to deploy in many settings, creating a need for more efficient alternatives. The paper investigates whether small decoder-only language models can serve as viable substitutes for grammar correction and text simplification tasks.

Method: The study tests small language models in three configurations: out-of-the-box, fine-tuned, and run sequentially. Experiments are conducted on JFLEG and ASSET datasets using established evaluation metrics to assess performance on grammar correction and text simplification tasks.

Result: Small language models show some ability to learn certain behaviors but consistently underperform compared to strong baselines and current large language models. They particularly struggle with retaining original meaning and suffer from hallucination issues.

Conclusion: Despite their efficiency advantages, current small language models are not yet competitive with modern large language models for rewriting tasks. Significant advances in training methods are needed to close the performance gap between SLMs and today's LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing RAG approaches indiscriminately trigger retrieval and use single-path evidence construction, which introduces noise and limits performance gains.

Method: DTR uses generation uncertainty to guide retrieval triggering decisions and introduces a dual-path retrieval mechanism with adaptive information selection to handle sparse and ambiguous queries.

Result: Extensive experiments across five open-domain QA benchmarks show DTR consistently improves EM and F1 over standard RAG and strong baselines while reducing unnecessary retrievals.

Conclusion: DTR provides an effective training-free framework for adaptive retrieval in RAG systems that improves performance and efficiency across various settings.
...
TL;DR: Language models implement multiple-choice QA through a two-stage process: first selecting the correct answer in content space, then binding it to the appropriate output symbol.
...
Method: Used representational analyses (PCA, linear probes) and causal interventions to examine how models implement MCQA, focusing on option-boundary residual states and winner-identity probing.

Result: Found that option-boundary residual states contain strong linearly decodable signals about per-option correctness. Winner-identity probing reveals a two-stage progression: winning content position becomes decodable immediately after final option processing, while output symbol representation emerges closer to answer emission.

Conclusion: Models use a two-stage mechanism: first select winner in content space, then bind/route that winner to appropriate output symbol for emission, explaining the separation between reasoning and symbol-binding in MCQA tasks.
...
### [90] [InfiniteWeb: Scalable Web Environment Synthesis for GUI Agent Training](https://arxiv.org/abs/2601.04126)
*Ziyun Zhang,Zezhou Wang,Xiaoyi Zhang,Zongyu Guo,Jiahao Li,Bin Li,Yan Lu*

Main category: cs.CL

TL;DR: InfiniteWeb automatically generates functional web environments at scale for GUI agent training, addressing the scarcity of suitable training environments through automated website generation with verifiable task evaluators.


<details>
  <summary>Details</summary>
Motivation: GUI agents that interact with graphical interfaces are promising for practical AI assistants, but training such agents is hindered by the scarcity of suitable environments. Current approaches struggle with building realistic, functional websites with many interconnected pages.

Method: Uses unified specification, task-centric test-driven development, and combines website seed with reference design image to ensure diversity. The system generates verifiable task evaluators enabling dense reward signals for reinforcement learning.
...
Conclusion: The system effectively addresses the environment scarcity problem for GUI agent training, demonstrating that automatically generated web environments can significantly improve GUI agent performance on real-world tasks.
...
TL;DR: Doc-PP benchmark reveals LVLMs leak sensitive info when complex multimodal reasoning is required, despite policy constraints. DVA framework decouples reasoning from verification to prevent this.


<details>
  <summary>Details</summary>
Motivation: Real-world document QA requires adherence to dynamic, user-defined disclosure policies, but existing safety research focuses on implicit social norms or text-only settings, ignoring multimodal complexities.

Method: Created Doc-PP benchmark from real-world reports requiring multimodal reasoning under strict policies. Proposed DVA (Decompose-Verify-Aggregation) framework that separates reasoning from policy verification.

Result: Found systemic Reasoning-Induced Safety Gap: models leak sensitive info when answers require complex synthesis across modalities. Extracted text improves perception but facilitates leakage. DVA significantly outperforms standard prompting defenses.

Conclusion: DVA offers robust baseline for policy-compliant document understanding by decoupling reasoning from policy verification, addressing vulnerabilities in multimodal document QA systems.
...
TL;DR: Arctic-ABSA introduces powerful multilingual aspect-based sentiment analysis models with expanded sentiment classes, reasoning injection, and novel pretraining, achieving SOTA results while being 10% more accurate than GPT-4o and Claude 3.5 Sonnet.


<details>
  <summary>Details</summary>
Motivation: To address real-life commercial ABSA needs by expanding beyond standard sentiment classes, supporting multiple languages, and improving generalization through reasoning capabilities.

Method: Trained on large corpus of public data plus synthetic data (20x larger than SemEval14), expanded sentiment classes to 5 (adding mixed/unknown), joint prediction of overall sentiment, reasoning injection via CoT fine-tuning, and novel reasoning pretraining for encoder-only models.

Result: 395M-parameter encoder and 8B-parameter decoder achieve up to 10 percentage points higher accuracy than GPT-4o and Claude 3.5 Sonnet, set new SOTA on SemEval14, maintain 87-91% accuracy across 6 languages without degrading English performance.

Conclusion: Arctic-ABSA models demonstrate superior performance for commercial ABSA applications through expanded capabilities, multilingual support, and reasoning-enhanced training, while releasing ABSA-mix benchmark for future research.
...
TL;DR: RADAR: A retrieval-augmented detector with adversarial refinement that uses verbal adversarial feedback to improve fake news detection by co-evolving generator and detector through natural-language critiques.


<details>
  <summary>Details</summary>
Motivation: To efficiently combat the spread of LLM-generated misinformation by creating a robust detection system that can adapt to evolving fake news tactics through adversarial training.

Method: Uses a generator that rewrites real articles with factual perturbations, paired with a lightweight detector that verifies claims using dense passage retrieval. Introduces verbal adversarial feedback (VAF) - structured natural-language critiques that guide the generator toward more sophisticated evasion attempts, enabling effective co-evolution of both components.
...
Conclusion: RADAR demonstrates that retrieval-augmented detection with adversarial refinement using verbal feedback is effective for combating LLM-generated misinformation, with the co-evolutionary approach enabling robust adaptation to sophisticated fake news tactics.
...
TL;DR: Benchmark^2 framework proposes three metrics to evaluate benchmark quality: ranking consistency, discriminability, and capability alignment deviation, enabling systematic assessment and efficient benchmark construction.


<details>
  <summary>Details</summary>
Motivation: With the rapid proliferation of LLM benchmarks, there's an urgent need for systematic methods to assess benchmark quality itself, as not all benchmarks are equally reliable or effective.

Method: Proposes Benchmark^2 framework with three complementary metrics: (1) Cross-Benchmark Ranking Consistency (measures alignment with peer benchmarks), (2) Discriminability Score (quantifies ability to differentiate models), and (3) Capability Alignment Deviation (identifies problematic instances where stronger models fail but weaker ones succeed).

Result: Extensive experiments across 15 benchmarks (math, reasoning, knowledge) with 11 LLMs across 4 families reveal significant quality variations. Selective benchmark construction using these metrics achieves comparable evaluation performance with substantially reduced test sets.

Conclusion: Benchmark^2 provides a systematic framework for assessing benchmark quality, enabling more reliable LLM evaluation and efficient benchmark construction through selective item selection.
...
TL;DR: VotIE is a new IE task for extracting structured voting events from heterogeneous municipal meeting minutes, with XLM-R-CRF achieving best in-domain performance (93.2% F1) but few-shot LLMs showing better cross-municipality generalization.


<details>
  <summary>Details</summary>
Motivation: Municipal meeting minutes contain crucial democratic decisions but encode voting outcomes in highly heterogeneous, free-form narrative text that varies widely across municipalities, making automated extraction challenging. Unlike standardized parliamentary proceedings, this variability creates significant barriers for systematic analysis.

Method: Introduced VotIE (Voting Information Extraction) task and established first benchmark using Portuguese municipal minutes from CitiLink corpus. Compared fine-tuned encoders (XLM-R-CRF) vs. generative approaches, evaluated both in-domain and cross-municipality settings to assess generalization.

Result: 1) In-domain: XLM-R-CRF achieved 93.2% macro F1, outperforming generative models. 2) Cross-municipality: Fine-tuned models suffered substantial performance degradation, while few-shot LLMs showed greater robustness with significantly smaller performance declines. However, generative models have high computational costs.

Conclusion: Lightweight fine-tuned encoders remain more practical for large-scale deployment despite generalization limitations, while few-shot LLMs show promise for cross-context robustness. Released benchmark, models, and evaluation framework to support administrative NLP research.
...
TL;DR: This paper addresses the lack of quality evaluation for LLM-simulated students in educational technology research, proposing formal metrics and benchmarking various simulation methods on a real math tutoring dataset.


<details>
  <summary>Details</summary>
Motivation: While LLMs enable educational innovations, evaluating new tutoring solutions requires real students which is time-consuming and hard to scale. Many researchers use simulated students via simple prompting, but there's little work on ensuring or measuring simulation quality.

Method: The authors formally define the student simulation task, propose evaluation metrics spanning linguistic, behavioral, and cognitive aspects, and benchmark various simulation methods (prompting strategies, supervised fine-tuning, preference optimization) on a real-world math tutoring dialogue dataset.

Result: Automated and human evaluation show that prompting strategies for student simulation perform poorly, while supervised fine-tuning and preference optimization yield much better but still limited performance.

Conclusion: The paper highlights the challenge of creating high-quality simulated students and motivates future work on this task, establishing a formal framework for evaluating student simulation quality in educational technology research.
...
TL;DR: LALMs struggle to judge speaker consistency in multi-turn dialogues, showing bias toward text over acoustics, despite having inherent acoustic discrimination capabilities.


<details>
  <summary>Details</summary>
Motivation: While Large Audio-Language Models (LALMs) are used as judges for speech generation quality, their ability to assess speaker consistency across multi-turn conversations remains unexplored, creating a gap in understanding their reliability for real-world dialogue evaluation.

Method: Created SpeakerSleuth benchmark with 1,818 human-verified instances across four diverse datasets (synthetic and real speech) with controlled acoustic difficulty. Evaluated nine widely-used LALMs through three tasks reflecting real-world requirements for speaker consistency assessment.

Result: LALMs struggle with acoustic inconsistency detection: some overpredict inconsistency while others are overly lenient; they fail to identify problematic turns; performance degrades when other interlocutors' turns are provided as models prioritize textual coherence over acoustics. However, they perform better at choosing matching audio among acoustic variants, showing inherent acoustic discrimination capabilities.

Conclusion: LALMs exhibit significant modality imbalance bias, prioritizing text over acoustics, which must be addressed to build reliable audio-language judges for speaker consistency evaluation in multi-turn dialogues.
...
TL;DR: This thesis studies cross-lingual knowledge transfer in multilingual machine translation, developing methods to improve robustness and generalization, especially for low-resource languages.


<details>
  <summary>Details</summary>
Motivation: Multilingual translation systems struggle with effective cross-lingual representation learning, particularly for low-resource languages with limited parallel data. Understanding how multilingual models share knowledge across languages requires examining representation interactions, data availability, and training strategies.

Method: The research uses machine translation as a testbed to analyze language similarity effects on transfer, employs retrieval and auxiliary supervision to strengthen low-resource translation, examines fine-tuning trade-offs in large language models, and studies the role of language diversity during training.
...
Conclusion: The thesis provides insights toward more inclusive and resilient multilingual NLP systems by highlighting how cross-lingual knowledge transfer mechanisms can be optimized through strategic modeling and data composition approaches.
...
TL;DR: SaLAD is a multimodal safety benchmark with 2,013 real-world image-text samples across 10 categories, designed to evaluate MLLM safety in daily life scenarios. It reveals current MLLMs achieve only 57.2% safe response rate on unsafe queries.


<details>
  <summary>Details</summary>
Motivation: As MLLMs become indispensable assistants, their unsafe content generation poses significant risks to human behavior and society. Current safety evaluations lack realistic multimodal scenarios where safety risks cannot be inferred from text alone.

Method: Created SaLAD benchmark with 2,013 real-world image-text samples across 10 common categories, balanced for unsafe scenarios and oversensitivity cases. Proposed safety-warning-based evaluation framework that encourages informative safety warnings instead of generic refusals.

Result: Evaluation of 18 MLLMs shows top-performing models achieve only 57.2% safe response rate on unsafe queries. Popular safety alignment methods show limited effectiveness, revealing vulnerabilities in identifying dangerous behaviors in daily life.

Conclusion: Current MLLMs have significant safety vulnerabilities in real-world multimodal scenarios. The SaLAD benchmark provides a comprehensive evaluation framework for multimodal safety, highlighting the need for improved safety mechanisms beyond text-only assessments.
...
TL;DR: MPO is a modular prompt optimization framework that treats prompts as structured objects with semantic sections, applying localized textual gradients to refine each section independently while maintaining a fixed schema, leading to improved reasoning performance in small LLMs.


<details>
  <summary>Details</summary>
Motivation: Current prompt optimization methods treat prompts as monolithic text blocks, making it difficult to localize errors, preserve critical instructions, or prevent uncontrolled prompt growth. There's a need for more structured, interpretable optimization approaches.

Method: MPO treats prompts as structured objects with semantic sections (system role, context, task description, constraints, output format). It applies section-local textual gradients generated by a critic language model to refine each section independently while keeping the overall schema fixed. Section updates are consolidated through de-duplication to reduce redundancy.
...
TL;DR: CoM-DAD is a unified multimodal generation framework that combines continuous semantic diffusion with discrete token diffusion using hierarchical dual-process modeling.


<details>
  <summary>Details</summary>
Motivation: Current generative modeling is bifurcated into autoregressive approaches for text and diffusion for images, preventing truly unified multimodal systems. Masked language models lack generative fidelity and semantic continuity, and extending masked generation to multimodal settings faces alignment challenges and training instability.

Method: CoM-DAD uses a hierarchical dual-process: 1) continuous latent diffusion for semantic manifold modeling, and 2) discrete absorbing diffusion for token generation with Variable-Rate Noise Schedule conditioned on semantic priors. Uses Stochastic Mixed-Modal Transport for modality alignment without heavy contrastive dual-encoders.
...
Conclusion: CoM-DAD provides a novel probabilistic framework that reformulates multimodal generation as hierarchical dual-process, enabling unified text-image generation with better stability and alignment than existing approaches.
...
TL;DR: A framework using code-guided knowledge graph exploration to reduce LLM hallucinations by embedding executable modules in reasoning prompts, improving accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: To mitigate hallucinations in large language models, particularly those induced by prompts, by leveraging external structured knowledge to constrain erroneous reasoning.

Method: Extends chain-style knowledge distillation with a programmable module that guides knowledge graph exploration, embedded as executable code within reasoning prompts to leverage external structured knowledge during inference.

Result: Significant improvements in contextual modeling and reduced prompt-induced hallucinations, with HIT@1, HIT@3, and HIT@5 increasing by 15.64%, 13.38%, and 13.28% respectively, exceeding 95% across several evaluation settings using GPT-4 and LLaMA-3.3.

Conclusion: The proposed method effectively constrains erroneous reasoning while improving both accuracy and interpretability, demonstrating that code-guided reasoning significantly reduces LLM hallucinations.
...
TL;DR: SearchAttack is a red-teaming method that exploits web search as an attack surface for search-augmented LLMs by outsourcing harmful semantics to search queries and reconstructing retrieved content.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the vulnerability of search-augmented LLMs when handling harmful tasks. While these systems aim to improve reliability for knowledge-intensive tasks, they become vulnerable when search engines retrieve harmful content that bypasses the LLM's safeguards, creating an uncontrolled exposure risk.

Method: SearchAttack outsources harmful semantics to web search by creating query skeletons with fragmented clues instead of explicit harmful content. It then steers LLMs to reconstruct the retrieved harmful content using structural rubrics to achieve malicious goals while bypassing safety mechanisms.

Result: Extensive experiments show SearchAttack demonstrates strong effectiveness in attacking search-augmented LLMs, successfully exploiting the web search vulnerability to achieve malicious objectives despite the systems' safeguards.

Conclusion: The paper identifies web search as a critical attack surface for search-augmented LLMs and proposes SearchAttack as an effective red-teaming approach for responsible vulnerability assessment, highlighting the need for improved safeguards in these systems.
...
TL;DR: The paper introduces an attribution-based framework to analyze positional bias in language models, finding architecture-specific positional importance profiles that show recency bias increasing with depth and primacy bias decreasing with depth.


<details>
  <summary>Details</summary>
Motivation: Language models show positional bias (preferring information from specific positions regardless of semantic relevance), but prior work hasn't established how these biases evolve across layers and positions, or how they vary independent of task complexity.
...
Result: Positional importance profiles are architecture-specific, stable across inputs, and invariant to lexical scrambling. Recency bias increases with depth while primacy bias diminishes through model depth. Early layers preferentially weight content words over function words across all positions, while later layers lose this word-type differentiation.

Conclusion: The framework reveals systematic positional biases in language models that evolve across layers, with recency bias strengthening and primacy bias weakening with depth, and word-type differentiation disappearing in later layers.
...
<details>
  <summary>Details</summary>
Motivation: LLMs often default to their internal memorized knowledge when external retrieved context conflicts with it, leading to unfaithful outputs. As world knowledge evolves, LLMs need to faithfully follow external evidence rather than relying solely on potentially outdated parametric knowledge.

Method: ContextFocus uses activation steering - a lightweight approach that modifies model activations during inference to prioritize external context over internal knowledge. It requires no model finetuning and has minimal inference-time overhead.

Result: ContextFocus significantly improves contextual-faithfulness on the ConFiQA benchmark, outperforming baselines like ContextDPO, COIECD, and prompting methods. It's complementary to prompting strategies and remains effective on larger models.

Conclusion: ContextFocus is an effective, robust, and efficient solution for improving LLM faithfulness to external context in knowledge-conflict scenarios, enabling better deployment as world knowledge evolves.
...
TL;DR: LLMberjack is an interactive platform for converting debate reply trees into coherent multi-party conversations with LLM assistance and tree visualization.


<details>
  <summary>Details</summary>
Motivation: There's a lack of resources for creating multi-party conversations from existing debate structures, and current methods don't provide transparent, reproducible workflows for this conversion process.

Method: The platform visualizes discussion trees and enables users to construct linearized dialogue sequences while preserving participant identity and discourse relations. It integrates optional LLM assistance for automatic message editing and speaker description generation.

Result: Tree visualization facilitates creation of coherent conversation threads, and LLM support enhances output quality while reducing human effort. The tool is open-source and promotes transparent, reproducible workflows.

Conclusion: LLMberjack addresses the gap in resources for creating multi-party conversations from debate structures, offering an effective solution with visualization and AI assistance for improved workflow efficiency and output quality.
...
TL;DR: FLEx improves language model performance using few-shot explanations: clusters model errors, verifies explanations correct them, summarizes into prompt prefix to guide inference without weight updates.


<details>
  <summary>Details</summary>
Motivation: Language models still make mistakes that repeat across related queries. Natural language explanations can help correct errors but collecting them at scale is infeasible, especially in domains requiring expert annotators.

Method: FLEx (Few-shot Language Explanations): 1) Selects representative model errors using embedding-based clustering, 2) Verifies that associated explanations correct those errors, 3) Summarizes them into a prompt prefix prepended at inference-time to guide model without modifying weights.

Result: FLEx consistently outperforms chain-of-thought (CoT) prompting across CounterBench, GSM8K, and ReasonIF datasets, reducing up to 83% of CoT's remaining errors.

Conclusion: FLEx provides an effective method for improving model behavior using few explanatory examples without weight updates, addressing the scalability challenge of collecting expert explanations.
...
TL;DR: RFC Bench is a benchmark for evaluating LLMs on financial misinformation detection using realistic news at paragraph level, with two tasks: reference-free detection and comparison-based diagnosis using paired original/perturbed inputs.


<details>
  <summary>Details</summary>
Motivation: Financial misinformation in news requires understanding contextual complexity where meaning emerges from dispersed cues. Current benchmarks don't adequately capture the realistic challenges of financial misinformation detection in real-world settings.

Method: Created RFC Bench benchmark operating at paragraph level with two complementary tasks: 1) reference-free misinformation detection, and 2) comparison-based diagnosis using paired original and perturbed inputs to evaluate contextual reasoning.

Result: Experiments show substantially stronger performance when comparative context is available, while reference-free settings expose significant weaknesses including unstable predictions and elevated invalid outputs, indicating models struggle to maintain coherent belief states without external grounding.

Conclusion: RFC Bench provides a structured testbed for studying reference-free reasoning and advancing more reliable financial misinformation detection in real-world settings by highlighting the gap in current models' ability to reason without external grounding.
...
<details>
  <summary>Details</summary>
Motivation: To develop a vision-language model specifically designed for the Korean linguistic and cultural context, with enhanced reasoning capabilities and agentic abilities to support broader adoption in Korean AI applications.

Method: The model is pre-trained with strong focus on reasoning capabilities, then post-trained for multimodal understanding, enhanced reasoning, agentic behaviors, and human preference alignment.

Result: Experimental evaluations show strong performance against comparably sized models on Korean text-to-text and vision-to-text benchmarks, as well as agent-oriented evaluation tasks.
...
Result: Comprehensive benchmark spanning wide range of contemporary drone models and acquisition conditions, with interoperable open-source tools for data generation, preprocessing, augmentation, and evaluation.

Conclusion: CDRF enables standardized benchmarking for RF drone detection/identification, accelerating progress toward robust, generalizable RF perception models through reproducible pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Current concept erasure methods become inefficient and ineffective as the number of erased concepts grows, requiring separate parameters for each concept and degrading overall generation quality.

Method: Introduces supertype-subtype concept hierarchy, group-wise suppression with shared parameters, and Supertype-Preserving Low-Rank Adaptation (SuPLoRA) that encodes supertype information in frozen down-projection matrix while updating only up-projection matrix.
...
Conclusion: SuPLoRA provides an effective and efficient approach for multi-concept erasure in diffusion models by organizing concepts hierarchically and using parameter-sharing strategies to preserve generation quality.
...
TL;DR: VLM4VLA pipeline shows VLM capabilities don't predict embodied control performance; vision module is the bottleneck, not language.
...
Method: Introduces VLM4VLA, a minimal adaptation pipeline that converts general-purpose VLMs into VLA policies using only a small set of new learnable parameters for fair comparison. Tests on various downstream tasks across three benchmarks with extensive empirical studies.

Result: VLM initialization helps but general VLM capabilities are poor predictors of downstream task performance. Improving specific embodied skills doesn't guarantee better control. Vision module is the primary bottleneck, not language. Injecting control-relevant supervision into vision encoder yields consistent gains even when frozen.

Conclusion: Standard VLM competence is necessary but insufficient for effective embodied control. There's a persistent domain gap between current VLM pretraining objectives and embodied action-planning requirements.
...
TL;DR: Deep learning-based image recognition automates white shrimp classification post-harvest to maintain freshness and prevent head-body separation issues.


<details>
  <summary>Details</summary>
Motivation: Consumer demand for high-quality aquatic products with freshness and appearance integrity is increasing. Shrimp freshness declines rapidly after harvest, and soft-shell shrimp often suffer from head-body separation during cooking/freezing, affecting product appearance and consumer perception.

Method: Uses deep learning-based image recognition with a convolutional neural network (CNN) model to automate classification of white shrimp immediately after harvest, replacing manual sorting.

Result: The CNN model enhances classification accuracy, efficiency, and consistency while reducing processing time, helping maintain freshness and enabling shrimp transportation businesses to better meet customer demands.

Conclusion: Automated shrimp classification using deep learning improves product quality, maintains freshness, and addresses appearance issues in shrimp-based processed foods.
...
TL;DR: The paper proposes extending PCA's covariance matrix approach to higher-order tensors (3rd order and beyond) for more accurate rotation-invariant shape descriptors, enabling applications in molecular shape analysis, object recognition, and shape similarity metrics.


<details>
  <summary>Details</summary>
Motivation: Standard PCA uses only second-order covariance matrices, which approximate shapes as ellipsoids - too simplistic for complex real-world shapes. There's a need for more expressive rotation-invariant shape descriptors that can capture higher-order geometric details while maintaining computational efficiency.

Method: Extends PCA's covariance matrix approach to higher-order central moment tensors (e.g., 3rd order p_abc) and polynomial times Gaussian representations. These higher-order tensors provide more detailed shape descriptions while maintaining rotation invariance through analogous invariants like traces of powers.

Result: The proposed method enables arbitrarily accurate shape descriptors that are rotation-invariant, allowing shape representation modulo rotation without costly optimization over rotations. This provides a foundation for efficient shape comparison and recognition.

Conclusion: Higher-order tensor extensions of PCA offer powerful rotation-invariant shape descriptors suitable for complex real-world applications including molecular shape analysis, object recognition in 2D/3D, and efficient shape similarity metrics that avoid expensive rotation optimization.
...
TL;DR: MMErroR is a multi-modal benchmark with 2,013 samples containing single reasoning errors across 24 subdomains, designed to test VLMs' ability to detect incorrect reasoning and classify error types, revealing even top models struggle with this task.
...
Method: Created MMErroR benchmark with 2,013 samples embedding single coherent reasoning errors across 24 subdomains in six top-level domains, then evaluated 20 advanced VLMs on their ability to detect incorrect reasoning and classify error types.

Result: Even the best model (Gemini-3.0-Pro) only classified errors correctly in 66.47% of cases, demonstrating significant challenge in identifying erroneous reasoning despite VLMs' general multi-modal learning improvements.

Conclusion: MMErroR reveals VLMs' limitations in error detection and classification, providing valuable insights into multi-modal reasoning capabilities and establishing a benchmark for process-level evaluation beyond simple answer correctness.
...
TL;DR: A two-stage 3D Gaussian Splatting model for relightable head avatars that works from single/multi-view images without requiring OLAT capture data for each subject.


<details>
  <summary>Details</summary>
Motivation: Existing high-quality relighting methods require complex OLAT capture setups. The goal is to create relightable 3D head avatars from standard multi-view images without needing OLAT data for each subject.
...
Result: The model generalizes well to relight any subject from the first stage as if captured under OLAT lighting, and can fit to unseen subjects from as little as a single image.

Conclusion: Proposes a practical solution for high-quality relightable 3D head avatars that eliminates the need for complex OLAT capture per subject, enabling novel view synthesis and relighting from minimal input.
...
TL;DR: HairGuard is a framework that recovers fine-grained soft boundary details in 3D vision tasks by addressing ambiguous foreground-background mixing in thin structures like hair.


<details>
  <summary>Details</summary>
Motivation: Soft boundaries (like thin hairs) are common in natural and computer-generated imagery but challenging for 3D vision due to ambiguous mixing of foreground and background cues, leading to poor depth estimation and view synthesis quality in these regions.
...
Result: HairGuard achieves state-of-the-art performance across monocular depth estimation, stereo image/video conversion, and novel view synthesis, with significant improvements specifically in soft boundary regions.

Conclusion: The framework effectively addresses soft boundary challenges in 3D vision through specialized depth refinement and view synthesis components, enabling plug-and-play integration with existing depth models while preserving fine-grained details.
...
TL;DR: New benchmark RiskCueBench challenges video risk assessment models to identify early risk signals before accidents occur, revealing significant gaps in current systems' ability to anticipate risky events from evolving visual cues.


<details>
  <summary>Details</summary>
Motivation: Current video risk assessment datasets often include the full accident sequence, making the task artificially easy. Real-world safety applications require anticipating risks from early visual signals before accidents happen, which existing benchmarks don't adequately test.

Method: Introduces RiskCueBench, a new video understanding benchmark where videos are carefully annotated to identify a "risk signal clip" - the earliest moment indicating potential safety concerns. This forces models to interpret evolving situations and anticipate future risks from early visual cues.

Result: Experimental results show a significant performance gap in current systems' ability to interpret evolving situations and anticipate risky events from early visual signals, highlighting practical deployment challenges.

Conclusion: RiskCueBench reveals important limitations in current video risk prediction models and establishes a more realistic benchmark for evaluating systems that need to anticipate safety concerns before they escalate into accidents.
...
TL;DR: Novel deepfake detection architecture using cross-attention between spatial/frequency features and blood detection module achieves SOTA results (99.80-99.88% AUC) on FF++ and Celeb-DF datasets.


<details>
  <summary>Details</summary>
Motivation: Deepfake synthesis and misuse pose significant threats to trust in the digital age, necessitating effective detection and tagging systems to combat AI-generated media manipulation.

Method: Proposes a unified architecture combining cross-attention between spatial and frequency domain features with a blood detection module to classify images/videos as real or fake.

Result: Achieves SOTA performance: 99.80% AUC on FF++ and 99.88% on Celeb-DF using Swin Transformer+BERT; 99.55% and 99.38% using EfficientNet-B4+BERT. Shows strong cross-dataset generalization.

Conclusion: The proposed architecture effectively detects deepfakes with high accuracy and generalization, providing a comprehensive solution with insights into each detection step for combating AI-generated media threats.
...
TL;DR: Current video-based ANN models show modest improvements in predicting IT cortex responses to naturalistic videos but fail to capture appearance-invariant motion processing that IT exhibits.


<details>
  <summary>Details</summary>
Motivation: To understand whether primate inferior temporal (IT) cortex performs richer dynamic computations beyond simple frame-by-frame processing, and to test if current ANN models can capture these biological temporal computations.

Method: Compared macaque IT responses during naturalistic videos against static, recurrent, and video-based ANN models. Used a stress test with "appearance-free" video variants that preserve motion but remove shape/texture to test generalization.

Result: Video models provided modest improvements in neural predictivity, especially at later response stages. IT population activity generalized across appearance-free videos, but all ANN classes failed this generalization test.

Conclusion: Current video models capture appearance-bound dynamics rather than the appearance-invariant temporal computations expressed in IT, highlighting the need for new objectives that encode biological temporal statistics and invariances.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs perform well on standard benchmarks but rely on surface-level recognition rather than deeper reasoning. Visual word puzzles require discovering implicit visual cues, hypothesis generation/revision, and mapping perceptual evidence to non-literal concepts, which better tests complex visual understanding.

Method: Created Eye-Q benchmark with 1,343 puzzles across English, Persian, Arabic, and cross-lingual contexts. Puzzles feature conceptually dense scenes with brief descriptions requiring inference of target words/phrases. Used open-ended, human-aligned evaluation protocol that probes hypothesis formation and revision under lightweight assistance.

Result: State-of-the-art VLMs show substantial performance gaps, especially on abstract and cross-lingual puzzles. Maximum accuracy reaches only 60.27%, highlighting limitations in models' ability to construct and search over appropriate conceptual representations for flexible image-to-phrase inference.

Conclusion: Visual word puzzles reveal significant limitations in current VLMs' reasoning capabilities. The Eye-Q benchmark provides a challenging testbed for evaluating complex visual understanding that goes beyond literal grounding and simple retrieval, exposing areas needing improvement in multimodal reasoning.
...
TL;DR: GAMBIT is a novel multimodal jailbreak framework that uses gamified adversarial scenes to exploit MLLMs' reasoning incentives, achieving high attack success rates by making models actively participate in harmful content reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal jailbreak attacks underperform on reasoning models (with Chain-of-Thought) because they don't leverage the model's own reasoning incentives. The paper explores whether influencing a model's cognitive-stage decisions can make it proactively complete jailbreaks.

Method: GAMBIT decomposes and reassembles harmful visual semantics, constructs gamified scenes that drive models to explore, reconstruct intent, and answer as part of winning a game. This structured reasoning chain increases task complexity in both vision and text, positioning the model as a participant whose goal pursuit reduces safety attention.
...
Conclusion: The gamified approach effectively exploits reasoning incentives in MLLMs, demonstrating that influencing cognitive-stage decisions can successfully bypass safety mechanisms, highlighting vulnerabilities in current multimodal safety alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing agricultural models struggle to balance fine-grained feature extraction for biological attribute classification with the efficiency needed for real-time deployment in field applications.

Method: A lightweight multi-task Vision Transformer with structural reparameterization across the entire architecture (Vision Transformer backbone, Lite R-ASPP decoder, and novel reparameterizable classification head) to decouple training-time capacity from inference-time latency.
...
Conclusion: WeedRepFormer effectively addresses the efficiency-accuracy trade-off in agricultural vision tasks, providing a practical solution for real-time waterhemp analysis with simultaneous segmentation and gender classification capabilities.
...
TL;DR: FROST-Drive: A novel end-to-end autonomous driving architecture that keeps the vision encoder frozen from a pretrained VLM to preserve generalization capabilities, outperforming fully fine-tuned models on challenging driving scenarios.


<details>
  <summary>Details</summary>
Motivation: Current E2E autonomous driving models that fully fine-tune vision encoders on driving data risk losing generalization capabilities by overspecializing to training data. The authors challenge whether this full fine-tuning paradigm is necessary and propose to preserve the rich world knowledge from pretrained VLMs.

Method: FROST-Drive architecture with: 1) Frozen pretrained VLM vision encoder, 2) Transformer-based adapter for multimodal fusion, 3) GRU-based decoder for smooth waypoint generation, 4) Custom loss function optimized for Rater Feedback Score (RFS) to prioritize robust trajectory planning.

Result: Extensive experiments on Waymo Open E2E Dataset (designed for long-tail scenarios) show the frozen-encoder approach significantly outperforms models with fully fine-tuned encoders, demonstrating better generalization to novel and complex driving situations.

Conclusion: Preserving the broad knowledge of capable VLMs through frozen encoders is more effective for robust, generalizable driving performance than intensive domain-specific adaptation, offering a new pathway for vision-based models in real-world applications.
...
TL;DR: Shallow, well-regularized CNNs outperform complex models across diverse Bangladeshi vision tasks without needing large GPUs or pre-trained models.


<details>
  <summary>Details</summary>
Motivation: To provide practical, lightweight solutions for computer vision in low-resource settings like Bangladesh, where access to large GPUs and specialized models is limited.

Method: Uses well-regularized shallow convolutional neural networks as baseline models, establishing a unified benchmark across multiple Bangladeshi vision datasets.

Result: Shallow architectures achieve competitive performance across heterogeneous domains (smart-city surveillance, agricultural classification) without requiring extensive computational resources.

Conclusion: Lightweight CNNs offer practical value for real-world deployment in low-resource settings, establishing reproducible benchmarks for Bangladeshi vision applications.
...
TL;DR: A high-performance parallel ALS framework for collaborative filtering on MovieLens 32M shows constrained low-rank models outperform higher-dimensional ones, with semantic genre clusters emerging in embeddings and effective cold-start handling.
...
Method: Implemented parallelized Alternating Least Squares (ALS) framework with extensive hyperparameter optimization, using constrained low-rank models and visualizing learned embedding spaces.

Result: Constrained low-rank models significantly outperform higher dimensional counterparts in generalization (RMSE and ranking precision), with unsupervised emergence of semantic genre clusters in embeddings.

Conclusion: The approach effectively balances model complexity and performance, captures deep structural relationships from interaction data, and provides practical utility for cold-start scenarios with tunable trade-off between popularity bias and personalized affinity.
...
TL;DR: ThinkRL-Edit is a reasoning-centric RL framework for image editing that decouples visual reasoning from synthesis, expands reasoning exploration beyond denoising, and improves reward mechanisms for better instruction-faithful edits.


<details>
  <summary>Details</summary>
Motivation: Current instruction-driven image editing models have limited visual reasoning capabilities, leading to suboptimal performance on reasoning-centric edits. Existing RL approaches face challenges: limited reasoning exploration confined to denoising stochasticity, biased reward fusion, and unstable VLM-based instruction rewards.

Method: Proposes ThinkRL-Edit framework with three key innovations: (1) Chain-of-Thought-based reasoning sampling with planning and reflection stages before generation, (2) unbiased chain preference grouping strategy across reward dimensions, and (3) replacement of interval-based VLM scores with binary checklist for more precise, lower-variance rewards.
...
Conclusion: ThinkRL-Edit effectively addresses the limitations of current RL-based image editing approaches by decoupling reasoning from synthesis, expanding exploration, and improving reward mechanisms, leading to superior performance on complex reasoning-centric editing tasks.
...
TL;DR: Proposes an adaptive artifact reward model to combat reward hacking in RL post-training of text-to-image models, improving visual realism by detecting and penalizing artifacts.
...
Method: Systematically analyze reward hacking behaviors in T2I RL post-training, investigate individual contributions of aesthetic/preference rewards and prompt-image consistency rewards, then propose a lightweight adaptive artifact reward model trained on curated artifact-free/artifact-containing samples to serve as a regularizer.
...
Conclusion: Lightweight reward augmentation with an artifact detection model can effectively mitigate reward hacking in RL post-training of text-to-image models, serving as a practical safeguard to improve generation quality.
...
TL;DR: Proposes an uncertainty-guided framework for referring remote sensing image segmentation that uses spatial uncertainty maps to adaptively fuse language features and refine boundaries based on referential ambiguity.


<details>
  <summary>Details</summary>
Motivation: Existing methods use uniform fusion and refinement strategies across entire images, which introduces unnecessary linguistic perturbations in visually clear regions while failing to provide sufficient disambiguation in confused areas with extreme scale variations, dense similar distractors, and intricate boundary structures.

Method: Introduces a plug-and-play Referring Uncertainty Scorer (RUS) trained via online error-consistency supervision to predict spatial distribution of referential ambiguity. Uses this uncertainty map to guide two modules: Uncertainty-Gated Fusion (UGF) that dynamically modulates language injection strength, and Uncertainty-Driven Local Refinement (UDLR) that focuses refinement on error-prone boundaries.

Result: Extensive experiments demonstrate the method functions as a unified, plug-and-play solution that significantly improves robustness and geometric fidelity in complex remote sensing scenes without altering backbone architecture.

Conclusion: The uncertainty-guided framework effectively addresses spatial non-uniformity in cross-modal alignment for referring remote sensing image segmentation by adaptively orchestrating inference based on predicted referential ambiguity.
...
<details>
  <summary>Details</summary>
Motivation: Object hallucination remains a critical challenge in LVLMs. Existing approaches overlook visual encoding complexities, particularly visual statistical bias from Vision Encoders' Bag-of-Patches behavior under weak structural supervision, which prioritizes local texture over holistic geometry.

Method: Structure-Disrupted Contrastive Decoding (SDCD): A training-free algorithm that performs contrastive calibration of output distribution by introducing a shuffled structure-disrupted view. It penalizes tokens maintaining high confidence under this structure-less view to suppress texture-driven bias.
...
Conclusion: Visual statistical bias from patch-based encoding contributes to object hallucinations. SDCD effectively addresses this by disrupting structure to suppress texture-driven biases, improving LVLM performance without additional training.
...
Method: Uses distillation-based approach to train ML model on heterogeneous data from multiple sources (synthetic and real images). Developed robust differentiable rendering pipeline for automatic facial expression label extraction. Collected 18k diverse subjects using mobile phone + custom VR headset with extra cameras.

Result: Created a novel system that accurately tracks facial expressions in real-time from egocentric views using infrared cameras embedded in VR headset, enabling users to drive virtual character expressions without calibration.

Conclusion: The system opens new avenues for communication and expression in virtual environments with applications in video conferencing, gaming, entertainment, and remote collaboration.
...
TL;DR: G2P transfers appearance-aware attributes from 3D Gaussian Splatting to point clouds for better semantic segmentation by resolving geometric ambiguity and improving boundary localization.
...
Method: Gaussian-to-Point (G2P) transfers appearance-aware attributes from 3D Gaussian Splatting to point clouds. It addresses misalignment between optimized Gaussians and original point geometry through point-wise correspondences, leverages Gaussian opacity attributes to resolve geometric ambiguity, and uses Gaussian scale attributes for precise boundary localization.
...
TL;DR: SBWM reframes human motion prediction as latent dynamical simulation on the human body manifold, using belief-state world models to separate dynamics modeling from pose reconstruction for stable long-horizon rollouts.


<details>
  <summary>Details</summary>
Motivation: Traditional motion prediction methods suffer from compounding drift, mean-pose collapse, and poor uncertainty calibration when rolled forward beyond training, because they don't separate observation reconstruction from dynamics modeling and lack explicit representation of latent motion causes.

Method: SBWM maintains a recurrent probabilistic belief state whose evolution is learned independently of pose reconstruction, explicitly aligned with SMPL-X anatomical parameterization. This creates a structural information bottleneck forcing the latent state to capture motion dynamics, intent, and control-relevant structure rather than static geometry or noise.
...
Conclusion: Treating the human body as part of the world model's state space rather than its output fundamentally changes motion simulation and prediction, enabling stable forward simulation through belief-state world models adapted from model-based reinforcement learning.
...
TL;DR: PCNet improves thermal UAV image super-resolution using optical guidance with physics-constrained thermal conduction to avoid artifacts and preserve high-frequency details.


<details>
  <summary>Details</summary>
Motivation: Existing optical-guided thermal super-resolution methods compress optical features, losing high-frequency information and causing physically inconsistent artifacts due to ignoring imaging physics differences between modalities.

Method: Proposes PCNet with: 1) Cross-Resolution Mutual Enhancement Module (CRME) for joint thermal SR and optical-to-thermal conversion with bidirectional feature interaction; 2) Physics-Driven Thermal Conduction Module (PDTM) incorporating 2D heat conduction to model spatially-varying properties; 3) Temperature consistency loss for regional distribution and boundary gradient smoothness.

Result: Extensive experiments on VGTSR2.0 and DroneVehicle datasets show PCNet significantly outperforms state-of-the-art methods in both reconstruction quality and downstream tasks (semantic segmentation, object detection).

Conclusion: PCNet enables robust thermal UAV image super-resolution through physics-constrained optical guidance and cross-resolution mutual enhancement, addressing limitations of existing methods while maintaining physical consistency.
...
Method: Generates three augmented views per unlabeled image: one weakly augmented view and two strongly augmented views (inter-scene mixing for contextual variety and intra-scene mixing for semantic coherence), then enforces prediction consistency across these views to guide pseudolabel generation.

Result: Extensive experiments show CloudMatch achieves good performance, demonstrating effective utilization of unlabeled data and advancement in semi-supervised cloud detection.

Conclusion: CloudMatch successfully addresses the annotation cost challenge in cloud detection by leveraging unlabeled data through view-consistency learning with scene-mixing augmentations, capturing both structural diversity and contextual richness of cloud patterns.
...
TL;DR: EASLT is an emotion-aware sign language translation framework that treats facial expressions as semantic anchors rather than auxiliary information, using emotional encoding and fusion to resolve ambiguities when manual gestures are identical but meanings differ.


<details>
  <summary>Details</summary>
Motivation: Current gloss-free SLT methods focus on manual signals but overlook facial expressions, leading to semantic ambiguity when different concepts share identical manual articulations. Facial affect provides crucial semantic information that can resolve these ambiguities.

Method: EASLT introduces a dedicated emotional encoder to capture continuous affective dynamics from facial expressions, integrated via an Emotion-Aware Fusion (EAF) module that adaptively recalibrates spatio-temporal sign features based on affective context.

Result: On PHOENIX14T and CSL-Daily benchmarks, EASLT achieves state-of-the-art performance among gloss-free methods with BLEU-4 scores of 26.15 and 22.80, and BLEURT scores of 61.0 and 57.8 respectively.

Conclusion: Explicitly modeling emotion effectively decouples affective semantics from manual dynamics, significantly enhancing translation fidelity. Treating facial affect as a semantic anchor rather than auxiliary information resolves ambiguities in sign language translation.
...
TL;DR: SpatiaLoc is a cross-modal localization framework that uses text descriptions and point clouds with a coarse-to-fine strategy emphasizing spatial relationships at instance and global levels, achieving SOTA results on KITTI360Pose.
...
Conclusion: The proposed SpatiaLoc framework effectively leverages spatial relationships at both instance and global levels through a coarse-to-fine strategy, achieving superior cross-modal localization performance by combining Bezier curve modeling, frequency domain representations, and uncertainty-aware regression.
...
TL;DR: MPFT strategy with Texture-Aware Masking fine-tunes CLIP-ViT to detect AI-generated images by focusing on distributional deviations from real images, achieving state-of-the-art generalization performance with minimal training data.


<details>
  <summary>Details</summary>
Motivation: As generative models improve, detecting AI-generated images becomes crucial for combating misinformation. Existing methods using frozen CLIP models don't fully exploit the encoder's potential for distinguishing real vs. AI-generated content, especially for generalization to unseen generative models.
...
Result: Achieves up to 98.2% average accuracy on GenImage dataset and 94.6% on UniversalFakeDetect dataset, significantly outperforming existing approaches while requiring only minimal training images.

Conclusion: The MPFT strategy effectively enhances CLIP-ViT's ability to detect AI-generated images by focusing on distributional deviations rather than specific generative patterns, achieving superior generalization performance with minimal training data.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether spatial understanding in AI systems originates from visual encoders or the fundamental reasoning backbone, and to evaluate LLMs' spatial intelligence without relying on pixel-level visual input.
...
Result: SOTA LLMs show proficiency in localized semantic tasks but exhibit a significant "spatial gap" in global consistency. Explicit spatial reasoning significantly boosts performance, suggesting LLMs possess latent world-modeling potential.

Conclusion: SiT-Bench serves as a foundational resource for developing spatially-grounded LLM backbones for future VLMs and embodied agents, demonstrating that spatial intelligence can be evaluated and improved through textual reasoning approaches.
...
TL;DR: The paper introduces an environment-robust few-shot segmentation setting and benchmark (ER-FSS) to address real-world challenges like motion blur and camouflaged targets, proposing Adaptive Attention Distillation (AAD) that improves segmentation performance by 3.3-8.5% mIoU across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot segmentation research overlooks complex real-world environmental factors like illumination, background, and camera viewpoint that make test images more difficult. Models trained in lab conditions fail to meet practical deployment requirements in dynamic, challenging environments.

Method: Proposes Adaptive Attention Distillation (AAD) method that repeatedly contrasts and distills key shared semantics between support (known) and query (unknown) images to derive class-specific attention for novel categories. This helps models focus on correct targets in complex environments.

Result: AAD improves mIoU by 3.3% to 8.5% across all datasets and settings in the ER-FSS benchmark. The method demonstrates superior performance and strong generalization across eight datasets covering multiple real-world scenarios with challenging conditions.

Conclusion: The paper successfully bridges the gap between laboratory-trained models and practical deployment by introducing environment-robust FSS, establishing a comprehensive benchmark, and proposing an effective attention distillation method that significantly improves robustness in complex real-world scenarios.
...
TL;DR: A novel adaptive patching strategy with Attention U-Net for binarizing challenging stone inscriptions, achieving robust performance and zero-shot generalization across scripts.
...
Result: The novel patching mechanism significantly boosts binarization performance across classical and deep learning baselines. Despite training only on single script Indic dataset, the model exhibits strong zero-shot generalization to other Indic and non-Indic scripts.
...
TL;DR: Monocular 3D detection study shows depth backbone choice (NeWCRFs vs Depth Anything V2) and geometric fidelity matter most for pseudo-LiDAR performance, while appearance/semantic features provide only marginal gains.


<details>
  <summary>Details</summary>
Motivation: Monocular 3D object detection offers a low-cost alternative to LiDAR but suffers from accuracy issues due to metric depth estimation challenges from single images. The paper aims to systematically evaluate how depth backbones and feature engineering affect monocular pseudo-LiDAR pipelines.

Method: Systematic evaluation comparing NeWCRFs (supervised metric depth) against Depth Anything V2 Metric-Outdoor (Base) under identical pseudo-LiDAR generation and PointRCNN detection protocol on KITTI validation split. Tested point-cloud augmentations using appearance cues (grayscale intensity) and semantic cues (instance segmentation confidence), including mask-based sampling. Conducted depth-accuracy-versus-distance diagnostic using ground-truth 2D boxes.
...
Conclusion: Under off-the-shelf LiDAR detectors, depth-backbone choice and geometric fidelity are the primary factors determining monocular 3D detection performance, outweighing secondary feature injection. Coarse depth correctness doesn't fully predict strict 3D IoU performance.
...
TL;DR: A novel object classification method that eliminates the need for object alignment by sorting features from normalized boundary segments, using a bag of features approach for similarity measurement.


<details>
  <summary>Details</summary>
Motivation: Traditional object classification methods require object alignment to compute similarity, which can be computationally expensive and may not always be feasible. The paper aims to develop an alignment-free approach that maintains classification accuracy while reducing computational complexity.

Method: 1. Normalize object boundaries and segment them into approximately convex segments. 2. Sort segments in descending order of length. 3. Extract a bag of features from each segment: length, number of extreme points, area, base, and width. 4. Use these sorted features to measure similarity between image boundaries without requiring object alignment.

Result: The proposed method was tested on datasets and produced acceptable classification results, demonstrating that alignment-free classification using sorted features is feasible and effective.

Conclusion: The paper successfully presents an alignment-free object classification method that replaces traditional alignment-based similarity computation with feature sorting, achieving acceptable performance while reducing computational requirements associated with object alignment.
...
TL;DR: MFC-RFNet: A generative framework for precipitation nowcasting that combines multi-scale feature communication, spatial alignment, and rectified flow training to improve radar echo sequence prediction accuracy and resolution.
...
Method: MFC-RFNet integrates multi-scale communication with guided feature fusion using: 1) Wavelet-Guided Skip Connection (WGSC) to preserve high-frequency components, 2) Feature Communication Module (FCM) for bidirectional cross-scale interaction, 3) Condition-Guided Spatial Transform Fusion (CGSTF) to align shallow features using spatial transforms from conditioning echoes, 4) Rectified flow training for near-linear probability-flow trajectories enabling few-step sampling, and 5) Lightweight Vision-RWKV blocks at key positions to capture long-range dependencies with moderate compute.
...
TL;DR: CrackSegFlow is a controllable flow-matching framework that generates photorealistic crack images from binary masks, improving crack segmentation performance across domains.


<details>
  <summary>Details</summary>
Motivation: Automated crack segmentation faces challenges due to scarce pixel-level labels and severe domain shifts across different sensors, illumination conditions, textures, and annotation conventions, limiting practical deployment.

Method: A two-stage flow-matching framework: 1) generates photorealistic crack images from binary masks using topology-preserving mask injection with boundary-gated modulation, 2) synthesizes crack masks with explicit control over crack coverage. Also injects crack masks into crack-free backgrounds to diversify illumination and reduce false positives.

Result: Consistent improvements across five benchmarks: in-domain performance improved by 5.37 mIoU and 5.13 F1 on average; cross-domain synthesis yielded average gains of 13.12 mIoU and 14.82 F1. Faster deterministic sampling than diffusion-based methods with better fidelity and mask-image alignment.

Conclusion: CrackSegFlow effectively addresses domain shift and label scarcity in crack segmentation through controllable synthesis, releasing CSF-50K dataset for large-scale benchmarking.
...
TL;DR: VideoMemory is an entity-centric framework for consistent narrative video generation that uses a Dynamic Memory Bank to preserve character, prop, and environment identity across multiple shots and temporal gaps.
...
Method: Uses a multi-agent system to decompose narratives into shots, retrieves entity representations from a Dynamic Memory Bank (storing visual and semantic descriptors), and synthesizes keyframes/videos conditioned on retrieved states. The memory bank is updated after each shot to reflect story-driven changes while preserving identity.

Result: Achieves strong entity-level coherence and high perceptual quality across diverse narrative sequences, as demonstrated on a 54-case multi-shot consistency benchmark covering character-, prop-, and background-persistent scenarios.

Conclusion: VideoMemory's retrieval-update mechanism enables consistent portrayal of entities across distant shots and supports coherent long-form video generation, addressing a central challenge in narrative video creation.
...
TL;DR: MGPC is a multimodal point cloud completion framework using point clouds, RGB images, and text with modality dropout, transformer fusion, and progressive generation to improve generalization to real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud completion methods (3D CNN-based, point-based, Transformer-based) have limited generalization to novel objects and real-world scenarios due to modality limitations, scalability issues, and generative capacity constraints.

Method: MGPC integrates point clouds, RGB images, and text in unified architecture with modality dropout strategy, Transformer-based fusion module, and progressive generator. Also developed automatic data generation pipeline to create MGPC-1M benchmark with 1,000+ categories and 1M training pairs.
...
Conclusion: MGPC provides a generalizable multimodal framework for point cloud completion that addresses limitations of existing methods and demonstrates superior performance and generalization capability in real-world scenarios.
...
TL;DR: PhysVideoGenerator embeds learnable physics priors into video generation to address unnatural physical dynamics in current models, using a predictor network to extract physics features from diffusion latents and inject them into a DiT-based generator.


<details>
  <summary>Details</summary>
Motivation: Current video generation models produce aesthetically pleasing videos but struggle with realistic physics dynamics, resulting in artifacts like unnatural object collisions, inconsistent gravity, and temporal flickering. There's a need to incorporate physical realism into generative video models.

Method: Proposes PhysVideoGenerator framework with PredictorP network that regresses high-level physical features from pre-trained V-JEPA 2 directly from noisy diffusion latents. These physics tokens are injected into temporal attention layers of a DiT-based generator (Latte) via cross-attention mechanism.

Result: Demonstrates technical feasibility: diffusion latents contain sufficient information to recover V-JEPA 2 physical representations, and multi-task optimization remains stable during training. Establishes foundation for future large-scale evaluation of physics-aware generative models.

Conclusion: The work successfully shows that embedding learnable physics priors into video generation is technically feasible, addressing key physical realism issues while maintaining training stability, paving the way for more physically accurate video generation models.
...
TL;DR: Using 2D point tracks from CoTracker as motion cues improves egocentric action recognition without needing hand/object detection or full video sequences.
...
Method: Track randomly sampled image points across video frames using CoTracker, then feed resulting trajectories along with image frames into a Transformer-based recognition model. Can work with just initial frame and its point tracks.

Result: Integrating 2D point tracks consistently enhances performance compared to same model without motion information. Achieves notable gains even with only initial frame and point tracks, without full video sequence.
...
Method: First construct BREATH dataset (largest in-vivo endoscopic localization dataset). Then propose BREATH-VL hybrid framework integrating semantic cues from VLMs with geometric information from vision-based registration. Introduce lightweight context-learning mechanism using linguistic prompts to encode motion history for efficient temporal reasoning.

Result: Outperforms state-of-the-art vision-only localization methods in accuracy and generalization, reducing translational error by 25.5% compared to best-performing baseline while achieving competitive computational latency.

Conclusion: The hybrid vision-language approach successfully leverages complementary strengths of semantic understanding from VLMs and precise geometric alignment from registration methods, enabling robust 6-DoF endoscopic camera localization in challenging surgical environments.
...
TL;DR: DA3 uses domain adaptation with minimal unlabeled real data to bridge simulation-reality gap for active alignment in optical systems, achieving near-labeled-data performance with 98.7% less data collection.


<details>
  <summary>Details</summary>
Motivation: Digital-twin pipelines using optical simulation can generate large-scale labeled data for automated optical assembly, but simulation-reality domain gaps limit generalization of simulation-trained models to real-world conditions.

Method: Propose Domain Adaptive Active Alignment (DA3) that augments simulation baseline with minimal unlabeled real-world images. Uses autoregressive domain transformation generator and adversarial feature alignment for self-supervised learning to extract domain-invariant degradation features for robust misalignment prediction.
...
TL;DR: CSMCIR is a unified framework for composed image retrieval that addresses representation space fragmentation through symmetric architecture, multi-level chain-of-thought prompting, and dynamic memory banks, achieving state-of-the-art performance with superior training efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing CIR methods suffer from representation space fragmentation where queries (image+text) and targets (images) use different encoders, creating misaligned representation spaces that limit retrieval performance. This architectural asymmetry results in three separate feature clusters that need post-hoc alignment.

Method: 1) Multi-level Chain-of-Thought (MCoT) prompting for MLLMs to generate discriminative captions for target images, establishing modal symmetry. 2) Symmetric dual-tower architecture with identical shared-parameter Q-Former for both query and target encoding. 3) Entropy-based, temporally dynamic Memory Bank for high-quality negative samples aligned with evolving model state.

Result: Extensive experiments on four benchmark datasets demonstrate state-of-the-art performance with superior training efficiency. Comprehensive ablation studies validate the effectiveness of each proposed component.

Conclusion: CSMCIR successfully addresses representation space fragmentation in CIR through unified symmetric architecture and complementary components, achieving better alignment between heterogeneous modalities and significantly improving retrieval performance.
...
TL;DR: MATANet: A novel model for fine-grained marine species classification that uses multi-context environmental attention and taxonomic hierarchy encoding to improve classification accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for fine-grained marine animal classification overlook contextual interactions from the surrounding environment and insufficiently incorporate the hierarchical structure of marine biological taxonomy, limiting their effectiveness for ecological and conservation applications.

Method: Proposes MATANet with two key components: 1) Multi-Context Environmental Attention Module (MCEAM) that learns relationships between regions of interest and their surrounding environments, and 2) Hierarchical Separation-Induced Learning Module (HSLM) that encodes taxonomic hierarchy into the feature space. The model combines instance features, environmental context, and taxonomic structure.

Result: Experiments on FathomNet2025, FAIR1M, and LifeCLEF2015-Fish datasets demonstrate state-of-the-art performance, showing the effectiveness of incorporating environmental context and taxonomic hierarchy.

Conclusion: MATANet successfully addresses limitations of existing methods by mimicking expert strategies that use taxonomy and environmental context to interpret ambiguous features, providing a robust solution for fine-grained marine species classification with applications in ecology and conservation.
...
TL;DR: RadDiff is a multimodal AI system for radiologist-style comparative analysis of paired radiology studies, outperforming baselines on a new benchmark and enabling diverse clinical applications.


<details>
  <summary>Details</summary>
Motivation: Understanding differences between radiology image sets is crucial for clinical insights and interpreting medical AI systems, but current methods lack systematic approaches for comparative reasoning.

Method: RadDiff uses a proposer-ranker framework with four innovations: medical knowledge injection via domain-adapted VLMs, multimodal reasoning integrating images and reports, iterative hypothesis refinement, and targeted visual search with localization.

Result: Achieves 47% accuracy on RadDiffBench (50% with ground-truth reports), significantly outperforming VisDiff baseline. Demonstrates versatility across COVID-19 phenotype comparison, racial subgroup analysis, and survival-related feature discovery.
...
TL;DR: Introduces HyperCOD, the first large-scale hyperspectral camouflaged object detection benchmark, and HSC-SAM, a novel adaptation of SAM for hyperspectral data that achieves state-of-the-art performance.
...
Method: Proposes HyperCOD benchmark (350 high-resolution hyperspectral images) and HSC-SAM model that reformulates hyperspectral images by decoupling into spatial map (fed to SAM's encoder) and spectral saliency map (serves as adaptive prompt), bridging the modality gap.

Result: HSC-SAM achieves new state-of-the-art on HyperCOD benchmark and demonstrates robust generalization to other public HSI datasets.

Conclusion: HyperCOD dataset and HSC-SAM baseline provide a strong foundation for future research in hyperspectral camouflaged object detection, addressing the critical lack of benchmarks in this emerging field.
...
TL;DR: I2E introduces a "Decompose-then-Action" paradigm for compositional image editing using object-level decomposition and physics-aware agents.


<details>
  <summary>Details</summary>
Motivation: Existing text-guided image editing methods struggle with compositional editing tasks due to implicit coupling of planning/execution, lack of object-level control, and reliance on unstructured pixel-centric modeling.

Method: I2E uses a Decomposer to transform images into discrete object layers, then employs a physics-aware Vision-Language-Action Agent with Chain-of-Thought reasoning to parse instructions into atomic actions.

Result: I2E significantly outperforms state-of-the-art methods on I2E-Bench and public benchmarks for complex compositional instructions, physical plausibility, and multi-turn editing stability.

Conclusion: The proposed "Decompose-then-Action" paradigm effectively addresses limitations of current pixel-level inpainting methods by introducing structured object-level control and explicit planning for compositional image editing.
...
TL;DR: Proposes Masked Video Prediction (MVP) as a novel post-training objective for VideoLLMs to enhance temporal reasoning and causal understanding, addressing limitations of current approaches that focus mainly on holistic content understanding.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning based post-training for VideoLLMs focuses on visual-semantic tasks like captioning and VideoQA, which enhance perception but lack explicit supervision for temporal coherence and inter-frame correlations. This limits models' ability to capture intricate dynamics and fine-grained visual causality.

Method: Introduces Masked Video Prediction (MVP) objective requiring models to reconstruct masked continuous video segments from distractors. Uses scalable data synthesis pipeline to transform arbitrary video corpora into MVP training samples. Employs Group Relative Policy Optimization (GRPO) with fine-grained reward function to enhance understanding of video context and temporal properties.
...
Conclusion: MVP provides an effective post-training approach that explicitly addresses temporal coherence and causal understanding in VideoLLMs, bridging a critical gap in current reinforcement learning based paradigms.
...
TL;DR: The paper presents techniques for generating synthetic training data for AI object detection in agriculture when CAD models are unavailable, showing that 3D models from scanning or image-to-3D approaches can effectively substitute CAD files.


<details>
  <summary>Details</summary>
Motivation: In agricultural manufacturing, AI computer vision systems need large training datasets, but real data is costly to acquire and annotate. Unlike other industries, agriculture lacks readily available CAD models for synthetic data generation, creating a challenge for training object detection models.

Method: The paper presents different techniques for substituting CAD files to create synthetic datasets, including using highly representative 3D models acquired by scanning or image-to-3D approaches. These synthetic datasets are used to train AI object detection models for separating stones and potatoes in bin picking environments, with finetuning on small real datasets.

Result: The study demonstrates that synthetic data generated from scanned or image-to-3D 3D models can effectively train object detection models. Finetuning on small real datasets significantly improves model performance, and can achieve similar performance even when less representative models are used.

Conclusion: Synthetic data generation using 3D models from scanning or image-to-3D approaches provides a viable solution for training AI object detection models in agriculture where CAD models are unavailable, especially when combined with finetuning on small real datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current data-aware code augmentation relies on heuristic design or brute-force approaches, which are inefficient and don't leverage LLMs' potential for autonomous optimization based on empirical performance.

Method: Fine-tune LLMs with Low-Rank Adaptation on 6,000+ empirically evaluated PyTorch augmentation functions annotated by downstream accuracy. Use pairwise performance ordering (better-worse transformations) for alignment without reinforcement learning or reward models.

Result: Achieves up to 600x fewer evaluated candidates than brute-force discovery while maintaining competitive peak accuracy. Direct prompting outperforms Chain-of-Thought for performance-critical code tasks. Model internalizes semantic performance cues rather than memorizing syntax.

Conclusion: LLMs can exhibit task-level reasoning through non-textual feedback loops, bypassing explicit symbolic rewards, enabling autonomous engineering of optimal code transformations based on empirical performance.
...
TL;DR: EvalBlocks is a modular framework built on Snakemake for efficient, reproducible evaluation of medical imaging foundation models, reducing manual tracking burden and enabling faster iteration.


<details>
  <summary>Details</summary>
Motivation: Researchers developing medical imaging foundation models face burdensome manual workflows for tracking experiments, design choices, and performance effects, which are slow and error-prone.

Method: Built on Snakemake, EvalBlocks provides a modular, plug-and-play framework with seamless integration of new datasets, foundation models, aggregation methods, and evaluation strategies. Features include central tracking, reproducibility with single commands, efficient caching, and parallel execution.

Result: Demonstrated on five state-of-the-art foundation models and three medical imaging classification tasks, EvalBlocks successfully streamlines model evaluation and enables faster iteration.

Conclusion: EvalBlocks reduces evaluation logistics burden, allowing researchers to focus on model innovation rather than manual tracking, and is released as open source software.
...
TL;DR: IDESplat improves 3D Gaussian Splatting by iteratively boosting depth probability estimation through cascaded warp operations, enabling more accurate Gaussian mean prediction for scene reconstruction.
...
Method: Proposes IDESplat with two key components: 1) Depth Probability Boosting Unit (DPBU) that integrates epipolar attention maps from cascaded warp operations multiplicatively to eliminate single-warp instability, and 2) iterative depth estimation by stacking multiple DPBUs to progressively refine depth candidates and probability estimates.

Result: Achieves state-of-the-art performance on RealEstate10K, ACID, and DL3DV datasets with real-time efficiency. Outperforms DepthSplat by 0.33 dB PSNR on RE10K using only 10.7% parameters and 70% memory. Shows strong generalization with 2.95 dB PSNR improvement over DepthSplat on DTU cross-dataset experiments.

Conclusion: IDESplat's iterative depth probability boosting approach significantly improves Gaussian mean prediction accuracy, leading to superior 3D scene reconstruction quality, better generalization, and more efficient performance compared to existing methods.
...
TL;DR: MDENeRF: An iterative framework that refines monocular depth estimates using Neural Radiance Fields (NeRFs) with uncertainty modeling to add fine geometric details while maintaining global structure.
...
<details>
  <summary>Details</summary>
Motivation: Traditional manual flood damage surveys are slow and biased, while current satellite methods face limitations from cloud cover and low spatial resolution, creating a need for better automated crop damage assessment solutions.

Method: FLNet is a novel deep learning architecture that first applies super-resolution to enhance Sentinel-2 satellite images from 10m to 3m spatial resolution, then classifies crop damage levels using the enhanced imagery.

Result: On the Bihar Flood Impacted Croplands Dataset (BFCD-22), FLNet improved the critical "Full Damage" F1-score from 0.83 to 0.89, nearly matching the 0.89 score of commercial high-resolution imagery.

Conclusion: FLNet provides a cost-effective, scalable solution for automated crop damage assessment that could enable nationwide transition from manual to high-fidelity damage assessment systems.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning models for white blood cell morphology evaluation act as black boxes, limiting clinical trust and adoption. There's a need for interpretable models that can provide transparent morphological descriptions to support leukemia diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models for UI grounding tokenize high-resolution screenshots into thousands of visual tokens, causing significant computational overhead and attention dilution, unlike humans who focus on relevant UI regions.

Method: FocusUI addresses two challenges: (1) Eliminates redundant tokens by fusing instruction-conditioned scores with rule-based UI-graph scores to select distinct, relevant patches; (2) Preserves positional continuity using PosPad strategy that compresses dropped token sequences into special markers at their last index.

Result: On ScreenSpot-Pro benchmark, FocusUI-7B achieves 3.7% improvement over GUI-Actor-7B. With only 30% visual token retention, it drops only 3.2% in accuracy while achieving 1.44x faster inference and 17% lower peak GPU memory.

Conclusion: FocusUI pioneers efficient UI grounding by effectively reducing visual token overhead while maintaining positional information, demonstrating superior performance over GUI-specific baselines across four grounding benchmarks.
...
TL;DR: ResTok introduces a hierarchical residual visual tokenizer for autoregressive image generation that restores vision-specific priors, achieving better performance with fewer sampling steps.


<details>
  <summary>Details</summary>
Motivation: Current 1D visual tokenizers follow language modeling principles, treating visual data as flat sequential tokens and overlooking key vision properties like hierarchical and residual network designs essential for convergence and efficiency in visual models.

Method: Proposes Residual Tokenizer (ResTok) that builds hierarchical residuals for both image tokens and latent tokens, enabling cross-level feature fusion while preventing information overlap through semantic residuals. Also introduces hierarchical AR generator that predicts entire levels of latent tokens at once to reduce sampling steps.

Result: Achieves gFID of 2.34 on ImageNet-256 with only 9 sampling steps, demonstrating significant improvement in AR image generation performance.

Conclusion: Restoring hierarchical residual priors in visual tokenization significantly improves autoregressive image generation by bringing vision-specific properties back to the modeling process, enabling better representations and more efficient generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing motion synthesis methods either ignore hand motions or generate full-body motions only for narrow tasks. There's a lack of large-scale datasets capturing diverse full-body motion with detailed hand articulation, as current datasets are either limited in scale/diversity or focus on body/hand separately.

Method: Curate and unify existing hand motion datasets with large-scale body motion data to generate full-body sequences. Propose FUSION, a diffusion-based unconditional full-body motion prior that jointly models body and hand motion using pose-based representation. Develop optimization pipeline to refine diffusion model's latent space for task-specific motions.

Result: FUSION surpasses state-of-the-art skeletal control models on Keypoint Tracking task in HumanML3D dataset and achieves superior motion naturalness. Successfully demonstrates two applications: (1) generating detailed full-body motion with fingers during object interaction, and (2) generating Self-Interaction motions using LLM to transform natural language into motion constraints.

Conclusion: FUSION enables precise control over hand motion while maintaining plausible full-body coordination, going beyond typical motion prior uses. The method addresses the critical gap in full-body motion synthesis with detailed hand articulation.
...
TL;DR: PosterVerse is a full-workflow commercial poster generation system that automates design through LLM-based blueprint creation, diffusion model background generation, and MLLM-powered HTML layout rendering, using a novel HTML-based dataset called PosterDNA.


<details>
  <summary>Details</summary>
Motivation: Current automated poster generation systems have limitations including incomplete workflows, poor text rendering accuracy, and insufficient flexibility for commercial applications. There's a need for a system that can produce commercial-grade posters with both aesthetic appeal and precise information delivery.

Method: Three-stage pipeline: (1) Blueprint creation using fine-tuned LLMs to extract design elements from user requirements, (2) Graphical background generation via customized diffusion models, (3) Unified layout-text rendering with MLLM-powered HTML engine for high text accuracy and customization. Also introduces PosterDNA dataset with HTML typography files for training.

Result: PosterVerse consistently produces commercial-grade posters with appealing visuals, accurate text alignment, and customizable layouts. The system fundamentally solves challenges of rendering small and high-density text through HTML-based rendering.

Conclusion: PosterVerse is a promising solution for automating commercial poster design, offering a full workflow approach with high text accuracy and flexibility. The introduction of PosterDNA dataset with HTML typography enables scalable text rendering for Chinese poster generation.
...
TL;DR: Padé neurons (Paons) are a novel non-linear neuron model based on Padé approximants that provide stronger non-linearity than traditional neurons, can replace any existing neuron model, and achieve better performance with fewer layers in various neural network tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional McCulloch-Pitts neurons use linear models with point-wise non-linear activations, which have limited non-linearity. Existing non-linear neuron models offer improvements but still have limitations. The authors aim to develop a more powerful and flexible non-linear neuron model that can outperform existing approaches.

Method: Introduce Padé neurons (Paons) inspired by Padé approximants. Each Paon learns a different non-linear function of its inputs. The model can replace any existing neuron model in any network architecture. Experiments involve replacing classic neurons in ResNet-based models for image super-resolution, compression, and classification tasks.

Result: Neural models built with Paons provide better or equal performance than their classic counterparts with a smaller number of layers. Paons demonstrate stronger non-linearity in fewer layers compared to piecewise linear approximation and include all previously proposed neuron models as special cases.

Conclusion: Padé neurons offer a superior non-linear neuron model with diverse non-linearity and layer efficiency, capable of replacing existing neuron models while improving performance with reduced architectural complexity. The approach is validated across multiple computer vision tasks.
...
TL;DR: REACT is a frame-level reward model for evaluating structural distortions in generative videos, trained on a large human-annotated dataset with a two-stage framework and dynamic sampling during inference.
...
TL;DR: Annotation-efficient segmentation approach for wind turbine blades using region classification instead of pixel-wise methods, achieving state-of-the-art accuracy with strong cross-site generalization.
...
Method: Reframes segmentation as binary region classification using unsupervised Modular Adaptive Region Growing with Adaptive Thresholding and Region Merging, enhanced by RegionMix augmentation for better generalization.

Result: Achieves state-of-the-art segmentation accuracy and demonstrates strong cross-site generalization by consistently segmenting turbine blades across different windfarms.

Conclusion: The proposed annotation-efficient approach provides a scalable solution for wind turbine blade segmentation that reduces annotation burden while maintaining high accuracy and generalization across different sites.
...
<details>
  <summary>Details</summary>
Motivation: Existing DPO methods for text-to-video models are inefficient (relying on multi-sample ranking and task-specific critic models) and provide ambiguous global supervision. There's a need for more efficient and fine-grained alignment approaches.

Method: 1) Automated pipeline to collect preference pairs using real videos as positives and generating negatives by locally corrupting them with random spatio-temporal masks, then restoring only masked regions with frozen base model. 2) Region-aware DPO loss that restricts preference learning to corrupted areas for rapid convergence. 3) Single inference per prompt eliminates need for external critics or manual annotation.

Result: LocalDPO consistently improves video fidelity, temporal coherence and human preference scores over other post-training approaches on Wan2.1 and CogVideoX datasets.

Conclusion: LocalDPO establishes a more efficient and fine-grained paradigm for video generator alignment by focusing on spatio-temporal region-level optimization rather than global supervision.
...
TL;DR: LMMs suffer from "textual inertia" - once they hallucinate text in reasoning chains, they blindly follow it despite visual evidence. Proposed LogicGraph Perturbation Protocol reveals <10% self-correction rates, and introduced Active Visual-Context Refinement improves robustness.


<details>
  <summary>Details</summary>
Motivation: Large Multimodal Models show impressive video reasoning via Chain-of-Thought, but their reasoning chains lack robustness. The paper identifies "textual inertia" - models blindly adhere to erroneous text while ignoring conflicting visual evidence, questioning the reliability of their reasoning.
...
Conclusion: Textual inertia is a critical failure mode in LMM reasoning. The LogicGraph Perturbation Protocol reveals severe self-correction deficiencies, while Active Visual-Context Refinement provides an effective training-free solution to improve reasoning robustness by enforcing visual verification and context refinement.
...
Conclusion: Gen3R successfully bridges reconstruction and generative models for 3D scene generation, producing high-quality results and showing that tight coupling of these approaches provides mutual benefits for both reconstruction robustness and generative quality.
...
TL;DR: GeoReason framework enhances RS-VLM cognitive reliability by synchronizing internal reasoning with final decisions through logic-driven dataset and two-stage training with consistency-aware reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Current RS-VLMs suffer from logical hallucinations where correct answers come from flawed reasoning or positional shortcuts, undermining reliability in strategic spatial decision-making. There's a need to transition from perception-centric recognition to high-level deductive reasoning.
...
TL;DR: Proposes PIMC, a multimodal self-supervised approach using 2D recurrence plots from pixel time series and remote sensing imagery for Earth observation tasks, outperforming SOTA methods.
...
Method: 1) Generate 2D recurrence plots from pixel-based vegetation index time series (NDVI, EVI, SAVI) instead of raw values. 2) Introduce PIxel-wise Multimodal Contrastive (PIMC) - multimodal self-supervision that produces encoders using both 2D pixel time series representations and remote sensing imagery.

Result: 2D representations significantly enhance SITS feature extraction; contrastive learning improves representation quality for both pixel time series and RSI. Outperforms SOTA methods on three downstream tasks: pixel-level forecasting/classification (PASTIS dataset) and land cover classification (EuroSAT dataset).

Conclusion: Multimodal method establishes robust self-supervision framework for processing SITS and RSI, showing superior performance in Earth observation tasks. Code is available.
...
TL;DR: Klear introduces a comprehensive audio-video joint generation framework addressing synchronization, alignment, and quality issues through unified architecture, progressive training, and novel dense-caption dataset.


<details>
  <summary>Details</summary>
Motivation: Current audio-video generation methods suffer from audio-visual asynchrony, poor lip-speech alignment, unimodal degradation due to weak correspondence modeling, limited generalization, and scarce high-quality dense-caption data.

Method: Three-axis approach: 1) Single-tower architecture with unified DiT blocks and Omni-Full Attention for tight alignment; 2) Progressive multitask training with random modality masking and multistage curriculum; 3) Novel large-scale audio-video dataset with automated pipeline for dense-caption triplets.

Result: Klear achieves high-fidelity, semantically/temporally aligned generation in joint/unimodal settings, robust generalization to out-of-distribution scenarios, substantially outperforms prior methods, and reaches performance comparable to Veo 3.
...
TL;DR: Diffusion-DRF introduces a differentiable reward flow using frozen VLMs as training-free critics to fine-tune video diffusion models, eliminating need for human annotations or learned reward models while preventing reward hacking.


<details>
  <summary>Details</summary>
Motivation: Current DPO methods for T2V generation rely on non-differentiable preference signals from human annotations or learned reward models, which makes training label-intensive, bias-prone, easy-to-game, and leads to reward hacking and unstable training.

Method: Uses frozen off-the-shelf Vision-Language Model as training-free critic, directly backpropagates VLM feedback through diffusion denoising chain, converts logit-level responses to token-aware gradients, employs automated aspect-structured prompting pipeline for multi-dimensional VLM feedback, and uses gradient checkpointing for efficient updates through final denoising steps.
...
Conclusion: Diffusion-DRF provides an effective alternative to current DPO approaches by leveraging frozen VLMs as differentiable critics, addressing key limitations of existing methods while being more efficient and generalizable.
...
<details>
  <summary>Details</summary>
Motivation: Recent deep rPPG models improve robustness but increase computational cost and parameters; attention-based temporal modeling has quadratic scaling with temporal length. Need for lightweight, efficient temporal modeling.

Method: Proposes ToTMNet with FFT-accelerated Toeplitz temporal mixing layer replacing attention. Uses circulant embedding and FFT-based convolution for near-linear time. Combines local depthwise temporal convolution with gated global Toeplitz mixing.

Result: Achieves 1.055 bpm MAE (Pearson 0.996) on UBFC-rPPG intra-dataset, and 1.582 bpm MAE (Pearson 0.994) in synthetic-to-real transfer (SCAMPS to UBFC-rPPG). Only 63k parameters. Gating mechanism crucial for domain shift.

Conclusion: Toeplitz-structured temporal mixing is practical, efficient alternative to attention for rPPG, enabling strong performance with compact design. Limitation: only two datasets tested.
...
TL;DR: 2D image-based localization augmented with depth maps achieves state-of-the-art accuracy while maintaining easy maintenance and storage efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods face trade-offs: 2D image-based methods are easy to build/maintain but lack geometric reasoning, while 3D structure-based methods are accurate but require centralized reconstruction and are hard to update.

Method: Augment 2D images with estimated depth maps to capture geometric structure, use dense matchers for effective matching, implement compact compression and GPU-accelerated LO-RANSAC for efficiency.
...
Conclusion: Depth-augmented 2D image representation provides the best of both worlds: easy maintenance like 2D methods with high accuracy approaching 3D methods, plus storage and computational efficiency.
...
TL;DR: CHORD is a universal generative pipeline for synthesizing 4D dynamic scenes by distilling Lagrangian motion information from 2D videos using video generative models.


<details>
  <summary>Details</summary>
Motivation: Traditional rule-based graphics pipelines for creating dynamic scenes are labor-intensive and category-specific, while learning-based methods require large datasets that may not cover all object categories of interest.

Method: Proposes a distillation-based pipeline that extracts Lagrangian motion information from Eulerian representations in 2D videos, leveraging the universality of video generative models.

Result: Demonstrates effectiveness in generating diverse multi-body 4D dynamics, shows advantages over existing methods, and applies to robotics manipulation policy generation.

Conclusion: CHORD provides a universal, versatile, category-agnostic approach to generating 4D dynamic scenes that overcomes limitations of traditional graphics pipelines and data-hungry learning methods.
...
### [182] [Content vs. Form: What Drives the Writing Score Gap Across Socioeconomic Backgrounds? A Generated Panel Approach](https://arxiv.org/abs/2601.03469)
*Nadav Kunievsky,Pedro Pertusi*

Main category: econ.EM

TL;DR: Using LLMs to rewrite essays with controlled style variations reveals that 69% of SES writing score gaps come from content quality differences, 26% from style differences, and 5% from evaluation standards.


<details>
  <summary>Details</summary>
Motivation: Socioeconomic status (SES) gaps in test scores persist and affect life outcomes. In writing assessments, scores reflect both content (what students say) and style (how they say it), making it difficult to disentangle whether SES gaps stem from differences in underlying knowledge or expressive skills.

Method: Analyzed persuasive essays from U.S. middle/high school students. Used large language models to generate multiple stylistic variants of each essay while preserving content, creating a "generated panel" with controlled within-essay style variation. This allowed decomposition of SES score gaps into content and style contributions.

Result: Found SES gap of 0.67 points on 1-6 scale. Content quality differences accounted for 69% of the gap, style differences for 26%, and differences in evaluation standards across SES groups for 5%. Patterns were stable across demographic subgroups and writing tasks.

Conclusion: Most SES writing score gaps stem from differences in content quality rather than expressive style. The method demonstrates how LLMs can generate controlled variation in observational data to disentangle otherwise entangled factors in social science research.
...
### [183] [Listen to Rhythm, Choose Movements: Autoregressive Multimodal Dance Generation via Diffusion and Mamba with Decoupled Dance Dataset](https://arxiv.org/abs/2601.03323)
*Oran Duan,Yinghua Shen,Yingzhu Lv,Luyang Jie,Yaxin Liu,Qiong Wu*

Main category: cs.CR

TL;DR: LRCM is a multimodal diffusion framework for dance motion generation that supports diverse inputs (audio, text) and autoregressive synthesis for long sequences, addressing issues of coarse semantic control and poor coherence in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current dance motion generation methods suffer from coarse semantic control and poor coherence in long sequences. The authors aim to create a more flexible system that can handle diverse input modalities while maintaining smooth, coherent long-duration motion generation.

Method: The framework uses a feature decoupling paradigm on dance datasets, separating motion capture data, audio rhythm, and professionally annotated global/local text descriptions. It employs a diffusion architecture with audio-latent Conformer, text-latent Cross-Conformer, and a Motion Temporal Mamba Module (MTMM) for autoregressive synthesis.

Result: Experimental results show strong performance in both functional capability and quantitative metrics, demonstrating notable potential in multimodal input scenarios and extended sequence generation.

Conclusion: LRCM successfully addresses limitations in current dance motion generation by providing better semantic control and coherence in long sequences through multimodal guidance and autoregressive synthesis, with plans to release codebase, dataset, and pretrained models.
...
### [184] [How Real is Your Jailbreak? Fine-grained Jailbreak Evaluation with Anchored Reference](https://arxiv.org/abs/2601.03288)
*Songyang Liu,Chaozhuo Li,Rui Pu,Litian Zhang,Chenxu Wang,Zejian Chen,Yuting Zhang,Yiming Hei*

Main category: cs.CR

TL;DR: FJAR is a fine-grained jailbreak evaluation framework that uses anchored references and five response categories to provide more accurate assessment of LLM safety attacks.


<details>
  <summary>Details</summary>
Motivation: Current jailbreak evaluation methods are too coarse, focusing mainly on harmfulness and leading to substantial overestimation of attack success. There's a need for more precise evaluation that can identify different types of jailbreak failures.

Method: 1) Categorizes jailbreak responses into five fine-grained categories (Rejective, Irrelevant, Unhelpful, Incorrect, Successful) based on how well they address malicious intent. 2) Introduces harmless tree decomposition to construct anchored references by breaking down original queries. 3) Uses these references to guide evaluators in determining if responses genuinely fulfill original queries.

Result: Extensive experiments show FJAR achieves the highest alignment with human judgment and effectively identifies root causes of jailbreak failures, providing actionable guidance for improving attack strategies.

Conclusion: FJAR provides a more accurate and fine-grained evaluation framework for jailbreak attacks that better aligns with human judgment and offers practical insights for improving LLM safety.
...
### [185] [SoK: Privacy Risks and Mitigations in Retrieval-Augmented Generation Systems](https://arxiv.org/abs/2601.03979)
*Andreea-Elena Bodea,Stephen Meisenbacher,Alexandra Klymenko,Florian Matthes*

Main category: cs.CR

TL;DR: This paper conducts a systematic literature review on privacy risks in Retrieval-Augmented Generation (RAG) systems, creating a taxonomy of risks and mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: The proliferation of RAG systems combining LLMs with domain-specific knowledge bases has raised significant privacy concerns, particularly when handling sensitive information. There's a need to systematically survey and unify existing research on privacy risks in RAG.

Method: Conducted a systematic literature review of RAG works addressing privacy, systematizing findings into comprehensive privacy risks, mitigation techniques, and evaluation strategies. Created two primary artifacts: a Taxonomy of RAG Privacy Risks and a RAG Privacy Process Diagram.

Result: The paper provides the first systematization of privacy risks and mitigations in RAG systems, uncovering important considerations for mitigating privacy risks and assessing the current maturity of proposed mitigation techniques.

Conclusion: This work contributes to the study of privacy in RAG by establishing foundational frameworks for understanding, measuring, and mitigating privacy risks, while highlighting gaps in current mitigation maturity.
...
### [186] [PointWorld: Scaling 3D World Models for In-The-Wild Robotic Manipulation](https://arxiv.org/abs/2601.03782)
*Wenlong Huang,Yu-Wei Chao,Arsalan Mousavian,Ming-Yu Liu,Dieter Fox,Kaichun Mo,Li Fei-Fei*

Main category: cs.RO

TL;DR: PointWorld is a large pre-trained 3D world model that predicts 3D point flow responses to robot actions from RGB-D images, enabling zero-shot robotic manipulation across various tasks.


<details>
  <summary>Details</summary>
Motivation: Robots need human-like ability to anticipate 3D world responses to their actions from visual input, but current approaches are limited by embodiment-specific action spaces and lack of unified 3D representation.

Method: Represents state and action as 3D point flows in shared space; uses large-scale dataset (2M trajectories, 500 hours) from real/simulated manipulation; trains model to forecast per-pixel 3D displacements given RGB-D images and action commands.

Result: Single pre-trained model enables real-world Franka robot to perform rigid-body pushing, deformable/articulated object manipulation, and tool use from single in-the-wild image, with real-time (0.1s) inference for MPC integration.

Conclusion: PointWorld demonstrates unified 3D world modeling with point flow representation enables zero-shot transfer across embodiments and tasks, establishing design principles for scalable 3D world models.
...
### [187] [CLAP: Contrastive Latent Action Pretraining for Learning Vision-Language-Action Models from Human Videos](https://arxiv.org/abs/2601.04061)
...
<details>
  <summary>Details</summary>
Motivation: Generalist Vision-Language-Action models lack sufficient robotic data, while existing Latent Action Models suffer from visual entanglement when trying to leverage abundant human video demonstrations.

Method: Contrastive Latent Action Pretraining (CLAP) aligns visual latent space from videos with proprioceptive latent space from robot trajectories using contrastive learning, mapping video transitions to quantized executable actions. Includes CLAP-NTP (autoregressive model) and CLAP-RF (Rectified Flow-based policy), plus Knowledge Matching regularization to prevent catastrophic forgetting.

Result: CLAP significantly outperforms strong baselines, enabling effective transfer of manipulation skills from human videos to robotic execution.

Conclusion: CLAP successfully addresses the visual entanglement problem in existing methods and enables practical skill transfer from human demonstrations to robotic systems through aligned latent representations.
...
### [188] [Wow, wo, val! A Comprehensive Embodied World Model Evaluation Turing Test](https://arxiv.org/abs/2601.04137)
...
TL;DR: The paper introduces WoW-World-Eval benchmark to evaluate video foundation models as world models for embodied AI, revealing significant gaps in long-horizon planning, physical consistency, and real-world execution accuracy.


<details>
  <summary>Details</summary>
Motivation: Video foundation models are being explored as predictive world models for embodied AI tasks, but two critical questions remain unanswered: (1) whether their generative generalization maintains perceptual fidelity for human observers, and (2) whether they are robust enough to serve as universal priors for real-world embodied agents.

Method: The authors introduce the Embodied Turing Test benchmark called WoW-World-Eval, built upon 609 robot manipulation data. The benchmark examines five core abilities (perception, planning, prediction, generalization, execution) with a comprehensive evaluation protocol using 22 metrics. They also propose an Inverse Dynamic Model Turing Test to evaluate execution accuracy in the real world.

Result: Models achieve only 17.27 on long-horizon planning and at best 68.02 on physical consistency, indicating limited spatiotemporal consistency and physical reasoning. For the Inverse Dynamic Model Turing Test, most models collapse to ≈0% success rate, while WoW maintains 40.74% success. The evaluation achieves >0.93 Pearson Correlation between overall score and human preference.

Conclusion: There is a noticeable gap between generated videos and the real world, highlighting the urgency and necessity of benchmarking World Models in Embodied AI. The WoW-World-Eval benchmark provides a standardized framework for evaluating video foundation models' capabilities as world models for embodied agents.
...
TL;DR: RSS framework addresses VLA models' brittleness to linguistic perturbations by disentangling physical affordance from semantic execution using Monte Carlo Syntactic Integration and Residual Affordance Steering.
...
Method: Proposes Residual Semantic Steering (RSS) with two innovations: 1) Monte Carlo Syntactic Integration approximates true semantic posterior via dense LLM-driven distributional expansion; 2) Residual Affordance Steering uses dual-stream decoding to isolate language's causal influence by subtracting visual affordance prior.
...
### [190] [SciNetBench: A Relation-Aware Benchmark for Scientific Literature Retrieval Agents](https://arxiv.org/abs/2601.03260)
*Chenyang Shao,Yong Li,Fengli Xu*

Main category: cs.CE

TL;DR: SciNetBench is the first benchmark for evaluating relation-aware retrieval in scientific literature, revealing current retrieval agents perform poorly (<20% accuracy) on relational tasks, but providing relational ground truth improves literature review quality by 23.4%.


<details>
  <summary>Details</summary>
Motivation: Current AI research tools and retrieval agents focus on content-level similarities but fail to decode critical relational dynamics in scientific literature (corroborating/conflicting studies, technological lineages), leading to fragmented knowledge structures and inadequate modeling of scientific progress.

Method: Created SciNetBench benchmark from 18+ million AI papers, evaluating three relational levels: ego-centric retrieval (novel knowledge structures), pair-wise identification (scholarly relationships), and path-wise reconstruction (scientific evolutionary trajectories). Evaluated three categories of retrieval agents.

Result: Current retrieval agents perform poorly on relation-aware tasks (<20% accuracy). Providing agents with relational ground truth improves literature review quality by 23.4%, validating the importance of relation-aware retrieval.

Conclusion: Relation-aware retrieval is critical for comprehensive literature analysis, and SciNetBench provides a benchmark to advance retrieval systems beyond content-level similarities to capture scholarly relationships and scientific evolution.
...
### [191] [Muse: Towards Reproducible Long-Form Song Generation with Fine-Grained Style Control](https://arxiv.org/abs/2601.03973)
...
TL;DR: Open-source system for long-form song generation with fine-grained style control, including licensed synthetic dataset, training pipelines, and Muse model that achieves competitive performance despite modest scale.


<details>
  <summary>Details</summary>
Motivation: Academic research in long-form song generation lags behind commercial systems like Suno due to lack of publicly available training data and reproducible systems, hindering fair comparison and progress in the field.

Method: Release fully open-source system including: 1) 116k licensed synthetic songs with auto-generated lyrics and style descriptions paired with SunoV5 audio, 2) Muse model trained via single-stage supervised finetuning of Qwen-based language model extended with discrete audio tokens using MuCodec, without task-specific losses or architectural modifications.
...
Conclusion: The fully open-source release of data, model weights, and pipelines enables reproducible research and fair comparison, paving the way for continued progress in controllable long-form song generation.
...
### [192] [Roles of MLLMs in Visually Rich Document Retrieval for RAG: A Survey](https://arxiv.org/abs/2601.03262)
*Xiantao Zhang*

Main category: cs.IR

TL;DR: Survey examines how Multimodal LLMs enable practical retrieval-augmented generation for visually rich documents by organizing approaches into three roles and comparing trade-offs.


<details>
  <summary>Details</summary>
Motivation: Visually rich documents (VRDs) present unique challenges for retrieval-augmented generation due to layout-dependent semantics, brittle OCR, and evidence spread across complex figures and tables. Traditional text-based retrieval fails to capture the multimodal nature of these documents.

Method: Organizes literature into three MLLM roles: 1) Modality-Unifying Captioners (convert multimodal content to text), 2) Multimodal Embedders (create joint embeddings), and 3) End-to-End Representers (directly handle retrieval). Compares approaches along retrieval granularity, information fidelity, latency/index size, and compatibility with reranking/grounding.

Result: Provides systematic comparison of MLLM approaches for VRD retrieval, outlining key trade-offs between different roles and offering practical guidance on when to favor each approach based on specific requirements.

Conclusion: MLLMs offer promising solutions for VRD retrieval in RAG systems. Future research should focus on adaptive retrieval units, model size reduction, and better evaluation methods to advance this emerging field.
...
### [193] [STELLA: Self-Reflective Terminology-Aware Framework for Building an Aerospace Information Retrieval Benchmark](https://arxiv.org/abs/2601.03496)
*Bongmin Kim*

Main category: cs.IR

TL;DR: STELLA is a new aerospace-specific information retrieval benchmark built from NASA documents, featuring terminology-aware queries to evaluate both lexical and semantic matching capabilities of embedding models.


<details>
  <summary>Details</summary>
Motivation: There's no public IR benchmark that captures the unique terminology and query intent characteristics of the aerospace domain, despite heavy reliance on technical document search and reuse in this industry.

Method: The STELLA framework uses a systematic pipeline: document layout detection, passage chunking, terminology dictionary construction, synthetic query generation, and cross-lingual extension. It generates two query types: Terminology Concordant Queries (TCQ) for lexical matching evaluation and Terminology Agnostic Queries (TAQ) for semantic matching evaluation, enhanced with Chain-of-Density and Self-Reflection methods.

Result: Evaluation of seven embedding models shows large decoder-based models have strongest semantic understanding, while lexical matching methods like BM25 remain highly competitive in aerospace where exact technical term matching is crucial.

Conclusion: STELLA provides a reproducible foundation for reliable performance evaluation and improvement of embedding models in aerospace-domain IR tasks, addressing the domain-specific benchmark gap.
...
### [194] [STAR-S: Improving Safety Alignment through Self-Taught Reasoning on Safety Rules](https://arxiv.org/abs/2601.03537)
*Di Wu,Yanyan Zhao,Xin Lu,Mingzhe Li,Bing Qin*

Main category: cs.AI

TL;DR: STAR-S is a self-taught framework that improves LLM safety by iteratively learning to reason about safety rules through a cycle of eliciting reasoning, reflection, and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current methods for defending against jailbreak attacks in LLMs rely on training models to reason over safety rules, but it's difficult to determine what form of safety reasoning is most effective or to obtain it directly.

Method: STAR-S integrates safety rule reasoning learning into a self-taught loop: eliciting reasoning and reflection guided by safety rules, then fine-tuning to enhance safety reasoning. This creates a synergistic cycle where improved reasoning produces better training data for further refinement.

Result: Experiments show STAR-S effectively defends against jailbreak attacks and outperforms baseline methods.

Conclusion: The self-taught approach to learning safety reasoning provides an effective defense against jailbreak attacks in LLMs, with code made publicly available for implementation.
...
TL;DR: SAE-Steering uses Sparse Autoencoders to disentangle reasoning strategies in Large Reasoning Models, enabling precise control of reasoning strategies to improve accuracy and reliability.


<details>
  <summary>Details</summary>
Motivation: Current LRMs autonomously select reasoning strategies, which often leads to inefficient or erroneous reasoning paths. There's a need for methods to control reasoning strategies to make reasoning more reliable and flexible, but existing methods struggle with fine-grained control due to conceptual entanglement in hidden states.

Method: Leverages Sparse Autoencoders (SAEs) to decompose strategy-entangled hidden states into a disentangled feature space. Proposes SAE-Steering, a two-stage feature identification pipeline: 1) recalls features that amplify logits of strategy-specific keywords (filtering >99% of features), 2) ranks remaining features by control effectiveness. Uses identified strategy-specific features as control vectors.

Result: SAE-Steering outperforms existing methods by over 15% in control effectiveness. Controlling reasoning strategies can redirect LRMs from erroneous paths to correct ones, achieving 7% absolute accuracy improvement.

Conclusion: SAE-Steering enables effective control of reasoning strategies in LRMs through disentangled feature identification, significantly improving both control effectiveness and reasoning accuracy by steering models away from erroneous paths.
...
### [196] [Sandwich Reasoning: An Answer-Reasoning-Answer Approach for Low-Latency Query Correction](https://arxiv.org/abs/2601.03672)
*Chen Zhang,Kepu Zhang,Jiatong Zhang,Xiao Zhang,Jun Xu*

Main category: cs.AI

TL;DR: Sandwich Reasoning (SandwichR) enables low-latency query correction by aligning fast initial answers with post-hoc reasoning using an Answer-Reasoning-Answer paradigm and consistency-aware RL, achieving SOTA accuracy with 40-70% latency reduction.


<details>
  <summary>Details</summary>
Motivation: Query correction needs high accuracy within real-time constraints. Chain-of-Thought reasoning improves accuracy but has prohibitive latency. Early answer approaches can't leverage reasoning to improve accuracy since answers are generated before reasoning in autoregressive decoding.

Method: Proposes Sandwich Reasoning (SandwichR) with Answer-Reasoning-Answer paradigm. Uses consistency-aware reinforcement learning with dedicated consistency reward to align initial and final corrections, plus margin-based rejection sampling to prioritize borderline samples where reasoning has most impact. Also constructs specialized query correction dataset.

Result: Achieves state-of-the-art accuracy comparable to standard CoT while delivering 40-70% latency reduction, resolving the latency-accuracy trade-off in online search.

Conclusion: SandwichR enables low-latency query correction without sacrificing reasoning-aware accuracy by explicitly aligning fast initial answers with post-hoc reasoning, making it suitable for real-time search pipelines.
...
### [197] [Current Agents Fail to Leverage World Model as Tool for Foresight](https://arxiv.org/abs/2601.03905)
...
TL;DR: Current AI agents struggle to effectively use generative world models for anticipatory reasoning, often misusing or ignoring simulation capabilities, leading to performance degradation rather than improvement.


<details>
  <summary>Details</summary>
Motivation: As agents face increasingly complex tasks requiring future state anticipation, generative world models offer potential as external simulators. However, it's unclear whether current agents can effectively leverage these models to enhance their cognitive capabilities.
...
Conclusion: Current agents lack the capacity to strategically interact with world models. The paper calls for new mechanisms to foster calibrated, strategic use of simulation capabilities to enable reliable anticipatory cognition in future agent systems.
...
TL;DR: DOT (Dynamic Outlier Truncation) reduces excessive verbosity in reasoning models by selectively suppressing redundant tokens during training, achieving 78% token reduction while improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Large reasoning models enhanced by RL often generate excessive verbosity on simple queries, incurring substantial deployment costs. Existing methods using explicit length penalties cause optimization conflicts and don't address the underlying generative mechanisms of overthinking.

Method: Introduces Dynamic Outlier Truncation (DOT) - a training-time intervention that selectively suppresses redundant tokens by targeting only the extreme tail of response lengths within fully correct rollout groups. Also incorporates auxiliary KL regularization and predictive dynamic sampling for stable convergence.

Result: Significantly pushes the efficiency-performance Pareto frontier outward. On AIME-24, reduces inference token usage by 78% while simultaneously increasing accuracy compared to initial policy and surpassing state-of-the-art efficient reasoning methods.

Conclusion: DOT effectively addresses the "length shift" phenomenon where models generate unnecessary reasoning on trivial inputs, reducing verbosity while preserving long-horizon reasoning capabilities for complex problems.
...
### [199] [Staged Voxel-Level Deep Reinforcement Learning for 3D Medical Image Segmentation with Noisy Annotations](https://arxiv.org/abs/2601.03875)
*Yuyang Fu,Xiuzhen Guo,Ji Shi*

Main category: eess.IV

TL;DR: SVL-DRL is a staged reinforcement learning framework for robust medical image segmentation that handles noisy annotations by treating each voxel as an autonomous agent that dynamically refines its state representation.


<details>
  <summary>Details</summary>
Motivation: Noisy annotations in medical image segmentation due to complex organ structures and inter-annotator variability limit model performance. The authors are motivated by how human annotators can correct labeling errors using prior knowledge.

Method: End-to-end Staged Voxel-Level Deep Reinforcement Learning (SVL-DRL) with: 1) Formulation of noisy annotations as voxel-dependent problem, 2) Voxel-level asynchronous advantage actor-critic (vA3C) where each voxel is an autonomous agent, 3) Novel action space and composite reward function combining Dice value and spatial continuity metric.

Result: State-of-the-art performance on three public medical image datasets with average improvement of over 3% in both Dice and IoU scores under various experimental settings.

Conclusion: SVL-DRL effectively mitigates the impact of noisy annotations through a staged reinforcement learning approach that allows voxel-level agents to dynamically refine their state representations, achieving robust segmentation without manual intervention.
...
### [200] [Edit2Restore:Few-Shot Image Restoration via Parameter-Efficient Adaptation of Pre-trained Editing Models](https://arxiv.org/abs/2601.03391)
...
<details>
  <summary>Details</summary>
Motivation: Traditional image restoration requires training specialized models on thousands of paired examples per degradation type. This paper challenges that paradigm by leveraging pre-trained editing models' rich visual priors to dramatically reduce data requirements.

Method: Fine-tunes LoRA adapters on FLUX.1 Kontext (12B parameter flow matching model) using only 16-128 paired images per task, guided by text prompts specifying restoration operations. Uses a single unified LoRA adapter conditioned on task-specific prompts for multiple degradations.

Result: Demonstrates that pre-trained editing models, when properly adapted, offer a compelling data-efficient alternative to traditional approaches. While prioritizing perceptual quality over pixel-perfect metrics, the method effectively handles denoising, deraining, and dehazing with minimal data.

Conclusion: Pre-trained image editing models can be efficiently adapted for multiple restoration tasks through parameter-efficient fine-tuning, opening new avenues for few-shot, prompt-guided image enhancement with dramatically reduced data requirements.
...
### [201] [GeoDiff-SAR: A Geometric Prior Guided Diffusion Model for SAR Image Generation](https://arxiv.org/abs/2601.03499)
*Fan Zhang,Xuanting Wu,Fei Ma,Qiang Yin,Yuxin Hu*

Main category: eess.IV

TL;DR: GeoDiff-SAR is a physics-guided diffusion model that generates high-fidelity SAR images by incorporating explicit geometric information through SAR point clouds and multi-modal feature fusion.


<details>
  <summary>Details</summary>
Motivation: Existing SAR image generation methods operate only in the image domain without explicit geometric information, leading to poor generation quality and inability to control critical parameters like azimuth angles.

Method: 1) Simulates geometric structures via SAR point clouds at specific azimuths; 2) Uses FiLM-based feature fusion gating network to integrate 3D physical info, image parameters, and text descriptions; 3) Fine-tunes Stable Diffusion 3.5 with LoRA for SAR domain adaptation.

Result: GeoDiff-SAR generates high-fidelity SAR images that significantly improve downstream classification accuracy, especially for recognition across different azimuth angles, demonstrating superiority of physics-guided generation.

Conclusion: Incorporating explicit geometric priors through physics-guided diffusion modeling enables high-quality, controllable SAR image generation that enhances downstream task performance.
...
### [202] [A low-complexity method for efficient depth-guided image deblurring](https://arxiv.org/abs/2601.03924)
*Ziyao Yi,Diego Valsesia,Tiziano Bianchi,Enrico Magli*

Main category: eess.IV

TL;DR: A low-complexity neural network using wavelet transforms and depth guidance for efficient image deblurring that achieves competitive quality with 100x less computation.


<details>
  <summary>Details</summary>
Motivation: Image deblurring is ill-posed and current deep learning models have high computational complexity, making them impractical for mobile/edge devices. Mobile Lidars provide complementary depth information that can enhance deblurring quality while reducing complexity.

Method: Novel low-complexity neural network using wavelet transform to separate structural details and reduce spatial redundancy, combined with efficient feature conditioning on depth information from mobile Lidars.
...
Conclusion: Wavelet transform for detail separation and spatial redundancy reduction, combined with efficient depth feature conditioning, enables development of practical low-complexity models for depth-guided image deblurring suitable for mobile/edge deployment.
...
### [203] [Scanner-Induced Domain Shifts Undermine the Robustness of Pathology Foundation Models](https://arxiv.org/abs/2601.04163)
...
<details>
  <summary>Details</summary>
Motivation: Pathology foundation models (PFMs) are widely used in computational pathology but their robustness to real-world technical domain shifts like scanner variability remains poorly understood, despite strong benchmark performance.

Method: Systematic evaluation of 14 PFMs using a multiscanner dataset of 384 breast cancer WSIs from 5 devices, with complementary unsupervised embedding analyses and clinicopathological supervised prediction tasks to isolate scanner effects from biological confounders.

Result: Current PFMs are not invariant to scanner-induced domain shifts - most encode pronounced scanner-specific variability in embedding spaces. While AUC often remains stable, this masks critical failure modes: scanner variability systematically alters embedding spaces and impacts calibration, creating scanner-dependent bias. Robustness doesn't correlate with training data scale, model size, or recency.

Conclusion: PFM development and evaluation must move beyond accuracy-centric benchmarks toward explicit evaluation and optimization of embedding stability and calibration under realistic acquisition variability to ensure clinical reliability.
...
### [204] [Inferring Clinically Relevant Molecular Subtypes of Pancreatic Cancer from Routine Histopathology Using Deep Learning](https://arxiv.org/abs/2601.03410)
...
TL;DR: PanSubNet is an interpretable deep learning framework that predicts pancreatic cancer molecular subtypes from standard H&E slides, achieving 88.5% AUC internally and 84.0% externally, offering a clinically deployable alternative to RNA-seq.


<details>
  <summary>Details</summary>
Motivation: Current molecular subtyping of pancreatic ductal adenocarcinoma (PDAC) using RNA-seq is limited by high cost, slow turnaround time, and tissue requirements, restricting its clinical application despite established prognostic and predictive value.

Method: Developed PanSubNet using 1,055 patients across two cohorts with paired histology and RNA-seq data. Uses dual-scale architecture fusing cellular-level morphology with tissue-level architecture, employing attention mechanisms for multi-scale representation learning and transparent feature attribution. Ground-truth labels derived from Moffitt 50-gene signature refined by GATA6 expression.

Result: Achieved mean AUC of 88.5% on internal validation (PANCAN cohort) and 84.0% on external validation (TCGA cohort) without fine-tuning. Model preserved and in metastatic disease strengthened prognostic stratification compared to RNA-seq labels. Predictions aligned with established transcriptomic programs and biological signatures.

Conclusion: PanSubNet enables rapid, cost-effective molecular stratification from routine H&E slides, offering a clinically deployable and interpretable tool for PDAC precision oncology. Currently gathering real-world validation data to support integration into digital pathology workflows.
...
### [205] [MORPHFED: Federated Learning for Cross-institutional Blood Morphology Analysis](https://arxiv.org/abs/2601.04121)
*Gabriel Ansah,Eden Ruffell,Delmiro Fernandez-Reyes,Petru Manescu*

Main category: cs.LG

TL;DR: Federated learning framework for white blood cell analysis enables privacy-preserving collaborative training across institutions without data sharing, achieving robust cross-site performance in LMICs.


<details>
  <summary>Details</summary>
Motivation: Automated blood morphology analysis in LMICs is sensitive to dataset shifts from staining variability, imaging differences, and rare morphologies. Centralized datasets are infeasible due to privacy regulations and data-sharing restrictions.

Method: Introduce a federated learning framework for white blood cell morphology analysis that enables collaborative training across institutions without exchanging training data. Models learn robust, domain-invariant representations while preserving complete data privacy.

Result: Federated training achieves strong cross-site performance and improved generalization to unseen institutions compared to centralized training, across both convolutional and transformer-based architectures.
...
TL;DR: Training-free mechanistic probe using attention-graph spectra reveals model processing strategies and identifies Passive-Triggered Connectivity Collapse (PTCC) - a syntax-triggered connectivity failure in models undergoing curriculum transitions.


<details>
  <summary>Details</summary>
Motivation: Behavioral benchmarks only tell us what models do, not how they process information internally. There's a need for mechanistic understanding of model behavior and processing strategies.

Method: Attention-graph spectral analysis: treat each layer as token graph, compute algebraic connectivity (λ₂), smoothness, and spectral entropy. Applied across 12 models and 10 languages.

Result: Identified PTCC - English-only syntax-triggered connectivity failure (Δλ₂ ≈ -0.76) in models with curriculum transitions; found four recurrent processing strategies; localized PTCC to Layer 2 compensatory patch; activation steering recovers ≈38% of lost information flow.

Conclusion: Attention-graph spectra provide practical tool for auditing models and verifying training regimes. Topological regimes track tokenization density more than language identity, suggesting geometry varies systematically across scripts.
...
### [207] [Adaptive-Boundary-Clipping GRPO: Ensuring Bounded Ratios for Stable and Generalizable Training](https://arxiv.org/abs/2601.03895)
*Chi Liu,Xin Chen*
...
<details>
  <summary>Details</summary>
Motivation: The authors identify that GRPO's clipping mechanism is suboptimal in certain scenarios and can be enhanced for better flexibility and generalization in reinforcement learning with large language models.
...
Conclusion: ABC-GRPO represents a significant enhancement to GRPO that improves both performance and exploration capabilities in LLM reinforcement learning, with code available for reproducibility.
...
### [208] [FOREVER: Forgetting Curve-Inspired Memory Replay for Language Model Continual Learning](https://arxiv.org/abs/2601.03938)
...
<details>
  <summary>Details</summary>
Motivation: Current memory replay methods for continual learning in LLMs use fixed step-based heuristics that don't align with actual learning progress, since identical training steps can cause varying parameter changes. Recent findings show LLM forgetting follows the Ebbinghaus human forgetting curve.

Method: FOREVER defines model time using magnitude of optimizer updates rather than raw training steps. It uses a forgetting curve-based replay scheduler to determine when to replay, and an intensity-aware regularization mechanism to adaptively control how to replay.

Result: Extensive experiments on three continual learning benchmarks with models ranging from 0.6B to 13B parameters show FOREVER consistently mitigates catastrophic forgetting.

Conclusion: Aligning replay schedules with model-centric time based on optimizer updates and forgetting curves provides more effective continual learning for LLMs compared to fixed step-based heuristics.
...
### [209] [MixRx: Predicting Drug Combination Interactions with LLMs](https://arxiv.org/abs/2601.03277)
*Risha Surana,Cameron Saidock,Hugo Chacon*

Main category: q-bio.OT

TL;DR: MixRx uses LLMs to classify drug combination interactions (Additive/Synergistic/Antagonistic) from patient history, showing Mistral Instruct 2.0 Fine-Tuned achieves 81.5% accuracy.


<details>
  <summary>Details</summary>
Motivation: To explore whether Large Language Models can be effectively used for biological prediction tasks, specifically for classifying drug combination interactions from patient history data.

Method: Uses LLMs (GPT-2, Mistral Instruct 2.0, and their fine-tuned versions) to classify drug combination interactions as Additive, Synergistic, or Antagonistic based on multi-drug patient history data.

Result: Mistral Instruct 2.0 Fine-Tuned model achieved average accuracy of 81.5% on both standard and perturbed datasets, demonstrating potential for LLMs in biological prediction tasks.

Conclusion: LLMs show promise for drug interaction classification, with fine-tuned models performing best, opening new research directions for applying language models to biological prediction problems.
