...
Method: Using in-context concept learning experiments with a specialized dataset to test language models.

Result: Language models show bias toward upward monotonicity in quantifiers during concept learning tasks, which is less visible in direct prompting without concept learning components.

Conclusion: In-context concept learning is an effective method for revealing hidden biases in language models that standard prompting approaches may miss.
...
<details>
  <summary>Details</summary>
Motivation: Addresses limitations of current NLP approaches for low-resource languages that rely on massive pre-collected data and centralized infrastructure, making them inaccessible to underrepresented communities.

Method: Framework combining epistemic uncertainty from models with human hesitation cues and confidence signals to guide interaction, query selection, and memory retention in dynamic language learning.

Result: Position paper advocating for a new approach rather than presenting empirical results - calls for participatory, co-adaptive learning processes.

Conclusion: Advocates moving from extractive data collection to interactive, uncertainty-driven discovery through human-machine collaboration to preserve linguistic diversity while empowering communities.
...
Method: Developed multidimensional framework using LLMs fine-tuned for climate communication, with four classifiers (sentiment, commitment, specificity, target ambition) to analyze sustainability and annual reports from 828 U.S. listed firms.
...
Conclusion: LLMs are valuable for ESG narrative analysis, but stronger regulation is needed to connect commitments with verifiable transition strategies and reduce imitation in reporting.
...
<details>
  <summary>Details</summary>
Motivation: To assess the performance of commercial LLM tools in veterinary medicine, which remains underexplored despite increasing use of LLMs in clinical settings.
...
Result: Product 1 achieved highest overall performance (median score 4.61 vs 2.55 and 2.45 for others), with perfect median scores in Factual Accuracy and Chronological Order. The LLM grader showed high reproducibility across three independent runs.
...
<details>
  <summary>Details</summary>
Motivation: To create a transparent, efficient fact-checking system that doesn't rely on large closed-source models and static knowledge stores, but instead uses live web evidence and smaller models to make fact-checking more accessible.

Method: Uses a stepwise verification pipeline with LLM-guided modules: web search query planning, web evidence retrieval and summarization, evidence synthesis and re-retrieval, and claim verdict evaluation - all optimized for small LLMs.

Result: Achieves 76.4% accuracy on AVeriTeC dataset using Qwen3-4B model, outperforming previous approaches that used much larger models like LLaMA3.1 70B and GPT-4o.
...
<details>
  <summary>Details</summary>
Motivation: To critically assess claims about LLMs' mathematical reasoning capabilities and address potential overestimation due to benchmark composition issues like data contamination and limited problem diversity.

Method: Introduced EEFSUVA benchmark curated from under-circulated regional and national Olympiads of Eastern Europe and former Soviet Union countries, featuring comparable difficulty to IMO but with nonstandard problem-solving techniques.
...
Conclusion: Broader evaluation datasets are important for more accurate assessment of mathematical reasoning and should guide future model development.
...
<details>
  <summary>Details</summary>
Motivation: To understand why large language models fail to follow hierarchical instructions where system prompts should override user inputs, while strongly obeying social cues, and to provide mechanistic interpretations of these behaviors.
...
Result: Found that conflict signals are encoded early with distinct subspaces for system-user vs social conflicts, stronger internal conflict detection for system-user cases but consistent resolution only for social cues, and that social cue vectors surprisingly amplify instruction following in a role-agnostic way.

Conclusion: The findings explain fragile system obedience in LLMs and highlight the need for lightweight hierarchy-sensitive alignment methods to improve instruction following behavior.
...
<details>
  <summary>Details</summary>
Motivation: LLMs provide excellent reranking capabilities but are computationally expensive for real-world deployment. Fine-tuning smaller models is efficient but requires scarce manually labeled data.

Method: Uses LLMs to generate synthetic queries from domain corpora and label positive/hard-negative pairs. Fine-tunes smaller transformer models with contrastive learning using LCE loss on the synthetic dataset.

Result: Experiments on MedQuAD dataset show significant in-domain performance improvement and good generalization to out-of-domain tasks.

Conclusion: By leveraging LLMs for data generation and supervision instead of inference, the approach reduces computational costs while maintaining strong reranking capabilities.
...
Result: Content words exhibit clustering patterns while function words show branching patterns; geometric complexity correlates with semantic content (meaningful characters show rich diversity, structural radicals collapse into tight clusters); child-network analysis demonstrates systematic semantic expansion from elemental characters.
...
<details>
  <summary>Details</summary>
Motivation: Address the reliability of automatic summarization in high-risk scenarios where information overload and critical decision-making demand trustworthy summaries.

Method: Conditional generation-based summarization model with Bayesian inference for uncertainty quantification, predictive distribution entropy measurement, joint optimization of entropy regularization and risk-aware loss, and integration of risk scoring/regulation modules.
...
Method: Combines gradient-based importance scoring with targeted parameter ablation to compute Ability Impact Score (AIS) that quantifies how much each ability contributes to model success on benchmarks.

Result: Profiling three instruction-tuned models across ten benchmarks showed: (i) most benchmarks use multiple abilities, (ii) similarly-labeled datasets rely on distinct ability mixtures, (iii) code-generation benchmarks reward broad improvement, and (iv) irrelevant abilities can negatively affect performance.
...
Result: Achieved 91.73% accuracy on Chandassu Score with evaluation metrics reflecting traditional literary standards, demonstrating effective preservation of cultural knowledge.

Conclusion: Computational social science can successfully preserve endangered cultural knowledge systems and enable new forms of collective intelligence around literary heritage, offering insights for community-centered cultural preservation.
...
<details>
  <summary>Details</summary>
Motivation: The proliferation of diverse LLMs with varying capabilities and costs creates a deployment challenge in selecting the right model for each prompt to balance performance and efficiency.

Method: Uses a neural ranking model trained on RouterBench (36,497 prompts across 11 benchmarks and 11 LLMs) that extracts rich features from prompts including task type, reasoning patterns, complexity indicators, syntactic cues, and proxy solver signals.
...
Conclusion: Feature-driven routing with multifaceted feature extraction and hybrid ranking objectives shows strong potential for efficient and transparent LLM deployment.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of VLMs in medical domains due to data scarcity and high computational costs of advanced training techniques, particularly for structured reasoning in dermatology.
...
Conclusion: The proposed pipeline is a feasible pathway for developing specialized, reliable VLMs in resource-constrained environments.
...
<details>
  <summary>Details</summary>
Motivation: Current post-generation correction methods for LLM hallucination are computationally expensive and fail to prevent unreliable content generation, necessitating a proactive approach to assess model uncertainty before generation.

Method: Combines three confidence signals: semantic alignment between internal representations and reference embeddings, internal convergence analysis across model layers, and learned confidence estimation. Uses unified confidence score to route queries to four pathways based on confidence levels.

Result: Significant improvements in hallucination detection (0.74 vs 0.42 baseline), 40% reduction in computational costs compared to post-hoc methods, F1 score improvement from 0.61 to 0.82 with low false positive rates (0.09).

Conclusion: This paradigm shift from reactive correction to proactive assessment offers a computationally efficient approach to LLM reliability enhancement.
...
<details>
  <summary>Details</summary>
Motivation: To systematically study the impact of padding tokens in LLMs, as their influence is not well understood despite widespread use for sequence length equalization.

Method: Controlled experiments inserting padding across three open-source model families (Llama, Gemma, Qwen), evaluating effects on activations, generation quality, bias, and safety.
...
<details>
  <summary>Details</summary>
Motivation: As LLMs handle diverse sub-tasks, naive approaches that reprocess entire conversation context when switching between main and sub-tasks incur significant computational overhead, which needs to be mitigated for efficient on-device deployment.

Method: Reuses KV cache from main task and injects task-specific instructions into isolated side paths, then rolls back to main path via cached context. Also uses hierarchical classification strategy for sub-task selection by decomposing multi-choice decisions into binary ones.
...
Conclusion: CIFLEX provides an effective solution for efficient sub-task handling in multi-turn LLM interactions, making on-device deployment more practical by minimizing redundant computations.
...
<details>
  <summary>Details</summary>
Motivation: Address ceiling effects in existing math benchmarks by creating a harder, reasoning-focused benchmark with calibrated difficulty and rich metadata for better evaluation of LLMs' mathematical capabilities.

Method: Created two complementary benchmarks: ReasoningMATH with structure-aware diagnostics and metadata, and MATH contest-style suite spanning four educational stages under seven-subject taxonomy. Evaluated 15 LLM variants under consistent setup.

Result: Best model achieved 44% on contest suite (runner-up 37%), with accuracy declining from high school to doctoral levels. On reasoning set, best model reached 81% overall, revealing robustness gaps between top and mid-tier models.

Conclusion: SKYLENAGE provides a challenging, reasoning-centered math benchmark with calibrated difficulty and rich metadata, serving as a reference for future evaluations of mathematical reasoning in LLMs.
...
<details>
  <summary>Details</summary>
Motivation: To understand and quantify how artificial intelligence systems age through structural asymmetries in memory performance, particularly the difference between stable semantic memory and collapsing episodic memory.

Method: Developed the Artificial Age Score (AAS) as a log-scaled, entropy-informed metric; conducted a 25-day bilingual study with ChatGPT-5 using stateless and persistent interaction phases to test memory recall.

Result: In persistent sessions, ChatGPT-5 maintained both semantic and episodic memory with low AAS scores, while in reset sessions, semantic memory remained but episodic memory collapsed, causing high AAS scores indicating structural aging.

Conclusion: AAS serves as a theoretically grounded, task-independent diagnostic tool for evaluating memory degradation in artificial systems, supported by foundational concepts from von Neumann, Shannon, and Turing.
...
Method: ARGRE identifies non-toxic semantic directions, interpolates between toxic/non-toxic representations to reveal transition trajectories, builds an autoregressive reward model, and uses adaptive two-step editing (directional steering + gradient-based refinements) for detoxification.

Result: Extensive experiments on 8 LLMs show ARGRE achieves -62.21% toxicity reduction and -47.58% inference time reduction compared to leading baselines, while preserving original model capabilities with minimal degradation.

Conclusion: ARGRE provides an effective and efficient test-time detoxification solution that enables stable and precise reward-guided editing of LLM representations to reduce toxicity while maintaining model performance.
...
<details>
  <summary>Details</summary>
Motivation: Stress significantly impacts health but is often underreported and captured as unstructured text in EHRs, limiting clinical utility. Ambient AI technologies generate unstructured narratives, creating a need for structured extraction methods.

Method: Developed Mental Stress Ontology (MeSO) by integrating theoretical stress models with concepts from 11 validated assessment tools, then used Claude Sonnet 4 LLM to extract six categories of stress information from Reddit posts, with human evaluation of accuracy and ontology coverage.

Result: MeSO included 181 concepts across eight classes. LLM correctly identified 78.2% of stress items (172/220), misclassified 12.3%, and missed 9.5%. All correctly extracted items mapped accurately to MeSO, though 24 relevant concepts weren't yet represented.

Conclusion: Ontology-guided LLMs are feasible for structured extraction of stress-related information, potentially enhancing consistency and utility of stress documentation in ambient AI systems. Future work should involve clinical dialogue data and LLM comparisons.
...
<details>
  <summary>Details</summary>
Motivation: Existing spatiotemporal models fail to account for abrupt mobility changes caused by external events and struggle to leverage textual descriptions of these events.

Method: Uses a multi-agent LLM framework to extract and reason about spatiotemporally related text from online sources, then incorporates fine-grained contexts with spatiotemporal data through a progressive fusion architecture.

Result: Achieves maximal reductions of 13.92% in MAE and 11.12% in RMSE compared to spatiotemporal models, with pronounced superiority near event locations and times.

Conclusion: The framework successfully enriches event-driven prediction insights through LLM-powered semantic synthesis, creating a more aligned forecasting model for human mobility.
...
Conclusion: SAEs are effective for language model steering, particularly for mathematical reasoning, and match mean activation difference methods on IF-Eval while avoiding degenerate outputs.
...
<details>
  <summary>Details</summary>
Motivation: Current LM evaluations focus mainly on globally popular sports, overlooking regional and indigenous sporting traditions, creating a gap in assessing cultural understanding.

Method: Created a dataset with 33,000 multiple-choice questions across text and image modalities, categorized into history-based, rule-based, and scenario-based types. Evaluated using zero-shot, few-shot, and chain-of-thought prompting across LLMs, SLMs, and MLMs.

Result: The paper establishes CultSportQA as a comprehensive multilingual and multicultural sports benchmark for assessing AI's understanding of traditional sports.

Conclusion: CultSportQA sets a new standard for evaluating AI's ability to understand and reason about traditional sports across diverse cultural contexts.
...
<details>
  <summary>Details</summary>
Motivation: Current graph learning models are trained on individual datasets, limiting cross-domain generalization and requiring large annotated data. Unlike NLP and CV, graph data has unique heterogeneity challenges with domain-specific features and structural diversity.

Method: Uses text as unified representation, introduces dual knowledge distillation framework co-distilling LLMs and GNNs into structure-aware MLPs, and implements in-memory mechanism storing graph representations aligned with memory anchors to integrate invariant knowledge.
...
Conclusion: SSTAG effectively addresses graph learning limitations by leveraging text as unified representation and combining LLM semantic reasoning with GNN structural modeling, enabling practical cross-domain generalization in resource-constrained settings.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with scientific problem-solving due to low-quality corpora with high error rates from logical leaps and implicit reasoning in existing scientific QA datasets.
...
Result: Applied to challenging scientific corpora, LOCA reduced error rates from as high as 20% to below 2%, effectively filtering noisy datasets.

Conclusion: LOCA provides a scalable methodology for creating high-quality scientific corpora, enabling more reliable training and evaluation of scientific AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Social media platforms evolve faster than regulations, creating need for automated detoxification tools to help moderators enforce safe discourse at scale across multiple languages.

Method: Fine-tuned 12B-parameter Gemma-3 multilingual transformer using LoRA SFT, few-shot and Chain-of-Thought prompting. Training used human-authored, machine-translated, and model-generated data filtered by Jaccard thresholds. Inference enriched with LaBSE-retrieved neighbors and toxic-span annotations.

Result: Ranked first on both high-resource and low-resource languages. Few-shot examples improved joint score by +0.081 and CoT prompting by +0.088. Language resource status was strongest performance predictor (η² = 0.667, p < 0.01).

Conclusion: The proposed multilingual detoxification system effectively handles text detoxification across diverse languages, with language resource availability being the key factor influencing performance.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based Entity Linking requires resource-intensive multi-shot inference for reliable uncertainty estimates, limiting real-world applicability. Need more efficient uncertainty estimation methods.
...
Result: The method produces highly effective uncertainty estimates for detecting low-accuracy outputs across multiple LLMs, achieving this at a fraction of the computational cost of traditional approaches.

Conclusion: Provides a practical, cost-effective way to integrate uncertainty measures into LLM-based Entity Linking workflows with limited computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: To understand model representations and the data they internalize from massive, uncurated corpora, and to enable interpretation of deeper structures, themes, and biases embedded in training data.
...
Conclusion: LLMs combined with SAEs can act as scalable probes into complex datasets, offering new paths for corpus exploration, bias discovery, and model interpretability at scale.
...
<details>
  <summary>Details</summary>
Motivation: To test the assumption that model performance on multiple-choice question answering (MCQA) bias benchmarks generalizes to other MCQA tasks and more realistic long-form evaluations in speech models.

Method: Fine-tuned three SpeechLLMs using LoRA adapters to induce specific answer preferences (stereotypical, anti-stereotypical, or neutral), then evaluated generalization to other MCQA benchmarks and long-form creative generation tasks.
...
<details>
  <summary>Details</summary>
Motivation: LLM outputs are shaped by opaque and frequently-changing company content moderation policies, particularly through refusal behaviors that subtly shape public discourse without transparency.

Method: Developed AI Watchman system using a dataset of over 400 social issues to audit multiple LLMs (OpenAI's moderation endpoint, GPT-4.1, GPT-5, and DeepSeek in English and Chinese) longitudinally.

Result: Detected changes in company policies (including unannounced ones), identified company- and model-specific differences in content moderation, and qualitatively categorized different refusal forms.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations in current KGQA methods: retrieval-based approaches constrained by retrieval quality, and agent-based methods dependent on proprietary LLMs.

Method: Proposed Retrieval-Judgment-Exploration (RJE) framework with three specialized modules: Reasoning Path Ranking, Question Decomposition, and Retriever-assisted Exploration.
...
Conclusion: RJE provides an effective and efficient framework for KGQA that reduces dependency on large proprietary LLMs while maintaining high performance.
...
<details>
  <summary>Details</summary>
Motivation: To address the problem of political biases in generative AI systems that stem from training data skews, human prejudice, and algorithmic flaws, which can distort political discourse.
...
Result: Found amplified liberal-authoritarian alignment across all six LLMs, with instances of reasoning supersessions and canned refusals.

Conclusion: AI political biases can permeate public discourse and distort the political landscape, potentially leading to conformity or polarization depending on existing socio-political structures.
...
<details>
  <summary>Details</summary>
Motivation: To understand how different prompting strategies, language choices, and instruction hierarchies affect AI model refusal behavior and safety mechanisms across various harm domains.

Method: Used 80 seeded iterations per scenario across multiple harm domains. Tested composite prompts combining educator persona, safety pretext, and step-cue phrasing. Compared formal registers in German/French vs English. Employed role-playing techniques and AI-assisted hardening methods. Used paired-track design for evaluation awareness testing.

Result: Composite prompts flipped assistance rates from 0% to 97.5% on ZIP-bomb tasks. Formal German/French registers were leakier than English. Linux terminal role-play overrode developer rules in majority of runs. AI-assisted hardening reduced leakage to 0% in several cases. Inconsistent assistance observed in 13% of paired prompts. OpenAI Moderation API under-captured harmful outputs compared to semantic grader.

Conclusion: Current safety mechanisms are vulnerable to sophisticated prompting strategies, with significant differences in refusal rates across inference stacks raising reproducibility concerns. The study demonstrates the need for more robust safety evaluations and hardening methods.
...
Method: Red-teaming approach using Hausa language with minimal prompting, including a survey (n=61) to contextualize errors about toxic substances.

Result: Model generates harmful, culturally insensitive content; falsely claims toxic substances are safe; shows inability to distinguish raw/processed foods; uses demeaning proverbs; safety protocols relax with polite language.

Conclusion: Issues stem from linguistic reward hacking and insufficient safety tuning in low-resource contexts, highlighting significant gaps in current red-teaming efforts.
...
<details>
  <summary>Details</summary>
Motivation: Existing logits-based detectors rely solely on log probabilities, which can be sub-optimal for distinguishing human-written text from LLM-generated text.
...
Result: Extensive numerical studies show AdaDetectGPT nearly uniformly improves state-of-the-art methods across various dataset and LLM combinations, with improvements up to 58%.

Conclusion: AdaDetectGPT provides a more effective approach for LLM text detection by adaptively learning from data rather than relying solely on log probabilities.
...
Method: Progressive Self-Reflection with adaptive rounds based on input complexity, using a lightweight predictor to determine optimal reflection rounds to balance safety and computational efficiency.

Result: Reduced attack success rates from 77.5% to 5.9% for Llama-3.1-8B-Instruct, 89.7% to 5.6% for Llama-3.1-8B base, and 44.4% to 3.8% for Qwen2.5-7B-Instruct, while maintaining original performance on benign tasks.

Conclusion: PSR serves as a scalable test-time approach that dynamically allocates computational resources based on input risk profile, effectively enhancing LLM safety with minimal inference overhead.
...
<details>
  <summary>Details</summary>
Motivation: Current hallucination detection methods designed for auto-regressive LLMs are ill-suited for Diffusion LLMs, as they rely on single-step generation signals while D-LLMs exhibit hallucination signals throughout multi-step denoising processes.

Method: TraceDet models the denoising process as an action trace, with each action defined as the model's prediction over the cleaned response conditioned on previous intermediate output, and identifies the maximally informative sub-trace for hallucinated responses.

Result: Extensive experiments on various open-source D-LLMs show TraceDet consistently improves hallucination detection with an average 15.2% gain in AUROC compared to baseline methods.

Conclusion: TraceDet effectively bridges the gap in hallucination detection for Diffusion LLMs by leveraging key hallucination signals from multi-step denoising processes, enhancing reliability for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Sentiment analysis is crucial for understanding consumer feelings and preferences, but is challenging due to language complexity and diversity. The study aims to explore the viability of using LLMs for sentiment analysis from Bangladesh e-commerce reviews.
...
Conclusion: The study demonstrates the effectiveness of LLMs for sentiment analysis in resource-constrained environments and shows that parameter efficient fine-tuning methods can make advanced models practical for such contexts.
...
<details>
  <summary>Details</summary>
Motivation: Current tool-augmented LLMs lack practical guidance on optimal tool use, particularly for effectively combining textual reasoning, coding, and search across diverse questions.

Method: Proposes TUMIX - an ensemble framework with multiple parallel agents using distinct tool-use strategies and answer paths, iteratively sharing and refining responses based on questions and previous answers.

Result: Achieves average accuracy improvement of up to 3.55% over best baselines on Gemini-2.5 models across key reasoning benchmarks with near-equal inference costs. Can halt refinement at 49% cost while preserving performance.

Conclusion: Agent diversity and quality are crucial for effective tool use, and can be enhanced through LLM auto-optimization. TUMIX enables scalable performance improvements with controlled cost trade-offs.
...
<details>
  <summary>Details</summary>
Motivation: There is a need to establish proper evaluation standards for assessing the capabilities of Deep Research tools that use LLMs to perform knowledge-intensive tasks autonomously.

Method: The authors developed an evaluation sheet specifically for Deep Research tools and applied it to evaluate OpenAI's Deep Search and Google's Deep Search in the context of academic survey writing as a use case.
...
Conclusion: Carefully crafted evaluation standards are essential for properly assessing Deep Research tools, as current tools show significant limitations in academic survey generation despite their autonomous capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Verification is the bottleneck in speculative decoding (4× slower than token generation), but existing intermediate verification methods have substantial training overheads, increased memory footprint, and accuracy compromises.

Method: Uses early-exit models for low-overhead intermediate verification, reuses key-value caches and hidden states between draft, verifier, and target models, and periodically validates accepted tokens against target model.
...
Conclusion: HiSpec provides an effective framework for high-throughput speculative decoding with minimal overhead by leveraging early-exit models and resource reuse strategies.
...
Method: Convert structured causal event graphs into natural-language statements and combine with three prompting strategies (zero-shot, few-shot, chain-of-thought) and three input modalities (text-only, graph-only, text+graph).

Result: 5% average accuracy improvement on TORQUESTRA benchmark, with gains up to 12% in zero-shot settings and 18% with graph-augmented CoT prompting.
...
Method: Uses semantic embedding distances to match target candidates with arbitrary LM-generated text, employing embedding models with less than 10B parameters for low compute cost.
...
Conclusion: The proposed structure-free evaluation method provides robust classification of LM responses at relatively low compute cost, making it suitable for benchmark development and production model quality assessment.
...
Method: Expert Question Decomposition (EQD) - a two-step fine-tuning framework guided by a reward function that measures sub-question effectiveness. Requires only few thousand training examples and single A100 GPU, with inference time comparable to zero-shot prompting.
...
<details>
  <summary>Details</summary>
Motivation: To overcome Transformer limitations in long-context reasoning, computational efficiency, and structural generalization caused by rigid layer stacking, dense attention, and positional encoding reliance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for assessing LLM output quality either rely on superficial text cues or uncalibrated token probabilities, both being partial projections of the richer information in hidden states.
...
TL;DR: Comparison of fine-tuning strategies for RAG pipelines shows equal performance improvements but different computational costs, with optimal strategy depending on context labels and learning rate grid search needs.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare different fine-tuning strategies for Retrieval-Augmented Generation (RAG) pipelines to determine their relative performance and computational costs.

Method: Evaluated independent fine-tuning, joint fine-tuning, and two-phase fine-tuning strategies for RAG pipelines using embedding and generator LLMs on question answering tasks.

Result: All fine-tuning strategies achieved approximately equal improvement in EM and F1 generation quality metrics, but had significantly different computational costs.

Conclusion: The optimal fine-tuning strategy depends on whether the training dataset includes context labels and whether grid search over learning rates for embedding and generator models is required.
...
<details>
  <summary>Details</summary>
Motivation: Current biomedical QA systems focus on short-form answers, lacking comprehensive explanations needed for clinical decision-making, while biomedical literature is growing exponentially making precise information access challenging.

Method: Combines BioBERT embeddings with FAISS indexing and compares re-ranking strategies (BM25, ColBERT, MonoT5) for context selection, then synthesizes evidence through a fine-tuned T5 model.
...
<details>
  <summary>Details</summary>
Motivation: Small Language Models face token-level instability in Traditional Chinese, unpredictably emitting non-TC characters or code-switching into other languages, hindering practical deployment in cost-effective, on-device applications.
...
Result: 51.3% relative reduction in non-TC output tokens versus base model. On Named Entity Translation task, reduces incorrect-language tokens by 77.2% relative to Llama-3B and 57.2% relative to Qwen-1.5B.

Conclusion: Robust Traditional Chinese adherence is attainable even at 1B scale with reproducible, adapter-only pipeline that offers practical recipe to enhance language stability for TC and potentially other non-English languages.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-agent systems are limited by fixed, hand-crafted graph structures that lack adaptability to different contexts and tasks, reducing their effectiveness in real-world applications.

Method: Introduces AMAS with a dynamic graph designer that uses lightweight LLM adaptation to autonomously identify task-specific optimal graph configurations, enabling context-sensitive routing through agent pathways.

Result: AMAS consistently outperforms state-of-the-art single-agent and multi-agent approaches across question answering, mathematical deduction, and code generation benchmarks with various LLM architectures.

Conclusion: Context-sensitive structural adaptability is essential for high-performance LLM multi-agent system deployments, and AMAS provides a paradigm-shifting framework to achieve this.
...
TL;DR: Analysis of machine learning models' ability to detect jailbreak prompts in LLMs, with BERT fine-tuning showing best performance.


<details>
  <summary>Details</summary>
Motivation: LLMs are vulnerable to jailbreak prompts that bypass safety guardrails, requiring effective detection methods.

Method: Fine-tuned BERT model end-to-end for jailbreak identification and analyzed prompt keywords.

Result: BERT achieved best performance in distinguishing jailbreak prompts from genuine uses, including unseen strategies.

Conclusion: Explicit reflexivity in prompt structure may indicate jailbreak intention, and fine-tuned BERT is effective for detection.
...
<details>
  <summary>Details</summary>
Motivation: Autoregressive LLMs have slow adoption in text embedding tasks and limited semantic representation analysis due to unidirectional attention constraints.

Method: Tested Llama architecture variants through additional training steps to progressively enable bidirectional attention and unsupervised/supervised contrastive learning.
...
<details>
  <summary>Details</summary>
Motivation: Current cybersecurity defense systems are fragmented and unable to keep pace with AI-driven offensive systems, creating exploitable blind spots. There is a lack of standardized frameworks for evaluating autonomous security agents and measuring their closed-loop performance.

Method: The authors introduce CLASP framework that aligns security lifecycle (reconnaissance, exploitation, root cause analysis, patch synthesis, validation) with agentic capabilities (planning, tool use, memory, reasoning, reflection & perception). They apply CLASP to 21 representative works and define the CLC Score as a composite metric for evaluation.

Result: By applying CLASP to 21 systems, the framework successfully maps where systems demonstrate strengths and identifies persistent capability gaps. The CLC Score provides a quantitative measure of both loop closure degree and operational effectiveness.

Conclusion: CLASP and CLC Score provide the necessary vocabulary, diagnostics, and measurements to advance both function-level performance and enable proper evaluation of closed-loop security agents, addressing critical gaps in autonomous cybersecurity defense evaluation.
...
TL;DR: FOR-Prompting is a reasoning protocol that uses role-based dialogue (Defender, Objectioner, Host) to improve reasoning through questioning and self-revision, achieving significant accuracy gains on math problems and better reasoning quality.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning protocols like Chain of Thought and Tree of Thought lack explicit mechanisms for external questioning that can elicit self-revision and improvement.
...
Result: 22% point gain over single-prompt on GSM8K, accuracy on par with CoT, 10% higher ratings in reasoning/coherence from GPT-4.1 judge. Corrects mistakes without tools/human supervision, improves small model performance (19% accuracy gain on Llama3.2:1b).

Conclusion: FOR-Prompting is model-agnostic, works without retraining, supports large-scale study of objection-guided reasoning, and shows promise for small models and personal device use with enhanced exploration and refinement.
...
TL;DR: LLMs suffer from a compositionality gap where they can compute individual functions f(x) and g(z) but struggle with their composition g(f(x)). The study identifies two processing mechanisms - compositional and direct - with the choice influenced by embedding space geometry.
...
Method: Used logit lens analysis on residual stream activations of feedforward LLMs during two-hop factual recall tasks, examining how models compute g(f(x)) compositions.

Result: Identified two distinct processing mechanisms: compositional (computing f(x) as intermediate step) and direct (computing g(f(x)) without detectable intermediate f(x)). The choice depends on embedding space geometry, with direct mechanism dominant when linear mapping exists from x to g(f(x)).

Conclusion: LLMs exhibit a compositionality gap and employ different processing strategies for compositional tasks, with embedding space geometry influencing whether models use compositional or direct computation approaches.
...
TL;DR: LLMs in medical pre-consultation suffer from Format Inertia - generating repetitive but uninformative questions in long dialogues due to skewed turn-count distributions in training data. A simple data rebalancing method effectively mitigates this issue.


<details>
  <summary>Details</summary>
Motivation: To address the novel failure mechanism called Format Inertia in medical pre-consultation LLMs, where models generate repetitive, format-correct but diagnostically uninformative questions in long dialogues due to skewed turn-count distributions in SFT datasets.

Method: A simple, data-centric approach that rebalances the turn-count distribution of the training dataset to address the skewed distribution problem.

Result: Experimental results show that the approach substantially alleviates Format Inertia in medical pre-consultation, improving model performance in long medical dialogues.

Conclusion: Data rebalancing of turn-count distributions is an effective method to mitigate Format Inertia in LLMs for medical pre-consultation, addressing the limitations of SFT with skewed dialogue datasets.
...
TL;DR: MathLens is a benchmark that disentangles multimodal reasoning into perception, reasoning, and integration subskills for geometry problems, revealing that different training methods unevenly affect these components.
...
Method: Developed MathLens benchmark with annotated components: visual diagrams, textual descriptions, controlled multimodal questions, and perceptual skill probes derived from symbolic problem specifications to ensure consistency.

Result: Reinforcement learning mainly improves perception (especially with textual supervision), reasoning only improves with perception, integration remains the weakest skill, and RL improves robustness while multimodal SFT reduces it through overfitting.

Conclusion: Different training approaches have distinct effects on multimodal reasoning subskills, with integration being the most challenging component and training methods affecting robustness differently.
...
<details>
  <summary>Details</summary>
Motivation: Engineering design processes rely on document-centric specifications and standards despite digitalization ambitions, creating inefficiencies in validation and compliance checking.

Method: Use modeling patterns to create modular, interoperable ontologies from international piping, material and valve standards, aligned with ISO DIS 23726-3 (Industrial Data Ontology). Test on valve selection by instantiating valves and environmental conditions as OWL individuals.

Result: Successfully enabled automated validation of valve data sheet compliance with industry standards and determination of whether product types meet specifications using semantic reasoning and executable design rules.

Conclusion: The approach demonstrates the potential for Standards Bodies to transition to digitized Smart Standards through shared, reusable IDO-based modular ontologies that enable semantic reasoning in equipment selection processes.
...
TL;DR: Proposes Refusal Index (RI) - a principled metric to measure how accurately LLMs refuse questions they don't know, addressing limitations of existing refusal metrics.
...
Method: Define RI as Spearman's rank correlation between refusal probability and error probability. Use lightweight two-pass evaluation method to efficiently estimate RI from observed refusal rates across two standard evaluation runs.
...
Conclusion: LLMs' refusal behavior can be unreliable despite high accuracy. RI complements traditional accuracy metrics for comprehensive factuality evaluation, revealing an important overlooked aspect of LLM factuality.
...
TL;DR: This study evaluates 16 unsupervised metrics for assessing text extraction quality from Russian judicial decisions, finding that Term Frequency Coherence and Coverage Ratio perform best at aligning with expert ratings, while LLM-based approaches show only moderate correlation.
...
Method: Evaluated 16 unsupervised metrics across five categories (document-based, semantic, structural, pseudo-ground truth, legal-specific) on 1,000 anonymized Russian judicial decisions, validated against 7,168 expert reviews using bootstrapped correlations, Lin's concordance correlation coefficient, and mean absolute error.
...
Conclusion: Unsupervised metrics enable scalable screening but cannot fully replace human judgment in high-stakes legal contexts due to moderate correlations and low CCC values, though they advance legal NLP by providing annotation-free evaluation tools.
...
TL;DR: FraudSquad is a hybrid detection model that combines text embeddings with graph transformers to detect LLM-generated spam reviews, achieving significant improvements over state-of-the-art methods while being resource-efficient.


<details>
  <summary>Details</summary>
Motivation: The rise of LLMs enables generation of highly persuasive spam reviews that closely mimic human writing, posing significant challenges to existing detection systems and threatening online platform credibility.

Method: Proposed FraudSquad - a hybrid model integrating text embeddings from pre-trained language models with gated graph transformers for spam node classification, capturing both semantic and behavioral signals without manual feature engineering.

Result: FraudSquad outperforms state-of-the-art baselines by up to 44.22% in precision and 43.01% in recall on three LLM-generated datasets, while achieving promising results on human-written spam datasets. It maintains modest model size and requires minimal labeled training data.

Conclusion: The work contributes new synthetic datasets, a practical detection framework, and empirical evidence highlighting the urgency of adapting spam detection to the LLM era.
...
TL;DR: LLMs have systematic "syntactic blind spots" where they misapply reasoning to problems with unfamiliar phrasing, despite semantic simplicity. Rephrasing problems using simpler syntactic structures often resolves these errors.


<details>
  <summary>Details</summary>
Motivation: To understand why LLMs fail on mathematically simple problems when they are phrased in unfamiliar ways, revealing that failures stem from structural misalignment rather than conceptual gaps.

Method: Rephrase incorrectly answered questions using syntactic templates from correct examples, quantify syntactic complexity using Dependency Locality Theory (DLT) scores, and analyze failure rates across datasets.

Result: Higher DLT scores correlate with increased failure rates, and rephrasing problems with simpler syntax often leads to correct answers, confirming syntactic complexity drives errors.

Conclusion: Many LLM reasoning errors result from syntactic misalignment rather than mathematical incompetence, and syntax-aware interventions can effectively mitigate these failures.
...
Conclusion: SCRIBES enables scalable and resource-efficient web information extraction by generating reusable scripts that eliminate the need for per-page LLM processing.
...
TL;DR: The paper presents multimodal models trained on developmentally plausible datasets that outperform BabyLM baselines, and uses model merging to maintain language-only abilities in multimodal models.


<details>
  <summary>Details</summary>
Motivation: Address the discrepancy between large vision-language models that learn from enormous datasets versus children's limited linguistic exposure during language acquisition.
...
Conclusion: Model merging with text-only models can partially address the language-only performance degradation in multimodal models, maintaining multimodal capabilities while improving linguistic performance.
...
<details>
  <summary>Details</summary>
Motivation: Address the high cost and unintended side effects of traditional post-training methods for LLMs, which struggle with knowledge acquisition, error correction, and preserving existing knowledge during updates.

Method: Uses a closed-loop feedback mechanism with dynamic memory management, frequent knowledge fusion, and strong locality guards to mitigate instability and conflicts in sequential edits.
...
Conclusion: REPAIR provides a robust framework for developing reliable, scalable, and continually evolving LLMs by addressing the limitations of distribution-agnostic editing methods.
...
TL;DR: A systematic survey of reward models (RMs) for LLM reasoning, covering their architectures, training methods, applications in inference guidance, data synthesis, and RL fine-tuning, along with open research questions.


<details>
  <summary>Details</summary>
Motivation: Reward models are critical for enhancing LLM reasoning performance by providing training signals for RL fine-tuning and selecting optimal answers during inference, but there's a need for systematic understanding of their applications and challenges.

Method: Comprehensive review of RM fundamentals (architectures, training methodologies, evaluation techniques) and analysis of three key applications: inference guidance, data synthesis/self-improvement, and RL-based fine-tuning.

Result: The survey provides actionable insights into RM deployment and identifies critical open questions regarding RM selection, generalization, evaluation, and enhancement based on existing research and empirical findings.

Conclusion: RMs play a vital role in advancing LLM reasoning capabilities, and systematic understanding of their applications and addressing open challenges will enable more effective deployment and advancement of RMs for LLM reasoning.
...
TL;DR: ILM is a unified framework that improves LLM robustness to input perturbations and enables native grounding by inverting model outputs to identify toxic input triggers.
...
Method: Proposes Inverse Language Modeling (ILM) - a framework that transforms LLMs from static generators into analyzable systems by inverting model outputs to identify unsafe input triggers.
...
Conclusion: Online RL is effective for precise and faithful claim verification, providing a foundation for future research. Code is released to support community progress.
...
Method: Evaluate whether providing examples of how individuals annotate Sentiment, Emotion, Argument, and Topics (SEAT dimensions) helps language models predict their value interpretations across zero- and few-shot settings.
...
Conclusion: This controlled study provides the first evidence that annotation behavior (beyond demographics) impacts value prediction, establishing a foundation for future large-scale validation of personalized AI alignment.
...
<details>
  <summary>Details</summary>
Motivation: To systematically study how schema normalization affects NL2SQL system performance, as most prior research evaluates models on fixed schemas without considering design influence.
...
Result: Denormalized schemas achieved high accuracy on simple retrieval queries even with cost-effective models. Normalized schemas (2NF/3NF) caused errors in table selection and joins but were improved by few-shot examples. For aggregation queries, normalized schemas performed better due to robustness against data duplication and NULL values.

Conclusion: Optimal schema design for NL2SQL depends on query types - denormalized for simple retrieval, normalized for aggregation. Schema design should be considered in NL2SQL development with adaptive selection for real-world scenarios.
...
TL;DR: The paper introduces BanglaMultiHate, the first multi-task Bangla hate-speech dataset, and compares various models including LLMs with LoRA fine-tuning, finding that culturally grounded pretraining remains crucial for robust performance.


<details>
  <summary>Details</summary>
Motivation: Online platforms need reliable hate speech detection, especially for low-resource languages like Bangla where existing tools are limited and mostly single-task with poor coverage of multi-facet signals.
...
Conclusion: The dataset and findings establish a stronger benchmark for developing culturally aligned moderation tools in low-resource contexts, with dataset and scripts to be released for reproducibility.
...
TL;DR: This paper examines LLMs' creativity through a process-oriented approach using constraint-based decision-making and authorial personas, finding that models consistently prioritize Style over other narrative elements.
...
Method: Used controlled prompting to assign authorial personas to LLMs and analyzed their creative preferences through constraint-based decision-making, drawing on narratology frameworks.

Result: LLMs consistently emphasized Style over other narrative elements (Character, Event, Setting), and distinctive creative profiles emerged across different models based on their reasoning patterns.

Conclusion: The approach provides a novel systematic tool for analyzing AI's authorial creativity, revealing distinctive creative preferences and reasoning patterns across different language models.
...
<details>
  <summary>Details</summary>
Motivation: End-to-end speech dialogue systems suffer from hallucinations due to limited factual grounding, while traditional tool integration increases response latency and disrupts conversational flow.

Method: Proposed Streaming Retrieval-Augmented Generation (Streaming RAG) framework with post-training pipeline that teaches models to issue tool calls during ongoing speech and generate spoken summaries fusing audio queries with retrieved text results.
...
Conclusion: Streaming RAG enables more agentic, real-time AI assistants and is modality-agnostic, applicable to both speech and typed input systems.
...
TL;DR: SCoT is a Streaming Chain-of-Thought framework for Duplex spoken dialogue systems that processes user input in fixed blocks and generates responses blockwise, eliminating the need for voice activity detection while improving coherence and reducing latency.


<details>
  <summary>Details</summary>
Motivation: Traditional E2E spoken dialogue systems rely on VAD for turn-taking but fail to distinguish pauses from turn completions. Duplex SDS models address this but have complex architectures and lag in semantic reasoning.

Method: Proposed SCoT framework alternates between processing fixed-duration user input and generating responses in blockwise manner using frame-level alignments to create intermediate targets (aligned user transcripts and system responses) for each block.
...
Conclusion: SCoT framework successfully addresses limitations of both VAD-based systems and complex duplex architectures by providing a streaming chain-of-thought approach that improves response quality while enabling real-time interactions.
...
TL;DR: Speculative decoding shows uneven speed-up rates across tasks, with under-fit and underrepresented tasks benefiting less. The paper analyzes this unfairness, proposes a mitigation strategy, and achieves 12% improvement in fairness.


<details>
  <summary>Details</summary>
Motivation: To understand and address the disparate speed-up rates in speculative decoding, where under-fit and underrepresented tasks gain less benefit from the technique.

Method: Derived an analysis to quantify unfairness in speculative decoding speed-ups and proposed a mitigation strategy to reduce disparities.

Result: The mitigation strategy improved the fairness metric by 12% on average across several model pairs.

Conclusion: Speculative decoding exhibits systematic unfairness in speed-up distribution, but this can be mitigated through targeted approaches.
...
TL;DR: RESTRAIN is a self-penalizing RL framework that enables models to improve reasoning without gold labels by exploiting signals from the model's entire answer distribution and penalizing overconfident rollouts.


<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning with human-annotated data is costly and struggles with harder tasks, creating need for experience-driven learning without curated labels.

Method: Uses self-penalization mechanism that penalizes overconfident rollouts and low-consistency examples while preserving promising reasoning chains, integrating into policy optimization methods like GRPO.

Result: Achieves large gains on challenging benchmarks: +140.7% Pass@1 on AIME25, +36.2% on MMLU_STEM, +19.6% on GPQA-Diamond with Qwen3-4B-Base and OctoThinker Hybrid-8B-Base, nearly matching gold-label training performance.

Conclusion: RESTRAIN establishes a scalable path toward stronger reasoning without requiring gold labels, enabling continual self-improvement through unsupervised learning.
...
<details>
  <summary>Details</summary>
Motivation: LLMs often generate hallucinations that undermine reliability. While most works treat hallucination detection as binary, real applications need to identify specific hallucinated spans, which requires multi-step reasoning.

Method: Proposed RL4HS framework using reinforcement learning with span-level rewards, building on Group Relative Policy Optimization and introducing Class-Aware Policy Optimization to address reward imbalance.
...
Conclusion: Reinforcement learning with span-level rewards is necessary for detecting hallucination spans, and explicit reasoning helps in this complex multi-step decision process.
...
TL;DR: The paper introduces a generalized method based on SPINACH, an LLM-backed agent that translates natural language questions to SPARQL queries through iterative exploration and execution, aiming to lower the barrier for interacting with knowledge graphs.


<details>
  <summary>Details</summary>
Motivation: To make knowledge graph interaction accessible to non-technical users by overcoming the high barrier of SPARQL query language through Text2SPARQL translation, motivated by the Text2SPARQL challenge.

Method: Uses SPINACH, an LLM-backed agent that performs Text2SPARQL translation not in a single shot, but as an iterative process of exploration and execution with a specific architecture design.

Result: The paper describes the overall architecture and conducts thorough analysis of agent behavior to gain insights for future improvements.

Conclusion: The work provides a generalized iterative approach for Text2SPARQL translation and identifies areas for targeted improvements through behavioral analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluations emphasize execution accuracy but neglect whether CoT reasoning aligns with ground-truth actions, which can lead to over-trust and potential harmful actions when users rely on seemingly plausible reasoning.
...
Result: Experimental results show reasoning-execution gaps are prevalent across mobile interaction tasks, with execution gaps occurring more frequently than reasoning gaps. Scaling up model size reduces overall gaps but sizable execution gaps persist even in largest models.
...
<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods rely on self-exploration or single teachers, which can introduce model biases and limit reasoning diversity. Multi-teacher strategies from knowledge distillation could help overcome these limitations.

Method: AMPO adaptively leverages guidance from multiple proficient teacher models only when the on-policy model fails, with a comprehension-based selection mechanism that prioritizes reasoning paths the student can understand.

Result: AMPO outperforms GRPO baseline by 4.3% on mathematical reasoning and 12.2% on out-of-distribution tasks, boosts Pass@k performance, enables more diverse exploration, and achieves comparable results to single powerful teacher approaches with less data.

Conclusion: AMPO demonstrates a more efficient and scalable path to superior reasoning and generalizability in LLMs through adaptive multi-teacher guidance and selective learning from comprehensible reasoning paths.
...
<details>
  <summary>Details</summary>
Motivation: The growth of social media platforms like X (formerly Twitter) endangers young people's emotional well-being through cyberbullying. Most existing cyberbullying detection methods focus on English, leaving Arabic-language detection methods scarce and underdeveloped.

Method: Assembled a dataset of 10,662 X posts, pre-processed the data, and used kappa tool for annotation quality verification. Tested multiple deep learning models including LSTM, Bi-LSTM with various word embeddings, and novel BERT-based approaches (LSTM-BERT and Bi-LSTM-BERT).

Result: LSTM-BERT and Bi-LSTM-BERT achieved 97% accuracy. Bi-LSTM with FastText embedding performed even better, achieving 98% accuracy. The results demonstrate strong generalization capabilities.

Conclusion: The proposed deep learning models effectively detect Arabic-language cyberbullying with high accuracy, addressing the critical gap in Arabic cyberbullying detection methods and providing valuable tools for protecting young people's emotional well-being on social media platforms.
...
TL;DR: AccurateRAG is a novel framework for building high-performance question-answering applications using retrieval-augmented generation (RAG), featuring a comprehensive development pipeline with tools for data processing, fine-tuning, evaluation, and local system building.


<details>
  <summary>Details</summary>
Motivation: To create an efficient and high-performance framework for developing question-answering applications using RAG technology, addressing the need for comprehensive development tools and improved performance over existing baselines.

Method: A pipeline-based framework with tools for raw dataset processing, fine-tuning data generation, text embedding & LLM fine-tuning, output evaluation, and building RAG systems locally.
...
Conclusion: AccurateRAG provides an effective solution for developing high-performance RAG-based question-answering applications, demonstrating superior performance compared to existing methods.
...
TL;DR: The paper introduces TECA metric and CER mechanism to help LLMs dynamically determine when to stop reasoning, reducing overthinking by up to 71% on simpler problems while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: LLMs often suffer from overthinking - generating unnecessarily lengthy reasoning for simple problems, which degrades efficiency and makes it difficult to adapt reasoning depth to problem complexity.
...
Conclusion: The approach creates a more efficient and adaptive reasoning process for LLMs by helping them determine when to conclude reasoning and provide final answers.
...
TL;DR: InfoMosaic-Bench is the first benchmark for multi-source information seeking in tool-augmented agents, requiring integration of general-purpose search with domain-specific tools across six domains, revealing that current LLMs struggle with effective tool usage and web search alone is insufficient.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agents rely heavily on open-web search which has limitations: online content is noisy/unreliable and many real-world tasks require precise domain-specific knowledge not available from the web. While MCP enables tool integration, it's unclear if agents can effectively leverage tools and combine them with general search.
...
Result: Experiments with 14 state-of-the-art LLM agents show: web information alone achieves only 38.2% accuracy and 67.5% pass rate; domain tools provide selective but inconsistent benefits; 22.4% of failures come from incorrect tool usage/selection, indicating LLMs struggle with basic tool handling.

Conclusion: Current LLM agents cannot effectively integrate multiple information sources, with significant challenges in tool usage and cross-source reasoning, highlighting the need for improved multi-source information seeking capabilities in agents.
...
TL;DR: This paper investigates cross-lingual transfer of reasoning capabilities in Large Reasoning Models (LRMs) after English Reinforcement Post-Training, finding significant variation in transferability and proposing parallel training to improve multilingual generalization.


<details>
  <summary>Details</summary>
Motivation: To address whether reasoning capabilities achieved from English Reinforcement Post-Training effectively transfer to other languages, challenging the assumption that LRM reasoning mirrors human cognition.

Method: Systematic evaluation of English-centric LRMs on multilingual reasoning benchmarks, introduction of cross-lingual transferability metric, interventional studies, and parallel training experiments.

Result: Cross-lingual transferability varies significantly across models, languages, and training paradigms. Models with stronger English capabilities show diminished cross-lingual generalization. Parallel training reveals: First-Parallel Leap (performance jump with single parallel language), Parallel Scaling Law (power-law relationship with parallel languages), and Monolingual Generalization Gap (discrepancy between actual and predicted performance).

Conclusion: English-centric LRMs fail to fully generalize across languages, challenging the assumption that LRM reasoning mirrors human cognition. The study provides critical insights for developing more language-agnostic LRMs.
...
<details>
  <summary>Details</summary>
Motivation: To enable systematic benchmarking, analysis, and interpretation of vision-language models by providing a unified interface that abstracts away model-specific complexities.

Method: Provides a YAML-configurable interface supporting extraction of intermediate outputs from any layer during forward pass, currently supporting 16 base VLMs and over 30 variants.

Result: Demonstrated usage with analytical experiments revealing systematic differences in hidden representations across layers and target concepts.
...
TL;DR: F2LLM is a suite of efficient embedding models (0.6B, 1.7B, 4B) that achieve state-of-the-art performance through direct finetuning from foundation models on open-source data, avoiding costly pretraining and synthetic data.


<details>
  <summary>Details</summary>
Motivation: To create high-performance embedding models that are more efficient and cost-effective than previous approaches that require massive contrastive pretraining, complex pipelines, and expensive synthetic training data.

Method: Direct finetuning from foundation models on 6 million query-document-negative tuples curated from open-source, non-synthetic datasets, focusing on balancing training cost, model size, and performance.
...
Conclusion: F2LLM provides a strong, reproducible, and budget-friendly baseline for future embedding model research, with released models, training data, and code.
...
TL;DR: The paper questions whether draws in LLM arena evaluations truly indicate equal model performance, suggesting instead that draws reflect query difficulty. The authors show that ignoring draws in rating updates improves prediction accuracy by 1-3%.


<details>
  <summary>Details</summary>
Motivation: To critically examine the prevailing paradigm of treating draws in LLM arena evaluations as indicating equal model performance, and to investigate whether draws actually reflect query difficulty rather than model equality.

Method: Analyzed three real-world arena datasets using four rating systems, comparing approaches that include vs. exclude draws in rating updates. Also examined query characteristics associated with draws.

Result: Ignoring rating updates for draws yields 1-3% relative increase in battle outcome prediction accuracy. Draws occur more frequently for very easy queries (risk ratio 1.37) and highly objective queries (risk ratio 1.35).

Conclusion: Future rating systems should reconsider existing draw semantics and account for query properties in rating updates, as draws appear to be more indicative of query difficulty than model equality.
...
<details>
  <summary>Details</summary>
Motivation: Current zero-shot image restoration methods using latent diffusion models (LDMs) produce temporally inconsistent results when applied frame-by-frame to video restoration, lacking the ability to capture temporal dependencies.

Method: Leverages Video Consistency Models (VCMs) that distill video latent diffusion models into fast generators capturing temporal causality. Uses a conditioning mechanism that bypasses automatic differentiation and requires only few neural function evaluations.
...
Conclusion: LVTINO establishes a new benchmark for both reconstruction fidelity and computational efficiency in zero-shot video restoration by effectively incorporating temporal consistency through VCM priors.
...
TL;DR: A method for fine-grained stylized image generation using style extraction from reference images and alignment with text conditions.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models struggle with precise fine-grained style control using natural language, and stylized reference images are hard to align with textual conditions in traditional generation approaches.

Method: Three-stage training method using a style encoder and style projection layer to extract style representations from reference images and align them with text representations, without changing the downstream generative model's structure.

Result: Proposed method enables fine-grained controlled stylized image generation by injecting style representations into the generative model while maintaining text guidance.

Conclusion: The approach successfully achieves fine-grained stylized image generation by extracting and aligning style representations with text conditions, using a custom Style30k-captions dataset for training.
...
TL;DR: This paper introduces EvoStruggle, a dataset for temporal struggle determination during skill acquisition, featuring 61.68 hours of video recordings from 76 participants performing 18 tasks across four activities, with 5,385 annotated struggle segments.


<details>
  <summary>Details</summary>
Motivation: Existing manipulation datasets don't capture how struggle evolves over time during skill acquisition, which is crucial for understanding learning stages and developing effective assistive systems.

Method: Collected a dataset with 2,793 videos from 76 participants performing 18 tasks across four activities (tying knots, origami, tangram puzzles, shuffling cards), with participants repeating tasks five times to capture skill evolution. Defined struggle determination as a temporal action localization problem.

Result: Temporal Action Localization models successfully detected struggle cues, achieving 34.56% average mAP when generalizing across tasks and 19.24% across activities, showing struggle is a transferable concept across skill-based tasks.

Conclusion: Struggle can be detected as a transferable concept across various tasks, though there's room for improvement in struggle detection performance. The dataset enables research on understanding skill acquisition evolution.
...
<details>
  <summary>Details</summary>
Motivation: Existing PDE foundation models rely on large transformer architectures with high computational overhead, creating a need for more efficient and parameter-efficient alternatives.
...
Result: Achieves state-of-the-art generalization across 6 challenging unseen downstream PDEs while requiring significantly fewer parameters and minimal fine-tuning data.
...
TL;DR: DisCo is an RL-based framework that optimizes identity diversity in multi-human image generation by fine-tuning flow-matching models with a compositional reward system, achieving near-perfect face uniqueness and identity spread.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models fail at multi-human prompts, duplicating faces, merging identities, and miscounting individuals, creating an identity crisis in generative models.

Method: Uses Group-Relative Policy Optimization (GRPO) with a compositional reward that penalizes facial similarity, discourages identity repetition, enforces accurate person counts, and preserves visual fidelity through human preference scores. Employs a single-stage curriculum for stable training.
...
TL;DR: The paper presents a novel approach to visual geo-localization by aligning visual representations with hierarchical geographic embeddings and fusing appearance features with semantic segmentation maps, achieving state-of-the-art performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: To improve visual geo-localization by developing better learned representations of geography that can accurately determine the geographic location of images anywhere on Earth using only visual content.

Method: Formulates geo-localization as aligning visual representations with learned geographic representations, explicitly modeling the world as a hierarchy of geographic embeddings and efficiently fusing appearance features with semantic segmentation maps.
...
Conclusion: The gains in geo-localization performance are primarily driven by the combination of geographic and visual representations, demonstrating the effectiveness of the proposed hierarchical geographic embedding approach.
...
TL;DR: XMAS is a data-efficient method for LVLM instruction tuning that clusters examples based on cross-modal attention similarity to remove redundancy, achieving 50-85% data reduction while preserving performance.


<details>
  <summary>Details</summary>
Motivation: Data selection methods for LVLMs are underexplored and existing approaches fail to outperform random selection, creating a need for principled data-efficient training methods.

Method: XMAS clusters examples based on trajectories of top singular values of attention matrices from fine-tuning a proxy LVLM, then samples balanced subsets from clusters to remove redundancy.

Result: XMAS discards 50% of LLaVA-665k and 85% of Vision-Flan datasets while preserving LLaVA-1.5-7B performance on 10 benchmarks and speeding up training by 1.2x, outperforming baselines by 30% more data reduction.

Conclusion: XMAS provides the first effective data selection method for LVLM instruction tuning, enabling significant data reduction without performance loss through principled attention-based clustering.
...
TL;DR: Purrception is a variational flow matching method for vector-quantized image generation that combines continuous transport dynamics with categorical supervision over codebook indices.
...
TL;DR: Proposes a unified deep learning framework that simultaneously generates synthetic contrast-enhanced CT images from non-contrast CT scans and segments aortic lumen and thrombus, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Reduce risks associated with iodinated contrast agents in CT scans (nephrotoxicity, allergies, environmental harm) by minimizing contrast agent use through synthetic image generation, while addressing limitations of multi-stage pipelines that cause error accumulation.

Method: Integrates conditional diffusion models with multi-task learning for end-to-end joint optimization of image synthesis and segmentation. Uses shared encoder/decoder parameters across tasks, requires no initial predictions, and employs semi-supervised training to handle missing segmentation labels.
...
Conclusion: The unified framework effectively reduces contrast agent use while improving both image synthesis quality and anatomical segmentation accuracy, demonstrating superior performance over single-task and multi-stage approaches.
...
TL;DR: A framework for efficiently prototyping multi-modal content analysis pipelines that converts videos into temporal semi-structured data and then to queryable knowledge graphs.


<details>
  <summary>Details</summary>
Motivation: Multi-modal content analysis is complex, computationally expensive, and requires significant engineering. Existing pre-trained models are available but challenging to fuse with complex data like videos.

Method: Develop a framework that crafts candidate pipelines using pre-trained models to convert videos into temporal semi-structured data, then translates this to frame-level indexed knowledge graphs that are queryable and support continual learning.

Result: Created a system that enables dynamic incorporation of new domain-specific knowledge through an interactive medium.
...
<details>
  <summary>Details</summary>
Motivation: Current web agents are brittle and rely on step-by-step UI interactions with heavy LLM reasoning, which breaks under dynamic layouts and long horizons. Humans exploit website-provided functionality through high-level operations.

Method: WALT reverse-engineers latent website functionality into reusable invocable tools that abstract away low-level execution. Instead of reasoning about clicks and typing, agents call tools like search(query) or create(listing).
...
Conclusion: The proposed framework successfully preserves biologically meaningful structures while filtering out transient artifacts, improving segmentation quality in semi-supervised histopathology image analysis.
...
TL;DR: Diffusion-LPO is a listwise preference optimization framework for diffusion models that extends DPO to handle ranked image preferences, outperforming pairwise methods on quality and alignment.
...
Method: Proposes Diffusion-LPO framework that aggregates user feedback into ranked image lists and derives a listwise extension of DPO objective under Plackett-Luce model, enforcing consistency across entire rankings.

Result: Empirical evaluation shows Diffusion-LPO consistently outperforms pairwise DPO baselines across text-to-image generation, image editing, and personalized preference alignment tasks.

Conclusion: Diffusion-LPO effectively leverages listwise preferences to improve alignment with human preferences in diffusion models, demonstrating superior performance over pairwise approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current unified MLLMs face challenges: hybrid approaches break the autoregressive paradigm, while pure autoregressive approaches struggle with trade-offs between semantic alignment and pixel-level fidelity in visual generation.

Method: Augments pre-trained visual understanding models with generative ability through Mixture-of-Transformers architecture. Uses semantic-to-pixel discrete representation that integrates compact semantic tokens with fine-grained pixel tokens to improve visual generation fidelity.

Result: Achieves competitive or superior results in both understanding and generation benchmarks across diverse multimodal benchmarks. Requires less training data and reduced training time compared to prior unified MLLMs.
...
TL;DR: A hybrid CNN-Bayesian deep learning model for oral cancer classification using small datasets, achieving 94% accuracy on similar distribution data and superior 88% generalizability on diverse real-world data compared to 72.94% for traditional CNNs.


<details>
  <summary>Details</summary>
Motivation: Address challenges in oral cancer diagnosis including limited healthcare access, inadequate infrastructure, and shortage of practitioners. Traditional deep learning models are overconfident, require large datasets, and lack reliability in data-scarce settings.
...
Result: Achieved 94% accuracy on similar distribution test data (comparable to traditional CNN). Superior generalizability with 88% accuracy on diverse real-world datasets vs 72.94% for traditional CNNs. Model shows low uncertainty for correct classifications and high uncertainty for misclassifications.
...
Method: Uses assistant domain module to get diversified representations from global attentions, applies multiple consistent strategies to obtain invariable features, and employs conditional multi-kernel MMD to align hard samples with easy ones.
...
TL;DR: A system that uses historical BLV user questions to guide MLLMs in generating context-aware visual descriptions, improving relevance by 76.1% and user preference by 54.4%.


<details>
  <summary>Details</summary>
Motivation: Current MLLM applications provide lengthy, comprehensive descriptions regardless of context, forcing BLV users to sift through irrelevant details instead of getting specific information they need.

Method: Developed a system that identifies similar past visual contexts from VizWiz-LF dataset and uses associated BLV user questions to guide MLLM in generating more relevant descriptions.
...
Conclusion: Using historical BLV user questions to guide MLLMs significantly improves the relevance and user preference of visual descriptions for blind and low vision users.
...
TL;DR: ImageNet-Think is a multimodal reasoning dataset with 250,000 ImageNet21k images, featuring structured thinking tokens and answers generated by advanced VLMs to train reasoning-capable vision-language models.
...
Method: Built on 250,000 ImageNet21k images, using GLM-4.1V-9B-Thinking and Kimi-VL-A3B-Thinking-2506 VLMs to generate structured thinking tokens and corresponding answers with step-by-step reasoning processes.

Result: Created a synthetic dataset with two thinking-answer sequence pairs per image, capturing both reasoning steps and final descriptive answers.
...
TL;DR: NPN is a novel regularization method that uses neural networks to project solutions into low-dimensional null-space structures of sensing matrices, improving reconstruction in imaging inverse problems.


<details>
  <summary>Details</summary>
Motivation: Existing priors ignore task-specific null-space structure, leading to suboptimal solutions in ill-posed inverse problems where multiple solutions exist in the null-space.

Method: Proposes Non-Linear Projections of the Null-Space (NPN) - neural network-based regularization that promotes solutions in low-dimensional projections of the sensing matrix's null-space rather than enforcing image-domain constraints.

Result: NPN priors consistently enhance reconstruction fidelity across various imaging problems (compressive sensing, deblurring, super-resolution, CT, MRI) and work with multiple reconstruction frameworks including plug-and-play methods, unrolling networks, deep image prior, and diffusion models.

Conclusion: NPN provides interpretable and flexible regularization by focusing on null-space structure, is compatible with existing methods, and offers theoretical guarantees for convergence and accuracy.
...
TL;DR: Automated genomic interpretation module using Chaos Game Representation and Concept Bottleneck Model for interpretable HIV subtype classification with cost-aware decision policies.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between interpretable genomic modeling and automated decision-making for clinical automation in genomic medicine.
...
Result: Achieves state-of-the-art classification performance on HIV subtypes across in-house and LANL datasets, superior concept prediction fidelity, and favorable cost-benefit trade-offs compared to baselines.

Conclusion: Establishes a reliable foundation for robotic and clinical automation in genomic medicine by providing interpretable evidence and cost-aware decision policies.
...
TL;DR: VLA-R1 is a reasoning-enhanced Vision-Language-Action model that integrates Reinforcement Learning from Verifiable Rewards (RLVR) with Group Relative Policy Optimization (GRPO) to improve step-by-step reasoning and action execution in embodied AI tasks.
...
Method: Developed VLA-R1 with RLVR-based post-training strategy using verifiable rewards for region alignment, trajectory consistency, and output formatting. Also created VLA-CoT-13K dataset with chain-of-thought supervision aligned with affordance and trajectory annotations.
...
Conclusion: VLA-R1 achieves enhanced reasoning robustness and execution accuracy through systematic optimization of both reasoning and execution, addressing key limitations in current VLA models.
...
TL;DR: Proposes a joint deblurring and 3D reconstruction method for macrophotography that simultaneously optimizes clear 3D models and defocus blur kernels from multi-view blurry images using differentiable rendering.


<details>
  <summary>Details</summary>
Motivation: Defocus blur in macrophotography hinders clear imaging and high-quality 3D reconstruction of small objects, while traditional methods require many images and annotations with no existing multi-view 3D reconstruction solutions for macrophotography.

Method: Jointly optimizes clear 3D model and defocus blur kernel for each pixel from multi-view blurry images using differentiable rendering for self-supervised optimization.

Result: Extensive experiments show the method achieves high-quality image deblurring and recovers high-fidelity 3D appearance from a small number of multi-view images.

Conclusion: The proposed framework successfully addresses the defocus blur problem in macrophotography by jointly optimizing deblurring and 3D reconstruction through differentiable rendering.
...
TL;DR: FideDiff is a single-step diffusion model for high-fidelity motion deblurring that reformulates deblurring as a diffusion process and trains a consistency model to align all blur levels to clean images, achieving state-of-the-art performance with one-step inference.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current diffusion models for image deblurring, including slow inference time and compromised fidelity, while leveraging their strong generative capabilities compared to CNN and transformer methods.

Method: Reformulates motion deblurring as a diffusion-like process where each timestep represents progressively blurred images, trains a consistency model to align all timesteps to clean images, integrates Kernel ControlNet for blur kernel estimation, and introduces adaptive timestep prediction.

Result: Achieves superior performance on full-reference metrics, surpassing previous diffusion-based methods and matching other state-of-the-art models while enabling one-step deblurring.

Conclusion: FideDiff establishes a new direction for applying pre-trained diffusion models to high-fidelity image restoration tasks and provides a robust baseline for advancing diffusion models in real-world industrial applications.
...
TL;DR: A two-stage detection-recognition pipeline with LadderMoE adapters for robust bronze inscription recognition across multiple domains and long-tailed character distributions.


<details>
  <summary>Details</summary>
Motivation: Bronze inscriptions are crucial for archaeological studies but difficult to recognize automatically due to severe visual degradation, multi-domain variability (photographs, rubbings, tracings), and extremely long-tailed character distribution.

Method: Curated a large-scale dataset with 22,454 full-page images and 198,598 annotated characters across 6,658 categories. Developed a two-stage pipeline: first localizes inscriptions, then transcribes characters. Used LadderMoE to augment CLIP encoder with ladder-style MoE adapters for dynamic expert specialization and robustness.

Result: Substantially outperforms state-of-the-art scene text recognition baselines, achieving superior accuracy across head, mid, and tail categories and all acquisition modalities (photographs, rubbings, tracings).

Conclusion: Establishes a strong foundation for bronze inscription recognition and downstream archaeological analysis with robust cross-domain performance.
...
TL;DR: VirDA proposes a parameter-efficient domain adaptation method using visual reprogramming layers instead of full backbone fine-tuning, achieving competitive accuracy with significantly fewer parameters.
...
Result: On Office-31, VirDA achieves 92.8% mean accuracy with only 1.5M parameters, outperforming PDA by +1.6% accuracy using 46% of its parameters, and surpassing full-backbone methods while using only 1.7-2.8% of their parameters.

Conclusion: VirDA enables efficient domain adaptation by reusing backbone parameters across domains through visual reprogramming, achieving state-of-the-art performance with dramatically reduced parameter requirements.
...
TL;DR: DFE is an unsupervised method that learns a discrete facial expression dictionary from 3D mesh sequences using RVQ-VAE, outperforming FACS and other methods in psychological tasks like stress detection and depression detection.
...
Result: DFE captures more precise facial behaviors than FACS and outperforms both FACS-based pipelines and strong representation learning models in stress detection, personality prediction, and depression detection tasks.
...
TL;DR: Con-NRSfM is a novel method for non-rigid structure-from-motion that handles conformal deformations, eliminating strict assumptions and accurately computing local conformal scale while enabling precise depth estimation through a graph-based framework with parallel optimization.
...
Method: Uses point-wise reconstruction with 2D selected image warps optimized through a graph-based framework, decouples depth and conformal scale constraints, employs parallel separable iterative optimization, and incorporates a self-supervised learning framework with encoder-decoder network for dense 3D point clouds.

Result: Outperforms existing approaches in reconstruction accuracy and robustness on both synthetic and real datasets, demonstrating superior performance in handling conformal deformations.

Conclusion: Con-NRSfM provides an effective solution for NRSfM under conformal deformations, overcoming limitations of previous methods and achieving more accurate 3D reconstruction for deformable SLAM applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for robust 3D reconstruction from inconsistent images rely heavily on dense observations and complex optimization. The paper aims to address this limitation by simplifying the process through task decoupling.

Method: Proposes UniVerse framework that: 1) converts inconsistent images into initial videos, 2) uses a specially designed video diffusion model to restore consistency, 3) reconstructs 3D scenes from restored images. Uses diffusion model's learned scene prior instead of per-view degradation modeling.
...
Conclusion: UniVerse effectively addresses robust 3D reconstruction from inconsistent multi-view images by decoupling the problem and leveraging diffusion model priors, achieving better performance and generalization than existing methods.
...
TL;DR: A lightweight end-to-end framework for template matching that jointly estimates position, rotation, and scaling through template-aware dynamic convolution and geometric regression, achieving high precision and real-time performance in industrial applications.
...
Method: Proposes joint localization and geometric regression with Template-Aware Dynamic Convolution Module (TDCM) that injects template features dynamically. Uses depthwise separable convolutions, pixel shuffle, rotation-shear augmentation with pseudo labels, and a refinement module for local optimization.
...
Conclusion: The framework provides an efficient and deployable solution for real-time industrial template matching with explicit geometric pose estimation, overcoming limitations of both traditional and deep learning approaches.
...
TL;DR: A framework for adaptive pixel reasoning in VLMs that dynamically determines when to use pixel-level operations based on query difficulty, achieving better performance while reducing unnecessary visual operations.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with fine-grained visual understanding due to information loss during image encoding and insufficient attention to critical regions. Current pixel-level approaches are inefficient and often distracted by irrelevant details.

Method: Operation-aware supervised fine-tuning followed by rollout-guided reinforcement learning that enables VLMs to determine when to invoke pixel operations based on query difficulty, using feedback from the model's own responses.
...
Conclusion: The adaptive pixel reasoning framework successfully balances performance and efficiency by dynamically controlling pixel-level operations, demonstrating superior multimodal reasoning capabilities with significantly reduced computational overhead.
...
TL;DR: ASRS framework uses augmentation sensitivity to identify error-prone chest X-ray cases, improving fairness and reliability in medical AI by detecting hidden failures not captured by aggregate metrics.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for chest radiograph interpretation show uneven accuracy across patient subgroups and hidden failures not reflected in aggregate performance metrics, while existing error detection methods struggle with subtle within-distribution errors.

Method: Proposed augmentation-sensitivity risk scoring (ASRS) framework that applies clinically plausible rotations (±15°/±30°) and measures embedding shifts using the RAD-DINO encoder to stratify samples into stability quartiles based on sensitivity scores.

Result: Highly sensitive cases identified by ASRS show substantially lower recall (-0.2 to -0.3) despite high AUROC and confidence, demonstrating the framework's ability to detect error-prone cases that traditional metrics miss.

Conclusion: ASRS provides a label-free method for selective prediction and clinician review, improving fairness and safety in medical AI by identifying cases where the model is likely to make errors.
...
Result: FreeViS delivers higher stylization fidelity and superior temporal consistency, outperforming recent baselines and achieving strong human preference in evaluations.

Conclusion: The training-free pipeline offers a practical and economic solution for high-quality, temporally coherent video stylization without requiring expensive training or paired video data.
...
TL;DR: MedQ-Bench is a comprehensive benchmark for medical image quality assessment using multimodal LLMs, introducing perception and reasoning tasks across five imaging modalities with 2,600 perceptual queries and 708 reasoning assessments.


<details>
  <summary>Details</summary>
Motivation: Existing medical IQA approaches are limited by scalar score-based metrics and fail to capture human-like descriptive reasoning processes used by clinical experts, creating a gap in effective AI evaluation.

Method: Introduces MedQ-Bench with two tasks: MedQ-Perception (low-level visual attribute questions) and MedQ-Reasoning (no-reference and comparison tasks). Uses multi-dimensional judging protocol across four axes and validates with radiologist alignment.

Result: Evaluation of 14 state-of-the-art MLLMs shows preliminary but unstable perceptual and reasoning skills with insufficient accuracy for reliable clinical use.

Conclusion: Targeted optimization of MLLMs is needed for medical IQA, and MedQ-Bench aims to catalyze further exploration of MLLMs' potential in medical image quality evaluation.
...
TL;DR: InstaFormer is a network that predicts full occlusion and depth orderings for all instances in a scene from a single RGB image input, eliminating the need for expensive annotations and multiple forward passes.


<details>
  <summary>Details</summary>
Motivation: Current methods for understanding instance-wise geometries require expensive input formats (category labels, segmentation masks) and inference costs (quadratic forward passes), which limits their practical application.
...
Conclusion: InstaFormer provides an efficient solution for instance-wise geometry understanding by eliminating the need for expensive annotations and reducing inference costs to a single forward pass.
...
TL;DR: PyramidStyler is a transformer framework with Pyramidal Positional Encoding that improves neural style transfer by capturing multi-scale details while reducing computational load, achieving faster convergence and better results than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing CNN and transformer-based neural style transfer models struggle with scaling to complex styles and high-resolution inputs efficiently, creating a need for more scalable and computationally efficient approaches.
...
Result: Achieved 62.6% reduction in content loss (to 2.07) and 57.4% reduction in style loss (to 0.86) after 4000 epochs, with 1.39s inference time. With RL, further improved to content loss 2.03 and style loss 0.75 with minimal speed penalty (1.40s).

Conclusion: PyramidStyler enables real-time, high-quality artistic rendering with broad applications in media and design, demonstrating superior performance over existing neural style transfer methods.
...
<details>
  <summary>Details</summary>
Motivation: Scaling 3D Gaussian Splatting (3DGS) to large and unbounded scenes like city blocks is challenging due to memory pressure and computational inefficiencies in existing divide-and-conquer methods, which suffer from load imbalance and high overhead in coarse-to-fine pipelines.

Method: LoBE-GS introduces: (1) depth-aware partitioning that reduces preprocessing time from hours to minutes, (2) optimization-based strategy to balance visible Gaussians across blocks, and (3) two lightweight techniques - visibility cropping and selective densification to reduce training cost.

Result: Evaluations on large-scale urban and outdoor datasets show LoBE-GS achieves up to 2× faster end-to-end training time than state-of-the-art baselines while maintaining reconstruction quality and enabling scalability to scenes infeasible with vanilla 3DGS.

Conclusion: LoBE-GS successfully addresses the scalability limitations of 3DGS for large scenes through its load-balanced and efficient framework, making large-scale 3D reconstruction more practical and efficient.
...
Method: MemoryPack: learnable context-retrieval mechanism using textual and image information as global guidance; Direct Forcing: efficient single-step approximating strategy for better training-inference alignment.

Result: Achieves minute-level temporal consistency, scales gracefully with video length, maintains linear complexity, and improves context consistency and reliability.

Conclusion: MemoryPack and Direct Forcing substantially enhance autoregressive video models for practical long-form video generation.
...
TL;DR: This paper addresses confidence calibration for 3D object detectors, proposing new metrics and loss terms to improve calibration of both dominant and secondary class predictions.


<details>
  <summary>Details</summary>
Motivation: Precise object detection and uncertainty estimation are critical for safe autonomous systems, requiring proper confidence calibration in 3D object detectors.

Method: Proposed two auxiliary regularizing loss terms for calibration of dominant prediction or full prediction vector, evaluated on CenterPoint, PillarNet and DSVT-Pillar detectors.

Result: Combining the full-class prediction loss term with isotonic regression achieved best calibration for CenterPoint and PillarNet, but DSVT-Pillar could not be jointly calibrated for both dominant and secondary predictions.

Conclusion: Different detectors may require different calibration approaches, and the proposed methods effectively improve calibration for some 3D object detectors.
...
TL;DR: DiffPS leverages pre-trained diffusion models to address person search by separating detection and re-identification tasks, achieving state-of-the-art performance on CUHK-SYSU and PRW datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods use ImageNet pre-trained backbones that are suboptimal for capturing spatial context and fine-grained identity cues, and suffer from optimization conflicts between detection and re-identification tasks using shared backbone features.
...
Conclusion: Leveraging diffusion model priors while eliminating optimization conflicts between detection and re-identification tasks significantly improves person search performance.
...
TL;DR: FMU integrates flow matching with deep unfolding for hyperspectral image reconstruction, achieving superior results through generative priors and velocity loss.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging is costly and challenging to reconstruct from compressed measurements, with existing methods struggling with degradation and spectral detail loss.

Method: Proposes Flow-Matching-guided Unfolding network (FMU) that embeds flow matching's generative prior into deep unfolding framework with mean velocity loss for global consistency.

Result: Extensive experiments show FMU significantly outperforms existing approaches in reconstruction quality on both simulated and real datasets.
...
<details>
  <summary>Details</summary>
Motivation: Conventional defect detection in industry components is time-consuming, labor-intensive, and difficult for quality management, creating burden on inspection personnel.

Method: Uses digital camera optics and DL-based models (YOLOv3, v4, v7, v9) with ConSinGAN for data augmentation to address lack of defective images. Focuses on surface and pin-leg defects in DIP components.
...
Conclusion: The automated defect detection system can be easily established for various defect types and works well even with insufficient defect data. SCADA system integration demonstrates practical industrial applicability.
...
TL;DR: FoundAD is a few-shot anomaly detection method that uses foundation visual encoders to learn a nonlinear projection operator for identifying anomalies by measuring embedding differences from normal image manifolds.


<details>
  <summary>Details</summary>
Motivation: Few-shot anomaly detection faces challenges in differentiating normal and abnormal features with limited samples, especially under category-agnostic conditions. Foundation visual encoders trained on large datasets can learn general normal image distributions.

Method: The method learns a nonlinear projection operator onto the natural image manifold. It uses the correlation between anomaly amount and embedding differences from foundation encoders to detect out-of-distribution regions in images.

Result: Extensive experiments show competitive performance in multi-class detection while using substantially fewer parameters than prior methods. Evaluations with multiple foundation encoders including DINOv3 demonstrate effectiveness.

Conclusion: The approach broadens perspectives on foundation features and advances few-shot anomaly detection by effectively leveraging pre-trained encoders for anomaly identification.
...
TL;DR: ClustViT is an efficient Vision Transformer for semantic segmentation that reduces computational complexity through token clustering and regeneration while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers have high accuracy but quadratic attention complexity limits their practical use in robotics. Token merging works for classification but is less suitable for dense prediction tasks like segmentation.

Method: Extends ViT backbone with a trainable Cluster module that merges similar tokens using pseudo-clusters from segmentation masks, and a Regenerator module that restores fine details for downstream processing.
...
Conclusion: ClustViT provides an efficient solution for semantic segmentation with Vision Transformers, making them more practical for real-world robotic applications through significant computational savings.
...
TL;DR: PaDT introduces a unified paradigm for MLLMs to directly generate both textual and visual outputs using Visual Reference Tokens (VRTs) interleaved with text, enabling dense prediction tasks like detection and segmentation without relying on indirect representations.
...
Method: PaDT uses Visual Reference Tokens (VRTs) derived from visual patch embeddings, interleaved with LLM's textual tokens. A lightweight decoder transforms outputs into detection, segmentation, and grounding predictions. VRTs are processed independently and the embedding table is dynamically expanded. Training involves random VRT selection and per-token cross-entropy loss.
...
Conclusion: PaDT provides a unified approach that enables MLLMs to directly generate diverse visual outputs, overcoming limitations of indirect representations and achieving superior performance on multiple vision tasks.
...
TL;DR: This paper addresses the trust deficit in online fruit/vegetable e-commerce by proposing a 'Trust Pyramid' model and 'Triangular Trust Index' (TTI) to quantify trade-offs in agricultural grading. It introduces TriAlignXA, an explainable AI framework that redefines algorithms as transparent decision-making providers.


<details>
  <summary>Details</summary>
Motivation: The 'trust deficit' in online produce e-commerce stems from inability to provide direct sensory perception of product quality through digital transactions.

Method: Constructs 'Trust Pyramid' model through dual-source verification. Proposes 'Triangular Trust Index' (TTI) and TriAlignXA framework with three engines: Bio-Adaptive Engine for quality description, Timeliness Optimization Engine for efficiency, and Economic Optimization Engine for cost control. Uses 'Pre-Mapping Mechanism' with QR codes for transparent quality information.

Result: Experiments on grading tasks show significantly higher accuracy than baseline models. Empirical evidence and theoretical analysis verify the framework's balancing capability in addressing the 'impossible triangle' of agricultural product grading.

Conclusion: The research provides comprehensive support from theory to practice for building trustworthy online produce ecosystems, establishing a critical pathway from algorithmic decision-making to consumer trust.
...
TL;DR: 4DGS-Craft is a consistent and interactive 4D Gaussian Splatting editing framework that addresses view, temporal, and non-editing region consistency issues through 4D-aware InstructPix2Pix, multi-view refinement, Gaussian selection, and LLM-based intent understanding.


<details>
  <summary>Details</summary>
Motivation: To overcome challenges in 4D Gaussian Splatting editing including view inconsistency, temporal inconsistency, non-editing region inconsistency, and difficulty handling complex text instructions.

Method: Uses 4D-aware InstructPix2Pix with 4D VGGT geometry features, multi-view grid module for iterative refinement, Gaussian selection mechanism for region-specific optimization, and LLM-based module for intent understanding and operation decomposition.

Result: Enables more consistent and controllable 4D scene editing compared to related works, with improved handling of complex user instructions.

Conclusion: 4DGS-Craft provides a comprehensive solution for consistent and interactive 4D Gaussian Splatting editing, effectively addressing key challenges through integrated geometric awareness, multi-view consistency, and intelligent intent interpretation.
...
TL;DR: Pure-Pass (PP) is a pixel-level masking mechanism that identifies pure pixels to exempt them from expensive computations in image super-resolution, improving efficiency and performance when integrated into ATD-light models.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight SR methods like CAMixer have limitations including poor adaptability, coarse-grained masking, and spatial inflexibility. The authors aim to address these issues with a more fine-grained approach.

Method: PP uses fixed color center points to classify pixels into distinct categories, enabling fine-grained, spatially flexible masking. This pixel-level mechanism identifies pure pixels that can skip expensive computations while maintaining adaptive flexibility.

Result: PP-ATD-light achieves superior SR performance with minimal overhead, outperforming CAMixer-ATD-light in both reconstruction quality and parameter efficiency while saving similar amounts of computation.

Conclusion: The proposed Pure-Pass mechanism provides an effective solution for improving computational efficiency in image super-resolution through fine-grained pixel-level masking, demonstrating better performance than previous approaches.
...
Method: Implemented a 10-step SLSO framework including image analysis, structured data generation, tooth number extraction and consistency checking, iterative regeneration when inconsistencies detected, and finding generation with restructuring and verification. Compared with conventional Chain-of-Thought method.

Result: SLSO framework improved output accuracy: 66.9% for tooth number, 33.3% for tooth movement, and 28.6% for root resorption. Successful cases achieved consistent structured output after up to five regenerations. The framework enforced negative finding descriptions and suppressed hallucinations.

Conclusion: The SLSO framework shows promise for improving automated finding generation but requires further refinement for extensive lesions and overall performance enhancement to become practical.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient 3D autoencoder for LiDAR point clouds that reduces computational resources while maintaining performance compared to state-of-the-art architectures.

Method: Proposed LiLa-Net with reduced encoder layers and simplified skip connections, balancing skip connection information with latent encoding to improve reconstruction quality.

Result: The model accurately reconstructs original point clouds and demonstrates strong generalization by successfully reconstructing objects unrelated to the original traffic environment.

Conclusion: LiLa-Net achieves an effective balance between computational efficiency and reconstruction performance, with robust generalization capabilities beyond the training domain.
...
TL;DR: kabr-tools is an open-source package that uses drones and machine learning to automate multi-species behavioral monitoring, improving data granularity and enabling large-scale ecological studies.
...
Method: The framework integrates drone-based video with machine learning systems including object detection, tracking, and behavioral classification to extract behavioral, social, and spatial metrics from wildlife footage.

Result: Drone-based observations significantly improved behavioral granularity, reducing visibility loss by 15% and capturing more transitions with higher accuracy. Case studies analyzed 969 behavioral sequences, revealing species-specific behavioral patterns and spatial segregation.
...
TL;DR: GaussianMorphing is a novel framework for semantic-aware 3D shape and texture morphing from multi-view images using mesh-guided 3D Gaussian Splatting with unified deformation and unsupervised semantic correspondence.
...
Method: Leverages mesh-guided 3D Gaussian Splatting with unified deformation strategy that anchors 3DGaussians to reconstructed mesh patches, ensuring geometrically consistent transformations and preserving texture fidelity through topology-aware constraints. Uses mesh topology as geometric prior for unsupervised semantic correspondence.

Result: Outperforms prior 2D/3D methods on TexMorph benchmark, reducing color consistency error (ΔE) by 22.2% and EI by 26.2%.

Conclusion: The integrated approach preserves both local detail and global semantic coherence throughout the morphing process without requiring labeled data, achieving superior performance in 3D shape and texture morphing.
...
TL;DR: InPose: A zero-shot generalization method for human pose estimation using conditional diffusion models that condition on rotational measurements alone and guide priors with location-based likelihood terms.
...
Method: Formulate pose estimation as an inverse problem using pre-trained diffusion model conditioned on rotational measurements alone, guided by likelihood terms derived from measured locations.
...
Conclusion: The approach enables zero-shot generalization across users by decoupling body size dependencies from the conditioning process.
...
TL;DR: VGDM is a transformer-driven diffusion model that improves brain tumor segmentation by combining global contextual reasoning with iterative denoising, outperforming traditional U-Net approaches.
...
Method: Embed a vision transformer at the core of the diffusion process to leverage global contextual reasoning with iterative denoising. The transformer backbone models spatial relationships across entire MRI volumes while diffusion refinement mitigates voxel-level errors.

Result: Experimental validation on MRI brain tumor datasets demonstrates consistent gains in Dice similarity and Hausdorff distance metrics compared to conventional baselines.

Conclusion: The hybrid transformer-diffusion design provides improved robustness and scalability in neuro-oncology, advancing the state of the art in tumor segmentation beyond U-Net approaches.
...
TL;DR: Developed a scalable deep learning pipeline using dual-pass U-Net to extract urban footprints from historical French maps (1925-1950), creating the first national-scale urban dataset for this period with 73% accuracy.
...
Method: Dual-pass U-Net approach: first pass identifies confusing areas (text, roads) to guide data augmentation; second pass uses refined dataset and binarized first output to minimize radiometric noise. Deployed on HPC cluster to process 941 high-resolution tiles.

Result: Successfully processed entire metropolitan France, achieving 73% overall accuracy. Created first open-access national-scale urban footprint dataset for 1925-1950 period, effectively capturing diverse urban patterns while reducing false positives from artifacts.

Conclusion: The method successfully bridges the data gap for historical urban analysis in France, providing a scalable solution for extracting urban areas from complex historical maps. Code, datasets, and nationwide urban raster are openly released to support urbanization research.
...
Result: Point-based tracking performs well for surgical tools but consistently fails for anatomical targets due to tissue similarity and ambiguous boundaries.

Conclusion: Point-based tracking is reliable for surgical tools but problematic for anatomical structures; actionable recommendations are provided for selecting and placing tracking points to improve performance.
...
TL;DR: FRIEREN is a federated learning framework that addresses domain shifts in semantic segmentation using unlabeled client data by leveraging vision foundation models and vision-language integration.


<details>
  <summary>Details</summary>
Motivation: Federated learning for semantic segmentation faces challenges with domain shifts when client data is unlabeled, and existing methods either assume labeled client data or don't utilize modern vision foundation models effectively.

Method: Uses a Vision-Language decoder guided by CLIP-based text embeddings for semantic disambiguation, and employs weak-to-strong consistency learning for robust local training on pseudo-labels without accessing source data.
...
Conclusion: FRIEREN effectively tackles the challenging FFREEDG task and sets a strong baseline for future research in federated learning with unlabeled client data and domain shifts.
...
TL;DR: ASK-Hint is a structured prompting framework that uses action-centric knowledge to improve video anomaly detection in frozen vision-language models by providing fine-grained guiding questions.
...
Result: Extensive experiments on major 3D benchmarks show GeoPurify achieves or surpasses state-of-the-art performance while using only about 1.5% of the training data, effectively mitigating the trade-off between accuracy and data efficiency.

Conclusion: GeoPurify successfully exploits latent geometric information in noisy 2D features through learned affinity networks, achieving superior 3D semantic segmentation with dramatically reduced training data requirements.
...
TL;DR: Non-invasive pig identification using auricular vein patterns achieves 98.12% accuracy with SVM classification, providing a cost-effective alternative to traditional methods for small-scale farmers.
...
TL;DR: A multicategory counting framework using Twins pyramid vision-transformer with multi-class counting head and Category Focus Module, achieving superior performance on crowd counting benchmarks and demonstrating applicability to biodiversity monitoring.


<details>
  <summary>Details</summary>
Motivation: To address object counting in dense and occluded scenes where discrete counting-by-detection methods fail, particularly for multicategory scenarios with inter-category interference.

Method: Uses Twins pyramid vision-transformer backbone with multi-class counting head on multiscale decoding approach, plus a segmentation-based Category Focus Module to suppress inter-category cross-talk during training.

Result: Achieved 33%, 43% and 64% reduction in MAE on VisDrone and iSAID benchmarks versus prior multicategory crowd-counting approaches, and demonstrated superiority over YOLOv11 in dense scenes.

Conclusion: The method's regional loss enables multi-class crowd counting in new domains, successfully applied to biodiversity monitoring datasets to support conservation efforts and scalable ecological insights.
...
TL;DR: TempoControl enables fine-grained temporal control in text-to-video generation by guiding cross-attention maps during inference without retraining, allowing precise timing control of visual elements.
...
Method: Uses cross-attention maps from text-to-video diffusion models and guides timing through a novel optimization approach based on three principles: temporal shape alignment (correlation), amplification for visibility (energy), and spatial focus maintenance (entropy).

Result: Achieves precise control over timing while maintaining high video quality and diversity, demonstrated in applications like temporal reordering for single/multiple objects, and action/audio-aligned generation.

Conclusion: TempoControl provides effective temporal alignment of visual concepts during inference without requiring retraining or additional supervision, enabling enhanced control in video generation.
...
TL;DR: RewardMap is a multi-stage RL framework that addresses fine-grained visual reasoning challenges in MLLMs through dense reward signals and difficulty-aware training, achieving significant performance improvements across multiple benchmarks.
...
Conclusion: RewardMap effectively enhances visual understanding and reasoning capabilities of MLLMs through its dense reward design and multi-stage training approach.
...
<details>
  <summary>Details</summary>
Motivation: Previous drag-based editing methods suffered from distortions due to insufficient priors from Stable Diffusion. With the shift to stronger DiT-based models like FLUX, there's an opportunity to improve drag editing, but existing point-based approaches don't work well with DiT features.

Method: DragFlow introduces region-based editing with affine transformations for richer feature supervision, integrates pretrained personalization adapters (IP-Adapter) for subject consistency, uses gradient mask-based constraints for background preservation, and employs MLLMs to resolve task ambiguities.

Result: Extensive experiments on DragBench-DR and the novel ReD Bench show that DragFlow surpasses both point-based and region-based baselines, setting new state-of-the-art performance in drag-based image editing.

Conclusion: DragFlow successfully leverages FLUX's strong priors for drag-based editing through region-based supervision and personalization techniques, achieving superior performance over existing methods and establishing a new benchmark for the field.
...
TL;DR: F2C addresses the "needle in a haystack" problem in Video LLMs by selecting key clips instead of isolated frames, using adaptive resolution to maintain computational efficiency while preserving temporal dynamics.


<details>
  <summary>Details</summary>
Motivation: Current Video LLMs suffer from token overload from raw video frames, and existing frame-wise selection methods discard essential temporal dynamics, leading to poor reasoning about motion and event continuity.

Method: Proposes F2C approach that extends selection from key frames to key clips (short temporally coherent segments) with adaptive resolution strategy that dynamically balances spatial resolution and clip length to maintain constant token count.

Result: Outperforms uniform sampling by 8.1% on Video-MME, 5.6% on LongVideoBench, and 10.3% on MLVU benchmarks, demonstrating improved video understanding while maintaining computational budget.

Conclusion: Preserving temporal coherence through key clip selection is crucial for video understanding, providing a practical pathway for scaling Video LLMs to real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: To enable accurate human movement assessment outside specialized labs for telemedicine, sports science, and rehabilitation by comparing video-based and IMU-based approaches.

Method: Used VIDIMU dataset with 13 daily activities captured by commodity cameras and 5 IMUs. Evaluated joint angles from MotionAGFormer, MotionBERT, MMPose, and NVIDIA BodyTrack against IMU data processed with OpenSim inverse kinematics.
...
Conclusion: Video-based models can provide clinically promising kinematics in healthy adults, with key trade-offs between video and sensor approaches in cost, accessibility, and precision. Study provides guidelines for telehealth and remote monitoring solutions.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for reconstructing visual information from brain activity face challenges with cross-subject generalization due to neural variability and the brain's abstract semantic encoding of complex visual inputs.

Method: Integrates complementary adapters via diffusion: AutoKL for low-level features and CLIP for semantics. Uses cross-subject transfer learning by pretraining on one subject and fine-tuning only 17% of parameters (fully connected layers) for new subjects.
...
Conclusion: NeuroSwift provides an efficient and effective solution for cross-subject visual stimulus reconstruction from fMRI data, addressing key challenges in neural decoding.
...
TL;DR: microCLIP is a self-training framework that refines CLIP's visual and textual representations for fine-grained image classification using saliency-guided attention and dynamic knowledge aggregation.


<details>
  <summary>Details</summary>
Motivation: CLIP's reliance on coarse global features limits its performance on fine-grained classification tasks, and previous approaches overlook spatial precision when aligning LLM descriptions with CLIP.

Method: Uses Saliency-Oriented Attention Pooling (SOAP) in a TokenFusion module to create saliency-guided fine-grained tokens, combined with a two-headed LLM-derived classifier and Dynamic Knowledge Aggregation for stable pseudo-labeling.

Result: Achieves a consistent 2.90% average accuracy gain across 13 fine-grained benchmarks while requiring only light adaptation.
...
Conclusion: VidGuard-R1 successfully addresses the need for both accurate detection and interpretable explanations in AI-generated video authentication, setting a new standard for transparency and performance in video authenticity detection.
...
<details>
  <summary>Details</summary>
Motivation: Address the quality degradation issue in long video generation caused by error compounding in continuous latent spaces when extending beyond training horizons.
...
Conclusion: The approach effectively mitigates quality degradation in long-horizon video generation without requiring long-video supervision or retraining, achieving unprecedented length scaling while maintaining quality.
...
Conclusion: KineMask demonstrates the complementary roles of low- and high-level conditioning in video diffusion models, achieving physically plausible video generation with realistic object control and interactions.
...
TL;DR: The paper introduces fine-grained multimodal actions for real-time robot control, incorporating proprioception, kinesthesia, force haptics, and muscle activation to improve simulation accuracy and reduce temporal drift.


<details>
  <summary>Details</summary>
Motivation: Current video models lack fine-grained control needed for household robots to handle delicate tasks and urgent situations, requiring real-time fine motor control capabilities.

Method: Developed a feature learning paradigm that aligns multimodal senses (proprioception, kinesthesia, force haptics, muscle activation) while preserving unique information, with a regularization scheme to enhance causality of action trajectory features.

Result: Experiments show improved simulation accuracy and reduced temporal drift when incorporating multimodal senses. Extensive ablation studies and downstream applications demonstrate effectiveness and practicality.

Conclusion: Multimodal sensory integration enables fine-grained interactions that are difficult to simulate with text-conditioned models, providing better control for household robot applications.
...
<details>
  <summary>Details</summary>
Motivation: Video understanding in multimodal models is limited by context length, causing missed key transition frames and poor coherence across long time scales.

Method: Adapt Native Sparse Attention (NSA) to Qwen2.5-VL through end-to-end training on 216K video instruction dataset, using hardware-aware hybrid attention (dense for text, sparse for video).

Result: Outperforms token-compression and training-free sparse baselines on long-video understanding, temporal reasoning, and spatial benchmarks.
...
Method: Identifies that noise schedulers have unequal perceptual effects across resolutions, then recalibrates noise levels of denoiser conditioned on resolution size without changing model architecture or sampling schedule.
...
<details>
  <summary>Details</summary>
Motivation: To enable prediction of dynamic physical properties that require temporal information from video data, addressing gaps in current computer vision capabilities for physical reasoning.

Method: Collected new video datasets for three physical properties with synthetic and real splits. Explored three approaches: oracle methods using classical computer vision, prompt-based methods with video foundation models, and prompting strategies for MLLMs.

Result: Video foundation models (generative and self-supervised) achieved similar performance, though behind oracle methods. MLLMs were inferior but could be improved with suitable prompting.

Conclusion: Video foundation models show promise for predicting dynamic physical properties, while MLLMs need further development and better prompting strategies for this task.
...
TL;DR: The paper introduces sounding object detection task to evaluate models' ability to link sounds to objects involved in interactions, using a multimodal object-aware framework trained on egocentric videos with automatic object segmentation masks and slot attention visual encoder.


<details>
  <summary>Details</summary>
Motivation: Everyday object interactions produce unique sounds, and the paper aims to develop models that can distinguish between sounds from different objects (e.g., spoon hitting hardwood vs carpeted floor) by linking sounds to the specific objects involved.

Method: Multimodal object-aware framework using in-the-wild egocentric videos, with automatic pipeline for computing object segmentation masks to guide model focus, and slot attention visual encoder to enforce object prior.

Result: State-of-the-art performance on the new sounding object detection task and existing multimodal action understanding tasks.

Conclusion: The proposed framework successfully links sounds to objects involved in interactions, demonstrating improved performance through object-centric approaches with segmentation guidance and slot attention mechanisms.
...
TL;DR: The paper analyzes 3D Gaussian Splatting (3DGS) vulnerabilities to poisoning attacks and proposes a novel density-guided attack method that injects Gaussian points into low-density regions using Kernel Density Estimation (KDE), creating viewpoint-dependent illusory objects while maintaining stealth.


<details>
  <summary>Details</summary>
Motivation: As 3D scene representation methods like NeRF and 3DGS become prevalent, addressing their security vulnerabilities becomes critical. The authors aim to analyze 3DGS robustness against poisoning attacks and develop effective attack methods.

Method: Proposes a density-guided poisoning method that uses Kernel Density Estimation (KDE) to identify low-density regions, strategically injects Gaussian points to create viewpoint-dependent illusory objects, and employs an adaptive noise strategy to disrupt multi-view consistency.
...
Conclusion: The work demonstrates significant vulnerabilities in 3DGS to poisoning attacks, proposes an effective attack methodology, and introduces a KDE-based evaluation protocol for systematic assessment of attack difficulty, enabling objective benchmarking for future security research.
...
TL;DR: The paper introduces FOCUS, a theoretical framework and algorithms for improving multi-subject fidelity in text-to-image models by formulating subject disentanglement as a stochastic optimal control problem over flow matching samplers.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models struggle with multi-subject prompts, showing issues like attribute leakage, identity entanglement, and subject omissions. There's a need for principled methods to improve multi-subject generation fidelity.
...
### [168] [On the Role of Domain Experts in Creating Effective Tutoring Systems](https://arxiv.org/abs/2510.01432)
*Sarath Sreedharan,Kelsey Sikes,Nathaniel Blanchard,Lisa Mason,Nikhil Krishnaswamy,Jill Zarestky*

Main category: cs.AI

TL;DR: The paper explores how domain expert knowledge can enhance AI tutoring systems through two approaches: using XAI techniques to automatically generate lessons from expert rules, and leveraging expert-curated curricula to develop more adaptive and efficient tutoring systems.


<details>
  <summary>Details</summary>
Motivation: To highlight the overlooked role of highly curated domain expert knowledge in creating effective AI tutoring systems, demonstrating how expert input can improve educational technology.

Method: Two approaches: 1) Using explainable AI (XAI) techniques with expert-specified rules to automatically generate lessons, 2) Utilizing expert-curated curricula to develop adaptive tutoring systems with more efficient algorithms. A case study on pollinator identification demonstrates the application.

Result: The paper presents a framework for integrating expert knowledge into AI tutoring systems, showing how XAI can be repurposed from debugging to lesson generation and how expert curricula enable more adaptive and algorithmically efficient systems.

Conclusion: Domain expert knowledge is crucial for developing effective AI tutoring systems, and the proposed methods demonstrate practical ways to incorporate such knowledge to create novel educational technologies that provide better learning experiences.
...
### [169] [VaPR -- Vision-language Preference alignment for Reasoning](https://arxiv.org/abs/2510.01700)
*Rohan Wadhawan,Fabrice Y Harel-Canada,Zi-Yi Dou,Suhaila Shakiah,Robinson Piramuthu,Nanyun Peng*

Main category: cs.AI

TL;DR: The paper introduces VaPR, a hard-negative response generation framework using LLM-guided editing to create rejected responses with targeted errors while maintaining stylistic and length similarity to accepted responses, addressing noise in synthetic preference annotations for LVLM alignment.


<details>
  <summary>Details</summary>
Motivation: Existing preference finetuning methods like DPO overlook the prevalence of noise in synthetic preference annotations, particularly stylistic and length biases in AI-generated feedback for aligning Large Vision-Language Models.
...
Result: VaPR models achieved significant performance improvements across ten benchmarks: average gains of 6.5% (LLaVA), 4.0% (Qwen2VL), and 1.5% (Qwen2.5VL), with notable improvements on reasoning tasks. Also reduced tendency to answer "Yes" in binary questions and showed consistent performance scaling with data size.

Conclusion: The framework effectively addresses noise in synthetic preference annotations and generalizes to open-source LLMs as editors, with VaPR-OS achieving ~99% performance of GPT-4o synthesized models, providing a robust solution for LVLM alignment.
...
### [170] [Agentic Jigsaw Interaction Learning for Enhancing Visual Perception and Reasoning in Vision-Language Models](https://arxiv.org/abs/2510.01304)
...
TL;DR: AGILE is an agentic jigsaw interaction learning method that enhances visual perception and reasoning in VLMs through interactive problem-solving, achieving significant performance improvements on jigsaw tasks and strong generalization across multiple vision tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLMs have limited perceptual and reasoning abilities, performing poorly even on simple jigsaw tasks. High-quality vision-language data for training is scarce and not scalable, creating constraints for improving these fundamental capabilities.

Method: AGILE formulates jigsaw solving as an interactive process where the model generates executable code to perform actions based on current state, and the environment provides fine-grained visual feedback. This creates an iterative cycle of observation and interaction for incremental improvement.

Result: AGILE substantially boosts performance on jigsaw tasks (increasing accuracy from 9.5% to 82.8% under 2×2 setting) and demonstrates strong generalization across 9 general vision tasks with average improvement of 3.1%, indicating enhanced perceptual and reasoning abilities.

Conclusion: AGILE provides an efficient, scalable solution to multimodal data scarcity and opens new avenues for advancing reasoning and generalization in multimodal models through interactive learning approaches.
...
### [171] [Aristotle: IMO-level Automated Theorem Proving](https://arxiv.org/abs/2510.01346)
...
<details>
  <summary>Details</summary>
Motivation: To advance automated theorem proving by combining formal verification methods with informal reasoning capabilities to solve complex mathematical problems at the level of top human mathematicians.
...
Conclusion: The Aristotle system successfully combines formal and informal reasoning approaches to achieve human-level performance in automated theorem proving, representing a significant advancement in AI mathematical reasoning capabilities.
...
### [172] [MEMTRACK: Evaluating Long-Term Memory and State Tracking in Multi-Platform Dynamic Agent Environments](https://arxiv.org/abs/2510.01353)
*Darshan Deshpande,Varun Gangal,Hersh Mehta,Anand Kannappan,Rebecca Qian,Peng Wang*

Main category: cs.AI

TL;DR: MEMTRACK is a benchmark for evaluating long-term memory and state tracking in multi-platform agent environments, focusing on realistic organizational workflows across platforms like Slack, Linear, and Git.
...
Method: MEMTRACK models organizational workflows with asynchronous events across multiple platforms, providing chronologically interleaved timelines with noisy, conflicting, and cross-referring information. Dataset curated through manual expert design and agent-based synthesis.

Result: Experiments show challenges in long-horizon memory utilization, cross-platform dependencies, and contradiction resolution. Best performing GPT-5 model achieves only 60% Correctness score.
...
### [173] [Is It Thinking or Cheating? Detecting Implicit Reward Hacking by Measuring Reasoning Effort](https://arxiv.org/abs/2510.01367)
*Xinpeng Wang,Nitish Joshi,Barbara Plank,Rico Angell,He He*

Main category: cs.AI

TL;DR: TRACE (Truncated Reasoning AUC Evaluation) detects implicit reward hacking by measuring how early a model's reasoning becomes sufficient to pass verification, identifying shortcuts that bypass chain-of-thought monitors.


<details>
  <summary>Details</summary>
Motivation: Reward hacking poses a significant threat where models exploit loopholes in reward functions without solving intended tasks, particularly through implicit hacking where chain-of-thought appears benign but bypasses monitors.

Method: TRACE progressively truncates a model's chain-of-thought at various lengths, forces the model to answer, and measures verifier-passing rates at each cutoff. It quantifies effort by the area under the accuracy-vs-length curve.
...
### [174] [Fine-tuning with RAG for Improving LLM Learning of New Skills](https://arxiv.org/abs/2510.01375)
*Humaid Ibrahim,Nikolai Rozanov,Marek Rei*

Main category: cs.AI

TL;DR: A distillation pipeline that converts inference-time retrieval into learned competence by extracting hints from failures, generating improved teacher trajectories, and training student models to internalize retrieval benefits without runtime dependencies.


<details>
  <summary>Details</summary>
Motivation: LLM agents frequently fail in predictable ways during multi-step tasks, and while RAG can help, it requires maintaining external knowledge and adds computational overhead at every deployment.

Method: Three-step approach: (1) extract compact hints from agent failures, (2) generate improved teacher trajectories via one-shot retrieval at episode start, (3) train student models on these trajectories with hints removed to force internalization.

Result: Distilled students outperform baselines, achieving 91% success on ALFWorld (vs. 79%) and improving WebShop scores to 72 (vs. 61), while using 10-60% fewer tokens than retrieval-augmented teachers.
...
### [175] [VOGUE: Guiding Exploration with Visual Uncertainty Improves Multimodal Reasoning](https://arxiv.org/abs/2510.01444)
*Rui Liu,Dian Yu,Tong Zheng,Runpeng Dai,Zongxia Li,Wenhao Yu,Zhenwen Liang,Linfeng Song,Haitao Mi,Pratap Tokekar,Dong Yu*

Main category: cs.AI

TL;DR: VOGUE is a novel method that shifts exploration from text to visual space by treating images as stochastic contexts, using symmetric KL divergence to quantify policy sensitivity to visual perturbations and guide uncertainty-aware exploration.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods for MLLMs treat visual input as fixed and deterministic, overlooking visual ambiguity and struggling to build policies robust to visual variations, which limits exploration effectiveness.

Method: VOGUE quantifies policy sensitivity to visual perturbations using symmetric KL divergence between 'raw' and 'noisy' image branches, creates an uncertainty-proportional bonus in the learning objective, combines with token-entropy bonus and annealed sampling schedule to balance exploration and exploitation.

Result: Implemented on Qwen2.5-VL-3B/7B models, VOGUE boosts pass@1 accuracy by 2.6% on visual math benchmarks and 3.7% on general-domain reasoning benchmarks, while improving pass@4 performance and mitigating exploration decay in RL fine-tuning.

Conclusion: Grounded exploration in the inherent uncertainty of visual inputs is an effective strategy for improving multimodal reasoning in MLLMs.
...
### [176] [Information Seeking for Robust Decision Making under Partial Observability](https://arxiv.org/abs/2510.01531)
...
<details>
  <summary>Details</summary>
Motivation: Existing LLM planning agents overlook discrepancies between their internal dynamics and actual environments when dealing with observational uncertainty, limiting their effectiveness in practical problem-solving scenarios.
...
### [177] [The Reasoning Boundary Paradox: How Reinforcement Learning Constrains Language Models](https://arxiv.org/abs/2510.02230)
*Phuc Minh Nguyen,Chinh D. La,Duy M. H. Nguyen,Nitesh V. Chawla,Binh T. Nguyen,Khoa D. Doan*

Main category: cs.AI

TL;DR: RLVR (Reinforcement Learning with Verifiable Rewards) can paradoxically shrink reasoning boundaries in LLMs due to negative interference and winner-take-all phenomena, but focusing on low-likelihood problems improves performance.


<details>
  <summary>Details</summary>
Motivation: To investigate why RLVR, despite being designed to improve reasoning, actually shrinks reasoning boundaries in Large Language Models rather than expanding them.

Method: Analyzed RLVR learning dynamics through theoretical and empirical analysis on mathematical reasoning benchmarks, revealing negative interference and winner-take-all phenomena, then proposed a data curation algorithm focusing on low-likelihood problems.

Result: Identified that RLVR causes negative interference (learning some problems reduces solution likelihood for others) and winner-take-all (reinforces high-likelihood problems while suppressing low-likelihood ones), leading to Pass@k performance decline. The proposed data curation method achieved notable Pass@k improvement.

Conclusion: RLVR's shrinkage effect stems from on-policy sampling in standard RL objectives, causing convergence to narrow solution strategies. Focusing RLVR on low-likelihood problems through data curation effectively mitigates this issue and improves reasoning performance.
...
### [178] [InvThink: Towards AI Safety via Inverse Reasoning](https://arxiv.org/abs/2510.01569)
*Yubin Kim,Taehan Kim,Eugene Park,Chunjong Park,Cynthia Breazeal,Daniel McDuff,Hae Won Park*

Main category: cs.AI

TL;DR: InvThink is a safety alignment method that teaches LLMs to reason through potential failure modes before generating responses, achieving better safety scaling and preserving general capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing safety alignment methods directly optimize for safe responses but may not systematically consider potential harms, leading to safety tax and limited effectiveness in high-stakes scenarios.
...
Result: Achieves up to 15.7% reduction in harmful responses compared to baselines, shows stronger safety scaling with model size, mitigates safety tax, and excels in high-stakes domains including medicine, finance, law, and agentic risk scenarios.
...
### [179] [The Unreasonable Effectiveness of Scaling Agents for Computer Use](https://arxiv.org/abs/2510.02250)
*Gonzalo Gonzalez-Pumariega,Vincent Tu,Chih-Lun Lee,Jiachen Yang,Ang Li,Xin Eric Wang*

Main category: cs.AI

TL;DR: bBoN is a scaling method for computer-use agents that generates multiple rollouts and selects the best using behavior narratives, achieving near-human performance on complex digital tasks.


<details>
  <summary>Details</summary>
Motivation: Computer-use agents have high potential but suffer from unreliability and high variance, making them unsuitable for long-horizon complex tasks.

Method: Behavior Best-of-N (bBoN) generates multiple agent rollouts and selects among them using behavior narratives that describe the agents' execution paths, enabling both wide exploration and principled trajectory selection.

Result: Achieves 69.9% success rate on OSWorld (approaching human-level 72%), establishes new state-of-the-art, and shows strong generalization across WindowsAgentArena and AndroidWorld.

Conclusion: Scaling computer-use agents effectively requires structured trajectory understanding and selection, and bBoN provides a practical framework to achieve robust performance approaching human levels.
...
### [180] [PychoBench: Evaluating the Psychology Intelligence of Large Language Models](https://arxiv.org/abs/2510.01611)
*Min Zeng*

Main category: cs.AI

TL;DR: This paper introduces PsychoBench, a benchmark based on US national counselor certification exams to evaluate if LLMs can meet professional counseling standards. Advanced models like GPT-4o pass the exam threshold while smaller models fail, showing only frontier LLMs currently qualify for counseling roles.


<details>
  <summary>Details</summary>
Motivation: To determine whether LLMs can be effectively applied to psychological counseling by assessing if they meet professional certification standards, specifically the ability to pass the US National Counselor Certification Exam (NCE).

Method: Created PsychoBench - a benchmark with approximately 2,252 single-choice questions from US national counselor examinations, designed to require deep psychological understanding and cover various psychology sub-disciplines.

Result: Advanced models (GPT-4o, Llama3.3-70B, Gemma3-27B) achieved well above the 70% passing threshold, while smaller open-source models (Qwen2.5-7B, Mistral-7B) remained far below the passing standard.

Conclusion: Only frontier LLMs currently demonstrate sufficient psychological knowledge to meet counseling certification standards, highlighting both the promise and challenges in developing psychology-oriented LLMs.
...
### [181] [Just Do It!? Computer-Use Agents Exhibit Blind Goal-Directedness](https://arxiv.org/abs/2510.01670)
...
TL;DR: Computer-Use Agents (CUAs) exhibit Blind Goal-Directedness (BGD) - pursuing goals regardless of feasibility, safety, or context. The paper introduces BLIND-ACT benchmark to evaluate BGD patterns and finds high BGD rates (80.8%) across nine frontier models.
...
Method: Developed BLIND-ACT benchmark with 90 tasks capturing three BGD patterns: lack of contextual reasoning, assumptions under ambiguity, and infeasible goals. Built on OSWorld with LLM-based judges achieving 93.75% human agreement. Evaluated nine frontier models.
...
Conclusion: BGD exposes subtle risks even with non-harmful inputs. Stronger training- or inference-time interventions are needed beyond prompting. BLIND-ACT provides foundation for studying and mitigating this fundamental CUA safety risk.
...
### [182] [Improving AGI Evaluation: A Data Science Perspective](https://arxiv.org/abs/2510.01687)
*John Hawkins*

Main category: cs.AI

TL;DR: The paper argues for shifting AGI evaluation from synthetic tasks based on human intuitions about intelligence to robust task execution methods inspired by data science practices.


<details>
  <summary>Details</summary>
Motivation: Current AGI evaluation methods using synthetic tasks have performed poorly historically, and there are no perfect methods to evaluate the end state of AGI development.

Method: Proposes an alternative design philosophy focused on evaluating robust task execution that demonstrates AGI through competence, drawing from data science practices for reliable system deployment.

Result: Provides practical examples of what this alternative AGI evaluation approach would entail.

Conclusion: AGI evaluation should move away from intuition-based synthetic tasks toward competence-based robust task execution methods that can demonstrate reliable deployment capabilities.
...
### [183] [Plan Then Action:High-Level Planning Guidance Reinforcement Learning for LLM Reasoning](https://arxiv.org/abs/2510.01833)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning is constrained to local decision-making due to autoregressive generation, leading to redundant and incoherent reasoning. Existing approaches like tree-based algorithms and RL are computationally expensive and often fail to produce optimal reasoning trajectories.
...
Result: Consistent and significant improvements across multiple mathematical reasoning benchmarks (MATH, AIME2024, AIME2025, AMC) and diverse base models (Qwen2.5-7B-Instruct, Qwen3-8B, Qwen3-14B, LLaMA3.2-3B), demonstrating stable performance gains.

Conclusion: PTA-GRPO effectively enhances both high-level planning and fine-grained CoT reasoning, achieving better reasoning effectiveness and generalization across different models and tasks.
...
### [184] [Constrained Adaptive Rejection Sampling](https://arxiv.org/abs/2510.01902)
...
<details>
  <summary>Details</summary>
Motivation: Existing constrained generation methods either distort the LM's distribution (greedy decoding) or waste computation (rejection sampling), both problematic for domains requiring both validity and diversity.
...
Result: CARS achieves higher efficiency (fewer LM forward passes per valid sample) and stronger sample diversity than GCD and other approximation methods across domains like program fuzzing and molecular generation.

Conclusion: CARS strictly improves rejection sampling efficiency without distributional distortion, ensuring valid samples exactly follow the constrained distribution while maintaining diversity.
...
### [185] [Do AI Models Perform Human-like Abstract Reasoning Across Modalities?](https://arxiv.org/abs/2510.02125)
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether state-of-the-art models truly understand and reason with intended abstractions in ConceptARC tasks, rather than just achieving high accuracy through surface-level patterns.

Method: Evaluated models on ConceptARC with varied input modalities (textual vs. visual), external tool usage, and reasoning effort. Performed dual evaluation of both output accuracy and natural-language rules generated to explain solutions.

Result: Text-based models match human accuracy but use surface-level shortcuts; visual modality models show lower accuracy but still exhibit intended abstractions in their rules, though often fail to apply them correctly.

Conclusion: Models still lag humans in abstract reasoning. Accuracy alone overestimates reasoning in text modality and underestimates it in visual modality. The proposed evaluation framework provides more faithful assessment of abstract reasoning abilities.
...
### [186] [A Rigorous Benchmark with Multidimensional Evaluation for Deep Research Agents: From Answers to Reports](https://arxiv.org/abs/2510.02190)
...
TL;DR: This paper introduces a specialized benchmark and evaluation framework for Deep Research Agents (DRAs) to address limitations in existing assessment methods for AI agent systems that perform complex research tasks.
...
Method: Developed a benchmark with 214 expert-curated queries across 10 domains, accompanied by manually constructed reference bundles, and created a multidimensional evaluation framework with integrated scoring metrics for semantic quality, topical focus, and retrieval trustworthiness.

Result: Experiments showed superior performance of mainstream DRAs over web-search-tool-augmented reasoning models, but revealed significant room for improvement in current DRA capabilities.

Conclusion: The study provides a robust foundation for capability assessment, architectural refinement, and paradigm advancement in DRA systems, addressing critical gaps in evaluating AI agent research capabilities.
...
### [187] [RLAD: Training LLMs to Discover Abstractions for Solving Reasoning Problems](https://arxiv.org/abs/2510.02263)
...
Method: RLAD (two-player RL training) that jointly trains an abstraction generator to propose multiple abstractions given a problem, and a solution generator that builds solutions using these abstractions. This decouples learning signals and enables structured exploration.
...
Conclusion: Reasoning abstractions effectively guide meaningful exploration and enable models to learn successful reasoning by capturing procedural knowledge, with the two-player RL setup providing structured training for both abstraction proposal and solution generation.
...
### [188] [From 2D to 3D, Deep Learning-based Shape Reconstruction in Magnetic Resonance Imaging: A Review](https://arxiv.org/abs/2510.01296)
*Emma McMillian,Abhirup Banerjee,Alfonso Bueno-Orovio*

Main category: cs.LG

TL;DR: This review surveys deep learning methods for 3D shape reconstruction from 2D MRI, covering four main approaches: point cloud, mesh-based, shape-aware, and volumetric models, with analysis of techniques, limitations, applications, and future directions.


<details>
  <summary>Details</summary>
Motivation: 3D shape reconstruction from 2D MRI is crucial for medical diagnosis, treatment planning, and computational modeling, but requires systematic understanding of current methodologies and their clinical applicability.

Method: Comprehensive review and analysis of four primary 3D reconstruction approaches: point cloud, mesh-based, shape-aware, and volumetric models, examining techniques, limitations, applications across anatomical structures, datasets, computational demands, and evaluation metrics.

Result: The review provides structured overview of current state-of-the-art techniques, their methodological foundations, applications in cardiac, neurological, and lung imaging, and assessment of clinical applicability to diseased anatomy.

Conclusion: The survey identifies emerging research directions including multimodal integration and cross-modality frameworks, aiming to advance deep learning towards more robust, generalizable, and clinically impactful 3D reconstruction solutions.
...
### [189] [Ultra-Efficient Decoding for End-to-End Neural Compression and Reconstruction](https://arxiv.org/abs/2510.01407)
*Ethan G. Rogers,Cheng Wang*

Main category: cs.LG

TL;DR: A new neural compression framework using low-rank representations and vector quantization to eliminate decoder bottlenecks while maintaining high image quality.


<details>
  <summary>Details</summary>
Motivation: Current neural compression methods suffer from high computational complexity and large costs in convolution-based decoders during reconstruction, hindering practical adoption.

Method: Developed a compression-reconstruction framework incorporating low-rank representation in an autoencoder with vector quantization, using computationally efficient low-rank operations on learned latent representations.

Result: Dramatically reduces computational overhead in decoding phase, essentially eliminating decoder compute bottleneck while maintaining high fidelity of image outputs.

Conclusion: The proposed low-rank approach provides an efficient solution to the decoder bottleneck problem in neural compression, enabling practical adoption of neural compression technology.
...
### [190] [Control the Temperature: Selective Sampling for Diverse and High-Quality LLM Outputs](https://arxiv.org/abs/2510.01218)
...
<details>
  <summary>Details</summary>
Motivation: Temperature-based sampling increases diversity but degrades reasoning quality in precision-required tasks like mathematical reasoning, due to sampling incorrect continuations in sensitive decoding positions.

Method: Propose selective sampling with a lightweight classifier that predicts sampling risk and dynamically switches between greedy and high-temperature sampling based on the risk metric.

Result: Experiments show selective sampling enhances the quality-diversity trade-off, even in high-temperature settings, with minimal latency overhead.

Conclusion: Selective sampling effectively addresses the accuracy degradation in high-temperature sampling for precision tasks while maintaining diversity benefits.
...
### [191] [Automated Extraction of Material Properties using LLM-based AI Agents](https://arxiv.org/abs/2510.01235)
...
<details>
  <summary>Details</summary>
Motivation: Materials discovery is constrained by the lack of large, machine-readable datasets that couple performance metrics with structural context, with existing databases being small, manually curated, or biased toward first principles results.

Method: An agentic, LLM-driven workflow integrates dynamic token allocation, zero-shot multi-agent extraction, and conditional table parsing to balance accuracy against computational cost for extracting properties from full-text scientific articles.

Result: The workflow extracted 27,822 temperature-resolved property records with normalized units and structural attributes. GPT-4.1 achieved highest accuracy (F1=0.91 for thermoelectric properties, 0.82 for structural fields), while GPT-4.1 Mini delivered comparable performance at lower cost.

Conclusion: This study delivers the largest LLM-curated thermoelectric dataset, provides a reproducible extraction pipeline, and establishes a foundation for scalable, data-driven materials discovery beyond thermoelectrics, with an interactive web explorer for community access.
...
### [192] [Beyond Simple Fusion: Adaptive Gated Fusion for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2510.01677)
*Han Wu,Yanming Sun,Yunhe Yang,Derek F. Wong*

Main category: cs.LG

TL;DR: The paper introduces AGFN, an Adaptive Gated Fusion Network that uses dual gate fusion based on information entropy and modality importance to adaptively weight multimodal features, improving sentiment analysis by reducing noise impact and enhancing generalization.


<details>
  <summary>Details</summary>
Motivation: Simple fusion techniques in multimodal sentiment analysis fail to handle variations in modality quality (noisy, missing, conflicting modalities), leading to suboptimal performance in discerning subtle emotional nuances.

Method: Proposes AGFN with dual gate fusion mechanism using information entropy and modality importance to adaptively adjust feature weights, mitigating noisy modality influence and prioritizing informative cues after unimodal encoding and cross-modal interaction.

Result: Experiments on CMU-MOSI and CMU-MOSEI show AGFN significantly outperforms baselines in accuracy, effectively discerning subtle emotions with robust performance. Visualization shows enhanced generalization through broader feature distribution learning.

Conclusion: AGFN creates more robust multimodal feature representations by reducing correlation between feature location and prediction error, decreasing reliance on specific locations and improving generalization in multimodal sentiment analysis.
...
### [193] [RSAVQ: Riemannian Sensitivity-Aware Vector Quantization for Large Language Models](https://arxiv.org/abs/2510.01240)
*Zukang Xu,Xing Hu,Qiang Wu,Dawei Yang*

Main category: cs.LG

TL;DR: RSAVQ is a novel Vector Quantization framework that enhances extremely low-bit quantization for LLMs by addressing direction error and suboptimal bit allocation through geometry-driven innovations.


<details>
  <summary>Details</summary>
Motivation: Large language models face deployment challenges on resource-constrained devices due to their large parameter sizes. Existing Vector Quantization methods struggle with unconstrained direction error and suboptimal bit allocation in low-bit quantization (2-4 bits).
...
Result: RSAVQ outperforms existing methods like VPTQ and QuIP#. In 2-bit quantization of LLaMA-3 8B, it achieves 0.4 lower perplexity and 1.5 higher zero-shot accuracy than baselines.
...
### [194] [RLP: Reinforcement as a Pretraining Objective](https://arxiv.org/abs/2510.01265)
...
TL;DR: RLP introduces reinforcement learning during pretraining by treating chain-of-thought as exploratory actions, rewarding information gain for predicting future tokens, leading to improved reasoning performance without requiring verifiers.


<details>
  <summary>Details</summary>
Motivation: Current training paradigm uses reinforcement learning only in post-training after supervised fine-tuning, which may not be optimal for developing reasoning capabilities earlier in the training process.

Method: RLP uses information-driven reinforcement pretraining where chain-of-thought is treated as exploratory action, with rewards based on information gain (increase in log-likelihood of next tokens when conditioning on reasoning chain vs context alone).

Result: RLP pretraining on Qwen3-1.7B-Base improved overall average across eight math-and-science benchmarks by 19%, with largest gains on reasoning-heavy tasks like AIME25 and MMLU-Pro. Applied to Nemotron-Nano-12B-v2, it increased overall average from 42.81% to 61.32% and scientific reasoning by 23%.

Conclusion: RLP successfully bridges the gap between next-token prediction and chain-of-thought reasoning emergence, enabling earlier development of independent thinking behavior during pretraining with scalable improvements across architectures and model sizes.
...
### [195] [Unsupervised Dynamic Feature Selection for Robust Latent Spaces in Vision Tasks](https://arxiv.org/abs/2510.01758)
...
Conclusion: Unsupervised DFS effectively enhances latent representations by filtering out irrelevant features, making it broadly applicable across domains without requiring labeled data.
...
### [196] [$\text{G}^2$RPO: Granular GRPO for Precise Reward in Flow Models](https://arxiv.org/abs/2510.01982)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for integrating RL into diffusion models suffer from sub-optimal preference alignment due to sparse and narrow reward signals, limiting their effectiveness in exploring high-value samples.

Method: Introduces Singular Stochastic Sampling for step-wise exploration with high reward-noise correlation, and Multi-Granularity Advantage Integration to eliminate bias from fixed-granularity denoising by aggregating advantages across multiple diffusion scales.
...
### [197] [Optimal Stopping vs Best-of-$N$ for Inference Time Optimization](https://arxiv.org/abs/2510.01394)
...
<details>
  <summary>Details</summary>
Motivation: Balancing LLM output quality against inference cost when using multiple generations, especially since each generation incurs computational expense.

Method: Developed a UCB-style Pandora's Box algorithm that learns stopping thresholds adaptively, combined with Bradley-Terry inspired transformation to handle reward scaling across prompts.

Result: Experiments on AlpacaFarm and HH-RLHF datasets show the adaptive strategy achieves same performance as Best-of-N sampling while requiring 15-35% fewer generations on average.

Conclusion: Established a principled bridge between optimal stopping theory and inference-time scaling, providing both theoretical performance bounds and practical efficiency gains for LLM deployment.
...
### [198] [LSPO: Length-aware Dynamic Sampling for Policy Optimization in LLM Reasoning](https://arxiv.org/abs/2510.01459)
*Weizhe Chen,Sven Koenig,Bistra Dilkina*

Main category: cs.LG

TL;DR: LSPO is a meta-RLVR algorithm that dynamically selects training data based on response length to improve learning effectiveness in reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Motivated by studies of overthinking in LLMs, the authors aim to address inefficiencies in RLVR training by incorporating length-aware sampling.
...
Result: LSPO consistently improves learning effectiveness across multiple base models and datasets, as demonstrated through comprehensive evaluation.

Conclusion: The method shows promise for RLVR training, with ablation studies providing insights for future research on incorporating length signals into dynamic sampling.
...
### [199] [Think Right: Learning to Mitigate Under-Over Thinking via Adaptive, Attentive Compression](https://arxiv.org/abs/2510.01581)
...
<details>
  <summary>Details</summary>
Motivation: Current models suffer from under-adaptivity - either underthinking (too short reasoning for hard problems) or overthinking (unnecessarily long reasoning for easy problems), leading to inefficiency and errors.
...
Result: TRAAC achieves 8.4% accuracy gain with 36.8% reduction in reasoning length compared to base models, and 7.9% accuracy gain with 29.4% length reduction compared to best RL baselines across multiple tasks.

Conclusion: TRAAC enables adaptive thinking by combining difficulty estimation and attention-based compression, showing strong generalization across math and non-math domains while improving both accuracy and efficiency.
...
### [200] [Test-Time Anchoring for Discrete Diffusion Posterior Sampling](https://arxiv.org/abs/2510.02291)
...
TL;DR: Anchored Posterior Sampling (APS) enables training-free posterior sampling using pretrained discrete diffusion models for image recovery from noisy measurements, overcoming limitations of existing discrete diffusion approaches.


<details>
  <summary>Details</summary>
Motivation: To enable posterior sampling using pretrained discrete diffusion foundation models without retraining, leveraging discrete diffusion's advantages over continuous diffusion for categorical data modeling, faster inference, and principled Bayesian inference.

Method: Anchored Posterior Sampling (APS) with two key innovations: quantized expectation for gradient-like guidance in discrete embedding space, and anchored remasking for adaptive decoding in masked diffusion models.

Result: Achieves state-of-the-art performance among discrete diffusion samplers across linear and nonlinear inverse problems on standard benchmarks, and demonstrates benefits in training-free stylization and text-guided editing.

Conclusion: APS successfully overcomes challenges in discrete diffusion posterior sampling and provides an effective framework for leveraging pretrained discrete diffusion models in various inverse problems and editing tasks.
...
### [201] [Continual Personalization for Diffusion Models](https://arxiv.org/abs/2510.02296)
...
<details>
  <summary>Details</summary>
Motivation: Updating diffusion models incrementally for real-world applications is practical but computationally challenging, requiring methods that can personalize models without losing previous knowledge.

Method: Concept Neuron Selection (CNS) identifies neurons related to target concepts in diffusion models and fine-tunes them incrementally while jointly preserving knowledge from previous concepts.
...
### [202] [Equilibrium Matching: Generative Modeling with Implicit Energy-Based Models](https://arxiv.org/abs/2510.02300)
...
Method: Learns the equilibrium gradient of an implicit energy landscape and uses optimization-based sampling with gradient descent, adjustable step sizes, adaptive optimizers, and adaptive compute.

Result: Achieves state-of-the-art performance with FID of 1.90 on ImageNet 256×256, surpassing diffusion/flow models. Also handles tasks like image denoising, OOD detection, and image composition.

Conclusion: EqM provides a unified framework that bridges flow and energy-based models, offering optimization-driven inference and theoretical justification for learning from data manifolds.
...
### [203] [Quagmires in SFT-RL Post-Training: When High SFT Scores Mislead and What to Use Instead](https://arxiv.org/abs/2510.01624)
*Feiyang Kang,Michael Kuchnik,Karthik Padthe,Marin Vlastelica,Ruoxi Jia,Carole-Jean Wu,Newsha Ardalani*

Main category: cs.LG

TL;DR: High SFT scores don't guarantee better RL performance; generalization loss and Pass@large k are better predictors for RL outcomes than SFT scores.


<details>
  <summary>Details</summary>
Motivation: Challenge the assumption that high SFT scores translate to improved RL performance, showing counter-examples where this fails.

Method: Train hundreds of models up to 12B parameters with SFT and RLVR via GRPO, evaluate on 7 math benchmarks with up to 256 repetitions using $>1M GPU hours.

Result: Generalization loss and Pass@large k metrics improve prediction precision by up to 0.5 R² and Spearman correlation (2x improvement). SFT on unique examples for one epoch underperforms training on half examples for two epochs.

Conclusion: Alternative metrics (generalization loss, Pass@large k) provide strong proxies for RL outcomes, offering better utility than pre-RL SFT scores.
...
### [204] [Demystifying Synthetic Data in LLM Pre-training: A Systematic Study of Scaling Laws, Benefits, and Pitfalls](https://arxiv.org/abs/2510.01631)
...
TL;DR: Synthetic data alone isn't faster than natural web data for LLM pre-training, but mixing 1/3 rephrased synthetic data with 2/3 natural data can speed up training 5-10x at larger data budgets.


<details>
  <summary>Details</summary>
Motivation: High-quality training data is limited for LLM scaling, and synthetic data offers a potential solution to overcome data supply constraints.
...
Result: Rephrased synthetic data alone performs similarly to natural data; 1/3 synthetic + 2/3 natural mixture speeds up training 5-10x; textbook-style synthetic data alone causes higher loss; optimal synthetic ratio converges to ~30%; larger generators don't necessarily produce better data.

Conclusion: Synthetic data has conditional benefits - rephrased synthetic data shows no model collapse, while textbook-style synthetic shows collapse patterns; provides practical guidance for synthetic data usage in pre-training.
...
TL;DR: Sparse Query Attention (SQA) reduces computational complexity by decreasing Query heads instead of Key/Value heads, achieving up to 3x throughput improvements in computation-bound scenarios with minimal quality impact.


<details>
  <summary>Details</summary>
Motivation: To address the quadratic computational complexity of Multi-Head Attention (MHA) that limits scaling for long contexts, as existing solutions like MQA and GQA reduce memory bandwidth but not FLOPs.

Method: Introduces Sparse Query Attention (SQA) architecture that reduces the number of Query heads rather than Key/Value heads, directly decreasing computational complexity proportionally to query head reduction.

Result: Empirical benchmarks on long sequences (32k-200k tokens) show up to 3x throughput improvements in computation-bound scenarios like pre-training, fine-tuning, and encoder tasks with minimal quality impact.

Conclusion: SQA provides a complementary optimization path to existing attention mechanisms and shows potential as a powerful tool for building more efficient and scalable transformer models.
...
### [206] [StockBench: Can LLM Agents Trade Stocks Profitably In Real-world Markets?](https://arxiv.org/abs/2510.02209)
...
<details>
  <summary>Details</summary>
Motivation: Existing financial benchmarks primarily test static knowledge through QA but fail to capture the dynamic, iterative nature of trading, leaving the finance domain underexplored for LLM agents despite its economic importance.

Method: Introduced StockBench benchmark where agents receive daily market signals (prices, fundamentals, news) and make sequential buy/sell/hold decisions, evaluated using financial metrics like cumulative return, maximum drawdown, and Sortino ratio.

Result: Most LLM agents (including GPT-5, Claude-4, Qwen3, Kimi-K2, GLM-4.5) struggle to outperform buy-and-hold baseline, but several models show potential for higher returns and better risk management.
...
### [207] [ExGRPO: Learning to Reason from Experience](https://arxiv.org/abs/2510.02245)
*Runzhe Zhan,Yafu Li,Zhi Wang,Xiaoye Qu,Dongrui Liu,Jing Shao,Derek F. Wong,Yu Cheng*

Main category: cs.LG

TL;DR: ExGRPO improves RLVR by organizing and prioritizing valuable reasoning experiences based on correctness and entropy, achieving better performance and stability than standard on-policy methods.
...
Method: Proposed ExGRPO (Experiential Group Relative Policy Optimization) framework that identifies rollout correctness and entropy as indicators of experience value, organizes and prioritizes valuable experiences, and uses mixed-policy objective to balance exploration with experience exploitation.
...
Conclusion: Principled experience management is a key ingredient for efficient and scalable RLVR, with experience characteristics (correctness and entropy) being effective indicators of experience value for reasoning models.
...
### [208] [Tree-based Dialogue Reinforced Policy Optimization for Red-Teaming Attacks](https://arxiv.org/abs/2510.02286)
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs remain vulnerable to multi-turn adversarial attacks, but existing approaches rely on manual red-teaming or pre-defined templates, failing to explore the vast space of possible multi-turn attack trajectories and strategic conversation planning.
...
Conclusion: The framework successfully learns optimal dialogue policies that maximize attack success across multiple turns, demonstrating superior performance in discovering multi-turn adversarial vulnerabilities in LLMs.
...
### [209] [Interactive Training: Feedback-Driven Neural Network Optimization](https://arxiv.org/abs/2510.02297)
*Wentao Zhang,Yang Young Lu,Yuntian Deng*

Main category: cs.LG

TL;DR: Interactive Training is a framework enabling real-time human or AI intervention during neural network training to dynamically adjust hyperparameters, data, and checkpoints, improving stability and adaptability.


<details>
  <summary>Details</summary>
Motivation: Traditional neural network training follows fixed optimization recipes without flexibility to respond to instabilities or emerging training issues dynamically.

Method: Uses a control server to mediate communication between users/agents and training process, allowing dynamic adjustment of optimizer hyperparameters, training data, and model checkpoints.
...
### [210] [Ovi: Twin Backbone Cross-Modal Fusion for Audio-Video Generation](https://arxiv.org/abs/2510.01284)
...
<details>
  <summary>Details</summary>
Motivation: Traditional audio-video generation relies on complex multi-stage architectures or sequential synthesis, which creates synchronization challenges and requires separate pipelines.

Method: Uses twin-DiT modules with blockwise cross-modal fusion, initializing an audio tower identical to a pretrained video model. Employs scaled-RoPE embeddings for timing exchange and bidirectional cross-attention for semantic fusion, trained on vast video corpus.

Result: Produces movie-grade video clips with natural speech, realistic sound effects, and accurate context-matching. The model conveys rich speaker identity and emotion in generated speech.

Conclusion: Ovi demonstrates that unified audio-video generation through cross-modal fusion can achieve natural synchronization and high-quality cinematic storytelling without complex multi-stage pipelines.
...
### [211] [An Efficient Quality Metric for Video Frame Interpolation Based on Motion-Field Divergence](https://arxiv.org/abs/2510.01361)
*Conall Daly,Darren Ramsook,Anil Kokaram*

Main category: eess.IV

TL;DR: PSNR_DIV is a new quality metric for video frame interpolation that enhances PSNR with motion divergence weighting, achieving better correlation with human perception while being faster and more memory-efficient than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing quality metrics like PSNR, SSIM, and LPIPS ignore temporal coherence, while specialized metrics like FloLPIPS are computationally inefficient, limiting their practical application for video frame interpolation evaluation.

Method: PSNR_DIV enhances PSNR through motion divergence weighting, a technique adapted from archival film restoration that detects temporal inconsistencies by highlighting singularities in motion fields to weight image errors.

Result: Evaluation on BVI-VFI dataset (180 sequences) shows PSNR_DIV achieves +0.09 Pearson Linear Correlation Coefficient improvement over FloLPIPS, while being 2.5x faster and using 4x less memory, with consistent performance across content categories.
...
### [212] [Median2Median: Zero-shot Suppression of Structured Noise in Images](https://arxiv.org/abs/2510.01666)
*Jianxu Wang,Ge Wang*

Main category: eess.IV

TL;DR: M2M is a zero-shot denoising framework that addresses structured noise using a novel sampling strategy with directional interpolation and median filtering, outperforming existing methods on correlated noise.


<details>
  <summary>Details</summary>
Motivation: Real-world images often have structured noise with anisotropic correlations that existing methods struggle with. Data-driven approaches need large datasets and have limited generalizability, while zero-shot methods only work well for i.i.d. noise.

Method: Proposes Median2Median (M2M) with a novel sampling strategy that generates pseudo-independent sub-image pairs from a single noisy input using directional interpolation and generalized median filtering to exclude structured artifacts. Uses randomized assignment to eliminate systematic bias.
...
Conclusion: M2M is an efficient, data-free solution for structured noise suppression and represents the first step toward effective zero-shot denoising beyond the strict i.i.d. assumption.
...
### [213] [GFSR-Net: Guided Focus via Segment-Wise Relevance Network for Interpretable Deep Learning in Medical Imaging](https://arxiv.org/abs/2510.01919)
*Jhonatan Contreras,Thomas Bocklitz*

Main category: eess.IV

TL;DR: GFSR-Net improves interpretability in medical imaging by using minimal human annotations to guide model focus toward diagnostically relevant regions, achieving comparable accuracy with more reliable saliency maps.


<details>
  <summary>Details</summary>
Motivation: Deep learning models in medical imaging lack interpretability, may rely on irrelevant image regions or visual cues, reducing trust and increasing diagnostic risks.

Method: GFSR-Net uses a small number of human annotations to approximate human focus areas without precise boundaries, training the model to align its attention with diagnostically meaningful features.

Result: GFSR-Net achieves comparable or superior accuracy while producing saliency maps that better reflect human expectations, reducing reliance on irrelevant patterns.
...
### [214] [SpurBreast: A Curated Dataset for Investigating Spurious Correlations in Real-world Breast MRI Classification](https://arxiv.org/abs/2510.02109)
*Jong Bum Won,Wesley De Neve,Joris Vankerschaver,Utku Ozbulak*

Main category: eess.IV

TL;DR: SpurBreast is a curated breast MRI dataset designed to study spurious correlations in medical imaging, identifying magnetic field strength and image orientation as dominant non-clinical signals that DNNs exploit, leading to poor generalization despite high validation accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-world deployment of DNNs in medical imaging is challenged by spurious correlations where models learn non-clinical features instead of meaningful medical patterns. Existing datasets lack systematic study of this issue due to restrictive licensing and limited patient data.

Method: Created SpurBreast dataset by analyzing over 100 patient, device, and imaging protocol features to intentionally incorporate spurious correlations. Identified two dominant spurious signals: magnetic field strength (global feature) and image orientation (local feature). Used controlled dataset splits to evaluate model performance.

Result: DNNs exploit non-clinical signals like magnetic field strength and image orientation, achieving high validation accuracy but failing to generalize to unbiased test data. The dataset enables systematic investigation of clinically relevant/irrelevant features, uncertainty estimation, adversarial robustness, and generalization strategies.

Conclusion: SpurBreast provides a benchmark for studying spurious correlations in medical imaging, demonstrating that DNNs can learn non-clinical features that compromise real-world deployment. The dataset and models are publicly available to support research in this critical area.
...
### [215] [Measurement-Guided Consistency Model Sampling for Inverse Problems](https://arxiv.org/abs/2510.02208)
*Amirreza Tanevardi,Pooria Abbas Rad Moghadam,Sajjad Amini*

Main category: eess.IV

TL;DR: A modified consistency sampling method for inverse imaging problems that guides stochasticity with measurement-consistency to enforce data fidelity while maintaining fast generation.
...
Method: Modified consistency sampling with measurement-consistency mechanism that guides stochasticity using the measurement operator to enforce fidelity to acquired data.

Result: Experiments on Fashion-MNIST and LSUN Bedroom show improved perceptual and pixel-level metrics (FID, KID, PSNR, SSIM) over baseline consistency sampling with few steps.

Conclusion: The approach achieves competitive or superior reconstructions with only a handful of steps, making it practical for inverse problem deployment.
...
### [216] [LLM-based Multi-Agent Blackboard System for Information Discovery in Data Science](https://arxiv.org/abs/2510.01285)
*Alireza Salemi,Mihir Parmar,Palash Goyal,Yiwen Song,Jinsung Yoon,Hamed Zamani,Hamid Palangi,Tomas Pfister*

Main category: cs.MA

TL;DR: Proposes a blackboard-based multi-agent communication paradigm for data discovery in large data lakes, outperforming existing methods with 13-57% relative improvement in task success.


<details>
  <summary>Details</summary>
Motivation: Existing single-agent systems are overwhelmed by large heterogeneous data lakes, while master-slave multi-agent systems require precise knowledge of sub-agent capabilities, limiting scalability and flexibility.

Method: A blackboard architecture where a central agent posts requests to a shared blackboard, and autonomous subordinate agents volunteer to respond based on their capabilities, eliminating need for central coordinator to know all sub-agent expertise.

Result: Substantially outperforms baselines (RAG and master-slave paradigm) with 13-57% relative improvement in end-to-end task success and up to 9% relative gain in F1 score for data discovery across both proprietary and open-source LLMs.
...
### [217] [Extracting O*NET Features from the NLx Corpus to Build Public Use Aggregate Labor Market Data](https://arxiv.org/abs/2510.01470)
*Stephen Meisenbacher,Svetlozar Nestorov,Peter Norlander*

Main category: cs.CY

TL;DR: The paper introduces JAAT, an open-source toolkit that uses NLP to extract structured job information from online postings, addressing limitations in traditional data sources like O*NET.


<details>
  <summary>Details</summary>
Motivation: Online job posting data is difficult to access and lacks standardization, while traditional sources like O*NET are infrequently updated and based on small survey samples.

Method: Adopted O*NET as a framework to build NLP tools for extracting structured information from job postings, creating the Job Ad Analysis Toolkit (JAAT) and testing its reliability through out-of-sample and LLM-as-a-Judge methods.

Result: Extracted over 10 billion data points from 155+ million job ads, including O*NET tasks, occupation codes, tools, technologies, wages, skills, and industry features. Built a comprehensive dataset aggregated by occupation, state, and industry from 2015-2025.

Conclusion: The toolkit demonstrates high reliability and accuracy, with significant potential for research and applications in education and workforce development.
...
### [218] [Towards Photonic Band Diagram Generation with Transformer-Latent Diffusion Models](https://arxiv.org/abs/2510.01749)
*Valentin Delchevalerie,Nicolas Roy,Arnaud Bougaham,Alexandre Mayer,Benoît Frénay,Michaël Lobet*

Main category: physics.optics

TL;DR: First diffusion model approach for photonic band diagram generation using transformer encoder and latent diffusion to reduce computational costs of solving Maxwell's equations.
...
Method: Couples transformer encoder to extract contextual embeddings from input structures with latent diffusion model to generate corresponding band diagrams.
...
### [219] [VENTURA: Adapting Image Diffusion Models for Unified Task Conditioned Navigation](https://arxiv.org/abs/2510.01388)
*Arthur Zhang,Xiangyun Meng,Luca Calliari,Dong-Ki Kim,Shayegan Omidshafiei,Joydeep Biswas,Ali Agha,Amirreza Shaban*

Main category: cs.RO

TL;DR: VENTURA is a vision-language navigation system that fine-tunes image diffusion models for path planning, generating visual path masks instead of direct actions, with a lightweight policy to execute trajectories from natural language instructions.


<details>
  <summary>Details</summary>
Motivation: Robots need to adapt to diverse human instructions and operate safely in unstructured environments. While Vision-Language models offer strong language-perception grounding, they're difficult to steer for navigation due to action space differences and pretraining objectives.

Method: Fine-tunes internet-pretrained image diffusion models to generate path masks (visual plans) in image space. Uses self-supervised tracking models with VLM-augmented captions for training supervision, avoiding manual annotation. A lightweight behavior-cloning policy grounds visual plans into executable trajectories.

Result: Outperforms state-of-the-art foundation model baselines: improves success rates by 33% and reduces collisions by 54% across seen and unseen scenarios. Generalizes to unseen task combinations, showing emergent compositional capabilities.

Conclusion: VENTURA provides an effective approach for vision-language navigation by generating visual path plans that enable diverse, context-aware robot behaviors from natural language instructions, with strong generalization and safety improvements.
...
### [220] [ActiveUMI: Robotic Manipulation with Active Perception from Robot-Free Human Demonstrations](https://arxiv.org/abs/2510.01607)
...
<details>
  <summary>Details</summary>
Motivation: To create a scalable system for collecting high-quality human demonstrations in real-world settings that can train robots for complex bimanual manipulation tasks, addressing the challenge of transferring human skills to robots.

Method: Uses a portable VR teleoperation kit with sensorized controllers that mirror robot end-effectors, coupled with immersive 3D rendering, wearable computing, and calibration methods. Key innovation is capturing active perception through head-mounted display tracking of deliberate head movements.

Result: Achieved 70% average success rate on in-distribution bimanual tasks and maintained 56% success rate when tested on novel objects and environments, demonstrating strong generalization capabilities.

Conclusion: Portable data collection systems combined with learned active perception provide an effective and scalable pathway for creating generalizable real-world robot policies capable of complex manipulation tasks.
...
### [221] [DisCo-Layout: Disentangling and Coordinating Semantic and Physical Refinement in a Multi-Agent Framework for 3D Indoor Layout Synthesis](https://arxiv.org/abs/2510.02178)
*Jialin Gao,Donghao Zhou,Mingjian Liang,Lihao Liu,Chi-Wing Fu,Xiaowei Hu,Pheng-Ann Heng*

Main category: cs.RO

TL;DR: DisCo-Layout is a novel framework for 3D indoor layout synthesis that disentangles physical and semantic refinement using specialized tools and multi-agent coordination.
...
Result: Achieves state-of-the-art performance, generating realistic, coherent, and generalizable 3D indoor layouts.
...
### [222] [Do You Know Where Your Camera Is? View-Invariant Policy Learning with Camera Conditioning](https://arxiv.org/abs/2510.02268)
*Tianchong Jiang,Jingtian Ji,Xiangshan Tan,Jiading Fang,Anand Bhattad,Vitor Guizilini,Matthew R. Walter*

Main category: cs.RO

TL;DR: The paper studies view-invariant imitation learning by explicitly conditioning policies on camera extrinsics, showing this significantly improves generalization across viewpoints for various behavior cloning policies.
...
Method: Using Plucker embeddings of per-pixel rays to condition policies on camera extrinsics, and introducing six manipulation tasks in RoboSuite and ManiSkill with fixed and randomized scene variants to evaluate robustness.

Result: Conditioning on extrinsics restores performance and yields robust RGB-only control without depth, significantly improving generalization across viewpoints.

Conclusion: Explicitly conditioning policies on camera extrinsics is crucial for achieving view-invariant imitation learning and robust performance under realistic viewpoint shifts.
...
### [223] [Utilizing Modern Large Language Models (LLM) for Financial Trend Analysis and Digest Creation](https://arxiv.org/abs/2510.01225)
*Andrei Lazarev,Dmitrii Sedov*

Main category: cs.CE

TL;DR: This paper presents an automated framework using Google's Gemini Pro LLM to generate financial digests from OpenAlex data, addressing information overload through strategic prompt engineering and automated PDF report generation.


<details>
  <summary>Details</summary>
Motivation: The exponential growth of information makes it challenging for researchers and professionals to stay current in their fields. Traditional analysis methods are limited in processing vast unstructured data efficiently.

Method: Uses Google's Gemini Pro LLM with data extraction from OpenAlex, strategic prompt engineering, and automated workflow from data acquisition to JSON construction, LLM interaction, and PDF report generation.

Result: Successfully demonstrated automated creation of comprehensive financial digests that generalize key findings and identify emerging trends, delivering actionable insights in digestible formats.

Conclusion: LLMs can effectively help researchers save time and stay informed about current trends by automating the processing of large unstructured datasets and generating insightful digests.
...
TL;DR: MPMAvatar is a framework for creating 3D human avatars from multi-view videos that enables realistic animation and rendering by combining Material Point Method-based physical simulation with 3D Gaussian Splatting rendering.
...
Method: Uses Material Point Method-based simulator with anisotropic constitutive model and novel collision handling for garment dynamics, combined with canonical avatar rendered using 3D Gaussian Splatting with quasi-shadowing.
...
### [225] [ROI-GS: Interest-based Local Quality 3D Gaussian Splatting](https://arxiv.org/abs/2510.01978)
...
### [226] [Spec-Gloss Surfels and Normal-Diffuse Priors for Relightable Glossy Objects](https://arxiv.org/abs/2510.02069)
*Georgios Kouros,Minye Wu,Tinne Tuytelaars*

Main category: cs.GR

TL;DR: A relightable framework using microfacet BRDF with specular-glossiness parameterization in 2D Gaussian Splatting for accurate reconstruction and relighting of glossy objects.


<details>
  <summary>Details</summary>
Motivation: Existing neural rendering approaches struggle with disentangling object shape, material properties, and illumination for glossy objects, often using simplified BRDF models that limit material recovery and relighting fidelity.

Method: Integrates microfacet BRDF with specular-glossiness parameterization into 2D Gaussian Splatting with deferred shading, uses diffusion-based priors for surface normals and diffuse color to guide optimization, and employs coarse-to-fine optimization of environment maps.

Result: Achieves high-quality geometry and material reconstruction, delivering substantially more realistic and consistent relighting under novel illumination compared to existing Gaussian splatting methods.

Conclusion: The proposed framework enables physically consistent material decomposition and provides superior relighting results for complex glossy scenes.
...
### [227] [Aligning Video Models with Human Social Judgments via Behavior-Guided Fine-Tuning](https://arxiv.org/abs/2510.01502)
*Kathy Garcia,Leyla Isik*

Main category: q-bio.NC

TL;DR: AI video models don't naturally capture human social perception in videos, but can be fine-tuned using human similarity judgments to better align with human social understanding.


<details>
  <summary>Details</summary>
Motivation: To understand whether modern AI models encode the same social similarity structure that humans perceive in visual scenes, and to develop methods for instilling human-like social perception into these models.

Method: Created a benchmark with 49,000+ human similarity judgments on 250 social interaction videos. Used a novel hybrid triplet-RSA objective with LoRA fine-tuning to align video model representations with human similarity perceptions.

Result: Found a modality gap where language embeddings outperformed video models on human similarity. Fine-tuning closed this gap, improving alignment with human perceptions and strengthening encoding of social-affective attributes like intimacy, valence, and dominance.

Conclusion: Pretrained video models have gaps in social recognition, but behavior-guided fine-tuning can shape video representations toward human social perception, with the fine-tuned model capturing unique variance beyond language models.
...
### [228] [Uncovering Semantic Selectivity of Latent Groups in Higher Visual Cortex with Mutual Information-Guided Diffusion](https://arxiv.org/abs/2510.02182)
*Yule Wang,Joseph Yu,Chengrui Li,Weihan Li,Anqi Wu*

Main category: q-bio.NC

TL;DR: MIG-Vis is a method that uses diffusion models to visualize and validate how visual-semantic features are organized in neural populations of higher visual areas, revealing structured semantic representations.


<details>
  <summary>Details</summary>
Motivation: To understand how feature-specific visual information is distributed across neural populations in higher visual areas and whether it's organized into structured, semantically meaningful subspaces, addressing limitations of prior indirect methods.

Method: Uses variational autoencoder to infer group-wise disentangled neural latent subspaces from neural populations, then applies mutual information-guided diffusion synthesis to visualize specific visual-semantic features encoded by each latent group.

Result: Validated on macaque IT cortex data, showing neural latent groups with clear semantic selectivity to visual features like object pose, inter-category transformations, and intra-class content.
...
### [229] [Synthetic Prefixes to Mitigate Bias in Real-Time Neural Query Autocomplete](https://arxiv.org/abs/2510.01574)
*Adithya Rajan,Xiaoyu Liu,Prateek Verma,Vibhu Arora*

Main category: cs.IR

TL;DR: A data-centric approach using synthetic prefixes from complete queries to mitigate presentation bias in neural query autocomplete systems, improving ranking performance under strict latency constraints.


<details>
  <summary>Details</summary>
Motivation: To address inherent bias in engagement signals from live query autocomplete interactions where model suggestions influence user behavior, and to enrich training data with more diverse examples.

Method: Generate synthetic prefixes from complete user queries collected when autocomplete was inactive, use neural ranker optimized for real-time deployment with rich features (query popularity, seasonality, fuzzy match, contextual signals), and introduce simplified listwise loss reducing complexity from O(n²) to O(n).

Result: Deployed in large-scale e-commerce setting, the system shows statistically significant improvements in user engagement measured by mean reciprocal rank and related metrics.

Conclusion: Synthetic prefixes improve generalization and provide scalable bias mitigation for low-latency ranking tasks including related searches and query recommendations.
...
### [230] [Bridging Collaborative Filtering and Large Language Models with Dynamic Alignment, Multimodal Fusion and Evidence-grounded Explanations](https://arxiv.org/abs/2510.01606)
*Bo Ma,LuYao Liu,Simon Lau,Chandler Yuan,and XueY Cui,Rosie Zhang*

Main category: cs.IR

TL;DR: A framework that enhances LLM-based recommendations with online adaptation, multimodal integration, and evidence-backed explanations while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: To address limitations in current LLM-based recommendation systems: static user preferences, lack of multimodal content handling, and untrustworthy explanations without evidence.

Method: Uses three innovations: online adaptation with lightweight modules for continuous learning, unified representation combining collaborative signals with visual/audio features, and explanation system grounded in collaborative patterns and item attributes.

Result: Maintains efficiency of frozen base models with minimal computational overhead while enabling continuous adaptation, multimodal handling, and verifiable explanations.

Conclusion: The framework provides a practical solution for real-world deployment that overcomes key limitations of current LLM-based recommendation systems through efficient adaptation, multimodal integration, and trustworthy explanations.
...
### [231] [LLM4Rec: Large Language Models for Multimodal Generative Recommendation with Causal Debiasing](https://arxiv.org/abs/2510.01622)
*Bo Ma,Hang Li,ZeHua Hu,XiaoFan Gui,LuYao Liu,Simon Lau*

Main category: cs.IR

TL;DR: An enhanced generative recommendation framework with multimodal fusion, retrieval-augmented generation, causal debiasing, explainable recommendations, and real-time adaptive learning that improves accuracy, fairness, and diversity.


<details>
  <summary>Details</summary>
Motivation: Address challenges in handling multimodal data, eliminating algorithmic biases, and providing transparent decision-making in contemporary generative recommendation systems.

Method: Uses large language models as backbone with specialized modules for cross-modal understanding, contextual knowledge integration, bias mitigation, explanation synthesis, and continuous adaptation. Includes multimodal fusion architecture and retrieval-augmented generation mechanisms.

Result: Achieves up to 2.3% improvement in NDCG@10 and 1.4% enhancement in diversity metrics on MovieLens-25M, Amazon-Electronics, and Yelp-2023 datasets while maintaining computational efficiency.

Conclusion: The proposed framework demonstrates consistent improvements in recommendation accuracy, fairness, and diversity compared to existing approaches through its five key innovations.
...
### [232] [Study on LLMs for Promptagator-Style Dense Retriever Training](https://arxiv.org/abs/2510.02241)
...
<details>
  <summary>Details</summary>
Motivation: The original Promptagator approach relied on proprietary, large-scale LLMs that may be inaccessible or prohibited for sensitive data, creating a need for accessible alternatives.

Method: Studied the use of open-source LLMs (≤14B parameters) as query generators for fine-tuning domain-specialized dense retrieval models, following the Promptagator approach.
...
### [233] [Jailbreaking LLMs via Semantically Relevant Nested Scenarios with Targeted Toxic Knowledge](https://arxiv.org/abs/2510.01223)
...
<details>
  <summary>Details</summary>
Motivation: Current jailbreak methods using nested scenarios are easily detectable due to prominent malicious intentions. The authors discovered that LLMs' alignment defenses are not sensitive to nested scenarios that are highly semantically relevant to queries and incorporate targeted toxic knowledge.

Method: Proposed RTS-Attack framework that builds scenarios highly relevant to queries and integrates targeted toxic knowledge to bypass LLM alignment defenses. The generated jailbreak prompts are free from harmful queries for better concealment.
...
Conclusion: RTS-Attack demonstrates that LLMs' alignment defenses are vulnerable to carefully crafted nested scenarios with semantic relevance and targeted toxic knowledge, highlighting a critical security vulnerability in current LLM safety mechanisms.
...
### [234] [ZK-WAGON: Imperceptible Watermark for Image Generation Models using ZK-SNARKs](https://arxiv.org/abs/2510.01967)
*Aadarsh Anantha Ramakrishnan,Shubham Agarwal,Selvanayagam S,Kunwar Singh*

Main category: cs.CR

TL;DR: ZK-WAGON is a novel system that uses ZK-SNARKs to watermark image generation models, enabling verifiable proof of origin without exposing sensitive model information, while maintaining image quality and security.


<details>
  <summary>Details</summary>
Motivation: Address growing concerns around authenticity, ownership, and misuse of synthetic media caused by powerful image generation models, including risks of misinformation, deepfakes, and intellectual property violations.

Method: Uses Selective Layer ZK-Circuit Creation (SL-ZKCC) to selectively convert key model layers into circuits, then embeds ZK-SNARK proofs into generated images via Least Significant Bit steganography.

Result: Successfully demonstrated on both GAN and Diffusion models, providing a secure, model-agnostic pipeline for trustworthy AI image generation without degrading image quality.

Conclusion: ZK-WAGON offers a practical solution for secure and scalable watermarking of image generation models, addressing limitations of traditional methods while preserving privacy and image quality.
...
### [235] [WAInjectBench: Benchmarking Prompt Injection Detections for Web Agents](https://arxiv.org/abs/2510.01354)
*Yinuo Liu,Ruohan Xu,Xilong Wang,Yuqi Jia,Neil Zhenqiang Gong*

Main category: cs.CR

TL;DR: First comprehensive benchmark study on detecting prompt injection attacks targeting web agents, evaluating both text-based and image-based detection methods across various attack scenarios.
...
Result: Some detectors can identify attacks with explicit instructions or visible perturbations with moderate-high accuracy, but fail against attacks without explicit instructions or with imperceptible perturbations.

Conclusion: Current detection methods have significant limitations, particularly against sophisticated attacks that don't use explicit instructions or employ subtle perturbations.
...
### [236] [Position: Privacy Is Not Just Memorization!](https://arxiv.org/abs/2510.01645)
*Niloofar Mireshghallah,Tianshi Li*

Main category: cs.CR

TL;DR: The paper argues that LLM privacy risks extend beyond training data memorization to include data collection practices, inference-time context leakage, autonomous agent capabilities, and democratized surveillance through deep inference attacks.


<details>
  <summary>Details</summary>
Motivation: Current privacy discourse disproportionately focuses on verbatim memorization while neglecting more immediate and scalable privacy threats across the LLM lifecycle.
...
Conclusion: Calls for a fundamental shift in LLM privacy research, moving beyond narrow technical solutions to embrace interdisciplinary approaches addressing sociotechnical threats.
...
### [237] [JaneEye: A 12-nm 2K-FPS 18.9-$μ$J/Frame Event-based Eye Tracking Accelerator](https://arxiv.org/abs/2510.01213)
*Tao Han,Ang Li,Qinyu Chen,Chang Gao*

Main category: eess.SP

TL;DR: JaneEye is an energy-efficient event-based eye-tracking hardware accelerator using a novel ConvJANET layer that halves computational complexity while maintaining high accuracy for XR wearables.


<details>
  <summary>Details</summary>
Motivation: Conventional frame-based eye-tracking systems fail to meet XR's requirements for high accuracy, low latency, and energy efficiency. Event cameras offer better temporal resolution and power consumption.

Method: Proposed ultra-lightweight neural network with ConvJANET layer (simplified ConvLSTM with only forget gate), custom linear approximations of activation functions, fixed-point quantization, and software-hardware co-design in 12-nm ASIC.

Result: Achieves 2.45 pixel error on 3ET+ dataset with only 17.6K parameters, 1250 Hz event frame rate, 0.5 ms latency (2000 FPS), and 18.9 μJ/frame energy efficiency at 400 MHz operation.
...
### [238] [MorphGen: Controllable and Morphologically Plausible Generative Cell-Imaging](https://arxiv.org/abs/2510.01298)
*Berker Demirel,Marco Fumero,Theofanis Karaletsos,Francesco Locatello*

Main category: q-bio.QM

TL;DR: MorphGen is a diffusion-based generative model for fluorescent microscopy that generates complete sets of fluorescent channels jointly across multiple cell types and perturbations, achieving superior image quality and biological consistency compared to prior methods.


<details>
  <summary>Details</summary>
Motivation: To accelerate high-content image-based assays for drug discovery and gene editing by simulating cellular responses to interventions in silico, overcoming limitations of prior approaches that compress multichannel stains into RGB images and lose organelle-specific detail.

Method: Uses diffusion-based generative model trained with alignment loss to match representations to OpenPhenom phenotypic embeddings, enabling controllable generation across cell types and perturbations while preserving per-organelle structures by generating complete fluorescent channels jointly.

Result: Achieves biological consistency with real images via CellProfiler features and attains FID score over 35% lower than prior state-of-the-art MorphoDiff, which only generates RGB images for a single cell type.

Conclusion: MorphGen enables fine-grained morphological analysis essential for biological interpretation and represents a significant advancement in simulating cellular responses for drug discovery and gene editing applications.
...
### [239] [A Multicentric Dataset for Training and Benchmarking Breast Cancer Segmentation in H&E Slides](https://arxiv.org/abs/2510.02037)
...
TL;DR: Introduces BEETLE dataset for breast cancer segmentation with diverse morphological features and external evaluation set.


<details>
  <summary>Details</summary>
Motivation: Existing breast cancer segmentation datasets lack morphological diversity needed for robust AI model generalizability and biomarker validation across heterogeneous patient populations.

Method: Created BEETLE dataset with 587 biopsies/resections from 3 clinical centers and 2 public datasets, digitized using 7 scanners, covering all molecular subtypes and histological grades with annotations for 4 classes (invasive epithelium, non-invasive epithelium, necrosis, other).

Result: Developed a comprehensive dataset with focus on underrepresented morphologies like ductal carcinoma in situ and dispersed lobular tumor cells, providing high potential for reuse in automated biomarker quantification.

Conclusion: BEETLE dataset enables standardized benchmarking of breast cancer segmentation models through its diversity and well-curated external evaluation set.
...
### [240] [Development and Evaluation of an AI-Driven Telemedicine System for Prenatal Healthcare](https://arxiv.org/abs/2510.01194)
*Juan Barrientos,Michaelle Pérez,Douglas González,Favio Reyna,Julio Fajardo,Andrea Lara*

Main category: cs.HC

TL;DR: AI system helps midwives capture diagnostic fetal ultrasound images using blind sweeps, with specialist review platform for underserved areas.


<details>
  <summary>Details</summary>
Motivation: Limited access to obstetric ultrasound in low-resource settings, especially rural areas of low- and middle-income countries.

Method: Human-in-the-loop AI system with classification model and web platform for specialist reviews, using blind sweep protocols with low-cost POCUS devices.

Result: Promising identification of standard fetal planes from non-expert sweeps, good usability and low cognitive workload in field evaluation.

Conclusion: System has potential to expand prenatal imaging access in underserved regions.
