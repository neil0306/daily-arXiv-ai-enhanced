...
TL;DR: Omnilingual ASR is the first large-scale, extensible ASR system that enables adding new languages with minimal data, covering over 1,600 languages including 500 previously unsupported ones.


<details>
  <summary>Details</summary>
Motivation: Most of the world's 7,000+ languages lack ASR support due to high costs and architectural limitations, creating a digital divide while raising ethical concerns about top-down development without community involvement.

Method: Uses 7B parameter self-supervised pre-training for robust speech representations, encoder-decoder architecture with LLM-inspired decoder for zero-shot generalization, trained on massive diverse corpus combining public resources and community-sourced recordings.

Result: Achieves substantial gains over prior systems in low-resource conditions, strong generalization capabilities, and expands coverage to largest scale with 1,600+ languages (500+ never served before).

Conclusion: Open-sourcing models and tools lowers barriers for researchers and communities, enabling new participation forms while addressing ethical considerations through compensated local partnerships and community collaboration.
...
TL;DR: Example ordering in in-context learning has comparable impact to example selection, challenging prior assumptions and highlighting the equal importance of both factors in prompt design.


<details>
  <summary>Details</summary>
Motivation: To challenge the common assumption in ICL that example selection matters more than ordering, and systematically compare their relative impacts on performance.

Method: Controlled experiments on classification and generation tasks using multiple model families (0.5B to 27B parameters) and GPT-5, comparing performance variance from different orderings vs different example sets.

Result: Variance from different example orderings is comparable to variance from using entirely different example sets. Strong orderings can be identified using development sets, achieving near-oracle performance.

Conclusion: Example selection and ordering are equally important and intertwined in ICL, requiring reexamination of prior assumptions about prompt design.
...
TL;DR: Morphologically-aware tokenization improves Latin language model performance, especially for out-of-domain texts, demonstrating the value of linguistic resources for morphologically rich languages.


<details>
  <summary>Details</summary>
Motivation: Standard tokenization methods prioritize compression over linguistic goals like morphological alignment, which is suboptimal for morphologically rich languages like Latin where tokenization quality impacts downstream performance.

Method: Investigated morphologically-aware tokenization for Latin using curated lexical resources, focusing on morphological alignment rather than just compression efficiency.

Result: Morphologically-guided tokenization improved performance on four downstream tasks, with most significant gains for out-of-domain texts, showing improved generalization ability.

Conclusion: Linguistic resources can significantly improve language modeling for morphologically complex languages, serving as a viable alternative to large-scale pretraining data for low-resource languages.
...
TL;DR: NLP can extract main topics from large text corpora like Presidential Directives, showing utility for research but with discrepancies from human analysis that need further validation.


<details>
  <summary>Details</summary>
Motivation: To investigate how NLP can extract signaling themes from Presidential Directives and assess its utility for analyzing large written corpora in social science research.

Method: Applied NLP to identify main topics in Presidential Directives from Reagan through Clinton administrations, comparing results with human analyst identification.

Result: Both NLP and human analysts identified relevant documents, demonstrating NLP's potential utility, but discrepancies were found between NLP and human-labeled results.

Conclusion: NLP shows promise for large corpus analysis but requires more research to validate its accuracy, especially given rapid AI/ML advancements that may improve existing tools.
...
TL;DR: Small LLMs (1B parameters) can effectively detect meaning-altering translation errors with good quality-efficiency trade-off, enabling on-device deployment for privacy-sensitive workflows.


<details>
  <summary>Details</summary>
Motivation: Large LLMs are effective at machine translation evaluation but are too resource-intensive for edge devices and privacy-sensitive applications, requiring smaller models that can still detect critical translation errors.

Method: Benchmarked sub-2B parameter models using standardized prompts, lightweight logit-bias calibration, majority voting, and fine-tuning across multiple datasets (WMT21, WMT22, SynCED-EnDe-2025).

Result: Gemma-3-1B provides the best quality-efficiency trade-off (MCC=0.77, F1-ERR=0.98) with 400ms latency on MacBook Pro M4 Pro. Qwen-3-1.7B achieves highest MCC but with higher compute cost. Ultra-small models (0.6B) are usable but under-detect certain errors.

Conclusion: Compact instruction-tuned LLMs with lightweight calibration can deliver trustworthy on-device critical error detection for machine translation, enabling private and low-cost error screening in real-world workflows.
...
TL;DR: Analysis of predicate-argument structure divergences in Chinese-English parallel sentences reveals asymmetric language transfer in cross-lingual NLP.


<details>
  <summary>Details</summary>
Motivation: To address linguistic divergences that hinder cross-lingual transfer, especially between typologically distant languages like Chinese and English.

Method: Qualitative and quantitative analysis of predicate-argument structures using annotation projection experiments between Chinese and English parallel sentences.

Result: Language transfer is asymmetric - the choice of source language significantly impacts transfer effectiveness.

Conclusion: Source language selection requires careful investigation in cross-lingual NLP applications, as asymmetric transfer affects scientific claims about cross-lingual performance.
...
TL;DR: TARG is a training-free adaptive retrieval gating method that decides when to retrieve in RAG systems using only short draft outputs and uncertainty scores, reducing retrieval by 70-90% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Retrieval-Augmented Generation (RAG) improves factuality but retrieving for every query hurts quality, increases tokens and latency, creating a need for selective retrieval.

Method: Uses single-shot policy with short no-context drafts from base model, computes lightweight uncertainty scores (mean token entropy, margin signal from top-1/top-2 logit gap, or small-N variance across stochastic prefixes), triggers retrieval only when score exceeds threshold.

Result: On NQ-Open, TriviaQA, and PopQA, TARG matches or improves EM/F1 while reducing retrieval by 70-90%, cuts end-to-end latency, and remains close to Never-RAG in overhead.

Conclusion: Margin signal is robust default under modern instruction-tuned LLMs, with small-N variance offering conservative alternative; method provides explicit budget trade-offs without training requirements.
...
<details>
  <summary>Details</summary>
Motivation: Khmer spellchecking faces unique challenges including word segmentation misalignments, multiple word forms, loose compound word formation, and proper noun misidentification due to lack of NER models. Existing solutions don't adequately address these issues.

Method: Integrated approach combining Khmer subword segmentation, named-entity recognition (NER), grapheme-to-phoneme (G2P) conversion, and a Khmer language model to identify and rank correction candidates.

Result: Achieved state-of-the-art Khmer spellchecking accuracy of 94.4%, significantly outperforming existing solutions. Created benchmark datasets for Khmer spellchecking and NER tasks.

Conclusion: The holistic approach successfully addresses Khmer-specific spellchecking challenges and sets a new performance standard. The publicly available datasets will facilitate future research in Khmer language processing.
...
TL;DR: This paper proposes a course recommendation system using data analytics and machine learning to help students select courses aligned with industry trends and career goals.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of course selection for students by bridging the gap between university learning and industry requirements, helping students make data-driven decisions.

Method: Combines data mining, collaborative filtering, and machine learning algorithms with user preferences and academic criteria. Includes iterative prototyping and user feedback for front-end interface development.

Result: Developed an optimized course recommendation system with an easy-to-use interface that provides personalized course suggestions based on industry trends and individual career goals.

Conclusion: The system serves as a valuable tool for students, instructors, and career advisors to promote lifelong learning and professional development, potentially improving graduate outcomes in higher education.
...
TL;DR: A question answering system using fine-tuned LLM with RAG method to handle course forum queries, reducing instructor workload and improving response time.


<details>
  <summary>Details</summary>
Motivation: Address challenges in course forums where increasing student enrollment leads to delayed responses and repetitive questions for instructors.

Method: Fine-tune open source LLM on course data, use RAG with local knowledge base containing course content, and integrate multi chain-of-thought reasoning to reduce hallucination.

Result: Experimental results on HotpotQA dataset show strong performance in question answering tasks.

Conclusion: The proposed system effectively handles course forum queries and reduces instructor burden through automated question answering.
...
TL;DR: TermGPT is a multi-level contrastive fine-tuning framework that addresses LLMs' isotropy problem in domain-specific terminology representation, particularly for legal and financial contexts.


<details>
  <summary>Details</summary>
Motivation: LLMs suffer from isotropy in embedding spaces, leading to poor discrimination of domain-specific terminology in legal and financial domains, which hinders critical downstream tasks like legal judgment prediction and financial risk analysis.

Method: Constructs sentence graphs to capture semantic relations, generates discriminative positive/negative samples using contextual and topological cues, and employs multi-level contrastive learning at sentence and token levels.

Result: TermGPT outperforms existing baselines in term discrimination tasks within finance and legal domains, with evaluation supported by a new financial terminology dataset from official regulatory documents.

Conclusion: The proposed multi-level contrastive fine-tuning framework effectively enhances terminology discrimination in domain-specific contexts, addressing a key limitation of current LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for training LLMs in chain-of-thought reasoning have limitations: supervised fine-tuning penalizes valid alternative rationales, while reinforcement learning struggles with credit assignment and high computational costs.
...
Result: Across six math-reasoning benchmarks, InTRO consistently outperforms baselines, improving solution accuracy by up to 20% relative to base models while producing more concise rationales with reduced verbosity.

Conclusion: InTRO demonstrates robust generalization with cross-domain transfer capabilities, successfully adapting to out-of-domain reasoning tasks beyond mathematics.
...
<details>
  <summary>Details</summary>
Motivation: LLMs have high computational and memory costs that limit deployment in resource-constrained or real-time settings, creating a need for more efficient inference methods.
...
Result: Experiments with three open-source LLMs across six benchmarks show HierRouter improves response quality by up to 2.4x compared to individual models, with only minimal additional inference cost on average.
...
TL;DR: EnchTable is a framework that maintains safety alignment in fine-tuned LLMs by using NTK-based safety vector distillation and interference-aware merging, achieving better safety and utility across diverse models and tasks.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning LLMs for specialized domains often degrades safety alignment, increasing risks of harmful outputs and undermining ethical guidelines.

Method: Uses Neural Tangent Kernel (NTK)-based safety vector distillation to decouple safety from task reasoning, and interference-aware merging to balance safety and utility across different model architectures.

Result: Achieves significantly lower unsafe rates and higher utility scores than baselines, shows robust resistance to jailbreaking attacks, and works across 3 task domains, 3 LLM architectures, and 11 datasets with generalization to different vendors.

Conclusion: EnchTable effectively maintains safety alignment in fine-tuned LLMs without extensive retraining, demonstrating universal applicability and seamless integration into deployment pipelines.
...
Result: Achieves state-of-the-art performance in both literal accuracy and semantic fidelity on the purpose-built HI-Dialogue dataset.

Conclusion: Establishes foundation for applying Omni-Models to assistive communication technology, providing end-to-end modeling framework and essential processing tools for future research.
...
TL;DR: Norm-RAG is a retrieval-augmented framework for social norm inference in multi-turn dialogues that models utterance attributes and grounds them in normative documentation using Semantic Chunking, improving norm detection and generalization.
...
Method: Norm-RAG models utterance-level attributes (communicative intent, speaker roles, interpersonal framing, linguistic cues) and grounds them in structured normative documentation via Semantic Chunking. Introduces MINDS dataset with 31 bilingual multi-turn conversations annotated for norm categories and adherence.
...
Conclusion: The framework enables interpretable and context-aware reasoning about norm adherence and violation across multilingual dialogues, addressing the challenges of subjective and culturally dependent social norm inference.
...
<details>
  <summary>Details</summary>
Motivation: Manual KC identification by experts is a bottleneck in adaptive learning systems. LLMs offer automation potential but suffer from generating excessive and redundant KC labels when scaled.

Method: Used GPT-4o-mini to generate KCs for 646 questions via 'simulated textbook' prompting, then applied cosine similarity-based merging to combine semantically similar KC labels.

Result: Initial LLM approach performed worse than expert model (RMSE 0.4285 vs 0.4206) with 569 vs 101 KCs. After merging at 0.8 similarity threshold, KC count reduced to 428 and RMSE improved to 0.4259.

Conclusion: Scaled LLM generation alone is insufficient for KC identification, but combining it with semantic merging provides a viable automation path that refines and improves the process.
...
TL;DR: REAP is a novel RAG method for multi-hop reasoning that uses recursive evaluation and adaptive planning to maintain global reasoning perspective and extract fine-grained facts, outperforming existing methods.
...
Method: Proposes REAP with two core modules: Sub-task Planner (SP) for global reasoning direction and task state evaluation, and Fact Extractor (FE) for fine-grained analysis of retrieved content. Uses recursive evaluation and adaptive planning to maintain structured sub-tasks and facts.
...
Conclusion: REAP enhances reliability and traceability of multi-hop reasoning through its structured approach and unified task paradigm, proving effective for complex reasoning tasks.
...
TL;DR: LLMs struggle with numerical reasoning in fact-checking tasks, showing significant accuracy drops (up to 62%) under perturbations, with no model proving robust across all conditions.


<details>
  <summary>Details</summary>
Motivation: Large language models perform well on knowledge-intensive tasks but have limitations in numerical reasoning, requiring systematic evaluation of their robustness in numerical fact-checking scenarios.
...
Result: Leading proprietary systems experience accuracy drops up to 62% under certain perturbations; increasing context length generally reduces accuracy; enriched context with perturbed demonstrations helps most models recover substantially.

Conclusion: Current language models have critical limitations in numerical fact-checking, and robustness remains an open challenge despite some recovery with enriched context.
...
TL;DR: ETC is a training-free method that uses entropy trend analysis to determine optimal retrieval timing in dynamic RAG systems, enabling earlier and more precise knowledge retrieval than confidence-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic RAG methods trigger retrieval based on low token-level confidence, which often leads to delayed intervention after errors have already propagated through the generation process.

Method: ETC utilizes first- and second-order differences of the entropy sequence to detect emerging uncertainty trends, enabling trend-aware uncertainty modeling for retrieval timing decisions without requiring additional training.

Result: Experiments on six QA benchmarks with three LLM backbones show ETC consistently outperforms strong baselines while reducing retrieval frequency, with particular effectiveness in domain-specific scenarios and robust generalization capabilities.

Conclusion: Trend-aware uncertainty modeling yields more effective retrieval timing than confidence-based approaches, and ETC provides a plug-and-play, model-agnostic solution that can be readily integrated into existing decoding pipelines.
...
TL;DR: Multilingual RAG systems suffer from language drift where models generate responses in unintended languages, especially during reasoning. The paper identifies decoder-level collapse as the cause and proposes Soft Constrained Decoding to mitigate this issue.


<details>
  <summary>Details</summary>
Motivation: To address the problem of output language drift in multilingual RAG systems where retrieved evidence differs in language from user queries, causing models to generate responses in unintended languages during reasoning-intensive tasks.

Method: Proposed Soft Constrained Decoding (SCD) - a lightweight, training-free decoding strategy that penalizes non-target-language tokens to gently steer generation toward the target language without modifying model architecture or requiring additional data.

Result: Experiments across three multilingual datasets and diverse languages show that SCD consistently improves language alignment and task performance in multilingual RAG settings.

Conclusion: SCD provides an effective and generalizable solution to mitigate language drift in multilingual RAG systems by addressing decoder-level collapse through gentle language constraints during generation.
...
<details>
  <summary>Details</summary>
Motivation: BERTScore's inability to detect numerical variations is problematic in finance where numerical precision directly affects meaning (e.g., distinguishing 2% gain vs 20% loss).
...
Conclusion: Reveals fundamental limitations of embedding-based metrics for finance and motivates numerically-aware evaluation frameworks for financial NLP.
...
<details>
  <summary>Details</summary>
Motivation: LLMs show potential for personalized education but face challenges adapting to specific curricula like NCERT in India, requiring accuracy and pedagogical alignment.

Method: Developed NCERT-QA dataset with Factoid, Inferential, and Other question types; evaluated using meta-prompt, few-shot, and CoT prompting on various LLMs.

Result: Analyzed performance of open-source LLMs (Gemma3, Llama3.2, Nemotron-mini) and high-end models (Llama-4-Scout, Deepseek-r1) as educational tools.

Conclusion: Framework provides insights into LLM strengths/limitations for curriculum-aligned education, enabling better AI learning tool development for formal education systems.
...
<details>
  <summary>Details</summary>
Motivation: Standard self-attention has quadratic complexity that limits Transformer models for long-context tasks, and existing efficient variants require architectural changes and costly pre-training from scratch.

Method: Segment long inputs into overlapping chunks, generate compressed context-aware representations for the decoder, and use a parameter-free fusion mechanism that enriches chunk boundaries with cumulative context from preceding and succeeding chunks.
...
Conclusion: ScaleFormer provides a simple plug-and-play framework that adapts pre-trained models to process long sequences effectively while maintaining linear complexity and strong performance on long-context tasks.
...
TL;DR: MLLMs show phonetic intuition aligning with linguistic research on sound symbolism, using attention patterns to focus on iconic phonemes across text and audio modalities.


<details>
  <summary>Details</summary>
Motivation: To investigate how Multimodal Large Language Models interpret auditory information through phonetic iconicity, bridging AI and cognitive linguistics.

Method: Created LEX-ICON dataset with 8,052 words from 4 languages and 2,930 pseudo-words, analyzed MLLMs' layer-wise processing using phoneme-level attention scores across 25 semantic dimensions.

Result: MLLMs demonstrate phonetic intuitions that align with linguistic research and show phonosemantic attention patterns focusing on iconic phonemes.

Conclusion: First large-scale quantitative analysis of phonetic iconicity in MLLMs, providing insights into models' interpretability and bridging AI with cognitive linguistics.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat multi-turn dialogue responses as isolated tasks and fail to explicitly incorporate relational constraints across turns, leading to poor performance on complex long-distance constraints in conversations.

Method: Three components: 1) agent-based relation extraction to construct dialogue graphs, 2) relation graph prompt generation to convert graphs to natural language, 3) response rewriting using graph prompts to refine LLM outputs.

Result: Significant improvements across all four multi-turn instruction-following evaluation metrics on two long multi-turn dialogue datasets when integrated with instruction-tuned LLMs.

Conclusion: Graph structures are effective for modeling multi-turn instruction following, and GraphIF provides a seamless plug-and-play solution that enhances LLM capabilities without requiring dataset collection or fine-tuning.
...
TL;DR: ADI-20 extends ADI-17 to cover all Arabic dialects with 3,556 hours of data from 19 dialects plus MSA, used to train and evaluate state-of-the-art ADI systems including ECAPA-TDNN and Whisper-based models.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive Arabic dialect identification dataset covering all Arabic-speaking countries and enable training and evaluation of advanced ADI systems.

Method: Extended ADI-17 dataset to ADI-20 with 3,556 hours from 19 dialects plus MSA; fine-tuned pre-trained ECAPA-TDNN models and Whisper encoder blocks with attention pooling and classification layers; studied effects of training data size and model parameters.

Result: Small decrease in F1 score when using only 30% of training data; dataset and models open-sourced for reproducibility.

Conclusion: ADI-20 provides comprehensive dialect coverage and enables effective ADI system development with minimal performance loss using reduced training data.
...
<details>
  <summary>Details</summary>
Motivation: With increasing scientific paper submissions, there's a need for systems to assist reviewers in evaluating research claims. Current multimodal LLMs' robustness in verifying scientific claims across different evidence formats (tables vs charts) remains underexplored.

Method: Adapted two existing scientific paper datasets with annotations for multimodal claim verification, then evaluated 12 multimodal LLMs on their ability to verify claims using both table and chart evidence. Also conducted human evaluations for comparison.
...
Conclusion: Current multimodal LLMs have a critical gap in chart understanding capabilities. Future models should focus on improving chart understanding to better support scientific claim verification.
...
TL;DR: Elyadata & LIA's NADI 2025 submission achieved 1st place in Arabic Dialect Identification and 2nd place in multi-dialectal Arabic ASR using fine-tuned large pre-trained models.


<details>
  <summary>Details</summary>
Motivation: To develop effective systems for Arabic dialect identification and automatic speech recognition across multiple Arabic dialects using modern speech processing approaches.
...
Result: Achieved 79.83% accuracy on ADI test set, and average WER of 38.54% and CER of 14.53% on multi-dialectal ASR test set.

Conclusion: Large pre-trained speech models with targeted fine-tuning are effective for Arabic speech processing tasks across multiple dialects.
...
TL;DR: This paper examines military applications of NLP and LLMs like GPT, testing Microsoft Copilot's knowledge and Azure's capabilities for building such applications.


<details>
  <summary>Details</summary>
Motivation: To explore how natural language processing and large language models can be applied in military contexts, given their recent advancements and capabilities.

Method: 1) Interrogated GPT-based models (Microsoft Copilot) to reveal their knowledge about military applications; 2) Studied how commercial cloud services (Microsoft Azure) could be used to build such applications and assessed feasibility.

Result: Found that summarization and generative properties of language models directly facilitate many military applications, with other features having specific uses.

Conclusion: LLMs have significant potential for military applications, with summarization and generation capabilities being particularly useful, and commercial cloud services provide feasible implementation platforms.
...
TL;DR: Proposes a causal approach to mitigate event-related biases in social media disaster monitoring systems, improving generalization to future events.


<details>
  <summary>Details</summary>
Motivation: Social media platforms are essential for disaster monitoring but existing systems suffer from event-related biases that limit generalization to emerging events. Debiasing methods are underexplored in this domain.

Method: Uses causal learning approach to reduce event- and domain-related biases in disaster classification systems.
...
Conclusion: Causal bias mitigation effectively enhances generalization in disaster monitoring systems, addressing key limitations of existing approaches.
...
TL;DR: VISTA is a web-based visual interactive system that helps analyze LLM reasoning in multi-turn conversations through visualization, interactive modification, and automatic reasoning dependency trees.
...
Result: VISTA significantly reduces the complexity of analyzing reasoning chains and provides a transparent view of models' step-by-step logical paths across different LLMs.
...
TL;DR: Text2SQL-Flow is a SQL-aware data augmentation framework that generates large-scale, diverse Text-to-SQL pairs from minimal seed data, creating SQLFlow dataset (89,544 examples) that improves LLM performance in both fine-tuning and retrieval settings.


<details>
  <summary>Details</summary>
Motivation: Address limitations of scarce, simplistic, and low-diversity datasets in Text-to-SQL by creating a scalable data-centric solution for generating high-quality training data.
...
Result: Created SQLFlow dataset of 89,544 examples; fine-tuning on SQLFlow improves open-source LLM performance; masked alignment retrieval method outperforms existing methods for closed-source LLMs.
...
TL;DR: EffiReason-Bench is a unified benchmark for evaluating efficient reasoning methods in LLMs, addressing fragmented evaluation practices and proposing the E3-Score metric for smooth trade-off assessment.


<details>
  <summary>Details</summary>
Motivation: Current LLMs with Chain-of-Thought prompting produce unnecessarily long explanations, increasing costs and sometimes reducing accuracy. Fair comparison of efficiency-oriented approaches is hindered by fragmented evaluation practices.

Method: Introduces EffiReason-Bench with verified CoT annotations for CommonsenseQA and LogiQA using a pipeline that enforces standardized reasoning structures, comprehensive option-wise analysis, and human verification. Evaluates 7 methods across 6 open-source LLMs (1B-70B) on 4 datasets spanning mathematics, commonsense, and logic.
...
Conclusion: Efficient reasoning methods require careful selection based on specific contexts, and EffiReason-Bench enables rigorous cross-paradigm evaluation to identify optimal approaches for different scenarios.
...
TL;DR: Proposes PAL framework for personalized dialogue generation that treats persona alignment as direct training objective, outperforming SOTA methods and LLMs.


<details>
  <summary>Details</summary>
Motivation: Current models using token-level training like Next Token Prediction tend to neglect personas and generate generic responses, needing better persona sensitivity.

Method: Two-stage training: Persona-aware Learning and Persona Alignment, with Select then Generate inference strategy to improve persona relevance at semantics level.

Result: Outperforms state-of-the-art personalized dialogue methods and large language models in experiments.

Conclusion: PAL framework effectively addresses persona neglect by directly optimizing for persona alignment, generating more persona-relevant responses.
...
<details>
  <summary>Details</summary>
Motivation: Existing multilingual data selection methods overlook linguistic structure, making multilingual capability sensitive to training data composition. Language separability provides a new perspective for more effective data selection.

Method: Two-stage framework: 1) Filter training data based on language separability scores (how distinguishable languages are in representation space), 2) Refine subset using existing selection methods like text quality, diversity, or task relevance.
...
Conclusion: Language separability offers a new perspective on data utility in multilingual contexts and can guide curriculum learning. The approach supports development of more linguistically informed LLMs by considering intrinsic linguistic structure in data selection.
...
<details>
  <summary>Details</summary>
Motivation: Current end-to-end spoken language models suffer from high response latency due to autoregressive token generation and complex flow-matching models for speech synthesis, limiting real-time interactive applications.

Method: Integrates multi-codebook tokenizer and multi-token prediction strategy to directly generate multi-codebook speech tokens, bypassing the need for flow-matching models while improving generation efficiency.

Result: Achieves substantial latency reduction (from ~725ms to ~350ms for first chunk) while maintaining competitive performance compared to mainstream SLMs, with improved generation efficiency.

Conclusion: VocalNet-M2 provides an effective solution for low-latency spoken language processing and offers valuable insights on multi-codebook strategies for developing efficient real-time interactive SLMs.
...
TL;DR: MTR-DuplexBench is a new benchmark for evaluating Full-Duplex Speech Language Models (FD-SLMs) in multi-round conversations, addressing limitations of existing single-round benchmarks by enabling turn-by-turn assessment of dialogue quality, conversational dynamics, instruction following, and safety.


<details>
  <summary>Details</summary>
Motivation: Existing FD-SLM benchmarks focus on single-round interactions and overlook multi-round communication complexities, instruction following, and safety. Current evaluation methods struggle with blurred turn boundaries and context inconsistency in real-time conversations.
...
Result: Experimental results show that current FD-SLMs have difficulties maintaining consistent performance across multiple rounds and different evaluation dimensions, demonstrating the need for more comprehensive evaluation methods.

Conclusion: MTR-DuplexBench effectively addresses the limitations of existing FD-SLM evaluation methods and reveals critical performance gaps in current models, highlighting the importance of multi-round evaluation for full-duplex conversational AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Organizations face a dilemma between using cloud-based AI systems that compromise data privacy or maintaining local processing that delivers poor accuracy. This work aims to resolve this trade-off.

Method: Combines semantic understanding with keyword precision using two complementary retrieval strategies, operates entirely on local infrastructure without internet access, and uses consumer-grade hardware acceleration.

Result: The system achieves competitive accuracy on complex queries across legal, scientific, and conversational documents while keeping all data on local machines, with minimal errors.
...
TL;DR: The paper identifies imbalanced evaluation preference in LLMs' mathematical reasoning critiques, proposes a perplexity-aware reinforcement learning method to rectify this bias, and validates the approach on a new benchmark.


<details>
  <summary>Details</summary>
Motivation: To improve LLMs' multi-step mathematical reasoning by addressing the underlying issue of imbalanced evaluation preference where models tend to judge solutions with lower perplexity as correct, rather than just creating more fine-tuning demonstrations.

Method: Built a One-to-many Problem-Solution (OPS) benchmark to quantify LLMs' critiquing behavior, conducted statistical preference analysis on perplexity, and proposed Group Relative Policy Optimization using perplexity as guidance to explore trajectories that correct the imbalanced evaluation preference.

Result: Extensive experiments on OPS and existing critic benchmarks demonstrate the method's effectiveness in improving LLMs' critiquing capability by rectifying the imbalanced evaluation preference.

Conclusion: The proposed perplexity-aware reinforcement learning successfully addresses the core issue of imbalanced evaluation preference in LLMs' mathematical reasoning critiques, leading to enhanced critiquing performance.
...
TL;DR: Systematic study on generating and evaluating synthetic multilingual pretraining data for Indic languages, creating BhashaKritika dataset (540B tokens) using 5 techniques across 10 languages, with quality evaluation pipeline.


<details>
  <summary>Details</summary>
Motivation: Address uneven distribution of LLM benefits across languages, particularly in low-resource settings, by using synthetic data as scalable alternative for high-quality pretraining data.

Method: Constructed large-scale synthetic dataset using 5 generation techniques; explored grounding in documents, personas, topics; compared translation vs native generation; developed modular quality evaluation pipeline with script/language detection, metadata checks, n-gram analysis, and perplexity filtering.

Result: Created BhashaKritika dataset of 540B tokens across 10 Indic languages; empirical model runs revealed trade-offs in generation strategies and identified best practices for multilingual corpora construction.

Conclusion: Proposed framework enables robust quality control for synthetic multilingual data; highlighted key trade-offs and best practices for effective multilingual corpus construction in low-resource language settings.
...
TL;DR: ATR4CH is a systematic LLM-based methodology for extracting Cultural Heritage knowledge from texts into structured Knowledge Graphs, validated through authenticity assessment case studies with high performance metrics.


<details>
  <summary>Details</summary>
Motivation: Cultural Heritage texts contain rich knowledge but are difficult to query systematically due to unstructured discourse, requiring conversion into structured Knowledge Graphs.
...
Result: High performance across extraction tasks: 0.96-0.99 F1 for metadata, 0.7-0.8 F1 for entity recognition, 0.65-0.75 F1 for hypothesis extraction, 0.95-0.97 for evidence extraction, and 0.62 G-EVAL for discourse representation. Smaller models performed competitively.
...
TL;DR: TruthfulRAG is a framework that uses Knowledge Graphs to resolve factual-level knowledge conflicts in Retrieval-Augmented Generation systems, outperforming existing methods by improving response accuracy and trustworthiness.
...
TL;DR: Evaluating base LLMs' reasoning capabilities is methodologically flawed due to mismatch between pretraining objectives and normative reasoning qualities.
...
Method: Analysis of the fundamental mismatch between base LLMs' pretraining objective (linguistic pattern conformity) and normative reasoning qualities (correctness, logical validity).

Result: Base LLMs generate logically valid/invalid conclusions as coincidental byproducts of linguistic pattern adherence, not as genuine reasoning attempts.

Conclusion: Existing work on base LLMs' reasoning should be re-examined, and future work must account for these methodological pitfalls regarding generalization to post-trained LLMs.
...
TL;DR: DELICATE is a neuro-symbolic Entity Linking method for historical Italian that combines BERT with Wikidata context, outperforming larger models and providing explainable results. ENEIDE is a new multi-domain corpus for this task.
...
Method: DELICATE combines BERT-based encoder with contextual Wikidata information using temporal plausibility and entity type consistency. ENEIDE corpus is semi-automatically extracted from 19th-20th century literary and political texts.

Result: DELICATE outperforms other EL models in historical Italian, even compared to larger architectures with billions of parameters. It provides more explainable and interpretable results than purely neural methods.

Conclusion: The neuro-symbolic approach effectively addresses EL challenges in humanities, with DELICATE showing superior performance and explainability, while ENEIDE provides valuable domain-specific resources.
...
TL;DR: Analogical paradigm organization enables lightweight models to achieve competitive performance with minimal data (100 examples vs. vast datasets), outperforming GPT-3 on linguistic rule learning tasks.


<details>
  <summary>Details</summary>
Motivation: To explore whether cognitive-inspired analogical organization can help lightweight models match large language model performance with dramatically less training data.

Method: Computational approach implementing three principles: analogical structure, contrastive learning, and minimal contextual cues. Tested with structured completion tasks using BERT+CNN models (0.5M parameters) on English causative/inchoative alternations.

Result: Lightweight models trained on only 100 examples achieved F1=0.95, outperforming zero-shot GPT-3 (F1=0.87). Ablation studies confirmed analogical organization and contrastive structure improve performance. Cross-phenomenon validation replicated efficiency gains.
...
<details>
  <summary>Details</summary>
Motivation: Address intent misunderstandings in LLMs that frustrate users and create safety risks when models implicitly commit to one interpretation.

Method: Train models with reinforcement learning and customized reward functions using multiple valid answers as supervision.
...
Conclusion: Approach promotes transparency with explicit interpretations, achieves efficiency with one generation step, and supports downstream applications through structured output.
...
TL;DR: LLMs can track state in complex tasks, with newer models like GPT-4 and Llama3 performing well using Chain of Thought, while older models fail after multiple steps.


<details>
  <summary>Details</summary>
Motivation: To evaluate LLMs' state tracking capabilities independently from other factors by creating a focused benchmark.

Method: Proposed a benchmark with three well-defined state tracking tasks and tested LLMs across different scenarios, including with Chain of Thought integration.

Result: Recent LLMs (GPT-4, Llama3) successfully track state, especially with Chain of Thought, while older models fail after several steps despite understanding the task initially.

Conclusion: Newer LLMs demonstrate effective state tracking capabilities, but performance depends on model generation and reasoning mechanisms like Chain of Thought.
...
TL;DR: LocalBench is the first benchmark for evaluating LLMs on county-level local knowledge across the US, revealing significant limitations in handling hyper-local information despite advances in macro-scale geographic reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to capture hyper-local knowledge needed for real-world applications like civic platforms and community journalism, creating a gap in understanding LLMs' ability to reason about neighborhood-specific dynamics and local governance.

Method: Developed LocalBench with 14,782 validated question-answer pairs across 526 US counties, integrating Census data, local subreddit discourse, and regional news across physical, cognitive, and relational dimensions of locality.

Result: Even top-performing LLMs achieve only 56.8% accuracy on narrative questions and below 15.5% on numerical reasoning. Web augmentation shows inconsistent benefits - improves Gemini by +13.6% but reduces GPT-series performance by -11.4%.
...
TL;DR: KPPO is a knowledge-provision-based prompt optimization framework that systematically integrates knowledge into prompts rather than just eliciting model capabilities, achieving ~6% performance improvement over baselines while reducing token usage by up to 29%.


<details>
  <summary>Details</summary>
Motivation: Existing prompt optimization methods focus on elicitation-based strategies that operate within fixed parametric boundaries, lacking the factual knowledge, terminology precision, and reasoning patterns needed for knowledge-intensive tasks in specialized domains.

Method: KPPO introduces three innovations: 1) knowledge gap filling mechanism for identification and remediation, 2) batch-wise candidate evaluation considering performance and distributional stability, 3) adaptive knowledge pruning strategy balancing performance and token efficiency.

Result: Extensive evaluation on 15 knowledge-intensive benchmarks shows KPPO achieves ~6% average performance improvement over the strongest baseline while achieving comparable or lower token consumption, with up to 29% token reduction.

Conclusion: KPPO successfully reformulates prompt optimization as systematic knowledge integration rather than potential elicitation, demonstrating superiority over elicitation-based methods for knowledge-intensive tasks across various domains.
...
TL;DR: The paper introduces AdvancedIF, a benchmark for evaluating complex instruction following in LLMs, and proposes RIFL, a training pipeline using rubric-based reinforcement learning to improve instruction following capabilities.


<details>
  <summary>Details</summary>
Motivation: Advanced instruction following for complex, multi-turn, and system-prompted instructions remains challenging in LLMs, hindered by lack of high-quality benchmarks and reliable reward signals for training.

Method: Proposes RIFL (Rubric-based Instruction-Following Learning) pipeline with rubric generation, finetuned rubric verifier, and reward shaping for reinforcement learning. Also introduces AdvancedIF benchmark with 1,600+ prompts and expert-curated rubrics.

Result: RIFL achieves 6.7% absolute gain on AdvancedIF benchmark and strong results on public benchmarks. Ablation studies confirm effectiveness of each RIFL component.
...
<details>
  <summary>Details</summary>
Motivation: Olympiad-level physics problems require sophisticated integration of calculation, abstract reasoning, and physical principles, making them challenging for both humans and AI. The Chinese Physics Olympiad serves as an ideal testbed for advanced capabilities.
...
Result: Achieved near-perfect score of 313 out of 320 points, surpassing the highest-scoring human competitor and significantly outperforming all baseline methods.

Conclusion: LOCA-R demonstrates strong performance on complex physics reasoning tasks, showing potential for advanced AI problem-solving capabilities.
...
TL;DR: Linguistic style manipulation (fear, curiosity, compassion) can significantly increase jailbreak success rates in LLMs, revealing a systemic vulnerability that current safety evaluations overlook.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety evaluations focus on semantic equivalence but overlook linguistic style variation as an attack surface for jailbreaks.

Method: Created style-augmented jailbreak benchmark by transforming prompts from 3 datasets into 11 linguistic styles using templates and LLM rewrites, then tested 16 models and introduced style neutralization preprocessing.

Result: Stylistic reframing increased jailbreak success rates by up to +57 percentage points, with fearful, curious and compassionate styles being most effective. Style neutralization significantly reduced jailbreak rates.

Conclusion: Linguistic style manipulation represents a systemic and scaling-resistant vulnerability in current LLM safety pipelines that requires new mitigation approaches.
...
<details>
  <summary>Details</summary>
Motivation: Address fundamental challenges in current memory evaluation frameworks including statistical power, data generation consistency, and evaluation flexibility that limit existing benchmarks.

Method: Created a large-scale benchmark with 75,336 question-answer pairs across diverse categories (user facts, assistant recall, abstention, preferences, temporal changes, implicit connections) and compared full-context approaches with RAG-based memory systems.

Result: Simple full-context approaches achieve 70-82% accuracy on challenging multi-message evidence cases, while sophisticated RAG-based memory systems achieve only 30-45% for conversations under 150 interactions. Identified transition points: long context excels for first 30 conversations, viable up to 150 conversations, requires hybrid/RAG beyond that.

Conclusion: Conversational memory's small-corpus advantage enables exhaustive search approaches that outperform general RAG solutions, indicating dedicated research is needed rather than simply applying RAG to conversation histories.
...
<details>
  <summary>Details</summary>
Motivation: To explore whether genre is a valid formal designation vs institutional one, examining how narrative features and author gender influence literary classification.

Method: Used CONLIT dataset; Welch's ANOVA to compare narrative features by gender and genre; logistic regression modeling literary classification; analyzed stylistic/semantic vector representations.

Result: Found statistically significant formal markers for each literary category; female authorship narrows and blurs criteria for achieving literary status.

Conclusion: Genre has valid formal characteristics, but female authorship creates different pathways to literary recognition, suggesting institutional factors still play a role.
...
TL;DR: URaG is a framework that unifies retrieval and generation in multimodal large language models for efficient long document understanding, achieving SOTA performance while reducing computation by 44-56%.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs struggle with long document understanding due to information interference from irrelevant content and quadratic computational costs of Transformers. Current approaches either sacrifice details through token compression or add complexity with external retrievers.

Method: URaG introduces a lightweight cross-modal retrieval module that converts early Transformer layers into an evidence selector, identifying relevant pages while discarding irrelevant content. This enables deeper layers to focus computational resources on pertinent information.
...
Conclusion: URaG successfully leverages MLLMs' inherent evidence localization capabilities to perform retrieval during reasoning, enabling efficient long document understanding through a unified retrieval-generation framework.
...
TL;DR: DESS improves Aspect Sentiment Triple Extraction by integrating DeBERTa's enhanced attention mechanism with LSTM in a dual-channel framework, achieving significant performance gains in identifying aspect-opinion pairs and sentiment accuracy.


<details>
  <summary>Details</summary>
Motivation: Current ASTE approaches using BERT and GNNs haven't fully leveraged advanced language models' potential for understanding complex language patterns and relationships between aspects, opinions, and sentiment polarities.

Method: Dual-channel framework combining DeBERTa (for enhanced attention and context understanding) with LSTM (for grammatical patterns), with refined component integration focusing on language information interactions.

Result: Achieved F1-score improvements of 4.85, 8.36, and 2.42 on standard datasets for aspect-opinion pair identification and sentiment accuracy; DeBERTa's attention helps handle complex sentence structures and long-distance dependencies.

Conclusion: Thoughtful integration of advanced language models like DeBERTa can lead to substantial improvements in fine-grained sentiment analysis, particularly for handling complex linguistic structures.
...
TL;DR: MedGemma model evaluated for medical order extraction using three prompting methods; one-shot approach outperformed more complex ReAct and agentic workflows on validation data.


<details>
  <summary>Details</summary>
Motivation: To reduce clinical documentation burdens and ensure patient safety by accurately extracting medical orders from doctor-patient conversations.

Method: Systematic evaluation of three prompting paradigms with MedGemma: one-shot approach, ReAct framework, and multi-step agentic workflow.

Result: One-shot prompting achieved highest performance on official validation set, while more complex frameworks showed lower performance.

Conclusion: Simple one-shot prompting is more robust and efficient for clinical order extraction on manually annotated transcripts, as complex reasoning can introduce noise through 'overthinking'.
...
TL;DR: Two approaches for AI-assisted wound care response generation: mined prompting using similar examples as demonstrations, and metadata-guided generation using predicted clinical attributes to enhance response quality.


<details>
  <summary>Details</summary>
Motivation: Address the increased provider workload from asynchronous remote care by developing AI systems to efficiently manage patient queries, particularly for wound care with images.

Method: 1) Mined prompting: embed training data and retrieve top-k similar examples as few-shot demonstrations. 2) Metadata-guided generation: use classifiers to predict four key metadata attributes and incorporate them into generation pipeline with confidence-based adjustments.

Result: Mined prompting improves response relevance, while metadata-guided generation further enhances clinical precision. Combined methods show promise for reliable wound care support.

Conclusion: The complementary approaches demonstrate promising directions for developing AI tools that can provide efficient and reliable wound care support by improving both relevance and clinical precision of generated responses.
...
TL;DR: Encoder-augmented causal decoder models achieve better compression than causal transformers and enable per-token entropy estimation, showing that models trained to approach estimated entropy exhibit superior generalization.


<details>
  <summary>Details</summary>
Motivation: Current causal language models are computationally infeasible for accurate language entropy estimation, and there's a need for more efficient architectures that can approach the theoretical entropy limits of language.

Method: Introduced encoder-augmented causal decoder model architectures that combine causal decoding with encoder components for superior training efficiency and compression performance.

Result: Achieved higher compression than causal transformers on modest hardware, enabled per-token entropy estimation, and demonstrated that models trained to approach estimated entropy exhibit better generalization than those trained beyond entropy limits.

Conclusion: Training language models to approach but not exceed estimated per-token entropies leads to improved generalization, providing a practical approach to model training that respects the fundamental information-theoretic constraints of language.
...
TL;DR: SSR is a framework that improves LLM reasoning by decomposing responses into verifiable sub-steps, enabling fine-grained confidence estimation and iterative refinement of unreliable reasoning steps.


<details>
  <summary>Details</summary>
Motivation: Existing self-verification and self-correction methods for LLMs are too coarse-grained, limiting their effectiveness on complex reasoning tasks that require precise step-level evaluation.
...
Conclusion: SSR provides both performance improvements and a principled black-box approach for evaluating and understanding LLM reasoning processes, offering more accurate and interpretable reasoning chains.
...
TL;DR: Instella is a family of fully open 3B parameter language models trained on open data and codebase, achieving SOTA results among fully open models and competing with leading open-weight models of similar size.
...
Method: Large-scale pre-training on openly available data using AMD Instinct MI300X GPUs, followed by general-purpose instruction tuning and human preference alignment. Also developed specialized variants: Instella-Long for 128K context and Instella-Math enhanced for mathematical reasoning.
...
TL;DR: Generative Adversarial Distillation (GAD) enables black-box LLM distillation by framing the student as a generator and training a discriminator to distinguish its outputs from the teacher's, creating a minimax game.


<details>
  <summary>Details</summary>
Motivation: To enable effective distillation from proprietary teacher models without access to their internal parameters or logits, using only text outputs.

Method: GAD frames student LLM as generator, trains discriminator to distinguish student vs teacher responses, creating co-evolving minimax game with discriminator serving as on-policy reward model.

Result: GAD consistently outperforms sequence-level knowledge distillation, with Qwen2.5-14B-Instruct student becoming comparable to GPT-5-Chat teacher on LMSYS-Chat evaluation.

Conclusion: GAD establishes a promising and effective paradigm for black-box LLM distillation through adversarial training approach.
...
<details>
  <summary>Details</summary>
Motivation: Existing PTQ methods struggle with outliers in weights and activations of LLMs, especially in reasoning tasks where errors accumulate across long chains of thought, leading to accuracy degradation.

Method: Combines hardware-efficient independent Givens rotations with channel-wise scaling to balance magnitude across channels and reduce dynamic range within quantization groups, with co-designed inference kernels for GPU parallelism.

Result: Achieves average 2.4% accuracy improvement over AWQ on reasoning tasks with less than 10% inference overhead.

Conclusion: Enables more efficient and accurate deployment of reasoning LLMs by effectively handling outliers while maintaining low computational overhead.
...
TL;DR: FedeCouple is a federated learning method that balances global generalization and local adaptability through joint learning of feature representations, dynamic knowledge distillation, and privacy-preserving anchors.
...
Method: Jointly learns global and local feature representations, employs dynamic knowledge distillation for classifier generalization, and introduces privacy-preserving anchors to refine feature space while reducing communication overhead.
...
Conclusion: FedeCouple effectively addresses the coordination challenge between feature extractors and classifiers in federated learning, providing theoretical convergence guarantees and superior practical performance while maintaining privacy.
...
TL;DR: The paper identifies a failure mode in thinking-aware generation where sequential approaches degrade performance due to error propagation, and proposes MMaDA-Parallel, a parallel multimodal diffusion framework that improves cross-modal alignment by enabling continuous text-image interaction throughout denoising.
...
Method: Proposes MMaDA-Parallel, a parallel multimodal diffusion framework that enables continuous bidirectional interaction between text and images throughout denoising, trained with supervised finetuning and optimized by Parallel Reinforcement Learning (ParaRL) with semantic rewards.
...
TL;DR: PriVi is a large-scale primate video dataset that enables better computer vision models for primate behavior analysis through data-centric pretraining, outperforming existing methods across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current computer vision methods for primate behavior analysis rely on human-centric models and single datasets, limiting generalization. There's a need for primate-specific pretraining data to improve performance.

Method: Created PriVi dataset with 424 hours of curated primate videos from research and web sources. Pretrained V-JEPA model on PriVi and evaluated with frozen classifier on four benchmark datasets.

Result: Outperformed prior work including fully finetuned baselines across ChimpACT, BaboonLand, PanAf500, and ChimpBehave datasets. Showed better scaling with fewer labels and improved data efficiency.
...
Method: Modified ordinal regression framework with novel loss functions that incorporate soft labels representing annotator rating distributions to account for label uncertainty.

Result: Predictive performance approaching clinical expert levels with well-calibrated uncertainty estimates.
...
TL;DR: SliderEdit enables continuous, fine-grained control over individual instructions in multi-instruction image editing by exposing each instruction as an adjustable slider, allowing users to precisely control edit intensity.


<details>
  <summary>Details</summary>
Motivation: Current instruction-based image editing models apply instructions with fixed strength, limiting users' ability to precisely control the intensity of individual edits in multi-instruction prompts.
...
Result: Substantial improvements in edit controllability, visual consistency, and user steerability when applied to state-of-the-art models like FLUX-Kontext and Qwen-Image-Edit, enabling continuous interpolation along edit dimensions while preserving spatial locality and semantic consistency.
...
<details>
  <summary>Details</summary>
Motivation: To address temporal challenges in video-based crowd analysis and create a scalable framework for real-time crowd monitoring in applications like public safety and event management.

Method: Integrates denoising probabilistic model with diffusion processes, uses narrow Gaussian kernels for multiple density maps, incorporates regression branch for feature extraction, consolidation mechanism for combining maps, and event-driven sampling with Farneback optical flow to capture significant crowd movements.

Result: Model effectively captures crowd dynamics in both dense and sparse settings, reduces computational load and storage through selective frame sampling while maintaining essential crowd events, as validated by qualitative and quantitative evaluations including MAE.

Conclusion: The work provides an efficient and scalable framework for real-time crowd monitoring that addresses video-specific temporal challenges, making it suitable for practical applications in public safety and event management.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of existing vision-based indoor localization methods like PALMS, which are constrained by smartphone LiDAR's short range and indoor layout ambiguities.

Method: Reconstructs scale-aligned 3D point clouds from posed RGB images using Depth Pro foundation model, then performs geometric layout matching via convolution with floor plans to output location/orientation posteriors.

Result: Outperforms PALMS and F3Loc on Structured3D and campus datasets (80 observations across 4 buildings) in stationary accuracy without training. Achieves lower errors in sequential localization on 33 real-world trajectories.

Conclusion: PALMS+ demonstrates robust infrastructure-free indoor localization suitable for camera-free tracking applications, offering improved performance over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Most pedestrian trajectory prediction approaches treat pedestrians as point entities, ignoring the physical space each person occupies, leading to unrealistic collision predictions in dynamic and crowded environments.

Method: Proposes a Dynamic Occupied Space loss function that combines average displacement error with a density-sensitive collision penalty, integrated into Social LSTM framework. Uses five datasets from real pedestrian trajectories with varying crowd densities for training and evaluation.

Result: Achieves up to 31% reduction in collision rate and reduces average displacement error by 5% and final displacement error by 6% on average across all datasets compared to baseline. Consistently outperforms state-of-the-art models across most test sets.

Conclusion: The proposed model effectively addresses the limitation of ignoring pedestrian physical space in trajectory prediction, demonstrating improved collision avoidance and prediction accuracy across diverse crowd density scenarios.
...
TL;DR: This paper treats soiling detection for automotive cameras as a semantic segmentation problem, compares segmentation methods vs classification approaches, identifies data leakage in Woodscape dataset, and creates a smaller but effective dataset subset.


<details>
  <summary>Details</summary>
Motivation: Soiling detection is crucial for robust advanced driver assistance systems to handle external conditions like weather and dust, making cameras more reliable.

Method: Treat soiling detection as semantic segmentation problem, compare popular segmentation methods against tile-level classification, analyze Woodscape dataset for data leakage, create new data subset.
...
TL;DR: Medical foundation models outperform general-domain ones in chest X-ray analysis, but their effectiveness depends on task type - strong for classification and salient anatomy segmentation, but poor for subtle pathology localization without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To understand how pre-training domain, paradigm, and architecture influence embedding quality for radiology tasks, enabling optimal encoder selection for specific medical imaging applications.

Method: Evaluated vision encoders from 8 medical and general-domain foundation models for chest X-ray analysis using linear probing and fine-tuning on classification (pneumothorax, cardiomegaly) and segmentation (pneumothorax, cardiac boundary) tasks.

Result: Medical pre-training provides significant advantage; medical FMs outperformed general-domain models in linear probing. Pre-trained embeddings were strong for global classification and salient anatomy segmentation, but poor for subtle pathology localization without fine-tuning. Text-image alignment was not necessary - image-only and label-supervised FMs performed well.

Conclusion: Medical pre-training is beneficial but not universally effective; architectural choices are critical, and supervised models remain competitive for complex localization tasks where pre-trained features fall short.
...
TL;DR: A novel iris image augmentation method that traverses generative model latent space to manipulate specific iris attributes while preserving identity, using gradient-based guidance for geometrical, textural, and quality features.


<details>
  <summary>Details</summary>
Motivation: Need for diverse iris datasets with realistic variations and anomalies for reliable iris recognition and presentation attack detection, addressing challenges in synthesizing same-identity iris images with controlled attributes.

Method: Traverse generative model's latent space using gradient guidance of specific iris features (sharpness, pupil size, iris size, pupil-to-iris ratio) while preserving identity. Uses GAN inversion to project real images into latent space and manipulate attributes via differentiable loss terms.

Result: The approach enables controlled manipulation of iris image properties while maintaining identity consistency, and can work with both generated and real iris images through GAN inversion.

Conclusion: Proposed method provides flexible iris image augmentation that can manipulate any differentiable attribute while preserving identity, enhancing dataset diversity for iris recognition and attack detection systems.
...
TL;DR: STORM is an annotation-free real-time 6D pose estimation system that uses vision-language understanding and self-supervised feature matching to track objects without manual segmentation masks, with automatic re-registration for handling occlusions and rapid motion.


<details>
  <summary>Details</summary>
Motivation: Existing 6D pose estimation methods require labor-intensive manual annotation of segmentation masks in the first frame, which reduces performance under occlusions or rapid movement. The goal is to eliminate manual annotation while maintaining robustness.

Method: Three-stage pipeline: 1) Contextual object descriptions guide localization, 2) Self-cross-attention identifies candidate regions, 3) Segmentation model produces precise masks for pose estimation. Includes automatic re-registration mechanism that monitors feature similarity to detect and recover from tracking failures.
...
Conclusion: STORM provides a practical annotation-free solution that significantly reduces deployment overhead for applications like flexible manufacturing and intelligent quality control, while maintaining robustness to challenging conditions.
...
TL;DR: PANDA is a patch-and-distribution-aware augmentation framework that addresses dual-level imbalances in exemplar-free continual learning by amplifying low-frequency classes through CLIP-based patch transplantation and smoothing inter-task imbalances.


<details>
  <summary>Details</summary>
Motivation: Real-world data streams exhibit dual-level imbalances (dataset-level distributions with extreme/reversed skews within tasks), creating intra-task and inter-task disparities that hinder effective learning and generalization in exemplar-free continual learning.
...
Conclusion: PANDA effectively addresses dual-level imbalances in exemplar-free continual learning, enhancing performance and reducing forgetting through patch-aware augmentation and distribution-aware balancing.
...
TL;DR: STS is a lightweight test-time adaptation framework that steers VLM representations using spectral subspaces without backpropagation, achieving faster inference and lower memory usage than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods for VLMs require backpropagation through large encoders or modify core components, which is computationally expensive and impractical for real-time applications.
...
Result: Outperforms state-of-the-art test-time adaptation methods, achieves 8x faster inference with 12x smaller memory footprint than conventional test-time prompt tuning.

Conclusion: STS provides an efficient and effective solution for test-time adaptation of VLMs that maintains performance while significantly reducing computational overhead.
...
TL;DR: Lumos3D is a generalizable pose-free framework for 3D low-light scene restoration that directly restores illumination and structure from unposed, low-light multi-view images without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D low-light restoration approaches depend on precomputed camera poses and scene-specific optimization, which restricts scalability to dynamic real-world environments.

Method: Built on a geometry-grounded backbone, Lumos3D reconstructs normal-light 3D Gaussian representation using cross-illumination distillation where teacher network transfers geometric information to student model, with dedicated Lumos loss for photometric consistency.

Result: Experiments show Lumos3D achieves high-fidelity low-light 3D scene restoration with accurate geometry and strong generalization to unseen cases, and naturally extends to over-exposure correction.

Conclusion: Lumos3D provides a versatile framework for diverse lighting restoration tasks that eliminates the need for precomputed poses and per-scene optimization while maintaining high-quality results.
...
TL;DR: A zero-shot cross-view image retrieval framework that uses pretrained vision encoders and LLMs for street-to-satellite matching without training, outperforming supervised methods and enabling automatic dataset construction.


<details>
  <summary>Details</summary>
Motivation: Existing cross-view retrieval methods require supervised training on curated datasets and rely on specialized images (panoramic/UAV), limiting real-world deployment. Need for training-free, scalable solutions.

Method: Extracts geographic cues from monocular street-view images via web search and LLM location inference, generates satellite queries through geocoding API, and retrieves matches using pretrained vision encoder (DINOv2) with PCA-based whitening feature refinement.

Result: Outperforms prior learning-based approaches on benchmark dataset under zero-shot settings despite using no ground-truth supervision or finetuning.

Conclusion: Proposed method provides effective zero-shot cross-view retrieval, enables automatic construction of semantically aligned street-to-satellite datasets, and offers scalable alternative to manual annotation.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting has achieved state-of-the-art results for novel-view synthesis but remains unexplored for human-scene animation and interaction, while existing methods rely on meshes or point clouds.

Method: Uses 3DGS as 3D representation with Gaussian-aligned motion module that synthesizes motion without explicit scene geometry, using opacity-based cues and projected Gaussian structures, plus human-scene Gaussian refinement optimization for realistic contact and navigation.

Result: Evaluated on Scannet++ and SuperSplat scenes with avatars from sparse/dense multi-view capture, demonstrating geometry-consistent free-viewpoint rendering and novel applications like editing monocular RGB videos with new animated humans.

Conclusion: The framework showcases the unique advantage of 3DGS for monocular video-based human animation, enabling decoupled rendering and motion synthesis without paired data requirements.
...
TL;DR: CertMask is a certifiably robust defense against adversarial patch attacks that uses a single round of masking with O(n) complexity, improving certified accuracy by up to 13.4% over previous methods while maintaining clean accuracy.


<details>
  <summary>Details</summary>
Motivation: Adversarial patch attacks pose serious risks to real-world vision systems, and existing defenses like PatchCleanser are computationally expensive with O(n²) complexity.

Method: Constructs a provably sufficient set of binary masks using a mathematically rigorous coverage strategy that ensures each patch location is covered at least k times, performing only single-round masking.
...
<details>
  <summary>Details</summary>
Motivation: Space weather poses risks to satellites and infrastructure, but automated classification of solar wind structures is challenging due to variable plasma properties. Need to bridge gap between remote sensing and in situ observations.

Method: Fine-tuned foundation model embeddings concatenated with spacecraft position and solar magnetic connectivity using Fourier features, creating neural field-based model for classification using Parker Solar Probe measurements.

Result: Overall classification performance is modest, likely due to coarse labeling, class imbalance, and limited transferability of pretrained model, but demonstrates feasibility of approach.

Conclusion: First proof-of-concept showing foundation model embeddings can be leveraged for in situ solar wind tasks, laying groundwork for improved space weather predictions. Code publicly available for reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: Point clouds are widely used in AR and robotics where relighting and texture editing require accurate albedo-shade separation, but conventional image-based models fail on non-grid point clouds and existing point-cloud models ignore global-light direction.
...
Conclusion: IPCD-Net effectively decomposes point clouds into albedo and shade, with verified real-world applicability for various computer vision tasks requiring illumination-invariant processing.
...
Method: Three-step Decay Resilience Strategies: SD-DRS amplifies distant semantic signals, DC-DRS modulates attention weights by distance to suppress noise, and reRD-DRS reinforces remaining remote dependencies.

Result: Extensive VQA benchmark experiments show T-DRS consistently improves performance in a training-free manner.
...
Conclusion: The proposed SAM-DAQ successfully adapts SAM2 for RGB-D VSOD by effectively integrating depth and temporal information in a unified framework, demonstrating superior performance over existing methods.
...
Result: Reduces parameter count by 4.18x and improves memory efficiency by 1.37x compared to PointSSC, while achieving state-of-the-art performance on multiple datasets.

Conclusion: The proposed lightweight network effectively addresses the limitations of existing dense architectures while maintaining competitive performance in semantic scene completion tasks.
...
TL;DR: HCC-3D is a hierarchical compression method that reduces 3D token computational overhead by 98% while maintaining state-of-the-art performance in 3D vision-language models.


<details>
  <summary>Details</summary>
Motivation: Current 3D-VLMs have high computational costs due to processing all 3D tokens in LLMs, limiting their practical applications.

Method: Proposes Hierarchical Compensatory Compression with two modules: Global Structure Compression using global queries to compress tokens, and Adaptive Detail Mining to recover lost details through complementary scoring.

Result: Achieves approximately 98% compression ratio compared to previous 3D-VLMs while achieving new state-of-the-art performance.

Conclusion: HCC-3D successfully balances computational efficiency and performance, enabling more practical deployment of 3D-VLMs.
...
TL;DR: A method combining Scale-Aware Relay Layer (SARL) and Scale-Adaptive Loss (SAL) to improve tiny object detection in aerial images by addressing feature degradation and imbalanced regression penalties.


<details>
  <summary>Details</summary>
Motivation: Modern detectors struggle with tiny object detection due to limited features being degraded during network propagation and disproportionate regression penalties favoring larger objects over smaller ones.

Method: Proposes SARL with cross-scale spatial-channel attention to enrich features and strengthen cross-layer sharing, and SAL that reshapes IoU-based losses to dynamically assign lower weights to larger objects, focusing training on tiny objects.
...
Conclusion: The proposed SARL and SAL effectively address tiny object detection challenges in aerial images and are compatible with top-performing detection frameworks.
...
TL;DR: Swin-BART encoder-decoder with regional attention module achieves state-of-the-art medical image captioning, improving ROUGE and BERTScore while providing interpretable regional attributions.
...
Result: Achieves ROUGE 0.603, BERTScore 0.807, outperforming baselines (ResNet-CNN: ROUGE 0.356, BLIP2-OPT: ROUGE 0.255). Provides competitive BLEU, CIDEr, and METEOR scores with transparent regional attributions via heatmaps.
...
Method: Introduces controllable Gaussian noise injection and multi-scale Gaussian blurring for initial feature perturbations, followed by temporal modeling and liquid parameter adjustment mechanism to generate adaptive modulation parameters for smooth continuous domain adaptation.

Result: Significant performance improvements on the Diverse Weather dataset and Real-to-Art benchmark, demonstrating superior generalization and robustness to unseen domain shifts.
...
Method: Developed DocWeaver, a multi-agent pipeline using Large Language Models to automatically generate MosaicDoc - a bilingual benchmark with 72K images from newspapers/magazines featuring diverse complex layouts and comprehensive annotations.

Result: Created MosaicDoc with 72K images, 600K+ QA pairs, diverse layouts from 196 publishers, supporting OCR, VQA, reading order, and localization tasks. Evaluation shows current models struggle with real-world document complexity.

Conclusion: MosaicDoc serves as a definitive VRDU benchmark, revealing current model limitations and providing clear direction for future research in handling complex document understanding tasks.
...
TL;DR: SLDC introduces drift compensation methods to address distribution drift in sequential fine-tuning for class-incremental learning, achieving performance comparable to joint training.


<details>
  <summary>Details</summary>
Motivation: Sequential fine-tuning of vision transformers suffers from distribution drift caused by sequential optimization, leading to performance degradation over time due to mismatch between learned class distributions and updater model.
...
Conclusion: SLDC effectively mitigates distribution drift in sequential fine-tuning, making it a viable alternative to joint training for class-incremental learning tasks.
...
TL;DR: Proposes a debiased dual-invariant defense framework for person re-identification that addresses model bias and composite generalization requirements through data balancing and bi-adversarial self-meta defense.


<details>
  <summary>Details</summary>
Motivation: Person ReID models are vulnerable to adversarial attacks, and existing defenses fail to address the unique challenges of adversarially robust ReID, particularly model bias and composite generalization requirements.

Method: Two-phase framework: 1) Data balancing phase using diffusion-model-based data resampling for fairness and diversity, 2) Bi-adversarial self-meta defense phase with metric adversarial training and farthest negative extension softening, plus adversarially-enhanced self-meta mechanism for dual-generalization.

Result: Experiments show the method significantly outperforms existing state-of-the-art defenses.

Conclusion: The proposed framework effectively addresses the key challenges in adversarial defense for person ReID and achieves superior performance compared to current defense methods.
...
TL;DR: AdaptViG is an efficient Vision Graph Neural Network that uses Adaptive Graph Convolution with Exponential Decay Gating for selective long-range connections and hybrid strategy combining gating in early stages with global attention in final stage.


<details>
  <summary>Details</summary>
Motivation: Vision GNNs face computational challenges from graph construction phase, hindering efficiency. Need for more efficient vision architectures that maintain performance.

Method: Proposes Adaptive Graph Convolution with static axial scaffold and dynamic Exponential Decay Gating for selective long-range connections. Uses hybrid strategy: gating in early stages, Global Attention in final stage.

Result: Achieves SOTA trade-off between accuracy and efficiency. AdaptViG-M: 82.6% top-1 accuracy (outperforms ViG-B by 0.3% with 80% fewer params, 84% fewer GMACs). Downstream: 45.8 mIoU, 44.8 APbox, 41.1 APmask (surpasses EfficientFormer-L7 with 78% fewer params).

Conclusion: AdaptViG provides efficient and powerful hybrid Vision GNN with novel graph construction mechanism, achieving superior accuracy-efficiency trade-off compared to existing methods.
...
TL;DR: TSPE-GS improves 3D Gaussian Splatting for semi-transparent surfaces by modeling multi-modal depth distributions instead of single-depth assumptions, enabling better reconstruction of transparent objects while maintaining performance on opaque scenes.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting struggles with semi-transparent surfaces because it assumes a single depth per pixel, which fails when multiple surfaces are visible through transparency.

Method: Proposes TSPE-GS which uniformly samples transmittance to model pixel-wise multi-modal distributions of opacity and depth, progressively fuses truncated signed distance functions to reconstruct external and internal surfaces separately.

Result: Significantly improves semi-transparent geometry reconstruction on public and self-collected datasets while maintaining performance on opaque scenes.

Conclusion: TSPE-GS provides a unified framework that generalizes to other Gaussian-based reconstruction pipelines without extra training overhead, effectively resolving cross-surface depth ambiguity in transparent objects.
...
TL;DR: A novel adaptive fusion framework for NR-IQA that combines CLIP's cosine similarity with magnitude-aware quality cues, outperforming existing methods without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Standard CLIP-based IQA methods only use semantic similarity (cosine similarity between image and text prompts), ignoring the important quality cue in CLIP image feature magnitudes that strongly correlate with perceptual quality.

Method: Extract absolute CLIP image features, apply Box-Cox transformation to normalize feature distribution and reduce semantic sensitivity, then use confidence-guided fusion to adaptively combine the magnitude cue with cosine similarity.
...
Conclusion: The magnitude of CLIP image features provides a valuable quality cue that, when properly normalized and fused with semantic similarity, significantly enhances NR-IQA performance without requiring task-specific training.
...
Method: Leverage VLMs to generate pseudo-labels, then use per-object co-teaching where two YOLO models collaboratively filter noisy bounding boxes based on peer loss values during training.
...
Method: EquS imposes equivariant information through dual sampling trajectories and uses Timestep-Aware Schedule (TAS) to prioritize deterministic steps for better certainty and efficiency.
...
Conclusion: EquS and EquS+ effectively address limitations of current DMIR methods by better utilizing diffusion priors through dual trajectories and optimized scheduling.
...
TL;DR: DiVE preserves geometric structure in vision-language models during fine-tuning by constraining difference vectors between pre-trained and fine-tuned embeddings to be equal across data samples, maintaining strong OOD and zero-shot performance.
...
Method: Proposes Difference Vector Equalization (DiVE) with two losses: Average Vector Loss (AVL) to preserve global geometric structure by constraining difference vectors to their weighted average, and Pairwise Vector Loss (PVL) to preserve local geometric structure through consistent multimodal alignment.
...
Conclusion: DiVE successfully addresses the geometric structure distortion problem in vision-language model fine-tuning, maintaining generalization capabilities while improving performance on target tasks.
...
Method: Proposes STELLAR with language-adaptive glyph encoder, multi-stage training (pre-training on synthetic data then fine-tuning on real images), and constructs STIPLAR dataset. Also introduces Text Appearance Similarity (TAS) metric.
...
Conclusion: STELLAR effectively addresses key challenges in multilingual scene text editing through its novel architecture, training strategy, dataset, and evaluation metric.
...
TL;DR: MOBA introduces a physically realizable backdoor attack for LiDAR-based 3D object detection by modeling material properties, using TiO2 triggers and realistic simulation, achieving 93.5% attack success rate.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks lack physical realizability due to digital-to-physical domain gaps and unoptimized physical triggers that are ineffective or easily detectable.

Method: Systematic material selection (TiO2 for high reflectivity) and novel simulation pipeline with angle-independent BRDF approximation and distance-aware scaling for realistic LiDAR intensity modeling.

Result: Achieves 93.50% attack success rate, outperforming prior methods by over 41% on LiDAR-based and Camera-LiDAR fusion models.

Conclusion: Reveals new physically realizable threats and emphasizes the need for defenses considering material-level properties in real-world environments.
...
TL;DR: DBGroup is a weakly supervised 3D instance segmentation framework that uses scene-level annotations instead of point-level or bounding box supervision, achieving competitive performance with reduced labeling effort.
...
Conclusion: DBGroup provides an efficient and scalable solution for 3D instance segmentation using scene-level annotations, reducing labeling costs while maintaining competitive performance.
...
Result: Extensive experiments show LampQ provides state-of-the-art performance in quantizing ViTs pre-trained on various tasks including image classification, object detection, and zero-shot quantization.

Conclusion: LampQ successfully overcomes limitations of previous mixed precision quantization methods for Vision Transformers, achieving superior performance through its layer-wise approach and optimal bit allocation strategy.
...
TL;DR: MIRNet integrates self-supervised pre-training with graph-based reasoning for medical image analysis, achieving SOTA on tongue diagnosis using MAE, GAT, constraint optimization, and ensemble methods.
...
Method: Self-supervised MAE pre-training, graph attention networks for label correlations, constraint-aware optimization with KL divergence, asymmetric loss for imbalance, and boosting ensembles.

Result: Achieves state-of-the-art performance on TongueAtlas-4K benchmark (4,000 images, 22 diagnostic labels) and demonstrates generalizability to broader medical imaging tasks.

Conclusion: MIRNet provides an effective framework for medical image analysis that addresses key challenges and generalizes beyond tongue diagnosis to broader diagnostic imaging applications.
...
TL;DR: Fine-grained 3D embodied reasoning task requiring agents to predict spatial location, motion type, and motion axis for affordance elements based on task instructions, solved using AffordBot framework with MLLMs and chain-of-thought reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing approaches lack coherent instruction-driven grounding and reasoning for fine-grained affordance understanding in physical environments, operating at object level or disjointedly handling affordance reasoning.

Method: AffordBot integrates Multimodal Large Language Models with chain-of-thought reasoning, renders surround-view images from 3D scenes, projects 3D element candidates into views, and uses active perception to select viewpoints before step-by-step reasoning for localization and motion inference.
...
Conclusion: Proposed framework effectively bridges 3D input with 2D-compatible MLLMs for fine-grained embodied reasoning, enabling comprehensive affordance understanding in physical environments.
...
TL;DR: Anomagic is a zero-shot anomaly generation method that creates semantically coherent anomalies without exemplars, using crossmodal prompt encoding and contrastive refinement to improve anomaly detection accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating realistic anomalies without requiring exemplar anomaly data, enabling better training of anomaly detection models.

Method: Uses crossmodal prompt encoding to unify visual and textual cues, an inpainting-based generation pipeline, and contrastive refinement to align synthesized anomalies with their masks. Trained on AnomVerse dataset of 12,987 anomaly-mask-caption triplets.
...
TL;DR: DGFusion is a dual-guided multi-modal 3D object detection method that addresses hard instance detection by combining point-guide-image and image-guide-point paradigms through difficulty-aware instance matching.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal 3D object detection methods follow single-guided paradigms and fail to handle hard instances (distant, small, or occluded objects) effectively, which compromises autonomous driving safety.
...
Result: Outperforms baseline methods with +1.0% mAP, +0.8% NDS, and +1.3% average recall on nuScenes dataset. Shows consistent robustness gains for hard instance detection across various scenarios.

Conclusion: The dual-guided paradigm effectively addresses limitations of single-guided approaches and significantly improves detection of hard instances in autonomous driving systems.
...
TL;DR: Proposes a 3D VAE framework using unsigned distance fields (UDFs) with local-to-global architecture for high-fidelity 3D content generation, achieving state-of-the-art performance at ultra-high resolutions up to 2048^3.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with complex topologies like open surfaces and intricate internal structures. SDF-based approaches require costly watertight preprocessing and handle non-manifold geometries poorly, while point-cloud representations suffer from sampling artifacts and surface discontinuities.

Method: Uses unsigned distance fields (UDFs) with a local-to-global architecture that partitions UDF into uniform subvolumes (UBlocks). Combines 3D convolutions for local detail with sparse transformers for global coherence, plus Pad-Average strategy for smooth boundary transitions.

Result: Achieves state-of-the-art performance in reconstruction accuracy and generative quality, with superior surface smoothness and geometric flexibility. Enables seamless scaling to ultra-high resolutions up to 2048^3.

Conclusion: The proposed UDF-based 3D VAE with local-to-global architecture effectively overcomes limitations of existing methods, providing a robust framework for generating high-fidelity 3D content with complex topologies at unprecedented resolutions.
...
TL;DR: FreDFT is a frequency domain fusion transformer for visible-infrared object detection that addresses information imbalance between modalities through frequency domain attention, cross-modal global modeling, and local feature enhancement.


<details>
  <summary>Details</summary>
Motivation: Visible-infrared object detection faces information imbalance between modalities in complex scenarios, leading to inadequate cross-modal fusion and degraded performance. Existing methods ignore the advantages of frequency domain transformers for mining complementary information.

Method: Proposes FreDFT with multimodal frequency domain attention (MFDA) to mine complementary information, frequency domain feed-forward layer (FDFFL) with mixed-scale fusion, cross-modal global modeling module (CGMM) for pixel-wise inter-modal interaction, and local feature enhancement module (LFEM) using various convolutions and channel shuffle.
...
Conclusion: The proposed frequency domain fusion transformer effectively addresses multimodal information imbalance and enhances detection performance in visible-infrared object detection tasks.
...
TL;DR: MuSc-V2 is a zero-shot anomaly classification and segmentation framework that leverages the property that normal patches find many similar patches across 2D/3D modalities while anomalies remain isolated, achieving significant performance gains over previous methods.
...
Method: Proposes Mutual Scoring framework with Iterative Point Grouping for improved 3D representation, Similarity Neighborhood Aggregation with Multi-Degrees for multi-scale patch features, Mutual Scoring Mechanism for intra-modal scoring, Cross-modal Anomaly Enhancement for fusing 2D/3D scores, and Re-scoring with Constrained Neighborhood to suppress false positives.

Result: Achieves +23.7% AP gain on MVTec 3D-AD dataset and +19.3% boost on Eyecandies dataset, surpassing previous zero-shot benchmarks and outperforming most few-shot methods.

Conclusion: MuSc-V2 provides a flexible framework that works on both full datasets and smaller subsets with robust performance, ensuring adaptability across diverse product lines while significantly advancing zero-shot anomaly detection capabilities.
...
TL;DR: The paper addresses poor image screening performance in MLLMs by creating a large dataset (128k samples, 640k images) and proposing HCM-GRPO method with hard cases mining and dynamic proportional accuracy reward to improve image aesthetic reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have unsatisfactory image screening performance due to lack of data and weak image aesthetic reasoning ability, with even state-of-the-art models performing like random guessing.

Method: Created comprehensive dataset with 4 evaluation aspects; used multiple annotation approaches; introduced HCM-GRPO method with Hard Cases Mining strategy and Dynamic Proportional Accuracy reward in GRPO framework.

Result: HCM-GRPO enables smaller models to surpass both large-scale open-source and leading closed-source models in image aesthetic reasoning, while GPT4o and Qwen-VL-Max perform like random guessing.

Conclusion: The proposed complete solution with dataset and HCM-GRPO method significantly improves image aesthetic reasoning in MLLMs, demonstrating superior performance over state-of-the-art models with smaller model size.
...
TL;DR: The paper introduces AV-ConfuseBench to test MLLMs' ability to detect audio-absent objects in videos, and proposes RL-CoMM, a reinforcement learning approach that improves audio-visual reasoning accuracy by 10-30%.


<details>
  <summary>Details</summary>
Motivation: To address MLLMs' inability to detect when visually present objects lack corresponding audio, due to visually dominated reasoning that ignores audio cues.

Method: RL-CoMM uses a two-stage approach: 1) Step-wise Reasoning Reward with an external Large Audio Language Model to generate audio-only reasoning, and 2) Answer-centered Confidence Optimization to reduce uncertainty in heterogeneous reasoning.
...
Conclusion: The proposed RL-CoMM framework effectively addresses MLLMs' audio-visual confusion problem and significantly enhances their reasoning capabilities in detecting audio-absent objects.
...
TL;DR: The paper introduces CPREval-6k dataset and GaussMedAct framework for fine-grained medical action evaluation, achieving 92.1% accuracy with real-time inference.


<details>
  <summary>Details</summary>
Motivation: Address challenges in medical vision action evaluation including lack of comprehensive datasets, precision requirements, and insufficient spatiotemporal modeling of rapid actions.

Method: Proposes GaussMedAct framework using multivariate Gaussian encoding to project joint motions into adaptive 3D Gaussian tokens with anisotropic covariance modeling, plus hybrid spatial encoding with Cartesian and Vector dual-stream strategy.
...
<details>
  <summary>Details</summary>
Motivation: Current models in hyperspectral image classification often mistake high predictive confidence for correctness, leading to confirmation bias and poor generalization, especially under sparse annotations or class imbalance.

Method: CABIN uses a closed-loop process: 1) perceptual awareness through epistemic uncertainty estimation, 2) uncertainty-guided dual sampling strategy for exploration and stable pseudo-labels, and 3) fine-grained dynamic assignment strategy to categorize and correct noisy pseudo-labels with tailored losses.

Result: Experimental results demonstrate that integrating CABIN with state-of-the-art methods improves labeling efficiency and classification performance across various hyperspectral image classification tasks.

Conclusion: CABIN effectively mitigates overconfidence issues in hyperspectral image classification by incorporating cognitive-aware learning principles, leading to more robust and generalizable models under limited supervision.
...
TL;DR: VLF-MSC is a unified multimodal semantic communication system that transmits a single vision-language feature representation to generate both images and text at the receiver, improving spectral efficiency and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing semantic communication techniques process each modality separately, requiring multiple streams or retransmissions, which reduces spectral efficiency. There is a need for a unified approach that can handle both vision and language modalities efficiently.

Method: Uses a pre-trained vision-language model to encode source images into vision-language semantic features (VLF), which are transmitted over wireless channels. At the receiver, both a decoder-based language model and diffusion-based image generator are conditioned on the VLF to produce text descriptions and semantically aligned images.
...
Conclusion: VLF-MSC provides an efficient unified framework for multimodal semantic communication that eliminates modality-specific streams, improves spectral efficiency, and maintains semantic fidelity while being robust to channel noise.
...
TL;DR: GlobalDiff is a diffusion-based framework that generates co-speech motion using global joint rotations instead of local ones, avoiding hierarchical error accumulation through multi-level structural constraints.


<details>
  <summary>Details</summary>
Motivation: Existing methods using local joint rotations suffer from cumulative errors leading to unstable motions at end-effectors due to hierarchical dependencies.

Method: Proposes GlobalDiff framework operating in global joint rotation space with multi-level constraints: joint structure constraint (virtual anchor points), skeleton structure constraint (angular consistency), and temporal structure constraint (multi-scale variational encoder).
...
Conclusion: Operating directly in global rotation space with structural constraints effectively addresses hierarchical error accumulation in co-speech motion generation.
...
TL;DR: GridPrune introduces a two-stage visual token pruning method for MLLMs that first allocates token budgets across spatial zones and then performs local selection, achieving 96.98% performance retention with only 11.1% of tokens.


<details>
  <summary>Details</summary>
Motivation: Existing visual token pruning methods focus only on 'what to select' using attention scores, leading to inefficient spatial allocation, positional bias, and retention of irrelevant tokens. Inspired by human visual attention, the paper addresses the missing 'where to look' component.

Method: GridPrune replaces global Top-K with a 'guide-globally, select-locally' zonal selection system: 1) text-conditional guidance dynamically allocates token budgets across spatial zones, 2) local selection within each budgeted zone.

Result: On LLaVA-NeXT-7B, GridPrune retains 96.98% of full performance while using only 11.1% of tokens, outperforming the best baseline by 2.34% at the same pruning rate. Superior performance across various MLLM architectures.

Conclusion: The two-stage pruning approach inspired by human visual attention effectively addresses spatial allocation inefficiencies in MLLMs, demonstrating significant efficiency improvements while maintaining performance.
...
TL;DR: SUGAR is a novel framework that combines LLMs with skeleton data for action recognition by using visual-motion knowledge from video models to supervise skeleton learning, enabling LLMs to understand and classify actions without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To leverage LLMs' rich knowledge and transferability for skeleton-based action recognition, addressing how LLMs can understand skeleton data and distinguish between different actions.

Method: Uses off-the-shelf video models to generate visual-motion knowledge, supervises skeleton learning with this prior knowledge to create discrete representations, and employs pre-trained LLMs to understand these representations for action classification and description. Includes Temporal Query Projection (TQP) for modeling long skeleton sequences.

Result: Achieves strong performance on multiple skeleton-based action classification benchmarks and demonstrates superior versatility in zero-shot scenarios compared to linear-based methods.

Conclusion: SUGAR effectively bridges the gap between LLMs and skeleton data, enabling robust action recognition without requiring LLM fine-tuning, with particular strength in zero-shot settings.
...
TL;DR: MTAttack is the first multi-target backdoor attack framework for Large Visual Language Models that can embed multiple independent triggers with different attack targets in a single training pass, overcoming feature interference challenges through novel optimization constraints.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks only focus on single-target attacks, while real-world threats require multi-target capabilities. LVLMs are vulnerable to backdoor attacks, and multi-target attacks pose greater security risks that need investigation.

Method: Proposed MTAttack with two optimization constraints: Proxy Space Partitioning and Trigger Prototype Anchoring. Jointly optimizes multiple triggers in latent space, ensuring each trigger independently maps clean images to unique proxy classes while maintaining separability.

Result: Achieves high success rate for multi-target attacks, substantially outperforming existing methods. Shows strong generalizability across datasets and robustness against backdoor defense strategies.

Conclusion: LVLMs are highly vulnerable to multi-target backdoor attacks, highlighting urgent need for mitigation strategies. MTAttack demonstrates the feasibility and effectiveness of such attacks in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Stereo depth estimation faces challenges from dynamic domain shifts, sparse supervision, and costly ground-truth labels. Existing TTA methods are limited by static domain assumptions and input-invariant strategies.

Method: Proposes RobIA with two components: (1) Attend-and-Excite Mixture-of-Experts (AttEx-MoE) that dynamically routes inputs to frozen experts using lightweight self-attention, and (2) Robust AdaptBN Teacher that provides dense pseudo-supervision to complement sparse labels.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on implicit modeling with fragmented video features, failing to capture temporal coherence across event sequences and comprehensive visual semantics.
...
Conclusion: CACMI effectively addresses limitations of implicit modeling by explicitly capturing temporal coherence and semantic context, achieving superior dense video captioning performance.
...
TL;DR: Text-to-image models fail dramatically at logical composition (negation, counting, spatial relations), showing severe interference when primitives are combined, due to data gaps, architectural limitations, and flawed evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: To investigate why current text-to-image models fundamentally fail at logical composition despite good performance on individual primitives, exposing critical architectural flaws.

Method: Analysis across three core logical primitives (negation, counting, spatial relations), examining training data distribution, attention architectures, and evaluation metrics through recent benchmarks and methods.

Result: Models show dramatic performance collapse when primitives are combined, with severe interference effects. Training data lacks explicit negations, continuous attention architectures are unsuitable for discrete logic, and metrics prioritize visual plausibility over constraint satisfaction.
...
TL;DR: The paper proposes a split-layer MLP architecture that divides layers into parallel branches and combines them via Hadamard product, significantly enhancing implicit neural representation capacity without prohibitive computational costs.


<details>
  <summary>Details</summary>
Motivation: Conventional MLP architectures in implicit neural representations have limited representational capacity due to low-dimensional feature spaces, and widening MLPs leads to quadratic growth in computational and memory costs.

Method: Proposed split-layer MLP construction that divides each layer into multiple parallel branches and integrates their outputs using Hadamard product, effectively creating a high-degree polynomial space.
...
Conclusion: The split-layer approach effectively enhances implicit neural representation capacity by expanding feature space dimensionality while avoiding prohibitive computational overhead, demonstrating superior performance across multiple applications.
...
TL;DR: Proposes a dual-mechanism framework for fair deepfake detection that maintains accuracy while improving fairness across demographic groups through structural decoupling and distribution alignment.


<details>
  <summary>Details</summary>
Motivation: Address biases in deepfake detection models that lead to systemic misjudgments across demographic groups (gender, race), which exacerbate digital divide and social inequities, while current fairness methods sacrifice detection accuracy.

Method: Dual-mechanism collaborative optimization framework combining structural fairness decoupling (separating demographic-sensitive channels at architectural level) and global distribution alignment (reducing distance between overall sample distribution and demographic group distributions at feature level).
...
Conclusion: The proposed framework successfully addresses the fairness-accuracy trade-off in deepfake detection by integrating architectural and feature-level optimizations to achieve equitable performance across demographic groups.
...
Method: Two parallel modules: Text-Guided Token Enhancement (TGTE) uses diffusion-generated images as intermediate semantic bridges, and Generative Intermediate Fusion (GIF) combines cross-attention between generated images, original images, and text features with triplet alignment loss.
...
Conclusion: GEA effectively addresses modality gap issues in text-to-image person retrieval through generative enhancement, achieving improved cross-modal alignment and retrieval performance.
...
TL;DR: InterIR: An interpretability-driven deep unfolding network for multi-degradation image restoration using improved second-order semi-smooth Newton algorithm and explainable convolution modules inspired by human brain processing.


<details>
  <summary>Details</summary>
Motivation: Real-world images often contain multiple degradations simultaneously (rain, noise, haze), but most existing methods only target single degradation types. Current approaches also suffer from limited interpretability when stacking modules for performance improvement.

Method: Deep unfolding network mapping iterative optimization algorithms into learnable structure, using improved second-order semi-smooth Newton algorithm for physical interpretability, and explainable convolution modules inspired by human brain's flexible information processing and image characteristics.
...
TL;DR: CephRes-MHNet is a multi-head residual CNN for cephalometric landmark detection that achieves state-of-the-art accuracy with 1.23mm MRE and 85.5% SDR @ 2.0mm, using only 25% of parameters compared to baseline models.


<details>
  <summary>Details</summary>
Motivation: Manual cephalometric landmark annotation is time-consuming and error-prone, while automated methods struggle with low contrast and anatomical complexity in 2D lateral skull X-rays.
...
Result: Achieved mean radial error (MRE) of 1.23 mm and success detection rate (SDR) @ 2.0 mm of 85.5%, outperforming all evaluated models including AFPF-Net (MRE=1.25mm, SDR=84.1%) while using <25% of parameters.
...
TL;DR: Evaluation of NASA/IBM's Prithvi-EO-2.0 model for shoreline delineation using satellite images, achieving high performance with minimal training data.


<details>
  <summary>Details</summary>
Motivation: To assess the transfer learning capability of geospatial foundation models for coastal monitoring in data-poor regions, particularly for small sandy island shoreline delineation.

Method: Curated and labeled 225 multispectral images of Maldivian islands, fine-tuned Prithvi-EO-2.0 models (300M and 600M parameters) on training subsets ranging from 5 to 181 images.
...
Conclusion: Prithvi-EO-2.0 shows strong potential for supporting coastal monitoring in data-poor regions through effective transfer learning with minimal training data.
...
TL;DR: VISTA is a recursive goal-conditioned transformer for multi-agent trajectory forecasting that combines long-term intent modeling with fine-grained social interactions, achieving state-of-the-art accuracy and substantially fewer collisions.
...
Result: On MADRAS benchmark, reduces collision rate from 2.14% to 0.03%; on SDD, achieves zero collisions while improving ADE, FDE, and minFDE metrics. Achieves state-of-the-art accuracy.
...
<details>
  <summary>Details</summary>
Motivation: To address the computational overhead and real-time limitations of diffusion models in 3D LiDAR scene completion for autonomous vehicles.

Method: Uses Noise-to-Coarse Module for single-pass denoising, Refine Module for precise refinement, and Distance-aware Selected Repeat strategy for uniform point cloud distribution.
...
Conclusion: LiNeXt demonstrates superior efficiency and effectiveness for real-time scene completion in autonomous driving applications.
...
TL;DR: HeatV2X is a scalable V2X collaborative perception framework that addresses multi-modal heterogeneity and scalability challenges through local and global fine-tuning with specialized adapters, achieving superior performance with minimal training cost.
...
Method: Train a high-performance agent using heterogeneous graph attention, then apply Local Heterogeneous Fine-Tuning with Hetero-Aware Adapters for modality-specific alignment, and Global Collaborative Fine-Tuning with Multi-Cognitive Adapter for cross-agent collaboration.
...
Conclusion: HeatV2X provides an effective solution for scalable V2X collaborative perception that handles agent heterogeneity while minimizing training costs, with promising experimental results.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal detection methods struggle with generalization across unseen manipulations, require pretraining on real samples, and may fail against manipulations that preserve audio-visual alignment by overlooking intra-modal artifacts.

Method: Proposes a single-stage training framework incorporating next-frame prediction for both uni-modal and cross-modal features, with a window-level attention mechanism to capture discrepancies between predicted and actual frames.
...
TL;DR: TubeRMC is a framework for spatio-temporal video grounding that generates text-conditioned candidate tubes and refines them through tube-conditioned reconstruction with mutual constraints between spatial and temporal proposals.


<details>
  <summary>Details</summary>
Motivation: Existing weakly-supervised STVG methods use simple late-fusion approaches that generate tubes independent of text descriptions, leading to failed target identification and inconsistent tracking.

Method: Proposes Tube-conditioned Reconstruction with Mutual Constraints (TubeRMC) that uses pre-trained visual grounding models to generate text-conditioned candidate tubes, then refines them through three reconstruction strategies (temporal, spatial, spatio-temporal) with mutual constraints between proposals.

Result: Outperforms existing methods on VidSTG and HCSTVG benchmarks, and visualization shows effective mitigation of target identification errors and inconsistent tracking.
...
TL;DR: Proposes JudgeMind, a novel AQA method that simulates referee judgment by segmenting action videos into stages, using stage-aware feature enhancement and knowledge-based decoding for improved accuracy and reliability.
...
Method: Segments action videos into stages, uses stage-aware feature enhancement and fusion module, and incorporates knowledge-based grade-aware decoder with deduction items as prior knowledge.

Result: Achieves state-of-the-art performance on the new aerial skiing AQA benchmark dataset with fine-grained annotations.

Conclusion: JudgeMind significantly enhances AQA performance and reliability by mimicking professional referee scoring processes through stage segmentation and knowledge incorporation.
...
TL;DR: Facial-R1 is a three-stage alignment framework for Facial Emotion Analysis that addresses hallucinated reasoning and misalignment issues in VLMs through instruction fine-tuning, reinforcement training, and iterative data synthesis.
...
Method: Three-stage framework: 1) Instruction fine-tuning for basic emotional reasoning, 2) Reinforcement training using emotion and AU labels as reward signals to align reasoning with predicted emotion, 3) Iterative data synthesis pipeline that expands training data for scalable self-improvement.

Result: Facial-R1 achieves state-of-the-art performance across eight standard benchmarks, with strong generalization and robust interpretability. Introduces FEA-20K dataset with 17,737 training and 1,688 test samples.

Conclusion: The proposed three-stage alignment framework effectively addresses hallucinated reasoning and misalignment challenges in facial emotion analysis, demonstrating superior performance and interpretability while enabling scalable self-improvement through iterative data synthesis.
...
TL;DR: H3Former is a token-to-region framework for fine-grained visual classification that uses high-order semantic relations and hypergraph modeling to aggregate local features, with hyperbolic hierarchical contrastive learning for better class separation.


<details>
  <summary>Details</summary>
Motivation: Existing FGVC methods fail to comprehensively capture discriminative cues while introducing category-agnostic redundancy, due to limitations in feature-selection mechanisms and region-proposal strategies.

Method: Proposes Semantic-Aware Aggregation Module (SAAM) using multi-scale contextual cues to construct weighted hypergraphs among tokens, and Hyperbolic Hierarchical Contrastive Loss (HHCL) for hierarchical semantic constraints in non-Euclidean space.

Result: Comprehensive experiments on four standard FGVC benchmarks validate the superiority of the H3Former framework.

Conclusion: H3Former effectively addresses limitations of existing FGVC methods by leveraging high-order semantic relations and structured region-level modeling with hyperbolic hierarchical contrastive learning.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models struggle with complex visual reasoning where early errors cascade through multi-step dependencies, and existing methods either require costly step-level annotations (SFT) or provide only sparse outcome-level feedback (RLVR).
...
Conclusion: PROPA establishes strong reasoning and generalization capabilities for visual reasoning tasks by optimizing reasoning at intermediate steps without human annotations through process-level reward generation and alignment.
...
TL;DR: Evaluating smaller VLMs (500M and 2.2B parameters) for blind/low-vision accessibility, introducing BLV-specific evaluation frameworks and testing mobile deployment with different precision levels.


<details>
  <summary>Details</summary>
Motivation: Large VLMs have high resource demands that hinder practical use for BLV users who need detailed, context-aware video descriptions for accessibility.

Method: Tested SmolVLM2 variants on AVCaps and Charades datasets using novel BLV evaluation frameworks, evaluated prompt strategies, and deployed on smartphones with FP32/INT8 precision.

Result: Not specified in abstract - requires full paper for experimental results comparing model sizes and mobile performance.

Conclusion: The study provides frameworks for evaluating VLMs specifically for BLV accessibility and demonstrates the feasibility of deploying smaller models on mobile devices.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods to mitigate object hallucination in LVLMs incur substantial computational overhead through extra forward passes, limiting their practicality for real-world, latency-sensitive deployments.

Method: Uses Contextual Activation Residual Direction (CARD) vectors extracted from self-attention layer residuals during standard forward passes, combined with a Bayesian-inspired adaptive gate for token-wise corrective signal injection.

Result: Achieves performance comparable to state-of-the-art methods on benchmarks like POPE and CHAIR while introducing negligible computational latency.

Conclusion: RUDDER provides a pragmatic and effective approach for improving LVLM reliability without significant efficiency compromise.
...
<details>
  <summary>Details</summary>
Motivation: Satellite-based slum mapping can estimate global urban poverty, but morphological heterogeneity of informal settlements prevents models from generalizing to unseen regions, requiring a solution that works without labeled data from target areas.

Method: Two-phase test-time adaptation framework using Mixture-of-Experts architecture: trains on million-scale dataset from 12 cities across 4 continents, captures region-specific features while learning universal patterns, and uses prediction consistency during adaptation to filter unreliable pseudo-labels.

Result: GRAM outperforms state-of-the-art baselines in low-resource settings like African cities, demonstrating robust generalization to previously unseen regions without requiring labeled target data.

Conclusion: GRAM provides a scalable and label-efficient solution for global slum mapping and data-driven urban planning by enabling robust segmentation across diverse regions through test-time adaptation.
...
TL;DR: LLaViT transforms LLMs into vision transformers by enabling them to function as vision encoders through specialized QKV projections, bidirectional attention, and multi-scale visual representations, outperforming LLaVA and larger models.


<details>
  <summary>Details</summary>
Motivation: Address the inherent modality mismatch in LLaVA architecture where visual features struggle to integrate effectively with text due to fundamental differences between vision and language modalities.

Method: Three key modifications: (1) learning separate QKV projections for vision modality, (2) enabling bidirectional attention on visual tokens, (3) incorporating both global and local visual representations.
...
Conclusion: LLaViT establishes a more effective approach to vision-language modeling by enabling LLMs to simultaneously function as powerful vision encoders.
...
TL;DR: This paper proposes new evaluation metrics for pedestrian detection that categorize errors into 8 types using image segmentation, enabling more fine-grained and safety-critical performance comparison of models.


<details>
  <summary>Details</summary>
Motivation: Current pedestrian detection benchmarks have weaknesses in realistic performance evaluation, particularly lacking fine-grained error analysis for safety-critical automated driving systems.

Method: Proposed 8 different error categories for pedestrian detection and new metrics based on image segmentation to automatically distinguish between error types during evaluation.

Result: Achieved state-of-the-art performance on CityPersons-reasonable dataset without extra training data using a simple architecture, and demonstrated more robust model comparison.

Conclusion: The proposed metrics provide a more fine-grained and safety-critical way to compare pedestrian detection models, addressing limitations of current evaluation benchmarks.
...
TL;DR: CLIP4VI-ReID: A CLIP-driven network for visible-infrared person re-identification using text semantics as a bridge to align visible and infrared modalities through text semantic generation, infrared feature embedding, and high-level semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Address the huge gap in physical characteristics between natural visible images and infrared images in VI-ReID tasks, enabling effective cross-modal alignment and shared representation learning.

Method: Three-stage approach: 1) Text Semantic Generation (TSG) for visible images, 2) Infrared Feature Embedding (IFE) to rectify infrared features using text semantics, 3) High-level Semantic Alignment (HSA) to refine id-related semantic alignment.

Result: Achieves superior performance compared to state-of-the-art methods on widely used VI-ReID datasets.

Conclusion: The proposed CLIP4VI-ReID effectively bridges the modality gap between visible and infrared images using text semantics as an intermediate representation, enabling accurate cross-modal alignment and discriminative shared representations.
...
TL;DR: A novel computational framework that integrates depth-of-field supervision and multi-view consistency supervision to improve 3D Gaussian Splatting for scenes with extreme depth variations.
...
Result: Achieves superior depth fidelity with 0.8 dB PSNR improvement over state-of-the-art method on Waymo Open Dataset.
...
Method: Two-level approach: 1) Coarse-grained: self-guided normality modeling branch reconstructs video features using learned normal prototypes to separate normal patterns from anomalies. 2) Fine-grained: decoupled contrastive semantic alignment temporally decomposes videos into event-centric and background-centric components using anomaly scores, then applies visual-language contrastive learning.
...
Conclusion: The proposed disentangled semantic alignment approach effectively enhances the distinguishability between abnormal and normal features at both coarse and fine-grained levels, improving video anomaly detection performance.
...
TL;DR: A novel SDG object detection framework using vMF distribution and Fourier transformation with CLIP guidance to improve cross-domain generalization.
...
Method: Integrates von Mises-Fisher distribution for modeling directional features and Fourier-based augmentation for amplitude/phase perturbation to simulate domain shifts.
...
Conclusion: Combining vMF distribution modeling with Fourier transformation in CLIP-guided pipeline effectively enhances feature robustness and domain generalization for object detection.
...
TL;DR: DermAI is a smartphone app for real-time skin lesion capture and classification that addresses dataset bias and validation limitations through on-device quality checks and local model adaptation.


<details>
  <summary>Details</summary>
Motivation: AI-based dermatology faces limitations due to biased datasets, variable image quality, and limited validation, hindering clinical adoption.

Method: Developed DermAI - a lightweight smartphone application with real-time capture, annotation, classification, on-device quality checks, and local model adaptation using a diverse clinical dataset.
...
TL;DR: SHRUG-FM is a framework that combines OOD detection in input/embedding spaces and predictive uncertainty to improve reliability of geospatial foundation models, particularly for burn scar segmentation in underrepresented environments.


<details>
  <summary>Details</summary>
Motivation: Geospatial foundation models often fail in underrepresented environments during deployment, creating reliability issues for climate-sensitive applications like burn scar detection.

Method: Integrates three signals: OOD detection in input space, OOD detection in embedding space, and task-specific predictive uncertainty to flag unreliable predictions.

Result: OOD scores correlate with lower performance in specific environmental conditions; uncertainty-based flags help discard poorly performing predictions; failures concentrate in low-elevation zones and large river areas due to pretraining underrepresentation.

Conclusion: SHRUG-FM provides a pathway for safer, more interpretable deployment of geospatial foundation models, bridging the gap between benchmark performance and real-world reliability in climate applications.
...
TL;DR: MSGNav is a zero-shot navigation system using Multi-modal 3D Scene Graphs (M3DSG) that preserves visual cues through image-based relational edges, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: To address limitations of existing zero-shot navigation methods that compress visual observations into text-only relations, leading to high construction cost, irreversible visual loss, and constrained vocabularies.

Method: Uses M3DSG with dynamically assigned images instead of text edges, plus Key Subgraph Selection, Adaptive Vocabulary Update, Closed-Loop Reasoning modules, and Visibility-based Viewpoint Decision for last-mile problem.
...
Conclusion: MSGNav effectively addresses key limitations in zero-shot navigation by preserving visual evidence and solving the last-mile problem, enabling robust open-vocabulary generalization with low training overhead.
...
TL;DR: Current defense methods against facial identity leakage in personalized AI applications like DreamBooth are ineffective due to visible artifacts and vulnerability to simple purification attacks.


<details>
  <summary>Details</summary>
Motivation: To address the privacy risks of facial identity leakage in personalized AI generation systems and expose the limitations of existing defense mechanisms.

Method: Proposed AntiDB_Purify evaluation framework to systematically test existing defenses against purification threats including traditional filters and adversarial purification.

Result: All current defense methods fail to maintain protection effectiveness under purification threats, revealing their fragility and false sense of security.

Conclusion: There is an urgent need for more imperceptible and robust protection methods to truly safeguard user identity in personalized generation systems.
...
<details>
  <summary>Details</summary>
Motivation: Real-world environmental challenges like background clutter, varying illumination, and occlusions pose significant obstacles to lane detection, especially for data-driven approaches that require expensive data collection and annotation. Lane detection needs to leverage contextual and global information from surrounding lanes and objects.

Method: Proposed SAMIRO (Spatial Attention Mutual Information Regularization) with a pretrained model as Oracle, which transfers knowledge from pretrained models while preserving domain-agnostic spatial information. It has plug-and-play characteristics and can be integrated into various lane detection approaches.
...
Conclusion: SAMIRO effectively enhances lane detection performance by leveraging pretrained model knowledge while maintaining spatial information, making it a versatile solution that can be integrated into various state-of-the-art approaches.
...
TL;DR: Physics-informed Transformer-VAE for PROSAIL model inversion to estimate vegetation parameters from Sentinel-2 data using only simulated training data, achieving performance comparable to methods requiring real satellite imagery.


<details>
  <summary>Details</summary>
Motivation: Accurate retrieval of vegetation biophysical variables is crucial for ecosystem monitoring and agricultural management, but previous methods require real satellite images for training.

Method: Transformer-VAE architecture incorporating PROSAIL radiative transfer model as differentiable physical decoder, trained exclusively on simulated data without real satellite images or in-situ labels.

Result: Achieves retrieval of leaf area index (LAI) and canopy chlorophyll content (CCC) on real-world datasets (FRM4Veg and BelSAR) with accuracy comparable to state-of-the-art methods using real Sentinel-2 data.
...
TL;DR: MonkeyOCR v1.5 is a unified vision-language framework for document parsing that handles complex layouts through a two-stage pipeline combining layout understanding and content recognition, with specialized modules for tables with embedded images and cross-page structures.


<details>
  <summary>Details</summary>
Motivation: Real-world documents often have complex layouts with multi-level tables, embedded images/formulas, and cross-page structures that challenge existing OCR systems, requiring better layout understanding and content recognition.
...
Conclusion: MonkeyOCR v1.5 provides an effective unified framework for parsing complex documents, demonstrating superior performance and robustness through its two-stage approach and specialized table handling modules.
...
TL;DR: GrounDiff is a diffusion-based framework for generating Digital Terrain Models from Digital Surface Models, using iterative denoising with gated design and confidence-guided filtering. It outperforms state-of-the-art methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional DTM generation methods rely on manually tuned parameters, while learning-based approaches need complex architectures and post-processing. There's a need for more effective and scalable solutions for bare-earth elevation modeling.

Method: Proposes Ground Diffusion (GrounDiff) framework using diffusion models for iterative non-ground structure removal, with gated design and confidence-guided generation. Also introduces Prior-Guided Stitching (PrioStitch) for scalability using downsampled global priors.

Result: Achieves 93% RMSE reduction on ALS2DTM and 47% on USGS benchmarks. For road reconstruction, achieves 81% lower distance error on GeRoD benchmark while maintaining competitive smoothness using only DSM inputs.

Conclusion: GrounDiff provides a robust diffusion-based solution for DTM generation that outperforms existing methods across diverse datasets and applications, with GrounDiff+ variant further improving surface smoothness for specialized tasks.
...
Method: Combines YOLOMS with multi-scale detection and sliding-window cropping for enhanced fault feature extraction, uses a lightweight KV mapping module to convert visual outputs to structured text, and employs a domain-tuned LLM for semantic reasoning.
...
TL;DR: GT++ and 3DFETUS automate standard facial plane localization in 3D fetal ultrasound, achieving 4.13mm translation and 7.93° rotation errors, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Manual fetal facial plane acquisition in ultrasound is challenging due to fetal movement, orientation variability, and operator dependency, leading to inconsistencies and diagnostic bias.

Method: GT++ algorithm estimates standard facial planes using annotated anatomical landmarks, while 3DFETUS deep learning model automates plane localization in 3D fetal ultrasound volumes.

Result: Achieved mean translation error of 4.13 mm and mean rotation error of 7.93 degrees per plane, outperforming state-of-the-art methods. Clinical assessments confirmed statistically significant improvements.

Conclusion: The proposed approach effectively automates and standardizes fetal facial plane localization in 3D ultrasound, addressing key challenges in clinical practice.
...
TL;DR: A curated video dataset for detecting convulsive events in laboratory rodents using top-down and side-view video clips, achieving 97% F1-score with TimeSformer classifier.
...
Method: Collected 13,053 video clips (10,101 negative, 2,952 positive) from 19 subjects, used TimeSformer transformer-based video classifier with five-fold cross-validation and strict subject-wise partitioning.

Result: TimeSformer achieved 97% average F1-score for discriminating between seizure and normal activity in rodents.

Conclusion: The publicly released RodEpil dataset enables reproducible research on video-based seizure monitoring in preclinical epilepsy studies.
...
TL;DR: HIT uses semantic segmentation to extract glands from pathology images as meaningful patches for machine learning, improving cancer detection accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current digital pathology methods use grid-based tiling that ignores tissue architecture, introducing irrelevant information and limiting interpretability.

Method: Histology-informed tiling (HIT) uses semantic segmentation to extract glands from whole slide images as biologically meaningful input patches for multiple-instance learning and phenotyping.

Result: HIT achieved gland-level Dice score of 0.83, improved MIL model AUCs by 10% for detecting copy number variations, and revealed 15 gland clusters associated with cancer relapse, mutations, and high Gleason scores.
...
TL;DR: OpenSR-SRGAN is an open, modular framework for single-image super-resolution in Earth Observation that provides unified SRGAN-style model implementations with configuration-driven workflows.


<details>
  <summary>Details</summary>
Motivation: To lower the entry barrier for researchers and practitioners wanting to experiment with SRGANs for satellite data super-resolution by providing easy configuration, extension, and application without code modification.

Method: Provides unified SRGAN-style model implementations exposed through configuration files for generators, discriminators, loss functions, and training schedules, with built-in hooks for logging, validation, and large-scene inference.

Result: A practical tool and benchmark implementation that ships with ready-to-use configurations for common remote sensing scenarios and sensible default settings for adversarial training.

Conclusion: The framework enables reproducible model comparison and deployment of super-resolution pipelines across diverse Earth-observation datasets through configuration-driven workflows.
...
TL;DR: Automated CT imaging approach detects pancreatic surface lobularity as a biomarker for Type 2 Diabetes screening, achieving 0.90 AUC for prediction.
...
Method: Used four deep learning models to segment pancreas in 584 patients, automatically detected pancreatic surface lobularity, and developed multivariate models using CT biomarkers for T2DM prediction.

Result: Pancreatic surface lobularity was significantly higher in diabetic patients (4.26±8.32 vs 3.19±3.62, p=0.01). The PancAP model achieved best segmentation (Dice: 0.79±0.17). T2DM prediction model attained 0.90 AUC, 66.7% sensitivity, and 91.9% specificity.
...
TL;DR: SPOT is a framework that detects and removes redundant tokens in Vision Transformers using attention dynamics and token interactions, achieving up to 40% computational efficiency gains while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers have high computational costs that scale quadratically with token count. Compact attention representations can guide early detection of less important tokens before expensive attention computation.

Method: SPOT uses lightweight predictors that analyze token embeddings, interactions, and attention dynamics across layers to infer token importance. It can be plugged into various ViT architectures for input-specific token prioritization and sparsification.

Result: Empirical evaluations show up to 40% efficiency gains compared to standard ViTs while maintaining or even improving accuracy. The framework is versatile and adaptable to different resource constraints.

Conclusion: SPOT provides an effective approach for computational efficiency in Vision Transformers through context-aware token sparsification, offering significant speedups without performance degradation.
...
<details>
  <summary>Details</summary>
Motivation: Classical Total Variation (TV) depends heavily on the lambda parameter, limiting its efficiency and making it difficult to use effectively for image reconstruction tasks.
...
TL;DR: SemanticVLA is a Vision-Language-Action framework that addresses perceptual redundancy and instruction-vision misalignment in robotic manipulation through semantic-aligned sparsification and enhancement.


<details>
  <summary>Details</summary>
Motivation: Current VLA models suffer from inefficient processing of irrelevant visual inputs (perceptual redundancy) and poor alignment between instructions and visual perception, which limits their practical deployment in robotic manipulation.

Method: 1) Semantic-guided Dual Visual Pruner (SD-Pruner) with Instruction-driven Pruner and Spatial-aggregation Pruner to sparsify redundant perception while preserving semantic alignment; 2) Semantic-complementary Hierarchical Fuser (SH-Fuser) to integrate semantics with spatial geometry; 3) Semantic-conditioned Action Coupler (SA-Coupler) to enhance perception-to-action transformation.

Result: SemanticVLA achieves state-of-the-art performance, surpassing OpenVLA on LIBERO benchmark by 21.1% in success rate while reducing training cost by 3.0-fold and inference latency by 2.7-fold.

Conclusion: SemanticVLA provides an efficient and effective VLA framework for robotic manipulation that addresses key limitations of existing approaches through semantic-aligned sparsification and enhancement.
...
TL;DR: Proposes Separate-then-Map (StM) strategy for reconstructing dynamic humans in real-world environments from monocular videos, addressing spatial inconsistencies at human-scene boundaries through dedicated information mapping.


<details>
  <summary>Details</summary>
Motivation: Existing 4D neural rendering approaches either model scenes holistically (neglecting distinct motion characteristics) or separately (causing spatial inconsistencies and artifacts at boundaries), requiring better integration between human and scene components.

Method: Introduces Separate-then-Map strategy with dedicated information mapping mechanism using shared transformation functions for Gaussian attributes to unify separately modeled components, avoiding exhaustive pairwise interactions while ensuring coherence.

Result: Significantly outperforms state-of-the-art methods in visual quality and rendering accuracy on monocular video datasets, particularly at challenging human-scene interaction boundaries.

Conclusion: StM strategy effectively bridges separately defined models through information mapping, achieving superior reconstruction quality and spatial coherence for dynamic human-scene interactions from monocular videos.
...
TL;DR: A framework for evaluating diversity in text-to-image models through systematic assessment of concepts and their variation factors, including human evaluation templates and prompt sets.


<details>
  <summary>Details</summary>
Motivation: Current T2I models often generate homogeneous outputs lacking diversity, creating a need for robust diversity evaluation methods.

Method: Systematic diversity assessment using human evaluation templates, curated prompt sets with identified variation factors, binomial tests for model comparison, and rigorous comparison of image embeddings.

Result: Enables ranking of T2I models by diversity and identifies specific categories where models struggle with diversity generation.

Conclusion: Provides a robust methodology for T2I diversity evaluation that can guide improvements in model diversity and metric development.
...
TL;DR: CoTyle introduces code-to-style image generation where a single numerical code controls visual style, enabling novel and consistent style generation without complex prompts or reference images.


<details>
  <summary>Details</summary>
Motivation: Existing generative methods struggle with style consistency, limited creativity, and complex style representations. Current approaches rely on lengthy prompts, reference images, or fine-tuning, but lack the ability to generate novel styles from minimal input.

Method: Trains a discrete style codebook from image collections to extract style embeddings, uses these as conditions for text-to-image diffusion models, and trains an autoregressive style generator to model style embedding distribution for novel style synthesis.

Result: CoTyle effectively turns numerical codes into style controllers, demonstrating that one code can represent a complete visual style. The method offers unparalleled simplicity and diversity in style generation.

Conclusion: A style is worth one numerical code. CoTyle unlocks a vast space of reproducible styles from minimal input, filling the gap in open-source research for code-to-style generation previously only explored by industry.
...
TL;DR: OmniVGGT is a 3D foundation model that effectively incorporates geometric modalities (depth, camera intrinsics/extrinsics) through a GeoAdapter with zero-initialized convolutions and stochastic multimodal fusion, achieving SOTA results in various 3D vision tasks and enhancing vision-language-action models for robotics.
...
Method: Proposes GeoAdapter with zero-initialized convolutions to encode geometric information without disrupting representation space, and stochastic multimodal fusion that randomly samples modality subsets during training to enable arbitrary modality inputs during testing.

Result: Outperforms prior methods with auxiliary inputs and achieves state-of-the-art results even with RGB-only input on monocular/multi-view depth estimation, multi-view stereo, and camera pose estimation. Enhanced VLA models outperform vanilla baselines and achieve consistent gains on robotic tasks.

Conclusion: OmniVGGT effectively leverages geometric modalities with minimal overhead, demonstrating superior performance across diverse 3D vision tasks and practical utility in robotics applications through enhanced vision-language-action models.
...
TL;DR: M&M-3D is a parameter-free 3D reasoning architecture for Digital Breast Tomosynthesis that enables direct weight transfer from 2D mammography models while maintaining volumetric information, achieving superior performance in both classification and localization tasks.


<details>
  <summary>Details</summary>
Motivation: Limited annotated DBT data constrains deep learning development, while existing methods either discard volumetric information by flattening DBT volumes or require complex architectures that need more training data.

Method: Proposes M&M-3D architecture that constructs malignancy-guided 3D features and learns 3D reasoning through repeatedly mixing 3D features with slice-level information without adding parameters, enabling direct weight transfer from FFDM models.

Result: Outperforms 2D projection and 3D slice-based methods by 11-54% for localization and 3-10% for classification. Beats complex 3D variants by 20-47% for localization and 2-10% for classification in low-data regime, while matching performance in high-data regime. On BCS-DBT benchmark, beats previous top baseline by 4% for classification and 10% for localization.

Conclusion: M&M-3D effectively addresses DBT data scarcity by enabling parameter-free 3D reasoning with direct weight transfer from 2D models, achieving state-of-the-art performance in breast cancer detection tasks.
...
TL;DR: A novel Multitask Glocal OBIA-Mamba (MSOM) model for Sentinel-2 land use classification that combines object-based analysis with Mamba architecture and global-local dual branches to achieve higher accuracy and finer details than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Sentinel-2 based land use classification faces challenges like spatial heterogeneity, context information, and signature ambiguity, requiring improved methods to handle these data complexities.

Method: Three key components: 1) OBIA-Mamba using superpixels as Mamba tokens to reduce redundancy while preserving details; 2) Global-local dual-branch CNN-Mamba architecture for joint modeling of local spatial details and global context; 3) Multitask optimization with dual loss functions to balance local precision and global consistency.
...
Conclusion: The MSOM framework effectively addresses key challenges in Sentinel-2 classification by integrating object-based analysis, Mamba architecture, and global-local modeling, demonstrating superior performance over state-of-the-art approaches.
...
TL;DR: LUA is a lightweight latent upscaler adapter that performs super-resolution directly on diffusion model's latent code before VAE decoding, enabling efficient high-resolution image generation without modifying the base model.


<details>
  <summary>Details</summary>
Motivation: Diffusion models struggle with high-resolution scaling due to slow direct sampling and post-hoc super-resolution methods that introduce artifacts and additional latency after decoding.

Method: A shared Swin-style backbone with scale-specific pixel-shuffle heads performs super-resolution in latent space before VAE decoding, requiring only a single feed-forward pass and no modifications to the base diffusion model.

Result: LUA achieves comparable perceptual quality to pixel-space SR with 3x lower decoding and upscaling time (only +0.42s for 1024px from 512px vs 1.87s for SwinIR), and generalizes well across different VAE latent spaces.

Conclusion: LUA provides a practical and efficient path to scalable, high-fidelity image synthesis in diffusion pipelines, closely matching native high-resolution generation while being easy to deploy.
...
TL;DR: Depth Anything 3 (DA3) is a transformer-based model that predicts consistent geometry from multiple visual inputs, achieving state-of-the-art performance in camera pose estimation, multi-view geometry, and monocular depth estimation.


<details>
  <summary>Details</summary>
Motivation: To develop a minimal yet powerful model for predicting spatially consistent geometry from arbitrary visual inputs, eliminating the need for architectural specialization and complex multi-task learning.

Method: Uses a single plain transformer (vanilla DINO encoder) as backbone with teacher-student training paradigm, employing a singular depth-ray prediction target instead of multi-task learning.

Result: Sets new SOTA on visual geometry benchmark: 44.3% improvement in camera pose accuracy and 25.1% in geometric accuracy over prior VGGT, and outperforms Depth Anything 2 in monocular depth estimation.

Conclusion: DA3 demonstrates that simple transformer architectures with minimal modeling can achieve superior geometry prediction performance across multiple tasks, establishing new benchmarks in visual geometry.
...
TL;DR: Self-Consistency Sampling (SCS) addresses the problem of unfaithful reasoning trajectories in multimodal LLMs by introducing visual perturbations and repeated sampling to differentiate genuine reasoning from lucky guesses.


<details>
  <summary>Details</summary>
Motivation: Current outcome-reward RL methods in multimodal reasoning benchmarks reward both genuine reasoning and unfaithful trajectories that guess correctly after faulty reasoning, creating a fundamental flaw in the training process.

Method: SCS introduces small visual perturbations and performs repeated truncation/resampling of initial trajectories, using agreement among resulting trajectories to compute a differentiable consistency score that down-weights unreliable traces during policy updates.

Result: SCS improves accuracy by up to 7.7 percentage points on six multimodal benchmarks when integrated with RLOO, GRPO, and REINFORCE++ methods, with negligible extra computation. Benefits also extend to other models like Qwen2.5-VL-3B-Instruct and InternVL3-8B.

Conclusion: SCS provides a simple, general solution to the unfaithful trajectory problem in outcome-reward RL for multimodal LLMs, effectively distinguishing genuine reasoning from lucky guesses.
...
TL;DR: Music Flamingo is a large audio-language model that advances music understanding by addressing challenges in dynamic, layered musical content through curated datasets and enhanced training methods.


<details>
  <summary>Details</summary>
Motivation: Current audio-language models struggle with music due to its complex nature and lack of high-quality music data, limiting them to surface-level understanding and poor cross-cultural generalization.

Method: Curated MF-Skills dataset with rich captions and QA pairs, fine-tuned enhanced Audio Flamingo 3 backbone, and introduced post-training with MF-Think chain-of-thought dataset and GRPO-based reinforcement learning.

Result: Achieved state-of-the-art results across 10+ music understanding benchmarks, demonstrating superior reasoning and generalization capabilities in music analysis.

Conclusion: Music Flamingo sets a new standard for music understanding, moving from surface-level recognition to human-like layered perception, providing a foundation for future music AI systems.
...
TL;DR: LLM-based agents show stronger skepticism against erroneous messages, enabling better Byzantine fault tolerance than traditional agents. The proposed CP-WBFT mechanism leverages LLMs' reflective capabilities to enhance MAS reliability across different topologies.
...
Method: Designed CP-WBFT, a confidence probe-based weighted Byzantine Fault Tolerant consensus mechanism that uses probe-based, weighted information flow transmission to capitalize on LLMs' intrinsic reflective and discriminative capabilities.
...
Conclusion: LLM-based agents demonstrate stronger skepticism and better Byzantine fault tolerance than traditional agents, and the proposed CP-WBFT mechanism effectively enhances the stability and reliability of multi-agent systems with different topologies.
...
### [176] [Simulating Misinformation Propagation in Social Networks using Large Language Models](https://arxiv.org/abs/2511.10384)
*Raj Gaurav Maurya,Vaibhav Shukla,Raj Abhijit Dandekar,Rajat Dandekar,Sreedath Panat*

Main category: cs.SI

TL;DR: LLM personas with cognitive biases are used to simulate misinformation propagation, showing that identity-driven personas accelerate misinformation while expert personas preserve facts.


<details>
  <summary>Details</summary>
Motivation: To understand how misinformation spreads through cognitive biases and identity-driven reasoning in social media by modeling these mechanisms using LLM personas.

Method: Model LLM personas as synthetic agents with biases and ideological alignments, use auditor-node framework to track misinformation evolution through networks, and quantify factual degradation with misinformation indices.

Result: Identity- and ideology-based personas act as misinformation accelerators in politics, marketing, and technology, while expert personas maintain factual stability. Early distortions in heterogeneous networks rapidly escalate to propaganda-level misinformation.

Conclusion: LLMs serve dual roles as proxies for human biases and as auditors for tracking information fidelity, providing an interpretable framework for studying and mitigating misinformation diffusion.
...
### [177] [Efficient Automated Diagnosis of Retinopathy of Prematurity by Customize CNN Models](https://arxiv.org/abs/2511.10023)
*Farzan Saeedi,Sanaz Keshvari,Nasser Shoeibi*

Main category: eess.IV

TL;DR: Deep learning approach for Retinopathy of Prematurity diagnosis using customized CNN models that outperform pre-trained networks in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: To improve ROP diagnosis through precise and efficient deep learning methods that can serve as valuable diagnostic aids in clinical settings.

Method: Employed CNN-based approaches with focus on dataset curation, preprocessing, and model architecture. Implemented voting system and customized CNN models to reduce computational burden.

Result: Tailored CNN models achieved superior performance over pre-trained models with higher accuracy and F1-scores. Voting system further enhanced performance. Models demonstrated feasibility for clinical deployment.

Conclusion: Deep learning models significantly enhance ROP diagnostic precision and efficiency, with customized CNNs offering better performance and reduced computational costs compared to pre-trained networks.
...
### [178] [Probability-Biased Attention over Directed Bipartite Graphs for Long-Tail ICD Coding](https://arxiv.org/abs/2511.09559)
*Tianlei Chen,Yuxiao Chen,Yang Li,Feifei Wang*

Main category: cs.LG

TL;DR: Proposes a graph-based method for automated ICD coding that models co-occurrence relationships between common and rare codes using a directed bipartite graph encoder with probability-based attention bias to improve rare code classification.


<details>
  <summary>Details</summary>
Motivation: Automated ICD coding faces challenges due to large label space (10K-20K codes) and long-tail distribution where rare codes lack sufficient training data, requiring methods to leverage relationships between codes.

Method: Constructs Directed Bipartite Graph Encoder with common and rare code nodes, using edges directed from common to rare codes with probability-based bias derived from conditional co-occurrence probabilities. Uses LLM-generated code descriptions for enriched embeddings.
...
Conclusion: The proposed co-occurrence encoding method effectively enriches rare code representations by leveraging statistical relationships with common codes, demonstrating strong performance in handling the long-tail distribution problem in automated ICD coding.
...
### [179] [OutSafe-Bench: A Benchmark for Multimodal Offensive Content Detection in Large Language Models](https://arxiv.org/abs/2511.10287)
*Yuping Yan,Yuhan Xie,Yuanshuai Li,Yingchao Yu,Lingjuan Lyu,Yaochu Jin*

Main category: cs.LG

TL;DR: OutSafe-Bench is a comprehensive multimodal safety evaluation benchmark with 18K+ bilingual prompts across 4 modalities and 9 risk categories, featuring novel metrics MCRS and FairScore to assess overlapping risks and ensure fair evaluation.


<details>
  <summary>Details</summary>
Motivation: Growing concerns about unsafe content output from Multimodal Large Language Models (MLLMs) including toxic language, biased imagery, privacy violations, and misinformation, with current safety benchmarks being limited in modality coverage and evaluation scope.

Method: Created OutSafe-Bench with large-scale dataset spanning text, images, audio, and video; introduced Multidimensional Cross Risk Score (MCRS) for overlapping risk assessment; developed FairScore framework using top-performing models as adaptive juries for bias mitigation.

Result: Evaluation of nine state-of-the-art MLLMs revealed persistent and substantial safety vulnerabilities, demonstrating the urgent need for improved safety measures in multimodal AI systems.

Conclusion: MLLMs have significant safety gaps that require robust safeguards, and OutSafe-Bench provides a comprehensive framework for systematic safety evaluation across multiple modalities and risk categories.
...
### [180] [AgentEvolver: Towards Efficient Self-Evolving Agent System](https://arxiv.org/abs/2511.10395)
...
TL;DR: AgentEvolver is a self-evolving agent system that uses LLMs to enable autonomous learning through self-questioning, self-navigating, and self-attributing mechanisms, reducing the need for manual datasets and improving exploration efficiency.


<details>
  <summary>Details</summary>
Motivation: Current approaches to developing autonomous agents using LLMs are costly and inefficient, requiring manually constructed datasets and reinforcement learning pipelines with extensive random exploration, leading to high costs and poor efficiency.

Method: AgentEvolver introduces three synergistic mechanisms: self-questioning for curiosity-driven task generation, self-navigating for improved exploration efficiency through experience reuse and hybrid policy guidance, and self-attributing for enhanced sample efficiency via differentiated rewards.

Result: Preliminary experiments show that AgentEvolver achieves more efficient exploration, better sample utilization, and faster adaptation compared to traditional RL-based baselines.
...
### [181] [Impact of Layer Norm on Memorization and Generalization in Transformers](https://arxiv.org/abs/2511.10566)
*Rishi Singhal,Jung-Eun Kim*

Main category: cs.LG

TL;DR: LayerNorm's impact on memorization and learning differs between Pre- and Post-LayerNorm transformers - it stabilizes learning in Pre-LayerNorm models but affects memorization in Post-LayerNorm models.


<details>
  <summary>Details</summary>
Motivation: To understand how LayerNorm influences memorization and learning across Pre- and Post-LayerNorm transformer architectures, as this impact remains unclear despite LayerNorm being a fundamental component.

Method: Analyzed the effects of removing LayerNorm parameters in both Pre- and Post-LayerNorm transformers, with specific focus on early vs middle/later layers, validated across 13 models and 6 Vision/Language datasets.

Result: Removing LayerNorm parameters exacerbates memorization and destabilizes learning in Pre-LayerNorm models, while it mitigates memorization by restoring genuine labels in Post-LayerNorm models. Early layers' LayerNorm are most critical.

Conclusion: LayerNorm plays distinct roles in Pre- vs Post-LayerNorm transformers - crucial for stable learning in Pre-LayerNorm and for controlling memorization in Post-LayerNorm architectures.
...
TL;DR: Proposes a Responsible Reinforcement Learning (RRL) framework that integrates emotional context and ethical constraints into healthcare decision systems to ensure emotionally aligned and safe interventions.


<details>
  <summary>Details</summary>
Motivation: Existing healthcare decision systems use static rules or engagement-maximizing heuristics that overlook users' emotional context and ethical constraints, risking insensitive or unsafe interventions in sensitive domains like mental health.
...
Result: Conceptual framework that operationalizes empathy and responsibility in ML policy optimization, bridging safe RL, affective computing and responsible AI for emotionally aware personalization systems.

Conclusion: The RRL framework initiates methodological conversation about ethically aligned reinforcement learning for trustworthy personalization systems in human-centric domains like behavioral health and digital therapeutics.
...
### [183] [PRISM: Diversifying Dataset Distillation by Decoupling Architectural Priors](https://arxiv.org/abs/2511.09905)
...
TL;DR: PRISM is a dataset distillation framework that uses multiple teacher models to overcome the inductive bias of single-teacher approaches, improving synthetic data diversity and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods inherit bias from single teacher models, leading to overly smooth samples with reduced intra-class diversity and limited generalization as dataset size increases.

Method: PRISM decouples logit-matching and regularization objectives, using different teacher architectures: a primary model for logits and a stochastic subset for batch-normalization alignment. It includes teacher selection strategies and scalable cross-class batch formation.

Result: On ImageNet-1K, PRISM consistently outperforms single-teacher methods and recent multi-teacher variants at low- and mid-IPC regimes, with significantly richer intra-class diversity and notable drop in cosine similarity between features.

Conclusion: PRISM effectively disentangles architectural priors during synthesis, producing more diverse and generalizable synthetic data compared to existing approaches.
...
### [184] [eXIAA: eXplainable Injections for Adversarial Attack](https://arxiv.org/abs/2511.10088)
...
<details>
  <summary>Details</summary>
Motivation: To expose vulnerabilities in current explainability methods by showing how explanations can be manipulated while maintaining the same predicted class, raising concerns about reliability in safety-critical applications.

Method: Single-step black-box attack that modifies explanations (saliency maps, integrated gradients, DeepLIFT SHAP) for pretrained ResNet-18 and ViT-B16 on ImageNet, without requiring model access or weights.

Result: The attack successfully generates dramatically different explanations without changing predictive probabilities, with effectiveness validated using mean absolute difference and SSIM metrics.

Conclusion: The low requirements of this attack reveal critical vulnerabilities in current explainability methods, questioning their reliability for safety-critical applications.
...
### [185] [How does My Model Fail? Automatic Identification and Interpretation of Physical Plausibility Failure Modes with Matryoshka Transcoders](https://arxiv.org/abs/2511.10094)
*Yiming Tang,Abhijeet Sinha,Dianbo Liu*

Main category: cs.LG

TL;DR: Matryoshka Transcoders framework automatically discovers and interprets physical plausibility failures in generative models using hierarchical sparse feature learning and large multimodal models.


<details>
  <summary>Details</summary>
Motivation: Current generative models often produce physically implausible outputs that escape detection by existing evaluation methods, and there's no framework for automatically identifying specific physical error patterns.

Method: Extends Matryoshka representation learning to transcoder architectures for hierarchical sparse feature learning, training on intermediate representations from physical plausibility classifiers and using large multimodal models for interpretation.

Result: Identifies diverse physics-related failure modes without manual feature engineering, achieving superior feature relevance and accuracy compared to existing approaches, and establishes a benchmark for evaluating physical plausibility.

Conclusion: The framework provides valuable insights into how state-of-the-art generative models fail to follow physical constraints, enabling targeted model improvements.
...
### [186] [Intrinsic Dimensionality as a Model-Free Measure of Class Imbalance](https://arxiv.org/abs/2511.10475)
...
<details>
  <summary>Details</summary>
Motivation: Traditional imbalance measures based on class cardinalities ignore redundant examples and inherent learning difficulties, while complex measures like training loss require model training. There's a need for an easy-to-compute, model-free imbalance measure.

Method: The paper introduces Intrinsic Dimensionality (ID) as a measure of class imbalance that can be computed without training models and can be incorporated into various imbalance mitigation techniques like re-weighting and re-sampling.

Result: Experiments across five datasets with diverse imbalance ratios show that ID-based methods consistently outperform cardinality-based re-weighting and re-sampling techniques. Combining ID with cardinality further improves performance.

Conclusion: Intrinsic Dimensionality is an effective, model-free measure for class imbalance that outperforms traditional cardinality-based approaches and can be enhanced by combining with cardinality information.
...
### [187] [Trapped by Their Own Light: Deployable and Stealth Retroreflective Patch Attacks on Traffic Sign Recognition Systems](https://arxiv.org/abs/2511.10050)
*Go Tsuruoka,Takami Sato,Qi Alfred Chen,Kazuki Nomoto,Ryunosuke Kobayashi,Yuna Tanaka,Tatsuya Mori*

Main category: cs.CR

TL;DR: ARP is a stealthy adversarial attack using retroreflective patches that activate under headlights, achieving high success rates against traffic sign recognition systems while being nearly undetectable.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks on traffic sign recognition systems have limitations in visual detectability or practical implementation, leaving unexplored vulnerabilities that need to be addressed.

Method: Developed Adversarial Retroreflective Patch (ARP) using retroreflective materials activated by headlights, combined with retroreflection simulation and black-box optimization to maximize attack effectiveness.

Result: ARP achieved ≥93.4% success rate in dynamic scenarios at 35 meters and ≥60% success rate against commercial TSR systems, with higher stealthiness scores than previous attacks.

Conclusion: ARP demonstrates significant security vulnerabilities in TSR systems, and the proposed DPR Shield defense using polarized filters provides ≥75% protection against such attacks.
...
### [188] [VEDA: 3D Molecular Generation via Variance-Exploding Diffusion with Annealing](https://arxiv.org/abs/2511.09568)
*Peining Zhang,Jinbo Bi,Minghu Song*

Main category: physics.chem-ph

TL;DR: VEDA is a unified SE(3)-equivariant framework that combines variance-exploding diffusion with annealing to efficiently generate conformationally accurate 3D molecular structures, achieving both high accuracy and computational efficiency.
...
Method: Combines variance-exploding diffusion with annealing using: (1) VE schedule for noise injection analogous to simulated annealing, (2) novel preconditioning scheme reconciling SE(3)-equivariant networks with residual-based diffusion, (3) arcsin-based scheduler concentrating sampling in critical intervals.

Result: Matches flow-based model efficiency with only 100 sampling steps, achieves state-of-the-art valency stability and validity on QM9 and GEOM-DRUGS datasets, and generates remarkably stable structures with median relaxation energy of only 1.72 kcal/mol (vs 32.3 kcal/mol baseline).
...
### [189] [Enhancing the Medical Context-Awareness Ability of LLMs via Multifaceted Self-Refinement Learning](https://arxiv.org/abs/2511.10067)
...
Method: Data-driven approach using attribute-conditioned query generation, LLM self-evaluation across three facets, response refinement, and supervised fine-tuning with knowledge distillation.

Result: Significantly improved LLM performance on HealthBench (63.8%) and hard subset (43.1%), with Qwen3-32B surpassing teacher model to achieve new SOTA.

Conclusion: MuSeR effectively enhances LLMs' context-awareness in medical applications through multifaceted self-refinement, enabling better real-world performance.
...
### [190] [ProgRAG: Hallucination-Resistant Progressive Retrieval and Reasoning over Knowledge Graphs](https://arxiv.org/abs/2511.10240)
*Minbae Park,Hyemin Yang,Jeonghyun Kim,Kunsoo Park,Hyunjoon Kim*

Main category: cs.AI

TL;DR: ProgRAG is a multi-hop KGQA framework that decomposes complex questions into sub-questions, progressively extends reasoning paths with uncertainty-aware pruning, and optimizes context organization to improve reasoning reliability and quality.


<details>
  <summary>Details</summary>
Motivation: Address retrieval and reasoning failures in KG-enhanced LLMs, including inaccurate retrieval, reasoning failures due to long input contexts, and premature/incomplete reasoning caused by LLMs self-assessing evidence sufficiency.

Method: Decomposes complex questions into sub-questions, progressively extends partial reasoning paths by answering each sub-question, uses external retrievers with uncertainty-aware pruning by LLM, and optimizes context organization of reasoning paths.

Result: Outperforms existing baselines on three well-known datasets for multi-hop KGQA, demonstrating improved reliability and reasoning quality.

Conclusion: ProgRAG effectively addresses retrieval and reasoning challenges in KG-enhanced LLMs through progressive decomposition and uncertainty-aware pruning, offering a more reliable approach for complex knowledge-intensive tasks.
...
### [191] [FactGuard: Event-Centric and Commonsense-Guided Fake News Detection](https://arxiv.org/abs/2511.10281)
*Jing He,Han Zhang,Yuanhui Xiao,Wei Guo,Shaowen Yao,Renyang Liu*

Main category: cs.AI

TL;DR: FactGuard is a novel fake news detection framework that uses LLMs to extract event-centric content, reducing style dependency, with dynamic usability assessment and knowledge distillation for practical deployment.


<details>
  <summary>Details</summary>
Motivation: Traditional style-based fake news detection is becoming less effective as adversaries imitate authentic writing styles, while current LLM approaches suffer from shallow exploration, ambiguous usability, and high costs.

Method: Uses LLMs to extract event-centric content, introduces dynamic usability mechanism to identify contradictions and ambiguous cases, and employs knowledge distillation to create efficient FactGuard-D for resource-constrained scenarios.

Result: Comprehensive experiments on two benchmark datasets show consistent outperformance of existing methods in both robustness and accuracy.

Conclusion: FactGuard effectively addresses style sensitivity and LLM usability challenges in fake news detection through event-centric content extraction and adaptive reasoning.
...
<details>
  <summary>Details</summary>
Motivation: EMS responders face intense cognitive demands in high-stakes situations, and AI cognitive assistants could help reduce this burden by supporting real-time data collection and decision making.

Method: Created EgoEMS dataset with 233 simulated emergency scenarios performed by 62 participants (including 46 EMS professionals), captured using an open-source, low-cost, replicable system with multimodal annotations including keysteps, audio transcripts, action quality metrics, and segmentation masks.

Result: Developed the first end-to-end, high-fidelity, multimodal, multiperson EMS dataset with realistic responder-patient interactions aligned with national standards, plus benchmark tasks for keystep recognition and action quality estimation.

Conclusion: EgoEMS provides a foundation for developing AI support tools for EMS and aims to inspire research that improves patient outcomes through intelligent EMS systems.
...
### [193] [Learning to Pose Problems: Reasoning-Driven and Solver-Adaptive Data Synthesis for Large Reasoning Models](https://arxiv.org/abs/2511.09907)
*Yongxian Wei,Yilin Zhao,Li Shen,Xinrui Chen,Runxi Cheng,Sinan Du,Hao Yu,Gang Liu,Jiahong Yan,Chun Yuan,Dian Li*

Main category: cs.AI

TL;DR: A reasoning-based problem generator that creates adaptive difficulty problems by using solver feedback as reward signal, enabling co-evolution between generator and solver models.
...
Method: Construct related problem pairs with intermediate problem-design CoT, use solver feedback as reward to calibrate difficulty, and enable co-evolution between generator and solver.

Result: Achieves 2.5% average improvement on 10 reasoning benchmarks, generalizes to language and vision-language models, and co-evolution yields additional 0.7% performance gain.
...
### [194] [Querying Labeled Time Series Data with Scenario Programs](https://arxiv.org/abs/2511.10627)
*Edward Kim,Devan Shanker,Varun Bharadwaj,Hongbeen Park,Jinkyu Kim,Hazem Torfah,Daniel J Fremont,Sanjit A Seshia*

Main category: cs.AI

TL;DR: The paper addresses the sim-to-real gap in autonomous vehicle testing by developing a formal method to validate simulated failure scenarios using real-world sensor data.
...
Method: Introduces a formal definition for matching labeled time series sensor data to abstract scenarios represented as scenario programs using Scenic probabilistic programming language, and presents a querying algorithm to identify matching data subsets.
...
Conclusion: The proposed approach provides an effective method for validating simulated failure scenarios against real-world data, bridging the sim-to-real gap in autonomous vehicle testing.
