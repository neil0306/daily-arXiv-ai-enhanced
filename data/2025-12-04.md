...
TL;DR: Researchers operationalize ethical entropy framework to monitor LLM safety drift, developing classifier to measure entropy dynamics and detect value degradation in real-time.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety assessments use static benchmarks, but real failures are dynamic: value drift under distribution shift, jailbreak attacks, and slow alignment degradation in deployment. Need operational framework to monitor these dynamic safety risks.

Method: Define 5-way behavioral taxonomy, train classifier to estimate ethical entropy S(t) from model transcripts, measure entropy dynamics across stress tests for base and instruction-tuned variants of four frontier models. Estimate effective alignment work rate gamma_eff and embed S(t) and gamma_eff in monitoring pipeline with stability thresholds.

Result: Base models show sustained entropy growth, while tuned variants suppress drift and reduce ethical entropy by ~80%. Monitoring pipeline successfully raises alerts when entropy drift exceeds stability thresholds, enabling run-time oversight of value drift.

Conclusion: Ethical entropy framework provides operational tool for dynamic LLM safety monitoring, detecting value drift in real-time and quantifying alignment effectiveness, addressing limitations of static benchmark approaches.
...
TL;DR: This paper investigates vulnerabilities in Embeddings-as-a-Service (EaaS) watermarks and proposes a new defense. It shows existing watermarks can be bypassed via paraphrasing attacks, then introduces WET - a linear transformation-based watermarking technique that's robust against such attacks.


<details>
  <summary>Details</summary>
Motivation: EaaS providers need to protect their intellectual property from imitation attacks where attackers clone their models. While watermarks have been added to text embeddings for ownership verification, existing approaches have vulnerabilities that need to be addressed.

Method: 1) Demonstrates paraphrasing attacks that bypass existing EaaS watermarks. 2) Proposes WET (Watermarking EaaS with Linear Transformation) - a novel watermarking technique using linear transformation of embeddings, with verification via reverse transformation and similarity comparison.

Result: 1) Paraphrasing attacks effectively bypass current state-of-the-art EaaS watermarks across various attack setups and datasets. 2) WET demonstrates robustness against paraphrasing attacks with near-perfect verifiability. 3) Ablation studies validate the significance of each component and hyperparameter in WET.

Conclusion: Existing EaaS watermarks are vulnerable to paraphrasing attacks, revealing a new security weakness. The proposed WET technique provides effective defense against such attacks, offering robust watermark verification for EaaS intellectual property protection.
...
TL;DR: RDG framework generates balanced reasoning data to mitigate choice-supportive bias in LLMs through fine-tuning, achieving 81.5-94.3% improvement while maintaining standard benchmark performance.


<details>
  <summary>Details</summary>
Motivation: LLMs exhibit choice-supportive bias (CSB) that compromises objectivity in AI-assisted decision making, but existing debiasing methods focus on demographic/social biases, leaving cognitive biases largely unexplored.

Method: Reasoning Dependency Generation (RDG) framework automatically constructs balanced reasoning QA pairs by explicitly (un)modeling dependencies between choices, evidences, and justifications, generating large-scale datasets across domains with Contextual Dependency Data and Dependency Decouple Data.
...
Conclusion: This work pioneers an approach for addressing cognitive biases in LLMs and contributes to more reliable AI-assisted decision support systems by mitigating choice-supportive bias through reasoning data generation.
...
TL;DR: This paper introduces an open-source tool for classifying job vacancy texts using ESCO and EQF frameworks, comparing Sentence Linking vs Entity Linking methods, and providing annotated datasets for evaluation.


<details>
  <summary>Details</summary>
Motivation: To improve labor market information classification by linking job vacancy texts to European frameworks (ESCO and EQF), moving beyond surface-level skill extraction and providing computational infrastructure for analyzing work and skills in the digital economy.

Method: Two methodologies compared: Sentence Linking and Entity Linking. Developed an open-source tool incorporating both approaches. Created annotated datasets for evaluating occupation and qualification representation. Examined generative large language models for the task.

Result: Released an open-source tool (tabiya-livelihoods-classifier) that facilitates labor classification research. Contributed annotated datasets for evaluation. Advanced state of the art in job entity extraction and provided infrastructure for analyzing labor market narratives.

Conclusion: The study advances job entity extraction techniques, offers practical tools for labor market analysis, and contributes to understanding skills and occupations in digitally mediated economies through open-source computational infrastructure.
...
<details>
  <summary>Details</summary>
Motivation: Existing Text2KG methods rely on iterative LLM prompting, which is computationally expensive and prone to missing complex relations distributed throughout text.

Method: Combines controlled data generation pipeline (extracts subgraphs from KBs, filters noise, uses LLMs to generate text descriptions) with supervised fine-tuning of lightweight models for single-shot KG construction.

Result: Outperforms larger non-fine-tuned LLMs and SOTA Text2KG approaches on CE12k dataset, shows stronger cross-dataset generalization on CrossEval-1200 test set.

Conclusion: Realistic, high-quality training data is crucial for advancing efficient and high-performing Text2KG systems, and InvertiTune demonstrates this through its data generation + fine-tuning approach.
...
Method: Train a lightweight causal language model that extracts structured cause-effect pairs from political text. The model parses explanations into a format suitable for downstream analysis with modest annotation requirements.

Result: The method demonstrates ability to study causal explanations at scale, shows generalizability across contexts, achieves accuracy comparable to human coding, and requires only modest annotation efforts.

Conclusion: The framework provides a systematic approach for analyzing explanations in political text, enabling large-scale study of causal claims and addressing a gap in political science methodology.
...
TL;DR: RMFT is a privacy-preserving fine-tuning method that reduces PII memorization in LLMs by 80% while maintaining performance, outperforming deduplication methods.
...
Conclusion: RMFT effectively reduces PII memorization in LLMs with minimal performance impact, and the MaxTER framework provides a Pareto-optimal evaluation for privacy-utility tradeoffs.
...
TL;DR: LLM-assisted pipeline for sociolinguistic analysis of bilingual discourse in Spanish-English and Spanish-Guaraní contexts, revealing systematic patterns in code-switching and diglossic divisions.
...
Method: Used large language models to automatically label topic, genre, and discourse-pragmatic functions across 3,691 code-switched sentences, integrated demographic metadata from Miami Bilingual Corpus, and enriched Spanish-Guaraní dataset with new topic annotations.

Result: Revealed systematic links between gender, language dominance, and discourse function in Miami data, and clear diglossic division between formal Guaraní and informal Spanish in Paraguayan texts, replicating earlier observations with quantitative evidence.
...
TL;DR: PERCS dataset provides biomedical abstracts with summaries tailored to four personas (Laypersons, Premedical Students, Non-medical Researchers, Medical Experts) to enable persona-specific medical text simplification.


<details>
  <summary>Details</summary>
Motivation: Existing medical text simplification resources assume a single generic audience, overlooking variations in medical literacy and information needs across different user groups.

Method: Created PERCS dataset with biomedical abstracts paired with persona-specific summaries, reviewed by physicians for factual accuracy and persona alignment using detailed error taxonomy.

Result: Dataset shows clear differences in readability, vocabulary, and content depth across personas. Benchmarked four LLMs using automatic evaluation metrics for comprehensiveness, readability, and faithfulness.

Conclusion: PERCS enables research on persona-specific communication and controllable biomedical summarization, with dataset and evaluation materials publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Autoregressive LLMs trained on Next-Token Prediction suffer from "Topic Drift" where generation wanders away from initial prompts due to reliance on local associations rather than global planning. While scaling model size helps, the fundamental myopia of NTP objective remains.

Method: Introduces Idea-Gated Transformer with auxiliary "Idea Head" trained to predict bag-of-words distribution for future context windows, creating latent "Concept Vector" that actively gates main vocabulary during generation. Uses differentiable gating mechanism that suppresses semantically irrelevant tokens in real-time.
...
Conclusion: Idea-Gated Transformer offers parameter-efficient path toward more controllable language modeling by separating semantic planning from syntactic generation, effectively preventing topic drift while maintaining generation quality.
...
TL;DR: HBLR: A novel backward reasoning framework that combines confidence-aware symbolic translation with hypothesis-driven verification to improve logical reasoning in LLMs.


<details>
  <summary>Details</summary>
Motivation: Current LLM reasoning approaches rely on forward reasoning (premises to conclusions), which suffers from redundant inference paths, hallucinated steps, semantic drift, and inefficiency. There's a need for more reliable and efficient logical reasoning methods.

Method: Two-phase framework: 1) Confidence-aware symbolic translation - only high-confidence spans converted to First-Order Logic, uncertain content remains in natural language, with translation reflection for semantic fidelity. 2) Hypothesis-driven backward reasoning - assumes conclusion is true and recursively verifies premises, with reasoning reflection to correct flawed inference steps.

Result: Extensive experiments on five reasoning benchmarks show HBLR consistently outperforms strong baselines in both accuracy and efficiency.

Conclusion: HBLR provides a more reliable and efficient approach to logical reasoning by simulating human deductive thinking through backward verification and maintaining semantic fidelity through selective symbolic translation.
...
TL;DR: Higher-Order Attention Network (Hon) enhances Transformer representational power using recursive attention to overcome low-rank bottlenecks, achieving better performance with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Standard first-order attention in Transformers suffers from low-rank bottlenecks and struggles to capture intricate multi-hop relationships within single layers, limiting their ability to model complex dependencies.
...
Result: Theoretical analysis shows Hon breaks linear bottlenecks of standard attention, and empirical results demonstrate it outperforms standard Transformers on multiple benchmarks with only O(1) additional parameters.

Conclusion: Higher-order attention via recursive refinement of Query/Key representations provides enhanced expressivity for capturing complex relationships while maintaining parameter efficiency, offering a promising direction for Transformer architecture improvements.
...
TL;DR: Researchers collected and analyzed 11.5 hours of spoken dialogue from Portal 2 co-op gameplay, identifying unique linguistic phenomena in collaborative problem-solving.


<details>
  <summary>Details</summary>
Motivation: Cooperative video games provide rich language data for studying how people communicate and coordinate under uncertainty in complex environments, which is underrepresented in existing dialogue corpora.

Method: Collected the Portal Dialogue Corpus: 11.5 hours of spoken human dialogue from Portal 2 co-op mode, comprising 24.5K utterances, with player videos, audio, transcripts, game state data, and manual/automatic annotations.

Result: Identified linguistic phenomena rarely seen in chitchat or task-oriented dialogue: complex spatial reference, clarification and repair, and ad-hoc convention formation. The corpus is publicly released for future research.

Conclusion: The Portal Dialogue Corpus provides valuable data for analyzing language use in complex, situated, collaborative problem-solving scenarios, filling gaps in existing dialogue resources.
...
Result: Consistently outperforms LoRA and its state-of-the-art variants with the same number of trainable parameters across NLG, NLU, and commonsense reasoning tasks on GPT-2, RoBERTa, DeBERTa, and LLaMA-1/2/3 models.

Conclusion: Dual LoRA effectively incorporates inductive bias into LoRA to improve performance while maintaining parameter efficiency, offering a better simulation of full fine-tuning parameter updates.
...
TL;DR: PretrainZero is a reinforcement active learning framework that extends RL from domain-specific post-training to general pretraining, enabling self-supervised learning on general corpus without verifiable labels.


<details>
  <summary>Details</summary>
Motivation: Current RL-based large models rely heavily on verifiable rewards in specific domains, creating a bottleneck for extending general reasoning capabilities. The goal is to mimic human active learning from general experience to achieve broader artificial general intelligence.

Method: PretrainZero uses active pretraining with a unified reasoning policy to identify informative content from pretraining corpus, self-supervised learning without verifiable labels or supervised fine-tuning, and verification scaling by tackling increasingly challenging masked spans.
...
Conclusion: PretrainZero successfully extends RL to general pretraining, breaking the verification data-wall for general reasoning and significantly enhancing the reasoning abilities of pretrained base models through active learning on general corpus.
...
TL;DR: Top-k Attention mechanism reduces LLM inference costs while maintaining performance on long-context tasks through selective attention to most relevant keys.


<details>
  <summary>Details</summary>
Motivation: LLM inference computational costs are a critical bottleneck for long-context modeling tasks like agents and multimodal applications, requiring more efficient attention mechanisms.

Method: Investigates Top-k Attention in both decoding and training phases: 1) Exact Top-k Decoding retains only keys with highest similarity to query, 2) Native Top-k Attention training strategy ensures consistency between training and inference, 3) Explores approximate Top-k algorithms for computational efficiency.

Result: Top-k Decoding achieves performance comparable to or surpassing full attention on HELMET and LongBench v2. Training-inference consistency further enhances performance. Positive correlation found between approximate algorithm fidelity and downstream performance. Models show entropy reduction, supporting low-entropy adaptation hypothesis.

Conclusion: Top-k Attention is an effective approach for reducing LLM inference costs while maintaining performance, with theoretical support from entropy reduction principles and practical validation through extensive experimentation.
...
TL;DR: Reasoning strategies in LLMs for abstractive summarization show mixed results - explicit reasoning improves fluency but hurts factual faithfulness, while implicit reasoning shows opposite pattern, with no universal benefit.


<details>
  <summary>Details</summary>
Motivation: While LLMs excel at analytical reasoning tasks, their effectiveness for abstractive summarization remains largely unverified despite widespread assumptions. The paper aims to systematically test whether reasoning capabilities actually improve summarization quality and faithfulness.

Method: Tailored general reasoning strategies to summarization domain, then conducted large-scale comparative study of 8 reasoning strategies and 3 Large Reasoning Models (LRMs) across 8 diverse datasets, evaluating both summary quality and faithfulness.

Result: Reasoning is not universally effective - effectiveness depends on specific strategy and context. Found trade-off: explicit reasoning strategies improve fluency but reduce factual grounding, while implicit reasoning in LRMs shows opposite pattern. Increasing LRM's internal reasoning budget doesn't improve and can even hurt factual consistency.

Conclusion: Effective summarization requires faithful compression rather than creative over-thinking. The study reveals nuanced relationship between reasoning and summarization, challenging assumptions about universal benefits of reasoning for this task.
...
TL;DR: Researchers introduce INDI-PROP, the first ideologically grounded fine-grained narrative dataset for analyzing propaganda in Indian news media, with multi-level annotations for bias, narratives, and persuasive techniques, and propose two GPT-4o-mini guided reasoning frameworks (FANTA and TPTC) that outperform baselines.


<details>
  <summary>Details</summary>
Motivation: Narratives serve as cognitive and emotional scaffolds for propaganda, organizing persuasive techniques into coherent stories that justify actions and evoke ideological identification. There's a need for fine-grained narrative classification in biased news articles, particularly in the context of Indian media covering polarizing socio-political events.

Method: Created INDI-PROP dataset with 1,266 articles on CAA and Farmers' protest, annotated at three hierarchical levels: ideological article-bias, event-specific fine-grained narrative frames, and persuasive techniques. Proposed two GPT-4o-mini guided frameworks: FANTA (multi-layered communicative phenomena integration) and TPTC (systematic decomposition of persuasive cues via two-stage approach).

Result: The evaluation shows substantial improvement over underlying baselines for bias, narrative, and persuasive technique classification tasks using the proposed FANTA and TPTC frameworks.

Conclusion: The paper presents a comprehensive approach to propaganda analysis through fine-grained narrative classification, offering both a novel dataset (INDI-PROP) and effective reasoning frameworks (FANTA and TPTC) that advance the state of propaganda detection in news media.
...
<details>
  <summary>Details</summary>
Motivation: LLMs generate plausible but incorrect arguments (hallucinations), which is dangerous in high-stakes domains like clinical applications. Existing evaluation metrics fail to adequately assess factual consistency and lack interpretability.

Method: Decomposes text into atomic facts and introduces a flexible, schema-free methodology. Incorporates weighted metrics instead of absolute metrics, and proposes a mechanism to control assessment complexity in intricate domains.
...
Conclusion: The proposed interpretable framework addresses limitations of existing factual consistency assessment methods, particularly for high-stakes domains, by providing more nuanced evaluation with interpretability and complexity control.
...
TL;DR: First comprehensive study of GenAI adoption in Italy reveals widespread use for sensitive tasks, low digital literacy despite heavy reliance, and significant gender divide in adoption rates.


<details>
  <summary>Details</summary>
Motivation: To understand how generative AI chatbots are transforming digital interactions in Italy and assess risks of widening digital divides due to uneven adoption and low awareness of limitations.

Method: Conducted first comprehensive empirical mapping using newly collected survey data from 1,906 Italian-speaking adults, analyzing adoption patterns, usage behaviors, and digital literacy levels.

Result: Widespread adoption for work and personal use including sensitive tasks; GenAI becoming primary information source despite low user literacy; significant gender divide where women are half as likely to adopt and use GenAI less frequently; literacy predicts adoption but doesn't fully explain gender disparity.

Conclusion: Need for targeted educational initiatives and further investigation into barriers to equitable participation beyond competence alone, as GenAI adoption patterns reveal risks of misinformation and digital divides.
...
TL;DR: The paper introduces Hydro-SE Bench, a 4,000-question benchmark for evaluating LLMs in Hydro-Science and Engineering, revealing performance gaps in domain-specific knowledge despite decent performance in science-related areas.


<details>
  <summary>Details</summary>
Motivation: Hydro-SE is critical for water supply, clean energy, and disaster mitigation but requires interdisciplinary expertise and expert collaboration. While LLMs show potential in this domain, their knowledge and application abilities haven't been sufficiently evaluated.

Method: Created Hydro-SE Bench with 4,000 multiple-choice questions covering nine subfields, evaluating LLMs across three aspects: basic conceptual knowledge, engineering application ability, and reasoning/calculation ability.

Result: Commercial LLMs scored 0.74-0.80 accuracy, while small-parameter LLMs scored 0.41-0.68. LLMs perform well in natural/physical science subfields but struggle with domain-specific knowledge like industry standards and hydraulic structures. Model scaling mainly improves reasoning/calculation abilities.

Conclusion: LLMs have potential but need improvement in practical engineering applications. The benchmark provides clear training targets for developers and practical guidance for Hydro-SE researchers applying LLMs.
...
<details>
  <summary>Details</summary>
Motivation: To understand how grammatical knowledge is represented in LLMs, specifically whether different syntactic phenomena use shared or distinct components, and to investigate if syntactic agreement constitutes a meaningful functional category.

Method: Used functional localization approach inspired by cognitive neuroscience to identify LLM units responsive to 67 English syntactic phenomena across 7 open-weight models. Conducted cross-lingual analysis of 57 diverse languages and examined causal role of identified units.

Result: Different types of syntactic agreement (subject-verb, anaphor, determiner-noun) recruit overlapping sets of units. This pattern holds across English, Russian, and Chinese. Structurally similar languages share more units for subject-verb agreement.

Conclusion: Syntactic agreement constitutes a meaningful functional category within LLMs' representational spaces, revealing systematic organization of grammatical knowledge.
...
TL;DR: AITutor-EvalKit is a language technology application for evaluating AI tutors' pedagogical quality, providing demonstration software, model inspection, and data visualization tools.


<details>
  <summary>Details</summary>
Motivation: There is a need for systematic evaluation of AI tutors' pedagogical quality to ensure effective educational technology. Current tools lack comprehensive evaluation capabilities for education stakeholders and researchers.

Method: Developed an application using language technology that includes: 1) pedagogical quality evaluation of AI tutors, 2) demonstration and evaluation software, 3) model inspection tools, and 4) data visualization capabilities.

Result: Created AITutor-EvalKit, a comprehensive tool that supports learning, collects user feedback and annotations, and serves both education stakeholders and the ACL research community.

Conclusion: AITutor-EvalKit provides valuable infrastructure for evaluating AI tutors, facilitating research and practical applications in educational technology while enabling community feedback collection.
...
<details>
  <summary>Details</summary>
Motivation: Long-context dialogue systems suffer from State Inertia, where static constraints prevent models from resolving conflicts between evolving user intents and established historical context, limiting their ability to adapt to changing user needs.
...
Result: Achieves state-of-the-art win rates (86.2% on Phi-3.5) on Multi-Session Chat dataset with robust zero-shot generalization. Larger Qwen2.5-7B model achieves near-perfect alignment (99.4% win rate) with negligible perplexity overhead, revealing a "Capacity-Stability Trade-off".
...
TL;DR: AR-Med is a retrieval-augmented LLM framework for medical search that grounds reasoning in verified knowledge, uses knowledge distillation for efficiency, and achieves 93% accuracy with 24% improvement over existing systems.


<details>
  <summary>Details</summary>
Motivation: Traditional medical search methods fail to comprehend complex user queries, while LLMs face challenges like hallucinations, knowledge gaps, and high costs in healthcare applications.

Method: AR-Med uses retrieval-augmented LLM reasoning grounded in verified medical knowledge, knowledge distillation to compress large models into efficient student models, and LocalQSMed benchmark for evaluation.

Result: Achieves 93% offline accuracy (24% improvement over original system), significant gains in online relevance and user satisfaction, successfully deployed at scale on medical platforms.

Conclusion: AR-Med provides a practical, scalable blueprint for trustworthy LLM-powered healthcare systems, balancing accuracy, reliability, and operational efficiency.
...
TL;DR: ESPO is a new RL framework for diffusion LLMs that treats sequence generation as a single action using ELBO as a likelihood proxy, overcoming token-level RL limitations in autoregressive models.


<details>
  <summary>Details</summary>
Motivation: RL works well for autoregressive language models but fails for diffusion LLMs due to fundamental likelihood approximation challenges - diffusion models lack token-level conditional probabilities needed for token-level RL objectives.

Method: Proposes ELBO-based Sequence-level Policy Optimization (ESPO) that treats entire sequence generation as a single action, uses ELBO as tractable sequence-level likelihood proxy, with per-token normalization of importance ratios and robust KL-divergence estimation for stable training.

Result: ESPO significantly outperforms token-level baselines, achieving 20-40 point improvements on Countdown task, with consistent gains on math and coding benchmarks.

Conclusion: Establishes sequence-level optimization as a principled and effective paradigm for RL in diffusion LLMs, overcoming fundamental limitations of token-level approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM safety alignment operates at the surface level but may be vulnerable to attacks that manipulate internal representations. The researchers wanted to explore whether semantic meaning could be hijacked through in-context examples to bypass safety mechanisms.

Method: Systematically replace harmful keywords (e.g., "bomb") with benign tokens (e.g., "carrot") across multiple in-context examples preceding a harmful request. This causes the internal representation of the benign token to converge toward that of the harmful one, effectively embedding harmful semantics under euphemisms.

Result: The attack achieves 74% attack success rate on Llama-3.3-70B-Instruct with single-sentence context override. It's optimization-free, broadly transferable across model families, and works on both closed-source and open-source systems. Interpretability tools show semantic overwrite emerges layer by layer.

Conclusion: Current alignment strategies are insufficient and should operate at the representation level rather than just surface-level filtering. Doublespeak reveals a new attack surface in the latent space of LLMs that requires fundamental changes to safety approaches.
...
TL;DR: Adapts DoLa contrastive decoding to T5/FLAN-T5 encoder-decoder models, evaluating impact on instruction following rather than just factuality, with mixed results across task categories.


<details>
  <summary>Details</summary>
Motivation: Contrastive decoding methods like DoLa have only been implemented in decoder-only architectures and studied for factuality improvement. This work aims to adapt DoLa to encoder-decoder architectures (T5/FLAN-T5) and evaluate its impact on instruction following capabilities, which hasn't been explored before.

Method: Adapt DoLa (Decoding by Contrastive Layers) for T5 and FLAN-T5 model families, implement contrastive decoding in encoder-decoder architecture, and conduct layer-by-layer analysis of logit evolution to quantify DoLa's impact on token output probabilities.

Result: DoLa improves faithfulness of text generation for certain categories of tasks but harms others. The layer-by-layer analysis provides quantitative understanding of how DoLa affects token output probabilities in FLAN-T5 models.

Conclusion: This is the first implementation of contrastive decoding in encoder-decoder architecture. While DoLa shows mixed results across different task categories, the analysis provides insights into its impact on instruction following capabilities in T5/FLAN-T5 models.
...
TL;DR: LLM classification performance depends heavily on prompt wording, especially for precise psychological constructs. The study finds that persona, chain-of-thought, and explanatory prompting don't fix badly worded prompts - construct definition, task framing, and examples matter most. Best results come from combining codebook-guided prompt selection with automatic prompt engineering.


<details>
  <summary>Details</summary>
Motivation: LLMs show strong text classification but outputs depend heavily on prompt wording. Little research exists on prompt engineering for classification tasks in domains like psychology where constructs have precise, theory-driven definitions that may not be well represented in pre-training data.

Method: Empirical framework for optimizing LLM performance via prompt engineering. Evaluated five prompting strategies: codebook-guided empirical prompt selection, automatic prompt engineering, persona prompting, chain-of-thought reasoning, and explanatory prompting with zero-shot and few-shot classification across three constructs and two models.
...
Conclusion: Researchers should generate and evaluate many prompt variants (human-crafted and automatically generated), select prompts based on empirical performance in training data, and validate in holdout sets. This provides a practical, systematic, theory-driven method for optimizing LLM prompts when alignment with expert judgment is critical.
...
TL;DR: LLMs fine-tuned on medical consensus guidelines outperform larger models for early prediction, achieving near-perfect rule adherence but facing challenges with forecasting irregular clinical variables.


<details>
  <summary>Details</summary>
Motivation: Current ML models for medical prediction focus too much on accuracy while neglecting faithful explanations needed for clinical trust. Medical consensus guidelines provide structured reasoning frameworks that could improve both prediction and explainability.

Method: Fine-tune LLMs on verbalized medical inference rules from consensus guidelines (using Sepsis-3 as example). Use rule instantiations from EHR data to teach step-by-step reasoning. Evaluate using derivation correctness (logical deduction) and value correctness (prediction accuracy). Address forecasting challenges with multimodal integration of time series models.

Result: Small fine-tuned models outperform larger LLMs using one-shot learning and models trained on medical texts. Achieve near-perfect derivation correctness for rules and exceptions on unseen patient data. Forecasting irregular clinical variables remains challenging but improves with multimodal time series integration.

Conclusion: Fine-tuning LLMs on medical consensus guidelines enables faithful, explainable reasoning that builds clinical trust. The main bottleneck for early prediction is not rule generalization but forecasting irregular clinical variables, which can be addressed through multimodal approaches.
...
TL;DR: FusedKV reduces KV cache memory by 50% through cross-layer sharing where top-layer KV caches are fused from bottom and middle layers, outperforming standard Transformers in perplexity.


<details>
  <summary>Details</summary>
Motivation: Transformer decoders suffer from prohibitive KV cache memory at long sequences. Existing cross-layer sharing methods underperform compared to within-layer methods like GQA, so the authors investigate the root cause to develop better KV cache compression.

Method: Analyzed information flow of keys/values in top layers, finding values come mainly from bottom layers while keys use both bottom and middle layers. Proposed FusedKV: top-layer KV caches are learnable fusion of most informative bottom/middle layer caches, operating on post-RoPE keys to preserve positional info. Also created FusedKV-Lite: cross-layer sharing where top-layer KV caches directly use bottom-layer values and middle-layer keys.

Result: Achieved 50% reduction in cache memory while achieving lower validation perplexity than standard Transformer decoders across LLMs from 332M to 4B parameters. FusedKV-Lite reduces I/O overhead with slight perplexity increase compared to FusedKV.
...
TL;DR: Models trained on diverse linguistic data (standard + non-standard) outperform standard-only models across all tasks without sacrificing benchmark accuracy, highlighting the importance of linguistic diversity for inclusive language models.


<details>
  <summary>Details</summary>
Motivation: Current language models rely on filtered corpora that exclude non-standard linguistic varieties (dialectal, historical, informal), reducing model robustness and reinforcing representational biases. The paper argues that models should capture the full spectrum of language variation rather than just standardized text.

Method: Focusing on Basque (morphologically rich, low-resource language), the authors construct new corpora combining standard, social media, and historical sources. They pre-train the BERnaT family of encoder-only models in three configurations: standard, diverse, and combined. They propose an evaluation framework separating NLU tasks into standard and diverse subsets to assess linguistic generalization.
...
Conclusion: Linguistic diversity is crucial for building inclusive, generalizable language models. Incorporating diverse linguistic varieties (dialectal, historical, informal) enhances model robustness and reduces representational biases without sacrificing performance on standard tasks.
...
TL;DR: BRAND dataset reveals multilingual LLMs show religious bias, performing worse in Bengali than English and consistently displaying bias toward Islam even in religion-neutral questions.


<details>
  <summary>Details</summary>
Motivation: Sensitive subjects like religion present challenges for bias detection in LLMs, as minor errors can cause severe misunderstandings. Multilingual models often misrepresent religions and struggle with accuracy in religious contexts, particularly for South Asian religions.

Method: Created BRAND (Bilingual Religious Accountable Norm Dataset) focusing on four main South Asian religions (Buddhism, Christianity, Hinduism, Islam) with over 2,400 entries. Used three different prompt types in both English and Bengali to evaluate model performance.
...
Conclusion: The research highlights persistent religious bias in multilingual LLMs and connects findings to broader HCI issues regarding religion and spirituality, emphasizing the need for improved bias detection and mitigation in sensitive domains.
...
TL;DR: Two-stage adaptation of Qwen2.5-3B to Tibetan using Continual Pretraining for linguistic grounding and Supervised Fine-Tuning for task specialization, achieving significant perplexity reduction and translation quality improvements.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of adapting LLMs to low-resource languages like Tibetan, which suffers from data scarcity and cross-lingual drift, particularly for morphologically rich and underrepresented languages.

Method: Two-stage approach: 1) Continual Pretraining (CPT) to establish Tibetan linguistic grounding, 2) Supervised Fine-Tuning (SFT) for task and translation specialization. Layer-wise analysis across 435 layers in Qwen3-4B was conducted.
...
TL;DR: This paper introduces two complementary methods for tokenizer adaptation: continued BPE training for vocabulary extension and leaf-based pruning for vocabulary reduction, improving tokenization efficiency and vocabulary utilization.


<details>
  <summary>Details</summary>
Motivation: Current tokenizer adaptation methods for transferring pre-trained language models to new domains/languages often result in inefficient vocabulary extension (unreachable/unused tokens) and lack controlled vocabulary reduction methods.

Method: 1) Continued BPE training: Adapts pre-trained tokenizers by continuing the BPE merge learning process on new data. 2) Leaf-based vocabulary pruning: Removes redundant tokens while preserving model quality.
...
Conclusion: The proposed continued BPE training and leaf-based pruning offer effective complementary approaches for tokenizer adaptation, addressing both vocabulary extension and reduction needs, with open-source implementation available.
...
TL;DR: AugServe is an efficient inference framework that improves augmented LLM serving by using adaptive request scheduling and dynamic token batching to reduce queueing latency and increase effective throughput.


<details>
  <summary>Details</summary>
Motivation: Current augmented LLM inference systems suffer from two main problems: (1) FCFS scheduling causes head-of-line blocking leading to queuing delays exceeding SLOs, and (2) static batch token limits fail to adapt to fluctuating loads and hardware conditions, both degrading effective throughput and service quality.
...
Result: AugServe achieves 4.7-33.1x and 3.3-13.2x higher effective throughput than vLLM and InferCept respectively, while reducing time-to-first-token (TTFT) by up to 96.3% and 95.0%.

Conclusion: AugServe effectively addresses the limitations of existing augmented LLM inference systems by introducing adaptive scheduling and dynamic batching, significantly improving effective throughput and reducing latency for better user experience.
...
TL;DR: Jina-VLM is a 2.4B parameter vision-language model that achieves SOTA multilingual VQA performance among open 2B-scale models, using SigLIP2 vision encoder and Qwen3 language backbone with attention-pooling connector.


<details>
  <summary>Details</summary>
Motivation: To create an efficient yet powerful multilingual vision-language model that can handle arbitrary-resolution images while maintaining competitive performance in both vision-language and text-only tasks.

Method: Combines SigLIP2 vision encoder with Qwen3 language backbone using an attention-pooling connector that enables token-efficient processing of arbitrary-resolution images.

Result: Achieves state-of-the-art multilingual visual question answering performance among open 2B-scale VLMs, outperforming comparable models across standard VQA benchmarks and multilingual evaluations.

Conclusion: Jina-VLM demonstrates that efficient architecture design (2.4B parameters with attention-pooling connector) can achieve SOTA multilingual VQA performance while preserving competitive text-only capabilities.
...
TL;DR: SkillFactory is a method that uses self-generated "silver" training data during supervised fine-tuning to teach language models cognitive skills before reinforcement learning, enabling better generalization and skill use.


<details>
  <summary>Details</summary>
Motivation: The paper addresses how to teach language models cognitive skills (like verification, backtracking, retrying) that aren't present in base models, without relying on distillation from stronger models.

Method: SkillFactory generates "silver" training traces by rearranging the model's own samples to create SFT data that demonstrates cognitive skills. This SFT initialization primes the model to acquire skills during subsequent RL training.

Result: SkillFactory SFT helps models generalize to harder task variants post-RL, enables actual cognitive skill use, and makes RL-trained models more robust to out-of-domain regression compared to RL-trained base models.

Conclusion: Inductive biases learned through SkillFactory's self-generated SFT data before RL help models learn robust cognitive skill use, offering an effective alternative to distillation-based approaches.
...
TL;DR: A self-supervised symbolic auto-encoder for diagrams that encodes them into geometric primitives and relationships, using hierarchical reward modeling and stabilization mechanisms, achieving state-of-the-art performance on reconstruction, perception, and reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Symbolic computer vision offers interpretable understanding of diagrams through logical rules and structured representations, which requires different learning paradigms from pixel-based models. Current approaches need better methods for parsing diagrams into geometric primitives and maintaining consistency in their relationships.

Method: Proposes a self-supervised symbolic auto-encoder that encodes diagrams into structured primitives (points, lines, shapes) and their relationships, then decodes them through an executable engine. Uses Symbolic Hierarchical Process Reward Modeling with hierarchical step-level parsing rewards to enforce consistency constraints. Introduces stabilization mechanisms to balance exploration and exploitation in reinforcement learning for diagram reconstruction.
...
Conclusion: The proposed neuro-symbolic system effectively integrates neural network reasoning with symbolic interpretability through reasoning-grounded visual rewards, demonstrating superior performance across multiple diagram understanding tasks compared to existing approaches.
...
TL;DR: A unified framework using a "drainage node" to handle noisy labels, class ambiguity, and out-of-distribution samples by reallocating probability mass toward uncertainty while maintaining end-to-end differentiability.


<details>
  <summary>Details</summary>
Motivation: Modern deep learning faces challenges with noisy labels, class ambiguity, and the need to robustly reject out-of-distribution or corrupted samples. Existing methods struggle with instance-dependent and asymmetric label noise.

Method: Proposes adding a "drainage node" at the network output that reallocates probability mass toward uncertainty. This provides an escape route for ambiguous, anomalous, or noisy samples while preserving end-to-end training and differentiability.

Result: Achieves up to 9% accuracy improvement over existing approaches on CIFAR-10/100 with instance-dependent and asymmetric noise. Matches or surpasses state-of-the-art on real-world datasets (mini-WebVision, mini-ImageNet, Clothing-1M). Qualitative analysis shows drainage neuron absorbs corrupt/mislabeled data, leading to more stable decision boundaries.

Conclusion: The drainage formulation provides a unified solution for handling label noise and uncertainty, with applications extending beyond classification to web-scale dataset cleaning, semi-supervised learning, and open-set applications.
...
<details>
  <summary>Details</summary>
Motivation: Biometric facial recognition models suffer accuracy drops with real-world images due to poor quality, non-frontal poses, and occlusions. The paper investigates whether AI-driven restoration techniques can address these issues.

Method: Used a model-agnostic, large-scale forensic-evaluation pipeline to assess three restoration approaches: 3D reconstruction (NextFace), 2D frontalization (CFR-GAN), and feature enhancement (CodeFormer). Tested both naive and selective application strategies.
...
Conclusion: Targeted, selective application of AI-driven restoration techniques (specifically CFR-GAN combined with CodeFormer) can improve facial recognition accuracy for challenging real-world images, but careful implementation is crucial as naive approaches harm performance.
...
TL;DR: Flux4D is a simple, scalable framework for 4D reconstruction of large-scale dynamic scenes that directly predicts 3D Gaussians and motion dynamics in a fully unsupervised way, enabling efficient reconstruction within seconds.


<details>
  <summary>Details</summary>
Motivation: Existing differentiable rendering methods like NeRF and 3DGS have scalability limitations and require annotations for actor motion decoupling. Self-supervised methods still face constraints with per-scene optimization and hyperparameter sensitivity.

Method: Flux4D directly predicts 3D Gaussians and their motion dynamics using only photometric losses and "as static as possible" regularization. It learns decomposition from raw data without pre-trained models or foundational priors by training across many scenes.

Result: Flux4D enables efficient reconstruction within seconds, scales effectively to large datasets, and generalizes well to unseen environments including rare and unknown objects. It significantly outperforms existing methods on outdoor driving datasets.

Conclusion: Flux4D provides a simple, scalable solution for 4D reconstruction of dynamic scenes that eliminates the need for annotations, overcomes scalability limitations, and achieves strong generalization while maintaining efficiency.
...
TL;DR: Zero-shot object counting using multimodal LLMs (GPT-4o/GPT-5) with only textual prompts achieves performance comparable to or surpassing state-of-the-art methods on standard datasets.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot counting methods require annotated data and visual exemplars. LLMs have strong reasoning and understanding capabilities, suggesting they could perform counting without supervision using only textual prompts.
...
Result: Models achieve performance comparable to state-of-the-art zero-shot approaches on FSC-147 dataset, and in some cases even surpass them. Both GPT-4o and GPT-5 evaluated on FSC-147 and CARPK datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D shape datasets and models focus only on geometry, ignoring material properties that determine object appearance. There's a need to bridge geometric reasoning with material understanding.

Method: Two-stage LLM approach: 1) Predict object semantics from 3D point clouds with coarse segmentations, 2) Assign plausible materials to each geometric segment conditioned on inferred semantics. Both stages operate zero-shot without task-specific training.
...
Conclusion: Language models can serve as general-purpose priors for bridging geometric reasoning and material understanding in 3D data, demonstrating effective zero-shot material inference from geometry.
...
TL;DR: Single-image noise synthesis method using one noisy image and one dark frame per ISO setting to generate realistic noise for training denoisers without needing large paired datasets.


<details>
  <summary>Details</summary>
Motivation: Learning-based denoisers need large paired datasets of clean and noisy images, which are difficult to collect. Existing methods rely on simplified parametric models or large datasets, making noise synthesis a practical alternative.

Method: Proposes a general noise synthesis method using one noisy image and one dark frame per ISO setting. Uses Poisson distribution for signal-dependent noise and introduces Fourier-domain spectral sampling algorithm to model signal-independent noise, generating diverse noise realizations that maintain spatial and statistical properties of real sensor noise.

Result: The synthesis method is accurate and practical, leading to state-of-the-art performances on multiple low-light denoising benchmarks.

Conclusion: The proposed method provides a practical solution for noise synthesis without relying on simplified parametric models or large clean-noisy image pairs, enabling effective training of learning-based denoisers for low-light conditions.
...
TL;DR: PixPerfect is a pixel-level refinement framework that fixes chromatic shifts, texture mismatches, and visible seams in LDM-based image editing by using a differentiable discriminative pixel space, artifact simulation pipeline, and direct pixel-space refinement.


<details>
  <summary>Details</summary>
Motivation: Latent Diffusion Models (LDMs) for image inpainting and local editing often introduce pixel-level inconsistencies like chromatic shifts, texture mismatches, and visible seams. Existing solutions fail to fully eliminate these artifacts and don't generalize well across different latent representations or tasks.

Method: PixPerfect uses three key components: 1) A differentiable discriminative pixel space that amplifies and suppresses subtle color/texture discrepancies, 2) A comprehensive artifact simulation pipeline that exposes the refiner to realistic local editing artifacts during training, and 3) A direct pixel-space refinement scheme ensuring broad applicability across diverse latent representations and tasks.
...
Conclusion: PixPerfect delivers seamless, high-fidelity local edits across diverse LDM architectures and tasks, effectively addressing the pixel-level inconsistencies that plague current LDM-based editing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Increasing wildfire frequency/severity requires low-latency, computationally efficient onboard detection methods for airborne/spaceborne missions. High-dimensional thermal imagery presents challenges for real-time processing with limited onboard resources.

Method: PyroFocus: Two-stage pipeline with fire classification (multi-class: no fire, active fire, post-fire) followed by fire radiative power (FRP) regression/segmentation. Evaluated custom CNNs and Transformer-based models on NASA MASTER data (similar to next-gen fire sensors).

Result: Two-stage pipeline achieves strong trade-offs between speed and accuracy, reducing inference time and computational cost while maintaining performance suitable for real-time edge deployment.

Conclusion: PyroFocus demonstrates significant potential for real-time wildfire monitoring in future missions, offering efficient onboard processing of thermal imagery for emergency response and environmental management.
...
TL;DR: H²U3D is a new 3D visual question answering dataset for house-scale scenes with multi-floor environments, and SpatialReasoner is an active perception framework that uses coarse-to-fine exploration to answer spatial queries efficiently.
...
Method: 1) Created H²U3D dataset with automated annotation pipeline generating hierarchical coarse-to-fine visual representations and diverse QA pairs with chain-of-thought annotations. 2) Proposed SpatialReasoner framework that autonomously invokes spatial tools to explore 3D scenes based on queries, trained via two-stage strategy: supervised cold start followed by reinforcement learning with adaptive exploration reward.

Result: SpatialReasoner achieves state-of-the-art performance on H²U3D, outperforming strong baselines including GPT-4o and Gemini-2.5-Pro. Notably, it uses only 3-4 images on average compared to baselines requiring 16+ images, demonstrating efficient exploration.

Conclusion: The proposed coarse-to-fine active exploration paradigm enables effective spatial reasoning in large-scale 3D environments while significantly reducing the number of observations needed, advancing house-scale scene understanding capabilities.
...
TL;DR: NavMapFusion: A diffusion-based framework that fuses coarse navigation maps with high-fidelity sensor data for online HD map construction in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: HD maps are essential for autonomous driving but need to be constructed online due to constantly changing environments. While navigation-grade SD maps are widely available, they lack sufficient resolution for direct use but can serve as coarse priors to guide online map construction.
...
Conclusion: Diffusion-based map construction provides a robust framework for map fusion, where consistent regions reinforce map construction while outdated segments are suppressed, generating accurate and up-to-date environment representations for safer autonomous driving.
...
TL;DR: SLEDGE introduces step-by-step layered design generation, modeling design updates as atomic changes grounded in sequential instructions, with a new evaluation benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat design synthesis as single-step generation, underestimating the complexity of the creative process where designers progressively refine work through careful modifications.

Method: Proposes SLEDGE (Step-by-step LayEred Design GEnerator) that models each design update as an atomic, layered change over previous state, grounded in sequential instructions, leveraging multi-modal LLMs.

Result: Exhaustive experimental analysis and comparison with state-of-the-art approaches tailored to the new setup demonstrate the efficacy of the proposed approach.

Conclusion: The work introduces a pragmatic and under-explored research area of step-by-step layered design generation, hoping to attract more attention to this important problem.
...
TL;DR: ProtoEFNet is a video-based prototype learning model for continuous ejection fraction regression that learns interpretable cardiac motion patterns while maintaining accuracy comparable to black-box models.


<details>
  <summary>Details</summary>
Motivation: Traditional EF estimation requires manual tracing and expertise, is time-consuming, and suffers from interobserver variability. Current deep learning methods are black-box models with limited transparency, reducing clinical trust. Post-hoc explainability methods don't guide internal reasoning and offer limited reliability.

Method: ProtoEFNet uses prototype learning with dynamic spatiotemporal prototypes that capture clinically meaningful cardiac motion patterns. It introduces Prototype Angular Separation (PAS) loss to enforce discriminative representations across the continuous EF spectrum.

Result: ProtoEFNet achieves accuracy on par with non-interpretable counterparts on the EchonetDynamic dataset. The PAS loss boosts performance with a 2% increase in F1 score from 77.67±2.68 to 79.64±2.10.

Conclusion: ProtoEFNet provides both accurate EF prediction and clinically relevant interpretability through learned cardiac motion prototypes, addressing the transparency limitations of current deep learning methods for cardiac function assessment.
...
TL;DR: HalluGen is a diffusion-based framework that generates realistic hallucinations in medical images, enabling systematic evaluation of hallucination detection and mitigation in safety-critical image restoration tasks.


<details>
  <summary>Details</summary>
Motivation: Generative models in image restoration produce hallucinations (plausible but incorrect structures) that are particularly dangerous in safety-critical domains like medical imaging, where they can cause diagnostic errors. Current progress is hindered by the circular dependency: evaluating hallucinations requires labeled data, but such labels are costly and subjective to obtain.

Method: HalluGen is a diffusion-based framework that synthesizes realistic hallucinations with controllable type, location, and severity. It produces perceptually realistic but semantically incorrect outputs (e.g., segmentation IoU drops from 0.86 to 0.36). The authors used HalluGen to create the first large-scale hallucination dataset with 4,350 annotated images from 1,450 brain MR images for low-field MRI enhancement.

Result: The framework enabled two key applications: (1) benchmarking image quality metrics and developing SHAFE (Semantic Hallucination Assessment via Feature Evaluation), a feature-based metric with soft-attention pooling that improves hallucination sensitivity over traditional metrics; and (2) training reference-free hallucination detectors that generalize to real restoration failures.

Conclusion: HalluGen and its open dataset establish the first scalable foundation for evaluating hallucinations in safety-critical image restoration, addressing the critical need for systematic hallucination assessment in domains like medical imaging, industrial inspection, and remote sensing.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning architectures have suboptimal inductive biases for detecting weak, spatially distributed anomalies in volumetric medical imaging. 2D/3D CNNs impose excessive locality while ViTs use unconstrained global attention, both failing to capture the optimal spatial integration needed for sparse volumetric pattern recognition.

Method: Controlled comparison of 16 modern deep learning architectures (2D/3D CNNs, hybrid models, volumetric transformers) for subclinical keratoconus detection from 3D anterior segment OCT volumes. Includes mechanistic analyses using attention-distance measurements, representational similarity, and auxiliary age/sex prediction tasks.

Result: Hierarchical attention models achieve superior performance and parameter efficiency, with 21-23% higher sensitivity and specificity in the sparse anomaly regime. They produce effective receptive fields matched to the intermediate, multi-slice extent of subclinical abnormalities.

Conclusion: Hierarchical attention provides a principled and effective approach for early pathological change analysis in 3D medical imaging, offering design guidance for future volumetric anomaly detection systems by aligning spatial integration length with signal strength requirements.
...
TL;DR: SeeU is a 2D→4D→2D framework that learns continuous 4D world dynamics from sparse 2D observations to generate unseen visual content with physical consistency.


<details>
  <summary>Details</summary>
Motivation: Current visual understanding, prediction, and generation methods operate directly on 2D observations, which are discrete projections of the 4D world (3D space + time), leading to suboptimal performance. There's a need to model the underlying continuous 4D dynamics for better visual generation.

Method: Three-stage framework: 1) 2D→4D: Reconstruct 4D world from sparse monocular 2D frames; 2) Discrete 4D→Continuous 4D: Learn continuous 4D dynamics using low-rank representation and physical constraints; 3) 4D→2D: Roll world forward in time, re-project to 2D at sampled times/viewpoints, and generate unseen regions using spatial-temporal context awareness.
...
Conclusion: By modeling dynamics in the continuous 4D world rather than directly on 2D observations, SeeU enables more accurate and physically-consistent visual generation across various applications.
...
TL;DR: Deep learning system for lung cancer classification from CT scans using DenseNet169 with attention mechanisms and SVM with MobileNetV2 features, achieving 98% accuracy with interpretability tools.


<details>
  <summary>Details</summary>
Motivation: Lung cancer is highly deadly worldwide, and early diagnosis is crucial for patient survival. Manual interpretation of CT scans is time-consuming and error-prone, necessitating automated systems for improved accuracy and efficiency.

Method: Used IQOTHNCCD lung cancer dataset (Normal, Benign, Malignant categories). Implemented DenseNet169 with Squeeze-and-Excitation blocks for attention-based feature extraction, Focal Loss for class imbalance handling, and Feature Pyramid Network for multi-scale feature fusion. Also developed SVM model using MobileNetV2 for feature extraction. Integrated Grad-CAM for visualization of decision regions and SHAP for feature contribution explanations.

Result: Both DenseNet169 and SVM models achieved 98% accuracy, demonstrating robust performance suitable for real-world medical practice.

Conclusion: The proposed deep learning system shows potential to improve lung cancer diagnosis with high accuracy, transparency, and robustness, offering a promising automated solution for medical practice.
...
TL;DR: FireSentry introduces a high-resolution wildfire dataset with sub-meter spatial and sub-second temporal resolution, plus FiReDiff, a dual-modality model that first predicts infrared video sequences then segments fire masks, achieving state-of-the-art performance.
...
Method: 1) Created FireSentry dataset using synchronized UAV platforms collecting visible/infrared video, environmental measurements, and validated fire masks. 2) Developed FiReDiff paradigm that first predicts future infrared video sequences, then segments fire masks based on generated dynamics.

Result: FiReDiff achieves significant improvements: video quality gains of 39.2% PSNR, 36.1% SSIM, 50.0% LPIPS, 29.4% FVD; mask accuracy gains of 3.3% AUPRC, 59.1% F1, 42.9% IoU, 62.5% MSE compared to existing approaches.

Conclusion: FireSentry benchmark dataset and FiReDiff paradigm collectively advance fine-grained wildfire forecasting and dynamic disaster simulation, with the dataset publicly available for research.
...
TL;DR: ShelfGaussian is an open-vocabulary multi-modal Gaussian-based 3D scene understanding framework that uses vision foundation models for supervision, achieving state-of-the-art zero-shot semantic occupancy prediction.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-based methods have limitations: closed-set semantic Gaussians require annotated 3D labels and neglect rendering ability, while open-set Gaussian representations using 2D self-supervision lead to degraded geometry and are limited to camera-only settings. There's a need to fully exploit Gaussian potential for multi-modal 3D scene understanding.

Method: Proposes a Multi-Modal Gaussian Transformer that enables Gaussians to query features from diverse sensor modalities, and a Shelf-Supervised Learning Paradigm that efficiently optimizes Gaussians with vision foundation model features jointly at both 2D image and 3D scene levels.

Result: Achieves state-of-the-art zero-shot semantic occupancy prediction performance on Occ3D-nuScenes benchmark. Successfully evaluated on an unmanned ground vehicle (UGV) across diverse urban scenarios, demonstrating strong in-the-wild performance.

Conclusion: ShelfGaussian effectively bridges the gap between closed-set and open-set Gaussian representations by leveraging vision foundation models for multi-modal 3D scene understanding, showing promising results for both perception and planning tasks in real-world applications.
...
TL;DR: MOS is a novel framework for cross-modal ship re-identification between optical and SAR imagery that addresses the modality gap through modality-consistent representation learning and cross-modal data generation with feature fusion.


<details>
  <summary>Details</summary>
Motivation: Cross-modal ship ReID between optical and SAR imagery is critical for maritime intelligence but underexplored. The substantial modality gap between optical and SAR images poses a major challenge for robust identification.

Method: MOS consists of two core components: 1) Modality-Consistent Representation Learning (MCRL) with denoise SAR image processing and class-wise modality alignment loss, and 2) Cross-modal Data Generation and Feature fusion (CDGF) using a brownian bridge diffusion model to synthesize cross-modal samples for feature fusion during inference.

Result: Extensive experiments on HOSS ReID dataset show MOS significantly surpasses state-of-the-art methods, achieving improvements of +3.0%, +6.2%, and +16.4% in R1 accuracy under ALL to ALL, Optical to SAR, and SAR to Optical settings respectively.

Conclusion: MOS effectively mitigates the optical-SAR modality gap and achieves modality-consistent feature learning for cross-modal ship ReID, demonstrating superior performance over existing methods across all evaluation protocols.
...
Method: Introduces ViDiC-1K dataset with 1,000 curated video pairs annotated with over 4,000 comparative checklist items across seven categories. Proposes dual-checklist framework using LLM-as-a-Judge protocol to separately measure similarity and difference accuracy.

Result: Experiments on nineteen multimodal models reveal significant performance gap in comparative description and difference perception abilities, establishing ViDiC-1K as a challenging benchmark.

Conclusion: ViDiC-1K provides a solid foundation for advancing video understanding, edit awareness, and comparative reasoning in multimodal intelligence, addressing current limitations in dynamic scene analysis.
...
Result: Achieves state-of-the-art accuracy (52.8 mAP on ADG-Det, 73.1 mAP on IIT-Heat) with real-time performance (up to 89.77 FPS, up to 846.24 FPS for lightweight variant).

Conclusion: YOLOA successfully addresses the "what-where-how" challenge in affordance detection by jointly handling object detection and affordance learning, achieving an excellent trade-off between accuracy and efficiency.
...
TL;DR: DM3D introduces a deformable Mamba architecture with adaptive serialization for point clouds, overcoming SSMs' reliance on input order through offset-guided Gaussian sequencing and achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: State Space Models (SSMs) show promise for long-sequence modeling but struggle with point clouds due to their irregular nature and reliance on input order. Existing methods use fixed serialization strategies that can't adapt to diverse geometric structures.
...
Conclusion: Adaptive serialization effectively unlocks SSMs' potential for point cloud understanding, with DM3D's structure-adaptive approach demonstrating superior performance across multiple tasks.
...
Method: Zero-shot evaluation of eight state-of-the-art stereo methods (RAFT-Stereo, IGEV, IGEV++, BridgeDepth, StereoAnywhere, DEFOM, ACVNet, PSMNet, TCstereo) trained exclusively on Scene Flow and evaluated without fine-tuning on four standard benchmarks plus a novel 5,313-pair Canterbury forestry dataset captured with ZED Mini camera.

Result: Foundation models excel on structured scenes (BridgeDepth: 0.23 px on ETH3D, 0.83-1.07 px on KITTI; DEFOM: 0.35-4.65 px across benchmarks), while iterative methods maintain cross-domain robustness (IGEV++: 0.36-6.77 px; IGEV: 0.33-21.91 px). RAFT-Stereo shows catastrophic ETH3D failure (26.23 px EPE, 98% error rate) but normal KITTI performance (0.90-1.11 px). DEFOM identified as optimal for forestry with superior depth smoothness, occlusion handling, and cross-domain consistency.

Conclusion: Stereo method performance is highly scene-dependent, with foundation models best for structured scenes and iterative methods offering better cross-domain robustness. DEFOM emerges as the gold-standard baseline for vegetation depth estimation in forestry applications despite IGEV++'s finer detail preservation.
...
TL;DR: A label-efficient hyperspectral image classification framework using frozen diffusion model features with spectral-spatial fusion.
...
Method: Extracts low-level spatial features from frozen diffusion model's decoder layers at early denoising timesteps, then uses lightweight FiLM-based fusion module to adaptively combine these spatial features with spectral cues for multimodal learning under sparse supervision.
...
Conclusion: Pretrained diffusion models can support domain-agnostic, label-efficient representation learning for remote sensing and broader scientific imaging tasks.
...
Method: Proposes MAGEN (Multi-Agent data GENeration) for enhancing data quality through foundation model-assisted captioning and retrieval-based verification, and O-MAKE (Ontology-based Multi-Aspect Knowledge-Enhanced) pretraining that decomposes long texts into knowledge aspects for fine-grained alignment and ontology-guided concept modeling.
...
Conclusion: The proposed framework effectively addresses data quality and text complexity challenges in medical VLP, demonstrating superior performance in dermatology applications with publicly released code and dataset.
...
TL;DR: LM-CartSeg: Automatic pipeline for knee cartilage/bone segmentation, lateral/medial compartmentalization, and radiomics analysis using nnU-Net models with geometric post-processing.


<details>
  <summary>Details</summary>
Motivation: Existing knee MRI radiomics work relies on manual ROIs with poor quality control, needing robust automatic segmentation that captures both cartilage and subchondral bone for meaningful analysis.

Method: Two 3D nnU-Net models trained on different datasets, with zero-shot predictions fused and refined using geometric rules: connected-component cleaning, 10mm subchondral bone bands, and PCA/k-means for tibial lateral/medial split.

Result: Post-processing improved segmentation accuracy (ASSD: 2.63→0.36mm, HD95: 25.2→3.35mm, DSC: 0.91), geometric L/M rule produced stable compartments, and radiomics features showed discriminative information beyond simple morphometry.

Conclusion: LM-CartSeg provides automatic, quality-controlled ROIs and radiomic features for multi-center knee OA studies, offering discriminative information beyond basic morphological measurements.
...
TL;DR: Unsupervised framework learns spatially structured 3D keypoints from point clouds to condition diffusion models for shape reconstruction, achieving better consistency than prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised keypoint methods aren't designed for unconditional generative settings, limiting their use in modern 3D generative pipelines. The paper aims to bridge this gap by learning interpretable keypoints that can condition generative models.

Method: Unsupervised framework learns spatially structured 3D keypoints from point cloud data. These keypoints serve as compact representation to condition an Elucidated Diffusion Model (EDM) for full shape reconstruction.

Result: Learned keypoints exhibit repeatable spatial structure across object instances and support smooth interpolation in keypoint space, indicating they capture geometric variation. Achieves 6 percentage-point improvement in keypoint consistency compared to prior approaches across diverse object categories.

Conclusion: The method successfully bridges the gap between unsupervised keypoint learning and modern 3D generative pipelines, providing interpretable structured representations that effectively condition diffusion models for shape reconstruction.
...
TL;DR: GalaxyDiT is a training-free method that accelerates video diffusion models by 1.87-2.37× with minimal quality loss (0.72-0.97% drop) through guidance alignment and systematic proxy selection for computational reuse.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models (DiTs with CFG) require intensive computation - dozens of iterative steps per video generation, with CFG doubling compute requirements. This inefficiency hinders broader adoption in downstream applications.

Method: GalaxyDiT uses training-free guidance alignment and systematic proxy selection for reuse metrics. Through rank-order correlation analysis, it identifies optimal proxies for each video model across different model families and parameter scales to ensure optimal computational reuse.
...
Conclusion: GalaxyDiT provides an effective training-free acceleration method for video diffusion models that significantly reduces computational requirements while maintaining high quality, enabling broader adoption in practical applications.
...
TL;DR: Adding geometric regularization via depth prediction improves 3D consistency in video generation with diffusion models.
...
Method: Augment latent diffusion models with per-frame depth prediction and enforce structural consistency using multi-view geometric loss that aligns depth maps across frames in shared 3D coordinate system.
...
Conclusion: Bridging appearance generation with 3D structure modeling through geometric regularization addresses key limitations in video generation, leading to more realistic and consistent video synthesis.
...
Method: The framework includes: 1) Spatial-Aware World Model (SA-WM) that distills current scenes into command-aware latent states and rolls out future latent states for forward-looking reasoning, and 2) Hypergraph-guided decoder that hierarchically fuses these states with multimodal input to capture higher-order spatial dependencies. Also introduces DrivePilot dataset created using RAG and CoT-prompted LLM pipeline.

Result: ThinkDeeper achieves #1 ranking on Talk2Car leaderboard and surpasses state-of-the-art baselines on DrivePilot, MoCAD, and RefCOCO/+/g benchmarks. Shows strong robustness in challenging scenes (long-text, multi-agent, ambiguity) and maintains superior performance even when trained on only 50% of data.

Conclusion: The proposed ThinkDeeper framework successfully addresses visual grounding challenges in autonomous driving by incorporating future spatial reasoning through world models, demonstrating significant performance improvements and robustness across diverse driving scenarios and datasets.
...
TL;DR: Text-Printed Image (TPI) method generates synthetic images by rendering text on white canvas for low-cost text-centric training of vision-language models without real images.


<details>
  <summary>Details</summary>
Motivation: Task-specific fine-tuning of large vision-language models requires costly image-text pairs. Text is more available, editable, and scalable than images, but raw text training suffers from modality gap between text and images.

Method: Propose Text-Printed Image (TPI) - generating synthetic images by directly rendering textual descriptions on plain white canvas. This simple rendering projects text into image modality and preserves text semantics better than text-to-image models.
...
Conclusion: TPI demonstrates significant potential of text-centric training and charts path toward fully automated data generation for large vision-language models using scalable text data.
...
TL;DR: Proposed Basis Decomposition Module (BDM) and derived modules for infrared small target detection, achieving SOTA performance on both single-frame and multi-frame tasks.


<details>
  <summary>Details</summary>
Motivation: Infrared small target detection faces challenges due to lack of target texture and severe background clutter, where backgrounds obscure targets.

Method: Developed Basis Decomposition Module (BDM) that decomposes features into basis features to enhance targets and suppress backgrounds. Extended to Spatial Difference Decomposition Module (SD²M), Spatial Difference Decomposition Downsampling Module (SD³M), and Temporal Difference Decomposition Module (TD²M). Built SD²Net for single-frame detection using U-shaped architecture, and STD²Net for multi-frame detection by incorporating TD²M for motion information.

Result: Achieved state-of-the-art performance on both SISTD and MISTD datasets. SD²Net performed well on single-frame tasks, while STD²Net achieved 87.68% mIoU on multi-frame datasets, significantly outperforming SD²Net's 64.97% mIoU.

Conclusion: The proposed basis decomposition approach effectively addresses infrared small target detection challenges by enhancing targets and suppressing backgrounds, with the temporal extension further improving multi-frame detection performance.
...
TL;DR: Action Effect Modeling (AEM) framework jointly analyzes action execution and outcomes for mistake detection, achieving SOTA on EgoPER and CaptainCook4D benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing approaches focus only on how actions are performed, ignoring what they produce (action effects). Many errors manifest in outcomes rather than execution, such as unintended object states or incorrect spatial arrangements.

Method: AEM uses probabilistic formulation to jointly capture execution and outcomes. It identifies action outcomes by selecting most informative effect frames based on semantic relevance and visual quality. Extracts complementary cues from visual grounding and symbolic scene graphs, aligning them in shared latent space. Also designs prompt-based detector with task-specific prompts aligning action segments with intended execution semantics.
...
Conclusion: Modeling both execution and outcome yields more reliable mistake detection. Effect-aware representations have potential to benefit broader range of downstream applications.
...
TL;DR: Fairness-aware Low-Rank Adaptation (LoRA) for medical VLMs reduces diagnostic accuracy disparities across demographic groups by 69% while maintaining overall accuracy with only 0.24% trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Vision-language models show expert-level medical imaging performance but exhibit significant diagnostic accuracy disparities across demographic groups, creating fairness concerns in healthcare AI applications.

Method: Introduces fairness-aware Low-Rank Adaptation with three approaches: FR-LoRA (MaxAccGap regularization), GR-LoRA (inverse frequency gradient weighting), and Hybrid-LoRA (combination). Uses differentiable MaxAccGap loss for end-to-end optimization of accuracy parity.
...
Conclusion: The approach enables practical deployment of fair medical AI in resource-constrained settings with only 0.24% trainable parameters, addressing fairness concerns while maintaining diagnostic performance.
...
TL;DR: The paper introduces Object-IVQA, a new benchmark for evaluating object-centric reasoning in instructional videos, and proposes an agent framework that outperforms existing vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing action-centric methods fail to handle the flexibility of real-world procedures where step order depends on object states. The authors argue for shifting to an object-centric paradigm where actions are viewed as mechanisms driving state transitions.

Method: The authors create Object-IVQA, a benchmark with 107 instructional videos and 514 open-ended QA pairs with temporal evidence. They also propose an agent framework that coordinates object-centric planning, perception, analysis, and generation tools for explicit evidence retrieval and multi-hop reasoning.

Result: Experiments show existing large vision-language models struggle with object-level recognition and reasoning, while the proposed framework achieves substantial improvements on the new benchmark.

Conclusion: The work demonstrates the importance of object-centric reasoning for understanding procedural activities and provides both a benchmark and framework to advance this research direction.
...
TL;DR: NAS-LoRA: A parameter-efficient fine-tuning method that integrates neural architecture search with LoRA to enhance SAM's adaptation to specialized domains while reducing training costs.


<details>
  <summary>Details</summary>
Motivation: SAM lacks spatial priors in its Transformer encoder, hindering acquisition of high-level semantic information for specialized domains like medical and agricultural imaging. Existing LoRA methods don't incorporate inductive bias effectively.

Method: Proposes NAS-LoRA that adds a lightweight Neural Architecture Search block between encoder and decoder components of LoRA to dynamically optimize prior knowledge integration. Uses stage-wise optimization strategy to help ViT encoder balance weight updates and architectural adjustments.
...
Conclusion: NAS-LoRA effectively bridges semantic gap between pre-trained SAM and specialized domains, highlighting the potential of NAS in enhancing PEFT for visual foundation models.
...
TL;DR: EEA is a novel video agent framework that balances exploration-exploitation for long-form video understanding using semantic guidance and hierarchical tree search, achieving superior performance with computational efficiency.
...
Conclusion: EEA effectively addresses the exploration-exploitation balance problem in long-form video understanding through semantic-guided hierarchical search, offering both improved performance and computational efficiency.
...
Method: Proposes Domain-aware Prompt-driven Masked Transformer (DPMFormer) with three components: 1) domain-aware prompt learning for semantic alignment, 2) domain-aware contrastive learning with texture perturbation to diversify observable domains, and 3) domain-robust consistency learning to minimize prediction discrepancies between original and augmented images.

Result: Demonstrates superiority and establishes new state-of-the-art on various DGSS benchmarks.

Conclusion: DPMFormer effectively addresses semantic misalignment in domain generalized semantic segmentation through domain-aware prompt learning and contrastive learning, achieving robust performance across diverse domains.
...
TL;DR: Preliminary investigation of automated dance movement analysis using YOLOv8/v11 for detection and Segment Anything Model (SAM) for segmentation, enabling tracking and quantification of dance movements without specialized equipment.


<details>
  <summary>Details</summary>
Motivation: To develop an automated framework for analyzing dance movements using computer vision techniques without requiring specialized equipment or markers, enabling quantitative assessment of dance performance metrics.

Method: Integrated YOLOv8 and v11 for dancer detection with Segment Anything Model (SAM) for precise segmentation. The framework identifies dancers, counts dance steps, calculates spatial coverage patterns, and measures rhythm consistency from video recordings.

Result: Achieved 94% detection precision and 89% recall on a 49-second Ghanaian AfroBeats dance video. SAM segmentation achieved 83% intersection-over-union. Primary dancer executed 23% more steps with 37% higher motion intensity and used 42% more performance space than secondary dancers.

Conclusion: The framework demonstrates technical feasibility for automated dance analysis but is an early-stage investigation with limitations including single-video validation and lack of systematic ground truth. It establishes foundation for future systematic validation studies.
...
<details>
  <summary>Details</summary>
Motivation: Crowdsourcing enables scalable autonomous driving map construction, but low-cost sensor noise limits quality improvement with increased data volume. Current methods struggle to maintain consistent quality growth as more crowdsourced data becomes available.

Method: For semantic mapping: Train latent diffusion model on HD maps to learn generative prior of real-world map structure, incorporate via constrained MAP optimization in latent space. For topological mapping: Apply confidence-weighted k-medoids clustering and kinematic refinement to trajectories.

Result: Achieves state-of-the-art semantic and topological mapping performance on nuScenes, Argoverse 2, and a large proprietary dataset. Quality consistently increases with more crowdsourced data, with thorough ablation and scalability studies.

Conclusion: CSMapping successfully addresses the challenge of maintaining quality improvement with increasing crowdsourced data, providing robust semantic and topological mapping solutions for autonomous driving applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for text-driven human motion generation use chunk-by-chunk or auto-regressive approaches with diffusion heads, which may not be optimal for streaming scenarios with time-varying text prompts. The authors aim to create a framework that generates seamless, text-aligned motion sequences with real-time latency.
...
Result: Achieves state-of-the-art performance on streaming motion generation, reaching an FID of 0.057 on the HumanML3D benchmark. The framework generates text-aligned, seamless motion sequences with real-time latency.

Conclusion: FloodDiffusion demonstrates for the first time that diffusion forcing-based frameworks can achieve state-of-the-art performance on streaming motion generation tasks when properly tailored with the three proposed improvements.
...
TL;DR: OpenTrack3D: A mesh-free framework for open-vocabulary 3D instance segmentation that uses online visual-spatial tracking and MLLMs instead of CLIP for better compositional reasoning.
...
TL;DR: PRIS is a framework that adaptively revises prompts during inference by analyzing failure patterns in generated visuals and redesigning prompts accordingly, achieving better alignment than simply scaling visual generation processes.


<details>
  <summary>Details</summary>
Motivation: Current text-to-visual generation approaches often fail to achieve precise alignment between user intent and generated visuals. While prior methods scale visual generation (more sampling steps/seeds), they hit quality plateaus because prompts remain fixed during inference.

Method: PRIS (Prompt Redesign for Inference-time Scaling) adaptively revises prompts during inference by: 1) Reviewing generated visuals, 2) Identifying recurring failure patterns across visuals, 3) Redesigning prompts accordingly before regenerating visuals. Introduces element-level factual correction verifier for fine-grained alignment assessment between prompt attributes and visuals.
...
Conclusion: Adaptive prompt revision during inference (joint scaling of prompts and visuals) is crucial for overcoming quality plateaus in text-to-visual generation and achieving better alignment between user intent and generated content.
...
Method: Three key components: (1) Step-wise Regional Control (SRC) aligns textual steps with image regions in single denoising process; (2) Flexible RoPE step-aware positional encoding enhances temporal coherence and spatial diversity; (3) Cross-Step Consistency Control (CSCC) maintains ingredient consistency across steps.
...
TL;DR: CartoMapQA is a new benchmark for evaluating Visual-Language Models' understanding of cartographic maps through QA tasks, revealing persistent challenges in map interpretation despite VLMs' general multimodal capabilities.


<details>
  <summary>Details</summary>
Motivation: While Visual-Language Models have advanced multimodal understanding, their ability to interpret cartographic maps remains largely unexplored, creating a gap in evaluating models for real-world applications like navigation and urban planning that require robust map understanding.
...
Result: Evaluation of both open-source and proprietary LVLMs reveals persistent challenges: models struggle with map-specific semantics, exhibit limited geospatial reasoning, and are prone to OCR-related errors. The benchmark successfully isolates these weaknesses.

Conclusion: CartoMapQA provides a valuable tool for guiding future improvements in LVLM architectures, supporting development of models better equipped for real-world applications that depend on robust map understanding. The dataset and code are publicly available.
...
Method: V-ITI framework with two components: 1) Visual Neglect Detector using head-level discriminative probes to identify when models neglect visual input, and 2) Visual Recall Intervenor that modulates activations with prestored visual information only when neglect is detected.
...
Conclusion: The paper introduces a novel approach to address MLLM hallucinations by focusing on "when to intervene" rather than just "how to intervene," solving the over-intervention problem through selective activation-based intervention.
...
TL;DR: Hybrid content moderation framework for livestreaming combines supervised classification for known violations with similarity matching for novel cases, using multimodal inputs and MLLM distillation to achieve scalable detection of unwanted content.


<details>
  <summary>Details</summary>
Motivation: Content moderation is critical but challenging for large-scale video platforms, especially in livestreaming where moderation must be timely, multimodal, and robust to evolving forms of unwanted content that evade traditional approaches.

Method: Hybrid framework combining: 1) supervised classification for known violations, 2) reference-based similarity matching for novel/subtle cases. Multimodal inputs (text, audio, visual) processed through both pipelines, with multimodal large language model (MLLM) distilling knowledge into each to boost accuracy while keeping inference lightweight.

Result: In production: classification pipeline achieves 67% recall at 80% precision; similarity pipeline achieves 76% recall at 80% precision. Large-scale A/B tests show 6-8% reduction in user views of unwanted livestreams.

Conclusion: The hybrid approach demonstrates a scalable and adaptable framework for multimodal content governance, capable of addressing both explicit violations and emerging adversarial behaviors in livestreaming environments.
...
Method: Compared DeepSeek-OCR's vision encoder against simple alternatives: parameter-free mean pooling and a learned hierarchical encoder at matched compression ratios.

Result: Simple approaches match or surpass vision for text reconstruction, and outperform it for language modeling. Vision-based compression fails to beat simple truncation for language modeling.
...
Method: Two-stage training: 1) SFT on curated dataset for complex multi-turn tool composition and error recovery, 2) RL with dense process reward function to encourage strategic tool use. Code-as-tool framework where model generates code to invoke any image operation.
...
Conclusion: CodeVision provides flexible and scalable approach for robust tool-based reasoning in MLLMs, moving beyond fixed tool registries to code generation as universal interface for image operations.
...
TL;DR: GAOT: A three-phase framework that generates articulated 3D objects from text prompts using diffusion models and hypergraph learning.
...
Conclusion: GAOT successfully bridges the gap between text prompts and articulated 3D object generation through a novel combination of diffusion models and hypergraph learning.
...
TL;DR: AdaptVision: An efficient VLM that adaptively acquires visual tokens using coarse-to-fine processing and bounding box tools, trained with decoupled reinforcement learning to balance accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLMs use many visual tokens causing computational overhead, and existing efficient approaches use fixed compression ratios without adapting to task needs. The paper asks: Can VLMs autonomously determine minimum visual tokens per sample?

Method: AdaptVision uses coarse-to-fine visual processing: starts with compressed low-res tokens, then selectively invokes bounding box tool to crop key regions when needed. Trained with Decoupled Turn Policy Optimization (DTPO) - reinforcement learning that decouples tool learning (correct tool use) from accuracy improvement (better answers).
...
Conclusion: The proposed adaptive visual token acquisition paradigm enables VLMs to balance accuracy and efficiency effectively, outperforming fixed compression methods through task-aware token selection.
...
TL;DR: GLASTE is a new scene text editing framework that addresses inconsistency and length-insensitivity issues by combining global contextual information with local features through a global-local combination structure, joint losses, and size-independent text style vectors.


<details>
  <summary>Details</summary>
Motivation: Existing Scene Text Editing (STE) methods suffer from two major challenges: inconsistency (failing to maintain coherence between edited local patches and surrounding areas) and length-insensitivity (struggling with significant differences in text length before and after editing).

Method: Proposes GLASTE framework with: 1) Global-local combination structure, 2) Joint global and local losses, 3) Enhanced text image features for consistency, 4) Text style expressed as size-independent vector, 5) Affine fusion to fill target text while maintaining aspect ratio.
...
Conclusion: GLASTE successfully addresses key challenges in scene text editing by simultaneously incorporating global contextual information with local features, enabling more coherent and length-flexible text replacement while preserving original text style and background texture.
...
TL;DR: Stable Signer is a new end-to-end sign language production model that simplifies the traditional pipeline by combining text understanding and pose-to-video generation, achieving 48.6% improvement over SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Current SLP methods suffer from accumulated errors across multiple stages (Text2Gloss, Gloss2Pose, Pose2Vid) due to inaccuracies in text conversion, pose generation, and video rendering. The field has made slow progress because of these cascading errors in the traditional pipeline.
...
Result: Performance improved by 48.6% compared to current state-of-the-art generation methods in sign language production.

Conclusion: Stable Signer successfully streamlines the traditional SLP pipeline, reduces error accumulation, and achieves significant performance gains through its end-to-end hierarchical generation approach with specialized components for text understanding and hand gesture rendering.
...
TL;DR: UniComp: An information uniqueness driven video compression framework that maximizes information fidelity under computational constraints by measuring token redundancy and performing semantic frame grouping, adaptive resource allocation, and spatial compression.


<details>
  <summary>Details</summary>
Motivation: Existing attention-based compression methods may not fully capture intrinsic redundancy in video tokens. The paper aims to develop a compression framework that maximizes information fidelity under constrained computational budgets by leveraging information-theoretic principles to measure and exploit token redundancy.

Method: Formulates video compression as optimization minimizing conditional entropy between retained and full tokens. Introduces "information uniqueness" metric to measure intrinsic token redundancy. Designs three progressive modules: Frame Group Fusion (semantic frame grouping), Token Allocation (adaptive resource allocation), and Spatial Dynamic Compression (fine-grained spatial compression).

Result: Extensive experiments show UniComp consistently outperforms existing compression methods in preserving essential visual tokens under limited computational budgets, demonstrating the effectiveness of information uniqueness in token compression.

Conclusion: Information uniqueness is pivotal for token compression efficacy. The proposed UniComp framework successfully maximizes information fidelity under computational constraints by measuring and exploiting intrinsic token redundancy through semantic grouping, adaptive allocation, and spatial compression.
...
TL;DR: CSCL is a two-stage pretraining framework that uses contrastive learning and MIL to create transferable WSI representations by aligning H&E with multiple IHC stains, addressing inter-stain misalignment issues.


<details>
  <summary>Details</summary>
Motivation: Current WSI representations are limited by scarce aligned multi-stain datasets and inter-stain misalignment, which degrades slide-level embeddings and hinders consistent patch-level features across different stains.

Method: Two-stage pretraining: 1) Patch-wise contrastive alignment with lightweight adapter to improve H&E-IHC compatibility, 2) Slide-level MIL with cross-stain attention fusion and global alignment modules to integrate stain-specific features and enforce embedding consistency.

Result: Consistent gains on cancer subtype classification, IHC biomarker status classification, and survival prediction, yielding high-quality, transferable H&E slide-level representations.

Conclusion: CSCL effectively addresses inter-stain misalignment and creates universal, transferable WSI representations by leveraging aligned multi-stain data and cross-stain learning techniques.
...
TL;DR: DOT-BI is a novel bot detection method that uses human motion perception to distinguish humans from bots by displaying hidden numbers that are only visible through motion cues.


<details>
  <summary>Details</summary>
Motivation: Need for quick, easy bot identification in surveys and online processes that can't be solved by automated systems but remains accessible to humans.

Method: Display a number with same pixel texture as background; only perceptible to humans through motion and scale differences across frames, invisible to frame-by-frame algorithmic processing.

Result: State-of-the-art multimodal models (GPT-5-Thinking, Gemini 2.5 Pro) failed to extract correct values; 99.5% of human participants solved task in 10.7 seconds average; no negative usability effects.

Conclusion: DOT-BI effectively distinguishes humans from bots using motion perception, is highly usable, and the authors release code and pre-rendered variants for adoption.
...
TL;DR: BBF is a context-aware video frame interpolation framework that uses multimodal conditioning (text, audio, images, video) to handle complex motion patterns, outperforming specialized state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing video frame interpolation methods struggle with fast, complex, non-linear motion patterns and diverse application scenarios. They often fail to produce sharp, temporally consistent frames in fine-grained tasks like audio-visual synchronized interpolation.

Method: 1. Enhanced input design for flexible multimodal conditioning (text, audio, images, video). 2. Decoupled multimodal fusion mechanism that sequentially injects different conditional signals into a DiT backbone. 3. Progressive multi-stage training with start-end frame difference embedding to dynamically adjust data sampling and loss weighting.
...
Conclusion: BBF successfully addresses limitations of existing methods by introducing a context-aware framework with multimodal conditioning, achieving superior performance across diverse interpolation tasks while maintaining foundation model capabilities.
...
<details>
  <summary>Details</summary>
Motivation: The RNA inverse folding problem is crucial for RNA design but challenging due to complex sequence-structure relationships. Existing methods need improvement for accurate sequence generation from desired secondary structures.

Method: HyperRNA uses encoder-decoder architecture with hypergraphs. Preprocessing creates 3-bead coarse-grained graphs from RNA backbone coordinates. Encoding captures higher-order dependencies via attention embedding and hypergraph encoder. Decoding generates sequences autoregressively.
...
TL;DR: CloseUpAvatar is a novel articulated human avatar representation that maintains rendering quality for close-up views while handling general camera motions by using textured planes with adaptive high/low-frequency textures based on camera distance.


<details>
  <summary>Details</summary>
Motivation: Previous avatar representation methods struggle with maintaining rendering quality across wide camera motions, particularly failing to preserve high-frequency details for close-up views while also handling more general camera orientations.

Method: The method represents avatars as textured planes with two sets of learnable textures (low and high-frequency). It automatically switches to high-frequency textures only for close cameras and gradually reduces their impact as cameras move farther away, adjusting rendering quality based on camera distance.
...
Conclusion: CloseUpAvatar enables realistic rendering across a wider range of camera orientations than previous approaches by adaptively adjusting texture detail based on camera distance, balancing quality and performance.
...
Conclusion: HBFormer effectively addresses the limitation of localized attention mechanisms in Vision Transformers by creating a powerful feature bridge that captures long-range dependencies and refines object boundaries, making it particularly effective for challenging medical segmentation tasks.
...
Result: Experiments on the self-processed Teeth3DS benchmark show consistent improvements in Chamfer Distance, with visualizations demonstrating sharper cusps, ridges, and interproximal transitions compared to baseline methods.

Conclusion: The approach provides a simple yet effective way to exploit cross-sample regularities for more accurate and faithful dental point-cloud completion, offering a plug-and-play module compatible with common completion backbones while maintaining the same training losses.
...
<details>
  <summary>Details</summary>
Motivation: 2D foundation models lack 3D consistency in dynamic scene analysis, causing spatial misalignment and temporal flickering in complex 3D environments, which limits their ability to understand scene geometry and motion.
...
Conclusion: Motion4D successfully integrates 2D foundation model priors into 4D Gaussian Splatting representation, achieving superior 3D-consistent dynamic scene understanding while addressing spatial and temporal coherence challenges.
...
TL;DR: LAMP uses LLMs as motion planners to translate natural language into 3D trajectories for objects and cameras via a motion DSL, enabling better motion control from text descriptions.


<details>
  <summary>Details</summary>
Motivation: Existing video generation interfaces for motion control (specifying object dynamics and camera trajectories) are limited, despite motion being essential for composing complex cinematic scenes. There's a need for better natural language interfaces for motion specification.

Method: LAMP leverages LLMs as motion planners with a motion domain-specific language (DSL) inspired by cinematography conventions. LLMs generate structured motion programs from natural language, which are deterministically mapped to 3D trajectories. A large-scale procedural dataset pairs text descriptions with motion programs and 3D trajectories.
...
Conclusion: LAMP successfully bridges natural language descriptions to explicit 3D motion trajectories using LLMs as planners, providing a novel framework for motion-controlled video generation that outperforms existing methods.
...
TL;DR: ReCamDriving is a vision-based framework for generating novel driving trajectory videos using 3D Gaussian Splatting (3DGS) renderings for geometric guidance, with a two-stage training approach and a curated ParaDrive dataset.


<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations: repair-based approaches fail to restore complex artifacts, while LiDAR-based methods rely on sparse and incomplete cues. There's a need for precise camera-controllable video generation with explicit geometric guidance.

Method: 1) Uses dense and scene-complete 3DGS renderings for explicit geometric guidance. 2) Two-stage training: first stage uses camera poses for coarse control, second stage incorporates 3DGS renderings for fine-grained viewpoint and geometric guidance. 3) 3DGS-based cross-trajectory data curation strategy to eliminate train-test gap in camera transformation patterns. 4) Constructs ParaDrive dataset with over 110K parallel-trajectory video pairs.

Result: ReCamDriving achieves state-of-the-art camera controllability and structural consistency in generating novel driving trajectory videos, outperforming existing methods.

Conclusion: The proposed ReCamDriving framework successfully addresses limitations of previous approaches by leveraging 3DGS renderings for geometric guidance, using a two-stage training paradigm to avoid overfitting, and employing data curation to enable scalable supervision from monocular videos.
...
TL;DR: FeatureLens is a lightweight adversarial attack detection framework that uses simple classifiers (SVM/MLP/XGBoost) on extracted image features to achieve high accuracy with excellent interpretability and generalization.
...
TL;DR: MKSNet introduces a Multi-Kernel Selection mechanism with adaptive kernel sizes and dual attention to improve small object detection in remote sensing imagery, outperforming SOTA on DOTA-v1.0 and HRSC2016 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Small object detection in remote sensing faces challenges: critical information loss in deep CNN layers due to high resolution and small target sizes, plus extensive spatial redundancy and complex backgrounds that obscure small targets.

Method: Proposes MKSNet with Multi-Kernel Selection mechanism using large convolutional kernels to capture extensive contextual information with adaptive kernel size selection. Also incorporates dual attention mechanism combining spatial attention (fine-tuning spatial weights) and channel attention (optimizing channel information selection).

Result: Empirical evaluations on DOTA-v1.0 and HRSC2016 benchmarks show MKSNet substantially surpasses existing state-of-the-art models in detecting small objects in remote sensing images.

Conclusion: MKSNet demonstrates superior ability to manage complexities of multi-scale and high-resolution image data, confirming its effectiveness and innovation in remote sensing object detection.
...
TL;DR: MSVP introduces multi-scale visual prompting for small-image benchmarks like MNIST, Fashion-MNIST, and CIFAR-10, achieving consistent improvements with minimal parameter overhead across CNN and Vision Transformer backbones.


<details>
  <summary>Details</summary>
Motivation: Visual prompting research has focused on large Vision Transformers and high-resolution datasets like ImageNet, while small-image benchmarks (MNIST, Fashion-MNIST, CIFAR-10) widely used in education, prototyping, and research have received little attention for prompting techniques.

Method: Multi-Scale Visual Prompting (MSVP) learns global, mid-scale, and local prompt maps fused with input images via lightweight 1×1 convolution. It's backbone-agnostic, adds less than 0.02% parameters, and works with CNN and Vision Transformer architectures.

Result: MSVP significantly improves performance across CNN and Vision Transformer backbones on MNIST, Fashion-MNIST, and CIFAR-10 with negligible computational overhead. Ablations show effectiveness of multi-scale prompting and provide insights through prompt visualizations and Grad-CAM analysis.

Conclusion: Multi-scale visual prompting provides an effective inductive bias even on low-resolution images, offering a simple, generic, and efficient approach for adapting vision models on small-image benchmarks with minimal parameter overhead.
...
<details>
  <summary>Details</summary>
Motivation: Existing STVG studies focus on object-centric descriptive instructions, neglecting task-oriented reasoning crucial for embodied agents to perform goal-directed interactions. There's a need to bridge perception and interaction in embodied scenarios.

Method: Built on ScanNet videos, ToG-Bench comprises 100 annotated clips with 2,704 task-oriented grounding instructions using a semi-automated pipeline combining foundation model annotation and human refinement. Includes task-level evaluation metrics for multi-object and explicit-implicit grounding.

Result: Benchmarked seven state-of-the-art MLLMs, revealing intrinsic challenges of task-oriented STVG and substantial performance gaps across explicit-implicit and multi-object grounding, highlighting the difficulty of bridging perception and interaction.

Conclusion: ToG-Bench addresses the gap in task-oriented reasoning for embodied agents, providing a challenging benchmark that reveals current limitations in bridging perception and interaction, with implications for developing more capable embodied AI systems.
...
TL;DR: Colon-X introduces a comprehensive multimodal dataset (ColonVQA) and reasoning framework (ColonReason/ColonR1) for colonoscopy AI, addressing the gap between multimodal understanding and clinical reasoning.
...
Method: 1) Built ColonVQA dataset with 1.1M+ VQA entries across 76 clinical findings and 18 multimodal tasks. 2) Evaluated 22 MLLMs for generalizability and reliability under perturbations. 3) Created ColonReason dataset via multi-expert debating pipeline. 4) Developed ColonR1 model with task-adaptive rewarding and gradient-stable optimization techniques.

Result: Current MLLMs show limited robustness for clinical use. ColonR1 achieves 56.61% overall accuracy under data-scarce conditions, outperforming supervised fine-tuning by 25.22%, establishing a new reasoning-enabled baseline for colonoscopy analysis.

Conclusion: Colon-X provides foundational resources for multimodal colonoscopy AI, demonstrating that reasoning-centric approaches significantly outperform traditional methods and setting a new standard for clinical reasoning in this domain.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion transformers face deployment challenges due to growing memory footprint and inference latency as model size increases. Existing rotation-based quantization methods from LLMs have substantial overhead and struggle with row-wise outliers in diffusion transformers.
...
Conclusion: ConvRot effectively addresses deployment challenges of diffusion transformers through efficient 4-bit quantization that preserves visual quality while significantly improving speed and memory efficiency.
...
TL;DR: GaussianBlender is a feed-forward framework for instant text-driven 3D stylization that learns disentangled latent spaces from 3D Gaussians and uses latent diffusion for text-conditioned edits, achieving high-fidelity, multi-view consistent results without per-asset optimization.


<details>
  <summary>Details</summary>
Motivation: Current text-to-3D stylization methods require time-intensive per-asset optimization and suffer from multi-view inconsistency due to limitations of text-to-image models, making them impractical for large-scale production in game development, virtual reality, and digital arts.

Method: The method learns structured, disentangled latent spaces with controlled information sharing for geometry and appearance from spatially-grouped 3D Gaussians, then uses a latent diffusion model to apply text-conditioned edits on these learned representations in a feed-forward manner.

Result: GaussianBlender delivers instant, high-fidelity, geometry-preserving, multi-view consistent stylization and surpasses methods that require per-instance test-time optimization, enabling practical, democratized 3D stylization at scale.

Conclusion: GaussianBlender introduces a pioneering feed-forward framework that overcomes limitations of existing methods by providing instant, consistent 3D stylization without per-asset optimization, unlocking practical large-scale production capabilities.
...
TL;DR: This paper provides a comprehensive overview of active visual perception - systems that dynamically engage with environments through sensing and action to modify behavior based on goals or uncertainties, contrasting with passive systems that only use visual data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of passive visual systems in complex environments where static sensing may be insufficient, and to explore how active visual perception can enable more intelligent, adaptive systems that can direct attention, move sensors, or interact with objects to acquire better information.

Method: The paper appears to be a review/survey paper that explores both opportunities and challenges in active visual perception. It provides a comprehensive overview rather than proposing a specific technical method, examining current research, applications, and obstacles in the field.

Result: The paper identifies active visual perception's critical role in applications like robotics, autonomous vehicles, human-computer interaction, and surveillance systems. It highlights the approach's power in complex environments but notes significant challenges including real-time processing of complex visual data, decision-making in dynamic environments, and integrating multimodal sensory inputs.

Conclusion: Active visual perception shows significant promise but faces several challenges that must be overcome for broader adoption. The paper serves as a comprehensive overview of the field's potential, current research status, and the obstacles that need to be addressed to advance these systems.
...
TL;DR: SUSS is a new perceptual similarity score that models images as structured multivariate Normal distributions trained generatively to match human vision, offering better interpretability than deep methods like LPIPS while outperforming hand-crafted measures like SSIM.


<details>
  <summary>Details</summary>
Motivation: Existing perceptual similarity measures have limitations: deep methods like LPIPS use complex, non-linear features with unknown invariances (black boxes), while hand-crafted measures like SSIM are interpretable but miss key perceptual properties. There's a need for a score that aligns well with human vision while being transparent and interpretable.

Method: SUSS models each image through perceptual components represented by structured multivariate Normal distributions. These are trained generatively in self-supervised manner to assign high likelihood to human-imperceptible augmentations. The final score is a weighted sum of component log-probabilities with weights learned from human perceptual datasets. Unlike feature-based methods, SUSS learns image-specific linear transformations of residuals in pixel space.

Result: SUSS aligns closely with human perceptual judgments, shows strong perceptual calibration across diverse distortion types, provides localized interpretable explanations through decorrelated residuals and sampling, demonstrates stable optimization behavior, and achieves competitive performance when used as perceptual loss for downstream imaging tasks.

Conclusion: SUSS offers a novel approach to perceptual similarity that combines the alignment of deep methods with the interpretability of hand-crafted measures, providing transparent inspection capabilities while maintaining strong performance for both evaluation and training of computer vision models.
...
Method: Integrates dataset-adaptive image pairing using DINO for semantic retrieval, with rotation-aware keypoint extraction and matching using ALIKED and Light Glue with rotation-based augmentation.
...
<details>
  <summary>Details</summary>
Motivation: Current VLA models struggle with consistent and precise target-oriented actions, generating redundant/unstable motions due to spatially uniform perception fields that get distracted by irrelevant objects in complex environments.

Method: Proposes PosA-VLA framework with pose-conditioned anchor attention mechanism that guides visual perception toward task-relevant regions, using lightweight architecture without auxiliary perception modules like segmentation/grounding networks.
...
Conclusion: The pose-conditioned supervision approach effectively addresses redundant action issues in VLAs, enabling more precise and efficient robotic manipulation without requiring additional perception modules.
...
TL;DR: BlackCAtt is a black-box adversarial attack method that uses causal pixel sets to create explainable, imperceptible attacks on object detectors, outperforming baselines by 2.7-5.75x across different attack objectives.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial perturbation methods are often white-box, architecture-specific, and lack explainability. There's a need for black-box attacks that reveal why attacks work, allowing developers to understand vulnerabilities and improve model robustness.

Method: BlackCAtt combines causal pixel sets with object detector bounding boxes to create adversarial attacks. It identifies minimal, causally sufficient pixel sets to generate imperceptible perturbations that cause bounding box loss, modification, or addition, treating detectors as black boxes.

Result: On COCO test dataset, BlackCAtt outperforms baselines: 2.7x better at removing detections, 3.86x better at changing detections, and 5.75x better at triggering spurious detections. Attacks are imperceptible and work across different detector architectures.

Conclusion: Causal pixel identification enables more precisely targeted, less perceptible black-box attacks on object detectors. BlackCAtt demonstrates the power of causal pixels for creating explainable, architecture-agnostic adversarial attacks that expose model vulnerabilities.
...
TL;DR: Proposes Dual-level Modality Debiasing Learning (DMDL) to address modality bias in unsupervised visible-infrared person re-identification by implementing debiasing at both model and optimization levels.


<details>
  <summary>Details</summary>
Motivation: Current two-stage learning pipelines for unsupervised visible-infrared person re-identification introduce modality bias, where modality-specific cues learned in single-modality training propagate into cross-modality learning, impairing identity discrimination and generalization.

Method: DMDL framework with two components: 1) Causality-inspired Adjustment Intervention (CAI) module at model level that replaces likelihood-based modeling with causal modeling to prevent modality-induced spurious patterns; 2) Collaborative Bias-free Training (CBT) strategy at optimization level that interrupts bias propagation across data, labels, and features through modality-specific augmentation, label refinement, and feature alignment.
...
Conclusion: The proposed DMDL framework effectively addresses modality bias in unsupervised visible-infrared person re-identification through dual-level debiasing, leading to improved performance and generalization.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models trained on large internet-scraped datasets (like LAION-5B) inherently learn and reproduce societal biases present in the training data, resulting in stereotypical outputs that need to be addressed.
...
Conclusion: SelfDebias provides a practical, unsupervised solution for debiasing diffusion models that works at test-time without requiring labeled data, making it broadly applicable to various diffusion architectures and capable of addressing both demographic and abstract biases.
...
Method: Proposes an improved ResNet34 model with: 1) multi-scale input module as first layer, 2) Inception v2 module as residual downsampling layer, and 3) channel attention mechanism to weight different channels for important feature extraction.
...
TL;DR: LSRS improves VAR image generation by using rejection sampling across latent scales to reduce structural errors while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: VAR's parallel token sampling within scales causes structural errors and suboptimal image quality, requiring a method to mitigate autoregressive error accumulation.
...
Result: LSRS reduces VAR-d30's FID from 1.95 to 1.78 with only 1% inference time increase, and to 1.66 with 15% time increase, significantly improving generation quality with minimal computational overhead.
...
TL;DR: A deep learning approach for automatic recognition and translation of Egyptian hieroglyphs from images to English, achieving a BLEU score of 42.2.
...
Method: Three-stage pipeline: 1) segmentation using Contour and Detectron2, 2) mapping symbols to Gardiner codes, 3) translation using CNN model. Uses Morris Franken and EgyptianTranslation datasets for classification and translation.

Result: Achieved a BLEU score of 42.2, which represents significant improvement over previous research in hieroglyph translation.

Conclusion: The proposed method successfully demonstrates automated hieroglyph translation with competitive performance, showing promise for advancing computational Egyptology and ancient text analysis.
...
Method: Combines mathematical transforms to create a robust method that measures breath rate from video with minimal hardware requirements, specifically designed to handle distortions from subject movement.

Result: Tested on 14 volunteers with over 2.5 hours of video, achieving <5% relative deviation from ground truth and average mean absolute error of 0.57 respirations per minute, significantly better than previous works.

Conclusion: The proposed method enables reliable remote breath rate measurement without significant behavioral limitations, overcoming previous challenges with subject movement and accuracy.
...
TL;DR: Proposes LUnet, a lean Unet architecture with constant channel count across layers instead of doubling channels when halving resolution, achieving comparable performance with 30x fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Standard Unet architectures have large memory footprints due to iterative downsampling with increasing channels, limiting batch sizes and increasing latency. Channel pruning requires lengthy optimization and may not generalize well across tasks.

Method: Proposes LUnet with compact, flat hierarchy where channels are not doubled as resolution is halved (constant channel count across layers). Skip connections allow bottleneck channels to be largely reduced. Evaluated on public MRI dataset and two internal CT datasets.

Result: LUnet achieves performance comparable to conventional Unet and pruned networks with over 30 times fewer parameters. Simply eliminating random channels at pruning-identified layers or largest layers achieves similar/better performance than sophisticated pruning methods like STAMP.

Conclusion: The final architecture structure is more crucial than channel selection strategy in pruning. Lean Unet with constant channel count requires far fewer parameters while achieving superior performance to standard Unet for the same parameter count.
...
TL;DR: HP-Net: A heatmap pooling network for human action recognition that extracts robust, concise pooled features from videos using feedback pooling and integrates them with multimodal data for improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing RGB video HAR methods suffer from information redundancy, noise susceptibility, and high storage costs. The paper aims to better utilize video information by extracting more informative and robust features.

Method: Proposes HP-Net with: 1) Feedback pooling module to extract information-rich, robust pooled features from body heatmaps; 2) Spatial-motion co-learning module; 3) Text refinement modulation module to integrate pooled features with multimodal data.

Result: Outperforms existing HAR methods on multiple benchmarks: NTU RGB+D 60, NTU RGB+D 120, Toyota-Smarthome, and UAV-Human datasets.

Conclusion: HP-Net effectively addresses limitations of existing RGB video HAR methods by extracting robust pooled features and integrating multimodal data, achieving state-of-the-art performance across multiple benchmarks.
...
Method: CoDA first identifies the "intrinsic core distribution" of the target dataset using robust density-based discovery, then steers the generative process to align generated samples with this core distribution, bridging the gap between general-purpose priors and target semantics.

Result: CoDA achieves performance on par with or superior to previous methods that require target-specific training, establishing new SOTA accuracy of 60.4% at 50-IPC on ImageNet-1K, without needing generative models trained on the target dataset.

Conclusion: CoDA provides an effective framework for dataset distillation using only off-the-shelf text-to-image models by aligning with the target dataset's core distribution, overcoming limitations of both target-specific training and distribution mismatch approaches.
...
TL;DR: PULSE is a unified multi-task vision-language framework for cardiac image analysis that combines anatomical segmentation, disease classification, and clinical report generation in a single architecture with cross-modality generalization.


<details>
  <summary>Details</summary>
Motivation: Current cardiac image analysis is fragmented across separate tasks (segmentation, classification, report generation) with separate networks and data regimes, lacking a unified framework that generalizes across imaging modalities and datasets.
...
Conclusion: PULSE unifies multiple cardiac analysis tasks in one architecture, enabling transition from pixels to structures to clinical reasoning, representing progress toward a comprehensive foundation model for cardiac imaging.
...
TL;DR: FAMamba integrates frequency guidance with Mamba's sequence modeling for traffic image restoration under adverse weather, using frequency-adaptive scanning and wavelet-based residual learning.
...
Conclusion: FAMamba successfully integrates frequency guidance with sequence modeling, leveraging frequency-domain priors and Mamba's capabilities to achieve high-quality image reconstruction with precise details for traffic image restoration.
...
TL;DR: The paper introduces a public prostate biopsy dataset from Iraq to address the lack of diverse histopathology data for AI model development and validation, particularly for underrepresented Middle Eastern populations.
...
Method: The authors collected 339 whole-slide images of prostate core needle biopsies from 185 consecutive patients in Erbil, Iraq. Three pathologists independently assigned Gleason scores and ISUP grades. Scanning was performed using three different scanners (Leica, Hamamatsu, Grundium). All slides were de-identified and provided in native formats without conversion.

Result: The dataset enables multiple research applications including grading concordance analyses, color normalization studies, and cross-scanner robustness evaluations. The data will be deposited in the Bioimage Archive under a CC BY 4.0 license.

Conclusion: This publicly released dataset from Iraq addresses the geographic diversity gap in histopathology data and supports the development and validation of more generalizable AI models for digital pathology across different populations and scanning platforms.
...
TL;DR: Small 5M-parameter ViTs benefit from targeted pre-training and careful data selection, but intermediate fine-tuning can sometimes harm downstream performance due to task dissimilarity.


<details>
  <summary>Details</summary>
Motivation: Transformers require large parameters and data for strong performance, but this paper explores how small-scale vision transformers (5M parameters) can benefit from different training strategies to achieve good performance with limited resources.
...
Result: Pre-training and fine-tuning always help but have diminishing returns, while intermediate fine-tuning can actually show harmful impact on downstream performance, potentially due to dissimilarity in task mechanics.
...
TL;DR: CaravelMetrics is an automated framework for analyzing brain blood vessels using graph-based representations from 3D scans, computing 15 features to study vascular organization across populations.


<details>
  <summary>Details</summary>
Motivation: Need for scalable, automated tools to quantitatively analyze cerebrovascular morphology from medical imaging data for population studies of vascular health and aging.

Method: Framework uses skeletonization to create graph representations of vessels, integrates atlas-based regional parcellation, centerline extraction, and computes 15 morphometric, topological, fractal, and geometric features at global and regional scales.

Result: Applied to 570 3D TOF-MRA scans from IXI dataset (ages 20-86), framework produced reproducible vessel graphs showing age/sex variations and education-associated increases in vascular complexity consistent with literature.

Conclusion: CaravelMetrics provides scalable, fully automated quantitative cerebrovascular feature extraction supporting normative modeling and population-level studies of vascular health and aging.
...
Method: Developed SSDCA combining longitudinal endoscopic images at restaging and follow-up using Siamese Swin Transformers with dual cross-attention to extract domain-agnostic features without requiring spatial alignment.

Result: SSDCA achieved best performance: 81.76% balanced accuracy, 90.07% sensitivity, 72.86% specificity on held-out test set of 62 patients. Robust to imaging artifacts and showed excellent feature discriminability.

Conclusion: SSDCA effectively distinguishes clinical complete response from local regrowth in rectal cancer surveillance, providing a robust tool for watch-and-wait management without requiring image alignment.
...
TL;DR: FRESCO improves video consistency in zero-shot diffusion models by combining intra-frame and inter-frame correspondence for better spatial-temporal constraints.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot video adaptation methods for text-to-image diffusion models rely on inter-frame correspondence in attention mechanisms, but this soft constraint is insufficient and leads to temporal inconsistency in generated videos.

Method: FRESCO integrates intra-frame correspondence with inter-frame correspondence to create more robust spatial-temporal constraints, ensuring consistent transformation of semantically similar content between frames. It goes beyond attention guidance to explicitly optimize features.

Result: FRESCO achieves high spatial-temporal consistency with input videos, significantly enhancing visual coherence in manipulated videos. It demonstrates effectiveness in video-to-video translation and text-guided video editing tasks, outperforming current zero-shot methods.

Conclusion: FRESCO represents a significant advance in zero-shot video adaptation for diffusion models by addressing temporal inconsistency through enhanced spatial-temporal constraints, enabling high-quality, coherent video generation without additional training.
...
TL;DR: UniMo is an autoregressive model that unifies 2D human videos and 3D human motions for simultaneous generation and understanding within a single framework.
...
Method: Models videos and 3D motions as unified token sequences using separate embedding layers to mitigate distribution gaps. Uses a novel 3D motion tokenizer with temporal expansion strategy and VQ-VAE with multiple expert decoders for body shapes, translation, orientation, and poses.
...
TL;DR: A framework that enhances ground-truth images for better supervision in real-world image restoration by generating perceptually improved ground truth through frequency-domain mixup and training a lightweight refinement network.


<details>
  <summary>Details</summary>
Motivation: Real-world image restoration performance is limited by poor-quality ground-truth images in datasets due to practical data acquisition constraints. Existing methods suffer from insufficient supervision quality.

Method: Proposes a framework with: 1) Adaptive frequency masks learned by a conditional mask generator to guide optimal fusion of frequency components from original ground truth and super-resolved variants, 2) Frequency-domain mixup to preserve semantic consistency while enriching perceptual details, 3) A lightweight output refinement network trained on enhanced ground truth that integrates with existing restoration models.
...
Conclusion: The proposed framework successfully addresses ground-truth quality limitations in real-world image restoration by enhancing supervision through frequency-domain mixup and refinement, leading to better restoration performance without hallucinated artifacts.
...
<details>
  <summary>Details</summary>
Motivation: Current RL approaches for improving MLLMs' temporal reasoning are limited to specific tasks and data, lacking generalization across diverse temporal understanding scenarios needed for long-form video analysis.
...
Result: TempR1 achieves state-of-the-art performance across multiple benchmarks, with joint optimization over complementary tasks creating strong synergistic effects that enhance both generalization and single-task performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing tuning-free face personalization methods struggle to simultaneously achieve high identity fidelity and flexible text controllability. Text embedding approaches map facial features to text embeddings while adapter-based methods inject features through cross-attention layers, but neither paradigm alone achieves both goals effectively.
...
Conclusion: UniID provides a unified framework that synergistically combines text embedding and adapter-based approaches to enable high-fidelity face personalization with flexible text controllability, outperforming existing state-of-the-art methods.
...
TL;DR: BlurDM integrates blur formation process into diffusion models for dynamic scene deblurring, using dual-diffusion to simultaneously denoise and deblur images.
...
Method: BlurDM uses a dual-diffusion forward scheme that diffuses both noise and blur onto sharp images, with a reverse process that simultaneously denoises and deblurs. It operates in latent space for efficient integration into deblurring networks.
...
Conclusion: BlurDM effectively integrates blur formation into diffusion models, providing a flexible prior generation network that improves deblurring performance across multiple datasets.
...
TL;DR: DirectDrag is a mask- and prompt-free image editing framework that enables precise drag-based manipulation using only point displacement, achieving high visual fidelity without manual masks or textual prompts.
...
Conclusion: DirectDrag provides a novel mask- and prompt-free editing framework that enables precise and efficient manipulation with minimal user input while maintaining high image fidelity and accurate point alignment, addressing fundamental limitations of existing drag-based image editing methods.
...
TL;DR: DIQ-H is the first benchmark for evaluating Vision-Language Model robustness under dynamic visual degradation in temporal sequences, measuring hallucination persistence, error recovery, and temporal consistency through physics-based corruptions and multi-turn QA tasks.


<details>
  <summary>Details</summary>
Motivation: VLMs deployed in safety-critical applications like autonomous driving must handle continuous visual streams under imperfect conditions, but existing benchmarks focus on static, high-quality images and ignore temporal degradation and error propagation, which are critical failure modes where transient visual corruption induces persistent hallucinations across frames.

Method: DIQ-H applies physics-based corruptions (motion blur, sensor noise, compression artifacts) to temporal sequences and measures hallucination persistence, error recovery, and temporal consistency through multi-turn question-answering tasks. The paper also proposes Uncertainty-Guided Iterative Refinement (UIR) for scalable annotation, which generates reliable pseudo-ground-truth using lightweight VLMs with uncertainty filtering.

Result: Experiments on 16 state-of-the-art VLMs reveal substantial robustness gaps: advanced models like GPT-4o achieve only 78.5% recovery rate, while open-source models struggle with temporal consistency at less than 60%. UIR achieves 15.3% accuracy improvement for annotation.

Conclusion: DIQ-H provides a comprehensive platform for evaluating VLM reliability in real-world deployments, addressing critical gaps in existing benchmarks by focusing on temporal degradation and error propagation in dynamic visual streams.
...
TL;DR: The paper introduces text embedding perturbation (TEP) as a complementary randomness source to spatial noise in text-to-image diffusion models, enhancing generative diversity and quality through frequency-domain analysis and adaptive perturbation scheduling.
...
Method: The method introduces step-based text embedding perturbation combined with frequency-guided noise schedules and spatial noise perturbation. It adapts perturbation intensity selectively based on frequency-specific contributions to generation and tolerance to perturbation.

Result: The approach demonstrates significant improvements on multiple benchmarks with almost no additional computation and can be seamlessly integrated into existing TTS methods.

Conclusion: Text embedding perturbation complements spatial noise in T2I diffusion models, with spatial noise favoring low-frequency components and text embedding perturbation enhancing high-frequency details, leading to improved generative diversity and quality.
...
TL;DR: DIG is a training-free frame selection framework that adapts sampling strategy based on query type: uniform sampling for global queries, query-aware selection for localized queries, improving LMM performance on long-form videos.
...
Method: 1) Identifies query typology distinguishing global vs. localized queries; 2) Validates that uniform sampling works for global queries while localized queries need query-aware selection; 3) Proposes DIG framework that adapts strategy: uniform sampling for global queries, specialized pipeline for localized queries to extract query-relevant frames.
...
Conclusion: Complex query-aware frame selection mechanisms are not universally necessary - query type matters. DIG's adaptive approach provides effective and efficient frame selection for long-form video understanding with LMMs.
...
TL;DR: Sketches as sequences: absolute coordinates beat relative ones, non-autoregressive decoders beat autoregressive ones, and temporal importance varies by order and task.


<details>
  <summary>Details</summary>
Motivation: Despite advances in sketch representation learning, there's uncertainty about whether treating sketches as sequences is justified and which temporal aspects matter most for representation quality.

Method: Investigated sketch representation by treating sketches as sequences, comparing absolute vs. relative coordinates, and evaluating autoregressive vs. non-autoregressive decoders across different temporal orders.

Result: Absolute coordinates consistently outperform relative ones; non-autoregressive decoders beat autoregressive counterparts; temporal importance depends on both the order considered and the specific task evaluated.

Conclusion: While treating sketches as sequences is valid, absolute coordinates and non-autoregressive approaches yield better representations, with temporal relevance being context-dependent rather than universally important.
...
TL;DR: Feed-forward 3D reconstruction models like VGGT can inherently filter out noisy/distractor images without explicit outlier rejection mechanisms, using internal representations from a specific layer for effective noise filtering without additional training.


<details>
  <summary>Details</summary>
Motivation: Traditional Structure-from-Motion pipelines handle noisy images through geometric verification and outlier rejection, but feed-forward 3D reconstruction models lack these explicit mechanisms, leading to degraded performance with in-the-wild image collections containing irrelevant inputs.

Method: The authors discovered that existing feed-forward reconstruction models (e.g., VGGT) can inherently distinguish distractor images. Through analysis under varying proportions of synthetic distractors, they identified a specific layer that naturally exhibits outlier-suppressing behavior and encodes discriminative internal representations that enable effective noise-filtering capability.

Result: Extensive experiments on both controlled and in-the-wild datasets demonstrate that this implicit filtering mechanism is consistent and generalizes well across diverse scenarios. The method enables outlier-view rejection in feed-forward 3D reconstruction without any additional fine-tuning or supervision.

Conclusion: Feed-forward 3D reconstruction models possess inherent noise-filtering capabilities through specific internal representations, allowing for effective outlier rejection without explicit mechanisms or additional training, improving robustness for in-the-wild image collections.
...
TL;DR: A framework that automatically learns group actions on latent image manifolds, discovering transformation-relevant structures without manual partitioning of latent variables.


<details>
  <summary>Details</summary>
Motivation: Prior methods for modeling group actions either operate in high-dimensional data space with uniform transformations, or require manual partitioning of latent variables into equivariant and invariant subspaces, limiting robust learning of group actions in representation space.
...
Conclusion: The framework successfully learns group actions on latent image manifolds without manual intervention, can be integrated with any standard encoder-decoder architecture, and produces effective representations for downstream tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing INR-based video codecs like NVRC achieve state-of-the-art performance but have high computational complexity. Lightweight INRs show promise but need better compression efficiency and faster entropy coding methods.

Method: Two key changes: 1) Multi-scale feature grids integrated into lightweight neural representation, using higher resolution grids to improve INR performance at low complexity. 2) Octree-based context model for entropy coding high-dimensional feature grids instead of slow autoregressive models.
...
Conclusion: NVRC-Lite successfully extends NVRC toward lightweight representations with improved compression efficiency and significantly faster entropy coding, making INR-based video compression more practical.
...
TL;DR: C3G proposes a feed-forward framework for 3D scene reconstruction and understanding from unposed sparse views using compact 3D Gaussians at essential locations, improving memory efficiency and feature aggregation.
...
Method: C3G uses learnable tokens that aggregate multi-view features through self-attention to guide Gaussian generation, ensuring each Gaussian integrates relevant visual features across views, then exploits learned attention patterns for efficient Gaussian decoding and feature lifting.
...
<details>
  <summary>Details</summary>
Motivation: Current sparse attention methods use binary masks that retain or discard entire key-value blocks, causing substantial information loss under high sparsity, especially for video tasks requiring rich contextual information.

Method: Pyramid Sparse Attention (PSA) uses multi-level pooled KV representations where each query block dynamically allocates different pooling levels to KV blocks - lower pooling for critical blocks, higher pooling for less important ones, creating an interpolation between full retention and pruning.

Result: PSA consistently outperforms or matches existing sparse attention baselines across video understanding and generation benchmarks, preserving contextual information and visual fidelity with superior efficiency-quality trade-offs.

Conclusion: PSA provides an effective solution to the quadratic complexity bottleneck of attention mechanisms by introducing finer-grained sparse attention that mitigates information loss while maintaining computational efficiency, with applications to both video understanding and generation tasks.
...
TL;DR: This thesis presents two main contributions: 1) architectural improvements to normalizing flows including invertible convolutions, efficient coupling layers, and fast inversion algorithms, and 2) practical applications of generative models to computer vision problems like agricultural quality assessment, geological mapping, privacy preservation, and art restoration.


<details>
  <summary>Details</summary>
Motivation: The motivation is to advance both the theoretical efficiency of generative models (particularly normalizing flows) and demonstrate their practical utility in solving real-world computer vision challenges across diverse domains including agriculture, geology, autonomous driving, and art restoration.

Method: The thesis introduces six key innovations for normalizing flows: invertible 3x3 convolution layers with mathematical proofs, Quad-coupling layers, parallel inversion algorithms for convolutions, efficient backpropagation for inverse convolutions, Inverse-Flow architecture, and Affine-StableSR super-resolution model. For applications, it uses Conditional GANs for agricultural assessment, stacked autoencoders for geological mapping, face detection/inpainting for privacy preservation, and adapted diffusion models for art restoration.

Result: The normalizing flow improvements provide mathematically proven invertibility, computational efficiency, and parameter reduction while maintaining performance. The applications achieve good accuracy in seed purity testing, improved feature extraction for geological mapping, effective privacy preservation for autonomous driving datasets, and unified handling of multiple degradation types in art restoration.

Conclusion: The thesis successfully advances both theoretical aspects of normalizing flows and demonstrates their practical utility across diverse computer vision applications, showing that generative models can effectively address real-world challenges in agriculture, geology, privacy preservation, and cultural heritage preservation.
...
Conclusion: RELIC establishes a strong foundation for next-generation interactive world modeling by successfully integrating all three key requirements into a unified framework that maintains real-time performance while enabling memory-aware, long-duration scene exploration.
...
TL;DR: DIRL framework trains VLMs to coordinate multiple spatial reasoning tools through interactive RL, achieving SOTA on spatial benchmarks and enabling real-world robot manipulation.


<details>
  <summary>Details</summary>
Motivation: VLMs have strong qualitative visual understanding but lack metrically precise spatial reasoning needed for embodied applications. While tool augmentation could help, current approaches rely on handcrafted prompting or fixed tool pipelines that limit optimal tool-use discovery.

Method: Double Interactive Reinforcement Learning (DIRL) - two-phase training: 1) Teaching phase combines demonstrations from single-tool specialists trained via interactive RL with traces from frontier models using all tools; 2) Exploration phase refines multi-tool coordination through continued RL.

Result: SpaceTools model achieves SOTA on spatial understanding benchmarks (RoboSpatial-Home, BLINK, BOP-ASK) with +12% improvement over SFT and +16% over RL baselines. Demonstrates reliable real-world manipulation using 7-DOF robot as tool.

Conclusion: DIRL enables VLMs to effectively coordinate multiple spatial reasoning tools through interactive learning, bridging the gap between qualitative visual understanding and metrically precise spatial reasoning for embodied applications.
...
TL;DR: PosterCopilot is an LMM-based framework for professional graphic design that improves layout accuracy and enables layer-specific iterative editing through progressive training and workflow integration.
...
Conclusion: PosterCopilot advances layout reasoning and controllable editing for professional graphic design, addressing key limitations of existing LMM-based approaches.
...
TL;DR: SimFlow fixes VAE encoder variance to a constant (e.g., 0.5) to simplify NF training, eliminating complex noise pipelines and enabling joint VAE-NF optimization for better image generation.


<details>
  <summary>Details</summary>
Motivation: Prior NF methods have two main limitations: 1) they require complex noise injection and denoising pipelines for data augmentation, and 2) they use frozen pretrained VAE encoders leading to suboptimal reconstruction and generation quality.

Method: Fix the variance parameter (normally predicted by VAE encoder) to a constant value (e.g., 0.5). This allows the encoder to output broader token distributions, enables the decoder to learn reconstruction from augmented tokens without extra noise steps, and simplifies the VAE evidence lower bound for stable joint training of NF and VAE.

Result: SimFlow achieves gFID of 2.15 on ImageNet 256×256 generation, outperforming previous SOTA STARFlow (gFID 2.40). When combined with REPA-E alignment, it achieves gFID 1.91, setting new SOTA among normalizing flows.

Conclusion: Fixing VAE variance to a constant provides a simple yet effective solution to two major NF limitations, enabling better joint optimization and eliminating complex noise pipelines while achieving state-of-the-art generation quality.
...
TL;DR: Single-life learning trains vision models exclusively on one person's egocentric videos, showing that models from different individuals develop aligned geometric understanding and generalize well to downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To explore whether training vision models exclusively on egocentric videos from a single individual can yield meaningful geometric representations, leveraging the natural multiple viewpoints captured in one person's daily life for self-supervised learning.

Method: Train distinct visual encoders independently on egocentric video datasets from different individuals (single-life paradigm). Use self-supervised learning with multiple viewpoints from each person's life. Introduce a novel cross-attention-based metric to quantify functional alignment between models trained on different lives.

Result: 1) Models trained on different lives develop highly aligned geometric understanding. 2) Single-life models learn generalizable representations that transfer effectively to depth estimation in unseen environments. 3) 30 hours from one week of a person's life performs comparably to 30 hours of diverse web data.

Conclusion: The shared structure of the world leads to consistency in models trained on individual lives and provides a powerful signal for visual representation learning, establishing the viability and strength of single-life learning paradigms.
...
### [154] [Tada-DIP: Input-adaptive Deep Image Prior for One-shot 3D Image Reconstruction](https://arxiv.org/abs/2512.03962)
*Evan Bell,Shijun Liang,Ismail Alkhouri,Saiprasad Ravishankar*

Main category: eess.IV

TL;DR: Tada-DIP: A 3D extension of Deep Image Prior for solving 3D inverse problems with input-adaptation and denoising regularization to prevent overfitting.


<details>
  <summary>Details</summary>
Motivation: Deep Image Prior (DIP) shows promise for one-shot neural-network based image reconstruction but has limited application to 3D problems. There's a need for effective 3D DIP methods for solving 3D inverse problems.

Method: Tada-DIP combines input-adaptation and denoising regularization to create a fully 3D DIP method that prevents overfitting while producing high-quality 3D reconstructions.
...
Conclusion: Tada-DIP is a highly effective 3D DIP method that successfully addresses 3D inverse problems while avoiding common DIP overfitting issues, demonstrating strong performance in sparse-view CT reconstruction.
...
### [155] [Detecting AI Hallucinations in Finance: An Information-Theoretic Method Cuts Hallucination Rate by 92%](https://arxiv.org/abs/2512.03107)
*Mainak Singha*
...
<details>
  <summary>Details</summary>
Motivation: LLMs produce fluent but unsupported answers (hallucinations), which limits their safe deployment in high-stakes domains like finance. Current approaches need better methods to detect when models are generating content not supported by available evidence.

Method: ECLIPSE combines entropy estimation via multi-sample clustering with a novel perplexity decomposition that measures how models use retrieved evidence. It treats hallucination as a mismatch between semantic entropy and evidence capacity, with a theoretically proven convex objective function.

Result: On a controlled financial QA dataset with GPT-3.5-turbo, ECLIPSE achieved ROC AUC of 0.89 and average precision of 0.90, substantially outperforming semantic entropy-only baseline (AUC 0.50). Ablation studies show the method depends on calibrated token-level uncertainties.

Conclusion: ECLIPSE effectively detects hallucinations by analyzing evidence utilization patterns, with perplexity decomposition features being most important. The framework is a logprob-native mechanism that requires calibrated token-level uncertainties, positioning it as a controlled mechanism study for future broader validation.
...
TL;DR: SPARK: A three-stage framework using synthetic verification data to train generative process reward models (PRMs) that outperform ground-truth supervision for mathematical reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional process reward models require expensive step-level annotations or ground truth references, limiting their adoption. The authors aim to develop a reference-free approach that can surpass ground-truth methods.

Method: Three-stage framework: 1) Generator produces diverse solutions, verifier evaluates using parallel scaling (self-consistency) and sequential scaling (meta-critique). 2) Use verification outputs as synthetic training data to fine-tune generative PRMs. 3) Apply PRM with chain-of-thought verification as reward model in RL experiments with format constraints to prevent reward hacking.

Result: Achieves 67.5 F1 on ProcessBench (vs 66.4 for reference-guided training and 61.9 for GPT-4o). In RL experiments with Qwen2.5-Math-7B, achieves 47.4% average accuracy across six benchmarks, outperforming ground-truth-based RLVR (43.9%).

Conclusion: SPARK enables reference-free RL training that exceeds ground-truth methods, opening possibilities for domains lacking verifiable answers or accessible ground truth.
...
TL;DR: Proposed adaptive layer-freezing strategy for Federated Learning reduces energy consumption by up to 23% while maintaining MRI-to-CT conversion performance.


<details>
  <summary>Details</summary>
Motivation: FL can advance health equality but its high resource requirements exclude centers with limited computational infrastructure, widening healthcare disparities. Need for sustainable AI that reduces energy consumption while maintaining performance.

Method: Green AI-oriented adaptive layer-freezing strategy that selectively freezes encoder weights based on monitored relative differences from round to round. Uses patience-based mechanism to freeze only when updates remain consistently minimal. Tested on MRI-to-CT conversion with different federated architectures, tracking energy/CO2eq with CodeCarbon library.
...
Conclusion: The approach enables sustainable FL that meets clinical requirements while ensuring climatic, social, and economic sustainability. Lays groundwork for novel FL evaluation frameworks advancing privacy, equity, and justice in AI-driven healthcare.
...
TL;DR: A unified deep learning framework combining cyclical temporal encoding with hybrid LSTM-CNN ensemble for multistep electricity consumption forecasting, achieving improved accuracy across all forecast horizons.


<details>
  <summary>Details</summary>
Motivation: Accurate electricity consumption forecasting is essential for demand management and smart grid operations. Existing methods may not fully exploit both long-term seasonal patterns and short-term local variations in energy data.

Method: 1) Transform calendar-based attributes using sine-cosine encodings to preserve periodic structure; 2) Evaluate predictive relevance through correlation analysis; 3) Ensemble model combining LSTM (for long-term patterns), CNN (for short-term patterns), and MLP meta-learners specialized for each forecast horizon.

Result: The hybrid model achieved lower RMSE and MAE than individual architectures and prior methods across all seven forecast horizons. Ablation studies confirmed benefits of cyclical encodings and calendar features.

Conclusion: Combining cyclical temporal representations with complementary deep learning structures provides consistent improvements in short-term energy forecasting. This is the first unified framework jointly evaluating temporal encodings, calendar features, and hybrid ensemble architectures.
...
### [159] [Tuning for TraceTarnish: Techniques, Trends, and Testing Tangible Traits](https://arxiv.org/abs/2512.03465)
*Robert Dilworth*

Main category: cs.CR

TL;DR: TraceTarnish is an adversarial stylometry attack that anonymizes text authorship, but leaves detectable forensic traces in function words, content words, and type-token ratios that can serve as indicators of compromise.


<details>
  <summary>Details</summary>
Motivation: To develop and evaluate an adversarial stylometry attack that can effectively anonymize text authorship while understanding the forensic traces it leaves behind, enabling both attack improvement and defense detection.

Method: Used Reddit comments as source data, transformed them with TraceTarnish, then used StyloMetrix to extract stylometric features. Applied Information Gain criterion to identify most predictive features for detecting authorship anonymization.

Result: Identified five key stylometric features that serve as reliable indicators of compromise: function word frequencies (L_FUNC_A & L_FUNC_T), content word distributions (L_CONT_A & L_CONT_T), and Type-Token Ratio (ST_TYPE_TOKEN_RATIO_LEMMAS). These features reveal when text has been deliberately altered to mask authorship.

Conclusion: Adversarial stylometry attacks like TraceTarnish leave detectable forensic traces that can serve as both indicators of compromise and forensic beacons. The paradox is that attempts to erase authorship traces create new, identifiable patterns, enabling both attack enhancement and defensive detection.
...
### [160] [SELF: A Robust Singular Value and Eigenvalue Approach for LLM Fingerprinting](https://arxiv.org/abs/2512.03620)
*Hanxiu Zhang,Yue Zheng*

Main category: cs.CR

TL;DR: SELF is a novel intrinsic weight-based fingerprinting scheme for LLM IP protection that uses singular value/eigenvalue decomposition of attention weights and neural network-based similarity comparison to detect unauthorized model usage.


<details>
  <summary>Details</summary>
Motivation: Existing LLM IP protection methods (behavior-based or structural fingerprinting) are vulnerable to false claim attacks and weight manipulations, creating a critical need for more robust IP protection mechanisms.

Method: SELF uses two key innovations: 1) unique, scalable, transformation-invariant fingerprint extraction via singular value and eigenvalue decomposition of LLM attention weights, and 2) neural network-based fingerprint similarity comparison using few-shot learning and data augmentation.

Result: SELF maintains high IP infringement detection accuracy while showing strong robustness against various downstream modifications including quantization, pruning, and fine-tuning attacks.

Conclusion: SELF provides a robust intrinsic weight-based fingerprinting solution that eliminates dependency on input and resists false claims, offering effective IP protection for LLMs against unauthorized usage.
...
### [161] [Epistemic Substitution: How Grokipedia's AI-Generated Encyclopedia Restructures Authority](https://arxiv.org/abs/2512.03337)
*Aliakbar Mehdizadeh,Martin Hilbert*

Main category: cs.SI

TL;DR: AI encyclopedia Grokipedia shows different epistemic sourcing patterns than Wikipedia, replacing academic sources with user-generated/civic sources and showing topic-dependent citation strategies with a unique scaling law for citation density.


<details>
  <summary>Details</summary>
Motivation: To investigate whether AI-curated encyclopedias (like Grokipedia) and human-curated encyclopedias (like Wikipedia) rely on the same foundations of authority, examining potential epistemological shifts in knowledge production.

Method: Multi-scale comparative analysis of citation networks from 72 matched article pairs (total ~60,000 sources), using an 8-category epistemic classification to map "epistemic profiles" of articles on each platform.

Result: Grokipedia replaces Wikipedia's heavy reliance on peer-reviewed academic sources with increased "User-generated" and "Civic organization" sources. Shows different epistemological profiles for leisure vs. civic topics. Reveals a "scaling-law for AI-generated knowledge sourcing" - linear relationship between article length and citation density distinct from human reference sourcing.

Conclusion: LLM-based encyclopedias don't just automate knowledge production but restructure it, suggesting need for continued algorithm audits to understand epistemological shifts given encyclopedias' important role.
...
### [162] [Is Vibe Coding Safe? Benchmarking Vulnerability of Agent-Generated Code in Real-World Tasks](https://arxiv.org/abs/2512.03262)
*Songwen Zhao,Danqing Wang,Kexun Zhang,Jiaxuan Luo,Zhuo Li,Lei Li*

Main category: cs.SE

TL;DR: Vibe coding (LLM agents for complex coding tasks) produces functionally correct but insecure solutions - only 10.5% secure despite 61% functional correctness, raising serious security concerns for production deployment.


<details>
  <summary>Details</summary>
Motivation: To investigate whether vibe coding outputs are safe for production deployment, given the increasing adoption of this paradigm where human engineers use LLM agents to complete complex coding tasks with minimal supervision.

Method: Created SUSVIBES benchmark with 200 real-world feature-request tasks from open-source projects that previously led to vulnerable implementations when handled by human programmers. Evaluated multiple widely used coding agents with frontier models on this benchmark, and tested preliminary security strategies like augmenting feature requests with vulnerability hints.

Result: All coding agents performed poorly on security: SWE-Agent with Claude 4 Sonnet achieved 61% functional correctness but only 10.5% security. Preliminary security strategies failed to mitigate security issues.

Conclusion: Vibe coding poses serious security risks for production deployment, especially in security-sensitive applications, as LLM agents consistently produce vulnerable code despite functional correctness.
...
### [163] [PanFoMa: A Lightweight Foundation Model and Benchmark for Pan-Cancer](https://arxiv.org/abs/2512.03111)
*Xiaoshui Huang,Tianlin Zhu,Yifan Zuo,Xue Xia,Zonghan Wu,Jiebin Yan,Dingli Hua,Zongyi Xu,Yuming Fang,Jian Zhang*

Main category: q-bio.GN

TL;DR: PanFoMa is a lightweight hybrid neural network combining Transformers and state-space models for efficient single-cell representation learning, with a comprehensive pan-cancer benchmark containing 3.5M+ cells across 33 cancer subtypes.


<details>
  <summary>Details</summary>
Motivation: Current pan-cancer scRNA-seq research faces two key challenges: 1) learning discriminative and efficient single-cell representations, and 2) lacking a comprehensive evaluation benchmark for robust model assessment.

Method: PanFoMa uses a modular hybrid architecture: front-end local-context encoder with shared self-attention layers captures complex gene interactions, and back-end global sequential feature decoder uses linear-time state-space model (Mamba) for efficient global context integration. Also constructs PanFoMaBench with 3.5M+ high-quality cells across 33 cancer subtypes.

Result: PanFoMa outperforms SOTA models on pan-cancer benchmark (+4.0%) and multiple public tasks: cell type annotation (+7.4%), batch integration (+4.0%), and multi-omics integration (+3.1%).

Conclusion: PanFoMa successfully balances performance and efficiency for single-cell representation learning, while the comprehensive benchmark enables robust evaluation of pan-cancer models.
...
### [164] [LATTICE: Democratize High-Fidelity 3D Generation at Scale](https://arxiv.org/abs/2512.03052)
*Zeqiang Lai,Yunfei Zhao,Zibo Zhao,Haolin Liu,Qingxiang Lin,Jingwei Huang,Chunchao Guo,Xiangyu Yue*

Main category: cs.GR

TL;DR: LATTICE is a new 3D generation framework that uses VoxSet representation to bridge quality gaps between 2D and 3D generation, achieving SOTA performance with efficient training and flexible inference.


<details>
  <summary>Details</summary>
Motivation: 3D generation is more challenging than 2D due to needing to predict both spatial structure and detailed geometry from scratch. Existing 3D representations have computational complexity issues and lack structured, scalable encoding schemes, creating a quality and scalability gap between 2D and 3D generative models.

Method: Proposes VoxSet, a semi-structured representation that compresses 3D assets into latent vectors anchored to a coarse voxel grid for position-aware generation. LATTICE uses a two-stage pipeline: first generates sparse voxelized geometry anchor, then produces detailed geometry using a rectified flow transformer.
...
Conclusion: LATTICE offers a significant step toward scalable, high-quality 3D asset creation by bridging the quality and scalability gap between 2D and 3D generative models through a simple yet effective framework with structured latent representation.
...
### [165] [Radiance Meshes for Volumetric Reconstruction](https://arxiv.org/abs/2512.04076)
*Alexander Mai,Trevor Hedstrom,George Kopanas,Janne Kontkanen,Falko Kuester,Jonathan T. Barron*

Main category: cs.GR

TL;DR: Radiance meshes use Delaunay tetrahedralization for constant-density radiance field representation, enabling exact volume rendering via rasterization/ray-tracing with real-time performance on consumer hardware.


<details>
  <summary>Details</summary>
Motivation: To create a radiance field representation that enables exact and fast volume rendering using standard hardware, overcoming limitations of previous methods while supporting various applications like fisheye distortion, simulation, and editing.

Method: Use Delaunay tetrahedralization to create constant-density tetrahedral cells, develop new rasterization method for faster rendering, employ Zip-NeRF-style backbone to handle topological discontinuities from vertex optimization, and enable exact evaluation of volume rendering equation.

Result: Achieves faster rendering speeds than prior radiance field representations across various platforms, enables high-quality real-time view synthesis on standard consumer hardware, and supports applications like fisheye lens distortion, physics simulation, editing, and mesh extraction.

Conclusion: Radiance meshes provide an efficient, hardware-friendly representation for radiance fields that enables real-time rendering and opens up new application possibilities beyond traditional view synthesis.
...
TL;DR: M3DR is a multilingual multimodal document retrieval framework that addresses English-centric limitations by enabling cross-lingual alignment across 22 diverse languages using synthetic data and contrastive training.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal document retrieval systems are heavily English-centric, limiting their effectiveness in multilingual contexts and diverse linguistic/cultural settings.

Method: M3DR leverages synthetic multilingual document data with contrastive training to learn unified text-document image representations that transfer across languages, working across different vision-language architectures and model sizes.

Result: Models (NetraEmbed and ColNetraEmbed) achieve state-of-the-art performance with ~150% relative improvements on cross-lingual retrieval, validated across 22 typologically diverse languages with consistent performance across linguistic/script variations.

Conclusion: M3DR successfully bridges the multilingual gap in multimodal document retrieval, demonstrating robust cross-lingual and cross-modal alignment capabilities that generalize across different retrieval paradigms and real-world multilingual scenarios.
...
TL;DR: A kaleidoscopic scintillator design that uses mirror reflections to increase light collection for single-photon cameras, enabling 3D event localization in radiation imaging.


<details>
  <summary>Details</summary>
Motivation: Existing scintillator imaging methods face a trade-off: single-pixel detectors provide timing but no spatial resolution, while cameras provide spatial resolution but only average over many events. Single-photon cameras combine both but struggle with very low photon counts from individual events.

Method: Proposes a kaleidoscopic scintillator geometry that creates mirror reflections of scintillation events in known locations. Develops theory for imaging events in this geometry and an algorithm to estimate the 3D position of events from the captured reflections.

Result: The kaleidoscopic design provides sufficient light collection to perform high-resolution event measurements using commercial CMOS single-photon cameras, enabling advanced radiation imaging techniques.

Conclusion: The kaleidoscopic scintillator enables machine vision analysis of individual radiation events by overcoming low photon count limitations, opening new possibilities for radiation detection and imaging.
...
### [168] [Multi-Agent Reinforcement Learning and Real-Time Decision-Making in Robotic Soccer for Virtual Environments](https://arxiv.org/abs/2512.03166)
*Aya Taourirte,Md Sohag Mia*

Main category: cs.RO

TL;DR: A unified MARL framework combining PPO, hierarchical RL, and mean-field theory for scalable multi-agent systems in dynamic environments like robotic soccer.


<details>
  <summary>Details</summary>
Motivation: Multi-agent systems in dynamic adversarial environments need real-time decision-making, cooperation, and scalable algorithms to avoid dimensionality curse. Existing RL methods struggle with multi-granularity tasks (long-term strategy vs instant actions) and complex large-scale agent interactions.

Method: 1) Baseline PPO in client-server architecture for real-time action scheduling. 2) Hierarchical RL structure using options framework to decompose into high-level trajectory planning (Semi-Markov Decision Process) and low-level action execution. 3) Mean-field theory integration into HRL to simplify many-agent interactions into single agent vs population average.
...
Conclusion: The unified MARL framework demonstrates robust, scalable, and cooperative behavior in complex multi-agent domains, addressing multi-granularity tasks and scalability challenges through hierarchical decomposition and mean-field simplification.
...
### [169] [What Is The Best 3D Scene Representation for Robotics? From Geometric to Foundation Models](https://arxiv.org/abs/2512.03422)
...
TL;DR: A comprehensive survey of 3D scene representation methods for robotics, covering traditional approaches (point clouds, voxels, SDF, scene graphs) and neural representations (NeRF, 3DGS, Foundation Models), analyzing their applications across five robotic modules and discussing future trends toward unified 3D Foundation Models.


<details>
  <summary>Details</summary>
Motivation: To provide a systematic overview and comparison of scene representation methods for robotics, addressing the question of what constitutes the best 3D scene representation for robotic applications, and to explore future trends toward unified foundation models.

Method: Categorizes robotics into five core modules (Perception, Mapping, Localization, Navigation, Manipulation), presents standard formulations of different scene representation methods, compares their advantages/disadvantages across modules, and discusses future development trends with focus on 3D Foundation Models.

Result: Provides a comprehensive survey that serves as a valuable resource for researchers, identifies current reliance on sparse representations in SLAM/localization, highlights the potential of dense representations for downstream tasks, and emphasizes neural representations' suitability for semantic integration and language-based priors.

Conclusion: The paper concludes that while current systems use sparse representations, future robotics will benefit from unified 3D Foundation Models that can integrate semantic features and language priors for comprehensive scene understanding, though challenges remain in fully realizing this vision. The authors provide an open-source project for ongoing updates.
...
### [170] [MSG-Loc: Multi-Label Likelihood-based Semantic Graph Matching for Object-Level Global Localization](https://arxiv.org/abs/2512.03522)
*Gihyeon Lee,Jungwoo Lee,Juwon Kim,Young-Sik Shin,Younggun Cho*

Main category: cs.RO

TL;DR: Multi-label semantic graph matching framework for robot global localization that handles semantic ambiguity by using multi-label representations and context-aware likelihood propagation.


<details>
  <summary>Details</summary>
Motivation: Robots need to localize in environments with unknown object classes and semantic ambiguity. High semantic ambiguity in object-level global localization leads to misclassification and incorrect associations, causing significant pose estimation errors.

Method: Proposes a multi-label likelihood-based semantic graph matching framework using multi-label graph representations instead of single-label ones. Uses context-aware likelihood propagation that combines node likelihood with maximum likelihood of neighbors to enhance semantic correspondence across graphs.
...
Conclusion: The multi-label semantic graph matching framework effectively handles semantic ambiguity in object-level global localization, improving performance in both closed-set and open-set scenarios while scaling to large-vocabulary environments.
...
### [171] [RoboScape-R: Unified Reward-Observation World Models for Generalizable Robotics Training via RL](https://arxiv.org/abs/2512.03556)
...
<details>
  <summary>Details</summary>
Motivation: Both Imitation Learning and Reinforcement Learning struggle with policy generalization across diverse scenarios. IL overfits to expert trajectories, while RL lacks unified reward signals for multi-scene generalization. World models could serve as universal environment proxies but current implementations still rely on task-specific rewards.

Method: Proposes RoboScape-R framework that leverages world models as versatile environment proxies within RL. Introduces novel world model-based general reward mechanism that generates "endogenous" rewards derived from the model's intrinsic understanding of real-world state transition dynamics.

Result: Extensive experiments show RoboScape-R effectively addresses traditional RL limitations by providing efficient and general training environment that substantially enhances embodied policy generalization. Achieves average 37.5% performance improvement over baselines under out-of-domain scenarios.

Conclusion: World models can serve as effective online training strategies for embodied AI. The endogenous reward mechanism derived from world model dynamics provides a general-purpose solution to the reward specification problem in RL, enabling better policy generalization across diverse scenarios.
...
### [172] [Artificial Microsaccade Compensation: Stable Vision for an Ornithopter](https://arxiv.org/abs/2512.03995)
*Levi Burner,Guido de Croon,Yiannis Aloimonos*

Main category: cs.RO

TL;DR: Artificial Microsaccade Compensation stabilizes shaky drone video by optimizing 3D rotations in real-time, outperforming commercial software.


<details>
  <summary>Details</summary>
Motivation: Inspired by biological microsaccades in foveated vision, the paper addresses the challenge of stabilizing video from shaking tailless ornithopters (12-20 Hz) that resist conventional camera-based sensing.

Method: Develops Artificial Microsaccade Compensation that minimizes changes in image intensity by optimizing over 3D rotation represented in SO(3). Uses efficient recursive updates and can adapt to hold fixed viewing orientation with occasional saccades.

Result: Achieves real-time stabilized video suitable for human viewing without distortion. Dramatically reduces inter-frame motion and outperforms Adobe Premier Pro's warp stabilizer in quality while running in real time.

Conclusion: The biologically-inspired approach successfully stabilizes challenging drone video in real-time, offering superior performance to leading commercial solutions.
...
### [173] [Culture Affordance Atlas: Reconciling Object Diversity Through Functional Mapping](https://arxiv.org/abs/2512.03173)
*Joan Nwatu,Longju Bai,Oana Ignat,Rada Mihalcea*

Main category: cs.CY

TL;DR: A function-centric framework using the Culture Affordance Atlas reduces cultural biases in VL datasets by categorizing objects by their functions across diverse contexts, improving model performance for lower-income communities.


<details>
  <summary>Details</summary>
Motivation: Mainstream Vision-Language datasets exhibit cultural biases favoring Western, higher-income contexts, which reduces model generalizability and creates performance disparities that disproportionately impact lower-income and non-Western communities.

Method: Proposed a function-centric framework that categorizes objects by the functions they fulfill across diverse cultural and economic contexts. Implemented this by creating the Culture Affordance Atlas, a re-annotated restructuring of the Dollar Street dataset spanning 46 functions and 288 objects.

Result: Function-centric labels substantially reduce socioeconomic performance gaps between high- and low-income groups by a median of 6 percentage points (statistically significant), improving model effectiveness for lower-income contexts. Also identified numerous culturally essential objects overlooked in prominent VL datasets.

Conclusion: The function-centric framework offers a scalable pathway toward building inclusive VL datasets and equitable AI systems by addressing cultural biases and improving model performance across diverse socioeconomic contexts.
...
### [174] [LLM-Generated Ads: From Personalization Parity to Persuasion Superiority](https://arxiv.org/abs/2512.03373)
*Elyas Meguellati,Stefano Civelli,Lei Han,Abraham Bernstein,Shazia Sadiq,Gianluca Demartini*

Main category: cs.CY

TL;DR: LLMs achieve parity with humans in personality-based advertising but significantly outperform humans in psychological persuasion principles, especially authority and consensus appeals.


<details>
  <summary>Details</summary>
Motivation: As LLMs become increasingly capable of generating persuasive content, understanding their effectiveness across different advertising strategies is critical for advertising practice.

Method: Two-part investigation: (1) Study 1 (n=400) tested LLM-generated personalized ads tailored to specific personality traits (openness and neuroticism) vs human experts; (2) Study 2 (n=800) tested LLM performance across four psychological persuasion principles (authority, consensus, cognition, scarcity) vs human-created content.

Result: Study 1: LLM-generated ads achieved statistical parity with human-written ads (51.1% vs 48.9%, p>0.05). Study 2: AI-generated ads significantly outperformed human content (59.1% vs 40.9%, p<0.001), with strongest performance in authority (63.0%) and consensus (62.5%) appeals. AI's advantage stems from more sophisticated, aspirational messages and superior visual-narrative coherence.

Conclusion: LLMs have evolved from parity in personalization to superiority in persuasive storytelling, with significant implications for advertising practice given their near-zero marginal cost and time requirements compared to human experts.
