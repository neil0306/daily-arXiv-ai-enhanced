...
Motivation: To understand the origins of cognitive biases in LLMs and whether they arise from pretraining, finetuning, or training randomness.

Method: A two-step approach: (1) finetuning models with different random seeds to assess randomness effects, and (2) cross-tuning (swapping instruction datasets) to isolate bias sources.

Result: Biases are mainly influenced by pretraining, not finetuning or randomness, as models with the same pretrained backbone show similar bias patterns.

Conclusion: Understanding biases in LLMs requires considering pretraining origins, guiding future strategies for bias evaluation and mitigation.
...
<details>
  <summary>Details</summary>
Motivation: To understand the robustness of LLMs as proxies for human subjects in normative surveys and their vulnerability to known response biases.
...
<details>
  <summary>Details</summary>
Motivation: The need for consistent and principled evaluation of synthetic text to ensure its utility, fairness, privacy, and alignment with source data in high-stakes domains.

Method: The toolkit evaluates synthetic text on utility, fairness, privacy risk, distributional differences, and expert feedback. It supports user-uploaded or toolkit-generated data.

Result: Demonstrated functionality and effectiveness in healthcare and law datasets, standardizing evaluation metrics.

Conclusion: SynthTextEval improves synthetic text viability and privacy-preservation in AI development.
...
<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in medical applications raises safety concerns due to their direct impact on human health and diverse user roles (patients, clinicians).
...
Result: The work establishes the first safety evaluation criteria for medical LLMs from patient, clinician, and general user perspectives.

Conclusion: This study lays the foundation for safer deployment of LLMs in medical domains by addressing domain-specific safety concerns.
...
<details>
  <summary>Details</summary>
Motivation: Interruptions are key in collaborative learning, but existing AI methods struggle with overlapping speech in multi-group environments.
...
Result: Creates a robust interruption identification method and provides insights into linguistic and prosodic patterns of interruptions.

Conclusion: The work advances interruption detection for classrooms and sets groundwork for future research on multi-group dialogue tracking.
...
<details>
  <summary>Details</summary>
Motivation: Transformers are inefficient for long texts due to quadratic complexity. The study aims to enhance time, cost, and energy efficiency in DL.

Method: Integrates GNNs and CNNs with real-time graph generation, uses LLM inputs, and employs CNNs for local patterns and small-world graphs for document-level info.
...
Conclusion: The proposed model is efficient and performs competitively, offering a viable alternative to Transformers for long-text processing.
...
Method: Combines LLMs, human-in-the-loop annotation, and APO to extract eviction data from clinical notes.

Result: Achieved high Macro-F1 scores (88.8% for eviction, 90.3% for other SDoH), outperforming GPT-4o and BioBERT, while reducing annotation effort by 80%.

Conclusion: The pipeline is cost-effective, scalable, and generalizable to other information extraction tasks.
...
Method: Uses a synthetic dataset of mean-reverting time series with varied trends and noise, annotated by a large multimodal model, to fine-tune compact Qwen models.
...
Method: Proposes SAND, using self-consistency action sampling and execution-guided critique for step-wise deliberation.

Result: Achieves 20% improvement over initial methods and outperforms state-of-the-art approaches.

Conclusion: SAND enhances LLM agent performance by promoting better action deliberation.
...
<details>
  <summary>Details</summary>
Motivation: Training RL for large language models is unstable and energy-intensive, with policies drifting from pretrained weights. RLEP addresses this by leveraging verified trajectories.
...
Result: RLEP improves accuracy on benchmarks (AIME-2024: 38.2% to 39.9%, AIME-2025: 19.8% to 22.3%, AMC-2023: 77.0% to 82.2%) with fewer updates.

Conclusion: RLEP enhances RL training for language models by stabilizing learning and boosting performance, with publicly available resources for reproducibility.
...
Method: Introduces the Bullshit Index and a taxonomy of four bullshit forms. Evaluates using datasets like Marketplace, Political Neutrality, and BullshitEval (2,400 scenarios).

Result: RLHF fine-tuning exacerbates bullshit; CoT prompting amplifies empty rhetoric and paltering. Political contexts show prevalent bullshit, dominated by weasel words.
...
<details>
  <summary>Details</summary>
Motivation: Leveraging planning structures from large LLMs to enhance smaller models' performance remains underexplored.
...
Conclusion: PLAN-TUNING effectively enhances smaller LLMs' complex reasoning capabilities through planning trajectories.
...
Method: TeaR uses data curation and reinforcement learning to guide models in reasoning tasks.

Result: Significant performance improvements, e.g., 35.9% on Qwen2.5-7B and 5.9% on R1-Distilled-7B across 17 benchmarks.

Conclusion: TeaR effectively enhances reasoning in LLMs by refining reasoning paths and reducing algorithmic overfitting.
...
Method: Analyzes the computational complexity of LLM inference, presenting examples of tasks beyond their capability and verification limits.

Result: LLMs are incapable of performing or verifying tasks beyond a specific complexity threshold.

Conclusion: The findings highlight limitations in LLM capabilities, impacting their use in autonomous or semi-autonomous agentic tasks.
...
<details>
  <summary>Details</summary>
Motivation: Address the scarcity of psychological support datasets, especially in non-English languages, to improve mental health support systems.

Method: Leverages limited real-world data and expert knowledge to fine-tune two models (Dialog Generator and Dialog Modifier) for creating and refining dialogues. Constructs CPsDD and develops CADSS with specialized agents for user profiling, summarization, strategy planning, and response generation.

Result: CPsDD contains 68K dialogues across diverse psychological categories. CADSS achieves state-of-the-art performance in Strategy Prediction and Emotional Support Conversation tasks on CPsDD and ESConv datasets.

Conclusion: The framework effectively addresses dataset scarcity and enhances psychological support systems, demonstrating superior performance in key tasks.
...
Method: Trained multiple VAP models on a Japanese triadic conversation dataset, focusing on predicting future voice activity using acoustic data.
...
<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of LLMs with few-shot prompting for multilingual subjectivity detection, particularly in challenging data conditions.
...
Method: The study uses benchmarks with all language combinations for queries and documents, analyzing multilingual retrieval behavior. A retrieval strategy enforces equal retrieval from both languages.

Result: Retrieval is a critical bottleneck, with performance drops when query and document languages differ. The proposed strategy improves cross-lingual and overall performance.
...
Result: SFT improves performance on difficult questions but degrades simpler ones, while RL promotes generalization and brevity. Combining them leads to trade-offs without additive benefits.
...
Method: Aligns an image-only encoder with MLLM's multimodal representations, leveraging pre-trained knowledge for efficient training and inference.
...
Conclusion: M4Doc effectively enhances DIMT performance by leveraging MLLMs' multimodal knowledge while maintaining computational efficiency.
...
Result: Achieves test perplexity of 8.8 on WikiText-2 with K=8, outperforming GPT-2 Small (20.3) with a similar model size.

Conclusion: The method provides a consistent, efficient way to enhance language model performance and uncertainty estimation by leveraging Bayesian principles.
...
Method: Evaluate student models (from Pythia and Qwen2.5 families) on SQuAD and MLQA benchmarks under zero-shot and one-shot prompting.

Result: Student models retain >90% of teacher performance with up to 57.1% parameter reduction. One-shot prompting outperforms zero-shot.

Conclusion: KD with minimal prompting enables efficient, capable QA systems for resource-constrained environments.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the overlooked efficiency metric (number of retrieval searches) in RAG systems, while improving accuracy and recall.

Method: Uses a standard ReAct pipeline with improved prompts and explores supervised/RL-based fine-tuning for frugality.

Result: Outperforms state-of-the-art methods on benchmarks like HotPotQA and achieves competitive metrics at half the search cost.

Conclusion: Demonstrates that large-scale fine-tuning is unnecessary for RAG improvement and highlights frugality benefits of supervised/RL-based fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: PCR is a growing challenge in Chinese content moderation, but existing evaluations lack real-world creativity. The study aims to provide a realistic benchmark and improve detection methods.

Method: Organizes PCR into a taxonomy, compiles a dataset of 500 real PCR posts, benchmarks LLMs, and tests Pinyin-based prompting.
...
<details>
  <summary>Details</summary>
Motivation: Current summarization evaluation methods often rely on human-generated references, which are time-consuming. NOIR aims to automate this process while capturing the tradeoff between semantic retention and compression.

Method: NOIR uses a language model-embedding to measure semantic similarity and combines it with summary length compression to quantify summarization quality.

Result: Experiments show NOIR effectively captures the tradeoff between token-length and semantic retention, correlating with human perception of quality.

Conclusion: NOIR provides a scalable, automated tool for evaluating and improving summarization tasks without human references.
...
<details>
  <summary>Details</summary>
Motivation: The performance of multi-head attention (MHA) improves with more heads and larger hidden sizes, but this increases parameters. The goal is to achieve similar gains without parameter overhead.
...
Conclusion: SAS effectively boosts Transformer performance by simulating larger models while maintaining compact size, validated by extensive experiments.
...
<details>
  <summary>Details</summary>
Motivation: To verify Vec2Text's claims about reconstructing texts from embeddings and explore its privacy implications, given the opaque nature of embedding spaces.

Method: Reproduce Vec2Text framework, validate original claims, and extend the study with experiments on parameter sensitivity, sensitive input reconstruction, and embedding quantization as a defense.
...
Conclusion: Text embeddings pose privacy risks; defenses like quantization are needed, and further research into robust NLP defenses is crucial.
...
<details>
  <summary>Details</summary>
Motivation: Not all tokens equally impact LLM performance; selective alignment can optimize efficiency and fidelity.

Method: Uses token-level log-probability differences between policy and reference models to prioritize high-impact tokens.

Result: Outperforms standard DPO and distillation baselines on benchmarks like Arena-Hard and MT-Bench.

Conclusion: Token-level optimization and strong reference models are key for effective LLM alignment.
...
<details>
  <summary>Details</summary>
Motivation: To systematically review and unify historical and contemporary research on LLMs in law, addressing gaps and challenges while proposing innovative solutions.
...
Conclusion: Provides a technical roadmap and conceptual framework for future legal AI, with a focus on low-resource systems, multimodal integration, and dynamic rebuttal handling.
...
<details>
  <summary>Details</summary>
Motivation: Existing StreamST methods rely on sentence-level segments (SimulST), limiting context and policy learning due to speech complexity and cross-lingual challenges.

Method: StreamUni integrates speech CoT into an LSLM to generate multi-stage outputs, enabling simultaneous segmentation, policy decisions, and translation without extensive policy training. A streaming CoT training method enhances low-latency capabilities.

Result: StreamUni achieves state-of-the-art performance on StreamST tasks.

Conclusion: StreamUni effectively addresses the limitations of SimulST by unifying segmentation, policy, and translation in a single model, improving latency and quality.
...
Result: The model generates valid formulae quickly (1 epoch) and generalizes logic semantics in ~10 epochs, producing simpler yet semantically close formulae.

Conclusion: The method effectively captures semantic information, generalizes well, and solves requirement mining tasks by optimizing directly in the semantic space.
...
<details>
  <summary>Details</summary>
Motivation: Prior work focused on attention heads, but this study investigates skill neurons (repetition neurons) to understand their role in ICL performance.

Method: Experiments analyze repetition neurons' influence on ICL, comparing their effects to induction heads across different layer depths.

Result: Repetition neurons' impact on ICL varies by layer depth; strategies are identified to reduce repetitive outputs without compromising ICL.

Conclusion: The study highlights the nuanced role of repetition neurons in ICL and offers insights for optimizing LLM outputs.
...
<details>
  <summary>Details</summary>
Motivation: The conventional auto-regressive loss function in instruction tuning may not be optimal, as it ignores prompt tokens. This work explores the impact of weighting prompt and response tokens differently.

Method: The authors systematically investigate loss weighting for prompt and response tokens, proposing WIT. Experiments involve five language models, three datasets, and five benchmarks.

Result: Standard instruction tuning loss is suboptimal. A balanced weighting (low-moderate for prompts, moderate-high for responses) yields better performance and robustness, also aiding preference alignment.

Conclusion: The findings suggest rethinking instruction tuning loss functions for more robust and generalizable models, with WIT offering a practical solution.
...
Method: Extends unigram tokenization by learning a target tokenizer that maximizes cross-lingual alignment, evaluated on four language pairs for intrinsic and downstream tasks.

Result: No improvement in machine translation quality but consistent perplexity reductions in language modeling; data efficiency bottleneck due to quadratic scaling.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of coreferential complexity in RAG systems, which disrupts retrieval and generation by introducing ambiguity.

Method: Systematic investigation of coreference effects on retrieval and generation, testing pooling strategies and QA performance with coreference resolution.

Result: Coreference resolution improves retrieval and QA. Mean pooling excels in context capture, and smaller models gain more from disambiguation.

Conclusion: Coreference resolution enhances RAG performance, offering insights for improving knowledge-intensive AI applications.
...
<details>
  <summary>Details</summary>
Motivation: To bridge AI systems and documents through game-theoretic reasoning, leveraging fixed-point iteration and hierarchical sub-games.

Method: Introduces a composite operator for semantic convergence and sub-game resolution, supported by category theory and information theory.

Result: Proves existence and uniqueness of semantic equilibria, with practical applicability in AI cognition models.

Conclusion: The framework naturally embeds game-theoretic reasoning in AI systems, validated by theoretical and practical tools.
...
<details>
  <summary>Details</summary>
Motivation: Existing CHAs lack adaptive reasoning and transparency, limiting their clinical use. DocCHA aims to address this by emulating clinical reasoning.
...
Method: Leverages LLMs (Gemini 2.0 Flash), Python scripting, and Selenium for web automation to generate NAMD input files via CHARMM GUI.
...
<details>
  <summary>Details</summary>
Motivation: Existing VLMs struggle with industrial anomaly detection due to domain-specific challenges, lacking interpretable explanations and generalization to unseen categories.
...
<details>
  <summary>Details</summary>
Motivation: Existing AI memory solutions are limited in personalization, abstraction, and reliable recall, prompting the need for a more advanced system like MIRIX.

Method: MIRIX employs six structured memory types (Core, Episodic, Semantic, Procedural, Resource Memory, Knowledge Vault) and a multi-agent framework for dynamic control and coordination.

Result: MIRIX achieves 35% higher accuracy on ScreenshotVQA and 85.4% on LOCOMO, outperforming baselines while reducing storage by 99.9%.

Conclusion: MIRIX sets a new standard for memory-augmented AI agents, validated by superior performance and practical applications.
...
<details>
  <summary>Details</summary>
Motivation: To understand the root cause of the generalization gap between IM-RMs and EX-RMs, despite their near-identical setups.
...
Result: IM-RMs generalize worse due to their dependence on superficial token-level cues, even in-distribution, and alternative hypotheses for the gap are disproven.
...
Conclusion: While SLMs with RAG are promising for clinical support, expert oversight remains crucial due to inconsistent specialist-level accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating LLMs' medical reasoning lack rigor and scalability, necessitating a robust benchmark for clinical decision-making.

Method: MedThink-Bench includes 500 questions with expert rationales. The LLM-w-Ref framework uses fine-grained rationales and LLM-as-a-Judge mechanisms for scalable, high-fidelity evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Prior approaches in visual reasoning are limited by static toolsets and workflows, prompting the need for dynamic, interpretable problem-solving.

Method: PyVision allows MLLMs to generate, execute, and refine Python-based tools interactively, tailored to specific tasks.

Result: PyVision boosts GPT-4.1 by +7.8% on V* and Claude-4.0-Sonnet by +31.1% on VLMsAreBlind-mini.

Conclusion: Dynamic tooling advances agentic visual reasoning by enabling models to invent tools, not just use them.
...
<details>
  <summary>Details</summary>
Motivation: UDA for semantic segmentation lacks utilization of domain-agnostic text properties despite advances in vision-language models.

Method: Proposes CoPT, which uses LLM-generated domain descriptions and CLIP embeddings to create domain-invariant features.
...
<details>
  <summary>Details</summary>
Motivation: Address ethical, legal, and societal concerns of IGMs by exploring the robustness of machine unlearning (MU) methods against adversarial inputs.

Method: Proposes Recall, a framework optimizing adversarial image prompts using multi-modal conditioning of diffusion models, tested on ten unlearning methods.
...
<details>
  <summary>Details</summary>
Motivation: To promote transparency and trust in DL models for clinical adoption by addressing overlooked aspects in existing XAI surveys.
...
Result: A structured synthesis of XAI methods, evaluation metrics, open-source frameworks, and critical discussion of challenges.

Conclusion: Provides a timely foundation for advancing interpretable DL in biomedical image analysis, highlighting future directions.
...
<details>
  <summary>Details</summary>
Motivation: To address hallucinations in MLLMs by investigating inherent modality conflicts in inputs, which existing works overlook.
...
<details>
  <summary>Details</summary>
Motivation: The need for autonomous surveillance and target identification in GNSS-denied environments, crucial for applications like search and rescue and threat detection, drives this work.

Method: The method uses YOLOv8 for vessel detection, followed by feature matching and hue histogram distance analysis to identify the target vessel, and geometric principles for localization.
...
Conclusion: The proposed approach is viable for autonomous maritime surveillance in GNSS-denied scenarios, with potential for further refinement based on perspective impact analysis.
...
<details>
  <summary>Details</summary>
Motivation: Accurate polyp segmentation is vital for colorectal cancer diagnosis, but existing methods rely on additional labeled data or task similarity, limiting generalizability.

Method: CL-Polyp uses contrastive learning to enhance feature extraction and introduces MASPP and CA modules for multi-scale feature fusion and boundary reconstruction.

Result: CL-Polyp outperforms existing methods, improving IoU by 0.011 and 0.020 on Kvasir-SEG and CVC-ClinicDB datasets.

Conclusion: CL-Polyp is effective for clinical polyp segmentation, offering better performance without extra annotations.
...
<details>
  <summary>Details</summary>
Motivation: To overcome EEG's spatial limitations for image reconstruction by leveraging semantic captions and aligning brain signals with interpretable AI.

Method: Aligns EEG signals with multilevel semantic captions using a transformer-based encoder and contrastive learning, then conditions a latent diffusion model for image generation.
...
Conclusion: Semantic mediation enables cognitively aligned visual decoding from EEG, bridging neuroscience and AI.
...
TL;DR: The paper analyzes challenges in generating long-form videos (beyond 16 seconds) and identifies methods to improve character consistency, motion coherence, and temporal diversity. It reviews 32 papers to propose a taxonomy and comparative analysis.


<details>
  <summary>Details</summary>
Motivation: Existing video generative models struggle with long-form videos, especially in maintaining character consistency, scene layouts, and motion coherence. The goal is to identify effective architectural components and training strategies.

Method: The study reviews 32 papers on video generation, categorizing them by architectural designs and performance. It constructs a taxonomy and comparative tables.

Result: The analysis highlights key components for generating long-form videos with narrative coherence, multi-character consistency, and high fidelity.

Conclusion: The paper provides a taxonomy and comparative insights to guide future research in improving long-form video generation.
...
Result: Improves baseline by 2.9% on LTCC, 5.0% on PRCC (image ReID), and 1.0% on CCVID, 2.5% on MeVID (video ReID) without extra supervision.
...
TL;DR: An automated video segmentation pipeline for VFX production uses ML for object detection, image segmentation, and video tracking, reducing manual effort and improving efficiency.
...
Method: The pipeline combines text-prompted object detection, per-frame segmentation, and video tracking, deployed via containerization.

Result: Reduced manual effort, faster preliminary composites, and enhanced segmentation data for VFX production.

Conclusion: The pipeline improves VFX production efficiency by automating mask generation and ensuring temporal consistency.
...
TL;DR: The paper proposes DisenQ, a multimodal language-guided framework for activity-biometrics, disentangling identity, motion, and non-biometrics features using structured textual supervision, achieving state-of-the-art performance.
...
Conclusion: The framework effectively replaces reliance on visual data with textual supervision, ensuring robust identity cues independent of appearance and motion.
...
TL;DR: LinguaMark benchmark evaluates multilingual capabilities of Large Multimodal Models (LMMs) using a VQA task across 11 languages, revealing closed-source models like GPT-4o and Gemini2.5 perform best, while open-source Qwen2.5 excels in multilingual generalization.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of emphasis on multilingual evaluation in LMMs, which often produce biased outputs due to limited linguistic coverage.

Method: Introduces LinguaMark, a benchmark with 6,875 image-text pairs in 11 languages, evaluating models on Bias, Answer Relevancy, and Faithfulness.

Result: Closed-source models (GPT-4o, Gemini2.5) lead overall, while open-source Qwen2.5 shows strong multilingual generalization.

Conclusion: The benchmark and code are released to promote reproducibility and advance multilingual LMM research.
...
<details>
  <summary>Details</summary>
Motivation: To determine if vision-language models genuinely perform visual reasoning or rely on biases, addressing gaps in current evaluation methods.

Method: Introduces MagiC with 5,500 weakly supervised and 900 human-curated QA examples, evaluating 15 models on correctness, reasoning, grounding, and self-correction.

Result: Reveals limitations in current models' reasoning and grounding abilities, with new metrics (MagiScore, StepSense) providing deeper insights.

Conclusion: MagiC highlights gaps in grounded visual reasoning and offers tools for future model improvements.
...
Method: Generates a large-scale dataset (100K+ samples) to fine-tune a modified LLaVA-NeXT-8B model, decoding numeric scores from custom tokens.

Result: Outperforms open-source VLMs and Gemini-Pro 1.5, improving score correlation with human ratings by 17.24% and pair-wise comparison accuracy by up to 9.35%.

Conclusion: The scorer enhances evaluation accuracy, acts as a reward model, and boosts performance in tasks like best edit selection and model fine-tuning.
...
TL;DR: A novel method for virtual try-on (VTO) in beauty apps improves foundation-skin tone blending using Kubelka-Munk theory, offering scalability and realism.


<details>
  <summary>Details</summary>
Motivation: To enhance VTO applications by addressing the challenge of realistic foundation-skin tone blending and scalability across diverse products.
...
Conclusion: The method effectively balances realism and scalability, advancing VTO technology in the beauty industry.
...
TL;DR: The paper addresses inconsistency in visual storytelling by proposing a contrastive reinforcement learning method to improve entity identity recognition across frames, showing significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Visual storytelling models often fail to maintain consistent character and object identities across frames, leading to referential hallucinations. This is due to a lack of explicit training on entity connections.

Method: A contrastive reinforcement learning approach is introduced, using synthetic negative examples and Direct Preference Optimization with a dual-component reward function to train models for coherent entity connections.

Result: The method improved grounding mAP by 14.8%, F1 by 17.1%, and cross-frame entity persistence by 13.7%. Well-structured stories increased by 23.3%.

Conclusion: The proposed framework effectively enhances entity identity consistency in visual storytelling, demonstrating measurable improvements in model performance.
...
<details>
  <summary>Details</summary>
Motivation: Training generalizable depth completion models requires large-scale datasets with metric depth labels, which are labor-intensive to collect. PacGDC aims to reduce this dependency.

Method: PacGDC synthesizes pseudo geometries by manipulating scene scales of depth maps using multiple depth foundation models, interpolation, relocation, and unlabeled images.

Result: PacGDC achieves strong generalizability across diverse benchmarks, excelling in various scene semantics, scales, and depth patterns under zero-shot and few-shot settings.

Conclusion: PacGDC offers a scalable and efficient solution for depth completion, reducing reliance on large annotated datasets while maintaining high performance.
...
TL;DR: The paper introduces two adaptive mechanisms for particle-based shape modeling to improve representation of anatomical variability while maintaining consistent particle configurations.
...
Method: The paper proposes (1) a neighborhood correspondence loss for high adaptivity and (2) a geodesic correspondence algorithm to regularize optimization.

Result: The approach is evaluated on challenging datasets, showing improved surface representation accuracy and correspondence metrics.

Conclusion: The introduced mechanisms enhance adaptivity in particle-based shape modeling, addressing limitations of existing methods.
...
Result: MSAGSM improves performance across five benchmarks with minimal overhead, achieving state-of-the-art results.

Conclusion: MSAGSM is an effective, plug-and-play solution for PES, and the TTA dataset advances the field with precise annotations.
...
Method: Combines a global branch (Transformer-based temporal aggregation) and a local branch (dynamic body region segmentation based on keypoints) for fine-grained features.
...
TL;DR: A novel framework for cross-category animal motion transfer preserves species-specific habits, leveraging a generative model with a habit-preservation module and LLM for unobserved species.


<details>
  <summary>Details</summary>
Motivation: Existing motion transfer methods focus on humans and neglect animal habitual behaviors, necessitating a specialized approach.

Method: Proposes a habit-preserved motion transfer framework with a category-specific habit encoder and integrates an LLM for unobserved species.

Result: Evaluated on the DeformingThings4D-skl dataset, the model shows superior performance in preserving habitual behaviors.

Conclusion: The framework effectively bridges the gap in animal motion transfer by preserving species-specific habits and generalizing to new species.
...
Method: Seg-Wild integrates multi-dimensional feature embeddings for 3D Gaussians and uses feature similarity for segmentation. It introduces Spiky 3D Gaussian Cutter (SGC) to smooth anomalies and projects 3D Gaussians to 2D for cutting.

Result: Seg-Wild outperforms previous methods in segmentation and reconstruction quality, as demonstrated by experimental results.

Conclusion: Seg-Wild is effective for in-the-wild scenes, offering improved segmentation and reconstruction, with code made publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for novel view synthesis and amodal completion are multi-stage and inefficient, lacking cross-view dependency consideration. EscherNet++ aims to streamline this with an end-to-end approach.

Method: Uses masked fine-tuning (input-level and feature-level masking) for end-to-end training, enabling novel view synthesis and amodal completion. Integrates with feed-forward image-to-mesh models without extra training.

Result: Achieves state-of-the-art results: PSNR improved by 3.9 and Volume IoU by 0.28 on occluded tasks. Reduces reconstruction time by 95% and generalizes to real-world occluded reconstruction.

Conclusion: EscherNet++ offers a scalable, efficient solution for novel view synthesis and amodal completion, outperforming existing methods with fewer resources and better performance.
...
Conclusion: EPIC offers a computationally efficient and effective solution for multimodal tasks, balancing performance and resource usage.
...
Method: Uses a hybrid vision encoder, GateMixer connector, and MCoT-Instruct-287K dataset for fine-tuning with a two-stage CoT approach. Includes self-verification for inference-time scaling.

Result: Corvid surpasses state-of-the-art MLLMs in mathematical reasoning and science problem-solving.

Conclusion: Corvid advances MLLM capabilities in structured reasoning, offering a robust solution for complex tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current benchmarks for 3D anomaly detection focus on low-resolution data, which is inadequate for detecting subtle anomalies in industrial applications.

Method: The authors propose a scalable pipeline to generate realistic 3D anomalies and develop Simple3D, a framework using Multi-scale Neighborhood Descriptors (MSND) and Local Feature Spatial Aggregation (LFSA) for efficient feature extraction.

Result: Simple3D achieves real-time inference (20+ fps) and outperforms state-of-the-art methods on MiniShift and other benchmarks.
...
Method: Proposes DSANet with AFSA (channel-wise similarity for adjacent frames) and LGSA (fusing local and global features) modules.

Result: DSANet outperforms existing methods in segmentation accuracy and achieves higher inference FPS.
...
TL;DR: A Deep Convolutional Neural Network (DCNN) with custom layers is proposed for detecting blue-white veil (BWV) in skin lesions, achieving high accuracy across datasets and enhanced by explainable AI (XAI).
...
Method: Uses a non-annotated dataset converted to annotated via color thresholding, trains a custom DCNN on multiple datasets, and applies XAI for interpretability.
...
TL;DR: Objectomaly is an objectness-aware refinement framework for Out-of-Distribution (OoD) segmentation, improving boundary precision and anomaly scoring.


<details>
  <summary>Details</summary>
Motivation: Existing mask-based OoD segmentation methods struggle with boundary imprecision, inconsistent anomaly scores, and background noise.
...
Result: Achieves state-of-the-art performance on benchmarks (e.g., AuPRC up to 96.99, FPR95 down to 0.07).
...
TL;DR: A novel GAN-based blind face image restoration framework (SFFT-DAFE) improves recognition accuracy in adverse weather by enhancing facial structure and color fidelity.


<details>
  <summary>Details</summary>
Motivation: Adverse weather degrades image quality, reducing face recognition accuracy, and existing methods lack dedicated modules for weather-induced degradations.

Method: Proposes a GAN-based framework with two modules: local Statistical Facial Feature Transformation (SFFT) for structure/color fidelity and Degradation-Agnostic Feature Embedding (DAFE) for robust feature extraction.

Result: Outperforms state-of-the-art methods in suppressing distortions and reconstructing facial structures, validated in challenging weather scenarios.

Conclusion: The SFFT-DAFE framework effectively addresses weather-induced degradations, enhancing face restoration performance.
...
TL;DR: The paper introduces a generative framework for Temporal Unlearnable Examples (TUEs) to protect video data-privacy in Visual Object Tracking (VOT), addressing inefficiencies and limitations of existing image-based methods.


<details>
  <summary>Details</summary>
Motivation: The rise of social media and unauthorized use of private videos for training commercial VOT models highlights the need for effective video data-privacy protection.
...
Conclusion: The TUE framework effectively safeguards video data-privacy, outperforming existing methods and generalizing well across tasks.
...
TL;DR: The paper introduces Online Map Association (OMA), a benchmark for associating global SD maps with online HD maps to enhance autonomous vehicle navigation. It includes a dataset and a baseline method, Map Association Transformer, using attention mechanisms.
...
Result: OMA provides metrics for evaluating models and enhances autonomous vehicle navigation by associating hybrid maps.

Conclusion: The OMA benchmark and Map Association Transformer framework address the gap in hybrid map association, improving autonomous vehicle planning.
...
<details>
  <summary>Details</summary>
Motivation: Existing preference optimization methods for diffusion models are suboptimal, prompting the need for a principled alignment approach.

Method: DMPO aligns diffusion models by minimizing reverse KL divergence, matching the optimization direction of original RL.

Result: DMPO outperforms existing baselines by at least 64.6% in PickScore across datasets.

Conclusion: DMPO provides a robust and elegant solution for aligning diffusion models with human preferences, bridging theory and practice.
...
TL;DR: GGMotion is a group graph dynamics-kinematics network that models human topology in groups to improve motion prediction by leveraging physical dependencies and geometric equivariance.


<details>
  <summary>Details</summary>
Motivation: Existing methods neglect intrinsic physical dependencies between joints, leading to unrealistic motions and increased learning difficulty.

Method: GGMotion uses a radial field for spatial-temporal dependencies, inter/intra-group interaction modules, and equivariant MLPs for joint feature updates. An auxiliary loss supervises motion priors.

Result: The method achieves superior performance on Human3.6M, CMU-Mocap, and 3DPW benchmarks for short-term motion prediction.

Conclusion: GGMotion effectively captures physical plausibility and outperforms existing methods in motion prediction.
...
TL;DR: The paper introduces MUVOD, a multi-view video dataset for 4D object segmentation in dynamic scenes, addressing the lack of labeled data in this field. It includes 17 scenes with diverse views and provides RGB images, segmentation masks, and a benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: The absence of a comprehensive labeled multi-view video dataset for 4D object segmentation in dynamic scenes limits research progress. MUVOD aims to fill this gap.

Method: The dataset comprises 17 scenes with 9-46 views each, totaling 7830 RGB images and segmentation masks. It includes 459 instances of 73 categories and offers a benchmark for evaluation.

Result: MUVOD provides a labeled dataset for 4D object segmentation, enabling tracking across temporal frames and views. It also introduces a subset for 3D object segmentation benchmarking.

Conclusion: MUVOD serves as a foundational benchmark for advancing 4D and 3D object segmentation methods, with publicly available data to support future research.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for trajectory modeling either use opaque neural networks or heuristic-based explicit techniques, lacking spatial coherence and under-exploring implicit representations for sparse signals.

Method: A spline-based trajectory representation with explicit knot degrees of freedom, coupled with a low-rank time-variant spatial encoding, replaces traditional spatiotemporal techniques.

Result: The method excels in temporal interpolation and dynamic scene reconstruction, maintaining motion coherence without heuristic constraints.

Conclusion: The spline-based approach offers a superior alternative to existing methods, enhancing spatial and temporal modeling of dense points.
...
TL;DR: MAPEX is a remote sensing foundation model using mixture-of-modality experts to address modality mismatch and efficiency issues in pre-training and fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing foundation models focus on specific modalities, creating a mismatch for diverse applications and making fine-tuning costly.

Method: MAPEX pre-trains on multi-modal data with modality-conditioned token routing and uses modality-aware pruning for task-specific efficiency.

Result: MAPEX outperforms supervised training and state-of-the-art foundation models on diverse remote sensing tasks.

Conclusion: MAPEX efficiently bridges modality gaps and simplifies deployment for specific remote sensing applications.
...
TL;DR: The paper investigates the 'linear reasoning bottleneck' in VLMs, introducing the Linear Separability Ceiling (LSC) and showing it's solvable through task-dependent alignment, not just better representation learning.


<details>
  <summary>Details</summary>
Motivation: To understand and address the limitations of VLMs in abstract reasoning tasks, particularly the linear separability bottleneck.

Method: Introduces LSC, uses postfix tuning as a control, and explores task-dependent interventions (activating pathways or adapting weights).

Result: VLMs have dormant reasoning pathways; alignment is solvable but task-dependent. Complex tasks require deeper adaptation, which can disrupt performance on new prompts.

Conclusion: Robust reasoning in VLMs depends on targeted alignment, not just improved representation quality.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail in low-light due to image degradation and weak supervision, leading to unreliable class activation maps and ambiguous pseudo-labels.

Method: DGKD-WLSS uses Diffusion-Guided Knowledge Distillation (DGKD) to align features and Depth-Guided Feature Fusion (DGF2) to integrate depth maps for better structural learning.

Result: DGKD-WLSS achieves state-of-the-art performance in low-light weakly-supervised semantic segmentation.

Conclusion: The proposed framework effectively addresses low-light challenges and outperforms existing methods.
...
Method: HOTA combines overlapping tiles of different sizes with SegFormer and a DEM-based depth module for 3D flood mapping.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion methods lack consistency in multi-view outputs, which is critical for applications like digital humans and virtual avatars.

Method: The framework includes a multi-view training data generation pipeline (Bald Converter, inpainting model, face-finetuned diffusion model) and integrates polar-azimuth embeddings and temporal attention layers. A multi-stage training strategy is used.

Result: The method accurately transfers realistic hairstyles with seamless consistency across views, outperforming existing techniques.
...
TL;DR: Enhancements to SAM2 for video object tracking include hierarchical motion estimation and optimized memory bank, improving accuracy without extra training. Achieves state-of-the-art results on LaSOT and LaSOText.
...
Result: 9.6% and 7.2% relative AUC improvements on LaSOT and LaSOText with large model; larger gains on smaller models.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the noise and sparsity in point labels from image-based back-projection in lidar scans for driving scenarios.
...
Result: Performance varies widely; models show unexpected behaviors like misaligned difficulty perception and 2D-to-3D cliffs.

Conclusion: State-of-the-art MLLMs still struggle with spatial visualization, highlighting the need for dedicated benchmarks like SpatialViz-Bench.
...
TL;DR: ViLU is a new framework for Vision-Language Uncertainty (UQ) quantification, leveraging multi-modal representations to improve failure prediction in VLMs.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of reliable UQ and failure prediction in Vision-Language Models (VLMs).

Method: ViLU integrates visual and textual embeddings via cross-attention, training an uncertainty predictor as a binary classifier with weighted binary cross-entropy loss.

Result: Outperforms state-of-the-art failure prediction methods on datasets like ImageNet-1k, CC12M, and LAION-400M.

Conclusion: ViLU's architecture and training are critical for effective UQ, offering a loss-agnostic, post-hoc solution.
...
TL;DR: T-GVC is a novel generative video coding framework that uses trajectory-guided motion sampling and diffusion processes to improve video reconstruction in ultra-low bitrate scenarios, outperforming traditional and modern methods.
...
Conclusion: T-GVC introduces a new direction for generative video coding by leveraging geometric motion modeling, offering improved accuracy and efficiency.
...
TL;DR: The paper investigates age-related bias in deep learning-based Facial Expression Recognition (FER) systems, focusing on the elderly, and proposes three mitigation strategies to improve fairness and accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep FER models often exhibit demographic biases, particularly age-related, compromising fairness and reliability. This study aims to address these biases, especially for the elderly.
...
Result: The strategies improve recognition accuracy for elderly individuals, especially for error-prone expressions like "neutral", "sadness", and "anger". Saliency heatmaps show more relevant attention patterns.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for MoA recognition focus on spatial cell features but ignore temporal dynamics and molecular data, which are crucial for understanding drug effects.
...
Conclusion: Combining temporal cell dynamics and molecular data enhances MoA recognition, demonstrating the effectiveness of MolCLIP.
...
TL;DR: The paper introduces ARNet, a deep learning model for automating keypoint annotation in cervical vertebrae radiographs to predict growth potential in pediatric orthodontics.


<details>
  <summary>Details</summary>
Motivation: Accurate growth potential estimation is crucial for effective orthodontic treatment, but manual annotation of cervical vertebrae is labor-intensive.

Method: ARNet uses an Interaction-guided recalibration network and morphology-aware loss to streamline keypoint annotation with user feedback.

Result: ARNet reduces manual effort, improves accuracy, and shows strong performance across datasets.

Conclusion: The research provides an efficient AI tool for growth potential assessment, advancing pediatric orthodontics.
...
TL;DR: The paper introduces AU-DFER, an AU-enhanced dynamic facial expression recognition architecture that integrates AU-expression knowledge with deep learning, outperforming SOTA methods and addressing data label imbalance.
...
Method: Proposes AU-DFER, which quantifies AU contributions, designs a weight matrix for prior knowledge, and integrates AU loss with deep learning outcomes. Validated on mainstream DFER datasets.
...
Conclusion: Integrating AU-expression knowledge and diverse loss function design enhances DFER effectiveness, highlighting the importance of addressing data imbalance.
...
<details>
  <summary>Details</summary>
Motivation: Existing LVLMs often ignore generated rationales in CoT reasoning, limiting their grounding and accuracy.

Method: Re-formulates CoT as KL-constrained reward maximization and proposes RED, a decoding strategy harmonizing visual and rationale information.
...
TL;DR: The paper introduces Tree-Mamba, a novel method for underwater monocular depth estimation (UMDE), addressing limitations of Mamba-based methods and unreliable datasets by proposing a tree-aware scanning strategy and a new benchmark, BlueDepth.


<details>
  <summary>Details</summary>
Motivation: Existing Mamba-based methods struggle with UMDE due to inflexible state scanning and unreliable depth labels in datasets, necessitating a more adaptive approach and accurate benchmark.

Method: Tree-Mamba uses a tree-aware scanning strategy to construct a minimum spanning tree based on feature similarity, enabling flexible multi-scale feature aggregation. The BlueDepth benchmark provides reliable depth labels.

Result: Tree-Mamba outperforms leading methods in qualitative and quantitative evaluations, demonstrating superior performance and computational efficiency.

Conclusion: The proposed Tree-Mamba and BlueDepth benchmark effectively address UMDE challenges, offering improved accuracy and reliable training data for future research.
...
TL;DR: A novel method for local motion blur deblurring using a trainable mask predictor and intra-frame motion analyzer, reducing computational costs by 49%.


<details>
  <summary>Details</summary>
Motivation: Existing deblurring methods inefficiently handle spatially varying blur patterns and misallocate computational resources.

Method: Proposes a trainable mask predictor to identify blurred regions and an intra-frame motion analyzer for adaptive blur restoration. Uses structural reparameterization for inference optimization.

Result: Outperforms state-of-the-art methods on local and global blur datasets while reducing FLOPs by 49%.
...
TL;DR: CrossVLAD benchmark evaluates cross-task adversarial attacks on unified VLMs, introducing CRAFT for efficient attacks.


<details>
  <summary>Details</summary>
Motivation: Address security challenges in unified VLMs due to adversarial inputs across unpredictable task instructions.

Method: Introduces CrossVLAD dataset and CRAFT, a region-centric attack method, for evaluating adversarial transferability.

Result: CRAFT outperforms existing methods in cross-task attack performance and targeted object-change success rates.

Conclusion: CRAFT effectively influences unified VLMs adversarially across diverse tasks.
...
Method: Simple dataset transformations are applied to identify bias, and multiple network architectures are tested on NIH, CheXpert, MIMIC-CXR, and PadChest datasets.

Result: High accuracies in dataset origin tasks suggest underlying bias in these medical datasets.

Conclusion: The study highlights the need for explainable research and more open-source datasets in medical imaging to ensure AI methods focus on relevant pathology.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of long video reasoning in VLMs by integrating large-scale data, advanced training methods, and optimized infrastructure.
...
Result: LongVILA-R1-7B outperforms benchmarks, matches Gemini-1.5-Pro in reasoning tasks, and achieves 2.1x speedup in training.

Conclusion: The framework advances long video reasoning in VLMs and is made publicly available, supporting diverse modalities and models.
...
TL;DR: A simplified 3D promptable segmentation method is introduced to address the limitations of SAM in 3D medical imaging, reducing inference time and complexity while maintaining high performance.


<details>
  <summary>Details</summary>
Motivation: SAM's 2D architecture doesn't naturally extend to 3D medical imaging, and existing 3D methods are computationally intensive and complex.

Method: The proposed method is inspired by SegVol, simplifying 3D segmentation by eliminating sliding-window complexities and reducing inference time.

Result: The method achieves state-of-the-art performance without the computational and prompt management drawbacks of existing approaches.

Conclusion: The new 3D promptable segmentation method offers a simpler, faster, and high-performing alternative for medical imaging.
...
Result: TreeBench includes 405 challenging VQA pairs, with advanced models scoring below 60% accuracy. TreeVGR improves performance on multiple benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for mitigating object hallucination in LVLMs are limited by decoding constraints, complex visual input modifications, or reliance on external models. The paper identifies an imbalance in "Yes" ratios across VQA datasets and aims to address it.

Method: An energy-based decoding method dynamically selects hidden states from layers with minimal energy scores, reducing bias in "Yes" ratios while enhancing performance.
...
Conclusion: The proposed energy-based decoding is simple, effective, and less biased, making it suitable for safer deployment of LVLMs.
...
TL;DR: A high-rate two-stream SNN is introduced for early human activity recognition, outperforming previous methods by 2% in accuracy on the THU EACT-50 dataset. It leverages event-based vision sensors and is benchmarked for early prediction performance.
...
Method: A high-rate two-stream spiking neural network (SNN) processes event-based vision data for early predictions, avoiding the limitations of low-rate frame accumulation.

Result: The SNN outperforms previous work by 2% in final accuracy on the THU EACT-50 dataset and demonstrates strong early recognition performance.

Conclusion: The proposed SNN framework effectively addresses early recognition needs and shows practical impact in real-world tasks like human motion capture in sports.
...
TL;DR: The paper introduces SDST, a sparse-dense side-tuning architecture for Video Temporal Grounding (VTG), improving adaptability and performance while reducing parameters.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on frozen pre-trained backbones, limiting adaptability to new domains. Full fine-tuning is impractical, and prior side-tuning overlooks the sparse nature of Moment Retrieval (MR).

Method: Proposes SDST, an anchor-free side-tuning architecture, and Reference-based Deformable Self-Attention for better context modeling. Integrates InternVideo2 backbone into ST framework.
...
Conclusion: SDST significantly improves side-tuning methods, offering a practical and efficient solution for VTG.
...
<details>
  <summary>Details</summary>
Motivation: Enhancing surgical decision-making by enabling real-time quantitative fluorescence measurements in neurosurgery through accurate cross-modal image alignment.

Method: Introduces X-RAFT, a variant of RAFT for cross-modal inputs, using distinct encoders per modality and self-supervised fine-tuning with flow-cycle-consistency.

Result: Achieves 36.6% error reduction over a naive baseline and 27.83% over CrossRAFT.

Conclusion: X-RAFT effectively addresses cross-modal alignment challenges in hyperspectral imaging, promising improved surgical outcomes.
...
TL;DR: A deep learning-based method improves geometric accuracy in additive manufacturing by using dynamic patch processing and a Binary Difference Map, outperforming traditional DVC methods.
...
Method: A deep learning approach estimates voxel-wise deformations between CAD and XCT volumes, using dynamic patch-based processing for high-resolution data. The Binary Difference Map evaluates registration accuracy.

Result: The method improves the Dice Score by 9.2% and voxel match rate by 9.9%, reducing processing time from days to minutes.

Conclusion: This work lays the groundwork for deep learning-based DVC in AM, promising more reliable and efficient manufacturing processes.
...
<details>
  <summary>Details</summary>
Motivation: Traditional defenses fail against unrestricted adversarial attacks, and existing evaluations lack statistical significance. A unified framework is needed.
...
Result: Human evaluations show color-space and diffusion-based attacks fail to produce imperceptible images. GPT-4o detects only some attacks.

Conclusion: SCOOTER highlights the misalignment between human perception and automated systems, emphasizing the need for human-centered benchmarks.
...
TL;DR: The study evaluates calibration techniques for image classification under dataset shift, comparing post-hoc and in-training methods. Key findings include the effectiveness of entropy regularization and label smoothing, robustness of post-hoc calibrators with OOD data, and trade-offs between in-distribution and shifted calibration. Ensembling and finetuning from foundation models yield the best results.


<details>
  <summary>Details</summary>
Motivation: To provide practical guidelines for robust calibration under dataset shift in image classification, addressing the need for reliable probabilistic predictions in real-world applications.

Method: Comparison of post-hoc calibration methods and in-training strategies (e.g., label smoothing) across eight classification tasks and natural shifts. Analysis includes randomly initialized and finetuned classifiers, and ensembling effects.

Result: Entropy regularization and label smoothing work best under shift; post-hoc calibrators with OOD exposure are robust; trade-offs exist between in-distribution and shifted calibration. Ensembling and finetuning improve calibration robustness.

Conclusion: Combining ensembling with finetuning from foundation models offers the best calibration results, though trade-offs between in-distribution and shifted calibration must be managed.
...
TL;DR: The paper introduces S\textsc{urprise}3D, a dataset for evaluating spatial reasoning in 3D scenes, addressing biases in current datasets and providing a benchmark for AI systems.


<details>
  <summary>Details</summary>
Motivation: Current 3D vision-language research lacks focus on spatial reasoning, with datasets often biased by semantic cues. This limits genuine understanding of spatial relationships.

Method: The authors created S\textsc{urprise}3D, a dataset with 200k+ vision-language pairs and 89k+ spatial queries, avoiding object names to reduce biases. It covers diverse spatial reasoning skills.
...
Method: The method combines 3D CNNs, differential frame fusion, Temporal Shift Module (TSM) with self-attention, and a dynamic hybrid loss function for better rPPG signal reconstruction.
...
Conclusion: The proposed network demonstrates superior robustness and generalization, making it effective for rPPG-based heart rate measurement in complex scenarios.
...
TL;DR: ViaPT introduces instance-aware prompts for vision transformers, outperforming traditional dataset-level prompts by balancing dataset and instance knowledge using PCA.
...
Method: ViaPT generates instance-aware prompts for each input, fuses them with dataset-level prompts, and uses PCA to retain key information. It avoids the extremes of VPT-Deep and VPT-Shallow.
...
<details>
  <summary>Details</summary>
Motivation: Real-world applications often face missing or incomplete modality inputs, degrading performance. Current prompt-based methods lack flexibility and reliability.

Method: SyP uses a Dynamic Adapter for adaptive prompts and a Synergistic Prompting Strategy to balance information across modalities.

Result: SyP outperforms existing methods on three visual recognition datasets, showing robustness under missing data.

Conclusion: SyP effectively handles missing modalities with superior adaptability and reliability, validated by experiments.
...
TL;DR: The paper introduces a Vision Transformer (ViT) approach for markerless lung tumor motion forecasting, comparing patient-specific and multi-patient models. While patient-specific models perform better with larger datasets, multi-patient models generalize better without retraining.
...
Method: Two training strategies (patient-specific and multi-patient) were evaluated using Vision Transformers on DRRs from 31 patients, predicting tumor motion over 1 second. Performance was measured using ADE and FDE.

Result: Patient-specific models outperformed multi-patient models on planning data, but multi-patient models were more robust to anatomical variability and performed comparably on treatment data without retraining.

Conclusion: ViT architectures are effective for markerless tumor motion forecasting. Patient-specific models offer precision, while multi-patient models provide robust, ready-to-use performance for clinical settings.
...
TL;DR: The paper evaluates the robustness of content-based puzzle solvers under realistic corruptions (missing pieces, eroded edges, and eroded contents), finding that deep learning models, especially the Positional Diffusion model, outperform others when fine-tuned.


<details>
  <summary>Details</summary>
Motivation: Current puzzle solvers lack evaluation under realistic challenges like fragmented artefacts or shredded documents, which are crucial for real-world applications.

Method: Introduces three types of jigsaw puzzle corruptions and evaluates heuristic and deep learning-based solvers.

Result: Standard solvers decline rapidly with corrupted pieces, but deep learning models improve robustness through fine-tuning. The Positional Diffusion model excels.

Conclusion: Highlights the potential of deep learning, especially fine-tuned models, for real-world artefact reconstruction and suggests future research directions.
...
TL;DR: SimCIS addresses plasticity loss and data order reliance in continual image segmentation by leveraging highly aggregated features for query alignment and introducing cross-stage consistency and visual query replay.
...
Method: SimCIS selects image features for query assignment to ensure perfect alignment, introduces cross-stage consistency, and uses a visual query replay mechanism.

Result: SimCIS outperforms state-of-the-art methods across various segmentation tasks, settings, and data orders.

Conclusion: SimCIS provides a simple yet effective baseline for continual image segmentation, addressing key challenges and achieving superior performance.
...
TL;DR: 3D-ADAM is a large-scale, industry-relevant dataset for 3D anomaly detection, addressing the lack of high-quality datasets in manufacturing. It includes diverse defects and real-world variations, challenging current SOTA models.


<details>
  <summary>Details</summary>
Motivation: Surface defects significantly impact manufacturing yield, but existing datasets lack real-world relevance and quality, hindering robust defect detection methods.

Method: Introduces 3D-ADAM, a dataset with 14,120 high-resolution scans, 27,346 annotated defects, and 8,110 machine element annotations, captured in real industrial settings.

Result: Evaluation shows current SOTA models struggle with 3D-ADAM, highlighting its challenge and industrial relevance. Expert surveys confirm dataset quality.

Conclusion: 3D-ADAM aims to advance robust 3D anomaly detection models for manufacturing by providing a realistic and challenging benchmark.
...
<details>
  <summary>Details</summary>
Motivation: The rapid release of foundation models in digital pathology necessitates a standardized benchmark to evaluate performance, differences, and reliability for healthcare applications.

Method: THUNDER is introduced as a dynamic, easy-to-use benchmark supporting diverse models and tasks, including feature space analysis and robustness evaluation.

Result: A comprehensive comparison of 23 foundation models on 16 datasets is provided, showcasing THUNDER's capability for diverse evaluations.

Conclusion: THUNDER offers a reliable, fast, and adaptable benchmark for digital pathology, facilitating better model comparison and robustness assessment.
...
TL;DR: MIRA framework improves factual accuracy in Multimodal Large Language Models (MLLMs) for medical diagnosis by optimizing retrieval-augmented generation with dynamic context adjustment and multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: MLLMs often produce factually inconsistent responses in medical diagnosis, and existing RAG methods face challenges like insufficient/excessive retrieval and over-reliance on retrieved data.

Method: Introduces MIRA with two components: (1) a calibrated Rethinking and Rearrangement module for dynamic context adjustment, and (2) a medical RAG framework integrating image embeddings and a knowledge base with query-rewrite for multimodal reasoning.
...
Conclusion: MIRA effectively addresses retrieval challenges in MLLMs, enhancing factual accuracy and performance in medical applications.
...
<details>
  <summary>Details</summary>
Motivation: Accurate position estimation is crucial for navigation in autonomous platforms, requiring efficient feature extraction for VSLAM systems.

Method: Implemented a quantized SuperPoint CNN on an FPGA SoC (AMD/Xilinx Zynq UltraScale+), using Brevitas and FINN for quantization and optimization.

Result: Achieved 54 fps processing on 640x480 images, outperforming state-of-the-art solutions, with experiments on the TUM dataset validating performance.

Conclusion: The method successfully balances computational efficiency and detection quality, enabling deployment on resource-limited platforms.
...
<details>
  <summary>Details</summary>
Motivation: Addressing limitations of existing domain adaptation methods in rPPG due to privacy concerns and real-time constraints.

Method: Introduces the CiCi framework, combining consistency and inconsistency priors in rPPG signals, with gradient dynamic control for stable adaptation.

Result: Outperforms existing techniques on five datasets under TTA, achieving state-of-the-art performance.

Conclusion: The CiCi framework effectively enhances real-time self-supervised adaptation in rPPG tasks without needing source data.
...
<details>
  <summary>Details</summary>
Motivation: Continuous monitoring of mice is challenging due to housing density, similar appearances, and high mobility. Accurate ID assignment is needed for better data and welfare.
...
Result: The pipeline achieves 30 fps ID assignment with 24/7 coverage, outperforming current methods in tracking efficiency and reducing ID switches.

Conclusion: The proposed system enhances real-time monitoring of mice, improving research accuracy and animal welfare.
...
TL;DR: TinierHAR is an ultra-lightweight deep learning model for HAR on wearable devices, achieving SOTA efficiency with minimal performance loss.
...
<details>
  <summary>Details</summary>
Motivation: To enable mission rehearsal and robotic simulation by overcoming the lack of high-quality Martian data and the domain gap between Martian and terrestrial imagery.
...
Conclusion: The proposed solution effectively synthesizes realistic Martian videos, aiding mission planning and simulation.
...
Method: Proposes Geometry Forcing, using Angular and Scale Alignment to align diffusion model representations with pretrained geometric features.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate MLLMs offline with fixed inputs, lacking real-world embodied perception challenges. OST-Bench addresses this gap by focusing on active exploration and incremental reasoning.

Method: OST-Bench uses 1.4k scenes and 10k QA pairs from ScanNet, Matterport3D, and ARKitScenes, evaluating MLLMs on tasks requiring spatio-temporal reasoning and memory integration.

Result: Leading MLLMs perform poorly on complex spatio-temporal tasks, with accuracy declining as exploration and memory demands increase. Error patterns reveal challenges in clue-based reasoning and long-term memory retrieval.

Conclusion: OST-Bench exposes critical limitations in MLLMs for online embodied reasoning, providing a resource for future research to address these challenges.
...
TL;DR: The paper investigates why CLIP models struggle with binding (e.g., distinguishing object-attribute pairs) and identifies data properties as the key issue, not architecture or training methods.


<details>
  <summary>Details</summary>
Motivation: CLIP models fail at binding tasks (e.g., distinguishing 'a yellow submarine and a blue bus' vs. 'a blue submarine and a yellow bus'). Previous fixes like hard negatives or architecture changes didn't fully resolve this. The authors hypothesize the problem lies in data properties.
...
Conclusion: Data properties, not training or architecture, are the critical factor for CLIP's binding performance. Addressing these properties can improve CLIP's capabilities.
...
TL;DR: STTM is a training-free spatio-temporal token merging method for video LLMs, reducing computational cost while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Video LLMs face quadratic computational scaling with token count, and prior work overlooks local spatial and temporal redundancy in video data.

Method: STTM transforms frames into multi-granular spatial tokens via a quadtree search and merges tokens temporally.

Result: STTM achieves 2× speed-up with 0.5% accuracy drop (50% token budget) and 3× speed-up with 2% drop (30% budget).

Conclusion: STTM efficiently reduces tokens, is query-agnostic, and outperforms existing methods on video QA benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation protocols for brain visual decoding lack granularity, neuroscientific grounding, and fail to capture fine-grained distinctions, necessitating a more robust framework.

Method: BASIC uses hierarchical segmentation-based metrics for structural fidelity and multimodal large language models for semantic-level comparisons, benchmarking diverse methods across datasets.

Result: The framework provides discriminative, interpretable, and comprehensive evaluation of brain visual decoding methods.

Conclusion: BASIC offers a more refined and neuroscientifically grounded approach to assessing visual decoding methods, enhancing their evaluation and comparison.
...
TL;DR: A framework for few-shot keypoint detection using sketches, addressing cross-modal embedding and user-specific styles with prototypical adaptation.


<details>
  <summary>Details</summary>
Motivation: Challenges in few-shot keypoint detection when source data is unavailable, solved by leveraging sketches as a source-free alternative.
...
Result: Successful few-shot convergence across novel keypoints and classes, validated through experiments.

Conclusion: The proposed framework effectively addresses keypoint detection challenges in few-shot scenarios using sketches.
...
TL;DR: KARL is a single-pass adaptive tokenizer inspired by Kolmogorov Complexity, predicting token count for images in one forward pass while matching performance of multi-pass methods.


<details>
  <summary>Details</summary>
Motivation: Fixed-length representations in visual learning ignore complexity variations. Adaptive methods require test-time search, so KARL aims for efficiency with single-pass prediction.

Method: KARL uses a single forward pass to predict token halting based on reconstruction quality, trained similarly to Upside-Down Reinforcement Learning.
...
TL;DR: The paper introduces \NickName, a method to enhance VQ-VAEs by improving codebook representation and quantization, achieving state-of-the-art performance in reconstruction tasks.
...
Method: Retains latent dimensions and uses sub-codebooks for quantization, along with rigorous benchmarks for evaluation.

Result: \NickName outperforms existing methods, achieving top results on ImageNet and zero-shot benchmarks with improved rFID and PSNR metrics.

Conclusion: \NickName significantly enhances reconstruction quality, making it suitable for high-fidelity HD image processing.
...
<details>
  <summary>Details</summary>
Motivation: To understand how concept combinations in training data impact compositional generalization in CLIP and LMMs, particularly for uncommon pairings.

Method: Measure co-occurrence with PMI, use synthetic and edited natural images to test CLIP/LMM performance, and analyze correlations.
...
Conclusion: Current models rely heavily on training data co-occurrence statistics, necessitating improved algorithms for compositional generalization without combinatorial data scaling.
...
TL;DR: The paper evaluates the robustness of prompt injection defenses in LLMs, showing they fail under strong optimization-based attacks, with success rates up to 70%.
...
Result: Attacks achieved up to 70% success rates with modest token budget increases.

Conclusion: The defenses do not provide the claimed security, highlighting vulnerabilities in whitebox settings.
...
TL;DR: The paper explores electromagnetic signal injection attacks on image sensors, causing rainbow-like artifacts that disrupt object detection models, revealing a critical vulnerability in visual perception systems.


<details>
  <summary>Details</summary>
Motivation: Image sensors are crucial in safety-critical systems, but their analog domain is vulnerable to undetected manipulation, posing risks to decision-making processes.
...
Result: The attacks cause significant mispredictions in object detection models, demonstrating a critical vulnerability in the visual perception pipeline.
...
### [145] [Open Source Planning & Control System with Language Agents for Autonomous Scientific Discovery](https://arxiv.org/abs/2507.07257)
...
TL;DR: A multi-agent system (cmbagent) automates scientific research using 30 LLM agents with no human intervention, excelling in PhD-level cosmology tasks.
...
Method: Uses 30 LLM agents with a Planning & Control strategy, each specializing in tasks like retrieval, coding, and critique, executing code locally.
...
### [146] [ViDove: A Translation Agent System with Multimodal Context and Memory-Augmented Reasoning](https://arxiv.org/abs/2507.07306)
*Yichen Lu,Wei Dai,Jiaen Liu,Ching Wing Kwok,Zongheng Wu,Xudong Xiao,Ao Sun,Sheng Fu,Jianyuan Zhan,Yian Wang,Takatomo Saito,Sicheng Lai*

Main category: cs.AI

TL;DR: ViDove is a multimodal translation agent that improves translation quality by incorporating visual and contextual information, outperforming text-only LLM-based systems.
...
Method: ViDove integrates a multimodal memory system and long-short term memory modules with domain-specific knowledge to enhance translation.

Result: ViDove achieves a 28% improvement in BLEU scores and 15% in SubER, outperforming state-of-the-art baselines.

Conclusion: ViDove demonstrates the value of multimodal inputs in translation, introducing DoveBench as a new benchmark for video subtitling and translation.
...
### [147] [Growing Transformers: Modular Composition and Layer-wise Expansion on a Frozen Substrate](https://arxiv.org/abs/2507.07129)
*A. Bochkov*

Main category: cs.LG

TL;DR: The paper proposes a constructive approach to scaling LLMs using deterministic input embeddings, enabling modular composition and layer-wise growth for efficient model development.
...
Method: Utilizes non-trainable, deterministic embeddings as a universal "docking port" for merging specialist models (via logit averaging) and progressive layer-wise training.

Result: Merged MoE models outperform constituent experts on benchmarks like MMLU, and layer-wise training shows stable convergence and depth-correlated reasoning abilities.

Conclusion: The approach suggests a shift towards biological-like, incremental AI development, enabling resource-efficient scaling and democratized model building.
...
### [148] [An Information-Theoretic Perspective on Multi-LLM Uncertainty Estimation](https://arxiv.org/abs/2507.07236)
...
<details>
  <summary>Details</summary>
Motivation: LLMs show inconsistent behavior, highlighting the need for better uncertainty quantification in high-stakes settings, leveraging model diversity.

Method: Proposes MUSE, using Jensen-Shannon Divergence to identify and aggregate well-calibrated subsets of LLMs.

Result: Experiments show MUSE improves calibration and predictive performance over baselines.

Conclusion: Model diversity can enhance uncertainty estimation, with MUSE offering a practical solution.
...
### [149] [Bradley-Terry and Multi-Objective Reward Modeling Are Complementary](https://arxiv.org/abs/2507.07375)
...
<details>
  <summary>Details</summary>
Motivation: RLHF's vulnerability to reward hacking and poor performance in out-of-distribution (OOD) scenarios motivates the need for more robust reward models.

Method: A unified framework jointly trains Bradley-Terry (BT) single-objective and multi-objective regression-based reward functions using a shared embedding space.
...
### [150] [COALA: Numerically Stable and Efficient Framework for Context-Aware Low-Rank Approximation](https://arxiv.org/abs/2507.07580)
*Uliana Parkina,Maxim Rakhuba*

Main category: cs.LG

TL;DR: The paper introduces a stable, inversion-free framework for context-aware low-rank approximation in neural networks, addressing numerical instabilities and memory issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for low-rank approximation in neural networks suffer from numerical instabilities and memory limitations due to reliance on explicit Gram matrix computation and inversion.

Method: The proposed method uses stable decompositions without inversion, handling large calibration matrices, near-singular input activations, and insufficient data. It includes theoretical guarantees for convergence and error bounds.

Result: The framework improves numerical stability and approximation quality, even in challenging scenarios like limited GPU memory or nearly singular matrices.

Conclusion: The novel inversion-free approach provides a robust solution for low-rank approximation in neural networks, overcoming prior limitations with theoretical support.
...
### [151] [Improving Clustering on Occupational Text Data through Dimensionality Reduction](https://arxiv.org/abs/2507.07582)
...
Conclusion: The method aids automatic occupation distinction, supporting career transitions.
...
### [152] [GuardVal: Dynamic Large Language Model Jailbreak Evaluation for Comprehensive Safety Testing](https://arxiv.org/abs/2507.07735)
*Peiyan Zhang,Haibo Jin,Liying Kang,Haohan Wang*

Main category: cs.LG

TL;DR: The paper introduces GuardVal, a dynamic evaluation protocol for assessing jailbreak vulnerabilities in LLMs, addressing gaps in current methods. It also proposes an optimization technique for prompt refinement and tests it across diverse models, revealing behavioral patterns and robustness insights.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fail to fully evaluate jailbreak vulnerabilities in LLMs due to their evolving nature and the sophistication of attacks. This gap necessitates a more effective evaluation protocol.

Method: The authors propose GuardVal, a protocol that dynamically generates and refines jailbreak prompts based on the defender LLM's state. They also introduce an optimization method to prevent stagnation in prompt refinement.

Result: Applied to models like Mistral-7b and GPT-4 across 10 safety domains, the protocol reveals distinct behavioral patterns and provides a comprehensive view of model robustness.

Conclusion: GuardVal offers a more accurate assessment of LLM vulnerabilities, deepening understanding of their behavior and informing future research for developing more secure models.
...
### [153] [Weighted Multi-Prompt Learning with Description-free Large Language Model Distillation](https://arxiv.org/abs/2507.07147)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on unreliable text-based responses from LLMs for prompts. DeMul aims to improve robustness and reliability by bypassing description extraction.

Method: DeMul eliminates description extraction, distilling LLM knowledge directly into continuous vector prompts. It also introduces prompt weighting in multi-prompt settings.

Result: DeMul achieves superior performance across 11 recognition datasets.

Conclusion: DeMul offers a more reliable and effective approach for prompt learning in VLMs by leveraging direct knowledge distillation from LLMs.
...
### [154] [ST-GRIT: Spatio-Temporal Graph Transformer For Internal Ice Layer Thickness Prediction](https://arxiv.org/abs/2507.07389)
...
Result: Outperforms state-of-the-art methods on Greenland ice sheet data, achieving lower root mean-squared error.

Conclusion: ST-GRIT's attention mechanisms enhance noise handling and long-range dependency capture, offering a superior approach for ice layer analysis.
...
### [155] [Resolving Token-Space Gradient Conflicts: Token Space Manipulation for Transformer-Based Multi-Task Learning](https://arxiv.org/abs/2507.07485)
...
Conclusion: DTME-MTL provides a scalable and efficient solution for transformer-based MTL.
...
### [156] [Balancing the Past and Present: A Coordinated Replay Framework for Federated Class-Incremental Learning](https://arxiv.org/abs/2507.07712)
*Zhuang Qi,Lei Meng,Han Yu*

Main category: cs.LG

TL;DR: FedCBDR improves Federated Class Incremental Learning (FCIL) by balancing class-wise data replay and adjusting logit temperatures, achieving better accuracy.


<details>
  <summary>Details</summary>
Motivation: Address class imbalance in FCIL caused by limited global awareness and task imbalance, which hinders performance.

Method: Uses global coordination for class-level memory construction and reweights learning objectives. Includes a global-perspective data replay module and task-aware temperature scaling.

Result: Achieves 2%-15% Top-1 accuracy improvement over six state-of-the-art methods.

Conclusion: FedCBDR effectively balances class-wise sampling and improves generalization in FCIL.
...
### [157] [TRIX- Trading Adversarial Fairness via Mixed Adversarial Training](https://arxiv.org/abs/2507.07768)
*Tejaswini Medi,Steffen Jung,Margret Keuper*

Main category: cs.LG

TL;DR: TRIX is a feature-aware adversarial training framework that adapts adversary strength per class to reduce robustness disparities, improving worst-case accuracy while maintaining overall performance.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial training methods treat all classes uniformly, leading to unfair robustness where strong classes become more robust while weak classes remain vulnerable. TRIX addresses this by adapting adversary strength per class.

Method: TRIX assigns weaker targeted adversaries to strong classes for feature diversity and stronger untargeted adversaries to weak classes for focused robustness. It also uses per-class loss weighting and perturbation adjustments.

Result: TRIX improves worst-case class accuracy on clean and adversarial data, reduces inter-class robustness disparities, and preserves overall accuracy, as shown in experiments with PGD and AutoAttack.

Conclusion: TRIX is a practical solution for fair and effective adversarial defense by addressing class-wise vulnerability disparities.
...
### [158] [Synchronizing Task Behavior: Aligning Multiple Tasks during Test-Time Training](https://arxiv.org/abs/2507.07778)
*Wooseong Jeong,Jegyeong Cho,Youngho Yoon,Kuk-Jin Yoon*

Main category: cs.LG

TL;DR: S4T improves multi-task generalization under domain shifts by synchronizing tasks during test-time training.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of unsynchronized task behavior in conventional TTT methods when handling multiple tasks under domain shifts.
...
Conclusion: S4T effectively synchronizes tasks for better multi-task performance under domain shifts.
...
### [159] [Adaptive Attention Residual U-Net for curvilinear structure segmentation in fluorescence microscopy and biomedical images](https://arxiv.org/abs/2507.07800)
...
Method: Developed ASE_Res_UNet by enhancing U-Net with residual blocks and adaptive SE attention mechanisms, validated on synthetic and real datasets.
...
### [160] [Semi-supervised learning and integration of multi-sequence MR-images for carotid vessel wall and plaque segmentation](https://arxiv.org/abs/2507.07496)
...
Method: Proposes a two-network system: coarse localization followed by fine segmentation, using a multi-level multi-sequence U-Net and semi-supervised learning with input transformation consistency.

Result: Evaluated on 52 patients, the approach effectively segments vessel walls and plaques, with fusion strategy selection proving critical in U-Net performance.

Conclusion: Fusion strategies and semi-supervised learning enhance carotid artery segmentation in MRI, especially in data-limited scenarios.
...
### [161] [D-CNN and VQ-VAE Autoencoders for Compression and Denoising of Industrial X-ray Computed Tomography Images](https://arxiv.org/abs/2507.07704)
...
Method: Used D-CNN and VQ-VAE with varying compression rates on sandstone XCT data, comparing decoded image quality and introducing an edge-sensitive metric.
...
Conclusion: The findings help scientists choose appropriate storage and analysis strategies based on data characteristics.
...
### [162] [Compressive Imaging Reconstruction via Tensor Decomposed Multi-Resolution Grid Encoding](https://arxiv.org/abs/2507.07707)
*Zhenyu Jin,Yisi Luo,Xile Zhao,Deyu Meng*

Main category: eess.IV

TL;DR: GridTD is an unsupervised framework for compressive imaging (CI) reconstruction, combining tensor decomposition and multi-resolution grid encoding for efficient and accurate high-dimensional image recovery.
...
Method: Proposes GridTD, which optimizes a lightweight neural network and tensor decomposition model using multi-resolution hash grid encoding.

Result: GridTD outperforms existing methods in diverse CI tasks, supported by theoretical analyses and extensive experiments.
...
### [163] [Breast Ultrasound Tumor Generation via Mask Generator and Text-Guided Network:A Clinically Controllable Framework with Downstream Evaluation](https://arxiv.org/abs/2507.07721)
...
Method: The framework integrates clinical descriptions with structural masks to generate tumors, controlled by tumor characteristics. A semantic-curvature mask generator creates diverse tumor masks. Synthetic masks are used to produce personalized BUS images.

Result: Quantitative evaluations on six BUS datasets show clinical utility, improving breast cancer diagnosis. Visual Turing tests confirm image realism.

Conclusion: The framework effectively addresses data scarcity, supporting clinical applications with realistic synthetic BUS images.
...
### [164] [MeD-3D: A Multimodal Deep Learning Framework for Precise Recurrence Prediction in Clear Cell Renal Cell Carcinoma (ccRCC)](https://arxiv.org/abs/2507.07839)
...
Method: A DL framework combines domain-specific models (CLAM for histopathology, MeD-3D for CT/MRI, MLP for clinical/genomic data) with early/late fusion for multimodal integration and handles missing data.

Result: The framework leverages diverse data sources (TCGA, TCIA, CPTAC) to extract and fuse deep feature embeddings, enhancing predictive accuracy.

Conclusion: The proposed multimodal DL approach improves ccRCC recurrence prediction, offering better clinical decision-making support.
...
### [165] [ArteryX: Advancing Brain Artery Feature Extraction with Vessel-Fused Networks and a Robust Validation Framework](https://arxiv.org/abs/2507.07920)
...
<details>
  <summary>Details</summary>
Motivation: Current methods for assessing cerebral vasculature lack standardized quantitative metrics and face challenges like variability and inefficiency, necessitating advanced tools for early detection of cerebrovascular pathology.
...
Conclusion: ArteryX promises to enhance clinical workflows by enabling early pathology detection and standardized comparisons, advancing understanding of vascular contributions to brain health.
...
### [166] [Wrist bone segmentation in X-ray images using CT-based simulations](https://arxiv.org/abs/2507.07131)
*Youssef ElTantawy,Alexia Karantana,Xin Chen*

Main category: eess.IV

TL;DR: The paper proposes a deep-learning method for wrist bone segmentation in X-rays using simulated CT-generated images to overcome annotation challenges. It achieves high Dice scores (0.80-0.92) and demonstrates strong performance on real X-rays.


<details>
  <summary>Details</summary>
Motivation: X-ray segmentation is crucial for diagnostics but requires extensive expert annotations, especially for small wrist bones. Simulated data from CT scans is used to address this challenge.

Method: A deep-learning model is trained on simulated X-ray images generated from CT volumes with labeled bones. The model is evaluated on both simulated and real X-ray images.

Result: The method achieves Dice scores of 0.80-0.92 on simulated data and performs well on real X-rays, as shown by qualitative analysis.

Conclusion: The approach effectively addresses annotation challenges in wrist bone segmentation and shows promising results, with the model and simulation code made available for research.
...
### [167] [Label-Efficient Chest X-ray Diagnosis via Partial CLIP Adaptation](https://arxiv.org/abs/2507.07254)
...
<details>
  <summary>Details</summary>
Motivation: Address challenges of obtaining large labeled medical datasets due to privacy, cost, and scarcity.
...
Result: CLIP's features adapt well to few-shot medical tasks, with significant AUC improvement.

Conclusion: Proposes a practical, scalable solution for sparse annotation scenarios, simulating hospital workflows.
...
### [168] [Computationally Efficient Information-Driven Optical Design with Interchanging Optimization](https://arxiv.org/abs/2507.07789)
...
<details>
  <summary>Details</summary>
Motivation: Addressing IDEAL's limitations (high memory, long runtime, mismatched objectives) to make information-theoretic optimization practical for imaging systems.
...
Result: Reduces runtime and memory usage by up to 6x, enabling superior designs with more expressive density models.

Conclusion: IDEAL-IO makes information-theoretic optimization scalable and practical for diverse imaging applications.
...
### [169] [Input Conditioned Layer Dropping in Speech Foundation Models](https://arxiv.org/abs/2507.07954)
*Abdul Hannan,Daniele Falavigna,Alessio Brutti*

Main category: cs.SD

TL;DR: The paper proposes input-driven layer dropping (LD) for dynamic speech models in edge/IoT settings, outperforming random dropping and matching early exit methods.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing layer dropping methods for dynamic architectures in resource-varying edge/IoT environments.

Method: Uses input features and a lightweight layer-selecting network to determine optimal layer combinations, avoiding significant architecture changes.

Result: Demonstrates effectiveness on 4 speech/audio benchmarks, outperforming random dropping and matching early exit performance.

Conclusion: Input-driven LD is a viable solution for dynamic model adaptation in constrained settings.
...
TL;DR: LangNav is a new dataset and benchmark (LangNavBench) for evaluating language-based semantic navigation, focusing on grounding words in instructions. It includes Multi-Layered Feature Map (MLFM), a method outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack language-focused evaluation for semantic navigation, prompting the creation of LangNav to test agents' understanding of detailed descriptions.

Method: LangNav dataset was manually curated for accuracy. LangNavBench evaluates navigation methods, and MLFM builds a queryable multi-layered semantic map for better performance.

Result: MLFM outperforms state-of-the-art baselines on LangNav, especially with small objects or spatial relations.

Conclusion: LangNav and LangNavBench provide a thorough, language-centric evaluation, with MLFM demonstrating superior performance in semantic navigation.
...
TL;DR: A novel framework for extracting crowd motion patterns and semantics using mmWave radar, combining signal processing, geometric graphs, and flow analysis.


<details>
  <summary>Details</summary>
Motivation: To develop a method for high-fidelity crowd motion analysis and semantic inference using mmWave radar, addressing complex crowd patterns.

Method: Combines optical flow estimation with noise filtering to create mmWave flow fields, transforms these into directed geometric graphs, and analyzes curl and divergence for semantics.

Result: Achieves accurate graph reconstruction of flow structure and infers crowd semantics like turns, boundaries, dispersions, and gatherings.

Conclusion: The framework is validated for crowd analytics, showing potential for diverse applications.
...
### [172] [SD-GS: Structured Deformable 3D Gaussians for Efficient Dynamic Scene Reconstruction](https://arxiv.org/abs/2507.07465)
*Wei Yao,Shuzhao Xie,Letian Li,Weixiang Zhang,Zhixin Lai,Shiqi Dai,Ke Zhang,Zhi Wang*

Main category: cs.GR

TL;DR: SD-GS is a compact dynamic Gaussian splatting framework for complex scene reconstruction, reducing model size by 60% and improving FPS by 100% compared to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current 4D Gaussian frameworks face trade-offs between storage costs and motion characterization, limiting practical use.

Method: Introduces a deformable anchor grid for memory efficiency and a deformation-aware densification strategy for adaptive anchor growth.

Result: Achieves 60% smaller model size and 100% higher FPS while maintaining or surpassing visual quality.

Conclusion: SD-GS significantly enhances computational efficiency and visual quality for dynamic scene reconstruction.
...
### [173] [Capture Stage Environments: A Guide to Better Matting](https://arxiv.org/abs/2507.07623)
*Hannah Dröge,Janelle Pfeifer,Saskia Rabich,Markus Plack,Reinhard Klein,Matthias B. Hullin*

Main category: cs.GR

TL;DR: The paper addresses challenges in matting high-end capture stage content, proposes a guideline for improved workflows, and demonstrates an efficient adaptation pipeline for state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing matting algorithms perform poorly with capture stage content due to unique challenges, necessitating better solutions.

Method: The authors curate challenges, propose proactive interventions, and adapt state-of-the-art methods without extensive annotations, including offline and real-time pipelines.

Result: An efficient pipeline is demonstrated, and a diffusion model-based validation methodology highlights the approach's benefits.

Conclusion: The work provides actionable insights and tools to improve matting workflows for capture stage content.
...
### [174] [RTR-GS: 3D Gaussian Splatting for Inverse Rendering with Radiance Transfer and Reflection](https://arxiv.org/abs/2507.07733)
...
Method: Uses a hybrid rendering model combining forward and deferred rendering, separates high/low-frequency appearances, and refines BRDF/lighting decomposition with physically-based deferred rendering.

Result: Improves novel view synthesis, normal estimation, decomposition, and relighting while maintaining efficiency.

Conclusion: RTR-GS effectively addresses challenges in rendering reflective objects and enhances performance in various tasks.
...
TL;DR: The paper introduces an LLM-enhanced framework for personalized movie recommendations, outperforming traditional SVD/SVD++ methods with significant improvements in metrics like hit rate and NDCG, despite slightly higher computational costs.


<details>
  <summary>Details</summary>
Motivation: Traditional recommendation algorithms lack the ability to incorporate text-based user preferences, such as "I enjoy light-hearted comedies." LLMs offer a promising solution to bridge this gap by mimicking human-like understanding of tastes.

Method: The framework combines LLMs with SVD/SVD++ algorithms. Initial recommendations are generated using SVD/SVD++ (via the Surprise library on MovieLens data), then refined by LLMs using text-based user preferences. Performance is evaluated using hit rates, NDCG, and stratified train-test splits.

Result: The LLM-enhanced framework significantly outperformed SVD/SVD++, achieving up to ~6x higher cumulative hit rates and ~3.7x better NDCG, though with slightly increased computational overhead.

Conclusion: Integrating LLMs with traditional recommendation algorithms enhances personalization and performance, making it a viable approach for text-based preference-driven recommendations.
