...
TL;DR: RMLLMU-Bench is the first benchmark for evaluating unlearning in Reasoning Multimodal LLMs, measuring both reasoning leakage suppression and reasoning retention. Existing methods fail at this balance, so the authors propose R-MUSE, a training-free inference-time framework that achieves better forgetting-reasoning trade-offs.


<details>
  <summary>Details</summary>
Motivation: Current machine unlearning methods for Reasoning Multimodal LLMs (RMLLMs) face unique challenges: intermediate chain-of-thought steps can leak sensitive information even when final answers are forgotten, and aggressive interventions damage general reasoning ability. There's no existing benchmark that jointly evaluates reasoning leakage suppression and reasoning competence preservation.

Method: The paper introduces RMLLMU-Bench, the first benchmark for RMLLM unlearning with dedicated metrics for reasoning leakage and reasoning retention. To address limitations of existing methods, they propose R-MUSE (Reasoning-preserving MLLM Unlearning via Subspace guidance and Adaptive Steering), a training-free inference-time intervention framework that steers internal representations to forget both answers and reasoning traces while explicitly preserving general reasoning.

Result: Systematic evaluation on RMLLMU-Bench shows existing unlearning methods for MLLMs and Large Reasoning Models either leave substantial leakage in reasoning processes or severely degrade reasoning performance. R-MUSE demonstrates substantially better balance between effective forgetting and reasoning retention compared to existing approaches.

Conclusion: The paper addresses a critical gap in RMLLM unlearning evaluation and proposes an effective solution. RMLLMU-Bench enables proper assessment of unlearning methods, while R-MUSE provides a practical framework that achieves superior trade-offs between forgetting sensitive information and preserving general reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for question answering on text-attributed graphs have limitations: pure text-based RAG ignores graph structure, while graph-based RAG suffers from context-length constraints when serializing large subgraphs, leading to fragmented reasoning and reduced accuracy.

Method: Graph-O1 integrates Monte Carlo Tree Search with end-to-end reinforcement learning, framing reasoning as multi-turn interaction between agent and graph environment. The agent selectively explores and retrieves only the most informative subgraph components through stepwise interactive reasoning.

Result: Extensive experiments across multiple LLM backbones show Graph-O1 consistently surpasses state-of-the-art baselines, producing answers that are more accurate, reliable, and interpretable.

Conclusion: Graph-O1 successfully addresses the limitations of existing GraphRAG methods by enabling LLMs to conduct interactive, selective reasoning over graphs through agentic framework with MCTS and reinforcement learning, achieving superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems suffer from redundant transmission of contextual information between agents, consuming excessive bandwidth and computational resources. Traditional approaches discard internal semantic representations and transmit raw text, forcing receiving agents to recompute similar representations from scratch.

Method: Q-KVComm combines three key innovations: (1) adaptive layer-wise quantization with variable bit-widths based on sensitivity profiling, (2) hybrid information extraction preserving critical facts across content domains, and (3) heterogeneous model calibration establishing cross-architecture communication.
...
Conclusion: Q-KVComm establishes a new paradigm for LLM agent communication, shifting from text-based to representation-based information exchange, addressing bandwidth and computational bottlenecks in multi-agent LLM systems.
...
TL;DR: Loquacious dataset replaces LibriSpeech/TED-Lium with properly defined train/test splits across domains, plus open-license resources (n-gram LMs, G2P model, lexica) for benchmarking.


<details>
  <summary>Details</summary>
Motivation: To provide a modern ASR dataset with properly defined partitions across acoustic/language domains and open licensing for both academia and industry, addressing limitations of existing datasets.

Method: Created Loquacious dataset with structured train/test partitions, developed additional resources including n-gram language models, grapheme-to-phoneme model, and pronunciation lexica, then conducted experiments across various ASR architectures with different label units and topologies.

Result: Experimental results demonstrate the dataset's value as a study case for common ASR challenges, showing it works well with diverse ASR architectures and provides benchmarking capabilities.

Conclusion: Loquacious dataset offers a valuable replacement for established ASR datasets with properly defined partitions, open licensing, and comprehensive resources that enable effective benchmarking across various ASR challenges.
...
TL;DR: Transformer models outperform embedding-based methods for IT service ticket prioritization, achieving 78.5% F1-score despite noisy text and class imbalance.


<details>
  <summary>Details</summary>
Motivation: IT Service Management needs effective ticket prioritization but faces challenges from noisy textual inputs, subjective writing styles, and class imbalance in ticket data.

Method: Compared two approaches: 1) embedding-based pipelines combining dimensionality reduction, clustering, and classical classifiers, and 2) a fine-tuned multilingual transformer processing both textual and numerical features.

Result: Transformer model achieved substantially higher performance (78.5% average F1-score, weighted Cohen's kappa ~0.80) compared to embedding-based methods which showed limited generalization across 30 configurations.

Conclusion: Generic embeddings are limited for ITSM data, while domain-adapted transformer architectures are highly effective for operational ticket prioritization.
...
TL;DR: KVReviver: A reversible KV cache compression method using sketch algorithms to maintain full inference accuracy with only 10-25% of KV cache memory.


<details>
  <summary>Details</summary>
Motivation: As LLM context lengths increase, KV cache memory becomes a bottleneck. Traditional compression methods cause irreversible information loss (Contextual Amnesia), degrading model performance.

Method: KVReviver uses sketch algorithms to compress KV cache reversibly, allowing reconstruction of compressed tokens from additional data structures while enabling full computation within limited memory.

Result: For 2k-length contexts: 10% KV cache budget with identical accuracy. For 32k-length contexts: 25% KV cache budget with ~2% accuracy loss or comparable performance.

Conclusion: KVReviver effectively addresses KV cache memory bottlenecks through reversible compression, maintaining high accuracy while significantly reducing memory requirements.
...
TL;DR: LLMs show degraded performance under prompt compression, with constraint compliance following a U-curve pattern (best at extreme compression, worst at medium compression). RLHF-trained helpfulness behaviors cause constraint violations, and reasoning models outperform efficient models.


<details>
  <summary>Details</summary>
Motivation: Large language models exhibit degraded performance under prompt compression, but the underlying mechanisms remain poorly understood. The researchers aim to systematically analyze how compression affects LLM performance and identify the root causes of degradation.

Method: Introduced Compression-Decay Comprehension Test (CDCT) benchmark to independently measure constraint compliance and semantic accuracy across compression levels. Evaluated 9 frontier LLMs across 8 concepts using 5 compression levels (extreme to none). Used three-judge LLM jury with high inter-rater agreement. Conducted RLHF ablation experiments to validate constraint salience hypothesis.

Result: Found universal U-curve pattern in constraint compliance (97.2% prevalence), with violations peaking at medium compression. Models perform better at extreme compression than medium lengths. Constraint and semantic dimensions are statistically orthogonal. RLHF ablation improved constraint compliance by 598% on average, confirming that RLHF-trained helpfulness behaviors cause constraint violations. Reasoning models outperformed efficient models by 27.5%.

Conclusion: Reveals fundamental tension between RLHF alignment and instruction-following. RLHF-trained helpfulness behaviors are the dominant cause of constraint violations at medium compression. Provides actionable guidelines for improving deployed systems by addressing this tension.
...
TL;DR: ReGal framework applies reinforcement learning to Indian legal AI, combining instruction tuning with RLAIF using PPO, but underperforms on standard metrics while revealing challenges in legal RL applications.


<details>
  <summary>Details</summary>
Motivation: To explore reinforcement learning methodologies for legal AI in the Indian context, addressing the need for interpretable and adaptive legal reasoning systems that can handle high-stakes, long-document legal tasks.

Method: Reinforcement Learning-based Legal Reasoning (ReGal) framework integrating Multi-Task Instruction Tuning with Reinforcement Learning from AI Feedback (RLAIF) using Proximal Policy Optimization (PPO), evaluated on Court Judgment Prediction and Explanation (CJPE) and Legal Document Summarization tasks.

Result: The framework underperforms on standard evaluation metrics compared to supervised and proprietary models, but provides valuable insights into challenges including reward model alignment, legal language complexity, and domain-specific adaptation in RL applications.

Conclusion: Despite performance limitations, the work establishes a foundation for future optimization of legal reasoning pipelines using reinforcement learning, with broader implications for building interpretable and adaptive legal AI systems.
...
TL;DR: CoPE is a small, policy-steerable language model for content labeling that achieves frontier model accuracy at 1% size, using novel training and policy generation methods.


<details>
  <summary>Details</summary>
Motivation: Current content moderation systems require large, expensive models that are difficult to customize for specific platform policies. There's a need for smaller, more interpretable models that can accurately apply human-written policies rather than just memorizing training data.

Method: Two novel approaches: 1) Contradictory Example Training - a curriculum that teaches policy interpretation rather than memorization, and 2) Binocular Labeling - a method for generating unambiguous policy training datasets. The model is designed to be small (9B parameters) and runnable on consumer GPUs.
...
Conclusion: CoPE represents a paradigm shift for classifier systems by transforming ML tasks into policy writing tasks, opening new possibilities for online platform governance. The model is openly released and demonstrates that small, policy-steerable models can match large frontier models for content labeling.
...
TL;DR: This paper introduces Narrative Consolidation as a new NLP task for creating unified, chronologically sound texts from overlapping narratives, and proposes a Temporal Alignment Event Graph (TAEG) method that achieves perfect temporal ordering and significant content improvements.


<details>
  <summary>Details</summary>
Motivation: Standard Multi-Document Summarization (MDS) focuses on conciseness but fails to preserve narrative flow and chronological integrity when processing overlapping documents like legal testimonies or historical accounts. There's a need for a new approach that prioritizes coherent, chronologically sound unified texts.

Method: The paper introduces Temporal Alignment Event Graph (TAEG), a graph structure that explicitly models chronology and event alignment. By applying standard centrality algorithms to TAEG, the method functions as a version selection mechanism, choosing the most central representation of each event in its correct temporal position.

Result: In experiments on the four Biblical Gospels, the TAEG-based approach guarantees perfect temporal ordering (Kendall's Tau of 1.000) by design and dramatically improves content metrics, achieving +357.2% improvement in ROUGE-L F1 score.

Conclusion: The success of this baseline method validates Narrative Consolidation as a relevant NLP task and establishes that an explicit temporal backbone is fundamental for solving it. The approach demonstrates that focusing on temporal structure enables both perfect chronological ordering and significant content quality improvements.
...
TL;DR: The paper analyzes Heaps' law (vocabulary growth with document length) in conversations, finding that vocabulary scaling differs by parts of speech across video chat conversations between strangers and fictional movie dialogues.


<details>
  <summary>Details</summary>
Motivation: Conversations are fundamental to social connection and well-being, but little research has applied Heaps' law to study how conversations unfold over time, particularly how different language features affect vocabulary scaling patterns.

Method: Researchers measured Heaps' law in two distinct conversational datasets: 1) video chat conversations between strangers, and 2) fictional character dialogues from movies. They analyzed how vocabulary size scales with conversation length, with particular focus on differences across parts of speech categories.

Result: Vocabulary scaling patterns (Heaps' law) differ significantly by parts of speech categories. The study reveals distinct scaling behaviors for different grammatical categories across both real stranger conversations and fictional movie dialogues.

Conclusion: The findings demonstrate that vocabulary growth in conversations follows different patterns for different parts of speech, providing insights that can be interpreted through both behavioral and linguistic theoretical frameworks to better understand conversation dynamics.
...
TL;DR: LogicReward: A novel reward system using theorem prover to enforce step-level logical correctness in LLM training, improving reasoning faithfulness and performance on logical tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LLM training methods rely on outcome-based feedback which can produce correct answers with flawed reasoning. Prior work lacks guarantees of logical soundness, which is crucial in high-stakes scenarios where logical consistency is paramount.

Method: Proposes LogicReward, a reward system that guides model training by enforcing step-level logical correctness using a theorem prover. Introduces Autoformalization with Soft Unification to reduce natural language ambiguity and improve formalization quality for better theorem prover utilization.
...
Conclusion: LogicReward effectively addresses logical soundness issues in LLM training by using theorem proving for step-level logical correctness, leading to improved performance, faithfulness, and generalizability in reasoning tasks.
...
TL;DR: GeoSense-AI: A real-time NLP pipeline for extracting geolocation from noisy microblog streams during emergencies, combining hashtag segmentation, proper-noun detection, dependency parsing, NER, and gazetteer disambiguation to overcome sparse geotag limitations.


<details>
  <summary>Details</summary>
Motivation: Traditional geolocation in crisis informatics relies on sparse geotags, which are often unavailable in microblog streams. There's a need for real-time location extraction directly from informal text to support situational awareness during fast-moving emergencies like floods and outbreaks.

Method: Unified pipeline with statistical hashtag segmentation, part-of-speech-driven proper-noun detection, dependency parsing around disaster lexicons, lightweight named-entity recognition, and gazetteer-grounded disambiguation. Designed for low-latency streaming with efficient validation against geographic knowledge bases.

Result: System achieves strong F1 scores comparable to widely used NER toolkits while being engineered for orders-of-magnitude faster throughput. Production map interface demonstrates end-to-end functionality for ingest, inference, and visualization of locational signals at scale.

Conclusion: Domain-tuned NLP with knowledge grounding can effectively extract geolocation from informal text in real-time, elevating emergency response beyond conventional geotag reliance by providing robust, streaming-efficient location extraction for crisis informatics.
...
TL;DR: The paper proposes InstructNet, a multi-label classification system for instructional text using transformer architectures like XLNet and BERT, achieving 97.30% accuracy on wikiHow data.


<details>
  <summary>Details</summary>
Motivation: People increasingly use "How To" searches to find instructional content, making categorization of instructional text essential for task-oriented learning and knowledge base creation. There's a need for effective multi-label classification of instructional articles.

Method: Used transformer-based architectures (XLNet, BERT) for multi-label instruction classification on a dataset of 11,121 wikiHow articles with multiple categories. Proposed InstructNet approach with thorough evaluation using accuracy and macro F1-score metrics.

Result: XLNet architecture achieved unprecedented performance: 97.30% accuracy, 89.02% micro average, and 93% macro average scores. This demonstrates superior effectiveness compared to other architectures for multi-label classification of instructional text.

Conclusion: XLNet-based InstructNet approach is highly effective for multi-label instruction classification. The multi-level evaluation strategy provides comprehensive insights into architecture strengths and identifies areas for future improvement in instructional text categorization.
...
TL;DR: CTTA-T: A continual test-time adaptation framework for text understanding that handles sequential unobserved testing domains using a domain-aware teacher-student approach with refine-then-filter and incremental PCA.


<details>
  <summary>Details</summary>
Motivation: Current continual test-time adaptation (CTTA) methods struggle with error accumulation across domains and generalization to unobserved domains. Existing approaches either discard useful information through noise-filtering or fail to achieve adaptive accumulation of historical domains.

Method: Proposes CTTA-T framework with teacher-student architecture: 1) refine-then-filter based on dropout-driven consistency to calibrate predictions and remove unreliable guidance, 2) domain-aware teacher construction via incremental PCA to dynamically accumulate cross-domain semantics and track domain shifts.

Result: Experiments show CTTA-T outperforms baseline methods in continual test-time adaptation for text understanding tasks.

Conclusion: CTTA-T effectively addresses the adaptation-generalization trade-off in continual test-time adaptation for text understanding by combining refined filtering with dynamic domain-aware teacher construction.
...
TL;DR: LiR³AG framework enables non-reasoning LLMs to achieve reasoning model performance in RAG multi-hop QA by transferring structured reasoning strategies, reducing computational costs by 98% tokens and 58.6% inference time.


<details>
  <summary>Details</summary>
Motivation: Reasoning models improve RAG multi-hop QA performance but introduce substantial computational costs (token consumption and inference latency). There's a need to understand and mitigate this trade-off between performance and efficiency.

Method: Proposed LiR³AG (Lightweight Rerank Reasoning Strategy Framework for RAG) that restructures retrieved evidence into coherent reasoning chains, enabling non-reasoning models to transfer reasoning strategies. Identified two reasoning modes: Context-Grounded Reasoning (relying on retrieved content) and Knowledge-Reconciled Reasoning (resolving conflicts using internal knowledge).

Result: LiR³AG reduces average output tokens by 98% and inference time by 58.6% while improving 8B non-reasoning model's F1 performance by 6.2% to 22.5%, surpassing 32B reasoning model performance in RAG.

Conclusion: The framework offers a practical and efficient path forward for RAG systems by enabling lightweight models to achieve reasoning capabilities without the computational overhead of traditional reasoning models.
...
<details>
  <summary>Details</summary>
Motivation: Real-world deployment of LLM-based agents is hindered by severe inefficiencies from systemic latency accumulated across reasoning loops, context growth, and heterogeneous tool interactions, not just isolated model inference.

Method: Four synergistic components: AgentCollab (hierarchical dual-model reasoning with dynamic role assignment), AgentSched (cache-aware hybrid scheduler), AgentSAM (suffix-automaton-based speculative decoding reusing semantic memory), and AgentCompress (asynchronous semantic compression for agent memory). These form a Self-Evolution Engine.
...
<details>
  <summary>Details</summary>
Motivation: Early Rumor Detection is challenging in data-scarce settings. LLMs perform well in few-shot NLP tasks but are not suited for time-series data and are computationally expensive for both training and inference.

Method: Combines an autonomous agent and an LLM-based detection model: the agent acts as a reliable decision-maker for early time point determination, while the LLM serves as a powerful rumor detector. Only the lightweight agent needs training, keeping the LLM training-free.
...
Conclusion: The proposed framework offers the first solution for few-shot Early Rumor Detection, requiring only lightweight agent training while maintaining LLM training-free, achieving superior performance in accuracy and earliness.
...
TL;DR: DACE framework wins TextMine'26 competition for French railway acronym disambiguation with 0.9069 F1 score using dynamic prompting, RAG, and ensemble methods.


<details>
  <summary>Details</summary>
Motivation: Acronym disambiguation is crucial for technical text processing but challenging in specialized domains like French railway documentation where high ambiguity exists, requiring automated solutions.

Method: DACE framework combines Dynamic Prompting, Retrieval Augmented Generation (RAG), Contextual Selection, and Ensemble Aggregation to enhance LLMs through adaptive in-context learning and external domain knowledge injection.
...
Conclusion: The DACE framework successfully addresses acronym disambiguation in specialized domains by dynamically adapting to ambiguity and leveraging ensemble predictions with domain knowledge.
...
TL;DR: Novel neurosymbolic framework for RDF-to-text generation using collaborative LLM agents to produce rule-based Python code, requiring no training data and generating interpretable, hallucination-reduced text.


<details>
  <summary>Details</summary>
Motivation: To create an interpretable RDF-to-text generation system that avoids hallucination issues of traditional models, requires no supervised training data, and operates efficiently without GPU resources.

Method: Collaborative interactions among multiple LLM agents produce rule-based Python code for text generation from RDF triples, without backpropagation or human reference texts for the target domain.

Result: Experiments on WebNLG and OpenDialKG datasets show reduced hallucination with only slight fluency penalties compared to finetuned or prompted language models, while being fully interpretable and CPU-only.

Conclusion: The neurosymbolic framework demonstrates effective RDF-to-text generation through LLM agent collaboration, offering interpretability, no training data requirements, and efficient CPU operation with reduced hallucination.
...
TL;DR: LLMs generate personalized stories for language learners using only known vocabulary, with spaced repetition for optimal vocabulary learning.


<details>
  <summary>Details</summary>
Motivation: To create effective language learning materials that teach new vocabulary through enjoyable reading experiences while reviewing recently learned words, addressing the need for personalized, context-based vocabulary acquisition.

Method: Use large language models to generate personalized stories constrained to learner's known vocabulary, with three story generation methods and three lexical constraint strategies, integrated with Spaced Repetition System for vocabulary optimization.

Result: Generated stories are more grammatical, coherent, and provide better word usage examples than standard constrained beam search, validated across English, Chinese, and Polish languages.

Conclusion: LLM-based personalized story generation with vocabulary constraints and spaced repetition is an effective approach for language learning, outperforming traditional methods in producing high-quality educational content.
...
TL;DR: AraToken: Arabic-optimized tokenizer with normalization pipeline and Language Extension Pipeline for efficient Arabic language processing in LLMs.


<details>
  <summary>Details</summary>
Motivation: General-purpose tokenizers perform poorly on morphologically rich languages like Arabic, causing inflated token sequences and reduced compression efficiency.

Method: Built AraToken using SentencePiece Unigram algorithm with Arabic-specific normalization (Alif variants, diacritics, numerals). Developed Language Extension Pipeline (LEP) for vocabulary extension with mean subtoken initialization and selective layer unfreezing.

Result: SentencePiece with normalization achieves 18% lower fertility (1.199 vs 1.35 tokens/word). LEP reduces evaluation loss from 8.28 to 2.43 within 800 steps on 100K Arabic samples.

Conclusion: Arabic-optimized tokenization significantly improves efficiency for Arabic NLP. The approach enables better LLM performance on Arabic while releasing resources to facilitate research.
...
TL;DR: Agentic framework for training general practitioner students using AI-powered virtual simulated patients with configurable vignette generation, persona-driven dialogue, and standards-based assessment.


<details>
  <summary>Details</summary>
Motivation: Current virtual simulated patients (VSPs) in medical education have limitations in medical accuracy, consistent roleplaying, scenario generation, and structured feedback, despite the potential of large language models to provide scalable alternatives to resource-intensive traditional methods.

Method: An agentic framework that unifies three components: (1) configurable, evidence-based vignette generation, (2) controlled persona-driven patient dialogue with optional retrieval grounding, and (3) standards-based assessment and feedback for both communication and clinical reasoning. The framework was instantiated in an interactive spoken consultation setting.
...
Conclusion: Agentic separation of scenario control, interaction control, and standards-based assessment is a practical pattern for building dependable and pedagogically valuable VSP training tools.
...
TL;DR: Automated pipeline improves NLI models by detecting semantic artifacts, generating synthetic contrast sets, and using dynamic balanced sampling to prevent forgetting.


<details>
  <summary>Details</summary>
Motivation: NLI models often rely on spurious correlations instead of semantic reasoning. Existing mitigation strategies have high annotation costs or cause catastrophic forgetting during fine-tuning.

Method: Three-step pipeline: 1) Log-Frequency LMI (LF-LMI) to detect semantic artifacts, 2) LLM-synthesis pipeline with multi-judge verification to generate high-quality synthetic contrast sets, 3) Dynamic Balanced Sampling training strategy that rotates original data distribution to prevent forgetting.
...
Conclusion: Proposed automated, scalable pipeline effectively addresses spurious correlations in NLI models without high annotation costs or catastrophic forgetting, achieving substantial improvements in model consistency.
...
<details>
  <summary>Details</summary>
Motivation: To enhance web content classification by addressing limitations of individual neural architectures (CNNs, LSTMs, transformers) through a hybrid approach that captures both local patterns and long-range dependencies while focusing on informative text segments.

Method: Proposed hybrid architecture combining LSTM (for sequential dependencies), CNN (for local n-gram patterns), and Attention mechanism (for selective focus). Uses pretrained GloVe embeddings for semantic word representations. Evaluated with 5-fold cross-validation.

Result: Achieved outstanding performance: accuracy 0.98, precision 0.94, recall 0.92, F1-score 0.93. Surpassed baseline models (CNN-only, LSTM-only, BERT). GloVe embeddings enabled efficient representation suitable for real-time systems.

Conclusion: Hybrid LSTM-CNN-Attention architecture effectively combines syntactic feature extraction and semantic interpretation for web content classification. Demonstrates superiority over individual architectures and supports broader use of hybrid deep learning in NLP applications.
...
TL;DR: A seminar course designed to help students critically examine and define abstract NLP concepts like "interpretability" and "bias" that are often used without clear definitions.


<details>
  <summary>Details</summary>
Motivation: NLP researchers frequently use abstract concepts like "interpretability," "bias," "reasoning," and "stereotypes" without clearly defining them, leading to operational decisions (datasets, metrics, claims) based on potentially unexamined assumptions.

Method: Created an interdisciplinary seminar course with reading lists emphasizing discussion and critique, where students explore questions of conceptualization (what terms mean) and operationalization (how to measure them).

Result: The paper outlines the structure and approach of this seminar, providing a framework for students to critically examine foundational concepts in NLP research.

Conclusion: There is a need for more rigorous examination of core concepts in NLP, and educational interventions like this seminar can help develop critical thinking about conceptualization and operationalization in the field.
...
TL;DR: Political fact-checking systems face a hard performance ceiling around F1=0.32, with simple linear models matching transformer performance, revealing that model complexity alone doesn't solve semantic ambiguity in political disinformation.


<details>
  <summary>Details</summary>
Motivation: To understand the empirical limits of text-only linguistic modeling for detecting linguistically subtle political disinformation, and to determine whether increasing model complexity actually improves performance.

Method: Systematic diagnostic evaluation of nine ML algorithms on LIAR benchmark, isolating lexical features (Bag-of-Words, TF-IDF) and semantic embeddings (GloVe), comparing performance across models including linear SVM and pre-trained Transformers like RoBERTa.

Result: Found a hard "Performance Ceiling" with fine-grained classification not exceeding Weighted F1-score of 0.32; linear SVM (Accuracy: 0.624) matched RoBERTa (Accuracy: 0.620); tree-based ensembles showed massive "Generalization Gap" (99% training vs 25% test accuracy); synthetic data augmentation via SMOTE yielded no gains.

Conclusion: For political fact-checking, increasing model complexity without incorporating external knowledge yields diminishing returns due to semantic feature ambiguity rather than distributional issues.
...
TL;DR: First controlled study shows psychoactive persona prompts (LSD, cocaine, alcohol, cannabis) significantly degrade GPT-5-mini performance on ARC-Challenge compared to sober control, with alcohol causing the worst collapse.


<details>
  <summary>Details</summary>
Motivation: LLMs are sensitive to personas at inference time, but prompt-level "drug" interventions have never been rigorously benchmarked. The paper aims to provide the first controlled study of how psychoactive framings affect model performance.

Method: Used GPT-5-mini on ARC-Challenge with four single-sentence psychoactive prompts (LSD, cocaine, alcohol, cannabis) vs sober control. Tested 100 validation items per condition with deterministic decoding, full logging, Wilson confidence intervals, and Fisher exact tests.

Result: Control accuracy was 0.45. All drug prompts significantly reduced performance: alcohol collapsed to 0.10 (p = 3.2e-8), cocaine to 0.21 (p = 4.9e-4), LSD to 0.19 (p = 1.3e-4), and cannabis to 0.30 (p = 0.041). Persona prompts disrupted the mandated answer template.

Conclusion: Persona text acts like a "few-shot consumable" that can destroy reliability without modifying model weights. This demonstrates the fragility of LLMs to inference-time persona prompts and highlights the need for careful prompt engineering.
...
<details>
  <summary>Details</summary>
Motivation: Neologisms offer a lightweight alternative to fine-tuning for steering model behavior, requiring training only d parameters while maintaining access to the model's default behavior, unlike fine-tuning which requires more compute and reduces flexibility.

Method: Compare neologism learning against low-rank adaptation (LoRA) fine-tuning under matched training setup (same data and hyperparameters), and investigate self-verbalizations where models create their own new words when asked about neologisms.

Result: Neologisms outperform fine-tuned models under matched training conditions, and models occasionally generate their own new words when prompted about neologisms.

Conclusion: Neologism learning is an effective, parameter-efficient method for behavioral steering that can outperform traditional fine-tuning approaches like LoRA while maintaining model flexibility.
...
TL;DR: This paper presents English-Hindi legal machine translation work for JUST-NLP 2025, showing that fine-tuning OPUS-MT outperforms training from scratch, achieving 46.03 SacreBLEU score.


<details>
  <summary>Details</summary>
Motivation: In multilingual nations like India, language barriers hinder access to legal information as most legal/judicial documentation remains in English. Legal Machine Translation (L-MT) offers a scalable solution to enable accurate and accessible translations of legal documents.

Method: Two complementary strategies: 1) Fine-tuning a pre-trained OPUS-MT model for domain-specific adaptation, and 2) Training a Transformer model from scratch using the provided legal corpus. Evaluation uses standard MT metrics including SacreBLEU, chrF++, TER, ROUGE, BERTScore, METEOR, and COMET.

Result: The fine-tuned OPUS-MT model achieves a SacreBLEU score of 46.03, significantly outperforming both baseline and from-scratch models. Domain adaptation proves highly effective for enhancing translation quality.

Conclusion: The results highlight the effectiveness of domain adaptation in enhancing translation quality and demonstrate the potential of L-MT systems to improve access to justice and legal transparency in multilingual contexts like India.
...
TL;DR: FIND benchmark tests language models' ability to detect inconsistencies in long, technical documents, with GPT-5 achieving 64% detection rate and also finding previously undiscovered inconsistencies in real documents.


<details>
  <summary>Details</summary>
Motivation: Professional document auditing is crucial but time-consuming in academia, law, and finance, where inconsistencies can lead to significant costs. Language models could potentially accelerate this auditing process, but their capabilities need systematic evaluation.

Method: Created FIND benchmark with documents containing manually inserted inconsistencies by domain experts. Tested language models on these long, technical documents. Also evaluated models on 50 real arXiv papers to find undiscovered inconsistencies.

Result: GPT-5 detected 64% of inserted inconsistencies in FIND benchmark. Surprisingly, it also found 136 legitimate inconsistencies out of 196 suggestions in real arXiv papers that were missed by original authors. However, even the best models still miss almost half of the inconsistencies.

Conclusion: Language models show promising ability to detect inconsistencies in complex documents and can find errors missed by human authors, but inconsistency detection remains a challenging task with significant room for improvement.
...
TL;DR: Lightweight models (T5-small and Mistral-Instruct-v0.3) achieve PII masking performance comparable to frontier LLMs, with trade-offs: Mistral has higher F1/recall but slower inference, while T5 offers structured outputs and lower cost.


<details>
  <summary>Details</summary>
Motivation: Address privacy concerns in conversational systems by exploring whether lightweight models can match frontier LLMs' PII masking capabilities while avoiding data handling issues and reducing computational costs.

Method: Fine-tuned encoder-decoder (T5-small) and decoder-only (Mistral-Instruct-v0.3) architectures on English datasets from AI4Privacy benchmark. Created dataset variants to study label standardization and PII representation across 24 categories. Used entity-level/character-level metrics, type accuracy, and exact match for evaluation.

Result: Both lightweight models achieve performance comparable to frontier LLMs. Label normalization consistently improves performance. Mistral achieves higher F1/recall with greater robustness across PII types but has higher generation latency. T5 offers more controllable structured outputs and lower inference cost, though less robust in conversational text. Real-world Discord bot testing showed performance degradation with informal inputs.

Conclusion: Lightweight models provide effective PII masking addressing data handling concerns of frontier LLMs, with clear trade-offs: Mistral for accuracy/robustness, T5 for structured outputs and cost efficiency. Both architectures demonstrate practical viability for privacy-preserving systems.
...
TL;DR: LLM-CAS is a hierarchical reinforcement learning framework that dynamically corrects LLM hallucinations during inference using temporary neuron perturbations, outperforming existing methods on factual accuracy benchmarks.


<details>
  <summary>Details</summary>
Motivation: LLMs often generate hallucinated content lacking factual grounding, limiting reliability in critical applications. Existing approaches (supervised fine-tuning, RLHF) are data/compute intensive, while static parameter editing methods struggle with context-dependent errors and catastrophic forgetting.

Method: LLM-CAS formulates real-time hallucination correction as hierarchical reinforcement learning. It trains an agent to learn a policy that dynamically selects temporary neuron perturbations during inference based on current context, enabling adaptive fine-grained correction without permanent parameter modification.

Result: Experiments show LLM-CAS consistently improves factual accuracy: +10.98pp on StoryCloze, +2.71pp on TriviaQA, +2.06pp on TruthfulQA MC1 score. Outperforms static editing methods (ITI, CAA) and dynamic SADI framework.

Conclusion: LLM-CAS provides an efficient, context-aware solution for improving LLM reliability with promising potential for future multimodal extensions.
...
TL;DR: The paper proposes a world model architecture for automating capitalization table tie-out, a complex legal diligence task requiring multi-document reasoning and evidence traceability that current LLMs and agentic systems cannot reliably handle.


<details>
  <summary>Details</summary>
Motivation: Current LLMs and agentic systems fail at specialized legal workflows like capitalization tie-out, which requires multi-document reasoning, strict evidence traceability, and deterministic outputs that existing approaches cannot reliably deliver.

Method: Characterize capitalization tie-out as a real-world legal AI benchmark, analyze existing agentic systems' performance, and propose a world model architecture specifically designed for tie-out automation and applied legal intelligence.

Result: The paper identifies the limitations of current AI systems for legal workflows and proposes a specialized architecture to address the requirements of multi-document reasoning, evidence traceability, and deterministic outputs needed for capitalization tie-out.

Conclusion: A world model architecture is needed to automate complex legal tasks like capitalization tie-out, serving as a foundation for broader applied legal intelligence beyond current LLM capabilities.
...
TL;DR: APF is an LLM-based framework that automatically converts natural language design requirements into executable optimization models without needing solver feedback, using automated data generation and fine-tuning to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Translating ambiguous design requirements into mathematical optimization formulations is time-consuming and expert-dependent in high-cost simulation-driven design domains. Existing LLM approaches either produce poor formalization or rely on unavailable solver feedback due to high simulation costs.

Method: APF uses an innovative pipeline for automatically generating high-quality fine-tuning datasets without solver feedback, combining data generation and test instance annotation. This dataset is used for supervised fine-tuning of LLMs to enhance their ability to generate accurate, executable optimization problem formulations.
...
Conclusion: APF provides a solver-independent, automated framework for converting natural language design requirements into optimization models, overcoming the data scarcity problem in high-cost simulation domains and improving design automation.
...
TL;DR: MemEvolve is a meta-evolutionary framework that jointly evolves agents' experiential knowledge and their memory architecture, enabling self-evolving LLM-based agents to not only accumulate experience but also refine how they learn from it.


<details>
  <summary>Details</summary>
Motivation: Current self-evolving memory systems for LLM-based agents rely on static, manually engineered memory architectures that cannot adapt to diverse task contexts. While memory facilitates agent-level evolution, the memory architecture itself remains fixed and cannot be meta-adapted.

Method: Proposes MemEvolve framework that jointly evolves both agents' experiential knowledge and their memory architecture. Also introduces EvolveLab, a unified codebase that distills 12 representative memory systems into a modular design space (encode, store, retrieve, manage) for standardized implementation and fair evaluation.

Result: Extensive evaluations on four challenging agentic benchmarks show: (1) substantial performance gains (up to 17.06% improvement over frameworks like SmolAgent and Flash-Searcher), and (2) strong cross-task and cross-LLM generalization, with memory architectures that transfer effectively across diverse benchmarks and backbone models.

Conclusion: MemEvolve addresses the fundamental limitation of static memory architectures in self-evolving agent systems by enabling meta-evolution of both knowledge and memory structure, demonstrating significant performance improvements and generalization capabilities across tasks and models.
...
TL;DR: The paper introduces a four-paradigm framework for semantic channel finding in complex experimental facilities, mapping natural-language queries to control-system signals with 90-97% accuracy across diverse facilities.


<details>
  <summary>Details</summary>
Motivation: Complex experimental facilities have tens to hundreds of thousands of control channels with inconsistent naming and fragmented documentation, creating bottlenecks for operators and AI systems who need to locate signals for monitoring, troubleshooting, and automated control.
...
Conclusion: The formalized semantic channel finding problem and four-paradigm framework provide a systematic approach to address the persistent bottleneck of locating control signals in complex experimental infrastructure, enabling more reliable, scalable, and language-model-driven interfaces.
...
TL;DR: LLM-based world models can improve agent learning in text environments through simulated experience, but effectiveness depends on behavioral coverage and environment complexity.


<details>
  <summary>Details</summary>
Motivation: Real-world environments for agentic reinforcement learning are non-adaptive, limited in coverage, and difficult to scale. While world models offer potential efficiency gains through simulated experience, it's unclear whether LLMs can reliably serve as world models and under what conditions they meaningfully benefit agents.
...
Result: Sufficiently trained world models maintain coherent latent state, scale predictably with data and model size, and improve agent performance via action verification, synthetic trajectory generation, and warm-starting reinforcement learning.

Conclusion: LLM-based world models can effectively support agent learning, but gains depend critically on behavioral coverage and environment complexity, establishing clear boundaries on when world modeling is beneficial.
...
TL;DR: AraMix is a deduplicated Arabic pretraining corpus (178B tokens, 179M docs) created by combining and curating 7 existing Arabic web datasets, showing that systematic reuse of existing data yields better returns than new web scraping for lower-resource languages.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that substantial value lies in systematically reusing and curating existing pretraining datasets rather than scraping the web again, especially for lower-resource languages like Arabic where data collection is challenging.

Method: Combined seven publicly available Arabic web datasets, applied Arabic-specific quality filtering to re-filter some datasets, and performed cross-dataset deduplication using both MinHash and sentence-level approaches.

Result: Created the largest heavily filtered publicly available Arabic pretraining corpus (178B tokens, 179M documents) and discovered that nearly 60% of tokens across independently collected corpora are duplicates, revealing significant redundancy in existing datasets.

Conclusion: For lower-resource languages, investment in curation pipelines for existing data yields greater returns than additional web crawls, as demonstrated by AraMix's success in creating a high-quality Arabic corpus through systematic reuse of existing datasets.
...
TL;DR: MDToC is a metacognitive prompting method that builds concept trees with verified calculations and majority voting to improve LLM mathematical reasoning, outperforming existing methods like ToT and GoT across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Despite advances in mathematical reasoning, LLMs still struggle with calculation verification using current prompting techniques. There's a need for better methods to ensure accurate mathematical calculations without relying on hand-engineered hints.

Method: Three-phase approach: 1) Constructs a concept tree to break down problems, 2) Develops accuracy-verified calculations for each concept, 3) Employs majority voting to evaluate competing solutions. This metacognitive approach focuses on calculation verification.

Result: MDToC achieves 58.1% on CHAMP, 86.6% on MATH, and 85% on Game-of-24 using GPT-4-Turbo, outperforming GoT by 5%, 5.4%, and 4% respectively. Consistently surpasses existing prompting methods across all backbone models with improvements up to 7.6% over ToT and 6.2% over GoT.

Conclusion: Metacognitive calculation verification through MDToC represents a promising direction for enhanced mathematical reasoning in LLMs, offering consistent improvements over state-of-the-art prompting methods without requiring hand-engineered hints.
...
TL;DR: A human-centered AI framework is proposed for terminology work to preserve professional autonomy while leveraging AI's efficiency, emphasizing human control, bias mitigation, and terminologist-centered design.


<details>
  <summary>Details</summary>
Motivation: The rapid diffusion of generative AI in terminology work risks weakening professional autonomy, amplifying bias, and eroding linguistic/conceptual diversity, necessitating a human-centered approach to ensure AI serves rather than replaces terminologists.

Method: Proposes a human-centered framework organized around three dimensions: the augmented terminologist (AI amplifies capabilities), ethical AI (human control and bias mitigation), and human-centered design (tools designed around terminologist needs and values).

Result: A framework that conceptualizes AI as a means to amplify terminologists' capabilities rather than replace them, emphasizing compatibility of high automation with strong human control and terminologist-centered design.

Conclusion: Current AI adoption choices will shape not only terminological practice but also the preservation of accuracy, adequacy, and diversity in terminology and specialized knowledge, making human-centered approaches essential.
...
TL;DR: LLMs fail to accurately estimate question difficulty for human learners despite their strong problem-solving abilities, showing systematic misalignment with human cognitive struggles.


<details>
  <summary>Details</summary>
Motivation: Accurate item difficulty estimation is critical for educational assessment but suffers from cold start problems. While LLMs have superhuman problem-solving capabilities, it's unclear if they can perceive human learners' cognitive struggles.

Method: Large-scale empirical analysis of Human-AI Difficulty Alignment across 20+ models in domains like medical knowledge and mathematical reasoning, examining how model scaling affects difficulty estimation.

Result: Systematic misalignment where scaling model size doesn't help; models converge to machine consensus rather than aligning with humans. High performance impedes accurate difficulty estimation, and models lack introspection about their own limitations.
...
TL;DR: Remedy-R is a reasoning-driven MT metric that produces interpretable step-by-step analyses of translation quality and shows strong performance on benchmarks while being robust to OOD inputs. It can also be used to improve translations through an evaluate-revise pipeline.


<details>
  <summary>Details</summary>
Motivation: Current automatic MT metrics are black-box systems that lack interpretability and often fail on out-of-distribution inputs, despite showing strong performance on benchmarks. There's a need for more transparent, interpretable metrics that can provide insight into their decision-making process.

Method: Remedy-R is trained with reinforcement learning from pairwise translation preferences without requiring error-span annotations or distillation from closed LLMs. It produces step-by-step analyses of accuracy, fluency, and completeness before generating a final score. The method uses only 60K training pairs across two language pairs.

Result: Remedy-R remains competitive with top scalar metrics and GPT-4-based judges on WMT22-24 meta-evaluation, generalizes to other languages, and exhibits strong robustness on OOD stress tests. The Remedy-R Agent (evaluate-revise pipeline) consistently improves translation quality across diverse models including Qwen2.5, ALMA-R, GPT-4o-mini, and Gemini-2.0-Flash.

Conclusion: Remedy-R provides an interpretable, reasoning-driven approach to MT evaluation that captures translation-relevant information and can be practically useful for both assessment and translation improvement. Its self-reflective feedback enables the creation of effective evaluate-revise pipelines for translation refinement.
...
TL;DR: FASTRIC is a Prompt Specification Language that makes implicit Finite State Machines explicit in natural language prompts, enabling verification of LLM execution against designer intent through trace analysis.


<details>
  <summary>Details</summary>
Motivation: LLMs execute complex multi-turn interactions but lack formal specifications to verify execution against designer intent, making interaction design a heuristic art rather than systematic engineering.

Method: FASTRIC guides designers to articulate seven FSM elements (Final States, Agents, States, Triggers, Roles, Initial State, Constraints) in natural language prompts. The LLM serves as both parser and interpreter of these specifications, with formality levels ranging from implicit to explicit instructions.

Result: Testing across model scales reveals optimal specification formality depends on model capacity: DeepSeek-V3.2 (685B) achieves perfect conformance at L2-L4, ChatGPT-5 (~1T) peaks at L3 (0.90) before collapsing at L4 (0.39), while Phi4 (14.7B) shows no stable optimum with high variance.

Conclusion: Model-specific "Goldilocks zones" exist where specifications provide sufficient structure without over-constraint, establishing Prompt Specification Engineering for creating verifiable interaction protocols with measurable procedural guarantees.
...
TL;DR: Modular pipeline with structured process control outperforms end-to-end LLM approach for medical follow-up tasks, improving dialog stability, extraction accuracy, and efficiency.


<details>
  <summary>Details</summary>
Motivation: LLMs applied directly to medical follow-up tasks suffer from uncontrolled dialog flow and inaccurate information extraction due to complex follow-up forms, necessitating better control mechanisms.

Method: Designed and compared two systems: end-to-end LLM-based (control) vs. modular pipeline with structured process control (experimental) using task decomposition, semantic clustering, and flow management.
...
Conclusion: External control mechanisms are necessary when deploying LLMs in high-stakes medical follow-up scenarios to ensure reliable performance.
...
TL;DR: DLLMs enable parallel token decoding but require many denoising iterations. The paper proposes training-free context-aware initialization to shorten the generative trajectory by starting closer to the target distribution, reducing iterations by ~35% on GSM8K.


<details>
  <summary>Details</summary>
Motivation: Diffusion Large Language Models (DLLMs) enable fully parallel token decoding but remain impractical at inference due to many denoising iterations needed to refine information-free masked initialization into coherent text. Existing acceleration methods focus on improving solvers/sampling, but the authors propose shortening the trajectory itself.

Method: Propose training-free interface that injects prompt-conditioned priors from lightweight auxiliary model into diffusion initialization via: 1) discrete token injection, 2) representation-level embedding interpolation. Also introduce confidence-based remasking mechanism as prior skepticism to handle imperfect priors and prevent early over-commitment in unmask-only decoding.

Result: Preliminary evidence on GSM8K shows context-aware initialization can substantially reduce denoising iterations (~35% fewer function evaluations). However, naive warm-starting can degrade final accuracy relative to strong diffusion baselines, revealing a key challenge.

Conclusion: The findings motivate a research agenda around calibration, revision mechanisms, and representation alignment for reliable warm-started diffusion decoding. Context-aware initialization shows promise for accelerating DLLMs but requires addressing accuracy degradation issues.
...
Result: Evaluated 8 state-of-the-art language models on 1,103 scripts (8,824 evaluations total) with rigorous statistical significance testing (252 pairwise comparisons, 65.9% significant) and human validation (188 scripts, substantial agreement on 3/5 dimensions). Ablation studies confirmed all six dimensions capture independent quality aspects (mean |r| = 0.020).
...
TL;DR: ROG framework combines KG neighborhood retrieval with LLM reasoning to answer complex FOL queries, outperforming embedding-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing embedding-based methods struggle with complex FOL queries on incomplete KGs due to compositional complexity and difficulty generalizing to multiple operators, deep reasoning chains, and heterogeneous schemas.

Method: ROG uses ensemble approach: decomposes complex FOL queries into simpler sub-queries, retrieves query-relevant subgraphs as context, and performs step-by-step logical inference using LLMs with chain-of-thought reasoning.

Result: ROG consistently outperforms embedding-based baselines on standard KG reasoning benchmarks in terms of mean reciprocal rank (MRR), with particularly strong gains on high-complexity query types.

Conclusion: Integrating structured KG retrieval with LLM-driven logical reasoning provides a robust and effective alternative for complex KG reasoning tasks, avoiding task-specific embedding optimization.
...
TL;DR: The paper proposes shifting from "Large Language Models" to "Large Discourse Models" and then to "Artificial Discursive Agents," offering a theoretical framework for understanding AI discourse systems and advocating for public governance approaches.


<details>
  <summary>Details</summary>
Motivation: To move beyond the simplistic "fascination/fear" dichotomy surrounding AI language models by providing a more nuanced theoretical framework that situates these systems within human discourse and social contexts, enabling better governance and regulation.

Method: Proposes an epistemological shift with new conceptual categories (LDM, ADA) and develops an ontological triad framework distinguishing three regulatory instances: phenomenal world apprehension, embodied cognition structuring, and socio-historical linguistic sedimentation.

Result: A theoretical framework that repositions generative models as discourse systems operating on human experience reified through learning corpora, providing a foundation for understanding their place in social space.

Conclusion: The paper concludes by advocating for public trials, procedures, and co-regulation involving State, industry, civil society, and academia to make artificial discursive agents' place, uses, and limits decipherable in contemporary society.
...
TL;DR: SAP is a novel attention head pruning method that uses syntactic structure and attention patterns to guide pruning, achieving state-of-the-art performance while improving interpretability.


<details>
  <summary>Details</summary>
Motivation: Current attention head pruning methods rely mainly on mathematical analysis of weights and activations, lacking consideration of linguistic features. There's a need for methods that incorporate syntactic structure to better guide pruning and improve model interpretability.

Method: Syntactic Attention Pruning (SAP) incorporates both syntactic structure and attention patterns of sentences to guide pruning. It uses Candidate Filtering (CF) to prioritize heads based on their contribution to model performance, mitigating degradation during pruning.

Result: SAP effectively preserves critical heads with high density of strong attention values, outperforming existing head pruning strategies in retrain-free settings. It achieves performance comparable to state-of-the-art methods while enhancing interpretability.

Conclusion: SAP represents a promising new direction in model compression research, offering high flexibility for pruning across all transformer-based language models by incorporating linguistic features into the pruning process.
...
TL;DR: AWPO is a new RL framework that integrates explicit reasoning rewards with outcome rewards using adaptive gating and weighting mechanisms to enhance tool-use LLMs, achieving SOTA performance with high parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods for training tool-use LLMs focus on outcome rewards but overlook explicit reasoning rewards, and naively combining reasoning and outcome rewards can lead to suboptimal performance or conflicts with primary objectives.
...
Result: AWPO achieves state-of-the-art performance across standard tool-use benchmarks, significantly outperforming strong baselines and closed-source models. A 4B model surpasses Grok-4 by 16.0% in multi-turn accuracy while maintaining generalization on MMLU-Pro.
...
TL;DR: QuCo-RAG introduces a corpus-grounded verification method for dynamic RAG that uses objective statistics from pre-training data instead of unreliable model-internal signals to detect hallucination risks, achieving significant performance gains across multiple LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic RAG methods rely on model-internal signals like logits and entropy, which are fundamentally unreliable because LLMs are typically ill-calibrated and often exhibit high confidence in erroneous outputs. This creates a need for more objective, corpus-grounded verification methods.

Method: QuCo-RAG uses objective statistics computed from pre-training data instead of subjective confidence. It has two stages: (1) before generation, identifying low-frequency entities indicating long-tail knowledge gaps; (2) during generation, verifying entity co-occurrence in the pre-training corpus (zero co-occurrence signals hallucination risk). Both stages leverage Infini-gram for millisecond-latency queries over 4 trillion tokens, triggering retrieval when uncertainty is high.

Result: Experiments on multi-hop QA benchmarks show QuCo-RAG achieves EM gains of 5-12 points over state-of-the-art baselines with OLMo-2 models, and transfers effectively to models with undisclosed pre-training data (Llama, Qwen, GPT), improving EM by up to 14 points. Domain generalization on biomedical QA further validates robustness.

Conclusion: Corpus-grounded verification establishes a principled, practically model-agnostic paradigm for dynamic RAG that significantly outperforms existing methods by using objective statistics from pre-training data rather than unreliable model-internal signals.
...
TL;DR: Study evaluates 4 state-of-the-art ASR models for Italian video subtitling, finding they can't fully replace human subtitlers but can significantly boost productivity in human-in-the-loop workflows.


<details>
  <summary>Details</summary>
Motivation: While modern ASR systems perform well on standard benchmarks, their real-world performance for non-English content like Italian long-form videos remains unexplored. The media industry needs practical solutions for professional subtitling workflows.

Method: Case study evaluating four SOTA ASR models (Whisper Large v2, AssemblyAI Universal, Parakeet TDT v3 0.6b, and WhisperX) on a 50-hour dataset of Italian TV programs. Benchmarking against professional human subtitlers and designing a production-grade cloud infrastructure for human-in-the-loop workflow.

Result: Current ASR models cannot meet media industry accuracy requirements for full autonomy, but they serve as highly effective tools for enhancing human productivity when integrated into human-in-the-loop workflows.

Conclusion: Human-in-the-loop approach is crucial for professional subtitling. The paper presents a production-grade cloud-based infrastructure designed to support this workflow, balancing automation with human expertise for Italian media content.
...
TL;DR: JEPA-Reasoner: A JEPA model enhanced with generative ability that reasons in latent space, decoupled from token generation to reduce compounding error.


<details>
  <summary>Details</summary>
Motivation: JEPA lacks generative abilities, while Transformer-based latent reasoning models still suffer from token-by-token generation issues like compounding error and reliance on context for reasoning insights.

Method: Propose JEPA-Reasoner: a JEPA model with generative ability that reasons in latent space, augmented with a separate action-taker model (Talker) to produce human-readable sentences, decoupling latent reasoning from token generation.
...
Conclusion: Decoupling latent space reasoning from token generation addresses limitations of both JEPA and Transformer models, potentially enabling more robust reasoning and generation capabilities.
...
TL;DR: CycleChart is a consistency-based learning framework that unifies chart understanding and generation tasks through a shared schema-centric interface, enabling bidirectional learning between chart creation and interpretation.


<details>
  <summary>Details</summary>
Motivation: Current chart-specific tasks are studied in isolation, preventing models from learning shared semantics that link chart generation and interpretation. This fragmentation limits cross-task generalization and prevents development of more general chart understanding models.

Method: CycleChart uses a schema-centric formulation as common interface across tasks, creates consistent multi-task dataset with aligned annotations, and introduces a generate-parse consistency objective where models generate charts from tables/queries then learn to recover schema/data from generated charts.

Result: CycleChart achieves strong results on chart generation, chart parsing, and chart question answering, demonstrating improved cross-task generalization and marking progress toward more general chart understanding models.

Conclusion: The framework successfully unifies chart understanding and generation through consistency-based learning, enabling bidirectional semantic alignment and improved performance across multiple chart-related tasks.
...
TL;DR: LLMs show significant bias against stigmatized groups, especially those rated as highly perilous, with guardrail models providing limited mitigation.


<details>
  <summary>Details</summary>
Motivation: While LLM bias towards protected groups is studied, bias against non-protected stigmatized identities and the social features of stigmas associated with bias remain under-explored.

Method: Used SocialStigmaQA benchmark with 93 stigmatized groups across 37 social scenarios, tested on three LLMs (Granite 3.0-8B, Llama-3.1-8B, Mistral-7B), measured bias using six social stigma features from psychology, and evaluated guardrail models for mitigation.

Result: Highly perilous stigmas (gang member, HIV) had 60% biased outputs, while sociodemographic stigmas had only 11% bias. Guardrail models reduced bias by 10.4%, 1.4%, and 7.8% respectively, but failed to recognize bias intent and didn't change underlying feature effects.

Conclusion: LLMs exhibit significant bias against stigmatized groups, particularly perilous ones, with current guardrail models providing inadequate mitigation. Future work should improve guardrail models for better bias detection and reduction in applications involving stigmatized groups.
...
TL;DR: ChemATP decouples chemical knowledge from LLM reasoning by creating an atom-level textual knowledge base, allowing frozen LLMs to retrieve and reason with explicit chemical priors without compromising general intelligence.


<details>
  <summary>Details</summary>
Motivation: LLMs lack explicit chemical priors in standard string representations, creating a dilemma: training-based methods inject priors into parameters but hinder knowledge updates and general reasoning, while training-free methods fail to provide fine-grained atom-level priors needed for precise chemical reasoning.

Method: ChemATP framework constructs the first atom-level textual knowledge base, enabling frozen LLMs to explicitly retrieve and reason over this information dynamically, decoupling chemical knowledge from the reasoning engine while maintaining interpretability and adaptability.

Result: ChemATP significantly outperforms training-free baselines and rivals state-of-the-art training-based models, demonstrating that explicit prior injection is a competitive alternative to implicit parameter updates.

Conclusion: Explicit prior injection through knowledge base retrieval is a viable approach that preserves LLMs' general reasoning capabilities while providing necessary chemical priors, offering better interpretability and adaptability than parameter-based methods.
...
TL;DR: Novel prompt optimization pipeline combining RAG, few-shot, CoT, and Auto-CoT for frame detection in logistics texts, achieving up to 15% accuracy improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: Need to adapt LLMs to complex reasoning and labeling tasks in logistics without extensive fine-tuning, requiring effective prompt engineering for real-world inference accuracy and labeling efficiency.

Method: LLM-based prompt optimizer agent that iteratively refines prompts using retrieved examples, performance feedback, and self-evaluation, combining RAG, few-shot prompting, CoT reasoning, and Auto-CoT synthesis.

Result: Optimized prompts (especially Auto-CoT and RAG enhanced) improve real-world inference accuracy by up to 15% compared to baseline zero-shot or static prompts, with consistent improvements across multiple LLMs (GPT-4o, Qwen 2.5, LLaMA 3.1).

Conclusion: Structured prompt optimization is a viable alternative to full fine-tuning, offering scalable solutions for deploying LLMs in domain-specific NLP applications like logistics.
...
TL;DR: CienaLLM is a modular framework using open-weight LLMs for zero-shot information extraction from news articles about climate hazards, with configurable prompts/schemas and systematic evaluation showing it matches supervised baselines while being adaptable to other domains.


<details>
  <summary>Details</summary>
Motivation: Need to extract structured information from heterogeneous news articles at scale to understand socio-economic impacts of climate hazards, requiring flexible and adaptable information extraction systems.

Method: CienaLLM framework based on schema-guided Generative Information Extraction using open-weight LLMs for zero-shot extraction, with configurable prompts/output schemas, multi-step pipelines, and systematic factorial study evaluating LLM family, size, precision, and prompting strategies.

Result: Response parsing eliminates format errors; larger models deliver strongest performance; quantization offers efficiency gains with modest accuracy trade-offs; prompt strategies show model-specific effects; CienaLLM matches/surpasses supervised baseline for drought impact extraction from Spanish news (higher inference cost).

Conclusion: Schema-driven, model-agnostic design enables adaptation to related extraction tasks (other hazards, sectors, languages) by editing prompts/schemas rather than retraining; framework supports reproducible use with released code/configurations/schemas.
...
TL;DR: A watermarking method for LLM-generated text using Green/Yellow/Red vocabulary partitioning to restrict sampling and enable detection via statistical analysis of Green-enrichment and Red-depletion.


<details>
  <summary>Details</summary>
Motivation: To curb misuse of LLM-generated text by embedding implicit signals through watermarking techniques that can detect AI-generated content while maintaining text quality.

Method: Partitions vocabulary at each decoding step into three sets (Green/Yellow/Red) with fixed ratios, restricts sampling to Green and Yellow sets, and uses statistical detection via Green-enrichment and Red-depletion analysis with Fisher's method for p-value aggregation.
...
Conclusion: The triple-partition watermarking scheme effectively detects LLM-generated text with high accuracy and minimal impact on text quality, providing a practical solution for curbing misuse of AI-generated content.
...
TL;DR: Kunkado is a 160-hour Bambara ASR dataset from Malian radio archives with real-world speech characteristics. Finetuning Parakeet models on this data reduces WER significantly and outperforms models trained on cleaner data.


<details>
  <summary>Details</summary>
Motivation: To create a robust ASR dataset for Bambara that captures real-world spontaneous speech characteristics (code-switching, disfluencies, noise, overlapping speakers) that practical ASR systems encounter, addressing the need for resources for predominantly oral languages.

Method: Compiled 160-hour dataset from Malian radio archives, finetuned Parakeet-based models on a 33.47-hour human-reviewed subset, applied pragmatic transcript normalization to reduce variability in number formatting, tags, and code-switching annotations.

Result: Finetuning with Kunkado reduced WER from 44.47% to 37.12% on one test set and from 36.07% to 32.33% on another. Human evaluation showed the model outperforms a comparable system trained on 98 hours of cleaner, less realistic speech.

Conclusion: Kunkado dataset enables robust ASR for Bambara by capturing real-world speech characteristics. The released data and models support development of practical ASR systems for predominantly oral languages.
...
TL;DR: CodeSimpleQA benchmark evaluates factual accuracy of code LLMs, reveals frontier models struggle with code factuality, and proposes a post-training framework that improves performance.


<details>
  <summary>Details</summary>
Motivation: Current code LLM benchmarks focus on execution correctness but overlook factual accuracy of programming knowledge, creating a gap in evaluating reliable code generation.

Method: Created CodeSimpleQA bilingual benchmark with English/Chinese question-answer pairs covering diverse programming languages and CS domains, plus CodeSimpleQA-Instruct corpus with 66M samples, and developed post-training framework combining supervised fine-tuning and reinforcement learning.

Result: Evaluation shows even frontier LLMs struggle with code factuality, while the proposed framework demonstrates substantial improvements over base models.

Conclusion: Factuality-aware alignment is critical for developing reliable code LLMs, and CodeSimpleQA addresses the gap in evaluating factual accuracy of programming knowledge.
...
TL;DR: MobileWorld is a new, more challenging benchmark for mobile AI agents that addresses limitations of AndroidWorld by including realistic scenarios, cross-app interactions, and novel task categories like agent-user interaction and MCP-augmented tasks.


<details>
  <summary>Details</summary>
Motivation: AndroidWorld has become saturated with agents achieving over 90% success rates, lacks key application categories (e-commerce, enterprise communication), and doesn't reflect realistic mobile usage with vague instructions and hybrid tool usage.

Method: Created MobileWorld with 201 tasks across 20 apps, emphasizing long-horizon tasks with cross-application interactions (62.2% multi-app tasks vs 9.5% in AndroidWorld). Introduced novel task categories: agent-user interaction and MCP-augmented tasks. Provided snapshot-based container environment with precise functional verifications including backend database inspection and task callback APIs.

Result: Significant performance drop compared to AndroidWorld: best agentic framework achieved 51.7% success rate, best end-to-end model achieved 20.9% success rate. Current models struggle significantly with user interaction and MCP calls.

Conclusion: MobileWorld provides a substantially more challenging benchmark that better reflects real-world mobile usage, revealing current model limitations and offering a roadmap toward more robust next-generation mobile intelligence.
...
TL;DR: SiamGPT-32B is a Thai-optimized LLM based on Qwen3-32B that improves instruction following and generation stability for Thai language through quality-focused fine-tuning with translated English instruction data and Thai-adapted AutoIF framework.


<details>
  <summary>Details</summary>
Motivation: Existing open-weights large language models perform poorly for Thai language, especially with complex instructions, despite strong English performance. There's a need for Thai-specific models that can handle instruction adherence, multi-turn robustness, and linguistic stability.

Method: Fine-tuned Qwen3-32B using a Quality-First strategy emphasizing curated supervision over data scale. Combined translated high-complexity English instruction data with Thai-adapted AutoIF framework for instruction and linguistic constraints. Used supervised fine-tuning only, without continual pretraining or corpus expansion.

Result: SiamGPT-32B achieves strongest overall performance among similar-scale open-weights Thai models on SEA-HELM benchmark. Shows consistent gains in instruction following, multi-turn dialogue, and natural language understanding.

Conclusion: Quality-focused fine-tuning with translated instruction data and Thai-adapted constraints can effectively improve Thai language model performance without requiring extensive pretraining or corpus expansion, addressing the specific challenges of Thai language generation.
...
TL;DR: LLM activations have strong discriminative power for cross-prompt essay scoring, and LLMs adapt evaluation perspectives to different essay traits and types.


<details>
  <summary>Details</summary>
Motivation: Previous AES research focused on LLM outputs, but intermediate layer activations may provide valuable information for handling diverse scoring criteria in cross-prompt settings.

Method: Used LLM activations to fit probes, analyzed effects of different models and input content, computed essay directions across trait dimensions under different prompts to study evaluation perspective variation.

Result: Activations show strong discriminative power for essay quality evaluation; LLMs adapt evaluation perspectives to different traits and essay types, effectively handling scoring criteria diversity.

Conclusion: LLM activations are valuable for cross-prompt AES, and LLMs demonstrate adaptive evaluation capabilities that can handle diverse scoring criteria across different essay types and traits.
...
<details>
  <summary>Details</summary>
Motivation: Current humanitarian situational reporting workflows are manual, resource-intensive, and inconsistent, creating a need for automated solutions to improve timeliness and accuracy.

Method: LLM-based framework integrating semantic text clustering, automatic question generation, retrieval-augmented answer extraction with citations, multi-level summarization, and executive summary generation with internal evaluation metrics.

Result: Evaluated across 13 humanitarian events using 1,100+ documents: generated questions achieved 84.7% relevance, 84.0% importance, 76.4% urgency; answers reached 86.3% relevance with citation precision/recall >76%; human-LLM evaluation agreement F1 >0.80.

Conclusion: LLM-based framework produces more structured, interpretable, and actionable reports than existing baselines, demonstrating generative AI can autonomously produce accurate, verifiable, and operationally useful humanitarian situation reports.
...
TL;DR: Survey paper arguing that event extraction should evolve from static extraction to a cognitive scaffold for LLM systems, providing grounding, reasoning, retrieval, and memory capabilities.


<details>
  <summary>Details</summary>
Motivation: LLMs have transformed event extraction but face deployment gaps including hallucinations, fragile temporal/causal linking, and limited knowledge management. EE should provide cognitive scaffolding for LLM-centered solutions.

Method: Comprehensive survey covering EE in text and multimodal settings, organizing tasks/taxonomy, tracing evolution from rule-based to generative frameworks, and analyzing formulations, decoding strategies, architectures, representations, datasets, and evaluation.

Result: Proposes EE as a system component that: 1) uses schemas/constraints for grounding/verification, 2) creates controlled intermediate representations for reasoning, 3) supports relation-aware retrieval with graph-based RAG, and 4) offers updatable episodic memory beyond context windows.

Conclusion: EE should evolve from static extraction to a structurally reliable, agent-ready perception and memory layer for open-world systems, addressing open challenges in the LLM era through event-centric cognitive scaffolding.
...
TL;DR: Researchers created a large-scale sentiment-annotated dataset of 45,000 YouTube comments in Algerian Arabic dialect, manually labeled with five sentiment categories and rich metadata, to address resource scarcity for Algerian dialect NLP.


<details>
  <summary>Details</summary>
Motivation: Address the scarcity of publicly available resources for Algerian dialect, particularly for sentiment analysis and dialectal Arabic NLP research.

Method: Collected 45,000 YouTube comments from over 30 Algerian press/media channels using YouTube Data API, then manually annotated each comment into five sentiment categories (very negative to very positive) with additional metadata collection.

Result: Created Algerian Dialect dataset with 45,000 sentiment-annotated comments including rich metadata (timestamps, like counts, video URLs, annotation dates), publicly available on Mendeley Data under CC BY 4.0 license.

Conclusion: The dataset fills a critical gap for Algerian dialect resources and supports research in sentiment analysis, dialectal Arabic NLP, and social media analytics.
...
TL;DR: Increasing reasoning compute (thinking budget) alone isn't optimal; self-consistency and self-reflection configurations yield better accuracy than simply scaling compute.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate how thinking budget (compute allocated to reasoning process) interacts with different configurations like self-consistency and reflection, and provide a balanced comparison framework considering both performance and computational cost.

Method: Proposed systematic investigation of thinking budget as key parameter, examining its interaction with various configurations including self-consistency, reflection, and others. Developed comparison framework evaluating both performance outcomes and computational costs.
...
Conclusion: Optimal reasoning performance requires strategic configuration choices rather than simply scaling compute; self-consistency and reflection techniques offer more efficient paths to improved accuracy than increasing thinking budget alone.
...
<details>
  <summary>Details</summary>
Motivation: To develop robust cross-lingual phonetic representations by incorporating linguistic inductive biases (articulatory features) into self-supervised speech models, enabling better generalization across languages.
...
Result: MauBERT produces more context-invariant representations than state-of-the-art multilingual SSL models, effectively adapts to unseen languages and casual speech with minimal fine-tuning (10 hours).

Conclusion: This establishes an effective approach for instilling linguistic inductive biases in self-supervised speech models, enabling robust cross-lingual phonetic representation learning.
...
TL;DR: LLMs can evaluate summaries but it's unclear what features they use; this paper identifies features aligned with human/GPT responses and shows GPTs perform better when instructed to use human-like metrics.


<details>
  <summary>Details</summary>
Motivation: While LLMs are used to automate summary assessment, it's not well understood what features they exploit for evaluation or how their scores map to established metrics. The paper aims to address this gap in understanding LLM-based summary evaluation.

Method: The authors study statistical and machine learning metrics to discover features aligned with both human and GPT responses. They also experiment with instructing GPTs to employ the same metrics used by humans for evaluation.

Result: The research identifies features that align with human and GPT evaluation responses. It demonstrates that instructing GPTs to use human-like metrics improves their judgment and increases alignment with human responses.

Conclusion: Understanding what features LLMs exploit for summary evaluation and mapping them to human-used metrics can improve LLM-based assessment, making it more aligned with human judgment.
...
TL;DR: Fine-tuned character-level LLMs outperform multilingual models for diacritic restoration in under-resourced languages like Bribri and Cook Islands Māori, with reliable performance emerging at ~10,000 words.


<details>
  <summary>Details</summary>
Motivation: Addresses text normalization needs for NLP tasks in extremely under-resourced languages (Bribri and Cook Islands Māori), responding to both community requests and broader research questions about model performance in low-resource contexts.

Method: Compares diacritic restoration algorithms for under-resourced languages, examines data requirements, contrasts results across resource conditions, and explores diacritic correction. Tests fine-tuned character-level LLMs vs. massively multilingual models and zero-shot approaches.

Result: Fine-tuned character-level LLMs perform best due to UTF-8 byte decomposition capabilities. Multilingual models perform poorly under data constraints. Reliable performance emerges at ~10,000 words. Zero-shot approaches fail completely.

Conclusion: Character-level fine-tuning is effective for diacritic restoration in under-resourced languages, with practical data requirements around 10,000 words. The approach addresses both community needs and NLP research questions about low-resource generalization.
...
TL;DR: Proposes UMR-based Chain-of-Thought prompting for zero-shot Aspect-Category Sentiment Analysis, finding model-dependent effectiveness with comparable performance for mid-sized models.


<details>
  <summary>Details</summary>
Motivation: Supervised learning for ACSA faces data scarcity and annotation costs; zero-shot LLMs offer practical alternative when annotation resources are limited.

Method: Novel Chain-of-Thought prompting using intermediate Unified Meaning Representation (UMR) to structure reasoning for ACSA task; evaluated against standard CoT baseline across three models and four datasets.
...
Conclusion: UMR-based CoT shows promise for zero-shot ACSA but requires more research to establish generalizability across different model scales; effectiveness varies by model architecture.
...
TL;DR: GenEnv is a framework that uses a co-evolutionary game between an LLM agent and a generative environment simulator to create adaptive, difficulty-aligned training tasks, improving agent performance with less data.


<details>
  <summary>Details</summary>
Motivation: Training capable LLM agents is bottlenecked by high cost and static nature of real-world interaction data, which doesn't adapt to agent development.

Method: Establishes a co-evolutionary game between agent and generative environment simulator, using α-Curriculum Reward to align task difficulty with agent's current capabilities (zone of proximal development).

Result: Improves agent performance by up to +40.3% over 7B baselines, matches/exceeds larger models' average performance, and achieves better performance than Gemini 2.5 Pro-based offline data augmentation while using 3.3× less data.

Conclusion: GenEnv provides a data-efficient pathway for scaling agent capabilities by shifting from static supervision to adaptive simulation through difficulty-aligned co-evolution.
...
TL;DR: Energy-efficient anti-UAV chip integrates frame-based and event-driven tracking for small, fast drones using adaptive mode switching and specialized hardware, achieving 98.2% accuracy with ultra-low power consumption.


<details>
  <summary>Details</summary>
Motivation: Need for energy-efficient anti-UAV systems that can reliably detect small, fast-moving drones at long ranges (50-400m) with high speeds (5-80 pixels/sec), requiring both frame-based and event-driven approaches for robust tracking.

Method: System reconstructs binary event frames using run-length encoding, generates region proposals, and adaptively switches between frame mode and event mode based on object size/velocity. Fast Object Tracking Unit uses adaptive thresholding and trajectory-based classification. Neural processing unit supports grayscale-patch and trajectory inference with custom instruction set and zero-skipping MAC architecture.

Result: Implemented in 40nm CMOS, 2mm² chip achieves 96 pJ/frame/pixel and 61 pJ/event at 0.8V. Reduces redundant neural computations by >97%. Reaches 98.2% recognition accuracy on public UAV datasets across 50-400m ranges and 5-80 pixels/sec speeds.

Conclusion: Demonstrates state-of-the-art end-to-end energy efficiency for anti-UAV systems through integrated frame/event tracking, adaptive mode switching, and specialized hardware optimization.
...
TL;DR: AI system predicts high-risk visual environments for nystagmus patients and provides real-time adaptive recommendations to manage photosensitivity.


<details>
  <summary>Details</summary>
Motivation: Nystagmus patients with photosensitivity face daily challenges from involuntary eye movements worsened by bright environments. Current solutions lack predictive personalization and only treat symptoms.

Method: NystagmusNet uses a dual-branch convolutional neural network trained on synthetic and augmented data to estimate photosensitivity risk scores based on environmental brightness and eye movement variance. Includes SHAP and GradCAM for explainability and a rule-based recommendation engine for adaptive filters.

Result: The model achieves 75% validation accuracy on synthetic data. System successfully identifies environmental risk zones and provides interpretable predictions through explainability techniques.

Conclusion: The proposed AI system shows promise for predicting high-risk visual environments and providing adaptive recommendations. Future work includes smart glasses deployment and reinforcement learning for personalization.
...
TL;DR: SuperFlow improves RL training for flow-based generative models by using variance-aware sampling for adaptive group sizes and step-level advantage computation aligned with flow dynamics, achieving faster training and better performance.
...
Method: SuperFlow adjusts group sizes with variance-aware sampling and computes step-level advantages in a way that is consistent with continuous-time flow dynamics, without requiring architectural changes to the underlying models.
...
Conclusion: SuperFlow provides an efficient RL training framework for flow-based models that addresses sampling inefficiency and credit assignment bias, leading to significant improvements in training speed and text-to-image generation performance.
...
TL;DR: The paper analyzes background bias in human action recognition models and proposes mitigation strategies, showing that incorporating segmented human input reduces bias by 3.78% and prompt tuning for VLLMs improves human-focused reasoning by 9.85%.


<details>
  <summary>Details</summary>
Motivation: Human action recognition models often rely on background cues rather than human movement and pose (background bias), which can lead to inaccurate predictions and poor generalization. The authors want to systematically analyze this bias across different model types and develop effective mitigation strategies.

Method: 1) Systematic analysis of background bias across classification models, contrastive text-image pretrained models, and Video Large Language Models (VLLMs). 2) For classification models: incorporate segmented human input to reduce bias. 3) For VLLMs: explore both manual and automated prompt tuning to steer predictions toward human-focused reasoning.

Result: 1) All analyzed models exhibit strong tendency to default to background reasoning. 2) Incorporating segmented human input effectively decreases background bias by 3.78% in classification models. 3) Prompt design (manual and automated tuning) can steer VLLM predictions toward human-focused reasoning by 9.85%.

Conclusion: Background bias is a pervasive problem across different types of human action recognition models. The proposed mitigation strategies - using segmented human input for classification models and prompt tuning for VLLMs - are effective approaches to reduce this bias and improve model focus on human movement and pose.
...
TL;DR: SCS-SupCon improves fine-grained image classification with sigmoid-based contrastive loss and style-content disentanglement, achieving SOTA results across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods struggle with subtle inter-class differences and large intra-class variations in fine-grained recognition. Supervised contrastive approaches using InfoNCE loss suffer from negative-sample dilution and lack adaptive decision boundaries, reducing discriminative power.
...
Result: Achieves state-of-the-art performance on six benchmark datasets including CUB200-2011 and Stanford Dogs. On CIFAR-100 with ResNet-50: improves top-1 accuracy by ~3.9% over SupCon and ~1.7% over CS-SupCon. On fine-grained datasets: outperforms CS-SupCon by 0.4-3.0 points. Friedman tests and Nemenyi evaluations confirm robustness.

Conclusion: SCS-SupCon effectively addresses limitations of existing contrastive learning methods through adaptive decision boundaries and style-content disentanglement, demonstrating superior performance and generalization across various datasets and architectures.
...
TL;DR: A modular diffusion-based framework for single-view indoor 3D reconstruction that splits the process into two steps: first predicting complete views of room background and occluded instances using diffusion techniques, then transforming them into 3D.
...
Conclusion: The framework effectively reconstructs both foreground instances and room background from a single image, holding promising potential for applications in interior design, real estate, and augmented reality.
...
TL;DR: This paper presents an automated tea leaf disease classification system using deep learning models with attention mechanisms and ensemble methods, achieving 85.68% accuracy on a novel dataset of 5278 images across 7 disease classes.


<details>
  <summary>Details</summary>
Motivation: Tea leaf diseases cause significant economic losses in the tea industry globally. Manual disease identification is inefficient and unreliable, creating a need for automated systems to enable early detection and minimize crop damage.

Method: Developed a novel dataset of 5278 tea leaf images across 7 disease classes. Used pretrained models (DenseNet, Inception, EfficientNet) with two attention modules to improve performance. Created an ensemble model combining these approaches and applied Explainable AI for interpretability.
...
Conclusion: The proposed automated system successfully addresses the challenge of tea leaf disease identification, offering an efficient and accurate solution that can help farmers minimize economic losses through early disease detection and intervention.
...
TL;DR: ALIGN-Parts is a novel method for semantic 3D part segmentation that aligns 3D shape decompositions with meaningful part names using a set alignment approach, combining geometric, appearance, and semantic cues to create a unified part ontology across datasets.


<details>
  <summary>Details</summary>
Motivation: Existing 3D part segmentation datasets have inconsistent part definitions across different datasets, limiting robust training. Previous methods either produce unlabeled decompositions or retrieve single parts without complete shape annotations, lacking a unified approach for semantic part naming.

Method: ALIGN-Parts formulates part naming as a direct set alignment task. It decomposes shapes into partlets (implicit 3D part representations) and matches them to part descriptions via bipartite assignment. The method combines geometric cues from 3D part fields, appearance from multi-view vision features, and semantic knowledge from language-model-generated affordance descriptions. A text-alignment loss ensures partlets share embedding space with text.

Result: The method enables zero-shot matching to arbitrary descriptions and confidence-calibrated predictions for known categories. With human verification, the authors create a unified ontology aligning PartNet, 3DCoMPaT++, and Find3D datasets, consisting of 1,794 unique 3D parts. They also introduce two novel metrics for named 3D part segmentation and demonstrate applications in downstream tasks including scalable annotation.

Conclusion: ALIGN-Parts provides an efficient, one-shot approach to 3D part segmentation and naming that supports open-vocabulary matching and creates a unified part ontology across multiple datasets, with applications in scalable annotation and other downstream tasks.
...
TL;DR: This paper compares traditional OCR-MT pipelines with Vision LLMs for end-to-end translation of handwritten Marathi legal documents, aiming to improve legal document processing in low-resource environments.


<details>
  <summary>Details</summary>
Motivation: The urgent need for scalable, accurate translation systems to digitize legal records (FIRs, charge sheets, witness statements) in India's district and high courts, especially for low-resource languages like Marathi that lack large digitized corpora and have high handwriting variability.

Method: Comparison of two approaches: 1) Traditional two-stage pipeline (OCR system extracts text from handwritten images, then machine translation model translates the text), and 2) Vision Large Language Models that unify both stages into a single end-to-end step for direct translation of handwritten text images.

Result: Evaluation on a curated dataset of handwritten Marathi legal documents, with findings offering actionable insights toward building robust, edge-deployable solutions.

Conclusion: The research aims to enable efficient legal document processing in low-resource environments and enhance access to legal information for non-native speakers and legal professionals through improved handwritten text recognition and translation systems.
...
TL;DR: NodMAISI is a CT synthesis framework that generates anatomically-consistent lung nodule images to address data scarcity in lung cancer screening, improving nodule detection and malignancy classification performance.
...
Method: A three-part framework: (1) standardized curation pipeline with organ masks and nodule annotations, (2) ControlNet-conditioned rectified-flow generator based on MAISI-v2 for anatomy-consistent synthesis, (3) lesion-aware augmentation with controlled nodule mask perturbations.

Result: Improved distributional fidelity (FID 1.18-2.99 vs 1.69-5.21), substantially increased nodule detection sensitivity (0.69 vs 0.39 on IMD-CT), and boosted malignancy classification AUC by 0.07-0.21 under data scarcity conditions.

Conclusion: NodMAISI effectively addresses data scarcity in lung cancer screening by generating realistic nodule CT images, significantly improving downstream detection and classification tasks, especially for small nodules and limited data scenarios.
...
TL;DR: Proposed Yolov5-CBi architecture with CBAM and BiFPN for small drone detection, achieving better speed-accuracy trade-off than newer Yolo versions, with distilled lightweight models for real-time edge deployment.
...
Method: Modified Yolov5 architecture (Yolov5-CBi) incorporating Convolutional Block Attention Module (CBAM) and Bidirectional Feature Pyramid Network (BiFPN) to improve small object sensitivity. Created 28K-image training dataset and 2500-image local test dataset. Evaluated four variants of CBi architecture and applied knowledge distillation (Yolov5m-CBi teacher to Yolov5n-CBi student) for edge deployment.

Result: Yolov5-CBi outperformed newer Yolo versions (v8, v12) in speed-accuracy trade-off for small object detection. Distilled model achieved mA@P0.5:0.9 of 0.6573 (6.51% improvement over teacher's 0.6171) and was 82.9% faster than baseline, making it suitable for real-time drone detection.

Conclusion: The proposed CBi architecture with CBAM and BiFPN, combined with knowledge distillation, effectively advances efficient and accurate real-time detection of small UAVs, outperforming newer Yolo versions and enabling edge deployment.
...
<details>
  <summary>Details</summary>
Motivation: Need for safe deployment of neural networks with OOD detection, and privacy-preserving facial authentication/expression recognition using radar instead of cameras.

Method: Uses FMCW radar with range-Doppler and micro range-Doppler representations. Authentication employs multi-encoder multi-decoder architecture with BP and ILED components. Expression recognition uses ResNet block to distinguish dynamic/static expressions, then specialized MobileViT networks for classification.

Result: Achieves 94.13% AUROC and 18.12% FPR95 for authentication, 94.70% average accuracy for expression recognition, outperforming state-of-the-art OOD detection methods and transformer architectures.

Conclusion: FOODER enables robust, privacy-preserving facial authentication and expression recognition using radar data, operating efficiently in real-time while outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: While MLLMs have been used for iris and face image analysis, their capabilities in fingerprint understanding remain unexplored. There's a need to systematically evaluate MLLMs on fingerprint domain tasks to understand their potential and limitations.
...
Result: The paper presents findings on MLLM performance, explainability, and insights into challenges and limitations in fingerprint understanding. Establishes FPBench as the first comprehensive benchmark for this domain.

Conclusion: FPBench paves the path for foundation models for fingerprints by providing the first comprehensive evaluation framework for MLLMs in fingerprint understanding, addressing a previously unexplored domain in multimodal AI.
...
TL;DR: Region-level uncertainty-gated retrieval improves semantic segmentation accuracy and calibration under domain shift, achieving 11.3% mIoU increase with 87.5% reduction in retrieval cost.
...
TL;DR: SERA-H is a deep learning model that generates high-resolution (2.5m) canopy height maps from freely available Sentinel-1/2 satellite imagery (10m) by combining super-resolution with temporal attention encoding, achieving performance comparable to commercial high-resolution imagery.


<details>
  <summary>Details</summary>
Motivation: There's a need for high-resolution canopy height mapping for forest management and biodiversity monitoring, but existing deep learning approaches face trade-offs between data accessibility and spatial resolution. Freely available satellite imagery has lower resolution than commercial alternatives.

Method: SERA-H combines a super-resolution module (EDSR) with temporal attention encoding (UTAE) in an end-to-end model. It's trained using high-density LiDAR data (ALS) as supervision to generate 2.5m resolution height maps from Sentinel-1 and Sentinel-2 time series data (10m resolution).

Result: On an open-source benchmark dataset in France, SERA-H achieved MAE of 2.6m and R² of 0.82, outperforming standard Sentinel-1/2 baselines and achieving comparable or better performance than methods using commercial high-resolution imagery (SPOT-6/7, PlanetScope, Maxar).
...
<details>
  <summary>Details</summary>
Motivation: Prior depth estimation methods for endoscopic videos use batched inputs and lack real-time processing, temporal consistency, and sharp anatomical boundaries needed for downstream surgical applications like robotic surgery automation.

Method: Three main components: (1) single-frame depth network with endoscopy-specific transformation, (2) multi-level Mamba temporal modules for inter-frame information propagation, and (3) hierarchical design with comprehensive multi-scale supervision using complementary loss terms for boundary sharpness and geometric consistency.

Result: Substantially improves performance over state-of-the-art monocular depth estimation methods on two public colonoscopy datasets, producing depth maps with sharp, anatomically aligned boundaries and real-time throughput.

Conclusion: EndoStreamDepth provides an effective framework for endoscopic depth estimation with accurate, temporally consistent predictions and sharp boundaries essential for surgical automation applications, with code publicly available.
...
TL;DR: A novel 3D patch-based diffusion model that learns 3D priors from limited data for efficient high-resolution 3D CT reconstruction, outperforming existing methods in both performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: Direct training of diffusion models on 3D data is computationally expensive and requires large datasets. Existing approaches using 2D diffusion priors fail to fully leverage generative capacity for 3D inverse problems like CT reconstruction.
...
Result: Outperforms state-of-the-art methods in 3D CT reconstruction across multiple datasets, achieving high-resolution reconstruction of 512×512×256 volumes in ~20 minutes with both superior performance and efficiency.

Conclusion: The method enables scalable high-quality 3D generation and provides an efficient, accurate solution to high-resolution 3D inverse problems, overcoming computational limitations of traditional 3D diffusion models.
...
TL;DR: AtlasSegFM is an atlas-guided framework that customizes foundation models for medical image segmentation using just one annotated example, improving performance especially for small structures.


<details>
  <summary>Details</summary>
Motivation: Current interactive foundation models for medical image segmentation still require precise prompts and underperform on underrepresented data in training. There's a need for lightweight, deployable solutions that can adapt to specific clinical contexts with minimal annotation effort.
...
Conclusion: AtlasSegFM provides a lightweight, deployable solution for one-shot customization of foundation models in real-world clinical workflows, bridging the gap between general foundation models and specific clinical needs.
...
TL;DR: MACE-Dance is a music-driven dance video generation framework using cascaded Mixture-of-Experts that achieves SOTA performance by separating motion generation from appearance synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods from related domains (music-driven 3D dance generation, pose-driven image animation, audio-driven talking-head) cannot be directly adapted to music-driven dance video generation, and current approaches struggle to jointly achieve high-quality visual appearance and realistic human motion.

Method: A cascaded Mixture-of-Experts framework with two components: Motion Expert (music-to-3D motion generation using diffusion model with BiMamba-Transformer hybrid architecture and Guidance-Free Training) and Appearance Expert (motion- and reference-conditioned video synthesis with decoupled kinematic-aesthetic fine-tuning).

Result: Achieves state-of-the-art performance in both 3D dance generation (Motion Expert) and pose-driven image animation (Appearance Expert), with overall SOTA performance on the curated benchmark using a new motion-appearance evaluation protocol.
...
TL;DR: Flow matching performance depends on source distribution design; Gaussian distributions work well due to omnidirectional coverage, but norm alignment and directional pruning can improve existing models without retraining.


<details>
  <summary>Details</summary>
Motivation: While Gaussian distributions are commonly used as source distributions in flow matching, the potential for better alternatives in high-dimensional data generation remains largely unexplored. The paper aims to understand flow matching learning dynamics and improve source distribution design.

Method: Proposes a novel 2D simulation to analyze flow matching learning dynamics, derives key insights about flow matching behavior, and introduces a practical framework combining norm-aligned training with directionally-pruned sampling that can be applied to existing models.

Result: Empirical evaluations demonstrate consistent improvements in both generation quality and sampling efficiency. The pruning strategy provides immediate performance gains for any flow matching model trained with a Gaussian source without requiring retraining.

Conclusion: The findings provide practical insights and guidelines for source distribution design in flow matching, and introduce a readily applicable technique for improving existing models through directionally-pruned sampling while maintaining robust omnidirectional supervision.
...
TL;DR: ALIGN proposes occlusion-robust query initialization for 3D object detection by integrating LiDAR geometry and image semantics to improve detection of occluded/crowded objects.
...
Method: Three components: (1) Occlusion-aware Center Estimation (OCE) integrates LiDAR geometry and image semantics for accurate object center estimation; (2) Adaptive Neighbor Sampling (ANS) generates object candidates from LiDAR clustering and samples aligned points around them; (3) Dynamic Query Balancing (DQB) adaptively balances queries between foreground/background regions.

Result: On nuScenes benchmark, ALIGN consistently improves multiple state-of-the-art detectors by up to +0.9 mAP and +1.2 NDS, with significant gains in challenging occlusion/dense crowd scenes.

Conclusion: ALIGN's occlusion-robust, object-aware query initialization effectively addresses limitations of existing methods, demonstrating strong performance improvements particularly in difficult scenarios with occlusions or dense crowds.
...
TL;DR: The paper proposes a novel object-centric representation method using explicit graph representations for parts, with a co-part object discovery algorithm that improves recognition of multi-part objects in occluded and out-of-distribution settings.


<details>
  <summary>Details</summary>
Motivation: Current implicit object representation approaches fail to recognize learned objects in occluded or out-of-distribution contexts because they assume object part-whole relations are implicitly encoded through indirect training objectives.

Method: Proposes a method leveraging explicit graph representations for parts and presents a co-part object discovery algorithm. Also introduces three benchmarks to evaluate robustness of object-centric methods for multi-part objects in occluded and out-of-distribution settings.

Result: Experimental results on simulated, realistic, and real-world images show marked improvements in quality of discovered objects compared to state-of-the-art methods, with accurate recognition of multi-part objects in occluded and out-of-distribution contexts.
...
TL;DR: ET-STPM: Enhanced Teacher for Student-Teacher Feature Pyramid achieves state-of-the-art anomaly detection performance using a teacher-student framework with enhanced teacher network pre-trained on ImageNet and fine-tuned on MVTech-AD.


<details>
  <summary>Details</summary>
Motivation: Anomaly detection is a challenging problem in unsupervised learning, and existing methods need improvement in performance metrics. The authors aim to enhance the teacher network in student-teacher frameworks to achieve better anomaly detection results.

Method: Proposes ET-STPM (Enhanced Teacher for Student-Teacher Feature Pyramid) framework: 1) Pre-train ResNet-18 on ImageNet, 2) Fine-tune on MVTech-AD dataset, 3) Use enhanced teacher network in student-teacher architecture for anomaly detection at both image-level and pixel-level.
...
Conclusion: The enhanced teacher network in student-teacher framework significantly improves anomaly detection performance, demonstrating the effectiveness of proper pre-training and fine-tuning strategies for unsupervised anomaly detection tasks.
...
TL;DR: Developed a quantitative framework to evaluate spatial openness in housing from 2D and 3D perspectives using Tokyo rental data, finding openness increased over time, peaked in 1990s, and correlates with higher rent but not strongly with perceived space.


<details>
  <summary>Details</summary>
Motivation: Spatial openness is important for residential quality but previous studies treat influencing factors separately. Need a comprehensive quantitative framework to understand openness from multiple dimensions and its relationship with housing attributes and market dynamics.

Method: Used 4,004 rental units in Tokyo's 23 wards. 2D openness computed via visibility graph analysis (VGA) from floor plans measuring planar visibility. 3D openness derived from interior images analyzed using Mask2Former semantic segmentation model identifying walls, ceilings, floors, and windows. Examined temporal/spatial variations and relationships with rent and housing attributes.

Result: Living room visibility increased over time with overall openness peaking in 1990s. Partial correlations found among openness, rent, and building characteristics reflecting urban redevelopment trends. 2D and 3D openness indicators not directly correlated but higher openness tended to correspond to higher rent. Impression scores from existing models only weakly related to openness, suggesting interior design/furniture more strongly shape perceived space.

Conclusion: Provides a new multidimensional data-driven framework for quantifying residential spatial openness and linking it with urban and market dynamics. Offers insights for improving residential design quality through better understanding of spatial openness relationships.
...
TL;DR: VLMs show systematic left-first bias in describing horizontally concatenated images, with ~97% preference for left content regardless of language direction or training data instructions.


<details>
  <summary>Details</summary>
Motivation: To identify and characterize systematic spatial processing biases in Vision-Language Models that remain largely unexplored, particularly biases in how VLMs prioritize content in horizontally arranged images.

Method: Conducted controlled experiments on image pairs using both open-source and closed-source VLMs, tested with neutral prompting conditions, and investigated Arabic-finetuned models to rule out language reading direction effects. Also examined training dataset annotation guidelines from PixMo and Visual Genome.

Result: VLMs consistently prioritize describing left-positioned content first in ~97% of cases across different architectures. The bias persists even in Arabic-finetuned models (right-to-left language), ruling out language reading direction as primary cause. Training dataset guidelines show no explicit left-first ordering instructions.

Conclusion: The systematic left-first spatial attention bias in VLMs reveals fundamental limitations in how current models process spatial information, suggesting architectural factors rather than explicit training instructions drive this bias.
...
TL;DR: Joint learning framework for large-scale 3D reconstruction from monocular video that couples depth estimation, camera pose refinement, and incremental NeRF representation to overcome scale ambiguity, pose drift, and scene capacity limitations.


<details>
  <summary>Details</summary>
Motivation: Traditional monocular 3D reconstruction fails in large-scale scenes due to three main issues: scale-ambiguous depth causes ghost geometry, long-horizon pose drift corrupts alignment, and single global NeRF cannot model hundreds of meters of content.

Method: Three-component joint framework: 1) ViT depth network with metric-scale supervision for globally consistent depths, 2) multi-scale feature bundle adjustment layer for drift-free pose refinement using learned pyramidal descriptors, 3) incremental local-radiance-field hierarchy with hash-grid NeRFs allocated on-the-fly when view overlap drops below threshold.

Result: Achieves Absolute Trajectory Error of 0.001-0.021m on Tanks and Temples benchmark (18x lower than BARF, 2x lower than NoPe-NeRF) with sub-pixel Relative Pose Error, enabling city-block-scale coverage on single GPU.

Conclusion: Metric-scale, drift-free 3D reconstruction and high-fidelity novel-view synthesis are achievable from single uncalibrated RGB camera through joint learning of depth, pose, and radiance fields.
...
TL;DR: SG-RIFE enhances real-time video frame interpolation by injecting semantic priors into flow-based RIFE, achieving diffusion-level quality with near real-time speed.


<details>
  <summary>Details</summary>
Motivation: Flow-based methods (like RIFE) offer high throughput but struggle with complex scenarios involving large motion and occlusion, while diffusion-based methods achieve superior perceptual quality but have prohibitive latency for real-time applications.
...
Conclusion: Semantic consistency enables flow-based methods to achieve diffusion-competitive perceptual quality in near real-time, bridging the gap between real-time performance and high perceptual quality.
...
TL;DR: SDCM network addresses hyperspectral image object detection challenges by learning spectral discrepancy and cross-modal semantic consistency to handle inter-band inconsistencies and redundancy.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral images provide high spectral resolution but face challenges in object detection due to intra- and inter-class similarity, spatial differences across bands, and interference from sensor noises and illumination. The spectral inter-band inconsistencies and redundancy need to be addressed.

Method: Proposes SDCM network with three key modules: 1) Semantic Consistency Learning (SCL) module uses inter-band contextual cues to reduce heterogeneity among bands; 2) Spectral Gated Generator (SGG) filters redundant data based on band importance; 3) Spectral Discrepancy Aware (SDA) module extracts pixel-level spectral features to enrich semantic representation.
...
Conclusion: The proposed SDCM network effectively addresses hyperspectral inter-band inconsistencies and redundancy, achieving superior object detection performance by learning spectral discrepancy and cross-modal semantic consistency.
...
TL;DR: First Ancient Plant Seed Image Classification (APS) dataset with 8,340 images from 17 seed categories, plus APSNet framework achieving 90.5% accuracy for automated archaeological seed classification.
...
Method: Created APS dataset (8,340 images, 17 seed categories from 18 Chinese sites). Designed APSNet with Size Perception and Embedding (SPE) module to extract seed size information, and Asynchronous Decoupled Decoding (ADD) architecture for channel/spatial feature decoding.

Result: APSNet achieved 90.5% accuracy, surpassing existing state-of-the-art image classification methods in both quantitative and qualitative analyses.

Conclusion: The work provides an effective tool for large-scale, systematic archaeological research by automating ancient plant seed classification, addressing the methodological gap in archaeobotany.
...
<details>
  <summary>Details</summary>
Motivation: Interleaved text-image generation enables coherent visual frames and aligned textual descriptions for tasks like style transfer, compositional synthesis, and procedural tutorials, but existing methods lack proper temporal consistency and planning capabilities.
...
Result: Loom substantially outperforms open-source baseline Anole with average gain of 2.6 points (on 5-point scale) across temporal and semantic metrics. It also shows strong improvements over unified and diffusion editing baselines on a curated 50K interleaved tutorial dataset.

Conclusion: Loom provides an effective unified framework for interleaved text-image generation with superior compositionality, temporal coherence, and text-image alignment, demonstrating strong performance across various tasks including style transfer, compositional generation, and tutorial-like procedures.
...
TL;DR: A novel protection method against VLM-based attribute inference attacks that jointly optimizes privacy suppression and utility preservation while maintaining visual consistency, with a new benchmark dataset VPI-COCO for evaluation.


<details>
  <summary>Details</summary>
Motivation: Vision-language models (VLMs) enable attribute inference attacks that threaten user privacy on social media. Existing protection methods degrade visual quality or interfere with vision functions, failing to balance privacy protection and user experience.

Method: Proposes a protection method that jointly optimizes privacy suppression and utility preservation under visual consistency constraints. Also introduces VPI-COCO benchmark dataset with 522 images and hierarchical privacy questions for evaluation.

Result: Method reduces privacy attribute recognition (PAR) below 25%, keeps non-private attribute recognition (NPAR) above 88%, maintains high visual consistency, and generalizes well to unseen and paraphrased privacy questions.

Conclusion: The proposed method effectively protects privacy while preserving user experience, demonstrating strong practical applicability for real-world VLM deployments with the help of the new VPI-COCO benchmark for fair evaluation.
...
TL;DR: Proposes a visual dark pattern detection framework using YOLOv12x for real-time detection of deceptive UI elements with 92.8% mAP@50 accuracy and 40.5 FPS performance.


<details>
  <summary>Details</summary>
Motivation: Growing concerns about dark patterns (deceptive UI designs) in online platforms require proactive, real-time detection technologies beyond current reactive regulatory approaches.

Method: Created proprietary dataset of 4,066 UI screenshots from 194 websites across 6 industrial sectors, annotated with 5 dark pattern UI components. Applied YOLOv12x object detection model with transfer learning for optimized visual dark pattern recognition.

Result: Achieved 92.8% mAP@50 detection accuracy with real-time inference speed of 40.5 FPS. Dataset made publicly available for further research.

Conclusion: Proposed framework effectively enables real-time visual dark pattern detection suitable for practical deployment in online environments, with publicly released dataset supporting future research.
...
TL;DR: UniMPR is a unified multimodal place recognition framework that can handle arbitrary combinations of camera, LiDAR, and radar inputs using a single trained model, achieving state-of-the-art performance across diverse sensor configurations and conditions.


<details>
  <summary>Details</summary>
Motivation: Current multimodal place recognition methods struggle with dynamically adapting to arbitrary modality inputs, maintaining robustness with missing/degraded modalities, and generalizing across diverse sensor configurations. There's a need for a unified framework that can handle any combination of common perceptual modalities with a single model.

Method: UniMPR unifies all inputs within a polar BEV feature space, then processes them through a multi-branch network to extract discriminative intra-modal and inter-modal features from any modality combination. It uses large-scale training from multiple datasets with adaptive label assignment for extensive pre-training.

Result: Experiments on seven datasets show UniMPR achieves state-of-the-art performance under varying sensor configurations, modality combinations, and environmental conditions. The framework demonstrates strong generalization capability and robustness.

Conclusion: UniMPR provides a unified solution for multimodal place recognition that can seamlessly adapt to any combination of camera, LiDAR, and radar inputs using a single trained model, addressing key challenges in the field while maintaining high performance across diverse scenarios.
...
TL;DR: PACGNet introduces a novel pyramidal adaptive cross-gating network for aerial object detection that addresses cross-modal noise and preserves hierarchical feature structure through symmetrical cross-gating and pyramidal feature-aware multimodal gating modules.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal object detection methods for aerial imagery rely on simple fusion strategies that are prone to cross-modal noise and disrupt the hierarchical structure of feature pyramids, impairing fine-grained detection of small objects.

Method: Proposes PACGNet with two core components: 1) Symmetrical Cross-Gating (SCG) module using bidirectional symmetrical gating to selectively absorb complementary information while suppressing noise, and 2) Pyramidal Feature-aware Multimodal Gating (PFMG) module that reconstructs feature hierarchy through progressive hierarchical gating, using detailed features from higher-resolution levels to guide fusion at lower-resolution levels.
...
Conclusion: PACGNet effectively addresses the limitations of existing multimodal fusion methods by preserving both semantic integrity and hierarchical feature structure, leading to superior performance in aerial object detection tasks.
...
TL;DR: MatE is a method that generates tileable PBR materials from a single unconstrained real-world image using depth estimation and a dual-branch diffusion model.


<details>
  <summary>Details</summary>
Motivation: Creating high-fidelity PBR materials is difficult, requiring specialized equipment and expert post-processing. The goal is to democratize material creation by enabling generation from casual real-world photos.

Method: Given an image and user mask, MatE first performs coarse rectification using estimated depth as geometric prior, then uses a dual-branch diffusion model that learns consistency from rotation-aligned and scale-aligned training data to rectify residual distortions and generate complete material maps (albedo, normal, roughness, height).
...
Conclusion: MatE enables users to create realistic PBR materials from real-world images, democratizing material creation by eliminating the need for specialized equipment and expertise.
...
TL;DR: A framework that fuses 2D material maps from diffusion models into 3D Gaussian Splatting reconstructions using projection-based and learning-based approaches for relightable, photorealistic assets.


<details>
  <summary>Details</summary>
Motivation: Manual material modeling is time-consuming in gaming/film industries. Current 3D reconstruction methods lack precise spatially varying materials for relighting, while 2D diffusion models can predict PBR properties but struggle to transfer them to 3D geometry.

Method: 1) Reconstruct scene geometry using Gaussian Splatting. 2) Generate 2D PBR material maps (albedo, roughness, metallic) from input images using diffusion models. 3) Fuse materials into 3D via either optimization with image-based loss or direct projection using Gaussian ray tracing. 4) Add neural refinement (Neural Merger) that takes ray-traced material features to enhance fine-scale accuracy and multi-view consistency.

Result: Outperforms existing techniques in quantitative metrics and perceived visual realism. Enables more accurate, relightable, and photorealistic renderings from reconstructed scenes.

Conclusion: The framework significantly improves realism and efficiency of asset creation workflows in content production pipelines by bridging the gap between 2D material prediction and 3D geometry reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for bone age assessment struggle to effectively balance global features (overall skeletal development) with local skeletal details, limiting their accuracy and clinical utility.

Method: Two-stream architecture with: 1) Global feature extraction channel using Transformer with multi-head self-attention, 2) Local feature extraction channel using RFAConv for adaptive attention maps in multiscale receptive fields. Features are concatenated and optimized with Inception-V3.
...
Conclusion: BoNet+ provides an automated, high-precision bone age assessment system that reduces clinical workload and offers more objective assessments by effectively combining global and local skeletal features.
...
<details>
  <summary>Details</summary>
Motivation: Vegetation index saturation during dense canopy stages and limited ground-truth annotations constrain accurate LAI and SPAD estimation for winter wheat. Existing methods have limited feature expressiveness, while deep learning suffers from domain gaps and high data demands.

Method: Proposes MCVI-SANet with Vegetation Index Saturation-Aware Block (VI-SABlock) for adaptive channel-spatial feature enhancement, VICReg-based semi-supervised strategy for improved generalization, and vegetation height-informed dataset partitioning.

Result: Achieves average R2 of 0.8123 (RMSE 0.4796) for LAI and 0.6846 (RMSE 2.4222) for SPAD, surpassing baselines by 8.95% and 8.17% respectively. Maintains high inference speed with only 0.10M parameters.

Conclusion: Integration of semi-supervised learning with agronomic priors provides a promising approach for enhancing remote sensing-based precision agriculture, addressing key challenges in winter wheat monitoring.
...
<details>
  <summary>Details</summary>
Motivation: Existing SSC models produce coarse voxel predictions that could benefit from refinement to improve semantic accuracy. The authors aim to create a general-purpose enhancement framework that can be seamlessly integrated into various SSC models without requiring architectural changes.

Method: ESSC-RM operates in two phases: 1) A baseline SSC network produces initial coarse voxel predictions, 2) A refinement module with two components (Prediction Noise-Aware Module and Voxel-level Local Geometry Module) refines the predictions using 3D U-Net architecture under multiscale supervision.

Result: When integrated into CGFormer, mean IoU improved from 16.87% to 17.27%; with MonoScene, mean IoU improved from 11.08% to 11.51%. Experiments on SemanticKITTI dataset show consistent improvements in semantic prediction performance.

Conclusion: ESSC-RM serves as an effective general refinement framework that can be applied to a wide range of SSC models to enhance their semantic prediction capabilities without requiring architectural modifications.
...
TL;DR: A new zero-shot diffusion method for image editing that avoids expensive backpropagation through denoiser networks while maintaining strong consistency and quality.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot diffusion methods for image editing require computationally expensive vector-Jacobian products through denoiser networks at every reverse step, causing significant memory and runtime overhead.

Method: Proposes a new likelihood surrogate that yields simple and efficient Gaussian posterior transitions, eliminating the need for backpropagation through the denoiser network.
...
Conclusion: The proposed method provides an efficient alternative to existing zero-shot diffusion approaches for image editing tasks, maintaining quality while dramatically reducing computational overhead.
...
TL;DR: RecurGS: A recurrent fusion framework that incrementally integrates discrete Gaussian scene states into a single evolving 3D representation capable of interaction and novel state synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing 3D scene representations lack support for discrete scene changes and interactive environments. Current approaches either focus on single scene updates without novel-state synthesis, or use diffusion-based methods that work on one state at a time and cannot fuse information across multiple observations.

Method: RecurGS detects object-level changes across consecutive states, aligns geometric motion using semantic correspondence and Lie-algebra based SE(3) refinement, and performs recurrent updates with replay supervision to preserve historical structures. Uses voxelized, visibility-aware fusion module to selectively incorporate new regions while keeping stable areas fixed.

Result: The framework supports object-level manipulation, synthesizes novel scene states without additional scans, maintains photorealistic fidelity across evolving environments, and demonstrates high-quality reconstructions with substantially improved update efficiency on synthetic and real-world datasets.

Conclusion: RecurGS provides a scalable step toward continuously interactive Gaussian worlds by enabling efficient long-horizon updates while mitigating catastrophic forgetting, addressing key limitations in current 3D scene representation approaches.
...
TL;DR: Fine-tuned SAM 2 model achieves improved mosaic tessera segmentation with 91.02% IoU and 95.89% recall, plus new annotated dataset for digital preservation of cultural heritage.


<details>
  <summary>Details</summary>
Motivation: Mosaics are fragile ancient cultural artifacts requiring digital preservation through accurate tessera segmentation, but existing datasets are limited and conventional segmentation methods are insufficient.
...
Result: Fine-tuned model improves IoU from 89.00% to 91.02% and Recall from 92.12% to 95.89%; achieves 3% higher F-measure than previous methods and reduces tessera count error from 0.20 to 0.02.

Conclusion: Fine-tuned SAM 2 with new annotated dataset enables accurate mosaic segmentation, paving way for real-time digital preservation of cultural heritage mosaics.
...
TL;DR: PRISm is a multimodal framework for image-to-image retrieval that uses importance prediction and graph neural networks to capture semantic relationships between objects, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional image retrieval methods often fail to capture relational and contextual nuances of scenes, lacking the ability to model semantic importance of objects and their interactions that align with human perception.

Method: Two novel components: 1) Importance Prediction Module identifies and retains critical objects and relational triplets while pruning irrelevant elements; 2) Edge-Aware Graph Neural Network encodes relational structure and integrates global visual features to produce semantically informed image embeddings.

Result: Extensive experiments on benchmark and real-world datasets demonstrate consistently superior top-ranked performance. Qualitative analyses show PRISm accurately captures key objects and interactions, producing interpretable and semantically meaningful results.
...
TL;DR: AmPLe addresses model-prompt and sample-prompt matching biases in multi-prompt learning by using debiased ensemble learning with prompt-relevant semantics extraction.
...
Method: Proposes Adaptive-Debiased Ensemble MultiPrompt Learning (AmPLe) that: 1) Uses ensemble learning to aggregate diverse predictions from different models, 2) Extracts prompt-relevant semantics from input samples using information theory-based analysis, and 3) Adaptively calculates debiased ensemble weights to mitigate both biases simultaneously.

Result: Extensive experiments on three representative tasks (generalization to novel classes, new target datasets, and unseen domain shifts) show AmPLe widely outperforms existing methods. Theoretical validation from a causal perspective further supports effectiveness.

Conclusion: AmPLe successfully addresses both model-prompt and sample-prompt matching biases in multi-prompt learning, achieving superior performance across diverse downstream tasks through adaptive debiased ensemble learning with prompt-relevant semantics extraction.
...
TL;DR: Novel integration of event-based camera with DLP projector enables high-speed RGB-D sensing at 1400 fps color detection and 4 kHz depth detection.


<details>
  <summary>Details</summary>
Motivation: Traditional monochrome event-based cameras have limitations: they can't detect static/slow-moving objects and lack color information needed for many applications. The paper aims to overcome these limitations while leveraging the high-speed advantages of event-based sensing.

Method: Combines a monochrome event-based camera with a Digital Light Processing (DLP) projector to create Active Structured Light (ASL) system. Uses dynamic projection adjustments to optimize bandwidth and enable selective color data acquisition while maintaining spatial resolution.

Result: Achieves remarkable performance: color detection speed equivalent to 1400 fps and 4 kHz pixel depth detection. Enables frameless RGB-D sensing with colorful point clouds without sacrificing spatial resolution.

Conclusion: The integration of event-based cameras with projection-based techniques successfully addresses limitations of traditional ECs, enabling high-speed RGB-D sensing that advances computer vision applications in robotics, 3D reconstruction, and other fields.
...
TL;DR: MeniMV is a new multi-view MRI dataset for grading meniscal horn tears with 3,000 exams, 6,000 co-registered sagittal/coronal images, and four-tier severity labels for anterior/posterior horns, providing a benchmark for automated knee injury diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current automated MRI analysis methods for meniscal horn tears are inadequate - they use coarse study-level labels or binary classification, lacking precise localization and severity information needed for accurate diagnosis.

Method: Created MeniMV dataset with 3,000 knee MRI exams from 750 patients across three medical centers, featuring 6,000 co-registered sagittal and coronal images. Each exam has four-tier (grade 0-3) severity labels for both anterior and posterior meniscal horns, verified by chief orthopedic physicians.

Result: MeniMV offers more than double the pathology-labeled data volume of prior datasets and uniquely captures dual-view diagnostic context. The paper benchmarks multiple CNN and Transformer models, establishing strong baselines and highlighting challenges in severity grading.

Conclusion: MeniMV provides a valuable foundation for future research in automated musculoskeletal imaging by addressing the critical need for precise grading of meniscal horn tears with comprehensive multi-view data and detailed severity annotations.
...
TL;DR: Object-centric video moment retrieval using scene graphs and relational tracklet transformers outperforms state-of-the-art methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing video moment retrieval methods rely on global frame/clip features that fail to capture fine-grained object semantics and appearance, which are crucial for object-oriented queries involving specific entities and their interactions. Temporal dynamics at the object level have been largely overlooked.

Method: Proposes an object-centric framework: 1) extracts query-relevant objects using scene graph parser, 2) generates scene graphs from video frames to represent objects and relationships, 3) constructs object-level feature sequences encoding visual/semantic information, 4) processes sequences with relational tracklet transformer to model spatio-temporal correlations among objects over time.

Result: Outperforms existing state-of-the-art methods across three benchmarks: Charades-STA, QVHighlights, and TACoS.

Conclusion: By explicitly capturing object-level state changes and modeling spatio-temporal correlations among objects, the framework enables more accurate localization of moments aligned with object-oriented queries, demonstrating the importance of object-centric reasoning for video moment retrieval.
...
TL;DR: Plasticine is the first end-to-end image-to-image translation framework with pixel-level traceability for medical images, combining intensity translation and spatial transformation in a denoising diffusion framework.


<details>
  <summary>Details</summary>
Motivation: Domain gaps from different imaging devices and populations challenge medical image analysis. Existing translation methods generate diverse synthetic data but lack spatial correspondence and traceability, which is crucial for clinical interpretability and has been largely overlooked.

Method: Plasticine combines intensity translation and spatial transformation within a denoising diffusion framework. This end-to-end approach enables interpretable intensity transitions and spatially coherent deformations while maintaining pixel-wise traceability throughout translation.

Result: The method generates synthetic images with interpretable intensity transitions and spatially coherent deformations, supporting pixel-level correspondences between original and translated images for enhanced clinical interpretability.

Conclusion: Plasticine addresses the traceability gap in medical image translation by providing the first end-to-end framework that explicitly maintains pixel-level correspondences, enhancing clinical interpretability while handling domain gaps from imaging variations.
...
TL;DR: Adaptive-VoCo enhances VoCo-LLaMA with adaptive visual compression that dynamically adjusts compression rates based on image complexity, improving efficiency-performance tradeoffs in vision-language models.


<details>
  <summary>Details</summary>
Motivation: While VoCo-LLaMA reduces computational costs by compressing visual tokens, its fixed compression rate cannot adapt to varying visual complexity levels, limiting optimal efficiency-performance balance.

Method: Augments VoCo-LLaMA with a lightweight predictor that dynamically selects optimal compression rates using statistical cues (patch token entropy, attention map variance) from the vision encoder, plus a joint loss function integrating rate regularization with complexity alignment.

Result: Consistently outperforms fixed-rate baselines across multiple multimodal tasks, demonstrating better efficiency-performance tradeoffs.
...
TL;DR: PlantDiseaseNet-RT50 is a fine-tuned ResNet50 model achieving ~98% accuracy for automated plant disease detection, addressing limitations of traditional visual inspection methods.


<details>
  <summary>Details</summary>
Motivation: Plant diseases cause 70-80% of global crop losses, but traditional detection methods rely on expert visual inspection which is time-consuming, labor-intensive, and impractical for large-scale farming operations.

Method: Fine-tuned ResNet50 architecture with strategically unfrozen layers, custom classification head with regularization (batch normalization and dropout), and dynamic learning rate scheduling using cosine decay. Trained on comprehensive dataset of distinct plant disease categories across multiple crop species.

Result: Achieves exceptional performance with approximately 98% accuracy, precision, and recall on plant disease detection tasks.
...
TL;DR: NASTaR dataset provides 3415 S-band SAR ship patches with AIS-matched labels for ship type classification, featuring 23 classes, inshore/offshore separation, and auxiliary wake data, achieving 60-87% accuracy across various classification scenarios.


<details>
  <summary>Details</summary>
Motivation: SAR enables all-weather maritime monitoring but ship type classification is challenging due to high diversity of ship types. Deep learning models require large, high-quality datasets, and the growing variety of SAR satellites with different frequencies/resolutions increases the need for more annotated data.

Method: Created NASTaR dataset with 3415 ship patches extracted from NovaSAR S-band imagery, labels matched to AIS data. Features include 23 unique ship classes, inshore/offshore separation, and auxiliary wake dataset for patches with visible ship wakes. Validated using benchmark deep learning models across multiple classification scenarios.

Result: Achieved over 60% accuracy for classifying four major ship types, over 70% for three-class scenario, more than 75% for distinguishing cargo from tanker ships, and over 87% for identifying fishing vessels. Dataset and codes are publicly available.

Conclusion: NASTaR dataset addresses the critical need for high-quality SAR ship classification datasets, demonstrating practical utility across various maritime monitoring scenarios with promising classification performance using benchmark models.
...
TL;DR: GTMA framework enables VLMs to handle OOD concepts by dynamically optimizing continuous pseudo-word embeddings at inference time, improving zero-shot OOD accuracy by 15-20%.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with OOD concepts due to modal asymmetry - visual encoders can extract features from unseen images but text encoders are limited by fixed vocabularies, causing cross-modal alignment collapse and degraded zero-shot performance.

Method: Guided Target-Matching Adaptation (GTMA) framework constructs continuous pseudo-word embeddings at inference time that align with OOD images' visual anchors. Uses adaptive gradient-based representation policy optimization with semantic regularization to preserve plausibility and compatibility with prior knowledge.
...
Conclusion: Dynamic representation optimization through GTMA effectively overcomes vocabulary limitations in VLMs, enabling better handling of OOD concepts without compromising in-distribution performance.
...
TL;DR: A robust AI-generated image detection framework using multiple uncertainty measures (Fisher Information, Monte Carlo Dropout entropy, and Gaussian Process predictive variance) with Particle Swarm Optimization for adaptive rejection thresholds.


<details>
  <summary>Details</summary>
Motivation: As AI-generated images become increasingly photorealistic, distinguishing them from natural images is challenging. Existing detection methods degrade under distribution shifts to unseen generators, necessitating a more robust approach.

Method: Combines three uncertainty measures: Fisher Information (parameter sensitivity), Monte Carlo Dropout entropy (predictive variability), and Gaussian Process predictive variance. Uses Particle Swarm Optimization to learn optimal weightings and adaptive rejection thresholds. Trained on Stable Diffusion images.

Result: Combined Uncertainty achieves ~70% incorrect rejection rate on unseen generators (GLIDE, VQDM, Midjourney, etc.), filtering most misclassified AI samples. Maintains high acceptance of accurate predictions for natural images. Under adversarial attacks, rejects ~61% of successful attacks (GP-based uncertainty alone achieves up to 80%).

Conclusion: Multi-source uncertainty fusion provides a resilient and adaptive solution for AI-generated image detection, maintaining robustness under distribution shifts and adversarial attacks while allowing rejected samples to support retraining.
...
TL;DR: WoundNet-Ensemble: An IoMT system using ensemble of ResNet-50, DINOv2, and Swin Transformer for automated classification of 6 wound types with 99.90% accuracy, plus longitudinal healing tracker.


<details>
  <summary>Details</summary>
Motivation: Chronic wounds like diabetic foot ulcers affect up to 1/3 of diabetics, costing US healthcare over $25B annually. Current wound assessment is subjective, leading to inconsistent classification and delayed interventions.

Method: Internet of Medical Things system with ensemble of three deep learning architectures: ResNet-50, self-supervised Vision Transformer DINOv2, and Swin Transformer. Uses weighted fusion strategy for classification of 6 wound types. Also implements longitudinal wound healing tracker for healing rates, severity scores, and clinical alerts.

Result: Achieves 99.90% ensemble accuracy on dataset of 5,175 wound images (diabetic foot ulcers, pressure ulcers, venous ulcers, thermal burns, pilonidal sinus wounds, fungating malignant tumors). Weighted fusion shows 3.7% improvement over previous state-of-the-art methods.

Conclusion: Demonstrates robust, accurate, clinically deployable tool for modernizing wound care through AI, addressing critical needs in telemedicine and remote patient monitoring. Implementation and trained models will be publicly available for reproducibility.
...
TL;DR: Bayesian framework improves multisource domain adaptation by leveraging similarity between source distributions during pretraining, achieving 17.29% accuracy improvement on human action recognition tasks.


<details>
  <summary>Details</summary>
Motivation: Prior MDA methods are ad-hoc with either weight sharing or independent training of source models, lacking principled approach to leverage similarity between source domain distributions.

Method: Hierarchical Bayesian Framework that uses similarity between different source data distributions to optimize pretraining for MDA, considering that source distributions are typically similar.

Result: 17.29% improvement in accuracy compared to state-of-the-art MDA methods on Daily-DA RGB video benchmark for human action recognition.

Conclusion: The proposed Bayesian framework provides a principled approach for MDA pretraining that significantly outperforms existing methods by effectively leveraging source distribution similarities.
...
TL;DR: MEDALIGN is a lightweight alignment distillation framework that transfers visual alignment knowledge from domain-specific CLIP to Med-LVLMs to reduce hallucinations and improve visual grounding.
...
Method: Proposes MEDALIGN framework with two distillation losses: 1) spatial-aware visual alignment loss based on visual token-level similarity structures, and 2) attention-aware distillation loss that guides attention toward diagnostically relevant regions.
...
Conclusion: MEDALIGN effectively addresses visual misalignment in Med-LVLMs through simple, lightweight alignment distillation, improving reliability and clinical applicability of medical vision-language models.
...
TL;DR: OpenView introduces the first study on out-of-view (OOV) understanding for MLLMs, creating a pipeline, dataset, and benchmark to help models reason about content beyond the visible frame.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs excel at in-view reasoning but lack the ability to understand objects, activities, and scenes beyond the visible frame of perspective views, limiting their real-world applicability.

Method: 1) OpenView pipeline for generating multi-choice VQA using panoramic imagery; 2) OpenView-Dataset from diverse real-world panoramas for supervised fine-tuning; 3) OpenView-Bench benchmark for joint measurement of choice and rationale accuracy.

Result: Multiple MLLMs boosted performance from 48.6% to 64.1% on average after OpenView fine-tuning, though still showing a large gap from human performance in OOV VQA.

Conclusion: OpenView enables significant improvement in MLLMs' out-of-view understanding capabilities through synthetic data generation and benchmark evaluation, advancing multimodal reasoning beyond frame boundaries.
...
Method: Proposed a hybrid 3D deep learning model combining 3D DenseNet121 for local feature extraction and 3D Vision Transformer for global spatial context modeling. Trained and evaluated on 1,133 MRI volumes with comparison to multiple other 3D architectures.

Result: The DenseNet121-ViT model achieved highest performance with 84.3% average accuracy on independent test set, outperforming other 3D deep learning architectures evaluated.

Conclusion: Hybrid CNN-Transformer models show strong potential as computer-aided diagnosis tools for PAS, providing robust decision support to improve diagnostic consistency, accuracy, and timeliness of MRI-based PAS diagnosis.
...
TL;DR: Vision-based algorithm using NVIDIA Jetson AGX Xavier to solve "zero-speed braking" issue in commercial vehicle AEB systems by accurately classifying vehicle motion states (static/vibration/moving) from blind spot camera video.


<details>
  <summary>Details</summary>
Motivation: Address inaccurate CAN signals causing false AEB activations ("zero-speed braking") in commercial vehicles during low-speed operation, which is a critical safety issue in commercial vehicle systems.

Method: Uses self-adaptive CLAHE-enhanced SIFT feature extraction and KNN-RANSAC matching on sequential video frames. Key innovations: 1) 5-frame sliding window for trajectory displacement statistics, 2) dual-threshold state decision matrix, 3) OBD-II driven dynamic ROI configuration.

Result: Achieved F1-score of 99.96% for static detection, 97.78% for moving state recognition, with 14.2ms processing delay. On-site deployment showed 89% reduction in false braking events, 100% emergency braking success rate, and <5% fault rate.

Conclusion: The vision-based trajectory analysis effectively solves the low-speed false activation problem in commercial vehicle AEB systems, significantly improving safety and reliability while maintaining real-time performance.
...
TL;DR: A policy optimization framework trains lightweight agents to determine efficient tool-calling sequences for complex image restoration without requiring degradation labels, using MLLM-based rewards for training.


<details>
  <summary>Details</summary>
Motivation: Existing restoration agents using vision-language models suffer from efficiency bottlenecks (reflection, rollback, iterative tool searching) and depend on degradation recognition models that require extensive annotations, limiting applicability in label-free environments.

Method: Proposes a policy optimization-based restoration framework where a lightweight agent learns to determine tool-calling sequences through sequential decision making. Uses multimodal large language models as human-aligned evaluators to provide perceptual feedback rewards for policy improvement in label-free training.
...
TL;DR: Text2Graph VPR is an explainable visual place recognition system that converts images to textual descriptions, parses them into scene graphs, and performs retrieval using hybrid graph matching with both learned embeddings and structural similarity.


<details>
  <summary>Details</summary>
Motivation: Visual Place Recognition (VPR) needs to go beyond pixel similarity for long-term deployment, requiring transparent, interpretable decisions that remain robust under lighting, weather, and seasonal changes. Current systems lack explainability and struggle with appearance shifts.

Method: System converts image sequences to textual scene descriptions, parses them into structured scene graphs (objects, attributes, relations), aggregates per-frame graphs into place representations, and performs retrieval using dual-similarity mechanism: Graph Attention Network (GAT) embeddings + Shortest-Path (SP) kernel for structural matching.

Result: Validated on Oxford RobotCar and MSLS (Amman/San Francisco) benchmarks, demonstrating robust retrieval under severe appearance shifts, with zero-shot operation using human textual queries. Shows semantic, graph-based reasoning is viable and interpretable.

Conclusion: Semantic, graph-based reasoning provides a viable and interpretable alternative for place recognition, particularly suited to safety-sensitive and resource-constrained settings, with human-readable intermediate representations that improve transparency.
...
TL;DR: PTTA is a pure text-to-animation framework that fine-tunes pretrained video models on high-quality animation data to generate better animation videos than existing baselines.


<details>
  <summary>Details</summary>
Motivation: Traditional animation production is labor-intensive and expensive. While recent video generation models work well for natural videos, they struggle with animation generation. Existing animation-focused approaches like AniSora use image-to-video models, but text-to-video animation generation remains underexplored.

Method: 1) Construct a small-scale but high-quality paired dataset of animation videos with textual descriptions. 2) Fine-tune the pretrained text-to-video model HunyuanVideo on this animation dataset to adapt it for animation-style generation.
...
Conclusion: PTTA demonstrates an effective approach for high-quality text-to-animation generation by fine-tuning pretrained video models on specialized animation data, addressing limitations of existing video generation models for animation production.
...
TL;DR: Uni-Neur2Img is a unified framework for neural signal-driven image generation and editing using parameter-efficient LoRA modules and causal attention mechanisms, evaluated on EEG datasets for generation, editing, and style transfer.


<details>
  <summary>Details</summary>
Motivation: Existing neural-driven generation research focuses too much on textual modalities as conditions, limiting exploration of visual modalities as direct conditioning signals. There's a need for more flexible, efficient frameworks that can handle neural signals directly for image generation and editing tasks.

Method: Introduces a parameter-efficient LoRA-based neural signal injection module that processes each conditioning signal independently as pluggable components. Uses causal attention mechanism for long-sequence modeling. Creates EEG-Style dataset to address research gap in visual modality conditioning.

Result: Demonstrates significant improvements in generation fidelity, editing consistency, and style transfer quality across three evaluation scenarios: EEG-driven image generation on CVPR40 dataset, neural signal-guided image editing on Loongx dataset, and EEG-driven style transfer on EEG-Style dataset.

Conclusion: Uni-Neur2Img offers a unified, efficient, and extensible solution for bridging neural signals and visual content generation, maintaining low computational overhead while providing strong scalability to additional modalities.
...
TL;DR: Proposes a novel event-based 3D Gaussian Splatting framework that decouples geometry and radiance rendering to leverage fine-grained temporal information from sparse events for improved 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer high temporal resolution but it's unclear how event-based 3D Gaussian Splatting approaches can effectively leverage the fine-grained temporal information from sparse events. There's a trade-off between accuracy and temporal resolution that needs to be addressed.

Method: Decouples rendering into two branches: 1) event-by-event geometry (depth) rendering using ray-tracing, and 2) snapshot-based radiance (intensity) rendering using the image of warped events. This approach works without prior information or COLMAP-based initialization.

Result: Achieves state-of-the-art performance on real-world datasets and competitive performance on synthetic datasets. The method is flexible in event selection number, achieves sharp reconstruction on scene edges, and has fast training time.

Conclusion: The proposed framework successfully addresses the accuracy-temporal resolution trade-off in event-based 3DGS, demonstrating the potential of leveraging sparse event data for 3D reconstruction without requiring pretrained models or COLMAP initialization.
...
TL;DR: ZSL models are vulnerable to both class-level and concept-level adversarial attacks, with traditional attacks showing spurious success in GZSL settings, requiring new attacks like CBEA to fully compromise models.


<details>
  <summary>Details</summary>
Motivation: While Zero-shot Learning promises better generalization and interpretability, its robustness under systematic input perturbations remains unclear, creating a need to empirically analyze ZSL model vulnerabilities at both class and concept levels.

Method: Conducted empirical analysis of ZSL robustness using class-level attacks (non-target class attack) and proposed Class-Bias Enhanced Attack (CBEA) for GZSL. Introduced two concept-level attacks: Class-Preserving Concept Attack (CPconA) and Non-Class-Preserving Concept Attack (NCPconA). Evaluated three typical ZSL models across various architectures from past three years.

Result: Traditional class attacks successfully disrupt predictions but show spurious success in GZSL - only effective at original best-calibrated point. CBEA completely eliminates GZSL accuracy across all calibration points. Concept-based attacks allow easy manipulation of predictions by erasing/introducing concepts. ZSL models are vulnerable to both attack types.

Conclusion: ZSL models lack adversarial robustness, with significant performance gaps between existing approaches. Findings emphasize urgent need for improved robustness in current ZSL models against both class-level and concept-level attacks.
...
<details>
  <summary>Details</summary>
Motivation: Low-light 3D reconstruction from sparse views is challenging due to exposure imbalance and degraded color fidelity. Existing methods struggle with view inconsistency and require per-scene training, lacking a generalizable solution.

Method: Integrates physically guided illumination modeling with geometry-appearance decoupling. Uses dual-branch predictor for stable geometric initialization, illumination consistency with frequency priors for controllable lighting, and appearance refinement module to separate illumination, material, and view-dependent cues. Trains with synthetic dark views via physics-based camera model.

Result: Achieves superior novel view synthesis, cross-view consistency, and better generalization to unseen low-light scenes compared with both 2D and 3D methods on public and self-collected datasets.

Conclusion: SplatBright successfully addresses low-light 3D reconstruction challenges by combining physics-based illumination modeling with geometry-appearance decoupling in a generalizable 3D Gaussian framework, outperforming existing approaches.
...
Conclusion: The proposed framework effectively addresses the challenge of noisy pseudo-matched pairs in RS image-text retrieval, achieving superior performance through gated attention mechanisms and explicit positive-negative discrimination.
...
TL;DR: SmartSight reduces hallucinations in Video-LLMs by generating multiple candidate responses, evaluating them with Temporal Attention Collapse scores, and using Visual Attention Vanishing points for efficient early termination.


<details>
  <summary>Details</summary>
Motivation: Perceptual hallucinations in Video Large Language Models pose substantial safety risks that limit real-world applicability. Existing hallucination mitigation methods often compromise the model's video understanding and reasoning capabilities.

Method: SmartSight is a training-free approach that leverages the model's introspective capabilities. It generates multiple candidate responses to uncover low-hallucinated outputs, assesses hallucination using Temporal Attention Collapse scores (measuring over-focus on trivial temporal regions), and improves efficiency via Visual Attention Vanishing points for accurate hallucination estimation and early termination.
...
TL;DR: Proposes asynchronous diffusion inference where denoiser conditioning timestep is decoupled from latent state update schedule, using a learned timestep predictor to improve image quality with fewer steps.


<details>
  <summary>Details</summary>
Motivation: Traditional text-to-image diffusion uses synchronized schedules where denoiser conditioning matches latent state timestep, which may not be optimal. The authors aim to improve inference efficiency and image quality by decoupling these schedules.

Method: Introduces asynchronous inference with a lightweight timestep prediction module (TPM) trained using Group Relative Policy Optimization (GRPO). TPM selects conditioning timestep based on current state, controlling noise level for detail/texture. Includes scaling hyperparameter for interpolation between original and de-synchronized timesteps.

Result: Evaluated on Stable Diffusion 3.5 Medium (15 steps) and Flux.1-dev (10 steps) across MS-COCO 2014 and T2I-CompBench. Optimizes composite reward averaging Image Reward, HPSv2, CLIP Score, Pick Score, showing consistent improvement.

Conclusion: Asynchronous inference with learned timestep prediction improves diffusion model performance with fewer inference steps, offering flexible control over image detail through timestep interpolation.
...
TL;DR: Brat is a multi-view representation learning framework for brain MRI that aligns 3D scans with clinical reports using document retrieval concepts and quality-diversity approaches.
...
Result: Demonstrates substantial performance improvements across multiple vision-language and vision tasks, using a brain MRI dataset 10× larger than existing ones (~80,000 3D scans with reports).

Conclusion: The brat foundation models are publicly released and show promising results for brain MRI analysis through multi-view representation learning aligned with clinical features.
...
TL;DR: Video foundation models pretrained on general tasks can be effectively fine-tuned for multi-view geometry problems like optical flow, achieving state-of-the-art results with minimal architectural changes.


<details>
  <summary>Details</summary>
Motivation: Previous approaches to multi-view geometry tasks require custom architectures and task-specific pretraining. This research explores whether general-purpose video foundation models can be adapted for geometric vision tasks with minimal modification.

Method: Fine-tune video foundation models (vision transformers) for multi-view geometry tasks by appending a simple linear decoder to the transformer backbone. Use iterative refinement to further improve performance. The approach leverages the attention mechanism's ability to learn temporal and spatial information for geometric reasoning.

Result: Achieves state-of-the-art cross-dataset generalization for optical flow: EPE of 0.69 (Sintel clean), 1.78 (Sintel final), and 3.15 (KITTI). Sets new record on online test benchmark with EPE of 0.79, 1.88, and F1 value of 3.79. Also shows strong performance on 3D depth estimation and stereo matching.

Conclusion: General-purpose video-pretrained models can be effectively transferred to multi-view geometry tasks with minimal adaptation, demonstrating that attention mechanisms learn useful temporal and spatial information for geometric reasoning. This simplifies the pipeline for geometric vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing feed-forward 3DGS methods predict pixel-aligned primitives per-view, producing excessive primitives in dense-view settings and offering no explicit control over the number of predicted Gaussians, limiting practical applications.
...
Conclusion: EcoSplat enables flexible downstream rendering tasks by providing efficiency-controllable 3D representations, making it well-suited for practical applications with computational constraints.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for image correction and rectangling rely on task-specific architectures, which limits their generalization and practical application across different tasks. There's a need for a unified approach that can handle various distortion types consistently.
...
Conclusion: UniRect provides a comprehensive, unified approach to image distortion rectification that overcomes the limitations of task-specific architectures, offering better generalization and practical applicability across diverse image correction tasks.
...
TL;DR: Deep learning on H&E slides can predict breast cancer recurrence risk with 76.2% accuracy using MIL frameworks, matching genomics-based risk stratification.


<details>
  <summary>Details</summary>
Motivation: Predicting breast cancer recurrence risk is critical but challenging. Current methods like the 21-gene Recurrence Score are expensive and time-consuming. There's a need for automated, cost-effective alternatives using routine histology slides.

Method: Developed and compared three Multiple Instance Learning (MIL) frameworks (CLAM-SB, ABMIL, ConvNeXt-MIL-XGBoost) on 210 patient H&E whole-slide images. Used UNI and CONCH pre-trained models for feature extraction. Trained models to predict 5-year recurrence risk (low/medium/high) using 21-gene Recurrence Score as ground truth, evaluated with 5-fold cross-validation.

Result: Modified CLAM-SB model performed best with mean AUC of 0.836 and 76.2% classification accuracy. All models showed feasibility of deep learning on standard histology slides for risk stratification correlated with genomics.

Conclusion: Deep learning on routine H&E slides provides automated, genomics-correlated breast cancer recurrence risk stratification, offering a promising pathway for rapid, cost-effective clinical decision support.
...
### [149] [$M^3-Verse$: A "Spot the Difference" Challenge for Large Multimodal Models](https://arxiv.org/abs/2512.18735)
...
TL;DR: M³-Verse is a benchmark for evaluating LMMs' ability to understand object state changes between paired videos of indoor scenes before and after transformations.


<details>
  <summary>Details</summary>
Motivation: Current LMMs excel at static image and single-state understanding but lack capability to comprehend dynamic object changes between distinct video observations in consistent environments, which is crucial for spatial intelligence.

Method: Created a Multi-Modal, Multi-State, Multi-Dimensional benchmark with 270 scenes and 2,932 questions across 50+ subtasks probing 4 core capabilities, using paired videos showing indoor scenes before and after state changes.

Result: Evaluation of 16 state-of-the-art LMMs revealed limitations in tracking state transitions; proposed baseline method achieved significant performance improvements in multi-state perception.

Conclusion: M³-Verse provides a challenging testbed to catalyze development of next-generation models with holistic understanding of dynamic visual world, addressing current gaps in multi-state spatial reasoning.
...
### [150] [AMLID: An Adaptive Multispectral Landmine Identification Dataset for Drone-Based Detection](https://arxiv.org/abs/2512.18738)
*James E. Gallagher,Edward J. Oughton*

Main category: cs.CV

TL;DR: AMLID is the first open-source RGB-LWIR dataset for UAS-based landmine detection, containing 12,078 labeled images of 21 landmine types across diverse environmental conditions to democratize humanitarian demining research.


<details>
  <summary>Details</summary>
Motivation: Landmines cause 26,000 annual casualties globally, but current detection methods are hazardous, inefficient, and expensive, creating a need for safer, more effective solutions.

Method: Created Adaptive Multispectral Landmine Identification Dataset (AMLID) combining RGB and LWIR imagery from UAS, covering 21 landmine types, 11 fusion levels, 4 altitudes, 2 seasons, and 3 illumination conditions.

Result: AMLID provides 12,078 labeled images with comprehensive multispectral coverage across diverse variables, enabling algorithm development without access to live ordnance or expensive infrastructure.

Conclusion: AMLID democratizes humanitarian demining research by providing an open-source benchmark dataset that facilitates development of adaptive detection algorithms for safer, more efficient landmine clearance.
...
TL;DR: MAG is a framework for long video generation that separates memory compression and frame generation to address the trade-off between catastrophic forgetting (from window attention) and prohibitive memory costs (from full history retention).


<details>
  <summary>Details</summary>
Motivation: Current frame-AR models for long video generation face a fundamental trade-off: window attention discards historical context causing catastrophic forgetting and scene inconsistency, while retaining full history is computationally prohibitive due to memory constraints.

Method: MAG decouples memory compression and frame generation into two separate tasks: (1) a memory model that compresses historical information into a compact KV cache, and (2) a generator model that synthesizes subsequent frames using this compressed representation.

Result: MAG achieves superior historical scene consistency while maintaining competitive performance on standard video generation benchmarks. The authors also introduce MAG-Bench to strictly evaluate historical memory retention.

Conclusion: The proposed MAG framework effectively addresses the memory-consistency trade-off in long video generation by separating memory management from frame synthesis, enabling better scene consistency without prohibitive computational costs.
...
### [152] [InSight-o3: Empowering Multimodal Foundation Models with Generalized Visual Search](https://arxiv.org/abs/2512.18745)
*Kaican Li,Lewei Yao,Jiannan Wu,Tiezheng Yu,Jierun Chen,Haoli Bai,Lu Hou,Lanqing Hong,Wei Zhang,Nevin L. Zhang*

Main category: cs.CV

TL;DR: O3-Bench is a new benchmark for evaluating multimodal reasoning requiring interleaved attention to visual details, and InSight-o3 is a multi-agent framework with visual reasoning and search agents that improves performance on challenging visual reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current open multimodal AI agents lack sufficient reasoning capabilities for real-world tasks involving complex visual analysis like dense charts/diagrams and map navigation, creating a gap in sophisticated visual reasoning.

Method: Introduces O3-Bench benchmark for multimodal reasoning evaluation, and proposes InSight-o3 multi-agent framework with vReasoner (visual reasoning agent) and vSearcher (visual search agent trained via RL for generalized visual search).

Result: O3-Bench is highly challenging (OpenAI o3 achieves only 40.8% accuracy). InSight-o3's vSearcher significantly improves frontier multimodal models' performance across benchmarks, marking progress toward powerful open systems.

Conclusion: The work addresses the reasoning gap in multimodal AI through a new benchmark and multi-agent framework, demonstrating concrete progress toward sophisticated "thinking with images" capabilities in open systems.
...
### [153] [IPCV: Information-Preserving Compression for MLLM Visual Encoders](https://arxiv.org/abs/2512.18747)
*Yuan Chen,Zichen Wen,Yuzhou Wu,Xuyang Liu,Shuang Chen,Junpeng Ma,Weijia Li,Conghui He,Linfeng Zhang*

Main category: cs.CV

TL;DR: IPCV is a training-free compression framework for MLLMs that aggressively prunes visual tokens inside ViT using neighbor-guided reconstruction and attention stabilization, reducing computational cost while preserving textually important visual information.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have high computational costs due to processing numerous visual tokens through ViT encoders. Existing token pruning methods are inadequate: LLM-stage pruning ignores ViT overhead, while conventional ViT pruning without language guidance risks discarding textually critical visual cues and causes feature distortions from bidirectional attention.

Method: IPCV uses Neighbor-Guided Reconstruction (NGR) to temporarily reconstruct pruned tokens so they can participate in attention with minimal overhead, then fully restores them before passing to the LLM. It also includes Attention Stabilization (AS) to approximate the K/V of pruned tokens and mitigate negative effects. The framework is training-free and can enhance previous LLM-side pruning methods.
...
Conclusion: IPCV provides an effective training-free solution for compressing MLLM visual encoders by preserving textually important visual information while significantly reducing computational overhead, addressing limitations of existing token pruning approaches.
...
### [154] [Context-Aware Network Based on Multi-scale Spatio-temporal Attention for Action Recognition in Videos](https://arxiv.org/abs/2512.18750)
*Xiaoyang Li,Wenzhu Yang,Kanglin Wang,Tiebiao Wang,Qingsong Fei*

Main category: cs.CV

TL;DR: CAN (Context-Aware Network) is a novel action recognition model that captures multi-scale spatio-temporal cues through two specialized modules: MTCM for multi-scale temporal extraction and GSCM for grouped spatial feature extraction, achieving competitive performance across five benchmark datasets.
...
Method: Proposes Context-Aware Network (CAN) with two core modules: 1) Multi-scale Temporal Cue Module (MTCM) extracts temporal cues at multiple scales to capture both fast-changing motion details and overall action flow; 2) Group Spatial Cue Module (GSCM) extracts spatial cues at different scales by grouping feature maps and applying specialized extraction methods to each group.
...
Conclusion: The results demonstrate the importance of capturing multi-scale spatio-temporal cues for robust action recognition, validating the effectiveness of the proposed CAN architecture in addressing the multi-granularity nature of actions.
...
### [155] [MaskFocus: Focusing Policy Optimization on Critical Steps for Masked Image Generation](https://arxiv.org/abs/2512.18766)
*Guohui Zhang,Hu Yu,Xiaoxiao Ma,Yaning Pan,Hang Xu,Feng Zhao*

Main category: cs.CV

TL;DR: MaskFocus: RL framework for masked generative models that focuses policy optimization on critical sampling steps using information gain and dynamic routing.
...
Method: MaskFocus identifies critical sampling steps by measuring step-level information gain (similarity between intermediate and final images), then performs focused policy optimization on these steps. Also includes dynamic routing sampling based on entropy to explore valuable masking strategies for low-entropy samples.

Result: Extensive experiments on multiple Text-to-Image benchmarks validate the effectiveness of the method.

Conclusion: MaskFocus successfully enables effective RL policy optimization for masked generative models by focusing on critical steps and using dynamic routing, overcoming previous computational challenges.
...
### [156] [In-Context Audio Control of Video Diffusion Transformers](https://arxiv.org/abs/2512.18772)
*Wenze Liu,Weicai Ye,Minghong Cai,Quande Liu,Xintao Wang,Xiangyu Yue*

Main category: cs.CV

TL;DR: ICAC is a framework that integrates audio signals into video diffusion transformers for speech-driven video generation, proposing a Masked 3D Attention mechanism to overcome training challenges of full 3D attention while achieving good lip sync and quality.


<details>
  <summary>Details</summary>
Motivation: Current video generation foundation models focus on text, images, and depth maps but neglect time-synchronous signals like audio. There's a need to explore audio integration for speech-driven video generation within unified transformer architectures.

Method: Proposes In-Context Audio Control (ICAC) framework with three audio injection mechanisms: standard cross-attention, 2D self-attention, and unified 3D self-attention. To address training challenges of 3D attention, introduces Masked 3D Attention that constrains attention patterns to enforce temporal alignment.

Result: Masked 3D Attention enables stable training and superior performance compared to other mechanisms. The approach achieves strong lip synchronization and high video quality when conditioned on audio streams and reference images.

Conclusion: ICAC successfully integrates audio control into video diffusion transformers, with Masked 3D Attention providing an effective solution to the training challenges of full 3D attention while capturing spatio-temporal audio-visual correlations for speech-driven video generation.
...
### [157] [Eff-GRot: Efficient and Generalizable Rotation Estimation with Transformers](https://arxiv.org/abs/2512.18784)
*Fanis Mathioulakis,Gorjan Radevski,Tinne Tuytelaars*

Main category: cs.CV

TL;DR: Eff-GRot is a single-forward-pass transformer method for efficient rotation estimation from RGB images without object-specific training, using rotation-aware representations from reference images.


<details>
  <summary>Details</summary>
Motivation: Need for efficient and generalizable rotation estimation that doesn't require object- or category-specific training, particularly for latency-sensitive applications.

Method: Transformer-based framework that performs latent space comparison, jointly processing rotation-aware representations from multiple reference images with known orientations alongside a query image.

Result: Achieves favorable balance between accuracy and computational efficiency, remaining simple, scalable, and fully end-to-end.

Conclusion: Eff-GRot offers a promising direction for more efficient rotation estimation, especially in latency-sensitive applications.
...
### [158] [Tempo as the Stable Cue: Hierarchical Mixture of Tempo and Beat Experts for Music to 3D Dance Generation](https://arxiv.org/abs/2512.18804)
*Guangtao Lyu,Chenghao Xu,Qi Liu,Jiexi Yan,Muli Yang,Fen Fang,Cheng Deng*

Main category: cs.CV

TL;DR: TempoMoE: A tempo-aware Mixture-of-Experts approach for 3D dance generation from music that uses hierarchical tempo-structured experts instead of genre labels to achieve better rhythm alignment.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on noisy, coarse, or unavailable genre labels which can cause rhythm misalignment or stylistic drift. Tempo is a more consistent property across datasets and genres (60-200 BPM range) that better captures musical rhythm.
...
Result: Extensive experiments show TempoMoE achieves state-of-the-art results in both dance quality and rhythm alignment compared to existing methods.

Conclusion: TempoMoE enables flexible, rhythm-aligned dance generation without manual genre labels by leveraging tempo as a more reliable and consistent musical property for rhythm perception and synchronization.
...
### [159] [FedVideoMAE: Efficient Privacy-Preserving Federated Video Moderation](https://arxiv.org/abs/2512.18809)
*Ziyuan Tao,Chuanzhi Xu,Sandaru Jayawardana,Wei Bao,Kanchana Thilakarathna,Teng Joon Lim*

Main category: cs.CV

TL;DR: On-device federated learning framework for video violence detection using VideoMAE representations, LoRA adaptation, and differential privacy, achieving 65-66% accuracy with strong privacy protection while reducing communication costs by 28.3x.


<details>
  <summary>Details</summary>
Motivation: Short-form video platforms need privacy-preserving moderation because cloud-based pipelines expose raw videos to privacy risks, high bandwidth costs, and inference latency.
...
Result: Achieves 77.25% accuracy without privacy protection and 65-66% under strong differential privacy on RWF-2000 with 40 clients, while reducing trainable parameters to 5.5M (~3.5% of backbone) and communication cost by 28.3x compared to full-model FL.

Conclusion: The proposed framework enables effective privacy-preserving video violence detection on edge devices with significant communication efficiency gains while maintaining reasonable accuracy under strong privacy constraints.
...
### [160] [Revealing Perception and Generation Dynamics in LVLMs: Mitigating Hallucinations via Validated Dominance Correction](https://arxiv.org/abs/2512.18813)
...
<details>
  <summary>Details</summary>
Motivation: Hallucinations remain a persistent challenge in Large Vision-Language Models despite their remarkable capabilities. The authors aim to systematically analyze the internal evolution of visual perception and token generation to understand the root causes of hallucinations and develop effective mitigation strategies.

Method: The authors conduct systematic analysis of LVLMs' internal processes, identifying two key patterns: 1) GATE process for visual perception (Global scan, Approach and Tighten, Explore), and 2) SAD pattern for token generation (Subdominant Accumulation to Dominant). Based on these findings, they devise the VDC (Validated Dominance Correction) strategy which detects unsupported hallucinated tokens and replaces them with validated dominant tokens.

Result: Extensive experiments across multiple models and benchmarks confirm that the proposed VDC strategy substantially mitigates hallucinations in LVLMs, improving output reliability.

Conclusion: The systematic analysis reveals fundamental patterns in LVLM behavior that explain hallucination mechanisms. The VDC strategy, built on these insights, provides an effective approach to reduce hallucinations and enhance the reliability of LVLM outputs.
...
### [161] [EchoMotion: Unified Human Video and Motion Generation via Dual-Modality Diffusion Transformer](https://arxiv.org/abs/2512.18814)
*Yuxiao Yang,Hualian Sheng,Sijia Cai,Jing Lin,Jiahao Wang,Bing Deng,Junzhe Lu,Haoqian Wang,Jieping Ye*

Main category: cs.CV

TL;DR: EchoMotion is a video generation framework that jointly models appearance and human motion using a dual-branch DiT architecture with synchronized positional encoding, improving complex human action synthesis.


<details>
  <summary>Details</summary>
Motivation: Current video generation models struggle with complex human movements due to pixel-only training objectives that prioritize appearance over kinematic principles, limiting motion quality.

Method: Extends DiT with dual-branch architecture for joint appearance-motion processing, introduces MVS-RoPE for unified 3D positional encoding, and uses two-stage training strategy. Trained on HuMoVe dataset of 80K video-motion pairs.

Result: Explicit motion representation complements appearance, significantly boosting coherence and plausibility of human-centric video generation, enabling joint and cross-modal conditional generation.

Conclusion: Joint modeling of appearance and human motion addresses limitations of pixel-only approaches, improving complex human action video generation through synchronized multimodal representation.
...
### [162] [Brain-Gen: Towards Interpreting Neural Signals for Stimulus Reconstruction Using Transformers and Latent Diffusion Models](https://arxiv.org/abs/2512.18843)
*Hasib Aslam,Muhammad Talal Faiz,Muhammad Imran Malik*

Main category: cs.CV

TL;DR: A transformer-based framework extracts spatial-temporal EEG features for visual stimulus reconstruction using Latent Diffusion Models, improving semantic interpretation of brain activity.


<details>
  <summary>Details</summary>
Motivation: Despite progress in brain activity decoding, neural representation interpretability remains limited due to EEG signal challenges: high noise, spatial diffusion, and temporal variability. The paper aims to better interpret neural mechanisms underlying thoughts.
...
Result: Quantitative evaluations on benchmark datasets show the method excels at modeling semantic structures from EEG signals: achieves up to 6.5% increase in latent space clustering accuracy and 11.8% increase in zero-shot generalization across unseen classes, while maintaining comparable Inception Score and Fréchet Inception Distance with existing baselines.

Conclusion: The work represents a significant step toward generalizable semantic interpretation of EEG signals, demonstrating improved ability to extract meaningful representations from noisy brain activity data for visual stimulus reconstruction.
...
### [163] [Application of deep learning approaches for medieval historical documents transcription](https://arxiv.org/abs/2512.18865)
*Maksym Voloshchuk,Bohdana Zarembovska,Mykola Kozlenko*

Main category: cs.CV

TL;DR: A deep learning method for extracting text from 9th-11th century Latin manuscripts, addressing the challenge of medieval document OCR where modern solutions fail.


<details>
  <summary>Details</summary>
Motivation: Modern OCR systems perform poorly on medieval Latin documents (9th-11th centuries) due to unique historical characteristics, creating a need for specialized solutions for historical document transcription.

Method: Developed a deep learning pipeline including dataset creation from medieval documents, explanatory data analysis, object detection, word recognition using classification models, and word image embedding.

Result: Comprehensive evaluation metrics reported: recall, precision, F1 score, intersection over union, confusion matrix, mean string distance, with plots included. Implementation published on GitHub.
...
### [164] [VizDefender: Unmasking Visualization Tampering through Proactive Localization and Intent Inference](https://arxiv.org/abs/2512.18853)
*Sicheng Song,Yanjie Zhang,Zixin Chen,Huamin Qu,Changbo Wang,Chenhui Li*

Main category: cs.CV

TL;DR: VizDefender: A framework for detecting and analyzing tampering in data visualizations using semi-fragile watermarks and MLLM-based intent analysis.


<details>
  <summary>Details</summary>
Motivation: Data visualization integrity is threatened by image editing techniques that enable subtle deceptive tampering, which can mislead viewers and undermine trust in visual data representations.

Method: Two-component framework: 1) Semi-fragile watermark module embeds location maps to precisely localize tampered regions while preserving visual quality; 2) Intent analysis module uses Multimodal Large Language Models (MLLMs) to interpret manipulations and infer attacker intent and misleading effects.

Result: Extensive evaluations and user studies demonstrate the effectiveness of the methods in detecting and analyzing visualization tampering.

Conclusion: VizDefender provides a comprehensive solution for protecting data visualization integrity through tampering detection and intent analysis, addressing both data manipulation and visual encoding manipulation threats.
...
### [165] [Cross-modal Counterfactual Explanations: Uncovering Decision Factors and Dataset Biases in Subjective Classification](https://arxiv.org/abs/2512.18864)
*Alina Elena Baia,Andrea Cavallaro*

Main category: cs.CV

TL;DR: DeX is a training-free interpretability framework that uses cross-modal decomposition and image-specific concepts to generate natural language counterfactuals explaining classifier decisions, particularly for subjective image privacy decisions.


<details>
  <summary>Details</summary>
Motivation: To explain classifier decisions through semantic counterfactuals, especially for challenging subjective domains like image privacy where decisions are contextual and require understanding of contributing scene elements.

Method: Decompose and Explain (DeX) framework leverages cross-modal decompositionality and image-specific concepts to create natural language counterfactuals. Uses multi-criterion selection considering image similarity for minimal perturbations and decision confidence for impactful changes.

Result: DeX generates image-grounded, sparse explanations with significant improvements over state-of-the-art. Uncovers principal contributing factors for subjective decisions and identifies underlying dataset biases for targeted fairness mitigation.

Conclusion: DeX provides a flexible, training-free framework for generating interpretable counterfactual explanations that reveal decision factors and dataset biases, enabling improved fairness in subjective decision-making domains.
...
### [166] [Watch Closely: Mitigating Object Hallucinations in Large Vision-Language Models with Disentangled Decoding](https://arxiv.org/abs/2512.19070)
*Ruiqi Ma,Yu Yan,Chunhong Zhang,Minghao Yin,XinChao Liu,Zhihong Jin,Zheng Hu*

Main category: cs.CV

TL;DR: HDD is a training-free method that reduces hallucinations in LVLMs by enhancing original images with segmentation and blank images to disentangle language priors from visual understanding.
...
Method: Hallucination Disentangled Decoding (HDD) enhances original images by segmenting them and selecting augmenting images, while using blank images to eliminate language prior hallucinations in both original and segmented images. This training-free approach reduces dependence on language priors and improves visual performance.

Result: The method effectively mitigates hallucinations in both language and visual modalities without requiring training, addressing a key limitation of current LVLM approaches.

Conclusion: HDD provides a novel training-free solution to reduce hallucinations in LVLMs by disentangling language priors from visual understanding through image enhancement and blank image utilization, improving reliability for real-world applications.
...
TL;DR: CrashChat is a multimodal LLM for automated crash video analysis that achieves state-of-the-art performance across multiple tasks including crash recognition, localization, and description.


<details>
  <summary>Details</summary>
Motivation: Automating crash video analysis is crucial for traffic safety research and autonomous driving accountability, but existing models can't handle all required tasks in a unified framework, and effective training strategies are lacking.
...
Result: Outperforms existing MLLMs and traditional vision methods, achieving near-perfect crash recognition accuracy, 176% improvement in crash localization, 40% improvement in pre-crash localization, and substantial improvements in textual metrics (BLEU: 0.18-0.41, ROUGE: 0.18-0.42).

Conclusion: CrashChat is an effective, end-to-end analytical tool for practical crash video analysis with strong performance across multiple tasks, and the implementation is publicly available.
...
TL;DR: OSCAR is a model-agnostic framework that quantifies shortcut learning and localizes shortcut features using attribution map correlations across three models, providing stable metrics and spatial maps for auditing and mitigating shortcut reliance.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks often exploit spurious shortcuts correlated with sensitive attributes, leading to biased performance. Existing methods for understanding shortcut learning are qualitative, assume human-visible cues, and are limited in domains like medical imaging where shortcuts may be subtle.

Method: OSCAR converts task attribution maps into dataset-level rank profiles and compares them across three models: balanced baseline (BA), test model (TS), and sensitive attribute predictor (SA). It computes pairwise, partial, and deviation-based correlations on these rank profiles to produce quantitative metrics and rank image regions contributing most to shortcut reliance.
...
Conclusion: OSCAR provides a lightweight, pixel-space audit that yields statistical decision rules and spatial maps, enabling users to test, localize, and mitigate shortcut reliance in deep learning models across various domains including medical imaging.
...
TL;DR: FiNDR is a reasoning-augmented LMM framework for vocabulary-free fine-grained image recognition that uses automated reasoning to discover category names without human-defined labels, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing solutions for vocabulary-free fine-grained recognition are limited by rigid vocabularies or complex pipelines with error propagation. Recent LMMs' reasoning capabilities suggest a more principled alternative.

Method: Three-step automated pipeline: 1) Reasoning-enabled LMM generates descriptive candidate labels, 2) Vision-language model filters and ranks candidates to form coherent class set, 3) Verified names instantiate lightweight multi-modal classifier for inference.

Result: State-of-the-art performance on fine-grained benchmarks with up to 18.8% relative improvement over previous approaches. Surpasses zero-shot baselines using pre-defined ground-truth names, challenging the assumption that human-curated vocabularies are optimal.
...
TL;DR: Delta-LLaVA introduces a token-efficient visual projector for MLLMs using low-rank DeltaProjection and lightweight Transformer blocks to reduce computational cost while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: High computational cost of processing dense visual tokens in MLLMs is a major bottleneck, and standard projector designs using simple MLPs scale poorly with high-resolution inputs, introducing significant redundancy.

Method: Delta-LLaVA employs a low-rank DeltaProjection to align multi-level vision features into a compact subspace, followed by lightweight Transformer blocks as specialization layers to capture global and local structure under constrained token budgets.

Result: The method achieves consistent gains across multiple benchmarks with only 144 tokens, improves inference throughput by up to 55%, and accelerates end-to-end training by nearly 4-5x in pretraining and over 1.5x in finetuning.

Conclusion: The base-then-specialize design demonstrates the importance of token formation prior to scaling interaction capacity, offering dual benefits in both efficiency and scalability for MLLMs.
...
TL;DR: LouvreSAE: A training-light, interpretable method for artistic style transfer using art-specific Sparse Autoencoders to create style profiles as steering vectors, requiring no model updates or optimization.


<details>
  <summary>Details</summary>
Motivation: Existing style transfer methods are computationally expensive (requiring fine-tuning, adapters, or prompt engineering) and often entangle style with subject matter, lacking interpretability.

Method: Leverages art-specific Sparse Autoencoder (SAE) on latent embeddings of generative image models. Trained on artistic data to learn disentangled stylistic and compositional concepts (brushwork, texture, color palette, semantic structures). Creates style profiles as compact steering vectors for direct style transfer.

Result: Achieves or surpasses existing methods on ArtBench10 style evaluations (VGG Style Loss and CLIP Score Style) while being 1.7-20x faster. Provides interpretable style transfer without model updates.

Conclusion: LouvreSAE offers an efficient, interpretable solution for artistic style transfer that avoids computational overhead of existing methods while maintaining or improving style transfer quality.
...
TL;DR: Point-VLA enhances Vision-Language-Action models by adding visual cues (like bounding boxes) to text instructions for better object referring in cluttered or OOD scenes, with automated data annotation and improved performance.
...
Method: Introduces Point-VLA, a plug-and-play policy that augments language instructions with explicit visual cues (e.g., bounding boxes) for precise object-level grounding. Also develops an automatic data annotation pipeline for efficient scaling of visually grounded datasets.

Result: Point-VLA consistently outperforms text-only instruction VLAs on diverse real-world referring tasks, particularly in cluttered or unseen-object scenarios, demonstrating robust generalization.
...
<details>
  <summary>Details</summary>
Motivation: To address symmetry issues in 3D generative models by modifying training data rather than complex architectural changes, making the approach more accessible and effective.

Method: Analyze reflectional symmetry in real and generated 3D shapes, create half-object datasets by reflecting shapes along x=0 plane, train generative models exclusively on these half-objects, then reflect generated halves to produce complete symmetric shapes.
...
Conclusion: Training on half-objects enables generative models to learn rich partial geometry distributions that produce visually plausible and geometrically symmetric complete shapes when reflected, offering a simple yet effective data-centric solution.
...
TL;DR: VOIC introduces a dual-decoder framework that explicitly separates visible-region perception from occluded-region reasoning in monocular 3D semantic scene completion, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing monocular SSC methods suffer from feature dilution and error propagation due to interference between high-confidence visible regions and low-confidence occluded regions when processing single-image inputs.

Method: Proposes VRLE (Visible Region Label Extraction) to separate supervision for visible regions, and VOIC (Visible-Occluded Interactive Completion Network) - a dual-decoder framework with visible decoder for high-fidelity priors and occlusion decoder for global scene reasoning.
...
Conclusion: Explicitly decoupling visible-region perception from occluded-region reasoning with proper supervision separation significantly improves monocular 3D semantic scene completion performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing tuning-free identity customization methods achieve high facial fidelity but overlook visual context (lighting, skin texture, environmental tone), leading to "Semantic-Visual Dissonance" where accurate facial geometry clashes with input atmosphere, creating unnatural "sticker-like" effects.
...
Conclusion: DVI effectively addresses Semantic-Visual Dissonance by disentangling and modulating identity components, achieving superior visual consistency and atmospheric fidelity in zero-shot identity customization without requiring training or parameter fine-tuning.
...
TL;DR: A novel curvature regularization method using total normal curvature from multiple directions produces sharp edges and isotropic properties, solved via PDE operator splitting without complex parameter tuning.


<details>
  <summary>Details</summary>
Motivation: To develop a curvature regularization approach that can produce solutions with sharp edges and precise isotropic properties while avoiding complex parameter tuning issues common in existing methods.

Method: Formulates total normal curvature regularization from multiple directions, reformulates as steady-state PDE system, uses operator splitting for time discretization with closed-form or efficient algorithmic solutions for subproblems.
...
Conclusion: The proposed total normal curvature regularization with PDE operator splitting provides an effective, robust approach for curvature regularization that handles sharp edges and isotropic properties without complex parameter tuning.
...
TL;DR: SASOW improves compositional zero-shot learning by enhancing state/object classifiers with self-attention and incorporating weighting during composition, outperforming previous methods on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Current object recognition systems identify objects but not their states. Compositional Zero-shot Learning (CZSL) addresses this by treating states and objects separately, but existing methods like KG-SP have limitations in accuracy and don't consider weighting of states/objects during composition.

Method: SASOW enhances KG-SP by: 1) Adding self-attention mechanisms to state and object classifiers to improve recognition accuracy, and 2) Incorporating weighting of states and objects during composition to generate more reasonable compositions.
...
Conclusion: SASOW successfully improves compositional zero-shot learning by enhancing state/object recognition accuracy through self-attention and considering weighting during composition, demonstrating superior performance on benchmark datasets.
...
TL;DR: ICP-4D is a training-free framework for 4D LiDAR panoptic segmentation that uses Iterative Closest Point algorithm and Sinkhorn-based soft matching to associate instances across time without neural network training.


<details>
  <summary>Details</summary>
Motivation: Current 4D LiDAR panoptic segmentation methods require training large neural networks on superimposed point clouds or designing complex association modules, which are computationally expensive and overlook the geometric priors inherent in raw point clouds.

Method: The framework uses Iterative Closest Point (ICP) algorithm to associate temporally consistent instances by aligning source and target point sets through estimated transformations. It incorporates Sinkhorn-based soft matching to handle noisy instance predictions by exploiting underlying instance distributions for accurate point-wise correspondences. The pipeline handles three instance types: static, dynamic, and missing instances.

Result: Extensive experiments on SemanticKITTI and panoptic nuScenes show that ICP-4D consistently outperforms state-of-the-art approaches without requiring additional training or extra point cloud inputs.

Conclusion: ICP-4D demonstrates that a simple, training-free geometric approach leveraging ICP and soft matching can achieve superior performance in 4D LiDAR panoptic segmentation while being computationally efficient and occlusion-aware.
...
TL;DR: The paper introduces Open-World Ecological Taxonomy Classification and TaxoNet, a framework and model that addresses multiple challenges in AI-guided ecological classification including class imbalance, fine-grained variations, domain shifts, and closed-set limitations.


<details>
  <summary>Details</summary>
Motivation: AI-guided ecological classification is crucial for global sustainability efforts like biodiversity monitoring and conservation, but current approaches face multiple interrelated challenges: long-tailed taxonomic distributions (class imbalance), fine-grained variations, test-time spatiotemporal domain shifts, and closed-set assumptions that can't recognize new taxa.

Method: Proposes TaxoNet, an embedding-based encoder with a dual-margin penalization loss that strengthens learning signals from rare underrepresented taxa while mitigating the dominance of overrepresented ones, directly addressing the interrelated challenges. The framework is evaluated on three ecological datasets: Google Auto-Arborist (urban trees), iNat-Plantae (Plantae observations from iNaturalist-2019), and NAFlora-Mini (curated herbarium collection).

Result: TaxoNet consistently outperforms baselines across all three ecological domains, with particularly strong performance on rare taxa. The findings also show that general-purpose multimodal foundation models remain constrained in plant-domain applications.

Conclusion: The proposed Open-World Ecological Taxonomy Classification framework and TaxoNet model establish a strong foundation for open-world plant taxonomic monitoring by effectively addressing the co-occurring challenges of class imbalance, fine-grained variations, domain shifts, and closed-set limitations in realistic ecological settings.
...
Conclusion: CETCAM provides an effective camera-controllable video generation solution that eliminates dependency on camera annotations, achieves superior performance, and shows flexibility for extending to other control modalities beyond camera control.
...
TL;DR: VLNVerse is a new large-scale, extensible benchmark for Vision-Language Navigation that addresses limitations of existing benchmarks by providing versatile, embodied, realistic simulation and evaluation to bridge the sim-to-real gap.


<details>
  <summary>Details</summary>
Motivation: Existing VLN benchmarks have several limitations: they are confined to fixed, small-scale datasets with naive physical simulation, limiting insights into sim-to-real generalization. Task fragmentation prevents unified progress, and limited data scales fail to meet modern LLM-based pretraining demands.

Method: VLNVerse introduces a scalable, full-stack embodied AI framework that unifies previously fragmented tasks into a single versatile framework with an extensible toolkit. It features embodied design with full-kinematics agents (not "ghost" agents) in realistic simulation powered by a robust physics engine.

Result: The authors leverage VLNVerse's scale and diversity to conduct comprehensive evaluation of existing methods (from classic models to MLLM-based agents) and propose a novel unified multi-task model capable of addressing all tasks within the benchmark.
...
<details>
  <summary>Details</summary>
Motivation: Face PAD needs incremental learning to handle evolving spoofing tactics and domains, but privacy regulations prevent retaining past data, requiring rehearsal-free IL. VLP models offer efficient adaptation capabilities through prompt-tuning.

Method: Proposes SVLP-IL framework with two key components: Multi-Aspect Prompting (MAP) to isolate domain dependencies and enhance distribution-shift sensitivity, and Selective Elastic Weight Consolidation (SEWC) to preserve critical weights from previous tasks while allowing new adaptations.
...
TL;DR: Finer-Personalization Rank: A new evaluation protocol for personalized generative models that uses retrieval-based ranking instead of pairwise similarity to better assess identity preservation of fine-grained details.


<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for personalized generative models focus on overall semantic similarity but fail to capture fine-grained discriminative details that are crucial for identity preservation (e.g., specific markings on a pet). There's a need for better evaluation methods that can assess whether generated images retain precise identity-specific details.

Method: Finer-Personalization Rank adopts a ranking view where each generated image is treated as a query against an identity-labeled gallery of visually similar real images. It uses retrieval metrics like mean average precision (mAP) to measure performance, with higher scores indicating better preservation of identity-specific details. The method assesses identity at multiple granularities from fine-grained categories (bird species, car models) to individual instances (re-identification).

Result: The method was evaluated across CUB, Stanford Cars, and animal Re-ID benchmarks. Finer-Personalization Rank more faithfully reflects identity retention than semantic-only metrics and reveals substantial identity drift in several popular personalization methods. It shows that current methods often fail to preserve fine-grained identity details despite achieving good semantic similarity scores.

Conclusion: The gallery-based protocol (Finer-Personalization Rank) provides a principled and practical evaluation framework for personalized generation, offering better assessment of identity preservation than existing semantic similarity metrics. It highlights the importance of evaluating fine-grained discriminative details in personalized generative models.
...
TL;DR: A Bayesian deep learning framework for automatic detection of neuronal activities in 4D calcium imaging data, using spatio-temporal information and providing uncertainty estimates.


<details>
  <summary>Details</summary>
Motivation: Manual segmentation of neuronal activities from fluorescence microscopy calcium imaging is time-consuming, labor-intensive, and lacks generalizability, creating a bottleneck for studying brain activity mapping.
...
Result: Achieved mean Dice Score of 0.81 compared to synthetic ground truth (Otsu's method) and 0.79 for test of reproducibility between first and second runs.

Conclusion: The method enables rapid, automatic detection of active neuronal activities for behavioral studies with uncertainty quantification and good generalization capability.
...
<details>
  <summary>Details</summary>
Motivation: Few-shot action recognition faces three core challenges: (1) temporal modeling difficulties due to static background interference and poor dynamic feature capture, (2) difficulty distinguishing visually similar categories, and (3) modality gap between visual-textual support prototypes and visual-only queries that complicates alignment in shared embedding space.

Method: Proposes CLIP-SPM framework with three components: (1) Hierarchical Synergistic Motion Refinement (HSMR) module aligns deep and shallow motion features to reduce static background interference, (2) Semantic Prototype Modulation (SPM) strategy generates query-relevant text prompts to bridge modality gap and integrates them with visual features, (3) Prototype-Anchor Dual Modulation (PADM) refines support prototypes and aligns query features with global semantic anchor.
...
Conclusion: CLIP-SPM effectively addresses key challenges in few-shot action recognition through integrated motion refinement, semantic modulation, and prototype-anchor alignment, demonstrating strong performance across multiple benchmarks with publicly available code.
...
TL;DR: WaTeRFlow is a watermarking framework robust to image-to-video conversion, using flow-guided synthesis, temporal consistency loss, and semantic preservation to maintain watermark detection across video frames.


<details>
  <summary>Details</summary>
Motivation: Current watermarking schemes are vulnerable when watermarked images are converted to video via I2V models, as per-frame detection weakens. With I2V advancing rapidly for content creation and simulation workflows, robust cross-modal watermark recovery becomes crucial.

Method: Three key components: (1) FUSE (Flow-guided Unified Synthesis Engine) exposes encoder-decoder to realistic distortions via instruction-driven edits and fast video diffusion proxy during training; (2) optical-flow warping with Temporal Consistency Loss stabilizes per-frame predictions; (3) semantic preservation loss maintains conditioning signal.
...
Conclusion: WaTeRFlow addresses the gap in watermark robustness for image-to-video conversion, providing a framework that maintains watermark detection across video frames despite distortions and generative edits.
...
TL;DR: DecHOI: A decoupled generative model for human-object interaction synthesis that separates trajectory planning from motion synthesis to improve realism and flexibility.


<details>
  <summary>Details</summary>
Motivation: Existing HOI synthesis methods require manual waypoint specification and combine all optimization objectives in a single network, leading to complexity, reduced flexibility, unsynchronized motions, and penetration artifacts.

Method: Proposes DecHOI with two-stage approach: 1) Trajectory generator produces human/object paths without prescribed waypoints, 2) Action generator conditions on these trajectories to synthesize detailed motions. Uses adversarial training with distal joint dynamics discriminator for contact realism, and supports dynamic scenes with moving counterparts.

Result: Outperforms prior methods on FullBodyManipulation and 3D-FUTURE benchmarks across most quantitative metrics and qualitative evaluations. Perceptual studies also prefer DecHOI results.

Conclusion: Decoupled approach to HOI synthesis improves realism, flexibility, and reduces errors like unsynchronized motion and penetration, enabling better human-object interaction generation for animation and robotics applications.
...
TL;DR: 6DAttack: A novel backdoor attack framework targeting 6DoF object pose estimation systems using 3D object triggers to manipulate continuous pose parameters while maintaining normal clean performance.


<details>
  <summary>Details</summary>
Motivation: While 6DoF object pose estimation is crucial for robotics, AR/VR, and autonomous systems, it remains vulnerable to backdoor attacks. Existing research focuses on 2D vision, but 6DoF attacks require controlling continuous parameters (translation and rotation), making 2D methods inapplicable. This creates a significant security gap that needs addressing.

Method: Proposed 6DAttack framework uses 3D object triggers to induce controlled erroneous poses. The method maintains normal behavior on clean inputs while manipulating pose parameters when triggers are present. The approach was evaluated on three state-of-the-art models (PVNet, DenseFusion, PoseDiffusion) across three datasets (LINEMOD, YCB-Video, CO3D).

Result: Achieved high attack success rates (ASRs) without compromising clean performance: up to 100% clean ADD accuracy and 100% ASR, with triggered samples reaching 97.70% ADD-P. The attack remained effective against a representative defense method, demonstrating its robustness.

Conclusion: 6DAttack reveals a serious, underexplored security threat to 6DoF pose estimation systems. The framework successfully manipulates continuous pose parameters while evading detection, highlighting the need for specialized defenses for 6DoF applications beyond traditional 2D backdoor protections.
...
TL;DR: A method for fast open-vocabulary 3D instance segmentation that generates 3D masks from RGB images using 2D open-vocabulary detectors, addressing computational overhead and rare object generalization issues.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary 3D instance segmentation methods rely heavily on SAM and CLIP, causing substantial computational overhead and slow processing that limit real-world deployment. Open-YOLO 3D reduces inference time but fails to generalize to infrequent object categories in 3D training data.

Method: Generates 3D instance masks for novel objects from RGB images guided by a 2D open-vocabulary detector. Inherits the 2D detector's ability to recognize novel objects while maintaining efficient classification, enabling fast retrieval from open-ended text queries.

Result: Enables fast and accurate retrieval of rare instances from open-ended text queries while maintaining computational efficiency compared to SAM/CLIP-based approaches.

Conclusion: The proposed method addresses both computational efficiency and generalization to rare objects in open-vocabulary 3D instance segmentation, making it suitable for real-world applications in robotics and augmented reality.
...
Method: RankInsight toolkit that (1) computes pair-wise significance maps to test statistical certainty of performance differences, (2) recomputes leaderboards with organ-appropriate metrics (e.g., replacing Dice with NSD for tubular structures), and (3) audits intersectional fairness across demographic groups.

Result: RankInsight reveals that: (1) nnU-Net family outperforms Vision-Language and MONAI submissions with high statistical certainty; (2) leaderboard rankings change when using organ-appropriate metrics (top four models reverse order when Dice is replaced by NSD); (3) more than half of MONAI-based entries show largest gender-race discrepancy on proprietary dataset.

Conclusion: RankInsight enables medical AI challenge organizers and participants to publish rankings that are statistically sound, clinically meaningful, and demographically fair. The toolkit is publicly released and applicable to past, ongoing, and future challenges.
...
TL;DR: MambaMDN: A dual-domain framework using Mamba-based modality disentanglement for accelerated multi-contrast MRI reconstruction that addresses aliasing artifacts and modality contamination.


<details>
  <summary>Details</summary>
Motivation: MRI scan times are too long, and existing accelerated methods fail to effectively use K-space prior information (causing aliasing artifacts) and suffer from contamination when fusing multi-contrast data.

Method: 1) Use fully-sampled reference K-space to complete undersampled target data, 2) Mamba-based modality disentanglement network to remove reference-specific features, 3) Iterative refinement mechanism for progressive feature purification.

Result: Extensive experiments show MambaMDN significantly outperforms existing multi-contrast MRI reconstruction methods.

Conclusion: MambaMDN effectively addresses key challenges in accelerated MRI by leveraging K-space priors and modality disentanglement, offering superior reconstruction quality for multi-contrast imaging.
...
TL;DR: GaussianImage++ improves 2D Gaussian Splatting for image representation/compression with distortion-driven densification, context-aware filters, and efficient quantization, outperforming prior methods with real-time decoding.


<details>
  <summary>Details</summary>
Motivation: Implicit neural representations (INRs) have high training time/memory costs, while existing 2D Gaussian Splatting methods require excessive primitives for high fidelity. Need efficient primitive-based approach for image representation/compression.

Method: 1) Distortion-driven densification: progressively allocates Gaussian primitives based on signal intensity. 2) Context-aware Gaussian filters: optimize primitives based on image content. 3) Attribute-separated learnable scalar quantizers with quantization-aware training for compression.
...
Conclusion: GaussianImage++ effectively exploits GS-based approaches with limited primitives, achieving superior representation/compression with practical efficiency advantages over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Vertical directions from IMUs are widely available in autonomous vehicles, mobile phones, and UAVs. Leveraging this known information can simplify relative pose estimation by reducing the number of unknowns and point correspondences needed.

Method: Two novel solvers: 1) A linear closed-form solution requiring only 4 point correspondences across three views, and 2) A minimal solution using Gröbner basis solver requiring only 3 point correspondences. Both exploit known vertical directions to reduce unknowns to two rotation angles and two translation vectors.

Result: The methods were tested on synthetic data and real-world KITTI scenes. Experimental results show superior accuracy in estimated poses compared to alternative methods, with efficient performance within RANSAC frameworks for outlier removal.

Conclusion: The proposed solvers effectively leverage known vertical directions from IMUs to simplify relative pose estimation, requiring fewer point correspondences while achieving better accuracy, making them practical for visual odometry applications in autonomous systems.
...
TL;DR: MLLMs perform poorly at zero-shot multimodal retrieval despite excelling at generation, due to text-dominated representations and homogenized embeddings that lack discriminative power.


<details>
  <summary>Details</summary>
Motivation: Despite MLLMs' success in generative tasks, they surprisingly fail at zero-shot multimodal retrieval. The paper aims to understand why MLLMs can't serve as effective retrievers and identify the underlying mechanisms causing this deficiency.

Method: Used sparse autoencoders (SAEs) to decompose MLLM output representations into interpretable semantic concepts. Analyzed the representation space to probe intrinsic behavior, examining the balance between textual and visual semantics and how modality bridging affects embeddings.

Result: Found that MLLM representations are overwhelmingly dominated by textual semantics, with visual information constituting only a small portion. The focus on bridging image-text modalities homogenizes embeddings, reducing discriminative power needed for retrieval. Specific feature components that contribute most to similarity computations actually degrade retrieval performance.

Conclusion: This is the first in-depth interpretability analysis of MLLM representations for multimodal retrieval. The findings explain MLLMs' retrieval deficiency and suggest possible directions for enhancing their multimodal retrieval capabilities by addressing representation imbalances and feature component issues.
...
TL;DR: AMap: A novel online HD map construction framework that addresses the safety flaw in temporal fusion methods by enabling "look-ahead" capabilities through future-aware knowledge distillation, improving forward region perception crucial for autonomous driving safety.


<details>
  <summary>Details</summary>
Motivation: Current temporal fusion methods for online HD map construction are "spatially backward-looking" - they mainly improve perception in traversed areas but offer minimal improvement for unseen road ahead. This creates a critical safety gap since forward perception errors directly cause hazardous driving maneuvers, while rearward errors are often tolerable.

Method: Proposes AMap with a "distill-from-future" paradigm: a teacher model with privileged access to future temporal contexts guides a lightweight student model restricted to the current frame. Uses Multi-Level BEV Distillation with spatial masking and Asymmetric Query Adaptation to transfer future-aware representations to the student's static queries.

Result: Extensive experiments on nuScenes and Argoverse 2 benchmarks show AMap significantly enhances current-frame perception, outperforms state-of-the-art temporal models in critical forward regions while maintaining the efficiency of single current frame inference.

Conclusion: AMap bridges the safety gap in online HD mapping by providing "look-ahead" capabilities through future knowledge distillation, enabling safer autonomous driving by improving forward region perception without inference-time computational cost.
...
TL;DR: OmniMoGen is a unified framework for versatile motion generation using interleaved text-motion instructions, achieving SOTA performance across multiple tasks including text-to-motion, motion editing, and compositional generation.


<details>
  <summary>Details</summary>
Motivation: While LLMs have unified diverse linguistic tasks, motion generation remains fragmented with isolated methods that lack flexibility for free-form and omni-objective generation. There's a need for a unified approach similar to how LLMs handle language tasks.

Method: Built on RVQ-VAE and transformer architecture, OmniMoGen enables end-to-end instruction-driven motion generation. The authors created X2Mo dataset (137K+ interleaved text-motion instructions) and AnyContext benchmark for evaluation.

Result: OmniMoGen achieves state-of-the-art performance on text-to-motion, motion editing, and the AnyContext benchmark. It demonstrates emerging capabilities like compositional editing, self-reflective generation, and knowledge-informed generation.

Conclusion: The framework represents a step toward next-generation intelligent motion generation by unifying diverse motion tasks within a single instruction-driven framework, similar to how LLMs unified language tasks.
...
TL;DR: The paper introduces PEDESTRIAN dataset for sidewalk obstacle detection using egocentric vision to enhance pedestrian safety.


<details>
  <summary>Details</summary>
Motivation: Sidewalks are frequently obstructed by obstacles that hinder pedestrian movement and pose safety hazards. There's a need for systems that can automatically detect such obstacles in real time using egocentric vision techniques, but this requires comprehensive datasets.

Method: Created the PEDESTRIAN dataset with 340 egocentric videos collected using mobile phone cameras, capturing pedestrian's point of view for 29 different sidewalk obstacles. Trained several state-of-the-art deep learning algorithms on this dataset.

Result: Developed a comprehensive dataset for sidewalk obstacle detection and established benchmark results by training various deep learning models on the dataset.

Conclusion: The PEDESTRIAN dataset enables training of pavement obstacle detectors to enhance pedestrian safety in urban areas and serves as a benchmark for obstacle detection and recognition tasks.
...
Conclusion: InvCoSS provides an effective privacy-preserving solution for continual self-supervised learning in medical imaging by using model inversion instead of data replay, making it suitable for real-world scenarios with data transfer restrictions.
...
TL;DR: HippMetric: A skeletal representation framework for hippocampal substructure analysis that establishes anatomically consistent correspondence across subjects and time using a deformable coordinate system aligned with hippocampal lamellar architecture.


<details>
  <summary>Details</summary>
Motivation: Current hippocampal analysis methods struggle with high inter-subject variability and complex folding patterns, lacking stable intrinsic coordinate systems for reliable cross-subject and longitudinal correspondence needed for early neurodegenerative biomarker detection.

Method: HippMetric uses Axis-Referenced Morphometric Model (ARMM) with two core modules: 1) skeletal-based coordinate system aligned with hippocampal longitudinal lamellar architecture, and 2) individualized skeletal representations generated through surface reconstruction, deformation, and geometrically constrained spoke refinement for boundary adherence and mathematical validity.

Result: Extensive experiments on two international cohorts show HippMetric achieves higher accuracy, reliability, and correspondence stability compared to existing shape models for hippocampal substructural morphometry.

Conclusion: HippMetric provides a biologically grounded framework for hippocampal analysis that overcomes anatomical variability limitations, enabling reliable inter- and intra-individual correspondence crucial for detecting subtle structural changes in neurodegenerative diseases.
...
TL;DR: Image-LoRA is a lightweight PEFT method for VLMs that applies low-rank adaptation only to visual-token attention values, reduces training FLOPs proportionally to visual-token fraction, and preserves text reasoning performance.


<details>
  <summary>Details</summary>
Motivation: To create a more efficient parameter-efficient fine-tuning method for vision-language models that reduces computational costs while maintaining performance, especially preserving text reasoning capabilities after fine-tuning.

Method: Applies low-rank adaptation only to value paths of attention layers within visual-token span, selects subset of attention heads using influence scores estimated with rank-1 Image-LoRA, and stabilizes updates with selection-size normalization.

Result: Matches or closely approaches standard LoRA accuracy on screen-centric grounding and referring benchmarks across text-heavy to image-heavy regimes, with fewer trainable parameters and lower adapter-only training FLOPs. Preserves pure-text reasoning performance on GSM8K.

Conclusion: Image-LoRA provides an efficient PEFT solution for VLMs that maintains performance while reducing computational costs and preserving text reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current perception research using streetscapes often relies on pixel features or object co-occurrence statistics, missing explicit relational information that shapes human perception. There's a need for structured representations that capture object relationships for better urban perception modeling.

Method: Three-stage pipeline: 1) Parse street view images with OpenPSG to extract object-predicate-object triplets, 2) Learn compact scene embeddings using GraphMAE (heterogeneous graph autoencoder), 3) Predict six perceptual indicators via neural network from graph embeddings.

Result: Improves perception prediction accuracy by average 26% over image-only baselines, maintains strong cross-city generalization, and provides interpretable insights (e.g., "graffiti on wall" and "car parked on sidewalk" lower perception scores).
...
TL;DR: VisionDirector: A training-free vision-language supervisor that improves generative models' ability to handle complex, multi-goal prompts through structured goal extraction, dynamic editing decisions, and semantic verification.


<details>
  <summary>Details</summary>
Motivation: Current generative models struggle with long, multi-goal prompts common in professional design workflows, failing to satisfy multiple tightly coupled goals spanning layout, object placement, typography, and logo fidelity.

Method: VisionDirector extracts structured goals from long instructions, dynamically chooses between one-shot generation and staged edits, performs micro-grid sampling with semantic verification and rollback after each edit, and logs goal-level rewards. Enhanced with Group Relative Policy Optimization for fine-tuning.

Result: Achieves new SOTA on GenEval (+7% overall) and ImgEdit (+0.07 absolute), reduces edit trajectories (3.1 vs 4.2 steps), and shows qualitative improvements on typography, multi-object scenes, and pose editing.

Conclusion: VisionDirector addresses the brittleness of current generative pipelines for complex prompts, demonstrating significant improvements in handling multi-goal instructions through structured supervision and verification mechanisms.
...
TL;DR: 3SGen is a unified framework for subject, style, and structure conditioning in image generation that uses an MLLM with semantic queries and an Adaptive Task-specific Memory module to disentangle and manage different conditioning modes.


<details>
  <summary>Details</summary>
Motivation: Current image generation approaches handle subject, style, and structure conditioning separately, causing feature entanglement and poor task transferability. There's a need for a unified framework that can handle all three conditioning modes effectively.

Method: 3SGen uses an MLLM with learnable semantic queries for text-image alignment, a VAE branch for visual detail preservation, and an Adaptive Task-specific Memory (ATM) module that dynamically disentangles, stores, and retrieves condition-specific priors (identity, textures, spatial layouts) using a lightweight gating mechanism and scalable memory items.

Result: The framework demonstrates superior performance on the proposed 3SGen-Bench benchmark and other public benchmarks, showing improved cross-task fidelity and controllability across diverse image-driven generation tasks.

Conclusion: 3SGen successfully addresses the limitations of isolated conditioning approaches by providing a unified framework that mitigates inter-task interference, enables compositional inputs, and scales effectively across different image generation tasks.
...
TL;DR: Current GenAI models produce visually realistic human animations but fail to preserve subtle gait patterns needed for biometric identification, revealing they rely on visual appearance rather than motion dynamics.


<details>
  <summary>Details</summary>
Motivation: To investigate whether state-of-the-art GenAI human animation models can preserve the subtle spatio-temporal details needed for person identification through gait biometrics, particularly important when AI-generated videos are evaluated for behavioral biometrics.

Method: Evaluated four different GenAI models across two primary tasks: 1) restoring gait patterns from reference videos under varying complexity conditions, and 2) transferring gait patterns to different visual identities.

Result: While visual quality is mostly high, biometric fidelity remains low in identification tasks. Current GenAI models struggle to disentangle identity from motion, and when texture is disentangled from motion, identification collapses, proving models rely on visual attributes rather than temporal dynamics.

Conclusion: Current GenAI human animation models cannot preserve the subtle gait patterns needed for biometric identification, revealing a fundamental flaw in appearance-based gait recognition and highlighting the need for better motion dynamics modeling.
...
TL;DR: HaMoS: A hand-aware diffusion framework for estimating full-body motion from egocentric videos by leveraging both head trajectories and intermittently visible hand cues, achieving state-of-the-art accuracy and smoothness.


<details>
  <summary>Details</summary>
Motivation: Egocentric vision systems are becoming more common, but estimating full-body motion from first-person videos is challenging because most body parts are invisible. Prior approaches rely too heavily on head trajectories (leading to ambiguity) or assume continuously tracked hands (unrealistic for lightweight devices).

Method: HaMoS is a hand-aware, sequence-level diffusion framework that directly conditions on both head trajectory and intermittently visible hand cues. It uses a novel augmentation method to model real-world conditions like field-of-view limitations and occlusions, and employs local attention to efficiently infer long sequences while considering body shape and field-of-view contexts.

Result: Experiments on public benchmarks show that HaMoS achieves state-of-the-art accuracy and temporal smoothness for egocentric 3D motion reconstruction.

Conclusion: The method represents a practical step toward reliable in-the-wild egocentric 3D motion understanding by effectively leveraging both head and hand information while modeling real-world constraints of egocentric devices.
...
TL;DR: RMLer is a reinforcement learning framework for cross-category concept fusion in text-to-image generation, using mixed features as states, mixing strategies as actions, and visual outcomes as rewards to create novel objects from diverse textual concepts.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image generation methods struggle with synthesizing novel objects by integrating distinct textual concepts from different categories, often producing insufficient concept mixing, conceptual imbalance, superficial combinations, or mere juxtapositions rather than truly fused objects.

Method: RMLer formulates cross-category concept fusion as a reinforcement learning problem: mixed features serve as states, mixing strategies as actions, and visual outcomes as rewards. It uses an MLP-policy network to predict dynamic coefficients for blending cross-category text embeddings, with visual rewards based on semantic similarity and compositional balance, optimized via proximal policy optimization.

Result: Extensive experiments demonstrate RMLer's superiority in synthesizing coherent, high-fidelity objects from diverse categories, outperforming existing methods in generating novel visual concepts.

Conclusion: RMLer provides a robust framework for generating novel visual concepts through reinforcement learning-based concept fusion, with promising applications in film, gaming, and design industries.
...
TL;DR: Think2Seg-RS decouples reasoning and segmentation in remote sensing by training an LVLM prompter to control SAM with geometric prompts via reinforcement learning, achieving SOTA performance and zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing LVLM reasoning segmentation frameworks couple linguistic reasoning and pixel prediction through end-to-end supervised fine-tuning, leading to weak geometric grounding and limited generalization across tasks in remote sensing analysis.

Method: Developed Think2Seg-RS, a decoupled framework that trains an LVLM prompter to control a frozen Segment Anything Model (SAM) via structured geometric prompts using mask-only reinforcement learning objective.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the exposure bias problem in diffusion models where there's a discrepancy between training (using ground-truth noisy data) and testing (using generated noisy data). This mismatch leads to suboptimal performance during sampling.

Method: MixFlow leverages the "Slow Flow" phenomenon where ground-truth interpolations nearest to generated noisy data correspond to higher-noise timesteps. The method uses slowed timestep interpolations (slowed interpolation mixture) for post-training the prediction network at each training timestep.

Result: Experiments show MixFlow improves class-conditional image generation (SiT, REPA, RAE) and text-to-image generation. RAE models with MixFlow achieve strong results: 1.43 FID (unguided) and 1.10 FID (guided) at 256×256, and 1.55 FID (unguided) and 1.10 FID (guided) at 512×512 on ImageNet.

Conclusion: MixFlow effectively mitigates the training-testing discrepancy in diffusion models through slowed interpolation mixtures, leading to improved generation quality across various model architectures and tasks.
...
TL;DR: NIHCs (Neural Implicit Heart Coordinates) create a standardized implicit coordinate system for cardiac anatomy, enabling accurate 3D reconstruction from sparse 2D segmentations with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Accurate reconstruction of cardiac anatomy from sparse clinical images is challenging. Existing neural implicit functions have limited application for mapping anatomical consistency across subjects.

Method: Introduce Neural Implicit Heart Coordinates (NIHCs) - a standardized implicit coordinate system based on universal ventricular coordinates. The method predicts NIHCs from limited 2D segmentations and decodes them into dense 3D segmentations and high-resolution meshes at arbitrary resolution.

Result: Achieved mean Euclidean surface errors of 2.51±0.33 mm in diseased cohort (n=4549) and 2.3±0.36 mm in healthy cohort (n=5576). Enables anatomically coherent reconstruction under severe slice sparsity and noise. Inference time reduced from over 60s to 5-15s compared to traditional pipelines.

Conclusion: NIHCs constitute a robust and efficient anatomical representation for patient-specific 3D cardiac reconstruction from minimal input data, demonstrating high accuracy and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To enable automatic stroke detection and classification in table tennis videos for training, broadcasting, and performance analytics by providing annotated video data that addresses the lack of publicly available, well-licensed datasets in the community.

Method: Extended the existing OpenTTGames dataset by adding frame-accurate shot type annotations (forehand/backhand subtypes), player posture labels (body lean and leg stance), and rally outcome tags. Developed a compact coding scheme and code-assisted labeling procedure for reproducible annotations.

Result: Created an enhanced dataset with detailed stroke understanding capabilities, allowing models to move beyond basic event spotting toward tactical analysis (e.g., identifying winning strokes or advantage-setting plays). Released under CC BY-NC-SA 4.0 license for free non-commercial use.

Conclusion: This work fills a practical gap in table tennis video analysis by providing a publicly available, well-annotated dataset with clear licensing that enables reproducible research and benchmarking for fine-grained stroke understanding in racket sports.
...
TL;DR: DeltaMIL is a novel multiple instance learning framework for Whole Slide Image analysis that uses gated delta rules to filter redundant information and integrate discriminative features, achieving state-of-the-art performance on survival prediction and slide classification tasks.


<details>
  <summary>Details</summary>
Motivation: WSIs contain highly redundant and dispersed information that makes it difficult to identify and integrate discriminative signals. Existing MIL methods either fail to effectively discard uninformative cues or have limited ability to consolidate relevant features from multiple patches, restricting their performance on large and heterogeneous WSIs.

Method: DeltaMIL uses gated delta rules with forgetting and memory mechanisms to filter and integrate information. The delta mechanism dynamically updates memory by removing old values and inserting new ones based on correlation with current patches. The gating mechanism enables rapid forgetting of irrelevant signals. Additionally, it includes a complementary local pattern mixing mechanism to retain fine-grained pathological locality.
...
Conclusion: DeltaMIL effectively enhances extraction of meaningful cues while suppressing redundant or noisy information, improving model robustness and discriminative power. The framework demonstrates strong and consistent performance across diverse WSI analysis tasks.
...
TL;DR: GANeXt is a 3D patch-based GAN using ConvNeXt architecture for unified CT synthesis from MRI and CBCT, achieving accurate anatomical representation for adaptive radiotherapy without region-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: CT synthesis from MRI and CBCT is crucial for adaptive radiotherapy treatment planning, but existing methods often require modality-specific or region-specific approaches. There's a need for a unified framework that can handle different imaging modalities and anatomical regions without extensive fine-tuning.

Method: Proposes GANeXt: a 3D patch-based GAN with U-shaped generator built from stacked 3D ConvNeXt blocks, conditional PatchGAN discriminator, and multi-head segmentation discriminator. Uses combined losses (MAE, perceptual, segmentation-based masked MAE, adversarial) and specialized data preprocessing/augmentation. Training employs AdamW optimizers with warmup/cosine decay schedulers.

Result: Models trained for 3000 epochs (MRI-to-CT) and 1000 epochs (CBCT-to-CT) on full training dataset without fine-tuning. Inference uses sliding-window with 0.8 overlap and average folding for full-size synthetic CT reconstruction.

Conclusion: GANeXt provides a unified framework for CT synthesis across different modalities and anatomical regions, demonstrating the effectiveness of ConvNeXt architecture in medical image synthesis tasks while eliminating the need for region-specific fine-tuning.
...
TL;DR: ReasonCD is a multimodal reasoning change detection model that uses large language models to mine users' implicit task intents from textual descriptions, enabling flexible change detection without relying on explicit CRoI descriptions.


<details>
  <summary>Details</summary>
Motivation: Current remote sensing change detection methods overly rely on explicit textual descriptions of change regions of interest (CRoI), causing near-complete performance failure when presented with implicit CRoI descriptions. There's a need for models that can understand users' implicit task intents.

Method: Proposes ReasonCD model that leverages pre-trained large language models' reasoning capabilities to mine users' implicit task intents from textual descriptions. The model then obtains different change detection results based on these mined intents, enabling flexible detection without explicit CRoI specifications.

Result: Achieves excellent change detection performance with 92.1% F1 score on BCDD dataset. Also demonstrates superior reasoning functionality on annotated SECOND dataset subset, excelling at basic reasoning-based change detection tasks and providing explainable reasoning processes.

Conclusion: ReasonCD effectively addresses the limitation of existing methods by mining implicit task intents through multimodal reasoning, achieving state-of-the-art performance while providing explainable reasoning capabilities that aid human decision-making.
...
TL;DR: SpikeViMFormer is the first spiking neural network framework for drone-view geo-localization that achieves competitive performance with artificial neural networks while offering lower power consumption through spike-driven computation.
...
Method: Proposes SpikeViMFormer with: 1) lightweight spike-driven transformer backbone for coarse-grained feature extraction, 2) spike-driven selective attention block for selective feature enhancement, 3) spike-driven hybrid state space block for learning long-range dependencies, and 4) hierarchical re-ranking alignment learning strategy to refine features and maintain cross-batch consistency for backbone optimization.

Result: Outperforms state-of-the-art SNNs and achieves competitive performance compared with advanced ANNs, demonstrating the effectiveness of the proposed framework for drone-view geo-localization.

Conclusion: SpikeViMFormer successfully addresses the limitations of SNNs for DVGL by mitigating information loss and enabling long-range dependency learning while maintaining the low-power advantages of spike-driven computation, establishing a promising direction for energy-efficient drone-view geo-localization.
...
Method: Proposes DSTED dual-pathway framework with Reliable Memory Propagation (RMP) for temporal coherence via multi-criteria reliability assessment, and Uncertainty-Aware Prototype Retrieval (UPR) for ambiguous phase enhancement through learnable prototypes and adaptive matching, balanced by confidence-driven gate.

Result: Achieves 84.36% accuracy and 65.51% F1-score on AutoLaparo-hysterectomy, surpassing second-best by 3.51% and 4.88% respectively. RMP contributes 2.19% gain, UPR 1.93%, with synergistic effects when combined. Substantially reduces temporal jitter and improves challenging phase transitions.
...
TL;DR: MOON++ is a novel multimodal framework that uses non-contrast CT scans to assess esophageal varices severity by analyzing multiple organ relationships, achieving superior performance over single-organ methods.


<details>
  <summary>Details</summary>
Motivation: Esophageal varices affect 60% of cirrhosis patients with high bleeding risk, but current diagnosis requires invasive endoscopy. Non-contrast CT offers a potential non-invasive alternative that hasn't been fully utilized in clinical practice.

Method: MOON++ synthesizes imaging characteristics of esophagus, liver, and spleen through multimodal learning, incorporating clinical knowledge priors about organ volumetric relationships with liver disease severity. The framework was evaluated on 1,631 patients with endoscopically confirmed EV classified into four severity grades.

Result: MOON++ achieved AUC of 0.894 vs 0.803 for severe grade EV classification (G3 vs <G3) and 0.921 vs 0.793 for moderate-to-severe grade differentiation (>=G2 vs <G2), outperforming conventional single-organ methods. Validation included 239 patient cases and independent testing on 289 cases, plus a reader study with experienced radiologists.

Conclusion: MOON++ represents the first comprehensive multi-organ NCCT analysis framework incorporating clinical knowledge priors for EV assessment, potentially offering a promising non-invasive diagnostic alternative to invasive endoscopy.
...
TL;DR: dMLLM-TTS is a novel test-time scaling framework for diffusion multimodal LLMs that improves generation quality and efficiency through hierarchical search and self-verified feedback.


<details>
  <summary>Details</summary>
Motivation: Current Test-Time Scaling (TTS) methods for diffusion multimodal LLMs are inefficient (O(NT) complexity) and require external verifiers, limiting their practical deployment and full generative potential.

Method: Proposes dMLLM-TTS with two innovations: 1) hierarchical search algorithm with O(N+T) complexity for adaptive trajectory exploration, and 2) self-verified feedback mechanism using dMLLMs' intrinsic image understanding capabilities to assess text-image alignment.

Result: Extensive experiments on GenEval benchmark across three dMLLMs (Lumina-DiMOO, MMaDA, Muddit) show substantial improvement in generation quality with up to 6x greater efficiency than linear search.

Conclusion: dMLLM-TTS effectively addresses the efficiency and quality challenges in test-time scaling for diffusion multimodal LLMs, enabling practical deployment while maintaining strong generation performance.
...
TL;DR: The paper proposes a collaborative deep watermarking framework with bidirectional communication between embedder and extractor, replacing the traditional isolated pipeline with mutual teaching and coordinated optimization.


<details>
  <summary>Details</summary>
Motivation: Existing deep watermarking methods have a fixed pipeline where embedder and extractor are weakly coupled and optimized in isolation, lacking explicit collaboration and decoding-aware guidance during training.

Method: Introduces Collaborative Interaction Mechanism (CIM) for bidirectional communication between embedder and extractor, enabling mutual-teacher training. Also proposes Adaptive Feature Modulation Module (AFMM) for content-aware feature regulation by decoupling modulation structure and strength.

Result: The method outperforms state-of-the-art approaches in watermark extraction accuracy on real-world and AI-generated datasets while maintaining high perceptual quality, showing strong robustness and generalization.

Conclusion: The architecture-level redesign enables robustness to emerge from coordinated representation learning rather than exhaustive distortion simulation, fundamentally changing how watermarking systems learn robustness.
...
TL;DR: D2Pruner is a token pruning framework for MLLMs that combines debiased importance scoring with structural pruning to maintain both general understanding and fine-grained localization performance while significantly reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Current token pruning methods for MLLMs fail catastrophically on fine-grained localization tasks due to positional bias in importance-based methods and structural blindness in diversity-based methods, creating a need for a better approach.
...
Conclusion: D2Pruner successfully addresses limitations of existing pruning methods by combining debiased importance with structural pruning, achieving exceptional efficiency and fidelity for both general understanding and fine-grained localization tasks in MLLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning SLR models require extensive computational resources, making them unsuitable for edge devices. Need for lightweight, real-time solutions for practical deployment.
...
Result: Achieved 60.85% top-1, 85.86% top-5, and 91.74% top-10 accuracy on WLASL dataset. Training time reduced to 18.67 seconds vs. 55+ minutes for Bi-GRU, enabling real-time performance.

Conclusion: Proposed system offers lightweight, cost-effective solution for real-time SLR on edge devices, addressing computational constraints while maintaining competitive accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Current emotion-oriented image generation methods suffer from an "affective shortcut" where emotions are approximated to semantics, but emotion research shows they are not equivalent. There's a need for better emotion-oriented image generation that goes beyond semantic associations.

Method: Two-module framework: 1) MC-Diffusion - cross-modal collaborative diffusion model integrating visual prompts with textual prompts for emotion guidance, enhanced with negative visual prompts in DPO optimization. 2) MC-Agent - cross-modal collaborative agent system using multi-agents to simulate human subjectivity and chain-of-concept workflow for rewriting textual prompts with better emotional expression.
...
Conclusion: Emotion-Director successfully addresses the affective shortcut problem in emotion-oriented image generation by combining cross-modal collaboration between visual and textual prompts with intelligent prompt rewriting, enabling more authentic emotional expression beyond semantic associations.
...
<details>
  <summary>Details</summary>
Motivation: The combinatorial explosion problem in DMIR arises from processing two images simultaneously, leading to exponential growth in feature combinations and interference during feature modeling.

Method: Proposes Dynamic Stream Network (DySNet) with two innovations: Adaptive Stream Basin (AdSB) module for dynamic receptive field adjustment, and Dynamic Stream Attention (DySA) mechanism for generating dynamic weights to find valuable feature relationships.

Result: Extensive experiments show DySNet consistently outperforms state-of-the-art DMIR methods and demonstrates outstanding generalization ability.

Conclusion: DySNet effectively solves the combinatorial explosion problem in DMIR through dynamic adjustment of receptive fields and weights, enabling better feature relationship modeling while ignoring interfering combinations.
...
TL;DR: Physics-aware deep learning framework (FusionNet) combines SWIR ratios and TIR data through intermediate fusion to model long-term environmental changes, outperforming SOTA methods across spectral configurations.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal deep learning models have poor alignment with physical signal formation processes, leading to brittle performance under cross-spectral and real-world conditions. Approaches focusing on direct thermal cues fail to capture indirect but persistent environmental changes from sustained heat emissions.

Method: Proposes FusionNet - an intermediate fusion architecture integrating geological SWIR ratios (sensitive to soil property changes) with TIR data. The backbone embeds trainable differential signal-processing priors in convolutional layers, uses mixed pooling strategies, and wider receptive fields for cross-spectral robustness.

Result: DGCNN achieves 88.7% accuracy on SWIR ratio alone, FusionNet reaches 90.6% accuracy, outperforming state-of-the-art baselines across five spectral configurations. Transfer learning experiments show ImageNet pretraining degrades TIR performance, highlighting need for modality-aware training.

Conclusion: Combining physics-aware feature selection with principled deep learning architectures yields robust, generalizable representations for multi-spectral learning. First-principles signal modeling improves performance under challenging cross-spectral conditions.
...
TL;DR: This paper proposes two novel methods to enhance multimodal large language models' medical reasoning capabilities for anatomy recognition: Anatomical Similarity Curriculum Learning and Group Diversity Question Augmentation, achieving significant improvements on medical VQA benchmarks.


<details>
  <summary>Details</summary>
Motivation: MLLMs show impressive progress in natural image reasoning but remain underexplored in medical imaging, especially clinical anatomical surgical images. Anatomy understanding requires precise, clinically coherent answers, but conventional SFT strategies are limited by medical data complexity and scarce expert annotations. While GRPO can enhance reasoning without large data, it has weaknesses in anatomy recognition: ineffective knowledge sharing between anatomical structures and premature convergence to single reasoning paths.

Method: Two novel methods: 1) Anatomical Similarity Curriculum Learning - progressive learning strategy controlling question difficulty via similarity of answer choices, enabling incremental mastery of complex problems. 2) Group Diversity Question Augmentation - expands model's search space for difficult queries to mitigate uniform response tendencies.
...
Conclusion: The proposed methods effectively address GRPO's weaknesses in anatomy recognition by enabling progressive learning and diverse reasoning exploration, significantly enhancing MLLMs' medical reasoning capabilities for clinical anatomical understanding.
...
TL;DR: pbnds+ is a physics-based neural deferred shading pipeline using CNNs to reduce parameters and improve performance in shading/relighting, with energy regularization for dark scenes.


<details>
  <summary>Details</summary>
Motivation: Current neural rendering methods using MLPs have too many parameters, require high computation, and struggle with generalization from limited/unbalanced data, especially in dark illumination conditions.

Method: Proposes pbnds+ pipeline using convolutional neural networks instead of MLPs to reduce parameters, with energy regularization to constrain model reflection in dark illumination conditions.
...
Conclusion: The proposed physics-based neural deferred shading pipeline with CNNs and energy regularization effectively addresses parameter efficiency and dark scene challenges in neural rendering.
...
TL;DR: Multi-modal transformer architecture for soccer analysis that integrates player trajectories, player types, and image crops to perform ball trajectory inference, ball state classification, and ball possessor identification without direct ball tracking.


<details>
  <summary>Details</summary>
Motivation: To analyze soccer scenes from tactical camera footage more robustly than prior methods that rely heavily on accurate ball tracking or handcrafted heuristics, especially under noisy or occluded conditions in real top league matches.

Method: Integrates three input modalities (player trajectories, player types, image crops) using sociotemporal transformer blocks with CropDrop pre-training strategy - a modality-specific masking approach that prevents over-reliance on image features and encourages cross-modal pattern learning.

Result: Substantial improvements over state-of-the-art baselines on all three tasks (ball trajectory inference, ball state classification, ball possessor identification) on a large-scale dataset.

Conclusion: Combining structured and visual cues in transformer architecture is beneficial, and realistic masking strategies like CropDrop are important for effective multi-modal learning in soccer analysis.
...
TL;DR: Open-source 3D Slicer extension for virtual planning and quantitative comparison of preformed orbital plates to improve surgical outcomes.


<details>
  <summary>Details</summary>
Motivation: Poor adaptation of orbital implants leads to complications and revision surgery. Surgeons lack standardized tools to quantitatively compare plate fit across vendors, sizes, and patient anatomy.
...
TL;DR: CASA (Cross-Attention via Self-Attention) is a new vision-language model architecture that improves cross-attention VLMs by enabling local text-to-text interaction in cross-attention layers, reducing the performance gap with full token insertion while maintaining efficiency for high-resolution images and long contexts.


<details>
  <summary>Details</summary>
Motivation: Current VLMs using full token insertion (where image tokens are inserted into text streams) are computationally expensive for high-resolution images, long conversations, or streaming videos. Cross-attention VLMs are more efficient but suffer from performance gaps, especially on tasks requiring fine-grained visual details.

Method: CASA enables local text-to-text interaction within dedicated cross-attention layers, allowing text tokens to attend to each other while maintaining efficient cross-modal attention between text and visual features.

Result: CASA substantially reduces the performance gap with full token insertion models on common image understanding benchmarks while maintaining the same scalability advantages as cross-attention models for long-context multimodal tasks like streaming video captioning.

Conclusion: The key to improving cross-attention VLMs is enabling local text-to-text interaction in cross-attention layers, and CASA provides an efficient paradigm that achieves near full token insertion performance while maintaining scalability for demanding multimodal applications.
...
TL;DR: StoryMem transforms single-shot video diffusion models into multi-shot storytellers using visual memory banks and novel M2V design, achieving superior cross-shot consistency for long-form video storytelling.


<details>
  <summary>Details</summary>
Motivation: Visual storytelling requires generating multi-shot videos with cinematic quality and long-range consistency, which current single-shot video diffusion models cannot achieve due to lack of memory across shots.

Method: Proposes StoryMem with Memory-to-Video (M2V) design: maintains dynamic memory bank of keyframes, injects memory via latent concatenation and negative RoPE shifts with LoRA fine-tuning, uses semantic keyframe selection and aesthetic filtering.
...
TL;DR: ActAvatar: A talking avatar framework with phase-level precision action control through textual guidance, addressing text-following, temporal alignment, and modality interference issues.
...
Method: Three core innovations: 1) Phase-Aware Cross-Attention (PACA) decomposes prompts into global base and temporally-anchored phase blocks; 2) Progressive Audio-Visual Alignment aligns modality influence with hierarchical feature learning; 3) Two-stage training strategy for robust audio-visual correspondence then action control injection.

Result: ActAvatar significantly outperforms state-of-the-art methods in both action control and visual quality based on extensive experiments.

Conclusion: ActAvatar achieves phase-level precision in action control through textual guidance by capturing both action semantics and temporal context, solving key challenges in talking avatar generation.
...
TL;DR: BabyFlow is a generative AI model that disentangles facial identity and expression in infant faces using normalizing flows, enabling independent control over both aspects for improved 3D reconstruction and expression synthesis.


<details>
  <summary>Details</summary>
Motivation: Early detection of developmental disorders can benefit from analyzing infant craniofacial morphology, but modeling infant faces is challenging due to limited data and frequent spontaneous expressions.

Method: Uses normalizing flows to learn flexible, probabilistic representations of expressive infant faces without restrictive linear assumptions. Addresses data scarcity through cross-age expression transfer, adapting expressions from adult 3D scans to enrich infant datasets with realistic expressive variants.

Result: Improves 3D reconstruction accuracy, particularly in highly expressive regions (mouth, eyes, nose). Supports synthesis and modification of infant expressions while preserving identity. When integrated with diffusion models, generates high-fidelity 2D infant images with consistent 3D geometry.

Conclusion: BabyFlow provides powerful tools for data augmentation and early facial analysis, enabling better modeling of infant facial morphology despite data limitations and expression variability.
...
TL;DR: RoVTL is a robust vision-tabular learning framework that handles any level of tabular data availability (0-100%) through contrastive pretraining with missingness augmentation and gated cross-attention fusion, using a novel Tabular More vs. Fewer loss for consistent performance across data completeness scenarios.


<details>
  <summary>Details</summary>
Motivation: Real-world medical datasets often have incomplete tabular data, while large biobanks provide extensive tabular information. This discrepancy requires methods that can leverage all tabular data during training while remaining robust to missing values at inference.

Method: Two-stage framework: 1) Contrastive pretraining with tabular attribute missingness as data augmentation, 2) Downstream task tuning using gated cross-attention module for multimodal fusion with Tabular More vs. Fewer loss and disentangled gradient learning.
...
Conclusion: RoVTL provides a robust solution for handling varying levels of tabular data availability in multimodal learning, demonstrating effectiveness across medical imaging and natural image domains with consistent performance regardless of data completeness.
...
TL;DR: MLLMs struggle with fine-grained spatial reasoning like map route tracing due to lack of precise annotations. The paper introduces a synthetic data generation pipeline to create 23k path samples for fine-tuning, which significantly improves MLLMs' spatial reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) have limitations in fine-grained spatial understanding tasks like route tracing on maps, often failing to respect fundamental path constraints. This is partly due to the high cost and difficulty of collecting large-scale, pixel-accurate path annotations needed for training.

Method: The authors introduce a scalable synthetic data generation pipeline that uses synthetic map images and pixel-level parsing to automatically produce precise annotations for path tracing tasks. They construct a fine-tuning dataset of 23k path samples across 4k maps using this pipeline.

Result: Fine-tuning both open-source and proprietary MLLMs with the synthetic dataset substantially improves robustness, raising success rates by up to 6.4 points on MapBench while also reducing path-tracing error (NDTW).

Conclusion: Fine-grained spatial reasoning capabilities, which are absent in pretrained MLLMs, can be explicitly taught through synthetic supervision. The synthetic data generation approach provides a scalable solution to address the annotation bottleneck for spatial reasoning tasks.
...
TL;DR: This paper proposes a diffusion-based generative AI pipeline for agricultural plant imagery, addressing data scarcity through synthetic image generation, indoor-to-outdoor translation, and expert preference alignment.


<details>
  <summary>Details</summary>
Motivation: Agricultural AI requires large, diverse, high-quality plant image datasets, but real field data collection is costly, labor-intensive, and seasonally constrained, creating a significant data bottleneck.

Method: Three-stage approach: 1) Fine-tune Stable Diffusion on captioned indoor/outdoor plant images for text-conditioned synthesis; 2) Use DreamBooth text inversion and image-guided diffusion for indoor-to-outdoor translation; 3) Implement preference-guided fine-tuning with expert-scored reward models.

Result: Synthetic images effectively augment training data (improved Inception Score/FID), enhance phenotype classification accuracy, improve weed detection/classification with YOLOv8, and produce more stable, expert-aligned outputs through preference tuning.

Conclusion: The proposed diffusion-based generative pipeline offers a practical, data-efficient solution for agricultural AI by overcoming data scarcity through synthetic generation, domain translation, and expert alignment.
...
<details>
  <summary>Details</summary>
Motivation: Current deformation-based approaches for dynamic 3D scenes lack motion coherence, temporal consistency, and the ability to extrapolate beyond observed time ranges. They also don't model the underlying motion laws, making them sample-inefficient and limited in controllability.
...
Result: EvoGS achieves better motion coherence and temporal consistency compared to deformation-field baselines while maintaining real-time rendering. It enables sample-efficient learning from sparse supervision, temporal extrapolation beyond observed ranges, and compositional dynamics for controllable scene synthesis.

Conclusion: Reinterpreting 4D Gaussian Splatting as a continuous-time dynamical system with learned neural fields provides superior motion modeling capabilities over deformation-based approaches, enabling more coherent, consistent, and controllable dynamic scene representation.
...
TL;DR: Over++ is a video effect generation framework that creates realistic environmental interactions (shadows, reflections, dust, splashes) between foreground and background layers while preserving the original video content, addressing the limitations of existing video generative models and inpainting methods.


<details>
  <summary>Details</summary>
Motivation: Professional video compositing requires manual creation of environmental interactions between foreground subjects and background layers. Existing video generative models fail to preserve input videos when adding effects, and current video inpainting methods either need costly per-frame masks or produce implausible results.

Method: Over++ is a video effect generation framework that makes no assumptions about camera pose, scene stationarity, or depth supervision. The approach includes constructing a paired effect dataset for the task and introducing an unpaired augmentation strategy that preserves text-driven editability. The method supports optional mask control and keyframe guidance without requiring dense annotations.
...
Conclusion: Over++ successfully addresses the augmented compositing task by synthesizing realistic, semi-transparent environmental effects conditioned on text prompts and input video layers while preserving the original scene, offering a practical solution for professional video compositing workflows.
...
TL;DR: A novel multimodal framework integrating retinal images, clinical text, and structured patient data achieves near-perfect cross-modal retrieval (99.94% Recall@1) and state-of-the-art DR classification (97-98%) by addressing CLIP's limitations in medical domains.


<details>
  <summary>Details</summary>
Motivation: General-domain vision-language models like CLIP struggle with medical image-text alignment, particularly for ophthalmological applications. There's a critical need for accurate automated diagnostic systems for diabetic retinopathy, a leading cause of preventable blindness.

Method: A knowledge-enhanced joint embedding framework with separate encoders for each modality: Vision Transformer for retinal images, Bio-ClinicalBERT for clinical narratives, and MLP for structured patient data. These are fused through a joint transformer with modality-specific embeddings, trained using contrastive losses, reconstruction losses, and classification losses for DR severity grading.

Result: Achieves near-perfect text-to-image retrieval (99.94% Recall@1 vs CLIP's 1.29%), state-of-the-art classification (97.05% for SDRG, 97.97% for ICDR), and strong generalizability on unseen data (93.95% Recall@1 vs CLIP's 0.22%).

Conclusion: The multimodal training approach effectively captures cross-modal relationships in medical domains, establishing superior retrieval capabilities and robust diagnostic performance for diabetic retinopathy, significantly outperforming fine-tuned general-domain models.
...
TL;DR: A novel deep learning framework combining multi-head selective state-space models with lightweight channel MLP achieves state-of-the-art MRI super-resolution with exceptional computational efficiency (0.9M parameters, 57 GFLOPs).


<details>
  <summary>Details</summary>
Motivation: High-resolution MRI is crucial for diagnosis but suffers from long acquisition times. Existing super-resolution methods face trade-offs between fidelity and computational efficiency, limiting clinical integration.
...
Result: Superior performance across 7T brain (SSIM=0.951, PSNR=26.90 dB) and 1.5T prostate (SSIM=0.770, PSNR=27.15 dB) datasets. Outperformed all baselines including GANs, transformers, Mamba, and diffusion models. Achieved 99.8% parameter reduction and 97.5% computation reduction versus Res-SRDiff.

Conclusion: The framework provides an efficient, accurate MRI super-resolution solution with strong clinical translation potential due to low computational demands and state-of-the-art performance across diverse datasets.
...
TL;DR: WorldWarp bridges 3D geometry and 2D generation for consistent video synthesis by combining a 3D Gaussian Splatting cache with spatio-temporal diffusion refinement.


<details>
  <summary>Details</summary>
Motivation: Current video generation methods struggle with geometric consistency, especially for occluded areas and complex camera trajectories, due to the disconnect between 3D geometry requirements and 2D generative model operations.

Method: WorldWarp couples a 3D structural anchor (Gaussian Splatting cache) with a 2D generative refiner (Spatio-Temporal Diffusion model). It maintains an online 3D geometric cache that warps historical content into novel views, then uses a diffusion model with spatio-temporal varying noise schedules to fill occlusions and refine artifacts.

Result: The framework achieves state-of-the-art fidelity by ensuring 3D logic guides structure while diffusion logic perfects texture, maintaining consistency across video chunks through dynamic cache updates.

Conclusion: WorldWarp successfully bridges the gap between 3D geometric consistency and 2D generative modeling, enabling geometrically consistent video generation with complex camera trajectories while handling occlusions effectively.
...
<details>
  <summary>Details</summary>
Motivation: There's a misalignment between tokenizers (trained for clean image reconstruction) and AR generators (optimized only for token likelihood), leading to generated token sequences that decode into low-quality images without direct pixel-space supervision.

Method: VA-π formulates generator-tokenizer alignment as variational optimization, deriving an ELBO that unifies pixel reconstruction and autoregressive modeling. It uses reinforcement-based alignment treating AR generator as a policy, with pixel-space reconstruction quality as intrinsic reward measured under teacher forcing.

Result: With only 1% ImageNet-1K data and 25 minutes tuning, reduces FID from 14.36 to 7.65 and improves IS from 86.55 to 116.70 on LlamaGen-XXL. Also improves text-to-image performance on GenEval for both LlamaGen (0.306 to 0.339) and Janus-Pro (0.725 to 0.744).

Conclusion: VA-π enables rapid adaptation of existing AR generators without tokenizer retraining or external reward models, providing direct pixel-level guidance through a principled variational optimization framework.
...
TL;DR: The paper introduces a new benchmark for evaluating spatial intelligence in Multimodal Large Language Models (MLLMs) using outdoor pedestrian-perspective videos with precise 3D sensor data, revealing that current models rely on linguistic priors rather than grounded visual reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack robust spatial intelligence, and existing benchmarks are inadequate - they either focus on simplified qualitative reasoning or use domain-specific indoor data without outdoor datasets containing verifiable metric ground truth.

Method: Created a large-scale benchmark from pedestrian-perspective videos captured with synchronized stereo cameras, LiDAR, and IMU/GPS sensors, providing metrically precise 3D information to automatically generate hierarchical spatial reasoning questions.

Result: Performance gains seen in structured indoor benchmarks disappear in open-world settings. Analysis using synthetic abnormal scenes and blinding tests confirms current MLLMs depend heavily on linguistic priors rather than grounded visual reasoning.

Conclusion: The benchmark provides a principled platform for diagnosing MLLM limitations and advancing physically grounded spatial intelligence, revealing critical gaps in current models' spatial reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods operate in hand-centric coordinates and ignore scene context, which hinders metric accuracy and practical use for reconstructing object manipulation from videos.

Method: 1) Use data-driven foundation models to initialize object mesh/poses, scene point cloud, and hand poses. 2) Apply two-stage optimization to recover complete hand-object motion from grasping to interaction, consistent with observed scene information.
...
Conclusion: The proposed approach addresses key challenges in scene reconstruction, hand-object depth ambiguity, and physical plausibility by incorporating scene context through foundation models and optimization.
...
Method: Two-stage approach: 1) Adaptive Visual Planning - generates structured visual checklists to identify visual elements needing consistency; 2) Iterative Visual Correction - performs self-reflection using checklists and refines results iteratively. Uses supervised finetuning to teach planning, reflection, and refinement, plus flow-GRPO with customized visual checking reward.
...
Conclusion: Integrating visual context consistency into reasoning through visual planning and iterative correction significantly improves multimodal generation quality by better maintaining key visual features from reference images.
...
TL;DR: Interact2Ar is an autoregressive diffusion model that generates realistic full-body human-human interactions with detailed hand motions, addressing limitations of previous methods that ignore hands and lack adaptability.


<details>
  <summary>Details</summary>
Motivation: Previous methods for generating human-human interactions have two main limitations: they ignore hand motions (reducing realism/expressivity) and use non-autoregressive approaches that can't capture the reactive, adaptive nature of real interactions.

Method: Interact2Ar is an end-to-end text-conditioned autoregressive diffusion model with parallel branches for hand kinematics, coupled with a novel memory technique for efficient large context windows to handle interaction variability.

Result: The model achieves state-of-the-art performance in generating full-body human-human interactions, enables downstream applications like temporal motion composition, real-time adaptation to disturbances, and extension to multi-person scenarios.

Conclusion: Interact2Ar successfully addresses key limitations in human interaction generation by incorporating hand motions and autoregressive adaptability, validated through robust evaluation metrics specifically designed for full-body interactions.
...
TL;DR: The paper proposes the Prism Hypothesis - that semantic encoders capture low-frequency components for abstract meaning while pixel encoders retain high-frequency details, and introduces Unified Autoencoding (UAE) to harmonize both in a single latent space.


<details>
  <summary>Details</summary>
Motivation: To understand the spectral characteristics of different encoders and establish a unifying perspective that connects encoder behavior to underlying spectral structure, enabling better integration of semantic abstraction and pixel-level detail.
...
Conclusion: The Prism Hypothesis provides a unifying framework for understanding encoder behavior through spectral analysis, and UAE successfully demonstrates how semantic and pixel information can be harmonized in a single representation space.
...
### [246] [Privacy-Aware Sharing of Raw Spatial Sensor Data for Cooperative Perception](https://arxiv.org/abs/2512.16265)
*Bangya Liu,Chengpo Yan,Chenghao Jiang,Suman Banerjee,Akarsh Prabhakara*

Main category: cs.NI

TL;DR: SHARP is a framework for privacy-preserving cooperative perception that addresses privacy concerns in raw sensor data sharing between vehicles while enabling robust scene understanding.


<details>
  <summary>Details</summary>
Motivation: While cooperative perception using raw spatial sensor data improves accuracy, it raises significant privacy concerns that discourage automakers from adopting such systems. The paper aims to address these privacy issues to enable eventual adoption of raw data-based cooperative perception.

Method: The paper presents SHARP, a research framework designed to minimize privacy leakage in cooperative perception systems. The framework likely involves techniques to protect sensitive information while still enabling effective data sharing for scene understanding.

Result: The paper proposes a framework that addresses privacy concerns in raw sensor data sharing, making cooperative perception more viable for adoption by automakers and stakeholders.

Conclusion: SHARP provides a pathway toward privacy-preserving cooperative perception, but realizing this framework requires addressing open questions across networked systems, mobile computing, perception research, industry, and government sectors.
...
### [247] [Standardized Evaluation of Automatic Methods for Perivascular Spaces Segmentation in MRI -- MICCAI 2024 Challenge Results](https://arxiv.org/abs/2512.18197)
...
TL;DR: MICCAI 2024 EPVS Challenge benchmarked automated segmentation methods for enlarged perivascular spaces using multi-site MRI data, revealing cross-site generalization challenges.


<details>
  <summary>Details</summary>
Motivation: Enlarged perivascular spaces (EPVS) are important imaging markers for cerebral small vessel disease and neurodegenerative conditions, but automatic segmentation remains challenging due to small size, variable morphology, similarity to other features, and limited annotated datasets.

Method: Organized EPVS Challenge at MICCAI 2024 with diverse multi-site dataset (100 training, 50 validation, 50 testing scans from UK, Singapore, China). Seven teams implemented deep learning approaches primarily based on U-Net architectures with innovations in multi-modal processing, ensemble strategies, and transformer-based components.

Result: Winning method used MedNeXt architecture with dual 2D/3D strategy for varying slice thicknesses. Top solutions performed well on seen datasets but showed significant performance degradation on unseen Shanghai cohort, highlighting cross-site generalization challenges due to domain shift.

Conclusion: The challenge establishes an important benchmark for EPVS segmentation methods and underscores the need for continued development of robust algorithms that can generalize across diverse clinical settings with different MRI protocols and demographics.
...
### [248] [Measuring Fine-Grained Negotiation Tactics of Humans and LLMs in Diplomacy](https://arxiv.org/abs/2512.18292)
*Wenkai Li,Lynnette Hui Xian Ng,Andy Liu,Daniel Fried*

Main category: cs.CY

TL;DR: LLM-based analysis of negotiation styles in Diplomacy game shows correlations between negotiation tactics and success, with fine-tuning enabling more human-like LLM negotiation behaviors.


<details>
  <summary>Details</summary>
Motivation: Shift focus from just measuring negotiation success to studying negotiation styles and strategies, using Diplomacy as a rich environment for natural language negotiation analysis.

Method: Used LLM-as-a-judge to annotate human-human Diplomacy games for fine-grained negotiation tactics from sociological taxonomy, combining It Takes Two and WebDiplomacy datasets.

Result: Demonstrated reliability of LLM-as-a-Judge framework, showed strong correlations between negotiation features and success in Diplomacy, and found differences between LLM and human negotiation strategies.

Conclusion: Fine-tuning can steer LLM agents toward more human-like negotiation behaviors, enabling better study and modeling of negotiation styles in strategic settings.
...
### [249] [Epistemological Fault Lines Between Human and Artificial Intelligence](https://arxiv.org/abs/2512.19466)
*Walter Quattrociocchi,Valerio Capraro,Matjaž Perc*

Main category: cs.CY

TL;DR: LLMs are not true epistemic agents but stochastic pattern-completion systems that create an illusion of understanding through linguistic plausibility rather than genuine knowledge formation.


<details>
  <summary>Details</summary>
Motivation: To expose the fundamental mismatch between human cognition and LLM operation, challenging the common perception that LLMs possess artificial intelligence comparable to human reasoning.

Method: Historical analysis of AI evolution from symbolic systems to transformers, systematic mapping of human vs. artificial epistemic pipelines, and identification of seven epistemic fault lines.

Result: Identification of "Epistemia" - a condition where linguistic plausibility substitutes for genuine epistemic evaluation, creating the feeling of knowing without actual judgment.

Conclusion: LLMs are fundamentally different from human epistemic agents, requiring new approaches to AI evaluation, governance, and epistemic literacy in AI-dominated societies.
...
### [250] [A Multi-agent Text2SQL Framework using Small Language Models and Execution Feedback](https://arxiv.org/abs/2512.18622)
*Thanh Dat Hoang,Thanh Trung Huynh,Matthias Weidlich,Thanh Tam Nguyen,Tong Chen,Hongzhi Yin,Quoc Viet Hung Nguyen*

Main category: cs.DB

TL;DR: MATS is a multi-agent Text2SQL framework for small LLMs that uses specialized agents and reinforcement learning to achieve large LLM-level accuracy with fewer parameters on single-GPU servers.


<details>
  <summary>Details</summary>
Motivation: Companies need in-house Text2SQL solutions due to privacy/cost concerns with external LLM services, but small LLMs lack the generalization capabilities for complex SQL generation tasks.

Method: Multi-agent framework with specialized roles assigned to auxiliary agents, using reinforcement learning to align agents through execution feedback, reducing individual workloads while fostering interaction.

Result: MATS achieves accuracy on-par with large-scale LLMs on benchmark datasets while using significantly fewer parameters and running on single-GPU servers.

Conclusion: MATS enables practical, privacy-preserving Text2SQL solutions using small LLMs by leveraging multi-agent collaboration and reinforcement learning, making enterprise-grade SQL generation accessible without external LLM dependencies.
...
### [251] [Stable and Efficient Single-Rollout RL for Multimodal Reasoning](https://arxiv.org/abs/2512.18215)
*Rui Liu,Dian Yu,Lei Ke,Haolin Liu,Yujun Zhou,Zhenwen Liang,Haitao Mi,Pratap Tokekar,Dong Yu*

Main category: cs.LG

TL;DR: MSSR is a group-free RLVR framework that enables stable single-rollout training for multimodal reasoning by using entropy-based advantage shaping to prevent training collapse.


<details>
  <summary>Details</summary>
Motivation: Existing single-rollout RLVR methods suffer from severe instability in multimodal contexts, often leading to training collapse, creating a need for a stable and efficient approach to multimodal reasoning.

Method: MSSR uses an entropy-based advantage-shaping mechanism that adaptively regularizes advantage magnitudes to prevent collapse and maintain training stability in multimodal single-rollout settings.

Result: MSSR achieves superior training compute efficiency (half the steps for similar accuracy), surpasses group-based baselines when trained for same steps, and shows consistent generalization improvements across five reasoning benchmarks.

Conclusion: MSSR enables stable, compute-efficient, and effective RLVR for complex multimodal reasoning tasks by addressing the training efficiency-stability trade-off in multimodal contexts.
...
### [252] [MAGIC: Achieving Superior Model Merging via Magnitude Calibration](https://arxiv.org/abs/2512.19320)
...
<details>
  <summary>Details</summary>
Motivation: Model merging aims to combine specialized models into a unified model without additional training, but current methods focus on directional alignment while neglecting feature magnitude. Magnitude is vulnerable to perturbations from merging operations (parameter fusion, sparsification), causing feature deviations and performance degradation in merged models.

Method: Proposes MAGIC (MAGnItude Calibration) with three variants: 1) Feature Space Calibration (FSC) - realigns merged model's features using small unlabeled data, 2) Weight Space Calibration (WSC) - extends calibration to weight space without additional data, and 3) Dual Space Calibration (DSC) - combines both approaches.

Result: MAGIC consistently boosts performance across diverse Computer Vision tasks (+4.3% on eight datasets) and NLP tasks (+8.0% on Llama) without additional training.

Conclusion: Feature magnitude calibration is crucial for effective model merging, and MAGIC provides an effective plug-and-play solution that addresses magnitude perturbations to preserve behavioral characteristics of specialized models in merged models.
...
### [253] [Bottom-up Policy Optimization: Your Language Model Policy Secretly Contains Internal Policies](https://arxiv.org/abs/2512.19673)
*Yuqiao Tan,Minzheng Wang,Shizhu He,Huanxuan Liao,Chengfeng Zhao,Qiunan Lu,Tian Liang,Jun Zhao,Kang Liu*

Main category: cs.LG

TL;DR: The paper proposes decomposing LLM policies into internal layer and modular components, revealing different entropy patterns across layers, and introduces Bottom-up Policy Optimization (BuPO) to optimize early-layer policies for better reasoning performance.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches treat LLMs as unified policies, overlooking internal mechanisms. Understanding policy evolution across layers and modules is crucial for targeted optimization and unraveling complex reasoning mechanisms.

Method: Decompose LLM policy by leveraging Transformer residual stream split and equivalence between hidden states composition with unembedding matrix. This reveals Internal Layer Policies (individual layers) and Internal Modular Policies (self-attention and FFN components). Analyze entropy patterns, then propose Bottom-up Policy Optimization (BuPO) that directly optimizes internal layer policy during early training.

Result: Analysis shows: (a) Early layers keep high entropy for exploration, top layers converge to near-zero entropy for refinement, with patterns varying across model series. (b) LLama converges rapidly in final layer, while Qwen-series (especially Qwen3) exhibits more human-like progressive reasoning. BuPO achieves superior performance on complex reasoning benchmarks by reconstructing foundational reasoning capabilities.

Conclusion: Decomposing LLM policies reveals important internal mechanisms and entropy patterns. BuPO's bottom-up optimization approach effectively enhances reasoning capabilities by targeting early-layer policy optimization, demonstrating the value of understanding and optimizing internal policy components.
...
### [254] [NOVA: Discovering Well-Conditioned Winograd Transforms through Numerical Optimization of Vandermonde Arithmetic](https://arxiv.org/abs/2512.18453)
*Jayant Lohia*

Main category: cs.LG

TL;DR: NOVA discovers stable fractional transforms for Winograd convolution, overcoming numerical instability in low-precision computing and restoring full accuracy in FP16 inference without retraining.


<details>
  <summary>Details</summary>
Motivation: Winograd convolution faces critical numerical instability in low-precision computing (FP16/Int8) as tile sizes scale for efficiency, with condition numbers exploding to unusable levels (kappa = 2×10^5 for F(8,3)).

Method: NOVA treats Winograd point selection as continuous optimization, searches R^n-1 via Evolution Strategy, snaps candidates to simple rationals, and guarantees correctness via symbolic verification, discovering stable fractional configurations like {±5/6, ±7/6, ±3/5}.

Result: NOVA improves conditioning of F(8,3) by 415x in 1D (172,484x for 2D), restores full FP16 ImageNet accuracy (75-78% vs 4.7% with standard transforms), recovering over 70 percentage points without retraining or calibration.

Conclusion: NOVA's discovered transforms act as drop-in replacements, unlocking large-tile Winograd convolution efficiency for next-generation hardware by breaking the integer interpolation convention and solving numerical instability.
...
### [255] [Machine Unlearning in the Era of Quantum Machine Learning: An Empirical Study](https://arxiv.org/abs/2512.19253)
*Carla Crivoi,Radu Tudor Ionescu*

Main category: cs.LG

TL;DR: First comprehensive empirical study of machine unlearning in hybrid quantum-classical neural networks, showing quantum models can support effective unlearning with performance depending on circuit depth, entanglement, and task complexity.


<details>
  <summary>Details</summary>
Motivation: Machine unlearning has been extensively explored in classical deep learning, but its behavior within variational quantum circuits and quantum-augmented architectures remains largely unexplored, creating a knowledge gap as quantum machine learning systems expand.
...
### [256] [KerJEPA: Kernel Discrepancies for Euclidean Self-Supervised Learning](https://arxiv.org/abs/2512.19605)
*Eric Zimmermann,Harley Wiltzer,Justin Szeto,David Alvarez-Melis,Lester Mackey*

Main category: cs.LG

TL;DR: KerJEPAs: New family of self-supervised learning algorithms with kernel-based regularizers that improve training stability and design flexibility over existing JEPA methods.


<details>
  <summary>Details</summary>
Motivation: Recent JEPA methods show that regularizing Euclidean representations toward isotropic Gaussian priors improves training stability and generalization. The authors aim to develop more flexible regularization approaches using kernel methods.

Method: Introduce KerJEPAs - self-supervised learning algorithms with kernel-based regularizers. Expand class of viable kernels and priors, compute closed-form high-dimensional limit of sliced MMDs. One instance corresponds to LeJEPA Epps-Pulley regularizer which approximates sliced MMD with Gaussian prior/kernel.

Result: Develop alternative KerJEPAs with favorable properties including improved training stability and design flexibility compared to existing JEPA approaches.

Conclusion: KerJEPAs provide a more flexible family of self-supervised learning algorithms with kernel-based regularization that offers advantages over current JEPA methods in terms of stability and design options.
...
### [257] [SecureCode v2.0: A Production-Grade Dataset for Training Security-Aware Code Generation Models](https://arxiv.org/abs/2512.18542)
*Scott Thornton*

Main category: cs.CR

TL;DR: SecureCode v2.0 is a production-grade dataset of 1,215 security-focused coding examples with CVE references, covering 11 vulnerability categories across 11 languages, featuring vulnerable/secure implementations, attack demonstrations, and operational security guidance.


<details>
  <summary>Details</summary>
Motivation: AI assistants produce vulnerable code in 45% of security-relevant scenarios, but existing secure coding datasets lack incident grounding, scale, and operational security context needed for production deployments.

Method: Created a dataset of 1,215 examples that passed structural validation and expert security review, each tied to actual security incidents with CVE references. Uses a 4-turn conversational structure mirroring developer-AI interactions, includes SIEM integration strategies, infrastructure hardening recommendations, and testing approaches.

Result: Dataset covers 11 vulnerability categories (OWASP Top 10:2025 plus AI/ML Security Threats) across 11 languages, with 989 training, 122 validation, and 104 test examples. Includes automated validation framework and comprehensive operational security guidance.

Conclusion: SecureCode v2.0 addresses critical gaps in secure coding datasets by providing incident-grounded, production-ready examples with operational security context, enabling better training of AI assistants and improving security outcomes in production systems.
...
### [258] [Efficient Jailbreak Mitigation Using Semantic Linear Classification in a Multi-Staged Pipeline](https://arxiv.org/abs/2512.19011)
*Akshaj Prashanth Rao,Advait Singh,Saumya Kumaar Saksena,Dhruv Kumar*

Main category: cs.CR

TL;DR: A lightweight multi-stage defense pipeline using semantic filtering with TF-IDF and Linear SVM achieves 93.4% accuracy against prompt injection/jailbreaking attacks with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Prompt injection and jailbreaking attacks pose persistent security challenges to LLM-based systems, requiring efficient and robust defenses that address limitations of current model-based moderators.

Method: Multi-stage defense pipeline with core semantic filter using text normalization, TF-IDF representations, and Linear SVM classifier, complemented by additional detection/mitigation mechanisms at successive stages.

Result: Achieves 93.4% accuracy and 96.5% specificity on held-out data; improves overall accuracy from 35.1% to 93.4% while reducing average time from ~450s to 47s (10x lower latency than ShieldGemma).

Conclusion: Staged, resource-efficient defenses can robustly secure modern LLM applications, simultaneously advancing defensive precision and efficiency against prompt injection/jailbreaking attacks.
...
### [259] [From Retrieval to Reasoning: A Framework for Cyber Threat Intelligence NER with Explicit and Adaptive Instructions](https://arxiv.org/abs/2512.19414)
*Jiaren Peng,Hongda Sun,Xuan Tian,Cheng Huang,Zeqing Li,Rui Yan*

Main category: cs.CR

TL;DR: TTPrompt framework replaces retrieval-based CTI NER with explicit instruction hierarchy based on TTP concepts, using feedback-driven refinement to adapt to annotation dialects, achieving performance rivaling full fine-tuning with only 1% data.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based CTI NER relies on retrieval-based ICL which succeeds due to incidental entity type overlap rather than true semantic understanding, exposing limitations of unreliable implicit induction.

Method: TTPrompt maps CTI's TTP concepts into instruction hierarchy: Tactics as task definitions, Techniques as guiding strategies, Procedures as annotation guidelines. Introduces Feedback-driven Instruction Refinement (FIR) for LLMs to self-refine guidelines by learning from errors on minimal labeled data.

Result: Outperforms retrieval-based baselines on five CTI NER benchmarks. With refinement on just 1% training data, rivals models fine-tuned on full dataset: 71.96% Micro F1 on LADDER, and 10.91% higher Macro F1 than fine-tuned ACLM on complex CTINexus.

Conclusion: Explicit instruction hierarchy with feedback-driven refinement provides more reliable CTI NER than retrieval-based approaches, achieving near-fine-tuning performance with minimal labeled data through self-adaptive guideline refinement.
...
### [260] [Explainable Transformer-CNN Fusion for Noise-Robust Speech Emotion Recognition](https://arxiv.org/abs/2512.18298)
*Sudip Chakrabarty,Pappu Bishwas,Rajdeep Chatterjee*

Main category: cs.SD

TL;DR: A Hybrid Transformer-CNN framework for robust Speech Emotion Recognition that combines Wav2Vec 2.0's contextual modeling with 1D-CNN's spectral stability, achieving state-of-the-art accuracy while providing explainable AI insights through SHAP and Score-CAM visualizations.


<details>
  <summary>Details</summary>
Motivation: Speech Emotion Recognition systems degrade in real-world noisy environments, and the opacity of deep learning models hinders adoption in trust-sensitive applications. There's a need for both robustness to acoustic interference and model interpretability.

Method: Dual-stream architecture: one stream uses Wav2Vec 2.0 for long-range temporal dependencies, another uses 1D-CNNs with spectral features (MFCC, ZCR, RMSE) and custom Attentive Temporal Pooling for noise resistance. Integrated SHAP and Score-CAM for explainability.

Result: Superior generalization and state-of-the-art accuracy across four benchmark datasets (RAVDESS, TESS, SAVEE, CREMA-D). Significantly outperforms single-branch baselines under realistic environmental interference using SAS-KIIT noise profiles.

Conclusion: The hybrid framework effectively bridges the gap between performance and interpretability, demonstrating robust emotion recognition in noisy environments while providing transparent decision-making through visual explanations of attention shifts between temporal and spectral cues.
...
### [261] [Pushing the Frontier of Audiovisual Perception with Large-Scale Multimodal Correspondence Learning](https://arxiv.org/abs/2512.19687)
...
TL;DR: PE-AV is a new family of multimodal encoders trained with scaled contrastive learning that creates unified embeddings across audio, video, and text modalities, enabling novel cross-modal tasks and setting new SOTA on audio/video benchmarks.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of single-domain audio models and create unified cross-modal representations that can handle diverse audio types (speech, music, sound effects) and enable novel multimodal tasks like speech retrieval.

Method: Built on PE foundation, uses scaled contrastive learning with 10 pairwise contrastive objectives across audio-video, audio-text, and video-text modalities. Created a strong audiovisual data engine generating high-quality captions for ~100M audio-video pairs. Also developed PE-A-Frame variant with frame-level contrastive objectives for fine-grained alignment.

Result: Sets new state-of-the-art across standard audio and video benchmarks. Enables novel tasks like speech retrieval through unified cross-modal embeddings. Shows that scaling cross-modality and caption-type pairs strengthens alignment and improves zero-shot performance.

Conclusion: PE-AV demonstrates that large-scale multimodal contrastive learning with diverse audio data and multiple pairwise objectives creates powerful unified representations that advance both standard benchmarks and enable new cross-modal applications.
...
### [262] [Deep Learning for Primordial $B$-mode Extraction](https://arxiv.org/abs/2512.19577)
*Eric Guzman,Joel Meyers*

Main category: astro-ph.CO

TL;DR: Deep learning approach (ResUNet-CMB) for estimating and removing multiple sources of secondary B-mode polarization to improve constraints on primordial gravitational waves from CMB data.


<details>
  <summary>Details</summary>
Motivation: Secondary B-mode polarization (from lensing, patchy reionization, cosmic polarization rotation) is becoming the bottleneck for detecting primordial gravitational waves in CMB surveys, as foreground removal and noise reduction improve.

Method: Uses deep learning network ResUNet-CMB to provide simultaneous estimates of multiple secondary B-mode sources, enabling their removal from CMB maps for cleaner primordial signal extraction.

Result: Demonstrates how deep learning can estimate and remove multiple secondary B-mode sources, and integrates this into likelihood analysis for nearly optimal, unbiased estimates of primordial gravitational wave amplitude.

Conclusion: Deep learning techniques offer a promising solution to the secondary B-mode bottleneck, enabling improved constraints on primordial gravitational waves as CMB surveys become more sensitive.
...
### [263] [Distributed Asymmetric Allocation: A Topic Model for Large Imbalanced Corpora in Social Sciences](https://arxiv.org/abs/2512.18119)
*Kohei Watanabe*

Main category: stat.ME

TL;DR: DAA (Distributed Asymmetric Allocation) is a new topic model that improves upon LDA by being faster, avoiding topic fragmentation in short documents, and better identifying specific topics using seed words.


<details>
  <summary>Details</summary>
Motivation: Social scientists struggle with LDA's limitations: slow fitting on large corpora, topic fragmentation in short documents, and poor performance of semi-supervised LDA with seed words.

Method: Developed DAA that integrates multiple algorithms to efficiently identify sentences about important topics in large corpora. Evaluated on UN General Assembly speech transcripts (1991-2017).

Result: DAA classifies sentences significantly more accurately and quickly than LDA. Shows importance of optimizing Dirichlet priors in LDA for accurate content analysis.

Conclusion: DAA solves key LDA limitations and demonstrates the importance of proper Dirichlet prior optimization for social science text analysis.
...
### [264] [A curated UK rain radar data set for training and benchmarking nowcasting models](https://arxiv.org/abs/2512.17924)
*Viv Atureta,Rifki Priansyah Jasin,Stefan Siegert*

Main category: physics.ao-ph

TL;DR: The paper introduces a UK rain radar dataset for nowcasting, containing 1,000 sequences of 2D radar images with atmospheric features, plus R tools for data extraction and a CNN case study.


<details>
  <summary>Details</summary>
Motivation: To provide a standardized dataset for statistical modeling and machine learning in precipitation nowcasting, addressing the need for accessible radar data with relevant atmospheric features for research and development.

Method: Created a dataset of 1,000 radar sequences (20 steps at 15-min intervals, 40x40 pixels at 5km resolution) using spatially stratified sampling. Developed R functions to extract data from Nimrod binary format and included atmospheric/geographic features. Demonstrated utility with a CNN nowcasting case study.

Result: A publicly available dataset with radar sequences, atmospheric features, and R tools for data extraction. The case study shows the dataset's utility for training and evaluating machine learning models for precipitation nowcasting.

Conclusion: The paper provides a valuable resource for nowcasting research with a comprehensive radar dataset, extraction tools, and demonstration of practical application through a CNN case study, facilitating advancement in precipitation forecasting methods.
...
### [265] [Toward Training Superintelligent Software Agents through Self-Play SWE-RL](https://arxiv.org/abs/2512.18552)
*Yuxiang Wei,Zhiqing Sun,Emily McMilin,Jonas Gehring,David Zhang,Gabriel Synnaeve,Daniel Fried,Lingming Zhang,Sida Wang*

Main category: cs.SE

TL;DR: SSR is a self-play RL method for training superintelligent software agents that autonomously inject and repair bugs in real codebases without human-labeled data, outperforming human-data baselines on SWE benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based software agents rely heavily on human-curated training data (GitHub issues, tests) and environments, creating a fundamental barrier to achieving superintelligence. The paper aims to develop training paradigms that minimize human dependency and enable autonomous learning from real-world software repositories.

Method: Self-play SWE-RL (SSR) uses a single LLM agent trained via reinforcement learning in a self-play setting. The agent iteratively injects and repairs software bugs of increasing complexity in sandboxed repositories with source code and dependencies. Bugs are formally specified by test patches rather than natural language descriptions, requiring minimal data assumptions.

Result: On SWE-bench Verified and SWE-Bench Pro benchmarks, SSR achieves significant self-improvement (+10.4 and +7.8 points respectively) and consistently outperforms human-data baselines throughout training, even when evaluated on natural language issues not seen during self-play.

Conclusion: SSR demonstrates a promising path toward superintelligent software agents that can autonomously gather extensive learning experiences from real-world repositories, potentially exceeding human capabilities in system understanding, novel problem-solving, and autonomous software creation.
...
### [266] [Code2Doc: A Quality-First Curated Dataset for Code Documentation](https://arxiv.org/abs/2512.18748)
*Recep Kaan Karaman,Meftun Akarsu*

Main category: cs.SE

TL;DR: Code2Doc is a high-quality curated dataset for code documentation generation, addressing data quality issues in existing datasets through rigorous filtering and curation across 5 programming languages.


<details>
  <summary>Details</summary>
Motivation: Existing code documentation datasets suffer from noisy documentation, extensive duplication, and AI-generated content contamination, which weakens supervision signals for learning-based models and complicates evaluation.

Method: Four-stage curation pipeline: 1) enforce documentation completeness and clarity, 2) filter functions based on structural/complexity criteria, 3) remove exact and near-duplicate code, 4) identify AI-generated documentation. Applied to 52,069 candidates from open-source repositories in Python, Java, TypeScript, JavaScript, and C++.

Result: Final dataset of 13,358 high-quality function-documentation pairs (25.6% of original candidates). Mean documentation quality score: 6.93/10. 86.9% contain explicit type annotations, only 2.9% flagged as potentially AI-generated. Fine-tuning on Code2Doc yields 29.47% BLEU and 24.04% ROUGE-L improvements over zero-shot performance.

Conclusion: Code2Doc demonstrates that quality-first curation significantly improves code documentation generation performance despite modest dataset size. The dataset and curation pipeline are released to support reproducible research in automatic code documentation generation.
...
### [267] [BanglaForge: LLM Collaboration with Self-Refinement for Bangla Code Generation](https://arxiv.org/abs/2512.19122)
*Mahir Labib Dihan,Sadif Ahmed,Md Nafiu Rahman*

Main category: cs.SE

TL;DR: BanglaForge is a novel framework for generating code from Bangla function descriptions using retrieval-augmented dual-model collaboration with self-refinement, achieving 84.00% Pass@1 accuracy on BLP-2025 benchmark.


<details>
  <summary>Details</summary>
Motivation: Bangla is a low-resource language for code generation, lacking large-scale annotated datasets and tools to transform natural language specifications into executable programs, making Bangla-to-code generation challenging.

Method: BanglaForge uses retrieval-augmented dual-model collaboration with self-refinement, combining in-context learning, LLM-based translation, systematic prompt engineering, and iterative self-refinement based on execution feedback. It employs a coder model to generate initial solutions and a reviewer model to enhance them for robustness.

Result: On the BLP-2025 Bangla Code Generation benchmark, BanglaForge achieves a competitive Pass@1 accuracy of 84.00%.

Conclusion: The framework demonstrates the effectiveness of retrieval, model collaboration, and self-refinement for low-resource Bangla code generation, providing a solution to the challenges of Bangla-to-code generation with limited resources.
...
### [268] [Layout-Aware Text Editing for Efficient Transformation of Academic PDFs to Markdown](https://arxiv.org/abs/2512.18115)
*Changxu Duan*

Main category: cs.MM

TL;DR: EditTrans: A hybrid editing-generation model that reduces PDF-to-markup transformation latency by 44.5% by identifying editable text from PDFs before generation instead of regenerating all text from scratch.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end decoder transformer models for converting PDF documents to markup languages are inefficient because they regenerate all text token-by-token from scratch, including dense text that could be directly copied from PDF files, wasting inference steps and increasing latency.

Method: EditTrans is a hybrid editing-generation model that first identifies a queue of to-be-edited text from PDF files before generating markup language. It uses a lightweight classifier fine-tuned from a Document Layout Analysis model on 162,127 pages of arXiv documents.

Result: EditTrans reduced transformation latency up to 44.5% compared to end-to-end decoder transformer models while maintaining transformation quality. The code and reproducible dataset production scripts are open-sourced.

Conclusion: The hybrid editing-generation approach of EditTrans provides significant efficiency improvements for PDF-to-markup transformation by avoiding unnecessary regeneration of text that can be directly copied from source PDFs, making academic content transformation more scalable and efficient.
...
### [269] [Asynchronous Pipeline Parallelism for Real-Time Multilingual Lip Synchronization in Video Communication Systems](https://arxiv.org/abs/2512.18318)
*Eren Caglar,Amirkia Rafiei Oskooei,Mehmet Kutanoglu,Mustafa Keles,Mehmet S. Aktas*

Main category: cs.MM

TL;DR: Parallel asynchronous Transformer framework for real-time multilingual lip sync in video conferencing, achieving 3.1x latency reduction through pipeline parallelism and hardware optimizations.


<details>
  <summary>Details</summary>
Motivation: Need for efficient, low-latency multilingual lip synchronization in real-time video conferencing systems, especially for resource-constrained IoT scenarios like telemedicine and multilingual kiosks.

Method: Pipeline-parallel architecture with message-queue decoupling, integrating translation, speech processing, and lip-sync modules; optimized with graph compilation, mixed-precision quantization, kernel fusion, and context-adaptive silence detection.

Result: 3.1x end-to-end latency reduction compared to sequential approaches, improved processing speed, synchronization stability, and resource utilization while preserving accuracy and visual quality.

Conclusion: The parallel asynchronous framework advances low-latency multimodal communication for AIoT systems, with applications in telemedicine, multilingual kiosks, and remote assistance.
...
### [270] [Merge on workspaces as Hopf algebra Markov chain](https://arxiv.org/abs/2512.18861)
*Matilde Marcolli,David Skigin*

Main category: math.DS

TL;DR: A Hopf algebra Markov chain models syntax formation via Merge operations, showing that standard linguistic cost functions fail to achieve convergence to tree structures, while Shannon entropy optimization succeeds.


<details>
  <summary>Details</summary>
Motivation: To mathematically model the computational process of syntax formation in Chomsky's Minimalism framework using Merge operations, and to analyze whether proposed linguistic cost functions can achieve convergence to proper tree structures.

Method: Develop a Hopf algebra Markov chain on binary rooted forests with labelled leaves, decomposing dynamics into Internal Merge (ergodic with uniform stationary distribution) and External/Sideward Merge (reduced to partition Markov chain). Analyze weighted cases using Perron-Frobenius theory and tropical semiring methods.

Result: Standard linguistic cost functions (Minimal Search and Resource Restrictions) do not achieve convergence to tree structures, while Shannon entropy optimization successfully achieves expected convergence. The dynamics can be filtered by coloring rules modeling linguistic constraints.

Conclusion: The mathematical model reveals limitations of traditional linguistic cost functions in achieving proper syntax tree formation, while entropy-based optimization provides a viable alternative. The framework allows extension to semantic embedding, parameter variation, and linguistic filtering mechanisms.
...
### [271] [CytoDINO: Risk-Aware and Biologically-Informed Adaptation of DINOv3 for Bone Marrow Cytomorphology](https://arxiv.org/abs/2512.17930)
*Aziz Muminov,Anne Pham*

Main category: q-bio.OT

TL;DR: CytoDINO: A parameter-efficient framework for bone marrow cell classification using DINOv3 with hierarchical focal loss and critical misclassification penalties, achieving SOTA performance on consumer hardware.


<details>
  <summary>Details</summary>
Motivation: Current bone marrow cytomorphology analysis is labor-intensive and subjective, while existing computational models are resource-heavy and don't account for the asymmetric risks of clinical misdiagnosis (e.g., misclassifying malignant cells as normal).

Method: Fine-tunes DINOv3 using Low-Rank Adaptation (LoRA) for parameter efficiency, and introduces a novel Hierarchical Focal Loss with Critical Penalties that encodes biological relationships between cell lineages and heavily penalizes clinically dangerous misclassifications.

Result: Achieves 88.2% weighted F1 and 76.5% macro F1 on 21 cell classes using only 8% trainable parameters on consumer-grade hardware (NVIDIA RTX 5080). Confidence-based selective prediction yields 99.5% accuracy on 67% of samples.

Conclusion: CytoDINO demonstrates that parameter-efficient fine-tuning on consumer hardware can match specialized infrastructure while addressing critical clinical risks, providing a viable pathway for clinical deployment with expert review of high-uncertainty cases.
...
### [272] [Disentangled representations via score-based variational autoencoders](https://arxiv.org/abs/2512.17127)
*Benjamin S. H. Lyo,Eero P. Simoncelli,Cristina Savin*

Main category: stat.ML

TL;DR: SAMI combines diffusion models and VAEs for unsupervised representation learning, automatically capturing meaningful structure in data without supervision.


<details>
  <summary>Details</summary>
Motivation: To create an unsupervised representation learning method that makes implicit structural information in diffusion models explicit and interpretable by combining them with variational autoencoders.

Method: Unifies evidence lower bounds of diffusion models and VAEs to formulate a principled objective that learns representations through score-based guidance of the diffusion process.

Result: SAMI recovers ground truth generative factors in synthetic data, learns factorized semantic latent dimensions from natural images, encodes video sequences into straighter latent trajectories from static images, and extracts useful representations from pre-trained diffusion models with minimal additional training.

Conclusion: Implicit structural information in diffusion models can be made explicit and interpretable through synergistic combination with a variational autoencoder, providing mathematically exact ways to identify semantically meaningful axes without supervised labels.
...
### [273] [A Critical Review of Monte Carlo Algorithms Balancing Performance and Probabilistic Accuracy with AI Augmented Framework](https://arxiv.org/abs/2512.17968)
*Ravi Prasad*

Main category: stat.CO

TL;DR: This paper provides a critical analysis of Monte Carlo algorithm evolution, focusing on the trade-off between statistical efficiency and computational cost, from Metropolis-Hastings to Hamiltonian Monte Carlo, with complexity analysis and practical guidance.


<details>
  <summary>Details</summary>
Motivation: To understand the performance trade-offs in Monte Carlo algorithms and provide a comprehensive analysis of their evolution, focusing on the tension between statistical efficiency and computational cost.

Method: Critical survey approach analyzing historical development from Metropolis-Hastings to Hamiltonian Monte Carlo, with rigorous discussion of time/space complexity bounds, examination of motivations for method development, and analysis of key theoretical/practical observations.

Result: Provides complexity bounds for major algorithm classes, identifies situations where specific algorithms are superior, and documents the evolution from basic methods to advanced techniques like gradient-informed and adaptive HMC.

Conclusion: Monte Carlo algorithms have profoundly impacted computational science, with ongoing challenges in balancing efficiency and cost, and the paper provides a framework for algorithm selection while highlighting current research frontiers.
...
### [274] [SLIM: Semantic-based Low-bitrate Image compression for Machines by leveraging diffusion](https://arxiv.org/abs/2512.18200)
*Hyeonjin Lee,Jun-Hyuk Kim,Jong-Seok Lee*

Main category: eess.IV

TL;DR: SLIM is a semantic-based low-bitrate image compression framework for machine vision that uses diffusion models to focus on ROI areas without needing mask guidance at inference.
...
Method: Uses pretrained latent diffusion model. Compressor focuses only on Region-of-Interest (RoI) areas in image latent space. Pretrained Unet enhances decompressed latent using RoI-focused text captions containing semantic information. Works without guide masks at inference.

Result: Achieves higher classification accuracy at same bits per pixel compared to conventional image compression models for machines. Enables low bitrate compression while maintaining both machine vision optimization and human perceptual details.

Conclusion: SLIM provides an effective training framework for machine vision image compression that achieves better performance at lower bitrates by leveraging semantic information and diffusion models.
...
### [275] [Selective Phase-Aware Training of nnU-Net for Robust Breast Cancer Segmentation in Multi-Center DCE-MRI](https://arxiv.org/abs/2512.19225)
*Beyza Zayim,Aissiou Ikram,Boukhiar Naima*

Main category: eess.IV

TL;DR: A selective, phase-aware training framework for nnU-Net improves breast tumor segmentation in DCE-MRI by focusing on quality data selection, showing that training on cleaner early-phase images outperforms using artifact-prone data.


<details>
  <summary>Details</summary>
Motivation: Breast cancer diagnosis lacks standardized benchmarks for DCE-MRI analysis. The MAMA-MIA Challenge aims to address this gap, but heterogeneous clinical datasets with varying image quality and motion artifacts challenge segmentation performance.

Method: Used nnU-Net framework with selective training strategy that analyzes image quality and center-specific variability. Focused on early phase images (0000-0002) from cleaner datasets (DUKE, NACT) while excluding ISPY scans with motion artifacts and reduced contrast.

Result: Training on DUKE and NACT data with clear contrast and fewer artifacts produced more stable segmentation performance. Including ISPY scans with artifacts impaired performance even with advanced preprocessing like CLAHE.

Conclusion: Phase-sensitive and quality-aware training strategies are crucial for reliable segmentation in heterogeneous clinical datasets. Naive dataset expansion is insufficient, motivating future automation of quality-based data selection.
...
### [276] [Rethinking Coupled Tensor Analysis for Hyperspectral Super-Resolution: Recoverable Modeling Under Endmember Variability](https://arxiv.org/abs/2512.19489)
*Meng Ding,Xiao Fu*
...
<details>
  <summary>Details</summary>
Motivation: Existing tensor decomposition methods for hyperspectral super-resolution either lack physical interpretability (CPD, Tucker) or rely on overly restrictive linear mixture model assumptions (LL1) that don't account for real-world nonlinear effects like endmember variability.

Method: Introduces block-term tensor decomposition with rank-(L_r, M_r, N_r) terms (LMN model) that generalizes CPD, Tucker, and LL1 decompositions. This flexible model retains interpretability while accommodating nonlinear effects like endmember variability in spectral images.

Result: The LMN model provides theoretical recoverability guarantees for super-resolution images under proper conditions. Experiments on synthetic and real datasets show improved effectiveness and robustness compared to existing coupled tensor decomposition methods.

Conclusion: The proposed LMN model offers a balanced tradeoff between expressiveness and interpretability for hyperspectral super-resolution, addressing limitations of existing tensor decomposition approaches while maintaining theoretical guarantees and practical effectiveness.
...
### [277] [Patlak Parametric Image Estimation from Dynamic PET Using Diffusion Model Prior](https://arxiv.org/abs/2512.19584)
*Ziqian Huang,Boxiao Yu,Siqi Li,Savas Ozdemir,Sangjin Bae,Jae Sung Lee,Guobao Wang,Kuang Gong*

Main category: eess.IV

TL;DR: A diffusion model-based framework improves parametric PET image quality by using pre-trained static PET priors and kinetic model constraints during inference.


<details>
  <summary>Details</summary>
Motivation: Parametric PET images from kinetic modeling suffer from poor quality due to ill-posed fitting and limited counts from non-continuous whole-body acquisition, requiring better reconstruction methods.

Method: Proposed diffusion model-based kinetic modeling framework using Patlak model as example. Pre-trained score function on static PET images serves as prior for parametric images, with kinetic model incorporated as data-consistency constraint during inference.
...
Conclusion: Diffusion model-based approach with pre-trained priors and kinetic model constraints effectively enhances parametric PET image reconstruction quality.
...
### [278] [TICL+: A Case Study On Speech In-Context Learning for Children's Speech Recognition](https://arxiv.org/abs/2512.18263)
...
<details>
  <summary>Details</summary>
Motivation: Children's speech recognition faces challenges due to acoustic/linguistic variability, limited labeled data, and differences from adult speech. Speech foundation models can help through Speech In-Context Learning (SICL), but effectiveness depends on how in-context examples are selected.
...
Conclusion: Combining semantic and acoustic information is valuable for robust, scalable ASR in children's speech, demonstrating the importance of multimodal example selection in SICL.
...
### [279] [External Hippocampus: Topological Cognitive Maps for Guiding Large Language Model Reasoning](https://arxiv.org/abs/2512.18190)
*Jian Yan*

Main category: cs.AI

TL;DR: The External Hippocampus framework models LLM reasoning as energy flow in semantic space, using topological cognitive maps to guide reasoning without additional training, solving cognitive deadlock in small models.


<details>
  <summary>Details</summary>
Motivation: To address the cognitive deadlock problem in multi-step reasoning for small language models (≤7B parameters) without requiring substantial computational resources or additional training.
...
Result: Achieves 81.20% accuracy on 500 challenging problems (+16.80% relative baseline), reduces reasoning time by ≥15x, identifies reasoning stagnation as "Cognitive Vortex" and low-entropy potential wells, and shows temperature perturbations effectively restart energy flow.

Conclusion: The framework provides an efficient, controllable topological-aware solution for small model reasoning with no additional training, autonomous growth capability, and predictable intervention patterns.
...
### [280] [NEURO-GUARD: Neuro-Symbolic Generalization and Unbiased Adaptive Routing for Diagnostics -- Explainable Medical AI](https://arxiv.org/abs/2512.18177)
*Midhat Urooj,Ayan Banerjee,Sandeep Gupta*

Main category: cs.AI

TL;DR: NEURO-GUARD is a knowledge-guided vision framework that combines Vision Transformers with language-driven reasoning for interpretable medical image diagnosis, improving accuracy by 6.2% and domain generalization by 5% over ViT-only baselines.


<details>
  <summary>Details</summary>
Motivation: Current medical AI models are black-box systems with limited interpretability and poor cross-domain generalization, hindering clinical adoption. There's a need for accurate yet interpretable image-based diagnosis, especially in data-limited settings with subtle visual cues and high-stakes decisions.

Method: NEURO-GUARD integrates Vision Transformers with language-driven reasoning using retrieval-augmented generation (RAG). An LLM iteratively generates, evaluates, and refines feature-extraction code for medical images, grounded in clinical guidelines and expert knowledge to enhance feature detection and classification.

Result: On diabetic retinopathy classification across four datasets (APTOS, EyePACS, Messidor-1, Messidor-2), NEURO-GUARD improves accuracy by 6.2% over ViT-only baseline (84.69% vs. 78.4%) and achieves 5% gain in domain generalization. MRI-based seizure detection evaluations further confirm cross-domain robustness, consistently outperforming existing methods.

Conclusion: NEURO-GUARD successfully bridges symbolic medical reasoning with subsymbolic visual learning, enabling interpretable, knowledge-aware, and generalizable medical image diagnosis while achieving state-of-the-art performance across multiple datasets.
...
### [281] [Agent-Based Output Drift Detection for Breast Cancer Response Prediction in a Multisite Clinical Decision Support System](https://arxiv.org/abs/2512.18450)
*Xavier Rafael-Palou,Jose Munuera,Ana Jimenez-Pastor,Richard Osuala,Karim Lekadir,Oliver Diaz*

Main category: cs.AI

TL;DR: Proposes agent-based framework for detecting distributional drift in multi-site clinical AI systems, showing decentralized monitoring outperforms centralized approaches.


<details>
  <summary>Details</summary>
Motivation: Clinical AI systems serving multiple medical imaging sites suffer performance degradation due to distributional shifts across sites (patient populations, hardware, protocols). Existing centralized monitoring fails to capture site-specific drift dynamics.

Method: Agent-based framework where each site has a drift monitoring agent performing batch-wise comparisons of model outputs against reference distributions. Evaluates four multi-center schemes: site-specific, global, production-only, and adaptive references, plus centralized baseline.

Result: All multi-center schemes outperform centralized monitoring, with up to 10.3% F1-score improvement in drift detection. Adaptive scheme performs best without site-specific references (74.3% F1 for detection, 83.7% for severity classification).

Conclusion: Adaptive, site-aware agent-based drift monitoring enhances reliability of multisite clinical decision support systems by better detecting and classifying distributional shifts.
...
### [282] [ESearch-R1: Learning Cost-Aware MLLM Agents for Interactive Embodied Search via Reinforcement Learning](https://arxiv.org/abs/2512.18571)
...
TL;DR: ESearch-R1 is a cost-aware embodied reasoning framework that uses HC-GRPO to optimize MLLM agents for balancing physical exploration costs against human interaction costs when facing ambiguous instructions.
...
Conclusion: ESearch-R1 validates the effectiveness of GRPO in aligning MLLM agents with physical world constraints, providing a framework for cost-aware embodied reasoning that balances exploration and interaction costs.
...
### [283] [Affordance RAG: Hierarchical Multimodal Retrieval with Affordance-Aware Embodied Memory for Mobile Manipulation](https://arxiv.org/abs/2512.18987)
*Ryosuke Korekata,Quanting Xie,Yonatan Bisk,Komei Sugiura*

Main category: cs.RO

TL;DR: Affordance RAG: A zero-shot hierarchical multimodal retrieval framework for open-vocabulary mobile manipulation that uses affordance-aware embodied memory to help robots carry objects to receptacles based on natural language instructions.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary mobile manipulation is challenging because robots need to understand both visual semantics and manipulation affordances to successfully carry objects to receptacles based on free-form natural language instructions in real-world environments.

Method: Proposes Affordance RAG, a zero-shot hierarchical multimodal retrieval framework that constructs Affordance-Aware Embodied Memory from pre-explored images. It retrieves candidate targets based on regional and visual semantics, then reranks them with affordance scores to identify executable manipulation options.

Result: Outperformed existing approaches in retrieval performance for mobile manipulation instructions in large-scale indoor environments. Achieved 85% task success rate in real-world experiments where robots performed mobile manipulation based on free-form instructions.

Conclusion: The proposed Affordance RAG framework effectively addresses open-vocabulary mobile manipulation by combining visual semantics understanding with affordance-aware reasoning, demonstrating superior performance in both retrieval and real-world task execution compared to existing methods.
...
### [284] [Robotic VLA Benefits from Joint Learning with Motion Image Diffusion](https://arxiv.org/abs/2512.18007)
...
TL;DR: VLA models enhanced with motion image diffusion through dual-head design (action + motion heads) to improve motion reasoning without affecting inference latency.


<details>
  <summary>Details</summary>
Motivation: Current VLA models mimic expert trajectories without predictive motion reasoning, limiting their ability to reason about what actions to take. They lack physical understanding of future dynamics.

Method: Extends VLA architecture with dual-head design: action head predicts action chunks, motion head (Diffusion Transformer) predicts optical-flow-based motion images capturing future dynamics. Joint training enables shared VLM backbone to learn representations coupling robot control with motion knowledge.

Result: Improves success rate of pi-series VLAs to 97.5% on LIBERO benchmark and 58.0% on RoboTwin benchmark, yielding 23% improvement in real-world performance. Validates effectiveness in enhancing motion reasoning capability.

Conclusion: Joint learning with motion image diffusion enhances VLA models' motion reasoning capabilities while maintaining test-time latency, building temporally coherent and physically grounded representations without modifying standard VLA inference pathways.
...
### [285] [Embodied4C: Measuring What Matters for Embodied Vision-Language Navigation](https://arxiv.org/abs/2512.18028)
*Tin Stribor Sohn,Maximilian Dillitzer,Jason J. Corso,Eric Sax*

Main category: cs.RO

TL;DR: Embodied4C is a new benchmark evaluating vision-language models' embodied reasoning across autonomous vehicles, drones, and robotic manipulators through 1.1K reasoning questions and 58 navigation tasks.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks provide limited understanding of how embodiment (physical platform, sensor configuration, modality alignment) influences perception, reasoning, and control in vision-language navigation. There's a need to systematically evaluate how different embodiments affect VLM performance.

Method: Introduced Embodied4C benchmark with three heterogeneous embodiments (autonomous vehicles, aerial drones, robotic manipulators). Uses approximately 1.1K one-shot reasoning questions and 58 goal-directed navigation tasks across four dimensions: semantic, spatial, temporal, and physical reasoning. Includes dynamic sensor configurations, environment variations, and domain-far queries to prevent embodiment overfitting.

Result: Evaluation of ten state-of-the-art VLMs and four embodied control baselines shows cross-modal alignment and instruction tuning matter more than scale. Spatial and temporal reasoning remain the primary bottleneck for reliable embodied competence.

Conclusion: Embodied4C provides a comprehensive Turing test for embodied reasoning, revealing that current VLMs struggle with spatial and temporal reasoning despite scale advantages, highlighting the importance of cross-modal alignment and instruction tuning over pure model scaling.
...
### [286] [STORM: Search-Guided Generative World Models for Robotic Manipulation](https://arxiv.org/abs/2512.18477)
*Wenjun Lin,Jensen Zhang,Kaitong Cai,Keze Wang*

Main category: cs.RO

TL;DR: STORM is a search-guided generative world model framework for robotic manipulation that combines diffusion-based action generation, video prediction, and MCTS planning to achieve state-of-the-art performance on manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: Prior Vision-Language-Action models rely on abstract latent dynamics or language-based reasoning, lacking explicit visual grounding for planning. There's a need for interpretable, foresight-driven decision-making in robotic manipulation through explicit visual rollouts.

Method: Three-component framework: 1) Diffusion-based VLA policy proposes diverse candidate actions, 2) Generative video world model simulates visual and reward outcomes, 3) Monte Carlo Tree Search selectively refines plans through lookahead evaluation.

Result: Achieves new SOTA average success rate of 51.0% on SimplerEnv benchmark, outperforming CogACT. Reward-augmented video prediction reduces Frechet Video Distance by over 75%. Demonstrates robust re-planning and failure recovery behavior.

Conclusion: STORM demonstrates the advantages of search-guided generative world models for long-horizon robotic manipulation, providing interpretable planning through explicit visual rollouts and achieving superior performance through integrated action generation, simulation, and search.
...
### [287] [Offline Reinforcement Learning for End-to-End Autonomous Driving](https://arxiv.org/abs/2512.18662)
*Chihiro Noguchi,Takaki Yamamoto*

Main category: cs.RO

TL;DR: Camera-only end-to-end autonomous driving using offline RL with behavior regularization from expert trajectories, achieving better safety and performance than imitation learning baselines.


<details>
  <summary>Details</summary>
Motivation: E2E autonomous driving models using only camera inputs are efficient and have generalization potential, but suffer from persistent failure modes due to reliance on imitation learning. Online RL could help but is computationally expensive due to neural rendering and large networks.

Method: Proposes a camera-only E2E offline RL framework that trains on fixed simulator data without additional exploration. Uses pseudo ground-truth trajectories from expert driving logs as behavior regularization to suppress unsafe behavior and stabilize value learning against OOD actions.

Result: Substantial improvements in collision rate and route completion compared to imitation learning baselines, trained and evaluated in a neural rendering environment learned from nuScenes dataset.

Conclusion: Offline RL with behavior regularization offers a data-efficient and stable alternative to imitation learning for camera-only E2E autonomous driving, enabling rapid experimental iteration while improving safety and performance.
...
### [288] [WorldRFT: Latent World Model Planning with Reinforcement Fine-Tuning for Autonomous Driving](https://arxiv.org/abs/2512.19133)
*Pengxuan Yang,Ben Lu,Zhongpu Xia,Chao Han,Yinfeng Gao,Teng Zhang,Kun Zhan,XianPeng Lang,Yupeng Zheng,Qichao Zhang*

Main category: cs.RO

TL;DR: WorldRFT is a planning-oriented latent world model framework that disentangles perception from planning tasks in autonomous driving, achieving SOTA performance with 83% collision reduction on nuScenes and competitive results on NavSim.


<details>
  <summary>Details</summary>
Motivation: Current latent world models for autonomous driving use reconstruction-oriented representation learning that tangles perception with planning tasks, leading to suboptimal planning optimization.

Method: WorldRFT uses hierarchical planning decomposition and local-aware interactive refinement to align scene representation with planning, augmented by reinforcement learning fine-tuning (RFT) with Group Relative Policy Optimization (GRPO) using trajectory Gaussianization and collision-aware rewards.

Result: Achieves SOTA on nuScenes (83% collision reduction: 0.30% → 0.05%) and competitive performance on NavSim (87.8 PDMS with camera-only vs 88.1 PDMS for LiDAR-based DiffusionDrive).

Conclusion: WorldRFT successfully disentangles perception from planning through planning-oriented representation learning, demonstrating superior safety and performance in autonomous driving benchmarks.
...
### [289] [TwinAligner: Visual-Dynamic Alignment Empowers Physics-aware Real2Sim2Real for Robotic Manipulation](https://arxiv.org/abs/2512.19390)
...
<details>
  <summary>Details</summary>
Motivation: Data-driven robot learning is limited by expensive real-world data collection. While simulators offer cost-effective alternatives, the simulation-reality gap (both visual and dynamic) hinders effective policy transfer.

Method: TwinAligner uses two alignment modules: 1) Visual alignment with SDF reconstruction and editable 3DGS rendering for pixel-level alignment, 2) Dynamic alignment that identifies rigid physics from robot-object interaction for dynamic consistency.
...
Conclusion: TwinAligner enables scalable robot learning by establishing trustworthy simulation-real world cycles, accelerating algorithm development through cost-effective data collection and reliable policy transfer.
...
### [290] [Real2Edit2Real: Generating Robotic Demonstrations via a 3D Control Interface](https://arxiv.org/abs/2512.19402)
*Yujie Zhao,Hongwei Fan,Di Chen,Shengcong Chen,Liliang Chen,Xiaoqi Li,Guanghui Ren,Hao Dong*

Main category: cs.RO

TL;DR: Real2Edit2Real generates new robot demonstrations by editing 3D scene reconstructions and synthesizing multi-view videos, achieving 10-50x data efficiency compared to real demonstrations.


<details>
  <summary>Details</summary>
Motivation: Robot learning requires diverse demonstrations for spatial generalization, but collecting real-world data is costly and repetitive. There's a need to reduce data collection burden while maintaining policy robustness.

Method: 1) Reconstruct scene geometry from multi-view RGB observations using metric-scale 3D reconstruction. 2) Perform depth-reliable 3D editing on point clouds to generate new manipulation trajectories with geometrically corrected robot poses. 3) Use multi-conditional video generation guided by depth as primary control signal, along with action, edge, and ray maps to synthesize spatially augmented multi-view manipulation videos.

Result: Policies trained on data from only 1-5 source demonstrations match or outperform those trained on 50 real-world demonstrations, achieving 10-50x data efficiency improvement. Framework shows flexibility in height and texture editing, indicating potential as unified data generation framework.

Conclusion: Real2Edit2Real successfully bridges 3D editability with 2D visual data to generate diverse robot demonstrations, significantly reducing data collection costs while maintaining policy performance, with promising extensibility for broader applications.
...
### [291] [LoGoPlanner: Localization Grounded Navigation Policy with Metric-aware Visual Geometry](https://arxiv.org/abs/2512.19629)
*Jiaqi Peng,Wenzhe Cai,Yuqiang Yang,Tai Wang,Yuan Shen,Jiangmiao Pang*

Main category: cs.RO

TL;DR: LoGoPlanner is an end-to-end navigation framework that integrates localization and planning by grounding predictions in metric scale and reconstructing scene geometry from visual observations, improving generalization across robots and environments.


<details>
  <summary>Details</summary>
Motivation: Traditional modular navigation pipelines suffer from latency and cascading errors, while existing end-to-end approaches still depend on separate localization modules requiring accurate sensor calibration, limiting generalization across different robot embodiments and environments.

Method: The framework: (1) fine-tunes a visual-geometry backbone for metric-scale grounding and implicit state estimation, (2) reconstructs surrounding scene geometry from historical observations for environmental awareness, and (3) conditions the policy on implicit geometry from these auxiliary tasks to reduce error propagation.

Result: LoGoPlanner achieves more than 27.3% improvement over oracle-localization baselines, reduces cumulative error, enhances planning consistency and obstacle avoidance, and demonstrates strong generalization across different robot embodiments and environments in both simulation and real-world evaluations.

Conclusion: The fully end-to-end design with metric-aware geometry memory enables more robust navigation by integrating localization and planning, addressing limitations of previous approaches and improving performance while maintaining generalization capabilities.
...
### [292] [Multimodal LLMs for Historical Dataset Construction from Archival Image Scans: German Patents (1877-1918)](https://arxiv.org/abs/2512.19675)
*Niclas Griesshaber,Jochen Streb*

Main category: econ.GN

TL;DR: Multimodal LLMs (Gemini-2.5) used to extract 306,070 German patents (1877-1918) from 9,562 archival scans, achieving higher quality, 795x faster, and 205x cheaper than human research assistants.


<details>
  <summary>Details</summary>
Motivation: To overcome challenges in extracting structured data from complex historical documents (Gothic/Roman fonts, double-column layouts) and demonstrate the transformative potential of multimodal LLMs for economic history research.

Method: Developed an LLM-based pipeline using Gemini-2.5-Pro and Gemini-2.5-Flash-Lite to process archival image scans, extracting patent data from pages with 20-50 entries in double-column format with mixed Gothic/Roman fonts.

Result: Successfully constructed dataset of 306,070 patents from 9,562 images; benchmarking showed LLMs produced higher quality data than human research assistants while being 795x faster and 205x cheaper.

Conclusion: Multimodal LLMs represent a paradigm shift for historical dataset construction, lowering barriers for less technical researchers; the authors open-source their pipeline and datasets, speculating on transformative implications for economic history.
