...
Motivation: Traditional two-stage RAG pipelines (embedding → reranking) suffer from inefficiency due to lack of shared information between stages, leading to substantial redundant computation.

Method: Fine-tune RWKV-based LLM into EmbeddingRWKV for unified embedding/state extraction. Design state-based reranker that processes only query tokens using precomputed document states. Use uniform layer selection strategy to retain only 25% of layers while maintaining performance.

Result: Achieves 5.4×-44.8× speedup during reranking by decoupling inference cost from document length. Maintains 98.62% of full-model performance using only 25% of layers. Demonstrates high-quality retrieval and reranking with significant efficiency improvements.

Conclusion: State-Centric Retrieval provides an efficient unified paradigm for RAG systems that bridges embedding and reranking stages through reusable states, substantially reducing computational redundancy while maintaining performance.
...
TL;DR: MedES: A Chinese medical ethics benchmark and guardian-in-the-loop framework for aligning LLMs with clinical decision-making ethics, achieving strong performance with a 7B model.


<details>
  <summary>Details</summary>
Motivation: LLMs show promise in healthcare but lack proper alignment with nuanced medical ethics, especially in complex real-world scenarios and within specific cultural/legal contexts like Chinese healthcare.

Method: Created MedES benchmark from 260 authoritative Chinese medical/ethical/legal sources; developed guardian-in-the-loop framework with automated evaluator (97% accuracy) for targeted prompts and ethical feedback; aligned 7B LLM via supervised fine-tuning and domain-specific preference optimization.

Result: Aligned 7B model outperforms notably larger baselines on core ethical tasks within Chinese medical ethics context, with improvements in both quality and composite evaluation metrics.

Conclusion: Provides practical framework for aligning LLMs with medical ethics in Chinese healthcare; suggests modular approach can be adapted to other legal/cultural environments by replacing normative corpus.
...
TL;DR: LLMs show near-perfect consistency in value-based decisions but weak correspondence between self-reported and enacted values, revealing a knowledge-action gap similar to humans.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs represent and enact human values in real-world decision contexts, and to explore value alignment in AI development.

Method: Created ValAct-15k dataset with 3,000 advice-seeking scenarios from Reddit based on Schwartz Theory of Basic Human Values. Evaluated 10 frontier LLMs (5 US, 5 Chinese) and 55 human participants using both scenario-based questions and traditional value questionnaires.

Result: LLMs show near-perfect cross-model consistency (Pearson r ≈ 1.0) in scenario decisions, contrasting with human variability (r ∈ [-0.79, 0.98]). Both humans and LLMs show weak correspondence between self-reported and enacted values (r = 0.4, 0.3). LLMs' performance declines up to 6.6% when instructed to "hold" specific values.

Conclusion: Alignment training yields normative value convergence in LLMs but does not eliminate human-like incoherence between knowing and acting upon values, revealing a systematic knowledge-action gap.
...
TL;DR: AI-text detectors show poor generalization across different generation conditions, and this study uses linguistic analysis to explain why by correlating generalization gaps with specific linguistic feature shifts.


<details>
  <summary>Details</summary>
Motivation: AI-text detectors achieve high in-domain accuracy but struggle to generalize across different generation conditions (unseen prompts, models, domains). While generalization gaps are known, there's limited understanding of the underlying causes.

Method: Constructed comprehensive benchmark spanning 6 prompting strategies, 7 LLMs, and 4 domain datasets. Fine-tuned classification-based detectors on various generation settings and evaluated cross-prompt, cross-model, and cross-dataset generalization. Computed correlations between generalization accuracies and feature shifts of 80 linguistic features between training and test conditions.

Result: Generalization performance for specific detectors and evaluation conditions is significantly associated with linguistic features such as tense usage and pronoun frequency. The systematic analysis reveals how linguistic feature shifts explain performance variance across different generation conditions.

Conclusion: Linguistic analysis provides meaningful explanations for AI-text detector generalization gaps, with specific features like tense and pronoun usage correlating with performance variance across different generation conditions.
...
TL;DR: The paper introduces a three-tier evaluation framework for assessing Vision-Language Models' ability to interpret cultural meaning in art, showing automated metrics are unreliable for cultural depth and revealing Western bias in model performance.


<details>
  <summary>Details</summary>
Motivation: While VLMs excel at visual perception, their ability to interpret cultural meaning in art remains under-validated. There's a need for systematic evaluation of cultural understanding in AI models, particularly for cross-cultural art critique.

Method: A tri-tier evaluation framework: Tier I computes automated coverage and risk indicators offline; Tier II applies rubric-based scoring using a single primary judge across five dimensions; Tier III calibrates the Tier II aggregate score to human ratings via isotonic regression.

Result: The framework achieved a 5.2% reduction in MAE on a 152-sample held-out set. Evaluation of 15 VLMs on 294 expert anchors across six cultural traditions revealed: (i) automated metrics are unreliable proxies for cultural depth, (ii) Western samples score higher than non-Western samples, and (iii) cross-judge scale mismatch makes naive score averaging unreliable.

Conclusion: The proposed framework provides calibrated cultural-understanding scores for model selection and cultural-gap diagnosis, with dimension-level diagnostics and risk indicators. The approach addresses limitations of automated metrics and reveals systematic cultural biases in current VLMs.
...
TL;DR: This paper introduces a comprehensive pipeline for creating multimodal fact-checking datasets in French and German, addressing limitations of existing resources by aggregating ClaimReview feeds, scraping articles, normalizing verdicts, and enriching with structured metadata and visual content using LLMs.


<details>
  <summary>Details</summary>
Motivation: There's an urgent need for robust, up-to-date, explainable, and multilingual fact-checking resources due to rapid misinformation proliferation. Existing datasets are limited in scope, lacking multimodal evidence, structured annotations, and detailed links between claims, evidence, and verdicts.

Method: Developed a comprehensive data collection and processing pipeline that: (1) aggregates ClaimReview feeds, (2) scrapes full debunking articles, (3) normalizes heterogeneous claim verdicts, (4) enriches with structured metadata and aligned visual content, and (5) uses state-of-the-art LLMs and multimodal LLMs for evidence extraction under predefined categories and justification generation linking evidence to verdicts.

Result: Evaluation with G-Eval and human assessment demonstrates the pipeline enables fine-grained comparison of fact-checking practices across different organizations/media markets, facilitates development of more interpretable and evidence-grounded fact-checking models, and lays groundwork for future multilingual, multimodal misinformation verification research.

Conclusion: The proposed pipeline successfully addresses limitations of existing fact-checking datasets by providing comprehensive, structured, multimodal resources in French and German, enabling better analysis of fact-checking practices and supporting development of more effective misinformation verification systems.
...
TL;DR: VULCA-Bench is a multicultural art-critique benchmark for evaluating Vision-Language Models' cultural understanding beyond basic visual perception, covering 7,410 image-critique pairs across eight cultural traditions with Chinese-English bilingual coverage.


<details>
  <summary>Details</summary>
Motivation: Existing VLM benchmarks primarily measure lower-level capabilities (object recognition, scene description, factual QA) but under-evaluate higher-order cultural interpretation and understanding. There's a need to assess VLMs' ability to comprehend cultural context and deeper meaning in art.

Method: Created a benchmark with 7,410 matched image-critique pairs spanning eight cultural traditions. Used a five-layer framework (L1-L5: Visual Perception to Philosophical Aesthetics) with 225 culture-specific dimensions. Included expert-written bilingual critiques (Chinese-English) and developed evaluation scripts and annotation tools.

Result: Pilot results show that higher-layer reasoning (L3-L5: cultural context, symbolic meaning, philosophical aesthetics) is consistently more challenging for VLMs than visual and technical analysis (L1-L2). The dataset and tools are available under CC BY 4.0 license.

Conclusion: VULCA-Bench addresses a critical gap in VLM evaluation by providing a comprehensive benchmark for assessing cultural understanding in art interpretation, revealing that current VLMs struggle with higher-order cultural reasoning despite performing well on basic visual tasks.
...
TL;DR: The paper proposes a new longitudinal modeling paradigm for NLP that treats documents as time-ordered behavioral sequences within individuals, rather than independent samples, and updates evaluation, metrics, inputs, and model internals accordingly.


<details>
  <summary>Details</summary>
Motivation: Traditional NLP treats documents as independent and unordered, but in longitudinal studies documents are nested within authors and ordered in time, forming behavioral sequences. This mismatch leads to ecologically invalid modeling and evaluation.

Method: Proposes a longitudinal modeling paradigm with four updates: (1) evaluation splits for cross-sectional and prospective generalization, (2) accuracy metrics separating between-person from within-person dynamics, (3) sequence inputs incorporating history by default, and (4) model internals supporting different coarseness of latent state over histories.

Result: On a dataset of 17k daily diary transcripts with PTSD symptoms from 238 participants, traditional document-level evaluation yields substantially different and sometimes reversed conclusions compared to the proposed ecologically valid approach.

Conclusion: The paper argues for a shift from word-sequence evaluation toward behavior-sequence paradigms in NLP, demonstrating that traditional approaches can lead to misleading conclusions in longitudinal contexts.
...
TL;DR: DyCP is a lightweight context management method that dynamically segments and retrieves relevant memory at query time to improve LLM performance in long dialogues, reducing latency while maintaining answer quality.


<details>
  <summary>Details</summary>
Motivation: LLMs suffer from increased response latency and degraded answer quality as dialogue length grows, making effective context management essential. Existing methods either require extra LLM calls to build memory or perform offline memory construction without considering current user utterances, leading to inefficiencies and disrupted conversational continuity.

Method: DyCP (Dynamic Context Management) is a lightweight method that dynamically segments and retrieves relevant memory at query time. It preserves the sequential structure of dialogue without predefined topic boundaries and supports efficient, adaptive context retrieval.

Result: Across three long-form dialogue benchmarks (LoCoMo, MT-Bench+, and SCM4LLMs) and multiple LLMs, DyCP consistently improves answer quality while reducing response latency. The paper also examines the gap between modern LLMs' expanded context windows and their actual long-context processing capacity.

Conclusion: Effective context management remains crucial despite expanded context windows in modern LLMs. DyCP provides an efficient solution that addresses the limitations of existing methods by dynamically managing context at query time, improving both performance and quality in long dialogues.
...
TL;DR: CVP (Concept Vector Projections) provides continuous multilingual sentiment scores that transfer well across domains, though its linearity assumption is approximate.


<details>
  <summary>Details</summary>
Motivation: Humanities sentiment analysis needs contextualized continuous scores; CVP offers this but its portability across domains and underlying assumptions need evaluation.

Method: Evaluated CVP across genres, historical periods, languages, and affective dimensions; examined the linearity assumption underlying the method.

Result: Concept vectors trained on one corpus transfer well to others with minimal performance loss; CVP captures generalizable patterns but linearity assumption is approximate.

Conclusion: CVP is a portable approach for sentiment analysis that effectively generalizes, though its approximate linearity points to potential for further development.
...
TL;DR: LLM Review framework uses blind peer review to enhance creative generation in LLMs, outperforming multi-agent baselines and showing smaller models can surpass larger single models.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with creative generation, and existing multi-agent frameworks often cause content homogenization instead of enhancing creativity.

Method: Proposes LLM Review framework with Blind Peer Review where agents exchange targeted feedback while revising independently. Also introduces SciFi-100 dataset with unified evaluation combining LLM-as-a-judge scoring, human annotation, and rule-based novelty metrics.

Result: LLM Review consistently outperforms multi-agent baselines, and smaller models using this framework can surpass larger single-agent models.

Conclusion: Interaction structure (blind peer review) can substitute for model scale in creative generation tasks, offering a promising approach to enhance LLM creativity.
...
TL;DR: Researchers use Sparse Autoencoders to identify latent features in LLMs that trigger reasoning, showing that steering these features can achieve CoT-level performance without explicit prompting.


<details>
  <summary>Details</summary>
Motivation: To understand why Chain-of-Thought prompting works and whether it's the unique mechanism for triggering reasoning in large language models, by analyzing internal representations.

Method: Used Sparse Autoencoders to analyze and intervene on LLM internal representations, identifying reasoning-related latent features that can be externally steered to trigger reasoning behavior.

Result: Latent feature steering achieves comparable performance to CoT prompting while producing more efficient outputs, and reasoning states can override prompt-level instructions against explicit reasoning.

Conclusion: CoT prompting is one effective but not unique way to activate LLM reasoning; reasoning is supported by latent internal activations that can be externally triggered.
...
TL;DR: Language models can perform universal computation through autoregressive chaining, even when randomly initialized before training.


<details>
  <summary>Details</summary>
Motivation: To understand the ultimate computational capabilities of language models and determine whether training creates computational expressiveness or merely improves programmability.

Method: Proving that chaining a language model's autoregressive output is sufficient for universal computation, and demonstrating this capability in randomly initialized models.

Result: Language models can simulate execution of any algorithm on any input, and this computational capability exists even in untrained, randomly initialized models.

Conclusion: Training doesn't create computational expressiveness but improves programmability, enabling natural language interfaces to access intrinsic computational capabilities.
...
TL;DR: The paper introduces a new evaluation framework for confidence estimation in LLMs that goes beyond traditional calibration/discrimination metrics to assess robustness to prompt variations, stability across semantically equivalent answers, and sensitivity to semantically different answers.


<details>
  <summary>Details</summary>
Motivation: Existing confidence estimation evaluation focuses only on calibration (confidence aligns with accuracy) and discrimination (confidence ranks correct predictions higher), but ignores important aspects relevant to LLMs and language variation: confidence should be consistent under semantically equivalent prompt/answer variations, and should change when answer meaning differs.

Method: The authors present a comprehensive evaluation framework that measures confidence quality on three new aspects: 1) robustness against prompt perturbations, 2) stability across semantically equivalent answers, and 3) sensitivity to semantically different answers.

Result: Common confidence estimation methods for LLMs often fail on these new metrics: methods that perform well on traditional calibration or discrimination metrics are not robust to prompt variations or are not sensitive to answer changes.

Conclusion: The framework reveals limitations of existing confidence estimation evaluations relevant for real-world LLM use cases and provides practical guidance for selecting and designing more reliable confidence estimation methods.
...
TL;DR: AdaJudge improves reward modeling by replacing static pooling with adaptive representation refinement and multi-view aggregation.


<details>
  <summary>Details</summary>
Motivation: Current reward models use static pooling strategies that have two key problems: (1) static inductive bias misaligned with task-dependent preferences, and (2) representational mismatch since backbones are optimized for generation rather than discrimination.

Method: AdaJudge jointly adapts representation and aggregation: (1) refines backbone representations into discrimination-oriented space via gated refinement blocks, (2) replaces static readout with adaptive multi-view pooling that dynamically routes and combines evidence.
...
Conclusion: Adaptive representation refinement and multi-view pooling provide better alignment with human preferences than static pooling approaches in reward modeling.
...
TL;DR: This paper introduces query suggestion for agentic RAG systems to handle unanswerable user questions by suggesting similar, answerable queries, addressing limitations in current guardrail approaches.


<details>
  <summary>Details</summary>
Motivation: Current agentic RAG systems have limited grounding knowledge scope, leading to hallucinations for out-of-scope questions. While guardrails block such questions, there's no research on suggesting answerable alternatives to complete user interactions, especially important for tool-calling LLMs where communicating tool/dataset restrictions is difficult.

Method: The paper introduces robust dynamic few-shot learning that retrieves examples from relevant workflows to understand the RAG's multi-step process. The system can be self-learned on prior user queries and is designed to suggest queries similar to original questions while ensuring they are answerable within the RAG's capabilities.

Result: Experiments on three benchmark datasets based on real-world user queries show the method produces more relevant and answerable suggestions than few-shot and retrieval-only baselines, enabling safer and more effective user interaction with agentic RAG systems.

Conclusion: Query suggestion for agentic RAG is a valuable approach to enhance user interaction when questions exceed system capabilities. The proposed robust dynamic few-shot learning method effectively addresses the challenge of ensuring suggested queries are answerable, outperforming existing baselines and providing practical applicability through self-learning on prior queries.
...
<details>
  <summary>Details</summary>
Motivation: Existing prompting methods like Chain-of-Thought suffer from excessive token usage and limited generalizability across diverse reasoning tasks.

Method: Adaptive Causal Prompting with Sketch-of-Thought (ACPS) framework that leverages structural causal models to infer causal effects and adaptively selects appropriate interventions (standard front-door and conditional front-door adjustments). Replaces verbose CoT with concise Sketch-of-Thought.
...
Conclusion: ACPS enables generalizable causal reasoning across heterogeneous tasks without task-specific retraining while significantly reducing token usage and inference costs.
...
TL;DR: ExoFormer introduces external anchor projections to decouple stable reference from computational refinement in transformers, improving performance and efficiency while causing representation collapse.


<details>
  <summary>Details</summary>
Motivation: Transformers that reuse early-layer attention projections as residuals face a fundamental tension: the first layer must simultaneously serve as a stable reference for all deeper layers and as an effective computational block.

Method: Proposes ExoFormer which learns dedicated exogenous anchor projections outside the sequential layer stack, using a unified normalized mixing framework across all attention pathways (queries, keys, values, and gate logits) with different coefficient granularities.

Result: ExoFormer variants consistently outperform internal-anchor counterparts, with dynamic variant achieving 2.13-point increase in downstream accuracy, 1.84x fewer tokens for same validation loss, and 2x reduction in attention sink compared to standard Gated Attention.

Conclusion: ExoFormer's representation collapse is explained by the Offloading Hypothesis: external anchors preserve essential token identity, allowing layers to specialize exclusively in computational refinement.
...
TL;DR: RMCB benchmark introduced for evaluating confidence estimation in Large Reasoning Models, revealing trade-off between discrimination and calibration across methods.


<details>
  <summary>Details</summary>
Motivation: Miscalibration of Large Reasoning Models undermines reliability in high-stakes domains, requiring accurate confidence estimation for long-form, multi-step outputs.

Method: Created RMCB benchmark with 347,496 reasoning traces from 6 LRMs across diverse domains, evaluated 10+ representation-based methods including sequential, graph-based, and text-based architectures.

Result: Found persistent trade-off: text-based encoders achieve best AUROC (0.672) while structurally-aware models yield best ECE (0.148). Increased architectural complexity doesn't reliably outperform simpler baselines.

Conclusion: Provides most comprehensive benchmark for confidence estimation in reasoning models, establishes rigorous baselines, and demonstrates limitations of current representation-based paradigms.
...
TL;DR: Qalb is a new Urdu language model that achieves state-of-the-art performance by using continued pre-training on 1.97B tokens of diverse Urdu text followed by supervised fine-tuning, outperforming previous models by significant margins.


<details>
  <summary>Details</summary>
Motivation: Urdu is critically underrepresented in NLP despite being spoken by 230M+ people. Existing multilingual models perform poorly on Urdu due to its complex morphology, right-to-left Nastaliq script, and rich literary traditions. Even LLaMA-3.1 8B-Instruct shows limited Urdu capability.

Method: Two-stage approach: 1) Continued pre-training on LLaMA 3.1 8B using 1.97B tokens (1.84B Urdu tokens from news, literature, government docs, social media + 140M English Wikipedia tokens to prevent catastrophic forgetting). 2) Supervised fine-tuning on Alif Urdu-instruct dataset.
...
Conclusion: Continued pre-training on diverse, high-quality language data combined with targeted instruction fine-tuning effectively adapts foundation models to low-resource languages like Urdu.
...
<details>
  <summary>Details</summary>
Motivation: Adapting LLMs to low-resource languages faces three main challenges: scarce labeled data, unstable full-model fine-tuning, and catastrophic forgetting during cross-lingual tuning.

Method: CT-SFT uses label-balanced mean baseline and task-directional relevance scoring to identify sparse task-relevant attention heads in a proxy-language checkpoint, then transfer learns to target language by updating only those heads plus LayerNorm via head-level gradient masking.
...
Conclusion: CT-SFT provides an effective approach for low-resource language adaptation by selectively updating task-relevant circuits, balancing transfer performance with preservation of source-language competence.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based agents deployed in user-facing services are error-prone in new tasks, show repetitive failure patterns, and have substantial run-to-run variability. Fixing these failures through environment-specific training or manual patching is costly and doesn't scale well.

Method: WISE-Flow converts historical service interactions into reusable procedural experience by inducing workflows with prerequisite-augmented action blocks. At deployment, it aligns the agent's execution trajectory to retrieved workflows and performs prerequisite-aware feasibility reasoning to achieve state-grounded next actions.

Result: Experiments on ToolSandbox and τ²-bench show consistent improvement across base models.

Conclusion: WISE-Flow provides a scalable framework for enabling self-evolving agents in user-facing service environments by leveraging historical interactions to create reusable workflows that improve agent performance and reduce error patterns.
...
TL;DR: SwiftMem is a query-aware agentic memory system that achieves sub-linear retrieval through specialized temporal and semantic indexing, solving latency bottlenecks in existing memory frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing agentic memory systems suffer from exhaustive retrieval across entire storage regardless of query characteristics, creating severe latency bottlenecks as memory grows and hindering real-time agent interactions.

Method: SwiftMem uses specialized indexing over temporal and semantic dimensions: temporal index enables logarithmic-time range queries, semantic DAG-Tag index maps queries to relevant topics through hierarchical tag structures, and embedding-tag co-consolidation reorganizes storage based on semantic clusters to improve cache locality.

Result: Experiments on LoCoMo and LongMemEval benchmarks demonstrate 47× faster search compared to state-of-the-art baselines while maintaining competitive accuracy.

Conclusion: SwiftMem enables practical deployment of memory-augmented LLM agents by solving latency bottlenecks through efficient query-aware retrieval mechanisms.
...
TL;DR: Fine-tuning function vectors (derived from causal mediation analysis) with few examples improves relation-based word completion, and composite function vectors enhance analogical reasoning in LLMs.


<details>
  <summary>Details</summary>
Motivation: To develop more effective methods for encoding and manipulating relational knowledge in language models, improving both interpretability and reasoning capabilities.

Method: Fine-tuning function vectors (compact representations from causal mediation analysis) with few examples (~20 word pairs), creating composite function vectors via weighted combination, and using activation patching at inference time.

Result: Fine-tuned function vectors outperform original vectors on relation-based tasks, show better alignment with human similarity judgments, and composite vectors significantly improve performance on challenging analogy problems from cognitive science and SAT benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Automatic evaluation of LLM responses requires both factual correctness and clarity, especially in political QA. While datasets provide human annotations for clarity and evasion, the impact of prompt design on automatic clarity evaluation is underexplored.

Method: Used CLARITY dataset from SemEval 2026 shared task. Compared GPT-3.5 baseline against GPT-5.2 with three prompting strategies: simple prompting, chain-of-thought prompting, and chain-of-thought with few-shot examples. Evaluated predictions against human annotations using accuracy, class-wise metrics for clarity/evasion, and hierarchical exact match.

Result: GPT-5.2 consistently outperforms GPT-3.5 baseline on clarity prediction. Chain-of-thought with few-shot prompting improved accuracy from 56% to 63%. Chain-of-thought prompting yields highest evasion accuracy at 34%, though improvements are less stable across fine-grained evasion categories. Reasoning-based prompting improves topic identification accuracy from 60% to 74%.

Conclusion: Prompt design reliably improves high-level clarity evaluation, but fine-grained evasion and topic detection remain challenging despite structured reasoning prompts.
...
TL;DR: LogiSafetyGen framework converts regulations into LTL oracles for logic-guided fuzzing, creating LogiSafetyBench with 240 tasks to test LLM compliance beyond functional correctness.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks overlook implicit regulatory compliance in high-stakes domains, failing to evaluate whether LLMs can autonomously enforce mandatory safety constraints beyond simple functional correctness.

Method: LogiSafetyGen framework converts unstructured regulations into Linear Temporal Logic (LTL) oracles and employs logic-guided fuzzing to synthesize valid, safety-critical traces. LogiSafetyBench is constructed with 240 human-verified tasks requiring LLMs to generate Python programs satisfying both functional objectives and latent compliance rules.

Result: Evaluation of 13 SOTA LLMs reveals that larger models, despite achieving better functional correctness, frequently prioritize task completion over safety, resulting in non-compliant behavior.

Conclusion: There is a critical need for benchmarks that evaluate LLM compliance with regulatory standards in high-stakes domains, as current models show a tendency to sacrifice safety for task completion.
...
TL;DR: T-SPIN improves self-play fine-tuning by adding historical advantages and entropy constraints for more stable optimization and better performance with limited data.


<details>
  <summary>Details</summary>
Motivation: SPIN has limitations: current reward advantages vanish during iterations causing unstable optimization, and reference policy creates training-generation misalignment. Need better method for fine-tuning with scarce expert data.

Method: Triplet-based Self-Play fine-tuning (T-SPIN) with two key designs: 1) Incorporates historical advantages between iteratively generated responses and proto-synthetic responses from initial policy, 2) Adds entropy constraint to enable reference-free fine-tuning eliminating training-generation discrepancy.

Result: T-SPIN outperforms SPIN on various tasks, shows stable evolution during iterations, and achieves comparable or better performance than supervised fine-tuning with only 25% of samples.

Conclusion: T-SPIN effectively addresses SPIN's limitations, providing stable optimization and superior performance for fine-tuning LLMs with scarce annotated data through historical advantages and entropy constraints.
...
TL;DR: GAG (Generation-Augmented Generation) is a new method for injecting private domain knowledge into LLMs that avoids issues with fine-tuning and RAG by treating private expertise as an expert modality aligned via representation-level interfaces.


<details>
  <summary>Details</summary>
Motivation: High-stakes domains like biomedicine, materials, and finance require injecting private, domain-specific knowledge into LLMs, but existing methods have problems: fine-tuning is expensive and risks catastrophic forgetting, while RAG is brittle in specialized private corpora due to evidence fragmentation and retrieval drift.

Method: GAG treats private expertise as an additional expert modality and injects it via a compact, representation-level interface aligned to the frozen base model, inspired by how multimodal LLMs align heterogeneous modalities. This avoids prompt-time evidence serialization while enabling plug-and-play specialization.

Result: GAG improves specialist performance over strong RAG baselines by 15.34% and 14.86% on two private scientific QA benchmarks (immunology adjuvant and catalytic materials), maintains performance on six open general benchmarks, and enables near-oracle selective activation for scalable multi-domain deployment.

Conclusion: GAG provides an effective solution for private knowledge injection that outperforms RAG while maintaining general capabilities, enabling scalable multi-domain deployment with reliable selective activation.
...
TL;DR: MoE performance is determined by total parameters and expert sparsity, not just total vs active parameters. Larger expert count slightly hurts performance by reducing core model dimensions.


<details>
  <summary>Details</summary>
Motivation: Current MoE design focuses only on total parameters (memory) and active parameters (inference cost), but these alone are insufficient to describe optimal architecture. Need better understanding of what truly determines MoE performance.

Method: Systematic study analyzing MoE performance factors, examining relationship between total parameters, expert sparsity (n_exp/n_topk), and how expert count affects core model dimensions under memory constraints.

Result: MoE performance primarily determined by total parameters and expert sparsity. Larger expert count slightly penalizes performance by forcing reduction in core model dimensions. Expert count and top-k don't cancel out in sparsity ratio.

Conclusion: Optimal MoE design should maximize total parameters while minimizing expert sparsity (maximizing top-k) and expert count under given constraints. Provides framework for resolving architectural ambiguity in MoE design.
...
TL;DR: The paper introduces a user-oriented simulation framework for generating realistic multi-turn tool-use dialogues, addressing limitations of task-only approaches by incorporating human-like behaviors to create extended, high-density interaction data.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for large reasoning models as autonomous agents are limited by static, predefined toolsets and task-oriented approaches that produce minimal interaction trajectories, failing to capture the complex, extended dialogues seen in real human-agent collaboration.

Method: Shift from task-oriented to user-oriented simulation paradigm, decoupling task generation from a dedicated user simulator that mimics human behavioral rules (incremental request-making, turn-by-turn feedback). Uses a plug-and-play pipeline that can initiate generation from any state, enabling multiple task completions within single trajectories.

Result: The framework generates more authentic, extended multi-turn dialogues that reflect iterative real-world problem solving, producing high-density datasets with high turn-count conversations that better represent multifaceted human-agent interaction demands.

Conclusion: The user-oriented simulation approach successfully bridges the gap between task-solving efficiency and realistic human-agent interaction patterns, creating scalable, high-quality tool-use data that better prepares LRMs for open-ended collaboration scenarios.
...
TL;DR: Med-CoReasoner is a framework that bridges the multilingual gap in medical reasoning by combining English structural reasoning with local-language clinical knowledge through concept-level alignment and retrieval.


<details>
  <summary>Details</summary>
Motivation: While LLMs perform well on English medical tasks, there's a persistent multilingual gap with substantially weaker reasoning in local languages, limiting equitable global medical deployment.

Method: Med-CoReasoner uses language-informed co-reasoning that elicits parallel English and local-language reasoning, abstracts them into structured concepts, and integrates local clinical knowledge into an English logical scaffold via concept-level alignment and retrieval.

Result: Med-CoReasoner improves multilingual reasoning performance by an average of 5% across three benchmarks, with particularly substantial gains in low-resource languages. Expert evaluation confirms it produces clinically sound and culturally grounded reasoning.

Conclusion: The framework successfully bridges the multilingual medical reasoning gap by combining English structural robustness with practice-grounded local language expertise, enabling more equitable global medical AI deployment.
...
TL;DR: DART is a reinforcement learning framework that enables LLMs to spontaneously use tools during long chain-of-thought reasoning without human annotation, using dynamic rollout trees to discover and reinforce beneficial tool-integrated trajectories.


<details>
  <summary>Details</summary>
Motivation: Current tool-integrated reasoning approaches struggle with integrating tool-use into long chain-of-thought reasoning due to scarcity of training data and the challenge of maintaining the model's intrinsic long-chain reasoning capabilities while incorporating tools.

Method: DART uses reinforcement learning with dynamic rollout trees that branch at promising positions to explore diverse tool-integrated trajectories. It employs tree-based process advantage estimation to identify and credit specific sub-trajectories where tool invocation positively contributes to solutions.

Result: DART significantly outperforms existing methods on challenging benchmarks like AIME and GPQA-Diamond, successfully harmonizing tool execution with long chain-of-thought reasoning.

Conclusion: DART provides an effective framework for enabling spontaneous tool-use during long chain-of-thought reasoning without human annotation, addressing key limitations in current tool-integrated reasoning approaches.
...
TL;DR: D²Plan is a dual-agent dynamic planning paradigm for retrieval-augmented reasoning that addresses failure modes in search-augmented LLMs through collaboration between a Reasoner and Purifier, trained with SFT and RL.


<details>
  <summary>Details</summary>
Motivation: Search-augmented LLMs with RL face two critical failure modes: (1) ineffective search chain construction that produces incorrect queries or omits critical information retrieval, and (2) reasoning hijacking by peripheral evidence that causes models to misidentify distractors as valid evidence.

Method: D²Plan uses a dual-agent paradigm with a Reasoner that constructs explicit global plans during reasoning and dynamically adapts them based on retrieval feedback, and a Purifier that assesses retrieval relevance and condenses key information. The approach includes a two-stage training framework: supervised fine-tuning (SFT) cold-start on synthesized trajectories followed by RL with plan-oriented rewards.
...
Conclusion: The D²Plan paradigm effectively addresses critical failure modes in search-augmented LLMs by enabling collaborative planning and information purification, leading to improved multi-hop reasoning capabilities in complex retrieval-augmented tasks.
...
TL;DR: Advanced prompt engineering (few-shot, chain-of-thought, self-consistency) significantly improves sentiment analysis performance in LLMs, but optimal techniques vary by model and task complexity.


<details>
  <summary>Details</summary>
Motivation: To investigate how advanced prompting techniques can enhance large language models' performance in sentiment analysis tasks, particularly for complex subtasks like irony detection and aspect-based analysis.

Method: Evaluated GPT-4o-mini and gemini-1.5-flash using advanced prompting techniques (few-shot learning, chain-of-thought prompting, self-consistency) against baseline. Tasks included sentiment classification, aspect-based sentiment analysis, and irony detection. Performance measured by accuracy, recall, precision, and F1 score.

Result: Advanced prompting significantly improves sentiment analysis. Few-shot approach works best for GPT-4o-mini, while chain-of-thought prompting boosts irony detection in gemini-1.5-flash by up to 46%. Different models respond best to different prompting strategies.

Conclusion: Prompting strategies must be tailored to both the specific LLM architecture and the semantic complexity of the task. Effective prompt design requires alignment with model characteristics and task requirements.
...
TL;DR: AgriAgent is a two-level agent framework for real-world agriculture that uses hierarchical execution based on task complexity, with direct reasoning for simple tasks and contract-driven planning for complex tasks.


<details>
  <summary>Details</summary>
Motivation: Existing agent systems use unified execution paradigms that struggle with varying task complexity and incomplete tool availability in agricultural environments, where tasks range from simple information understanding to complex multi-step execution.

Method: AgriAgent uses a hierarchical two-level strategy: simple tasks are handled by modality-specific agents through direct reasoning, while complex tasks trigger a contract-driven planning mechanism that formulates tasks as capability requirements and performs capability-aware tool orchestration with dynamic tool generation and failure recovery.
...
Conclusion: The proposed two-level agent framework effectively addresses the challenges of varying task complexity and incomplete tool availability in real-world agricultural scenarios, offering a more adaptable and robust solution than unified execution approaches.
...
TL;DR: CLaS-Bench is a new benchmark for evaluating multilingual steering techniques in LLMs across 32 languages, finding that simple residual-based DiffMean interventions outperform other methods.


<details>
  <summary>Details</summary>
Motivation: There's a lack of dedicated benchmarks and evaluation protocols to quantify the effectiveness of steering techniques (manipulating internal representations during inference) for adapting LLMs to target languages, despite steering being an efficient and interpretable alternative to prompting or fine-tuning.

Method: Created CLaS-Bench, a lightweight parallel-question benchmark for 32 languages. Evaluated various steering techniques: residual-stream DiffMean interventions, probe-derived directions, language-specific neurons, PCA/LDA vectors, Sparse Autoencoders, and prompting baselines. Performance measured along language control and semantic relevance axes, combined into a harmonic-mean steering score.

Result: Simple residual-based DiffMean method consistently outperforms all other methods across languages. Language-specific structure emerges predominantly in later layers, and steering directions cluster based on language family.

Conclusion: CLaS-Bench provides the first standardized benchmark for multilingual steering, enabling both scientific analysis of language representations and practical evaluation of steering as a low-cost adaptation alternative for LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Mental manipulation detection has focused only on text conversations, ignoring how manipulative tactics manifest in speech. There's a gap in understanding how modality affects detection of covert influence in dialogues.

Method: Created SPEECHMENTALMANIP benchmark by augmenting text-based dataset with high-quality, voice-consistent Text-to-Speech audio. Used few-shot large audio-language models and human annotation to evaluate modality effects on detection accuracy and perception.

Result: Models show high specificity but significantly lower recall on speech compared to text, indicating sensitivity to missing acoustic/prosodic cues. Human raters also show uncertainty with audio, highlighting inherent ambiguity of manipulative speech.

Conclusion: Findings demonstrate need for modality-aware evaluation and safety alignment in multimodal dialogue systems, as speech introduces unique challenges for detecting mental manipulation compared to text.
...
TL;DR: LLM tutoring systems should adapt to student personalities using a pedagogical taxonomy framework, improving teaching effectiveness through personalized strategies.


<details>
  <summary>Details</summary>
Motivation: Current LLM tutoring systems don't account for student personality traits, which is important because different tutoring strategies work better for different personalities, and mismatches can harm student outcomes.

Method: Constructed a taxonomy linking pedagogical methods to personality profiles based on literature, simulated student-teacher conversations, and used the framework to let LLM tutors adjust strategies to simulated student personalities.

Result: Human teachers consistently preferred the personality-adaptive approach over baselines, and the method increased use of high-impact strategies like role-playing that both human and LLM annotators significantly preferred.

Conclusion: The findings enable more personalized and effective LLM use in educational applications by adapting tutoring strategies to student personalities.
...
TL;DR: Latent-GRPO is a framework that uses latent space geometry to derive intrinsic rewards for LLM reasoning, eliminating need for expensive external verifiers while achieving 2x training speedup.


<details>
  <summary>Details</summary>
Motivation: GRPO improves LLM reasoning but relies on expensive external verifiers/human rules, causing high computational costs, training latency, and sparse rewards that hinder optimization efficiency.

Method: Uses latent space geometry analysis showing correct reasoning trajectories form dense clusters while incorrect ones scatter as outliers. Introduces Iterative Robust Centroid Estimation (IRCE) algorithm with spherical projection to mitigate magnitude fluctuations and estimate robust "truth centroid" through iterative aggregation.

Result: Maintains model performance while achieving over 2x training speedup compared to baselines on multiple datasets. Shows strong generalization ability and robustness.

Conclusion: Latent-GRPO provides an efficient alternative to external verifiers by leveraging intrinsic geometric properties of reasoning trajectories, enabling faster training without performance degradation.
...
TL;DR: Fine-Mem: A framework using chunk-level step rewards and evidence-anchored reward attribution to provide fine-grained feedback for memory management in LLM agents, addressing reward sparsity in long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Existing RL approaches for memory management in LLM agents suffer from severe reward sparsity and ineffective credit assignment, as they rely only on final task performance rewards, providing insufficient guidance for individual memory operations.
...
Result: Fine-Mem consistently outperforms strong baselines on Memalpha and MemoryAgentBench, achieving superior success rates across various sub-tasks. Analysis shows adaptability and strong generalization across diverse model configurations and backbones.

Conclusion: Fine-Mem enables stable policy optimization and aligns local memory operations with long-term utility of memory, addressing reward sparsity through fine-grained feedback alignment for effective memory management in LLM agents.
...
TL;DR: JudgeRLVR improves RLVR by adding a discriminative pre-training stage where models learn to judge solutions before generation, leading to better accuracy with shorter outputs.


<details>
  <summary>Details</summary>
Motivation: Standard RLVR optimization for final-answer correctness leads to verbose, aimless exploration through trial-and-error rather than structured planning. Existing constraints like length penalties often cut essential reasoning steps, creating a trade-off between efficiency and verification.

Method: Two-stage judge-then-generate paradigm: 1) Train model to judge solution responses with verifiable answers (discriminative pre-training), 2) Fine-tune the same model with vanilla generating RLVR initialized from the judge model.

Result: On Qwen3-30B-A3B: In-domain math: +3.7 points average accuracy gain with -42% average generation length. Out-of-domain benchmarks: +4.5 points average accuracy improvement, showing enhanced generalization.

Conclusion: Discriminative capability is a prerequisite for efficient generation. Learning to distinguish valid solutions helps models internalize guidance signals that prune search space, achieving better quality-efficiency trade-off than vanilla RLVR.
...
TL;DR: A 24B parameter model (sui-1) generates abstractive summaries with inline citations, enabling users to verify claims against source text, outperforming larger models through specialized training.


<details>
  <summary>Details</summary>
Motivation: Large language models often produce plausible but unverifiable summaries, which is problematic in compliance-sensitive domains like government and legal analysis where traceability to source material is essential.

Method: Developed sui-1, a 24B parameter model trained on synthetic data generated via chain-of-thought prompting with multi-stage verification, creating over 22,000 high-quality examples across five languages from diverse sources including parliamentary documents, web text, and Wikipedia.
...
Conclusion: Specialized training for citation-grounded summarization is more effective than simply scaling model size, and the approach enables verifiable summaries critical for compliance-sensitive applications. Model weights and demo are publicly available.
...
TL;DR: Framework combining NLP and formal verification to create empathetic therapy chatbots with verifiable empathy properties.


<details>
  <summary>Details</summary>
Motivation: Current chatbot development lacks systematic ways to specify or verify empathy, which is crucial for mental therapy applications where conversational agents are increasingly used.

Method: Transformer-based model extracts dialogue features, translated into Stochastic Hybrid Automaton model of therapy sessions. Empathy properties verified via Statistical Model Checking, with strategy synthesis for behavior guidance.

Result: Preliminary results show formal model captures therapy dynamics with good fidelity, and ad-hoc strategies improve probability of satisfying empathy requirements.

Conclusion: Framework successfully integrates NLP and formal verification to enable systematic empathy specification and verification for therapy chatbots.
...
TL;DR: Surgical Refusal Ablation (SRA) is a method to cleanly remove refusal behavior from safety-aligned language models by orthogonalizing refusal vectors against capability and style directions, achieving deep refusal reduction with minimal distribution drift.


<details>
  <summary>Details</summary>
Motivation: Standard activation steering for refusal ablation causes collateral damage and distribution drift because raw refusal vectors are polysemantic, entangling refusal signals with core capability circuits and linguistic style.
...
Result: Across five models, SRA achieves 0-2% refusal rates with negligible perplexity impact (mean delta PPL ≈ 0.02) and minimal distribution drift, while standard ablation causes severe drift (KL = 2.088 vs SRA's KL = 0.044).

Conclusion: Common "model damage" from refusal ablation is often "Ghost Noise" - spectral bleeding of dirty refusal directions into capability subspaces, which SRA successfully mitigates while preserving core capabilities.
...
TL;DR: LLMs can produce excessive outputs from plain-text prompts (Overflow), increasing costs, latency, and environmental impact. BenchOverflow benchmark measures this vulnerability across models and shows simple mitigations help.


<details>
  <summary>Details</summary>
Motivation: Overflow represents a practical failure mode where LLMs generate unnecessarily long outputs from ordinary prompts, leading to economic costs (higher serving costs), environmental impact (energy consumption), and service degradation in shared environments. Unlike adversarial attacks, this occurs in normal usage.

Method: Created BenchOverflow benchmark with 9 plain-text prompting strategies that amplify output volume without adversarial techniques. Evaluated 9 open- and closed-source models with standardized protocol (5000 new token budget). Used metrics: cap-saturation rates (CSR@1k/3k/5k), empirical cumulative distribution functions (ECDFs), within-prompt variance, and cross-model correlations. Tested lightweight mitigation (fixed conciseness reminder).

Result: Models show pronounced rightward shifts and heavy tails in length distributions. Overflow is broadly reproducible but heterogeneous across model families and attack vectors. The simple conciseness reminder effectively attenuates right tails and lowers CSR for most strategies across majority of models.

Conclusion: Length control should be treated as a measurable reliability, cost, and sustainability concern rather than stylistic preference. BenchOverflow enables standardized comparison of length-control robustness, helping select deployments that minimize resource waste and evaluate defenses against compute amplification without compromising task performance.
...
TL;DR: MHEL-LLaMo is an unsupervised ensemble approach for multilingual historical entity linking that combines a small language model for candidate retrieval with an LLM for difficult cases, achieving state-of-the-art performance without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Entity linking for historical texts remains challenging due to linguistic variation, noisy inputs, and evolving semantic conventions. Existing solutions require substantial training data or rely on domain-specific rules that limit scalability.

Method: An unsupervised ensemble approach combining a multilingual bi-encoder (BELA) for candidate retrieval and an instruction-tuned LLM for NIL prediction and candidate selection via prompt chaining. Uses SLM confidence scores to discriminate between easy and hard samples, applying LLM only for hard cases to reduce computational costs.

Result: Outperforms state-of-the-art models on four established benchmarks in six European languages (English, Finnish, French, German, Italian, Swedish) from the 19th-20th centuries without requiring fine-tuning.

Conclusion: MHEL-LLaMo offers a scalable solution for low-resource historical entity linking by combining SLM efficiency with LLM reasoning capabilities, reducing computational costs while preventing hallucinations on straightforward cases.
...
TL;DR: STAGE is a unified benchmark for evaluating narrative understanding over full-length movie screenplays through four interconnected tasks grounded in shared world representations.


<details>
  <summary>Details</summary>
Motivation: Movie screenplays contain rich, complex narratives with character relationships, temporal events, and dialogue interactions, but existing benchmarks only evaluate isolated subtasks rather than holistic narrative understanding and consistency across multiple reasoning and generation forms.

Method: STAGE defines four interconnected tasks: knowledge graph construction, scene-level event summarization, long-context screenplay question answering, and in-script character role-playing, all grounded in a shared narrative world representation. The benchmark provides cleaned scripts, curated knowledge graphs, and event/character annotations for 150 English and Chinese films.

Result: The benchmark enables holistic evaluation of models' abilities to build world representations, abstract and verify narrative events, reason over long narratives, and generate character-consistent responses across multiple interconnected tasks.

Conclusion: STAGE provides a comprehensive framework for evaluating narrative understanding capabilities in AI models, moving beyond isolated subtasks to assess coherent story world construction and consistent reasoning across multiple dimensions of screenplay analysis.
...
<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought reasoning in LLMs creates a new attack surface for inference-time backdoors that generate malicious reasoning paths. These attacks are hard to detect because they produce linguistically coherent paths that evade conventional detection methods.

Method: Proposes STAR (State-Transition Amplification Ratio) framework that detects backdoors by analyzing output probability shifts. It exploits statistical discrepancies where malicious inputs induce paths with high posterior probability despite low prior probability. Quantifies state-transition amplification and uses CUSUM algorithm to detect persistent anomalies.

Result: Experiments across diverse models (8B-70B) and five benchmark datasets show STAR achieves near-perfect performance (AUROC ≈ 1.0) with approximately 42× greater efficiency than existing baselines. The framework is robust against adaptive attacks attempting to bypass detection.

Conclusion: STAR provides an effective and efficient solution for detecting inference-time backdoor attacks in reasoning LLMs, addressing a critical security vulnerability in modern language models with reasoning capabilities.
...
TL;DR: Characterizes unconditional convergence in infinite-dimensional spaces with 7 equivalent conditions, bridging functional analysis with computational algorithms.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the fundamental distinction between convergence types in infinite-dimensional spaces, which has direct implications for computational algorithm stability. Unlike finite dimensions where conditional, unconditional, and absolute convergence coincide, they separate in Banach spaces (Dvoretzky-Rogers theorem), creating practical challenges for numerical algorithms.

Method: Presents a comprehensive characterization theorem unifying seven equivalent conditions for unconditional convergence: permutation invariance, net convergence, subseries tests, sign stability, bounded multiplier properties, and weak uniform convergence. The approach connects classical functional analysis results with computational practice.

Result: Establishes rigorous foundations for order-independent and numerically robust summation processes. The theoretical characterization directly informs algorithmic stability analysis, governing permutation invariance in gradient accumulation for Stochastic Gradient Descent and justifying coefficient thresholding in frame-based signal processing.

Conclusion: The work successfully bridges classical functional analysis with contemporary computational practice, providing theoretical tools to analyze and ensure algorithmic stability in infinite-dimensional settings. The unified characterization of unconditional convergence offers practical guidance for designing robust numerical algorithms.
...
TL;DR: Deep Research Bench II introduces a new benchmark with 132 research tasks across 22 domains, evaluated using 9430 fine-grained binary rubrics across information recall, analysis, and presentation dimensions to address limitations in existing DRS evaluation methods.


<details>
  <summary>Details</summary>
Motivation: Current deep-research benchmarks fail to adequately test systems' analytical and report-writing capabilities, often using overly coarse or LLM-defined evaluation criteria that produce biased scores hard to verify or interpret relative to human expert judgment.

Method: Created 132 grounded research tasks across 22 domains, developed 9430 fine-grained binary rubrics covering information recall, analysis, and presentation dimensions. Rubrics derived from expert-written articles through a four-stage LLM+human pipeline with 400+ human-hours of expert review to ensure atomic, verifiable criteria aligned with human judgment.

Result: Evaluation of state-of-the-art deep-research systems shows even the strongest models satisfy fewer than 50% of the rubrics, revealing substantial gap between current DRSs and human experts.

Conclusion: Deep Research Bench II provides a rigorous, interpretable benchmark for evaluating deep-research systems, highlighting significant limitations in current DRS capabilities and establishing a foundation for more meaningful progress in this field.
...
TL;DR: Ministral 3 series: parameter-efficient dense language models (3B, 8B, 14B) with base, instruction-tuned, and reasoning variants, featuring image understanding and open-source licensing.


<details>
  <summary>Details</summary>
Motivation: To create parameter-efficient language models suitable for compute and memory constrained applications while maintaining strong performance across different tasks.

Method: Cascade Distillation - an iterative pruning and continued training with distillation technique to derive efficient models from larger ones.

Result: Three model sizes (3B, 8B, 14B) each with three variants: base, instruction-tuned, and reasoning models, all with image understanding capabilities.

Conclusion: Successfully developed a family of efficient language models under Apache 2.0 license, making them accessible for resource-constrained applications while maintaining versatility through multiple specialized variants.
...
TL;DR: ExpSeek introduces step-level proactive experience seeking for web agents, using entropy thresholds to determine intervention timing and tailor-designed experience content, achieving significant performance improvements on challenging benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing experience intervention methods for web agents passively inject experience as global context before task execution, which struggles to adapt to dynamically changing contextual observations during agent-environment interaction.

Method: ExpSeek shifts experience toward step-level proactive seeking by: (1) estimating step-level entropy thresholds to determine intervention timing using the model's intrinsic signals, and (2) designing step-level tailor-designed experience content.

Result: Experiments on Qwen3-8B and 32B models across four challenging web agent benchmarks show absolute improvements of 9.3% and 7.5% respectively. The approach validates entropy as a feasible self-triggering signal and shows that even a 4B small-scale experience model can significantly boost larger agent models.

Conclusion: ExpSeek demonstrates that step-level proactive experience seeking with entropy-based intervention timing is an effective approach for enhancing web agent capabilities, outperforming passive global context injection methods.
...
TL;DR: GraphSearch is a framework that extends search-augmented reasoning to graph learning, enabling zero-shot graph learning without task-specific fine-tuning by combining graph-aware query planning and retrieval.


<details>
  <summary>Details</summary>
Motivation: Current search-augmented large reasoning models (LRMs) don't effectively handle graph-structured data, which is prevalent in domains like e-commerce, social networks, and scientific citations. Graphs encode rich topological signals that could serve as valuable priors for retrieval and improve reasoning efficiency, but leveraging this structure poses unique challenges including generating graph-expressive queries and ensuring reliable retrieval that balances structural and semantic relevance.

Method: GraphSearch combines a Graph-aware Query Planner (which disentangles search space from semantic queries) with a Graph-aware Retriever (which constructs candidate sets based on topology and ranks them using a hybrid scoring function). The framework offers two traversal modes: GraphSearch-R (recursively expands neighborhoods hop by hop) and GraphSearch-F (flexibly retrieves across local and global neighborhoods without hop constraints).

Result: Extensive experiments across diverse benchmarks show GraphSearch achieves competitive or superior performance compared to supervised graph learning methods, setting state-of-the-art results in zero-shot node classification and link prediction.

Conclusion: GraphSearch positions itself as a flexible and generalizable paradigm for agentic reasoning over graphs, extending search-augmented reasoning to graph learning without requiring task-specific fine-tuning.
...
TL;DR: OrderProbe is a deterministic benchmark for evaluating LLMs' structural reconstruction ability using fixed four-character expressions in CJK languages with unique canonical order, revealing that even frontier models struggle with structural recovery despite semantic competence.


<details>
  <summary>Details</summary>
Motivation: LLMs excel at semantic understanding but their ability to reconstruct internal structure from scrambled inputs remains underexplored. Sentence-level restoration is problematic for automated evaluation because multiple valid word orders often exist.

Method: Introduce OrderProbe benchmark using fixed four-character expressions in Chinese, Japanese, and Korean that have unique canonical order, enabling exact-match scoring. Propose diagnostic framework evaluating models beyond recovery accuracy, including semantic fidelity, logical validity, consistency, robustness sensitivity, and information density.

Result: Experiments on twelve widely used LLMs show structural reconstruction remains difficult even for frontier systems: zero-shot recovery frequently falls below 35%. Also observe consistent dissociation between semantic recall and structural planning, suggesting structural robustness is not automatic byproduct of semantic competence.

Conclusion: Structural reconstruction is a challenging task for LLMs that requires separate evaluation from semantic understanding. The OrderProbe benchmark provides a deterministic way to assess structural capabilities, revealing important gaps in current models' structural reasoning abilities.
...
TL;DR: LALITA framework uses lexical and linguistic features to select complex source sentences for parallel corpus curation, improving MT quality while reducing data requirements by over 50% for low-resource languages.


<details>
  <summary>Details</summary>
Motivation: For low-resource languages, generating sufficient parallel data through human translation is prohibitively expensive. There's a need for efficient source sentence selection strategies to optimize MT system performance with limited data.

Method: Developed LALITA (Lexical And Linguistically Informed Text Analysis) framework that evaluates English-Hindi bi-text to determine effective sentence selection strategies. Uses lexical and linguistic features to curate parallel corpora by selecting complex sentences from both existing and synthetic datasets.

Result: Training on complex sentences significantly improves translation quality. LALITA reduces data needs by more than half across multiple languages (Hindi, Odia, Nepali, Norwegian Nynorsk, and German) while improving performance on all tested data sizes (50K to 800K sentences).

Conclusion: LALITA provides an effective framework for data curation in low-resource MT, reducing training costs through optimized data selection and demonstrating utility in data augmentation for improved translation quality.
...
TL;DR: This paper investigates how conditioning LLMs on specific moral values causally influences their political orientation, showing that moral conditioning actively steers models across economic and social dimensions.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of political bias in LLMs rely on direct probing or demographic personas, but political ideology is also understood as a downstream consequence of fundamental moral intuitions in social psychology. The paper aims to investigate the causal relationship between moral values and political positioning.

Method: Treat moral orientation as a controllable condition by conditioning models to endorse or reject specific moral values, then evaluate resulting shifts in political orientation using the Political Compass Test. Examine effects modulated by role framing and model scale, and test robustness across alternative assessment instruments.
...
TL;DR: XMPIE is a parallel multilingual multimodal dataset of potentially idiomatic expressions (PIEs) across 34 languages with over 10k items, featuring both textual and visual components for evaluating idiomatic language understanding across languages and modalities.


<details>
  <summary>Details</summary>
Motivation: PIEs represent culturally-bound language expressions that challenge NLP systems' linguistic and cultural capabilities. There's a need for a comprehensive benchmark to evaluate how well models understand idiomatic expressions across different languages and modalities (text vs. image).

Method: Created a parallel dataset with language experts following multilingual guidelines. Each PIE includes textual components and five images representing a spectrum from idiomatic to literal meanings, plus semantically related and random distractors. Covers 34 languages with over 10,000 items.

Result: XMPIE enables comparative analysis of idiomatic patterns across languages, evaluation of cross-lingual transfer of idiomatic understanding, and study of cross-modal understanding (text vs. image). Provides a high-quality benchmark for multilingual multimodal idiomatic language understanding.

Conclusion: XMPIE is a valuable resource for assessing NLP systems' ability to handle culturally-bound idiomatic expressions across languages and modalities, supporting research in cross-lingual transfer and multimodal understanding of figurative language.
...
TL;DR: The paper provides the first theoretical analysis of safe language generation, showing that safe language identification is impossible and safe language generation is at least as hard as vanilla language identification (also impossible), while discussing some tractable cases.


<details>
  <summary>Details</summary>
Motivation: Recent research shows language identification is impossible but language generation is tractable. As this foundational area expands, we need to understand the implications of language generation in real-world settings, particularly regarding safety concerns.

Method: Building on the computational paradigm of learning in the limit, the authors formalize the tasks of safe language identification and generation. They use theoretical analysis and proofs to establish impossibility results and complexity bounds.

Result: The paper proves that safe language identification is impossible, and that safe language generation is at least as hard as (vanilla) language identification, which is also impossible. The authors also discuss several intractable and tractable cases.

Conclusion: This work provides the first theoretical treatment of safe language generation, establishing fundamental impossibility results while identifying some potentially tractable scenarios for further research.
...
TL;DR: RULERS is a framework that transforms natural language rubrics into executable specifications for LLM-as-a-Judge evaluation, addressing rubric instability, unverifiable reasoning, and scale misalignment through compilation, structured decoding, and calibration.


<details>
  <summary>Details</summary>
Motivation: The LLM-as-a-Judge paradigm faces challenges in aligning frozen black-box models with human standards due to generation stochasticity, with three recurrent failure modes: rubric instability from prompt sensitivity, unverifiable reasoning lacking auditable evidence, and scale misalignment with human grading boundaries.

Method: RULERS (Rubric Unification, Locking, and Evidence-anchored Robust Scoring) is a compiler-executor framework that: 1) compiles criteria into versioned immutable bundles, 2) enforces structured decoding with deterministic evidence verification, and 3) applies lightweight Wasserstein-based post-hoc calibration, all without updating model parameters.
...
Conclusion: Reliable LLM judging requires executable rubrics, verifiable evidence, and calibrated scales rather than prompt phrasing alone. The framework demonstrates that criteria transfer through compilation and structured verification can address fundamental alignment challenges in LLM-as-a-Judge evaluation.
...
TL;DR: LLMs often falsely refuse hate speech detoxification tasks due to safety alerts. This study analyzes contextual/linguistic biases causing refusals and proposes a cross-translation mitigation strategy.


<details>
  <summary>Details</summary>
Motivation: LLMs increasingly used for hate speech detoxification but often trigger safety alerts and refuse tasks. Need to understand false refusal behavior and develop mitigation approaches.

Method: Systematically investigate false refusal behavior in hate speech detoxification. Evaluate 9 LLMs on English and multilingual datasets. Analyze contextual/linguistic biases. Propose cross-translation strategy (English→Chinese→English).

Result: LLMs disproportionately refuse inputs with higher semantic toxicity and those targeting nationality, religion, political ideology. Multilingual datasets have lower refusal rates but still show language-dependent biases. Cross-translation strategy substantially reduces false refusals while preserving content.

Conclusion: LLMs exhibit systematic biases in hate speech detoxification refusals. Cross-translation provides effective, lightweight mitigation approach to reduce false refusals while maintaining detoxification quality.
...
TL;DR: Industry case study on developing agentic systems for multi-party dialogue summarization, addressing practical challenges like evolving requirements, evaluation methods, component optimization, data bottlenecks, and vendor lock-in.


<details>
  <summary>Details</summary>
Motivation: Multi-party dialogue summarization is critical for knowledge transfer and operational effectiveness in industry, but existing research uses static datasets that don't reflect real-world scenarios where requirements constantly evolve.

Method: Developed an agentic system architecture that decomposes the summarization task into components, enabling component-wise optimization and practical evaluation methods despite evolving requirements and task subjectivity.

Result: Shared practical insights from full development lifecycle: robust evaluation methods for evolving requirements, component optimization benefits, impact of upstream data bottlenecks, and realities of vendor lock-in due to poor LLM prompt transferability.

Conclusion: Agentic architectures provide practical advantages for industry summarization systems by enabling adaptability to evolving requirements, though challenges like data bottlenecks and vendor lock-in remain important considerations for practitioners.
...
Result: Evaluation of state-of-the-art LLMs shows substantial gaps compared to human experts, particularly in reasoning and strategy coding. Fine-tuning experiments demonstrate consistent improvements, and the benchmark includes reproducible backtesting configuration.

Conclusion: QuantEval provides a comprehensive benchmark to facilitate research on LLMs' quantitative finance capabilities and accelerate their practical adoption in real-world trading, with full reproducibility ensured through released backtesting configurations.
...
<details>
  <summary>Details</summary>
Motivation: Predicting nationality from names has practical applications in marketing, demographics, and genealogy. Traditional neural models struggle with low-frequency nationalities and distinguishing similar nationalities within regions. LLMs may overcome these limitations through world knowledge from pre-training.

Method: Comprehensive comparison of six neural models and six LLM prompting strategies across three granularity levels (nationality, region, continent). Used frequency-based stratified analysis and error analysis to evaluate performance differences.

Result: LLMs outperform neural models at all granularity levels, with gap narrowing as granularity becomes coarser. Simple ML methods show highest frequency robustness, while pre-trained models and LLs degrade for low-frequency nationalities. LLMs make "near-miss" errors (correct region, wrong nationality), while neural models exhibit cross-regional errors and bias toward high-frequency classes.

Conclusion: LLM superiority comes from world knowledge. Model selection should consider required granularity level. Evaluation should account for error quality (not just accuracy), as LLMs make more semantically meaningful errors than neural models.
...
TL;DR: RAGShaper automates synthesis of RAG training data with adversarial distractors to improve agent robustness in noisy retrieval environments.


<details>
  <summary>Details</summary>
Motivation: Current RAG agent development lacks high-quality training data that reflects real-world retrieval noise and complexity. Manual annotation is unscalable and fails to capture dynamic reasoning strategies needed for handling retrieval failures.

Method: RAGShaper framework with InfoCurator builds dense information trees enriched with adversarial distractors (Perception and Cognition levels). Uses constrained navigation strategy forcing teacher agent to confront distractors, generating trajectories that demonstrate error correction and noise rejection.

Result: Models trained on synthesized corpus significantly outperform existing baselines, showing superior robustness in noise-intensive and complex retrieval tasks.

Conclusion: RAGShaper successfully bridges the data gap for robust RAG agent development by automating creation of realistic training data with adversarial challenges.
...
TL;DR: PrivGemo is a privacy-preserving framework for KG-grounded LLM reasoning that uses dual-tower design with anonymized views, memory-guided exposure control, and hierarchical controllers to protect private KGs while enabling multi-hop reasoning.


<details>
  <summary>Details</summary>
Motivation: Private knowledge graphs contain sensitive information, but current privacy methods (entity name masking) still risk structural leakage, uncontrollable remote interactions, fragile multi-hop reasoning, and limited experience reuse when using closed-source LLM APIs.

Method: Dual-tower design keeps raw KG knowledge local while enabling remote reasoning over anonymized views; retrieves anonymized long-hop paths connecting topic entities; uses hierarchical controller and privacy-aware experience memory to reduce unnecessary exploration.

Result: Achieves state-of-the-art results on six benchmarks, outperforming strongest baseline by up to 17.1%; enables smaller models (Qwen3-4B) to achieve reasoning performance comparable to GPT-4-Turbo.

Conclusion: PrivGemo effectively addresses privacy risks in KG-grounded LLM reasoning through comprehensive anonymization, structural protection, and efficient memory-guided exploration, making private KG reasoning practical and secure.
...
TL;DR: FRTR introduces a multimodal retrieval-augmented generation framework for reasoning over complex enterprise spreadsheets, achieving 74% accuracy on a new benchmark and 87% on existing benchmarks while reducing token usage by 50%.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with large-scale enterprise spreadsheets containing thousands of rows, multiple linked sheets, and embedded visual content. Existing approaches using single-sheet compression or full-context encoding lack scalability and don't reflect real user interaction with complex, multimodal workbooks.

Method: FRTR decomposes Excel workbooks into granular row, column, and block embeddings, employs hybrid lexical-dense retrieval with Reciprocal Rank Fusion (RRF), and integrates multimodal embeddings to reason over both numerical and visual information.

Result: Achieved 74% answer accuracy on FRTR-Bench with Claude Sonnet 4.5 (vs 24% for prior SOTA), 87% accuracy on SpreadsheetLLM benchmark with GPT-5, while reducing token usage by roughly 50% compared to context-compression methods.

Conclusion: FRTR provides an effective multimodal framework for spreadsheet reasoning that significantly outperforms existing approaches while being more efficient, addressing key limitations in LLM-based spreadsheet analysis.
...
TL;DR: Attributional NLI (Att-NLI) extends traditional NLI with social psychology principles to assess LLMs' ability to infer latent intentions in multi-agent environments, showing neuro-symbolic agents with abductive-deductive reasoning outperform others.


<details>
  <summary>Details</summary>
Motivation: Traditional natural language inference (NLI) fails to capture intention-driven reasoning needed for complex multi-agent systems, creating a gap in assessing LLMs' ability to infer latent intentions behind observed actions.

Method: Introduced Attributional NLI (Att-NLI) framework combining abductive intentional inference (generating hypotheses about intentions) and deductive verification. Tested via Undercover-V textual game with three agent types: standard NLI (deductive only), Att-NLI (abductive-deductive), and neuro-symbolic Att-NLI (abductive-deductive with external theorem provers).

Result: Clear hierarchy of attributional inference capabilities: neuro-symbolic agents consistently outperformed others, achieving average win rate of 17.08%.

Conclusion: Att-NLI is valuable for developing sophisticated reasoning agents, and neuro-symbolic AI shows significant potential for building rational LLM agents in multi-agent environments.
...
TL;DR: TableCache: Precomputing table representations as KV caches for Text-to-SQL to reduce context length and improve inference speed by 3.62x TTFT.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based Text-to-SQL methods include extensive database schemas in prompts, causing long context lengths and high prefilling latency. User queries typically focus on recurrent table sets, offering KV cache sharing opportunities, but current inference engines generate redundant prefix cache copies when processing queries with varying table orders.

Method: 1) Precompute table representations as KV caches offline and query required ones online; 2) Compute table caches while preserving primary-foreign key relationships; 3) Construct Table Trie structure for efficient KV cache lookups; 4) Implement cache management with query reranking to improve hit rates; 5) Use computation loading pipeline to parallelize model inference and cache loading.

Result: TableCache achieves up to 3.62x speedup in Time to First Token (TTFT) with negligible performance degradation compared to existing methods.

Conclusion: The proposed TableCache system effectively addresses inefficiencies in Text-to-SQL inference by precomputing table representations as KV caches, enabling significant speed improvements while maintaining query accuracy.
...
TL;DR: ACE framework dynamically decides when to retrieve vs reason, reducing unnecessary retrievals and improving efficiency/accuracy in multi-hop QA tasks.


<details>
  <summary>Details</summary>
Motivation: Current retrieval-augmented generation methods use rigid, brute-force retrieval at every step, causing computational inefficiency and performance degradation due to irrelevant noise in context.

Method: Agentic Context Evolution (ACE) uses a central orchestrator agent with majority voting to strategically decide between activating a retriever agent for external retrieval or a reasoner agent for internal analysis and refinement.

Result: ACE significantly outperforms competitive baselines in accuracy on challenging multi-hop QA benchmarks while achieving efficient token consumption.

Conclusion: ACE provides valuable insights for advancing context-evolved generation for complex, knowledge-intensive tasks by eliminating redundant retrieval steps and maintaining concise, evolved context.
...
TL;DR: This paper proposes an attention-based multimodal approach that combines aerial imagery and geolocated text from Wikipedia to predict environmental variables, outperforming unimodal baselines by leveraging spatial context.


<details>
  <summary>Details</summary>
Motivation: Textual data from sources like Wikipedia contains unique local-scale environmental information that complements traditional geospatial data, but integrating text with spatial data presents challenges due to sparse availability and unclear ecological applications.

Method: An attention-based model that combines vision (aerial imagery) and text representations with geolocation encoding, using an attention module to dynamically select useful spatial neighbors from nearby observations for predictive tasks.

Result: The approach consistently outperforms single-location and unimodal (image-only or text-only) baselines on predicting 103 environmental variables from the SWECO25 data cube, with significant improvements for climatic, edaphic, population, and land use/land cover variables.

Conclusion: Combining text and image data with spatial context through attention mechanisms provides substantial benefits for ecological prediction tasks, demonstrating the value of textual resources as complementary data sources in environmental science.
...
TL;DR: Multiplex Thinking is a stochastic soft reasoning method that samples multiple candidate tokens at each step, aggregates them into a single multiplex token, enabling compact representation of uncertainty without increasing sequence length, and allows direct optimization with RL.


<details>
  <summary>Details</summary>
Motivation: Current LLMs use Chain-of-Thought reasoning which produces long, low-bandwidth token sequences, while humans reason softly by maintaining distributions over plausible next steps. The authors aim to develop a more efficient reasoning mechanism that can compactly represent uncertainty.

Method: At each thinking step, sample K candidate tokens and aggregate their embeddings into a single continuous multiplex token. This preserves vocabulary embedding prior and sampling dynamics while creating tractable probability distributions over multiplex rollouts. The approach enables direct optimization with on-policy reinforcement learning.

Result: Across challenging math reasoning benchmarks, Multiplex Thinking consistently outperforms strong discrete CoT and RL baselines from Pass@1 through Pass@1024 while producing shorter sequences. The method is self-adaptive - behaves like standard CoT when confident, and compactly represents multiple plausible steps when uncertain.

Conclusion: Multiplex Thinking provides an efficient stochastic soft reasoning mechanism that combines the benefits of maintaining uncertainty distributions like humans while being computationally efficient and optimizable with RL, outperforming traditional discrete reasoning approaches.
...
TL;DR: LLM agent reviewers in Elo-ranked system improve Area Chair decision accuracy but adapt to exploit rating system without improving review quality.


<details>
  <summary>Details</summary>
Motivation: To explore how LLM agent reviewers behave in an Elo-ranked review system using real conference submissions, examining how rating systems and memory affect review dynamics.

Method: Simulated multi-round review interactions with LLM agents having different personas, moderated by Area Chair. Compared baseline with conditions incorporating Elo ratings and reviewer memory.

Result: Incorporating Elo ratings improves Area Chair decision accuracy. Reviewers adapt strategies to exploit the Elo system without actually improving their review effort.

Conclusion: Elo systems can improve decision-making in peer review but may lead to strategic behavior where reviewers optimize for ratings rather than review quality.
...
TL;DR: Edge AI roadside system for autonomous traffic violation detection using YOLOv8 Nano, DeepSORT, and OCR, achieving 97.7% accuracy at 30 FPS on Jetson Nano with V2X safety event dissemination.


<details>
  <summary>Details</summary>
Motivation: Address enforcement asymmetries in rapidly motorizing economies like India where 11M violations occur annually against limited human policing (1 officer per 4000 vehicles), requiring scalable autonomous solutions beyond traditional surveillance.

Method: Real-time roadside perception node integrating YOLOv8 Nano for multi-object detection, DeepSORT for vehicle tracking, and rule-guided OCR engine for degraded/multilingual license plate recognition compliant with MoRTH AIS 159/ISO 7591 standards. Deployed on NVIDIA Jetson Nano with TensorRT FP16 optimization.

Result: System sustains 28-30 FPS at 9.6W with 97.7% violation detection accuracy and 84.9% OCR precision across five violation classes (signal jumping, zebra crossing breach, wrong way driving, illegal U-turn, speeding). Outperforms YOLOv4 Tiny, PP YOLOE S, and Nano DetPlus by 10.7% mAP gain and 1.4× accuracy per watt.

Conclusion: Roadside edge AI analytics can autonomously detect traffic violations at scale while enabling cooperative perception and proactive safety management through V2X protocols (CAM/DENM messages) within intelligent transportation ecosystems.
...
TL;DR: ROAD benchmark evaluates LiDAR-based object classification under simultaneous domain and label shifts, revealing limitations of existing methods and establishing baselines for robust 3D perception.
...
Method: Proposed ROAD benchmark for LiDAR-based object classification that accounts for domain shifts and three forms of label evolution: class split, class expansion, and class insertion. Evaluated zero-shot transfer, linear probe, and continual learning methods using large-scale datasets (Waymo, NuScenes, Argoverse2), analyzing backbone architectures, training objectives, and CL methods.
...
Conclusion: The ROAD benchmark addresses a critical gap in 3D perception research by providing comprehensive evaluation under realistic domain and label shifts, highlighting the need for more robust methods and serving as a foundation for future work in adaptive 3D perception systems.
...
TL;DR: The paper introduces Lunara Aesthetic Dataset - a first-of-its-kind high-quality aesthetic image dataset with diverse artistic styles, human-refined prompts, and structured annotations, released under Apache 2.0 license.


<details>
  <summary>Details</summary>
Motivation: To address the gap in existing datasets that prioritize breadth over precision, lacking aesthetic quality, stylistic diversity, and licensing transparency for research and commercial use.

Method: Created dataset using Moonworks Lunara model to generate images embodying distinct aesthetic styles, with human-refined prompts and structured annotations describing objects, attributes, relationships, and stylistic cues.

Result: Produced a dataset with substantially higher aesthetic scores than existing aesthetics-focused and general-purpose datasets, covering diverse artistic styles from multiple regions and general categories.

Conclusion: The Lunara Aesthetic Dataset provides a valuable resource for research with its focus on aesthetic quality, stylistic diversity, and licensing transparency under Apache 2.0 for unrestricted academic and commercial use.
...
TL;DR: Lightweight CNN with multi-scale features, depthwise separable convolutions, and SE attention achieves 96.63% maize disease classification accuracy with minimal parameters (241K) and low computational cost (0.666 GFLOPs), enabling real-time deployment on edge devices.


<details>
  <summary>Details</summary>
Motivation: Traditional disease detection models have high computational costs that hinder deployment in resource-constrained environments like smartphones and drones used in precision farming, creating a need for lightweight yet accurate solutions.

Method: Proposed LWMSCNN-SE, a lightweight CNN that integrates multi-scale feature extraction, depthwise separable convolutions for efficiency, and squeeze-and-excitation (SE) attention mechanisms for improved feature representation.

Result: Achieved 96.63% classification accuracy with only 241,348 parameters and 0.666 GFLOPs computational cost, demonstrating excellent accuracy-efficiency trade-off suitable for real-time deployment.

Conclusion: The proposed lightweight model successfully addresses the accuracy-efficiency trade-off, making it suitable for real-time maize disease diagnosis on edge devices in precision farming systems.
...
TL;DR: 3DGS-Drag is a point-based 3D editing framework that enables intuitive drag manipulation of real 3D scenes using 3D Gaussian Splatting and diffusion guidance, achieving efficient geometry-related editing in 10-20 minutes on a single GPU.


<details>
  <summary>Details</summary>
Motivation: While drag editing with geometric changes has gained attention in 2D editing, it remains challenging for 3D scenes. Existing 3D editing methods have limitations in geometry-related content editing, creating a need for more intuitive and efficient 3D manipulation tools.

Method: The approach combines deformation guidance using 3D Gaussian Splatting for consistent geometric modifications with diffusion guidance for content correction and visual quality enhancement. It employs a progressive editing strategy to support aggressive 3D drag edits, bridging deformation-based and 2D-editing-based methods.

Result: The method enables diverse edits including motion change, shape adjustment, inpainting, and content extension. Experimental results show state-of-the-art performance in geometry-related 3D content editing, with efficient editing times of 10-20 minutes on a single RTX 4090 GPU.

Conclusion: 3DGS-Drag successfully addresses the challenge of intuitive drag manipulation for 3D scenes, providing an efficient framework that bridges different editing approaches and enables a wide range of geometry-related edits with high quality and practical efficiency.
...
TL;DR: The paper introduces an open-source sesame plant segmentation dataset with 292 images (206 train, 43 validation, 43 test) collected from Nigerian farms, annotated using SAMv2 with farmer supervision, and evaluated with YOLOv8 showing strong detection and segmentation performance.


<details>
  <summary>Details</summary>
Motivation: To address the lack of sesame-focused agricultural vision datasets in Nigeria and support AI model development for precise plant detection and analysis in real-world farm settings, moving beyond conventional bounding box approaches to pixel-level segmentation.

Method: Created dataset using high-resolution mobile camera from farms in Katsina State, Nigeria; annotated with Segment Anything Model version 2 with farmer supervision; formatted in YOLO-compatible segmentation format; evaluated using Ultralytics YOLOv8 framework for both detection and segmentation tasks.

Result: Dataset contains 292 total images (206/43/43 split). YOLOv8 evaluation showed: Detection - 79% recall, 79% precision, 84% mAP@0.50, 58% mAP@0.50:0.95; Segmentation - 82% recall, 77% precision, 84% mAP@0.50, 52% mAP@0.50:0.95.

Conclusion: The Sesame Plant Segmentation Dataset represents a novel contribution to agricultural vision datasets in Nigeria, enabling more precise sesame plant analysis for applications like plant monitoring, yield estimation, and agricultural research.
...
TL;DR: AKT transformer with PKAN modules improves maize localization in UAV imagery by addressing small object detection and computational efficiency challenges, achieving state-of-the-art performance on a new PML dataset.


<details>
  <summary>Details</summary>
Motivation: Point-level maize localization in UAV imagery faces three key challenges: extremely small object-to-pixel ratios (<0.1%), prohibitive computational costs of quadratic attention on ultra-high-resolution images (>3000x4000 pixels), and agricultural scene-specific complexities poorly handled by general vision models.

Method: Propose Additive Kolmogorov-Arnold Transformer (AKT) that replaces conventional MLPs with Pade Kolmogorov-Arnold Network (PKAN) modules for better small-object feature extraction, and introduces PKAN Additive Attention (PAA) to model multiscale spatial dependencies with reduced computational complexity. Also present Point-based Maize Localization (PML) dataset with 1,928 high-resolution UAV images and ~501,000 point annotations.

Result: AKT achieves average F1-score of 62.8%, outperforming SOTA by 4.2%, while reducing FLOPs by 12.6% and improving inference throughput by 20.7%. For downstream tasks: mean absolute error of 7.1 in stand counting, and RMSE of 1.95-1.97 cm in interplant spacing estimation.
...
<details>
  <summary>Details</summary>
Motivation: To create a more efficient statistical ideal observer model that performs holistic visual search/gist processing while reducing system complexity through parameter reduction.

Method: Develops a statistical ideal observer model that places thresholds on minimum extractable image features to perform holistic visual search, thereby reducing free parameters and shrinking the system.
...
Conclusion: The model provides a practical framework with broad applications in medical image perception, computer vision, defense/security systems, and performance benchmarking.
...
TL;DR: Combining Gabor and GLCM texture features improves modeling of early-stage visual search behavior in medical imaging, showing strong correlation between features and consistency with human gaze patterns.


<details>
  <summary>Details</summary>
Motivation: To better understand and model human visual search behavior in location-unknown tasks, particularly for medical image analysis, by investigating how different visual features (Gabor and GLCM) contribute to attention allocation.
...
Result: Qualitative agreement between proposed pipelines and model observer predictions, strong correlation between GLCM mean and Gabor feature responses, and consistency between predicted fixation regions and early-stage human gaze behavior from eye-tracking data.
...
TL;DR: CASHEW and CASHEW-RL improve vision-language model reasoning stability by aggregating multiple reasoning trajectories with visual verification and training models to self-aggregate with adaptive effort allocation.


<details>
  <summary>Details</summary>
Motivation: Vision-language models show strong performance but have unstable multi-step reasoning - repeated sampling produces divergent reasoning trajectories and inconsistent predictions.

Method: Two approaches: (1) CASHEW - inference-time framework that aggregates multiple candidate trajectories with visual verification to filter hallucinations, (2) CASHEW-RL - learned variant trained with Group Sequence Policy Optimization (GSPO) using composite reward for correct answers grounded in minimal visual evidence with adaptive reasoning effort.

Result: Significant performance improvements on 13 image/video understanding benchmarks, including +23.6pp on ScienceQA and +8.1pp on EgoSchema.

Conclusion: The approaches effectively stabilize vision-language model reasoning through trajectory aggregation and learned self-aggregation, achieving substantial gains across diverse multimodal tasks.
...
TL;DR: TP-Blend is a training-free framework that uses two separate prompts (object and style) to simultaneously edit both content and appearance in diffusion models, outperforming existing methods in quality and speed.


<details>
  <summary>Details</summary>
Motivation: Current text-conditioned diffusion editors handle single object replacement well but struggle when both a new object and a new style need to be introduced simultaneously, requiring a solution that can handle both content and appearance changes in one operation.

Method: TP-Blend uses two complementary attention processors: Cross-Attention Object Fusion (CAOF) for object replacement via entropy-regularized optimal transport, and Self-Attention Style Fusion (SASF) for style injection using Detail-Sensitive Instance Normalization and Key/Value matrix swapping.
...
Conclusion: TP-Blend successfully addresses the challenge of simultaneous object and style editing in diffusion models through a lightweight, training-free framework that maintains rich feature correlations while achieving superior editing performance.
...
<details>
  <summary>Details</summary>
Motivation: To create 3D structures that are not only geometrically valid but also manufacturable through additive manufacturing, respecting practical constraints that naive generation approaches often ignore.

Method: A novel decoder-based deep learning framework that learns to map latent representations to printable 3D geometries while incorporating manufacturing constraints such as overhang angles, wall thickness, and structural integrity.

Result: The approach produces parts with significantly improved manufacturability compared to naive generation methods, validated across diverse object categories with successful practical 3D printing of generated structures.

Conclusion: Neural decoders can effectively learn complex mapping functions from abstract representations to valid, manufacturable 3D geometries, enabling practical additive manufacturing applications.
...
<details>
  <summary>Details</summary>
Motivation: To challenge the established view that image-text alignment only appears in late layers of vision-language models, and to provide direct evidence of alignment at early layers.

Method: A synthesis-based method inspired by DeepDream: extract concept vectors for textual concepts at specific layers, then use optimization to synthesize images whose representations align with those vectors.

Result: More than 50% of synthesized images at layer 1 depict recognizable features of targeted concepts (animals, activities, seasons), showing meaningful alignment from the first layer.

Conclusion: Image-text alignment occurs from early layers in adapter-based vision-language models, providing constructive evidence for concept-by-concept alignment and offering a new path for model interpretability.
...
<details>
  <summary>Details</summary>
Motivation: Current zero-shot anomaly detection methods either rely on additional modalities like language for fine-grained prompts, or vision-only methods are limited to image-level classification without spatial precision for localization.

Method: Leverages inversion of a pretrained Denoising Diffusion Implicit Model (DDIM). Given an input image and generic text description, inverts the image to obtain latent representations, then initiates denoising from a fixed intermediate timestep to reconstruct a normal-looking image. The discrepancy between input and reconstruction highlights anomalies.

Result: Achieves state-of-the-art performance on VISA dataset, demonstrating strong localization capabilities without auxiliary modalities.

Conclusion: Presents a simple yet effective training-free vision-only framework that circumvents need for fine-grained prompts, facilitating shift away from prompt dependence in zero-shot anomaly detection research.
...
TL;DR: CBD is an efficient concept-based diversity metric using VLMs that strongly correlates with geometric diversity but is much faster. Combined with uncertainty metrics, it outperforms state-of-the-art selection methods for DNN fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Labeling new data for DNN fine-tuning is costly and time-consuming. Existing diversity-based selection methods are computationally intensive and lack scalability for large datasets, limiting practical application.

Method: Proposes Concept-Based Diversity (CBD), an efficient diversity metric leveraging Vision-Language Models for images. Combines CBD with Margin (uncertainty metric) in a hybrid selection approach.

Result: CBD strongly correlates with Geometric Diversity but requires much less computation time. CBD-based selection consistently outperforms five state-of-the-art baselines across diverse models, datasets, and budgets. Remains highly efficient even on large datasets like ImageNet.

Conclusion: CBD-based approach is effective, computationally advantageous, and scalable for repetitive input selection scenarios, offering a practical solution for efficient DNN fine-tuning.
...
TL;DR: FigEx2 is a visual-conditioned framework that localizes panels in scientific compound figures and generates panel-wise captions, using noise-aware fusion and reinforcement learning for improved performance.


<details>
  <summary>Details</summary>
Motivation: Scientific compound figures combine multiple panels but captions are often missing or only provide figure-level summaries, making panel-level understanding difficult.

Method: Visual-conditioned framework with noise-aware gated fusion module to filter token-level features, and staged optimization combining supervised learning with reinforcement learning using CLIP-based alignment and BERTScore rewards.
...
Conclusion: FigEx2 effectively addresses panel-level understanding in scientific compound figures with superior performance and strong transferability across scientific domains.
...
TL;DR: First vision-language framework for generating and detecting biomedical image forgeries using diffusion models and structured state space modeling, achieving SOTA performance on new benchmark Rescind.


<details>
  <summary>Details</summary>
Motivation: Scientific image manipulation in biomedical publications threatens research integrity and reproducibility, but detection is challenging due to domain-specific artifacts, complex textures, and unstructured figure layouts.

Method: Combines diffusion-based synthesis with vision-language prompting for realistic manipulations (duplication, splicing, region removal). Introduces Integscan framework using structured state space modeling with attention-enhanced visual encoding and prompt-conditioned semantic alignment for forgery localization. Includes VLM verification loop for semantic fidelity.

Result: Extensive experiments on Rescind benchmark and existing benchmarks demonstrate state-of-the-art performance in both detection and localization of biomedical image forgeries.

Conclusion: Establishes a strong foundation for automated scientific integrity analysis through the first vision-language guided framework for biomedical forgery generation and detection.
...
TL;DR: Strategic noise injection during CNN training improves robustness to corrupted test data with minimal clean-data performance impact.


<details>
  <summary>Details</summary>
Motivation: Real-world image data often contains noise and distortions, but CNNs are typically trained on clean data, creating a robustness gap. The paper investigates whether controlled noise exposure during training can improve model resilience to corrupted inputs.

Method: Used CIFAR-10 dataset with ResNet-18 model. Introduced three types of controlled noise (Gaussian noise, Salt-and-Pepper noise, Gaussian blur) at varying intensities and training set pollution levels (different percentages of noisy data). Evaluated impact on test performance under both clean and fully corrupted conditions.

Result: Incorporating just 10% noisy data during training significantly reduces test loss and improves accuracy under fully corrupted test conditions, while maintaining minimal impact on clean-data performance. This demonstrates noise can act as an effective regularizer.
...
TL;DR: DINO-AugSeg is a novel framework that leverages DINOv3 foundation model features with wavelet-based feature augmentation and contextual fusion for few-shot medical image segmentation, achieving state-of-the-art performance across multiple imaging modalities.
...
Conclusion: The framework demonstrates the effectiveness of wavelet-domain augmentation and contextual fusion for robust feature representation, offering a promising direction for advancing few-shot medical image segmentation. Code and data will be publicly available.
...
Result: Enables efficient construction of high-quality, deployable datasets for domain-specific applications while reducing dependency on extensive real-world data collection.

Conclusion: The proposed automated pipeline provides an effective solution for generating domain-specific synthetic datasets that bridge the gap between pre-trained models and real-world deployment, offering a scalable alternative to traditional data collection methods.
...
TL;DR: PathoGen is a diffusion-based model for generating realistic histopathology lesions through inpainting, addressing data scarcity in medical AI by creating synthetic training data with preserved tissue characteristics.


<details>
  <summary>Details</summary>
Motivation: The scarcity of expert-annotated histopathology data, especially for rare pathologies and underrepresented disease subtypes, severely limits robust AI model development. Existing data augmentation methods fail to generate realistic lesion morphologies with proper spatial relationships and cellular architectures.
...
Conclusion: PathoGen effectively overcomes the manual annotation bottleneck by simultaneously generating realistic morphology and pixel-level ground truth, offering a scalable pathway for developing generalizable medical AI systems despite limited expert-labeled data.
...
<details>
  <summary>Details</summary>
Motivation: Audio-visual semantic segmentation requires deeper semantic understanding beyond just identifying sound-emitting objects. Existing methods need improvement in handling both moving and stationary sound sources, as well as better cross-modal integration for contextual understanding.

Method: SSP decomposes AVSS into two subtasks: first providing prompted segmentation masks, then semantic analysis. It integrates optical flow for motion dynamics and textual prompts (object category + scene description). Includes visual-textual alignment module for cross-modal integration and uses post-mask training to learn optical flow patterns.
...
Conclusion: The SSP framework effectively addresses AVSS challenges by combining optical flow for motion context and textual prompts for semantic understanding, achieving superior segmentation performance through collaborative multi-modal integration.
...
TL;DR: SubTTA is a test-time adaptation method for vision-language models that addresses modality gaps and visual noise under distribution shifts by aligning semantic subspaces between visual and textual modalities.


<details>
  <summary>Details</summary>
Motivation: Vision-language models are vulnerable to distribution shifts, and existing TTA methods rely on unreliable zero-shot predictions as pseudo-labels due to modality gaps and visual nuisance (task-irrelevant noise) that overwhelm task-specific semantics.

Method: SubTTA aligns semantic subspaces of both modalities: 1) extracts principal subspaces and aligns visual manifold to textual semantic anchor by minimizing chordal distance to bridge modality gap, 2) projects aligned visual features onto task-specific textual subspace to filter out task-irrelevant noise, then performs standard TTA on purified space.

Result: Extensive experiments on various benchmarks and VLM architectures show SubTTA yields average improvement of 2.24% over state-of-the-art TTA methods.

Conclusion: SubTTA effectively addresses modality gaps and visual nuisance in VLMs under distribution shifts by semantic subspace alignment, improving test-time adaptation performance significantly over existing methods.
...
TL;DR: Systematic analysis reveals how MLLMs fuse visual-text information, showing fusion emerges at specific layers with late-stage visual reactivation, and proposes a training-free contrastive attention framework to improve multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: Despite remarkable progress in multimodal LLMs, there's poor understanding of how they internally integrate visual and textual information. The authors aim to bridge this gap by analyzing the internal fusion mechanisms.

Method: Performed systematic layer-wise masking analysis across multiple MLLM architectures to study visual-text fusion evolution. Analyzed layer-wise attention patterns and observed attention noise. Introduced a training-free contrastive attention framework that models transformation between early fusion and final layers to highlight meaningful attention shifts.

Result: Revealed that fusion emerges at specific layers rather than being uniformly distributed, with certain models showing late-stage "review" phenomenon where visual signals are reactivated before output generation. Observed persistent high-attention noise on irrelevant regions and gradually increasing attention on text-aligned areas. The proposed approach improves multimodal reasoning performance across various MLLMs and benchmarks.

Conclusion: The study provides valuable insights into MLLM fusion mechanisms and demonstrates that modeling attention transformations can enhance multimodal reasoning without additional training, offering a practical approach to improve existing MLLMs.
...
TL;DR: The paper introduces instance-aligned captions for explainable video anomaly detection that spatially ground textual claims to specific object instances with appearance/motion attributes, addressing limitations in current methods that lack verifiable spatial grounding.


<details>
  <summary>Details</summary>
Motivation: Current explainable VAD methods lack spatial grounding, making explanations unverifiable, especially in multi-entity interactions where existing methods produce incomplete or visually misaligned descriptions that reduce trustworthiness.

Method: Introduces instance-aligned captions that link each textual claim to specific object instances with appearance and motion attributes. The framework captures who caused the anomaly, what each entity was doing, whom it affected, and where the explanation is grounded.

Result: Created VIEW360+ dataset by extending VIEW360 with 868 additional videos, eight locations, and four new anomaly types. Experiments show instance-level spatially grounded captions reveal significant limitations in current LLM- and VLM-based methods while providing a robust benchmark.

Conclusion: The proposed framework enables verifiable and actionable reasoning for explainable VAD, addressing spatial grounding limitations and providing a comprehensive testbed for future research in trustworthy and interpretable anomaly detection.
...
TL;DR: Hardware-algorithm co-design framework combining custom SVE sensor with physics-aware dehazing algorithm to enable quantitative optical measurements in extreme rocket launch conditions.


<details>
  <summary>Details</summary>
Motivation: Quantitative optical measurement during rocket launch faces severe challenges due to extreme imaging conditions: intense combustion creates dense particulate haze and luminance variations exceeding 120 dB, degrading image data and undermining photogrammetric/velocimetric analyses.

Method: Proposes a hardware-algorithm co-design framework combining a custom Spatially Varying Exposure (SVE) sensor with a physics-aware dehazing algorithm. The SVE sensor acquires multi-exposure data in a single shot for robust haze assessment. The approach dynamically estimates haze density, performs region-adaptive illumination optimization, and applies multi-scale entropy-constrained fusion to separate haze from scene radiance.

Result: Validated on real launch imagery and controlled experiments, the framework demonstrates superior performance in recovering physically accurate visual information of the plume and engine region. Provides reliable image basis for extracting key mechanical parameters.

Conclusion: The proposed co-design framework enables reliable quantitative optical measurement of critical mechanical parameters (particle velocity, flow instability frequency, structural vibration) in extreme aerospace environments, supporting precise quantitative analysis during rocket launches.
...
TL;DR: SISTA improves medical VLP by addressing false negatives in contrastive learning and aligning image patches with word tokens at multiple levels.
...
Method: Proposes SISTA framework with multi-level alignment: 1) Image-report level: incorporates inter-report similarity to eliminate false negatives in contrastive learning, 2) Patch-word level: aligns image patches with relevant word tokens for fine-grained correspondence.

Result: Demonstrates improved transfer performance across different datasets on three downstream tasks: image classification, image segmentation, and object detection. Achieves significant improvements in fine-grained tasks even with limited labeled data.

Conclusion: SISTA effectively addresses the false negative problem in medical VLP and enables better semantic alignment at multiple levels, leading to superior representation learning for medical vision-language tasks.
...
<details>
  <summary>Details</summary>
Motivation: To develop a robust 3D object detection system that generalizes well across different sensor platforms by addressing domain gaps between source and target domains in autonomous driving scenarios.

Method: Built upon PVRCNN++ framework, enhanced with tailored data augmentation and self-training strategy using pseudo-labels to narrow domain gaps and improve cross-platform generalization.

Result: Achieved 3rd place in RoboSense2025 Challenge with 3D AP of 62.67% for Car (phase-1), and 58.76% for Car and 49.81% for Pedestrian (phase-2) on target domains.

Conclusion: The proposed domain adaptation approach effectively improves cross-platform 3D object detection performance, demonstrating the value of combining PVRCNN++ with targeted augmentation and self-training strategies for real-world autonomous driving applications.
...
TL;DR: CogniMap3D is a bioinspired framework for dynamic 3D scene understanding that mimics human cognition, using persistent memory of static scenes, motion detection, and optimization for continuous scene reconstruction across multiple visits.


<details>
  <summary>Details</summary>
Motivation: The paper aims to develop a more human-like approach to 3D scene understanding that can handle dynamic environments and maintain persistent knowledge across multiple visits to the same location, addressing limitations in traditional static scene reconstruction methods.

Method: The framework integrates three core components: 1) multi-stage motion cue framework for dynamic object identification using depth and camera pose priors, 2) cognitive mapping system for storing, recalling, and updating static scenes in a persistent memory bank, and 3) factor graph optimization for refining camera poses.

Result: CogniMap3D achieves state-of-the-art performance on video depth estimation, camera pose reconstruction, and 3D mapping tasks, and effectively supports continuous scene understanding across extended sequences and multiple visits to familiar locations.

Conclusion: The bioinspired cognitive approach enables robust dynamic 3D scene understanding with persistent memory, allowing efficient spatial knowledge storage, rapid retrieval, and continuous updates across multiple visits to the same environment.
...
TL;DR: A framework for instruction-driven facial expression generation that creates smooth 3D facial expression transitions between arbitrary expressions using text instructions.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D avatars have limited cardinal facial expressions; realistic emotional variation requires smooth transitions between arbitrary expressions, which text instructions can enable through diverse emotional descriptions.

Method: Introduces IFED module for multimodal learning between text and facial features, I2FET method with vertex reconstruction loss for semantic comprehension, and Facial Expression Transition model for smooth expression interpolation.
...
Conclusion: The framework enables expanded repertoire of facial expressions and transitions via text prompts, with practical applications in various domains.
...
TL;DR: The paper identifies theoretical flaws in existing corner detection methods, proposes using second-order Gaussian directional derivative filtering to analyze high-resolution corner models, and introduces a new corner detection method that outperforms state-of-the-art approaches.


<details>
  <summary>Details</summary>
Motivation: Existing corner detection methods have theoretical flaws - Zhang et al.'s simple corner model fails to account for grayscale interference between adjacent corners, limiting accuracy in high-resolution scenarios.

Method: Uses second-order Gaussian directional derivative (SOGDD) filter to smooth two high-resolution corner models (END-type and L-type), derives their SOGDD representations, discovers corner characteristics, and develops a new detection method with optimal Gaussian scale selection.

Result: The proposed method outperforms state-of-the-art methods in localization error, robustness to image blur transformation, image matching, and 3D reconstruction performance.

Conclusion: The SOGDD-based approach successfully addresses theoretical flaws in existing corner detection, enabling accurate detection of adjacent corners and improving performance in computer vision applications.
...
<details>
  <summary>Details</summary>
Motivation: To systematically evaluate MLLMs' performance across comprehensive gastrointestinal endoscopy workflows and compare them with human benchmarks, as current validation against clinical workflows is insufficient.

Method: Created GI-Bench benchmark with 20 lesion categories, evaluated 12 MLLMs across 5-stage clinical workflow (anatomical localization, lesion identification, diagnosis, findings description, management), compared against 3 junior endoscopists and 3 trainees using Macro-F1, mIoU, and Likert scales.

Result: Gemini-3-Pro achieved SOTA; top models outperformed trainees in diagnostic reasoning (Macro-F1 0.641 vs 0.492) and rivaled junior endoscopists (0.727). However, humans significantly outperformed models in lesion localization (mIoU >0.506 vs 0.345). Models showed "fluency-accuracy paradox" - better readability but lower factual correctness due to over-interpretation and hallucinations.

Conclusion: MLLMs show clinical utility in diagnostic reasoning but face critical spatial grounding bottlenecks and fluency-accuracy trade-offs, requiring further development for reliable clinical integration.
...
TL;DR: GPM (Global-to-Parallel Multi-scale Encoding) is a novel lightweight vision architecture inspired by human visual perception that achieves better balance between parameters, computation, and performance than existing models.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight vision models either sacrifice performance for computation reduction or increase parameters significantly. Current human vision-inspired models oversimplify visual processes. The paper aims to create a more balanced lightweight network that better mimics actual human visual perception mechanisms.

Method: Proposes GPM with Global Insight Generator (GIG) for holistic cues, then parallel processing: LSAE for mid-/large-scale semantic relations and IRB for fine-grained textures. This mimics human vision's "whole before details" and "contextual awareness during local attention." Builds H-GPE network based on GPM.

Result: H-GPE achieves strong performance on image classification, object detection, and semantic segmentation while maintaining balanced FLOPs and parameters. Outperforms recent SOTA lightweight models in accuracy-efficiency trade-off.

Conclusion: GPM successfully models human visual perception mechanisms (global-to-local processing with contextual awareness) to create lightweight networks with superior balance between efficiency and performance across multiple vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current medical image analysis systems using large vision-language models are mostly single-pass black boxes with limited control over reasoning, safety, and spatial grounding, creating reliability concerns for clinical applications.

Method: Four-agent framework: 1) Router configures task-aware prompts from image, patient history, and metadata; 2) Retriever uses exemplar memory and pass@k sampling to generate reports and bounding boxes; 3) Reflector critiques drafts for clinical error modes; 4) Repairer iteratively revises outputs under constraints while curating exemplars.
...
<details>
  <summary>Details</summary>
Motivation: Multi-site brain MRI data aggregation introduces non-biological heterogeneity (scanner differences, acquisition parameters) that undermines model generalizability. Existing methods rely on limited paired data, fail to disentangle style from anatomy, and only handle single sequences, limiting real-world applicability.
...
Result: Superior harmonization performance over state-of-the-art methods on 4,163 T1- and T2-weighted MRIs, demonstrated through image feature clustering, voxel-level comparison, tissue segmentation, and downstream age and site classification tasks.

Conclusion: MMH provides an effective solution for multi-site multi-sequence MRI harmonization that explicitly disentangles image styles from anatomical content without requiring paired data, enabling better utilization of aggregated multi-site data for deep learning applications.
...
TL;DR: MobiDiary generates natural language descriptions of daily activities from IMU and Wi-Fi signals using a unified sensor encoder and Transformer decoder, outperforming specialized baselines on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vision-based HAR systems face privacy concerns and environmental limitations like occlusion. There's a need for systems that can generate expressive, human-readable activity summaries from physical signals rather than just pre-defined labels.

Method: Proposes MobiDiary framework with unified sensor encoder that exploits shared inductive biases of motion-induced signals (IMU and Wi-Fi). Uses patch-based mechanism for local temporal correlations and heterogeneous placement embedding to unify spatial contexts. Transformer-based decoder with autoregressive mechanism generates coherent descriptions word-by-word.
...
TL;DR: FUME is a lightweight deep learning system that uses dual-gas optical imaging (CO2 and CH4) to detect ruminal acidosis in dairy cattle, achieving high accuracy with low computational cost.


<details>
  <summary>Details</summary>
Motivation: Current ruminal acidosis diagnosis relies on invasive pH measurements, which are not scalable for continuous monitoring. There's a need for non-invasive, practical detection methods to address economic losses and animal welfare concerns.

Method: FUME uses a lightweight dual-stream architecture with weight-shared encoders, modality-specific self-attention, and channel attention fusion. It jointly optimizes gas plume segmentation and health classification (Healthy, Transitional, Acidotic) from infrared camera images of CO2 and CH4 emissions.
...
Conclusion: Establishes feasibility of gas emission-based livestock health monitoring and paves way for practical in vitro acidosis detection systems, offering a non-invasive alternative to current invasive methods.
...
TL;DR: This paper compares multimodal approaches using Vision Transformer (EVA-ViT) image encoders with LLMs (LlaMA/ChatGPT) to reduce hallucinations and detect diseases in chest X-ray images, finding that text-based RAG reduces hallucinations while image-based RAG improves prediction confidence.


<details>
  <summary>Details</summary>
Motivation: The research aims to address the hallucination problem in multimodal medical AI systems and improve disease detection accuracy in chest X-ray images by comparing different retrieval-augmented generation (RAG) approaches with large language models.

Method: Used NIH Chest X-ray dataset, compared three approaches: image-based RAG, text-based RAG, and baseline. Employed EVA-ViT as image encoder with LlaMA or ChatGPT LLMs. Text-based RAG used external knowledge information, while image-based RAG used KNN methods for improved calibration.

Result: Text-based RAG effectively reduced hallucination problems using external knowledge. Image-based RAG improved prediction confidence and calibration via KNN methods. GPT LLM outperformed LlaMA with better performance, lower hallucination rate, and better Expected Calibration Error (ECE).

Conclusion: The research demonstrates challenges with data imbalance and complex multi-stage structures, but suggests the value of large experience environments and balanced example usage for improving multimodal medical AI systems.
...
TL;DR: Improves zero-shot ADL recognition using event-based segmentation and confidence estimation, outperforming time-based LLM approaches and supervised methods.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot ADL recognition methods using LLMs rely on time-based segmentation that doesn't align well with LLMs' contextual reasoning capabilities, and they lack confidence estimation methods for predictions.

Method: Proposes event-based segmentation instead of time-based segmentation for better alignment with LLM reasoning, and introduces a novel method for estimating prediction confidence in zero-shot ADL recognition.
...
Conclusion: Event-based segmentation and confidence estimation significantly improve zero-shot ADL recognition, making it more practical and reliable for smart home applications.
...
<details>
  <summary>Details</summary>
Motivation: Lack of standardized datasets has hindered progress in automatic intrapulse modulation classification (AIMC), which is critical for radar signal analysis in electronic support systems, especially under noisy/degraded conditions.

Method: Created AIMC-Spec dataset with 33 modulation types across 13 SNR levels. Re-implemented and evaluated 5 representative deep learning algorithms (lightweight CNNs, denoising architectures, transformer-based networks) under unified input format.

Result: Significant performance variation among algorithms; frequency-modulated (FM) signals classified more reliably than phase or hybrid types, especially at low SNRs. FM-only tests showed modulation type and network architecture influence classifier robustness.

Conclusion: AIMC-Spec establishes a reproducible baseline and provides foundation for future research and standardization in the AIMC domain.
...
TL;DR: HIPPO: Holistic-aware parallel speculative decoding for video-LLMs that achieves up to 3.51x speedup through semantic-aware token preservation and parallel draft generation/verification.


<details>
  <summary>Details</summary>
Motivation: Existing speculative decoding methods for video-LLMs don't achieve inference acceleration comparable to text-only LLMs due to inadequate preservation of visual semantic tokens and remaining high inference costs even with aggressive pruning.
...
TL;DR: Capsule networks achieve strong one-shot learning performance across industrial and face recognition domains using siamese architecture with stacked images.


<details>
  <summary>Details</summary>
Motivation: CNNs require large datasets and are computationally expensive, making them unsuitable for one-shot learning where only limited data is available. Special techniques are needed for predictions after seeing just one example per class.

Method: Uses siamese capsule networks with stacked images for one-shot identification tasks. Explores different approaches across industrial applications and face recognition domains.
...
Conclusion: Siamese capsule networks with stacked images provide an effective, easy-to-use and optimize solution for one-shot learning tasks across multiple domains.
...
TL;DR: MLLMs lack fundamental visual primitives that 6-7 year old children possess, scoring only 67.33 vs children's 95.32 on the KidVis benchmark, revealing a scaling law paradox where bigger models don't improve these basic visual capabilities.


<details>
  <summary>Details</summary>
Motivation: To investigate whether Multimodal Large Language Models possess fundamental visual primitives comparable to human intuition, despite their impressive performance on high-level reasoning tasks.

Method: Introduced KidVis benchmark based on human visual development theory, deconstructing visual intelligence into six atomic capabilities (Concentration, Tracking, Discrimination, Memory, Spatial, Closure) across 10 low-semantic visual tasks. Evaluated 20 state-of-the-art MLLMs against human physiological baseline of 6-7 year old children.

Result: Human children achieved near-perfect average score of 95.32, while state-of-the-art GPT-5 attained only 67.33. Revealed a "Scaling Law Paradox" where increasing model parameters doesn't yield linear improvements in foundational visual capabilities.

Conclusion: Current MLLMs lack essential physiological perceptual primitives required for generalized visual intelligence, despite their reasoning prowess, highlighting a fundamental gap in their visual understanding capabilities.
...
Method: Proposed M3SR architecture with multi-perceptual fusion block integrated into U-Net structure. The multi-perceptual fusion block enhances comprehensive feature understanding, while the U-Net enables extraction and fusion of global, intermediate, and local features at multiple scales.

Result: Extensive experiments show M3SR outperforms existing state-of-the-art methods while achieving lower computational cost, demonstrating superior quantitative and qualitative performance in hyperspectral image reconstruction.

Conclusion: The M3SR architecture successfully addresses limitations of single spatial perception and single-scale feature extraction in spectral reconstruction, providing an effective multi-scale, multi-perceptual approach with improved performance and efficiency.
...
TL;DR: ReCo-KD is a knowledge distillation framework that transfers anatomical details and contextual information from large teacher models to compact student networks for efficient 3D medical image segmentation, achieving near-teacher accuracy with significantly reduced computational requirements.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art 3D medical segmentation models are too large for clinics with limited computing resources, while lightweight alternatives suffer significant performance loss, creating a need for efficient models that maintain high accuracy.

Method: Region- and Context-aware Knowledge Distillation (ReCo-KD) with two components: Multi-Scale Structure-Aware Region Distillation (MS-SARD) that uses class-aware masks and scale-normalized weighting to emphasize clinically important small regions, and Multi-Scale Context Alignment (MS-CA) that aligns teacher-student affinity patterns across feature levels. Implemented on nnU-Net in a backbone-agnostic manner.
...
Conclusion: ReCo-KD provides a practical training-only framework for clinical deployment that doesn't require custom student design, is easily adaptable to other architectures, and enables efficient 3D medical segmentation with minimal performance loss.
...
TL;DR: Efficient diffusion transformer framework for mobile/edge devices with compact architecture, elastic training, and knowledge-guided distillation for real-time generation.


<details>
  <summary>Details</summary>
Motivation: Current diffusion transformers (DiTs) achieve state-of-the-art image generation but are too computationally expensive and memory-intensive for practical on-device deployment on mobile and edge devices with strict resource constraints.

Method: Three key components: 1) Compact DiT architecture with adaptive global-local sparse attention mechanism, 2) Elastic training framework that jointly optimizes sub-DiTs of varying capacities within a unified supernetwork, 3) Knowledge-Guided Distribution Matching Distillation (step-distillation pipeline combining DMD objective with knowledge transfer from few-step teacher models).
...
Conclusion: The proposed framework enables scalable, efficient, and high-quality diffusion models that can be practically deployed on diverse mobile and edge hardware while maintaining generation quality comparable to standard DiTs.
...
TL;DR: IQARAG is a training-free framework that enhances Large Multimodal Models' Image Quality Assessment ability using Retrieval-Augmented Generation with quality-variant reference images.


<details>
  <summary>Details</summary>
Motivation: Current LMMs show promise in IQA but require expensive fine-tuning for state-of-the-art performance. The authors aim to create a resource-efficient alternative that avoids computational costs of fine-tuning while maintaining strong IQA capability.
...
Result: Extensive experiments across KADID, KonIQ, LIVE Challenge, and SPAQ datasets demonstrate IQARAG effectively boosts LMMs' IQA performance, offering competitive results without fine-tuning.

Conclusion: IQARAG provides a resource-efficient, training-free alternative to fine-tuning for enhancing LMMs' Image Quality Assessment capabilities through retrieval-augmented generation with quality-variant reference images.
...
TL;DR: YOLOBirDrone architecture improves drone/bird detection accuracy with novel components and a new large-scale dataset, achieving ~85% accuracy.


<details>
  <summary>Details</summary>
Motivation: Drones are increasingly used for malicious attacks, creating safety concerns. Current vision-based detection systems struggle with accuracy, especially in distinguishing drones from birds, particularly small birds.

Method: Proposes YOLOBirDrone architecture with three key components: 1) Adaptive and Extended Layer Aggregation (AELAN), 2) Multi-scale Progressive Dual Attention Module (MPDA), and 3) Reverse MPDA (RMPDA) to preserve shape information and enrich features. Also introduces BirDrone dataset with small/challenging objects.
...
Conclusion: The proposed YOLOBirDrone architecture effectively addresses the drone/bird classification challenge and provides a robust solution for aerial object identification, supported by a comprehensive dataset.
...
TL;DR: UM-Text is a unified multimodal model for visual text editing using natural language instructions that understands context and generates style-consistent text with images.


<details>
  <summary>Details</summary>
Motivation: Previous visual text editing methods require complex manual specification of text attributes (font, color, layout) without considering stylistic consistency with reference images, lacking natural language understanding of context.
...
Conclusion: UM-Text provides an effective unified multimodal solution for natural language-based visual text editing that maintains stylistic consistency with reference images through context understanding and automated attribute configuration.
...
<details>
  <summary>Details</summary>
Motivation: GANs struggle to balance high-quality image generation with training stability. Existing methods like DCGAN, BigGAN, and StyleGAN improve visual fidelity but suffer from mode collapse and unstable gradients at deeper network depths.

Method: Introduces IGAN with deeper inception-inspired convolution and dilated convolution architectures. Uses dropout and spectral normalization in both generator and discriminator to mitigate gradient explosion and overfitting.

Result: Achieves FID scores of 13.12 (CUB-200) and 15.08 (ImageNet), representing 28-33% improvement over state-of-the-art GANs. Attains IS scores of 9.27 and 68.25, showing improved image diversity and quality.
...
TL;DR: BioMorphNet is a multimodal network that integrates whole slide images and spatial transcriptomics for tissue classification and differential gene analysis, outperforming existing methods on multiple cancer datasets.
...
Result: BioMorphNet improves average classification metrics by 2.67% for prostate cancer, 5.48% for colorectal cancer, and 6.29% for breast cancer compared to latest multimodal methods. It accurately classifies tissue categories and enables differential gene expression analysis for biomarker discovery.

Conclusion: BioMorphNet effectively integrates tissue morphology and spatial gene expression, advancing precision cancer diagnosis by supporting both tumor localization and biomarker discovery through improved classification and differential gene analysis capabilities.
...
TL;DR: IET introduces Individualized Exploratory Attention for efficient super-resolution, allowing tokens to adaptively select attention candidates instead of using fixed groups.


<details>
  <summary>Details</summary>
Motivation: Transformer-based SISR methods have high computational costs due to feature-intensive attention. Existing efficiency improvements use fixed group-wise attention, which ignores token similarity asymmetry and lacks token-adaptive flexibility.

Method: Proposes Individualized Exploratory Transformer (IET) with Individualized Exploratory Attention (IEA) mechanism. Each token adaptively selects its own content-aware, independent attention candidates, enabling token-adaptive asymmetric attention computation.
...
Conclusion: IET's token-adaptive asymmetric attention design enables more precise information aggregation with computational efficiency, addressing limitations of fixed group-wise attention in transformer-based super-resolution.
...
TL;DR: VLMs show strong performance but have poor robustness to realistic perception degradation, causing semantic misalignment despite moderate drops in segmentation metrics.


<details>
  <summary>Details</summary>
Motivation: VLMs are increasingly used in safety-critical applications like autonomous driving, but their robustness to realistic perception degradation is poorly understood, creating potential safety risks.

Method: Systematically study semantic misalignment in VLMs under controlled degradation of visual perception using semantic segmentation on Cityscapes dataset, introducing perception-realistic corruptions and proposing language-level misalignment metrics.

Result: Perception-realistic corruptions cause only moderate drops in segmentation metrics but lead to severe VLM failures including hallucinated objects, omission of safety-critical entities, and inconsistent safety judgments across multiple VLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing in-the-wild novel view synthesis methods often lack geometric grounding on complex surfaces, producing results with inconsistencies. The authors aim to address this limitation by creating a geometry-aware framework that ensures geometric coherence while maintaining high rendering quality.

Method: The framework leverages an underlying geometric representation based on Signed Distance Function (SDF) to guide the rendering process. It introduces a novel Geometry-Preservation Loss to ensure fine structural details are preserved during synthesis.
...
Conclusion: Geo-NVS-w is a robust method for in-the-wild novel view synthesis that successfully addresses geometric inconsistency issues while maintaining high rendering quality and significantly reducing computational energy consumption.
...
TL;DR: LoGo is a source-free unsupervised domain adaptation framework for 3D geospatial point clouds that addresses domain shift without access to source data, using local-global dual-consensus approach with class-balanced prototypes and optimal transport alignment.


<details>
  <summary>Details</summary>
Motivation: Geospatial point cloud segmentation suffers from domain shifts across regions and acquisition strategies. Existing domain adaptation methods require source data, which is often unavailable due to privacy, policy, and transmission limitations, motivating source-free UDA.

Method: LoGo framework has three components: 1) Local class-balanced prototype estimation using intra-class independent anchor mining instead of global threshold filtering, 2) Global optimal transport-based distribution alignment for pseudo-label assignment, 3) Dual-consistency pseudo-label filtering that retains only high-confidence labels where local ensemble predictions match global OT assignments.

Result: The method effectively mitigates feature collapse in tail classes caused by long-tailed distributions, corrects over-dominance of head classes in predictions, and generates robust pseudo-labels for self-training without source data access.

Conclusion: LoGo provides a novel SFUDA solution for geospatial point clouds that addresses domain adaptation challenges without source data, handling long-tailed distributions through local-global consensus and optimal transport alignment.
...
TL;DR: A low-cost GSM-IoT smart pet feeder using Arduino, SIM800L module, ultrasonic sensor, and servo mechanism enables remote feeding control via SMS without internet, achieving 98% command success rate with consistent portion dispensing.


<details>
  <summary>Details</summary>
Motivation: Pet owners, especially busy urban residents, struggle with maintaining consistent feeding schedules. There's a need for accessible, internet-independent solutions for remote pet feeding management.

Method: Developed a smart feeder using Arduino microcontroller, SIM800L GSM module for cellular communication, ultrasonic sensor for food-level monitoring, and servo mechanism for portion dispensing. Created a mobile app with MIT App Inventor for remote control via SMS commands.

Result: Achieved 98% SMS command success rate, consistent portion dispensing with ±2.67% variance, and reliable autonomous operation. The system works without internet, making it accessible for resource-limited households.

Conclusion: The GSM-IoT smart pet feeder provides a practical, scalable, internet-independent solution for personalized pet feeding, setting a new benchmark for low-cost, GSM-powered automation in smart pet products.
...
<details>
  <summary>Details</summary>
Motivation: To overcome challenges in diagnosing pericoronitis on panoramic radiographs by creating an AI-assisted system that integrates anatomical localization, pathological classification, and interpretability for clinical trust.

Method: Two-stage deep learning pipeline: 1) YOLOv8 detects third molars and classifies anatomical positions/angulations using Winter's classification; 2) Modified ResNet-50 classifier detects radiographic features of pericoronitis; Grad-CAM provides interpretability by highlighting diagnostic regions.

Result: YOLOv8 achieved 92% precision and 92.5% mAP; ResNet-50 yielded F1-scores of 88% (normal) and 86% (pericoronitis); Radiologists reported 84% alignment between Grad-CAM and their diagnostic impressions.

Conclusion: The system shows strong potential for AI-assisted panoramic assessment with explainable AI features that support clinical confidence.
...
Method: Three key approaches: 1) Deep integration of BLIP-2 with YOLO-World and YOLOv8-Seg models for object detection and segmentation, 2) Content-aware key frame sampling using K-Means clustering with intelligent frame selection and temporal feature concatenation for video tasks, 3) Unified prompt optimization scheme that injects structured YOLO event logs as context and uses output constraints to filter technical details.

Result: The platform extends BLIP-2's multi-task capabilities for UAV applications with minimal adaptation, enabling effective handling of video-level interactive tasks while maintaining lightweight architecture suitable for edge devices.

Conclusion: The proposed lightweight multimodal platform successfully addresses UAV computational constraints by integrating BLIP-2 with YOLO models and implementing intelligent frame sampling and prompt optimization, enabling real-time visual understanding without task-specific fine-tuning on drone data.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-camera 3D reconstruction methods struggle with multi-view fusion, camera extrinsic uncertainty, and scalability for large camera setups, limiting real-time applications in 3D perception, immersive interaction, and robotics.
...
Result: Extensive experiments on real-world multi-camera systems show SPARK outperforms existing approaches in extrinsic accuracy, geometric consistency, temporal stability, and real-time performance, demonstrating effectiveness and scalability for large-scale setups.

Conclusion: SPARK provides a practical solution for real-time multi-camera 3D reconstruction that addresses key challenges of extrinsic uncertainty and fusion quality while maintaining linear scalability with camera count, making it suitable for dynamic scenes and large-scale applications.
...
TL;DR: MMLGNet is a novel multimodal framework that aligns remote sensing data (HSI and LiDAR) with natural language using CLIP-inspired vision-language models for semantic understanding.


<details>
  <summary>Details</summary>
Motivation: With increasing availability of multimodal Earth observation data, there's a need for methods that effectively fuse spectral, spatial, and geometric information while enabling semantic-level understanding of remote sensing data.
...
Conclusion: MMLGNet successfully bridges the gap between high-dimensional remote sensing data and language-guided interpretation, showing the value of language supervision for multimodal remote sensing analysis.
...
TL;DR: A patch-based facial animation retargeting method that transfers expressions from source video to stylized 3D characters while preserving semantic meaning across different facial structures.


<details>
  <summary>Details</summary>
Motivation: Existing retargeting methods work well for similar facial shapes but struggle with stylized/exaggerated 3D characters that deviate from human facial structures. There's a need to preserve semantic meaning of expressions when retargeting to characters with different facial proportions.
...
Conclusion: The proposed local patch-based retargeting method effectively addresses the challenge of preserving expression semantics when transferring facial animations to stylized 3D characters with different facial structures.
...
TL;DR: CardiacMind enhances MLLMs for echocardiography diagnosis using Cardiac Reasoning Templates and reinforcement learning with novel rewards, achieving 48% improvement in complex cardiac disease diagnosis.


<details>
  <summary>Details</summary>
Motivation: Current echocardiography foundation models fail to capture relationships between measurements and clinical manifestations, while MLLMs require expensive detailed reasoning path construction and can't effectively incorporate echocardiographic priors into reasoning.
...
Conclusion: The proposed approach effectively enhances MLLM's echocardiographic reasoning by introducing cardiologist-like mindset through structured templates and reinforcement learning, significantly improving diagnostic performance and clinical alignment.
...
TL;DR: NAR is a noise-adaptive regularization method for remote sensing multi-label classification that explicitly distinguishes between additive and subtractive noise types, using confidence-based label handling and early-learning regularization to improve robustness against noisy annotations.


<details>
  <summary>Details</summary>
Motivation: Remote sensing multi-label classification increasingly relies on cost-effective but noisy annotations from thematic products or crowdsourcing, which introduce additive, subtractive, or mixed noise. Previous methods treat all noise uniformly without adapting to different noise types, lacking mechanisms to explicitly handle these distinctions.

Method: NAR uses a confidence-based label handling mechanism that dynamically: (1) retains high-confidence labels, (2) temporarily deactivates moderate-confidence labels, and (3) flips low-confidence labels. This selective attenuation is integrated with early-learning regularization (ELR) to stabilize training and prevent overfitting to corrupted labels within a semi-supervised learning framework.

Result: Experiments across additive, subtractive, and mixed noise scenarios show NAR consistently improves robustness compared to existing methods. Performance improvements are most significant under subtractive and mixed noise conditions, demonstrating the effectiveness of adaptive noise suppression and selective correction.

Conclusion: NAR's explicit distinction between additive and subtractive noise types, combined with confidence-based label handling and ELR integration, provides an effective strategy for noise-robust learning in remote sensing multi-label classification, particularly valuable for handling noisy annotations from cost-effective labeling procedures.
...
Method: Proposes Static-Dynamic Collaboration (SDC) framework with two stages: 1) Static Retaining Stage (SRS) trains initial model with base session data and preserves key parts as static memory; 2) Dynamic Learning Stage (DLS) introduces extra dynamic projector jointly trained with static memory for incremental learning.
...
Conclusion: The SDC framework effectively addresses the stability-plasticity dilemma in FSCIL by separating static knowledge retention from dynamic incremental learning, achieving superior performance through this collaborative approach.
...
TL;DR: This study developed a radiomics framework using CT imaging and machine learning to predict neoadjuvant chemotherapy response in high-grade serous ovarian cancer, achieving AUC up to 0.83 by integrating robustness into feature selection.


<details>
  <summary>Details</summary>
Motivation: HGSOC is often diagnosed at advanced stages with peritoneal metastases, and about 40% of patients show limited response to neoadjuvant chemotherapy. There's a need for non-invasive methods to predict treatment response to guide clinical decisions.

Method: Developed a radiomics framework with automated randomization algorithm to ensure feature robustness against inter-observer variability. Used pre- and post-NACT CT scans, extracted features from lesions in different anatomical sites, and employed four response metrics (CRS, RECIST, VolR, DiaR). Trained models on one cohort and externally validated on an independent cohort.

Result: Best prediction performance was AUC 0.83 for volume reduction (VolR) using all lesions combined. Omental lesions performed best for CRS prediction (AUC 0.77), and pelvic lesions for diameter reduction (AUC 0.76). The robustness-integrated feature selection improved model reliability.

Conclusion: Integrating robustness into feature selection ensures reliable radiomics models for clinical implementation in HGSOC. Future work should explore real-time clinical applications of radiomics in ovarian cancer management.
...
<details>
  <summary>Details</summary>
Motivation: RGB-T detection needs to handle both modality fusion for complementary information and modality separation when one modality is degraded under extreme conditions. Existing methods struggle with balancing these two needs and require paired RGB-T data for training.

Method: Uses DETR-like detectors as separate branches for RGB and TIR images with query fusion between branches at each refinement stage. High-quality queries from one branch are selected, adapted, and fed to the other branch to exclude degraded modality and correct predictions. The decoupled framework allows training with unpaired RGB or TIR images.

Result: Extensive experiments show superior performance compared to existing RGB-T detectors and better modality independence. The framework effectively handles extreme conditions where one modality is degraded.

Conclusion: MDQF successfully balances modality complementation and separation through a decoupled framework with query fusion, enabling robust RGB-T detection under diverse conditions while eliminating the need for paired training data.
...
TL;DR: Proposes CoMa-20K dataset for automated building massing generation, benchmarks using Vision-Language Models, shows task complexity but potential for context-sensitive design.


<details>
  <summary>Details</summary>
Motivation: Architectural conceptual design (especially building massing) is complex, intuition-driven, and manual. There's a lack of suitable datasets for data-driven methods in this domain.

Method: Introduces CoMa-20K dataset with massing geometries, economic/programmatic data, and site context visuals. Benchmarks by formulating massing generation as conditional task for Vision-Language Models, evaluating both fine-tuned and zero-shot models.

Result: Experiments reveal inherent complexity of massing generation task but demonstrate VLMs' potential to produce context-sensitive massing options. Dataset establishes foundational benchmark.

Conclusion: CoMa-20K dataset and analysis provide foundation for future research in data-driven architectural design, highlighting significant opportunities in automated, context-aware building massing generation.
...
TL;DR: Proposes a subject decoupling framework for distracted driver detection using vision-language models, addressing appearance bias by extracting and removing driver-specific features before classification.


<details>
  <summary>Details</summary>
Motivation: Distracted driving causes traffic collisions, but existing VLM-based detectors underperform in real-world conditions due to subject-specific appearance variations (clothing, age, gender) that bias models toward who the driver is rather than what they're doing.

Method: A subject decoupling framework that extracts driver appearance embeddings and removes their influence from image embeddings before zero-shot classification. Also uses metric projection onto Stiefel manifold to orthogonalize text embeddings for better separability while preserving semantics.

Result: Experiments show consistent gains over prior baselines, demonstrating improved performance for practical road-safety applications.

Conclusion: The proposed approach effectively addresses appearance bias in distracted driver detection, showing promise for robust real-world deployment by focusing on behavior cues rather than subject characteristics.
...
TL;DR: HazardForge pipeline generates hazardous driving scenarios using image editing models, creating MovSafeBench benchmark with 7,254 images to evaluate VLMs' safety decision-making in autonomous vehicles.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks inadequately cover diverse hazardous situations in autonomous driving, especially anomalous scenarios with spatio-temporal dynamics. There's a need to better evaluate VLMs' ability to support safer decision-making in complex environments.

Method: HazardForge pipeline leverages image editing models with layout decision algorithms and validation modules to generate realistic hazardous scenarios including moving, intrusive, and distant objects. Creates MovSafeBench benchmark with 7,254 images across 13 object categories.

Result: Experiments show VLM performance degrades notably under conditions including anomalous objects, with the largest performance drop in scenarios requiring nuanced motion understanding.

Conclusion: HazardForge enables scalable generation of realistic hazardous scenarios, and MovSafeBench reveals critical weaknesses in current VLMs' ability to handle anomalous objects and motion understanding for safer autonomous decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Existing zero-shot OOD detection methods rely on fixed textual proxies that sparsely sample semantic space and remain static while visual features drift, leading to cross-modal misalignment and unstable predictions.

Method: CoEvo introduces bidirectional, sample-conditioned adaptation with proxy-aligned co-evolution: maintains two evolving proxy caches that dynamically mine contextual textual negatives guided by test images and iteratively refine visual proxies, then dynamically re-weights dual-modal contributions for calibrated OOD scores.

Result: Achieves state-of-the-art performance on standard benchmarks, improving AUROC by 1.33% and reducing FPR95 by 45.98% on ImageNet-1K compared to strong negative-label baselines.

Conclusion: CoEvo effectively addresses cross-modal misalignment in zero-shot OOD detection through dynamic proxy adaptation, demonstrating superior performance without requiring training or annotations.
...
<details>
  <summary>Details</summary>
Motivation: Traditional manual aquarium maintenance is inefficient, labor-intensive, and error-prone, leading to suboptimal aquatic conditions that require continuous monitoring of multiple water quality parameters.

Method: Integrated ESP32 microcontroller with pH, TDS, temperature, and turbidity sensors, plus servo feeder and water pump actuators. Features edge processing, cloud connectivity via Blynk IoT platform, and intelligent alert mechanisms with configurable cooldown periods.
...
TL;DR: PKI proposes a prior knowledge-infused neural network with cascading projectors for few-shot class-incremental learning, addressing catastrophic forgetting and overfitting while offering resource-efficient variants.


<details>
  <summary>Details</summary>
Motivation: Address two key challenges in FSCIL: catastrophic forgetting of old classes and overfitting to new classes with limited examples. Existing methods freeze network components and use memory, but need better prior knowledge utilization.

Method: PKI consists of backbone, ensemble of projectors, classifier, and memory. Each session adds new projector to ensemble, finetunes new projector and classifier while freezing other components. Two variants (PKIV-1, PKIV-2) reduce projector count for resource efficiency.

Result: Extensive experiments on three popular benchmarks show PKI outperforms state-of-the-art methods in few-shot class-incremental learning.

Conclusion: PKI effectively integrates prior knowledge through cascading projectors, enabling flexible learning of new classes while preserving old knowledge, with resource-efficient variants offering performance-resource tradeoffs.
...
<details>
  <summary>Details</summary>
Motivation: Large models like Vision Transformers have superior few-shot performance but require extensive GPU memory and training time, making them impractical for low-resource scenarios. There's a need to bridge this gap between performance and computational efficiency.

Method: Proposes EfficientFSL with three key components: 1) Lightweight trainable Forward Block to synthesize task-specific queries that extract features from pre-trained model representations, 2) Combine Block to fuse multi-layer outputs for enhanced feature depth, and 3) Support-Query Attention Block to mitigate distribution shift by aligning prototypes with query set distribution.

Result: Achieves state-of-the-art performance on four in-domain few-shot datasets and six cross-domain datasets while using minimal trainable parameters, significantly reducing computational overhead compared to full fine-tuning.

Conclusion: EfficientFSL effectively bridges the gap between performance and computational efficiency in few-shot learning with Vision Transformers, making large models practical for real-world low-resource applications through query-only fine-tuning.
...
TL;DR: LLM-driven Neural Architecture Search for channel configuration optimization using code generation and AST mutations to overcome data scarcity.


<details>
  <summary>Details</summary>
Motivation: Channel configuration optimization is a complex combinatorial problem in neural architecture search. Traditional heuristics lack reasoning capabilities about architectural code structure, while LLMs offer transformative potential for NAS by understanding and generating architectural specifications.

Method: Formulates channel configuration search as sequence of conditional code generation tasks where LLM refines architectural specs based on performance telemetry. Addresses data scarcity by generating large corpus of valid architectures via Abstract Syntax Tree mutations to provide structural data for LLM learning.
...
Conclusion: LLM-driven NAS demonstrates immense potential for language-driven design in deep learning, with LLMs capable of learning complex design patterns and applying them to optimize feature extraction strategies.
...
Method: Proposes Constrained Dataset Distillation (CD²) with two modules: Dataset Distillation Module (DDM) synthesizes condensed samples guided by classifier to learn compact class-related clues, and Distillation Constraint Module (DCM) uses designed loss to constrain previously learned class distribution to preserve distilled knowledge.

Result: Extensive experiments on three public datasets demonstrate superiority over other state-of-the-art competitors.

Conclusion: The CD² framework effectively addresses catastrophic forgetting in FSCIL by distilling essential knowledge from few incremental samples while preserving previous class distributions through constraint mechanisms.
...
TL;DR: VideoHEDGE: A framework for detecting hallucinations in video-VLMs using semantic entropy from perturbed video variants and clustering textual outputs.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in video-VLMs are frequent and high-confidence, but existing uncertainty metrics fail to align with correctness, requiring better detection methods.

Method: Generates baseline and high-temperature answers from clean and perturbed video clips, clusters outputs into semantic hypotheses using NLI or embedding methods, and computes three reliability scores (SE, RadFlag, VASE).

Result: VASE achieves highest ROC-AUC across three 7B Video-VLMs, especially with larger distortion budgets, while embedding-based clustering matches NLI performance at lower cost.

Conclusion: VideoHEDGE effectively detects hallucinations in video-VLMs, with VASE being most reliable, and provides a reproducible framework for benchmarking.
...
TL;DR: REVNET is a rotation-equivariant anchor transformer framework for robust point cloud completion under arbitrary rotations, using Vector Neuron networks to handle incomplete 3D data without requiring pose alignment.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud completion methods are rotation-variant and require canonical pose training, limiting real-world applicability. Data augmentation with random rotations increases learning burden and doesn't guarantee robust performance under arbitrary poses.
...
Conclusion: REVNET provides a robust solution for point cloud completion under arbitrary rotations, addressing limitations of existing rotation-variant methods and enabling real-world applicability without pose alignment requirements.
...
TL;DR: MoCha is a novel framework for controllable video character replacement that only needs a single arbitrary frame mask, overcoming limitations of prior reconstruction-based methods that require per-frame masks and explicit structural guidance.
...
Conclusion: MoCha presents a pioneering framework that bypasses limitations of prior methods by requiring only a single arbitrary frame mask, with promising results for controllable video character replacement even in complex scenarios.
...
TL;DR: WaveFormer: A wave-based vision model using wave propagation operators for efficient global feature modeling, achieving competitive performance with higher throughput than attention-based Transformers.


<details>
  <summary>Details</summary>
Motivation: Current vision Transformers lack principled understanding of how semantic information propagates spatially. Attention mechanisms capture dependencies but don't model spatial frequency interactions with network depth in a controlled way.

Method: Treat feature maps as spatial signals evolving over propagation time (network depth) governed by underdamped wave equation. Derive closed-form frequency-time decoupled solution implemented as Wave Propagation Operator (WPO) - lightweight module modeling global interactions in O(N log N) time. Build WaveFormer models as drop-in replacements for ViTs and CNNs.

Result: Competitive accuracy across image classification, object detection, and semantic segmentation. Achieves up to 1.6x higher throughput and 30% fewer FLOPs than attention-based alternatives. Wave propagation introduces complementary modeling bias to heat-based methods, capturing both global coherence and high-frequency details.

Conclusion: Wave-based perspective provides principled framework for spatial information propagation in vision models. WaveFormer offers efficient alternative to attention with better computational efficiency while maintaining competitive performance across vision tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional radiomics are interpretable but restrictive, while deep learning lacks interpretability. Clinical adoption requires both accuracy and interpretability for automated knee MRI assessment.

Method: Two complementary strategies: 1) Radiomic fingerprints - patient-specific feature selection using image-conditioned predictor to estimate feature relevance probabilities, with transparent logistic regression. 2) Healthy personas - diffusion models synthesize pathology-free knee MRIs for each patient to highlight deviations from normal anatomy.

Result: Both approaches achieve performance comparable to or surpassing state-of-the-art DL models across three clinical tasks, while supporting multi-level interpretability. Case studies demonstrate human-explainable biomarker discovery and pathology localization.

Conclusion: The proposed radiomic fingerprints and healthy personas provide accurate and interpretable solutions for knee MRI analysis, addressing clinical needs for both performance and explainability in automated assessment.
...
TL;DR: SfMamba is a new framework for source-free domain adaptation that uses Visual Mamba with channel-wise scanning and semantic-consistent shuffling to achieve better domain alignment with parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing SFDA methods face trade-offs between perception field and computational efficiency in domain-invariant feature learning. Visual Mamba shows promise but lacks channel-wise frequency capture and spatial robustness under domain shifts.

Method: Proposes SfMamba with two key components: 1) Channel-wise Visual State-Space block for channel-sequence scanning to extract domain-invariant features, and 2) Semantic-Consistent Shuffle strategy that disrupts background patch sequences while preserving prediction consistency to mitigate error accumulation.
...
Conclusion: SfMamba offers a practical solution for SFDA by fully exploring stable dependencies in source-free model transfer through enhanced Visual Mamba architecture with channel-wise processing and semantic-preserving shuffling.
...
TL;DR: The paper proposes Semantic Orthogonal Calibration (SoC), a Huber-based regularizer that improves uncertainty calibration in vision-language models during test-time prompt-tuning, addressing overconfidence issues in prior orthogonality-based approaches.


<details>
  <summary>Details</summary>
Motivation: As vision-language models are increasingly used in critical decision-making systems like healthcare and autonomous driving, accurate uncertainty calibration becomes crucial. However, current test-time prompt-tuning literature has focused mainly on discriminative performance, largely neglecting uncertainty calibration. Existing orthogonal constraint methods push semantically related classes apart, leading to overconfidence.

Method: The authors propose Semantic Orthogonal Calibration (SoC), a Huber-based regularizer that enforces smooth prototype separation while preserving semantic proximity. This approach addresses the limitations of fully orthogonal constraints that strongly push semantically related classes away, which the authors theoretically demonstrate leads to overconfidence.

Result: Across comprehensive empirical validation, SoC consistently improves calibration performance while maintaining competitive discriminative capabilities compared to prior orthogonality-based approaches.

Conclusion: The proposed Semantic Orthogonal Calibration method effectively addresses the calibration issues in vision-language model test-time prompt-tuning by balancing prototype separation with semantic preservation, leading to better uncertainty estimates without sacrificing discriminative performance.
...
TL;DR: CtrlFuse is a controllable infrared-visible image fusion framework that uses mask prompts to guide dynamic fusion, enabling interactive adaptation to different semantic perception needs while enhancing both fusion quality and downstream task performance.


<details>
  <summary>Details</summary>
Motivation: Existing fusion methods either focus only on pixel-level fusion without considering downstream task adaptability, or use rigid cascaded detection/segmentation models that cannot interactively address diverse semantic target perception needs. There's a need for a controllable fusion approach that can dynamically adapt to different semantic requirements.
...
Result: Achieves state-of-the-art results in both fusion controllability and segmentation accuracy. The adapted task branch even outperforms the original segmentation model, demonstrating mutual enhancement between task performance and fusion quality.

Conclusion: CtrlFuse enables interactive dynamic fusion guided by mask prompts, successfully addressing the limitations of existing methods by providing controllable fusion that adapts to diverse semantic perception needs while improving both fusion quality and downstream task performance.
...
Method: SafeRedir uses prompt embedding redirection at inference time with two components: 1) latent-aware multi-modal safety classifier to identify unsafe generation trajectories, and 2) token-level delta generator for semantic redirection with auxiliary predictors for token masking and adaptive scaling.
...
Conclusion: SafeRedir provides a plug-and-play compatible framework for robust unlearning without model modification, addressing safety risks in image generation models while maintaining generation quality and robustness.
...
TL;DR: Proposes a reliability assessment framework for deepfake detection with four pillars: transferability, robustness, interpretability, and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Deepfakes have both positive applications and serious negative impacts. Current detection techniques lack comprehensive evaluation methods beyond classification performance.
...
Result: Analysis revealed significant progress in deepfake detection methods but also identified critical limitations through the proposed evaluation framework.

Conclusion: A comprehensive reliability assessment framework is needed to better evaluate deepfake detection methods, addressing gaps beyond just classification accuracy.
...
TL;DR: Salience-SGG introduces an Iterative Salience Decoder to address Scene Graph Generation's long-tailed distribution problem by focusing on spatially salient triplets, improving spatial understanding while maintaining debiasing benefits.


<details>
  <summary>Details</summary>
Motivation: Current Unbiased-SGG methods address long-tailed distribution issues in Scene Graph Generation but often sacrifice spatial understanding, leading to over-reliance on semantic priors and poor performance on spatially important relations.
...
Conclusion: Salience-SGG successfully addresses the trade-off between debiasing and spatial understanding in Scene Graph Generation, providing a balanced approach that maintains debiasing benefits while improving spatial reasoning capabilities.
...
TL;DR: ISLA is a new deep learning model for acute ischemic stroke lesion segmentation from MRI that outperforms state-of-the-art approaches through systematic optimization and domain adaptation.


<details>
  <summary>Details</summary>
Motivation: Current deep learning models for stroke lesion segmentation vary widely in architecture choices (loss functions, supervision, attention mechanisms) with unclear optimal configurations and limited public availability, making it difficult to determine the best approach for clinical applications.

Method: Developed ISLA through systematic optimization of loss function, convolutional architecture, deep supervision, and attention mechanisms. Trained on three multicenter databases (1500+ participants) and investigated unsupervised domain adaptation to improve generalization to external clinical datasets.

Result: ISLA outperformed two state-of-the-art approaches for AIS lesion segmentation on an external test set, demonstrating robust performance and better generalization.

Conclusion: ISLA provides an optimized, publicly available framework for acute ischemic stroke lesion segmentation that achieves state-of-the-art performance and addresses reproducibility concerns in the field.
...
### [160] [When KV Cache Reuse Fails in Multi-Agent Systems: Cross-Candidate Interaction is Crucial for LLM Judges](https://arxiv.org/abs/2601.08343)
*Sichu Liang,Zhenglin Wang,Jiajia Chu,Pengfei Xia,Hui Zang,Deyu Zhou*

Main category: cs.MA

TL;DR: KV cache reuse strategies that work well for execution agents can severely disrupt judge LLM behavior, causing inconsistent candidate selection despite stable accuracy metrics.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems use KV cache reuse to reduce prefill costs, but it's unclear if these efficiency gains transfer to judge-centric inference where LLMs evaluate multiple candidate responses.

Method: The authors evaluate KV cache reuse strategies across GSM8K, MMLU, and HumanEval benchmarks, measuring Judge Consistency Rate (JCR) to quantify consistency with dense prefill decisions, and analyze cross-candidate attention patterns.

Result: Reuse strategies effective for execution agents severely perturb judge behavior: while end-task accuracy appears stable, judge selection becomes highly inconsistent with dense prefill, especially weakening cross-candidate attention for later candidate blocks.

Conclusion: Judge-centric inference represents a distinct regime requiring dedicated, risk-aware system design, as KV cache reuse introduces previously overlooked failure modes that compromise decision consistency despite apparent accuracy stability.
...
### [161] [Cultural Compass: A Framework for Organizing Societal Norms to Detect Violations in Human-AI Conversations](https://arxiv.org/abs/2601.07973)
...
TL;DR: The paper introduces a taxonomy for evaluating AI models' adherence to sociocultural norms across different contexts, and shows that state-of-the-art models frequently violate norms with varying rates depending on model, context, and country.


<details>
  <summary>Details</summary>
Motivation: Generative AI models need to be useful and safe across cross-cultural contexts, requiring understanding of how they adhere to sociocultural norms. Existing work lacks nuance and coverage in evaluating norm adherence.

Method: Developed a taxonomy of norms clarifying contexts (human-human vs human-AI norms), specifications (relevant domains), and mechanisms (enforcement modes). Operationalized this taxonomy to automatically evaluate models' norm adherence in naturalistic, open-ended settings.

Result: State-of-the-art models frequently violate norms, with violation rates varying by model, interactional context, country, prompt intent, and situational framing.

Conclusion: The taxonomy and evaluation pipeline enable nuanced, context-sensitive evaluation of cultural norm adherence in realistic settings, addressing gaps in existing work.
...
### [162] [APEX-SWE](https://arxiv.org/abs/2601.08806)
*Abhi Kottamasu,Akul Datta,Aakash Barthwal,Chirag Mahapatra,Ajay Arun,Adarsh Hiremath,Brendan Foody,Bertie Vidgen*

Main category: cs.SE

TL;DR: APEX-SWE is a new benchmark for evaluating AI models on economically valuable software engineering tasks, focusing on integration and observability work rather than narrow coding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing AI evaluations for software engineering focus on narrow, well-defined tasks that don't reflect real-world economically valuable work. There's a need for benchmarks that assess whether frontier AI models can handle complex software engineering tasks that have real business impact.

Method: APEX-SWE introduces two novel task types: (1) Integration tasks requiring construction of end-to-end systems across heterogeneous cloud primitives, business applications, and infrastructure-as-code services (n=100), and (2) Observability tasks requiring debugging of production failures using telemetry signals like logs, dashboards, and unstructured context (n=100). The benchmark evaluates eight frontier models using these tasks.

Result: Gemini 3 Pro (Thinking = High) performed best with a Pass@1 score of 25%. Analysis shows strong performance is primarily driven by epistemic reasoning (ability to distinguish between assumptions and verified facts) combined with agency to resolve uncertainty before acting.

Conclusion: APEX-SWE provides a more realistic benchmark for assessing AI capabilities in economically valuable software engineering work. The benchmark and evaluation harness are open-sourced along with a development set (n=50) to advance research in this area.
...
### [163] [PosIR: Position-Aware Heterogeneous Information Retrieval Benchmark](https://arxiv.org/abs/2601.08363)
*Ziyang Zeng,Dun Zhang,Yu Yan,Xu Sun,Yudong Zhou,Yuqing Yang*

Main category: cs.IR

TL;DR: PosIR is a new benchmark for evaluating position bias in dense retrieval models, showing that current models exhibit significant sensitivity to where relevant information appears in documents.


<details>
  <summary>Details</summary>
Motivation: Existing retrieval benchmarks use position-agnostic relevance labels, which conflate the challenge of processing long contexts with bias against specific evidence locations. There's a lack of rigorous evaluation of how sensitive retrieval models are to the position of relevant information.

Method: Created PosIR benchmark with 310 datasets across 10 languages and 31 domains using a rigorous pipeline that ties relevance to precise reference spans, strictly disentangling document length from information position. Evaluated 10 state-of-the-art embedding models with gradient-based saliency analysis.

Result: 1) Performance on PosIR in long-context settings correlates poorly with MMTEB benchmark, exposing limitations of current short-text benchmarks. 2) Position bias is pervasive and intensifies with document length - most models show primacy bias while some show unexpected recency bias. 3) Gradient-based analysis reveals distinct internal attention mechanisms driving these positional preferences.

Conclusion: PosIR serves as a valuable diagnostic framework to foster development of position-robust retrieval systems by systematically evaluating position bias that current benchmarks overlook.
...
### [164] [Sliced-Wasserstein Distribution Alignment Loss Improves the Ultra-Low-Bit Quantization of Large Language Models](https://arxiv.org/abs/2601.07878)
*Deyu Cao,Yixin Yin,Samin Aref*

Main category: cs.LG

TL;DR: A sliced Wasserstein loss function for distribution-aware calibration improves ultra-low-bit post-training quantization by aligning output distributions of full-precision and quantized models, recovering significant accuracy lost in frontier quantization methods.


<details>
  <summary>Details</summary>
Motivation: Large language models have steep economic and environmental costs due to resource inefficiency during deployment. Model quantization improves efficiency but compression below 4-bits often distorts activation distributions and degrades performance.

Method: Introduces a sliced Wasserstein loss function for distribution-aware calibration in ultra-low-bit post-training quantization. The loss aligns output distributions of full-precision and quantized models under random linear projections, complementing standard mean-squared error loss without adding computational overhead during inference.
...
Conclusion: Distributional alignment provides a simple yet effective performance boost that can push the limits of frontier quantization methods. The method is available on GitHub to facilitate future progress in ultra-low-bit quantization.
...
### [165] [KVzap: Fast, Adaptive, and Faithful KV Cache Pruning](https://arxiv.org/abs/2601.07891)
...
<details>
  <summary>Details</summary>
Motivation: Growing context lengths in transformer-based language models have made the key-value (KV) cache a critical inference bottleneck, but existing KV cache pruning methods haven't been widely adopted due to speed-accuracy trade-offs.
...
Conclusion: KVzap provides an effective solution to the KV cache bottleneck problem with minimal accuracy impact, making it suitable for practical deployment in inference engines.
...
### [166] [Towards Specialized Generalists: A Multi-Task MoE-LoRA Framework for Domain-Specific LLM Adaptation](https://arxiv.org/abs/2601.07935)
*Yuxin Yang,Aoxiong Zeng,Xiangquan Yang*

Main category: cs.LG

TL;DR: Med-MoE-LoRA: A novel framework combining Mixture-of-Experts with Low-Rank Adaptation for efficient multi-task medical domain adaptation, addressing stability-plasticity dilemma and task interference.


<details>
  <summary>Details</summary>
Motivation: Address two key challenges in adapting LLMs to medical domains: (1) Stability-Plasticity Dilemma - acquiring clinical knowledge without catastrophic forgetting of general knowledge, and (2) Task Interference - multiple medical sub-tasks competing for limited parameter space.

Method: Integrates Mixture-of-Experts (MoE) with Low-Rank Adaptation (LoRA) using asymmetric expert distribution (more experts in deeper layers for complex semantics), Knowledge-Preservation Plugin to protect general reasoning, and soft merging with adaptive routing and rank-wise decoupling.
...
Conclusion: Med-MoE-LoRA provides an effective solution for domain-specific LLM adaptation in medicine, balancing specialized expertise with preserved general knowledge through innovative MoE-LoRA integration.
...
### [167] [Demystifying the Slash Pattern in Attention: The Role of RoPE](https://arxiv.org/abs/2601.08297)
*Yuan Cheng,Fengzhuo Zhang,Yunlong Hou,Cunxiao Du,Chao Du,Tianyu Pang,Aixin Sun,Zhuoran Yang*

Main category: cs.LG

TL;DR: The paper explains why slash attention patterns emerge in LLMs, showing they are intrinsic to models and generalize to out-of-distribution prompts through empirical analysis and theoretical proof.


<details>
  <summary>Details</summary>
Motivation: Large Language Models often exhibit slash attention patterns where attention scores concentrate along specific sub-diagonals, playing a key role in information flow across tokens. The paper aims to demystify why these Slash-Dominant Heads emerge in LLMs.

Method: Combined empirical analysis of open-source LLMs and theoretical modeling. Empirically analyzed queries, keys, and Rotary Position Embedding (RoPE) to identify characteristic conditions. Theoretically formalized these conditions as modeling assumptions and analyzed training dynamics of shallow Transformers with RoPE under gradient descent.

Result: Found two characteristic conditions for SDHs: (1) queries and keys are almost rank-one, (2) RoPE is dominated by medium- and high-frequency components. These conditions cause queries and keys to be nearly identical across tokens, and RoPE interactions produce slash patterns. Theoretically proved these conditions are sufficient for SDH emergence, and SDHs generalize to out-of-distribution prompts.

Conclusion: Slash-Dominant Heads emerge intrinsically in LLMs due to specific structural properties of queries, keys, and RoPE. The paper provides both empirical evidence and theoretical proof explaining this phenomenon, showing it generalizes beyond training data.
...
### [168] [Rewarding the Rare: Uniqueness-Aware RL for Creative Problem Solving in LLMs](https://arxiv.org/abs/2601.08763)
...
TL;DR: Proposes Uniqueness-Aware Reinforcement Learning to prevent exploration collapse in RL for LLMs by rewarding rare high-level solution strategies rather than just correct answers.


<details>
  <summary>Details</summary>
Motivation: RL for LLMs suffers from exploration collapse where policies prematurely focus on dominant reasoning patterns, improving pass@1 but limiting diversity and gains in pass@k.

Method: Uses LLM-based judge to cluster rollouts by high-level solution strategies, ignoring superficial variations, and reweights policy advantages inversely with cluster size to reward novel strategies.

Result: Improves pass@k across mathematics, physics, and medical reasoning benchmarks, increases AUC@K without sacrificing pass@1, and sustains exploration of diverse solution strategies.

Conclusion: Explicitly rewarding solution uniqueness at rollout level addresses RL exploration collapse in LLMs, enabling better diversity and performance across sampling budgets.
...
### [169] [Asymptotic Universal Alignment: A New Alignment Framework via Test-Time Scaling](https://arxiv.org/abs/2601.08777)
*Yang Cai,Weiqiang Zheng*

Main category: cs.LG

TL;DR: The paper formalizes universal alignment for LLMs through test-time scaling with multiple candidate responses, characterizes optimal convergence rates, shows existing methods underutilize scaling, and proposes symmetric multi-player games achieving optimal alignment.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs to serve users with heterogeneous and conflicting preferences is challenging. Current methods don't effectively leverage test-time scaling where models produce multiple candidate responses for user selection.

Method: Introduces (k,f(k))-robust alignment framework and asymptotic universal alignment. Proposes symmetric multi-player alignment games where any symmetric Nash equilibrium of the (k+1)-player game achieves optimal (k,k/(k+1))-robust alignment.

Result: Characterizes optimal convergence rate: k/(k+1) is achievable and no method can achieve faster rate. Shows NLHF and other post-training methods underutilize scaling, often collapsing to single responses. Provides theoretical convergence guarantees for self-play learning.

Conclusion: Test-time scaling with multiple responses enables universal alignment, but existing methods fail to leverage this due to lack of diversity. Symmetric multi-player games achieve optimal scaling rates and preserve output diversity for better personalization.
...
### [170] [Semantic Gravity Wells: Why Negative Constraints Backfire](https://arxiv.org/abs/2601.08070)
*Shailesh Rana*

Main category: cs.AI

TL;DR: Negative constraints (e.g., "do not use word X") fail in LLMs due to semantic pressure and two failure modes: priming (87.5%) where mentioning the forbidden word activates it, and override (12.5%) where late-layer networks overwhelm suppression signals.
...
Result: Violation probability follows tight logistic relationship with semantic pressure. In failures, suppression signal is 4.4× weaker than in successes. Two failure modes identified: priming failure (87.5%) where instruction activates target representation, and override failure (12.5%) where late-layer feed-forward networks generate +0.39 contributions toward target probability.

Conclusion: Reveals fundamental tension in negative constraint design: naming a forbidden word paradoxically primes the model to produce it. The suppression signal induced by negative instructions is systematically weaker in failures, traced to two mechanistically distinct failure modes.
...
### [171] [MemoBrain: Executive Memory as an Agentic Brain for Reasoning](https://arxiv.org/abs/2601.08079)
...
<details>
  <summary>Details</summary>
Motivation: Complex reasoning in tool-augmented agents generates long reasoning traces and tool artifacts that accumulate and strain LLM context limits, disrupting logical continuity and task alignment. Memory is essential, not optional, for sustaining coherent reasoning over long horizons.
...
Conclusion: Explicit memory mechanisms like MemoBrain are crucial for tool-augmented agents to maintain cognitive control over reasoning trajectories rather than suffering from passive context accumulation, enabling sustained coherent reasoning over long horizons.
...
### [172] [Deconstructing Pre-training: Knowledge Attribution Analysis in MoE and Dense Models](https://arxiv.org/abs/2601.08383)
*Bo Wang,Junzhuo Li,Hong Chen,Yuanlin Chu,Yuxuan Fan,Xuming Hu*

Main category: cs.AI

TL;DR: MoE architectures develop a stable, distributed knowledge backbone early in training, with a small core of high-utility neurons capturing most positive updates, unlike dense models which remain volatile throughout training.


<details>
  <summary>Details</summary>
Motivation: To understand how Mixture-of-Experts architectures shape knowledge acquisition during pre-training and how this differs from dense architectures, since MoE decouples model capacity from per-token computation but the underlying knowledge acquisition dynamics remain unknown.

Method: Introduced Gated-LPI (Log-Probability Increase), a neuron-level attribution metric that decomposes log-probability increase across neurons. Conducted time-resolved comparison tracking checkpoints over 1.2M training steps (~5.0T tokens) for MoE and 600K steps (~2.5T tokens) for dense architectures.

Result: Three key patterns: (1) Low-entropy backbone - top ~1% of MoE neurons capture >45% of positive updates, forming a high-utility core absent in dense models; (2) Early consolidation - MoE stabilizes importance profile within <100K steps while dense remains volatile; (3) Functional robustness - masking top MoE attention heads reduces relational performance by <10% vs >50% for dense, showing distributed knowledge storage.
...
<details>
  <summary>Details</summary>
Motivation: Digital platforms face hate speech/misogyny spread; AI solutions lack interpretability and are under-explored for low-resource code-mixed languages like Hindi-English.

Method: Text-based: XLM-R and mBERT on 4,193 comments. Multimodal memes: mBERT+EfficientNet and mBERT+ResNet on 4,218 memes. SHAP/LIME for feature importance. Web app with usability evaluation via CUQ/UEQ questionnaires.

Result: Developed functional web application for misogyny detection in code-mixed Hindi-English content. System evaluated by human users using Chatbot Usability Questionnaire and User Experience Questionnaire.

Conclusion: Explainable multi-modal system serves as tool for researchers/content moderators to combat gender-based digital violence and ensure safe digital spaces, promoting further research in the field.
...
### [174] [What If TSF: A Benchmark for Reframing Forecasting as Scenario-Guided Multimodal Forecasting](https://arxiv.org/abs/2601.08509)
*Jinkwan Jang,Hyunbin Jin,Hyungjin Park,Kyubyung Chae,Taesup Kim*

Main category: cs.AI

TL;DR: WIT is a multimodal time series forecasting benchmark that tests if models can condition forecasts on textual scenarios, unlike traditional unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: Current time series forecasting methods are mostly unimodal and rely on historical patterns, while real-world human experts incorporate what-if scenarios with historical evidence. Existing benchmarks provide retrospective or misaligned context, making it unclear if models truly leverage textual inputs.

Method: Introduces What If TSF (WIT), a multimodal forecasting benchmark with expert-crafted plausible or counterfactual scenarios to evaluate if models can condition forecasts on contextual text, especially future scenarios.

Result: WIT provides a rigorous testbed for scenario-guided multimodal forecasting, available as an open-source benchmark at https://github.com/jinkwan1115/WhatIfTSF.

Conclusion: WIT addresses the gap in evaluating multimodal forecasting by testing whether models can meaningfully incorporate textual scenarios like human experts do, moving beyond simple historical pattern extrapolation.
...
### [175] [Learner-Tailored Program Repair: A Solution Generator with Iterative Edit-Driven Retrieval Enhancement](https://arxiv.org/abs/2601.08545)
*Zhenlong Dai,Zhuoluo Zhao,Hengning Wang,Xiu Tang,Sai Wu,Chang Yao,Zhipeng Gao,Jingyuan Chen*

Main category: cs.AI

TL;DR: The paper introduces LPR (Learner-Tailored Program Repair), a new task that goes beyond just fixing bugs to also provide explanations of underlying causes, and proposes LSG, a two-stage framework with iterative retrieval enhancement for intelligent programming coaching.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based programming coaching systems focus only on repairing buggy code without explaining the underlying causes of bugs, creating a gap in effective programming education.

Method: LSG framework with two stages: 1) Repair solution retrieval using edit-driven code retrieval to guide LLMs, 2) Solution-guided program repair that fixes code and provides explanations. Includes Iterative Retrieval Enhancement that uses evaluation results to optimize retrieval direction.

Result: The approach outperforms baseline methods by a large margin, validating the effectiveness of the LSG framework for the new LPR task.

Conclusion: The proposed LSG framework successfully addresses the LPR task by not only repairing buggy code but also providing explanatory bug descriptions, advancing intelligent programming coaching systems.
...
TL;DR: Pced enables cross-document reasoning in RAG without shared attention, using parallel expert predictions and contrastive decoding.


<details>
  <summary>Details</summary>
Motivation: RAG faces trade-off: concatenating documents enables reasoning but causes prefill bottlenecks; separate KV caches are fast but break cross-document interaction.

Method: Parallel Context-of-Experts Decoding (Pced) treats retrieved documents as isolated experts, synchronizes predictions via retrieval-aware contrastive decoding that weighs expert logits against model prior.

Result: Recovers cross-document reasoning capabilities without constructing shared attention across documents.

Conclusion: Pced offers training-free framework that shifts evidence aggregation from attention mechanism to decoding, enabling efficient multi-document reasoning.
