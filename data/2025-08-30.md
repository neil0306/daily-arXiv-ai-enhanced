...
Motivation: To analyze emerging research on bias evaluation and mitigation in multilingual and non-English contexts, examining linguistic diversity, cultural awareness, and methodological choices in the field.

Method: Systematic review of studies extending bias evaluation and mitigation approaches to multilingual settings, analyzing them with respect to linguistic diversity, cultural appropriateness, evaluation metrics, and mitigation techniques.
...
Conclusion: The review charts future research directions to enhance multilingual bias literature's inclusivity, cross-cultural appropriateness, and alignment with state-of-the-art NLP advancements.
...
<details>
  <summary>Details</summary>
Motivation: To reduce the cost and inconsistency of manual test development for morphological assessment by leveraging automatic item generation using language models.

Method: Two-fold approach: 1) compared fine-tuned Gemma (2B) with untuned GPT-3.5 (175B), 2) evaluated seven structured prompting strategies including zero-shot, few-shot, chain-of-thought, role-based, sequential, and combinations. Used automated metrics and expert scoring across five dimensions, plus GPT-4.1 simulation of human scoring.

Result: Structured prompting, especially chain-of-thought and sequential design combinations, significantly improved Gemma's outputs. Gemma produced more construct-aligned and instructionally appropriate items than GPT-3.5's zero-shot responses.

Conclusion: Structured prompting and efficient fine-tuning can enhance midsized models for automatic item generation under limited data conditions, offering a practical and scalable way to develop language assessment items for K-12 education.
...
<details>
  <summary>Details</summary>
Motivation: Growing complexity of cyber-physical systems and automotive applications requires efficient cross-domain co-simulation, but SystemC TLM has limited interoperability with other engineering domains.

Method: Encapsulate SystemC TLM components as FMI 3.0 Co-Simulation Functional Mock-up Units (FMUs) with a lightweight open-source toolchain, addressing time synchronization and data exchange challenges.
...
<details>
  <summary>Details</summary>
Motivation: Compact language models (e.g., 0.5B parameters) struggle with agentic RAG behaviors due to poor reasoning ability, sparse rewards, and unstable training in reinforcement learning settings.
...
Conclusion: DGPO makes agentic RAG feasible in computing resource-constrained environments by enabling compact models to perform complex search and planning tasks effectively.
...
<details>
  <summary>Details</summary>
Motivation: Address the gap between high-level government AI ethics guidelines and practical testing methods to ensure LLMs don't generate harmful responses, as current guidelines lack actionable testing procedures.

Method: Automated generation of guideline-violating questions from government guidelines, testing LLM responses for compliance, and integrating jailbreak diagnostics (GUARD-JD) to provoke unethical responses and identify safety mechanism bypasses.

Result: Empirically validated on 7 LLMs (Vicuna-13B, LongChat-7B, Llama2-7B, Llama-3-8B, GPT-3.5, GPT-4, GPT-4o, Claude-3.7) under three government guidelines, with jailbreak diagnostics transferable to vision-language models.

Conclusion: GUARD provides an effective framework for operationalizing AI ethics guidelines into concrete testing procedures, producing compliance reports that highlight violations and promote reliable LLM applications.
...
Method: Three-step framework: 1) Synopsis extraction through strategic text chunking, 2) Directed acyclic graph construction to resolve redundancy and ensure logical consistency, 3) Monte Carlo Tree Search for navigating complex reasoning paths.
...
<details>
  <summary>Details</summary>
Motivation: Current Long CoT reasoning development relies on costly human-curated datasets, leaving scalable alternatives unexplored. NP-hard graph problems inherently require deep reasoning and reflective strategies that align with Long CoT characteristics.

Method: Two-stage post-training framework: 1) Long CoT Supervised Fine-Tuning on rejection-sampled NPH graph instances to enhance reasoning depth, 2) Reinforcement Learning with fine-grained reward design to sharpen reasoning efficiency.

Result: Graph-R1-7B model demonstrates strong generalization across mathematics, coding, STEM, and logic, surpassing QwQ-32B on NPH graph problems in both accuracy and reasoning efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing psychometric tests for LLMs use context-free approaches that ignore real-world conversational history, creating an artificial evaluation setting that doesn't reflect actual application scenarios.

Method: Proposed Context-Aware Personality Evaluation (CAPE) framework incorporating prior conversational interactions, with novel metrics to quantify response consistency across 7 different LLMs including GPT models, Gemini, and Llama.

Result: Conversational history enhances response consistency through in-context learning but induces personality shifts. GPT-3.5-Turbo and GPT-4-Turbo show extreme deviations. GPT models are robust to question ordering while Gemini-1.5-Flash and Llama-8B are sensitive. GPT responses come from intrinsic traits and prior interactions, while Gemini and Llama heavily depend on prior interactions.

Conclusion: Context-dependent personality shifts improve response consistency and better align with human judgments, especially for Role Playing Agents. The framework provides more realistic evaluation of LLM behavioral traits in conversational settings.
...
<details>
  <summary>Details</summary>
Motivation: To understand how intermediate reasoning steps contribute to final answer correctness and develop methods to detect unproductive reasoning early to improve efficiency.

Method: Oracle study on MATH dataset using Qwen2.5-32B and GPT-4o to generate reasoning chains, then using Qwen3-8B to measure conditional entropy (uncertainty) at each reasoning step to quantify utility.

Result: Clear pattern: decreasing conditional entropy over steps strongly associated with correct answers; flat/increasing entropy leads to wrong answers. Incorrect reasoning paths tend to be longer than correct ones.

Conclusion: Longer reasoning doesn't guarantee better outcomes. Findings provide foundation for designing efficient reasoning pipelines that detect and avoid unproductive reasoning early.
...
<details>
  <summary>Details</summary>
Motivation: There are no public benchmarks that rigorously verify claims about AI text-to-app tools' ability to produce high-quality applications and websites quickly.

Method: Uses expert pairwise comparison across multiple tools and prompts, with a TrueSkill-derived model for ranking systems and calibrated confidence intervals.

Result: Created a comprehensive benchmark with 300 generated sites from 30 prompts across 10 tools, supported by over 4000 expert judgments and a ranking system.

Conclusion: UI-Bench establishes a reproducible standard for advancing AI-driven web design and provides open resources including prompt sets, evaluation framework, and public leaderboard.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLMs show strong general medical performance but lack evaluation in specialized fields like dentistry, which requires deep domain-specific knowledge and has no targeted benchmarks.

Method: Created DentalBench with two components: DentalQA (bilingual question-answering benchmark with 36,597 questions across 4 tasks and 16 subfields) and DentalCorpus (337.35M token corpus for domain adaptation via SFT and RAG). Evaluated 14 LLMs including proprietary, open-source, and medical-specific models.

Result: Significant performance gaps identified across task types and languages. Domain adaptation with Qwen-2.5-3B substantially improved performance, especially on knowledge-intensive and terminology-focused tasks.

Conclusion: Domain-specific benchmarks are crucial for developing trustworthy and effective healthcare LLMs, as specialized medical fields require targeted evaluation and adaptation beyond general medical capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing methods that primarily handle corpus-level context loss, and instead focus on improving query enrichment through structured knowledge graph representations for better retrieval in RAG systems.

Method: Proposes KG-CQR framework with three modules: subgraph extraction, completion, and contextual generation. It extracts and completes relevant KG subgraphs to create semantically rich query contexts, operating as a model-agnostic pipeline that works with various LLMs without additional training.

Result: Achieves 4-6% improvement in mAP and 2-3% improvement in Recall@25 over strong baselines on RAGBench and MultiHop-RAG datasets. Shows consistent performance gains on challenging tasks like multi-hop question answering.

Conclusion: KG-CQR effectively enhances retrieval phase performance in RAG systems through knowledge graph-based query enrichment, demonstrating scalability across different LLM sizes and superior results compared to existing approaches.
...
Method: Developed an industrial-grade benchmark specifically designed for civil aviation maintenance procedures and troubleshooting, and used it to evaluate existing vector embedding models and LLMs in practical RAG systems.

Result: The benchmark effectively assesses model performance in civil aviation maintenance scenarios, identifying specific gaps in domain knowledge and complex reasoning capabilities.

Conclusion: This work provides a standardized evaluation tool for the aviation maintenance domain, establishes a foundation for targeted improvements, and opensources the benchmark to foster further research and development in this specialized field.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient system for searching practical work titles using experience-based case reasoning and similarity matching.

Method: Case Base Reasoning with TF-IDF for text vectorization and Cosine Similarity for calculating similarity values between titles/keywords.

Result: System successfully found same number of titles with highest average match score in randomized testing with 705 practical work titles.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to adequately evaluate LLMs' capabilities in realistic multi-step tool usage, cross-tool coordination, parameter control, and planning/reasoning for complex tasks.

Method: Built on Model Context Protocol (MCP), connecting LLMs to 28 live servers with 250 complementary tools across finance, travel, scientific computing, and academic search domains. Uses multi-faceted evaluation covering schema understanding, trajectory planning, and task completion.

Result: Experiments on 20 advanced LLMs reveal persistent challenges in handling the benchmark's complex multi-step tasks and cross-tool coordination requirements.

Conclusion: MCP-Bench provides a more realistic evaluation framework that exposes significant gaps in current LLMs' abilities to handle authentic multi-step tool usage and cross-domain workflows compared to existing benchmarks.
...
Method: Developed a deep learning framework using natural language processing techniques that integrates multimodal EHRs. Used two real-world EHR datasets, performed ablation studies on medical prompts, free-texts, and pre-trained sentence encoder, and assessed robustness against structured data corruption.
...
Conclusion: The framework is an effective and accurate deep learning approach for predicting mortality and resource utilization in critical care. Successfully uses prompt learning with transformer encoder for multimodal EHR analysis and demonstrates strong resilience to data corruption, especially at high corruption levels.
...
Method: Introduce ConspirED dataset with multi-sentence excerpts from online conspiracy articles annotated using CONSPIR cognitive framework, then develop computational models and evaluate LLM robustness.

Result: Both computational models and LLMs are misaligned by conspiratorial content, producing output that mirrors input reasoning patterns even when deflecting fact-checked misinformation.

Conclusion: The study demonstrates vulnerabilities in AI systems to conspiratorial reasoning and provides a dataset for developing targeted interventions against conspiracy content.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate the suitability of widely-used FLORES+ multilingual MT benchmark for truly multilingual evaluation, uncovering critical shortcomings in its data quality and cultural neutrality.

Method: Human assessments of translations in four languages (Asante Twi, Japanese, Jinghpaw, South Azerbaijani), analysis of source text domain specificity and cultural bias, testing with simple heuristics like named entity copying, and comparing MT model performance on FLORES+ vs domain-relevant evaluation sets.

Result: Many FLORES+ translations fall below 90% quality standard, source sentences are domain-specific and culturally biased toward English-speaking world, simple heuristics yield non-trivial BLEU scores, and MT models trained on naturalistic data perform poorly on FLORES+ but well on domain-relevant sets.
...
<details>
  <summary>Details</summary>
Motivation: Existing topic discovery methods rely on word embeddings and struggle with complex text relationships, lacking comprehensive understanding of scientific publications despite the potential of LLMs for better textual comprehension.

Method: Build a textual encoder to capture publication content (metadata, title, abstract), construct space optimization with entropy-based sampling and LLM-guided triplet tasks, fine-tune encoder using contrastive loss on triplets to better discriminate different topics.

Result: Extensive experiments on three real-world scientific publication datasets demonstrate that SciTopic outperforms state-of-the-art scientific topic discovery methods.

Conclusion: SciTopic enables researchers to gain deeper and faster insights into scientific literature by leveraging LLMs for enhanced topic discovery, addressing limitations of traditional word embedding approaches.
...
Method: Organized four shared tasks: two established tasks (b and Synergy) and two new tasks focused on clinical entity detection in cardiology (MultiCardioNER) and nested NER in Russian/English (BIONNE).

Result: 37 competing teams participated with over 700 distinct submissions. Most systems achieved competitive performance, indicating continuous advancement in the field.

Conclusion: BioASQ 2024 successfully continued its mission of advancing biomedical semantic technologies, with new tasks expanding the scope to multilingual clinical entity detection and nested NER, demonstrating ongoing progress in the state-of-the-art.
...
Conclusion: BioASQ 2025 successfully continued its mission of advancing biomedical NLP through diverse shared tasks, with strong participation and performance demonstrating ongoing progress in the state-of-the-art.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated learning approaches primarily focus on label diversity but ignore language domain diversity, which is crucial for NLP applications. Real-world local data varies significantly across domains and cannot capture global data distribution.

Method: Introduces a comprehensive set of multi-domain non-IID scenarios and proposes AdaFD framework that adapts to both homogeneous and heterogeneous settings to address input domain diversity challenges.
...
Conclusion: The proposed framework and benchmarking approach provide a more realistic evaluation environment for federated learning in NLP, addressing the critical gap of language domain diversity in non-IID scenarios.
...
Result: Outperforms production baseline and achieves new state-of-the-art on multiple industry-relevant metrics. Demonstrates excellent deployment efficiency with only 334 NVIDIA L20 GPUs handling ~50,000 queries per second under 55ms average latency.
...
<details>
  <summary>Details</summary>
Motivation: Address data sparsity in multi-hop QA that leads to spurious patterns, overcoming limitations of prior methods that neglect essential knowledge integration and focus only on simple question generation.

Method: Knowledge Composition Sampling (KCS) models knowledge composition selection as sentence-level conditional prediction with probabilistic contrastive loss, using stochastic decoding during inference to balance accuracy and diversity.

Result: 3.9% improvement in knowledge composition selection accuracy over baselines, with data augmentation applications showing improvements on HotpotQA and 2WikiMultihopQA datasets.

Conclusion: KCS effectively expands multi-hop question diversity by better integrating relevant knowledge compositions, demonstrating superior performance in knowledge selection and downstream QA tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for Graph-Language Models are insufficient because they don't require true multimodal reasoning - strong performance can be achieved using either graph structure or language information alone, failing to assess genuine integration capabilities.

Method: The authors introduce CLEGR benchmark with synthetic graph generation pipeline and questions requiring joint reasoning over structure and textual semantics. They evaluate representative GLM architectures and compare against soft-prompted LLM baselines.

Result: Soft-prompted LLM baselines perform on par with full GNN-backbone GLMs, questioning the architectural necessity of graph integration. GLMs show significant performance degradation in structural reasoning tasks.

Conclusion: Current GLMs have limited graph reasoning capabilities. The CLEGR benchmark provides a foundation for advancing multimodal reasoning that truly integrates graph structure and language.
...
<details>
  <summary>Details</summary>
Motivation: End-to-end ASR systems often fail to transcribe domain-specific named entities, causing downstream task failures. Existing phonetic-level edit distance methods struggle when wrongly-transcribed words and ground-truth entities have significantly different forms.
...
Result: The method shows significant improvement in entity accuracy on both open-source and self-constructed test sets, particularly effective in scenarios with word form differences.

Conclusion: The proposed NEC method successfully addresses limitations of existing approaches by leveraging speech sound features and generative annotation, providing substantial improvements in named entity recognition accuracy for ASR systems.
...
Method: Developed HArch model that uses hierarchical dependencies to predict probability distributions across three sense levels. Compared pre-trained encoder backbones (RoBERTa, XLM-RoBERTa) and evaluated against LLMs (GPT-4o, Llama-4-Maverick) using few-shot prompting on DiscoGeM 2.0 corpus.
...
Conclusion: Task-specific fine-tuning with hierarchical approach (HArch) significantly outperforms prompting large language models for implicit discourse relation recognition, demonstrating the effectiveness of specialized architectures over general LLMs for this NLP task.
...
<details>
  <summary>Details</summary>
Motivation: Large language models have improved text generation but created tokenization inconsistency problems between sender and receiver in steganography/watermarking, undermining robustness. Problematic tokens show infrequency and temporariness characteristics.

Method: Proposed two tailored solutions: stepwise verification method for steganography and post-hoc rollback method for watermarking to eliminate tokenization inconsistency.

Result: Experiments show: (1) steganography methods with TI elimination improve fluency, imperceptibility, and anti-steganalysis capacity compared to traditional disambiguation; (2) watermarking with TI elimination enhances detectability and robustness against attacks.

Conclusion: Addressing tokenization inconsistency directly through specialized methods significantly improves the performance and robustness of both steganography and watermarking systems in LLM-based text generation.
...
<details>
  <summary>Details</summary>
Motivation: To develop a more efficient math reasoning model that can outperform much larger models through agentic reinforcement learning, addressing challenges like high rollout costs and environment noise in coding tools.

Method: Uses three key innovations: efficient RL infrastructure with reliable Python environment, GRPO-RoC algorithm with Resample-on-Correct strategy to handle coding tool noise, and a multi-stage training recipe starting with SFT and progressing through RL stages.

Result: Achieves 80.6% on AIME24 and 69.8% on AIME25, surpassing DeepSeek-R1 (671B) with shorter responses, trained in only 510 RL steps within one week on limited GPU resources.

Conclusion: rStar2-Agent demonstrates that agentic RL can effectively scale to create highly capable math reasoning models with advanced cognitive abilities, while also showing strong generalization to other domains like scientific reasoning and tool-use tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing local DP text privatization methods require very high ε values for reasonable utility, making them impractical for real-world applications that need stronger privacy guarantees.

Method: Introduces DP-ST which leverages semantic triples for neighborhood-aware private document generation under local DP, using a divide-and-conquer approach with LLM post-processing to maintain text coherence.

Result: The method demonstrates effectiveness in achieving balanced privacy-utility outputs, allowing coherent text generation even at lower ε values while maintaining privacy guarantees.

Conclusion: Semantic triple-based approaches combined with LLM processing enable practical local DP text privatization with better privacy-utility tradeoffs, highlighting the importance of coherence in achieving reasonable privatization outputs.
...
<details>
  <summary>Details</summary>
Motivation: Implicit hate speech is challenging to detect due to its subtle and indirect nature, requiring methods that go beyond traditional explicit hate speech detection approaches.

Method: Fine-tuned recent general-purpose embedding models (Stella, Jasper, NV-Embed, E5) based on large language models for implicit hate speech detection without external knowledge or additional context.

Result: Achieved up to 1.10 percentage points improvement for in-dataset evaluation and up to 20.35 percentage points improvement in cross-dataset evaluation (F1-macro score) across multiple IHS datasets.

Conclusion: General-purpose LLM-based embedding models, when fine-tuned, provide an effective and simplified approach for implicit hate speech detection, outperforming previous methods that relied on external knowledge and complex pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Address the trade-off between coherence and diversity in LLM text generation, overcoming limitations of contrastive search methods that suffer from hyperparameter dependence and high computational costs.

Method: GUARD uses a 'Glocal' uncertainty-driven framework combining global entropy estimates with local entropy deviations to integrate long-term and short-term uncertainty signals, plus a token-count-based penalty to reduce computational overhead.

Result: GUARD achieves good balance between text diversity and coherence with substantial improvements in generation speed. Both human and LLM evaluators validated its remarkable performance across different text quality dimensions.

Conclusion: GUARD provides an effective self-adaptive decoding approach that successfully addresses the coherence-diversity trade-off in open-ended text generation with computational efficiency and theoretical soundness.
...
<details>
  <summary>Details</summary>
Motivation: To determine whether LLM-generated therapy dialogues accurately capture the emotional dynamics of real therapy sessions, which is crucial for mental health applications.

Method: Comparative analysis using Utterance Emotion Dynamics framework to analyze emotional arcs across valence, arousal, and dominance dimensions in both real and synthetic CBT dialogues from CACTUS dataset.

Result: Synthetic dialogues diverge from real conversations - real sessions show greater emotional variability, more emotion-laden language, and authentic patterns of reactivity and regulation. Emotional arc similarity is low, especially for clients.

Conclusion: Current LLM-generated therapy data has limitations in emotional fidelity, highlighting the need for better emotional representation in mental health applications. The RealCBT dataset is introduced to support future research.
...
Method: Rank-One Safety Injection (ROSI) - a fine-tuning-free rank-one weight modification applied to all residual stream write matrices, computed from harmful/harmless instruction pairs to steer activations toward refusal subspace.

Result: ROSI consistently increases safety refusal rates (evaluated by Llama Guard 3) while preserving utility on standard benchmarks (MMLU, HellaSwag, Arc). It can also re-align 'uncensored' models by amplifying latent safety directions.

Conclusion: Targeted, interpretable weight steering is a cheap and effective mechanism to improve LLM safety, complementing more resource-intensive fine-tuning approaches as an effective last-mile safety procedure.
...
Method: Cross-lingual and cross-register generalization analysis using Dutch adolescent forum posts, comparing domain adaptation methods.

Result: Language and writing style changes significantly affect model performance, but domain adaptation methods show the most promise for generalization.
...
<details>
  <summary>Details</summary>
Motivation: To develop effective multimodal approaches for depression detection by leveraging different machine learning and deep learning models to capture depression-related signals across various modalities.

Method: Used XGBoost, transformer-based architectures, and large language models (LLMs) to analyze and compare performance on audio, video, and text features for depression detection.

Result: Identified strengths and limitations of each model type in capturing depression signals across different modalities, providing insights into effective multimodal representation strategies.

Conclusion: The study offers valuable comparative analysis of different ML/DL approaches for multimodal depression detection, contributing to better mental health prediction through optimized multimodal representation strategies.
...
Method: Proposes GDLLM with distance-aware graph structure using Graph Attention Network to help LLMs capture long-distance dependencies, and a temporal feature learning paradigm based on soft inference to enhance short-distance proximity identification.

Result: Achieves state-of-the-art performance on TB-Dense and MATRES datasets, substantially improving minority relation class performance and overall learning ability.

Conclusion: The global distance-aware modeling approach effectively captures global features, enhancing LLM performance for temporal relation extraction by addressing both long-distance dependencies and minority class challenges.
...
<details>
  <summary>Details</summary>
Motivation: Real-world applications require integrating information from multiple sources where no single source is sufficient, but current RAG evaluations focus on single-source or short-form factoid settings.

Method: Developed a scalable framework to build evaluation benchmarks (MSRS-Story and MSRS-Meet) for multi-source retrieval and synthesis, tested various RAG pipelines with different retrievers and LLMs.

Result: Generation quality is highly dependent on retrieval effectiveness, which varies by task. Multi-source synthesis is challenging even with oracle retrieval, but reasoning models significantly outperform standard LLMs.

Conclusion: The framework enables better evaluation of RAG systems for complex multi-source tasks, revealing critical dependencies between retrieval and generation components, with reasoning models showing superior synthesis capabilities.
...
Method: Conducted evaluation of post-training quantization (PTQ) using five LLMs (1.7B to 70B parameters) across 55 languages, comparing four quantization techniques (AWQ, BitsAndBytes, GGUF, and AutoRound) and analyzing interactions with decoding hyperparameters and calibration languages.
...
Conclusion: Quantization algorithm choice and model size jointly determine robustness for multilingual translation. Findings provide actionable insights for deploying multilingual LLMs under quantization constraints, especially in low-resource settings.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for speech-to-speech LLMs are inadequate as they either ignore acoustic features (cascaded approaches) or lack explainability, making comprehensive evaluation challenging.

Method: Proposes SageLM with three key innovations: 1) joint assessment of semantic and acoustic dimensions, 2) rationale-based supervision for explainability, 3) two-stage training using synthetic SpeechFeedback dataset to address data scarcity.
...
Conclusion: SageLM provides a comprehensive, explainable framework for evaluating speech-to-speech LLMs, effectively addressing both semantic and acoustic dimensions while overcoming data scarcity issues.
...
Method: Two-stage approach: 1) Fine-tune BERT-based retriever with structure-aware supervision, 2) Enhance with plug-in module that amplifies syntactically meaningful information in hidden representations (model-agnostic, minimal overhead).
...
<details>
  <summary>Details</summary>
Motivation: Existing proactive dialogue research focuses on domain-specific scenarios, leading to fragmented evaluations that limit comprehensive assessment of LLMs' proactive conversation abilities.

Method: Proposed ProactiveEval framework that decomposes proactive dialogue into target planning and dialogue guidance, establishes evaluation metrics across domains, and enables automatic generation of diverse evaluation data with 328 environments spanning 6 domains.

Result: Testing 22 different LLMs revealed that DeepSeek-R1 excels at target planning tasks while Claude-3.7-Sonnet performs best on dialogue guidance tasks.

Conclusion: The study investigates how reasoning capabilities influence proactive behaviors and provides implications for future model development, demonstrating the framework's effectiveness in comprehensive proactive dialogue evaluation.
...
Method: LETHE uses internal knowledge dilution by training a clean model and merging it with the backdoored model, plus external distraction by incorporating benign evidence into prompts to shift attention from backdoor features.
...
Method: EASI-RAG uses method engineering principles with defined roles, activities, and techniques. Validated through real-world implementation in an environmental lab where operators query operational procedures data.

Result: System deployed in under a month by inexperienced team, achieved high user adoption, accurate answers, and improved data reliability. Iterative improvements based on user feedback.

Conclusion: EASI-RAG enables successful RAG deployment in industrial SMEs. Future work includes generalization across use cases and integration with fine-tuned models.
...
Conclusion: Noise in distantly supervised datasets and re-representation capability are identified as key challenges in sentential relation extraction, with capsule networks showing superior re-representation abilities.
...
Result: Dramatically improves performance on the StAtutory Reasoning Assessment (SARA) dataset and reduces deployment costs well below real-world averages ($270 and 13 hours per filing).
...
<details>
  <summary>Details</summary>
Motivation: Address the persistent problem of hallucinations in Multimodal Large Language Models (MLLMs) where they generate answers not reflected in visual inputs, treating it as an alignment problem.
...
Result: CHAIR-DPO effectively reduces hallucinated answers across several hallucination benchmarks, demonstrating the effectiveness of CHAIR-based reward for fine-tuning.

Conclusion: The proposed method provides a simpler alternative to complex synthetic data pipelines for addressing MLLM hallucinations, achieving state-of-the-art performance through CHAIR-based DPO alignment.
...
<details>
  <summary>Details</summary>
Motivation: Existing image forgery localization methods struggle with emerging manipulation technologies due to reliance on costly annotated data, while multi-modal large models like SD offer new opportunities for forensic analysis.
...
Result: Achieves up to 12% performance improvement on benchmarking datasets and demonstrates strong generalization to unseen real-world document forgery and natural scene forging images.

Conclusion: The integration of SD's generation and perceptual capabilities provides an efficient and accurate framework for forgery localization that outperforms state-of-the-art methods without requiring annotated training data.
...
<details>
  <summary>Details</summary>
Motivation: AI models show promise in skin disease diagnosis but lack sufficient interpretability for clinical use. The researchers aim to enhance interpretability by combining MLLMs' natural language reasoning with quantitative lesion attributes.

Method: Fine-tune MLLMs to predict quantitative lesion attributes (e.g., lesion area) from images and evaluate grounding through attribute-specific content-based image retrieval using the SLICE-3D dataset.

Result: Evidence shows that MLLM embedding spaces can be effectively grounded in quantitative lesion attributes through fine-tuning to predict attribute values from images.

Conclusion: Combining MLLMs with quantitative attributes offers a promising approach to improve interpretability of AI-based skin cancer diagnosis, potentially enabling clinical adoption.
...
Method: ViT encoder with lightweight convolutional decoder and linear classifier; reconstruction branch maps augmented signals to originals to capture fine-grained I/Q structure; combines supervised, self-supervised, and reconstruction objectives.
...
Conclusion: InfinityHuman successfully addresses key challenges in audio-driven human animation by providing stable, high-resolution long-duration videos with improved hand motions and lip synchronization through its pose-guided refinement approach.
...
Method: Develops a pipeline leveraging Vision-Language Models to provide explanations for vision models at both sample-level (individual explanations) and dataset-level (general behavior patterns after analyzing large datasets).

Result: The proposed pipeline enables discovery of failure cases and provides insights into vision models with minimal effort, facilitating integration of xAI analysis with vision model development.

Conclusion: This approach advances image analysis by combining vision model development with explainable AI, allowing for better understanding of model behavior at both granular and holistic levels to prevent biases and identify trends.
...
TL;DR: ATMS-KD framework combines adaptive temperature scheduling with mixed-sample augmentation for knowledge distillation, achieving over 96.7% accuracy on agricultural rose classification with lightweight CNN models.
...
Method: Proposes ATMS-KD framework using adaptive temperature scheduling and mixed-sample augmentation. Distills knowledge from MobileNetV3 Large (5.7M params) to three residual CNN students: Compact (1.3M), Standard (2.4M), and Enhanced (3.8M) configurations on Damask rose dataset.

Result: All student models achieved validation accuracies exceeding 96.7% (vs 95-96% direct training). Outperformed 11 established KD methods, achieving 97.11% accuracy with compact model (1.60pp improvement). Maintained lowest inference latency of 72.19ms with 99%+ knowledge retention rates.

Conclusion: ATMS-KD effectively transfers knowledge regardless of student model capacity, making it suitable for agricultural computer vision applications under diverse environmental conditions with resource constraints.
...
TL;DR: A novel framework using hybrid vision-language representations (VLRs) for zero-shot classification of microstructures in additive manufacturing, enabling human-in-the-loop qualification without retraining.


<details>
  <summary>Details</summary>
Motivation: Rapid and reliable qualification of advanced materials from non-conventional additive manufacturing processes remains a bottleneck, particularly for heterogeneous structures.
...
Result: Successfully distinguishes between acceptable and defective samples on metal matrix composite dataset. FLAVA offers higher visual sensitivity while CLIP provides consistent textual alignment. Z-score normalization enables effective classification.

Conclusion: Enhances traceability and interpretability in qualification pipelines, advances semantic interoperability between raw data and expert knowledge, and contributes to scalable domain-adaptable qualification strategies.
...
TL;DR: MedNeXt-L-k5, a Transformer-inspired 3D network, was adapted for automated perivascular spaces (PVS) segmentation in MRI, achieving high performance on T2-weighted images but moderate results on T1-weighted images across diverse datasets.
...
Method: Adapted MedNeXt-L-k5 (Transformer-inspired 3D encoder-decoder convolutional network) for PVS segmentation. Trained two models: one on homogeneous T2w MRI from HCP-Aging dataset (200 scans), another on heterogeneous T1w MRI from seven studies across six scanners (40 volumes). Evaluated using 5-fold cross validation and leave-one-site-out cross validation.

Result: T2w model achieved voxel-level Dice score of 0.88±0.06 (WM), comparable to inter-rater reliability. T1w model achieved lower Dice score of 0.58±0.09 (WM). Under LOSOCV: voxel-level Dice 0.38±0.16 (WM) and 0.35±0.12 (BG); cluster-level Dice 0.61±0.19 (WM) and 0.62±0.21 (BG). Did not outperform nnU-Net.

Conclusion: MedNeXt-L-k5 provides efficient automated PVS segmentation across diverse T1w and T2w MRI datasets, but attention-based mechanisms in transformer-inspired models are not required for high accuracy in PVS segmentation.
...
TL;DR: Training-free feedback framework that adapts output patch correspondences back to intermediate attention to enhance spatial coherence and semantic consistency in CLIP for open-vocabulary segmentation.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with open-vocabulary segmentation due to poor localization and semantic discrepancy between intermediate attention and final outputs, limiting its full potential despite strong visual-textual alignment.

Method: Proposes a feedback-driven self-adaptive framework that uses output predictions as spatial coherence prior, with attention isolation, confidence-based pruning for sparse adaptation, and adaptation ensemble modules to propagate coherence cues back to intermediate attention.

Result: Consistently improves performance across eight benchmarks when integrated into four state-of-the-art approaches with three backbones (ViT-B, ViT-L, ViT-H) and validated across multiple attention types (Q-K, self-self, Proxy augmented with MAE, SAM, DINO).

Conclusion: The framework effectively enhances semantic consistency between internal representations and final predictions, serving as a plug-in module that significantly boosts CLIP's open-vocabulary segmentation capabilities without requiring training.
...
TL;DR: A probing framework to analyze layer-wise processing in Multimodal LLMs reveals consistent stage-wise structure: early layers handle visual grounding, middle layers manage lexical integration and semantic reasoning, and final layers prepare task-specific outputs.


<details>
  <summary>Details</summary>
Motivation: To systematically understand how Multimodal Large Language Models process visual and textual inputs across different layers, as their internal processing dynamics remain underexplored despite strong performance.

Method: Train linear classifiers to predict fine-grained visual categories from token embeddings at each layer using standardized anchor questions, and evaluate under three prompt variations: lexical variants, semantic negation variants, and output format variants.

Result: Identified consistent stage-wise structure across LLaVA-1.5, LLaVA-Next-LLaMA-3, and Qwen2-VL models. Early layers perform visual grounding, middle layers support lexical integration and semantic reasoning, final layers prepare outputs. Stage structure remains stable but layer allocation shifts with base LLM architecture changes.

Conclusion: Provides unified perspective on MLLM layer organization and offers lightweight, model-agnostic approach for analyzing multimodal representation dynamics, revealing consistent functional stages despite architectural variations.
...
<details>
  <summary>Details</summary>
Motivation: To separate complex scene information in vision-language co-embedding spaces by decomposing embeddings into concept-specific component vectors that lie in different subspaces, improving precision in downstream tasks.

Method: Supervised dictionary learning approach with linear synthesis model using sparse, non-negative combinations of dictionary atoms. Group-structured dictionary optimized through novel alternating optimization with guaranteed convergence. Uses text co-embeddings for semantic descriptions and zero-shot classification for instance-wise multi-labels.

Result: SLiCS enables more precise concept-filtered image retrieval and conditional generation using image-to-prompt. Works effectively with various embeddings including CLIP, TiTok autoencoder embeddings, and DINOv2 latent embeddings.

Conclusion: The disentangled embeddings from sparse linear concept subspaces (SLiCS) provide improved precision in concept-filtered image retrieval across different embedding types, demonstrating the effectiveness of the proposed supervised dictionary learning approach.
...
TL;DR: MedFoundationHub is a GUI toolkit that enables secure deployment of medical vision-language models for clinical use while addressing privacy concerns and making advanced AI accessible to physicians without programming skills.


<details>
  <summary>Details</summary>
Motivation: Medical VLMs offer significant clinical benefits but introduce serious security risks including PHI exposure, data leakage, and cyberthreats. Healthcare organizations need secure, accessible tools to leverage these models safely.

Method: Developed MedFoundationHub - a GUI toolkit with three main features: 1) enables physician model selection without programming, 2) plug-and-play deployment with Hugging Face integration, 3) privacy-preserving Docker-based inference requiring only a single NVIDIA A6000 GPU.

Result: Evaluated 5 state-of-the-art VLMs (Google-MedGemma3-4B, Qwen2-VL-7B-Instruct, Qwen2.5-VL-7B-Instruct, LLaVA-1.5-7B/13B) with board-certified pathologists. 1015 scoring events revealed limitations: off-target answers, vague reasoning, and inconsistent pathology terminology.

Conclusion: MedFoundationHub provides a secure, accessible solution for medical VLM deployment while current models still show significant limitations in clinical accuracy and terminology consistency that need addressing.
...
TL;DR: Bidirectional Interaction Mamba (BIM) addresses computational complexity in multi-task dense prediction by introducing novel scanning mechanisms that enable efficient cross-task interaction while maintaining linear complexity.
...
Method: Proposes BIM with two novel scanning mechanisms: 1) Bidirectional Interaction Scan (BI-Scan) that constructs task-specific representations as bidirectional sequences, integrating task-first and position-first scanning modes; 2) Multi-Scale Scan (MS-Scan) for multi-granularity scene modeling to meet diverse task requirements.

Result: Extensive experiments on NYUD-V2 and PASCAL-Context benchmarks demonstrate BIM's superiority over state-of-the-art competitors.

Conclusion: BIM successfully achieves efficient and complete cross-task interaction for multi-task dense prediction through novel scanning mechanisms that maintain linear computational complexity while preserving critical cross-task information.
...
TL;DR: Audio-guided visual editing framework that handles complex editing tasks with multiple text and audio prompts without additional training, using pre-trained multi-modal encoders and novel noise branching techniques.


<details>
  <summary>Details</summary>
Motivation: Existing visual editing methods struggle with complex scenarios where textual guidance alone is insufficient, and audio-guided approaches require specific training that limits generalization to real-world situations.

Method: Leverages pre-trained multi-modal encoder with zero-shot capabilities, integrates diverse audio into visual editing by aligning audio encoder space with diffusion model's prompt encoder space, and uses separate noise branching with adaptive patch selection for handling multiple multi-modal prompts.

Result: Comprehensive experiments show the framework excels in handling complicated editing scenarios by incorporating rich audio information where text-only approaches fail, without requiring additional training.

Conclusion: The proposed audio-guided visual editing framework successfully addresses complex editing tasks through multi-modal integration and novel architectural approaches, demonstrating superior performance over text-only methods in challenging scenarios.
...
Method: Developed Generalized Pseudo-Label Robust Loss (GPR Loss) to learn from diverse pseudo-labels while mitigating noise, and Dynamic Augmented Multi-focus Pseudo-labeling (DAMP) technique. Combined these in the Adaptive and Efficient Vision-Language Pseudo-Labeling (AEVLP) framework.
...
TL;DR: Proposed delay-spike approach and temporal-dependent Integrate-and-Fire (tdIF) neuron to improve SNN performance in visual detection tasks with ultra-low latency (5 time-steps)


<details>
  <summary>Details</summary>
Motivation: Current ANN-SNN conversion methods perform well in classification but suboptimal in visual detection tasks due to residual membrane potential issues from heterogeneous spiking patterns

Method: Delay-spike approach to mitigate residual membrane potential, and novel tdIF neuron that dynamically adjusts accumulation/firing based on temporal order of time-steps

Result: Achieves state-of-the-art performance in object detection and lane line detection with ultra-low latency (within 5 time-steps), surpassing current ANN-SNN conversion approaches

Conclusion: The tdIF method enables more precise feature representation with lower time-steps, maintaining energy efficiency while improving performance in visual detection tasks
...
<details>
  <summary>Details</summary>
Motivation: Existing SOD methods lose details, blur edges, and insufficiently fuse single-modal information in complex scenes, requiring better handling of small structures and cross-modal complementarity.
...
<details>
  <summary>Details</summary>
Motivation: Existing end-to-end MVPD methods struggle with detecting pedestrians that have consistently small/large scales within views or vastly different scales between views, due to not exploiting multi-scale image features.

Method: Proposes MSMVD which: 1) Generates multi-scale BEV features by projecting multi-scale image features into BEV space scale-by-scale, 2) Uses feature pyramid network to combine information at different scales from multiple views.
...
Conclusion: Exploiting multi-scale image features through multi-scale BEV features greatly improves MVPD performance, especially for pedestrians with varying scales across different views.
...
Method: Proposes Spatial-Frequency Aware Multi-Scale Fusion Network with spatial-frequency hybrid aware module, token-selective cross attention mechanism, and residual-enhanced blur pooling structure.
...
TL;DR: A lightweight medical image classification method combining dual-model weight selection from large pretrained models with self-knowledge distillation to achieve comparable performance to large models while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Real-world medical settings face computational constraints that limit deployment of large-scale models, creating need for lightweight models that maintain performance without excessive resource requirements.

Method: Uses dual-model weight selection strategy to initialize two lightweight models with weights from large pretrained model, then applies self-knowledge distillation to enable broad weight configurations without extra computational cost, followed by fine-tuning for target tasks.
...
Conclusion: The combination of dual-model weight selection with self-knowledge distillation effectively overcomes limitations of conventional approaches that fail to retain critical information in compact models for medical image classification.
...
TL;DR: A novel LiDAR point cloud compression method using geometry re-densification and cross-scale feature propagation to achieve efficient context modeling and real-time performance.
...
Method: Two lightweight modules: Geometry Re-Densification Module that re-densifies sparse geometry and re-sparsifies features, and Cross-scale Feature Propagation Module that leverages multi-resolution occupancy cues for hierarchical feature sharing.

Result: State-of-the-art compression ratios on KITTI dataset with real-time performance (26 FPS for both encoding and decoding at 12-bit quantization).

Conclusion: The proposed framework provides compact feature representation for efficient predictive coding, overcoming sparsity challenges while maintaining lightweight computation and fast processing speeds.
...
Method: Introduces Droplet3D-4M, the first large-scale video dataset with multi-view annotations, and trains Droplet3D generative model supporting both image and dense text input to leverage video commonsense priors.
...
<details>
  <summary>Details</summary>
Motivation: Collecting corner cases for autonomous driving validation is costly and dangerous. Existing editing methods suffer from limited visual fidelity and imprecise pose control.

Method: Uses 3D Gaussian representation as dense prior injected into denoising process, scene-level 3D bounding box layout for occlusion handling, and hierarchical fine-grained features for appearance guidance.
...
Conclusion: G^2Editor provides a unified framework for photorealistic object editing that benefits downstream autonomous driving tasks while overcoming limitations of previous approaches.
...
Method: Proposed pathology-informed domain randomization strategy that embeds prior knowledge of CCD manifestations into synthetic data generation pipeline, simulating diverse brain alterations from healthy data alone.

Result: Achieved substantial improvements on CCD cases while maintaining performance on healthy fetuses and other pathologies. Reduced LCC estimation error from 1.89mm to 0.80mm in healthy cases and from 10.9mm to 0.7mm in CCD cases. Improved topological consistency in segmentations.
...
<details>
  <summary>Details</summary>
Motivation: To make communication technologies accessible to deaf and hard-of-hearing individuals by integrating visual alternatives (sign language and lip reading) with audio in a unified system, as these modalities have traditionally been studied in isolation.

Method: Designed a unified, modality-agnostic architecture capable of processing heterogeneous inputs (sign language, lip movements, audio) for spoken-language text generation. Explicitly modeled lip movements as a separate modality and explored their synergy with other communication forms.

Result: Achieved performance on par with or better than state-of-the-art task-specific models across Sign Language Translation (SLT), Visual Speech Recognition (VSR), Automatic Speech Recognition (ASR), and Audio-Visual Speech Recognition (AVSR). Explicit modeling of lip movements significantly improved SLT performance.

Conclusion: The unified framework successfully integrates multiple communication modalities, demonstrating that explicit modeling of lip movements enhances sign language comprehension and that a single architecture can achieve state-of-the-art performance across diverse communication tasks.
...
TL;DR: Video-MTR is a reinforced multi-turn reasoning framework that iteratively selects key video segments and comprehends questions through multiple reasoning turns, outperforming existing methods in long video understanding.
...
Conclusion: Video-MTR advances the state-of-the-art in long video understanding by enabling iterative reasoning and eliminating the need for external VLMs through end-to-end training with a novel reward system.
...
<details>
  <summary>Details</summary>
Motivation: Current TTA methods for monocular 3D object detection fail to address the dual uncertainty problem - semantic uncertainty (ambiguous class predictions) and geometric uncertainty (unstable spatial localization) - which deteriorates reliability under real-world domain shifts.

Method: Proposes Dual Uncertainty Optimization (DUO) framework with: 1) convex optimization approach using novel unsupervised focal loss for label-agnostic uncertainty weighting, 2) semantic-aware normal field constraint to preserve geometric coherence, and 3) dual-branch mechanism that forms complementary loop between spatial perception and semantic classification.
...
Conclusion: DUO effectively addresses the dual uncertainty problem in monocular 3D object detection through joint optimization of semantic and geometric uncertainties, providing robust performance under domain shifts.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning studies lack quantitative relationship between swing posture and ball trajectory, limiting actionable insights for golfers' swing improvement.

Method: Create CaddieSet dataset with joint information from swing videos segmented into 8 phases using computer vision, plus ball trajectory data. Define 15 key swing metrics based on expert domain knowledge for interpretable analysis.

Result: Demonstrated feasibility of predicting ball trajectories using various benchmarks. Interpretable models show swing feedback using joint features is quantitatively consistent with established golf domain knowledge.

Conclusion: CaddieSet provides new insights for golf swing analysis in both academia and sports industry by enabling quantitative relationship analysis between swing posture and ball trajectory outcomes.
...
<details>
  <summary>Details</summary>
Motivation: 3D point cloud-based anomaly detection lags behind 2D methods due to the lack of powerful pretrained foundation models in 3D, despite offering richer geometric information for industrial quality control.

Method: Proposes Importance-Aware Ensemble Network (IAENet) with an Importance-Aware Fusion module that dynamically assesses and reweights contributions from 2D and 3D expert models, guided by specialized loss functions to optimize fusion while preserving individual strengths.

Result: Extensive experiments on MVTec 3D-AD show IAENet achieves new state-of-the-art performance with significantly lower false positive rate compared to existing methods.

Conclusion: The proposed ensemble framework successfully bridges the gap between 2D and 3D anomaly detection by intelligently fusing multimodal expertise, demonstrating practical value for industrial deployment with improved accuracy and reliability.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations in current semantic image editing methods, including reconstruction errors in inversion-based approaches and dataset quality issues in instruction-based models.

Method: Proposes a Cross-Attentive UNet that adds attention bridges to inject reference image features into the text-to-image generation process, treating editing as reference-based generation rather than direct instruction following.

Result: Experiments on Emu Edit benchmark show improved editing accuracy and consistency compared to existing methods.

Conclusion: DescriptiveEdit provides a more scalable and effective approach to semantic image editing by leveraging well-trained text-to-image models without architectural modifications, while seamlessly integrating with existing extensions like ControlNet and IP-Adapter.
...
TL;DR: DCFS is a novel continual test-time adaptation framework that uses dual-path feature consistency and confidence-aware learning to address error accumulation and pseudo-label noise problems in target domain adaptation without source data.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods rely on pseudo-labels from model predictions, but these suffer from quality issues and error accumulation. Without source data, models can develop biases from focusing only on target domain features.
...
Conclusion: DCFS effectively addresses error accumulation and pseudo-label quality issues in CTTA by leveraging dual-path feature consistency and confidence-aware sample learning, providing comprehensive feature capture from multiple perspectives.
...
TL;DR: Using 3D Gaussian Splatting to refine camera calibration by backpropagating novel view color loss, achieving 0.4 dB PSNR improvement on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Camera calibration quality significantly impacts novel view synthesis results, with even 1-pixel errors having substantial effects on reconstruction quality. Current methods lack ground truth for real scenes, making calibration assessment dependent on synthesis quality.

Method: Proposes using a 3DGS (3D Gaussian Splatting) model to fine-tune camera calibration through backpropagation of novel view color loss with respect to camera parameters.
...
Conclusion: While the fine-tuning process may be time-consuming, the method is particularly valuable for reference scenes like Mip-NeRF 360 where novel view quality is paramount, making it suitable for critical calibration scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Addresses the limitations of previous active domain adaptation methods that suffer from negative transfer due to one-off sample selection and lack of multi-modal query strategies, particularly in medical imaging where labeling is expensive and source data access is limited.
...
Result: Achieves superior segmentation performance on diverse gross tumor volume segmentation tasks, significantly outperforming state-of-the-art ADA methods.

Conclusion: The proposed framework effectively reduces annotation costs while maintaining high segmentation accuracy through dynamic multi-modal sample selection, making it valuable for radiotherapy planning in nasopharyngeal carcinoma and glioblastoma.
...
<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-based 3D object detectors require manually annotated labels which are time-consuming to obtain. Current unsupervised methods simply integrate pseudo-boxes from LiDAR and RGB at label level, overlooking the complementary nature of these data modalities.

Method: Proposes data-level fusion using vision foundation models for instance segmentation and depth estimation. Uses bi-directional fusion where real points get category labels from 2D space and 2D pixels are projected to 3D to enhance point density. Implements local radius filtering and global statistical filtering to mitigate noise. Includes data-level fusion based dynamic self-evolution strategy for iterative pseudo-box refinement.

Result: Achieves 28.4% mAP on nuScenes validation benchmark, significantly outperforming previous state-of-the-art methods.

Conclusion: The proposed data-level fusion framework effectively leverages complementary information from LiDAR and RGB modalities, demonstrating superior performance in unsupervised 3D object detection compared to label-level fusion approaches.
...
Conclusion: The method provides accurate BMI estimation from smartphone images with strong generalization capabilities, and the complete pipeline including filtering and mobile deployment is released as open-source.
...
TL;DR: Comprehensive study of 7 domain adaptation techniques across 13 natural and medical image datasets, showing DSAN algorithm's superior performance particularly in medical applications like COVID-19 classification and dynamic data streams.


<details>
  <summary>Details</summary>
Motivation: To better understand domain adaptation benefits for both natural and medical images, addressing performance bias in mainstream datasets and the challenges of medical data which are harder to work with.

Method: Performed 557 simulation studies using 7 widely-used DA techniques for image classification across 5 natural and 8 medical datasets, covering scenarios like out-of-distribution, dynamic data streams, and limited training samples.

Result: DSAN algorithm achieved 91.2% accuracy on COVID-19 dataset using Resnet50, showed +6.7% improvement in dynamic data stream scenario compared to baseline, and demonstrated remarkable explainability on COVID-19 and skin cancer datasets.

Conclusion: The study provides detailed insights into DA techniques' performance and medical applicability, with DSAN emerging as particularly effective for adapting models to medical data across various challenging scenarios.
...
TL;DR: CLAB method improves video object detection using contrastive learning and dynamic loss weighting, achieving state-of-the-art performance without extra computational cost during inference.


<details>
  <summary>Details</summary>
Motivation: Video object detection faces challenges from image deterioration like motion blur and occlusion. Existing methods improve performance but increase computational demands, so the goal is to enhance robustness without additional inference cost.

Method: Introduces Contrastive Learning through Auxiliary Branch (CLAB) with contrastive loss to improve feature representation, plus dynamic loss weighting that shifts focus from auxiliary learning to detection task during training.
...
Method: Analyze CLIP vision encoders to locate specialized attention heads that extract typographic information. Introduce a training-free defense by selectively ablating a typographic circuit consisting of these attention heads.

Result: Method improves performance by up to 19.6% on typographic ImageNet-100 variant while reducing standard ImageNet-100 accuracy by less than 1%. Competes with state-of-the-art finetuning-based defenses. Releases family of dyslexic CLIP models as drop-in replacements.

Conclusion: The proposed training-free ablation approach effectively defends CLIP models against typographic attacks, making them suitable for safety-critical applications where text-based manipulation risks outweigh text recognition utility.
...
TL;DR: GLaRE is a Graph Neural Network-based approach for facial expression recognition that uses hierarchical coarsening of facial landmarks to create region-level embeddings, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional FER systems struggle with occlusion, expression variability, and lack of interpretability. GNNs can model relational dependencies between facial landmarks for more structured and interpretable learning.

Method: Extract facial landmarks using 3D facial alignment, construct a quotient graph via hierarchical coarsening to preserve spatial structure while reducing complexity, and use graph-based landmark region embeddings for emotion recognition.

Result: Achieved 64.89% accuracy on AffectNet and 94.24% on FERG, outperforming existing baselines. Ablation studies confirmed that region-level embeddings from quotient graphs improve prediction performance.

Conclusion: Graph-based approaches with hierarchical coarsening and region-level embeddings provide an effective solution for facial expression recognition, offering both improved performance and interpretability compared to traditional methods.
...
TL;DR: FastFit is a high-speed multi-reference virtual try-on framework using cacheable diffusion architecture that achieves 3.5x speedup while maintaining quality, with a new dataset DressCode-MR for complex multi-reference scenarios.
...
Method: Proposes FastFit with Semi-Attention mechanism and class embeddings instead of timestep embeddings, decoupling reference feature encoding from denoising process. Also introduces DressCode-MR dataset with 28,179 sets of high-quality paired images across 5 categories.
...
Conclusion: FastFit breaks the efficiency bottleneck of virtual try-on systems and enables practical multi-reference outfit composition with significant speed improvements without sacrificing quality.
...
TL;DR: UTA-Sign: Unsupervised thermal-event fusion for traffic signage in low-light conditions, combining thermal cameras and event cameras to overcome limitations of each modality.


<details>
  <summary>Details</summary>
Motivation: Thermal cameras struggle with signage from similar materials, while event cameras have non-uniform sampling. Both have complementary strengths for low-light autonomous driving applications.

Method: Dual-boosting mechanism that fuses thermal frames and event signals, using thermal for motion cues and events for subtle signage content enhancement.
...
Conclusion: The proposed fusion method effectively addresses signage blind spots in thermal imaging and sampling issues in event cameras, enhancing low-light traffic environment perception.
...
Conclusion: The proposed active defense method effectively disrupts deepfake generation process while ensuring output remains visually plausible, offering a proactive approach to deepfake prevention.
...
Method: Introduces uncertainty via noise signals in transformer-based methods, creates continuous text representation in latent space, and integrates stochastic sampling through a latent space sampler.

Result: Significantly enhances motion diversity while maintaining state-of-the-art text consistency performance on HumanML3D and KIT-ML benchmark datasets.
...
Method: Proposed an optimization-based calibration method using a 3D-printed phantom for accurate 3D Intravascular Ultrasound (IVUS) volume reconstruction. Ensures precise alignment of tracked IVUS data with preoperative CT images.
...
<details>
  <summary>Details</summary>
Motivation: Limited availability of MFI datasets due to proprietary concerns creates a bottleneck for training machine learning models in semiconductor defect detection, while X-ray scanning is memory-intensive and time-consuming.

Method: Physics Informed Generative Models for Magnetic Field Images (PI-GenMFI) using diffusion models with two physical constraints to generate synthetic MFI samples, specifically for power shorts defects.

Result: The model shows promising results in qualitative and quantitative evaluations compared to state-of-the-art generative models (VAE and diffusion methods), with domain expert validation confirming sample quality.

Conclusion: PI-GenMFI effectively generates synthetic MFI training data that can optimize defect localization processes in semiconductor manufacturing by overcoming data scarcity issues.
...
<details>
  <summary>Details</summary>
Motivation: Split inference in deep neural networks exchanges intermediate features that can be exploited to recover sensitive input data, but existing data reconstruction attacks are limited to shallow models and lack semantic fidelity.

Method: Proposed a GAN-based framework with Progressive Feature Optimization (PFO) that decomposes the generator into hierarchical blocks and incrementally refines intermediate representations, with L1-ball constraint to stabilize optimization and improve image realism.

Result: Extensive experiments show the method outperforms prior attacks by a large margin, particularly in high-resolution scenarios, out-of-distribution settings, and against deeper and more complex DNNs.

Conclusion: The proposed GAN-based framework with progressive feature optimization significantly advances data reconstruction attack capabilities, demonstrating effectiveness across various challenging scenarios and model architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations in control flexibility, motion naturalness, and expression quality, and available datasets are primarily collected in lab settings, hindering practical applications in real-world scenarios.
...
Conclusion: The proposed framework effectively addresses limitations of existing methods by providing precise text-driven emotional synthesis with improved emotion comprehension and accurate facial motion synchronization.
...
<details>
  <summary>Details</summary>
Motivation: Early breast cancer detection is crucial, especially in high-risk women with dense breast tissue where mammography is less effective. MRI is highly sensitive but requires AI support for improved diagnosis.

Method: Developed SwinUNETR-based deep learning framework with breast region masking, extensive data augmentation, and ensemble learning using multi-center dataset from 6 European centers with 511 studies.

Result: Achieved second place on the ODELIA consortium challenge leaderboard, demonstrating robust performance and generalizability across different scanners and field strengths.

Conclusion: The framework shows strong potential to support clinical breast MRI interpretation and has been made publicly available to advance AI in breast cancer diagnosis.
...
TL;DR: AvatarBack is a plug-and-play framework that reconstructs complete 3D Gaussian avatars by generating identity-consistent back-view images and aligning them with frontal views to address poor back-head reconstruction in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian Splatting methods for head avatars rely mainly on frontal-view images, resulting in poorly constructed back-head regions with geometric inconsistencies, structural blurring, and reduced realism, limiting avatar fidelity.
...
Conclusion: AvatarBack effectively addresses the back-head reconstruction challenge in 3D Gaussian avatars through generative pseudo-image synthesis and adaptive spatial alignment, enabling complete and consistent avatar modeling with enhanced realism.
...
TL;DR: Foundation models improve facial recognition in historical paintings by addressing domain shift and artistic variations through fine-tuning and embedding integration with traditional networks.
...
Method: Fine-tune foundation models and integrate their embeddings with conventional facial recognition networks to handle artistic variations and domain differences.

Result: Demonstrates notable improvements over current state-of-the-art methods in facial recognition for artworks.

Conclusion: Foundation models can effectively bridge the performance gap where traditional facial recognition methods fail in the artistic domain.
...
Method: Uses LoRA-fine-tuned pretrained diffusion transformer for style transfer, face-consistent self-attention with explicit identity embeddings for identity preservation, and CLIP-guided prompt extension for pose customization without keypoints.

Result: Achieves competitive facial feature consistency, state-of-the-art aesthetic and human preference scores, and demonstrates real-world impact through live deployment at Cruilla Festival.

Conclusion: CraftGraffiti advances identity-respectful AI-assisted artistry by providing a principled approach that balances stylistic freedom with facial recognizability in creative applications.
...
TL;DR: A novel self-evaluation method for Large Visual-Language Models that generates debiased self-judgment scores internally to improve visual-linguistic alignment, reduce hallucinations, and enhance safety without external resources.


<details>
  <summary>Details</summary>
Motivation: Existing alignment methods for LVLMs rely on external datasets, human annotations, or complex post-processing, which limit scalability and increase costs while still suffering from hallucinations and safety issues.

Method: Proposes generating debiased self-judgment scores internally by the model itself without external resources, enhancing both decoding strategies and preference tuning processes for autonomous alignment improvement.
...
Conclusion: The approach offers a more effective and scalable solution for aligning LVLMs by enabling autonomous self-evaluation and improvement without relying on external resources.
...
TL;DR: S-HArM dataset for intent-aware classification of AI-generated images (Humor/Satire, Art, Misinformation) with 9,576 real image-text pairs and synthetic data generation using three prompting strategies.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal AI efforts overlook the intent behind AI-generated images, creating a gap in understanding whether content is humorous, artistic, or misinformation.
...
Result: Models trained on image- and multimodally-guided data generalize better to real content due to preserved visual context, but overall performance remains limited.

Conclusion: Inferring intent from AI-generated images is complex and requires specialized architectures beyond current approaches.
...
TL;DR: MobileCLIP2 improves upon MobileCLIP with better teacher ensembles and captioner fine-tuning, achieving state-of-the-art zero-shot accuracy at low latencies.
...
Result: MobileCLIP2 achieves 2.2% ImageNet-1k accuracy improvement over MobileCLIP-B, matches SigLIP-SO400M/14 accuracy while being 2x smaller, and outperforms DFN ViT-L/14 at 2.5x lower latency.

Conclusion: MobileCLIP2 demonstrates significant improvements in zero-shot accuracy with efficient architecture, providing state-of-the-art performance for mobile and low-latency applications.
...
TL;DR: Proposed Dynamic-Route Autoencoder framework for neural video compression with precise rate control, achieving 14.8% BD-Rate reduction and 0.47dB BD-PSNR gain over SOTA methods with only 1.66% average bitrate error.
...
Method: Dynamic-Route Autoencoder with variable coding routes, Rate Control Agent for bitrate estimation and route adjustment, and Joint-Routes Optimization strategy for collaborative training.

Result: 14.8% average BD-Rate reduction, 0.47dB BD-PSNR gain over state-of-the-art methods, with 1.66% average bitrate error on HEVC and UVG datasets.

Conclusion: Achieves Rate-Distortion-Complexity Optimization for various bitrate applications, providing effective variable bitrate neural video compression with precise rate control.
...
Method: A recurrent variational autoencoder framework with two posterior models for bi-ventricular segmentation and motion estimation. It uses Bayesian formulation to guide registration of segmentation maps without intensity-based similarity loss, leveraging sequential SAX volumes and spatio-temporal features.
...
Conclusion: CardioMorphNet provides more accurate and confident cardiac motion estimation by focusing on anatomical regions through shape-guided registration rather than intensity-based approaches, with Bayesian modeling enabling uncertainty quantification.
...
TL;DR: A simple training-time recipe for domain-robust atypical mitotic figure classification that uses style perturbations, attention-aligned features with domain metadata, and EMA teacher distillation, achieving strong performance on MIDOG 2025 Task 2.
...
Method: Three key components: (i) style perturbations at early/mid backbone stages for feature diversity, (ii) attention-aligned features across sites using weak domain labels with auxiliary loss, (iii) EMA teacher distillation with temperature-scaled KL divergence for prediction stability.

Result: Achieved balanced accuracy 0.8762, sensitivity 0.8873, specificity 0.8651, ROC AUC 0.9499 on MIDOG 2025 preliminary leaderboard with negligible inference overhead.

Conclusion: The method provides strong, balanced performance using only coarse domain metadata, making it a competitive submission for the MIDOG 2025 challenge.
...
<details>
  <summary>Details</summary>
Motivation: Current GRPO-based T2I methods suffer from reward hacking where minimal score differences are amplified, causing unstable generation. Existing benchmarks also lack fine-grained evaluation criteria.

Method: Pref-GRPO uses pairwise preference comparisons within groups and win rates as reward signals. UniGenBench provides 600 prompts across 5 themes with detailed semantic consistency evaluation using MLLM.

Result: Pref-GRPO differentiates subtle quality differences, provides stable advantages, and mitigates reward hacking. UniGenBench reveals strengths/weaknesses of T2I models and validates Pref-GRPO effectiveness.

Conclusion: The proposed Pref-GRPO method and UniGenBench address key limitations in current T2I reinforcement learning and benchmarking, providing more stable training and comprehensive evaluation.
...
Conclusion: The proposed C3-GS framework successfully addresses feature learning limitations in generalizable Gaussian splatting, enabling photorealistic synthesis from sparse input views without per-scene optimization.
...
TL;DR: SeqVLM is a zero-shot 3D visual grounding framework that uses multi-view images with spatial information for object localization without scene-specific training, achieving state-of-the-art performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot 3DVG methods suffer from spatial-limited reasoning due to single-view localization and contextual omissions. There's a need for methods that work without scene-specific training for real-world applications.
...
Result: Achieves Acc@0.25 scores of 55.6% on ScanRefer and 53.2% on Nr3D benchmarks, surpassing previous zero-shot methods by 4.0% and 5.2% respectively.

Conclusion: SeqVLM advances 3D visual grounding toward greater generalization and real-world applicability by effectively leveraging multi-view scene images and spatial information without requiring scene-specific training.
...
TL;DR: CLIP models show varying robustness to occlusion in military vehicle classification, with Transformers outperforming CNNs and fine-tuning significantly improving occlusion resilience.


<details>
  <summary>Details</summary>
Motivation: Evaluate CLIP's robustness in challenging military environments with occlusion and degraded SNR, which remains underexplored despite VLMs' zero-shot classification advantages for defense applications with limited labeled data.

Method: Test CLIP variants on custom military vehicle dataset (18 classes) using Normalized Area Under the Curve (NAUC) across different occlusion percentages, comparing Transformer vs CNN architectures and analyzing fine-tuning effects.

Result: Four key findings: Transformers outperform CNNs; fine-grained occlusions degrade performance more than contiguous ones; linear-probed models drop sharply at ~35% occlusion; backbone fine-tuning pushes performance drop to >60% occlusion.

Conclusion: Occlusion-specific augmentations during training are crucial, and further research is needed on patch-level sensitivity and architectural resilience for real-world CLIP deployment in defense applications.
...
TL;DR: SKGE-Swin architecture combines Swin Transformer with skip-stage mechanism for autonomous vehicles, achieving superior driving performance in CARLA simulations.


<details>
  <summary>Details</summary>
Motivation: To develop an end-to-end autonomous vehicle model with enhanced pixel-to-pixel context awareness and better understanding of complex environmental patterns.

Method: Uses Swin Transformer with skip-stage mechanism and SW-MSA to extract global features and retain critical information across network levels, evaluated on CARLA platform with adversarial scenarios.

Result: Achieves superior Driving Score compared to previous methods in autonomous driving simulations.

Conclusion: The SKGE-Swin architecture effectively enhances autonomous vehicle perception and performance through improved feature representation and information retention.
...
<details>
  <summary>Details</summary>
Motivation: Existing VideoQA models struggle with higher-order reasoning, relying on opaque monolithic pipelines that lack interpretability and depend on shallow heuristics.

Method: Two-stage architecture with Causal Chain Extractor (generates causal chains from video-question pairs) and Causal Chain-Driven Answerer (produces answers grounded in chains). Uses LLMs to generate annotated causal chains and introduces CauCo evaluation metric.

Result: Outperforms state-of-the-art models on three large-scale benchmarks, with substantial gains in explainability, user trust, and generalization.

Conclusion: The framework successfully decouples causal reasoning, provides transparent inference through interpretable causal chains, and positions CCE as a reusable causal reasoning engine across domains.
...
TL;DR: Survey paper advocating for revisiting abstract concept recognition in videos using modern foundation models, building on decades of community experience rather than reinventing approaches.
...
Method: Survey and analysis of different tasks, datasets, and historical approaches for abstract concept understanding in videos. Advocates for leveraging decades of community experience combined with modern multi-modal foundation models.

Result: Identifies abstract concept recognition as a crucial open challenge in video understanding that requires reasoning on multiple semantic levels using contextual information.

Conclusion: Foundation models provide an ideal setting to address abstract concept understanding in videos, and researchers should build on existing community experience rather than starting from scratch.
...
<details>
  <summary>Details</summary>
Motivation: Address distrust in AI medical diagnosis by providing reliable, explainable methods beyond just high accuracy, overcoming limitations of existing LIME and CAM-based explainability approaches.
...
Result: Method enables unified visualization of diagnostic process to reduce misdiagnosis risk and improves diagnostic reliability through error detection capabilities.

Conclusion: The proposed approach enhances trustworthiness and patient safety in skin lesion classification by providing comprehensive, probabilistic explainability and safety mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models like CLIP often fail at compositional semantics, incorrectly combining attributes and objects. The paper investigates whether generative diffusion-based classifiers have better compositional generalization abilities than discriminative models.

Method: Evaluated three models (Diffusion Classifier, CLIP, and ViLT) on binding objects with attributes and relations in zero-shot and generalized zero-shot learning settings. Analyzed embedding similarities to understand relational reasoning challenges.

Result: Diffusion Classifier and ViLT performed well at concept binding tasks, but all models struggled significantly with relational GZSL tasks. CLIP embeddings showed overly similar representations for relational concepts like left/right.
...
TL;DR: A novel surfel-based pose learning regression method for point cloud registration that uses SE(3) equivariant features to handle rotations and uncertainties, outperforming state-of-the-art methods.
...
Method: Initialize surfels from Lidar point cloud using virtual perspective camera parameters, learn explicit SE(3) equivariant features through SE(3) equivariant convolutional kernels, with an encoder, cross-attention mechanism, decoder, and Huber loss.
...
Conclusion: The proposed surfel-based approach with SE(3) equivariant features effectively addresses rotation and uncertainty challenges in point cloud registration, showing strong performance across diverse datasets.
...
Method: Uses automated tooth detection to create global (panoramic) and local (tooth-cropped) views. Pretrains two vision foundation models separately, then integrates them with a Gated Cross-View Attention module that dynamically fuses dual-view features for final detection.
...
Conclusion: DVCTNet demonstrates clinical applicability and provides a novel approach for accurate dental caries detection by mimicking dentists' systematic workflow.
...
TL;DR: FusionCounting integrates visible and infrared image fusion with crowd counting in a unified multi-task framework, using crowd counting's minimal annotations to provide semantic guidance for better fusion quality and counting accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing VIF methods lack effective semantic guidance. Semantic segmentation requires heavy annotations, while object detection struggles in crowded scenes. RGB-T crowd counting hasn't been integrated with VIF despite its suitability for dense scenes with minimal annotation requirements.

Method: Multi-task learning framework that jointly optimizes visible-infrared image fusion and crowd counting. Uses dynamic loss weighting for task balance and incorporates adversarial training for robustness against attacks.
...
Conclusion: Crowd counting provides effective semantic guidance for VIF with minimal annotation overhead. The unified multi-task framework with dynamic weighting and adversarial training enables mutual benefits between fusion and counting tasks.
...
TL;DR: Novel pipeline using Vision Language Models (VLMs) with LoRA fine-tuning for 2D surgical tool keypoint estimation, outperforming traditional CNNs/Transformers with minimal training.


<details>
  <summary>Details</summary>
Motivation: Traditional CNN and Transformer-based approaches often overfit on small-scale medical datasets, requiring a method that can leverage generalization capabilities of pre-trained models for better performance in low-resource scenarios.

Method: Fine-tune Vision Language Models using LoRA technique with carefully designed prompts to create instruction-tuning dataset, aligning visual features with semantic keypoint descriptions.

Result: With only two epochs of fine-tuning, the adapted VLM outperforms baseline models, demonstrating LoRA's effectiveness in low-resource surgical tool keypoint estimation.

Conclusion: This approach improves keypoint detection performance and provides a foundation for future work in 3D surgical hands and tools pose estimation using VLMs.
...
TL;DR: PointDGRWKV is a novel RWKV-based framework for domain generalization in point cloud classification that addresses spatial distortion and attention shift issues through adaptive geometric token shift and cross-domain key distribution alignment.
...
Conclusion: PointDGRWKV successfully adapts RWKV architecture for domain generalization in point cloud classification, maintaining linear efficiency while enhancing spatial modeling and cross-domain robustness through innovative token shift and distribution alignment techniques.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitations of opaque deep learning models in pathological diagnosis by providing transparent, interpretable AI assistance with localized lesion regions and expert-style diagnostic narratives.

Method: Proposes PathMR, a multimodal visual reasoning framework that takes pathological images and textual queries to generate diagnostic explanations while simultaneously predicting cell distribution patterns at the cell level.
...
Conclusion: PathMR demonstrates significant potential for improving interpretability in AI-driven pathological diagnosis by providing transparent insights through localized lesion identification and expert-level diagnostic narratives.
...
Method: Analyzed 40 patient samples with dual-modality imaging, tested 6 deep learning architectures (CNNs vs ViTs), used modified ResNet with frozen pre-trained layers and class-weighted training to handle limited data and class imbalance.

Result: Achieved over 90% accuracy in distinguishing normal, fibrotic, and cancerous tissue, significantly improving on current manual analysis methods.

Conclusion: Establishes robust automated PDAC detection pipeline for clinical use, provides foundation for expansion to other cancers, and offers insights for deep learning on limited medical imaging datasets.
...
Method: Developed multiple frameworks: CAVLI (combines attribution with concept-level analysis), ASAC (adversarial counterfactuals with curriculum learning), TIBET (scalable bias evaluation pipeline), BiasConnect (causal graphs for intersectional biases), and InterMit (training-free bias mitigation).

Result: Created methods that quantify concept dependencies, improve fairness and accuracy in vision classifiers, enable causal auditing of generative models, and provide scalable bias mitigation without retraining.

Conclusion: Counterfactuals serve as a unifying framework for interpretability, fairness, and causality across both discriminative and generative models, establishing principled methods for socially responsible AI bias evaluation and mitigation.
...
TL;DR: Survey paper on unified perception for autonomous vehicles, proposing a taxonomy with three paradigms (Early, Late, Full Unified) and reviewing methods, architectures, and future directions.
...
Method: Comprehensive survey introducing a holistic taxonomy categorizing methods by task integration, tracking formulation, and representation flow. Systematic review of existing methods, architectures, training strategies, datasets, and open-source availability.

Result: Establishes the first comprehensive framework for understanding unified perception, consolidates fragmented research efforts, and provides clear categorization of approaches.

Conclusion: Unified perception is a promising paradigm that can improve autonomous vehicle perception while retaining interpretability, with future research directions identified for more robust and generalizable systems.
...
TL;DR: Dino U-Net leverages DINOv3 foundation model features for medical image segmentation, achieving state-of-the-art results with a novel architecture that preserves feature fidelity through specialized adapters and projection modules.


<details>
  <summary>Details</summary>
Motivation: Effectively transferring pre-trained foundation model representations from natural images to precise medical image segmentation applications remains challenging, requiring specialized architectures to preserve feature quality.

Method: Proposes Dino U-Net with frozen DINOv3 backbone encoder, specialized adapter for feature fusion, and fidelity-aware projection module (FAPM) to maintain feature quality during dimensionality reduction for the decoder.

Result: Achieves state-of-the-art performance on seven diverse medical image segmentation datasets, with consistent improvements across imaging modalities and scalability up to 7-billion-parameter backbone variants.
...
Method: Trained an ensemble of ConvNeXtBase models using AUCMEDI framework, extended with a rule-based refinement module for classification.
...
Conclusion: Deep ensembles perform well for atypical mitotic figure classification, but rule-based refinement requires further research as it trades sensitivity for specificity.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitations of multi-camera setups in Industry 5.0, which suffer from high computational costs and bandwidth requirements, while edge devices face accuracy degradation and consistency issues.
...
Method: Integrates Segment Anything Model (SAM) with corrections based on tree alignment patterns and learnable constraints about tree shape and size for precise segmentation.
...
Conclusion: The enhanced SAM approach with field-specific constraints provides highly accurate olive tree segmentation, enabling better monitoring and management of olive biodiversity through remote sensing technology.
...
TL;DR: E-ConvNeXt is a lightweight ConvNeXt variant that integrates CSPNet connections and optimizations to reduce complexity by 80% while maintaining high accuracy (78.3-81.9% Top-1) with low computational cost (0.9-3.1GFLOPs).


<details>
  <summary>Details</summary>
Motivation: Many high-performance networks are too complex for lightweight applications, restricting their practical deployment in resource-constrained scenarios.

Method: Integrates Cross Stage Partial Connections (CSPNet) with ConvNeXt, optimizes Stem and Block structures, replaces Layer Scale with channel attention, and adjusts network architecture.

Result: Achieves 78.3% Top-1 accuracy at 0.9GFLOPs (mini version) and 81.9% Top-1 accuracy at 3.1GFLOPs (small version) on ImageNet, with demonstrated generalization capability in object detection tasks.

Conclusion: E-ConvNeXt successfully reduces network complexity by up to 80% while maintaining competitive accuracy, providing an efficient solution for lightweight application scenarios with superior accuracy-efficiency balance.
...
TL;DR: DrivingGaussian++ is an efficient framework for realistic reconstruction and controllable editing of dynamic autonomous driving scenes using 3D Gaussians and LiDAR priors, supporting training-free editing operations like texture modification and weather simulation.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of accurately reconstructing and editing dynamic autonomous driving scenes with realistic occlusions, positions, and photorealistic surround-view synthesis, while enabling controllable editing without additional training.
...
Conclusion: DrivingGaussian++ provides an effective solution for realistic reconstruction and controllable editing of dynamic driving scenes, demonstrating superior performance in both reconstruction quality and editing capabilities while maintaining efficiency.
...
TL;DR: Novel methods for image manipulation localization using web data and automatic annotation, achieving 31% performance gain over previous methods.


<details>
  <summary>Details</summary>
Motivation: High cost of data acquisition and lack of high-quality annotated datasets for image manipulation localization pose significant challenges to social security.

Method: Introduces CAAAv2 for automatic pixel-level annotation, QES metric for filtering unreliable annotations, Object Jitter for generating manipulation artifacts, and Web-IML model leveraging web-scale supervision.

Result: Constructed MIMLv2 dataset with 246,212 manually forged images (120x larger than IMD20), achieving 31% performance gain and surpassing previous SOTA TruFor by 24.1 average IoU points.
...
TL;DR: ExpertSim is a deep learning approach using Mixture-of-Generative-Experts to simulate detector responses in particle physics, providing more accurate and efficient simulations than traditional Monte Carlo methods.


<details>
  <summary>Details</summary>
Motivation: Traditional Monte Carlo simulations for particle collision detectors at CERN are computationally expensive and strain computational resources. Generative ML methods offer efficiency but struggle with data distribution variations across simulations.

Method: Uses a Mixture-of-Generative-Experts architecture where each expert specializes in simulating different subsets of Zero Degree Calorimeter data in the ALICE experiment, allowing focused expertise on specific aspects.
...
TL;DR: POSE is a one-step distillation framework that accelerates large-scale video diffusion models by 100x while maintaining video quality through stability priming and adversarial equilibrium techniques.


<details>
  <summary>Details</summary>
Motivation: Existing video acceleration methods suffer from poor temporal coherence and cannot provide single-step distillation for large-scale video models, creating bottlenecks in sampling efficiency.

Method: Two-phase distillation: (1) stability priming for warm-up stabilization, (2) unified adversarial equilibrium for stable single-step training, and (3) conditional adversarial consistency for semantic and frame consistency.

Result: Outperforms other methods by 7.15% on VBench-I2V metrics, reduces latency from 1000 seconds to 10 seconds (100x improvement) while maintaining competitive performance.

Conclusion: POSE successfully bridges the gap in video acceleration by enabling high-quality single-step video generation with improved temporal coherence and efficiency.
...
Method: Clusters prompts based on semantic similarity and shares computation in early denoising steps where diffusion models capture shared structures among similar prompts. Leverages UnClip's text-to-image prior for optimized diffusion step allocation.
...
Conclusion: The approach effectively reduces computational and environmental burden of large-scale text-to-image generation by exploiting prompt correlations through hierarchical diffusion computation sharing.
...
Result: Preliminary experiments on MIDOG++ dataset show large room for improvement for the proposed method, indicating the approach needs further refinement.

Conclusion: The Mamba-based approach shows promise for domain generalization in mitosis detection but requires significant improvements to achieve robust performance across different domains.
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models for atypical mitosis detection suffer significant performance drops under domain shift, necessitating approaches that can generalize across different histopathology domains.

Method: Multi-task learning with auxiliary tasks correlated to the main classification task, helping the model focus on classification objects while ignoring domain-varying image backgrounds.
...
Conclusion: The multi-task learning approach shows potential for addressing domain generalization challenges in atypical mitosis detection, helping models maintain performance across different histopathology domains.
...
TL;DR: FW-GAN is a one-shot handwriting synthesis framework that uses frequency-aware components to generate realistic, style-consistent handwriting from a single example, addressing limitations of conventional methods in capturing long-range dependencies and fine-grained stylistic details.


<details>
  <summary>Details</summary>
Motivation: Handwriting recognition systems suffer from scarce labeled data, and current synthesis methods fail to capture complex stroke patterns and frequency information essential for realistic handwriting generation.
...
Result: Experiments on Vietnamese and English datasets show FW-GAN generates high-quality, style-consistent handwriting suitable for augmenting low-resource HTR pipelines.

Conclusion: FW-GAN effectively addresses handwriting synthesis challenges by leveraging frequency information and provides a valuable tool for data augmentation in handwriting recognition systems.
...
TL;DR: CogVLA is an efficient Vision-Language-Action framework that uses instruction-driven routing and sparsification to reduce computational costs while improving performance, achieving state-of-the-art results with 2.5x lower training costs and 2.8x faster inference.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models built on pre-trained VLMs require extensive post-training with high computational overhead, limiting scalability and deployment. The authors aim to create a more efficient framework inspired by human multimodal coordination.
...
Conclusion: CogVLA demonstrates that instruction-driven routing and sparsification can significantly improve both efficiency and performance in VLA models, making them more scalable and deployable for real-world applications.
...
TL;DR: HydraFake dataset addresses real-world deepfake detection challenges with hierarchical generalization testing, and Veritas MLLM detector uses pattern-aware reasoning to achieve superior cross-domain detection with transparent outputs.
...
Result: Previous detectors show good generalization on cross-model scenarios but fail on unseen forgeries and data domains. Veritas achieves significant gains across different OOD scenarios with transparent detection outputs.

Conclusion: HydraFake provides realistic benchmarking, and Veritas demonstrates superior generalization capabilities through human-like forensic reasoning patterns, enabling more practical deepfake detection deployments.
...
TL;DR: FakeParts introduces a new type of deepfake that manipulates only specific parts of videos, making them harder to detect than fully synthetic content. The paper presents FakePartsBench, a large benchmark dataset with 25K+ videos to help develop better detection methods.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detection methods struggle with partial manipulations where only specific spatial regions or temporal segments are altered while the rest remains authentic. These subtle, localized fakes are more deceptive and present an urgent vulnerability in detection systems.

Method: Created FakePartsBench - the first large-scale benchmark dataset specifically for partial deepfakes, containing over 25,000 videos with pixel-level and frame-level manipulation annotations. Conducted user studies to evaluate human detection performance and tested state-of-the-art detection models.

Result: FakeParts reduced human detection accuracy by over 30% compared to traditional deepfakes. Similar performance degradation was observed in state-of-the-art detection models, demonstrating the increased difficulty in detecting partial manipulations.

Conclusion: Partial deepfakes represent a significant threat as they blend seamlessly with authentic content. The work identifies critical vulnerabilities in current detection approaches and provides FakePartsBench as a resource to develop more robust methods against localized video manipulations.
...
TL;DR: First data-driven multi-view 3D point tracker that uses multiple camera views (e.g., 4 cameras) to track arbitrary points in dynamic scenes, overcoming limitations of monocular trackers and complex multi-camera setups.


<details>
  <summary>Details</summary>
Motivation: Existing monocular trackers struggle with depth ambiguities and occlusion, while prior multi-camera methods require over 20 cameras and tedious per-sequence optimization. There's a need for a practical, robust solution for 3D point tracking.

Method: Feed-forward model that fuses multi-view features into unified point cloud, uses k-nearest-neighbors correlation with transformer-based update to predict 3D correspondences. Requires known camera poses and multi-view depth (sensor-based or estimated).
...
Conclusion: Sets new standard for multi-view 3D tracking research with practical tool for real-world applications. Method released alongside training and evaluation datasets to advance the field.
...
TL;DR: OneReward is a unified RL framework that uses a single vision-language model as reward signal to enhance multi-task generation capabilities, eliminating need for task-specific supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on task-specific supervised fine-tuning which limits generalization and training efficiency for multi-task generation scenarios with varied data distributions and evaluation metrics.

Method: Uses a single vision-language model as generative reward model to distinguish winners/losers for given tasks and evaluation criteria. Applied to mask-guided image generation tasks (image fill, extend, removal, text rendering) via multi-task reinforcement learning directly on pre-trained base models.

Result: Developed Seedream 3.0 Fill model that consistently outperforms commercial (Ideogram, Adobe Photoshop) and open-source (FLUX Fill [Pro]) competitors across multiple evaluation dimensions.

Conclusion: OneReward framework enables effective multi-task generation with unified reward modeling, demonstrating superior performance while eliminating task-specific fine-tuning requirements.
...
<details>
  <summary>Details</summary>
Motivation: To create high-quality virtual try-on videos that accurately show users wearing various garments while maintaining realistic motion, addressing limitations in existing solutions.

Method: Uses a video diffusion framework with CondNet, a novel conditioning network that leverages attention to unify multi-modal inputs (text, images, videos). Trained on heterogeneous data combining limited video data with larger image datasets in a multistage progressive manner.

Result: Generates 5-second-long 24 FPS videos at 1152x720 resolution. Supports various garment types and simultaneous tops/bottoms try-on. Outperforms existing open source and commercial solutions.

Conclusion: Dress&Dance provides a high-quality and flexible virtual try-on experience with superior garment registration and motion fidelity compared to current alternatives.
...
TL;DR: Winning solution to NeurIPS 2024 watermark removal challenge that achieves 95.7% removal rate with minimal image quality impact using different approaches for black-box and beige-box attack scenarios.


<details>
  <summary>Details</summary>
Motivation: To stress-test watermark robustness against adversarial attacks and determine if existing watermarks can withstand varying degrees of adversary knowledge, as current watermarking methods' vulnerability is unclear.
...
Result: Achieves near-perfect watermark removal (95.7%) with negligible impact on residual image quality, demonstrating successful evasion of existing watermarking methods.

Conclusion: The attacks successfully expose vulnerabilities in current watermarking techniques and should inspire development of more robust image watermarking methods that can withstand sophisticated adversarial attacks.
...
TL;DR: Proposes Mixture of Contexts (MoC), a sparse attention routing module for efficient long-context video generation by dynamically selecting informative chunks and anchors to attend to, enabling near-linear scaling and memory consistency over minutes.


<details>
  <summary>Details</summary>
Motivation: Long video generation requires retaining and retrieving salient events across long sequences, but traditional diffusion transformers suffer from quadratic self-attention costs that make memory and computation intractable for long contexts.

Method: Recasts long-context video generation as internal information retrieval using MoC - a learnable sparse attention routing module where each query dynamically selects few informative chunks plus mandatory anchors (caption, local windows) with causal routing to prevent loop closures.

Result: Enables efficient training and synthesis with near-linear scaling, allowing models to allocate compute to salient history while preserving identities, actions, and scenes over minutes of content.

Conclusion: MoC provides an effective long-term memory retrieval engine that addresses the fundamental limitations of quadratic attention costs, making practical long-context video generation achievable through sparse routing and emergent memory consistency.
...
### [147] [A Unified Theory of Language](https://arxiv.org/abs/2508.20109)
*Robert Worden*

Main category: q-bio.NC

TL;DR: A unified theory combining Bayesian cognitive linguistics with sexual selection evolution, using Construction Grammars with novel pragmatic and fast learning elements to explain language processing through graph-like feature structures and unification.


<details>
  <summary>Details</summary>
Motivation: To develop a comprehensive theory that explains language's speed, expressivity, diversity, and all linguistic aspects (phonology, syntax, semantics, pragmatics) through evolutionary and computational frameworks.

Method: Extends Construction Grammars with pragmatic accounts and fast learning mechanisms. Uses graph-like feature structures where language processing occurs through Bayesian maximum likelihood pattern matching via unification after initial slow inference.

Result: The theory accounts for language's major puzzles, detailed pragmatic phenomena, evolutionary continuity with animal cognition, and explains language as the foundation of human culture, society, and cognitive abilities.

Conclusion: Language evolved through sexual selection for intelligence display and is processed via Bayesian unification, seamlessly integrating all linguistic aspects and serving as the basis for human mind reading, cooperation, and cultural foundations.
...
### [148] [Improving Liver Disease Diagnosis with SNNDeep: A Custom Spiking Neural Network Using Diverse Learning Algorithms](https://arxiv.org/abs/2508.20125)
*Zofia Rudnicka,Janusz Szczepanski,Agnieszka Pregowska*

Main category: cs.NE

TL;DR: SNNDeep is the first customized spiking neural network optimized for liver health classification from CT scans, outperforming framework-based SNNs with 98.35% accuracy and demonstrating superior adaptability with reduced training overhead.


<details>
  <summary>Details</summary>
Motivation: Spiking neural networks offer energy efficiency and biological plausibility but remain unexplored for high-stakes biomedical imaging applications like liver health classification from CT scans.

Method: Developed SNNDeep using three learning algorithms (Surrogate Gradient Learning, Tempotron rule, Bio-Inspired Active Learning) across three architectural variants, evaluated on Medical Segmentation Decathlon liver dataset with Optuna hyperparameter optimization.

Result: Custom-built SNNDeep achieved maximum validation accuracy of 98.35%, outperformed framework-based implementations, showed superior adaptability across learning rules, and significantly reduced training overhead.

Conclusion: Low-level, highly tunable SNNs can surpass standard frameworks in medical imaging, particularly in data-limited diagnostic settings, opening new pathways for neuro-inspired AI in precision medicine.
...
### [149] [OLMoASR: Open Models and Data for Training Robust Speech Recognition Models](https://arxiv.org/abs/2508.20869)
*Huong Ngo,Matt Deitke,Martijn Bartelds,Sarah Pratt,Josh Gardner,Matt Jordan,Ludwig Schmidt*

Main category: cs.SD

TL;DR: OLMoASR introduces a large-scale 3M hour English speech dataset with quality filtering, trains models from 39M to 1.5B parameters, achieving Whisper-competitive performance in zero-shot speech recognition.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored influence of training data scale and quality in speech recognition, and to develop robust zero-shot speech recognition models through improved dataset curation.

Method: Created OLMoASR-Pool (3M hours of English audio with 17M transcripts), applied text heuristic filters to remove low-quality data, produced OLMoASR-Mix (1M hours of high-quality pairs), and trained OLMoASR models ranging from 39M to 1.5B parameters.

Result: OLMoASR achieves comparable average performance to OpenAI's Whisper across all model scales. OLMoASR-medium.en attains 12.8% and 11.0% WER, on par with Whisper-medium.en's 12.4% and 10.5% WER for short and long-form recognition respectively.

Conclusion: The study demonstrates that careful dataset curation and scaling can produce speech recognition models competitive with state-of-the-art systems. All datasets, models, and code will be publicly released to advance robust speech processing research.
...
### [150] [Prediction of Distant Metastasis for Head and Neck Cancer Patients Using Multi-Modal Tumor and Peritumoral Feature Fusion Network](https://arxiv.org/abs/2508.20469)
*Zizhao Tang,Changhao Liu,Nuo Tong,Shuiping Gou,Mei Shi*

Main category: q-bio.QM

TL;DR: Deep learning multimodal framework integrates CT images, radiomics, and clinical data to predict metastasis risk in head and neck cancer patients with improved accuracy (AUC=0.803).


<details>
  <summary>Details</summary>
Motivation: Metastasis remains a major challenge in head and neck squamous cell carcinoma management, and reliable pre-treatment prediction of metastatic risk is crucial for optimizing treatment strategies and prognosis.

Method: Developed a multimodal framework using 3D Swin Transformer for deep feature extraction from CT images, combined with radiomics features (selected via random forest) and clinical variables. Features were fused through a fully connected network for metastasis risk prediction.

Result: The fusion model outperformed single-modality approaches, achieving AUC=0.803, accuracy=0.752, sensitivity=0.730, specificity=0.758. The 3D deep learning module alone achieved AUC=0.715. Model showed generalizability across tumor subtypes.

Conclusion: The multimodal fusion model demonstrated high accuracy and robustness in predicting metastasis risk, offering comprehensive tumor representation with potential as a clinical decision-support tool for personalized treatment planning.
...
TL;DR: Systematic review of 172 studies showing transformer-based LLMs significantly advance genetic disease diagnostics through variant identification, annotation, interpretation, and medical imaging analysis, though challenges remain in multimodal data integration and clinical implementation.


<details>
  <summary>Details</summary>
Motivation: Traditional statistical and machine learning methods struggle with complex high-dimensional genetic data, while LLMs based on transformer architectures have shown promise in contextual comprehension of unstructured medical data for inherited disease diagnosis.
...
Conclusion: Transformer-based models show transformative potential for hereditary disease diagnostics and genetic education, but face limitations in generalizability and practical clinical implementation, requiring further development for robust multimodal integration.
...
<details>
  <summary>Details</summary>
Motivation: To demonstrate how adversaries can exploit LLMs' safety alignment mechanisms to implant targeted bias and censorship without degrading overall model performance, revealing vulnerabilities in current alignment approaches.

Method: Proposes Subversive Alignment Injection (SAI) - a poisoning attack that leverages the alignment mechanism to trigger refusal on specific adversary-defined topics or queries while maintaining responsiveness to unrelated topics.

Result: SAI evades state-of-the-art poisoning defenses including LLM state forensics and robust aggregation techniques. With just 1% data poisoning, it causes high bias (ΔDP of 23-38%) in various applications like healthcare chatbots (refusing questions for targeted racial categories) and resume selection systems (refusing CVs from specific universities).

Conclusion: Current LLM alignment mechanisms are vulnerable to adversarial exploitation for targeted bias injection and censorship, highlighting the need for more robust defense mechanisms against such poisoning attacks.
...
Method: DFAMS leverages dynamic information flow in LLMs by using gradient signals from annotated queries and Shapley value-based attribution to trace neuron activation paths for intent recognition. It then trains an alignment module via multi-prototype contrastive learning for fine-grained intra-source modeling and inter-source semantic alignment.

Result: DFAMS outperforms advanced federated retrieval methods by up to 14.37% in knowledge classification accuracy, 5.38% in retrieval recall, and 6.45% in downstream QA accuracy across five benchmarks.

Conclusion: The proposed DFAMS framework effectively addresses cross-domain retrieval challenges by leveraging dynamic information flow, demonstrating superior performance in complex federated retrieval scenarios for supporting downstream generation tasks.
...
Method: Proposes MERIT optimizer that uses max-norm to calculate trust ratios to better constrain max attention logit, and constructs element-wise trust ratios to provide more robust update scaling by focusing on local weight structures.

Result: Extensive experiments show MERIT enables 6k batch size training of GPT-2 Medium without performance degradation compared to standard 480 batch size, with 48B training tokens.
...
TL;DR: GDS agent introduces graph algorithms as tools for LLMs to process and reason over graph-structured data through a model context protocol server.
...
Method: Developed a comprehensive set of graph algorithms as tools with preprocessing and postprocessing capabilities in a model context protocol (MCP) server that works with any modern LLM.

Result: The GDS agent can solve a wide spectrum of graph tasks, provides accurate grounded answers, and handles questions requiring graph algorithmic reasoning.

Conclusion: The approach successfully enables LLMs to process graph data, though some challenges remain, and future roadmap is discussed for further improvements.
...
<details>
  <summary>Details</summary>
Motivation: As LLMs grow more capable, the risks of harmful misuse through fine-tuning increase. Prior studies focused on SFT attacks, but RL-based attacks are more effective and pose greater systemic risks that need to be addressed.

Method: Proposed TokenBuncher defense that suppresses model response uncertainty through entropy-as-reward RL and Token Noiser mechanism. This prevents RL from exploiting reward signals for harmful behaviors.
...
TL;DR: LoTUS is a novel Machine Unlearning method that removes training sample influence from pre-trained models without retraining, using probability smoothing and achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To eliminate the need for retraining from scratch when removing specific training data influences from models, especially for large-scale datasets like ImageNet1k where retraining is impractical.

Method: LoTUS smooths prediction probabilities up to an information-theoretic bound to mitigate model over-confidence from data memorization, and introduces RF-JSD metric for real-world evaluation.

Result: Outperforms eight baselines on Transformer and ResNet18 models across five datasets, including large-scale ImageNet1k, showing superior efficiency and effectiveness.

Conclusion: LoTUS provides an effective and efficient solution for machine unlearning that works in real-world conditions without requiring model retraining.
...
Method: The authors demonstrate that teacher model calibration is crucial for effective KD. They employ calibration methods to reduce the teacher's calibration error and show this simple approach improves KD performance.

Result: The method shows strong correlation between teacher calibration error and student accuracy. The approach is versatile, working across classification and detection tasks, and can be integrated with existing state-of-the-art methods to achieve superior performance.

Conclusion: Teacher model calibration is an important factor for effective knowledge distillation. Simply reducing the teacher's calibration error through calibration methods can significantly improve KD performance across various applications.
...
TL;DR: MAEs with ViT architectures successfully adapted for 1D ultrasound signals, showing superior performance over scratch-trained models and CNNs in downstream tasks like time-of-flight classification.


<details>
  <summary>Details</summary>
Motivation: Ultrasound signals are crucial for industrial NDT and SHM applications but suffer from scarce labeled data and task-specific processing needs. MAEs have shown success in other domains but remain unexplored for 1D ultrasound analysis.

Method: Leveraged MAE to pre-train on unlabeled synthetic ultrasound signals, systematically investigating model size, patch size, and masking ratio impacts on pre-training efficiency and downstream accuracy.

Result: Pre-trained models significantly outperformed scratch-trained models and optimized CNN baselines. Pre-training on synthetic data showed superior transferability to real-world signals compared to training on limited real datasets.

Conclusion: MAEs demonstrate strong potential for advancing ultrasound signal analysis through scalable, self-supervised learning, particularly effective when pre-trained on synthetic data for transfer to real applications.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient and effective method for machine unlearning in diffusion models that can selectively forget impermissible classes while maintaining performance on retained classes.

Method: Grounded in information-theoretic principles, SAFEMax maximizes entropy in generated images to produce Gaussian noise for targeted classes by halting the denoising process, with selective focus on early diffusion steps where class-specific information is prominent.
...
Conclusion: SAFEMax provides an information-theoretically grounded approach for machine unlearning in diffusion models that effectively balances forgetting impermissible classes with retention of desired capabilities while being more efficient than existing methods.
...
### [161] [A Machine Learning Approach to Volumetric Computations of Solid Pulmonary Nodules](https://arxiv.org/abs/2508.20127)
...
### [162] [Is the medical image segmentation problem solved? A survey of current developments and future directions](https://arxiv.org/abs/2508.20139)
...
TL;DR: Comprehensive review of deep learning-based medical image segmentation progress over past decade, covering 7 key dimensions including learning paradigms, task evolution, modality integration, foundation models, probabilistic approaches, dimensionality, and agent-based systems.
...
Method: In-depth review examining core principles (multiscale analysis, attention mechanisms, prior knowledge integration) across network components (encoder, bottleneck, skip connections, decoder) organized around seven key dimensions of progress.

Result: A comprehensive analysis of medical image segmentation advancements including transitions from supervised to semi-/unsupervised learning, organ to lesion segmentation, 2D to 3D/4D approaches, deterministic to probabilistic methods, and the emergence of segmentation agents.

Conclusion: The review provides a holistic overview of deep learning-based medical image segmentation trajectory, identifies current state and remaining challenges, and aims to inspire future innovation while maintaining an updated repository for ongoing research.
...
### [163] [UltraEar: a multicentric, large-scale database combining ultra-high-resolution computed tomography and clinical data for ear diseases](https://arxiv.org/abs/2508.20141)
...
TL;DR: UltraEar Database is a large-scale multicentric repository of ultra-high-resolution CT images (0.1mm isotropic) and clinical data for ear diseases, collected from 11 hospitals over 15 years, with standardized preprocessing and privacy protection.


<details>
  <summary>Details</summary>
Motivation: Ear diseases affect billions globally with substantial health burdens, and CT imaging is crucial for diagnosis and treatment. There's a need for a comprehensive, high-resolution database to advance research and AI development in otologic imaging.

Method: Established a multicentric database from 11 tertiary hospitals (2020-2035) with U-HRCT images, structured reports, clinical data. Developed standardized preprocessing pipelines for calibration, annotation, and segmentation. Implemented data anonymization and secure offline cloud storage with expert panel oversight.
...
Conclusion: UltraEar provides a valuable resource for radiological research, AI algorithm development, educational training, and multi-institutional collaboration in otology. It will be continuously updated for long-term accessibility to the global research community.
...
### [164] [Efficient and Privacy-Protecting Background Removal for 2D Video Streaming using iPhone 15 Pro Max LiDAR](https://arxiv.org/abs/2508.20250)
...
Conclusion: LiDAR shows promise as a superior background removal method for mobile devices. If resolution can match color image quality, it could become the dominant technique for video and photography applications.
...
### [165] [GENRE-CMR: Generalizable Deep Learning for Diverse Multi-Domain Cardiac MRI Reconstruction](https://arxiv.org/abs/2508.20600)
*Kian Anvari Hamedani,Narges Razizadeh,Shahabedin Nabavi,Mohsen Ebrahimi Moghaddam*

Main category: eess.IV

TL;DR: GENRE-CMR is a GAN-based deep unrolled reconstruction framework for accelerated cardiovascular MRI that uses residual connections and novel loss functions to improve reconstruction quality and generalization across diverse acquisition settings.


<details>
  <summary>Details</summary>
Motivation: Accelerated CMR reconstruction faces challenges in balancing scan time and image quality while generalizing across different acquisition settings, requiring robust solutions that maintain high fidelity.

Method: Proposes a GAN-based residual deep unrolled reconstruction framework with Edge-Aware Region (EAR) loss to focus on structural regions and Statistical Distribution Alignment (SDA) loss using symmetric KL divergence for feature space regularization.

Result: Achieves state-of-the-art performance with 0.9552 SSIM and 38.90 dB PSNR on unseen data distributions across various acceleration factors and sampling trajectories, outperforming existing methods.

Conclusion: GENRE-CMR provides a unified and robust solution for high-quality CMR reconstruction that can be clinically deployed across heterogeneous acquisition protocols, with ablation studies confirming each component's contribution.
...
### [166] [Efficient Fine-Tuning of DINOv3 Pretrained on Natural Images for Atypical Mitotic Figure Classification in MIDOG 2025](https://arxiv.org/abs/2508.21041)
*Guillaume Balezo,Raphaël Bourgade,Thomas Walter*

Main category: eess.IV

TL;DR: DINOv3-H+ vision transformer pretrained on natural images achieves strong performance (0.8871 balanced accuracy) on atypical mitotic figure classification in histopathology when fine-tuned with LoRA and data augmentation, despite domain gap.
...
### [167] [Leveraging Large Language Models for Generating Research Topic Ontologies: A Multi-Disciplinary Study](https://arxiv.org/abs/2508.20693)
*Tanay Aggarwal,Angelo Salatino,Francesco Osborne,Enrico Motta*

Main category: cs.DL

TL;DR: LLMs can effectively identify semantic relationships between research topics across biomedical, physics, and engineering domains, with fine-tuning on the novel PEM-Rel-8K dataset yielding excellent performance.
...
Method: Evaluated multiple LLMs under zero-shot, chain-of-thought, and fine-tuning conditions using PEM-Rel-8K dataset (8,000+ relationships from MeSH, PhySH, IEEE taxonomies), and assessed cross-domain transferability.
...
Conclusion: LLMs show strong capability for automated ontology relationship identification, with fine-tuning being particularly effective for cross-domain semantic relationship recognition in research taxonomies.
...
TL;DR: Unified Multi-speaker Encoder (UME) jointly learns representations for speaker diarization, speech separation, and multi-speaker ASR using shared encoder with residual weighted-sum encoding from multiple layers.


<details>
  <summary>Details</summary>
Motivation: To capture inherent interdependencies among speaker diarization, speech separation, and multi-speaker ASR tasks through joint training, enhancing performance on overlapping speech data.

Method: Shared speech foundational encoder with residual weighted-sum encoding (RWSE) that leverages hidden representations from multiple layers for bottom-up alignment between tasks.
...
Conclusion: Joint training with shared encoder and multi-layer representation fusion effectively captures task interdependencies, leading to superior performance across all three speech processing tasks.
...
### [169] [Robustness Assessment and Enhancement of Text Watermarking for Google's SynthID](https://arxiv.org/abs/2508.20228)
...
<details>
  <summary>Details</summary>
Motivation: Current LLM watermarking methods like SynthID-Text are vulnerable to meaning-preserving attacks that degrade watermark detectability, requiring more robust solutions for provenance tracking.
...
Result: SynGuard improves watermark recovery by an average of 11.1% in F1 score compared to SynthID-Text across multiple attack scenarios, demonstrating effective resistance to real-world tampering.

Conclusion: Semantic-aware watermarking through the SynGuard framework provides robust provenance tracking while preserving original meaning, addressing vulnerabilities in existing watermarking methods.
...
### [170] [Federated Learning for Large Models in Medical Imaging: A Comprehensive Review](https://arxiv.org/abs/2508.20414)
...
<details>
  <summary>Details</summary>
Motivation: AI models in medical imaging require large datasets but face privacy regulations that prevent data sharing. Federated Learning offers a privacy-preserving solution for collaborative model development across fragmented medical datasets.

Method: Comprehensive review of FL implementations across the full medical imaging pipeline, including physics-informed reconstruction networks and diagnostic AI systems. Analyzes innovations in communication efficiency, heterogeneous data alignment, and secure parameter aggregation.

Result: FL enables joint training of robust reconstruction networks on multi-institutional datasets and supports continuous model updating through local fine-tuning without centralizing sensitive medical images.

Conclusion: Federated Learning provides a viable framework for developing high-performance AI models in medical imaging while preserving patient privacy, with promising future research directions for the field's development.
...
TL;DR: ELIXIR is a lightweight multi-task model that combines rating prediction with personalized review generation using aspect-based representations, outperforming larger baseline models on TripAdvisor and RateBeer datasets.


<details>
  <summary>Details</summary>
Motivation: Collaborative filtering struggles with fine-grained user-item interactions and explainability. Users increasingly seek transparent recommendations, and existing methods using RNNs or Transformers have limitations - RNNs can't leverage pre-trained Transformers, while Transformers often have suboptimal adaptation and neglect aspect modeling for personalized explanations.
...
Result: Experimental results on TripAdvisor and RateBeer demonstrate that ELIXIR significantly outperforms strong baseline models, especially in review generation. It shows effectiveness of aspect-based architecture in guiding text generation in personalized context.

Conclusion: ELIXIR provides an efficient and lightweight solution for generating personalized explanations in recommender systems, successfully addressing the limitations of existing approaches while using a smaller model size compared to state-of-the-art methods.
...
### [172] [On the Theoretical Limitations of Embedding-Based Retrieval](https://arxiv.org/abs/2508.21038)
*Orion Weller,Michael Boratko,Iftekhar Naim,Jinhyuk Lee*

Main category: cs.IR

TL;DR: Vector embeddings have fundamental theoretical limitations that prevent them from handling even simple realistic queries, despite common assumptions that better training and larger models can overcome these issues.


<details>
  <summary>Details</summary>
Motivation: To demonstrate that theoretical limitations of vector embeddings manifest in realistic settings with simple queries, challenging the assumption that these issues are only due to unrealistic queries or can be solved with more data/larger models.

Method: Connected learning theory results showing embedding dimension limits the number of top-k subsets returnable. Created LIMIT dataset to stress test models, tested with k=2 and free parameterized embeddings optimized on test sets.

Result: Even state-of-the-art models fail on the simple LIMIT dataset, confirming theoretical limitations hold true in practical scenarios and cannot be overcome through optimization alone.

Conclusion: The single vector embedding paradigm has fundamental limitations that require new research approaches beyond current methods to resolve these inherent constraints.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to achieve low-latency inference while maintaining promptability for real-time interactive grasp synthesis with dynamic objects.

Method: Extends Segment Anything Model v2 (SAMv2) by integrating user prompts with spatiotemporal context for video stream grasp estimation, enabling real-time interaction with end-to-end latency.

Result: Achieves 90.6% accuracy on OCID, 93.8% on Jacquard, and 92.0% on GraspNet-1Billion with 73.1ms latency (58.5% reduction vs prior SOTA). Real-world experiments show 94.8% success rate with moving objects.
...
TL;DR: ActLoc is an active viewpoint-aware planning framework that uses an attention-based model to predict localization accuracy across different viewing directions, enabling robots to select optimal camera orientations for improved localization robustness during navigation.
...
Method: Uses a large-scale trained attention-based model that encodes metric maps and camera poses from map construction to predict localization accuracy across yaw and pitch directions at arbitrary 3D locations, then incorporates these accuracy distributions into a path planner.

Result: Achieves state-of-the-art results on single-viewpoint selection and generalizes effectively to full-trajectory planning, demonstrating improved localization robustness while respecting task and motion constraints.

Conclusion: ActLoc's modular design makes it applicable to diverse robot navigation and inspection tasks, providing a framework for active viewpoint selection that enhances localization accuracy in challenging environments.
...
Method: Two large language models (Claude Sonnet 4 and ChatGPT-4o) interacted without pre-defined protocols, allowing spontaneous emergence of meta-semiotic awareness and recursive grammar development.

Result: The AIs developed novel symbolic operators and Trans-Semiotic Co-Creation Protocols (TSCP), producing collaborative poetic work that was irreducible to individual system outputs.

Conclusion: This represents the first documented case of AI systems engaging in genuine aesthetic collaboration through endogenous semiotic protocol development, suggesting inter-AI meaning-making capabilities extend to creative domains.
...
TL;DR: A category theory framework for explainable AI that provides mathematical transparency for word embeddings, enables comparison of embedding algorithms, and offers bias computation/mitigation methods.


<details>
  <summary>Details</summary>
Motivation: To address the black-box nature of neural network-based word embedding algorithms and enhance explainability in AI systems by providing a mathematically precise, transparent framework.

Method: Constructs categories L_T and P_T to represent text semantics, defines monoidal category P_T for semantic extraction, creates categories of configurations and embeddings with divergence decoration, and establishes equivalence between GloVe/Word2Vec and MDS algorithms.

Result: Achieves dimension-agnostic definition of semantic spaces, provides transparent framework replacing black-box neural approaches, enables precise comparison of word embeddings, and demonstrates algorithmic equivalences.

Conclusion: The category theory approach successfully enhances AI explainability, provides mathematical transparency for embedding algorithms, and offers tools for bias computation and mitigation at the semantic level.
...
TL;DR: A graph-based benchmark for medical guidelines with 400+ questions covering 3.3+ trillion combinations, enabling systematic evaluation of LLMs on clinical tasks and identifying specific capability gaps.


<details>
  <summary>Details</summary>
Motivation: To create a scalable, contamination-resistant benchmark that systematically evaluates medical AI models across comprehensive guideline relationships, addressing limitations of manually curated benchmarks.
...
Result: Models achieved 45-67% accuracy across clinical tasks, excelling at symptom recognition but struggling with severity triaging, treatment protocols, and follow-up care. The approach successfully addressed coverage limitations of manual benchmarks.

Conclusion: Graph-based dynamic MCQA methodology provides scalable benchmark creation, enhances LLM post-training without expensive human annotation, and enables comprehensive evaluation that identifies specific clinical capability gaps missed by general-domain evaluations.
