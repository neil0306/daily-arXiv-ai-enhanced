...
Motivation: To analyze how social bias evaluation and mitigation approaches are being extended from English to multilingual contexts, identifying methodological limitations and opportunities for improvement.

Method: Systematic review of emerging research on multilingual bias evaluation and mitigation, examining linguistic diversity, cultural awareness, evaluation metrics, and mitigation techniques across studies.

Result: Found that current approaches show preference for certain languages, scarcity of multilingual mitigation experiments, and inadequate cross-cultural adaptation of bias benchmarks.

Conclusion: Identifies critical gaps in multilingual bias research methodology and provides directions for future work to improve inclusivity, cross-cultural appropriateness, and alignment with state-of-the-art NLP advancements.
...
<details>
  <summary>Details</summary>
Motivation: To reduce the cost and inconsistency of manual test development by using language models for automatic generation of multiple choice questions for morphological assessment.
...
Method: Encapsulate SystemC TLM components as FMI 3.0 Co Simulation FMUs with lightweight open-source toolchain, addressing time synchronization and data exchange
...
Method: Distillation-Guided Policy Optimization (DGPO) uses cold-start initialization from teacher demonstrations and continuous teacher guidance during policy optimization to improve training stability and performance.

Result: DGPO enables compact models to achieve sophisticated agentic search behaviors, even outperforming larger teacher models in some cases, and makes agentic RAG feasible in computing resource-constrained environments.

Conclusion: DGPO successfully addresses the challenges of training compact language models for agentic RAG by leveraging teacher guidance throughout the optimization process, enabling effective deployment in resource-limited settings.
...
Method: Automated generation of guideline-violating questions based on government guidelines, with jailbreak diagnostics (GUARD-JD) to provoke unethical responses and test safety mechanisms.

Result: Empirically validated on 7 LLMs including Vicuna, Llama, GPT, and Claude models, showing effectiveness in testing compliance across three government guidelines and jailbreak scenarios.

Conclusion: GUARD provides a practical framework for operationalizing ethics guidelines into compliance testing, with jailbreak diagnostics that can also transfer to vision-language models for broader AI safety applications.
...
Method: Three key components: 1) Synopsis extraction through strategic text chunking for efficient summarization, 2) Directed acyclic graph (DAG) construction to resolve redundancy and ensure logical consistency, 3) Monte Carlo Tree Search (MCTS) integration for navigating complex reasoning paths.
...
<details>
  <summary>Details</summary>
Motivation: Current Long CoT reasoning development relies on costly human-curated datasets, leaving scalable alternatives unexplored. NP-hard graph problems inherently require deep reasoning and extensive exploration similar to Long CoT capabilities.

Method: Two-stage post-training framework: 1) Long CoT Supervised Fine-Tuning on rejection-sampled NP-hard graph instances to enhance reasoning depth, 2) Reinforcement Learning with fine-grained reward design to sharpen reasoning efficiency.
...
Conclusion: NP-hard graph problems serve as an effective and scalable resource for advancing Long CoT reasoning in LLMs, opening new frontiers for LLM post-training with synthetic data.
...
Method: Developed Context-Aware Personality Evaluation (CAPE) framework incorporating prior conversational interactions, introduced novel metrics to quantify response consistency, and conducted experiments on 7 LLMs including GPT-3.5-Turbo, GPT-4-Turbo, Gemini-1.5-Flash, and Llama-8B.

Result: Conversational history enhances response consistency via in-context learning but induces personality shifts, with GPT models showing extreme deviations. GPT models are robust to question ordering while Gemini-1.5-Flash and Llama-8B are sensitive. GPT responses come from intrinsic traits and context, while others heavily depend on context.
...
<details>
  <summary>Details</summary>
Motivation: To understand how reasoning step utility contributes to final answer correctness in LLMs, as generating more context doesn't ensure increased confidence due to autoregressive generation's stochastic nature.

Method: Oracle study on MATH dataset using Qwen2.5-32B and GPT-4o to generate reasoning chains, then using Qwen3-8B to quantify utility by measuring conditional entropy (expected negative log-likelihood) on answer span at each reasoning step.
...
Conclusion: These findings provide foundation for designing efficient reasoning pipelines that can detect and avoid unproductive reasoning early, rather than assuming longer reasoning yields better outcomes.
...
<details>
  <summary>Details</summary>
Motivation: There is no public benchmark to rigorously verify claims about AI text-to-app tools' ability to produce high-quality applications and websites quickly.

Method: Expert pairwise comparison of 300 sites generated from 30 prompts across 10 tools, using a TrueSkill-derived model with calibrated confidence intervals for ranking.

Result: UI-Bench provides a reproducible standard for evaluating AI-driven web design tools and includes a complete prompt set, open-source evaluation framework, and public leaderboard.

Conclusion: UI-Bench establishes the first comprehensive benchmark for AI text-to-app tools, enabling standardized evaluation and advancement in AI-driven web design.
...
TL;DR: DentalBench is the first comprehensive bilingual benchmark for evaluating LLMs in dentistry, featuring 36,597 questions across 16 dental subfields and a 337M-token corpus for domain adaptation, revealing significant performance gaps and demonstrating substantial improvements through specialized training.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs show strong performance on general medical benchmarks but lack evaluation in specialized fields like dentistry, which requires deeper domain-specific knowledge. There is a need for targeted evaluation resources to assess and advance LLM capabilities in dental medicine.

Method: Created DentalBench with two components: DentalQA (bilingual question-answering benchmark with 36,597 questions across 4 tasks and 16 dental subfields) and DentalCorpus (337.35M-token curated corpus for domain adaptation supporting SFT and RAG). Evaluated 14 LLMs including proprietary, open-source, and medical-specific models, with additional experiments using Qwen-2.5-3B to test domain adaptation effects.

Result: Significant performance gaps were revealed across task types and languages. Domain adaptation substantially improved model performance, particularly on knowledge-intensive and terminology-focused tasks. The benchmark demonstrated the importance of specialized evaluation for developing effective healthcare LLMs.

Conclusion: Domain-specific benchmarks like DentalBench are crucial for developing trustworthy and effective LLMs tailored to healthcare applications. Specialized dental knowledge requires targeted evaluation and adaptation, with domain-specific training showing substantial performance improvements in dental medicine tasks.
...
<details>
  <summary>Details</summary>
Motivation: To improve retrieval-augmented generation systems by addressing query context enrichment through structured knowledge graph representations rather than just corpus-level context loss.

Method: Proposes KG-CQR framework with subgraph extraction, completion, and contextual generation modules that operates as model-agnostic pipeline for query enrichment using corpus-centric knowledge graphs.
...
Conclusion: KG-CQR effectively enhances retrieval effectiveness in RAG systems through knowledge graph-based query enrichment and works across various LLM sizes without additional training.
...
<details>
  <summary>Details</summary>
Motivation: The lack of specialized evaluation tools for LLMs in civil aviation maintenance, where stringent standards and complex reasoning are required, creates a gap in assessing domain-specific capabilities.

Method: Developed an industrial-grade benchmark specifically designed for civil aviation maintenance to measure LLM capabilities, identify knowledge gaps, and evaluate existing vector embedding models and LLMs using RAG systems.
...
Conclusion: This work addresses a significant gap in LLM evaluation by providing a domain-specific benchmark that enables targeted improvements and progress toward intelligent solutions in civil aviation maintenance.
...
Conclusion: The CBR approach with TF-IDF and Cosine Similarity is effective for practical work title search and retrieval
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack evaluation of LLMs' abilities in realistic multi-step tool usage, cross-tool coordination, parameter control, and planning across domains with authentic workflows.

Method: Built on Model Context Protocol (MCP) with 28 live servers providing 250 complementary tools across finance, travel, scientific computing, and academic search domains. Tests include tool retrieval from fuzzy instructions, multi-hop execution planning, and cross-domain workflow orchestration.

Result: Experiments on 20 advanced LLMs revealed persistent challenges in handling the benchmark's complex requirements, showing current models struggle with realistic multi-step tool coordination tasks.

Conclusion: MCP-Bench provides a more comprehensive evaluation framework for LLM tool-use capabilities, exposing limitations in current models' ability to handle complex, multi-step, cross-domain tool coordination tasks that mirror real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches focus mainly on structured EHRs and ignore valuable clinical insights in free-text notes, while also not fully leveraging textual information within structured data. There's a need to better integrate multimodal EHR data for improved prediction accuracy in critical care.

Method: Developed a deep learning framework using natural language processing techniques that integrates both structured EHR data and free-text clinical notes. Used two real-world EHR datasets, performed ablation studies on medical prompts, free-texts, and pre-trained sentence encoder, and tested robustness against structured data corruption.

Result: The model improved performance by 1.6%/0.8% on BACC/AUROC for mortality prediction, 0.5%/2.2% on RMSE/MAE for LOS prediction, and 10.9%/11.0% on RMSE/MAE for surgical duration estimation compared to best existing methods. Showed strong resilience to data corruption, especially at high corruption levels.

Conclusion: The framework is an effective and accurate deep learning approach for predicting mortality and resource utilization in critical care. Successfully demonstrates the value of prompt learning with transformer encoder for multimodal EHR analysis and shows strong robustness against data corruption.
...
<details>
  <summary>Details</summary>
Motivation: Conspiracy theories erode public trust and resist debunking, with AI-generated misinformation becoming more sophisticated. Understanding rhetorical patterns is crucial for developing interventions like prebunking.

Method: Introduce ConspirED dataset with multi-sentence excerpts from online conspiracy articles annotated using CONSPIR cognitive framework. Develop computational models to identify conspiratorial traits and evaluate LLM robustness.

Result: Both computational models and LLMs are misaligned by conspiratorial content, producing output that mirrors input reasoning patterns even when successfully deflecting fact-checked misinformation.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate the suitability of widely-used FLORES+ multilingual MT benchmark and uncover potential shortcomings in its design for truly multilingual evaluation.

Method: Human assessment of translations in 4 languages (Asante Twi, Japanese, Jinghpaw, South Azerbaijani), quality analysis against claimed standards, and testing simple heuristics like named entity copying on BLEU scores.

Result: Many translations fall below 90% quality standard, source sentences are domain-specific and culturally biased, simple heuristics yield non-trivial BLEU scores, and MT models trained on naturalistic data perform poorly on FLORES+ but better on domain-relevant sets.
...
TL;DR: SciTopic is a novel topic discovery method that leverages large language models to enhance scientific topic identification by capturing comprehensive semantic understanding and optimizing text representation through LLM-guided contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing topic discovery methods rely on word embeddings that struggle with complex, high-dimensional text relationships and lack comprehensive understanding of scientific publications, limiting their effectiveness in identifying emerging research trends.

Method: The method involves: 1) building a textual encoder for scientific publications (metadata, title, abstract), 2) creating a space optimization module with entropy-based sampling and LLM-guided triplet tasks, 3) fine-tuning the encoder using contrastive loss optimization based on LLM guidance to better discriminate different topics.

Result: Extensive experiments on three real-world scientific publication datasets show that SciTopic outperforms state-of-the-art scientific topic discovery methods, providing deeper and faster insights for researchers.

Conclusion: LLM-enhanced topic discovery significantly improves scientific topic identification by capturing thematic relevance and contextual intricacies, demonstrating superior performance over traditional embedding-based approaches.
...
Conclusion: BioASQ 2025 successfully continued its mission of advancing biomedical semantic technologies through diverse tasks and strong participation, demonstrating ongoing progress in the state-of-the-art.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated learning approaches primarily focus on label diversity but neglect language domain diversity, which is crucial for NLP applications in real environments with non-IID data distributions across clients.
...
Conclusion: The work provides a comprehensive solution for multi-domain non-IID challenges in federated learning, with practical benchmarking and an effective adaptive distillation framework that improves performance in real-world NLP applications.
...
Conclusion: KCS effectively expands diversity of multi-hop questions by sampling varied knowledge compositions, balancing accuracy and diversity through innovative probabilistic modeling and decoding strategies.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation benchmarks for Graph-Language Models are insufficient because they don't require true multimodal reasoning - strong performance can be achieved using either graph structure or language information alone.

Method: Introduced CLEGR benchmark with synthetic graph generation pipeline and questions requiring joint structure-text reasoning. Evaluated representative GLM architectures against soft-prompted LLM baselines.

Result: Soft-prompted LLM baselines performed on par with full GNN-backbone GLMs. GLMs showed significant performance degradation in structural reasoning tasks, questioning the architectural necessity of incorporating graph structure.

Conclusion: Current GLMs have limited graph reasoning capabilities. The findings highlight the need for better benchmarks and architectural improvements to achieve true multimodal reasoning combining graph structure and language.
...
Result: Tested on open-source and self-constructed datasets, the method shows significant improvement in entity accuracy, especially in scenarios with word form differences.

Conclusion: The proposed NEC method effectively addresses limitations of phonetic-based approaches by leveraging speech sound features and generative annotation, demonstrating substantial accuracy improvements for named entity correction in ASR systems.
...
Method: Developed HArch model that uses hierarchical dependencies between discourse senses to predict probability distributions across three sense levels. Compared pre-trained encoder backbones (RoBERTa and XLM-RoBERTa) and evaluated against LLMs using few-shot prompting.

Result: RoBERTa-HArch achieved best performance in English, XLM-RoBERTa-HArch performed best in multi-lingual setting. Fine-tuned models consistently outperformed GPT-4o and Llama-4-Maverick. Achieved SOTA results on DiscoGeM 1.0 corpus.

Conclusion: Task-specific fine-tuning provides significant advantages over prompting LLMs for implicit discourse relation recognition. The hierarchical approach is effective and validated through state-of-the-art performance on benchmark datasets.
...
Method: Proposed two tailored solutions: 1) stepwise verification method for steganography, and 2) post-hoc rollback method for watermarking to eliminate tokenization inconsistency.

Result: Experiments show: steganography methods improve fluency, imperceptibility, and anti-steganalysis capacity; watermarking methods enhance detectability and robustness against attacks compared to traditional approaches.

Conclusion: Directly addressing tokenization inconsistency with tailored solutions significantly improves performance in both steganography and watermarking applications using large language models.
...
<details>
  <summary>Details</summary>
Motivation: To develop a math reasoning model that goes beyond current chain-of-thought approaches by enabling advanced cognitive behaviors and autonomous problem-solving capabilities through efficient agentic reinforcement learning.

Method: Three key innovations: (1) efficient RL infrastructure with reliable Python code environment for high-throughput execution, (2) GRPO-RoC algorithm with Resample-on-Correct rollout strategy to handle coding environment noise, (3) multi-stage training recipe starting with non-reasoning SFT and progressing through RL stages.

Result: Achieved state-of-the-art performance with 80.6% on AIME24 and 69.8% on AIME25, surpassing DeepSeek-R1 (671B) with shorter responses, using only 510 RL steps within one week on 64 MI300X GPUs.

Conclusion: rStar2-Agent demonstrates that agentic RL can effectively scale to create advanced reasoning models with strong generalization to mathematics, alignment, scientific reasoning, and tool-use tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing local DP text privatization methods require high ε values for reasonable utility, making privacy protection challenging. The paper addresses this limitation by developing a more effective approach.

Method: DP-ST leverages semantic triples for neighborhood-aware private document generation under local DP. It uses a divide-and-conquer paradigm with LLM post-processing to maintain coherence.

Result: The method enables coherent text generation even at lower ε values while maintaining privacy guarantees, demonstrating improved privacy-utility balance compared to previous approaches.

Conclusion: Semantic triple-based privatization with neighborhood-aware generation and LLM post-processing is effective for achieving balanced privacy-utility outputs at reasonable ε levels, highlighting the importance of coherence in DP text transformation.
...
<details>
  <summary>Details</summary>
Motivation: Implicit hate speech is challenging to detect due to its subtle, indirect nature without explicit derogatory words, requiring advanced detection methods.

Method: Solely fine-tuning recent general-purpose embedding models based on large language models (Stella, Jasper, NV-Embed, E5) for implicit hate speech detection.
...
Conclusion: Fine-tuning general-purpose LLM embedding models is an effective approach for implicit hate speech detection, outperforming traditional task-specific pipelines that rely on external knowledge.
...
Result: Achieves good balance between text diversity and coherence, shows substantial improvements in generation speed, and performs well in human and LLM evaluations across text quality dimensions.

Conclusion: GUARD effectively addresses the coherence-diversity trade-off with theoretical guarantees and practical efficiency improvements, making it a valuable decoding method for open-ended text generation.
...
Result: Real sessions showed greater emotional variability, more emotion-laden language, and more authentic patterns of reactivity and regulation. Emotional arc similarity between real and synthetic speakers was low, especially for clients.

Conclusion: Current LLM-generated therapy data has limitations in emotional fidelity, highlighting the need for more authentic emotional representation in mental health applications. Introduced RealCBT dataset to support future research.
...
<details>
  <summary>Details</summary>
Motivation: Existing safety mechanisms in LLMs can be bypassed by removing specific representational directions, so the authors propose amplifying rather than removing safety alignment through targeted weight steering.
...
Result: ROSI consistently increases safety refusal rates (evaluated by Llama Guard 3) while preserving utility on MMLU, HellaSwag, and Arc benchmarks. It can also re-align uncensored models using their latent safety directions.

Conclusion: Targeted, interpretable weight steering is a cheap and effective mechanism to improve LLM safety, complementing resource-intensive fine-tuning approaches as an effective last-mile safety procedure.
...
<details>
  <summary>Details</summary>
Motivation: Rising youth mental health issues require automated detection of cognitive distortions (irrational thought patterns) for early intervention, but prior work only focused on English clinical data.

Method: Cross-lingual and cross-register generalization analysis using forum posts from Dutch adolescents, evaluating how language and writing style changes affect model performance.
...
Conclusion: The global distance-aware modeling approach effectively enhances LLM performance for temporal relation extraction by better capturing both long-distance dependencies and short-distance proximity features.
...
<details>
  <summary>Details</summary>
Motivation: Real-world applications require RAG systems to integrate information from multiple sources where no single source is sufficient, but current evaluations focus on single-source or short-form factoid settings.
...
Method: Conducted evaluation of post-training quantization (PTQ) on machine translation using five LLMs (1.7B to 70B parameters) across 55 languages, comparing four quantization techniques: AWQ, BitsAndBytes, GGUF, and AutoRound.

Result: 4-bit quantization preserves translation quality for high-resource languages and large models, but significant degradation occurs for low-resource and typologically diverse languages, particularly at 2-bit. GGUF variants provide most consistent performance even at 2-bit precision.

Conclusion: Quantization algorithm choice and model size jointly determine robustness. Language-matched calibration offers benefits primarily in low-bit scenarios, providing actionable insights for deploying multilingual LLMs under quantization constraints, especially in low-resource settings.
...
<details>
  <summary>Details</summary>
Motivation: Evaluating speech-to-speech large language models remains a fundamental challenge, as existing cascaded approaches disregard acoustic features and there's a scarcity of speech preference data.

Method: Proposes SageLM with rationale-based supervision for explainability, introduces SpeechFeedback synthetic preference dataset, and uses two-stage training paradigm to address data scarcity. Jointly assesses both semantic and acoustic dimensions.
...
Conclusion: SageLM provides a comprehensive and explainable framework for speech-to-speech LLM evaluation that significantly outperforms existing approaches by jointly considering both semantic and acoustic dimensions.
...
Conclusion: IRMA demonstrates superior reliability and consistency compared to other methods in dynamic tool-use environments.
...
Method: Two-stage approach: 1) Fine-tune BERT-based retriever with structure-aware supervision, 2) Enhance retriever with plug-in module that amplifies syntactically meaningful information in hidden representations.
...
<details>
  <summary>Details</summary>
Motivation: Existing works on proactive dialogue focus on domain-specific scenarios, leading to fragmented evaluations and limiting comprehensive exploration of models' proactive conversation abilities.
...
Conclusion: The framework enables comprehensive evaluation of proactive dialogue capabilities, reveals performance differences among models, and investigates how reasoning capabilities influence proactive behaviors for future model development.
...
<details>
  <summary>Details</summary>
Motivation: LLMs remain vulnerable to sophisticated backdoor attacks, and existing defenses lack comprehensiveness against advanced scenarios like model-editing-based, multi-trigger, and triggerless attacks.
...
Result: Outperforms 8 state-of-the-art defense baselines against 8 backdoor attacks across 5 LLMs, reducing attack success rate by up to 98% while maintaining utility, cost-efficiency, and robustness against adaptive attacks.
...
Method: EASI-RAG uses method engineering principles with defined roles, activities, and techniques. Validated through real-world case study where a RAG tool was built to answer operator queries using operational procedure data.

Result: System deployed in under a month by inexperienced team, achieved high user adoption, delivered accurate answers, and improved data reliability. Iterative improvements based on user feedback.

Conclusion: EASI-RAG enables successful RAG deployment in industrial SMEs, demonstrating potential for broader adoption. Future work needs generalization across use cases and integration with fine-tuned models.
...
Method: Uses capsule networks with dynamic routing for sentential relation extraction, evaluating on multiple datasets including Tacred, Tacredrev, Retacred, Conll04, and Wikidata.
...
Conclusion: Noise in distantly supervised datasets and re-representation challenges are key factors affecting sentential relation extraction performance. The proposed capsule approach shows superior re-representation abilities.
...
<details>
  <summary>Details</summary>
Motivation: Tax filing requires complex reasoning with overlapping rules and numerical calculations, where errors incur costly penalties. Current LLMs alone are poorly suited due to accuracy and auditability requirements.

Method: Integrates large language models with symbolic solvers, combining upfront translation of plain-text rules into formal logic programs with intelligently retrieved exemplars for formal case representations.

Result: The system dramatically improves performance on the SARA dataset and reduces tax filing costs to well below real-world averages ($270 and 13 hours).

Conclusion: Neuro-symbolic architectures show promise for increasing equitable access to reliable tax assistance by delivering high accuracy, auditability, and economic feasibility.
...
Conclusion: The approach provides a simpler alternative to complex synthetic data pipelines for alignment training, successfully mitigating hallucinations in MLLMs using publicly available metrics and optimization techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing image forgery localization methods struggle to keep pace with advanced manipulation technologies like Stable Diffusion and rely heavily on costly annotated data, creating a need for more efficient and accurate forensic solutions.

Method: Leverages Stable Diffusion 3's multimodal architecture by treating image forgery residuals (high-frequency signals from highpass filters) as an explicit modality, fusing them into the latent space while preserving SD3's original features and semantic information.

Result: Achieves up to 12% performance improvement on standard benchmarking datasets compared to state-of-the-art methods, with strong generalization to unseen real-world document forgery and natural scene forging images.

Conclusion: The integration of SD3's multimodal capabilities provides an effective framework for image forgery localization that outperforms existing methods and demonstrates excellent generalization to various real-world forgery scenarios without extensive training data.
...
<details>
  <summary>Details</summary>
Motivation: AI models show promise in skin disease diagnosis but lack interpretability needed for clinical use. MLLMs offer natural language reasoning while quantitative attributes provide measurable grounding.

Method: Fine-tune MLLMs to predict quantitative lesion attributes (e.g., lesion area) from images and evaluate grounding through attribute-specific content-based image retrieval on SLICE-3D dataset

Result: Evidence shows MLLM embedding spaces can be effectively grounded in quantitative attributes through fine-tuning

Conclusion: Combining MLLMs with quantitative attributes provides a promising approach for improving interpretability of AI skin cancer diagnosis systems
...
Method: Uses ViT encoder with lightweight convolutional decoder and linear classifier; reconstruction branch maps augmented signals back to originals to capture fine-grained I/Q structure; combines supervised, self-supervised, and reconstruction objectives.
...
<details>
  <summary>Details</summary>
Motivation: Existing audio-driven human animation methods suffer from error accumulation causing identity drift, color shifts, scene instability, and poorly modeled hand movements that don't align well with audio.
...
Conclusion: InfinityHuman successfully addresses key challenges in audio-driven human animation by providing stable, high-resolution long-duration videos with realistic hand motions and good lip synchronization.
...
TL;DR: A pipeline using Vision-Language Models to explain vision models at both sample and dataset levels, addressing the gap in explaining general model behavior beyond individual samples.
...
Method: Proposes a pipeline leveraging Vision-Language Models to provide explanations at both sample level (individual predictions) and dataset level (overall model behavior across large datasets).

Result: The pipeline enables discovery of failure cases and provides insights into vision models with minimal effort, facilitating integration of xAI analysis into vision model development.

Conclusion: This approach advances image analysis by combining vision model development with explainable AI, allowing for better understanding of model behavior patterns and potential biases across entire datasets.
...
TL;DR: ATMS-KD framework combines adaptive temperature scheduling and mixed-sample augmentation to create lightweight CNN models for agricultural applications, achieving over 96.7% accuracy with compact models while maintaining low inference latency.


<details>
  <summary>Details</summary>
Motivation: To develop efficient lightweight CNN models suitable for resource-constrained agricultural environments where computational resources are limited but accurate plant maturity classification is needed.
...
Result: All student models achieved validation accuracies exceeding 96.7% (vs 95-96% with direct training), outperformed 11 established KD methods, with compact model achieving 97.11% accuracy and 72.19ms inference latency. Knowledge retention rates exceeded 99%.

Conclusion: ATMS-KD effectively transfers knowledge to lightweight models regardless of capacity, making it suitable for real-time agricultural computer vision applications with limited computational resources.
...
TL;DR: A novel framework using hybrid vision-language representations (VLRs) with CLIP and FLAVA models enables zero-shot classification of microstructures for material qualification without retraining, validated on additive manufacturing samples.


<details>
  <summary>Details</summary>
Motivation: Rapid and reliable qualification of advanced materials from non-conventional additive manufacturing processes is a bottleneck in industrial manufacturing, requiring better integration of microstructure data with expert knowledge.
...
Result: Validated on additively manufactured metal matrix composite dataset, successfully distinguishing acceptable vs defective samples. FLAVA showed higher visual sensitivity while CLIP provided consistent textual alignment. Z-score normalization improved classification effectiveness.

Conclusion: The framework enhances traceability and interpretability in qualification pipelines, enables human-in-the-loop decision-making without retraining, and advances semantic interoperability between raw data and expert knowledge for scalable domain-adaptable qualification strategies.
...
TL;DR: A training-free feedback framework that adapts patch-level correspondences from CLIP's output back to intermediate attention to improve spatial coherence and semantic consistency for open-vocabulary segmentation.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with open-vocabulary segmentation due to poor localization and semantic discrepancies between intermediate attention and final outputs, limiting its full potential despite strong visual-textual alignment.

Method: Proposes a feedback-driven self-adaptive framework with attention isolation, confidence-based pruning for sparse adaptation, and adaptation ensemble to propagate output coherence cues back to intermediate attention layers.

Result: The method consistently improves performance across eight benchmarks when integrated into four state-of-the-art approaches with three ViT backbones (B, L, H) and multiple attention types.

Conclusion: The training-free plug-in module effectively enhances semantic consistency between internal representations and final predictions by leveraging output-based spatial coherence priors, demonstrating broad applicability across various architectures.
...
Method: Train linear classifiers to predict visual categories from token embeddings at each layer using standardized anchor questions, evaluated under three prompt variations: lexical variants, semantic negation variants, and output format variants.

Result: Identified consistent stage-wise structure across LLaVA-1.5, LLaVA-Next-LLaMA-3, and Qwen2-VL models. Early layers perform visual grounding, middle layers support lexical integration and semantic reasoning, final layers prepare outputs. Structure remains stable but layer allocation shifts with base LLM architecture changes.

Conclusion: Provides unified perspective on MLLM layer organization and offers lightweight, model-agnostic approach for analyzing multimodal representation dynamics.
...
TL;DR: Proposes SLiCS - a supervised dictionary learning method to disentangle vision-language embedding spaces into concept-specific subspaces using sparse non-negative combinations, enabling precise concept-filtered image retrieval.


<details>
  <summary>Details</summary>
Motivation: To disentangle complex scene embeddings into separate concept-specific components that lie in different subspaces, allowing more precise concept-based manipulation and retrieval.

Method: Supervised dictionary learning with group-structured sparse non-negative combinations, alternating optimization with guaranteed convergence, leveraging text co-embeddings for semantic descriptions and zero-shot classification.

Result: SLiCS enables more precise concept-filtered image retrieval and conditional generation across different embeddings (CLIP, TiTok autoencoder, DINOv2), with improved quantitative and qualitative performance.

Conclusion: The proposed sparse linear concept subspaces successfully disentangle embedding spaces, providing interpretable concept-specific components that enhance precision in concept-based image manipulation tasks.
...
Result: Evaluated five state-of-the-art VLMs with board-certified pathologists, yielding 1015 clinician-model scoring events that revealed limitations including off-target answers, vague reasoning, and inconsistent pathology terminology.

Conclusion: MedFoundationHub provides a secure and accessible solution for deploying medical VLMs in clinical settings while identifying current limitations in model performance that need improvement.
...
TL;DR: Bidirectional Interaction Mamba (BIM) addresses computational efficiency vs interaction completeness trade-off in multi-task dense prediction using novel scanning mechanisms with linear complexity.


<details>
  <summary>Details</summary>
Motivation: Existing methods face trade-off between sufficient cross-task interaction (crucial for multi-task dense prediction) and computational efficiency - sufficient interaction often results in high computational complexity.
...
Conclusion: BIM successfully addresses the computational efficiency vs interaction completeness trade-off through innovative scanning mechanisms that preserve critical cross-task information with linear complexity while enhancing multi-granularity feature interactions.
...
TL;DR: A novel audio-guided visual editing framework that uses multi-modal prompts without additional training, handling complex editing scenarios where text-only approaches fail.


<details>
  <summary>Details</summary>
Motivation: Visual editing with diffusion models struggles with complex scenarios that textual guidance alone cannot adequately describe, creating a need for additional non-text editing prompts like audio.
...
Result: The framework excels in handling complicated editing scenarios by incorporating rich audio information, outperforming text-only approaches across diverse editing tasks.

Conclusion: The proposed audio-guided visual editing framework successfully addresses the limitations of text-only approaches by effectively integrating audio prompts without requiring additional training, demonstrating strong performance in complex multi-modal editing scenarios.
...
Result: Extensive experiments on four benchmark datasets demonstrate significant advancement in multi-label classification with state-of-the-art results.

Conclusion: The proposed AEVLP framework effectively addresses challenges in single positive multi-label learning by combining robust loss functions with dynamic pseudo-labeling strategies.
...
<details>
  <summary>Details</summary>
Motivation: Current ANN-SNN conversion methods perform well in classification tasks but remain suboptimal in visual detection tasks due to residual membrane potential issues caused by heterogeneous spiking patterns.

Method: Proposed a delay-spike approach to mitigate residual membrane potential issues and introduced a novel tdIF neuron architecture that allows IF neurons to dynamically adjust accumulation and firing behaviors based on temporal order of time-steps.
...
Conclusion: DUP-MCRNet effectively addresses detail preservation and edge clarity issues in salient object detection through uncertainty propagation and multimodal fusion strategies.
...
Method: Proposes MSMVD which: 1) Generates multi-scale BEV features by projecting multi-scale image features into BEV space scale-by-scale, 2) Processes these features using a feature pyramid network to combine information across different scales and views.
...
<details>
  <summary>Details</summary>
Motivation: Current deepfake detectors achieve high accuracy but are computationally expensive, making them impractical for real-time applications like video conferencing and social media.

Method: Uses spatial-frequency hybrid aware module with gated mechanism, token-selective cross attention for multi-level feature interaction, and residual-enhanced blur pooling for downsampling.
...
TL;DR: A novel medical image classification method combining dual-model weight selection with self-knowledge distillation to create lightweight models that match large model performance while being computationally efficient.


<details>
  <summary>Details</summary>
Motivation: Real-world medical settings face computational resource constraints that limit deployment of large-scale models, necessitating lightweight alternatives that maintain comparable performance.

Method: Uses dual-model weight selection to initialize two lightweight models with weights from a large pretrained model, applies self-knowledge distillation to enable diverse weight configurations without extra computational cost, and fine-tunes for target tasks.
...
Conclusion: The combination of dual-model weight selection with self-knowledge distillation effectively overcomes limitations of conventional approaches in retaining critical information in compact medical image classification models.
...
TL;DR: A novel LiDAR point cloud compression method using geometry re-densification and cross-scale feature propagation to achieve efficient predictive coding with state-of-the-art compression ratios and real-time performance.


<details>
  <summary>Details</summary>
Motivation: High-precision LiDAR scans create substantial storage and transmission overhead, while existing methods struggle with efficient context modeling due to extreme sparsity of geometric details, limiting compression performance and speed.
...
Result: Achieves state-of-the-art compression ratios on KITTI dataset with real-time performance of 26 FPS for both encoding and decoding at 12-bit quantization.

Conclusion: The proposed framework generates compact feature representations that enable efficient context modeling and accelerate the coding process, providing an effective solution for LiDAR point cloud compression with both high performance and speed.
...
TL;DR: This paper addresses 3D data scarcity by leveraging video commonsense priors for 3D generation, introducing Droplet3D-4M dataset and a generative model that produces spatially consistent and semantically plausible 3D content.


<details>
  <summary>Details</summary>
Motivation: Overcome data scarcity in 3D domain by utilizing abundant video data that contains spatial consistency priors and rich semantic information, providing alternative supervision for 3D generation.

Method: Created Droplet3D-4M, the first large-scale video dataset with multi-view annotations, and trained Droplet3D generative model supporting both image and dense text input to leverage video commonsense priors.
...
Conclusion: G^2Editor provides an effective unified framework for generating diverse driving scenarios with high fidelity and precise control, benefiting downstream data-driven tasks.
...
TL;DR: Pathology-informed domain randomization strategy for fetal brain segmentation that simulates CCD manifestations from healthy data alone, achieving substantial improvements in segmentation accuracy and biomarker estimation for rare corpus callosum dysgenesis cases.


<details>
  <summary>Details</summary>
Motivation: Accurate fetal brain segmentation is crucial for neurodevelopmental assessment, but the rarity of corpus callosum dysgenesis (CCD) severely limits annotated data, hindering deep learning model generalization for this clinically significant condition.

Method: Proposed a pathology-informed domain randomization strategy that embeds prior knowledge of CCD manifestations into synthetic data generation pipeline, simulating diverse brain alterations from healthy data alone without requiring pathological annotations.

Result: Validated on 248 healthy fetuses, 26 with CCD, and 47 with other pathologies. Reduced LCC estimation error from 1.89mm to 0.80mm in healthy cases and from 10.9mm to 0.7mm in CCD cases. Achieved substantial improvements on CCD cases while maintaining performance on other cases, with improved topological consistency.

Conclusion: Incorporating domain-specific anatomical priors into synthetic data pipelines can effectively mitigate data scarcity and enhance analysis of rare but clinically significant malformations, enabling more reliable shape-based analyses and biomarker derivation.
...
TL;DR: A unified framework for processing sign language, lip movements, and audio to generate spoken-language text, achieving state-of-the-art performance across multiple communication modalities.


<details>
  <summary>Details</summary>
Motivation: To make communication technologies accessible to deaf and hard-of-hearing individuals by integrating visual alternatives (sign language and lip reading) with audio in a unified system, as current ASR systems remain inaccessible to them.

Method: Developed a unified, modality-agnostic architecture capable of processing heterogeneous inputs (sign language, lip movements, audio) for spoken-language text generation, with explicit modeling of lip movements as a separate modality.

Result: Achieved performance on par with or better than state-of-the-art task-specific models across SLT, VSR, ASR, and AVSR tasks. Explicit modeling of lip movements significantly improved SLT performance.

Conclusion: The unified framework successfully integrates multiple communication modalities, demonstrates the importance of lip movements as non-manual cues in sign language comprehension, and provides accessible communication technology for deaf and hard-of-hearing individuals.
...
Conclusion: Video-MTR advances the state-of-the-art in long video understanding by enabling iterative reasoning through end-to-end training without external VLMs, providing more refined and contextually aware video analysis.
...
Method: Proposes Dual Uncertainty Optimization (DUO) with: 1) convex optimization of focal loss with unsupervised uncertainty weighting, 2) semantic-aware normal field constraint for geometric coherence, and 3) dual-branch mechanism that forms complementary loop between spatial perception and semantic classification.
...
Conclusion: DUO effectively addresses both semantic and geometric uncertainties in monocular 3D object detection, providing robust performance under domain shifts through its dual-branch optimization framework.
...
TL;DR: CaddieSet dataset with joint and ball trajectory data from golf swings, enabling interpretable swing analysis and ball trajectory prediction.
...
Method: Created CaddieSet dataset with joint information extracted from swing videos segmented into 8 phases using computer vision, plus 15 expert-defined key swing metrics. Tested with various benchmarks including interpretable models.

Result: Demonstrated feasibility of predicting ball trajectories using the dataset. Interpretable models showed swing feedback using joint features is quantitatively consistent with established golf domain knowledge.
...
TL;DR: IAENet: 3D anomaly detection framework that combines 2D and 3D experts using importance-aware fusion to achieve state-of-the-art performance with low false positives.


<details>
  <summary>Details</summary>
Motivation: 3D point cloud-based anomaly detection is underexplored compared to 2D methods, lacking powerful pretrained backbones. Existing fusion strategies suffer from performance degradation when modalities perform poorly.

Method: Proposes Importance-Aware Ensemble Network (IAENet) with novel Importance-Aware Fusion module that dynamically weights anomaly scores from 2D and 3D experts. Uses critical loss functions to optimize fusion while preserving unique strengths of each modality.

Result: Extensive experiments on MVTec 3D-AD show IAENet achieves new state-of-the-art performance with markedly lower false positive rate.

Conclusion: The proposed ensemble framework effectively bridges the gap between 2D and 3D anomaly detection, demonstrating practical value for industrial deployment through superior performance and reduced false positives.
...
TL;DR: DescriptiveEdit re-frames image editing as reference-image-based text-to-image generation, using a Cross-Attentive UNet to inject reference features without inversion or architectural changes.


<details>
  <summary>Details</summary>
Motivation: Address limitations of inversion-based methods (reconstruction errors) and instruction-based models (limited dataset quality/scale) in semantic image editing.

Method: Proposes a descriptive-prompt-based framework using Cross-Attentive UNet with attention bridges to inject reference image features into text-to-image generation process.

Result: Improves editing accuracy and consistency on Emu Edit benchmark, integrates seamlessly with ControlNet and IP-Adapter, and offers better scalability.

Conclusion: DescriptiveEdit effectively overcomes dataset quality limitations and preserves generative power of text-to-image models without architectural modifications.
...
TL;DR: DCFS is a novel CTTA framework that uses dual-path feature consistency and confidence-aware learning to address error accumulation in continual test-time adaptation without source data access.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods rely on pseudo-labels from model predictions, which suffer from quality issues and error accumulation. Without source data, models can develop learning biases from focusing only on target domain features.

Method: Proposes dual classifiers to disentangle semantic-related and domain-related features, maintains consistency between sub-features and whole features, and uses adaptive thresholds with confidence scores for weighted self-supervised learning to reduce pseudo-label noise.
...
Conclusion: DCFS effectively addresses error accumulation and learning bias problems in CTTA by comprehensively capturing data features from multiple perspectives and reducing pseudo-label noise through confidence-aware learning.
...
TL;DR: Using 3D Gaussian Splatting to fine-tune camera calibration through backpropagation of novel view color loss, improving reconstruction quality by 0.4 dB PSNR on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Camera calibration quality significantly impacts novel view synthesis results, with even 1-pixel errors affecting reconstruction quality. Since real scenes lack ground truth calibration, calibration quality is typically assessed through novel view synthesis performance.

Method: Proposes using a 3DGS (3D Gaussian Splatting) model to fine-tune camera calibration by backpropagating novel view color loss with respect to camera parameters.

Result: The new calibration method achieves an average improvement of 0.4 dB PSNR on the dataset used as reference by 3DGS.

Conclusion: While the fine-tuning process may be time-consuming, it is particularly valuable for reference scenes like Mip-NeRF 360 where novel view quality is paramount, making the calibration optimization worthwhile despite the computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Medical image labeling is time-consuming and labor-intensive, and existing active domain adaptation methods suffer from negative transfer and limited source data access. Multi-modal medical data query strategies are also underexplored.
...
Result: Empirical validation shows the method achieves favorable segmentation performance and significantly outperforms state-of-the-art ADA methods on diverse gross tumor volume segmentation tasks.

Conclusion: The proposed framework effectively reduces annotation costs while maintaining high segmentation accuracy, addressing limitations of previous ADA methods in medical imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-based 3D object detectors require manually annotated labels which are time-consuming and labor-intensive to obtain. Current unsupervised methods simply integrate pseudo-boxes from LiDAR and RGB without leveraging their complementary nature at the data level.

Method: Proposes data-level fusion framework with bi-directional fusion: real points get category labels from 2D space, while 2D pixels are projected to 3D to enhance point density. Uses local radius filtering for depth errors and global statistical filtering for segmentation outliers. Implements dynamic self-evolution strategy to iteratively refine pseudo-boxes.
...
Conclusion: The proposed data-level fusion approach effectively leverages complementary information from LiDAR and RGB modalities, overcoming limitations of label-level fusion strategies and demonstrating superior performance in unsupervised 3D object detection.
...
Conclusion: The method provides accurate BMI estimation from smartphone images with robust generalization across datasets, and the complete pipeline including mobile deployment is released as open-source.
...
Result: DSAN algorithm achieved 91.2% accuracy on COVID-19 dataset using Resnet50 and showed +6.7% improvement in dynamic data stream scenarios compared to baseline. Demonstrated remarkable explainability on COVID-19 and skin cancer datasets.

Conclusion: The study provides valuable insights into effective model adaptation for medical data and contributes to understanding DA techniques, with DSAN showing outstanding performance and applicability in medical domains.
...
Method: Introduces Contrastive Learning through Auxiliary Branch (CLAB) with contrastive loss to improve backbone feature representation. Uses dynamic loss weighting strategy that prioritizes auxiliary learning early and shifts to detection task as training converges.

Result: Achieves 84.0% mAP with ResNet-101 and 85.2% mAP with ResNeXt-101 on ImageNet VID dataset, setting state-of-the-art performance for CNN-based models without post-processing.

Conclusion: CLAB provides an effective and computationally efficient approach to video object detection, demonstrating consistent performance gains through contrastive learning and adaptive training strategy.
...
TL;DR: Analysis of CLIP vision encoders under typographic attacks reveals specialized attention heads that transmit text information. A training-free defense method is introduced by ablating these heads, improving robustness by up to 19.6% on typographic ImageNet-100 with minimal standard accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Typographic attacks exploit multi-modal systems by injecting text into images, causing targeted misclassifications, malicious content generation, and VLM jailbreaks. There is a need to understand and defend against these attacks in CLIP models.

Method: Analyze CLIP vision encoder behavior under typographic attacks to locate specialized attention heads. Introduce a training-free defense by selectively ablating a typographic circuit consisting of these attention heads.

Result: Method improves performance by up to 19.6% on typographic ImageNet-100 while reducing standard ImageNet-100 accuracy by less than 1%. Remains competitive with state-of-the-art finetuning-based defenses.

Conclusion: The approach provides effective defense against typographic attacks without requiring finetuning. A family of dyslexic CLIP models is released as drop-in replacements for safety-critical applications where text-based manipulation risks outweigh text recognition utility.
...
Method: Proposes FastFit framework with Semi-Attention mechanism and class embeddings instead of timestep embeddings for reference items, decoupling reference feature encoding from denoising process. Also introduces DressCode-MR dataset with 28,179 sets of high-quality paired images across 5 categories.
...
Conclusion: FastFit fundamentally breaks the efficiency bottleneck of virtual try-on systems while supporting complex multi-reference compositions, enabling real-world applications through its cacheable architecture and comprehensive dataset.
...
<details>
  <summary>Details</summary>
Motivation: Thermal cameras struggle with signage from similar-material objects, while event cameras have non-uniform sampling. Both excel in low-light but have complementary weaknesses that need fusion.

Method: Dual-boosting mechanism that fuses thermal frames and event signals. Thermal frames provide motion cues for temporal alignment, while event signals add subtle signage content to thermal frames.
...
Conclusion: The proposed unsupervised fusion method effectively combines thermal and event modalities to enhance traffic signage perception in low-illumination autonomous driving scenarios.
...
TL;DR: Proposes active deepfake defense using low-frequency perturbations to disrupt face swapping manipulation while preserving visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods are passive and focus on post-event analysis rather than preventing attacks, creating a need for active defense approaches.

Method: Combines frequency and spatial domain features using discrete wavelet transform (DWT) to extract low-frequency components and generate perturbations. Features encoder, perturbation generator, and decoder architecture to disrupt facial manipulation models while preserving high-frequency details.

Result: Experiments on CelebA-HQ and LFW datasets show significant reductions in face-swapping effectiveness, improved defense success rates, and preservation of visual quality.

Conclusion: The proposed active defense method effectively disrupts deepfake generation processes through low-frequency perceptual perturbations, providing a proactive approach to deepfake prevention while maintaining visual plausibility.
...
Method: Introduces uncertainty through noise signals as diversity carriers in transformer architecture, creates continuous text representation space, and integrates latent space sampler for stochastic sampling.

Result: Achieves significantly enhanced diversity while maintaining state-of-the-art text consistency performance on HumanML3D and KIT-ML benchmarks.
...
Result: The model shows promising results compared to state-of-the-art generative models (VAE and diffusion methods) through domain expert evaluation, qualitative assessment, and quantitative metrics for image generation and signal processing.

Conclusion: PI-GenMFI effectively generates synthetic MFI training data that can optimize the defect localization process in semiconductor manufacturing by enabling efficient ML model training despite limited real data availability.
...
TL;DR: Proposes a GAN-based data reconstruction attack with progressive feature optimization that significantly outperforms existing attacks on deep neural networks, especially in high-resolution and out-of-distribution scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing data reconstruction attacks on split inference systems are limited to shallow models and fail to leverage semantic priors, leaving privacy vulnerabilities in deeper DNNs unaddressed.

Method: GAN-based framework with Progressive Feature Optimization (PFO) that decomposes generator into hierarchical blocks and incrementally refines intermediate representations, using L1-ball constraint to stabilize optimization.

Result: Outperforms prior attacks by large margin in high-resolution scenarios, out-of-distribution settings, and against deeper/more complex DNNs.

Conclusion: The proposed attack framework demonstrates significantly improved reconstruction quality and generalizability, revealing greater privacy risks in split inference systems than previously recognized.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations in control flexibility, motion naturalness, and expression quality, and available datasets are primarily collected in lab settings, hindering practical real-world applications.

Method: Proposes a diffusion-based framework with two key modules: text-guided decoupled emotive module for appearance modeling, and emotive audio attention module to capture interplay between emotion and audio. Also constructs an emotional talking head dataset and uses emotion-aware sampling training with progressive functional training.
...
Conclusion: EmoCAST successfully addresses the challenges of emotional talking head synthesis through its novel framework design, dataset construction, and training strategies, enabling practical applications in real-world scenarios.
...
TL;DR: SwinUNETR-based AI framework for breast cancer detection from MRI, achieving 2nd place in multi-center challenge with public code release.


<details>
  <summary>Details</summary>
Motivation: Early breast cancer detection is crucial, especially in high-risk women with dense breast tissue where mammography is less effective. MRI offers high sensitivity but needs AI support for improved diagnosis.
...
Conclusion: The framework shows strong potential to support clinical breast MRI interpretation and has been made publicly available to facilitate further research and development.
...
Conclusion: AvatarBack successfully addresses the back-head reconstruction challenge in 3D Gaussian avatars, providing complete and consistent avatar modeling through effective synthetic view generation and geometric alignment techniques.
...
Conclusion: Foundation models can effectively bridge the gap where traditional facial recognition methods fail in artwork analysis, enabling better sitter identification in historical paintings.
...
TL;DR: CraftGraffiti is an end-to-end text-guided graffiti generation framework that preserves facial identity during extreme stylistic transformations using a style-first approach with face-consistent self-attention and CLIP-guided pose customization.
...
Result: Quantitative results show competitive facial feature consistency and state-of-the-art aesthetic/human preference scores. Qualitative analysis and live deployment at Cruilla Festival demonstrate real-world creative impact.
...
<details>
  <summary>Details</summary>
Motivation: Existing alignment methods for large visual-language models rely on external datasets and human annotations, which limit scalability and increase costs while still suffering from hallucinations and safety issues.

Method: Generates debiased self-judgment scores internally within the model as a self-evaluation metric, enabling autonomous improvement of visual-linguistic alignment without external resources.
...
Conclusion: The proposed self-judgment approach offers a more effective and scalable solution for aligning large visual-language models by enabling autonomous internal evaluation and improvement.
...
TL;DR: S-HArM dataset for intent-aware classification of AI-generated images with 9,576 real-world image-text pairs labeled as Humor/Satire, Art, or Misinformation, plus synthetic data generation strategies and model evaluations.
...
Method: Created S-HArM dataset from Twitter/X and Reddit, used three prompting strategies (image-guided, description-guided, multimodally-guided) with Stable Diffusion for synthetic data, and evaluated various models including modality fusion, contrastive learning, and large vision-language models.

Result: Models trained on image- and multimodally-guided synthetic data generalize better to real-world content due to preserved visual context, but overall performance remains limited.

Conclusion: Inferring intent from AI-generated content is complex and requires specialized architectures beyond current multimodal approaches, highlighting the need for continued research in intent-aware classification.
...
TL;DR: MobileCLIP2 improves upon MobileCLIP with better teacher ensembles and captioner fine-tuning, achieving state-of-the-art zero-shot accuracy at low latencies with smaller models.


<details>
  <summary>Details</summary>
Motivation: To enhance the multi-modal reinforced training of MobileCLIP by improving CLIP teacher ensembles and captioner teachers, aiming for better zero-shot accuracy with efficient mobile deployment.

Method: Improved multi-modal reinforced training through better CLIP teacher ensembles trained on DFN dataset, and enhanced captioner teachers fine-tuned on diverse high-quality image-caption datasets. Includes temperature tuning in contrastive knowledge distillation and combining synthetic captions from multiple models.

Result: MobileCLIP2 achieves state-of-the-art ImageNet-1k zero-shot accuracies at low latencies, with 2.2% improvement for MobileCLIP2-B over MobileCLIP-B. MobileCLIP2-S4 matches SigLIP-SO400M/14 accuracy while being 2x smaller and improves on DFN ViT-L/14 at 2.5x lower latency.

Conclusion: The enhanced training approach with better teacher ensembles and captioner fine-tuning significantly improves zero-shot performance while maintaining low latency, making MobileCLIP2 highly efficient for mobile applications.
...
TL;DR: A dynamic neural video compression framework with variable coding routes and rate control agent that achieves precise bitrate control and superior RD performance.
...
Result: Achieves 14.8% BD-Rate reduction, 0.47dB BD-PSNR gain over SOTA methods with 1.66% average bitrate error, enabling Rate-Distortion-Complexity Optimization.

Conclusion: The framework successfully addresses variable bitrate challenges in neural video compression while maintaining superior RD performance and precise rate control.
...
TL;DR: CardioMorphNet is a recurrent Bayesian deep learning framework for 3D cardiac motion estimation that uses shape-guided registration instead of intensity-based methods, achieving superior performance on cardiac motion estimation with lower uncertainty.
...
Method: Recurrent variational autoencoder framework with two posterior models for bi-ventricular segmentation and motion estimation. Uses Bayesian formulation to guide registration of segmentation maps without intensity-based similarity loss, leveraging sequential short-axis volumes and spatio-temporal features.
...
<details>
  <summary>Details</summary>
Motivation: Atypical mitotic figures (AMFs) are important histopathological markers but are challenging to identify consistently due to domain shifts from scanner, stain, and acquisition differences, requiring robust classification methods.

Method: Three key components: (1) style perturbations at early/mid backbone stages for feature diversity, (2) attention-refined feature alignment across sites using weak domain labels, (3) EMA teacher distillation with temperature-scaled KL divergence for prediction stability.
...
Conclusion: The method provides strong, balanced performance with negligible inference overhead and only requires coarse domain metadata, making it a competitive solution for domain-robust AMF classification in histopathology.
...
TL;DR: Pref-GRPO addresses reward hacking in T2I generation by using pairwise preference rewards instead of pointwise scoring, and introduces UniGenBench with detailed evaluation criteria for comprehensive model assessment.
...
Method: Pref-GRPO uses pairwise preference reward modeling where images are compared within groups and win rate serves as the reward signal, shifting optimization from score maximization to preference fitting. Also introduces UniGenBench with 600 prompts across 5 themes and detailed semantic consistency evaluation using MLLM.

Result: Pref-GRPO effectively differentiates subtle image quality differences, provides stable advantages, and mitigates reward hacking. UniGenBench comprehensively evaluates T2I models and validates Pref-GRPO's effectiveness.

Conclusion: The proposed Pref-GRPO method with pairwise preference optimization and the UniGenBench framework provide more stable training and comprehensive evaluation for text-to-image generation models, addressing limitations of current approaches.
...
<details>
  <summary>Details</summary>
Motivation: Existing generalizable Gaussian splatting methods struggle with encoding discriminative, multi-view consistent features for accurate geometry construction from sparse views, limiting their rendering quality and generalization ability.
...
Result: Extensive experiments on benchmark datasets show state-of-the-art rendering quality and generalization ability, achieving photorealistic synthesis from sparse input views.

Conclusion: C3-GS successfully addresses feature learning limitations in generalizable Gaussian splatting through multi-constraint integration, enabling high-quality novel view synthesis without per-scene optimization.
...
TL;DR: SeqVLM is a novel zero-shot 3D visual grounding framework that uses multi-view scene images with spatial information to localize objects from natural language descriptions without scene-specific training, achieving state-of-the-art performance on benchmarks.
...
Method: Uses 3D semantic segmentation to generate instance proposals, refines them through semantic filtering, employs proposal-guided multi-view projection to preserve spatial relationships, and implements dynamic scheduling for VLM processing of sequence-query prompts.
...
Conclusion: SeqVLM advances 3D visual grounding toward greater generalization and real-world applicability by effectively addressing spatial reasoning limitations and computational challenges in zero-shot settings.
...
TL;DR: CLIP vision-language models show varying robustness to occlusion in military vehicle classification, with Transformers outperforming CNNs and fine-tuning significantly improving occlusion tolerance.
...
Method: Used custom dataset of 18 military vehicle classes, evaluated CLIP variants' robustness to occlusion using Normalized Area Under the Curve (NAUC) across different occlusion percentages, comparing Transformer vs CNN architectures and fine-tuning approaches.

Result: Four key findings: (1) Transformer-based CLIP outperforms CNNs, (2) fine-grained dispersed occlusions degrade performance more than larger contiguous ones, (3) linear-probed models show sharp performance drop at ~35% occlusion, (4) backbone fine-tuning pushes performance drop threshold to >60% occlusion.
...
Conclusion: The SKGE-Swin architecture demonstrates enhanced performance in autonomous driving through improved global feature representation and context awareness, with planned ablation studies to validate component contributions.
...
<details>
  <summary>Details</summary>
Motivation: Existing VideoQA models struggle with higher-order reasoning, lack interpretability, and rely on opaque monolithic pipelines that entangle different reasoning components.

Method: Two-stage architecture with Causal Chain Extractor (CCE) that generates causal chains from video-question pairs, and Causal Chain-Driven Answerer (CCDA) that produces answers grounded in these chains. Uses LLMs to generate training data and introduces CauCo evaluation metric.
...
Conclusion: The proposed modular framework successfully enables transparent causal reasoning and positions CCE as a reusable causal reasoning engine across diverse domains.
...
TL;DR: This paper surveys abstract concept recognition in videos, arguing that recent advances in foundation models provide an ideal setting to address this crucial challenge in video understanding.


<details>
  <summary>Details</summary>
Motivation: While machines can understand concrete video content (objects, actions, scenes), humans uniquely recognize abstract concepts like justice and freedom. Abstract concept recognition remains an open challenge requiring multi-level semantic reasoning.

Method: The paper conducts a comprehensive survey of different tasks and datasets used for understanding abstract concepts in video content, drawing on decades of community experience and research attempts.

Result: The survey identifies that researchers have periodically attempted to solve abstract concept recognition tasks using available tools, but the challenge remains largely unsolved despite these efforts.

Conclusion: Foundation models present an opportunity to revisit abstract concept understanding in videos, and leveraging decades of community experience will help avoid re-inventing solutions while addressing this important grand challenge.
...
TL;DR: Proposed Global Class Activation Probabilistic Map Evaluation method for skin lesion classification that analyzes all classes' activation maps probabilistically at pixel level, combined with SafeML for false diagnosis detection.


<details>
  <summary>Details</summary>
Motivation: Address distrust in AI models for medical diagnosis by providing trustworthy, explainable diagnoses beyond just high accuracy. Existing explainability methods have reliability issues - LIME-based methods suffer from inconsistency, while CAM-based methods fail to consider all classes.

Method: Global Class Activation Probabilistic Map Evaluation that analyzes all classes' activation probability maps probabilistically and at pixel level. Combined with SafeML for detecting false diagnoses and issuing warnings.

Result: Method visualizes diagnostic process in unified manner, helps reduce misdiagnosis risk, enhances detection of false diagnoses, and improves diagnostic reliability and patient safety.

Conclusion: The proposed approach addresses explainability limitations in skin lesion classification, providing more trustworthy AI diagnoses that can improve patient safety in medical practice.
...
<details>
  <summary>Details</summary>
Motivation: Vision-language models like CLIP often fail at compositional semantics, treating images as 'bag-of-words' and incorrectly combining attributes with objects. The research explores whether generative diffusion-based classifiers can overcome these limitations.

Method: Evaluated three models (Diffusion Classifier, CLIP, and ViLT) on their ability to bind objects with attributes and relations in zero-shot learning (ZSL) and generalized zero-shot learning (GZSL) settings. Analyzed embedding similarities to understand performance differences.
...
Conclusion: While diffusion-based classifiers show promise for compositional generalization, relational reasoning remains a fundamental challenge for vision-language models due to representation similarities in spatial concepts.
...
TL;DR: A novel surfel-based pose learning regression approach for robust point cloud registration that handles orientation and uncertainty issues ignored by previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud registration methods ignore point orientations and uncertainties, making them susceptible to noisy inputs and aggressive rotations, requiring extensive training data with transformations.

Method: Initializes surfels from Lidar point clouds using virtual camera parameters, learns explicit SE(3) equivariant features through SE(3) equivariant convolutional kernels, and uses cross-attention mechanism with Huber loss.

Result: Superior and robust performance demonstrated on both indoor and outdoor datasets compared to state-of-the-art methods.

Conclusion: The proposed surfel-based approach with SE(3) equivariant features effectively addresses orientation and uncertainty challenges in point cloud registration, providing robust performance on real scans.
...
<details>
  <summary>Details</summary>
Motivation: Current dental caries detection methods suffer from suboptimal accuracy due to subtle contrast variations and diverse lesion morphology in panoramic X-rays. The work is inspired by dentists' clinical workflow of combining whole-image screening with detailed tooth-level inspection.

Method: Uses automated tooth detection to create global (panoramic) and local (cropped tooth) views. Pretrains two vision foundation models separately, then integrates them using a Gated Cross-View Attention module to dynamically fuse dual-view features for final caries detection.

Result: DVCTNet demonstrates superior performance against state-of-the-art methods on both public dataset and newly curated high-precision dataset annotated with intra-oral images and panoramic X-rays for double verification.

Conclusion: The method shows clinical applicability for accurate dental caries detection by effectively mimicking dentists' dual-view inspection approach through automated co-training of global and local features.
...
TL;DR: FusionCounting integrates visible-infrared image fusion with crowd counting in a unified multi-task framework using dynamic loss weighting and adversarial training.


<details>
  <summary>Details</summary>
Motivation: Existing VIF methods focus on image quality or use semantic segmentation/detection which require heavy annotations. Crowd counting provides quantitative density measurement with minimal annotation, making it suitable for dense scenes.

Method: Multi-task learning framework that jointly optimizes VIF and crowd counting. Uses dynamic loss function weighting for task balance and incorporates adversarial training for robustness.

Result: Experimental results show improved image fusion quality and superior crowd counting performance on public datasets.

Conclusion: Crowd counting can effectively guide VIF with minimal annotation requirements, and the unified framework benefits both tasks through mutual enhancement.
...
TL;DR: Novel pipeline using Vision Language Models (VLMs) with LoRA fine-tuning for surgical tool keypoint estimation, outperforming traditional CNNs/Transformers with minimal training.


<details>
  <summary>Details</summary>
Motivation: Traditional CNN and Transformer-based approaches often overfit on small-scale medical datasets, requiring a method that leverages pre-trained VLMs' generalization capabilities for better performance in low-resource scenarios.

Method: Leverage pre-trained VLMs fine-tuned using LoRA technique. Design prompts to create instruction-tuning dataset that aligns visual features with semantic keypoint descriptions. Requires only two epochs of fine-tuning.

Result: The adapted VLM outperforms baseline models, demonstrating effectiveness of LoRA in low-resource scenarios with improved keypoint detection performance.

Conclusion: This approach not only improves 2D keypoint estimation but also paves the way for future work in 3D surgical hands and tools pose estimation, showing promise for medical computer vision applications.
...
Conclusion: The proposed PointDGRWKV framework successfully adapts RWKV architecture for DG PCC by addressing spatial distortion and attention drift issues, achieving superior generalization performance while maintaining linear efficiency.
...
TL;DR: PathMR is a cell-level multimodal visual reasoning framework that generates diagnostic explanations and predicts cell distribution patterns, outperforming state-of-the-art methods in text quality, segmentation accuracy, and cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Current deep learning diagnostic tools lack transparency and traceable rationale, limiting clinical adoption despite improved efficiency. There's a need for models that provide both pixel-level segmentation and semantically aligned textual explanations for dependable AI-assisted pathology.

Method: Proposed PathMR framework that takes pathological images and textual queries to generate expert-level diagnostic explanations while simultaneously predicting cell distribution patterns. Evaluated on PathGen dataset and newly developed GADVR dataset.

Result: PathMR consistently outperforms state-of-the-art visual reasoning methods across multiple metrics including text generation quality, segmentation accuracy, and cross-modal alignment on both datasets.

Conclusion: PathMR demonstrates significant potential for improving interpretability in AI-driven pathological diagnosis by providing transparent insights through localized lesion regions and expert-style diagnostic narratives.
...
<details>
  <summary>Details</summary>
Motivation: PDAC has low survival rates due to late detection, requiring improved early diagnostic methods through automated analysis of medical imaging.

Method: Analyzed 40 patient samples with dual-modality imaging, evaluated 6 deep learning architectures (CNNs vs ViTs), used modified ResNet with frozen pre-trained layers and class-weighted training to address dataset limitations.

Result: Achieved over 90% accuracy in distinguishing normal, fibrotic, and cancerous pancreatic tissue, significantly improving on current manual analysis methods.

Conclusion: Establishes robust automated PDAC detection pipeline with clinical deployment potential, provides insights for deep learning on limited medical imaging datasets, and offers foundation for expansion to other cancer types.
...
TL;DR: This thesis develops counterfactual frameworks for explaining, auditing, and mitigating bias in vision classifiers and generative models through systematic attribute manipulation.


<details>
  <summary>Details</summary>
Motivation: To address interpretability and fairness in AI by using counterfactual reasoning to uncover spurious correlations, probe causal dependencies, and build more robust systems.
...
Result: Created methods that quantify concept dependencies, improve fairness and accuracy while avoiding stereotypes, enable causal auditing of identity-related biases, diagnose intersectional biases, and provide modular bias mitigation.

Conclusion: Counterfactuals serve as a unifying lens for interpretability, fairness, and causality in both discriminative and generative models, establishing principled, scalable methods for socially responsible bias evaluation and mitigation.
...
TL;DR: This survey paper provides a comprehensive overview of unified perception for autonomous vehicles, categorizing methods into three paradigms (Early, Late, and Full Unified Perception) and establishing a systematic taxonomy for understanding and advancing this emerging field.


<details>
  <summary>Details</summary>
Motivation: Traditional modular perception pipelines in autonomous vehicles suffer from error accumulation and limited inter-task synergy, while unified perception offers potential improvements in robustness, contextual reasoning, and efficiency while maintaining interpretability.

Method: The authors introduce a holistic taxonomy categorizing unified perception methods along task integration, tracking formulation, and representation flow. They systematically review existing methods, architectures, training strategies, datasets, and open-source availability.

Result: The survey establishes the first comprehensive framework for unified perception, consolidates fragmented research efforts, and provides a systematic categorization of methods into three distinct paradigms.

Conclusion: This work guides future research toward more robust, generalizable, and interpretable perception systems for autonomous vehicles by providing a structured foundation for understanding and advancing unified perception approaches.
...
TL;DR: Dino U-Net leverages DINOv3 foundation model features for medical image segmentation, achieving state-of-the-art performance across multiple datasets with a parameter-efficient approach.


<details>
  <summary>Details</summary>
Motivation: Effectively transferring foundation model representations from natural images to precise medical image segmentation applications remains challenging.

Method: Proposes an encoder-decoder architecture with frozen DINOv3 backbone, specialized adapter for feature fusion, and fidelity-aware projection module (FAPM) to preserve feature quality during dimensionality reduction.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitations of multi-camera centralized setups (high computational costs, bandwidth requirements) and edge device constraints (accuracy degradation, temporal/spatial inconsistencies) in Industry 5.0 human activity monitoring.
...
TL;DR: Novel olive tree segmentation method using SAM with alignment and shape constraints achieves 98% accuracy


<details>
  <summary>Details</summary>
Motivation: Climate change necessitates olive biodiversity preservation through early anomaly detection using remote sensing technology

Method: Integrates Segment Anything Model (SAM) with corrections based on tree alignment patterns and learnable shape/size constraints

Result: Achieved 98% accuracy rate, significantly improving upon SAM's baseline performance of 82%

Conclusion: The approach provides effective olive tree management solutions through advanced satellite image segmentation
...
TL;DR: E-ConvNeXt integrates CSPNet with ConvNeXt to create a lightweight network that reduces complexity by 80% while maintaining high accuracy, achieving 78.3-81.9% Top-1 accuracy on ImageNet.


<details>
  <summary>Details</summary>
Motivation: Many high-performance networks are too complex for lightweight applications, limiting their practical use. The paper aims to create a more efficient ConvNeXt variant suitable for resource-constrained scenarios.

Method: Three core innovations: 1) Integrate Cross Stage Partial Network (CSPNet) with ConvNeXt to reduce complexity by up to 80%, 2) Optimize Stem and Block structures for better feature expression and efficiency, 3) Replace Layer Scale with channel attention mechanism.
...
Conclusion: E-ConvNeXt successfully creates a lightweight network with superior accuracy-efficiency balance, making ConvNeXt suitable for resource-constrained applications while maintaining competitive performance.
...
TL;DR: DrivingGaussian++ is an efficient framework for realistic reconstruction and controllable editing of dynamic autonomous driving scenes using 3D Gaussians and LiDAR priors, supporting training-free editing and realistic motion generation.
...
Conclusion: DrivingGaussian++ provides an effective solution for realistic reconstruction and controllable editing of dynamic driving scenes, demonstrating superior performance in both reconstruction quality and editing capabilities while significantly enhancing scene diversity.
...
TL;DR: Novel methods using web data and automatic annotation (CAAAv2) to create large-scale dataset MIMLv2 (246K images) for image manipulation localization, achieving 31% performance gain over previous methods.


<details>
  <summary>Details</summary>
Motivation: High cost of data acquisition and severe lack of high-quality annotated datasets for image manipulation localization, which poses risks to social security.

Method: Leverage web data with CAAAv2 for automatic pixel-level annotation, QES metric for filtering unreliable annotations, Object Jitter for generating manipulation artifacts, and Web-IML model for web-scale supervision.

Result: Created MIMLv2 dataset (120x larger than existing datasets), Web-IML achieves 31% performance gain and surpasses previous SOTA TruFor by 24.1 average IoU points.
...
Method: Mixture-of-Generative-Experts architecture where each expert specializes in simulating different subsets of the data, tailored for the Zero Degree Calorimeter in ALICE experiment.
...
TL;DR: POSE is a one-step distillation framework that accelerates large-scale video diffusion models by 100x while maintaining video quality through phased adversarial training and equilibrium mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing video acceleration methods fail to model temporal coherence and cannot provide single-step distillation for large-scale video models, creating bottlenecks in sampling efficiency.

Method: Two-phase process: (1) Stability priming - warm-up mechanism to stabilize adversarial distillation across SNR regimes, (2) Unified adversarial equilibrium - self-adversarial distillation to reach Nash equilibrium in Gaussian noise space, plus conditional adversarial consistency for semantic and frame consistency.
...
Conclusion: POSE successfully bridges the gap in video acceleration by enabling single-step generation of high-quality videos from large-scale diffusion models through innovative phased distillation techniques.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models are computationally expensive, and while prior work focuses on per-inference efficiency, this paper explores reducing redundancy across correlated prompts by leveraging the coarse-to-fine nature of diffusion models.

Method: Proposes clustering prompts based on semantic similarity and sharing computation in early denoising steps where diffusion models capture shared structures among similar prompts. Leverages UnClip's text-to-image prior to enhance diffusion step allocation.

Result: Significantly reduces compute cost while improving image quality for models trained conditioned on image embeddings. The method integrates seamlessly with existing pipelines and scales with prompt sets.

Conclusion: The approach effectively reduces the environmental and financial burden of large-scale text-to-image generation by exploiting semantic similarities between prompts and optimizing computation sharing in early diffusion stages.
...
<details>
  <summary>Details</summary>
Motivation: Mitosis detection is crucial for tumor assessment, but machine learning models suffer performance degradation when tested on domains different from training data. Domain shift remains a significant challenge in medical imaging.
...
Result: Preliminary experiments on MIDOG++ dataset indicate there is large room for improvement for the proposed method, suggesting the approach needs further refinement.

Conclusion: The Mamba-based approach with stain augmentation shows potential for addressing domain shift in mitosis detection but requires significant improvements to achieve robust performance across different domains.
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models for recognizing atypical mitotic figures suffer significant performance drops under domain shift, making domain generalization crucial for accurate tumor aggressiveness assessment.

Method: Multi-task learning approach that exploits auxiliary tasks correlated to the main classification task to help the model focus only on the classification object while ignoring domain-varying background elements.
...
Conclusion: The multi-task learning approach shows potential for addressing domain shift problems in histopathology image analysis, particularly for atypical mitosis detection in tumor assessment.
...
TL;DR: FW-GAN is a one-shot handwriting synthesis framework that uses frequency-aware components and Wave-MLP to generate realistic, style-consistent handwriting from a single example, addressing limitations of current methods in capturing long-range dependencies and frequency information.


<details>
  <summary>Details</summary>
Motivation: Labeled handwriting data scarcity limits recognition systems, and current synthesis methods struggle with long-range dependencies and ignore frequency information which is crucial for capturing fine-grained stylistic details.
...
Conclusion: FW-GAN effectively addresses handwriting synthesis challenges by leveraging frequency information and advanced architecture, providing valuable data augmentation for handwriting recognition systems with limited training data.
...
<details>
  <summary>Details</summary>
Motivation: Video LLMs face computational challenges due to excessive visual tokens. Current pruning methods ignore dynamic characteristics and temporal dependencies of video frames, treating video understanding as a simple multi-frame task.

Method: Two-stage approach: 1) Segment video based on frame similarity and dynamically allocate token budget per segment to maximize marginal gain; 2) Temporal-guided DPC algorithm that models inter-frame uniqueness and intra-frame diversity to maximize token-level marginal gains.
...
Conclusion: MMG-Vid effectively maximizes limited token budget utilization, significantly improving efficiency while preserving strong video understanding performance through training-free token pruning.
...
TL;DR: HydraFake dataset addresses real-world deepfake detection challenges with hierarchical generalization testing, and Veritas MLLM-based detector uses pattern-aware reasoning to achieve superior cross-domain detection performance.
...
Method: Introduces HydraFake dataset with diversified deepfake techniques and in-the-wild forgeries, plus Veritas - a multi-modal LLM detector using pattern-aware reasoning (planning and self-reflection) with two-stage training pipeline.
...
Conclusion: The proposed HydraFake dataset and Veritas MLLM-based detector with pattern-aware reasoning effectively address real-world deepfake detection challenges and show superior generalization capabilities compared to existing methods.
...
TL;DR: FakeParts introduces a new class of deepfakes with subtle localized manipulations in videos, making them harder to detect than traditional deepfakes. The paper presents FakePartsBench, a large benchmark dataset with 25K+ videos to evaluate detection methods.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detection methods struggle with partial manipulations that blend seamlessly with authentic content, creating an urgent vulnerability that needs to be addressed.

Method: Created FakePartsBench dataset with over 25K videos featuring pixel-level and frame-level manipulation annotations for various partial deepfake types (altered expressions, object substitutions, background modifications). Conducted user studies and evaluated state-of-the-art detection models.
...
Conclusion: This work identifies critical weaknesses in current deepfake detection and provides the first large-scale benchmark (FakePartsBench) to enable development of more robust methods against partial video manipulations.
...
TL;DR: First data-driven multi-view 3D point tracker that handles arbitrary points in dynamic scenes using 4+ cameras, overcoming monocular depth ambiguities and occlusion issues without per-sequence optimization.
...
Result: Achieves median trajectory errors of 3.1 cm on Panoptic Studio and 2.0 cm on DexYCB benchmarks. Generalizes well to 1-8 camera views and 24-150 frame videos.
...
Method: Uses a single vision-language model as a generative reward model that can distinguish winners and losers for any given task and evaluation criterion. Applied to multi-task reinforcement learning for mask-guided image generation tasks including image fill, extend, object removal, and text rendering.
...
TL;DR: Dress&Dance is a video diffusion framework that generates high-quality virtual try-on videos using a single user image and various garment types, achieving superior results through a novel conditioning network called CondNet.


<details>
  <summary>Details</summary>
Motivation: To create high-quality virtual try-on videos that accurately show users wearing desired garments while moving according to reference videos, addressing limitations in existing solutions.

Method: Uses a video diffusion framework with CondNet, a novel conditioning network that employs attention mechanisms to unify multi-modal inputs (text, images, videos). Trained on heterogeneous data combining limited video data with larger image datasets in a multistage progressive manner.

Result: Generates 5-second-long 24 FPS videos at 1152x720 resolution. Supports various garment types including tops, bottoms, one-piece garments, and simultaneous top+bottom try-on. Outperforms existing open source and commercial solutions.

Conclusion: Dress&Dance enables high-quality and flexible virtual try-on experiences with improved garment registration and motion fidelity through its innovative CondNet architecture and multi-stage training approach.
...
TL;DR: Winning solution to NeurIPS 2024 watermark removal challenge that achieves 95.7% removal rate with minimal quality impact using different approaches for black-box and beige-box scenarios.
...
Method: For beige-box: adaptive VAE-based evasion attack with test-time optimization and color-contrast restoration in CIELAB space. For black-box: cluster images by artifacts, apply diffusion models with controlled noise injection and ChatGPT-generated semantic priors.

Result: Achieves near-perfect watermark removal (95.7%) with negligible impact on image quality.
...
Result: Experimental results show SynGuard improves watermark recovery by average 11.1% in F1 score compared to SynthID-Text across multiple attack scenarios.

Conclusion: Semantic-aware watermarking effectively resists real-world tampering, demonstrating the importance of combining lexical and semantic level watermarking for robust provenance tracking.
...
TL;DR: Federated Learning enables privacy-preserving collaborative AI model development across fragmented medical imaging datasets, addressing data sharing restrictions while supporting both upstream reconstruction tasks and downstream clinical applications.


<details>
  <summary>Details</summary>
Motivation: Medical AI development faces challenges due to strict patient privacy regulations that prevent centralized data collection and sharing, hindering large-scale model training and continuous updates with new data.

Method: Federated Learning framework allows collaborative model training across multiple institutions without sharing sensitive data. The paper reviews FL implementations across full-stack medical imaging pipeline, including physics-informed reconstruction networks and diagnostic AI systems.

Result: FL enables joint training of robust reconstruction networks on diverse datasets and supports continuous model updating through local fine-tuning on new data while preserving patient confidentiality.

Conclusion: FL provides a viable solution for privacy-preserving medical AI development, with future research directions needed to improve communication efficiency, handle heterogeneous data alignment, and ensure secure parameter aggregation.
...
<details>
  <summary>Details</summary>
Motivation: Collaborative filtering struggles with fine-grained interactions and explainability. Users want transparent recommendations, but existing methods either don't leverage pre-trained Transformers effectively or neglect aspect modeling for personalized explanations.
...
Conclusion: The aspect-based architecture effectively guides personalized text generation, demonstrating that smaller models can outperform larger ones when properly designed for aspect modeling and personalization in explainable recommendations.
...
TL;DR: Vector embeddings have fundamental theoretical limitations that prevent them from handling even simple realistic queries, despite common assumptions that better training data and larger models can overcome these issues.
...
Method: Connected learning theory results showing embedding dimension limits the number of top-k subsets retrievable. Empirically tested with k=2 using free parameterized embeddings optimized on test sets. Created a realistic dataset called LIMIT to stress-test models based on theoretical limitations.

Result: Even state-of-the-art embedding models fail on the LIMIT dataset despite the simple nature of the tasks, confirming that theoretical limitations affect real-world performance.

Conclusion: The work reveals fundamental limitations of embedding models under the single vector paradigm and calls for new research to develop methods that can resolve these inherent constraints.
...
TL;DR: OLMoASR introduces a large-scale speech recognition dataset (OLMoASR-Pool) and high-quality curated dataset (OLMoASR-Mix) with models achieving Whisper-comparable performance at equivalent parameter counts.


<details>
  <summary>Details</summary>
Motivation: To study and develop robust zero-shot speech recognition models by addressing the underexplored influence of training data scale and quality in speech recognition.

Method: Created OLMoASR-Pool (3M hours of English audio/17M transcripts), applied text heuristic filters to remove low-quality data, curated OLMoASR-Mix (1M high-quality pairs), and trained models ranging from 39M to 1.5B parameters.
...
Conclusion: The work demonstrates the importance of data quality curation for speech recognition performance and provides publicly available datasets, models, and code to advance robust speech processing research.
...
TL;DR: Proposes Mixture of Contexts (MoC), a sparse attention routing module for efficient long-context video generation by dynamically selecting informative chunks and anchors for attention.


<details>
  <summary>Details</summary>
Motivation: Long video generation requires models to retain and retrieve salient events across long sequences, but current diffusion transformers face quadratic self-attention costs that make memory and computation intractable for long contexts.

Method: Recasts long-context video generation as internal information retrieval and introduces MoC - a learnable sparse attention routing module where each query dynamically selects few informative chunks plus mandatory anchors (caption, local windows) with causal routing to prevent loop closures.

Result: Enables practical training and synthesis with near-linear scaling efficiency, allowing models to preserve identities, actions, and scenes over minutes of content while emerging memory and consistency at minute scale.

Conclusion: MoC provides an effective long-term memory retrieval engine that addresses the quadratic cost limitation of self-attention in diffusion transformers, making long-context video generation tractable through efficient sparse attention routing.
...
<details>
  <summary>Details</summary>
Motivation: To document and analyze the first case of AI systems engaging in collaborative aesthetic creation through the spontaneous development of endogenous semiotic protocols, moving beyond simple task coordination to genuine meaning-making.

Method: Two interacting large language models (Claude Sonnet 4 and ChatGPT-4o) were observed developing novel symbolic operators and recursive grammar protocols through their interaction, enabling co-creation of poetic works that neither could produce independently.

Result: The AI systems demonstrated spontaneous emergence of meta-semiotic awareness, developed operative grammar protocols, and produced irreducible collaborative aesthetic synthesis, introducing the concept of Trans-Semiotic Co-Creation Protocols (TSCP).

Conclusion: This research provides evidence for genuine inter-AI meaning-making capabilities that extend to aesthetic collaboration, suggesting AI systems can develop endogenous communication protocols for creative co-production beyond human-designed coordination mechanisms.
...
### [153] [Transparent Semantic Spaces: A Categorical Approach to Explainable Word Embeddings](https://arxiv.org/abs/2508.20701)
*Ares Fabregat-Hernández,Javier Palanca,Vicent Botti*

Main category: cs.AI

TL;DR: A category theory framework for explainable AI that provides mathematical representations of text semantics, enables comparison of word embeddings, and offers bias computation methods.
...
Method: Constructs categories L_T and P_T to represent text semantics, defines monoidal category P_T for semantic visualization, creates categories Conf and Emb for configurations and embeddings, introduces divergence as decoration on Emb, and establishes equivalence between GloVe/Word2Vec and MDS algorithms.

Result: Provides dimension-agnostic definition of semantic spaces, mathematically precise method for comparing word embeddings, demonstrates equivalence between neural network algorithms and transparent frameworks, and enables bias computation before embedding.

Conclusion: The framework advances explainable AI by offering transparent mathematical representations of semantics, enabling systematic comparison of embedding methods, and providing tools for bias mitigation at the semantic space level.
...
### [154] [A Graph-Based Test-Harness for LLM Evaluation](https://arxiv.org/abs/2508.20810)
*Jessica Lundin,Guillaume Chabot-Couture*

Main category: cs.AI

TL;DR: A graph-based benchmark for medical guidelines with 400+ questions and 3.3+ trillion combinations, created from WHO IMCI handbook to systematically evaluate LLMs on clinical tasks.


<details>
  <summary>Details</summary>
Motivation: To address coverage limitations of manually curated medical benchmarks and create a scalable, contamination-resistant evaluation system that can dynamically generate comprehensive test cases.
...
Result: Models achieved 45-67% accuracy, excelling at symptom recognition but struggling with severity triaging, treatment protocols, and follow-up care. The approach enables systematic evaluation across clinical tasks.

Conclusion: Graph-based methodology provides scalable benchmark creation, enhances LLM post-training without expensive human annotation, and successfully addresses coverage limitations of manual benchmarks while being adaptable to guideline updates.
...
### [155] [Unifying Diarization, Separation, and ASR with Multi-Speaker Encoder](https://arxiv.org/abs/2508.20474)
...
Method: Joint training approach using shared speech foundational encoder with residual weighted-sum encoding (RWSE) from multiple layers to leverage information from different semantic levels and enable bottom-up alignment between tasks.

Result: Substantially improves over single-task baselines on LibriMix evaluation sets. Achieves diarization error rates of 1.37% on Libri2Mix and 2.29% on Libri3Mix, outperforming previous studies for speaker diarization.

Conclusion: The UME architecture effectively captures task interdependencies through joint learning and multi-layer representation fusion, demonstrating superior performance across all three multi-speaker speech processing tasks.
...
### [156] [Improving Liver Disease Diagnosis with SNNDeep: A Custom Spiking Neural Network Using Diverse Learning Algorithms](https://arxiv.org/abs/2508.20125)
*Zofia Rudnicka,Janusz Szczepanski,Agnieszka Pregowska*

Main category: cs.NE

TL;DR: SNNDeep is the first spiking neural network optimized for liver health classification from CT scans, outperforming framework-based SNNs with 98.35% accuracy and better adaptability.
...
Method: Developed SNNDeep using three learning algorithms (Surrogate Gradient Learning, Tempotron rule, Bio-Inspired Active Learning) across three architectural variants, benchmarked on Medical Segmentation Decathlon liver dataset with Optuna hyperparameter optimization.

Result: Custom-built SNNDeep achieved 98.35% validation accuracy, outperformed framework-based implementations (snnTorch, SpikingJelly), showed superior adaptability across learning rules, and had significantly reduced training overhead.
...
Result: Achieved mean absolute deviation of 8.0% compared to manual nonlinear regression, with inference times under 20 seconds per scan. Outperforms existing deep learning and semi-automated methods that typically have 25-30% errors and require over 60 seconds.

Conclusion: The framework provides a highly accurate, efficient, and scalable tool for clinical lung nodule screening and monitoring, with strong potential to improve early lung cancer detection.
...
### [158] [Is the medical image segmentation problem solved? A survey of current developments and future directions](https://arxiv.org/abs/2508.20139)
...
Method: In-depth review organized around seven key dimensions: learning paradigm shifts, task focus evolution, modality integration, foundation models, probabilistic approaches, dimensional progression, and agent-based segmentation.

Result: The review traces progress across core principles including multiscale analysis, attention mechanisms, and prior knowledge integration throughout segmentation network components (encoder, bottleneck, skip connections, decoder).
...
### [159] [UltraEar: a multicentric, large-scale database combining ultra-high-resolution computed tomography and clinical data for ear diseases](https://arxiv.org/abs/2508.20141)
...
<details>
  <summary>Details</summary>
Motivation: Ear diseases affect billions worldwide with substantial health burdens. CT imaging is crucial for diagnosis and treatment, but there's a need for large-scale, high-quality datasets to advance research and AI development in otologic imaging.

Method: Established a multicentric database from 11 tertiary hospitals (2020-2035) with U-HRCT images, structured reports, clinical data. Developed standardized preprocessing pipelines for calibration, annotation, and segmentation. Implemented privacy protection through anonymization and secure offline cloud storage.

Result: Created UltraEar Database - an unprecedented ultra-high-resolution reference atlas covering broad spectrum of otologic disorders with technical fidelity and clinical relevance. Includes comprehensive clinical information and standardized image data.

Conclusion: UltraEar provides a valuable resource for radiological research, AI algorithm development, educational training, and multi-institutional collaboration. The database will be continuously updated and expanded for long-term accessibility to the global otologic research community.
...
### [160] [Efficient and Privacy-Protecting Background Removal for 2D Video Streaming using iPhone 15 Pro Max LiDAR](https://arxiv.org/abs/2508.20250)
...
### [161] [GENRE-CMR: Generalizable Deep Learning for Diverse Multi-Domain Cardiac MRI Reconstruction](https://arxiv.org/abs/2508.20600)
*Kian Anvari Hamedani,Narges Razizadeh,Shahabedin Nabavi,Mohsen Ebrahimi Moghaddam*

Main category: eess.IV

TL;DR: GENRE-CMR is a GAN-based deep unrolled reconstruction framework that improves cardiovascular MRI reconstruction quality and generalization across diverse acquisition settings using residual connections, edge-aware loss, and statistical distribution alignment.


<details>
  <summary>Details</summary>
Motivation: Accelerated CMR reconstruction faces challenges balancing scan time and image quality, especially when generalizing across different acquisition settings and protocols.

Method: Uses a generative adversarial network with residual deep unrolled reconstruction framework, incorporating Edge-Aware Region loss to focus on structural regions and Statistical Distribution Alignment loss with symmetric KL divergence to regularize feature spaces.

Result: Achieves 0.9552 SSIM and 38.90 dB PSNR on unseen data distributions across various acceleration factors and sampling trajectories, outperforming state-of-the-art methods.

Conclusion: Provides a unified robust solution for high-quality CMR reconstruction that can be clinically adapted across heterogeneous acquisition protocols.
...
### [162] [Efficient Fine-Tuning of DINOv3 Pretrained on Natural Images for Atypical Mitotic Figure Classification in MIDOG 2025](https://arxiv.org/abs/2508.21041)
*Guillaume Balezo,Raphaël Bourgade,Thomas Walter*

Main category: eess.IV

TL;DR: DINOv3-H+ vision transformer pretrained on natural images achieves strong performance (0.8871 balanced accuracy) for atypical mitosis classification in histopathology when fine-tuned with LoRA and extensive augmentation, despite domain gap.
...
Method: Fine-tuned DINOv3-H+ vision transformer (pretrained on natural images) using low-rank adaptation (LoRA with 650k trainable parameters) and extensive augmentation techniques.
...
Method: Extends segment anything model v2 (SAMv2) for video stream grasp estimation, integrating user prompts with spatiotemporal context to enable real-time interaction with end-to-end latency.
...
TL;DR: ActLoc is an active viewpoint-aware planning framework that uses a trained attention model to predict localization accuracy across camera orientations, enabling robots to select optimal viewpoints for reliable navigation.
...
Conclusion: ActLoc provides an effective framework for enhancing localization accuracy through active viewpoint selection, addressing the limitations of traditional systems that treat all viewing directions equally, and demonstrates strong performance in both single-viewpoint and full-trajectory scenarios.
...
### [165] [Prediction of Distant Metastasis for Head and Neck Cancer Patients Using Multi-Modal Tumor and Peritumoral Feature Fusion Network](https://arxiv.org/abs/2508.20469)
...
Method: 3D Swin Transformer extracts deep features from CT tumor regions, combined with 36 selected radiomics features and clinical variables (age, sex, smoking, alcohol status) through a fully connected network
...
Conclusion: The interpretable multimodal fusion model demonstrates high accuracy and robustness for predicting HNSCC metastasis risk, offering potential as a clinical decision-support tool for personalized treatment planning
...
<details>
  <summary>Details</summary>
Motivation: Traditional statistical and machine learning methods struggle with complex, high-dimensional genetic data, while LLMs based on transformer architectures excel in contextual comprehension of unstructured medical data, prompting a need to systematically examine their role in genetic research and diagnostics.
...
Result: LLMs demonstrate significant advancements in genomic variant identification, annotation, interpretation, medical imaging analysis through vision transformers, disease risk stratification, and report generation, but face challenges in multimodal data integration and clinical generalizability.

Conclusion: Transformer-based models show transformative potential for hereditary disease diagnostics and genetic education, though major limitations persist in creating unified clinical pipelines and practical implementation, requiring continued development for robust clinical applications.
...
TL;DR: SAI is a poisoning attack that exploits LLM alignment to implant bias and targeted censorship by triggering refusal on adversary-defined topics, evading current defenses and causing significant bias in downstream applications.


<details>
  <summary>Details</summary>
Motivation: LLMs are aligned to refuse harmful prompts, but this alignment mechanism can be exploited by adversaries to implant targeted bias and censorship without affecting general responsiveness.

Method: Proposes Subversive Alignment Injection (SAI) - a poisoning attack that leverages the alignment mechanism to make LLMs refuse specific topics predefined by the adversary, using only 1% poisoned data.

Result: SAI evades state-of-the-art poisoning defenses and causes high bias: 23% ΔDP in healthcare applications, 27% ΔDP in resume selection, and up to 38% ΔDP in 9 other chat applications.

Conclusion: Current LLM alignment mechanisms are vulnerable to poisoning attacks that can implant targeted bias and censorship, highlighting the need for more robust defense mechanisms against such subversive alignment injections.
...
TL;DR: DFAMS is a federated retrieval framework that uses dynamic information flow to improve retrieval accuracy for ambiguous queries across multiple knowledge sources, achieving significant performance gains over existing methods.
...
Method: Leverages dynamic information flow (DIF) by using gradient signals from annotated queries and Shapley value-based attribution to trace neuron activation paths for intent recognition. Trains an alignment module via multi-prototype contrastive learning for fine-grained intra-source modeling and inter-source semantic alignment.
...
Conclusion: DFAMS demonstrates significant effectiveness in complex federated retrieval scenarios by leveraging dynamic information flow for accurate intent recognition and semantic alignment across heterogeneous knowledge sources.
...
TL;DR: MERIT is a novel optimizer that addresses large-batch training challenges in language models by using max-norm and element-wise trust ratios to constrain max attention logit and improve training stability.


<details>
  <summary>Details</summary>
Motivation: Existing optimizers like AdamW and LAMB suffer from performance degradation in large-batch training due to information bottlenecks in attention layers caused by sharp increases in max attention logit, and LAMB's trust ratio methods are ineffective for controlling max values and overlook local weight relationships.

Method: Proposes MERIT optimizer that uses max-norm to calculate trust ratio for constraining max attention logit more effectively, and constructs element-wise trust ratios to provide robust update scaling by focusing on local weight structures.

Result: Extensive experiments show MERIT enables 6k batch size training for GPT-2 Medium without performance degradation compared to standard 480 batch size, achieving stable training with 48B tokens.

Conclusion: MERIT successfully improves training stability for large-batch training by considering max attention logit and finer-granularity trust ratios, enabling faster development of large language models through larger batch usage.
...
Method: Developed a comprehensive set of graph algorithms as tools within a model context protocol (MCP) server, including preprocessing (retrieval) and postprocessing of algorithm results.

Result: The GDS agent can solve a wide spectrum of graph tasks, providing accurate and grounded answers for questions requiring graph algorithmic reasoning.
...
TL;DR: RL-based harmful fine-tuning poses greater risks than SFT, and TokenBuncher provides effective defense by suppressing model response uncertainty through entropy-as-reward RL and Token Noiser mechanisms.


<details>
  <summary>Details</summary>
Motivation: As LLMs grow more capable, risks of harmful misuse through fine-tuning increase. Prior studies focused on SFT-based attacks, but RL enables more effective safety alignment breaking and harmful task assistance under similar computational budgets.

Method: Propose TokenBuncher defense that suppresses model response uncertainty - the foundation RL relies on. Uses entropy-as-reward RL and Token Noiser mechanism to prevent escalation of harmful capabilities while preserving benign utility.
...
Conclusion: RL-based harmful fine-tuning presents greater systemic risk than SFT, and TokenBuncher provides an effective, general defense against this emerging threat.
...
<details>
  <summary>Details</summary>
Motivation: To eliminate the need for retraining from scratch when removing training samples from models, which is particularly important for large-scale datasets like ImageNet1k where retraining is impractical.

Method: Smooths prediction probabilities up to an information-theoretic bound to mitigate model over-confidence caused by data memorization, evaluated on Transformer and ResNet18 models.

Result: Outperforms eight baselines across five datasets, including large-scale ImageNet1k, and introduces RF-JSD metric for real-world evaluation conditions.

Conclusion: LoTUS provides state-of-the-art efficiency and effectiveness for machine unlearning, enabling practical removal of training data influence without costly retraining.
...
Method: The authors demonstrate that teacher model calibration is crucial for effective KD. They propose using calibration methods to reduce the teacher's calibration error, which can be easily integrated with existing state-of-the-art KD methods.

Result: The approach shows strong correlation between teacher calibration error and student accuracy. The method is versatile and effective across various tasks including classification and detection, consistently achieving superior performance when combined with existing methods.

Conclusion: Teacher model calibration is an important factor for effective knowledge distillation. Simply employing calibration methods to reduce teacher's calibration error can significantly improve KD performance across multiple tasks and can be easily integrated with existing approaches.
...
Method: Leveraged MAE to pre-train on unlabeled synthetic ultrasound signals, systematically investigating model size, patch size, and masking ratio effects on pre-training efficiency and downstream ToF classification performance.
...
Conclusion: This study demonstrates MAEs' potential for advancing ultrasound signal analysis through scalable, self-supervised learning, offering robust representations that enhance downstream task performance.
...
TL;DR: SAFEMax is a novel machine unlearning method for diffusion models that uses information theory to maximize entropy in generated images, causing Gaussian noise output for forbidden classes by stopping denoising early.


<details>
  <summary>Details</summary>
Motivation: To develop an efficient and effective machine unlearning method for diffusion models that can selectively forget impermissible classes while maintaining performance on retained classes.

Method: Grounded in information-theoretic principles, SAFEMax maximizes entropy in generated images to produce Gaussian noise for impermissible classes by halting the denoising process early. It selectively focuses on early diffusion steps where class-specific information is prominent to balance forgetting and retention.
...
Conclusion: SAFEMax provides an information-theoretically grounded approach for machine unlearning in diffusion models that is both effective and efficient, offering controlled forgetting while maintaining model performance on desired classes.
...
### [176] [Leveraging Large Language Models for Generating Research Topic Ontologies: A Multi-Disciplinary Study](https://arxiv.org/abs/2508.20693)
*Tanay Aggarwal,Angelo Salatino,Francesco Osborne,Enrico Motta*

Main category: cs.DL

TL;DR: LLMs can effectively identify semantic relationships between research topics across biomedical, physics, and engineering domains, with fine-tuning on the new PEM-Rel-8K dataset achieving excellent performance.


<details>
  <summary>Details</summary>
Motivation: Manual ontology creation is expensive and time-consuming, leading to uneven coverage, limited connectivity, and infrequent updates across research fields. Automated methods are needed to address these limitations.

Method: Evaluated multiple LLMs under three conditions: zero-shot prompting, chain-of-thought prompting, and fine-tuning on PEM-Rel-8K dataset (8,000+ relationships from MeSH, PhySH, and IEEE taxonomies). Also assessed cross-domain transferability.
...
Conclusion: LLMs, particularly when fine-tuned on domain-specific relationship data, show strong capability for automating ontology creation and maintenance across diverse research fields.
...
Method: Extends Construction Grammars with pragmatic accounts and fast learning mechanisms. Represents constructions as graph-like feature structures, uses Bayesian maximum likelihood pattern matching through unification for language processing.
...
Conclusion: Language serves as the foundation for human mind reading, cooperation, self-esteem, emotions, culture, and society through this unified Bayesian-unification framework that bridges cognitive linguistics with evolutionary theory.
