...
TL;DR: GRPO-based tool-integrated RL suffers from training collapse due to Lazy Likelihood Displacement (LLD), which triggers a self-reinforcing death spiral. The paper proposes LLDS regularization to stabilize training and achieves significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Tool-integrated RL enables LLMs to perform multi-step reasoning with external tools, and GRPO offers fast convergence without value functions. However, GRPO consistently suffers from training collapse, limiting its practical application.

Method: Identifies Lazy Likelihood Displacement (LLD) as the core failure mechanism, then proposes LLDS - a lightweight likelihood-preserving regularization that activates only when trajectory likelihood decreases and regularizes only responsible tokens.

Result: Method stabilizes training, prevents gradient explosion, and yields substantial improvements: +37.8% gains on Qwen2.5-3B and +32.0% gains on Qwen2.5-7B across seven open-domain and multi-hop QA benchmarks.
...
TL;DR: Multinomial Naive Bayes achieved 85.89% accuracy for Libyan dialect classification using word and character n-grams, outperforming other classifiers on Twitter data.


<details>
  <summary>Details</summary>
Motivation: To develop effective methods for classifying Libyan dialect utterances from Twitter, addressing challenges of orthographic variations and non-standard spellings in Arabic dialects.

Method: Used QADI corpus (540k sentences across 18 dialects), applied chi-square feature selection to exclude non-significant features, tested logistic regression, linear SVM, multinomial Naive Bayes, and Bernoulli Naive Bayes with different word/character n-gram representations.

Result: Multinomial Naive Bayes achieved highest accuracy (85.89%) and F1-score (0.85741) with (1,2) word n-gram and (1,5) character n-gram. Logistic Regression and Linear SVM showed slightly lower performance (84.41% and 84.73% respectively).

Conclusion: Carefully selected n-gram representations and classification models significantly improve Libyan dialect identification accuracy, with MNB performing best. Provides empirical benchmarks for Arabic dialect NLP research.
...
TL;DR: SQuARE is a hybrid retrieval framework for spreadsheet QA that routes queries based on sheet complexity, using either structure-preserving chunk retrieval or SQL over automatically constructed relational views, with an agent supervising the process.


<details>
  <summary>Details</summary>
Motivation: Spreadsheet QA is challenging due to complex structures like multi-row headers, merged cells, and unit annotations that disrupt naive chunking, while SQL views fail on files with inconsistent schemas.

Method: Hybrid retrieval framework with sheet-level, complexity-aware routing. Computes continuous score based on header depth and merge density, then routes queries through either structure-preserving chunk retrieval or SQL over automatically constructed relational representation. Lightweight agent supervises retrieval, refinement, or combination of results when confidence is low.

Result: Consistently surpasses single-strategy baselines and ChatGPT-4o on both retrieval precision and end-to-end answer accuracy across multi-header corporate balance sheets, heavily merged World Bank workbook, and diverse public datasets, while maintaining predictable latency.

Conclusion: SQuARE maintains header hierarchies, time labels, and units, ensuring faithful values to original cells. By decoupling retrieval from model choice, it's compatible with emerging tabular foundation models and offers a practical bridge toward more robust table understanding.
...
<details>
  <summary>Details</summary>
Motivation: Real enterprise data workflows involve both data engineering (transforming raw data into analytical tables) and data analysis (converting tables into insights). Existing benchmarks don't adequately capture these complex, multi-stage workflows, making it hard to evaluate autonomous data agents for enterprise settings.

Method: DAComp includes 210 tasks mirroring real enterprise workflows. Data engineering tasks require repository-level engineering on industrial schemas, including designing multi-stage SQL pipelines from scratch and evolving existing systems. Data analysis tasks pose open-ended business problems requiring strategic planning, exploratory analysis, and actionable recommendations. Engineering tasks use execution-based multi-metric evaluation, while open-ended tasks use an LLM-judge guided by hierarchical rubrics.

Result: State-of-the-art agents perform poorly on DAComp: DE tasks have success rates under 20%, exposing critical bottlenecks in holistic pipeline orchestration. DA tasks average below 40%, highlighting deficiencies in open-ended reasoning. The benchmark shows engineering and analysis are distinct capabilities that current agents struggle with.

Conclusion: DAComp provides a rigorous, realistic testbed for developing truly capable autonomous data agents. It clearly diagnoses limitations in current systems, particularly in holistic pipeline orchestration and open-ended reasoning, and will drive development of better agents for enterprise data intelligence workflows.
...
TL;DR: ClusterFusion: A hybrid framework using LLMs as the core clustering mechanism guided by lightweight embeddings, achieving SOTA performance on standard and domain-specific tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional clustering with pre-trained embeddings struggles in domain-specific contexts without costly fine-tuning, while LLMs offer strong contextual reasoning but are underutilized as main clustering components.

Method: Three-stage hybrid framework: 1) embedding-guided subset partition, 2) LLM-driven topic summarization, and 3) LLM-based topic assignment. Uses LLM as clustering core guided by lightweight embeddings.

Result: Achieves state-of-the-art performance on three public benchmarks and shows substantial gains on two new domain-specific datasets. Releases newly constructed dataset and benchmark results.

Conclusion: ClusterFusion effectively leverages LLMs' contextual adaptability for clustering tasks, enabling direct incorporation of domain knowledge and user preferences while outperforming existing methods.
...
TL;DR: LangSAT is an RL-enhanced framework that converts English descriptions to CNF and solves SAT problems using an RL-based CDCL solver, making SAT-solving more accessible without requiring CNF input.


<details>
  <summary>Details</summary>
Motivation: Existing SAT-solving platforms require CNF as input, which limits accessibility. The authors aim to bridge the gap between natural language inputs and propositional logic, making SAT-solving more accessible to users who can describe problems in English rather than formal logic.

Method: The framework has two components: Lang2Logic translates English sentences into CNF expressions, and SmartSAT is an RL-based SAT solver that encodes clause-variable relationships as structured graph representations and extracts global SAT-specific features to provide contextual information to the RL agent.

Result: Lang2Logic successfully processed natural language inputs up to 450 words, and SmartSAT demonstrated comparable performance to traditional CDCL heuristics in solving time. The combined framework offers an accessible and scalable solution for SAT-solving tasks.

Conclusion: LangSAT provides a novel RL-based framework that makes SAT-solving more accessible by accepting natural language inputs while maintaining competitive solving performance, with applications in reasoning, formal verification, and debugging.
...
TL;DR: MASE framework provides model-agnostic local explanations for text-based predictive models using embedding layer perturbations instead of raw word inputs.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks in NLP lack interpretability, and traditional post-hoc interpretation methods are not well-suited for discrete word data.

Method: Model-agnostic Saliency Estimation (MASE) framework using Normalized Linear Gaussian Perturbations (NLGP) on embedding layers to estimate input saliency without requiring model architecture knowledge.

Result: MASE outperforms other model-agnostic interpretation methods, particularly in Delta Accuracy, making it effective for explaining text-based models.

Conclusion: MASE is a promising tool for elucidating the decision-making processes of text-based models in NLP by providing local explanations without model-specific knowledge.
...
TL;DR: Classical ML with feature engineering achieves ~0.57 F1-score for sarcasm detection on Reddit comments without neural networks or conversational context.


<details>
  <summary>Details</summary>
Motivation: Sarcasm is prevalent in online discussions but challenging for machines to detect due to contradictory literal vs. intended meanings. The paper aims to establish a clear, reproducible baseline using classical ML methods without relying on neural networks or contextual information from parent comments.

Method: Uses a 100,000-comment subsample from SARC 2.0 dataset. Combines word-level and character-level TF-IDF features with simple stylistic indicators. Evaluates four classical ML models: logistic regression, linear SVM, multinomial Naive Bayes, and random forest.

Result: Naive Bayes and logistic regression perform strongest, achieving F1-scores around 0.57 for sarcastic comments. The lack of conversational context limits overall performance, but the approach provides a lightweight, interpretable baseline.

Conclusion: Classical machine learning with explicit feature engineering offers a clear, reproducible baseline for sarcasm detection. While performance is limited without conversational context, the approach demonstrates the value of lightweight, interpretable methods as a foundation for more complex systems.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM unlearning methods are problematic: retraining is too costly, approximate methods are unstable, and performance degrades when forget sets are small or imbalanced. There's a need for efficient, stable unlearning that preserves general knowledge.

Method: Two-stage framework: 1) Fast estimation module computes per-sample influence scores, 2) Maps these scores into adaptive update weights that guide selective parameter updates. This enables targeted forgetting of harmful behavior while minimizing impact on general knowledge.

Result: On Mistral-7B and Llama-3-8B across Dolly-15k and Alpaca-57k datasets: Achieves up to 100x higher efficiency than full retraining, consistently outperforms Fisher, GA, and LoReUn methods on both in-distribution and out-of-distribution forgetting tasks.

Conclusion: Influence-guided parameter reweighting establishes a scalable and interpretable paradigm for LLM unlearning, offering an efficient solution to the challenge of removing specific data influences while maintaining model utility.
...
TL;DR: MSME is a multi-stage, multi-expert framework for zero-shot stance detection that addresses complex real-world challenges by integrating knowledge retrieval, specialized reasoning modules, and decision aggregation.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based approaches struggle with complex real-world stance detection where dynamic background knowledge is needed, targets involve compound entities/events requiring explicit linking to labels, and rhetorical devices like irony obscure author intent.

Method: MSME uses a three-stage framework: (1) Knowledge Preparation for retrieving background knowledge and clarifying stance labels, (2) Expert Reasoning with three specialized modules (Knowledge Expert, Label Expert, Pragmatic Expert), and (3) Decision Aggregation where a Meta-Judge integrates all expert analyses.

Result: Experiments on three public datasets show that MSME achieves state-of-the-art performance across all datasets.

Conclusion: The MSME framework effectively addresses complex challenges in zero-shot stance detection by leveraging multi-stage, multi-expert reasoning, demonstrating superior performance over existing approaches.
...
TL;DR: The paper presents methods for extracting chemotherapy timelines from clinical notes using LLMs, achieving best performance with fine-tuned Qwen3-14B (score 0.678) in the ChemoTimelines shared task.


<details>
  <summary>Details</summary>
Motivation: To benchmark and develop methods for constructing accurate timelines of systemic anticancer treatment from electronic health records, specifically addressing the challenge of extracting chemotherapy events from unstructured clinical notes.

Method: Two-step workflow: 1) LLM extracts chemotherapy events from individual clinical notes, 2) algorithm normalizes and aggregates events into patient-level timelines. Evaluated multiple strategies including chain-of-thought thinking, supervised fine-tuning, direct preference optimization, and dictionary-based lookup.

Result: Multiple approaches achieved competitive performance, with fine-tuned Qwen3-14B achieving the best official score of 0.678 on the test set leaderboard.

Conclusion: The results provide useful insights for future attempts on chemotherapy timeline extraction tasks and the design of similar clinical NLP tasks, demonstrating the effectiveness of LLM-based approaches with appropriate training strategies.
...
TL;DR: Proposes LF-Edit, a new lifelong free-text knowledge editing task for LLMs, with MRLF-Bench benchmark and EvoEdit method using latent perturbation augmentation and knowledge-driven parameter fusion.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge editing methods have two key limitations: 1) they rely on structured triplets misaligned with LLMs' free-text pretraining nature, and 2) they typically support only one-time updates rather than sequential/lifelong editing.

Method: Introduces EvoEdit approach with two main components: 1) Latent Perturbation Augmentation to enhance knowledge injection, and 2) Knowledge-driven Parameter Fusion to preserve prior information during continual editing.

Result: EvoEdit substantially outperforms existing knowledge editing methods on the proposed LF-Edit task, as demonstrated through experiments on the new MRLF-Bench benchmark containing 16,835 free-text edit requests.

Conclusion: The paper addresses critical gaps in knowledge editing by introducing a more natural free-text lifelong editing paradigm, providing both a comprehensive benchmark and an effective method that better aligns with how LLMs are trained and used in practice.
...
TL;DR: AdmTree is a novel hierarchical context compression framework for LLMs that uses adaptive segmentation and gist tokens in a semantic binary tree structure to efficiently preserve semantic information in long contexts while minimizing trainable parameters.


<details>
  <summary>Details</summary>
Motivation: The quadratic complexity of self-attention in LLMs limits their ability to process long contexts, which is essential for advanced applications. Existing context compression methods have trade-offs: explicit methods lose local details, while implicit methods suffer from positional biases, information degradation, or poor long-range dependency capture.

Method: AdmTree dynamically segments input based on information density and uses gist tokens to summarize variable-length segments as leaves of a semantic binary tree. It employs a lightweight aggregation mechanism with a frozen backbone LLM to minimize trainable parameters, enabling efficient hierarchical abstraction of context while preserving both fine-grained details and global semantic coherence.

Result: AdmTree robustly retains semantic information of long contexts by preserving fine-grained details alongside global semantic coherence, mitigating positional bias, and dynamically adapting to content through its hierarchical compression approach.

Conclusion: AdmTree provides an effective solution for context compression in LLMs that addresses limitations of existing approaches by combining adaptive segmentation, hierarchical abstraction, and minimal parameter training to maintain semantic fidelity while improving efficiency for long-context processing.
...
TL;DR: ADAPT is a meta-learning algorithm that learns optimal task sampling proportions for multi-task instruction tuning under token budget constraints, outperforming static mixtures while using fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Traditional multi-task instruction tuning uses fixed task weights, which may not be optimal under token budget constraints. The paper aims to develop an adaptive approach that learns task sampling proportions to maximize performance while respecting budget limitations.

Method: ADAPT maintains a continuous distribution over tasks and updates it via meta-gradients of a smooth worst-case validation objective. This creates an adaptive curriculum that allocates more tokens to useful tasks while avoiding collapse. The method is tested on three ~1B-parameter LLMs trained on 20 Natural Instructions task types under different token budgets.

Result: ADAPT matches or slightly improves average downstream performance relative to the best static mixture while using fewer effective training tokens. It reallocates budget toward harder, benchmark-aligned tasks across 11 out-of-domain benchmarks spanning reasoning, reading comprehension, code generation, and instruction following.

Conclusion: ADAPT provides an effective meta-learning approach for adaptive task sampling in multi-task instruction tuning, offering better token efficiency and performance compared to static mixing strategies under budget constraints.
...
TL;DR: LexGenius is a Chinese legal benchmark for evaluating legal general intelligence in LLMs, using a Dimension-Task-Ability framework with multiple-choice questions from recent cases and exams.
...
Method: Created LexGenius benchmark with Dimension-Task-Ability framework covering 7 dimensions, 11 tasks, and 20 abilities. Used recent legal cases and exam questions to create multiple-choice questions with manual and LLM reviews to reduce data leakage risks.
...
Conclusion: LexGenius can effectively assess legal intelligence abilities of LLMs and enhance legal GI development, providing a systematic benchmark for evaluating legal expertise in AI systems.
...
TL;DR: Empirical analysis of generic masculines in German press texts reveals lexeme-specific patterns, predominant use in plural/indefinite contexts, and challenges previous assumptions about their usage.


<details>
  <summary>Details</summary>
Motivation: To address the gap in corpus-based analyses of generic masculines (GM) in German, as existing research shows conflicting views on their gender-neutrality and psycholinguistic studies suggest male bias, but empirical data on actual usage patterns is scarce.

Method: Manual annotation of the complete inflectional paradigm of 21 personal nouns in a large corpus of contemporary German press texts, resulting in 6,195 annotated tokens to analyze lexeme-specific differences and grammatical patterns.
...
Conclusion: The study provides empirical evidence for nuanced understanding of GM usage in authentic written language, offering a foundation for better alignment of linguistic stimuli in psycholinguistic research with real-world language patterns.
...
TL;DR: OsmT is an open-source tag-aware language model that bridges natural language and OverpassQL for OpenStreetMap data, using tag retrieval augmentation and reverse translation tasks.
...
Method: OsmT introduces a Tag Retrieval Augmentation (TRA) mechanism that incorporates contextually relevant tag knowledge to capture hierarchical and relational dependencies in OSM data. It also defines a reverse OverpassQL-to-Text task for query interpretation.
...
Conclusion: Open-source pre-trained language models can effectively bridge natural language and structured query languages in schema-rich geospatial environments, offering transparency, adaptability, and lightweight deployment advantages over closed-source alternatives.
...
TL;DR: SignRoundV2 is a post-training quantization framework that achieves production-grade LLM performance at 4-5 bits with only ~1% accuracy variance, and maintains strong results even at 2 bits.


<details>
  <summary>Details</summary>
Motivation: Extreme low-bit quantization (2-4 bits) is essential for efficient LLM deployment but causes severe performance degradation with existing methods like MXFP4. There's a need for quantization techniques that maintain accuracy at very low bit-widths without requiring mixed-precision approaches.

Method: Two key components: (1) A fast sensitivity metric combining gradient information with quantization-induced deviations to guide layer-wise bit allocation, and (2) A lightweight pre-tuning search for quantization scales to improve extremely low-bit quantization performance.

Result: SignRoundV2 closes the gap with full-precision models, sustaining competitive accuracy for LLMs with about 1% variance at 4-5 bits and strong results even at 2 bits, achieving production-grade performance.

Conclusion: SignRoundV2 provides an effective post-training quantization solution for LLMs that enables efficient deployment at extreme low-bit widths (2-5 bits) while maintaining competitive accuracy without requiring mixed-precision approaches.
...
TL;DR: TTSV is a lightweight plug-and-play method that prepends steering vectors to LLM inputs to enhance reasoning on test data without tuning model parameters, achieving significant performance gains while preserving pre-existing abilities.


<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods require tuning model parameters, which is computationally expensive and risks degrading the model's pre-existing abilities. There's a need for a lightweight, efficient approach that can unlock LLMs' reasoning potential for specific tasks without modifying the model itself.

Method: Introduces Test-Time Steering Vectors (TTSV) - lightweight components prepended to input while keeping LLM parameters frozen. TTSV is optimized on test data to minimize output entropy, steering the model toward higher confidence states and activating task-relevant inherent abilities.

Result: Achieves 45.88% relative performance gain on MATH500 with Qwen2.5-Math-7B and 16.22% gain with Qwen3-4B. TTSV is lightweight, efficient to optimize, and shows robust generalization with steering vectors transferable across diverse tasks.

Conclusion: TTSV provides an effective plug-and-play enhancement for LLMs that preserves pre-trained abilities while significantly boosting performance on specific tasks through lightweight test-time adaptation.
...
<details>
  <summary>Details</summary>
Motivation: Traditional knowledge editing methods show a significant gap between controlled evaluations and real-world effectiveness in lifelong learning scenarios. They suffer from overfitting to new facts (degrading pre-trained capabilities) and lack knowledge consolidation, leading to mismatch between parametric knowledge and actual generation behavior.

Method: Two-stage approach: (1) Targeted Proximal Supervised Fine-Tuning (TPSFT) localizes edits via trust-region objective to limit policy drift and prevent overfitting; (2) Consolidation stage using Group Relative Policy Optimization (GRPO) aligns edited knowledge with CoT-based inference policy by optimizing trajectory-level behavior under comprehensive reward signals.
...
Conclusion: The Edit-then-Consolidate paradigm effectively bridges the gap between theoretical knowledge editing methods and practical applicability by addressing overfitting and knowledge integration issues, making LLM knowledge editing more reliable for real-world lifelong learning scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Systematic evaluation of LLMs for non-English languages remains limited, and existing efforts often focus on leaderboards rather than methodological rigor. There's a need for comprehensive, community-driven evaluation frameworks for Italian language models.

Method: Federated collaboration with 80+ contributors from academia, industry, and public sector to design, document, and evaluate diverse tasks. Established centralized evaluation pipeline supporting heterogeneous datasets and metrics. Created benchmark with 20+ tasks and nearly 100 subtasks covering linguistic competence, reasoning, fairness, summarization, translation, and code generation.

Result: Evaluated four open-weight LLMs, revealing systematic strengths and weaknesses across different abilities. Exposed methodological lessons about fine-grained metrics, harmonized pipelines, and community engagement benefits/limitations. Created most comprehensive Italian benchmark to date.

Conclusion: CALAMITA offers both a resource (comprehensive Italian benchmark) and a framework for sustainable community-driven evaluation. The approach provides a blueprint for other languages seeking inclusive and rigorous LLM evaluation practices, with rolling benchmark design enabling continuous integration.
...
TL;DR: AdiBhashaa creates first open parallel corpora and baseline MT systems for four Indian tribal languages through community-driven approach, combining participatory data creation with technical evaluation.
...
Result: Created first open parallel corpora and baseline MT systems for four major Indian tribal languages: Bhili, Mundari, Gondi, and Santali.
...
TL;DR: Enhanced benchmark for evaluating linguistic acceptability in Danish using 14 corruption functions to generate incorrect sentences, providing more comprehensive assessment than existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: To create a more rigorous benchmark for evaluating linguistic acceptability in Danish that is broader and more comprehensive than current state-of-the-art benchmarks, addressing the need for better assessment tools for Large Language Models.

Method: 1) Analyze common errors in written Danish, 2) Develop 14 corruption functions that systematically introduce errors into correct Danish sentences, 3) Validate corruptions using manual and automatic methods, 4) Use resulting dataset as benchmark for LLM evaluation on linguistic acceptability tasks.

Result: The benchmark is more comprehensive than existing ones, increases task difficulty (evidenced by lower LLM performance), and has higher discriminatory power to better distinguish between well-performing and low-performing models.

Conclusion: The enhanced Danish linguistic acceptability benchmark provides a more rigorous evaluation framework that better assesses LLM capabilities and offers improved discriminatory power for model comparison.
...
TL;DR: Info-Mask is a novel framework for detecting transition points in mixed human-AI authored text, combining stylometric cues, perplexity signals, and boundary modeling, with adversarial robustness testing on a new benchmark dataset.


<details>
  <summary>Details</summary>
Motivation: As LLMs blur boundaries between human and AI text, there's a critical need to identify authorship transitions in collaborative content for authenticity, trust, and human oversight purposes.

Method: Info-Mask integrates stylometric cues, perplexity-driven signals, and structured boundary modeling. The framework is tested against adversarial perturbations using a new benchmark dataset (MAS) and includes Human-Interpretable Attribution overlays for explainability.
...
Conclusion: The research demonstrates both promise and limitations of adversarially robust, interpretable mixed-authorship detection, with important implications for trust and oversight in human-AI co-authorship scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Expanding linguistic diversity of instruct LLMs is hindered by costly labeled data requirements and catastrophic forgetting during adaptation to new languages, especially under low-resource constraints.

Method: Source-Shielded Updates (SSU) uses a small set of source data and parameter importance scoring to identify critical parameters for source knowledge preservation, then applies column-wise freezing to protect these parameters before adaptation with unlabeled target language data.

Result: SSU reduces catastrophic forgetting to only 3.4% (7B) and 2.8% (13B) performance degradation on source tasks vs 20.3% and 22.3% from full fine-tuning, while achieving competitive target-language performance across five diverse languages.

Conclusion: SSU effectively enables multilingual adaptation of instruct LLMs under low-resource constraints by proactively preserving source knowledge while achieving strong target language performance.
...
TL;DR: SEAL introduces a two-stage semantic parsing framework with self-evolving agentic learning for knowledge-based conversational QA, achieving SOTA on SPICE benchmark with improved structural accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing KBCQA approaches struggle with coreference resolution, contextual dependencies, and complex logical reasoning. Both end-to-end semantic parsing and stepwise agent-based methods suffer from structural inaccuracies and high computational costs, especially for complex queries over large knowledge graphs.

Method: Two-stage framework: 1) LLM extracts minimal S-expression core, refined by agentic calibration module for syntactic consistency and entity/relation alignment; 2) Template-based completion with question-type prediction and placeholder instantiation to build executable S-expression. Includes self-evolving mechanism with local/global memory and reflection module for continuous adaptation from dialog history and execution feedback.

Result: Extensive experiments on SPICE benchmark show state-of-the-art performance, particularly in multi-hop reasoning, comparison, and aggregation tasks. Demonstrates significant gains in both structural accuracy and computational efficiency.

Conclusion: SEAL provides a robust and scalable framework for conversational reasoning that effectively addresses core challenges in KBCQA through decomposition, agentic refinement, and self-evolving adaptation without explicit retraining.
...
TL;DR: LLMs can learn latent linguistic structures from text or explicit features for genre classification, but different linguistic features contribute unevenly across tasks, highlighting the need for incorporating more complex linguistic signals during training.


<details>
  <summary>Details</summary>
Motivation: While LLMs show strong performance on language tasks, it's unclear whether they truly capture deeper linguistic properties like syntax, phonetics, and metrical patterns from raw text. The paper aims to analyze if LLMs can effectively learn these features and apply them to important natural language tasks like genre classification.

Method: Created a novel multilingual genre classification dataset from Project Gutenberg with thousands of sentences per binary task (poetry vs. novel, drama vs. poetry, drama vs. novel) in six languages. Augmented each with three explicit linguistic feature sets: syntactic tree structures, metaphor counts, and phonetic metrics. Evaluated LLM classifiers on their ability to learn from raw text versus explicitly provided features.

Result: LLM classifiers can learn latent linguistic structures from both raw text and explicitly provided features. However, different linguistic features contribute unevenly across classification tasks, suggesting that not all features are equally useful for all genre discrimination tasks.

Conclusion: The findings underscore the importance of incorporating more complex linguistic signals during model training, as different linguistic features have varying importance across different genre classification tasks, and LLMs can benefit from explicit linguistic feature guidance.
...
TL;DR: The paper introduces Nex ecosystem for scaling LLM agent training through complexity, diversity, and fidelity dimensions, achieving SOTA performance on agentic tasks.


<details>
  <summary>Details</summary>
Motivation: LLMs are evolving from passive responders to autonomous agents, requiring a shift from static imitation to incentive-driven decision making, but current infrastructure lacks scalable methods for constructing high-quality interaction signals for effective policy learning.

Method: Three-dimensional scaling approach: (1) NexAU for building complex agent hierarchies via configurations, (2) NexA4A for generating diverse agent hierarchies from natural language, and (3) NexGAP for bridging simulation-reality gap with dynamic real-world environments.

Result: Nex-N1 trained on this infrastructure consistently outperforms SOTA open-source models and achieves competitive performance against frontier proprietary models on benchmarks like SWE-bench and tau2 for complex agentic tasks.

Conclusion: The Nex ecosystem provides comprehensive infrastructure for scaling agent training, enabling effective policy learning for autonomous LLM agents, with open-sourced components to facilitate further research.
...
TL;DR: CER is a retrieval method that fine-tunes embeddings with contrastive learning and generates token-level attribution rationales, using subjectivity-based hard negatives to align embeddings with evidential reasoning.


<details>
  <summary>Details</summary>
Motivation: To improve retrieval systems by restructuring them around factual evidence, particularly to mitigate hallucinations in RAG systems and enhance reliability in safety-critical domains like clinical trials.

Method: Self-Explaining Contrastive Evidence Re-Ranking (CER) combines: 1) fine-tuning embeddings with contrastive learning, 2) generating token-level attribution rationales for retrieved passages, and 3) automatically selecting hard negatives using a subjectivity-based criterion to push apart subjective/misleading explanations while pulling factual rationales closer.

Result: Initial experimental results on clinical trial reports show CER improves retrieval accuracy, mitigates potential for hallucinations in RAG systems, and provides transparent, evidence-based retrieval that enhances reliability.

Conclusion: CER creates an embedding space explicitly aligned with evidential reasoning, offering a promising approach for safety-critical domains by making retrieval more factual, transparent, and reliable.
...
TL;DR: Arbitrage: A step-level speculative generation framework that uses a lightweight router to dynamically choose between draft and target models for reasoning tasks, achieving 2x latency reduction at matched accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional token-level speculative decoding struggles with reasoning tasks due to unnecessary rejections of semantically equivalent steps. Existing step-level methods still waste compute by regenerating many rejected steps with little improvement.

Method: Proposes Arbitrage framework with a lightweight router trained to predict when the target model is likely to produce meaningfully better steps. Routes generation dynamically based on relative advantage between draft and target models instead of using fixed acceptance thresholds.
...
Conclusion: Arbitrage achieves near-optimal efficiency-accuracy trade-offs by approximating an ideal Arbitrage Oracle that always chooses the higher-quality step, making speculative decoding more effective for reasoning tasks.
...
TL;DR: FormatRL uses reinforcement learning with structure-aware rewards (TreeSim and Node-chrF) to improve document-level XML/HTML translation quality and structural accuracy.


<details>
  <summary>Details</summary>
Motivation: Current structured text translation methods are limited to sentence level and struggle with complex document-level XML/HTML structures, creating a need for better document-level translation approaches.

Method: Proposes Format Reinforcement Learning (FormatRL) using Group Relative Policy Optimization on supervised fine-tuning model, optimizing two novel structure-aware rewards: TreeSim (structural similarity) and Node-chrF (translation quality at XML node level).
...
Conclusion: FormatRL effectively addresses document-level structured text translation challenges by optimizing structure-aware rewards, demonstrating measurable improvements in both structural accuracy and translation quality.
...
TL;DR: SSB is a self-distillation technique that uses the same LLM as both teacher and student with different semantic contexts to improve math reasoning without human intervention, achieving 10%+ accuracy gains over RLVR methods.


<details>
  <summary>Details</summary>
Motivation: RLVR methods for training LLMs on reasoning tasks have bottlenecks like lack of dense rewards and poor sample efficiency, requiring significant compute resources in post-training.

Method: SSB uses the same base model as both teacher and student. The teacher generates multiple rollouts for math problems, filters correct and common incorrect responses, then provides these in context to produce step-by-step explanations. This creates paired teacher-student training data automatically, with the student learning to match teacher logits from just the bare question.

Result: On Qwen2.5-3B-Instruct fine-tuned on GSM8K, SSB achieved 10.6% and 10% accuracy improvements on MATH500 and AIME2024 benchmarks respectively over GRPO (a common RLVR algorithm).

Conclusion: SSB provides an effective self-distillation approach that overcomes RLVR limitations, improving math reasoning performance without human intervention and with better sample efficiency.
...
TL;DR: Proposes a synthetic video generation method that creates training data with subtle kinematic inconsistencies to improve deepfake detection generalization to unseen manipulations.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods struggle with generalization to unseen manipulations, especially in video domain. Current video methods only model frame-to-frame instabilities but overlook the violation of natural motion dependencies between different facial regions.

Method: Train an autoencoder to decompose facial landmark configurations into motion bases. Manipulate these bases to selectively break natural correlations in facial movements, then introduce these artifacts into pristine videos via face morphing to create synthetic training data.

Result: Network trained on this synthetic data learns to spot sophisticated biomechanical flaws and achieves state-of-the-art generalization results on several popular benchmarks.

Conclusion: The proposed method effectively addresses the generalization challenge in deepfake video detection by focusing on kinematic inconsistencies in facial movements, outperforming existing approaches.
...
Method: OnSight Pathology uses continuous custom screen captures to provide real-time AI inferences as users review digital slide images. It operates as a single executable file that runs locally on consumer PCs without complex software integration, and includes a multi-modal chat assistant for verifiable image descriptions.

Result: Demonstrated utility across 2,500+ publicly available whole slide images and clinical cases. Software showed robustness across routine histopathological tasks including brain tumor classification, mitosis detection, and immunohistochemical stain quantification. Also compatible with live microscope camera feeds including smartphone cameras.
...
TL;DR: PARSE is an unsupervised framework that learns hierarchical event structure from streaming video through predictive learning, achieving state-of-the-art performance on temporal segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Humans naturally perceive continuous experience as a hierarchy of temporally nested events, but current computer vision models lack the ability to segment video predictively and hierarchically in an unsupervised manner.

Method: PARSE uses a hierarchy of recurrent predictors operating at different temporal granularities, with lower layers modeling short-term dynamics and higher layers integrating longer-term context through attention-based feedback. Event boundaries emerge naturally from transient peaks in prediction error.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-video models generate photorealistic videos but struggle with physical validity - they don't faithfully obey physics principles or maintain motion coherence, especially for Newtonian phenomena.

Method: MoReGen framework uses multi-agent LLMs, physics simulators, and renderers to generate reproducible videos in the code domain. Introduces object-trajectory correspondence as evaluation metric and creates MoReSet benchmark with 1,275 annotated videos across 9 Newtonian phenomena classes.

Result: State-of-the-art T2V models show poor physical validity, while MoReGen establishes a principled approach for physically coherent video synthesis. The MoReSet benchmark enables quantitative evaluation of physical precision.

Conclusion: MoReGen provides a physics-grounded direction for T2V generation, addressing the core challenge of physical validity through systematic integration of physics principles and quantitative evaluation metrics.
...
TL;DR: ReasonX uses MLLM as perceptual judge to provide relative comparisons for fine-tuning intrinsic decomposition models on unlabeled real-world images via GRPO rewards, improving generalization across architectures.


<details>
  <summary>Details</summary>
Motivation: Current intrinsic decomposition models rely on synthetic paired supervision and struggle to generalize to diverse real-world scenarios, creating a need for better adaptation to unlabeled, in-the-wild images.

Method: Leverages multimodal LLM as perceptual judge for relative intrinsic comparisons, uses these comparisons as GRPO rewards to fine-tune models on unlabeled images by aligning model outputs with judge's relational assessments.

Result: Significant improvements across multiple architectures: 9-25% WHDR reduction on IIW albedo and up to 46% depth accuracy gains on ETH3D, demonstrating effective generalization.

Conclusion: MLLM-guided comparative supervision effectively bridges low- and high-level vision reasoning, offering model-agnostic framework for improving intrinsic decomposition in real-world scenarios.
...
TL;DR: AdversarialAnatomyBench is a new benchmark for testing vision-language models on rare anatomical variants, revealing significant performance drops (74% to 29% accuracy) and persistent anatomical biases that current scaling and interventions cannot resolve.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for clinical vision-language models focus on common anatomical presentations, failing to capture challenges posed by rare anatomical variants. There's a critical gap in evaluating how well these models handle atypical human anatomy that violates learned priors about "typical" anatomy.
...
Result: Three key findings: 1) Mean accuracy dropped from 74% on typical anatomy to 29% on atypical anatomy, with best models (GPT-5, Gemini 2.5 Pro, Llama 4 Maverick) showing 41-51% performance drops; 2) Model errors closely mirrored expected anatomical biases; 3) Neither model scaling nor interventions resolved these issues.

Conclusion: Current vision-language models have poor generalization to rare anatomical presentations, highlighting a critical unquantified limitation. AdversarialAnatomyBench provides a foundation for systematically measuring and mitigating anatomical bias in multimodal medical AI systems.
...
Method: Two-stage pipeline: 1) Novel representations bridge 3D layout with consistent image-based condition signals, 2) Image-conditioned multi-view generation with layout-aware epipolar attention. Also includes iterative framework for recursive multi-view generation to handle varying scene complexities.
...
Conclusion: MVRoom provides an effective solution for controllable 3D indoor scene generation with strong multi-view consistency, supporting both layout-based and text-to-scene generation through its iterative framework.
...
TL;DR: UniLight is a unified lighting representation that creates a joint latent space for multiple modalities (text, images, irradiance, environment maps) using contrastive learning and spherical harmonics prediction, enabling cross-modal lighting transfer and manipulation.


<details>
  <summary>Details</summary>
Motivation: Lighting is crucial for visual appearance but difficult to understand and represent. Existing lighting representations (environment maps, irradiance, spherical harmonics, text) are incompatible, limiting cross-modal transfer and manipulation.

Method: Proposes UniLight - a joint latent space that unifies multiple lighting modalities. Uses modality-specific encoders for text, images, irradiance, and environment maps trained contrastively to align representations. Includes auxiliary spherical-harmonics prediction task for directional understanding. Multi-modal data pipeline enables large-scale training.

Result: Enables three tasks: lighting-based retrieval, environment-map generation, and lighting control in diffusion-based image synthesis. Captures consistent and transferable lighting features across modalities, allowing flexible manipulation.

Conclusion: UniLight provides a unified lighting representation that bridges multiple modalities, enabling effective cross-modal transfer and manipulation of lighting in various applications.
...
TL;DR: GRU-SNF improves video motion transfer diversity by adding MCMC stochastic sampling to GRU-NF during inference, enabling better multimodal future predictions without retraining.


<details>
  <summary>Details</summary>
Motivation: Real-time video applications need accurate yet diverse future predictions for realistic synthesis and robust decision-making under uncertainty. Current GRU-NF models have limited expressivity due to deterministic transformations.

Method: Propose GRU-SNF that combines GRU-NF with stochastic sampling via MCMC steps during inference, inspired by Stochastic Normalizing Flows. This enables exploration of richer output space without retraining.

Result: GRU-SNF outperforms GRU-NF in generating diverse outputs without sacrificing accuracy, especially under longer prediction horizons. Better captures multimodal behavior in keypoint-based video motion transfer.

Conclusion: Integrating stochastic dynamics with flow-based sequence models enhances generative time series forecasting, showing potential for applications requiring diverse yet accurate future predictions.
...
TL;DR: Theoretical analysis of PnP-Flow leads to SDE model that improves image restoration performance through error quantification and acceleration techniques.


<details>
  <summary>Details</summary>
Motivation: While PnP-Flow has shown empirical success in image restoration, there's a lack of theoretical understanding behind its performance. The paper aims to bridge this gap by developing a theoretical framework.

Method: Derived a continuous limit for PnP-Flow to obtain a stochastic differential equation (SDE) surrogate model. Used this SDE model to: (1) quantify restoration error and improve step scheduling/regularization, (2) accelerate existing PnP-Flow models via extrapolation.
...
Conclusion: Theoretical analysis via SDE modeling provides practical insights for improving PnP-Flow performance, bridging the gap between empirical success and theoretical understanding in flow matching-based image restoration.
...
<details>
  <summary>Details</summary>
Motivation: Data loading has become a major bottleneck in deep learning despite advances in specialized hardware, especially as models and input data sizes grow increasingly complex. The computational overhead of full JPEG decoding significantly slows down training and inference.

Method: Propose a lightweight super-resolution pipeline that operates directly on JPEG discrete cosine transform (DCT) coefficients in the frequency domain, bypassing the need for full JPEG decoding. This approach extends prior work on recognition tasks to the restoration task of single-image super-resolution (SISR).

Result: Achieves 2.6x speedup in data loading and 2.5x speedup in training compared to standard approaches. The method preserves visual quality comparable to conventional SISR methods that work with fully decoded images.

Conclusion: Training models directly on encoded JPEG features effectively addresses data loading bottlenecks in deep learning, particularly for restoration tasks like super-resolution, offering significant speed improvements without sacrificing quality.
...
TL;DR: GfM is a lightweight monocular geometry estimation method that decouples global and local road structure to accurately recover fine-scale 3D road geometry like bumps and slopes without full calibration.


<details>
  <summary>Details</summary>
Motivation: Conventional monocular depth estimation oversmooths fine-scale road geometry features (bumps, slopes, irregularities) that are critical for safe vehicle control and motion planning, creating a need for more accurate perception of 3D surroundings.

Method: GfM resolves projective ambiguity by decoupling global and local structure: predicts a dominant road plane plus residual variations expressed as gamma (dimensionless vertical deviation ratio). Using only camera height above ground, it recovers metric depth via closed-form solution without full extrinsic calibration.

Result: Achieves state-of-the-art near-field accuracy in both depth and gamma estimation on KITTI and RSRD datasets while maintaining competitive global depth performance. The 8.88M-parameter model adapts robustly across diverse camera setups and is the first self-supervised monocular approach evaluated on RSRD.

Conclusion: GfM provides a physically interpretable, lightweight solution for accurate 3D road geometry estimation that prioritizes near-road detail and enables self-supervised learning without large annotated datasets, making it suitable for practical vehicle control applications.
...
<details>
  <summary>Details</summary>
Motivation: While CLIP calibration has been studied in offline settings, its behavior under federated learning (FL) fine-tuning remains unexplored. The paper investigates how FL affects CLIP calibration and aims to improve reliability in distributed settings.

Method: The paper first analyzes Textual Prompt Tuning approaches under FL, evaluates existing in-training calibration techniques across four global aggregation methods, then proposes FL²oRA - a straightforward LoRA-based approach that naturally improves calibration in FL. The method focuses on which components to fine-tune rather than how to aggregate or calibrate.

Result: Textual Prompt Tuning degrades calibration metrics under FL, and existing calibration techniques provide limited improvements. FL²oRA consistently produces well-calibrated models across multiple benchmarks, reducing the need for explicit calibration procedures.

Conclusion: The key challenge in FL calibration is not just how to aggregate or calibrate, but which components to fine-tune. FL²oRA effectively addresses this by using LoRA-based fine-tuning to naturally improve calibration in federated learning settings.
...
Method: Uses CLIP representations with modality gap reduction, then prompts a pre-trained language model decoder with retrieved caption examples and latent vector representations for generation.

Result: TOMCap outperforms other training-free and text-only methods, with analysis showing impact of retrieval-augmentation and modality gap reduction configurations.

Conclusion: The proposed text-only training method effectively performs image captioning without aligned image-caption pairs, demonstrating competitive performance through CLIP-based prompting and retrieval augmentation.
...
TL;DR: A low-cost automated system for sex-based sorting of house crickets using computer vision and physical actuation achieves 86.8% sorting accuracy.


<details>
  <summary>Details</summary>
Motivation: Current cricket farming lacks automated sex sorting despite benefits for selective breeding, optimized reproduction, and nutritional differentiation. The global demand for sustainable protein sources drives interest in efficient insect farming.

Method: Combines computer vision and physical actuation using Raspberry Pi 5 with Raspberry AI Camera and custom YOLOv8 nano object detection model, plus servo-actuated sorting arm.

Result: Model achieved mAP@0.5 of 0.977, and real-world experiments showed 86.8% overall sorting accuracy for cricket groups.

Conclusion: Demonstrates feasibility of lightweight deep learning models on resource-constrained devices for insect farming, offering practical solution to improve efficiency and sustainability in cricket production.
...
TL;DR: DisentangleFormer is a Vision Transformer architecture that decouples spatial and channel processing for hyperspectral imaging, achieving SOTA performance with reduced computational cost.
...
Method: The architecture uses spatial-channel decoupling with three core components: 1) Parallel Disentanglement for independent spatial-token and channel-token streams, 2) Squeezed Token Enhancer for adaptive fusion of spatial and channel streams, and 3) Multi-Scale FFN for capturing fine-grained local context alongside global attention.

Result: Achieves state-of-the-art performance on hyperspectral benchmarks (Indian Pine, Pavia University, Houston, BigEarthNet, infrared pathology dataset) while maintaining competitive ImageNet accuracy with 17.8% reduction in FLOPs.

Conclusion: DisentangleFormer demonstrates that principled spatial-channel decoupling enables robust multi-channel vision representation, addressing fundamental limitations of standard self-attention in Vision Transformers for hyperspectral imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: Modeling dynamic 3D scenes from multiple views is challenging due to high-dimensional nature and the need to aggregate information across views. Real-world video sets are often unsynchronized, making it difficult to reconstruct time-evolving 3D geometry and motion accurately.
...
Result: Achieves sub-frame synchronization accuracy with average temporal error below 0.26 frames. High-fidelity 4D reconstruction reaching 26.3 PSNR scores on Panoptic Studio dataset. Produces synchronized 4DGS representation with dense, explicit 3D trajectories and temporal offsets for each video.

Conclusion: First general 4D Gaussian Splatting approach for unsynchronized video sets that doesn't require predefined scene objects or prior models. Successfully handles real-world, unsynchronized video sets by leveraging dense 4D track representations for simultaneous synchronization and reconstruction.
...
TL;DR: This paper presents a novel B-spline-based method for generating realistic DIC datasets and introduces Bayes-DIC Net, a Bayesian neural network architecture for displacement field prediction with uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for large-scale, realistic DIC datasets to train deep learning algorithms and the importance of uncertainty estimation in real-world displacement field predictions.

Method: 1) Generate DIC datasets using non-uniform B-spline surfaces with random control points to create realistic displacement fields. 2) Propose Bayes-DIC Net architecture with multi-level feature extraction, single skip connections, and lightweight convolutional blocks. 3) Transform network into Bayesian neural network using dropout modules during inference for uncertainty estimation.

Result: The method enables generation of large-scale DIC datasets capturing real-world displacement scenarios. Bayes-DIC Net provides both displacement predictions and confidence levels, enhancing practical reliability for real-world applications.

Conclusion: The paper offers innovative approaches for DIC dataset generation and algorithm enhancement, with the Bayesian network transformation providing valuable uncertainty quantification for practical displacement field prediction tasks.
...
TL;DR: NN-RAG is a system that extracts, validates, and makes searchable reusable neural network modules from PyTorch codebases, enabling cross-repository architectural pattern migration and expanding neural architecture diversity.
...
Method: NN-RAG performs scope-aware dependency resolution, import-preserving reconstruction, and validator-gated promotion to extract validated neural modules. It uses multi-level de-duplication (exact, lexical, structural) and enables cross-repository migration by automatically identifying reusable modules and regenerating them with complete dependencies in new contexts.

Result: Applied to 19 major repositories, the pipeline extracted 1,289 candidate blocks and validated 941 (73.0%). Over 80% were structurally unique, and NN-RAG contributed approximately 72% of all novel network structures to the LEMUR dataset. The system uniquely enables cross-repository migration of architectural patterns.
...
<details>
  <summary>Details</summary>
Motivation: Existing face forgery detection methods are limited to binary classification or known fake categories, making them vulnerable to emerging novel types of forgeries. The rapid development of face forgery generation algorithms creates a need for systems that can detect previously unseen fake categories.

Method: Proposes Dual-Level Evidential face forgery Detection (DLED) that reformulates open-set detection through uncertainty estimation. The method collects and fuses category-specific evidence at both spatial and frequency levels to estimate prediction uncertainty, enabling detection of novel fake categories.

Result: DLED achieves state-of-the-art performance, outperforming baseline models by an average of 20% in detecting forgeries from novel fake categories. It also shows competitive performance on traditional Real-vs-Fake face forgery detection tasks.

Conclusion: The proposed DLED approach effectively addresses the open-set face forgery detection problem by leveraging uncertainty estimation from dual-level evidence, making it robust to emerging novel fake categories while maintaining strong performance on traditional detection tasks.
...
TL;DR: SANTA is a framework that reduces factual hallucinations in video captioning by using self-augmented contrastive alignment to match visual objects and temporal actions with their corresponding phrases.
...
Conclusion: SANTA effectively addresses the challenging problem of video hallucination mitigation through self-augmented contrastive alignment, improving faithfulness in both object and action descriptions for dynamic video content.
...
TL;DR: MAFNet: A Multi-frequency Adaptive Fusion Network for stereo matching using only efficient 2D convolutions, achieving better accuracy-speed balance than existing real-time methods.


<details>
  <summary>Details</summary>
Motivation: Existing stereo matching networks have limitations: 3D convolution-based methods have high computational overhead, while iterative optimization methods lack non-local context modeling. Both have poor compatibility with resource-constrained mobile devices, limiting real-time deployment.

Method: Proposes MAFNet with two key components: 1) Adaptive frequency-domain filtering attention module that decomposes cost volume into high- and low-frequency volumes for separate feature aggregation, and 2) Linformer-based low-rank attention mechanism to adaptively fuse high- and low-frequency information for robust disparity estimation.

Result: MAFNet significantly outperforms existing real-time methods on Scene Flow and KITTI 2015 datasets, showing favorable balance between accuracy and real-time performance.

Conclusion: The proposed MAFNet achieves high-quality disparity maps using only efficient 2D convolutions, making it suitable for resource-constrained mobile devices and real-time applications while maintaining competitive accuracy.
...
TL;DR: FMA-Net++ is a video restoration framework that jointly handles super-resolution and deblurring while explicitly modeling the coupled effects of motion and dynamically varying exposure, achieving state-of-the-art performance with improved efficiency.
...
Method: Uses sequence-level architecture with Hierarchical Refinement with Bidirectional Propagation blocks for parallel, long-range temporal modeling. Includes Exposure Time-aware Modulation layer to condition features on per-frame exposure, and exposure-aware Flow-Guided Dynamic Filtering to infer motion- and exposure-aware degradation kernels. Decouples degradation learning from restoration.

Result: Achieves state-of-the-art accuracy and temporal consistency on new benchmarks (REDS-ME, REDS-RE) and GoPro, outperforming recent methods in both restoration quality and inference speed. Generalizes well to challenging real-world videos despite being trained only on synthetic data.

Conclusion: FMA-Net++ effectively addresses the coupled motion-exposure degradation problem in real-world video restoration through explicit modeling of exposure variations and motion, demonstrating superior performance and generalization capabilities.
...
Method: FARL uses Fourier analysis with a dual cross-attention mechanism: learnable tokens separately query structural features from phase spectrum and stylistic features from amplitude spectrum. The disentangled tokens are injected deep into VLM encoders using an asymmetric injection strategy to force robust vision-language alignment.
...
Conclusion: Explicitly disentangling visual representations through Fourier analysis and deep injection into VLMs enhances generalization and vision-language alignment for few-shot learning tasks.
...
TL;DR: Transfer learning with pre-trained CNN models improves medical image classification, with InceptionV3 performing best on chest X-ray disease detection, while ResNet shows better performance with increased depth.


<details>
  <summary>Details</summary>
Motivation: Deep learning is effective for medical image classification but training large models from scratch is often infeasible. Transfer learning offers a solution by reusing pre-trained models for new medical tasks, especially with limited data.

Method: Comprehensive analysis of transfer learning techniques using six pre-trained CNN models (AlexNet, VGG16, ResNet18, ResNet34, ResNet50, InceptionV3) on a custom chest X-ray dataset for disease detection, including uncertainty analysis and runtime comparison.

Result: InceptionV3 consistently outperforms other models across all standard metrics. ResNet family shows progressively better performance with increasing depth. VGG16 and AlexNet perform reasonably well but with lower accuracy. Transfer learning is beneficial, especially with limited data.

Conclusion: Transfer learning is effective for medical image classification, with model selection depending on architecture, dataset size, and domain similarity. A well-trained feature extractor with lightweight feedforward model can provide efficient predictions, offering practical guidance for medical AI applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing distillation methods for remote sensing object detection struggle with mixed features in RSIs and neglect discrepancies caused by subtle feature variations, leading to entangled knowledge confusion.

Method: Proposes DS2D2 with explicit and implicit distillation using spectral decomposition: 1) First-order wavelet transform for spectral decomposition to preserve spatial characteristics, with Density-Independent Scale Weight for dense/small objects; 2) Implicit knowledge extraction from subtle student-teacher feature discrepancies via full-frequency and high-frequency amplifiers.

Result: Extensive experiments on DIOR and DOTA datasets show effectiveness: On DIOR, achieves 4.2% AP50 improvement for RetinaNet and 3.8% AP50 improvement for Faster R-CNN, outperforming existing distillation approaches.

Conclusion: DS2D2 is an effective architecture-agnostic distillation method that addresses mixed features and subtle variations in remote sensing object detection through spectral decomposition and dual-stream knowledge extraction.
...
TL;DR: Proposes differentiable triangle-based ray tracing pipeline that directly uses triangles as rendering primitives, achieving higher quality than proxy-based methods while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Existing ray tracing methods for 3D Gaussian particles use proxy geometry (complex meshes) requiring costly intersection tests, creating limitations since Gaussian particles aren't well-suited as unified primitives for both ray tracing and rasterization.

Method: Differentiable triangle-based ray tracing pipeline that directly treats triangles as rendering primitives without relying on proxy geometry. Can render triangles optimized by rasterization-based Triangle Splatting method.

Result: Achieves significantly higher rendering quality than existing ray tracing approaches while maintaining real-time rendering performance. Unifies primitives used in novel-view synthesis.

Conclusion: Proposed method provides a unified approach using triangles as rendering primitives for both ray tracing and rasterization, overcoming limitations of proxy-based methods and enabling higher quality real-time rendering.
...
<details>
  <summary>Details</summary>
Motivation: Existing gait analysis approaches for Parkinson's disease detection are limited by single-modality inputs, low robustness, lack of clinical transparency, and difficulty capturing fine-grained gait patterns under realistic conditions.

Method: Uses dual YOLOv11 encoders for RGB-D feature extraction, Multi-Scale Local-Global Extraction (MLGE) module, Cross-Spatial Neck Fusion for spatial-temporal representation, and frozen LLM to translate visual embeddings into clinical explanations.

Result: Achieves higher recognition accuracy, improved robustness to environmental variations (low lighting, occlusion), and provides clear visual-linguistic reasoning compared to single-input baselines on multimodal gait datasets.

Conclusion: The RGB-D fusion framework bridges visual recognition with clinical understanding through multimodal feature learning and language-based interpretability, offering a novel vision-language paradigm for reliable and explainable Parkinson's disease gait analysis.
...
<details>
  <summary>Details</summary>
Motivation: Current automatic trailer generation methods use a "selection-then-ranking" paradigm that suffers from error propagation and limits trailer quality. The authors aim to move beyond this paradigm to improve automatic trailer generation.

Method: SSMP uses a Transformer encoder trained via masked prediction to reconstruct trailer shot sequences from randomly masked counterparts. It features self-paced mask ratios for adaptive difficulty and progressive self-correction during generation, where high-confidence shots are filled first and remaining positions are re-masked for next predictions.

Result: SSMP achieves state-of-the-art results in automatic trailer generation, with both quantitative results and user studies demonstrating superiority over existing methods.

Conclusion: The proposed SSMP method successfully overcomes limitations of traditional "selection-then-ranking" approaches through its self-paced, self-corrective masked prediction framework, providing a more effective solution for automatic movie trailer generation.
...
TL;DR: MindDrive harmonizes trajectory generation and decision reasoning for autonomous driving using "context simulation - candidate generation - multi-objective trade-off" paradigm with Future-aware Trajectory Generator and VLM-oriented Evaluator.
...
Method: MindDrive establishes a structured reasoning paradigm: 1) Future-aware Trajectory Generator (FaTG) based on World Action Model performs ego-conditioned "what-if" simulations to predict future scenes and generate foresighted trajectory candidates, 2) VLM-oriented Evaluator (VLoE) leverages large vision-language model reasoning to conduct multi-objective evaluations across safety, comfort, and efficiency dimensions.

Result: Extensive experiments on NAVSIM-v1 and NAVSIM-v2 benchmarks demonstrate state-of-the-art performance across multi-dimensional driving metrics, significantly enhancing safety, compliance, and generalization.
...
TL;DR: StreamEQA is a new benchmark for streaming video question answering in embodied scenarios, evaluating models on embodied reasoning (perception, interaction, planning) and streaming reasoning (backward, real-time, forward) across 156 long videos with 21K QA pairs.


<details>
  <summary>Details</summary>
Motivation: As embodied intelligence moves toward real-world deployment, agents need continuous perception and reasoning over streaming visual inputs to maintain situational awareness, comprehend interactions, and plan actions based on past, present, and future contexts.

Method: Created StreamEQA benchmark with 156 independent long videos, 42 tasks, and ~21K QA pairs with precise timestamps using hybrid pipeline of automated generation and human refinement. Evaluates models on two dimensions: Embodied (perception, interaction, planning) and Streaming (backward, real-time, forward reasoning).
...
Conclusion: StreamEQA addresses a critical gap in evaluating streaming video understanding for embodied applications and aims to catalyze research in this direction, as current models still underperform in this challenging setting.
...
TL;DR: GuidNoise: A diffusion-based method that uses just one noisy/clean image pair to generate realistic synthetic noise for data augmentation, improving denoising performance without needing camera metadata or extensive paired data.


<details>
  <summary>Details</summary>
Motivation: Existing generative noise synthesis methods require camera metadata and extensive noisy-clean image pairs, which are costly to acquire and show limited generalization between different settings.

Method: Proposes GuidNoise, a Single-Pair Guided Diffusion model that uses just one noisy/clean pair as guidance. Introduces guidance-aware affine feature modification (GAFM) and noise-aware refine loss to leverage diffusion models' potential for realistic noise generation.

Result: GuidNoise synthesizes high-quality noisy images under diverse noise environments without additional metadata. Enables efficient generation of noisy-clean pairs at inference time, significantly improving denoising performance, especially with lightweight models and limited training data.

Conclusion: GuidNoise effectively addresses the data acquisition bottleneck in image denoising by requiring only a single noisy/clean pair, making synthetic noise generation more practical and accessible while improving denoising performance through self-augmentation.
...
TL;DR: dVLM-AD: A diffusion-based vision-language model for end-to-end autonomous driving that improves consistency between reasoning and planning compared to autoregressive VLMs.


<details>
  <summary>Details</summary>
Motivation: Existing AR-based vision-language models for autonomous driving suffer from inconsistency between high-level reasoning and low-level planning due to causal attention and sequential token generation limitations. The paper aims to address OOD driving challenges by leveraging diffusion models' superior controllability and reliability.

Method: Introduces dVLM-AD, a diffusion-based vision-language model that unifies perception, structured reasoning, and low-level planning for end-to-end driving. Uses discrete diffusion VLMs with bidirectional attention and iterative denoising for better consistency and controllability.

Result: dVLM-AD achieves planning performance comparable to existing driving VLM/VLA systems despite modest backbone. Outperforms AR-based baselines with 9% improvement in behavior-trajectory consistency and 6% increase in RFS on long-tail WOD-E2E scenarios.

Conclusion: Diffusion-based VLMs offer a more controllable and reliable pathway for scalable end-to-end autonomous driving compared to AR-based approaches, particularly for handling out-of-distribution scenarios.
...
Method: Proposes UniTS framework based on flow matching generative paradigm with diffusion transformer architecture, featuring Adaptive Condition Injector (ACor) for multimodal inputs and Spatiotemporal-aware Modulator (STM) for capturing complex dependencies.

Result: UniTS outperforms existing methods, especially for severe cloud contamination, modality absence, and forecasting phenological variations, demonstrating exceptional generative and cognitive capabilities.

Conclusion: UniTS provides a general framework for unified spatiotemporal modeling across multiple satellite time series tasks, with new benchmark datasets (TS-S12, TS-S12CR) advancing the field.
...
TL;DR: DeRA is a novel 1D video tokenizer that decouples spatial-temporal learning into appearance and motion streams, achieving better efficiency and performance than previous methods.


<details>
  <summary>Details</summary>
Motivation: Current video tokenization methods struggle with training efficiency and performance due to coupled spatial-temporal representation learning. The authors aim to create a more efficient video tokenizer by decoupling these aspects while maintaining a compact 1D latent space.

Method: DeRA factorizes video encoding into appearance and motion streams aligned with pretrained vision foundation models. It introduces a Symmetric Alignment-Conflict Projection (SACP) module to address gradient conflicts from heterogeneous supervision by suppressing conflicting gradient components.

Result: DeRA outperforms previous state-of-the-art video tokenizer LARP by 25% on UCF-101 in rFVD. It also achieves new state-of-the-art results on UCF-101 class-conditional generation and K600 frame prediction when used for autoregressive video generation.

Conclusion: DeRA successfully demonstrates that decoupling spatial-temporal representation learning in video tokenization leads to improved training efficiency and performance, establishing new benchmarks for video generation tasks.
...
TL;DR: The paper introduces NABLA, a bird image dataset for evaluating identity-preserving generation models, showing current SOTA methods fail on this fine-grained domain and proposing species/age/sex grouping for improvement.


<details>
  <summary>Details</summary>
Motivation: Current identity-preserving image generation models work well for humans and rigid objects but struggle with non-rigid, fine-grained categories like birds that lack accessible high-quality data. Birds are ideal for testing because they have high diversity, require fine-grained identification cues, and come in various poses.
...
Result: State-of-the-art baselines fail to maintain bird identity on the NABLA dataset. Training with images grouped by species, age, and sex substantially improves performance on both seen and unseen species.

Conclusion: The NABLA benchmark reveals limitations of current identity-preserving models for fine-grained domains like birds, and demonstrates that grouping by biological attributes can significantly improve identity preservation, advancing applications requiring accuracy and fine detail.
...
TL;DR: COMET: A real-time autoregressive framework for stable, controllable character motion generation with fine-grained joint control and long-term stability.


<details>
  <summary>Details</summary>
Motivation: Existing character motion generation methods lack fine-grained control and suffer from motion degradation over long sequences, limiting their use in interactive applications.

Method: An autoregressive framework with efficient Transformer-based conditional VAE for precise joint control, plus a novel reference-guided feedback mechanism for long-term stability and real-time style transfer.

Result: COMET generates high-quality motion at real-time speeds, significantly outperforming state-of-the-art approaches in complex motion control tasks.

Conclusion: The framework enables versatile character control and robust long-horizon synthesis, confirming its readiness for demanding interactive applications.
...
TL;DR: SEASON is a training-free method that enhances temporal and spatial faithfulness in VideoLLMs by adaptively diagnosing hallucination tendencies and applying contrastive decoding against temporal/spatial negatives.
...
Method: Self-Diagnostic Contrastive Decoding (SEASON) - a training-free method that dynamically diagnoses each token's hallucination tendency and applies adaptive contrastive decoding against corresponding temporal and spatial negatives to enhance faithfulness.
...
Conclusion: SEASON effectively addresses temporal hallucination issues in VideoLLMs through adaptive contrastive decoding, demonstrating superior performance on both hallucination-specific and general video understanding tasks without requiring additional training.
...
TL;DR: MM-SHR is a multi-model architecture for specular highlight removal that combines CNN for local details and transformer attention for global dependencies, achieving state-of-the-art performance across various surfaces.


<details>
  <summary>Details</summary>
Motivation: Specular highlights in practical environments impair visual performance and degrade task effectiveness. Existing single-type models (CNN or transformer) struggle to balance local fine-grained details and global long-range dependencies for highlights of different scales.

Method: Proposes MM-SHR architecture that uses convolution operations in shallow layers for local detail extraction and attention mechanisms in deep layers for global feature capture. Introduces OAIBlock (Omni-Directional Attention Integration Block) and HDDAConv (Adaptive Region-Aware Hybrid-Domain Dual Attention Convolutional Network) using coarse-to-fine approach with omni-directional pixel-shifting and window-dividing operations.

Result: Extensive experiments on three benchmark tasks and six surface materials show MM-SHR outperforms state-of-the-art methods in both accuracy and efficiency for specular highlight removal.

Conclusion: The multi-model architecture effectively addresses the scale variation problem in specular highlights by combining local and global modeling capabilities, achieving superior performance with implementation made publicly available.
...
TL;DR: The paper investigates motion representations and loss functions in diffusion models for human motion synthesis, comparing six common motion representations and analyzing training configurations to improve conditional motion diffusion models.


<details>
  <summary>Details</summary>
Motivation: While diffusion models have become successful in human motion synthesis (action-to-motion, text-to-motion, audio-to-motion), there are fundamental questions about motion representations and loss functions that need systematic investigation to enhance understanding and improve state-of-the-art conditional motion diffusion models.

Method: The authors conduct empirical studies using a proxy motion diffusion model (MDM) with v loss (vMDM), where v is the weighted sum of motion data and noise. They evaluate six common motion representations from literature, compare training time under various configurations, and conduct evaluation analysis on a large motion dataset.
...
Conclusion: The study provides foundational insights into motion representations and training configurations for diffusion models, offering guidance for improving conditional motion diffusion models through systematic evaluation of design choices.
...
TL;DR: UltraImage addresses content repetition and quality degradation in image diffusion transformers when generating beyond training scales using frequency analysis and adaptive attention concentration.


<details>
  <summary>Details</summary>
Motivation: Recent image diffusion transformers achieve high-fidelity generation but struggle with content repetition and quality degradation when generating images beyond training scales, limiting their scalability and practical applications.
...
Result: UltraImage consistently outperforms prior methods on Qwen-Image and Flux (around 4K) across three generation scenarios, reducing repetition and improving visual fidelity. Can generate images up to 6K*6K without low-resolution guidance from training resolution of 1328p, demonstrating extreme extrapolation capability.

Conclusion: UltraImage provides a principled framework that successfully addresses both content repetition and quality degradation issues in image diffusion transformers when generating beyond training scales, enabling high-quality extreme-scale image generation.
...
TL;DR: DuGI-MAE: A dual-domain guided infrared foundation model that improves upon InfMAE with entropy-based deterministic masking and dual-domain guidance for better global relationship modeling and noise filtering, pretrained on new Inf-590K dataset.


<details>
  <summary>Details</summary>
Motivation: Existing foundation models like MAE perform poorly on infrared images due to domain differences. Even specialized infrared models like InfMAE have limitations: they omit informative tokens, insufficiently model global associations, and ignore non-uniform noise common in infrared imagery.

Method: 1) Deterministic masking strategy based on token entropy (preserves high-entropy tokens for reconstruction); 2) Dual-Domain Guidance (DDG) module that captures global token relationships and adaptively filters non-uniform background noise; 3) Constructed Inf-590K dataset with diverse infrared scenes, targets, and resolutions for large-scale pretraining.

Result: DuGI-MAE demonstrates strong generalization across infrared object detection, semantic segmentation, and small target detection tasks. Experimental results show superiority over both supervised and self-supervised comparison methods.

Conclusion: The proposed DuGI-MAE effectively addresses limitations of existing infrared foundation models through entropy-based masking and dual-domain guidance, achieving state-of-the-art performance on various infrared vision tasks when pretrained on the comprehensive Inf-590K dataset.
...
Method: EgoLCD combines: 1) Long-Term Sparse KV Cache for stable global context, 2) attention-based short-term memory extended by LoRA for local adaptation, 3) Memory Regulation Loss to enforce consistent memory usage, and 4) Structured Narrative Prompting for explicit temporal guidance.
...
Conclusion: EgoLCD represents a significant step toward building scalable world models for embodied AI by addressing the challenge of content drift in long-context video generation through effective memory management.
...
TL;DR: VideoSSM combines autoregressive diffusion with state-space memory for coherent, interactive long-video generation that scales linearly with sequence length.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive diffusion models struggle with minute-scale video generation due to accumulated errors, motion drift, and content repetition. There's a need for better long-term coherence in streaming, interactive video synthesis.

Method: Proposes VideoSSM, a hybrid architecture unifying AR diffusion with state-space memory. Uses SSM as evolving global memory for scene dynamics across entire sequence, plus context window for local motion cues and fine details.

Result: Achieves state-of-the-art temporal consistency and motion stability on short- and long-range benchmarks, especially at minute-scale horizons. Enables content diversity and interactive prompt-based control with linear time scaling.

Conclusion: VideoSSM establishes a scalable, memory-aware framework for long video generation that maintains global consistency without repetitive patterns and supports interactive control.
...
TL;DR: BA-TTA-SAM enhances SAM's zero-shot medical image segmentation via test-time adaptation without source-domain training, achieving 12.4% average DICE improvement.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces challenges due to scarce annotated data and high computational costs. SAM shows promise for zero-shot segmentation but suffers from domain shifts on medical datasets, requiring efficient enhancement methods.
...
Conclusion: The framework significantly enhances SAM's generalization ability for medical image segmentation through test-time adaptation, providing an effective solution for zero-shot enhancement without domain-specific training.
...
TL;DR: WiFi-based gesture recognition using Doppler spectra and attention mechanisms achieves high cross-domain performance


<details>
  <summary>Details</summary>
Motivation: Existing WiFi gesture sensing solutions work well in-domain but lack cross-domain capabilities, limiting practical deployment in untrained environments

Method: Extract Doppler spectra from CSI, concatenate along time axis to create fused multi-angle images, then use ResNet18 backbone with novel attention mechanism combining multi-semantic spatial attention and self-attention-based channel attention
...
Conclusion: The proposed network effectively extracts domain-independent features through attention mechanisms, enabling robust cross-domain WiFi gesture recognition with high accuracy
...
TL;DR: ICRE network improves VI-ReID by mining modality-specific identity clues through multi-perception feature refinement, semantic distillation cascade enhancement, and identity clues guided loss.
...
Method: Proposes ICRE network with: 1) MPFR module to aggregate shallow features for capturing modality-specific attributes, 2) SDCE module to distill identity-aware knowledge from shallow features to guide modality-invariant learning, 3) ICG Loss to reduce modality discrepancies and promote diverse representation space.

Result: Extensive experiments on multiple public datasets show ICRE outperforms existing state-of-the-art methods.
...
TL;DR: Auto3R is a data-driven uncertainty quantification model that automates 3D scanning and reconstruction of scenes/objects, including challenging materials like specular surfaces, by predicting optimal scanning viewpoints without ground truth knowledge.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D scanning requires manual planning, but with the rise of embodied systems (drones, robots), there's a need for fully automated, high-quality 3D reconstruction that can handle complex materials like non-lambertian and specular surfaces.

Method: Auto3R uses a data-driven uncertainty quantification model that operates in iterative 3D reconstruction and scanning cycles. It predicts uncertainty distribution over potential scanning viewpoints without requiring ground truth geometry or appearance information, enabling efficient viewpoint selection.

Result: Auto3R achieves superior performance, outperforming state-of-the-art methods by a large margin in extensive experiments. It was successfully deployed on a robot arm with a camera, demonstrating effective digitization of real-world 3D objects into ready-to-use, photorealistic digital assets.

Conclusion: Auto3R provides an effective automated solution for 3D scanning and reconstruction that works with challenging materials and can be deployed on embodied systems, advancing the field of automated digital asset creation.
...
TL;DR: DraCo introduces an interleaved reasoning paradigm that uses low-resolution draft images as visual planning in chain-of-thought reasoning, enabling better verification and refinement for text-to-image generation.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal LLMs for text-to-image generation are limited - they either treat models as standalone generators or rely on abstract textual planning, lacking concrete visual guidance and verification capabilities.

Method: 1) Generate low-resolution draft image as visual preview, 2) Use model's understanding to verify semantic alignment between draft and prompt, 3) Perform selective corrections through super-resolution refinement, 4) Train with DraCo-240K dataset for three atomic capabilities, 5) Use DraCo-CFG specialized guidance strategy.
...
Conclusion: The interleaved reasoning paradigm with visual draft generation and verification addresses fundamental challenges in text-to-image generation, enabling better handling of rare attribute combinations and improving generation quality through concrete visual planning.
...
TL;DR: PhyVLLM introduces a physics-guided video-language framework that explicitly incorporates physical motion modeling into Video LLMs to address their limitations in understanding physical dynamics, using dual-branch encoding, Neural ODEs, and self-supervised learning.


<details>
  <summary>Details</summary>
Motivation: Current Video LLMs rely on appearance-based matching and fail in scenarios requiring deeper understanding of physical dynamics, which is crucial for comprehensive video understanding but presents challenges in disentangling motion from appearance, modeling continuous-time dynamics, and obtaining costly physical annotations.

Method: PhyVLLM uses a dual-branch encoder to disentangle visual appearance and object motion, incorporates Neural ODEs to model continuous physical dynamics, projects motion-aware representations into LLM token space, and employs self-supervised learning to avoid explicit physical annotations.
...
Conclusion: Incorporating explicit physical motion modeling through disentangled representations and Neural ODEs enables Video LLMs to achieve better physical reasoning while maintaining general video understanding capabilities, addressing key limitations of appearance-based approaches.
...
Conclusion: Refaçade effectively addresses the object retexturing task with precise texture transfer capabilities for both images and videos, offering improved controllability over existing approaches.
...
TL;DR: Novel video-based facial analysis method for alcohol intoxication detection using Graph Attention Network and 3D ResNet with adaptive feature fusion, achieving 95.82% accuracy on a new dataset.
...
Method: Integrates facial landmark analysis via Graph Attention Network (GAT) with spatiotemporal visual features from 3D ResNet, using dynamic fusion with adaptive prioritization for enhanced classification.
...
Conclusion: The model demonstrates strong potential for practical deployment in public safety systems as a non-invasive, reliable alcohol intoxication detection solution.
...
TL;DR: X-Humanoid is a generative video editing approach that converts human videos into humanoid robot videos using a fine-tuned Wan 2.2 model, creating large-scale training data for embodied AI.


<details>
  <summary>Details</summary>
Motivation: Progress in Vision-Language-Action models and world models is limited by scarce training data. Existing methods that overlay robot arms on egocentric videos cannot handle complex full-body motions and scene occlusions in third-person videos, making them unsuitable for converting human videos to humanoid robot videos.

Method: Adapts Wan 2.2 model into video-to-video structure and fine-tunes it for human-to-humanoid translation. Created scalable data pipeline using Unreal Engine to generate 17+ hours of paired synthetic human-humanoid videos for training. Applied trained model to 60 hours of Ego-Exo4D videos to generate robotized humanoid videos.

Result: Generated over 3.6 million robotized humanoid video frames (new large-scale dataset). Quantitative analysis and user studies show superiority: 69% of users rated it best for motion consistency, and 62.1% for embodiment correctness.

Conclusion: X-Humanoid successfully bridges the gap in robotizing human videos by handling complex full-body motions and scene occlusions, providing a scalable solution for creating diverse training data for humanoid robot AI systems.
...
TL;DR: VideoMem: A novel framework for ultra-long video understanding using adaptive memory management and progressive RL optimization to handle videos beyond existing VLM context limits.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models fail at ultra-long video understanding due to limited context length and poor long-term memory retention. Current RAG-based approaches have excessive storage and computational overhead.

Method: VideoMem frames long video understanding as sequential generation with adaptive memory management. It uses a global memory buffer that dynamically retains critical information while discarding redundancy. Training employs Progressive Grouped Relative Policy Optimization (PRPO) with Progressive State Propagation (PSP) for state management and Temporal Cascading Reward (TCR) to address reward sparsity.
...
Conclusion: VideoMem provides an effective solution for ultra-long video understanding through adaptive memory management and efficient RL-based training, overcoming limitations of current VLMs and RAG systems.
...
<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting excels at rendering but struggles with accurate surface reconstruction. Well-reconstructed surfaces exhibit low configurational entropy, where dominant primitives define geometry clearly while redundant components are suppressed.
...
Result: Achieves competitive geometric precision on DTU and T&T benchmarks, with superior Chamfer Distance (0.64) on DTU and F1 score (0.44) on T&T. Also delivers best rendering quality on Mip-NeRF 360 with SSIM (0.855) and LPIPS (0.136).

Conclusion: GEF framework successfully enhances surface reconstruction accuracy in 3D Gaussian Splatting without compromising photometric fidelity, demonstrating the effectiveness of entropy-based regularization for geometric improvement.
...
TL;DR: This paper introduces a novel adversarial attack on Document Visual Question Answering (DocVQA) systems that forges document content in visually imperceptible ways to induce incorrect answers, demonstrating vulnerabilities in state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: While recent DocVQA models show impressive reasoning capabilities, they remain vulnerable to adversarial attacks. The authors aim to expose critical security vulnerabilities by creating attacks that forge document content in ways that are visually imperceptible but semantically targeted to manipulate model outputs.

Method: The authors develop specialized attack algorithms that produce adversarially forged documents tailored to different attacker goals. These attacks range from targeted misinformation to systematic model failure scenarios. They test their approach against two state-of-the-art models: Pix2Struct (vision-language transformer) and Donut (transformer-based document image processing model).

Result: The attacks successfully demonstrate vulnerabilities in current DocVQA systems, showing that both Pix2Struct and Donut can be manipulated to produce specific or generally incorrect answers through visually imperceptible document forgeries.

Conclusion: The findings highlight critical security vulnerabilities in current DocVQA systems and call for the development of more robust defenses against adversarial attacks that exploit visual-semantic manipulation of document content.
...
TL;DR: COOPER is a unified MLLM that enhances spatial reasoning by learning to generate auxiliary modalities (depth, segmentation) and performing adaptive interleaved reasoning, achieving significant improvements in spatial intelligence while maintaining general performance.
...
Method: COOPER is a two-stage training approach: 1) learns to generate auxiliary modalities (depth and segmentation) from RGB inputs, 2) learns adaptive interleaved reasoning where the model can dynamically choose between different reasoning strategies. The model leverages these auxiliary modalities as additional inputs for spatial understanding.

Result: COOPER achieves an average 6.91% improvement in spatial reasoning tasks while maintaining general performance. Even a variant trained only for auxiliary modality generation achieves 7.92% gain on distance and size estimation, showing that learning to generate auxiliary modalities helps internalize spatial knowledge.

Conclusion: A unified MLLM can develop intrinsic spatial intelligence by learning to generate auxiliary modalities and performing adaptive interleaved reasoning. This approach significantly enhances spatial understanding without sacrificing general capabilities, demonstrating that spatial perception and reasoning can be effectively integrated in a single model.
...
TL;DR: A comprehensive review providing practical guidance for creating high-quality, large-scale datasets for supervised deep learning in pathology image analysis, addressing challenges in image acquisition, annotation, and domain variability.


<details>
  <summary>Details</summary>
Motivation: The development and validation of DL models for microscopic image analysis requires high-quality datasets, but dataset creation is complex, resource-intensive, and hindered by challenges like time constraints, domain variability, and bias risks.

Method: The review provides a comprehensive guide covering: 1) image acquisition, 2) selection of annotation software, and 3) annotation creation. It addresses domain shifts, proposes quality criteria (correctness, completeness, consistency), explores advanced annotation techniques, and includes a standard operating procedure as supplemental material.

Result: The review establishes best practices for dataset development, emphasizes the importance of open datasets for innovation and reproducibility, and provides practical recommendations to overcome dataset creation challenges.

Conclusion: By addressing dataset creation challenges and offering practical guidance, this review aims to advance the availability of high-quality, large-scale datasets, ultimately contributing to the development of generalizable and robust DL models for pathology applications.
...
TL;DR: Robotic assembly task inspired by handmade crafts: building target objects using available objects that don't directly correspond to parts, with focus on object selection from RGB images.


<details>
  <summary>Details</summary>
Motivation: Inspired by traditional handmade crafts where people improvise assemblies using available objects that don't directly match target parts. The goal is to enable robotic systems to similarly assemble objects from available components.

Method: 1) Use mask segmentation network to identify visible parts from RGB image; 2) Retrieve labeled template meshes; 3) Pose optimization to find best template; 4) Simplify template parts to primitive shapes (cuboids/cylinders); 5) Design search algorithm for correspondences based on local/global proportions.

Result: Achieves comparable results to baselines (considering all possible combinations) for two different scenes. Shows qualitative results for real-world implementation.

Conclusion: Proposes a novel Craft Assembly Task for robotics, demonstrating a viable approach for selecting available objects to assemble target objects from RGB images, with promising results in both simulated and real-world scenarios.
...
TL;DR: TARDis is a physics-aware framework that treats missing CT phases as missing time points on continuous Time-Attenuation Curves, using disentangled static anatomical and dynamic perfusion features to hallucinate missing hemodynamic data.


<details>
  <summary>Details</summary>
Motivation: Clinical CT often has incomplete multi-phase series due to radiation concerns or scanning limitations, creating a "missing modality" problem. Existing methods treat missing phases as absent independent channels, ignoring the temporal continuity of contrast agent dynamics.

Method: TARDis redefines missing modalities as missing sample points on continuous Time-Attenuation Curves. It uses a dual-path architecture: (1) quantization-based path with learnable embedding dictionary for time-invariant anatomical features, and (2) probabilistic path using Conditional Variational Autoencoder for time-dependent perfusion features conditioned on estimated scan time.
...
Conclusion: TARDis demonstrates potential for reducing radiation exposure while maintaining diagnostic precision by effectively hallucinating missing hemodynamic features through physics-aware modeling of contrast agent dynamics as continuous temporal processes.
...
TL;DR: SiamDFF: A dynamic feature fusion Siamese network with feature enhancement and knowledge distillation for infrared UAV target tracking, achieving state-of-the-art performance with real-time speed.


<details>
  <summary>Details</summary>
Motivation: Infrared UAV targets have weak features and complex backgrounds, making accurate tracking challenging. Existing methods struggle with these issues in anti-UAV applications.

Method: Proposes SiamDFF with three key components: 1) STEN for adaptive region enhancement using intensity-aware multi-head cross-attention, 2) DSFAM for multi-scale feature integration with spatial attention, 3) DCFAM for template mixing to reduce background interference. Also introduces target-aware contextual attention knowledge distillation to transfer target priors from teacher to student network.

Result: Extensive experiments on real infrared UAV datasets show the approach outperforms state-of-the-art trackers under complex backgrounds while maintaining real-time tracking speed.

Conclusion: SiamDFF effectively addresses the challenges of infrared UAV tracking through dynamic feature fusion and knowledge distillation, achieving superior performance without additional computational burden.
...
TL;DR: SAM3-I enhances SAM3 by enabling direct instruction-following segmentation with rich natural language expressions instead of just noun phrases, while preserving original concept capabilities.


<details>
  <summary>Details</summary>
Motivation: SAM3's noun-phrase prompts are too coarse for real-world usage requiring rich expressions with attributes, spatial relations, functionalities, actions, states, and implicit reasoning. Current workarounds using external agents for iterative filtering are inefficient and imprecise.

Method: Introduces instruction-aware cascaded adaptation mechanism to align expressive instruction semantics with SAM3's vision-language representations. Creates structured instruction taxonomy (concept, simple, complex levels) and scalable data engine for diverse instruction-mask pairs.

Result: SAM3-I delivers appealing performance, effectively extending SAM3 to follow natural-language instructions while preserving strong concept grounding. Framework is open-sourced with practical fine-tuning workflows.

Conclusion: SAM3-I successfully unifies concept-level understanding and instruction-level reasoning within the SAM family, enabling direct instruction-following segmentation without sacrificing original capabilities.
...
TL;DR: The paper introduces AbstainEQA, a dataset for evaluating when embodied question answering agents should abstain from answering ambiguous questions, showing current models perform poorly compared to humans.


<details>
  <summary>Details</summary>
Motivation: Existing EQA benchmarks assume all questions must be answered, but embodied agents need to know when they lack sufficient information to answer reliably. Human analysis shows 32.4% of queries contain missing or underspecified context.

Method: Derived five abstention categories from human communication error theories, augmented OpenEQA by transforming well-posed questions into ambiguous variants, creating AbstainEQA with 1,636 annotated abstention cases paired with original instances.
...
Conclusion: Abstention is a fundamental prerequisite for reliable embodied interaction and necessary basis for effective clarification. Current models struggle significantly with this capability compared to humans.
...
<details>
  <summary>Details</summary>
Motivation: Current NSFW flags lack granularity - moderators need to know what specific objects make content illegal and where they are located for effective content moderation.

Method: Uses SAM for candidate object masks, refines into larger regions, scores each region for malicious relevance using vision-language models with open-vocabulary prompts, fuses scores into consolidated malicious object map, and employs ensemble segmentation for robustness against attacks.

Result: Achieves 85.8% element-level recall, 78.1% precision, 92.1% segment-success rate on 790-image dataset (drug, sexual, violent, extremist content). Outperforms direct VLM localization by 27.4% recall. Maintains robustness with ≤10% performance drop against PGD adversarial attacks.

Conclusion: First practical tool for fine-grained, explainable malicious-image moderation that processes images in seconds, integrates with existing VLM workflows, and demonstrates strong robustness against adaptive attacks.
...
TL;DR: HeFT is a zero-shot point tracking framework that uses pretrained video diffusion models' visual priors. It analyzes VDiT attention heads, selects informative heads and low-frequency features, and achieves state-of-the-art performance on TAP-Vid benchmarks without training data.


<details>
  <summary>Details</summary>
Motivation: To leverage the rich spatiotemporal priors in pretrained video diffusion models for point tracking tasks without requiring annotated training data, and to understand how these models encode visual information for better feature extraction.

Method: Analyzes VDiT attention heads to discover their specializations (matching, semantic, positional), identifies low-frequency components as crucial for correspondences, proposes head- and frequency-aware feature selection, extracts features via single-step denoising, and uses soft-argmax localization with forward-backward consistency checks.

Result: HeFT achieves state-of-the-art zero-shot tracking performance on TAP-Vid benchmarks, approaching supervised method accuracy while eliminating the need for annotated training data.

Conclusion: Video diffusion models serve as powerful foundation models for downstream tasks like point tracking, demonstrating the promise of unified visual foundation models through zero-shot adaptation of their rich visual priors.
...
TL;DR: I2I-Bench is a comprehensive benchmark for image-to-image editing models that addresses limitations of existing benchmarks through diverse tasks, automated evaluation dimensions, and validation of human alignment.
...
Conclusion: I2I-Bench provides a comprehensive, scalable benchmark for evaluating image editing models, addressing limitations of existing evaluation approaches and facilitating future research through open-source availability.
...
TL;DR: Live Avatar: A real-time streaming avatar generation system using a 14B diffusion model with novel parallelization techniques for infinite-length, high-fidelity video synthesis.
...
Method: 1) Timestep-forcing Pipeline Parallelism (TPP) - distributes denoising steps across multiple GPUs to break autoregressive bottlenecks; 2) Rolling Sink Frame Mechanism (RSFM) - maintains temporal consistency and mitigates identity drift using cached reference images; 3) Self-Forcing Distribution Matching Distillation - enables causal, streamable adaptation of large models without quality loss.
...
TL;DR: CVPC task and CrossPoint-Bench benchmark reveal VLMs' poor point-level cross-view correspondence, with 54.65% gap vs humans. CrossPoint-378K dataset and CroPond model address this, achieving 39.7% improvement over Gemini-2.5-Pro.
...
Method: 1) Propose Cross-View Point Correspondence (CVPC) task and CrossPoint-Bench benchmark with hierarchical design (perceive, reason, correspond). 2) Construct CrossPoint-378K dataset with 378K QA pairs across 900 scenes focused on actionable affordance regions. 3) Develop CroPond model trained on CrossPoint-378K.

Result: State-of-the-art models like Gemini-2.5-Pro show 54.65% accuracy gap compared to humans. CroPond achieves state-of-the-art performance on CrossPoint-Bench, surpassing Gemini-2.5-Pro by 39.7% accuracy.

Conclusion: The work exposes VLMs' limitations in fine-grained cross-view correspondence, provides benchmark/dataset for evaluation, and demonstrates CroPond's effectiveness in bridging the gap, offering foundation for future embodied AI research.
...
TL;DR: OmniScaleSR is a diffusion-based framework for realistic arbitrary-scale super-resolution that achieves both high fidelity and realism by combining explicit scale control with implicit diffusion adaptation.


<details>
  <summary>Details</summary>
Motivation: Existing arbitrary-scale SR methods using implicit neural representations have limited ability to synthesize fine details, while diffusion-based Real-ISR models lack explicit scale control, causing hallucination or blurry outputs at different magnification levels.
...
Conclusion: OmniScaleSR effectively addresses the limitations of both INR-based and diffusion-based approaches for arbitrary-scale SR by combining explicit scale control with diffusion priors, achieving superior results across various magnification levels.
...
TL;DR: The paper proposes a complete ecosystem for generative psychological analysis of conversations, addressing articulatory-affective ambiguity in VLMs and lack of verifiable evaluation metrics through a hierarchical visual encoder (MIND), a new dataset (ConvoInsight-DB), and an automated evaluation framework (PRISM).


<details>
  <summary>Details</summary>
Motivation: Two fundamental challenges in generative psychological analysis of conversations: (1) Vision-Language Models fail to resolve Articulatory-Affective Ambiguity where visual speech patterns mimic emotional expressions, and (2) lack of verifiable evaluation metrics for assessing visual grounding and reasoning depth.

Method: Three-part ecosystem: (1) Multilevel Insight Network for Disentanglement (MIND) - hierarchical visual encoder with Status Judgment module to suppress ambiguous lip features based on temporal variance; (2) ConvoInsight-DB - large-scale dataset with expert annotations for micro-expressions and psychological inference; (3) Mental Reasoning Insight Rating Metric (PRISM) - automated dimensional framework using expert-guided LLM to measure multidimensional performance.

Result: MIND significantly outperforms all baselines on the PRISM benchmark, achieving +86.95% gain in micro-expression detection over prior state-of-the-art. Ablation studies confirm the Status Judgment disentanglement module is the most critical component for this performance leap.

Conclusion: The proposed ecosystem successfully addresses the twin challenges of articulatory-affective ambiguity and evaluation metrics for generative psychological analysis, with MIND demonstrating substantial improvements in micro-expression detection and the code being made publicly available.
...
TL;DR: E3AD is an emotion-aware vision-language-action framework for autonomous driving that incorporates passenger emotional state into trajectory planning using continuous emotion modeling and dual-pathway spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving systems ignore passenger emotional states, which are crucial for comfort and AD acceptance. The paper introduces Open-Domain End-to-End autonomous driving where vehicles must interpret natural language commands, infer emotion, and plan physically feasible trajectories.

Method: Proposes E3AD framework with two cognitively inspired components: 1) continuous Valence-Arousal-Dominance (VAD) emotion model to capture tone and urgency from language, and 2) dual-pathway spatial reasoning module that fuses egocentric and allocentric views for human-like spatial cognition. Uses consistency-oriented training combining modality pretraining with preference-based alignment.

Result: Across real-world datasets, E3AD improves visual grounding and waypoint planning and achieves state-of-the-art VAD correlation for emotion estimation. Shows that injecting emotion into VLA-style driving yields more human-aligned grounding, planning, and human-centric feedback.

Conclusion: Incorporating emotional awareness into autonomous driving systems leads to more human-aligned behavior, better visual grounding, improved planning, and enhanced human-centric feedback, demonstrating the importance of emotional intelligence in AD systems.
...
TL;DR: Instance-aware depth completion framework using binary instance masks as spatial priors to refine depth predictions, outperforming U-Net baseline and semantic-guided methods on Virtual KITTI 2 dataset.


<details>
  <summary>Details</summary>
Motivation: Existing depth completion approaches often rely on semantic segmentation but overlook object-level understanding. The paper aims to leverage instance-aware cues to improve depth completion without requiring dense semantic labels.

Method: Four-component framework: 1) frozen YOLO V11 instance segmentation branch for binary instance masks, 2) U-Net-based depth completion backbone, 3) cross-attention fusion module to integrate instance guidance, and 4) attention-guided prediction head for refinement.

Result: Achieves lower RMSE compared to U-Net-only baseline and previous semantic-guided methods on Virtual KITTI 2 dataset, while maintaining competitive MAE. Effectively enhances depth accuracy near object boundaries, occlusions, and thin structures.

Conclusion: Incorporating instance-aware cues offers a promising direction for improving depth completion without relying on dense semantic labels, demonstrating the value of object-level understanding in depth refinement.
...
TL;DR: VRSketch2Shape: A framework for generating 3D shapes from sequential VR sketches using temporal stroke ordering, with a new dataset and order-aware diffusion model.


<details>
  <summary>Details</summary>
Motivation: Existing sketch-to-shape models ignore temporal ordering of strokes, discarding crucial structural and design intent cues. VR sketching offers faster, more intuitive 3D design but needs better modeling of sequential stroke information.
...
TL;DR: PaCo-RL is a reinforcement learning framework for consistent image generation that combines a specialized consistency reward model (PaCo-Reward) with an efficient RL algorithm (PaCo-GRPO) to learn complex visual consistency criteria without large supervised datasets.


<details>
  <summary>Details</summary>
Motivation: Supervised training struggles with consistent image generation due to lack of large-scale consistency datasets and difficulty modeling subjective human perceptual preferences. RL offers a data-free alternative to learn these complex visual criteria.

Method: Two components: 1) PaCo-Reward - pairwise consistency evaluator trained on automated sub-figure pairing dataset, using generative autoregressive scoring with task-aware instructions and CoT reasoning. 2) PaCo-GRPO - efficient RL algorithm with resolution-decoupled optimization to reduce cost and log-tamed multi-reward aggregation for balanced optimization.
...
Conclusion: PaCo-RL demonstrates promise as a practical and scalable solution for consistent image generation, addressing key challenges in preserving identities, styles, and logical coherence across multiple images.
...
TL;DR: LaFiTe introduces a 3D-native texturing framework using a sparse latent color field representation that achieves >10 dB PSNR improvement over state-of-the-art methods, enabling high-fidelity texture generation directly on 3D surfaces without UV mapping limitations.
...
Result: Achieves unprecedented fidelity with >10 dB PSNR improvement in reconstruction over state-of-the-art methods. Effectively disentangles texture appearance from mesh topology and UV parameterization, enabling flexible downstream applications.

Conclusion: LaFiTe sets a new benchmark for 3D-native texturing and enables next-generation 3D content creation workflows through its powerful sparse latent representation and flexible texture synthesis capabilities.
...
TL;DR: EMMA is an efficient unified multimodal architecture that achieves state-of-the-art performance in understanding, generation, and editing tasks with only 4B parameters, outperforming larger models through innovative compression, token reduction, and task-sharing techniques.


<details>
  <summary>Details</summary>
Motivation: To create an efficient unified architecture that can handle multimodal understanding, generation, and editing tasks simultaneously, addressing the inefficiency and imbalance issues in existing approaches that often require separate specialized models or suffer from training imbalances.

Method: Four key innovations: 1) Efficient autoencoder with 32x compression ratio to reduce token count, 2) Channel-wise concatenation instead of token-wise to further reduce visual tokens, 3) Shared-and-decoupled network for mutual task improvements while maintaining task-specific requirements, 4) Mixture-of-experts mechanism in visual encoder to enhance perceptual capabilities with minimal parameter increase.

Result: EMMA-4B significantly outperforms state-of-the-art unified multimodal approaches like BAGEL-7B in both efficiency and performance, while achieving competitive results compared to specialized multimodal understanding and generation experts such as Qwen3-VL and Qwen-Image.

Conclusion: EMMA provides a solid foundation for future unified multimodal architectures by demonstrating that efficient, high-performance multimodal systems can be achieved through careful architectural design that balances compression, token efficiency, task sharing, and specialized capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with in-the-wild scenes containing transient objects (moving objects) and illumination variations, leading to artifacts in rendered images. The Gaussian densification process unintentionally models these transient disturbances, causing quality issues.

Method: Three key designs: 1) Delayed Gaussian growth strategy that prioritizes optimizing static structure before allowing splitting/cloning; 2) Scale-cascaded mask bootstrapping that starts with lower-resolution feature similarity for reliable transient mask estimation, then progresses to high-resolution supervision; 3) Integration of delayed growth and mask bootstrapping with appearance modeling.

Result: Extensive experiments on multiple challenging datasets show the method outperforms existing approaches, demonstrating robustness and effectiveness in handling in-the-wild scenes with transients and illumination variations.

Conclusion: RobustSplat++ provides a robust solution for 3DGS in challenging real-world scenarios by preventing overfitting to transient elements through delayed Gaussian optimization and multi-scale mask estimation, achieving superior performance compared to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the strong generative capabilities of flow matching (FM) for medical image segmentation, enabling uncertainty-aware predictions and diverse segmentation outputs while operating efficiently in latent space.

Method: 1. Design two VAEs to encode medical images and masks into lower-dimensional latent space. 2. Estimate conditional velocity field based on input image to guide the flow. 3. Sample multiple latent representations to synthesize diverse segmentation outputs with pixel-wise variance estimation.

Result: Achieves superior segmentation accuracy on ISIC-2018 and CVC-Clinic datasets compared to prior deterministic and generative baselines. Generates confidence maps for uncertainty quantification and remains highly efficient in latent space.

Conclusion: LatentFM demonstrates that flow-based models in latent space can provide accurate, uncertainty-aware medical image segmentation with diverse outputs and confidence quantification, offering richer information for clinical analysis.
...
TL;DR: FreeGen: A co-training framework for free-viewpoint driving scene synthesis that combines reconstruction for geometric consistency with generation for visual realism.


<details>
  <summary>Details</summary>
Motivation: Existing datasets and generative models for autonomous driving simulation lack consistent off-trajectory observations, limiting large-scale evaluation and training. Current methods struggle to balance interpolation consistency and extrapolation realism without per-scene optimization.
...
Result: FreeGen achieves state-of-the-art performance for free-viewpoint driving scene synthesis, demonstrating improved interpolation consistency and extrapolation realism without requiring per-scene optimization.

Conclusion: The proposed co-training framework successfully addresses the limitations of existing methods by combining reconstruction and generation models, enabling scalable pre-training and closed-loop simulation for autonomous driving with consistent free-viewpoint scene synthesis.
...
TL;DR: SBM is a Transformer-based model for BIM layout synthesis that tokenizes buildings by unifying architectural elements into sequences, learns joint representations of categorical/continuous features, and operates in encoder-only (room embeddings) and encoder-decoder (autoregressive prediction) modes.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of tokenizing buildings for layout synthesis in BIM scenes, particularly how to unify heterogeneous feature sets of architectural elements while preserving compositional structure.

Method: 1) Represent building features as sparse attribute-feature matrices capturing room properties; 2) Design unified embedding module for joint categorical/continuous feature representations; 3) Train Transformer backbone in two modes: encoder-only for room embeddings and encoder-decoder for autoregressive Data-Driven Entity Prediction (DDEP).
...
Conclusion: SBM effectively addresses building tokenization for BIM layout synthesis, providing both high-quality room embeddings for retrieval and generative capabilities for producing functional architectural layouts.
...
TL;DR: Proposes DevDet framework for multi-domain face forgery detection to address domain-dominant issues in real-world deepfake detection scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detectors struggle with real-world applications due to domain dominance in multi-domain training scenarios - domain differences overshadow real/fake distinctions, leading to poor single-image judgment accuracy despite high AUC within domains.
...
Conclusion: The proposed approach effectively addresses domain-dominant issues in multi-domain deepfake detection, enabling better real-world frame-by-frame independent detection performance.
...
TL;DR: LineAR is a training-free KV cache compression method for autoregressive image generation that reduces memory usage and increases throughput by managing cache at line level while maintaining generation quality.
...
Method: LineAR uses a progressive KV cache compression pipeline that manages cache at line level with a 2D view, preserving visual dependency regions while evicting less-informative tokens guided by inter-line attention.

Result: LineAR achieves significant improvements: ImageNet FID from 2.77 to 2.68, COCO FID from 23.85 to 22.86 with only 1/6 KV cache; up to 67.61% memory reduction and 7.57x speedup on LlamaGen-XL; 39.66% memory reduction and 5.62x speedup on Janus-Pro-7B.

Conclusion: LineAR enables efficient AR image generation with substantial memory savings and throughput improvements while maintaining or even enhancing generation quality, validated across multiple AR image generation models.
...
TL;DR: BioTUCH: A framework combining visual pose estimation with bioimpedance sensing to improve 3D human pose reconstruction accuracy by incorporating self-contact information.


<details>
  <summary>Details</summary>
Motivation: Video-based pose estimation often fails in scenarios involving self-contact (e.g., hand touching face), while wearable bioimpedance sensing can cheaply measure ground-truth skin-to-skin contact. Combining these approaches could provide more accurate 3D human pose capture in the wild.

Method: BioTUCH initializes pose using off-the-shelf estimators, then performs contact-aware pose optimization during measured self-contact. It minimizes reprojection error and deviations from input estimates while enforcing vertex proximity constraints based on bioimpedance measurements.

Result: Validated on a new dataset of synchronized RGB video, bioimpedance measurements, and 3D motion capture. Demonstrated average 11.7% improvement in reconstruction accuracy across three input pose estimators. Also presented a miniature wearable bioimpedance sensor for large-scale data collection.

Conclusion: BioTUCH effectively combines visual pose estimation with bioimpedance sensing to improve 3D human pose reconstruction by incorporating self-contact information, with potential for large-scale training data collection to enhance pose estimation and generation methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing promptable detection frameworks for chest X-ray lesion detection rely on manual expert annotations as prompts, which are labor-intensive and impractical for clinical applications. There's a need for automated prompt generation to make these systems more practical.

Method: SP-Det introduces: 1) Expert-free dual-text prompt generator (DTPG) with semantic context prompts (global pathological patterns) and disease beacon prompts (disease-specific manifestations); 2) Bidirectional feature enhancer (BFE) that integrates diagnostic context with disease-specific embeddings.

Result: Extensive experiments on two chest X-ray datasets with diverse thoracic disease categories show SP-Det outperforms state-of-the-art detection methods while completely eliminating dependency on expert-annotated prompts.

Conclusion: SP-Det provides a practical solution for automated lesion detection in chest X-rays by generating its own prompts, making promptable detection frameworks more viable for clinical applications without requiring labor-intensive manual annotations.
...
TL;DR: SDG-Track is a real-time UAV tracking system for edge devices that solves the resolution-speed conflict using an Observer-Follower architecture with sparse optical flow and training-free recovery mechanisms.


<details>
  <summary>Details</summary>
Motivation: Real-time UAV tracking on edge devices faces a fundamental resolution-speed conflict: downsampling high-resolution imagery causes small target features to become undetectable, while processing native 1080p frames yields insufficient throughput for smooth gimbal control.
...
Conclusion: SDG-Track effectively reconciles the resolution-speed conflict for real-time UAV tracking on edge devices through a novel Observer-Follower architecture with sparse optical flow and training-free recovery mechanisms, enabling practical deployment on resource-constrained platforms.
...
TL;DR: YOTO framework addresses catastrophic forgetting in object detection by combining YOLO11n for localization with DeIT and Proxy Anchor Loss for feature extraction, achieving 3x training efficiency and 580ms inference time on edge devices.


<details>
  <summary>Details</summary>
Motivation: Object detection faces catastrophic forgetting when new products are introduced, requiring full retraining with both old and new datasets. This leads to high training costs and time consumption, especially problematic in retail checkout where new products are frequently added.

Method: YOTO integrates YOLO11n for object localization with DeIT and Proxy Anchor Loss for feature extraction and metric learning. Classification uses cosine similarity between target product embeddings and those stored in a Qdrant vector database, avoiding retraining when new products are added.

Result: In a retail case study with 140 products, the framework achieved encouraging accuracy for both new and existing products. Training efficiency was nearly 3x faster than classical approaches, with efficiency improving as more products are added. Average inference time was 580ms per multi-product image on edge devices.

Conclusion: YOTO effectively addresses catastrophic forgetting in object detection, significantly reducing training time and costs while maintaining accuracy. The framework is practical for real-world applications like retail checkout where new products are frequently introduced.
...
TL;DR: E(3)-Pose is a fast pose estimation method that jointly models rotation equivariance and object symmetry for fetal head pose estimation in MRI scans, enabling automatic adaptive prescription of 2D diagnostic slices.
...
Method: E(3)-Pose jointly and explicitly models rotation equivariance and object symmetry by construction. It captures anatomical symmetries and rigid pose equivariance to yield robust estimates of fetal head pose, overcoming challenges like pose ambiguities from anatomical symmetries, low resolution, noise, and artifacts.
...
Conclusion: E(3)-Pose provides a robust solution for fetal head pose estimation in MRI by explicitly modeling rotation equivariance and object symmetry, enabling clinical translation for automatic adaptive prescription of diagnostic slices.
...
TL;DR: ReflexFlow: A reflexive refinement method for Flow Matching that addresses exposure bias through Anti-Drift Rectification and Frequency Compensation, achieving significant quality improvements across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Flow Matching methods suffer from exposure bias due to discrepancies between training and inference, caused by lack of generalization to biased inputs and insufficient low-frequency content capture during early denoising.

Method: Proposes ReflexFlow with two components: 1) Anti-Drift Rectification (ADR) that adjusts prediction targets for biased inputs using redesigned loss with training-time scheduled sampling, and 2) Frequency Compensation (FC) that reflects on missing low-frequency components and compensates them by reweighting the loss using exposure bias.

Result: ReflexFlow outperforms prior approaches in mitigating exposure bias, achieving a 35.65% reduction in FID on CelebA-64, with improvements demonstrated on CIFAR-10, CelebA-64, and ImageNet-256 datasets.

Conclusion: ReflexFlow is a simple, effective, model-agnostic solution that dynamically corrects exposure bias in Flow Matching, compatible with all Flow Matching frameworks and improves generation quality across diverse datasets.
...
TL;DR: SFD introduces a semantic-first diffusion paradigm that asynchronously denoises semantic and texture latents with temporal offset, achieving state-of-the-art FID scores and 100x faster convergence.


<details>
  <summary>Details</summary>
Motivation: Current LDMs denoise semantic and texture information synchronously despite natural coarse-to-fine generation where semantics form before textures, missing the opportunity for semantic guidance to improve texture generation.

Method: Constructs composite latents with compact semantic latent (from pretrained encoder via Semantic VAE) and texture latent, then denoises them asynchronously using separate noise schedules with semantics preceding textures by temporal offset.
...
Conclusion: Asynchronous, semantics-led modeling in latent diffusion explicitly prioritizes semantic formation, providing clearer high-level guidance for texture refinement and enabling more natural coarse-to-fine generation.
...
<details>
  <summary>Details</summary>
Motivation: Herculaneum Papyri contain valuable unseen Greek/Latin texts but are too fragile to physically unroll. Virtual unrolling via CT scanning is needed, but manual tracing is extremely laborious in gigavoxel-sized scans, requiring automated approaches.

Method: Top-down approach that globally fits an explicit parametric model of the deformed scroll to existing neural network predictions of where the rolled papyrus likely passes. Guarantees a single continuous 2D sheet even through regions where surface is not detectable.

Result: Method successfully unrolls large regions of high-resolution CT scans of two scrolls and exceeds performance of the only existing automated unrolling method suitable for this data.

Conclusion: The presented automated surface fitting approach enables effective virtual unrolling of severely damaged Herculaneum Papyri, providing access to previously inaccessible ancient texts through computational methods.
...
TL;DR: LiteVGGT is an efficient 3D vision foundation model that achieves 10x speedup and significant memory reduction for large-scale scene processing by using geometry-aware cached token merging.
...
Method: Proposes geometry-aware cached token merging with two key insights: (1) tokens from local image regions have inherent geometric correlations leading to redundancy, and (2) token similarity across adjacent layers remains stable. The method analyzes geometric importance for anchor token selection and caches/reuses merge indices across layers.

Result: Achieves up to 10x speedup and substantial memory reduction, enabling efficient processing of 1000-image scenes while retaining VGGT's core performance. Also supports efficient fine-tuning and FP8 quantization for further gains.

Conclusion: LiteVGGT provides an effective, scalable, and robust solution for efficient 3D reconstruction at scale, overcoming computational limitations of existing foundation models for large-scale scene processing.
...
TL;DR: A novel multimodal action recognition method using deep neural networks with adaptive gating mechanisms for RGB, optical flow, audio, and depth data fusion, outperforming traditional unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of traditional unimodal action recognition methods by leveraging multiple modalities for more comprehensive action understanding and enabling sophisticated applications in surveillance and human-computer interaction.

Method: Deep neural network architecture with adaptive fusion strategies using gating mechanisms to selectively integrate relevant information from RGB, optical flows, audio, and depth modalities.

Result: Superior performance over conventional unimodal methods across human action recognition, violence detection, and self-supervised learning tasks on benchmark datasets with promising accuracy improvements.

Conclusion: The gating-based multimodal fusion approach represents a significant advancement with potential to revolutionize action recognition systems, particularly for applications in surveillance and active assisted living.
...
TL;DR: FASTER framework improves VLA models by introducing efficient action tokenization with better reconstruction fidelity and faster inference, outperforming previous SOTA in both speed and performance.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive VLA models face a trade-off between reconstruction fidelity and inference efficiency in their action tokenization process, limiting their practical deployment in robotics.

Method: FASTER integrates a learnable tokenizer (FASTERVQ) that encodes action chunks as single-channel images to capture spatio-temporal dependencies, combined with an autoregressive policy (FASTERVLA) using block-wise decoding and lightweight action expert.

Result: FASTERVQ achieves superior reconstruction quality, high token utilization, and strong cross-task/cross-embodiment generalization. FASTERVLA surpasses previous SOTA VLA models in both inference speed and task performance across simulated and real-world benchmarks.

Conclusion: The FASTER framework provides an efficient and generalizable solution for robot learning that balances reconstruction fidelity with inference efficiency, enabling practical deployment of VLA models in robotics.
...
TL;DR: GeoPE extends RoPE to 2D vision by using quaternions in 3D Euclidean space with geometric mean in Lie algebra, outperforming existing 2D RoPE variants across vision tasks.


<details>
  <summary>Details</summary>
Motivation: Standard ViTs flatten 2D images into 1D sequences, disrupting spatial topology. Existing 2D RoPE approaches treat spatial axes independently, failing to decouple false sequential proximity from true spatial distance.
...
Result: Extensive experiments on image classification, object detection, and 3D semantic segmentation demonstrate GeoPE consistently outperforms existing 2D RoPE variants and significantly enhances shape bias.

Conclusion: GeoPE effectively restores 2D spatial manifold and captures true geometric structure, confirming its ability to separate spatial dimensions and improve vision transformer performance.
...
TL;DR: A balanced few-shot learning framework for retinal disease diagnosis using balanced episodic sampling, targeted augmentation (CLAHE), and ResNet-50 achieves improved accuracy and reduces bias toward majority classes in imbalanced datasets.


<details>
  <summary>Details</summary>
Motivation: Automated retinal disease diagnosis faces challenges due to costly annotated datasets and class imbalance, limiting reliability. Few-shot learning can address data constraints, but existing approaches may still suffer from bias toward majority disease classes.

Method: Proposes a balanced few-shot episodic learning framework with three components: (1) balanced episodic sampling ensuring equal class participation in 5-way 5-shot episodes, (2) targeted augmentation including CLAHE and color/geometry transformations for minority-class diversity, and (3) ResNet-50 encoder pretrained on ImageNet for fine-grained feature capture. Uses prototype computation in embedding space with cosine similarity classification.

Result: The framework achieves substantial accuracy gains and reduces bias toward majority classes, with notable improvements for underrepresented diseases. Trained on 100 episodes and evaluated on 1,000 test episodes on the RFMiD dataset's ten most represented classes.

Conclusion: Dataset-aware few-shot pipelines with balanced sampling and CLAHE-enhanced preprocessing can deliver more robust and clinically fair retinal disease diagnosis under data-constrained conditions, addressing class imbalance challenges in medical imaging.
...
<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detection methods primarily use final-layer CLIP-ViT features, but the authors discovered earlier layers provide more localized and generalizable features that often surpass final-layer performance. Different layers capture distinct aspects of data, suggesting untapped potential in multi-layer feature integration.

Method: MoLD (Multi-layer adaptive Detection) - a novel adaptive method that dynamically integrates features from multiple ViT layers using a gating-based mechanism. The approach systematically analyzes layer-wise feature contributions and creates an adaptive integration framework that can be applied to various pre-trained ViTs.

Result: Extensive experiments on GAN- and diffusion-generated images show MoLD significantly improves detection performance, enhances generalization across diverse generative models, and exhibits robustness in real-world scenarios. The method also demonstrates scalability by successfully applying to other pre-trained ViTs like DINOv2.

Conclusion: Multi-layer feature integration via adaptive gating mechanisms provides superior AI-generated image detection compared to final-layer only approaches, offering better performance, generalization, and robustness across different generative models and real-world applications.
...
TL;DR: A stable contrastive learning method for pixel-level representations that captures both semantic and geometric information without momentum-based teacher-student training.


<details>
  <summary>Details</summary>
Motivation: To learn pixel-level representations that jointly capture semantic and geometric information in a stable manner without requiring complex momentum-based teacher-student architectures.

Method: Uses a family of stable contrastive losses to map each pixel to an overcomplete descriptor that is both view-invariant and semantically meaningful. The approach avoids momentum-based teacher-student training.

Result: The method enables precise point-correspondence across images and is demonstrated through experiments in synthetic 2D and 3D environments, showing the properties of the loss and resulting overcomplete representations.

Conclusion: The proposed stable contrastive learning approach successfully learns pixel-level representations that capture both semantic and geometric information, enabling accurate point correspondence without complex momentum-based training schemes.
...
TL;DR: LVLM-based text-to-image models produce more socially biased images than non-LVLM models, with system prompts identified as the primary driver. The paper introduces FairPro, a training-free meta-prompting framework that enables LVLMs to self-audit and construct fairness-aware prompts.


<details>
  <summary>Details</summary>
Motivation: While LVLM-based T2I systems dominate image generation, their tendency to amplify social biases remains insufficiently understood. The paper aims to systematically investigate and address demographic bias in these models.
...
<details>
  <summary>Details</summary>
Motivation: VANICP is efficient for point cloud registration but has high memory requirements that prevent deployment on resource-constrained embedded systems with limited GPU memory.

Method: Proposes a GPU-oriented dynamic memory assignment strategy that optimizes memory usage of the dilation operation, creating an enhanced VANICP framework with reduced memory footprint.

Result: Achieves over 97% reduction in memory consumption while preserving the original performance of VANICP, enabling deployment on embedded GPUs.

Conclusion: The memory-efficient optimization makes high-performance point cloud registration feasible on resource-constrained embedded systems without sacrificing computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the generalization capabilities of foundation diffusion models for reflection removal, addressing the shortage of suitable training data and moving beyond task-specific architectures.

Method: Repurpose pre-trained DiT-based foundation model by conditioning on reflection-contaminated inputs and guiding toward clean transmission layers. Create PBR pipeline in Blender for realistic synthetic data. Use efficient LoRA-based adaptation of foundation model.
...
Conclusion: Pretrained diffusion transformers, combined with physically grounded data synthesis and efficient adaptation, provide an effective solution for reflection removal that generalizes well across domains.
...
<details>
  <summary>Details</summary>
Motivation: Transparent objects are challenging for depth sensors due to refraction/reflection. Supervised methods require expensive depth annotations. Need cost-effective training approach.
...
Conclusion: Proposed self-supervised method effectively trains depth completion networks for transparent objects without expensive annotations, performing comparably to supervised methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing perceptual codecs rely on image generative priors that lack temporal modeling, causing perceptual flickering. There's a need for video-native generative priors that can ensure temporal coherence while removing compression artifacts.

Method: Uses a unified flow-matching latent refinement module with video diffusion transformer for joint intra- and inter-frame enhancement. Initializes refinement from decoded latents (not pure noise) and learns a correction term for compression degradation. Includes conditioning adaptor to inject compression-aware cues into DiT layers.

Result: Surpasses both traditional and learned codecs in perceptual quality, significantly reduces flickering artifacts even below 0.01 bpp, and maintains temporal coherence under extreme bitrate constraints.

Conclusion: GNVC-VD demonstrates the promise of integrating video-native generative priors into neural codecs for next-generation perceptual video compression, effectively addressing temporal coherence issues that plagued previous approaches.
...
TL;DR: HTR-ConvText: A hybrid CNN-ViT model for handwritten text recognition that combines stroke-level local features with global context, achieving better generalization with limited data.


<details>
  <summary>Details</summary>
Motivation: Handwritten Text Recognition faces challenges due to limited data, high writing style variance, and complex diacritics. Existing methods struggle to generalize without massive synthetic data.
...
TL;DR: RAMEN is a resolution-adjustable multimodal encoder that learns shared visual representations across diverse Earth observation data in a sensor-agnostic way, enabling coherent analysis across modalities with controllable spatial resolution as an output parameter.
...
Conclusion: RAMEN provides a unified, sensor-agnostic approach for Earth observation data analysis that enables explicit trade-offs between spatial precision and computational cost while maintaining strong generalization across diverse EO modalities.
...
TL;DR: Proposes a semantic-guided hierarchical synthesis architecture for facial image inpainting that handles large irregular masks better than existing methods by combining local CNN and global Vision Transformer features for semantic layout generation, followed by multi-modal texture refinement.
...
Method: Two-stage semantic-guided hierarchical synthesis: 1) Semantic layout generation combining local CNN features and global Vision Transformer features, 2) Multi-Modal Texture Generator that refines layouts using multi-scale information. Handles arbitrary masks through dynamic attention without mask-specific training.
...
Conclusion: The proposed architecture effectively addresses challenges in facial image inpainting by leveraging semantic guidance and hierarchical synthesis, producing high-quality results with preserved identity and structural consistency even for large irregular masks.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for 4D scene generation from single images decouple geometry from motion, leading to spatiotemporal inconsistencies and poor generalization. There's also a scarcity of high-quality 4D scene data for training.

Method: 1) Created TrajScene-60K dataset with 60K video samples and dense point trajectories. 2) Proposed 4D-STraG diffusion model to generate geometrically consistent 4D point trajectories. 3) Designed depth-guided motion normalization and motion-aware module for geometry-dynamics integration. 4) Developed 4D-ViSM to render videos from 4D point track representations with arbitrary camera trajectories.

Result: MoRe4D generates high-quality 4D scenes with multi-view consistency and rich dynamic details from single images, outperforming existing methods in producing spatiotemporally consistent results.

Conclusion: The proposed MoRe4D framework successfully addresses the limitations of existing 4D scene generation methods by jointly modeling motion and geometry, enabled by a new large-scale dataset and diffusion-based trajectory generation approach.
...
TL;DR: 4DLangVGGT: First Transformer-based feed-forward framework for 4D language grounding that jointly integrates geometric perception and language alignment, enabling efficient open-vocabulary 4D scene understanding without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 4D semantic field construction methods rely on scene-specific Gaussian splatting, which requires per-scene optimization, has limited generalization, and is difficult to scale to real-world applications. There's a need for a more practical approach that can handle dynamic scenes efficiently.

Method: Proposes 4DLangVGGT with two key components: 1) 4D Visual Geometry Transformer (StreamVGGT) captures spatio-temporal geometric representations of dynamic scenes; 2) Semantic Bridging Decoder (SBD) projects geometry-aware features into language-aligned semantic space. The framework is jointly trainable across multiple scenes and directly applicable during inference.

Result: Achieves state-of-the-art performance on HyperNeRF and Neu3D datasets with up to 2% gains under per-scene training and 1% improvements under multi-scene training. Demonstrates strong generalization and deployment efficiency compared to prior methods.

Conclusion: 4DLangVGGT establishes a new paradigm for open-vocabulary 4D scene understanding by providing a unified, feed-forward framework that eliminates costly per-scene optimization, improves scalability, and enhances practical deployment for embodied AI and AR/VR applications.
...
TL;DR: A 4D-controllable video diffusion framework that decouples scene dynamics from camera motion, enabling independent control over both aspects through continuous world-time sequences and camera trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models couple scene dynamics with camera motion, limiting precise spatial and temporal control. There's a need for fine-grained manipulation of both scene dynamics and camera viewpoint independently.

Method: Uses a 4D-controllable video diffusion framework with continuous world-time sequences and camera trajectories as conditioning inputs. Implements 4D positional encoding in attention layers and adaptive normalizations for feature modulation. Trained on a unique dataset with independently parameterized temporal and camera variations.
...
Conclusion: The framework successfully decouples scene dynamics from camera motion, enabling precise 4D control over video generation while maintaining visual fidelity, representing an advancement in controllable video synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Object reconstruction is challenging due to partial observations from cameras, especially with occlusion. Vision signals alone are ambiguous for complete geometry reconstruction.

Method: Combines two information sources: 1) shape priors from generative models for unseen geometry, and 2) contact information from videos/physical interactions as sparse constraints. Uses contact-guided 3D generation inspired by drag-based editing in generative models.
...
Conclusion: Integrating shape priors with contact constraints through guided generation effectively reduces ambiguity and enhances object geometry reconstruction for robot manipulation.
...
<details>
  <summary>Details</summary>
Motivation: Existing autoregressive video diffusion methods suffer from temporal repetition, drift, and motion deceleration during long rollouts. StreamingLLM-style attention sinks degrade video fidelity and cause motion stagnation.

Method: Two training-free mechanisms: 1) Deep Sink dedicates half sliding window to persistent sink tokens with re-aligned temporal RoPE phase; 2) Participative Compression performs importance-aware KV cache pruning to preserve active tokens and discard redundant history.

Result: Enables 12x extrapolation (5s-trained to 60s+ generation) with better imaging quality than LongLive, better aesthetic quality than RollingForcing, maintains overall consistency, improves dynamic degree, and maintains real-time generation.

Conclusion: Training-free KV-cache management can match or exceed training-based approaches for autoregressively streaming long-video generation, demonstrating effective solutions for temporal coherence in extended video synthesis.
...
TL;DR: The paper introduces Visual Reasoning Tracer (VRT) to make MLLMs reveal their intermediate visual reasoning steps, addressing the opacity of current models that only output final predictions.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) perform well on visual tasks but lack transparency - they output only final predictions without showing intermediate reasoning steps or evidence, unlike human visual reasoning which operates through chains of reasoning.

Method: Proposes the Visual Reasoning Tracer (VRT) task requiring models to localize target objects AND predict intermediate objects forming the reasoning path. Creates VRT-Bench (evaluation benchmark), a new metric for reasoning trace quality, and VRT-80k (large-scale training dataset).

Result: Existing models often produce correct final outputs but struggle to ground intermediate reasoning. Models trained on VRT-80k achieve substantial improvements in tracing reasoning paths, demonstrating the effectiveness of the proposed approach.

Conclusion: The VRT framework successfully addresses the transparency gap in MLLMs by requiring explicit intermediate reasoning traces, with benchmark, metric, and dataset enabling better evaluation and training of visual reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing Image Quality Assessment (IQA) methods for AI-generated images focus mainly on portraits and artistic images, lacking systematic evaluation of interior scenes, which have distinct spatial and aesthetic requirements.

Method: 1) Define Spatial Aesthetics paradigm with four dimensions: layout, harmony, lighting, and distortion. 2) Construct SA-BENCH benchmark with 18,000 images and 50,000 annotations. 3) Develop SA-IQA through MLLM fine-tuning and multidimensional fusion. 4) Apply SA-IQA to downstream tasks: GRPO reinforcement learning optimization and Best-of-N selection filtering.

Result: SA-IQA significantly outperforms existing IQA methods on SA-BENCH benchmark, setting a new standard for spatial aesthetics evaluation in interior scenes.

Conclusion: The proposed Spatial Aesthetics paradigm and SA-IQA framework effectively address the gap in interior scene evaluation, with practical applications in optimizing AIGC generation pipelines and improving image quality through selection filtering.
...
TL;DR: EvoIR is an All-in-One Image Restoration framework that combines evolutionary frequency modulation with adaptive optimization to handle diverse degradation types more effectively than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing AiOIR approaches lack explicit frequency modeling and rely on fixed/heuristic optimization schedules, limiting generalization across heterogeneous degradation types.
...
Conclusion: EvoIR successfully addresses limitations of existing AiOIR methods through evolutionary frequency modulation and adaptive optimization, achieving superior performance across diverse degradation types by synergistically combining explicit frequency modeling with dynamic optimization strategies.
...
<details>
  <summary>Details</summary>
Motivation: Standard diffusion corrupts both magnitude and phase, destroying spatial structure needed for geometric consistency in tasks like re-rendering, simulation enhancement, and image-to-image translation.

Method: Model-agnostic reformulation that preserves input phase while randomizing magnitude, plus Frequency-Selective Structured (FSS) noise for continuous control over structural rigidity via frequency-cutoff parameter.

Result: Enables controllable, spatially aligned results for photorealistic/stylized re-rendering and sim-to-real enhancement; improves CARLA-to-Waymo planner performance by 50%; adds no inference-time cost.

Conclusion: φ-PD provides structure-preserving diffusion for geometric consistency tasks, complementary to existing conditioning approaches, broadly applicable to image/video generation without architectural changes.
...
<details>
  <summary>Details</summary>
Motivation: To bridge algorithmic design with artistic storytelling by creating a practical pipeline for shadow-drawing compositional art that transforms 3D objects into meaningful visual narratives.
...
TL;DR: ARM-Thinker is an agentic multimodal reward model that uses external tools for verification to improve visual grounding and reliability in complex reasoning tasks.
...
TL;DR: Splannequin improves frozen 3D scene synthesis from monocular Mannequin-Challenge videos using dynamic Gaussian splatting with temporal anchoring to fix ghosting and blur artifacts.


<details>
  <summary>Details</summary>
Motivation: Synthesizing high-fidelity frozen 3D scenes from monocular MC videos is distinct from standard dynamic scene reconstruction. The goal is to create frozen scenes while preserving subtle dynamics for user-controlled instant selection, but existing methods suffer from artifacts like ghosting and blur due to sparse temporal supervision.

Method: Uses dynamic Gaussian splatting to model scenes dynamically, then renders static scenes by fixing time parameter. Proposes Splannequin - an architecture-agnostic regularization that detects hidden and defective Gaussian states and applies temporal anchoring. Hidden states anchor to recent well-observed past, defective states anchor to future states with stronger supervision.

Result: Markedly improved visual quality enabling high-fidelity, user-selectable frozen-time renderings, validated by 96% user preference. Integrates into existing dynamic Gaussian pipelines via simple loss terms, requires no architectural changes, and adds zero inference overhead.

Conclusion: Splannequin effectively addresses artifacts in frozen scene synthesis from monocular MC videos through temporal anchoring of Gaussian states, achieving high-quality results with minimal computational overhead and strong user preference.
...
<details>
  <summary>Details</summary>
Motivation: Moving beyond simple relighting to achieve generative modeling of real-world scenes requires joint control of both camera trajectory and illumination, since visual dynamics are inherently shaped by both geometry and lighting. Existing methods face a trade-off between lighting fidelity and temporal consistency.

Method: 1) Disentangled design decoupling geometry and lighting: geometry/motion captured via dynamic point clouds projected along user-defined camera trajectories, while illumination cues come from relit frames consistently projected into the same geometry. 2) Light-Syn pipeline with inverse-mapping synthesizes training pairs from in-the-wild monocular footage to address lack of paired multi-view and multi-illumination videos.

Result: Extensive experiments show Light-X outperforms baseline methods in joint camera-illumination control and surpasses prior video relighting methods under both text- and background-conditioned settings. The method works on static, dynamic, and AI-generated scenes.

Conclusion: Light-X successfully enables controllable video rendering with both viewpoint and illumination control from monocular videos, advancing toward generative modeling of real-world scenes through effective disentanglement of geometry and lighting signals.
...
### [154] [Retrieval-Augmented Few-Shot Prompting Versus Fine-Tuning for Code Vulnerability Detection](https://arxiv.org/abs/2512.04106)
*Fouad Trad,Ali Chehab*

Main category: cs.SE

TL;DR: Retrieval-augmented prompting outperforms standard few-shot and zero-shot methods for code vulnerability detection, achieving 74.05% F1 score, while avoiding fine-tuning costs.


<details>
  <summary>Details</summary>
Motivation: Few-shot prompting effectiveness depends heavily on example selection, especially in complex domains like code vulnerability detection where identifying security weaknesses is critical.

Method: Systematic evaluation using Gemini-1.5-Flash across three approaches: standard few-shot with random examples, retrieval-augmented prompting with semantically similar examples, and retrieval-based labeling without model inference.

Result: Retrieval-augmented prompting achieved 74.05% F1 score and 83.90% partial match accuracy at 20 shots, outperforming zero-shot (36.35% F1) and fine-tuned Gemini (59.31% F1), though CodeBERT fine-tuning achieved higher performance (91.22% F1).

Conclusion: Retrieval-augmented prompting offers a practical balance between performance and efficiency for code vulnerability detection, avoiding training costs while significantly improving over standard few-shot approaches.
...
### [155] [The changing surface of the world's roads](https://arxiv.org/abs/2512.04092)
...
TL;DR: First global multi-temporal dataset of road pavedness and width for 9.2 million km of critical arterial roads using deep learning on satellite imagery, achieving 95.5% coverage where nearly half was previously unclassified.


<details>
  <summary>Details</summary>
Motivation: Resilient road infrastructure is critical for UN Sustainable Development Goals, but there's a lack of comprehensive global baseline data on road surface information as a primary indicator of network functionality and resilience.

Method: Applied deep learning framework to global mosaic of Planetscope satellite imagery from 2020 and 2024 to create multi-temporal dataset of road pavedness and width.

Result: Created first global dataset covering 9.2 million km of critical arterial roads with 95.5% coverage; shows rate of change in pavedness correlates with HDI (0.65); reveals multi-scale geography of development; enables creation of Humanitarian Passability Matrix; identifies infrastructure vulnerabilities.

Conclusion: Provides foundational dataset and multi-scale analytical framework for monitoring global infrastructure, offering direct physical infrastructure measurement at higher spatial resolution than traditional proxies like nighttime lights.
...
### [156] [Hardware-aware Neural Architecture Search of Early Exiting Networks on Edge Accelerators](https://arxiv.org/abs/2512.04705)
*Alaa Zniber,Arne Symons,Ouassim Karrakchou,Marian Verhelst,Mounir Ghogho*

Main category: cs.CC

TL;DR: Hardware-aware NAS framework optimizes early exit placement in neural networks for edge devices, reducing computation by 50%+ while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Edge devices have strict computational/energy constraints that challenge deployment of large DL models. Early Exiting Neural Networks (EENN) offer efficiency but their performance suffers from hardware heterogeneity and quantization effects, with limited research on automated optimization for edge hardware.

Method: Proposed a hardware-aware Neural Architecture Search (NAS) framework that systematically integrates quantization effects and hardware resource allocation to optimize early exit point placement within network backbones.

Result: On CIFAR-10 dataset, the NAS framework discovered architectures achieving over 50% reduction in computational costs compared to conventional static networks.

Conclusion: The hardware-aware NAS framework successfully optimizes EENN designs for edge deployment, making them more suitable for resource-constrained environments by significantly reducing computational overhead while maintaining performance.
...
### [157] [Shared Multi-modal Embedding Space for Face-Voice Association](https://arxiv.org/abs/2512.04814)
*Christopher Simic,Korbinian Riedhammer,Tobias Bocklet*

Main category: cs.SD

TL;DR: First-place solution for FAME 2026 challenge using separate face/voice pipelines with age-gender features, AAM loss, achieving 23.99% EER on multilingual face-voice association tasks.


<details>
  <summary>Details</summary>
Motivation: The FAME 2026 challenge presents two demanding tasks: training face-voice associations in a multilingual setting, including testing on languages not seen during training, requiring robust cross-modal and cross-lingual generalization.

Method: Separate uni-modal processing pipelines for face and voice with general feature extraction, complemented by additional age-gender feature extraction. Features are projected into a shared embedding space and trained with Adaptive Angular Margin (AAM) loss.

Result: Achieved first place in the FAME 2026 challenge with an average Equal-Error Rate (EER) of 23.99% on the multilingual face-voice association tasks.

Conclusion: The proposed approach combining separate uni-modal pipelines with age-gender features and AAM loss effectively addresses the challenging multilingual face-voice association problem, demonstrating strong cross-modal and cross-lingual generalization capabilities.
...
TL;DR: A two-stage pipeline that enables humanoid robots to execute human actions from noisy generated videos through 4D human representation lifting and physics-aware reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Video generation models can synthesize human actions in novel contexts, potentially serving as high-level planners for robot control. However, executing these actions on humanoids is challenging due to noise and morphological distortions in generated videos.
...
Result: GenMimic can mimic human actions from noisy generated videos. Created GenMimicBench dataset for benchmarking. Shows improvements over baselines in simulation and achieves coherent, physically stable motion tracking on Unitree G1 humanoid without fine-tuning.

Conclusion: This work provides a promising path to realize video generation models as high-level policies for robot control by enabling zero-shot execution of generated human actions on humanoid robots.
...
### [159] [Network of Theseus (like the ship)](https://arxiv.org/abs/2512.04198)
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning assumes architecture must persist from training to deployment, limiting architectural choices due to optimization difficulties. This prevents using architectures with better efficiency or design properties.

Method: Progressively converts guide network architecture into target architecture part-by-part using representational similarity metrics to align components. Works with trained or untrained networks.

Result: Preserves functionality under substantial architectural changes (e.g., CNN to MLP, GPT-2 to RNN). Enables architecture conversion while maintaining performance.

Conclusion: NoT decouples optimization from deployment, expanding viable inference-time architectures for better accuracy-efficiency tradeoffs and more directed architectural exploration.
...
### [160] [Natural Language Actor-Critic: Scalable Off-Policy Learning in Language Space](https://arxiv.org/abs/2512.04601)
*Joey Hong,Kang Liu,Zhan Ling,Jiecao Chen,Sergey Levine*

Main category: cs.LG

TL;DR: NLAC is a novel actor-critic algorithm that uses a generative LLM critic producing natural language feedback instead of scalar rewards to train LLM agents more efficiently and stably in long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Current policy gradient methods for training LLM agents suffer from noisy training signals in sparse-reward, long-horizon tasks, leading to unstable training and high sample complexity. Exploration in natural language action spaces is also challenging.

Method: Natural Language Actor-Critic (NLAC) uses a generative LLM critic that produces natural language explanations for why actions are suboptimal, providing richer training signals. It can be trained off-policy without policy gradients.

Result: NLAC outperforms existing training approaches on reasoning, web browsing, and tool-use with dialogue tasks, offering more scalable and stable training for LLM agents.

Conclusion: NLAC presents a promising alternative to traditional policy gradient methods by leveraging LLMs' natural language capabilities for richer feedback, enabling more data-efficient and stable training of LLM agents in complex environments.
...
TL;DR: MemLoRA enables on-device memory-augmented conversations by equipping small language models with specialized memory adapters, while MemLoRA-V extends this to multimodal contexts with visual understanding capabilities.


<details>
  <summary>Details</summary>
Motivation: Current memory-augmented LLM systems are too costly for local on-device deployment, and small models lack sufficient performance. Additionally, existing systems lack native visual capabilities for multimodal applications.

Method: Introduces MemLoRA with specialized memory adapters trained via knowledge distillation for specific operations: knowledge extraction, memory update, and memory-augmented generation. MemLoRA-V integrates small Vision-Language Models for visual understanding.

Result: MemLoRA outperforms 10× larger baseline models (Gemma2-27B) and matches 60× larger models (GPT-OSS-120B) on LoCoMo benchmark. MemLoRA-V achieves 81.3 vs 23.7 accuracy over caption-based approaches on visual tasks while maintaining strong text performance.

Conclusion: The proposed memory adapter approach enables accurate on-device memory operations without cloud dependency, with MemLoRA-V demonstrating effective multimodal capabilities for both text and visual understanding tasks.
...
### [162] [CARL: Critical Action Focused Reinforcement Learning for Multi-Step Agent](https://arxiv.org/abs/2512.04949)
*Leyang Shen,Yang Zhang,Chun Kai Ling,Xiaoyan Zhao,Tat-Seng Chua*

Main category: cs.LG

TL;DR: CARL is a reinforcement learning algorithm that focuses training on critical actions in multi-step agents, improving performance and efficiency by optimizing only high-criticality actions.


<details>
  <summary>Details</summary>
Motivation: Conventional group-level policy optimization algorithms assume all actions have equal contribution, which is suboptimal for multi-step agents where only a small fraction of actions are critical to the final outcome.

Method: CARL (Critical-Action-focused Reinforcement Learning) provides action-level optimization signals for high-criticality actions while excluding low-criticality actions from model updates, enabling focused training.

Result: Extensive experiments show CARL achieves both stronger performance and higher efficiency during training and inference across diverse evaluation settings.
...
### [163] [Multi-LLM Collaboration for Medication Recommendation](https://arxiv.org/abs/2512.05066)
*Huascar Sanchez,Briland Hitaj,Jules Bergmann,Linda Briesemeister*

Main category: cs.LG

TL;DR: LLM Chemistry framework improves medication recommendation reliability by modeling multi-LLM collaboration for effective, stable, and calibrated ensembles.
...
Method: Apply LLM Chemistry framework (quantifies collaborative compatibility) to medication recommendation. Use Chemistry-inspired interaction modeling to guide multi-LLM collaboration for effective, stable, and calibrated ensembles.
...
Conclusion: LLM Chemistry-guided collaboration offers a promising path toward reliable and trustworthy AI assistants in clinical practice by improving ensemble reliability.
...
### [164] [Studying Various Activation Functions and Non-IID Data for Machine Learning Model Robustness](https://arxiv.org/abs/2512.04264)
*Long Dang,Thushari Hapuarachchi,Kaiqi Xiong,Jing Lin*

Main category: cs.LG

TL;DR: The paper studies ML model robustness through adversarial training with different activation functions in both centralized and federated learning environments, proposing improved training methods and analyzing performance across various settings.


<details>
  <summary>Details</summary>
Motivation: Most existing adversarial training research focuses on ReLU activation and centralized environments, leaving gaps in understanding how different activation functions affect robustness and how adversarial training performs in federated learning settings with IID/non-IID data.

Method: 1) Proposed advanced adversarial training approach with model architecture changes, soft labeling, simplified data augmentation, and varying learning rates. 2) Evaluated ten different activation functions beyond ReLU. 3) Extended approach to federated learning with both IID and non-IID data settings. 4) Introduced data sharing strategy for non-IID federated learning.

Result: Centralized training achieved 77.08% natural and 67.96% robust accuracy on CIFAR-10 against FGSM attacks. ReLU performed best among activation functions. Federated learning showed significant robust accuracy drops, especially with non-IID data. With 40% data sharing, achieved 70.09% natural and 54.79% robust accuracy, surpassing CalFAT algorithm.

Conclusion: Proper data sharing can significantly improve ML model robustness in federated learning with non-IID data, making the approach useful for real-world applications. ReLU generally performs best for adversarial robustness, but the proposed training methods improve performance across different environments.
...
### [165] [STeP-Diff: Spatio-Temporal Physics-Informed Diffusion Models for Mobile Fine-Grained Pollution Forecasting](https://arxiv.org/abs/2512.04385)
*Nan Zhou,Weijie Hong,Huandong Wang,Jianfeng Zheng,Qiuhua Wang,Yali Song,Xiao-Ping Zhang,Yong Li,Xinlei Chen*

Main category: cs.LG

TL;DR: STeP-Diff: A physics-informed diffusion model that forecasts fine-grained air pollution using incomplete data from mobile sensors, achieving up to 89% improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: Fine-grained air pollution forecasting is important for urban management and healthy buildings. Mobile sensors on cars/buses provide low-cost, wide-coverage data collection, but produce incomplete and temporally inconsistent data due to random movement patterns.
...
Result: Deployed 59 portable sensing devices in two cities for 14 days. Compared to second-best algorithm, achieved improvements up to 89.12% in MAE, 82.30% in RMSE, and 25.00% in MAPE. Extensive evaluations show STeP-Diff effectively captures spatio-temporal dependencies in air pollution fields.

Conclusion: STeP-Diff successfully integrates physics constraints with diffusion models to handle incomplete mobile sensor data, providing accurate fine-grained air pollution forecasting that respects both measurements and fundamental pollution dispersion physics.
...
### [166] [Feature Engineering vs. Deep Learning for Automated Coin Grading: A Comparative Study on Saint-Gaudens Double Eagles](https://arxiv.org/abs/2512.04464)
*Tanmay Dogra,Eric Ngo,Mohammad Alam,Jean-Paul Talavera,Asim Dahal*

Main category: cs.LG

TL;DR: Feature-based ANN with expert-designed features outperforms deep learning CNN and SVM for grading rare coins with limited data, challenging the assumption that deep learning always wins.


<details>
  <summary>Details</summary>
Motivation: To challenge the common belief that deep learning always outperforms older techniques, especially in niche domains with limited data and imbalanced classes where expert knowledge matters.

Method: Compared three approaches: 1) Feature-based ANN with 192 custom features from Sobel edge detection and HSV color analysis, 2) Hybrid CNN combining EfficientNetV2, and 3) SVM as control. Tested on 1,785 expert-graded Saint-Gaudens Double Eagle gold coins.

Result: ANN achieved 86% exact grade matches and 98% within 3-grade tolerance. CNN and SVM performed poorly with only 31% and 30% exact matches respectively, mostly guessing the most common grade. CNN's broader tolerance metrics were misleading due to regression averaging.

Conclusion: When dealing with limited data (<2,000 samples) and imbalanced classes, incorporating expert knowledge through feature engineering outperforms black-box deep learning approaches. This insight applies to other niche quality inspection tasks where domain expertise is more valuable than computational power.
...
### [167] [Rethinking Decoupled Knowledge Distillation: A Predictive Distribution Perspective](https://arxiv.org/abs/2512.04625)
*Bowen Zheng,Ran Cheng*

Main category: cs.LG

TL;DR: GDKD improves upon DKD by introducing a more flexible logit decoupling method and analyzing teacher predictive distributions, leading to better knowledge distillation performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: While DKD revived interest in logit-based distillation, its mechanisms need deeper exploration. The authors aim to rethink DKD from a predictive distribution perspective to uncover overlooked insights and improve knowledge transfer.

Method: Proposes Generalized Decoupled Knowledge Distillation (GDKD) with enhanced logit decoupling, analyzes teacher predictive distributions' impact on gradients, and develops an efficient partition strategy to handle multimodal distributions.

Result: GDKD outperforms original DKD and other leading distillation methods across CIFAR-100, ImageNet, Tiny-ImageNet, CUB-200-2011, and Cityscapes benchmarks.

Conclusion: GDKD successfully advances logit-based distillation by providing better decoupling mechanisms and leveraging insights from teacher predictive distributions, offering a more effective knowledge distillation approach.
...
### [168] [TV2TV: A Unified Framework for Interleaved Language and Video Generation](https://arxiv.org/abs/2512.05103)
...
TL;DR: TV2TV is a unified video-text model that interleaves text and video generation, allowing the model to "think in words" before generating frames, improving video quality and controllability.
...
Result: TV2TV shows substantial improvements in visual quality and prompt alignment on video game data, and scales to natural videos with strong performance on complex real-world action sequences.

Conclusion: TV2TV represents a promising step toward video generation with open-ended textual reasoning and control by offloading reasoning to language modeling while generating high-quality video frames.
...
### [169] [Deep infant brain segmentation from multi-contrast MRI](https://arxiv.org/abs/2512.05114)
*Malte Hoffmann,Lilla Zöllei,Adrian V. Dalca*

Main category: cs.LG

TL;DR: BabySeg: A unified deep learning framework for pediatric brain MRI segmentation that handles diverse protocols, repeat scans, and missing modalities using domain randomization and flexible feature pooling.


<details>
  <summary>Details</summary>
Motivation: Pediatric brain MRI segmentation is challenging due to developmental changes, imaging constraints, motion artifacts, and inconsistent modality availability. Existing specialized models are fragmented - limited to specific image types or age groups, and fragile for variable clinical images.

Method: Uses domain randomization to synthesize training images beyond realistic bounds for dataset shift invariance. Includes a mechanism for flexible pooling and interaction of features from any number of input scans, supporting diverse MRI protocols and repeat scans.

Result: Achieves state-of-the-art performance matching or exceeding several existing methods across various age cohorts and input configurations using a single model, with significantly faster runtime than existing tools.

Conclusion: BabySeg addresses method fragmentation in pediatric brain MRI segmentation by providing a unified framework that handles diverse clinical scenarios, offering both accuracy and efficiency improvements over specialized approaches.
...
### [170] [Value Gradient Guidance for Flow Matching Alignment](https://arxiv.org/abs/2512.05116)
*Zhen Liu,Tim Z. Xiao,Carles Domingo-Enrich,Weiyang Liu,Dinghuai Zhang*

Main category: cs.LG

TL;DR: VGG-Flow is a gradient-matching method for fine-tuning flow matching models that achieves both efficient adaptation and probabilistically sound prior preservation by leveraging optimal control theory.


<details>
  <summary>Details</summary>
Motivation: Existing methods for aligning flow matching models with human preferences fail to achieve both adaptation efficiency and probabilistically sound prior preservation simultaneously.

Method: VGG-Flow leverages optimal control theory to match the optimal difference between fine-tuned and pretrained velocity fields with the gradient field of a value function, incorporating first-order reward model information and heuristic initialization for fast adaptation.

Result: The method successfully fine-tunes flow matching models (tested on Stable Diffusion 3) under limited computational budgets while achieving effective and prior-preserving alignment.

Conclusion: VGG-Flow provides a theoretically grounded approach that enables efficient fine-tuning of flow matching models while maintaining probabilistic soundness of the prior distribution.
...
### [171] [The Universal Weight Subspace Hypothesis](https://arxiv.org/abs/2512.05117)
...
<details>
  <summary>Details</summary>
Motivation: To understand the intrinsic organization of information within deep networks and investigate whether neural networks systematically converge to shared spectral subspaces across different tasks, architectures, and initializations.

Method: Conducted mode-wise spectral analysis of over 1100 models including 500 Mistral-7B LoRAs, 500 Vision Transformers, and 50 LLaMA-8B models. Applied spectral decomposition techniques to weight matrices across various architectures trained on diverse tasks and datasets.

Result: Identified universal subspaces capturing majority variance in just a few principal directions. Found sparse, joint subspaces consistently exploited within shared architectures across diverse tasks and datasets, demonstrating remarkable similarity in low-dimensional parametric subspaces.

Conclusion: Neural networks exhibit intrinsic universal spectral structure with significant implications for model reusability, multi-task learning, model merging, and efficient algorithm development, potentially reducing computational costs and carbon footprint of large-scale models.
...
TL;DR: Post-deployment alignment framework using KTO and DPO improves LLM safety in healthcare by 42% on harmful query detection, revealing architecture-dependent calibration biases and trade-offs with erroneous refusals.
...
Conclusion: Importance of adopting best practices that balance patient safety, user trust, and clinical utility in conversational medical assistant design. Post-deployment alignment with KTO/DPO effectively improves safety while revealing calibration trade-offs.
...
TL;DR: Position paper proposing a research agenda for ensuring ethical behavior in multi-agent LLM systems through mechanistic interpretability, focusing on evaluation frameworks, understanding emergent behaviors, and parameter-efficient alignment techniques.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly deployed as autonomous agents in multi-agent systems, they pose significant ethical challenges despite their promise for enhancing capabilities and enabling complex tasks. There's a need to ensure ethical behavior in these multi-agent LLM systems (MALMs).

Method: Proposes a research agenda focused on mechanistic interpretability approach with three key components: 1) developing comprehensive evaluation frameworks for ethical behavior assessment at individual, interactional, and systemic levels; 2) using mechanistic interpretability to elucidate internal mechanisms behind emergent behaviors; 3) implementing targeted parameter-efficient alignment techniques to steer systems toward ethical behaviors without performance compromise.

Result: This is a position paper outlining a proposed research agenda rather than presenting completed research results. The paper identifies three key research challenges and proposes a framework for addressing ethical concerns in multi-agent LLM systems.

Conclusion: A mechanistic interpretability approach is needed to ensure ethical behavior in multi-agent LLM systems, requiring coordinated research efforts in evaluation frameworks, understanding emergent behaviors, and developing targeted alignment techniques that preserve system performance.
...
TL;DR: Open-source multilingual LLMs struggle with zero-shot comorbidity extraction from Italian EHRs, showing inconsistent performance compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional NLP techniques often fail with clinical text due to its complexity and variability, while LLMs show promise for better understanding EHRs and extracting information in real-time.

Method: Experimental evaluation of open-source multilingual LLMs for zero-shot comorbidity extraction from Italian Electronic Health Records in on-premises settings, comparing against native pattern matching and manual annotations.

Result: Some LLMs struggle in zero-shot settings, showing significant performance variation and difficulty generalizing across different diseases compared to traditional methods.

Conclusion: While LLMs have potential for clinical information extraction, current open-source multilingual models face challenges with zero-shot EHR understanding in Italian, particularly with generalization across diseases.
...
TL;DR: STELLA is a framework that enhances LLM-based time series forecasting by dynamically mining and injecting structured semantic-temporal information through hierarchical semantic anchors, outperforming state-of-the-art methods.
...
Conclusion: STELLA successfully addresses the limitations of current LLM adaptations for time series forecasting by systematically mining and injecting structured supplementary and complementary information through semantic-temporal alignment, enabling more effective utilization of LLM reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: There's a need to assess whether frontier AI models can effectively perform real-world consumer tasks that people find valuable in their daily lives, beyond traditional academic benchmarks.

Method: Created ACE with 400 hidden test cases across four consumer domains, plus 80 open-source devset cases. Evaluated 10 frontier models with websearch enabled using novel grading methodology that dynamically checks if responses are grounded in retrieved web sources.

Result: GPT 5 (Thinking = High) scored highest at 56.1%, followed by o3 Pro (55.2%) and GPT 5.1 (55.1%). Performance varies across domains, with Shopping being particularly challenging (top model under 50%). Models show high hallucination rates for specific requests like prices and working links.

Conclusion: There's a substantial gap between even the best AI models' performance and consumers' actual needs, highlighting significant room for improvement in making AI truly useful for everyday consumer tasks.
...
<details>
  <summary>Details</summary>
Motivation: LLMs show surprising effectiveness in complex reasoning tasks, with capabilities often improving through iterative refinement of solutions. There's a need for a theoretical framework to understand and formalize these reasoning algorithms that treat LLMs as probabilistic oracles.

Method: The authors introduce a theoretical framework that formalizes reasoning algorithms using LLMs as probabilistic oracles. The framework is grounded in experimental evidence rather than architectural specifics, providing a general perspective applicable to various reasoning oracles.

Result: The framework formalizes principles underlying popular iterative improvement and answer aggregation techniques, providing a foundation for designing more powerful reasoning methods that can extend to current and future reasoning oracles.

Conclusion: The proposed theoretical framework offers a general, experimentally-grounded approach to understanding and designing reasoning algorithms that use LLMs as probabilistic oracles, potentially extending to a wide range of current and future reasoning systems.
...
### [178] [Efficient Spatially-Variant Convolution via Differentiable Sparse Kernel Complex](https://arxiv.org/abs/2512.04556)
*Zhizhen Wu,Zhe Cao,Yuchi Huo*

Main category: cs.GR

TL;DR: Differentiable kernel decomposition framework for efficient image convolution with complex kernels using sparse kernel samples, enabling high-fidelity filtering on resource-limited devices.


<details>
  <summary>Details</summary>
Motivation: Direct dense convolution with complex kernels is computationally prohibitive on resource-limited devices, and existing approximations lack efficiency or fail to capture non-convex kernels.
...
### [179] [Can machines perform a qualitative data analysis? Reading the debate with Alan Turing](https://arxiv.org/abs/2512.04121)
*Stefano De Paoli*

Main category: cs.CY

TL;DR: The paper argues that critiques of using LLMs for qualitative analysis focus on wrong problems, proposing instead to empirically test whether LLMs can produce analyses comparable to human analysts, framing the debate through Turing's perspective.


<details>
  <summary>Details</summary>
Motivation: To reframe the debate about using LLMs in qualitative data analysis by shifting focus from whether machines can perform analysis in principle to whether they can produce analyses comparable to human analysts, using Turing's framework.

Method: Uses empirical evidence and critical reflection to analyze current critiques, applies Turing's "Computing Machinery and Intelligence" framework to reframe the debate, and employs Turing's rhetorical style to discuss contrary views.

Result: The paper demonstrates that current critical debates focus on wrong problems and proposes a new research focus on empirically investigating whether LLMs can produce analyses sufficiently comparable to human analysts.

Conclusion: The debate should shift from questioning whether machines can perform qualitative analysis in principle to empirically testing whether LLMs can produce analyses comparable to human analysts, using Turing's framework as a guide.
...
### [180] [Human-Centred Evaluation of Text-to-Image Generation Models for Self-expression of Mental Distress: A Dataset Based on GPT-4o](https://arxiv.org/abs/2512.04087)
*Sui He,Shenbin Qian*

Main category: q-bio.NC

TL;DR: AI-generated images using GPT-4o with counseling-based personas help Chinese international students express mental distress, with illustrator persona being most effective.


<details>
  <summary>Details</summary>
Motivation: International students face linguistic and cultural barriers in communicating mental distress in healthcare settings, creating a need for alternative communication methods.

Method: 20 Chinese international students described mental distress experiences; GPT-4o with four counseling-based persona prompts generated images; participants evaluated helpfulness of images.

Result: Prompt design significantly affects perceived helpfulness; illustrator persona achieved highest ratings; created first public text-to-image evaluation dataset with human judgment scores in mental health.

Conclusion: AI-generated images can effectively support mental distress expression; persona-based prompt engineering is crucial; dataset enables future research in mental health communication and multimodal AI.
...
### [181] [Limit cycles for speech](https://arxiv.org/abs/2512.04642)
*Adamantios I. Gafos,Stephan R. Kuberski*

Main category: q-bio.NC

TL;DR: The paper reveals that speech articulatory movements have a hidden rhythmic limit cycle organization, resolving the tension between biological rhythmicity and discrete goal-oriented actions in speech production.


<details>
  <summary>Details</summary>
Motivation: There's a tension between the known rhythmicity in acoustic speech signals/cortical oscillations and the conventional view of speech movements as discrete, goal-oriented actions. The paper aims to resolve this by investigating whether rhythmicity exists at the articulatory movement level.

Method: The authors use an unintuitive but principled alternative representation for discrete movements that reveals a pervasive limit cycle organization, allowing recovery of previously inaccessible rhythmic structure in speech motor activity.

Result: The analysis reveals a pervasive limit cycle organization in speech articulatory movements, uncovering rhythmic structure that was previously inaccessible using conventional movement representations.

Conclusion: Speech articulatory movements have rhythmic organization at the most fundamental level of individual actions, resolving the long-standing tension between biological rhythmicity and discreteness in speech production.
...
### [182] [Towards Contextual Sensitive Data Detection](https://arxiv.org/abs/2512.04120)
*Liang Telkamp,Madelon Hulsebos*

Main category: cs.CR

TL;DR: The paper introduces contextual sensitive data detection mechanisms that go beyond traditional privacy-focused approaches by considering dataset context, using LLMs to improve accuracy in identifying sensitive information.


<details>
  <summary>Details</summary>
Motivation: Current sensitive data detection methods focus too narrowly on personal privacy data, ignoring contextual sensitivity. There's a need to broaden definitions of sensitive data to include contextual factors, especially for open data portals where datasets are published and exchanged.

Method: Two contextualization mechanisms: 1) Type contextualization - detects semantic types of data values and considers overall dataset context; 2) Domain contextualization - determines sensitivity based on retrieved rules from documents specifying data sensitivity (topic, geographic origin). Both use LLMs for implementation.

Result: Type-contextualization reduces false positives and achieves 94% recall vs 63% with commercial tools. Domain-contextualization is effective for non-standard domains like humanitarian data. LLM explanations improve consistency in manual data auditing.

Conclusion: Contextual approaches significantly improve sensitive data detection accuracy, especially for non-standard domains. The framework is open-sourced for broader adoption and further research.
...
### [183] [Topology Matters: Measuring Memory Leakage in Multi-Agent LLMs](https://arxiv.org/abs/2512.04668)
*Jinbo Liu,Defu Cao,Yifei Wei,Tianyao Su,Yuan Liang,Yushun Dong,Yue Zhao,Xiyang Hu*

Main category: cs.CR

TL;DR: Graph topology significantly affects memory leakage in multi-agent LLM systems, with fully connected networks showing maximum leakage and chains providing strongest protection.


<details>
  <summary>Details</summary>
Motivation: Graph topology is a fundamental determinant of memory leakage in multi-agent LLM systems, but its effects remain poorly quantified. The paper aims to systematically measure how network structure shapes privacy leakage in these systems.

Method: Introduces MAMA (Multi-Agent Memory Attack) framework that uses synthetic documents with labeled PII entities to generate sanitized task instructions. Executes a two-phase protocol: Engram (seeding private info into target agent's memory) and Resonance (multi-round interaction where attacker attempts extraction). Evaluates six network topologies with varying agent counts, attacker-target placements, and base models over up to 10 interaction rounds.

Result: Fully connected graphs exhibit maximum leakage while chains provide strongest protection. Shorter attacker-target graph distance and higher target centrality significantly increase vulnerability. Leakage rises sharply in early rounds before plateauing. Model choice shifts absolute leakage rates but preserves topology rankings. Temporal/locational PII attributes leak more readily than identity credentials or regulated identifiers.

Conclusion: Provides first systematic mapping from architectural choices to measurable privacy risk. Recommends preferring sparse or hierarchical connectivity, maximizing attacker-target separation, limiting node degree and network radius, avoiding shortcuts bypassing hubs, and implementing topology-aware access controls.
