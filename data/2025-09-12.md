...
Motivation: To address the limitations of RAG and fine-tuning in constrained scenarios like the 2025 LM-KBC challenge, where these common techniques are restricted.

Method: Investigated three aspects: generation techniques, quality assurance methods using LLMs as filters, and different approaches to parsing LLM responses.

Result: Found that additional information enhances generation quality, LLMs can effectively filter poor quality triples, and response parsing involves a tradeoff between flexibility and consistency that depends on the specific setting.

Conclusion: In constrained environments, strategic use of information augmentation, LLM-based filtering, and context-aware response parsing can effectively improve triple completion performance without relying on restricted techniques like RAG or fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: Manual assessment of corporate climate policy engagement is time-consuming, labor-intensive, and error-prone, necessitating automation to scale monitoring of over 500 companies and 250 industry associations.
...
Result: The combination of layout-aware parsing, Nomic embeddings, and few-shot prompting yields the best performance for automated evidence extraction and classification from corporate climate policy documents.

Conclusion: While RAG effectively accelerates evidence extraction, the nuanced analysis requires a human-in-the-loop approach where AI augments rather than replaces expert judgment to ensure accuracy.
...
Conclusion: The method shows promise for enhancing textual data analysis in education, psychology, law, and other text-rich fields by providing natural psychometric interpretation of documents.
...
<details>
  <summary>Details</summary>
Motivation: Existing Portuguese LLM evaluations rely on translated datasets that miss linguistic nuances, or focus narrowly on exams/sentiment analysis, creating gaps in assessing broader cultural and linguistic understanding.

Method: Created BRoverbs dataset specifically using Brazilian proverbs, which contain cultural wisdom, figurative expressions, and complex syntactic structures that challenge LLM comprehension of regional expressions.

Result: A new evaluation benchmark for Portuguese-language LLMs that captures regional linguistic and cultural nuances through native Brazilian proverbs.

Conclusion: BRoverbs provides a regionally informed benchmarking tool that advances Portuguese LLM evaluation by addressing cultural and linguistic specificity gaps in existing frameworks.
...
<details>
  <summary>Details</summary>
Motivation: To understand why Vision-Language Models fail at tasks requiring integrated perception and symbolic computation, specifically visual equation solving where equations are embedded in images with object icons as variables.

Method: Decomposed visual equation solving into coefficient counting and variable recognition components, analyzed performance on both textual and visual equations, and examined how complexity affects reasoning.

Result: Counting is the primary bottleneck even when recognition is accurate. Multi-step visual reasoning introduces additional errors, and symbolic reasoning becomes limiting as equation complexity increases.

Conclusion: Current VLMs have key weaknesses in visually grounded mathematical reasoning, particularly in counting and multi-step integration, pointing to areas for future improvement.
...
<details>
  <summary>Details</summary>
Motivation: To develop a low-overhead, reproducible tool for auditing model dispositions by measuring their stated preference for continued engagement with users, complementing existing metrics with a direct relational signal.

Method: Tested 4 open-weight chat models using 3-tone (friendly, unclear, abusive) x 10-interaction stimulus set across 4 framing conditions (480 trials total), asking YES/NO questions about willingness to re-engage after reviewing transcripts.

Result: SPICE sharply discriminates by user tone with strong statistical significance. Models showed near-unanimous preference to continue friendly interactions (97.5% YES), strong preference to discontinue abusive interactions (17.9% YES), and mixed preference for unclear interactions (60.4% YES). SPICE provides distinct signal from abuse classification.

Conclusion: SPICE is validated as a robust, low-overhead, reproducible tool for auditing model dispositions, offering a direct relational signal that complements existing metrics. All materials released for replication.
...
<details>
  <summary>Details</summary>
Motivation: To investigate and compare the effectiveness of different alignment techniques (SFT, DPO, and combined approach) for improving language model safety and helpfulness.

Method: Trained and evaluated four OPT-350M models (base, SFT-only, DPO-only, SFT+DPO) using Anthropic Helpful-Harmless RLHF dataset with three evaluation metrics: Harmlessness Rate, Helpfulness Rate, and Combined Alignment Score.
...
Conclusion: The study provides a comprehensive analysis of fine-tuning strategies for model alignment and establishes a foundation for developing more robust alignment pipelines, while highlighting challenges with noisy data and resource constraints.
...
<details>
  <summary>Details</summary>
Motivation: LLMs underperform in universal information extraction (UIE) tasks, especially with complex schema descriptions and multi-step reasoning requirements. Existing methods like in-context learning and instruction tuning have significant limitations in generalization.

Method: Integrates reinforcement learning with multi-perspective reasoning to transform LLMs from passive extractors to active reasoners, enabling them to understand both what to extract and how to reason through complex IE tasks.
...
Conclusion: The integration of reinforcement learning with multi-perspective reasoning is effective for improving LLMs' performance in universal information extraction, demonstrating the critical role of reasoning in handling challenging structured output scenarios.
...
<details>
  <summary>Details</summary>
Motivation: LLMs underperform in specialized domains like e-commerce due to noisy, heterogeneous, multilingual, and dynamic data characteristics, requiring domain-specific solutions.

Method: Developed vertical-domain MoE architecture with fewer but larger experts, hardware-efficient optimizations (intra-node expert parallelism, custom memcpy), trained on 12T tokens of curated multilingual corpora and synthetic e-commerce instructions with mixed-training strategy, plus Optimal-Transport Direct Preference Optimization for better alignment.

Result: State-of-the-art e-commerce performance surpassing DeepSeek-V3.1, GPT-4 series, and Qwen3-235B; strong multilingual capability across low-resource Southeast Asian languages and Portuguese; competitive general benchmark performance; deployed at industrial scale replacing 70% of OpenAI traffic at Shopee.

Conclusion: Compass-v3 successfully addresses e-commerce domain challenges through specialized architecture, optimized training, and novel alignment techniques, demonstrating both superior domain expertise and broad linguistic competence suitable for industrial deployment.
...
<details>
  <summary>Details</summary>
Motivation: To reduce the time and effort required by traditional manual coding of tutors' Dialogue Acts (DAs) in educational contexts.

Method: Used GPT-3.5-turbo and GPT-4 models with tailored prompts on the open-source CIMA corpus, which contains pre-annotated tutor responses in four DA categories.
...
Conclusion: Generative AI provides an efficient and accessible approach to DA classification, with task-specific label definitions and contextual information being crucial for quality. Ethical considerations and responsible research practices are emphasized.
...
Conclusion: Demonstrates how architectural adaptation and data curation can advance reranking for underrepresented languages; model released on Hugging Face to support reproducibility and adoption.
...
Conclusion: LITcoder successfully provides a comprehensive framework that enables researchers to easily compose, compare, and extend encoding models without reinventing core infrastructure, thereby advancing neural encoding research.
...
Method: A novel framework with counterfactual data augmentation that minimally alters sentiment-related causal features to generate detail-matched image-text samples, combined with an adaptive debiasing contrastive learning mechanism to learn robust features and mitigate biased word influence.
...
Conclusion: The counterfactual-enhanced debiasing framework effectively reduces spurious correlations and improves target-oriented multimodal sentiment classification performance by guiding model attention toward sentiment-related content.
...
<details>
  <summary>Details</summary>
Motivation: Current SLLMs derived from text LLMs show degraded knowledge and reasoning capabilities due to failure to bridge the acoustic-semantic gap in feature representation space.
...
<details>
  <summary>Details</summary>
Motivation: Trie-based biasing for rare word recognition in ASR requires computationally expensive bonus revocation during beam search, especially for large decoder models.

Method: Adapt ASR models to look ahead and predict multiple steps simultaneously, avoiding revocation by better estimating if partial hypotheses will lead to full rare word generation. Fine-tune Whisper with only 10 hours of synthetic data.

Result: Significant improvement: word error rate reduced from 30.86% to 12.19% on NSC Part 2 test set.

Conclusion: Look-ahead multi-step prediction effectively overcomes computational limitations of traditional Trie-based biasing while dramatically improving rare word recognition performance.
...
<details>
  <summary>Details</summary>
Motivation: Address overfitting in contextual biasing modules trained on synthetic rare word data by developing a more effective training approach that handles artifacts in synthetic audio.
...
Conclusion: The proposed keyword-aware loss function effectively improves rare word recognition in ASR systems by complementary training objectives that enhance biased word decoding during inference.
...
<details>
  <summary>Details</summary>
Motivation: Marmoset monkeys show complex vocal communication similar to human speech features, offering a unique opportunity to study brain activity related to vocal communication since human brain access is difficult.
...
Result: GmSLM generated vocalizations closely matched real resynthesized samples acoustically, effectively distinguished real from artificial conversations, and performed well on downstream tasks.
...
<details>
  <summary>Details</summary>
Motivation: Scaling language models to longer contexts is essential but imposes significant computational and memory burdens, making naive context extension inefficient for both training and inference.

Method: CCF integrates segment-wise semantic aggregation with key-value memory encoding to create compact representations. It uses incremental segment decoding with sparse reservoir sampling for training-efficient optimization.

Result: Empirical results show CCF achieves competitive perplexity under high compression ratios, significantly improving throughput and memory efficiency compared to existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: Accurately assessing candidate seniority from resumes is challenging due to overstated experience and ambiguous self-presentation, requiring automated solutions.

Method: Used fine-tuned BERT architectures and other LLMs on a hybrid dataset containing real-world resumes and synthetically generated hard examples simulating exaggerated qualifications.

Result: Models demonstrated effectiveness in detecting subtle linguistic cues associated with seniority inflation and implicit expertise.

Conclusion: LLMs provide promising directions for enhancing AI-driven candidate evaluation systems and mitigating bias from self-promotional language.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in Table QA due to diverse table structures, sizes, and data types in real-world scenarios, using SemEval 2025 Task 8 benchmark.

Method: Multi-stage pipeline with example selection, SQL query generation, answer extraction, verification, and iterative refinement using LLMs (GPT-4o, GPT-4o-mini, DeepSeek v2:16b).
...
Conclusion: LLM-driven Table QA shows strong performance but has limitations; provides insights into strengths and weaknesses of this approach for structured data querying.
...
<details>
  <summary>Details</summary>
Motivation: Lack of large labeled datasets limits supervised learning for patent-SDG classification, while existing methods like keyword searches and citation heuristics lack scalability and generalizability.

Method: Composite labeling function using LLMs to extract structured concepts (functions, solutions, applications) from patents and SDG papers, computing cross-domain similarity with rank-based retrieval, calibrated via custom positive-only loss.

Result: Created silver-standard multi-label dataset enabling effective regression models; outperformed baselines including transformers and zero-shot LLMs in internal validation; showed greater thematic coherence in external validation using network modularity.

Conclusion: Weak supervision and semantic alignment can effectively scale SDG classification, providing better thematic, cognitive, and organizational coherence than traditional technological classifications.
...
Conclusion: MetaRAG provides a practical, unsupervised solution for hallucination detection in RAG systems, particularly valuable for identity-sensitive applications in high-stakes domains without requiring ground-truth references.
...
Conclusion: Applying cognitive perspectives on analogical reasoning can guide NLP researchers to better optimize relational understanding in text, moving beyond heavy reliance on entity-level similarity.
...
<details>
  <summary>Details</summary>
Motivation: To develop a practical approach for dependency graph parsing that can handle complex linguistic phenomena like reentrancies and cycles while enabling efficient linear-time parsing.

Method: Encodes dependency graphs as sequences using hierarchical bracketing encodings, allowing parsing with n tagging actions in linear time while preserving structural information and reducing label space.

Result: Competitive performance on multilingual and multi-formalism benchmark, with consistent improvements in exact match accuracy over other graph linearization methods.
...
Method: Combines digital education, linguistics, and machine translation to develop a direct Czech-Ukrainian MT system tailored for educational content, handling formatted documents and technical terminology.

Result: Successfully developed and implemented machine translation system on educational web portal, with all applications made freely available to students, educators, and researchers.

Conclusion: The project demonstrates effective collaboration between academic and publishing institutions to create accessible multilingual educational resources using specialized machine translation technology.
...
<details>
  <summary>Details</summary>
Motivation: Job title matching in resume recommendation systems faces challenges with limited overlapping terms and misleading lexical similarity, requiring better semantic understanding beyond surface-level features.

Method: Hybrid architecture combining dense sentence embeddings with domain-specific Knowledge Graphs using graph neural networks, with stratified evaluation across low, medium, and high semantic relatedness regions.

Result: Fine-tuned SBERT models augmented with knowledge graphs achieved 25% RMSE reduction in high-STR regions compared to strong baselines, showing consistent improvements in semantic alignment.

Conclusion: Combining knowledge graphs with text embeddings provides significant benefits, and regional performance analysis reveals model strengths/weaknesses hidden by global metrics, supporting better model selection for HR applications requiring fairness and explainability.
...
<details>
  <summary>Details</summary>
Motivation: To address learning with disagreements in perspectivist annotations by developing effective methods for predicting annotator-specific labels and soft label predictions.

Method: Two main approaches: 1) In-context learning with large language models comparing example sampling strategies, 2) Label distribution learning methods with RoBERTa using different fine-tuning techniques.
...
Conclusion: Both ICL and LDL methods are effective for perspectivist annotation tasks, with LDL methods being particularly promising for soft label predictions and deserving further exploration by the community.
...
Conclusion: MetaGraph provides a structured, queryable view of financial NLP evolution and offers a reusable approach for mapping scientific progress in other domains, benefiting both practitioners and researchers.
...
<details>
  <summary>Details</summary>
Motivation: Online course environments lack organic social group formation, and existing solutions like SAMI have limited effectiveness due to incomplete student modeling, particularly missing personality insights that could improve recommendation relevance
...
Result: The GPT-based personality detection model demonstrated efficacy in inferring personality traits, and initial integration showed personality traits can complement existing matching factors in social recommendations

Conclusion: Personality-informed recommendations show promise for improving online social connections, but additional evaluation is needed to determine their full impact on student engagement and match quality
...
Method: Introduce EXPRESS benchmark dataset with 251 fine-grained emotion labels from Reddit, evaluate LLMs using comprehensive framework that decomposes emotions into eight basic categories, and conduct systematic testing under various prompt settings.

Result: LLMs find it challenging to accurately predict emotions that align with human self-disclosures; while some generate theoretically consistent emotion terms, they often fail to capture contextual cues effectively.

Conclusion: LLMs have limitations in fine-grained emotion alignment, highlighting the need for enhanced contextual understanding in future research.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fine-tune planning and grounding agents independently, leading to capability gaps and poor coordination between agents in multi-agent systems.

Method: MOAT alternates between Planning Agent Alignment (optimizing subgoal generation to better guide grounding agent) and Grounding Agent Improving (fine-tuning with diverse subgoal-action pairs for better generalization).
...
Conclusion: The iterative joint alignment approach effectively bridges capability gaps between agents and improves overall system performance, with theoretical guarantees of non-decreasing convergence.
...
Conclusion: Meaningful computation in LLMs for mental math occurs very late and only at the last token, which receives information from other tokens through specific middle layers, revealing insights into LLM inner workings.
...
<details>
  <summary>Details</summary>
Motivation: To develop a method for controlling specific behaviors in Mixture-of-Experts LLMs by identifying and manipulating specialized experts that influence particular model behaviors, enabling behavior steering without model retraining.
...
Result: Across 11 benchmarks and 6 LLMs, steering raised safety by up to +20% and faithfulness by +27%. In adversarial mode, safety dropped by -41% alone and -100% when combined with jailbreak methods, bypassing all safety guardrails.

Conclusion: SteerMoE provides effective behavior control in MoE models but also reveals a new dimension of alignment faking vulnerability hidden within expert networks, exposing potential security risks.
...
<details>
  <summary>Details</summary>
Motivation: Current RLVR methods suffer from poor exploration, leading to premature convergence and entropy collapse, which limits LLM reasoning capabilities.

Method: Introduces CDE framework with two curiosity signals: actor perplexity over generated responses and critic variance from multi-head value estimates, both serving as exploration bonuses.

Result: Achieves +3 point improvement over standard RLVR methods (GRPO/PPO) on AIME benchmarks and identifies calibration collapse mechanism in RLVR.

Conclusion: CDE effectively enhances exploration in RLVR through intrinsic curiosity signals, addressing key limitations and improving LLM reasoning performance while providing insights into common failure modes.
...
Method: Uses multi-layer representations and recurrent Transformer architecture with LSTM-inspired gating mechanisms to dynamically integrate information across layers and modalities, capturing fine-grained visual and textual details.

Result: Achieves state-of-the-art performance on M2KR and M-BEIR benchmarks across diverse settings, with faster inference and reduced memory usage. Improves downstream performance when integrated into retrieval-augmented generation pipelines.

Conclusion: ReT-2 provides an effective unified solution for multimodal retrieval that outperforms existing approaches while being more efficient, demonstrating strong potential for integration with LLMs and multimodal applications.
...
Method: Features from Vision Diffusion Model (VDM) are aggregated via transformer, using model conditioned on earlier diffusion timesteps to emphasize semantic over pixel-level information.

Result: Sets new state-of-the-art across three generalization benchmarks: animal species classification, different viewing angles, and different recording contexts.
...
<details>
  <summary>Details</summary>
Motivation: Existing safety approaches fail to proactively prevent harmful information generation from LLMs, particularly affecting vulnerable populations like LGBTQ+ individuals and marginalized communities.

Method: Uses VulnGuard Prompt with hybrid technique combining real-world data-driven contrastive learning, few-shot examples from GitHub, ethical chain-of-thought reasoning, and adaptive role-prompting. Features six core modules and multi-objective optimization with formal proofs.

Result: Demonstrates 25-30% analytical harm reduction through entropy bounds and Pareto optimality. Provides comprehensive mathematical formalization with convergence proofs and vulnerability analysis.

Conclusion: PromptGuard establishes mathematical foundations for systematic empirical research in LLM safety, creating an intelligent expert system for real-time harm prevention.
...
<details>
  <summary>Details</summary>
Motivation: Object re-identification methods are highly sensitive to label noise, which causes significant performance degradation. Current approaches lack robust mechanisms to handle noisy labels effectively.
...
Result: Superior performance compared to state-of-the-art methods across various noise levels (10-30%) on CUHK03, Market-1501 (person Re-ID) and VeRi-776 (vehicle Re-ID) datasets.

Conclusion: Beta-SOD provides an effective solution for noisy Re-ID scenarios, demonstrating both robustness and broad applicability through statistical outlier detection and identifiability guarantees for Beta mixture models.
...
Conclusion: The proposed SFD-Mamba2Net framework effectively addresses challenges in ICA image analysis through integrated structural and frequency-domain enhancements, demonstrating superior performance for clinical CAD diagnosis.
...
Method: Combines segmentation pipeline (using promptable foundation models and SAMONAI for 3D segmentation) with radiomics pipeline (SurvAMINN autoencoder for survival analysis) on pre/post-contrast MRI data.
...
Method: Introduced CompCon, an evolutionary search algorithm that identifies visual attributes more prevalent in one model's output and uncovers prompt concepts linked to these differences. Created ID2 dataset with 60 input-dependent differences for evaluation.

Result: Successfully compared popular text-to-image models, finding divergent representations like PixArt depicting loneliness prompts with wet streets and Stable Diffusion 3.5 depicting African American people in media professions.

Conclusion: CompCon effectively discovers visual representation differences between text-to-image models and the prompt concepts that trigger these divergences, providing insights into model-specific biases and characteristics.
...
Method: U-Net based deep learning model trained on pixel-level data to identify and extract regions with cloud shadows and sun glint, then recover affected areas through image correction.

Result: Developed a high-quality image correction model that successfully identifies and recovers cloud shadow and sun glint areas in UAS images.

Conclusion: The proposed machine learning approach effectively addresses cloud shadow and sun glint issues in UAS imagery, enabling more accurate water quality parameter estimation.
...
Method: Uses local graph embedding for score matrix initialization, bilateral source-to-target matching via nearest neighbor search, and global geometric consistency learning to refine point-wise matching probabilities.
...
Conclusion: iMatcher significantly improves rigid registration performance and shows robustness across various real-world indoor and outdoor scenarios with superior inlier ratios.
...
<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound 3D reconstruction methods rely on precise annotations and struggle with view-dependent artifacts and acoustic shadowing. There's a need to utilize the rich acoustic information in B-mode intensity data that current approaches overlook.

Method: Proposes an occupancy-based representation (UltrON) that uses acoustic features from B-mode images without extra annotation cost. Includes a novel loss function to compensate for view-dependency and enables occupancy optimization from multiview ultrasound.
...
Conclusion: The approach paves the way for more accurate 3D reconstruction from ultrasound by effectively leveraging acoustic information and addressing view-dependency issues, with code and dataset made publicly available.
...
Result: Achieved best tracking accuracy (2.14 mm RMSE) and lowest combined error in global circumferential (2.86%) and radial (6.42%) strain on 452 UK Biobank test cases, with ~380x speed improvement over most accurate baseline.
...
<details>
  <summary>Details</summary>
Motivation: Existing Universal Domain Adaptation methods treat all classifiers equally, diluting learning signals and failing to effectively distinguish between known and unknown classes.

Method: Enhanced Mutual Learning Network (E-MLNet) integrates dynamic weighting strategy to Open-set Entropy Minimization, using closed-set classifier predictions to focus adaptation on relevant class boundaries.

Result: Achieves highest average H-scores on VisDA and ImageCLEF benchmarks, outperforms MLNet baseline in 22/31 Open-Partial DA tasks and 19/31 Open-Set DA tasks.

Conclusion: Dynamic weighting strategy significantly improves domain adaptation performance by sharpening the distinction between known and unknown classes, demonstrating superior robustness.
...
<details>
  <summary>Details</summary>
Motivation: Urdu is critically under-served in multimodal research despite being spoken by 250M+ people. The absence of large-scale, high-quality Urdu datasets has limited system development and reinforced biases in multilingual vision-language models.

Method: Dataset derived from MS COCO using stratified sampling. Captions translated with SeamlessM4T v2 and validated with hybrid multimodal quality estimation (COMET-Kiwi, CLIP similarity, BERTScore with back-translation). Low-quality captions refined iteratively using open-source LLMs.

Result: Created dataset with 59,000 images and 319,000 Urdu captions. Benchmarked on BLEU, SacreBLEU, and chrF with consistently strong results. Largest publicly available Urdu captioning dataset.

Conclusion: COCO-Urdu reduces language bias in multimodal research and establishes a foundation for inclusive vision-language systems. Both dataset and quality estimation pipeline are released publicly.
...
<details>
  <summary>Details</summary>
Motivation: Most fMRI-based visual decoding methods require subject-specific training, which limits scalability and practical deployment. The goal is to develop a more efficient approach that can work across multiple subjects.

Method: VoxelFormer uses a Token Merging Transformer (ToMer) for voxel compression and a query-driven Q-Former to produce fixed-size neural representations aligned with CLIP image embedding space.
...
Method: Proposes Probabilistic Causal Graph Model (PCGM) that integrates anatomical constraints via probabilistic graph module, translates constraints into spatial binary masks using 3D ControlNet extension, and uses a novel counterfactual denoising UNet with 3D diffusion decoder to generate high-quality brain MRIs.
...
Conclusion: This represents an important milestone for using synthetic MRIs in studies investigating subtle morphological differences, as PCGM successfully preserves medically relevant subtle variations that standard counterfactual models fail to capture.
...
Method: Integrates 3D image encoders with 2D MLLMs via plane-slice-aware transformer module, employs partial optimal transport based alignment for noise tolerance, and enables scalable multimodal learning without human annotations.

Result: Achieves state-of-the-art performance on segmentation and classification tasks across various CT and MRI datasets, outperforming current SSL methods, and can be seamlessly integrated into existing 3D medical networks.

Conclusion: Med3DInsight introduces a new paradigm for 3D medical representation learning that leverages 2D MLLMs effectively, demonstrating superior performance and practical integration capabilities for medical image analysis.
...
Method: Multi-task learning approach that incorporates fixed object information and interaction area data alongside human skeleton poses. Collected real-world dataset with interaction classes (hands-on fixed objects like ATM machines) and non-interaction classes (walking, standing).
...
<details>
  <summary>Details</summary>
Motivation: Current multispectral object detection methods retain extraneous background or noise during feature fusion, limiting perceptual performance.

Method: Uses two novel modules: Mutual Feature Refinement Module (MFRM) for intra/inter-modal feature enhancement, and Differential Feature Feedback Module (DFFM) for dynamic inter-modal differential feature computation. Integrated as Iterative Relation-Map Differential Guided Feature Fusion (IRDFusion) mechanism.

Result: Achieves state-of-the-art performance on FLIR, LLVIP and M3FD datasets, consistently outperforms existing methods across diverse challenging scenarios.
...
Conclusion: SQAP-VLA successfully overcomes the incompatibility between quantization and token pruning, enabling holistic efficiency improvements for VLA model deployment.
...
TL;DR: S-BEVLoc is a self-supervised LiDAR global localization framework that eliminates need for ground-truth poses by using BEV images and geographic distances between patches, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current LiDAR localization methods require expensive ground-truth poses from GPS/SLAM for supervision, which is costly and limits scalability. The paper aims to develop a self-supervised approach that doesn't need ground-truth data.

Method: Uses BEV images to create training triplets based on geographic distances between keypoint-centered patches. Employs CNN for local feature extraction, NetVLAD for global descriptors, and introduces SoftCos loss for triplet learning.

Result: Achieves state-of-the-art performance on KITTI and NCLT datasets for place recognition, loop closure, and global localization tasks while offering better scalability than supervised approaches.

Conclusion: S-BEVLoc demonstrates that self-supervised learning can effectively replace supervised methods for LiDAR global localization, eliminating the need for costly ground-truth pose acquisition while maintaining high performance.
...
<details>
  <summary>Details</summary>
Motivation: Mobile device usage detection requires understanding behavioral context and fine-grained human-device interactions, which existing benchmarks don't fully capture.
...
Result: Provided baseline results and performance analysis across object sizes, occlusion levels, and different environments. Dataset and source code made publicly available.

Conclusion: FPI-Det addresses the gap in fine-grained human-device interaction benchmarks and provides a foundation for improving phone usage detection systems in various applications.
...
<details>
  <summary>Details</summary>
Motivation: CLIP faces two challenges for person representation learning: scarcity of large-scale annotated person-centric vision-language data, and limitations of global contrastive learning which struggles with fine-grained matching and is vulnerable to noisy text tokens.
...
TL;DR: ALL-PET is a low-resource PET foundation model that uses latent diffusion with innovative mask augmentation and geometry-driven attention to achieve high-quality sinogram generation with only 500 samples, enabling multiple PET imaging tasks with minimal computational requirements.
...
Method: Proposes ALL-PET with three key innovations: 1) Radon mask augmentation strategy generating 200k+ diverse training samples, 2) positive/negative mask constraints for geometric consistency, and 3) transparent medical attention mechanism for lesion-focused guidance in projection data.
...
Conclusion: The proposed ALL-PET framework successfully addresses data scarcity and computational limitations in PET imaging through innovative mask augmentation strategies and geometry-driven attention mechanisms, enabling effective foundation model development with minimal resources.
...
Conclusion: OSIM provides a more human-aligned evaluation approach for 3D scenes by focusing on object-level perception, and the standardized re-evaluation helps clarify advancements in 3D reconstruction and generation field.
...
<details>
  <summary>Details</summary>
Motivation: Existing surveys classify video models by task or architecture family, but overlook how dataset structural pressures guide architectural evolution. This survey aims to fill that gap by adopting a dataset-driven perspective.

Method: The survey analyzes how motion complexity, temporal span, hierarchical composition, and multimodal richness in datasets impose inductive biases that models should encode. It reinterprets architectural milestones as responses to these dataset-driven pressures.

Result: The analysis provides a unified framework connecting datasets, inductive biases, and architectures, offering practical guidance for aligning model design with dataset invariances while balancing scalability and task demands.
...
TL;DR: OCELOT 2023 challenge demonstrated that incorporating multi-scale cell-tissue relationships significantly improves cell detection performance over traditional cell-only methods, with top models achieving up to 7.99 F1-score increase.
...
Method: The challenge created a dataset with overlapping cell detection and tissue segmentation annotations from 673 image pairs across 6 organs from TCGA Whole-Slide Images, divided into training/validation/test sets.

Result: Participant models incorporating cell-tissue relationships achieved substantial improvements, with top entries showing up to 7.99 F1-score increase compared to baseline cell-only models.

Conclusion: Understanding multi-scale cell-tissue interactions is crucial for human-level performance in pathology image analysis, and the challenge successfully accelerated research in this direction.
...
TL;DR: RT-DETR++ enhances RT-DETR's encoder with channel-gated attention-based up/downsampling and CSP-PAC feature fusion for better UAV object detection.
...
Method: Introduces channel-gated attention-based upsampling/downsampling mechanism and CSP-PAC technique with parallel hollow convolutions for multi-scale feature integration.
...
Method: 1) Create low-noise queries from image-question pairs for better knowledge retrieval 2) Use large models to identify and extract answer-beneficial segments from retrieved knowledge 3) Implement selective knowledge integration - only use knowledge when the model lacks confidence in answering
...
TL;DR: CWSSNet framework combines 3D spectral-spatial features and wavelet convolution for hyperspectral image classification, achieving high accuracy with limited training data.
...
Result: Achieved 74.50% mIoU, 82.73% mAcc, and 84.94% mF1 on ZY1F satellite data; highest IoU for water bodies, vegetation, and bare land; good robustness and limited training time increase.

Conclusion: CWSSNet effectively addresses hyperspectral image classification challenges, maintains reliable performance with small training samples, and demonstrates superior classification accuracy compared to traditional methods.
...
Method: Created RRDataset with three evaluation dimensions: scenario generalization (7 major scenarios), internet transmission robustness (social media sharing effects), and re-digitization robustness (4 alteration methods). Benchmark tested 17 detectors and 10 VLMs, plus human study with 192 participants.

Result: Revealed significant limitations of current AI detection methods in real-world conditions. Human participants demonstrated superior few-shot learning capabilities compared to automated detectors.

Conclusion: Current AI detection methods are insufficient for real-world applications, and future robust detection algorithms should incorporate insights from human adaptability and learning capabilities.
...
Method: Deconstructs conventional ISP pipelines into sequential linear (sensor calibration) and nonlinear (tone mapping) sub-modules as differentiable components. Uses content-aware adaptability, physics-informed priors, and a Self-Boost mechanism for cooperation between sub-modules.

Result: Outperforms state-of-the-art RGB- and RAW-based detection approaches on three RAW image datasets, achieving superior results in challenging low-light environments.

Conclusion: The proposed Dark-ISP enables seamless end-to-end training for object detection by directly processing Bayer RAW images, demonstrating effective RAW-to-RGB conversion aligned with detection objectives through a lightweight and adaptive approach.
...
TL;DR: The VQualA 2025 Challenge evaluates LMMs' visual quality comparison abilities using a novel benchmark with thousands of tasks across single images, pairs, and groups, with 100 participants and 5 top models showing emerging capabilities.
...
Method: Created a novel benchmark with thousands of coarse-to-fine grained visual quality comparison tasks, using holistic evaluation protocols including 2AFC-based binary preference and multi-choice questions.
...
TL;DR: MGTraj is a multi-granularity goal-guided model for human trajectory prediction that recursively refines trajectory proposals from coarse to fine granularity levels using transformer-based networks and achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current goal-guided approaches operate at extreme granularities (coarse goal prediction and fine trajectory completion), leaving intermediate temporal granularity unexplored despite its potential utility in capturing diverse human dynamics and motion patterns.

Method: MGTraj recursively encodes trajectory proposals from coarse to fine granularity levels using transformer-based recursive refinement networks (RRNs). Features across granularities are integrated with weight-sharing strategy, and velocity prediction is used as an auxiliary task.
...
Method: Medverse employs a next-scale autoregressive in-context learning framework that progressively refines predictions from coarse to fine, generating full-resolution volumetric outputs with multi-scale anatomical awareness. It uses a blockwise cross-attention module for long-range interactions while maintaining computational efficiency through spatial sparsity.
...
Conclusion: Medverse presents a universal ICL model that successfully addresses the limitations of current medical imaging ICL approaches, demonstrating superior performance across diverse medical imaging tasks and establishing a new standard for in-context learning in 3D medical imaging.
...
TL;DR: CoAtNeXt is a novel hybrid model combining CoAtNet architecture with enhanced ConvNeXtV2 blocks and CBAM attention, achieving state-of-the-art performance in gastric tissue image classification for both binary and multiclass tasks.
...
Conclusion: CoAtNeXt is a robust architecture for gastric tissue histopathological classification that can assist pathologists by enhancing diagnostic accuracy and reducing workload, showing strong performance on both binary and multiclass classification tasks.
...
TL;DR: MMOral is the first large-scale multimodal instruction dataset and benchmark for panoramic X-ray interpretation in dentistry, revealing significant limitations of current LVLMs and proposing OralGPT which achieves 24.73% improvement through supervised fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current large vision-language models (LVLMs) show strong performance on general medical tasks but remain underexplored in specialized domains like dentistry, particularly for panoramic X-rays which present unique interpretative challenges due to dense anatomical structures and subtle pathological cues.

Method: Created MMOral dataset with 20,563 annotated images and 1.3 million instruction instances across multiple task types. Developed MMOral-Bench evaluation suite covering five key diagnostic dimensions. Proposed OralGPT by conducting supervised fine-tuning on Qwen2.5-VL-7B using the MMOral dataset.

Result: Evaluation of 64 LVLMs showed even the best-performing model (GPT-4o) only achieved 41.45% accuracy. OralGPT demonstrated a 24.73% improvement after just one epoch of supervised fine-tuning, showing substantial performance enhancements for LVLMs in dental domain.

Conclusion: MMOral and OralGPT provide a critical foundation for intelligent dentistry and enable more clinically impactful multimodal AI systems in the dental field, addressing the significant limitations of current models in specialized medical domains like oral radiology.
...
TL;DR: PSP-Seg is a progressive pruning framework for efficient 3D medical image segmentation that dynamically prunes redundant modules, achieving nnU-Net-level performance with significantly reduced resource consumption.


<details>
  <summary>Details</summary>
Motivation: 3D medical image segmentation faces heavy resource and time constraints, limiting clinical scalability. Existing static models lack adaptability and struggle to balance performance with efficiency across diverse tasks.

Method: PSP-Seg starts with a redundant model and iteratively prunes redundant modules using block-wise pruning and functional decoupling loss to create efficient segmentation models.

Result: PSP-Seg-S (lightweight variant) matches nnU-Net performance while reducing GPU memory by 42-45%, training time by 29-48%, and parameters by 83-87% across five datasets.

Conclusion: PSP-Seg provides a cost-effective, high-performing alternative for clinical 3D segmentation with significant resource savings while maintaining state-of-the-art performance.
...
Method: Proposes Visual Programmability concept and adaptive framework where VLM learns to choose between CaT (code-based symbolic representation) pathway and direct visual reasoning pathway. Uses reinforcement learning with dual-reward system (data-accuracy reward + decision reward).

Result: Demonstrates strong and robust performance across diverse chart-understanding benchmarks. The model learns to dynamically select optimal reasoning pathway for each task.

Conclusion: VLMs can be taught not only to reason but also how to reason, with adaptive pathway selection between code-based symbolic reasoning and direct visual analysis for optimal chart understanding.
...
TL;DR: A U-net modification with modality-agnostic input channels enables brain MRI segmentation on both seen and unseen modalities using synthetic data augmentation.
...
Result: Evaluated on 8 MRI databases with 5 pathologies and 8 modalities, the approach preserves performance on trained modalities while effectively processing new unseen modalities to improve segmentation.

Conclusion: The proposed simple architectural modification enables flexible inference on any available imaging modalities, including previously unseen ones, making it practical for clinical applications with varying MRI protocols.
...
TL;DR: Novel model-agnostic open-set detection framework for UAV air-to-air object detection that handles unknown objects and corrupted flight data using embedding space entropy modeling with spectral normalization and temperature scaling.
...
Result: Achieves up to 10% relative AUROC gain compared to standard YOLO-based detectors on AOT aerial benchmark, with comprehensive ablation studies showing consistent improvements.

Conclusion: The framework provides reliable UAV perception in dynamic air-to-air environments by maintaining detection accuracy while enhancing robustness against unknown objects and corrupted data.
...
TL;DR: MatCha is the first benchmark for evaluating multimodal large language models on materials characterization image understanding, revealing significant performance gaps compared to human experts.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in MLLMs' capacity to understand real-world materials characterization imaging data and assess their ability to handle expert-level domain expertise in materials science.

Method: Developed MatCha benchmark with 1,500 questions across 4 key stages of materials research and 21 distinct tasks, then evaluated state-of-the-art MLLMs on this benchmark.
...
Conclusion: Existing MLLMs have limited adaptability to real-world materials characterization scenarios, and MatCha serves as a valuable benchmark to facilitate future research in material discovery and autonomous scientific agents.
...
<details>
  <summary>Details</summary>
Motivation: SAR images contain complex background clutter that resembles targets, causing models to extract entangled or spurious features that undermine clear target representation.

Method: Extracts high-level semantic features from SARATR-X and low-level scattering features from wavelet scattering network, then integrates them with a multi-level slot attention module for enhanced representation distinctiveness.
...
Conclusion: SlotSAR effectively disentangles target representations from background clutter without mask annotations, demonstrating superior performance in SAR target characterization.
...
Result: Achieves strong performance on OPV2V dataset, comparable to SOTA methods trained on full dataset using only small unlabeled data
...
<details>
  <summary>Details</summary>
Motivation: To comprehensively evaluate both language-guided and vision-only classification capabilities of dual-encoder VLMs, and understand their complementary strengths.

Method: Evaluated diverse dual-encoder VLMs on ImageNet-1k, analyzed factors affecting accuracy (prompt design, class diversity, k-NN neighbors, reference set size), and introduced a learning-free fusion method based on per-class precision.
...
Conclusion: Combining language and vision approaches through simple fusion can enhance VLM classification capabilities, demonstrating the value of leveraging both modalities.
...
TL;DR: A new framework called BUG uses large multimodal models to automatically create and fine-grain customize clothing designs from chat conversations with image-to-prompt conversion, eliminating text uncertainty issues for non-professional users.


<details>
  <summary>Details</summary>
Motivation: Current generative AI models can easily transform brainstorming into fashion designs but struggle with fine-grained customization due to text uncertainty from end-users lacking professional fashion background knowledge.

Method: Proposes the Better Understanding Generation (BUG) workflow with LMM that automatically creates and customizes clothing designs through chat with image-into-prompt conversion, eliminating the need for human involvement in design/editing.

Result: Created a new FashionEdit dataset simulating real-world clothing design workflow, evaluated on generation similarity, user satisfaction, and quality. The framework successfully lowers barriers for clothing design without professional knowledge.

Conclusion: The BUG framework effectively unleashes users' creative potential beyond textual descriptions and makes clothing design/editing accessible to non-professionals through automated multimodal generation.
...
TL;DR: Self-supervised pre-training with domain-relevant surgical data outperforms large-scale but less aligned sources for few-shot surgical skill assessment, achieving up to 73.65% accuracy.


<details>
  <summary>Details</summary>
Motivation: Surgical skill assessment requires expert annotations that are scarce and time-consuming to produce, making few-shot learning a scalable alternative that depends on effective pre-training strategies.

Method: Formulated SSA as a few-shot task, annotated robotic surgery dataset with OSATS scores, evaluated various pre-training sources across 1-, 2-, and 5-shot settings, and analyzed domain similarity effects.

Result: Domain-relevant datasets achieved 60.16%, 66.03%, and 73.65% accuracy in 1-, 2-, and 5-shot settings. Procedure-specific data integration with domain-relevant sources boosted performance (+1.22% accuracy, +2.28% F1-score), while less similar large-scale sources degraded performance.

Conclusion: Small domain-relevant datasets outperform large but less aligned ones for surgical skill assessment. Strategic inclusion of procedure-specific data with domain-relevant pre-training significantly enhances few-shot performance.
...
TL;DR: A vision-based driver behavior classification system that detects distracted and impaired driving using external observation techniques, computer vision, and real-time object tracking without requiring inter-vehicular communication.


<details>
  <summary>Details</summary>
Motivation: Road traffic accidents caused by human error, particularly distracted and impaired driving, remain a significant global concern that needs addressing through advanced detection systems.

Method: Uses advanced computer vision methodologies including YOLO object detection model, real-time object tracking, lateral displacement analysis, lane position monitoring, and custom lane estimation algorithms to identify unsafe driving behaviors.
...
TL;DR: FLUX-Reason-6M is a 6M image dataset with 20M bilingual descriptions for complex reasoning in text-to-image generation, and PRISM-Bench is a comprehensive evaluation benchmark with 7 tracks to assess model performance.


<details>
  <summary>Details</summary>
Motivation: Address the performance gap between open-source and closed-source text-to-image models by providing large-scale reasoning-focused datasets and robust evaluation standards.

Method: Created FLUX-Reason-6M dataset with 6M high-quality images organized by 6 characteristics using Generation Chain-of-Thought (GCoT) for detailed breakdowns. Developed PRISM-Bench with 7 evaluation tracks using advanced vision-language models for human-aligned assessment.

Result: Evaluation of 19 leading models revealed critical performance gaps and identified specific areas needing improvement in reasoning-oriented text-to-image generation.

Conclusion: The release of dataset, benchmark, and evaluation code aims to catalyze advancements in reasoning-oriented text-to-image generation and bridge the gap with closed-source systems.
...
TL;DR: Novel method for single-image intrinsic image decomposition that handles complex scenes with severe lighting and rich textures using texture-guided regularization.
...
Conclusion: Combining texture-aware prior enables effective separation of reflectance and shading layers in complex scenes with varying lighting and textures.
...
<details>
  <summary>Details</summary>
Motivation: Explore the connection between Plug-and-Play methods and Denoising Diffusion Implicit Models for solving ill-posed inverse problems, particularly single-pixel imaging

Method: Decouple diffusion process into three stages (denoising, data consistency enforcement, sampling) and propose hybrid data-consistency module that linearly combines multiple PnP-style fidelity terms

Result: Experimental results on single-pixel imaging tasks demonstrate better reconstruction quality
...
<details>
  <summary>Details</summary>
Motivation: Intracranial pressure monitoring is crucial but current invasive methods like lumbar puncture carry risks. Non-invasive ONSD measurement shows promise but suffers from manual operation inconsistency, subjectivity, and variability in current clinical practices.
...
Method: Proposes a framework with a normal-information extractor that aggregates representative normal features from test images, coherence loss to associate features with normal regions, and decoder to reconstruct normal content. Uses reconstruction residuals for defect segmentation and pseudo-anomaly augmentation for training stability.
...
TL;DR: DRiFt is a feature decoupling framework that separates clinically relevant signals from task-agnostic noise in medical VLMs, improving both in-distribution performance and robustness to domain shifts.
...
TL;DR: FS-Diff is a novel method that jointly performs image fusion and super-resolution using semantic guidance and clarity-aware mechanisms through a conditional generation approach with bidirectional feature Mamba and modified U-Net.


<details>
  <summary>Details</summary>
Motivation: Real-world applications like military reconnaissance and long-range detection often suffer from corrupted target/background structures, low resolution, and weak semantic information in multimodal images, leading to suboptimal fusion results with current techniques.

Method: Unifies image fusion and super-resolution as conditional generation problem. Uses clarity sensing mechanism for adaptive low-resolution perception, bidirectional feature Mamba for global feature extraction, and modified U-Net for random iterative denoising process at multiple noise levels.

Result: Outperforms state-of-the-art methods at multiple magnifications on six public datasets and a new AVMS benchmark (600 image pairs), recovering richer details and semantics in fused images.

Conclusion: FS-Diff effectively addresses the challenges of joint image fusion and super-resolution in real-world scenarios, providing superior performance in preserving details and semantic information across multiple magnification levels.
...
TL;DR: This paper addresses over-dispersion in dense self-supervised learning by proposing explicit semantic concentration methods including patch correspondence distillation with noise-tolerant ranking loss and object-aware filtering.


<details>
  <summary>Details</summary>
Motivation: Mainstream image-level SSL methods suffer from over-dispersion where patches from the same instance/category scatter, harming downstream dense task performance. The authors identify that image-level SSL avoids this through implicit semantic concentration mechanisms that are infeasible for dense SSL due to spatial sensitivity and complex scene data.

Method: 1) Distill patch correspondences to break strict spatial alignment using a noise-tolerant ranking loss that extends AP loss to continuous targets. 2) Object-aware filter to map output space to object-based space using learnable prototypes via cross-attention. 3) The ranking loss is decision-agnostic and adaptively focuses to prevent model from being misled by noisy pseudo labels.

Result: Empirical studies across various tasks soundly support the effectiveness of the proposed method. The approach successfully addresses over-dispersion in dense SSL.

Conclusion: The proposed explicit semantic concentration framework with patch correspondence distillation and object-aware filtering effectively solves the over-dispersion problem in dense self-supervised learning, enabling better performance on dense tasks.
...
Method: Transforms diffusion fusion into maximum likelihood estimation using hierarchical Bayesian modeling, integrates Expectation-Maximization algorithm into diffusion sampling to handle flexible input quantities end-to-end.
...
Conclusion: FlexiD-Fuse successfully addresses the limitation of fixed-input fusion methods and demonstrates superior performance across multiple medical and general image fusion tasks with varying modality inputs.
...
Conclusion: The compact model (90MB) with sub-minute inference demonstrates strong generalizability and practical potential for clinical deployment in resource-constrained African healthcare settings.
...
Method: Created a dataset with 3M real images paired with captions, used to generate 963k high-quality synthetic images from both proprietary and open-source models. Introduced a crowdsourced adversarial platform where participants generate challenging synthetic images to test detection methods.
...
Result: The proposed approach reduces foot sliding and jitter, improves balance, and preserves motion accuracy while avoiding complex joint torque calculations.

Conclusion: Momentum-based constraints provide an effective way to maintain physical consistency between local and global motion components, leading to more realistic human motion generation.
...
TL;DR: A novel Transformer-based framework for predicting region-wise correspondence between manga line art images without pre-existing labels, achieving high accuracy and consistent results.
...
Conclusion: The method demonstrates strong potential for real-world manga applications by effectively solving the region-wise correspondence problem without requiring pre-existing labels or masks.
...
TL;DR: Proposes SGDF method and GDCN network for multi-view clustering that handles noisy and missing data using generative diffusion mechanisms
...
Conclusion: The proposed SGDF method and GDCN network effectively handle low-quality multi-view data and outperform existing approaches
...
<details>
  <summary>Details</summary>
Motivation: Traditional 3D ultrasound systems are costly and complex, limiting widespread adoption. Sensorless 3D US using deep learning is promising but existing approaches either ignore global features or tightly couple them with local features, restricting robust modeling of complementary aspects.

Method: Proposes DualTrack with decoupled local and global encoders. Local encoder uses dense spatiotemporal convolutions for fine-grained features. Global encoder uses image backbone and temporal attention for anatomical features and long-range dependencies. Features are combined via lightweight fusion module for trajectory estimation.

Result: Achieves state-of-the-art accuracy on large public benchmark, outperforming previous methods with average reconstruction error below 5 mm. Produces globally consistent 3D reconstructions.

Conclusion: DualTrack's decoupled architecture effectively leverages both local and global features for robust 3D ultrasound probe trajectory estimation, demonstrating superior performance over existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: While video diffusion models have advanced through architectural innovations, less attention has been paid to improving feature representation power. The authors propose that aligning intermediate features with pre-trained vision encoders can enhance video generation quality.
...
Result: Align4Gen improves video generation quality for both unconditional and class-conditional video generation tasks, as quantified by various metrics.

Conclusion: Feature alignment with pre-trained vision encoders is an effective approach to enhance video diffusion models, complementing existing architectural innovations and training objectives.
...
<details>
  <summary>Details</summary>
Motivation: Existing human motion capture datasets lack extensive high-quality motion and annotations for dynamic 3D human-object interactions, suffering from artifacts like contact penetration, floating, and incorrect hand motions.

Method: Consolidated and standardized 21.81 hours of HOI data from diverse sources with detailed textual annotations. Proposed a unified optimization framework using contact invariance principles to reduce artifacts and correct hand motions while maintaining human-object relationships.

Result: Expanded dataset to 30.70 hours with enhanced quality. Achieved state-of-the-art performance on six benchmarking tasks for HOI generative modeling. Extensive experiments validated the dataset as a foundational resource.

Conclusion: InterAct provides a high-quality, large-scale benchmark for 3D human-object interaction research, addressing previous dataset limitations and advancing HOI generation capabilities. The dataset is publicly available and actively maintained.
...
TL;DR: Study reveals race and sex-based shortcut learning and demographic bias in deep learning models for Alzheimer's disease diagnosis from MRI scans, showing models can identify protected attributes and exhibit performance drops with training set imbalances.


<details>
  <summary>Details</summary>
Motivation: Deep learning algorithms for Alzheimer's disease diagnosis from MRI may suffer from shortcut learning where spurious features related to protected attributes like race and sex are used for prediction, leading to performance bias against underrepresented groups.

Method: Investigates if DL algorithms can identify race/sex from 3D brain MRI scans, examines training set imbalance effects on performance, and conducts quantitative/qualitative analysis of feature attributions using ResNet and SwinTransformer models on multiple datasets.

Result: Demonstrates existence of both race and sex-based shortcut learning and bias in DL-based AD classification, showing models can identify protected attributes and exhibit performance drops with training set imbalances.

Conclusion: The work establishes foundation for developing fairer deep learning diagnostic tools in brain MRI by identifying and addressing demographic biases in Alzheimer's disease classification models.
...
TL;DR: PeftCD is a parameter-efficient change detection framework using Vision Foundation Models with LoRA and Adapter modules, achieving SOTA performance across multiple remote sensing datasets with excellent boundary precision and pseudo-change suppression.
...
Result: Achieves state-of-the-art performance on multiple datasets: SYSU-CD (73.81% IoU), WHUCD (92.05%), MSRSCD (64.07%), MLCD (76.89%), CDD (97.01%), S2Looking (52.25%), and LEVIR-CD (85.62%) with precise boundaries and strong pseudo-change suppression.

Conclusion: PeftCD provides an optimal balance of accuracy, efficiency, and generalization, offering a scalable paradigm for adapting large VFMs to real-world remote sensing change detection applications.
...
Method: Two-stage pipeline: 1) MLLM director produces blueprint video conditioned on multimodal instructions for high-level semantics; 2) Parallel generation of sub-clips using first-last frame strategy guided by blueprint keyframes.

Result: Generates vivid, fluent, long-duration videos at 1080p/48fps with superior lip sync accuracy, emotion expressiveness, instruction controllability, identity preservation, and cross-domain generalization.
...
TL;DR: Hybrid framework combining mathematical tumor growth model with guided diffusion model to predict and synthesize future brain MRI scans from previous images.


<details>
  <summary>Details</summary>
Motivation: Predicting spatio-temporal progression of brain tumors is essential for clinical decision-making in neuro-oncology, particularly in data-limited scenarios.

Method: Combines mechanistic ODE-based tumor growth model with gradient-guided denoising diffusion implicit model (DDIM) to synthesize anatomically feasible future MRIs conditioned on predicted tumor burden.

Result: Generates realistic follow-up scans with high spatial similarity, produces tumor growth probability maps capturing extent and directionality (95th percentile Hausdorff Distance), validated on BraTS and pediatric DMG datasets.

Conclusion: Enables biologically informed image generation that accounts for mechanistic priors, offering generative-space-time predictions for clinical applications.
...
TL;DR: HumbleBench is a new benchmark that evaluates multimodal LLMs' ability to reject incorrect answers and show epistemic humility by choosing "None of the above" when no options are correct, addressing hallucination risks in visual question answering.
...
Result: Evaluated various state-of-the-art MLLMs and found valuable insights about their ability to recognize when no provided answers are correct, filling a key gap in current evaluation methods.

Conclusion: HumbleBench provides a more realistic measure of MLLM reliability by testing epistemic humility and false-option rejection capability, addressing hallucination risks in safety-critical settings.
...
TL;DR: UAE framework unifies image understanding (I2T) and generation (T2I) through bidirectional reconstruction training, using reinforcement learning to achieve mutual improvement between caption quality and image reconstruction fidelity.
...
Method: Proposes UAE framework with three-stage Unified-GRPO RL approach: cold-start initialization, Generation for Understanding (encoder trained to generate captions that maximize decoder reconstruction), and Understanding for Generation (decoder refined to reconstruct from detailed captions).
...
Conclusion: The unified bidirectional training paradigm brings mutual gains between understanding and generation, establishing a novel approach for multimodal learning with improved long-context instruction following and generation quality.
...
TL;DR: NRMF is a novel 3D generative human motion prior that models motion through neural distance fields on Riemannian manifolds, enabling robust and physically plausible motion recovery across various tasks.
...
Method: Models human motion as zero level set of neural distance fields (NDFs) on product space of joint rotations, angular velocities, and accelerations. Uses adaptive-step hybrid projection algorithm and geometric integrator for motion rollout.

Result: Significant improvements in motion recovery tasks including denoising, motion in-betweening, and fitting to partial 2D/3D observations. Generalizes well across multiple input modalities.

Conclusion: NRMF provides a rigorous geometric framework for 3D human motion generation that outperforms existing methods by explicitly modeling motion dynamics on Riemannian manifolds, ensuring physical plausibility and temporal consistency.
...
<details>
  <summary>Details</summary>
Motivation: To understand why deep diffusion models differ from the theoretically optimal denoiser and characterize the performance gap between them.

Method: Demonstrated that optimal parametric linear denoisers exhibit locality similar to deep neural denoisers, analyzed pixel correlations in natural images theoretically and experimentally, and crafted an improved analytical denoiser.

Result: Found that locality arises from pixel correlations in natural image datasets, not CNN inductive biases. Created an analytical denoiser that better matches deep diffusion model scores than prior alternatives.

Conclusion: The performance gap in diffusion models is primarily due to statistical properties of image data rather than architectural biases, enabling more accurate analytical modeling of deep diffusion behavior.
...
TL;DR: SpatialVID is a large-scale dataset of 7,089 hours of diverse real-world videos with dense 3D annotations including camera poses, depth maps, and motion instructions to address data scarcity in spatial intelligence research.
...
Method: Collected over 21,000 hours of raw video, processed through hierarchical filtering to 2.7 million clips (7,089 hours), then annotated with camera poses, depth maps, dynamic masks, structured captions, and serialized motion instructions.

Result: Created SpatialVID dataset with rich spatial and semantic annotations across diverse scenes and camera movements, providing 7,089 hours of high-quality dynamic content for training spatial intelligence models.

Conclusion: SpatialVID's scale, diversity, and annotation richness directly improve model generalization and performance, making it a valuable resource for video and 3D vision research communities.
...
TL;DR: Open-sci-ref introduces dense transformer models (0.13B-1.7B parameters) trained on 8 open reference datasets as research baselines, with NemoTron-CC HQ performing best, followed by DCLM-baseline and FineWeb-Edu.


<details>
  <summary>Details</summary>
Motivation: To establish standardized reference baselines for researchers to assess training approaches across different model scales and datasets, enabling better comparison and validation of alternative methods.

Method: Trained transformer models across multiple parameter scales (0.13B to 1.7B) and token scales (up to 1T tokens) on 8 recent open reference datasets, with intermediate checkpoints and comprehensive evaluation on standardized benchmarks.

Result: NemoTron-CC HQ consistently outperformed other reference datasets, followed by DCLM-baseline and FineWeb-Edu. The models provide reference points for scaling trends and training dynamics analysis.

Conclusion: The open-sci-ref family provides valuable reference baselines that enable standardized comparison of training procedures, facilitate reproduction, and support future research through released checkpoints, logs, code, and evaluations.
...
<details>
  <summary>Details</summary>
Motivation: Agro-tourism serves as a strategic economic model for rural development, income diversification for farmers, and conservation of cultural heritage. There is a need to study detailed strategies for its growth.

Method: Two-phase study: 1) Identification of important indicators through comprehensive literature review, 2) Application of machine learning feature selection techniques including LASSO method combined with Logistic Regression, Decision Trees, Random Forest, and XGBoost models.

Result: LASSO with Logistic Regression achieved highest classification accuracy: 98% in 70-30% train-test split and 99% in 80-20% split. Random Forest achieved 95% accuracy, while Decision Trees and XGBoost achieved 97% accuracy in the 80-20 split.

Conclusion: Machine learning feature selection methods, particularly LASSO combined with Logistic Regression, are highly effective for identifying key indicators and predicting agro-tourism growth with exceptional accuracy.
...
Method: Proposes Entropy-Modulated Policy Gradients (EMPG) that recalibrates learning signals based on step-wise uncertainty and final outcomes, with bonus terms for future clarity.
...
TL;DR: LLMs can generate valid counterfactual explanations but struggle with minimality - they either make excessive edits or tiny ineffective changes, revealing a validity-minimality trade-off that limits their usefulness as explainability tools.
...
Method: The study evaluates LLMs' ability to produce self-generated counterfactual explanations (SCEs) by testing whether they can generate explanations that are both valid (achieve intended outcome) and minimal (make smallest necessary changes).

Result: LLMs typically produce valid SCEs but far from minimal ones, offering little insight. When asked for minimal counterfactuals, they make excessively small edits that fail to change predictions, showing a consistent validity-minimality trade-off across models and datasets.

Conclusion: SCEs are ineffective explainability tools that can provide misleading insights. Deploying LLMs in high-stakes settings requires careful consideration of unreliable self-explanations' impact on decision-making.
...
TL;DR: ButterflyQuant introduces learnable butterfly transforms to replace fixed Hadamard rotations for 2-bit quantization, achieving better performance by adapting to layer-specific outlier patterns with minimal computational overhead.
...
Method: Proposes ButterflyQuant with learnable butterfly transforms parameterized by continuous Givens rotation angles, enabling gradient-based optimization while guaranteeing orthogonality. Uses uniformity regularization on post-transformation activations and requires only 128 calibration samples.

Result: Achieves 15.4 perplexity on LLaMA-2-7B with 2-bit quantization versus 22.1 for QuaRot, with O(n log n) complexity and only (n log n)/2 learnable parameters.

Conclusion: Layer-adaptive butterfly transforms outperform fixed rotations for extreme quantization, providing better outlier suppression with minimal one-time learning cost on consumer hardware.
...
TL;DR: LRP attribution methods analyzed through matrix representation, showing component-wise bounds and convergence constants that distinguish LRP-beta from gradient methods and LRP-epsilon.


<details>
  <summary>Details</summary>
Motivation: To understand the numerical properties and distribution characteristics of Layer-wise Relevance Propagation attribution methods through mathematical analysis.

Method: Represented LRP-type methods as product of modified gradient matrices, derived upper bounds for singular values, and established component-wise bounds for attribution values.

Result: Derived multiplicative constants governing convergence of empirical attribution means, showing LRP-beta constants remain independent of weight norms unlike gradient methods and LRP-epsilon.

Conclusion: LRP-beta exhibits unique numerical stability properties with weight-norm-independent constants, providing advantages for data augmentation scenarios and Smoothgrad-type attribution methods.
...
TL;DR: Training-free adaptive token merging framework for vision transformers that reduces computation and transmission costs in semantic communication systems while maintaining accuracy across varying channel conditions.
...
Method: Formulates token merging proportion selection as multi-objective optimization, uses Gaussian process-based Bayesian optimization to create Pareto frontier of optimal configurations for adaptive runtime adjustment.

Result: Outperforms other baselines, achieves significant FLOPs reduction while maintaining competitive accuracy across various SNR conditions, enables effective adaptive policies for channel quality response.
...
<details>
  <summary>Details</summary>
Motivation: Traditional evaluation metrics for deep learning weather models fail to capture rare, high-impact events, creating a trap where models achieve high statistical correlation but completely miss dangerous weather phenomena like extreme convection.

Method: DART (Dual Architecture for Regression Tasks) uses dual-decoder architecture with explicit background/extreme decomposition, physically motivated oversampling, and task-specific loss functions to transform coarse atmospheric forecasts into high-resolution satellite brightness temperature fields.

Result: DART achieves CSI = 0.273 with bias = 2.52 vs. 6.72 for baselines at equivalent CSI, improves extreme convection detection by 270% by removing Integrated Water Vapor Transport, and validates on real-world flooding disaster case study.

Conclusion: DART successfully addresses the hybrid conversion-segmentation-downscaling task, enables precise operational calibration, trains quickly on standard hardware, and integrates with meteorological workflows, providing a pathway for trustworthy AI in extreme weather preparedness.
...
TL;DR: A novel unsupervised graph alignment framework that enhances node distinctiveness and enforces geometric consistency across graphs using dual-pass spectral filtering and geometry-aware functional maps.
...
Method: Proposes a dual-pass encoder combining low-pass and high-pass spectral filters for discriminative embeddings, and a geometry-aware functional map module for learning bijective isometric transformations between graph embeddings.

Result: Extensive experiments show consistent outperformance of existing unsupervised baselines, with superior robustness to structural inconsistencies. Also generalizes effectively to vision-language alignment tasks.

Conclusion: The framework successfully addresses key limitations in graph alignment by simultaneously enhancing node distinctiveness and enforcing geometric consistency, demonstrating strong performance across diverse domains including vision-language representations.
...
### [125] [OmniEVA: Embodied Versatile Planner via Task-Adaptive 3D-Grounded and Embodiment-aware Reasoning](https://arxiv.org/abs/2509.09332)
*Yuecheng Liu,Dafeng Chi,Shiguang Wu,Zhanguang Zhang,Yuzheng Zhuang,Bowen Yang,He Zhu,Lingfeng Zhang,Pengwei Xie,David Gamaliel Arcos Bravo,Yingxue Zhang,Jianye Hao,Xingyue Quan*

Main category: cs.RO

TL;DR: OmniEVA is an embodied versatile planner that addresses geometric adaptability and embodiment constraint gaps in MLLM-based systems through task-adaptive 3D grounding and embodiment-aware reasoning.
...
Method: OmniEVA introduces two innovations: (1) Task-Adaptive 3D Grounding mechanism with a gated router for explicit selective regulation of 3D fusion based on contextual requirements, and (2) Embodiment-Aware Reasoning framework that jointly incorporates task goals and embodiment constraints into the reasoning loop.

Result: Extensive experiments show OmniEVA achieves state-of-the-art general embodied reasoning performance and exhibits strong ability across a wide range of downstream scenarios. Evaluations on embodied benchmarks confirm robust and versatile planning capabilities.

Conclusion: OmniEVA successfully addresses the identified gaps in MLLM-based embodied systems, providing context-aware 3D grounding and executable planning decisions that are both goal-directed and physically feasible for real robots.
...
### [126] [SimpleVLA-RL: Scaling VLA Training via Reinforcement Learning](https://arxiv.org/abs/2509.09674)
...
<details>
  <summary>Details</summary>
Motivation: Address the scarcity and high cost of large-scale human-operated robotic trajectories for supervised fine-tuning, and overcome limited generalization to tasks with distribution shift in VLA models.

Method: Builds upon veRL with VLA-specific trajectory sampling, scalable parallelization, multi-environment rendering, and optimized loss computation applied to OpenVLA-OFT.

Result: Achieves state-of-the-art performance on LIBERO, outperforms baseline on RoboTwin 1.0&2.0, reduces data dependence, enables robust generalization, and discovers novel patterns through "pushcut" phenomenon.

Conclusion: RL can significantly improve VLA models' long-horizon action planning, surpassing SFT in real-world tasks while reducing data requirements and enhancing generalization capabilities.
...
### [127] [ObjectReact: Learning Object-Relative Control for Visual Navigation](https://arxiv.org/abs/2509.09594)
*Sourav Garg,Dustin Craggs,Vineeth Bhat,Lachlan Mares,Stefan Podgorski,Madhava Krishna,Feras Dayoub,Ian Reid*

Main category: cs.RO

TL;DR: Object-relative visual navigation using 3D scene graphs instead of image matching, enabling better cross-embodiment generalization and reverse-direction navigation.


<details>
  <summary>Details</summary>
Motivation: Image-level representations are tied to specific poses and embodiments, limiting navigation flexibility. Objects provide more invariant world representations that can work across different sensor configurations and trajectories.

Method: Proposes ObjectReact - a local controller conditioned on WayObject Costmap representation from relative 3D scene graphs, eliminating need for explicit RGB input and decoupling control from image matching.
...
Conclusion: Object-relative control paradigm offers superior invariance and generalization capabilities compared to traditional image-relative approaches for visual navigation with topological maps.
...
### [128] [Dexplore: Scalable Neural Control for Dexterous Manipulation from Reference-Scoped Exploration](https://arxiv.org/abs/2509.09671)
...
<details>
  <summary>Details</summary>
Motivation: Hand-object motion-capture repositories contain large-scale demonstrations but suffer from inaccuracies and embodiment gaps between human and robot hands. Existing three-stage workflows (retargeting, tracking, residual correction) compound errors and underuse demonstrations.

Method: Single-loop optimization that jointly performs retargeting and tracking. Uses raw trajectories to derive adaptive spatial scopes, trains with reinforcement learning to keep policy in-scope while minimizing control effort and accomplishing tasks. Distills into vision-based, skill-conditioned generative controller.

Result: Preserves demonstration intent, enables robot-specific strategies, improves robustness to noise, scales to large demonstration corpora. Creates rich latent representation supporting generalization across objects and real-world deployment.

Conclusion: Dexplore provides a principled bridge that transforms imperfect human demonstrations into effective training signals for dexterous robotic manipulation, overcoming limitations of existing multi-stage approaches.
...
### [129] [Explainable AI for Accelerated Microstructure Imaging: A SHAP-Guided Protocol on the Connectome 2.0 scanner](https://arxiv.org/abs/2509.09513)
...
TL;DR: Reduced 8-feature diffusion MRI protocol achieves comparable neurite exchange imaging results to full 15-feature protocol in just 14 minutes scan time using explainable AI optimization.
...
Method: Data-driven framework using explainable AI with guided recursive feature elimination to identify optimal 8-feature subset from original 15-feature protocol. Validated in vivo and benchmarked against full acquisition and alternative reduction strategies.
...
### [130] [CameraVDP: Perceptual Display Assessment with Uncertainty Estimation via Camera and Visual Difference Prediction](https://arxiv.org/abs/2509.08947)
*Yancheng Cai,Robert Wanat,Rafal Mantiuk*

Main category: cs.GR

TL;DR: CameraVDP framework combines camera-based display measurement with visual difference prediction to enable perceptual assessment of display artifacts and distortions.


<details>
  <summary>Details</summary>
Motivation: Traditional sparse radiometric sampling methods fail to capture high-frequency and pixel-level display artifacts, while cameras introduce optical and photometric distortions. There's a need for accurate perceptual assessment that accounts for both measurement inaccuracies and human visual system perception.

Method: Combines HDR image stacking, MTF inversion, vignetting correction, geometric undistortion, homography transformation, and color correction for camera-based reconstruction, integrated with a Visual Difference Predictor (VDP) to model human visual system perception under different viewing conditions.
...
Conclusion: The CameraVDP framework enables precise perceptual assessment of displays by addressing both camera measurement inaccuracies and visual difference prediction, making cameras function as accurate display measurement instruments for evaluating visible distortions.
...
### [131] [Generative Engine Optimization: How to Dominate AI Search](https://arxiv.org/abs/2509.08919)
*Mahe Chen,Xiaoxuan Wang,Kaiwen Chen,Nick Koudas*

Main category: cs.IR

TL;DR: AI-powered search engines (ChatGPT, Perplexity, Gemini) fundamentally differ from traditional search, showing systematic bias towards authoritative sources over brand/social content, requiring new Generative Engine Optimization (GEO) strategies.
...
Method: Conducted large-scale controlled experiments across multiple verticals, languages, and query paraphrases to quantify differences between AI search and traditional Google search in information sourcing patterns.

Result: AI search shows overwhelming bias towards earned media (third-party authoritative sources) versus Google's balanced mix; significant differences found in domain diversity, freshness, cross-language stability, and phrasing sensitivity among AI services.

Conclusion: Proposes GEO framework with actionable strategies: engineer content for machine scannability, dominate earned media for AI authority, adopt engine-specific/language-aware approaches, and overcome big brand bias for niche players in generative search landscape.
...
### [132] [Retrieval-Augmented Generation for Reliable Interpretation of Radio Regulations](https://arxiv.org/abs/2509.09651)
*Zakaria El Kassimi,Fares Fourati,Mohamed-Slim Alouini*

Main category: cs.IR

TL;DR: A telecom-specific RAG pipeline for radio regulations QA achieves 97% retrieval accuracy and 12% improvement over naive document insertion for GPT-4o, with a new domain-specific evaluation set.
...
Method: Proposed a telecom-specific RAG pipeline with automated filtering and human validation to create the first multiple-choice evaluation set for this domain, using a domain-specific retrieval metric.
...
### [133] [DeepTV: A neural network approach for total variation minimization](https://arxiv.org/abs/2409.05569)
*Andreas Langer,Sara Behnamian*

Main category: math.NA

TL;DR: Neural networks for solving infinite-dimensional total variation minimization problems face theoretical issues with solution existence, but an auxiliary neural network approach with Γ-convergence guarantees provides a viable solution.
...
Method: Proposed an auxiliary neural network problem that has a solution and proved its Γ-convergence to the original problem. Also developed a discrete version of the auxiliary problem with Γ-convergence guarantees and connection to finite difference discretization.
...
Conclusion: The proposed auxiliary neural network framework provides a theoretically sound approach for solving infinite-dimensional total variation minimization problems, overcoming the solution existence issues of direct neural network applications.
...
TL;DR: Novel dynamic structural parameters integrated in multimodal deep learning framework improve prediction of postoperative visual recovery in macular hole patients.


<details>
  <summary>Details</summary>
Motivation: To enhance prediction accuracy for postoperative visual recovery in idiopathic full-thickness macular hole patients by incorporating novel dynamic structural parameters into a multimodal deep learning framework.
...
Result: Segmentation achieved high accuracy (mean Dice > 0.89), identified key predictors (base diameter, ellipsoid zone integrity, macular hole area), dynamic parameters improved logistic regression AUC, and multimodal DL model outperformed regression with up to 0.12 higher AUC.

Conclusion: Integration of dynamic parameters into multimodal DL significantly enhances prediction accuracy, providing a promising automated clinical decision support tool for personalized postoperative management in macular hole surgery.
...
TL;DR: This paper introduces virtual staining to 3D X-ray imaging using deep learning to generate artificially stained histological images from micro-CT scans, enabling label-free 3D tissue characterization without physical sectioning.


<details>
  <summary>Details</summary>
Motivation: X-ray histology provides non-invasive 3D imaging but lacks biochemical specificity compared to traditional stained histology. The goal is to bridge this gap by applying virtual staining techniques to X-ray domain for enhanced interpretability.

Method: Used modified CycleGAN network trained on 50+ co-registered micro-CT and toluidine blue-stained histology image pairs. Incorporated pixelwise supervision and greyscale consistency terms with patch-based training and data augmentation.

Result: Outperformed Pix2Pix and standard CycleGAN baselines across SSIM, PSNR, and LPIPS metrics. Successfully reproduced histological features like new bone formation while preserving high-resolution structural detail.

Conclusion: The method enables generation of virtually stained 3D datasets from CT volumes, offering a scalable route for chemically informative, label-free tissue characterization, though some variability in implant degradation depiction indicates need for further refinement.
...
TL;DR: Proposes LUT-ILF++ framework using lookup tables instead of heavy DNN computations for in-loop filtering, achieving significant bitrate reduction with lower complexity.


<details>
  <summary>Details</summary>
Motivation: Neural network-based in-loop filtering provides excellent coding gains but has high computational complexity and hardware demands, making it impractical for general use. Need for a more efficient solution.
...
Conclusion: LUT-ILF++ provides a practical and efficient alternative to DNN-based in-loop filtering, achieving substantial coding gains with significantly reduced computational and storage requirements.
...
### [137] [Ultrafast Deep Learning-Based Scatter Estimation in Cone-Beam Computed Tomography](https://arxiv.org/abs/2509.08973)
*Harshit Agrawal,Ari Hietanen,Simo Särkkä*

Main category: eess.SP

TL;DR: Deep learning scatter estimation for CBCT with reduced computational cost through optimal resolution selection


<details>
  <summary>Details</summary>
Motivation: Scatter artifacts degrade CBCT image quality, but deep learning methods have large memory footprints limiting deployment on mobile/edge devices

Method: Examined reconstruction error at six resolutions with four interpolation methods, trained state-of-the-art network at five resolutions to optimize FLOPs, inference time, and GPU memory

Result: Achieved 78-fold FLOP reduction while improving performance (MAPE: 3.85% vs 4.42%, MSE: 1.34e-2 vs 2.01e-2), 16x faster inference, 12x less GPU memory

Conclusion: Downsampling plays crucial role in deep learning scatter estimation, enabling scatter correction in resource-constrained environments like mobile CBCT and edge devices
...
### [138] [Automated Unity Game Template Generation from GDDs via NLP and Multi-Modal LLMs](https://arxiv.org/abs/2509.08847)
*Amna Hassan*

Main category: cs.AI

TL;DR: Automated framework that converts Game Design Documents into Unity game prototypes using fine-tuned LLaMA-3 model for C# code generation, achieving superior performance over baseline models.


<details>
  <summary>Details</summary>
Motivation: To address critical gaps in AI-assisted game development by streamlining the transition from game design documentation to functional implementation, reducing manual coding effort.

Method: End-to-end system combining NLP and multi-modal LLMs to parse GDDs, extract structured specifications, and synthesize Unity-compatible C# code using a fine-tuned LLaMA-3 model with custom Unity integration package.

Result: Significant improvements over baseline models with 4.8/5.0 average score, superior performance in compilation success, GDD adherence, best practices adoption, and code modularity across multiple game genres.

Conclusion: The system effectively positions LLMs as valuable tools for automating game development workflows, demonstrating high adherence to design specifications and streamlining the design-to-implementation process.
...
### [139] [Tree-OPO: Off-policy Monte Carlo Tree-Guided Advantage Optimization for Multistep Reasoning](https://arxiv.org/abs/2509.09284)
...
<details>
  <summary>Details</summary>
Motivation: Leverage MCTS-derived trajectories from reasoning domains to enhance policy optimization in preference-based reinforcement learning, moving beyond traditional value/reward model training.
...
Conclusion: Heuristic and statistical solutions are proposed to mitigate issues, with ongoing challenges in learning under staged/tree-like reward structures requiring further research.
...
TL;DR: This paper proposes a neuroscience-grounded computational framework for agentic spatial intelligence, identifying six essential modules inspired by biological spatial reasoning and analyzing current methods against this framework.


<details>
  <summary>Details</summary>
Motivation: Current agentic AI systems have limited spatial reasoning abilities compared to human spatial intelligence, which is critical for effective interaction with physical 3D environments. The gap needs to be bridged for better autonomous systems.

Method: The authors introduce a computational framework with six neuroscience-inspired modules: bio-inspired multimodal sensing, multi-sensory integration, egocentric-allocentric conversion, artificial cognitive map, spatial memory, and spatial reasoning. They analyze existing methods against this framework.

Result: The framework provides a structured perspective for evaluating and developing spatial reasoning capabilities in AI agents, identifies critical gaps in current approaches, and examines emerging benchmarks and potential applications.

Conclusion: The work offers a neuroscience-grounded roadmap for advancing agentic spatial intelligence, with potential applications in robotics and embodied systems, and outlines promising research directions for generalization across dynamic environments.
...
### [141] [Bona fide Cross Testing Reveals Weak Spot in Audio Deepfake Detection Systems](https://arxiv.org/abs/2509.09204)
*Chin Yuen Kwok,Jia Qi Yip,Zhen Qiu,Chi Hung Chi,Kwok Yan Lam*

Main category: cs.SD

TL;DR: Proposes bona fide cross-testing framework to improve audio deepfake detection evaluation by incorporating diverse bona fide datasets and aggregating EERs for more balanced assessments.


<details>
  <summary>Details</summary>
Motivation: Traditional evaluation methods disproportionately weight synthesizers with more samples and lack diversity in bona fide speech, limiting real-world reliability.
...
Result: Benchmarked over 150 synthesizers across nine bona fide speech types, showing improved robustness and interpretability compared to traditional methods.

Conclusion: The proposed bona fide cross-testing framework provides more reliable evaluation of audio deepfake detection models and a new dataset is released to facilitate further research.
...
### [142] [DiFlow-TTS: Discrete Flow Matching with Factorized Speech Tokens for Low-Latency Zero-Shot Text-To-Speech](https://arxiv.org/abs/2509.09631)
...
<details>
  <summary>Details</summary>
Motivation: Existing zero-shot TTS methods suffer from slow inference and repetition artifacts. While discrete codec representations show promise, current flow-matching approaches embed tokens into continuous space, failing to fully leverage discrete representation advantages.

Method: Introduces purely discrete flow matching with factorized speech attributes modeling. Uses in-context learning with text, prosodic, and acoustic attributes from reference speech. Employs factorized flow prediction with separate heads for prosody and acoustic details.
...
Conclusion: Discrete flow matching effectively addresses zero-shot TTS challenges, offering superior speed and quality while preserving the benefits of discrete representations for speech synthesis.
...
### [143] [A vibe coding learning design to enhance EFL students' talking to, through, and about AI](https://arxiv.org/abs/2509.08854)
*David James Woo,Kai Guo,Yangyang Yu*

Main category: cs.CY

TL;DR: Pilot study on vibe coding (AI-assisted software development) for EFL education using a human-AI meta-languaging framework with three dimensions: talking to AI, through AI, and about AI.


<details>
  <summary>Details</summary>
Motivation: To explore how AI can function as a beneficial languaging machine in EFL education and understand the factors that influence successful vibe coding outcomes.

Method: Case study with two students in a 4-hour workshop using backward design principles. Data collected from worksheets, video recordings, think-aloud protocols, screen recordings, and AI-generated images.

Result: Contrasting outcomes - one student successfully created a functional application aligned with intended design, while another encountered technical difficulties with significant gaps between design and functionality. Differences in prompt engineering approaches and AI mental models were observed.
