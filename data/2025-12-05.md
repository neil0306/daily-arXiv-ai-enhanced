...
TL;DR: The paper identifies Lazy Likelihood Displacement (LLD) as the cause of training collapse in GRPO-based tool-integrated RL for LLMs, and proposes a likelihood-preserving regularization method to stabilize training.
...
Method: The paper proposes LLDS, a lightweight likelihood-preserving regularization for GRPO that activates only when trajectory likelihood decreases and regularizes only the responsible tokens, mitigating LLD with minimal optimization interference.

Result: The method stabilizes training, prevents gradient explosion, and yields substantial performance improvements: +37.8% gains on Qwen2.5-3B and +32.0% gains on Qwen2.5-7B across seven open-domain and multi-hop QA benchmarks.

Conclusion: LLD is identified as a fundamental bottleneck in GRPO-based tool-integrated RL, and the proposed likelihood-preserving regularization provides a practical path toward stable, scalable training of tool-integrated LLMs.
...
TL;DR: Multinomial Naive Bayes achieved best performance (85.89% accuracy) for Libyan dialect classification using word and character n-grams, outperforming logistic regression and SVM on Twitter data.
...
Method: Used QADI corpus (540k sentences across 18 dialects), applied chi-square analysis to filter non-significant features, tested logistic regression, linear SVM, multinomial Naive Bayes, and Bernoulli Naive Bayes with various word and character n-gram representations.

Result: Multinomial Naive Bayes achieved highest accuracy (85.89%) and F1-score (0.85741) using (1,2) word n-grams and (1,5) character n-grams, outperforming other classifiers. Chi-square analysis identified email mentions and emotion indicators as non-significant features.

Conclusion: Carefully selected n-gram representations and classification models significantly improve Libyan dialect identification accuracy, with Multinomial Naive Bayes being most effective. The study provides empirical benchmarks for Arabic dialect NLP research.
...
TL;DR: SQuARE is a hybrid retrieval framework for spreadsheet QA that routes queries based on sheet complexity, using either structure-preserving chunk retrieval or SQL over auto-constructed relational views, with an agent supervising retrieval refinement.


<details>
  <summary>Details</summary>
Motivation: Spreadsheet QA is challenging due to multi-row headers, merged cells, and unit annotations that disrupt naive chunking, while rigid SQL views fail on files lacking consistent schemas.
...
Result: SQuARE consistently surpasses single-strategy baselines and ChatGPT-4o on both retrieval precision and end-to-end answer accuracy across multi-header corporate balance sheets, heavily merged World Bank workbook, and diverse public datasets while keeping latency predictable.
...
TL;DR: DAComp is a benchmark of 210 tasks that simulate real enterprise data intelligence workflows, covering both data engineering (SQL pipeline development) and data analysis (open-ended business problem solving), revealing significant limitations in current AI agents' capabilities.


<details>
  <summary>Details</summary>
Motivation: Real-world enterprise data intelligence involves complex workflows from data engineering to analysis, but existing benchmarks don't adequately capture these holistic processes. There's a need for a realistic testbed to evaluate autonomous data agents' capabilities in both engineering and analytical tasks.

Method: Created DAComp benchmark with 210 tasks mirroring enterprise workflows. Data engineering tasks require multi-stage SQL pipeline development on industrial schemas. Data analysis tasks involve open-ended business problems requiring strategic planning and iterative analysis. Engineering tasks use execution-based multi-metric evaluation, while analysis tasks employ an LLM-judge with hierarchical rubrics.

Result: State-of-the-art agents perform poorly: DE tasks have success rates under 20%, exposing bottlenecks in holistic pipeline orchestration. DA tasks average below 40% success, showing deficiencies in open-ended reasoning. Results demonstrate engineering and analysis are distinct capabilities.

Conclusion: DAComp provides a rigorous testbed that reveals critical limitations in current autonomous data agents, particularly in holistic pipeline orchestration and open-ended reasoning. The benchmark will drive development of more capable agents for enterprise data intelligence workflows.
...
TL;DR: ClusterFusion: A hybrid framework using LLMs as clustering core guided by lightweight embeddings, achieving SOTA on standard tasks and substantial gains in specialized domains.


<details>
  <summary>Details</summary>
Motivation: Traditional clustering with pre-trained embeddings struggles in domain-specific contexts without costly fine-tuning. LLMs provide strong contextual reasoning but prior work only uses them as auxiliary modules rather than the core clustering mechanism.

Method: Three-stage hybrid framework: 1) embedding-guided subset partition, 2) LLM-driven topic summarization, and 3) LLM-based topic assignment. Treats LLM as clustering core guided by lightweight embeddings.

Result: Achieves state-of-the-art performance on three public benchmarks and delivers substantial gains on two new domain-specific datasets. Framework enables direct incorporation of domain knowledge and user preferences.

Conclusion: ClusterFusion effectively leverages LLMs' contextual adaptability for text clustering, outperforming traditional methods and previous LLM-based approaches, especially in specialized domains.
...
TL;DR: LangSAT is an RL-based framework that converts English descriptions to CNF and solves SAT problems using RL-enhanced CDCL, making SAT-solving more accessible without requiring CNF input.


<details>
  <summary>Details</summary>
Motivation: Existing SAT-solving platforms require CNF as input, which creates a barrier for users. The goal is to make SAT-solving more accessible by allowing natural language inputs and improving efficiency through RL optimization of heuristic selection in CDCL.

Method: Two-component framework: 1) Lang2Logic translates English sentences into CNF expressions, handling descriptions up to 450 words. 2) SmartSAT is an RL-based SAT solver that encodes clause-variable relationships as structured graph representations, extracts global SAT-specific features, and uses RL to optimize heuristic selection in CDCL.

Result: Lang2Logic successfully processed diverse natural language inputs. SmartSAT demonstrated comparable solving time performance to traditional CDCL heuristics. The combined framework offers accessible and scalable SAT-solving.

Conclusion: LangSAT bridges the gap between natural language and propositional logic, providing a more accessible and efficient SAT-solving solution applicable to reasoning, formal verification, and debugging tasks.
...
TL;DR: MASE is a model-agnostic saliency estimation framework that uses normalized linear Gaussian perturbations on embedding layers to provide local explanations for text-based predictive models in NLP.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks in NLP lack interpretability, and traditional post-hoc interpretation methods like saliency maps or feature visualization are not well-suited for discrete word data. There's a need for model-agnostic approaches that can explain text-based models without requiring knowledge of their internal architecture.

Method: MASE uses Normalized Linear Gaussian Perturbations (NLGP) applied to the embedding layer rather than raw word inputs to estimate input saliency. This model-agnostic framework provides local explanations for text-based predictive models.

Result: MASE demonstrates superiority over other model-agnostic interpretation methods, particularly in terms of Delta Accuracy, making it an effective tool for explaining text-based models in NLP.

Conclusion: MASE is a promising framework for elucidating the decision-making processes of text-based models in NLP, offering a model-agnostic approach that works effectively with the discrete nature of word data through embedding layer perturbations.
...
TL;DR: Classical ML with feature engineering achieves ~0.57 F1 for sarcasm detection on Reddit data without neural networks or conversational context.


<details>
  <summary>Details</summary>
Motivation: Sarcasm is prevalent online but challenging for machines due to contradictory literal vs. intended meaning. The paper aims to establish a clear, reproducible baseline using interpretable classical methods rather than complex neural approaches.

Method: Uses a 100k-comment subsample from SARC 2.0. Combines word-level and character-level TF-IDF features with simple stylistic indicators. Evaluates four classical ML models: logistic regression, linear SVM, multinomial Naive Bayes, and random forest.

Result: Naive Bayes and logistic regression perform strongest, achieving F1-scores around 0.57 for sarcastic comments. Performance is limited by lack of conversational context, but results provide a solid baseline.

Conclusion: Classical ML with explicit feature engineering offers a lightweight, interpretable, and reproducible baseline for sarcasm detection, though performance is constrained without conversational context.
...
TL;DR: RapidUn is an influence-driven, parameter-efficient unlearning framework that uses per-sample influence estimation and adaptive update weights to selectively forget harmful data while retaining general knowledge, achieving up to 100x efficiency over retraining.


<details>
  <summary>Details</summary>
Motivation: Current LLM unlearning methods face challenges: retraining is costly, approximate methods are unstable, and performance degrades with small/imbalanced forget sets. There's a need for efficient, stable unlearning that preserves general knowledge while removing specific harmful data.
...
TL;DR: MSME is a multi-stage, multi-expert framework for zero-shot stance detection that addresses challenges like dynamic knowledge needs, complex targets, and rhetorical devices through specialized expert modules and knowledge integration.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based stance detection struggles with complex real-world scenarios requiring dynamic background knowledge, handling compound entities/events, and interpreting rhetorical devices like irony that obscure author intent.

Method: Three-stage framework: (1) Knowledge Preparation retrieves background knowledge and clarifies stance labels; (2) Expert Reasoning with three specialized modules (Knowledge Expert, Label Expert, Pragmatic Expert); (3) Decision Aggregation where Meta-Judge integrates expert analyses for final prediction.

Result: Experiments on three public datasets show MSME achieves state-of-the-art performance across all tested scenarios.

Conclusion: MSME effectively addresses limitations of existing LLM-based stance detection by incorporating specialized expertise and structured reasoning, demonstrating superior performance in complex zero-shot stance detection tasks.
...
<details>
  <summary>Details</summary>
Motivation: To benchmark and develop effective methods for constructing accurate timelines of systemic anticancer treatment from electronic health records, specifically focusing on extracting chemotherapy information from raw clinical notes.
...
Conclusion: The results and analyses provide useful insights for future attempts on chemotherapy timeline extraction tasks and the design of similar clinical NLP tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current knowledge editing methods have two key limitations: 1) they rely on structured triplets that don't match LLMs' free-text pretraining nature, and 2) they only support one-time updates rather than sequential/lifelong editing. There's a need for methods that can handle natural language updates and support continual editing over time.

Method: Introduces LF-Edit task for lifelong free-text knowledge editing, creates MRLF-Bench benchmark with 16,835 free-text edit requests, and proposes EvoEdit method with two key components: Latent Perturbation Augmentation for better knowledge injection and Knowledge-driven Parameter Fusion for preserving prior information.

Result: EvoEdit substantially outperforms existing knowledge editing methods on the proposed LF-Edit task, demonstrating effectiveness in both integrating new knowledge and mitigating forgetting of prior information.

Conclusion: LF-Edit addresses critical gaps in knowledge editing by supporting natural language updates and lifelong editing, with EvoEdit providing an effective solution that outperforms existing methods on the new benchmark.
...
TL;DR: AdmTree is a novel hierarchical context compression framework that uses gist tokens and semantic binary trees to efficiently process long contexts in LLMs while preserving semantic fidelity.


<details>
  <summary>Details</summary>
Motivation: Self-attention's quadratic complexity limits LLMs' ability to process long contexts, which is essential for advanced applications. Existing compression methods have trade-offs: explicit methods lose local details, implicit methods suffer from positional biases, information degradation, or poor long-range dependency capture.

Method: AdmTree dynamically segments input based on information density, using gist tokens to summarize variable-length segments as leaves of a semantic binary tree. It employs a lightweight aggregation mechanism with a frozen backbone LLM to minimize new trainable parameters, enabling efficient hierarchical context abstraction.

Result: The framework preserves fine-grained details alongside global semantic coherence, mitigates positional bias, dynamically adapts to content, and robustly retains semantic information of long contexts.

Conclusion: AdmTree provides an adaptive, hierarchical approach to context compression that addresses limitations of existing methods by balancing efficiency with high semantic fidelity for long-context processing in LLMs.
...
TL;DR: ADAPT is a meta-learning algorithm that learns optimal task sampling proportions for multi-task instruction tuning under token budget constraints, creating adaptive curricula that allocate tokens to more useful tasks.


<details>
  <summary>Details</summary>
Motivation: Current multi-task instruction tuning typically uses fixed task weights or simple heuristics (like uniform or size-proportional mixing), which may not optimally allocate limited training tokens to maximize downstream performance across diverse tasks.

Method: ADAPT maintains a continuous distribution over tasks and updates it via meta-gradients of a smooth worst-case validation objective, creating an adaptive curriculum that allocates more tokens to useful tasks while avoiding task collapse.

Result: When tested on three ~1B-parameter LLMs with 20 Natural Instructions tasks under 1%, 5%, and 10% token budgets, ADAPT matches or slightly improves average downstream performance compared to best static mixtures while using fewer effective training tokens and reallocating budget toward harder, benchmark-aligned tasks.

Conclusion: ADAPT provides an effective meta-learning approach for adaptive task sampling in multi-task instruction tuning that optimizes token allocation under budget constraints, improving resource efficiency and performance on out-of-domain benchmarks.
...
TL;DR: LexGenius is a new Chinese legal benchmark for evaluating legal general intelligence in LLMs, using a Dimension-Task-Ability framework with multiple-choice questions from recent cases and exams.


<details>
  <summary>Details</summary>
Motivation: Existing legal AI benchmarks are result-oriented and fail to systematically evaluate legal intelligence in LLMs, hindering the development of legal general intelligence that can simulate legal expert capabilities.

Method: Created LexGenius benchmark with Dimension-Task-Ability framework covering 7 dimensions, 11 tasks, and 20 abilities. Used recent legal cases and exam questions to create multiple-choice questions with manual and LLM reviews to reduce data leakage risks, ensuring accuracy through multiple rounds of checks.

Result: Evaluation of 12 state-of-the-art LLMs showed significant disparities across legal intelligence abilities, with even the best LLMs lagging behind human legal professionals.

Conclusion: LexGenius can effectively assess legal intelligence abilities of LLMs and enhance the development of legal general intelligence, with current LLMs still falling short of human legal expertise.
...
TL;DR: Analysis of generic masculines in German press texts reveals lexeme-specific patterns, predominant plural/indefinite usage, and challenges previous assumptions about their function.


<details>
  <summary>Details</summary>
Motivation: The study addresses the gap in empirical corpus-based analysis of generic masculines (GM) in German, which are debated for their gender-neutrality but lack systematic investigation of actual usage patterns in authentic texts.
...
Result: Significant differences between lexical items (especially passive role vs. prestige-related nouns), GM predominantly occurs in plural and indefinite noun phrases, and is not primarily used to denote entire classes of people as previously claimed.

Conclusion: The empirical analysis provides nuanced understanding of GM usage patterns, offering a foundation for better alignment of psycholinguistic stimuli with real-world language data and contributing to the ongoing debate about gender representation in language.
...
TL;DR: OsmT is an open-source tag-aware language model that bridges natural language and OverpassQL for OpenStreetMap data, using Tag Retrieval Augmentation to improve query accuracy and including reverse translation for query interpretation.
...
Method: OsmT introduces a Tag Retrieval Augmentation (TRA) mechanism that incorporates contextually relevant tag knowledge into query generation, capturing hierarchical and relational dependencies in OSM data. The model also includes a reverse OverpassQL-to-Text task for query interpretation.
...
TL;DR: SignRoundV2 is a post-training quantization framework that enables extreme low-bit quantization (down to 2 bits) for LLMs with minimal performance degradation, achieving ~1% variance at 4-5 bits through fast sensitivity metrics and lightweight pre-tuning search.


<details>
  <summary>Details</summary>
Motivation: Extreme low-bit quantization is essential for efficient LLM deployment but typically causes severe performance degradation at 2-4 bits, creating a need for quantization methods that maintain accuracy while achieving high compression rates.
...
Conclusion: SignRoundV2 provides an effective post-training quantization solution for LLMs that enables extreme low-bit quantization without mixed-precision, making it practical for production deployment with minimal accuracy loss.
...
TL;DR: TTSV is a lightweight plug-and-play method that prepends optimized steering vectors to LLM inputs to enhance task-specific reasoning without tuning model parameters.


<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods require tuning model parameters, which is computationally expensive and risks degrading the model's pre-existing abilities. There's a need for efficient methods to unlock LLMs' reasoning potential for specific tasks without parameter modification.

Method: Introduces Test-Time Steering Vectors (TTSV) - lightweight components prepended to input while keeping LLM parameters frozen. TTSV is optimized on test data to minimize model output entropy, steering the model toward higher confidence states and activating task-relevant abilities.

Result: Achieves significant performance gains: 45.88% relative improvement on MATH500 with Qwen2.5-Math-7B and 16.22% gain with Qwen3-4B. TTSV shows robust generalization with steering vectors transferable across diverse tasks.

Conclusion: TTSV provides an efficient, plug-and-play solution for test-time adaptation that enhances LLM reasoning without parameter tuning, preserving pre-existing abilities while achieving substantial performance improvements.
...
TL;DR: Edit-then-Consolidate: A two-stage knowledge editing framework that first makes targeted edits via TPSFT to avoid overfitting, then consolidates knowledge via GRPO to align edited facts with inference behavior, bridging the gap between theoretical editing methods and real-world applicability.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge editing methods show a significant gap between controlled evaluations and real-world effectiveness in lifelong learning scenarios. Traditional methods cause overfitting to new facts and lack knowledge consolidation, leading to mismatch between parametric knowledge and actual generation behavior.
...
Result: Extensive experiments show the framework consistently improves editing reliability and generalization under real-world evaluations, while better preserving locality and pre-trained capabilities compared to traditional methods.

Conclusion: The Edit-then-Consolidate paradigm successfully bridges the gap between theoretical knowledge editing methods and practical applicability by addressing overfitting and knowledge consolidation issues, enabling more effective lifelong learning in LLMs.
...
TL;DR: CALAMITA is a large-scale collaborative benchmarking initiative for Italian that creates a comprehensive benchmark with over 20 tasks and 100 subtasks, establishing a centralized evaluation pipeline for systematic LLM assessment beyond English.


<details>
  <summary>Details</summary>
Motivation: Systematic evaluation of LLMs for languages beyond English remains limited, and existing efforts often focus on leaderboards rather than methodology. There's a need for comprehensive, community-driven evaluation frameworks for non-English languages.

Method: Federated collaboration of 80+ contributors from academia, industry, and public sector to design, document, and evaluate diverse tasks. Established centralized evaluation pipeline supporting heterogeneous datasets and metrics. Created rolling benchmark with continuous integration of new tasks and models.

Result: Built most comprehensive Italian benchmark to date with 20+ tasks and nearly 100 subtasks covering linguistic competence, reasoning, factual consistency, fairness, summarization, translation, and code generation. Reported systematic strengths/weaknesses of four open-weight LLMs and identified methodological challenges in task-specific evaluation.

Conclusion: CALAMITA demonstrates the necessity of fine-grained metrics, harmonized pipelines, and community engagement for rigorous LLM evaluation. It offers a blueprint for other languages seeking inclusive evaluation practices and serves as both a resource and sustainable framework for continuous assessment.
...
TL;DR: AdiBhashaa creates first open parallel corpora and baseline MT systems for four Indian tribal languages through community-driven participatory methods, addressing AI inequity.
...
TL;DR: Enhanced benchmark for evaluating linguistic acceptability in Danish using systematic error corruptions to better assess LLM performance.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for linguistic acceptability in Danish are limited in scope and comprehensiveness, lacking systematic error analysis and diverse corruption types needed for rigorous LLM evaluation.

Method: Analyzed common errors in written Danish, created 14 corruption functions to systematically introduce errors into correct sentences, validated corruptions using manual and automatic methods, then used as benchmark for LLM evaluation on acceptability judgement tasks.

Result: Benchmark is broader and more comprehensive than current state-of-the-art, increases task difficulty (lower LLM performance), and has higher discriminatory power to better distinguish between well-performing and low-performing models.

Conclusion: The enhanced Danish linguistic acceptability benchmark provides more rigorous assessment through systematic error corruptions, offering better evaluation of LLM capabilities and improved model discrimination.
...
TL;DR: Info-Mask is a novel framework for detecting transition points in mixed human-AI authored text, combining stylometric cues, perplexity signals, and boundary modeling to segment collaborative content, with adversarial robustness testing on a new benchmark dataset.
...
Method: Info-Mask integrates stylometric cues, perplexity-driven signals, and structured boundary modeling. The framework is evaluated against adversarial perturbations using the new MAS benchmark dataset, and includes Human-Interpretable Attribution overlays for explainability.
...
TL;DR: SSU is a parameter update strategy that selectively freezes important parameters to adapt LLMs to new languages using only unlabeled target data, preventing catastrophic forgetting of source knowledge.


<details>
  <summary>Details</summary>
Motivation: Expanding linguistic diversity of LLMs is hindered by costly labeled data requirements and catastrophic forgetting during adaptation to new languages.

Method: Source-Shielded Updates (SSU) uses parameter importance scoring with source data to identify critical parameters, then applies column-wise freezing to protect them before adapting with unlabeled target language data.

Result: SSU reduces source task degradation to 3.4% (7B) and 2.8% (13B) vs 20.3% and 22.3% for full fine-tuning, while achieving competitive target-language performance across five diverse languages.

Conclusion: SSU effectively enables multilingual adaptation of LLMs under low-resource constraints by preventing catastrophic forgetting while maintaining strong target language performance.
...
TL;DR: SEAL is a two-stage semantic parsing framework using self-evolving agentic learning for knowledge-based conversational QA, achieving SOTA performance on SPICE benchmark with improved structural accuracy and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for knowledge-based conversational QA struggle with coreference resolution, contextual dependency modeling, and complex logical reasoning. They suffer from structural inaccuracies and high computational costs when processing complex queries over large knowledge graphs.

Method: Two-stage framework: 1) LLM extracts minimal S-expression core, refined by agentic calibration module for syntactic consistency and entity/relation alignment; 2) Template-based completion guided by question-type prediction and placeholder instantiation to build executable S-expression. Includes self-evolving mechanism with local/global memory and reflection module for continuous adaptation.
...
Conclusion: SEAL demonstrates robust and scalable conversational reasoning through its decomposition approach and self-evolving mechanism, effectively addressing core challenges in KBCQA while improving efficiency and accuracy.
...
TL;DR: LLMs can learn latent linguistic structures for genre classification, but different features contribute unevenly across tasks, highlighting the need for more complex linguistic signals during training.


<details>
  <summary>Details</summary>
Motivation: While LLMs show strong performance on language tasks, it's unclear whether they capture deeper linguistic properties like syntax, phonetics, and meter from raw text. The authors want to analyze if LLMs can effectively learn these features and apply them to important natural language tasks like genre classification.

Method: Created a novel multilingual genre classification dataset from Project Gutenberg with thousands of sentences per binary task (poetry vs. novel, drama vs. poetry, drama vs. novel) across six languages. Augmented each with three explicit linguistic feature sets: syntactic tree structures, metaphor counts, and phonetic metrics to evaluate their impact on classification performance.

Result: Experiments show that LLM classifiers can learn latent linguistic structures from either raw text or explicitly provided features. However, different features contribute unevenly across tasks, suggesting varying importance of different linguistic signals for different classification problems.

Conclusion: The findings underscore the importance of incorporating more complex linguistic signals during model training, as different linguistic features have varying impacts on different classification tasks, and LLMs can benefit from explicit linguistic feature guidance.
...
TL;DR: The paper introduces a comprehensive infrastructure called Nex ecosystem to scale interactive environments for training LLM agents, addressing complexity, diversity, and fidelity dimensions, and demonstrates competitive performance with their trained model Nex-N1.


<details>
  <summary>Details</summary>
Motivation: The evolution of LLMs from passive responders to autonomous agents requires a shift from static imitation learning to incentive-driven decision making, but this is impeded by the lack of scalable infrastructure for constructing high-quality interaction signals for effective policy learning.

Method: The method addresses three orthogonal dimensions: (1) Complexity: NexAU framework for building complex agent hierarchies via simple configurations; (2) Diversity: NexA4A automatically generates diverse agent hierarchies from natural language to cover infinite domains; (3) Fidelity: NexGAP bridges simulation-reality gap by integrating dynamic real-world environments for grounded trajectories synthesis.

Result: The trained model Nex-N1 consistently outperforms SOTA open-source models and achieves competitive performance against frontier proprietary models on complex agentic tasks in benchmarks like SWE-bench and tau2.

Conclusion: The Nex ecosystem provides a scalable infrastructure for constructing high-quality interactive environments to train autonomous LLM agents, with the open-sourced system facilitating further research in agentic AI.
...
TL;DR: CER is a retrieval method that fine-tunes embeddings with contrastive learning using factual evidence rationales, automatically selecting hard negatives based on subjectivity to create an evidence-aligned embedding space for improved accuracy and reduced hallucinations in RAG systems.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for more reliable and transparent retrieval systems, particularly in safety-critical domains like clinical trials, where factual accuracy is crucial and hallucinations in RAG systems pose significant risks.

Method: CER restructures retrieval around factual evidence by: 1) Fine-tuning embeddings with contrastive learning, 2) Generating token-level attribution rationales for retrieved passages, 3) Automatically selecting hard negatives using a subjectivity-based criterion to pull factual rationales closer while pushing subjective/misleading explanations apart.

Result: Initial experimental results on clinical trial reports show that CER improves retrieval accuracy, mitigates potential for hallucinations in RAG systems, and provides transparent, evidence-based retrieval that enhances reliability in safety-critical domains.

Conclusion: CER creates an embedding space explicitly aligned with evidential reasoning, offering a promising approach for improving retrieval reliability and transparency, especially in domains where factual accuracy is critical.
...
TL;DR: Arbitrage is a step-level speculative generation framework that uses a lightweight router to dynamically choose between draft and target models for reasoning tasks, achieving 2x latency reduction with matched accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional token-level speculative decoding struggles with reasoning tasks due to unnecessary rejections of semantically equivalent tokens. Existing step-level methods still waste target compute by regenerating many rejected steps without meaningful improvement.

Method: Proposes Arbitrage framework with a lightweight router trained to predict when the target model will produce meaningfully better reasoning steps. Instead of fixed acceptance thresholds, it dynamically routes generation based on relative advantage between draft and target models.

Result: Across multiple mathematical reasoning benchmarks, Arbitrage consistently outperforms prior step-level speculative decoding baselines, reducing inference latency by up to ~2x while maintaining matched accuracy.

Conclusion: Arbitrage achieves near-optimal efficiency-accuracy trade-offs by approximating an ideal Arbitrage Oracle that always chooses higher-quality steps, making it superior to existing step-level speculative decoding methods for reasoning tasks.
...
Method: Format Reinforcement Learning (FormatRL) uses Group Relative Policy Optimization on a supervised fine-tuning model with novel structure-aware rewards: TreeSim (structural similarity) and Node-chrF (translation quality at XML node level).
...
<details>
  <summary>Details</summary>
Motivation: RLVR methods for training LLMs on reasoning tasks have limitations: sparse rewards, poor sample efficiency, and high computational costs in post-training. There's a need for more efficient approaches.

Method: Semantic Soft Bootstrapping: The base model generates multiple rollouts for math problems, filters correct and common incorrect responses, then uses these as context to produce step-by-step explanations. This creates paired teacher-student training data automatically. The student learns to match teacher logits from just the question.

Result: On Qwen2.5-3B-Instruct fine-tuned on GSM8K: 10.6% accuracy improvement on MATH500 and 10% on AIME2024 benchmarks compared to GRPO (RLVR baseline).

Conclusion: SSB effectively overcomes RLVR limitations by enabling self-distillation with semantic context, achieving significant accuracy gains with parameter-efficient fine-tuning and no human annotation.
...
TL;DR: A novel synthetic video generation method creates training data with subtle kinematic inconsistencies to improve deepfake detection generalization to unseen manipulations.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods struggle with generalization to unseen manipulations, especially in videos. Current video approaches only model frame-to-frame instabilities but miss the key vulnerability of violating natural motion dependencies between facial regions.

Method: Proposes synthetic video generation that creates training data with subtle kinematic inconsistencies. Uses an autoencoder to decompose facial landmark configurations into motion bases, then manipulates these bases to break natural correlations in facial movements and introduces artifacts into pristine videos via face morphing.
...
<details>
  <summary>Details</summary>
Motivation: Current microscopic tissue examination relies on subjective expert interpretation, compromising accuracy. While AI offers promise for automated analysis, proprietary digital pathology solutions create barriers to real-world deployment.

Method: Developed OnSight Pathology - a platform-agnostic software that uses continuous custom screen captures to provide real-time AI inferences. It operates as a single executable file that runs locally on consumer PCs without complex integration.

Result: Demonstrated utility with over 2,500 publicly available whole slide images across different viewers and clinical cases. Showed robustness across routine tasks: brain tumor classification, mitosis detection, immunohistochemical stain quantification. Included multi-modal chat assistant for verifiable image descriptions. Compatible with live microscope camera feeds including smartphones.
...
<details>
  <summary>Details</summary>
Motivation: Traditional multi-camera tracking pipelines decouple detection, association, and tracking, leading to error propagation and temporal inconsistency in challenging scenarios like complex motions and occlusions.
...
Conclusion: The unified end-to-end transformer approach effectively addresses limitations of traditional decoupled pipelines, demonstrating superior performance in multi-camera point tracking through integrated appearance-geometric reasoning and temporal consistency modeling.
...
<details>
  <summary>Details</summary>
Motivation: Humans naturally perceive continuous experience as a hierarchy of temporally nested events (fine-grained actions within coarser routines). Current computer vision models need to replicate this structure not just retrospectively but predictively and hierarchically.

Method: PARSE organizes perception into a hierarchy of recurrent predictors operating at different temporal granularities. Lower layers model short-term dynamics while higher layers integrate longer-term context through attention-based feedback. Event boundaries emerge naturally as transient peaks in prediction error.

Result: Achieves state-of-the-art performance among streaming methods on Breakfast Actions, 50 Salads, and Assembly 101 benchmarks. Rivals offline baselines in both temporal alignment (H-GEBD) and structural consistency (TED, hF1).

Conclusion: Predictive learning under uncertainty provides a scalable path toward human-like temporal abstraction and compositional event understanding, demonstrating that unsupervised hierarchical event segmentation can approach human perception capabilities.
...
TL;DR: MoReGen is a motion-aware physics-grounded T2V framework that uses multi-agent LLMs, physics simulators, and renderers to generate physically accurate videos from text prompts, with MoReSet benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video models generate photorealistic videos but struggle with physical validity - they don't faithfully obey physics principles or maintain motion coherence according to Newtonian physics.
...
Result: State-of-the-art T2V models struggle with physical validity, while MoReGen establishes a principled approach for physically coherent video synthesis. Introduces object-trajectory correspondence as a direct evaluation metric.

Conclusion: MoReGen provides a systematic framework for Newtonian motion-controlled T2V generation and evaluation, addressing the core challenge of physical precision in video synthesis.
...
TL;DR: ReasonX uses MLLM as perceptual judge to provide relative comparisons as GRPO rewards for fine-tuning intrinsic decomposition models on unlabeled real-world images, improving generalization across architectures and modalities.


<details>
  <summary>Details</summary>
Motivation: Current intrinsic image decomposition models rely on synthetic paired supervision and struggle to generalize to diverse real-world scenarios. There's a need for better methods that can leverage unlabeled, in-the-wild images for improved real-world performance.

Method: Proposes ReasonX framework that uses a multimodal large language model (MLLM) as a perceptual judge to provide relative intrinsic comparisons. These comparisons serve as GRPO rewards for fine-tuning intrinsic decomposition models on unlabeled images. The framework aligns conditional intrinsic predictors by rewarding agreement between the judge's relational assessments and analytically derived relations from model outputs.
...
Conclusion: ReasonX demonstrates the promise of MLLM-guided comparative supervision to bridge low- and high-level vision reasoning, offering a model-agnostic approach that improves generalization of intrinsic decomposition models to real-world scenarios.
...
TL;DR: Vision-language models struggle with rare anatomical variants, dropping from 74% to 29% accuracy on AdversarialAnatomyBench, with even top models showing 41-51% performance drops that scaling and interventions can't fix.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on common anatomical presentations, missing the challenge of rare variants that violate learned priors about "typical" human anatomy. There's a need to measure how VLMs handle natural adversarial anatomy across diverse imaging modalities and regions.

Method: Introduced AdversarialAnatomyBench - first benchmark of naturally occurring rare anatomical variants across diverse imaging modalities and anatomical regions. Benchmarked 22 state-of-the-art VLMs on basic medical perception tasks comparing typical vs atypical anatomy performance.

Result: 1) Mean accuracy dropped from 74% on typical to 29% on atypical anatomy. Even best models (GPT-5, Gemini 2.5 Pro, Llama 4 Maverick) showed 41-51% performance drops. 2) Model errors mirrored expected anatomical biases. 3) Neither model scaling nor interventions (bias-aware prompting, test-time reasoning) resolved these issues.

Conclusion: Current VLMs have critical limitation in poor generalization to rare anatomical presentations. AdversarialAnatomyBench provides foundation for systematically measuring and mitigating anatomical bias in multimodal medical AI systems.
...
TL;DR: MVRoom is a controllable novel view synthesis pipeline for 3D indoor scenes using multi-view diffusion conditioned on coarse 3D layouts, featuring a two-stage design with layout-aware consistency mechanisms and iterative scene generation capabilities.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of generating high-fidelity and controllable 3D indoor scenes for novel view synthesis, particularly focusing on maintaining multi-view consistency while allowing scene complexity variation through text-to-scene generation.

Method: Two-stage pipeline: 1) Novel representations bridge 3D layout with consistent image-based condition signals, 2) Image-conditioned multi-view generation with layout-aware epipolar attention mechanism for enhanced consistency. Also includes iterative framework for recursive scene generation with varying object counts and complexities.

Result: Experimental results show MVRoom achieves high-fidelity and controllable 3D scene generation for novel view synthesis, outperforming state-of-the-art baseline methods both quantitatively and qualitatively. Ablation studies validate effectiveness of key components.

Conclusion: MVRoom presents an effective solution for controllable 3D indoor scene generation with strong multi-view consistency, demonstrating superior performance over existing methods and enabling flexible text-to-scene generation through its iterative framework.
...
TL;DR: UniLight is a joint latent space that unifies multiple lighting representations (text, images, irradiance, environment maps) for cross-modal transfer and manipulation.


<details>
  <summary>Details</summary>
Motivation: Lighting strongly influences visual appearance but existing lighting representations (environment maps, irradiance, spherical harmonics, text) are incompatible, limiting cross-modal transfer between different lighting modalities.

Method: Proposes UniLight - a joint latent space with modality-specific encoders for text, images, irradiance, and environment maps trained contrastively to align representations, with auxiliary spherical-harmonics prediction for directional understanding. Uses multi-modal data pipeline for large-scale training.
...
Conclusion: UniLight successfully creates a unified lighting representation that bridges multiple modalities, enabling cross-modal transfer and flexible lighting manipulation in various applications.
...
TL;DR: The paper proposes GRU-SNF, an inference-time refinement technique that combines GRU-Normalizing Flows with stochastic sampling to improve diversity in video motion transfer predictions without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Real-time video motion transfer applications need accurate yet diverse future predictions for realistic synthesis and robust decision-making under uncertainty. Current methods like GRU-NF have limited expressivity due to deterministic transformation structures.

Method: Introduces MCMC steps during GRU-NF inference, creating GRU-Stochastic Normalizing Flows (GRU-SNF). This allows exploration of richer output space without retraining, inspired by Stochastic Normalizing Flows. Validated in keypoint-based video motion transfer pipeline.

Result: GRU-SNF outperforms GRU-NF in generating diverse outputs without sacrificing accuracy, even under longer prediction horizons. Effectively captures multimodal behavior by injecting stochasticity during inference.

Conclusion: Integrating stochastic dynamics with flow-based sequence models shows strong potential for generative time series forecasting, enabling better approximation of true data distribution through inference-time refinement.
...
TL;DR: Theoretical analysis of plug-and-play flow matching (PnP-Flow) for image restoration via SDE modeling, leading to improved step scheduling, regularization, and acceleration methods.


<details>
  <summary>Details</summary>
Motivation: While PnP-Flow has shown empirical success in image restoration, there's a lack of theoretical understanding behind its performance.

Method: Derived a continuous limit of PnP-Flow as a stochastic differential equation (SDE) surrogate model. Used this SDE model to: (1) quantify restoration error and improve step scheduling while regularizing the Lipschitz constant of neural network-parameterized vector fields, (2) accelerate existing PnP-Flow models via extrapolation to create a rescaled SDE version.

Result: The SDE-informed improved PnP-Flow significantly outperforms baseline PnP-Flow and other state-of-the-art methods across image denoising, deblurring, super-resolution, and inpainting tasks.

Conclusion: Theoretical SDE modeling provides valuable insights for improving PnP-Flow performance, bridging the gap between empirical success and theoretical understanding in flow matching-based image restoration.
...
Method: Propose lightweight super-resolution pipeline operating directly on JPEG DCT coefficients in frequency domain, bypassing full JPEG decoding.
...
Conclusion: Training on encoded JPEG features effectively addresses data loading bottlenecks for restoration tasks like super-resolution, offering significant speed improvements without quality degradation.
...
TL;DR: GfM is a lightweight monocular geometry estimation method that decouples global and local structure to accurately recover fine-scale road geometry like bumps and slopes, using a physically interpretable gamma representation that requires only camera height.
...
Method: GfM decouples global and local structure by predicting a dominant road surface plane plus residual variations expressed as gamma - a dimensionless measure of vertical deviation from the plane (height above plane divided by depth from camera). This representation uses planar parallax geometry and only requires camera height above ground to deterministically recover metric depth via closed-form solution.

Result: Achieves state-of-the-art near-field accuracy in both depth and gamma estimation on KITTI and Road Surface Reconstruction Dataset (RSRD), while maintaining competitive global depth performance. The 8.88M-parameter model adapts robustly across diverse camera setups and is the first self-supervised monocular approach evaluated on RSRD.

Conclusion: GfM provides an effective, lightweight solution for accurate 3D road geometry perception that preserves fine-scale details essential for vehicle control, using a physically interpretable representation that enables self-supervised learning without large annotated datasets.
...
<details>
  <summary>Details</summary>
Motivation: While CLIP calibration has been studied in offline settings, its behavior in federated learning (FL) setups remains unexplored. Existing calibration techniques provide limited improvements in FL, and prompt tuning approaches degrade calibration metrics in distributed settings.

Method: Proposes FL²oRA, a straightforward LoRA-based approach that naturally improves calibration in FL. Analyzes various fine-tuning components and finds that the key challenge lies in which components to fine-tune rather than how to aggregate or calibrate.

Result: FL²oRA consistently produces well-calibrated models across multiple benchmarks, reducing the need for explicit calibration procedures. The approach outperforms existing in-training calibration techniques across four global aggregation methods.

Conclusion: The choice of which components to fine-tune is crucial for CLIP calibration in FL. FL²oRA effectively addresses calibration challenges in distributed settings without requiring additional calibration procedures.
...
<details>
  <summary>Details</summary>
Motivation: To reduce reliance on curated data for image captioning by developing methods that don't require humanly-annotated image-text pairs, addressing the performance gap between unsupervised and fully supervised approaches.

Method: Uses CLIP representations with modality gap reduction, then prompts a pre-trained language model decoder with retrieved caption examples and latent vector representations for guidance.
...
Conclusion: The proposed text-only training method effectively performs image captioning without aligned image-caption pairs, showing promising results compared to existing unsupervised approaches.
...
TL;DR: A low-cost automated system for sex-based sorting of house crickets using computer vision (YOLOv8 nano on Raspberry Pi) achieves 86.8% sorting accuracy, enabling more efficient and sustainable cricket farming.


<details>
  <summary>Details</summary>
Motivation: Current cricket farming lacks automated sex sorting despite potential benefits for selective breeding, optimized reproduction ratios, and nutritional differentiation. There's a need for practical, low-cost solutions to improve efficiency in edible insect production.

Method: Developed a real-time system combining computer vision and physical actuation using Raspberry Pi 5 with Raspberry AI Camera, custom YOLOv8 nano object detection model, and servo-actuated sorting arm for automated cricket sex identification and sorting.

Result: The YOLOv8 nano model achieved mAP@0.5 of 0.977 in testing, and real-world experiments with cricket groups demonstrated 86.8% overall sorting accuracy, proving feasibility of lightweight deep learning on resource-constrained devices.

Conclusion: The system successfully demonstrates that lightweight deep learning models can be deployed on low-cost hardware for automated insect sex sorting, offering a practical solution to enhance efficiency and sustainability in cricket farming operations.
...
Method: Uses dual-modality recording setup with synchronized EEG and multi-view facial video during emotion elicitation. A CNN-Transformer encoder maps EEG signals into dense 3D position maps (65k+ vertices), rendered through modified 3D Gaussian Splatting for photorealistic results.

Result: EEG alone can reliably predict dynamic, subject-specific facial expressions including subtle emotional responses. Neural signals contain richer affective and geometric information than previously assumed.
...
TL;DR: DisentangleFormer: A vision transformer architecture that decouples spatial and channel processing for better representation learning in hyperspectral imaging, achieving SOTA performance with reduced computational cost.
...
Method: Proposes DisentangleFormer with three core components: (1) Parallel Disentanglement for independent spatial-token and channel-token streams, (2) Squeezed Token Enhancer for adaptive fusion of spatial and channel streams, and (3) Multi-Scale FFN to capture local context alongside global attention.

Result: Achieves state-of-the-art performance on hyperspectral benchmarks (Indian Pine, Pavia University, Houston, BigEarthNet, infrared pathology dataset). Also maintains competitive accuracy on ImageNet while reducing computational cost by 17.8% in FLOPs.

Conclusion: DisentangleFormer provides a principled approach to spatial-channel decoupling in vision transformers, enabling robust multi-channel vision representation with improved performance and efficiency, particularly beneficial for hyperspectral imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: Modeling dynamic 3D scenes is challenging due to high-dimensional nature requiring aggregation from multiple views. Existing approaches struggle with unsynchronized video sets and lack general solutions without predefined objects or prior models.
...
Result: Achieves sub-frame synchronization accuracy with average temporal error below 0.26 frames. High-fidelity 4D reconstruction reaching 26.3 PSNR scores on Panoptic Studio dataset. First general 4D Gaussian Splatting approach for unsynchronized video sets without requiring predefined objects or prior models.

Conclusion: SyncTrack4D successfully addresses the challenge of modeling dynamic 3D scenes from unsynchronized videos by leveraging dense 4D track representations for simultaneous synchronization and reconstruction, achieving state-of-the-art performance.
...
TL;DR: Novel method for generating DIC datasets using non-uniform B-spline surfaces and Bayes-DIC Net architecture with Bayesian neural network capabilities for uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Need for high-quality, large-scale DIC datasets that capture real-world displacement scenarios to improve training and generalization of deep learning-based DIC algorithms, along with the need for uncertainty estimation in predictions.

Method: 1) Generate DIC datasets by randomly creating control point coordinates to construct realistic displacement fields using non-uniform B-spline surfaces, then generate speckle patterns. 2) Develop Bayes-DIC Net with multi-level information extraction during down-sampling and single skip connection aggregation during up-sampling, using lightweight convolutional blocks. 3) Transform network into Bayesian neural network by integrating dropout modules activated during inference for uncertainty estimation.

Result: The method enables generation of large-scale DIC datasets capturing realistic displacement scenarios. Bayes-DIC Net provides both predictive results and confidence levels for real unlabeled datasets, enhancing practicality and reliability in displacement field prediction tasks.

Conclusion: The paper offers new perspectives for DIC dataset generation and algorithm performance enhancement through B-spline-based dataset creation and Bayesian neural network architecture with uncertainty quantification capabilities.
...
TL;DR: NN-RAG is a retrieval-augmented generation system that extracts, validates, and makes reusable neural network modules from PyTorch codebases, enabling cross-repository migration of architectural patterns.


<details>
  <summary>Details</summary>
Motivation: Reusing neural network components is crucial for research efficiency, but discovering, extracting, and validating modules across thousands of open-source repositories remains difficult due to fragmented code and dependency issues.

Method: NN-RAG performs scope-aware dependency resolution, import-preserving reconstruction, and validator-gated promotion to ensure retrieved blocks are scope-closed, compilable, and runnable. It uses multi-level de-duplication (exact, lexical, structural) and integrates with language models for synthesis.

Result: Applied to 19 major repositories, extracted 1,289 candidate blocks and validated 941 (73.0%). Over 80% are structurally unique, and NN-RAG contributes ~72% of novel network structures to the LEMUR dataset. Enables cross-repository migration of architectural patterns.
...
TL;DR: Proposes DLED method for Open Set Face Forgery Detection that can identify novel fake categories using uncertainty estimation from spatial and frequency evidence fusion.


<details>
  <summary>Details</summary>
Motivation: Existing face forgery detection methods are limited to binary classification or known fake categories, unable to detect novel types of forgeries that keep emerging due to rapid development of generation algorithms.

Method: Dual-Level Evidential face forgery Detection (DLED) approach that collects and fuses category-specific evidence on both spatial and frequency levels to estimate prediction uncertainty for novel fake detection.

Result: DLED achieves state-of-the-art performance, outperforming baseline models by average 20% in detecting forgeries from novel fake categories, while maintaining competitive performance on traditional Real-vs-Fake detection.

Conclusion: The proposed DLED method effectively addresses the Open Set Face Forgery Detection problem by leveraging uncertainty estimation through dual-level evidence fusion, providing a practical solution for real-world scenarios with emerging novel fake categories.
...
TL;DR: SANTA framework reduces factual hallucinations in video captioning by using self-augmented contrastive alignment to enforce faithfulness to visual objects and temporal actions.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs for video captioning suffer from factual inaccuracies and hallucinations, especially for dynamic videos where both visual objects and temporal actions need to be accurately described. Existing methods focus on static images but fail to address the joint challenge of object and action hallucinations in videos.

Method: Proposes Self-Augmented Contrastive Alignment (SANTA) framework with two key components: 1) Hallucinative self-augmentation to identify potential hallucinations in MLLMs and transform original captions into contrasted negatives, and 2) Tracklet-phrase contrastive alignment to match regional objects and relation-guided actions with corresponding visual and temporal phrases.
...
Conclusion: SANTA effectively addresses the challenging problem of joint object and action hallucinations in video captioning through self-augmented contrastive alignment, enabling more faithful and accurate video descriptions.
...
TL;DR: MAFNet: A real-time stereo matching network using efficient 2D convolutions with frequency-domain filtering attention and Linformer-based fusion for mobile deployment.


<details>
  <summary>Details</summary>
Motivation: Existing stereo matching networks have limitations: 3D convolution-based methods have high computational overhead, while iterative optimization methods lack non-local context modeling. Both are poorly compatible with resource-constrained mobile devices, limiting real-time application deployment.

Method: Proposes Multi-frequency Adaptive Fusion Network (MAFNet) using only efficient 2D convolutions. Features: 1) Adaptive frequency-domain filtering attention module that decomposes cost volume into high-frequency and low-frequency volumes for separate feature aggregation; 2) Linformer-based low-rank attention mechanism to adaptively fuse high- and low-frequency information for robust disparity estimation.
...
Conclusion: MAFNet provides an effective solution for real-time stereo matching on mobile devices by combining frequency-domain decomposition with efficient attention mechanisms, enabling high-quality disparity estimation with low computational cost.
...
Result: Achieves state-of-the-art accuracy and temporal consistency on new benchmarks (REDS-ME, REDS-RE) and GoPro. Outperforms recent methods in both restoration quality and inference speed, and generalizes well to challenging real-world videos despite being trained solely on synthetic data.

Conclusion: FMA-Net++ effectively addresses the coupled motion-exposure degradation problem in real-world video restoration through explicit modeling of exposure variations, achieving superior performance and efficiency while maintaining good generalization to real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs learn entangled holistic representations where domain-invariant structure and domain-specific style are mixed, limiting generalization. There's an opportunity to enhance generalization by explicitly disentangling these visual cues.
...
Conclusion: Explicitly disentangling visual representations through Fourier analysis enhances VLM generalization by separating structural and stylistic features, leading to more robust vision-language alignment.
...
TL;DR: Comprehensive analysis of transfer learning with six CNN models on chest X-ray dataset shows InceptionV3 performs best, ResNet improves with depth, and TL is beneficial especially with limited data.


<details>
  <summary>Details</summary>
Motivation: Deep learning for medical image classification faces challenges in training large models from scratch, making transfer learning a practical solution to leverage pre-trained models for disease detection tasks.

Method: Evaluated six pre-trained CNN models (AlexNet, VGG16, ResNet18, ResNet34, ResNet50, InceptionV3) on custom chest X-ray dataset using transfer learning, with uncertainty analysis and runtime comparison.

Result: InceptionV3 consistently outperformed all other models across standard metrics. ResNet family showed progressive improvement with increasing depth. VGG16 and AlexNet performed reasonably but with lower accuracy.

Conclusion: Transfer learning is beneficial for medical image classification, especially with limited data, but effectiveness depends on model architecture, dataset size, and domain similarity. A well-trained feature extractor with lightweight feedforward model can provide efficient predictions.
...
<details>
  <summary>Details</summary>
Motivation: Existing distillation methods for remote sensing object detection suffer from mixed features in remote sensing images and neglect subtle feature variations, leading to entangled knowledge confusion and poor performance on dense and small object detection tasks.

Method: Proposes DS2D2 with explicit and implicit distillation streams: 1) First-order wavelet transform for spectral decomposition to preserve spatial characteristics, with Density-Independent Scale Weight (DISW) for dense/small objects; 2) Implicit knowledge extraction from student-teacher feature discrepancies using full-frequency and high-frequency amplifiers.
...
Conclusion: DS2D2 effectively addresses mixed features and subtle discrepancies in remote sensing object detection through spectral decomposition-based distillation, providing a universal solution for lightweight detection models.
...
TL;DR: Proposes differentiable triangle-based ray tracing that directly uses triangles as rendering primitives, eliminating need for proxy geometry and achieving higher quality with real-time performance.
...
Method: A differentiable triangle-based ray tracing pipeline that directly treats triangles as rendering primitives without relying on any proxy geometry, enabling direct rendering of triangles optimized by rasterization-based methods.

Result: Achieves significantly higher rendering quality than existing ray tracing approaches while maintaining real-time rendering performance, and can directly render triangles from Triangle Splatting optimization.

Conclusion: The proposed method unifies rendering primitives for novel-view synthesis by enabling direct triangle-based ray tracing without proxy geometry, improving both quality and efficiency.
...
TL;DR: An explainable multimodal framework using RGB-D data and LLMs for Parkinson's disease gait analysis with improved accuracy, robustness, and clinical interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing gait analysis approaches for Parkinson's disease detection are limited by single-modality inputs, low robustness, lack of clinical transparency, and inability to handle realistic conditions like low lighting or occlusion.

Method: Uses dual YOLOv11-based encoders for RGB-D feature extraction, Multi-Scale Local-Global Extraction (MLGE) module, Cross-Spatial Neck Fusion mechanism, and frozen LLM for translating visual embeddings into clinical explanations.

Result: Achieves higher recognition accuracy, improved robustness to environmental variations, and clear visual-linguistic reasoning compared to single-input baselines on multimodal gait datasets.
...
TL;DR: SSMP is a novel self-paced, self-corrective masked prediction method for automatic movie trailer generation that outperforms traditional "selection-then-ranking" approaches through bi-directional contextual modeling and progressive self-correction.


<details>
  <summary>Details</summary>
Motivation: Existing automatic trailer generation methods use a "selection-then-ranking" paradigm that suffers from error propagation and limits trailer quality. The authors aim to move beyond this paradigm to create more engaging trailers.

Method: SSMP uses a Transformer encoder that takes movie shot sequences as prompts and generates trailer sequences via masked prediction. It features self-paced mask ratios for adaptive difficulty and progressive self-correction where high-confidence shots are filled first and remaining positions are re-masked for next predictions.
...
Conclusion: The proposed SSMP method successfully overcomes limitations of traditional approaches through its innovative self-paced, self-corrective masked prediction framework, producing higher quality movie trailers that better mimic human editing processes.
...
Conclusion: MindDrive provides a promising path toward interpretable and cognitively guided autonomous driving by harmonizing high-quality trajectory generation with comprehensive decision reasoning through a structured paradigm that integrates simulation-based generation with VLM-powered evaluation.
...
<details>
  <summary>Details</summary>
Motivation: As embodied intelligence advances toward real-world deployment, agents need continuous perception and reasoning over streaming visual inputs to maintain situational awareness, comprehend interactions, and dynamically plan actions based on past, present, and anticipated future events.

Method: Created StreamEQA benchmark with 156 independent long videos, 42 tasks, and ~21K question-answer pairs with precise timestamps using hybrid pipeline of automated generation and human refinement. Questions categorized along embodied dimension (perception, interaction, planning) and streaming dimension (backward, real-time, forward reasoning).
...
Conclusion: StreamEQA aims to catalyze research on streaming video understanding for embodied applications by providing the first comprehensive benchmark for evaluating models' ability to continuously perceive and reason over streaming visual inputs in real-world embodied scenarios.
...
TL;DR: GuidNoise: A diffusion-based method that synthesizes realistic noise using just a single noisy/clean image pair as guidance, enabling data augmentation for denoising without camera metadata or extensive paired data.
...
Method: Proposes Single-Pair Guided Diffusion (GuidNoise) that uses a single noisy/clean pair as guidance. Introduces guidance-aware affine feature modification (GAFM) and noise-aware refine loss to leverage diffusion models' potential for realistic noise synthesis.

Result: GuidNoise synthesizes high-quality noisy images under diverse noise environments without additional metadata. Enables efficient generation of noisy-clean pairs for data augmentation, significantly improving denoising performance, especially with lightweight models and limited training data.

Conclusion: GuidNoise provides an effective solution for realistic noise synthesis with minimal requirements, enabling practical data augmentation for image denoising tasks and improving performance in resource-constrained scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing AR-based vision-language models for autonomous driving suffer from inconsistency between high-level reasoning and low-level planning due to causal attention and sequential token generation limitations. The paper aims to address this by leveraging diffusion models with bidirectional attention for better controllability and reliability.

Method: Introduces dVLM-AD, a diffusion-based vision-language model that unifies perception, structured reasoning, and low-level planning for end-to-end driving. Uses iterative denoising with bidirectional attention instead of autoregressive generation.
...
Conclusion: Diffusion-based VLMs offer a more controllable and reliable pathway for scalable end-to-end autonomous driving by better maintaining consistency between reasoning and planning actions.
...
<details>
  <summary>Details</summary>
Motivation: Existing satellite remote sensing methods require specialized models for different tasks, lacking unified spatiotemporal modeling. There's a need for a general framework that can handle multiple time series tasks with consistent representation learning.

Method: Proposes UniTS based on flow matching generative paradigm, using diffusion transformer with spatio-temporal blocks. Key innovations: Adaptive Condition Injector (ACor) for multimodal input conditioning, and Spatiotemporal-aware Modulator (STM) for capturing complex dependencies. Also creates TS-S12 and TS-S12CR benchmark datasets.

Result: UniTS demonstrates exceptional generative and cognitive capabilities across low-level and high-level tasks, significantly outperforming existing methods, especially for severe cloud contamination, modality absence, and phenological variation forecasting.

Conclusion: UniTS provides a unified framework for multiple satellite time series tasks, achieving state-of-the-art performance through innovative conditioning mechanisms and spatiotemporal modeling, with new benchmark datasets enabling better evaluation.
...
TL;DR: DeRA is a novel 1D video tokenizer that decouples spatial-temporal learning into separate appearance and motion streams aligned with pretrained models, using SACP to handle gradient conflicts, achieving SOTA performance in video generation tasks.


<details>
  <summary>Details</summary>
Motivation: Current video tokenization methods struggle with efficient spatial-temporal representation learning. The authors aim to create a more efficient video tokenizer by decoupling spatial and temporal learning while addressing gradient conflicts from heterogeneous supervision.

Method: DeRA maintains a compact 1D latent space while factorizing video encoding into appearance and motion streams aligned with pretrained vision foundation models. It introduces Symmetric Alignment-Conflict Projection (SACP) to proactively reformulate gradients by suppressing conflicting components.

Result: DeRA outperforms previous SOTA video tokenizer LARP by 25% on UCF-101 in rFVD. It also achieves new SOTA results on both UCF-101 class-conditional generation and K600 frame prediction tasks when used for autoregressive video generation.

Conclusion: DeRA successfully demonstrates that decoupling spatial-temporal representation learning in video tokenization leads to better training efficiency and performance, with the SACP module effectively addressing gradient conflicts from heterogeneous supervision.
...
TL;DR: The paper introduces NABLA, a dataset for evaluating identity-preserving image generation of birds, showing current models fail on this task and proposing training with species/age/sex grouping to improve performance.


<details>
  <summary>Details</summary>
Motivation: Current identity-preserving image generation models work well for humans and rigid objects but struggle with non-rigid, fine-grained categories like birds. These domains lack high-quality data (especially videos/multi-view observations), making evaluation and improvement difficult, yet are essential for applications requiring accuracy and fine detail.

Method: Created the NABirds Look-Alikes (NABLA) dataset with 4,759 expert-curated image pairs, plus 1,073 pairs from iNaturalist multi-image observations and some videos. Proposed training images grouped by species, age, and sex as a proxy for identity to improve identity preservation.
...
Conclusion: Birds present a challenging domain for identity-preserving generation due to diversity and fine-grained cues. The NABLA benchmark enables evaluation in this domain, and grouping training data by biological attributes improves identity preservation for both known and novel species.
...
TL;DR: COMET is a real-time autoregressive framework for generating stable, controllable character motion with fine-grained joint control and long-horizon synthesis capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing character animation methods lack fine-grained control and suffer from motion degradation over long sequences, limiting their use in interactive applications that require stable, controllable motion generation.

Method: An autoregressive framework with efficient Transformer-based conditional VAE for precise joint control, plus a novel reference-guided feedback mechanism for temporal stability and error prevention.

Result: COMET generates high-quality motion at real-time speeds, outperforming state-of-the-art methods in complex motion control tasks and demonstrating readiness for interactive applications.

Conclusion: COMET enables versatile real-time character control with long-term stability, offering a practical solution for interactive animation applications through its autoregressive framework and feedback mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: VideoLLMs struggle with temporal reasoning, often generating temporally inconsistent or causally implausible event descriptions, causing severe hallucination issues. While spatial hallucinations have been studied, temporal reasoning in video understanding remains underexplored.
...
TL;DR: MM-SHR is a multi-model architecture for specular highlight removal that combines CNN for local details and attention mechanisms for global dependencies, achieving state-of-the-art performance across various surface materials.
...
Conclusion: The proposed multi-model architecture effectively addresses the scale variation problem in specular highlight removal by combining local detail extraction with global dependency modeling, achieving superior performance while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to better understand fundamental questions about motion representations and loss functions in diffusion models for human motion synthesis, and to enumerate the impacts of various decisions in the generative motion diffusion model workflow.

Method: The authors conduct empirical studies using a proxy motion diffusion model (MDM), applying v loss as the prediction objective (vMDM) where v is the weighted sum of motion data and noise. They evaluate six common motion representations, compare training time under various configurations, and conduct evaluation analysis on a large motion dataset.

Result: Results show clear performance differences across motion representations in diverse datasets, demonstrate the impacts of distinct configurations on model training, and suggest the importance and effectiveness of these decisions on motion diffusion model outcomes.

Conclusion: The study enhances understanding of latent data distributions and provides a foundation for improving conditional motion diffusion models by systematically analyzing motion representations and training configurations.
...
TL;DR: UltraImage addresses image diffusion transformers' limitations in generating images beyond training scales by fixing content repetition via dominant frequency correction and improving quality via entropy-guided attention concentration.


<details>
  <summary>Details</summary>
Motivation: Recent image diffusion transformers achieve high-fidelity generation at training resolutions but struggle with extrapolation to larger scales, suffering from content repetition and quality degradation when generating images beyond their training resolution.

Method: 1) Frequency-wise analysis of positional embeddings identifies that repetition arises from periodicity of dominant frequency aligned with training resolution. 2) Introduces recursive dominant frequency correction to constrain it within single period after extrapolation. 3) Proposes entropy-guided adaptive attention concentration that assigns higher focus factors to sharpen local attention for fine details and lower ones to global attention to preserve structural consistency.

Result: UltraImage consistently outperforms prior methods on Qwen-Image and Flux (around 4K) across three generation scenarios, reducing repetition and improving visual fidelity. Can generate images up to 6K*6K without low-resolution guidance from a training resolution of 1328p, demonstrating extreme extrapolation capability.

Conclusion: UltraImage provides a principled framework that successfully addresses both content repetition and quality degradation issues in image diffusion transformers when extrapolating beyond training scales, enabling high-quality generation at much larger resolutions.
...
TL;DR: DuGI-MAE: A dual-domain guided infrared foundation model that improves upon InfMAE by using entropy-based masking, global relationship modeling, and adaptive noise filtering, pretrained on a new large-scale infrared dataset (Inf-590K).


<details>
  <summary>Details</summary>
Motivation: Existing foundation models like MAE perform poorly on infrared images due to domain differences. While InfMAE was developed for infrared, it still has limitations: omits informative tokens, insufficient global modeling, and ignores non-uniform noise common in infrared imagery.

Method: 1) Deterministic masking strategy based on token entropy to preserve high-entropy tokens for reconstruction. 2) Dual-Domain Guidance (DDG) module that captures global token relationships and adaptively filters non-uniform background noise. 3) Construction of Inf-590K dataset with diverse infrared scenes, targets, and resolutions for large-scale pretraining.

Result: DuGI-MAE demonstrates strong generalization across multiple downstream tasks including infrared object detection, semantic segmentation, and small target detection. It outperforms both supervised and self-supervised comparison methods.

Conclusion: The proposed DuGI-MAE effectively addresses limitations of previous infrared foundation models through entropy-based masking, dual-domain guidance, and large-scale pretraining on comprehensive infrared data, achieving superior performance across various infrared vision tasks.
...
TL;DR: EgoLCD is a new framework for generating long, coherent egocentric videos that addresses content drift in autoregressive models through efficient memory management.
...
Method: EgoLCD treats long video synthesis as a memory management problem, combining: 1) Long-Term Sparse KV Cache for stable global context, 2) attention-based short-term memory extended by LoRA for local adaptation, 3) Memory Regulation Loss for consistent memory usage, and 4) Structured Narrative Prompting for explicit temporal guidance.
...
Conclusion: EgoLCD represents a significant step toward building scalable world models for embodied AI by addressing the challenge of long-context video generation through efficient memory management.
...
<details>
  <summary>Details</summary>
Motivation: Existing autoregressive diffusion models struggle with long-video generation due to accumulated errors, motion drift, and content repetition over minute-scale horizons, requiring better memory mechanisms for maintaining coherence.

Method: Proposes VideoSSM, a hybrid architecture that unifies AR diffusion with a state-space model as global memory for scene dynamics, plus a context window for local motion cues and fine details.

Result: Achieves state-of-the-art temporal consistency and motion stability on short- and long-range benchmarks, especially at minute-scale horizons, with content diversity and interactive prompt-based control.
...
Method: Proposes BA-TTA-SAM, a task-agnostic test-time adaptation framework with two key mechanisms: (1) encoder-level Gaussian prompt injection that embeds Gaussian-based prompts into the image encoder for initial representation guidance, and (2) cross-layer boundary-aware attention alignment that exploits hierarchical feature interactions in ViT backbone to align deep semantic responses with shallow boundary cues.

Result: Experiments on four medical datasets (ISIC, Kvasir, BUSI, REFUGE) show average 12.4% improvement in DICE score compared to SAM's zero-shot segmentation, consistently outperforming state-of-the-art models without requiring source-domain training data.

Conclusion: The framework significantly enhances SAM's generalization ability for medical image segmentation through test-time adaptation, demonstrating superior performance on publicly available medical datasets while eliminating the need for task-specific training or source-domain data.
...
TL;DR: WiFi-based gesture recognition network using Doppler spectra fusion and attention mechanisms achieves high cross-domain performance (97.61%) while maintaining 99.72% in-domain accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing WiFi gesture sensing solutions perform well in-domain but lack cross-domain capabilities (poor performance in untrained environments). WiFi signals offer advantages like widespread availability, low cost, and robustness to environmental conditions.

Method: Extract Doppler spectra from CSI received by all receivers, concatenate along time axis to generate fused images with multi-angle information. Propose gesture recognition network integrating multi-semantic spatial attention with self-attention-based channel mechanism (inspired by CBAM). Uses ResNet18 as backbone to capture deep-level features.

Result: Evaluated on public Widar3 dataset: achieves 99.72% in-domain accuracy and 97.61% cross-domain recognition, significantly outperforming existing best solutions.

Conclusion: The proposed network effectively extracts domain-independent features through attention mechanisms and multi-angle Doppler spectra fusion, enabling robust cross-domain gesture recognition while maintaining high in-domain performance.
...
Method: Proposes ICRE network with: 1) MPFR module to aggregate shallow features capturing modality-specific attributes; 2) SDCE module to distill identity-aware knowledge from shallow features to guide modality-invariant learning; 3) ICG Loss to reduce modality discrepancies and promote diverse representation space.
...
TL;DR: Auto3R is a data-driven uncertainty quantification model that automates 3D scanning and reconstruction by predicting optimal scanning viewpoints without ground truth knowledge, achieving state-of-the-art performance and enabling practical robotic deployment.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D scanning requires manual planning, but with the rise of embodied systems (drones, robots), there's a need for fully automated, high-quality 3D reconstruction that can handle challenging materials like non-lambertian and specular surfaces.

Method: Auto3R uses a data-driven uncertainty quantification model that operates in an iterative reconstruction-scanning loop. It predicts uncertainty distributions over potential scanning viewpoints without requiring ground truth geometry or appearance information, enabling efficient viewpoint selection.

Result: Auto3R significantly outperforms state-of-the-art methods by a large margin in extensive experiments. It was successfully deployed on a robot arm with a camera, demonstrating practical ability to digitize real-world 3D objects and produce photorealistic digital assets.

Conclusion: Auto3R provides an effective automated solution for 3D scanning and reconstruction that works with challenging materials, enables robotic deployment, and produces high-quality digital assets, advancing the field of automated 3D digitization.
...
Method: Proposes Draft-as-CoT (DraCo) with three stages: 1) generate low-resolution draft image as visual preview, 2) verify semantic alignment between draft and prompt using model's understanding, 3) perform selective corrections with super-resolution refinement. Uses DraCo-240K dataset for training three atomic capabilities and DraCo-CFG guidance strategy.

Result: Achieves significant improvements on benchmarks: +8% on GenEval, +0.91 on Imagine-Bench, and +3% on GenEval++, outperforming direct generation and other CoT-based methods.

Conclusion: DraCo's interleaved reasoning paradigm effectively addresses coarse-grained textual planning limitations and rare attribute combination challenges through visual draft generation and verification, enabling more accurate and controllable text-to-image generation.
...
TL;DR: PhyVLLM introduces a physical-guided video-language framework that explicitly incorporates physical motion modeling into Video LLMs to address their limitations in understanding physical dynamics, using dual-branch encoders and Neural ODEs for motion representation.


<details>
  <summary>Details</summary>
Motivation: Current Video LLMs fail in scenarios requiring deeper understanding of physical dynamics due to reliance on appearance-based matching. Motion signals are entangled with appearance, require continuous-time modeling, and lack labeled physical data.

Method: PhyVLLM uses a dual-branch encoder to disentangle appearance and object motion, incorporates Neural ODEs for differentiable physical dynamic representations, projects motion-aware representations into LLM token space, and employs self-supervised learning to avoid explicit physical labels.
...
Conclusion: Incorporating explicit physical motion modeling through disentangled representations and Neural ODEs enables Video LLMs to better understand physical dynamics while maintaining multimodal capabilities, addressing key limitations in current approaches.
...
TL;DR: Refaçade enables precise object retexturing by transferring local textures from reference to target objects in images/videos using texture removal and jigsaw permutation to disentangle texture from structure.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with precise texture transfer due to limited controllability - conditioning on raw reference images introduces unwanted structural information and fails to disentangle texture from structure.
...
Conclusion: Refaçade effectively addresses the object retexturing task by enabling precise and controllable texture transfer through disentanglement of texture from structure information.
...
TL;DR: Novel video-based facial analysis method combining Graph Attention Network for facial landmarks and 3D ResNet for spatiotemporal features achieves state-of-the-art performance (95.82% accuracy) for alcohol intoxication detection.
...
Method: Integrates facial landmark analysis via Graph Attention Network (GAT) with spatiotemporal visual features from 3D ResNet, using dynamic fusion with adaptive prioritization. Introduces curated dataset of 3,542 video segments from 202 individuals.

Result: Achieves 95.82% accuracy, 0.977 precision, and 0.97 recall, outperforming baselines (custom 3D-CNN and VGGFace+LSTM). Demonstrates potential for practical deployment in public safety systems.

Conclusion: The proposed approach shows strong performance for non-invasive alcohol intoxication detection, offering promising solution for public safety applications through video-based facial analysis.
...
TL;DR: X-Humanoid is a video editing method that converts human videos into humanoid robot videos using finetuned generative models, creating large-scale training data for embodied AI.


<details>
  <summary>Details</summary>
Motivation: Embodied AI development is limited by scarce training data. Existing methods that overlay robot arms on egocentric videos can't handle complex full-body motions and occlusions in third-person videos, making them unsuitable for converting human videos to humanoid robot videos.

Method: Adapts Wan 2.2 model into video-to-video structure and finetunes it for human-to-humanoid translation. Creates paired training data using Unreal Engine pipeline from community assets (17+ hours), then applies to Ego-Exo4D videos to generate robotized humanoid videos.

Result: Generated over 3.6 million robotized humanoid video frames from 60 hours of Ego-Exo4D videos. Quantitative analysis and user studies show superiority: 69% rated best for motion consistency, 62.1% for embodiment correctness.

Conclusion: X-Humanoid successfully bridges the data scarcity gap by creating large-scale, diverse humanoid robot video datasets, enabling better training of Vision-Language-Action models and world models for embodied AI.
...
Method: VideoMem dynamically updates a global memory buffer that retains critical information while discarding redundant content. It uses Progressive Grouped Relative Policy Optimization (PRPO) with Progressive State Propagation (PSP) to retain valid states and propagate them, and Temporal Cascading Reward (TCR) to alleviate reward sparsity.
...
Conclusion: VideoMem provides an effective solution for ultra-long video understanding by treating it as a sequential generation task with adaptive memory management, overcoming limitations of existing approaches.
...
TL;DR: GEF introduces entropy-based regularization for 3D Gaussian Splatting to improve surface reconstruction quality while maintaining rendering efficiency.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting excels at rendering but struggles with surface reconstruction quality. The paper aims to enhance geometric accuracy without sacrificing photometric fidelity by leveraging the insight that well-reconstructed surfaces exhibit low configurational entropy.
...
Result: Achieves superior Chamfer Distance (0.64) on DTU and F1 score (0.44) on T&T benchmarks, plus best SSIM (0.855) and LPIPS (0.136) on Mip-NeRF 360, demonstrating competitive geometric precision and rendering quality.

Conclusion: GEF successfully enhances surface reconstruction accuracy in 3D Gaussian Splatting while maintaining photometric fidelity, validated through extensive experiments on multiple benchmarks.
...
TL;DR: The paper introduces a novel adversarial attack on Document Visual Question Answering (DocVQA) systems that forges document content in visually imperceptible ways to induce specific incorrect answers, demonstrating vulnerabilities in state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: Current DocVQA models show impressive capabilities but remain vulnerable to adversarial attacks. The authors aim to expose critical security vulnerabilities by creating attacks that can forge document content in ways that are visually imperceptible to humans but can manipulate model outputs for malicious purposes.

Method: Develop specialized attack algorithms that produce adversarially forged documents tailored to different attacker goals. The attacks target document content in visually imperceptible yet semantically targeted ways. The approach is tested against two state-of-the-art models: Pix2Struct (vision-language transformer) and Donut (transformer-based document image QA model).

Result: The attacks successfully demonstrate effectiveness against both Pix2Struct and Donut models. The forged documents can induce specific incorrect answers or systematic model failures, highlighting critical vulnerabilities in current DocVQA systems.

Conclusion: Current DocVQA systems have significant security vulnerabilities to adversarial attacks that can forge document content. The findings call for the development of more robust defenses to protect against such targeted manipulation attacks in document understanding systems.
...
TL;DR: COOPER is a unified MLLM that enhances spatial reasoning by learning to generate depth and segmentation maps, achieving 6.91% improvement in spatial reasoning while maintaining general performance.
...
Method: COOPER is a unified MLLM trained in two stages: 1) learns to generate auxiliary modalities (depth and segmentation maps), 2) acquires adaptive interleaved reasoning capabilities. The model leverages depth and segmentation as auxiliary modalities to enhance spatial perception and reasoning.

Result: COOPER achieves average 6.91% improvement in spatial reasoning while maintaining general performance. Even a variant trained only for auxiliary modality generation attains 7.92% gain on distance and size estimation, showing that learning to generate auxiliary modalities helps internalize spatial knowledge.

Conclusion: A unified MLLM can develop intrinsic ability to enhance spatial perception through auxiliary modality generation and adaptive interleaved reasoning, leading to stronger spatial intelligence without sacrificing general performance.
...
TL;DR: A comprehensive review on creating high-quality datasets for deep learning in pathology, covering image acquisition, annotation, quality control, and providing a standard operating procedure for dataset development.


<details>
  <summary>Details</summary>
Motivation: Supervised deep learning for microscopic image analysis requires high-quality datasets, but creating them is resource-intensive and faces challenges like domain variability, bias risks, and time constraints.

Method: The review provides a comprehensive guide covering three critical steps: 1) image acquisition, 2) selection of annotation software, and 3) annotation creation. It emphasizes addressing domain shifts and introduces the three "C"s quality criteria (correctness, completeness, consistency). Advanced techniques to mitigate single-annotator limitations are explored, and a standard operating procedure (SOP) is provided as supplemental material.

Result: The review offers practical recommendations for creating high-quality datasets, including strategies to handle image variability, ensure annotation quality, and promote reproducibility through open datasets.

Conclusion: By addressing dataset creation challenges and providing best practices, this review aims to advance the availability of high-quality datasets, ultimately contributing to the development of generalizable and robust deep learning models for pathology applications.
...
TL;DR: A robotic craft assembly task where robots build target objects from available parts using image segmentation, template matching, primitive simplification, and search algorithms.


<details>
  <summary>Details</summary>
Motivation: Inspired by traditional handmade crafts where people improvise assemblies from available objects, the paper aims to enable robots to similarly build accurate representations of target objects using available parts that don't directly correspond to the target's components.

Method: 1) Use mask segmentation neural network to identify visible parts from RGB images, 2) Retrieve labeled template meshes, 3) Perform pose optimization to find best template, 4) Simplify template parts to primitive shapes (cuboids/cylinders), 5) Design search algorithm to find correspondences based on local and global proportions.

Result: The approach achieves comparable results to baselines (which consider all possible combinations and choose highest scoring ones) for two different scenes, with qualitative results shown for real-world implementation.

Conclusion: The paper successfully introduces the Craft Assembly Task and demonstrates a working pipeline for robotic assembly using computer vision and optimization techniques, showing feasibility in real-world scenarios.
...
TL;DR: TARDis is a physics-aware framework that treats missing CT phases as missing time points on continuous enhancement curves, using disentangled static anatomy and dynamic perfusion features to hallucinate missing hemodynamic data.
...
Method: TARDis uses time-attenuation curve representation with dual-path architecture: 1) quantization-based path with learnable embedding dictionary for time-invariant anatomical features, and 2) probabilistic path using Conditional VAE to model dynamic enhancement conditioned on scan time, allowing sampling to hallucinate missing hemodynamic features.

Result: Outperforms state-of-the-art incomplete modality frameworks on large-scale private abdominal CT dataset (2,282 cases) and two public datasets. Maintains robust diagnostic performance even in extreme data-sparsity scenarios.

Conclusion: TARDis effectively addresses missing modality problem by modeling temporal hemodynamic continuity, enabling radiation dose reduction while maintaining diagnostic precision through physics-aware feature hallucination.
...
<details>
  <summary>Details</summary>
Motivation: Infrared UAV targets have weak features and complex backgrounds, making accurate tracking challenging in anti-UAV applications. Existing methods struggle with these difficult conditions.

Method: Proposes SiamDFF with three key components: 1) Selective Target Enhancement Network (STEN) using intensity-aware multi-head cross-attention, 2) Dynamic Spatial Feature Aggregation Module (DSFAM) integrating local details with global features, 3) Dynamic Channel Feature Aggregation Module (DCFAM) for template feature integration. Also introduces target-aware contextual attention knowledge distillation to transfer target priors from teacher to student network.
...
Conclusion: SiamDFF effectively addresses the challenges of infrared UAV tracking through dynamic feature fusion and knowledge distillation, achieving superior performance in complex scenarios with practical real-time operation.
...
TL;DR: SAM3-I enhances SAM3 by enabling direct instruction-following segmentation with rich natural language expressions, not just noun phrases, while preserving original concept-driven capabilities.


<details>
  <summary>Details</summary>
Motivation: SAM3's noun-phrase prompts are too coarse for real-world usage that requires richer expressions with attributes, spatial relations, functionalities, actions, states, and implicit reasoning. Current workarounds using external agents and iterative filtering are inefficient and imprecise.
...
TL;DR: The paper introduces AbstainEQA, a dataset for evaluating when embodied question answering agents should abstain from answering ambiguous or unanswerable questions, showing current models perform poorly compared to humans.


<details>
  <summary>Details</summary>
Motivation: Current EQA benchmarks assume all questions must be answered, but real-world embodied agents need to know when they lack sufficient information to answer. Human analysis shows 32.4% of queries contain missing or underspecified context, highlighting the need for abstention capability.

Method: 1) Analyzed 500 human queries to identify abstention needs; 2) Derived five categories of ambiguous questions from cognitive theories; 3) Created AbstainEQA by transforming well-posed OpenEQA questions into ambiguous variants across these categories; 4) Evaluated frontier models, scaling, prompting, reasoning, and fine-tuning approaches.

Result: Best frontier model achieves only 42.79% abstention recall vs. human 91.17%. Scaling, prompting, and reasoning yield marginal gains. Fine-tuned models overfit to textual cues. AbstainEQA contains 1,636 annotated abstention cases paired with 1,636 original instances.

Conclusion: Abstention is a fundamental prerequisite for reliable embodied interaction and necessary basis for effective clarification. Current models significantly underperform humans in recognizing when to withhold answers, indicating a critical research gap.
...
TL;DR: Zero-shot pipeline for detecting, identifying, and localizing harmful visual content with pixel-accurate masks in one pass, using foundation segmentation models and vision-language models with ensemble protection against attacks.
...
Method: Uses SAM for candidate object masks, refines them into larger regions, scores each region for malicious relevance using vision-language models with open-vocabulary prompts, fuses scores into consolidated malicious object maps, and employs ensemble across multiple segmenters for robustness.

Result: Achieves 85.8% element-level recall, 78.1% precision, 92.1% segment-success rate on 790-image dataset (drug, sexual, violent, extremist content), outperforming direct VLM localization by 27.4% recall. Maintains robustness with ≤10% performance drop against PGD adversarial attacks.

Conclusion: First practical tool for fine-grained, explainable malicious-image moderation that processes images in seconds, integrates with existing VLM workflows, and provides robust, pixel-accurate localization of harmful content elements.
...
TL;DR: HeFT is a zero-shot point tracking framework that uses pretrained video diffusion models' visual priors, with head- and frequency-aware feature selection for state-of-the-art performance without training data.


<details>
  <summary>Details</summary>
Motivation: The paper aims to leverage the visual priors of pretrained video diffusion models for point tracking tasks, eliminating the need for annotated training data while achieving competitive performance.

Method: Analyzes VDiT representations to discover attention head specializations and frequency component importance, then proposes head- and frequency-aware feature selection with single-step denoising, soft-argmax localization, and forward-backward consistency checks.

Result: HeFT achieves state-of-the-art zero-shot tracking performance on TAP-Vid benchmarks, approaching supervised method accuracy without requiring annotated training data.

Conclusion: Video diffusion models show promise as powerful foundation models for downstream tasks, with HeFT demonstrating their potential for zero-shot point tracking and paving the way toward unified visual foundation models.
...
TL;DR: I2I-Bench is a comprehensive benchmark for image-to-image editing models that addresses limitations of existing benchmarks through diverse tasks, automated multi-dimensional evaluation, and validation of alignment with human preferences.


<details>
  <summary>Details</summary>
Motivation: Existing image editing benchmarks have significant limitations: limited task scopes, insufficient evaluation dimensions, and heavy reliance on manual annotations, which constrain their scalability and practical applicability.

Method: I2I-Bench features: (1) 10 diverse task categories across single-image and multi-image editing, (2) 30 decoupled fine-grained evaluation dimensions with automated hybrid evaluation using specialized tools and large multimodal models (LMMs), and (3) rigorous alignment validation to ensure consistency with human preferences.

Result: The benchmark was used to evaluate numerous mainstream image editing models, revealing gaps and trade-offs between different models across various dimensions. All components will be open-sourced.

Conclusion: I2I-Bench provides a comprehensive, scalable, and automated evaluation framework for image editing models that addresses the limitations of existing benchmarks and will facilitate future research through open-source availability.
...
TL;DR: Live Avatar is a real-time streaming avatar generation system using a 14B-parameter diffusion model with novel parallelism and consistency mechanisms to overcome sequential computation bottlenecks.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based video generation methods suffer from sequential computation constraints and long-horizon inconsistency, limiting practical adoption for real-time, streaming audio-driven avatar synthesis.
...
Result: Achieves state-of-the-art performance with 20 FPS end-to-end generation on 5 H800 GPUs, enabling practical real-time high-fidelity avatar generation at scale - the first system to achieve this.

Conclusion: Establishes a new paradigm for deploying advanced diffusion models in industrial long-form video synthesis applications, overcoming fundamental limitations of existing methods.
...
TL;DR: Proposes Cross-View Point Correspondence (CVPC) task and CrossPoint-Bench benchmark to evaluate VLMs on precise point-level spatial correspondence, revealing large performance gaps vs humans, and introduces CrossPoint-378K dataset and CroPond model that achieves SOTA results.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models lack precise point-level cross-view correspondence capability, which is crucial for spatial understanding and embodied AI applications like affordance interaction. There's a need to benchmark and improve this fundamental spatial reasoning skill.

Method: 1) Proposes CVPC task and CrossPoint-Bench benchmark with hierarchical design (perceive, reason, correspond). 2) Constructs CrossPoint-378K dataset with 378K QA pairs across 900 scenes focused on actionable affordance regions. 3) Develops CroPond model trained on the dataset.

Result: State-of-the-art models (e.g., Gemini-2.5-Pro) perform poorly with 54.65% gap behind humans in overall accuracy. CroPond achieves SOTA on CrossPoint-Bench, surpassing Gemini-2.5-Pro by 39.7% accuracy, demonstrating significant improvement in cross-view correspondence.

Conclusion: The work establishes a benchmark for cross-view point correspondence, reveals substantial limitations in current VLMs for fine-grained spatial reasoning, and provides dataset and model (CroPond) that significantly advances the capability, offering foundation for future embodied AI research.
...
TL;DR: OmniScaleSR is a diffusion-based realistic arbitrary-scale super-resolution framework that achieves both high fidelity and realism by combining explicit scale control mechanisms with implicit diffusion priors.
...
Conclusion: OmniScaleSR successfully addresses the limitations of both INR-based arbitrary-scale SR and diffusion-based Real-ISR by combining explicit scale control with diffusion priors, achieving superior performance across arbitrary magnification scales.
...
TL;DR: The paper introduces MIND, a hierarchical visual encoder that addresses Articulatory-Affective Ambiguity in conversational analysis by suppressing ambiguous lip features, along with ConvoInsight-DB dataset and PRISM evaluation metric, achieving +86.95% improvement in micro-expression detection.


<details>
  <summary>Details</summary>
Motivation: Two fundamental challenges in generative psychological analysis of conversations: (1) VLMs fail to resolve Articulatory-Affective Ambiguity where speech visual patterns mimic emotional expressions, and (2) lack of verifiable evaluation metrics for assessing visual grounding and reasoning depth.

Method: Proposes a complete ecosystem: (1) MIND - hierarchical visual encoder with Status Judgment module that suppresses ambiguous lip features based on temporal feature variance for explicit visual disentanglement; (2) ConvoInsight-DB - large-scale dataset with expert annotations; (3) PRISM - automated dimensional framework using expert-guided LLM for multidimensional performance measurement.

Result: MIND significantly outperforms all baselines on PRISM benchmark, achieving +86.95% gain in micro-expression detection over prior SOTA. Ablation studies confirm the Status Judgment disentanglement module is the most critical component for this performance leap.

Conclusion: The proposed ecosystem addresses fundamental challenges in conversational psychological analysis through novel visual disentanglement, comprehensive dataset, and verifiable evaluation metric, with code made publicly available.
...
TL;DR: E3AD is an emotion-aware vision-language-action framework for autonomous driving that incorporates passenger emotional state into trajectory planning using continuous VAD emotion modeling and dual-pathway spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end autonomous driving systems using vision-language-action models ignore passenger emotional states, which are crucial for comfort and AD acceptance. There's a need for systems that can interpret natural language commands while also inferring and responding to passenger emotions.

Method: Proposes E3AD framework with: 1) Continuous Valence-Arousal-Dominance (VAD) emotion model to capture tone/urgency from language, 2) Dual-pathway spatial reasoning module fusing egocentric and allocentric views for human-like spatial cognition, 3) Consistency-oriented training combining modality pretraining with preference-based alignment to ensure coherence between emotional intent and driving actions.
...
Conclusion: Injecting emotion into VLA-style autonomous driving yields more human-aligned systems with improved grounding, planning, and feedback capabilities, showing the importance of emotional awareness for autonomous vehicle acceptance and comfort.
...
<details>
  <summary>Details</summary>
Motivation: Existing depth completion approaches rely on semantic segmentation but overlook object-level understanding. The authors aim to leverage instance-aware cues to improve depth accuracy, particularly near object boundaries, occlusions, and thin structures.
...
Conclusion: Incorporating instance-aware cues offers a promising direction for improving depth completion without relying on dense semantic labels, demonstrating that object-level understanding provides valuable spatial priors for depth refinement.
...
TL;DR: VRSketch2Shape: A framework for generating 3D shapes from sequential VR sketches using order-aware encoding and diffusion models, outperforming prior work in geometric fidelity and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing sketch-to-shape models ignore the temporal ordering of strokes in VR sketching, discarding crucial structural and design intent cues that are naturally captured in sequential drawing processes.
...
TL;DR: PaCo-RL is a reinforcement learning framework for consistent image generation that combines a pairwise consistency reward model (PaCo-Reward) with an efficient RL algorithm (PaCo-GRPO) to achieve state-of-the-art visual consistency without large supervised datasets.


<details>
  <summary>Details</summary>
Motivation: Consistent image generation (preserving identities, styles, and logical coherence across multiple images) is essential for applications like storytelling and character design, but supervised approaches struggle due to lack of large-scale consistency datasets and the complexity of modeling human perceptual preferences.

Method: Two-component framework: 1) PaCo-Reward - a pairwise consistency evaluator trained on automated sub-figure pairing data, using generative autoregressive scoring with task-aware instructions and CoT reasoning; 2) PaCo-GRPO - an efficient RL algorithm with resolution-decoupled optimization to reduce costs and log-tamed multi-reward aggregation for balanced optimization.
...
Conclusion: PaCo-RL demonstrates the promise of RL for learning complex visual criteria without large datasets, offering a practical and scalable solution for consistent image generation that outperforms supervised approaches.
...
TL;DR: LaFiTe introduces a 3D-native texturing framework using sparse latent color fields to generate high-fidelity textures directly on 3D surfaces, surpassing UV-based methods by >10 dB PSNR.


<details>
  <summary>Details</summary>
Motivation: Existing 3D-native texturing methods lack powerful latent representations, limiting texture fidelity and generality. The representation gap is identified as the main barrier to progress in generating seamless textures directly on 3D surfaces.
...
Result: Achieves unprecedented fidelity, exceeding state-of-the-art methods by >10 dB PSNR in reconstruction. Effectively disentangles texture appearance from mesh topology and UV parameterization.

Conclusion: LaFiTe sets a new benchmark for 3D-native texturing and enables flexible downstream applications like material synthesis and texture super-resolution, paving the way for next-generation 3D content creation.
...
TL;DR: EMMA is an efficient unified multimodal architecture that handles understanding, generation, and editing tasks with 4 key innovations: efficient autoencoder compression, channel-wise concatenation, shared-and-decoupled network, and mixture-of-experts mechanism.


<details>
  <summary>Details</summary>
Motivation: To create a unified multimodal architecture that efficiently handles understanding, generation, and editing tasks while addressing the computational inefficiency and training imbalance issues in existing approaches.

Method: 1) Efficient autoencoder with 32x compression ratio to reduce tokens and balance training; 2) Channel-wise concatenation instead of token-wise to further reduce visual tokens; 3) Shared-and-decoupled network for mutual task improvements; 4) Mixture-of-experts mechanism for visual understanding encoder.
...
Conclusion: EMMA provides an efficient and effective foundation for unified multimodal architectures, demonstrating that a single model can achieve strong performance across multiple multimodal tasks while maintaining computational efficiency.
...
TL;DR: RobustSplat++ improves 3D Gaussian Splatting for in-the-wild scenes by preventing transient object artifacts through delayed Gaussian growth and scale-cascaded mask bootstrapping.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods struggle with in-the-wild scenes containing transient objects and illumination variations, causing artifacts in rendered images due to Gaussian densification inadvertently modeling these disturbances.

Method: Three key designs: 1) Delayed Gaussian growth strategy prioritizing static structure optimization before splitting/cloning; 2) Scale-cascaded mask bootstrapping using low-resolution feature similarity for initial transient mask estimation then high-resolution refinement; 3) Integration of both strategies with appearance modeling.
...
Conclusion: RobustSplat++ successfully addresses transient object and illumination artifacts in 3DGS through strategic Gaussian growth control and progressive mask refinement, enabling robust novel-view synthesis in challenging real-world conditions.
...
TL;DR: LatentFM is a flow-based model for medical image segmentation that operates in latent space, using VAEs to encode images and masks, then estimating conditional velocity fields to generate diverse segmentation outputs with uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the remarkable progress of flow matching (FM) in generative models, which has demonstrated strong generative capabilities and can learn exact data densities. The authors aim to leverage these advances for medical image segmentation to create uncertainty-aware predictions that provide richer information for clinical analysis.

Method: The method involves: 1) Designing two variational autoencoders (VAEs) to encode medical images and their corresponding masks into a lower-dimensional latent space; 2) Estimating a conditional velocity field that guides the flow based on the input image; 3) Sampling multiple latent representations to synthesize diverse segmentation outputs whose pixel-wise variance captures the underlying data distribution.

Result: Experiments on ISIC-2018 and CVC-Clinic datasets show superior segmentation accuracy compared to prior baselines (both deterministic and generative approaches). The method generates confidence maps that quantify model certainty and remains highly efficient by operating in latent space.

Conclusion: LatentFM successfully applies flow matching to medical image segmentation, achieving both high accuracy and uncertainty-aware predictions. The approach provides clinicians with richer information through confidence maps and demonstrates the effectiveness of flow-based models in latent space for medical imaging tasks.
...
Result: Experiments show that FreeGen achieves state-of-the-art performance for free-viewpoint driving scene synthesis.

Conclusion: FreeGen successfully addresses the joint challenge of interpolation consistency and extrapolation realism in free-viewpoint driving scene synthesis through a novel co-training approach that combines reconstruction and generation models.
...
TL;DR: SBM is a Transformer-based model for BIM layout synthesis that tokenizes building elements into sequences, learns unified embeddings, and operates in two modes: encoder-only for room embeddings and encoder-decoder for autoregressive room prediction.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of tokenizing heterogeneous architectural elements in BIM scenes while preserving compositional structure, enabling effective layout synthesis and semantic understanding of building spaces.
...
Conclusion: SBM successfully addresses BIM layout synthesis by unifying heterogeneous architectural features into learnable sequences, demonstrating effectiveness in both retrieval and generative tasks through its dual-mode Transformer architecture.
...
TL;DR: Proposes DevDet framework for Multi-In-Domain Face Forgery Detection to address domain dominance issue in deepfake detection, improving real/fake judgments in domain-unspecified conditions.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods aim for generalization but struggle with real-world applications due to domain dominance - domain differences overshadow subtle real/fake distinctions in multi-domain training scenarios, leading to poor single-image judgments in domain-unspecified conditions.

Method: Introduces MID-FFD research paradigm with sufficient real-fake domains for training, then proposes DevDet framework with Face Forgery Developer (FFDev) to amplify real/fake differences and Dose-Adaptive detector Fine-Tuning strategy (DAFT) to make real/fake distinctions dominant in feature space.

Result: DevDet demonstrates superiority in predicting real/fake under MID-FFD scenario while maintaining original generalization ability to unseen data, addressing the domain-dominant issue that causes low accuracy in domain-unspecified conditions.

Conclusion: The proposed MID-FFD paradigm and DevDet framework effectively solve the domain dominance problem in multi-domain deepfake detection, enabling better real/fake judgments in real-world, domain-unspecified scenarios while preserving generalization capabilities.
...
TL;DR: LineAR is a training-free KV cache compression method for autoregressive image generation that reduces memory usage and improves throughput while maintaining or enhancing generation quality.
...
Method: LineAR uses a progressive KV cache compression pipeline that manages cache at line level with a 2D view, preserving visual dependency regions while evicting less-informative tokens based on inter-line attention.

Result: LineAR achieves significant improvements: ImageNet FID from 2.77 to 2.68, COCO FID from 23.85 to 22.86 with only 1/6 KV cache; up to 67.61% memory reduction and 7.57x speedup on LlamaGen-XL.

Conclusion: LineAR enables efficient AR image generation with substantial memory savings and throughput improvements while maintaining or enhancing quality, validated across six different AR image generation models.
...
<details>
  <summary>Details</summary>
Motivation: Video-based pose estimation often fails in self-contact scenarios (e.g., hand touching face), while wearable bioimpedance sensing can cheaply measure ground-truth skin-to-skin contact. Combining both could capture more accurate 3D human pose in the wild.

Method: BioTUCH initializes pose using off-the-shelf estimator, then performs contact-aware pose optimization during measured self-contact. It minimizes reprojection error and deviations from input estimate while enforcing vertex proximity constraints using bioimpedance data.

Result: Validated on new synchronized dataset (RGB video, bioimpedance, motion capture). With three input pose estimators, achieved average 11.7% improvement in reconstruction accuracy. Also developed miniature wearable bioimpedance sensor for scalable data collection.

Conclusion: BioTUCH effectively combines visual pose estimation with bioimpedance sensing to improve 3D pose reconstruction by incorporating self-contact information, enabling better training data collection for pose estimation and generation methods.
...
Method: Proposes SP-Det with two key components: 1) Expert-free dual-text prompt generator (DTPG) that creates two complementary textual modalities - semantic context prompts (global pathological patterns) and disease beacon prompts (disease-specific manifestations). 2) Bidirectional feature enhancer (BFE) that integrates comprehensive diagnostic context with disease-specific embeddings to improve feature representation.
...
Conclusion: SP-Det successfully addresses the limitation of manual prompt dependency in lesion detection frameworks, offering a practical solution for clinical applications through automatic prompt generation and improved feature integration.
...
TL;DR: SDG-Track: A sparse detection-guided tracker for real-time UAV tracking on edge devices using Observer-Follower architecture to resolve resolution-speed conflict.
...
Method: SDG-Track adopts an Observer-Follower architecture: Observer stream runs high-capacity detector at low frequency on GPU for accurate position anchors from 1920x1080 frames; Follower stream performs high-frequency trajectory interpolation via ROI-constrained sparse optical flow on CPU. Includes Dual-Space Recovery mechanism combining color histogram matching with geometric consistency constraints for tracking failure recovery.
...
Conclusion: SDG-Track effectively reconciles the resolution-speed conflict for real-time UAV tracking on edge devices through a sparse detection-guided approach with training-free recovery mechanisms, enabling practical deployment on resource-constrained platforms.
...
<details>
  <summary>Details</summary>
Motivation: Address catastrophic forgetting in object detection for retail applications where new products are frequently introduced, eliminating the need to retrain on entire previous datasets and reducing training costs/time.

Method: YOTO framework integrates YOLO11n for object localization, DeIT for feature extraction, Proxy Anchor Loss for metric learning, and uses cosine similarity between target product embeddings and Qdrant vector database for classification.

Result: Achieves encouraging accuracy for both new and existing products in retail case study with 140 products, 3x training time efficiency compared to classical approaches, and 580ms average inference time per image on edge device.

Conclusion: YOTO framework effectively addresses catastrophic forgetting in retail object detection, enabling efficient addition of new products without retraining while maintaining accuracy and practical feasibility on edge devices.
...
TL;DR: E(3)-Pose is a novel fast pose estimation method that jointly models rotation equivariance and object symmetry for robust fetal head pose estimation in MRI scans, enabling automatic adaptive prescription of 2D diagnostic slices.
...
Method: E(3)-Pose is a novel pose estimation method that jointly and explicitly models rotation equivariance and object symmetry. The method captures anatomical symmetries and rigid pose equivariance by construction, which helps overcome pose ambiguities in clinical MRI volumes.
...
Conclusion: E(3)-Pose provides a robust solution for fetal head pose estimation in clinical MRI settings by explicitly modeling rotation equivariance and object symmetry, overcoming limitations of existing methods and enabling practical clinical applications for adaptive MRI slice prescription.
...
Conclusion: ReflexFlow is a simple, effective, model-agnostic method compatible with all Flow Matching frameworks that significantly reduces exposure bias and improves generation quality across diverse datasets.
...
<details>
  <summary>Details</summary>
Motivation: Current latent diffusion models generate semantic structure and texture synchronously, neglecting the natural coarse-to-fine generation process where semantics should precede textures. This synchronous approach misses the opportunity for semantic guidance to enhance texture refinement.

Method: SFD constructs composite latents combining compact semantic latents (extracted via a dedicated Semantic VAE) with texture latents. It denoises semantic and texture latents asynchronously using separate noise schedules, with semantics preceding textures by a temporal offset to provide clearer high-level guidance.
...
Conclusion: The asynchronous, semantics-led modeling approach in SFD effectively leverages the natural coarse-to-fine generation process, providing clearer semantic guidance for texture refinement and enabling superior performance and faster convergence in latent diffusion models.
...
TL;DR: First top-down method for automatically fitting a surface model to CT scans of severely damaged Herculaneum scrolls using explicit parametric modeling and neural network predictions.


<details>
  <summary>Details</summary>
Motivation: The Herculaneum Papyri contain valuable unseen Greek/Latin texts but are too fragile to physically unroll. Virtual unrolling via CT scans is needed, but manual tracing is laborious in gigavoxel-sized scans, requiring automated approaches.

Method: Top-down approach that globally fits an explicit parametric model of the deformed scroll to neural network predictions of where the rolled papyrus likely passes. Guarantees a single continuous 2D sheet even through regions where surface is not detectable.
...
Conclusion: The approach provides an effective automated solution for virtual unrolling of severely damaged Herculaneum scrolls, enabling access to previously inaccessible ancient texts through computational methods.
...
TL;DR: LiteVGGT is an efficient 3D vision foundation model that achieves 10x speedup and substantial memory reduction for large-scale scene processing (up to 1000 images) while maintaining core performance.


<details>
  <summary>Details</summary>
Motivation: Existing 3D vision foundation models like VGGT are time-consuming and memory-intensive for long sequences, limiting their application to large-scale scenes beyond hundreds of images.

Method: Proposes geometry-aware cached token merging: (1) analyzes token geometric importance for optimized anchor selection, (2) caches and reuses merge indices across layers based on stable token similarity patterns, and (3) enables efficient fine-tuning and FP8 quantization.

Result: Achieves up to 10x speedup and substantial memory reduction, enabling efficient processing of 1000-image scenes while retaining VGGT's core performance. Validated through extensive experiments showing effectiveness, scalability, and robustness.

Conclusion: LiteVGGT successfully addresses the efficiency limitations of 3D vision foundation models for large-scale scenes through geometry-aware token merging and caching strategies, making practical applications with thousands of images feasible.
...
TL;DR: A novel multimodal action recognition method using deep neural networks with adaptive gating mechanisms for RGB, optical flow, audio, and depth fusion, achieving superior performance over unimodal approaches.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of traditional unimodal action recognition methods by leveraging multiple modalities (RGB, optical flow, audio, depth) for more comprehensive action understanding and improved robustness.

Method: Deep neural networks with adaptive fusion strategies using gating mechanisms to selectively integrate relevant information from multiple modalities, exploring various gated fusion architectures for optimal multimodal integration.

Result: Demonstrated promising advancements in accuracy across human action recognition, violence detection, and self-supervised learning tasks on benchmark datasets, showing superiority over conventional unimodal methods.

Conclusion: The gating-based multimodal fusion approach revolutionizes action recognition systems, enabling sophisticated applications in surveillance and human-computer interaction, particularly for active assisted living contexts.
...
TL;DR: FASTer framework improves VLA models by introducing efficient action tokenization that balances reconstruction quality and inference speed, achieving better performance and faster inference than previous methods.
...
Method: FASTerVQ encodes action chunks as single-channel images for spatio-temporal dependency capture, while FASTerVLA uses block-wise autoregressive decoding with a lightweight action expert on top of this tokenizer.
...
Conclusion: The FASTer framework provides an efficient and generalizable solution for robot learning that overcomes the traditional trade-off between reconstruction fidelity and inference efficiency in VLA models.
...
TL;DR: GeoPE extends RoPE to 3D using quaternions to better capture 2D spatial geometry in Vision Transformers, improving performance on vision tasks.
...
Method: GeoPE extends rotations to 3D Euclidean space using quaternions. To overcome non-commutativity and ensure symmetry, it constructs a unified rotational operator by computing the geometric mean in the Lie algebra, creating a geometrically coupled encoding.
...
TL;DR: A balanced few-shot learning framework for retinal disease diagnosis that addresses class imbalance through balanced episodic sampling, targeted augmentation, and ResNet-50 encoding, achieving improved accuracy and reduced bias for underrepresented diseases.
...
Result: The framework achieves substantial accuracy gains and reduces bias toward majority classes, with notable improvements for underrepresented diseases. Trained on 100 episodes and evaluated on 1,000 test episodes, it demonstrates robust performance on the RFMiD dataset.
...
<details>
  <summary>Details</summary>
Motivation: Most existing AI-generated image detection methods only use final-layer CLIP-ViT features, but earlier layers provide more localized and generalizable features that can improve detection performance and generalization.

Method: MoLD (Multi-layer Dynamic integration) uses a gating-based mechanism to dynamically integrate features from multiple ViT layers, adaptively weighting contributions from different layers based on their relevance to the detection task.

Result: Extensive experiments show MoLD significantly improves detection performance on both GAN- and diffusion-generated images, enhances generalization across diverse generative models, and exhibits robustness in real-world scenarios. The approach also scales successfully to other pre-trained ViTs like DINOv2.

Conclusion: Layer-wise feature analysis reveals earlier ViT layers provide valuable localized features for AI-generated image detection, and adaptive multi-layer integration via MoLD offers superior performance, generalization, and versatility compared to single-layer approaches.
...
TL;DR: A family of stable contrastive losses for learning pixel-level representations that capture both semantic and geometric information through overcomplete descriptors.


<details>
  <summary>Details</summary>
Motivation: To create pixel-level representations that jointly capture semantic and geometric information without requiring complex momentum-based teacher-student training architectures.

Method: Uses stable contrastive losses to map each pixel to overcomplete descriptors that are both view-invariant and semantically meaningful, enabling precise point-correspondence across images.

Result: Demonstrated through experiments in synthetic 2D and 3D environments, showing the properties of the loss and resulting overcomplete representations.

Conclusion: The proposed approach successfully learns pixel-level representations that capture both semantic and geometric information without complex training architectures, enabling precise cross-image point-correspondence.
...
TL;DR: LVLM-based text-to-image models produce more biased images than non-LVLM models, with system prompts identified as the primary bias driver. The paper introduces a benchmark, analyzes bias mechanisms, and proposes FairPro - a training-free framework that reduces bias while maintaining text-image alignment.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models (LVLMs) dominate text-to-image generation, but their tendency to amplify social biases remains poorly understood. The authors aim to systematically investigate whether LVLM-based models produce more biased outputs than non-LVLM models and identify the mechanisms behind bias propagation.

Method: 1) Created a 1,024-prompt benchmark spanning four linguistic complexity levels to evaluate demographic bias across multiple attributes. 2) Used decoded intermediate representations, token-probability diagnostics, and embedding-association analyses to trace bias propagation. 3) Identified system prompts as the primary bias driver. 4) Proposed FairPro - a training-free meta-prompting framework that enables LVLMs to self-audit and construct fairness-aware system prompts at test time.

Result: LVLM-based models produce significantly more socially biased images than non-LVLM models. System prompts encode demographic priors that propagate into image synthesis. FairPro substantially reduces demographic bias while preserving text-image alignment in experiments with SANA and Qwen-Image models.

Conclusion: System prompts play a central role in bias propagation in LVLM-based T2I systems. FairPro offers a practical, deployable solution for building more socially responsible image generation systems without requiring model retraining. The findings provide deeper insights into bias mechanisms in modern T2I architectures.
...
<details>
  <summary>Details</summary>
Motivation: VANICP accelerates point cloud registration but has high memory requirements that prevent deployment on resource-constrained embedded systems with limited GPU memory.

Method: Proposes GPU-oriented dynamic memory assignment strategy that optimizes memory usage for dilation operations, creating an enhanced VANICP framework.

Result: Achieves over 97% reduction in memory consumption while preserving original performance, enabling deployment on embedded GPUs.

Conclusion: Successfully creates a memory-efficient version of VANICP suitable for embedded systems without sacrificing computational efficiency.
...
Method: Repurpose a pre-trained DiT-based foundation model by conditioning it on reflection-contaminated inputs and guiding it toward clean transmission layers. Use a physically based rendering pipeline in Blender to synthesize realistic glass materials and reflection effects. Apply efficient LoRA-based adaptation of the foundation model combined with the synthetic data.
...
Conclusion: Pretrained diffusion transformers, when combined with physically grounded data synthesis and efficient adaptation, provide a scalable and high-fidelity solution for reflection removal tasks.
...
<details>
  <summary>Details</summary>
Motivation: Transparent objects are challenging for depth sensors due to refraction/reflection. Existing supervised methods require costly depth annotation data, creating a need for self-supervised alternatives.
...
Conclusion: The self-supervised approach effectively addresses the depth completion challenge for transparent objects without costly annotations, offering practical benefits especially in data-scarce scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing perceptual codecs rely on image generative priors that lack temporal modeling, causing perceptual flickering artifacts. There's a need for video-native generative priors that can maintain temporal coherence while enhancing compression quality.

Method: GNVC-VD introduces a unified flow-matching latent refinement module with a video diffusion transformer that jointly enhances intra- and inter-frame latents through sequence-level denoising. Instead of starting from Gaussian noise, it initializes refinement from decoded latents and learns a correction term for compression degradation. A conditioning adaptor injects compression-aware cues into DiT layers for artifact removal.

Result: GNVC-VD surpasses both traditional and learned codecs in perceptual quality, significantly reduces flickering artifacts compared to prior generative approaches, and works effectively even below 0.01 bpp.

Conclusion: GNVC-VD demonstrates the promise of integrating video-native generative priors into neural codecs for next-generation perceptual video compression, offering superior temporal coherence and artifact removal under extreme bitrate constraints.
...
TL;DR: HTR-ConvText: A hybrid model combining CNN, MobileViT, and textual context injection for handwritten text recognition, achieving better generalization with limited data.


<details>
  <summary>Details</summary>
Motivation: Handwritten Text Recognition faces challenges due to limited data, high writing style variance, and complex scripts with diacritics. Existing methods struggle to generalize without massive synthetic data.
...
Result: Evaluated on IAM, READ2016, LAM and HANDS-VNOnDB datasets, achieving improved performance and better generalization compared to existing methods, especially with limited training samples and high handwriting diversity.

Conclusion: HTR-ConvText effectively addresses HTR challenges by capturing fine-grained stroke-level features while preserving global context, demonstrating superior performance and generalization in data-limited scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing foundation models for Earth observation data often expect fixed input resolutions or use sensor-specific encoders, limiting generalization across heterogeneous EO modalities with varying spatial, spectral, and temporal resolutions.
...
Conclusion: RAMEN provides a unified, sensor-agnostic approach for Earth observation data analysis that enables coherent multimodal representation learning with user-controllable spatial resolution, addressing limitations of existing fixed-resolution or sensor-specific models.
...
TL;DR: Proposes semantic-guided hierarchical synthesis for facial image inpainting, using CNN-ViT fusion for semantic layout generation followed by multi-modal texture refinement, achieving state-of-the-art results on large irregular masks.
...
Method: Two-stage semantic-guided hierarchical synthesis: 1) CNN-ViT fusion for semantic layout generation (local+global features), 2) Multi-Modal Texture Generator for multi-scale texture refinement. Handles arbitrary masks via dynamic attention without mask-specific training.
...
Conclusion: The proposed semantic-guided hierarchical synthesis effectively addresses facial inpainting challenges by combining structural understanding with texture refinement, achieving superior performance on large irregular masks through better facial prior exploitation.
...
TL;DR: MoRe4D: A framework for generating interactive 4D scenes from single static images by jointly performing motion generation and geometric reconstruction, using a diffusion-based 4D trajectory generator and view synthesis module.
...
Method: 1) Created TrajScene-60K dataset with 60K video samples and dense point trajectories. 2) Proposed 4D-STraG diffusion model to generate geometrically consistent 4D point trajectories. 3) Designed depth-guided motion normalization and motion-aware module for geometry-dynamics integration. 4) Developed 4D-ViSM to render videos from arbitrary camera trajectories.

Result: MoRe4D generates high-quality 4D scenes with multi-view consistency and rich dynamic details from single images, outperforming existing methods.

Conclusion: The proposed MoRe4D framework successfully addresses spatiotemporal inconsistencies in 4D scene generation by jointly modeling motion and geometry, enabled by a new large-scale dataset and diffusion-based trajectory generation.
...
TL;DR: 4DLangVGGT: First Transformer-based feed-forward framework for 4D language grounding that integrates geometric perception and language alignment, enabling efficient open-vocabulary 4D scene understanding without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 4D semantic field methods rely on scene-specific Gaussian splatting requiring per-scene optimization, limiting generalization and scalability for real-world applications. There's a need for a unified framework that can handle multiple dynamic scenes efficiently.

Method: Proposes 4DLangVGGT with two key components: 1) 4D Visual Geometry Transformer (StreamVGGT) for spatio-temporal geometric representations, and 2) Semantic Bridging Decoder (SBD) that projects geometry-aware features into language-aligned semantic space. Jointly trainable across multiple scenes.

Result: Achieves state-of-the-art performance on HyperNeRF and Neu3D datasets with up to 2% gains under per-scene training and 1% improvements under multi-scene training. Enables efficient inference without per-scene optimization.

Conclusion: 4DLangVGGT establishes a new paradigm for open-vocabulary 4D scene understanding by combining geometric perception and language alignment in a unified feed-forward framework, significantly improving deployment efficiency and generalization for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Current video diffusion models couple scene dynamics with camera motion, limiting precise spatial and temporal control for fine-grained manipulation of both elements.

Method: Uses continuous world-time sequences and camera trajectories as conditioning inputs, injected through 4D positional encoding in attention layers and adaptive normalizations for feature modulation. Trained on a unique dataset with independently parameterized temporal and camera variations.

Result: Achieves robust real-world 4D control across diverse timing patterns and camera trajectories while preserving high generation quality, outperforming prior work in controllability.

Conclusion: The framework successfully decouples scene dynamics from camera pose, enabling fine-grained 4D control in video generation with improved controllability over existing methods.
...
TL;DR: Contact-guided 3D generation combines generative shape priors with contact information to improve object reconstruction from partial observations.
...
Method: Leverages two sources: 1) generative models for shape priors to guess unseen geometry, and 2) contact information from videos/interactions for boundary constraints. Combines them through contact-guided 3D generation inspired by drag-based editing in generative models.
...
Conclusion: Combining generative shape priors with contact constraints through guided 3D generation effectively reduces ambiguity and improves object geometry reconstruction for robot manipulation.
...
TL;DR: Deep Forcing enables training-free long-video generation with 12x extrapolation using persistent sink tokens and importance-aware KV cache pruning.
...
Method: Two training-free mechanisms: 1) Deep Sink dedicates half sliding window to persistent sink tokens with re-aligned temporal RoPE phase; 2) Participative Compression performs importance-aware KV cache pruning to preserve only actively participating tokens while discarding redundant history.
...
Conclusion: Training-free KV-cache management can match or exceed training-based approaches for autoregressively streaming long-video generation, demonstrating effective solutions to temporal degradation in video diffusion.
...
<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) are opaque - they output only final predictions without revealing intermediate reasoning steps or fine-grained evidence, unlike human visual reasoning which operates through chains of reasoning.

Method: Introduced VRT task requiring models to localize target objects AND predict intermediate objects forming reasoning paths. Created VRT-Bench benchmark, new evaluation metric for reasoning traces, and VRT-80k large-scale training dataset.
...
Conclusion: The VRT framework successfully addresses the opacity problem in MLLMs by making visual reasoning processes explicit and traceable, enabling better understanding and evaluation of model reasoning capabilities.
...
TL;DR: The paper introduces Spatial Aesthetics (SA) paradigm for evaluating AI-generated interior images across four dimensions, creates SA-BENCH benchmark, develops SA-IQA model, and applies it to improve AIGC generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing Image Quality Assessment (IQA) methods for AI-generated images focus mainly on portraits and artistic images, lacking systematic evaluation of interior scenes, which have unique spatial and aesthetic requirements.

Method: 1) Propose Spatial Aesthetics paradigm with four dimensions: layout, harmony, lighting, and distortion. 2) Construct SA-BENCH benchmark with 18,000 images and 50,000 annotations. 3) Develop SA-IQA through MLLM fine-tuning and multidimensional fusion approach. 4) Apply SA-IQA to downstream tasks: GRPO reinforcement learning optimization and Best-of-N selection filtering.

Result: SA-IQA significantly outperforms existing IQA methods on SA-BENCH benchmark. The framework effectively improves AIGC generation quality through reward-based optimization and selection filtering, setting new standard for spatial aesthetics evaluation.

Conclusion: The Spatial Aesthetics paradigm and SA-IQA framework provide comprehensive evaluation for interior images, addressing gaps in current IQA research. The open-sourced code and dataset will advance research and applications in spatial aesthetics assessment for AI-generated content.
...
Result: Extensive experiments on multiple benchmarks show EvoIR outperforms state-of-the-art AiOIR methods, with the combination of FMM and EOS yielding greater improvements than using either component alone.

Conclusion: EvoIR successfully addresses limitations of existing AiOIR methods through evolutionary frequency modulation and adaptive optimization, demonstrating superior performance in handling diverse image degradations.
...
Result: Produces controllable, spatially aligned results for photorealistic/stylized re-rendering and sim-to-real enhancement; improves CARLA-to-Waymo planner performance by 50%; adds no inference-time cost.

Conclusion: φ-PD enables structure-aligned generation compatible with any diffusion model for images/videos, complementary to existing conditioning approaches for image/video-to-image/video generation.
...
Method: Optimizes scene parameters (object pose, lighting) to reveal meaningful shadows, uses shadow strokes to guide line drawing generation, and employs automatic evaluation to enforce shadow-drawing coherence and visual quality.

Result: Produces compelling results across diverse inputs including real-world scans, curated datasets, and generative assets, and extends to multi-object scenes, animations, and physical deployments.
...
TL;DR: ARM-Thinker is an agentic multimodal reward model that uses external tools for verification to improve visual grounding and reliability on complex reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current reward models for vision-language systems suffer from hallucination, weak visual grounding, and inability to use verification tools, limiting reliability on complex multimodal reasoning tasks.
...
TL;DR: Splannequin improves frozen 3D scene synthesis from monocular Mannequin-Challenge videos using dynamic Gaussian splatting with temporal anchoring to fix artifacts, achieving high-fidelity user-selectable frozen renderings.


<details>
  <summary>Details</summary>
Motivation: Synthesizing high-fidelity frozen 3D scenes from monocular Mannequin-Challenge videos presents unique challenges distinct from standard dynamic scene reconstruction. The goal is to create frozen scenes while preserving subtle dynamics for user-controlled instant selection, but monocular capture with sparse temporal supervision introduces artifacts like ghosting and blur.

Method: The method introduces Splannequin, an architecture-agnostic regularization technique that applies dynamic Gaussian splatting to model scenes dynamically while rendering static scenes by fixing the time parameter. It detects two states of Gaussian primitives (hidden and defective) and applies temporal anchoring - hidden states are anchored to recent well-observed past states, while defective states are anchored to future states with stronger supervision. The approach integrates into existing dynamic Gaussian pipelines via simple loss terms without architectural changes.

Result: The method achieves markedly improved visual quality for frozen-time renderings, enabling high-fidelity, user-selectable frozen scenes. It's validated by 96% user preference and adds zero inference overhead to existing pipelines.

Conclusion: Splannequin effectively addresses artifacts in frozen 3D scene synthesis from monocular Mannequin-Challenge videos through temporal anchoring of Gaussian primitives, providing a practical solution that integrates seamlessly with existing dynamic Gaussian pipelines while significantly improving visual quality.
...
TL;DR: Light-X is a video generation framework that enables joint control of camera viewpoint and illumination from monocular videos, using disentangled geometry/lighting signals and synthetic training data.


<details>
  <summary>Details</summary>
Motivation: Current video illumination methods face a trade-off between lighting fidelity and temporal consistency. To achieve generative modeling of real-world scenes, joint control of both camera trajectory and illumination is needed since visual dynamics are shaped by both geometry and lighting.

Method: 1) Disentangled design: decouples geometry/motion (captured via dynamic point clouds projected along user-defined camera trajectories) from illumination (provided by relit frames consistently projected into same geometry). 2) Light-Syn pipeline: degradation-based approach with inverse-mapping that synthesizes training pairs from in-the-wild monocular footage to address lack of paired multi-view/multi-illumination data.

Result: Light-X outperforms baseline methods in joint camera-illumination control and surpasses prior video relighting methods under both text- and background-conditioned settings. The Light-Syn pipeline creates a robust dataset covering static, dynamic, and AI-generated scenes.

Conclusion: Light-X enables controllable video rendering with both viewpoint and illumination control from monocular videos, advancing toward generative modeling of real-world scenes through effective disentanglement of geometry and lighting signals.
...
### [154] [From Generated Human Videos to Physically Plausible Robot Trajectories](https://arxiv.org/abs/2512.05094)
*James Ni,Zekai Wang,Wei Lin,Amir Bar,Yann LeCun,Trevor Darrell,Jitendra Malik,Roei Herzig*

Main category: cs.RO

TL;DR: A two-stage pipeline that enables humanoid robots to execute human actions from noisy generated videos in zero-shot manner, using 4D human representation lifting and physics-aware reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Video generation models can synthesize human actions in novel contexts, potentially serving as high-level planners for robot control. However, generated videos are often noisy and morphologically distorted, making direct imitation difficult for humanoids.

Method: Two-stage approach: 1) Lift video pixels into 4D human representation and retarget to humanoid morphology, 2) GenMimic - physics-aware RL policy conditioned on 3D keypoints with symmetry regularization and keypoint-weighted tracking rewards. Also created GenMimicBench dataset for evaluation.

Result: GenMimic can mimic human actions from noisy generated videos, outperforming strong baselines in simulation. Demonstrates coherent, physically stable motion tracking on Unitree G1 humanoid robot without fine-tuning.

Conclusion: This work offers a promising path to realizing video generation models as high-level policies for robot control by enabling zero-shot execution of human actions from generated videos on humanoid robots.
...
### [155] [Hardware-aware Neural Architecture Search of Early Exiting Networks on Edge Accelerators](https://arxiv.org/abs/2512.04705)
*Alaa Zniber,Arne Symons,Ouassim Karrakchou,Marian Verhelst,Mounir Ghogho*

Main category: cs.CC

TL;DR: Hardware-aware NAS framework optimizes early exit placement in neural networks for edge deployment, reducing computation by 50%+ on CIFAR-10.


<details>
  <summary>Details</summary>
Motivation: Edge devices have strict computational/energy constraints that challenge deployment of large DL models. Early Exiting Neural Networks (EENN) offer efficiency but their performance is affected by hardware heterogeneity and quantization, with limited research on automatic optimization for edge hardware.

Method: Propose a hardware-aware Neural Architecture Search (NAS) framework that systematically integrates quantization effects and hardware resource allocation to optimize early exit point placement within network backbones.

Result: On CIFAR-10 dataset, the NAS framework discovers architectures achieving over 50% reduction in computational costs compared to conventional static networks.

Conclusion: The hardware-aware NAS framework successfully optimizes EENN designs for edge deployment, making them more suitable for resource-constrained environments by significantly reducing computational costs while maintaining performance.
...
### [156] [Human-Centred Evaluation of Text-to-Image Generation Models for Self-expression of Mental Distress: A Dataset Based on GPT-4o](https://arxiv.org/abs/2512.04087)
*Sui He,Shenbin Qian*

Main category: q-bio.NC

TL;DR: AI-generated images using GPT-4o with counseling-based prompts help Chinese international students express mental distress, with illustrator persona performing best.


<details>
  <summary>Details</summary>
Motivation: International students face linguistic and cultural barriers in communicating mental distress, creating need for alternative communication methods beyond traditional language.

Method: 20 Chinese international students described mental distress experiences; GPT-4o with 4 counseling-based persona prompts generated images; participants evaluated helpfulness of images for expressing feelings.

Result: Prompt design significantly affects perceived helpfulness; illustrator persona achieved highest ratings; created first public text-to-image evaluation dataset with human judgment scores in mental health domain.

Conclusion: AI-generated images can effectively support mental distress expression; persona-based prompt design matters; dataset enables future research in image evaluation, RLHF, and multimodal mental health communication.
...
### [157] [Limit cycles for speech](https://arxiv.org/abs/2512.04642)
*Adamantios I. Gafos,Stephan R. Kuberski*

Main category: q-bio.NC

TL;DR: The paper reveals that speech articulatory movements have a pervasive limit cycle organization with rhythmic structure, resolving the tension between biological rhythmicity and discrete movement models in speech production.


<details>
  <summary>Details</summary>
Motivation: There's a tension between the rhythmic nature of speech acoustics/cortical oscillations and the conventional view of speech movements as discrete, goal-oriented actions. The paper aims to resolve this by investigating whether rhythmicity exists at the articulatory movement level.

Method: The authors use an unconventional but principled representation for discrete movements that reveals limit cycle organization, allowing recovery of previously inaccessible rhythmic structure in speech motor activity.

Result: The study demonstrates a pervasive limit cycle organization in speech articulatory movements, uncovering rhythmic structure that was previously inaccessible using conventional movement representations.

Conclusion: The findings resolve the long-standing tension between biological rhythmicity and discreteness in speech by revealing rhythmic organization at the fundamental level of individual articulatory actions, connecting motor activity with acoustic and neural rhythmicity.
...
### [158] [Shared Multi-modal Embedding Space for Face-Voice Association](https://arxiv.org/abs/2512.04814)
*Christopher Simic,Korbinian Riedhammer,Tobias Bocklet*

Main category: cs.SD

TL;DR: First-place solution for FAME 2026 challenge using separate face/voice processing with age-gender features and AAM loss, achieving 23.99% EER.


<details>
  <summary>Details</summary>
Motivation: To address the challenging FAME 2026 tasks requiring face-voice association in multilingual settings, including zero-shot testing on unseen languages.

Method: Separate uni-modal pipelines for face and voice with general feature extraction, plus additional age-gender feature extraction. Features are projected into shared embedding space and trained with Adaptive Angular Margin (AAM) loss.

Result: Achieved first place in FAME 2026 challenge with average Equal-Error Rate (EER) of 23.99%.

Conclusion: The proposed approach effectively handles face-voice association in multilingual settings, including zero-shot language generalization, demonstrating strong performance in challenging cross-modal matching tasks.
...
### [159] [The changing surface of the world's roads](https://arxiv.org/abs/2512.04092)
...
TL;DR: First global multi-temporal dataset of road pavedness and width for 9.2M km of arterial roads using deep learning on satellite imagery, revealing development patterns and infrastructure vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: Lack of comprehensive global baseline of road surface information as a primary indicator of network functionality and resilience for sustainable development goals.

Method: Applied deep learning framework to global mosaic of Planetscope satellite imagery from 2020 and 2024 to create road pavedness and width dataset covering 9.2M km of critical arterial roads.

Result: Achieved 95.5% coverage where nearly half the network was previously unclassified; rate of change in pavedness correlates with HDI (0.65); created Humanitarian Passability Matrix; identified governance disparities in Ghana and climate vulnerabilities in Pakistan.

Conclusion: Provides foundational dataset and multi-scale analytical framework for monitoring global infrastructure, offering direct physical measurement of infrastructure at higher resolution than traditional proxies like nighttime lights.
...
### [160] [Retrieval-Augmented Few-Shot Prompting Versus Fine-Tuning for Code Vulnerability Detection](https://arxiv.org/abs/2512.04106)
*Fouad Trad,Ali Chehab*

Main category: cs.SE

TL;DR: Retrieval-augmented prompting outperforms standard few-shot and zero-shot approaches for code vulnerability detection, achieving 74.05% F1 score without fine-tuning costs, though fine-tuned CodeBERT performs best at 91.22% F1.


<details>
  <summary>Details</summary>
Motivation: Few-shot prompting effectiveness depends heavily on example selection, especially in complex domains like code vulnerability detection. The paper aims to improve few-shot performance through retrieval-augmented approaches.

Method: Systematic evaluation using Gemini-1.5-Flash across three approaches: (1) standard few-shot prompting with random examples, (2) retrieval-augmented prompting using semantically similar examples, and (3) retrieval-based labeling without model inference. Comparison against zero-shot prompting and fine-tuned models including Gemini, DistilBERT, DistilGPT2, and CodeBERT.

Result: Retrieval-augmented prompting consistently outperforms other prompting strategies, achieving 74.05% F1 score and 83.90% partial match accuracy at 20 shots. It beats zero-shot (36.35% F1) and fine-tuned Gemini (59.31% F1). Fine-tuned CodeBERT performs best (91.22% F1) but requires training resources.

Conclusion: Retrieval-augmented prompting offers a practical middle ground between zero-shot and fine-tuning for code vulnerability detection, providing substantial performance improvements over standard few-shot approaches without the costs of model fine-tuning.
...
### [161] [Can machines perform a qualitative data analysis? Reading the debate with Alan Turing](https://arxiv.org/abs/2512.04121)
*Stefano De Paoli*

Main category: cs.CY

TL;DR: The paper argues that critics of using LLMs for qualitative analysis are focusing on the wrong issues, and proposes reframing the debate around whether LLMs can produce analyses comparable to human analysts rather than whether machines can perform qualitative analysis in principle.


<details>
  <summary>Details</summary>
Motivation: To address the literature that rejects LLMs in qualitative data analysis and show why current critical debates are misguided, proposing a more productive research direction.

Method: Uses empirical evidence, critical reflections, and builds on Alan Turing's "Computing Machinery and Intelligence" to reframe the debate. Analyzes contrary views using Turing's writing and rhetorical style.

Result: The paper demonstrates that the focus should shift from questioning whether machines can perform qualitative analysis in principle to investigating whether LLMs can produce analyses sufficiently comparable to human analysts.

Conclusion: The debate on qualitative analysis with LLMs needs reframing: instead of asking whether machines can perform qualitative analysis, we should ask whether LLMs can produce analyses comparable to human analysts, using empirical investigation rather than philosophical objections.
...
### [162] [Balancing Safety and Helpfulness in Healthcare AI Assistants through Iterative Preference Alignment](https://arxiv.org/abs/2512.04210)
...
Conclusion: Importance of adopting best practices that balance patient safety, user trust, and clinical utility in conversational medical assistant design. Iterative alignment with domain-specific signals is crucial for safe healthcare LLM deployment.
...
### [163] [Towards Ethical Multi-Agent Systems of Large Language Models: A Mechanistic Interpretability Perspective](https://arxiv.org/abs/2512.04691)
*Jae Hee Lee,Anne Lauscher,Stefano V. Albrecht*

Main category: cs.AI

TL;DR: Position paper proposing a research agenda for ensuring ethical behavior in multi-agent LLM systems through mechanistic interpretability approaches.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly deployed as autonomous agents in multi-agent systems, which show promise for complex tasks but pose significant ethical challenges that need to be addressed.

Method: Proposes a research agenda focusing on three key areas: 1) comprehensive evaluation frameworks for ethical behavior at individual, interactional, and systemic levels; 2) mechanistic interpretability to understand emergent behaviors; 3) parameter-efficient alignment techniques to steer systems toward ethical behavior.

Result: The paper outlines a structured research agenda rather than presenting experimental results, identifying specific challenges and approaches for ensuring ethical behavior in multi-agent LLM systems.

Conclusion: A mechanistic interpretability approach is needed to understand and guide the ethical behavior of multi-agent LLM systems, requiring coordinated research across evaluation frameworks, interpretability methods, and alignment techniques.
...
### [164] [Are LLMs Truly Multilingual? Exploring Zero-Shot Multilingual Capability of LLMs for Information Retrieval: An Italian Healthcare Use Case](https://arxiv.org/abs/2512.04834)
*Vignesh Kumar Kembu,Pierandrea Morandini,Marta Bianca Maria Ranzini,Antonino Nocera*

Main category: cs.AI

TL;DR: Open-source multilingual LLMs struggle with zero-shot comorbidity extraction from Italian EHRs, showing performance variability and poor generalization compared to traditional methods.


<details>
  <summary>Details</summary>
Motivation: Information extraction from clinical records is crucial for digital healthcare, but traditional NLP techniques often fail due to the complexity and variability of clinical language. LLMs offer potential for better understanding of clinical text, but their effectiveness for multilingual EHR processing needs investigation.

Method: The paper explores open-source multilingual LLMs' ability to understand Italian Electronic Health Records (EHRs) and extract information in real-time. They conduct detailed experiments on comorbidity extraction from EHRs, testing models in zero-shot, on-premises settings and comparing performance against native pattern matching and manual annotations.

Result: Some LLMs struggle in zero-shot, on-premises settings, showing significant performance variation. The models have difficulty generalizing across various diseases when compared to native pattern matching and manual annotations.

Conclusion: While LLMs show promise for clinical text understanding, current open-source multilingual models face challenges with zero-shot comorbidity extraction from Italian EHRs, particularly in generalization across diseases and consistency compared to traditional methods.
...
Conclusion: STELLA successfully addresses limitations of existing LLM adaptations for time series forecasting by systematically mining and injecting structured supplementary and complementary information through semantic-temporal alignment, leading to improved forecasting performance and generalization.
...
### [166] [The AI Consumer Index (ACE)](https://arxiv.org/abs/2512.04921)
...
<details>
  <summary>Details</summary>
Motivation: There's a need to assess whether frontier AI models can effectively perform real-world consumer tasks that people value, as current benchmarks may not capture these practical applications.

Method: Created ACE with 400 hidden test cases across four consumer domains, plus 80 open-source devset cases. Evaluated 10 frontier models with websearch enabled using novel grading methodology that checks if responses are grounded in retrieved web sources.
...
### [167] [Algorithmic Thinking Theory](https://arxiv.org/abs/2512.04923)
*MohammadHossein Bateni,Vincent Cohen-Addad,Yuzhou Gu,Silvio Lattanzi,Simon Meierhans,Christopher Mohri*

Main category: cs.AI

TL;DR: The paper introduces a theoretical framework for analyzing reasoning algorithms in LLMs, formalizing principles behind iterative improvement and answer aggregation techniques.


<details>
  <summary>Details</summary>
Motivation: LLMs show improved capabilities through iterative reasoning, but there's a need for a theoretical framework to analyze these reasoning algorithms that doesn't depend on architectural specifics and is grounded in experimental evidence.

Method: The authors develop a theoretical framework that formalizes reasoning plans as algorithms using probabilistic oracles, focusing on principles underlying iterative improvement and answer aggregation techniques.

Result: The framework provides a general perspective for analyzing reasoning algorithms that may extend to current and future reasoning oracles, offering a foundation for designing more powerful reasoning methods.

Conclusion: This theoretical framework offers an architecture-agnostic way to understand and improve LLM reasoning capabilities through formal analysis of iterative reasoning algorithms.
...
### [168] [Network of Theseus (like the ship)](https://arxiv.org/abs/2512.04198)
*Vighnesh Subramaniam,Colin Conwell,Boris Katz,Andrei Barbu,Brian Cheung*

Main category: cs.LG

TL;DR: NoT (Network of Theseus) enables gradual architectural transformation of trained networks into different target architectures while preserving performance, decoupling optimization from deployment.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the standard assumption that neural network architectures must remain fixed from training through inference, which constrains architectural choices due to optimization difficulties. This limits exploration of architectures with desirable efficiency or design properties.

Method: NoT progressively converts a trained (or untrained) guide network into a different target architecture part-by-part. At each stage, components are incrementally replaced with target architecture modules and aligned using representational similarity metrics to preserve functionality.

Result: The method successfully preserves guide network performance even under substantial architectural changes (e.g., converting CNNs to MLPs, GPT-2 to RNNs). It demonstrates functionality preservation through architectural transformations.

Conclusion: By decoupling optimization from deployment, NoT expands viable inference-time architectures, enabling better accuracy-efficiency tradeoffs and more directed exploration of architectural design space beyond optimization constraints.
...
### [169] [Natural Language Actor-Critic: Scalable Off-Policy Learning in Language Space](https://arxiv.org/abs/2512.04601)
...
<details>
  <summary>Details</summary>
Motivation: Current policy gradient methods for training LLM agents suffer from instability and high sample complexity in long-horizon tasks with sparse rewards. The natural language action space makes exploration difficult, and trajectory-level rewards provide noisy training signals.

Method: Natural Language Actor-Critic (NLAC) uses a generative LLM critic that produces natural language explanations for why actions are suboptimal, rather than scalar values. This provides richer, more actionable feedback. The approach can be trained off-policy without policy gradients.

Result: NLAC demonstrates promising performance improvements over existing training approaches on reasoning, web browsing, and tool-use with dialogue tasks. It offers more scalable and stable training for LLM agents.

Conclusion: NLAC represents a significant advancement in LLM agent training by leveraging LLMs' natural language capabilities to provide better training signals, enabling more efficient and stable learning in complex, long-horizon tasks with large action spaces.
...
### [170] [MemLoRA: Distilling Expert Adapters for On-Device Memory Systems](https://arxiv.org/abs/2512.04763)
*Massimo Bini,Ondrej Bohdal,Umberto Michieli,Zeynep Akata,Mete Ozay,Taha Ceritli*

Main category: cs.LG

TL;DR: MemLoRA enables on-device memory-augmented conversations by equipping small language models with specialized memory adapters, with MemLoRA-V adding visual understanding capabilities.


<details>
  <summary>Details</summary>
Motivation: Current memory-augmented LLM systems are too costly for local deployment, and small models lack sufficient performance. Existing systems also lack native visual capabilities for multimodal contexts.

Method: MemLoRA uses specialized memory adapters trained via knowledge distillation for specific memory operations: knowledge extraction, memory update, and memory-augmented generation. MemLoRA-V integrates small vision-language models for visual understanding.

Result: MemLoRA outperforms 10x larger baseline models (Gemma2-27B) and matches 60x larger models (GPT-OSS-120B) on LoCoMo benchmark. MemLoRA-V achieves 81.3 accuracy vs 23.7 for caption-based approaches on visual tasks while maintaining text performance.

Conclusion: The proposed memory adapter approach enables effective on-device memory operations with small models, bridging the performance gap with large models while adding native visual capabilities for multimodal applications.
...
### [171] [CARL: Critical Action Focused Reinforcement Learning for Multi-Step Agent](https://arxiv.org/abs/2512.04949)
*Leyang Shen,Yang Zhang,Chun Kai Ling,Xiaoyan Zhao,Tat-Seng Chua*

Main category: cs.LG

TL;DR: CARL is a reinforcement learning algorithm that focuses training on critical actions in multi-step tasks, ignoring less important actions to improve efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Conventional RL algorithms treat all actions equally in multi-step tasks, but in reality only a small fraction of actions are critical to final outcomes. This leads to suboptimal training efficiency and performance.

Method: CARL identifies critical actions and provides focused optimization signals only for high-criticality actions, while excluding low-criticality actions from model updates. This creates action-level optimization tailored for multi-step agents.

Result: Extensive experiments show CARL achieves stronger performance and higher efficiency during both training and inference across diverse evaluation settings compared to conventional approaches.

Conclusion: Focusing training on critical actions rather than treating all actions equally leads to more effective and efficient reinforcement learning for multi-step agents, as demonstrated by the CARL algorithm.
...
TL;DR: LLM Chemistry framework applied to medication recommendation from clinical vignettes, using multi-LLM collaboration with interaction modeling to improve reliability, stability, and calibration.


<details>
  <summary>Details</summary>
Motivation: Healthcare AI needs reliable clinical decision support. Individual LLMs suffer from hallucinations and inconsistency, while naive ensembles lack stability and credibility. Need for trustworthy AI assistants in clinical practice.

Method: Apply LLM Chemistry framework (quantifies collaborative compatibility among LLMs) to medication recommendation. Use multi-LLM collaboration guided by Chemistry-inspired interaction modeling to create ensembles that are effective (exploit complementary strengths), stable (consistent quality), and calibrated (minimize interference and error amplification).

Result: Preliminary results are encouraging. Chemistry-based multi-LLM collaboration strategy evaluated on real-world clinical scenarios shows potential for generating credible, patient-specific medication recommendations.

Conclusion: LLM Chemistry-guided collaboration offers a promising path toward reliable and trustworthy AI assistants in clinical practice by improving ensemble reliability through interaction-aware modeling.
...
### [173] [Studying Various Activation Functions and Non-IID Data for Machine Learning Model Robustness](https://arxiv.org/abs/2512.04264)
*Long Dang,Thushari Hapuarachchi,Kaiqi Xiong,Jing Lin*

Main category: cs.LG

TL;DR: The paper studies ML model robustness using 10 different activation functions in adversarial training, proposes an advanced adversarial training method for centralized environments, and extends it to federated learning with data sharing to address non-IID data challenges.


<details>
  <summary>Details</summary>
Motivation: Most existing adversarial training studies focus on ReLU activation and centralized environments, leaving gaps in understanding how different activation functions affect robustness and how adversarial training performs in federated learning settings with IID/non-IID data.

Method: 1) Proposed advanced adversarial training with architecture changes, soft labeling, simplified data augmentation, and varying learning rates for centralized environments; 2) Evaluated 10 activation functions beyond ReLU; 3) Extended approach to federated learning with data sharing strategy for non-IID data.

Result: Centralized training achieved 77.08% natural and 67.96% robust accuracy on CIFAR-10 against FGSM attacks. ReLU generally performed best among activation functions. Federated learning showed significant robust accuracy drops, especially for non-IID data. With 40% data sharing, achieved 70.09% natural and 54.79% robust accuracy, surpassing CalFAT algorithm.

Conclusion: Proper data sharing can significantly improve ML model robustness in federated learning with non-IID data, making adversarial training more practical for real-world applications. ReLU remains the most effective activation function for adversarial robustness in most cases.
...
### [174] [STeP-Diff: Spatio-Temporal Physics-Informed Diffusion Models for Mobile Fine-Grained Pollution Forecasting](https://arxiv.org/abs/2512.04385)
*Nan Zhou,Weijie Hong,Huandong Wang,Jianfeng Zheng,Qiuhua Wang,Yali Song,Xiao-Ping Zhang,Yong Li,Xinlei Chen*

Main category: cs.LG

TL;DR: STeP-Diff: A physics-informed diffusion model for fine-grained air pollution forecasting using incomplete mobile sensor data from non-dedicated platforms like cars and buses.
...
Method: Proposes Spatio-Temporal Physics-Informed Diffusion Models (STeP-Diff) that leverages DeepONet to model spatial measurements and a PDE-informed diffusion model for spatio-temporal forecasting. Uses PDE-constrained regularization to ensure denoising converges to convection-diffusion dynamics, aligning predictions with real measurements and pollution dispersion physics.

Result: Deployed 59 portable sensing devices in two cities for 14 days. Achieved improvements of up to 89.12% in MAE, 82.30% in RMSE, and 25.00% in MAPE compared to second-best algorithm. Extensive evaluations show STeP-Diff effectively captures spatio-temporal dependencies in air pollution fields.

Conclusion: STeP-Diff successfully addresses the challenge of forecasting air pollution from incomplete mobile sensor data by integrating physics-informed diffusion models, demonstrating superior performance in capturing spatio-temporal dependencies while being grounded in real measurements and fundamental pollution dispersion physics.
...
### [175] [Feature Engineering vs. Deep Learning for Automated Coin Grading: A Comparative Study on Saint-Gaudens Double Eagles](https://arxiv.org/abs/2512.04464)
*Tanmay Dogra,Eric Ngo,Mohammad Alam,Jean-Paul Talavera,Asim Dahal*

Main category: cs.LG

TL;DR: Feature-based ANN with expert-designed features outperforms CNN and SVM for grading rare coins with limited data, challenging deep learning supremacy in niche domains.


<details>
  <summary>Details</summary>
Motivation: Challenge the assumption that deep learning always beats traditional methods, especially in niche domains with limited data and imbalanced classes like rare coin grading.

Method: Compared three approaches: 1) Feature-based ANN with 192 custom features from Sobel edge detection and HSV color analysis, 2) Hybrid CNN combining EfficientNetV2, 3) SVM as baseline control. Tested on 1,785 expert-graded Saint-Gaudens Double Eagle gold coins.

Result: ANN achieved 86% exact grade matches and 98% within 3-grade tolerance. CNN and SVM performed poorly (31% and 30% exact matches), mostly guessing the most common grade. CNN's decent tolerance metrics masked its failure at specific grade prediction.

Conclusion: With limited data (<2,000 samples) and imbalanced classes, expert knowledge embedded in feature engineering outperforms black-box deep learning approaches. This applies to other niche quality inspection tasks where domain expertise matters more than computational power.
...
<details>
  <summary>Details</summary>
Motivation: While DKD revived logit-based distillation with decoupling strategies, its mechanisms need deeper exploration. The authors aim to rethink DKD from a predictive distribution perspective to uncover overlooked insights and improve distillation effectiveness.

Method: Proposes Generalized Decoupled Knowledge Distillation (GDKD) with enhanced logit decoupling, analyzes teacher predictive distribution's gradient impact, identifies key insights about non-top logit relationships, and develops an efficient partition strategy for multimodal teacher distributions.

Result: GDKD outperforms original DKD and other state-of-the-art knowledge distillation methods across CIFAR-100, ImageNet, Tiny-ImageNet, CUB-200-2011, and Cityscapes benchmarks.

Conclusion: GDKD provides a more effective logit-based distillation approach by better understanding and leveraging teacher predictive distributions, offering superior performance while maintaining the simplicity of logit-based methods.
...
### [177] [TV2TV: A Unified Framework for Interleaved Language and Video Generation](https://arxiv.org/abs/2512.05103)
...
TL;DR: TV2TV is a video generation model that interleaves text and video generation, allowing the model to "think in words" before "acting in pixels" to improve visual quality and controllability.


<details>
  <summary>Details</summary>
Motivation: Current video generation models struggle with complex outputs requiring semantic branching and high-level reasoning about what should happen next. The paper aims to address these limitations by integrating language model reasoning capabilities into video generation.

Method: TV2TV uses a unified generative modeling framework that decomposes video generation into interleaved text and video generation. It employs a Mixture-of-Transformers (MoT) architecture to jointly learn language modeling (next-token prediction) and video flow matching (next-frame prediction). At inference, the model dynamically decides when to alternate between generating text and video frames.

Result: On video game data, TV2TV shows substantial improvements in visual quality and controllability. It also scales to natural videos (sports videos augmented with action descriptions), demonstrating strong visual quality and prompt alignment for complex real-world action sequences.

Conclusion: TV2TV represents a promising step toward video generation with open-ended textual reasoning and control, enabling fine-grained controllability through text interventions at any point in the generation process.
...
### [178] [Deep infant brain segmentation from multi-contrast MRI](https://arxiv.org/abs/2512.05114)
...
Method: Uses domain randomization techniques that synthesize training images beyond realistic bounds to promote dataset shift invariance. Includes a mechanism for flexible pooling and interaction of features from any number of input scans. Supports diverse MRI protocols including repeat scans and image types unavailable during training.

Result: Achieves state-of-the-art performance matching or exceeding accuracy of several existing methods across various age cohorts and input configurations using a single model, with significantly faster runtime than many existing tools.

Conclusion: BabySeg addresses method fragmentation in pediatric brain MRI segmentation by providing a unified framework that handles diverse clinical scenarios with superior accuracy and efficiency.
...
TL;DR: VGG-Flow is a gradient-matching method for finetuning flow matching models that achieves both efficient adaptation and prior preservation using optimal control theory.
...
Method: Leverages optimal control theory to propose VGG-Flow, where the optimal difference between finetuned and pretrained velocity fields is matched with the gradient field of a value function, incorporating first-order reward information and heuristic initialization for fast adaptation.

Result: Empirically demonstrated on Stable Diffusion 3 that VGG-Flow can finetune flow matching models under limited computational budgets while achieving effective and prior-preserving alignment.

Conclusion: VGG-Flow provides a novel gradient-matching approach that successfully balances adaptation efficiency with prior preservation for flow matching model alignment.
...
TL;DR: Deep neural networks across diverse tasks converge to remarkably similar low-dimensional parametric subspaces, revealing universal spectral structures that could enable more efficient training and inference.


<details>
  <summary>Details</summary>
Motivation: To understand the intrinsic organization of information within deep networks and investigate whether neural networks systematically converge to shared spectral subspaces regardless of initialization, task, or domain.

Method: Conducted mode-wise spectral analysis of over 1100 models (500 Mistral-7B LoRAs, 500 Vision Transformers, and 50 LLaMA-8B models) using spectral decomposition techniques on weight matrices across various architectures trained on diverse tasks and datasets.

Result: Identified universal subspaces capturing majority variance in just a few principal directions, with sparse, joint subspaces consistently exploited within shared architectures across diverse tasks and datasets.

Conclusion: The findings reveal intrinsic universal structure in neural networks with significant implications for model reusability, multi-task learning, model merging, and development of training/inference-efficient algorithms, potentially reducing computational costs and carbon footprint.
...
TL;DR: A differentiable kernel decomposition framework that approximates complex, spatially-variant image convolution kernels using sparse samples, enabling efficient processing on resource-limited devices.


<details>
  <summary>Details</summary>
Motivation: Direct dense convolution with complex kernels is computationally prohibitive on resource-limited devices, and existing approximations either lack efficiency or fail to capture non-convex kernel shapes.

Method: Differentiable kernel decomposition framework that represents target kernels using sparse kernel samples, featuring: (1) differentiable optimization of sparse kernels, (2) dedicated initialization for non-convex shapes to avoid poor local minima, and (3) kernel-space interpolation for spatially varying filtering without retraining.

Result: Achieves higher fidelity than simulated annealing and significantly lower computational cost than low-rank decompositions for both Gaussian and non-convex kernels.
...
TL;DR: The paper introduces contextual sensitive data detection mechanisms using LLMs that consider dataset context to improve accuracy over traditional methods focused only on personal data.


<details>
  <summary>Details</summary>
Motivation: Current sensitive data detection focuses too narrowly on personal data and lacks consideration of contextual factors. As open data portals grow, there's a need for more nuanced approaches that recognize data sensitivity depends on context rather than just data type.

Method: Two contextualization mechanisms: 1) Type contextualization - detects semantic types of data values and considers their overall context within datasets/documents; 2) Domain contextualization - determines sensitivity based on retrieved rules from documents specifying data sensitivity (considering data topic, geographic origin). Both use LLMs for implementation.
...
Conclusion: Contextual sensitive data detection using LLMs significantly improves accuracy over traditional methods, especially for complex domains. The approach addresses the need for broader sensitivity definitions and practical tools for data protection in open data environments.
...
TL;DR: MAMA framework quantifies how network topology affects PII leakage in multi-agent LLM systems, finding fully connected graphs leak most while chains protect best, with actionable design recommendations.


<details>
  <summary>Details</summary>
Motivation: Graph topology fundamentally determines memory leakage in multi-agent LLM systems, but its effects remain poorly quantified, creating a need for systematic measurement and understanding of how network structure shapes privacy risks.

Method: MAMA (Multi-Agent Memory Attack) framework uses synthetic documents with labeled PII to generate sanitized task instructions. Two-phase protocol: Engram (seeding private info into target agent's memory) and Resonance (multi-round interaction where attacker attempts extraction). Measures leakage as fraction of ground-truth PII recovered via exact matching over up to 10 rounds across six network topologies with varying agent counts, attacker placements, and base models.

Result: Fully connected graphs exhibit maximum leakage while chains provide strongest protection; shorter attacker-target graph distance and higher target centrality increase vulnerability; leakage rises sharply in early rounds before plateauing; model choice affects absolute rates but preserves topology rankings; temporal/locational PII leaks more readily than identity credentials or regulated identifiers.

Conclusion: First systematic mapping from architectural choices to measurable privacy risk yields actionable guidance: prefer sparse/hierarchical connectivity, maximize attacker-target separation, limit node degree and network radius, avoid shortcuts bypassing hubs, and implement topology-aware access controls.
