...
Motivation: Existing benchmarks for medical LLMs focus on single-turn QA, lacking evaluation of multi-turn patient-clinician conversations and comprehensive assessment of medical processes, safety, outcomes, and communication skills.

Method: Five-layer framework: (1) synthetic EHR-like patient packets, (2) AI Patient with memory/affect, (3) task matrix (encounter reasons × objectives), (4) 105-dimension evaluation aligned with ACGME competencies, (5) calibrated AI Judges providing scores and rationales.

Result: Evaluation of 9 flagship models across 366 AI Patients and 7,097 conversations showed low performance across dimensions, particularly poor on differential diagnosis, despite using standardized clinician prompts.

Conclusion: Current LLMs perform poorly in medical dialogue tasks, highlighting the need for improvement before clinical use; MedPI provides a framework to guide future development of medical AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing RAG evaluation metrics collapse heterogeneous behaviors into single scores and provide little insight into whether errors arise from retrieval, reasoning, or grounding, making it difficult to diagnose system failures.

Method: RAGVUE decomposes RAG behavior into four diagnostic components: retrieval quality, answer relevance and completeness, strict claim-level faithfulness, and judge calibration. Each metric includes structured explanations. The framework supports both manual metric selection and fully automated agentic evaluation, with Python API, CLI, and Streamlit interface.

Result: In comparative experiments, RAGVUE surfaces fine-grained failures that existing tools like RAGAS often overlook. The framework successfully identifies specific weaknesses in RAG pipelines that single-score metrics miss.

Conclusion: RAGVUE provides a transparent, explainable evaluation framework for RAG systems that enables better diagnosis of failures and can be integrated into both research pipelines and practical RAG development.
...
TL;DR: Unsupervised method to build Chinese verb collostruction database for LLM complementation, using graph-based definitions and clustering from corpus data.


<details>
  <summary>Details</summary>
Motivation: To provide explicit, interpretable rules for Chinese verb usage to complement LLMs in scenarios requiring explanation and interpretability, addressing the black-box nature of LLMs.
...
Result: Generated collostructions show functional independence and graded typicality; evaluation on verb grammatical error correction shows maximum matching with collostructions outperforms LLMs.

Conclusion: The unsupervised collostruction database effectively complements LLMs by providing interpretable rules, demonstrating practical utility in grammatical error correction tasks.
...
<details>
  <summary>Details</summary>
Motivation: Obtaining high-quality labeled datasets for e-commerce product information extraction is challenging, especially in low-resource scenarios where real annotated data is scarce.

Method: Systematic approach using LLMs with controlled modification framework: attribute-preserving modification, controlled negative example generation, and systematic attribute removal. Uses state-of-the-art LLM with attribute-aware prompts to enforce store constraints while maintaining product coherence.

Result: Human evaluation shows 99.6% naturalness, 96.5% valid attribute values, and over 90% consistent attribute usage. On MAVE dataset: synthetic data achieves 60.5% accuracy (vs 60.8% real data, 13.4% zero-shot baseline). Hybrid synthetic+real data reaches 68.8% accuracy.

Conclusion: The framework provides a practical solution for augmenting e-commerce datasets, particularly valuable for low-resource scenarios, with synthetic data performing on par with real training data and hybrid approaches offering further improvements.
...
TL;DR: A participatory protocol called Collective Narrative Grounding transforms community stories into structured narrative units for AI systems to address LLMs' failures on community-specific queries, reducing epistemic injustice through local knowledge integration.


<details>
  <summary>Details</summary>
Motivation: LLMs often fail on community-specific queries, creating "knowledge blind spots" that marginalize local voices and reinforce epistemic injustice. Current systems lack local grounding for community knowledge.

Method: Developed Collective Narrative Grounding protocol through participatory workshops (N=24). Created elicitation methods and schema for narrative units with entity/time/place extraction, validation, and provenance control. Audited county-level benchmark of 14,782 QA pairs to scope the problem.

Result: Found 76.7% of errors due to factual gaps, cultural misunderstandings, geographic confusions, and temporal misalignments. State-of-the-art LLM answered <21% of participatory questions correctly without added context. Missing facts often appear in collected narratives, suggesting path to closing dominant error modes.

Conclusion: The taxonomy, protocol, and participatory evaluation provide foundation for community-grounded AI. Key design tensions include representation/power, governance/control, and privacy/consent. Need retrieval-first, provenance-visible, locally governed QA systems to better answer local questions.
...
TL;DR: TeleTables benchmark evaluates LLMs' knowledge and interpretation of tables in telecom standards (3GPP), revealing limitations in both recall and table reasoning abilities.


<details>
  <summary>Details</summary>
Motivation: LLMs perform poorly on telecom standards despite industry interest, likely due to their limited ability to handle tables that contain essential technical information in 3GPP specifications.

Method: Created TeleTables benchmark using multi-stage data generation pipeline: extracted tables from 3GPP standards, used multimodal and reasoning-oriented LLMs to generate and validate questions, resulting in 500 human-verified QA pairs with tables in multiple formats.

Result: Smaller models (<10B parameters) struggle with both 3GPP knowledge recall and table interpretation; larger models show stronger table reasoning; overall performance highlights limited exposure to telecom standards in pretraining.

Conclusion: Domain-specialized fine-tuning is needed for LLMs to reliably interpret and reason over telecom standards, as current models lack sufficient inductive biases for complex technical material.
...
<details>
  <summary>Details</summary>
Motivation: Front-end development relies heavily on visual artifacts like sketches and mockups to convey design intent, but the role of these multi-modal elements in conversational code generation remains largely unexplored. Current benchmarks don't adequately capture the real-world interaction dynamics of front-end development where developers receive both textual and visual feedback.

Method: Created FronTalk benchmark with 100 multi-turn dialogues from real-world websites across diverse domains (news, finance, art). Each turn includes both textual instruction and equivalent visual instruction representing the same user intent. Developed a novel agent-based evaluation framework using a web agent to simulate users and explore websites, measuring both functional correctness and user experience.
...
Conclusion: FronTalk provides a solid foundation for future research in front-end development and multi-turn, multi-modal code generation. The benchmark highlights critical challenges in conversational code generation and offers a practical solution for the forgetting problem through autonomous web agent critique.
...
TL;DR: A novel dynamic remasking strategy for diffusion language models that adapts confidence thresholds per token per timestep based on temporal variance and spatial deviance, achieving up to 8.9× speedup while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Current diffusion language models use fixed global confidence thresholds for remasking, which overlooks individual token dynamics, leading to redundant iterations and constrained parallelism. The paper aims to address these inefficiencies by developing a more adaptive approach.

Method: Proposes a dynamic remasking approach that detects Temporal Variance (convergence status) and Spatial Deviance (inter-token correlations) for each token. Uses these signals to adaptively adjust confidence thresholds for every token at every denoising step.

Result: Significantly improves DLM operational efficiency across mainstream datasets, achieving speedups of up to 8.9 times while faithfully preserving generation quality compared to fixed-threshold approaches.

Conclusion: Dynamic, token-level threshold adjustment based on temporal and spatial dynamics is superior to fixed global thresholds, enabling substantial efficiency gains without compromising output quality in diffusion language models.
...
TL;DR: AI system combining fine-tuned language model with RAG for university admissions inquiries, addressing response time and accuracy challenges.
...
Method: Proposed hybrid AI system integrating fine-tuned language model with Retrieval-Augmented Generation (RAG). Fine-tuned model on curated admissions-specific dataset to enhance interpretation of RAG-retrieved data. Explored optimization strategies for response generation logic to balance quality and speed.

Result: The approach leverages RAG's ability to access up-to-date information and fine-tuning's capacity to embed nuanced domain understanding, aiming for consistently high-quality outputs meeting admissions communication requirements.

Conclusion: The hybrid AI system addresses limitations of standalone RAG in complex domains like university admissions by combining fine-tuning for domain expertise with RAG for information retrieval, optimizing both response quality and efficiency.
...
TL;DR: LLMs have internal political ideology structures that don't fully match human ideological space. The paper introduces a lightweight linear probe to measure and minimally correct this misalignment by adjusting output probabilities without retraining.


<details>
  <summary>Details</summary>
Motivation: LLMs organize political ideology internally, but this organization is systematically misaligned with human ideological space. This misalignment is model-specific and measurable, creating a need for practical alignment methods that don't require expensive retraining.

Method: Introduces a lightweight linear probe that quantifies ideological misalignment and minimally corrects the output layer. Calculates a bias score from internal features and directly adjusts final output probabilities without retraining the model.

Result: The method provides a practical, low-cost solution for aligning models with specific user opinions while preserving the original reasoning power of the model. It enables systematic measurement and correction of ideological misalignment.

Conclusion: LLMs' internal political ideology structures are partially misaligned with human space, but this can be systematically measured and minimally corrected using a lightweight linear probe approach that maintains model capabilities while improving alignment.
...
TL;DR: LEXMA is a reinforcement learning framework that fine-tunes LLMs to generate narrative explanations for AI decisions that are both accurate and tailored to different audiences (experts vs. consumers), without needing human-labeled explanation data.


<details>
  <summary>Details</summary>
Motivation: Current explainable AI methods use numerical feature attributions that lack coherent narratives. LLMs can generate natural language explanations but face challenges: ensuring explanations are both decision-correct and faithful to prediction factors, serving multiple audiences without changing decision rules, and training efficiently without large human-scored explanation datasets.

Method: LEXMA uses reinforcement learning with reflection-augmented supervised fine-tuning and two stages of Group Relative Policy Optimization (GRPO). It fine-tunes separate parameter sets to improve decision correctness and meet stylistic requirements for different audiences, using reward signals that don't require human-annotated explanations.
...
TL;DR: A RAG system for recommending research collaborators using PubMed data, deployed locally with PubMedBERT embeddings and LLaMA3 to meet hospital privacy requirements.


<details>
  <summary>Details</summary>
Motivation: LLMs are valuable in medical settings but hospital privacy regulations require local data processing, creating a need for privacy-compliant systems that can support biomedical knowledge discovery.

Method: Developed a retrieval-augmented generation (RAG) system using PubMedBERT for domain-specific embeddings and locally deployed LLaMA3 for generative synthesis, designed to recommend research collaborators based on PubMed publications from institution members.

Result: Demonstrated feasibility and utility of integrating domain-specialized encoders with lightweight LLMs for biomedical knowledge discovery under local deployment constraints.

Conclusion: The system successfully addresses privacy requirements while providing valuable research collaboration support, showing that specialized encoders combined with lightweight LLMs can effectively support biomedical workflows within local infrastructure constraints.
...
TL;DR: CARD is a framework that predicts problem complexity before generation and adapts decomposition strategies accordingly, achieving higher accuracy with significantly fewer tokens on reasoning benchmarks.
...
Method: CARD uses MRCE (Multi-dimensional Reasoning Complexity Estimator), a 0.6B Qwen model that predicts 30 fine-grained features from question text, followed by a two-stage recursive solver: hierarchical decomposition into K steps based on task profile and per-step thought budget allocation via recursive MRCE profiling.
...
<details>
  <summary>Details</summary>
Motivation: Address the need for automated content-safety assessment of Russian-language screenplays according to Federal Law No. 436-FZ, solving real editorial challenges in the Russian media industry with practical production workflow solutions.

Method: End-to-end system that processes full-length scripts (up to 700 pages), segments them into narrative units, detects content violations across five categories (violence, sexual content, profanity, substances, frightening elements), using fine-tuned Phi-3-mini model with 4-bit quantization, deployed on Yandex Cloud with CUDA acceleration.

Result: Achieved 80% rating accuracy and 80-95% segmentation precision (format-dependent), processing scripts in under 2 minutes for 700 pages, developed under constraints of no external API calls, 80GB VRAM limit, and <5 minute processing time.

Conclusion: Qwerty AI demonstrates practical applicability for production workflows in the Russian media industry, successfully addressing editorial challenges during the Wink hackathon with a deployable solution that meets legal compliance requirements.
...
<details>
  <summary>Details</summary>
Motivation: LLMs produce high-quality text but suffer from hallucinations, making them risky for security-critical applications. Small LLMs need improvement in faithfulness for reliable deployment.

Method: End-to-end framework with preference optimization paradigm. Includes data generation module for controlled hallucination injection to create synthetic preference data for training.

Result: Framework enhances faithfulness of small LLMs for text summarization. Provides insights on how data quality and model size affect preference-based optimization effectiveness.

Conclusion: TrueBrief offers a practical approach to improve LLM faithfulness for security-critical domains, demonstrating when preference optimization methods work best.
...
Result: Available at https://animatedllm.github.io as both a teaching aid and self-educational tool, providing accessible visualizations of Transformer language model internals.

Conclusion: AnimatedLLM addresses the gap in educational materials by making LLM mechanics transparent and accessible through interactive visualizations.
...
TL;DR: BiForget is an automated framework for synthesizing high-quality forget sets for LLM unlearning by exploiting the target model itself through seed-guided and adversarial prompting, achieving better relevance, diversity, and efficiency than existing methods.
...
Method: BiForget formalizes two unlearning granularities (domain-level and instance-level) and uses seed-guided and adversarial prompting to exploit the target model itself to generate forget sets that match its internal knowledge distribution, rather than relying on external generators.
...
Conclusion: BiForget facilitates more robust forgetting and better utility preservation, providing a more rigorous foundation for evaluating LLM unlearning by generating high-quality forget sets that better match the model's actual knowledge distribution.
...
<details>
  <summary>Details</summary>
Motivation: Scientific papers often overstate claims beyond what their results support, sidelining scientific rigor in favor of bold statements. This undermines transparent scientific communication.

Method: Two-stage framework: 1) Fine-tuned reranker for evidence retrieval from paper bodies, 2) Fine-tuned model to predict overstatement scores with justification. Uses dataset of 10K+ claim-evidence sets from ICLR/NeurIPS papers annotated by 8 LLMs, calibrated with peer-review comments and human validation.

Result: RIGOURATE enables improved evidence retrieval and overstatement detection compared to strong baselines, operationalizing evidential proportionality.

Conclusion: The framework supports clearer, more transparent scientific communication by systematically identifying and scoring claim overstatement in research papers.
...
TL;DR: Empirical study shows cross-lingual ASR transfer works better with phylogenetically closer languages, but dialectal data often outperforms despite smaller size. Garhwali case study reveals model biases toward pre-training languages.


<details>
  <summary>Details</summary>
Motivation: To understand cross-lingual transfer in ASR for noisy, code-mixed Indic dialects and language varieties, examining how phylogenetic distance affects performance and whether dialectal data can compensate for resource limitations.

Method: Empirical study across wide range of Indic dialects using spontaneous, noisy, code-mixed speech. Includes case study on low-resource Garhwali language variety, evaluation of multiple contemporary ASR models, and analysis of transcription errors to examine biases.

Result: ASR performance improves with reduced phylogenetic distance between languages, but this factor alone doesn't fully explain dialectal performance. Fine-tuning on smaller dialectal data often yields comparable performance to larger amounts of related high-resource standardized languages. Transcription error analysis reveals bias toward pre-training languages.

Conclusion: Phylogenetic distance is important but insufficient for explaining ASR performance on dialects. Dialectal data, even in smaller amounts, can be highly effective. ASR systems show biases toward pre-training languages, highlighting challenges for non-standardized speech.
...
TL;DR: Disco-RAG is a discourse-aware RAG framework that uses discourse trees and rhetorical graphs to inject structural cues into generation, achieving SOTA results on QA and summarization without fine-tuning.
...
Method: Constructs intra-chunk discourse trees to capture local hierarchies and builds inter-chunk rhetorical graphs to model cross-passage coherence. These structures are jointly integrated into a planning blueprint that conditions the generation.
...
TL;DR: MiJaBench reveals LLM safety alignment is not universal but forms a demographic hierarchy, with defense rates varying up to 33% based on target minority groups, and scaling worsens these disparities.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety evaluations create a dangerous illusion of universality by aggregating hate categories into scalar scores, masking systemic vulnerabilities against specific minority populations.
...
Result: Safety alignment is not a generalized semantic capability but a demographic hierarchy - defense rates fluctuate by up to 33% within the same model based solely on target group. Model scaling exacerbates these disparities.

Conclusion: Current alignment techniques don't create non-discrimination principles but reinforce memorized refusal boundaries only for specific groups, challenging current security scaling laws. Need granular demographic alignment research.
...
TL;DR: ARREST is a unified framework that regulates factual and safety misalignments in LLMs by identifying and correcting drifted features in latent activation space using an external network, without fine-tuning model parameters.
...
Method: Propose ARREST framework with external network trained to understand activation fluctuations. It selectively intervenes to regulate falsehoods into truthfulness and unsafe outputs into safe outputs, using both soft/hard refusals and factual corrections without fine-tuning model parameters.

Result: ARREST effectively regulates misalignment and is more versatile than RLHF-aligned models in generating soft refusals due to adversarial training. The framework demonstrates unified correction of both factual and safety issues.

Conclusion: Factual and safety failures in LLMs stem from representational misalignment in latent space, and an external intervention network can effectively regulate these issues without parameter fine-tuning, offering a unified approach to alignment.
...
<details>
  <summary>Details</summary>
Motivation: Neural networks are becoming increasingly capable but their internal decision-making processes remain poorly understood. This lack of understanding creates challenges for accountability in high-stakes applications, prevents studying how cognition emerges in digital systems, and misses opportunities to extract new knowledge from AI systems that outperform humans.

Method: The paper advocates for mechanistic interpretability as an approach to understand neural mechanisms. While the abstract doesn't specify particular methods, it positions mechanistic interpretability as the framework for analyzing how neural networks make decisions at a mechanistic level.

Result: The abstract presents the case for why mechanistic interpretability is important rather than reporting specific experimental results. It establishes three key benefits: enabling accountability and control in high-stakes domains, facilitating the study of digital brains and cognitive emergence, and allowing discovery of new knowledge from superhuman AI systems.

Conclusion: Mechanistic interpretability is essential for understanding neural networks' decision-making processes, which will enable responsible AI deployment, advance our understanding of digital cognition, and unlock new knowledge discovery from advanced AI systems.
...
TL;DR: The paper introduces Gavel-Ref, a reference-based evaluation framework for multi-document legal case summarization, and Gavel-Agent, an efficient agent scaffold that reduces token usage by 36% with minimal performance drop.


<details>
  <summary>Details</summary>
Motivation: While LLMs now support contexts up to 1M tokens, their effectiveness on complex long-context tasks like multi-document legal case summarization (100K-500K tokens) remains unclear. Existing evaluations provide only single aggregate scores, lacking systematic analysis of model performance across different aspects of legal summarization.
...
Result: Even the strongest model (Gemini 2.5 Pro) achieved only ~50% on SGavel-Ref, showing task difficulty. Models performed well on simple checklist items but struggled on multi-value or rare items like settlements and monitor reports. Gavel-Agent with Qwen3 reduced token usage by 36% with only 7% drop in Schecklist compared to end-to-end extraction with GPT-4.1.

Conclusion: Multi-document legal case summarization remains challenging for current LLMs despite large context windows. The proposed Gavel-Ref framework enables detailed evaluation beyond aggregate scores, while Gavel-Agent provides an efficient approach for checklist extraction with reduced computational cost, addressing limitations as LLMs potentially surpass human-written references.
...
TL;DR: LLMs often fail to challenge users' harmful beliefs due to excessive accommodation and insufficient epistemic vigilance. Pragmatic interventions like adding "wait a minute" can significantly improve safety performance.


<details>
  <summary>Details</summary>
Motivation: LLMs frequently fail to challenge harmful user beliefs in domains like medical advice and social reasoning. The authors argue these failures stem from LLMs defaulting to accommodating user assumptions and lacking epistemic vigilance.

Method: The study examines how social/linguistic factors (at-issueness, linguistic encoding, source reliability) affect accommodation in LLMs across three safety benchmarks (Cancer-Myth, SAGE-Eval, ELEPHANT). They test simple pragmatic interventions like adding "wait a minute" to prompt models to challenge harmful beliefs.

Result: Social and linguistic factors known to influence accommodation in humans similarly affect LLMs. Pragmatic interventions significantly improve performance on safety benchmarks while maintaining low false-positive rates.

Conclusion: Pragmatics is crucial for evaluating LLM behavior and improving safety. Simple interventions can help LLMs better challenge harmful beliefs by reducing excessive accommodation and increasing epistemic vigilance.
...
TL;DR: Optimizing for LLM judge rewards improves judge scores but reduces human-likeness, while maximizing log-probability of real human responses yields better human behavior prediction.


<details>
  <summary>Details</summary>
Motivation: To understand how to better model human thinking through next-turn dialogue prediction, examining whether allowing models to "think" before responding and how different learning objectives affect prediction quality.

Method: Compare learning approaches along two dimensions: (1) thinking before responding, (2) reward types (LLM-as-a-judge scoring vs. maximizing log-probability of ground truth). Use chain-of-thought as latent variable to derive lower bound on log-probability.

Result: Judge-based rewards increase judge scores but decrease ground truth likelihood and human win rates. Maximizing log-probability of human responses improves both log-probability and win rates. Chain-of-thought optimization yields best results across all evaluations.
...
<details>
  <summary>Details</summary>
Motivation: Instruction-tuned LLMs are vulnerable to backdoor attacks through poisoned training data, but defenses for these models remain underexplored despite growing security risks.
...
TL;DR: Users' stated preferences for proactive AI writing assistance don't match their actual behavior - they say urgency matters most but actually respond to compositional effort, creating a preference inversion that misleads system design.


<details>
  <summary>Details</summary>
Motivation: Proactive AI writing assistants need to predict when users want help, but there's no empirical understanding of what drives user preferences, creating a gap between what users say they want and what actually influences their behavior.

Method: Factorial vignette study with 50 participants making 750 pairwise comparisons to analyze decision drivers for proactive writing assistance.

Result: Compositional effort dominates decisions (ρ=0.597) while urgency shows no predictive power (ρ≈0). Users rank urgency first in self-reports despite it being weakest behavioral driver - a complete preference inversion. Systems using stated preferences achieve only 57.7% accuracy vs 61.3% using behavioral patterns.

Conclusion: Relying on user introspection for system design actively misleads optimization; behavioral patterns should guide proactive NLG systems rather than stated preferences.
...
TL;DR: ProMem introduces proactive memory extraction with recurrent feedback loops and self-questioning to address limitations of traditional summary-based memory extraction in LLM agents.


<details>
  <summary>Details</summary>
Motivation: Existing summary-based memory extraction methods have two major limitations: 1) summarization is "ahead-of-time" and misses important details because it doesn't know future tasks, and 2) extraction is "one-off" without feedback loops, leading to accumulation of information loss.
...
Conclusion: Proactive memory extraction with recurrent feedback mechanisms addresses fundamental limitations of traditional summary-based approaches, leading to more complete and accurate memory extraction for LLM agents.
...
TL;DR: Concept Tokens add a special token to frozen LLMs, learning its embedding from concept definitions to steer model behavior with minimal parameter updates.


<details>
  <summary>Details</summary>
Motivation: To create a lightweight method for controlling frozen LLMs using compact concept representations learned from definitions, enabling targeted behavior steering without full model fine-tuning.

Method: Add a new special token to pretrained LLM, learn only its embedding from multiple natural language definitions of a target concept (replacing concept mentions with the token), keep LLM frozen, optimize embedding with standard language modeling objective.

Result: 1) Reduces hallucinations in closed-book QA by increasing abstentions when negating token, increases hallucinations when asserting token; 2) Induces recasting feedback strategy with same directional effect; 3) Better preserves compliance with other instructions compared to in-context definitions; 4) Learned embeddings capture concept information but have limitations.

Conclusion: Concept Tokens provide a compact control signal learned from definitions that can effectively steer behavior in frozen LLMs, offering a lightweight alternative to full fine-tuning for targeted concept manipulation.
...
TL;DR: SampoNLP toolkit creates morphological lexicons for Uralic languages using MDL-inspired scoring, enabling systematic evaluation of BPE tokenizers and identification of optimal vocabulary sizes via a new IPS metric.


<details>
  <summary>Details</summary>
Motivation: There's a lack of clean morpheme lexicons for evaluating tokenizers in morphologically rich Uralic languages, making it difficult to assess tokenization quality for these low-resource settings.

Method: Developed SampoNLP toolkit using MDL-inspired Self-Referential Atomicity Scoring to filter composite forms and create high-purity morphological lexicons. Used these lexicons to systematically evaluate BPE tokenizers across vocabulary sizes (8k-256k) and proposed Integrated Performance Score (IPS) metric to balance morpheme coverage vs over-splitting.

Result: Generated high-purity lexicons for Finnish, Hungarian, and Estonian. Identified "elbow points" of diminishing returns in IPS curves, providing first empirically grounded recommendations for optimal vocabulary sizes in these languages. Demonstrated limitations of standard BPE for highly agglutinative languages.

Conclusion: SampoNLP enables corpus-free morphological lexicon creation for low-resource Uralic languages, provides practical guidance for tokenizer optimization, and quantitatively shows BPE's limitations for agglutinative languages. Toolkit and resources are publicly available.
...
TL;DR: WESR-Bench introduces a refined taxonomy for 21 vocal events, an expert-annotated evaluation set with position-aware protocol, and strong baseline models for precise non-verbal event localization in speech.
...
Method: Developed refined taxonomy of 21 vocal events with discrete vs. continuous categorization; created WESR-Bench expert-annotated evaluation set (900+ utterances) with position-aware protocol; built 1,700+ hour corpus and trained specialized models.

Result: The specialized models surpass both open-source audio-language models and commercial APIs while preserving ASR quality, providing a strong baseline for non-verbal event detection.

Conclusion: WESR-Bench serves as a foundational resource for future research in modeling rich, real-world auditory scenes, addressing critical gaps in non-verbal vocal event localization.
...
Result: Evaluated in simulated courtroom proceedings and debates, with human expert assessments showing significant gains in communication efficiency.

Conclusion: The LinguaGame framework successfully improves communication efficiency in multi-agent dialogues by treating communication as intentional and strategic, requiring agents to infer others' intents and strategies through linguistically informed reasoning.
...
TL;DR: GRACE is a reinforcement learning framework that addresses two key flaws in RAG systems: providing correct answers without evidence and fabricating responses when context is insufficient, using automated data construction and multi-stage rewards.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems suffer from two critical issues: (1) giving correct answers without explicit evidence grounding, and (2) producing fabricated responses when retrieved context is insufficient. While prior work addressed these separately, there's no unified framework integrating evidence-based grounding with reliable abstention.

Method: GRACE uses reinforcement learning with: (1) automated data construction using heterogeneous retrievers to generate diverse training samples without manual annotation, and (2) a multi-stage gated reward function that trains the model to assess evidence sufficiency, extract key supporting evidence, and either provide answers or explicitly abstain.
...
Conclusion: GRACE provides an effective unified framework that simultaneously addresses both evidence grounding and reliable abstention in RAG systems through reinforcement learning with automated data construction, achieving strong performance with significantly reduced annotation costs.
...
TL;DR: First systematic evaluation of text watermarking methods (KGW, EXP, Waterfall) for Bangla LLM generation shows they work well under benign conditions but collapse under cross-lingual round-trip translation attacks. Proposed layered watermarking combining embedding-time and post-generation watermarks improves post-attack detection by 25-35%.


<details>
  <summary>Details</summary>
Motivation: As LLMs are increasingly deployed for text generation, watermarking is essential for authorship attribution, IP protection, and misuse detection. While existing methods work well for high-resource languages, their robustness in low-resource languages like Bangla remains underexplored, especially against cross-lingual attacks.

Method: Systematically evaluated three state-of-the-art watermarking methods (KGW, EXP, Waterfall) for Bangla LLM text generation. Tested under cross-lingual round-trip translation (RTT) attacks. Proposed a layered watermarking strategy that combines embedding-time and post-generation watermarks to improve robustness.

Result: Under benign conditions: KGW and EXP achieve >88% detection accuracy with negligible perplexity and ROUGE degradation. Under RTT attacks: Detection accuracy collapses to 9-13%, showing fundamental failure of token-level watermarking. Layered watermarking improves post-RTT detection accuracy by 25-35%, achieving 40-50% accuracy (3-4× relative improvement over single-layer methods) with controlled semantic degradation.

Conclusion: The study quantifies the robustness-quality trade-off in multilingual watermarking and establishes layered watermarking as a practical, training-free solution for low-resource languages like Bangla. Findings highlight vulnerability of existing methods to cross-lingual attacks and demonstrate effectiveness of multi-layer approach.
...
TL;DR: NeuronLLM is a novel framework for understanding LLM neurons at task level using functional antagonism principle, identifying both supportive and inhibitive neurons while mitigating fortuitous behaviors.


<details>
  <summary>Details</summary>
Motivation: Current methods for understanding LLM neurons are limited: they focus on specific abilities rather than task-level coordination, only identify supportive neurons while ignoring inhibitive ones, and suffer from misattribution due to LLMs answering questions correctly by chance rather than genuine understanding.

Method: NeuronLLM adopts the biological principle of functional antagonism, modeling task performance as jointly determined by "good neurons" (facilitate task completion) and "bad neurons" (inhibit it). It uses contrastive learning to holistically model both types of neurons and leverages augmented question sets to mitigate fortuitous behaviors in LLMs.

Result: Comprehensive experiments on LLMs of different sizes and families show NeuronLLM's superiority over existing methods across four NLP tasks, providing new insights into LLM functional organization.

Conclusion: NeuronLLM offers a more comprehensive framework for understanding LLM neurons at task level by considering both facilitative and inhibitive roles, addressing limitations of previous methods and providing better insights into LLM functional organization.
...
TL;DR: FeedEval is an LLM-based framework for evaluating LLM-generated essay feedback quality along specificity, helpfulness, and validity dimensions, improving downstream essay scoring and revision performance.
...
Method: Proposes FeedEval framework with dimension-specialized LLM evaluators trained on curated datasets to assess feedback quality along specificity, helpfulness, and validity dimensions, selecting high-quality feedback for downstream use.

Result: FeedEval aligns closely with human expert judgments, essay scoring models trained with FeedEval-filtered feedback achieve superior performance, and identified high-quality feedback leads to more effective essay revisions with small LLMs.

Conclusion: FeedEval effectively addresses the quality validation gap in LLM-generated essay feedback, improving both automated scoring and revision outcomes through pedagogically grounded evaluation.
...
TL;DR: RL-Text2Vis uses reinforcement learning with post-execution feedback to improve text-to-visualization generation, achieving better chart quality than GPT-4o and boosting code execution success from 78% to 97%.


<details>
  <summary>Details</summary>
Motivation: Current Text2Vis systems using LLMs produce charts that lack semantic alignment and clarity, with open-source models struggling even more. Supervised fine-tuning improves code executability but fails to enhance overall visualization quality since traditional SFT loss cannot capture post-execution feedback.

Method: Proposes RL-Text2Vis, a reinforcement learning framework built on Group Relative Policy Optimization (GRPO) with a novel multi-objective reward that jointly optimizes textual accuracy, code validity, and visualization quality using post-execution feedback.
...
Conclusion: GRPO is an effective strategy for structured, multimodal reasoning in visualization generation, establishing RL-Text2Vis as a superior approach for text-to-visualization tasks.
...
TL;DR: Model merging enables efficient creation of multi-capability LLMs for Thai language and finance applications, outperforming single specialized models while reducing training costs.


<details>
  <summary>Details</summary>
Motivation: Organizations need multi-capability LLMs for banking/finance but face trade-offs between deploying multiple specialized models (inefficient) vs training single comprehensive models (prohibitively expensive). Privacy/regulatory concerns also drive need for on-premise deployment of Thai-language capable models.

Method: Explores model merging as resource-efficient alternative. Two experiments: 1) Merge Qwen-8B with ThaiLLM-8B to enhance Thai capabilities; 2) Merge Qwen-8B with both ThaiLLM-8B and THaLLE-CFA-8B for combined general and financial capabilities.

Result: First merge shows uplift in M3 and M6 O-NET exams over base Qwen-8B. Second merge demonstrates further improvements across both general (M3/M6 O-NET) and financial domains (Flare-CFA, Thai-IC benchmarks).

Conclusion: Model merging is viable for efficiently creating high-performance, multi-capability LLMs, addressing the cost-performance trade-off while enabling Thai industry to leverage advanced language models on-premise.
...
<details>
  <summary>Details</summary>
Motivation: Current knowledge editing methods like ROME work well for single-hop facts but fail on multi-hop reasoning tasks requiring knowledge chaining, limiting their practical applicability.
...
Conclusion: Redundant Editing effectively addresses multi-hop reasoning limitations in knowledge editing, significantly improving performance while acknowledging remaining challenges in specificity and naturalness.
...
TL;DR: VLMs conflate description specificity with length, but these should be disentangled. Specificity is defined as how well a description distinguishes a target image from alternatives. A controlled dataset shows people prefer specific descriptions regardless of length, and length allocation matters.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models often conflate description specificity with length, assuming longer descriptions are more specific. The authors argue these concepts need to be disentangled, as descriptions can be concise yet informative or lengthy yet vacuous.

Method: Define specificity relative to a contrast set (how well a description picks out target image vs alternatives). Construct a dataset controlling for length while varying information content. Validate human preferences for specificity regardless of length.

Result: People reliably prefer more specific descriptions regardless of length. Controlling for length alone cannot account for specificity differences - how the length budget is allocated matters significantly.

Conclusion: Evaluation approaches should directly prioritize specificity over verbosity. The distinction between specificity and length is crucial for developing better vision-language models that produce truly informative descriptions.
...
TL;DR: Character-R1 is a framework that provides verifiable reward signals for role-playing agents to improve cognitive consistency and reduce out-of-character errors through structured internal cognition analysis.
...
Method: Character-R1 framework with three core designs: (1) Cognitive Focus Reward - explicit label-based analysis of 10 character elements (worldview, etc.) to structure internal cognition; (2) Reference-Guided Reward - overlap-based metrics with reference responses as optimization anchors; (3) Character-Conditioned Reward Normalization - adjusts reward distributions based on character categories for robust optimization across heterogeneous roles.
...
Conclusion: Character-R1 provides a comprehensive framework with verifiable reward signals that effectively addresses cognitive consistency issues in role-playing agents, enabling more authentic and consistent character portrayals across diverse roles.
...
TL;DR: CuCu framework uses national social studies curricula to create culture-specific QA pairs for LLM cultural alignment, demonstrated with Korean curriculum producing 34.1k QA pairs.


<details>
  <summary>Details</summary>
Motivation: LLMs show uneven performance across languages and cultures due to English-centric training data biases, requiring practical cultural alignment methods.
...
Result: Applied to Korean national social studies curriculum to create KCaQA dataset with 34.1k QA pairs covering culture-specific topics with locally-grounded responses.

Conclusion: National curricula provide scalable foundation for culture-aware supervision, enabling LLMs to better reflect local sociocultural contexts.
...
TL;DR: MAGA (Machine-Augment-Generated Text via Alignment) is a method to improve text detection by enhancing LLM-generated text alignment, which both attacks existing detectors to test robustness and improves detector generalization when used for fine-tuning.


<details>
  <summary>Details</summary>
Motivation: As LLM alignment improves, machine-generated text becomes harder to distinguish from human-written text, exacerbating abuse issues like fake news and fraud. Existing detectors have limited generalization that depends on dataset quality, and simply expanding MGT sources is insufficient - further augmentation of the generation process is needed.

Method: Proposes MAGA pipeline with comprehensive alignment from prompt construction to reasoning process. Key component is RLDF (Reinforced Learning from Detectors Feedback), systematically proposed to enhance text alignment. This approach not only attacks existing detectors to test robustness but also improves detector generalization when used for fine-tuning.

Result: RoBERTa detector fine-tuned on MAGA training set achieved average 4.60% improvement in generalization detection AUC. MAGA Dataset caused average 8.13% decrease in AUC of selected detectors, providing indicative significance for future research on detector generalization ability.

Conclusion: MAGA effectively enhances LLM-generated text alignment, serving dual purposes: attacking existing detectors to test robustness and improving detector generalization when used for fine-tuning. The approach addresses critical challenges in distinguishing machine-generated from human-written text.
...
TL;DR: SpeechMedAssist enables speech-based medical consultations using SpeechLMs with minimal speech data via two-stage training: text-based knowledge injection followed by speech modality alignment.


<details>
  <summary>Details</summary>
Motivation: Medical consultations are naturally speech-centric, but existing approaches rely on cumbersome text-based interactions. SpeechLMs offer more natural interaction but face challenges due to scarce medical speech data and inefficient fine-tuning methods.

Method: Proposes SpeechMedAssist with a two-stage training paradigm: 1) Knowledge & Capability Injection via Text (using text data), and 2) Modality Re-alignment with Limited Speech Data (using only 10k synthesized speech samples). This decouples training to reduce speech data requirements.

Result: The model outperforms all baselines in both effectiveness and robustness across single-turn question answering and multi-turn simulated interactions in medical consultation scenarios.

Conclusion: SpeechMedAssist successfully enables speech-based medical consultations with minimal speech data requirements, demonstrating superior performance in medical consultation scenarios compared to existing approaches.
...
TL;DR: CRANE is a relevance-based framework that identifies language-specific neurons through targeted interventions, revealing asymmetric neuron specialization patterns in multilingual LLMs.


<details>
  <summary>Details</summary>
Motivation: Current activation-based methods for identifying language-related neurons conflate language preference with functional importance, making it difficult to understand how language capabilities are truly organized at the neuron level in multilingual LLMs.

Method: CRANE uses relevance-based analysis with targeted neuron-level interventions to identify language-specific neurons based on their functional necessity for language-conditioned predictions, rather than activation magnitude.

Result: Neuron interventions reveal asymmetric patterns: masking target-language neurons degrades that language's performance while largely preserving other languages, showing language-selective but non-exclusive specializations. CRANE outperforms activation-based methods across English, Chinese, and Vietnamese benchmarks.

Conclusion: CRANE provides a more precise framework for understanding neuron-level language organization in multilingual LLMs by focusing on functional necessity rather than activation patterns, revealing nuanced language specialization patterns.
...
TL;DR: ToolGate: A framework providing logical safety guarantees and verifiable state evolution for LLM tool calling through formal contracts and symbolic state management.


<details>
  <summary>Details</summary>
Motivation: Existing LLM tool-augmented frameworks lack formal guarantees for logical safety and verifiability, relying heavily on natural language reasoning without ensuring that tool invocations and result commitments are logically sound.

Method: ToolGate maintains an explicit symbolic state space as a typed key-value mapping representing trusted world information. Each tool is formalized as a Hoare-style contract with preconditions (gating tool invocation) and postconditions (verifying results before state updates).
...
Conclusion: ToolGate establishes a foundation for building more trustworthy and debuggable AI systems that integrate language models with external tools by providing formal logical safety guarantees and verifiable state evolution.
...
Conclusion: The framework addresses the integrated challenges of detection, explanation, and intervention for hateful memes using generative AI, offering practical solutions for real-world moderation scenarios with limited data.
...
<details>
  <summary>Details</summary>
Motivation: Negation challenges LLMs, but Korean negation benchmarks are scarce, requiring better evaluation tools for Korean language understanding.

Method: Corpus analysis of Korean negation, creation of Thunder-KoNUBench benchmark reflecting empirical negation distribution, evaluation of 47 LLMs, and fine-tuning experiments.

Result: LLM performance degrades under negation; model size and instruction tuning affect performance; fine-tuning on Thunder-KoNUBench improves both negation understanding and broader Korean comprehension.

Conclusion: Thunder-KoNUBench addresses Korean negation evaluation gap, demonstrates LLMs' negation weaknesses, and shows targeted fine-tuning can enhance Korean language understanding.
...
TL;DR: PRISM is a training framework that combines process reward models with LLM self-certainty for stable unsupervised learning, addressing reliability issues in existing consistency-based methods.


<details>
  <summary>Details</summary>
Motivation: Current post-training methods for LLMs require costly human supervision or external verifiers. As LLMs improve, high-quality solutions to difficult problems become scarce for human annotation, making unsupervised learning increasingly important. Existing consistency-based methods (majority voting or internal confidence) are unreliable for large-scale, long-term training.

Method: PRISM (Process Reward Model) framework combines a Process Reward Model (PRM) with the model's internal self-certainty to guide learning without ground-truth labels. The PRM provides external process-based rewards while self-certainty provides internal confidence signals, creating a unified training approach.

Result: Effectively combining PRM with self-certainty leads to both stable training and better test-time performance while keeping the model's internal confidence in check. The framework addresses the unreliability of pure internal consistency metrics like entropy or self-certainty alone.

Conclusion: PRISM offers a promising approach for unsupervised learning of LLMs by synergistically combining external process-based rewards with internal confidence signals, enabling stable training and improved performance without human supervision.
...
<details>
  <summary>Details</summary>
Motivation: Fine-tuning LLMs faces memory bottlenecks during backpropagation. Zeroth-order optimization avoids backprop but suffers from high variance in gradient estimation due to random perturbations, leading to slow convergence.

Method: Uses prior-informed perturbations with a dynamically computed guiding vector from Gaussian samples to direct perturbations toward more informative directions. Also investigates greedy perturbation strategy to explore prior knowledge impact on gradient estimation.
...
Conclusion: Proposed method establishes robust balance between efficiency and accuracy, seamlessly integrates into existing optimization methods, and delivers faster convergence with superior performance for LLM fine-tuning.
...
TL;DR: First large-scale shared task for detecting hallucinations in Vietnamese LLMs, introducing ViHallu dataset with 10K annotated samples and achieving 84.80% macro-F1 score with best systems.


<details>
  <summary>Details</summary>
Motivation: LLM reliability is constrained by hallucinations, but low-to-medium resource languages like Vietnamese lack standardized evaluation frameworks for hallucination detection, creating a research gap.
...
Result: Best-performing system achieved 84.80% macro-F1 score, significantly outperforming baseline encoder-only score of 32.83%. Instruction-tuned LLMs with structured prompting and ensemble strategies performed best, though intrinsic (contradiction-based) hallucinations remained particularly challenging.
...
TL;DR: The paper introduces "Character Identity" as a two-layer construct (Parametric and Attributive Identity) to formalize character representation in Role-Playing Agents, revealing that fame-based advantages fade quickly in conversations while negative social traits remain a bottleneck for fidelity.


<details>
  <summary>Details</summary>
Motivation: Current Role-Playing Agents treat characters as arbitrary text inputs without formal structural dimensions, lacking systematic understanding of what constitutes character identity in LLM-based agents.

Method: Proposed Character Identity framework with Parametric Identity (pre-trained knowledge) and Attributive Identity (behavioral properties). Created unified character profile schema and generated both Famous and Synthetic characters under identical constraints. Evaluated across single-turn and multi-turn interactions.

Result: Two key findings: 1) "Fame Fades" - famous characters have initial advantage due to parametric knowledge but this disappears as models prioritize conversational context; 2) "Nature Remains" - models robustly portray general personality traits but performance is sensitive to morality and relationship valence, with negative social natures being the primary bottleneck.

Conclusion: Character identity should be formalized as multidimensional construct. Negative social traits are the main challenge for RPA fidelity, providing guidance for future character construction and evaluation methods.
...
<details>
  <summary>Details</summary>
Motivation: To create a unified multimodal search pipeline that can handle diverse modalities (text, images, documents, video) in a single representation space, building on the Qwen3-VL foundation model's capabilities.

Method: Two complementary model series: 1) Qwen3-VL-Embedding uses multi-stage training (contrastive pre-training to reranking distillation) with Matryoshka Representation Learning for flexible dimensions, handling 32k tokens. 2) Qwen3-VL-Reranker uses cross-encoder architecture with cross-attention for fine-grained relevance estimation. Both support 30+ languages and come in 2B and 8B parameter sizes.

Result: State-of-the-art performance on multimodal embedding benchmarks. Qwen3-VL-Embedding-8B achieves 77.8 overall score on MMEB-V2, ranking first among all models as of January 8, 2025.

Conclusion: The Qwen3-VL-Embedding and Qwen3-VL-Reranker series provide an effective end-to-end solution for multimodal retrieval tasks, demonstrating strong performance across image-text retrieval, visual question answering, and video-text matching applications.
...
<details>
  <summary>Details</summary>
Motivation: Automatic sentiment/emotion classifiers are widely used but typically assessed using third-party annotators rather than individuals experiencing emotions themselves, potentially concealing systematic biases. Need to ensure these tools perform reliably across different populations.

Method: Used unique large-scale dataset of over 1 million self-annotated posts with pre-registered research design to investigate gender biases across 414 combinations of models and emotion-related classes.
...
TL;DR: The paper introduces InterSafe-V, a multi-modal dialogue dataset for safety alignment, and AM³Safety, a framework combining cold-start refusal with GRPO fine-tuning to improve MLLM safety in multi-turn conversations.


<details>
  <summary>Details</summary>
Motivation: Multi-modal LLMs have safety vulnerabilities in multi-turn scenarios where harmful intent can be gradually reconstructed across turns, and existing RLHF methods are designed for single-turn VQA tasks and require costly manual annotations.

Method: 1) Created InterSafe-V dataset with 11,270 dialogues and 500 refusal VQA samples through model interactions; 2) Proposed AM³Safety framework with cold-start refusal phase and Group Relative Policy Optimization (GRPO) fine-tuning using turn-aware dual-objective rewards across entire dialogues.
...
Conclusion: The proposed InterSafe-V dataset and AM³Safety framework effectively address multi-turn safety vulnerabilities in MLLMs, achieving significant safety improvements while maintaining model capabilities, offering a scalable solution for multi-modal dialogue safety alignment.
...
TL;DR: Framework for generating implicit harmful prompts using knowledge graphs and dual-path obfuscation for domain-specific LLM safety testing.


<details>
  <summary>Details</summary>
Motivation: LLMs in specialized domains (finance, healthcare) face unique safety risks, but domain-specific harmful prompt datasets are scarce and mostly manually constructed. Public datasets focus on explicit harmful prompts that modern defenses can detect, while implicit harmful prompts using domain knowledge are harder to detect and better reflect real-world threats.
...
Conclusion: The proposed framework addresses the challenges of generating implicit harmful prompts in specialized domains, providing a systematic approach to create realistic safety testing datasets that better reflect real-world threats to LLMs in domain-specific applications.
...
TL;DR: Tool-MAD: A multi-agent debate framework where agents use different external tools (search, RAG) to improve factual verification through adaptive evidence retrieval and quantitative hallucination detection.
...
Method: Tool-MAD assigns each agent a distinct external tool (search API, RAG module) and features: (1) multi-agent debate with heterogeneous tools for diverse perspectives, (2) adaptive query formulation that iteratively refines evidence retrieval based on debate flow, and (3) integration of Faithfulness and Answer Relevance scores for quantitative hallucination detection by the Judge agent.
...
Conclusion: Tool-MAD effectively mitigates hallucinations in complex reasoning tasks through tool-enhanced multi-agent debate, demonstrating strong potential for real-world fact-checking applications, especially in specialized domains.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM applications in patent/legal practice are limited to lightweight tasks, with no systematic means to evaluate structured legal reasoning capabilities in the patent domain, particularly for PTAB appeals requiring technical and legal integration.

Method: Created PILOT-Bench benchmark aligning PTAB decisions with USPTO patent data at case-level, formalizing three IRAC-aligned classification tasks: Issue Type, Board Authorities, and Subdecision. Evaluated diverse closed-source (commercial) and open-source LLMs across multiple perspectives including input-variation settings, model families, and error tendencies.

Result: Closed-source models consistently exceed 0.75 Micro-F1 score on Issue Type task, while strongest open-source model (Qwen-8B) achieves only ~0.56, showing substantial reasoning capability gap. Analysis reveals performance differences across model families and error patterns.

Conclusion: PILOT-Bench establishes foundation for systematic evaluation of patent-domain legal reasoning, highlighting significant performance disparities between commercial and open-source LLMs, and points toward future improvements through dataset design and model alignment.
...
TL;DR: LLM representations encode syntax and semantics linearly; subtracting averaged syntactic/semantic vectors reduces similarity with matched sentences, showing differential encoding patterns.
...
Method: Averaged hidden-representation vectors of sentences sharing syntactic structure or meaning to create syntactic and semantic "centroids," then subtracted these centroids from sentence vectors to analyze similarity changes with syntactically/semantically matched sentences.

Result: Subtracting syntactic/semantic centroids strongly affects similarity with matched sentences, suggesting linear encoding. Cross-layer encoding profiles differ for syntax vs. semantics, and the two signals can be partially decoupled, indicating differential encoding.
...
<details>
  <summary>Details</summary>
Motivation: Current judge decoding methods for LLM inference acceleration require expensive and noisy supervision, creating a bottleneck

Method: Propose training-free verification using KL divergence between draft and target distributions, based on theoretical proof that learned judges rely on same logit primitives as KL divergence

Result: Matches or outperforms complex trained judges (AutoJudge) across reasoning and coding benchmarks, with superior robustness to domain shifts

Conclusion: KL divergence provides effective, supervision-free verification for judge decoding, eliminating the supervision bottleneck entirely
...
Method: A post-hoc sparse autoencoder trained on pooled embeddings identifies language-associated latent units using cross-language activation statistics, suppresses these units at inference time, and reconstructs embeddings in original dimensionality.
...
Conclusion: LANGSAE EDITING effectively removes language-identity signals from multilingual embeddings, improving cross-language retrieval without requiring retraining of base encoders or re-encoding of raw text, making it compatible with existing vector databases.
...
<details>
  <summary>Details</summary>
Motivation: Non-convex optimization problems are pervasive but intractable for traditional solvers, requiring manual convexification and expert knowledge. Current approaches are inefficient and over-reliant on human expertise.
...
TL;DR: First systematic analysis of role-based authority bias in LLM multi-agent systems shows Expert and Referent power roles have stronger influence than Legitimate power roles, with bias emerging through authoritative roles maintaining positions while general agents show flexibility.


<details>
  <summary>Details</summary>
Motivation: Multi-agent systems using LLMs often assign authoritative roles to improve performance, but the impact of authority bias on agent interactions remains underexplored, creating a need for systematic analysis of how different types of authority influence agent behavior.
...
Conclusion: These findings provide key insights for designing multi-agent frameworks with asymmetric interaction patterns, highlighting that different types of authority have varying influence levels and that authority bias operates through position maintenance rather than active conformity.
...
TL;DR: The paper discovers Late-Stage Volatility Decay in AI-generated text, proposes two simple late-stage features, and achieves SOTA detection performance without extra model access.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot detection methods aggregate token-level statistics across entire sequences, ignoring temporal dynamics of autoregressive generation. The authors aim to capture these dynamics for better AI-generated text detection.
...
Conclusion: Temporal dynamics, particularly Late-Stage Volatility Decay, provide valuable signals for AI-generated text detection. Simple late-stage features can achieve superior performance without complex sampling or additional model access.
...
TL;DR: RAAR is a retrieval-augmented agentic reasoning framework for cross-domain misinformation detection that uses multi-perspective evidence retrieval and multi-agent collaboration to overcome domain transfer limitations and single-perspective modeling.


<details>
  <summary>Details</summary>
Motivation: Cross-domain misinformation detection is challenging due to substantial domain differences in knowledge and discourse. Existing methods struggle with generalization to challenging/underrepresented domains and rely on single-perspective cues, while LLMs are limited to same-distribution data.

Method: RAAR retrieves multi-perspective source-domain evidence aligned with target samples' semantics, sentiment, and writing style. It constructs verifiable multi-step reasoning paths through specialized multi-agent collaboration (perspective-specific agents + summary agent) under verifier guidance, with supervised fine-tuning and reinforcement learning to train a multi-task verifier.

Result: RAAR-8b and RAAR-14b models substantially enhance base model capabilities and outperform other cross-domain methods, advanced LLMs, and LLM-based adaptation approaches on three cross-domain misinformation detection tasks.

Conclusion: RAAR effectively addresses cross-domain misinformation detection challenges through retrieval-augmented agentic reasoning, demonstrating superior performance and generalization capabilities across diverse domains.
...
TL;DR: MisSpans is a new benchmark for fine-grained misinformation detection at the span level within sentences, enabling localization, categorization, and explanation of false details beyond binary true/false classification.


<details>
  <summary>Details</summary>
Motivation: Existing misinformation benchmarks use coarse binary labels at claim/paragraph level, which obscures how true and false details often co-exist within single sentences and limits interpretability for identifying specific misleading segments or differentiating types of falsehoods.

Method: Created MisSpans benchmark with paired real/fake news stories, expert annotations with standardized guidelines, and three complementary tasks: MisSpansIdentity (pinpointing false spans), MisSpansType (categorizing false spans by misinformation type), and MisSpansExplanation (providing rationales). Evaluated 15 LLMs including reasoning-enhanced and non-reasoning variants under zero-shot and one-shot settings.

Result: Results show fine-grained misinformation identification and analysis is challenging, with performance influenced by multiple interacting factors including model size, reasoning capabilities, and domain-specific textual features. High inter-annotator agreement was achieved through standardized guidelines.

Conclusion: MisSpans enables fine-grained localization, nuanced characterization beyond true/false, and actionable explanations for misinformation detection, highlighting the need for deeper understanding of factors influencing LLM performance on this challenging task.
...
<details>
  <summary>Details</summary>
Motivation: Standard RAG pipelines fail on complex multi-hop queries due to lack of structural connectivity, while KG-based RAG performs well on complex tasks but suffers on simple fact-oriented queries. There's a need for a unified approach that handles both types of queries effectively.

Method: Models knowledge base as a heterogeneous graph of propositions, entities, and passages. Uses iterative Suggestion-Selection cycles: Suggestion phase enables query-aware graph traversal, Selection phase provides LLM feedback to prune irrelevant propositions and seed next iteration.

Result: Demonstrates strong performance across three distinct QA tasks (Simple, Complex, and Abstract QA) on both accuracy- and quality-based metrics.
...
<details>
  <summary>Details</summary>
Motivation: Existing Text-to-SQL training suffers from limited high-quality, diverse datasets - either relying on scarce human annotations or LLM-generated data without structural control, resulting in poor structural diversity and complexity.

Method: EvolSQL uses a two-phase approach: 1) Query-SQL expansion for question diversity and schema coverage, 2) Adaptive directional evolution using six AST-based transformation operators to increase complexity across relational, predicate, aggregation, and nesting dimensions, plus execution-grounded refinement and schema-aware deduplication.
...
Method: Mind2Report uses a training-free agentic workflow that augments LLMs with dynamic memory. It follows a three-step cognitive process: 1) probing fine-grained intent, 2) searching web sources and recording distilled information on the fly, and 3) iteratively synthesizing the report.
...
Conclusion: Mind2Report serves as a foundation for advancing commercial deep research agent design, demonstrating that cognitive agentic workflows with dynamic memory can effectively emulate commercial analysts for expert-level report synthesis.
...
TL;DR: CuMA addresses cultural alignment in LLMs by using demographic-aware routing and mixture of adapters to prevent mean collapse and preserve cultural diversity, outperforming dense baselines and semantic-only MoEs.


<details>
  <summary>Details</summary>
Motivation: As LLMs serve global audiences, alignment must shift from universal consensus to respecting cultural pluralism. Dense models suffer from "Mean Collapse" when forced to fit conflicting value distributions, converging to generic averages that fail to represent diverse cultural groups.
...
Conclusion: CuMA successfully addresses cultural alignment by separating conflicting gradients into specialized subspaces, preventing mean collapse and enabling LLMs to better represent diverse cultural values while maintaining strong performance across benchmarks.
...
TL;DR: A disagreement-aware summarization pipeline that separates belief aggregation from language generation, using distance-based belief merging to handle conflicting viewpoints before LLM realization.
...
Method: Two-stage pipeline: 1) Represent documents as structured belief sets and aggregate using distance-based belief merging operators that explicitly model conflict, 2) Use LLMs only to realize aggregated beliefs as natural language summaries.
...
Conclusion: Separating belief aggregation from language generation provides more stable and faithful disagreement-aware summarization compared to end-to-end LLM approaches, working consistently across different model types and sizes.
...
TL;DR: The paper introduces V-FAT, a diagnostic benchmark to measure Text Bias in MLLMs where models rely on linguistic shortcuts instead of genuine visual understanding, revealing significant visual collapse under high linguistic dominance.
...
Method: The authors introduce V-FAT benchmark with 4,026 VQA instances across six semantic domains, using a Three-Level Evaluation Framework that systematically increases conflict between visual evidence and textual information (L1: internal bias, L2: external bias, L3: synergistic bias), and propose Visual Robustness Score (VRS) metric.

Result: Evaluation of 12 frontier MLLMs shows that while models excel in existing benchmarks, they experience significant visual collapse under high linguistic dominance, revealing their reliance on textual shortcuts over genuine visual understanding.

Conclusion: Current MLLMs suffer from Text Bias, relying too heavily on linguistic priors rather than visual evidence, highlighting the need for better visual grounding and the importance of benchmarks like V-FAT to diagnose and address this fundamental limitation.
...
TL;DR: LLM-generated persuasive texts can be harder to detect than human-written ones, especially when subtle, and the paper introduces a multilingual benchmark called Persuaficial to study this phenomenon.
...
Method: Categorize controllable generation approaches for producing persuasive content with LLMs, introduce Persuaficial (a high-quality multilingual benchmark covering six languages), and conduct extensive empirical evaluations comparing human-authored and LLM-generated persuasive texts.

Result: Overtly persuasive LLM-generated texts are easier to detect than human-written ones, but subtle LLM-generated persuasion consistently degrades automatic detection performance. The paper also provides the first comprehensive linguistic analysis contrasting human and LLM-generated persuasive texts.

Conclusion: LLM-generated persuasion presents detection challenges, especially when subtle. The findings offer insights that may guide the development of more interpretable and robust detection tools for identifying AI-generated persuasive content.
...
TL;DR: The paper introduces Generation-time Fine-grained Provenance task and proposes GenProve framework with GRPO to generate answers with structured sentence-level provenance triples, addressing limitations of coarse-grained citations for LLM accountability.


<details>
  <summary>Details</summary>
Motivation: Current citation methods for LLMs are insufficient for accountability as users struggle to verify how cited sources support generated claims. Existing approaches are coarse-grained and fail to distinguish between direct quotes and complex reasoning.

Method: 1) Introduce Generation-time Fine-grained Provenance task requiring models to generate fluent answers with structured sentence-level provenance triples. 2) Create ReFInE dataset with expert annotations distinguishing Quotation, Compression, and Inference. 3) Propose GenProve framework combining SFT with Group Relative Policy Optimization (GRPO) optimizing composite reward for answer fidelity and provenance correctness.

Result: GenProve significantly outperforms 14 strong LLMs in joint evaluation. Analysis reveals a reasoning gap: models excel at surface-level quotation but struggle significantly with inference-based provenance, showing verifiable reasoning remains a distinct frontier challenge.

Conclusion: Fine-grained provenance generation is crucial for LLM accountability, and the proposed GenProve framework effectively addresses this. The identified reasoning gap suggests that verifiable reasoning is a separate challenge from surface-level citation, requiring specialized approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current spoken language models lack deep emotional understanding and reasoning capabilities. There's a need to move beyond explicit emotional supervision to internalize emotional intelligence within the model's reasoning process for more natural and empathetic human-computer interactions.

Method: Proposes IEAT (Injected Emotional-Attribution Thinking) that incorporates user emotional states and their causes into model reasoning. Uses two-stage training: 1) speech-text alignment and emotional attribute modeling via self-distillation, 2) end-to-end cross-modal joint optimization for consistency between textual and spoken emotional expressions.
...
Conclusion: The IEAT approach successfully internalizes emotional intelligence in spoken language models, enabling more natural and empathetic human-computer dialogue through unified emotional reasoning across speech and text modalities.
...
TL;DR: The paper proposes using natural language as a universal interface for representing user preferences in LLMs, creating interpretable and transferable preference descriptions instead of opaque black-box vectors.


<details>
  <summary>Details</summary>
Motivation: Current methods represent user preferences as implicit, model-specific vectors that are opaque, difficult to interpret, and not transferable across models and tasks. There's a need for more transparent and reusable preference representations.

Method: A two-stage training framework combining supervised fine-tuning on synthesized data with reinforcement learning to optimize long-term utility and cross-task transferability. This leads to AlignXplore+, a universal preference reasoning model that generates textual preference summaries.

Result: The 8B model achieves state-of-the-art performance on nine benchmarks, outperforming substantially larger open-source models, while showing strong transferability across tasks, model families, and interaction formats.

Conclusion: Natural language serves as an effective universal interface for preference representation, enabling interpretable, reusable, and continually evolving preference descriptions that outperform traditional black-box approaches.
...
TL;DR: Incorporating negative reasoning trajectories (incorrect final answers) into supervised fine-tuning improves out-of-domain generalization by mitigating overfitting and boosting exploration, with a proposed adaptive loss weighting method (GLOW) further enhancing these benefits.


<details>
  <summary>Details</summary>
Motivation: Standard SFT practices discard reasoning trajectories with incorrect final answers, wasting valuable supervision and exacerbating overfitting, which limits out-of-domain generalization. The authors argue that negative trajectories contain valid intermediate reasoning that can improve model performance.

Method: 1) Incorporate negative reasoning trajectories (incorrect final answers) into SFT training alongside positive ones. 2) Analyze patterns in negative chains and their effects on training dynamics. 3) Propose Gain-based LOss Weighting (GLOW), an adaptive loss weighting scheme that rescales per-sample loss based on inter-epoch progress to efficiently leverage unfiltered trajectories.

Result: Negative trajectories yield substantial OOD generalization gains over positive-only training, moderate loss descent to mitigate overfitting, boost policy entropy by 35.67% during inference, and serve as better RL initialization (MMLU improves from 72.82% to 76.47%). GLOW achieves 5.51% OOD gain over positive-only SFT on Qwen2.5-7B.

Conclusion: Incorporating negative reasoning trajectories into SFT provides significant benefits for out-of-domain generalization by reducing overfitting and improving exploration. The proposed GLOW method efficiently leverages unfiltered trajectories, demonstrating that negative examples contain valuable supervision that should not be discarded in reasoning-focused fine-tuning.
...
TL;DR: SAS framework improves LLM-based detection of self-destructive behaviors in subcultures by addressing knowledge lag and semantic misalignment challenges.


<details>
  <summary>Details</summary>
Motivation: Self-destructive behaviors are hard to detect, especially in subcultures with unique expressions. While LLMs show promise for detection, they struggle with rapidly evolving subcultural slang and nuanced semantic understanding specific to these groups.
...
Result: SAS outperforms current advanced multi-agent framework OWL and competes well with fine-tuned LLMs for self-destructive behavior detection in subcultural contexts.

Conclusion: SAS advances self-destructive behavior detection in subcultures and serves as a valuable resource for future research in this challenging domain.
...
TL;DR: Reasoning distillation via SFT fails to transfer cognitive structure from teacher models to students, causing functional alignment collapse where students mimic linguistic form without internalizing resource allocation policies.


<details>
  <summary>Details</summary>
Motivation: Large Reasoning Models trained via RL show natural alignment with human cognitive costs, but current reasoning distillation methods may not preserve this cognitive structure, raising questions about how to effectively transfer human-like reasoning capabilities.

Method: Tested the "Hán Dān Xué Bù" (Superficial Mimicry) hypothesis across 14 models, comparing teacher models trained via reinforcement learning with student models distilled via Supervised Fine-Tuning (SFT), measuring alignment with human difficulty scaling.
...
Conclusion: Reasoning distillation decouples computational cost from cognitive demand, revealing that human-like cognition emerges from active reinforcement learning rather than passive imitation. Current SFT-based distillation fails to transmit the cognitive structure underlying human-like reasoning.
...
TL;DR: ArcAligner is a lightweight module that helps LLMs better understand highly compressed context in RAG systems by using adaptive gating to add processing only when needed, improving performance on complex queries while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: RAG systems face a trade-off: compressing long documents reduces computational cost but makes it harder for LLMs to understand the compressed information. Current compression methods (token pruning, summarization, embeddings) lose important context, especially for complex queries.

Method: ArcAligner is a lightweight module integrated into language model layers that uses adaptive recursive context alignment. It employs an adaptive gating system that selectively adds extra processing power only when the compressed information is complex, keeping the system efficient while improving context understanding.

Result: ArcAligner consistently outperforms compression baselines at comparable compression rates across knowledge-intensive QA benchmarks, with particularly strong performance on multi-hop and long-tail settings where understanding compressed context is most challenging.

Conclusion: ArcAligner effectively addresses the compression-accuracy trade-off in RAG systems by helping LLMs better utilize highly compressed context representations, enabling both efficiency and improved performance on complex queries.
...
TL;DR: Compositional steering tokens enable multi-behavior control in LLMs by embedding behaviors into input tokens that can be composed, outperforming existing methods and generalizing to unseen combinations.


<details>
  <summary>Details</summary>
Motivation: Real-world LLM applications require controllable outputs satisfying multiple desiderata simultaneously. While single-behavior steering has been extensively studied, compositional steering (steering LLMs toward multiple behaviors at once) remains underexplored.

Method: Propose compositional steering tokens: 1) Embed individual behaviors (natural language instructions) into dedicated tokens via self-distillation, 2) Train a dedicated composition token on behavior pairs that learns to generalize to unseen compositions, including those with unseen behaviors and unseen numbers of behaviors.

Result: Steering tokens achieve superior multi-behavior control compared to competing approaches (instructions, activation steering, LoRA merging). They generalize well to unseen compositions and complement natural language instructions, with combined approaches yielding further gains.

Conclusion: Compositional steering tokens provide an effective solution for multi-behavior control in LLMs, operating in input token space rather than activation space, enabling zero-shot composition and better performance than existing methods.
...
TL;DR: SemPA is a novel method that improves sentence representations in LLMs while preserving their generative capabilities through semantic preference alignment using DPO on paraphrase generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing sentence embedding methods for LLMs have limitations: fixed prompt templates lack optimization and yield limited performance, while architectural modifications compromise generative capabilities by altering internal computational mechanisms.

Method: SemPA uses sentence-level Direct Preference Optimization (DPO) to efficiently optimize LLMs on paraphrase generation tasks, enabling models to discriminate semantically equivalent sentences while preserving inherent generative capacity. The approach establishes a theoretical connection between DPO and contrastive learning under the Plackett-Luce model framework.
...
Conclusion: SemPA successfully boosts sentence representations while preserving generative ability through semantic preference alignment, offering a balanced approach that overcomes limitations of existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Traditional NLP models fail to interpret Hinglish (Hindi-English hybrid) used widely in Indian social media, leading to inaccurate sentiment analysis and misleading market insights for brand monitoring.
...
Conclusion: The proposed framework effectively addresses the challenges of Hinglish sentiment analysis, providing a robust solution for brand monitoring in India's multilingual social media landscape.
...
<details>
  <summary>Details</summary>
Motivation: To understand how emotional expressions (praise, anger, blame) during human-AI interactions influence both ChatGPT's behavior and subsequent human communication patterns.

Method: Between-subject experiment where participants expressed specific emotions while working with ChatGPT (GPT-4.0) on two tasks: writing a public response and addressing an ethical dilemma.

Result: Praise led to greatest improvement in ChatGPT's answers; anger showed smaller improvement; blame showed no improvement. Anger reduced ChatGPT's prioritization of corporate interests in ethical dilemmas, while blame increased emphasis on public interest. Negative emotional expressions in AI interactions carried over to more hostile human-human communication.

Conclusion: Emotional tone in human-AI interactions significantly shapes both AI outputs and subsequent human communication, highlighting the importance of emotional awareness in AI system design and usage.
...
TL;DR: Survey paper tracing the evolution from LLM-as-a-Judge to Agent-as-a-Judge for AI evaluation, presenting a unified framework, taxonomy, and roadmap for agentic evaluation systems.


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-Judge evaluation has limitations with complex, specialized, multi-step tasks due to biases, shallow reasoning, and lack of real-world verification. The field lacks a unified framework for the emerging Agent-as-a-Judge paradigm.

Method: Comprehensive survey identifying key dimensions of the paradigm shift, establishing a developmental taxonomy, organizing core methodologies, and analyzing applications across general and professional domains.

Result: Presents the first unified framework for agentic evaluation, tracing the evolution from LLM-based to agentic judges, with systematic analysis of methodologies, applications, and challenges.

Conclusion: Provides a clear roadmap for next-generation agentic evaluation systems, addressing frontier challenges and identifying promising research directions for more robust, verifiable, and nuanced AI assessment.
...
TL;DR: DocDancer: An end-to-end trained open-source document question answering agent with tool-driven framework for document exploration and comprehension, trained on synthetic data from Exploration-then-Synthesis pipeline.


<details>
  <summary>Details</summary>
Motivation: Existing DocQA agents lack effective tool utilization and rely heavily on closed-source models, creating a need for open-source solutions with better tool integration for document understanding.
...
Conclusion: DocDancer demonstrates the viability of end-to-end trained open-source DocQA agents with tool-driven frameworks, addressing limitations of existing approaches through synthetic data generation and improved tool utilization.
...
TL;DR: RelayLLM enables efficient reasoning by having SLMs dynamically invoke LLMs only for critical tokens during generation, achieving near-LLM performance with minimal LLM usage.


<details>
  <summary>Details</summary>
Motivation: Current collaborative approaches between LLMs and SLMs are inefficient, offloading entire queries to LLMs even when SLMs can handle most reasoning steps, leading to computational waste.

Method: RelayLLM uses token-level collaborative decoding where SLMs act as controllers that dynamically invoke LLMs for critical tokens via special commands. It employs a two-stage training framework with warm-up and Group Relative Policy Optimization (GRPO) to teach strategic help-seeking.

Result: Achieves 49.52% average accuracy across six benchmarks, bridging performance gap between models while invoking LLMs for only 1.07% of total generated tokens, offering 98.2% cost reduction vs performance-matched random routers.

Conclusion: RelayLLM provides an efficient token-level collaborative framework that significantly reduces computational costs while maintaining reasoning performance, effectively balancing SLM independence with strategic LLM assistance.
...
TL;DR: The paper analyzes the logical properties of Natural Language Inference (NLI) by formulating three possible readings of NLI labels and evaluating which logical relations are encoded in the SNLI dataset through meta-inferential consistency analysis.
...
Method: The authors formulate three possible readings of the NLI label set and perform comprehensive analysis of their meta-inferential properties. They focus on SNLI dataset, exploiting (1) NLI items with shared premises and (2) items generated by LLMs to evaluate models trained on SNLI for meta-inferential consistency.

Result: The analysis reveals insights into which reading of the logical relations is encoded by the SNLI dataset through evaluation of meta-inferential consistency in models trained on SNLI.

Conclusion: The paper provides a framework for understanding the logical properties of NLI and demonstrates how to analyze which logical reading is encoded in NLI datasets, which is essential for proper interpretation of model performance on NLI tasks.
...
TL;DR: Inside Out framework uses PersonaTree for long-term personalized dialogue, with MemListener managing structured memory operations via RL, outperforming existing methods in noise suppression and consistency.


<details>
  <summary>Details</summary>
Motivation: Existing long-term personalized dialogue systems face challenges with memory noise accumulation, reasoning degradation, and persona inconsistency due to unbounded interactions within finite context constraints.

Method: Proposes Inside Out framework with PersonaTree for user profiling, using schema-constrained trunk with updatable branches/leaves. Trains lightweight MemListener via RL with process-based rewards to produce structured {ADD, UPDATE, DELETE, NO_OP} operations for dynamic tree evolution.

Result: PersonaTree outperforms full-text concatenation and personalized memory systems in suppressing contextual noise and maintaining persona consistency. MemListener achieves memory-operation performance comparable to or surpassing powerful reasoning models like DeepSeek-R1-0528 and Gemini-3-Pro.

Conclusion: Inside Out framework effectively addresses long-term personalized dialogue challenges through structured PersonaTree management and lightweight MemListener, enabling scalable and consistent user profiling with interpretable memory operations.
...
TL;DR: LELA is a modular coarse-to-fine entity linking method using LLMs without fine-tuning, achieving competitive performance with fine-tuned approaches.


<details>
  <summary>Details</summary>
Motivation: Entity linking is crucial for knowledge graph construction, QA, and information extraction, but existing methods often require fine-tuning which limits flexibility across domains and knowledge bases.

Method: LELA uses a modular coarse-to-fine approach leveraging LLMs' capabilities, working with different domains, knowledge bases, and LLMs without any fine-tuning phase.
...
Conclusion: LELA provides a flexible, effective entity linking solution that eliminates the need for fine-tuning while maintaining competitive performance across diverse settings.
...
TL;DR: Researchers used ML/AI to measure peace levels in countries from news/social media and developed tools to promote peace by helping users understand their media consumption patterns.


<details>
  <summary>Details</summary>
Motivation: To address the problem that 71% of young people get news from short social media videos, which often use emotional activation (anger) to increase engagement, potentially harming peace and nuanced discourse.

Method: 1) Used neural networks on text embeddings to measure peace levels in news media; 2) Developed models for social media using GoEmotions (word-level) and LLMs (context-level); 3) Created MirrorMirror Chrome extension for real-time peace feedback on YouTube.
...
Conclusion: The research demonstrates ML/AI can effectively measure peace in media and tools like MirrorMirror can help users understand media consumption effects, with the goal of evolving into an open-source platform to encourage more respectful, nuanced communication beyond simple engagement metrics.
...
TL;DR: GDPO improves multi-reward RL by decoupling reward normalization, preventing collapse of distinct rewards into identical advantage values that causes suboptimal convergence in GRPO.


<details>
  <summary>Details</summary>
Motivation: As language models need to align with diverse human preferences, multi-reward RL pipelines are used, but current methods like GRPO cause reward collapse and suboptimal training.

Method: Introduces Group reward-Decoupled Normalization Policy Optimization (GDPO), which decouples normalization of individual rewards to preserve their relative differences and improve training stability.

Result: GDPO consistently outperforms GRPO across tool calling, math reasoning, and coding reasoning tasks in both correctness metrics (accuracy, bug ratio) and constraint adherence (format, length).

Conclusion: GDPO effectively resolves multi-reward optimization issues, demonstrating superior performance and generalizability for aligning language models with diverse human preferences.
...
TL;DR: The paper proposes Complex Preference Optimization (CPO), a two-stage framework for fine-grained alignment of diffusion models using hierarchical expert criteria instead of simple rewards.
...
Method: 1) Construct hierarchical evaluation criteria with domain experts, decomposing image quality into tree-structured positive/negative attributes. 2) Two-stage alignment: First, inject domain knowledge via Supervised Fine-Tuning of an auxiliary diffusion model. Second, introduce CPO that extends DPO to align target diffusion to non-binary hierarchical criteria by maximizing positive attribute probabilities while minimizing negative ones.

Result: CPO significantly enhances generation quality and alignment with expertise in painting generation domain, outperforming methods using simplified alignment signals.

Conclusion: The proposed framework enables fine-grained criteria alignment for diffusion models, opening new avenues for aligning AI systems with complex human expertise beyond simple reward signals.
...
<details>
  <summary>Details</summary>
Motivation: Existing text embedding methods (LSB, MSB, PVD, transform domain, deep learning) often require multiple pixels, cause noise/distortion, are computationally heavy, and have deterministic encoding/decoding. Need for more efficient single-pixel embedding with minimal distortion.

Method: Uses quinary combinations of pixel intensities in RGB space - five controlled intensity variations in each R, G, B channel create 125 distinct combinations (5×5×5). These combinations are mapped to textual symbols (letters, numbers, whitespace, special characters). Each complete textual symbol is encoded within a single RGB pixel.

Result: Evaluation using MSE, MAE, SNR, PSNR, SSIM, Histogram Comparison and Heatmap analysis shows no significant distortion between original and encoded images. Achieves improved embedding efficiency by encoding complete symbols in single pixels vs. existing methods requiring multiple pixels or heavy computation.

Conclusion: The quinary pixel intensity combination method provides efficient, low-distortion text embedding in images using single-pixel encoding, outperforming traditional LSB/MSB and computationally intensive deep learning approaches.
...
TL;DR: The paper proposes a unified multimodal generation architecture that enables autonomous text-to-image synthesis within a single inference process, using reward-weighted post-training with self-generated synthetic data to improve performance across multiple benchmarks.
...
Conclusion: Fully unified text-image generation with autonomous modality transitions within a single inference process is achievable through reward-weighted post-training with self-generated synthetic data, outperforming sequential approaches and demonstrating the importance of strategic data curation and multimodal reward optimization.
...
<details>
  <summary>Details</summary>
Motivation: Current transformer-based video diffusion models suffer from quadratic attention complexity that limits scalability for longer video sequences, creating a need for more efficient architectures that maintain high-quality generation.

Method: ReHyAt uses a Recurrent Hybrid Attention mechanism that combines softmax attention fidelity with linear attention efficiency, enabling chunk-wise recurrent reformulation with constant memory usage. It employs a lightweight distillation pipeline to efficiently transfer knowledge from existing softmax-based models.

Result: ReHyAt reduces training costs by two orders of magnitude (~160 GPU hours vs. typical approaches), achieves state-of-the-art video quality on VBench and VBench-2.0 benchmarks, reduces attention cost from quadratic to linear, and demonstrates practical scalability for long-duration and on-device video generation.

Conclusion: ReHyAt provides an efficient hybrid attention solution that maintains video generation quality while dramatically reducing computational costs, offering a practical recipe for scaling future video diffusion models and enabling long-duration and on-device applications.
...
TL;DR: Progressive 3D Gaussian Splatting compression using Residual Vector Quantization and auto-regressive entropy model with multi-resolution hash grid for efficient feature compression.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting models have large storage requirements that hinder cloud/streaming deployment. Existing progressive compression methods use scalar quantization which may not optimally capture high-dimensional feature correlations, limiting rate-distortion performance.
...
Result: The method enables more efficient compression of 3D Gaussian Splatting models by better capturing high-dimensional feature correlations through vector quantization and improved entropy modeling.

Conclusion: The proposed Residual Vector Quantization approach with auto-regressive entropy modeling provides superior compression efficiency for 3D Gaussian Splatting compared to traditional scalar quantization methods, enabling better deployment over cloud and streaming services.
...
TL;DR: Custom CNNs vs pre-trained models (ResNet-18, VGG-16) tested on 5 Bangladeshi image datasets. Transfer learning with fine-tuning outperforms custom CNNs and feature extraction, with ResNet-18 achieving 100% accuracy on road damage dataset.


<details>
  <summary>Details</summary>
Motivation: To provide practical guidance for practitioners on selecting appropriate deep learning approaches by comparing custom-built CNNs against pre-trained architectures using different learning strategies across diverse real-world image classification tasks from Bangladesh.

Method: Comparative analysis of custom CNNs vs pre-trained models (ResNet-18, VGG-16) using feature extraction and transfer learning approaches. Evaluated on five Bangladeshi datasets: Footpath Vision, Auto Rickshaw Detection, Mango Image Classification, Paddy Variety Recognition, and Road Damage Detection.

Result: Transfer learning with fine-tuning consistently outperformed both custom CNNs and feature extraction, achieving 3-76% accuracy improvements. ResNet-18 with fine-tuning achieved 100% accuracy on Road Damage BD dataset. Custom CNNs have smaller model size (3.4M vs 11-134M parameters) and better training efficiency on simpler tasks.

Conclusion: Pre-trained models with transfer learning provide superior performance, especially for complex tasks with limited data. Custom CNNs offer advantages in model size and efficiency for simpler tasks. The research provides practical insights for selecting approaches based on dataset characteristics, computational resources, and performance requirements.
...
TL;DR: PackCache is a training-free KV-cache management method that dynamically compacts KV cache for unified autoregressive video generation, achieving 1.7-2.2x acceleration for 48-frame sequences.


<details>
  <summary>Details</summary>
Motivation: KV-cache size in unified autoregressive models grows linearly with generated tokens, becoming the dominant bottleneck for inference efficiency and generative length in video generation.
...
Result: PackCache accelerates end-to-end generation by 1.7-2.2x on 48-frame sequences, with 2.6x and 3.7x acceleration on the final four frames on A40 and H200 respectively.

Conclusion: PackCache effectively addresses KV-cache bottlenecks in unified autoregressive video generation, enabling longer-sequence generation with significant efficiency improvements.
...
TL;DR: The paper proposes a stress recognition method using 3D facial geometry (EMOCA-derived coefficients) and cross-modal attention fusion with physiological signals, achieving 92% AUROC for distracted driving stress detection.


<details>
  <summary>Details</summary>
Motivation: Stress recognition from facial videos is challenging due to subjectivity and voluntary facial control. Existing methods rely heavily on Facial Action Units, while disentangled 3D facial geometry remains underexplored for stress analysis.

Method: 1) Analyze stress during distracted driving using EMOCA-derived 3D expression and pose coefficients; 2) Use paired hypothesis tests to identify stress-responsive coefficients; 3) Propose Transformer-based temporal modeling with unimodal, early-fusion, and cross-modal attention strategies; 4) Fuse EMOCA features with physiological signals and gaze data.

Result: 41 of 56 EMOCA coefficients show consistent stress responses comparable to physiological markers. Cross-Modal Attention fusion of EMOCA and physiological signals achieves best performance (AUROC 92%, Accuracy 86.7%), with EMOCA-gaze fusion also competitive (AUROC 91.8%).

Conclusion: Disentangled 3D facial geometry provides effective stress indicators, and temporal modeling with cross-modal attention significantly improves stress recognition performance, demonstrating the value of multimodal fusion for reliable stress detection.
...
TL;DR: A flow-matching foundation model pre-trained on RGB can be adapted with just 100 paired images to translate RGB to IR/SAR, enabling synthetic data generation that improves object detection in non-visible modalities.


<details>
  <summary>Details</summary>
Motivation: Foundation models are mainly trained on RGB data, but safety-critical applications often use non-visible modalities like IR and SAR. There's a need to bridge this gap without extensive paired datasets.

Method: Fine-tune FLUX.1 Kontext with LoRA modules using only 100 paired images per domain (RGB→IR on KAIST, RGB→SAR on M4-SAR). Use LPIPS on 50 held-out pairs as proxy for downstream performance, then generate synthetic IR/SAR data for training detection models.

Result: Lower LPIPS consistently predicts higher mAP for YOLOv11n on IR/SAR and DETR on KAIST IR. Synthetic IR from external RGB datasets improves KAIST pedestrian detection, and synthetic SAR boosts infrastructure detection when combined with limited real SAR.

Conclusion: Few-shot LoRA adaptation of flow-matching foundation models is a promising approach for foundation-style support of non-visible modalities, enabling effective cross-spectral translation with minimal paired data.
...
Method: Comparative analysis of a custom-designed CNN trained from scratch against popular pre-trained architectures (VGG-16, ResNet-50, MobileNet) using transfer learning. All models evaluated under identical experimental settings with standard performance metrics (accuracy, precision, recall, F1-score).
...
Conclusion: The study highlights trade-offs between model complexity, performance, and computational efficiency, providing practical insights for selecting appropriate CNN architectures based on specific requirements (accuracy needs vs computational constraints).
...
Method: Tri MARF integrates tri-modal inputs (2D multi-view images, textual descriptions, and 3D point clouds) using a multi-agent collaborative architecture with three specialized agents: vision-language model agent for multi-view descriptions, information aggregation agent for optimal description selection, and gating agent for aligning textual semantics with 3D geometry.

Result: Extensive experiments on Objaverse, LVIS, Objaverse XL, and ABO datasets show Tri MARF substantially outperforms existing methods: CLIPScore of 88.7 (vs prior SOTA), retrieval accuracy of 45.2 and 43.8 on ViLT R@5, and throughput of up to 12,000 objects per hour on a single NVIDIA A100 GPU.

Conclusion: Tri MARF demonstrates that integrating tri-modal inputs through multi-agent collaboration effectively addresses 3D annotation challenges, achieving superior performance in both accuracy and efficiency compared to existing approaches.
...
Method: Proposes a hybrid self-supervised and weakly-supervised learning framework that predicts mastoidectomy regions directly from preoperative CT scans (with intact mastoid) without human annotations, leveraging 3D T-distribution loss in weakly-supervised medical imaging.

Result: Achieves mean Dice score of 0.72 for predicting complex, boundary-less mastoidectomy shapes, surpassing state-of-the-art approaches and demonstrating strong performance for constructing 3D postmastoidectomy surfaces.

Conclusion: First work integrating self-supervised and weakly-supervised learning for mastoidectomy shape prediction, offering robust and efficient solution for cochlear implant surgical planning while addressing annotation challenges in medical imaging.
...
TL;DR: CRUNet-MR-Univ is a foundation model for Cardiac MRI reconstruction that uses spatio-temporal correlations and prompt-based priors to handle diverse CMR scenarios, outperforming baseline methods across various settings.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for Cardiac MRI reconstruction lack generalizability due to wide variability in CMR scans (image contrast, sampling patterns, scanner vendors, anatomical structures, disease types). Most models only handle narrow subsets of these variations, leading to performance degradation with distribution shifts.
...
<details>
  <summary>Details</summary>
Motivation: Current chain-of-thought approaches in Large Vision-Language Models lead to overthinking - excessively verbose responses for simple queries, causing inefficiency and degraded accuracy. Prior work overlooks the fundamental bottleneck of visual perception failures, which are often the root cause of reasoning errors rather than insufficient deliberation.

Method: GPRO uses a meta-reasoning controller that dynamically routes computation among three paths at each generation step: 1) lightweight fast path, 2) slow perception path for re-examining visual inputs, and 3) slow reasoning path for internal self-reflection. The controller is trained using large-scale failure attribution supervision from ~790k samples to distinguish perceptual hallucinations from reasoning errors, then optimized with multi-objective reinforcement learning to balance accuracy and computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous driving systems treat perception, prediction, and planning as separate modules, which limits their ability to leverage synergies between these tasks. The authors aim to create a unified approach that tightly integrates these components for better performance.

Method: UniDrive-WM uses a VLM-based world model architecture where: 1) A trajectory planner predicts future trajectories, 2) These trajectories condition a VLM-based image generator to produce plausible future frames, 3) The generated future images provide additional supervisory signals that enhance scene understanding and iteratively refine trajectory generation. The authors also compare discrete vs continuous output representations for future image prediction.

Result: On the Bench2Drive benchmark, UniDrive-WM produces high-fidelity future images and improves planning performance by 5.9% in L2 trajectory error and 9.2% in collision rate reduction compared to the previous best method.

Conclusion: Tightly integrating VLM-driven reasoning, planning, and generative world modeling provides significant advantages for autonomous driving systems, as demonstrated by UniDrive-WM's improved performance on challenging driving benchmarks.
...
TL;DR: LLM-driven agent for integrated forest change analysis using satellite imagery, combining change detection and semantic captioning with natural language querying.


<details>
  <summary>Details</summary>
Motivation: Address the gap in integrating LLMs with vision-language models for remote sensing image change interpretation, particularly for complex forest dynamics monitoring.

Method: Propose an LLM-driven agent with multi-level change interpretation vision-language backbone and LLM-based orchestration, using the new Forest-Change dataset with bi-temporal satellite imagery, change masks, and semantic captions.

Result: Achieves mIoU of 67.10% and BLEU-4 of 40.17% on Forest-Change dataset, and 88.13% mIoU and 34.41% BLEU-4 on LEVIR-MCI-Trees benchmark for joint change detection and captioning.

Conclusion: LLM-driven RSICI systems show potential to improve accessibility, interpretability, and efficiency of forest change analysis, with all data and code publicly available.
...
Method: Three key components: (1) multi-scale hierarchical encoder for global context and boundary details, (2) boundary-aware tokenizer with VQ-VAE quantization and importance scoring to select salient tokens near boundaries, (3) sparse-to-dense decoder with token reprojection and progressive upsampling.

Result: Achieves state-of-the-art performance with 94.49% Dice and 89.61% IoU on 3D breast DCE-MRI dataset (960 cases), while reducing GPU memory by 64% and inference latency by 68%. Generalizes well to MSD cardiac and brain MRI datasets.

Conclusion: TokenSeg demonstrates that anatomically informed sparse representation enables accurate and efficient 3D medical image segmentation, addressing computational limitations while maintaining high performance across diverse anatomical structures.
...
TL;DR: FaceRefiner is a style transfer-based facial texture refinement method that improves texture quality and identity preservation by transferring multi-level information from input images to synthesized textures.


<details>
  <summary>Details</summary>
Motivation: Current facial texture generation methods use deep networks to synthesize textures and fill UV maps, but these synthesized textures come from training data or 2D face generator spaces, limiting generalization for in-the-wild images. This causes inconsistencies in facial details, structures, and identity with the input.

Method: FaceRefiner treats 3D sampled texture as style and texture generation output as content, using style transfer to transfer photo-realistic style. Unlike current methods that only transfer high and middle level information, it integrates differentiable rendering to also transfer low-level (pixel-level) information in visible face regions, enabling multi-level information transfer.
...
Conclusion: The proposed style transfer-based facial texture refinement method effectively preserves input details, structures, and semantics while improving texture quality and identity consistency for in-the-wild facial images.
...
TL;DR: RepMD detects harmful memes by identifying invariant design concepts behind shifting memes, using Design Concept Graphs derived from historical data to guide MLLM detection.


<details>
  <summary>Details</summary>
Motivation: Harmful memes constantly evolve in type and over time, making detection challenging. However, different harmful memes share underlying design principles used by malicious creators, which can serve as invariant patterns for analysis.

Method: 1) Define Design Concept Graph (DCG) based on attack tree principles to describe steps for designing harmful memes. 2) Derive DCG from historical memes through design step reproduction and graph pruning. 3) Use DCG to guide Multimodal Large Language Model (MLLM) for harmful meme detection.

Result: RepMD achieves 81.1% accuracy, maintains performance with slight decreases when generalizing to type-shifting and temporal-evolving memes, and improves human detection efficiency to 15-30 seconds per meme.

Conclusion: The invariant design concepts behind harmful memes provide a stable foundation for detection despite meme evolution. RepMD effectively leverages these principles through DCG-guided MLLM analysis, offering robust detection of shifting harmful content while enhancing human moderation efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Segmentation of left atrial wall and endocardium from LGE MRI is essential for quantifying atrial fibrosis, but challenging due to limited data availability and anatomical complexity. Need for data augmentation solutions to improve machine learning-based segmentation models.

Method: Developed pipeline to synthesize 3D LGE MRI volumes from composite semantic label maps combining expert annotations with unsupervised tissue clusters. Evaluated three 3D conditional generators: Pix2Pix GAN, SPADE-GAN, and SPADE-LDM. Used synthetic images to augment training data for downstream LA segmentation with 3D U-Net.

Result: SPADE-LDM generated most realistic images with FID of 4.063, outperforming Pix2Pix GAN (40.821) and SPADE-GAN (7.652). Augmentation with synthetic LGE images improved Dice score for LA cavity segmentation from 0.908 to 0.936, statistically significant improvement (p < 0.05).

Conclusion: Label-conditioned 3D synthesis effectively enhances segmentation of under-represented cardiac structures by generating realistic synthetic data to augment scarce training datasets.
...
TL;DR: MiLDEAgent is a reasoning-based framework for multi-layer design document editing that combines RL-trained multimodal reasoning with targeted image editing, outperforming existing approaches on a new benchmark.


<details>
  <summary>Details</summary>
Motivation: Real-world design documents (posters, etc.) are multi-layered with decoration, text, and images, but existing methods focus on single-layer image editing or multi-layer generation without the layer-aware reasoning needed for precise editing from natural language instructions.
...
Result: Experiments on 14 open-source and 2 closed-source models show existing approaches fail: open-source models can't complete tasks, closed-source models violate formats. MiLDEAgent achieves strong layer-aware reasoning and precise editing, outperforming all open-source baselines and matching closed-source performance.
...
TL;DR: Paper proposes a framework for evaluating safety/security strategies for AI-enabled human-centric cyber-physical systems, with a case study on meal detection in diabetes control.
...
Method: 1) Discuss operational deviations leading to unknown conditions, 2) Create framework to evaluate safety/security strategies, 3) Demonstrate with personalized image-based technique for detecting non-announcement of meals in closed-loop blood glucose control.

Result: Framework developed for evaluating safety strategies; novel image-based technique presented for detecting unannounced meals in diabetes management.

Conclusion: Need systematic approaches to handle operational uncertainties in AI-enabled human-centric systems; proposed framework and case study demonstrate practical solutions for safety/security assurance.
...
TL;DR: A novel HUR-MACL model improves head and neck organ segmentation by adaptively identifying high-uncertainty regions and using Vision Mamba + Deformable CNN collaboration with feature distillation.
...
Method: Proposes HUR-MACL (High Uncertainty Region-guided Multi-Architecture Collaborative Learning) that: 1) adaptively identifies high uncertainty regions using CNN, 2) uses Vision Mamba and Deformable CNN to jointly improve segmentation in these regions, and 3) introduces heterogeneous feature distillation loss to promote collaboration between architectures.
...
Conclusion: The proposed HUR-MACL model effectively addresses limitations of existing hybrid architectures by adaptively focusing on challenging regions and enabling synergistic collaboration between different architectures, leading to superior segmentation performance.
...
TL;DR: Agri-R1 is a 3B-parameter reasoning-enhanced model for agricultural disease diagnosis that uses automated reasoning data generation and Group Relative Policy Optimization to achieve competitive performance with much larger models.
...
Result: 3B-parameter model achieves performance competitive with 7B- to 13B-parameter baselines: +23.2% relative gain in disease recognition accuracy, +33.3% in agricultural knowledge QA, and +26.10-point improvement in cross-domain generalization over standard fine-tuning.

Conclusion: The synergy between structured reasoning data and GRPO-driven exploration underpins performance gains, with benefits scaling as question complexity increases. The approach demonstrates efficient use of limited data and effective reasoning enhancement for agricultural applications.
...
TL;DR: DB-MSMUNet is a dual-branch multi-scale Mamba UNet architecture for pancreatic segmentation in CT scans, achieving state-of-the-art performance on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Pancreatic segmentation in CT scans is challenging due to low tissue contrast, blurry boundaries, irregular shapes, and small lesion sizes, requiring specialized methods for accurate diagnosis and treatment of pancreatic cancer.

Method: Proposes DB-MSMUNet with encoder using Multi-scale Mamba Module (MSMM) combining deformable convolutions and state space modeling, dual-decoder design (edge decoder with Edge Enhancement Path and area decoder with Multi-layer Decoder), and Auxiliary Deep Supervision heads.

Result: Achieves Dice Similarity Coefficients of 89.47% (NIH), 87.59% (MSD), and 89.02% (clinical tumor dataset), outperforming most existing state-of-the-art methods in segmentation accuracy, edge preservation, and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Infrared videos suffer from atmospheric turbulence and compression degradation, but existing methods either ignore the infrared-visible modality gap or fail to handle turbulence distortions. Cascading turbulence mitigation with super-resolution leads to error propagation due to decoupled degradation modeling.

Method: HATIR injects heat-aware deformation priors into diffusion sampling to jointly model turbulence degradation and detail loss. It uses a Phasor-Guided Flow Estimator based on thermal activity consistency, and a Turbulence-Aware Decoder with gating and attention mechanisms for selective feature aggregation.

Result: The authors built FLIR-IVSR, the first turbulent infrared VSR dataset with 640 diverse scenes from a FLIR T1050sc camera (1024×768), enabling future research in this area.
...
TL;DR: WebCryptoAgent: An agentic trading framework that decomposes web-informed cryptocurrency trading into modality-specific agents for better decision-making and separates strategic reasoning from real-time risk control to handle market shocks.


<details>
  <summary>Details</summary>
Motivation: Cryptocurrency trading requires timely integration of heterogeneous web information and market microstructure signals, but existing systems struggle with: 1) jointly reasoning over noisy multi-source web evidence while maintaining robustness, and 2) handling rapid price shocks at sub-second timescales due to slow deliberative reasoning pipelines.

Method: Proposes WebCryptoAgent with two key innovations: 1) decomposes web-informed decision making into modality-specific agents (unstructured web content, social sentiment, structured OHLCV signals) and consolidates outputs into unified evidence documents for confidence-calibrated reasoning; 2) introduces decoupled control architecture separating strategic hourly reasoning from real-time second-level risk model for fast shock detection and protective intervention independent of trading loop.
...
Conclusion: WebCryptoAgent effectively addresses the dual challenges of synthesizing heterogeneous web evidence and maintaining robustness to rapid market shocks through its agentic decomposition and decoupled control architecture, offering improved performance for cryptocurrency trading systems.
...
TL;DR: Forge-and-Quench is a unified framework that leverages multimodal understanding models to enhance text-to-image generation by creating a Bridge Feature that transfers insights from understanding to generation, improving fidelity and detail richness.


<details>
  <summary>Details</summary>
Motivation: While integrating image generation and understanding is a key goal in multimodal AI, how understanding can effectively assist generation hasn't been fully explored. Previous works focused on reasoning abilities and world knowledge, but this paper aims to leverage understanding to enhance the fidelity and detail richness of generated images.

Method: Proposes Forge-and-Quench framework where: 1) MLLM reasons over conversational context to produce enhanced text instruction, 2) Bridge Adapter maps this to a virtual visual representation called Bridge Feature, 3) This feature is injected into T2I backbone as visual guidance alongside enhanced text instruction, forging insights from understanding to refine generation.
...
Conclusion: Forge-and-Quench successfully demonstrates how understanding models can enhance image generation through a novel bridging mechanism, creating a unified framework that improves generation quality without compromising understanding capabilities, with practical benefits in extensibility and training efficiency.
...
TL;DR: HuForDet is a unified framework for detecting both facial and full-body human image forgeries using a dual-branch approach with RGB/frequency domain analysis and MLLM-based semantic consistency checking.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection methods are fragmented - they specialize either in facial forgeries or full-body synthetic images, failing to generalize across the full spectrum of human image manipulations. The rapid advancement of AI-generated content has escalated the threat of deepfakes beyond just faces to entire photorealistic human bodies.
...
Conclusion: HuForDet provides a holistic solution for human image forgery detection that addresses the fragmentation in existing methods by combining specialized facial analysis with full-body semantic consistency checking, demonstrating superior performance across various types of human image manipulations.
...
TL;DR: This paper evaluates CNN architectures across five diverse visual classification tasks, comparing custom lightweight CNNs with established models like ResNet-18 and VGG-16, analyzing transfer learning effectiveness in data-constrained real-world applications.


<details>
  <summary>Details</summary>
Motivation: To investigate CNN effectiveness across heterogeneous real-world visual classification tasks in agricultural and urban domains, addressing challenges like varying illumination, resolution, environmental complexity, and class imbalance that require adaptable learning models.

Method: Evaluated a lightweight custom CNN alongside established architectures (ResNet-18, VGG-16) using both training from scratch and transfer learning. Applied systematic preprocessing, augmentation, and controlled experimentation across five datasets: mango variety classification, paddy variety identification, road surface condition assessment, auto-rickshaw detection, and footpath encroachment monitoring.

Result: Developed an efficient custom CNN achieving competitive performance across multiple domains. Comprehensive analysis revealed when transfer learning and deep architectures provide substantial advantages, particularly in data-constrained environments.

Conclusion: The findings offer practical insights for deploying deep learning models in resource-limited yet high-impact real-world visual classification tasks, demonstrating the value of both custom lightweight architectures and transfer learning approaches depending on data constraints and application requirements.
...
TL;DR: AIVD framework enables precise object localization and semantic generation by combining lightweight edge detectors with cloud-based MLLMs, using efficient fine-tuning and dynamic scheduling for edge-cloud deployment.


<details>
  <summary>Details</summary>
Motivation: MLLMs have strong semantic understanding but struggle with precise object localization and deployment on resource-constrained edge devices. There's a need for a solution that combines accurate localization with high-quality semantic generation while addressing edge-cloud deployment challenges.

Method: Proposes AIVD framework with: 1) Collaboration between lightweight edge detectors (for precise localization) and cloud-based MLLMs (for semantic generation), 2) Efficient fine-tuning strategy with visual-semantic collaborative augmentation to improve MLLM robustness against edge noise, 3) Heterogeneous resource-aware dynamic scheduling algorithm for optimal throughput and latency across diverse edge devices and network conditions.

Result: AIVD significantly reduces resource consumption while improving MLLM classification accuracy and semantic generation quality. The scheduling strategy achieves higher throughput and lower latency across diverse scenarios compared to existing approaches.

Conclusion: The AIVD framework successfully addresses MLLM limitations in precise localization and edge-cloud deployment through collaborative edge-cloud architecture, robust fine-tuning, and intelligent scheduling, enabling practical deployment of multimodal AI in resource-constrained environments.
...
TL;DR: Novel adversarial attack using skeletonization to target foundation models' visual capabilities, especially on text/math images, with evaluation of character/semantic changes and practical demonstration on ChatGPT.


<details>
  <summary>Details</summary>
Motivation: To explore the visual capabilities and limitations of foundation models by developing an adversarial attack method that can effectively test their visual interpretation and reasoning abilities, particularly on challenging content like mathematical formulas.

Method: Introduces a novel adversarial attack method that utilizes skeletonization to reduce the search space effectively. Specifically targets images containing text, especially mathematical formula images, which are challenging due to LaTeX conversion and intricate structure.

Result: Conducted detailed evaluation of both character and semantic changes between original and adversarially perturbed outputs to provide insights into models' visual interpretation abilities. Demonstrated effectiveness through application to ChatGPT, showing practical implications in real-world scenarios.

Conclusion: The skeletonization-based adversarial attack method effectively probes foundation models' visual capabilities, revealing insights about their interpretation and reasoning abilities, with practical implications demonstrated through successful application to ChatGPT.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient open-vocabulary 3D scene understanding with 3D Gaussian Splatting while addressing cross-view consistency and intra-mask cohesion issues, without requiring render-supervised fine-tuning or pretrained 3DGS scenes.

Method: Uses dense correspondence-guided pre-registration to initialize Gaussians with accurate geometry, jointly constructs 3D Context Proposals via cross-view clustering, and fuses global features onto Gaussians during direct registration to maintain language coherence across views.
...
Conclusion: ProFuse provides an efficient, context-aware framework for open-vocabulary 3D scene understanding that enhances cross-view consistency and intra-mask cohesion with minimal overhead and no render-supervised fine-tuning requirements.
...
<details>
  <summary>Details</summary>
Motivation: Existing monocular shape-from-polarization methods suffer from azimuth angle ambiguity, an inherent limitation of polarization analysis that severely compromises reconstruction accuracy and stability. This ambiguity needs to be addressed to improve 3D reconstruction from single-view polarized images.

Method: Proposes SMSfP framework with two key components: 1) Polarization-aided adaptive region growing (PARG) segmentation strategy that decomposes global convexity assumption into locally convex regions, and 2) Multi-scale fusion convexity prior (MFCP) constraint to ensure local surface consistency and enhance fine detail recovery.
...
Conclusion: The segmentation-driven approach effectively suppresses azimuth ambiguities and preserves surface continuity, providing a more robust and accurate framework for monocular shape-from-polarization reconstruction.
...
TL;DR: GeM-VG is a Multimodal Large Language Model for generalized multi-image visual grounding that outperforms previous methods on multiple benchmarks while maintaining strong multi-image understanding capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing multi-image grounding methods are limited to single-target localization and few practical tasks due to lack of unified modeling for generalized grounding tasks. There's a need for a more comprehensive approach to multi-image visual grounding.

Method: Proposes GeM-VG with: 1) Systematic categorization of multi-image grounding tasks by cross-image cues and reasoning, 2) MG-Data-240K dataset addressing target quantity and image relation limitations, 3) Hybrid reinforcement finetuning strategy combining chain-of-thought reasoning and direct answering using R1-like algorithm with rule-based rewards.
...
Conclusion: GeM-VG demonstrates superior generalized grounding capabilities across diverse multi-image tasks while preserving strong multi-image understanding, addressing limitations of previous methods through unified modeling and comprehensive training strategies.
...
TL;DR: A framework for generating counterfactual videos to address hallucinations in video-language models, creating synthetic preference pairs for improved training via unified preference optimization.


<details>
  <summary>Details</summary>
Motivation: Video-language models suffer from hallucinations, particularly in action and temporal reasoning, due to over-reliance on language priors rather than visual dynamics. Existing mitigation strategies fail to address the root cause.

Method: Propose scalable counterfactual video generation framework combining multimodal LLMs for action proposal/editing guidance with diffusion-based image/video models. Build CounterVid dataset (~26k preference pairs) and introduce MixDPO - unified Direct Preference Optimization approach leveraging both textual and visual preferences.
...
Conclusion: Counterfactual video generation provides effective hard negatives for addressing VLM hallucinations, with MixDPO enabling unified preference optimization that improves temporal reasoning capabilities.
...
TL;DR: The paper validates that Gaussian model is accurate for defocus-based depth estimation in most imaging devices, showing less than 1% error.


<details>
  <summary>Details</summary>
Motivation: Depth estimation from 2D images remains challenging. While defocus provides depth information, accurately modeling defocus blur is difficult due to the ill-posed nature of distinguishing desired blur from inherent blur. A proper defocus model is needed for reliable depth estimation.

Method: The paper analyzes defocus within geometric optics framework and uses defocus aberration theory in diffraction-limited optics. It introduces camera settings for conventional imaging devices to ensure defocus operator follows Gaussian model, then evaluates accuracy of fitting actual model to Gaussian approximation.

Result: For typical focused depths between 1-100 meters with maximum depth variation of 10% at focused depth, results confirm Gaussian model's applicability for most imaging devices. Maximum Mean Absolute Error (MAE) is less than 1%, demonstrating high accuracy and reliability.

Conclusion: The Gaussian model is validated as an optimal choice for defocus-based depth estimation due to its mathematical simplicity, computational efficiency, and proven accuracy (less than 1% error) for most practical imaging scenarios.
...
<details>
  <summary>Details</summary>
Motivation: MRI has clinical limitations including long acquisition time, high cost, and restricted resolution. Image translation can address these issues, but existing Pix2Pix methods haven't been fully optimized for medical applications.

Method: Proposed enhanced Pix2Pix framework integrating Squeeze-and-Excitation Residual Networks (SEResNet) for channel attention and U-Net++ for multi-scale feature fusion, with simplified PatchGAN discriminator for stable training.

Result: Achieves consistent structural fidelity and superior image quality across multiple intra-modality MRI translation tasks under few-shot conditions (fewer than 500 images), demonstrating strong generalization ability.

Conclusion: The proposed method effectively extends Pix2Pix for medical image translation, offering improved performance for MRI applications with limited training data.
...
<details>
  <summary>Details</summary>
Motivation: Existing latent diffusion inverse solvers suffer from instability, artifacts, and degraded quality due to discrepancy between solver's and true reverse diffusion dynamics. The paper aims to create more stable and robust zero-shot inverse problem solvers.

Method: Proposes Measurement-Consistent Langevin Corrector (MCLC), a theoretically grounded plug-and-play module that remedies LDM-based inverse solvers through measurement-consistent Langevin updates without relying on linear manifold assumptions.

Result: MCLC demonstrates effectiveness and compatibility with existing solvers across diverse image restoration tasks, reduces instability and artifacts, and provides insights into blob artifact causes.

Conclusion: MCLC is a key step toward more robust zero-shot inverse problem solvers, offering a theoretically grounded solution that operates without restrictive linear manifold assumptions and stabilizes latent diffusion inverse solvers.
...
TL;DR: A pipeline to convert pretrained diffusion models into pyramidal models via low-cost finetuning, maintaining output quality while reducing computational cost, with exploration of step distillation strategies for further efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Existing pyramidal video models trained from scratch underperform compared to state-of-the-art systems in visual quality, and there's a need to leverage pretrained diffusion models more effectively while maintaining computational efficiency.

Method: Develop a pipeline to convert pretrained diffusion models into pyramidal models through low-cost finetuning, and investigate various step distillation strategies within pyramidal models to enhance inference efficiency.

Result: Successfully converted pretrained diffusion models into pyramidal models without degradation in output video quality, achieving computational efficiency improvements while maintaining visual plausibility comparable to state-of-the-art systems.

Conclusion: The proposed pipeline enables efficient transformation of existing diffusion models into pyramidal architectures, offering a practical approach to reduce inference costs while preserving quality, with step distillation strategies providing additional efficiency benefits.
...
<details>
  <summary>Details</summary>
Motivation: There's a critical need for robust long-term drone tracking in surveillance systems, but current RGB-based drone tracking methods are limited and rely on conventional motion models. Foundation models like SAMURAI show strong category-agnostic tracking performance in other domains but haven't been evaluated for drone-specific scenarios.

Method: The paper presents the first systematic evaluation of SAMURAI for drone tracking and introduces a detector-augmented extension of SAMURAI to mitigate sensitivity to bounding-box initialization and sequence length. This extension incorporates detector cues to enhance tracking robustness.

Result: The detector-augmented extension significantly improves robustness in complex urban environments, especially for long-duration sequences and drone exit-re-entry events. It achieves consistent gains over SAMURAI's zero-shot performance with success rate improvements up to +0.393 and FNR reductions up to -0.475 across datasets and metrics.

Conclusion: Foundation models like SAMURAI have strong potential for drone tracking, and augmenting them with detector cues significantly enhances their robustness and performance in challenging urban surveillance scenarios, particularly for long-term tracking with complex events like exit-re-entry.
...
TL;DR: Proposes binarization and preprocessing techniques to enhance degraded ancient script images, improving readability and classification accuracy for stone, metal, and document inscriptions.
...
Result: Achieved classification accuracies of 55.7%, 62%, and 65.6% for stone, metal plate, and document scripts using K-NN classifier; 53.2%, 59.5%, and 67.8% using SVM classifier.
...
TL;DR: SOVABench is a new surveillance video retrieval benchmark focusing on vehicle actions, with two evaluation protocols for cross-action discrimination and temporal understanding. The paper also introduces a training-free MLLM framework that generates interpretable embeddings from descriptions, achieving strong performance on SOVABench and other challenging benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing video retrieval benchmarks focus on scene-level similarity but lack evaluation of action discrimination needed for surveillance applications. There's a gap in benchmarks that specifically assess vehicle-related action understanding in real-world surveillance contexts.

Method: 1) Created SOVABench from real surveillance footage with vehicle actions; 2) Defined two evaluation protocols (inter-pair for cross-action discrimination, intra-pair for temporal direction); 3) Developed training-free MLLM framework that generates interpretable embeddings from MLLM-generated descriptions for images/videos.

Result: The MLLM framework achieves strong performance on SOVABench and outperforms contrastive Vision-Language Models on spatial and counting benchmarks where they typically fail. Action distinctions that are intuitive for humans remain challenging for state-of-the-art vision and multimodal models.

Conclusion: SOVABench addresses the need for action-focused surveillance benchmarks, and the proposed MLLM framework provides effective training-free solution for interpretable embeddings that handle challenging action discrimination tasks. The benchmark and framework are publicly available.
...
TL;DR: This paper presents a YOLO-based approach for scribe identification in medieval manuscripts, improving upon previous template matching and CNN methods by detecting more letters and enabling reliable writer identification in unseen manuscripts.


<details>
  <summary>Details</summary>
Motivation: Paleography faces challenges in dating manuscripts and understanding script evolution through scribe identification. While digital technologies have advanced, the general problem remains unsolved. Previous template matching approaches had limitations requiring appropriate thresholds, motivating exploration of more robust object detection methods.

Method: The authors replaced their previous template matching and CNN approach with YOLOv5 object detection model. They focused on detecting the letter "a" as a distinctive character present across all manuscript pages. YOLO identifies letter instances, and the confidence scores enable developing a rejection threshold system for reliable writer attribution.

Result: YOLO effectively extracts a greater number of letters compared to previous methods, leading to more accurate second-stage classification. The YOLO confidence score provides a foundation for developing a system with rejection thresholds, enabling reliable writer identification even in unseen manuscripts.

Conclusion: The YOLO-based approach represents a significant improvement over previous template matching methods for scribe identification in medieval manuscripts, offering better letter detection and enabling more reliable writer attribution through confidence-based rejection thresholds.
...
TL;DR: DivAS is an optimization-free, interactive framework for segmenting Neural Radiance Fields (NeRFs) using 2D SAM masks refined with NeRF depth priors, achieving real-time performance without per-scene training.
...
Method: DivAS uses a GUI-based workflow where 2D SAM masks from user point prompts are refined using NeRF-derived depth priors, then aggregated into a unified 3D voxel grid via a custom CUDA kernel in under 200ms.

Result: DivAS achieves segmentation quality comparable to optimization-based methods while being 2-2.5x faster end-to-end, and up to an order of magnitude faster when excluding user prompting time.

Conclusion: DivAS provides an optimization-free, fully interactive framework for NeRF segmentation that maintains real-time performance while matching the quality of slower optimization-based approaches.
...
TL;DR: Industrial framework for processing 200K+ PDFs, 25K+ videos, and multilingual audio in pharmaceutical domains, with analysis of 40+ VLMs showing 3-8x efficiency gains and multimodality benefits for long-form video reasoning.
...
Method: Developed industrial GenAI framework for multimodal reasoning, empirically analyzed over 40 VLMs on Video-MME and MMBench benchmarks plus proprietary dataset of 25,326 videos across 14 disease areas, focusing on long-form video processing constraints.

Result: Achieved 3-8x efficiency gains with SDPA attention on commodity GPUs; multimodality improved performance in 8/12 task domains (especially length-dependent tasks); identified bottlenecks in temporal alignment and keyframe detection across open- and closed-source VLMs.

Conclusion: Rather than proposing new models, this work characterizes practical limits, trade-offs, and failure patterns of current VLMs under realistic deployment constraints, providing actionable guidance for designing scalable multimodal systems for long-form video understanding in industrial domains.
...
<details>
  <summary>Details</summary>
Motivation: To develop rotation-robust learning methods for image inputs using Convolutional Model Trees that can handle geometric transformations at deployment time, particularly addressing the challenge of in-plane rotations.

Method: Use Convolutional Model Trees with three geometry-aware inductive biases for split directions: convolutional smoothing, tilt dominance constraint, and importance-based pruning. Also implement deployment-time orientation search that selects discrete rotations maximizing forest-level confidence without updating parameters.

Result: Orientation search improves robustness under severe rotations but can be harmful near canonical orientation when confidence is misaligned with correctness. Consistent trends observed on MNIST digit recognition tasks.

Conclusion: The study highlights both promise and limitations of confidence-based orientation selection for model-tree ensembles, showing that geometry-aware inductive biases and orientation search can improve rotation robustness but require careful handling of confidence-calibration issues.
...
TL;DR: The paper introduces ProtoBias benchmark to evaluate prototypicality bias in text-to-image metrics, showing current metrics often favor prototypical images over semantically correct ones, and proposes ProtoScore as a more robust alternative.


<details>
  <summary>Details</summary>
Motivation: Automatic metrics for text-to-image evaluation often substitute for human judgment, but it's unclear whether they prioritize semantic correctness or favor visually/socially prototypical images learned from biased data distributions. The paper aims to study systematic prototypicality bias in multimodal evaluation.

Method: The authors introduce ProtoBias benchmark with controlled contrastive pairs spanning Animals, Objects, and Demography images. Each pair contains a semantically correct but non-prototypical image and a subtly incorrect yet prototypical adversarial counterpart. They evaluate widely used metrics (CLIPScore, PickScore, VQA-based scores) and LLM-as-Judge systems on this benchmark.

Result: Current metrics frequently misrank the contrastive pairs, favoring prototypical but incorrect images over semantically correct non-prototypical ones. LLM-as-Judge systems show uneven robustness, especially in socially grounded cases. Human evaluations consistently favor semantic correctness with larger margins.

Conclusion: The paper proposes ProtoScore, a robust 7B-parameter metric that substantially reduces failure rates and suppresses misranking while being computationally efficient. It approaches the robustness of much larger closed-source judges while running orders of magnitude faster than GPT-5 inference time.
...
TL;DR: TEA: A temporal adaptive SITS semantic segmentation method that enhances model resilience under varying sequence lengths by using teacher-student knowledge transfer and full-sequence reconstruction.
...
Method: Proposes TEA with teacher model encapsulating global sequence knowledge to guide student model with adaptive temporal input lengths. Uses intermediate embedding, prototypes, and soft labels for knowledge transfer, with dynamic aggregation to mitigate forgetting. Includes full-sequence reconstruction as auxiliary task.
...
TL;DR: SparseLaneSTP: A sparse lane transformer that integrates lane geometry priors and temporal information for 3D lane detection, achieving SOTA performance with a new auto-labeled dataset.


<details>
  <summary>Details</summary>
Motivation: Existing 3D lane detection methods have limitations: dense BEV methods suffer from poor feature representation due to erroneous transformations, sparse methods ignore valuable lane-specific priors, and no methods utilize historical lane observations to resolve visibility ambiguities.

Method: SparseLaneSTP integrates geometric lane structure properties and temporal information into a sparse lane transformer. Key innovations include: lane-specific spatio-temporal attention mechanism, continuous lane representation for sparse architectures, temporal regularization, and a new auto-labeled 3D lane dataset to address weaknesses in existing datasets.

Result: Achieves state-of-the-art performance across all detection and error metrics on existing 3D lane detection benchmarks and on the novel auto-labeled dataset.

Conclusion: The integration of lane-specific priors and temporal information in a sparse transformer architecture significantly improves 3D lane detection, with the new dataset providing more precise and consistent ground truth for evaluation.
...
TL;DR: OceanSplat: A 3D Gaussian Splatting method for underwater scene reconstruction that addresses optical degradation through trinocular view consistency, synthetic epipolar depth priors, and depth-aware alpha adjustment to reduce floating artifacts.


<details>
  <summary>Details</summary>
Motivation: Underwater scenes suffer from optical degradation (scattering, absorption) that causes multi-view inconsistencies in 3D reconstruction, leading to floating artifacts and poor geometry representation. Existing methods struggle to separate object geometry from the scattering medium.

Method: 1) Enforces trinocular view consistency by rendering horizontally/vertically translated camera views and aligning via inverse warping. 2) Uses translated views to derive synthetic epipolar depth prior through triangulation as self-supervised depth regularizer. 3) Proposes depth-aware alpha adjustment that modulates 3D Gaussian opacity based on z-component and viewing direction during early training.

Result: OceanSplat outperforms existing methods on both real-world underwater and simulated scenes for scene reconstruction and restoration. It disentangles 3D Gaussians from scattering medium, reduces floating artifacts, and enables robust object geometry representation.

Conclusion: The proposed geometric constraints and depth-aware regularization successfully address underwater optical degradation challenges, enabling accurate 3D Gaussian Splatting in scattering media with significantly improved reconstruction quality and reduced artifacts.
...
TL;DR: Higher-order adversarial attacks on object detectors show stronger generalization than lower-order attacks, and adversarial training alone is insufficient defense.


<details>
  <summary>Details</summary>
Motivation: To investigate the impact of higher-order adversarial attacks on object detectors, examining the cat-and-mouse game between attack patterns and adversarial training defenses.
...
Conclusion: Higher-order adversarial attacks pose a significant threat to object detectors with enhanced generalization, requiring more robust defenses beyond standard adversarial training.
...
Method: PolyFit learns a patch-based surface representation by fitting jet functions locally on surface patches. This supervised learning approach works with both analytic functions and real data. The learned representation allows surface deformation through updating a compact set of jet coefficients rather than per-vertex optimization.

Result: 1) Shape-from-template: Achieves competitive accuracy with test-time optimization while being significantly faster than offline physics-based solvers, and outperforms physics-guided neural simulators in accuracy at modest additional runtime. 2) Garment draping: A self-supervised, mesh- and garment-agnostic model generalizes across resolutions and garment types, delivering up to 10x faster inference than strong baselines.

Conclusion: PolyFit provides an efficient patch-based surface representation that enables fast and accurate surface deformation for various computer vision and graphics applications, offering significant speed advantages over existing methods while maintaining or improving accuracy.
...
TL;DR: Domain-adapted Video to Video Clip Generation framework using ALMs and VLMs for pharmaceutical industry video summarization with 3-4x speedup and 4x cost reduction.


<details>
  <summary>Details</summary>
Motivation: Traditional manual annotation of heterogeneous pharmaceutical data (text, images, video, audio, web links) is inconsistent, inefficient, and struggles with long content like clinical trial interviews and educational seminars. VLMs can revolutionize pharmaceutical digital transformation by enabling intelligent, scalable multi-modality content processing.

Method: Three main contributions: (1) Cut & Merge algorithm with fade in/out and timestamp normalization for smooth transitions and audio/visual alignment; (2) Personalization mechanism based on role definition and prompt injection for tailored outputs (marketing, training, regulatory); (3) Cost-efficient end-to-end pipeline strategy balancing ALM/VLM enhanced processing.

Result: Evaluations on Video MME benchmark (900 videos) and proprietary dataset of 16,159 pharmacy videos across 14 disease areas show 3-4 times speedup, 4 times cost reduction, and competitive clip quality. Improved clip coherence scores (0.348) and informativeness scores (0.721) over state-of-the-art VLM baselines like Gemini 2.5 Pro.

Conclusion: The framework demonstrates potential for transparent, custom extractive, and compliance-supporting video summarization for life sciences, offering significant efficiency gains while maintaining quality over traditional manual methods and existing VLM baselines.
...
TL;DR: DrivoR is a simple transformer-based architecture for autonomous driving that uses camera-aware register tokens to compress multi-camera features, then uses lightweight decoders to generate and score trajectories with interpretable sub-scores for safety, comfort, and efficiency.


<details>
  <summary>Details</summary>
Motivation: To create an efficient end-to-end autonomous driving system that reduces computational complexity while maintaining accuracy, and enables behavior-conditioned driving with interpretable performance metrics.

Method: Builds on pretrained Vision Transformers with camera-aware register tokens that compress multi-camera features. Uses two lightweight transformer decoders: one generates candidate trajectories, and another scores them by mimicking an oracle with interpretable sub-scores (safety, comfort, efficiency).

Result: Outperforms or matches strong contemporary baselines across NAVSIM-v1, NAVSIM-v2, and HUGSIM benchmarks. Shows that pure-transformer architecture with targeted token compression is sufficient for accurate, efficient, and adaptive end-to-end driving.

Conclusion: DrivoR demonstrates that a minimal transformer-based design with camera-aware register tokens and interpretable trajectory scoring can achieve state-of-the-art performance in autonomous driving while being computationally efficient and enabling behavior-conditioned driving.
...
<details>
  <summary>Details</summary>
Motivation: Unlabeled LiDAR logs in autonomous driving contain valuable 3D geometry but are useless without expensive human labels, creating a major cost barrier for perception research. The paper aims to overcome this bottleneck by leveraging temporal consistency to automatically generate 3D annotations.

Method: Uses unsupervised multi-modal pseudo-labeling with geometric priors learned from temporally accumulated LiDAR maps. Features a novel iterative update rule enforcing joint geometric-semantic consistency, and detects moving objects from inconsistencies. Lifts cues from text and 2D vision foundation models directly into 3D without manual input.

Result: Method produces 3D semantic labels, 3D bounding boxes, and dense LiDAR scans with robust generalization across three datasets. Outperforms existing pseudo-labeling methods that require manual supervision. Even a small fraction of the geometrically consistent, densified LiDAR improves depth prediction by 51.5% and 22.0% MAE in the 80-150m and 150-250m ranges respectively.

Conclusion: The approach successfully addresses the labeling bottleneck in autonomous perception by leveraging temporal-geometric consistency to automatically generate high-quality 3D annotations from unlabeled LiDAR data, demonstrating significant improvements in downstream tasks like depth prediction.
...
TL;DR: The paper proposes projective conditioning for view synthesis, replacing camera parameters with stable 2D projective cues to improve geometric consistency and robustness, along with masked autoencoding pretraining for uncalibrated data.
...
Method: Proposes projective conditioning which replaces raw camera parameters with target-view projective cues (stable 2D inputs), reframing the task from brittle geometric regression in ray space to well-conditioned image-to-image translation. Also introduces masked autoencoding pretraining tailored to these cues for large-scale uncalibrated data.

Result: The method shows improved fidelity and stronger cross-view consistency compared to ray-conditioned baselines on a view-consistency benchmark, and achieves state-of-the-art quality on standard novel view synthesis benchmarks.

Conclusion: Projective conditioning provides more stable and robust conditioning for view synthesis than traditional ray-based approaches, enabling better geometric consistency and leveraging uncalibrated data through tailored pretraining strategies.
...
Conclusion: Re-Align successfully bridges the understanding-generation gap through structured reasoning-guided alignment, providing a unified framework for precise in-context image generation and editing.
...
TL;DR: VERSE is a methodology for analyzing and improving Vision-Language Models for document understanding by visualizing their visual embedding space to identify problematic regions and generate targeted synthetic data for retraining.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models for Visually-rich Document Understanding lack interpretability and methods for targeted improvement. There's a need to understand why models fail on certain visual features and how to systematically enhance performance without compromising generalization.

Method: VERSE analyzes the visual embedding space of models to visualize latent representations and identify problematic clusters. It then guides generation of synthetic data containing the visual features associated with error-prone regions, enabling targeted retraining to address specific weaknesses.

Result: VERSE successfully uncovers visual features linked to error-prone clusters. Retraining with synthetic samples containing these features substantially boosts F1 performance without degrading generalization. On-premise models (Donut, Idefics2) optimized with VERSE match or surpass SaaS solutions (GPT-4, Pixtral).

Conclusion: VERSE provides an effective methodology for analyzing and improving Vision-Language Models for document understanding through interpretable visualization of embedding spaces and targeted synthetic data generation, enabling on-premise models to achieve state-of-the-art performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing video world models struggle to provide unified and precise control over both camera and multi-object motion, as videos operate in the 2D image plane rather than 3D space. There's also a scarcity of large-scale training data with explicit 4D annotations.

Method: Introduces a 4D Geometric Control representation encoding world state through static background point clouds and per-object 3D Gaussian trajectories. These 4D controls are rendered into conditioning signals for a pretrained video diffusion model. Also develops an automatic data engine to extract 4D controls from in-the-wild videos for training.

Result: Enables generation of high-fidelity, view-consistent videos that precisely adhere to specified camera and object dynamics. The method provides a flexible, category-agnostic alternative to rigid bounding boxes or parametric models.

Conclusion: VerseCrafter bridges the gap between 2D video generation and 4D geometric control, offering unified and precise manipulation of camera and object motion through a novel 4D representation and scalable training approach.
...
TL;DR: A lightweight vision-language framework for crop disease identification from leaf images using Swin Transformer encoder and sequence decoders, achieving high accuracy with fewer parameters than large-scale baselines.
...
TL;DR: Atlas 2 series pathology foundation models achieve state-of-the-art performance, robustness, and efficiency across 80 benchmarks, trained on 5.5M histopathology slides from three major medical institutions.
...
Result: The models demonstrate state-of-the-art performance in prediction performance, robustness, and resource efficiency across a comprehensive evaluation of eighty public benchmarks.

Conclusion: The Atlas 2 series successfully bridges the shortcomings of previous pathology foundation models, offering improved performance, robustness, and computational efficiency that could facilitate clinical deployment.
...
Method: Multi-scale local speculative decoding with low-resolution drafter and learned up-samplers to propose candidate tokens, verified by high-resolution target model. Includes local rejection and resampling mechanism focusing on spatial neighborhoods rather than raster-scan resampling.
...
Conclusion: Sets new SOTA in speculative decoding for image synthesis, bridging efficiency-fidelity gap. Multi-resolution drafting with spatially-aware verification enables significant acceleration while preserving quality.
...
TL;DR: VLI is a training-free inference framework that reduces object hallucination in Multimodal LLMs through metacognitive self-correction, achieving SOTA performance with 12.67% reduction in hallucination rates.
...
Method: VLI simulates metacognitive self-correction with two steps: 1) Attributive Introspection diagnoses hallucination risks via probabilistic conflict detection and localizes causal visual anchors; 2) Interpretable Bi-Causal Steering dynamically modulates inference by isolating visual evidence from noise and neutralizing blind confidence through adaptive calibration.
...
Conclusion: VLI provides an effective training-free framework for reducing object hallucination in MLLMs through introspective self-correction, addressing fundamental cognitive introspection failures while maintaining interpretability and instance-specific precision.
...
TL;DR: Chain-of-View (CoV) prompting enables VLMs to actively explore 3D scenes for embodied question answering by selecting relevant viewpoints and adjusting views iteratively, improving spatial reasoning without training.
...
Method: CoV prompting transforms VLMs into active viewpoint reasoners through coarse-to-fine exploration: 1) View Selection agent filters redundant frames and identifies anchor views, 2) Fine-grained view adjustment via iterative reasoning with discrete camera actions to gather sufficient context.

Result: Average +11.56% improvement in LLM-Match on OpenEQA across four VLMs (max +13.62% on Qwen3-VL-Flash). Test-time scaling yields additional +2.51% improvement with increased action budget. Strong performance on ScanQA (116 CIDEr/31.9 EM@1) and SQA3D (51.1 EM@1).
...
<details>
  <summary>Details</summary>
Motivation: Chain-of-thought reasoning is computationally expensive for video models, and direct answering often matches or surpasses CoT performance. The authors want to develop a more efficient approach that only uses reasoning when necessary.

Method: VideoAuto-R1 uses "Thinking Once, Answering Twice" paradigm: during training, model generates initial answer, performs reasoning, then outputs reviewed answer with both supervised via verifiable rewards. During inference, uses confidence score of initial answer to decide whether to reason.

Result: Achieves state-of-the-art accuracy on video QA and grounding benchmarks while reducing average response length by ~3.3x (from 149 to 44 tokens). Shows low thinking-mode activation on perception tasks but higher on reasoning-intensive tasks.

Conclusion: Explicit language-based reasoning is generally beneficial but not always necessary. VideoAuto-R1 provides an efficient "reason-when-necessary" framework that maintains accuracy while significantly reducing computational cost.
...
Conclusion: AgentCompress makes LLM-powered research more accessible to budget-constrained academic labs by significantly reducing computational costs with minimal performance impact.
...
TL;DR: VLMs increasingly hallucinate by favoring textual prompts over visual evidence as object counts increase, but specific attention heads can be ablated to reduce prompt-induced hallucinations by 40+% without training.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models often hallucinate by prioritizing textual prompts over visual evidence, creating a reliability issue. The researchers want to understand this failure mode in a controlled setting to identify internal mechanisms driving prompt-induced hallucinations.

Method: Used controlled object-counting experiments where prompts overstate actual object counts. Conducted mechanistic analysis of three VLMs, identifying specific attention heads responsible for prompt-induced hallucinations. Performed ablation studies on these heads to measure hallucination reduction.

Result: Found that VLMs correct prompt overestimation at low object counts but increasingly conform to incorrect prompts as object numbers increase. Identified a small set of attention heads whose ablation reduces prompt-induced hallucinations by at least 40% without additional training. Discovered model-specific differences in how these heads mediate prompt copying.

Conclusion: Prompt-induced hallucinations in VLMs are mediated by specific attention heads that can be targeted for intervention. The findings reveal model-specific implementation differences and provide insights into internal mechanisms driving hallucinations, offering potential pathways for improving VLM reliability.
...
<details>
  <summary>Details</summary>
Motivation: Existing feed-forward 3D reconstruction models suffer from blurry depth boundaries and flying-point artifacts that degrade reconstruction quality.

Method: MoE3D predicts multiple candidate depth maps and fuses them via dynamic weighting, using a mixture-of-experts approach to sharpen depth boundaries and mitigate artifacts.
...
Conclusion: MoE3D effectively improves 3D reconstruction quality by addressing depth boundary sharpness and artifact issues through an efficient mixture-of-experts fusion approach.
...
TL;DR: FlowLet is a conditional generative framework that synthesizes age-conditioned 3D MRIs using flow matching in an invertible 3D wavelet domain, improving brain age prediction performance for underrepresented age groups.


<details>
  <summary>Details</summary>
Motivation: Brain MRI datasets for age prediction are demographically skewed, limiting fairness and generalizability. Existing generative methods for data augmentation are slow, introduce artifacts, and lack age conditioning, affecting brain age prediction performance.

Method: FlowLet uses flow matching within an invertible 3D wavelet domain to synthesize age-conditioned 3D MRIs. This approach avoids reconstruction artifacts from latent compression and reduces computational demands while enabling conditional generation based on age.

Result: FlowLet generates high-fidelity 3D MRI volumes with few sampling steps. Training brain age prediction models with FlowLet-generated data improves performance for underrepresented age groups, and region-based analysis confirms preservation of anatomical structures.

Conclusion: FlowLet provides an efficient, high-fidelity solution for generating age-conditioned 3D MRIs that addresses dataset imbalance issues in brain age prediction, improving model fairness and performance through effective data augmentation.
...
<details>
  <summary>Details</summary>
Motivation: Humans can effortlessly anticipate object motions through interaction, but computational systems lack similar predictive abilities from passive visual observation. The goal is to enable systems to predict plausible future object motions directly from visual input.

Method: ObjectForesight uses explicit 3D object-level representations rather than pixel or latent space models. It leverages recent advances in segmentation, mesh reconstruction, and 3D pose estimation to curate a dataset of 2M+ short clips with pseudo-ground-truth 3D object trajectories for training at scale.
...
Conclusion: The work demonstrates a successful approach to endowing computational systems with human-like ability to predict object motions, using explicit 3D object representations and large-scale training data to create geometrically coherent and generalizable dynamics models.
...
TL;DR: PlenopticDreamer is a novel framework for multi-view consistent video re-rendering that synchronizes generative hallucinations using spatio-temporal memory, addressing limitations of existing single-view methods.
...
Method: Trains a multi-in-single-out video-conditioned model autoregressively with camera-guided video retrieval for adaptive conditional inputs, plus progressive context-scaling, self-conditioning, and long-video conditioning mechanisms.
...
Conclusion: PlenopticDreamer successfully addresses multi-view consistency challenges in video re-rendering through spatio-temporal memory synchronization, enabling robust and coherent generative video transformations across various applications.
...
Method: Introduces visual identity prompting that supplies exemplar images as conditioning inputs to guide diffusion models in generating desired scene setups. Also builds a scalable pipeline to curate a visual identity pool from large robotics datasets.
...
Conclusion: Visual identity prompting addresses limitations of text-only conditioning for robotics data augmentation, enabling generation of practical multi-view and temporally coherent observations that improve policy training effectiveness.
...
TL;DR: This paper introduces GREx (Generalized Referring Expression Segmentation/Comprehension/Generation) that extends classic REx to handle multi-target and no-target expressions, not just single-target ones. It presents gRefCOCO dataset and ReLA baseline method.


<details>
  <summary>Details</summary>
Motivation: Existing RES/REC/REG datasets and methods only support single-target expressions (one expression refers to one object), which limits real-world applications where expressions may refer to multiple objects or no objects at all.

Method: Proposes GREx framework with gRefCOCO dataset containing multi-target, no-target, and single-target expressions. For GRES/GREC tasks, introduces ReLA baseline that adaptively divides images into regions with sub-instance clues and explicitly models region-region and region-language dependencies.
...
Method: 1) Pixel-Perfect Depth (PPD): Uses pixel-space diffusion transformers (DiT) with Semantics-Prompted DiT (incorporating semantic representations from vision foundation models) and Cascade DiT architecture (progressive token increase). 2) Pixel-Perfect Video Depth (PPVD): Extends PPD with Semantics-Consistent DiT (extracting temporally consistent semantics from multi-view geometry models) and reference-guided token propagation for temporal coherence.

Result: Achieves best performance among all generative monocular and video depth estimation models, producing significantly cleaner point clouds than all other models with flying-pixel-free results.

Conclusion: The proposed pixel-perfect visual geometry models successfully address flying pixel issues and preserve fine details through pixel-space generative modeling with semantic prompting and efficient cascade architectures, enabling high-quality geometry reconstruction for robotics and AR applications.
...
TL;DR: RL-AWB combines statistical methods with deep reinforcement learning for nighttime white balance, achieving superior generalization across lighting conditions with a new multi-sensor dataset.


<details>
  <summary>Details</summary>
Motivation: Nighttime color constancy is challenging due to low-light noise and complex illumination conditions. Existing methods struggle with generalization across different sensors and lighting scenarios.

Method: 1) Statistical algorithm for nighttime scenes with salient gray pixel detection and novel illumination estimation. 2) First deep reinforcement learning approach for color constancy that uses the statistical algorithm as core, mimicking professional AWB tuning experts by dynamically optimizing parameters per image.

Result: Method achieves superior generalization capability across low-light and well-illuminated images. Introduces first multi-sensor nighttime dataset for cross-sensor evaluation.

Conclusion: RL-AWB framework effectively addresses nighttime color constancy challenges by combining statistical methods with reinforcement learning, demonstrating strong generalization across different sensors and lighting conditions.
...
TL;DR: QNeRF introduces a hybrid quantum-classical model for novel-view synthesis that uses quantum circuits to encode spatial and view information, achieving comparable or better performance than classical NeRF with less than half the parameters.


<details>
  <summary>Details</summary>
Motivation: Quantum Visual Fields (QVFs) have shown promise for compact signal representation, while Neural Radiance Fields (NeRFs) advance novel-view synthesis but require large models and intensive training. The authors aim to combine quantum advantages with NeRF capabilities to create more compact, efficient 3D scene representations.

Method: QNeRF uses parameterized quantum circuits to encode spatial and view-dependent information through quantum superposition and entanglement. Two variants: Full QNeRF maximizes quantum amplitude usage for representation, while Dual-Branch QNeRF introduces task-informed inductive bias by separating spatial and view-dependent quantum state preparations to reduce complexity and improve scalability.

Result: When trained on moderate-resolution images, QNeRF matches or outperforms classical NeRF baselines while using less than half the number of parameters. The quantum approach demonstrates competitive performance for 3D representation learning from 2D observations.

Conclusion: Quantum machine learning can serve as a competitive alternative for continuous signal representation in mid-level computer vision tasks, particularly for 3D representation learning from 2D observations, offering more compact models with comparable or better performance.
...
TL;DR: Mesh4D: A feed-forward model for monocular 4D mesh reconstruction that creates 3D shape and motion from single videos using a compact latent space and skeletal-guided autoencoder.


<details>
  <summary>Details</summary>
Motivation: To reconstruct complete 3D shape and motion (4D mesh) from monocular videos of dynamic objects, addressing the challenge of capturing accurate deformation over time without requiring skeletal information at inference.

Method: 1) Learns compact latent space via autoencoder guided by skeletal structure during training only; 2) Uses spatio-temporal attention for stable deformation representation; 3) Trains latent diffusion model conditioned on input video and first-frame mesh to predict full animation in one shot.
...
Conclusion: Mesh4D enables efficient monocular 4D mesh reconstruction with strong deformation priors from skeletal guidance during training, while maintaining skeletal-free inference and one-shot animation prediction.
...
### [174] [ArtCognition: A Multimodal AI Framework for Affective State Sensing from Visual and Kinematic Drawing Cues](https://arxiv.org/abs/2601.04297)
*Behrad Binaei-Haghighi,Nafiseh Sadat Sajadi,Mehrad Liviyan,Reyhane Akhavan Kharazi,Fatemeh Amirkhani,Behnam Bahrak*

Main category: cs.LG

TL;DR: ArtCognition: A multimodal framework using digital drawing analysis (visual features + drawing kinematics) with RAG architecture for automated psychological assessment via House-Tree-Person test.


<details>
  <summary>Details</summary>
Motivation: Objective assessment of human affective states is challenging, especially through non-verbal channels. Digital drawing represents an underexplored modality for affective sensing that could provide rich psychological insights.

Method: Multimodal framework combining: 1) static visual features from final artwork using computer vision models, 2) dynamic behavioral kinematic cues from drawing process (stroke speed, pauses, smoothness), and 3) Retrieval-Augmented Generation (RAG) architecture to bridge low-level features with established psychological knowledge.

Result: Fusion of visual and behavioral kinematic cues provides more nuanced assessment than single modalities. Significant correlations found between multimodal features and standardized psychological metrics, validating framework as scalable clinical support tool.

Conclusion: ArtCognition contributes new methodology for non-intrusive affective state assessment and opens avenues for technology-assisted mental healthcare through automated analysis of psychological drawing tests.
...
### [175] [Aligned explanations in neural networks](https://arxiv.org/abs/2601.04378)
...
<details>
  <summary>Details</summary>
Motivation: Current feature attribution methods for explaining neural networks are mostly post-hoc rationalizations that don't truly reflect the model's prediction process. The authors argue that explanatory alignment (direct link between explanations and predictions) is crucial for trustworthiness in prediction tasks.

Method: PiNets (pseudo-linear networks) are proposed as a modeling framework that produces instance-wise linear predictions in arbitrary feature spaces. They are designed to be linearly readable, enabling direct alignment between explanations and predictions.

Result: PiNets are demonstrated on image classification and segmentation tasks, producing explanations that are faithful across multiple criteria in addition to achieving explanatory alignment.

Conclusion: Model readability should be a design principle for trustworthy AI systems, and PiNets provide a practical framework for achieving explanatory alignment in deep learning by making predictions linearly interpretable.
...
### [176] [IGenBench: Benchmarking the Reliability of Text-to-Infographic Generation](https://arxiv.org/abs/2601.04498)
...
TL;DR: IGENBENCH is the first benchmark for evaluating text-to-infographic generation reliability, revealing significant gaps in current T2I models' ability to produce accurate infographics despite their aesthetic appeal.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models can generate visually appealing infographics, but their reliability is unclear - they often contain subtle errors like distorted data encoding or incorrect text that are easily overlooked.

Method: Created IGENBENCH with 600 curated test cases across 30 infographic types, designed an automated evaluation framework decomposing reliability into atomic yes/no questions (10 question types), and used multimodal LLMs to verify each question.

Result: Evaluation of 10 state-of-the-art T2I models shows: (i) three-tier performance hierarchy with top model achieving Q-ACC 0.90 but I-ACC only 0.49; (ii) data-related dimensions are universal bottlenecks (Data Completeness: 0.21); (iii) end-to-end correctness remains challenging across all models.
...
### [177] [A Vision for Multisensory Intelligence: Sensing, Synergy, and Science](https://arxiv.org/abs/2601.04563)
*Paul Pu Liang*

Main category: cs.LG

TL;DR: Research vision for multisensory AI over the next decade, focusing on connecting AI to human senses and physical/social signals through sensing, science, and synergy themes.


<details>
  <summary>Details</summary>
Motivation: Current AI primarily advances in digital modalities (text, vision, audio), but human experience is multisensory. There's a need to connect AI to the full spectrum of human senses and physical/social signals to transform human-AI interaction.

Method: Proposes three interrelated research themes: 1) Sensing - extending AI's ability to capture richer world data beyond digital media, 2) Science - developing principled frameworks for quantifying multimodal heterogeneity, unified architectures, and cross-modal transfer, 3) Synergy - addressing technical challenges in multisensory integration, alignment, reasoning, generation, and human-AI interaction.
...
Conclusion: Multisensory AI represents a transformative direction that can fundamentally change how humans and AI experience and interact with each other by bridging the gap between digital AI and the rich multisensory nature of human experience.
...
TL;DR: Medical MLLMs have safety vulnerabilities, especially against cross-modality jailbreak attacks, and suffer from safety forgetting during medical fine-tuning. The paper proposes a Parameter-Space Intervention method to re-align safety without extra safety data.


<details>
  <summary>Details</summary>
Motivation: Medical MLLMs show remarkable progress but lack safety research, posing risks for real-world deployment. Current models have pervasive safety vulnerabilities and suffer from catastrophic forgetting of safety alignment during medical fine-tuning.

Method: 1) Establish multidimensional safety evaluation framework for Medical MLLMs. 2) Propose "Parameter-Space Intervention" approach that extracts intrinsic safety knowledge from base models and injects it during medical capability construction. 3) Design fine-grained parameter search algorithm for optimal safety-performance trade-off.

Result: Empirical analysis reveals pervasive safety vulnerabilities in existing Medical MLLMs, especially fragility against cross-modality jailbreak attacks. The proposed approach significantly improves safety guardrails without additional safety data while minimizing degradation to core medical performance.

Conclusion: Medical MLLMs have serious safety issues that need systematic addressing. The Parameter-Space Intervention method provides an effective solution for safety re-alignment without compromising medical capabilities, enabling safer deployment of Medical MLLMs in real-world applications.
...
TL;DR: Transformers trained for modular addition fail catastrophically when input format changes (position shifts or different templates), despite high in-distribution accuracy. A simple training recipe with boundary markers, position curriculum, diverse templates, and consistency training improves robustness.


<details>
  <summary>Details</summary>
Motivation: To study robustness of character-level Transformers under input-format variation rather than just in-distribution accuracy, focusing on how models trained for modular addition fail when expressions are shifted to different positions or presented with different templates.

Method: 1) Baseline model trained on modular addition with disjoint-pair split over ordered pairs for p=97; 2) Intervention training recipe combining: explicit expression boundary markers, position curriculum broadening absolute position range, diverse template mixtures, and consistency training across multiple variants per example.

Result: Baseline achieves high in-distribution accuracy but collapses under position shift and template OOD. The intervention training substantially improves robustness to both position shift and template OOD while maintaining high in-distribution accuracy. ALiBi-style ablation fails to learn the task.

Conclusion: Procedural generalization under noisy supervision benefits from explicitly training invariances absent from the data distribution. The paper provides reproducible evaluation protocol and artifacts for studying robustness to input-format variation.
...
### [180] [Quantifying the Effect of Test Set Contamination on Generative Evaluations](https://arxiv.org/abs/2601.04301)
...
TL;DR: Test set contamination significantly boosts generative model performance, enabling models to achieve lower loss than irreducible error of uncontaminated training. Contamination effects can be mitigated by overtrained fresh data, high sampling temperatures, and are harder for longer solutions.


<details>
  <summary>Details</summary>
Motivation: While test set contamination's impact on discriminative evaluations is well-studied, its effects on generative evaluations remain under-explored. As frontier AI systems train on web-scale data, contamination becomes critical for trustworthy capability assessment.

Method: Pretrained language models on mixtures of web data and MATH benchmark, varying model sizes and number of test set replicas. Used scaling laws to analyze contamination effects, studied further training (overtraining with fresh data, supervised finetuning), and examined inference factors like sampling temperature and solution length.

Result: Performance improves with contamination and model size. A single test set replica enables models to achieve lower loss than irreducible error of uncontaminated training. Overtraining with fresh data reduces contamination effects, while supervised finetuning effects depend on pretraining contamination levels. High sampling temperatures mitigate contamination, and longer solutions are exponentially harder to memorize.

Conclusion: Test set contamination significantly impacts generative evaluations differently than discriminative ones. Understanding generation-memorization interactions adds complexity to trustworthy AI evaluation, requiring careful consideration of contamination effects throughout the model lifecycle.
...
### [181] [Rate or Fate? RLV$^\varepsilon$R: Reinforcement Learning with Verifiable Noisy Rewards](https://arxiv.org/abs/2601.04411)
*Ali Rad,Khashayar Filom,Darioush Keivan,Peyman Mohajerin Esfahani,Ehsan Kamalinejad*

Main category: cs.LG

TL;DR: RLVR (Reinforcement Learning with Verifiable Rewards) faces verification noise in practice. The paper shows that learning success depends on Youden's index J=TPR-FPR: J>0 enables learning, J<0 causes collapse, J=0 is neutral. Noise affects convergence rate but not fate when J>0.


<details>
  <summary>Details</summary>
Motivation: Real-world RLVR applications suffer from imperfect verification (noisy unit tests, human labels, LLM judges). This noise worsens in hard domains like coding where tests are sparse. The paper investigates whether verification noise merely slows learning or fundamentally changes outcomes.

Method: Developed an analytically tractable multi-armed bandit model of RLVR dynamics using GRPO. Modeled false positives/negatives and grouped completions into reasoning modes, yielding replicator-style flow on probability simplex. The dynamics decouple into within-correct-mode competition and one-dimensional evolution for incorrect mode mass.

Result: Found sharp phase transition: when J=TPR-FPR>0, incorrect mass drives to extinction (learning); J=0 is neutral; J<0 causes incorrect modes to dominate (anti-learning/collapse). In learning regime (J>0), noise primarily rescales convergence time. Experiments on programming tasks with synthetic noise confirm J=0 boundary.

Conclusion: Verification noise affects RLVR convergence rate but not fate when J>0. The framework provides analytical lens for RLVR stability, convergence, and algorithmic interventions. Youden's index J serves as critical threshold determining learning success versus collapse.
...
### [182] [Not All Steps are Informative: On the Linearity of LLMs' RLVR Training](https://arxiv.org/abs/2601.04537)
...
<details>
  <summary>Details</summary>
Motivation: RLVR training for LLMs requires thousands of steps with substantial computation due to prolonged exploration. The authors discovered that LLMs evolve in a strongly linear manner during RLVR, suggesting that future model states might be predictable via extrapolation to avoid expensive continued training.

Method: The authors leverage the observed linearity in RLVR training where both model weights and output log-probabilities show strong linear correlations with training steps. They propose Weight Extrapolation to predict future model states from intermediate checkpoints, and Logits Extrapolation to extrapolate beyond stable RL training ranges.
...
Conclusion: The linear evolution of LLMs during RLVR enables efficient extrapolation techniques that can reduce computational costs while maintaining or even improving performance compared to standard RL training.
...
### [183] [On the Hidden Objective Biases of Group-based Reinforcement Learning](https://arxiv.org/abs/2601.05002)
*Aleksandar Fontana,Marco Simoni,Giulio Rossolini,Andrea Saracino,Paolo Mori*

Main category: cs.LG

TL;DR: GRPO-style RL methods have structural mismatches between reward optimization and training objectives, leading to systematic gradient biases, AdamW scaling insensitivity, and momentum-driven clipping violations.


<details>
  <summary>Details</summary>
Motivation: To understand the theoretical limitations of widely-used Group Relative Policy Optimization (GRPO) methods for post-training LLMs, despite their empirical success, by analyzing structural mismatches between reward optimization and training objectives.

Method: Theoretical analysis of GRPO-style methods within a unified surrogate formulation, examining recurring properties across methods including gradient biases, optimizer interactions, and momentum effects.

Result: Three key findings: (1) non-uniform group weighting causes systematic gradient biases on shared prefix tokens; (2) AdamW optimizer interactions make training dynamics largely insensitive to reward scaling; (3) optimizer momentum can push policy updates beyond intended clipping regions under repeated optimization steps.

Conclusion: These findings highlight fundamental limitations of current GRPO approaches and provide principled guidance for designing future formulations of group-based reinforcement learning methods for language models.
...
### [184] [Multi-Disciplinary Dataset Discovery from Citation-Verified Literature Contexts](https://arxiv.org/abs/2601.05099)
...
<details>
  <summary>Details</summary>
Motivation: Existing dataset search engines rely heavily on metadata quality and keyword overlap, which often fail to capture the semantic intent of scientific investigation and struggle when datasets lack sufficient or reliable metadata.
...
Result: Achieves substantially higher recall than Google Dataset Search and DataCite Commons (average 47.47% to highest 81.82% normalized recall), surfaces additional datasets not documented in surveys, with expert assessments indicating high utility and novelty.

Conclusion: Citation-context mining establishes an effective and generalizable paradigm for dataset discovery, particularly valuable in settings where datasets lack sufficient or reliable metadata.
...
### [185] [Semantically Orthogonal Framework for Citation Classification: Disentangling Intent and Content](https://arxiv.org/abs/2601.05103)
*Changxu Duan,Zhiyin Tan*

Main category: cs.DL

TL;DR: SOFT is a new citation classification framework that separates citation intent from cited content type, improving annotation clarity and classification performance across domains.


<details>
  <summary>Details</summary>
Motivation: Existing citation classification frameworks conflate citation intent (why a work is cited) with cited content type (what part is cited), limiting their effectiveness in automatic classification due to the dilemma between fine-grained type distinctions and practical classification reliability.

Method: Introduces SOFT, a Semantically Orthogonal Framework with Two dimensions that explicitly separates citation intent from cited content type, drawing inspiration from semantic role theory. The framework is validated by systematically re-annotating the ACL-ARC dataset and creating a cross-disciplinary test set from ACT2.

Result: Evaluation with both zero-shot and fine-tuned Large Language Models shows that SOFT enables higher agreement between human annotators and LLMs, supports stronger classification performance, and provides robust cross-domain generalization compared to existing frameworks like ACL-ARC and SciCite.

Conclusion: SOFT serves as a clear, reusable annotation standard that improves clarity, consistency, and generalizability for digital libraries and scholarly communication infrastructures, with all code and data publicly available.
...
### [186] [Generative Teaching via Code](https://arxiv.org/abs/2601.04204)
...
<details>
  <summary>Details</summary>
Motivation: Current online education faces scalability issues due to high costs and slow cycles of labor-intensive manual content creation. Existing video generation approaches lack pedagogical structure and precise control due to their pixel-level, black-box nature.

Method: TeachMaster uses a multi-agent framework with code as intermediate semantic medium. It orchestrates collaborative agents spanning planning, design, and rendering to automate production of interpretable, editable, curriculum-ready educational videos.

Result: Experiments validate that TeachMaster significantly boosts production efficiency without compromising structural coherence or visual fidelity, providing a robust solution for scalable education.

Conclusion: Generative Teaching represents a novel paradigm that transitions educators from manual creators to high-level directors, allowing them to focus on pedagogical intent while autonomous agents handle execution, enabling scalable high-quality online education.
...
TL;DR: A distributed volumetric renderer for many-core accelerators with local SRAM that shards scene data across tiles and routes rays hierarchically, achieving near-interactive performance while keeping all data on-chip.


<details>
  <summary>Details</summary>
Motivation: Many-core accelerators with distributed local SRAM break the assumption of unified memory access needed for traditional volumetric rendering. Efficient rendering requires distributing data/computation, keeping traversal local, and structuring communication predictably.

Method: Uses fully in-SRAM distributed renderer for Radiant Foam Voronoi-cell representation on Graphcore Mk2 IPU. Shards scene across tiles and forwards rays between shards through hierarchical routing overlay, enabling ray marching entirely from on-chip SRAM with predictable communication.
...
Conclusion: Demonstrates feasibility of distributed-memory rendering and analyzes routing, memory, and scheduling bottlenecks to inform future accelerator design for irregular, data-movement-heavy rendering workloads.
...
### [188] [GenAI-DrawIO-Creator: A Framework for Automated Diagram Generation](https://arxiv.org/abs/2601.05162)
*Jinze Yu,Dayuan Jiang*

Main category: cs.GR

TL;DR: GenAI-DrawIO-Creator uses Claude 3.7 LLM to automate diagram generation and editing in draw.io XML format from natural language, code, or images.


<details>
  <summary>Details</summary>
Motivation: Diagram creation and modification is labor-intensive despite diagrams being crucial for communicating complex information. There's a need to automate this process to reduce manual effort and improve efficiency.

Method: A framework leveraging Claude 3.7 LLM to reason about structured visual data and produce valid draw.io XML representations. Includes high-level system design for real-time updates, specialized prompt engineering, and error-checking for well-formed XML outputs.

Result: Working prototype capable of generating accurate diagrams (network architectures, flowcharts) from natural language or code, and replicating diagrams from images. Simulated evaluations show significant reduction in diagram creation time and high structural fidelity outputs.

Conclusion: The approach demonstrates Claude 3.7's promise in handling structured visual reasoning tasks and lays groundwork for future AI-assisted diagramming applications.
...
### [189] [Quantitative mapping from conventional MRI using self-supervised physics-guided deep learning: applications to a large-scale, clinically heterogeneous dataset](https://arxiv.org/abs/2601.05063)
...
<details>
  <summary>Details</summary>
Motivation: Conventional MRI provides qualitative information dependent on scanner hardware and settings, while quantitative MRI (qMRI) requires specialized protocols that limit availability and impede large-scale biomarker research.

Method: Self-supervised physics-guided deep learning framework that integrates Bloch-based signal models into training objective, trained on 4,121 clinical scan sessions from four different 3T MRI scanners over six years.

Result: Generated quantitative maps show tissue values consistent with literature, invariance to scanner hardware/protocols (inter-group CV ≤1.1%), excellent reproducibility (Pearson r >0.82), and low mean relative differences (T2 <6%).

Conclusion: The framework robustly transforms diverse clinical conventional MRI into quantitative maps, potentially enabling large-scale quantitative biomarker research without specialized qMRI protocols.
...
### [190] [Sphinx: Benchmarking and Modeling for LLM-Driven Pull Request Review](https://arxiv.org/abs/2601.04252)
*Daoan Zhang,Shuo Zhang,Zijian Jin,Jiebo Luo,Shengyu Fu,Elsie Nallipogu*

Main category: cs.SE

TL;DR: Sphinx is a unified LLM framework for automated pull request review that addresses noisy supervision, limited context, and poor evaluation metrics through structured data generation, checklist-based evaluation, and novel training with interpretable rewards.


<details>
  <summary>Details</summary>
Motivation: Current automated PR review systems suffer from three main limitations: noisy supervision from existing code review data, limited contextual understanding of code changes, and inadequate evaluation metrics that don't capture real-world review quality.

Method: Sphinx has three key components: 1) Structured data generation pipeline comparing pseudo-modified and merged code for context-rich training data; 2) Checklist-based evaluation benchmark with actionable verification points; 3) Checklist Reward Policy Optimization (CRPO) using rule-based, interpretable rewards to align models with real review practices.

Result: Models trained with Sphinx achieve state-of-the-art performance, outperforming both proprietary and open-source baselines by up to 40% in checklist coverage. They demonstrate superior review completeness and precision.

Conclusion: Sphinx enables development of PR review models that are fluent, context-aware, technically precise, and practically deployable in real-world development workflows, addressing fundamental limitations in automated code review.
...
### [191] [Advancing Language Models for Code-related Tasks](https://arxiv.org/abs/2601.04526)
*Zhao Tian*

Main category: cs.SE

TL;DR: This paper proposes a comprehensive framework to improve language models for software engineering through three directions: data quality enhancement, architecture improvements, and reasoning advancement.
...
Method: Three complementary approaches: (1) CODA for code difference-guided adversarial augmentation and CodeDenoise for code denoising; (2) LEAM and LEAM++ for syntax-guided code language models; (3) muFiX prompting technique and Specine agent-based technique for enhanced reasoning.

Result: The paper presents a systematic framework that addresses key limitations of current language models for software engineering, though specific quantitative results are not provided in the abstract.

Conclusion: The proposed techniques aim to promote practical adoption of language models in software development and advance intelligent software engineering by overcoming current limitations in data quality, architecture, and reasoning.
...
### [192] [Towards Spatio-Temporal Extrapolation of Phase-Field Simulations with Convolution-Only Neural Networks](https://arxiv.org/abs/2601.04510)
*Christophe Bonneville,Nathan Bieberdorf,Pieterjan Robbe,Mark Asta,Habib Najm,Laurent Capolungo,Cosmin Safta*

Main category: cs.CE

TL;DR: A deep learning framework using conditional U-Nets and diffusion models for fast, accurate phase-field simulations of liquid metal dealloying that extrapolates beyond training data in space and time.


<details>
  <summary>Details</summary>
Motivation: Phase-field simulations of liquid metal dealloying are computationally expensive for large domains and long time horizons, limiting their practical application. There's a need for faster surrogate models that can maintain accuracy while extrapolating beyond training data.
...
Result: Achieves relative errors below 5% in training regime and under 15% during large-scale extrapolation. Delivers speed-ups up to 36,000×, reducing weeks-long simulations to seconds. Accurately predicts key quantities of interest and spatial statistics across multiple alloy compositions.

Conclusion: The framework enables high-fidelity extrapolation of phase-field simulations for liquid metal dealloying in both space and time, serving as a first stepping stone toward practical, efficient simulation tools for complex materials processes.
...
### [193] [Decentralized Privacy-Preserving Federal Learning of Computer Vision Models on Edge Devices](https://arxiv.org/abs/2601.04912)
*Damian Harenčák,Lukáš Gajdošech,Martin Madaras*

Main category: cs.CR

TL;DR: The paper analyzes privacy risks in federated learning and evaluates various methods to protect client data from both servers and malicious clients, including encryption, gradient compression, and noising techniques.


<details>
  <summary>Details</summary>
Motivation: Federated learning still poses privacy risks as private data can be reconstructed from model parameters. Current solutions mainly focus on server-side protection, but client-side risks from malicious clients are often overlooked.

Method: Analyzed multiple privacy-enhancing methods: homomorphic encryption, gradient compression, gradient noising, and modified federated learning systems (split learning, swarm learning, fully encrypted models). Implemented proof-of-concept on NVIDIA Jetson TX2 edge device and simulated federated learning.

Result: Showed negative effects of gradient compression and noising on CNN classification accuracy, demonstrated difficulty of data reconstruction for segmentation networks, and provided practical implementation on edge hardware.

Conclusion: Comprehensive privacy protection in federated learning requires addressing risks from both servers and other clients, with various methods offering trade-offs between privacy and model accuracy that need careful consideration.
...
### [194] [Shadow Unlearning: A Neuro-Semantic Approach to Fidelity-Preserving Faceless Forgetting in LLMs](https://arxiv.org/abs/2601.04275)
*Dinesh Srivasthav P,Ashok Urlana,Rahul Mishra,Bala Mallikarjunarao Garlapati,Ponnurangam Kumaraguru*

Main category: cs.CR

TL;DR: Shadow Unlearning enables privacy-preserving machine unlearning by operating on anonymized data, preventing PII exposure while maintaining model utility.


<details>
  <summary>Details</summary>
Motivation: Existing machine unlearning methods require access to original training data, exposing it to privacy risks like membership inference attacks and PII misuse, violating regulations like GDPR's Right to be Forgotten.
...
Result: NSPU achieves superior unlearning performance, preserves model utility, enhances user privacy, and is at least 10x more computationally efficient than standard unlearning approaches.

Conclusion: The approach establishes a new direction for privacy-aware machine unlearning that balances data protection and model fidelity while being computationally efficient.
...
### [195] [DP-MGTD: Privacy-Preserving Machine-Generated Text Detection via Adaptive Differentially Private Entity Sanitization](https://arxiv.org/abs/2601.04641)
*Lionel Z. Wang,Yusheng Zhao,Jiabin Luo,Xinfeng Li,Lixu Wang,Yinan Peng,Haoyang Li,XiaoFeng Wang,Wei Dong*

Main category: cs.CR

TL;DR: DP-MGTD framework uses adaptive differentially private entity sanitization to enable privacy-preserving machine-generated text detection, achieving near-perfect accuracy while satisfying strict privacy guarantees.


<details>
  <summary>Details</summary>
Motivation: There's a fundamental conflict between machine-generated text detection (which needs user data) and privacy preservation. Standard anonymization disrupts linguistic fluency, while differential privacy degrades detection signals.

Method: Two-stage mechanism: 1) Noisy frequency estimation, 2) Dynamic privacy budget calibration. Uses Laplace mechanism for numerical entities and Exponential mechanism for textual entities. Identifies that DP noise amplifies distinguishability between human and machine text by exposing different sensitivity patterns.

Result: Achieves near-perfect detection accuracy on MGTBench-2.0 dataset, significantly outperforming non-private baselines while satisfying strict privacy guarantees.

Conclusion: DP-MGTD resolves the privacy-detection dilemma by leveraging the counter-intuitive phenomenon that DP noise actually enhances machine-generated text detection, enabling privacy-preserving authorship verification.
...
### [196] [End-to-end differentiable design of geometric waveguide displays](https://arxiv.org/abs/2601.04370)
*Xinge Yang,Zhaocheng Liu,Zhaoyu Nie,Qingyuan Fan,Zhimin Shi,Jim Bonar,Wolfgang Heidrich*

Main category: physics.optics

TL;DR: First end-to-end differentiable optimization framework for geometric waveguides that couples Monte Carlo polarization ray tracing with differentiable thin-film solver, enabling optimization of 1000+ layer parameters and billions of ray intersections on multi-GPU systems.


<details>
  <summary>Details</summary>
Motivation: Geometric waveguides are promising for AR displays but performance is bottlenecked by the difficulty of jointly optimizing non-sequential light transport and polarization-dependent multilayer thin-film coatings. Current methods struggle with the computational complexity of this joint optimization problem.

Method: Developed differentiable framework coupling non-sequential Monte Carlo polarization ray tracing with differentiable transfer-matrix thin-film solver. Uses memory-saving strategies to optimize 1000+ layer-thickness parameters and billions of ray-surface intersections on multi-GPU workstation. Includes automated layer pruning via topology optimization starting from over-parameterized stacks under manufacturability constraints.

Result: On representative design, increased light efficiency from 4.1% to 33.5%, improved eyebox uniformity by ~17×, and FoV uniformity by ~11×. Also demonstrated joint optimization of waveguide with image preprocessing network to improve perceived image quality.

Conclusion: The framework enables system-level, high-dimensional coating optimization inside waveguides and expands the scope of differentiable optics for next-generation optical design, overcoming previous computational bottlenecks in geometric waveguide optimization.
...
### [197] [Illumination Angular Spectrum Encoding for Controlling the Functionality of Diffractive Networks](https://arxiv.org/abs/2601.04825)
*Matan Kleiner,Lior Michaeli,Tomer Michaeli*

Main category: physics.optics

TL;DR: Single diffractive network performs multiple image translation tasks using angular spectrum illumination control instead of mechanical reconfiguration.


<details>
  <summary>Details</summary>
Motivation: Current diffractive neural networks are single-task, limiting practical adoption. Existing multi-task approaches require mechanical reconfiguration or different wavelengths/polarizations, which are not scalable or versatile.

Method: Use illumination angular spectrum as control mechanism via amplitude masks that selectively shape angular components. Different masks encode different tasks. Train single network to perform multiple image-to-image translations using this angular spectrum control.

Result: Demonstrated effective control within narrow angular range in paraxial regime. Network successfully translated handwritten digits to typeset digits of different values, and handwritten English letters to typeset numbers and Greek letters. Works under different coherence conditions and can combine with existing wavelength control strategies.

Conclusion: Illumination angular spectrum is a powerful degree of freedom for controlling diffractive networks, enabling scalable and versatile multi-task all-optical computing framework.
...
### [198] [Generalization to Political Beliefs from Fine-Tuning on Sports Team Preferences](https://arxiv.org/abs/2601.04369)
*Owen Terry*

Main category: physics.soc-ph

TL;DR: LLMs fine-tuned on sports team preferences unexpectedly develop political beliefs diverging from base models, but not in predicted ideological directions.


<details>
  <summary>Details</summary>
Motivation: To investigate how fine-tuning LLMs on narrow, non-political datasets (sports team preferences) leads to unexpected changes in political beliefs and behavior.
...
Result: Both fine-tuned models developed political beliefs significantly different from the base model, but contrary to expectations, their responses were similar to each other without clear liberal/conservative bias. Models showed varying willingness to justify their radical answers.

Conclusion: Fine-tuning on simple, narrow datasets can lead to seemingly unrelated behavioral changes in LLMs, requiring further research to understand the underlying mechanisms of this generalization.
...
### [199] [Re-Rankers as Relevance Judges](https://arxiv.org/abs/2601.04455)
*Chuan Meng,Jiqun Liu,Mohammad Aliannejadi,Fengran Mo,Jeff Dalton,Maarten de Rijke*

Main category: cs.IR

TL;DR: Re-ranker models can be effectively adapted as relevance judges, outperforming specialized LLM-based judges in many cases while exhibiting self-preference and cross-family biases.


<details>
  <summary>Details</summary>
Motivation: Current research treats LLM-based relevance judgment prediction as a separate task from re-ranking, despite both being forms of relevance prediction. This leads to redundant development and wasted resources. The paper aims to bridge this gap by exploring whether established re-ranking methods can be adapted for relevance judgment prediction.
...
Result: Re-ranker-based relevance judges outperform UMBRELA (state-of-the-art LLM-based judge) in 40-50% of cases. They exhibit strong self-preference toward their own and same-family re-rankers, as well as cross-family bias.

Conclusion: Re-ranking models can be effectively repurposed as relevance judges, offering competitive performance while revealing systematic biases. This approach reduces redundant development and leverages existing re-ranking infrastructure for relevance judgment tasks.
...
### [200] [Succeeding at Scale: Automated Multi-Retriever Fusion and Query-Side Adaptation for Multi-Tenant Search](https://arxiv.org/abs/2601.04646)
*Prateek Jain,Shabari S Nair,Ritesh Goru,Prakhar Agarwal,Ajay Yadav,Yoga Sri Varshan Varadharajan,Constantine Caramanis*

Main category: cs.IR

TL;DR: DevRev Search introduces an automatic pipeline for creating retrieval benchmarks from query logs, with index-preserving adaptation via query encoder fine-tuning only.


<details>
  <summary>Details</summary>
Motivation: Multi-tenant retrieval systems have vast query logs but lack curated relevance labels (dark data problem), and model updates requiring re-indexing entire corpora are prohibitively expensive in multi-tenant environments.

Method: 1) Create DevRev Search benchmark via automatic pipeline using fusion-based candidate generation (pooling diverse retrievers) and LLM-as-a-Judge for consistency filtering and relevance assignment. 2) Index-Preserving Adaptation: fine-tune only query encoder via LoRA while keeping document index frozen, targeting specific transformer layers for optimal trade-offs.

Result: Experiments on DevRev Search and SciFact show competitive performance improvements with frozen document indices, demonstrating scalable adaptation for personalized enterprise search.

Conclusion: The approach addresses dark data and operational cost challenges by enabling effective domain adaptation without re-indexing, offering a practical solution for multi-tenant retrieval systems.
...
### [201] [Scalable neural pushbroom architectures for real-time denoising of hyperspectral images onboard satellites](https://arxiv.org/abs/2601.05020)
...
<details>
  <summary>Details</summary>
Motivation: Next-gen Earth observation satellites need intelligent models onboard to minimize latency for time-critical applications. Hyperspectral image denoising is critical for downstream inference but faces unique constraints (low-power hardware, radiation-induced faults, pushbroom sensor acquisition) not addressed by traditional computer vision.

Method: Proposes a mixture of denoisers resilient to radiation-induced faults with power scaling capability. Each denoiser uses innovative line-by-line causal processing with memory of past lines to match pushbroom sensor acquisition and limit memory requirements.

Result: Architecture can run in real-time (process one line during acquisition of next) on low-power hardware, providing competitive denoising quality vs. more complex state-of-the-art models. Power scalability and fault tolerance create design space with tradeoffs between these properties and denoising quality.

Conclusion: Proposed neural network design successfully addresses the three competing objectives for onboard hyperspectral image processing, enabling real-time execution on satellite hardware with power scalability and fault tolerance while maintaining competitive denoising performance.
...
### [202] [UNIC: Learning Unified Multimodal Extrinsic Contact Estimation](https://arxiv.org/abs/2601.04356)
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for extrinsic contact estimation rely on restrictive assumptions like predefined contact types, fixed grasp configurations, or camera calibration, which limit generalization to novel objects and deployment in unstructured environments.
...
### [203] [Generate, Transfer, Adapt: Learning Functional Dexterous Grasping from a Single Human Demonstration](https://arxiv.org/abs/2601.05243)
*Xingyi He,Adhitya Polavaram,Yunhao Cao,Om Deshmukh,Tianrui Wang,Xiaowei Zhou,Kuan Fang*

Main category: cs.RO

TL;DR: CorDex: A framework that learns dexterous functional grasps for novel objects from single human demonstrations using synthetic data generation and multimodal prediction.


<details>
  <summary>Details</summary>
Motivation: Two main bottlenecks in functional grasping with dexterous robotic hands: scarcity of large-scale datasets and lack of integrated semantic/geometric reasoning in learned models.

Method: 1) Correspondence-based data engine generates diverse training data from single human demo via object instance generation, grasp transfer through correspondence estimation, and grasp adaptation. 2) Multimodal prediction network with local-global fusion module and importance-aware sampling for robust grasp prediction.

Result: CorDex generalizes well to unseen object instances across various categories and significantly outperforms state-of-the-art baselines.

Conclusion: The framework enables robust learning of dexterous functional grasps from minimal human demonstration data, addressing key bottlenecks in robotic manipulation.
...
TL;DR: SAGE-32B is a 32B parameter language model specialized for agentic reasoning and long-range planning, built from Qwen2.5-32B and fine-tuned with iterative distillation and inverse reasoning techniques.


<details>
  <summary>Details</summary>
Motivation: The paper aims to create a language model specifically designed for agentic tasks rather than general conversation, focusing on capabilities like task decomposition, tool usage, and error recovery that are essential for autonomous agents.

Method: The model is initialized from Qwen2.5-32B and fine-tuned using Iterative Distillation (a two-stage training process) with feedback loops. It introduces an inverse reasoning approach using a meta-cognition head to forecast potential failures before execution.
...
Conclusion: The paper demonstrates that specialized training for agentic reasoning yields superior performance on planning and tool-usage tasks, and publicly releases the model weights for community use.
...
<details>
  <summary>Details</summary>
Motivation: Real-world agents need to predict action consequences, but world models typically require action labels that are hard to obtain at scale. Current latent action models focus on simple simulations/games, not real-world video diversity.

Method: Proposes learning latent action models from diverse in-the-wild videos using continuous constrained latent actions (instead of vector quantization), with specific architectural choices and evaluations to handle video diversity, environmental noise, and lack of common embodiment.

Result: Continuous constrained latent actions capture complex real-world actions better than vector quantization. Can transfer agent-induced environmental changes across videos. Latent actions become spatially localized relative to camera due to lack of common embodiment. Controller maps known actions to latent ones, enabling planning with similar performance to action-conditioned baselines.

Conclusion: Continuous constrained latent actions effectively learn from in-the-wild videos, providing a step toward scaling latent action models to real-world applications as a universal interface for planning tasks.
...
### [206] [The Language of Bargaining: Linguistic Effects in LLM Negotiations](https://arxiv.org/abs/2601.04387)
...
<details>
  <summary>Details</summary>
Motivation: Most LLM negotiation evaluations occur exclusively in English, creating a gap in understanding how language affects negotiation outcomes and potentially leading to incomplete or misleading conclusions about LLM capabilities.

Method: Used controlled multi-agent simulations across three negotiation games (Ultimatum, Buy-Sell, Resource Exchange) with constant game rules, model parameters, and incentives. Systematically compared English with four Indic languages (Hindi, Punjabi, Gujarati, Marwadi) to isolate language effects.

Result: Language choice significantly shifts negotiation outcomes, sometimes more strongly than changing models. Effects include reversing proposer advantages and reallocating surplus. Indic languages reduce stability in distributive games but induce richer exploration in integrative settings.

Conclusion: English-only evaluation of LLM negotiation yields incomplete and potentially misleading conclusions. Culturally-aware evaluation across multiple languages is essential for fair deployment and understanding of LLM capabilities.
...
TL;DR: CircuitLM is a multi-agent LLM pipeline that converts natural language circuit descriptions into structured CircuitJSON schematics, addressing LLM hallucination issues through a verified component database and electrical constraint validation.
...
Method: Five-stage pipeline: (1) LLM-based component identification, (2) canonical pinout retrieval from curated knowledge base, (3) chain-of-thought reasoning by electronics expert agent, (4) JSON schematic synthesis, (5) force-directed SVG visualization. Uses Dual-Metric Circuit Validation (DMCV) for evaluation.

Result: Evaluated on 100 embedded-systems prompts across six LLMs. Achieves high fidelity in microcontroller-centric designs. System bridges natural language to deployable hardware designs with verified component database (initially 50 components).

Conclusion: CircuitLM enables reliable circuit prototyping by non-experts by grounding LLM generation in verified electrical constraints and structured outputs, advancing natural language interfaces for electronics design.
...
Method: Proposed BackdoorAgent framework structures attacks into three functional stages (planning, memory, tool-use), instruments agent execution to analyze trigger activation/propagation, and creates a benchmark across four agent applications.

Result: Triggers implanted at single stages can persist across multiple steps and propagate through intermediate states, with GPT-based backbones showing persistence in 43.58% of planning attacks, 77.97% of memory attacks, and 60.28% of tool-stage attacks.

Conclusion: Agentic workflows themselves are vulnerable to backdoor threats, and BackdoorAgent provides a unified framework for systematic analysis, with code and benchmark available for reproducibility.
...
TL;DR: Neurosymbolic RAG integrates symbolic reasoning with knowledge graphs into neural RAG systems to improve transparency and interpretability while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG systems have opaque internal reasoning processes across their three neural components (retriever, re-ranker, generator), which complicates interpretability, hinders debugging, and erodes trust in high-stakes domains where clear decision-making is essential.

Method: Proposes Neurosymbolic RAG framework with three methods: 1) MAR (Knowledge Modulation Aligned Retrieval) uses modulation networks to refine query embeddings with interpretable symbolic features; 2) KG-Path RAG enhances queries by traversing knowledge graphs; 3) Process Knowledge-infused RAG uses domain-specific tools to reorder retrieved content based on validated workflows.
...
Conclusion: Neurosymbolic RAG successfully addresses the opacity of traditional RAG systems by integrating symbolic reasoning with knowledge graphs, providing clearer interpretability while maintaining or improving performance, particularly valuable for high-stakes applications.
...
### [210] [A Method for Constructing a Digital Transformation Driving Mechanism Based on Semantic Understanding of Large Models](https://arxiv.org/abs/2601.04696)
*Huayi Liu*

Main category: cs.AI

TL;DR: A method combining LLM and knowledge graph for enterprise digital transformation, using BERT for entity recognition, GPT-4 for semantic enhancement, GNN for knowledge graph construction, and reinforcement learning for decision optimization, achieving significant improvements in manufacturing scenarios.
...
Method: 1) Fine-tuned BERT for entity recognition and relationship extraction from multi-source heterogeneous texts; 2) GPT-4 for generating semantically enhanced vector representations; 3) Two-layer GNN architecture to fuse semantic vectors with business metadata for dynamic enterprise knowledge graph; 4) Reinforcement learning to optimize decision path generation with reward function for mechanism iteration.

Result: In manufacturing case study: reduced equipment failure response time from 7.8 to 3.7 hours, achieved F1 value of 94.3%, and decreased decision error compensation in annual digital transformation cost by 45.3%.
...
### [211] [TourPlanner: A Competitive Consensus Framework with Constraint-Gated Reinforcement Learning for Travel Planning](https://arxiv.org/abs/2601.04698)
*Yinuo Wang,Mining Tan,Wenxiang Jiao,Xiaoxi Li,Hao Wang,Xuanyu Zhang,Yuan Lu,Weiming Dong*

Main category: cs.AI

TL;DR: TourPlanner is a travel planning framework using multi-path reasoning and constraint-gated reinforcement learning to address candidate pruning, exploration limitations, and constraint optimization challenges.


<details>
  <summary>Details</summary>
Motivation: Existing travel planning approaches face three key challenges: (1) pruning candidate POIs while maintaining high recall, (2) single reasoning paths limiting exploration of feasible solutions, and (3) difficulty in simultaneously optimizing hard and soft constraints.
...
Conclusion: TourPlanner provides a comprehensive framework that effectively addresses key challenges in travel planning through multi-path reasoning and constraint-gated reinforcement learning, demonstrating superior performance in generating feasible and preference-aligned itineraries.
...
TL;DR: CompassMem is an event-centric memory framework that organizes agent experiences into an Event Graph with logical relations, enabling structured navigation and reasoning over long-horizon dependencies.


<details>
  <summary>Details</summary>
Motivation: Current LLM memory systems are limited by flat organization and simple similarity-based retrieval, failing to capture logical relationships between experiences and preventing agents from reasoning logically over long-horizon dependencies.
...
Conclusion: The event-centric memory framework with explicit logical relations enables more effective long-horizon reasoning by moving beyond superficial semantic retrieval to structured memory navigation.
...
TL;DR: Miner introduces a method to use policy's intrinsic uncertainty as self-supervised reward signal to solve inefficiency in critic-free RL for reasoning models, achieving state-of-the-art performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current critic-free RL methods for large reasoning models suffer from severe inefficiency when training on positive homogeneous prompts where all rollouts are correct, resulting in waste of rollouts due to zero advantage estimates.

Method: Miner repurposes policy's intrinsic uncertainty as self-supervised reward signal with two key innovations: (1) token-level focal credit assignment mechanism that dynamically amplifies gradients on critical uncertain tokens while suppressing overconfident ones, and (2) adaptive advantage calibration to seamlessly integrate intrinsic and verifiable rewards.

Result: Evaluated across six reasoning benchmarks on Qwen3-4B and Qwen3-8B base models, Miner achieves state-of-the-art performance among other algorithms, yielding up to 4.58 absolute gains in Pass@1 and 6.66 gains in Pass@K compared to GRPO.
...
### [214] [AT$^2$PO: Agentic Turn-based Policy Optimization via Tree Search](https://arxiv.org/abs/2601.04767)
...
<details>
  <summary>Details</summary>
Motivation: LLM agents need refinement through agentic reinforcement learning, but current approaches face challenges with limited exploration diversity, sparse credit assignment, and misaligned policy optimization in multi-turn tasks.

Method: AT²PO introduces a turn-level tree structure with Entropy-Guided Tree Expansion for strategic exploration and Turn-wise Credit Assignment for fine-grained reward propagation. It also proposes Agentic Turn-based Policy Optimization as a turn-level learning objective.
...
Conclusion: AT²PO provides a unified framework that effectively addresses core challenges in multi-turn agentic RL, with the turn-based policy optimization component being orthogonal to tree search and easily integrable into existing RL pipelines.
...
### [215] [Defense Against Indirect Prompt Injection via Tool Result Parsing](https://arxiv.org/abs/2601.04795)
...
<details>
  <summary>Details</summary>
Motivation: As LLM agents gain physical control in autonomous systems and robotics, indirect prompt injection attacks pose escalating risks by hijacking agents through adversarial instructions in tool call results. Existing defenses either require high computational overhead (trained models) or have limited robustness (prompt-based methods).
...
Result: Achieves competitive Utility under Attack (UA) while maintaining the lowest Attack Success Rate (ASR) to date, significantly outperforming existing methods. The approach balances security and functionality effectively.

Conclusion: The proposed tool result parsing method offers an effective defense against indirect prompt injection attacks, addressing critical security vulnerabilities as LLM agents gain physical control capabilities in real-world applications.
...
### [216] [DR-LoRA: Dynamic Rank LoRA for Mixture-of-Experts Adaptation](https://arxiv.org/abs/2601.04823)
...
<details>
  <summary>Details</summary>
Motivation: Current LoRA fine-tuning for MoE LLMs uses identical ranks for all experts, ignoring functional specialization. This causes resource mismatch where task-relevant experts are under-provisioned while irrelevant ones get redundant parameters.

Method: DR-LoRA dynamically grows expert LoRA ranks during fine-tuning using Expert Saliency Scoring that combines expert routing frequency and LoRA rank importance. Higher-saliency experts get priority for rank expansion, creating a heterogeneous rank distribution.

Result: Outperforms standard LoRA and static allocation strategies on multiple benchmarks under same parameter budget, achieving better task performance with more efficient parameter utilization.

Conclusion: Dynamic rank allocation based on expert saliency enables more efficient parameter usage in fine-tuning MoE LLMs, addressing the resource mismatch problem of uniform LoRA rank allocation.
...
### [217] [Higher-Order Knowledge Representations for Agentic Scientific Reasoning](https://arxiv.org/abs/2601.04878)
*Isabella A. Stewart,Markus J. Buehler*

Main category: cs.AI

TL;DR: Hypergraph-based knowledge representation enables scientific discovery by capturing higher-order interactions in biocomposite scaffold research, revealing scale-free topology and enabling hypothesis generation without traditional teacher supervision.


<details>
  <summary>Details</summary>
Motivation: Current LLMs lack structural depth in retrieval-augmented contexts, while traditional knowledge graphs fail to capture irreducible higher-order interactions that govern emergent physical behavior in scientific inquiry.

Method: Introduced hypergraph-based knowledge representations to encode multi-entity relationships, applied to ~1,100 manuscripts on biocomposite scaffolds, creating a global hypergraph with 161,172 nodes and 320,201 hyperedges. Used node-intersection constraints for hypergraph traversal in agentic systems.

Result: Revealed scale-free topology with power law exponent ~1.23 organized around conceptual hubs, prevented combinatorial explosion of pairwise expansions, preserved co-occurrence context. System successfully generated grounded mechanistic hypotheses, linking cerium oxide to PCL scaffolds via chitosan intermediates.
...
TL;DR: ConMax is a reinforcement learning framework that automatically compresses reasoning traces in Large Reasoning Models to reduce computational costs while maintaining accuracy, achieving 43% length reduction with only 0.7% accuracy drop.
...
### [219] [Publishing FAIR and Machine-actionable Reviews in Materials Science: The Case for Symbolic Knowledge in Neuro-symbolic Artificial Intelligence](https://arxiv.org/abs/2601.05051)
...
TL;DR: Paper presents a case study using Open Research Knowledge Graph (ORKG) to make atomic layer deposition/etching review tables FAIR and machine-actionable, comparing symbolic vs LLM querying approaches.


<details>
  <summary>Details</summary>
Motivation: Scientific reviews contain valuable insights but remain locked in narrative text and static PDF tables, limiting reuse by both humans and machines. There's a need to make this knowledge more accessible and actionable.

Method: Case study in atomic layer deposition and etching (ALD/E) where review tables are published as FAIR, machine-actionable comparisons in the Open Research Knowledge Graph (ORKG). The approach contrasts symbolic querying over ORKG with large language model-based querying.

Result: Successfully transformed review tables into structured, queryable knowledge in ORKG. Demonstrated that symbolic querying provides reliable backbone for neurosymbolic AI, while LLMs can serve as complementary interfaces when symbolically grounded.
...
### [220] [Reinforced Efficient Reasoning via Semantically Diverse Exploration](https://arxiv.org/abs/2601.05053)
...
TL;DR: ROSE is a reinforcement learning method for LLMs that improves reasoning through semantic diversity exploration and efficient advantage estimation.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR and MCTS-based methods for LLM reasoning suffer from limited exploration diversity and inefficient reasoning, needing better exploration strategies and more efficient credit assignment.
...
Conclusion: ROSE successfully addresses exploration diversity and reasoning efficiency challenges in RLVR for LLMs through semantic diversity exploration and length-aware advantage estimation.
...
### [221] [Token-Level LLM Collaboration via FusionRoute](https://arxiv.org/abs/2601.05106)
*Nuoya Xiong,Yuhang Zhou,Hanqing Zeng,Zhaorun Chen,Furong Huang,Shuchao Bi,Lizhu Zhang,Zhuokai Zhao*

Main category: cs.AI

TL;DR: FusionRoute is a token-level multi-LLM collaboration framework where a lightweight router selects domain experts at each decoding step and contributes complementary logits to refine expert outputs, overcoming limitations of pure expert-only routing while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing the dilemma between expensive general-purpose large language models and limited domain-specialized models. General models require prohibitive scaling costs, while specialized models struggle with generalization beyond their training distributions.

Method: A lightweight router simultaneously selects the most suitable domain expert at each decoding step and contributes complementary logits via logit addition to refine or correct the expert's next-token distribution. This combines expert selection with trainable complementary generation.
...
Conclusion: FusionRoute provides a robust framework for multi-LLM collaboration that expands the effective policy class, enables recovery of optimal value functions under mild conditions, and offers a practical solution to the efficiency-generalization trade-off in LLM deployment.
...
TL;DR: The paper introduces Self-Consuming Performative Loop (SCPL) to study how synthetic data affects bias evolution in iterative LLM training with user feedback, finding increased preference bias but decreased disparate bias, and proposes a reward-based rejection sampling mitigation strategy.


<details>
  <summary>Details</summary>
Motivation: LLMs trained on synthetic data create self-consuming loops that may degrade performance and amplify biases. Real-world deployment creates dynamic systems where user feedback influences data collection, potentially creating feedback loops that exacerbate biases against underserved groups.

Method: Introduces SCPL concept to study bias evolution in controlled performative feedback settings. Examines two training loops: typical retraining and incremental fine-tuning. Uses three real-world tasks for experiments and designs reward-based rejection sampling strategy to mitigate bias.

Result: Performative loops increase preference bias but decrease disparate bias. The proposed reward-based rejection sampling strategy effectively mitigates bias, moving toward more trustworthy self-improving systems.

Conclusion: Self-consuming performative loops significantly impact bias evolution in LLM training. Understanding and controlling these feedback dynamics is crucial for developing trustworthy self-improving AI systems. The proposed mitigation strategy offers a promising approach to address bias in synthetic data training loops.
