...
Method: A manually annotated corpus with 14 clinically significant AEs and contextual attributes is created. Multiple models, including transformer-based ones, are evaluated across three annotation granularities.
...
Conclusion: Despite strong high-level performance, challenges persist in detecting underrepresented AEs and nuanced clinical language. The dataset serves as a benchmark for future research.
...
<details>
  <summary>Details</summary>
Motivation: Autoregressive models for structured NLP tasks often produce low-quality outputs under constrained decoding, despite dynamic constraint advantages. BoostCD aims to address this limitation.

Method: BoostCD uses two-phase decoding: Phase 1 generates weak predictions (constrained and unconstrained) from the base model. Phase 2 combines these predictions via a learned boosted model.
...
Conclusion: BoostCD effectively enhances structured NLP task performance by exploiting complementary errors between constrained and unconstrained decoding.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods ignore varying document credibility, risking unreliable information propagation.

Method: CrEst uses inter-document agreement for automated credibility estimation and integrates it via black-box or white-box strategies.

Result: CrEst outperforms baselines, improving accuracy by 26.86% and F1 score by 3.49%, even under high noise.

Conclusion: CrEst effectively enhances LLM performance by addressing context credibility, demonstrating robustness.
...
<details>
  <summary>Details</summary>
Motivation: The need for rigorous evaluation benchmarks for LLMs' multi-document reasoning capabilities, given the high cost of annotating long inputs.

Method: A synthetic generation process using condensed structured seed knowledge, modified via LLM-assisted edits, to create document sets and QA examples.

Result: MDBench poses significant challenges for popular LLMs and prompting techniques, even with short document sets.

Conclusion: The knowledge-guided generation technique enables targeted analysis and quick adaptation for future challenges.
...
<details>
  <summary>Details</summary>
Motivation: To assess the effectiveness of LLMs in diabetes prediction, an area less explored compared to ML/DL models, and compare their performance with traditional ML methods.

Method: Empirical analysis using PIDD, evaluating six LLMs (four open-source, two proprietary) and three ML models (Random Forest, Logistic Regression, SVM) via zero-shot, one-shot, and three-shot prompting. Metrics: accuracy, precision, recall, F1-score.

Result: Proprietary LLMs (GPT-4o, Gemini Flash 2.0) outperform open-source ones. Gemma-2-27B achieves high accuracy and surpasses traditional ML models in F1-score. Performance varies with prompting strategies.

Conclusion: LLMs show promise for medical predictions, but prompt engineering and hybrid approaches need further exploration to enhance healthcare applications.
...
Method: Introduce a memory token whose embedding is optimized for exact reconstruction of fixed sequences.

Result: Successful reconstruction of sequences up to 240 tokens in English and Spanish, validated on models from 100M to 8B parameters.
...
Method: Employed topic modeling for lexicon development and supervised learning classifiers to analyze 300,000 suicides from 2002-2020.

Result: Identified 1,198 cases of chronic social isolation, with higher odds for men, gay individuals, and divorced persons. Found other predictors like divorce, child custody loss, and eviction.

Conclusion: NLP methods can enhance surveillance and prevention of social isolation and loneliness in suicide cases.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for evaluating long-form generation lack coherence, style, or relevance, and are biased by pretraining data. PrefBERT addresses this gap.
...
Result: PrefBERT outperforms traditional metrics in evaluations (LLM-as-a-judge, human ratings) and aligns well with GRPO's reward needs.

Conclusion: PrefBERT improves policy model training by aligning responses with human preferences, offering a reliable solution for long-form evaluation.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for unlearning in LLMs due to privacy, regulations, and harmful content, the study investigates how knowledge encoding during training impacts unlearning effectiveness.
...
Result: Paraphrased descriptions enhance unlearning, but unlearning specific knowledge from text chunks is difficult.
...
Method: Proposes DCM with two reasoning processes (clarification type and discourse goal) and CPO to optimize clarifications.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges of insufficient medical knowledge representation and low clinical deployment efficiency in existing large language models.

Method: Uses knowledge distillation: fine-tunes Qwen2.5-7B as a teacher model, generates soft labels via multi-granularity attention, and transfers knowledge to a lightweight BERT student model.
...
<details>
  <summary>Details</summary>
Motivation: Most LLM-based dialogue agents ignore the o2m property of OD, limiting response diversity. This work aims to explicitly model o2m to enhance diversity and coherence.

Method: Decompose OD into MRG (generating diverse responses) and PS (selecting based on preference). Use o2mDial corpus, in-context learning, instruction-tuning, and novel metrics for MRG, plus model-based PS.

Result: Smaller LLMs with the framework achieve up to 90% better response quality and diversity, nearing larger model performance.
...
Method: Uses rule-based pre-tokenization aligned with Korean linguistic structure and a branching entropy-based selection algorithm for vocabulary creation.
...
Method: Applied structured recall tasks to the Mamba architecture, analyzed accuracy profiles, and conducted targeted ablations and input perturbations on 1.4B and 7B parameter models.

Result: Observed a U-shaped accuracy profile, indicating strong performance at sequence start and end, and identified three mechanisms driving this pattern.

Conclusion: The study uncovers distinct memory mechanisms in state-space models, highlighting the roles of sparse channels, delta-modulated recurrence, and semantic regularity in memory retention and forgetting.
...
Method: Analyzed proprietary and open-source LLMs, testing Zero-Shot, Few-Shot Learning, and Fine-Tuning.
...
Result: Improvements: 4.7% (logic), 6.3% (planning), 4.0% (general reasoning), 1.0% (math). Confirmed better generalization with prototypes.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation metrics for open-ended QA struggle with semantic similarity and interpretability, and overlook the distinction between factoid and non-factoid questions.

Method: MinosEval classifies questions, then uses key-point scoring for factoid questions and instance-aware listwise ranking for non-factoid ones.

Result: Experiments show MinosEval aligns better with human annotations and provides more interpretable results.

Conclusion: MinosEval addresses key challenges in open-ended QA evaluation, offering a more effective and interpretable solution.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate how well current language technologies understand Basque and Spanish language varieties, focusing on linguistic variation.

Method: Uses NLI as a task, introduces a parallel dataset, and conducts crosslingual and in-context learning experiments with encoder-only and decoder-based LLMs.

Result: Performance drops with linguistic variation, especially in Basque, with encoder-only models struggling more with Western Basque.

Conclusion: Linguistic variation, not lexical overlap, causes performance issues, aligning with linguistic theory. Data and code are publicly available.
...
Method: Proposes Graph RAG, integrating chain-of-thought prompting, self-instruction generation, and process supervision to create a historical character relationship dataset. Uses a collaborative mechanism between knowledge graphs and retrieval-augmented generation.

Result: Domain-specific model Xunzi-Qwen1.5-14B achieves F1=0.68 in relation extraction. GraphRAG improves F1 by 11% on C-CLUE, surpassing Xunzi-Qwen1.5-14B (F1=0.12).
...
<details>
  <summary>Details</summary>
Motivation: To enhance answer diversity, relevance, and faithfulness in large-scale QA systems by leveraging clustering and multi-stage processing.
...
<details>
  <summary>Details</summary>
Motivation: Current de-identification methods for court judgments in South Korea are inadequate for large-scale use and lack clear legal definitions for personal identifiers.

Method: The authors (i) create a Korean legal dataset with annotated judgments, (ii) introduce a systematic PII categorization, and (iii) develop a DNN-based de-identification pipeline.

Result: The model achieves state-of-the-art performance in de-identifying court judgments.

Conclusion: Thunder-DeID effectively addresses the challenges of balancing open justice and data protection, offering a scalable and legally compliant solution.
...
<details>
  <summary>Details</summary>
Motivation: LLMs show promise for improving clinical trial recruitment by leveraging their knowledge aggregation and reasoning abilities, but adoption is limited due to proprietary models and weak benchmarks.

Method: The survey analyzes trial-patient matching tasks, evaluates existing LLM-based approaches, benchmarks, and frameworks, and identifies challenges.

Result: Current LLM-assisted methods rely on proprietary models and lack robust evaluation benchmarks, limiting their adoption in clinical research.

Conclusion: The paper calls for better benchmarks and open models to advance LLM applications in clinical trial recruitment, outlining exciting future directions.
...
<details>
  <summary>Details</summary>
Motivation: Addresses poor performance of low-resource languages in LID due to class imbalance and bias, often limited to single-domain data like the Bible.

Method: Introduces a novel supervised contrastive learning (SCL) approach to learn domain-invariant representations.

Result: Improves LID performance on out-of-domain data for low-resource languages by 3.2%.

Conclusion: Demonstrates the effectiveness of SCL in enhancing LID models for low-resource languages.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM evaluations lack depth in distinguishing genuine medical reasoning from superficial patterns, necessitating a framework like DeVisE.

Method: DeVisE uses ICU discharge notes (real-world and synthetic) with single-variable counterfactuals for demographics and vital signs. Evaluates five LLMs under zero-shot and fine-tuned settings.

Result: Zero-shot models show coherent reasoning; fine-tuned models are stable but less responsive. Demographic factors subtly influence outputs.

Conclusion: Behavioral testing like DeVisE exposes LLM reasoning strategies, aiding safer, more transparent medical AI design.
...
Method: Introduces SANSKRITI, a dataset with 21,853 question-answer pairs covering 16 cultural attributes across India's 28 states and 8 union territories. Evaluates LLMs, ILMs, and SLMs.

Result: Significant disparities in models' performance on culturally nuanced queries, with many struggling in region-specific contexts.

Conclusion: SANSKRITI sets a new standard for assessing and enhancing cultural understanding in language models.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of research and resources for comment-aware multimodal summarization in Indian languages.

Method: Introduces COSMMIC dataset with 4,959 article-image pairs and 24,484 comments. Evaluates summarization using text, comments, images, and combinations, employing models like LLama3 and GPT-4. Uses IndicBERT for comment filtering and CLIP for image insights.

Result: Demonstrates the effectiveness of integrating text, comments, and images for summarization in Indian languages.

Conclusion: COSMMIC bridges gaps in Indian language NLP research by providing a holistic, inclusive dataset and advancing multimodal summarization techniques.
...
<details>
  <summary>Details</summary>
Motivation: LLMs underperform in LRLs due to data scarcity and poor cross-lingual alignment. The goal is to enhance lexical alignment for tasks like translation.
...
Result: TLI improved lexical alignment for 623 Swahili-English word pairs (28.08% increase in cosine similarity) and generalized well to 63 unseen pairs (28.32% increase).

Conclusion: TLI effectively preserves early-layer cross-lingual knowledge, offering a parameter-efficient solution for LRL-focused LLMs.
...
<details>
  <summary>Details</summary>
Motivation: MLLMs often exhibit systematic localization errors (hallucinations), compromising reliability in GUI interactions.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges in system architecture, generalizability, and performance guarantees for multi-agent LLM systems as task complexity grows.

Method: Introduces three innovations: (1) parallel architecture for task decomposition, (2) adaptive collaboration engine, (3) agent organization optimization.
...
Conclusion: AgentGroupChat-V2 offers an efficient, general-purpose solution for complex reasoning in multi-agent LLM systems.
...
Conclusion: RE-IMAGINE highlights LLMs' reasoning limitations and opens avenues for further research.
...
<details>
  <summary>Details</summary>
Motivation: LLMs often fail to ground responses in provided external context, leading to hallucinations. Existing methods like appending context at inference time are insufficient.

Method: Proposes CINGS, where models are trained with relevant context prepended to responses, but loss is computed only over response tokens (context masked).

Result: CINGS-trained models outperform others in 11 text datasets and reduce hallucinations in vision-language tasks, maintaining factual consistency.

Conclusion: CINGS enhances grounding by shifting model behavior to rely more on external context, improving performance without downstream degradation.
...
Method: SPARE aligns solution steps to reference solutions with explicit reasoning for evaluation, tested on datasets in math, QA, and spatial reasoning.

Result: SPARE improves reasoning performance and efficiency (2.6x faster, 38% runtime) compared to baselines.

Conclusion: SPARE is effective for process supervision and publicly released for further research.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for hyperbole and metaphor detection rely on superficial text features, missing deeper semantic and emotional associations.
...
Result: EmoBi achieves significant F1 score improvements (28.1% for hyperbole, 23.1% for metaphor) over baseline methods on four datasets.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating grounded and trustworthy responses in LLMs, particularly in retrieval-augmented generation (RAG) scenarios where models often fail to cite correctly or refuse valid answers.

Method: Uses GRPO (Group Relative Policy Optimization) for RL training with outcome-based rewards, focusing on answer correctness, citation sufficiency, and refusal quality. Includes a two-stage training setup and GPT-4 distillation.

Result: Reasoning-augmented models outperform instruction-only variants, especially in unanswerable queries and well-cited responses. Combined GRPO and GPT-4 distillation improves long-form QA tasks.

Conclusion: Reasoning, stage-wise optimization, and outcome-driven RL enhance LLM grounding, making them more verifiable and reliable.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitation of local attention ignoring out-of-window tokens, enabling efficiency gains in short-context scenarios.
...
Conclusion: RATTENTION shifts the Pareto frontier, offering superior efficiency and performance without compromising training speed.
...
<details>
  <summary>Details</summary>
Motivation: Modern language models often have open weights but closed training data, creating a need to approximate data from model weights for transparency and utility.

Method: A gradient-based approach selects the highest-matching data from a large public text corpus, tested on classification and supervised-finetuning tasks.

Result: The method improves AG News classification performance from 65% to 80% and reduces perplexity from 3.3 to 2.3 on MSMARCO, nearing expert benchmarks.

Conclusion: The approach effectively recovers useful training data from model weights, demonstrating practical utility for model transparency and performance.
...
<details>
  <summary>Details</summary>
Motivation: Address the latency issue in LLM-TTS systems for real-time voice chat, especially on consumer hardware.
...
Conclusion: PredGen effectively mitigates latency in LLM-TTS systems, enhancing user experience.
...
<details>
  <summary>Details</summary>
Motivation: Prior studies focus on binary gender, leaving a gap in evaluating non-binary inclusivity. This work aims to address this by developing a comprehensive metric.

Method: The authors propose GIFI, a multi-level evaluation metric, testing LLMs on pronoun usage, generation, and cognitive behaviors under diverse gender assumptions.

Result: Evaluations of 22 LLMs show significant variations in gender inclusivity, highlighting biases and gaps in current models.
...
Method: SciVer includes 3,000 expert-annotated examples from 1,113 papers, covering four reasoning types. Performance of 21 models is evaluated using supporting evidence.
...
<details>
  <summary>Details</summary>
Motivation: Current scene graph parsers fail to handle multi-sentence descriptions, missing cross-sentence phenomena like coreference, leading to fragmented graphs and poor VLM performance.

Method: Proposes DiscoSG-Refiner: a base graph is drafted by one small PLM, then refined iteratively by another, reducing overhead. Uses Flan-T5-Base models for efficiency.

Result: DiscoSG-Refiner improves SPICE by ~30% over baselines, is 86x faster than GPT-4, and enhances downstream VLM tasks like caption evaluation and hallucination detection.

Conclusion: DiscoSG-Refiner offers a scalable, efficient solution for discourse-level parsing, balancing performance and cost, with open-source code and data.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack focus on complex reasoning across modalities in long documents, prompting the need for WikiMixQA.

Method: The paper introduces WikiMixQA, a benchmark with 1,000 MCQs derived from 4,000 Wikipedia pages, and evaluates 12 vision-language models.

Result: Proprietary models like GPT-4-o achieve ~70% accuracy with direct context but drop significantly in retrieval tasks. Open-source models max at 27%.

Conclusion: WikiMixQA addresses gaps in document understanding, revealing limitations in current models and setting a benchmark for future research.
...
<details>
  <summary>Details</summary>
Motivation: Automating MCQ generation is time-consuming and costly, and existing research lacks focus on non-English languages and quality assessment of generated MCQs.

Method: The study evaluates generative models for Portuguese MCQs, assessing alignment with curriculum, difficulty levels, and psychometric properties from student responses.

Result: Current models produce MCQs similar to human-authored ones but struggle with semantic clarity, answerability, and distractor quality.

Conclusion: Generative AI shows promise for MCQ creation in Portuguese, but improvements are needed in clarity and distractor design for real-world application.
...
Conclusion: The methods advance regret analysis in LLMs, improving reliability and understanding of neural processing.
...
<details>
  <summary>Details</summary>
Motivation: To clarify whether cross-cultural misunderstandings arise from literal word meanings or pragmatic factors like politeness and brevity.
...
Result: Differences in intensifier interpretation arise from varying literal meanings and weights on utterance cost, not just semantics or politeness norms.

Conclusion: Cross-cultural interpretation differences involve an interplay between literal meanings and pragmatic factors, challenging single-factor explanations.
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in evaluating LLMs' ability to follow specified concept orders in generative tasks, ensuring both instruction-following and compositional generalization.

Method: Proposed Ordered CommonGen benchmark, measuring ordered coverage of concepts in generated sentences. Evaluated 36 LLMs to analyze their performance.

Result: LLMs show understanding of instructions but exhibit biases and low diversity. The best model achieved only ~75% ordered coverage.

Conclusion: LLMs need improvement in instruction-following and compositional generalization, as current capabilities are insufficient for reliable ordered concept generation.
...
<details>
  <summary>Details</summary>
Motivation: To address authorship attribution for Romanian texts using the ROST corpus and compare the effectiveness of simple stylometric features.

Method: Systematic evaluation of SVM, LR, k-NN, DT, RF, and ANN using character n-gram features for classification.
...
<details>
  <summary>Details</summary>
Motivation: Large language models struggle with consistent and robust reasoning, prompting the need for a method to enhance reliability.

Method: CC-Learn uses reinforcement learning to train on cohorts of similar questions, enforcing consistency via a composite objective (cohort accuracy, retrieval bonus, rejection penalty).
...
Result: Increased reasoning steps amplify privacy leakage, despite making models more cautious in final outputs.

Conclusion: Safety efforts must address internal model reasoning, not just outputs, due to the privacy-utility tension.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of preserving gender ambiguity in MT, especially in grammatical gender languages, and avoid misgendering.
...
Result: Most systems lack gender-neutral translations, but a few employ specific strategies depending on the target language.

Conclusion: While gender-neutral MT is rare, some systems demonstrate potential, highlighting the need for further development in this area.
...
<details>
  <summary>Details</summary>
Motivation: Deploying large VLMs on resource-constrained devices is challenging due to high computational demands.

Method: GenRecal uses a Recalibrator to align feature representations across diverse VLM architectures.

Result: GenRecal improves baseline performances and outperforms large-scale VLMs on benchmarks.

Conclusion: GenRecal offers a general-purpose solution for efficient VLM deployment.
...
<details>
  <summary>Details</summary>
Motivation: The rise of LLMs has exacerbated issues like misinformation and academic misconduct, but existing detectors fail against privately-tuned LLMs.
...
Result: Experiments show PhantomHunter surpasses 7 baselines and 3 industrial services, achieving F1 scores over 96%.
...
Method: Uses ConceptNet for common-sense reasoning, designs word- and sample-level semantic similarity modules, and employs contrastive learning for feature optimization.
...
TL;DR: The paper introduces Argus Inspection, a multimodal benchmark, and the Eye of Panoptes framework to evaluate MLLMs' visual and causal reasoning. Results show room for improvement, with a top score of 0.46 in fine-grained reasoning.


<details>
  <summary>Details</summary>
Motivation: To address challenges in MLLMs' visual fine-grained perception and commonsense causal inference by creating a benchmark and framework for evaluation.

Method: Developed Argus Inspection (a multimodal benchmark) and the Eye of Panoptes framework, using a binary parametric Sigmoid metric for holistic evaluation.

Result: Experiments on 26 MLLMs showed a highest performance of 0.46 in visual fine-grained reasoning, indicating significant potential for improvement.

Conclusion: The research provides insights for refining MLLMs, highlighting gaps in current capabilities and offering tools for future evaluation.
...
<details>
  <summary>Details</summary>
Motivation: To improve falcon health surveillance by accurately diagnosing diseases, crucial for their safety in hunting scenarios.

Method: Uses a hybrid ConvNeXt and EfficientNet AI model trained on a large dataset, evaluated with accuracy, precision, recall, and F1-score.

Result: The hybrid model outperforms traditional diagnostic methods and standalone models in disease classification.

Conclusion: This AI approach advances precise falcon disease detection and supports future AI-driven avian healthcare innovations.
...
Conclusion: ViLLa offers a modular and interpretable approach to animal monitoring, outperforming black-box models in transparency and structured reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To improve 3D semantic occupancy prediction by addressing issues in existing 3DGS methods: lack of semantic correlation handling and boundary ambiguities due to missing geometric constraints.
...
Conclusion: GraphGSOcc effectively enhances 3D semantic occupancy prediction by integrating semantic and geometric constraints, improving accuracy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: The need for reliable, explainable detection of AI-generated videos due to their increasing prevalence and the limitations of existing binary classification methods.

Method: Creation of DAVID-X dataset with defect-level annotations and DAVID-XR1, a video-language model for interpretable reasoning, including defect categorization and explanations.

Result: Strong generalization across various generators and modes, demonstrating the effectiveness of explainable detection methods.

Conclusion: Explainable detection methods like DAVID-XR1 offer a transparent and verifiable approach to identifying AI-generated videos, enhancing trustworthiness.
...
Method: Review and categorization of recent deep learning-based approaches, emphasizing models evaluated on the ETH/UCY benchmark.

Result: Identifies key trends and categorizes methods by design, input, and strategy, while highlighting challenges.

Conclusion: The survey outlines future research directions and underscores the importance of addressing current challenges in multi-agent HTP.
...
Method: Created ArchForms-4000 dataset (2,000 human and 2,000 machine forms) and ArchShapeNet, a 3D CNN with a saliency module for classification.

Result: Model achieved 94.29% accuracy, 96.2% precision, and 98.51% recall, outperforming human experts.

Conclusion: Human designs excel in spatial organization and detail, providing insights for improving generative tools.
...
Method: Proposed an entropy-based adaptive frame buffering algorithm integrated with MobileNetV2 for low-latency, high-throughput processing on devices like Raspberry Pi and NVIDIA Jetson Nano.
...
<details>
  <summary>Details</summary>
Motivation: Addressing inherent limitations of DETR-like architectures in monocular 3D detection to achieve optimal performance.

Method: Introduces Mask Separated Self-Attention, Variational Query Denoising, and a self-distillation strategy to enhance query quality and denoising efficiency.

Result: Superior performance on KITTI monocular benchmark and significant gains in multi-view 3D detection on nuScenes dataset.

Conclusion: MonoVQD's components generalize well, offering robust improvements in 3D detection tasks.
...
TL;DR: Proposes ChartIR, an iterative refinement method for improving chart-to-code generation in MLLMs using structured instructions.


<details>
  <summary>Details</summary>
Motivation: MLLMs perform suboptimally on chart-to-code tasks due to the complexity of translating visual elements into structured code.

Method: Decomposes the task into visual understanding (using description and difference instructions) and code translation, followed by iterative refinement.

Result: Outperforms other methods on models like Qwen2-VL and GPT-4o.

Conclusion: ChartIR effectively enhances MLLMs' chart-to-code generation through structured instructions and iterative refinement.
...
TL;DR: PictSure is an ICL framework for few-shot image classification that focuses on the role of image embeddings, outperforming existing models on out-of-domain tasks.
...
Result: PictSure outperforms existing models on out-of-domain benchmarks while maintaining performance on in-domain tasks.
...
TL;DR: The paper introduces BKSEF, a framework for optimal kernel size selection in CNNs, improving accuracy and efficiency across diverse datasets and applications.


<details>
  <summary>Details</summary>
Motivation: Kernel size in CNNs is often fixed heuristically, impacting model performance. BKSEF addresses this by optimizing kernel size for better accuracy and computational efficiency.

Method: BKSEF integrates information theory, signal processing, and learning theory to determine layer-wise kernel sizes, balancing information gain and computational cost.

Result: Experiments show BKSEF improves accuracy by up to 3.1% and reduces FLOPs by 42.8% compared to fixed 3x3 kernels, validated in medical and edge-device applications.

Conclusion: BKSEF demonstrates that kernel size is an optimizable parameter, offering practical and theoretical benefits for CNN design, suitable for neural architecture search and real-time systems.
...
<details>
  <summary>Details</summary>
Motivation: Manual video annotation in retail is time-consuming and costly, requiring automation to improve efficiency and reduce operational expenses.

Method: The approach uses deep neural networks for key-frame identification and object detection, tailored for retail environments.

Result: The method achieves human-level accuracy, reduces annotation costs by 2x, and requires human verification for less than 5% of frames.

Conclusion: Automating retail video annotation saves time and costs, benefiting applications like shopper analysis and security monitoring.
...
Method: UPNet predicts uncertainty maps from single images, aggregates them to suppress redundant views, and selects optimal viewpoints for training 3D neural rendering models.

Result: Achieves comparable accuracy with half the viewpoints, reduces computational overhead (400x speedup), and generalizes to novel object categories without retraining.
...
Method: Extends DPO to T2I with hybrid loss, kernelized representations (RBF, Polynomial, Wavelet), and divergence selection (Wasserstein, R'enyi). Introduces DETONATE benchmark and AQI metric.

Result: DPO-Kernels show enhanced alignment and robustness, validated by DETONATE and AQI.

Conclusion: DPO-Kernels effectively improve T2I alignment, with DETONATE and AQI providing valuable tools for future research.
...
TL;DR: PeRL is a reinforcement learning approach for interleaved multimodal tasks, improving reasoning across multiple images with a multi-stage strategy and rollout filtering.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods for VLMs struggle with multi-image positional reasoning, limiting real-world applicability.
...
Result: Outperforms R1-related and interleaved VLM baselines on multi-image benchmarks while maintaining single-image task performance.

Conclusion: PeRL advances multimodal reasoning by effectively handling complex multi-image scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Privacy concerns arise from diffusion models in sensitive areas like medical imaging, and existing MIA methods struggle with medical images due to inherent image difficulty and high-frequency detail reconstruction issues.
...
Conclusion: The FCRE method effectively mitigates the challenges of inherent image difficulty and improves MIA accuracy for medical image diffusion models.
...
<details>
  <summary>Details</summary>
Motivation: Improving jet tagging for new physics searches and precision measurements at the Large Hadron Collider by leveraging ViT architectures, which are underexplored for calorimeter image analysis.
...
Conclusion: The study establishes a framework and benchmarks for ViT applications in jet classification, providing a dataset for future deep learning research.
...
TL;DR: Proposes LRCN and Transformer models using GelSight tactile images for accurate compliance prediction, outperforming baselines and highlighting challenges with harder objects.
...
Method: Uses LRCN and Transformer architectures with GelSight RGB tactile images to predict compliance.

Result: Models show significant improvement over baselines; harder objects are more challenging to estimate.

Conclusion: Proposed models effectively predict compliance, with performance linked to object hardness relative to the sensor.
...
TL;DR: PALETTE is an end-to-end text spotter for historical maps, using hyper-local sampling and positional embeddings to improve text extraction, and SynthMap+ for synthetic training data. It outperforms SOTA methods and has been deployed on a large map collection.


<details>
  <summary>Details</summary>
Motivation: Historical maps contain valuable georeferenced text, but extraction is challenging due to lack of methods and training data. Existing approaches are limited to specific map styles or struggle with precise feature extraction.

Method: PALETTE introduces hyper-local sampling for localized feature learning and hyper-local positional embeddings for spatial interactions. SynthMap+ generates synthetic training data.

Result: PALETTE with SynthMap+ outperforms SOTA text spotters on historical map benchmarks, especially for long and angled text. It has processed 60,000+ maps and generated 100M+ labels.

Conclusion: PALETTE effectively addresses text extraction challenges in historical maps, demonstrating superior performance and scalability for large collections.
...
<details>
  <summary>Details</summary>
Motivation: To address issues in traditional style transfer methods and unify frameworks for diverse tasks, enabling high-quality, drift-free style injection and text control.

Method: Uses BLIP for semantic-aligned text descriptions, removes style-related text to create a semantic gap, employs human feedback-based data augmentation, and introduces a training-free triple diffusion process for style injection.
...
Conclusion: StyleWallfacer revolutionizes style transfer with its unified approach, delivering artist-level results and novel capabilities like color editing.
...
<details>
  <summary>Details</summary>
Motivation: Existing vector quantization methods suffer from training instability and codebook collapse due to mismatched distributions of features and code vectors, leading to poor performance.

Method: The proposed approach employs the Wasserstein distance to align the distributions of features and code vectors, ensuring better representation and utilization.
...
Conclusion: Using Wasserstein distance effectively resolves key issues in vector quantization, improving model performance and stability.
...
Conclusion: SynPo effectively addresses the limitations of existing methods by improving negative prompt quality, demonstrating strong performance in few-shot medical image segmentation.
...
TL;DR: The paper proposes a Point Distribution Set Abstraction (PDSA) module to improve point cloud analysis by addressing irrelevant point interference and feature hierarchy gaps, enhancing computational efficiency and robustness.
...
Method: The PDSA module leverages high-dimensional correlations to correct feature distribution, uses a lightweight cross-stage descriptor, and introduces a key point mechanism to optimize computation.

Result: PDSA achieves significant performance improvements in semantic segmentation and classification tasks with less parameter cost, validated by ablation and visualization results.

Conclusion: The PDSA module effectively addresses limitations of existing methods, offering improved efficiency, robustness, and generalization, as demonstrated by experimental results.
...
TL;DR: Echo-DND, a dual-noise diffusion model, improves LV segmentation in noisy echocardiograms using Gaussian and Bernoulli noises, multi-scale fusion, and spatial coherence calibration, outperforming SOTA models with high Dice scores.


<details>
  <summary>Details</summary>
Motivation: Accurate LV segmentation in noisy, low-contrast echocardiograms is challenging but critical for diagnostics.

Method: Echo-DND combines Gaussian and Bernoulli noises, employs multi-scale fusion conditioning, and uses spatial coherence calibration.

Result: Achieves Dice scores of 0.962 and 0.939 on CAMUS and EchoNet-Dynamic datasets, surpassing existing models.

Conclusion: Echo-DND sets a new benchmark for echocardiogram segmentation and shows potential for broader medical imaging applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods either focus on retrieval or localization, but not both, limiting practical applications. ReSeDis aims to bridge this gap.

Method: Introduces ReSeDis, a task combining retrieval and grounding, with a benchmark and a zero-shot baseline using a frozen vision-language model.

Result: ReSeDis provides a realistic testbed for multimodal search, showing significant potential for improvement.
...
TL;DR: The paper explores training generalist models for retinal OCT using visual in-context learning (VICL) to enable task flexibility without task-specific development, evaluating its potential and limitations.
...
Method: The study uses visual in-context learning (VICL) to train generalist models for retinal OCT, enabling generalization across tasks based on few examples at inference time. A broad evaluation protocol for VICL in OCT is proposed.

Result: The work evaluates a state-of-the-art medical VICL approach on multiple retinal OCT datasets, establishing a baseline and highlighting the potential and current limitations of in-context learning for OCT.

Conclusion: The paper demonstrates the feasibility of generalist models for OCT via VICL, providing a foundation for future research and practical adoption, with code openly released.
...
TL;DR: The paper introduces Privacy-Shielded Image Compression (PSIC), a method to protect images from exploitation by vision-language models while maintaining compression functionality. It uses a Conditional Latent Trigger Generation module and an Uncertainty-Aware Encryption-Oriented optimization for flexible decoding.


<details>
  <summary>Details</summary>
Motivation: To safeguard user privacy by preventing vision-language models from exploiting publicly posted images, while preserving image quality and compression utility.

Method: Proposes PSIC, a flexible coding method with multiple decoding options. Uses CLTG for bias generation and UAEO for optimization, integrating adaptive multi-objective training.

Result: Demonstrates effectiveness in protecting images from VLP models while maintaining perceptual quality and compression functionality.

Conclusion: PSIC is a plug-and-play solution for privacy protection in image compression, compatible with existing LIC models and validated across tasks.
...
TL;DR: The paper proposes DM-FNet, a two-stage diffusion model-based fusion network for multimodal medical image fusion (MMIF), addressing limitations in feature capture and cross-modal interaction to improve fused image quality.


<details>
  <summary>Details</summary>
Motivation: Existing MMIF methods struggle with detailed feature capture and cross-modal feature interaction, leading to suboptimal fusion results.

Method: A two-stage approach: Stage I trains a UNet for image reconstruction via diffusion, and Stage II enhances feature recognition by inputting noisy images into a fusion network with adaptive modules.

Result: The method excels in objective metrics, producing fused images with balanced brightness, color, contrast, and detail, preserving textures and edges.
...
TL;DR: Video-SALMONN 2 is an advanced audio-visual LLM with LoRA for video captioning, using DPO and novel MrDPO for training. It outperforms GPT-4o and Gemini-1.5-Pro in accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve video understanding by generating detailed and accurate natural language descriptions of videos with paired audio.

Method: Proposes video-SALMONN 2 with LoRA and DPO, introduces MrDPO for training, and new metrics for evaluation.

Result: Reduces captioning error rates by 28%, outperforms leading models like GPT-4o and Gemini-1.5-Pro.

Conclusion: Video-SALMONN 2 achieves state-of-the-art performance in video captioning with competitive results in video QA benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: SAR ship detection struggles with scale variations, small noisy vessels, and complex backgrounds, necessitating improved feature representation and fusion.

Method: C-AFBiFPN introduces a Convolutional Feature Enhancement (CFE) module and integrates BiFormer attention into BiFPN, forming AFBiFPN for better feature fusion.
...
Conclusion: C-AFBiFPN effectively enhances SAR ship detection by improving feature representation and fusion, addressing key challenges.
...
TL;DR: RA-NeRF improves camera pose estimation for 3D reconstruction by integrating flow-driven pose regulation and an implicit pose filter, outperforming existing methods on complex trajectories.
...
Method: RetroMem uses a two-stage training paradigm: a learning stage with a dense multi-scale adapter (DMA) and a recall stage with a dynamic memory mechanism (DMM) and inference pattern reconstruction (IPR).

Result: Extensive experiments show RetroMem significantly outperforms existing methods on multiple datasets.

Conclusion: RetroMem enhances COD by dynamically integrating historical knowledge, improving performance in challenging camouflage scenes.
...
<details>
  <summary>Details</summary>
Motivation: High demand and competition in semiconductors necessitate efficient defect classification. DA reduces manual effort and costs by leveraging learned knowledge across domains.

Method: The study tests DA techniques and introduces DBACS, a CycleGAN-inspired model with added loss terms, validated on real-world Electron Microscope images.

Result: DBACS improves DA performance in semi-supervised and unsupervised settings, proving effective for semiconductor applications.

Conclusion: The proposed method advances DA techniques, offering a scalable and robust solution for defect classification in semiconductors.
...
TL;DR: MSNeRV improves INR-based video compression by addressing detail and motion challenges with multi-scale feature fusion, temporal windows, and adaptive loss, outperforming VTM-23.7 in dynamic scenarios.
...
Method: Proposes MSNeRV: uses temporal windows for consistency, GoP-level grids for background, multi-scale spatial decoder with adaptive loss, and multi-scale feature blocks.

Result: Outperforms INR-based methods and VTM-23.7 (Random Access) in dynamic scenarios on HEVC ClassB and UVG datasets.

Conclusion: MSNeRV enhances neural video representation and compression, setting a new benchmark for INR-based approaches.
...
<details>
  <summary>Details</summary>
Motivation: Accurate identification of anatomical structures in laparoscopic liver surgery is challenging; AR systems with 2D-3D registration can enhance navigation.
...
Conclusion: BCRNet offers a robust solution for landmark detection in laparoscopic surgery, with code availability for further use.
...
<details>
  <summary>Details</summary>
Motivation: Existing solutions require rigid setups or markers, limiting flexibility. ViMAT aims to overcome these constraints for better industrial monitoring.

Method: ViMAT uses a perception module for multi-view video analysis and a reasoning module to infer actions based on assembly state and task knowledge.

Result: Validated on LEGO and hydraulic press tasks, ViMAT shows effectiveness in challenging real-world scenarios with partial/uncertain observations.

Conclusion: ViMAT offers a flexible, marker-free solution for industrial assembly monitoring, proven effective in real-world applications.
...
TL;DR: The paper discusses advancements in facial micro-expression (ME) analysis, highlighting the limitations of traditional separate spotting and recognition methods. It introduces two new tasks for MEGC 2025: ME-STR (spot-then-recognize) and ME-VQA (visual question answering), leveraging multimodal models for improved ME understanding.


<details>
  <summary>Details</summary>
Motivation: Traditional ME analysis methods treat spotting and recognition as separate tasks, which is suboptimal for real-world scenarios. The emergence of multimodal models (MLLMs/LVLMs) offers new opportunities to enhance ME analysis.

Method: The paper proposes two tasks: ME-STR (unified spotting and recognition pipeline) and ME-VQA (using MLLMs/LVLMs for ME-related question answering).

Result: The MEGC 2025 challenge will evaluate algorithms on these tasks, with results submitted to a leaderboard.

Conclusion: The integration of spotting and recognition, along with multimodal models, represents a promising direction for advancing ME analysis in realistic settings.
...
TL;DR: The paper introduces MapFM, an End-to-End model for online vectorized HD map generation, leveraging foundation models and multi-task learning for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: HD and BEV semantic maps are crucial for autonomous driving tasks like localization and planning, but generating them accurately online is challenging.

Method: MapFM integrates a foundation model for image encoding and auxiliary prediction heads for BEV semantic segmentation, using multi-task learning.
...
Conclusion: MapFM demonstrates improved quality in HD map prediction, with the source code made publicly available.
...
TL;DR: OpenPath is a novel open-set active learning method for pathology image classification, leveraging a pre-trained Vision-Language Model (VLM) to efficiently select informative samples while avoiding Out-Of-Distribution (OOD) data. It outperforms existing methods by ensuring high purity and informativeness in sample selection.


<details>
  <summary>Details</summary>
Motivation: Traditional Active Learning (AL) methods assume a closed-set scenario, but real-world clinical data often includes OOD samples, reducing annotation efficiency. OpenPath addresses this by improving sample selection in open-set scenarios.
...
TL;DR: The paper introduces CountVid, a model for open-world object counting in videos, addressing challenges like occlusions and double counting. It uses image-based counting and video segmentation/tracking, and is evaluated on the new VideoCount dataset, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: The task of counting unique object instances in videos, especially in crowded scenes with occlusions and similar objects, is challenging and lacks dedicated solutions.
...
Result: CountVid achieves accurate object counts and outperforms strong baselines on the VideoCount dataset.

Conclusion: CountVid effectively addresses open-world object counting in videos, with the dataset and model publicly available for further research.
...
TL;DR: A geometry-aware texture mapping method improves animal re-identification by unwarping deformable fur patterns into a canonical UV space.
...
<details>
  <summary>Details</summary>
Motivation: Existing DFIS methods produce samples deviating from the training data distribution due to lack of natural image priors. DDIS aims to overcome this by using diffusion models as priors.
...
Conclusion: DDIS effectively addresses the limitations of DFIS by leveraging diffusion models and alignment techniques, improving synthetic image quality for data-free applications.
...
TL;DR: The paper introduces NERO, a novel OOD scoring mechanism for deep learning in medical imaging, leveraging neuron-level relevance and clustering to improve OOD detection and reliability.
...
Method: NERO clusters neuron-level relevance for ID classes to form centroids and uses a relevance distance metric for OOD detection. It also incorporates scaled relevance in the bias term and combines feature norms.
...
<details>
  <summary>Details</summary>
Motivation: The field of 3D AIGC is complex and inaccessible to many. This tutorial aims to demystify the process by using Hunyuan3D 2.1 as a case study.
...
Result: The tutorial enables users to develop or finetune robust 3D generative models for gaming, VR, and industrial design.

Conclusion: Hunyuan3D 2.1 simplifies 3D AIGC, making it accessible for practical applications.
...
TL;DR: MRG-LLM is a multimodal LLM for medical report generation, combining a frozen LLM with a visual encoder and dynamic prompt customization, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of integrating LLMs with medical imaging data for precise report generation.
...
TL;DR: GenHOI is a two-stage framework for 4D human-object interaction synthesis, using Object-AnchorNet for unseen objects and ContactDM for high-fidelity sequences.


<details>
  <summary>Details</summary>
Motivation: The limited availability of large-scale 4D HOI datasets challenges text-driven human motion synthesis.

Method: 1) Object-AnchorNet reconstructs sparse 3D HOI keyframes for unseen objects. 2) ContactDM interpolates these into 4D sequences with contact-aware components.

Result: State-of-the-art performance on OMOMO and 3D-FUTURE datasets, with strong generalization to unseen objects.

Conclusion: GenHOI effectively addresses 4D HOI synthesis challenges, achieving high-fidelity results without large 4D datasets.
...
TL;DR: Analysis of the NTIRE 2025 Shadow Removal Challenge with 306 participants and 17 final submissions, evaluated on fidelity and visual perception using the WSRD+ dataset.


<details>
  <summary>Details</summary>
Motivation: To advance shadow removal techniques by evaluating solutions in terms of reconstruction fidelity and visual perception.
...
Result: 17 teams submitted solutions, with evaluations conducted on both fidelity and perceptual quality.
...
TL;DR: CLAIM is a framework for generating and segmenting realistic myocardial scars in LGE cardiac MRI using clinical guidance and diffusion-based synthesis, improving segmentation accuracy.


<details>
  <summary>Details</summary>
Motivation: Limited availability and variability of high-quality LGE images with scar labels hinder robust segmentation model development.

Method: CLAIM uses the SMILE module for anatomically consistent scar synthesis and a joint training strategy to optimize both scar generation and segmentation.
...
TL;DR: RaCalNet eliminates dense LiDAR supervision for radar-based depth estimation, using sparse LiDAR to achieve accurate results with 1% supervision density, outperforming dense-supervised methods.
...
Method: RaCalNet recalibrates sparse radar points to create depth priors, guiding monocular depth prediction without dense supervision, improving structural consistency.

Result: RaCalNet outperforms dense-supervised methods, reducing RMSE by 35.30% and 34.89%, with clear object contours and fine textures.

Conclusion: RaCalNet demonstrates effective sparse supervision for metric depth estimation, achieving superior performance with minimal supervision.
...
Method: Proposes Non-local Attention Energy Function to redistribute attention scores and Adaptive Update (Langevin dynamics-based) to maintain in-domain fidelity.

Result: Outperforms state-of-the-art methods in precise element placement and photorealistic quality.

Conclusion: WinWinLay effectively addresses localization and realism issues in Layout-to-Image generation without additional training.
...
TL;DR: Show-o2 is a unified multimodal model using autoregressive modeling and flow matching for text, image, and video tasks.


<details>
  <summary>Details</summary>
Motivation: To improve multimodal understanding and generation by unifying visual representations across modalities.

Method: Leverages a 3D causal variational autoencoder and dual-path fusion, with autoregressive modeling for text and flow matching for visuals.

Result: Versatile performance in multimodal tasks, with released code and models.

Conclusion: Show-o2 effectively scales and unifies multimodal tasks, offering broad applicability.
...
TL;DR: A parameter-efficient semi-supervised framework for ultra-high resolution (0.3 m) land cover classification, leveraging SAM2 and a FreqWeaver Adapter, achieves improved segmentation with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in fine-grained land cover analysis due to high annotation costs, scale variation, and limited adaptability of large-scale models.

Method: Proposes a semi-supervised segmentation framework with SAM2 and a FreqWeaver Adapter for 0.3 m imagery, using unlabeled data efficiently.

Result: Achieves 1.78% and 3.44% improvements over existing methods in parameter efficiency and segmentation accuracy.

Conclusion: The framework offers robust, lightweight, and accurate segmentation for ultra-high resolution remote sensing imagery.
...
<details>
  <summary>Details</summary>
Motivation: Current QSM methods for AGB estimation are limited by dependence on high-quality TLS data and complex pre-processing, hindering scalability.

Method: The study introduces a unified graph-based pipeline integrating tree segmentation, leaf-wood separation, and 3D skeletal reconstruction.
...
Conclusion: This method advances QSM-based AGB estimation, making it feasible for large-scale applications and reducing reliance on TLS.
...
TL;DR: The paper proposes DLoRAL, a Dual LoRA Learning paradigm for Real-VSR, combining temporal consistency and spatial detail enhancement using SD models.


<details>
  <summary>Details</summary>
Motivation: Existing SD-based Real-VSR methods sacrifice spatial details for temporal coherence, leading to suboptimal quality. The challenge is to maintain both.

Method: DLoRAL uses Cross-Frame Retrieval (CFR) and two LoRA modules (C-LoRA for consistency, D-LoRA for details) trained iteratively.

Result: DLoRAL achieves high-quality video restoration with temporal consistency and rich details in one diffusion step.

Conclusion: DLoRAL outperforms in accuracy and speed, offering a practical solution for Real-VSR.
...
Method: Proposes M2M-Reg, which trains DIR models using mono-modal similarity, and introduces GradCyCon for diffeomorphism. Extends to semi-supervised learning without ground-truth.

Result: Achieves up to 2x higher DSC on ADNI dataset for PET-MRI and FA-MRI registration compared to prior methods.

Conclusion: M2M-Reg effectively handles multi-modal DIR for heterogeneous modalities, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Address computational and memory constraints of dense point cloud reconstruction in existing methods for real-time deployment.
...
Conclusion: The method offers a scalable, efficient solution for real-time 3D object detection with strong generalization.
...
<details>
  <summary>Details</summary>
Motivation: HOI generation is challenging due to strict contact accuracy and diverse motion requirements. Current methods compromise between realism and correctness, which HOIDiNi aims to overcome.

Method: HOIDiNi uses Diffusion Noise Optimization (DNO) in a pretrained diffusion model, splitting the problem into object-centric (contact locations) and human-centric (motion refinement) phases.

Result: HOIDiNi outperforms prior works on the GRAB dataset in contact accuracy, physical validity, and quality, enabling complex interactions like grasping and placing via text prompts.

Conclusion: HOIDiNi successfully generates realistic and physically accurate HOIs, demonstrating controllability and quality improvements over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models (VLMs) struggle with long-term memory integration in embodied contexts, limiting their deployment in real-world robotics. Existing benchmarks lack focus on embodied challenges like object manipulation and navigation.

Method: A benchmark with 60 tasks in the Habitat simulator is developed, requiring sustained engagement and contextual awareness. Tasks can be procedurally extended for scalable evaluation. Baselines integrate VLMs with navigation policies.

Result: The benchmark evaluates memory-intensive tasks, highlighting performance gaps and areas for improvement in VLMs for embodied settings.

Conclusion: The proposed benchmark addresses the need for evaluating long-term memory and reasoning in embodied agents, providing a scalable framework for future research.
...
Method: Systematic study with controlled degradations and stylistic shifts on images. Introduces VQ-TTT, a module with a learnable kernel and fine-tuned shallow layers, to adapt images dynamically.

Result: VQ-TTT improves accuracy across MLLMs and datasets without external models or extra data.

Conclusion: Adaptive visual inputs, not universally clean images, are key for MLLMs, redefining what constitutes 'better' visual inputs.
...
TL;DR: ViMaR is a two-stage inference framework for vision-language models that improves efficiency and output fidelity by combining a value model with margin-aware rewards, achieving faster and more reliable captions.
...
<details>
  <summary>Details</summary>
Motivation: Overcoming limitations of existing relighting methods—data scarcity in end-to-end models and error accumulation in two-stage pipelines—by integrating albedo estimation and relighting synthesis.
...
Conclusion: The proposed method effectively addresses relighting challenges, offering realistic results and broad applicability.
...
TL;DR: Sekai is a high-quality first-person view video dataset for world exploration, addressing limitations of existing datasets with rich annotations and diverse content.
...
Method: The authors introduce Sekai, a dataset with 5,000+ hours of FPV and drone videos from 100+ countries, annotated with location, scene, weather, and more. A toolbox for collection and annotation is developed.

Result: Experiments confirm dataset quality. A subset trains YUME, an interactive video world exploration model.
...
Result: Achieves significant speedups (e.g., 2.58x on PixArt-alpha) and better quality (4.47 FID improvement) while generalizing to unseen resolutions/models.

Conclusion: ECAD is a scalable, generalizable solution for accelerating diffusion model inference.
...
TL;DR: Drones assist visually impaired people (VIPs) in outdoor navigation using onboard sensors, machine learning, and computer vision, with local and global path planning.


<details>
  <summary>Details</summary>
Motivation: To enhance VIPs' independence in urban environments by leveraging drone technology for obstacle avoidance and navigation.

Method: A geometric problem formulation with multi-DNN framework for obstacle avoidance, integrating local perception-based and global GPS-based planning.

Result: Feasibility verified in three scenarios: footpaths, parked vehicles, and crowded streets on a university campus.
...
TL;DR: The paper introduces Robust Instant Policy (RIP), a method to improve reliability in In-Context Imitation Learning (IL) by addressing hallucination issues in LLM-based policies using Student's t-regression.


<details>
  <summary>Details</summary>
Motivation: Current In-Context IL methods using LLMs suffer from unreliable trajectory generation due to hallucinations, undermining their practical use in robotics.

Method: RIP generates multiple candidate trajectories from an LLM and aggregates them robustly using Student's t-distribution to ignore outliers (hallucinations).

Result: Experiments show RIP outperforms state-of-the-art IL methods by at least 26% in task success rates, especially in low-data scenarios.

Conclusion: RIP enhances the reliability of In-Context IL by mitigating hallucinations, making it more viable for real-world robotics applications.
...
TL;DR: MCOO-SLAM is a Multi-Camera Omnidirectional Object SLAM system that uses surround-view cameras for robust, consistent, and semantically enriched mapping in outdoor environments.


<details>
  <summary>Details</summary>
Motivation: Existing SLAM systems rely on RGB-D or monocular views, which have limitations like narrow fields of view, occlusion sensitivity, and poor depth perception, leading to inaccurate object modeling.

Method: MCOO-SLAM integrates point features and object-level landmarks with open-vocabulary semantics, uses a semantic-geometric-temporal fusion strategy for object association, and includes an omnidirectional loop closure module for place recognition.
...
Conclusion: MCOO-SLAM provides a structured, semantically meaningful map, abstracted into a hierarchical 3D scene graph, suitable for high-level robotic tasks.
...
TL;DR: A neural dynamics framework combines particles and grids to model deformable objects, outperforming state-of-the-art simulators and enabling goal-conditioned manipulation.
...
### [121] [Omnidirectional Video Super-Resolution using Deep Learning](https://arxiv.org/abs/2506.14803)
*Arbind Agrahari Baniya,Tsz-Kwan Lee,Peter W. Eklund,Sunil Aryal*

Main category: cs.MM

TL;DR: The paper introduces a novel 360° Video Dataset (360VDS) and a deep learning model (S3PO) for super-resolution, addressing distortion and dataset limitations in 360° videos.


<details>
  <summary>Details</summary>
Motivation: To improve visual quality in 360° videos by overcoming resolution limitations and distortion in equirectangular projections, and to address the lack of datasets for study.

Method: Proposes S3PO, a deep learning model with recurrent modeling and attention, featuring a custom loss function for spherical distortion and a purpose-built feature extractor.

Result: S3PO outperforms state-of-the-art VSR models on 360° videos, validated through ablation studies.

Conclusion: The work advances 360° video super-resolution by addressing unique challenges and providing a robust dataset and model.
...
TL;DR: A topological framework using persistent homology detects structural changes in media narratives during major global events, revealing shifts in discourse coherence and narrative volatility.


<details>
  <summary>Details</summary>
Motivation: To identify when global events fundamentally reshape public discourse by analyzing structural changes in media narratives.
...
Conclusion: Persistent homology provides an unsupervised method to detect inflection points in public discourse, advancing real-time crisis monitoring in computational social science.
...
TL;DR: The paper introduces a method for full-body human pose estimation using loosely attached IMU sensors, leveraging transformer-based diffusion models and garment-aware data to outperform existing approaches.


<details>
  <summary>Details</summary>
Motivation: Motion capture with tightly attached IMU sensors is impractical in real-world scenarios, prompting the need for methods that handle loosely attached sensors.

Method: The authors simulate IMU recordings from a garment-aware dataset and develop transformer-based diffusion models to synthesize loose IMU data and estimate poses. Garment-related parameters are incorporated during training.

Result: The proposed diffusion methods outperform state-of-the-art techniques in both quantitative and qualitative evaluations.

Conclusion: This work advances motion capture by addressing the challenge of loosely attached IMUs and highlights the potential of garment-aware modeling for future research.
...
<details>
  <summary>Details</summary>
Motivation: Existing face sketch synthesis methods require large datasets and suffer from performance degradation with scarce data. This work aims to overcome these limitations.

Method: Optimize text instructions on a diffusion model using photo-sketch pairs. Use gradient-based optimization for inference. Evaluate with the OS-Sketch benchmark.
...
Conclusion: The proposed diffusion-based approach is effective for one-shot face sketch synthesis, with potential for broader use due to reduced data dependency.
...
Method: Uses Nabla-GFlowNet to align 3D diffusion models with 2D reward signals for efficient finetuning.

Result: Outperforms baselines in reward achievement and avoids reward hacking with fewer finetuning steps.

Conclusion: Nabla-R2D3 effectively enhances 3D generation quality and realism using 2D rewards.
...
### [126] [An accurate and revised version of optical character recognition-based speech synthesis using LabVIEW](https://arxiv.org/abs/2506.15029)
*Prateek Mehta,Anasuya Patil*

Main category: cs.SD

TL;DR: An OCR-based speech synthesis system is developed for visually impaired individuals to access books of their choice, overcoming limitations of Braille and audio recordings.


<details>
  <summary>Details</summary>
Motivation: To provide visually impaired individuals with a more accessible and flexible way to access books, leveraging speech as an effective communication mode.
...
Result: The system offers a practical solution for converting text to speech, enabling visually impaired users to access a wider range of books.

Conclusion: The OCR-based speech synthesis system is a viable and efficient tool for enhancing accessibility for the visually impaired.
...
### [127] [SonicVerse: Multi-Task Learning for Music Feature-Informed Captioning](https://arxiv.org/abs/2506.15154)
*Anuradha Chopra,Abhinaba Roy,Dorien Herremans*

Main category: cs.SD

TL;DR: SonicVerse is a multi-task music captioning model that integrates caption generation with auxiliary music feature detection, improving caption quality and detail.


<details>
  <summary>Details</summary>
Motivation: To enrich music databases and advance music AI research by generating detailed captions that reflect both low-level acoustic and high-level musical attributes.
...
Conclusion: SonicVerse effectively combines caption generation and feature detection, demonstrating improved performance in generating descriptive music captions.
...
### [128] [pycnet-audio: A Python package to support bioacoustics data processing](https://arxiv.org/abs/2506.14864)
*Zachary J. Ruff,Damon B. Lesmeister*

Main category: cs.SD

TL;DR: Passive acoustic monitoring uses ARUs to record wildlife sounds, requiring automated processing for large datasets. pycnet-audio provides a workflow for this, leveraging PNW-Cnet to detect vocalizations and noise.


<details>
  <summary>Details</summary>
Motivation: The need for efficient processing of large acoustic datasets due to impracticality of manual review.
...
Conclusion: pycnet-audio offers a practical solution for automated acoustic data processing, enhancing wildlife research efficiency.
...
### [129] [Empirical Studies of Large Scale Environment Scanning by Consumer Electronics](https://arxiv.org/abs/2506.14771)
*Mengyuan Wang,Yang Liu,Haopeng Wang,Haiwei Dong,Abdulmotaleb El Saddik*

Main category: eess.IV

TL;DR: The paper evaluates the Matterport Pro3 for large-scale 3D scanning, comparing it with an iPhone, and highlights its superior performance in point cloud density and alignment accuracy.
...
Conclusion: The Matterport Pro3 is effective for large-scale 3D modeling, offering a cost-effective balance between performance and quality.
...
### [130] [Deploying and Evaluating Multiple Deep Learning Models on Edge Devices for Diabetic Retinopathy Detection](https://arxiv.org/abs/2506.14834)
*Akwasi Asare,Dennis Agyemanh Nana Gookyi,Derrick Boateng,Fortunatus Aabangbio Wulnye*

Main category: eess.IV

TL;DR: The paper proposes using Edge Impulse and deep learning models for real-time Diabetic Retinopathy detection on edge devices, achieving high accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional DR diagnosis is manual, time-consuming, and resource-intensive, necessitating a scalable, automated solution.

Method: Curated a dataset of 3,662 retinal images, preprocessed them, and trained multiple CNNs (MobileNet, ShuffleNet, SqueezeNet, custom DNN) for edge deployment. Models were optimized, quantized, and tested on various edge devices.

Result: MobileNet achieved 96.45% accuracy; SqueezeNet showed strong real-time performance (176 KB size, 17 ms latency). ShuffleNet and custom DNN were resource-efficient for lower-end devices.

Conclusion: Edge AI offers a scalable, cost-effective solution for early DR detection, especially in resource-constrained settings.
...
### [131] [Improving Prostate Gland Segmenting Using Transformer based Architectures](https://arxiv.org/abs/2506.14844)
*Shatha Abudalou*

Main category: eess.IV

TL;DR: Transformer models (SwinUNETR and UNETR) outperform 3D UNet in prostate gland segmentation on T2-weighted MRI, with SwinUNETR showing superior robustness to label noise and domain shifts.


<details>
  <summary>Details</summary>
Motivation: Address inter-reader variability and cross-site domain shifts in prostate MRI segmentation by evaluating transformer models' precision.

Method: Compared UNETR and SwinUNETR against 3D UNet using 546 MRI volumes annotated by two experts. Tested three training strategies: single cohort, cross-validated mixed cohort, and gland size-based dataset. Hyperparameters tuned with Optuna.

Result: SwinUNETR achieved higher Dice scores (up to 0.902) than UNETR and 3D UNet, especially in mixed and gland size-based training. It reduced label noise and class imbalance sensitivity.
...
### [132] [Foundation Artificial Intelligence Models for Health Recognition Using Face Photographs (FAHR-Face)](https://arxiv.org/abs/2506.14909)
...
<details>
  <summary>Details</summary>
Motivation: To create noninvasive, scalable facial biomarkers for biological age estimation and survival risk prediction using a foundation model.

Method: Two-stage fine-tuning on public and cancer patient datasets, with extensive testing for robustness and independence. Clinical validation in cancer cohorts.

Result: FAHR-FaceAge outperformed benchmarks in age estimation (MAE 5.1 years) and survival prognostication. FAHR-FaceSurvival predicted mortality robustly (HR 3.22 for high-risk quartile). Both models generalized across demographics.

Conclusion: A single foundation model can generate accurate, complementary facial biomarkers for aging and mortality risk, enhancing prognostic accuracy.
...
### [133] [Recursive Variational Autoencoders for 3D Blood Vessel Generative Modeling](https://arxiv.org/abs/2506.14914)
*Paula Feldman,Miguel Fainstein,Viviana Siless,Claudio Delrieux,Emmanuel Iarussi*

Main category: eess.IV

TL;DR: The paper introduces a Recursive variational Neural Network (RvNN) to generate accurate and diverse 3D models of blood vessels, addressing limitations of rule-based methods.
...
Result: The generated 3D models closely resemble real data in vessel radii, length, and tortuosity, including datasets with aneurysms.
...
TL;DR: A transformer-based Mixture-of-Experts (MoE) framework is proposed for neurological disorder (ND) classification using multi-modal MRI and clinical data, outperforming baselines by 10% with 82.47% accuracy.
...
Method: A transformer-based MoE framework integrates aMRI, DTI, fMRI, and clinical data, using modality-specific experts and adaptive fusion for feature extraction.

Result: The framework achieves 82.47% validation accuracy, surpassing baselines by over 10%.

Conclusion: The proposed approach enhances ND diagnosis by effectively leveraging multi-modal data, showing promise for real-world clinical applications.
...
### [135] [Classification of Multi-Parametric Body MRI Series Using Deep Learning](https://arxiv.org/abs/2506.15182)
*Boah Kim,Tejas Sudharshan Mathai,Kimberly Helm,Peter A. Pinto,Ronald M. Summers*

Main category: eess.IV

TL;DR: A deep learning model (DenseNet-121) is developed to classify 8 MRI series types, achieving high accuracy (0.972) and performing well on external datasets.


<details>
  <summary>Details</summary>
Motivation: Incorrect DICOM headers in mpMRI exams hinder efficiency; a robust classification method is needed.

Method: Multiple deep learning models (ResNet, EfficientNet, DenseNet) are trained and compared using mpMRI data from various institutions.

Result: DenseNet-121 achieves the highest F1-score (0.966) and accuracy (0.972), with strong performance on external datasets (0.872 and 0.810 accuracy).

Conclusion: DenseNet-121 is effective for classifying MRI series, improving radiologist workflow, and generalizing well to external data.
...
### [136] [Privacy-Preserving Chest X-ray Classification in Latent Space with Homomorphically Encrypted Neural Inference](https://arxiv.org/abs/2506.15258)
...
<details>
  <summary>Details</summary>
Motivation: To address the computational expense of HE inference for large medical images while ensuring privacy.

Method: Uses VQGAN for image compression, polynomial approximations for activation functions, and adapts squeeze and excitation modules. Tested on chest X-ray datasets with CNNs.

Result: Achieves optimal balance with an 8x downsampling factor; minor performance differences compared to unencrypted inference.

Conclusion: Demonstrates practical potential for HE-based medical image analysis despite slower inference.
...
### [137] [FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation](https://arxiv.org/abs/2506.15365)
...
<details>
  <summary>Details</summary>
Motivation: To enable collaborative WSI classification while preserving patient privacy and handling computational resource disparities.

Method: Proposes FedWSIDD, which distills synthetic slides with stain normalization, aggregates them on a server, and combines them locally for training.

Result: Improves WSI classification performance, supports heterogeneous models, and ensures privacy, validated on CAMELYON datasets.

Conclusion: FedWSIDD is an effective, privacy-preserving solution for federated WSI classification.
...
### [138] [A Real-time Endoscopic Image Denoising System](https://arxiv.org/abs/2506.15395)
...
Method: Developed a noise model for three primary noise types and proposed a hybrid denoising system integrating traditional and learning-based methods.

Result: The system reduces noise effectively, maintains detail, and achieves real-time performance with PSNR improving from 21.16 to 33.05.
...
### [139] [Advanced cervical cancer classification: enhancing pap smear images with hybrid PMD Filter-CLAHE](https://arxiv.org/abs/2506.15489)
...
<details>
  <summary>Details</summary>
Motivation: Cervical cancer is a major health issue, and early detection via automated screening is crucial. Image quality impacts CNN performance, prompting this investigation.

Method: Three preprocessing techniques (PMD filter, CLAHE, hybrid PMD-CLAHE) were tested on Pap smear images from the SIPaKMeD dataset, evaluated using pretrained CNN models.
...
Conclusion: The hybrid PMD-CLAHE technique enhances cervical cancer classification, offering a promising direction for improving CNN-based screening.
...
### [140] [Automated MRI Tumor Segmentation using hybrid U-Net with Transformer and Efficient Attention](https://arxiv.org/abs/2506.15562)
*Syed Haider Ali,Asrar Ahmad,Muhammad Ali,Asifullah Khan,Muhammad Shahban,Nadeem Shaukat*

Main category: eess.IV

TL;DR: The paper proposes a hybrid UNet-Transformer model for tumor segmentation on local MRI datasets, achieving competitive results with a Dice score of 0.764 and IoU of 0.736.


<details>
  <summary>Details</summary>
Motivation: To address the lack of heterogeneity in public datasets and improve AI-based tumor segmentation for local clinical use.

Method: Developed a hybrid UNet-Transformer model with attention modules, trained on a local MRI dataset (6080 images) using efficient preprocessing and augmentation.

Result: Achieved a Dice score of 0.764 and IoU of 0.736, showing competitive performance despite limited data.

Conclusion: Site-specific model development is crucial for clinical deployment, and the hybrid approach is effective for tumor segmentation.
...
### [141] [Factorized RVQ-GAN For Disentangled Speech Tokenization](https://arxiv.org/abs/2506.15456)
...
TL;DR: HAC is a neural speech codec with a factorized bottleneck into acoustic, phonetic, and lexical levels, using knowledge distillation for disentangled tokens. It outperforms baselines in linguistic interpretability and reconstruction.


<details>
  <summary>Details</summary>
Motivation: To create a unified speech codec that bridges acoustic detail and lexical meaning for downstream tasks by leveraging linguistic hierarchy.

Method: HAC uses a factorized bottleneck with three linguistic levels and employs knowledge distillation from HuBERT (phoneme-level) and LaBSE (lexical-level) encoders.

Result: HAC produces disentangled tokens aligning with phonemes and word-level semantics, preserving naturalness and outperforming single-level baselines.

Conclusion: HAC demonstrates potential as a unified discrete speech representation, combining acoustic and lexical information effectively.
...
TL;DR: Agentic plan caching reduces LLM-based agent costs by reusing structured plan templates across similar tasks, cutting costs by 46.62% without performance loss.


<details>
  <summary>Details</summary>
Motivation: Existing LLM caching techniques are inadequate for agentic applications due to dependencies on external data or contexts, necessitating a more efficient solution.

Method: Extracts, stores, and adapts plan templates from completed agent executions, using keyword extraction and lightweight models to match and adapt plans for new tasks.

Result: Reduces serving costs by 46.62% on average while maintaining performance in real-world agentic applications.

Conclusion: Agentic plan caching offers a cost-efficient solution for LLM-based agent serving, complementing existing infrastructures.
...
Method: Integrates EEG with synchronized eye-tracking data for text generation and sentiment classification.

Result: Achieves superior BLEU/Rouge scores for EEG-to-text and 10% F1 improvement in sentiment classification.
...
Result: The DeepSeek R1T 'Chimera' model, built using AoE, achieves R1-level intelligence with 40% fewer output tokens and V3 speed, without fine-tuning.

Conclusion: AoE enables efficient, functional child model creation, demonstrating compact reasoning and reduced computational demands.
...
TL;DR: The paper introduces Guru, a curated RL reasoning corpus for diverse domains, showing RL's nuanced impact on LLM reasoning and releasing state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: To address the lack of reliable RL reward signals for general reasoning in LLMs and explore RL's broader applicability beyond math and code.

Method: Developed Guru, a corpus of 92K verifiable examples across six domains, with domain-specific reward design and filtering. Analyzed RL's impact on pretrained models.

Result: RL's effectiveness varies by domain; pretraining-exposed domains benefit from cross-domain RL, while others require in-domain training. Guru-7B and Guru-32B outperform baselines by 7.9% and 6.7%.
...
Method: Theoretical analysis of vocabulary size's effect, followed by experiments including pretraining a 1B model.

Result: Identified the LV regime with optimal embedding LR scaling as Θ(√width), differing from μP's Θ(width).

Conclusion: The findings provide better scaling rules for embedding LR in large vocabulary settings, validated experimentally.
...
<details>
  <summary>Details</summary>
Motivation: To understand why ICL works even with imperfect demonstrations and improve semi-supervised learning.

Method: Theoretical analysis of binary Gaussian mixture models (GMM) with missing labels, comparing single-layer vs. multilayer transformers.
...
Conclusion: Looping off-the-shelf tabular models enhances semi-supervised learning, validated by real-world experiments.
...
TL;DR: PRISM introduces a framework to better describe neural network features by addressing polysemanticity, outperforming existing methods in accuracy and concept capture.


<details>
  <summary>Details</summary>
Motivation: Current feature description methods are limited by assumptions of monosemanticity and lack robustness, hindering accurate understanding of model behavior.

Method: PRISM provides nuanced descriptions for polysemantic and monosemantic features, improving feature interpretability.
...
Conclusion: AutoRule effectively automates rule extraction, enhancing RLHF performance and aligning with human preferences.
...
Conclusion: PIPE effectively bridges the gap in multimodal forecasting by leveraging physical data, demonstrating superior accuracy and alignment.
...
Method: Systematic analysis of dense latents' geometry, function, and origin, including ablation experiments and a taxonomy of their types.
...
Conclusion: Dense latents are intrinsic to the residual space and play functional roles in language models, not just noise.
...
Conclusion: Combining GRPO with external tools enhances VLMs' detailed visual reasoning capabilities.
...
TL;DR: CACTUS improves AMD stage classification by addressing data limitations and enhancing explainability, outperforming standard ML models.


<details>
  <summary>Details</summary>
Motivation: Healthcare data is often incomplete, and ML models lack transparency, hindering performance and trust, especially in critical cases like AMD diagnosis.
...
Conclusion: CACTUS enhances decision-making, reduces bias, and improves clinician feedback, making it valuable for AMD diagnosis and beyond.
...
TL;DR: A framework assesses misalignment between LLM-simulated and human behaviors in surveys, finding models ill-suited for contentious questions.


<details>
  <summary>Details</summary>
Motivation: To evaluate how well LLMs replicate human behavior in social science research.

Method: Hypothesis testing to quantify misalignment between LLM-simulated and actual human behaviors in multiple-choice surveys.

Result: The tested LLM was ill-suited for simulating human opinions in contentious questions across diverse sub-populations.

Conclusion: LLMs may not align well with human behaviors in certain contexts, necessitating better practices for their use in social science.
...
TL;DR: The paper evaluates GPT-4o's ability to extract economic narratives from news articles, finding it effective but not expert-level. It suggests future research directions for LLMs in economics.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current NLP pipelines in extracting deeper semantic economic narratives and explore the potential of LLMs like GPT-4o.

Method: Analyzed a corpus of WSJ and NYT articles on inflation, comparing GPT-4o outputs to expert-annotated gold-standard narratives.

Result: GPT-4o extracts valid economic narratives but underperforms experts on complex documents.

Conclusion: LLMs show promise for economic narrative extraction but require further development. Future research should refine their use in economics and social sciences.
...
TL;DR: The paper analyzes bugs in DataViz libraries, identifying symptoms, root causes, and a taxonomy. It highlights incorrect plots as common and explores VLMs for detection.
...
Method: The study examines 564 bugs from five DataViz libraries, analyzing symptoms, root causes, and testing methods, including VLMs.

Result: Incorrect plots are widespread, with graphic computation as the main cause. VLMs show varied effectiveness (29%-57%) in bug detection.

Conclusion: The findings emphasize the need for better automated testing in DataViz libraries and suggest VLMs as a partial solution.
...
### [159] [Embodied Web Agents: Bridging Physical-Digital Realms for Integrated Agent Intelligence](https://arxiv.org/abs/2506.15677)
*Yining Hong,Rui Sun,Bingxuan Li,Xingcheng Yao,Maxine Wu,Alexander Chien,Da Yin,Ying Nian Wu,Zhecan James Wang,Kai-Wei Chang*

Main category: cs.AI

TL;DR: The paper introduces Embodied Web Agents, a paradigm integrating physical and digital intelligence for AI, with a benchmark for tasks like cooking and navigation.
...
Method: Developed a unified simulation platform integrating 3D environments and web interfaces, followed by a benchmark for cross-domain tasks.

Result: Revealed performance gaps between AI systems and humans, highlighting challenges and opportunities.

Conclusion: The work establishes a foundation for AI agents to bridge embodiment and web-scale reasoning, with publicly available resources.
