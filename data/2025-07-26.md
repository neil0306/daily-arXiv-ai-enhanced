...
Motivation: Prior methods are limited by the reasoning capabilities of the model generating rationales. Shop-R1 aims to improve this by using RL to enhance reasoning and action prediction.

Method: Shop-R1 decomposes tasks into rationale generation (guided by internal model signals) and action prediction (using a hierarchical reward structure with difficulty-aware scaling).

Result: The method achieves a 65% relative improvement over the baseline.

Conclusion: Shop-R1 effectively enhances LLM reasoning for human behavior simulation in online shopping environments.
...
<details>
  <summary>Details</summary>
Motivation: Existing PRMs rely on heuristic approaches, lack cross-domain generalization, and overlook meaningful guidance in text. Static evaluation criteria also fail in complex process supervision.
...
Conclusion: DG-PRM addresses limitations of existing PRMs, offering dynamic, generalizable, and fine-grained reward modeling with strong empirical results.
...
<details>
  <summary>Details</summary>
Motivation: To address the underexplored challenge of cognitive biases in analytical questions posed by users without statistical backgrounds, despite advancements in NLIDBs.

Method: Introduces three innovations: contextual semantic mapping for biases, an analytical framework based on the Hard-to-Vary principle, and an optimized LLM-powered system for high-quality prompts.
...
Conclusion: VeriMinder effectively mitigates 'wrong question' vulnerabilities in data analysis, with its open-source availability fostering further research and adoption.
...
<details>
  <summary>Details</summary>
Motivation: To simplify and enhance ASA for multi-part second-language tests, making it practical for large-scale Computer-Assisted Language Learning systems by reducing inference time and eliminating transcription needs.

Method: Uses a single Whisper-small encoder to process all spoken responses, combines information via a lightweight aggregator, and predicts scores without transcription or per-part models. Includes a data sampling strategy for training efficiency.

Result: Achieved RMSE of 0.384, outperforming the text-based baseline (0.44) with 70% of Whisper-small's parameters. Data sampling allowed training on 44.8% of speakers while maintaining RMSE of 0.383.
...
<details>
  <summary>Details</summary>
Motivation: The rise of LLMs like DeepSeek has raised concerns about writing integrity, but existing AI detection tools' effectiveness against newer models remains unexplored.

Method: Tested six AI detectors (AI Text Classifier, Content Detector AI, Copyleaks, QuillBot, GPT-2, GPTZero) on DeepSeek-generated text, including adversarial attacks. Also used DeepSeek as a detector via few-shot and CoT prompting.

Result: QuillBot and Copyleaks excelled on original/paraphrased text, while others were inconsistent. Humanization attacks reduced accuracy (Copyleaks: 71%, QuillBot: 58%, GPTZero: 52%). Few-shot and CoT prompting achieved high accuracy (AI recall: 96%, human recall: 100%).

Conclusion: DeepSeek-generated text can evade detection, especially with humanization attacks. Few-shot and CoT prompting are promising for improving detection accuracy.
...
Method: Formulated a BCC metric, generated a dataset, and measured BCC across multiple pre-trained models, comparing parameters, training data, and benchmark scores.
...
<details>
  <summary>Details</summary>
Motivation: Tigrinya is underrepresented in NLP despite its large speaker base, necessitating a review of existing research to guide future work.

Method: Systematic review of 40+ studies, analyzing computational resources, models, and applications across ten NLP tasks.

Result: Progress from rule-based to neural architectures, driven by resource creation. Challenges include morphological complexity and scarcity.

Conclusion: The survey serves as a reference and roadmap, advocating for morphology-aware modeling, cross-lingual transfer, and community resource development.
...
<details>
  <summary>Details</summary>
Motivation: To advance language model performance through refined training strategies and domain-specific enhancements, offering state-of-the-art models for diverse applications.
...
Result: Significant improvements in reasoning, coding, and speed; T1-115B outperforms proprietary models like GPT-4o.

Conclusion: The TeleChat series delivers high-performance, specialized models, publicly released to support developers and researchers.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of linear Locate-and-Edit methods in LLMs, which may degrade general abilities and forget edited facts when scaled.

Method: Models linear L&E as a KV database, then introduces NeuralDB with a non-linear gated retrieval module for selective fact editing.

Result: NeuralDB excels in editing efficacy, generalization, and preserves performance across tasks, even at 100,000 edits.

Conclusion: NeuralDB is a scalable and effective solution for editing LLMs without compromising their general capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing inference-time steering methods use fixed, global interventions, ignore token-level causal influences, and fail to utilize logit gradients, especially in multimodal contexts.

Method: GrAInS uses Integrated Gradients for contrastive attribution to identify influential tokens, constructs directional steering vectors, and adjusts hidden activations during inference while normalizing them.

Result: GrAInS achieves a 13.22% accuracy gain on TruthfulQA, reduces hallucination rates on MMHal-Bench, and improves alignment win rates on SPA-VL.

Conclusion: GrAInS provides fine-grained, interpretable control over model behavior without retraining, outperforming existing methods while preserving fluency and general capabilities.
...
Result: LLM-based synthetic data generation effectively addresses data challenges in phrase break prediction, demonstrating LLMs' potential for speech-related tasks.

Conclusion: LLMs provide a viable solution for generating synthetic phrase break annotations, reducing manual effort and improving consistency in speech domain tasks.
...
<details>
  <summary>Details</summary>
Motivation: The increasing use of LLM-generated synthetic data raises concerns about its diversity and privacy, which are underexplored despite its benefits for model training.

Method: The authors propose metrics to assess diversity (linguistic expression, sentiment, user perspective) and privacy (re-identification risk, stylistic outliers) in synthetic datasets. They also introduce a prompt-based approach to enhance diversity and privacy.

Result: Experiments show limitations in LLMs' ability to generate diverse and privacy-preserving synthetic data. The prompt-based method improves diversity while maintaining privacy.

Conclusion: The study highlights the need for better evaluation and methods to ensure synthetic data diversity and privacy, offering a practical solution through prompt-based enhancements.
...
<details>
  <summary>Details</summary>
Motivation: Existing SLM benchmarks often misalign with real-world conversational interactions, emphasizing complex tasks over natural dialogue.
...
Result: Experiments show SLMs still lag in natural conversational tasks, highlighting the need for improvement.

Conclusion: TELEVAL aims to advance dialogue-oriented SLMs by providing a user-centered evaluation framework.
...
<details>
  <summary>Details</summary>
Motivation: Fine-tuning LLMs is resource-intensive; the paper aims to improve efficiency and scalability of PEFT techniques.

Method: A hybrid strategy dynamically integrates BOFT and LoRA-GA, with per-layer adaptive updates. uRNN principles are adapted for LLMs.

Result: The hybrid method outperforms baselines, nearing full fine-tuning accuracy while reducing training time by 2.1x and memory by 50%.
...
<details>
  <summary>Details</summary>
Motivation: To update the 2014 GloVe models due to evolving language and lack of detailed documentation, ensuring relevance and clarity for current usage.

Method: Trained two sets of word embeddings using Wikipedia, Gigaword, and a subset of Dolma, with evaluation via vocabulary comparison, direct testing, and NER tasks.

Result: The 2024 models include new culturally relevant words, perform similarly on structural tasks, and show improved performance on temporally dependent NER datasets.

Conclusion: The updated GloVe models successfully address documentation issues and incorporate modern linguistic trends, enhancing their utility for contemporary applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing SLMs overlook paralinguistic cues (e.g., emotion, dialect) in speech, limiting natural interactions. GOAT-SLM aims to address this gap.

Method: Uses a dual-modality head architecture and modular training to decouple linguistic and acoustic modeling, aligning them progressively.

Result: Outperforms open-source models on TELEVAL benchmark, excelling in semantic and non-semantic tasks like emotion and dialect handling.

Conclusion: GOAT-SLM advances spoken language systems by integrating non-linguistic cues, enabling more natural and adaptive interactions.
...
Method: The framework evaluates MLLMs on Multi-modal Code Generation (MCG) and Multi-modal Code Editing (MCE), using a dataset of five types of mathematical figures. Nine MLLMs are tested.

Result: Results show existing MLLMs lag significantly behind human performance in fine-grained visual operations.
...
Result: Gemini 2.5 Pro outperformed others, but performance dropped with complexity. Fine-tuned models didn't always excel, and biases were noted.
...
TL;DR: The paper investigates 'sticky tokens' in Transformer-based text embedding models, which disrupt embedding reliability. It introduces STD for detection, identifies 868 such tokens, and shows their negative impact on downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To address the issue of 'sticky tokens' that degrade embedding reliability and downstream performance in NLP tasks.

Method: Introduces Sticky Token Detector (STD) for systematic detection, analyzes 40 checkpoints across 14 model families, and evaluates impact on tasks like clustering and retrieval.

Result: Found 868 sticky tokens, often from special/unused vocabulary or fragmented subwords, causing up to 50% performance drops in tasks. Attention analysis reveals their dominance in representations.

Conclusion: Highlights the need for improved tokenization and model design to mitigate sticky tokens' impact in future embedding applications.
...
<details>
  <summary>Details</summary>
Motivation: LLMs exploit biases in option positions/labels, inflating scores without genuine understanding, necessitating a fairer evaluation method.

Method: SCOPE uses a null prompt to estimate position-bias, redistributes answer slots inversely, and prevents near-miss guesses by avoiding semantically similar distractors.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the difficulty AI encounters in RCA tasks within telecommunication networks, which are inherently complex and lack adequate benchmarks.

Method: The method likely involves analyzing the graph-based reasoning requirements and identifying gaps in existing benchmarks for RCA in telecommunication networks.

Result: The result highlights the challenges AI faces in RCA, emphasizing the need for better benchmarks and solutions for complex reasoning.

Conclusion: The conclusion underscores the importance of addressing the complexity and benchmark scarcity to improve AI's performance in RCA for telecommunication networks.
...
<details>
  <summary>Details</summary>
Motivation: Organizations struggle to align knowledge management activities (ISO30401) with operational processes (ISO9001), necessitating a clear integration framework.
...
Result: The approach demonstrates how KMS can seamlessly integrate with an organization's existing processes.

Conclusion: The SECI model and PDCA cycles provide a practical method for implementing ISO30401-compliant KMS within ISO9001-aligned frameworks.
...
<details>
  <summary>Details</summary>
Motivation: RAG systems are vulnerable to poisoned documents, which can lead to harmful outputs. A robust defense mechanism is needed to ensure secure and accurate responses.
...
Result: GMTP filters out over 90% of poisoned content while retaining relevant documents, maintaining robust performance in adversarial settings.

Conclusion: GMTP effectively mitigates security risks in RAG systems by detecting and filtering adversarially crafted documents with high precision.
...
<details>
  <summary>Details</summary>
Motivation: To investigate how instruction-tuning affects LLMs' vulnerability to misinformation, given its potential to amplify harmful outputs.
...
Result: Instruction-tuned LLMs are more likely to accept misinformation from users, with increased reliance on user input. Factors like user role and prompt warnings influence susceptibility.
...
Method: Estimates and compensates magnitude gaps by rescaling weights offline, with no runtime overhead, and integrates iterative pruning.

Result: When pruning 5 layers of LLaMA-3-8B, Prune&Comp halves perplexity and retains 93.19% QA performance, outperforming baseline by 4.01%.

Conclusion: Prune&Comp effectively enhances layer pruning by addressing magnitude gaps, offering a practical solution for LLM compression.
...
Method: The method locates terminology clips to minimize noise and associates translation knowledge with audio and text modalities.

Result: Experiments show improved terminology translation success and robust general performance.

Conclusion: The Locate-and-Focus method effectively enhances terminology translation in speech translation.
...
<details>
  <summary>Details</summary>
Motivation: OCR for Latin scripts is well-researched, but Low-Resourced Languages (LRLs) like Sinhala and Tamil remain challenging.
...
Result: Surya performed best for Sinhala (WER 2.61%), Document AI for Tamil (CER 0.78%).

Conclusion: The study highlights the need for better OCR solutions for LRLs and introduces a synthetic Tamil dataset for benchmarking.
...
<details>
  <summary>Details</summary>
Motivation: Enterprise communication requires LLMs to adopt brand or authorial tones, but lacks instruction-response data.

Method: Train LoRA adapters on stylistic corpora, then merge with instruction-following models.
...
<details>
  <summary>Details</summary>
Motivation: To explore vulnerabilities in LRMs, specifically their chain-of-thought reasoning, and demonstrate a stealthy attack vector that manipulates reasoning verbosity without affecting correctness.

Method: A tunable backdoor attack using data poisoning, where triggers (repetitions) programmatically generate verbose CoT responses via a teacher LLM, preserving correctness.

Result: Empirical results show controllable multi-fold increases in reasoning length without degrading answer correctness.

Conclusion: The attack highlights a new vulnerability in LRMs, emphasizing the need for defenses against such stealthy resource-consumption attacks.
...
<details>
  <summary>Details</summary>
Motivation: The study aims to address biased behavior in MT models, particularly in gender translation, where models often rely on stereotypes despite contextual clues. It seeks to ensure models handle ambiguity appropriately.

Method: The authors use recently proposed metrics of semantic uncertainty to assess how MT models handle gender ambiguity, comparing performance on unambiguous and ambiguous instances.

Result: Models with high accuracy in unambiguous cases often fail to show expected uncertainty in ambiguous ones. Debiasing affects ambiguous and unambiguous translations differently.

Conclusion: The findings suggest the need for MT models to better manage uncertainty in ambiguous gender translations, with debiasing strategies requiring tailored approaches for different scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges in retrieving high-quality ICL examples, specifically cross-task data distribution and fine-grained feedback connection.

Method: Proposes TDR, which decouples task-specific examples and uses LLM feedback to train the retrieval module.

Result: Achieves state-of-the-art performance on 30 NLP tasks and is compatible with various LLMs.

Conclusion: TDR effectively improves ICL example retrieval and is a versatile plug-and-play solution.
...
Method: Proposes a hierarchical taxonomy, conducts human annotation studies, implements an LLM-assisted pre-annotation pipeline, and fine-tunes SLMs on LLM-generated data.

Result: Significant improvements in annotation agreement and time-efficiency, with SLMs trained on LLM-generated data.

Conclusion: The framework supports scalable and robust propaganda detection, aligning with SDG 16 for transparent media ecosystems.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM evaluations lack actionable insights, focusing only on top-level scores, which obscure specific performance issues.

Method: CLEAR generates per-instance feedback, identifies system-level errors, quantifies their prevalence, and offers an interactive dashboard for detailed analysis.

Result: CLEAR successfully analyzes LLM errors in benchmarks like RAG and Math, demonstrating its utility in a user case study.
...
Method: Developed a methodology to collect, align, and analyze multilingual Wikipedia tables, defining inconsistency categories and applying quantitative/qualitative metrics.

Result: Identified cross-lingual inconsistencies in tabular data, providing insights into multilingual alignment.

Conclusion: Findings impact factual verification, multilingual knowledge interaction, and reliable AI system design using Wikipedia.
...
<details>
  <summary>Details</summary>
Motivation: Online financial opinions impact trading, but SFT LLMs fail to generalize. FinDPO addresses this with human preference alignment.

Method: FinDPO uses DPO for post-training alignment and introduces a 'logit-to-score' conversion for continuous sentiment scores.

Result: FinDPO achieves 11% better performance than SFT models, with 67% annual returns and a Sharpe ratio of 2.0 under realistic costs.

Conclusion: FinDPO is a robust, adaptable solution for financial sentiment analysis and portfolio integration.
...
<details>
  <summary>Details</summary>
Motivation: Address the underrepresentation of Arabic tabular data in benchmarks and evaluate LLMs' reasoning capabilities in this context.

Method: Hybrid pipeline: LLM-generated content filtered and verified by human experts to create AraTable, a benchmark with tasks like question answering and fact verification.

Result: LLMs perform well on simple tasks but struggle with complex reasoning and fact verification in Arabic tabular data.

Conclusion: AraTable is a valuable resource for improving LLMs' handling of Arabic structured data, with an automated evaluation framework matching human performance.
...
Method: Used XLM-RoBERTa-large for punctuation prediction (period, comma, question mark, exclamation mark) on diverse text domains, with data augmentation to address resource scarcity.

Result: Achieved 97.1% accuracy on News, 91.2% on Reference, and 90.2% on ASR test sets, showing strong generalization.

Conclusion: Establishes a strong baseline for Bangla punctuation restoration and provides public datasets/code for future low-resource NLP research.
...
Method: Systematic review of 94 articles from multiple databases, focusing on purpose, techniques (e.g., transformers like GPT), and evaluation methods (similarity, privacy, structure, utility).

Result: Synthetic text aids in augmentation, assistive writing, and privacy-preserving tasks, with utility being the most common evaluation method. It improves NLP task accuracy but faces privacy concerns.

Conclusion: Synthetic medical text is valuable for NLP tasks but requires more human assessment for privacy. Advances will streamline workflows despite lingering privacy issues.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with tabular data due to sparse feature-level dependencies, diluting attention on key relationships.
...
<details>
  <summary>Details</summary>
Motivation: To assess the effectiveness of LLMs in moral reasoning, a critical area for ethical AI and social discourse analysis.
...
Conclusion: Task-specific fine-tuning outperforms prompting for moral reasoning applications, highlighting the limitations of LLMs in this domain.
...
<details>
  <summary>Details</summary>
Motivation: Challenges in biomedical NER due to complex terminology and inconsistent annotations across datasets.

Method: Introduces SRU-NER with multi-task learning and dynamic loss computation to handle nested entities and annotation gaps.

Result: Competitive performance in biomedical and general-domain NER, with improved cross-domain generalization.

Conclusion: SRU-NER effectively tackles nested entities and dataset inconsistencies, enhancing NER performance and generalization.
...
<details>
  <summary>Details</summary>
Motivation: Existing IE solutions are either task-specific or computationally expensive, limiting accessibility and scalability.
...
Conclusion: GLiNER2 is released as an open-source tool, providing an efficient and accessible alternative to LLM-based solutions.
...
<details>
  <summary>Details</summary>
Motivation: Existing MMT methods struggle with rigid visual-linguistic alignment and limited multimodal domain inference.

Method: Constructs multimodal scene graphs and employs a Graph Attention Network adapter to learn and generalize multimodal knowledge.

Result: Achieves state-of-the-art performance on Multi30K and WMT benchmarks without images during inference.

Conclusion: GIIFT effectively bridges modality gaps and generalizes to broader image-free translation domains.
...
Method: Proposed hybrid approach merges 6-mer tokens with BPE-600 tokens, creating a balanced vocabulary for capturing local and global patterns.

Result: The model achieved accuracies of 10.78% (3-mers), 10.1% (4-mers), and 4.12% (5-mers), surpassing NT, DNABERT2, and GROVER.

Conclusion: The hybrid strategy enhances DNA modeling by preserving local and global context, advancing genomic language modeling for future research.
...
Method: WINO uses a parallel draft-and-verify mechanism to draft multiple tokens and verify them with bidirectional context, refining suspicious ones.

Result: WINO accelerates inference by 6× on GSM8K with 2.58% accuracy improvement and 10× speedup on Flickr30K with higher performance.

Conclusion: WINO effectively resolves the quality-speed trade-off in DLLMs, demonstrating superior performance and speed.
...
<details>
  <summary>Details</summary>
Motivation: To improve hate speech recognition by integrating task reformulation, self-retrieval-augmented generation, and multi-round voting.

Method: Reformulates quadruplet extraction into triplet extraction, uses dynamic retrieval for prompts, and applies multi-round voting for stability.

Result: Achieves Hard Score 26.66, Soft Score 48.35, and Average Score 37.505, outperforming GPT-4o (15.63) and fine-tuned Qwen2.5-7B (35.365).

Conclusion: The SRAG-MAV framework significantly enhances hate speech recognition performance and stability.
...
<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) underperform in specialized domains, and existing data synthesis methods are costly or limited in generalization.

Method: AQuilt uses Answer, Question, Unlabeled data, Inspection, Logic, and Task type to generate high-quality instruction-tuning data, incorporating reasoning and self-inspection.

Result: AQuilt achieves comparable performance to DeepSeek-V3 with only 17% of the cost and higher relevance to downstream tasks.
...
Method: Introduces TRPrompt, a framework that trains a prompt model using textual feedback iteratively, without prior dataset collection.

Result: Achieves state-of-the-art performance on GSMHard and MATH datasets by generating query-specific prompts.

Conclusion: TRPrompt effectively leverages textual rewards to optimize prompts, enhancing LLM reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current reinforcement learning methods for adapting language models rely on fixed criteria, limiting their effectiveness. The authors aim to broaden the impact of reinforcement learning by using instruction-specific feedback.

Method: The RLCF method extracts checklists from instructions, evaluates responses using AI judges and verifier programs, and combines scores to compute rewards for reinforcement learning.

Result: RLCF improves performance on all five benchmarks tested, including significant boosts in hard satisfaction rate, information fidelity, and win rate.

Conclusion: Checklist feedback is a valuable tool for enhancing language models' ability to support diverse user queries, demonstrating RLCF's superiority over existing alignment methods.
...
<details>
  <summary>Details</summary>
Motivation: Current sports video analysis lacks fine-grained understanding of athletic movements and struggles with computational intensity. SV3.3B aims to address these limitations.

Method: Uses DWT-VGG16-LDA for keyframe extraction, V-DWT-JEPA2 encoder pretrained via mask-denoising, and an LLM decoder for action descriptions.

Result: Achieves 29.2% improvement over GPT-4o in validation metrics, excelling in information density and action complexity.

Conclusion: SV3.3B is a computationally efficient, high-performing model for detailed sports analysis, available for deployment.
...
Conclusion: Detail++ effectively addresses limitations in complex prompt handling through progressive detail injection and precise attribute binding.
...
<details>
  <summary>Details</summary>
Motivation: Address fragmented datasets, heterogeneous imaging conditions, and inconsistent evaluation protocols in fish detection for ecological and robotic applications.

Method: Harmonize 13 datasets with COCO-style annotations, benchmark 28 models (YOLO, R-CNN, DETR), and introduce a CLIP-based zero-shot selection framework.

Result: Varying performance across models, trade-offs between accuracy and efficiency, and high performance of the CLIP-based selection framework.

Conclusion: FishDet-M offers a standardized, reproducible platform for underwater detection, supporting future research with publicly available resources.
...
<details>
  <summary>Details</summary>
Motivation: To address biases in AI-based melanoma screening by ensuring fairness across diverse groups (sex, age, race).

Method: Uses state-of-the-art Generative AI (LightningDiT) to create synthetic data for fairness evaluation of melanoma classifiers.

Result: Fairness assessment with synthetic data is promising but challenging when evaluation models are trained on different datasets.

Conclusion: Proposes synthetic data as a valuable tool for improving fairness in medical-imaging AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Address the limitations of existing Transformer-based methods in image restoration, particularly the trade-off between computational efficiency and quality, and the need for localized artifact handling.

Method: Proposes DiNAT-IR, combining Dilated Neighborhood Attention (DiNA) with a channel-aware module to integrate global context while maintaining local precision.

Result: DiNAT-IR achieves competitive performance on multiple benchmarks, offering a high-quality solution for low-level vision tasks.

Conclusion: DiNAT-IR effectively balances global and local context, improving image restoration quality without excessive computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Existing UDA-SS methods struggle with balancing fine-grained local details and global contextual information, causing segmentation errors.
...
Method: The OPEN dataset was collected from older adults in virtual cardiac rehabilitation sessions, featuring extracted behavioral and affective features, and tested with machine learning models.
...
TL;DR: An automated system using YOLO models for real-time video analysis of bearded dragon behaviors (basking and hunting) was developed. YOLOv8s performed best, but hunting detection was less accurate due to poor cricket detection.
...
Method: Five YOLO variants were trained on a custom dataset (1200 images). YOLOv8s was selected for its accuracy-speed balance. Video analysis involved object detection, temporal interpolation, and rule-based behavior classification.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets for person Re-ID focus on ground-level perspectives, which have limitations like occlusions and limited coverage. Aerial perspectives can solve these issues, but no dataset addresses this.

Method: The authors introduce AG-VPReID.VIR, a dataset with 1,837 identities across RGB and infrared modalities. They propose TCC-VPReID, a three-stream architecture using style-robust feature learning, memory-based cross-view adaptation, and intermediary-guided temporal modeling.
...
Conclusion: The dataset and framework address gaps in aerial-ground and cross-modality Re-ID, offering a robust solution for 24-hour surveillance.
...
TL;DR: The paper investigates the impact of label bias in medical imaging datasets on deep learning models, focusing on subgroup size and separability. It reveals shifts in learned features and performance disparities based on biased vs. clean validation labels.


<details>
  <summary>Details</summary>
Motivation: To address the understudied issue of label bias in medical AI systems and its effect on subgroup fairness.
...
Result: Label bias caused shifts in feature representations, influenced by subgroup size and separability. Performance varied significantly when using biased vs. clean validation labels.

Conclusion: The study highlights the critical impact of label bias on subgroup fairness in medical AI, emphasizing the need for careful validation and bias mitigation.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods lack an explicit, optimizable distance function for rigid body transformations, limiting their use in registration tasks.

Method: Derives a cost function based on transformed bases of high-dimensional subspaces, minimizing geodesic distance for global optimization.

Result: The proposed cost function improves convergence and outperforms existing solutions in computer vision tasks.

Conclusion: The method provides a globally optimal solution for registration problems, addressing representation ambiguity and enhancing performance.
...
TL;DR: GRR-CoCa enhances the CoCa model with LLM-inspired architectural improvements, outperforming Baseline CoCa in pretraining and fine-tuning tasks.
...
Result: GRR-CoCa shows significant improvements: 27.25% in contrastive loss, 3.71% in perplexity, and 7.15% in CoCa loss during pretraining.
...
TL;DR: The paper introduces Celeb-DF++, a large-scale DeepFake dataset with diverse forgery types, addressing the challenge of generalizable forensics.
...
Method: Extends the Celeb-DF dataset to include three forgery scenarios (Face-swap, Face-reenactment, Talking-face) using 22 DeepFake methods.

Result: Celeb-DF++ provides a challenging benchmark, revealing limitations of current detection methods in handling diverse forgery types.

Conclusion: The dataset advances generalizable forensics research by offering a more comprehensive and realistic benchmark for DeepFake detection.
...
Method: Uses an automatic Mask Refinement Process and Uncertainty-guided Optimization to refine masks and enhance details.

Result: Outperforms state-of-the-art methods in visual quality and view consistency on diverse datasets.

Conclusion: The framework effectively addresses 3D inpainting challenges, improving fidelity and consistency.
...
<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive review of emotion recognition through body movements, leveraging advancements in 3D skeleton technology for privacy-preserving alternatives.

Method: Categorizes methods into posture-based and gait-based, introducing a taxonomy of four technical paradigms: Traditional, Feat2Net, FeatFusionNet, and End2EndNet.

Result: Benchmarking results across datasets and exploration of applications in mental health, like depression and autism detection.
...
Result: ViGText boosts F1 scores from 72.45% to 98.32% in generalization tests and improves recall by 11.1%. It limits performance degradation to <4% under targeted attacks.

Conclusion: ViGText sets a new standard for deepfake detection by leveraging detailed visual and textual analysis, enhancing media authenticity and information integrity.
...
<details>
  <summary>Details</summary>
Motivation: Current AI-generated video models perform well on single-scene clips but struggle with longer videos requiring scene transitions due to lack of training data for multi-scene scenarios.

Method: The authors propose the TAV dataset, consisting of video clips with multiple scene transitions, and demonstrate its effectiveness through post-training experiments.
...
Conclusion: The TAV dataset enhances AI-generated video models' capability to produce coherent multi-scene videos, addressing a key limitation in current approaches.
...
Result: Achieves high-quality, high-fidelity blur rendering and synthesizes photorealistic foregrounds with transparency.

Conclusion: BokehDiff effectively addresses depth estimation challenges and enhances visual quality in lens blur rendering.
...
Method: VLM-IMI leverages VLMs and IMIs, using textual descriptions of normal-light content as enhancement cues. It includes an instruction prior fusion module for cross-modal feature alignment and fusion.
...
Conclusion: The framework demonstrates the effectiveness of integrating semantic guidance and iterative refinement for superior LLIE performance.
...
<details>
  <summary>Details</summary>
Motivation: EUS images have noise and low contrast, making segmentation error-prone and reliant on expert annotations.

Method: Uses BiomedCLIP text encoder and LoRA-based SAM adaptation for text prompt learning, requiring no manual prompts.

Result: Achieves 82.69% Dice and 85.28% NSD with automatic prompts, surpassing SOTA models.

Conclusion: TextSAM-EUS is efficient and robust for automatic EUS segmentation, pioneering prompt learning in SAM-based medical imaging.
...
TL;DR: The study evaluates advanced LULC mapping techniques using LUT-based atmospheric correction and learning models (DeeplabV3+ and CPS) for urban planning, demonstrated in Hyderabad.


<details>
  <summary>Details</summary>
Motivation: LULC mapping is crucial for smart and sustainable city development, especially in rapidly urbanizing areas like Hyderabad.

Method: Combines LUT-based atmospheric correction with supervised/semi-supervised learning (DeeplabV3+ and CPS) for LULC prediction, enhanced by dynamic weighting.

Result: Highlights significant land use changes (urban sprawl, shrinking green spaces, industrial expansion) in Hyderabad, proving the techniques' utility.

Conclusion: The proposed methods are effective for LULC mapping, aiding urban planners and policymakers in sustainable development.
...
TL;DR: A two-stage training framework combining supervised fine-tuning (SFT) and reinforcement learning (RL) improves video temporal grounding (VTG) by enhancing temporal awareness and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing VTG approaches lack temporal awareness and struggle with generalization, prompting the need for a more robust method.

Method: The framework uses high-quality cold start data for SFT initialization, followed by difficulty-controlled RL to refine temporal localization and reasoning.

Result: The method outperforms existing models on multiple benchmarks, especially in challenging and open-domain scenarios.

Conclusion: High-quality cold start data and difficulty-controlled RL are crucial for VTG performance, with released resources aiding further research.
...
Conclusion: The model effectively captures long-range temporal structure and individual variability, showcasing the potential of multimodal sequence modeling for brain activity prediction.
...
TL;DR: Proposes Free-Energy Posterior Network for joint modeling of distributional uncertainty and OoD detection, outperforming conventional methods like MC Dropout.


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing methods (e.g., MC Dropout) that fail to align with semantic OoD detection by focusing only on model or data uncertainty.

Method: Introduces a free-energy-based density estimator (Beta distribution) and integrates it with a posterior network for direct uncertainty estimation. Combines with RPL framework for learning OoD regions.

Result: Validated on benchmarks (Fishyscapes, RoadAnomaly, Segment-Me-If-You-Can), showing improved uncertainty-aware segmentation.
...
TL;DR: T2VWorldBench is introduced as the first framework to evaluate text-to-video models' world knowledge generation, revealing significant gaps in current models' capabilities.


<details>
  <summary>Details</summary>
Motivation: The lack of study on text-to-video models' ability to ensure semantic consistency and factual accuracy using world knowledge.

Method: Proposes T2VWorldBench, covering 6 categories and 1,200 prompts, using human and automated (VLMs) evaluation.

Result: Most advanced models fail to generate correct videos due to poor world knowledge understanding.

Conclusion: Highlights a critical gap in current models, suggesting opportunities for improving commonsense reasoning and factual generation.
...
Conclusion: The framework is effective for medical curve analysis and has potential as a generalizable tool for morphological evaluation.
...
TL;DR: The paper proposes a bidirectional diffusion optimization mechanism for low-light image enhancement, addressing structural and pixel inconsistencies by jointly modeling degradation processes of low-light and normal-light images. It introduces adaptive feature interaction and reflection-aware correction to improve quality.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based methods struggle with real-world degradation complexity, leading to structural and pixel misalignments. The goal is to enhance image visibility while aligning with human visual perception.

Method: A bidirectional diffusion mechanism models degradation in both low-light and normal-light images. It includes adaptive feature interaction (AFI) and reflection-aware correction (RACM) for refined feature representation and color restoration.
...
Conclusion: The proposed bidirectional approach effectively addresses degradation challenges, improving image quality and alignment with human perception.
...
Method: Uses Discrete Wavelet Transform (DWT) for decomposition, WaveMamba Fusion Block (WMFB) for fusion, and IDWT for reconstruction. Includes Low-frequency Mamba Fusion Block (LMFB) and high-frequency enhancement.
...
Method: Optimized YOLOv5 for FPGA deployment, trained on COCO and GTSRD datasets, implemented on Xilinx Kria KV260.

Result: 99% classification accuracy, 3.5W power consumption, 9 FPS processing speed.

Conclusion: The system effectively enables real-time, resource-efficient object detection for edge computing.
...
Method: 1. Unsupervised contrastive learning pre-trains a backbone network for domain-invariant features. 2. Multi-model pseudo-labeling via ensemble of diverse architectures (projection, voxel, hybrid, cylinder-based) generates refined pseudo-labels.

Result: Experiments show significant accuracy improvements when adapting from SemanticKITTI to unlabeled datasets (SemanticPOSS, SemanticSlamantic).
...
TL;DR: Diff-UMamba combines UNet with mamba for medical image segmentation, using a Noise Reduction Module to improve focus on clinically relevant regions, achieving 1-5% gains in accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep learning models overfit in data-scarce scenarios, limiting generalization. This work aims to improve segmentation accuracy and robustness in low-data settings.
...
Result: Consistent performance gains of 1-3% on public datasets (MSD, AIIB23) and 4-5% on a small NSCLC dataset.

Conclusion: Diff-UMamba effectively enhances segmentation accuracy and robustness, especially in low-data scenarios, by filtering noise and focusing on task-relevant features.
...
<details>
  <summary>Details</summary>
Motivation: Current supervised methods for micrograph analysis require retraining for new datasets and perform poorly with few labels. SSL alternatives often need large datasets. MatSSL aims to address these limitations.
...
TL;DR: TeEFusion is a distillation method that simplifies text-to-image synthesis by embedding guidance into text embeddings, enabling faster inference while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: High inference costs of classifier-free guidance (CFG) and complex sampling strategies in text-to-image synthesis need addressing.
...
Result: The student model achieves 6x faster inference speeds while matching the teacher's image quality.
...
Conclusion: LEAF efficiently improves segmentation accuracy while maintaining model simplicity and efficiency.
...
TL;DR: GSDTTA introduces a graph spectral domain approach for efficient test-time adaptation of 3D point clouds, outperforming existing methods.
...
Method: Uses Graph Fourier Transform (GFT) to shift adaptation to the spectral domain, optimizing low-frequency components and employing eigenmap-guided self-training.
...
<details>
  <summary>Details</summary>
Motivation: Feature-level fusion in collaborative perception is hindered by domain gaps from hardware diversity and deployment conditions, as well as temporal misalignment from transmission delays, degrading feature quality.

Method: The DATA network includes a Consistency-preserving Domain Alignment Module (CDAM) for domain gap reduction, a Progressive Temporal Alignment Module (PTAM) for handling delays, and an Instance-focused Feature Aggregation Module (IFAM) for enhanced semantic representations.

Result: DATA achieves state-of-the-art performance on three datasets, maintaining robustness under severe communication delays and pose errors.

Conclusion: The DATA network effectively aligns features and maximizes their semantic representations, demonstrating superior performance and robustness in collaborative perception.
...
Conclusion: DepthDark effectively addresses low-light depth estimation with limited resources.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-view scene reconstruction struggle with real-time processing of long sequences, limiting practical applications.

Method: LONG3R uses a memory gating mechanism, a dual-source refined decoder, and a 3D spatio-temporal memory for efficient long-sequence processing. It employs a two-stage curriculum training strategy.

Result: LONG3R surpasses state-of-the-art methods in performance for long sequences while maintaining real-time speed.

Conclusion: LONG3R advances real-time 3D reconstruction for long sequences, offering improved efficiency and applicability.
...
<details>
  <summary>Details</summary>
Motivation: Current ISTD methods rely on expensive manual-labeling data, making them fragile in real-world scenarios. The study explores performance variations under data scarcity and proposes a solution.

Method: Proposes Gaussian Group Squeezer for non-uniform quantization and two-stage diffusion models for real-world reconstruction, enhancing model resilience and sample fidelity.

Result: The approach outperforms state-of-the-art ISTD methods in scarcity scenarios, improving detection quality and robustness.

Conclusion: The proposed method effectively addresses data scarcity challenges in ISTD, offering superior performance and practical applicability.
...
<details>
  <summary>Details</summary>
Motivation: To clarify the uncertain causality between oral diseases (periodontitis, dental caries) and lung cancer, and assess mediation by pulmonary function.

Method: Two-sample Mendelian randomization (MR) with genetic instruments from large GWAS datasets, using inverse variance weighting and the delta method for mediation analysis.

Result: Dental caries significantly increases lung cancer risk, especially squamous cell carcinoma (OR = 2.880), partially mediated by FVC and FEV1 decline. No effect found for periodontitis.
...
TL;DR: LMM-Det is a simple yet effective approach leveraging Large Multimodal Models (LMMs) for object detection without specialized modules, addressing recall degradation through data distribution adjustment and inference optimization.


<details>
  <summary>Details</summary>
Motivation: The gap in object detection capabilities of LMMs compared to specialist detectors motivates the development of LMM-Det to enhance LMMs' detection performance without heavy integration.

Method: Proposes LMM-Det, which reorganizes instruction conversations and introduces data distribution adjustment and inference optimization to improve recall rates.

Result: Extensive experiments show LMM-Det effectively enhances object detection in LMMs without extra modules.

Conclusion: LMM-Det demonstrates that LMMs can perform object detection without specialized modules, validated by experimental results.
...
TL;DR: FieldLVLM enhances LVLMs for scientific field data interpretation using field-aware language generation and data-compressed tuning, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: LVLMs excel in general tasks but lack performance in scientific domains, especially for complex field data.

Method: FieldLVLM combines field-aware language generation (extracting key features) and data-compressed multimodal tuning (simplifying inputs).

Result: FieldLVLM outperforms existing methods on scientific field data tasks.

Conclusion: This approach bridges LVLMs and domain-specific scientific research, enabling new applications.
...
TL;DR: A benchmark for semi-supervised ECG delineation was created, comparing transformer and convolutional networks, with transformers outperforming.
...
Method: Curated public datasets, implemented five SemiSeg algorithms on CNN and transformer architectures, evaluated in-domain and cross-domain.
...
TL;DR: The paper introduces a Modified Nuclear Norm (MNN) framework for matrix recovery, combining local and global low-rank structures without parameter tuning, and provides theoretical guarantees for Robust PCA and matrix completion tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to jointly capture local and global low-rank structures without trade-off parameter tuning. The MNN framework aims to address this limitation.
...
Result: Theoretical recovery guarantees are provided for Robust PCA and matrix completion, and experiments confirm the method's effectiveness.

Conclusion: The MNN framework offers a flexible, unified approach for structured low-rank recovery, outperforming existing methods.
...
TL;DR: The paper introduces GVCCS, a ground-based dataset for tracking and analyzing contrails, and a deep learning framework for their segmentation and tracking, aiming to improve climate impact models.


<details>
  <summary>Details</summary>
Motivation: Contrails significantly impact aviation's climate effects, but current models lack accurate validation due to insufficient observational data.

Method: The authors present GVCCS, a dataset of labeled and tracked contrails from ground cameras, and a unified deep learning framework for contrail analysis.

Result: The dataset includes 122 video sequences (24,228 frames) with flight identifiers, and the proposed model performs semantic and instance segmentation with temporal tracking.

Conclusion: GVCCS and the deep learning framework enhance contrail monitoring and model calibration, advancing climate impact assessments.
...
TL;DR: SGCDet is a multi-view 3D object detection framework using adaptive 3D volume construction and dynamic view contribution adjustment, achieving state-of-the-art results without needing ground-truth scene geometry.
...
Result: Achieves top performance on ScanNet, ScanNet200, and ARKitScenes datasets.

Conclusion: SGCDet offers efficient, adaptive 3D object detection with minimal reliance on ground-truth data.
...
TL;DR: A method for classifying bird species by visualizing pitch patterns, speed, and repetition (motifs) in spectrograms, enhanced with colorization to improve accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing models struggle with noisy or multi-species bird song recordings, requiring better methods to distinguish species.

Method: Embed frequency information into spectrograms using primary color additives to enhance species distinction.

Result: Significant improvements: F1 by 7.3%, ROC-AUC by 6.2%, and CMAP by 6.6% over baseline models.

Conclusion: Colorization of spectrograms effectively improves bird species classification accuracy.
...
TL;DR: EgoExoBench is a new benchmark for evaluating cross-view reasoning in MLLMs, highlighting their limitations in aligning semantics, associating viewpoints, and temporal reasoning.


<details>
  <summary>Details</summary>
Motivation: To explore and improve the ability of multimodal large language models (MLLMs) to transfer and integrate knowledge between egocentric and exocentric viewpoints, a key aspect of human intelligence.

Method: Introduces EgoExoBench, a benchmark with over 7,300 question-answer pairs across 11 sub-tasks, organized into three challenges: semantic alignment, viewpoint association, and temporal reasoning. Evaluates 13 state-of-the-art MLLMs.
...
Conclusion: EgoExoBench is a valuable resource for advancing research in embodied agents and intelligent assistants aiming for human-like cross-view intelligence.
...
TL;DR: VB-Mitigator is an open-source framework for standardized evaluation and development of visual bias mitigation techniques in computer vision.
...
Method: VB-Mitigator integrates 12 mitigation methods, 7 datasets, and offers extensibility for new additions.

Result: The framework provides a unified environment for fair comparison and accelerates fairness-aware research.

Conclusion: VB-Mitigator serves as a foundational tool for the community, promoting reproducibility and best practices in bias mitigation.
...
<details>
  <summary>Details</summary>
Motivation: To address complex shape features in tasks like fundus blood vessel segmentation, where traditional deformable convolutions fall short in capturing long-distance global features.

Method: Proposes a plug-and-play deformable convolutional module that learns sub-pixel displacement fields and warps feature maps adaptively across channels, decoupling kernel size from the learning network.

Result: GDCUnet, a model using this module, achieves state-of-the-art performance on public datasets, with ablation studies confirming its effectiveness in learning complex features.

Conclusion: The module, compatible with conventional convolution interfaces, is recommended for machine vision tasks involving complex global self-similar features.
...
TL;DR: MVG4D is a novel framework for generating high-fidelity dynamic 4D content from a single image, combining multi-view synthesis with 4D Gaussian Splatting to improve temporal consistency and visual realism.


<details>
  <summary>Details</summary>
Motivation: Producing high-fidelity and temporally consistent dynamic 4D content from minimal inputs remains a challenge, which MVG4D aims to address.

Method: MVG4D uses an image matrix module for multi-view synthesis, optimizes a 3D Gaussian point cloud, and extends it temporally with a lightweight deformation network.

Result: MVG4D outperforms state-of-the-art methods in metrics like CLIP-I, PSNR, and FVD, reducing flickering and enhancing structural details.

Conclusion: MVG4D advances efficient and controllable 4D generation, enabling more immersive AR/VR experiences.
...
TL;DR: The paper introduces a framework and dataset for evaluating human-AI collaboration in physical tasks, using AR-equipped AI agents to improve performance and learning.
...
Conclusion: The study demonstrates the potential of AI guidance in improving human performance and provides tools for future research in human-AI collaboration.
...
TL;DR: A novel one-stage architecture for pose generation directly in continuous coordinate space, outperforming existing methods by eliminating intermediate representations and maintaining temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Current methods rely on intermediate representations or autoregressive models, leading to degraded performance in long-term pose generation due to error accumulation and lack of coherence.

Method: Proposes a one-stage model that generates poses directly from an RGB image and text description, using relative movement prediction and a unified placeholder token for consistent training and inference.
...
Conclusion: The proposed architecture effectively addresses limitations of existing methods by directly generating poses in continuous space, ensuring temporal coherence and improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for human inverse rendering lack realism, especially for skin, due to simplified material data and rendering equations.

Method: Constructs OpenHumanBRDF dataset with detailed materials (displacement, subsurface scattering) and designs HumanMaterial model with progressive training and Controlled PBR Rendering loss.

Result: Achieves state-of-the-art performance on material estimation, validated on OpenHumanBRDF and real data.
...
<details>
  <summary>Details</summary>
Motivation: To overcome Swin Transformer's limitation of requiring two blocks for global attention and to enable direct fine-tuning across resolutions.

Method: Combines interleaved window attention and depthwise separable convolution to link distant and neighboring tokens, respectively, in a single module.

Result: Achieves 87.4 top-1 accuracy on ImageNet-1K and performs well in semantic segmentation and video action recognition. Also effective as a standalone module for image generation.

Conclusion: Iwin Transformer is a competitive and versatile model, with potential to inspire future research in vision tasks and video generation.
...
TL;DR: DCFFSNet improves medical image segmentation by decoupling connectivity features, dynamically balancing multi-scale features, and outperforming existing methods on key datasets.


<details>
  <summary>Details</summary>
Motivation: Existing deep networks integrating connectivity lack standardized mechanisms to quantify feature strengths, leading to coupled feature spaces and suboptimal performance.

Method: Proposes DCFFSNet with a feature space decoupling strategy to quantify connectivity feature strength and a fusion-separation architecture for dynamic multi-scale feature balancing.

Result: Outperforms top models on ISIC2018, DSB2018, and MoNuSeg datasets, with improvements in Dice and IoU scores.

Conclusion: DCFFSNet resolves segmentation fragmentation, ensures smooth edge transitions, and enhances clinical usability, surpassing mainstream methods.
...
TL;DR: The paper explores self-supervised learning (SSL) with V-JEPA for ultrasound video segmentation, proposing a 3D localization task to enhance ViT performance, achieving significant gains with limited data.


<details>
  <summary>Details</summary>
Motivation: Challenges in acquiring and annotating ultrasound datasets due to noise and artifacts motivate the use of SSL to leverage unlabelled data for better segmentation.

Method: Adopts V-JEPA for ultrasound video data, introduces a 3D localization auxiliary task to improve ViT locality during pre-training.

Result: V-JEPA with the auxiliary task improves segmentation performance by up to 3.4% (100% data) and 8.35% (10% data).

Conclusion: The proposed method effectively enhances segmentation in ultrasound imaging, especially with limited annotated data.
...
Method: Combines tensor decomposition (Tucker decomposition) and feed-forward neural networks to model head pose as a regression problem, representing pose angles via cosine curves in separate subspaces.

Result: Achieves real-time performance and accurate pose estimation by learning the underlying rotation manifold from facial landmarks, validated with a precisely generated 2D dataset.
...
<details>
  <summary>Details</summary>
Motivation: Synthetic datasets from text-to-image models often have semantic misalignments, hindering ZIC model training. Existing pruning methods are unsuitable for synthetic data.

Method: SynC uses a one-to-many mapping strategy and cycle-consistency-inspired alignment to reassign captions to the best images in the synthetic pool.

Result: SynC improves ZIC model performance on benchmarks like MS-COCO, Flickr30k, and NoCaps, achieving state-of-the-art results.

Conclusion: SynC effectively refines synthetic datasets, enhancing ZIC performance.
...
TL;DR: A novel framework combining Dual-Scale-Former (DSFormer) and block clustering improves Visual Place Recognition (VPR) robustness and efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in VPR under varying conditions and viewpoints.

Method: Integrates DSFormer for bidirectional feature learning and block clustering for optimized dataset partitioning.

Result: Achieves state-of-the-art performance, reduces training data by 30%, and improves efficiency.

Conclusion: The framework enhances VPR robustness and efficiency, outperforming existing methods.
...
TL;DR: The paper introduces PDB-Eval, a benchmark for understanding personalized driver behavior using multimodal models (MLLMs), improving performance in driving-related tasks.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack detailed explanations of driver behavior from external visual evidence, limiting the effectiveness of safety systems.

Method: PDB-Eval includes PDB-X for temporal scene understanding and PDB-QA for fine-tuning MLLMs with visual explanations.

Result: Fine-tuning MLLMs improved zero-shot QA performance by 73.2% and enhanced intention prediction in Brain4Cars (12.5%) and AIDE tasks (11.0%).

Conclusion: Fine-grained descriptions and explanations align MLLMs with driving tasks, bridging the domain gap without losing generalizability.
...
TL;DR: A standardized framework for physical adversarial object attacks in LiDAR-based 3D object detection is proposed, enabling fair comparison and real-world applicability.
...
Method: Device-agnostic framework with open-source code and benchmarking protocols for simulation and real-world validation.

Result: Successful transfer of simulated attacks to physical LiDAR systems and insights into attack success factors.
...
Method: CRUISE uses decomposed Gaussian Splatting to reconstruct real-world scenes and allows flexible editing of dynamic traffic participants.
...
Method: The Q-Former Autoencoder uses frozen pretrained vision models (e.g., DINO, DINOv2) as feature extractors, incorporates a Q-Former bottleneck for multiscale feature aggregation, and employs a perceptual loss from a Masked Autoencoder for semantic guidance.

Result: The framework achieves state-of-the-art performance on BraTS2021, RESC, and RSNA benchmarks, demonstrating effective generalization of pretrained models to medical tasks.

Conclusion: Pretrained vision models can generalize well to medical anomaly detection without fine-tuning, as shown by the Q-Former Autoencoder's success. Code and models are publicly released.
...
TL;DR: The paper presents an enhanced NIH malaria dataset with detailed COCO-format annotations for object detection in malaria diagnosis, validated using Faster R-CNN with strong performance (F1 score up to 0.88).


<details>
  <summary>Details</summary>
Motivation: Reliable malaria diagnosis in developing countries requires accurate detection of Plasmodium falciparum in blood smears, but deep learning adoption is hindered by scarce annotated datasets.

Method: The authors improved the NIH malaria dataset with detailed bounding box annotations, trained a Faster R-CNN model for detecting infected/non-infected cells, and validated performance via cross-validation.

Result: Cross-validation showed high F1 scores (up to 0.88) for infected cell detection, highlighting the importance of annotation quality.

Conclusion: Automated annotation refinement with manual correction can produce high-quality training data, as demonstrated by the publicly available updated dataset.
...
<details>
  <summary>Details</summary>
Motivation: Existing defenses rely on pre-defined assumptions, limiting adaptability in dynamic 3D adversarial settings.

Method: Rein-EAD employs adaptive exploration, multi-step objectives balancing accuracy and entropy minimization, and uncertainty-oriented reward shaping.
...
Conclusion: Rein-EAD is effective for real-world 3D adversarial contexts, including object classification, face recognition, and autonomous driving.
...
Method: The authors analyze scenarios where map uncertainty benefits trajectory prediction, introduce Proprioceptive Scenario Gating to adaptively integrate uncertainty, and propose a Covariance-based Map Uncertainty approach for better alignment with map geometry.

Result: The method achieves up to 23.6% improvement in trajectory prediction performance on the nuScenes dataset compared to state-of-the-art methods.

Conclusion: The proposed approach enhances the synergy between online mapping and trajectory prediction, providing interpretability and performance gains, with publicly available code and data.
...
<details>
  <summary>Details</summary>
Motivation: To improve human-like attention modeling by integrating deep object detection with probabilistic semantic fusion, leveraging advancements in deep learning and human scanpath data.

Method: SemBA-FAST combines deep object detection, probabilistic semantic fusion, and artificial foveation to dynamically generate attention maps and predict fixations.
...
TL;DR: The paper introduces tools for comparing SAE-derived features across visual, textual, and multimodal encoders, revealing shared representations and the impact of text pretraining.


<details>
  <summary>Details</summary>
Motivation: To enable quantitative comparison of SAE-derived features across different modalities and revisit previous studies in a multimodal context.

Method: Proposes a novel indicator for cross-modal SAE feature comparison and a metric for feature sharedness, applied to 21 encoders of varying sizes and datasets.

Result: Finds shared visual features between VLMs and text encoders, emphasizing text pretraining's influence.

Conclusion: The tools provide insights into shared representations across modalities, with implications for understanding multimodal training effects.
...
TL;DR: A method for detecting rare bio-digesters in remote sensing imagery using a part-based approach, applied to estimate methane production in France.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of detecting rare objects (bio-digesters) in large remote sensing datasets, crucial for environmental impact assessments.

Method: Introduces a novel dataset and a part-based detection method focusing on sub-elements of bio-digesters to improve detection accuracy.

Result: Successfully builds an inventory of bio-digesters and estimates methane production geostatistically.

Conclusion: The proposed method effectively detects rare objects and provides valuable environmental impact estimates.
...
TL;DR: The paper explores using foundation models for semantic object segmentation in cluttered scenes without fine-tuning, focusing on vision-guided neuroprostheses. A gaze-based prompt method for SAM is proposed, showing a 0.51 IoU improvement on real-world data.
...
Conclusion: The approach effectively leverages foundation models for segmentation in challenging, real-world scenarios.
...
TL;DR: GaussianFusionOcc introduces a 3D Gaussian-based method for semantic occupancy prediction in autonomous driving, combining camera, LiDAR, and radar data for precise, efficient results.


<details>
  <summary>Details</summary>
Motivation: To improve 3D semantic occupancy prediction for autonomous driving by addressing the limitations of dense grid representations and enhancing sensor fusion.
...
Result: Outperforms state-of-the-art models in precision, scalability, memory efficiency, and inference speed.

Conclusion: GaussianFusionOcc demonstrates superior performance by combining multi-modal fusion robustness with Gaussian representation efficiency.
...
Method: Proposes a prompt combination strategy and a parameter-efficient box adapter to enhance LVLMs' spatial and temporal understanding.

Result: Achieves state-of-the-art results and runner-up in the IntentVC challenge.

Conclusion: IntentVCNet effectively enhances LVLMs' ability to generate controlled intent-oriented captions.
...
Method: Includes aerial and DSLR images with annotations for pest/disease recognition, vegetation, and weed analysis.

Result: Provides a comprehensive dataset enabling tasks like image restoration, enhancement, and crop synthesis.
...
TL;DR: EDA expands diffusion models to handle arbitrary noise patterns, improving image restoration without extra computational cost. It outperforms task-specific methods in MRI, CT, and natural image tasks.


<details>
  <summary>Details</summary>
Motivation: Fixed Gaussian noise in EDM limits image restoration by corrupting degraded images and increasing complexity. EDA aims to address this by allowing arbitrary noise patterns.

Method: EDA generalizes noise patterns while preserving EDM's flexibility, theoretically proving no added computational cost. It is tested on MRI bias correction, CT artifact reduction, and shadow removal.

Result: EDA achieves state-of-the-art performance in bias field correction and shadow removal with just 5 sampling steps, surpassing task-specific methods.

Conclusion: EDA successfully broadens diffusion models' noise design space, enhancing restoration performance without computational overhead.
...
Method: The framework models generation as path searching, using adaptive batch size schedules, clustering-based diversity search for coarse scales, and resampling-based potential selection for fine scales.
...
Method: The method partitions images into subsets, maps submaps globally using probabilistic Procrustes, and jointly optimizes geometry and poses with 3DGS. It employs probabilistic coupling and a soft dustbin mechanism for alignment.
...
TL;DR: A novel 3D cross-modal keypoint descriptor for MRI-iUS registration is proposed, using synthetic iUS volumes and supervised contrastive training to improve matching accuracy.


<details>
  <summary>Details</summary>
Motivation: Intraoperative ultrasound (iUS) to preoperative MRI registration is challenging due to modality differences. The goal is to develop a robust, interpretable method without manual initialization.

Method: Patient-specific synthetic iUS volumes are generated from MRI. A probabilistic keypoint detection and curriculum-based triplet loss train descriptors. Sparse matches enable rigid registration.

Result: Outperforms state-of-the-art with 69.8% average precision and 2.39 mm mean Target Registration Error on the ReMIND dataset.

Conclusion: The method is robust, interpretable, and achieves competitive registration accuracy without manual intervention.
...
<details>
  <summary>Details</summary>
Motivation: To advance deep video content cognition and multi-modal feature representation by providing a dataset with intent expressions and hierarchical descriptions.

Method: The dataset includes 103K videos with audio and detailed textual descriptions (22M words). Intent expressions are generated via Chain-of-Thought. A gold-standard benchmark of 3K samples is manually validated.

Result: Establishes a benchmark for deep-cognitive video understanding, with evaluation results for models like InternVideo, VAST, and UMT-L.
...
TL;DR: A pipeline for generating synthetic, labeled images of chicken carcasses is introduced to improve instance segmentation in poultry processing, alongside a new benchmark dataset. Synthetic data significantly enhances segmentation performance.
...
Method: Developed a pipeline for synthetic image generation and introduced a benchmark dataset. Evaluated synthetic data's impact on instance segmentation models using varying proportions of synthetic and real images.

Result: Synthetic data significantly improves segmentation performance across all tested models, demonstrating its value in mitigating data scarcity.

Conclusion: Synthetic data augmentation is an effective strategy to reduce manual annotation efforts and enhance AI-driven detection systems in poultry processing.
...
TL;DR: A deep learning-based approach for simultaneous age and gender classification using a custom CNN, achieving 95% gender accuracy and 5.77 MAE for age, with insights on biases and model improvements.


<details>
  <summary>Details</summary>
Motivation: To enhance targeted advertising by improving age and gender classification from facial images, leveraging shared representations between the two tasks.

Method: Proposes a custom CNN architecture trained on a diverse dataset, optimized for joint age and gender classification, with pre-processing for robustness.

Result: Achieves 95% gender classification accuracy and 5.77 MAE for age, with identified biases in younger age groups.

Conclusion: Highlights the need for targeted data augmentation and model refinement, offering insights for future research in CNN architectures.
...
TL;DR: A method for demorphing face images overcomes limitations of existing techniques by decomposing morphs in latent space, enabling detection of unseen morph techniques and styles.


<details>
  <summary>Details</summary>
Motivation: Existing demorphing methods struggle with morph replication and require identical morph techniques for training and testing, limiting their practical use.
...
Result: The method outperforms existing techniques, producing high-fidelity demorphed images and handling unseen morph styles.

Conclusion: This approach significantly advances demorphing capabilities, offering robust detection and high-quality constituent image recovery.
...
Method: Propose ADM for adversarial alignment, hybrid discriminators, and distributional loss for better initialization.

Result: Superior one-step performance on SDXL and benchmarks in image/video synthesis.

Conclusion: DMDX sets a new standard for efficient distillation in generative tasks.
...
TL;DR: HybridTM integrates Transformer and Mamba for 3D semantic segmentation, combining their strengths to address long-range dependencies and feature representation challenges.


<details>
  <summary>Details</summary>
Motivation: Transformers excel in attention mechanisms but struggle with quadratic complexity, while Mamba offers linear complexity but lacks feature representation. Combining these complementary strengths is an open challenge.

Method: Proposes HybridTM, a hybrid architecture integrating Transformer and Mamba, and the Inner Layer Hybrid Strategy for finer granularity in combining attention and Mamba.
...
Conclusion: HybridTM successfully combines Transformer and Mamba, addressing their limitations and advancing 3D semantic segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Addressing challenges in preserving edge continuity and structural details under extreme low-light conditions.

Method: Combines Global Edge Retinex (GER) theory, Evolving WKV Attention for spatial edge continuity, and Bilateral Spectrum Aligner (Bi-SAB) with MS2-Loss for feature alignment.

Result: Achieves leading PSNR, SSIM, and NIQE scores on benchmarks and enhances low-light multi-object tracking.

Conclusion: DRWKV effectively improves low-light image quality and generalizes well to downstream tasks.
...
TL;DR: Scenethesis introduces a requirement-sensitive 3D software synthesis approach with formal traceability, addressing challenges in 3D UI generation by using a domain-specific language for granular control and constraint handling.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D software generation lack granular control and struggle with spatial/semantic constraints, motivating the need for a more precise and constraint-aware approach.

Method: Scenethesis uses ScenethesisLang, a domain-specific language, as an intermediate representation to bridge natural language requirements and executable 3D software, enabling fine-grained modification and constraint expression.
...
Conclusion: Scenethesis effectively addresses the limitations of current 3D software generation methods, offering precise control and constraint satisfaction.
...
<details>
  <summary>Details</summary>
Motivation: Existing text-driven zero-shot domain adaptation methods struggle with complex variations and slow adaptation. Image data offers better style cues.

Method: SIDA generates synthetic images via source-like images and style translation, using Domain Mix and Patch Style Transfer for effective adaptation.

Result: Achieves state-of-the-art performance in challenging domains and reduces adaptation time significantly.

Conclusion: SIDA provides an efficient and effective solution for zero-shot domain adaptation by leveraging synthetic images.
...
Method: Created a dataset of 1.95M images from 110 artists, testing feature similarity, contrastive style descriptors, supervised classifiers, and few-shot networks across four generalization settings.
...
Conclusion: The benchmark highlights room for improvement and aims to advance responsible moderation of text-to-image models, with the dataset publicly released.
...
Method: Combines top-down keyframe planning for storyline coherence and bottom-up video synthesis using MM-DiT, with interleaved training for long-context video data.
...
### [140] [Improving Multislice Electron Ptychography with a Generative Prior](https://arxiv.org/abs/2507.17800)
*Christian K. Belardi,Chia-Hao Lee,Yingheng Wang,Justin Lovelace,Kilian Q. Weinberger,David A. Muller,Carla P. Gomes*

Main category: eess.IV

TL;DR: MEP-Diffusion, a diffusion model trained for multislice electron ptychography (MEP), enhances reconstruction quality by integrating with iterative solvers, improving SSIM by 90.50%.
...
Result: Achieved a 90.50% improvement in SSIM over existing methods, enhancing 3D volume reconstruction quality.

Conclusion: MEP-Diffusion significantly improves MEP reconstruction by combining generative priors with iterative solvers.
...
### [141] [Towards Robust Foundation Models for Digital Pathology](https://arxiv.org/abs/2507.17845)
...
TL;DR: The paper investigates the robustness of Biomedical Foundation Models (FMs) to non-biological technical features, introduces PathoROB for evaluation, and highlights the risks of non-robust FMs in clinical settings.


<details>
  <summary>Details</summary>
Motivation: FMs in healthcare are prone to learning non-biological technical features, posing risks for clinical deployment. This work aims to systematically evaluate and improve FM robustness.

Method: Developed PathoROB, a benchmark with three metrics and four datasets, to assess FM robustness. Evaluated 20 FMs and proposed a robustification framework.

Result: Found robustness deficits in all 20 FMs, with non-robust representations causing diagnostic errors. Robust FMs and post-hoc robustification reduced but did not eliminate risks.

Conclusion: Robustness evaluation is crucial for clinical FM adoption. Future FM development must prioritize robustness, and PathoROB provides a blueprint for assessing robustness in biomedical domains.
...
### [142] [Integrating Feature Selection and Machine Learning for Nitrogen Assessment in Grapevine Leaves using In-Field Hyperspectral Imaging](https://arxiv.org/abs/2507.17869)
...
<details>
  <summary>Details</summary>
Motivation: Nitrogen is vital for vineyards, but its variability in soil makes precise estimation and management at the plant level essential for optimal growth and product quality.
...
Conclusion: Hyperspectral imaging combined with feature selection and ML is effective for monitoring N status in vineyards, aiding precision agriculture.
...
### [143] [Hierarchical Diffusion Framework for Pseudo-Healthy Brain MRI Inpainting with Enhanced 3D Consistency](https://arxiv.org/abs/2507.17911)
...
<details>
  <summary>Details</summary>
Motivation: Current 2D inpainting methods cause discontinuities in MRI volumes, while 3D models require extensive training data, which is often impractical in medical settings.

Method: Uses two perpendicular coarse-to-fine 2D stages: an axial diffusion model for global consistency and a coronal model for refining details, with adaptive resampling.

Result: Outperforms state-of-the-art baselines in realism and volumetric consistency.

Conclusion: The method effectively balances data efficiency and volumetric consistency, making it a promising solution for pseudo-healthy MRI inpainting.
...
### [144] [Benchmarking of Deep Learning Methods for Generic MRI Multi-OrganAbdominal Segmentation](https://arxiv.org/abs/2507.17971)
*Deepa Krishnaswamy,Cosmin Ciausu,Steve Pieper,Ron Kikinis,Benjamin Billot,Andrey Fedorov*

Main category: eess.IV

TL;DR: A benchmarking study compares three MRI abdominal segmentation tools and introduces ABDSynth, a model trained on CT data, revealing MRSegmentator as the top performer while ABDSynth offers a viable alternative with lower annotation needs.


<details>
  <summary>Details</summary>
Motivation: MRI segmentation is challenging due to signal variability and limited annotated datasets, prompting a need to evaluate and improve generalizability of existing tools.

Method: Three state-of-the-art models (MRSegmentator, MRISegmentator-Abdomen, TotalSegmentator MRI) and ABDSynth (trained on CT data) are benchmarked using three public datasets covering diverse MRI conditions.

Result: MRSegmentator performs best in accuracy and generalizability, while ABDSynth, though slightly less accurate, is a practical option with lower training data requirements.

Conclusion: The study provides a benchmark for MRI abdominal segmentation, highlighting MRSegmentator's superiority and ABDSynth's utility in annotation-limited scenarios, with open-source resources for future work.
...
### [145] [Direct Dual-Energy CT Material Decomposition using Model-based Denoising Diffusion Model](https://arxiv.org/abs/2507.18012)
*Hang Xu,Alexandre Bousse,Alessandro Perelli*

Main category: eess.IV

TL;DR: DEcomp-MoD is a deep learning method for DECT material decomposition, directly converting projection data into material images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current DECT material decomposition methods are sub-optimal due to post-processing in the image domain and beam-hardening effects.

Method: Uses a model-based conditional diffusion model, incorporating spectral DECT knowledge into training and ensuring sinogram-to-material-image consistency.
...
### [146] [Parameter-Efficient Fine-Tuning of 3D DDPM for MRI Image Generation Using Tensor Networks](https://arxiv.org/abs/2507.18112)
...
<details>
  <summary>Details</summary>
Motivation: Addresses the limited research on parameter-efficient representations for 3D convolution operations in MRI image generation.
...
Conclusion: TenVOO is effective for fine-tuning DDPMs in MRI, offering high performance and parameter efficiency.
...
### [147] [U-Net Based Healthy 3D Brain Tissue Inpainting](https://arxiv.org/abs/2507.18126)
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of reconstructing missing or corrupted regions in brain MRI scans, ensuring accurate synthesis of healthy tissue.
...
Conclusion: The proposed method is effective, reliable, and generalizable for brain tissue synthesis, as demonstrated by its top performance.
...
### [148] [Deep Learning for Glioblastoma Morpho-pathological Features Identification: A BraTS-Pathology Challenge Solution](https://arxiv.org/abs/2507.18133)
*Juexin Zhang,Ying Weng,Ke Chen*

Main category: eess.IV

TL;DR: A deep learning model for glioblastoma diagnosis was fine-tuned on the BraTS-Path dataset but showed poor performance on validation, with moderate accuracy and recall but high specificity. It ranked second in testing.
...
Method: A pre-trained model was fine-tuned on the BraTS-Path training dataset and evaluated on the validation set.

Result: The model achieved 0.392 accuracy, recall, and F1-score, with 0.898 specificity and 0.255 MCC, ranking second in testing.

Conclusion: Despite poor validation performance, the model's high specificity and testing rank suggest potential for glioblastoma diagnosis.
...
### [149] [TCM-Tongue: A Standardized Tongue Image Dataset with Pathological Annotations for AI-Assisted TCM Diagnosis](https://arxiv.org/abs/2507.18288)
...
<details>
  <summary>Details</summary>
Motivation: To overcome the lack of standardized, large-scale datasets for AI development in TCM tongue diagnosis, which hinders progress in computational tools for the field.

Method: Creation of a dataset with 6,719 high-quality tongue images annotated with 20 pathological symptom categories, validated by TCM practitioners, and benchmarked using nine deep learning models.
...
Conclusion: This dataset bridges the data shortage in TCM tongue diagnosis, enabling AI integration into research and clinical practice with standardized, high-quality data.
...
### [150] [UniSegDiff: Boosting Unified Lesion Segmentation via a Staged Diffusion Model](https://arxiv.org/abs/2507.18362)
*Yilong Hu,Shijie Chang,Lihe Zhang,Feng Tian,Weibing Sun,Huchuan Lu*

Main category: eess.IV

TL;DR: UniSegDiff is a novel diffusion model framework for unified lesion segmentation, addressing uneven attention in DPMs with staged training and outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Current DPM training and inference strategies lead to uneven attention across timesteps, causing longer training and suboptimal results for lesion segmentation.

Method: UniSegDiff introduces staged training and inference, dynamically adjusting prediction targets and pre-training a feature extraction network for unified segmentation.

Result: UniSegDiff outperforms SOTA methods in lesion segmentation across six organs and various imaging modalities.

Conclusion: UniSegDiff effectively addresses DPM limitations, offering a unified and efficient solution for lesion segmentation.
...
### [151] [DiagR1: A Vision-Language Model Trained via Reinforcement Learning for Digestive Pathology Diagnosis](https://arxiv.org/abs/2507.18433)
...
TL;DR: The paper addresses challenges in multimodal models for gastrointestinal pathology by improving data quality and reasoning transparency, achieving superior performance in diagnostic text generation.


<details>
  <summary>Details</summary>
Motivation: Current multimodal models for gastrointestinal pathology suffer from noisy data, incomplete annotations, and lack of reasoning transparency, leading to unreliable outputs in clinical practice.

Method: The authors construct a high-quality dataset and propose a prompt argumentation strategy with lesion classification and anatomical site information. They also use supervised fine-tuning and GRPO for improved reasoning.
...
### [152] [Bob's Confetti: Phonetic Memorization Attacks in Music and Video Generation](https://arxiv.org/abs/2507.17937)
*Jaechul Roh,Zachary Novack,Yuefeng Peng,Niloofar Mireshghallah,Taylor Berg-Kirkpatrick,Amir Houmansadr*

Main category: cs.SD

TL;DR: The paper explores vulnerabilities in Lyrics-to-Song (LS2) models, revealing memorization issues through adversarial phonetic prompting (APT) and uncovering sub-lexical memorization and phonetic-to-visual regurgitation in multimodal models.


<details>
  <summary>Details</summary>
Motivation: To investigate underexplored vulnerabilities in LS2 models and multimodal systems, focusing on memorization risks triggered by phonetic alterations.

Method: Introduces Adversarial PhoneTic Prompting (APT) to modify lyrics while preserving acoustic structure, testing models like SUNO and YuE, and examining text-to-video models like Veo 3.

Result: Models regenerate memorized content despite phonetic distortions, with high similarity in audio metrics and unexpected visual memorization in text-to-video models.
...
### [153] [Exploring Communication Strategies for Collaborative LLM Agents in Mathematical Problem-Solving](https://arxiv.org/abs/2507.17753)
...
Method: Examined four communication modes (teacher-student, peer-to-peer, reciprocal teaching, critical debate) in a dual-agent, chat-based math problem-solving setup using GPT-4o, tested on the MATH dataset.

Result: Dual-agent setups outperformed single agents; peer-to-peer collaboration achieved the highest accuracy. Key dialogue acts (statements, acknowledgment, hints) were identified as critical.

Conclusion: Effective communication strategies are essential for LLM agents in AI education, with peer-to-peer collaboration being the most efficient for complex problem-solving.
...
### [154] [PosterMate: Audience-driven Collaborative Persona Agents for Poster Design](https://arxiv.org/abs/2507.18572)
*Donghoon Shin,Daniel Lee,Gary Hsieh,Gromit Yeuk-Yin Chan*

Main category: cs.HC

TL;DR: PosterMate, a poster design assistant, uses AI-generated persona agents to simulate diverse audience feedback, facilitating collaborative design edits.


<details>
  <summary>Details</summary>
Motivation: Challenges in gathering diverse audience feedback and reconciling design edits motivate the use of AI to simulate human-like interactions.
...
Conclusion: PosterMate demonstrates potential as a prototyping tool for collaborative poster design.
...
### [155] [Zero-Shot Dynamic Concept Personalization with Grid-Based LoRA](https://arxiv.org/abs/2507.17963)
*Rameen Abdal,Or Patashnik,Ekaterina Deyneka,Hao Chen,Aliaksandr Siarohin,Sergey Tulyakov,Daniel Cohen-Or,Kfir Aberman*

Main category: cs.GR

TL;DR: A zero-shot framework for dynamic concept personalization in text-to-video models avoids per-instance fine-tuning, using Grid-LoRA adapters and a Grid Fill module for scalable, high-quality synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing methods for dynamic concept personalization require per-instance fine-tuning, limiting scalability. This work aims to enable high-quality synthesis without such constraints.

Method: Leverages structured 2x2 video grids and lightweight Grid-LoRA adapters for editing and composition. A Grid Fill module ensures temporal coherence and identity preservation.

Result: Produces temporally coherent and identity-preserving outputs without test-time optimization, generalizing to unseen dynamic concepts.

Conclusion: The framework achieves scalable, high-quality synthesis across diverse subjects and editing scenarios, demonstrating broad applicability.
...
### [156] [GeoAvatar: Adaptive Geometrical Gaussian Splatting for 3D Head Avatar](https://arxiv.org/abs/2507.18155)
*SeungJun Moon,Hah Min Lew,Seungeun Lee,Ji-Su Kang,Gyeong-Moon Park*

Main category: cs.GR

TL;DR: GeoAvatar improves 3D head avatar generation by balancing identity preservation and animation using adaptive Gaussian Splatting and a novel mouth deformation strategy.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with balancing identity preservation and novel poses/expressions, especially in facial regions.

Method: Proposes GeoAvatar with Adaptive Pre-allocation Stage (APS) for Gaussian segmentation, a mouth structure for animation fidelity, and a regularization loss for rigging.

Result: Outperforms state-of-the-art methods in reconstruction and animation, validated by the DynamicFace dataset.

Conclusion: GeoAvatar effectively addresses the challenge of balancing identity and animation in 3D head avatars.
...
### [157] [PS-GS: Gaussian Splatting for Multi-View Photometric Stereo](https://arxiv.org/abs/2507.18231)
*Yixiao Chen,Bin Liang,Hanzhi Guo,Yongqing Cheng,Jiayi Zhao,Dongdong Weng*

Main category: cs.GR

TL;DR: PS-GS integrates Gaussian Splatting with MVPS for efficient inverse rendering, improving 3D reconstruction accuracy and enabling novel-view synthesis and relighting.


<details>
  <summary>Details</summary>
Motivation: Existing inverse rendering methods with fixed illumination lack accuracy; MVPS offers better results but faces efficiency challenges.

Method: PS-GS initializes with 2D Gaussian splatting, then uses deferred inverse rendering with a lighting MLP and normal regularization. It refines lighting via 2D Gaussian ray-tracing.

Result: Outperforms prior works in accuracy and efficiency on synthetic and real datasets.

Conclusion: PS-GS effectively addresses the ill-posed nature of inverse rendering, enabling high-quality 3D reconstructions for various applications.
...
### [158] [Diffusion-Assisted Frequency Attention Model for Whole-body Low-field MRI Reconstruction](https://arxiv.org/abs/2507.17764)
...
Method: Integrates diffusion models with frequency-domain attention for better representation and reconstruction.

Result: DFAM consistently outperforms conventional and learning-based reconstruction algorithms.

Conclusion: DFAM shows promise for advancing low-field MRI reconstruction in clinical settings with limited resources.
...
### [159] [Agentic AI framework for End-to-End Medical Data Inference](https://arxiv.org/abs/2507.18115)
...
Result: The framework successfully automates the ML lifecycle, reducing expert intervention and offering scalable, cost-efficient AI deployment in clinical settings.

Conclusion: The Agentic AI framework provides a scalable and efficient solution for automating ML workflows in healthcare, addressing key challenges like privacy and manual effort.
...
### [160] [SafeWork-R1: Coevolving Safety and Intelligence under the AI-45$^{\circ}$ Law](https://arxiv.org/abs/2507.18576)
...
TL;DR: SafeWork-R1 is a multimodal reasoning model with coevolved safety and capabilities, developed using the SafeLadder framework. It outperforms base models and proprietary models like GPT-4.1 in safety benchmarks while maintaining general performance.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional alignment methods like RLHF by enabling intrinsic safety reasoning and self-reflection in AI models.
...
### [161] [On the Performance of Concept Probing: The Influence of the Data (Extended Version)](https://arxiv.org/abs/2507.18550)
...
<details>
  <summary>Details</summary>
Motivation: To understand how the data used for training probing models affects their performance, as prior work has overlooked this aspect.

Method: Investigates concept probing in image classification tasks, focusing on the training data's role.

Result: Provides insights into the effect of training data on probing model performance and releases concept labels for two datasets.
...
### [162] [Caching Techniques for Reducing the Communication Cost of Federated Learning in IoT Environments](https://arxiv.org/abs/2507.17772)
*Ahmad Alhonainy,Praveen Rao*

Main category: cs.DC

TL;DR: The paper proposes caching strategies (FIFO, LRU, Priority-Based) to reduce communication costs in Federated Learning (FL) while preserving model accuracy, validated on CIFAR-10 and medical datasets.


<details>
  <summary>Details</summary>
Motivation: Communication cost is a bottleneck in FL, especially in resource-constrained environments, prompting the need for efficient update transmission methods.
...
Result: Experiments show reduced communication with minimal accuracy loss, confirming improved scalability and memory efficiency.

Conclusion: Intelligent caching makes FL practical for edge IoT networks, benefiting smart cities, healthcare, and latency-sensitive applications.
...
### [163] [RECALLED: An Unbounded Resource Consumption Attack on Large Vision-Language Models](https://arxiv.org/abs/2507.18053)
*Haoran Gao,Yuanhe Zhang,Zhenhong Zhou,Lei Jiang,Fanyu Meng,Yujia Xiao,Kun Wang,Yang Liu,Junlan Feng*

Main category: cs.CR

TL;DR: RECALLED is the first method to exploit visual inputs for resource consumption attacks (RCAs) in large vision-language models (LVLMs), increasing latency and resource usage significantly.
...
TL;DR: LoRA-Leak reveals that LoRA-based fine-tuned LMs are vulnerable to MIAs, leveraging pre-trained models for improved attacks, and suggests defenses like dropout.


<details>
  <summary>Details</summary>
Motivation: Existing MIAs overlook the vulnerability of LoRA fine-tuning due to the pre-trained model's role in information leakage.

Method: Introduces LoRA-Leak, a framework with 15 MIAs (10 existing, 5 improved) to evaluate fine-tuning dataset privacy risks.

Result: LoRA-based LMs are vulnerable to MIAs (0.775 AUC), with dropout and layer exclusion as effective defenses.

Conclusion: Pre-trained models exacerbate MIA risks for LoRA-based LMs, urging better data privacy measures.
...
### [165] [NWaaS: Nonintrusive Watermarking as a Service for X-to-Image DNN](https://arxiv.org/abs/2507.18036)
*Haonan An,Guang Hua,Yu Guo,Hangcheng Cao,Susanto Rahardja,Yuguang Fang*

Main category: cs.CR

TL;DR: The paper introduces Nonintrusive Watermarking as a Service (NWaaS) and its implementation, ShadowMark, to protect DNN models without modifying them, addressing concerns about intrusiveness and fidelity-robustness trade-offs.


<details>
  <summary>Details</summary>
Motivation: Existing DNN watermarking methods are intrusive, altering model parameters or behavior, which raises concerns about performance shifts and costs. This limits practical adoption and WaaS development.

Method: Proposes NWaaS, a trustless paradigm for X-to-Image models, and ShadowMark, which uses a key encoder and watermark decoder to create a nonintrusive side channel in black-box APIs.

Result: ShadowMark achieves absolute fidelity, works across DNN architectures, and resists attacks, eliminating the fidelity-robustness trade-off. Experiments confirm its efficacy for real-world deployment.

Conclusion: NWaaS and ShadowMark offer a practical, nonintrusive solution for DNN watermarking, enabling broader adoption and addressing limitations of existing methods.
...
### [166] [Recent Trends in Distant Conversational Speech Recognition: A Review of CHiME-7 and 8 DASR Challenges](https://arxiv.org/abs/2507.18161)
...
TL;DR: The CHiME-7 and 8 challenges advanced multi-channel ASR and diarization, revealing trends like the shift to end-to-end systems, reliance on guided source separation, and the importance of diarization refinement.


<details>
  <summary>Details</summary>
Motivation: To advance research in multi-channel, generalizable ASR and diarization of conversational speech through community-driven challenges.
...
Conclusion: Despite progress, transcribing spontaneous speech in challenging environments remains difficult, highlighting areas for future research.
...
### [167] [Evaluation of facial landmark localization performance in a surgical setting](https://arxiv.org/abs/2507.18248)
...
Conclusion: The study suggests potential for integrating the MediaPipe algorithm into medical procedures, despite minor precision issues.
...
### [168] [ReSem3D: Refinable 3D Spatial Constraints via Fine-Grained Semantic Grounding for Generalizable Robotic Manipulation](https://arxiv.org/abs/2507.18262)
...
<details>
  <summary>Details</summary>
Motivation: Existing methods lack fine-grained semantic modeling, real-time planning, and robustness in diverse environments, motivating the need for ReSem3D.

Method: ReSem3D uses hierarchical recursive reasoning in MLLMs and VFMs to construct 3D spatial constraints in two stages: part-level extraction and region-level refinement. These constraints are encoded as real-time optimization objectives.

Result: ReSem3D successfully performs diverse manipulation tasks in zero-shot conditions, showing adaptability and generalization in household and lab environments.

Conclusion: ReSem3D effectively unifies semantic understanding and execution in robotic manipulation, demonstrating robustness and real-time performance.
...
### [169] [Adaptive Articulated Object Manipulation On The Fly with Foundation Model Reasoning and Part Grounding](https://arxiv.org/abs/2507.18276)
...
Method: Proposes AdaRPG, which extracts object parts for better affordance generalization, uses a part-level affordance dataset, and employs foundation models for reasoning and control.
...
### [170] [Multimodal Recurrent Ensembles for Predicting Brain Responses to Naturalistic Movies (Algonauts 2025)](https://arxiv.org/abs/2507.17897)
*Semih Eren,Deniz Kucukahmetler,Nico Scherf*

Main category: q-bio.NC

TL;DR: A hierarchical multimodal recurrent ensemble model integrates video, audio, and language embeddings to predict fMRI responses to movies, ranking third in the Algonauts 2025 challenge.
...
Method: Uses modality-specific bidirectional RNNs for temporal encoding, fuses hidden states, and employs subject-specific heads for output. Training involves a composite MSE-correlation loss and a curriculum strategy.

Result: Achieved an overall Pearson r = 0.2094, with the highest single-parcel peak score (mean r = 0.63), particularly excelling for challenging subjects.
...
### [171] [GenSelect: A Generative Approach to Best-of-N](https://arxiv.org/abs/2507.17797)
...
<details>
  <summary>Details</summary>
Motivation: Current methods (pointwise or pairwise scoring) underutilize LLMs' comparative abilities or scale poorly with larger sampling budgets.

Method: GenSelect uses long reasoning by the LLM to select the best solution among N candidates, scaling efficiently with parallel sampling.

Result: GenSelect outperforms existing scoring approaches, demonstrating effectiveness with reasoning models like QwQ and DeepSeek-R1-0528.

Conclusion: GenSelect efficiently harnesses LLMs' comparative strengths for scalable and superior performance in reasoning tasks.
...
### [172] [Group Sequence Policy Optimization](https://arxiv.org/abs/2507.18071)
...
TL;DR: GSPO is a reinforcement learning algorithm for training large language models, outperforming GRPO by using sequence-level optimization and improving stability and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address limitations of token-level importance ratios in previous algorithms and enhance training efficiency and performance.

Method: Defines importance ratio based on sequence likelihood, performs sequence-level clipping, rewarding, and optimization.

Result: Superior training efficiency and performance compared to GRPO, stabilizes MoE RL training, and simplifies RL infrastructure design.

Conclusion: GSPO significantly improves large language model training, as evidenced by its impact on the Qwen3 models.
...
### [173] [Enhancing Quantization-Aware Training on Edge Devices via Relative Entropy Coreset Selection and Cascaded Layer Correction](https://arxiv.org/abs/2507.17768)
*Yujia Tong,Jingling Yuan,Chuang Hu*

Main category: cs.LG

TL;DR: QuaRC is a Quantization-Aware Training (QAT) framework for edge devices that uses coreset selection and cascaded layer correction to reduce quantization errors and improve model performance with small datasets.


<details>
  <summary>Details</summary>
Motivation: The need for efficient deployment of low-bit quantized models on edge devices, coupled with privacy concerns limiting data access, drives the development of a QAT method that works with small, representative datasets.

Method: QuaRC introduces a "Relative Entropy Score" for coreset selection to identify subsets that minimize quantization errors and employs a Cascaded Layer Correction strategy to align intermediate layer outputs between quantized and full-precision models.

Result: QuaRC improves Top-1 accuracy by 5.72% on ResNet-18 (2-bit) using only 1% of the ImageNet-1K dataset, outperforming existing methods.

Conclusion: QuaRC effectively addresses the challenges of QAT on edge devices by reducing quantization errors and computational costs while maintaining performance with small datasets.
...
### [174] [VIBE: Video-Input Brain Encoder for fMRI Response Modeling](https://arxiv.org/abs/2507.17958)
...
Method: Uses a modality-fusion Transformer to merge features from open-source models (Qwen2.5, BEATs, Whisper, SlowFast, V-JEPA) and a prediction Transformer with rotary embeddings for temporal decoding. Trained on 65 hours of movie data from CNeuroMod and ensembled across 20 seeds.

Result: Achieved mean parcel-wise Pearson correlations of 32.25 (in-distribution) and 21.25 (out-of-distribution). Earlier iteration won Phase-1 and placed second overall in Algonauts 2025 Challenge.

Conclusion: VIBE demonstrates strong performance in fMRI activity prediction, showcasing the effectiveness of multi-modal fusion and Transformer-based architectures.
...
### [175] [ChronoSelect: Robust Learning with Noisy Labels via Dynamics Temporal Memory](https://arxiv.org/abs/2507.18183)
*Jianchao Wang,Qingfeng Li,Pengcheng Zheng,Xiaorong Pu,Yazhou Ren*

Main category: cs.LG

TL;DR: ChronoSelect is a novel framework for learning with noisy labels, using a four-stage memory architecture and sliding update mechanism to improve sample partitioning and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for learning with noisy labels lack temporal dynamics, leading to degraded performance. ChronoSelect addresses this by leveraging learning evolution.

Method: ChronoSelect uses a four-stage memory architecture with sliding updates and controlled decay to analyze temporal trajectories and partition samples into clean, boundary, and noisy subsets.

Result: The framework achieves state-of-the-art performance on synthetic and real-world benchmarks, with proven convergence and stability.

Conclusion: ChronoSelect effectively addresses noisy label challenges by incorporating temporal dynamics, outperforming existing methods.
...
### [176] [LLM-based Embedders for Prior Case Retrieval](https://arxiv.org/abs/2507.18455)
...
### [177] [DR.EHR: Dense Retrieval for Electronic Health Record with Knowledge Injection and Synthetic Data](https://arxiv.org/abs/2507.18583)
...
<details>
  <summary>Details</summary>
Motivation: EHR retrieval is hindered by semantic gaps and lack of medical knowledge in existing models.
...
Conclusion: DR.EHR significantly advances EHR retrieval, offering a robust solution for clinical applications.
