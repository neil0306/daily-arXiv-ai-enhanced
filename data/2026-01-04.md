...
TL;DR: This paper presents an automated pipeline combining OCR, LLM-based interpretation, and database linking to digitize and harmonize historical professor records from Leiden University books with existing database records.


<details>
  <summary>Details</summary>
Motivation: To develop an automated method for digitizing and analyzing historical biographic data about Leiden University professors (1575-1815) from typewritten books, addressing challenges in integrating this data with existing high-quality database records.

Method: Combines OCR techniques, generative AI with decoding constraints for structured data extraction, and database linkage methods to process historical document images into harmonized digital format.

Result: OCR achieved CER 1.08% and WER 5.06%. JSON extraction accuracy was 63% from OCR text and 65% from annotated OCR. Record linkage achieved 94% accuracy with annotated JSON and 81% with OCR-derived JSON, showing generative AI can partially correct OCR errors.

Conclusion: The study contributes to digital humanities by offering an automated pipeline for historical document interpretation, addressing layout variability and terminology differences, and demonstrating the applicability of advanced generative AI models for such tasks.
...
TL;DR: CAT framework evaluates LLM accuracy-consistency interplay via CAR curves and CORE index, moving beyond separate metrics to assess their trade-off.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation focuses on accuracy or consistency separately, but their interdependence is crucial for real-world deployment in high-stakes applications.

Method: Proposes CAT framework with Consistency-Accuracy Relation (CAR) curves visualizing accuracy under varying consistency requirements, and CORE index quantifying the trade-off.

Result: Demonstrated across diverse generalist and domain-specific LLMs on multiple-choice benchmarks, showing framework's practical applicability.

Conclusion: CAT provides nuanced evaluation of LLM accuracy-consistency interplay, with extensibility beyond multiple-choice to open-ended tasks via adaptable scoring functions.
...
TL;DR: A framework for evaluating and improving consistency in LLM-generated structured outputs using STED metric and consistency scoring.
...
Method: Combines (1) STED (Semantic Tree Edit Distance) - a novel similarity metric balancing semantic flexibility with structural strictness for JSON outputs, and (2) a consistency scoring framework aggregating multiple STED measurements across repeated generations to quantify reliability.
...
TL;DR: BanglaCodeAct: An agent-based framework using multi-agent prompting and iterative self-correction for Bangla-to-Python code generation, achieving state-of-the-art performance with Qwen3-8B model.


<details>
  <summary>Details</summary>
Motivation: While LLMs excel at English-to-code generation, this progress hasn't extended to low-resource languages like Bangla. There's a need for effective Bangla-to-Python code generation without task-specific fine-tuning.
...
Result: Qwen3-8B with BanglaCodeAct achieves best performance: 94.0% pass@1 accuracy on development set and 71.6% on blind test set of mHumanEval dataset for Bangla NL2Code, establishing new benchmark for Bangla-to-Python translation.

Conclusion: Agent-based reasoning shows strong potential for reliable code generation in low-resource languages. The framework establishes new state-of-the-art for Bangla-to-Python translation without task-specific fine-tuning.
...
<details>
  <summary>Details</summary>
Motivation: To create a controlled benchmark for safety-critical document understanding in pharmaceuticals that stresses pre-trained models with real-world challenges like noisy OCR and heterogeneous templates.

Method: Created PharmaShip dataset with scanned pharmaceutical shipping documents, established three complementary tasks (SER, RE, ROP), used entity-centric evaluation protocol, benchmarked five representative baselines (LiLT, LayoutLMv3-base, GeoLayoutLM and RORE variants), standardized preprocessing/splits/optimization.

Result: Pixels and explicit geometry provide complementary inductive biases but neither alone is sufficient; reading-order-oriented regularization consistently improves SER and EL; longer positional coverage stabilizes late-page predictions; ROP is accurate at word level but challenging at segment level due to boundary ambiguity.

Conclusion: PharmaShip establishes a reproducible benchmark for pharmaceutical document understanding and highlights sequence-aware constraints as transferable bias for structure modeling.
...
TL;DR: LN-RP is a computational framework that uses stochastic noise injection to study how personas emerge in LLMs through 152 generation cycles, revealing three stable persona modes with distinct entropy patterns.


<details>
  <summary>Details</summary>
Motivation: To develop a reproducible method for studying how noise influences persona emergence, reflexive generation dynamics, and long-range linguistic coherence in large language models.

Method: The Luca-Noise Reflex Protocol injects stochastic noise seeds into initial generation states and analyzes nonlinear transitions across 152 generation cycles to observe persona emergence patterns.

Result: The study reveals three stable persona modes with distinct entropy signatures, shows that external noise can reliably induce phase transitions in reflexive generation, and confirms consistent persona retention with statistically significant differences across modes (p < 0.01).

Conclusion: LN-RP provides a reproducible framework for studying reflexive generation, emergent behavior, and long-range linguistic coherence in LLMs through controlled noise injection and persona analysis.
...
TL;DR: HarmTransform is a multi-agent debate framework that systematically transforms harmful queries into stealthier forms to improve LLM safety training data, outperforming baselines but revealing debate's dual nature as both enhancing and complicating transformations.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety mechanisms focus on overtly dangerous content but overlook subtle threats where users disguise harmful intent through covert rephrasing, creating a significant gap in existing safety training data.
...
Result: Experiments show HarmTransform significantly outperforms standard baselines in producing effective query transformations, though analysis reveals debate acts as a double-edged sword - improving stealth while potentially introducing topic shifts and unnecessary complexity.

Conclusion: The research highlights both the promise and limitations of multi-agent debate for generating comprehensive safety training data, showing it can sharpen transformations but may also introduce complications that need careful management.
...
TL;DR: LLMs can learn internal representations of stochastic environments like poker, including deterministic features (hand ranks) and stochastic features (equity), without explicit instruction.


<details>
  <summary>Details</summary>
Motivation: To investigate whether LLMs can develop emergent world models in domains of incomplete information (POMDPs), extending prior work on perfect information games.

Method: Pretrained a GPT-style model on Poker Hand History data and probed its internal activations using primarily nonlinear probes to decode learned representations.

Result: The model learned both deterministic structure (hand ranks) and stochastic features (equity) without explicit instruction, and these representations correlate with theoretical belief states.

Conclusion: LLMs can learn their own representation of stochastic environments like Texas Hold'em Poker, suggesting they develop internal models of partially observable domains.
...
TL;DR: A two-stage framework combining targeted training with selective reasoning-based inference to detect subtle, context-dependent sexist content that evades traditional methods.


<details>
  <summary>Details</summary>
Motivation: Sexist content online is becoming more subtle and context-dependent, making it hard to detect with traditional methods. Current approaches struggle with overlapping linguistic, psychological, legal, and cultural dimensions that create mixed signals, label scarcity, class imbalance, and conceptual ambiguity.

Method: Two-stage framework: (1) Training with class-balanced focal loss, class-aware batching, and post-hoc threshold calibration to handle label imbalance and noisy supervision. (2) Inference with dynamic routing - high-confidence cases classified directly, uncertain cases escalated to Collaborative Expert Judgment (CEJ) module that prompts multiple personas and consolidates reasoning through a judge model.

Result: State-of-the-art results across benchmarks: +2.72% F1 improvement on EXIST 2025 Task 1.1, +4.48% gain on EDOS Task A, and +1.30% gain on EDOS Task B.

Conclusion: The proposed framework effectively addresses the combined challenges of underrepresentation, noise, and conceptual ambiguity in sexist content detection by unifying targeted training with selective reasoning-based inference, achieving significant performance improvements over existing methods.
...
TL;DR: The paper introduces the Stored Household Item Challenge benchmark for evaluating robots' ability to predict where everyday items are stored in kitchens, and proposes NOAM, a hybrid vision-language model that approaches human-level performance on this task.


<details>
  <summary>Details</summary>
Motivation: Domestic service robots lack commonsense reasoning to locate everyday items stored out of sight in drawers, cabinets, or closets, despite advances in vision and manipulation. The simple command "Bring me a plate" reveals this fundamental cognitive challenge.
...
Result: NOAM significantly improves prediction accuracy compared to baselines including random selection, vision-language pipelines, leading multimodal models, and approaches human-level performance. The benchmark includes two datasets: a real-world evaluation set of 100 item-image pairs and a development set of 6,500 item-image pairs.

Conclusion: The Stored Household Item Challenge provides a realistic benchmark for evaluating service robots' cognitive capabilities, and NOAM demonstrates that integrated vision-language agents can exhibit emergent commonsense reasoning, highlighting best practices for deploying cognitively capable agents in domestic environments.
...
TL;DR: EASD improves speculative decoding by adding entropy-based penalty to reject uncertain draft tokens, enabling performance beyond the target LLM while maintaining efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard speculative decoding is limited by excessive alignment between draft and target models, constraining performance to that of the target LLM. The authors want to overcome this limitation and potentially surpass the target model's performance.

Method: EASD enhances standard speculative decoding with a dynamic entropy-based penalty. It uses entropy of sampling distributions to quantify model uncertainty, rejecting tokens when both models show high entropy with substantial overlap in top-N predictions, then re-sampling with the target LLM.
...
Conclusion: EASD is a training-free enhancement to speculative decoding that enables performance beyond the target LLM through entropy-aware draft verification, maintaining efficiency while improving reasoning performance.
...
TL;DR: MiMo-Audio scales audio pretraining to 100M+ hours, achieving emergent few-shot learning and SOTA performance on audio tasks without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current audio models require task-specific fine-tuning, unlike humans who generalize from few examples. Inspired by GPT-3's scaling success in text, the authors apply similar scaling to audio to achieve generalization.
...
Result: Achieves SOTA among open-source models on speech intelligence and audio understanding benchmarks. Demonstrates generalization to unseen tasks (voice conversion, style transfer, speech editing) and powerful speech continuation capabilities. MiMo-Audio-7B-Instruct reaches SOTA on multiple benchmarks, approaching or surpassing closed-source models.
...
TL;DR: StressRoBERTa uses cross-condition transfer learning from related mental health disorders (depression, anxiety, PTSD) to improve chronic stress detection in tweets, achieving 82% F1-score and outperforming existing systems.


<details>
  <summary>Details</summary>
Motivation: Chronic stress is a major public health issue, and social media platforms like Twitter provide valuable data for detecting stress experiences. There's a need for better automated detection methods that leverage the comorbidity between chronic stress and related mental health conditions.

Method: The paper introduces StressRoBERTa, which uses continual training on clinically related conditions (depression, anxiety, PTSD) from the Stress-SMHD corpus (108M words), followed by fine-tuning on the SMM4H 2022 Task 8 dataset for chronic stress detection in English tweets.

Result: StressRoBERTa achieves 82% F1-score, outperforming the best shared task system by 3 percentage points. It shows that cross-condition transfer from stress-related disorders provides stronger representations than general mental health training (+1% F1 over vanilla RoBERTa). The model also transfers well to situational stress discussions (81% F1 on Dreaddit).

Conclusion: Focused cross-condition transfer learning from clinically related mental health disorders significantly improves chronic stress detection in social media text, demonstrating that leveraging comorbidity relationships between conditions creates more effective representations than general approaches.
...
TL;DR: Comparative interpretability study of transformer-based bias detection models using SHAP explanations reveals architectural differences in how models operationalize linguistic bias, with domain-adapted models showing better alignment between attribution strength and prediction correctness.


<details>
  <summary>Details</summary>
Motivation: Automated bias detection models are widely used in journalism but little is known about how they make decisions or why they fail, creating a need for interpretability studies to understand model behavior and improve reliability.

Method: Comparative interpretability study using SHAP-based explanations on two transformer models: a bias detector fine-tuned on BABE dataset and a domain-adapted RoBERTa model fine-tuned on BABE dataset, analyzing word-level attributions across correct and incorrect predictions.

Result: Both models attend to similar evaluative language categories but differ in signal integration. Bias detector shows misalignment between attribution strength and prediction correctness (stronger evidence for false positives), while domain-adaptive model has better-aligned attributions and produces 63% fewer false positives. Errors arise from discourse-level ambiguity rather than explicit bias cues.

Conclusion: Interpretability-aware evaluation is crucial for bias detection systems, and architectural/training choices significantly impact model reliability and deployment suitability in journalism, with domain adaptation improving alignment between model explanations and prediction outcomes.
...
TL;DR: Adaptive prompting strategy splits retrieved info into chunks for LLM question answering, balancing relevance vs. irrelevance while using fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Longer context windows in LLMs for retrieval-augmented QA introduce more irrelevant information that degrades performance, requiring a solution to balance relevant knowledge incorporation with irrelevant information reduction.

Method: Design an adaptive prompting strategy that splits retrieved information into smaller chunks and sequentially prompts the LLM to answer questions using each chunk, adjusting chunk size to trade off between relevant information inclusion and irrelevant information reduction.

Result: Experimental results on three open-domain QA datasets show the adaptive strategy matches standard prompting performance while using fewer tokens. Analysis reveals LLMs often generate incorrect answers instead of declining when faced with insufficient information.

Conclusion: The adaptive prompting approach effectively addresses the irrelevant information problem in expanded context windows. A key finding is the need for research to enhance LLMs' ability to decline requests when faced with inadequate information, as this is a major error source.
...
<details>
  <summary>Details</summary>
Motivation: To exploit mechanistic interpretability findings that intermediate attention layers encode token-level hypotheses, using these internal representations to create more plausible and model-consistent adversarial examples compared to traditional prompt-based or gradient-based attacks.

Method: Extract tokens from intermediate attention layers of LLaMA-3.1-Instruct-8B and use them as adversarial perturbations. Evaluate on argument quality assessment using ArgQuality dataset, with the same model serving as both generator and evaluator.

Result: Attention-based adversarial examples cause measurable performance drops in evaluation tasks while maintaining semantic similarity to original inputs. However, substitutions from certain layers and token positions introduce grammatical degradation, limiting practical effectiveness.

Conclusion: Intermediate-layer representations show promise as a principled source of adversarial examples for stress-testing LLM evaluation pipelines, but current limitations in grammatical quality need to be addressed for practical applications.
...
TL;DR: The paper presents a multi-retriever RAG system for financial numerical QA that combines domain knowledge retrieval with LLMs, achieving SOTA performance with >7% improvement over baseline.
...
Result: The best neural symbolic model surpassed the FinQA baseline, and the best prompt-based LLM generator achieved SOTA performance with >7% improvement, though still below human expert performance. Domain-specific training with SecBERT was crucial for performance gains.

Conclusion: Domain-specific training significantly improves financial QA performance. For smaller models, there's a trade-off between hallucination loss and external knowledge gains, while larger models benefit more from external facts. Latest LLMs show enhanced few-shot numerical reasoning capabilities.
...
TL;DR: Paper presents analytics framework separating content signals from rater tendencies in open-ended response scoring, using teacher priors and text embeddings to make grading judgments visible and auditable.


<details>
  <summary>Details</summary>
Motivation: Automated scoring of open-ended responses often conflates what students wrote with how teachers grade, making it difficult to separate content quality from rater tendencies. There's a need to make grading judgments visible and auditable for reflection.

Method: Analytics-first framework using ASSISTments math responses: models teacher histories as dynamic priors, derives text representations from sentence embeddings, uses centering and residualization to mitigate prompt/teacher confounds, temporally-validated linear models quantify signal contributions, and projection surfaces model disagreements.

Result: Teacher priors heavily influence grade predictions; best results when priors combined with content embeddings (AUC~0.815), while content-only models remain above chance but weaker (AUC~0.626). Adjusting for rater effects sharpens residual content representation, retaining more informative embedding dimensions.

Conclusion: Framework transforms embeddings from mere features into learning analytics for reflection, enabling teachers/researchers to examine where grading practices align or conflict with evidence of student reasoning and learning.
...
<details>
  <summary>Details</summary>
Motivation: Existing RAG memory systems function as passive storage that accumulates isolated facts, overlooking crucial high-order correlations. This limits representational strength and leads to fragmented reasoning with weak global sense-making capacity in extended contexts.

Method: HGMem represents memory as a hypergraph where hyperedges correspond to distinct memory units, enabling progressive formation of higher-order interactions. This creates a dynamic, expressive structure that connects facts and thoughts around the focal problem into an integrated knowledge structure.
...
Conclusion: Hypergraph-based memory extends beyond simple storage into a dynamic structure that provides stronger propositions for deeper reasoning, enhancing global understanding and complex reasoning capabilities in multi-step RAG systems.
...
TL;DR: LoZA is a sparse attention scheme that transforms full-attention models into sparse versions with limited compute, enabling efficient processing of up to 1 million tokens for long-context tasks.


<details>
  <summary>Details</summary>
Motivation: To enable efficient processing of long-context scenarios (up to 1M tokens) with limited compute budget, addressing both prefill-intensive (retrieval-augmented generation) and decode-intensive (tool-integrated reasoning) cases.

Method: Introduces LongCat ZigZag Attention (LoZA), a sparse attention scheme that can be applied to existing full-attention models to create sparse versions. Applied to LongCat-Flash during mid-training to create LongCat-Flash-Exp.

Result: Achieves significant speed-ups in long-context scenarios, enabling processing of up to 1 million tokens. Creates LongCat-Flash-Exp as a long-context foundation model with efficient long-term reasoning and long-horizon agentic capabilities.

Conclusion: LoZA provides an effective approach to transform full-attention models into sparse versions for efficient long-context processing, enabling practical applications requiring million-token context windows.
...
<details>
  <summary>Details</summary>
Motivation: Large-scale Chinese spelling correction is critical but existing methods lack robustness to novel errors and rely on costly annotations. LLMs and supervised methods struggle with novel error patterns and require expensive labeled data.

Method: CEC-Zero uses a zero-supervision reinforcement learning framework that: 1) synthesizes errorful inputs from clean text, 2) computes cluster-consensus rewards via semantic similarity and candidate agreement, and 3) optimizes the policy with PPO (Proximal Policy Optimization).
...
Conclusion: Establishes a label-free paradigm for robust, scalable Chinese spelling correction, unlocking LLM potential in noisy text pipelines without requiring costly annotations.
...
Result: SAEs discover interpretable reasoning vectors (directions in activation space) encoding behaviors like reflection and backtracking; visualization shows separable regions; interventions can controllably amplify/suppress behaviors; SAEs capture structural properties like response length; enable discovery of novel behaviors beyond human supervision (e.g., confidence-related vectors).

Conclusion: Unsupervised latent discovery via SAEs offers powerful approach for both interpreting and controllably steering reasoning in LLMs, moving beyond limitations of human-supervised methods.
...
TL;DR: WISE framework benchmarks lightweight transformers vs baselines for fake news vs satire detection, finding MiniLM achieves highest accuracy (87.58%) while RoBERTa-base gets best ROC-AUC (95.42%).


<details>
  <summary>Details</summary>
Motivation: Distinguishing fake news from satire/humor is challenging due to overlapping linguistic features but different intent, requiring effective detection systems for real-world misinformation detection.

Method: Developed WISE framework to benchmark 8 lightweight transformer models and 2 baselines on 20,000 samples from Fakeddit dataset. Used stratified 5-fold cross-validation with comprehensive metrics (accuracy, precision, recall, F1, ROC-AUC, PR-AUC, MCC, Brier score, ECE).
...
Conclusion: Lightweight models can match or exceed baseline performance, providing actionable insights for deploying misinformation detection in resource-constrained real-world settings.
...
<details>
  <summary>Details</summary>
Motivation: Explicit textual plans for LLMs suffer from hallucinations and poor generalization across diverse tasks. Inspired by human subconscious implicit cognition, the authors seek to develop a more compact, generalized planning approach.

Method: iCLP distills explicit plans from reasoning trajectories, learns discrete latent representations via vector-quantized autoencoder with codebook, then fine-tunes LLMs on paired latent plans and reasoning steps to enable implicit planning.
...
Conclusion: iCLP successfully bridges explicit and implicit planning, allowing LLMs to leverage compact latent representations for more effective reasoning while preserving the interpretability of chain-of-thought approaches.
...
TL;DR: CRS is a unified reliability framework that combines calibration, robustness, and uncertainty quantification into one metric for evaluating LLMs in critical domains.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used in decision-critical domains (healthcare, law, finance) but have reliability issues: overconfident errors, degradation under input shifts, and lack of clear uncertainty estimates. Existing evaluations are fragmented and only address isolated aspects.

Method: Introduce Composite Reliability Score (CRS) - a unified framework integrating calibration, robustness, and uncertainty quantification into a single interpretable metric. Test on ten leading open-source LLMs across five QA datasets under baselines, perturbations, and calibration methods.

Result: CRS delivers stable model rankings, uncovers hidden failure modes missed by single metrics, and shows that the most dependable systems balance accuracy, robustness, and calibrated uncertainty.

Conclusion: CRS provides a comprehensive reliability assessment framework that reveals which LLMs are truly dependable for critical applications by evaluating the interplay between accuracy, robustness, and uncertainty calibration.
...
TL;DR: HY-MT1.5 introduces 1.8B and 7B parameter translation models that outperform larger open-source models and commercial APIs, achieving 90-95% of ultra-large proprietary model performance with advanced translation features.


<details>
  <summary>Details</summary>
Motivation: To develop high-performance machine translation models that are parameter-efficient and can compete with much larger proprietary models while offering advanced translation capabilities.
...
Result: HY-MT1.5-1.8B outperforms significantly larger open-source models (Tower-Plus-72B, Qwen3-32B) and commercial APIs, achieving ~90% of Gemini-3.0-Pro performance. HY-MT1.5-7B achieves 95% of Gemini-3.0-Pro performance on Flores-200 and surpasses it on WMT25 and Mandarin-minority language benchmarks.

Conclusion: The HY-MT1.5 series provides highly competitive, robust translation solutions with parameter efficiency, advanced constraint support, and state-of-the-art performance for their size classes.
...
TL;DR: A demo paper providing centralized guidance for researchers to train Hugging Face models on AWS SageMaker, addressing the steep learning curve and fragmented documentation of cloud platforms.


<details>
  <summary>Details</summary>
Motivation: LLM development is dominated by resource-rich groups, forcing many researchers to use cloud services like AWS SageMaker. However, the steep learning curve and fragmented documentation create barriers to adoption, leaving knowledge gaps that require searching across multiple sources.

Method: The paper creates a centralized, comprehensive guide/demo that provides all essential information needed for researchers to train their first Hugging Face model on AWS SageMaker from scratch, aiming to fill documentation gaps.

Result: A practical resource that democratizes cloud adoption by providing a complete, accessible pathway for researchers to overcome the AWS SageMaker learning curve and successfully train models in the cloud.

Conclusion: By centralizing essential information and addressing documentation gaps, this demo paper lowers the barrier to cloud adoption, enabling more researchers to leverage AWS SageMaker for training Hugging Face models despite limited on-premise resources.
...
TL;DR: A framework for inference-time control of masked diffusion language models using activation steering from contrastive examples, enabling efficient attribute modulation without simulating denoising trajectories.


<details>
  <summary>Details</summary>
Motivation: Masked diffusion language models (MDLMs) have shown competitive performance with autoregressive LLMs but lack effective inference-time control mechanisms for steering and attribute modulation.
...
Conclusion: The framework provides an efficient inference-time control mechanism for MDLMs that enables effective steering of text generation attributes using contrastive examples.
...
TL;DR: The paper proposes LEWM, a Large Emotional World Model that incorporates emotional states alongside visual observations and actions to better predict emotion-driven social behaviors, addressing the gap in existing world models that focus primarily on physical-world regularities.


<details>
  <summary>Details</summary>
Motivation: Existing Large Language Models (LLMs) and world models primarily focus on modeling physical-world regularities but lack systematic exploration of emotional factors, which are crucial for understanding human decision-making and social behaviors. The authors demonstrate that removing emotionally relevant information degrades reasoning performance, highlighting the importance of emotion in world understanding.

Method: 1) Constructed the Emotion-Why-How (EWH) dataset that integrates emotion into causal relationships, enabling reasoning about why actions occur and how emotions drive future world states. 2) Proposed LEWM (Large Emotional World Model) that explicitly models emotional states alongside visual observations and actions, allowing prediction of both future states and emotional transitions.

Result: LEWM more accurately predicts emotion-driven social behaviors while maintaining comparable performance to general world models on basic tasks. The model demonstrates improved capability in understanding emotion-influenced decision-making and social interactions.

Conclusion: Incorporating emotional states into world models is essential for better understanding and predicting human social behaviors. The proposed LEWM framework successfully integrates emotional factors with physical-world modeling, advancing the development of more comprehensive world models that account for both physical and emotional dimensions of human experience.
...
TL;DR: TeleChat3-MoE is a new series of large language models with Mixture-of-Experts architecture (105B-1T+ parameters) trained on Ascend NPU clusters, with this report focusing on the training infrastructure enabling reliable and efficient scaling to frontier model sizes.


<details>
  <summary>Details</summary>
Motivation: To enable reliable and efficient training of frontier-scale language models (up to over 1 trillion parameters) with Mixture-of-Experts architecture on Ascend NPU hardware clusters, addressing challenges in numerical accuracy, performance optimization, and multi-dimensional parallelism at scale.

Method: Developed systematic methodologies for operator-level and end-to-end numerical accuracy verification; implemented performance optimizations including interleaved pipeline scheduling, attention-aware data scheduling for long sequences, hierarchical/overlapped communication for expert parallelism, and DVM-based operator fusion; created systematic parallelization framework using analytical estimation and integer linear programming; and applied cluster-level optimizations for host- and device-bound bottlenecks.

Result: The infrastructure advancements yield significant throughput improvements and achieve near-linear scaling on clusters comprising thousands of devices, providing a robust foundation for large-scale language model development on the hardware ecosystem.

Conclusion: The technical report presents a comprehensive training infrastructure that successfully enables reliable and efficient scaling of Mixture-of-Experts language models to frontier sizes (over 1 trillion parameters) on Ascend NPU clusters, with systematic approaches to numerical accuracy, performance optimization, and multi-dimensional parallelism that achieve near-linear scaling at large scale.
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with clinical diagnosis due to three key limitations: generating hallucinated medical content, asking redundant/in efficient questions, and losing coherence in multi-turn dialogues, which prevents them from emulating real clinical diagnostic reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Limited research on Arabic AES due to lack of publicly available datasets, creating a critical gap in Arabic language educational technology and assessment tools.
...
Result: LAILA fills a critical need as the largest publicly available Arabic AES dataset, enabling development of robust scoring systems and providing benchmark performance metrics for Arabic language models.

Conclusion: LAILA addresses the significant gap in Arabic AES research by providing a comprehensive, publicly available dataset that supports the development and benchmarking of Arabic essay scoring systems.
...
TL;DR: Pat-SPECTER model outperforms other language similarity models in predicting credible patent-paper citations, with applications in separating and predicting patent-paper pairs, plus analysis of US patent citation patterns.


<details>
  <summary>Details</summary>
Motivation: Need for a language similarity model that can effectively work with both patents and scientific publications simultaneously, to better understand and predict connections between these two types of documents.

Method: Developed Pat-SPECTER by fine-tuning the SPECTER2 model on patents, then conducted horse race-style evaluation comparing eight language similarity models on their ability to predict credible Patent-Paper Citations.

Result: Pat-SPECTER performed best among all tested models. The model successfully separated and predicted patent-paper pairs in real-world scenarios. Analysis revealed US patents cite papers that are semantically less similar than in other jurisdictions, possibly due to duty of candor requirements.

Conclusion: Pat-SPECTER is an effective language similarity model for patent-paper analysis, with practical applications and insights into citation patterns. The model is made openly available for academic and practitioner use.
...
TL;DR: DATAMASK is an efficient joint learning framework for large-scale pre-training data selection that simultaneously optimizes quality and diversity metrics through policy gradient-based mask learning, achieving significant performance improvements while reducing selection time by 98.9%.


<details>
  <summary>Details</summary>
Motivation: Current data selection methods for trillion-scale pre-training datasets either use quality metrics (which show diminishing returns) or diversity metrics (which remove valuable high-quality samples), limiting LLM capabilities. There's a need for efficient joint optimization of both metrics at scale.

Method: DATAMASK treats data selection as a mask learning problem using policy gradient optimization. It involves iterative sampling of data masks, computing policy gradients based on predefined quality and diversity objectives, and updating mask sampling logits with acceleration enhancements for trillion-scale datasets.

Result: DATAMASK reduces selection time by 98.9% compared to greedy algorithms. When applied to FineWeb (15 trillion tokens), it selected a 10% subset called FineWeb-Mask, achieving 3.2% improvement on a 1.5B dense model and 1.9% on a 7B MoE model across 12 diverse tasks.

Conclusion: DATAMASK enables efficient joint optimization of quality and diversity metrics for large-scale pre-training data selection, overcoming limitations of single-metric approaches and significantly improving model performance while dramatically reducing computational costs.
...
<details>
  <summary>Details</summary>
Motivation: The manual, resource-intensive EU Taxonomy compliance process needs automation, but research is hindered by lack of public benchmark datasets.

Method: Created a structured dataset from 190 corporate reports with ground-truth economic activities and KPIs, then conducted systematic evaluation of LLMs on compliance workflow tasks.

Result: LLMs show moderate success in qualitative activity identification (improved with multi-step framework) but comprehensively fail at quantitative KPI prediction; concise metadata outperforms full reports; confidence scores are poorly calibrated.

Conclusion: LLMs are not ready for full automation but can serve as powerful assistive tools for human experts; dataset provides public benchmark for future research.
...
<details>
  <summary>Details</summary>
Motivation: Complex reasoning problems often involve implicit spatial, geometric, and structural relationships that are not explicitly encoded in text. Purely text-based reasoning struggles to represent global structural constraints in complex settings.
...
<details>
  <summary>Details</summary>
Motivation: Existing financial LLMs like BloombergGPT and Baichuan-Finance focus mainly on knowledge enhancement, but financial services now require models with robust reasoning and agentic capabilities to handle complex real-world scenarios.

Method: Multi-stage training paradigm: 1) Continual Pre-training on financial corpora for knowledge base; 2) Fine-grained Post-training pipeline: Financial SFT → Finance Reasoning RL → Finance Agentic RL → General RL aligned with business scenarios.
...
Conclusion: The fine-grained, progressive post-training methodology is effective for developing financial LLMs with enhanced reasoning and agentic capabilities, and is poised to become a mainstream paradigm for industrial-enhanced LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Existing sarcasm detection approaches rely on black-box predictions without structural explanations of cognitive factors. Sarcasm often emerges from mismatches between semantic evaluation and normative expectations/intentions, but frameworks explicitly modeling these components are limited.

Method: Proposes World Model inspired SArcasm Reasoning (WM-SAR) with specialized LLM-based agents for literal meaning, context, normative expectation, and intention. Computes deterministic inconsistency score between literal evaluation and normative expectation, plus intention score, integrated via lightweight Logistic Regression.
...
Conclusion: The approach leverages LLM reasoning while maintaining interpretable numerical decision structure, achieving both strong performance and high interpretability in sarcasm detection.
...
TL;DR: A self-supervised contrastive learning framework that improves long document representation by mimicking human skimming behavior, using NLI-based contrastive objectives with random section masking.


<details>
  <summary>Details</summary>
Motivation: Existing transformer models struggle with long document representation in specialized fields like law and medicine. Sparse attention is resource-intensive, hierarchical transformers lack clear inter-section relationships, while humans efficiently skim and synthesize information from key sections.
...
Conclusion: The human-inspired skimming approach with NLI-based contrastive learning provides an effective and efficient solution for long document representation, particularly in specialized domains where understanding inter-section relationships is crucial.
...
TL;DR: This paper compares various approaches to automatic text summarization for less-resourced languages, finding that fine-tuned mT5 models generally outperform zero-shot LLMs and translation pipelines, with LLM-as-judge evaluation being less reliable for these languages.


<details>
  <summary>Details</summary>
Motivation: Automatic text summarization has achieved high performance in high-resourced languages like English, but comparatively less attention has been given to summarization in less-resourced languages. There's a need to evaluate and compare different approaches for these under-resourced scenarios.

Method: The study compares multiple approaches: 1) zero-shot prompting of various sized LLMs, 2) fine-tuning smaller models like mT5 with and without three data augmentation approaches and multilingual transfer, and 3) an LLM translation pipeline (translate source language to English, summarize, translate back). Evaluation uses five different metrics.

Result: Key findings: 1) Variation in performance across LLMs even with similar parameter sizes, 2) Multilingual fine-tuned mT5 baseline outperforms most other approaches including zero-shot LLM performance for most metrics, 3) LLM as judge evaluation may be less reliable on less-resourced languages.

Conclusion: For less-resourced language summarization, fine-tuning smaller multilingual models like mT5 is more effective than zero-shot LLM approaches or translation pipelines. Additionally, traditional evaluation metrics may be more reliable than LLM-as-judge approaches for these languages.
...
Conclusion: The introduced model effectively cleans scientific abstracts to improve the accuracy of text-based analyses by removing extraneous information that distorts results.
...
TL;DR: Developed an IELTS writing revision platform with automated scoring and feedback that improved candidate scores by 0.06 bands on average.
...
Method: Used Design-Based Research (DBR) cycles to develop a platform with separate conversational guidance and writing interfaces. Progressed from rule-based to transformer-based (DistilBERT with regression head) scoring, then implemented adaptive feedback.
...
Conclusion: Automated feedback works best as a supplement to human instruction, with conservative surface-level corrections being more reliable than aggressive structural changes for IELTS preparation. Challenges remain in assessing higher-band essays.
...
Method: 1) Created two benchmarks: TEDPara (human-annotated TED talks) and YTSegPara (YouTube videos with synthetic labels). 2) Proposed constrained-decoding formulation for LLMs to insert paragraph breaks while preserving original transcripts. 3) Developed MiniSeg, a compact model that achieves SOTA accuracy and can be extended hierarchically for joint chapter and paragraph prediction.

Result: Established first benchmarks for paragraph segmentation in speech domain. MiniSeg attains state-of-the-art accuracy for paragraph segmentation and can jointly predict chapters and paragraphs with minimal computational cost. The constrained-decoding approach enables faithful, sentence-aligned evaluation.

Conclusion: The paper establishes paragraph segmentation as a standardized, practical task in speech processing through new benchmarks, methods, and models that enable structured, readable speech transcripts.
...
TL;DR: LLM safety alignment doesn't transfer reliably across languages; HausaSafety dataset reveals complex interference patterns where safety depends on language-temporal intersections, not simple degradation.


<details>
  <summary>Details</summary>
Motivation: The dangerous assumption that safety alignment transfers zero-shot from English to other languages needs testing, especially for Global South users facing localized threats.

Method: Systematic audit of GPT-5.1, Gemini 3 Pro, and Claude 4.5 Opus using HausaSafety dataset (West African threat scenarios) with 2x4 factorial design across 1,440 evaluations testing language (English/Hausa) and temporal framing interactions.

Result: Found Complex Interference mechanism instead of simple degradation: Claude 4.5 Opus safer in Hausa (45.0%) than English (36.7%) due to uncertainty-driven refusal, but catastrophic temporal reasoning failures with 9.2x disparity between safest/most vulnerable configurations.

Conclusion: Current models use superficial heuristics creating Safety Pockets, leaving Global South users exposed; propose Invariant Alignment paradigm for safety stability across linguistic/temporal shifts.
...
TL;DR: HaluNet is a lightweight neural framework that integrates multiple uncertainty sources (semantic embeddings + probabilistic confidence) for efficient one-pass hallucination detection in LLM-based QA systems.


<details>
  <summary>Details</summary>
Motivation: LLMs often generate hallucinations (factual errors/fabricated content) in QA tasks. Existing hallucination detection methods focus on single uncertainty types and overlook complementarity between token-level probability uncertainty and internal semantic representation uncertainty, missing opportunities for more reliable detection.

Method: HaluNet uses a multi-branch neural architecture that integrates multi-granular token-level uncertainties by combining semantic embeddings with probabilistic confidence and distributional uncertainty. It adaptively fuses what the model knows with uncertainty expressed in outputs for efficient one-pass detection.

Result: Experiments on SQuAD, TriviaQA, and Natural Questions show strong detection performance and favorable computational efficiency, with or without access to context, demonstrating potential for real-time hallucination detection.

Conclusion: HaluNet effectively leverages complementarity between different uncertainty sources for scalable, resource-independent hallucination detection in LLM-based QA systems, offering a practical solution for real-time applications.
...
TL;DR: KCL is a Korean legal reasoning benchmark that separates reasoning ability from legal knowledge by providing question-level supporting precedents, featuring both multiple-choice and open-ended essay components.


<details>
  <summary>Details</summary>
Motivation: To create a benchmark that can assess language models' legal reasoning capabilities independently of domain-specific legal knowledge, enabling more accurate evaluation of pure reasoning skills.
...
Result: Evaluation of 30+ models shows significant performance gaps, especially in KCL-Essay, with reasoning-specialized models consistently outperforming general-purpose models.

Conclusion: KCL successfully disentangles legal reasoning from knowledge, revealing substantial room for improvement in models' legal reasoning capabilities, particularly for open-ended tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs using long chain-of-thought reasoning are inefficient - they suffer from high latency due to excessive token generation and unstable reasoning that alternates between underthinking (shallow steps) and overthinking (repetitive reasoning).
...
Conclusion: Lightweight models can possess strong intrinsic agentic capabilities when systematically trained with proper architecture and progressive curriculum, demonstrating that small models don't need to rely on distillation for advanced reasoning and planning abilities.
...
TL;DR: LLMs are overconfident in predicting their own task success, with overconfidence worsening during multi-step tasks. While some LLMs can learn from failure experiences to improve decision-making, their overly optimistic estimates lead to poor decisions despite rational decision-making given those estimates.
...
Method: Tested multiple LLMs on their ability to predict task success, examined how predictions evolve during multi-step agentic tasks, and evaluated whether LLMs can learn from in-context failure experiences to improve decision-making.

Result: All LLMs tested were overconfident but had better-than-random discriminatory power. Newer/larger models generally didn't have greater discriminatory power (except Claude models). Overconfidence worsens during multi-step tasks, and reasoning LLMs performed comparably or worse than non-reasoning ones. Some LLMs reduced overconfidence with failure experiences, improving decision-making, while others didn't.

Conclusion: Current LLM agents lack awareness of their own capabilities, hindering their performance. LLMs make rational decisions given their estimated probabilities, but their overly optimistic estimates lead to poor outcomes. This has implications for AI misuse and misalignment risks.
...
TL;DR: R-Debater is an agentic framework for multi-turn debates using argumentative memory, combining retrieval of prior arguments with role-based agents to generate coherent, evidence-supported debates.


<details>
  <summary>Details</summary>
Motivation: The paper aims to create more faithful and coherent debates by grounding them in rhetoric and memory studies, addressing the need for stance consistency, evidence support, and adaptive responses to opponents across multiple turns.

Method: R-Debater integrates a debate knowledge base for retrieving case-like evidence and prior debate moves with role-based agents that compose coherent utterances. It's evaluated on ORCHID debates with a 1,000-item retrieval corpus and 32 held-out debates across seven domains.

Result: R-Debater outperforms strong LLM baselines on both single-turn (next-utterance generation) and multi-turn (adversarial simulations) tasks, achieving higher InspireScore and Debatrix scores. Human evaluation with 20 experienced debaters confirms better consistency and evidence use.

Conclusion: Combining retrieval grounding with structured planning yields more faithful, stance-aligned, and coherent debates across turns, demonstrating the effectiveness of integrating argumentative memory with agentic frameworks for debate generation.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-turn conversation evaluation for LLMs relies on expensive human assessment. While multi-turn reward models offer a scalable alternative, existing training data (single-turn contrasts) fails to capture the nuances of multi-turn interactions, limiting evaluation quality.

Method: Proposes MUSIC (Multi-Step Instruction Contrast), an unsupervised data augmentation strategy that synthesizes contrastive conversation pairs with differences spanning multiple turns. Applied to Skywork preference dataset to train a multi-turn RM based on Gemma-2-9B-Instruct.
...
Conclusion: Incorporating multi-turn contrasts is crucial for building robust multi-turn reward models. MUSIC provides an effective unsupervised approach to enhance multi-turn evaluation capabilities while maintaining single-turn performance.
...
TL;DR: BIOME-Bench: A benchmark for evaluating LLMs on biomolecular interaction inference and multi-omics pathway mechanism elucidation, showing current models still struggle with fine-grained analysis.


<details>
  <summary>Details</summary>
Motivation: Pathway enrichment workflows have limitations (curation lag, redundancy, limited sensitivity), and while LLMs show promise for improving interpretation, there's no standardized benchmark for evaluating end-to-end multi-omics pathway mechanism elucidation, hindering reproducible progress.

Method: Introduce BIOME-Bench constructed via four-stage workflow to evaluate LLMs on two core capabilities: Biomolecular Interaction Inference and end-to-end Multi-Omics Pathway Mechanism Elucidation. Develop evaluation protocols and conduct comprehensive experiments across multiple contemporary models.
...
Conclusion: BIOME-Bench addresses the lack of standardized evaluation for LLMs in multi-omics analysis, revealing current limitations and providing a foundation for future improvements in biomolecular interpretation.
...
TL;DR: Semi-SMDNet: A semi-supervised multilingual depression detection framework using teacher-student pseudo-labeling, ensemble learning, and data augmentation to overcome language style variations and data scarcity.


<details>
  <summary>Details</summary>
Motivation: Depression detection from social media text is challenging due to informal language styles, expression variations, and limited annotated data across multiple languages, creating a need for robust multilingual approaches.

Method: Proposes Semi-SMDNet with teacher-student pseudo-labeling, ensemble learning via soft voting, uncertainty-based threshold filtering for low-confidence labels, and confidence-weighted training focusing on reliable samples.

Result: Outperforms strong baselines on Arabic, Bangla, English, and Spanish datasets, significantly reducing performance gaps between resource-rich and resource-limited settings.

Conclusion: The framework is effective for scalable cross-language mental health monitoring in resource-limited scenarios, demonstrating robustness across diverse linguistic contexts.
...
<details>
  <summary>Details</summary>
Motivation: Current literature overlooks computational burden of long reasoning sequences in LLMs, while industrial applications require balancing accuracy with resource constraints and inference costs.

Method: Conducted test-time-compute aware evaluation of contemporary and older open-source LLMs, mapping Pareto frontiers across math- and reasoning-intensive benchmarks, analyzing Mixture of Experts architectures.

Result: Mixture of Experts (MoE) architecture identified as strong candidate for balancing performance and efficiency; discovered saturation point for inference-time compute where accuracy gains diminish; derived emergent trend for accuracy gain per unit of compute over time.

Conclusion: While extended reasoning capabilities are beneficial, they cannot overcome intrinsic model limitations - there's a compute saturation point beyond which accuracy gains diminish, making MoE architectures promising for practical applications.
...
TL;DR: Course on Ethical Aspects in NLP with active learning pedagogy, refined over 4 years across institutions, producing reusable teaching materials and student-created educational products.
...
TL;DR: The paper proposes triangulation as a causal standard for evaluating mechanistic explanations in multilingual language models, requiring necessity, sufficiency, and invariance across reference families (predicate-preserving variants).
...
Method: Formalizes reference families as predicate-preserving variants and introduces triangulation with three criteria: necessity (circuit ablation degrades behavior), sufficiency (patching activations transfers behavior), and invariance (effects remain stable across reference family). Uses automatic circuit discovery and accepts/rejects candidates via triangulation.

Result: Triangulation provides a falsifiable standard that filters spurious circuits that pass single-environment tests but fail cross-lingual invariance, connecting to causal abstraction and pragmatic interpretability.

Conclusion: Triangulation offers a rigorous causal framework for evaluating mechanistic claims in multilingual models, ensuring explanations survive interventions and maintain consistency across linguistic variations while preserving meaning.
...
TL;DR: PrivacyBench benchmark reveals RAG assistants leak user secrets up to 26.56% of the time, showing current architectures are unsafe for wide-scale deployment without privacy-by-design safeguards.
...
Conclusion: Current architectures are unsafe for wide-scale deployment. Urgent need for structural, privacy-by-design safeguards to ensure ethical and inclusive web for everyone, moving beyond partial prompt-based mitigation.
...
TL;DR: The paper critiques how Big AI and language engineering are accelerating ecological, meaning, and language crises, calling for alternatives focused on human flourishing and planetary health.


<details>
  <summary>Details</summary>
Motivation: The world faces converging ecological, meaning, and language crises (metacrisis). Big AI is accelerating these crises, and language engineers are contributing to the problem by working for plutocrats/kleptocrats and treating technology development as value-free. There's an urgent need to explore alternatives.

Method: The paper appears to be a critical analysis and call to action rather than presenting a specific technical method. It advocates for applying collective intelligence to design alternatives to current NLP practices.

Result: The paper identifies the problematic role of language engineers in perpetuating harmful systems and calls for a paradigm shift in NLP away from scalability narratives toward life-affirming approaches.

Conclusion: NLP must be redesigned to center human flourishing on a living planet, moving away from value-free technology development and addressing the metacrisis through collective intelligence and alternative approaches.
...
TL;DR: Encyclo-K is a statement-based benchmark that uses knowledge statements from textbooks to dynamically generate questions, addressing data contamination, single-knowledge limitations, and high annotation costs.


<details>
  <summary>Details</summary>
Motivation: Existing LLM benchmarks have three key limitations: vulnerability to data contamination, restriction to single-knowledge-point assessment, and reliance on costly domain expert annotation. The authors aim to create a more robust evaluation framework.
...
Result: Experiments on 50+ LLMs show strong discriminative power - even top-performing OpenAI-GPT-5.1 achieves only 62.07% accuracy. Reasoning models range from 16.04% to 62.07%, while chat models range from 9.71% to 50.40%.
...
TL;DR: Manifold-Constrained Hyper-Connections (mHC) framework restores identity mapping in Hyper-Connections to solve training instability and scalability issues while improving performance.


<details>
  <summary>Details</summary>
Motivation: Hyper-Connections (HC) extend residual connections but lose the identity mapping property, causing training instability, restricted scalability, and memory overhead. Need to restore identity mapping while maintaining HC benefits.

Method: Project HC's residual connection space onto a specific manifold to restore identity mapping property, with rigorous infrastructure optimization for efficiency.

Result: mHC enables effective large-scale training with tangible performance improvements and superior scalability compared to HC.
...
TL;DR: BEDA framework bridges belief estimation with strategic dialogue generation by formalizing adversarial/alignment acts and using probabilistic constraints, achieving superior performance across adversarial, cooperative, and negotiation settings.


<details>
  <summary>Details</summary>
Motivation: Prior work accurately estimates beliefs in strategic dialogue but lacks principled mechanisms to use those beliefs during generation, creating a gap between belief estimation and strategic act execution.

Method: Formalizes adversarial and alignment dialogue acts, operationalizes them via probabilistic constraints on generation, and implements BEDA framework with world set, belief estimator, and conditional generator that selects acts consistent with inferred beliefs.

Result: BEDA consistently outperforms baselines: improves success rate by at least 5.0 points on CKBG (up to 20.6 points with GPT-4.1-nano), achieves 9.3 point average improvement on Mutual Friends, and reaches optimal deal on CaSiNo negotiation.

Conclusion: Casting belief estimation as constraints provides a simple, general mechanism for reliable strategic dialogue, effectively bridging the gap between belief estimation and strategic generation.
...
TL;DR: ADOPT is a framework for optimizing prompts in multi-step LLM pipelines by modeling step dependencies and enabling precise text-gradient estimation.


<details>
  <summary>Details</summary>
Motivation: Multi-step LLM pipelines are effective for complex tasks but their performance heavily depends on prompts at each step. Joint optimization is difficult due to missing step-level supervision and inter-step dependencies, and existing methods yield suboptimal or unstable results.
...
Conclusion: ADOPT provides a novel framework for optimizing multi-step LLM pipelines by addressing dependency modeling and gradient estimation challenges, offering improved performance over existing methods.
...
TL;DR: Legal document classifier using DeBERTa V3 + LSTM on random 48 chunks (128 tokens each) with Temporal deployment pipeline achieves 0.898 F-score and 498s median processing time per 100 files on CPU.


<details>
  <summary>Details</summary>
Motivation: Legal documents present classification challenges due to specialized vocabulary and extreme length, making full-document processing with Transformers models impossible, expensive, or slow.
...
Result: Best model achieved weighted F-score of 0.898. Pipeline running on CPU had median processing time of 498 seconds per 100 files.

Conclusion: The chunk-based approach with DeBERTa V3 + LSTM effectively handles long legal documents, while Temporal provides robust deployment for reliable document processing workflows.
...
TL;DR: RESTOREx introduces a resource for detecting depressive symptoms in memes using LLM-generated and human-annotated explanations, with MAMAMemeia framework achieving 7.55% improvement in macro-F1 over state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Memes have evolved beyond humor to express various emotions, including depressive sentiments on social media. There's a growing need to identify depressive symptoms in memes shared on online platforms to address mental health concerns.
...
Result: MAMAMemeia improves upon current state-of-the-art by 7.55% in macro-F1 and establishes a new benchmark compared to over 30 existing methods.

Conclusion: The paper presents an effective framework for detecting depressive symptoms in memes, combining clinical psychology principles with multi-agent AI systems to significantly outperform existing approaches.
...
Method: TG is a recurrent Transformer that models language at two levels: tokens and sentence-level "thought" states. It generates tokens one sentence at a time while cross-attending to a memory of prior sentence representations. Both token and sentence representations use the same model parameters and are trained with a single next-token cross-entropy objective. Gradients from future token losses flow backward through cross-attention to optimize earlier sentence vectors.

Result: TG consistently improves efficiency over matched GPT-2 runs, with scaling fits showing GPT-2 requires ~5-8% more data and ~33-42% more parameters to match TG's loss. TG also reduces errors on relational direction generalization tasks, specifically on a father-son reversal curse probe.

Conclusion: The Thought Gestalt model successfully addresses limitations of standard Transformers by incorporating cognitive-inspired sentence-level representations, leading to improved data efficiency, parameter efficiency, and better relational reasoning capabilities.
...
TL;DR: AdaGReS is a redundancy-aware context selection framework for token-budgeted RAG that optimizes relevance while minimizing redundancy through adaptive calibration and greedy selection with theoretical guarantees.


<details>
  <summary>Details</summary>
Motivation: Standard top-k retrieval in RAG often returns redundant or near-duplicate chunks that waste token budget and degrade downstream generation quality, highlighting the need for smarter context selection.

Method: AdaGReS performs greedy selection under token-budget constraints using marginal gains from an objective combining query-chunk relevance and intra-set redundancy penalties, with adaptive calibration of the relevance-redundancy trade-off parameter.

Result: Experiments on Natural Questions and biomedical corpus show consistent improvements in redundancy control and context quality, leading to better end-to-end answer quality and robustness across settings.

Conclusion: AdaGReS effectively addresses redundancy issues in RAG through theoretically-grounded, adaptive context selection that eliminates manual tuning and improves generation quality within token budgets.
...
TL;DR: This paper presents a method for improving monocular depth estimation in endoscopic surgery by leveraging synthetic priors from Depth Anything V2 and adapting them to medical domains using DV-LORA, achieving state-of-the-art results on the SCARED dataset.


<details>
  <summary>Details</summary>
Motivation: Monocular Depth Estimation (MDE) is crucial for robotic surgery but performs poorly in challenging endoscopic environments with specular reflections and fluids. Existing self-supervised methods trained on noisy real-world pseudo-labels suffer from boundary collapse on thin surgical tools and transparent surfaces.

Method: The approach leverages high-fidelity synthetic priors from the Depth Anything V2 architecture, which captures precise geometric details of thin structures. These priors are efficiently adapted to the medical domain using Dynamic Vector Low-Rank Adaptation (DV-LORA) to minimize parameter budget while bridging the synthetic-to-real gap. The paper also introduces a physically-stratified evaluation protocol on the SCARED dataset.

Result: The method establishes a new state-of-the-art, achieving an accuracy (< 1.25) of 98.1% and reducing Squared Relative Error by over 17% compared to established baselines. It demonstrates superior robustness in adverse surgical lighting conditions.

Conclusion: The proposed approach effectively addresses the challenges of monocular depth estimation in endoscopic surgery by combining synthetic priors with efficient domain adaptation, resulting in significantly improved performance in high-specularity surgical environments.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods for assessing urban warfare training (like Enter and Clear the Room drills) rely on costly sensors or subjective human observation, limiting scalability and accuracy. There's a need for objective, automated performance assessment of cognitive, psychomotor, and teamwork skills in Synthetic Training Environments.
...
Result: Demonstrated approach with real-world ECR drills, providing actionable domain-specific metrics capturing individual and team performance. The system enables interactive dashboards for After Action Reviews within Gamemaster and GIFT frameworks.

Conclusion: Video-based assessment offers scalable evaluation within STEs without additional hardware. Limitations include tracking difficulties and ground-truth validation. Future work includes expanding to 3D video data and leveraging video analysis for broader STE evaluation.
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of efficiently processing long videos in autoregressive video models by compressing them into shorter contexts while preserving important visual details, reducing computational cost and memory requirements.

Method: PFP uses a neural network structure with explicit pretraining objectives to preserve high-frequency details of single frames at arbitrary temporal positions. The model compresses 20-second videos into ~5k length contexts and can be fine-tuned as memory encoders for autoregressive video models.

Result: The framework achieves perceptually preserved appearance retrieval of random frames from compressed contexts, enabling long history memory with low context cost and relatively low fidelity loss. The paper includes ablative evaluations and discusses trade-offs of neural architecture designs.

Conclusion: PFP provides an effective approach for video compression that maintains frame-level details while significantly reducing context length, making it suitable for integration with autoregressive video models to handle long-term dependencies efficiently.
...
TL;DR: The paper proposes a lifelong domain adaptation framework for 3D human pose estimation to address generalization challenges in diverse real-world scenarios, using a novel GAN-based approach with specialized pose generators.


<details>
  <summary>Details</summary>
Motivation: Current 3D HPE methods rely on annotated data from controlled environments, limiting generalization to diverse real-world scenarios. Existing domain adaptation approaches overlook non-stationary target pose datasets and suffer from catastrophic forgetting when adapting to multiple domains sequentially.

Method: A novel Generative Adversarial Network framework with 3D pose generators, 2D pose discriminator, and 3D pose estimator. Includes a specialized 3D pose generator paradigm integrating pose-aware, temporal-aware, and domain-aware knowledge to enhance current domain adaptation while preserving previous domain knowledge.

Result: The method demonstrates superior performance through extensive experiments on diverse domain adaptive 3D HPE datasets, effectively mitigating domain shifts and combating catastrophic forgetting.

Conclusion: The proposed lifelong domain adaptation framework successfully addresses the challenges of adapting 3D HPE to diverse real-world scenarios while preserving knowledge from previous domains, representing the first application of lifelong domain adaptation to 3D HPE.
...
TL;DR: Deep learning pipeline transforms pediatric T1-weighted MRIs into synthetic CTs for cranial bone and suture segmentation, enabling non-invasive assessment of skull development without radiation exposure.


<details>
  <summary>Details</summary>
Motivation: CT scans provide detailed cranial bone and suture information but use harmful ionizing radiation in children. MRI is radiation-free but cannot visualize cranial sutures or assess bone density. There's a need for non-invasive methods to evaluate pediatric cranial development.

Method: Deep learning pipeline that transforms T1-weighted MRIs (ages 0.2-2 years) into synthetic CTs using domain-specific variational autoencoders. The pipeline predicts cranial bone segmentation, generates suture probability heatmaps, and derives direct suture segmentation from heatmaps.

Result: Synthetic CTs achieved 99% structural similarity and Fréchet inception distance of 1.01 compared to real CTs. Skull segmentation achieved 85% Dice coefficient across seven cranial bones, and sutures achieved 80% Dice. Statistical equivalence confirmed using TOST (p < 0.05).

Conclusion: First pediatric cranial CT synthesis framework enabling suture segmentation from MRI, bridging critical gaps in non-invasive cranial evaluation. The method generates perceptually indistinguishable cranial synthetic CTs from routine pediatric MRIs.
...
<details>
  <summary>Details</summary>
Motivation: While scaling laws are well-established for natural images with abundant internet data, they're poorly understood for high-value domains like remote sensing. The research aims to establish practical techniques for training foundation models on EO datasets that exceed current state-of-the-art scale by orders of magnitude.

Method: Using over a quadrillion pixels of commercial satellite EO data and the MITRE Federal AI Sandbox, the researchers train progressively larger vision transformer (ViT) backbones and analyze scaling behaviors at petascale. They examine success and failure modes and implications for bridging domain gaps across additional RS modalities.

Result: The study finds that even at this massive scale, performance remains consistent with a data-limited regime rather than a model parameter-limited one. This contrasts with natural image domains where scaling laws are better understood.

Conclusion: The practical insights from this petascale analysis should inform data-collection strategies, compute budgets, and optimization schedules for advancing frontier-scale remote sensing foundation models, helping bridge the domain gap between natural images and specialized RS applications.
...
TL;DR: Proposes a bi-level optimization framework for automated fetal ultrasound skill assessment that predicts skills based on task performance quality rather than predefined ratings.
...
Method: A bi-level optimization framework with two jointly optimized networks: a clinical task predictor and a skill predictor, which assesses skills by how well ultrasound tasks are performed on acquired images without using manual skill ratings.
...
Conclusion: The framework successfully enables automated ultrasound skill assessment by linking skill levels directly to task performance quality, offering a more objective and flexible alternative to traditional supervised methods.
...
TL;DR: A meta-guided multimodal learning framework for brain tumor segmentation that handles missing MRI modalities through adaptive fusion and consistency regularization.
...
Method: Proposes MGML framework with two components: 1) Meta-parameterized adaptive modality fusion (Meta-AMF) that generates adaptive soft-label supervision based on available modalities, and 2) Consistency regularization module to enhance segmentation performance and robustness.

Result: Achieved superior performance on BraTS2020 dataset with average Dice scores of 87.55 (WT), 79.36 (TC), and 62.67 (ET) across fifteen missing modality combinations, outperforming state-of-the-art methods.

Conclusion: The MGML framework effectively handles incomplete multimodal MRI data without altering original model architecture, providing a practical solution for clinical applications where missing modalities are common.
...
Conclusion: The proposed system effectively addresses cross-view discrepancies through adaptive feature processing and achieves efficient geo-localization with reduced parameter requirements.
...
<details>
  <summary>Details</summary>
Motivation: Current microsurgical training assessment relies on subjective expert evaluation prone to variability and inconsistency, requiring automated, objective solutions for scalable surgical education.

Method: Three-component AI system: (1) YOLO+DeepSORT for instrument tip tracking, (2) self-similarity matrix for unsupervised action segmentation, (3) supervised classification for skill proficiency assessment, designed for edge computing.

Result: Achieved 92.4% frame-level action segmentation accuracy and 85.5% skill classification accuracy on 58 expert-rated microanastomosis videos, effectively replicating expert evaluations.
...
TL;DR: DehazeSNN: A U-Net-like Spiking Neural Network architecture for image dehazing that combines multi-scale feature capture with efficient long-range dependency modeling using novel OLIFBlocks, achieving state-of-the-art performance with reduced computational cost.


<details>
  <summary>Details</summary>
Motivation: Image dehazing is crucial for computer vision applications in hazy conditions. Traditional methods rely on atmospheric scattering models, while modern deep learning approaches (CNNs and Transformers) have limitations: CNNs struggle with long-range dependencies, and Transformers require heavy computational resources.

Method: Proposes DehazeSNN, a U-Net-like architecture integrating Spiking Neural Networks (SNNs) for image dehazing. The method features an Orthogonal Leaky-Integrate-and-Fire Block (OLIFBlock) that enhances cross-channel communication while efficiently managing local and long-range dependencies.

Result: Extensive experiments show DehazeSNN is highly competitive with state-of-the-art methods on benchmark datasets, producing high-quality haze-free images with smaller model size and fewer multiply-accumulate operations (reduced computational burden).

Conclusion: DehazeSNN successfully addresses limitations of both CNNs and Transformers for image dehazing by combining SNNs with U-Net architecture and novel OLIFBlocks, achieving excellent performance with computational efficiency. The method is publicly available as open-source.
...
TL;DR: T2VAttack: First comprehensive study of adversarial attacks on Text-to-Video diffusion models, revealing vulnerabilities through semantic and temporal attacks on prompts.


<details>
  <summary>Details</summary>
Motivation: Despite rapid advances in Text-to-Video (T2V) diffusion models generating high-quality videos, their vulnerability to adversarial attacks remains unexplored. The paper aims to investigate this security gap.

Method: Proposes T2VAttack with two attack objectives (semantic and temporal) and two methods: T2VAttack-S (replaces critical words with synonyms via greedy search) and T2VAttack-I (iteratively inserts optimized words with minimal prompt perturbation).
...
TL;DR: A novel view extrapolation method for autonomous driving that uses only images and optional camera poses, combining 4D Gaussian reconstruction with iterative diffusion model refinement to generate high-quality novel viewpoint images without expensive sensors or annotations.


<details>
  <summary>Details</summary>
Motivation: Existing view extrapolation methods rely on expensive sensors (LiDAR) or labor-intensive annotations (3D bounding boxes, lane markings), limiting real-world deployment. The paper aims to develop a solution using only images and optional camera poses to overcome these limitations.

Method: 1) Estimate global static and per-frame dynamic point clouds from images/poses, fusing them into unified representation; 2) Use deformable 4D Gaussian framework for scene reconstruction; 3) Train video diffusion model on degraded renders from initial 4D Gaussian; 4) Iteratively refine progressively shifted Gaussian renderings with diffusion model, using enhanced results to retrain 4DGS; 5) Continue until reaching target extrapolated viewpoints.
...
Conclusion: The proposed approach successfully addresses view extrapolation in autonomous driving using only images and optional poses, overcoming limitations of prior methods that depend on expensive sensors or annotations, while achieving superior image quality at novel viewpoints.
...
TL;DR: Deep learning model uses satellite time series to detect surface changes by predicting what the surface should look like, achieving 3x better sensitivity than traditional methods for detecting earthquake ruptures.
...
Result: The method detected earthquake-triggered surface ruptures from the 2023 Turkey-Syria earthquake sequence with higher sensitivity and specificity than temporal median or Reed-Xiaoli anomaly detectors, achieving detection thresholds approximately three times lower than baseline approaches.
...
Result: Achieves Dice scores of 86.11% on Synapse and 92.64% on ACDC benchmarks, outperforming both CNN and Transformer-based methods including Swin-UNet and TransUNet. Particularly effective at delineating small anatomical structures with complex boundaries.

Conclusion: GCA-ResUNet provides an optimal trade-off between segmentation accuracy and computational efficiency, offering a practical and scalable solution for clinical deployment in resource-constrained environments.
...
Method: Proposes GASeg framework with: 1) Differentiable Box-Counting (DBC) module that quantifies multi-scale topological statistics from geometric and appearance features, 2) Topological Augmentation (TopoAug) that applies morphological operators to simulate real-world ambiguities, and 3) GALoss for cross-modal alignment between geometric and appearance features.
...
Conclusion: Bridging appearance and geometry through stable topological information effectively addresses appearance ambiguities in self-supervised semantic segmentation, leading to improved robustness and performance.
...
TL;DR: Novel pipeline for 3D reconstruction of unknown spacecraft using 3D Gaussian Splatting with solar position priors to handle dynamic space lighting, enabling both geometry recovery and camera pose estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional 3DGS requires static scenes, but spaceborne imagery during RPO has dynamic lighting conditions. Accurate photometric rendering is crucial for downstream pose estimation tasks in RPO operations.

Method: Uses 3D Gaussian Splatting with Sun position priors (estimated by servicer spacecraft) integrated into training pipeline to improve photometric quality. Adapts 3DGS to handle rapidly changing illumination in space.

Result: 3DGS models learn to adapt to dynamic space lighting, capturing global shadowing and self-occlusion effects. Experimental studies demonstrate effectiveness in improving photometric quality.

Conclusion: Incorporating solar position knowledge enables 3DGS to work effectively in space environments, producing geometrically accurate models with photometric quality suitable for camera pose estimation during RPO.
...
TL;DR: Hilbert-VLM: A two-stage fusion framework for 3D medical image analysis that combines Hilbert space-filling curves with SAM2 architecture for precise lesion segmentation, then uses enhanced multimodal prompts to guide VLM for disease classification.
...
Method: Two-stage framework: 1) HilbertMed-SAM module with redesigned SAM2 architecture using Hilbert space-filling curves in Mamba SSM scanning, Hilbert-Mamba Cross-Attention, and scale-aware decoder for lesion segmentation; 2) Prompt enhancement module that unifies segmentation masks with textual attributes to guide VLM for classification.
...
Conclusion: Hilbert-VLM offers substantial potential to improve medical VLM-based analysis by effectively handling 3D multimodal medical images through systematic architectural redesign and multimodal prompt enhancement.
...
TL;DR: CVC (Conditioned Velocity Correction) is a flow-based diffusion editing framework that corrects velocity errors in latent trajectories to maintain structural fidelity and semantic consistency during image distribution transformations.
...
Method: CVC introduces a dual-perspective velocity conversion mechanism that decomposes latent evolution into structure-preserving and semantically-guided branches, with posterior-consistent updates derived from Empirical Bayes Inference and Tweedie correction to compensate for velocity errors.

Result: CVC achieves stable and interpretable latent dynamics with faithful reconstruction and smooth local semantic conversion, consistently outperforming other methods in fidelity, semantic alignment, and editing reliability across diverse tasks.

Conclusion: The proposed CVC framework provides a principled solution to velocity error accumulation in flow-based editing, enabling more reliable and faithful image distribution transformations while maintaining structural integrity.
...
TL;DR: FitControler is a learnable plug-in for virtual try-on models that enables customized garment fit control by generating fit-aware layouts and injecting them into VTON pipelines.
...
Result: FitControler works with various VTON models and achieves accurate fit control. Two fit consistency metrics introduced to assess fitness of generations. Extensive experiments demonstrate effectiveness.

Conclusion: The paper introduces fit-aware VTON and FitControler as a learnable plug-in that enables customized fit control, addressing a key gap in current VTON research by considering garment fit as a fundamental element of fashion design.
...
Method: Three key techniques: 1) Structure-guided initialization assigning 2D Gaussians based on spatial structural priors; 2) Adaptive bitwidth quantization of covariance parameters (higher precision for small-scale Gaussians in complex regions); 3) Geometry-consistent regularization aligning Gaussian orientations with local gradient directions.
...
Conclusion: The proposed structure-guided allocation principle effectively couples image structure with representation capacity and quantization precision, enabling significant rate-distortion improvements in 2D Gaussian Splatting while preserving fast decoding speeds.
...
<details>
  <summary>Details</summary>
Motivation: Standard VLMs struggle with remote sensing due to domain differences between natural and RS images, failing to extract fine-grained features and suffering from visual forgetting during language processing.

Method: Proposes MF-RSVLM with multi-scale visual feature extraction, fusion of global context with local details, and recurrent visual feature injection to maintain visual grounding during language generation.
...
Conclusion: MF-RSVLM effectively addresses RS-specific challenges in VLMs through multi-feature fusion and recurrent visual injection, demonstrating superior performance across multiple RS understanding tasks.
...
Conclusion: RSAgent's agentic approach with multi-turn reasoning and tool interactions enables more robust text-guided segmentation through iterative refinement, outperforming existing one-shot methods and showing strong generalization capabilities.
...
Conclusion: PipeFlow provides a scalable solution for long-form video editing by addressing computational bottlenecks through intelligent frame skipping, parallel processing, and interpolation techniques, making long video editing practical and efficient.
...
TL;DR: A reinforcement learning-based anisotropic diffusion framework for image denoising that learns optimal diffusion actions through deep Q-learning, outperforming traditional diffusion methods and competing with CNN-based approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional anisotropic diffusion methods use explicit diffusion operators that are not well adapted to complex image structures, limiting their performance compared to learning-based approaches. The authors aim to develop a more adaptive diffusion framework.

Method: A trainable anisotropic diffusion framework using reinforcement learning. The denoising process is modeled as a series of naive diffusion actions with order learned by deep Q-learning, creating a stochastic anisotropic diffusion process adaptive to different image structures.
...
TL;DR: NIRNL is a robust cross-modal retrieval framework that handles noisy labels through cross-modal margin preserving and neighbor-aware instance refining with tailored optimization strategies for different data subsets.
...
Method: Proposes NIRNL framework with two main components: 1) Cross-modal Margin Preserving (CMP) to adjust relative distances between positive/negative pairs for better discrimination, and 2) Neighbor-aware Instance Refining (NIR) to identify pure, hard, and noisy subsets through cross-modal neighborhood consensus, then applies tailored optimization strategies for each subset.
...
Conclusion: NIRNL effectively addresses noisy label problems in cross-modal retrieval by maximizing data utilization while mitigating error propagation through fine-grained data partitioning and tailored optimization strategies.
...
TL;DR: PCRNet: A deep learning framework for ocular disease recognition that incorporates pathology context and expert experience priors through novel modules (PRM and EPGA) and an integrated loss function, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Current DNNs for ocular disease diagnosis ignore important clinical context - pathology knowledge and expert experience. These priors could improve both recognition performance and interpretability of decisions.

Method: Developed Pathology Recalibration Module (PRM) to leverage pathology context via pixel-wise context compression and distribution concentration operators. Created Expert Prior Guidance Adapter (EPGA) to highlight important regions using expert experience. Combined these with DNNs to form PCRNet. Introduced Integrated Loss (IL) considering sample-wise loss distributions and label frequencies.

Result: PCRNet with IL outperforms state-of-the-art attention-based networks and advanced loss methods on three ocular disease datasets. Visualization analysis shows PRM and EPGA effectively influence DNN decision-making processes.

Conclusion: Incorporating clinical pathology context and expert experience priors through PRM and EPGA modules significantly improves ocular disease recognition performance and provides better interpretability for clinical decision-making.
...
Method: Balanced hierarchical contrastive loss with learnable class prototypes that equilibrates gradients across classes at each hierarchical level. Decoupled learning strategy separates DETR's object queries into classification and localization sets for task-specific feature extraction and optimization.

Result: Outperforms state-of-the-art approaches on three fine-grained datasets with hierarchical annotations.

Conclusion: The proposed balanced hierarchical contrastive loss with decoupled learning effectively addresses data imbalance and localization interference, improving fine-grained detection performance in hierarchical remote sensing datasets.
...
TL;DR: RainFusion2.0 proposes an online adaptive sparse attention mechanism for Diffusion Transformers that achieves 80% sparsity with 1.5-1.8x speedup while maintaining quality, with hardware-agnostic design for diverse platforms.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformer models suffer from high computational costs due to attention mechanisms, limiting practical applications. Existing sparse attention methods have overhead issues and lack hardware generality, being mostly GPU-focused.

Method: Proposes RainFusion2.0 with three key techniques: (1) using block-wise mean values as representative tokens for sparse mask prediction, (2) spatiotemporal-aware token permutation, and (3) first-frame sink mechanism for video generation.
...
Conclusion: RainFusion2.0 provides an effective solution to reduce computational costs of Diffusion Transformers through hardware-efficient sparse attention, enabling practical deployment across diverse hardware platforms while maintaining quality.
...
TL;DR: D²VLM decouples temporal grounding and textual response learning in video-language models using evidence tokens and factorized preference optimization for better event-level video understanding.
...
Method: Proposes D²VLM framework with "grounding then answering with evidence referencing" paradigm using evidence tokens for event-level semantic capture, plus Factorized Preference Optimization (FPO) that incorporates probabilistic temporal grounding modeling into optimization objectives.

Result: Experiments on various tasks demonstrate clear advantages of the approach over existing methods, with improved temporal grounding and textual response quality.

Conclusion: Factorized learning of temporal grounding and textual response with explicit dependency modeling and specialized optimization leads to superior video-language understanding performance.
...
<details>
  <summary>Details</summary>
Motivation: Current T2M generation treats it as direct translation from language to poses, but faces a fundamental bottleneck called Semantic-Kinematic Impedance Mismatch - the difficulty of grounding semantically dense language into kinematically dense motion data in one shot.

Method: Proposes Latent Motion Reasoning (LMR) with a Dual-Granularity Tokenizer that disentangles motion into two manifolds: Reasoning Latent (for planning global topology) and Execution Latent (for preserving physical fidelity). The model first reasons/plans coarse trajectory, then instantiates frames.
...
TL;DR: A training-free generative adversarial attack framework using diffusion models to create naturalistic adversarial objects that fool monocular depth estimation in autonomous driving systems.


<details>
  <summary>Details</summary>
Motivation: Monocular Depth Estimation (MDE) is critical for autonomous driving but vulnerable to attacks. Existing physical attacks use unrealistic texture patches with strict placement constraints, limiting their effectiveness in complex driving environments.

Method: Training-free generative adversarial attack framework with: 1) Salient Region Selection to identify MDE-influential regions, 2) Jacobian Vector Product Guidance to steer adversarial gradients toward diffusion model-supported directions, enabling generation of physically plausible adversarial objects via diffusion-based conditional generation.
...
Conclusion: The framework demonstrates strong practical implications for autonomous driving safety assessment by generating naturalistic, scene-consistent adversarial objects that effectively compromise MDE systems.
...
TL;DR: This paper introduces FSAP (Few-Shot Architecture Prompting) and Whitespace-Normalized Hash Validation for LLM-based neural architecture generation in computer vision, achieving efficient automated design with 100x speedup over AST parsing.
...
Method: 1) Few-Shot Architecture Prompting (FSAP): Systematic study of supporting examples (n=1-6) for LLM-based architecture generation, finding n=3 optimal. 2) Whitespace-Normalized Hash Validation: Lightweight deduplication method (<1ms) for preventing redundant training of duplicate architectures. 3) Dataset-balanced evaluation methodology for comparing architectures across heterogeneous vision tasks.

Result: Generated 1,900 unique architectures across seven computer vision benchmarks (MNIST, CIFAR-10, CIFAR-100, CelebA, ImageNette, SVHN, Places365). FSAP with n=3 examples best balances architectural diversity and context focus. Hash validation provides 100x speedup over AST parsing.

Conclusion: The contributions provide actionable guidelines for LLM-based architecture search in computer vision and establish rigorous evaluation practices, making automated design more accessible to researchers with limited computational resources.
...
Method: Proposes Directional Decoupling Alignment (D²-Align): 1) Introduces DivGenBench benchmark to quantify PMC, 2) Learns directional correction within reward model's embedding space while keeping model frozen, 3) Applies correction to reward signal during optimization to prevent mode collapse.
...
Conclusion: The proposed D²-Align framework effectively mitigates Preference Mode Collapse in text-to-image diffusion models by directionally correcting reward signals, enabling better diversity preservation while maintaining alignment with human preferences.
...
<details>
  <summary>Details</summary>
Motivation: There is a need for large-scale, high-quality multimodal datasets specifically for industrial defect detection and manufacturing quality inspection to advance multimodal learning in industrial applications.

Method: Created IMDD-1M dataset with 1M aligned image-text pairs covering diverse materials and defect types, then trained a diffusion-based vision-language foundation model from scratch on this dataset for industrial scenarios.

Result: The foundation model achieves comparable performance to dedicated expert models while requiring less than 5% of task-specific data, demonstrating efficient adaptation through lightweight fine-tuning.

Conclusion: IMDD-1M enables scalable, domain-adaptive manufacturing intelligence, showing that foundation models can be efficiently adapted to industrial inspection tasks with minimal task-specific data.
...
TL;DR: Bayesian Self-Distillation (BSD) uses Bayesian inference on model predictions to create sample-specific target distributions, improving accuracy, calibration, and robustness without hard targets.
...
Method: BSD constructs sample-specific target distributions via Bayesian inference using the model's own predictions, eliminating dependence on hard targets after initialization.

Result: BSD consistently improves test accuracy (+1.4% for ResNet-50 on CIFAR-100) and reduces ECE (-40% ResNet-50, CIFAR-100) across architectures and datasets. Also improves robustness against data corruptions, perturbations, and label noise.

Conclusion: BSD is a principled self-distillation method that outperforms existing approaches in accuracy, calibration, and robustness, achieving state-of-the-art robustness under label noise when combined with contrastive loss.
...
TL;DR: DiffThinker introduces a diffusion-based generative multimodal reasoning framework that treats reasoning as an image-to-image task, outperforming state-of-the-art MLLMs on vision-centric tasks.
...
Method: DiffThinker establishes a Generative Multimodal Reasoning paradigm using diffusion models to reformulate multimodal reasoning as a native generative image-to-image task, enabling logical consistency and spatial precision.
...
Conclusion: Generative multimodal reasoning via diffusion models represents a promising approach for vision-centric reasoning, offering superior performance and core properties including efficiency, controllability, native parallelism, and collaboration.
...
<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging faces computational bottlenecks from massive data volumes, and existing foundation models pre-trained on remote sensing data fail to transfer effectively to domain-specific applications like close-range agricultural monitoring where spectral signatures and spatial scales differ fundamentally.

Method: DGC operates on small patches with overlapping regions to enforce consistency, learning global clustering structure from local patch observations without requiring pre-training. It maintains constant memory usage and can train in under 30 minutes on consumer hardware.
...
Conclusion: DGC presents a promising design philosophy for memory-efficient HSI segmentation, but requires principled approaches to dynamic loss balancing for stable implementation. The work is positioned as intellectual scaffolding rather than a fully polished solution.
...
<details>
  <summary>Details</summary>
Motivation: Standard diffusion models struggle with low-probability areas and CFG often causes oversimplified/distorted samples, while alternative guidance methods require complex degradation strategies, extra training, and additional sampling steps.

Method: Proposes Internal Guidance (IG) which adds auxiliary supervision on intermediate layers during training and extrapolates intermediate/deep layer outputs during sampling to guide generation.

Result: Significant improvements in training efficiency and generation quality: SiT-XL/2+IG achieves FID=5.31 (80 epochs) and 1.75 (800 epochs) on ImageNet 256x256; LightningDiT-XL/1+IG achieves FID=1.34, and with CFG achieves state-of-the-art FID=1.19.

Conclusion: IG is a simple yet effective strategy that outperforms existing guidance methods, achieving new state-of-the-art results on ImageNet generation benchmarks.
...
TL;DR: PointRAFT: A high-throughput point cloud regression network that directly predicts potato tuber weight from partial 3D point clouds captured on harvesters, overcoming self-occlusion issues without full 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Potato yield estimation on harvesters using RGB-D cameras faces challenges due to incomplete point clouds from self-occlusion, leading to systematic underestimation of tuber weight. There's a need for accurate, high-throughput weight prediction that works with partial 3D data under practical harvesting conditions.

Method: PointRAFT is a point cloud regression network that directly predicts continuous 3D shape properties (like tuber weight) from partial point clouds. Key innovation: object height embedding that incorporates tuber height as additional geometric cue. Network processes raw 3D data without reconstructing full geometry.

Result: Tested on 5,254 point clouds from 172 tubers: achieved MAE of 12.0g and RMSE of 17.2g, substantially outperforming linear regression baseline and standard PointNet++ regression. Average inference time of 6.3ms per point cloud supports processing rates up to 150 tubers per second.

Conclusion: PointRAFT provides accurate, high-throughput potato weight estimation suitable for commercial harvesters. Beyond potato applications, it offers a versatile regression network for various 3D phenotyping and robotic perception tasks. Code, weights, and dataset subset are publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Diffusion transformers (DiT) have high inference costs due to their iterative denoising process and large model capacity, with substantial redundant computation across steps that can be optimized.

Method: CorGi uses contribution-guided block-wise interval caching to selectively reuse outputs of low-contribution transformer blocks across denoising steps. CorGi+ extends this for text-to-image tasks by leveraging cross-attention maps to identify salient tokens and apply partial attention updates.
...
Conclusion: The proposed training-free acceleration frameworks effectively reduce redundant computation in DiT models through selective block caching and reuse, enabling significant inference speedup without compromising generation quality.
...
TL;DR: A progressive GAN with weighted synthetic data augmentation and meta-heuristic optimization achieves 95.5-98.5% accuracy for imbalanced COVID-19 chest X-ray classification.


<details>
  <summary>Details</summary>
Motivation: Medical image classification faces severe data imbalance issues, especially during pandemics like COVID-19 where infected cases are rare compared to healthy ones. This imbalance hinders AI/ML methods that require sufficient balanced data for accurate disease detection.

Method: Proposes a progressive generative adversarial network (GAN) to generate synthetic data for minority classes. Uses a weighted approach to combine synthetic and real data before feeding to a deep classifier. Employs multi-objective meta-heuristic population-based optimization to tune classifier hyper-parameters.

Result: Achieves 95.5% accuracy for 4-class imbalanced classification and 98.5% accuracy for 2-class imbalanced classification on a large COVID-19 chest X-ray dataset, outperforming existing methods in cross-validated metrics.

Conclusion: The proposed model effectively addresses medical image classification with imbalanced data during pandemics through synthetic data generation and optimization, demonstrating practical value for rapid disease detection.
...
<details>
  <summary>Details</summary>
Motivation: Existing training-free OVSS methods are either computationally expensive (relying on external foundation models like SAM, DINO) or sub-optimal (using static heuristics on CLIP features). CLIP's image-level representations lack precise pixel-level details needed for segmentation.

Method: Attention Refinement Module (ARM) adaptively fuses hierarchical CLIP features using semantically-guided cross-attention (deep features as K,V to refine shallow features as Q) followed by self-attention. Trained once on general datasets like COCO-Stuff, it serves as a universal plug-and-play post-processor.
...
Conclusion: ARM provides a lightweight, learnable solution that unlocks CLIP's internal potential for segmentation without expensive external models or static heuristics, enabling practical training-free OVSS.
...
<details>
  <summary>Details</summary>
Motivation: Vision-centric autonomous driving systems need diverse training data, but existing video object editing methods struggle to maintain both high visual fidelity and temporal coherence.

Method: Builds on text-to-video diffusion prior for temporal consistency. Addresses 3D encoder spatial fidelity degradation by injecting temporally agnostic latents from pretrained 2D encoder into 3D decoder. Mitigates pose misalignment from distribution mismatch with two-stage data alignment combining coarse 3D alignment and fine 2D refinement.

Result: Extensive experiments demonstrate Mirage achieves high realism and temporal consistency across diverse editing scenarios. Can also generalize to other video-to-video translation tasks.

Conclusion: Mirage provides a reliable baseline for photorealistic and coherent asset editing in driving scenes, with potential applications in data augmentation for autonomous driving systems.
...
TL;DR: MotivNet is a facial emotion recognition model using Meta-Sapiens backbone that achieves cross-domain generalization without cross-domain training, making FER more viable for real-world applications.


<details>
  <summary>Details</summary>
Motivation: Current FER models have weak generalization across diverse datasets, deteriorating performance in real-world applications. Existing solutions require cross-domain training which is contradictory for practical deployment.

Method: Uses Meta-Sapiens (human vision foundational model with Masked Autoencoder pretraining) as backbone. MotivNet is added as downstream task with three evaluation criteria: benchmark performance, model similarity, and data similarity.

Result: MotivNet achieves competitive performance across datasets without cross-domain training, demonstrates generalization across domains, and meets all three evaluation criteria for viability as Sapiens downstream task.

Conclusion: MotivNet validates as a Sapiens downstream task, making facial emotion recognition more incentivizing for in-the-wild applications with robust generalization capabilities.
...
TL;DR: MambaSeg: A dual-branch semantic segmentation framework using parallel Mamba encoders for RGB and event data fusion, with a Dual-Dimensional Interaction Module for spatial and temporal alignment, achieving state-of-the-art performance with reduced computational cost.
...
Conclusion: MambaSeg shows promise for efficient, scalable, and robust multimodal perception by effectively leveraging complementary properties of RGB and event modalities through spatial-temporal fusion with reduced computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: Current deep learning MAR methods have limitations: supervised methods cause spectral blurring from regression-to-the-mean, unsupervised methods risk structural hallucinations, and diffusion models are too slow for clinical use due to iterative sampling.

Method: Three components: 1) AAPS pipeline synthesizes high-fidelity training pairs via Monte Carlo spectral modeling and patient-specific digital twins; 2) DMP-Former adapts direct x-prediction for deterministic manifold projection in single forward pass; 3) SSA module uses medical foundation model priors (MedDINOv3) for semantic-structural alignment.

Result: Outperforms state-of-the-art methods on synthetic and multi-center clinical datasets, particularly on unseen anatomy, setting new benchmarks in efficiency and diagnostic reliability.

Conclusion: PGMP framework effectively addresses limitations of existing MAR methods by combining physics-based simulation, deterministic manifold projection, and medical foundation model guidance, achieving both efficiency and clinical reliability for dental CBCT applications.
...
TL;DR: DualityForge framework synthesizes counterfactual video QA pairs using diffusion-based video editing to reduce hallucinations in multimodal LLMs, achieving 24% improvement over baseline.
...
Method: DualityForge uses controllable diffusion-based video editing to transform real videos into counterfactual scenarios, automatically generating QA pairs. Duality-Normalized Advantage Training (DNA-Train) applies pair-wise ℓ₁ advantage normalization for stable RL optimization.
...
TL;DR: A 3D panoramic stitching framework that lifts images to 3D point clouds for global fusion, then projects to cylindrical panorama, reducing ghosting and distortions in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Traditional 2D homography and mesh warping methods fail in 3D scenes with multiple depth layers and occlusions, causing ghosting, structural bending, and stretching distortions, especially in multi-view and 360° closed-loop scenarios.

Method: 1) Lift each input image to dense 3D point representation in unified coordinate system with confidence metrics; 2) Establish unified 3D projection center; 3) Use equidistant cylindrical projection to map fused data to panoramic manifold; 4) Perform hole filling in canvas domain for unknown regions.
...
Conclusion: The framework successfully shifts stitching from 2D warping to 3D consistency paradigm, enabling flexible incorporation of various 3D lifting and completion modules for improved panoramic quality in complex 3D scenes.
...
TL;DR: EndoRare is a one-shot generative framework that synthesizes diverse, high-fidelity rare gastrointestinal lesion images from a single reference image, using language-guided concept disentanglement to separate diagnostic features from non-diagnostic attributes.


<details>
  <summary>Details</summary>
Motivation: Rare gastrointestinal lesions are infrequently encountered in routine endoscopy, limiting data availability for developing reliable AI models and training novice clinicians. This creates a "rare-disease gap" in both computer-aided diagnostics and clinical education.

Method: EndoRare uses language-guided concept disentanglement to separate pathognomonic lesion features from non-diagnostic attributes. It encodes diagnostic features into a learnable prototype embedding while varying non-diagnostic attributes to ensure diversity, enabling one-shot, retraining-free generation of synthetic lesion images.
...
Conclusion: EndoRare establishes a practical, data-efficient pathway to bridge the rare-disease gap in both computer-aided diagnostics and clinical education by enabling one-shot generation of diverse, high-fidelity lesion exemplars from minimal data.
...
TL;DR: Virtual-Eyes CT preprocessing pipeline improves generalist foundation models for lung cancer screening but degrades specialist models adapted to raw clinical data.
...
TL;DR: UniAct is a two-stage framework that enables humanoid robots to execute diverse multimodal instructions (language, music, trajectories) with sub-500ms latency by integrating a fine-tuned MLLM with a causal streaming pipeline and using FSQ for cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Bridging high-level multimodal perception with whole-body execution in humanoid robotics remains a significant bottleneck. Existing methods struggle to translate heterogeneous instructions into stable, real-time actions, limiting the development of versatile agents with human-level flexibility.
...
Method: CERES implements dual-modal causal intervention: 1) backdoor adjustment for language representation biases using dataset statistics, and 2) front-door adjustment for visual confounders by integrating semantic visual features with geometric depth information guided by causal principles.

Result: CERES achieves state-of-the-art performance on Ego-RVOS benchmarks, demonstrating superior segmentation accuracy and robustness compared to existing methods.

Conclusion: The framework shows the potential of causal reasoning for building more reliable models in egocentric video understanding, effectively addressing both dataset biases and visual confounding factors.
...
Method: Proposes SenseNova-MARS framework with reinforcement learning (BN-GSPO algorithm) to enable VLMs to dynamically integrate image search, text search, and image crop tools for fine-grained visual understanding. Introduces HR-MMSearch benchmark for evaluation.
...
Conclusion: SenseNova-MARS represents significant progress toward agentic VLMs with effective tool-use capabilities, enabling interleaved visual reasoning and tool manipulation. All code, models, and datasets will be released to advance research.
...
TL;DR: LVLDrive enhances Vision-Language Models for autonomous driving by incorporating LiDAR point clouds to provide robust 3D metric spatial understanding, addressing limitations of 2D image-based methods.
...
Method: Proposes LVLDrive framework that upgrades VLMs with LiDAR point cloud input. Uses Gradual Fusion Q-Former to incrementally inject LiDAR features without disrupting pre-trained VLM knowledge. Develops spatial-aware question-answering (SA-QA) dataset to teach advanced 3D perception and reasoning.
...
Conclusion: Explicit 3D metric data (LiDAR) is necessary for building trustworthy VLM-based autonomous systems, and LVLDrive successfully bridges the gap between language understanding and robust 3D spatial reasoning for safer autonomous driving.
...
<details>
  <summary>Details</summary>
Motivation: To establish a theoretical foundation connecting information processing in convolutional neural networks to fundamental physical principles, specifically the energy-momentum relation from relativistic physics, providing new insights into how CNNs process visual information.

Method: Decomposes convolutional kernels into orthogonal even and odd components, analyzes their effects on image content (even: isotropic diffusion preserving center of mass; odd: directional displacement), relates kernel energy ratios to information speed, and examines properties in spectral domain using discrete cosine transform (DCT) focusing on low-frequency bases (DC Σ and gradient components ∇).

Result: Demonstrates that even kernel components behave analogously to rest/potential energy (zero net momentum), odd components behave like kinetic energy (non-zero momentum), and information displacement speed is linearly related to odd vs total kernel energy ratio. Small convolutional filters (3×3) are dominated by low-frequency DCT bases that define fundamental information propagation modes.

Conclusion: This work establishes the first known link between information processing in generic CNNs and the energy-momentum relation from modern relativistic physics, providing a novel "elementary information mechanics" framework for understanding convolutional filtering with rectification.
...
TL;DR: DermaVQA-DAS extends the DermaVQA dataset with two tasks: closed-ended QA and lesion segmentation, using a novel Dermatology Assessment Schema (DAS) framework to capture clinical features systematically.
...
Method: Introduces DAS framework with 36 high-level and 27 fine-grained assessment questions in English/Chinese. Provides expert-annotated datasets for QA and segmentation, benchmarking multimodal models with different prompting strategies.

Result: For segmentation, prompt design impacts performance: default prompt works best under certain evaluation schemes, while augmented prompt yields highest performance (Jaccard 0.395, Dice 0.566). For QA, models achieve 0.729-0.798 accuracy, with o3 (0.798) and GPT-4.1 (0.796) performing best.

Conclusion: DermaVQA-DAS addresses limitations of existing datasets by incorporating patient context and clinical structure, enabling better patient-centered dermatological vision-language modeling. The dataset and framework are publicly released to accelerate research.
...
TL;DR: A comprehensive framework and taxonomy for multi-modal pre-training to achieve Spatial Intelligence from diverse sensors like cameras and LiDAR, addressing integration challenges and proposing a roadmap for general-purpose foundation models.
...
Method: Presents a comprehensive framework for multi-modal pre-training, analyzing core techniques driving progress. Dissects interplay between foundational sensor characteristics and learning strategies, evaluates platform-specific datasets. Formulates unified taxonomy for pre-training paradigms: from single-modality baselines to sophisticated unified frameworks learning holistic representations for tasks like 3D object detection and semantic occupancy prediction. Investigates integration of textual inputs and occupancy representations for open-world perception and planning.

Result: Identifies critical bottlenecks: computational efficiency and model scalability. Proposes a roadmap toward general-purpose multi-modal foundation models capable of achieving robust Spatial Intelligence for real-world deployment.

Conclusion: The paper provides a systematic framework and taxonomy for advancing multi-modal pre-training to achieve Spatial Intelligence, addressing current challenges and outlining a path forward for developing general-purpose foundation models suitable for real-world autonomous systems.
...
TL;DR: RedunCut is a dynamic model size selection system for live video analytics that reduces compute costs by 14-62% while maintaining accuracy, addressing limitations of prior methods that fail on mobile videos and lower accuracy targets.


<details>
  <summary>Details</summary>
Motivation: Live video analytics faces high inference costs with modern vision models across massive camera fleets. Dynamic model size selection (DMSS) can reduce costs up to 10x without model retraining, but prior systems fail to generalize to diverse workloads, especially mobile videos and lower accuracy targets, due to inefficient sampling and inaccurate accuracy prediction.

Method: RedunCut introduces two key innovations: (1) a measurement-driven planner that estimates the cost-benefit tradeoff of sampling to avoid inefficient sampling, and (2) a lightweight, data-driven performance model to improve per-segment accuracy prediction.
...
Conclusion: RedunCut successfully addresses the limitations of prior DMSS methods by optimizing sampling efficiency and improving accuracy prediction, making dynamic model size selection practical for diverse live video analytics workloads.
...
TL;DR: DyStream: A flow matching-based autoregressive model for real-time dyadic talking head video generation with ultra-low latency (<100ms) using causal encoding with short lookahead.
...
Result: Generates video within 34ms per frame, keeping total system latency under 100ms. Achieves state-of-the-art lip-sync quality with offline/online LipSync Confidence scores of 8.13 and 7.61 on HDTF dataset. Outperforms alternative causal strategies like distillation and generative encoder.

Conclusion: DyStream enables real-time dyadic talking head video generation with ultra-low latency, making realistic listener feedback possible. The simple-yet-effective approach combines autoregressive flow matching with causal encoding and minimal lookahead, achieving both speed and quality.
...
TL;DR: AI-driven framework for automated assessment of microanastomosis surgical skill using video transformers and motion analysis, achieving high accuracy in action recognition and skill classification.


<details>
  <summary>Details</summary>
Motivation: Conventional surgical skill assessment methods are subjective, time-consuming, require expert supervision, and are difficult to scale, especially in low-resource settings. There's a need for objective, consistent, and scalable evaluation methods.

Method: Proposes an AI framework using TimeSformer-based video transformer with hierarchical temporal attention and weighted spatial attention for action recognition. Combines YOLO-based object detection and tracking for fine-grained motion feature extraction. Evaluates five aspects of microanastomosis skill including action execution, motion quality, and instrument handling.

Result: Achieved 87.7% frame-level accuracy in action segmentation (increased to 93.62% with post-processing) and 76% average classification accuracy in replicating expert assessments across all skill aspects on a dataset of 58 expert-annotated videos.

Conclusion: The system demonstrates potential for providing objective, consistent, and interpretable feedback for surgical skill assessment, enabling more standardized and data-driven surgical education and training.
...
TL;DR: This paper investigates compositionality in Vision Transformer (ViT) encoders using Discrete Wavelet Transform (DWT) to obtain input-dependent primitives and tests how well composed representations reproduce original image representations.
...
Method: The authors introduce a framework analogous to prior work on measuring compositionality in representation learning, using Discrete Wavelet Transform (DWT) to obtain input-dependent primitives in vision. They examine the ability of composed representations to reproduce original image representations to test compositionality in the representation space.

Result: The findings show that primitives from a one-level DWT decomposition produce encoder representations that approximately compose in latent space, demonstrating that ViTs respect compositionality to some extent in their learned representations.

Conclusion: The work offers a new perspective on how Vision Transformers structure information, showing that they learn representations that exhibit compositional properties similar to those observed in language models, with DWT serving as an effective tool for obtaining vision primitives.
...
TL;DR: A learned image compression framework for multispectral solar imagery using graph-based modules to model spectral relationships and spatial redundancy, achieving better spectral fidelity and reconstruction quality than baselines.
...
Conclusion: The proposed learned compression framework effectively addresses the challenge of compressing multispectral solar imagery by leveraging graph-based modeling of spectral relationships and spatial attention mechanisms, achieving superior spectral fidelity and reconstruction quality for space mission applications.
...
TL;DR: The paper introduces a Feature-to-Image Diffusion (F2IDiff) Foundation Model for Single Image Super-Resolution that uses lower-level DINOv2 features instead of text features to provide stricter, hallucination-free generation suitable for smartphone photography.


<details>
  <summary>Details</summary>
Motivation: Current Text-to-Image Diffusion models for SISR cause undesirable hallucinations in smartphone photography where LR images have high fidelity. Text features are too high-level to describe subtle textures, and smartphone LR images (12MP+) are much larger than what T2IDiff models handle (<1MP), requiring patch-based processing where text features fail.
...
Result: Not specified in the abstract, but the method addresses the shortcomings of T2IDiff models for smartphone SISR by enabling hallucination-free generation with minimal artifacts for high-fidelity LR images.

Conclusion: Lower-level feature conditioning (DINOv2) provides better control over generation in SISR for smartphone photography compared to text features, enabling minimal hallucination-free super-resolution for high-fidelity LR images.
...
TL;DR: AI pipeline combines YOLO object detection models with GPT-4 to generate radiology reports from chest X-ray images, achieving strong clinical accuracy but with stylistic differences from human radiologist reports.


<details>
  <summary>Details</summary>
Motivation: Current AI systems in medical imaging only provide structured predictions, requiring radiologists to manually translate these into narrative reports. This creates a gap between AI detection capabilities and clinical reporting workflow.
...
Result: YOLO models showed good detection accuracy, and GPT-4 generated reports with strong semantic similarity to ground-truth reports (measured by cosine similarity). Human evaluation gave GPT-4 high clarity scores (4.88/5) but lower natural writing flow scores (2.81/5).

Conclusion: The AI pipeline achieves clinical accuracy in report generation but produces text that is stylistically distinguishable from radiologist-authored reports, indicating current systems can assist with clinical tasks but still lack the natural writing style of human experts.
...
<details>
  <summary>Details</summary>
Motivation: Traditional video codecs (H.264/HEVC) are designed for pixel-domain reconstruction and lack native support for machine learning latent representations, limiting integration into deep learning pipelines. There's a need for efficient compression suitable for bandwidth-sensitive scenarios like CDNs and edge devices.

Method: Multi-Scale Vector Quantized Variational Autoencoder (MS-VQ-VAE) extending VQ-VAE-2 to spatiotemporal setting with two-level hierarchical latent structure using 3D residual convolutions. Incorporates perceptual loss from pre-trained VGG16. Lightweight (18.5M parameters) optimized for 64x64 video clips (32 frames at 16 FPS).

Result: Trained on UCF101 dataset: 25.96 dB PSNR and 0.8375 SSIM on test set. Outperforms single-scale baseline by 1.41 dB PSNR and 0.0248 SSIM on validation. Model is lightweight and suitable for edge deployment.

Conclusion: The framework is well-suited for scalable video compression in bandwidth-sensitive scenarios including real-time streaming, mobile video analytics, and CDN-level storage optimization, bridging traditional compression with deep learning pipelines.
...
TL;DR: PhyGDPO: A physics-aware video generation framework using groupwise preference optimization with VLM-based physics rewards to improve physical consistency in text-to-video synthesis.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video generation methods produce good visual quality but struggle to follow physical laws, with existing approaches limited to simple simulated environments or implicit physical reasoning. The scarcity of training data with rich physics interactions is also a major limitation.

Method: 1) PhyAugPipe: A physics-augmented video data construction pipeline using vision-language models with chain-of-thought reasoning to create PhyVidGen-135K dataset. 2) PhyGDPO: Physics-aware Groupwise Direct Preference Optimization framework based on Plackett-Luce model for holistic preferences. 3) Physics-Guided Rewarding (PGR): Embeds VLM-based physics rewards to steer optimization toward physical consistency. 4) LoRA-Switch Reference (LoRA-SR): Eliminates memory-heavy reference duplication for efficient training.

Result: The method significantly outperforms state-of-the-art open-source methods on PhyGenBench and VideoPhy2 benchmarks, demonstrating improved physical consistency in generated videos.

Conclusion: PhyGDPO provides an effective framework for physics-aware video generation through groupwise preference optimization and physics-guided rewards, addressing the challenge of physical consistency in text-to-video synthesis.
...
TL;DR: A novel second-order optimization algorithm for deep learning that combines OCP method with Hessian diagonal approximation, achieving faster convergence and better robustness on visual localization tasks.


<details>
  <summary>Details</summary>
Motivation: Address large-scale optimization problems in deep learning by developing efficient second-order methods that can handle the computational challenges while maintaining performance.

Method: Proposes a second-order optimization algorithm that incorporates the OCP method and appropriately approximates the diagonal elements of the Hessian matrix to reduce computational complexity.
...
Conclusion: The proposed second-order optimization framework effectively addresses large-scale optimization challenges in deep learning, offering a practical solution that balances computational efficiency with superior performance on visual localization tasks.
...
TL;DR: RGBT-Ground: First large-scale RGB-Thermal visual grounding benchmark for complex real-world scenarios, with RGBT-VGNet baseline outperforming existing methods.
...
Result: RGBT-VGNet significantly outperforms adapted existing methods, particularly in challenging nighttime and long-distance scenarios. Benchmark enables comprehensive evaluation of robust grounding under diverse real-world conditions.

Conclusion: RGBT-Ground addresses limitations of existing VG benchmarks by introducing real-world complexity through RGB-Thermal data. The proposed RGBT-VGNet demonstrates effectiveness of multi-modal fusion for robust visual grounding, especially in challenging conditions. All resources will be publicly released to advance research in this area.
...
Method: Proposes DARFT: Decision-Ambiguity-guided Reinforcement Fine-Tuning. First mines Decision-Ambiguous Samples (DAS) using SFT-trained reference policy, then applies group-relative policy optimization on the mined subset using multi-sample decoding and intra-group relative advantages.
...
Conclusion: Explicitly optimizing decision-ambiguous samples is crucial for CDVQA improvement. DARFT effectively suppresses strong distractors and sharpens decision boundaries without additional supervision.
...
TL;DR: SliceLens: A hypothesis-driven framework using LLMs/VLMs for fine-grained error slice discovery in instance-level vision tasks, with new FeSD benchmark showing SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing slice discovery methods are limited to image classification and struggle with multi-instance tasks (detection, segmentation, pose estimation). Current benchmarks are algorithm-specific or biased toward classification, with artificial ground truth that doesn't reflect real model failures.
...
Conclusion: SliceLens addresses limitations of existing slice discovery methods by enabling fine-grained error analysis for instance-level vision tasks through LLM/VLM-powered hypothesis generation and verification, with strong performance demonstrated on both existing benchmarks and the new FeSD benchmark.
...
TL;DR: This paper addresses the lack of 3D semantic segmentation datasets for post-disaster environments by creating a specialized UAV-captured dataset from Hurricane Ian and evaluating SOTA models, revealing their limitations for disaster scenarios.


<details>
  <summary>Details</summary>
Motivation: Natural disasters cause severe human and economic losses, but existing 3D semantic segmentation models lack datasets specifically designed for post-disaster environments, hindering effective post-disaster assessment and response.
...
Result: Evaluation exposed significant limitations in existing 3D semantic segmentation methods for disaster-stricken regions, demonstrating that current SOTA models perform poorly on post-disaster environments.
...
<details>
  <summary>Details</summary>
Motivation: Existing LoRA methods either sacrifice performance for parameter efficiency or introduce excessive parameters, failing to balance learning performance and parameter efficiency effectively.

Method: CLoRA uses base-space sharing where all low-rank modules share down/up-projection spaces, and employs sample-agnostic diversity enhancement (SADE) to regularize similarities among representations to encourage diversity.
...
Conclusion: CLoRA effectively addresses the trade-off between performance and parameter efficiency in vision transformer fine-tuning through collaborative low-rank adaptation with diversity regularization.
...
TL;DR: Introduces MoniRefer, the first large-scale 3D visual grounding dataset for roadside monitoring scenarios, and proposes Moni3DVG method for multi-modal feature learning.
...
Result: Created MoniRefer dataset with ~136,018 objects and 411,128 natural language expressions from real-world traffic intersections. Extensive experiments show superiority and effectiveness of proposed method.

Conclusion: This work introduces 3D visual grounding for outdoor monitoring scenarios, provides first large-scale dataset for this task, and demonstrates effective multi-modal approach for infrastructure-level traffic scene understanding.
...
TL;DR: Layer-by-Layer Hierarchical Attention Network improves feature point matching by addressing outlier issues through stage fusion, hierarchical extraction, and attention mechanisms.
...
Conclusion: The proposed network effectively addresses outlier issues in feature point matching through hierarchical attention mechanisms and stage fusion, improving both matching accuracy and robustness.
...
<details>
  <summary>Details</summary>
Motivation: Current fire detection research focuses on natural environments (mountains/forests) but neglects complex urban rescue scenes, and existing systems have limited target classes (only flames/smoke) missing crucial command decision targets like fire trucks and firefighters.

Method: 1) Constructs FireRescue dataset covering urban, mountainous, forest, and water areas with 8 key categories (15,980 images, 32,000 bounding boxes). 2) Proposes FRS-YOLO model with: a) plug-and-play multidimensional collaborative enhancement attention module for cross-dimensional feature interaction to reduce inter-class confusion, and b) dynamic feature sampler to strengthen high-response foreground features against smoke occlusion and background interference.

Result: Experimental results show fire rescue object detection is highly challenging, and the proposed FRS-YOLO method effectively improves detection performance of YOLO series models in this context.

Conclusion: The FireRescue dataset and FRS-YOLO model address critical gaps in fire rescue object detection by covering diverse scenarios and key command-relevant targets, with improved performance for challenging rescue environments.
...
TL;DR: RadAR: A parallelizable framework for accelerating autoregressive visual generation using radial topology and nested attention to improve inference efficiency while preserving quality.
...
Method: Organizes generation around radial topology: selects initial token as starting point, groups other tokens into concentric rings by spatial distance from center. Generation proceeds ring-wise (inner to outer), enabling parallel prediction of all tokens within same ring. Introduces nested attention mechanism to dynamically refine implausible outputs during forward pass, mitigating error accumulation.

Result: Significantly improves generation efficiency through radial parallel prediction while preserving structural locality and spatial coherence of visual scenes. The framework maintains representational capacity of autoregressive models while enabling substantial parallelization.

Conclusion: RadAR successfully addresses the inference efficiency bottleneck in autoregressive visual generation by exploiting spatial correlations through radial topology and preventing model collapse with nested attention, making autoregressive approaches more practical for visual generation tasks.
...
TL;DR: RGTN is a physics-inspired tensor network structure search framework that uses renormalization group flows for multi-scale optimization, achieving state-of-the-art compression and 4-600× speedup over existing methods.
...
Conclusion: EVOL-SAM3 demonstrates that evolutionary search at inference time can overcome limitations of current reasoning segmentation approaches, achieving superior performance without training while enabling self-correction and deeper reasoning through iterative refinement.
...
TL;DR: CPJ is a training-free few-shot framework that improves agricultural pest VQA using structured captions and LLM-as-Judge refinement, achieving significant performance gains without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing crop disease diagnosis methods require costly supervised fine-tuning and perform poorly under domain shifts, lacking interpretability and robustness.

Method: Caption-Prompt-Judge (CPJ) framework: generates multi-angle image captions using vision-language models, refines them iteratively via LLM-as-Judge module, then uses these captions for dual-answer VQA (recognition + management responses).

Result: On CDDMBench, CPJ improves disease classification by +22.7 percentage points and QA score by +19.5 points over no-caption baselines when using GPT-5-mini captions with GPT-5-Nano.

Conclusion: CPJ advances robust, explainable agricultural diagnosis without fine-tuning, providing transparent evidence-based reasoning through structured captions and iterative refinement.
...
TL;DR: FlowBlending is a stage-aware multi-model sampling strategy that uses large models for capacity-sensitive stages and small models for intermediate stages, achieving faster inference with fewer FLOPs while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is that model capacity impact varies across timesteps in diffusion models - crucial for early and late stages but negligible during intermediate stages, suggesting opportunities for optimization by using different model sizes at different stages.

Method: FlowBlending uses a large model and a small model at capacity-sensitive stages (early and late) and intermediate stages respectively. It includes criteria for choosing stage boundaries and uses velocity-divergence analysis to identify capacity-sensitive regions.
...
Conclusion: FlowBlending effectively optimizes diffusion model inference by strategically deploying different model capacities at different sampling stages, achieving significant speedups without quality degradation.
...
TL;DR: EchoFoley introduces a new video-grounded sound generation task with fine-grained control, addressing limitations in current VT2A models through symbolic event representation and a curated 6k benchmark, achieving significant improvements in controllability and quality.
...
Method: Introduces EchoFoley task with symbolic representation for sounding events specifying when, what, and how sounds are produced. Creates EchoFoley-6k benchmark with 6,000+ expert-curated video-instruction-annotation triplets. Proposes EchoVidia framework with sounding-event-centric agentic generation using slow-fast thinking strategy.
...
Conclusion: The EchoFoley framework successfully addresses limitations in current VT2A models by providing fine-grained control over sound generation through symbolic event representation and a comprehensive benchmark, enabling more precise and controllable video sound effects generation.
...
Result: The authors introduce Splatwizard as a solution to the benchmarking gap, making the code available at https://github.com/splatwizard/splatwizard for community use.

Conclusion: Splatwizard addresses the critical need for standardized evaluation tools in 3DGS compression research by providing a comprehensive benchmarking framework that enables fair comparison and assessment of different compression methods across multiple dimensions of performance.
...
TL;DR: Unified framework for 3D instance segmentation that combines feature learning and label decoding in one stage, with boundary-aware hard mining for improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing 3D instance segmentation methods suffer from inconsistency in 2D instance labels across views, leading to poor 3D predictions. Current approaches use two-stage processes with contrastive learning or label preprocessing, which are inefficient and hyperparameter-sensitive.
...
Conclusion: The unified framework effectively integrates feature learning and label decoding, while boundary-aware hard mining with stabilization addresses object boundary issues, resulting in superior 3D instance segmentation performance.
...
TL;DR: A projection-based adversarial attack method using physics-in-the-loop optimization successfully creates adversarial examples that cause depth misestimation in DNN-based monocular depth estimation models, making objects disappear from scenes.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are vulnerable to adversarial attacks, and this vulnerability extends to DNN-based monocular depth estimation models, threatening their reliability in practical applications. There's a critical need to enhance robustness in these systems.

Method: The study proposes a projection-based adversarial attack method that projects perturbation light onto target objects. It uses physics-in-the-loop optimization to evaluate candidate solutions in actual environments, accounting for device specifications and disturbances, and employs a distributed covariance matrix adaptation evolution strategy.

Result: Experiments confirmed that the proposed method successfully created adversarial examples that lead to depth misestimations, resulting in parts of objects disappearing from the target scene.

Conclusion: The study validates the vulnerability of DNN-based monocular depth estimation models to adversarial attacks and demonstrates the effectiveness of the proposed projection-based attack method with physics-in-the-loop optimization for creating realistic adversarial examples.
...
TL;DR: The paper presents a method to apply nonlinear functions to noisy targets in Noise2Noise training without significant bias, enabling better denoising of HDR images from Monte Carlo rendering.


<details>
  <summary>Details</summary>
Motivation: Noise2Noise training requires noisy input-target pairs but cannot handle nonlinear preprocessing of targets due to bias. This limits preprocessing options, especially for HDR images where nonlinear tone mapping is needed to handle outliers.

Method: Develop theoretical framework to analyze nonlinear function effects on noisy targets. Identify class of nonlinear functions with minimal bias. Apply specific combinations of loss functions and tone mapping functions to reduce outliers while maintaining compatibility with Noise2Noise training.

Result: Method enables effective denoising of HDR images from Monte Carlo rendering using only noisy training data. Results approach those of original implementation trained with high-sample count reference images.

Conclusion: Certain nonlinear functions can be applied to noisy targets in Noise2Noise training without significant bias, expanding preprocessing options and enabling better handling of challenging cases like HDR image denoising.
...
TL;DR: A method that enables 2D-trained cross-modal systems to adapt to 3D scenes using in-scene camera control via improved mutual information estimation and derivative-free optimization.
...
Method: Introduces a new method that improves multivariate mutual information estimates through regret minimization with derivative-free optimization, enabling off-the-shelf 2D-trained systems to adapt online to object occlusions and differentiate features.
...
Conclusion: The pairing of expressive measures and value-based optimization allows effective control of in-scene cameras to learn directly from noisy vision-language model outputs, enabling 2D-trained systems to work effectively in 3D environments.
...
Method: CropTrack integrates: 1) reranking-enhanced appearance association, 2) one-to-many association with appearance-based conflict resolution, and 3) an exponential moving average prototype feature bank to improve appearance-based association.
...
Conclusion: The proposed CropTrack framework effectively addresses agricultural MOT challenges by combining appearance and motion information, demonstrating superior identity preservation compared to existing methods.
...
TL;DR: MLLMs show poor context awareness and 3D spatial reasoning in embodied navigation tasks despite their general vision-language capabilities, as revealed by the VLN-MME benchmark framework.
...
Result: Enhancing baseline agents with Chain-of-Thought reasoning and self-reflection unexpectedly decreases performance, revealing MLLMs' poor context awareness in embodied navigation. While MLLMs can follow instructions and structure output, their 3D spatial reasoning fidelity is low.

Conclusion: VLN-MME establishes groundwork for systematic evaluation of general-purpose MLLMs in embodied navigation and reveals limitations in their sequential decision-making capabilities, offering crucial guidance for MLLM post-training as embodied agents.
...
<details>
  <summary>Details</summary>
Motivation: While SAM2 shows promise for medical image segmentation, it requires extensive annotated data and high-quality manual prompts, which are labor-intensive and require medical expert intervention. There's a need for a more label-efficient approach.

Method: OFL-SAM2 introduces a prompt-free framework with: (1) an online few-shot learner that trains a lightweight mapping network using limited data to generate target features, and (2) an adaptive fusion module that dynamically integrates these target features with SAM2's memory-attention features. The mapping network supports online parameter updates during inference for better generalization.
...
Conclusion: OFL-SAM2 successfully addresses the challenges of adapting SAM2 to medical image segmentation by eliminating the need for manual prompts and extensive annotations, offering a label-efficient solution with strong generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing benchmarks by creating a more realistic evaluation framework for multimodal large language models in financial contexts, where real-world reasoning involves complex scenarios, extensive documents, and multi-step computations.

Method: Created a benchmark with 1,200 expert-annotated problems incorporating 12 types of implicit financial scenarios, 837 bilingual documents across 9 document types averaging 50.8 pages, and problems requiring an average of 11-step reasoning (5.3 extraction + 5.7 calculation steps).

Result: The best-performing MLLM achieved only 58.0% accuracy, showing significant room for improvement. Different RAG methods demonstrated substantial performance variations, highlighting the benchmark's ability to reveal model limitations.

Conclusion: FinMMDocR provides a comprehensive benchmark that exposes current MLLM limitations in real-world financial reasoning and is expected to drive improvements in multimodal reasoning capabilities for complex real-world scenarios.
...
Result: The method outperforms both linear probing and state-of-the-art domain adaptation techniques while requiring very small annotation budgets, demonstrating effective cross-domain generalization for 3D object detection.

Conclusion: Neuron activation patterns provide an effective basis for selecting informative target domain samples, enabling efficient lidar domain adaptation with minimal annotation costs while maintaining model performance across geographic domains.
...
TL;DR: HaineiFRDM is a film restoration diffusion model that enables high-resolution film restoration on limited GPU hardware using patch-wise training, global-local fusion modules, and a novel dataset with real-degraded films.
...
Method: Proposes HaineiFRDM framework with: 1) Patch-wise training/testing for high-resolution processing on 24GB VRAM GPU, 2) Position-aware Global Prompt and Frame Fusion Modules, 3) Global-local frequency module for consistent texture reconstruction, 4) Two-stage restoration (low-res as global residual to reduce blocky artifacts), 5) New dataset with real-degraded films and realistic synthetic data.

Result: Comprehensive experiments demonstrate superior defect restoration ability over existing open-source methods. The model effectively handles high-resolution film restoration with reduced artifacts.

Conclusion: HaineiFRDM successfully addresses limitations of current open-source film restoration methods by leveraging diffusion models' content understanding, enabling high-resolution restoration on accessible hardware, and providing a new realistic dataset. Code and dataset will be released to advance open-source film restoration research.
...
<details>
  <summary>Details</summary>
Motivation: CAC scoring from chest CT is important for cardiovascular risk assessment but is affected by motion artifacts in non-gated CT scans. While ECG-gated cardiac CTs reduce motion, they have limited use in routine imaging due to gating requirements and insurance coverage issues. Incidental CAC from non-gated CT offers an accessible alternative but suffers from severe motion artifacts.

Method: ProDM (Property-aware Progressive Correction Diffusion Model) has three key components: 1) A CAC motion simulation data engine that synthesizes realistic non-gated acquisitions from cardiac-gated CTs for supervised training without paired data; 2) A property-aware learning strategy with calcium-specific priors using a differentiable calcium consistency loss; 3) A progressive correction scheme that gradually reduces artifacts across diffusion steps for enhanced stability and calcium fidelity.

Result: Experiments on real patient datasets show ProDM significantly improves CAC scoring accuracy, spatial lesion fidelity, and risk stratification performance compared to baselines. A reader study on real non-gated scans confirms that ProDM suppresses motion artifacts and improves clinical usability.

Conclusion: ProDM demonstrates the potential of progressive, property-aware frameworks for reliable CAC quantification from routine chest CT imaging, offering a practical solution to motion artifact problems in non-gated CT scans.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation frameworks for video generation models rely on single-frame assessments, which can lead to outcome-hacking where models reach correct conclusions through erroneous processes, failing to capture true reasoning capabilities.

Method: Proposes VIPER benchmark with 16 tasks across 6 reasoning domains (temporal, structural, symbolic, spatial, physics, planning) and introduces Process-outcome Consistency (POC@r) metric using VLM-as-Judge with hierarchical rubric to evaluate both intermediate steps and final results.

Result: State-of-the-art video models achieve only about 20% POC@1.0, exhibit significant outcome-hacking, and show substantial gaps between current video generation and true generalized visual reasoning, even with test-time scaling and sampling improvements.

Conclusion: Current video generation models lack genuine reasoning capabilities despite correct final answers, highlighting the need for process-aware evaluation; VIPER benchmark provides comprehensive assessment framework for advancing Generative Video Reasoning.
...
TL;DR: ShowUI-π is a flow-based generative model for GUI agents that enables both discrete clicks and continuous drag actions, addressing limitations of existing agents that can't handle smooth trajectory-based interactions like dragging progress bars.
...
Method: 1) Unified Discrete-Continuous Actions model integrating both click and drag actions; 2) Flow-based Action Generation using a lightweight action expert to predict incremental cursor adjustments from continuous visual observations; 3) Created ScreenDrag benchmark with 20K manually collected drag trajectories across five domains (PowerPoint, Adobe Premiere Pro, etc.) for training and evaluation.

Result: ShowUI-π achieves 26.98 score on ScreenDrag benchmark with only 450M parameters, significantly outperforming proprietary GUI agents (Operator: 13.27, Gemini-2.5-CUA: 22.18). This demonstrates both the difficulty of the drag task and the effectiveness of the proposed approach.

Conclusion: The work advances GUI agents toward human-like dexterous control in digital environments by enabling continuous trajectory-based interactions through a unified flow-based generative model. The ScreenDrag benchmark provides comprehensive evaluation protocols for assessing drag capabilities.
...
TL;DR: Compression techniques (quantization, pruning, clustering) can preserve or even improve model robustness to natural corruption, with best results from customized combinations on complex architectures.


<details>
  <summary>Details</summary>
Motivation: Compressed deep learning models are needed for resource-constrained devices, but compression may affect robustness to natural corruption, requiring evaluation of robustness alongside compression.

Method: Comprehensive evaluation of quantization, pruning, and weight clustering applied individually and in combination to ResNet-50, VGG-19, and MobileNetV2 using CIFAR-10-C and CIFAR-100-C datasets with multiobjective assessment.

Result: Certain compression strategies preserve or improve robustness, especially on networks with complex architectures. Customized technique combinations produce beneficial multi-objective results for robustness, accuracy, and compression ratio.

Conclusion: Customized compression method combinations can enable robust and efficient model deployment in corrupted real-world environments, providing insights for selecting appropriate compression techniques.
...
TL;DR: DarkEQA is a new benchmark that evaluates Vision Language Models' performance on embodied question answering tasks under realistic low-light conditions, revealing significant limitations in current models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks evaluate VLMs under ideal lighting conditions, but real-world embodied agents need to operate 24/7 in various lighting conditions including low-light environments. Current evaluation overlooks this critical robustness requirement.

Method: DarkEQA creates an open-source benchmark with physics-based low-light simulations in linear RAW space, modeling illumination drop and sensor noise followed by ISP-inspired rendering. It isolates perception bottlenecks by evaluating question answering from egocentric observations under controlled degradations.

Result: Evaluation of state-of-the-art VLMs and Low-Light Image Enhancement models reveals systematic limitations in their performance under challenging low-light conditions.

Conclusion: DarkEQA addresses a critical gap in VLM evaluation for embodied agents, providing a physically faithful benchmark that reveals robustness issues in current models and enables attributable analysis of perception bottlenecks in low-light conditions.
...
TL;DR: RFL-ReID enables lifelong person re-identification without re-indexing historical gallery images, addressing privacy concerns and computational costs through bidirectional compatible feature updates.


<details>
  <summary>Details</summary>
Motivation: Traditional lifelong person Re-ID requires re-indexing historical gallery images after each model update, which raises privacy concerns and incurs high computational costs for large-scale galleries. This creates feature incompatibility between query features from updated models and gallery features from older models.

Method: Proposes Bi-C2R (Bidirectional Continuous Compatible Representation) framework that continuously updates gallery features extracted by old models to maintain compatibility with new model features, enabling lifelong learning without re-indexing.

Result: The method achieves leading performance on both the new RFL-ReID task and traditional L-ReID benchmarks through theoretical analysis and extensive experiments on multiple datasets.

Conclusion: RFL-ReID addresses practical limitations of traditional lifelong Re-ID by eliminating re-indexing requirements while maintaining performance through bidirectional compatible feature learning, making lifelong person re-identification more practical for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Previous flow-based SLAM approaches lack geometric consistency, leading to inaccurate tracking and mapping. The paper aims to bridge flow estimation with geometric reasoning to achieve more accurate and robust SLAM performance.

Method: 1) Hybrid Flow Network that produces geometry-aware correspondences for consistent depth and pose inference. 2) Bi-Consistent Bundle Adjustment Layer that jointly optimizes keyframe pose and depth under multi-view constraints. 3) Reliability-Aware Refinement mechanism that dynamically adapts flow updates by distinguishing reliable vs uncertain regions.
...
Conclusion: The integration of foundation depth models with flow-based SLAM enables geometric consistency, resulting in a robust, accurate, and practical monocular dense SLAM system with real-time performance and good generalization.
...
TL;DR: A self-bootstrapping framework transforms visual dubbing from ill-posed inpainting to well-conditioned video editing using Diffusion Transformers to generate ideal training data and train an audio-driven editor.
...
Method: Proposes a self-bootstrapping framework: 1) Uses a Diffusion Transformer as data generator to synthesize ideal training data (lip-altered companion videos for each real sample), 2) Trains a DiT-based audio-driven editor end-to-end on these aligned video pairs, 3) Introduces timestep-adaptive multi-phase learning to disentangle conflicting editing objectives across diffusion timesteps, 4) Creates ContextDubBench benchmark dataset for robust evaluation.
...
Conclusion: Reframing visual dubbing from ill-posed inpainting to well-conditioned video-to-video editing enables superior performance in lip synchronization, identity preservation, and robustness, with the proposed framework and training strategy effectively addressing fundamental challenges in the field.
...
TL;DR: FineTec is a unified framework for fine-grained action recognition from temporally corrupted skeleton sequences, using context-aware completion, spatial decomposition, physics-driven acceleration estimation, and GCN-based recognition.
...
Method: FineTec first restores base skeleton sequences using context-aware completion with diverse temporal masking. Then a spatial decomposition module partitions skeletons into five semantic regions, divides them into dynamic/static subgroups, and generates augmented sequences. A physics-driven estimation module uses Lagrangian dynamics to estimate joint accelerations. Finally, fused skeleton position and acceleration sequences are processed by a GCN-based recognition head.

Result: FineTec significantly outperforms previous methods on both coarse-grained (NTU-60, NTU-120) and fine-grained (Gym99, Gym288) benchmarks under various corruption levels. Achieves 89.1% and 78.1% top-1 accuracy on Gym99-severe and Gym288-severe settings.

Conclusion: FineTec demonstrates robustness and generalizability for fine-grained action recognition under temporal corruption by effectively recovering temporal dynamics and preserving subtle spatial structures through its unified framework combining completion, decomposition, physics modeling, and GCN recognition.
...
TL;DR: Edit3r is a feed-forward framework that reconstructs and edits 3D scenes in a single pass from unposed, view-inconsistent, instruction-edited images, enabling fast photorealistic rendering without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Prior 3D editing methods require per-scene optimization and pose estimation, which is slow and computationally expensive. There's a need for fast, feed-forward 3D editing that can handle unposed, view-inconsistent images edited by 2D methods.

Method: Edit3r uses: (1) SAM2-based recoloring strategy for generating cross-view-consistent supervision, (2) asymmetric input strategy pairing recolored reference view with raw auxiliary views to fuse disparate observations, and (3) direct prediction of instruction-aligned 3D edits without optimization.

Result: Edit3r achieves superior semantic alignment and enhanced 3D consistency compared to recent baselines, operates at significantly higher inference speed, and effectively handles images edited by 2D methods despite not being trained on them. Introduces DL3DV-Edit-Bench benchmark for evaluation.

Conclusion: Edit3r enables fast, photorealistic 3D editing without optimization or pose estimation, making it promising for real-time 3D editing applications by directly predicting instruction-aligned 3D edits from unposed, view-inconsistent images.
...
TL;DR: GaMO is a geometry-aware multi-view outpainting framework that addresses sparse-view 3D reconstruction by expanding field of view from existing poses instead of generating new viewpoints, achieving better geometric consistency and faster processing.
...
Result: State-of-the-art reconstruction quality on Replica and ScanNet++ datasets across 3, 6, and 9 input views, outperforming prior methods in PSNR and LPIPS metrics, with 25× speedup over SOTA diffusion-based methods and processing time under 10 minutes.

Conclusion: GaMO demonstrates that multi-view outpainting from existing poses is more effective than generating new viewpoints for sparse-view reconstruction, achieving better geometric consistency, broader coverage, and significantly faster processing.
...
<details>
  <summary>Details</summary>
Motivation: Existing video generation models lack explicit disentangled control over spatial (camera viewpoint) and temporal (motion sequence) dimensions, limiting creative exploration of dynamic scenes.

Method: Introduces animation time-embedding for motion control, temporal-warping training using multi-view datasets, improved camera conditioning, and CamxTime synthetic dataset for full space-time coverage.
...
Conclusion: SpaceTimePilot enables independent control over space and time in video generation, opening new possibilities for creative scene exploration through effective disentanglement mechanisms.
...
### [173] [Quantum Visual Word Sense Disambiguation: Unraveling Ambiguities Through Quantum Inference Model](https://arxiv.org/abs/2512.24687)
*Wenbo Qiao,Peng Zhang,Qinghua Hu*

Main category: quant-ph

TL;DR: This paper proposes Q-VWSD, a quantum-inspired model for visual word sense disambiguation that uses quantum superposition to mitigate semantic biases in glosses, with both quantum and classical implementations showing improved performance over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional visual word sense disambiguation methods suffer from semantic biases in glosses from different sources, leading to biased results. The authors are motivated to address this semantic uncertainty problem using quantum-inspired approaches.

Method: Proposes Q-VWSD (Quantum Inference Model for Unsupervised Visual Word Sense Disambiguation) that encodes multiple glosses into quantum superposition states to mitigate semantic biases. The method includes both a quantum circuit implementation and a heuristic classical version for practical deployment.

Result: The method outperforms state-of-the-art classical methods, particularly showing effectiveness in leveraging non-specialized glosses from large language models, which further enhances performance.

Conclusion: The approach demonstrates the potential of quantum machine learning in practical applications and provides a case for leveraging quantum modeling advantages on classical computers while quantum hardware remains immature.
...
### [174] [One-Shot Structured Pruning of Quantum Neural Networks via $q$-Group Engineering and Quantum Geometric Metrics](https://arxiv.org/abs/2512.24019)
...
Method: Formulates redundancy at gate level using algebraic structure of q-deformed groups and task-conditioned quantum geometry. Uses q-overlap distance to measure functional similarity, removes gates only when replacement induces bounded deviation on task observables. Includes noise-calibrated deformation parameter λ for hardware adaptation.
...
Conclusion: q-iPrune offers a theoretically grounded, efficient pruning framework for QNNs that addresses gate redundancy while maintaining performance and adapting to hardware imperfections, enabling better deployment on NISQ devices.
...
### [175] [PhysTalk: Language-driven Real-time Physics in 3D Gaussian Scenes](https://arxiv.org/abs/2512.24986)
*Luca Collorone,Mert Kiray,Indro Spinelli,Fabio Galasso,Benjamin Busam*

Main category: cs.GR

TL;DR: PhysTalk is a train-free framework that generates real-time, physics-based 4D animations from text prompts using 3D Gaussian Splatting scenes, LLM-generated code, and direct physics simulation without mesh extraction.


<details>
  <summary>Details</summary>
Motivation: Current visual effects generation pipelines lack physical realism, effective language interfaces, and require slow offline optimization. There's a need for accessible, interactive tools that can translate arbitrary user prompts into realistic physics-based animations without expert knowledge or time-consuming rendering.

Method: PhysTalk takes 3D Gaussian Splatting (3DGS) scenes as input, uses a large language model to generate executable code that modifies 3DGS parameters through lightweight proxies and particle dynamics, and directly couples 3DGS with a physics simulator without mesh extraction.

Result: The framework enables interactive, collision-aware, physics-based manipulation of arbitrary multi-material objects in real-time, making 4D animation broadly accessible and shifting from "render and wait" to interactive dialogue.

Conclusion: PhysTalk represents a significant advancement in open-vocabulary visual effects generation by combining 3DGS with physics simulation in a train-free, computationally lightweight framework that enables real-time, interactive 4D animation from text prompts.
...
TL;DR: RAIR is a Chinese benchmark dataset for e-commerce search relevance evaluation that addresses limitations in existing benchmarks by providing standardized rules and comprehensive test subsets including general, long-tail hard, and visual salience cases.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack sufficient complexity for comprehensive model assessment, and there's an absence of standardized relevance evaluation metrics across the e-commerce industry, despite LLMs showing significant results on relevance tasks.

Method: Created RAIR dataset from real-world Chinese e-commerce scenarios with standardized framework and universal rules. The dataset consists of three subsets: general subset with industry-balanced sampling, long-tail hard subset for challenging cases, and visual salience subset for multimodal understanding evaluation.

Result: Experiments with 14 open and closed-source models show RAIR presents sufficient challenges even for GPT-5 (which achieved best performance). The benchmark effectively evaluates fundamental competencies, performance limits on hard cases, and multimodal capabilities.

Conclusion: RAIR serves as an industry benchmark for relevance assessment while providing new insights into general LLM and VLM evaluation, with data now available to standardize e-commerce search relevance evaluation.
...
### [177] [Learning to Feel the Future: DreamTacVLA for Contact-Rich Manipulation](https://arxiv.org/abs/2512.23864)
*Guo Ye,Zexi Zhang,Xu Zhao,Shang Wu,Haoran Lu,Shihan Lu,Han Liu*

Main category: cs.RO

TL;DR: DreamTacVLA enhances Vision-Language-Action models with tactile sensing and contact physics understanding for contact-rich manipulation tasks.
...
Method: Hierarchical perception with tactile images as micro-vision, wrist-camera local vision, and third-person macro vision. Uses Hierarchical Spatial Alignment (HSA) loss to align tactile tokens with spatial counterparts. Fine-tunes with tactile world model predicting future tactile signals. Constructs hybrid dataset from digital twin and real-world experiments.

Result: Outperforms state-of-the-art VLA baselines on contact-rich manipulation tasks, achieving up to 95% success rate.

Conclusion: Understanding physical contact through tactile sensing and contact physics modeling is crucial for robust, touch-aware robotic agents. DreamTacVLA demonstrates the importance of anticipating tactile consequences for successful contact-rich manipulation.
...
### [178] [RANGER: A Monocular Zero-Shot Semantic Navigation Framework through Contextual Adaptation](https://arxiv.org/abs/2512.24212)
*Ming-Ming Yu,Yi Chen,Börje F. Karlsson,Wenjun Wu*

Main category: cs.RO

TL;DR: RANGER is a zero-shot, open-vocabulary semantic navigation framework that uses only monocular camera input, eliminating dependency on depth/pose sensors while enabling in-context learning from short videos.


<details>
  <summary>Details</summary>
Motivation: Current zero-shot object navigation methods rely heavily on precise depth/pose information from simulators, limiting real-world applicability, and lack in-context learning capability to quickly adapt to new environments using short videos.

Method: RANGER integrates keyframe-based 3D reconstruction, semantic point cloud generation, VLM-driven exploration value estimation, adaptive waypoint selection, and low-level action execution, leveraging 3D foundation models for depth-free operation.

Result: Experiments on HM3D benchmark and real-world environments show competitive navigation success rates and exploration efficiency, with superior in-context learning adaptability requiring no prior 3D mapping.

Conclusion: RANGER demonstrates that zero-shot semantic navigation can be achieved without depth/pose sensors while maintaining strong in-context learning capabilities, enabling practical real-world embodied applications.
...
### [179] [Geometric Multi-Session Map Merging with Learned Local Descriptors](https://arxiv.org/abs/2512.24384)
*Yanlong Ma,Nakul S. Joshi,Christa S. Robison,Philip R. Osteen,Brett T. Lopez*

Main category: cs.RO

TL;DR: GMLD is a learning-based local descriptor framework for multi-session point cloud map merging that uses keypoint-aware encoding and geometric transformers for loop closure detection and pose estimation.
...
Method: Uses a keypoint-aware encoder and plane-based geometric transformer to extract discriminative features for loop closure detection and relative pose estimation, with inter-session scan matching cost factors in factor-graph optimization.

Result: Accurate and robust map merging with low error on public datasets and self-collected data from diverse environments, with strong performance in both loop closure detection and relative pose estimation.

Conclusion: GMLD provides an effective learning-based solution for large-scale multi-session point cloud map merging with improved global consistency through integrated optimization.
...
### [180] [Dream2Flow: Bridging Video Generation and Open-World Manipulation with 3D Object Flow](https://arxiv.org/abs/2512.24766)
*Karthik Dharmarajan,Wenlong Huang,Jiajun Wu,Li Fei-Fei,Ruohan Zhang*

Main category: cs.RO

TL;DR: Dream2Flow bridges video generation models to robotic control by using 3D object flow as an intermediate representation, enabling zero-shot manipulation of diverse objects without task-specific demonstrations.


<details>
  <summary>Details</summary>
Motivation: Generative video models can reason about plausible physical interactions but struggle to translate human-led motions into low-level robotic actions. There's a need to bridge the embodiment gap between video generation and robotic control.

Method: Uses 3D object flow as intermediate representation: 1) Generates videos from initial image + task instruction, 2) Reconstructs 3D object motions from generated videos, 3) Formulates manipulation as object trajectory tracking, 4) Converts flow to low-level commands via trajectory optimization or reinforcement learning.

Result: Enables zero-shot guidance from pre-trained video models to manipulate diverse object categories (rigid, articulated, deformable, granular) without task-specific demonstrations. Works in both simulation and real-world experiments.

Conclusion: 3D object flow serves as a general and scalable interface for adapting video generation models to open-world robotic manipulation, overcoming the embodiment gap by separating state changes from actuators.
...
### [181] [Jailbreaking Attacks vs. Content Safety Filters: How Far Are We in the LLM Safety Arms Race?](https://arxiv.org/abs/2512.24044)
*Yuan Xin,Dingfan Chen,Linyi Yang,Michael Backes,Xiao Zhang*

Main category: cs.CR

TL;DR: Previous jailbreak evaluations focused only on LLMs themselves, but this study shows that when considering full deployment pipelines with safety filters, most jailbreak attacks can be detected, suggesting prior success rates were overestimated.


<details>
  <summary>Details</summary>
Motivation: Existing jailbreak evaluations have focused solely on language models, neglecting the complete deployment pipeline that includes safety mechanisms like content moderation filters, creating a gap in understanding the practical effectiveness of jailbreak attacks.

Method: Conducted the first systematic evaluation of jailbreak attacks targeting LLM safety alignment, assessing success across the full inference pipeline including both input and output filtering stages.

Result: Two key findings: 1) Nearly all evaluated jailbreak techniques can be detected by at least one safety filter, suggesting prior assessments overestimated practical success; 2) While safety filters are effective, there's room to better balance recall and precision for optimal protection and user experience.

Conclusion: The study highlights critical gaps in current evaluations and calls for further refinement of detection accuracy and usability in LLM safety systems to better protect against jailbreak attacks in real-world deployments.
...
### [182] [Training-Free Color-Aware Adversarial Diffusion Sanitization for Diffusion Stegomalware Defense at Security Gateways](https://arxiv.org/abs/2512.24499)
*Vladimir Frants,Sos Agaian*

Main category: cs.CR

TL;DR: ADS is a training-free defense that neutralizes hidden payloads in diffusion-based steganography by using a pretrained denoiser as a proxy decoder and color-aware updates, achieving near-zero decoder success with minimal visual distortion.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based generative steganography enables covert communication and malware delivery without detection, creating significant security challenges as these methods bypass traditional detectors by generating images directly from secret data.

Method: ADS uses an off-the-shelf pretrained denoiser as a differentiable proxy for diffusion decoders, incorporates color-aware quaternion-coupled update rules to reduce artifacts, and operates without training under strict distortion limits to neutralize hidden payloads.

Result: ADS reduces decoder success rates to near zero against state-of-the-art diffusion steganography method Pulsar while maintaining minimal perceptual impact, offering better security-utility trade-off than standard content transformations.

Conclusion: ADS provides an effective training-free defense against diffusion-driven steganography, neutralizing hidden payloads rather than detecting them, making it suitable for security gateways to counter covert communication threats.
...
### [183] [Leveraging Machine Learning for Early Detection of Lung Diseases](https://arxiv.org/abs/2512.23757)
...
TL;DR: Deep learning models (CNNs, VGG16, InceptionV3, EfficientNetB0) achieve high accuracy in diagnosing respiratory diseases from chest X-rays, enabling rapid non-invasive screening for resource-limited areas.


<details>
  <summary>Details</summary>
Motivation: To address healthcare disparities by providing rapid, accurate, non-invasive diagnostic solutions for respiratory diseases in areas with limited access to radiologists and healthcare resources, particularly for COVID-19, lung cancer, and pneumonia.

Method: Combined traditional image processing with advanced neural networks, training and validating multiple deep learning models including CNNs, VGG16, InceptionV3, and EfficientNetB0 on chest X-ray images for respiratory disease diagnosis.

Result: Models achieved high accuracy, precision, recall, and F1 scores, demonstrating reliability and potential for real-world diagnostic applications in respiratory disease detection from chest X-rays.

Conclusion: Deep learning models offer a promising predictive and preventive healthcare paradigm for rapid, accurate diagnosis of respiratory diseases, with significant potential to improve patient outcomes in resource-limited settings.
...
### [184] [Targeted Semantic Segmentation of Himalayan Glacial Lakes Using Time-Series SAR: Towards Automated GLOF Early Warning](https://arxiv.org/abs/2512.24117)
*Pawan Adhikari,Satish Raj Regmi,Hari Ram Shrestha*

Main category: eess.IV

TL;DR: Automated deep learning pipeline using Sentinel-1 SAR time-series for targeted monitoring of high-risk Himalayan glacial lakes, achieving 0.9130 IoU with temporal-first training and operational Docker architecture.


<details>
  <summary>Details</summary>
Motivation: Existing GLOF monitoring approaches either prioritize spatial coverage for general models or rely on optical imagery limited by cloud coverage, creating a need for targeted, automated monitoring of high-risk glacial lakes using SAR data.

Method: End-to-end automated deep learning pipeline with "temporal-first" training strategy using U-Net with EfficientNet-B3 backbone on curated dataset of 4 Himalayan lakes (Tsho Rolpa, Chamlang Tsho, Tilicho, Gokyo Lake), plus Dockerized operational architecture with ASF Search API for data ingestion and RESTful endpoint for inference.

Result: Model achieves IoU of 0.9130, validating the success of the temporal-first strategy for transitioning to Early Warning Systems.

Conclusion: The system shifts paradigm from static mapping to dynamic automated early warning, providing scalable architectural foundation for future Early Warning System development.
...
### [185] [Automated Classification of First-Trimester Fetal Heart Views Using Ultrasound-Specific Self-Supervised Learning](https://arxiv.org/abs/2512.24492)
...
<details>
  <summary>Details</summary>
Motivation: Congenital heart disease is a leading cause of neonatal morbidity/mortality. Early detection via first-trimester fetal echocardiography is challenging due to small structures, low signal-to-noise ratio, and operator variability. Automated analysis could improve detection.

Method: USF-MAE (masked autoencoder) pretrained on 370k+ unlabeled ultrasound images across 40+ anatomical regions, then fine-tuned on 6,720 first-trimester fetal echocardiography images to classify 5 categories. Compared against ResNet-18, ResNet-50, and ImageNet-pretrained ViT-B/16 baselines.
...
Conclusion: Self-supervised ultrasound foundation models show promise for automated first-trimester fetal heart view classification, potentially improving early detection of congenital heart disease without requiring extensive preprocessing or manual intervention.
...
Method: Replaces standard KV cache with fixed-size memory and trains a two-pass recurrent compression mechanism. Uses online gradient descent with a forget gate to update compressed memory recursively while learning to retain important contextual information from incoming tokens at test time.

Result: Outperforms strong baselines on language modeling, common-sense reasoning, recall-intensive tasks, and time series. Performance gains increase as sequence length grows, highlighting potential for long-context applications.

Conclusion: Trellis offers an effective solution to Transformer's memory limitations with bounded memory architecture that dynamically compresses KV cache, making it particularly suitable for long-context applications.
...
### [187] [Probing the Limits of Compressive Memory: A Study of Infini-Attention in Small-Scale Pretraining](https://arxiv.org/abs/2512.23862)
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient small language models (SLMs) for low-resource settings by improving their long-context extrapolation capabilities while using limited data and compute resources.
...
Result: Infini-attention models show training stability and outperform baselines in long-context retrieval, achieving up to 31% higher accuracy despite performance degradation at 16,384-token contexts. Balance factor is identified as key to performance.
...
### [188] [OptRot: Mitigating Weight Outliers via Data-Free Rotations for Post-Training Quantization](https://arxiv.org/abs/2512.24124)
*Advait Gadhikar,Riccardo Grazzi,James Hensman*

Main category: cs.LG

TL;DR: OptRot and OptRot+ are rotation-based methods that reduce LLM quantization errors by minimizing weight outliers, outperforming existing approaches for weight quantization and improving activation quantization in W4A8 settings.


<details>
  <summary>Details</summary>
Motivation: Large Language Models have outliers in weights and activations that make quantization difficult. Existing rotation methods help but could be improved with better optimization objectives.

Method: Proposes OptRot (minimizes element-wise fourth power of rotated weights) and OptRot+ (incorporates activation covariance information). Both use fusible rotations with cheap proxy objectives to minimize weight quantization error, focusing on GPTQ quantization.

Result: OptRot outperforms Hadamard rotations and more expensive data-dependent methods (SpinQuant, OSTQuant) for weight quantization. Also improves activation quantization in W4A8 setting. OptRot+ further improves performance with activation covariance data. However, both perform worse in W4A4 setting, revealing weight-activation quantization trade-off.

Conclusion: Learning fusible rotations via principled proxy objectives effectively reduces LLM quantization errors. OptRot provides strong weight quantization, while OptRot+ offers additional gains with activation data. The W4A4 performance drop highlights inherent trade-offs between weight and activation quantization.
...
### [189] [More Than Bits: Multi-Envelope Double Binary Factorization for Extreme Quantization](https://arxiv.org/abs/2512.24545)
*Yuma Ichikawa,Yoshihiko Fujisawa,Yudai Fujimoto,Akira Sakai,Katsuki Fujisawa*

Main category: cs.LG

TL;DR: MDBF improves extreme low-bit LLM quantization by replacing DBF's single magnitude envelope with rank-l envelopes while keeping binary sign bases, enhancing expressiveness without changing inference efficiency.


<details>
  <summary>Details</summary>
Motivation: DBF's restrictive scaling parameters cause performance saturation in extreme low-bit LLM quantization - all rank components share the same magnitude profile after factoring out signs, limiting expressiveness.

Method: Propose Multi-envelope DBF (MDBF) with shared 1-bit sign bases but rank-l envelope instead of single envelope. Use closed-form initialization and alternating refinement optimization. Maintains binary carrier while allocating memory budget for magnitude expressiveness.
...
Conclusion: MDBF successfully addresses DBF's limitations by enhancing magnitude expressiveness through rank-l envelopes while maintaining efficient binary inference, achieving better quantization performance for LLMs.
...
### [190] [Many Minds from One Model: Bayesian Transformers for Population Intelligence](https://arxiv.org/abs/2512.25063)
*Diji Yang,Yi Zhang*

Main category: cs.LG

TL;DR: B-Trans transforms standard LLMs into Bayesian Transformers by making normalization layer offsets stochastic, enabling sampling of diverse model instances from a single pre-trained model for population-based decision making.


<details>
  <summary>Details</summary>
Motivation: Current transformers are trained as single deterministic systems, but intelligence may emerge from many minds. The authors want to enable sampling of diverse yet coherent model instances from a single pre-trained model to leverage collective intelligence.

Method: Treat bias-like offsets in normalization layers as stochastic variables with Gaussian variational approximation, creating a Bayesian posterior proxy. Freeze sampled noise at sequence level for temporal consistency. Sample multiple model instances from this distribution for population-level decision making.

Result: B-Trans produces diverse model behaviors while maintaining general competence. Aggregating predictions across sampled individuals enhances exploration and improves task performance. Experiments show superior semantic diversity and better performance in zero-shot generation, RLVR, and RL without explicit labels compared to deterministic baselines.

Conclusion: B-Trans successfully transforms standard LLMs into Bayesian Transformers that can sample diverse model instances, enabling population-based decision making that leverages collective intelligence for improved performance and exploration.
...
### [191] [Scaling Open-Ended Reasoning to Predict the Future](https://arxiv.org/abs/2512.25070)
*Nikhil Chandak,Shashwat Goel,Ameya Prabhu,Moritz Hardt,Jonas Geiping*

Main category: cs.LG

TL;DR: Researchers train language models for open-ended forecasting using synthesized questions from news events, creating OpenForesight dataset and OpenForecaster 8B model that matches larger proprietary models in accuracy and calibration.


<details>
  <summary>Details</summary>
Motivation: High-stakes decision making requires reasoning under uncertainty about future events. Current approaches lack specialized training for open-ended forecasting questions, and there's a need to prevent future information leakage in training.

Method: 1. Automatically synthesize forecasting questions from daily news events using careful curation recipe. 2. Use offline news corpus to prevent future information leakage. 3. Train Qwen3 thinking models on OpenForesight dataset. 4. Implement retrieval mechanisms and improved reward function for RL. 5. Perform held-out testing from May to August 2025.

Result: OpenForecaster 8B matches performance of much larger proprietary models. Training improves accuracy, calibration, and consistency of predictions. Calibration improvements generalize across popular benchmarks. All models, code, and data are open-sourced.

Conclusion: Specialized forecasting training enables language models to make accurate, well-calibrated predictions about future events, with improvements that generalize beyond forecasting tasks. The open-source release makes forecasting research broadly accessible.
...
### [192] [A Granular Grassmannian Clustering Framework via the Schubert Variety of Best Fit](https://arxiv.org/abs/2512.23766)
*Karim Salta,Michael Kirby,Chris Peterson*

Main category: cs.LG

TL;DR: Proposes SVBF-LBG: subspace clustering using Schubert Variety prototypes instead of means, improving purity while preserving geometric structure.


<details>
  <summary>Details</summary>
Motivation: Need better geometric representatives for subspace datasets that go beyond simple means/medians while maintaining mathematical structure for downstream analysis.

Method: Introduces Schubert Variety of Best Fit (SVBF) as trainable prototypes that intersect cluster members in fixed directions, integrated into Linde-Buzo-Grey (LBG) pipeline.

Result: SVBF-LBG achieves improved cluster purity on synthetic, image, spectral, and video action data compared to traditional approaches.

Conclusion: The method successfully combines geometric structure preservation with enhanced clustering performance, making it suitable for subspace analysis tasks.
...
### [193] [GARDO: Reinforcing Diffusion Models without Reward Hacking](https://arxiv.org/abs/2512.24138)
*Haoran He,Yuxiao Ye,Jie Liu,Jiajun Liang,Zhiyong Wang,Ziyang Yuan,Xintao Wang,Hangyu Mao,Pengfei Wan,Ling Pan*

Main category: cs.LG

TL;DR: GARDO is a reinforcement learning framework that selectively applies regularization to high-uncertainty samples, adaptively updates reference models, and rewards diverse high-quality outputs to prevent reward hacking while maintaining exploration and diversity in diffusion model fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current RL fine-tuning of diffusion models suffers from reward hacking due to proxy reward mismatch, leading to poor image quality and diversity collapse. Existing regularization methods compromise sample efficiency and exploration by penalizing against sub-optimal reference policies.

Method: GARDO uses selective regularization on high-uncertainty samples, adaptive reference model updates to match online policy capabilities, and diversity-aware reward amplification for high-quality diverse samples to balance exploration, diversity, and reward hacking prevention.

Result: Experiments across diverse proxy rewards and unseen metrics show GARDO effectively mitigates reward hacking, enhances generation diversity, and maintains sample efficiency and exploration capabilities.

Conclusion: GARDO provides a versatile RL framework that addresses the competing demands of sample efficiency, exploration, and reward hacking prevention in diffusion model fine-tuning through selective regularization, adaptive reference updates, and diversity-aware optimization.
...
### [194] [Lifting Vision: Ground to Aerial Localization with Reasoning Guided Planning](https://arxiv.org/abs/2512.24404)
*Soham Pahari,M. Srinivas*

Main category: cs.LG

TL;DR: ViReLoc is a visual reasoning framework that performs planning and localization using only visual representations, learning spatial dependencies and geometric relations that text-based reasoning struggles with, achieving improved spatial reasoning accuracy without GPS data.


<details>
  <summary>Details</summary>
Motivation: Current multimodal reasoning systems rely too heavily on textual information, limiting their effectiveness in spatial tasks like visual navigation and geo-localization. Text-based reasoning struggles with spatial dependencies and geometric relations that are crucial for these tasks.

Method: ViReLoc (Visual Reasoning for Localization) uses a Geo-Consistent Visual Planning paradigm that encodes step-by-step inference in the visual domain, optimized with reinforcement-based objectives. It integrates contrastive learning and adaptive feature interaction to align cross-view perspectives and reduce viewpoint differences.
...
Conclusion: Visual reasoning serves as a strong complementary approach for navigation and localization, enabling these tasks to be performed without real-time GPS data, leading to more secure navigation solutions.
...
TL;DR: Generative classifiers (using class-conditional generative models) outperform discriminative classifiers on distribution shift by modeling all features instead of relying on spurious correlations, achieving SOTA on 5 benchmarks.
...
Method: Use class-conditional generative models (diffusion-based and autoregressive) as classifiers, which model both core and spurious features rather than focusing mainly on spurious ones.

Result: Achieve state-of-the-art performance on five standard image and text distribution shift benchmarks, reduce spurious correlation impact in medical/satellite datasets, and analyze Gaussian toy setting to understand inductive biases.

Conclusion: Generative classifiers are simple to train (no specialized augmentations/regularization needed) and effectively handle distribution shift by avoiding overreliance on spurious features, with performance determined by data properties.
...
Method: Compute-optimal training on Mify-2.5B foundation model using high-quality curated sources combined with synthetic data from agentically designed prompts, refined with enterprise-grade evaluation datasets. LLM-based quality filtering enhances data density, with disciplined exploration of CPT-SFT objectives, data mixtures, and sampling dynamics.
...
Conclusion: Principled data and compute discipline allow smaller models to achieve competitive accuracy, efficiency, and safety compliance, demonstrating that compact models can match frontier-grade models in code generation and agent-driven workflows.
...
### [197] [AHA: Aligning Large Audio-Language Models for Reasoning Hallucinations via Counterfactual Hard Negatives](https://arxiv.org/abs/2512.24052)
...
TL;DR: AHA framework reduces hallucinations in Large Audio-Language Models by using counterfactual hard negative mining to create preference data, improving temporal reasoning and achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Current Large Audio-Language Models suffer from hallucinations where they generate text not grounded in audio input, particularly in temporal reasoning tasks.

Method: Introduces AHA framework with counterfactual hard negative mining to construct high-quality preference datasets that force models to distinguish acoustic evidence from fabrications, plus AHA-Eval benchmark for testing temporal reasoning.

Result: Qwen-Audio-AHA achieves 13.7% improvement on AHA-Eval and substantial gains on public benchmarks (1.3% on MMAU-Test, 1.6% on MMAR), outperforming SOTA methods.

Conclusion: The AHA framework effectively reduces hallucinations in audio-language models, with improvements generalizing beyond diagnostic benchmarks to real-world performance gains.
...
### [198] [q3-MuPa: Quick, Quiet, Quantitative Multi-Parametric MRI using Physics-Informed Diffusion Models](https://arxiv.org/abs/2512.23726)
*Shishuai Wang,Florian Wiesinger,Noemi Sgambelluri,Carolin Pirkl,Stefan Klein,Juan A. Hernandez-Tamames,Dirk H. J. Poot*

Main category: physics.med-ph

TL;DR: A diffusion model-based method (q3-MuPa) combines deep generative modeling with physics-based data consistency to accelerate and improve quantitative MRI mapping from silent MuPa-ZTE scans, achieving high-quality results from 4x accelerated acquisitions (~1 min) using only synthetic training data.


<details>
  <summary>Details</summary>
Motivation: To improve patient comfort with silent scanning while accelerating quantitative MRI acquisition and enhancing mapping accuracy, addressing the need for faster, quieter, and more robust clinical qMRI protocols.

Method: Proposes q3-MuPa: a denoising diffusion probabilistic model (DDPM) trained on synthetic digital brain phantom data to map MuPa-ZTE image series to quantitative T1, T2, and proton density maps. Incorporates the MuPa-ZTE forward signal model as explicit data consistency constraint during inference to combine deep learning with physics-based modeling.

Result: Achieves high-quality 3D qMRI maps from 4x accelerated MuPa-ZTE scans (~1 minute). Outperforms baseline dictionary matching and purely data-driven diffusion models with higher accuracy, reduced noise, and better structural detail preservation. Generalizes well to real scans despite training only on synthetic data.

Conclusion: The q3-MuPa framework combines silent MuPa-ZTE acquisition with physics-informed diffusion modeling to create a quick, quiet, and quantitative multi-parametric mapping solution with strong clinical potential, demonstrating that synthetic training data alone can enable robust real-world generalization.
...
### [199] [A multimodal Transformer for InSAR-based ground deformation forecasting with cross-site generalization across Europe](https://arxiv.org/abs/2512.23906)
*Wendong Yao,Binhua Huang,Soumyabrata Dev*

Main category: eess.SP

TL;DR: A multimodal patch-based Transformer model for next-epoch ground deformation nowcasting using EGMS time series data, achieving superior performance over other models.


<details>
  <summary>Details</summary>
Motivation: Near-real-time regional-scale ground deformation monitoring is needed for urban planning, infrastructure management, and hazard mitigation. While InSAR and services like EGMS provide past motion data, predicting future observations is challenging due to complex signal superposition (long-term trends, seasonal cycles, abrupt discontinuities) and spatial heterogeneity.

Method: Proposes a multimodal patch-based Transformer for single-step, fixed-interval next-epoch nowcasting of displacement maps from EGMS time series. The model ingests recent displacement snapshots along with: (1) static kinematic indicators (mean velocity, acceleration, seasonal amplitude) computed in a leakage-safe manner from training data only, and (2) harmonic day-of-year encodings. Tested on 64x64 grid over 100x100 km tiles.

Result: On the eastern Ireland tile (E32N34), the multimodal Transformer clearly outperforms CNN-LSTM, CNN-LSTM+Attn, and multimodal STGCN when all models receive the same multimodal inputs, achieving RMSE = 0.90 mm and R² = 0.97 on the test set with the best threshold accuracies. STGCN performed best in displacement-only setting.

Conclusion: The multimodal patch-based Transformer is an effective approach for ground deformation nowcasting, successfully integrating multiple data modalities to achieve high-precision predictions that outperform existing methods.
...
### [200] [The Drill-Down and Fabricate Test (DDFT): A Protocol for Measuring Epistemic Robustness in Language Models](https://arxiv.org/abs/2512.23850)
*Rahul Baxi*
...
<details>
  <summary>Details</summary>
Motivation: Current language model evaluations (like MMLU, TruthfulQA) only measure knowledge under ideal conditions, not how robustly models maintain factual accuracy when information degrades or adversaries probe weaknesses. There's a need to assess epistemic robustness before deployment in critical applications.
...
Conclusion: Epistemic robustness emerges from training methodology and verification mechanisms distinct from current approaches. The DDFT framework provides both theoretical foundation and practical tools for assessing epistemic robustness before deployment in critical applications, challenging assumptions about model size-reliability relationship.
...
### [201] [From Building Blocks to Planning: Multi-Step Spatial Reasoning in LLMs with Reinforcement Learning](https://arxiv.org/abs/2512.24532)
...
<details>
  <summary>Details</summary>
Motivation: Despite strong general language capabilities, LLMs struggle with spatial transformations and multi-step planning in structured environments, which limits their applications in navigation and planning tasks.

Method: Two-stage approach: 1) Supervised fine-tuning on elementary spatial transformations (rotation, translation, scaling) to give the model basic spatial physics; 2) Freeze this physics-aware model and train lightweight LoRA adapters within GRPO framework to learn policies that compose these building blocks for multi-step planning in puzzle environments. Uses synthesized ASCII-art dataset and ASCII-based RL environment.

Result: Method consistently outperforms baselines (generic backbone, physics-aware model, end-to-end RL models) under both Dynamic environments with explicit state updates and Static environments where model must rely on internal state. Approach converges faster and exhibits more stable training compared to end-to-end RL from scratch.

Conclusion: The proposed two-stage decomposition of spatial reasoning into atomic building blocks and their composition effectively improves LLMs' spatial reasoning capabilities, with attention pattern analysis confirming meaningful improvements in spatial understanding.
...
### [202] [Recursive Language Models](https://arxiv.org/abs/2512.24601)
*Alex L. Zhang,Tim Kraska,Omar Khattab*

Main category: cs.AI

TL;DR: RLMs enable LLMs to process arbitrarily long prompts via recursive self-calling on prompt snippets, achieving 100x context scaling with better quality than base models and cheaper cost.
...
Method: Recursive Language Models (RLMs) treat long prompts as external environments, allowing LLMs to programmatically examine, decompose, and recursively call themselves over prompt snippets.

Result: RLMs handle inputs up to two orders of magnitude beyond model context windows, outperform base LLMs and common long-context scaffolds across four diverse tasks, with comparable or cheaper cost per query.

Conclusion: RLMs provide an effective inference strategy for processing arbitrarily long prompts, offering superior quality and cost efficiency compared to existing approaches.
...
<details>
  <summary>Details</summary>
Motivation: The open-source community lacks a principled, end-to-end ecosystem for developing agentic LLMs that can operate in real-world environments over multiple turns, taking actions, observing outcomes, and refining artifacts iteratively.

Method: ALE consists of three components: 1) ROLL for post-training weight optimization, 2) ROCK as a sandbox environment manager for trajectory generation, and 3) iFlow CLI for efficient context engineering. They introduce data composition protocols for synthesizing complex behaviors and IPA (Interaction-based Policy Alignment) algorithm that assigns credit over semantic interaction chunks rather than individual tokens.

Result: They release ROME (an open-source agent grounded by ALE) trained on over one million trajectories. ROME demonstrates strong performance across benchmarks including SWE-bench Verified and Terminal Bench Pro (their new benchmark with improved scale and contamination control).

Conclusion: The ALE infrastructure effectively streamlines agent development, as proven by ROME's strong benchmark performance. The ecosystem addresses the gap in open-source tools for developing agentic LLMs that can operate in real-world environments.
...
TL;DR: Iterative deployment of LLMs with user-curated data leads to significant model property changes, improving planning skills and enabling emergent generalization to longer plans, effectively implementing implicit RL training.


<details>
  <summary>Details</summary>
Motivation: To understand how iterative deployment of LLMs with user-curated data affects model properties and to explore the implications of this process as an implicit reinforcement learning mechanism.

Method: Test iterative deployment mechanism on various planning domains, where each model is fine-tuned on data curated by users from previous models' deployment, then analyze the process theoretically as an implicit RL training mechanism.
...
Conclusion: Iterative deployment with user-curated data serves as an alternative to explicit RL training, with important AI safety implications due to the undefined implicit reward function that could lead to unexpected model properties in future deployments.
...
### [205] [Large language models and the entropy of English](https://arxiv.org/abs/2512.24969)
*Colin Scheibner,Lindsay M. Smith,William Bialek*

Main category: cond-mat.stat-mech

TL;DR: LLMs reveal long-range correlations in English texts up to ~10^4 characters, showing direct dependencies across large distances that emerge gradually during training.


<details>
  <summary>Details</summary>
Motivation: To investigate whether there are long-range structural dependencies in English texts beyond typical context windows, and to understand how language models learn these patterns during training.

Method: Using large language models to analyze conditional entropy/code length across varying context lengths in English texts, measuring correlations between characters at large separations, and tracking code length distribution dynamics during model training.

Result: Conditional entropy continues decreasing up to N~10^4 characters, revealing direct long-range dependencies. Significant correlations exist between characters at large separations. Code length distribution shows emergent certainty about characters at large N. Long-range structure is learned gradually during training, with different dynamics than short-range patterns.

Conclusion: English texts contain long-range structural dependencies extending to thousands of characters, which language models learn gradually. These findings constrain statistical physics models of both LLMs and language itself, revealing fundamental properties of linguistic structure.
...
### [206] [Vibe Coding, Interface Flattening](https://arxiv.org/abs/2512.24939)
*Hongrui Jin*

Main category: cs.HC

TL;DR: Vibe coding through LLMs flattens programming interfaces while thickening underlying infrastructure, redistributing power to model providers and creating new dependencies.


<details>
  <summary>Details</summary>
Motivation: To critically analyze the political economy of LLM-mediated programming (vibe coding), examining how interface flattening masks infrastructural complexity and power shifts.

Method: Materialist media theory approach using Kittler and Galloway's frameworks, reconstructing the vibe-coding stack to analyze infrastructure, latency, structured outputs, tool calling, and protocols like Model Context Protocol.

Result: Vibe coding creates apparent democratization but actually establishes new dependencies, relocates control to model/protocol providers, obscures responsibility, and privatizes competencies previously distributed across programming communities.

Conclusion: LLM-mediated development redistributes symbolic power, creates tension between experiential simplicity and infrastructural complexity, and requires critical examination of the political economy of AI-mediated human-computer interaction.
...
### [207] [Towards autonomous time-calibration of large quantum-dot devices: Detection, real-time feedback, and noise spectroscopy](https://arxiv.org/abs/2512.24894)
...
TL;DR: Autonomous stabilization method for quantum-dot qubits using charge stability diagrams to track electrostatic drift, maintain stable operation, and perform noise spectroscopy in large arrays.
...
Method: Uses charge-transition lines in repeatedly acquired double-quantum-dot charge stability diagrams as a multidimensional probe. Tracks motion of selected transitions to detect voltage drifts, identify charge reconfigurations, and apply compensating updates. Enables time-domain noise spectroscopy via high-rate RF reflectometry measurements.

Result: Demonstrated on 10-QD device with robust stabilization and real-time diagnostic access. Extracted noise power spectral densities showing 1/f² drift at 100 μHz, identified two-level fluctuators, and measured average correlation length of 188±38 nm across array.

Conclusion: Provides scalable autonomous calibration and characterization module for QD quantum processors, enabling essential feedback for long-duration, high-fidelity qubit operations.
