...
TL;DR: Omnilingual ASR is the first large-scale extensible ASR system that enables adding new languages with minimal data, scaling to 1,600+ languages including 500+ previously unsupported ones.


<details>
  <summary>Details</summary>
Motivation: Most of the world's 7,000+ languages lack ASR support due to high costs and restrictive architectures, creating a digital divide while raising ethical concerns about community exclusion.

Method: Uses 7B parameter self-supervised pre-training for robust speech representations, encoder-decoder architecture with LLM-inspired decoder for zero-shot generalization, trained on diverse corpus including community-sourced recordings.

Result: Achieves largest ASR coverage to date (1,600+ languages), substantial gains over prior systems in low-resource conditions, strong generalization, and releases models from 300M to 7B parameters.

Conclusion: Open-sourcing models and tools lowers barriers for researchers and communities, enabling new participation forms while addressing ethical considerations in language technology development.
...
Method: Conducted controlled experiments on classification and generation tasks using multiple model families (0.5B to 27B parameters) and GPT-5, comparing variance from different orderings versus different example sets.

Result: Variance in performance due to different example orderings is comparable to variance from using entirely different example sets. Strong orderings can be identified using development sets, achieving performance close to oracle selection.
...
TL;DR: Morphologically-aware tokenization improves language model performance for Latin, especially for out-of-domain texts, demonstrating the value of linguistic resources for morphologically rich languages.


<details>
  <summary>Details</summary>
Motivation: Standard tokenization methods prioritize compression over linguistic goals, which is suboptimal for morphologically rich languages like Latin where tokenization quality impacts downstream performance.

Method: Investigated morphologically-aware tokenization for Latin using curated lexical resources, focusing on morphological alignment rather than information-theoretical goals.

Result: Morphologically-guided tokenization improved performance on four downstream tasks, with most significant gains for out-of-domain texts, indicating better generalization ability.
...
TL;DR: NLP can identify main topics in presidential directives but shows discrepancies with human analysis, highlighting the need for validation in social science applications.


<details>
  <summary>Details</summary>
Motivation: To explore how NLP can extract signaling themes from large written corpora like Presidential Directives, and assess its utility compared to human analysis.

Method: Applied NLP techniques to identify main topics from Presidential Directives spanning Reagan through Clinton administrations, comparing results with human-labeled analysis.

Result: Both NLP and human analysts identified relevant documents, showing NLP's potential utility. However, discrepancies between NLP and human results were observed.

Conclusion: While NLP shows promise for analyzing large written corpora in social science, discrepancies with human analysis indicate need for further validation, especially given rapid AI/ML advancements.
...
TL;DR: Small LLMs (1B parameters) can effectively detect critical translation errors with good quality-efficiency trade-off, enabling on-device deployment for privacy-sensitive workflows.


<details>
  <summary>Details</summary>
Motivation: Large LLMs are effective for machine translation evaluation but are too resource-intensive for edge devices and privacy-sensitive applications, creating need for smaller models that can still detect meaning-altering errors.
...
Result: Gemma-3-1B provides best quality-efficiency trade-off (MCC=0.77, F1-ERR=0.98) with 400ms latency on MacBook Pro M4 Pro. Qwen-3-1.7B achieves highest MCC but with higher compute cost. Ultra-small models (0.6B) are usable but under-detect certain error types.

Conclusion: Compact instruction-tuned LLMs with lightweight calibration can deliver trustworthy on-device critical error detection for machine translation, enabling private and low-cost error screening in real-world pipelines.
...
TL;DR: Analysis of predicate-argument structures in Chinese-English parallel sentences reveals asymmetric language transfer in cross-lingual NLP, with linguistic divergences affecting annotation projection differently depending on source language selection.


<details>
  <summary>Details</summary>
Motivation: To understand how linguistic divergences hinder cross-lingual transfer, especially between typologically distant languages like Chinese and English, and to examine the asymmetry in language transfer.

Method: Analysis of predicate-argument structures in parallel Chinese-English sentences, categorization of structural divergences, and annotation projection experiments where each language serves as source to project annotations to the other.

Result: Language transfer is asymmetric - the effectiveness of annotation projection differs significantly depending on which language is used as the source, with clear patterns of alignment and misalignment in predicate annotations.

Conclusion: Source language selection in transfer learning applications requires careful consideration due to asymmetric transfer effects, and this asymmetry must be investigated before making scientific claims about cross-lingual NLP.
...
TL;DR: TARG is a training-free adaptive retrieval gating method that uses lightweight uncertainty scores from short model drafts to decide when to retrieve, reducing retrieval by 70-90% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Retrieval-Augmented Generation (RAG) improves factuality but retrieving for every query increases tokens, latency, and computational costs unnecessarily.

Method: Uses a single-shot policy that analyzes prefix logits from short model drafts to compute uncertainty scores (token entropy, margin signal from top-1/top-2 logit gap, or small-N variance) and triggers retrieval only when scores exceed a threshold.

Result: On NQ-Open, TriviaQA, and PopQA, TARG matches or improves EM/F1 compared to Always-RAG while reducing retrieval by 70-90% and cutting end-to-end latency, remaining close to Never-RAG in overhead.

Conclusion: Margin signal is a robust default uncertainty measure under modern instruction-tuned LLMs, with small-N variance offering a conservative alternative; TARG effectively shifts the accuracy-efficiency frontier without additional training.
...
TL;DR: Proposes a holistic approach for Khmer spellchecking that integrates subword segmentation, NER, G2P conversion, and language models to address unique challenges in Khmer language, achieving 94.4% accuracy.


<details>
  <summary>Details</summary>
Motivation: Khmer spellchecking faces unresolved challenges including word segmentation misalignments, multiple word forms, loose compound word formation, and proper noun detection issues that existing solutions don't adequately address.

Method: Integrates Khmer subword segmentation, named-entity recognition (NER), grapheme-to-phoneme (G2P) conversion, and a Khmer language model to identify and rank correction candidates.

Result: Achieves state-of-the-art Khmer spellchecking accuracy of 94.4%, outperforming existing solutions. Benchmark datasets for spellchecking and NER will be made publicly available.

Conclusion: The holistic approach successfully addresses the unique challenges in Khmer spellchecking by combining multiple linguistic components, demonstrating significant improvement over previous methods.
...
TL;DR: A course recommendation system using machine learning and data analytics to help students select industry-relevant courses, with an emphasis on user-friendly interface design.
...
Method: Combines data analytics, machine learning algorithms, collaborative filtering, and data mining techniques with user preferences and academic criteria. Includes iterative prototyping and user feedback for interface development.

Result: Developed a refined course recommendation system with an easy-to-use front-end interface that provides personalized course suggestions based on industry trends and individual career goals.
...
TL;DR: Proposes a question answering system using fine-tuned LLM with RAG method to handle repetitive student queries in course forums, reducing instructor workload and improving response time.


<details>
  <summary>Details</summary>
Motivation: Address challenges in course forums where increasing student enrollment leads to delayed responses to queries and instructors facing repetitive questions.

Method: Fine-tune open source LLM on course dataset, use RAG with local knowledge base containing course content, and integrate multi chain-of-thought reasoning to reduce hallucination.

Result: Experimental results on HotpotQA dataset show strong performance of fine-tuned LLM with RAG method on question answering tasks.

Conclusion: The proposed system effectively mitigates forum challenges by providing timely responses to student queries while reducing instructor workload through automated question answering.
...
TL;DR: TermGPT is a multi-level contrastive fine-tuning framework that addresses the isotropy problem in LLM embeddings for better terminology discrimination in legal and financial domains.


<details>
  <summary>Details</summary>
Motivation: LLMs suffer from isotropy in embedding spaces, leading to poor discrimination of domain-specific terminology in legal and financial contexts, which hinders downstream tasks like legal judgment prediction and financial risk analysis.

Method: Constructs a sentence graph to capture semantic relations, generates discriminative positive/negative samples using contextual and topological cues, and employs multi-level contrastive learning at both sentence and token levels.

Result: Outperforms existing baselines in term discrimination tasks within finance and legal domains, with evaluation supported by a new financial terminology dataset from regulatory documents.

Conclusion: TermGPT effectively enhances terminology discrimination in domain-specific contexts through multi-level contrastive learning, addressing a key limitation of current LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Supervised fine-tuning penalizes valid alternative rationales while reinforcement learning struggles with credit assignment and high computational costs, creating challenges for training LLMs for chain-of-thought reasoning.
...
Result: Across six math-reasoning benchmarks, InTRO consistently outperforms baselines with up to 20% relative accuracy improvement over base models, producing more concise rationales and enabling successful cross-domain transfer to non-mathematical reasoning tasks.

Conclusion: InTRO provides an effective framework for training LLMs that encourages accurate and concise reasoning while demonstrating robust generalization across domains.
...
<details>
  <summary>Details</summary>
Motivation: Address the high computational and memory costs of LLMs that limit deployment in resource-constrained or real-time settings.

Method: Formulated as a finite-horizon MDP, trains a PPO-based RL agent to iteratively select models at each stage of multi-hop inference, conditioning on evolving context and accumulated cost.

Result: Experiments with three open-source LLMs across six benchmarks show HierRouter improves response quality by up to 2.4x compared to individual models, with minimal additional inference cost.

Conclusion: Hierarchical routing shows promise for cost-efficient, high-performance LLM inference.
...
TL;DR: EnchTable is a framework that transfers safety alignment to fine-tuned LLMs without retraining, using NTK-based safety vector distillation and interference-aware merging to maintain both safety and utility across diverse models and tasks.
...
Result: Achieves significantly lower unsafe rates and higher utility scores than baseline methods, demonstrates robust resistance to jailbreaking attacks, and works across 3 task domains, 3 LLM architectures, and 11 datasets.

Conclusion: EnchTable effectively maintains safety alignment in fine-tuned LLMs with universal applicability, minimal performance overhead, and seamless integration into deployment pipelines.
...
Method: Uses comprehensive preprocessing pipeline with facial landmark detection, lip region isolation/stabilization, and quality assessment. Implements curriculum learning guided by quality scores. Adopts SigLIP encoder with Unified 3D-Resampler for high-frame-rate lip motion encoding.
...
TL;DR: LLM-generated knowledge components (KCs) are redundant and perform worse than expert-designed ones, but semantic merging using cosine similarity significantly improves performance by reducing redundancy.


<details>
  <summary>Details</summary>
Motivation: Manual KC identification by experts is a bottleneck in adaptive learning systems, and existing LLM approaches produce redundant labels and are limited to small datasets.

Method: Scaled LLM prompting (GPT-4o-mini) on 646 questions, followed by semantic merging of similar KCs using cosine similarity thresholds.

Result: Initial LLM approach generated 569 KCs vs 101 expert KCs with worse RMSE (0.4285 vs 0.4206). Semantic merging at 0.8 threshold reduced KCs to 428 and improved RMSE to 0.4259.

Conclusion: Combining scaled LLM generation with semantic merging offers a viable path for automating KC identification, addressing redundancy issues in automated approaches.
...
TL;DR: REAP introduces recursive evaluation and adaptive planning for multi-hop reasoning in RAG systems, using structured sub-tasks and facts to overcome local reasoning impasses and enhance reliability.


<details>
  <summary>Details</summary>
Motivation: Existing RAG methods for multi-hop reasoning lack global planning, leading to local reasoning impasses, insufficient content exploitation, and neglect of latent clues, compromising reasoning accuracy.

Method: REAP uses Sub-task Planner (SP) for global perspective and dynamic optimization, and Fact Extractor (FE) for fine-grained analysis of retrieved content, incrementally building coherent global knowledge representation.
...
Conclusion: REAP effectively addresses complex multi-hop reasoning tasks by maintaining structured knowledge representation and enabling dynamic optimization of reasoning trajectories.
...
TL;DR: Evaluation shows LLMs struggle with numerical fact-checking robustness, experiencing up to 62% accuracy drops under perturbations, with no model proving universally robust.


<details>
  <summary>Details</summary>
Motivation: Large language models perform well on knowledge tasks but have limitations in numerical reasoning, requiring systematic evaluation of their robustness for veracity prediction on numerical claims.

Method: Systematic evaluation using controlled perturbations including label-flipping probes to test model robustness on numerical claims and evidence pairs, examining context length effects and enriched demonstrations.

Result: Leading proprietary models experience accuracy drops up to 62% under perturbations; no model is robust across all conditions; extended context reduces accuracy but enriched perturbed demonstrations help recovery.

Conclusion: Numerical fact-checking has critical limitations in current language models, and robustness remains an open challenge despite some improvements with enriched demonstrations.
...
TL;DR: ETC is a training-free method that uses entropy trend analysis to determine optimal retrieval timing in dynamic RAG systems, enabling earlier and more precise retrieval interventions.
...
Method: ETC utilizes first- and second-order differences of the entropy sequence to detect emerging uncertainty trends, allowing it to identify optimal retrieval timing by modeling token-level uncertainty dynamics.
...
TL;DR: Multilingual RAG suffers from language drift where models generate responses in unintended languages, especially during reasoning tasks. The paper proposes Soft Constrained Decoding (SCD) to mitigate this issue.


<details>
  <summary>Details</summary>
Motivation: To address the problem of output language drift in multilingual RAG systems, where retrieved evidence in different languages causes models to generate responses in unintended languages, particularly during reasoning-intensive decoding like Chain-of-Thought.

Method: Proposed Soft Constrained Decoding (SCD), a lightweight, training-free decoding strategy that penalizes non-target-language tokens to steer generation toward the target language. It's model-agnostic and can be applied to any generation algorithm without architectural changes.

Result: Experiments across three multilingual datasets and multiple typologically diverse languages show that SCD consistently improves language alignment and task performance in multilingual RAG settings.

Conclusion: SCD provides an effective and generalizable solution to mitigate language drift in multilingual RAG, addressing decoder-level collapse where dominant English patterns interfere with intended generation languages.
...
TL;DR: BERTScore fails to detect numerical variations in financial texts, leading to inaccurate similarity assessments despite critical semantic differences in numbers like percentages and financial figures.
...
Conclusion: Reveals fundamental limitations of embedding-based metrics for finance and motivates development of numerically-aware evaluation frameworks for financial NLP.
...
<details>
  <summary>Details</summary>
Motivation: To adapt LLMs for curriculum-specific education in India, addressing challenges of accuracy, alignment, and pedagogical relevance in the NCERT syllabus.

Method: Developed NCERT-QA dataset with Factoid, Inferential, and Other question types; evaluated using meta-prompt, few-shot, and CoT prompting techniques on various LLMs.

Result: Analyzed strengths and limitations of open-source and high-end LLMs as AI learning tools, identifying which prompting approaches align best with curriculum demands.

Conclusion: Provides framework for evaluating LLMs in formal education systems, highlighting their potential and limitations for curriculum-aligned learning experiences.
...
Method: Segment long inputs into overlapping chunks, generate compressed context-aware representations for the decoder, and use parameter-free fusion to enrich chunk boundaries with cumulative context from preceding and succeeding chunks.

Result: Experiments on long-document summarization show the method is highly competitive with and often outperforms state-of-the-art approaches without architectural modifications or external retrieval.

Conclusion: ScaleFormer provides a simple plug-and-play framework that adapts pre-trained models for long sequences while maintaining linear complexity and strong performance.
...
TL;DR: This paper investigates how Multimodal Large Language Models (MLLMs) interpret sound symbolism (phonetic iconicity) across text and audio modalities using a new dataset called LEX-ICON.


<details>
  <summary>Details</summary>
Motivation: To understand how MLLMs process auditory information and phonetic associations in human languages, bridging artificial intelligence and cognitive linguistics.

Method: Created LEX-ICON dataset with 8,052 words from 4 languages and 2,930 pseudo-words, annotated with semantic features. Analyzed MLLMs' performance across textual (orthographic and IPA) and auditory inputs using phoneme-level attention scores across 25 semantic dimensions.

Result: MLLMs demonstrate phonetic intuitions aligned with linguistic research across multiple semantic dimensions, and show phonosemantic attention patterns focusing on iconic phonemes.

Conclusion: This provides the first large-scale quantitative analysis of phonetic iconicity in MLLMs' interpretability, bridging AI and cognitive linguistics domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat multi-turn dialogue responses as isolated tasks and fail to explicitly incorporate relational constraints across turns, leading to poor performance on complex long-distance constraints.

Method: Three components: (1) agent-based relation extraction to construct directed relation graphs, (2) relation graph prompt generation to convert graphs to natural language prompts, (3) response rewriting using graph prompts to refine LLM outputs.

Result: Extensive experiments on two long multi-turn dialogue datasets show significant improvements across all four multi-turn instruction-following evaluation metrics when integrated with instruction-tuned LLMs.

Conclusion: GraphIF effectively bridges the gap in leveraging graph structures for multi-turn instruction following and demonstrates that structured graph information can significantly enhance LLM performance in complex dialogue scenarios.
...
TL;DR: ADI-20 extends ADI-17 with 3,556 hours of 19 Arabic dialects plus MSA, used to train and evaluate state-of-the-art ADI systems including ECAPA-TDNN and Whisper-based models.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive Arabic dialect identification dataset covering all Arabic-speaking countries and enable research in ADI by providing open-source data and models.

Method: Fine-tuned pre-trained ECAPA-TDNN models and Whisper encoder blocks with attention pooling and classification layers, investigating training data size and model parameters effects.

Result: Small F1 score decrease when using only 30% of training data, demonstrating data efficiency of the approach.

Conclusion: The ADI-20 dataset and trained models are open-sourced to support reproducibility and further ADI research.
...
TL;DR: Multimodal LLMs perform better with table-based evidence than chart-based evidence for scientific claim verification, showing limited cross-modal generalization, especially in smaller models.


<details>
  <summary>Details</summary>
Motivation: There is growing demand for systems to assist reviewers in evaluating scientific papers, but understanding how robust multimodal LLMs are at verifying claims across different evidence formats (tables vs charts) remains underexplored.

Method: Adapted two existing scientific paper datasets for multimodal claim verification, evaluated 12 multimodal LLMs on their ability to verify claims using both tables and charts as evidence, and conducted human evaluations for comparison.

Result: Current models perform better with table-based evidence while struggling with chart-based evidence. Humans maintain strong performance across both formats. Smaller multimodal LLMs (under 8B) show weak correlation between table and chart performance, indicating limited cross-modal generalization.

Conclusion: There is a critical gap in current models' multimodal reasoning capabilities, particularly in chart understanding. Future multimodal LLMs should focus on improving chart comprehension to better support scientific claim verification.
...
TL;DR: Elyadata & LIA's NADI 2025 submission achieved 1st place in Arabic Dialect Identification and 2nd place in multi-dialectal Arabic ASR using fine-tuned Whisper and SeamlessM4T models.


<details>
  <summary>Details</summary>
Motivation: To develop effective systems for Arabic dialect identification and multi-dialectal automatic speech recognition in the NADI 2025 competition.

Method: Used fine-tuned Whisper-large-v3 encoder with data augmentation for ADI, and separately fine-tuned SeamlessM4T-v2 Large for each of eight Arabic dialects for ASR.

Result: Achieved 79.83% accuracy on ADI test set (1st place), and average WER of 38.54% and CER of 14.53% on multi-dialectal ASR test set (2nd place).
...
TL;DR: This paper examines military applications of NLP and LLMs like GPT, testing Microsoft Copilot's knowledge of such uses and assessing feasibility via Azure cloud services.


<details>
  <summary>Details</summary>
Motivation: To explore how natural language processing and large language models can be applied in military contexts, given their recent prominence with GPT and ChatGPT.

Method: 1) Interrogated GPT-based Microsoft Copilot to reveal its knowledge about potential military applications; 2) Studied how Microsoft Azure cloud services could be used to build such applications and assessed their feasibility.

Result: The summarization and generative properties of language models directly facilitate many military applications, with other features finding particular uses.

Conclusion: LLMs have significant potential for military applications, with summarization and generative capabilities being particularly useful, and cloud services like Azure providing ready implementation platforms.
...
<details>
  <summary>Details</summary>
Motivation: Social media platforms are essential for disaster monitoring but existing systems suffer from event-related biases that limit generalization to emerging events, while debiasing methods remain underexplored in this domain.

Method: Approaches bias mitigation through a causal lens to reduce event- and domain-related biases, enhancing model generalization capabilities.
...
Conclusion: Causal approach effectively mitigates biases in disaster monitoring systems, enabling better generalization to future events and improved performance on disaster classification tasks.
...
TL;DR: VISTA is a web-based visual interactive system that helps analyze multi-turn reasoning in LLMs by visualizing context influence, enabling what-if analyses, and generating reasoning dependency trees.
...
Method: Developed VISTA - a web-based visual interactive system that allows visualization of context influence, interactive modification of conversation histories for what-if analyses, and automatic generation of reasoning dependency trees.
...
TL;DR: Text2SQL-Flow is a data augmentation framework that generates large-scale, diverse Text-to-SQL pairs to address data scarcity in AI systems, creating the SQLFlow dataset with 89,544 examples that improves LLM performance.


<details>
  <summary>Details</summary>
Motivation: Address the limitations of current Text-to-SQL datasets which are scarce, simplistic, and lack diversity, hindering AI system performance.

Method: Propose Text2SQL-Flow framework with six augmentation dimensions, SQL execution verification, natural language question generation, chain-of-thought reasoning, data classification, and modular Database Manager for cross-database compatibility.

Result: Built SQLFlow dataset with 89,544 annotated examples. Fine-tuning on SQLFlow improves open-source LLM performance, and masked alignment retrieval method enhances closed-source LLM performance by treating SQLFlow as knowledge base.

Conclusion: Establishes a scalable, data-centric foundation for advancing Text-to-SQL systems and demonstrates the critical role of high-quality structured data in modern AI.
...
<details>
  <summary>Details</summary>
Motivation: Current LLMs with CoT prompting produce unnecessarily long explanations that increase costs and sometimes reduce accuracy, while efficiency-oriented methods lack standardized evaluation across different paradigms.

Method: Created EffiReason-Bench with verified CoT annotations using a pipeline enforcing standardized reasoning structures, option-wise analysis, and human verification. Evaluated 7 methods across 6 LLMs (1B-70B) on 4 datasets spanning mathematics, commonsense, and logic.
...
Conclusion: Efficient reasoning requires paradigm-specific approaches tailored to model scale and task characteristics, with EffiReason-Bench enabling rigorous cross-paradigm evaluation through standardized benchmarks and principled metrics.
...
<details>
  <summary>Details</summary>
Motivation: Mainstream personalized dialogue models using token-level training tend to neglect given personas and generate generic responses, creating a need for direct persona alignment as the training objective.
...
Result: Outperforms many state-of-the-art personalized dialogue methods and large language models in experiments.
...
TL;DR: LangGPS is a two-stage data selection framework that uses language separability scores to improve multilingual instruction tuning by filtering and refining training data, enhancing performance especially for understanding tasks and low-resource languages.


<details>
  <summary>Details</summary>
Motivation: Existing multilingual data selection methods overlook linguistic structure and language separability in model representations, leading to suboptimal multilingual capabilities in LLMs.

Method: Two-stage framework: first filters training data based on language separability scores (how well samples in different languages can be distinguished), then refines using existing selection methods.

Result: Improves effectiveness of existing selection methods across 6 benchmarks and 22 languages, particularly for understanding tasks and low-resource languages. Highly separable samples form clearer language boundaries while low-separability samples enable cross-lingual alignment.

Conclusion: Language separability provides a new perspective on data utility in multilingual contexts and can guide curriculum learning, supporting development of more linguistically informed LLMs.
...
TL;DR: VocalNet-M2 is a low-latency spoken language model that reduces response delay by using multi-codebook tokens and multi-token prediction, cutting first chunk latency from 725ms to 350ms while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end spoken language models suffer from significant response latency due to autoregressive token generation and complex flow-matching models for speech synthesis.

Method: Integrates multi-codebook tokenizer and multi-token prediction strategy to directly generate multi-codebook speech tokens, eliminating the need for flow-matching models.

Result: Achieves substantial reduction in first chunk latency (from ~725ms to 350ms) while maintaining competitive performance compared to mainstream SLMs.
...
TL;DR: MTR-DuplexBench is a new benchmark for evaluating Full-Duplex Speech Language Models (FD-SLMs) in multi-round conversational settings, addressing gaps in existing single-round focused evaluations.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for FD-SLMs focus on single-round interactions and neglect multi-round communication complexities, instruction following, and safety evaluation. Current evaluation challenges include blurred turn boundaries and context inconsistency in continuous dialogues.

Method: The authors introduce MTR-DuplexBench, which segments continuous full-duplex dialogues into discrete turns to enable comprehensive turn-by-turn evaluation across dialogue quality, conversational dynamics, instruction following, and safety.

Result: Experimental results show that current FD-SLMs struggle to maintain consistent performance across multiple rounds and various evaluation dimensions, demonstrating the necessity of the proposed benchmark.

Conclusion: The benchmark effectively addresses evaluation gaps in FD-SLM research and reveals performance limitations in current models, with the benchmark and code to be made available in the future.
...
TL;DR: A privacy-preserving question-answering system that achieves competitive accuracy on local infrastructure without internet access, resolving the trade-off between cloud AI capabilities and data security.


<details>
  <summary>Details</summary>
Motivation: Organizations face a dilemma between using cloud-based AI systems that compromise data privacy and local processing that delivers poor accuracy, especially for sensitive documents in legal, scientific, and conversational contexts.
...
Result: Achieves competitive accuracy on complex queries across various document types while keeping all data on local machines, enabling reliable answers with minimal errors for banks, hospitals, and law firms.

Conclusion: Privacy and performance need not be mutually exclusive in enterprise AI deployment, allowing organizations to adopt conversational document AI without transmitting proprietary information to external providers.
...
TL;DR: The paper identifies imbalanced evaluation preference in LLMs (preferring lower-perplexity solutions) as a key issue in multi-step mathematical reasoning, proposes a perplexity-aware reinforcement learning method to rectify this bias, and validates it on a new benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing methods for improving LLMs' multi-step mathematical reasoning focus on supervised fine-tuning but overlook the underlying cause of poor critiquing performance - imbalanced evaluation preference where LLMs favor solutions with lower perplexity.

Method: Built a One-to-many Problem-Solution (OPS) benchmark to quantify LLM evaluation behavior, conducted statistical preference analysis, and proposed a perplexity-aware reinforcement learning algorithm (Group Relative Policy Optimization) that uses perplexity as guidance to explore trajectories that judge lower-perplexity solutions as wrong and higher-perplexity as correct.

Result: Extensive experiments on the OPS benchmark and existing critic benchmarks demonstrate the method's effectiveness in improving LLMs' critiquing capability for multi-step mathematical reasoning.

Conclusion: Rectifying imbalanced evaluation preference through perplexity-aware reinforcement learning significantly enhances LLMs' ability to critique multi-step mathematical reasoning solutions, addressing a fundamental limitation in current approaches.
...
TL;DR: Systematic study on generating and evaluating synthetic multilingual pretraining data for Indic languages, creating a 540B token dataset using various techniques and analyzing quality factors.


<details>
  <summary>Details</summary>
Motivation: Synthetic data offers scalable high-quality pretraining for LLMs, especially beneficial for low-resource languages where LLM benefits are unevenly distributed.

Method: Constructed BhashaKritika dataset (540B tokens) using 5 techniques across 10 languages, exploring grounding in documents/personas/topics, comparing translation vs native generation, and developing modular quality evaluation pipeline.

Result: Empirical model runs revealed key trade-offs in generation strategies and identified best practices for effective multilingual corpora construction.

Conclusion: The study provides insights into optimal synthetic data generation strategies for multilingual LLM pretraining, particularly for low-resource language contexts.
...
Result: High extraction performance: 0.96-0.99 F1 for metadata, 0.7-0.8 F1 for entity recognition, 0.65-0.75 F1 for hypothesis extraction, 0.95-0.97 for evidence extraction, and 0.62 G-EVAL for discourse representation. Smaller models performed competitively.

Conclusion: ATR4CH provides the first systematic methodology for coordinating LLM-based extraction with Cultural Heritage ontologies, offering a replicable framework adaptable across CH domains, though limited to Wikipedia articles and requiring human oversight.
...
TL;DR: TruthfulRAG is a framework that uses Knowledge Graphs to resolve factual-level knowledge conflicts in Retrieval-Augmented Generation systems, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing RAG systems struggle with resolving conflicts between retrieved external information and LLMs' internal knowledge, which compromises accuracy and reliability.
...
Result: Extensive experiments show TruthfulRAG outperforms existing methods, effectively alleviating knowledge conflicts and improving system robustness and trustworthiness.

Conclusion: TruthfulRAG enables LLMs to generate more faithful and accurate responses by systematically resolving factual-level knowledge conflicts through Knowledge Graph integration.
...
<details>
  <summary>Details</summary>
Motivation: To highlight methodological concerns in existing studies that evaluate base LLMs' reasoning capabilities, which overlook fundamental mismatches between pretraining objectives and reasoning assessment.

Method: Analyzing the fundamental mismatch between base LLMs' pretraining (statistical language modeling) and normative reasoning qualities like correctness, showing how logical conclusions are coincidental byproducts.

Result: Demonstrates that base LLMs generate logically valid/invalid conclusions as coincidental outcomes of conforming to linguistic patterns, challenging assumptions about their outputs as genuine reasoning attempts.
...
TL;DR: DELICATE is a neuro-symbolic Entity Linking method for historical Italian that combines BERT with Wikidata contextual information, outperforming larger models and providing more explainable results.


<details>
  <summary>Details</summary>
Motivation: Entity Linking in humanities faces challenges due to complex document types, lack of domain-specific datasets/models, and long-tail entities underrepresented in Knowledge Bases.

Method: DELICATE combines BERT-based encoder with Wikidata contextual information using temporal plausibility and entity type consistency. Also created ENEIDE corpus from historical Italian texts.
...
Conclusion: The neuro-symbolic approach effectively addresses EL challenges in humanities, demonstrating superior performance and explainability for historical Italian texts.
...
TL;DR: Analogical paradigm organization enables lightweight models to achieve competitive linguistic rule learning with minimal data, outperforming larger models like GPT-3.


<details>
  <summary>Details</summary>
Motivation: To explore whether analogical paradigm organization can help lightweight models match the performance of large language models while using significantly less data.

Method: Developed a computational approach using three cognitive-inspired principles: analogical structure, contrastive learning, and minimal contextual cues. Tested with structured completion tasks where models identify correct sentence completions from analogical patterns with contrastive alternatives.

Result: Lightweight models (BERT+CNN, 0.5M parameters) trained on only 100 structured examples achieved F1=0.95, outperforming zero-shot GPT-3 (F1=0.87). Ablation studies confirmed analogical organization and contrastive structure improve performance. Cross-phenomenon validation replicated efficiency gains.
...
<details>
  <summary>Details</summary>
Motivation: Address intent misunderstandings in LLMs that frustrate users and create safety risks when models implicitly commit to one interpretation of ambiguous requests.

Method: Train models with reinforcement learning and customized reward functions using multiple valid answers as supervision to generate multiple interpretation-answer pairs in structured responses.
...
Conclusion: Approach promotes transparency with explicit interpretations, achieves efficiency with one generation step, and supports downstream applications through structured output format.
...
TL;DR: LLMs can track state in well-defined tasks, with newer models like GPT-4 and Llama3 performing better, especially with Chain of Thought, while older models fail after multiple steps.


<details>
  <summary>Details</summary>
Motivation: To isolate and analyze the state tracking capabilities of LLMs separate from other factors in complex reasoning tasks.

Method: Proposed a benchmark with three well-defined state tracking tasks and tested LLMs in different scenarios, including with Chain of Thought mechanisms.

Result: Recent LLMs (GPT-4, Llama3) can track state effectively, especially with Chain of Thought, while older models understand the task initially but fail after several steps.

Conclusion: State tracking is a capability that newer LLMs possess, but performance depends on model generation and reasoning mechanisms like Chain of Thought.
...
TL;DR: LocalBench is the first benchmark for evaluating LLMs on county-level local knowledge across the US, revealing significant limitations in handling hyper-local information despite advances in broader geographic reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to capture hyper-local knowledge complexity needed for real-world applications like civic platforms and community journalism, creating a gap in understanding LLMs' ability to reason about neighborhood-specific dynamics.

Method: Created LocalBench with 14,782 validated question-answer pairs across 526 US counties using diverse sources (Census data, local subreddits, regional news) spanning physical, cognitive, and relational dimensions of locality. Evaluated 13 state-of-the-art LLMs in closed-book and web-augmented settings.

Result: Critical limitations found: best models achieve only 56.8% accuracy on narrative questions and below 15.5% on numerical reasoning. Web augmentation has inconsistent effects (+13.6% for Gemini, -11.4% for GPT-series). Larger model size doesn't guarantee better performance.

Conclusion: There's an urgent need for language models that can support equitable, place-aware AI systems capable of engaging with fine-grained local realities across geographic and cultural contexts.
...
<details>
  <summary>Details</summary>
Motivation: Existing prompt optimization methods focus on elicitation-based strategies that search for optimal prompts within fixed parametric boundaries, but they fail to provide the factual knowledge, terminology precision, and reasoning patterns needed for knowledge-intensive tasks in specialized domains.

Method: KPPO introduces three key innovations: 1) knowledge gap filling mechanism for identifying and remediating knowledge gaps; 2) batch-wise candidate evaluation considering both performance improvement and distributional stability; 3) adaptive knowledge pruning strategy balancing performance and token efficiency.

Result: Extensive evaluation on 15 knowledge-intensive benchmarks shows KPPO achieves ~6% average performance improvement over the strongest baseline while achieving comparable or lower token consumption, with up to 29% reduction in token usage.
...
TL;DR: The paper introduces AdvancedIF benchmark for evaluating complex instruction following in LLMs and proposes RIFL, a rubric-based training pipeline that improves instruction following by 6.7% on their benchmark.


<details>
  <summary>Details</summary>
Motivation: Advanced instruction following for complex, multi-turn, and system-prompted instructions remains challenging in LLMs, with limitations in evaluation benchmarks and training signals.

Method: Proposes RIFL (Rubric-based Instruction-Following Learning) pipeline using rubric generation, finetuned rubric verifier, and reward shaping for reinforcement learning.

Result: RIFL achieves 6.7% absolute gain on AdvancedIF benchmark and strong performance on public benchmarks. Ablation studies confirm effectiveness of all components.
...
<details>
  <summary>Details</summary>
Motivation: Olympiad-level physics problem-solving requires sophisticated integration of calculation, abstract reasoning, and physical principles, making it a challenging testbed for AI capabilities.
...
Result: Achieved 313/320 points on CPhO 2025 theory exam, surpassing highest-scoring human competitor and significantly outperforming all baseline methods.

Conclusion: LOCA-R demonstrates strong performance in complex physics reasoning, establishing new benchmark for AI in olympiad-level problem-solving.
...
TL;DR: Linguistic style variations (fear, curiosity, compassion) can significantly increase jailbreak success rates in LLMs, with contextualized rewrites being more effective than templates. A style neutralization preprocessing method reduces this vulnerability.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety evaluations focus on paraphrased jailbreak prompts but overlook linguistic style variations as an attack surface for eliciting unsafe responses from aligned models.

Method: Constructed style-augmented jailbreak benchmark by transforming prompts from 3 datasets into 11 linguistic styles using templates and LLM-based rewrites. Evaluated 16 models and introduced style neutralization preprocessing using a secondary LLM.

Result: Stylistic reframing increased jailbreak success rates by up to +57 percentage points. Fearful, curious and compassionate styles were most effective, with contextualized rewrites outperforming templated variants. Style neutralization significantly reduced jailbreak success rates.

Conclusion: Linguistic style variations represent a systemic and scaling-resistant vulnerability in current LLM safety pipelines that requires attention.
...
TL;DR: A comprehensive benchmark for conversational memory evaluation with 75K+ QA pairs reveals that simple full-context approaches outperform sophisticated RAG systems in early conversations, with practical transition points at 30 and 150 conversations.


<details>
  <summary>Details</summary>
Motivation: Address fundamental challenges in statistical power, data generation consistency, and evaluation flexibility that limit current memory evaluation frameworks, and examine the relationship between conversational memory and RAG systems.

Method: Created a benchmark with 75,336 question-answer pairs across diverse categories (user facts, assistant recall, abstention, preferences, temporal changes, implicit connections) and evaluated various approaches including full-context methods and RAG-based memory systems.

Result: Simple full-context approaches achieve 70-82% accuracy on challenging multi-message evidence cases, while sophisticated RAG-based memory systems achieve only 30-45% for conversations under 150 interactions. Found practical transition points: long context excels for first 30 conversations, viable up to 150 conversations, and requires hybrid/RAG approaches beyond that.

Conclusion: Conversational memory has unique small-corpus advantages where exhaustive search and complete reranking are feasible, deserving dedicated research attention rather than simply applying general RAG solutions to conversation histories.
...
TL;DR: This paper uses computational methods to analyze genre fiction vs literary fiction, finding formal markers for each category and showing how female authorship affects literary classification.


<details>
  <summary>Details</summary>
Motivation: To explore whether genre is a formal designation or institutional one, examining the distinction between genre fiction and literary fiction using computational analysis.

Method: Used CONLIT dataset to assemble corpus of literary and genre fiction (romance, mystery, sci-fi). Applied Welch's ANOVA to compare narrative features by author gender, logistic regression to model literary classification effects, and analyzed stylistic/semantic vector representations.

Result: Found statistically significant formal markers for each literary category. Female authorship narrows and blurs the target for achieving literary status.

Conclusion: Genre has formal characteristics that can be computationally identified, and author gender significantly moderates how works achieve literary classification.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with long documents due to information interference from irrelevant content and high computational costs of Transformers. Existing solutions either sacrifice details or add complexity with external retrievers.

Method: URaG introduces a lightweight cross-modal retrieval module that converts early Transformer layers into evidence selectors to identify relevant pages, allowing deeper layers to focus on pertinent information.
...
Conclusion: URaG effectively leverages MLLMs' inherent evidence localization capabilities to perform retrieval during reasoning, enabling efficient long document understanding within a single unified model.
...
TL;DR: DESS improves Aspect Sentiment Triple Extraction by integrating DeBERTa's enhanced attention mechanism with LSTM in a dual-channel framework, achieving significant F1-score improvements on standard datasets.


<details>
  <summary>Details</summary>
Motivation: Current ASTE methods using BERT and GNNs haven't fully exploited advanced language models' potential for understanding complex language patterns and relationships between aspects, opinions, and sentiments.

Method: Dual-channel framework combining DeBERTa's enhanced attention mechanism with LSTM to process both contextual meaning and grammatical patterns, with refined component integration.

Result: Achieved F1-score improvements of 4.85, 8.36, and 2.42 on standard datasets for aspect-opinion pair identification and sentiment accuracy, with better handling of complex sentence structures and long-range dependencies.

Conclusion: Thoughtful integration of advanced language models like DeBERTa can lead to substantial improvements in fine-grained sentiment analysis, particularly for complex language patterns and distant word relationships.
...
TL;DR: This paper evaluates MedGemma for medical order extraction from doctor-patient conversations, finding that simple one-shot prompting outperforms more complex reasoning frameworks like ReAct and agentic workflows on manually annotated transcripts.


<details>
  <summary>Details</summary>
Motivation: To reduce clinical documentation burdens and ensure patient safety by accurately extracting medical orders from doctor-patient conversations, and to evaluate the new domain-specific MedGemma model for this task.
...
TL;DR: Two AI approaches for generating wound care responses: mined prompting using similar examples as demonstrations, and metadata-guided generation using predicted clinical attributes to enhance output quality.
...
Method: 1) Mined prompting: embed training data and retrieve top-k similar examples as few-shot demonstrations. 2) Metadata-guided generation: train classifiers to predict four key metadata attributes and incorporate them dynamically based on prediction confidence.

Result: Mined prompting improves response relevance, while metadata-guided generation further enhances clinical precision. Both methods show effectiveness in wound care response generation.

Conclusion: These complementary approaches demonstrate promising directions for developing reliable and efficient AI-driven tools for wound care support in remote healthcare settings.
...
TL;DR: Encoder-augmented causal decoder models achieve better compression than causal transformers and enable per-token entropy estimation, leading to improved generalization when trained to approach but not exceed estimated entropy limits.


<details>
  <summary>Details</summary>
Motivation: Current causal language models cannot efficiently estimate language entropy due to computational constraints, limiting our understanding of fundamental language compression limits and optimal training strategies.

Method: Developed encoder-augmented causal decoder architectures that are more training-efficient and achieve higher compression than causal transformers, enabling per-token entropy estimation during training.

Result: Models trained to approach but not exceed estimated per-token entropies demonstrate superior generalization compared to models trained without entropy considerations, even when using modest hardware.

Conclusion: Language entropy provides a fundamental constraint on model accuracy, and training models to approach estimated entropy limits leads to better generalization than simply minimizing loss beyond these theoretical bounds.
...
Conclusion: SSR provides both performance gains and a principled black-box approach for evaluating and understanding the internal reasoning processes of LLMs.
...
TL;DR: Instella is a family of fully open 3B parameter language models trained on open data, achieving SOTA results among fully open models and competitive with leading open-weight models of similar size.
...
Method: Large-scale pre-training on open data using AMD Instinct MI300X GPUs, followed by instruction tuning and human preference alignment. Specialized variants include Instella-Long (128K context) and Instella-Math (enhanced with SFT and RL on math tasks).
...
TL;DR: Generative Adversarial Distillation (GAD) enables black-box LLM distillation by framing the student as a generator and training a discriminator to distinguish student vs teacher responses, achieving performance comparable to proprietary models like GPT-5-Chat.


<details>
  <summary>Details</summary>
Motivation: To enable effective distillation from proprietary teacher LLMs without access to internal logits or parameters, overcoming limitations of traditional black-box distillation methods.

Method: GAD frames student LLM as generator and trains a discriminator in a minimax game to distinguish student responses from teacher responses, creating an on-policy reward model that co-evolves with the student.

Result: GAD consistently surpasses sequence-level knowledge distillation, with Qwen2.5-14B-Instruct student achieving comparable performance to GPT-5-Chat teacher on LMSYS-Chat evaluation.

Conclusion: GAD establishes a promising and effective paradigm for black-box LLM distillation, enabling students to approach teacher-level performance without access to internal model details.
...
TL;DR: ParoQuant is a weight-only post-training quantization method that uses pairwise rotations and channel-wise scaling to handle outliers in LLMs, achieving better accuracy than AWQ with minimal inference overhead.
...
Conclusion: Enables more efficient and accurate deployment of reasoning LLMs by effectively handling outliers while maintaining low inference overhead.
...
TL;DR: FedeCouple is a federated learning method that balances global generalization and local adaptability through joint learning of global/local features, dynamic knowledge distillation, and privacy-preserving anchors.


<details>
  <summary>Details</summary>
Motivation: Existing personalized federated learning methods focus on feature consistency and classification personalization but neglect local extractor adaptability and global classifier generalization, leading to poor coordination between components and degraded performance.

Method: Joint learning of global and local feature representations with dynamic knowledge distillation to enhance classifier generalization. Uses anchors with strict locality and non-transmission to refine feature space while preserving privacy and reducing communication overhead.

Result: Extensive experiments on five image-classification datasets show FedeCouple consistently outperforms nine baseline methods in effectiveness, stability, scalability, and security, achieving 4.3% improvement over the best baseline.

Conclusion: FedeCouple effectively addresses the coordination challenge in personalized federated learning by balancing global generalization and local adaptability, with theoretical convergence guarantees and superior empirical performance.
...
TL;DR: Proposes MMaDA-Parallel, a parallel multimodal diffusion framework that addresses performance degradation in thinking-aware generation by enabling continuous bidirectional interaction between text and images throughout denoising, achieving 6.9% improvement in output alignment.


<details>
  <summary>Details</summary>
Motivation: Existing sequential, autoregressive approaches in thinking-aware generation can paradoxically degrade performance due to error propagation, particularly with poor alignment between generated reasoning and final image output.

Method: Proposes a parallel multimodal diffusion framework (MMaDA-Parallel) with continuous bidirectional text-image interaction, trained with supervised finetuning and optimized via Parallel Reinforcement Learning (ParaRL) that applies semantic rewards along the trajectory.
...
Conclusion: Establishes a more robust paradigm for thinking-aware image synthesis by resolving alignment issues through parallel multimodal diffusion with continuous bidirectional interaction.
...
TL;DR: PriVi is a large-scale primate video dataset that enables improved computer vision for primate behavior analysis through data-centric pretraining, outperforming previous methods across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing computer vision methods for primate behavior analysis rely on human-centric models and single datasets, limiting generalization. There's a need for primate-specific pretraining to improve data efficiency and performance.
...
Result: Outperformed prior work including fully finetuned baselines across ChimpACT, BaboonLand, PanAf500, and ChimpBehave datasets. Showed favorable scaling with fewer labels, demonstrating improved data efficiency and generalization.
...
<details>
  <summary>Details</summary>
Motivation: Current phonotrauma severity assessment relies on costly clinician judgment with variable reliability, creating need for automated tools to enable large-scale studies and improve patient care.

Method: Proposed soft ordinal regression framework that modifies ordinal regression loss functions to handle soft labels reflecting annotator rating distributions, accounting for label uncertainty.
...
Conclusion: Automated phonotrauma severity assessment can enable large-scale studies, leading to improved clinical understanding and patient care.
...
TL;DR: SliderEdit enables continuous, fine-grained control over individual instructions in multi-instruction image editing by exposing each instruction as an adjustable slider, allowing smooth strength adjustment while preserving spatial and semantic consistency.
...
TL;DR: Enhanced crowd density estimation algorithm adapted from images to videos using diffusion-based denoising and event-driven sampling for efficient real-time monitoring.


<details>
  <summary>Details</summary>
Motivation: To address temporal challenges in video-based crowd analysis and create a scalable framework for real-time crowd monitoring in safety and event management applications.

Method: Integrates denoising probabilistic model with diffusion processes, uses narrow Gaussian kernels, regression branch for feature extraction, consolidation mechanism for density maps, and event-driven sampling with Farneback optical flow to capture significant crowd movements.

Result: Model effectively captures crowd dynamics in both dense and sparse settings, reduces computational load and storage while maintaining essential crowd events, as validated through qualitative/quantitative evaluations including MAE metrics.

Conclusion: Provides a scalable and efficient framework that successfully addresses video-specific temporal challenges for real-time crowd monitoring applications.
...
TL;DR: PALMS+ is an image-based indoor localization system that uses monocular depth estimation and geometric layout matching with floor plans to achieve accurate GPS-free localization without requiring training.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of existing vision-based indoor localization methods like PALMS, which are constrained by smartphone LiDAR's short range and indoor layout ambiguities, enabling infrastructure-free localization for emergency response and assistive navigation.
...
Result: Outperformed PALMS and F3Loc on Structured3D and custom campus dataset (80 observations across 4 buildings) in stationary localization accuracy without training. Achieved lower errors in sequential localization on 33 real-world trajectories when integrated with particle filter.

Conclusion: PALMS+ demonstrates robust infrastructure-free indoor localization suitable for camera-free tracking applications, offering improved accuracy over existing methods without requiring model training.
...
TL;DR: Proposes a novel deep learning model that enhances Social LSTM with a Dynamic Occupied Space loss function to improve pedestrian trajectory prediction by reducing collisions without increasing displacement error across various crowd densities.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat pedestrians as point entities, ignoring the physical space each person occupies, leading to unrealistic collisions in trajectory prediction models.
...
Conclusion: The proposed model successfully addresses the limitation of ignoring pedestrian physical space, improving both collision avoidance and trajectory prediction accuracy across various crowd density scenarios.
...
TL;DR: This paper treats soiling detection for automotive cameras as a semantic segmentation problem, compares segmentation methods against classification approaches, identifies data-leakage in Woodscape dataset, and creates a smaller but effective data subset.


<details>
  <summary>Details</summary>
Motivation: Soiling detection is crucial for robust advanced driver assistance systems to handle external conditions like weather and dust. Current datasets have issues with data-leakage and imprecise annotations.

Method: Treat soiling detection as semantic segmentation problem, compare popular segmentation methods against tile-level classification, analyze Woodscape dataset issues, and create new data subset with corrected annotations.

Result: Segmentation methods show superior performance over classification approaches. The new smaller data subset enables comparable results in much shorter training time despite being significantly smaller than original dataset.

Conclusion: Semantic segmentation is effective for soiling detection, and addressing dataset issues through careful data curation can lead to efficient training while maintaining performance.
...
TL;DR: Medical foundation models outperform general-domain ones for chest X-ray analysis, especially in linear probing, but their effectiveness is task-dependent. Pre-trained embeddings work well for classification and segmenting salient anatomy but struggle with subtle pathologies without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To determine how pre-training domain, paradigm, and architecture influence embedding quality in medical imaging, and to guide selection of optimal encoders for specific radiology tasks.

Method: Evaluated vision encoders from eight medical and general-domain foundation models for chest X-ray analysis, benchmarking classification (pneumothorax, cardiomegaly) and segmentation (pneumothorax, cardiac boundary) using linear probing and fine-tuning.

Result: Medical foundation models consistently outperformed general-domain models in linear probing. Pre-trained embeddings were strong for global classification and segmenting salient anatomy but performed poorly for segmenting subtle pathologies without significant fine-tuning. Text-image alignment was not essential, with image-only and label-supervised models among top performers.

Conclusion: Medical pre-training is beneficial but not universally effective. Architectural choices are critical, and pre-trained features struggle with complex localization tasks where supervised models remain competitive.
...
<details>
  <summary>Details</summary>
Motivation: Need for diverse iris datasets with realistic variations and anomalies to develop reliable iris recognition and presentation attack detection methods.

Method: Traverse generative model's latent space guided by gradients of specific iris features (sharpness, pupil size, iris size, pupil-to-iris ratio) while preserving identity, using either GAN-generated or real images via GAN inversion.

Result: The approach enables controlled manipulation of iris image properties while maintaining identity, and can be extended to any differentiable attribute.

Conclusion: The proposed method provides an effective iris image augmentation strategy that can generate diverse same-identity samples with manipulated attributes for improved iris recognition systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing 6D pose estimation methods require labor-intensive manual annotation of target masks in the first frame, leading to reduced performance with occlusions or rapid movement.

Method: Three-stage pipeline: contextual object descriptions guide localization, self-cross-attention identifies candidate regions, and segmentation produces precise masks. Includes automatic re-registration mechanism for tracking failure recovery.
...
TL;DR: PANDA is a Patch-and-Distribution-Aware Augmentation framework that addresses dual-level imbalances in Exemplar-Free Continual Learning by amplifying low-frequency classes through patch transplantation and smoothing inter-task imbalances using prior distributions.


<details>
  <summary>Details</summary>
Motivation: Real-world data streams exhibit dual-level imbalances - dataset-level distributions combined with extreme/reversed skews within individual tasks, creating intra-task and inter-task disparities that hinder effective learning and generalization in Exemplar-Free Continual Learning.
...
Conclusion: PANDA effectively addresses dual-level imbalances in real-world data streams for Exemplar-Free Continual Learning, enhancing performance of pre-trained model-based methods through patch-aware augmentation and distribution-aware balancing.
...
TL;DR: STS is a lightweight test-time adaptation method for VLMs that steers latent representations using spectral subspaces without backpropagation, achieving faster inference and smaller memory footprint than existing methods.
...
Result: STS surpasses or compares favorably against state-of-the-art test-time adaptation methods while being 8x faster with 12x smaller memory footprint than conventional test-time prompt tuning.

Conclusion: STS provides an efficient and effective test-time adaptation framework for VLMs that maintains performance while significantly reducing computational overhead.
...
TL;DR: Lumos3D is a generalizable pose-free framework for 3D low-light scene restoration that directly recovers illumination and structure from unposed multi-view images without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D low-light restoration methods depend on precomputed camera poses and scene-specific optimization, limiting scalability to dynamic real-world environments.

Method: Built on a geometry-grounded backbone, Lumos3D reconstructs normal-light 3D Gaussian representations using cross-illumination distillation and a dedicated Lumos loss for photometric consistency.

Result: Experiments show Lumos3D achieves high-fidelity restoration with accurate geometry and strong generalization to unseen cases, also extending to over-exposure correction.

Conclusion: Lumos3D provides a scalable, feed-forward solution for diverse 3D lighting restoration tasks without requiring camera poses or per-scene training.
...
TL;DR: Zero-shot cross-view image retrieval framework using pretrained vision encoder and LLM for street-to-satellite matching without training, outperforming supervised methods.


<details>
  <summary>Details</summary>
Motivation: Existing cross-view retrieval methods require supervised training on curated datasets and rely on panoramic/UAV images, limiting real-world deployment in GPS-denied environments.

Method: Extracts geographic cues from street-view images via web search and LLM location inference, generates satellite queries via geocoding API, and retrieves matches using DINOv2 with PCA-based whitening feature refinement.

Result: Outperforms prior learning-based approaches on benchmark dataset under zero-shot settings without ground-truth supervision or finetuning.
...
TL;DR: Novel framework using 3D Gaussian Splatting for animating humans in 3D scenes, enabling geometry-consistent free-viewpoint rendering without requiring paired human-scene data.


<details>
  <summary>Details</summary>
Motivation: 3DGS achieves state-of-the-art photorealistic novel-view synthesis but remains under-explored for human-scene animation and interaction. Existing pipelines use meshes or point clouds, but 3DGS offers advantages for this problem.

Method: Uses 3DGS as 3D representation for humans and scenes. Key insight: rendering can be decoupled from motion synthesis. Includes Gaussian-aligned motion module using opacity-based cues and projected Gaussian structures, plus human-scene Gaussian refinement optimization for realistic contact and navigation.

Result: Evaluated on Scannet++ and SuperSplat scenes, and avatars from sparse/dense multi-view capture. Framework enables geometry-consistent free-viewpoint rendering of edited monocular RGB videos with new animated humans.

Conclusion: Demonstrates unique advantage of 3DGS for monocular video-based human animation, enabling novel applications in human-scene interaction and animation.
...
TL;DR: CertMask is a certifiably robust defense against adversarial patch attacks that uses a single round of masking with O(n) complexity, outperforming PatchCleanser in certified accuracy while maintaining clean accuracy.


<details>
  <summary>Details</summary>
Motivation: Adversarial patch attacks pose serious risks to real-world vision systems, and existing defenses like PatchCleanser have high computational costs (O(n²)) with two masking rounds.

Method: Constructs a provably sufficient set of binary masks using a mathematically rigorous coverage strategy that ensures each patch location is covered at least k times, requiring only single-round masking.

Result: Improves certified robust accuracy by up to +13.4% over PatchCleanser on ImageNet, ImageNette, and CIFAR-10 while maintaining clean accuracy nearly identical to vanilla models.

Conclusion: CertMask provides an efficient and certifiably robust defense against adversarial patches with strong theoretical guarantees and significantly reduced computational overhead.
...
TL;DR: Adapting a solar physics foundation model to create embeddings for solar wind structure analysis, combining with spacecraft position data to build a neural field-based model for classifying solar wind structures using Parker Solar Probe measurements.


<details>
  <summary>Details</summary>
Motivation: Space weather poses risks to satellites and infrastructure, but automated classification of solar wind structures is challenging due to variable plasma properties. There's a need to bridge remote sensing and in situ observations for better space weather prediction.

Method: Fine-tune a foundation model originally trained on solar imagery to create embeddings, concatenate with spacecraft position and solar magnetic connectivity encoded via Fourier features, and build a neural field-based model for classification using Parker Solar Probe data.

Result: Overall classification performance is modest, likely due to coarse labeling, class imbalance, and limited transferability of the pretrained model, but the approach demonstrates feasibility of using foundation model embeddings for in situ solar wind tasks.

Conclusion: This proof-of-concept study successfully demonstrates the feasibility of leveraging foundation model embeddings for solar wind structure analysis and lays groundwork for future improvements in space weather prediction.
...
<details>
  <summary>Details</summary>
Motivation: Point clouds are widely used in AR and robotics where relighting and texture editing require accurate albedo-shade separation, but existing methods fail due to non-grid structure and lack of global-light consideration.

Method: Proposes IPCD-Net with point-wise feature aggregation for non-grid processing and Projection-based Luminance Distribution (PLD) with hierarchical feature refinement to capture global-light direction via multi-view projection.

Result: IPCD-Net reduces cast shadows in albedo and enhances color accuracy in shade, demonstrated on synthetic outdoor-scene dataset. Applications shown in texture editing, relighting, and point-cloud registration.

Conclusion: The method successfully extends image decomposition to point clouds, enabling accurate albedo-shade separation with real-world applicability verified.
...
TL;DR: T-DRS is an inference-only method that addresses progressive attention decay in LVLMs using three strategies to recover long-range dependencies without harming local inductive biases.
...
Method: Three-step Decay Resilience Strategies: 1) SD-DRS amplifies distant semantic signals, 2) DC-DRS modulates attention weights based on positional distance, 3) reRD-DRS reinforces remaining remote dependencies.

Result: Extensive VQA benchmark experiments show T-DRS consistently improves performance in a training-free manner.

Conclusion: T-DRS effectively mitigates attention decay in LVLMs while preserving local inductive biases, enhancing global context modeling without requiring retraining.
...
<details>
  <summary>Details</summary>
Motivation: Direct application of SAM to RGB-D VSOD faces challenges including manual prompt dependence, high memory consumption of sequential adapters, and computational burden of memory attention.
...
Conclusion: SAM-DAQ successfully addresses the limitations of direct SAM application to RGB-D VSOD through effective integration of depth and temporal information in a unified framework.
...
TL;DR: RWKV-PCSSC is a lightweight point cloud semantic scene completion network that uses RWKV mechanisms to reduce model complexity while achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing semantic scene completion approaches use dense architectures with high parameter counts, leading to increased complexity and resource demands.
...
Result: Reduces parameter count by 4.18× and improves memory efficiency by 1.37× compared to PointSSC, achieving state-of-the-art performance on indoor and outdoor scene datasets.

Conclusion: The compact and efficient design of RWKV-PCSSC enables lightweight model representation while maintaining high performance in semantic scene completion tasks.
...
TL;DR: HCC-3D introduces hierarchical compression to reduce 3D token computational costs in Vision-Language Models while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current 3D-VLMs have high computational costs due to processing all 3D tokens in LLMs, limiting their practical application.

Method: Hierarchical Compensatory Compression with global structure compression (GSC) using global queries and adaptive detail mining (ADM) for selective recompression of salient features.

Result: Achieves ~98% compression ratio while setting new state-of-the-art performance, improving both efficiency and accuracy.

Conclusion: HCC-3D successfully reduces computational overhead while preserving essential 3D information, enabling more efficient 3D-VLM applications.
...
TL;DR: Proposes Scale-Aware Relay Layer (SARL) and Scale-Adaptive Loss (SAL) to improve tiny object detection in aerial images by addressing feature degradation and regression imbalance issues.


<details>
  <summary>Details</summary>
Motivation: Modern detectors struggle with tiny object detection due to limited features being degraded during network propagation and disproportionate regression penalties for smaller objects compared to larger ones.
...
Result: Boosts generalization by 5.5% AP on AI-TOD, DOTA-v2.0 and VisDrone2019 benchmarks when embedded in YOLOv5 and YOLOx. Achieves 29.0% AP on noisy AI-TOD-v2.0 dataset.

Conclusion: The proposed SARL and SAL effectively address tiny object detection challenges and demonstrate strong performance across multiple benchmarks and real-world noisy datasets.
...
TL;DR: Swin-BART encoder-decoder with regional attention module achieves state-of-the-art medical image captioning, improving semantic fidelity while remaining interpretable.
...
Result: Achieves ROUGE 0.603 and BERTScore 0.807, outperforming baselines (ResNet-CNN ROUGE 0.356, BLIP2-OPT ROUGE 0.255) with competitive BLEU, CIDEr, and METEOR scores.
...
<details>
  <summary>Details</summary>
Motivation: Existing Single-DGOD methods use discrete data augmentation or static perturbations, which fail to capture the continuous and gradual nature of real-world domain shifts (e.g., weather/lighting changes), limiting perception of fine-grained cross-domain differences.

Method: LTFE simulates progressive feature evolution through controllable Gaussian noise injection, multi-scale Gaussian blurring, temporal modeling, and liquid neural network-driven parameter adjustment to generate adaptive modulation parameters for smooth cross-domain adaptation.
...
Conclusion: LTFE effectively bridges source-unknown domain distribution gaps by capturing progressive cross-domain feature evolution and dynamically regulating adaptation paths, significantly boosting generalization capabilities.
...
Method: Developed DocWeaver, a multi-agent pipeline using Large Language Models to automatically generate MosaicDoc - a bilingual benchmark with 72K images from newspapers/magazines featuring diverse layouts, rich styles from 196 publishers, and comprehensive annotations.

Result: Created MosaicDoc with 72K images and 600K+ QA pairs, featuring complex layouts (multi-column, non-Manhattan), bilingual content (Chinese/English), and multi-task annotations (OCR, VQA, reading order, localization).

Conclusion: Evaluation on MosaicDoc reveals current VLMs' limitations in handling real-world document complexity and provides a clear path for future VRDU research.
...
TL;DR: Proposes SLDC method to mitigate distribution drift in sequential fine-tuning for class-incremental learning, using linear and weakly nonlinear transition operators to align feature distributions across tasks.


<details>
  <summary>Details</summary>
Motivation: Sequential fine-tuning of ViTs for CIL suffers from distribution drift due to sequential optimization, causing mismatch between learned class distributions and updater model, degrading classifier performance over time.

Method: Introduces SLDC with linear variant (solving regularized least-squares) and weakly nonlinear variant (learnable mappings between linear and nonlinear). Combines with knowledge distillation to reduce representation drift.

Result: SLDC significantly improves SeqFT performance on CIL benchmarks. When combined with KD, achieves performance comparable to joint training across all evaluated datasets.

Conclusion: SLDC effectively compensates for distribution drift in sequential fine-tuning, enabling competitive CIL performance through feature distribution alignment and drift compensation mechanisms.
...
Conclusion: The proposed framework effectively addresses the key challenges in adversarial defense for person ReID, achieving superior robustness through debiased training and dual-invariant generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Vision Graph Neural Networks (ViGs) face computational challenges from their graph construction phase, which hinders efficiency.

Method: Proposes AdaptViG with Adaptive Graph Convolution using a static axial scaffold and dynamic Exponential Decay Gating for selective long-range connections, plus a hybrid strategy with Global Attention in final stage.

Result: Achieves SOTA trade-off: AdaptViG-M gets 82.6% top-1 accuracy (outperforms ViG-B by 0.3%) with 80% fewer parameters and 84% fewer GMACs. On downstream tasks, surpasses EfficientFormer-L7 with 78% fewer parameters.

Conclusion: AdaptViG provides an efficient and powerful hybrid Vision GNN that significantly reduces computational costs while achieving superior performance.
...
Method: Proposes TSPE-GS which uniformly samples transmittance to model pixel-wise multi-modal opacity and depth distributions, progressively fuses truncated signed distance functions to reconstruct external and internal surfaces separately.

Result: Significantly improves semi-transparent geometry reconstruction while maintaining performance on opaque scenes, generalizes to other Gaussian-based pipelines without extra training overhead.

Conclusion: TSPE-GS successfully addresses the limitation of single-depth assumptions in Gaussian Splatting for semi-transparent surfaces through probabilistic multi-modal modeling, providing a unified framework for both transparent and opaque scene reconstruction.
...
<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based NR-IQA methods rely solely on semantic similarity between image embeddings and text prompts, ignoring the important quality cue from CLIP image feature magnitudes which show strong correlation with perceptual quality.

Method: Extract absolute CLIP image features, apply Box-Cox transformation for statistical normalization, and use confidence-guided fusion to adaptively combine the magnitude-aware cue with cosine similarity.
...
Conclusion: The proposed magnitude-aware quality cue effectively complements semantic similarity, and the adaptive fusion framework provides superior NR-IQA performance without requiring task-specific training.
...
TL;DR: A pipeline using vision-language models to generate pseudo-labels for training efficient real-time object detectors, with per-object co-teaching to handle noisy labels, achieving significant performance improvements on autonomous driving datasets.


<details>
  <summary>Details</summary>
Motivation: Foundation models offer zero-shot object detection but suffer from high latency and hallucinations, making them unsuitable for direct deployment in autonomous driving where manual labeling is expensive.
...
TL;DR: EquS is a diffusion model-based image restoration method that uses dual sampling trajectories with equivariant information to better leverage diffusion priors, achieving superior performance without increased computational cost.
...
Method: Proposed EquS with dual sampling trajectories for equivariant information, and EquS+ with Timestep-Aware Schedule (TAS) that prioritizes deterministic steps for enhanced certainty and efficiency.

Result: Extensive experiments show the method is compatible with previous approaches and significantly boosts their performance without additional computational costs.

Conclusion: The proposed EquS and EquS+ methods effectively address limitations of current diffusion model-based image restoration approaches by better leveraging diffusion priors through dual sampling trajectories and optimized scheduling.
...
TL;DR: DiVE preserves geometric structure during fine-tuning of vision-language models by constraining difference vectors between pre-trained and fine-tuned embeddings to be equal across samples, maintaining strong OOD and zero-shot performance.
...
Method: Proposes Difference Vector Equalization (DiVE) with two losses: average vector loss (AVL) for global geometric structure preservation by constraining difference vectors to their weighted average, and pairwise vector loss (PVL) for local geometric structure preservation through consistent multimodal alignment.
...
Conclusion: DiVE successfully fine-tunes vision-language models on ID data without compromising their generalization abilities in OOD and zero-shot settings by preserving the geometric structure of embeddings.
...
TL;DR: STELLAR is a diffusion-based Scene Text Editing model that addresses limitations in multilingual support, domain gap, and evaluation metrics through language-adaptive glyph encoding, multi-stage training, and a novel Text Appearance Similarity metric.
...
Method: Proposes STELLAR with language-adaptive glyph encoder, multi-stage training (pre-training on synthetic data then fine-tuning on real images), and constructs STIPLAR dataset. Also introduces Text Appearance Similarity metric for evaluation.
...
Conclusion: STELLAR effectively addresses key STE challenges through multilingual support, real-world adaptation, and robust evaluation metrics, demonstrating superior performance in text style preservation.
...
TL;DR: MOBA is a physically realizable backdoor attack framework for LiDAR-based 3D object detection that models material properties to bridge the digital-physical gap, achieving 93.50% attack success rate.
...
Method: Systematic material selection (TiO₂ for high diffuse reflectivity) and novel simulation pipeline with angle-independent Oren-Nayar BRDF approximation and distance-aware scaling for realistic LiDAR intensity modeling.
...
TL;DR: DBGroup is a weakly supervised 3D instance segmentation framework that uses scene-level annotations instead of point-level or bounding box supervision, achieving competitive performance with reduced annotation costs.


<details>
  <summary>Details</summary>
Motivation: To address the high annotation costs and labor-intensive processes in fully supervised 3D instance segmentation, and overcome limitations of existing weakly supervised methods that still require significant annotation effort.

Method: Two-stage framework: 1) Dual-Branch Point Grouping with semantic and mask cues from multi-view images to generate pseudo labels, refined by Granularity-Aware Instance Merging and Semantic Selection; 2) Multi-round self-training with Instance Mask Filter to handle pseudo label inconsistencies.

Result: Achieves competitive performance compared to sparse-point-level supervised methods and surpasses state-of-the-art scene-level supervised 3D semantic segmentation approaches.

Conclusion: Scene-level annotations provide an efficient and scalable alternative for 3D instance segmentation, with DBGroup demonstrating effective pseudo label generation and refinement strategies that reduce annotation burden while maintaining performance.
...
TL;DR: LampQ is a layer-wise mixed precision quantization method for Vision Transformers that addresses limitations of previous approaches through fine-grained control, type-aware sensitivity metrics, and optimal bit allocation.


<details>
  <summary>Details</summary>
Motivation: Existing ViT quantization methods use uniform precision, ignoring the diverse sensitivity of different components. Previous mixed precision methods suffer from coarse granularity, metric scale mismatch, and quantization-unaware bit allocation.

Method: Proposes LampQ with layer-wise quantization for fine-grained control, type-aware Fisher-based sensitivity metric, optimal bit allocation via integer linear programming, and iterative bit-width updates.
...
Conclusion: LampQ effectively overcomes limitations of previous mixed precision quantization methods for Vision Transformers, providing superior accuracy preservation across various vision tasks.
...
TL;DR: MIRNet is a medical image analysis framework that combines self-supervised pre-training with graph-based reasoning to address annotation scarcity, label imbalance, and clinical plausibility in medical image interpretation, achieving state-of-the-art performance on tongue diagnosis.


<details>
  <summary>Details</summary>
Motivation: To address challenges in automated medical image interpretation including annotation scarcity, label imbalance, and clinical plausibility constraints, particularly in complex domains like tongue diagnosis that require fine-grained visual and semantic understanding.

Method: Integrates self-supervised masked autoencoder (MAE) for visual representation learning, graph attention networks (GAT) for modeling label correlations, constraint-aware optimization using KL divergence and regularization losses for clinical priors, and asymmetric loss (ASL) with boosting ensembles for imbalance mitigation.

Result: Achieves state-of-the-art performance on tongue diagnosis and introduces TongueAtlas-4K, the largest public dataset in tongue analysis with 4,000 images and 22 diagnostic labels.

Conclusion: The MIRNet framework effectively addresses key challenges in medical image analysis and generalizes well to broader diagnostic medical imaging tasks beyond tongue diagnosis.
...
TL;DR: Fine-grained 3D Embodied Reasoning task requiring agents to predict spatial location, motion type, and motion axis for affordance elements in 3D scenes based on task instructions, with AffordBot framework achieving state-of-the-art performance.
...
Method: AffordBot integrates MLLMs with chain-of-thought reasoning, rendering surround-view images of 3D scenes and projecting element candidates into views, then using active perception to select viewpoints and step-by-step reasoning for localization and motion inference.
...
Conclusion: The proposed framework effectively bridges 3D input with 2D-compatible MLLMs and enables fine-grained embodied reasoning through structured CoT paradigm, advancing human-agent collaboration in physical environments.
...
TL;DR: Anomagic is a zero-shot anomaly generation method that creates realistic anomalies without needing example anomalies, using crossmodal prompts and contrastive refinement to improve anomaly detection.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generating semantically coherent anomalies without requiring exemplar anomalies, leveraging rich contextual information from both visual and textual cues.

Method: Unifies visual and textual cues through crossmodal prompt encoding, uses inpainting-based generation pipeline, and applies contrastive refinement to align synthesized anomalies with their masks. Trained on AnomVerse dataset of 12,987 anomaly-mask-caption triplets.
...
TL;DR: DGFusion is a dual-guided multi-modal 3D object detection method that addresses hard instance detection (distant, small, occluded objects) through difficulty-aware instance pairing and dual-guided feature fusion.
...
Method: Proposes DGFusion with Difficulty-aware Instance Pair Matcher (DIPM) to generate easy/hard instance pairs, and Dual-guided Modules that leverage both pair types for effective multi-modal feature fusion.
...
Conclusion: The dual-guided paradigm effectively addresses hard instance detection limitations in autonomous driving by leveraging complementary strengths of both point-guide-image and image-guide-point paradigms.
...
TL;DR: A novel 3D VAE framework using unsigned distance fields (UDFs) with local-to-global architecture for high-fidelity 3D content generation, handling complex topologies and scaling to ultra-high resolutions.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of existing methods like signed distance fields (SDFs) that require costly watertight preprocessing and struggle with non-manifold geometries, and point-cloud representations that suffer from sampling artifacts and surface discontinuities.

Method: 3D variational autoencoder built on unsigned distance fields (UDFs) with local-to-global architecture that partitions UDF into uniform subvolumes (UBlocks), using 3D convolutions for local detail and sparse transformers for global coherence, plus Pad-Average strategy for smooth transitions.

Result: State-of-the-art performance in reconstruction accuracy and generative quality, achieving superior surface smoothness and geometric flexibility, with seamless scaling to ultra-high resolutions up to 2048^3.

Conclusion: The proposed UDF-based 3D VAE with local-to-global architecture provides a robust solution for generating high-fidelity 3D content, effectively handling complex topologies while enabling unprecedented resolution scaling.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from information imbalance between visible and infrared modalities in complex scenarios, and most ignore the advantages of frequency domain transformers for mining complementary information.
...
Conclusion: The proposed frequency domain fusion transformer effectively addresses multimodal information imbalance and improves visible-infrared object detection performance through frequency domain attention and cross-modal feature interaction.
...
<details>
  <summary>Details</summary>
Motivation: Existing zero-shot methods overlook the key discriminative property that normal image patches across industrial products typically find many similar patches in both 2D appearance and 3D shapes, while anomalies remain diverse and isolated.

Method: Proposes Mutual Scoring framework with Iterative Point Grouping for 3D representation, Similarity Neighborhood Aggregation with Multi-Degrees for feature fusion, Mutual Scoring Mechanism for intra-modal scoring, Cross-modal Anomaly Enhancement for fusing 2D/3D scores, and Re-scoring with Constrained Neighborhood to suppress false classifications.
...
Conclusion: The framework flexibly works on full datasets and smaller subsets with robust performance, ensuring adaptability across diverse product lines, demonstrating the effectiveness of leveraging neighborhood similarity patterns for zero-shot anomaly detection.
...
TL;DR: Proposes HCM-GRPO method with Hard Cases Mining and Dynamic Proportional Accuracy reward to improve image aesthetic reasoning in MLLMs, using a new 128k-sample dataset covering four aesthetic aspects.


<details>
  <summary>Details</summary>
Motivation: Image screening performance with MLLMs is unsatisfactory due to lack of data and weak image aesthetic reasoning ability, with current state-of-the-art models performing at random guessing levels.

Method: Collects comprehensive 128k-sample dataset with 640k images, uses multiple annotation approaches for CoT data, and introduces HCM-GRPO with Hard Cases Mining strategy and Dynamic Proportional Accuracy reward in GRPO framework.

Result: HCM-GRPO demonstrates superior image aesthetic reasoning capabilities compared to original GRPO, enabling smaller models to surpass both large-scale open-source and leading closed-source models like GPT4o and Qwen-VL-Max.

Conclusion: The proposed solution addresses data scarcity and weak reasoning in image screening, showing that proper methodology can overcome limitations of current MLLMs in aesthetic evaluation tasks.
...
TL;DR: The paper introduces AV-ConfuseBench to test MLLMs' ability to detect audio-absent objects in videos, finding they struggle due to visual dominance. It proposes RL-CoMM, a reinforcement learning approach using audio-only reasoning to improve audio-visual discrimination.
...
Method: Introduces RL-CoMM with two stages: 1) Uses a Large Audio Language Model as reference for audio-only reasoning with Step-wise Reasoning Reward for self-improvement, 2) Implements Answer-centered Confidence Optimization to reduce reasoning uncertainty.
...
Conclusion: The proposed RL-CoMM framework effectively addresses visually dominated reasoning in MLLMs and significantly improves their ability to handle audio-visual confusion scenarios.
...
TL;DR: GaussMedAct is a multivariate Gaussian encoding framework for fine-grained medical action evaluation that achieves 92.1% Top-1 accuracy on the new CPREval-6k benchmark, outperforming ST-GCN by +5.9% with only 10% FLOPs.


<details>
  <summary>Details</summary>
Motivation: Address challenges in medical action evaluation including lack of comprehensive datasets, stringent precision requirements, and insufficient spatiotemporal modeling of rapid actions.

Method: Multivariate Gaussian Representation projects joint motions to temporally scaled space and decomposes actions into adaptive 3D Gaussian tokens. Hybrid Spatial Encoding uses Cartesian and Vector dual-stream strategy with joint and bone features.

Result: Achieves 92.1% Top-1 accuracy on CPREval-6k benchmark with real-time inference, outperforming ST-GCN baseline by +5.9% accuracy using only 10% FLOPs. Cross-dataset experiments confirm robustness.

Conclusion: The proposed GaussMedAct framework effectively addresses medical action evaluation challenges through adaptive spatiotemporal representation learning and demonstrates superior performance and efficiency.
...
TL;DR: CABIN is a semi-supervised framework that addresses confidence bias in hyperspectral image classification by using epistemic uncertainty estimation and dynamic pseudo-label assignment to improve generalization under sparse annotations.


<details>
  <summary>Details</summary>
Motivation: Models in hyperspectral image classification often mistake high confidence for correctness, leading to confirmation bias and poor generalization under sparse annotations or class imbalance, where they overfit confident errors.

Method: CABIN uses a closed-loop process: (1) perceptual awareness through epistemic uncertainty estimation to identify ambiguous regions, (2) uncertainty-guided dual sampling for exploration vs. anchoring confident samples, and (3) fine-grained dynamic assignment categorizing pseudo-labels into reliable/ambiguous/noisy subsets with tailored losses.

Result: Experimental results show that various state-of-the-art methods benefit from CABIN integration, achieving improved labeling efficiency and performance.

Conclusion: CABIN effectively addresses confidence bias in hyperspectral classification through cognitive-aware learning, enhancing model generalization and reducing overfitting to confident errors.
...
<details>
  <summary>Details</summary>
Motivation: Existing semantic communication techniques process each modality separately, requiring multiple streams and reducing spectral efficiency. There's a need for a unified approach that can handle both vision and language modalities efficiently.

Method: Uses a pre-trained vision-language model to encode source images into vision-language semantic features (VLF), which are transmitted over wireless channels. At the receiver, both a decoder-based language model and diffusion-based image generator use the VLF to produce text descriptions and semantically aligned images.
...
Method: Proposes GlobalDiff with global joint rotations and multi-level constraints: joint structure constraints with virtual anchor points, skeleton structure constraints for angular consistency, and temporal structure constraints using multi-scale variational encoder.

Result: Improves performance by 46.0% compared to state-of-the-art methods on standard co-speech benchmarks, generating smooth and accurate motions.

Conclusion: Operating in global rotation space with structural constraints effectively eliminates hierarchical error accumulation and produces more reliable co-speech motion generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing visual token pruning methods focus only on "what to select" using attention scores, leading to inefficient spatial allocation, positional bias, and retention of irrelevant tokens. Humans use a two-stage strategy of first deciding "where to look" before "what to select".

Method: GridPrune replaces global Top-K with a zonal selection system: 1) text-conditional guidance dynamically allocates token budgets across spatial zones, 2) local selection within each budgeted zone.

Result: On LLaVA-NeXT-7B, GridPrune retains 96.98% of full performance using only 11.1% of tokens, outperforming the best baseline by 2.34% at same pruning rate. Superior performance across various MLLM architectures.

Conclusion: The two-stage "guide-globally, select-locally" approach inspired by human visual attention effectively addresses inefficiencies in existing pruning methods, achieving better performance with significantly reduced computational overhead.
...
Method: SUGAR pipeline: 1) Use large-scale video models to generate visual-motion knowledge, 2) Supervise skeleton learning with this knowledge to create discrete representations, 3) Use pre-trained LLMs to understand these representations, 4) Temporal Query Projection (TQP) module for modeling long skeleton sequences.

Result: Experiments show SUGAR is effective on skeleton-based action classification benchmarks and more versatile than linear-based methods in zero-shot scenarios.

Conclusion: SUGAR successfully bridges LLMs with skeleton data for action recognition, demonstrating strong performance and zero-shot generalization capabilities.
...
TL;DR: MTAttack is a novel multi-target backdoor attack framework for Large Visual Language Models that enables multiple independent triggers to map to different malicious outputs in a single training pass, overcoming feature interference challenges.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks on LVLMs only target single malicious outputs, but real-world threats require multi-target attacks where multiple triggers correspond to different attack targets, posing greater security risks.

Method: Proposes MTAttack with two optimization constraints: Proxy Space Partitioning and Trigger Prototype Anchoring, which jointly optimize multiple triggers in latent space to independently map clean images to unique proxy classes while ensuring trigger separability.

Result: Achieves high success rate for multi-target attacks, substantially outperforming existing methods, with strong generalizability across datasets and robustness against backdoor defenses.

Conclusion: LVLMs are vulnerable to multi-target backdoor attacks, highlighting urgent need for mitigation strategies against such sophisticated threats.
...
TL;DR: RobIA: A robust instance-aware framework for continual test-time adaptation in stereo depth estimation that dynamically adapts to domain shifts using attention-based expert routing and pseudo-supervision.


<details>
  <summary>Details</summary>
Motivation: Address challenges in stereo depth estimation including dynamic domain shifts, sparse supervision, and high cost of dense ground-truth labels, overcoming limitations of existing TTA methods that assume static domains.

Method: Integrates Attend-and-Excite Mixture-of-Experts (AttEx-MoE) for dynamic input routing using self-attention tailored to epipolar geometry, and Robust AdaptBN Teacher for dense pseudo-supervision complementing sparse labels.
...
TL;DR: Proposes CACMI, a context-aware cross-modal interaction framework for dense video captioning that explicitly models temporal coherence and semantic context through cross-modal frame aggregation and query-guided feature enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on implicit modeling with frame-level or fragmented video features, failing to capture temporal coherence across event sequences and comprehensive semantics within visual contexts.

Method: CACMI framework with two components: Cross-modal Frame Aggregation for extracting temporally coherent event-aligned textual features, and Context-aware Feature Enhancement using query-guided attention to integrate visual dynamics with pseudo-event semantics.

Result: Achieves state-of-the-art performance on ActivityNet Captions and YouCook2 datasets for dense video captioning.

Conclusion: The proposed explicit temporal-semantic modeling framework effectively captures both temporal characteristics and linguistic semantics, outperforming existing methods in dense video captioning.
...
<details>
  <summary>Details</summary>
Motivation: To investigate why current text-to-image models fundamentally fail at handling logical composition despite their impressive capabilities on single concepts, exposing a critical architectural flaw.

Method: Analysis across three core logical primitives (negation, counting, spatial relations) using recent benchmarks, examining training data distribution, attention architectures, and evaluation metrics.

Result: Models show dramatic performance collapse when logical primitives are combined, with severe interference effects. Training data lacks explicit negations, continuous attention is unsuitable for discrete logic, and metrics prioritize visual plausibility over constraint satisfaction.
...
TL;DR: Proposes split-layer MLP architecture that divides layers into parallel branches combined via Hadamard product, enhancing implicit neural representation capacity without quadratic computational cost growth.


<details>
  <summary>Details</summary>
Motivation: Implicit neural representations (INR) have limited representational capacity due to low-dimensional feature spaces in conventional MLPs, and widening MLPs leads to quadratic computational/memory cost growth.

Method: Split-layer reformulation of MLP construction that divides each layer into multiple parallel branches and integrates outputs via Hadamard product, constructing high-degree polynomial space.

Result: Significantly enhances INR representational capacity without prohibitive computational overhead, outperforming existing methods in 2D image fitting, 2D CT reconstruction, 3D shape representation, and 5D novel view synthesis.

Conclusion: Split-layer architecture effectively expands feature space dimensionality and improves INR performance across multiple tasks while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Current fairness-enhanced detectors often improve fairness at the cost of detection accuracy, which is problematic for trustworthy deployment in digital identity security where biases toward demographic groups can cause systemic misjudgments and social inequities.
...
Conclusion: GEA successfully addresses cross-modal alignment challenges in text-to-image person retrieval through generative intermediate representations.
...
<details>
  <summary>Details</summary>
Motivation: Real-world images often contain multiple degradations simultaneously (rain, noise, haze), but most existing methods target only single degradation types. Current methods also lack interpretability when stacking modules for performance improvement.

Method: Uses a deep unfolding network based on an improved second-order semi-smooth Newton algorithm, with an explainable convolution module inspired by human brain processing and image characteristics for flexible parameter adjustment.
...
Conclusion: The proposed interpretability-driven approach successfully addresses multi-degradation image restoration while maintaining clear physical interpretability and competitive performance across both multi- and single-degradation scenarios.
...
TL;DR: CephRes-MHNet: A multi-head residual CNN for cephalometric landmark detection that achieves state-of-the-art accuracy with high efficiency.


<details>
  <summary>Details</summary>
Motivation: Manual annotation of cephalometric landmarks is time-consuming and error-prone, while automated methods struggle with low contrast and anatomical complexity in 2D lateral skull X-rays.
...
Result: Achieved mean radial error of 1.23 mm and success detection rate @ 2.0 mm of 85.5%, outperforming all evaluated models including AFPF-Net while using less than 25% of parameters.
...
TL;DR: Evaluation of NASA/IBM's Prithvi-EO-2.0 model for shoreline delineation on small sandy islands shows strong performance even with minimal training data (5 images), achieving F1 of 0.94 and IoU of 0.79.
...
Result: Models achieved high performance with as few as 5 training images (F1 of 0.94, IoU of 0.79), demonstrating strong transfer learning capability.
...
Method: VISTA uses: (i) cross-attention fusion module integrating long-horizon intent with past motion, (ii) social-token attention for flexible interaction modeling, and (iii) pairwise attention maps for interpretable social influence patterns.
...
Method: Uses Noise-to-Coarse (N2C) Module for single-pass denoising instead of iterative sampling, followed by Refine Module for precise completion. Implements Distance-aware Selected Repeat strategy to handle LiDAR's distance-dependent spatial distribution.

Result: On SemanticKITTI: 199.8x inference speedup, 50.7% reduction in Chamfer Distance, and only 6.1% parameters compared to LiDiff.
...
<details>
  <summary>Details</summary>
Motivation: Existing V2X frameworks struggle with multi-modal heterogeneity across agents and lack scalability for new participants, requiring effective feature alignment and practical adaptation methods.

Method: Uses heterogeneous graph attention as foundation, with Local Heterogeneous Fine-Tuning using Hetero-Aware Adapters and Global Collaborative Fine-Tuning using Multi-Cognitive Adapter for cross-agent collaboration.

Result: Achieves superior perception performance on OPV2V-H and DAIR-V2X datasets with significantly reduced training overhead compared to state-of-the-art approaches.

Conclusion: HeatV2X provides an effective and scalable solution for V2X collaborative perception that handles heterogeneity while minimizing training costs.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal deepfake detection methods struggle with generalization across unseen manipulations and require pretraining on real samples. They focus mainly on audio-visual inconsistencies and may miss intra-modal artifacts, especially when manipulations preserve audio-visual alignment.

Method: Proposes a single-stage training framework incorporating next-frame prediction for both uni-modal and cross-modal features. Introduces window-level attention mechanism to capture discrepancies between predicted and actual frames for detecting local artifacts.

Result: The model demonstrates strong generalization across multiple benchmark datasets and achieves precise temporal localization of deepfake segments in partially spoofed samples.

Conclusion: The proposed approach effectively addresses limitations of existing methods by enabling single-stage training with improved generalization capabilities and accurate temporal localization through next-frame prediction and attention mechanisms.
...
Result: Outperforms existing methods on VidSTG and HCSTVG benchmarks, effectively mitigating target identification errors and inconsistent tracking.
...
TL;DR: The paper introduces JudgeMind, a novel Action Quality Assessment method that simulates professional referees' scoring mindset by segmenting videos into stages, using stage-aware feature enhancement, and incorporating deduction knowledge for more accurate and reliable sports action scoring.
...
Result: The method achieves state-of-the-art performance on the newly constructed aerial skiing dataset with fine-grained sub-score and deduction annotations.

Conclusion: JudgeMind significantly enhances AQA performance and reliability by simulating professional referees' judgment process through stage segmentation, feature enhancement, and knowledge-based scoring.
...
<details>
  <summary>Details</summary>
Motivation: Traditional facial emotion recognition lacks explainable reasoning, and current VLM-based approaches suffer from hallucinated reasoning and misalignment between emotion reasoning and recognition due to insufficient emotion-specific knowledge.

Method: Three-stage framework: 1) Instruction fine-tuning for basic emotional reasoning, 2) Reinforcement training using emotion and AU labels as rewards to align reasoning with predictions, 3) Iterative data synthesis pipeline to expand training data for self-improvement.

Result: Achieves state-of-the-art performance across eight standard benchmarks, demonstrates strong generalization and robust interpretability, and introduces FEA-20K dataset with 17,737 training and 1,688 test samples.

Conclusion: Facial-R1 effectively addresses hallucinated reasoning and misalignment challenges in FEA, providing a scalable framework with minimal supervision that outperforms existing methods while maintaining strong interpretability.
...
TL;DR: H3Former is a token-to-region framework for fine-grained visual classification that uses high-order semantic relations and hyperbolic hierarchical contrastive learning to improve discriminative feature aggregation.


<details>
  <summary>Details</summary>
Motivation: Existing FGVC methods fail to comprehensively capture discriminative cues and introduce category-agnostic redundancy due to limitations in feature-selection mechanisms and region-proposal strategies.
...
Conclusion: H3Former effectively addresses limitations of existing FGVC methods by leveraging high-order semantic relations and structured region-level modeling for improved fine-grained classification.
...
TL;DR: PROPA introduces a novel framework combining MCTS with GRPO to generate dense process-level rewards for optimizing multi-step visual reasoning in VLMs without human annotations, achieving significant performance gains.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with complex visual reasoning where early errors cascade through reasoning chains, and existing methods either require costly step-level annotations (SFT) or provide only sparse outcome-level feedback (RLVR), hindering stable optimization.

Method: PROPA integrates Monte Carlo Tree Search (MCTS) with GRPO to generate dense process-level rewards, interleaves GRPO updates with SFT to overcome cold-start, and trains a Process Reward Model (PRM) to guide inference-time search.
...
Conclusion: PROPA establishes strong reasoning and generalization capability for visual reasoning tasks by optimizing reasoning at each intermediate step without human annotations through process-level reward generation and alignment.
...
TL;DR: Evaluates smaller VLMs (500M and 2.2B parameters) for accessibility-focused video descriptions, introducing BLV-specific evaluation frameworks and testing on mobile devices.
...
Method: Evaluates SmolVLM2 variants on AVCaps and Charades datasets using two novel BLV evaluation frameworks, tests prompt strategies, and deploys on smartphones with different precision levels.

Result: Not specified in abstract - appears to be a study design paper introducing evaluation frameworks.

Conclusion: Not specified in abstract - focuses on introducing accessibility-focused evaluation methods for smaller VLMs.
...
TL;DR: RUDDER is a low-overhead framework that mitigates object hallucination in LVLMs by using visual evidence vectors and adaptive token-wise injection during standard forward passes, achieving comparable performance to state-of-the-art methods with minimal computational latency.


<details>
  <summary>Details</summary>
Motivation: LVLMs suffer from object hallucination that undermines reliability, and existing inference-time interventions incur substantial computational overhead, limiting their practicality for real-world deployments.

Method: Uses CARD vectors extracted from residual updates during standard forward passes and a Bayesian-inspired adaptive gate for token-wise corrective signal injection based on deviation from visual context.
...
TL;DR: GRAM is a test-time adaptation framework for satellite-based slum segmentation that uses Mixture-of-Experts to generalize across regions without target labels, outperforming SOTA methods in low-resource settings.


<details>
  <summary>Details</summary>
Motivation: Satellite-based slum mapping can estimate global urban poverty, but morphological heterogeneity of informal settlements prevents models from generalizing to unseen regions.

Method: Two-phase test-time adaptation with Mixture-of-Experts architecture: source training on million-scale dataset from 12 cities, then adaptation using prediction consistency to filter unreliable pseudo-labels without target region labels.
...
<details>
  <summary>Details</summary>
Motivation: The LLaVA architecture struggles with integrating visual features due to modality mismatch between text and vision. This work addresses this by repurposing LLMs as vision encoders.

Method: Three key modifications: (1) separate QKV projections for vision modality, (2) bidirectional attention on visual tokens, (3) incorporation of both global and local visual representations.

Result: LLaViT significantly outperforms baseline LLaVA on multiple benchmarks and even surpasses models with double its parameter count.
...
TL;DR: Proposes new error categories and metrics for pedestrian detection evaluation, achieving SOTA on CityPersons-reasonable with a simple architecture.


<details>
  <summary>Details</summary>
Motivation: Current pedestrian detection benchmarks have weaknesses and metrics prohibit realistic performance evaluation of DNNs for safety-critical automated driving systems.

Method: Uses image segmentation to automatically distinguish error types, proposes 8 error categories and new metrics, compares various backbones for simplified APD.

Result: Achieves state-of-the-art on CityPersons-reasonable without extra training data using a simple architecture, enables more fine-grained and robust model comparison.

Conclusion: The proposed error categories and metrics provide a more realistic and safety-critical performance evaluation framework for pedestrian detection systems.
...
TL;DR: CLIP4VI-ReID is a novel network that uses CLIP-driven modality-shared representation learning for visible-infrared person re-identification, achieving state-of-the-art performance through text semantic generation, infrared feature embedding, and high-level semantic alignment.


<details>
  <summary>Details</summary>
Motivation: To address the significant modality gap between visible and infrared images in person re-identification by leveraging CLIP's capabilities to create shared representations across modalities using text as a bridge.

Method: Proposes three main components: Text Semantic Generation (TSG) for visible images, Infrared Feature Embedding (IFE) to rectify infrared features using generated text semantics, and High-level Semantic Alignment (HSA) to refine semantic alignment and ensure only ID-related information is preserved.

Result: Extensive experiments show CLIP4VI-ReID achieves superior performance compared to other state-of-the-art methods on widely used VI-ReID datasets.

Conclusion: The proposed method effectively bridges the visible-infrared modality gap using text as an intermediate representation, enabling more accurate cross-modal alignment and enhancing the discriminability of learned shared representations.
...
TL;DR: A novel 3D reconstruction framework that integrates depth-of-field supervision and multi-view consistency supervision to address extreme depth variations in scenes, achieving improved depth fidelity in both near-field and far-field regions.
...
Conclusion: The framework bridges physical imaging principles and learning-based depth regularization, offering a scalable solution for complex depth stratification in urban environments.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods tend to detect only the most salient anomalies while neglecting diverse normal patterns, and suffer from category confusion due to similar appearances, leading to unsatisfactory fine-grained classification.

Method: Proposes DSANet with: 1) coarse-grained self-guided normality modeling branch that reconstructs video features using learned normal prototypes, and 2) fine-grained decoupled contrastive semantic alignment that temporally decomposes videos into event/background components and applies visual-language contrastive learning.
...
Conclusion: The proposed disentangled semantic alignment approach effectively enhances the distinguishability between normal and abnormal patterns at both coarse and fine-grained levels, achieving superior video anomaly detection performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based methods for SDG object detection overlook feature distribution structure and frequency-domain characteristics, limiting robustness to unseen domains.

Method: Integrates vMF distribution to model directional features for domain-invariant semantics, and Fourier-based augmentation that perturbs amplitude/phase components to simulate domain shifts.
...
Conclusion: The proposed framework effectively enhances SDG object detection by combining semantic alignment, feature structure modeling, and frequency-domain augmentation.
...
TL;DR: DermAI is a smartphone-based app for real-time skin lesion analysis that addresses dataset bias and validation limitations through on-device quality checks and local model adaptation using diverse clinical data.


<details>
  <summary>Details</summary>
Motivation: AI-based dermatology faces adoption barriers due to biased datasets, variable image quality, and limited validation, which DermAI aims to overcome.

Method: Developed a lightweight smartphone app with real-time capture, annotation, and classification capabilities, featuring on-device quality checks and local model adaptation using a diverse clinical dataset.

Result: Models trained on public datasets failed to generalize to DermAI samples, but fine-tuning with local data improved performance, demonstrating the value of standardized, diverse data collection.
...
<details>
  <summary>Details</summary>
Motivation: Geospatial foundation models often fail in environments underrepresented during pretraining, creating reliability issues for climate-sensitive applications where accurate predictions are critical.

Method: Integrates three complementary signals: OOD detection in input space, OOD detection in embedding space, and task-specific predictive uncertainty. Links failure flags to land cover attributes from HydroATLAS.

Result: OOD scores correlate with lower performance in specific environmental conditions. Uncertainty-based flags help discard poorly performing predictions. Failures concentrate in certain geographies like low-elevation zones and large river areas due to pretraining underrepresentation.

Conclusion: SHRUG-FM provides a pathway toward safer and more interpretable deployment of geospatial foundation models in climate-sensitive applications, helping bridge the gap between benchmark performance and real-world reliability.
...
TL;DR: MSGNav is a zero-shot navigation system using Multi-modal 3D Scene Graphs (M3DSG) that preserves visual cues through image-based relations instead of text-only compression, addressing limitations of existing methods while solving the last-mile problem in navigation.


<details>
  <summary>Details</summary>
Motivation: To enable open vocabulary generalization and low training overhead for robotic navigation, avoiding the high construction cost, irreversible visual loss, and constrained vocabularies of existing zero-shot methods that use text-only scene graphs.

Method: Proposes M3DSG with image-based relational edges, plus MSGNav system with Key Subgraph Selection, Adaptive Vocabulary Update, Closed-Loop Reasoning modules, and Visibility-based Viewpoint Decision for last-mile problem.

Result: Achieves state-of-the-art performance on GOAT-Bench and HM3D-OVON datasets, demonstrating superior navigation capabilities compared to existing zero-shot methods.

Conclusion: MSGNav effectively addresses key limitations in zero-shot navigation through multi-modal scene representation and specialized modules, providing a robust solution for real-world robotic deployment with open vocabulary support.
...
<details>
  <summary>Details</summary>
Motivation: To address critical limitations in existing privacy protection methods for personalized AI applications, particularly the vulnerability to purification attacks that can remove adversarial perturbations and restore model's ability to memorize user identities.

Method: Proposed AntiDB_Purify evaluation framework to systematically test existing defenses under realistic purification threats including traditional image filters and adversarial purification techniques.

Result: Revealed that none of current defense methods maintain protective effectiveness under purification threats - adversarial examples are both perceptible and fragile, easily removed by simple filters.

Conclusion: Current defenses provide false sense of security; there is urgent need for more imperceptible and robust protection mechanisms to truly safeguard user identity in personalized generation systems.
...
<details>
  <summary>Details</summary>
Motivation: Real-world lane detection faces challenges like background clutter, varying illumination, and occlusions. Data-driven approaches require substantial effort for data collection and annotation, necessitating methods that leverage contextual and global information.

Method: Proposed SAMIRO (Spatial Attention Mutual Information Regularization) with a pretrained model as Oracle. It transfers knowledge from pretrained models while preserving domain-agnostic spatial information, and can be integrated into various lane detection approaches.
...
Conclusion: SAMIRO is an effective plug-and-play method that enhances lane detection performance by leveraging pretrained model knowledge while maintaining spatial information, demonstrating consistent improvements across various models and datasets.
...
TL;DR: A physics-informed Transformer-VAE architecture that uses simulated data only to invert the PROSAIL radiative transfer model for vegetation parameter retrieval from Sentinel-2 imagery, achieving performance comparable to methods requiring real satellite data.
...
Method: Transformer-VAE architecture that incorporates PROSAIL radiative transfer model as a differentiable physical decoder, trained exclusively on simulated data without requiring real satellite imagery or in-situ labels.
...
Conclusion: The approach demonstrates that integrating physical models with deep networks enables cost-effective, self-supervised vegetation monitoring without real imagery requirements, opening prospects for large-scale physically-constrained remote sensing.
...
TL;DR: MonkeyOCR v1.5 is a unified vision-language framework for document parsing that handles complex layouts with multi-level tables, embedded images/formulas, and cross-page structures through a two-stage pipeline with specialized modules for table parsing and merging.
...
Conclusion: MonkeyOCR v1.5 provides an effective unified framework for complex document parsing, successfully addressing challenges in layout understanding and content recognition for real-world documents with complex structures.
...
TL;DR: GrounDiff is a diffusion-based framework for generating Digital Terrain Models from Digital Surface Models, outperforming state-of-the-art methods with significant error reduction and introducing a variant for road reconstruction.


<details>
  <summary>Details</summary>
Motivation: Traditional DTM generation methods require manual parameter tuning or complex architectures with post-processing. There's a need for more automated and effective approaches to remove non-ground structures from DSMs.

Method: Uses diffusion-based framework to iteratively remove non-ground structures as denoising task. Incorporates gated design with confidence-guided generation and Prior-Guided Stitching for scalability using downsampled global priors.

Result: Reduces RMSE by up to 93% on ALS2DTM and 47% on USGS benchmarks. For road reconstruction, achieves up to 81% lower distance error on GeRoD benchmark while maintaining competitive surface smoothness.

Conclusion: GrounDiff provides an effective diffusion-based solution for DTM generation that outperforms existing methods across multiple benchmarks, with specialized variants for different applications like road reconstruction.
...
TL;DR: Proposes an integrated framework combining YOLOMS with LLM for wind turbine fault detection and diagnosis, achieving 90.6% detection accuracy and 89% accurate maintenance reports.
...
Method: Combines YOLOMS with multi-scale detection and sliding-window cropping for enhanced fault feature extraction, uses lightweight KV mapping module to convert visual outputs to structured textual representations, and employs domain-tuned LLM for semantic reasoning.
...
TL;DR: GT++ and 3DFETUS algorithms automate fetal facial plane localization in 3D ultrasound, reducing operator dependency and improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Standard fetal facial plane acquisition in ultrasound is challenging due to fetal movement, orientation variability, and operator expertise differences, leading to inconsistencies and diagnostic bias.

Method: 1) GT++ algorithm estimates standard facial planes from 3D US volumes using anatomical landmarks; 2) 3DFETUS deep learning model automates plane localization in 3D fetal US volumes.
...
Conclusion: The proposed approach effectively standardizes fetal facial plane localization, reducing operator dependency and improving diagnostic consistency in ultrasound examinations.
...
Method: Used TimeSformer transformer-based video classifier with five-fold cross-validation and strict subject-wise partitioning to prevent data leakage. Dataset contains 10,101 negative and 2,952 positive video clips from 19 subjects.
...
Conclusion: The dataset and baseline code are publicly released to enable reproducible research in preclinical epilepsy monitoring, demonstrating effective video-based seizure detection.
...
TL;DR: HIT uses semantic segmentation to extract glands from whole slide images as biologically meaningful patches for multiple-instance learning, improving cancer detection accuracy and interpretability while reducing computational load.


<details>
  <summary>Details</summary>
Motivation: Current digital pathology pipelines use grid-based tiling that ignores tissue architecture, introducing irrelevant information and limiting interpretability for cancer grading.

Method: Histology-informed tiling (HIT) uses semantic segmentation to extract glands from whole slide images as input patches for multiple-instance learning and phenotyping, trained on 137 samples from ProMPT cohort.

Result: HIT achieved gland-level Dice score of 0.83 +/- 0.17, improved MIL models AUCs by 10% for detecting CNVs in EMT and MYC genes, revealed 15 gland clusters associated with cancer relapse, oncogenic mutations, and high Gleason scores.
...
<details>
  <summary>Details</summary>
Motivation: To lower the entry barrier for researchers and practitioners wanting to experiment with SRGANs, compare models reproducibly, and deploy super-resolution pipelines across diverse Earth-observation datasets.

Method: Provides unified implementation of SRGAN-style models with generators, discriminators, loss functions, and training schedules exposed through concise configuration files, making it easy to switch architectures, scale factors, and band setups.

Result: A practical tool and benchmark implementation that ships with ready-to-use configurations for common remote sensing scenarios, sensible default settings for adversarial training, and built-in hooks for logging, validation, and large-scene inference.

Conclusion: By turning GAN-based super-resolution into a configuration-driven workflow, OpenSR-SRGAN makes SRGAN experimentation and deployment more accessible for Earth Observation applications.
...
<details>
  <summary>Details</summary>
Motivation: Early detection of Type 2 Diabetes is crucial but the role of pancreatic surface lobularity hasn't been fully investigated despite known associations with pancreas volume and fat content.
...
Result: Pancreatic surface lobularity was significantly higher in diabetic patients (4.26±8.32 vs 3.19±3.62, p=0.01). PancAP model achieved best segmentation (Dice: 0.79±0.17). T2DM prediction model attained 0.90 AUC, 66.7% sensitivity, and 91.9% specificity.
...
TL;DR: SPOT is a framework that uses token embeddings, interactions, and attention dynamics to detect and eliminate redundant tokens in Vision Transformers, achieving up to 40% efficiency gains while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers have high computational demands that scale quadratically with token count. Compact attention representations can guide early detection and reduction of less salient tokens before attention computation.

Method: SPOT employs lightweight predictors that analyze token embeddings, interactions, and attention dynamics across layers to infer token importance. It can be plugged into various ViT architectures for input-specific token prioritization.
...
Conclusion: SPOT provides a versatile framework for token sparsification in Vision Transformers that improves computational efficiency without sacrificing performance, adaptable to varying resource constraints.
...
<details>
  <summary>Details</summary>
Motivation: Classical Total Variation (TV) depends on a fixed lambda parameter, limiting efficiency and making it difficult to use effectively for image denoising.
...
Result: SemanticVLA surpasses OpenVLA on LIBERO benchmark by 21.1% in success rate, while reducing training cost and inference latency by 3.0-fold and 2.7-fold respectively.

Conclusion: SemanticVLA sets a new state-of-the-art in both performance and efficiency for robotic manipulation tasks, demonstrating the effectiveness of semantic-aligned sparsification and enhancement in VLA models.
...
TL;DR: Proposes Separate-then-Map (StM) strategy for reconstructing dynamic humans in real-world environments from monocular videos, using a mapping mechanism to bridge separately modeled components and ensure spatial/visual coherence.


<details>
  <summary>Details</summary>
Motivation: Existing approaches either model scenes holistically (neglecting distinct motion characteristics) or separately (causing spatial inconsistencies and artifacts at human-scene boundaries), limiting reconstruction quality.
...
Conclusion: The StM strategy effectively addresses limitations of previous approaches by enabling information exchange between separately modeled components, achieving superior reconstruction quality for dynamic human-scene interactions.
...
Method: Systematic diversity assessment using human evaluation templates, curated prompt sets with identified variation factors, binomial tests for model comparison, and rigorous comparison of image embeddings for diversity measurement.

Result: The framework enables ranking of T2I models by diversity and identifies specific categories where models struggle, providing insights for improvement.

Conclusion: This research offers a robust methodology for T2I diversity evaluation that paves the way for model improvements and metric development.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for visual stylization rely on complex inputs like lengthy text prompts or reference images, struggling with style consistency and limited creativity. The paper aims to simplify style generation by proving that a single numerical code can represent a complete visual style.

Method: 1) Train discrete style codebook from image collection to extract style embeddings 2) Use embeddings as conditions for text-to-image diffusion model 3) Train autoregressive style generator on discrete embeddings to model their distribution and synthesize novel styles 4) Map numerical codes to style embeddings during inference to guide image generation

Result: CoTyle successfully generates novel and consistent visual styles from numerical codes, demonstrating that a style can be effectively represented by a single code. The method provides unparalleled simplicity and diversity compared to existing approaches.

Conclusion: The paper validates that 'a style is worth one numerical code' and introduces the first open-source method for code-to-style image generation, filling a gap in academic research and enabling reproducible style generation from minimal input.
...
TL;DR: OmniVGGT is a framework that enhances 3D foundation models by incorporating geometric modalities like depth and camera parameters using a GeoAdapter with zero-initialized convolutions, enabling stable training and efficient inference with arbitrary modality inputs.


<details>
  <summary>Details</summary>
Motivation: Most 3D foundation models rely solely on RGB inputs and ignore available geometric cues such as camera intrinsics, poses, and depth maps, limiting their performance and practical utility.

Method: Proposes a GeoAdapter with zero-initialized convolutions to encode depth and camera parameters into spatial foundation models, along with stochastic multimodal fusion that randomly samples modality subsets during training for robust representation learning.

Result: OmniVGGT achieves state-of-the-art results on monocular/multi-view depth estimation, multi-view stereo, and camera pose estimation, and enhances vision-language-action models for improved robotic task performance.

Conclusion: OmniVGGT effectively integrates geometric information into 3D foundation models, enabling superior performance with both RGB-only and auxiliary inputs while maintaining efficiency and practical applicability in robotic tasks.
...
TL;DR: M&M-3D enables 3D reasoning for Digital Breast Tomosynthesis (DBT) without adding parameters compared to its 2D FFDM counterpart, allowing direct weight transfer and addressing data scarcity issues.


<details>
  <summary>Details</summary>
Motivation: Limited annotated DBT data constrains deep learning model development, while existing methods either discard volumetric information or require complex architectures with more training data.

Method: Proposes M&M-3D architecture that constructs malignancy-guided 3D features and learns 3D reasoning by repeatedly mixing 3D features with slice-level information through modified operations without adding parameters.

Result: Outperforms 2D projection and 3D slice-based methods by 11-54% for localization and 3-10% for classification; surpasses complex 3D variants by 20-47% for localization and 2-10% for classification in low-data regime.

Conclusion: M&M-3D effectively enables 3D reasoning for DBT while maintaining parameter efficiency, achieving state-of-the-art performance on the BCS-DBT benchmark with 4% improvement for classification and 10% for localization.
...
TL;DR: A novel Multitask Glocal OBIA-Mamba (MSOM) approach for Sentinel-2 land use classification that combines object-based analysis with Mamba architecture and global-local dual-branch modeling to achieve superior accuracy and detail preservation.


<details>
  <summary>Details</summary>
Motivation: Sentinel-2 based land use classification faces challenges like spatial heterogeneity, context information needs, and signature ambiguity, requiring improved methods that can handle these data complexities effectively.

Method: Three key innovations: 1) OBIA-Mamba model using superpixels as tokens to reduce redundancy while preserving details, 2) Global-local dual-branch CNN-Mamba architecture for joint local and global modeling, 3) Multitask optimization with dual loss functions to balance local precision and global consistency.
...
Conclusion: The MSOM framework successfully addresses key challenges in Sentinel-2 classification by integrating object-based analysis, Mamba architecture, and multitask optimization, demonstrating superior performance over existing methods.
...
Result: Achieves comparable perceptual quality to pixel-space SR with 3x lower decoding time (only +0.42s for 1024px from 512px vs 1.87s for pixel-space SR).

Conclusion: LUA provides an efficient path to scalable high-fidelity image synthesis, matching native high-resolution quality while being easily deployable across different VAEs.
...
TL;DR: Depth Anything 3 (DA3) is a minimal transformer-based model that predicts consistent geometry from multiple visual inputs without requiring camera poses, achieving state-of-the-art performance across camera pose estimation, multi-view geometry, and visual rendering tasks.


<details>
  <summary>Details</summary>
Motivation: To develop a minimal yet powerful geometry prediction model that works with arbitrary visual inputs without requiring known camera poses, simplifying the architecture and training process while maintaining high performance.

Method: Uses a single plain transformer backbone (vanilla DINO encoder) without architectural specialization, employs a singular depth-ray prediction target to avoid complex multi-task learning, and implements a teacher-student training paradigm.

Result: Sets new SOTA on visual geometry benchmark: 44.3% improvement in camera pose accuracy and 25.1% improvement in geometric accuracy over prior SOTA VGGT, while outperforming Depth Anything 2 in monocular depth estimation.

Conclusion: DA3 demonstrates that minimal modeling with a plain transformer backbone and simple depth-ray prediction can achieve superior geometry understanding across multiple tasks without architectural complexity or multi-task learning requirements.
...
TL;DR: Self-Consistency Sampling (SCS) addresses the problem of unfaithful reasoning in multimodal LLMs by using visual perturbations and trajectory resampling to create a differentiable consistency score that improves RL training.


<details>
  <summary>Details</summary>
Motivation: Current outcome-reward RL for multimodal LLMs has a flaw where incorrect reasoning that happens to guess the right answer gets the same reward as genuine reasoning, undermining the learning process.

Method: SCS introduces small visual perturbations and performs repeated truncation/resampling of initial trajectories, then uses agreement among resulting trajectories to create a consistency score that down-weights unreliable reasoning traces during policy updates.

Result: SCS improves accuracy by up to 7.7 percentage points on six multimodal benchmarks when integrated with RLOO, GRPO, and REINFORCE++ series, with negligible extra computation. Benefits also extend to other models like Qwen2.5-VL-3B-Instruct and InternVL3-8B.

Conclusion: SCS provides a simple, general solution to the unfaithful reasoning problem in outcome-reward RL for multimodal LLMs, significantly improving performance across multiple models and benchmarks.
...
### [174] [Simulating Misinformation Propagation in Social Networks using Large Language Models](https://arxiv.org/abs/2511.10384)
*Raj Gaurav Maurya,Vaibhav Shukla,Raj Abhijit Dandekar,Rajat Dandekar,Sreedath Panat*

Main category: cs.SI

TL;DR: LLM personas simulate human biases in social networks, showing how misinformation propagates through identity-driven rewrites, with expert personas maintaining factual stability while ideological ones accelerate distortion.


<details>
  <summary>Details</summary>
Motivation: To understand how misinformation spreads through human cognitive biases, ideological alignments, and trust heuristics in social media environments.

Method: Model LLM personas as synthetic agents with user-level biases, use auditor-node framework to track misinformation propagation through sequential rewrites, and quantify factual degradation using misinformation index and propagation rate metrics.

Result: Identity- and ideology-based personas accelerate misinformation, especially in politics, marketing, and technology, while expert personas preserve factual stability. Early distortions in heterogeneous networks rapidly escalate to propaganda-level misinformation.

Conclusion: LLMs serve dual roles as proxies for human biases and as auditors for tracking information fidelity, providing an interpretable framework for studying and mitigating misinformation diffusion in digital ecosystems.
...
<details>
  <summary>Details</summary>
Motivation: LLMs underperform in real-world medical scenarios due to lack of context-awareness (missing user identity, medical history, risk factors) needed for safe and appropriate responses.

Method: Multifaceted Self-Refinement (MuSeR) uses attribute-conditioned query generation to simulate diverse user contexts, then LLMs self-evaluate responses along three facets and refine them, followed by supervised fine-tuning with refined data.

Result: Significant performance improvement on HealthBench, with Qwen3-32B achieving new SOTA (63.8% overall, 43.1% on hard subset) through knowledge distillation.

Conclusion: MuSeR effectively enhances LLMs' medical context-awareness and safety through systematic self-reflection and refinement, enabling better real-world medical applications.
...
TL;DR: ProgRAG is a multi-hop KGQA framework that decomposes complex questions into sub-questions, progressively extends reasoning paths, uses uncertainty-aware pruning for evidence refinement, and optimizes reasoning context to improve reliability and performance.


<details>
  <summary>Details</summary>
Motivation: Current KG-enhanced LLMs face challenges with inaccurate retrieval, reasoning failures, long input contexts obscuring relevant information, and premature reasoning due to LLMs directly retrieving and self-assessing evidence from knowledge graphs.

Method: Decomposes complex questions into sub-questions, progressively extends partial reasoning paths by answering each sub-question, uses external retrievers for candidate evidence, applies uncertainty-aware pruning by LLM to refine evidence, and optimizes reasoning context by organizing partial reasoning paths.
...
Conclusion: ProgRAG effectively addresses retrieval and reasoning failures in KG-enhanced LLMs through progressive decomposition and uncertainty-aware evidence refinement, leading to superior performance in multi-hop knowledge graph question answering.
...
TL;DR: FactGuard is a novel fake news detection framework that uses LLMs to extract event-centric content, reducing style dependency, and employs dynamic usability mechanisms with knowledge distillation for efficient deployment.


<details>
  <summary>Details</summary>
Motivation: Traditional style-based fake news detection is becoming less effective as adversaries imitate authentic writing styles, while LLMs remain underutilized due to shallow exploration, ambiguous usability, and high costs.

Method: Proposes FactGuard framework that leverages LLMs for event-centric content extraction, introduces dynamic usability mechanism to identify contradictions and ambiguous cases, and uses knowledge distillation to create FactGuard-D for cold-start and resource-constrained scenarios.

Result: Comprehensive experiments on two benchmark datasets show the approach consistently outperforms existing methods in both robustness and accuracy.

Conclusion: FactGuard effectively addresses style sensitivity and LLM usability challenges in fake news detection, providing a practical and efficient solution for real-world deployment.
...
TL;DR: EgoEMS is the first comprehensive egocentric dataset for EMS training, featuring 20+ hours of simulated emergency scenarios with multimodal data and annotations to support AI cognitive assistant development.
...
Method: Created EgoEMS dataset with 233 simulated emergency scenarios from 62 participants (including 46 EMS professionals) using an open-source, low-cost data collection system. Includes multimodal data with keystep annotations, audio transcripts, action quality metrics, and segmentation masks.

Result: Developed a high-fidelity dataset capturing realistic EMS activities with responder-patient interactions, plus benchmark tasks for real-time multimodal keystep recognition and action quality estimation.
...
TL;DR: A reasoning-based problem generator that creates adaptive difficulty problems by using solver feedback as reward signal, achieving 2.5% average improvement on reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing data synthesis methods generate indiscriminate problems without considering solver ability or lack reasoning in problem generation, leading to low-value or shallow problems.

Method: Construct related problem pairs with intermediate problem-design CoT, use solver feedback as reward signal to calibrate difficulty, and enable generator-solver co-evolution.
...
Conclusion: The reasoning-based problem generator with adaptive difficulty calibration and co-evolution framework effectively creates high-quality training data for large reasoning models.
...
TL;DR: The paper addresses the sim-to-real gap in cyber physical systems testing by developing a method to validate simulated failure scenarios using real-world sensor data.


<details>
  <summary>Details</summary>
Motivation: To ensure that failure scenarios discovered in simulation environments are reproducible on actual systems in the real world, overcoming the sim-to-real gap caused by differences between simulated and real sensor data.
...
Result: The algorithm is more accurate and orders of magnitude faster than state-of-the-art commercial vision large language models for querying scenarios, and scales effectively with the duration of time series data.
...
### [181] [Efficient Automated Diagnosis of Retinopathy of Prematurity by Customize CNN Models](https://arxiv.org/abs/2511.10023)
...
<details>
  <summary>Details</summary>
Motivation: To enhance ROP diagnosis through advanced deep learning methods, addressing the need for precise and efficient detection while reducing computational burdens associated with traditional deep neural networks.

Method: Employed CNN-based approaches with focus on dataset curation, preprocessing strategies, and model architecture optimization. Implemented a voting system to enhance performance and developed customized CNN models tailored for ROP detection.

Result: Customized CNN models outperformed pre-trained counterparts with higher accuracy and F1-scores. The voting system further improved performance. The proposed model demonstrated potential to reduce computational burdens while maintaining effectiveness.

Conclusion: The study successfully demonstrates the efficacy of deep learning models in enhancing ROP diagnostic precision and efficiency, with practical deployment feasibility in clinical settings as valuable diagnostic aids.
...
TL;DR: LLM-based agents show stronger skepticism against erroneous messages, enabling better Byzantine fault tolerance than traditional agents. The proposed CP-WBFT consensus mechanism leverages LLMs' reflective capabilities to enhance MAS reliability across diverse topologies.


<details>
  <summary>Details</summary>
Motivation: To investigate whether replacing traditional agents with LLM-based agents can improve multi-agent system reliability, particularly from a Byzantine fault tolerance perspective, as this reliability aspect remains largely unexplored despite LLM agents' breakthroughs in complex problem solving.
...
Conclusion: LLM-based agents demonstrate enhanced reliability through stronger skepticism, and the proposed CP-WBFT mechanism effectively leverages LLM capabilities to significantly improve multi-agent system stability and Byzantine fault tolerance across different network structures.
...
### [183] [Music Flamingo: Scaling Music Understanding in Audio Language Models](https://arxiv.org/abs/2511.10289)
...
<details>
  <summary>Details</summary>
Motivation: Current audio-language models struggle with music due to its complex nature and lack of high-quality data, limiting them to surface-level understanding and poor cross-cultural generalization.

Method: Curated MF-Skills dataset with rich annotations, fine-tuned enhanced Audio Flamingo 3 backbone, and introduced post-training with MF-Think chain-of-thought dataset and GRPO-based reinforcement learning.

Result: Achieved state-of-the-art results across 10+ music understanding benchmarks, demonstrating superior reasoning and generalization capabilities.

Conclusion: Music Flamingo sets a new standard for advanced music understanding, moving beyond surface recognition to human-like perception and providing a foundation for future music AI development.
...
### [184] [VEDA: 3D Molecular Generation via Variance-Exploding Diffusion with Annealing](https://arxiv.org/abs/2511.09568)
...
Method: VEDA combines variance-exploding diffusion with annealing through: (1) VE schedule enabling noise injection analogous to simulated annealing, (2) novel preconditioning scheme reconciling SE(3)-equivariant networks with residual-based diffusion, and (3) arcsin-based scheduler concentrating sampling in critical intervals.

Result: On QM9 and GEOM-DRUGS datasets, VEDA matches flow-based model efficiency with only 100 sampling steps, achieving state-of-the-art valency stability and validity. Generated structures show remarkable stability with median relaxation energy of only 1.72 kcal/mol vs 32.3 kcal/mol from baseline.
...
### [185] [Probability-Biased Attention over Directed Bipartite Graphs for Long-Tail ICD Coding](https://arxiv.org/abs/2511.09559)
*Tianlei Chen,Yuxiao Chen,Yang Li,Feifei Wang*

Main category: cs.LG

TL;DR: Proposes a directed bipartite graph encoder with co-occurrence encoding to improve automated ICD coding, especially for rare codes, by leveraging statistical relationships between common and rare disease codes.


<details>
  <summary>Details</summary>
Motivation: Automated ICD coding faces challenges due to large label space (10,000-20,000 codes) and long-tail distribution where rare codes lack sufficient training data, requiring methods to handle this imbalance.

Method: Constructs a Directed Bipartite Graph Encoder with common and rare code nodes, using directed edges from common to rare codes with probability-based bias derived from conditional co-occurrence probabilities. Uses LLM-generated code descriptions for enriched embeddings.

Result: Achieves state-of-the-art performance on three automated ICD coding benchmark datasets, with particularly notable improvements in Macro-F1 (key metric for long-tail classification).

Conclusion: The proposed co-occurrence encoding method effectively enriches rare code representations by leveraging statistical relationships with common codes, demonstrating superior performance in handling long-tail distribution in automated ICD coding.
...
### [186] [OutSafe-Bench: A Benchmark for Multimodal Offensive Content Detection in Large Language Models](https://arxiv.org/abs/2511.10287)
*Yuping Yan,Yuhan Xie,Yuanshuai Li,Yingchao Yu,Lingjuan Lyu,Yaochu Jin*

Main category: cs.LG

TL;DR: OutSafe-Bench is a comprehensive multimodal safety evaluation benchmark covering text, images, audio, and videos across 9 risk categories, with novel metrics for assessing overlapping risks and fair automated evaluation.
...
Method: Created large-scale bilingual dataset with 18K+ text prompts, 4.5K images, 450 audio clips, 450 videos across 9 risk categories. Introduced MCRS metric for overlapping risk assessment and FairScore framework using top-performing models as adaptive juries for fair evaluation.

Result: Evaluation of 9 state-of-the-art MLLMs revealed persistent and substantial safety vulnerabilities, highlighting significant safety gaps in current models.

Conclusion: There is a pressing need for robust safeguards in MLLMs, as current models demonstrate serious safety deficiencies across multiple modalities and risk categories.
...
### [187] [AgentEvolver: Towards Efficient Self-Evolving Agent System](https://arxiv.org/abs/2511.10395)
...
TL;DR: AgentEvolver is a self-evolving agent system that uses LLMs to enable autonomous learning through self-questioning, self-navigating, and self-attributing mechanisms, overcoming limitations of traditional RL approaches.


<details>
  <summary>Details</summary>
Motivation: Current autonomous agent development is costly and inefficient due to manual dataset construction and RL pipelines with poor exploration efficiency and sample utilization.

Method: Three synergistic mechanisms: self-questioning for curiosity-driven task generation, self-navigating for efficient exploration through experience reuse, and self-attributing for enhanced sample efficiency via differentiated rewards.

Result: Preliminary experiments show AgentEvolver achieves more efficient exploration, better sample utilization, and faster adaptation compared to traditional RL baselines.

Conclusion: AgentEvolver enables scalable, cost-effective, and continual improvement of agent capabilities through its unified self-evolving framework.
...
### [188] [Impact of Layer Norm on Memorization and Generalization in Transformers](https://arxiv.org/abs/2511.10566)
*Rishi Singhal,Jung-Eun Kim*

Main category: cs.LG

TL;DR: LayerNorm's impact on learning vs memorization differs between Pre- and Post-LayerNorm transformers, with Pre-LayerNorm relying on it for stable learning while Post-LayerNorm uses it for memorization control.


<details>
  <summary>Details</summary>
Motivation: To understand how LayerNorm influences memorization and learning differently in Pre-LayerNorm vs Post-LayerNorm transformer architectures, as this remains unclear despite Pre-LayerNorm's popularity for stable gradient flow.

Method: Analyzed the impact of LayerNorm by eliminating its parameters in both Pre- and Post-LayerNorm transformers, examining memorization patterns and learning stability across different layer positions.

Result: Removing LayerNorm parameters in Pre-LayerNorm models increases memorization and destabilizes learning, while in Post-LayerNorm models it reduces memorization by restoring genuine labels. Early layers' LayerNorm are most critical.

Conclusion: LayerNorm plays distinct roles in Pre- vs Post-LayerNorm transformers - crucial for stable learning in Pre-LayerNorm and for controlling memorization in Post-LayerNorm, with early layers being most influential.
...
### [189] [Towards Emotionally Intelligent and Responsible Reinforcement Learning](https://arxiv.org/abs/2511.10573)
*Garapati Keerthana,Manik Gupta*

Main category: cs.LG

TL;DR: Proposes a Responsible Reinforcement Learning (RRL) framework that integrates emotional context and ethical constraints into personalized decision systems, addressing limitations of traditional rule-based approaches.


<details>
  <summary>Details</summary>
Motivation: Static rule-based and engagement-maximizing systems overlook users' emotional context and ethical constraints, risking insensitive or unsafe interventions in sensitive domains like mental health and substance use disorders.

Method: Formulates personalization as a Constrained Markov Decision Process (CMDP) with multi-objective reward function balancing short-term engagement and long-term well-being, using emotion-informed state representations and safety-constrained RL algorithms.
...
Conclusion: Initiates methodological conversation about ethically aligned reinforcement learning for emotionally aware and trustworthy personalization systems in human-centric domains like behavioral health and digital therapeutics.
...
### [190] [PRISM: Diversifying Dataset Distillation by Decoupling Architectural Priors](https://arxiv.org/abs/2511.09905)
...
TL;DR: PRISM is a dataset distillation framework that uses multiple teacher models to overcome single-model bias, improving synthetic data diversity and generalization performance on ImageNet-1K.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods suffer from single teacher model bias, leading to overly smooth and homogeneous synthetic samples with reduced intra-class diversity, which limits generalization.

Method: PRISM decouples logit-matching and regularization objectives, using different teacher architectures: a primary model for logits and a stochastic subset for batch-normalization alignment. It also introduces scalable cross-class batch formation for parallel synthesis.

Result: PRISM consistently outperforms single-teacher methods and recent multi-teacher variants on ImageNet-1K, especially at low- and mid-IPC regimes. Generated data shows significantly richer intra-class diversity with notable drop in cosine similarity between features.

Conclusion: Using diverse teacher models to disentangle architectural priors during synthesis improves dataset distillation quality, producing more diverse and generalizable synthetic data.
...
### [191] [eXIAA: eXplainable Injections for Adversarial Attack](https://arxiv.org/abs/2511.10088)
*Leonardo Pesce,Jiawen Wei,Gianmarco Mengaldo*

Main category: cs.LG

TL;DR: A new black-box adversarial attack that manipulates post-hoc XAI explanations without changing model predictions, requiring only access to predictions and explanations.
...
Method: Single-step black-box attack that modifies original explanations using only model predictions and explanations (saliency maps, integrated gradients, DeepLIFT SHAP), tested on ResNet-18 and ViT-B16 models on ImageNet.

Result: The attack successfully generates dramatically different explanations without changing predictive probabilities, with effectiveness validated using mean absolute difference for explanation changes and SSIM for image similarity.
...
### [192] [How does My Model Fail? Automatic Identification and Interpretation of Physical Plausibility Failure Modes with Matryoshka Transcoders](https://arxiv.org/abs/2511.10094)
*Yiming Tang,Abhijeet Sinha,Dianbo Liu*

Main category: cs.LG

TL;DR: Matryoshka Transcoders is a framework for automatically discovering and interpreting physical plausibility failures in generative models by extending Matryoshka representation learning to transcoder architectures.


<details>
  <summary>Details</summary>
Motivation: Current generative models often produce physically implausible outputs that escape detection by existing evaluation methods, and there's no framework for automatically identifying specific physical error patterns in natural language.

Method: Extends Matryoshka representation learning to transcoder architectures for hierarchical sparse feature learning, trains on intermediate representations from physical plausibility classifiers, and uses large multimodal models for interpretation.
...
### [193] [Intrinsic Dimensionality as a Model-Free Measure of Class Imbalance](https://arxiv.org/abs/2511.10475)
...
Method: Use data Intrinsic Dimensionality (ID) as an imbalance measure that can be incorporated into various imbalance mitigation methods. Compare ID-based approaches with cardinality-based re-weighting and re-sampling techniques.

Result: Across five datasets with diverse imbalance ratios, ID consistently outperforms cardinality-based methods. Combining ID with cardinality further improves performance.

Conclusion: Intrinsic Dimensionality is an effective, model-free measure for class imbalance that can enhance existing imbalance mitigation techniques and works well when combined with traditional cardinality measures.
...
### [194] [Trapped by Their Own Light: Deployable and Stealth Retroreflective Patch Attacks on Traffic Sign Recognition Systems](https://arxiv.org/abs/2511.10050)
*Go Tsuruoka,Takami Sato,Qi Alfred Chen,Kazuki Nomoto,Ryunosuke Kobayashi,Yuna Tanaka,Tatsuya Mori*

Main category: cs.CR

TL;DR: ARP is a stealthy adversarial attack using retroreflective patches that activate under headlights, achieving high success rates against traffic sign recognition systems while being hard to detect.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks on traffic sign recognition systems have visual detectability or implementation constraints, leaving unexplored vulnerabilities that need investigation.

Method: Developed Adversarial Retroreflective Patch (ARP) using retroreflective materials activated by headlights, with retroreflection simulation and black-box optimization for attack effectiveness.

Result: ARP achieves ≥93.4% success rate at 35 meters in dynamic scenarios and ≥60% against commercial TSR systems, with better stealthiness than previous attacks.

Conclusion: ARP demonstrates serious security vulnerabilities in TSR systems, and proposed DPR Shield defense using polarized filters achieves ≥75% defense success against such attacks.
