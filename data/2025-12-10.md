...
TL;DR: The paper investigates the "short-context dominance hypothesis" - that most sequences only need a small local prefix (96 tokens) to predict next tokens accurately. The authors develop methods to detect challenging long-context sequences and mitigate bias in LLM outputs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how much context is actually needed for accurate next-token prediction in LLMs, given the computational cost of processing long contexts. The authors want to test whether the "short-context dominance hypothesis" holds and develop practical methods to handle cases where longer context is actually necessary.

Method: 1) Measure Minimum Context Length (MCL) needed for accurate predictions across datasets; 2) Introduce Distributionally Aware MCL (DaMCL) as a practical proxy that doesn't require next-token knowledge; 3) Develop a decoding algorithm that uses DaMCL to detect and boost long-range-relevant tokens.

Result: For sequences from long-context documents (1-7k tokens), 75-80% require only the last 96 tokens at most. DaMCL effectively detects long vs. short context sequences, and the proposed decoding algorithm improves performance across Q&A tasks and model architectures by mitigating short-context bias.

Conclusion: The short-context dominance hypothesis is largely valid, but there are important cases requiring longer context. The proposed DaMCL metric and decoding algorithm provide practical solutions to identify and handle these challenging long-context sequences, improving LLM performance while acknowledging the efficiency of short-context processing.
...
TL;DR: A scalable pipeline trains specialized retrieval models from unlabeled financial corpus using iterative student-teacher interaction, achieving significant improvements in financial document retrieval metrics without human annotation.


<details>
  <summary>Details</summary>
Motivation: General-purpose embedding models underperform in specialized domains like finance, while practical deployment of conversational AI agents faces computation costs, latency requirements, and need for precise domain-specific relevance measures.

Method: Uses iterative student-teacher interaction: starts with general-purpose retrieval embedding model as foundation, then interleaves retrieval-based mining of hard positive/negative examples from unlabeled corpus with iterative retraining of student model. Each iteration uses refined student to mine progressively harder examples for subsequent training.

Result: 27.7% improvement in MRR@5, 44.6% improvement in mean DCG@5 across 14 financial filing types (21,800 query-document pairs), and improved NDCG on 3 of 4 document classes in FinanceBench.

Conclusion: Provides cost-effective solution to bridge gap between general-purpose models and specialized domains without labor-intensive human annotation, adapting retrieval embeddings for RAG applications.
...
TL;DR: SEA is a universal framework for aligning subtitles to sign language videos using pretrained models for segmentation and embedding, followed by efficient dynamic programming alignment that works across multiple languages and datasets.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for aligning subtitles to sign language videos are typically language/dataset-specific and lack generality. There's a need for a universal method that works across multiple languages and domains to advance sign language processing.

Method: Segment, Embed, and Align (SEA) uses two pretrained models: one segments video frames into individual signs, another embeds sign video clips into a shared latent space with text. Alignment is done via lightweight dynamic programming that runs efficiently on CPUs.

Result: Experiments on four sign language datasets show state-of-the-art alignment performance. The method is efficient (runs within a minute for hour-long episodes on CPUs) and can adapt to various scenarios from small lexicons to large continuous corpora.

Conclusion: SEA provides a flexible, universal framework for subtitle-sign alignment across languages, enabling high-quality parallel data generation for advancing sign language processing. Code and models are openly available.
...
TL;DR: Universal adversarial suffixes (4-10 tokens) that reduce accuracy across tasks and models when appended to any input, trained using Gumbel-Softmax relaxation with entropy regularization.


<details>
  <summary>Details</summary>
Motivation: Language models used as zero-shot/few-shot classifiers are fragile to adversarial prompts, but prior work optimizes task- or model-specific triggers, making results hard to compare and limiting transferability.

Method: Learn universal adversarial suffixes in differentiable "soft" form using Gumbel-Softmax relaxation, then discretize for inference. Training maximizes calibrated cross-entropy on label region while masking gold tokens, with entropy regularization to avoid collapse.
...
Conclusion: Universal adversarial suffixes provide effective cross-task and cross-model attacks, demonstrating vulnerability of LM-based classifiers to short adversarial sequences and highlighting transferability of adversarial attacks across different models.
...
<details>
  <summary>Details</summary>
Motivation: Existing adversarial suffix methods (gradient search, rule-based) are brittle and task/model-specific; need more robust, transferable attacks

Method: Treat suffix as RL policy trained with Proximal Policy Optimization; use frozen model as reward oracle with calibrated cross-entropy rewards to remove label bias and aggregate across surface forms

Result: RL-trained suffixes consistently degrade accuracy across 5 NLP benchmarks (sentiment, NLI, paraphrase, commonsense) and transfer better across 3 models (Qwen2-1.5B, TinyLlama-1.1B, Phi-1.5) than previous methods

Conclusion: RL framework produces more effective and transferable adversarial suffixes than gradient/rule-based approaches, demonstrating vulnerability of language models to learned attacks
...
TL;DR: ClinicalTrialsHub is an interactive platform that combines ClinicalTrials.gov data with automatically extracted trial information from PubMed articles, increasing structured trial data access by 83.8% using LLMs for enhanced accessibility.


<details>
  <summary>Details</summary>
Motivation: To improve access to structured clinical trial data for patients, clinicians, researchers, and policymakers by addressing limitations of relying solely on ClinicalTrials.gov, thereby advancing evidence-based medicine.

Method: Uses large language models (GPT-5.1 and Gemini-3-Pro) to automatically parse full-text PubMed research articles, extract structured trial information, translate user queries into structured database searches, and provide attributed question-answering with source sentence links.

Result: Increases access to structured clinical trial data by 83.8% compared to ClinicalTrials.gov alone, demonstrated through user studies with clinicians, researchers, and students, plus systematic automatic evaluation of information extraction and QA capabilities.

Conclusion: ClinicalTrialsHub effectively enhances clinical trial data accessibility and has potential to significantly benefit various stakeholders in healthcare and research by providing comprehensive, structured trial information with evidence-grounded answers.
...
TL;DR: GLLMs show adequate F1 scores for text annotation but differ systematically from human annotations in prevalence, downstream results, and display bias toward each other rather than human judgments.


<details>
  <summary>Details</summary>
Motivation: To investigate potential biases in GLLM-based text annotations by conceptually replicating manual annotations from previous research, examining whether GLLMs produce systematically different results than human coders.

Method: Conceptual replication of Boukes (2024) manual annotations using four GLLMs (Llama3.1:8b, Llama3.3:70b, GPT4o, Qwen2.5:72b) with five different prompts to annotate five concepts: political content, interactivity, rationality, incivility, and ideology.

Result: GLLMs perform adequately in terms of F1 scores but differ from manual annotations in prevalence, yield substantively different downstream results, and display systematic bias where GLLMs overlap more with each other than with manual annotations. F1 scores fail to capture the degree of bias.

Conclusion: GLLM annotations show systematic biases compared to human annotations, with GLLMs converging more with each other than with human judgments, suggesting F1 scores alone are insufficient for evaluating annotation quality and that GLLM-based annotations may produce substantively different research outcomes.
...
TL;DR: The paper investigates gender bias in machine translation by analyzing which source context tokens trigger specific gender inflections in target translations, using contrastive explanations and saliency attribution to understand bias origins rather than just measuring it.


<details>
  <summary>Details</summary>
Motivation: Current interpretability research has been limited in addressing gender bias in machine translation and large language models. The authors aim to move beyond simply measuring bias to understanding its origins by examining how specific source context influences gender inflection choices in translations.

Method: Using gender-ambiguous natural source data, the study employs contrastive explanations and computes saliency attribution to analyze which input tokens trigger specific gender inflections. The research addresses the challenge of lacking scoring thresholds and examines different attribution levels of source words on model gender decisions, comparing results with human perceptions.

Result: The study demonstrates a noticeable overlap between human perceptions of gender triggers and model attribution patterns. A linguistic analysis of salient words is provided, showing that the method effectively identifies which source context influences gender decisions in translation models.

Conclusion: Understanding model translation decisions regarding gender is crucial, and this information should be leveraged to mitigate gender bias. The work shows that interpretability methods can reveal bias origins and that model attribution patterns align with human perceptions, providing valuable insights for bias mitigation strategies.
...
TL;DR: Proposes a soft inductive bias approach with explicit reasoning perspectives to improve Korean inappropriate utterance detection using LLMs, achieving ~3.89% accuracy improvement over standard supervised learning.


<details>
  <summary>Details</summary>
Motivation: Addresses social concerns about unchecked inappropriate remarks escalating to abuse in anonymous online spaces. Need for better detection techniques to create safer communication environments, especially for Korean language where LLM-based approaches remain limited.

Method: Soft inductive bias approach that explicitly defines reasoning perspectives to guide inference process, promoting rational decision-making and preventing reasoning errors. Fine-tunes Korean LLM (Kanana-1.5) using this method with different training strategies.

Result: Kanana-1.5 model achieves average accuracy of 87.0046, improving by approximately 3.89% over standard supervised learning. Method enables more precise and consistent judgments through constrained reasoning perspectives.

Conclusion: Proposed method goes beyond simple knowledge imitation by LLMs and demonstrates effectiveness for inappropriate utterance detection through constrained reasoning, enabling more accurate and consistent judgments in Korean conversational text analysis.
...
<details>
  <summary>Details</summary>
Motivation: Large Language Models and multi-agent systems struggle with long-horizon reasoning tasks and escalating computation costs, needing more efficient distributed reasoning approaches.

Method: Hierarchical architecture with 64x64 grid of lightweight agents, selective oracle, spatial curriculum that expands from center outward, NLL confidence measures, and Thompson Sampling curriculum manager for adaptive training zone selection.
...
Conclusion: The hierarchical multi-agent approach with spatial curriculum and adaptive training effectively addresses long-horizon reasoning challenges while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing reviews overlook important Healthcare NLP tasks like synthetic data generation, explainable clinical NLP, and methodologies like retrieval-augmented generation and neural-symbolic integration of LLMs and KGs.

Method: Three-layer hierarchical approach: 1) Data/Resource layer (annotation guidelines, ethics, governance, synthetic data), 2) NLP-Eval layer (NER, RE, sentiment analysis, linking/coding with explainable HealthAI), 3) Patients layer (PPIE, health literacy, translation, simplification, summarization, decision support).

Result: Provides comprehensive tutorial materials and hands-on sessions accessible via GitHub, targeting NLP practitioners, healthcare researchers, and students without requiring prior knowledge.

Conclusion: This tutorial offers a structured introduction to patient- and resource-oriented Healthcare NLP, addressing gaps in existing literature and providing practical applications for diverse audiences in the healthcare NLP domain.
...
<details>
  <summary>Details</summary>
Motivation: To support reproducible and reliable LLM-based research for in-silico surveys and annotation tasks by providing a systematic framework for evaluating questionnaire presentation, prompt perturbations, and response generation methods.

Method: Developed an open-source Python framework (QSTN) that enables systematic generation of responses from questionnaire-style prompts, with extensive evaluation of over 40 million survey responses to assess the impact of question structure and response generation methods.

Result: Question structure and response generation methods significantly impact alignment of generated survey responses with human answers, and can be obtained at a fraction of the compute cost. The framework includes a no-code UI for researchers without coding knowledge.

Conclusion: QSTN supports reproducibility and reliability of LLM-based research by providing a systematic framework for in-silico surveys and annotation tasks, making robust experimentation accessible to researchers with varying technical backgrounds.
...
TL;DR: MOSAIC is a multi-agent AI system for clinical communication annotation that achieves high accuracy (F1=0.928) by using specialized agents for planning, updating, annotation, and verification, outperforming single-task models.


<details>
  <summary>Details</summary>
Motivation: Clinical communication annotation is labor-intensive and inconsistent. Existing single-task LLM approaches lack adaptability, interpretability, and reliability across different communication frameworks and clinical domains.

Method: Developed MOSAIC using LangGraph-based architecture with four core agents: Plan Agent (codebook selection/workflow planning), Update Agent (maintaining retrieval databases), Annotation Agents (codebook-guided RAG with dynamic few-shot prompting), and Verification Agent (consistency checks/feedback).

Result: Achieved overall F1 score of 0.928 on 50 test transcripts. Highest performance in Rheumatology (F1=0.962) and strongest for Patient Behavior categories. Outperformed baseline benchmarking in ablation studies.

Conclusion: MOSAIC demonstrates that structured multi-agent AI systems can effectively automate clinical communication annotation with high accuracy, addressing scalability and consistency challenges while maintaining adaptability across domains.
...
TL;DR: First public Basque CEFR C1 essay dataset with 3,200 expert-annotated essays for automatic scoring and feedback generation, showing fine-tuned open-source models outperform proprietary systems like GPT-5.


<details>
  <summary>Details</summary>
Motivation: Address the lack of publicly available resources for Automatic Essay Scoring (AES) and feedback generation in low-resource languages like Basque, particularly at the CEFR C1 proficiency level, to enable transparent and reproducible NLP research in educational contexts.

Method: Created a dataset of 3,200 Basque essays with expert annotations (scores for correctness, richness, coherence, cohesion, task alignment plus detailed feedback). Fine-tuned open-source models (RoBERTa-EusCrawl and Latxa 8B/70B) for scoring and explanation generation. Proposed novel evaluation methodology combining automatic consistency metrics with expert validation of extracted learner errors.

Result: Encoder models remain reliable for AES. Supervised fine-tuning of Latxa significantly enhances performance, surpassing state-of-the-art closed-source systems (GPT-5, Claude Sonnet 4.5) in scoring consistency and feedback quality. Fine-tuned Latxa produces criterion-aligned, pedagogically meaningful feedback and identifies more error types than proprietary models.

Conclusion: This work establishes foundational resources and benchmarks for transparent, reproducible, and educationally grounded NLP research in low-resource languages like Basque, demonstrating that fine-tuned open-source models can outperform proprietary systems in educational applications.
...
TL;DR: Proposes on-policy post-training method for lower-resource languages to align language models with fluent preferences without needing instruction-tuning data in target language.


<details>
  <summary>Details</summary>
Motivation: Lower-resource languages lack native-speaker datasets and fluent language models for synthetic data generation, making preference alignment challenging while maintaining fluency.

Method: Uses on-policy training approach, compared with supervised finetuning on machine-translated data and multilingual finetuning. Case study on Norwegian Bokmål with native-speaker fluency evaluation.

Result: On-policy method outperforms alternatives without requiring hard-to-obtain data, showing crucial importance of on-policy aspect for maintaining fluency.

Conclusion: Effective preference alignment for lower-resource languages can be achieved through on-policy training without target-language instruction data, preserving model fluency.
...
TL;DR: The paper introduces an adaptive aggregation method for aligning LLMs with diverse human preferences in federated learning, achieving better fairness while maintaining competitive alignment quality.


<details>
  <summary>Details</summary>
Motivation: Standard methods for aligning LLMs in federated learning environments often fail to adequately represent diverse human viewpoints, creating a need for approaches that balance alignment quality with fairness across different groups.

Method: Proposes a federated learning framework where groups locally evaluate rollouts and produce reward signals, with the server aggregating group-level rewards without accessing raw data. Evaluates standard aggregation techniques (min, max, average) and introduces a novel adaptive scheme that dynamically adjusts preference weights based on groups' historical alignment performance.
...
Conclusion: The work provides both a robust evaluation framework for assessing LLM behavior across diverse populations and a practical solution for developing pluralistic and fairly aligned models in federated settings.
...
TL;DR: ROME: A framework that injects psychological knowledge into personality detection by using LLMs to simulate user responses to psychometric questionnaires, providing intermediate supervision and interpretable reasoning chains.


<details>
  <summary>Details</summary>
Motivation: Existing personality detection methods suffer from limited supervision due to label scarcity and under-specified semantic mappings between user language and abstract psychological constructs. Current approaches using LLMs still face these fundamental limitations.

Method: ROME leverages LLMs' role-play capability to simulate user responses to validated psychometric questionnaires. It uses a question-conditioned Mixture-of-Experts module to jointly route over post and question representations, learning to answer questionnaire items with explicit supervision. Predicted answers are summarized into interpretable answer vectors and fused with user representations in a multi-task learning framework.

Result: ROME consistently outperforms state-of-the-art baselines on two real-world datasets, achieving significant improvements (15.41% on Kaggle dataset).

Conclusion: By explicitly injecting psychological knowledge through simulated questionnaire responses, ROME addresses label scarcity and semantic mapping challenges in personality detection, providing both performance improvements and interpretable reasoning chains.
...
TL;DR: Gradual depth growth in Transformers improves reasoning by overcoming the Curse of Depth, leading to better depth utilization and distinct computational circuits.
...
Method: The study uses depth-wise analyses to examine how growth via gradual middle stacking affects model architecture. It also proposes a lightweight modification of MIDAS to further improve reasoning performance.

Result: Gradual depth growth yields more effective utilization of model depth, alters residual stream structure, facilitates formation of permutable computational blocks, and the proposed MIDAS modification shows further improvements in downstream reasoning benchmarks.

Conclusion: Gradual growth of model depth leads to distinct computational circuits and overcomes the limited depth utilization seen in standard non-grown models, providing a mechanistic understanding of MIDAS's benefits.
...
TL;DR: RAGLens is a lightweight hallucination detector for RAG systems that uses sparse autoencoders to identify hallucination-related features in LLM internal representations, achieving superior performance without large-scale training or expensive LLM judges.


<details>
  <summary>Details</summary>
Motivation: Existing RAG hallucination detection methods either require large annotated datasets for training or rely on expensive external LLM judges, while approaches using LLM internal representations have limited accuracy. There's a need for a more effective and efficient solution.

Method: Uses sparse autoencoders (SAEs) to disentangle LLM internal activations, identifies hallucination-specific features through information-based feature selection and additive feature modeling, creating a lightweight detector called RAGLens.

Result: RAGLens achieves superior hallucination detection performance compared to existing methods, provides interpretable rationales for decisions, enables post-hoc mitigation of unfaithful RAG outputs, and reveals insights about hallucination signal distribution in LLMs.

Conclusion: RAGLens demonstrates that mechanistic interpretability techniques like SAEs can effectively identify hallucination features in LLM representations, enabling accurate, lightweight detection without expensive training or external judges.
...
TL;DR: Researchers developed a deep learning system to detect cyberbullying in GIFs/stickers using VGG16 model, achieving 97% accuracy on a Twitter-collected dataset of 4100 GIFs.
...
Method: Collected GIF dataset from Twitter using cyberbullying-related hashtags via GIPHY API (4100 GIFs total), then applied pre-trained VGG16 deep learning model for classification of cyberbullying vs non-cyberbullying content.

Result: The VGG16 model achieved 97% accuracy in detecting cyberbullying from GIFs, and the researchers created a publicly available GIF dataset for future research in this area.

Conclusion: The study successfully addresses the gap in GIF-based cyberbullying detection, demonstrating high accuracy with deep learning models and providing a valuable dataset for researchers working on multimodal cyberbullying detection.
...
TL;DR: Deep learning with 4-band Planet Labs imagery enables near real-time monitoring of fire damage in Sudan's conflict zones, outperforming baseline methods with minimal delay.


<details>
  <summary>Details</summary>
Motivation: The ongoing war in Sudan requires rapid conflict monitoring. Advances in deep learning and readily available satellite imagery enable near real-time analysis of conflict-related damage.

Method: Uses 4-band imagery from Planet Labs with a deep learning model to monitor fire damage in armed conflicts with minimal delay. Tested on five case studies in Sudan.

Result: The automated method captures active fires and charred areas more accurately than baseline methods. Using 8-band imagery or time series provides only marginal gains.

Conclusion: 4-band satellite imagery with deep learning provides effective near real-time monitoring of conflict-related fire damage, offering a practical solution for rapid conflict analysis.
...
TL;DR: LivingSwap is the first video reference-guided face swapping model that uses keyframes as conditioning signals to inject target identity, enabling flexible editing with temporal consistency across long video sequences.


<details>
  <summary>Details</summary>
Motivation: Video face swapping in film production needs high fidelity and temporal consistency over long sequences, but current methods struggle with these requirements. The paper explores whether source video attributes can enhance both fidelity and coherence.

Method: Uses keyframes as conditioning signals to inject target identity, combines keyframe conditioning with video reference guidance for temporal stitching, and constructs Face2Face dataset with paired face-swapping data for training.

Result: Achieves state-of-the-art results, seamlessly integrating target identity with source video's expressions, lighting, and motion while reducing manual effort in production workflows.

Conclusion: LivingSwap successfully demonstrates that video reference guidance can enhance both fidelity and temporal coherence in video face swapping, offering a practical solution for film production with reduced manual effort.
...
TL;DR: Hierarchical semantic segmentation framework for dental anatomy improves fine-grained detection and anatomical coherence by explicitly encoding anatomical hierarchy through recurrent prediction and feature conditioning.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchy-aware segmentation methods provide weak supervision through loss functions only. Accurate anatomical structure understanding is essential for dental disease staging, requiring better hierarchical modeling.

Method: Recurrent level-wise prediction with restrictive output heads and top-down feature conditioning using Feature-wise Linear Modulation (FiLM). Probabilistic composition rule enforces parent-child consistency. Hierarchical loss combines per-level Dice/CE loss with consistency term.

Result: Hierarchical variants consistently increase IoU, Dice, and recall, especially for fine-grained anatomies, producing more anatomically coherent masks. However, increased recall over precision implies more false positives.

Conclusion: Explicit hierarchical structuring improves both performance and clinical plausibility in low-data dental imaging regimes, demonstrating value of anatomical hierarchy modeling for medical segmentation tasks.
...
TL;DR: FRIEDA is a new benchmark for evaluating complex cartographic reasoning in large vision-language models, focusing on spatial relations across multiple maps, where current SOTA models perform poorly compared to humans.


<details>
  <summary>Details</summary>
Motivation: Current map VQA evaluations treat maps as simple charts, missing the complex spatial reasoning required for real-world cartographic interpretation involving layered symbology, orientation, distance, and cross-map relationships.

Method: Created FRIEDA benchmark with real map images from various domains/regions, covering three GIS spatial relation categories (topological, metric, directional), requiring multi-step inference and often cross-map grounding.

Result: Tested 11 SOTA LVLMs in direct and contextual settings; best models (Gemini-2.5-Pro, GPT-5-Think) achieved only 38.20% and 37.20% accuracy, far below human performance of 84.87%.

Conclusion: There's a significant gap in multi-step cartographic reasoning capabilities in current LVLMs, positioning FRIEDA as a rigorous benchmark to advance spatial intelligence in AI systems.
...
TL;DR: SSplain is a novel explainer method for ROP classification from fundus images that generates pixel-wise explanations while preserving image smoothness and sparsity, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing explainer methods for medical diagnosis neural networks fail to generate explanations that preserve important input image structures like smoothness and sparsity, limiting their usefulness for clinicians who need realistic explanations to understand and trust model outputs.

Method: SSplain formulates an optimization problem with combinatorial constraints to generate pixel-wise explanations while enforcing smoothness and sparsity properties. The method uses Alternating Direction Method of Multipliers (ADMM) to solve this optimization problem.

Result: SSplain outperforms commonly used explainers in both post-hoc accuracy and smoothness analyses. It identifies features consistent with domain-understandable features clinicians consider discriminative for ROP, and demonstrates generalization on additional publicly available datasets.

Conclusion: SSplain provides realistic explanations that preserve image structures, enhancing clinician understanding and trust in black-box models for medical diagnosis, particularly for ROP classification from fundus images.
...
TL;DR: A unified model for sign language understanding that performs both sign language translation (SLT) and sign-subtitle alignment (SSA) using privacy-preserving visual features, sliding perceiver architecture, and multi-task training.


<details>
  <summary>Details</summary>
Motivation: To create practical tools for converting continuous signing videos to spoken language text and aligning signing with subtitles, which are essential for communication, corpus construction, and educational applications.

Method: Three components: (1) lightweight visual backbone using keypoints and lip-region images for privacy, (2) Sliding Perceiver network to aggregate visual features into word-level embeddings, (3) multi-task training strategy jointly optimizing SLT and SSA. Multilingual pretraining on BSL and ASL datasets.

Result: State-of-the-art results on BOBSL (BSL) dataset for both SLT and SSA. Strong zero-shot generalization and fine-tuned performance on How2Sign (ASL), demonstrating cross-linguistic scalability.

Conclusion: The unified model successfully addresses both translation and alignment tasks, showing potential for scalable sign language understanding across different sign languages through multilingual pretraining.
...
TL;DR: Frequency-domain masking strategy for universal deepfake detection that enhances generalization to unseen generators while being computationally efficient and robust to model pruning.
...
Method: Introduces frequency-domain masking as training strategy with random masking and geometric transformations, focusing on frequency masking for superior generalization properties.
...
Conclusion: Frequency-based masking is a practical step toward sustainable and generalizable deepfake detection, with code and models publicly available.
...
TL;DR: M2A: Simple continual test-time adaptation using random masking with consistency and entropy losses, outperforming complex methods on corrupted datasets.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods rely on complex masking designs with calibrated uncertainty or stable attention scores, adding complexity. The paper questions whether simple random masking can be sufficient under strong corruption.

Method: M2A generates short sequences of randomly masked views (spatial or frequency) and adapts using two objectives: mask consistency loss (aligning predictions across views) and entropy minimization loss (encouraging confident outputs).
...
TL;DR: This paper evaluates various change detection models for Amazon deforestation monitoring using PRODES data, addressing limitations of existing approaches through standardized evaluation, modern architectures, and preprocessing techniques.


<details>
  <summary>Details</summary>
Motivation: The preservation of the Amazon Rainforest is a global priority for climate change, biodiversity, and indigenous cultures. Existing machine learning approaches for deforestation detection using PRODES data have significant limitations: unsatisfactory effectiveness, lack of modern architectures (like self-attention mechanisms), and absence of methodological standardization for direct comparisons between studies.

Method: The authors evaluate various change detection models in a unified dataset, including both fully convolutional models and networks incorporating self-attention mechanisms based on Transformers. They investigate the impact of different pre- and post-processing techniques such as filtering deforested areas based on connected component size, texture replacement, and image enhancements. They also test different strategies for combining evaluated models to achieve superior results.
...
Conclusion: The work addresses key gaps in Amazon deforestation monitoring by providing standardized evaluation of modern change detection models, demonstrating that preprocessing techniques and model combination strategies can substantially improve detection performance, contributing to more effective satellite-based monitoring of deforestation.
...
Method: Introduces two complementary components in a Large Multimodal Model: 1) target-affinity token (central vision analog) that guides attention to query-relevant objects, and 2) allocentric grid (peripheral vision analog) that captures global scene context and spatial arrangements.
...
Conclusion: The central-peripheral vision-inspired framework provides structured, context-aware understanding of complex 3D environments by combining focused attention on relevant objects with global spatial context capture.
...
TL;DR: Fourier-RWKV: A novel dehazing framework using Multi-State Perception with linear complexity, achieving SOTA performance on non-uniform haze removal.
...
Method: Proposes Fourier-RWKV based on Multi-State Perception paradigm: (1) Spatial-form Perception via Deformable Quad-directional Token Shift (DQ-Shift) for local haze variations; (2) Frequency-domain Perception via Fourier Mix block extending RWKV's WKV attention to Fourier domain; (3) Semantic-relation Perception via Semantic Bridge Module with Dynamic Semantic Kernel Fusion for encoder-decoder alignment.
...
Conclusion: Fourier-RWKV achieves comprehensive haze degradation modeling with linear complexity, delivering superior dehazing performance while enabling real-time deployment through efficient multi-state perception architecture.
...
TL;DR: The study investigates the relationship between model accuracy and human similarity in monocular depth estimation, finding that improving accuracy doesn't necessarily lead to more human-like behavior, highlighting the need for human-centric evaluation beyond traditional metrics.


<details>
  <summary>Details</summary>
Motivation: While DNNs achieve high accuracy in monocular depth estimation, there's a need to align model representations with human perception for better robustness and interpretability. The paper investigates whether there's a trade-off between accuracy and human-like behavior similar to what's observed in object recognition, particularly for outdoor scenes where benchmarks rely on sensor data rather than human perception.

Method: Systematically investigated 69 monocular depth estimators using the KITTI dataset. Applied affine fitting to decompose prediction errors into interpretable components on a factor-by-factor basis to dissect error patterns.

Result: Found that while humans and DNNs share certain estimation biases (positive error correlations), there are distinct trade-off relationships between model accuracy and human similarity. Improving accuracy does not necessarily lead to more human-like behavior.

Conclusion: The findings underscore the necessity of developing multifaceted, human-centric evaluations beyond traditional accuracy metrics for monocular depth estimation models.
...
TL;DR: GeoLoom is a text-to-geometric-diagram generation framework that uses formal language translation and Monte Carlo optimization to ensure spatial accuracy, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Geometric diagram generation requires strict spatial accuracy while offering well-defined constraints, presenting both a challenge and opportunity for structured generation approaches.

Method: GeoLoom has two components: 1) autoformalization module translating natural language to GeoLingua formal language, and 2) coordinate solver using Monte Carlo optimization to map formal constraints to precise coordinates. Also introduces GeoNF dataset and constraint-based evaluation metric.

Result: GeoLoom significantly outperforms state-of-the-art baselines in structural fidelity, providing interpretable and scalable diagram generation with mathematically grounded supervision.

Conclusion: The framework offers a principled foundation for interpretable and scalable geometric diagram generation through formal language translation and constraint-based optimization.
...
TL;DR: The paper proposes an on-device framework for camera-based animal re-identification that enables deployment on low-power microcontrollers, achieving competitive accuracy with models 100x smaller through optimized architecture design and efficient fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Animal Re-ID is needed for wildlife monitoring and livestock management in remote outdoor environments with limited connectivity, but existing models are too large for deployment on low-power edge devices like microcontroller-based collar tags.

Method: 1) Characterized the gap between state-of-the-art Animal Re-ID models and MCU hardware constraints; 2) Designed a compact CNN-based MobileNetV2 architecture scaled for low-resolution inputs; 3) Developed a data-efficient fine-tuning strategy requiring only 3 images per animal identity for adaptation to new sites.

Result: The compact model achieves competitive retrieval accuracy across six public datasets while reducing model size by over 100x. On a self-collected cattle dataset, it performs fully on-device inference with minimal accuracy drop and unchanged Top-1 accuracy compared to cluster versions.

Conclusion: Practical, adaptable Animal Re-ID is achievable on MCU-class devices, enabling scalable deployment in real field environments for wildlife monitoring and livestock management applications.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for human avatar creation either produce geometrically inconsistent multi-view images or sacrifice photorealism, resulting in blurry outputs under diverse viewing angles and complex motions.

Method: Dual-conditioning architecture: 1) Human NeRF generates geometrically coherent multi-view renderings for target poses with 3D structural guidance; 2) Diffusion model refines these renderings while preserving details; 3) Hierarchical feature fusion incorporates texture, normal, and semantic priors from SMPL models.

Result: Blur2Sharp consistently surpasses state-of-the-art techniques in both novel pose and view generation tasks, particularly excelling under challenging scenarios involving loose clothing and occlusions.

Conclusion: The proposed framework successfully addresses the limitations of current approaches by combining 3D-aware neural rendering with diffusion models to generate sharp, geometrically consistent novel-view human avatars from single reference images.
...
<details>
  <summary>Details</summary>
Motivation: Current object understanding in computer vision is limited to simple category recognition. Comprehensive object understanding requires multi-modal data (visual annotations, textual knowledge) but existing data is task-oriented and not systematically organized enough for deep understanding.

Method: VisKnow framework extracts multi-modal, object-level knowledge by integrating enriched aligned text and image-source knowledge with region annotations at object and part levels, using a combination of expert design and large-scale model application.

Result: Constructed AnimalKB covering 406 animal categories with 22K textual knowledge triplets, 420K images, and corresponding region annotations. Experiments show AnimalKB enhances zero-shot recognition and fine-grained VQA, and serves as benchmark for knowledge graph completion and part segmentation.

Conclusion: Visual knowledge bases can significantly advance object-level visual understanding and practical applications, with potential for automatic construction to scale across domains.
...
TL;DR: This paper explores prompt tuning methods for 3D object detection, investigating whether models trained on Waymo dataset can serve as foundation models for other scenarios, and proposes a Scene-Oriented Prompt Pool (SOP²).


<details>
  <summary>Details</summary>
Motivation: To explore the effectiveness of prompt tuning methods (inspired by NLP success with LLMs) in 3D object detection domain, and investigate transfer learning capabilities from large-scale datasets to other scenarios.

Method: Examines impact of prompt tokens and prompt generators, then proposes Scene-Oriented Prompt Pool (SOP²) for 3D object detection adaptation.

Result: Demonstrates effectiveness of prompt pools in 3D object detection, showing successful adaptation from Waymo-trained foundation models to other scenarios.

Conclusion: Prompt tuning methods are effective for 3D object detection, and the proposed SOP² approach shows promise, potentially inspiring further research into prompt-based adaptation in 3D vision tasks.
...
Method: The authors investigate using Versatile Video Coding (VVC) for feature compression under the MPEG-AI Feature Coding for Machines (FCM) standard. They perform tool-level analysis to understand individual coding components' impact on compression efficiency and downstream task accuracy, then propose three lightweight VVC profiles.

Result: The Fast profile provides 2.96% BD-Rate gain while reducing encoding time by 21.8%. Faster achieves 1.85% BD-Rate gain with 51.5% speedup. Fastest reduces encoding time by 95.6% with only 1.71% loss in BD-Rate.

Conclusion: The proposed lightweight VVC profiles effectively compress neural network features for split inference systems, offering significant encoding speed improvements with minimal impact on compression efficiency and downstream task accuracy, addressing the unique requirements of feature compression where perceptual quality is irrelevant.
...
TL;DR: MM-CoT is a diagnostic benchmark that evaluates multimodal models' Chain-of-Thought reasoning by testing visual grounding and logical coherence through constrained event chain selection rather than free-form generation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks focus on generating Chain-of-Thought reasoning but neglect verification - assessing whether reasoning is visually consistent and logically valid. There's a gap in evaluating whether multimodal models' reasoning is genuinely grounded in visual evidence and coherent.

Method: Created MM-CoT benchmark where models must select the sole event chain satisfying two constraints: (1) visual consistency (anchored in observable evidence) and (2) logical coherence (causal and commonsense validity). Uses adversarial distractors engineered to violate one constraint to expose specific reasoning failures.
...
Conclusion: MM-CoT provides a foundation for developing future multimodal models that reason not just plausibly, but faithfully and coherently within the visual world, addressing critical gaps in current evaluation of Chain-of-Thought reasoning.
...
TL;DR: Proposes normal-guided sparse depth sampling using PCA-based surface normal estimation to create more realistic sparse depth inputs for depth completion, improving accuracy and reducing artifacts.


<details>
  <summary>Details</summary>
Motivation: Current depth completion methods use unrealistic sparse depth generation (uniform random sampling from ground truth), ignoring that real sensors exhibit geometry-dependent and spatially nonuniform reliability patterns.

Method: Uses PCA-based surface normal estimation on RGB-D point clouds to compute per-pixel depth reliability measures, then samples sparse depth according to this reliability distribution. Integrated with Marigold-DC diffusion-based depth completion model.

Result: Experiments on NYU Depth v2 show improved accuracy, reduced artifacts near edges/discontinuities, and more realistic training conditions that better reflect real sensor behavior.

Conclusion: Geometry-aware sparse depth sampling produces more realistic training data and improves depth completion performance by better simulating real sensor reliability patterns.
...
<details>
  <summary>Details</summary>
Motivation: Current camera-only BEV perception systems face a fundamental tension between achieving state-of-the-art performance and maintaining deployment tractability on vehicles, due to dependency on computationally expensive view transformations and platform-specific kernels.

Method: Introduces two principles: 1) "Deployable by Design" via novel view transformation that decomposes projection into standard Index-Gather-Reshape pipeline using deterministic pre-sorting, enabling TensorRT-native portability; 2) "Fast by Algorithm" with end-to-end depth-aware fusion, temporal aggregation, and robust data augmentation.

Result: Achieves new state-of-the-art 0.359 NDS on nuScenes benchmark while maintaining >134 FPS on Tesla T4 hardware, demonstrating both high accuracy and real-time performance.

Conclusion: FastBEV++ presents a mature and scalable design philosophy for production autonomous systems by eliminating custom plugins while maintaining high accuracy, reconciling the performance-deployment tension in BEV perception.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs face quadratic computational costs from feeding hundreds of visual patch tokens into LLMs, creating memory and context window strain. Traditional compression methods either dilute high-level semantics (continuous compression) or lose fine-grained details (discrete quantization).
...
Result: Achieves 87.2% average performance retention across seven benchmarks (GQA, VQAv2, MMBench, MME, POPE, SEED-Bench, ScienceQA-Image), outperforming leading continuous baseline (81.0%) with 580-to-1 compression ratio. Attention analysis shows compressed token prioritizes discrete anchor, validating semantic guidance.

Conclusion: A minimalist hybrid design can resolve the efficiency-fidelity dilemma in VLMs, enabling scalable multimodal reasoning by disentangling semantics and appearance through dual channels.
...
### [43] [Residual-SwinCA-Net: A Channel-Aware Integrated Residual CNN-Swin Transformer for Malignant Lesion Segmentation in BUSI](https://arxiv.org/abs/2512.08243)
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in breast ultrasound lesion segmentation by extracting robust local features and learning global dependencies while handling ultrasound noise, tissue continuity, and maintaining morphological integrity of malignant lesion contours.

Method: Proposes a hybrid Residual-SwinCA-Net framework with: 1) Residual CNN modules for local features, 2) Custom Swin Transformer blocks with internal residual pathways for global dependencies, 3) Laplacian-of-Gaussian operator for noise suppression and tissue enhancement, 4) Boundary-oriented operator for contour integrity, 5) Stage-wise contraction strategy for scale invariance, 6) Multi-Scale Channel Attention and Squeezing (MSCAS) module for selective feature emphasis, and 7) Pixel-Attention module for class-relevant spatial weighting.

Result: Achieved 99.29% mean accuracy, 98.74% IoU, and 0.9041 Dice score on BUSI dataset, outperforming existing CNN and Vision Transformer methods.

Conclusion: The Residual-SwinCA-Net framework significantly improves breast lesion diagnostic performance and supports timely clinical decision-making for breast cancer diagnosis.
...
### [44] [Beyond Real Weights: Hypercomplex Representations for Stable Quantization](https://arxiv.org/abs/2512.08524)
...
### [45] [Distilling Future Temporal Knowledge with Masked Feature Reconstruction for 3D Object Detection](https://arxiv.org/abs/2512.08247)
...
<details>
  <summary>Details</summary>
Motivation: Existing knowledge distillation methods for camera-based 3D object detection overlook future frames, focusing only on spatial features or temporal relations with strict frame alignment, making it hard for online models to learn future knowledge.

Method: Proposes Future Temporal Knowledge Distillation (FTKD) with: 1) future-aware feature reconstruction using sparse queries without strict frame alignment, and 2) future-guided logit distillation leveraging teacher's foreground/background context.

Result: Achieves up to 1.3 mAP and 1.3 NDS gains on nuScenes dataset, plus the most accurate velocity estimation, without increasing inference cost when applied to two high-performing 3D detection baselines.

Conclusion: FTKD effectively transfers future knowledge from offline to online models, addressing limitations of existing KD methods and improving 3D object detection performance while maintaining efficient inference.
...
### [46] [Pose-Based Sign Language Spotting via an End-to-End Encoder Architecture](https://arxiv.org/abs/2512.08738)
...
<details>
  <summary>Details</summary>
Motivation: Sign-to-sign retrieval (detecting specific signs in continuous sequences) is largely unexplored but crucial for bridging deaf-hearing communication gaps. Current approaches rely on intermediate gloss recognition or text matching, which have limitations.

Method: End-to-end model using pose keypoints extracted from sign videos as input, with encoder-only backbone and binary classification head to determine if query sign appears in target sequence. Focuses on pose representations to reduce computational cost and visual noise.

Result: Achieved 61.88% accuracy and 60.00% F1-score on Word Presence Prediction dataset from WSLP 2025 shared task, demonstrating effectiveness of pose-based framework for sign language spotting.

Conclusion: The pose-based approach provides a strong foundation for sign language retrieval and verification, offering computational efficiency and robustness to visual noise compared to RGB-based methods.
...
TL;DR: QHP addresses prototype bias in few-shot 3D segmentation by generating query-aware prototypes through hub identification and distribution optimization, achieving state-of-the-art results on S3DIS and ScanNet.


<details>
  <summary>Details</summary>
Motivation: Existing metric-based prototype learning methods in few-shot 3D point cloud segmentation generate prototypes only from support sets, ignoring query data relevance. This causes prototype bias where prototypes overfit support characteristics and fail to generalize to query distributions, especially with distribution shifts, degrading segmentation performance.

Method: Proposes Query-aware Hub Prototype (QHP) learning with two modules: 1) Hub Prototype Generation (HPG) constructs bipartite graph connecting query and support points, identifies frequently linked support hubs, and generates query-relevant prototypes capturing cross-set semantics; 2) Prototype Distribution Optimization (PDO) uses purity-reweighted contrastive loss to refine prototypes by pulling bad hubs and outlier prototypes closer to class centers.

Result: Extensive experiments on S3DIS and ScanNet demonstrate QHP achieves substantial performance gains over state-of-the-art methods, effectively narrowing the semantic gap between prototypes and query sets in FS-3DSeg.

Conclusion: QHP successfully addresses prototype bias in few-shot 3D segmentation by explicitly modeling semantic correlations between support and query sets through hub prototype generation and distribution optimization, leading to improved generalization and segmentation performance.
...
TL;DR: SFP proposes spatial and frequency priors for real-world scene recovery, using transmission map estimation from spatial domain and adaptive frequency enhancement with novel priors, integrated via weighted fusion.


<details>
  <summary>Details</summary>
Motivation: Existing scene recovery methods rely on single priors (insufficient for multiple degradations) or complex networks trained on synthetic data (poor generalization to real-world scenarios). Need for effective real-world scene recovery.

Method: 1) Spatial prior: Estimate transmission map from degraded image's spectral projection. 2) Frequency prior: Construct adaptive frequency enhancement mask using two novel priors: DC component mean intensity similarity and low radial frequency magnitude proportion (1%). 3) Weighted fusion strategy integrates spatial restoration, frequency enhancement, and salient input features.
...
Conclusion: SFP provides an effective approach for real-world scene recovery by leveraging complementary spatial and frequency priors, addressing limitations of existing methods.
...
Method: RLCNet is a novel end-to-end trainable deep learning framework for simultaneous online calibration of multimodal sensors. It incorporates an online calibration framework with weighted moving average and outlier rejection for dynamic adjustment of calibration parameters with reduced prediction noise.

Result: Validated on real-world datasets, RLCNet demonstrates robust performance under diverse conditions. Ablation study highlights significance of architectural choices, and comparisons with existing methods show superior accuracy and robustness.

Conclusion: RLCNet is designed for practical deployment and enables real-time operation with improved resilience to drift, making it a superior solution for online calibration of multimodal sensors in autonomous vehicles.
...
TL;DR: EgoX is a novel framework that generates realistic egocentric (first-person) videos from single exocentric (third-person) videos using adapted video diffusion models with geometry-guided attention.


<details>
  <summary>Details</summary>
Motivation: Translating exocentric videos to egocentric videos enables immersive understanding but is challenging due to extreme camera pose variations and minimal view overlap. This requires preserving visible content while synthesizing unseen regions with geometric consistency.

Method: EgoX leverages pretrained video diffusion models through lightweight LoRA adaptation, uses unified conditioning combining exocentric and egocentric priors via width/channel concatenation, and employs geometry-guided self-attention to selectively attend to spatially relevant regions.
...
Conclusion: EgoX successfully addresses the challenging task of exocentric-to-egocentric video translation by combining adapted diffusion models with geometric conditioning mechanisms, enabling immersive first-person video generation from third-person inputs.
...
TL;DR: PAVAS introduces physics-aware video-to-audio generation using physical parameter estimation and a physics-driven adapter to create more realistic sounds based on object properties like mass and velocity.


<details>
  <summary>Details</summary>
Motivation: Current V2A models are appearance-driven and capture visual-acoustic correlations but ignore the physical factors that shape real-world sounds, limiting their realism and physical plausibility.

Method: Uses Physics-Driven Audio Adapter (Phy-Adapter) with latent diffusion, Physical Parameter Estimator (PPE) with VLM for mass estimation and segmentation-based 3D reconstruction for motion trajectory/velocity computation.
...
Conclusion: Incorporating physical reasoning into V2A generation significantly improves audio realism and physical consistency, demonstrating the importance of modeling underlying physical factors in sound synthesis.
...
TL;DR: OpenSubject is a large-scale video-derived dataset (2.5M samples, 4.35M images) for subject-driven image generation and manipulation, featuring cross-frame identity priors and comprehensive verification, with improved performance in complex multi-subject scenes.


<details>
  <summary>Details</summary>
Motivation: Current subject-driven image generation models often fail to preserve reference identities and struggle with complex scenes containing multiple subjects, creating a need for better training data and evaluation benchmarks.
...
Result: Created OpenSubject dataset with 2.5M samples and 4.35M images, established comprehensive benchmark for subject-driven generation/manipulation, and demonstrated improved performance in complex scenes through extensive experiments.

Conclusion: OpenSubject addresses identity preservation challenges in subject-driven generation, particularly for complex multi-subject scenes, and provides a valuable large-scale dataset and benchmark for advancing the field.
...
<details>
  <summary>Details</summary>
Motivation: Perlin noise has been the standard for procedural world generation for decades but is fundamentally limited in realism and large-scale coherence. The paper aims to bridge the gap between the fidelity of modern diffusion models and the practical properties that made procedural noise indispensable.

Method: Introduces Terrain Diffusion with three key components: 1) InfiniteDiffusion algorithm for seamless infinite generation, 2) hierarchical stack of diffusion models coupling planetary context with local detail, 3) compact Laplacian encoding for stable outputs across Earth-scale dynamic ranges, 4) open-source infinite-tensor framework for constant-memory manipulation, and 5) few-step consistency distillation for efficient generation.

Result: Establishes diffusion models as a practical foundation for procedural world generation capable of synthesizing entire planets coherently, controllably, and without limits, while maintaining the key properties of procedural noise: seamless infinite extent, seed-consistency, and constant-time random access.

Conclusion: Terrain Diffusion represents an AI-era successor to Perlin noise, successfully combining the realism of diffusion models with the practical infinite generation capabilities needed for procedural world building, enabling the creation of boundless, coherent planetary landscapes.
...
<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods are confined to Euclidean spaces, which only capture linear structures and overlook the intrinsic geometry of real data (e.g., curvature). High-dimensional data often lie on low-dimensional manifolds, suggesting distilled data should align with original data manifold geometry.

Method: Proposes GeoDM framework operating in Cartesian product of Euclidean, hyperbolic, and spherical manifolds. Introduces learnable curvature and weight parameters for three geometries. Designs optimal transport loss to enhance distribution fidelity.

Result: Theoretical analysis shows geometry-aware distribution matching yields smaller generalization error bound than Euclidean counterparts. Extensive experiments demonstrate outperforming state-of-the-art data distillation methods across various distribution-matching strategies.

Conclusion: GeoDM provides a unified geometry-aware framework for dataset distillation that captures diverse data structures (flat, hierarchical, cyclical) and achieves superior performance by aligning distilled data manifold with original data geometry.
...
TL;DR: 3DTeethLand challenge introduces first public dataset for 3D teeth landmark detection to advance clinical orthodontics through deep learning methods.


<details>
  <summary>Details</summary>
Motivation: Precise teeth landmark detection is crucial for advanced diagnostics, personalized treatment strategies, and effective treatment monitoring in clinical dentistry, but faces challenges due to complex tooth geometry and individual variations.
...
Result: The challenge provides a benchmark dataset to assess state-of-the-art methods and encourages community contributions toward solving this clinically significant problem.

Conclusion: Development of advanced deep learning techniques is essential for precise 3D tooth landmark detection, and the 3DTeethLand challenge aims to accelerate progress in this clinically important field.
...
TL;DR: GeoDiffMM is a diffusion-based Lagrangian video motion magnification framework that uses optical flow as a geometric cue to separate true micro-motion from photon noise, enabling structurally consistent motion amplification.
...
Method: Three-stage approach: 1) Noise-free Optical Flow Augmentation synthesizes diverse nonrigid motion fields without photon noise for supervision; 2) Diffusion Motion Magnifier conditions denoising on optical flow as geometry prior and learnable magnification factor; 3) Flow-based Video Synthesis maps amplified motion back to image domain.
...
Conclusion: GeoDiffMM successfully addresses the photon noise problem in VMM by leveraging diffusion models conditioned on optical flow, enabling more accurate and structurally consistent motion magnification.
...
TL;DR: LSQMM is a novel color image classification method that treats RGB channels as pure quaternions to preserve channel correlations, using quaternion nuclear norm regularization for low-rank structure and ADMM optimization.


<details>
  <summary>Details</summary>
Motivation: Conventional color image classification treats features as vectors/matrices/tensors in real field, but this fails to preserve intrinsic coupling relationships among RGB channels. Quaternion algebra has shown success in image recovery/denoising, suggesting it could better capture channel correlations for classification tasks.

Method: Propose Low-rank Support Quaternion Matrix Machine (LSQMM) that treats RGB channels as pure quaternions. Adds quaternion nuclear norm regularization to hinge loss to promote low-rank structures from correlated color channels. Uses ADMM-based iterative algorithm to solve the quaternion optimization model.
...
Conclusion: LSQMM effectively preserves RGB channel correlations through quaternion representation and low-rank regularization, leading to superior classification performance over conventional approaches that treat channels separately.
...
TL;DR: This paper provides a systematic explainable AI analysis of image protection mechanisms like Glaze and Nightshade, revealing they operate as structured, low-entropy perturbations tightly coupled to image content rather than inducing global representational drift.


<details>
  <summary>Details</summary>
Motivation: While empirical effectiveness of image protection mechanisms (Glaze, Nightshade) is known, their internal structure, detectability, and representational behavior remain poorly understood. The study aims to provide systematic explainable AI analysis to understand how these protections work.
...
Result: Protection mechanisms operate as structured, low-entropy perturbations tightly coupled to underlying image content across representational, spatial, and spectral domains. Protected images preserve content-driven feature organization with protection-specific substructure. Detectability governed by perturbation entropy, spatial deployment, and frequency alignment. Frequency analysis shows Glaze and Nightshade redistribute energy along dominant image-aligned frequency axes rather than introducing diffuse noise.
...
TL;DR: PointDico integrates diffusion and contrastive models for 3D representation learning, achieving SOTA results on ScanObjectNN (94.32%) and ShapeNetPart (86.5% mIoU).


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised methods struggle with 3D data due to its unordered and uneven density. Contrastive models suffer from overfitting, while 3D Mask Autoencoders can't handle unordered point clouds effectively.

Method: Proposes PointDico that integrates diffusion and contrastive models through knowledge distillation. Uses hierarchical pyramid conditional generator for multi-scale geometric feature extraction and dual-channel design to integrate local and global contextual information.

Result: Achieves state-of-the-art results: 94.32% accuracy on ScanObjectNN and 86.5% instance mIoU on ShapeNetPart segmentation.

Conclusion: PointDico successfully combines the strengths of diffusion and contrastive models for 3D representation learning, overcoming limitations of existing methods and setting new benchmarks.
...
TL;DR: Bi²MAC: A bimodal bi-adaptive mask-aware convolution method for pansharpening that uses soft/hard masks to route features to different processing branches based on regional heterogeneity, achieving SOTA performance with lower computational costs.


<details>
  <summary>Details</summary>
Motivation: Current deep learning pansharpening methods struggle with regional heterogeneity in remote sensing images. Existing adaptive convolution approaches have high computational costs and limited effectiveness in capturing heterogeneous regions.

Method: Proposes Bi²MAC with a lightweight mask generation module that creates soft and hard masks. Soft masks modulate input features, while hard masks guide different region types to separate branches: redundant features go to a compact branch for low-cost global processing, and heterogeneous features go to a focused branch for fine-grained modeling with more computational resources.

Result: Achieves state-of-the-art performance on multiple benchmark datasets with substantially lower training time, parameter counts, and minimal computational cost among adaptive convolution models.

Conclusion: Bi²MAC effectively addresses regional heterogeneity in pansharpening while intelligently allocating computational resources, making it both effective and efficient compared to existing adaptive convolution methods.
...
TL;DR: HybridSplat accelerates 3D Gaussian splatting for reflective scenes using reflection-baked Gaussian tracing and hybrid splatting, achieving 7x faster rendering with 4x fewer primitives.
...
Method: Proposes HybridSplat with reflection-baked Gaussian tracing that bakes view-dependent reflection within each Gaussian primitive, uses tile-based Gaussian splatting for reflection rendering, integrates reflective and base Gaussian primitives in unified hybrid splatting framework, and adds pipeline-level acceleration with reflection-sensitive Gaussian pruning.

Result: Achieves 7x faster rendering speed across complex reflective scenes from Ref-NeRF and NeRF-Casting with 4x fewer Gaussian primitives than ray-tracing based Gaussian splatting baselines, while preserving reflection quality.

Conclusion: HybridSplat serves as a new state-of-the-art method for complex reflective scenes, significantly improving rendering speed and reducing memory requirements while maintaining high-fidelity reflection rendering.
...
TL;DR: DINO-BOLDNet uses DINOv3-guided multi-slice attention to generate BOLD images from T1w images, achieving better performance than conditional GAN baselines.


<details>
  <summary>Details</summary>
Motivation: To recover missing BOLD information when BOLD images are corrupted or unavailable, enabling downstream tasks that require functional MRI data.

Method: Proposes DINO-BOLDNet: integrates frozen self-supervised DINOv3 encoder with lightweight trainable decoder. Uses DINOv3 for within-slice structural representations, separate slice-attention module for cross-slice context fusion, multi-scale generation decoder for fine-grained functional contrast restoration, and DINO-based perceptual loss for structural/textural consistency.

Result: Experiments on 248-subject clinical dataset show DINO-BOLDNet surpasses conditional GAN baseline in both PSNR and MS-SSIM metrics. First framework capable of generating mean BOLD images directly from T1w images.

Conclusion: Demonstrates the potential of self-supervised transformer guidance for structural-to-functional mapping, offering a solution for BOLD image recovery when functional data is unavailable.
...
Conclusion: TrackingWorld successfully addresses limitations of existing methods by providing dense 3D tracking of almost all pixels within a world-centric coordinate system from monocular video.
...
TL;DR: SCU-CGAN model generates realistic fire images from nonfire images to augment limited fire datasets, improving fire detection model accuracy by 56.5% in mAP@0.5:0.95 for YOLOv5 nano.


<details>
  <summary>Details</summary>
Motivation: Fire detection is crucial for safety but limited by insufficient fire datasets. Home IoT fire detection systems need better data to improve model performance.

Method: Proposed SCU-CGAN model integrates U-Net, CBAM (Channel and Spatial Attention Module), and an additional discriminator to generate realistic fire images from nonfire images.

Result: SCU-CGAN achieved 41.5% improvement in KID score over CycleGAN, demonstrating superior image quality. Augmented dataset improved YOLOv5 nano's mAP@0.5:0.95 by 56.5% without model structure changes.

Conclusion: SCU-CGAN effectively generates high-quality fire images for dataset augmentation, significantly boosting fire detection model performance and addressing data scarcity in fire safety applications.
...
TL;DR: The paper identifies a norm disparity problem in MLLMs using Pre-Norm architecture, where visual tokens have much higher norms than text tokens, causing asymmetric learning dynamics that hinder cross-modal fusion. The authors propose adding a single LayerNorm after the visual projector to align norms, which significantly improves performance on both multimodal and text-only benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) using Pre-Norm architecture suffer from a critical flaw: severe norm disparity between high-norm visual tokens and low-norm text tokens. This imbalance creates asymmetric update dynamics where visual tokens learn much slower than text tokens, fundamentally impairing effective cross-modal feature fusion and limiting model performance.

Method: The authors first provide formal theoretical analysis showing the norm disparity induces "asymmetric update dynamic" with visual tokens exhibiting "representational inertia." They then propose a simple solution: inserting a single, carefully initialized LayerNorm layer after the visual projector to enforce norm alignment between visual and text tokens, addressing the architectural imbalance.

Result: Empirical validation across mainstream MLLMs confirms the norm disparity and asymmetric update rates are prevalent. Experiments on LLaVA-1.5 show the LayerNorm intervention yields significant performance gains on a wide suite of multimodal benchmarks and, notably, also improves text-only evaluations like MMLU, indicating more holistic model capability.

Conclusion: The norm disparity in Pre-Norm MLLMs is a fundamental architectural flaw that hinders cross-modal learning. A simple LayerNorm solution effectively aligns norms, resolving asymmetric update dynamics and leading to substantially improved performance across both multimodal and unimodal tasks, demonstrating the importance of architectural balance in MLLM design.
...
<details>
  <summary>Details</summary>
Motivation: Existing image enhancement methods either amplify noise excessively or only work well under specific lighting conditions, limiting their effectiveness in challenging illumination scenarios where captured images suffer from various degradations.

Method: Three-stage approach: 1) GDWGIF for illumination estimation and initial quality improvement, 2) Retinex decomposition into illumination/reflection layers with parallel processing (illumination correction + reflection enhancement), 3) Dynamic range optimization via multi-exposure fusion and linear stretching.

Result: Experimental evaluation on real-world datasets shows superior performance compared to state-of-the-art methods in both contrast enhancement and noise suppression.

Conclusion: The proposed framework effectively addresses the limitations of existing methods by simultaneously handling enhancement and noise suppression under complex illumination conditions.
...
TL;DR: This paper addresses the problem of facial retouching detection in biometric systems, proposing to use beauty assessment algorithms and AI-based feature extraction to improve detection rates, achieving 1.1% D-EER on single image detection.


<details>
  <summary>Details</summary>
Motivation: Facial retouching is widely used for beautification but becomes problematic when retouched images are used as biometric samples in face recognition systems, as it challenges system accuracy. Detection of facial retouching is increasingly necessary for biometric security.

Method: The paper studies changes in beauty assessment algorithms on retouched images, assesses different AI-based feature extraction methods for retouching detection, and evaluates whether face beauty assessment can enhance detection rates. The approach works in scenarios where the attacking retouching algorithm is unknown.
...
Conclusion: Facial retouching detection is crucial for biometric system integrity, and leveraging beauty assessment algorithms with AI-based feature extraction can effectively detect retouched images, achieving low error rates even in challenging scenarios with unknown retouching methods.
...
TL;DR: Optimized deep learning pipeline for fish re-identification using EM system data, with Swin-T outperforming ResNet-50 through hard triplet mining and custom image transformations.
...
Method: Developed optimized deep learning pipeline using AutoFish dataset (simulating EM conveyor belts with 6 similar fish species), employing hard triplet mining and custom image transformation pipeline with dataset-specific normalization, comparing Vision Transformer (Swin-T) vs CNN (ResNet-50).

Result: Swin-T consistently outperformed ResNet-50, achieving 41.65% mAP@k and 90.43% Rank-1 accuracy. Main challenge was intra-species errors where viewpoint inconsistency was more detrimental than partial occlusion.

Conclusion: The optimized pipeline with hard triplet mining and custom transformations effectively automates fish re-identification for EM systems, with transformer architectures showing superior performance over CNNs for this task.
...
Result: SAM-Body4D achieves improved temporal stability and robustness in challenging in-the-wild videos without any retraining, demonstrating better performance than per-frame methods.

Conclusion: The proposed training-free framework effectively addresses temporal inconsistency and occlusion issues in video-based human mesh recovery by leveraging inherent human continuity in videos.
...
TL;DR: OneClip-RAG: An efficient video retrieval augmentation method that enables MLLMs to process long videos by using one-shot video clips for knowledge integrity and semantic coherence, with query-guided chunking and progressive training.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) struggle with long videos due to excessive memory overhead, limiting them to processing only a few frames. There's a need for an efficient method to handle extended video content while maintaining performance.

Method: Proposes One-shot video-Clip based Retrieval AuGmentation (OneClip-RAG) with: 1) Query-guided video chunking algorithm that unifies clip chunking and cross-modal retrieval in one step, 2) Progressive training using SynLongVideo dataset for instruction following, 3) Integration with existing MLLMs for enhanced video understanding.

Result: Significant performance gains on long-video benchmarks: boosts InternLV2 8B and Qwen2-VL 7B to GPT-4o level on MLVU. Superior efficiency: enables LLaVA-Video to understand hour-long videos in under 2.2 minutes on a single 4090 GPU.

Conclusion: OneClip-RAG effectively addresses memory limitations of MLLMs for long video processing, achieving both high performance and computational efficiency through innovative retrieval augmentation and query-guided chunking techniques.
...
TL;DR: A depth-only 6D pose estimation method for industrial bin-picking using multi-view depth fusion with staged heatmap attention and sparse transformers to handle occlusions and clutter.


<details>
  <summary>Details</summary>
Motivation: Industrial bin-picking faces challenges with occlusions, reflections, and textureless parts in densely packed environments, requiring accurate 6D pose recovery for robotic applications.
...
Conclusion: The method enables high-resolution volumetric representations for accurate pose estimation in clutter, addressing challenges in close-range robotic applications where sparse 3D approaches have been underexplored.
...
TL;DR: LapFM is a surgical foundation model that learns robust segmentation from massive unlabeled laparoscopic images using hierarchical concept evolving pre-training, outperforming existing methods in universal laparoscopic segmentation.


<details>
  <summary>Details</summary>
Motivation: Surgical segmentation faces challenges with annotation scarcity and semantic inconsistency across diverse procedures. Existing approaches are limited as domain adapters rather than true surgical foundation models, struggling to generalize across surgical variability.

Method: LapFM uses Hierarchical Concept Evolving Pre-training: 1) Laparoscopic Concept Hierarchy (LCH) with hierarchical mask decoder and parent-child query embeddings to unify diverse entities; 2) Confidence-driven Evolving Labeling that iteratively generates and filters pseudo-labels based on hierarchical consistency, creating LapBench-114K dataset.

Result: LapFM significantly outperforms state-of-the-art methods, establishing new standards for granularity-adaptive generalization in universal laparoscopic segmentation.

Conclusion: LapFM successfully evolves robust segmentation capabilities from unlabeled surgical images, addressing annotation scarcity and semantic inconsistency while demonstrating superior generalization across diverse surgical targets.
...
TL;DR: Unified learning framework for color correction using high-res RGB + low-res multispectral sensors, achieving 50% error reduction over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing color correction methods treat pipeline in separate stages and discard multispectral data early, missing opportunities for improved color accuracy.

Method: End-to-end learning framework that jointly leverages high-resolution RGB and low-resolution multispectral sensor data, integrated within single model using refactored image-to-image architectures.

Result: Improves color accuracy and stability, reducing error by up to 50% compared to RGB-only and MS-driven baselines.

Conclusion: Proposed unified framework demonstrates superior performance for color correction by effectively integrating multispectral information throughout the pipeline.
...
TL;DR: Subset selection methods for explaining vision models degrade under out-of-distribution (OOD) conditions. The paper introduces an uncertainty-aware submodular optimization framework that improves explanation robustness in both ID and OOD settings without requiring additional training.


<details>
  <summary>Details</summary>
Motivation: Current subset selection methods for explaining deep vision models work well in in-distribution settings but their reliability degrades significantly under out-of-distribution conditions, producing redundant, unstable, and uncertainty-sensitive explanations. There's a need for more robust explanation methods that work reliably across different data distributions.

Method: The framework combines submodular subset selection with layer-wise, gradient-based uncertainty estimation. It estimates uncertainty through adaptive weight perturbations and uses these estimates to guide submodular optimization, ensuring diverse and informative subset selection without requiring additional training or auxiliary models.

Result: Empirical evaluations show the framework mitigates weaknesses of existing methods under OOD scenarios and also yields improvements in ID settings. It produces more robust, stable, and informative explanations compared to current subset-based approaches.

Conclusion: Current subset-based explanation methods have significant limitations under OOD conditions. The proposed uncertainty-driven optimization framework enhances attribution and object-level interpretability, paving the way for more transparent and trustworthy AI in real-world vision applications.
...
TL;DR: Lightweight SAM-based football tracking method combining Segment Anything Model with CSRT trackers and jersey color appearance models achieves 7.6-7.7 FPS with stable memory, maintaining high tracking success in most scenarios but struggling with long-term occlusions.


<details>
  <summary>Details</summary>
Motivation: Football player tracking faces challenges from frequent occlusions, similar player appearances, and rapid motion in crowded scenes, requiring robust yet lightweight solutions for practical deployment.

Method: Team-aware tracking system using SAM for precise initialization, CSRT trackers for continuous tracking, and HSV histogram-based appearance models for re-identification and occlusion recovery.

Result: Achieves 7.6-7.7 FPS with ~1880 MB memory, 100% tracking success in light occlusions, 90% in crowded penalty-box scenarios, and recovers 50% of heavy occlusions, but only 8.66% re-acquisition success for long-term occlusions where players leave frame.

Conclusion: SAM+CSRT combination provides consistent performance across crowd densities but requires stronger re-identification mechanisms for extended absences; offers practical guidelines for deploying football tracking under resource constraints.
...
TL;DR: ContextDrag is a new drag-based image editing method that leverages contextual information from reference images to preserve fine-grained texture details and improve edit coherence without requiring finetuning or inversion.
...
Method: Built on efficient WebGPU renderer with per-frame ONNX inference, introduces standardized Gaussian Generator contract for plug-and-play algorithms, offers three.js library integration, and uses GPU-based primitive sorting for efficiency.

Result: Achieves superior rendering efficiency compared to current Web viewers, supports multiple variants (MLP-based 3DGS, 4DGS, neural avatars, style transformation), and enables feedforward generative post-processing.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion model evaluation focuses only on final outputs, but understanding the dynamic generation process is crucial for assessing controllability, reliability, and failure modes. The paper investigates when noise transforms into specific concepts during denoising.

Method: Proposes Prompt-Conditioned Intervention (PCI) - a training-free, model-agnostic framework that analyzes Concept Insertion Success (CIS). CIS measures the probability that a concept inserted at a given timestep is preserved in the final image, characterizing temporal dynamics of concept formation.

Result: Applied to state-of-the-art text-to-image diffusion models across diverse concepts, PCI reveals varied temporal behaviors where certain trajectory phases are more favorable to specific concepts. The framework provides actionable insights for text-driven image editing, enabling more effective interventions without model access or training.

Conclusion: PCI offers a novel way to analyze diffusion model dynamics, showing when concepts lock in during generation. This enables stronger, more balanced image edits and provides fundamental understanding of diffusion model behavior beyond final outputs.
...
TL;DR: First on-the-fly 3D reconstruction framework for multi-camera rigs using 3D Gaussian Splatting, achieving drift-free trajectory estimation and efficient online reconstruction with hierarchical initialization and redundancy-free optimization.


<details>
  <summary>Details</summary>
Motivation: Existing on-the-fly 3DGS methods using monocular RGB streams fail to achieve complete 3D coverage due to limited field of view. Multi-camera rigs address this limitation but lack efficient reconstruction frameworks.

Method: 1) Hierarchical camera initialization for coarse inter-camera alignment without calibration; 2) Lightweight multi-camera bundle adjustment for stable trajectories; 3) Redundancy-free Gaussian sampling strategy; 4) Frequency-aware optimization scheduler to reduce primitives and iterations.

Result: Reconstructs hundreds of meters of 3D scenes within 2 minutes using only raw multi-camera video streams, demonstrating unprecedented speed, robustness, and fidelity for on-the-fly 3D scene reconstruction.

Conclusion: The framework enables efficient, drift-free, real-time 3D reconstruction from multi-camera rigs, overcoming FOV limitations of monocular approaches while maintaining reconstruction quality and performance.
...
TL;DR: ReasonBreak is an adversarial framework that disrupts MLRMs' hierarchical reasoning for geographic location inference using concept-aware perturbations, achieving significant privacy protection improvements.
...
Method: ReasonBreak introduces concept-aware perturbations that target critical conceptual dependencies within reasoning chains. The approach is based on the insight that disrupting geographic reasoning requires perturbations aligned with conceptual hierarchies rather than uniform noise. The framework generates perturbations that invalidate specific inference steps and cascade through subsequent reasoning stages. The authors also contribute GeoPrivacy-6K, a dataset of 6,341 ultra-high-resolution images with hierarchical concept annotations.
...
Conclusion: ReasonBreak establishes a new paradigm for privacy protection against reasoning-based threats by effectively disrupting hierarchical reasoning in MLRMs through concept-aware perturbations.
...
TL;DR: NoisyCLIP enables real-time detection of text-image misalignments during diffusion model generation by measuring semantic alignment in noisy latent space, reducing computational cost by 50% while maintaining 98% of CLIP performance.
...
Conclusion: Early detection of text-image misalignments during diffusion model generation is feasible and efficient. NoisyCLIP provides a practical solution for real-time alignment assessment that significantly reduces computational costs while maintaining high semantic fidelity.
...
<details>
  <summary>Details</summary>
Motivation: The main challenge is accurately reconstructing building surfaces from LiDAR point clouds that suffer from varying point densities and noise interference, requiring flexible methods to capture high-quality 3D profiles.

Method: OCCDiff combines latent diffusion with function autoencoder architecture to generate continuous occupancy functions evaluable at arbitrary locations. It uses a point encoder to provide condition features, constrain occupancy predictions, and insert multi-modal features. Multi-task training enhances feature learning.
...
Conclusion: OCCDiff effectively addresses building reconstruction challenges from LiDAR point clouds by leveraging latent diffusion in occupancy function space with robust feature learning through multi-task training.
...
TL;DR: sCoT is a new visual reasoning method that reformulates multimodal Chain-of-Thought as language-only reasoning with self-calling subagents, achieving better performance with significantly less training time.


<details>
  <summary>Details</summary>
Motivation: Current thinking-with-images paradigms face challenges in optimizing interleaved multimodal CoT through reinforcement learning due to scarcity of high-quality reasoning data.

Method: Proposes Self-Calling Chain-of-Thought (sCoT) where a main agent decomposes visual reasoning tasks into atomic subtasks and invokes parameter-sharing subagents to solve them in isolated context, using group-relative policy optimization for reinforcement.

Result: On HR-Bench 4K, sCoT improves overall reasoning performance by up to 1.9% while using ~75% fewer GPU hours compared to strong baselines.

Conclusion: sCoT provides an effective and efficient visual reasoning paradigm that avoids explicit interleaving between modalities and enhances optimization through self-calling architecture.
...
Conclusion: MVP effectively addresses coordinate prediction instability in GUI grounding by aggregating predictions from multiple views, providing a training-free solution that significantly improves model performance across different architectures and scales.
...
<details>
  <summary>Details</summary>
Motivation: Oil painting is challenging for digital generation/editing due to complex brushstroke dynamics and stylized characteristics. Existing methods are limited by training data distribution and focus mainly on modifying real photos rather than artistic creation.

Method: Three key technical advancements: 1) Enhanced training with spatial alignment and semantic enhancement conditioning (masks/sketches → spatial constraints, reference images/text → feature constraints), 2) Self-supervised style transfer pipeline using Stroke-Based Rendering to create large-scale paired training data, 3) AdaIN operator integration during inference for stylistic consistency.

Result: The system enables fine-grained editing while preserving artistic qualities, achieving unprecedented imagination realization in stylized oil painting generation and editing through extensive experiments.

Conclusion: Proposed unified multimodal framework successfully addresses oil painting generation/editing challenges by combining multiple input modalities with consistent style preservation, enabling interactive artistic creation.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D-native generators produce reliable geometry but lack realistic appearances due to limited availability of diverse, high-quality real-world 3D assets with rich texture details. Capturing such data is difficult due to diverse scene scales, non-rigid object motions, and limited 3D scanner precision.

Method: 1) Uses GPT-4o-Image generated images as data source; 2) Designs structure-aligned multi-view synthesis pipeline to address multi-view inconsistency; 3) Constructs detail-enhanced multi-view dataset paired with 3D geometry; 4) Implements realistic detail enhancement scheme using perceptual feature adaptation and semantic structure matching; 5) Provides dedicated training strategies for both geometry-texture coupled and decoupled 3D-native generation paradigms.
...
Conclusion: Photo3D successfully bridges the gap between reliable geometry generation and realistic appearance synthesis by leveraging AI-generated image data and novel consistency-preserving techniques, enabling high-quality photorealistic 3D generation without requiring extensive real-world 3D capture.
...
<details>
  <summary>Details</summary>
Motivation: AR-diffusion hybrid models suffer from high latency due to sequential AR generation and iterative denoising, creating a bottleneck that needs to be addressed.

Method: 1) Entropy-informed speculative strategy aligns draft model's entropy with target model to reduce rejection rates. 2) Dynamic scheduler integrates diffusion into end-to-end framework, prioritizing AR optimization. 3) Joint distillation combines trajectory and distribution matching for stable training. 4) Shallow feature entropy pre-filters low-entropy drafts during inference.
...
Conclusion: Fast-ARDiff successfully tackles the latency bottleneck in AR-diffusion hybrids through unified optimization, enabling efficient high-quality synthesis with minimal steps.
...
TL;DR: Proposes quaternion Wasserstein distance for color image generation, addressing chromatic aberration by modeling color channel correlations, with a novel Wasserstein quaternion GAN that outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing color image generation models ignore correlations among color channels, leading to chromatic aberration problems. Also, there's a lack of systematic theory for measuring color image datasets and their data distributions.

Method: Defines new quaternion Wasserstein distance and develops its dual theory using quaternion convex set separation theorem and quaternion Farkas lemma. Proposes Wasserstein quaternion generative adversarial network (WQGAN) using this distance.

Result: The novel WQGAN model surpasses both standard/quaternion GANs and Wasserstein GANs in terms of generation efficiency and image quality, as demonstrated through experiments.

Conclusion: Quaternion Wasserstein distance effectively addresses color channel correlation issues in image generation, providing a theoretical foundation for measuring color image datasets and improving generation performance.
...
Method: Derives explicit fixed-point expression from ideal inversion step, introduces error approximation using previous step's calculable error to estimate current unknown error, creating unbiased low-variance estimator without iterations.
...
Conclusion: Proposed iteration-free fixed-point estimator effectively addresses computational limitations of existing methods while maintaining or improving reconstruction accuracy in diffusion inversion tasks.
...
TL;DR: SSCATeR algorithm uses LiDAR scanning motion and temporal data recycling to focus object detection only on changing regions, achieving 6.61x speedup while maintaining accuracy identical to traditional sparse convolution methods.


<details>
  <summary>Details</summary>
Motivation: Traditional LiDAR object detection processes entire point clouds each frame, which is computationally expensive. Since LiDAR scans continuously and many regions remain unchanged between frames, there's significant redundancy in processing static areas repeatedly.

Method: Uses sliding time windows with short strides and stores convolution results between passes to track temporal changes. Extends scatter-based convolutions to support data reuse, creating SSCATeR (Sparse Scatter-Based Convolution Algorithm with Temporal Data Recycling) that only processes changing parts of point clouds.

Result: Achieves identical feature maps to traditional sparse convolution techniques while reducing processing time by up to 6.61x. The method effectively introduces extreme sparsity to detection data and exploits it through data reuse.

Conclusion: SSCATeR enables efficient LiDAR object detection by treating LiDAR data as a continuous stream and focusing computation only on changing regions, significantly improving computational efficiency without sacrificing detection accuracy.
...
TL;DR: Large-scale automated framework discovers and explains visual concept representations across human cortex using unsupervised pattern discovery and automated explanation generation with natural language descriptions.


<details>
  <summary>Details</summary>
Motivation: Understanding how the human brain represents visual concepts and where these representations are encoded remains challenging due to the complexity of brain signals and vast space of possible visual concepts. Most studies are small-scale, rely on manual inspection, focus on specific regions, and lack systematic validation.

Method: Two-stage framework: 1) Discover candidate interpretable patterns in fMRI activity through unsupervised data-driven decomposition methods; 2) Explain each pattern by identifying natural images that most strongly elicit it and generating natural-language descriptions of their shared visual meaning. Uses automated pipeline to test multiple candidate explanations, assign reliability scores, and select most consistent descriptions.
...
Conclusion: The automated large-scale approach enables systematic discovery and explanation of visual representations across the human cortex, overcoming limitations of traditional small-scale manual methods.
...
TL;DR: A modular neural ISP framework that processes raw images with full control over intermediate stages, enabling high-quality rendering, scalability, and flexible photo editing.


<details>
  <summary>Details</summary>
Motivation: Prior neural ISP designs lack modularity and control over intermediate rendering stages, limiting debuggability, scalability, generalization to different cameras, and flexibility for user preferences.

Method: Modular neural ISP framework with intermediate stage control, learning-based approach with variants of different capacities (0.5M to 3.9M parameters), supporting user-interactive photo-editing tool.

Result: Achieves competitive qualitative and quantitative results across multiple test sets, enables unlimited post-editable re-rendering, and supports diverse editing operations and picture styles.

Conclusion: Modular neural ISP design provides superior control, scalability, and flexibility compared to prior approaches, enabling practical photo-editing applications with high-quality rendering.
...
Result: Extensive experiments across eight CTTA and TTA scenarios (including synthetic-to-real and long-term shifts) show consistent outperformance of state-of-the-art techniques, setting new standard for semantic segmentation under evolving conditions.

Conclusion: The proposed approach produces more reliable supervision and mitigates error accumulation throughout continual adaptation, effectively handling the challenges of semantic segmentation in evolving domains.
...
TL;DR: HiGINE is a hierarchical graph-based method that predicts lung cancer patient survival from multiplex immunofluorescence images by modeling cell neighborhood interactions, with multimodal fusion of cancer stage data improving performance.


<details>
  <summary>Details</summary>
Motivation: The tumor microenvironment (TME) contains valuable prognostic biomarkers, but current analysis methods need to better capture complex cellular interactions to fully leverage TME's potential for improving cancer risk stratification.

Method: HiGINE uses hierarchical graph-based approach to encode local and global inter-relations in cell neighborhoods from mIF images, incorporating cell type and morphology information, with multimodal fusion of cancer stage data.

Result: Validated on two public datasets, HiGINE demonstrates improved risk stratification, robustness, and generalizability for predicting short vs. long survival in lung cancer patients.

Conclusion: HiGINE effectively captures TME complexity through hierarchical graph modeling and multimodal fusion, offering enhanced prognostic capabilities for lung cancer survival prediction.
...
TL;DR: Automated video alignment system for surgical recordings that addresses occlusion by surgeons by using multiple cameras on shadowless lamps, automatically realigning views after lamp movements, and selecting the least occluded camera to generate consistent surgical field videos.


<details>
  <summary>Details</summary>
Motivation: Video recordings of open surgeries are valuable for education and research, but surgeons frequently block the camera view. Current multi-camera setups on shadowless lamps help but require manual image alignment when surgeons move the lamp for lighting adjustments.

Method: The method identifies frames when the lighting system moves, automatically realigns them, and selects the camera with the least occlusion to generate a video that consistently presents the surgical field from a fixed perspective.

Result: Surgeon user study showed videos generated by this method were superior to conventional methods in ease of confirming surgical area and viewing comfort. The approach also improved video quality over existing techniques, and user studies assessed preferences for different synthesis options.

Conclusion: The proposed automated alignment system successfully addresses occlusion issues in surgical video recording, providing superior video quality and viewing experience compared to conventional methods, with validated surgeon preferences for different synthesis options.
...
TL;DR: Deep learning models (YOLOv8s and MobileNetV3L) applied to pollen grain detection/classification in optical and holographic microscopy, with performance improvements via dataset expansion techniques.
...
Result: Optical images: 91.3% mAP50 for detection, 97% overall accuracy for classification. Initial holographic images: substantially lower performance. After dataset expansion: detection improved from 2.49% to 13.3% mAP50, classification from 42% to 54%.

Conclusion: Deep learning techniques can be successfully paired with cost-effective lensless digital holographic microscopy devices for image classification tasks, demonstrating feasibility for veterinary cytology applications.
...
TL;DR: CLIP's few-shot learning suffers from template-sample similarity bias, where models rely on template proximity rather than true category alignment. The paper proposes using empty prompts to capture unbiased template features and offset this bias through pre-training and fine-tuning stages.


<details>
  <summary>Details</summary>
Motivation: The paper identifies that CLIP's few-shot learning performance is biased by template-sample similarity (TSS), where the model prioritizes template resemblance over genuine category alignment. This bias reduces both accuracy and robustness in classification tasks.

Method: A two-stage framework: 1) Pre-training uses empty prompts (text inputs conveying "emptiness" without category info) to reveal and reduce template-induced bias within CLIP encoder. 2) Few-shot fine-tuning employs a bias calibration loss to enforce correct image-category alignment, focusing on relevant visual cues.

Result: Experiments across multiple benchmarks show the template correction method significantly reduces performance fluctuations caused by TSS, yielding higher classification accuracy and stronger robustness compared to baseline CLIP.

Conclusion: The proposed framework effectively decouples template bias in CLIP, improving few-shot learning by ensuring models focus on genuine category alignment rather than template proximity, with code available for reproducibility.
...
TL;DR: OpenMonoGS-SLAM: First monocular SLAM framework combining 3D Gaussian Splatting with open-set semantic understanding using visual foundation models, without depth sensors or semantic ground truth.
...
Method: Unifies 3D Gaussian Splatting with open-set semantic understanding using Visual Foundation Models: MASt3R for visual geometry, SAM and CLIP for open-vocabulary semantics. Uses self-supervised learning without depth input or 3D semantic ground truth. Includes a memory mechanism for managing high-dimensional semantic features to construct Gaussian semantic feature maps.

Result: Achieves performance comparable to or surpassing existing baselines in both closed-set and open-set segmentation tasks. Works without supplementary sensors like depth maps or semantic annotations.

Conclusion: OpenMonoGS-SLAM demonstrates successful integration of 3D Gaussian Splatting with open-set semantic understanding using visual foundation models, enabling robust monocular SLAM with rich semantic perception in open-world environments without specialized hardware.
...
TL;DR: A novel method that estimates metric depth from video blur patterns and dense trajectories using joint optical design, achieving strong performance across large depth ranges with good generalization.


<details>
  <summary>Details</summary>
Motivation: Camera rotational dynamics during handheld shooting cause perspective-based blur that varies with object depth, creating depth-dependent blur patterns that can be exploited for depth estimation.

Method: Uses off-the-shelf vision encoder and point tracker for video information, estimates depth via windowed embedding and multi-window aggregation, and densifies sparse optical trajectories using vision-language models.

Result: Achieves strong performance on multiple depth datasets across large depth ranges with good generalization; optical algorithm shows superior precision compared to real handheld trajectories.

Conclusion: Depth can be effectively estimated from perspective-based blur patterns in videos, and the proposed joint optical design approach enables accurate metric depth estimation and dense trajectory reconstruction.
...
TL;DR: A unified aerial VLN framework using only monocular RGB images and language instructions, formulated as next-token prediction with multi-task learning, achieving strong results comparable to panoramic RGB-D methods.


<details>
  <summary>Details</summary>
Motivation: Existing aerial VLN methods require panoramic images, depth inputs, or odometry, which increase system cost and complexity, hindering practical deployment on lightweight UAVs. There's a need for simpler, more practical solutions.

Method: Formulates navigation as next-token prediction problem with prompt-guided multi-task learning. Uses keyframe selection to reduce visual redundancy, and action merging with label reweighting to address long-tailed supervision imbalance.

Result: Achieves strong results on Aerial VLN benchmark under monocular RGB-only setting, outperforming existing RGB-only baselines and narrowing performance gap with state-of-the-art panoramic RGB-D methods.

Conclusion: The proposed unified framework demonstrates that effective aerial VLN can be achieved with only monocular RGB observations, reducing system complexity while maintaining competitive performance.
...
TL;DR: CoIG framework makes image generation transparent by using LLMs to break prompts into step-by-step instructions, enabling monitoring and control like Chain-of-Thought does for LLMs.


<details>
  <summary>Details</summary>
Motivation: Current image generation models are "black boxes" with opaque internal processes, limiting human observation, intervention, reliability, safety, and control. Their non-human-like workflows make them difficult to interpret.

Method: CoIG uses an LLM to decompose complex prompts into sequential, simple step-by-step instructions. The image generation model then executes this plan by progressively generating and editing the image, with each step focusing on a single semantic entity.
...
Conclusion: CoIG reframes image generation as a sequential, semantic process analogous to human art creation, providing transparency, monitorability, and control while being model-agnostic and integrable with any image generation model.
...
<details>
  <summary>Details</summary>
Motivation: Driver distraction recognition needs real-time edge inference, but lightweight models struggle with fine-grained cues and generalization, while ROI methods increase computational costs, creating a need for balanced efficiency and accuracy.
...
Result: On State Farm Distracted Driver Detection Dataset, C-DIRA maintains high accuracy with significantly fewer FLOPs and lower latency than prior lightweight models, plus robustness to visual degradation (blur, low-light) and stable performance across unseen domains.

Conclusion: C-DIRA effectively achieves compactness, efficiency, and generalization for driver behavior recognition, balancing computational constraints with robust performance across varying conditions.
...
TL;DR: MNAME is a plug-and-play training framework that uses a memory bank mechanism to provide abundant negative samples for contrastive learning in generative models, eliminating dependency on external encoders while improving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Current denoising generative models (diffusion, flow-matching) have high training costs and inefficient representation learning. Existing approaches that inject discriminative representations via auxiliary alignment rely on external pre-trained encoders, introducing overhead and domain shift issues.

Method: Proposes MNAME framework with memory bank mechanism that maintains a large, dynamically updated queue of negative samples across training iterations. This decouples negative sample count from mini-batch size, providing abundant negatives for contrastive objectives without computational overhead. Uses low-dimensional projection head to minimize memory/bandwidth requirements.
...
Conclusion: MNAME offers three key advantages: (1) self-contained (no dependency on pretrained vision foundation models), (2) no additional parameters or computational cost during inference, and (3) enables faster convergence with superior generative quality.
...
TL;DR: A novel Dual-Branch Center-Surrounding Contrast (CSCon) framework for 3D point cloud SSL that combines masking strategies with patch-level contrastive learning to capture both high-level discriminative features and local geometric details.


<details>
  <summary>Details</summary>
Motivation: Existing 3D SSL methods are dominated by generative MAE approaches that struggle with high-level discriminative features, while contrastive methods (effective in 2D) are scarce in 3D and don't effectively capture 3D local details when directly applied.

Method: Proposes CSCon framework with dual-branch masking: separate masking of center and surrounding parts to create center-biased and surrounding-biased representations. Uses patch-level contrastive loss to enhance both high-level information and local sensitivity.

Result: Achieves comparable performance to generative methods under FULL/ALL protocols, and SOTA results under MLP-LINEAR, MLP-3, and ONLY-NEW protocols, surpassing cross-modal approaches. Specifically, outperforms Point-MAE baseline by 7.9%, 6.7%, and 10.3% on three ScanObjectNN variants under MLP-LINEAR.

Conclusion: CSCon effectively addresses limitations of existing 3D SSL methods by combining dual-branch masking with contrastive learning, achieving superior discriminative feature learning and local detail capture for 3D point clouds.
...
TL;DR: MoSAIC-ReID is a Mixture-of-Experts framework that quantifies pedestrian attribute importance for person re-identification, providing interpretability while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Current person re-identification models achieve high accuracy but lack transparency about which semantic attributes they actually rely on for matching.

Method: Uses LoRA-based experts (each linked to a single attribute) with an oracle router for controlled attribution analysis, enabling systematic quantification of attribute importance.

Result: Achieves competitive performance on Market-1501 and DukeMTMC datasets; reveals clothing colors and intrinsic characteristics contribute most strongly, while infrequent cues like accessories have limited effect.

Conclusion: Provides a principled framework for interpretable ReID and highlights requirements for integrating explicit semantic knowledge in practice.
...
TL;DR: Proposes knowledge distillation for Full Surround Monocular Depth Estimation (FSMDE) to address computational cost and metric-scale depth challenges by transferring knowledge from foundation models to lightweight networks.


<details>
  <summary>Details</summary>
Motivation: Foundation models show strong generalization in monocular depth estimation but face two key challenges when applied to FSMDE: (1) high computational cost limiting real-time performance, and (2) difficulty estimating metric-scale depth since they're typically trained for relative depth only.

Method: A novel knowledge distillation strategy with hybrid regression framework combining knowledge distillation (traditionally for classification) with depth binning module. Includes cross-interaction knowledge distillation to distill scale-invariant depth bin probabilities from foundation model to student network while guiding metric-scale depth bin centers from ground truth. Also proposes view-relational knowledge distillation to encode structural relationships among adjacent camera views for cross-view depth consistency.
...
Conclusion: The proposed knowledge distillation approach successfully addresses computational efficiency and metric-scale depth estimation challenges in FSMDE by transferring robust depth knowledge from foundation models to lightweight networks while maintaining real-time performance.
...
TL;DR: A training-free Open-Vocabulary Semantic Segmentation method for remote sensing using SAM 3, combining mask fusion and presence score filtering to handle dense small targets.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based OVSS methods struggle with precise localization and require complex pipelines, especially for remote sensing with numerous dense small targets. SAM 3 offers unified segmentation and recognition capabilities that could address these challenges.

Method: Two key adaptations: 1) Mask fusion strategy combining SAM 3's semantic segmentation head and Transformer decoder outputs for better land coverage, 2) Using presence score from presence head to filter out non-existent categories and reduce false positives from large vocabularies and patch-level processing.

Result: The simple adaptation achieves promising performance on extensive remote sensing datasets, demonstrating SAM 3's potential for remote sensing OVSS without any training.

Conclusion: SAM 3 shows strong potential for training-free Open-Vocabulary Semantic Segmentation in remote sensing applications, with simple adaptations addressing challenges of dense small targets and large vocabularies.
...
TL;DR: This paper introduces a distribution-based framework for evaluating and mitigating individual fairness in skin lesion classification by treating skin tone as a continuous attribute rather than categorical, using kernel density estimation and statistical distance metrics to address biases.


<details>
  <summary>Details</summary>
Motivation: Current fairness research in medical imaging relies on coarse subgroup categories that overlook individual-level variations, risking obscuring biases faced by outliers within subgroups. Skin color has historically been a focal point of discrimination, necessitating more granular fairness approaches.

Method: Treat skin tone as continuous attribute, use kernel density estimation (KDE) to model its distribution, compare twelve statistical distance metrics to quantify disparities, and propose distance-based reweighting (DRW) loss function to correct underrepresentation in minority tones.

Result: Experiments show limitations of categorical reweighting and superior performance of distribution-based reweighting, particularly with Fidelity Similarity (FS), Wasserstein Distance (WD), Hellinger Metric (HM), and Harmonic Mean Similarity (HS).
...
TL;DR: A novel workflow combining 3D rendering and constrained diffusion models generates photorealistic synthetic mushroom images with automatic annotations, achieving state-of-the-art segmentation performance on real-world datasets using only synthetic training data.
...
Method: Integrates 3D rendering in Blender with a constrained diffusion model to automatically generate high-quality annotated, photorealistic synthetic images of Agaricus Bisporus mushrooms. Preserves full control over 3D scene configuration and annotations without requiring specialized graphics expertise.
...
TL;DR: A skewness-guided pruning method for multimodal Swin Transformers achieves 36% model size reduction with no accuracy loss in federated learning for medical imaging on edge devices.
...
Method: Proposes skewness-guided pruning that selectively prunes Multi-Head Self-Attention and Multi-Layer Perceptron layers based on statistical skewness of their output distributions in a multimodal Swin Transformer.

Result: Achieves approximately 36% model size reduction with no loss in accuracy when validated in a horizontal federated learning environment for skin lesion classification.
...
TL;DR: Wan-Move is a scalable framework for precise motion control in video generation using dense point trajectories and feature propagation without modifying base model architecture.
...
Method: Represents object motions with dense point trajectories for fine-grained control, projects trajectories into latent space, propagates first frame's features along each trajectory to create aligned spatiotemporal feature maps, and integrates these as motion guidance into existing image-to-video models without architecture changes.

Result: Generates 5-second, 480p videos with motion controllability comparable to commercial systems like Kling 1.5 Pro's Motion Brush. Introduces MoveBench benchmark with diverse content, longer durations, and high-quality annotations. Outperforms existing methods on both MoveBench and public datasets.

Conclusion: Wan-Move achieves precise, scalable motion control for video generation without requiring architectural modifications to base models, eliminating the need for auxiliary motion encoders and enabling easy fine-tuning scalability.
...
TL;DR: Self-refining diffusion framework uses XAI-based flaw detection to improve image generation quality by identifying artifacts and unrealistic regions, achieving up to 27.3% improvement in FID scores across various diffusion models and tasks.


<details>
  <summary>Details</summary>
Motivation: While diffusion models have achieved remarkable success in image synthesis, addressing artifacts and unrealistic regions remains a critical challenge that needs to be solved to improve image generation quality.

Method: Proposes self-refining diffusion framework that uses an explainable AI (XAI)-based flaw highlighter to produce flaw activation maps (FAMs). These FAMs identify artifacts and unrealistic regions, then improve reconstruction by amplifying noise in flawed regions during forward process and focusing on these regions during reverse process.

Result: Achieves up to 27.3% improvement in Fréchet inception distance (FID) across various diffusion-based models, demonstrates consistently strong performance on diverse datasets, and shows robust effectiveness across different tasks including image generation, text-to-image generation, and inpainting.
...
TL;DR: LoFA is a hypernetwork framework that efficiently predicts personalized LoRA weights for visual generative models by identifying structured distribution patterns in parameter changes, enabling fast adaptation within seconds instead of hours.
...
Result: Extensive experiments show LoFA consistently predicts high-quality personalized priors within seconds across multiple tasks and user prompts, even outperforming conventional LoRA that requires hours of processing.

Conclusion: LoFA bridges the gap between efficient personalization and quality adaptation by leveraging structured distribution patterns in LoRA parameter changes, enabling practical and fast model personalization.
...
TL;DR: MatteViT is a document shadow removal framework using spatial and frequency-domain information with high-frequency amplification and continuous luminance-based shadow matte to preserve text details while removing shadows.


<details>
  <summary>Details</summary>
Motivation: Document shadow removal is essential for digitized document clarity, and preserving high-frequency details like text edges and lines is critical since shadows often obscure or distort fine structures.

Method: Proposes MatteViT with two key strategies: 1) Lightweight high-frequency amplification module (HFAM) that decomposes and adaptively amplifies high-frequency components, and 2) Continuous luminance-based shadow matte generated using custom dataset and shadow matte generator for precise spatial guidance from early stages.

Result: Achieves state-of-the-art performance on public benchmarks (RDD and Kligler), provides robust practical solution for real-world document shadow removal, and better preserves text-level details in downstream tasks like OCR with improved recognition performance.

Conclusion: MatteViT effectively eliminates shadows while preserving fine-grained structural details through spatial and frequency-domain processing, offering superior performance for document enhancement and downstream applications.
...
TL;DR: Training-free Dual Hyperbolic Adapters (T-DHA) adapts VLMs to new domains without fine-tuning by using hyperbolic space for hierarchical vision-language relationships, achieving superior few-shot recognition and domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing VLM adaptation methods suffer from performance degradation with domain changes or require heavy computational resources for fine-tuning in new domains.

Method: T-DHA models vision-language hierarchical relationships in hyperbolic space (Poincaré ball model) instead of Euclidean space, leveraging exponential volume growth for better hierarchical embedding. Uses dual hyperbolic adapters with negative learning for robust classification.
...
Conclusion: Hyperbolic space modeling provides more accurate and robust VLM adaptation with fewer feature dimensions, enabling training-free domain adaptation with superior performance.
...
TL;DR: InfiniteVL is a linear-complexity VLM that combines sliding window attention with Gated DeltaNet to overcome limitations of window attention and linear attention, achieving competitive performance with minimal training data while maintaining constant latency and memory footprint.


<details>
  <summary>Details</summary>
Motivation: Window attention suffers performance degradation when sequence length exceeds window size, while linear attention underperforms on information-intensive tasks like OCR and document understanding. Need for a VLM architecture that overcomes both limitations with linear complexity.

Method: Proposes InfiniteVL architecture that synergizes sliding window attention (SWA) with Gated DeltaNet. Uses three-stage training: distillation pretraining, instruction tuning, and long-sequence SFT. Achieves linear complexity while maintaining performance.

Result: Using less than 2% of training data of leading VLMs, InfiniteVL outperforms previous linear-complexity VLMs and matches Transformer-based VLMs. Achieves 3.6× inference speedup over FlashAttention-2 accelerated Transformers with constant latency/memory. Maintains 24 FPS real-time prefill in streaming video.

Conclusion: InfiniteVL successfully addresses limitations of both window attention and linear attention, providing an efficient linear-complexity VLM that maintains competitive performance with minimal training data and demonstrates effective long-term memory retention.
...
<details>
  <summary>Details</summary>
Motivation: To investigate whether generative approaches (with decoder inversion) are necessary for achieving human-level visual perception capabilities like compositional generalization, given that today's most successful vision models are non-generative encoders.

Method: Theoretical analysis formalizing inductive biases required for compositional generalization in both decoder-based (generative) and encoder-based (non-generative) methods. Empirical evaluation on photorealistic image datasets comparing generative and non-generative approaches.

Result: Generative methods can enforce necessary inductive biases through decoder constraints and inversion, enabling compositional generalization. Non-generative methods generally cannot enforce these biases through regularization or architecture, often failing to generalize compositionally without large-scale pretraining or added supervision.

Conclusion: Generative approaches with decoder inversion are crucial for achieving compositional generalization in visual perception, supporting the hypothesis that human-level perception requires generative methods. The inversion can be performed efficiently through gradient-based search or generative replay.
...
TL;DR: Tri-Bench: A compact benchmark testing VLMs' verifiable geometric reasoning on planar triangles under camera pose variations and object interference, revealing models fail to use explicit frame-of-reference hints and default to 2D image cues.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models often fail under realistic scene changes, and verifiable geometric reasoning is critical for trustworthy and controllable agentic AI. The paper aims to isolate relative geometric reasoning while stressing deployment-critical factors.

Method: Created Tri-Bench with planar triangle problems testing camera pose (planar vs. tilted) and scene context via object interference (10 everyday objects). Evaluated four recent VLMs using a single fixed prompt with explicit guardrail describing a surrounding square border for homography-based correct answers. Tested six simple tasks over binary and continuous targets.

Result: Overall accuracy with respect to 3D ground truth is modest (~69% average, best ~75%, worst ~64%). Responses align more closely with 2D projections (~72% accuracy). All VLMs consistently fail on recognizing minority shape classes (equilateral, isosceles, right-angled triangles) with ~0% accuracy. Accuracy degrades by ~4.1% under camera tilt. Object interference has no significant effect on accuracy.

Conclusion: VLMs fail to correctly utilize explicit frame-of-reference hints provided in prompts and default to 2D image plane cues, demonstrating limitations in verifiable geometric reasoning despite explicit guidance.
...
<details>
  <summary>Details</summary>
Motivation: Image captioning is important for assisting visually impaired individuals and improving content management, but current methods struggle with low-resolution images and require heavy computational resources from large transformer models, making retraining difficult.

Method: SOLI employs a Siamese network architecture with dual-pathway neural network structure to optimize latent embeddings for low-resolution images, specifically designed for lightweight captioning systems.

Result: The approach minimizes computational overhead without sacrificing performance, making it suitable for resource-constrained scenarios while enhancing efficiency and accuracy of image-to-text translation.

Conclusion: SOLI presents an effective solution for lightweight, low-resolution image captioning that addresses computational resource challenges while maintaining captioning quality.
...
TL;DR: Enhanced VLM-based visual grounding in satellite imagery through structured localization mechanism with dedicated grounding module and control tokens, achieving 24.8% improvement over previous methods.
...
Method: Finetune pretrained VLM on diverse instruction-following tasks while interfacing a dedicated grounding module through specialized control tokens for structured localization.
...
Conclusion: Integrating structured spatial reasoning into VLMs enhances precise object localization, paving the way for more reliable real-world satellite data analysis.
...
<details>
  <summary>Details</summary>
Motivation: Rotation invariance is crucial for precise object-level segmentation in UAV aerial imagery where targets have arbitrary orientations and fine details. Conventional convolution operators in architectures like U-Net lack rotation invariance, degrading segmentation accuracy across varying viewpoints. While rotation invariance can be achieved by expanding filter banks across orientations, this significantly increases computational cost and memory traffic.

Method: Introduces a GPU-optimized rotation-invariant convolution framework that eliminates the traditional im2col step required for matrix-multiplication-based convolution. The method exploits structured data sharing among symmetrically rotated filters to achieve multi-orientation convolution with reduced memory traffic and computational redundancy. The approach is further generalized to accelerate convolution with arbitrary (non-symmetric) rotation angles.

Result: Achieves 20-55% faster training and 15-45% lower energy consumption than CUDNN while maintaining accuracy comparable to state-of-the-art rotation-invariant methods. In eight-orientation setting: up to 45% speedup and 41% energy savings on 256×256 inputs, and 32% speedup and 23% lower energy usage on 1024×1024 inputs. Integrated into U-Net segmentation model yields up to 6% accuracy improvement over non-rotation-aware baseline.

Conclusion: The proposed method provides an effective and highly efficient alternative to existing rotation-invariant CNN frameworks, demonstrating practical benefits for UAV aerial imagery segmentation through significant computational efficiency gains while maintaining or improving segmentation accuracy.
...
TL;DR: VALOR is an annotation-free training framework that improves visual reasoning by combining LLM reasoning refinement via reinforcement learning with VLM visual grounding enhancement through automated hard-negative mining.
...
Method: Proposes an annotation-free training framework using AI-powered verifiers: (1) LLM verifier refines LLM reasoning via reinforcement learning, (2) VLM verifier strengthens visual grounding through automated hard-negative mining. Combines language-only reasoning models for decomposing spatial queries with vision specialist models improved via VLM critics.

Result: The method improves visual reasoning and surpasses both open-source and proprietary models. With the improved visual grounding model, it further outperforms recent text-only visual reasoning methods across diverse spatial reasoning tasks.

Conclusion: VALOR demonstrates that annotation-free training with AI-powered verifiers can effectively improve both reasoning and grounding in visual reasoning tasks, combining the strengths of language reasoning models and vision specialist models without requiring ground truth labels.
...
TL;DR: UniLayDiff: A unified diffusion transformer model that handles all content-aware layout generation tasks with a single end-to-end trainable framework, treating constraints as distinct modality and using multi-modal diffusion transformers.


<details>
  <summary>Details</summary>
Motivation: Current methods for content-aware layout generation either handle only subsets of tasks or require separate models for different constraints, lacking a truly unified solution for diverse real-world applications with varying input constraints.

Method: Proposes UniLayDiff, a Unified Diffusion Transformer that treats layout constraints as a distinct modality and uses Multi-Modal Diffusion Transformer framework to capture interactions between background images, layout elements, and diverse constraints. Integrates relation constraints through LoRA fine-tuning after pretraining on other tasks.
...
Conclusion: UniLayDiff provides a unified solution for content-aware layout generation that handles diverse constraints with a single model, enhancing both unification and overall layout quality through its multi-modal diffusion transformer approach.
...
TL;DR: EvoScene is a training-free framework that progressively reconstructs complete 3D scenes from single images by combining geometric reasoning from 3D models and visual knowledge from video models through iterative 2D-3D refinement.


<details>
  <summary>Details</summary>
Motivation: Existing image-to-3D generators have object-centric training that limits generalization to complex, large-scale scenes with faithful structure and texture. There's a need for better single-image 3D scene reconstruction that handles complex scenes with both geometric stability and view-consistent textures.
...
Conclusion: EvoScene successfully addresses the challenge of generating high-quality, textured 3D scenes from single images by leveraging complementary strengths of existing models through an iterative refinement process, enabling practical 3D scene reconstruction without additional training.
...
TL;DR: LiDAS is an active illumination system that dynamically optimizes headlight patterns to maximize perception performance at night, enabling daytime-trained models to work at night without retraining.


<details>
  <summary>Details</summary>
Motivation: Nighttime environments challenge camera-based perception because existing methods passively rely on scene lighting. There's a need for active illumination control to improve perception without requiring model retraining for nighttime conditions.

Method: Closed-loop active illumination system combining off-the-shelf perception models with high-definition headlights. Dynamically predicts optimal illumination fields to maximize perception performance by reallocating light from empty areas to object regions. Trained on synthetic data and deployed zero-shot in real-world driving scenarios.

Result: +18.7% mAP50 and +5.0% mIoU improvement over standard low-beam at equal power. Maintains performance while reducing energy use by 40%. Enables zero-shot nighttime generalization of daytime-trained models.

Conclusion: LiDAS offers a cost-effective solution for robust nighttime perception by turning existing headlights into active vision actuators, complementing domain-generalization methods without requiring model retraining.
...
TL;DR: UniT is a unified text restoration framework that combines Diffusion Transformer, Vision-Language Model, and Text Spotting Module to restore degraded text in images while reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: Diffusion models for image restoration often produce text hallucinations in text-centric tasks due to lack of explicit linguistic knowledge, creating a need for better text-aware restoration.

Method: UniT integrates three components iteratively: 1) VLM extracts textual content from degraded images for explicit guidance, 2) TSM generates intermediate OCR predictions at each denoising step, 3) DiT backbone uses these cues to recover fine-grained text while suppressing hallucinations.

Result: Experiments on SA-Text and Real-Text benchmarks show UniT faithfully reconstructs degraded text, substantially reduces hallucinations, and achieves state-of-the-art end-to-end F1-score performance.

Conclusion: UniT successfully addresses text hallucinations in TAIR by integrating explicit linguistic guidance through iterative refinement between VLM, TSM, and DiT components.
...
<details>
  <summary>Details</summary>
Motivation: Reconstructing complex geometry and motion of dynamic scenes from video is challenging, requiring efficient methods that can handle multiple reconstruction tasks simultaneously.

Method: Uses unified transformer architecture with novel querying mechanism that avoids dense per-frame decoding and multiple task-specific decoders; enables flexible probing of 3D positions across space and time.

Result: Sets new state-of-the-art, outperforming previous methods across wide spectrum of 4D reconstruction tasks; achieves lightweight, highly scalable training and inference.

Conclusion: D4RT provides a simple yet powerful solution for dynamic scene reconstruction that is efficient, scalable, and achieves superior performance compared to existing approaches.
...
TL;DR: Selfi improves VGGT's 3D reconstruction by adding geometric consistency through feature alignment, achieving SOTA in novel view synthesis and pose estimation.


<details>
  <summary>Details</summary>
Motivation: VGGT models learn 3D implicitly from data but lack explicit multi-view geometric consistency, which limits their performance in 3D reconstruction tasks like novel view synthesis and camera pose estimation.

Method: Selfi is a self-improving pipeline that trains a lightweight feature adapter using reprojection-based consistency loss, distilling VGGT outputs into a geometrically-aligned feature space that captures 3D spatial proximity.

Result: Achieves state-of-the-art performance in both novel view synthesis and camera pose estimation, demonstrating that feature alignment significantly improves downstream 3D reasoning.

Conclusion: Feature alignment is a highly beneficial step for enhancing 3D reconstruction from vision foundation models, bridging the gap between implicit 3D learning and explicit geometric consistency.
...
TL;DR: Astra is an interactive general world model that generates realistic future video predictions from past observations and diverse action inputs across various real-world scenarios like autonomous driving and robotics.


<details>
  <summary>Details</summary>
Motivation: While diffusion transformers have advanced video generation, world models capable of predicting long-horizon futures from past observations and diverse actions remain underexplored, especially for general-purpose scenarios with various action forms.

Method: Uses autoregressive denoising architecture with temporal causal attention to aggregate past observations and support streaming outputs. Employs noise-augmented history memory to balance responsiveness with temporal coherence. Introduces action-aware adapter for precise action control and mixture of action experts to dynamically route heterogeneous action modalities.

Result: Astra achieves interactive, consistent, and general long-term video prediction supporting various interaction forms. Experiments across multiple datasets show improvements in fidelity, long-range prediction, and action alignment over state-of-the-art world models.

Conclusion: Astra bridges the gap in interactive general world modeling, enabling realistic future prediction across diverse real-world tasks with precise action control and temporal coherence.
...
### [130] [ThreadWeaver: Adaptive Threading for Efficient Parallel Reasoning in Language Models](https://arxiv.org/abs/2512.07843)
...
TL;DR: ThreadWeaver is a framework for adaptive parallel reasoning in LLMs that matches sequential reasoning accuracy while reducing inference latency through parallel reasoning threads, without requiring custom inference engines.


<details>
  <summary>Details</summary>
Motivation: Sequential decoding in LLMs causes substantial latency on complex reasoning tasks, while existing parallel reasoning methods either have accuracy drops, require supervised behavior cloning, or need custom inference engines that complicate deployment.

Method: Three key innovations: 1) Two-stage parallel trajectory generator for high-quality parallel CoT data, 2) Trie-based training-inference co-design enabling parallel reasoning on standard autoregressive engines, 3) Parallelization-aware reinforcement learning to balance accuracy with effective parallelization.

Result: On six mathematical reasoning benchmarks, ThreadWeaver on Qwen3-8B achieves 71.9% average accuracy (79.9% on AIME24) comparable to state-of-the-art sequential models, with up to 1.53x average speedup in token latency.

Conclusion: ThreadWeaver establishes a new Pareto frontier between accuracy and efficiency by enabling adaptive parallel reasoning that matches sequential reasoning performance while significantly reducing inference latency, without requiring custom inference engines.
...
### [131] [Balanced Accuracy: The Right Metric for Evaluating LLM Judges -- Explained through Youden's J statistic](https://arxiv.org/abs/2512.08121)
*Stephane Collot,Colin Fraser,Justin Zhao,William F. Shen,Timon Willi,Ilias Leontiadis*

Main category: cs.LG

TL;DR: The paper argues that Balanced Accuracy (or Youden's J statistic) is better than traditional metrics like Accuracy, Precision, and F1 for selecting classifiers in LLM evaluation, as it's robust to class imbalance and doesn't distort prevalence estimates.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluation relies on classifiers (LLM-as-judge or human annotators) to estimate behavior prevalence, but traditional metrics for choosing classifiers are problematic - they're sensitive to class imbalance and can favor judges that distort prevalence estimates, undermining trustworthy evaluation.

Method: Theoretical analysis shows Youden's J statistic is aligned with choosing the best judge for model comparison, with Balanced Accuracy being an equivalent linear transformation. The paper uses analytical arguments, empirical examples, and simulations to demonstrate the superiority of Balanced Accuracy for classifier selection.

Result: Balanced Accuracy is theoretically justified as the optimal metric for selecting classifiers in LLM evaluation, as it's robust to class imbalance and doesn't distort prevalence estimates, unlike traditional metrics like Accuracy, Precision, and F1.

Conclusion: Researchers should use Balanced Accuracy (or Youden's J) instead of traditional metrics when selecting classifiers for LLM evaluation to ensure more robust and trustworthy prevalence estimates for model comparison.
...
### [132] [Revisiting the Scaling Properties of Downstream Metrics in Large Language Model Training](https://arxiv.org/abs/2512.08894)
*Jakub Krajewski,Amitis Shidani,Dan Busbridge,Sam Wiseman,Jason Ramapuram*

Main category: cs.LG

TL;DR: Direct scaling laws for downstream task performance from training budget, showing power law relationships that outperform previous two-stage approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional scaling laws focus on proxy metrics like pretraining loss, but predicting downstream task performance has been considered unreliable. The paper challenges this view by proposing a direct framework to model benchmark performance scaling from training budget.

Method: Proposes a direct framework to model scaling of benchmark performance from training budget. Uses simple power laws to describe scaling behavior of log accuracy on downstream tasks for fixed token-to-parameter ratios. Introduces functional forms that predict accuracy across token-to-parameter ratios and account for inference compute under repeated sampling.

Result: The direct approach extrapolates better than previously proposed two-stage procedure (which is prone to compounding errors). Validated on models up to 17B parameters trained on up to 350B tokens across two dataset mixtures. Complete pretraining losses and downstream evaluation results are released for reproducibility.

Conclusion: Downstream task performance can be reliably predicted from training budget using direct scaling laws, challenging the traditional view that such predictions are unreliable. The proposed framework provides better extrapolation than previous methods and accounts for practical considerations like inference compute.
...
TL;DR: LAPA is a log-domain attention prediction algorithm-architecture co-design for cross-stage sparse acceleration of Transformers, achieving 2.79-3.52x higher energy efficiency than SOTA methods.
...
Conclusion: The proposed LAPA co-design effectively addresses cross-stage sparse acceleration challenges in Transformers, significantly improving energy efficiency through algorithmic and architectural innovations.
...
### [134] [GSPN-2: Efficient Parallel Sequence Modeling](https://arxiv.org/abs/2512.07884)
...
TL;DR: GSPN-2 is a redesigned vision transformer that replaces quadratic self-attention with efficient linear-cost spatial propagation, optimized through joint algorithm-system improvements for GPU efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing vision transformers suffer from quadratic complexity with high-resolution images and long videos. While GSPN improved efficiency, it still had GPU overhead issues from kernel launches, memory transfers, and redundant computations.

Method: Joint algorithm-system redesign: (1) Single 2D kernel replaces thousands of micro-launches, (2) Warp pinning to channel slices with shared memory staging, (3) Compact channel propagation strategy replacing per-channel matrices, aligning with transformer attention affinity maps.

Result: GSPN-2 achieves transformer-level accuracy on image classification and text-to-image synthesis tasks with significantly lower computational cost, establishing new efficiency frontier for global spatial context modeling.

Conclusion: GSPN-2 successfully combines structured matrix transformations with GPU-optimized implementation to enable efficient vision transformers for real-world applications with high-resolution images and long videos.
...
### [135] [CIP-Net: Continual Interpretable Prototype-based Network](https://arxiv.org/abs/2512.07981)
*Federico Di Valerio,Michela Proietti,Alessio Ragno,Roberto Capobianco*

Main category: cs.LG

TL;DR: CIP-Net is a new exemplar-free self-explainable prototype-based model for continual learning that achieves SOTA performance without storing past examples, reducing memory overhead while providing interpretability.


<details>
  <summary>Details</summary>
Motivation: Continual learning faces catastrophic forgetting when learning new tasks. Existing explainable AI approaches either use post-hoc explanations or require additional memory per task, limiting scalability. There's a need for self-explainable models that don't store past examples while maintaining interpretability and performance.

Method: CIP-Net is an exemplar-free self-explainable prototype-based model. It avoids storing past examples, maintains a simple architecture, and generates explanations during prediction to help preserve knowledge across tasks.

Result: CIP-Net achieves state-of-the-art performance compared to previous exemplar-free and self-explainable methods in both task- and class-incremental settings, with significantly lower memory-related overhead.

Conclusion: CIP-Net provides a practical and interpretable solution for continual learning by combining strong performance, memory efficiency, and self-explainability without requiring stored examples from previous tasks.
...
### [136] [CLARITY: Medical World Model for Guiding Treatment Decisions by Modeling Context-Aware Disease Trajectories in Latent Space](https://arxiv.org/abs/2512.08029)
*Tianxingjian Ding,Yuanhao Zou,Chen Chen,Mubarak Shah,Yu Tian*

Main category: cs.LG

TL;DR: CLARITY is a medical world model that forecasts disease evolution in structured latent space, integrating temporal and clinical contexts to generate individualized treatment plans with a prediction-to-decision framework.


<details>
  <summary>Details</summary>
Motivation: Current static AI predictors cannot handle dynamic disease evolution in oncology. Existing medical world models ignore patient-specific contexts, lack feedback mechanisms linking predictions to treatment decisions, and focus on visual reconstruction rather than causal physiological transitions.

Method: CLARITY forecasts disease evolution directly within a structured latent space, explicitly integrating time intervals (temporal context) and patient-specific data (clinical context). It models treatment-conditioned progression as smooth, interpretable trajectories and introduces a novel prediction-to-decision framework that translates latent rollouts into actionable recommendations.
...
Conclusion: CLARITY addresses critical gaps in medical world models by providing physiologically faithful, individualized treatment plans with transparent, actionable recommendations through its integrated temporal/clinical context modeling and prediction-to-decision framework.
...
### [137] [TreeGRPO: Tree-Advantage GRPO for Online RL Post-Training of Diffusion Models](https://arxiv.org/abs/2512.08153)
*Zheng Ding,Weirui Ye*

Main category: cs.LG

TL;DR: TreeGRPO is an efficient RL framework for aligning generative models that recasts denoising as a search tree, achieving 2.4× faster training with better sample efficiency and fine-grained credit assignment.


<details>
  <summary>Details</summary>
Motivation: RL post-training is essential for aligning generative models with human preferences, but its high computational cost limits widespread adoption. Current methods suffer from inefficiency and uniform credit assignment problems.
...
Result: Achieves 2.4× faster training while establishing superior Pareto frontier in efficiency-reward trade-off. Consistently outperforms GRPO baselines across multiple benchmarks and reward models on both diffusion and flow-based models.

Conclusion: TreeGRPO provides a scalable and effective pathway for RL-based visual generative model alignment with dramatically improved training efficiency through tree-structured search and fine-grained credit assignment.
...
### [138] [Beyond Unified Models: A Service-Oriented Approach to Low Latency, Context Aware Phonemization for Real Time TTS](https://arxiv.org/abs/2512.08006)
*Mahta Fetrat,Donya Navabi,Zahra Dehghanian,Morteza Abolghasemi,Hamid R. Rabiee*

Main category: cs.SD

TL;DR: The paper proposes a service-oriented TTS architecture that decouples context-aware phonemization from the core TTS engine, enabling real-time use of high-quality phonemization models while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current lightweight TTS systems use simple phonemizers that struggle with context-dependent challenges, while advanced phonemizers with better linguistic understanding are too computationally expensive for real-time applications, creating a trade-off between quality and speed.

Method: Proposes lightweight strategies for context-aware phonemization and a service-oriented TTS architecture where phonemization modules run as independent services, decoupling heavy context-aware components from the core TTS engine to break latency barriers.

Result: Experimental results show the proposed system improves pronunciation soundness and linguistic accuracy while maintaining real-time responsiveness, making it suitable for offline and end-device TTS applications.

Conclusion: The service-oriented architecture successfully bridges the gap between phonemization quality and inference speed, enabling real-time use of high-quality context-aware phonemization models in TTS systems.
...
### [139] [Self-Reinforced Deep Priors for Reparameterized Full Waveform Inversion](https://arxiv.org/abs/2512.08284)
*Guangyuan Zou,Junlun Li,Feng Liu,Xuejing Zheng,Jianjian Xie,Guoyi Chen*

Main category: physics.geo-ph

TL;DR: SRDIP-FWI improves full waveform inversion by adaptively updating both network parameters and input using feedback, eliminating manual frequency/window selection while achieving better resolution and depth penetration.


<details>
  <summary>Details</summary>
Motivation: Existing DIP-FWI uses fixed random inputs, failing to utilize network input-output correlations and exacerbating ill-posedness in complex geology, leading to artifacts and unstable reconstructions.

Method: Self-reinforced DIP-FWI (SRDIP-FWI) with steering algorithm that alternately updates both network parameters and input at each iteration using feedback from current network output, enabling adaptive structural enhancement and improved regularization.

Result: SRDIP-FWI achieves superior resolution, improved accuracy, greater depth penetration compared to multiscale FWI, eliminates manual frequency-band selection and time-window picking, and simplifies inversion workflow.

Conclusion: SRDIP-FWI provides a novel, adaptive, robust framework for accurate subsurface velocity model reconstruction by addressing limitations of fixed-input DIP-FWI through self-reinforced learning mechanism.
...
### [140] [FlowSteer: Conditioning Flow Field for Consistent Image Restoration](https://arxiv.org/abs/2512.08125)
*Tharindu Wickremasinghe,Chenyang Qi,Harshana Weligampola,Zhengzhong Tu,Stanley H. Chan*

Main category: eess.IV

TL;DR: FlowSteer (FS) is a zero-shot conditioning method that improves flow-based text-to-image models for image restoration tasks by injecting measurement priors during sampling without retraining or adapters.


<details>
  <summary>Details</summary>
Motivation: Flow-based T2I models drift from measurement fidelity in image restoration tasks. Existing solutions require task-specific adapters or retraining, which are computationally heavy and not scalable across different restoration tasks.

Method: FlowSteer introduces an operator-aware conditioning scheme that injects measurement priors along the sampling path, coupling a frozen flow's implicit guidance with explicit measurement constraints. It leverages the nature of flow models and their noise sensitivities to design an effective scheduler.

Result: FS improves measurement consistency and identity preservation across super-resolution, deblurring, denoising, and colorization tasks in a strictly zero-shot setting (no retrained models, no adapters).

Conclusion: FlowSteer achieves higher fidelity of reconstructed images while leveraging the rich generative priors of flow models, providing an efficient and scalable solution for image restoration with flow-based T2I models.
...
### [141] [Tumor-anchored deep feature random forests for out-of-distribution detection in lung cancer segmentation](https://arxiv.org/abs/2512.08216)
*Aneesh Rangnekar,Harini Veeraraghavan*

Main category: eess.IV

TL;DR: RF-Deep is a lightweight, plug-and-play random forests-based OOD detection framework that uses deep features from pretrained transformers to detect out-of-distribution inputs in tumor segmentation from CT scans, achieving high AUROC scores for both near and far OOD datasets.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art tumor segmentation models are vulnerable to out-of-distribution inputs, producing confidently incorrect segmentations that pose clinical risks. Existing OOD detection methods have limitations: logit-based approaches suffer from task-specific biases, while architectural enhancements increase computational costs.

Method: RF-Deep is a post-hoc random forests-based framework that leverages hierarchical deep features from pretrained-then-finetuned backbone encoders. It extracts features from multiple regions of interest anchored to predicted tumor segmentations, making it scalable to varying fields-of-view. The approach uses limited outlier exposure and is plug-and-play.

Result: RF-Deep achieved AUROC > 93.50 for challenging near-OOD datasets (pulmonary embolism, negative COVID-19) and near-perfect detection (AUROC > 99.00) for far-OOD datasets (kidney cancer, healthy pancreas). It substantially outperformed logit-based and radiomics approaches and maintained consistent performance across networks of different depths and pretraining strategies.

Conclusion: RF-Deep is an effective, lightweight, architecture-agnostic approach that enhances the reliability of tumor segmentation from CT volumes by providing robust OOD detection without adding significant computational overhead or requiring architectural modifications.
...
### [142] [Sparse Variable Projection in Robotic Perception: Exploiting Separable Structure for Efficient Nonlinear Optimization](https://arxiv.org/abs/2512.07969)
...
Method: A VarPro scheme designed for problems with gauge symmetries that jointly exploits separability and sparsity. The method constructs a matrix-free Schur complement operator as a one-time preprocessing step, enabling efficient evaluation of costs, gradients, and Hessian-vector products for integration with standard NLS solvers.

Result: Achieves 2× to 35× faster runtimes than state-of-the-art methods across synthetic and real benchmarks in SLAM, SNL, and SfM while maintaining accuracy. Open-source C++ implementation and datasets released.

Conclusion: The proposed VarPro method successfully addresses gauge symmetry challenges in robotic perception, enabling efficient exploitation of separability and sparsity for significant computational speedups in practical applications.
...
### [143] [VLD: Visual Language Goal Distance for Reinforcement Learning Navigation](https://arxiv.org/abs/2512.07976)
*Lazar Milikic,Manthan Patel,Jonas Frey*

Main category: cs.RO

TL;DR: VLD learning decouples perception from policy learning for navigation by training a self-supervised distance predictor on internet-scale video data, then using RL policies in simulation with noisy distance signals to achieve robust multimodal navigation.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end navigation policies suffer from sim-to-real gap and limited labeled training data when trained directly from images to actions.

Method: Two-stage approach: 1) Train self-supervised distance-to-goal predictor on internet-scale video data that works with both image and text goals, 2) Train RL policy in simulation using privileged geometric distances with added noise to mimic predictor uncertainty, then deploy with VLD predictions.

Result: VLD outperforms prior temporal distance methods (ViNT, VIP), achieves competitive navigation performance in simulation, and supports flexible goal modalities (image/text).

Conclusion: Decoupling perception from policy learning provides a scalable path to reliable multimodal navigation by leveraging large-scale visual training for semantic understanding while using simulation for robust low-level behaviors.
...
### [144] [DIJIT: A Robotic Head for an Active Observer](https://arxiv.org/abs/2512.07998)
*Mostafa Kamali Tabrizi,Mingshi Chi,Bir Bikram Dey,Yu Qing Yuan,Markus D. Solbach,Yiqian Liu,Michael Jenkin,John K. Tsotsos*

Main category: cs.RO

TL;DR: DIJIT is a binocular robotic head designed for mobile agents to study human-like eye/head movements and their role in vision, featuring 9 mechanical DOFs and 4 optical DOFs comparable to human performance.


<details>
  <summary>Details</summary>
Motivation: To enable active vision research and study human-like eye and head-neck motions, their interrelationships, and contributions to visual ability, while exploring differences between human vision strategies and current computer vision methods.

Method: Designed a binocular robotic head with 9 mechanical degrees of freedom and 4 optical degrees of freedom, featuring ranges of motion for convergent stereo (vergence, version, cyclotorsion). Developed a new method for saccadic camera movements using direct relationship between camera orientation and motor values.

Result: The robotic head's mechanical design achieves ranges and speeds comparable to human performance. The saccadic camera movement method produces movements close to human movements in terms of accuracy.

Conclusion: DIJIT provides a platform for studying human-like active vision and exploring how eye/head movements contribute to visual ability, with ongoing research into the utility of these movements for both human and machine vision.
...
### [145] [RAVES-Calib: Robust, Accurate and Versatile Extrinsic Self Calibration Using Optimal Geometric Features](https://arxiv.org/abs/2512.08170)
*Haoxin Zhang,Shuaixin Li,Xiaozhou Zhu,Hongbo Chen,Wen Yao*

Main category: cs.RO

TL;DR: A targetless LiDAR-camera calibration toolkit that uses single laser point-image pairs, requires no initial transform, and adaptively weights features for superior accuracy and robustness across various sensors.


<details>
  <summary>Details</summary>
Motivation: Current LiDAR-camera calibration methods often require specific targets, initial transforms, or are sensitive to large extrinsic parameter variations. There's a need for a more user-friendly, robust, and accurate calibration solution that works in targetless environments with various sensor combinations.

Method: Uses Gluestick pipeline for 2D-3D point/line feature correspondences to get automatic initial guess. Quantitatively analyzes feature distribution impact and adaptively weights each feature's cost based on distribution metrics. Optimizes extrinsic parameters by filtering out inferior features' adverse effects.

Result: Validated through extensive experiments across various LiDAR-camera sensors in indoor/outdoor settings. Demonstrates superior robustness and accuracy compared to state-of-the-art techniques. Code is open-sourced on GitHub.

Conclusion: The proposed toolkit provides a user-friendly, targetless calibration solution that eliminates initial transform requirements, handles large extrinsic variations, and achieves better accuracy through adaptive feature weighting, benefiting the robotics and autonomous systems community.
...
### [146] [Embodied Tree of Thoughts: Deliberate Manipulation Planning with Embodied World Model](https://arxiv.org/abs/2512.08188)
*Wenjiang Xu,Cindy Wang,Rui Fang,Mingkang Zhang,Lusong Li,Jing Xu,Jiayuan Gu,Zecui Zeng,Rui Chen*

Main category: cs.RO

TL;DR: EToT is a Real2Sim2Real planning framework that uses physics-based digital twins as embodied world models for robot manipulation planning, combining priori and reflective branching for robust physical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current video-generation world models for robot manipulation lack physical grounding, leading to hallucinations and failure to maintain long-horizon physical constraints, necessitating more physically consistent planning approaches.

Method: EToT uses a physics-based interactive digital twin as an embodied world model and formulates planning as tree search with two mechanisms: Priori Branching (generates diverse candidate paths via semantic/spatial analysis) and Reflective Branching (uses VLMs to diagnose simulator failures and iteratively refine plans).

Result: EToT consistently outperforms baselines on short- and long-horizon manipulation tasks by effectively predicting physical dynamics and adapting to potential failures while adhering to rigid-body dynamics and collision constraints.

Conclusion: The physics-grounded EToT framework provides robust manipulation planning by combining high-level reasoning with physical simulation, addressing limitations of current video-generation world models through embodied digital twins and iterative refinement.
...
### [147] [Zero-Splat TeleAssist: A Zero-Shot Pose Estimation Framework for Semantic Teleoperation](https://arxiv.org/abs/2512.08271)
*Srijan Dokania,Dharini Raghavan*

Main category: cs.RO

TL;DR: Zero-Splat TeleAssist creates a shared 6-DoF world model from CCTV streams for multilateral robot teleoperation without requiring fiducials or depth sensors.


<details>
  <summary>Details</summary>
Motivation: To enable interaction-centric teleoperation by providing real-time global positions and orientations of multiple robots using only commodity CCTV cameras, eliminating the need for specialized sensors or markers.

Method: Integrates vision-language segmentation, monocular depth estimation, weighted-PCA pose extraction, and 3D Gaussian Splatting (3DGS) to transform 2D CCTV streams into a shared 6-DoF world model.

Result: A zero-shot sensor-fusion pipeline that provides every operator with real-time global positions and orientations of multiple robots in a shared world model.

Conclusion: TeleAssist enables multilateral teleoperation using only commodity CCTV cameras, making robot teleoperation more accessible and practical for real-world applications.
...
### [148] [Learning to Control Physically-simulated 3D Characters via Generating and Mimicking 2D Motions](https://arxiv.org/abs/2512.08500)
*Jianan Li,Xiao Chen,Tao Huang,Tien-Tsin Wong*

Main category: cs.GR

TL;DR: Mimic2DM learns 3D character motion controllers directly from 2D video keypoints, avoiding reliance on 3D motion capture data or reconstruction methods.


<details>
  <summary>Details</summary>
Motivation: Video data is more cost-effective than motion capture, but synthesizing realistic 3D behaviors directly from videos remains challenging due to limitations of existing motion reconstruction methods that struggle with generalizability and physical plausibility.

Method: A motion imitation framework that learns control policy directly from 2D keypoint trajectories by minimizing reprojection error. Uses a transformer-based autoregressive 2D motion generator integrated into hierarchical control, where generator produces reference trajectories to guide tracking policy.

Result: The approach learns to synthesize physically plausible and diverse motions across domains including dancing, soccer dribbling, and animal movements without any explicit 3D motion data.

Conclusion: Mimic2DM demonstrates versatility in learning 3D motion tracking capabilities from 2D video data alone, enabling cost-effective motion synthesis for challenging scenarios like human-object interaction and non-human characters.
...
### [149] [Fast and Robust Diffusion Posterior Sampling for MR Image Reconstruction Using the Preconditioned Unadjusted Langevin Algorithm](https://arxiv.org/abs/2512.05791)
*Moritz Blumenthal,Tina Holliber,Jonathan I. Tamir,Martin Uecker*

Main category: physics.med-ph

TL;DR: Preconditioned exact likelihood method accelerates diffusion-based MRI reconstruction, outperforming annealed sampling in speed and quality without parameter tuning.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based MRI reconstruction methods (ULA with diffusion models) provide good quality but suffer from slow reconstruction times and require extensive parameter tuning for posterior sampling methods like diffusion posterior sampling or likelihood annealing.

Method: Proposes using exact likelihood multiplied with diffused prior at all noise scales in reverse diffusion process, combined with preconditioning to accelerate convergence. Trained on fastMRI data and tested on retrospectively undersampled brain data.

Result: The new approach outperforms annealed sampling for both Cartesian and non-Cartesian accelerated MRI in terms of reconstruction speed and sample quality.

Conclusion: The preconditioned exact likelihood method enables rapid and reliable posterior sampling across various MRI reconstruction tasks without requiring parameter tuning.
...
### [150] [Generalizations of the Normalized Radon Cumulative Distribution Transform for Limited Data Recognition](https://arxiv.org/abs/2512.08099)
*Matthias Beckmann,Robert Beinert,Jonas Bresch*

Main category: math.NA

TL;DR: The paper proposes generalized normalizations for the Radon-CDT to enhance affine invariance and extends it to multi-dimensional/non-Euclidean settings using generalized Radon transforms, achieving near-perfect classification.


<details>
  <summary>Details</summary>
Motivation: The R-CDT is effective for image classification but lacks invariance to affine transformations that occur in real-world measurements (e.g., watermark recognition). Existing normalization methods are limited, and the approach needs extension to multi-dimensional and non-Euclidean data.

Method: 1) Proposes a family of generalized normalizations for R-CDT to improve affine invariance. 2) Extends R-CDT to multi-dimensional and non-Euclidean settings using generalized Radon transforms. 3) Proves theoretical invariance properties and linear separability in feature space.

Result: Theoretical proofs show the feature representations are invariant under certain transformations and allow linear separation. Numerical experiments on 2D images, 3D shapes, and 3D rotation matrices demonstrate near-perfect classification accuracies and clustering results.

Conclusion: The generalized normalization framework enhances R-CDT's flexibility and applicability, while the extension to multi-dimensional/non-Euclidean settings broadens its utility. The approach maintains theoretical guarantees while achieving excellent practical performance across diverse data types.
...
### [151] [Conditional Morphogenesis: Emergent Generation of Structural Digits via Neural Cellular Automata](https://arxiv.org/abs/2512.08360)
*Ali Sakour*

Main category: cs.NE

TL;DR: Conditional Neural Cellular Automata (c-NCA) that can generate distinct MNIST digit structures from a single generic seed using spatially broadcasted class vectors, enforcing strict locality and translation equivariance unlike traditional generative models.


<details>
  <summary>Details</summary>
Motivation: Existing NCA research has focused on continuous texture synthesis or single-target object recovery, leaving class-conditional structural generation largely unexplored. The paper aims to bridge the gap between texture-based NCAs and structural pattern formation.

Method: Proposes a Conditional Neural Cellular Automata (c-NCA) architecture that grows distinct topological structures from a single generic seed using spatially broadcasted class vectors. The model enforces strict locality and translation equivariance by injecting a one-hot condition into the cellular perception field, allowing a single set of local rules to learn to break symmetry and self-assemble into distinct geometric attractors.

Result: c-NCA achieves stable convergence, correctly forming digit topologies from a single pixel, and exhibits robustness characteristic of biological systems. The model demonstrates that a single set of local rules can learn to generate ten distinct MNIST digit structures.

Conclusion: This work bridges the gap between texture-based NCAs and structural pattern formation, offering a lightweight, biologically plausible alternative for conditional generation that mimics biological morphogenetic plasticity.
...
### [152] [Multi-domain performance analysis with scores tailored to user preferences](https://arxiv.org/abs/2512.08715)
*Sébastien Piérard,Adrien Deliège,Marc Van Droogenbroeck*

Main category: cs.PF

TL;DR: The paper shows that when averaging performance across domains, only certain scores (like ranking scores) allow the summarized performance to equal a weighted arithmetic mean of domain-specific performances, with weights depending on user preferences.


<details>
  <summary>Details</summary>
Motivation: Performance evaluation across different domains requires understanding how averaging affects results, especially since algorithm performance depends heavily on domain-specific distributions. The paper aims to scrutinize what happens during averaging of performance measures across domains.

Method: Adopts a probabilistic framework where performance is treated as a probability measure (e.g., normalized confusion matrix). Shows that weighted mean performance corresponds to summarization, and identifies specific scores (ranking scores parameterized by user preferences) where summarized performance equals weighted arithmetic mean of domain-specific performances.

Result: Establishes that only remarkable scores (including ranking scores parameterized by user preferences) allow summarized performance to equal weighted arithmetic mean of domain-specific performances. Defines four domain types (easiest, most difficult, preponderant, bottleneck) as functions of user preferences. Develops new visual tools for two-class classification.

Conclusion: The paper provides a rigorous theoretical framework for understanding performance averaging across domains, identifies specific scores that preserve arithmetic mean properties, defines domain characteristics based on user preferences, and offers practical visualization tools for classification tasks.
...
### [153] [MixLM: High-Throughput and Effective LLM Ranking via Text-Embedding Mix-Interaction](https://arxiv.org/abs/2512.07846)
...
TL;DR: MixLM is an LLM-based ranking framework that improves throughput 10x by using mixed text-embedding tokens instead of pure text, reducing context length while maintaining relevance.


<details>
  <summary>Details</summary>
Motivation: LLMs show impressive ranking performance but suffer from high computational overhead under industrial latency/throughput requirements, especially in cross-encoder systems with long context prefill-heavy workloads.

Method: Proposes mix-interaction: mixture of text and embedding tokens to represent input. Encodes all catalog items into few embedding tokens stored in nearline cache, reducing item length from thousands of text tokens to few embedding tokens during online inference.

Result: Increased throughput by 10.0x under same latency budget while maintaining relevance metrics. Enabled full-traffic deployment of LLM-powered search, resulting in 0.47% increase in Daily Active Users (DAU) in online A/B tests at LinkedIn.

Conclusion: MixLM successfully addresses LLM computational overhead in ranking systems through efficient mixed token representation, enabling practical industrial deployment with significant performance and business impact.
...
### [154] [Ontology-Based Knowledge Graph Framework for Industrial Standard Documents via Hierarchical and Propositional Structuring](https://arxiv.org/abs/2512.08398)
*Jiin Park,Hyuna Jeon,Yoonseo Lee,Jisu Hong,Misuk Kim*

Main category: cs.IR

TL;DR: Proposes a hierarchical semantic structure method for constructing knowledge graphs from complex industrial standards, using LLM-based triple extraction and ontology-aware KG-RAG framework.


<details>
  <summary>Details</summary>
Motivation: Industrial standards contain complex technical information with structured formats (tables, constraints, exceptions, calculations) that make knowledge graph construction challenging. Conventional methods fail to capture domain-specific semantics and logical structures effectively.

Method: Organizes documents into hierarchical semantic structure, decomposes sentences and tables into atomic propositions from conditional/numerical rules, integrates them into ontology-knowledge graph through LLM-based triple extraction, and implements ontology-aware KG-RAG framework.

Result: Achieves significant performance improvements across all QA types (rule, table, multi-hop QA, toxic clause detection) compared to existing KG-RAG approaches. Demonstrates reliable and scalable knowledge representation for complex industrial documents.

Conclusion: The method enables effective knowledge representation for industrial documents with intertwined conditions, constraints, and scopes, contributing to future domain-specific RAG development and intelligent document management.
...
### [155] [Accelerating Urban Science Research with AI Urban Scientist](https://arxiv.org/abs/2512.07849)
*Tong Xia,Jiankun Zhang,Ruiwen You,Ao Xu,Linghao Zhang,Tengyao Tu,Jingzhi Wang,Jinghua Piao,Yunke Zhang,Fengli Xu,Yong Li*

Main category: cs.CY

TL;DR: AI Urban Scientist: A knowledge-driven multi-agent system that autonomously conducts urban science research by generating hypotheses, analyzing data, and synthesizing insights to understand city dynamics.


<details>
  <summary>Details</summary>
Motivation: Cities are complex adaptive systems that remain difficult to understand despite data abundance. Urban science struggles to convert fragmented interdisciplinary information into coherent explanations of how cities function and evolve. Current AI systems lack the domain knowledge and methodological depth needed for urban science research.

Method: Built a knowledge-driven AI Urban Scientist using hypotheses, peer-review signals, datasets, and analytical patterns distilled from thousands of high-quality studies. Implemented as a coordinated multi-agent framework for end-to-end inquiry that generates structured hypotheses, retrieves and harmonizes heterogeneous datasets, conducts automated empirical analysis and simulation, and synthesizes insights.

Result: The system provides reusable analytical tools and supports community-driven extensions, lowering barriers to advanced urban analytics. It acts as an active collaborator rather than just an assistant in urban science research.

Conclusion: The AI Urban Scientist represents a new pathway for accelerating urban science transformation, capable of revealing mechanisms that shape urban systems and guiding the design of more resilient and equitable cities.
...
<details>
  <summary>Details</summary>
Motivation: Previous research showed LLMs performed poorly on CFA exams, but recent reasoning models have excelled on other professional exams, prompting evaluation of whether they can now pass the rigorous CFA certification.

Method: Evaluated top reasoning models on 980 mock CFA questions across three Level I, two Level II, and three Level III exams using same pass/fail criteria from prior studies.

Result: Most models cleared all three levels. Gemini 3.0 Pro led overall with 97.6% on Level I, GPT-5 scored 94.3% on Level II, and Gemini models excelled on Level III (86.4% multiple-choice, 92.0% constructed-response).

Conclusion: Modern reasoning LLMs demonstrate strong performance on CFA exams, surpassing previous LLM capabilities and achieving passing scores across all three certification levels.
...
### [157] [The High Cost of Incivility: Quantifying Interaction Inefficiency via Multi-Agent Monte Carlo Simulations](https://arxiv.org/abs/2512.08345)
*Benedikt Mangold*

Main category: cs.AI

TL;DR: LLM-based multi-agent systems simulate workplace toxicity through adversarial debates, showing toxic interactions increase conversation duration by 25%, providing an ethical alternative to human-subject research.


<details>
  <summary>Details</summary>
Motivation: Workplace toxicity is known to harm organizational culture, but quantifying its direct impact on operational efficiency is challenging due to ethical and practical constraints of studying conflict in human subjects.

Method: Uses LLM-based Multi-Agent Systems to simulate 1-on-1 adversarial debates as a "sociological sandbox." Employs Monte Carlo method to simulate hundreds of discussions, measuring convergence time (arguments needed to reach conclusion) between baseline control groups and treatment groups with "toxic" system prompts.

Result: Statistically significant increase of approximately 25% in conversation duration when toxic participants are involved. This "latency of toxicity" serves as a proxy for financial damage in corporate/academic settings.

Conclusion: Agent-based modeling provides a reproducible, ethical alternative to human-subject research for measuring social friction mechanics, demonstrating that toxic interactions significantly slow decision-making processes.
...
TL;DR: See-Control is a framework that enables smartphone operation via a low-DoF robotic arm, offering a platform-agnostic alternative to ADB-dependent methods.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based smartphone operation methods depend on Android Debug Bridge (ADB), limiting them to Android devices and requiring system access. There's a need for a platform-agnostic solution that enables physical interaction with smartphones.

Method: See-Control introduces the Embodied Smartphone Operation (ESO) task with three components: (1) ESO benchmark with 155 tasks and evaluation metrics, (2) MLLM-based embodied agent that generates robotic control commands without ADB or system access, and (3) annotated dataset of operation episodes.
...
Conclusion: See-Control represents a concrete step toward enabling home robots to perform smartphone-dependent tasks in realistic environments by providing a physical interaction approach that doesn't rely on system-level access.
