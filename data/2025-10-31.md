...
TL;DR: This paper introduces StreetMath, a benchmark to evaluate LLMs' approximation abilities in informal mathematical reasoning, revealing that models tend to compute exact values rather than approximate and use separate neural components for exact vs approximate operations.


<details>
  <summary>Details</summary>
Motivation: To address the gap in understanding LLMs' ability to perform approximate reasoning in informal, fast-paced mathematical operations, particularly for non-autoregressive decoder models, which has received less attention compared to precise arithmetic operations.
...
Result: LLMs generally attempt to compute exact values or invoke external tools even in approximation tasks, consume more tokens when solving approximation tasks, and exact/approximate arithmetic operations rely on largely separate neural components. Models don't exhibit cognitive miserliness like humans in street math settings.

Conclusion: LLMs do not exhibit the same cognitive miserliness as humans in street math contexts, preferring exact computation over approximation strategies, with distinct neural pathways for exact vs approximate mathematical reasoning.
...
TL;DR: Proposes a fuzzy logic-based approach for granular sentiment analysis that ranks entities based on opinion strength levels (very weak to very strong) by combining opinion words related to specific product aspects.


<details>
  <summary>Details</summary>
Motivation: Traditional lexicon-based sentiment analysis doesn't capture opinion strength levels, treating all opinions as equally positive or negative without considering intensity variations.

Method: Uses fuzzy logic to classify opinion words into granular categories and syntactic dependency resolution to identify relations between opinion words and specific aspect words of interest.

Result: The approach enables ranking entities based on both orientation and strength of reviews, providing more nuanced sentiment analysis for specific product aspects.

Conclusion: The proposed method offers improved sentiment analysis by considering opinion strength granularity and aspect-specific opinion relationships, enhancing entity ranking accuracy.
...
Method: Collected 563,299 labeled Korean sentences from Korean political parties' press releases and trained state-of-the-art deep learning models for stance detection.

Result: Created the LASTIST dataset supporting various stance detection tasks including target-independent stance detection and diachronic evolution analysis.

Conclusion: The LASTIST dataset fills an important research gap by providing Korean language resources for stance detection and enables new research directions in target-independent and diachronic stance analysis.
...
TL;DR: Proposes zFLoRA, a zero-latency fused low-rank adapter that eliminates inference overhead while maintaining performance comparable to LoRA and full fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing task-specific adapters in LLMs, though small in parameter count (less than 1% of base model), cause significant inference latency (up to 2.5x base model), creating deployment bottlenecks.

Method: Develops a fused low-rank adapter that integrates adapter parameters into the base model without additional computational overhead during inference.

Result: zFLoRA achieves comparable performance to LoRA and full fine-tuning across 18 tasks in commonsense reasoning, math reasoning, and summary-dialogue, while introducing zero to negligible latency on both NPU (Samsung Galaxy S25+) and GPU (NVIDIA H100) platforms.

Conclusion: zFLoRA enables efficient deployment of task-specific LLMs without compromising performance or inference speed, solving the latency problem of traditional adapter methods.
...
TL;DR: Proposes three improvements to circuit discovery in mechanistic interpretability: bootstrapping for consistent attribution scores, ratio-based edge selection, and integer linear programming instead of greedy selection.


<details>
  <summary>Details</summary>
Motivation: To address challenges in circuit discovery by improving the identification of which parts of a model perform specific tasks, building on the Mechanistic Interpretability Benchmark (MIB).

Method: Three key improvements: 1) Bootstrapping to identify edges with consistent attribution scores, 2) Ratio-based selection strategy for prioritizing strong positive-scoring edges, 3) Integer linear programming formulation replacing greedy selection.

Result: The methods yield more faithful circuits and outperform prior approaches across multiple MIB tasks and models.
...
TL;DR: LISTEN is a framework that uses LLMs as zero-shot preference oracles to help experts select optimal options from large sets with multiple objectives, guided by natural language priorities.


<details>
  <summary>Details</summary>
Motivation: Human experts struggle to formalize complex implicit preferences when selecting from large option sets with competing objectives, creating a decision-making bottleneck.

Method: Two iterative algorithms: LISTEN-U (refines parametric utility function using LLM) and LISTEN-T (non-parametric tournament-style selection over small batches).
...
Conclusion: The framework demonstrates promising direction for steering complex multi-objective decisions using natural language, reducing cognitive burden of traditional preference elicitation methods.
...
TL;DR: LongFilter is a framework for curating training data for long-context pretraining by identifying samples where long-range dependencies are essential through contrasting model predictions under long vs short contexts.
...
TL;DR: Persona adoption in LLMs subtly influences harmful content classification, creating ideological biases where models align more with personas from the same political ideology and show divergent judgments across ideological groups.


<details>
  <summary>Details</summary>
Motivation: To examine how persona adoption affects fairness and consistency in harmful content classification across different LLM architectures, model sizes, and content modalities, given LLMs' increasing use in content moderation systems.

Method: Analyzed persona influence on classification consistency and fairness across various LLM architectures and sizes, using agreement analyses and additional studies on politically targeted tasks to examine ideological alignment and behavioral shifts.

Result: While overall classification accuracy appears unaffected, personas with different ideological leanings show distinct propensities to label content as harmful. Larger models align more closely with personas from the same political ideology, strengthening within-ideology consistency while widening divergence across ideological groups.

Conclusion: Persona conditioning introduces subtle ideological biases into LLM outputs, raising concerns about AI systems potentially reinforcing partisan perspectives under the guise of neutrality in content moderation.
...
TL;DR: CLEAR method uses entity-aware retrieval for clinical question answering, achieving better performance with fewer tokens than traditional methods.


<details>
  <summary>Details</summary>
Motivation: EHR clinical documentation is stored as base64 encoded attachments, making semantic question answering difficult. Traditional vector database methods miss nuanced clinical relationships.

Method: Clinical Entity Augmented Retrieval (CLEAR) uses entity aware retrieval. Developed a Clinical Notes QA Evaluation Platform to validate CLEAR against zero shot large context inference and traditional chunk based retrieval augmented generation.

Result: CLEAR achieved F1 score of 0.90 vs 0.86 for embedding based retrieval, used 70% fewer tokens. Tested on 12 clinical notes (10k-65k tokens), achieved 58.3% win rate, 0.878 semantic similarity, 78% fewer tokens than wide context processing. Largest gains on long notes (75% win rate for >65k tokens).
...
TL;DR: This paper presents the first systematic survey of data-efficient LLM post-training from a data-centric perspective, proposing a taxonomy of methods and outlining future research directions.
...
Method: The authors conduct a systematic survey and propose a taxonomy covering data selection, data quality enhancement, synthetic data generation, data distillation and compression, and self-evolving data ecosystems.

Result: The paper summarizes representative approaches in each category of the taxonomy and provides a comprehensive overview of data-efficient LLM post-training methods.

Conclusion: The work highlights open problems and proposes potential research avenues to inspire further exploration into maximizing data utilization in large-scale model training.
...
TL;DR: Evaluation of LLM-based semantic role labeler for FrameNet-like annotation shows semi-automatic approach increases frame diversity with similar coverage compared to manual annotation, while full automation performs poorly except for speed.


<details>
  <summary>Details</summary>
Motivation: To evaluate the performance and impact of LLM-based tools for creating annotated datasets, particularly for semantic annotation, under a perspectivized NLP approach.

Method: Compared annotation time, coverage and diversity across three settings: manual, automatic (LLM-only), and semi-automatic (human-LLM hybrid) annotation for FrameNet-like semantic role labeling.

Result: Semi-automatic annotation achieved increased frame diversity and similar annotation coverage compared to manual annotation. Automatic annotation performed worse in all metrics except annotation time.

Conclusion: Hybrid semi-automatic annotation using LLMs is effective for semantic annotation, balancing efficiency with quality, while full automation is insufficient for quality annotation tasks.
...
<details>
  <summary>Details</summary>
Motivation: To verify what training data LLMs have seen when direct inspection is impossible, by eliciting and verifying memorized content through model outputs.

Method: Uses a feedback-driven loop with secondary model evaluation, discrepancy identification, correction hints, and jailbreaking module to overcome alignment refusals.

Result: On EchoTrace benchmark (30+ books), RECAP improved average ROUGE-L score from 0.38 to 0.47 with GPT-4.1 - a 24% increase in copyrighted text extraction.

Conclusion: RECAP effectively extracts memorized training data from LLMs through iterative feedback and jailbreaking, providing compelling evidence of training data exposure.
...
TL;DR: Multilingual data mixtures in LLM pretraining don't necessarily cause performance trade-offs. With sufficient token coverage, combining English and multilingual data maintains performance, English works well as a universal pivot language, and no significant curse of multilinguality was observed at 1.1B-3B scale.


<details>
  <summary>Details</summary>
Motivation: To investigate the impact of different multilingual data mixtures in pretraining LLMs and address concerns about potential trade-offs between language coverage and model performance (the curse of multilinguality).

Method: Trained 1.1B and 3B parameter LLMs on diverse multilingual corpora, varying the number of languages from 25 to 400, and analyzed the effects of language combinations and pivot language selection.

Result: Combining English and multilingual data doesn't degrade performance when languages have sufficient tokens; English works effectively as a universal pivot language across language families; no significant curse of multilinguality observed at this scale.
...
TL;DR: Machine Translation causes label drift due to cultural differences, especially in culturally sensitive domains, with LLMs amplifying this effect through encoded cultural knowledge.


<details>
  <summary>Details</summary>
Motivation: To investigate how cultural alignment between source and target languages affects semantic label preservation in Machine Translation, an underexplored factor despite MT's widespread use for generating synthetic data in low-resource languages.

Method: Conducted experiments across culturally sensitive and neutral domains using various MT systems including modern LLMs, analyzing label drift during translation and the role of cultural knowledge encoding.

Result: Found that MT systems induce label drift, particularly in culturally sensitive domains; LLMs encode cultural knowledge which amplifies drift; and cultural similarity between languages is crucial for label preservation.

Conclusion: Neglecting cultural factors in MT undermines label fidelity and risks misinterpretation and cultural conflict in downstream applications, highlighting the importance of considering cultural alignment.
...
TL;DR: SymCode is a neurosymbolic framework that improves mathematical reasoning in LLMs by generating verifiable code using SymPy instead of prose, achieving up to 13.6% accuracy improvements on benchmarks.
...
TL;DR: High-performance matrix multiplication optimization for LLM inference on AWS Trainium AI accelerator using kernel fusion and novel caching strategies to overcome architectural challenges.


<details>
  <summary>Details</summary>
Motivation: AWS Trainium provides cost-effective AI acceleration but its systolic array architecture and data layout requirements make achieving high performance challenging, especially for critical matrix multiplication kernels in LLM inference.

Method: Designed customized techniques for Trainium including kernel fusion and novel caching strategies to reduce data movement across software-managed memory hierarchy, maximize SRAM bandwidth, and avoid expensive matrix transpose operations.

Result: Achieved average 1.35x speedup (up to 2.22x) at matmul kernel level and average 1.66x speedup (up to 2.49x) for end-to-end LLM inference across nine datasets and four recent LLMs, outperforming AWS's state-of-the-art implementation.

Conclusion: The proposed optimization techniques successfully overcome Trainium's architectural challenges and significantly improve LLM inference performance on this AI accelerator platform.
...
TL;DR: AttnCache accelerates LLM prefill inference by reusing similar attention maps from a cache, reducing self-attention computation overhead.


<details>
  <summary>Details</summary>
Motivation: Many real-world workloads rely only on prefill inference where self-attention's quadratic complexity becomes the bottleneck, and semantically different sentences often produce similar attention maps.
...
Conclusion: AttnCache effectively accelerates prefill-only LLM inference by exploiting attention map similarity across different inputs, providing significant speed improvements with minimal accuracy impact.
...
TL;DR: SRL is a new training framework that combines supervised learning with reinforcement learning to improve multi-step reasoning in small LLMs by generating logical action sequences with internal reasoning.


<details>
  <summary>Details</summary>
Motivation: Small LLMs struggle with multi-step reasoning tasks - RLVR fails when correct solutions are rarely sampled, while SFT overfits through rigid token imitation.

Method: SRL reformulates problem solving as generating action sequences with internal reasoning monologues, providing step-wise rewards based on similarity to expert actions from SFT data.

Result: SRL enables small models to learn challenging problems unlearnable by SFT or RLVR alone, and SRL+RLVR initialization yields strongest performance. Also generalizes to agentic software engineering tasks.

Conclusion: SRL is a robust and versatile training framework that provides richer learning signals and flexible reasoning for reasoning-oriented LLMs.
...
TL;DR: PORTool is a reinforcement learning method that trains tool-use LLMs to explore multiple solution trajectories in dynamic tool-call environments, improving performance over static dataset training.


<details>
  <summary>Details</summary>
Motivation: Current tool-use LLMs trained on static datasets fail to explore possible solutions and perform poorly in dynamic tool-call environments, as they only imitate generic tool-call routines.

Method: Uses RL to generate multiple rollouts forming tree-like structures, assigns step-wise rewards based on correct answers and successful tool calls, and calculates fork-relative advantages blended with trajectory-relative advantages for training.

Result: Significant improvements in final accuracy and reduction in tool-call steps compared to other training approaches, validated through experiments with 17 tools covering time-sensitive and time-invariant topics.

Conclusion: PORTool effectively enables LLMs to explore diverse solution trajectories in dynamic tool environments, overcoming limitations of static dataset training and improving overall tool-use performance.
...
TL;DR: Cross-lingual alignment improves knowledge transfer but causes cultural erasure. The paper introduces a framework to quantify this trade-off and proposes Surgical Steering method to balance both objectives.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between cross-lingual knowledge transfer and cultural erasure in multilingual LLMs, where current alignment techniques improve factual transfer at the cost of culturally-situated responses.

Method: Introduces a transfer-localization plane evaluation framework and proposes Surgical Steering - an inference-time method that applies targeted activation steering to different model layers to disentangle factual transfer from cultural knowledge.

Result: Found that current CLA approaches consistently improve factual transfer but reduce cultural localization across six languages. Surgical Steering achieves better balance between knowledge transfer and cultural preservation.

Conclusion: Universal factual transfer and culturally-specific knowledge are optimally steerable at different layers, enabling targeted approaches like Surgical Steering to overcome limitations of current cross-lingual alignment techniques.
...
TL;DR: This study developed an NLP pipeline to analyze incidental thyroid findings in radiology reports, finding that 7.8% of patients had ITFs which were strongly associated with downstream thyroid cancer diagnosis, highlighting overdiagnosis concerns.


<details>
  <summary>Details</summary>
Motivation: Incidental thyroid findings are increasingly detected in imaging but their prevalence, features, and clinical consequences remain undefined, creating uncertainty in clinical management.

Method: Retrospective cohort study using a transformer-based NLP pipeline to identify ITFs and extract nodule characteristics from radiology reports across multiple imaging modalities from 115,683 patients at Mayo Clinic.

Result: 7.8% of patients had ITFs (92.9% were nodules), with poor documentation of nodule characteristics. ITFs were associated with higher odds of thyroid nodule diagnosis, biopsy, thyroidectomy and thyroid cancer diagnosis, with most cancers being papillary type.

Conclusion: ITFs are common and strongly associated with cascades leading to detection of small, low-risk cancers, underscoring their role in thyroid cancer overdiagnosis and the need for standardized reporting and more selective follow-up.
...
TL;DR: QCoder Benchmark evaluates LLMs on quantum programming using simulated hardware feedback and human-written code comparisons, showing reasoning-based models outperform standard LLMs and human coders.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used for code generation but remain underexplored in hardware-interactive domains like quantum programming, where Python code controls quantum computers.

Method: Introduces QCoder Benchmark with quantum simulator environment for domain-specific metrics (circuit depth, execution time, error classification) and uses human-written code from programming contests for comparison.

Result: GPT-4o achieves only 18.97% accuracy while reasoning-based models like o3 reach 78% accuracy, outperforming human-written code average success rate of 39.98%.

Conclusion: QCoder Benchmark reveals significant challenges in quantum programming for LLMs and demonstrates reasoning-based approaches' superiority, with dataset and API released for further research.
...
TL;DR: Proposes 1PNS training paradigm with Reasoning Path Divergence (RPD) metric to increase output diversity in LLMs, improving pass@k performance over traditional 1P1S training.


<details>
  <summary>Details</summary>
Motivation: Address low diversity in model outputs caused by 'one problem, one solution' training, which limits reasoning path variety and becomes a bottleneck for Test-Time Scaling effectiveness.

Method: Introduces Reasoning Path Divergence (RPD) metric to measure semantic differences between multi-step chains of thought, then curates diverse solution sets using RPD for fine-tuning Qwen3-4B-Base model.

Result: RPD-selected training yields more varied outputs with +2.80% average gain in pass@16 over 1P1S baseline and +4.99% gain on AIME24, demonstrating enhanced TTS effectiveness.

Conclusion: 1PNS training paradigm with RPD metric successfully increases inference diversity and amplifies Test-Time Scaling performance by exposing models to varied reasoning trajectories.
...
TL;DR: This paper uses LLMs to analyze the relationship between persuasion techniques and discourse relations, creating silver datasets that reveal six key discourse relations are crucial in persuasive texts.
...
Method: Used SemEval 2023 Task 3 dataset with 19 PTs, developed LLM-based classifiers with 4 LLMs and 10 prompts to label with 22 PDTB 3.0 level-2 DRs, created ensemble models with majority-pooling strategies to generate 5 silver datasets.
...
Conclusion: The identified discourse relations can contribute to detecting online propaganda and misinformation, enhancing our understanding of effective communication patterns.
...
TL;DR: MossNet is a novel mixture-of-state-space-experts architecture that emulates multi-head attention through MoE implementation in both MLP blocks and SSM kernels, achieving superior performance over transformers and SSMs.
...
Method: Proposes MossNet with mixture-of-experts implementation in channel-mixing MLP blocks and time-mixing SSM kernels to realize multiple attention heads, creating a linear multi-head attention equivalent.

Result: Outperforms transformer- and SSM-based architectures of similar size and data budgets, scales well to trillions of tokens, and shows favorable runtime speed and resource usage on mobile devices and GPUs.

Conclusion: MossNet represents a compelling new direction for efficient, high-performing recurrent LLM architectures that combine the benefits of multi-head attention with SSM efficiency.
...
TL;DR: SDM language models are fine-tuned sequence prediction models that use a final-layer activation layer for binary classification of instruction-following, improving statistical efficiency by reducing abstentions.


<details>
  <summary>Details</summary>
Motivation: To create language models that can better distinguish between well-calibrated, high-probability generations and improve instruction-following capabilities while reducing abstentions compared to existing supervised baselines.
...
Conclusion: Existing pre-trained language models can be effectively converted into SDM LMs through the proposed fine-tuning approach, resulting in improved instruction-following performance with better statistical efficiency.
...
TL;DR: RCScore is a framework that quantifies how instruction formulation affects LLM responses, revealing performance variations undetected by conventional metrics. It shows instruction style can shift accuracy by up to 16.7% and introduces Cross-Response Similarity to measure stylistic consistency.


<details>
  <summary>Details</summary>
Motivation: Current LLM evaluations often use single instruction templates, overlooking models' sensitivity to instruction style, which is critical for real-world deployments where instructions vary.
...
Result: Experiments across ten LLMs on four reasoning benchmarks show instruction style can shift accuracy by up to 16.7%. CRS strongly correlates with task accuracy, deterministic decoding produces more stable outputs, and model scale correlates with cross-style consistency.

Conclusion: RCScore provides a principled approach to assess instruction robustness, suggesting consistency as a valuable proxy for model reliability in real-world applications.
...
TL;DR: The paper identifies 'update forgetting' as a key failure mode in diffusion language models where uniform updates erase earlier edits, and proposes Token Timestep Allocation (TTA) to address this through semantic token ordering.


<details>
  <summary>Details</summary>
Motivation: Diffusion language models have practical controllability issues due to update forgetting, where uniform updates disrupt cumulative refinement and degrade fluency and coherence.

Method: Proposes Token Timestep Allocation (TTA) that implements soft semantic token ordering via per-token timestep schedules, freezing critical tokens early while continuing to refine uncertain tokens.

Result: TTA improves controllability and fluency: 20% higher accuracy and nearly halved perplexity in sentiment control using <1/5 steps; lower toxicity (12.2 vs 14.5) and perplexity (26.0 vs 32.0) in detoxification.
...
TL;DR: WIMHF is a method that uses sparse autoencoders to identify human-interpretable features in feedback data, revealing what preferences datasets can measure and what annotators actually express, enabling better data curation and personalization.


<details>
  <summary>Details</summary>
Motivation: Human feedback can unpredictably alter language models, and practitioners lack understanding of what feedback data encodes. Current methods struggle to automatically extract relevant features without pre-specifying hypotheses.

Method: Introduces WIMHF (What's In My Human Feedback?) using sparse autoencoders to characterize preferences in feedback data across 7 datasets, identifying interpretable features that explain preference prediction signals.

Result: Identified small number of human-interpretable features accounting for majority of preference prediction signal. Revealed diverse human preferences and context-dependent variations. Found potentially unsafe preferences like users voting against refusals in favor of toxic content. Enabled effective data curation (+37% safety gains) and fine-grained personalization.

Conclusion: WIMHF provides a human-centered analysis method for practitioners to better understand and use preference data, enabling improved data curation, safety improvements, and personalized preference modeling.
...
<details>
  <summary>Details</summary>
Motivation: Current RAG evaluation benchmarks focus on local retrieval (finding relevant chunks from small document subsets), but real-world applications often require global RAG - aggregating information across entire document collections to derive corpus-level insights.

Method: Proposed GlobalRAG: a multi-tool collaborative framework with chunk-level retrieval to preserve structural coherence, LLM-driven intelligent filters to eliminate noisy documents, and aggregation modules for precise symbolic computation.

Result: Existing RAG methods perform poorly on global tasks (strongest baseline: 1.51 F1). GlobalRAG achieves 6.63 F1 on Qwen2.5-14B model, showing significant improvement over baselines.

Conclusion: GlobalRAG framework effectively addresses the challenges of global RAG tasks, demonstrating substantial performance gains and validating the importance of specialized approaches for corpus-level information aggregation.
...
TL;DR: Using pragmatic theories (Gricean pragmatics, Relevance Theory) as prompts improves language models' ability to understand implied meanings through step-by-step reasoning, achieving up to 9.6% higher scores on pragmatic reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Language models need to accurately interpret implied meanings for effective human communication, but current approaches may not fully leverage pragmatic theories.

Method: Presenting pragmatic theories as prompts to guide language models through step-by-step reasoning processes, comparing with baseline 0-shot Chain-of-Thought prompting.

Result: Methods achieved up to 9.6% higher scores on pragmatic reasoning tasks compared to baseline. Even just mentioning theory names improved performance by 1-3% in larger models.

Conclusion: Providing pragmatic theories as prompts is an effective in-context learning approach for enhancing language models' understanding of implied meanings.
...
TL;DR: Pretrained language models perform poorly at identifying loanwords despite contextual information, showing bias toward loanwords over native vocabulary.


<details>
  <summary>Details</summary>
Motivation: To investigate whether pretrained language models can distinguish loanwords from native vocabulary, especially in bilingual communities where dominant languages influence minority languages.
...
Result: Models performed poorly in distinguishing loanwords from native ones, corroborating previous evidence of NLP systems exhibiting bias toward loanwords rather than native equivalents.
...
TL;DR: This paper presents an empirical study of knowledge distillation (KD) behavior in multilingual vision-language models, examining how different distillation approaches affect cross-lingual representation consistency and performance stability during model compression.


<details>
  <summary>Details</summary>
Motivation: Vision-language models show uneven performance across languages, especially when model size is reduced. While knowledge distillation shows promise for transferring knowledge from larger to smaller models, its application in multilingual contexts remains underexplored.

Method: The study evaluates five distillation approaches across CLIP and SigLIP2 models, analyzing their effects on cross-lingual representation consistency and downstream performance. Evaluation includes in-domain retrieval and out-of-domain visual QA tasks.

Result: Some distillation configurations preserve or even improve multilingual retrieval robustness despite halving model size, while others fail to maintain cross-task stability. The study reveals design-sensitive trade-offs that aggregate accuracy alone doesn't capture.

Conclusion: Knowledge distillation approaches in multilingual VLMs show varying effectiveness, with some maintaining performance across languages despite compression, but careful design choices are needed to ensure cross-task stability.
...
TL;DR: Neuron Agreement Decoding (NAD) is an unsupervised ensemble method that uses internal neuron activation patterns (sparsity and cross-sample agreement) instead of external signals to select the best reasoning candidates, enabling early correctness prediction and 99% token reduction.


<details>
  <summary>Details</summary>
Motivation: Existing ensemble decoding methods rely on poorly calibrated external signals like token probabilities or self-evaluations. Internal neuron activations provide richer, more reliable signals for identifying correct reasoning paths.

Method: NAD analyzes internal neuron activations to measure: (1) activation sparsity (correct responses activate fewer unique neurons) and (2) cross-sample neuron agreement (correct responses show stronger activation consistency). It selects candidates based on these internal signals without requiring comparable textual outputs.
...
Conclusion: Internal neuron activation signals provide reliable, scalable, and efficient guidance for label-free ensemble decoding, enabling aggressive early stopping and substantial computational savings while maintaining reasoning quality.
...
TL;DR: LLMs learn accurate and interchangeable number representations despite producing numeric errors, enabling universal probes to trace error causes to specific layers.


<details>
  <summary>Details</summary>
Motivation: To resolve the conflict between LLMs' accurate number representations and their propensity for numeric errors in outputs.

Method: Explore how language models manipulate numbers, quantify accuracy bounds, create universal probes for each LLM, and trace information to specific layers.
...
<details>
  <summary>Details</summary>
Motivation: While ChatGPT Atlas has demonstrated capabilities for information retrieval, its performance in dynamic, interactive web environments remains under-explored.

Method: Used browser-based games (T-Rex Runner, Sudoku, Flappy Bird, Stein.world) as test scenarios and employed in-game performance scores as quantitative metrics to assess performance across different task types.
...
TL;DR: SCRIBE is a framework for generating pedagogically valid feedback to students using small language models (3B/8B) that combine tool-augmented reasoning with self-reflective inference, achieving quality comparable to much larger models while addressing privacy and computational constraints in education.


<details>
  <summary>Details</summary>
Motivation: Address three key challenges in deploying language models for educational feedback: privacy concerns, limited computational resources, and the need for pedagogically valid responses that require small, open-source models running locally with reliable information grounding.

Method: Introduce SCRIBE framework with multi-hop, tool-augmented reasoning and self-reflective inference pipeline supporting iterative reasoning, tool use, and error recovery. Use two-stage LoRA fine-tuning on synthetic GPT-4o-generated data to distill capabilities into 3B and 8B models.

Result: 8B-SCRIBE models achieve comparable or superior quality to much larger models in relevance and actionability, and are perceived on par with GPT-4o and Llama-3.3 70B by students in user study with 108 participants.

Conclusion: SCRIBE demonstrates viability for low-resource, privacy-sensitive educational applications by enabling small language models to generate high-quality, pedagogically valid feedback comparable to much larger models.
...
TL;DR: ACER transforms generalist LLMs into domain experts through automated curriculum generation and continual pretraining, achieving significant improvements in specialized domains without sacrificing general capabilities.


<details>
  <summary>Details</summary>
Motivation: LLMs underperform in specialized domains requiring deep understanding, creating a need for methods that can transform generalist models into domain experts while preserving their broad capabilities.
...
Result: ACER boosts accuracy by 5 percentage points in challenging domains like microeconomics, achieves 3 percentage point macro-average improvement across target domains, and enables positive cross-domain knowledge transfer. It also improves performance on ARC and GPQA benchmarks by over 2 points while maintaining general reasoning capabilities.

Conclusion: ACER provides a scalable and effective approach for closing domain gaps in LLMs, enabling transformation into domain experts while preventing catastrophic forgetting and facilitating knowledge transfer.
...
TL;DR: MisSynth pipeline uses retrieval-augmented generation to create synthetic fallacy samples for fine-tuning LLMs, achieving over 35% F1-score improvement on scientific misinformation detection.
...
Method: Proposed MisSynth pipeline applies retrieval-augmented generation (RAG) to produce synthetic fallacy samples, which are then used to fine-tune LLM models.
...
TL;DR: Proposes an interaction-centric framework for automatic team composition of LLMs by analyzing semantic coherence in pairwise conversations to identify synergistic clusters without requiring prior knowledge of model internals.
...
Method: Constructs a 'language model graph' mapping relationships from pairwise conversation semantic coherence, then applies community detection to identify synergistic model clusters without needing model architectures, training data, or task performance knowledge.

Result: Method discovers functionally coherent groups reflecting latent specializations; synergistic teams identified through topic-primed conversations outperform random baselines and achieve comparable accuracy to manually-curated teams.
...
TL;DR: This paper investigates using pretrained language models and large language models for discourse relation classification in scientific publications, examining how context from discourse structure can help this task.


<details>
  <summary>Details</summary>
Motivation: With increasing use of generative AI in science workflows, the authors want to use discourse-level information to find evidence for AI-generated scientific claims, starting with discourse structure analysis.

Method: Used pretrained language models (PLMs) and large language models (LLMs) for discourse relation classification (DRC) in scientific publications, examining how context from discourse structure helps the task.
...
Conclusion: This preliminary investigation demonstrates the potential of using context from discourse structure for improving discourse relation classification in scientific writing, which could support evidence-finding for AI-generated scientific claims.
...
TL;DR: OmniEduBench is a comprehensive Chinese educational benchmark with 24.6K QA pairs covering knowledge and cultivation dimensions across 61 subjects, revealing significant performance gaps between LLMs and human intelligence.


<details>
  <summary>Details</summary>
Motivation: Existing LLM benchmarks focus mainly on knowledge evaluation while neglecting cultivation capabilities essential for real-world education, particularly in the Chinese context with limited diversity.

Method: Created OmniEduBench with 24.6K high-quality QA pairs divided into knowledge (18.1K) and cultivation (6.5K) dimensions, each with 6 categories covering 61 subjects and 11 question formats.

Result: Experiments on 11 LLMs show performance gaps: only Gemini-2.5 Pro exceeded 60% accuracy in knowledge dimension, and best model QWQ trailed humans by nearly 30% in cultivation dimension.

Conclusion: There is substantial room for improvement in LLMs' educational capabilities, highlighting challenges in applying LLMs effectively in educational contexts.
...
TL;DR: SSLC combines sparse optimization and low-rank approximation to compress LLMs, achieving 50% compression on Qwen2.5 with no performance loss and 1.63x speedup.
...
Method: Formulates low-rank approximation and sparse optimization as a unified problem solved through iterative optimization, without requiring additional training steps.

Result: Consistently outperforms standalone methods on LLaMA and Qwen2.5 models (7B-70B), achieving state-of-the-art compression with 50% size reduction and 1.63x speedup.

Conclusion: SSLC provides a practical solution for efficient LLM deployment by synergistically combining sparse and low-rank compression techniques.
...
<details>
  <summary>Details</summary>
Motivation: Address challenges of domain-specific LLMs including lack of transparency, high fine-tuning costs, inconsistent results across domains, and environmental impact from computational demands.

Method: Integrates predictions from FinBERT, RoBERTa, and BERTweet through probabilistic late fusion using Bayesian networks, modeling sentiment predictions as probabilistic nodes.

Result: Achieves consistent ~6% accuracy gains across three diverse financial corpora, demonstrating robustness to dataset variability and effectiveness of probabilistic fusion.

Conclusion: BNLF provides an effective probabilistic fusion approach for interpretable sentiment classification that outperforms individual LLMs while addressing transparency and consistency issues.
...
TL;DR: Ensemble of multiple LLM agents provides more reliable assessment of AI triage tool than single LLM, with open-source ensembles outperforming GPT-4o.


<details>
  <summary>Details</summary>
Motivation: To determine if an ensemble of multiple LLM agents could provide more reliable assessment of a pixel-based AI triage tool than a single LLM.

Method: Analyzed 29,766 non-contrast CT head exams using commercial ICH AI detection tool. Radiology reports were analyzed by ensemble of eight open-source LLMs and GPT-4o using multi-shot prompt. 1,726 examples manually reviewed. Performance compared between models and consensus ensembles.

Result: Llama3.3:70b and GPT-4o achieved highest AUC (0.78). Llama3.3:70b had highest F1 score (0.81) and recall (0.85). Ensemble methods (Full-9, Top-3, Consensus) showed better MCC scores than GPT-4o alone, with no significant differences between ensemble types.

Conclusion: Ensemble of medium to large open-source LLMs provides more consistent and reliable method for retrospective evaluation of clinical AI triage tools compared to single LLM.
...
TL;DR: CORE-KG framework reduces node duplication and noise in knowledge graphs from legal documents by integrating coreference resolution and structured prompts, with ablation study showing structured prompts are more crucial for noise reduction while coreference resolution better prevents duplication.


<details>
  <summary>Details</summary>
Motivation: Human smuggling networks are adaptive and hard to analyze using unstructured legal documents. Existing LLM-based approaches create noisy, fragmented graphs with duplicate nodes due to lack of guided extraction and coreference resolution.
...
Conclusion: Structured prompts are more effective for reducing noise while coreference resolution is better for preventing duplication, providing empirical insights for designing robust LLM-based extraction pipelines for complex legal texts.
...
<details>
  <summary>Details</summary>
Motivation: Hebrew diacritics restoration is crucial for accurate pronunciation and meaning disambiguation, as the language has high ambiguity when unvocalized. Recent ML approaches have advanced performance, but there's need for more effective methods.
...
Conclusion: Visual representations show promising potential for accurate and automated Hebrew diacritization, demonstrating the effectiveness of the DIVRIT approach in handling this fundamental NLP task.
...
TL;DR: Linear relational decoders in transformers are highly compressible and extract recurring semantic properties rather than distinct relations, explaining their redundancy and limited generalization.
...
Method: Systematically chart relation decoder organization, compress them using order-3 tensor networks, and develop cross-evaluation protocol to apply each decoder to subjects of other relations.

Result: Linear decoders are highly compressible without accuracy loss and extract coarse-grained semantic properties rather than distinct relations, explaining their redundancy and generalization patterns.

Conclusion: Linear relational decoding in transformers is primarily property-based rather than relation-specific, clarifying compressibility and generalization limitations to semantically close relations.
...
TL;DR: InfoFlow addresses low reward density in RLVR by decomposing tasks, providing failure-guided hints, and using dual-agent refinement to improve exploration efficiency.
...
Result: InfoFlow outperforms baselines on agentic search benchmarks, enabling lightweight LLMs to match advanced proprietary LLMs.

Conclusion: InfoFlow effectively solves the reward density optimization problem, making RLVR more practical for deep search applications.
...
<details>
  <summary>Details</summary>
Motivation: LLMs face significant inference latency due to autoregressive design and large size, while existing speculative decoding approaches neglect system variables like GPU devices and batch sizes.

Method: Introduces CAST, a dynamic tree decoding approach that considers inference costs (GPU configurations, batch sizes) to dynamically refine tree structure during speculative decoding.
...
Conclusion: Considering system variables in dynamic tree decoding significantly improves speculative decoding performance, making CAST an effective solution for reducing LLM inference latency.
...
TL;DR: SlideAgent is an agentic framework that improves understanding of multi-page visual documents like slide decks through specialized agents for global, page, and element-level reasoning.


<details>
  <summary>Details</summary>
Motivation: Current LLMs struggle with complex multi-page visual documents that require fine-grained reasoning over elements and pages, particularly for documents relying on layout, colors, icons, and cross-slide references.

Method: Uses specialized agents organized in three reasoning levels (global, page, element) to build structured, query-agnostic representations. During inference, selectively activates agents for multi-level reasoning and integrates their outputs.
...
Conclusion: SlideAgent provides an effective framework for understanding multi-modal, multi-page, and multi-layout documents through specialized agentic reasoning.
...
<details>
  <summary>Details</summary>
Motivation: The rapid transition to decoder-only models without rigorous comparative analysis raises concerns that encoder-decoder models' potential may have been overlooked, especially from a scaling perspective.
...
Result: RedLLM shows compelling scaling properties and strong performance, with comparable scaling and context length extrapolation to DecLLM. After instruction tuning, RedLLM achieves comparable or better results on downstream tasks with substantially better inference efficiency.

Conclusion: Encoder-decoder LLMs deserve re-examination as they show strong potential for developing powerful and efficient LLMs, with better inference efficiency after instruction tuning while maintaining competitive performance.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle in data-sensitive domains like healthcare due to lack of high-quality domain-specific training data, while domain experts have created valuable ontology rules that formalize knowledge relationships.
...
Result: Extensive experiments on medical QA benchmarks with Llama3-8B-Instruct and Med42-v2 show consistent outperformance over unmodified models and leading supervised baselines.

Conclusion: The approach is effective, efficient, and robust for low-resource domain adaptation of LLMs, confirming the value of leveraging ontology rules for knowledge enhancement.
...
<details>
  <summary>Details</summary>
Motivation: To develop a linear attention architecture that can outperform full attention under fair comparisons across different scenarios including short-context, long-context, and RL scaling regimes, while improving efficiency.

Method: Uses Kimi Delta Attention (KDA) - an expressive linear attention module extending Gated DeltaNet with finer-grained gating mechanism, and a bespoke chunkwise algorithm with specialized DPLR transition matrices. Pretrained a 3B activated parameter model with layerwise hybrid of KDA and Multi-Head Latent Attention (MLA).

Result: Outperforms full MLA with significant margin across all evaluated tasks, reduces KV cache usage by up to 75%, achieves up to 6x decoding throughput for 1M context, and serves as a drop-in replacement for full attention with superior performance and efficiency.

Conclusion: Kimi Linear demonstrates that linear attention architectures can outperform full attention while providing substantial efficiency gains, making it a viable replacement for full attention in various applications including long input/output tasks.
...
TL;DR: This paper investigates how value alignment emerges during LLM post-training, finding that supervised fine-tuning (SFT) establishes core values while preference optimization rarely re-aligns them, with different algorithms producing different alignment outcomes even with identical data.


<details>
  <summary>Details</summary>
Motivation: As LLMs become more influential, understanding how they learn human values during training is crucial, but prior work mostly evaluated final models without studying training dynamics.

Method: Analyzed value alignment dynamics during post-training using Llama-3 and Qwen-3 models with various SFT and preference optimization datasets/algorithms, plus controlled experiments with synthetic preference data.

Result: SFT phase primarily establishes model values; preference optimization rarely re-aligns these values; different preference optimization algorithms lead to different alignment outcomes even with identical preference data.
...
TL;DR: AMO-Bench is a new mathematical reasoning benchmark with Olympiad-level difficulty featuring 50 original human-crafted problems to address performance saturation in existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing math competition benchmarks are becoming less effective for evaluating top-tier LLMs due to performance saturation, requiring more rigorous challenges.

Method: Created 50 cross-validated original problems meeting IMO difficulty standards, requiring only final answers for automatic grading to prevent data memorization issues.
...
Conclusion: AMO-Bench highlights significant room for improvement in LLMs' mathematical reasoning and provides a challenging benchmark for advancing reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: As coding agents are increasingly deployed in large codebases, there is a need for automatically designing challenging, codebase-level evaluations to test their capabilities.

Method: Proposed Gistify task where coding LLMs are given full access to a codebase and must create a single minimal file that replicates the output of a specific command while containing only essential components.
...
Conclusion: Gistify presents a challenging evaluation framework that reveals limitations in current coding LLMs' ability to understand codebase structure and model execution flows for complex tasks.
...
Method: Two-phase approach: first evaluating T-YOLOv5 (temporal-enhanced YOLOv5) vs standard YOLOv5, then developing T-YOLOv5 with CBAM (Convolutional Block Attention Module) for enhanced spatial attention.

Result: T-YOLOv5 and T-YOLOv5 with CBAM significantly outperformed standard YOLOv5 with mAP@50-95 scores of 0.813 and 0.811 respectively, compared to 0.563 for YOLOv5. T-YOLOv5 with CBAM excels in challenging scenarios but loses some accuracy in simpler cases.

Conclusion: Temporal modeling significantly enhances detection reliability in underwater environments, and spatial attention mechanisms further improve performance in complex scenarios, though with trade-offs in simpler conditions.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image models trained on uncurated datasets don't align well with user preferences, and post-hoc reward-based selection methods harm diversity and efficiency.
...
Result: MIRO dramatically improves visual quality, significantly speeds up training, and achieves state-of-the-art performance on GenEval benchmark and user-preference scores.

Conclusion: Directly conditioning on multiple rewards during training is more effective than post-hoc selection for aligning text-to-image models with user preferences.
...
TL;DR: This paper presents a 3D LiDAR segmentation approach for bicycle safety using the 'SenseBike' platform and introduces the BikeScenes-lidarseg dataset to address the domain gap between automotive and bicycle perception systems.
...
Result: Fine-tuning on the BikeScenes dataset achieved a mean Intersection-over-Union (mIoU) of 63.6%, significantly outperforming the 13.8% obtained with SemanticKITTI pre-training alone, demonstrating the effectiveness of domain-specific training.

Conclusion: The research highlights the necessity of domain-specific training for bicycle-mounted perception systems and contributes the BikeScenes dataset as a valuable resource for advancing cyclist-centric LiDAR segmentation research.
...
TL;DR: Machine learning approach using physics-informed synthetic data to repair STM images and enable super-resolution, reducing acquisition time and tip conditioning needs.


<details>
  <summary>Details</summary>
Motivation: STM faces limitations from tip degradation and slow serial data acquisition, plus tip conditioning complexity during fabrication.

Method: Used physics-informed synthetic data generation pipeline with only 36 pristine experimental images to train flow-matching and diffusion models for image restoration.

Result: Models effectively restore images and offer 2-4x reduction in image acquisition time by reconstructing from sparse data, validated by CLIP Maximum Mean Discrepancy and structural similarity metrics.

Conclusion: Framework can significantly increase STM throughput by reducing tip-conditioning frequency and enhancing frame rates in high-speed STM systems.
...
TL;DR: Proposes a flow decomposition-and-aggregation framework for zero-shot image editing that addresses inversion inaccuracy and gradient entanglement issues in rectified flow models by semantically decomposing target prompts and adaptively aggregating sub-flows.


<details>
  <summary>Details</summary>
Motivation: Rectified flow models have limitations in image editing tasks due to inaccurate inversion processes and gradient entanglement issues, leading to outputs that don't faithfully reflect target prompts. Recent ODE-based approaches without inversion still yield suboptimal editing quality.
...
Result: Outperforms existing zero-shot editing approaches in terms of semantic fidelity and attribute disentanglement. The method enhances diversity in target space while maintaining consistency with the full target prompt.

Conclusion: The proposed flow decomposition-and-aggregation framework effectively addresses limitations of rectified flow models in image editing, providing improved semantic fidelity and attribute disentanglement through adaptive flow aggregation.
...
TL;DR: Brain-IT is a brain-inspired method using Brain Interaction Transformer (BIT) to reconstruct images from fMRI data with improved faithfulness, achieving state-of-the-art results with significantly less training data.
...
Method: Uses Brain Interaction Transformer (BIT) with functional clusters of brain-voxels shared across subjects. Predicts complementary patch-level features: high-level semantic features and low-level structural features to guide diffusion model reconstruction.

Result: Achieves faithful image reconstructions that surpass current state-of-the-art approaches both visually and by objective metrics. With only 1-hour of fMRI data per new subject, achieves comparable results to methods trained on 40-hour recordings.

Conclusion: Brain-IT provides an effective brain-inspired approach for image reconstruction from fMRI data, enabling high-fidelity results with significantly reduced training data requirements through shared functional clusters and direct information flow from brain-voxels to image features.
...
TL;DR: This paper presents a real-time tumor tracking method for MRI-guided radiotherapy using SAM 2.1 foundation model with mask-based prompts, achieving 0.8794 Dice score on TrackRAD2025 challenge despite data scarcity constraints.
...
Method: Used SAM 2.1 foundation model with mask-based prompts from first annotated slice, fine-tuned on small labeled subset. Training minimized overfitting with 1024x1024 patches, balanced Dice+IoU loss, and low uniform learning rate across all modules.

Result: Achieved Dice score of 0.8794 on hidden test set, ranking 6th overall in TrackRAD2025 challenge. The method maintained real-time performance within one-second runtime constraint.

Conclusion: Foundation models show strong potential for accurate and real-time tumor tracking in MRI-guided radiotherapy, demonstrating effective adaptation to data-scarce scenarios.
...
Result: Significant improvements in both quantitative metrics and qualitative visual fidelity on low-light image enhancement benchmarks, with reduced computational resource consumption and shorter inference time.
...
TL;DR: CAVE is the first benchmark for real-world visual anomalies with tasks for description, explanation, and justification, showing current VLMs struggle with anomaly perception despite advanced prompting.
...
Conclusion: CAVE provides a realistic, cognitively grounded benchmark to advance research in anomaly detection and commonsense reasoning in VLMs.
...
TL;DR: A lightweight CNN model for coastal flood prediction under sea-level rise scenarios, achieving 20% lower MAE than state-of-the-art methods and generalizing across Abu Dhabi and San Francisco.
...
<details>
  <summary>Details</summary>
Motivation: Current Video-LLM architectures have critical limitations in understanding complex temporal dynamics in videos, particularly struggling with action sequences and temporal progression.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of time-consuming and costly pixel-wise expert annotations for training deep learning models in ultrasound image segmentation, particularly for pediatric elbow and wrist fractures.

Method: Proposes FlexICL framework using in-context learning for intra-video segmentation, with novel image concatenation techniques and multiple augmentation strategies to enhance performance with limited labeled data.
...
<details>
  <summary>Details</summary>
Motivation: Traditional negative prompting uses fixed prompts, which may not adapt well to different contexts during the denoising process.
...
Conclusion: Dynamic negative prompting with VLMs provides adaptive negative guidance that improves over fixed negative prompts in diffusion models.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-modal diffusion models have inadequate alignment between text and image modalities, creating security risks for generating NSFW content that current prompt-based attacks don't address.

Method: Proposed Prompt-Restricted Multi-modal Attack (PReMA) creates adversarial images that, when combined with any specified prompt, manipulate model outputs without altering the prompt itself.
...
Conclusion: Cross-view temporal consistency is challenging for Video-LLMs. View-GRPO effectively addresses this limitation by reinforcing both view-specific reasoning and cross-view consistency through reinforcement learning.
...
TL;DR: OracleAgent is the first agent system for structured management and retrieval of Oracle Bone Script (OBS) information, integrating multiple OBS analysis tools with LLMs and leveraging a comprehensive multimodal knowledge base to assist experts in OBS research.


<details>
  <summary>Details</summary>
Motivation: Current OBS research faces challenges with complex interpretation workflows and inefficient information organization/retrieval, requiring substantial effort for resource management.

Method: Developed OracleAgent system that integrates multiple OBS analysis tools powered by LLMs, with a comprehensive multimodal knowledge base containing over 1.4M character rubbing images and 80K interpretation texts built through multi-year data collection and expert annotation.

Result: OracleAgent achieves superior performance in multimodal reasoning and generation tasks, surpassing leading MLLMs like GPT-4o, and significantly reduces time cost for OBS research in case studies.

Conclusion: OracleAgent represents a significant advancement toward practical deployment of OBS-assisted research and automated interpretation systems, effectively addressing current research bottlenecks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional novel view synthesis methods rely on external camera pose estimation tools like COLMAP, which introduce computational bottlenecks and propagate errors.
...
Result: Significantly outperforms existing COLMAP-free techniques in reconstruction quality and surpasses standard COLMAP-based baseline in general, particularly effective in challenging scenarios with large viewpoint variations and sparse feature distributions.

Conclusion: The proposed unified framework successfully eliminates the dependency on external camera pose estimation tools while achieving superior performance in novel view synthesis through joint optimization of scene representation and camera poses.
...
<details>
  <summary>Details</summary>
Motivation: Current E2E driving benchmarks primarily test nominal scenarios and existing metrics fail to adequately evaluate performance in rare, challenging situations, limiting the development of robust autonomous driving systems.

Method: Created WOD-E2E dataset with 4,021 driving segments (12 hours) specifically curated for long-tail scenarios occurring less than 0.03% frequency. Each segment includes routing information, ego states, and 360-degree camera views from 8 cameras. Introduced Rater Feedback Score metric that evaluates trajectory predictions against human rater preference labels.

Result: Developed a comprehensive dataset and evaluation framework for E2E driving in challenging scenarios. Released rater preference labels for validation set and established the 2025 WOD-E2E Challenge using held-out test set labels.

Conclusion: WOD-E2E aims to advance research in generalizable, robust, and safe end-to-end autonomous driving by providing a benchmark specifically designed for evaluating performance in complex real-world long-tail scenarios.
...
TL;DR: The paper proposes integrating gradient-based attention information with CNN features to improve RGB-D indoor SLAM performance, particularly in large environments.


<details>
  <summary>Details</summary>
Motivation: Existing approaches use attention visualization for CNN interpretability, but don't directly integrate gradient-based attention into CNN representations for semantic understanding, which could benefit visual tasks like SLAM.
...
Result: Experimental results show improved performance compared to baseline methods, with particular benefits for large environments.

Conclusion: Direct integration of gradient-based attention information with CNN representations enhances SLAM performance, especially in large-scale indoor environments.
...
Method: Uses implicit box vector-set diffusion for bounding box layout, then generates each part in its own fixed full-resolution voxel grid. Introduces center-point encoding to address misalignment between parts of different sizes.
...
TL;DR: BasicAVSR is a strong baseline for arbitrary-scale video super-resolution that integrates adaptive frequency priors, flow-guided propagation, second-order motion compensation, and hyper-upsampling to handle various scaling factors while maintaining spatial detail and temporal consistency.
...
Conclusion: The work advances state-of-the-art in AVSR and extends core components to multiple frameworks for diverse application scenarios, providing an effective and adaptable solution.
...
<details>
  <summary>Details</summary>
Motivation: Large annotated datasets for breast cancer CAD models are costly and time-consuming to acquire. Vision-Language Models pre-trained on image-text pairs offer a promising solution for enhancing robustness and data efficiency in medical imaging.

Method: Proposes MV-MLM with multi-view supervision using paired mammogram images and synthetic radiology reports. Employs cross-modal self-supervision across image-text pairs and a joint visual-textual learning strategy to learn rich representations from extensive radiology data.

Result: Achieves state-of-the-art performance in three classification tasks: malignancy classification, subtype classification, and image-based cancer risk prediction. Demonstrates strong data efficiency, outperforming fully supervised or VLM baselines while trained on synthetic text reports.

Conclusion: The proposed MV-MLM model effectively leverages synthetic radiology reports and multi-view supervision to achieve superior performance in breast cancer classification and risk prediction without requiring actual radiology reports, offering a data-efficient solution for medical imaging tasks.
...
TL;DR: A YOLOv8-based real-time detection system for auto-rickshaws in Bangladesh traffic, achieving 83.4% mAP50 and over 78% precision/recall.


<details>
  <summary>Details</summary>
Motivation: Auto-rickshaws in Bangladesh need monitoring due to traffic restrictions, but existing surveillance struggles to distinguish them from similar vehicles, and manual analysis is inefficient.

Method: Used YOLOv8 model for real-time object detection, trained on 1,730 annotated images collected under various traffic conditions.
...
Conclusion: The proposed system effectively automates auto-rickshaw detection in real-time, and the dataset has been publicly released for further research.
...
<details>
  <summary>Details</summary>
Motivation: There is no comprehensive benchmark for multi-modal RAG in wearable scenarios, despite smart glasses transforming how people interact with their surroundings by enabling visual entity queries.

Method: Created CRAG-MM benchmark with diverse datasets including 6.2K egocentric images mimicking wearable captures, carefully constructed questions reflecting real-world challenges like image quality issues, entity popularity, and information dynamism.

Result: Straightforward RAG approaches achieved only 32% truthfulness on single-turn QA and 43% on multi-turn QA, while state-of-the-art industry solutions showed similar performance (32%/45%), indicating significant room for improvement.

Conclusion: CRAG-MM successfully fills the benchmark gap for multi-modal RAG in wearable scenarios, has already attracted significant community participation (1K participants, 5K submissions in KDD Cup 2025), and winning solutions improved baseline performance by 28%, demonstrating its impact on advancing the field.
...
TL;DR: MoTDiff is the first high-resolution motion trajectory estimation framework using diffusion models that extracts fine-grained motion paths from single motion-blurred images, outperforming state-of-the-art methods in blind image deblurring and coded exposure photography.
...
Method: Proposes MoTDiff with two key components: 1) conditional diffusion framework using multi-scale feature maps from blurred images as condition, 2) training method promoting precise fine-grained trajectory identification, consistent shape/position estimation, and pixel connectivity along motion paths.
...
Conclusion: The proposed diffusion-based framework successfully achieves high-resolution motion trajectory estimation from single motion-blurred images, demonstrating superior performance over existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Dataset bias is common in machine learning datasets but systematically identifying these biases is challenging without costly, fine-grained attribute annotations. There's a need for scalable automated methods to analyze visual datasets and discover biases.
...
Result: ConceptScope captures diverse visual concepts including objects, textures, backgrounds, facial attributes, emotions, and actions. It produces spatial attributions aligned with semantically meaningful image regions. The framework reliably detects known biases (e.g., background bias in Waterbirds) and uncovers previously unannotated biases (e.g., co-occurring objects in ImageNet).

Conclusion: ConceptScope offers a practical tool for dataset auditing and model diagnostics by providing automated, scalable analysis of visual datasets to identify and quantify biases without requiring expensive manual annotations.
...
TL;DR: A novel approach for 3D human pose estimation from sketches using a 'learn from synthesis' strategy, creating a synthetic dataset SKEP-120K and combining diffusion models with neural networks for efficient and accurate pose estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional sketch-to-pose methods are constrained by lack of large-scale sketch-3D pose annotations, relying on time-consuming optimization with limited generalizability. The abstract and disproportionate nature of sketches presents unique challenges for 3D human pose estimation.

Method: Uses a 'learn from synthesis' strategy: first trains a diffusion model to synthesize sketch images from 2D poses projected from 3D human poses, creating SKEP-120K synthetic dataset. Then builds an end-to-end framework combining 2D pose detectors, generative diffusion priors, and a feed-forward neural network with heuristic loss functions for geometric coherence.
...
Conclusion: The proposed approach effectively addresses the challenges of sketch-based 3D human pose estimation through synthetic data generation and an integrated framework, achieving superior performance compared to previous methods.
...
TL;DR: The paper proposes a Chebyshev ensemble geometric network (Ch-EGN) that combines convolutional and geometric deep learning to enhance supply chain sustainability through risk management and product classification.


<details>
  <summary>Details</summary>
Motivation: Supply chain sustainability is crucial for optimal performance, and managing risks and proper product classification are fundamental problems for developing sustainable supply networks and improving efficiency.

Method: A novel geometric deep network is used to create an ensemble deep network (Ch-EGN) that leverages information dependencies in supply chains to derive invisible states of samples, tested on SupplyGraph Dataset and DataCo databases.

Result: Achieved 98.95% accuracy for risk management, 100% accuracy for 5 product group classification, 98.07% for 4 product relation classification, and 92.37% for 25 company relation classification, showing improvement over state-of-the-art approaches.

Conclusion: The proposed Ch-EGN method effectively enhances supply chain sustainability through accurate risk management and classification tasks, demonstrating superior performance compared to existing methods.
...
TL;DR: The paper introduces OmniLayout-1M, a million-scale diverse document layout dataset, and OmniLayout-LLM, a 0.5B model with coarse-to-fine learning for document layout generation, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Document layout generation is underexplored compared to document layout analysis, with existing datasets dominated by academic papers and lacking diversity in open-world document types like newspapers and magazines.

Method: Proposed OmniLayout-LLM, a 0.5B model with two-stage coarse-to-fine learning: 1) learning universal layout principles from OmniLayout-1M dataset with coarse categories, and 2) transferring knowledge to specific domains with fine-grained annotations.
...
Conclusion: The OmniLayout-1M dataset and OmniLayout-LLM model address the diversity gap in document layout generation and demonstrate superior performance across various document types.
...
TL;DR: VLMs struggle with temporal reasoning, performing poorly on judging video direction (forward/backward) compared to humans, especially for irreversible physical processes.


<details>
  <summary>Details</summary>
Motivation: To evaluate VLMs' temporal understanding capabilities, which are currently weak and under-evaluated, using a psychophysically validated benchmark.

Method: Created AoT-PsyPhyBENCH benchmark with natural videos to test arrow of time judgment, comparing VLMs against human behavioral baselines on irreversible processes and causal actions.

Result: Most VLMs perform near chance level, with even the best models lagging significantly behind human accuracy on physically irreversible processes and causal manual actions.

Conclusion: Current VLMs lack inductive biases for temporal continuity and causal understanding despite rich visual-semantic correlations, highlighting a fundamental gap in multimodal systems.
...
Method: Proposes a multi-scale mask-regulated variational bottleneck encoder for accurate low-level modal information extraction, combined with diffusion models integrated with physical laws through time-varying physical guidance mechanism.
...
TL;DR: Analysis of CNN complementarity for periocular verification across distances using SqueezeNet, MobileNetv2, and ResNet50 trained on VGGFace2 eye crops, with fusion methods and attention pattern analysis.


<details>
  <summary>Details</summary>
Motivation: To study how different CNN architectures complement each other for periocular verification at varying distances and understand their attention patterns to improve performance.

Method: Train three CNN architectures (SqueezeNet, MobileNetv2, ResNet50) on VGGFace2 eye crops, analyze with cosine/chi2 metrics, compare initializations, apply score-level fusion via logistic regression, and use LIME heatmaps with Jensen-Shannon divergence for attention analysis.

Result: ResNet50 performs best individually, but fusion of all three networks provides substantial performance gains. Heatmaps reveal networks focus on distinct image regions, explaining their complementarity. Achieves new state-of-the-art on UBIPr database.

Conclusion: Combining multiple CNN architectures through fusion significantly improves periocular verification performance due to their complementary attention patterns, with the three-network fusion achieving the best results and outperforming previous methods.
...
<details>
  <summary>Details</summary>
Motivation: To address mode collapse in imitation learning methods and the inability of existing generative approaches to directly incorporate safety and physical constraints during trajectory generation, which requires additional optimization stages.

Method: Proposes CATG framework using Constrained Flow Matching that explicitly models the flow matching process, imposes explicit constraints directly within flow matching, and parameterizes driving aggressiveness as a control signal for trajectory style manipulation.

Result: Achieved 2nd place on NavSim v2 challenge with EPDMS score of 51.31 and received Innovation Award, demonstrating effective trajectory generation with built-in constraints.

Conclusion: CATG successfully addresses limitations of existing methods by directly incorporating constraints into the generative process while enabling diverse trajectory generation and style control, representing a significant advancement in autonomous driving planning.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing periocular recognition works that use small-scale datasets by leveraging large-scale training data from VGGFace2 to improve recognition performance.

Method: Three CNN architectures of varying depth/complexity trained on 1,907,572 ocular crops from VGGFace2 database, evaluated on VGGFace2-Pose (in-the-wild) and UFPR-Periocular (mobile selfies) datasets.

Result: Achieved 1-2% EER on UFPR-Periocular (best reported to date), significantly better than 9-15% EER on VGGFace2-Pose due to higher image quality and consistent acquisition protocols.

Conclusion: Large-scale training data significantly improves periocular recognition performance, with controlled acquisition conditions yielding substantially better results than uncontrolled in-the-wild scenarios.
...
<details>
  <summary>Details</summary>
Motivation: AEOS constellations offer unprecedented flexibility but face scheduling challenges under large-scale scenarios, dynamic environments, and stringent constraints that existing methods oversimplify.

Method: Developed AEOS-Bench benchmark suite with 3,907 satellite assets and 16,410 scenarios generated via high-fidelity simulation, and AEOS-Former Transformer model with constraint-aware attention mechanism and internal constraint module.
...
Conclusion: The framework provides a robust solution for AEOS constellation scheduling, with code and data publicly available for further research.
...
<details>
  <summary>Details</summary>
Motivation: To understand the impact of different music genres on human emotions and demonstrate measurable brain activity changes during music listening.

Method: Used EEG helmet to measure brain activity while participants listened to various music genres, combined with subjective surveys from a diverse group of participants.

Result: Analysis revealed connections between emotions reported in questionnaires and observed brain activity patterns measured by EEG.

Conclusion: Different music genres elicit distinct emotional responses that are reflected in measurable brain activity patterns.
...
TL;DR: Proposes an evidential fusion paradigm to combine CNN and ViT backbones for diabetic retinopathy grading, leveraging local and global feature strengths respectively to overcome single-backbone limitations.


<details>
  <summary>Details</summary>
Motivation: Diabetic retinopathy causes vision loss and existing automated diagnosis systems using single CNN or ViT backbones have performance bottlenecks. Integrating both backbones could leverage their respective strengths in local and global feature extraction.

Method: Proposes evidential fusion paradigm that transforms features from different backbones into supporting evidences via deep evidential networks, forming aggregated opinions to adaptively tune fusion patterns between CNN and ViT.
...
Conclusion: The evidential fusion approach effectively combines CNN and ViT strengths for DR grading, achieving better performance while maintaining interpretability in feature fusion and decision processes.
...
TL;DR: GLYPH-SR is a vision-language-guided diffusion framework for image super-resolution that specifically optimizes for scene-text legibility while maintaining overall perceptual quality, addressing the limitations of traditional SR methods that treat text as generic texture.


<details>
  <summary>Details</summary>
Motivation: Scene-text in natural images carries critical actionable information, but traditional SR methods optimized for distortion or perceptual metrics are insensitive to character-level errors, causing OCR failures even when images appear sharp. Existing text SR research often uses simplified benchmarks, overlooking complex natural scenes.

Method: GLYPH-SR uses a Text-SR Fusion ControlNet guided by OCR data and a ping-pong scheduler that alternates between text- and scene-centric guidance. The framework trains these components on synthetic corpus while keeping the main SR branch frozen.
...
Conclusion: GLYPH-SR successfully delivers super-resolution that achieves both high text readability and high visual realism, making it effective for practical deployments where accurate scene-text recognition is crucial.
...
Method: Proposes a dual-conditioning framework using Adaptive Thinking Mapper (ATM) for EEG feature extraction and fine-tunes Stable Diffusion 2.1 via LoRA, with a ControlNet branch for spatial conditioning on saliency maps.
...
TL;DR: LoCoT2V-Bench is a new benchmark for evaluating long video generation with complex prompts, addressing gaps in existing benchmarks by focusing on fine-grained alignment, narrative coherence, and thematic expression.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-video benchmarks use simplified prompts and focus on low-level metrics, overlooking fine-grained alignment with prompts and abstract dimensions like narrative coherence and thematic expression.
...
Result: Evaluation of nine LVG models showed they perform well on basic visual/temporal aspects but struggle with inter-event consistency, fine-grained alignment, and high-level thematic adherence.
...
TL;DR: A-TPT is a novel test-time prompt tuning framework that improves vision-language model calibration by maximizing angular diversity between textual features on the unit hypersphere, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current TPT methods lack optimal angular separation between class-wise textual features, hurting calibration performance and raising concerns about VLMs' reliability and safety.

Method: Introduces angular diversity by maximizing the minimum pairwise angular distance between normalized textual features on the unit hypersphere to achieve uniform distribution.

Result: Consistently surpasses state-of-the-art TPT methods in reducing aggregate average calibration error while maintaining comparable accuracy across various backbones and datasets.

Conclusion: Promoting angular diversity achieves well-dispersed textual features, significantly improving VLM calibration during test-time adaptation, with superior performance on distribution shifts and medical datasets.
...
TL;DR: The paper adapts foundational 3D reconstruction models (DUSt3R/MASt3R) for point tracking using 3D grounded correspondence, achieving competitive results on multiple benchmarks by combining reconstruction loss with dynamic correspondence training and a visibility head.


<details>
  <summary>Details</summary>
Motivation: To leverage recent advances in foundational 3D reconstruction models for point tracking tasks, addressing limitations in handling both static and dynamic points in current benchmarks.
...
Result: Achieves competitive or superior performance on four datasets: TAP-Vid-DAVIS (73.8 δ_avg / 85.8% occlusion acc vs CoTracker2's 75.7/88.3%), significantly outperforms CoTracker3 on EgoPoints (61.3 vs 54.2) and RGB-S (87.0 vs 82.8).

Conclusion: 3D reconstruction models can be effectively adapted for point tracking through 3D grounded correspondence, achieving state-of-the-art performance on both static and dynamic point tracking benchmarks.
...
TL;DR: FineGrainedAD improves few-shot anomaly detection by providing multi-level fine-grained textual descriptions and using learnable prompts with semantic alignment to better localize patch-level visual anomalies.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from semantic misalignment between image-level descriptions and patch-level visual anomalies due to lack of detailed textual descriptions, leading to sub-optimal localization performance.

Method: Proposes Multi-Level Fine-Grained Semantic Caption (MFSC) for automatic dataset annotation, Multi-Level Learnable Prompt (MLLP) for fine-grained semantic injection, and Multi-Level Semantic Alignment (MLSA) with region aggregation and alignment training.
...
Conclusion: The proposed approach effectively addresses semantic misalignment in few-shot anomaly detection through fine-grained textual descriptions and multi-level alignment, significantly improving localization accuracy.
...
Method: Estimate object and background expectations in CLIP's representation space, synthesize counterfactual embeddings by recombining object features with diverse alternative contexts, and use Total Direct Effect with intervention simulation to subtract background-only activation.
...
TL;DR: The paper identifies the "Matthew effect" in self-improvement of LVLMs where models prioritize simple queries over complex ones, leading to imbalanced optimization. It proposes distribution-reshaping and trajectory-resampling strategies to re-balance head-tail data during self-improvement.


<details>
  <summary>Details</summary>
Motivation: Self-improvement in LVLMs suffers from the Matthew effect - models excel at simple queries but struggle with complex ones, causing imbalanced optimization that prioritizes simple reasoning skills and hinders complex reasoning capabilities over iterations.
...
Conclusion: The proposed head-tail re-balancing strategies effectively counteract the Matthew effect in self-improvement, enabling LVLMs to improve both simple and complex reasoning capabilities and overcome performance bottlenecks.
...
TL;DR: An adaptable edge detector using cellular automata optimized by meta-heuristics with transfer learning was developed. Expanding the optimization search space was ineffective, and transfer learning showed no significant improvements, though the model could adapt to input images.


<details>
  <summary>Details</summary>
Motivation: To address weaknesses in edge detectors, particularly difficulty detecting loose edges and lack of context for specific problems, by creating an adaptable detector.

Method: Developed a two-dimensional cellular automaton edge detector optimized by meta-heuristic combined with transfer learning techniques.

Result: Expanding the optimization search space was not effective. The model could adapt to input images, but transfer learning techniques showed no significant improvements.

Conclusion: The adaptable edge detector works but expanding search space optimization and transfer learning did not provide benefits for the tested image set.
...
TL;DR: SA²Net: A scale-adaptive structure-aware network for spine segmentation in ultrasound volume projection imaging that addresses challenges in learning global contextual knowledge and encoding structural bone features.


<details>
  <summary>Details</summary>
Motivation: Spine segmentation is crucial for intelligent scoliosis diagnosis but faces challenges: poor learning of global contextual knowledge due to neglected spatial correlation of bone features, and failure to encode rich structural knowledge about bone shapes and positions.

Method: Proposes SA²Net with: 1) scale-adaptive complementary strategy to learn cross-dimensional long-distance correlation features, 2) structure-affinity transformation that transforms semantic features with class-specific affinity using Transformer decoder for structure-aware reasoning, 3) feature mixing loss aggregation for enhanced training.
...
Conclusion: The proposed SA²Net shows strong potential as a promising tool for advanced scoliosis diagnosis through intelligent spinal image analysis, with adaptability to various backbones enhancing its practical applicability.
...
<details>
  <summary>Details</summary>
Motivation: Manual creation of multiple ad versions from longer videos is labor-intensive and time-consuming for advertisers.
...
Result: Outperforms state-of-the-art methods across Average Precision, AUC, Spearman, and Kendall metrics.

Conclusion: Audio-visual fusion approach effectively automates video ad clipping by considering both visual and audio elements.
...
TL;DR: Dynamic Context-Aware Scene Reasoning framework using Vision-Language Alignment for zero-shot scene understanding in unfamiliar real-world environments without labeled data.


<details>
  <summary>Details</summary>
Motivation: AI systems struggle with unfamiliar scenarios without labeled data, limiting deployment in dynamic, unstructured settings. Need for generalization across unseen contexts.
...
Result: 18% improvement in scene understanding accuracy on zero-shot benchmarks (COCO, Visual Genome, Open Images). Robust performance in ambiguous/cluttered scenes due to vision-language fusion.
...
<details>
  <summary>Details</summary>
Motivation: VQA models like LLaVA perform well on natural images but degrade significantly on out-of-domain scenarios (remote sensing, medical, math) due to distribution shifts and lack of effective domain adaptation mechanisms.

Method: Decouples visual and linguistic adaptation using: 1) domain classifier to identify image type, 2) dual adapter mechanism with Prompt Adapter for language modulation and Visual Adapter for vision feature adjustment, both dynamically injected via unified hook interface.

Result: Achieved consistent performance gains across four domain-specific VQA benchmarks: +2.3 BLEU on MathVQA, +2.6 VQA on MedVQA-RAD, +3.1 ROUGE on ChartQA without retraining backbone model.
...
TL;DR: Emu3.5 is a large-scale multimodal world model that predicts next states across vision and language using unified next-token prediction, achieving strong performance in multimodal generation and world modeling tasks.


<details>
  <summary>Details</summary>
Motivation: To create a unified model that can handle interleaved vision-language inputs and outputs, enabling complex multimodal reasoning and generation capabilities.

Method: Pre-trained end-to-end with next-token prediction on 10+ trillion tokens of vision-language data, then post-trained with large-scale reinforcement learning. Uses Discrete Diffusion Adaptation (DiDA) for 20x faster inference.

Result: Achieves performance comparable to Gemini 2.5 Flash Image on generation tasks, demonstrates strong capabilities in long-horizon generation, X2I generation, text-rich image generation, and open-world embodied manipulation.

Conclusion: Emu3.5 represents a significant advancement in multimodal world modeling with efficient inference and strong performance across diverse tasks, and is open-sourced for community research.
...
TL;DR: ResMatching is a novel computational super-resolution method using guided conditional flow matching to learn improved data priors for fluorescence microscopy, achieving the best trade-off between data fidelity and perceptual realism while providing calibrated uncertainty estimates.
...
Result: ResMatching achieves competitive results on 4 diverse biological structures from BioSR dataset, outperforming 7 baselines. It shows the best trade-off between data fidelity and perceptual realism, particularly effective in noisy conditions where strong priors are hard to learn.

Conclusion: ResMatching provides an effective CSR approach with calibrated uncertainty estimates, enabling users to reject uncertain predictions. The method is particularly valuable for challenging cases with noisy low-resolution images.
...
Method: Fine-tunes Prithvi-EO-2.0-600M geospatial foundation model for continuous regression, transforming multi-temporal satellite imagery into dense pixel-level yield maps.

Result: Demonstrates superior performance over existing deep learning-based yield prediction models on Canadian Prairies dataset, providing continuous high-resolution output.

Conclusion: Validates a novel approach bridging large-scale Earth observation with on-farm decision-making, offering scalable solution for detailed agricultural monitoring.
...
Conclusion: Tokenization preserves event camera properties without sacrificing accuracy, marking a new direction in event-based vision.
...
TL;DR: PT-DETR is a novel UAV object detection algorithm based on RT-DETR that improves small object detection through PADF and MFFF modules, achieving better performance with lower complexity.
...
TL;DR: This survey provides a forward-looking analysis of object detection in autonomous vehicles, focusing on emerging AI paradigms like Vision-Language Models and Large Language Models rather than outdated techniques.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in fragmented knowledge across multimodal perception, contextual reasoning, and cooperative intelligence in autonomous vehicle object detection.

Method: Systematically reviews AV sensors and fusion strategies, categorizes AV datasets (ego-vehicle, infrastructure-based, cooperative), and analyzes cutting-edge detection methodologies including 2D/3D pipelines and transformer-driven approaches.

Result: Delivers a structured understanding of sensor capabilities, dataset characteristics, and emerging detection frameworks powered by Vision Transformers, LLMs, and VLMs.

Conclusion: Provides a clear roadmap of current capabilities, open challenges, and future opportunities in autonomous vehicle object detection, emphasizing integration with advanced AI frameworks.
...
<details>
  <summary>Details</summary>
Motivation: Accurate sea ice drift estimation is critical for Arctic navigation and climate research, but current methods using classical optical flow have limitations in complex scenarios. Deep learning approaches have shown superior performance in computer vision but remain underexplored for geophysical applications.
...
Result: Several models achieved sub-kilometer accuracy (EPE 6-8 pixels, 300-400 m), which is small relative to spatial scales of sea ice motion. Models captured consistent regional drift patterns and demonstrated substantial improvement over classical methods.
...
TL;DR: The paper presents two scene-based information fusion techniques to improve object detection robustness against occlusions in RPN-DCNN networks, showing improved recall and precision.
...
Method: Two distinct scene-based information fusion techniques: (1) selecting custom object networks based on identified background scene (pre-prediction), and (2) fusing scene knowledge into initial object scores from RPN (post-detection).

Result: Demonstrated overall improvement in both recall and precision on challenging datasets with partial occlusions. Training on combination of occluded and unoccluded images showed improvement over other training methodologies.

Conclusion: The method is interpretable, easily adaptable to other datasets, and offers many future research directions and practical applications for occlusion handling in object detection.
...
Method: Industrial cameras monitor equipment operation, alignment, and hot bar motion in real-time. Live video streams are processed on a centralized video server using deep learning models, with server-based inference to minimize computational load on PLCs. Joint analysis of sensor data and visual inputs identifies failure locations and root causes.

Result: The system enables early prediction of equipment failures and process interruptions, reduces unplanned breakdown costs, and provides actionable insights for proactive maintenance while supporting scalable deployment across production lines.

Conclusion: This integrated machine vision approach enhances operational reliability, productivity, and profitability in industrial manufacturing environments through real-time anomaly detection and failure prediction.
...
TL;DR: This paper provides a comprehensive overview of the evolution and key research directions in 3D Gaussian Splatting (3DGS) since its introduction, highlighting advances in efficiency, dynamic representations, mathematical foundations, and real-world applications.


<details>
  <summary>Details</summary>
Motivation: 3DGS has rapidly transformed 3D scene representations and inspired extensive research, motivating the need to summarize key developments and emerging directions in this rapidly evolving field.

Method: The authors present a systematic overview and analysis of follow-up research, categorizing developments into several key directions including resource-efficient training/rendering, dynamic 4D representations, mathematical foundations, mobile/VR applications, massive-scale environments, and near-instant reconstruction.

Result: The survey reveals significant progress in multiple areas: enhanced efficiency and scalability, evolution to dynamic 4DGS representations, deeper mathematical understanding, successful deployment on mobile/VR platforms, extension to massive-scale environments, and near-instant radiance field reconstruction capabilities.

Conclusion: 3DGS has evolved from a breakthrough representation into a versatile and foundational tool for 3D vision and graphics, demonstrating its broad impact and ongoing development across multiple research directions.
...
<details>
  <summary>Details</summary>
Motivation: To enable fine-grained control over VLM outputs during inference without modifying model weights, while preserving performance on other tasks and avoiding manual tuning requirements.

Method: Learns from latent embeddings of paired prompts encoding target and converse behaviors to dynamically adjust activations connecting language modality with image context through dimension-wise activation modulation and adaptive steering across layers.

Result: Outperforms existing intervention techniques on steering and hallucination mitigation benchmarks, and introduces VNIA dataset for multimodal steering evaluation.

Conclusion: Proposes a robust solution for multimodal model control through activation engineering that enables inference-time control over complex output semantics without weight modifications.
...
TL;DR: This paper presents an improved semantic segmentation framework for AMD lesion detection in RGB fundus images, which outperforms all prior submissions in the ADAM challenge benchmark.
...
Method: Used U-Net connectivity as base framework, evaluated various improvements including pre-processing techniques, different encoder backbone networks of varying complexity, and specialized loss functions to handle class imbalances.
...
Conclusion: The research successfully developed an improved AMD detection framework that achieves state-of-the-art performance on the ADAM benchmark, with source code made publicly available.
...
TL;DR: The paper introduces ChartAlign Benchmark (ChartAB) to evaluate vision-language models' chart grounding capabilities, including data extraction, element localization, and attribute recognition, with a novel two-stage inference workflow for cross-chart comparison.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack accurate perception of details and struggle to extract fine-grained structures from charts, which limits their ability to compare multiple charts and reason over them.
...
Conclusion: The findings point to specific skills that need to be strengthened in current VLMs for better chart understanding, particularly in fine-grained perception and cross-chart comparison capabilities.
...
Conclusion: Provides an adaptable, data-driven hierarchical modeling paradigm applicable to a broad range of motion-centric tasks, offering improved generalizability and interpretability.
...
TL;DR: A framework that transfers knowledge from video generation to 3D human motion generation through a large-scale dataset (ViMoGen-228K), a unified diffusion transformer model (ViMoGen), and a hierarchical benchmark (MBench) for comprehensive evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing 3D motion generation models face generalization bottlenecks, while video generation has shown remarkable generalization in modeling human behaviors, suggesting transferable insights that motion generation can leverage.

Method: 1) Created ViMoGen-228K dataset with 228K high-quality motion samples integrating MoCap data, web videos, and synthesized samples; 2) Developed ViMoGen - a flow-matching-based diffusion transformer with gated multimodal conditioning; 3) Built ViMoGen-light as a distilled variant without video dependencies; 4) Proposed MBench hierarchical benchmark for fine-grained evaluation.

Result: Extensive experiments show the framework significantly outperforms existing approaches in both automatic and human evaluations across motion quality, prompt fidelity, and generalization ability.

Conclusion: The proposed knowledge transfer framework from video to motion generation successfully addresses generalization bottlenecks in 3D human motion generation through systematic data, modeling, and evaluation improvements.
...
<details>
  <summary>Details</summary>
Motivation: Standard image retrieval fails for global geo-localization due to massive image volumes and insufficient coverage. Existing scalable methods trade accuracy for scale, with global classification being too coarse and cross-view retrieval limited to small regions.

Method: Uses proxy classification during training to learn rich feature representations encoding precise location. Combines learned prototypes with aerial imagery embeddings to handle ground-level data sparsity, enabling direct fine-grained retrieval across large geographic areas.

Result: Achieves localization within 200m for over 68% of queries on a dataset covering large parts of Europe, demonstrating effective fine-grained geo-localization at continental scale.

Conclusion: The hybrid approach successfully bridges the gap between coarse global classification and limited cross-view retrieval, enabling precise geo-localization across large geographic expanses while maintaining fine-grained accuracy.
...
TL;DR: SEE4D is a pose-free video-to-4D generation method that uses virtual cameras and view-conditional inpainting to synthesize spatiotemporal content from casual videos without 3D supervision.


<details>
  <summary>Details</summary>
Motivation: Existing video-to-4D methods require manual camera pose annotations which are labor-intensive and brittle for in-the-wild footage. Warp-then-inpaint approaches entangle camera motion with scene dynamics, complicating modeling and inference.

Method: Proposes trajectory-to-camera framework with fixed virtual cameras, separating camera control from scene modeling. Uses view-conditional video inpainting trained to denoise warped images and inpaint missing regions. Implements spatiotemporal autoregressive inference with virtual-camera splines and overlapping windows.
...
TL;DR: Masked diffusion captioning (MDC) learns visual features by reconstructing masked text tokens from images, achieving competitive performance with autoregressive and contrastive methods.


<details>
  <summary>Details</summary>
Motivation: To develop a visual feature learning method that doesn't depend on token position like autoregressive captioning, reducing the need for auxiliary objectives.

Method: Train an image-conditioned masked diffusion language model where text tokens are randomly masked and a decoder reconstructs original text using visual features.
...
Conclusion: Masked diffusion captioning provides an effective alternative for visual feature learning that reduces position dependency and auxiliary objective requirements.
...
Method: Uses a lightweight cross-modal adapter structure to reuse 2D generative priors for panoramic perception, generation, and completion tasks. Constructs a large-scale synthetic panorama dataset with multimodal panoramas.
...
Conclusion: OmniX opens new possibilities for immersive and physically realistic virtual world generation by unifying panoramic vision tasks through 2D generative model repurposing.
...
TL;DR: Video generation models like Veo-3 show promising visual reasoning capabilities but are not yet reliable as standalone zero-shot reasoners, exhibiting strengths in spatial coherence and fine-grained grounding while struggling with long-horizon causal reasoning and abstract logic.


<details>
  <summary>Details</summary>
Motivation: To investigate whether current video generation models can serve as zero-shot reasoners in challenging visual reasoning scenarios, given their demonstrated ability to produce high-fidelity, temporally coherent videos and emerging visual perception behaviors.

Method: Conducted an empirical study evaluating Veo-3's reasoning behavior across 12 dimensions (spatial, geometric, physical, temporal, embodied logic) using MME-CoF benchmark, a curated dataset for Chain-of-Frame reasoning assessment.
...
### [129] [SP-MCQA: Evaluating Intelligibility of TTS Beyond the Word Level](https://arxiv.org/abs/2510.26190)
*Hitomi Jin Ling Tee,Chaoren Wang,Zijie Zhang,Zhizheng Wu*

Main category: cs.SD

TL;DR: Proposes Spoken-Passage Multiple-Choice Question Answering (SP-MCQA) as a novel evaluation method for TTS intelligibility, revealing that low WER doesn't guarantee high key-information accuracy and exposing gaps in traditional metrics.


<details>
  <summary>Details</summary>
Motivation: Existing TTS intelligibility assessments rely too heavily on word-by-word accuracy metrics like WER, which fail to capture real-world speech complexity and human comprehension needs.
...
Result: Experiments show low WER doesn't guarantee high key-information accuracy, and even SOTA models still lack robust text normalization and phonetic accuracy.

Conclusion: There's an urgent need for high-level, life-like evaluation criteria since many systems excel at WER but fall short on real-world intelligibility.
...
### [130] [Rethinking Text-to-SQL: Dynamic Multi-turn SQL Interaction for Real-world Database Exploration](https://arxiv.org/abs/2510.26495)
...
<details>
  <summary>Details</summary>
Motivation: Existing Text-to-SQL systems perform well in static single-turn tasks but fail in real-world interactive scenarios where users iteratively refine queries based on intermediate results, especially in domains like finance and business analytics.

Method: Built through an automated two-stage pipeline: task synthesis using structured tree representations from database tables with LLM-based generation, followed by interaction-oriented filtering and expert validation. Also includes a multi-turn evaluation framework simulating realistic interactions between LLM-simulated users, test models, and executable databases.

Result: DySQL-Bench covers 13 domains across BIRD and Spider 2 databases with 1,072 tasks. Human evaluation confirmed 100% correctness of synthesized data. Even GPT-4o achieved only 58.34% overall accuracy and 23.81% on Pass@5 metric, demonstrating the benchmark's difficulty.

Conclusion: The benchmark effectively captures the challenges of dynamic SQL generation in real-world interactive scenarios and reveals significant performance gaps in current models, highlighting the need for improved multi-turn reasoning capabilities in Text-to-SQL systems.
...
### [131] [Through the Judge's Eyes: Inferred Thinking Traces Improve Reliability of LLM Raters](https://arxiv.org/abs/2510.25860)
...
<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used as raters but lack reliability for subjective tasks where human judgments involve subtle reasoning beyond simple labels. Thinking traces are informative but hard to collect.

Method: Uses rejection sampling to reconstruct thinking traces from label-only annotations, then applies these traces to fine-tune open LLM raters and synthesize clearer annotation guidelines for proprietary LLM raters.

Result: Significantly improved LLM-human agreement across multiple datasets, and refined guidelines increased agreement among different LLM models.
...
### [132] [Approximating Human Preferences Using a Multi-Judge Learned System](https://arxiv.org/abs/2510.25884)
...
<details>
  <summary>Details</summary>
Motivation: LLM-based judges are difficult to calibrate and suffer from rubric sensitivity, bias, and instability, which limits their reliability for applications like RLHF reward models and model routing systems.
...
Conclusion: The framework enables modeling diverse, persona-based preferences and provides scalable synthesis of preference labels, addressing key challenges in LLM judge alignment.
...
Result: Consistent performance gains on Qwen3-4B and Llama-3.1-8B across multi-domain evaluation suite; both stages are necessary and math-first elicitation increases cognitive behaviors for complex problem solving.

Conclusion: Reasoning Curriculum provides a compact, easy-to-adopt recipe for developing general reasoning capabilities in LLMs through domain-transferable skill development.
...
### [134] [One Model to Critique Them All: Rewarding Agentic Tool-Use via Efficient Reasoning](https://arxiv.org/abs/2510.26167)
*Renhao Li,Jianhong Tu,Yang Su,Hamid Alinejad-Rokny,Derek F. Wong,Junyang Lin,Min Yang*

Main category: cs.AI

TL;DR: ToolRM is a family of lightweight generative reward models for tool learning that achieves superior performance in function-calling tasks, outperforming frontier models and enabling efficient inference.
...
Method: Proposed a novel pipeline using rule-based scoring and multidimensional sampling to construct ToolPref-Pairwise-30K dataset, then trained lightweight generative RMs on Qwen3-4B/8B series.

Result: ToolRM achieves up to 14.28% higher accuracy than Claude 4 and OpenAI o3 in pairwise reward judgments, reduces output token usage by over 66%, and generalizes to broader critique tasks like Best-of-N sampling and self-correction.

Conclusion: ToolRM effectively addresses the gap in tool-use reward modeling, demonstrating strong performance and efficiency while enabling future research through released data and models.
...
TL;DR: This paper provides a systematic overview of context engineering, tracing its evolution from early human-computer interaction frameworks to modern AI systems, and establishes a conceptual foundation for future development.


<details>
  <summary>Details</summary>
Motivation: To address how machines can better understand human situations and purposes by examining the historical development and conceptual framework of context engineering in AI systems.

Method: The authors situate context engineering, provide a systematic definition, outline its historical evolution through distinct phases shaped by machine intelligence levels, and examine key design considerations.

Result: A comprehensive conceptual foundation for context engineering is established, tracing its development from early 1990s human-computer interaction to current human-agent interaction paradigms.

Conclusion: The paper serves as a stepping stone for systematic context engineering in AI systems, sketching its promising future development toward human-level or superhuman intelligence.
...
### [136] [Normative Reasoning in Large Language Models: A Comparative Benchmark from Logical and Modal Perspectives](https://arxiv.org/abs/2510.26606)
*Kentaro Ozeki,Risako Ando,Takanobu Morishita,Hirohiko Abe,Koji Mineshima,Mitsuhiro Okada*

Main category: cs.AI

TL;DR: LLMs show normative reasoning capabilities with inconsistencies and human-like cognitive biases, despite generally following valid reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate LLMs' reasoning capabilities in normative domains (obligation, permission) which remain underexplored compared to other reasoning tasks.

Method: Created a new dataset covering formal reasoning patterns in normative and epistemic domains, comparing LLMs' performance between these two modal types while incorporating cognitive factors from human reasoning studies.

Result: LLMs generally adhere to valid reasoning patterns but exhibit notable inconsistencies in specific normative reasoning types and display cognitive biases similar to human reasoning patterns.

Conclusion: The findings highlight challenges in achieving logical consistency in LLMs' normative reasoning and provide insights for enhancing their reliability in handling normative reasoning tasks.
...
### [137] [The Era of Agentic Organization: Learning to Organize with Language Models](https://arxiv.org/abs/2510.26658)
*Zewen Chi,Li Dong,Qingxiu Dong,Yaru Hao,Xun Wu,Shaohan Huang,Furu Wei*

Main category: cs.AI

TL;DR: AsyncThink is a new reasoning paradigm that organizes LLM thinking into concurrent structures, reducing latency by 28% while improving mathematical reasoning accuracy.


<details>
  <summary>Details</summary>
Motivation: To enable collaborative AI agents that solve complex problems beyond individual intelligence through concurrent reasoning.

Method: A thinking protocol with dynamic sub-query assignment to workers, intermediate knowledge merging, and reinforcement learning optimization of thinking structures.

Result: 28% lower inference latency than parallel thinking with improved mathematical reasoning accuracy, plus generalization to unseen tasks without additional training.

Conclusion: AsyncThink enables efficient concurrent reasoning in LLMs, advancing towards agentic organization where multiple agents collaborate to solve complex problems.
...
### [138] [Cross-Platform Evaluation of Reasoning Capabilities in Foundation Models](https://arxiv.org/abs/2510.26732)
...
<details>
  <summary>Details</summary>
Motivation: To comprehensively evaluate reasoning capabilities across different computational infrastructures and establish reproducible benchmarks for foundation model performance tracking.

Method: Three-phase experimental approach: baseline establishment on HPC supercomputing, infrastructure validation on university cluster and cloud platforms, and extended evaluation across 79 problems spanning 8 academic domains.
...
Conclusion: The tri-infrastructure methodology and 79-problem benchmark enable longitudinal tracking of reasoning capabilities as foundation models evolve, with training data quality emerging as more important than model size.
...
### [139] [ORBIT -- Open Recommendation Benchmark for Reproducible Research with Hidden Tests](https://arxiv.org/abs/2510.26095)
...
TL;DR: ORBIT is a unified benchmark for reproducible recommendation system research, featuring standardized evaluation, public datasets, and a hidden test dataset (ClueWeb-Reco) to test generalization.


<details>
  <summary>Details</summary>
Motivation: Existing recommendation system research is hindered by unrealistic datasets and inconsistent evaluation settings that lead to ambiguous conclusions.

Method: ORBIT provides a standardized evaluation framework with reproducible splits, public leaderboard, and introduces ClueWeb-Reco - a synthetic dataset derived from real user browsing data for hidden testing.

Result: Benchmark results show general improvements in recommender systems on public datasets, while hidden test results reveal limitations of existing approaches in large-scale webpage recommendation and highlight LLM integration potential.

Conclusion: ORBIT addresses reproducibility issues in recommendation system research and demonstrates the need for better generalization testing, with LLM integration showing promise for improvement.
...
### [140] [Groupwise Registration with Physics-Informed Test-Time Adaptation on Multi-parametric Cardiac MRI](https://arxiv.org/abs/2510.26022)
*Xinqi Li,Yi Zhang,Li-Ting Huang,Hsiao-Huang Chang,Thoralf Niendorf,Min-Chi Ku,Qian Tao,Hsin-Jung Yang*

Main category: eess.IV

TL;DR: A physics-informed deep learning model with test-time adaptation was developed to align multiparametric MRI maps from different physical models (T1/T2 mapping) for improved myocardial tissue characterization.


<details>
  <summary>Details</summary>
Motivation: Misalignment between multiparametric MRI maps makes pixel-wise analysis challenging for myocardial tissue characterization.

Method: Developed a generalizable physics-informed deep-learning model using test-time adaptation with synthetic images from specific physics models as registration references, enabling transductive learning for various tissue contrasts.

Result: The model was validated in healthy volunteers with various MRI sequences and demonstrated improved multi-modal registration across a wide range of image contrast variability.

Conclusion: The physics-informed deep learning approach successfully addresses the challenge of misalignment in multiparametric mapping MRI, enabling more accurate pixel-wise analysis for myocardial tissue characterization.
...
### [141] [SPG-CDENet: Spatial Prior-Guided Cross Dual Encoder Network for Multi-Organ Segmentation](https://arxiv.org/abs/2510.26390)
*Xizhi Tian,Changjun Zhou,Yulin. Yang*

Main category: eess.IV

TL;DR: SPG-CDENet is a two-stage multi-organ segmentation method that uses a spatial prior network for coarse ROI localization and a cross dual encoder network with global/local encoders, symmetric cross-attention, and flow-based decoder to improve segmentation accuracy.
...
Method: Two-stage approach: 1) Spatial prior network generates coarse localization maps for ROI guidance, 2) Cross dual encoder network with global encoder, local encoder, symmetric cross-attention module across all layers, and flow-based decoder that propagates high-level features directly.

Result: Superior performance demonstrated through extensive experiments on two public datasets compared to existing segmentation methods. Ablation studies validate effectiveness of proposed modules.

Conclusion: SPG-CDENet effectively improves multi-organ segmentation accuracy by leveraging spatial priors and enhanced feature interaction between global and local contexts through cross-attention and flow-based decoding.
...
### [142] [Comparative Analysis of Deep Learning Models for Olive Tree Crown and Shadow Segmentation Towards Biovolume Estimation](https://arxiv.org/abs/2510.26573)
*Wondimagegn Abebe Demissie,Stefano Roccella,Rudy Rossetto,Antonio Minnocci,Andrea Vannini,Luca Sebastiani*

Main category: eess.IV

TL;DR: Comparative analysis of U-Net, YOLOv11m-seg, and Mask R-CNN for olive tree crown and shadow segmentation in UAV imagery to estimate biovolume, with Mask R-CNN achieving best accuracy and YOLOv11m-seg fastest speed.
...
Result: Mask R-CNN achieved best overall accuracy (F1 = 0.86; mIoU = 0.72), YOLOv11m-seg provided fastest throughput (0.12 second per image), estimated biovolumes ranged from 4 to 24 cubic meters.

Conclusion: Mask R-CNN is preferable for accuracy-critical applications, YOLOv11m-seg for speed-critical large-area deployments, and U-Net as lightweight high-sensitivity option; framework enables scalable orchard monitoring.
...
### [143] [SAMRI: Segment Anything Model for MRI](https://arxiv.org/abs/2510.26635)
*Zhao Wang,Wei Dai,Thuy Thanh Dao,Steffen Bollmann,Hongfu Sun,Craig Engstrom,Shekhar S. Chandra*

Main category: eess.IV

TL;DR: SAMRI adapts the Segment Anything Model (SAM) for MRI segmentation through efficient fine-tuning, achieving state-of-the-art performance with 94% less training time and 96% fewer trainable parameters than full retraining.


<details>
  <summary>Details</summary>
Motivation: MRI segmentation is crucial for clinical decisions but labor-intensive manually. CNN methods struggle with MRI's variable contrast, intensity inhomogeneity, and protocol variations. While SAM shows strong generalizability in natural images, existing adaptations overlook MRI-specific challenges.
...
Result: SAMRI achieves mean Dice of 0.87 across diverse MRI segmentation tasks, with state-of-the-art accuracy across anatomical regions and robust generalization on unseen structures, especially small and clinically important ones.

Conclusion: SAM can be effectively adapted to MRI through simple decoder fine-tuning, providing efficient and accurate segmentation while maintaining strong generalization capabilities for clinical applications.
...
### [144] [BRIQA: Balanced Reweighting in Image Quality Assessment of Pediatric Brain MRI](https://arxiv.org/abs/2510.26661)
*Alya Almsouti,Ainur Khamitova,Darya Taratynova,Mohammad Yaqub*

Main category: eess.IV

TL;DR: BRIQA is a method for automated artifact severity assessment in pediatric brain MRI that addresses class imbalance through gradient-based loss reweighting and rotating batching, improving average macro F1 score from 0.659 to 0.706.
...
Result: BRIQA improves average macro F1 score from 0.659 to 0.706, with notable gains across multiple artifact types: Noise (0.430), Zipper (0.098), Positioning (0.097), Contrast (0.217), Motion (0.022), and Banding (0.012).
...
### [145] [ProstNFound+: A Prospective Study using Medical Foundation Models for Prostate Cancer Detection](https://arxiv.org/abs/2510.26703)
...
Result: The model shows strong generalization to prospective data with no performance degradation compared to retrospective evaluation, aligning closely with clinical scores (PRI-MUS and PI-RADS) and producing interpretable heatmaps consistent with biopsy-confirmed lesions.
...
### [146] [MORE: Multi-Organ Medical Image REconstruction Dataset](https://arxiv.org/abs/2510.26759)
...
Conclusion: The MORE dataset enables robust training and rigorous evaluation of CT reconstruction models, demonstrating improved generalization through diverse data and optimization approaches.
...
### [147] [StructLayoutFormer:Conditional Structured Layout Generation via Structure Serialization and Disentanglement](https://arxiv.org/abs/2510.26141)
*Xin Hu,Pengfei Xu,Jin Zhou,Hongbo Fu,Hui Huang*

Main category: cs.GR

TL;DR: StructLayoutFormer is a Transformer-based approach for conditional structured layout generation that disentangles structural information from element placements to produce realistic layout structures explicitly.


<details>
  <summary>Details</summary>
Motivation: Structured layouts are preferable for 2D visual content like GUIs and webpages, but existing data-driven approaches fail to produce layout structures and require heavy labor input for computational frameworks.

Method: Uses a structure serialization scheme to represent structured layouts as sequences and disentangles structural information from element placements using a Transformer-based architecture.

Result: Outperforms existing data-driven layout generation approaches with post-processing for structure extraction, and effectively extracts and transfers layout structures.

Conclusion: First data-driven approach that achieves conditional structured layout generation and produces realistic layout structures explicitly, with code publicly available.
...
### [148] [DARTS: A Drone-Based AI-Powered Real-Time Traffic Incident Detection System](https://arxiv.org/abs/2510.26004)
*Bai Li,Achilleas Kourtellis,Rong Cao,Joseph Post,Brian Porter,Yu Zhang*

Main category: cs.RO

TL;DR: DARTS is a drone-based AI system for real-time traffic incident detection that uses thermal imaging and deep learning to achieve 99% accuracy, detecting incidents 12 minutes faster than conventional methods while monitoring congestion propagation.
...
Result: Achieved 99% detection accuracy on self-collected dataset, detected rear-end collision 12 minutes earlier than local transportation center, and successfully monitored incident-induced congestion propagation in field test.
...
### [149] [Self-localization on a 3D map by fusing global and local features from a monocular camera](https://arxiv.org/abs/2510.26170)
*Satoshi Kikuch,Masaya Kato,Tsuyoshi Tasaki*

Main category: cs.RO

TL;DR: Proposes a hybrid CNN-Vision Transformer method for 3D self-localization that improves accuracy in dynamic obstacle scenarios compared to state-of-the-art approaches.
...
Method: Combines CNN (for local features) with Vision Transformer (for global features from image patches) to create a hybrid approach for 3D self-localization.

Result: Achieved 1.5x higher accuracy improvement rate in CG datasets with dynamic obstacles, 20.1% smaller localization error than SOTA on public datasets, and average error of 7.51cm in robot experiments.

Conclusion: The CNN-Vision Transformer hybrid approach significantly improves self-localization accuracy, especially in challenging scenarios with dynamic obstacles.
...
### [150] [AgriGS-SLAM: Orchard Mapping Across Seasons via Multi-View Gaussian Splatting SLAM](https://arxiv.org/abs/2510.26358)
*Mirko Usuelli,David Rapado-Rincon,Gert Kootstra,Matteo Matteucci*

Main category: cs.RO

TL;DR: AgriGS-SLAM is a Visual-LiDAR SLAM framework for orchard robotics that combines LiDAR odometry with multi-camera 3D Gaussian Splatting to achieve robust real-time 3D scene understanding across seasonal changes and occlusions.


<details>
  <summary>Details</summary>
Motivation: Autonomous robots in orchards need real-time 3D scene understanding despite challenges like repetitive row geometry, seasonal appearance changes, wind-driven foliage motion, and occlusions.

Method: Couples direct LiDAR odometry and loop closures with multi-camera 3D Gaussian Splatting rendering. Uses batch rasterization across viewpoints to recover structure under occlusions, unified gradient-driven map lifecycle between keyframes, and pose refinement guided by probabilistic LiDAR-based depth consistency.

Result: Delivers sharper, more stable reconstructions and steadier trajectories than state-of-the-art 3DGS-SLAM baselines across seasons and sites while maintaining real-time performance on-tractor. Successfully deployed in apple and pear orchards across dormancy, flowering, and harvesting.
...
### [151] [MemEIC: A Step Toward Continual and Compositional Knowledge Editing](https://arxiv.org/abs/2510.25798)
...
<details>
  <summary>Details</summary>
Motivation: Current knowledge editing methods focus on single modalities in isolation, neglecting the multimodal nature of LVLMs and the need for continuous knowledge updates, leading to suboptimal outcomes.
...
Conclusion: The proposed MemEIC method successfully addresses the limitations of single-modality editing by enabling continual and compositional knowledge editing across both visual and textual modalities in LVLMs.
...
### [152] [Metis-SPECS: Decoupling Multimodal Learning via Self-distilled Preference-based Cold Start](https://arxiv.org/abs/2510.25801)
*Kun Chen,Peng Shi,Haibo Qiu,Zhixiong Zeng,Siqi Yang,Wenji Mao,Lin Ma*

Main category: cs.LG

TL;DR: SPECS is a self-distilled, preference-based cold start framework that decouples multimodal learning into surface-form training and deep reasoning, outperforming SFT-based approaches and improving generalization.
...
Method: SPECS framework: (1) generates introspective preference data pairs via self-distillation, (2) performs preference-based training on shallow surface-form criteria, and (3) hands off to RL for deep reasoning.
...
Conclusion: Preference-based cold start methods generalize better than SFT-based approaches, and the decoupled learning framework of SPECS provides significant performance improvements in multimodal reinforcement learning.
...
### [153] [Do Students Debias Like Teachers? On the Distillability of Bias Mitigation Methods](https://arxiv.org/abs/2510.26038)
...
Result: KD undermines debiasing capabilities; debiased models don't benefit from teacher knowledge; robustness varies across bias types; specific internal patterns identified.
...
### [154] [CorVS: Person Identification via Video Trajectory-Sensor Correspondence in a Real-World Warehouse](https://arxiv.org/abs/2510.26369)
*Kazuma Kano,Yuki Mori,Shin Katayama,Kenta Urano,Takuro Yonezawa,Nobuo Kawaguchi*

Main category: cs.LG

TL;DR: CorVS is a novel person identification method that matches visual tracking trajectories with wearable sensor measurements using deep learning to predict correspondence probabilities, enabling reliable worker identification in industrial settings.


<details>
  <summary>Details</summary>
Motivation: Worker location data is crucial for productivity in industrial sites. While cameras offer valuable environmental context, visual-only identification is impractical. Existing methods combining trajectories and sensor data often fail under real-world conditions.

Method: A two-step approach: 1) Deep learning model predicts correspondence probabilities and reliabilities for trajectory-sensor measurement pairs, 2) Algorithm matches trajectories and sensor measurements over time using these predictions.

Result: The method was validated using a dataset from actual warehouse operations, demonstrating effectiveness for real-world applications.

Conclusion: CorVS successfully addresses the limitations of existing person identification methods in industrial environments by combining visual tracking and sensor data through a data-driven approach.
...
### [155] [Deep sequence models tend to memorize geometrically; it is unclear why](https://arxiv.org/abs/2510.26745)
*Shahriar Noroozizadeh,Vaishnavh Nagarajan,Elan Rosenfeld,Sanjiv Kumar*

Main category: cs.LG

TL;DR: Transformers develop geometric representations of entities rather than just storing local co-occurrences, enabling efficient reasoning through geometric relationships rather than brute-force lookup.


<details>
  <summary>Details</summary>
Motivation: To challenge the associative view of memory in sequence modeling and demonstrate that Transformers synthesize geometric representations that encode global relationships between entities, including non-co-occurring ones.

Method: Analyzed Transformer reasoning instances incompatible with co-occurrence-based memory, connected findings to Node2Vec, and identified spectral bias as the source of geometric representations.

Result: Transformers naturally develop elegant geometric representations of entities despite optimizing only over local associations, and this geometry simplifies complex reasoning tasks into easier geometric operations.

Conclusion: The geometric view of parametric memory should replace default associative intuitions, revealing headroom for making Transformer memory more geometric and impacting areas like knowledge acquisition, capacity, discovery and unlearning.
...
### [156] [Remote Labor Index: Measuring AI Automation of Remote Work](https://arxiv.org/abs/2510.26787)
...
<details>
  <summary>Details</summary>
Motivation: To measure how AI research gains translate into actual economic value and practical automation, bridging the gap between research benchmarks and real-world applications.

Method: Introduced the Remote Labor Index (RLI) - a multi-sector benchmark of real-world, economically valuable projects to evaluate end-to-end AI agent performance in practical settings.

Result: AI agents performed near the floor on RLI, with the highest-performing agent achieving only 2.5% automation rate.

Conclusion: The results provide empirical evidence to ground discussions of AI automation, establishing a common basis for tracking AI impacts and helping stakeholders navigate AI-driven labor automation.
...
### [157] [Defeating the Training-Inference Mismatch via FP16](https://arxiv.org/abs/2510.26788)
*Penghui Qi,Zichen Liu,Xiangxin Zhou,Tianyu Pang,Chao Du,Wee Sun Lee,Min Lin*

Main category: cs.LG

TL;DR: Using FP16 instead of BF16 in RL fine-tuning of LLMs eliminates numerical mismatch between training and inference, improving stability, convergence, and performance.


<details>
  <summary>Details</summary>
Motivation: RL fine-tuning of LLMs suffers from instability due to numerical mismatch between training and inference policies, which prior work tried to address through algorithmic or engineering solutions.

Method: Simply revert from BF16 to FP16 precision, requiring only a few lines of code change with no modifications to model architecture or learning algorithm.
...
Conclusion: FP16 should be preferred over BF16 for RL fine-tuning of LLMs due to better numerical consistency and performance.
...
TL;DR: GRWM improves world models by enforcing geometric regularization in latent space, leading to more accurate long-horizon predictions in deterministic 3D environments.


<details>
  <summary>Details</summary>
Motivation: Current world models degrade over long horizons due to poor representation quality from lossy or entangled latents, making dynamics learning unnecessarily difficult.

Method: Proposes Geometrically-Regularized World Models (GRWM) that enforce consecutive points along sensory trajectories to remain close in latent space, improving latent manifold structure.

Result: GRWM significantly increases rollout fidelity and stability across deterministic 3D settings and long-horizon prediction tasks, learning superior geometric structure in latent space.

Conclusion: Improving representation learning through geometric regularization is a direct and effective path to building robust world models with reliable long-horizon predictions.
...
### [159] [Nexus: Execution-Grounded Multi-Agent Test Oracle Synthesis](https://arxiv.org/abs/2510.26423)
*Dong Huang,Mingzhe Du,Jie M. Zhang,Zheng Lin,Meng Luo,Qianru Zhang,See-Kiong Ng*

Main category: cs.SE

TL;DR: Nexus is a multi-agent framework that generates test oracles through collaborative deliberation, validation, and self-refinement, significantly outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Test oracle generation in non-regression testing is a longstanding challenge in software engineering, where the goal is to produce oracles that can accurately determine whether a function under test behaves as intended.

Method: Uses a multi-agent framework with four specialist agents embodying distinct testing philosophies. The process involves deliberation (collaborative critique and refinement), validation (execution in secure sandbox), and self-refinement (debugging based on runtime errors).

Result: Nexus substantially outperforms state-of-the-art baselines: improves test-level oracle accuracy on LiveCodeBench from 46.30% to 57.73% for GPT-4.1-Mini, increases bug detection rate on HumanEval from 90.91% to 95.45%, and improves automated program repair success rate from 35.23% to 69.32%.

Conclusion: Nexus demonstrates that multi-agent collaboration with structured deliberation, validation, and self-refinement can effectively address the challenge of test oracle generation in non-regression testing.
...
### [160] [SecureReviewer: Enhancing Large Language Models for Secure Code Review through Secure-aware Fine-tuning](https://arxiv.org/abs/2510.26457)
*Fang Liu,Simiao Liu,Yinghao Zhu,Xiaoli Lian,Li Zhang*

Main category: cs.SE

TL;DR: SecureReviewer is a new LLM-based approach for automated security-focused code review that addresses data scarcity and evaluation challenges through a tailored dataset, secure-aware fine-tuning, RAG integration, and a new SecureBLEU metric.


<details>
  <summary>Details</summary>
Motivation: Existing automated code review approaches primarily focus on general-purpose reviews and are ineffective at identifying security issues. There are challenges including data scarcity and inadequate evaluation metrics for security-focused code review.

Method: Constructed a security-focused dataset, fine-tuned LLMs with secure-aware strategy, integrated RAG to mitigate hallucinations and ground outputs in security knowledge, and introduced SecureBLEU evaluation metric.

Result: SecureReviewer outperforms state-of-the-art baselines in security issue detection accuracy and overall quality/practical utility of generated review comments.
...
Method: Uses a three-stage training curriculum with baseline fine-tuning, domain-adaptive training enhanced with focal loss, adversarial augmentation, and post-training quantization. Implements local computation through Chromium messaging API for DOM scraping, tokenization, Transformer inference, and UI rendering.

Result: DistilBERT-Quant model (67.6 MB) achieves 97.1% macro-F1, 97.4% accuracy, and 0.996 AUROC with 103 ms median latency. TinyBERT-Quant variant (14.7 MB) maintains 95.7% macro-F1 and 96.1% accuracy with 40 ms latency.

Conclusion: High-quality fake news detection is feasible under tight resource constraints with minimal performance loss, providing a privacy-preserving tool for policymakers and enabling large-scale data collection for research.
...
TL;DR: SIRAJ is a red-teaming framework for LLM agents that uses dynamic test case generation and iterative adversarial attacks to discover safety vulnerabilities, with model distillation to reduce costs while maintaining effectiveness.
...
Method: Two-step process: 1) Generate diverse seed test cases covering risk outcomes and tool trajectories, 2) Iteratively construct adversarial attacks using execution trajectories. Uses model distillation to train smaller but equally effective red-teamer models.

Result: Seed generation provides 2-2.5x boost in risk coverage. Distilled 8B model achieves 100% higher attack success rate than 671B Deepseek-R1 model. Framework demonstrates effective iterative attacks and generalization.

Conclusion: SIRAJ provides an effective, cost-optimized red-teaming framework that successfully identifies LLM agent vulnerabilities through systematic test generation and distilled attack models.
...
TL;DR: PVMark is a zero-knowledge proof based plugin that enables publicly verifiable watermark detection for LLMs without disclosing secret keys, resolving the trust issue in current watermarking schemes.


<details>
  <summary>Details</summary>
Motivation: Current LLM watermarking schemes face a trust dilemma: private detection keys make verification opaque, while public keys enable removal attacks. There's a need for publicly verifiable detection without compromising security.

Method: PVMark uses zero-knowledge proofs to create verifiable constraints for watermark detection execution, including mapping, random number generation, comparison, and summation operations. It implements multiple variants covering different watermarking schemes, hash functions, and ZKP protocols.

Result: Experimental results show PVMark efficiently enables public verifiability on state-of-the-art LLM watermarking schemes without compromising watermarking performance, making it practical for deployment.

Conclusion: PVMark successfully resolves the trust issue in LLM watermarking by providing publicly verifiable detection through zero-knowledge proofs, promising practical deployment while maintaining security.
