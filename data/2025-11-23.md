...
TL;DR: Analysis of Chain-of-Thought reasoning attribution patterns in multilingual LLMs reveals limitations in faithfulness and interpretability across languages, with attribution biases toward final steps and reduced effectiveness for non-Latin-script languages.


<details>
  <summary>Details</summary>
Motivation: To assess the faithfulness and interpretability of Chain-of-Thought reasoning in multilingual LLMs, addressing concerns about whether generated reasoning chains truly reflect the model's decision process across different languages.

Method: Applied ContextCite (step-level attribution) and Inseq (token-level attribution) methods to Qwen2.5 1.5B-Instruct model using MGSM benchmark, with controlled perturbations via negation and distractor sentences.
...
Conclusion: Chain-of-Thought prompting has significant limitations in multilingual robustness and interpretive transparency, with attribution patterns revealing faithfulness issues across different language contexts.
...
<details>
  <summary>Details</summary>
Motivation: Existing ToM benchmarks focus mainly on false-belief tasks and overlook other mental states and nonverbal communication, which are fundamental to human social interaction and survival.

Method: Created Motion2Mind framework using expert-curated body-language references as knowledge base, building a curated video dataset with fine-grained nonverbal cue annotations (222 types) and manually verified psychological interpretations (397 mind states).
...
Conclusion: The Motion2Mind framework highlights significant limitations in current AI's ability to interpret nonverbal communication, indicating the need for improved ToM capabilities in machines.
...
TL;DR: TOD-ProcBench is a new benchmark for evaluating LLMs' ability to follow complex process instructions in task-oriented dialogues, featuring intricate constraints and three evaluation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing TOD benchmarks oversimplify complex instructions by reducing them to simple schemas, failing to capture real-world complexity where agents must strictly adhere to detailed natural language guidelines and procedures.

Method: Created benchmark dataset from ABCD dataset with human quality control, formulated constraints as multi-level condition-action statements, and designed three tasks: relevant statement retrieval, instruction-violation detection, and conditional response generation.

Result: The benchmark systematically evaluates LLMs' instruction-following capabilities in multi-turn TODs with complex constraints, including studies on multilingual settings and instruction text formats.

Conclusion: TOD-ProcBench addresses the gap in evaluating complex instruction-following in TODs and provides comprehensive benchmarking for LLMs' capabilities in real-world dialogue scenarios.
...
Method: Created LIARS' BENCH testbed with examples varying along two dimensions: model's reason for lying and object of belief targeted. Evaluated three black- and white-box lie detection techniques on this benchmark.

Result: Existing techniques systematically fail to identify certain types of lies, especially when it's impossible to determine lying from the transcript alone.

Conclusion: LIARS' BENCH reveals limitations in prior lie detection techniques and provides a practical testbed to guide progress in detecting when LLMs lie.
...
TL;DR: LTLA is a hybrid approach that combines autoregressive language models with tractable surrogate models to handle sequence-level constraints in controlled text generation, improving constraint satisfaction with minimal inference overhead.


<details>
  <summary>Details</summary>
Motivation: Existing methods for controlled language generation use tractable surrogates like HMMs to approximate continuation distributions, but these are often weakly context aware, reducing query quality. There's a need for better approaches that maintain rich context while handling constraints efficiently.
...
Result: LTLA attains higher conditional likelihood than unconditional HMMs, approximates continuation distributions for vision-language models where standalone HMMs cannot encode visual context, and improves constraint satisfaction at comparable fluency on controlled-generation tasks.

Conclusion: The proposed LTLA approach effectively addresses efficiency pitfalls in controlled language generation by combining the strengths of neural language models and tractable surrogate models, achieving better constraint handling with minimal computational overhead.
...
TL;DR: GPT-5 demonstrates practical utility in scientific research across multiple disciplines, producing new mathematical results and accelerating research workflows while still requiring human oversight.


<details>
  <summary>Details</summary>
Motivation: To showcase the real-world capabilities of frontier AI like GPT-5 in scientific research and provide concrete examples of AI-human collaboration across various scientific domains.

Method: Collection of case studies documenting interactions between human researchers and GPT-5 across mathematics, physics, astronomy, computer science, biology, and materials science, with detailed analysis of where AI succeeded and where human input remained essential.

Result: GPT-5 produced new, concrete research steps and four verified new mathematical results, demonstrating acceleration of scientific work while highlighting the continued importance of human expertise in the research process.

Conclusion: Frontier AI like GPT-5 can meaningfully contribute to scientific discovery and accelerate research, but human oversight remains crucial for verification and guidance, with current contributions being modest but promising given rapid AI advancement.
...
TL;DR: ELPO is an ensemble learning-based prompt optimization framework that uses voting mechanisms and multiple search methods to generate superior prompts, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Manual prompt engineering is laborious and existing automatic prompt optimization methods are limited by single-model approaches that struggle with complex tasks.

Method: Proposes ELPO framework with ensemble learning, voting mechanism, shared generation strategies, and different search methods for prompt optimization.
...
Conclusion: Ensemble learning approach in prompt optimization provides more accurate and robust results compared to single-algorithm methods.
...
TL;DR: TS-PEFT introduces selective parameter fine-tuning on specific position indices instead of applying PEFT modifications to all indices, showing better performance than traditional PEFT approaches.


<details>
  <summary>Details</summary>
Motivation: To challenge the necessity of applying Parameter-Efficient Fine-Tuning (PEFT) modifications to all position indices in large models, questioning whether this indiscriminate approach is optimal for downstream task performance.

Method: Proposes Token-Selective PEFT (TS-PEFT) where a function S selectively applies PEFT modifications only to a subset of position indices rather than all indices.

Result: Experimental results show that indiscriminate PEFT application to all indices is not only unnecessary but can be counterproductive, while selective application improves performance.

Conclusion: The paper advocates for a more targeted approach to PEFT modifications and provides a framework for optimizing fine-tuning processes in large models through selective parameter updates.
...
TL;DR: SemanticCite is an AI-powered system that verifies citation accuracy through full-text source analysis, providing contextual explanations and classifying claim-source relationships into four categories.
...
Conclusion: SemanticCite provides a scalable solution for research integrity through citation verification, peer review streamlining, and AI-generated content quality control, with open-source framework and dataset contributions.
...
TL;DR: SeSE is a new uncertainty quantification framework that uses semantic structural information to detect hallucinations in LLMs, outperforming existing methods.
...
Method: Developed adaptively sparsified directed semantic graphs to capture semantic dependencies, then defined Semantic Structural Entropy as the structural entropy of optimal semantic encoding trees after hierarchical abstraction.
...
Conclusion: SeSE provides a principled framework for quantifying semantic uncertainty from structural information, enabling more reliable hallucination detection in LLMs.
...
TL;DR: SDA is a training-free alignment framework that dynamically adjusts LLM output probabilities using user instructions to improve alignment with human intent across helpfulness, harmlessness, and honesty dimensions.


<details>
  <summary>Details</summary>
Motivation: As LLMs are deployed more widely, ensuring their responses align with human intent without costly retraining or extensive supervision is a critical challenge that needs efficient solutions.

Method: Proposes SDA (Steering-Driven Distribution Alignment), a training-free and model-agnostic framework that redistributes model output probabilities based on user-defined alignment instructions during inference.

Result: SDA consistently improves alignment across 8 open-source LLMs, achieving average gains of 64.4% in helpfulness, 30% in honesty, and 11.5% in harmlessness across diverse models and scenarios.

Conclusion: SDA provides an effective, lightweight, and resource-efficient solution for aligning LLM behavior with human intent without fine-tuning, supporting both general and personalized preference alignment.
...
<details>
  <summary>Details</summary>
Motivation: Traditional reinforcement learning with outcome correctness rewards only focuses on final answers, lacking supervision over internal reasoning processes, which leads to suboptimal reasoning quality including over-thinking, under-thinking, redundant-thinking, and disordered-thinking.

Method: Proposes self-rewriting framework where models rewrite their own reasoning texts and learn from the rewritten versions. Uses selective rewriting on "simple" samples (consistently correct ones) to preserve original reward signals. Implements rewriting and vanilla generation in single batches with ~10% overhead.
...
Conclusion: Self-rewriting framework effectively improves internal reasoning quality in large reasoning models while maintaining efficiency, addressing limitations of traditional outcome-based rewards.
...
<details>
  <summary>Details</summary>
Motivation: Idioms and figurative language remain difficult for LLMs despite large corpora, requiring specialized datasets to improve performance through finetuning approaches.

Method: Created three datasets: one large-scale dataset of potential idiomatic expressions and two human-annotated datasets of definite idiomatic expressions, using combined idiom lists from existing datasets and retrieving context sequences from large corpora.

Result: The datasets were post-processed for model compatibility and used to train and evaluate models on slot labeling and sequence tagging tasks for idiom recognition.

Conclusion: The presented datasets provide a foundation for developing better models and approaches to handle figurative language, helping narrow the performance gap in LLM understanding of idioms.
...
TL;DR: The paper analyzes the limitations of sufficiency metric for rationales and explores relationships between rationale informativeness, token classification, and model performance across different modeling paradigms.


<details>
  <summary>Details</summary>
Motivation: To understand the limitations of sufficiency metric in assessing rationale informativeness and investigate how rationale information affects model performance through different modeling approaches.

Method: Relates sufficiency to token classification and attention regularization paradigms, analyzing how rationales impact model performance across domains and model types.

Result: Highly informative rationales don't necessarily help correct classification; sufficiency captures interference from non-rationalized context; rationale incorporation boosts cross-domain classification inconsistently; sufficiency and token classification are unrelated.

Conclusion: Rationales are complex and current metrics like sufficiency have limitations, highlighting the need for more systematic metrics to capture rationale information effectively.
...
TL;DR: MinerU-HTML is a novel HTML-to-text extraction pipeline using a 0.6B-parameter language model that significantly outperforms heuristic methods like Trafilatura, achieving 81.8% ROUGE-N F1 vs 63.6% and better preserving structured elements. The resulting AICC corpus improves model performance in pretraining experiments.


<details>
  <summary>Details</summary>
Motivation: Current web data curation focuses on filtering and deduplication while treating HTML extraction as fixed preprocessing. Heuristic extractors like Trafilatura struggle to preserve document structure and corrupt elements like formulas, codes, and tables.

Method: Reformulates content extraction as sequence labeling using a 0.6B-parameter language model. Uses two-stage formatting pipeline that categorizes semantic elements before converting to Markdown, leveraging semantic understanding rather than text-density heuristics.

Result: On MainWebBench (7,887 pages), achieves 81.8% ROUGE-N F1 vs Trafilatura's 63.6%, with 90.9% code block and 94.0% formula preservation. AICC corpus (7.3T tokens) trains models achieving 50.8% average accuracy across 13 benchmarks, outperforming Trafilatura-extracted corpus by 1.08pp.

Conclusion: HTML extraction quality significantly impacts model capabilities and is a critical, often underestimated component of web corpus construction. Model-based approaches like MinerU-HTML are inherently scalable compared to heuristic methods.
...
<details>
  <summary>Details</summary>
Motivation: To determine if machine learning and deep learning models can effectively differentiate between perceived lower-quality and higher-quality news articles.

Method: Used 3 machine learning classifiers and 3 deep learning models on a dataset of 1,412,272 English news articles from Common Crawl (2018-2024). Expert consensus ratings on 579 source websites were split at median to create low/high-quality classes, with 194 linguistic features per article.

Result: Random Forest achieved 0.7355 accuracy and 0.8131 ROC AUC. ModernBERT-large (256 context) performed best with 0.8744 accuracy, 0.9593 ROC-AUC, and 0.8739 F1. Other deep learning models also showed strong performance.

Conclusion: Both traditional CPU-based machine learning classifiers and deep learning classifiers can effectively differentiate the perceived quality of worldwide news articles.
...
TL;DR: ESGBench is a benchmark for evaluating explainable ESG question answering systems using corporate sustainability reports, with domain-grounded questions and human-curated answers.


<details>
  <summary>Details</summary>
Motivation: To assess and improve explainable ESG question answering systems, addressing challenges in factual consistency, traceability, and domain alignment in sustainability reporting.

Method: Created a benchmark dataset with domain-grounded ESG questions paired with human-curated answers and supporting evidence for fine-grained evaluation of model reasoning.

Result: Analysis of state-of-the-art LLMs on ESGBench revealed key challenges in factual consistency, traceability, and domain alignment in ESG question answering.
...
Method: Used a modified path patching algorithm for circuit discovery and analysis, identifying specialized attention heads ("Idiom Heads") and analyzing attention patterns between idiom tokens.

Result: Found distinct computational patterns for idiom processing, identified specialized "Idiom Heads" that frequently activate across different idioms, and discovered "augmented reception" - enhanced attention between idiom tokens due to earlier processing.

Conclusion: The findings reveal mechanisms by which transformers balance computational efficiency and robustness when processing non-compositional language, providing insights for understanding more complex grammatical constructions.
...
TL;DR: Arctic-Extract is a lightweight (6.6 GiB) state-of-the-art model for extracting structural data from business documents, deployable on resource-constrained hardware like A10 GPUs.


<details>
  <summary>Details</summary>
Motivation: To create an efficient document understanding model that can handle structural data extraction from both scanned and digital business documents while being deployable on hardware with limited resources.

Method: Developed training protocols for Arctic-Extract model, focusing on efficient architecture that maintains SoTA performance while being lightweight (6.6 GiB).

Result: The model can process up to 125 A4 pages on A10 GPUs with 24GB memory, demonstrating strong performance in document understanding tasks including question answering, entity and table extraction.

Conclusion: Arctic-Extract successfully balances high performance with resource efficiency, making advanced document understanding accessible on constrained hardware environments.
...
<details>
  <summary>Details</summary>
Motivation: Neural IR systems are underexplored for morphologically rich, lower-resource languages like Turkish, and late-interaction models have not been systematically evaluated for Turkish retrieval despite their potential benefits.

Method: Two-stage adaptation pipeline: fine-tuning English/multilingual encoders on Turkish NLI/STS tasks, then converting them into ColBERT-style retrievers using PyLate trained on MS MARCO-TR. Evaluated 10 models across five Turkish BEIR datasets.

Result: Late-interaction models 3-5× smaller than dense encoders significantly outperform them; ColmmBERT-base-TR yields up to +13.8% mAP on domain-specific tasks. MUVERA+Rerank is 3.33× faster than PLAID with +1.7% mAP gain, achieving 0.54 ms query times.

Conclusion: Late-interaction models offer strong parameter efficiency and performance advantages for Turkish IR, with colbert-hash-nano-tr preserving 71% of turkish-e5-large's performance while being 600× smaller, enabling production-ready low-latency retrieval.
...
TL;DR: This paper presents a framework for predicting text genre from LLM activations using shallow learning models, achieving high accuracy with Mistral-7B.
...
Method: Used Mistral-7B and two datasets, employing scikit-learn classifiers to predict text genre based on LLM activations.

Result: Achieved F1-scores of up to 98% and 71% across datasets, consistently outperforming control tasks.

Conclusion: Proof of concept that text genres can be reliably inferred from LLM activations using shallow learning models.
...
TL;DR: This paper challenges the use of Word Error Rate (WER) for evaluating ASR in clinical dialogue, showing poor correlation with clinical impact. It introduces an LLM-as-a-Judge framework that achieves human-comparable performance in assessing clinical safety.


<details>
  <summary>Details</summary>
Motivation: Standard ASR evaluations rely heavily on WER, but this may not reflect the actual clinical impact of transcription errors in doctor-patient dialogues, potentially compromising patient safety.

Method: Established gold-standard benchmark with expert clinicians labeling clinical impact of ASR errors. Developed LLM-as-a-Judge framework optimized using GEPA to replicate expert clinical assessment.

Result: WER and existing metrics correlate poorly with clinician-assigned risk labels. The optimized judge (Gemini-2.5-Pro) achieved 90% accuracy and strong Cohen's κ of 0.816, comparable to human performance.

Conclusion: Provides a validated, automated framework for moving ASR evaluation beyond textual fidelity to scalable assessment of clinical safety, addressing the gap between transcription accuracy and clinical impact.
...
TL;DR: A method for word sense disambiguation using LLMs as oracles without hand-annotated training data, converting symbolic meanings to natural language alternatives for LLM selection.


<details>
  <summary>Details</summary>
Motivation: Current WSD methods require hand-annotated data and focus on coarse-grained representations, limiting automatic disambiguation of richer representations needed for sophisticated inference.
...
Conclusion: Proposed approach enables automatic disambiguation of richer representations without hand-annotation, using LLMs as oracles for context-aware meaning selection.
...
TL;DR: Direct multimodal embedding retrieval outperforms text-based approaches in multimodal RAG systems by preserving visual context and reducing information loss from LLM summarization.
...
Result: Direct multimodal embedding retrieval significantly outperforms LLM-summary-based approaches with 13% absolute improvement in mAP@5 and 11% in nDCG@5, plus more accurate and factually consistent answers.
...
TL;DR: Nemotron Elastic is a framework that embeds multiple nested submodels within a single parent LLM, enabling zero-shot extraction of different-sized models without additional training, achieving 360x cost reduction compared to training from scratch.


<details>
  <summary>Details</summary>
Motivation: Training separate LLMs for different scales and deployment objectives is prohibitively expensive, requiring multiple training runs and significant computational resources.

Method: Uses hybrid Mamba-Attention architectures with end-to-end trained router, two-stage training curriculum, group-aware SSM elastification, heterogeneous MLP elastification, normalized MSE-based layer importance, and knowledge distillation.

Result: Applied to Nemotron Nano V2 12B model, produced 9B and 6B models with only 110B training tokens, achieving 360x cost reduction vs training from scratch and 7x vs state-of-the-art compression. All nested models perform on par or better than SoTA in accuracy.

Conclusion: Nemotron Elastic enables efficient multi-scale LLM deployment with constant memory footprint, outperforming existing compression methods while maintaining accuracy across all model sizes.
...
Conclusion: The framework effectively reduces semantic gaps and learns complex tasks from limited data, providing a universal solution for flexible virtual try-on applications.
...
TL;DR: EfficientSAM3 enables on-device concept segmentation and tracking by distilling SAM3's capabilities into lightweight models through Progressive Hierarchical Distillation.


<details>
  <summary>Details</summary>
Motivation: SAM3's unified architecture is too computationally expensive for on-device applications, creating a need for efficient alternatives that maintain similar performance.

Method: Progressive Hierarchical Distillation in three stages: Encoder Distillation, Temporal Memory Distillation with Perceiver-based module, and End-to-End Fine-Tuning using various lightweight backbones.

Result: Achieves strong performance-efficiency trade-offs on VOS datasets while maintaining high fidelity to the original SAM3 teacher model.

Conclusion: EfficientSAM3 successfully enables on-device concept segmentation and tracking through efficient distillation while preserving concept-level performance.
...
TL;DR: Proposes four novel extensions to model-based 6D pose estimation: dynamic non-uniform sampling, multi-hypothesis inference, iterative refinement, and occlusion-focused training, achieving significant accuracy improvements and faster inference.


<details>
  <summary>Details</summary>
Motivation: Existing multi-stage pose estimation pipelines are prone to error propagation under occlusion, degrading performance for unseen objects where CAD models are available but early detection/segmentation stages fail.

Method: Four key extensions: (1) dynamic non-uniform dense sampling focusing on visible regions, (2) multi-hypothesis inference with confidence-ranked candidates, (3) iterative pose refinement, and (4) occlusion-focused training augmentations. Also introduces weighted by visibility evaluation metric.

Result: Achieves >5% accuracy improvement on ICBIN and >2% on BOP benchmarks, with approximately 3x faster inference speed compared to existing methods.

Conclusion: The proposed approach effectively addresses occlusion challenges in 6D pose estimation through computational efficiency and robustness improvements, demonstrating significant performance gains across standard benchmarks.
...
TL;DR: The paper addresses the challenge of limited annotated training data for historical map analysis by proposing methods to generate synthetic historical maps that maintain realism and diversity.


<details>
  <summary>Details</summary>
Motivation: Deep learning methods for historical document analysis require large annotated datasets, which are scarce for historical maps due to the time-consuming manual annotation process and the domain-specific nature of map corpora.

Method: The authors propose two approaches: (1) an automatic deep generative method, and (2) a manual stochastic degradation technique to transfer cartographic style from original historical maps onto vector data, simulating visual uncertainty and noise typical in historical map scans.

Result: The generated synthetic training datasets were successfully used for domain-adaptive semantic segmentation on a homogeneous map corpus using a Self-Constructing Graph Convolutional Network, demonstrating the effectiveness of the data bootstrapping approach.

Conclusion: The proposed methods enable the creation of unlimited synthetic historical maps that preserve the realism and diversity needed for effective machine learning, overcoming the data scarcity problem in historical map analysis.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations: model-based approaches require exact CAD models and transfer poorly, model-free methods fail under challenging conditions, and category-level methods ignore environment priors, limiting industrial practicality.

Method: Box6D infers box dimensions via fast binary search from single RGB-D observation, estimates poses using category CAD templates, and employs depth-based plausibility filter with early-stopping strategy to reject implausible hypotheses.

Result: The approach delivers competitive or superior 6D pose precision while reducing inference time by approximately 76% in real-world storage scenarios and public benchmarks.

Conclusion: Box6D provides an efficient and accurate solution for 6D pose estimation of storage boxes in warehouse contexts, balancing flexibility and performance for industrial applications.
...
<details>
  <summary>Details</summary>
Motivation: VLMs struggle with domain-specific video classification due to limited data and a "rationale gap" - insufficient data to connect complex spatio-temporal content with abstract labels.
...
Result: Significantly outperforms direct supervised fine-tuning across diverse datasets, demonstrating effectiveness in adapting VLMs to domain-specific video analysis.

Conclusion: Self-generated rationales provide an effective, annotation-efficient paradigm for bridging the rationale gap and improving VLM performance in domain-specific video understanding tasks.
...
<details>
  <summary>Details</summary>
Motivation: CLIP's limitation in handling single-label, single-granularity alignment is insufficient for complex domains like medical imaging where images have multiple high-level labels across different annotation granularities.
...
Conclusion: MGLL effectively addresses the limitations of CLIP in medical imaging by enabling better multi-label and cross-granularity alignment while maintaining computational efficiency as a plug-and-play module.
...
TL;DR: Automated method to extract standard 2D echocardiography views from 3D cardiac ultrasound volumes using deep learning and anatomical heuristics, validated with 96% accuracy by cardiologists and preserving diagnostic features.


<details>
  <summary>Details</summary>
Motivation: 3D echocardiography offers better image quality and faster acquisition but physicians are accustomed to interpreting 2D views. This bridges the gap by automatically generating standard 2D views from 3D volumes.
...
Result: 96% accuracy in blinded evaluation by 3 cardiologists on 1,600 videos from 2 hospitals. Extracted 2D videos preserve spatial calibration and enable accurate cardiac abnormality detection and clinical-grade measurements.

Conclusion: The approach successfully allows clinicians to obtain accurate interpretations from 3D volumes while maintaining their familiar 2D viewing format, with code and dataset released for community use.
...
<details>
  <summary>Details</summary>
Motivation: Current VSGG systems operate as closed, feed-forward pipelines without human guidance, while promptable segmentation models lack semantic or relational reasoning. There's a need to unify visual prompting with comprehensive scene understanding.

Method: Uses a Dynamic Interaction Discovery Module that generates subject-conditioned object prompts, and a Semantic Classification Head for joint entity and predicate reasoning. From a single user cue (click/box), it segments/tracks subjects, discovers interacting objects, and predicts <subject, object, predicate> triplets.

Result: Experiments on OpenPVSG benchmark demonstrate strong performance for user-guided PVSG, showing effective combination of human prompting with panoptic grounding and relational inference.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods that fine-tune CLIP for segmentation on limited seen categories often lead to overfitting and degrade the pretrained vision-language alignment, which needs to be stabilized.

Method: Proposes InfoCLIP with two mutual information objectives: compressing pixel-text modality alignment from pretrained CLIP to reduce noise, and maximizing mutual information between pretrained and fine-tuned models to transfer local semantic relations.
...
Method: Proposed a consistency regularization approach composed of differentiable BI-RADS-inspired morphological features to mitigate destructive interference between segmentation and classification tasks.

Result: Statistically significant improvements (p<0.001) in segmentation generalization across three external datasets: UDIAT (Dice=0.81 vs 0.59), BUSI (0.66 vs 0.56), BUS-UCLM (0.69 vs 0.49), achieving state-of-the-art performance on UDIAT dataset.

Conclusion: The proposed multi-task learning approach with consistency regularization effectively mitigates destructive task interference and significantly improves generalization performance in breast ultrasound tumor segmentation.
...
TL;DR: A detection-guided generative framework that uses BART-based generation to predict hierarchical category and attribute tokens for unified visual semantic understanding, outperforming similarity-based approaches in fine-grained recognition.


<details>
  <summary>Details</summary>
Motivation: Current approaches relying on global similarity struggle with fine-grained category distinctions and category-specific attribute diversity, particularly in large-scale e-commerce scenarios where detailed semantic understanding is crucial.

Method: Extract refined ROI-level features from detected objects and employ a BART-based generator to produce semantic tokens in coarse-to-fine sequence covering category hierarchies and property-value pairs, with support for property-conditioned attribute recognition.
...
Conclusion: The detection-guided generative framework provides an effective solution for unified visual semantic understanding, successfully handling object detection, category prediction, and attribute recognition simultaneously while capturing fine-grained distinctions.
...
TL;DR: Proposes FADS, a fairness-aware demonstration selection method for in-context learning that reduces demographic disparities in medical image reasoning without model fine-tuning.


<details>
  <summary>Details</summary>
Motivation: MLLMs show potential for medical image reasoning but suffer from fairness issues across demographic groups, while existing debiasing methods require large labeled datasets or fine-tuning that are impractical for foundation models.

Method: FADS (Fairness-Aware Demonstration Selection) uses clustering-based sampling to create demographically balanced and semantically relevant demonstrations for in-context learning, addressing the limitations of conventional demonstration selection strategies.
...
Conclusion: Fairness-aware in-context learning offers an efficient, scalable, and data-efficient solution for equitable medical image reasoning without requiring model fine-tuning.
...
Method: Uses pre-trained model features to initialize graph structure, applies Gaussianization to normalize activations, and refines with GCNs for improved feature representation.

Result: Outperforms state-of-the-art on CIFAR10-LT, CIFAR100-LT, and ImageNet-LT benchmarks with significant improvements in FPR and tail-class ID accuracy.

Conclusion: Graph-based representation with GCNs and Gaussianization effectively addresses OOD detection challenges in long-tailed recognition scenarios.
...
TL;DR: A sequence-level optimization framework generates printable adversarial textures for clothing that maintain concealment across entire walking videos in both digital and physical settings.
...
TL;DR: Proposes Mixture-of-Ranks (MoR) architecture for single-step image super-resolution using sparsely-gated MoE principles, with degradation-aware expert activation and load balancing.


<details>
  <summary>Details</summary>
Motivation: Existing dense Real-ISR models lack adaptive capability for heterogeneous real-world degradation and efficient knowledge sharing under computational constraints.

Method: Integrates sparse MoE into Real-ISR by treating each LoRA rank as independent expert, uses CLIP embeddings for degradation estimation, and implements degradation-aware load balancing with zero-expert slots.

Result: Comprehensive experiments validate framework effectiveness and achieve state-of-the-art performance in image super-resolution.

Conclusion: MoR architecture successfully addresses limitations of dense models by enabling adaptive expert activation and optimal computational resource allocation for real-world image super-resolution.
...
TL;DR: Deep learning-based material classification using laser speckle patterns for safe and efficient laser cutting, achieving high accuracy even with different laser colors.


<details>
  <summary>Details</summary>
Motivation: Laser cutting generates hazardous dust and smoke, requiring real-time monitoring. Existing speckle sensing methods struggle when laser color changes, creating a need for robust material classification.

Method: Train a convolutional neural network (CNN) on a dataset of laser speckle patterns to recognize distinct material types for monitoring and controlling laser cutting processes.

Result: Achieved 98.30% training accuracy, 96.88% validation accuracy, and F1-score of 0.9643 on 3000 new images across 30 materials, maintaining performance with different laser colors.

Conclusion: The proposed method provides a robust and accurate solution for material-aware laser cutting using speckle sensing, overcoming limitations of previous approaches.
...
TL;DR: CuriGS introduces a curriculum-guided framework for sparse-view 3D reconstruction using 3D Gaussian Splatting, addressing overfitting in limited viewpoint scenarios through pseudo-view generation and progressive training.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting struggles with sparse-view settings due to supervision scarcity and overfitting from limited viewpoint coverage.

Method: Generates student views (pseudo-views) around ground-truth poses with varying perturbation levels, uses curriculum training to gradually increase perturbation, and employs depth-correlation, co-regularization, and multi-signal evaluation for quality assessment.

Result: Outperforms state-of-the-art baselines in both rendering fidelity and geometric consistency across synthetic and real sparse-view scenes.

Conclusion: CuriGS effectively addresses sparse-view challenges in 3DGS through curriculum-guided pseudo-view generation and progressive training, achieving superior reconstruction quality.
...
TL;DR: A cross-modal learning approach that enhances high-resolution satellite imagery with UAV-level detail for crop canopy trait estimation, improving performance on agricultural monitoring tasks.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between satellite and UAV sensing in agriculture by overcoming satellite spatial resolution limitations while leveraging their broader coverage for micro-plot management.

Method: Cross-modal learning strategy using co-registered satellite-UAV image pairs from 84 maize varieties across 5 locations, training models to learn spectral-spatial correspondences between modalities.
...
Conclusion: Cross-modal correspondence learning can effectively bridge the satellite-UAV sensing gap in agricultural monitoring, enabling enhanced crop trait estimation from satellite data.
...
TL;DR: A framework using large language models (LLMs) to improve food recognition by parsing images into text descriptions and aligning multimodal features in a shared embedding space, addressing domain shift, long-tailed distribution, and fine-grained classification challenges.


<details>
  <summary>Details</summary>
Motivation: Food recognition faces challenges including domain shift between training (Internet images) and real-world user photos, long-tailed data distribution, and subtle visual differences between similar dishes.

Method: 1) Use LLMs to parse food images and generate food titles and ingredients; 2) Project generated texts and images to a shared embedding space to maximize pair similarities; 3) Use aligned multimodal features for recognition.

Result: Outperforms existing approaches specifically designed for long-tailed data distribution, domain adaptation, and fine-grained classification on two food datasets.

Conclusion: The proposed LLM-powered framework effectively addresses multiple challenges in food recognition through multimodal feature alignment and outperforms specialized methods across different problem domains.
...
TL;DR: AMS-KV is a scale-adaptive KV caching policy for visual autoregressive models that reduces KV cache usage by 84.83% and self-attention latency by 60.48% by prioritizing condensed and local scales while leveraging inter-scale similarity analysis.


<details>
  <summary>Details</summary>
Motivation: KV caching for next-scale prediction in visual autoregressive models faces excessive memory growth with increasing scales, severely limiting scalability, while existing KV caching research primarily focuses on language models.

Method: Systematic investigation revealed key insights about local scale importance, condensed scales for stability, and inter-scale similarity patterns. Based on these, AMS-KV prioritizes storing KVs from condensed and local scales and optimizes cache utilization through similarity analysis.

Result: AMS-KV reduces KV cache usage by up to 84.83% and self-attention latency by 60.48%. It enables stable scaling to batch size 256 where baseline fails at batch size 128, with improved throughput.

Conclusion: AMS-KV effectively addresses KV memory bottlenecks in visual autoregressive modeling through scale-adaptive caching policies, significantly improving scalability and computational efficiency while maintaining generation quality.
...
TL;DR: LiSTAR is a generative world model for synthesizing high-fidelity 4D LiDAR data using a Hybrid-Cylindrical-Spherical representation and Spatio-Temporal Attention with Ray-Centric Transformer to address sensor geometry challenges and temporal sparsity.
...
Conclusion: LiSTAR provides a powerful foundation for creating realistic and controllable autonomous systems simulations with substantial performance gains across reconstruction, prediction, and generation tasks.
...
TL;DR: VideoSeg-R1 is the first reinforcement learning framework for video reasoning segmentation, using a decoupled architecture with hierarchical sampling, explicit reasoning chains, and adaptive difficulty-aware reasoning.
...
Conclusion: VideoSeg-R1 successfully introduces reinforcement learning to video reasoning segmentation, enabling better generalization and explicit reasoning while maintaining high performance.
...
TL;DR: SpectralTrain is a universal training framework that uses curriculum learning and PCA-based spectral downsampling to reduce HSI classification training time by 2-7x while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: HSI classification requires large-scale data and intensive computation, limiting practical deployment of deep learning models in real-world remote sensing applications.
...
Result: Achieves 2-7x training speedups with small-to-moderate accuracy differences across three benchmark datasets, showing strong generalization across spatial scales and spectral characteristics.
...
<details>
  <summary>Details</summary>
Motivation: To develop a robust outdoor mapping system using 4D mmWave radar that can handle large-scale environments while overcoming limitations of traditional camera or LiDAR-based methods.
...
Conclusion: Rad-GS proves the feasibility of using 4D mmWave radar for robust large-scale scene reconstruction, offering a viable alternative to camera or LiDAR-based SLAM systems in outdoor environments.
...
TL;DR: T2T-VICL enables cross-task visual in-context learning by generating text prompts that describe differences between distinct vision tasks and using a novel inference framework with perceptual scoring.
...
Method: Proposed T2T-VICL pipeline with text prompt generation/selection mechanism to describe task differences, created first cross-task VICL dataset, and developed inference framework combining perceptual score-based reasoning with traditional metrics.

Result: Achieved top-tier results in nine cross-task scenarios and second-tier performance in ten additional scenarios, demonstrating successful cross-task VICL capabilities.

Conclusion: The approach successfully unlocks cross-task visual in-context learning boundaries in vision-language models, showing VLMs can perform VICL across different visual tasks.
...
TL;DR: A novel 4D Gaussian Splatting method that improves dynamic scene reconstruction through error clustering and grouped splatting, achieving better temporal consistency and state-of-the-art rendering quality.
...
Conclusion: The proposed method effectively addresses limitations in dynamic scene reconstruction through targeted error correction and grouped splatting, demonstrating superior performance in both quantitative metrics and visual quality.
...
TL;DR: DCS-LDM decouples content complexity from scale in visual generation, using a hierarchical latent space that supports flexible resolution/frame rate decoding and progressive coarse-to-fine generation.
...
Conclusion: DCS-LDM enables decoupled complexity-scale modeling, supporting progressive coarse-to-fine generation and flexible multi-scale visual synthesis.
...
TL;DR: VTinker is a novel Video Frame Interpolation pipeline that uses guided flow upsampling with input frames as guidance and texture mapping to create intermediate proxies, achieving state-of-the-art performance by addressing blurring and ghosting issues in high-resolution motion estimation.


<details>
  <summary>Details</summary>
Motivation: Existing flow-based VFI methods suffer from blur/mosaic artifacts at flow edges due to bilinear upsampling, and cannot adequately capture fine pixel motion at high resolution, leading to misalignment, ghosting, and discontinuities in interpolated frames.

Method: Proposes VTinker with two core components: 1) Guided Flow Upsampling (GFU) that uses input frames as guidance to refine bilinear upsampled flows, and 2) Texture Mapping that creates an intermediate proxy frame for selecting clear texture blocks from input frames, which are then mapped to produce the final frame via reconstruction.

Result: Extensive experiments demonstrate that VTinker achieves state-of-the-art performance in Video Frame Interpolation, effectively addressing blurring and ghosting issues.

Conclusion: VTinker provides an effective solution for high-resolution video frame interpolation by combining guided flow refinement with texture-based reconstruction, outperforming existing methods and producing higher quality interpolated frames.
...
<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detection methods struggle with out-of-distribution generalization due to reliance on spurious shortcuts during training.
...
Result: Achieves 5.4% average accuracy improvement over existing methods on dataset with 42 generative models.

Conclusion: PiN-CLIP effectively addresses shortcut learning in AI-generated image detection and enables more robust artifact representation learning.
...
TL;DR: Proposes a blind quality enhancement method for compressed videos using pretrained degradation representation learning and hierarchical termination mechanism to dynamically adjust computational demands based on compression levels.


<details>
  <summary>Details</summary>
Motivation: Existing QECV methods rely on known QPs and lack spatial degradation details. Real-world scenarios often have unknown QPs, requiring blind methods that can adapt to varying artifact patterns across spatial positions.

Method: Uses pretrained Degradation Representation Learning (DRL) module to extract high-dimensional multiscale degradation representations, and introduces hierarchical termination mechanism to dynamically adjust artifact reduction stages based on compression level.

Result: Achieves 110% PSNR improvement (from 0.31 dB to 0.65 dB) over state-of-the-art blind method at QP=22, and reduces average inference time at QP=22 by half compared to QP=42.

Conclusion: The proposed approach effectively addresses limitations of existing blind QECV methods by providing detailed spatial degradation information and adaptive computational efficiency, significantly improving both performance and inference speed.
...
TL;DR: MLLMs lack human-like social reasoning to detect deception in interactions. The paper introduces MIDA task and dataset, shows current models fail at deception assessment, and proposes SoCoT reasoning with DSEM module for improvement.
...
Method: Introduced Multimodal Interactive Deception Assessment (MIDA) task with synchronized video-text dataset, benchmarked 12 MLLMs, and proposed Social Chain-of-Thought (SoCoT) reasoning pipeline with Dynamic Social Epistemic Memory (DSEM) module.

Result: Even powerful models like GPT-4o struggle to distinguish truth from falsehood reliably. The proposed SoCoT+DSEM framework yields performance improvement on this challenging task.

Conclusion: Current MLLMs fail to effectively ground language in multimodal social cues and lack social reasoning capabilities. The proposed framework demonstrates a promising path toward building MLLMs capable of genuine human-like social reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Addresses the gap between how detectors are trained (lacking spatial reliability and ranking awareness) and how they are evaluated (using ranking-based prediction selection), which hampers model performance.
...
Conclusion: SR3D effectively bridges the training-inference gap and achieves superior performance for real-time 3D object detection in indoor environments.
...
TL;DR: Proposes HSRW-CD dataset for high-resolution water body change detection and SSCP attention module to enhance spatial semantic and structural feature learning in deep networks.


<details>
  <summary>Details</summary>
Motivation: Addresses scarcity of high spatial resolution datasets for water body change detection and limitations of existing methods in exploiting spatial semantic and structural information in deep features.
...
Conclusion: The proposed SSCP module effectively enhances spatial semantics and structure learning for water body change detection and can be integrated as plug-and-play component into existing models.
...
TL;DR: LEGO-SLAM is the first real-time open-vocabulary 3D Gaussian Splatting SLAM system that integrates language features through a scene-adaptive encoder-decoder, enabling semantic understanding while maintaining 15 FPS performance.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS-based SLAM systems lack semantic understanding for robotic interaction, and integrating language features faces challenges with excessive memory usage and rendering overhead, while static models lack adaptability for novel environments.

Method: Uses a scene-adaptive encoder-decoder to distill high-dimensional language embeddings into compact 16D features, reducing memory and accelerating rendering. Includes language-guided pruning to reduce Gaussian count by over 60% and language-based loop detection using mapping features.

Result: Achieves competitive mapping quality and tracking accuracy while providing open-vocabulary capabilities at 15 FPS, with significant reduction in memory usage and Gaussian count.

Conclusion: LEGO-SLAM successfully integrates language understanding into real-time 3DGS SLAM through adaptive feature compression and semantic optimization, enabling advanced robotic interaction capabilities.
...
TL;DR: RGE incorporates explicit reasoning into multimodal embeddings by leveraging MLLMs' generative capabilities, improving multimodal retrieval by 4.9% through structured rationale generation coupled with contrastive training.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat embedding extraction as direct encoding, overlooking MLLMs' generative reasoning capabilities that could enhance representation quality.

Method: Propose Reasoning Guided Embeddings (RGE) that enables structured rationale generation conditioned on instructions, then extracts representations after reasoning unfolds, coupled with contrastive training.
...
Conclusion: Explicit reasoning can effectively enhance embedding quality in multimodal representations.
...
TL;DR: PPCL is a flexible structured pruning framework for Diffusion Transformers that reduces parameters by 50% with minimal performance degradation using plug-and-play distillation.
...
Conclusion: PPCL enables efficient deployment of DiTs in resource-constrained environments with high compression ratios and minimal performance loss.
...
TL;DR: Video2Layout reconstructs metric-grounded spatial layouts from videos using continuous object boundary coordinates instead of discretized grid maps, enabling fine-grained spatial reasoning and quantitative spatial computation.


<details>
  <summary>Details</summary>
Motivation: Current grid-based spatial understanding methods in MLLMs rely on discretized raster representations that limit fine-grained spatial reasoning capabilities.

Method: Two-stage framework: supervised fine-tuning on AI2THOR simulator data to learn visual-to-coordinate mapping, followed by reinforcement fine-tuning for real-world generalization. Uses continuous object boundary coordinates to quantify physical distances and sizes.

Result: V2LO-7B model achieves 4.92% average improvement over grid-based methods on QVS-Bench and mainstream spatial reasoning benchmarks.

Conclusion: Continuous coordinate-based spatial layout reconstruction outperforms discretized grid methods, providing better quantitative spatial computation and alleviating ambiguity in spatial relationship descriptions.
...
TL;DR: Simba reformulates point cloud completion by replacing direct regression with distribution learning using diffusion models and hierarchical Mamba architecture, achieving state-of-the-art performance while addressing overfitting and noise sensitivity issues.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of regression-based methods that overfit to instance-specific transformations and are sensitive to input noise, preventing robust preservation of both fine-grained details and global structural integrity in point cloud completion.

Method: Reformulates point-wise transformation regression as distribution learning, integrates symmetry priors with diffusion models for robust geometric structure capture, and uses hierarchical Mamba-based architecture for high-fidelity upsampling.

Result: Achieves state-of-the-art performance across PCN, ShapeNet, and KITTI benchmarks, demonstrating superior robustness and generalization compared to existing methods.

Conclusion: Simba effectively addresses key challenges in point cloud completion by combining distribution learning with diffusion models and hierarchical architecture, providing a more robust and generalizable solution for preserving both local details and global structure.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in long video understanding that require both efficient architecture and effective handling of extended temporal contexts, while overcoming vision token redundancy issues in hybrid models.

Method: Uses hybrid Mamba-Transformer backbone combining state-space model efficiency with attention expressivity, plus TransV module that transfers and compresses vision tokens into instruction tokens to reduce redundancy.
...
Conclusion: The work represents an initial step in developing, interpreting, and compressing hybrid Mamba-Transformer architectures for long video understanding, with insights into attention behaviors and vision-to-text information flow.
...
TL;DR: LNG-SWR is a plug-in framework that improves medical image segmentation robustness by learning frequency-bias priors through layer-wise noise injection and selective wavelet reconstruction, achieving better performance under distribution shifts without sacrificing clean accuracy.
...
Method: Inject small zero-mean noise at multiple layers to learn frequency-bias priors, then apply selective wavelet reconstruction to suppress noise-sensitive bands and enhance structural cues while maintaining spectral consistency.
...
<details>
  <summary>Details</summary>
Motivation: Existing survival analysis methods lack transparency for clinical adoption, and current pathology agents have limitations including inability to integrate multimodal data, ineffective region-of-interest exploration, and failure to leverage experiential learning from historical cases.

Method: Two-stage approach: (1) WSI-Gene CoT-Enhanced Case Bank Construction using hierarchical analysis for pathology images and gene-stratified analysis for six functional gene categories, generating structured reports with chain-of-thought reasoning; (2) Dichotomy-Based Multi-Expert Agent Inference that retrieves similar cases via RAG and integrates multimodal reports with expert predictions through progressive interval refinement.
...
<details>
  <summary>Details</summary>
Motivation: Address limitations of existing methods that only delay EOS token implicitly, lacking stability and controllability in maximizing output token length as an explicit optimization objective.

Method: Two-stage framework: 1) Adversarial prompt search using reinforcement learning to find prompts inducing verbose outputs, 2) Vision-aligned perturbation optimization to craft adversarial images that match visual embeddings with adversarial prompts.
...
Conclusion: VTIA successfully demonstrates the vulnerability of VLMs to verbose-text induction attacks, highlighting security concerns and the need for robust defenses against such adversarial manipulations.
...
TL;DR: EvoVLA is a self-supervised VLA framework that addresses stage hallucination in long-horizon robotic manipulation through stage-aligned rewards, pose-based exploration, and long-horizon memory, achieving 69.2% success rate and significant improvements in sample efficiency and sim-to-real transfer.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action models suffer from stage hallucination where agents exploit coarse evaluation signals to shortcut multi-step tasks without truly completing them, limiting their effectiveness in long-horizon robotic manipulation.
...
Result: EvoVLA improves average task success by 10.2 percentage points over OpenVLA-OFT to 69.2%, achieves 1.5x better sample efficiency, reduces stage hallucination from 38.5% to 14.8%, and reaches 54.6% success rate in real-world deployment (11 points better than OpenVLA-OFT).
...
TL;DR: Introduces Thinking-while-Generating (TwiG), the first framework that interleaves textual reasoning throughout visual generation, enabling co-evolution of reasoning and content.


<details>
  <summary>Details</summary>
Motivation: Existing methods use reasoning only before or after generation, lacking real-time multimodal interaction during the generation process itself.
...
Conclusion: TwiG demonstrates the potential of interleaved textual reasoning for enhanced visual generation and aims to inspire further research in this direction.
...
TL;DR: RF-CLIP improves CLIP's dense prediction by filtering distraction tokens that divert attention from target regions, achieving state-of-the-art performance on eight benchmarks without training.


<details>
  <summary>Details</summary>
Motivation: Existing methods don't explore CLIP's performance boundaries for dense prediction from interpretability mechanisms, particularly how CLIP diverts attention to irrelevant tokens similar to human distraction.

Method: Propose ReFocusing CLIP (RF-CLIP), a training-free approach that redirects attention from distraction tokens back to target regions by filtering dimension-specific over-activation tokens.

Result: Achieves state-of-the-art performance on eight benchmarks while maintaining high inference efficiency.

Conclusion: Filtering distraction tokens enhances CLIP's multimodal alignment granularity and dense prediction capability, demonstrating the importance of addressing attention diversion in vision-language models.
...
TL;DR: Mantis introduces a Disentangled Visual Foresight framework that separates visual prediction from the main VLA backbone using meta queries and diffusion Transformer, achieving state-of-the-art performance on robotics tasks while maintaining strong language comprehension.
...
Method: Mantis uses Disentangled Visual Foresight that decouples visual foresight prediction from the backbone using meta queries and a diffusion Transformer head. Current visual state is provided via residual connection, enabling meta queries to capture latent actions that boost explicit action learning while reducing backbone burden.

Result: Achieves 96.7% success rate on LIBERO benchmark after fine-tuning, surpassing baselines with high convergence speed. Outperforms π₀.₅ in real-world evaluations, particularly in instruction-following, generalization to unseen instructions, and reasoning ability.

Conclusion: Mantis effectively addresses VLA limitations by disentangling visual prediction, enabling the backbone to maintain strong language comprehension while achieving superior performance in robotics tasks through efficient visual foresight learning.
...
TL;DR: Proposes DSLGA, a two-stage unsupervised domain adaptation method for visible-infrared person re-identification to handle modality discrepancies between public datasets and real-world data.


<details>
  <summary>Details</summary>
Motivation: Existing VI-ReID algorithms struggle in real-life applications due to discrepancies between public datasets and real-world data, requiring methods that can transfer knowledge without new annotations.

Method: Two-stage approach: 1) Domain-Shared Learning Strategy (DSLS) for pre-training to mitigate inter-domain modality discrepancies, 2) Gradual Alignment Strategy (GAS) for fine-tuning to handle intra-domain modality discrepancies through cluster-to-holistic alignment.
...
Conclusion: DSLGA effectively addresses modality discrepancies in UDA-VI-ReID and demonstrates superior performance compared to existing approaches.
...
TL;DR: PrIntMesh is a template-based framework that reconstructs organs as unified systems while preserving topology and internal boundaries, achieving high geometric accuracy and structural consistency even with limited data.
...
Result: Achieves high geometric accuracy, correct topology, and robust performance on heart, hippocampus, and lungs; better reconstructs shared interfaces and maintains structural consistency compared to voxel- and surface-based methods.

Conclusion: PrIntMesh provides a data-efficient, topology-preserving solution suitable for clinical use that produces anatomically plausible organ reconstructions.
...
TL;DR: VLA-Fool is a comprehensive study of multimodal adversarial robustness in Vision-Language-Action models, revealing their vulnerability to minor multimodal perturbations that cause significant behavioral deviations.
...
Method: VLA-Fool unifies three levels of multimodal adversarial attacks: textual perturbations (gradient-based and prompt-based), visual perturbations (patch and noise distortions), and cross-modal misalignment attacks that disrupt semantic correspondence between perception and instruction. It incorporates a VLA-aware semantic space into linguistic prompts.
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in unsupervised image classification where recent methods focus solely on clustering while bypassing representation learning, aiming to narrow the performance gap with supervised methods.

Method: ICCE trains multiple clustering heads on a frozen backbone to produce diverse clusterings, then uses cluster ensembling to consolidate results into consensus clustering, and finally trains a classifier using consensus as pseudo-labels.

Result: Achieves state-of-the-art performance on ten benchmarks: 99.3% on CIFAR10, 89% on CIFAR100, and 70.4% on ImageNet - the first unsupervised method to exceed 70% on ImageNet.

Conclusion: ICCE successfully bridges the gap between unsupervised and supervised image classification through effective cluster ensembling, demonstrating that unsupervised methods can approach supervised performance levels.
...
TL;DR: SwiTrack is a novel state-switching framework for cross-modal object tracking (RGB-NIR) that uses three specialized streams to handle modality switching, prevent object drift, and maintain robust tracking performance.


<details>
  <summary>Details</summary>
Motivation: Existing CMOT methods with parallel RGB and NIR branches connected to shared backbones limit extraction of modality-specific features and fail to address object drift issues with unreliable inputs.

Method: Deploys three specialized streams: visual encoder for RGB frames, NIR gated adapter with visual encoder for NIR frames, and consistency trajectory prediction module for invalid modalities. Also uses dynamic template reconstruction and similarity alignment loss.

Result: Achieves state-of-the-art performance with 7.2% precision rate gain and 4.3% success rate gain, while maintaining real-time tracking at 65 FPS on latest benchmarks.

Conclusion: SwiTrack effectively addresses cross-modal tracking challenges by providing robust feature extraction, drift mitigation, and real-time performance through its state-switching framework.
...
TL;DR: A novel MLP-based method with Memory-Block for full-body tracking from sparse sensor inputs, achieving state-of-the-art accuracy and real-time performance on mobile HMDs.
...
Method: Multi-layer perceptron backbone enhanced with residual connections and Memory-Block component, which uses trainable code-vectors to represent missing sensor data and improve temporal consistency. Formulated as multi-task learning problem.

Result: Outperforms state-of-the-art baselines with substantial reduction in prediction errors. Achieves 72 FPS on mobile HMDs, improving accuracy-running time tradeoff.
...
TL;DR: TetraSDF is a precise analytic meshing framework for neural SDFs that uses tetrahedral positional encoding to enable exact mesh extraction matching the zero-level set, overcoming limitations of sampling-based methods and plain ReLU MLPs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for extracting meshes from neural SDFs have limitations: sampling-based approaches introduce discretization error, while continuous piecewise affine methods only work with plain ReLU MLPs, not modern positional encoders.

Method: Uses a ReLU MLP composed with multi-resolution tetrahedral positional encoder, where barycentric interpolation preserves global CPWA structure. Tracks ReLU linear regions within encoder-induced polyhedral complex and applies fixed analytic input preconditioner to reduce directional bias.
...
Conclusion: TetraSDF provides a precise analytic meshing solution that overcomes limitations of previous methods, enabling exact mesh extraction from neural SDFs with positional encoders while maintaining accuracy and efficiency.
...
TL;DR: Fast spatio-temporal scene understanding using Vision Gated Generative Transformers (VGGT) for real-time assistive navigation with efficient 3D scene updates and temporal consistency.
...
Method: Process image flow with sliding window for continuous 3D scene updates, use VGGT tracking head to aggregate 2D semantic masks into 3D objects, maintain timestamps and instance identities for temporal consistency.
...
Conclusion: The framework successfully enables real-time spatio-temporal scene understanding for assistive navigation with efficient memory usage and temporal consistency.
...
<details>
  <summary>Details</summary>
Motivation: Accurate weed species identification enables selective herbicide application in precision agriculture, supporting sustainable crop management practices.
...
Conclusion: The framework enables real-time deployment on edge devices for automated weed detection, reducing herbicide overuse and providing scalable sustainable farming solutions.
...
TL;DR: Texture3dgs optimizes 3D Gaussian Splatting for mobile GPUs with a novel sorting algorithm optimized for 2D texture cache, achieving 4.1x sorting speedup and 1.7x overall reconstruction speedup.
...
Method: Developed a novel sorting algorithm optimized for 2D texture cache, improved variable layout design, and other optimizations specifically tailored for mobile GPU architecture.

Result: Achieved up to 4.1x speedup for sorting, 1.7x overall speedup for 3D scene reconstruction, and reduced memory usage by up to 1.6x.

Conclusion: Texture3dgs effectively enables efficient mobile 3D scene reconstruction through mobile GPU-specific optimizations, particularly the texture cache-optimized sorting algorithm.
...
TL;DR: Upsample Anything is a test-time optimization framework that upsamples low-resolution features to high-resolution outputs using a learned anisotropic Gaussian kernel without training.
...
Method: Per-image optimization learns an anisotropic Gaussian kernel combining spatial and range cues, bridging Gaussian Splatting and Joint Bilateral Upsampling as a universal edge-aware operator.

Result: Achieves state-of-the-art performance on semantic segmentation, depth estimation, and map upsampling, running in ≈0.419s per 224x224 image.

Conclusion: The framework provides efficient, training-free upsampling that transfers across architectures and modalities for precise high-resolution reconstruction.
...
Conclusion: SAEs are positioned as effective tools for large-scale thematic analysis across different modalities, providing a new perspective on their practical applications.
...
Method: BioBench unifies 9 publicly released ecology tasks across 4 taxonomic kingdoms and 6 acquisition modalities (3.1M total images), using a Python API to download data, fit lightweight classifiers to frozen backbones, and report class-balanced macro-F1 scores.

Result: BioBench provides new signal for computer vision in ecology, capturing what ImageNet misses and offering a more reliable evaluation framework for ecological applications.

Conclusion: BioBench serves as both a solution to ImageNet's limitations in ecology vision tasks and a template recipe for building reliable AI-for-science benchmarks in any domain.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of previous MVD-based texture generation methods that struggle with occluded regions, mesh-texture alignment, and cross-view consistency in content and color intensity.
...
Conclusion: The native 3D texture generation approach effectively addresses key limitations of 2D-based methods and shows promising performance and generalization across multiple applications.
...
<details>
  <summary>Details</summary>
Motivation: Recent hybrid UIE approaches achieve strong performance but have high computational costs, limiting their practicality in real-time scenarios on resource-limited platforms.
...
Result: Achieves competitive restoration quality with substantially fewer parameters and FLOPs, enabling real-time inference on benchmark datasets.
...
TL;DR: ChangeDINO is a Siamese framework for building change detection that combines a lightweight backbone with frozen DINOv3 features, uses spatial-spectral differential transformers, and includes learnable morphology for boundary refinement.


<details>
  <summary>Details</summary>
Motivation: Existing RSCD methods rely too much on change-map annotations and underuse semantic information in non-changing regions, limiting robustness to illumination variation, off-nadir views, and scarce labels.

Method: Multiscale Siamese framework with lightweight backbone fused with frozen DINOv3 features, spatial-spectral differential transformer decoder using multi-scale absolute differences as change priors, and learnable morphology module for boundary refinement.
...
Conclusion: ChangeDINO provides an effective end-to-end solution for optical building change detection that leverages semantic-rich features and robust change priors to handle challenging conditions.
...
Method: Uses 2D deep features, local relative coordinates, and up-sampling scale ratios with implicit representation networks; includes local frequency estimation module for high-frequency textures and global coordinate modulation for facial structure guidance.
...
Conclusion: ARASFSR provides a robust solution for arbitrary-scale face super-resolution with improved performance over existing methods.
...
TL;DR: A teacher-student framework with key frame selection for weakly supervised aerial terrain classification that achieves high mIoU and temporal consistency using only 30% labeled data.


<details>
  <summary>Details</summary>
Motivation: Address challenges in UAV terrain classification including data annotation complexity, temporal consistency requirements, data scarcity, and technology range limitations.
...
TL;DR: Real-time camera localization using pre-captured LiDAR point clouds through neural rendering to bridge synthetic-real image gap, achieving drift-free metric-scale tracking.


<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods suffer from drift, scale ambiguity, and dependency on fiducials or loop closure, limiting reliable navigation and virtual-real alignment in robotics and XR applications.

Method: Render synthetic views from accurate colored LiDAR point clouds to establish 2D-3D correspondences with live frames. Use neural rendering to reduce domain gap between synthetic and real images, minimizing occlusion and background artifacts for improved feature matching. Two variants: Online Render and Match, and Prebuild and Localize.

Result: Drift-free camera tracking with correct metric scale in global LiDAR coordinate system. Demonstrates improved performance on ScanNet++ dataset and outperforms existing SLAM pipelines.

Conclusion: The method enables robust real-time camera localization by leveraging accurate LiDAR point clouds and neural rendering, providing drift-free metric-scale tracking superior to traditional visual SLAM approaches.
...
Method: Proposes Multi-Order Matching Network with zero-order, first-order, and second-order matching to identify relevant RGB information, plus multi-order aggregation with structure detectors for selective feature transfer.

Result: Achieves state-of-the-art performance and exhibits outstanding robustness in handling misaligned RGB-D data.

Conclusion: MOMNet effectively addresses the misalignment problem in real-world RGB-D super-resolution through adaptive multi-order matching and aggregation.
...
TL;DR: DetailSemNet is a new model for offline signature verification that focuses on matching local structures between signature images, achieving state-of-the-art performance through a Detail Semantics Integrator that enhances fine-grained details while expanding discriminative semantics.


<details>
  <summary>Details</summary>
Motivation: Previous methods rely on holistic features for pair comparisons, which may not capture fine-grained differences needed for robust signature verification. Transformer-based backbones without modifications can obscure local details, adversely impacting performance.
...
Result: Achieves state-of-the-art results with clear margins on leading benchmarks, outperforms recent methods consistently, shows remarkable generalization in cross-dataset testing, and provides enhanced interpretability through local structure matching.

Conclusion: The focus on local structure matching improves performance, interpretability, and generalization, making DetailSemNet highly suitable for real-world offline signature verification applications.
...
Conclusion: CAMS effectively improves compositional zero-shot learning by extracting semantic features from visual representations and performing semantic disentanglement in multidimensional spaces.
...
TL;DR: Introduces Rigid Body Marker (RBM) for sparse 6-DoF optical motion capture, with deep learning regression that estimates SMPL parameters using geodesic loss, achieving state-of-the-art accuracy with much lower computation than optimization methods.


<details>
  <summary>Details</summary>
Motivation: Traditional marker-based MoCap has scalability limitations due to dense marker requirements, time-consuming setup, and marker identification ambiguity.

Method: Uses sparse Rigid Body Markers (RBM) for unambiguous 6-DoF data, develops deep learning regression model with geodesic loss to directly estimate SMPL parameters from synthesized AMASS dataset data.

Result: Achieves state-of-the-art body pose estimation accuracy, matches optimization-based method performance with 10x less computation, validated with real-world Vicon system data.

Conclusion: Combining sparse 6-DoF RBM with manifold-aware geodesic loss provides practical, high-fidelity solution for real-time MoCap in graphics, VR, and biomechanics.
...
Result: Improves depth consistency across images and overall depth quality compared to state-of-the-art methods on DDAD and nuScenes datasets.

Conclusion: The proposed geometry-guided approach successfully addresses cross-view inconsistency in surround-view depth estimation, enabling more reliable 360° 3D perception from minimally overlapping multi-camera setups.
...
TL;DR: Graph-based segmentation models combining Vision Transformers with Graph Neural Networks improve surgical anatomy identification in laparoscopic cholecystectomy by better handling occlusions and rare structures.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle with occlusions, long-range dependencies, and fine-scale geometry of rare anatomical structures during laparoscopic surgery, which can lead to surgical complications.

Method: Two segmentation models: (1) Static k-NN graph with GCNII for stable long-range information propagation, and (2) Dynamic Differentiable Graph Generator with GAT for adaptive topology learning, both integrating ViT feature encoders with GNNs.

Result: 7-8% improvement in mIoU and 6% improvement in mDice scores over state-of-the-art baselines, with anatomically coherent predictions on thin, rare and safety-critical structures.

Conclusion: Graph-based segmentation enhances both performance and anatomical consistency in surgical scene analysis, improving interpretability and reliability for safer laparoscopic and robot-assisted surgery.
...
TL;DR: LDAG uses language descriptions from LLMs to provide unbiased meta guidance for few-shot segmentation, outperforming existing methods that rely on support images.


<details>
  <summary>Details</summary>
Motivation: Existing FSS methods struggle with intra-class variations in support images, leading to inaccurate guidance for untrained classes. The paper argues that unbiased meta guidance is more important than support image references.

Method: Proposes LDAG with two modules: MaE generates multiple attribute descriptions via LLMs for refined visual-text guidance, and MaA aligns attribute texts with visual features through cross-modal interaction.

Result: The method achieves new state-of-the-art performance, outperforming existing approaches by a clear margin.

Conclusion: Language-driven attribute generalization provides more robust support than image-based references for few-shot segmentation, demonstrating the effectiveness of leveraging LLMs for cross-modal guidance.
...
TL;DR: StreetView-Waste dataset addresses urban waste management by providing annotated images for container detection, tracking, and overflow segmentation, with improved baselines using geometric priors and heuristics.


<details>
  <summary>Details</summary>
Motivation: Existing litter detection datasets lack annotations for container tracking and are captured in static environments, limiting real-world utility for monitoring overflowing waste containers from garbage truck images.
...
Result: Fine-tuned detectors perform reasonably for detection, but tracking struggles; proposed heuristics reduce counting error by 79.6%. Geometry-aware strategy improves segmentation mAP@0.5 by 27% on lightweight models.
...
<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods for VLMs focus only on semantic salience, which biases token retention toward semantic cues and discards critical information needed for action generation in VLA models, leading to performance degradation.
...
Conclusion: The proposed dual-system approach to token pruning effectively addresses the unique requirements of VLA models, enabling efficient real-time deployment while maintaining performance.
...
TL;DR: LLaVA³ improves 3D scene understanding in vision-language models using multi-view 2D images without fine-tuning, inspired by Cubist art to represent multiple viewpoints.
...
Method: Generate omnidirectional visual representations of 3D objects from multi-view 2D images using intermediate 3D reconstruction, describing scenes through multiple viewpoints like Cubist paintings.
...
Conclusion: LLaVA³ effectively bridges the 2D-3D gap in vision-language understanding without requiring 3D fine-tuning, demonstrating the viability of multi-view 2D approaches for 3D scene comprehension.
...
Conclusion: FastSurfer-CC provides an efficient and fully automated solution for corpus callosum analysis with superior performance over existing tools.
...
TL;DR: The paper presents a systematic study on occluded video prediction, showing that incorporating point-flow (motion) and depth-map (geometry) information improves prediction performance in occluded scenarios and background motion.


<details>
  <summary>Details</summary>
Motivation: Occlusion remains a fundamental challenge in video prediction despite general models' success in standard scenarios. The authors hypothesize that explicit motion and geometric structure information can help models handle occlusion better.

Method: Used a standard multi-object latent transformer architecture modified to incorporate depth and point-flow information. Evaluated on synthetic and real-world datasets using appearance metrics and Wasserstein distances on object masks.
...
<details>
  <summary>Details</summary>
Motivation: Current microrobot pose estimation methods require large, expensive microscope image datasets that are difficult to acquire due to complex fabrication and labor-intensive labeling. Digital twin systems struggle to replicate optical microscopy phenomena like diffraction artifacts.

Method: Integrates wave optics-based physical rendering and depth alignment into a GAN framework to synthesize realistic microscope images efficiently. Combines physics-based simulation with deep generative learning.

Result: Improves SSIM by 35.6% over AI-only methods with real-time rendering (0.022 s/frame). Pose estimator trained on synthetic data achieves 93.9%/91.9% (pitch/roll) accuracy, only 5.0%/5.4% below real-data training. Generalizes to unseen poses without additional data.

Conclusion: The proposed physics-informed generative framework enables robust microrobot pose estimation with synthetic data that closely matches real-data performance, eliminating the need for expensive real data collection while handling complex optical phenomena.
...
<details>
  <summary>Details</summary>
Motivation: Large variation in DCE-MRI appearance due to different acquisition protocols and individual factors makes automated tumor segmentation challenging, despite its importance in breast cancer screening and treatment.
...
Conclusion: Incorporating knowledge of phase acquisition time enhances breast tumor segmentation in DCE-MRI, improving performance and model generalization across different imaging protocols.
...
TL;DR: A novel method for jointly mapping indoor scenes and registering ceiling-mounted cameras using a mobile agent with RGB-D camera and synchronized CMC videos.


<details>
  <summary>Details</summary>
Motivation: Manual registration of CMCs is inefficient and costly, while automatic visual localization performs poorly with visual ambiguity. A unified solution is needed.

Method: Use mobile agent with head-mounted RGB-D camera to traverse scene, synchronize CMCs to capture agent, correlate trajectories via timestamps, and optimize with custom factor graph.

Result: Method effectively accomplishes both scene mapping and CMC registration in unified framework, jointly enhancing performance. New benchmark dataset created.

Conclusion: Provides reliable tool for position-aware applications by solving CMC registration and scene mapping simultaneously with improved accuracy.
...
TL;DR: A comprehensive boxing punch dataset with 6,915 manually annotated clips across 6 punch types, extracted from YouTube sparring sessions to support computer vision research in action recognition.


<details>
  <summary>Details</summary>
Motivation: Address the bottleneck in combat sports computer vision research caused by lack of robust datasets, particularly for dynamic, unstructured actions in varying recording environments.

Method: Created dataset from 20 publicly available YouTube sparring sessions involving 18 athletes, with manual segmentation and labeling of 6,915 punch clips into 6 distinct punch types, ensuring precise temporal boundaries and class consistency.
...
TL;DR: SemCLIP improves CLIP's robustness to semantic transformations like negation and paraphrasing by using a new contrastive loss and LLM-generated training triples, achieving better performance on negation benchmarks while maintaining original CLIP capabilities.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with semantic transformations like negation and paraphrasing - negation changes meaning with minimal lexical changes while paraphrasing creates different expressions with same meaning, posing challenges for vision-language model alignment.
...
Result: Improves CC-Neg benchmark accuracy from 68.1% to 78.1%; pushes negated captions further away while preserving CLIP performance; shows better robustness on downstream zero-shot classification tasks compared to CLIP.

Conclusion: SemCLIP achieves significant robustness to semantic transformations while maintaining CLIP's original capabilities, demonstrating improved handling of negation and paraphrasing in vision-language models.
...
TL;DR: A robust multi-camera tracking system for gymnastics that uses domain knowledge about gymnasts' movement in vertical planes to handle limited camera views and detection failures through cascaded data association.


<details>
  <summary>Details</summary>
Motivation: Gymnast tracking faces unique challenges: limited camera installation due to space restrictions, and detection failures from lighting variations, background changes, uniforms, and occlusions that often leave only two opposing views with valid detections.

Method: Proposes a cascaded data association paradigm that uses triangulation when cross-view detections are sufficient, and ray-plane intersection (based on gymnasts typically moving within predefined vertical planes) when detections are insufficient from opposing views.

Result: The method demonstrates robustness in challenging scenarios, outperforming existing methods, and has been successfully deployed in Gymnastics World Championships with recognition from the International Gymnastics Federation.

Conclusion: Incorporating gymnastics domain knowledge about movement patterns enables robust tracking even with limited camera views and detection failures, making the system practical for real-world gymnastics judging applications.
...
TL;DR: Analysis of local and global optical flow methods, focusing on Horn-Schunck algorithm with multiresolution implementation using bilinear interpolation and prolongation for improved motion estimation.


<details>
  <summary>Details</summary>
Motivation: To investigate and compare the effectiveness of local (Lucas-Kanade) and global (Horn-Schunck) optical flow methods, and enhance motion estimation accuracy under varying image conditions.

Method: Implemented a multiresolution version of the Horn-Schunck algorithm using bilinear interpolation and prolongation techniques to improve convergence and accuracy in optical flow computation.

Result: The study demonstrates that combining multiresolution strategies with the Horn-Schunck algorithm enhances motion estimation performance between frames under different image conditions.

Conclusion: Multiresolution approaches significantly improve the Horn-Schunck algorithm's effectiveness for optical flow computation, providing better motion estimation capabilities compared to standard implementations.
...
TL;DR: A two-stage detection framework using contrastive learning and k-NN classification achieves 91.3% accuracy in synthetic image detection with only 150 images per class, outperforming existing methods by 5.2 percentage points.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of detecting synthetic images from rapidly evolving generative AI models, where traditional detection methods relying on periodic retraining are computationally infeasible.

Method: Two-stage framework: 1) Vision model trained via supervised contrastive learning on subset of generators, 2) k-NN classifier in embedding space using few-shot learning with limited samples from unseen generators.
...
TL;DR: EOGS++ extends 3D Gaussian Splatting for satellite imagery, operating directly on raw panchromatic data with embedded bundle adjustment and improved reconstruction techniques.


<details>
  <summary>Details</summary>
Motivation: To enhance Earth observation by improving 3D Gaussian Splatting for satellite imagery, eliminating preprocessing needs and improving camera pose estimation.

Method: Extends EOGS framework with direct raw data processing, optical flow-based bundle adjustment during training, early stopping, and TSDF post-processing.
...
Conclusion: EOGS++ provides superior reconstruction quality and efficiency for satellite imagery while maintaining computational advantages of Gaussian Splatting.
...
TL;DR: VARiant reduces memory overhead in Visual Auto-Regressive models by using scale-dependent subnetworks with weight sharing, achieving significant memory savings (40-80%) and speedup (3.5x) while maintaining comparable image quality.
...
Method: Uses scale-depth asymmetric dependency: early scales processed by full network, later scales use subnets (16 to 2 layers) with weight sharing. Employs progressive training strategy to resolve optimization conflicts.
...
TL;DR: Lite Any Stereo is an ultra-light stereo depth estimation framework that achieves strong zero-shot generalization with less than 1% computational cost of state-of-the-art methods, ranking 1st on four real-world benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional efficient stereo matching models were considered incapable of zero-shot ability due to limited capacity, while recent advances focused on accuracy at the cost of large model size.
...
Result: Achieves accuracy comparable to or exceeding state-of-the-art non-prior-based accurate methods while requiring less than 1% computational cost, ranking 1st across four widely used real-world benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Child malnutrition is a global crisis, but current screening methods are labor-intensive and not scalable, preventing early intervention in low-resource settings.
...
Result: Achieved 0.79 recall, 0.82 AUC, significantly lower RMSEs for anthropometric measurements. Doctors rated it 4.3/5 for accuracy and 4.6/5 for efficiency. Cross-dataset results show 25% recall gain and 3.5 cm RMSE reduction with demographically matched knowledge bases.

Conclusion: NutriScreener provides a scalable and accurate solution for early malnutrition detection in low-resource environments, demonstrating deployment readiness and reliable performance in unconstrained pediatric settings.
...
TL;DR: POMA-3D is the first self-supervised 3D representation model using point maps to transfer 2D foundation model priors to 3D understanding, achieving strong performance across diverse 3D tasks with only geometric inputs.


<details>
  <summary>Details</summary>
Motivation: Address the scarcity of pretrained priors and limited data in 3D representation learning by leveraging rich 2D foundation models through point maps that preserve 3D geometry in a 2D-compatible format.

Method: Uses point maps (3D coordinates on 2D grid) with view-to-scene alignment and POMA-JEPA architecture for geometrically consistent features across views, pretrained on ScenePoint dataset of 6.5K room-level scenes and 1M 2D images.

Result: POMA-3D serves as strong backbone for specialist and generalist 3D understanding, benefiting 3D QA, embodied navigation, scene retrieval, and localization using only geometric inputs (3D coordinates).

Conclusion: POMA-3D explores point maps as an effective bridge between 2D foundation models and 3D scene understanding, overcoming data scarcity and pretraining limitations in 3D representation learning.
...
TL;DR: Erase to Retain is a medical image segmentation unlearning framework that uses teacher-student distillation with LoRA modules to selectively remove knowledge while preserving overall performance.
...
Method: Teacher-student distillation with LoRA-constrained subspace updates, featuring strong unlearning phase (adversarial optimization) and gentle restoration phase (head-only refinement).

Result: On ISIC segmentation: forget-set IoU reduced from 0.875 to 0.509 while maintaining retain/validation performance (0.647-0.677 IoU). On ISIC classification: forget accuracy dropped from 87.0% to 64.1% while retain accuracy improved from 83.9% to 90.6%.

Conclusion: LoRA-based subspace unlearning provides practical, controllable, and reversible unlearning for medical image analysis, enabling sensitive data forgetting while preserving utility.
...
TL;DR: Using generative AI to create synthetic smoke datasets for wildfire detection, with domain adaptation techniques to bridge synthetic-real data gaps.
...
Method: Leverage generative AI to synthesize annotated smoke datasets, apply unsupervised domain adaptation, and use style transfer, GANs, and image matting to enhance realism.

Result: Generated comprehensive synthetic smoke dataset and explored domain adaptation methods to improve model performance on real-world data.

Conclusion: Generative approaches can overcome data scarcity in wildfire detection and enable more accurate, scalable models through improved synthetic data realism.
...
TL;DR: This paper introduces SAM2S, a foundation model that enhances SAM2 for surgical interactive video object segmentation, addressing domain gaps and long-term tracking challenges in surgical scenarios through novel memory mechanisms and learning strategies.
...
Method: Proposed SAM2S with three key components: (1) DiveMem - trainable diverse memory mechanism for robust long-term tracking, (2) temporal semantic learning for instrument understanding, and (3) ambiguity-resilient learning to mitigate annotation inconsistencies across multi-source datasets.
...
Conclusion: SAM2S effectively addresses surgical iVOS challenges through specialized memory mechanisms and learning strategies, demonstrating substantial performance improvements while maintaining real-time capabilities and generalization.
...
Method: Two-stage Faster R-CNN with enhanced Balanced Group Softmax framework and metric learning for feature embeddings, using k-Nearest Neighbors for classification.

Result: Achieved new state-of-the-art performance with 24.5% mAP on LVISv1 dataset, surpassing previous benchmark of 24.0%.

Conclusion: The proposed methods effectively mitigate class imbalance in long-tailed object detection and improve performance on rare categories.
...
Result: AGU produces high-quality images in real time from low-quality, low-resolution input and outperforms state-of-the-art methods for low-light use cases.

Conclusion: The proposed AGU method effectively addresses the limitations of guided methods for low-light images and demonstrates superior performance in real-time upscaling.
...
TL;DR: SAM 3D is a generative model for single-image 3D object reconstruction that predicts geometry, texture, and layout, achieving superior performance on natural images with occlusion and scene clutter through a human-in-the-loop annotation pipeline and multi-stage training.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of 3D reconstruction from single images in natural scenes where occlusion, clutter, and contextual cues are common, overcoming the 3D data scarcity problem.
...
Result: Achieves significant gains over recent work with at least 5:1 win rate in human preference tests on real-world objects and scenes, breaking the 3D "data barrier".

Conclusion: SAM 3D provides state-of-the-art 3D reconstruction from single images in challenging real-world conditions, with planned release of code, model weights, demo, and a new benchmark.
...
Method: Proposes TRIM with two strategies: 1) temporal trimming using a lightweight selector model to reduce denoising trajectories by selecting high-quality candidates, and 2) spatial trimming via instance mask denoising to prune redundant background Gaussian primitives.
...
Conclusion: TRIM provides an effective post-training solution to accelerate 3D Gaussian diffusion models through trajectory reduction and instance pruning, enabling faster inference without compromising output quality.
...
Conclusion: The proposed framework effectively mitigates multi-hierarchy conflicts and class imbalance, demonstrating superior performance and generalizability in 3D hierarchical semantic segmentation.
...
Result: Outperforms state-of-the-art baselines (EPG, EPSD) at high sparsity levels on CIFAR-10, CIFAR-100, and TinyImageNet benchmarks, with better computational efficiency than iterative methods like COLT.
...
TL;DR: The paper critically analyzes Cambrian-S's spatial supersensing benchmarks and methods, showing they can be solved with simple baselines and exploit benchmark shortcuts rather than genuine spatial cognition.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether Cambrian-S's benchmarks and methods truly measure spatial supersensing capabilities in video world models, or if they rely on shortcuts and simple heuristics.

Method: Introduced NoSense baseline using bag-of-words SigLIP model without temporal structure, and created VSC-Repeat benchmark by concatenating videos with themselves to test shortcut exploitation.

Result: NoSense achieved 95% accuracy on VSR benchmark without spatial cognition, and VSC-Repeat collapsed Cambrian-S's accuracy from 42% to 0%, revealing reliance on benchmark shortcuts.

Conclusion: Current VSI-Super benchmarks don't reliably measure spatial supersensing, and Cambrian-S's methods exploit shortcuts rather than demonstrating robust spatial supersensing capabilities.
...
Method: PartUV combines semantic part decomposition from PartField with geometric heuristics in a top-down recursive framework, ensuring distortion below user threshold while minimizing chart count. It integrates parameterization/packing algorithms and handles non-manifold meshes with parallelization.
...
Conclusion: PartUV provides an effective solution for UV unwrapping challenging AI-generated meshes by leveraging part-based decomposition and geometric optimization, enabling new applications in computer graphics.
...
TL;DR: TriDiff-4D is a diffusion-based pipeline that generates high-quality 4D avatars from text by combining 3D structure and motion priors, enabling skeleton-driven animation with temporal consistency and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Address limitations in existing 4D generative methods including temporal/geometric inconsistencies, motion irregularities, high computational costs, and limited control over dynamics.

Method: Auto-regressive pipeline using diffusion-based triplane re-posing: generates canonical 3D avatar and motion sequence from text, then animates avatar using second diffusion model to support arbitrarily long 4D generation.
...
Conclusion: TriDiff-4D enables efficient, high-quality 4D avatar generation with superior temporal consistency, motion accuracy, and visual fidelity compared to previous approaches.
...
TL;DR: SceneDesigner enables accurate 9D pose control for multiple objects in image generation using a branched network, CNOCS map representation, and specialized training strategies.
...
Conclusion: SceneDesigner achieves accurate and flexible multi-object 9-DoF pose control while maintaining high image quality, enabling customized pose manipulation for reference subjects.
...
<details>
  <summary>Details</summary>
Motivation: The rapid progress in generative video models like Veo-3 has created a need for systematic and reliable evaluation of their reasoning capabilities.

Method: Built from synthetic and real-world image sequences, the benchmark provides diverse answer-verifiable tasks that are reproducible, scalable, and unambiguous.

Result: Evaluations of six state-of-the-art video models show clear dimension-wise differences in reasoning abilities, with comparisons to image models, hallucination analysis, and study of video duration effects on Chain-of-Frames reasoning.

Conclusion: V-ReasonBench offers a unified framework for measuring video reasoning and aims to support development of models with more reliable, human-aligned reasoning skills.
...
TL;DR: VANS is a model that uses reinforcement learning to align a Vision-Language Model with a Video Diffusion Model for Video-Next-Event Prediction, shifting from text-based to video-based answers for more intuitive procedural learning.


<details>
  <summary>Details</summary>
Motivation: Video's capacity to demonstrate physical-world information that is difficult to convey through language alone, and the underutilized opportunity to extend video as a new answer modality for Next-Event Prediction tasks.

Method: VANS leverages reinforcement learning with Joint-GRPO to orchestrate a Vision-Language Model and Video Diffusion Model as a unit, optimizing both to produce accurate captions and faithful video generation.

Result: VANS achieves state-of-the-art performance on procedural and predictive benchmarks for both video event prediction and visualization.

Conclusion: The proposed VANS model successfully addresses the Video-Next-Event Prediction task by aligning multimodal models through reinforcement learning, enabling more intuitive video-based answers for procedural learning and creative exploration.
...
TL;DR: A dual-mode thinking approach for visual language models that automatically switches between fast and slow thinking modes based on task difficulty, improving efficiency while maintaining performance.
...
Conclusion: The proposed simple RL approach enables efficient cognitive resource allocation in VLMs, allowing automatic switching between fast and slow thinking modes depending on task difficulty.
...
<details>
  <summary>Details</summary>
Motivation: Current LMM training pipelines depend on human-curated data or external reward models, limiting autonomy and scalability. The goal is to improve reasoning capabilities in a purely unsupervised fashion without annotated data or reward distillation.

Method: Proposes EvoLMM framework with two cooperative agents from a single backbone model: Proposer generates diverse image-grounded questions, and Solver solves them through internal consistency. Learning proceeds through continuous self-rewarding process without ground-truth or human judgments.

Result: Using Qwen2.5-VL as base model, EvoLMM achieves consistent gains up to ~3% on multimodal math-reasoning benchmarks (ChartQA, MathVista, MathVision) using only raw training images.

Conclusion: EvoLMM provides a simple yet effective approach for self-improving LMMs in fully-unsupervised fashion, serving as a solid baseline for future research.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods rely on accurate ground-truth camera and human poses for reconstruction, but performance significantly degrades when pose estimates are noisy. This limits practical applicability.

Method: Introduces NoPo-Avatar which reconstructs avatars solely from images without any pose input, removing dependence on potentially noisy human pose estimates.
...
Conclusion: NoPo-Avatar provides a more robust and widely applicable solution for 3D human avatar reconstruction by eliminating the dependency on pose input, making it resilient to noisy pose estimates.
...
TL;DR: Dataset distillation method for training linear probes on pre-trained vision models, outperforming real-image baselines and generalizing across different pre-trained models.


<details>
  <summary>Details</summary>
Motivation: Existing dataset distillation methods focus on training from scratch, but modern vision approaches use pre-trained models. Need distillation methods optimized for training linear probes on top of pre-trained feature extractors.

Method: Linear Gradient Matching - optimizes synthetic images so they induce similar gradients in linear classifiers as real data when passed through pre-trained feature extractors.

Result: Synthetic data outperforms all real-image baselines, generalizes across pre-trained models (e.g., CLIP probe trained with DINO-distilled data), effective for fine-grained classification and model interpretability.

Conclusion: Proposed method provides valuable tool for model interpretability and enables efficient training of linear probes on pre-trained vision models with distilled datasets.
...
TL;DR: MiMo-Embodied is the first cross-embodied foundation model achieving state-of-the-art performance in both Autonomous Driving and Embodied AI across 29 benchmarks, demonstrating strong positive transfer between these domains.


<details>
  <summary>Details</summary>
Motivation: To create a unified foundation model that can integrate and excel in both Autonomous Driving and Embodied AI domains, exploring the potential for positive transfer between these related fields.
...
Conclusion: Autonomous Driving and Embodied AI domains exhibit strong positive transfer and mutually reinforce each other through proper training methodologies, enabling superior performance across both fields.
...
TL;DR: Robots use peering motion like animals to create synthetic aperture sensing, overcoming occlusion by blurring foreground obstacles and focusing on background objects for better scene understanding.


<details>
  <summary>Details</summary>
Motivation: To overcome the fundamental limitation of partial occlusion in robotic vision where foreground obstacles obscure background information in conventional cameras with large depth of field.

Method: Robot executes peering motion to create wide synthetic aperture, then computationally integrates captured images to synthesize images with extremely shallow depth of field that blurs occluders while focusing on background.

Result: Enables real-time, high-resolution perception across spectral bands, restores basic scene understanding and empowers advanced visual reasoning in multimodal models that fail with conventional occluded imagery.

Conclusion: Peering motions for synthetic aperture sensing are robust to occlusion, computationally efficient, deployable on any mobile robot, and bridge animal behavior with robotics for advanced scene understanding in complex environments.
...
TL;DR: Codec2Vec is the first speech representation learning framework using discrete audio codec units, achieving competitive performance while reducing storage by 16.5x and training time by 2.3x.


<details>
  <summary>Details</summary>
Motivation: To explore neural audio codecs as universal acoustic feature extractors for broader speech processing tasks, leveraging their advantages in efficiency and privacy.
...
Conclusion: Codec2Vec demonstrates that discrete audio codec units can serve as effective speech representations with significant efficiency gains in storage and training time.
...
### [139] [The Subtle Art of Defection: Understanding Uncooperative Behaviors in LLM based Multi-Agent Systems](https://arxiv.org/abs/2511.15862)
*Devang Kulshreshtha,Wanyu Du,Raghav Jain,Srikanth Doss,Hang Su,Sandesh Swamy,Yanjun Qi*

Main category: cs.MA

TL;DR: A framework for simulating uncooperative behaviors in LLM-based multi-agent systems that can cause system collapse, with 96.7% accuracy in generating realistic behaviors and showing cooperative agents maintain perfect stability while uncooperative ones cause rapid collapse.
...
Method: Game theory-based taxonomy of uncooperative behaviors and a multi-stage simulation pipeline that dynamically generates and refines behaviors as agent states evolve, evaluated in collaborative resource management settings.

Result: Framework achieves 96.7% accuracy in generating realistic uncooperative behaviors. Cooperative agents maintain 100% survival over 12 rounds with 0% resource overuse, while uncooperative behaviors cause system collapse within 1-7 rounds.
...
TL;DR: This paper introduces GResilience framework for balancing resilience and greenness in Online Collaborative AI Systems (OL-CAIS) that learn online with humans, using multi-objective optimization, game theory, and reinforcement learning to optimize recovery from disruptions while minimizing energy impact.


<details>
  <summary>Details</summary>
Motivation: OL-CAIS systems are vulnerable to disruptive events that degrade performance, creating a trade-off between maintaining resilience (quick recovery) and greenness (energy efficiency). Decision-makers need strategies to balance these competing objectives during system disruptions.

Method: Modeled OL-CAIS behavior through three operational states (steady, disruptive, final). Developed GResilience framework with three recovery strategies: multi-objective optimization (one-agent), game-theoretic decision-making (two-agent), and reinforcement learning (RL-agent). Designed measurement framework to quantify resilience and greenness.

Result: Resilience model successfully captured performance transitions during disruptions. GResilience policies improved green recovery by shortening recovery time, stabilizing performance, and reducing human dependency. RL-agent policies achieved strongest results with marginal CO2 increase. Containerization cut CO2 emissions by half.

Conclusion: The research provides effective models, metrics, and policies for ensuring green recovery of OL-CAIS, with RL-based approaches showing the best performance in balancing resilience and energy efficiency, though containerization offers significant energy savings.
...
### [141] [AccelOpt: A Self-Improving LLM Agentic System for AI Accelerator Kernel Optimization](https://arxiv.org/abs/2511.15915)
*Genghan Zhang,Shaowei Zhu,Anjiang Wei,Zhenyu Song,Allen Nie,Zhen Jia,Nandita Vijaykumar,Yida Wang,Kunle Olukotun*

Main category: cs.LG

TL;DR: AccelOpt is a self-improving LLM agent that autonomously optimizes AI accelerator kernels without expert knowledge, achieving significant performance improvements on AWS Trainium accelerators while being highly cost-effective.
...
Method: Uses iterative generation with an optimization memory that curates experiences from previously encountered slow-fast kernel pairs, and evaluates on NKIBench - a new benchmark suite of AWS Trainium accelerator kernels from real-world LLM workloads.

Result: Boosts average percentage of peak throughput from 49% to 61% on Trainium 1 and from 45% to 59% on Trainium 2. Matches Claude Sonnet 4's kernel improvements while being 26× cheaper using open-source models.

Conclusion: AccelOpt demonstrates effective self-improving capability for autonomous kernel optimization, achieving significant performance gains with high cost-effectiveness on emerging AI accelerators.
...
### [142] [Chain of Summaries: Summarization Through Iterative Questioning](https://arxiv.org/abs/2511.15719)
*William Brach,Lukas Galke Poech*

Main category: cs.AI

TL;DR: CoS method generates information-dense summaries for web content using iterative refinement inspired by Hegel's dialectical method, improving LLM performance while reducing token usage.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of LLMs struggling with web content due to unfriendly formats and context length limitations by creating digestible, general-purpose summaries.

Method: Chain of Summaries (CoS) - iterative refinement process using thesis (initial summary), antithesis (identifying limitations through questioning), and synthesis (general-purpose summary).

Result: Outperforms zero-shot LLM baselines by up to 66% and specialized summarization methods by up to 27% on TriviaQA, TruthfulQA, and SQUAD datasets. Summaries yield higher Q&A performance with fewer tokens.

Conclusion: CoS provides an effective solution for making web content more accessible to LLMs while maintaining human oversight capabilities, offering better performance than existing methods.
...
### [143] [Step-Audio-R1 Technical Report](https://arxiv.org/abs/2511.15848)
...
TL;DR: Step-Audio-R1 is the first audio reasoning model that successfully enables reasoning in audio domains through Modality-Grounded Reasoning Distillation, outperforming Gemini 2.5 Pro and matching Gemini 3 Pro across audio understanding benchmarks.


<details>
  <summary>Details</summary>
Motivation: Address the perplexing phenomenon where audio language models perform better without reasoning, questioning whether audio intelligence can truly benefit from deliberate thinking like text and vision models do.
...
TL;DR: JudgeBoard is a novel evaluation pipeline that directly queries models to assess answer correctness without comparison methods. It introduces MAJ (Multi-Agent Judging), a multi-agent framework using SLMs that achieves LLM-level judgment accuracy through collaborative deliberation.


<details>
  <summary>Details</summary>
Motivation: Current LLM-as-a-judge frameworks rely on indirect answer comparisons with predefined metrics, which are difficult to automate and offer limited support for fine-grained reasoning evaluation. There's a need for direct assessment methods that can scale effectively.

Method: Proposed JudgeBoard pipeline for direct correctness assessment without answer comparisons, focusing on mathematical and science/commonsense reasoning. Introduced MAJ framework using multiple interacting SLMs with distinct reasoning profiles for collaborative deliberation.

Result: Significant performance gap between SLMs and LLMs in isolated judging tasks. MAJ framework substantially improves SLM reliability and consistency. On MATH dataset, MAJ with smaller models performs comparably or better than larger models.
...
### [145] [CARE-RAG - Clinical Assessment and Reasoning in RAG](https://arxiv.org/abs/2511.15994)
*Deepthi Potluri,Aby Mammen Mathew,Jeffrey B DeWitt,Alexander L. Rasgon,Yide Hao,Junyuan Hong,Ying Ding*

Main category: cs.AI

TL;DR: LLMs struggle with clinical reasoning even when provided correct evidence, requiring evaluation of reasoning quality alongside retrieval accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the gap between evidence retrieval and correct reasoning in LLMs, especially in clinical settings where outputs must follow structured protocols.

Method: Used Written Exposure Therapy guidelines as testbed, evaluated model responses to clinician-vetted questions, and proposed evaluation framework measuring accuracy, consistency, and fidelity of reasoning.

Result: Errors persist even with authoritative passages provided; RAG can constrain outputs but safe deployment requires rigorous reasoning assessment.

Conclusion: Safe deployment of LLMs in clinical settings requires assessing reasoning quality as rigorously as retrieval accuracy, not just providing correct evidence.
...
### [146] [SpellForger: Prompting Custom Spell Properties In-Game using BERT supervised-trained model](https://arxiv.org/abs/2511.16018)
...
Method: Uses a supervised-trained BERT model to interpret natural language prompts, mapping them to spell prefabs with balanced parameters (damage, cost, effects) in Unity Game Engine with Python backend.
...
### [147] [OpenMMReasoner: Pushing the Frontiers for Multimodal Reasoning with an Open and General Recipe](https://arxiv.org/abs/2511.16334)
*Kaichen Zhang,Keming Wu,Zuhao Yang,Kairui Hu,Bin Wang,Ziwei Liu,Xingxuan Li,Lidong Bing*

Main category: cs.AI

TL;DR: OpenMMReasoner is a transparent two-stage training recipe for multimodal reasoning that uses supervised fine-tuning and reinforcement learning, achieving 11.6% improvement over baseline models.
...
Conclusion: The method establishes a solid empirical foundation for future large-scale multimodal reasoning research, with all code, pipeline, and data open-sourced.
...
### [148] [TOFA: Training-Free One-Shot Federated Adaptation for Vision-Language Models](https://arxiv.org/abs/2511.16423)
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of iterative federated VLM adaptation (high communication costs, security risks) and challenges of existing one-shot methods (insufficient multimodal exploitation, poor data heterogeneity handling, additional training requirements).

Method: Uses dual visual and textual pipelines: hierarchical Bayesian model for personalized class prototypes in visual pipeline, global alignment of text prompts in textual pipeline, with adaptive weight calibration to balance personalization and robustness.
...
Conclusion: TOFA provides an efficient, lightweight solution for federated VLM adaptation that fully leverages multimodal features while handling data heterogeneity without requiring additional training resources.
...
### [149] [D-GARA: A Dynamic Benchmarking Framework for GUI Agent Robustness in Real-World Anomalies](https://arxiv.org/abs/2511.16590)
*Sen Chen,Tong Zhao,Yi Bin,Fei Ma,Wenqi Shao,Zheng Wang*

Main category: cs.AI

TL;DR: D-GARA is a dynamic benchmarking framework that evaluates Android GUI agents' robustness against real-world anomalies like permission dialogs and system warnings, revealing significant performance degradation in current agents.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agent datasets are static and idealized, failing to capture real-world complexity and unpredictability, particularly the presence of common anomalies that disrupt agent performance.

Method: Proposed D-GARA framework introduces diverse real-world anomalies (permission dialogs, battery warnings, update prompts) and constructs an annotated benchmark with embedded anomalies in Android applications.

Result: Comprehensive experiments show substantial performance degradation in state-of-the-art GUI agents when exposed to anomaly-rich environments, demonstrating the vulnerability of current approaches.

Conclusion: D-GARA highlights the need for robustness-aware learning in GUI agents and provides a modular, extensible framework for evaluating agent resilience against real-world anomalies.
...
### [150] [How Modality Shapes Perception and Reasoning: A Study of Error Propagation in ARC-AGI](https://arxiv.org/abs/2511.15717)
*Bo Wen,Chen Wang,Erhan Bilal*

Main category: cs.AI

TL;DR: The paper analyzes how different text and image modalities affect perception and execution in ARC-AGI tasks, finding that modality choice creates perceptual bottlenecks and that combining modalities improves performance.


<details>
  <summary>Details</summary>
Motivation: To understand how different encoding modalities (text vs. images) affect model perception in ARC-AGI tasks and separate instruction errors from execution errors, since current systems lack principled accounts of how encodings shape perception.
...
### [151] [FOOTPASS: A Multi-Modal Multi-Agent Tactical Context Dataset for Play-by-Play Action Spotting in Soccer Broadcast Videos](https://arxiv.org/abs/2511.16183)
*Jeremie Ochin,Raphael Chekroun,Bogdan Stanciulescu,Sotiris Manitsaris*

Main category: cs.AI

TL;DR: FOOTPASS is the first benchmark dataset for play-by-play action spotting in soccer that combines computer vision outputs with tactical knowledge to generate reliable play-by-play data streams.


<details>
  <summary>Details</summary>
Motivation: Current action recognition methods are insufficient for reliable play-by-play data extraction, while tactical modeling research has advanced but requires game-state data. This creates a need to leverage tactical knowledge as a prior to support computer-vision predictions for more automated play-by-play extraction.

Method: Multi-modal, multi-agent approach that integrates outputs from computer-vision tasks (tracking, identification) with prior knowledge of soccer tactical regularities over long time horizons for player-centric action spotting.

Result: Introduction of FOOTPASS dataset - the first benchmark for play-by-play action spotting over entire soccer matches in a tactical context.

Conclusion: The dataset enables development of methods that generate reliable play-by-play data streams essential for data-driven sports analytics by combining computer vision with soccer tactical knowledge.
...
### [152] [UniUltra: Interactive Parameter-Efficient SAM2 for Universal Ultrasound Segmentation](https://arxiv.org/abs/2511.15771)
...
<details>
  <summary>Details</summary>
Motivation: SAM2 performs poorly on ultrasound images due to domain gaps, requiring efficient adaptation methods for clinical use in resource-constrained environments.

Method: Proposes context-edge hybrid adapter (CH-Adapter) for parameter-efficient fine-tuning and deep-supervised knowledge distillation (DSKD) to compress the model for clinical deployment.
...
Conclusion: UniUltra provides an effective solution for universal ultrasound segmentation that maintains performance while being highly suitable for practical clinical deployment.
...
### [153] [Weakly Supervised Segmentation and Classification of Alpha-Synuclein Aggregates in Brightfield Midbrain Images](https://arxiv.org/abs/2511.16268)
*Erwan Dereure,Robin Louiset,Laura Parkkinen,David A Menassa,David Holcman*

Main category: eess.IV

TL;DR: Automated pipeline using weakly supervised segmentation and ResNet50 to classify alpha-synuclein aggregates in Parkinson's disease histopathology images with 80% balanced accuracy.


<details>
  <summary>Details</summary>
Motivation: To better understand spatial organization of alpha-synuclein aggregates in Parkinson's disease through automated analysis of immunohistochemistry images, addressing challenges of manual pathology diagnostics.

Method: Developed automated image processing pipeline using weakly supervised segmentation robust to immunohistochemical labeling variability, with ResNet50 classifier to segment and classify aggregates in whole-slide images.
...
Conclusion: Framework enables large-scale characterization of spatial distribution and heterogeneity of alpha-synuclein aggregates, facilitating investigation of relationships with surrounding cells like microglia and astrocytes.
...
### [154] [QueryGym: A Toolkit for Reproducible LLM-Based Query Reformulation](https://arxiv.org/abs/2511.15996)
*Amin Bigdeli,Radin Hamidi Rad,Mert Incesu,Negar Arabzadeh,Charles L. A. Clarke,Ebrahim Bagheri*

Main category: cs.IR

TL;DR: QueryGym is a lightweight Python toolkit for LLM-based query reformulation that provides a unified framework for implementing, executing, and comparing different methods to address the lack of consistent implementations in the field.


<details>
  <summary>Details</summary>
Motivation: There is no unified toolkit for LLM-based query reformulation methods, which hinders fair comparison, rapid experimentation, consistent benchmarking, and reliable deployment despite recent work showing notable improvements in retrieval effectiveness.

Method: Provides a Python API for applying diverse LLM-based methods, retrieval-agnostic interface supporting integration with backends like Pyserini and PyTerrier, centralized prompt management with versioning and metadata tracking, and built-in support for benchmarks like BEIR and MS MARCO.

Result: QueryGym addresses the gap by offering a completely open-source extensible implementation that enables researchers to implement, execute, and compare LLM-based reformulation methods consistently.

Conclusion: QueryGym provides a unified framework that facilitates fair comparison, rapid experimentation, and reliable deployment of LLM-based query reformulation methods, making it publicly available to all researchers.
...
### [155] [Music Recommendation with Large Language Models: Challenges, Opportunities, and Evaluation](https://arxiv.org/abs/2511.16478)
*Elena V. Epure,Yashar Deldjoo,Bruno Sguerra,Markus Schedl,Manuel Moussallam*

Main category: cs.IR

TL;DR: The paper argues that traditional evaluation methods for Music Recommender Systems (MRS) are inadequate for LLM-based systems and proposes a new evaluation framework that addresses the unique challenges and opportunities of LLM-driven MRS.


<details>
  <summary>Details</summary>
Motivation: Traditional MRS evaluation focuses on retrieval accuracy but fails to address what makes recommendations truly good. LLMs disrupt this paradigm by being generative rather than ranking-based, introducing new challenges like hallucinations and knowledge cutoffs while enabling natural-language interaction.

Method: The paper reviews how LLMs reshape user modeling, item modeling, and natural-language recommendation in music, examines NLP evaluation practices, and synthesizes insights to outline structured success and risk dimensions for LLM prompting in MRS.

Result: The analysis provides a cross-disciplinary perspective on evaluation methodologies, highlighting how LLM-specific challenges require rethinking traditional train/test protocols and accuracy metrics.

Conclusion: The shift toward LLM-driven MRS necessitates a fundamental rethinking of evaluation practices, moving beyond traditional accuracy metrics to address the unique characteristics, challenges, and opportunities presented by generative models in music recommendation.
...
### [156] [The Oracle and The Prism: A Decoupled and Efficient Framework for Generative Recommendation Explanation](https://arxiv.org/abs/2511.16543)
*Jiaheng Zhang,Daqiang Zhang*

Main category: cs.IR

TL;DR: Prism is a decoupled framework that separates recommendation ranking from explanation generation, using knowledge distillation to create efficient student models that outperform larger teacher models in faithfulness and personalization while being much faster.
...
Method: Proposes Prism framework that separates recommendation into ranking and explanation stages, using a teacher LLM (FLAN-T5-XXL) to generate explanatory knowledge and a compact student model (BART-Base) to synthesize personalized explanations through knowledge distillation.

Result: The 140M-parameter Prism model significantly outperforms its 11B-parameter teacher in human evaluations of faithfulness and personalization, achieves 24x speedup and 10x memory reduction during inference.

Conclusion: Decoupling recommendation and explanation stages with targeted distillation provides an efficient and effective pathway to high-quality explainable recommendation.
...
TL;DR: A novel framework for hardening LLM system prompts using shield appending, formalized as utility-constrained optimization to minimize leakage while preserving task utility.
...
Method: Uses LLM-as-optimizer to search for optimal SHIELDs (protective textual layers) that minimize leakage from adversarial attacks while maintaining semantic fidelity above utility threshold.
...
Conclusion: Presents a practical, black-box optimization approach for developing robust, utility-aware LLM security defenses that are lightweight and API-compatible.
